...
1. Download the runtime for your OS (extracts into `$RECALL_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.recall/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$RECALL_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.recall/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Recall config file (`$RECALL_CONFIG_PATH`, default `~/.tryrecall/recall-agents.json`):
