...
1. Download the runtime for your OS (extracts into `$STEELENGINE_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.steelengine/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$STEELENGINE_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.steelengine/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active SteelEngine config file (`$STEELENGINE_CONFIG_PATH`, default `~/.steelengine/steelengine.json`):
