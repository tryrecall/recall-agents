...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${STEELENGINE_CONFIG_PATH:-${STEELENGINE_STATE_DIR:-$HOME/.steelengine}/steelengine.json}"
...
```text
${STEELENGINE_STATE_DIR:-$HOME/.steelengine}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with steelengine message send if route provided.
```
