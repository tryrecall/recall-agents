...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${RECALL_CONFIG_PATH:-${RECALL_STATE_DIR:-$HOME/.recall}/recall.json}"
...
```text
${RECALL_STATE_DIR:-$HOME/.recall}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with recall message send if route provided.
```
