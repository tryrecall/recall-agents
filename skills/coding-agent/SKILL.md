...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.recall`, `$RECALL_STATE_DIR`, or active Recall state dirs.
...
- Worker must send completion/failure via `recall message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/recall`; use an isolated checkout.
...
recall message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use recall system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t recall-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
<notification block>
EOF
