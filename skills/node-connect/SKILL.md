---
name: node-connect
description: "Diagnose SteelEngine Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify SteelEngine is advertising that route, then fix pairing/auth.
...
- whether `steelengine devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `steelengine qr --json`. It uses the same setup-code payload Android scans.

```bash
steelengine config get gateway.mode
steelengine config get gateway.bind
steelengine config get gateway.tailscale.mode
steelengine config get gateway.remote.url
steelengine config get gateway.auth.mode
steelengine config get gateway.auth.allowTailscale
steelengine config get plugins.entries.device-pair.config.publicUrl
steelengine qr --json
steelengine devices list
steelengine nodes status
```

If this SteelEngine instance is pointed at a remote gateway, also run:

```bash
steelengine qr --remote --json
```
...
`steelengine qr --json` success means:
...
If `steelengine qr --json` says `Gateway is only bound to loopback`:
...
- `steelengine devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run steelengine qr again, rescan, then approve the pending device pairing.`
