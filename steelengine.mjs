...
// Preserve Recall service environments during the migration. Canonical SteelEngine
// values always win when both names are present.
for (const [name, value] of Object.entries(process.env)) {
  if (!name.startsWith("RECALL_") || value === undefined) {
    continue;
  }
  const steelEngineName = `STEELENGINE_${name.slice("RECALL_".length)}`;
  if (process.env[steelEngineName] === undefined) {
    process.env[steelEngineName] = value;
  }
}
...
const ensureSupportedRuntimeVersion = () => {
  if (process.versions.bun) {
    process.stderr.write(
      "steelengine: the Bun runtime is unsupported because SteelEngine requires node:sqlite.\n" +
  ...
  process.stderr.write(
    `steelengine: Node.js ${SUPPORTED_NODE_RANGE} is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[steelengine] Failed to respawn launcher: ${
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.STEELENGINE_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    STEELENGINE_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["steelengine: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:steelengine/steelengine#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g steelengine@latest`.");
  return lines.join("\n");
};
...
const hasLauncherContainerTarget = (argv) => {
  if (normalizeLauncherMetadataValue(process.env.STEELENGINE_CONTAINER)) {
    return true;
  }
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.STEELENGINE_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.STEELENGINE_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.STEELENGINE_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "steelengine.json"), path.join(stateDir, "clawdbot.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".steelengine", "steelengine.json"),
    path.join(homeDir, ".steelengine", "clawdbot.json"),
    path.join(homeDir, ".clawdbot", "steelengine.json"),
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.STEELENGINE_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.STEELENGINE_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    ...
    process.stdout.write(
      commit ? `SteelEngine ${version} (${commit})\n` : `SteelEngine ${version}\n`,
    );
    return true;
  } catch {
    return false;
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.STEELENGINE_BUNDLED_VERSION) ?? "0.0.0";
}
