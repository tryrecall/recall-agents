...
describe("SQLite CLI maintenance ownership", () => {
  it("compacts after full CLI startup without retaining a config-health database handle", async () => {
    await withTempHome(
      ...
      { prefix: "steelengine-state-sqlite-cli-" },
    );
  }, 90_000);
    ...
    await withTempHome(
      async (tempHome) => {
        ...
        expect(`${result.stderr}\n${result.stdout}`).toContain(
          "outside the active SteelEngine state directory",
        );
        expect(fs.existsSync(externalStorePath)).toBe(false);
      },
      { prefix: "steelengine-session-sqlite-cli-" },
    );
  }, 90_000);
  ...
  it("rejects hard-linked SQLite sidecars before destructive maintenance", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        const externalWalPath = path.join(tempHome, "external-state", "steelengine-agent.sqlite-wal");
      ...
      { prefix: "steelengine-session-sqlite-sidecar-cli-" },
    );
  }, 90_000);
});
