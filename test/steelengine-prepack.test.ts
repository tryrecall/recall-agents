// SteelEngine prepack tests validate package prepack output.
...
import {
  ...
  runPrepackCommand,
} from "../scripts/steelengine-prepack.ts";
import { useAutoCleanupTempDirTracker } from "./helpers/temp-dir.js";

const tempDirs = useAutoCleanupTempDirTracker(afterEach);

describe("collectSourcePackWorkspaceDependencyErrors", () => {
  it("rejects the plain source pack that pnpm rewrites without bundling @steelengine/ai", () => {
    const rootDir = tempDirs.make("steelengine-source-pack-workspace-");
    ...
    const rootPackageJson = {
      ...
      name: "steelengine-source-pack-regression",
      version,
    };
    ...
    writeFileSync(
      ...
      `${JSON.stringify({ name: "@steelengine/ai", version }, null, 2)}\n`,
    );
    ...
    expect(packedPackageJson.dependencies?.["@steelengine/ai"]).toBe(version);
    expect(packedPackageJson.bundleDependencies).toBeUndefined();
    expect(existsSync(path.join(extractDir, "package", "node_modules", "@steelengine", "ai"))).toBe(
      false,
    );
    expect(collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {})).toEqual([
      "plain root packing cannot safely resolve @steelengine/ai from workspace:*: pnpm rewrites the workspace dependency to an exact version without bundling the package",
      "use `node scripts/package-steelengine-for-docker.mjs --allow-unreleased-changelog` for a self-contained source package; official npm release automation prepares and publishes @steelengine/ai separately",
    ]);
    expect(
      collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {
        STEELENGINE_PREPACK_PREPARED: "1",
      }),
    ).toEqual([]);
    expect(
      collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {
        ...
        STEELENGINE_OCM_WORKSPACE_DEPENDENCY_DIRS: aiDir,
      }),
    ).toEqual([]);
    expect(
      collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {
        npm_command: "pack",
        OCM_INTERNAL_NPM_BIN: path.join(rootDir, "scripts", "ocm-npm-workspace-deps.mjs"),
        STEELENGINE_OCM_WORKSPACE_DEPENDENCY_DIRS: rootDir,
      }),
    ).toHaveLength(2);
    expect(
      collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {
...
describe("resolvePrepackAllowUnreleasedChangelog", () => {
  it("requires an explicit non-publish opt-in", () => {
    for (const raw of [undefined, "", "0", "false"]) {
      expect(
        resolvePrepackAllowUnreleasedChangelog({
          STEELENGINE_PREPACK_ALLOW_UNRELEASED_CHANGELOG: raw,
        }),
      ).toBe(false);
    }
    ...
    expect(() =>
      resolvePrepackAllowUnreleasedChangelog({
        STEELENGINE_PREPACK_ALLOW_UNRELEASED_CHANGELOG: "yes",
      }),
    ).toThrow("invalid STEELENGINE_PREPACK_ALLOW_UNRELEASED_CHANGELOG: yes");
  });
});
...
describe("resolvePrepackBuildEnvironment", () => {
  it("pins one timestamp across package and Control UI builds", () => {
    ...
    expect(
      ...
      GIT_COMMIT: commit,
      STEELENGINE_BUILD_TIMESTAMP: "2026-07-10T12:34:56.000Z",
    });
    expect(
      resolvePrepackBuildEnvironment(
        { STEELENGINE_BUILD_TIMESTAMP: "2026-07-10T01:02:03.7Z" },
...
describe("resolvePrepackCommandTimeoutMs", () => {
  it("parses only positive integer environment timeouts", () => {
    ...
    expect(resolvePrepackCommandTimeoutMs({ STEELENGINE_PREPACK_COMMAND_TIMEOUT_MS: "" })).toBe(
      30 * 60 * 1000,
    );
    expect(resolvePrepackCommandTimeoutMs({ STEELENGINE_PREPACK_COMMAND_TIMEOUT_MS: "1234" })).toBe(
      1234,
    );

    for (const raw of ["nope", "10m", "1e3", "0", "-1", "9007199254740992"]) {
      expect(() =>
        resolvePrepackCommandTimeoutMs({ STEELENGINE_PREPACK_COMMAND_TIMEOUT_MS: raw }),
      ).toThrow(`invalid STEELENGINE_PREPACK_COMMAND_TIMEOUT_MS: ${raw}`);
    }
  });
});
