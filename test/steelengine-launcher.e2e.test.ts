// SteelEngine launcher E2E tests validate launcher process behavior.
...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = makeTempDir(fixtureRoots, "steelengine-launcher-");
  await fs.copyFile(
    path.resolve(process.cwd(), "steelengine.mjs"),
    path.join(fixtureRoot, "steelengine.mjs"),
  );
...
async function makeLauncherProbeFixture(
  ...
  const launcherPath = path.join(fixtureRoot, "steelengine.mjs");
  const launcher = await fs.readFile(launcherPath, "utf8");
  const bootstrapStart = "\nif (!waitingForCompileCacheRespawn) {";
  const bootstrapIndex = launcher.indexOf(bootstrapStart);
  if (bootstrapIndex < 0) {
    throw new Error("steelengine launcher bootstrap block was not found");
  }
  await fs.writeFile(
    launcherPath,
...
async function addGitMarker(fixtureRoot: string): Promise<void> {
  await fs.writeFile(path.join(fixtureRoot, ".git"), "gitdir: .git/worktrees/steelengine\n", "utf8");
}
...
async function addCompileCacheProbe(fixtureRoot: string): Promise<void> {
  await fs.writeFile(
    ...
    [
      ...
      '  `${module.getCompileCacheDir?.() ? "cache:enabled" : "cache:disabled"};respawn:${process.env.STEELENGINE_COMPILE_CACHE_DISABLED_RESPAWNED ?? "0"}`',
      ");",
    ].join("\n"),
    "utf8",
  );
...
function launcherEnv(extra: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  ...
  delete env.STEELENGINE_BUNDLED_PLUGINS_DIR;
  delete env.STEELENGINE_CONFIG_PATH;
  delete env.STEELENGINE_DISABLE_BUNDLED_PLUGINS;
  delete env.STEELENGINE_HOME;
  delete env.STEELENGINE_STATE_DIR;
  delete env.STEELENGINE_TEST_TRUST_BUNDLED_PLUGINS_DIR;
...
describe("steelengine launcher", () => {
    ...
    for (const testCase of cases) {
      ...
      if (testCase.supported) {
        ...
        expect(result.stderr, testCase.version).toContain(
          `steelengine: Node.js >=22.22.3 <23, >=24.15.0 <25, or >=25.9.0 is required (current: v${testCase.version}).`,
        );
      }
    }
  });
  ...
  it("rejects Bun even when its Node compatibility version is new enough", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", pathToFileURL(mockRuntime).href, path.join(fixtureRoot, "steelengine.mjs")],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
    ...
    expect(result.stderr).toContain(
      "the Bun runtime is unsupported because SteelEngine requires node:sqlite",
    );
  });
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      'import "missing-steelengine-launcher-dep";\nexport {};\n',
      "utf8",
    );

    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "steelengine.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
    ...
    expect(result.stderr).toContain("missing-steelengine-launcher-dep");
  ...
  it("prints root version without importing the runtime entry", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "steelengine.mjs"), "--version"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
    ...
    expect(result.stdout).toBe("SteelEngine 1.2.3-test (abcdef0)\n");
  ...
  it("defers container-targeted root version to the runtime entry", async () => {
    ...
    await fs.writeFile(
      path.join(fixtureRoot, "package.json"),
      JSON.stringify({ name: "steelengine", version: "1.2.3-test" }),
      "utf8",
    );
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "steelengine.mjs"), "--container", "demo", "--version"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
    ...
    const envResult = spawnSync(
      ...
      {
        cwd: fixtureRoot,
        env: launcherEnv({ STEELENGINE_CONTAINER: "demo" }),
        encoding: "utf8",
      },
    );
  ...
  it("treats Bun direct optional import misses as direct launcher misses", async () => {
    const fixtureRoot = await makeLauncherProbeFixture(
      ...
      [
        ...
        "    { message: \"Cannot find module './nested.js' from '/pkg/steelengine/dist/entry.js'\" },",
        "    './dist/entry.js',",
        "  ),",
        "  sameSpecifierTransitive: isDirectModuleNotFoundError(",
        "    { message: \"Cannot find module './dist/entry.js' from '/pkg/steelengine/dist/entry.js'\" },",
        "    './dist/entry.js',",
        "  ),",
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "steelengine.mjs")], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
  ...
  it.runIf(process.env.STEELENGINE_TEST_BUN_LAUNCHER === "1" && hasBunRuntime())(
  ...
  it.each([
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "steelengine.mjs"), params.command, "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it.each(["doctor", "gateway", "models", "plugins", "sessions", "tasks"])(
    ...
    async (command) => {
      ...
      const result = spawnSync(
        process.execPath,
        [path.join(fixtureRoot, "steelengine.mjs"), command, "--help"],
        {
          cwd: fixtureRoot,
          env: launcherEnv(),
          encoding: "utf8",
  ...
  it("uses precomputed subcommand help with leading root options", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "steelengine.mjs"), "--profile", "work", "--no-color", "models", "-h"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it("defers precomputed subcommand help to the runtime entry when container env is set", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "steelengine.mjs"), "models", "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv({ STEELENGINE_CONTAINER: "demo" }),
        encoding: "utf8",
  ...
  it.each([
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "steelengine.mjs"), ...params.args],
      {
        cwd: fixtureRoot,
  ...
  it("defers root help to the runtime entry when plugin config can change help", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const configPath = path.join(fixtureRoot, "steelengine.json");
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "cli-startup-metadata.json"),
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "steelengine.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ STEELENGINE_CONFIG_PATH: configPath }),
      encoding: "utf8",
    });
  ...
  it("defers nodes help to the runtime entry when plugin config can change help", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "steelengine.mjs"), "nodes", "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv({ STEELENGINE_CONFIG_PATH: configPath }),
        encoding: "utf8",
  ...
  it("checks the STEELENGINE_HOME default config path before using precomputed root help", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const steelengineHome = path.join(fixtureRoot, "home");
    const configDir = path.join(steelengineHome, ".steelengine");
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "steelengine.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ STEELENGINE_HOME: steelengineHome }),
      encoding: "utf8",
    });
  ...
  it("checks legacy config candidates before using precomputed root help", async () => {
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "steelengine.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ HOME: home, STEELENGINE_HOME: undefined }),
      encoding: "utf8",
    });
  ...
  it("explains how to recover from an unbuilt source install", async () => {
    ...
    expect(result.stderr).toContain("github:steelengine/steelengine#<ref>");
  });
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          'process.title = "steelengine-launcher-sigterm-test-child";',
      ...
      const launcher = spawn(process.execPath, [path.join(fixtureRoot, "steelengine.mjs")], {
        cwd: fixtureRoot,
        env: launcherEnv({
          NODE_COMPILE_CACHE: path.join(fixtureRoot, ".node-compile-cache"),
        }),
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          'import { writeFileSync } from "node:fs";',
          `writeFileSync(${JSON.stringify(childInfoPath)}, JSON.stringify({ pid: process.pid }) + "\\n");`,
          'process.title = "steelengine-launcher-sigterm-ignore-test-child";',
          'process.on("SIGTERM", () => {});',
          "setInterval(() => {}, 1000);",
          "",
        ].join("\n"),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await addGitMarker(fixtureRoot);
      await addCompileCacheProbe(fixtureRoot);
      const linkParent = makeTempDir(fixtureRoots, "steelengine-launcher-link-");
      const linkedRoot = path.join(linkParent, "steelengine-linked");
      await fs.symlink(fixtureRoot, linkedRoot, "dir");

      const result = spawnSync(process.execPath, [path.join(linkedRoot, "steelengine.mjs")], {
        cwd: linkParent,
        env: launcherEnv({
  ...
  it("scopes packaged launcher compile cache inside configured cache roots", async () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join(".node-compile-cache", "steelengine", "2026.4.29"));
  });

  it("falls back to the default packaged launcher compile cache when NODE_COMPILE_CACHE is empty", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const runCwd = makeTempDir(fixtureRoots, "steelengine-launcher-cwd-");
    const tmpRoot = makeTempDir(fixtureRoots, "steelengine-launcher-tmp-");
    await fs.writeFile(path.join(fixtureRoot, "package.json"), '{"version":"2026.4.29"}\n');
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      [
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain(path.join("node-compile-cache", "steelengine", "2026.4.29"));
    expect(result.stdout).not.toContain(path.join(runCwd, "steelengine"));
  });
