...
const LIVE_TEST_TRIGGER_ENV_KEYS = ["LIVE", "STEELENGINE_LIVE_TEST", "STEELENGINE_LIVE_GATEWAY"] as const;
const HERMETIC_TEST_ENV_KEYS = [
  ...LIVE_TEST_TRIGGER_ENV_KEYS,
  "STEELENGINE_LIVE_USE_REAL_HOME",
  "STEELENGINE_BUNDLED_PLUGINS_DIR",
  "STEELENGINE_TEST_TRUST_BUNDLED_PLUGINS_DIR",
...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.STEELENGINE_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    ...
    { key: "STEELENGINE_TEST_FAST", value: process.env.STEELENGINE_TEST_FAST },
    {
      key: "STEELENGINE_STRICT_FAST_REPLY_CONFIG",
      value: process.env.STEELENGINE_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "STEELENGINE_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.STEELENGINE_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "STEELENGINE_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.STEELENGINE_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "STEELENGINE_STATE_DIR", value: process.env.STEELENGINE_STATE_DIR },
    { key: "STEELENGINE_CONFIG_PATH", value: process.env.STEELENGINE_CONFIG_PATH },
    { key: "STEELENGINE_GATEWAY_PORT", value: process.env.STEELENGINE_GATEWAY_PORT },
    { key: "STEELENGINE_BRIDGE_ENABLED", value: process.env.STEELENGINE_BRIDGE_ENABLED },
    { key: "STEELENGINE_BRIDGE_HOST", value: process.env.STEELENGINE_BRIDGE_HOST },
    { key: "STEELENGINE_BRIDGE_PORT", value: process.env.STEELENGINE_BRIDGE_PORT },
    { key: "STEELENGINE_CANVAS_HOST_PORT", value: process.env.STEELENGINE_CANVAS_HOST_PORT },
    { key: "STEELENGINE_TEST_HOME", value: process.env.STEELENGINE_TEST_HOME },
    { key: "STEELENGINE_AGENT_DIR", value: process.env.STEELENGINE_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-test-home-"));

  setTestEnvValue("HOME", tempHome);
  setTestEnvValue("USERPROFILE", tempHome);
  setTestEnvValue("STEELENGINE_TEST_HOME", tempHome);
  setTestEnvValue("STEELENGINE_TEST_FAST", "1");
  setTestEnvValue("STEELENGINE_STRICT_FAST_REPLY_CONFIG", "1");
  deleteTestEnvValue("STEELENGINE_ALLOW_SLOW_REPLY_TESTS");
  ...
  deleteTestEnvValue("STEELENGINE_AGENT_DIR");
  ...
  deleteTestEnvValue("STEELENGINE_BRIDGE_ENABLED");
  deleteTestEnvValue("STEELENGINE_BRIDGE_HOST");
  deleteTestEnvValue("STEELENGINE_BRIDGE_PORT");
  deleteTestEnvValue("STEELENGINE_CANVAS_HOST_PORT");
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.STEELENGINE_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.STEELENGINE_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".steelengine");
  const priorIsolatedHome = params.env.STEELENGINE_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".steelengine")
  ) {
    realStateDir = path.join(params.realHome, ".steelengine");
  }
  const tempStateDir = path.join(params.tempHome, ".steelengine");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.STEELENGINE_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.STEELENGINE_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "steelengine.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "steelengine.json"),
...
export function installTestEnv(options?: InstallTestEnvOptions): {
  ...
  const allowRealHome = !hermetic && isTruthyEnvValue(process.env.STEELENGINE_LIVE_USE_REAL_HOME);
