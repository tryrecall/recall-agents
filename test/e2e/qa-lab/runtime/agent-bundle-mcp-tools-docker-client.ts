// SteelEngine bundle MCP tools Docker harness.
...
const probeTool = server.tool("docker_probe", "Docker SteelEngine MCP tool availability probe", async () => ({
...
async function main() {
  const tempState = await createE2eStateDir("steelengine-agent-bundle-mcp-");
