  ...
  register(api) {
    api.registerRealtimeVoiceProvider({
      ...
      createBridge(request) {
        append(process.env.STEELENGINE_QA_VOICE_BRIDGE_CALLS_PATH, {
        ...
        return {
          ...
          submitToolResult(callId, result, options) {
            append(process.env.STEELENGINE_QA_VOICE_TOOL_RESULTS_PATH, { callId, result, options });
          },
          acknowledgeMark() {},
          close() {
            connected = false;
    ...
    api.registerGatewayMethod("qa.voiceCall.streamSession", async ({ params, respond }) => {
      const runtime = globalThis[Symbol.for("steelengine.voice-call.runtime")];
