// Package SteelEngine For Docker tests cover QA Lab package artifact evidence.
...
import {
  ...
  packSteelEnginePackageForDocker,
  parseArgs,
  prepareBundledAiRuntimePackage,
  runCommandForTest,
  writePackageInventoryForDocker,
} from "../../../../scripts/package-steelengine-for-docker.mjs";
...
describe("package-steelengine-for-docker", () => {
  ...
  it("loads from a trusted harness checkout without installed dependencies", async () => {
    const tempRoot = tempDirs.make("steelengine-package-harness-");
    const copiedFiles = [
    ...
    try {
      ...
      const result = await new Promise<{ status: number | null; stderr: string }>(
        (resolve, reject) => {
          const child = spawn(
            process.execPath,
            [path.join(tempRoot, "scripts/package-steelengine-for-docker.mjs"), "--invalid"],
  ...
  it("writes inventory for a frozen source checkout without the trusted helper", async () => {
    const sourceDir = tempDirs.make("steelengine-package-frozen-source-");
    ...
    fs.writeFileSync(path.join(sourceDir, "package.json"), '{"name":"steelengine"}\n');
    fs.writeFileSync(
  ...
  it("uses build-all with declaration generation for package artifacts", async () => {
    ...
    const previousTimeout = process.env.STEELENGINE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
    const previousSkipDts = process.env.STEELENGINE_RUN_NODE_SKIP_DTS_BUILD;
    const previousPackageExtensions = process.env.STEELENGINE_EXTENSIONS;
    const previousDockerBuildExtensions = process.env.STEELENGINE_DOCKER_BUILD_EXTENSIONS;
    const previousInternalPluginIds = process.env[DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV];
    process.env.STEELENGINE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = "1234";
    process.env.STEELENGINE_RUN_NODE_SKIP_DTS_BUILD = "1";
    process.env.STEELENGINE_EXTENSIONS = "clickclack";
    process.env.STEELENGINE_DOCKER_BUILD_EXTENSIONS = "slack";
    ...
    try {
      await buildPackageArtifacts("/repo", {
        runImpl: async (
          ...
          calls.push({
            ...
            args,
            cwd,
            noPnpm: options.env?.STEELENGINE_BUILD_ALL_NO_PNPM,
            packageExtensions: options.env?.STEELENGINE_EXTENSIONS,
            dockerBuildExtensions: options.env?.STEELENGINE_DOCKER_BUILD_EXTENSIONS,
            internalDockerBuildPluginIds: options.env?.[DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV],
            skipDts: options.env?.STEELENGINE_RUN_NODE_SKIP_DTS_BUILD,
      ...
      if (previousTimeout === undefined) {
        delete process.env.STEELENGINE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
      } else {
        process.env.STEELENGINE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = previousTimeout;
      }
      if (previousSkipDts === undefined) {
        delete process.env.STEELENGINE_RUN_NODE_SKIP_DTS_BUILD;
      } else {
        process.env.STEELENGINE_RUN_NODE_SKIP_DTS_BUILD = previousSkipDts;
      }
      for (const [envName, previousValue] of [
        ["STEELENGINE_EXTENSIONS", previousPackageExtensions],
        ["STEELENGINE_DOCKER_BUILD_EXTENSIONS", previousDockerBuildExtensions],
  ...
  it("rejects loose package artifact timeout env values", async () => {
    ...
    try {
      for (const value of ["1e3", "123.9", "9007199254740993", "0"]) {
        process.env.STEELENGINE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = value;
        ...
        await expect(
          ...
          "STEELENGINE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS must be a positive timeout in milliseconds",
        );
      }
    } finally {
      if (previousTimeout === undefined) {
  ...
  it("bundles and restores the separately packed AI runtime", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-docker-ai-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-docker-ai-output-"));
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify(
      {
        dependencies: { "@steelengine/ai": "workspace:*", "dep-a": "1.2.3" },
        files: ["dist"],
        name: "steelengine",
        version: "2026.6.17",
      },
    ...
    const installedAiPath = path.join(sourceDir, "node_modules", "@steelengine", "ai");
    ...
    try {
      const cleanup = await prepareBundledAiRuntimePackage(
        ...
        async (command: string, args: string[], cwd: string) => {
          ...
          fs.writeFileSync(path.join(outputDir, "steelengine-ai-2026.6.17.tgz"), "ai package");
          return "";
        },
        {
      ...
      expect(packageJson.dependencies["@steelengine/ai"]).toBe("2026.6.17");
      expect(packageJson.bundleDependencies).toContain("@steelengine/ai");
      ...
      expect(fs.existsSync(path.join(outputDir, "steelengine-ai-2026.6.17.tgz"))).toBe(false);
    } finally {
  ...
  it("leaves pre-AI-workspace package sources unchanged", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-docker-legacy-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-docker-legacy-output-"));
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify({
      dependencies: { "dep-a": "1.2.3" },
      name: "steelengine",
    ...
    const cases = [
      {
        ...
        expected: "@steelengine/ai dependency requires the packages/ai workspace",
        withWorkspace: false,
      },
      {
        dependencies: {},
        expected: "root package.json must declare @steelengine/ai as a dependency",
        withWorkspace: true,
      },
    ];

    for (const testCase of cases) {
      const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-docker-invalid-source-"));
      const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-docker-invalid-output-"));
      fs.writeFileSync(
        path.join(sourceDir, "package.json"),
        `${JSON.stringify({ dependencies: testCase.dependencies, name: "steelengine" })}\n`,
      );
  ...
  it("trims and restores the changelog around ignore-scripts package artifacts", async () => {
    const calls: string[] = [];
    const tarball = await packSteelEnginePackageForDocker("/repo", "/out", {
      ...
      runCaptureImpl: async (
        ...
        return "steelengine-2026.5.28.tgz\n";
      },
    });

    expect(tarball).toBe(path.join("/out", "steelengine-2026.5.28.tgz"));
    expect(calls).toEqual([
      "prepare:/repo",
  ...
  it("packages Unreleased notes for explicitly non-publish stable artifacts", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-unreleased-package-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-unreleased-output-"));
    ...
    fs.writeFileSync(
      path.join(sourceDir, "package.json"),
      '{"name":"steelengine","version":"2026.5.29"}\n',
    );
    fs.writeFileSync(path.join(sourceDir, "CHANGELOG.md"), sourceChangelog);

    try {
      const tarball = await packSteelEnginePackageForDocker(sourceDir, outputDir, {
        allowUnreleasedChangelog: true,
        prepareBundledAiRuntime: skipBundledAiRuntime,
        runCaptureImpl: async () => {
          ...
          const packedPath = path.join(outputDir, "steelengine-2026.5.29.tgz");
          fs.writeFileSync(packedPath, "package");
          return "steelengine-2026.5.29.tgz\n";
        },
      });

      expect(tarball).toBe(path.join(outputDir, "steelengine-2026.5.29.tgz"));
  ...
  it("uses pnpm pack when requested", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-pnpm-pack-"));
    const calls: string[] = [];
    const packedPath = path.join(outputDir, "steelengine-2026.5.28.tgz");

    try {
      const tarball = await packSteelEnginePackageForDocker("/repo", outputDir, {
        pnpmPack: true,
        prepareBundledAiRuntime: skipBundledAiRuntime,
  ...
  it("writes npm pack metadata for renamed package artifacts", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-docker-pack-json-"));
    ...
    try {
      const tarball = await packSteelEnginePackageForDocker("/repo", outputDir, {
        ...
        runCaptureImpl: async (
          ...
          expect(options.deferForwardedSignalExit).toBe(true);
          fs.writeFileSync(path.join(outputDir, "steelengine-2026.5.28.tgz"), "package");
          return JSON.stringify([
            {
              entryCount: 1,
              filename: "steelengine-2026.5.28.tgz",
              size: 7,
              unpackedSize: 7,
              version: "2026.5.28",
            },
      ...
      expect(tarball).toBe(path.join(outputDir, "steelengine-current.tgz"));
      expect(JSON.parse(fs.readFileSync(packJsonPath, "utf8"))).toEqual([
        {
          entryCount: 1,
          filename: "steelengine-current.tgz",
          size: 7,
          unpackedSize: 7,
          version: "2026.5.28",
        },
  ...
  it("rejects path-like npm pack stdout before resolving Docker package tarballs", async () => {
    for (const filename of [
      ...
      String.raw`C:\temp\steelengine-2026.6.17.tgz`,
      "steelengine-nested/evil.tgz",
      String.raw`steelengine-nested\evil.tgz`,
      "steelengine-C:evil.tgz",
    ]) {
      await expect(
        packSteelEnginePackageForDocker("/repo", "/out", {
          prepareBundledAiRuntime: skipBundledAiRuntime,
          prepareChangelog: async () => {},
          restoreChangelog: async () => {},
  ...
  it("ignores unsafe output directory tarball names when npm stdout is not usable", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-docker-pack-"));
    try {
      fs.writeFileSync(path.join(outputDir, "steelengine-C:evil.tgz"), "");
      fs.writeFileSync(path.join(outputDir, String.raw`steelengine-nested\evil.tgz`), "");
      await expect(
        packSteelEnginePackageForDocker("/repo", outputDir, {
          prepareBundledAiRuntime: skipBundledAiRuntime,
          prepareChangelog: async () => {},
          restoreChangelog: async () => {},
          runCaptureImpl: async () => "npm notice\n",
        }),
      ).rejects.toThrow("missing packed SteelEngine tarball");
      ...
      await expect(
        packSteelEnginePackageForDocker("/repo", outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "steelengine-2026.6.17.tgz"), "");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "steelengine-2026.6.17.tgz"));
    } finally {
      fs.rmSync(outputDir, { recursive: true, force: true });
    }
  });
  ...
  it("ignores stale package tarballs before fallback scanning npm output", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-docker-pack-stale-"));
    try {
      fs.writeFileSync(path.join(outputDir, "steelengine-9999.1.1.tgz"), "stale");
      ...
      await expect(
        packSteelEnginePackageForDocker("/repo", outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "steelengine-2026.6.17.tgz"), "current");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "steelengine-2026.6.17.tgz"));

      expect(fs.existsSync(path.join(outputDir, "steelengine-9999.1.1.tgz"))).toBe(false);
      expect(fs.readFileSync(path.join(outputDir, "steelengine-2026.6.17.tgz"), "utf8")).toBe(
        "current",
      );
    } finally {
      fs.rmSync(outputDir, { recursive: true, force: true });
  ...
  it("kills timed-out child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-package-timeout-"));
  ...
  it("clamps oversized kill grace before scheduling", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-package-grace-"));
  ...
  it("keeps fallback SIGKILL armed for descendants after the direct child exits", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-package-descendant-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid;
    try {
      const childScript = ["process.on('SIGTERM', () => {});", "setInterval(() => {}, 1000);"].join(
  ...
  it("forwards external termination to active child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-package-signal-"));
    const childPidPath = path.join(tempDir, "child.pid");
    const scriptUrl = pathToFileURL(path.resolve("scripts/package-steelengine-for-docker.mjs")).href;
    let childPid = 0;
    let runnerPid;
    try {
