...
describe("qa-otel-smoke receiver bounds", () => {
  ...
  beforeAll(() => {
    configuredBodyLimitLoad = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          STEELENGINE_QA_OTEL_MAX_CAPTURED_BODY_TEXT_BYTES: "1024",
          STEELENGINE_QA_OTEL_MAX_COMPRESSED_BODY_BYTES: "2048",
          STEELENGINE_QA_OTEL_MAX_DECODED_BODY_BYTES: "4096",
        },
      },
    );
  });
  ...
  function makePassingSmokeAssertionInput(): Parameters<typeof testing.assertSmoke>[0] {
    return {
      ...
      metrics: [{ name: "steelengine.harness.duration_ms" }],
      requests: [
        {
          path: "/v1/traces",
          signal: "traces",
      ...
      spans: [
        {
          name: "steelengine.run",
          parent: false,
          attributes: {
            "steelengine.error": "QA OTEL provider stream failed OPENAI_API_KEY=***",
          },
        },
        {
          name: "steelengine.harness.run",
          parent: true,
          attributes: {
            "steelengine.error": "QA OTEL provider stream failed OPENAI_API_KEY=***",
          },
        },
        { name: "steelengine.context.assembled", parent: true, attributes: {} },
        { name: "steelengine.message.delivery", parent: true, attributes: {} },
        {
          ...
          attributes: {
            ...
            "steelengine.model": "gpt-5.6-luna",
  ...
  it("ignores inherited OTEL exporter endpoints during direct producer execution", () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "steelengine-qa-otel-env-isolation-"));
    try {
    ...
    input.stdoutLogRecords = [
      {
        ...
        "service.name": "steelengine-qa-lab-otel-smoke",
        severityText: "INFO",
        severityNumber: 9,
        body: "log",
        attributes: {
          "steelengine.log.level": "INFO",
        },
      },
    ];
  ...
  it("moves Docker collector telemetry off the default host port", async () => {
    ...
    const collector = await testing.startDockerOtelCollector(4317, {
      mkdtemp: async () => "/tmp/steelengine-otel-collector-test",
      platform: "linux",
    ...
    expect(stopDockerContainer).toHaveBeenCalledWith(
      "steelengine-otel-smoke-00000000-0000-4000-8000-000000000000",
    );
    expect(removePath).toHaveBeenCalledWith("/tmp/steelengine-otel-collector-test", {
      force: true,
      recursive: true,
    });
  });
  ...
  it("cleans Docker collector containers and temp config after readiness failures", async () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "steelengine-qa-otel-collector-"));
  ...
  it("reports bounded Docker collector output when readiness exits", async () => {
    const tempRoot = mkdtempSync(path.join(os.tmpdir(), "steelengine-qa-otel-collector-output-"));
    const collectorDir = path.join(tempRoot, "collector");
    const child = new EventEmitter() as EventEmitter & {
      stderr: EventEmitter;
      stdout: EventEmitter;
