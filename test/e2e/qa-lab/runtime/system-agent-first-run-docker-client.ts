// SteelEngine first-run Docker harness.
...
const DISCORD_CREDENTIAL_FIXTURE = ["steelengine", "discord", "fixture"].join("-");
...
function resolveDefaultModel(config: SteelEngineConfig): string | undefined {
...
async function runPackagedCli(args: string[]): Promise<PackagedCommandResult> {
  const child = spawn("steelengine", args, {
    env: process.env,
  ...
  if (timedOut) {
    throw new Error(
      `Packaged CLI timed out after ${PACKAGED_CLI_TIMEOUT_MS}ms: steelengine ${args.join(" ")}\n${stdout}\n${stderr}`,
    );
  }
...
async function main() {
  ...
  const tempState = await createE2eStateDir("steelengine-system-agent-first-run-");
  ...
  setEnvValue("STEELENGINE_STATE_DIR", stateDir);
  setEnvValue("STEELENGINE_CONFIG_PATH", configPath);
  ...
  clearConfigCache();
  assert(
    await shouldStartOnboardingForFreshInstall(["node", "steelengine"]),
    "fresh bare SteelEngine invocation did not route to onboarding",
  );

  const blocked = await runPackagedCli(["setup", "--message", "overview"]);
  assert(blocked.code === 1, "SteelEngine did not fail closed without inference");
  assert(
    `${blocked.stdout}\n${blocked.stderr}`.includes("steelengine onboard"),
    "blocked SteelEngine did not direct the user to inference onboarding",
  );
  ...
  const inferenceConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as SteelEngineConfig;
  assert(
  ...
  assert(
    ...
    "inference activation configured the rest before SteelEngine started",
  );
  ...
  assert(
    ...
    "modern compatibility entrypoint did not expose SteelEngine after activation",
  );
  ...
  for (const command of spec.commands) {
    ...
    assert(
      ...
      `SteelEngine first-run command ${command.id} did not apply: ${output}`,
    );
    if (command.planner) {
      assert(
        output.includes(`[steelengine] planner: ${spec.model}`) &&
          output.includes(FAKE_PLANNER_REPLY) &&
          output.includes(`[steelengine] interpreted: ${plannerCommand}`),
        `SteelEngine first-run command ${command.id} did not use the verified planner: ${output}`,
      );
    }
    ...
    assert(
      ...
      `SteelEngine command ${command.id} ran ${probeDelta} inference probes; expected at least ${minimumProbes} for preflight${command.approve ? " plus its persistent boundary" : ""}`,
    );
  }
  ...
  assert(reef, "SteelEngine did not create reef agent");
  assert(reef.workspace === spec.dockerAgentWorkspace, "SteelEngine did not write reef workspace");
  assert(
    reef.model === undefined,
    "SteelEngine wrote a per-agent model instead of inheriting the verified default",
  );
  assert(config.channels?.discord?.enabled === true, "SteelEngine did not enable Discord");
  ...
  assert(
    ...
    "SteelEngine did not write Discord token SecretRef",
  );
  assert(
    !JSON.stringify(config.channels.discord).includes(DISCORD_CREDENTIAL_FIXTURE),
    "SteelEngine persisted the raw Discord token",
  );
  ...
  assert(
    ...
    "SteelEngine persisted an unrelated ambient credential",
  );
  ...
  console.log("SteelEngine first-run Docker E2E passed");
}
