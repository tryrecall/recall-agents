...
const ALLOW_ENV = "STEELENGINE_QA_ALLOW_UPDATE_RUN_SELF";
    ...
    (resolve, reject) => {
      const child = spawn(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...
          STEELENGINE_DOCKER_ALL_LANES: SCENARIO_ID,
          ...
          STEELENGINE_UPDATE_RUN_SELF_UPGRADE_ARTIFACT_DIR: laneArtifactDir,
        },
  ...
  try {
    ...
    return await writer.write({
      artifacts: [
        ...
        { kind: "summary", filePath: path.join("lane", "steelengine-gateway.service") },
