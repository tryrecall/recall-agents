...
const QA_DOCKER_E2E_LANES = {
  ...
  "update-migration": {
    env: (env) => ({
      STEELENGINE_UPGRADE_SURVIVOR_BASELINE_SPEC:
        env.STEELENGINE_UPGRADE_SURVIVOR_BASELINE_SPEC ?? "steelengine@2026.4.23",
      STEELENGINE_UPGRADE_SURVIVOR_PUBLISHED_BASELINE: "1",
      STEELENGINE_UPGRADE_SURVIVOR_SCENARIO:
        env.STEELENGINE_UPGRADE_SURVIVOR_SCENARIO ?? "plugin-deps-cleanup",
    }),
    script: "scripts/e2e/upgrade-survivor-docker.sh",
  },
  "update-restart-auth": {
    env: (env) => ({
      STEELENGINE_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT:
        env.STEELENGINE_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT ?? "1500s",
      STEELENGINE_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE: "auto-auth",
    }),
    script: "scripts/e2e/upgrade-survivor-docker.sh",
  },
  "upgrade-survivor": {
