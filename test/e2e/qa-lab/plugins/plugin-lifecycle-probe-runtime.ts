...
function stateDir(env: ProbeEnv = process.env) {
  return env.STEELENGINE_STATE_DIR || path.join(env.HOME ?? os.homedir(), ".steelengine");
}

function configPath(env: ProbeEnv = process.env) {
  return env.STEELENGINE_CONFIG_PATH || path.join(stateDir(env), "steelengine.json");
}
...
function createMatrixStateEnv(resourceDir: string): MatrixEnv {
  ...
  const configFile = path.join(stateDir, "steelengine.json");
  ...
  return {
    ...
    STEELENGINE_TEST_WORKSPACE_DIR: workspaceDir,
    STEELENGINE_AUTH_PROFILE_SECRET_KEY: randomBytes(32).toString("hex"),
  };
}
...
function packageEntrypoint(prefix: string) {
  ...
  throw new Error(`SteelEngine package entrypoint not found under ${packageRoot}/dist/`);
}
...
async function installSteelEnginePackage(prefix: string, env: MatrixEnv) {
  const packageTgz = env.STEELENGINE_CURRENT_PACKAGE_TGZ;
  assertProbe(packageTgz, "STEELENGINE_CURRENT_PACKAGE_TGZ is required");
  const installLog = "/tmp/steelengine-plugin-lifecycle-install.log";
  process.stdout.write("Installing mounted SteelEngine package...\n");
  await runCommand(
    "npm",
    ["install", "-g", "--prefix", prefix, packageTgz, "--no-fund", "--no-audit"],
    {
      ...
      timeoutMs: parseDurationMs(env.STEELENGINE_E2E_NPM_INSTALL_TIMEOUT, "600s"),
    },
  );
}
...
async function runPluginLifecycleMatrix() {
  ...
  const packageName = "@steelengine/lifecycle-claw";
  const resourceDir = tempDirs.make("steelengine-plugin-lifecycle-matrix-");
  ...
  try {
    await installSteelEnginePackage(npmPrefix, env);
