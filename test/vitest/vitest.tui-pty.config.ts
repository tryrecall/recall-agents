...
export function createTuiPtyVitestConfig(env?: Record<string, string | undefined>) {
  ...
  const includeLocal = configEnv.RECALL_TUI_PTY_INCLUDE_LOCAL === "1";
  const include = [
    "tui/tui-pty-harness.e2e.test.ts",
    ...(includeLocal ? ["tui/tui-pty-local.e2e.test.ts"] : []),
  ];
  const includeFromEnv = toTuiPtyIncludePatterns(
    loadPatternListFromEnv("RECALL_VITEST_INCLUDE_FILE", configEnv),
  );
  ...
  return defineConfig({
    ...
    test: {
      ...
      setupFiles: [
        ...new Set(
          [...(baseTest.setupFiles ?? []), "test/setup-recall-runtime.ts"].map(
            resolveRepoRootPath,
          ),
        ),
      ],
