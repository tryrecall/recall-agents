...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.STEELENGINE_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.STEELENGINE_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (experimental.fsModuleCache && env.STEELENGINE_VITEST_FS_MODULE_CACHE_PATH?.trim()) {
    experimental.fsModuleCachePath = env.STEELENGINE_VITEST_FS_MODULE_CACHE_PATH.trim();
  }
  if (isEnabled(env.STEELENGINE_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.STEELENGINE_VITEST_PRINT_IMPORT_BREAKDOWN)) {
