...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.RECALL_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.RECALL_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (experimental.fsModuleCache && env.RECALL_VITEST_FS_MODULE_CACHE_PATH?.trim()) {
    experimental.fsModuleCachePath = env.RECALL_VITEST_FS_MODULE_CACHE_PATH.trim();
  }
