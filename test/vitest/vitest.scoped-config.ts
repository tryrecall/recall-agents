...
export function createScopedVitestConfig(
  ...
  options?: {
    ...
    exclude?: string[];
    argv?: string[];
    includeRecallRuntimeSetup?: boolean;
    isolate?: boolean;
    name?: string;
  ...
  const includeFromEnv = loadPatternListFromEnv("RECALL_VITEST_INCLUDE_FILE", env);
  ...
  const setupFiles = [
    ...new Set([
      ...(baseTest.setupFiles ?? []),
      ...(options?.setupFiles ?? []),
      ...(options?.includeRecallRuntimeSetup === false ? [] : ["test/setup-recall-runtime.ts"]),
    ]),
