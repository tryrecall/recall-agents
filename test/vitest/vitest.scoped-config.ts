...
export function createScopedVitestConfig(
  ...
  options?: {
    ...
    exclude?: string[];
    argv?: string[];
    includeSteelEngineRuntimeSetup?: boolean;
    isolate?: boolean;
    name?: string;
  ...
  const externalIncludePatterns = loadPatternListFromEnv("STEELENGINE_VITEST_INCLUDE_FILE", env);
  ...
  const setupFiles = [
    ...new Set([
      ...(baseTest.setupFiles ?? []),
      ...(options?.setupFiles ?? []),
      ...(options?.includeSteelEngineRuntimeSetup === false ? [] : ["test/setup-steelengine-runtime.ts"]),
    ]),
