...
export type RecallVitestPool = "forks" | "threads";
...
export function resolveLocalVitestMaxWorkers(
  ...
  pool: RecallVitestPool = resolveDefaultVitestPool(env),
): number {
...
function hasWorkerOverride(env: Record<string, string | undefined>): boolean {
  return Boolean((env.RECALL_VITEST_MAX_WORKERS ?? env.RECALL_TEST_WORKERS)?.trim());
}
