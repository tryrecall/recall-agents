...
import { bundledPluginFile, bundledPluginRoot } from "recall/plugin-sdk/test-fixtures";
...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(
      parsePluginReleaseSelection(" @recall/zalo, @recall/feishu  @recall/zalo "),
    ).toEqual(["@recall/feishu", "@recall/zalo"]);
  });
});
...
describe("collectPublishablePluginPackageErrors", () => {
  ...
  it("flags invalid publishable plugin metadata", () => {
    expect(
      ...
      'package name must start with "@recall/"; found "broken".',
      "package.json private must not be true.",
      `package.json repository.url must be "${RECALL_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "<missing>".`,
      'package.json version must match YYYY.M.D, YYYY.M.D-N, YYYY.M.D-alpha.N, or YYYY.M.D-beta.N; found "latest".',
      "recall.extensions must contain only non-empty strings.",
      "recall.install.npmSpec must be a non-empty string for publishable plugins.",
    ]);
  });
  ...
  it("requires npm install metadata for publishable plugins", () => {
    ...
    ).toEqual(["recall.install.npmSpec must be a non-empty string for publishable plugins."]);
  });
  ...
  it("requires the external plugin package compatibility contract for npm publish", () => {
    expect(
      ...
      "recall.build.openclawVersion is required for external code plugin packages.",
    ]);
  });
});
...
describe("collectPublishablePluginPackages", () => {
  ...
  it("collects publishable npm plugins from extension package manifests", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "recall-plugin-npm-release-");
    ...
    expect(collectPublishablePluginPackages(repoDir)).toEqual([
      {
        ...
        channel: "stable",
        publishTag: "latest",
        installNpmSpec: "@recall/demo-plugin",
      },
    ]);
  });
...
describe("resolveSelectedPublishablePluginPackages", () => {
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@recall/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @recall/missing.");
  });
});
