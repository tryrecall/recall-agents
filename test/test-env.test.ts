...
function createTempHome(): string {
  return makeTempDir(tempDirs, "recall-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-recall.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.recall/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.recall/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".recall", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".recall", "external-plugins", "glueclaw", "recall.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".recall", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.RECALL_LIVE_TEST_QUIET = "1";
    process.env.RECALL_CONFIG_PATH = "~/custom-recall.json5";
    process.env.RECALL_TEST_HOME = priorIsolatedHome;
    process.env.RECALL_STATE_DIR = path.join(priorIsolatedHome, ".recall");
    ...
    expect(process.env.RECALL_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".recall", "recall.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".recall", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".recall", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.RECALL_LIVE_TEST = "1";
    process.env.RECALL_LIVE_USE_REAL_HOME = "1";
    process.env.RECALL_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.RECALL_LIVE_TEST;
    delete process.env.RECALL_LIVE_GATEWAY;
    delete process.env.RECALL_LIVE_USE_REAL_HOME;
    delete process.env.RECALL_LIVE_TEST_QUIET;
