...
type OfficialChannelInstall = NonNullable<
  NonNullable<OfficialChannelCatalogEntry["steelengine"]>["install"]
>;
...
function requireInstall(entry: OfficialChannelCatalogEntry | undefined): OfficialChannelInstall {
  const install = entry?.steelengine?.install;
  if (!install) {
...
function summarizeCatalogEntry(entry: OfficialChannelCatalogEntry) {
  return {
    ...
    source: entry.source,
    plugin: entry.steelengine?.plugin,
    channel: entry.steelengine?.channel,
    install: entry.steelengine?.install,
  };
}
...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("steelengine-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@steelengine/whatsapp",
      version: "2026.3.23",
      description: "SteelEngine WhatsApp channel plugin",
      steelengine: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "@wecom/wecom-steelengine-plugin"),
      ),
    ).toEqual({
      name: "@wecom/wecom-steelengine-plugin",
      description: "SteelEngine WeCom channel plugin by the Tencent WeCom team.",
      source: "external",
      plugin: {
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "steelengine-plugin-yuanbao"),
      ),
    ).toEqual({
      name: "steelengine-plugin-yuanbao",
      description: "SteelEngine Yuanbao channel plugin by the Tencent Yuanbao team.",
      source: "external",
      plugin: {
        id: "steelengine-plugin-yuanbao",
        label: "Yuanbao",
      },
      channel: {
        id: "yuanbao",
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "@steelengine/whatsapp"),
      ),
    ).toEqual({
      name: "@steelengine/whatsapp",
      description: "SteelEngine WhatsApp channel plugin",
  ...
  it("keeps third-party official external catalog npm sources exactly pinned", () => {
    const repoRoot = makeRepoRoot("steelengine-official-channel-catalog-policy-");
    const entries = buildOfficialChannelCatalog({ repoRoot }).entries.filter(
      (entry) => entry.source === "external" && !entry.name?.startsWith("@steelengine/"),
    );
  ...
  it("allows official SteelEngine channel npm specs without integrity during launch", () => {
    const repoRoot = makeRepoRoot("steelengine-official-channel-catalog-steelengine-policy-");
    const twitch = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (entry) => entry.steelengine?.channel?.id === "twitch",
    );

    expect({
      name: twitch?.name,
      install: twitch?.steelengine?.install,
    }).toEqual({
  ...
  it("preserves ClawHub specs when generating publishable channel catalog entries", () => {
    const repoRoot = makeRepoRoot("steelengine-official-channel-catalog-clawhub-");
    writeJson(path.join(repoRoot, "extensions", "storepack-chat", "package.json"), {
      name: "@steelengine/storepack-chat",
      steelengine: {
        channel: {
          id: "storepack-chat",
        ...
        install: {
          clawhubSpec: "clawhub:@steelengine/storepack-chat",
          npmSpec: "@steelengine/storepack-chat",
          defaultChoice: "clawhub",
        },
        release: {
          publishToNpm: true,
    ...
    const entry = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (candidate) => candidate.steelengine?.channel?.id === "storepack-chat",
    );
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("steelengine-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@steelengine/whatsapp",
      steelengine: {
        channel: {
    ...
    expect(entries.map((entry: { name?: string }) => entry.name)).toContain(
      "@wecom/wecom-steelengine-plugin",
    );
    expect(entries.map((entry: { name?: string }) => entry.name)).toContain(
      "steelengine-plugin-yuanbao",
    );
    const whatsappEntry = findCatalogEntry(
      entries,
      (entry: { steelengine?: { channel?: { id?: string } } }) =>
        entry.steelengine?.channel?.id === "whatsapp",
    );
