...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.RECALL_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through recall infer", () => {
    const outputBase = path.join(os.tmpdir(), `recall-infer-image-${process.pid}.png`);
