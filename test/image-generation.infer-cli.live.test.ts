...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.STEELENGINE_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through steelengine infer", () => {
    const outputBase = path.join(os.tmpdir(), `steelengine-infer-image-${process.pid}.png`);
