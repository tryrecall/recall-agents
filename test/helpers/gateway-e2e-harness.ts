...
import { createRecallTestInstance, type RecallTestInstance } from "./recall-test-instance.js";
...
export type GatewayInstance = RecallTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createRecallTestInstance({ name });
  try {
