...
export function unwrapRemoteConfigSnapshot(raw: unknown): SteelEngineConfig {
  const rawObj = asRecord(raw);
  const resolved = asRecord(rawObj.resolved);
  if (Object.keys(resolved).length > 0) {
    return resolved as SteelEngineConfig;
  }

  const wrapped = asRecord(rawObj.config);
  if (Object.keys(wrapped).length > 0) {
    return wrapped as SteelEngineConfig;
  }
  ...
  if (Object.keys(legacyResolved).length > 0) {
    return legacyResolved as SteelEngineConfig;
  }

  const legacyConfig = asRecord(legacyPayload.config);
  if (Object.keys(legacyConfig).length > 0) {
    return legacyConfig as SteelEngineConfig;
  }

  if (Object.keys(rawObj).length > 0 && !Object.hasOwn(rawObj, "payload")) {
    return rawObj as SteelEngineConfig;
  }
