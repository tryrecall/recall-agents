// SteelEngine test instance helper spawns isolated SteelEngine processes.
...
} from "../../src/test-utils/steelengine-test-state.js";
import { sleep } from "../../src/utils.js";

type SteelEngineTestStateOptions = NonNullable<Parameters<typeof createSteelEngineTestState>[0]>;

type SteelEngineTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<SteelEngineTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

type SteelEngineTestInstanceCommandResult = {
  code: number | null;
...
type SteelEngineTestProcess = ChildProcessByStdio<null, Readable, Readable>;

export type SteelEngineTestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  stdout: string[];
  stderr: string[];
  child?: SteelEngineTestProcess;
  env: NodeJS.ProcessEnv;
  entrypoint: () => Promise<string[]>;
  cli: (
    args: string[],
    options?: { timeoutMs?: number },
  ) => Promise<SteelEngineTestInstanceCommandResult>;
...
type SteelEngineTestChildProcess = Pick<SteelEngineTestProcess, "kill" | "pid">;
...
async function prepareGatewayEntrypoint(cwd: string): Promise<string[]> {
  ...
  const child = spawn("node", ["scripts/run-node.mjs", "--help"], {
    ...
    detached: shouldUseSteelEngineTestProcessGroup(),
  });
  ...
  if (completed === null) {
    signalSteelEngineTestProcess(child, "SIGKILL");
...
async function waitForPortOpen(
  proc: Pick<SteelEngineTestProcess, "exitCode" | "signalCode">,
  chunksOut: string[],
  chunksErr: string[],
  port: number,
  timeoutMs: number,
...
async function waitForGatewayExit(child: SteelEngineTestProcess, timeoutMs: number): Promise<boolean> {
...
function hasChildExited(child: Pick<SteelEngineTestProcess, "exitCode" | "signalCode">) {
...
export async function createSteelEngineTestInstance(
  options: SteelEngineTestInstanceOptions,
): Promise<SteelEngineTestInstance> {
  ...
  let child: SteelEngineTestProcess | undefined;
  let cleaned = false;

  const instance: SteelEngineTestInstance = {
    name: options.name,
    port,
    ...
    stopGateway: async () => {
      ...
      if (!hasChildExited(child) && !child.killed) {
        try {
          signalSteelEngineTestProcess(child, "SIGTERM");
        } catch {
          // ignore
        }
      }
...
function shouldUseSteelEngineTestProcessGroup(): boolean {
  return process.platform !== "win32";
}

function signalSteelEngineTestProcess(
  child: SteelEngineTestChildProcess,
  signal: NodeJS.Signals,
  killProcess: (pid: number, signal: NodeJS.Signals) => boolean = (pid, nextSignal) =>
    process.kill(pid, nextSignal),
): void {
  if (shouldUseSteelEngineTestProcessGroup() && typeof child.pid === "number") {
    try {
