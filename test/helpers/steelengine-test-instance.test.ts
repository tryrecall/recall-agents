// SteelEngine test instance tests cover spawned test instance lifecycle.
import fs from "node:fs/promises";
import path from "node:path";
import { describe, expect, it, vi } from "vitest";
import { createSteelEngineTestInstance, testing } from "./steelengine-test-instance.js";
...
describe("steelengine test instance", () => {
  ...
  it("signals test instance process groups on POSIX", () => {
    ...
    testing.signalSteelEngineTestProcess(child, "SIGKILL", killProcess);
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".steelengine"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "steelengine.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.STEELENGINE_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.STEELENGINE_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.STEELENGINE_SKIP_CRON).toBe("0");
