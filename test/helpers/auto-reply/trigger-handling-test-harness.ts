...
const embeddedAgentMocks = getSharedMocks("steelengine.trigger-handling.embedded-agent-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("steelengine.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("steelengine.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("steelengine.trigger-handling.web-session-mocks", () => ({
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.STEELENGINE_STATE_DIR = join(home, ".steelengine");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "steelengine-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".steelengine", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): SteelEngineConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-7" },
        workspace: join(home, "steelengine"),
