...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked RECALL_HOME.
  delete process.env.RECALL_HOME;
  process.env.RECALL_STATE_DIR = path.join(base, ".recall");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "recall-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".recall", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
