...
import { createSteelEngineCodingTools } from "../../../src/plugin-sdk/agent-harness.js";
...
const WORKSPACE_DIR = "/tmp/steelengine-happy-path/workspace";
const AGENT_DIR = "/tmp/steelengine-happy-path/agent";
const SESSION_FILE = "/tmp/steelengine-happy-path/session.jsonl";
...
const CODEX_HEARTBEAT_COLLABORATION_INSTRUCTIONS = [
  "## SteelEngine Heartbeat Workspace",
  "",
...
const dynamicToolsConfig: SteelEngineConfig = {
  ...baseConfig,
  plugins: {
    enabled: true,
    slots: {
...
async function createScenarios(codexApi: CodexPromptSnapshotApi): Promise<PromptScenario[]> {
  ...
  const discordGroupCtx: TemplateContext = {
    ...
    SenderName: "Pash",
    SenderUsername: "pash",
    GroupSubject: "SteelEngine maintainers",
    GroupChannel: "#agent-sandbox",
    GroupSpace: "SteelEngine",
    ConversationLabel: "SteelEngine/#agent-sandbox",
    WasMentioned: true,
    InboundHistory: [
      {
        sender: "Peter",
      ...
      {
        sender: "Pash",
        body: "@SteelEngine please verify the Codex happy path too.",
      },
    ],
    Body: "@SteelEngine can you audit whether this prompt path has conflicting silence instructions?",
  ...
  return [
    {
      ...
      notes: [
        ...
        "A quiet turn is represented by not calling `message(action=send)`; the normal final assistant text is private to SteelEngine/Codex.",
      ],
      trigger: "user",
      ctx: telegramDirectCtx,
      prompt: createPrompt(
...
function renderModelBoundPromptLayers(params: {
  ...
  const codexConfigInstructions =
    ...
    typeof params.codexSnapshot.threadStartParams.config.instructions === "string"
      ...
      : "";
  const steelEngineDeveloperInstructions = params.codexSnapshot.developerInstructions;
  ...
  return [
      ...
      stableJson({
        ...
        steelEngineRuntime: {
          ...
          workspaceBootstrapContextFrom:
            "extensions/codex app-server turn/start input SteelEngine runtime context",
    ...
    markdownFence(
      ...
      stableJson({
        ...
        steelEngineDeveloperInstructions: textStats(steelEngineDeveloperInstructions),
    ...
    "",
    markdownFence("text", steelEngineDeveloperInstructions),
    "",
...
function buildCodexSteelEngineRuntimeContext(): string {
  return [
...
function prependCodexSteelEngineRuntimeContext(prompt: string): string {
  return [buildCodexSteelEngineRuntimeContext(), "", "Current user request:", prompt].join("\n");
}
...
function renderScenarioSnapshot(
  ...
  const codexTurnPromptText = prependCodexSteelEngineRuntimeContext(scenario.prompt);
...
function renderReadme(scenarios: PromptScenario[]): string {
  return [
    ...
    "",
    "The Markdown files show selected app-server thread/turn params plus a reconstructed model-bound prompt layer stack: Codex `gpt-5.5` model instructions from a pinned Codex model catalog fixture, Codex permission developer instructions for the happy-path yolo profile, SteelEngine developer instructions, turn input with simulated SteelEngine workspace bootstrap runtime context, heartbeat collaboration-mode guidance when applicable, and references to the complete dynamic tool catalog.",
    "",
    "The workspace bootstrap simulation includes dummy workspace contents so prompt reviewers can see how SteelEngine routes stable profile files into Codex developer instructions, keeps `MEMORY.md` in turn input, and points heartbeat turns at `HEARTBEAT.md` without inlining it. `AGENTS.md` is intentionally not repeated here because Codex loads it natively.",
    "",
    "The tool catalog is pinned to the canonical happy-path SteelEngine tools so optional locally installed plugin tools do not create fixture churn.",
    "",
    ...
    "",
    "These snapshots are still not a byte-for-byte raw OpenAI request capture. Codex-owned native `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions can be added inside the Codex runtime after SteelEngine sends thread and turn params.",
    "",
    "Regenerate with:",
    "",
