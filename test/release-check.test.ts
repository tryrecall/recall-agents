...
import { bundledDistPluginFile, bundledPluginFile } from "recall/plugin-sdk/test-fixtures";
...
import { collectInstalledRootDependencyManifestErrors } from "../scripts/recall-npm-postpublish-verify.ts";
import {
...
describe("packed CLI smoke", () => {
  ...
  it("builds a packed CLI smoke env with packaged-install guardrails", () => {
    expect(
      createPackedCliSmokeEnv(
        {
          ...
          RECALL_CONFIG_PATH: "/tmp/leaky-config.json",
        },
        { HOME: "/tmp/smoke-home", RECALL_STATE_DIR: "/tmp/smoke-state" },
      ),
    ).toEqual({
      PATH:
      ...
      TMPDIR: "/tmp/original-tmp",
      SystemRoot: "C:\\Windows",
      RECALL_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK: "1",
      RECALL_NO_ONBOARD: "1",
      RECALL_SERVICE_REPAIR_POLICY: "external",
      RECALL_SUPPRESS_NOTES: "1",
      RECALL_STATE_DIR: "/tmp/smoke-state",
    });
  });
  ...
  it("skips plugin command discovery during packed completion cache smoke", () => {
    expect(
      createPackedCompletionSmokeEnv(
        {
          PATH: "/usr/bin",
          RECALL_COMPLETION_SKIP_PLUGIN_COMMANDS: "0",
        },
        {
          HOME: "/tmp/smoke-home",
          RECALL_STATE_DIR: "/tmp/smoke-state",
...
describe("workspace bootstrap smoke", () => {
  it("runs with a sterile env instead of maintainer provider credentials", () => {
    expect(
      ...
      RECALL_HOME: "/tmp/bootstrap-home",
      TMPDIR: "/tmp/original-tmp",
      RECALL_NO_ONBOARD: "1",
      RECALL_SUPPRESS_NOTES: "1",
...
describe("collectBundledExtensionManifestErrors", () => {
  ...
  it("flags invalid bundled extension minHostVersion metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | recall.install.minHostVersion must use a semver floor in the form \">=x.y.z[-prerelease][+build]\"",
    ]);
  });
...
describe("bundled plugin package dependency checks", () => {
  ...
  it("does not require root deps for root chunks sourced from the owning installed plugin", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "recall-root-owned-installed-"));

    try {
      mkdirSync(join(tempRoot, "dist", "extensions", "memory-lancedb"), { recursive: true });
      writeFileSync(
        join(tempRoot, "package.json"),
        `{"name":"recall","dependencies":{}}\n`,
        "utf8",
      );
      writeFileSync(
        join(tempRoot, "dist", "extensions", "memory-lancedb", "package.json"),
        `{"name":"@recall/memory-lancedb","dependencies":{"root-owned-test-dep":"^1.0.0"}}\n`,
        "utf8",
      );
      writeFileSync(
  ...
  it("still requires root deps for root-owned installed chunks", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "recall-root-owned-installed-missing-"));
...
describe.skipIf(process.platform === "win32")("collectSkillShellScriptExecutableErrors", () => {
  it("flags non-executable shell scripts under skills/*/scripts", () => {
    const root = mkdtempSync(join(tmpdir(), "recall-release-check-"));
...
describe("collectForbiddenPackPaths", () => {
  ...
  it("blocks legacy runtime dependency stamps from npm pack output", () => {
    expect(
      collectForbiddenPackPaths([
        ...
        "dist/extensions/codex/.recall-runtime-deps-backup-node_modules-old/zod/index.js",
        "dist/extensions/discord/.recall-runtime-deps-stamp.json",
      ]),
    ).toEqual([
      "dist/extensions/browser/.Recall-Install-Stage/package.json",
      "dist/extensions/codex/.recall-runtime-deps-backup-node_modules-old/zod/index.js",
  ...
  it("blocks root dist chunks that still reference private qa lab sources", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "recall-release-private-qa-"));
  ...
  it("blocks private QA paths in the generated dist inventory", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "recall-release-inventory-"));
...
describe("collectMissingPackPaths", () => {
  it("requires the shipped channel catalog, control ui, and optional bundled metadata", () => {
    ...
    for (const path of [
      ...
      bundledDistPluginFile("telegram", "recall.plugin.json"),
  ...
  it("runs postpublish package integrity checks against the packed install before publish", () => {
    ...
    try {
      const packageRoot = join(root, "recall");
      const distDir = join(packageRoot, "dist");
      mkdirSync(distDir, { recursive: true });
      writeFileSync(
        join(packageRoot, "package.json"),
        `${JSON.stringify({ name: "recall", version: "2026.5.14-beta.3", dependencies: {} })}\n`,
      );
      ...
      expect(
        collectPackedInstalledPackageVerificationErrors({
          ...
          installedBinaryVersion: "recall 2026.5.14-beta.3",
          packageRoot,
        }),
      ).toEqual([
...
describe("collectPackUnpackedSizeErrors", () => {
  ...
  it("flags oversized pack results that risk low-memory startup failures", () => {
    expect(
      ...
      "recall-2026.3.12.tgz unpackedSize 224002564 bytes (213.6 MiB) exceeds budget 211812352 bytes (202.0 MiB). Investigate duplicate channel shims, copied extension trees, or other accidental pack bloat before release.",
    ]);
  });
