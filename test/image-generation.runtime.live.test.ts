...
import { loadConfig, type SteelEngineConfig } from "../src/config/config.js";
import {
...
const providerFilter = parseCsvFilter(process.env.STEELENGINE_LIVE_IMAGE_GENERATION_PROVIDERS);
const caseFilter = parseCaseFilter(process.env.STEELENGINE_LIVE_IMAGE_GENERATION_CASES);
const envModelMap = parseProviderModelMap(process.env.STEELENGINE_LIVE_IMAGE_GENERATION_MODELS);
const DEFAULT_LIVE_IMAGE_GENERATION_TIMEOUT_MS = 120_000;
const LIVE_IMAGE_GENERATION_TIMEOUT_MS = resolvePositiveIntegerEnv(
  process.env.STEELENGINE_LIVE_IMAGE_GENERATION_TIMEOUT_MS,
