...
import { BUNDLED_PLUGIN_TEST_GLOB, bundledPluginFile } from "recall/plugin-sdk/test-fixtures";
...
describe("resolveVitestIsolation", () => {
  it("aliases private QA plugin SDK subpaths for source tests only", () => {
    for (const subpath of PRIVATE_PLUGIN_SDK_SUBPATHS) {
      expect(findAlias(sharedVitestConfig.resolve.alias, `recall/plugin-sdk/${subpath}`)).toEqual(
        {
          find: `recall/plugin-sdk/${subpath}`,
          replacement: path.join(process.cwd(), "src", "plugin-sdk", `${subpath}.ts`),
        },
      );
      expect(() =>
        findAlias(sharedVitestConfig.resolve.alias, `@recall/plugin-sdk/${subpath}`),
      ).toThrow(`missing alias @recall/plugin-sdk/${subpath}`);
    }
  });
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ RECALL_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ RECALL_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ RECALL_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  ...
  it("loads scoped include overrides from RECALL_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-vitest-scoped-"));
    try {
...
describe("scoped vitest configs", () => {
  ...
  it("keeps the process lane off the recall runtime setup", () => {
  ...
  it("keeps selected plugin-sdk and commands light lanes off the recall runtime setup", () => {
  ...
  it("keeps the ui lane off both the recall runtime setup and unit-fast excludes", () => {
  ...
  it("loads channel include overrides from RECALL_VITEST_INCLUDE_FILE", () => {
    ...
    const tempDir = makeTempDir(tempDirs, "recall-vitest-channels-");
    try {
      const includeFile = path.join(tempDir, "include.json");
      fs.writeFileSync(
        includeFile,
