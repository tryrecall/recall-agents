...
import { BUNDLED_PLUGIN_TEST_GLOB, bundledPluginFile } from "steelengine/plugin-sdk/test-fixtures";
...
describe("resolveVitestIsolation", () => {
  it("aliases private QA plugin SDK subpaths for source tests only", () => {
    for (const subpath of PRIVATE_PLUGIN_SDK_SUBPATHS) {
      expect(findAlias(sharedVitestConfig.resolve.alias, `steelengine/plugin-sdk/${subpath}`)).toEqual(
        {
      ...
      expect(() =>
        findAlias(sharedVitestConfig.resolve.alias, `@steelengine/plugin-sdk/${subpath}`),
      ).toThrow(`missing alias @steelengine/plugin-sdk/${subpath}`);
    }
  });

  it("aliases private core packages to source for clean checkout tests", () => {
    expect(findAlias(sharedVitestConfig.resolve.alias, "@steelengine/media-core/mime")).toEqual({
      find: "@steelengine/media-core/mime",
      replacement: path.join(process.cwd(), "packages", "media-core", "src", "mime.ts"),
    });
    expect(findAlias(sharedVitestConfig.resolve.alias, "@steelengine/acp-core/runtime/types")).toEqual(
      {
        find: "@steelengine/acp-core/runtime/types",
        replacement: path.join(process.cwd(), "packages", "acp-core", "src", "runtime", "types.ts"),
      },
    );
    expect(findAlias(sharedVitestConfig.resolve.alias, "@steelengine/retry")).toEqual({
      find: "@steelengine/retry",
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ STEELENGINE_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ STEELENGINE_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ STEELENGINE_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  ...
  it("loads scoped include overrides from STEELENGINE_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-vitest-scoped-"));
    try {
  ...
  it("keeps shared gateway include files inside their actual child projects", () => {
    ...
    try {
      ...
      const env = { STEELENGINE_VITEST_INCLUDE_FILE: includeFile };
...
describe("scoped vitest configs", () => {
  ...
  it("keeps process, runtime config, and tooling lanes off the steelengine runtime setup", () => {
  ...
  it("keeps selected plugin-sdk and commands light lanes off the steelengine runtime setup", () => {
  ...
  it("keeps the ui lane off both the steelengine runtime setup and unit-fast excludes", () => {
  ...
  it("loads channel include overrides from STEELENGINE_VITEST_INCLUDE_FILE", () => {
    const tempDirs: string[] = [];
    const tempDir = makeTempDir(tempDirs, "steelengine-vitest-channels-");
    try {
      const includeFile = path.join(tempDir, "include.json");
      fs.writeFileSync(
        includeFile,
