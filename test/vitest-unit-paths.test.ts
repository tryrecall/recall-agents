...
describe("isUnitConfigTestFile", () => {
  ...
  it("rejects files excluded from the unit config", () => {
    ...
    expect(isUnitConfigTestFile("src/infra/steelengine-exec-env.test.ts")).toBe(false);
    expect(
      isUnitConfigTestFile(bundledPluginFile("matrix", "src/migration-snapshot.test.ts")),
    ).toBe(false);
    expect(isUnitConfigTestFile("src/infra/steelengine-root.test.ts")).toBe(false);
