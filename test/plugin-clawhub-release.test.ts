...
import {
  ...
  buildSteelEngineReleaseClawHubRuntimeState,
  parseSteelEngineReleaseClawHubPlanArgs,
} from "../scripts/lib/steelengine-release-clawhub-plan.ts";
import {
...
import {
  ...
  STEELENGINE_PLUGIN_NPM_REPOSITORY_URL,
...
function createClawPackBytes(
  ...
  const packageJson = JSON.stringify({
    name: packageName,
    version,
    steelengine: { release: { publishToClawHub: true } },
  });
  ...
  return gzipSync(
    Buffer.concat([
      ...packageJsonEntries,
      entry("package/steelengine.plugin.json", JSON.stringify({ id: "demo-plugin" })),
      Buffer.alloc(1024),
    ]),
  );
}
...
describe("collectClawHubPublishablePluginPackages", () => {
  it("requires the ClawHub external plugin contract", () => {
    ...
    expect(() => collectClawHubPublishablePluginPackages(repoDir)).toThrow(
      "steelengine.compat.pluginApi is required for external code plugin packages.",
    );
  });
  ...
  it("validates only selected package names when filters are provided", () => {
    ...
    expect(
      collectClawHubPublishablePluginPackages(repoDir, {
        packageNames: ["@steelengine/demo-plugin"],
      }).map((plugin) => plugin.packageName),
    ).toEqual(["@steelengine/demo-plugin"]);
  });
...
describe("SteelEngine dual-published plugin metadata", () => {
  const dualPublishedPlugins = [
    {
      extensionId: "cohere",
      packageName: "@steelengine/cohere-provider",
      install: {
    ...
    },
    {
      ...
      install: {
        clawhubSpec: "clawhub:@steelengine/diagnostics-prometheus",
        defaultChoice: "npm",
        minHostVersion: ">=2026.4.25",
        npmSpec: "@steelengine/diagnostics-prometheus",
      },
    },
    {
      extensionId: "gmi",
      packageName: "@steelengine/gmi-provider",
      install: {
  ...
  it("keeps dual-published plugins selectable through both ClawHub and npm release paths", () => {
    ...
    for (const plugin of dualPublishedPlugins) {
      ...
      expect(packageJson.steelengine?.install).toEqual(plugin.install);
      expect(packageJson.steelengine?.release).toEqual({
        publishToClawHub: true,
        publishToNpm: true,
      });
    }
...
describe("collectClawHubVersionGateErrors", () => {
  it("requires a version bump when a publishable plugin changes", () => {
    ...
    expect(errors).toEqual([
      "@steelengine/demo-plugin@2026.4.1: changed publishable plugin still has the same version in package.json.",
    ]);
  });
...
describe("collectPluginClawHubReleasePlan", () => {
  it("bounds parallel ClawHub package-state reads and preserves plan order", async () => {
    ...
    const packageNames = ["demo-plugin", ...extraExtensionIds].map(
      (extensionId) => `@steelengine/${extensionId}`,
    );
  ...
  it("rejects stale required dependencies before querying ClawHub", async () => {
    ...
    await expect(
      collectPluginClawHubReleasePlan({
        rootDir: repoDir,
        selection: ["@steelengine/demo-plugin"],
      ...
      '@steelengine/demo-plugin@2026.4.1: demo-runtime must match npm latest for release; found "1.2.3", latest is "1.2.4".',
    );
  });
  ...
  it("accepts required dependencies matching npm latest", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@steelengine/demo-plugin@2026.4.1": 404,
      },
    });
    ...
    expect(plan.candidates.map((plugin) => plugin.packageName)).toEqual(["@steelengine/demo-plugin"]);
  });
  ...
  it("fails closed when npm latest cannot be resolved", async () => {
    ...
    await expect(
      ...
      "@steelengine/demo-plugin@2026.4.1: could not resolve npm latest for demo-runtime: registry unavailable",
    );
  });
  ...
  it("keeps existing trusted packages with missing versions as normal candidates", async () => {
    ...
    expect(requests).toEqual([
      "/api/v1/packages/%40steelengine%2Fdemo-plugin",
      "/api/v1/packages/%40steelengine%2Fdemo-plugin/trusted-publisher",
      "/api/v1/packages/%40steelengine%2Fdemo-plugin/versions/2026.4.1",
    ]);
  });
  ...
  it("cancels unused ClawHub package and version response bodies", async () => {
    ...
    const fetchImpl: typeof fetch = async (input) => {
      ...
      if (url.pathname === "/api/v1/packages/%40steelengine%2Fdemo-plugin") {
        return new Response(
      ...
      if (url.pathname === "/api/v1/packages/%40steelengine%2Fdemo-plugin/trusted-publisher") {
        return new Response(
          JSON.stringify({
            trustedPublisher: {
              repository: "steelengine/steelengine",
      ...
      if (url.pathname === "/api/v1/packages/%40steelengine%2Fdemo-plugin/versions/2026.4.1") {
        return new Response(
          new ReadableStream<Uint8Array>({
            cancel() {
  ...
  it("retries a rate-limited trusted publisher lookup", async () => {
    ...
    const fetchImpl: typeof fetch = async (input) => {
      ...
      if (pathname === "/api/v1/packages/%40steelengine%2Fdemo-plugin") {
        return new Response("{}", { status: 200 });
      }
      if (pathname === "/api/v1/packages/%40steelengine%2Fdemo-plugin/trusted-publisher") {
      ...
      if (pathname === "/api/v1/packages/%40steelengine%2Fdemo-plugin/versions/2026.4.1") {
  ...
  it("preserves ClawHub response details after package retries are exhausted", async () => {
    ...
    await expect(
      ...
      "Failed to query ClawHub package @steelengine/demo-plugin: 503 Rate limit temporarily unavailable [retry-after=1; x-request-id=request-123]",
    );
  ...
  it.each([
    ...
    async ({ responseBody, expectedDetail }) => {
      ...
      await expect(
        ...
        `Failed to query ClawHub package @steelengine/demo-plugin: 503 ${expectedDetail}`,
      );
    },
  );
  ...
  it("routes missing package rows to bootstrap candidates instead of normal candidates", async () => {
    ...
    expect(plan.bootstrapCandidates[0]).toMatchObject({
      ...
      artifactName: "clawhub-package-steelengine-demo-plugin-2026.4.1",
      packageName: "@steelengine/demo-plugin",
      version: "2026.4.1",
    });
    expect(plan.missingTrustedPublisher).toStrictEqual([]);
  ...
  it("skips versions that already exist on ClawHub", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@steelengine/demo-plugin@2026.4.1": 200,
      },
    });
  ...
  it("plans selected packages without validating unrelated publishable packages", async () => {
    ...
    expect(plan.candidates.map((plugin) => plugin.artifactName)).toEqual([
      "clawhub-package-steelengine-demo-plugin-2026.4.1",
    ]);
  });
});

describe("buildSteelEngineReleaseClawHubPlan", () => {
  it("emits a dispatch plan that keeps ClawHub children on the release tag", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      packages: {
        ...
        "@steelengine/demo-two": {
          status: 404,
        },
        "@steelengine/demo-three": {
          status: 200,
          body: {
            package: {},
            owner: {},
      ...
      versions: {
        "@steelengine/demo-plugin@2026.4.1": 404,
        "@steelengine/demo-three@2026.4.1": 404,
      },
    });

    const plan = await buildSteelEngineReleaseClawHubPlan(
      {
    ...
    expect(plan.normal).toEqual({
      ...
      shouldDispatch: true,
      packages: ["@steelengine/demo-plugin"],
      inputs: {
        publish_scope: "selected",
        plugins: "@steelengine/demo-plugin",
    ...
    expect(plan.bootstrap).toEqual({
      ...
      shouldDispatch: true,
      packages: ["@steelengine/demo-two", "@steelengine/demo-three"],
      inputs: {
        bootstrap_workflow_sha: "d".repeat(40),
        ref: "a".repeat(40),
        release_tag: "v2026.4.1-beta.1",
        plugins: "@steelengine/demo-two,@steelengine/demo-three",
    ...
    expect(plan.summary).toEqual({
      ...
      normalPlugins: "@steelengine/demo-plugin",
      bootstrapPlugins: "@steelengine/demo-two,@steelengine/demo-three",
      missingTrustedPlugins: "@steelengine/demo-three",
    });
  ...
  it("routes already-published packages missing trusted publisher config to bootstrap repair", async () => {
    ...
    expect(plan.summary).toMatchObject({
      ...
      bootstrapCount: 1,
      missingTrustedPublisherCount: 1,
      bootstrapPlugins: "@steelengine/demo-plugin",
      missingTrustedPlugins: "@steelengine/demo-plugin",
    });
  });
  ...
  it("requires an exact lowercase release SHA for bootstrap targeting", () => {
    ...
    expect(() => parseSteelEngineReleaseClawHubPlanArgs(baseArgs)).toThrow(
      "--release-sha is required.",
    );
    expect(() =>
      parseSteelEngineReleaseClawHubPlanArgs([...baseArgs, "--release-sha", "ABCDEF"]),
  ...
  it("requires an exact parent release run attempt for bootstrap approval binding", () => {
    ...
    expect(() => parseSteelEngineReleaseClawHubPlanArgs(args)).toThrow(
      "--release-publish-run-attempt is required.",
    );
    expect(() =>
      parseSteelEngineReleaseClawHubPlanArgs([...args, "--release-publish-run-attempt", "0"]),
...
describe("runPluginClawHubReleaseCheck", () => {
  it("rejects stale required dependencies", async () => {
    ...
    await expect(
      runPluginClawHubReleaseCheck(["--plugins", "@steelengine/demo-plugin"], {
        rootDir: repoDir,
        resolveLatestVersion: () => "1.2.4",
      }),
    ).rejects.toThrow(
...
describe("buildSteelEngineReleaseClawHubRuntimeState", () => {
  it("includes the normal ClawHub run in verifier args when the release waits for it", () => {
    const state = buildSteelEngineReleaseClawHubRuntimeState({
      repository: "steelengine/steelengine",
      waitForClawHub: true,
    ...
    expect(state.proofLines.normal).toBe(
      "- plugin ClawHub publish: https://github.com/steelengineai/recall-agents/actions/runs/111",
    );
  ...
  it("includes a completed bootstrap run even when there is no normal ClawHub run", () => {
    ...
    expect(state.proofLines.bootstrap).toBe(
      "- plugin ClawHub bootstrap: https://github.com/steelengineai/recall-agents/actions/runs/222",
    );
  });
  ...
  it("skips ClawHub verification for non-awaited incomplete runs while keeping proof links", () => {
    ...
    expect(state.proofLines.normal).toBe(
      "- plugin ClawHub publish: dispatched separately, not awaited by this proof: https://github.com/steelengineai/recall-agents/actions/runs/111",
    );
    expect(state.proofLines.bootstrap).toBe(
      "- plugin ClawHub bootstrap: dispatched separately, not awaited by this proof: https://github.com/steelengineai/recall-agents/actions/runs/222",
    );
  });
...
if [[ "\${1:-}" == "package" && "\${2:-}" == "pack" ]]; then
  ...
  pack_path="$pack_destination/steelengine-demo-plugin-2026.4.1.tgz"
  printf 'fake tgz\\n' > "$pack_path"
  printf '{"path":"%s","name":"@steelengine/demo-plugin","version":"2026.4.1"}\\n' "$pack_path"
fi
exit 0
`,
    );
...
`,
    ...
    execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          STEELENGINE_CLAWHUB_MANUAL_OVERRIDE_REASON:
...
`,
    ...
    expect(existsSync(join(outputDir, "steelengine-demo-plugin-2026.4.1.tgz"))).toBe(true);
    const invocations = readFileSync(markerPath, "utf8");
  ...
  it("rejects duplicate normalized paths before invoking the ClawHub CLI", () => {
    ...
    const tgzBytes = createClawPackBytes("@steelengine/demo-plugin", "2026.4.1", {
      ...
      execFileSync(
        ...
        [join(process.cwd(), "scripts/plugin-clawhub-publish.sh"), "--validate-packed", tgzPath],
        {
          ...
          env: {
            ...
            EXPECTED_CLAWHUB_PACKAGE_NAME: "@steelengine/demo-plugin",
...
if [[ " $* " == *" --dry-run "* ]]; then
  printf '{"name":"@steelengine/demo-plugin","version":"2026.4.1"}\\n'
  exit 0
fi
attempts=0
    ...
    execFileSync(
      ...
      [join(process.cwd(), "scripts/plugin-clawhub-publish.sh"), "--publish-packed", tgzPath],
      {
        ...
        env: {
          ...
          EXPECTED_CLAWHUB_PACKAGE_NAME: "@steelengine/demo-plugin",
          EXPECTED_CLAWHUB_PACKAGE_VERSION: "2026.4.1",
          STEELENGINE_CLAWHUB_PUBLISH_ATTEMPTS: "2",
          STEELENGINE_CLAWHUB_PUBLISH_RETRY_DELAY_SECONDS: "1",
          PACKAGE_DIR: "extensions/demo-plugin",
          PATH: `${binDir}${delimiter}${process.env.PATH ?? ""}`,
        },
      },
    ...
    execFileSync(
      ...
      [join(process.cwd(), "scripts/plugin-clawhub-publish.sh"), "--publish-packed", tgzPath],
      {
        ...
        env: {
          ...
          EXPECTED_CLAWHUB_PACKAGE_VERSION: "2026.4.1",
          STEELENGINE_CLAWHUB_PUBLISH_ATTEMPTS: "2",
          STEELENGINE_CLAWHUB_PUBLISH_ATTEMPT_TIMEOUT_SECONDS: "1",
          STEELENGINE_CLAWHUB_PUBLISH_RETRY_DELAY_SECONDS: "1",
          PACKAGE_DIR: "extensions/demo-plugin",
          PATH: `${binDir}${delimiter}${process.env.PATH ?? ""}`,
        },
...
function createTempPluginRepo(
  ...
  const repoDir = makeTempRepoRoot(tempDirs, "steelengine-clawhub-release-");
  const extensionId = options.extensionId ?? "demo-plugin";
  const extensionIds = [extensionId, ...(options.extraExtensionIds ?? [])];

  writeFileSync(
    join(repoDir, "package.json"),
    JSON.stringify({ name: "steelengine-test-root", type: "module" }, null, 2),
  );
  ...
  for (const currentExtensionId of extensionIds) {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          name: `@steelengine/${currentExtensionId}`,
          version: "2026.4.1",
          type: "module",
          repository: {
            type: "git",
          ...
          steelengine: {
            ...
            install: {
              npmSpec: `@steelengine/${currentExtensionId}`,
            },
            release: {
