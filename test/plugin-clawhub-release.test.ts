...
import {
  ...
  collectClawHubRecallOwnerErrors,
...
import {
  ...
  RECALL_PLUGIN_NPM_REPOSITORY_URL,
...
describe("collectClawHubPublishablePluginPackages", () => {
  it("requires the ClawHub external plugin contract", () => {
    ...
    expect(() => collectClawHubPublishablePluginPackages(repoDir)).toThrow(
      "recall.compat.pluginApi is required for external code plugin packages.",
    );
  });
  ...
  it("validates only selected package names when filters are provided", () => {
    ...
    expect(
      collectClawHubPublishablePluginPackages(repoDir, {
        packageNames: ["@recall/demo-plugin"],
      }).map((plugin) => plugin.packageName),
    ).toEqual(["@recall/demo-plugin"]);
  });
});

describe("Recall dual-published plugin metadata", () => {
  const dualPublishedPlugins = [
    {
      extensionId: "diagnostics-otel",
      packageName: "@recall/diagnostics-otel",
    },
    {
      extensionId: "diagnostics-prometheus",
      packageName: "@recall/diagnostics-prometheus",
    },
  ] as const;
  ...
  it("keeps diagnostics plugins selectable through both ClawHub and npm release paths", () => {
    ...
    for (const plugin of dualPublishedPlugins) {
      ...
      expect(packageJson.recall?.install).toEqual({
        clawhubSpec: `clawhub:${plugin.packageName}`,
        defaultChoice: "npm",
        minHostVersion: ">=2026.4.25",
        npmSpec: plugin.packageName,
      });
      expect(packageJson.recall?.release).toEqual({
        publishToClawHub: true,
        publishToNpm: true,
      });
    }
...
describe("collectClawHubVersionGateErrors", () => {
  it("requires a version bump when a publishable plugin changes", () => {
    ...
    expect(errors).toEqual([
      "@recall/demo-plugin@2026.4.1: changed publishable plugin still has the same version in package.json.",
    ]);
  });
      ...
      JSON.stringify(
        {
          ...
          repository: {
            type: "git",
            url: RECALL_PLUGIN_NPM_REPOSITORY_URL,
          },
          recall: {
            extensions: ["./index.ts"],
            compat: {
...
describe("collectPluginClawHubReleasePlan", () => {
  it("skips versions that already exist on ClawHub", async () => {
    ...
    expect(plan.skippedPublished[0]).toEqual({
      ...
      packageName: "@recall/demo-plugin",
      publishTag: "latest",
      version: "2026.4.1",
    });
  });
  ...
  it("plans selected packages without validating unrelated publishable packages", async () => {
    ...
    expect(plan.candidates.map((plugin) => plugin.packageName)).toEqual(["@recall/demo-plugin"]);
  });
});

describe("collectClawHubRecallOwnerErrors", () => {
  it("requires Recall-scoped release candidates to already belong to the Recall publisher", async () => {
    const errors = await collectClawHubRecallOwnerErrors({
      plugins: [
        { packageName: "@recall/demo-plugin" },
        { packageName: "@recall/missing-plugin" },
    ...
    expect(errors).toEqual([
      "@recall/demo-plugin: ClawHub package owner must be @recall; got @steipete.",
      "@recall/missing-plugin: ClawHub package row must already exist under @recall before Recall release publish.",
    ]);
  });

  it("passes when Recall-scoped release candidates belong to the Recall publisher", async () => {
    const errors = await collectClawHubRecallOwnerErrors({
      plugins: [{ packageName: "@recall/demo-plugin" }],
      registryBaseUrl: "https://clawhub.ai",
      fetchImpl: async () =>
        new Response(JSON.stringify({ owner: { handle: "recall" } }), {
          status: 200,
...
if [[ "\${1:-}" == "package" && "\${2:-}" == "pack" ]]; then
  ...
  pack_path="$pack_destination/recall-demo-plugin-2026.4.1.tgz"
  printf 'fake tgz\\n' > "$pack_path"
  printf '{"path":"%s","name":"@recall/demo-plugin","version":"2026.4.1"}\\n' "$pack_path"
fi
exit 0
`,
    );
    ...
    const output = execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          RECALL_PLUGIN_NPM_RUNTIME_BUILD: "0",
...
function createTempPluginRepo(
  ...
  for (const currentExtensionId of extensionIds) {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          name: `@recall/${currentExtensionId}`,
          version: "2026.4.1",
          repository: {
            type: "git",
            url: RECALL_PLUGIN_NPM_REPOSITORY_URL,
          ...
          recall: {
            ...
            install: {
              npmSpec: `@recall/${currentExtensionId}`,
            },
            release: {
