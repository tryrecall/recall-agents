...
describe("plugin npm runtime build planning", () => {
  it("plans package-local runtime entries for every publishable plugin package", () => {
    ...
    for (const plan of resolvedPlans) {
      ...
      expect(plan.packagePeerMetadata.peerDependencies.steelengine).toBe(
        plan.packageJson.steelengine.compat.pluginApi,
      );
      expect(plan.packagePeerMetadata.peerDependenciesMeta.steelengine.optional).toBe(true);
    }
  });
