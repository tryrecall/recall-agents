...
describe("Mantis Telegram Desktop proof workflow", () => {
  ...
  it("releases Telegram Desktop proof leases left by interrupted agents", () => {
    ...
    expect(codexStep.env?.STEELENGINE_QA_CREDENTIAL_OWNER_ID).toContain(
      "mantis-telegram-desktop-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    expect(workflowStep("Prepare Codex user").run).toContain("STEELENGINE_QA_CREDENTIAL_OWNER_ID");
    ...
    expect(cleanupStep.env?.STEELENGINE_QA_CONVEX_SECRET_CI).toContain(
      "secrets.STEELENGINE_QA_CONVEX_SECRET_CI",
    );
    expect(cleanupStep.env?.STEELENGINE_QA_CONVEX_SITE_URL).toContain(
      "secrets.STEELENGINE_QA_CONVEX_SITE_URL",
    );
  ...
  it("uses the SteelEngine Mantis mention as the comment trigger", () => {
    const workflow = readFileSync(WORKFLOW, "utf8");
    const liveWorkflow = readFileSync(LIVE_WORKFLOW, "utf8");
    expect(workflow).toContain("@steelengine-mantis");
    expect(workflow).toContain("/steelengine-mantis");
  ...
  it("keeps Telegram Desktop proof credentials out of the generic qa-lab API", () => {
    ...
    expect(readFileSync(TELEGRAM_PROOF_SKILL, "utf8")).toContain(
      "STEELENGINE_TELEGRAM_USER_PROOF_CMD",
    );
  ...
  it("installs local proof tools before the Codex agent runs", () => {
    ...
    expect(install.run).toContain("/usr/local/bin/steelengine-telegram-user-crabbox-proof");
    ...
    expect(agent.env?.STEELENGINE_TELEGRAM_USER_DRIVER_SCRIPT).toBe(
      "${{ github.workspace }}/scripts/e2e/telegram-user-driver.py",
    );
    expect(agent.env?.STEELENGINE_TELEGRAM_USER_PROOF_CMD).toBe(
      "/usr/local/bin/steelengine-telegram-user-crabbox-proof",
    );
    expect(agent.env?.STEELENGINE_TELEGRAM_USER_CRABBOX_BIN).toBe("/usr/local/bin/crabbox");
    expect(agent.env?.CRABBOX_COORDINATOR).toContain(
      "secrets.CRABBOX_COORDINATOR || secrets.STEELENGINE_QA_MANTIS_CRABBOX_COORDINATOR",
    );
    expect(agent.env?.CRABBOX_COORDINATOR_TOKEN).toContain(
      "secrets.CRABBOX_COORDINATOR_TOKEN || secrets.STEELENGINE_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN",
    );

    const prepare = workflowStep("Prepare Codex user");
    expect(prepare.run).toContain(
      "STEELENGINE_TELEGRAM_USER_CRABBOX_BIN STEELENGINE_TELEGRAM_USER_CRABBOX_PROVIDER STEELENGINE_TELEGRAM_USER_DRIVER_SCRIPT STEELENGINE_TELEGRAM_USER_PROOF_CMD",
    );
    expect(prepare.run).toContain("MANTIS_CANDIDATE_TRUST");

    const prompt = readFileSync(PROMPT, "utf8");
    expect(prompt).toContain("$STEELENGINE_TELEGRAM_USER_PROOF_CMD");
