...
function publishedUpgradeSurvivorLane(
  ...
  return {
    command: `STEELENGINE_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/${name}" STEELENGINE_UPGRADE_SURVIVOR_BASELINE_SPEC='${baselineSpec}' ${
      scenario ? `STEELENGINE_UPGRADE_SURVIVOR_SCENARIO='${scenario}' ` : ""
...
function updateMigrationLane(name: string, baselineSpec: string): ReturnType<typeof summarizeLane> {
  return {
    command: `STEELENGINE_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/${name}" STEELENGINE_UPGRADE_SURVIVOR_BASELINE_SPEC='${baselineSpec}' STEELENGINE_UPGRADE_SURVIVOR_SCENARIO='plugin-deps-cleanup' STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-migration`,
    imageKind: "bare",
    live: false,
    name,
    resources: ["docker", "npm"],
...
function bundledPluginSweepLane(index: number): ReturnType<typeof summarizeLane> {
  return {
    command: `STEELENGINE_BUNDLED_PLUGIN_SWEEP_TOTAL=24 STEELENGINE_BUNDLED_PLUGIN_SWEEP_INDEX=${index} STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:bundled-plugin-install-uninstall`,
...
describe("scripts/lib/docker-e2e-plan", () => {
  ...
  it("routes live Docker scripts through the nested trusted release harness", () => {
    ...
    const tempRoot = tempDirs.make("steelengine-release-harness-");
    ...
    expect(sourceLane?.command).toContain(
      'harness="${STEELENGINE_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.}"',
    );
    ...
    const laneJson = execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          STEELENGINE_DOCKER_E2E_REPO_ROOT: tempRoot,
        },
      },
    );
    ...
    expect(lane.command).toContain(
      'harness="${STEELENGINE_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.release-harness}"',
    );
  ...
  it("plans package-backed Compose and package artifact proofs", () => {
    ...
    expect(plan.lanes.map(summarizeLane)).toEqual([
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:compose-setup",
        imageKind: "functional",
        live: false,
        name: "compose-setup",
      ...
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:package-install",
        imageKind: "bare",
        live: false,
  ...
  it("splits release-path package and plugin chunks across shorter CI jobs", () => {
    ...
    expect(packageUpdateCore.lanes.map(summarizeLane)).toEqual([
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:doctor-switch",
        imageKind: "bare",
        live: false,
        name: "doctor-switch",
        resources: ["docker", "npm"],
      ...
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-channel-switch",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:skill-install",
        imageKind: "bare",
        live: false,
        name: "skill-install",
        resources: ["docker", "npm"],
      ...
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:upgrade-survivor",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:published-upgrade-survivor",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:root-managed-vps-upgrade",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-restart-auth",
        imageKind: "bare",
        live: false,
    ...
    expect(pluginsRuntimeServices.lanes.map(summarizeLane)).toEqual([
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:cron-mcp-cleanup",
        imageKind: "functional",
        live: false,
      ...
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-web-search-minimal",
        imageKind: "functional",
        live: false,
  ...
  it("expands the published upgrade survivor lane across deduped baselines", () => {
    const plan = planFor({
      ...
      upgradeSurvivorBaselines:
        "steelengine@2026.4.29 2026.4.23 steelengine@2026.4.23 steelengine@2026.3.13-1",
    });

    expect(plan.lanes.map(summarizeLane)).toEqual([
      publishedUpgradeSurvivorLane("published-upgrade-survivor-2026.4.29", "steelengine@2026.4.29"),
      publishedUpgradeSurvivorLane("published-upgrade-survivor-2026.4.23", "steelengine@2026.4.23"),
      publishedUpgradeSurvivorLane(
        "published-upgrade-survivor-2026.3.13-1",
        "steelengine@2026.3.13-1",
      ),
    ]);
  });
  ...
  it("expands reported upgrade issue scenarios", () => {
    ...
    expect(plan.lanes.map((lane) => lane.name)).toEqual([
      "published-upgrade-survivor-2026.4.29",
      "published-upgrade-survivor-2026.4.29-acpx-steelengine-tools-bridge",
      "published-upgrade-survivor-2026.4.29-feishu-channel",
  ...
  it("omits trusted-current scenarios unsupported by a frozen target harness", () => {
    const targetRoot = tempDirs.make("steelengine-frozen-upgrade-harness-");
    ...
    expect(plan.omittedUnsupportedLanes).toEqual([
      "published-upgrade-survivor-2026.6.11-acpx-steelengine-tools-bridge",
    ]);
  });

  it("reads content-addressed scenario catalogs from pre-command frozen targets", () => {
    const targetRoot = tempDirs.make("steelengine-legacy-frozen-upgrade-harness-");
    const assertionsFile = join(targetRoot, "scripts/e2e/lib/upgrade-survivor/assertions.mjs");
  ...
  it("omits survivor lanes when the target exposes none of the requested scenarios", () => {
    const targetRoot = tempDirs.make("steelengine-frozen-empty-upgrade-harness-");
  ...
  it("omits baseline-only survivor lanes when the target lacks the implicit base scenario", () => {
    const targetRoot = tempDirs.make("steelengine-frozen-no-base-upgrade-harness-");
    writeFrozenScenarioContract(targetRoot, ["unrelated"]);
  ...
  it("omits an unconfigured survivor lane when the target lacks the implicit base scenario", () => {
    const targetRoot = tempDirs.make("steelengine-frozen-default-base-harness-");
    writeFrozenScenarioContract(targetRoot, ["unrelated"]);
  ...
  it("reports an unsupported survivor lane beside runnable selected lanes", () => {
    const targetRoot = tempDirs.make("steelengine-frozen-mixed-upgrade-harness-");
    writeFrozenScenarioContract(targetRoot, ["unrelated"]);
  ...
  it("reports an explicitly selected expanded survivor lane as unsupported", () => {
    const targetRoot = tempDirs.make("steelengine-frozen-expanded-upgrade-harness-");
    writeFrozenScenarioContract(targetRoot, ["unrelated"]);
  ...
  it("omits unsupported scenario-only survivor lanes without explicit baselines", () => {
    const targetRoot = tempDirs.make("steelengine-frozen-scenario-only-harness-");
    writeFrozenScenarioContract(targetRoot, ["unrelated"]);
  ...
  it("does not fall back to base when an unsupported scenario is baseline-incompatible", () => {
    const targetRoot = tempDirs.make("steelengine-frozen-incompatible-scenario-harness-");
    writeFrozenScenarioContract(targetRoot, ["unrelated"]);

    const plan = planFor({
      selectedLaneNames: ["published-upgrade-survivor"],
      upgradeSurvivorBaselines: "2026.4.21",
      upgradeSurvivorScenarios: "acpx-steelengine-tools-bridge",
      upgradeSurvivorTargetRoot: targetRoot,
    });
  ...
  it("fails closed when an unknown legacy scenario catalog lacks the command", () => {
    const targetRoot = tempDirs.make("steelengine-frozen-failed-scenario-harness-");
    const assertionsFile = join(targetRoot, "scripts/e2e/lib/upgrade-survivor/assertions.mjs");
    mkdirSync(dirname(assertionsFile), { recursive: true });
    writeFileSync(
      assertionsFile,
  ...
  it("fails closed when a frozen target scenario command returns non-JSON output", () => {
    const targetRoot = tempDirs.make("steelengine-frozen-non-json-scenario-harness-");
    const assertionsFile = join(targetRoot, "scripts/e2e/lib/upgrade-survivor/assertions.mjs");
    mkdirSync(dirname(assertionsFile), { recursive: true });
  ...
  it("fails closed when a frozen target scenario command returns an invalid catalog", () => {
    const targetRoot = tempDirs.make("steelengine-frozen-invalid-scenario-harness-");
    const assertionsFile = join(targetRoot, "scripts/e2e/lib/upgrade-survivor/assertions.mjs");
    mkdirSync(dirname(assertionsFile), { recursive: true });
  ...
  it("does not inspect a frozen survivor contract for unrelated selected lanes", () => {
    const targetRoot = tempDirs.make("steelengine-frozen-unrelated-lane-harness-");
    const assertionsFile = join(targetRoot, "scripts/e2e/lib/upgrade-survivor/assertions.mjs");
    mkdirSync(dirname(assertionsFile), { recursive: true });
  ...
  it("skips plugin dependency cleanup for baselines without packaged plugin dirs", () => {
    ...
    expect(plan.lanes.map((lane) => lane.name)).toEqual([
      ...
      "published-upgrade-survivor-2026.4.22-acpx-steelengine-tools-bridge",
  ...
  it("expands update migration across baselines and cleanup scenarios", () => {
    ...
    expect(plan.lanes.map(summarizeLane)).toEqual([
      updateMigrationLane("update-migration-2026.4.29-plugin-deps-cleanup", "steelengine@2026.4.29"),
      updateMigrationLane("update-migration-2026.4.23-plugin-deps-cleanup", "steelengine@2026.4.23"),
    ]);
  });
  ...
  it("marks the aggregate Gemini CLI backend lane advisory for auth drift", () => {
    ...
    expect(lane.command).toContain("STEELENGINE_LIVE_CLI_BACKEND_ADVISORY=1");
    expect(lane.command).toContain("STEELENGINE_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP=1");
    expect(lane.command).toContain(
      "STEELENGINE_LIVE_CLI_BACKEND_MODEL=google-gemini-cli/gemini-3-flash-preview",
    );
  });
  ...
  it("plans Codex harness Docker-all lanes for API-key Testbox auth", () => {
    for (const name of ["live-codex-harness", "live-codex-bind"]) {
      ...
      expect(lane.command, name).toContain("STEELENGINE_LIVE_CODEX_HARNESS_AUTH=api-key");
  ...
  it("plans the Codex npm plugin live lane as package-backed OpenAI proof", () => {
    ...
    expect(plan.lanes.map(summarizeLane)).toEqual([
      {
        command:
          'STEELENGINE_SKIP_DOCKER_BUILD=1 bash -c \'harness="${STEELENGINE_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.}"; STEELENGINE_LIVE_DOCKER_REPO_ROOT="${STEELENGINE_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/e2e/codex-npm-plugin-live-docker.sh"\'',
        imageKind: "bare",
        live: true,
  ...
  it("plans the Codex on-demand onboarding lane as package-backed npm proof", () => {
    ...
    const lane = requireFirstLane(plan);
    expect(lane.command).toBe("STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:codex-on-demand");
  ...
  it("plans the Droid ACP bind live lane as Factory-auth proof", () => {
    ...
    const lane = requireFirstLane(plan);
    expect(lane.command).toBe(
      'STEELENGINE_LIVE_ACP_BIND_AGENT=droid STEELENGINE_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1 STEELENGINE_SKIP_DOCKER_BUILD=1 bash -c \'harness="${STEELENGINE_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.}"; STEELENGINE_LIVE_DOCKER_REPO_ROOT="${STEELENGINE_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/test-live-acp-bind-docker.sh"\'',
    );
    expect(lane.imageKind).toBeUndefined();
    expect(lane.live).toBe(true);
  ...
  it("plans Docker package scripts that were previously only directly runnable", () => {
    ...
    expect(plan.lanes.map(summarizeLane)).toEqual([
      ...
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:multi-node-update",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "STEELENGINE_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-telegram-live",
        imageKind: "bare",
        live: true,
  ...
  it("maps installer E2E to provider-specific package install lanes", () => {
    ...
    expect(plan.lanes.map(summarizeLane)).toEqual([
      {
        command:
          'STEELENGINE_INSTALL_TAG=beta STEELENGINE_E2E_MODELS=openai STEELENGINE_INSTALL_E2E_IMAGE=steelengine-install-e2e-openai:local STEELENGINE_INSTALL_E2E_AGENT_TOOL_SMOKE=0 STEELENGINE_INSTALL_E2E_OPENAI_MODEL=openai/gpt-5.4-mini STEELENGINE_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS=120 STEELENGINE_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS=120 bash -c \'harness="${STEELENGINE_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.}"; STEELENGINE_LIVE_DOCKER_REPO_ROOT="${STEELENGINE_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/test-install-sh-e2e-docker.sh"\'',
        imageKind: "bare",
        live: true,
      ...
      {
        command:
          'STEELENGINE_INSTALL_TAG=beta STEELENGINE_E2E_MODELS=anthropic STEELENGINE_INSTALL_E2E_IMAGE=steelengine-install-e2e-anthropic:local bash -c \'harness="${STEELENGINE_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.}"; STEELENGINE_LIVE_DOCKER_REPO_ROOT="${STEELENGINE_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/test-install-sh-e2e-docker.sh"\'',
        imageKind: "bare",
        live: true,
  ...
  it("rejects unknown selected lanes with the available lane names", () => {
    expect(() => planFor({ selectedLaneNames: ["missing-lane"] })).toThrow(
      /STEELENGINE_DOCKER_ALL_LANES unknown lane\(s\): missing-lane/u,
    );
  });
});
