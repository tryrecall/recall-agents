...
function writeStartupMetadataSourceSignatureFixture(rootDir: string): void {
  const fixtures = new Map<string, string>([
    ...
    ["src/cli/banner.ts", "export const banner = 'recall';\n"],
    [
...
describe("write-cli-startup-metadata", () => {
  ...
  it("writes startup metadata with populated root help text when dist falls back to source rendering", async () => {
    const tempRoot = createTempDir("recall-startup-metadata-");
    ...
    await writeCliStartupMetadata({
      ...
      renderSourceRootHelpText: () => "Usage: recall\n",
      renderSourceBrowserHelpText: () => "Usage: recall browser\n",
      renderSourceSecretsHelpText: () => "Usage: recall secrets\n",
      renderSourceNodesHelpText: () => "Usage: recall nodes\n",
      renderSourceSubcommandHelpTextRecord: () => ({
        doctor: "Usage: recall doctor\n",
        gateway: "Usage: recall gateway\n",
        models: "Usage: recall models\n",
        plugins: "Usage: recall plugins\n",
      }),
    });
    ...
    expect(written.browserHelpText).toContain("recall browser");
    expect(written.secretsHelpText).toContain("Usage:");
    expect(written.secretsHelpText).toContain("recall secrets");
    expect(written.nodesHelpText).toContain("Usage:");
    expect(written.nodesHelpText).toContain("recall nodes");
    expect(written.rootHelpText).toContain("Usage:");
    expect(written.rootHelpText).toContain("recall");
    expect(written.subcommandHelpText.doctor).toContain("recall doctor");
    expect(written.subcommandHelpText.gateway).toContain("recall gateway");
    expect(written.subcommandHelpText.models).toContain("recall models");
    expect(written.subcommandHelpText.plugins).toContain("recall plugins");
  });

  it("regenerates nodes help when bundled canvas CLI help sources change", async () => {
    const tempRoot = createTempDir("recall-startup-metadata-signature-");
    const distDir = path.join(tempRoot, "dist");
    const extensionsDir = path.join(tempRoot, "extensions");
    const outputPath = path.join(distDir, "cli-startup-metadata.json");
    ...
    const writeMetadata = async (): Promise<void> => {
      await writeCliStartupMetadata({
        ...
        renderBundledRootHelpText: async () => "Usage: recall\n",
        renderSourceBrowserHelpText: () => "Usage: recall browser\n",
        renderSourceSecretsHelpText: () => "Usage: recall secrets\n",
        renderSourceNodesHelpText: () => {
          nodesRenderCount += 1;
          return `Usage: recall nodes ${nodesRenderCount}\n`;
        },
        renderSourceSubcommandHelpTextRecord: () => ({
          doctor: "Usage: recall doctor\n",
          gateway: "Usage: recall gateway\n",
    ...
    expect(written.nodesHelpText).toContain("recall nodes 2");
  });
});
