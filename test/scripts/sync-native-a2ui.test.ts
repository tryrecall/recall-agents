...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(tmpdir(), "steelengine-native-a2ui-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("scripts/sync-native-a2ui.mjs", () => {
  ...
  it("requires Linux Canvas to embed the plugin-owned resources", async () => {
    ...
    await fs.writeFile(
      linuxConsumerFile,
      [
        'include_bytes!("../../../../apps/shared/SteelEngineKit/Sources/SteelEngineKit/Resources/CanvasA2UI/index.html");',
        'include_bytes!("../../../../apps/shared/SteelEngineKit/Sources/SteelEngineKit/Resources/CanvasA2UI/a2ui.bundle.js");',
      ].join("\n"),
    );
