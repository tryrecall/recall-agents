...
describe("check-memory-fd-repro", () => {
  ...
  it("rejects loose numeric environment limits before generating files", () => {
    expect(
      withEnv(
        {
          STEELENGINE_MEMORY_FD_REPRO_FILES: "17",
          STEELENGINE_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS: "0",
          STEELENGINE_MEMORY_FD_REPRO_SAMPLE_DELAY_MS: "0",
          STEELENGINE_MEMORY_FD_REPRO_SETTLE_DELAY_MS: String(MAX_TIMER_TIMEOUT_MS + 1),
          STEELENGINE_MEMORY_FD_REPRO_TIMEOUT_MS: String(MAX_TIMER_TIMEOUT_MS + 1),
        },
        () => parseArgs([]),
      ),
    ).toMatchObject({
    ...
    expect(() =>
      withEnv({ STEELENGINE_MEMORY_FD_REPRO_FILES: "17files" }, () => parseArgs([])),
    ).toThrow("STEELENGINE_MEMORY_FD_REPRO_FILES must be a non-negative integer");
    expect(() =>
      withEnv({ STEELENGINE_MEMORY_FD_REPRO_TIMEOUT_MS: "1e3" }, () => parseArgs([])),
    ).toThrow("STEELENGINE_MEMORY_FD_REPRO_TIMEOUT_MS must be a non-negative integer");
  });
  ...
  it("lets explicit CLI numeric flags override malformed inherited env defaults", () => {
    expect(
      withEnv(
        {
          STEELENGINE_MEMORY_FD_REPRO_FILES: "17files",
          STEELENGINE_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS: "4fds",
          STEELENGINE_MEMORY_FD_REPRO_TIMEOUT_MS: "1e3",
          STEELENGINE_MEMORY_FD_REPRO_SAMPLE_DELAY_MS: "soon",
          STEELENGINE_MEMORY_FD_REPRO_SETTLE_DELAY_MS: "later",
        },
        () =>
          parseArgs([
            "--files",
  ...
  it("stops parsing options after the argument terminator", () => {
    ...
    expect(
      withEnv({ STEELENGINE_MEMORY_FD_REPRO_FILES: "17" }, () => parseArgs(["--", "--unknown"])),
    ).toMatchObject({
      fileCount: 17,
    });
  });
  ...
  it("writes an offline FTS-only memory search config for repro indexing", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-memory-fd-config-"));
    try {
