...
describe("scripts/lib/plugin-prerelease-test-plan.mjs", () => {
  ...
  it("uses kitchen-sink npm and ClawHub scenarios as the registry install canary", () => {
    ...
    expect(lane).toEqual({
      command: "RECALL_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin",
    ...
    expect(script).toContain("npm:@recall/kitchen-sink@latest");
    expect(script).toContain("npm-latest-conformance");
    expect(script).toContain("npm-latest-adversarial");
    expect(script).toContain("npm:@recall/kitchen-sink@beta");
    expect(script).toContain("clawhub:@recall/kitchen-sink@latest");
    expect(script).toContain("clawhub:@recall/kitchen-sink@beta");
    ...
    expect(sweepScript).toContain("RECALL_KITCHEN_SINK_PERSONALITY");
  ...
  it("keeps kitchen-sink RPC coverage package-backed and resource-guarded", () => {
    ...
    expect(script).toContain("RECALL_ENTRY=/app/recall.mjs");
    expect(script).toContain("docker stats --no-stream");
    ...
    expect(walkScript).toContain("usesPackagedRecallEntry(runner)");
  ...
  it("keeps the generic plugin Docker lane as an external install contract canary", () => {
    ...
    expect(lane).toEqual({
      command: "RECALL_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
      e2eImageKind: "functional",
      live: false,
      name: "plugins",
    ...
    expect(assertionsScript).toContain("assertClawHubExternalInstallContract");
    expect(assertionsScript).toContain('node_modules", "recall');
    expect(fixtureServer).toContain('"is-number": "7.0.0"');
    expect(fixtureServer).toContain('recall: ">=2026.4.11"');
  ...
  it("wires the full plugin prerelease plan into its release workflow", () => {
    ...
    expect(manifestEnv).toEqual({
      RECALL_CI_CHECKOUT_REVISION: "${{ steps.checkout_ref.outputs.sha }}",
      RECALL_CI_DOCS_CHANGED:
      ...
      RECALL_CI_EVENT_NAME: "${{ github.event_name }}",
      RECALL_CI_REPOSITORY: "${{ github.repository }}",
      RECALL_CI_RUN_ANDROID:
        "${{ github.event_name == 'workflow_dispatch' && inputs.include_android && 'true' || steps.changed_scope.outputs.run_android || 'false' }}",
      RECALL_CI_RUN_CONTROL_UI_I18N:
    ...
    expect(inspectorRun.env).toEqual({
      RECALL_PLUGIN_INSPECTOR_ROOT: ".artifacts/plugin-inspector",
      RECALL_PLUGIN_INSPECTOR_VERSION: "0.3.10",
    });
    expect(inspectorRun.run).toContain("extensions/");
    expect(inspectorRun.run).toContain(
      'npm exec --yes "@recall/plugin-inspector@${RECALL_PLUGIN_INSPECTOR_VERSION}" -- ci',
    );
  ...
  it("keeps release-check reruns independent while cancelling superseded umbrella runs", () => {
    const releaseChecksWorkflow = parse(
      readFileSync(".github/workflows/recall-release-checks.yml", "utf8"),
    );
    const fullReleaseWorkflow = readFullReleaseValidationWorkflow();

    expect(releaseChecksWorkflow.concurrency).toEqual({
      group:
        "recall-release-checks-${{ inputs.expected_sha || inputs.ref }}-${{ inputs.rerun_group }}",
  ...
  it("keeps runtime tool coverage blocking in release checks", () => {
    const releaseChecksSource = readFileSync(
      ".github/workflows/recall-release-checks.yml",
      "utf8",
    );
    ...
    expect(runtimeToolCoverage.steps).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          run: expect.stringContaining("pnpm recall qa coverage"),
        }),
      ]),
    );
    expect(runtimeToolCoverage.steps).toEqual(
  ...
  it("keeps the live-ish availability check redacted", () => {
    ...
    expect(output).toContain("channel-discord: present (DISCORD_TOKEN, RECALL_DISCORD_TOKEN)");
