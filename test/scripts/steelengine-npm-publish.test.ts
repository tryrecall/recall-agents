// SteelEngine NPM Publish tests cover publish wrapper argument safety.
...
const scriptPath = "scripts/steelengine-npm-publish.sh";
...
function makeReleaseCheckout(root: string, version: string): string {
  ...
  copyFileSync(
    "scripts/steelengine-npm-extended-stable-release.mjs",
    path.join(checkout, "scripts/steelengine-npm-extended-stable-release.mjs"),
  );
...
function makePackageTarball(root: string, packageJson?: string): string {
  ...
  const tarball = path.join(root, "steelengine.tgz");
...
describe("steelengine npm publish wrapper", () => {
  it("prints help without resolving release metadata", () => {
    const result = runPublishWrapper(["--help"]);

    expect(result.status).toBe(0);
    expect(result.stdout.trim()).toBe(
      "usage: bash scripts/steelengine-npm-publish.sh --publish [package.tgz]",
    );
  ...
  it("rejects extra publish arguments before npm publish", () => {
    const tempRoot = makeTempDir("steelengine-npm-publish-");
    const tarball = path.join(tempRoot, "steelengine.tgz");
  ...
  it.each(["beta", "latest"])("publishes the prepared tarball to the %s dist-tag", (distTag) => {
    ...
    const result = runPublishWrapper(
      ["--publish", tarball],
      {
        STEELENGINE_NPM_PUBLISH_TAG: distTag,
  ...
  it("rejects a tarball whose package version differs from the checkout", () => {
    ...
    const result = runPublishWrapper(["--publish", tarball], {
      STEELENGINE_NPM_PUBLISH_TAG: "beta",
    });
  ...
  it.each([
    ...
    ["missing version", JSON.stringify({ name: "steelengine" }), "has no valid version"],
  ...
  it("rejects publishing the current pre-.33 final version to extended-stable", () => {
    ...
    const result = runPublishWrapper(
      ["--publish"],
      {
        STEELENGINE_NPM_PUBLISH_TAG: "extended-stable",
      },
      checkout,
    );
  ...
  it("rejects unknown requested dist-tags instead of falling back to beta", () => {
    const result = runPublishWrapper(["--publish"], {
      STEELENGINE_NPM_PUBLISH_TAG: "nightly",
    });
