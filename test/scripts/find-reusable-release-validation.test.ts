...
function createRepo(options: { plistBuildVersion?: string } = {}, dirs = tempDirs) {
  ...
  mkdirSync(join(origin, "apps/macos/Sources/SteelEngine/Resources"), { recursive: true });
  writeFileSync(
    join(origin, "apps/macos/Sources/SteelEngine/Resources/Info.plist"),
...
function normalizedEvidence(options: {
  ...
  const roles = [
    ...
    [
      ...
      1,
      2,
      "SteelEngine Release Checks",
      "steelengine-release-checks.yml",
      "-release-checks",
    ],
    ["productPerformance", "204", 3, 2, "SteelEngine Performance", "steelengine-performance.yml", ""],
  ] as const;
...
if (
  ...
  repoIndex < 0 ||
  trustedRefIndex < 0 ||
  process.argv[repoIndex + 1] !== "steelengine/steelengine" ||
...
function setUpFixtures(runs: RunFixture[]): {
  ...
  writeFileSync(
    fixtureName(
      fixtures,
      "repos/steelengine/steelengine/actions/workflows/full-release-validation.yml/runs",
    ),
  ...
  return spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...
        GITHUB_OUTPUT: "",
        STEELENGINE_RELEASE_CI_SUMMARY_VALIDATOR: args.validatorPath,
