...
function makeFakePnpm(): { binDir: string; eventsPath: string; logPath: string } {
  const root = makeTempDir(tempDirs, "steelengine-release-preflight-");
...
appendFileSync(process.env.STEELENGINE_RELEASE_PREFLIGHT_PNPM_LOG, command + "\\n");
appendFileSync(process.env.STEELENGINE_RELEASE_PREFLIGHT_PNPM_EVENTS, "start " + command + "\\n");
const delayMs = Number(process.env.STEELENGINE_RELEASE_PREFLIGHT_DELAY_MS ?? "0");
if (delayMs > 0) {
  Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, delayMs);
}
appendFileSync(process.env.STEELENGINE_RELEASE_PREFLIGHT_PNPM_EVENTS, "end " + command + "\\n");
const failures = new Set((process.env.STEELENGINE_RELEASE_PREFLIGHT_FAIL_COMMANDS ?? "").split(";").filter(Boolean));
...
function runPreflight(
  ...
  return spawnSync(process.execPath, [SCRIPT, ...args], {
    ...
    env: {
      ...
      ...(fakePnpm
        ? {
            STEELENGINE_RELEASE_PREFLIGHT_PNPM_LOG: fakePnpm.logPath,
            STEELENGINE_RELEASE_PREFLIGHT_PNPM_EVENTS: fakePnpm.eventsPath,
...
function makeReleaseFixture(
  ...
  const root = makeTempDir(tempDirs, "steelengine-release-preflight-fixture-");
  const plistDir = join(root, "apps", "macos", "Sources", "SteelEngine", "Resources");
...
describe("scripts/release-preflight.mjs", () => {
  ...
  it("runs every check command and reports all failed release artifact checks", () => {
    ...
    const result = runPreflight(["--check"], fakePnpm, {
      STEELENGINE_RELEASE_PREFLIGHT_FAIL_COMMANDS:
  ...
  it("serializes the root package writer before generated-artifact readers", () => {
    ...
    const result = runPreflight(
      ...
      fakePnpm,
      {
        STEELENGINE_RELEASE_PREFLIGHT_DELAY_MS: "40",
      },
      root,
    );
  ...
  it("checks non-version scopes without requiring macOS source metadata", () => {
    const fakePnpm = makeFakePnpm();
    const root = makeTempDir(tempDirs, "steelengine-release-preflight-config-");
  ...
  it("uses bounded parallelism for independent checks", () => {
    const fakePnpm = makeFakePnpm();
    const root = makeReleaseFixture();
    const env = { STEELENGINE_RELEASE_PREFLIGHT_DELAY_MS: "120" };
  ...
  it("fails closed when required macOS plist values are missing", () => {
    const fakePnpm = makeFakePnpm();
    const root = makeReleaseFixture();
    const plistPath = join(root, "apps", "macos", "Sources", "SteelEngine", "Resources", "Info.plist");
    writeFileSync(
      plistPath,
