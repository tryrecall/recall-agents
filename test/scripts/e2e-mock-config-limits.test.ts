...
const scrubbedEnvKeys = [
  ...
  "MOCK_TLS_CERT",
  "MOCK_TLS_KEY",
  "STEELENGINE_CONFIG_RELOAD_LOG_MAX_READ_BYTES",
  "STEELENGINE_CONFIG_RELOAD_LOG_PATH",
  "STEELENGINE_CONFIG_RELOAD_LOG_TIMEOUT_MS",
  "STEELENGINE_MOCK_OPENAI_PORT",
  "RAW_SCHEMA_ERROR",
  "SUCCESS_MARKER",
];
...
describe("mock OpenAI response markers", () => {
  it("echoes dynamic SteelEngine E2E markers", async () => {
    await withMockServer(mockOpenAiPath, {}, async (baseUrl) => {
      for (const marker of ["STEELENGINE_E2E_SEED_0_123", "STEELENGINE_E2E_ANDROID_OK"]) {
...
describe("e2e mock and config helper numeric limits", () => {
  it("rejects loose mock OpenAI port env values", () => {
    ...
    const fallbackPort = runScript(mockOpenAiPath, {
      STEELENGINE_MOCK_OPENAI_PORT: "44080http",
    });
    expect(fallbackPort.status).not.toBe(0);
    expect(fallbackPort.stderr).toContain("invalid STEELENGINE_MOCK_OPENAI_PORT: 44080http");
  });
  ...
  it("rejects out-of-range mock OpenAI port env values", () => {
    ...
    const fallbackPort = runScript(mockOpenAiPath, {
      STEELENGINE_MOCK_OPENAI_PORT: "65536",
    });
    expect(fallbackPort.status).not.toBe(0);
    expect(fallbackPort.stderr).toContain("invalid STEELENGINE_MOCK_OPENAI_PORT: 65536");
  });
  ...
  it("rejects loose config-reload log timeout env values", () => {
    const result = runScript(configReloadAssertPath, {
      STEELENGINE_CONFIG_RELOAD_LOG_TIMEOUT_MS: "30000ms",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid STEELENGINE_CONFIG_RELOAD_LOG_TIMEOUT_MS: 30000ms");
  });

  it("rejects loose config-reload log read caps", () => {
    const result = runScript(configReloadAssertPath, {
      STEELENGINE_CONFIG_RELOAD_LOG_MAX_READ_BYTES: "256kb",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid STEELENGINE_CONFIG_RELOAD_LOG_MAX_READ_BYTES: 256kb");
  });

  it("returns a clear error when mock OpenAI cannot append request logs", async () => {
    const requestLogDirectory = await mkdtemp(join(tmpdir(), "steelengine-mock-request-log-"));
    try {
      await withMockServer(
        ...
        async (baseUrl, output) => {
          const response = await fetch(`${baseUrl}/v1/responses`, {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({ input: "STEELENGINE_E2E_OK" }),
          });
          const body = await response.json();
  ...
  it("returns a clear error when web-search mock cannot append request logs", async () => {
    const requestLogDirectory = await mkdtemp(join(tmpdir(), "steelengine-web-search-log-"));
    try {
      await withMockServer(
        webSearchMockPath,
        {
          ...
          SUCCESS_MARKER: "STEELENGINE_SCHEMA_E2E_OK",
        },
        async (baseUrl, output) => {
          const response = await fetch(`${baseUrl}/v1/responses`, {
            method: "POST",
