...
const metaManifestPath = "extensions/meta/steelengine.plugin.json";
...
describe("plugin npm extended-stable workflow", () => {
  ...
  it("uses one override for check, plan, preview, pack, and publish", () => {
    ...
    for (const name of [
      ...
      ).toMatchObject({ STEELENGINE_PLUGIN_NPM_PUBLISH_TAG: expectedOverride });
    }
  });
  ...
  it("prepares and independently reads back immutable package evidence", () => {
    ...
    expect(prepare.run).toContain('kind: "steelengine-plugin-npm-preflight"');
  ...
  it("attests the canonical Meta provider package and install route", () => {
    ...
    expect(packageJson.name).toBe("@steelengine/meta-provider");
    expect(packageJson.steelengine?.install?.npmSpec).toBe("@steelengine/meta-provider");
  ...
  it("publishes extended-stable with OIDC only and verifies every package tag", () => {
    ...
    expect(publish.env).toMatchObject({
      STEELENGINE_NPM_PUBLISH_AUTH_MODE: "trusted-publisher",
    });
    ...
    expect(bootstrap.run).not.toContain("@steelengine/meta-provider");
