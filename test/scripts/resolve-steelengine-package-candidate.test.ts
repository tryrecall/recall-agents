// Resolve Openclaw Package Candidate tests cover resolve steelengine package candidate script behavior.
...
import {
  ...
  cleanPackedSteelEngineTarballsForTest,
  downloadUrl,
  ...
  validateSteelEnginePackageSpec,
} from "../../scripts/resolve-steelengine-package-candidate.mjs";
...
describe("resolve-steelengine-package-candidate", () => {
  it("allows Unreleased notes when packaging an exact ref candidate", () => {
    const script = readFileSync("scripts/resolve-steelengine-package-candidate.mjs", "utf8");
    const refPackageBuild = script.slice(
      script.indexOf('if (options.source === "ref")'),
      script.indexOf('} else if (options.source === "npm")'),
    );

    expect(refPackageBuild).toContain('"scripts/package-steelengine-for-docker.mjs"');
    expect(refPackageBuild).toContain('"--allow-unreleased-changelog"');
  });

  it("accepts only SteelEngine release package specs for npm candidates", () => {
    for (const spec of [
      "steelengine@beta",
      "steelengine@alpha",
      "steelengine@extended-stable",
      "steelengine@latest",
      "steelengine@2026.4.27",
      "steelengine@2026.4.27-1",
      "steelengine@2026.4.27-beta.2",
      "steelengine@2026.4.27-alpha.2",
    ]) {
      expect(validateSteelEnginePackageSpec(spec), spec).toBeUndefined();
    }

    expect(() => validateSteelEnginePackageSpec("@evil/steelengine@1.0.0")).toThrow(
      "package_spec must be steelengine@alpha",
    );
    expect(() => validateSteelEnginePackageSpec("steelengine@canary")).toThrow(
      "package_spec must be steelengine@alpha",
    );
    expect(() => validateSteelEnginePackageSpec("steelengine@2026.04.27")).toThrow(
      "package_spec must be steelengine@alpha",
    );
    expect(() => validateSteelEnginePackageSpec("steelengine@npm:other-package")).toThrow(
      "package_spec must be steelengine@alpha",
    );
    expect(() => validateSteelEnginePackageSpec("steelengine@file:../other-package.tgz")).toThrow(
      "package_spec must be steelengine@alpha",
    );
  });
  ...
  it("parses optional empty workflow inputs without rejecting the command line", () => {
    expect(
      ...
      outputName: "steelengine-current.tgz",
      packageSha256: "",
      packageRef: "release/2026.4.27",
      packageSpec: "steelengine@beta",
      packageUrl: "",
      source: "npm",
      trustedSourceId: "",
  ...
  it("rejects duplicate package candidate CLI options", () => {
    ...
    const duplicateCases = [
      ...
      [
        "--package-spec",
        [...requiredArgs, "--package-spec", "steelengine@beta", "--package-spec", "steelengine@latest"],
      ],
      [
        "--package-url",
        [...requiredArgs, "--package-url", "", "--package-url", "https://example.com/steelengine.tgz"],
      ],
  ...
  it("rejects package candidate output names that escape the output directory", () => {
    for (const outputName of [
      "../steelengine-current.tgz",
      "nested/steelengine-current.tgz",
      "steelengine-current.zip",
      ".steelengine-current.tgz",
    ]) {
    ...
    expect(parseArgs(["--output-name", "steelengine-current.tar.gz"]).outputName).toBe(
      "steelengine-current.tar.gz",
    );
  });
  ...
  it("resolves npm package candidates through the Windows npm.cmd toolchain shim", () => {
    ...
    const runner = resolveNpmPackageCandidatePackRunner(
      "steelengine@2026.5.26-beta.1",
      "C:\\steelengine\\.artifacts\\docker-e2e-package",
      {
    ...
    expect(runner).toEqual({
      ...
      args: [
        ...
        "/s",
        "/c",
        `${npmCmdPath} pack steelengine@2026.5.26-beta.1 --ignore-scripts --json --pack-destination C:\\steelengine\\.artifacts\\docker-e2e-package`,
      ],
      shell: false,
  ...
  it("keeps npm pack filenames inside the package candidate output directory", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-npm-pack-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "steelengine-2026.6.17.tgz"), "package");

    await expect(
      moveNewestPackedTarballForTest(
        dir,
        JSON.stringify([{ filename: "steelengine-2026.6.17.tgz" }]),
        "steelengine-current.tgz",
      ),
    ).resolves.toBe(path.join(dir, "steelengine-current.tgz"));
    await expect(readFile(path.join(dir, "steelengine-current.tgz"), "utf8")).resolves.toBe("package");
  });
  ...
  it("rejects path-like npm pack filenames instead of renaming outside the output directory", async () => {
    ...
    const unsafeFilenames = [
      "../steelengine-2026.6.17.tgz",
      "nested/steelengine-2026.6.17.tgz",
      "nested\\steelengine-2026.6.17.tgz",
      "/tmp/steelengine-2026.6.17.tgz",
      "C:\\temp\\steelengine-2026.6.17.tgz",
      "steelengine-2026.6.17.tar.gz",
    ];

    for (const filename of unsafeFilenames) {
      await expect(
        moveNewestPackedTarballForTest(dir, JSON.stringify([{ filename }]), "steelengine-current.tgz"),
      ).rejects.toThrow("npm pack reported unsafe SteelEngine tarball filename");
    }
  });
  ...
  it("rejects unsafe text npm pack filenames instead of using loose stdout fallback", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-npm-pack-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "steelengine-2026.6.17.tgz"), "safe fallback");

    for (const filename of ["../steelengine-2026.6.17.tgz", "C:steelengine-2026.6.17.tgz"]) {
      await expect(
        moveNewestPackedTarballForTest(
          dir,
  ...
  it("cleans stale package tarballs before npm fallback scanning", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-npm-pack-stale-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "steelengine-9999.1.1.tgz"), "stale");
    await writeFile(path.join(dir, "steelengine-C:evil.tgz"), "unsafe");

    await cleanPackedSteelEngineTarballsForTest(dir);
    await writeFile(path.join(dir, "steelengine-2026.6.17.tgz"), "current");

    await expect(
      moveNewestPackedTarballForTest(dir, "npm notice\n", "steelengine-current.tgz"),
    ).resolves.toBe(path.join(dir, "steelengine-current.tgz"));
    await expect(missing(path.join(dir, "steelengine-9999.1.1.tgz"))).resolves.toBe(true);
    await expect(readFile(path.join(dir, "steelengine-C:evil.tgz"), "utf8")).resolves.toBe("unsafe");
    await expect(readFile(path.join(dir, "steelengine-current.tgz"), "utf8")).resolves.toBe("current");
  });
  ...
  it("kills timed-out package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-runner-timeout-"));
    tempDirs.push(dir);
  ...
  it("clamps oversized package runner kill grace before scheduling", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-runner-grace-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
  ...
  it("rejects timed-out package runner commands when descendants exit cleanly", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-runner-timeout-clean-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
    ...
    const childScript = [
      "const fs = require('node:fs');",
      "process.on('SIGTERM', () => {",
      "  fs.writeFileSync(process.env.STEELENGINE_TEST_CHILD_CLEANUP, 'clean');",
      "  setTimeout(() => process.exit(0), 25);",
      "});",
      "fs.writeFileSync(process.env.STEELENGINE_TEST_CHILD_READY, 'ready');",
      "setInterval(() => {}, 1000);",
    ].join("");
    const parentScript = [
      "const { spawn } = require('node:child_process');",
      ...
      "  env: {",
      "    ...process.env,",
      "    STEELENGINE_TEST_CHILD_CLEANUP: process.env.STEELENGINE_TEST_CHILD_CLEANUP,",
      "    STEELENGINE_TEST_CHILD_READY: process.env.STEELENGINE_TEST_CHILD_READY,",
      "  },",
      "});",
      "fs.writeFileSync(process.env.STEELENGINE_TEST_CHILD_PID, String(child.pid));",
    ...
    const timeoutAssertion = expect(
      runCommandForTest(process.execPath, ["-e", parentScript], {
        env: {
          ...process.env,
          STEELENGINE_TEST_CHILD_CLEANUP: cleanupPath,
          STEELENGINE_TEST_CHILD_PID: childPidPath,
          STEELENGINE_TEST_CHILD_READY: readyPath,
        },
        killAfterMs: 250,
        timeoutMs: 250,
      }),
  ...
  it("forwards external termination to package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-runner-signal-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
    const scriptUrl = pathToFileURL(
      path.resolve("scripts/resolve-steelengine-package-candidate.mjs"),
    ).href;
    let childPid: number | undefined;
  ...
  it("fails successful ref candidates when package source worktree cleanup fails", async () => {
    await expect(
      cleanupPackageSourceWorktreeForTest("/tmp/steelengine-package-source-stuck", {
        runImpl: async () => {
  ...
  it("preserves original ref candidate failures when worktree cleanup also fails", async () => {
    ...
    expect(warnings).toEqual([
      "warning: failed to remove temporary package source worktree /tmp/steelengine-package-source-stuck: worktree remove denied",
    ]);
  });

  it("loads named trusted package URL source policies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-trusted-package-source-"));
    tempDirs.push(dir);
      ...
      JSON.stringify({
        ...
        sources: {
          "enterprise-artifactory": {
            ...
            pathPrefixes: ["/artifactory/steelengine/"],
            ports: [443, 8443],
  ...
  it("rejects unsafe package_url downloads before fetching private targets", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-download-"));
    tempDirs.push(dir);
    const target = path.join(dir, "steelengine.tgz");

    await expect(
      downloadUrl("http://packages.example/steelengine.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must use https");
    await expect(
      downloadUrl("https://user@packages.example/steelengine.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "93.184.216.34", family: 4 }]),
      }),
    ).rejects.toThrow("package_url must not include credentials");
    await expect(
      downloadUrl("https://localhost/steelengine.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "127.0.0.1", family: 4 }]),
      }),
    ).rejects.toThrow(/private\/internal\/special-use/iu);
    await expect(
      downloadUrl("https://packages.example/steelengine.tgz", target, {
        fetchImpl: unexpectedFetch,
  ...
  it("allows private package_url downloads only through an explicit trusted source policy", async () => {
    ...
    await downloadUrl("https://packages.internal:8443/artifactory/steelengine/steelengine.tgz", target, {
    ...
    expect(requestedUrls).toEqual([
      "https://packages.internal:8443/artifactory/steelengine/steelengine.tgz",
    ]);
    await expect(readFile(target)).resolves.toEqual(Buffer.from([4, 5, 6]));

    await expect(
      downloadUrl("https://evil.internal:8443/artifactory/steelengine/steelengine.tgz", target, {
        fetchImpl: unexpectedFetch,
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/other/steelengine.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "203.0.113.8", family: 4 }]),
        trustedSource,
      }),
  ...
  it("matches trusted package_url path prefixes on path segment boundaries", async () => {
    ...
    await downloadUrl("https://packages.internal:8443/artifactory/steelengine/pkg.tgz", target, {
      fetchImpl: async (url: URL) => {
        requestedUrls.push(url.toString());
    ...
    expect(requestedUrls).toEqual(["https://packages.internal:8443/artifactory/steelengine/pkg.tgz"]);
    await expect(
      downloadUrl("https://packages.internal:8443/artifactory/steelengine-malicious/pkg.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "203.0.113.8", family: 4 }]),
        trustedSource,
      }),
  ...
  it("keeps trusted package_url redirects inside the named source policy", async () => {
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/artifactory/steelengine/steelengine.tgz", target, {
        fetchImpl: async () =>
          new Response(null, {
            headers: { location: "https://metadata.internal:8443/artifactory/steelengine/pwn.tgz" },
            status: 302,
          }),
        lookupHost: lookupAddresses([{ address: "10.0.0.8", family: 4 }]),
        trustedSource,
  ...
  it("does not forward trusted package auth headers to redirect hosts", async () => {
    ...
    tempDirs.push(dir);
    const target = path.join(dir, "steelengine.tgz");
    const previousToken = process.env.STEELENGINE_TRUSTED_PACKAGE_TOKEN;
    process.env.STEELENGINE_TRUSTED_PACKAGE_TOKEN = "token-123";
    const trustedSource = {
      allowPrivateNetwork: true,
      auth: { type: "bearer" },
      hosts: ["packages.internal"],
    ...
    try {
          ...
          fetchImpl: async (_url: URL, init?: RequestInit) => {
            ...
            if (requestHeaders.length === 1) {
              return new Response(null, {
                headers: {
                  location: "https://mirror.internal:8443/artifactory/steelengine/steelengine.tgz",
                },
                status: 302,
              });
            }
      ...
      if (previousToken === undefined) {
        delete process.env.STEELENGINE_TRUSTED_PACKAGE_TOKEN;
      } else {
        process.env.STEELENGINE_TRUSTED_PACKAGE_TOKEN = previousToken;
      }
    }
  ...
  it("validates redirects for package_url downloads", async () => {
    ...
    expect(requestedUrls).toEqual(["https://packages.example/steelengine.tgz"]);
  });
  ...
  it("bounds package_url downloads and writes completed files atomically", async () => {
    ...
    await downloadUrl("https://packages.example/steelengine.tgz", target, {
      fetchImpl: async () =>
  ...
  it("times out stalled package_url response bodies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-download-timeout-"));
    tempDirs.push(dir);
    const target = path.join(dir, "steelengine.tgz");
    let bodyCancelled = false;
    const startedAt = Date.now();
    ...
    await expect(
      ...
      "package_url download timed out after 25ms: https://packages.example/steelengine.tgz",
    );
  ...
  it("reads package source metadata from package artifacts", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-candidate-"));
    tempDirs.push(dir);
    await writeFile(
  ...
  it("normalizes artifact package source SHAs before workflow output", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-candidate-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("normalizes whitespace-only artifact package source SHAs to absent", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-candidate-empty-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("rejects malformed artifact package source SHAs", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-candidate-bad-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("accepts uppercase package artifact SHA-256 metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-sha-"));
    tempDirs.push(dir);
    const file = path.join(dir, "steelengine.tgz");
    await writeFile(file, "steelengine package bytes");
  ...
  it("rejects source artifact scans that exceed the filesystem entry limit", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-artifact-scan-"));
    tempDirs.push(dir);
    const maxEntries = 3;
  ...
  it("rejects source artifact directories with multiple tarballs", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-artifact-duplicates-"));
    tempDirs.push(dir);

    await writeFile(path.join(dir, "steelengine-a.tgz"), "a");
    ...
    expect(message).toContain("steelengine-a.tgz");
    expect(message).not.toContain(path.join(dir, "nested.tar.gz"));
    expect(message).not.toContain(path.join(dir, "steelengine-a.tgz"));
  });

  it("reads the source SHA from packed npm build metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-build-info-"));
    tempDirs.push(dir);
    const root = path.join(dir, "package");
    await mkdir(path.join(root, "dist"), { recursive: true });
    await writeFile(path.join(root, "package.json"), JSON.stringify({ name: "steelengine" }));
    await writeFile(
      path.join(root, "dist", "build-info.json"),
      JSON.stringify({ commit: "66CE632B9B7C5C7FDD3E66C739687D51638AD6E2" }),
    );
    const tarball = path.join(dir, "steelengine.tgz");
