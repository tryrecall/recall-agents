...
describe("website installer sync workflow", () => {
  ...
  it("treats all website installer scripts as Recall-owned inputs", () => {
  ...
  it("verifies installers on Linux Docker plus native macOS and Windows runners", () => {
    ...
    expect(workflow).toContain("bash /tmp/install.sh --version latest && recall --version");
    expect(workflow).not.toContain("bash /tmp/install.sh --no-prompt --no-onboard");
    expect(workflow).toContain("bash /tmp/install-cli.sh --prefix /tmp/recall");
    expect(workflow).toContain("macos-installer:");
    expect(workflow).toContain("runs-on: macos-latest");
    expect(workflow).toContain("node-version: 24");
    expect(workflow).toContain('RECALL_NO_ONBOARD: "1"');
    expect(workflow).toContain('RECALL_NO_PROMPT: "1"');
    expect(workflow).toContain("bash scripts/install.sh --no-onboard --no-prompt --version latest");
    expect(workflow).toContain("recall --version");
  ...
  it("syncs verified scripts to recall.ai only after all installer checks pass", () => {
    expect(workflow).toContain("needs: [static, linux-docker, macos-installer, windows-installer]");
    expect(workflow).toContain("repository: tryrecall/recall-agents.ai");
    expect(workflow).toContain("RECALL_GH_TOKEN: ${{ secrets.RECALL_GH_TOKEN }}");
    expect(workflow).toContain("RECALL_GH_TOKEN is not configured");
    expect(workflow).toContain("token: ${{ env.RECALL_GH_TOKEN }}");
    expect(workflow).toContain("cp recall/scripts/install.sh recall.ai/public/install.sh");
    expect(workflow).toContain(
      "cp recall/scripts/install-cli.sh recall.ai/public/install-cli.sh",
    );
    expect(workflow).toContain("cp recall/scripts/install.ps1 recall.ai/public/install.ps1");
    expect(workflow).toContain("rm -f recall.ai/public/install.cmd");
