...
const workflowPath = ".github/workflows/steelengine-npm-release.yml";
...
describe("minimal npm extended-stable workflow", () => {
  ...
  it("reuses the v1 preflight tarball and guards all three extended-stable gates", () => {
    ...
    expect(raw).toContain("steelengine-npm-preflight-${{ inputs.tag }}");
    expect(raw.match(/steelengine-npm-extended-stable-release\.mjs validate-request/g)).toHaveLength(
      3,
    );
    expect(step(parsed.jobs?.preflight_steelengine_npm, "Validate npm release request").run).toContain(
      "steelengine-npm-extended-stable-release.mjs validate-request",
    );
    expect(
      step(parsed.jobs?.preflight_steelengine_npm, "Validate npm release request").env?.PREFLIGHT_ONLY,
    ).toBe("${{ inputs.preflight_only }}");
    expect(
      step(parsed.jobs?.validate_publish_request, "Validate npm release request").run,
    ).toContain("steelengine-npm-extended-stable-release.mjs validate-request");
    expect(step(parsed.jobs?.publish_steelengine_npm, "Recheck npm release request").run).toContain(
      "steelengine-npm-extended-stable-release.mjs validate-request",
    );
    expect(
  ...
  it("threads an explicit, default-off extended-stable bypass through every policy gate", () => {
    ...
    const policySteps = [
      step(parsed.jobs?.preflight_steelengine_npm, "Validate npm release request"),
      step(parsed.jobs?.validate_publish_request, "Validate npm release request"),
      step(parsed.jobs?.publish_steelengine_npm, "Recheck npm release request"),
      step(parsed.jobs?.publish_steelengine_npm, "Publish"),
    ];
    ...
    const summary = step(
      parsed.jobs?.publish_steelengine_npm,
  ...
  it("accepts arbitrary SHA preflight targets and exercises every publishable plugin package", () => {
    const parsed = workflow();
    const preflight = parsed.jobs?.preflight_steelengine_npm;
    ...
    expect(plugins.env).toMatchObject({
      STEELENGINE_PLUGIN_NPM_PUBLISH_TAG: "extended-stable",
    });
    ...
    expect(plugins.run).toContain("STEELENGINE_PLUGIN_NPM_PACK_OUTPUT_DIR");
  ...
  it("authenticates exact extended-stable run and Full Validation identities", () => {
    ...
    expect(raw.match(/steelengine-npm-extended-stable-release\.mjs verify-run/g)).toHaveLength(3);
    expect(raw).toContain("steelengine-npm-extended-stable-release.mjs verify-manifest");
  });
  ...
  it("requires and authenticates the plugin npm run before an extended-stable core publish", () => {
    ...
    expect(verify.run).toContain("steelengine-npm-extended-stable-release.mjs verify-run");
  });

  it("captures selector fail closed, publishes extended-stable, retries, and summarizes", () => {
    const parsed = workflow();
    const publish = parsed.jobs?.publish_steelengine_npm;
    ...
    expect(capture.run).toContain("steelengine-npm-extended-stable-release.mjs capture-selector");
    expect(step(publish, "Publish").run).toContain("steelengine-npm-publish.sh");
    expect(readback.run).toContain("steelengine-npm-extended-stable-release.mjs verify-readback");
    expect(summary.if).toContain("always()");
    expect(summary.run).toContain("steelengine-npm-extended-stable-release.mjs repair-command");
  ...
  it("publishes only the tarball path verified from the preflight manifest", () => {
    const publish = workflow().jobs?.publish_steelengine_npm;
