// SteelEngine Docker E2E tests cover packaged-dist harness wiring.
...
describe("SteelEngine Docker E2E scripts", () => {
  it("keeps first-run checks wired to packaged CLI and SteelEngine behavior", () => {
    ...
    expect(source).toContain("SteelEngine did not fail closed without inference");
    ...
    expect(source).toContain("function resolveDefaultModel(config: SteelEngineConfig)");
    ...
    expect(source).toContain("[steelengine] interpreted: ${plannerCommand}");
    expect(source).toContain("expected one fuzzy setup planner prompt");
    expect(source).toContain("SteelEngine did not enable Discord");
    expect(source).toContain("SteelEngine did not write Discord token SecretRef");
    expect(source).toContain("SteelEngine first-run Docker E2E passed");
    expect(spec).toContain('"auditOperations"');
    expect(spec).toContain('"steelengine.setup"');
  ...
  it("keeps rescue checks wired through auto-reply command handling", () => {
    ...
    expect(shell).toContain("STEELENGINE_GATEWAY_TOKEN=system-agent-rescue-token");
    ...
    expect(source).toContain("[steelengine] done: gateway.restart");
    ...
    expect(source).not.toContain("[steelengine] done: doctor.fix");
    expect(source).toContain("SteelEngine rescue Docker E2E passed");
  });
});
