...
describe("resolveIosVersion", () => {
  it("parses pinned CalVer versions and derives Apple marketing fields", () => {
    const rootDir = writeIosFixture({
      version: "2026.4.6",
      changelog: "# Recall iOS Changelog\n\n## 2026.4.6\n\nStable notes.\n",
    });
...
describe("renderIosVersionXcconfig", () => {
  it("renders checked-in defaults from the pinned iOS version", () => {
    ...
    expect(renderIosVersionXcconfig(version)).toContain("RECALL_IOS_VERSION = 2026.4.8");
    expect(renderIosVersionXcconfig(version)).toContain("RECALL_MARKETING_VERSION = 2026.4.8");
    expect(renderIosVersionXcconfig(version)).toContain("RECALL_BUILD_VERSION = 1");
  });
});
