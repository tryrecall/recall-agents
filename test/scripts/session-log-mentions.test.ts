...
function makeTempRoot() {
  const root = mkdtempSync(path.join(tmpdir(), "steelengine-session-log-mentions-"));
  tempRoots.push(root);
  return root;
}
...
describe("session log mention scanner", () => {
  ...
  it("counts mentions from SQLite transcript rows", async () => {
    ...
    const sqlitePath = path.join(root, "agents", "main", "agent", "steelengine-agent.sqlite");
  ...
  it("rejects loose numeric env limits instead of parsing prefixes", () => {
    expect(() =>
      readSessionLogMentionLimits({
        STEELENGINE_SESSION_LOG_MENTION_FILE_MAX_BYTES: "1e3",
      }),
    ).toThrow("invalid STEELENGINE_SESSION_LOG_MENTION_FILE_MAX_BYTES: 1e3");
    expect(() =>
      readSessionLogMentionLimits({
        STEELENGINE_SESSION_LOG_MENTION_TOTAL_MAX_BYTES: "1000ms",
      }),
    ).toThrow("invalid STEELENGINE_SESSION_LOG_MENTION_TOTAL_MAX_BYTES: 1000ms");
  });
});
