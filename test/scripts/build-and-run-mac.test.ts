...
function runStopExistingLocalApp(params: { fakeLsof?: string; fakePgrep: string }) {
  const root = mkdtempSync(join(tmpdir(), "steelengine-build-run-mac-test-"));
  tempRoots.push(root);
  ...
  writeFileSync(
    ...
    [
      "#!/usr/bin/env bash",
      "set -euo pipefail",
      'BIN_ABS="/worktree/apps/macos/.build-local/debug/SteelEngine"',
      'BIN=".build-local/debug/SteelEngine"',
      'APP_CWD="/worktree/apps/macos"',
      "kill() {",
      '  printf "%s\\n" "$*" >> "$STEELENGINE_TEST_KILL_CALLS"',
      '  touch "$STEELENGINE_TEST_KILLED_MARKER"',
      "  return 0",
      "}",
      stopFunction,
  ...
  const result = spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      STEELENGINE_TEST_KILLED_MARKER: join(root, "killed"),
      STEELENGINE_TEST_KILL_CALLS: killCallsPath,
      STEELENGINE_TEST_PGREP_CALLS: pgrepCallsPath,
      STEELENGINE_TEST_PGREP_COUNT: join(root, "pgrep-count.txt"),
...
describe("scripts/build-and-run-mac.sh", () => {
  ...
  it("keeps launch logs isolated unless an explicit log path is provided", () => {
    ...
    expect(script).not.toContain("/tmp/steelengine.log");
  });
  ...
  it("stops only the local debug app binary before relaunching", () => {
    ...
    const { killCalls, pgrepCalls, result } = runStopExistingLocalApp({
      fakePgrep: [
        "#!/usr/bin/env bash",
        `printf '%s\\n' "$*" >> "$STEELENGINE_TEST_PGREP_CALLS"`,
        'count="$(cat "$STEELENGINE_TEST_PGREP_COUNT" 2>/dev/null || echo 0)"',
        'next="$((count + 1))"',
        'printf "%s\\n" "$next" > "$STEELENGINE_TEST_PGREP_COUNT"',
        'if [[ "$2" == "/worktree/apps/macos/.build-local/debug/SteelEngine" ]]; then exit 1; fi',
        'if [[ "$2" == ".build-local/debug/SteelEngine" && "$count" == "1" ]]; then echo 321; exit 0; fi',
        "exit 1",
      ].join("\n"),
    });
    ...
    expect(pgrepCalls).toContain("-f /worktree/apps/macos/.build-local/debug/SteelEngine");
    expect(pgrepCalls).toContain("-f .build-local/debug/SteelEngine");
  ...
  it("fails when the scoped local app process survives cleanup", () => {
    const { result } = runStopExistingLocalApp({
      fakePgrep: [
        "#!/usr/bin/env bash",
        'if [[ "$2" == ".build-local/debug/SteelEngine" ]]; then echo 321; exit 0; fi',
        "exit 1",
      ].join("\n"),
    });
