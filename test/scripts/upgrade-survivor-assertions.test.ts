...
function assertConfiguredPluginState(params: { installPath?: string } = {}): void {
  const root = mkdtempSync(join(tmpdir(), "recall-upgrade-survivor-"));
  try {
    ...
    writeJson(join(stateDir, "plugins", "installs.json"), {
      installRecords: {
        matrix: {
          source: "clawhub",
          spec: "clawhub:@recall/matrix",
          installPath: matrixInstallDir,
          clawhubPackage: "@recall/matrix",
    ...
    execFileSync(process.execPath, [ASSERTIONS_PATH, "assert-state"], {
      env: {
        ...
        RECALL_TEST_WORKSPACE_DIR: workspace,
        RECALL_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON: coveragePath,
        RECALL_UPGRADE_SURVIVOR_SCENARIO: "configured-plugin-installs",
      },
      stdio: "pipe",
    });
  } finally {
...
describe("upgrade survivor assertions", () => {
  ...
  it("rejects ClawHub npm-pack installs outside the managed extensions root", () => {
    const root = mkdtempSync(join(tmpdir(), "recall-upgrade-survivor-outside-"));
    try {
      expect(() =>
