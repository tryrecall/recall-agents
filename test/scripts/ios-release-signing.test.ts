...
describe("scripts/ios-release-signing.mjs", () => {
  ...
  it("emits manual App Store profile settings for every signed target", () => {
    ...
    expect(output).toContain("STEELENGINE_CODE_SIGN_STYLE = Manual");
    expect(output).toContain("STEELENGINE_CODE_SIGN_IDENTITY = Apple Distribution");
    expect(output).toContain("STEELENGINE_APP_GROUP_ID = group.ai.steelenginefoundation.app.shared");
    expect(output).toContain("STEELENGINE_APP_PROFILE = SteelEngine App Store ai.steelenginefoundation.app");
    expect(output).toContain(
      "STEELENGINE_SHARE_PROFILE = SteelEngine App Store ai.steelenginefoundation.app.share",
    );
    expect(output).toContain(
      "STEELENGINE_ACTIVITY_WIDGET_PROFILE = SteelEngine App Store ai.steelenginefoundation.app.activitywidget",
    );
    expect(output).toContain(
      "STEELENGINE_WATCH_APP_PROFILE = SteelEngine App Store ai.steelenginefoundation.app.watchkitapp",
    );
    expect(output).not.toContain("STEELENGINE_WATCH_EXTENSION_PROFILE");
  });
  ...
  it("documents the canonical release signing plan", () => {
    ...
    expect(output).toContain("Signing repo: git@github.com:steelengine/apps-signing.git");
    expect(output).toContain("Signing branch: main");
    expect(output).toContain("Signing setup and sync: Fastlane match");
    expect(output).not.toContain("SteelEngineWatchExtension");
    expect(output).toContain(
      "capabilities: PUSH_NOTIFICATIONS, APP_GROUPS, APP_ATTEST, HEALTH_KIT",
    );
    expect(output).toContain("app groups: group.ai.steelenginefoundation.app.shared");
  });
});
