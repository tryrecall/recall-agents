...
import { buildMarkdown, parseArgs } from "../../scripts/steelengine-performance-source-summary.mjs";

const tmpRoots: string[] = [];

function mkTmpRoot() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-source-summary-"));
  tmpRoots.push(root);
  return root;
}
...
function runCli(...args: string[]) {
  return spawnSync(process.execPath, ["scripts/steelengine-performance-source-summary.mjs", ...args], {
    cwd: path.resolve("."),
    encoding: "utf8",
  });
}
