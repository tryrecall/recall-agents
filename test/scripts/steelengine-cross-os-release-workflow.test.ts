// Openclaw Cross Os Release Workflow tests cover steelengine cross os release workflow script behavior.
import { spawnSync } from "node:child_process";
import { readFileSync } from "node:fs";
import { describe, expect, it } from "vitest";
import { parse } from "yaml";

const WORKFLOW_PATH = ".github/workflows/steelengine-cross-os-release-checks-reusable.yml";
const RELEASE_CHECKS_PATH = ".github/workflows/steelengine-release-checks.yml";
const WRAPPER_PATH = "scripts/github/run-steelengine-cross-os-release-checks.sh";
const SCRIPT_PATH = "scripts/steelengine-cross-os-release-checks.ts";
const HARNESS = "bash workflow/scripts/github/run-steelengine-cross-os-release-checks.sh";
...
describe("cross-OS release checks workflow", () => {
  ...
  it("binds the prepared release package to an immutable artifact and package tuple", () => {
    ...
    expect(metadata.run).toContain("file_name=steelengine-current.tgz");
  ...
  it("downloads and re-exports exact candidate artifacts only by immutable id", () => {
    ...
    expect(resolve.run).toContain("resolve-steelengine-package-candidate.mjs");
    ...
    expect(upload.with?.name).toBe(
      "steelengine-cross-os-release-checks-candidate-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    ...
    expect(baselineUpload.with?.name).toBe(
      "steelengine-cross-os-release-checks-baseline-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    ...
    expect(binding.env).toMatchObject({
      ...
      ARTIFACT_NAME:
        "${{ format('steelengine-cross-os-release-checks-candidate-{0}-{1}', needs.prepare.outputs.candidate_artifact_run_id, needs.prepare.outputs.candidate_artifact_run_attempt) }}",
      ...
      BASELINE_ARTIFACT_NAME:
        "${{ format('steelengine-cross-os-release-checks-baseline-{0}-{1}', needs.prepare.outputs.baseline_artifact_run_id, needs.prepare.outputs.baseline_artifact_run_attempt) }}",
  ...
  it("executes the release harness directly with Node", () => {
    ...
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          STEELENGINE_RELEASE_CHECKS_SCRIPT: SCRIPT_PATH,
        },
      },
    );
