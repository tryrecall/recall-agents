...
describe("k8s manifests", () => {
  ...
  it("keeps gateway service selectors and ports aligned with deployment labels", () => {
    ...
    expect(deployment).toMatchObject({
      apiVersion: "apps/v1",
      kind: "Deployment",
      metadata: { name: "steelengine" },
    });
    expect(matchLabels).toEqual({ app: "steelengine" });
  ...
  it("keeps deployment mounts, secrets, and security posture deployable", () => {
    ...
    expect(findNamed(env, "STEELENGINE_CONFIG_DIR")).toMatchObject({ value: "/home/node/.steelengine" });
    expect(findNamed(env, "STEELENGINE_GATEWAY_TOKEN")).toMatchObject({
      valueFrom: { secretKeyRef: { key: "STEELENGINE_GATEWAY_TOKEN", name: "steelengine-secrets" } },
    });
    expect(findNamed(volumes, "steelengine-home")).toMatchObject({
      persistentVolumeClaim: { claimName: "steelengine-home-pvc" },
    });
    expect(findNamed(volumes, "config")).toMatchObject({ configMap: { name: "steelengine-config" } });
  ...
  it("keeps config and persistence manifests aligned with the gateway", () => {
    ...
    const config = JSON.parse(String(data["steelengine.json"])) as Record<string, unknown>;
    const gateway = asRecord(config.gateway, "steelengine config gateway");
    const auth = asRecord(gateway.auth, "steelengine config auth");
    const agents = asRecord(config.agents, "steelengine config agents");
    const defaults = asRecord(agents.defaults, "steelengine config agent defaults");
    ...
    expect(configMap).toMatchObject({
      apiVersion: "v1",
      kind: "ConfigMap",
      metadata: { name: "steelengine-config" },
    });
    expect(gateway).toMatchObject({ mode: "local", port: 18789 });
    expect(auth).toMatchObject({ mode: "token" });
    expect(defaults).toMatchObject({ workspace: "~/.steelengine/workspace" });
    expect(data["AGENTS.md"]).toContain("SteelEngine Assistant");
    expect(pvc).toMatchObject({
      apiVersion: "v1",
      kind: "PersistentVolumeClaim",
      metadata: { name: "steelengine-home-pvc" },
    });
