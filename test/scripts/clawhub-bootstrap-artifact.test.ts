...
function fixture() {
  const root = mkdtempSync(join(tmpdir(), "steelengine-clawhub-bootstrap-"));
  tempDirs.push(root);
  ...
  writeFileSync(join(packageRoot, "steelengine-meta-2026.7.1-beta.3.tgz"), "packed meta");
  writeFileSync(
    join(existingPackageRoot, "steelengine-existing-2026.7.1-beta.3.tgz"),
    "packed existing",
  );
  ...
  writeFileSync(
    ...
    JSON.stringify([
      ...
      {
        packageName: "@steelengine/existing",
        version: "2026.7.1-beta.3",
...
function common(paths: ReturnType<typeof fixture>) {
  return {
    ...
    plugins: "@steelengine/meta,@steelengine/existing",
...
function writeClawPack(
  ...
  const root = mkdtempSync(join(tmpdir(), "steelengine-clawhub-packed-"));
  tempDirs.push(root);
...
describe("ClawHub bootstrap artifact manifest", () => {
  it("binds the exact package set and packed file identity", async () => {
    ...
    const meta = created.entries.find((entry) => entry.packageName === "@steelengine/meta");
  ...
  it("rejects changed bytes and extra artifact files", async () => {
    ...
    writeFileSync(
      join(paths.artifactRoot, "packages", "meta", "steelengine-meta-2026.7.1-beta.3.tgz"),
      "changed",
    );
    await expect(
      verifyClawHubBootstrapArtifactManifest({
  ...
  it("binds exact target bytes to configure-only repairs", async () => {
    ...
    const existing = manifest.entries.find((entry) => entry.packageName === "@steelengine/existing");
    expect(existing).toMatchObject({
      artifactPath: "packages/existing/steelengine-existing-2026.7.1-beta.3.tgz",
      size: 15,
    });
...
describe("ClawHub packed artifact identity", () => {
  const expectedIdentity = {
    ...
    expectedName: "@steelengine/meta-provider",
    ...
    const pack = writeClawPack([
      {
        ...
        contents: JSON.stringify({
          name: "@steelengine/meta-provider",
          version: "2026.7.1-beta.3",
          steelengine: {
            release: {
  ...
  it("rejects a whitespace-bearing alias before a later package.json", async () => {
    const pack = writeClawPack([
      ...
      },
      {
        ...
        contents: JSON.stringify({
          name: "@steelengine/other",
          version: "9.9.9",
        }),
      },
      {
        name: "package/steelengine.plugin.json",
        contents: JSON.stringify({ id: "meta" }),
      },
    ]);
  ...
  it("rejects a compressed artifact above the ClawHub package limit before reading it", async () => {
    const root = mkdtempSync(join(tmpdir(), "steelengine-clawhub-packed-limit-"));
    tempDirs.push(root);
  ...
  it("bounds expanded tar bytes", async () => {
    const root = mkdtempSync(join(tmpdir(), "steelengine-clawhub-expanded-limit-"));
    tempDirs.push(root);
