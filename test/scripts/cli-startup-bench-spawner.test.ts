...
describe("CLI startup benchmark script spawners", () => {
  ...
  it("does not require unrelated fixture cases for a narrowed preset", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-bench-budget-test-"));
    try {
      ...
      expect(() =>
        execFileSync(
          ...
          ],
          {
            ...
            env: {
              ...process.env,
              STEELENGINE_STARTUP_BENCH_ENFORCE_NONCANONICAL_ARCH: "1",
            },
            stdio: "pipe",
          },
        ),
  ...
  it("rejects narrowed preset reports with no matching current cases", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-bench-budget-empty-test-"));
    try {
      const baselinePath = path.join(tmpDir, "baseline.json");
      const reportPath = path.join(tmpDir, "current.json");
      fs.writeFileSync(
  ...
  it("rejects narrowed preset reports with unrelated current cases when baseline checks run", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-bench-budget-overlap-test-"));
    try {
      const baselinePath = path.join(tmpDir, "baseline.json");
      const reportPath = path.join(tmpDir, "current.json");
      const makeCase = (id: string, name: string) => ({
  ...
  it("allows skip-baseline reports without fixture case overlap", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-bench-budget-skip-test-"));
    try {
      const baselinePath = path.join(tmpDir, "baseline.json");
      const reportPath = path.join(tmpDir, "current.json");
      const makeCase = (id: string, name: string) => ({
  ...
  it("skips x64 startup budgets on noncanonical architectures", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-bench-budget-arch-test-"));
    try {
  ...
  it("fails reused reports with timed-out samples", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-bench-budget-timeout-test-"));
    try {
      const baselinePath = path.join(tmpDir, "baseline.json");
      const reportPath = path.join(tmpDir, "current.json");
      const timedOutCase = {
  ...
  it("fails reused reports with missing RSS samples", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-bench-budget-rss-test-"));
    try {
      const baselinePath = path.join(tmpDir, "baseline.json");
      const reportPath = path.join(tmpDir, "current.json");
  ...
  it("rejects malformed startup budget env vars before reading reports", () => {
    const result = spawnSync(process.execPath, ["scripts/test-cli-startup-bench-budget.mjs"], {
      ...
      env: {
        ...process.env,
        STEELENGINE_STARTUP_BENCH_MAX_RSS_REGRESSION_PCT: "20pct",
      },
    });
    ...
    expect(result.stderr).toContain(
      "STEELENGINE_STARTUP_BENCH_MAX_RSS_REGRESSION_PCT must be a non-negative number",
    );
