...
import {
  ...
  resolveUpgradeSurvivorSteelEngineCommand,
  runUpgradeSurvivorSteelEngineStep,
...
describe("upgrade survivor config recipe command resolution", () => {
  ...
  it("wraps Windows steelengine npm shims through cmd.exe", () => {
    expect(
      resolveUpgradeSurvivorSteelEngineCommand(
      ...
      args: [
        ...
        "/s",
        "/c",
        'steelengine.cmd config set models.providers.openai "{""apiKey"":""sk test""}" --strict-json',
      ],
      command: String.raw`C:\Windows\System32\cmd.exe`,
      commandLabel:
        'steelengine config set models.providers.openai {"apiKey":"sk test"} --strict-json',
      shell: false,
      windowsVerbatimArguments: true,
    });
  });

  it("keeps POSIX steelengine invocations direct", () => {
    expect(
      resolveUpgradeSurvivorSteelEngineCommand(["config", "validate"], {
        platform: "linux",
      }),
    ).toEqual({
      args: ["config", "validate"],
      command: "steelengine",
      commandLabel: "steelengine config validate",
      shell: false,
    });
  });
  ...
  it("bounds baseline config commands and reports spawn errors", () => {
    const calls: unknown[] = [];
    const timeoutError = Object.assign(new Error("spawnSync steelengine ETIMEDOUT"), {
      code: "ETIMEDOUT",
    });

    const outcome = runUpgradeSurvivorSteelEngineStep(
      {
    ...
    expect(outcome).toMatchObject({
      command: "steelengine config validate",
      errorCode: "ETIMEDOUT",
      errorMessage: "spawnSync steelengine ETIMEDOUT",
      ok: false,
      signal: "SIGTERM",
      status: null,
      stderr: "still validating",
  ...
  it("skips ACPX bridge config on baselines before the bridge field existed", () => {
    const root = mkdtempSync(join(tmpdir(), "steelengine-upgrade-recipe-acpx-"));
    try {
      const binDir = join(root, "bin");
      const logPath = join(root, "steelengine-argv.jsonl");
      const summaryPath = join(root, "summary.json");
      mkdirSync(binDir, { recursive: true });
      const steelengineLogPath = join(binDir, "steelengine-log.js");
      const steelenginePath = join(binDir, "steelengine");
      const steelengineCmdPath = join(binDir, "steelengine.cmd");
      writeFileSync(
        steelengineLogPath,
        `
...
`,
      ...
      writeFileSync(steelenginePath, `#!/usr/bin/env node\nrequire("./steelengine-log.js");\n`);
      chmodSync(steelenginePath, 0o755);
      writeFileSync(
        steelengineCmdPath,
        `@echo off\r\n"${process.execPath}" "%~dp0steelengine-log.js" %*\r\n`,
      );
      ...
      expect(summary.skippedIntents).toContain("acpx-steelengine-tools-bridge");
      expect(loggedArgs).not.toContainEqual(
        expect.arrayContaining([
          "set",
          "plugins",
          expect.stringContaining("steelEngineToolsMcpBridge"),
        ]),
      );
    } finally {
