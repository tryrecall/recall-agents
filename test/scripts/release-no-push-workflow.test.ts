...
const LIVE_E2E = ".github/workflows/steelengine-live-and-e2e-checks-reusable.yml";
const INSTALL_SMOKE = ".github/workflows/install-smoke.yml";
const SHARED_IMAGE_PUBLISHER = ".github/workflows/steelengine-shared-image-publish-reusable.yml";
const SCHEDULED_LIVE = ".github/workflows/steelengine-scheduled-live-checks.yml";
const DOCKER_RELEASE = ".github/workflows/docker-release.yml";
const UPDATE_MIGRATION = ".github/workflows/update-migration.yml";
const PERFORMANCE = ".github/workflows/steelengine-performance.yml";
...
describe("release validation no-push transport", () => {
  it("builds planned live images locally without entering pull fallback", () => {
    ...
    for (const jobName of [
      ...
      expect(runStep?.run, jobName).toContain("STEELENGINE_SKIP_DOCKER_BUILD=0");
      expect(runStep?.run, jobName).not.toContain("STEELENGINE_DOCKER_BUILD_ON_MISSING=1");
    }
  });
  ...
  it("models conditional reusable jobs as permission requests before scheduling", () => {
    const root = mkdtempSync(join(tmpdir(), "steelengine-permission-graph-"));
  ...
  it("keeps the Release SHA wrapper as the durable evidence identity", () => {
    ...
    expect(verify.run).toContain('"steelengine-release-checks.yml"');
    ...
    expect(dispatch.run).toContain("https://api.github.com/repos/steelengine/releases/dispatches");
  });
  ...
  it("pins every Full Release Docker caller to artifact-only transport", () => {
    ...
    expect(fullText).toContain("dispatch_and_wait steelengine-release-checks.yml");
    expect(fullText).toContain("gh workflow run steelengine-performance.yml");
  ...
  it("builds shared images locally, verifies artifacts, and cannot fall back to a registry", () => {
    ...
    const packageIdentity = step(dockerProducer, "Validate SteelEngine package artifact identity");
    ...
    for (const [name, condition] of [
      [
        "Download current-run SteelEngine Docker E2E package",
        "inputs.package_artifact_run_id == github.run_id",
      ],
      [
        "Download previous-run SteelEngine Docker E2E package",
        "inputs.package_artifact_run_id != github.run_id",
      ],
    ] as const) {
    ...
    expect(functionalBuild.run).toContain(
      "--build-context steelengine_package=.artifacts/docker-e2e-package",
    );
    ...
    const validatePackage = step(dockerProducer, "Validate SteelEngine Docker E2E package");
    ...
    for (const name of [
      ...
      expect(consumer.env?.STEELENGINE_DOCKER_E2E_REQUIRE_LOCAL_IMAGE).toContain("no-push-artifact");
      expect(step(consumer, "Download SteelEngine Docker E2E package").with).toMatchObject({
      ...
      expect(loadArtifact.env?.STEELENGINE_SHARED_IMAGE_RUN_ATTEMPT).toBe(
        "${{ needs.prepare_docker_e2e_image.outputs.image_artifact_run_attempt }}",
      );
      expect(loadArtifact.env?.STEELENGINE_SHARED_IMAGE_RUN_ID).toBe(
        "${{ needs.prepare_docker_e2e_image.outputs.image_artifact_run_id }}",
      );
      expect(loadArtifact.run).toContain("shared-image-artifact.sh");
      expect(loadArtifact.run).toContain('STEELENGINE_SHARED_IMAGE_ARCHIVE_SHA256="$ARCHIVE_SHA256"');
      expect(loadArtifact.run).toContain('STEELENGINE_SHARED_IMAGE_PACKAGE_SHA256="$PACKAGE_SHA256"');
    ...
    for (const name of [
      ...
      expect(consumer.env?.STEELENGINE_LIVE_REQUIRE_LOCAL_IMAGE).toContain("no-push-artifact");
    ...
    const requireLocalIndex = liveBuild.indexOf("STEELENGINE_LIVE_REQUIRE_LOCAL_IMAGE");
    ...
    const publisherCallers = readdirSync(".github/workflows")
      ...
      .filter((name) =>
        readFileSync(join(".github/workflows", name), "utf8").includes(
          "steelengine-shared-image-publish-reusable.yml",
        ),
      );
  ...
  it("fails a missing required local live image before any registry pull", () => {
    const root = mkdtempSync(join(tmpdir(), "steelengine-live-local-image-"));
...
`,
      ...
      const result = spawnSync("bash", [resolve(LIVE_BUILD)], {
        ...
        env: {
          ...
          STEELENGINE_LIVE_IMAGE: "steelengine-live-test:required-local",
          STEELENGINE_LIVE_REQUIRE_LOCAL_IMAGE: "1",
      ...
      expect(result.stderr).toContain(
        "Required local live-test image not found: steelengine-live-test:required-local",
      );
      expect(readFileSync(calls, "utf8")).toBe("image inspect steelengine-live-test:required-local\n");
    } finally {
      rmSync(root, { force: true, recursive: true });
    }
  });
  ...
  it("fails a missing required local Docker E2E image before pull or build fallback", () => {
    const root = mkdtempSync(join(tmpdir(), "steelengine-docker-e2e-local-image-"));
    const bin = join(root, "bin");
    const calls = join(root, "docker.log");
    try {
      mkdirSync(bin);
...
`,
      ...
      const result = spawnSync(
        ...
        [
          "-c",
          `source "$1"
docker_e2e_build_or_reuse "steelengine-e2e:required-local" "required local image test"`,
          "bash",
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            FAKE_DOCKER_LOG: calls,
            STEELENGINE_DOCKER_BUILD_ON_MISSING: "1",
            STEELENGINE_DOCKER_E2E_REQUIRE_LOCAL_IMAGE: "1",
            STEELENGINE_SKIP_DOCKER_BUILD: "1",
            PATH: `${bin}:${process.env.PATH ?? ""}`,
          },
        },
      ...
      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        "Required local Docker E2E image not found: steelengine-e2e:required-local",
      );
      expect(readFileSync(calls, "utf8")).toBe("image inspect steelengine-e2e:required-local\n");
    } finally {
      rmSync(root, { force: true, recursive: true });
    }
  });
