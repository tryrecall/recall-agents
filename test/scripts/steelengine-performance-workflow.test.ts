// Openclaw Performance Workflow tests cover steelengine performance workflow script behavior.
...
const WORKFLOW = ".github/workflows/steelengine-performance.yml";
...
describe("SteelEngine performance workflow", () => {
  it("uses an optional dispatch identifier to name parent-owned runs", () => {
    const workflow = readFileSync(WORKFLOW, "utf8");

    expect(workflow).toContain(
      "run-name: ${{ inputs.dispatch_id != '' && format('SteelEngine Performance {0}', inputs.dispatch_id) || 'SteelEngine Performance' }}",
    );
  ...
  it("resolves each target once before benchmark and publication fan out", () => {
    ...
    const resolveTarget = findStep("Resolve SteelEngine target ref", "resolve_target");
    const checkout = findStep("Checkout SteelEngine");
    const record = findStep("Record tested revision");
    const sourceCheckout = findStep("Checkout SteelEngine source target", "source_performance");
    ...
    expect(
      Object.values(workflow.jobs ?? {})
        ...
        .filter((step) => step.name === "Resolve SteelEngine target ref"),
    ).toHaveLength(1);
  });
  ...
  it("sparse-fetches only the public source baseline without publisher credentials", () => {
    ...
    expect(run).toContain('remote add origin "https://github.com/steelengine/clawgrit-reports.git"');
  ...
  it("builds only the QA and startup artifacts required by source probes", () => {
    const run = findStep("Run SteelEngine source performance probes", "source_performance").run ?? "";
    const build = "STEELENGINE_BUILD_PRIVATE_QA=1 node scripts/build-all.mjs sourcePerformance";
  ...
  it("isolates required publication in a fresh artifact-consuming job", () => {
    ...
    expect(kovaSteps.some((step) => step.name === "Run SteelEngine source performance probes")).toBe(
      false,
    );
    expect(
      workflow.jobs?.source_performance?.steps?.some(
        (step) => step.name === "Run SteelEngine source performance probes",
      ),
    ).toBe(true);
  ...
  it("keeps app credentials out of artifact processing and scopes them to Git push", () => {
    ...
    expect(prepare.run).toContain(
      'source_path="${input_root}/steelengine-performance-source-${GITHUB_RUN_ID}-${SOURCE_PRODUCER_ATTEMPT}/${LANE_ID}"',
    );
    ...
    expect(prepare.run).toContain(
      'remote add origin "https://github.com/steelengine/clawgrit-reports.git"',
    );
  ...
  it("publishes bounded bundle metadata while retaining full diagnostics as an artifact", () => {
    ...
    expect(upload.with?.path).not.toContain(".artifacts/steelengine-performance/source");
    expect(sourceUpload.with).toMatchObject({
      name: "steelengine-performance-source-${{ github.run_id }}-${{ github.run_attempt }}",
      path: ".artifacts/steelengine-performance/source",
  ...
  it("reuses the producing artifact when only publisher jobs rerun", () => {
    const artifact = findStep("Resolve Kova artifact", "publish");
    const root = mkdtempSync(join(realpathSync(tmpdir()), "steelengine-artifact-resolver-"));
    ...
    writeFileSync(
      ...
      `#!/bin/sh
printf '%s\\n' \
  '101	steelengine-performance-mock-provider-9001-1' \
  '202	steelengine-performance-source-9001-2' \
  '303	steelengine-performance-mock-provider-9001-3'
`,
    );
  ...
  it("advertises a clawgrit URL only after a direct or remotely verified push", () => {
    const publish = findStep("Publish to clawgrit reports", "publish");
    const root = mkdtempSync(join(realpathSync(tmpdir()), "steelengine-publish-shell-"));
    const bin = join(root, "bin");
    const reportsRoot = join(root, "reports");
    const reportUrl =
      "https://github.com/steelengine/clawgrit-reports/tree/main/steelengine-performance/main/123-1/mock-provider";
    mkdirSync(bin);
...
case "$*" in
  ...
  *"remote get-url origin"*) echo https://github.com/steelengine/clawgrit-reports.git ;;
...
`,
    ...
    const execute = ({
      ...
      const result = spawnSync("bash", ["-c", publish.run ?? ""], {
        ...
        env: {
          ...
          DEST_REL: "steelengine-performance/main/123-1/mock-provider",
  ...
  it("preserves both reports when concurrent writers update one latest pointer", () => {
    const root = mkdtempSync(join(realpathSync(tmpdir()), "steelengine-report-race-"));
    ...
    const latest = "steelengine-performance/main/latest-mock-provider.json";
    const reportA = "steelengine-performance/main/100-1/mock-provider";
    const reportB = "steelengine-performance/main/200-1/mock-provider";
    ...
    const commitReport = (repo: string, reportPath: string, marker: string) => {
      ...
      runGit(repo, ["add", "--", "steelengine-performance"]);
  ...
  it("installs local workspace packages beside the OCM root tarball", () => {
    ...
    expect(workflow.jobs?.kova?.env).not.toHaveProperty("STEELENGINE_OCM_RUNTIME_BUILD_PROFILE");
    ...
    expect(configure.run).toContain(
      "STEELENGINE_OCM_WORKSPACE_DEPENDENCY_DIRS=$workspace_dependency_dirs",
    );
  });
