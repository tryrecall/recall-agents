...
describe("scripts/ui windows spawn behavior", () => {
  ...
  it("detects direct execution through a junctioned script path", () => {
    const realScriptPath = path.resolve("repo/steelengine/scripts/ui.js");
    const junctionScriptPath = path.resolve("linked/steelengine/scripts/ui.js");
  ...
  it.runIf(process.platform !== "win32").each(["SIGTERM", "SIGHUP"] as const)(
    ...
    async (signal) => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-ui-wrapper-signals-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-ui-wrapper-tree-"));
      const runnerPath = path.join(tempDir, "pnpm.mjs");
      const readyFile = path.join(tempDir, "ready");
