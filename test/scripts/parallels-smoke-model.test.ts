...
describe("Parallels smoke model selection", () => {
  ...
  it("extracts the last SteelEngine version from a bounded log tail", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-log-tail-"));
    const logPath = join(tempDir, "phase.log");
    try {
      writeFileSync(logPath, ["SteelEngine 0.0.1", "x".repeat(4096), "SteelEngine 2026.6.7"].join("\n"));

      await expect(extractLastSteelEngineVersionFromLog(logPath, undefined, 128)).resolves.toBe(
        "2026.6.7",
      );
    } finally {
  ...
  it("owns the reusable Windows VM and SteelEngine baseline lifecycle", () => {
    ...
    expect(controller).toContain("pre-steelengine-native-e2e-");
    ...
    expect(controller).toContain("STEELENGINE_PARALLELS_WINDOWS_LIBRARY_ONLY");
    expect(controller).not.toContain("steelengine-windows-node");
  });
  ...
  it("preserves caller arguments when loaded as the Windows controller library", () => {
    const result = spawnSync(
      ...
      [
        "-c",
        'set -- run-tests --app-option; STEELENGINE_PARALLELS_WINDOWS_LIBRARY_ONLY=1 source "$1"; printf "%s\\n" "$*"',
        "bash",
  ...
  it("bounds Windows prerequisite metadata downloads", () => {
    const result = spawnSync(
      ...
      [
        "-c",
        'STEELENGINE_PARALLELS_WINDOWS_LIBRARY_ONLY=1 source "$1"; curl() { printf "%s\\n" "$@"; }; fetch_host_metadata "https://example.test/metadata"',
        "bash",
        WINDOWS_PREPARE_WRAPPER,
      ],
      { encoding: "utf8" },
    ...
    const tempDir = makeTempDir(tempDirs, "steelengine-windows-metadata-retry-");
    ...
    const retryResult = spawnSync(
      ...
      [
        "-c",
        `STEELENGINE_PARALLELS_WINDOWS_LIBRARY_ONLY=1 source "$1"
curl() {
...
fetch_host_metadata "https://example.test/metadata"`,
  ...
  it("accepts leading package-manager separators and still honors later terminators", () => {
    ...
    expect(parseNpmUpdateSmokeArgs(["--", "--package-spec", "steelengine@2026.5.1"]).packageSpec).toBe(
      "steelengine@2026.5.1",
    );
    expect(
  ...
  it("keeps provider auth and model defaults in the shared TypeScript helper", () => {
    ...
    expect(providerAuth).toContain("STEELENGINE_PARALLELS_OPENAI_MODEL");
    expect(providerAuth).toContain("STEELENGINE_PARALLELS_WINDOWS_OPENAI_MODEL");
  ...
  it("keeps Windows provider-only plugin isolation temp scripts per run", () => {
    ...
    expect(script).toContain("steelengine-parallels-plugin-isolation-");
    expect(script).not.toContain("'steelengine-parallels-plugin-isolation.cjs'");
    expect(script).toContain("try {");
    expect(script).toContain("} finally {");
    expect(script).toContain(
      "Remove-Item $isolationScriptPath -Force -ErrorAction SilentlyContinue",
    );
    expect(script).toContain("Remove-Item Env:STEELENGINE_PARALLELS_PLUGIN_ISOLATION");
  });
  ...
  it("keeps snapshot, host, package, and quote helpers shared", () => {
    ...
    expect(packageArtifactExports).toContain("packSteelEngine");
    expect(packageArtifactExports).toContain("packageVersionFromTgz");
    expect(packageArtifactExports).toContain("resolveSteelEngineRegistryVersion");
    ...
    expect(packageArtifact).toContain("export async function packSteelEngine");
    ...
    expect(hostServer).toContain('STEELENGINE_NPM_REGISTRY_UPSTREAM: "https://registry.npmjs.org"');
  ...
  it("accepts npm 10/11 array and npm 12 workspace result shapes", () => {
    expect(
      packageArtifactTesting.resolveNpmPackTarballFilename([
        { filename: "steelengine-2026.6.11.tgz" },
      ]),
    ).toBe("steelengine-2026.6.11.tgz");
    expect(
      packageArtifactTesting.resolveNpmPackTarballFilename({
        steelengine: { filename: "steelengine-2026.6.11.tgz" },
      }),
    ).toBe("steelengine-2026.6.11.tgz");
  });

  it("keeps fresh package locks with malformed owner pids", async () => {
    const lockDir = makeTempDir(tempDirs, "steelengine-parallels-package-lock-");
  ...
  it("removes a just-created package lock when owner writing fails", async () => {
    const parentDir = makeTempDir(tempDirs, "steelengine-parallels-package-lock-parent-");
  ...
  it("uses a temporary npmrc file and cleans it after resolving the latest package version", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "steelengine-parallels-version-"));
    let userConfigPath = "";
    try {
      const version = resolveLatestVersion("", {
        createTempDir: (prefix) => {
          expect(prefix).toBe(join(tmpdir(), "steelengine-npm-"));
          return mkdtempSync(join(tempRoot, "npm-"));
        },
        runCommand: (command, args, options) => {
          userConfigPath = args.at(-1) ?? "";
          expect(command).toBe("npm");
          expect(args).toEqual(["view", "steelengine", "version", "--userconfig", userConfigPath]);
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-host-server-"));
...
`,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-host-server-signal-"));
      const fakePython = join(tempDir, "python3");
      writeFileSync(
        fakePython,
        `#!/usr/bin/env bash
...
`,
  ...
  it("quotes shell args and resolves fuzzy snapshot hints through the shared TypeScript helper", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-helper-"));
    writeFakePrlctl(
      tempDir,
      `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  it("resolves a latest snapshot hint to the matching version before older LATEST labels", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-snapshot-latest-"));
    writeFakePrlctl(
      tempDir,
      `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  it("rejects skip-restore for combined Parallels smoke lanes", () => {
    ...
    expect(invalidSkipBothResult.stderr).toContain(
      "STEELENGINE_PARALLELS_SKIP_SNAPSHOT_RESTORE=1 requires --mode fresh or --mode upgrade",
    );
    expect(() =>
  ...
  it("uses one Ubuntu VM fallback resolver for Linux lanes", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-vm-helper-"));
    writeFakePrlctl(
      tempDir,
      `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  it("uses the only macOS VM when the default name is unavailable", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-macos-vm-helper-"));
    writeFakePrlctl(
      tempDir,
      `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  it("does not infer destructive macOS smoke targets from arbitrary names", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-macos-vm-guard-"));
    writeFakePrlctl(
      tempDir,
      `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  it("resumes suspended Parallels VMs", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-vm-resume-"));
...
`,
  ...
  it("keeps Linux bad-plugin diagnostics gated for historical update baselines", () => {
    ...
    expect(script).toContain("parseSteelEnginePackageVersion");
      ...
      withEnv(
        {
          ...
          STEELENGINE_PARALLELS_WINDOWS_OPENAI_MODEL: "openai/custom-windows",
        },
        () => resolveWindowsProviderAuth({ provider: "openai" }),
      ),
    ).toEqual({
  ...
  it("cleans POSIX guest scripts after the phase deadline is exhausted", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-posix-cleanup-"));
...
printf '%s\\n' "$*" >>"$log_path"
args=" $* "
if [[ "$args" == *" dd of=/tmp/steelengine-parallels-"* || "$args" == *" /bin/dd of=/tmp/steelengine-parallels-"* ]]; then
  cat >/dev/null
  exit 0
fi
if [[ "$args" == *" bash /tmp/steelengine-parallels-"* || "$args" == *" /bin/bash /tmp/steelengine-parallels-"* ]]; then
  exit 1
fi
if [[ "$args" == *" /bin/rm -f /tmp/steelengine-parallels-"* ]]; then
  printf 'cleanup\\n' >>"$log_path"
  exit 0
fi
exit 0
...
`,
  ...
  it("rejects Parallels macOS guest session false-success output", () => {
    const tempDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-session-unavailable-"));
...
`,
  ...
  it("streams full phase logs to disk while bounding the failure tail", async () => {
    const runDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-phase-"));
  ...
  it("clamps oversized phase timers before scheduling", async () => {
    const runDir = makeTempDir(tempDirs, "steelengine-parallels-phase-timeout-");
    const phaseRunner = new PhaseRunner(runDir, 128);
  ...
  it("preseeds dev update channel before stable-to-dev update lanes", () => {
    ...
    expect(macos).toContain("STEELENGINE_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1");
    expect(windows).toContain("STEELENGINE_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS");
  });
  ...
  it("passes aggregate model overrides into each OS fresh lane", () => {
    ...
    expect(script).toContain("STEELENGINE_PARALLELS_LINUX_DISABLE_BONJOUR");
  });
  ...
  it("keeps aggregate update guest scripts isolated from the npm-update orchestrator", () => {
    ...
    expect(updateScripts).toContain("Invoke-SteelEngine update");
  ...
  it("resolves macOS smoke commands from the guest PATH", () => {
    ...
    expect(macos).toContain('const guestSteelEngine = "steelengine"');
    expect(macos).toContain('const guestNode = "node"');
    expect(macos).toContain('const guestNpm = "npm"');
    expect(macos).toContain("$(npm root -g)/steelengine/steelengine.mjs");
    expect(macos).toContain("guestSteelEngineEntryExec");
    expect(macos).not.toContain('const guestSteelEngine = "/opt/homebrew/bin/steelengine"');
    ...
    expect(macos).not.toContain("/opt/homebrew/lib/node_modules/steelengine/steelengine.mjs");
  });

  it("keeps Windows gateway reachability on a real deadline with start recovery", () => {
    const script = readFileSync(TS_PATHS.windows, "utf8");

    expect(script).toContain("STEELENGINE_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S");
  ...
  it("runs Windows ref onboarding through a detached done-file runner", () => {
    ...
    expect(transports).toContain("Join-Path (Join-Path $env:WINDIR 'Temp\\\\steelengine-parallels')");
    expect(transports).toContain("icacls.exe $runDir /inheritance:r");
    expect(transports).toContain("__STEELENGINE_BACKGROUND_DONE__");
    expect(transports).toContain("__STEELENGINE_BACKGROUND_EXIT__");
    ...
    expect(transports).toContain("Write-SteelEngineUtf8File $pidPath ([string]$PID)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const tempDir = makeTempDir(tempDirs, "steelengine-parallels-host-command-drain-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-host-command-"));
...
`,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = makeTempDir(tempDirs, "steelengine-parallels-host-command-signal-");
      ...
      try {
        const grandchildScript = [
          "const { writeFileSync } = require('node:fs');",
          "writeFileSync(process.env.STEELENGINE_TEST_GRANDCHILD_PID, String(process.pid));",
          "process.on('SIGTERM', () => {});",
          "setInterval(() => {}, 1000);",
        ].join("\n");
        const parentScript = [
          ...
          "writeFileSync(process.env.STEELENGINE_TEST_READY_FILE, 'ready');",
          "process.on('SIGTERM', () => process.exit(0));",
          "setInterval(() => {}, 1000);",
        ].join("\n");
        writeFileSync(
          ...
          [
            ...
            "  env: {",
            "    ...process.env,",
            `    STEELENGINE_TEST_GRANDCHILD_PID: ${JSON.stringify(grandchildPidPath)},`,
            `    STEELENGINE_TEST_READY_FILE: ${JSON.stringify(readyPath)},`,
            "  },",
            "  quiet: true,",
            "  timeoutMs: 30_000,",
            "});",
  ...
  it.runIf(process.platform !== "win32")("preserves timed host command spawn errors", () => {
    expect(() =>
      run("steelengine-definitely-missing-host-command", [], {
        check: false,
        quiet: true,
        timeoutMs: 50,
      }),
  ...
  it("rejects streaming host commands when log writes fail", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "steelengine-parallels-host-command-log-"));
    try {
      await expect(
  ...
  it("clears streaming host command timers when spawn fails", async () => {
    ...
    try {
      await expect(
        runStreaming("steelengine-definitely-missing-host-command", [], {
          quiet: true,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = makeTempDir(tempDirs, "steelengine-parallels-streaming-host-command-drain-");
      const readyFile = join(tempDir, "ready");
      const drainFile = join(tempDir, "drained");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = makeTempDir(tempDirs, "steelengine-parallels-streaming-host-command-signal-");
      const runnerPath = join(tempDir, "runner.mjs");
      const readyPath = join(tempDir, "ready");
      const grandchildPidPath = join(tempDir, "grandchild.pid");
      const logPath = join(tempDir, "stream.log");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const result = run(
        process.execPath,
        ["-e", "process.stderr.write('__STEELENGINE_HOST_COMMAND_SPAWN_ERROR__{}\\n')"],
        {
          check: false,
          quiet: true,
          timeoutMs: 500,
  ...
  it("routes Windows host pnpm and npm shims through safe runners", () => {
    ...
    expect(
      resolveHostCommandInvocation("npm", ["view", "steelengine", "version"], {
        env: { ComSpec: comSpec },
        execPath,
      ...
      args: ["/d", "/s", "/c", `${npmCmdPath} view steelengine version`],
      command: comSpec,
      shell: false,
      windowsVerbatimArguments: true,
    });
  ...
  it("runs the Windows agent turn through the detached done-file runner", () => {
    ...
    expect(script).toContain('guestPowerShellBackground(\n      "agent-turn"');
    expect(script).toContain("STEELENGINE_PARALLELS_WINDOWS_AGENT_TIMEOUT_S");
    expect(script).toContain(
  ...
  it("gives GPT-5.6 Luna enough Parallels model time on slower desktop guests", () => {
    ...
    expect(readFileSync(TS_PATHS.macos, "utf8")).toContain(
      'this.agentTimeoutSeconds = readPositiveIntEnv("STEELENGINE_PARALLELS_MACOS_AGENT_TIMEOUT_S", 2700)',
    );
  ...
  it("rejects loose Parallels numeric limits before starting smoke lanes", () => {
    expect(
      withEnv({ STEELENGINE_PARALLELS_MODEL_TIMEOUT_S: "1200" }, () =>
        resolveParallelsModelTimeoutSeconds("linux"),
      ),
    ).toBe(1200);
    expect(
      withEnv({ STEELENGINE_PARALLELS_NUMERIC_TEST: " 42 " }, () =>
        readPositiveIntEnv("STEELENGINE_PARALLELS_NUMERIC_TEST", 7),
      ),
    ).toBe(42);

    expectFatalError(
      () =>
        withEnv({ STEELENGINE_PARALLELS_MACOS_MODEL_TIMEOUT_S: "1800s" }, () =>
          resolveParallelsModelTimeoutSeconds("macos"),
        ),
      "invalid STEELENGINE_PARALLELS_MACOS_MODEL_TIMEOUT_S: 1800s",
    );
    expectFatalError(
    ...
    expectFatalError(
      () =>
        withEnv({ STEELENGINE_PARALLELS_LINUX_AGENT_TIMEOUT_S: "1e3" }, () =>
          readPositiveIntEnv("STEELENGINE_PARALLELS_LINUX_AGENT_TIMEOUT_S", 1500),
        ),
      "invalid STEELENGINE_PARALLELS_LINUX_AGENT_TIMEOUT_S: 1e3",
    );
    expectFatalError(
      () =>
        withEnv({ STEELENGINE_PARALLELS_WINDOWS_AGENT_TIMEOUT_S: "2700s" }, () =>
          readPositiveIntEnv("STEELENGINE_PARALLELS_WINDOWS_AGENT_TIMEOUT_S", 2700),
        ),
      "invalid STEELENGINE_PARALLELS_WINDOWS_AGENT_TIMEOUT_S: 2700s",
    );
    expectFatalError(
      () =>
        withEnv({ STEELENGINE_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S: "12.5" }, () =>
          readPositiveIntEnv("STEELENGINE_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S", 7200),
        ),
      "invalid STEELENGINE_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S: 12.5",
    );
    expectFatalError(
    ...
    expect(readFileSync(TS_PATHS.macos, "utf8")).toContain(
      'this.updateDevTimeoutSeconds = readPositiveIntEnv(\n      "STEELENGINE_PARALLELS_MACOS_UPDATE_DEV_TIMEOUT_S"',
    );
    expect(readFileSync(TS_PATHS.linux, "utf8")).toContain(
    ...
    expect(readFileSync(TS_PATHS.packageArtifact, "utf8")).toContain(
      'readPositiveIntEnv("STEELENGINE_PARALLELS_PACKAGE_LOCK_TIMEOUT_MS", 30 * 60_000)',
    );
    expect(readFileSync(TS_PATHS.npmUpdate, "utf8")).toContain(
      'readPositiveIntEnv("STEELENGINE_PARALLELS_NPM_UPDATE_TIMEOUT_S", 1200)',
    );
  });
  ...
  it("keeps Windows update-only env flags scoped before verification", () => {
    ...
    expect(windows).toContain(
      "Invoke-WithScopedEnv @{ STEELENGINE_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS",
    );
    expect(windows).toContain("$script:SteelEngineUpdateExit = $LASTEXITCODE");
    expect(windows).not.toContain("$env:STEELENGINE_DISABLE_BUNDLED_PLUGINS = '1'");
  });
  ...
  it("resolves Windows SteelEngine commands without assuming the npm shim path", () => {
    const powershell = readFileSync(TS_PATHS.powershell, "utf8");
    const windows = readFileSync(TS_PATHS.windows, "utf8");

    expect(powershell).toContain("windowsSteelEngineResolver");
    expect(powershell).toContain("STEELENGINE_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED");
    expect(powershell).toContain("Programs\\nodejs");
    expect(powershell).toContain('selectedModelEntry.agentRuntime = { id: "steelengine" }');
    ...
    expect(powershell).toContain("Resolve-SteelEngineCommand");
    ...
    expect(windows).toContain("windowsSteelEngineResolver");
    expect(windows).toContain("Invoke-SteelEngine gateway");
    expect(windows).not.toContain("Join-Path $env:APPDATA 'npm\\\\steelengine.cmd'");
  });
});
