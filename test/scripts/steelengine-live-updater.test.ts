...
} from "../../.agents/skills/steelengine-live-updater/scripts/update-main.mjs";
import {
  BUILD_STAMP_FILE,
...
const script = path.join(repoRoot, ".agents/skills/steelengine-live-updater/scripts/update-main.mjs");
...
function initializeFixture(root: string) {
  ...
  const canonicalOrigin = "https://github.com/steelengineai/recall-agents.git";
...
function makeFixture(options?: { includeSeed?: boolean }) {
  ...
  const root = realpathSync(mkdtempSync(path.join(tmpdir(), "steelengine-live-updater-")));
  const origin = path.join(root, "origin.git");
  const seed = path.join(root, "seed");
  const mirror = path.join(root, "mirror");
...
describe("steelengine live updater", () => {
  beforeAll(() => {
    const root = realpathSync(mkdtempSync(path.join(tmpdir(), "steelengine-live-updater-template-")));
  ...
  test("audits only error and warning logs emitted after Gateway restart", () => {
    const output = [
      { type: "meta", file: "/tmp/steelengine.log" },
  ...
  test("parses the loaded launchd ProgramArguments block", () => {
    expect(
      parseLaunchctlArguments(`gui/501/ai.steelengine.gateway = {
\tprogram = /bin/sh
\targuments = {
\t\t/bin/sh
\t\t/Users/test/.steelengine/service-env/ai.steelengine.gateway-env-wrapper.sh
\t\t/Users/test/.steelengine/service-env/ai.steelengine.gateway.env
\t\t/opt/homebrew/bin/node
\t\t/Users/test/steelengine/dist/index.js
\t\tgateway
\t\t--port
\t\t18789
\t}
\tpid = 123
  ...
  test("ignores restart-window logs emitted by a foreign SteelEngine checkout", () => {
    const root = mkdtempSync(path.join(tmpdir(), "steelengine-log-attribution-"));
    const sourceRoot = path.join(root, "managed/steelengine/dist");
    const foreignRoot = path.join(root, "worktree/steelengine");
    mkdirSync(path.join(foreignRoot, ".git"), { recursive: true });
    writeFileSync(path.join(foreignRoot, "package.json"), '{"name":"steelengine"}\n');
    const output = [
      {
        "0": '{"subsystem":"gateway"}',
        "1": "managed warning",
  ...
  test("keeps managed restart logs when the deployment root is symlinked", () => {
    const root = mkdtempSync(path.join(tmpdir(), "steelengine-log-symlink-attribution-"));
    ...
    writeFileSync(path.join(releaseRoot, "package.json"), '{"name":"steelengine"}\n');
  ...
  test("scopes embedded RPC records without dropping unattributed errors", () => {
    const root = mkdtempSync(path.join(tmpdir(), "steelengine-rpc-log-attribution-"));
    const sourceRoot = path.join(root, "managed/steelengine/dist");
    const foreignRoot = path.join(root, "worktree/steelengine");
    mkdirSync(path.join(foreignRoot, ".git"), { recursive: true });
    writeFileSync(path.join(foreignRoot, "package.json"), '{"name":"steelengine"}\n');
    ...
    expect(
      resolveManagedPluginSourceRoots({
        plugins: [
          ...
          { id: "workspace", rootDir: "/srv/workspace/.steelengine/extensions/workspace" },
          { id: "global", rootDir: "/Users/test/.steelengine/extensions/global" },
        ],
      }),
    ).toEqual([
      "/opt/configured-plugin",
      "/srv/workspace/.steelengine/extensions/workspace",
      "/Users/test/.steelengine/extensions/global",
    ]);
  ...
  test("scopes restart logs to the effective managed runtime", () => {
    expect(
      resolveManagedGatewaySourceRoot("/srv/steelengine", {
  ...
  test("retries bounded Gateway readiness after restart", () => {
    ...
    expect(calls).toEqual([
      "pnpm steelengine gateway status --deep --require-rpc --json",
      "pnpm steelengine gateway status --deep --require-rpc --json",
      "pnpm steelengine gateway status --deep --require-rpc --json",
      "pnpm steelengine health --verbose --json",
    ]);
  });
  ...
  test("bounds built Gateway CLI probes and cleans their config overlay", () => {
    ...
    expect(() =>
      runBuiltGatewayCli(
        ...
        ["gateway", "status"],
        {
          configPath: path.join(root, "steelengine.json"),
          entrypoint,
    ...
    expect(
      readdirSync(root).filter((name) => name.startsWith(".steelengine-live-updater-config-")),
    ).toEqual([]);
  });
    ...
    expect(
      prepareGatewaySuspension(
        ...
        (
          ...
          expect(params.requestId).toMatch(/^steelengine-live-updater-/u);
  ...
  test("pins managed Gateway calls with a backward-compatible local overlay", () => {
    const root = realpathSync(mkdtempSync(path.join(tmpdir(), "steelengine-gateway-call-")));
    ...
    writeFileSync(
      capture,
      'import fs from "node:fs"; console.log(JSON.stringify({ argv: process.argv.slice(2), config: JSON.parse(fs.readFileSync(process.env.STEELENGINE_CONFIG_PATH, "utf8")), hasToken: Boolean(process.env.STEELENGINE_GATEWAY_TOKEN), url: process.env.STEELENGINE_GATEWAY_URL ?? null }));\n',
    );
    ...
    const result = JSON.parse(
      runBuiltGatewayCall(
        ...
        { requestId: "request-1" },
        {
          ...
          port: 19001,
          serviceEnvironment: { STEELENGINE_GATEWAY_TOKEN: ["fixture", "value"].join("-") },
          wrapperPath: null,
        },
      ),
    );
  ...
  test("accepts supported SteelEngine GitHub origins", () => {
    expect(originMatches("https://github.com/steelengineai/recall-agents.git")).toBe(true);
    expect(originMatches("git@github.com:steelengineai/recall-agents.git")).toBe(true);
    expect(originMatches("https://github.com/example/steelengine.git")).toBe(false);
  });
  ...
  test("accepts only immutable canonical runtime snapshots owned by the checkout", () => {
    ...
    const runtimeRoot = path.join(home, ".steelengine/runtime");
    const snapshot = path.join(runtimeRoot, `gateway-${head.slice(0, 7)}`);
    mkdirSync(runtimeRoot, { recursive: true });
    git(runtimeRoot, "clone", origin, snapshot);
    git(snapshot, "remote", "set-url", "origin", "https://github.com/steelengineai/recall-agents.git");
  ...
  test("accepts owned entrypoints only in supported LaunchAgent command layouts", () => {
    ...
    const entrypoint = "/Users/test/.steelengine/runtime/gateway-1234567/dist/index.js";
    const wrapper = `${home}/.steelengine/service-env/ai.steelengine.gateway-env-wrapper.sh`;
    const envFile = `${home}/.steelengine/service-env/ai.steelengine.gateway.env`;
    ...
    const customWrapper = `${customState}/service-env/ai.steelengine.gateway-env-wrapper.sh`;
    const customEnvFile = `${customState}/service-env/ai.steelengine.gateway.env`;
    expect(
      resolveManagedGatewayEntrypoint(
  ...
  test("retargets an accepted snapshot to the exact source build", () => {
    const checkout = "/Users/test/steelengine";
    const snapshot = "/Users/test/.steelengine/runtime/gateway-1234567/dist/index.js";
    const source = path.join(checkout, "dist/index.js");
    const replacements: string[] = [];
    const deployment = {
      configPath: "/Users/test/config/steelengine.json",
      entrypoint: snapshot,
      entrypointIndex: 1,
      label: "ai.steelengine.gateway",
      plistPath: "/Users/test/Library/LaunchAgents/ai.steelengine.gateway.plist",
      port: 18789,
    };
  ...
  test("replaces a wrapped LaunchAgent entrypoint without inserting another argument", () => {
    const snapshot = "/Users/test/.steelengine/runtime/gateway-1234567/dist/index.js";
    const source = "/Users/test/steelengine/dist/index.js";
    const original = [
      "/bin/sh",
      "/Users/test/.steelengine/service-env/ai.steelengine.gateway-env-wrapper.sh",
      "/Users/test/.steelengine/service-env/ai.steelengine.gateway.env",
  ...
  test("fails closed when Gateway service retargeting does not stick", () => {
    ...
    expect(() =>
      repointManagedGatewayDeployment(
        checkout,
        {
          configPath: "/Users/test/.steelengine/steelengine.json",
          entrypoint: snapshot,
          label: "ai.steelengine.gateway",
          port: 18789,
        },
  ...
  test("rejects Git URL rewrites that change the effective fetch source", () => {
    ...
    git(mirror, "config", `url.${origin}.insteadOf`, "https://github.com/steelengineai/recall-agents.git");
  ...
  test("classifies exact-head build, install, macOS rebuild, and native UI proof", () => {
    ...
    expect(
      classifyActions(["package.json", "apps/macos/Sources/SteelEngine/AppDelegate.swift"], {
        buildProvenanceKnown: true,
        buildRequired: true,
        nodeModulesPresent: true,
      }),
    ...
    expect(
      classifyActions(["apps/shared/SteelEngineKit/Sources/SteelEngineProtocol/GatewayModels.swift"], {
        buildProvenanceKnown: true,
        buildRequired: true,
        nodeModulesPresent: true,
      }),
  ...
  test("accepts only the delayed exact target bundle process", () => {
    const executable = "/Users/steipete/steelengine/dist/SteelEngine.app/Contents/MacOS/SteelEngine";
    const foreign = "41 /tmp/agent/SteelEngine.app/Contents/MacOS/SteelEngine";
  ...
  test("fast-forwards, builds exact SHA, restarts Gateway, then proves exact Mac target", () => {
    ...
    mkdirSync(path.join(seed, "apps/macos/Sources/SteelEngine"), { recursive: true });
    writeFileSync(path.join(seed, "apps/macos/Sources/SteelEngine/App.swift"), "// changed\n");
    git(seed, "add", ".");
    ...
    const output = maintainFixture(
      ...
      },
      {
        ...
        verifyMacTarget: () => ({
          executable: path.join(mirror, "dist/SteelEngine.app/Contents/MacOS/SteelEngine"),
          pid: 123,
        }),
      },
    );
    ...
    expect(output.buildChangedPaths).toEqual(["apps/macos/Sources/SteelEngine/App.swift"]);
    ...
    expect(commands.calls).toEqual([
      ...
      "pnpm build",
      "pnpm steelengine gateway restart",
      "pnpm steelengine gateway status --deep --require-rpc --json",
      "pnpm steelengine health --verbose --json",
    ...
    expect(output.macTarget?.executable).toBe(
      path.join(mirror, "dist/SteelEngine.app/Contents/MacOS/SteelEngine"),
    );
  });
  ...
  test("preserves the signed Mac bundle while a Gateway build replaces dist", () => {
    const { root, mirror } = makeFixture();
    mkdirSync(path.join(mirror, "node_modules"));
    const appBundle = path.join(mirror, "dist/SteelEngine.app");
    ...
    expect(
      readdirSync(path.join(mirror, ".git")).filter((entry) =>
        entry.startsWith(".steelengine-live-mac-"),
      ),
    ).toEqual([]);
  });
  ...
  test("restores the Mac bundle when the Gateway build fails", () => {
    const { root, mirror } = makeFixture();
    mkdirSync(path.join(mirror, "node_modules"));
    const appMarker = path.join(mirror, "dist/SteelEngine.app/Contents/signature-marker");
    mkdirSync(path.dirname(appMarker), { recursive: true });
    writeFileSync(appMarker, "signed\n");
  ...
  test("accepts a delayed external restore of the exact preserved Mac bundle", () => {
    ...
    writeFileSync(appMarker, "signed\n");
    const commands = fakeCommands(mirror);
    const delayedBundle = path.join(root, "delayed-steelengine.app");
    let restored = false;
  ...
  test("repoints an ancestor snapshot across the next source update", () => {
    ...
    const configPath = path.join(root, "steelengine.json");
    const plistPath = path.join(root, "ai.steelengine.gateway.plist");
    writeFileSync(configPath, "{}\n");
    ...
    expect(commands.calls).toEqual([
      `/bin/launchctl bootout gui/${uid}/ai.steelengine.gateway`,
      "prove gateway stopped",
      "pnpm build",
      `/bin/launchctl enable gui/${uid}/ai.steelengine.gateway`,
    ...
    expect(() =>
      maintainMain(
        { checkout: mirror, remote: "origin", lockPath: path.join(root, "maintenance.lock") },
        {
          fetchMain: fetchFixtureMain,
          runCommand(command: string, args: string[]) {
            if (command === "pnpm" && args.slice(0, 3).join(" ") === "steelengine gateway status") {
              statusCalls += 1;
              throw new Error("RPC unavailable");
            }
          },
