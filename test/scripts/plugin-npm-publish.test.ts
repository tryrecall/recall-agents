...
function makePackage(version: string): { packageDir: string; path: string } {
  const root = mkdtempSync(join(tmpdir(), "steelengine-plugin-publish-test-"));
  tempDirs.push(root);
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: "@steelengine/demo", version }),
  );
...
describe("plugin npm publish wrapper", () => {
  ...
  it("requires an explicit artifact directory for real pack mode", () => {
    ...
    expect(result.status).toBe(2);
    expect(result.stdout).toBe("");
    expect(result.stderr.trim()).toBe("--pack requires STEELENGINE_PLUGIN_NPM_PACK_OUTPUT_DIR");
  });
