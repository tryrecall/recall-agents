...
describe("gateway restart benchmark script", () => {
  it("prints help without running benchmark cases", () => {
    ...
    expect(result.stdout).toContain("Recall Gateway restart benchmark");
...
`),
  ...
  it("enables both startup and restart trace in the child gateway environment", () => {
    const env = testing.sanitizedEnv("/tmp/recall-bench", "/tmp/recall-bench/config.json", {
      config: {},
      id: "skipChannels",
      name: "gateway restart, skip channels",
    });

    expect(env.RECALL_GATEWAY_STARTUP_TRACE).toBe("1");
    expect(env.RECALL_GATEWAY_RESTART_TRACE).toBe("1");
    expect(env.RECALL_NO_RESPAWN).toBe("1");
    expect(env.RECALL_LOCAL_CHECK).toBeUndefined();
  });

  it("can pin ACPX startup probe policy per benchmark case", () => {
    const probeOffEnv = testing.sanitizedEnv(
      "/tmp/recall-bench",
      "/tmp/recall-bench/config.json",
      {
        config: {},
        env: { RECALL_ACPX_RUNTIME_STARTUP_PROBE: "0" },
        id: "skipChannelsNoAcpxProbe",
        name: "gateway restart, skip channels, ACPX startup probe off",
      },
    );

    expect(probeOffEnv.RECALL_ACPX_RUNTIME_STARTUP_PROBE).toBe("0");
  });
  ...
  it("writes restart intent files for the target gateway pid", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "recall-restart-bench-test-"));
    try {
      const env = { RECALL_STATE_DIR: path.join(root, "state") };
  ...
  it("writes plugin fixtures as a parent load path with explicit startup activation", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "recall-restart-bench-config-test-"));
    try {
      ...
      const manifest = JSON.parse(
        fs.readFileSync(
          path.join(root, "plugins", "bench-plugin-01", "recall.plugin.json"),
          "utf8",
        ),
