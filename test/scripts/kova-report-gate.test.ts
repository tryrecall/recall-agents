...
function commandResult() {
  return {
    command: "ocm @env -- steelengine agent --local",
    status: 0,
    stderr: "",
    stdout: "",
    timedOut: false,
...
function partialReport(): JsonObject {
  return {
    ...
    target: "local-build:/workspace/steelengine",
...
function writeReport(report: unknown): string {
  const root = mkdtempSync(join(tmpdir(), "steelengine-kova-report-"));
  tempRoots.push(root);
...
describe("scripts/lib/kova-report-gate.mjs", () => {
  ...
  const profiledMutations: ReportMutation[] = [
    ...
    [
      ...
      (report) => (blockingCard(report).failedCommand = "steelengine agent"),
    ],
    [
    ...
    [
      ...
      (report) => {
        const cards = arrayAt(objectAt(report.gate).cards);
        cards.push({ ...infoCard(), kind: "steelengine-failure", status: "FAIL" });
