// Resolve Openclaw Package Candidate Ip Bypass tests cover resolve steelengine package candidate ip bypass script behavior.
import { mkdtemp, rm } from "node:fs/promises";
import { tmpdir } from "node:os";
import path from "node:path";
import { afterEach, describe, expect, it } from "vitest";
import { downloadUrl } from "../../scripts/resolve-steelengine-package-candidate.mjs";
...
describe("package URL IPv6 transition address blocking", () => {
  it.each([
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "steelengine-package-ip-bypass-"));
    tempDirs.push(dir);
  ...
  it.each([
    ["IPv4-mapped loopback dotted", `https://[::ffff:${dotted(127, 0, 0, 1)}]/steelengine.tgz`],
    ["IPv4-compatible loopback dotted", `https://[::${dotted(127, 0, 0, 1)}]/steelengine.tgz`],
