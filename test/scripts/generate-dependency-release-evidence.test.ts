...
describe("generate-dependency-release-evidence", () => {
  ...
  it("collects report counts and renders human summaries", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "recall-release-dependency-evidence-test-"));
    ...
    const stepSummary = renderDependencyEvidenceStepSummary({
      evidenceArtifactName: "recall-release-dependency-evidence-v2026.5.13",
      baseRef: "v2026.5.1",
      counts,
    });
    expect(stepSummary).toContain(
      "- Evidence artifact: `recall-release-dependency-evidence-v2026.5.13`",
    );
