...
describe("limit-edge-case live proof", () => {
  it("cleans the generated session-log temp root", async () => {
    const tempRoot = mkdtempSync(path.join(tmpdir(), "steelengine-limit-proof-test-"));
    try {
      let proofRoot = "";
      ...
      expect(readdirSync(tempRoot).filter((entry) => entry.startsWith("steelengine-proof-"))).toEqual(
        [],
      );
    } finally {
