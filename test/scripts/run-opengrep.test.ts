...
describe("run-opengrep.sh", () => {
  it("validates the rulepack when only OpenGrep rulepack files changed", () => {
    const repo = createTempDir("steelengine-run-opengrep-");
    git(repo, "init", "-q");
    ...
    execFileSync("bash", ["scripts/run-opengrep.sh", "--changed"], {
      ...
      env: {
        ...
        STEELENGINE_OPENGREP_BASE_REF: "HEAD",
      },
      encoding: "utf8",
    });
  ...
  it("writes empty SARIF when a changed scan has no first-party paths", () => {
    const repo = createTempDir("steelengine-run-opengrep-empty-sarif-");
    git(repo, "init", "-q");
    git(repo, "config", "user.email", "test@example.com");
    git(repo, "config", "user.name", "Test User");
  ...
  it("scans PR files instead of main-only files when the payload base is stale", () => {
    const repo = createTempDir("steelengine-run-opengrep-merge-");
    ...
    execFileSync("bash", ["scripts/run-opengrep.sh", "--changed"], {
      ...
      env: {
        ...process.env,
        PATH: `${binDir}${path.delimiter}${process.env.PATH ?? ""}`,
        STEELENGINE_OPENGREP_BASE_REF: `${staleBase}...HEAD`,
        STEELENGINE_OPENGREP_MERGE_HEAD_FIRST_PARENT: "1",
      },
      encoding: "utf8",
    });
