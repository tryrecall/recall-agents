...
describe("gateway CPU scenario guard", () => {
  ...
  it("prebuilds private QA dist before running QA scenarios when it is missing", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        ...
        if (args.includes("steelengine") && args.includes("qa")) {
    ...
    expect(calls[0]?.env).toMatchObject({
      ...
      STEELENGINE_CONFIG_PATH: path.join(outputDir, "qa-state-root", "state", "steelengine.json"),
      STEELENGINE_ENABLE_PRIVATE_QA_CLI: "1",
      STEELENGINE_HOME: path.join(outputDir, "qa-state-root", "home"),
      STEELENGINE_RUN_NODE_SKIP_DTS_BUILD: "1",
      STEELENGINE_STATE_DIR: path.join(outputDir, "qa-state-root", "state"),
      STEELENGINE_TEST_DISABLE_UPDATE_CHECK: "1",
      PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
      USERPROFILE: path.join(outputDir, "qa-state-root", "home"),
    });
    expect(calls[0]?.env?.STEELENGINE_BUNDLED_PLUGIN_BUILD_IDS).toBeUndefined();
  });
  ...
  it("does not prebuild private QA dist when the required entries already exist", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      env: {
        HOME: "/real/user/home",
        STEELENGINE_CONFIG_PATH: "/real/user/.steelengine/steelengine.json",
        STEELENGINE_HOME: "/real/user/home",
        STEELENGINE_STATE_DIR: "/real/user/.steelengine",
      },
      silent: true,
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        calls.push({ args, env: opts?.env });
