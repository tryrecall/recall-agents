...
describe("qa-e2e script", () => {
  it("enables private QA plugin SDK subpaths before loading QA Lab", () => {
    ...
    expect(env.STEELENGINE_BUILD_PRIVATE_QA).toBe("1");
    expect(env.STEELENGINE_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(env.STEELENGINE_DISABLE_BUNDLED_PLUGINS).toBe("0");
  });

  it("overrides inherited environment that would break the private QA self-check", () => {
    const env: NodeJS.ProcessEnv = {
      STEELENGINE_BUILD_PRIVATE_QA: "0",
      STEELENGINE_ENABLE_PRIVATE_QA_CLI: "0",
  ...
  it("prints help before enabling private QA or loading QA Lab", async () => {
    ...
    expect(env.STEELENGINE_BUILD_PRIVATE_QA).toBeUndefined();
  });
