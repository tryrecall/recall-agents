...
function makeEnvMutationFixture(): string {
  const repoRoot = createTempDir("steelengine-test-env-mutations-");
  writeRepoFile(
    repoRoot,
    "src/runtime.ts",
    `
process.env.STEELENGINE_STATE_DIR = "ignored because this is not a test file";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/example.test.ts",
    `
process.env.STEELENGINE_STATE_DIR = "state";
delete process.env["STEELENGINE_CONFIG_PATH"];
vi.stubEnv("HOME", "home");
vi.stubEnv("STEELENGINE_WORKSPACE_DIR", "workspace");
process.env = { ...process.env, STEELENGINE_HOME: "home", OTHER_KEY: "ignored" };
const dynamicEnvKey = "STEELENGINE_STATE_DIR";
process.env[dynamicEnvKey] = "dynamic";
delete process.env[dynamicEnvKey];
const fixture = 'process.env.STEELENGINE_HOME = "not code"';
process.env.NOT_STEELENGINE = "ignored";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.STEELENGINE_CONFIG_PATH = "script-harness";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.STEELENGINE_AGENT_DIR = "agent";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/test-utils/steelengine-test-state.ts",
    `
...
describe("collectTestEnvMutationReport", () => {
  it("reports active SteelEngine env mutations while ignoring strings and non-test files", () => {
    ...
    expect(
      ...
      },
      {
        ...
        key: "STEELENGINE_AGENT_DIR",
        operation: "assign",
      },
      { file: "src/example.test.ts", key: "STEELENGINE_STATE_DIR", operation: "assign" },
      { file: "src/example.test.ts", key: "STEELENGINE_CONFIG_PATH", operation: "delete" },
      { file: "src/example.test.ts", key: "HOME", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "STEELENGINE_WORKSPACE_DIR", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "STEELENGINE_HOME", operation: "replace" },
  ...
  it("renders a non-blocking text report for active findings", () => {
    ...
    expect(rendered).toContain("SteelEngine test env mutation report");
    ...
    expect(rendered).toContain("L2 STEELENGINE_STATE_DIR assign process.env");
    ...
    for (const limit of ["1e3", ""]) {
      expect(() =>
        runTestEnvMutationReport([
          ...
          limit,
          "--repo-root",
          createTempDir("steelengine-env-limit-"),
        ]),
