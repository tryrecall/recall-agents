...
const PACKAGE_NAME = "@steelengine/meta-provider";
...
const TARBALL_NAME = "steelengine-meta-provider-2026.7.1-beta.3.tgz";
const MANUAL_OVERRIDE_REASON =
  "SteelEngine Release Publish run 12345 approved token release for v2026.7.1-beta.3";
...
const PUBLISHER_POLICY = {
  ...
  schema: "steelengine.plugin-npm-publisher-policy/v1",
...
function tempDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "steelengine-plugin-publication-artifact-"));
  tempDirs.push(dir);
  return dir;
}
...
function createFixture(
  ...
  const tarball = createTarball(
    options.tarEntries ?? [
      ...
      { content: '{"id":"meta"}\n', path: "package/steelengine.plugin.json" },
...
describe("plugin publication artifact", () => {
    ...
    expect(() =>
      verifyFixture(fixture, {
        manualOverrideReason:
          "SteelEngine Release Publish run 12345 approved token release for v2026.7.1-beta.4",
      }),
  ...
  it("rejects every mutable artifact identity dimension", () => {
    ...
    const mismatches: Array<[string, Record<string, unknown>]> = [
      ...
      ["repository", { repository: "steelengine/not-steelengine" }],
      ...
      ["package name", { packageName: "@steelengine/not-meta" }],
  ...
  it("binds authoritative workflow status, conclusion, and head repository", () => {
    const mutations = [
      ...
      (run: Record<string, unknown>) => {
        run.head_repository = { full_name: "steelengine/not-steelengine" };
      },
    ];
  ...
  it("requires the exact Meta package name, dir, and both publication flags", () => {
    ...
    const cases: Array<{
      ...
      {
        manifestOverrides: {
          steelengine: { release: { publishToClawHub: false, publishToNpm: true } },
        },
      },
      {
        manifestOverrides: {
          steelengine: { release: { publishToClawHub: true, publishToNpm: false } },
        },
      },
      {
        manifestOverrides: { name: "@steelengine/not-meta" },
        params: { packageName: "@steelengine/not-meta" },
      },
      {
