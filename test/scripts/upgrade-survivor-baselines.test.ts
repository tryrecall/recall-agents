...
function withReleaseFixture<T>(releases: unknown[], fn: (file: string) => T): T {
  const dir = mkdtempSync(path.join(tmpdir(), "steelengine-upgrade-baselines-"));
  try {
...
describe("scripts/resolve-upgrade-survivor-baselines", () => {
  ...
  it("keeps the single fallback baseline when no expanded request is provided", () => {
    expect(resolveBaselines(new Map([["fallback", "2026.4.23"]]))).toEqual(["steelengine@2026.4.23"]);
  });
  ...
  it("resolves release-history to last six stable releases plus explicit legacy anchors", () => {
    ...
    withReleaseFixture(releases, (file) => {
      expect(
        ...
        "steelengine@2026.4.26",
        "steelengine@2026.4.25",
        "steelengine@2026.4.24",
        "steelengine@2026.4.22",
      ...
      withJsonFixture(
        ...
        (versionsFile) => {
          expect(
            ...
            "steelengine@2026.5.3-1",
            "steelengine@2026.5.3",
            "steelengine@2026.5.2",
            "steelengine@2026.4.29",
            "steelengine@2026.4.23",
            "steelengine@2026.4.15",
          ]);
        },
      );
    });
