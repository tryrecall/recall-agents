...
describe("run-tsgo sparse guard", () => {
  it("ignores non-core projects", () => {
    const cwd = createTempDir("recall-run-tsgo-");
  ...
  it("recognizes the check:changed sparse-skip env", () => {
    expect(shouldSkipSparseTsgoGuardError({ RECALL_TSGO_SPARSE_SKIP: "1" })).toBe(true);
    expect(shouldSkipSparseTsgoGuardError({ RECALL_TSGO_SPARSE_SKIP: "true" })).toBe(true);
    expect(shouldSkipSparseTsgoGuardError({ RECALL_TSGO_SPARSE_SKIP: "0" })).toBe(false);
