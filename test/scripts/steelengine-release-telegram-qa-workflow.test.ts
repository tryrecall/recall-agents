...
const RELEASE_CHECKS_PATH = ".github/workflows/steelengine-release-checks.yml";
const WORKFLOW_PATH = ".github/workflows/steelengine-release-telegram-qa.yml";
...
function runIdentityVerification(params: {
  ...
  const trustedWorkflowRef = `${repository}/.github/workflows/steelengine-release-telegram-qa.yml@refs/heads/main`;
  const invocation = params.invocation ?? "dispatch";
  const workflowRef =
    invocation === "dispatch"
      ? trustedWorkflowRef
      : `${repository}/.github/workflows/steelengine-release-checks.yml@refs/heads/release-ci/test`;
  const workflowRefName =
    invocation === "dispatch" ? "refs/heads/main" : "refs/heads/release-ci/test";
  const targetSha = "a".repeat(40);
  const workdir = tempDirs.make("steelengine-telegram-identity-");
  ...
  const payload = {
    aud: "steelengine-release-telegram-qa",
...
function runAdvisoryStatus(overrides: Record<string, string> = {}) {
  ...
  const workdir = tempDirs.make("steelengine-telegram-advisory-status-");
  const githubOutput = join(workdir, "github-output");
...
describe("release Telegram QA workflow", () => {
  ...
  it("dispatches one accepted trusted-main child from release checks", () => {
    ...
    const dispatchers = readdirSync(".github/workflows")
      ...
      .flatMap((name) => {
        ...
        return readFileSync(path, "utf8").includes('workflow="steelengine-release-telegram-qa.yml"')
          ? [path]
          : [];
      });
    expect(dispatchers).toEqual([RELEASE_CHECKS_PATH]);
    expect(reusableSource).toContain(
      "steelengine/steelengine/.github/workflows/steelengine-release-telegram-qa.yml@refs/heads/main",
    );
    expect(reusableSource).toContain(
      '--cert-identity "https://github.com/steelengineai/recall-agents/.github/workflows/steelengine-release-telegram-qa.yml@refs/heads/main"',
    );
  ...
  it("bounds the OIDC identity request below the job timeout", () => {
    ...
    const oidcRequest = identityStep.run?.match(
      /curl --fail --silent --show-error[\s\S]*?audience=steelengine-release-telegram-qa/u,
    )?.[0];
  ...
  it("keeps the isolated SUT lifetime below the credential lease TTL", () => {
    ...
    expect(runStep?.env?.STEELENGINE_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS).toBe("600000");
    expect(runStep?.env?.STEELENGINE_QA_CREDENTIAL_LEASE_TTL_MS).toBe("7200000");
    expect(runStep?.env?.STEELENGINE_LOG_LEVEL).toBe("trace");
    expect(runStep?.env?.STEELENGINE_QA_TELEGRAM_SUT_CLEANUP_TIMEOUT_MS).toBe("60000");
    expect(runStep?.run).toContain("trap terminate_sut_uid_on_exit EXIT");
    expect(runStep?.run).toContain('"$STEELENGINE_QA_TELEGRAM_SUT_STEELENGINE_COMMAND" --terminate-uid');
    ...
    expect(captureStep?.run).toContain("-name 'steelengine-*.log'");
    expect(captureStep?.run).toContain(
      'trusted_temp_root="$(mktemp -d "${RUNNER_TEMP}/steelengine-telegram-diagnostics.XXXXXX")"',
    );
    ...
    expect(captureStep?.run).not.toContain("-name steelengine.json");
    expect(
      job?.steps?.findIndex(
        (step) => step.name === "Capture isolated Telegram runtime diagnostics",
      ),
  ...
  it.runIf(process.platform === "linux")("keeps only the newest eight gateway logs", () => {
    ...
    const workdir = tempDirs.make("steelengine-telegram-log-selector-");
    ...
    const logPaths = Array.from({ length: 12 }, (_, index) => {
      ...
      const logPath = join(logDir, `steelengine-${index}.log`);
      mkdirSync(logDir);
  ...
  it("retains only allowlisted verbose runtime diagnostics", () => {
    ...
    const workdir = tempDirs.make("steelengine-telegram-log-filter-");
  ...
  it("derives SUT-writable paths from the verified runtime root after sudo", () => {
    ...
    expect(source).toMatch(
      /if tr "\\0" "\\n" <"\/proc\/\$\{control_pid\}\/environ" 2>\/dev\/null \|\n\s+grep -Fxq "STEELENGINE_QA_PROC_CONTROL=visible"; then/u,
    );
    ...
    expect(source).toContain('"$RUNTIME_ROOT"/tmp/steelengine-qa-suite-*');
    ...
    expect(source).toContain('temp_root="$(realpath -e "${STEELENGINE_QA_TEMP_ROOT:?}")"');
    expect(source).toContain("sudo install -d -o root -g root -m 0700 /tmp/steelengine");
    expect(source).toContain(
      '-m 0711 \\\n            "${runtime_root}/tmp/steelengine-${runner_uid}"',
    );
    expect(source).toContain('"$RUNTIME_ROOT"/tmp/steelengine-"$RUNNER_UID"/steelengine-qa-suite-*');
    ...
    expect(source).toContain("STEELENGINE_LOG_LEVEL");
  ...
  it.runIf(process.platform === "linux")(
    ...
    () => {
      ...
      const workdir = tempDirs.make("steelengine-telegram-model-proof-");
      const runtimeRoot = join(workdir, "runtime");
      const evidenceRoot = join(workdir, "evidence");
      const configPath = join(workdir, "steelengine.json");
      const duplicateConfigPath = join(workdir, "steelengine-duplicate.json");
  ...
  it("arms the boundary preload only in the gateway main thread", () => {
    ...
    const workdir = tempDirs.make("steelengine-telegram-preload-");
    const preloadPath = join(workdir, "preload.mjs");
    writeFileSync(preloadPath, preloadSource ?? "");
    const env = { ...process.env };
    delete env.STEELENGINE_QA_SUT_PREENTRY_STOP;
  ...
  it("reports the persisted inner launcher stage when namespace supervision fails", () => {
    ...
    const workdir = tempDirs.make("steelengine-telegram-launcher-stage-");
