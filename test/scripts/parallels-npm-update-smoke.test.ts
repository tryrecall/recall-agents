...
function makeTempDir(): string {
  const root = mkdtempSync(path.join(tmpdir(), "steelengine-parallels-npm-update-"));
  tempDirs.push(root);
  return root;
}
...
function extractWindowsBackgroundControlMarkers(decoded: string): {
  ...
  return {
    done: marker("__STEELENGINE_BACKGROUND_DONE__", false),
    exitPrefix: marker("__STEELENGINE_BACKGROUND_EXIT__", true),
  };
}
...
describe("parallels npm update smoke", () => {
  it("accepts one prepared tarball target for update and fresh install", () => {
    expect(
      parseArgs([
        "--target-tarball",
        "/tmp/steelengine-candidate.tgz",
        "--dependency-tarball",
        "/tmp/steelengine-ai-candidate.tgz",
      ]),
    ).toMatchObject({
      dependencyTarballs: ["/tmp/steelengine-ai-candidate.tgz"],
      targetTarball: "/tmp/steelengine-candidate.tgz",
      updateTarget: "",
      freshTargetSpec: undefined,
    });
    expect(() =>
      parseArgs(["--target-tarball", "/tmp/steelengine-candidate.tgz", "--update-target", "beta"]),
    ).toThrow("--target-tarball cannot be combined");
    expect(() => parseArgs(["--dependency-tarball", "/tmp/steelengine-ai-candidate.tgz"])).toThrow(
  ...
  it("stops the host artifact server when the wrapper fails mid-run", async () => {
    ...
    await withEnvAsync({ OPENAI_API_KEY: "test-key" }, async () => {
      const smoke = new FailingNpmUpdateSmoke({
        ...
        dependencyTarballs: [],
        json: false,
        packageSpec: "steelengine@latest",
...
printf '%s\\n' "$*" >>"$log_path"
args=" $* "
if [[ "$args" == *" /usr/bin/tee /tmp/steelengine-parallels-npm-update-linux-"* ]]; then
  cat >/dev/null
  exit 0
fi
if [[ "$args" == *" /bin/chmod 755 /tmp/steelengine-parallels-npm-update-linux-"* ]]; then
  echo "chmod denied" >&2
  exit 7
fi
if [[ "$args" == *" /bin/rm -f /tmp/steelengine-parallels-npm-update-linux-"* ]]; then
  printf 'cleanup\\n' >>"$log_path"
  exit 0
fi
exit 1
`,
    ...
    expect(log).toContain("/bin/chmod 755 /tmp/steelengine-parallels-npm-update-linux-");
    expect(log).toContain("/bin/rm -f /tmp/steelengine-parallels-npm-update-linux-");
  ...
  it("has a one-command beta validation mode with fresh target coverage", () => {
    ...
    expect(script).toContain("resolveSteelEngineRegistryVersion");
    expect(script).toContain("this.options.updateTarget = version");
    expect(script).toContain("this.options.freshTargetSpec = `steelengine@${version}`");
  ...
  it("accepts keyed and nested npm metadata for published update targets", () => {
    ...
    expect(
      parseRegistryPackageMetadata(
        JSON.stringify({
          ...
          "dist.tarball": "https://registry.example/steelengine-keyed.tgz",
          gitHead: "abcdef0123456789",
        }),
      ),
    ).toEqual({
      version: "2026.5.20-beta.1",
      tarball: "https://registry.example/steelengine-keyed.tgz",
      gitHead: "abcdef0123456789",
    });
    ...
    expect(
      parseRegistryPackageMetadata(
        JSON.stringify({
          version: "2026.5.20-beta.1",
          dist: { tarball: "https://registry.example/steelengine-nested.tgz" },
        }),
      ),
    ).toEqual({
      version: "2026.5.20-beta.1",
      tarball: "https://registry.example/steelengine-nested.tgz",
      gitHead: "",
    });
  });
  ...
  it("guards beta validation against cross-version harness checkouts", () => {
    ...
    expect(script).toContain("steelEngineVersionFamily");
    expect(script).toContain("STEELENGINE_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH");
  ...
  it("bounds POSIX guest failure logs", () => {
    ...
    expect(scripts).toContain("STEELENGINE_PARALLELS_NPM_UPDATE_LOG_TAIL_BYTES");
    expect(scripts).toContain('print_log_tail "$output_file"');
    expect(scripts).toContain("print_log_tail /tmp/steelengine-parallels-macos-gateway.log >&2");
    expect(scripts).toContain("print_log_tail /tmp/steelengine-parallels-linux-gateway.log >&2");
    expect(scripts).not.toContain('cat "$output_file"');
    expect(scripts).not.toContain("cat /tmp/steelengine-parallels-");
  });
  ...
  it("passes platform model timeouts to POSIX update agent turns", () => {
    ...
    withEnv(
      {
        STEELENGINE_PARALLELS_LINUX_MODEL_TIMEOUT_S: undefined,
        STEELENGINE_PARALLELS_MACOS_MODEL_TIMEOUT_S: undefined,
        STEELENGINE_PARALLELS_MODEL_TIMEOUT_S: undefined,
      },
      () => {
    ...
    withEnv(
      {
        STEELENGINE_PARALLELS_LINUX_MODEL_TIMEOUT_S: "321",
        STEELENGINE_PARALLELS_MACOS_MODEL_TIMEOUT_S: "654",
      },
      () => {
  ...
  it("sets platform-aware fresh lane timeouts", () => {
    withEnv({ STEELENGINE_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: undefined }, () => {
    ...
    withEnv({ STEELENGINE_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: "3" }, () => {
      expect(freshLaneTimeoutMs("macos")).toBe(3000);
    });

    withEnv(
      { STEELENGINE_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: String(Number.MAX_SAFE_INTEGER) },
      () => {
  ...
  it("clears update stream timers when spawning the guest command fails", async () => {
    ...
    await expect(
      runStreamingToJobLog.call(smoke, "steelengine-definitely-missing-command", [], 60 * 60 * 1000, {
        append: () => undefined,
        logPath: "",
  ...
  it("cleans timed-out Windows background work", async () => {
    ...
    expect(commands).toContain("function Write-SteelEngineUtf8File");
    expect(commands).toContain("[System.Text.UTF8Encoding]::new($false)");
    expect(payloads).toContain("Write-SteelEngineUtf8File $exitPath '0'");
    expect(payloads).toContain("Write-SteelEngineUtf8File $donePath 'done'");
    expect(payloads).toContain("Write-SteelEngineUtf8File $pidPath ([string]$PID)");
    expect(commands).toContain('cmd.exe /d /s /c start "" /b powershell.exe');
    expect(commands).toContain("icacls.exe $runDir /inheritance:r");
    expect(commands).toContain("Stop-SteelEngineBackgroundProcessTree ([int]$backgroundPid)");
  ...
  it("does not treat Windows background log text as completion control", async () => {
    ...
    expect(decodedCommands.join("\n")).toContain(
      "Stop-SteelEngineBackgroundProcessTree ([int]$backgroundPid)",
    );
  });
  ...
  it("drains completed Windows background logs before cleanup", async () => {
    ...
    expect(decodedCommands.join("\n")).not.toContain("Stop-SteelEngineBackgroundProcessTree");
    expect(decodedCommands.join("\n")).toContain(
      "Remove-Item -Path $scriptPath, $logPath, $donePath, $exitPath, $pidPath",
    );
  });
...
`,
  ...
  it("scrubs future plugin entries before invoking old same-guest updaters", () => {
    ...
    expect(script).toContain("Invoke-WithScopedEnv @{ STEELENGINE_DISABLE_BUNDLED_PLUGINS = '1'");
    expect(macosScript).toContain('STEELENGINE_BIN="$(resolve_required_command steelengine)"');
    expect(macosScript).toContain("/usr/local/bin:/usr/local/sbin");
    expect(macosScript).toContain(
      'STEELENGINE_DISABLE_BUNDLED_PLUGINS=1 "$STEELENGINE_BIN" update --tag',
    );
    expect(macosScript).not.toContain("/opt/homebrew/bin/steelengine");
    expect(script).toContain("STEELENGINE_DISABLE_BUNDLED_PLUGINS=1 steelengine update --tag");
    expect(macosScript).toContain(
      'STEELENGINE_DISABLE_BUNDLED_PLUGINS=1 "$STEELENGINE_BIN" gateway stop',
    );
    expect(script).toContain(
      "STEELENGINE_DISABLE_BUNDLED_PLUGINS=1 STEELENGINE_ALLOW_ROOT=1 steelengine gateway stop",
    );
  });
  ...
  it("reenables bundled plugins before Windows post-update verification", () => {
    ...
    const updateIndex = script.indexOf("Invoke-SteelEngine update --tag");
    const scopedIndex = script.indexOf("Invoke-WithScopedEnv @{ STEELENGINE_DISABLE_BUNDLED_PLUGINS");
    const versionIndex = script.indexOf("Invoke-SteelEngine --version", scopedIndex);
    const restartIndex = script.indexOf("Invoke-SteelEngine gateway restart");
    const agentIndex = script.indexOf("Invoke-SteelEngine agent --local");
    ...
    expect(script).not.toContain("$env:STEELENGINE_DISABLE_BUNDLED_PLUGINS = '1'");
  });
  ...
  it("generates a .NET-safe Windows stale import regex in the update-failure guard", () => {
    ...
    expect(staleImportPattern).toBe(
      String.raw`node_modules\\steelengine\\dist\\[^\\]+-[A-Za-z0-9_-]+\.js`,
    );
    expect(staleImportPattern).not.toContain("node_modules\\steelengine\\dist\\");
    expect(staleImportPattern.match(/\\\\/g)).toHaveLength(4);
    const representativeUpdateFailure = String.raw`Error [ERR_MODULE_NOT_FOUND]: Cannot find module 'C:\Users\runner\AppData\Roaming\npm\node_modules\steelengine\dist\main-a1_B2.js' imported from C:\Users\runner\AppData\Roaming\npm\node_modules\steelengine\dist\cli.js`;
    const generatedRegex = new RegExp(staleImportPattern);
    expect(generatedRegex.test(representativeUpdateFailure)).toBe(true);
    expect(generatedRegex.test(String.raw`node_modules\steelengine\dist\main.js`)).toBe(false);
  });
});
