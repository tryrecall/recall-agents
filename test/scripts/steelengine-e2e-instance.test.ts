// Openclaw E2E Instance tests cover steelengine e2e instance script behavior.
...
const helperPath = path.resolve("scripts/lib/steelengine-e2e-instance.sh");
const hostPath = [
  ...
  return spawnSync(
    ...
    [
      ...
      [
        ...
        `steelengine_e2e_eval_test_state_from_b64 ${shellQuote(payload)}`,
        'printf "value=%s" "${STEELENGINE_E2E_INSTANCE_TEST:-unset}"',
      ].join("; "),
    ],
    { encoding: "utf8" },
  );
...
function writePackageFixture(packagePath: string): void {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-package-"));
  try {
    const packageDir = path.join(root, "package");
    fs.mkdirSync(packageDir);
    fs.writeFileSync(
      path.join(packageDir, "package.json"),
      JSON.stringify({ name: "steelengine-e2e-fixture", version: "0.0.0" }),
    );
...
function expectNpmInstallObserved(argsPath: string, expectedArgs: string, prefix: string): void {
  ...
  expect(
    fs.existsSync(path.join(prefix, "lib/node_modules/steelengine-e2e-fixture/package.json")),
  ).toBe(true);
}

describe("scripts/lib/steelengine-e2e-instance.sh", () => {
  it("sources decoded test-state scripts", () => {
    const result = runHelper(base64('export STEELENGINE_E2E_INSTANCE_TEST="ok"\n'));
  ...
  it("fails when the test-state payload is not valid base64", () => {
    ...
    expect(result.stderr).toContain("Invalid SteelEngine test-state base64 payload");
  });
  ...
  it("reads positive integer env values without treating decimal input as durations", () => {
    const fallback = runSourcedHelper(
      'printf "%s" "$(steelengine_e2e_read_positive_int_env STEELENGINE_E2E_SAMPLE_SECONDS 180)"',
    );
    const leadingZero = runSourcedHelper(
      'printf "%s" "$(steelengine_e2e_read_positive_int_env STEELENGINE_E2E_SAMPLE_SECONDS 180)"',
      { STEELENGINE_E2E_SAMPLE_SECONDS: "008" },
    );
    const duration = runSourcedHelper(
      "steelengine_e2e_read_positive_int_env STEELENGINE_E2E_SAMPLE_SECONDS 180",
      { STEELENGINE_E2E_SAMPLE_SECONDS: "30s" },
    );
    ...
    expect(duration.stderr).toContain("invalid STEELENGINE_E2E_SAMPLE_SECONDS: 30s");
  });

  it("reads non-negative integer env values without accepting shell-style sizes", () => {
    const fallback = runSourcedHelper(
      'printf "%s" "$(steelengine_e2e_read_nonnegative_int_env STEELENGINE_E2E_SAMPLE_BYTES 262144)"',
    );
    const zero = runSourcedHelper(
      'printf "%s" "$(steelengine_e2e_read_nonnegative_int_env STEELENGINE_E2E_SAMPLE_BYTES 262144)"',
      { STEELENGINE_E2E_SAMPLE_BYTES: "0" },
    );
    const size = runSourcedHelper(
      "steelengine_e2e_read_nonnegative_int_env STEELENGINE_E2E_SAMPLE_BYTES 262144",
      { STEELENGINE_E2E_SAMPLE_BYTES: "64kb" },
    );
    ...
    expect(size.stderr).toContain("invalid STEELENGINE_E2E_SAMPLE_BYTES: 64kb");
  });

  it("probes default and explicit mock OpenAI base URLs", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-mock-openai-url-"));
    try {
      const probePath = path.join(tempDir, "probe-url.txt");
      const result = runSourcedHelper(
        [
          `steelengine_e2e_probe_http() { printf "%s\\n" "$1" >>${shellQuote(probePath)}; return 0; }`,
          "steelengine_e2e_wait_mock_openai 44080 1 400",
          "steelengine_e2e_wait_mock_openai 443 1 400 https://api.openai.com:443",
        ].join("; "),
      );
  ...
  it("requires /readyz after the gateway ready log", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-readyz-required-"));
    try {
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            "steelengine_e2e_probe_http() { return 1; }",
            "sleep 30 &",
            'gateway_pid="$!"',
            "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
            `printf '[gateway] ready ws://127.0.0.1:23456\\n' >${shellQuote(logPath)}`,
            `if steelengine_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2; then`,
  ...
  it("probes /readyz on the explicit gateway port", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-readyz-port-"));
    try {
      const logPath = path.join(tempDir, "gateway.log");
      const probePath = path.join(tempDir, "probe-url.txt");
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `steelengine_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2 23456`,
          ].join("\n"),
        ],
        {
          encoding: "utf8",
  ...
  it("allows explicit legacy ready-log mode without /readyz", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-readyz-legacy-"));
    try {
      const logPath = path.join(tempDir, "gateway.log");
      const result = spawnSync(
        "/bin/bash",
        [
          ...
          [
            ...
            "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
            `printf '[gateway] ready\\n' >${shellQuote(logPath)}`,
            `steelengine_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2 23456 legacy-ready-log-ok`,
          ].join("\n"),
        ],
        {
          encoding: "utf8",
  ...
  it("wraps package installs with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-instance-"));
    try {
      ...
      const packagePath = path.join(tempDir, "steelengine.tgz");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `steelengine_e2e_install_package ${shellQuote(logPath)} ${shellQuote("fixture package")} ${shellQuote(prefixPath)}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
          env: shellTestEnv({
            ...
            STEELENGINE_CURRENT_PACKAGE_TGZ: packagePath,
            STEELENGINE_E2E_NPM_INSTALL_TIMEOUT: "42s",
            STEELENGINE_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            STEELENGINE_TEST_NPM_ARGS: npmArgsPath,
            STEELENGINE_TEST_NPM_BIN: path.join(tempDir, "npm"),
          }),
        },
      );
  ...
  it("falls back to plain timeout when kill-after is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-instance-plain-timeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
  ...
  it("uses gtimeout when GNU timeout is not on PATH", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-instance-gtimeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
  ...
  it("uses the Node watchdog when timeout is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-instance-no-timeout-"));
    try {
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
      const packagePath = path.join(tempDir, "steelengine.tgz");
  ...
  it("bounds npm install failure logs to the configured tail", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-instance-install-log-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const logPath = path.join(tempDir, "install.log");
      const packagePath = path.join(tempDir, "steelengine.tgz");
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: shellTestEnv({
            PATH: `${tempDir}${path.delimiter}${hostPath}`,
            STEELENGINE_CURRENT_PACKAGE_TGZ: packagePath,
            STEELENGINE_E2E_LOG_TAIL_BYTES: "80",
            STEELENGINE_E2E_NPM_INSTALL_TIMEOUT: "42s",
            STEELENGINE_TEST_TIMEOUT_ARGS: timeoutArgsPath,
          }),
        },
  ...
  it.each([
    ["bytes", "STEELENGINE_E2E_LOG_TAIL_BYTES", "64kb"],
    ["lines", "STEELENGINE_E2E_LOG_TAIL_LINES", "25 lines"],
  ])("rejects invalid E2E log tail %s before invoking tail", (_label, envName, value) => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-instance-log-tail-"));
    try {
      const logPath = path.join(tempDir, "install.log");
      fs.writeFileSync(logPath, "old log\nrecent log\n", "utf8");

      const result = runSourcedHelper(`steelengine_e2e_print_log ${shellQuote(logPath)}`, {
        [envName]: value,
      });
  ...
  it("bounds commands with the Node watchdog when timeout is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-instance-node-watchdog-"));
    try {
      writeNodeShim(tempDir);
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `steelengine_e2e_maybe_timeout 200ms ${shellQuote(process.execPath)} -e ${shellQuote("setInterval(() => {}, 1000)")}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
      ...
      expect(result.stderr).toContain("SteelEngine E2E command timed out after 200ms");
    } finally {
      fs.rmSync(tempDir, { force: true, recursive: true });
    }
  });
  ...
  for (const [shellSignal, expectedStatus] of [
    ...
    it(`escalates Node watchdog children that ignore parent SIG${shellSignal}`, () => {
      const tempDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "steelengine-e2e-instance-node-watchdog-signal-"),
      );
      try {
        writeNodeShim(tempDir);
...
set -euo pipefail
source ${shellQuote(helperPath)}
export STEELENGINE_E2E_TIMEOUT_KILL_GRACE_MS=100
steelengine_e2e_maybe_timeout 30s node ${shellQuote(childPath)} ${shellQuote(pidPath)} ${shellQuote(watchdogPidPath)} &
wrapper_pid="$!"
...
`;
  ...
  it("terminates only the tracked gateway process", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-gateway-terminate-"));
    try {
      ...
      writeBashExecutable(path.join(tempDir, "pkill"), [
        'printf "pkill %s\\n" "$*" >>"$STEELENGINE_TEST_FORBIDDEN_PROCESS_TOOL_LOG"',
        "exit 42",
      ]);
...
set -euo pipefail
source ${shellQuote(helperPath)}
steelengine_e2e_terminate_gateways ""
/bin/sleep 30 &
tracked_pid="$!"
steelengine_e2e_terminate_gateways "$tracked_pid"
if kill -0 "$tracked_pid" 2>/dev/null; then
  echo "tracked gateway process still alive" >&2
  exit 1
fi
[ ! -s "$STEELENGINE_TEST_FORBIDDEN_PROCESS_TOOL_LOG" ]
`;
      ...
      const result = spawnSync("/bin/bash", ["-c", script], {
        ...
        env: shellTestEnv({
          ...
          STEELENGINE_TEST_FORBIDDEN_PROCESS_TOOL_LOG: forbiddenToolLog,
        }),
        timeout: 5_000,
      });
  ...
  it("terminates descendants in the tracked process group", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-process-group-"));
...
set -euo pipefail
source ${shellQuote(helperPath)}
tracked_pid="$(steelengine_e2e_start_tracked_process ${shellQuote(logPath)} ${shellQuote(process.execPath)} ${shellQuote(parentPath)} ${shellQuote(childPath)} ${shellQuote(parentPidPath)} ${shellQuote(childPidPath)} ${shellQuote(childTermPath)})"
for ((i = 0; i < 100; i += 1)); do
...
steelengine_e2e_stop_process "$tracked_pid"
for ((i = 0; i < 100; i += 1)); do
...
`;
  ...
  it("bounds HTTP readiness probes when a server accepts connections but never responds", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-http-probe-"));
    try {
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `source ${shellQuote(helperPath)}`,
            'steelengine_e2e_probe_http_status "http://127.0.0.1:${port}/health" 200 100',
          ].join("; "),
        ],
  ...
  it("does not repeatedly grep the full gateway log while waiting for readiness", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-readyz-incremental-"));
    try {
      const logPath = path.join(tempDir, "gateway.log");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            "steelengine_e2e_probe_http() { return 0; }",
            "grep() {",
            ...
            `printf 'old log line\\n' >${shellQuote(logPath)}`,
            `printf '[gateway] ready ws://127.0.0.1:23456\\n' >>${shellQuote(logPath)}`,
            `steelengine_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2`,
          ].join("\n"),
        ],
        {
          encoding: "utf8",
  ...
  it("detects gateway ready markers split across incremental log reads", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-readyz-split-marker-"));
    try {
      const logPath = path.join(tempDir, "gateway.log");
      const result = spawnSync(
        "/bin/bash",
        [
          ...
          [
            ...
            `steelengine_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 8`,
          ].join("\n"),
        ],
        {
          encoding: "utf8",
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `steelengine_e2e_probe_http() { printf '%s' "$1" >${shellQuote(probePath)}; [[ "$1" = "http://127.0.0.1:23456/readyz" ]]; }`,
            "sleep 30 &",
            'gateway_pid="$!"',
            "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
  ...
  it("wraps logged SteelEngine E2E commands with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-instance-run-logged-"));
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          '[ "$#" -gt 0 ] || exit 127',
          "shift",
          'exec "$STEELENGINE_TEST_COMMAND_BIN" "$@"',
          "",
        ].join("\n"),
      );
      fs.writeFileSync(
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `steelengine_e2e_run_logged ${shellQuote(logLabel)} fixture-command one two`,
            `printf "%s" "$STEELENGINE_E2E_LAST_LOG_PATH" > ${shellQuote(logPathFile)}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
          env: shellTestEnv({
            PATH: `${tempDir}:${hostPath}`,
            STEELENGINE_E2E_LOG_DIR: logDir,
            STEELENGINE_E2E_COMMAND_TIMEOUT: "17s",
            STEELENGINE_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            STEELENGINE_TEST_COMMAND_ARGS: commandArgsPath,
            STEELENGINE_TEST_COMMAND_BIN: path.join(tempDir, "fixture-command"),
          }),
        },
      );
      ...
      expect(path.basename(logPath)).toMatch(new RegExp(`^steelengine-${logLabel}\\..+\\.log$`, "u"));
  ...
  it("bounds logged command failure output to the configured tail", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-instance-run-log-tail-"));
    const logLabel = path.basename(tempDir);
    const logDir = path.join(tempDir, "logs");
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `steelengine_e2e_run_logged ${shellQuote(logLabel)} fixture-command`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
      ...
      const logFile = expectDefined(fs.readdirSync(logDir)[0], "SteelEngine E2E command log file");
  ...
  it("installs the trash shim under isolated test state", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-trash-shim-"));
    try {
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `printf "%s" "$STEELENGINE_E2E_BIN_DIR" > ${shellQuote(binDirFile)}`,
      ...
      expect(binDir).not.toBe("/tmp/steelengine-bin");
  ...
  it("wraps package-installed SteelEngine CLI calls with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-instance-steelengine-cli-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const commandArgsPath = path.join(tempDir, "steelengine-args.txt");
      fs.writeFileSync(
        path.join(tempDir, "timeout"),
        [
          "#!/usr/bin/env bash",
          ...
          'if [ "${1:-}" = "--kill-after=1s" ]; then exit 0; fi',
          'printf "%s\\n" "$*" >"$STEELENGINE_TEST_TIMEOUT_ARGS"',
          `while [ "$#" -gt 0 ] && [ "$1" != ${shellQuote(path.join(tempDir, "steelengine"))} ]; do shift; done`,
          '[ "$#" -gt 0 ] || exit 127',
          "shift",
          'exec "$STEELENGINE_TEST_STEELENGINE_BIN" "$@"',
          "",
        ].join("\n"),
      );
      fs.writeFileSync(
        path.join(tempDir, "steelengine"),
        [
          "#!/usr/bin/env bash",
          "set -euo pipefail",
          'printf "%s\\n" "$*" >"$STEELENGINE_TEST_COMMAND_ARGS"',
      ...
      fs.chmodSync(path.join(tempDir, "timeout"), 0o755);
      fs.chmodSync(path.join(tempDir, "steelengine"), 0o755);
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: shellTestEnv({
            PATH: `${tempDir}:${hostPath}`,
            STEELENGINE_E2E_COMMAND_TIMEOUT: "23s",
            STEELENGINE_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            STEELENGINE_TEST_COMMAND_ARGS: commandArgsPath,
            STEELENGINE_TEST_STEELENGINE_BIN: path.join(tempDir, "steelengine"),
          }),
        },
      );
      ...
      expect(result.status).toBe(0);
      expect(fs.readFileSync(timeoutArgsPath, "utf8").trim()).toBe(
        `--kill-after=30s 23s ${path.join(tempDir, "steelengine")} plugins list --json`,
      );
  ...
  it("wraps interactive PTY scripts with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-e2e-instance-pty-timeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      ...
      fs.writeFileSync(
        ...
        [
          ...
          '[ "$#" -gt 0 ] || exit 127',
          "shift",
          'exec "$STEELENGINE_TEST_SCRIPT_BIN" "$@"',
          "",
        ].join("\n"),
      );
      fs.writeFileSync(
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `steelengine_e2e_run_script_with_pty ${shellQuote("node /tmp/entry onboard")} ${shellQuote(logPath)}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
          env: shellTestEnv({
            PATH: `${tempDir}:${hostPath}`,
            STEELENGINE_E2E_COMMAND_TIMEOUT: "31s",
            STEELENGINE_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            STEELENGINE_TEST_SCRIPT_ARGS: scriptArgsPath,
            STEELENGINE_TEST_SCRIPT_BIN: path.join(tempDir, "script"),
          }),
        },
      );
