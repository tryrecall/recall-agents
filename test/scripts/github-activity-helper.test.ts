...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/recall-pr-maintainer/scripts/github-activity.sh",
);
...
describe("recall-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("tryrecall/recall-agents last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/recall\/recall\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/recall\/recall\/commits/g)).toHaveLength(1);
