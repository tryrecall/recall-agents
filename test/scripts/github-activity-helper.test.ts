...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/steelengine-pr-maintainer/scripts/github-activity.sh",
);
...
describe("steelengine-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("steelengine/steelengine last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/steelengine\/steelengine\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/steelengine\/steelengine\/commits/g)).toHaveLength(1);
