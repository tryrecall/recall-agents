// Openclaw Cross Os Release Checks tests cover steelengine cross os release checks script behavior.
...
describe("scripts/steelengine-cross-os-release-checks", () => {
  ...
  it("bounds public installer fetches on Windows and POSIX", () => {
    const windowsScript = buildInstallerSmokeScript({
      installerUrl: "https://steelengine.ai/install.ps1",
      installTarget: "2026.7.1",
      platform: "win32",
    });
    const posixScript = buildInstallerSmokeScript({
      installerUrl: "https://steelengine.ai/install.sh",
      installTarget: "2026.7.1",
      platform: "linux",
    });

    expect(windowsScript).toContain(
      "curl.exe -fsSL --connect-timeout 10 --max-time 120 -o $installerPath 'https://steelengine.ai/install.ps1'",
    );
    expect(windowsScript).toContain("steelengine-installer-");
    ...
    expect(posixScript).toContain(
      'installer_path="$(mktemp "${TMPDIR:-/tmp}/steelengine-installer-XXXXXX")"',
    );
    expect(posixScript).toContain("trap 'rm -f \"$installer_path\"' EXIT");
    expect(posixScript).toContain(
      "curl -fsSL --connect-timeout 10 --max-time 120 -o \"$installer_path\" 'https://steelengine.ai/install.sh'",
    );
  ...
  it("requires dashboard root markers and same-origin asset URLs", () => {
    const html = [
      "<title>SteelEngine Control</title>",
      "<steelengine-app></steelengine-app>",
  ...
  it("keeps gateway status RPC probing when help probing is unavailable", () => {
    ...
    expect(buildGatewayStatusArgsFromHelpText("Usage: steelengine gateway status")).toEqual([
      "gateway",
      "status",
    ]);
    expect(
  ...
  it("gives the Windows packaged updater wrapper enough headroom for SteelEngine timeout output", () => {
  ...
  it("rejects malformed cross-OS positive integer environment values", () => {
    expect(parsePositiveIntegerEnv("STEELENGINE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {})).toBe(60);
    expect(
      parsePositiveIntegerEnv("STEELENGINE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
        STEELENGINE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: "25",
      }),
    ).toBe(25);
    ...
    for (const raw of ["1e3", "25ms", "1.5", "0", "-1", String(Number.MAX_SAFE_INTEGER + 1)]) {
      expect(() =>
        parsePositiveIntegerEnv("STEELENGINE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
          STEELENGINE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: raw,
        }),
      ).toThrow("STEELENGINE_CROSS_OS_COMMAND_HEARTBEAT_SECONDS must be a positive integer");
    }
  });
  ...
  it("accepts OK agent output from the captured log when stdout is empty", () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-agent-output-"));
    try {
  ...
  it("ignores stale OK markers outside the recent agent log tail", () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-agent-output-tail-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("requires explicit opt-in before cross-OS agent turns become optional", () => {
    ...
    expect(resolveCrossOsAgentTurnOptional({ STEELENGINE_CROSS_OS_AGENT_TURN_OPTIONAL: "1" })).toBe(
      true,
    );
    expect(
      resolveCrossOsAgentTurnOptional({ STEELENGINE_CROSS_OS_AGENT_TURN_OPTIONAL: "false" }),
    ).toBe(false);
  });

  it("detects embedded fallback agent turns as non-gateway proof", () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-agent-fallback-"));
    const logPath = join(dir, "agent.log");
    expect(
      agentTurnUsedEmbeddedFallback({
  ...
  it("skips optional live agent turns only for model availability failures", () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-agent-skip-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("does not classify stale timeout logs as current optional agent-turn failures", () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-agent-skip-tail-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("only skips opted-in cross-OS live agent turns after retry exhaustion", () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-agent-skip-retry-"));
    try {
      const logPath = join(dir, "agent.log");
  ...
  it("allows cross-OS provider smoke models to use faster CI overrides", () => {
    expect(
      resolveProviderConfig("openai", {
        STEELENGINE_CROSS_OS_OPENAI_MODEL: "openai/gpt-5.4-mini",
      })?.model,
    ).toBe("openai/gpt-5.4-mini");
    expect(
      resolveProviderConfig("openai", {
        STEELENGINE_CROSS_OS_MODEL: "openai/gpt-5.4-nano",
      })?.model,
  ...
  it("keeps release cross-OS OpenAI smoke on GPT-5.6 Luna", () => {
    ...
    const releaseChecks = readFileSync(".github/workflows/steelengine-release-checks.yml", "utf8");

    expect(workflow).toContain(
      "STEELENGINE_CROSS_OS_OPENAI_MODEL: ${{ inputs.openai_model || vars.STEELENGINE_CROSS_OS_OPENAI_MODEL || 'openai/gpt-5.6-luna' }}",
    );
  ...
  it("can stage packaged-upgrade baselines without npm lifecycle scripts", () => {
    expect(buildNpmGlobalInstallArgs("steelengine@2026.5.2", { ignoreScripts: true })).toEqual([
      "install",
      "-g",
      "steelengine@2026.5.2",
      "--omit=dev",
      "--no-fund",
      "--no-audit",
      "--ignore-scripts",
  ...
  it("rejects unsafe npm pack tarball filenames before staging release artifacts", () => {
    expect(resolveNpmPackTarballFileName("steelengine-2026.6.17.tgz")).toBe("steelengine-2026.6.17.tgz");
  ...
  it("accepts pnpm pack tarballs reported under the requested destination", () => {
    const packDir = resolvePath("/tmp/steelengine-pack");

    expect(resolvePackDestinationTarball("steelengine-2026.6.17.tgz", packDir, "pnpm pack")).toEqual({
      fileName: "steelengine-2026.6.17.tgz",
      path: resolvePath(packDir, "steelengine-2026.6.17.tgz"),
    });
    expect(
      resolvePackDestinationTarball(
        resolvePath(packDir, "steelengine-2026.6.17.tgz"),
        packDir,
        "pnpm pack",
      ),
    ).toEqual({
  ...
  it("rejects pnpm pack tarballs outside the requested destination", () => {
    ...
    const unsafeFilenames = [
      ...
      "nested/steelengine.tgz",
      "nested\\steelengine.tgz",
      resolvePath(dirname(packDir), "steelengine.tgz"),
      resolvePath(packDir, "nested", "steelengine.tgz"),
      "steelengine\u0000.tgz",
      "steelengine.tar.gz",
    ];
  ...
  it("falls back to pnpm pack for historical refs without the Docker package helper", () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-pack-command-"));
    try {
      ...
      const helperPath = join(dir, "scripts", "package-steelengine-for-docker.mjs");
  ...
  it("keeps packaged-upgrade release updates out of service restart flow", () => {
    const args = buildPackagedUpgradeUpdateArgs("http://127.0.0.1:49152/steelengine-current.tgz");
    expect(args.slice(0, 6)).toEqual([
      "update",
      "--tag",
      "http://127.0.0.1:49152/steelengine-current.tgz",
      "--yes",
      "--json",
      "--no-restart",
    ]);
  ...
  it("keeps cross-OS live smoke agent turns on GPT-5-safe timeouts and minimal context", () => {
    ...
    expect(source).toContain('agentRuntime: { id: "steelengine" }');
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'steelengine.cmd')");
    expect(script).toContain("$cmd = Get-Command steelengine -ErrorAction Stop");
  });
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        STEELENGINE_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        STEELENGINE_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        STEELENGINE_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("steelengine-2026.4.14.tgz")).toBe("application/octet-stream");
  });

  it("streams release artifacts from the static file server", async () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-static-server-"));
    const filePath = join(dir, "steelengine-2026.4.14.tgz");
  ...
  it("closes static release artifact sockets left by aborted clients", async () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-static-server-close-"));
    const filePath = join(dir, "steelengine-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("flushes static release artifact logs before close resolves", async () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-static-server-log-flush-"));
    const filePath = join(dir, "steelengine-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("uses the published installer URLs for native installer lanes", () => {
    expect(resolvePublishedInstallerUrl("darwin")).toBe("https://steelengine.ai/install.sh");
    expect(resolvePublishedInstallerUrl("linux")).toBe("https://steelengine.ai/install.sh");
    expect(resolvePublishedInstallerUrl("win32")).toBe("https://steelengine.ai/install.ps1");
  });
  ...
  it("runs the installed browser override import smoke only on native Windows", () => {
    ...
    expect(script).toContain('from "steelengine/plugin-sdk/plugin-runtime"');
    expect(script).toContain('overrideEnvVar: "STEELENGINE_BROWSER_CONTROL_MODULE"');
    ...
    const installedScript = buildInstalledBrowserOverrideImportProbeScript(
      "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/steelengine/dist/plugin-sdk/plugin-runtime.js",
    );
    expect(installedScript).toContain(
      'from "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/steelengine/dist/plugin-sdk/plugin-runtime.js"',
    );
    expect(readFileSync("scripts/lib/cross-os-release-checks/install.ts", "utf8")).toContain(
      "STEELENGINE_BROWSER_CONTROL_MODULE: pathToFileURL(overridePath).href",
    );
  });
  ...
  it("wraps Windows cmd shims without Node shell argv", () => {
    expect(
      resolveCommandSpawnInvocation(
        String.raw`C:\Program Files\nodejs\npm.cmd`,
        ["view", "steelengine@latest", "version"],
        {
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Program Files\nodejs\npm.cmd" view steelengine@latest version"`,
      ],
      shell: false,
  ...
  it("wraps installed Windows CLI cmd fallbacks without Node shell argv", () => {
    expect(
      resolveInstalledCliInvocation(
        win32.join(String.raw`C:\SteelEngine Prefix`, "steelengine.cmd"),
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\SteelEngine Prefix\steelengine.cmd" gateway run --port 1234"`,
      ],
      shell: false,
      windowsVerbatimArguments: true,
    });
  ...
  it("runs resolved command invocations and writes command logs", async () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-run-command-"));
    try {
  ...
  it("bounds retained command output while preserving full command logs", async () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-run-command-output-"));
    try {
      const logPath = join(dir, "command.log");
  ...
  it("flushes command logs before resolving", async () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-run-command-flush-"));
    try {
  ...
  it("reads npm debug logs from the Windows cache root", () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-npm-debug-"));
    try {
  ...
  it("prefers npm configured log directories over cache defaults", () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-npm-logs-dir-"));
    try {
      const homeDir = join(dir, "home");
  ...
  it("keeps npm debug log collection best-effort", () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-npm-debug-best-effort-"));
    try {
      const homeDir = join(dir, "home");
      const logPath = join(dir, "install.log");
  ...
  it("resolves relative npm log config from the install working directory", () => {
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-npm-relative-logs-"));
    try {
      const homeDir = join(dir, "home");
  ...
  it("kills timed-out command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-run-command-timeout-"));
  ...
  it("forwards external termination to command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-run-command-signal-"));
    const childPidPath = join(dir, "child.pid");
    ...
    try {
      ...
      const runner = spawn(
        ...
        ["--import", "tsx", "--input-type=module", "-e", runnerScript],
        {
          ...
          env: {
            ...process.env,
            STEELENGINE_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS: "200",
  ...
  it("exits promptly after externally signaled commands close", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-run-command-signal-exit-"));
    const childPidPath = join(dir, "child.pid");
    ...
    try {
      ...
      const runner = spawn(
        ...
        ["--import", "tsx", "--input-type=module", "-e", runnerScript],
        {
          ...
          env: {
            ...process.env,
            STEELENGINE_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS: "3000",
            STEELENGINE_TEST_CHILD_PID: childPidPath,
          },
          stdio: ["ignore", "ignore", "pipe"],
        },
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/steelengine", "darwin"),
    ).toBe("/Users/runner/.npm-global");
  });

  it("resolves Linux npm package roots when the CLI is a user-local shim", () => {
    const homeDir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-linux-home-"));
    try {
      const packageRoot = join(homeDir, ".npm-global", "lib", "node_modules", "steelengine");
      const distDir = join(packageRoot, "dist");
      const cliDir = join(homeDir, ".local", "bin");
      mkdirSync(distDir, { recursive: true });
      mkdirSync(cliDir, { recursive: true });
      writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "steelengine" }));
      writeFileSync(join(distDir, "entry.js"), "#!/usr/bin/env node\n");

      expect(
        resolveInstalledPackageRootFromCliPath(join(cliDir, "steelengine"), "linux", {
          HOME: homeDir,
        }),
      ).toBe(packageRoot);

      rmSync(join(cliDir, "steelengine"), { force: true });
      symlinkSync(join(distDir, "entry.js"), join(cliDir, "steelengine"));
  ...
  it("drops the bundled plugin postinstall disable flag for real updater calls", () => {
    expect(
      buildRealUpdateEnv({
        FOO: "bar",
        NODE_COMPILE_CACHE: "/tmp/stale-steelengine-cache",
      ...
      verifyPackagedUpgradeUpdateResult(
        {
          ...
          stderr:
            "[steelengine] Failed to start CLI: Error [ERR_MODULE_NOT_FOUND]: Cannot find module '/tmp/prefix/lib/node_modules/steelengine/dist/memory-state-old.js'",
        },
  ...
  it("recognizes the shipped Windows updater packaged-upgrade timeout", () => {
    const error = new Error(
      "Command timed out: C:\\hostedtoolcache\\windows\\node\\24.15.0\\x64\\node.exe C:\\Users\\RUNNER~1\\AppData\\Local\\Temp\\steelengine-upgrade-q9DsA7\\prefix\\node_modules\\steelengine\\steelengine.mjs update --tag http://127.0.0.1:49951/steelengine-2026.5.4-beta.1.tgz --yes --json --no-restart --timeout 1500",
    );
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error(
          "Command timed out: C:\\prefix\\node_modules\\steelengine\\steelengine.mjs update --tag http://127.0.0.1:49951/steelengine-current.tgz --yes --json --timeout 1500",
        ),
        "win32",
      ),
    ).toBe(true);
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error("Command timed out: node steelengine.mjs update --tag steelengine@beta"),
        "win32",
      ),
    ).toBe(false);
  });
      ...
      isRecoverableWindowsPackagedUpgradeSwapCleanupFailure(
        {
          exitCode: 1,
          stdout:
            "EPERM: operation not permitted, unlink '/tmp/prefix/node_modules/.steelengine-1-2/native.node'",
          stderr: "",
        },
        "linux",
      ),
  ...
  it("only treats pinned baseline specs as exact installer version assertions", () => {
    ...
    expect(resolveExplicitBaselineVersion("steelengine@latest")).toBe("");
    expect(resolveExplicitBaselineVersion("steelengine@2026.4.10")).toBe("2026.4.10");
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    const prefixDir = mkdtempSync(join(tmpdir(), "steelengine-cross-os-installed-version-"));
    try {
      const packageRoot =
        process.platform === "win32"
          ? join(prefixDir, "node_modules", "steelengine")
          : join(prefixDir, "lib", "node_modules", "steelengine");
  ...
  it("treats missing package scripts as optional in older refs", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "steelengine-cross-os-scripts-"));
    try {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
  ...
  it("rejects legacy plugin dependency staging debris before candidate inventory generation", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "steelengine-cross-os-stage-debris-"));
    try {
      mkdirSync(
        join(packageRoot, "dist", "Extensions", "demo", ".SteelEngine-Install-Stage", "node_modules"),
        { recursive: true },
      );
      writeFileSync(
        join(packageRoot, "dist", "Extensions", "demo", ".SteelEngine-Install-Stage", "package.json"),
        "{}\n",
        "utf8",
      );
  ...
  it("omits local build metadata from candidate package inventories", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "steelengine-cross-os-local-stamps-"));
    try {
      mkdirSync(join(packageRoot, "dist"), { recursive: true });
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({ name: "steelengine-fixture", version: "0.0.0", files: ["dist/"] }),
        "utf8",
      );
