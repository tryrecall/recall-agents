...
describe("real-behavior-proof-policy", () => {
  it.each([
    ...
    "Linked artifact: https://github.com/steelengineai/recall-agents/actions/runs/123456789/artifacts/987654321",
    "Redacted runtime log: gateway connected Discord channel and delivered the reply.",
    ["Terminal transcript:", "```text", "$ steelengine gateway status", "discord ready", "```"].join(
      "\n",
    ),
    ...
    const evaluation = evaluatePullRequestContext({
      pullRequest: externalPr(
        [
          ...
          "",
          "- Behavior addressed: The gateway dropped the configured Discord channel during startup.",
          "- Real environment tested: macOS 15.4, Node 24, local SteelEngine gateway.",
          "- Exact steps or command run after this patch: pnpm steelengine gateway restart",
  ...
  it("accepts out-of-scope follow-ups as not-tested proof detail", () => {
    const body = [
      ...
      "  - `lowSupported: true`",
      "  - `fallbackFromLow: low`",
      "  - `local command version: SteelEngine 2026.5.21`",
      "",
  ...
  it("accepts source PR proof when explicit gaps live in out-of-scope follow-ups", () => {
    const body = [
      ...
      "## Evidence",
      "",
      "- Real environment tested: Local macOS source checkout, Node v24.8.0, SteelEngine 2026.5.21 (c8a35c4), local `steelengine` shim pointed at the freshly built checkout. No channel credentials or provider API keys were used.",
      "- Exact steps or command run after this patch:",
      "  1. Built the local checkout with `node scripts/build-all.mjs`.",
      "  2. Updated `/Users/example/.local/bin/steelengine` to run this checkout's `steelengine.mjs` and verified `/Users/example/.local/bin/steelengine --version`.",
      ...
      "  - `lowSupported: true`",
      "  - `fallbackFromLow: low`",
      "  - `local command version: SteelEngine 2026.5.21 (c8a35c4)`",
      "",
      "## Out-of-scope Follow-ups",
  ...
  it("accepts exact SteelEngine ClawSweeper bot pass verdict markers when GitHub omits the app source", () => {
    const pullRequest = {
      number: 83581,
      head: {
        sha: "06ee95df6608d29a395c52ba8ab53fdd93a9dc4f",
    ...
    const comments = [
      {
        user: {
          login: "steelengine-clawsweeper[bot]",
          type: "Bot",
        },
        body: "<!-- clawsweeper-verdict:pass item=83581 sha=06ee95df6608d29a395c52ba8ab53fdd93a9dc4f confidence=high -->",
      },
...
describe("isMaintainerTeamMember", () => {
  ...
  it("returns true for active members", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://api.github.com/orgs/steelengine/teams/maintainer/memberships/private-maint",
