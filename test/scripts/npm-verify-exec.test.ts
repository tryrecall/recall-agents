...
function makeTempRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), "steelengine-npm-verify-exec-"));
  tempDirs.push(root);
  return root;
}
...
describe("npm verifier command execution", () => {
  ...
  it("rejects malformed command limit environment values", () => {
    ...
    withEnv({ STEELENGINE_NPM_VERIFY_COMMAND_TIMEOUT_MS: "5m" }, () => {
      expect(() =>
        runNpmVerifyCommand(
          { command: process.execPath, args: ["-e", "process.stdout.write('ok')"] },
          root,
        ),
      ).toThrow("invalid STEELENGINE_NPM_VERIFY_COMMAND_TIMEOUT_MS: 5m");
    });

    withEnv({ STEELENGINE_NPM_VERIFY_COMMAND_MAX_BUFFER_BYTES: "16mb" }, () => {
      expect(() =>
        runNpmVerifyCommand(
          { command: process.execPath, args: ["-e", "process.stdout.write('ok')"] },
          root,
        ),
      ).toThrow("invalid STEELENGINE_NPM_VERIFY_COMMAND_MAX_BUFFER_BYTES: 16mb");
    });
  });
});
