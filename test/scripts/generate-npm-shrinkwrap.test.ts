...
describe("generate-npm-shrinkwrap", () => {
  ...
  it("omits workspace packages that are published beside the package", () => {
    const normalized = packageJsonForShrinkwrap(
      {
        dependencies: { "@steelengine/ai": "workspace:2026.6.11", chalk: "5.6.2" },
  ...
  it("normalizes pnpm scoped override selectors for npm shrinkwrap", () => {
    expect(
      normalizeOverrides({
        "steelengine@2026.5.28>undici": "8.5.0",
        tar: 7.5,
      }),
    ).toEqual({
      "steelengine@2026.5.28": {
        undici: "8.5.0",
      },
      tar: "7.5",
    });
  ...
  it("validates shrinkwrap worker counts from flags and environment", () => {
    ...
    expect(resolveShrinkwrapJobs(undefined, { STEELENGINE_NPM_SHRINKWRAP_JOBS: "2" })).toBe(2);
    expect(() => resolveShrinkwrapJobs("0", {})).toThrow("invalid STEELENGINE_NPM_SHRINKWRAP_JOBS: 0");
  ...
  it("accepts strict npm shrinkwrap command timeout and buffer overrides", () => {
    expect(
      createNpmShrinkwrapExecOptions({ command: "npm", args: ["install"] }, "/tmp/package", {
        STEELENGINE_NPM_SHRINKWRAP_COMMAND_MAX_BUFFER_BYTES: "1048576",
        STEELENGINE_NPM_SHRINKWRAP_COMMAND_TIMEOUT_MS: "30000",
      }),
    ).toMatchObject({
      maxBuffer: 1024 * 1024,
      timeout: 30000,
  ...
  it("rejects loose npm shrinkwrap command timeout and buffer overrides", () => {
    expect(() =>
      createNpmShrinkwrapExecOptions({ command: "npm", args: ["install"] }, "/tmp/package", {
        STEELENGINE_NPM_SHRINKWRAP_COMMAND_TIMEOUT_MS: "30s",
      }),
    ).toThrow("invalid STEELENGINE_NPM_SHRINKWRAP_COMMAND_TIMEOUT_MS: 30s");
    expect(() =>
      createNpmShrinkwrapExecOptions({ command: "npm", args: ["install"] }, "/tmp/package", {
        STEELENGINE_NPM_SHRINKWRAP_COMMAND_MAX_BUFFER_BYTES: "64mb",
      }),
    ).toThrow("invalid STEELENGINE_NPM_SHRINKWRAP_COMMAND_MAX_BUFFER_BYTES: 64mb");
  });
  ...
  it("parses nested scoped package paths", () => {
    expect(
      parseLockPackagePath("node_modules/@steelengine/codex/node_modules/@anthropic-ai/sdk"),
    ).toEqual([
      {
        name: "@steelengine/codex",
        path: "node_modules/@steelengine/codex",
      },
      {
        name: "@anthropic-ai/sdk",
        path: "node_modules/@steelengine/codex/node_modules/@anthropic-ai/sdk",
      },
    ]);
  });
  ...
  it("disables embedded shrinkwraps that hide workspace overrides", () => {
    const lockfile = {
      packages: {
        ...
        "node_modules/@steelengine/codex": {
          version: "0.75.4",
          hasShrinkwrap: true,
        },
        "node_modules/@steelengine/codex/node_modules/protobufjs": {
          version: "7.5.9",
        },
        "node_modules/@steelengine/codex/node_modules/fetch-blob": {
          version: "4.0.0",
        },
        "node_modules/@steelengine/codex/node_modules/fetch-blob/node_modules/node-domexception": {
          version: "1.0.0",
        },
      },
    };
    ...
    expect(lockfile.packages["node_modules/@steelengine/codex"]).not.toHaveProperty("hasShrinkwrap");
    expect(
      lockfile.packages["node_modules/@steelengine/codex/node_modules/protobufjs"],
    ).toBeUndefined();
  });
  ...
  it("uses legacy peer resolution when the package has optional peers", () => {
    expect(
      shouldUseLegacyPeerDepsForShrinkwrap({
        ...
        peerDependencies: { steelengine: ">=2026.5.30" },
