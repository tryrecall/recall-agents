...
function git(cwd: string, args: string[]): string {
  return execFileSync("git", args, {
    ...
    env: {
      ...
      GIT_AUTHOR_EMAIL: "test@steelengine.invalid",
      GIT_COMMITTER_NAME: "SteelEngine Test",
      GIT_COMMITTER_EMAIL: "test@steelengine.invalid",
    },
  }).trim();
}
...
describe("release-note verification", () => {
  ...
  it("stores default GitHub snapshots in the shared Git common directory", () => {
    const commonDir = resolve("/tmp/steelengine-shared-git");
    expect(defaultGithubSnapshotPath("a".repeat(40), "b".repeat(40), commonDir)).toBe(
      join(
        commonDir,
        "steelengine-release-cache",
  ...
  it("reuses exact-range GitHub GraphQL snapshots without caching REST reads", () => {
    const cwd = mkdtempSync(join(tmpdir(), "steelengine-release-notes-snapshot-"));
    try {
      ...
      expect(
        githubApiWithSnapshot(["repos/steelengine/steelengine/releases/tags/v1"], fetchApi, first),
      ).toEqual({
        data: {
          request: ["repos/steelengine/steelengine/releases/tags/v1"],
          fetches: 2,
        },
      });
  ...
  it("records a canonical target SHA when --target is symbolic", () => {
    const cwd = mkdtempSync(join(tmpdir(), "steelengine-release-notes-"));
    try {
      git(cwd, ["init", "-q"]);
      writeFileSync(
