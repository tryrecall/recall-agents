...
describe("scripts/test-live", () => {
  ...
  posixIt("signals the live pnpm child when the wrapper is terminated", async () => {
    const root = mkdtempSync(join(tmpdir(), "steelengine-test-live-signal-"));
  ...
  posixIt("kills the live pnpm process group after the no-output timeout", async () => {
    const root = mkdtempSync(join(tmpdir(), "steelengine-test-live-timeout-"));
    const fakePnpmPath = join(root, "pnpm");
    const childPidPath = join(root, "child.pid");
    const descendantPidPath = join(root, "descendant.pid");
    const stderr: Buffer[] = [];
    ...
    const runner = spawn(process.execPath, ["scripts/test-live.mjs", "--", "fake.live.test.ts"], {
      env: {
        ...
        STEELENGINE_FAKE_PNPM_PID_PATH: childPidPath,
        STEELENGINE_FAKE_PNPM_DESCENDANT_PID_PATH: descendantPidPath,
        STEELENGINE_LIVE_WRAPPER_HEARTBEAT_MS: "25",
        STEELENGINE_VITEST_NO_OUTPUT_TIMEOUT_MS: "100",
        npm_execpath: fakePnpmPath,
      },
  ...
  it("rejects loose heartbeat intervals instead of parsing prefixes", () => {
    ...
    expect(resolveTestLiveHeartbeatMs({ STEELENGINE_LIVE_WRAPPER_HEARTBEAT_MS: "2500" })).toBe(2500);
    expect(() => resolveTestLiveHeartbeatMs({ STEELENGINE_LIVE_WRAPPER_HEARTBEAT_MS: "1e3" })).toThrow(
      "invalid STEELENGINE_LIVE_WRAPPER_HEARTBEAT_MS: 1e3",
    );
    expect(() =>
      resolveTestLiveHeartbeatMs({ STEELENGINE_LIVE_WRAPPER_HEARTBEAT_MS: "1000ms" }),
    ).toThrow("invalid STEELENGINE_LIVE_WRAPPER_HEARTBEAT_MS: 1000ms");
    expect(() => resolveTestLiveHeartbeatMs({ STEELENGINE_LIVE_WRAPPER_HEARTBEAT_MS: "0" })).toThrow(
      "invalid STEELENGINE_LIVE_WRAPPER_HEARTBEAT_MS: 0",
    );
  });
