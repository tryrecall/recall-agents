// Check Openclaw Package Tarball tests cover check steelengine package tarball script behavior.
...
const CHECK_SCRIPT = "scripts/check-steelengine-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "steelengine-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(
      join(packageRoot, "package.json"),
      JSON.stringify({ name: "steelengine", version, ...options.packageJson }),
    );
    ...
    const controlUiFiles =
      options.includeControlUi === false
        ? {}
        : {
            "dist/control-ui/index.html": "<!doctype html><steelengine-app></steelengine-app>",
      ...
      options.includeInstallGuard === false
        ...
        : {
            [PACKAGE_INSTALL_GUARD_RELATIVE_PATH]:
              "SteelEngine package preinstall has not completed.\n",
          };
    ...
    const tarball = join(root, "steelengine.tgz");
...
describe("check-steelengine-package-tarball", () => {
  it("prints help before touching tarball state", () => {
    const result = spawnSync("node", [CHECK_SCRIPT, "--help"], { encoding: "utf8" });

    expect(result.status, result.stderr).toBe(0);
    expect(result.stdout).toContain(
      "Usage: node scripts/check-steelengine-package-tarball.mjs [--require-bundled-workspace-deps] <steelengine.tgz>",
    );
  ...
  it("rejects option-like and extra arguments before tar inspection", () => {
    ...
    expect(unknown.stderr).toContain("Unknown SteelEngine package tarball check option: --tag");
    expect(unknown.stderr).not.toContain("SteelEngine package tarball does not exist");

    const extra = spawnSync("node", [CHECK_SCRIPT, "steelengine.tgz", "extra"], {
      encoding: "utf8",
    });

    expect(extra.status).not.toBe(0);
    expect(extra.stderr).toContain("Unexpected SteelEngine package tarball check argument: extra");
    expect(extra.stderr).not.toContain("SteelEngine package tarball does not exist");
  });

  it.runIf(process.platform !== "win32")(
    "removes the extract dir when tar extraction fails",
    () => {
      const root = mkdtempSync(join(tmpdir(), "steelengine-package-tarball-extract-fail-"));
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "fs.writeFileSync(process.env.STEELENGINE_TEST_EXTRACT_DIR_FILE, outputDir);",
        ...
        const result = spawnSync("node", [CHECK_SCRIPT, tarball], {
          ...
          env: {
            ...process.env,
            STEELENGINE_TEST_EXTRACT_DIR_FILE: extractDirFile,
  ...
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("SteelEngine package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
  ...
  it("rejects workspace protocol dependencies in package manifests", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "package.json dependencies.@steelengine/ai must not use workspace protocol workspace:*",
        );
      },
      "2026.6.11",
      { packageJson: { dependencies: { "@steelengine/ai": "workspace:*" } } },
    );
  });
  ...
  it("rejects workspace protocol dependencies in shrinkwrap root metadata", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "npm-shrinkwrap.json packages root dependencies.@steelengine/ai must not use workspace protocol workspace:*",
        );
      },
      "2026.6.11",
      { shrinkwrapRootPackage: { dependencies: { "@steelengine/ai": "workspace:*" } } },
    );
  });
  ...
  it("accepts separately published private workspace dependencies by default", () => {
    withTarball(
      ...
      "2026.6.11",
      { packageJson: { dependencies: { "@steelengine/ai": "2026.6.11" } } },
    );
  });
  ...
  it("rejects private workspace dependencies that are not bundled when strict packaging requires it", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "package.json dependencies.@steelengine/ai must be listed in bundleDependencies because it is private to the SteelEngine workspace",
        );
        expect(result.stderr).toContain(
          "package.json dependencies.@steelengine/ai must be bundled in node_modules/@steelengine/ai",
        );
      },
      "2026.6.11",
      { packageJson: { dependencies: { "@steelengine/ai": "2026.6.11" } } },
  ...
  it("rejects private workspace dependencies when only metadata is bundled", () => {
    withTarball(
      ...
      {
        "dist/index.js": "export {};\n",
        "node_modules/@steelengine/ai/package.json": AI_RUNTIME_PACKAGE_JSON,
      },
      (tarball) => {
        const result = spawnSync(
          "node",
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "bundled @steelengine/ai is missing required runtime entry dist/index.mjs",
        );
        expect(result.stderr).toContain(
          "bundled @steelengine/ai is missing required runtime entry dist/providers.mjs",
        );
        expect(result.stderr).toContain(
          "bundled @steelengine/ai is missing required runtime entry dist/internal/runtime.mjs",
        );
      },
      "2026.6.11",
      {
        packageJson: {
          dependencies: { "@steelengine/ai": "2026.6.11" },
  ...
  it("accepts private workspace dependencies when their runtime is bundled", () => {
    withTarball(
      ["dist/index.js"],
      {
        "dist/index.js": "export {};\n",
        "node_modules/@steelengine/ai/package.json": AI_RUNTIME_PACKAGE_JSON,
        "node_modules/@steelengine/ai/dist/index.mjs": "export {};\n",
        "node_modules/@steelengine/ai/dist/providers.mjs": "export {};\n",
        "node_modules/@steelengine/ai/dist/internal/runtime.mjs": "export {};\n",
      },
      (tarball) => {
        const result = spawnSync(
          "node",
  ...
  it("rejects bundled AI entries that its manifest does not export", () => {
    withTarball(
      ...
      {
        "dist/index.js": "export {};\n",
        "node_modules/@steelengine/ai/package.json": JSON.stringify({
          name: "@steelengine/ai",
          version: "2026.6.11",
          exports: {
            ".": "./dist/index.mjs",
      ...
      (tarball) => {
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "bundled @steelengine/ai runtime specifier @steelengine/ai/providers is not resolvable",
        );
      },
      "2026.6.11",
      {
  ...
  it("rejects missing relative imports from bundled AI runtime entries", () => {
    withTarball(
      ["dist/index.js"],
      {
        ...
        "node_modules/@steelengine/ai/dist/index.mjs": "export {};\n",
        "node_modules/@steelengine/ai/dist/providers.mjs": "export {};\n",
        "node_modules/@steelengine/ai/dist/internal/runtime.mjs": 'export * from "./missing.mjs";\n',
      },
      (tarball) => {
        const result = spawnSync(
          "node",
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "bundled @steelengine/ai dist/internal/runtime.mjs imports missing dist/internal/missing.mjs",
        );
      },
      "2026.6.11",
      {
