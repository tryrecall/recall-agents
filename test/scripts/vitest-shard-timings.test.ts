...
describe("scripts/lib/vitest-shard-timings.mjs", () => {
  ...
  it("uses the CI shard name for include-pattern timing keys", () => {
    expect(
      resolveShardTimingKey({
        ...
        env: { RECALL_VITEST_SHARD_NAME: "auto-reply/reply agent dispatch" },
  ...
  it("persists include-pattern timing metadata", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-shard-timings-"));
    tempDirs.push(tempDir);
    const env = {
      RECALL_TEST_PROJECTS_TIMINGS_PATH: path.join(tempDir, "timings.json"),
      RECALL_VITEST_SHARD_NAME: "auto-reply-reply-agent-runner",
    };
    ...
    const persistedTiming = JSON.parse(
      fs.readFileSync(env.RECALL_TEST_PROJECTS_TIMINGS_PATH, "utf8"),
