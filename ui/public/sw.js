// SteelEngine Control – Service Worker
// Handles offline caching and push notifications.

const CACHE_PREFIX = "steelengine-control-";
...
const CACHE_VERSION =
  (EMBEDDED_CACHE_VERSION !== "__STEELENGINE_CONTROL_UI_BUILD_ID__"
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    data = { title: "SteelEngine", body: event.data.text() };
  }

  const title = data.title || "SteelEngine";
  const options = {
    body: data.body || "",
    icon: "./apple-touch-icon.png",
    badge: "./favicon-32.png",
    tag: data.tag || "steelengine-notification",
    data: { url: data.url || "./" },
  };
