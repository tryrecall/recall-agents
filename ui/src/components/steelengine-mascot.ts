...
class SteelEngineMascot extends LitElement {
  static override styles = css`
    :host {
      display: inline-block;
      width: var(--steelengine-mascot-size, 120px);
      height: var(--steelengine-mascot-size, 120px);
      overflow: visible;
  ...
  protected override updated(changed: PropertyValues<this>): void {
    if (changed.has("size")) {
      this.style.setProperty("--steelengine-mascot-size", `${this.resolvedSize}px`);
    }
    if (changed.has("mood")) {
      this.animator.setMood(this.resolvedMood, currentSeconds());
    }
...
if (!customElements.get("steelengine-mascot")) {
  customElements.define("steelengine-mascot", SteelEngineMascot);
}
