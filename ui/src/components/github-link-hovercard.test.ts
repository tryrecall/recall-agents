...
const GITHUB_LINK_HOVERCARD_ELEMENT_NAME = `test-steelengine-github-link-hovercard-provider-${crypto.randomUUID()}`;
...
describe("steelengine-github-link-hovercard-provider", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it("renders and caches pull request details without changing the link", async () => {
    ...
    const href = "https://github.com/steelengineai/recall-agents/pull/99816";
    ...
    expect(card?.textContent).toContain("steelengine/steelengine #99816");
  ...
  it("renders issue comments and supports focus plus Escape", async () => {
    ...
    const { anchor, provider } = createLink(
      "https://github.com/steelengineai/recall-agents/issues/99815",
      "#99815",
    );
    provider.client = { request } as unknown as GatewayBrowserClient;
  ...
  it("ignores unsupported GitHub links and shows a quiet unavailable state", async () => {
    ...
    const unsupportedLink = createLink("https://github.com/steelengineai/recall-agents", "repository");
    ...
    const missingLink = createLink("https://github.com/steelengineai/recall-agents/issues/999999", "missing");
  ...
  it.each([
    "http://github.com/steelengine/steelengine/issues/99815",
    "https://user:password@github.com/steelengine/steelengine/issues/99815",
    "https://example.com/steelengine/steelengine/issues/99815",
    "javascript:alert(1)",
  ...
  it("preserves an existing description when hover ends before opening", async () => {
    const request = vi.fn();
    const { anchor, provider } = createLink("https://github.com/steelengineai/recall-agents/issues/99815");
    provider.client = { request } as unknown as GatewayBrowserClient;
