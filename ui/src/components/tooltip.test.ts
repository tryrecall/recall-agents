...
function createTooltip(content: string, triggerText = "trigger") {
  const tooltip = document.createElement("steelengine-tooltip") as TooltipElement;
...
function createProvider() {
  return document.createElement("steelengine-tooltip-provider") as TooltipProviderElement;
}
...
function expectOpenCount(count: number) {
  const open = [...document.querySelectorAll<TooltipElement>("steelengine-tooltip")].filter(
    (tooltip) => webAwesomeTooltip(tooltip)?.open,
  );
  expect(open).toHaveLength(count);
}

describe("steelengine-tooltip", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
