...
export class SteelEngineModalDialog extends SteelEngineLitElement {
  ...
  static override styles = css`
    ...
    wa-dialog {
      --width: min(var(--steelengine-modal-width, 540px), calc(100vw - 48px));
      --spacing: 0;
      --backdrop-filter: blur(4px);
    }

    wa-dialog::part(dialog) {
      max-height: var(--steelengine-modal-max-height, calc(100dvh - 48px));
      padding: 0;
      border: 0;
...
if (!customElements.get("steelengine-modal-dialog")) {
  customElements.define("steelengine-modal-dialog", SteelEngineModalDialog);
}

declare global {
  interface HTMLElementTagNameMap {
    "steelengine-modal-dialog": SteelEngineModalDialog;
  }
}
