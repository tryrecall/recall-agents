  ...
  it("disables cloud profiles with the runtime preflight reason", () => {
    ...
    render(
      renderCloudProfileMenuItems({
        ...
        submitting: false,
        disabled: true,
        disabledReason: "Cloud workers require the SteelEngine runtime; codex is selected.",
        onSelect: vi.fn(),
      }),
      container,
    );
    ...
    expect(button?.title).toBe("Cloud workers require the SteelEngine runtime; codex is selected.");
  });
});
