...
describeControlUiE2e("Control UI Workboard status persistence E2E", () => {
  beforeAll(async () => {
    if (!chromiumAvailable) {
      throw new Error(
        `Playwright Chromium is not installed at ${chromiumExecutablePath}. Run \`pnpm --dir ui exec playwright install chromium\`, set PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH to a compatible browser, or set STEELENGINE_UI_E2E_ALLOW_MISSING_CHROMIUM=1 only when intentionally skipping this lane.`,
      );
    }
  ...
  it("persists edit/reopen fields and does not bounce a dragged linked card from stale lifecycle", async () => {
    ...
    try {
      ...
      await page
        .locator('steelengine-modal-dialog[label="Edit card"] .workboard-modal__actions')
        .last()
