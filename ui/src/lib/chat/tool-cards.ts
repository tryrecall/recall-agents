...
function resolveTranscriptMessageId(message: Record<string, unknown>): string | undefined {
  ...
  const steelEngineMeta = message["__steelengine"];
  const transcriptMeta =
    steelEngineMeta && typeof steelEngineMeta === "object" && !Array.isArray(steelEngineMeta)
      ? (steelEngineMeta as Record<string, unknown>)
      : null;
...
function extractToolCards(message: unknown, prefix = "tool"): ToolCard[] {
  ...
  const isLiveToolStream = m["__steelengineToolStreamLive"] === true;
  ...
  for (let index = 0; index < content.length; index++) {
    ...
    if (isToolCall) {
      ...
      cards.push({
        ...
        ...(isLiveToolStream
          ? { live: true, completed: m["__steelengineToolStreamResultReceived"] === true }
          : {}),
