  ...
  const guidance =
    {
      ...
      "not-git-install":
        "Not a git checkout. Run `steelengine update` from the CLI for a global reinstall.",
      "not-steelengine-root":
        "Run the update from an SteelEngine checkout or use the CLI global reinstall path.",
      ...
      "doctor-failed": "Doctor repair failed. Run `steelengine doctor --non-interactive` and retry.",
...
function resolvePendingUpdateHandoffTimeoutBanner(): ApplicationStatusBanner {
  return {
    tone: "danger",
    text: "Update handoff started, but completion was not reported after reconnect. Run `steelengine update status` for the final result.",
  };
}
...
export function createApplicationOverlays(
  ...
  const stopEvents = gateway.subscribeEvents((event) => {
    ...
    if (
      ...
      event.event === "steelengine.approval.resolved"
    ) {
