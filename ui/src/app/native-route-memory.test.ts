...
describe("native route memory", () => {
  ...
  it("drops corrupt and invalid entries", () => {
    storage.setItem("steelengine.native.lastRoute", "{");
    expect(considerRouteRestore("chat", "", storage, true)).toBeNull();
    expect(storage.getItem("steelengine.native.lastRoute")).toBeNull();

    storage.setItem(
      "steelengine.native.lastRoute",
  ...
  it("does nothing outside the native host", () => {
    storage.setItem("steelengine.native.lastRoute", JSON.stringify({ routeId: "usage", search: "" }));
    persistRoute("chat", "", storage, false);
    expect(considerRouteRestore("chat", "", storage, false)).toBeNull();
    expect(JSON.parse(storage.getItem("steelengine.native.lastRoute") ?? "{}")).toEqual({
      routeId: "usage",
      search: "",
    });
  });
