...
const WORKSPACE = "/home/peter/steelengine";
const PICKED = "/home/peter/steelengine/packages";
...
async function replaceGatewayClient(page: Page) {
  await page.evaluate(() => {
    const app = document.querySelector("steelengine-app") as HTMLElement & {
      runtime?: { context: { gateway: { connect: () => void } } };
    };
    if (!app.runtime) {
      throw new Error("SteelEngine application runtime is unavailable");
    }
...
describeControlUiE2e("Control UI new-session page mocked Gateway E2E", () => {
  ...
  it("dispatches a cloud target before sending its first turn and shows placement", async () => {
    ...
    try {
        ...
        sessions: [
          {
            ...
            label: "Cloud session",
            updatedAt: Date.now(),
            worktree: { id: "worktree-1", branch: "steelengine/cloud-e2e", repoRoot: WORKSPACE },
      ...
      const stopWorker = page
        .locator("steelengine-session-menu")
    ...
    try {
      ...
      await page.evaluate(() => {
        ...
        Storage.prototype.setItem = function (key: string, value: string) {
          if (
            key.startsWith("steelengine.new-session.cloud-recovery.v1:") ||
            key.startsWith("steelengine.control-ui-e2e.")
          ) {
    ...
    try {
      ...
      await page.evaluate(({ gatewayUrl, recoveryScope }) => {
        sessionStorage.setItem(
          `steelengine.new-session.cloud-recovery.v1:${gatewayUrl}:${recoveryScope}`,
          JSON.stringify({
      ...
      await page.evaluate(() => {
        ...
        (
          document.querySelector("steelengine-new-session-page") as
    ...
    const readRecovery = () =>
      page.evaluate(() => {
        const key = Object.keys(sessionStorage).find((candidate) =>
          candidate.startsWith("steelengine.new-session.cloud-recovery.v1:"),
        );
  ...
  it("retries an unpersisted cloud turn with its original recovery identity", async () => {
    ...
    try {
      ...
      await page.evaluate(() => {
        const originalSetItem = sessionStorage.setItem.bind(sessionStorage);
        Storage.prototype.setItem = function (key: string, value: string) {
          if (key.startsWith("steelengine.new-session.cloud-recovery.v1:")) {
            originalSetItem(key, value);
            return;
          }
          throw new DOMException("composer storage disabled", "SecurityError");
  ...
  it("resets agent-derived workspace state when retargeted to a catalog", async () => {
    ...
    try {
      ...
      await expect
        .poll(() => page.locator(".new-session-page__runtime").textContent())
        .toContain("Claude Code");
      await expect.poll(() => folderLabel.textContent()).toBe("steelengine");
  ...
  it("adopts a created session when rejected-turn persistence exceeds browser storage", async () => {
    ...
    await page.addInitScript(() => {
      ...
      Storage.prototype.setItem = function (key: string, value: string) {
        if (key.startsWith("steelengine.control.chatComposer.v2:")) {
