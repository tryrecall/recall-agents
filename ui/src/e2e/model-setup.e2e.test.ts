...
describeControlUiE2e("Control UI Model Setup mocked Gateway E2E", () => {
  ...
  it("hands first-run model setup to the custodian in onboarding chrome", async () => {
    ...
    try {
      ...
      const detect = await gateway.waitForRequest("steelengine.setup.detect");
      expect(detect.params).toEqual({});
      const activate = await gateway.waitForRequest("steelengine.setup.activate");
      ...
      await page.getByRole("heading", { name: "SteelEngine", exact: true }).waitFor();
      await expect
        .poll(() => page.locator(".shell").getAttribute("class"))
        .toContain("shell--onboarding");
      expect(await page.locator(".shell-nav").isVisible()).toBe(false);

      const chatRequest = await gateway.waitForRequest("steelengine.chat");
  ...
  it("completes device-code sign-in and re-detects the configured model", async () => {
    ...
    const gateway = await installMockGateway(page, {
      ...
      methodResponses: {
        "steelengine.setup.detect": initialDetection,
        "steelengine.setup.auth.start": {
    ...
    try {
      ...
      const start = await gateway.waitForRequest("steelengine.setup.auth.start");
      ...
      await gateway.setMethodResponse("steelengine.setup.detect", {
        ...initialDetection,
        authOptions: [],
        configuredModel: "provider/verified-model",
        setupComplete: true,
      });
      const detectCountBeforeCompletion = (await gateway.getRequests("steelengine.setup.detect"))
        .length;
      ...
      await expect
        .poll(async () => (await gateway.getRequests("steelengine.setup.detect")).length)
  ...
  it("verifies the current model connection", async () => {
    ...
    try {
      ...
      const verify = await gateway.waitForRequest("steelengine.setup.verify");
      expect(verify.params).toEqual({});
      await page.getByText("Answered in 1234 ms").waitFor();
      const detectCountBeforeRefresh = (await gateway.getRequests("steelengine.setup.detect")).length;
