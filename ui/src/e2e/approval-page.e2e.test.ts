...
async function waitForApprovalPage(page: Page): Promise<void> {
  await page.locator("steelengine-approval-page").waitFor();
...
async function expectStandaloneApprovalPage(page: Page): Promise<void> {
  expect(await page.locator("steelengine-approval-page").count()).toBe(1);
  expect(await page.locator(".approval-page").count()).toBe(1);
  expect(await page.locator("steelengine-app-shell, .shell").count()).toBe(0);
  expect(await page.locator("steelengine-app-topbar, steelengine-app-sidebar").count()).toBe(0);
  expect(await page.locator("steelengine-exec-approval").count()).toBe(0);
}
...
describeControlUiE2e("Control UI standalone approval page", () => {
  ...
  it("keeps one canonical winner across conflicting surfaces and terminal reload", async () => {
    ...
    try {
      ...
      expect(await mobile.page.title()).toBe("Command approval — SteelEngine");
      ...
      expect(await mobile.page.title()).toBe("Approved here — SteelEngine");
      await waitForStableApprovalPaint(mobile.page);
      await mobile.page.screenshot({
      ...
      expect(await mobile.page.title()).toBe("Approved — SteelEngine");
      expect(new URL(mobile.page.url()).pathname).toBe(approvalPath(""));
  ...
  it("preserves a mounted deep link across the authentication gate", async () => {
    ...
    await surface.page.locator("steelengine-login-gate").waitFor();
  ...
  it("uses the page Gateway without replacing a saved remote selection", async () => {
    ...
    const selectionKey = `steelengine.control.currentGateway.v1:${pageGatewayScope}`;
    ...
    const remoteSettingsKey = `steelengine.control.settings.v1:${remoteGateway}`;
