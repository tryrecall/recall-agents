...
import type { RecallModalDialog } from "../ui/components/modal-dialog.ts";
...
export async function getRenderedModalDialog(container: HTMLElement) {
  const modal = container.querySelector<RecallModalDialog>("recall-modal-dialog");
  expect(modal).toBeInstanceOf(HTMLElement);
  if (!modal) {
    throw new Error("Expected recall-modal-dialog");
  }
