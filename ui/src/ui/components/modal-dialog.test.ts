...
import { type RecallModalDialog } from "./modal-dialog.ts";
...
function expectShadowElement(modal: RecallModalDialog, id: string): HTMLElement {
...
describe("recall-modal-dialog", () => {
  ...
  it("opens a labelled modal dialog with an optional description", async () => {
    ...
    expect(labelId).toBe("recall-modal-dialog-label");
    expect(descriptionId).toBe("recall-modal-dialog-description");
    expect(dialog.getRootNode()).toBe(modal.shadowRoot);
    expect(dialog.ownerDocument.querySelector(`#${labelId}`)).toBeNull();
    expect(expectShadowElement(modal, "recall-modal-dialog-label").textContent).toBe(
      "Confirm action",
    );
    expect(expectShadowElement(modal, "recall-modal-dialog-description").textContent).toBe(
