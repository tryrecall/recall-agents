...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/recall/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/recall");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/recall/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__RECALL_CONTROL_UI_BASE_PATH__ = "/recall";
    const app = mountApp("/recall/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/recall");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/recall/sessions");
  });
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      ...
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/recall" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("recall.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/recall",
    });
    expect(JSON.parse(localStorage.getItem("recall.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/recall";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/recall");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/recall#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/recall");
    expect(app.settings.token).toBe("");
