...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as RecallApp);
  }
  if (host.tab === "usage") {
    await loadUsage(host as unknown as RecallApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as RecallApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as RecallApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as RecallApp);
    await loadConfig(host as unknown as RecallApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as RecallApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as RecallApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as RecallApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as RecallApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as RecallApp);
    await loadDevices(host as unknown as RecallApp);
    await loadConfig(host as unknown as RecallApp);
    await loadExecApprovals(host as unknown as RecallApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as RecallApp);
    await loadConfig(host as unknown as RecallApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as RecallApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as RecallApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__RECALL_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as RecallApp;
...
async function loadOverviewLogs(host: RecallApp) {
...
function buildAttentionItems(host: RecallApp) {
  ...
  if (auth?.scopes && !hasOperatorReadAccess(auth)) {
    items.push({
      ...
      href: "https://docs.recall.ai/web/dashboard",
      external: true,
    });
  }
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as RecallApp, true),
    loadConfigSchema(host as unknown as RecallApp),
    loadConfig(host as unknown as RecallApp),
  ]);
}
