...
function applyNativeControlAuth(host: SettingsHost) {
  const nativeAuth = window["__RECALL_NATIVE_CONTROL_AUTH__"];
  if (!nativeAuth) {
    return;
  }
  try {
    delete window["__RECALL_NATIVE_CONTROL_AUTH__"];
  } catch {
    window["__RECALL_NATIVE_CONTROL_AUTH__"] = undefined;
  }
...
export function applySettingsFromUrl(host: SettingsHost) {
  ...
  if (hasTokenParam) {
    if (queryToken != null) {
      warnQueryToken = true;
      console.warn(
        "[recall] Auth token passed as query parameter (?token=). Use URL fragment instead: #token=<token>. Query parameters may appear in server logs.",
      );
    }
