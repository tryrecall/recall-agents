...
export function connectGateway(host: GatewayHost) {
  ...
  const client = new GatewayBrowserClient({
    ...
    onHello: (hello) => {
      ...
      void subscribeSessions(host as unknown as RecallApp);
      void loadAssistantIdentity(host as unknown as RecallApp);
      void loadAgents(host as unknown as RecallApp);
      void loadHealthState(host as unknown as RecallApp);
      void loadNodes(host as unknown as RecallApp, { quiet: true });
      void loadDevices(host as unknown as RecallApp, { quiet: true });
...
function handleTerminalChatEvent(
  ...
  if (runId && host.refreshSessionsAfterChat.has(runId)) {
    ...
    if (state === "final") {
      void loadSessions(host as unknown as RecallApp, {
  ...
  if (hadToolEvents && state === "final") {
    void loadChatHistory(host as unknown as RecallApp);
    return true;
  }
  return false;
}
...
function handleChatGatewayEvent(host: GatewayHost, payload: ChatEventPayload | undefined) {
  ...
  const state = handleChatEvent(host as unknown as RecallApp, payload);
