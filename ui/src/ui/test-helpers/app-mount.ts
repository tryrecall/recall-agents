...
const mountedApps = new Set<RecallApp>();

function collectMountedApps() {
  return new Set<RecallApp>([
    ...mountedApps,
    ...document.querySelectorAll<RecallApp>("recall-app"),
  ]);
}
...
async function waitForAppUpdates(apps: Iterable<RecallApp>) {
  for (const app of apps) {
    await app.updateComplete;
  }
}

async function drainAppWork(apps: Iterable<RecallApp>) {
  const snapshot = [...apps];
  await nextMicrotask();
