...
export async function handleWhatsAppStart(host: RecallApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: RecallApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: RecallApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: RecallApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: RecallApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: RecallApp): string {
...
function resolveGatewayHttpAuthHeader(host: RecallApp): string | null {
...
function buildGatewayHttpHeaders(host: RecallApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: RecallApp) {
...
export function handleNostrProfileToggleAdvanced(host: RecallApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: RecallApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: RecallApp) {
  const state = host.nostrProfileFormState;
