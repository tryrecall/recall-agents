  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __RECALL_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window["__RECALL_CONTROL_UI_BASE_PATH__"];
    return;
  }
  Object.defineProperty(window, "__RECALL_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", () => {
    ...
    setControlUiBasePath(" /recall/ ");

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/recall"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", () => {
    setTestLocation({
      ...
      pathname: "/apps/recall/chat",
    });

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/recall"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", () => {
    ...
    sessionStorage.setItem("recall.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "recall.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/recall",
    ...
    const settings = loadSettings();
    expect(settings.gatewayUrl).toBe("wss://gateway.example:8443/recall");
    expect(settings.token).toBe("");
    expect(settings.sessionKey).toBe("agent");
    const scopedKey = "recall.control.settings.v1:wss://gateway.example:8443/recall";
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      ...
      textScale: 100,
      sessionsByGateway: {
        "wss://gateway.example:8443/recall": {
          sessionKey: "agent",
  ...
  it("does not persist gateway tokens when saving settings", () => {
    ...
    const scopedKey = `recall.control.settings.v1:${gwUrl}`;
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      gatewayUrl: gwUrl,
      theme: "claw",
      themeMode: "system",
  ...
  it("normalizes persisted text scale to the nearest supported stop", () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    localStorage.setItem(
      `recall.control.settings.v1:${gwUrl}`,
      JSON.stringify({
        gatewayUrl: gwUrl,
        textScale: 123,
      }),
  ...
  it("caps persisted session scopes to the most recent gateways", () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    const scopedKey = `recall.control.settings.v1:wss://gateway.example:8443`;
  ...
  it("persists local user identity separately from gateway settings", () => {
    ...
    expect(JSON.parse(localStorage.getItem("recall.control.user.v1") ?? "{}")).toEqual({
      name: "Buns",
      avatar: "🦞",
    });
  });
  ...
  it("removes the persisted local user identity when cleared", () => {
    ...
    expect(localStorage.getItem("recall.control.user.v1")).toBeNull();
  });
});
