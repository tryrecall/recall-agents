...
describe("overview view rendering", () => {
  ...
  it("renders a dedicated scope-upgrade approval hint with the exact approve command", async () => {
    ...
    expect(compactText(hint)).toBe(
      "Scope upgrade pending approval. This device is already paired, but the requested wider scope is waiting for approval. recall devices approve req-123 recall devices list On mobile? Copy the full URL (including #token=...) from recall dashboard --no-open on your desktop. Docs: Device pairing",
    );
    expect([...container.querySelectorAll(".mono")].map((node) => node.textContent)).toEqual([
      "recall devices approve req-123",
      "recall devices list",
    ]);
  });
  ...
  it("does not suggest preview-only latest approval when the request id is absent", async () => {
    ...
    const hint = container.querySelector(".mono")?.closest(".muted") ?? null;
    expect(compactText(hint)).toBe(
      "Scope upgrade pending approval. This device is already paired, but the requested wider scope is waiting for approval. recall devices list On mobile? Copy the full URL (including #token=...) from recall dashboard --no-open on your desktop. Docs: Device pairing",
    );
    expect([...container.querySelectorAll(".mono")].map((node) => node.textContent)).toEqual([
      "recall devices list",
    ]);
