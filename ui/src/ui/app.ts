...
declare global {
  interface Window {
    __RECALL_CONTROL_UI_BASE_PATH__?: string;
  }
}
...
export class RecallApp extends LitElement {
  ...
  async toggleRealtimeTalk() {
    ...
    const session = new RealtimeTalkSession(
      ...
      this.sessionKey,
      {
        ...
        onTranscript: (entry) => {
          this.realtimeTalkTranscript = `${entry.role === "user" ? "You" : "Recall"}: ${entry.text}`;
...
if (!customElements.get("recall-app")) {
  customElements.define("recall-app", RecallApp);
}
