...
Security work is shared across a number of Recall maintainers, including engineers and security researchers from organizations such as NVIDIA and Tencent. See the [maintainer list](CONTRIBUTING.md#maintainers).

## Report a Security Issue

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [tryrecall/recall-agents](https://github.com/tryrecall/recall-agents)
- **macOS desktop app** — [tryrecall/recall-agents](https://github.com/tryrecall/recall-agents) (apps/macos)
- **iOS app** — [tryrecall/recall-agents](https://github.com/tryrecall/recall-agents) (apps/ios)
- **Android app** — [tryrecall/recall-agents](https://github.com/tryrecall/recall-agents) (apps/android)
- **ClawHub** — [recall/clawhub](https://github.com/recall/clawhub)
- **Trust and threat model** — [recall/trust](https://github.com/recall/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@recall.ai](mailto:security@recall.ai)** and we'll route it.

For Recall core issues, submit through a private [GitHub Security Advisory](https://github.com/tryrecall/recall-agents/security/advisories/new). Do not open a public issue or PR that discloses an unpatched vulnerability, exploit path, secret, or security-sensitive proof of concept.
...
For full reporting instructions see our [Trust page](https://trust.recall.ai).
For maintainer response workflow, see the [incident response plan](docs/security/incident-response.md).

Recall does not currently run a paid bug bounty program. Please still disclose responsibly so we can fix real issues quickly. The best way to help the project right now is to send high-signal reports and, when practical, focused PRs.
...
- The actual impact, including which Recall trust boundary is crossed.
- Any remediation advice or focused patch you can provide.

Reports without reproduction steps, demonstrated impact, and remediation advice are deprioritized. We receive a high volume of AI-generated scanner findings, so we prioritize vetted reports from researchers who can show how the issue crosses an Recall security boundary.
...
- Scanner-only, dependency-only, or stale-path reports without a working repro and demonstrated Recall impact.
...
- For dependency CVE reports, evidence that the shipped dependency version is actually affected, plus a PoC that reproduces impact through Recall. Showing that Recall can reach a native parser is not enough by itself.
- Demonstrated impact tied to Recall's documented trust boundaries.
- For exposed-secret reports: proof the credential is Recall-owned (or grants access to Recall-operated infrastructure/services).
...
- Reports that only show untrusted media bytes reaching a maintained native decoder dependency (for example Sharp/libvips/libheif) without proving the shipped dependency version is vulnerable and demonstrating crash, memory corruption, data exposure, or a boundary bypass through Recall. JavaScript header sniffing and image dimension fast-paths are preflight/UX checks, not the security boundary for native decoder correctness.
- Reports whose only impact is transient extra memory, CPU, or allocation work from decoding, base64 expansion, media transcoding, serialization, or other format conversion after the input was already accepted under Recall's configured size/trust limits, including base64 decode-before-size-estimate findings. These are performance issues, not vulnerabilities, unless the report demonstrates unauthenticated amplification, bypass of configured limits, crash/process termination, persistent resource exhaustion, data exposure, or another documented boundary bypass.
...
Recall does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Concretely, on the OpenAI-compatible HTTP surface:
  ...
  - narrower `x-recall-scopes` headers are ignored for that shared-secret path
...
- Recall can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Recall, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Recall's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

### Trusted Plugins

Plugins/extensions are part of Recall's trusted computing base for a gateway.
...
- Using Recall in ways that the docs recommend not to
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped Recall production code or a published package artifact intended for users.
...
- Reports that require write access to trusted local state (`~/.recall`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not Recall-owned and not granting access to Recall-operated infrastructure/services) without demonstrated Recall impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.
- SSRF reports limited to the operator-managed HTTP/WebSocket proxy-routing feature where the demonstrated mitigation is to enable/configure `proxy.enabled` with a filtering `proxy.proxyUrl`/`RECALL_PROXY_URL`, or where impact depends on a permissive/misconfigured operator proxy. This only covers normal process-local HTTP(S)/WebSocket egress (`fetch`, Node HTTP(S), and similar JavaScript clients); non-HTTP egress and other features are assessed separately. See [Network proxy](https://docs.recall.ai/security/network-proxy).

### Deployment Assumptions

Recall security guidance assumes:

- The host where Recall runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.recall` state/config (including `recall.json`) is effectively a trusted operator.
...
Recall's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Recall separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Recall can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the Recall process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

### Temp Folder Boundary

Recall uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/recall` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/recall` (or `recall-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the Recall-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Recall temp helpers (`resolvePreferredRecallTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-recall-dir.ts`
...
For threat model + hardening guidance (including `recall security audit --deep` and `--fix`), see:

- `https://docs.recall.ai/gateway/security`
...
Recall's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `recall gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - Recall keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `recall security audit` as dangerous findings.
...
Recall requires **Node.js 22.19.0 or later** (LTS). Node 24 is the recommended default runtime for new installs. The minimum version includes important security patches:
...
docker run --read-only --cap-drop=ALL \
  -v recall-data:/app/data \
  tryrecall/recall-agents:latest
```

## Security Scanning

Recall uses several security and release-validation layers. No single scanner is treated as the boundary.

### Secret Detection

Recall runs the pre-commit `detect-private-key` hook in CI and keeps secret-resolution behavior covered by the dedicated secrets test surface.
