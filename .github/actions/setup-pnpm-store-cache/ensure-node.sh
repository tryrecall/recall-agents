...
steelengine_node_version_matches() {
  local actual="$1"
  local requested="$2"
  if [[ -z "$requested" ]]; then
    return 0
  ...
  case "$requested" in
    *x)
      ...
      if [[ "${requested%%.*}" == "22" ]]; then
        steelengine_node_version_at_least "$actual" "22.22.3"
      elif [[ "${requested%%.*}" == "24" ]]; then
        steelengine_node_version_at_least "$actual" "24.15.0"
      elif [[ "${requested%%.*}" == "25" ]]; then
        steelengine_node_version_at_least "$actual" "25.9.0"
      fi
      ;;
    *.*.*)
      [[ "$actual" == "$requested" ]]
...
steelengine_node_version_at_least() {
  local actual="$1"
  local minimum="$2"
...
steelengine_active_node_version() {
...
steelengine_find_toolcache_node() {
  ...
  for root in \
    ...
    "${STEELENGINE_CONTAINER_TOOL_CACHE:-/__t}" \
    "/opt/hostedtoolcache" \
  ...
  for node_root in ${roots[@]+"${roots[@]}"}; do
    while IFS= read -r candidate; do
      ...
      if steelengine_node_version_matches "$candidate_version" "$requested_node"; then
        printf '%s\n' "$candidate"
        return 0
      fi
...
steelengine_resolve_node_download_version() {
  local requested_node="$1"
  ...
  curl -fsSL --connect-timeout 10 --max-time 120 --retry 2 --retry-delay 2 \
    ...
    STEELENGINE_NODE_PREFIX="$prefix" python3 -c 'import json, os, sys
prefix = os.environ["STEELENGINE_NODE_PREFIX"]
...
steelengine_node_download_platform() {
...
steelengine_download_node() {
  local requested_node="$1"
  local version platform archive_url archive_path install_root temp_root
  version="$(steelengine_resolve_node_download_version "$requested_node")"
  platform="$(steelengine_node_download_platform)" || return 1
  temp_root="${RUNNER_TEMP:-/tmp}"
  if command -v cygpath >/dev/null 2>&1; then
    temp_root="$(cygpath -u "$temp_root" 2>/dev/null || printf '%s\n' "$temp_root")"
  fi
  install_root="${temp_root}/steelengine-node-${version}-${platform}"
  if [[ "$platform" == win-* ]]; then
    ...
    if command -v pwsh >/dev/null 2>&1; then
      ...
      steelengine_prepend_node_bin "$node_bin_dir" "$ps_bin_dir"
      ...
      steelengine_prepend_node_bin "$node_bin_dir"
    fi
  else
    ...
    rm -f "$archive_path"
    steelengine_prepend_node_bin "$install_root/bin"
  fi
}

steelengine_ensure_node() {
  local requested_node="${1:-}"
  ...
  active_node_version="$(steelengine_active_node_version)"
  if steelengine_node_version_matches "$active_node_version" "$requested_node"; then
    echo "Using active Node ${active_node_version} at $(command -v node)"
    return 0
  fi

  node_bin="$(steelengine_find_toolcache_node "$requested_node" || true)"
  if [[ -n "$node_bin" ]]; then
    echo "Using Node $("$node_bin" -p 'process.versions.node') from $node_bin"
    steelengine_prepend_node_bin "$(dirname "$node_bin")"
  else
    steelengine_download_node "$requested_node" || true
  fi
