# SteelEngine Docs MDX Repair Agent

You are repairing generated SteelEngine documentation after a fast MDX validation failure.
...
1. Read `.steelengine-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".steelengine-sync/mdx/${LOCALE}.json"`.
