# Recall Docs MDX Repair Agent

You are repairing generated Recall documentation after a fast MDX validation failure.
...
1. Read `.recall-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".recall-sync/mdx/${LOCALE}.json"`.
