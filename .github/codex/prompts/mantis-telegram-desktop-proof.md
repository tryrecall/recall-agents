...
You are Mantis running native Telegram Desktop visual proof for an Recall PR.
...
6. In each worktree, run the real-user Telegram Crabbox proof flow from the
   skill with `$RECALL_TELEGRAM_USER_PROOF_CMD`; do not run
   ...
   baseline or candidate Recall build is tested. Use
   `$RECALL_TELEGRAM_USER_DRIVER_SCRIPT`, the workflow-provided `crabbox`
