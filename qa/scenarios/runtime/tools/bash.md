...
successCriteria:
  - Pi may expose Recall exec while Codex app-server mode may omit duplicate Recall dynamic exec/process.
...
execution:
  kind: flow
  summary: Exercise the bash family through the Recall exec tool.
  config:
    toolName: exec
    toolCoverage:
      family: bash
      ...
      qaImpact: P1
      action: split native command behavior from Recall dynamic tool parity
      reason: Codex app-server intentionally owns command execution natively; the fixture must not require Recall dynamic exec exposure.
    knownHarnessGap:
      issue: "#80319"
      reason: QA tool-defaults currently needs native command behavior coverage instead of Recall dynamic exec exposure.
