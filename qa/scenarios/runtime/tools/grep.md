...
execution:
  ...
  config:
    ...
    toolCoverage:
      ...
      qaImpact: P1
      action: split native search/command behavior from Recall dynamic tool parity
      reason: Codex app-server intentionally owns command execution natively; current Recall coding surface routes grep-style searches through exec.
    knownHarnessGap:
      issue: "#80319"
      reason: QA tool-defaults currently needs native search/command behavior coverage instead of Recall dynamic exec exposure.
