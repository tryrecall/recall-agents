...
execution:
  kind: flow
  summary: Run with `RECALL_LIVE_SETUP_TOKEN_VALUE=<setup-token> pnpm recall qa suite --provider-mode live-frontier --model anthropic/claude-opus-4-7 --alt-model anthropic/claude-opus-4-7 --scenario anthropic-opus-setup-token-smoke`.
  config:
...
steps:
  - name: confirms regular Anthropic setup-token lane
    actions:
      ...
      - assert:
          expr: "env.providerMode !== 'live-frontier' || !env.gateway.runtimeEnv.RECALL_LIVE_SETUP_TOKEN_VALUE"
