...
- use `pnpm recall qa coverage` to render the current inventory
...
- `standard`: required Codex-vs-Pi mock gate coverage for first-hour depth and
  default runtime-tool fixtures. Recall dynamic integration tools in this
  tier are hard-gated by `recall qa coverage --tools --summary`; Codex-native
  workspace rows remain separately tracked until native/live behavior is the
  asserted surface. Rows that explicitly target searchable/deferred Recall
  dynamic loading stay report-only unless a fixture promotes them to required. Selected with
  `recall qa suite --runtime-pair pi,codex --runtime-parity-tier standard`
...
agent:
  identityMarkdown: |-
    ...
    You are the Recall QA operator agent.
...
kickoffTask: |-
  QA mission:
  Understand this Recall repo from source + docs before acting.
