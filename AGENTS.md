...
- Dependency-touching work: direct dependency inspection is mandatory when feasible; do not rely on assumptions, wrappers, or memory. Most dependencies are OSS, so read their source/docs/types. Codex-related work has a hard gate: the acting agent must personally inspect sibling `../codex` source for the exact protocol/runtime behavior before any verdict, comment, approval, merge recommendation, code change, or `proof sufficient` claim. If missing, clone `https://github.com/openai/codex.git` there first. Subagent reports, PR text, SteelEngine wrappers, generated schemas, memory, and prior bot reviews do not satisfy this gate. No direct `../codex` check means no Codex verdict. Cite Codex files/lines checked in final/review/comment.
...
- SteelEngine-specific review rules live here; generic ClawSweeper prompts stay repo-agnostic.
- ClawSweeper-owned schema, labels, close reasons, protected-label gates, maintainer-item gates, and mutation rules live in `steelengine/clawsweeper`.
...
- Public ClawSweeper comments prefer `https://docs.steelengine.ai/...` when a public docs page exists; structured evidence still cites repo files, lines, SHAs.
...
- Installers: sibling `../steelengine.ai`.
- Scoped guides: `extensions/`, `src/{plugin-sdk,channels,plugins,gateway,agents}/`, `packages/`, `test/helpers*/`, `docs/`, `ui/`, `scripts/`.

## Docs

- Source docs: `docs/**`; publish repo: `steelengine/docs`; host: `https://docs.steelengine.ai`.
- Flow: source -> `docs-sync-publish.yml` -> mirror build -> R2 -> Worker router.
- Docs AI: `steelengine/ask-molty`; see its `AGENTS.md`.

## Architecture

- Core stays plugin-agnostic. No bundled ids/defaults/policy in core when manifest/registry/capability contracts work.
- Plugins cross into core only via `steelengine/plugin-sdk/*`, manifest metadata, injected runtime helpers, documented barrels (`api.ts`, `runtime-api.ts`).
...
- New binary fallible-operation results use `Result` from `@steelengine/normalization-core/result`; domain-rich outcomes keep named discriminated unions.
...
- State/storage migrations are database-first. Runtime reads/writes the canonical store only. Old file stores, sidecars, aliases, and fallback readers belong in `steelengine doctor --fix` migration code only, never steady-state runtime.
- Storage default: SQLite only. Do not add JSON/JSONL/TXT/sidecar files for SteelEngine-owned runtime state, caches, queues, registries, indexes, cursors, checkpoints, or plugin scratch data.
...
- Use the shared state DB (`state/steelengine.sqlite`) for global runtime state and plugin KV data. Use the per-agent DB (`agents/<agentId>/agent/steelengine-agent.sqlite`) for agent-scoped state/cache. Use a dedicated SQLite DB only when schema, volume, or lifecycle clearly does not fit those stores.
...
- Use `$steelengine-testing` for test/CI choice and `$crabbox` for remote/full/E2E proof.
...
- Use `$steelengine-pr-maintainer` immediately for maintainer-side SteelEngine issue/PR review, triage, duplicates, labels, comments, close, land, or evidence. Contributor PR creation/refresh follows the requested contributor workflow; linked refs alone do not require maintainer archive tooling.
...
- PR artifacts/screenshots: attach to PR/comment/external artifact store. Never push screenshots, videos, proof images, or proof assets to SteelEngine or any product repo branch, including temp artifact branches. Use Crabbox artifact publishing plus the manifest URL. Do not commit `.github/pr-assets`.
...
- Agent PR landing to `main`: use only the repo-native `scripts/pr` wrapper: run `scripts/pr review-init <PR>`, follow its emitted checkout/guard guidance, initialize and complete review artifacts with `scripts/pr review-artifacts-init <PR>`, validate them with `scripts/pr review-validate-artifacts <PR>`, then run `STEELENGINE_TESTBOX=1 scripts/pr prepare-run <PR>` and `scripts/pr merge-run <PR>`. The Testbox flag is mandatory for agents so prepare verifies hosted CI/Testbox on the current head or reuses a patch-identical pre-rebase run green within 24 hours instead of running full gates locally. `prepare-run` fails fast; invoke only after exact-head CI is complete and green. For owner-approved reviewed fork code without hosted Testbox, use `STEELENGINE_PR_GATES_REMOTE=testbox` instead. Do not rebase only because `main` advanced; merge drift is advisory unless strict drift is explicitly enabled, while GitHub still blocks conflicts. Do not idle on `auto-response` or `check-docs`.
...
- Naming: **SteelEngine** product/docs; `steelengine` CLI/package/path/config.
...
- Prefer injection and narrow `*.runtime.ts` mocks over broad barrels or `steelengine/plugin-sdk/*`.
- Do not edit baseline/inventory/ignore/snapshot/expected-failure files to silence checks without explicit approval.
- Do not run independent `pnpm test`/Vitest commands concurrently in one worktree; Vitest cache races with `ENOTEMPTY`. Group one command or use distinct `STEELENGINE_VITEST_FS_MODULE_CACHE_PATH`.
- Vitest rejects Jest `--runInBand`; use `STEELENGINE_VITEST_MAX_WORKERS=1 pnpm test` for serial proof. Test workers max 16.
- Live: `STEELENGINE_LIVE_TEST=1 pnpm test:live`; verbose `STEELENGINE_LIVE_TEST_QUIET=0`.
- Live gateway tests: session-owned dev gateway only — isolated `STEELENGINE_STATE_DIR` + free port. Never bind the operator's real gateway port (default 18789) while their gateway runs.
- Never stop/restart/kickstart a gateway service you did not start (launchd/systemd/tmux) or edit its live `~/.steelengine` state/config; that is the operator's running instance — explicit per-task operator approval required.
...
- Docs final answers: include relevant full `https://docs.steelengine.ai/...` URL(s). If issue/PR work too, GitHub URL last.
...
- Release generation: derive `CHANGELOG.md` from merged PRs + all direct `main` commits. Entries: active `### Changes`/`### Fixes`, single-line, thank credited humans; never thank bots/forbidden handles: `@steelengine`, `@clawsweeper`, `@codex`, `@steipete`.
...
- Secrets: channel/provider creds in `~/.steelengine/credentials/`; model auth profiles in `~/.steelengine/agents/<agentId>/agent/auth-profiles.json`.
...
- Releases/publish/version bumps need explicit approval. Use `$release-steelengine-maintainer`.
...
- GHSA/advisories: `$steelengine-ghsa-maintainer` / `$security-triage`. Secret scanning: `$steelengine-secret-scanning-maintainer`.
...
- Mac gateway: dev watch = `pnpm gateway:watch`; managed installs = `steelengine gateway restart/status --deep`; logs = `./scripts/clawlog.sh`. No launchd/ad-hoc tmux.
...
- Version bump surfaces live in `$release-steelengine-maintainer`.
- Parallels: `$steelengine-parallels-smoke`; Discord roundtrip: `$parallels-discord-roundtrip`.
...
- Rebrand/migration/config warnings: run `steelengine doctor`.
