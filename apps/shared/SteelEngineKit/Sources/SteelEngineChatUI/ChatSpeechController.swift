...
public struct SteelEngineChatSpeechClip: Equatable, Sendable {
...
public typealias SteelEngineChatSpeechSynthesis =
    @Sendable (_ text: String) async throws -> SteelEngineChatSpeechClip
...
@MainActor
@Observable
public final class SteelEngineChatSpeechController {
    ...
    private let synthesize: SteelEngineChatSpeechSynthesis
    ...
    public convenience init(synthesize: @escaping SteelEngineChatSpeechSynthesis) {
        self.init(
    ...
    init(
        synthesize: @escaping SteelEngineChatSpeechSynthesis,
    ...
    private func run(messageID: UUID, text: String, generation: UInt64) async {
        let clip: SteelEngineChatSpeechClip? = await {
            do {
