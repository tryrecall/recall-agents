...
public enum SteelEngineQuestionCardStatus: Sendable, Equatable {
    case pending
    case submitting
    case answered
...
@MainActor
@Observable
public final class SteelEngineQuestionCardModel: Identifiable {
    ...
    public func status(at date: Date = Date()) -> SteelEngineQuestionCardStatus {
    ...
    public func apply(resolved: SteelEngineQuestionResolvedEvent, at date: Date = Date()) {
...
struct SteelEngineQuestionCard: View {
    @Bindable private var model: SteelEngineQuestionCardModel
    private let onSubmit: @MainActor @Sendable (SteelEngineQuestionCardModel) async -> Void

    init(
        model: SteelEngineQuestionCardModel,
        onSubmit: @escaping @MainActor @Sendable (SteelEngineQuestionCardModel) async -> Void)
    {
        self.model = model
    ...
    var body: some View {
        TimelineView(.periodic(from: .now, by: 1)) { context in
            ...
            .padding(16)
            .background(SteelEngineChatTheme.subtleCard, in: RoundedRectangle(cornerRadius: 16))
    ...
    private func questionSection(_ question: Question, now: Date) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(question.header.uppercased())
                ...
                .foregroundStyle(SteelEngineChatTheme.accent)
        ...
        return Button {
            ...
            HStack(alignment: .top, spacing: 10) {
                Image(systemName: selected
                    ...
                    .foregroundStyle(selected ? SteelEngineChatTheme.accent : .secondary)
                VStack(alignment: .leading, spacing: 2) {
                    Text(option.label).font(SteelEngineChatTypography.body)
    ...
    private func footer(now: Date) -> some View {
        ...
        if status == .pending || status == .submitting {
            HStack {
                ...
                .font(SteelEngineChatTypography.body(size: 14, weight: .semibold, relativeTo: .callout))
            ...
            if let errorText = self.model.errorText {
                Text(errorText)
                    .font(SteelEngineChatTypography.caption)
                    .foregroundStyle(SteelEngineChatTheme.danger)
            }
        } else {
    ...
    private func terminalText(_ status: SteelEngineQuestionCardStatus) -> String {
        switch status {
    ...
    private func terminalIcon(_ status: SteelEngineQuestionCardStatus) -> String {
        switch status {
...
@MainActor
struct SteelEngineQuestionCards: View {
    let viewModel: SteelEngineChatViewModel

    var body: some View {
        ForEach(self.viewModel.visibleQuestionCards) { card in
            SteelEngineQuestionCard(model: card) { [weak viewModel = self.viewModel] model in
...
extension SteelEngineChatViewModel {
    public var visibleQuestionCards: [SteelEngineQuestionCardModel] {
    ...
    func refreshQuestions() async {
        ...
        do {
            ...
            self.questionCards = records.map { record in
                ...
                changed = true
                return SteelEngineQuestionCardModel(record: record)
            } + retainedTerminal
    ...
    func upsertQuestion(_ record: QuestionRecord) {
        if let model = self.questionCards.first(where: { $0.id == record.id }) {
            ...
            self.questionCards.append(SteelEngineQuestionCardModel(record: record))
        }
        self.questionStateRevision &+= 1
        self.syncQuestionEvictions()
        self.markTimelineChanged()
    ...
    func resolveQuestionEvent(_ event: SteelEngineQuestionResolvedEvent) {
    ...
    func submitQuestion(_ model: SteelEngineQuestionCardModel) async {
    ...
    func evictQuestionIfTerminalGraceElapsed(
        _ model: SteelEngineQuestionCardModel,
        at date: Date = Date())
    {
