...
private let chatUILogger = Logger(subsystem: "ai.steelengine", category: "SteelEngineChatUI")

@MainActor
@Observable
public final class SteelEngineChatViewModel {
    ...
    public internal(set) var messages: [SteelEngineChatMessage] = []
    ...
    public internal(set) var replyTarget: SteelEngineChatReplyTarget?
    @ObservationIgnored
    ...
    /// Setter is module-internal for the thinking-level extension only.
    public internal(set) var thinkingLevelOptions: [SteelEngineChatThinkingLevelOption]
    /// Setter is module-internal for the thinking-level extension only.
    ...
    public private(set) var modelChoices: [SteelEngineChatModelChoice] = []
    ...
    public internal(set) var slashCommands: [SteelEngineChatCommandChoice] = []
    ...
    struct SlashFilterCache {
        let query: String
        let filter: SteelEngineChatCommandFilter
        let result: [SteelEngineChatCommandChoice]
    }
    ...
    public var attachments: [SteelEnginePendingAttachment] = []
    ...
    public internal(set) var questionCards: [SteelEngineQuestionCardModel] = []
    ...
    public private(set) var pendingToolCalls: [SteelEngineChatPendingToolCall] = []
    public internal(set) var planSteps: [SteelEngineChatPlanStep] = []
    ...
    public internal(set) var sessions: [SteelEngineChatSessionEntry] = [] {
        didSet { syncContextUsageFraction() }
    }
    ...
    let transport: any SteelEngineChatTransport
    let haptics: SteelEngineChatHaptics
    let transcriptCache: (any SteelEngineChatTranscriptCache)?
    let outbox: (any SteelEngineChatCommandOutbox)?
    @ObservationIgnored
    ...
    public internal(set) var outboxStatesByMessageID: [UUID: SteelEngineChatOutboxMessageState] = [:]
    @ObservationIgnored
    ...
    var sessionDefaults: SteelEngineChatSessionsDefaults? {
        didSet { syncContextUsageFraction() }
    }
    ...
    var lastSuccessfulSettingsPatchResultsByTarget: [ModelPatchTarget: SteelEngineChatModelPatchResult] = [:]
    ...
    var acceptedSettingsPatchResultsByTarget: [ModelPatchTarget: SteelEngineChatModelPatchResult] = [:]
    ...
    struct FastModeState {
        let override: SteelEngineChatFastMode?
        let effective: SteelEngineChatFastMode?
    }
    ...
    private struct SettingsPatchTail {
        ...
        let routeLeaseTask: Task<SteelEngineChatSessionSettingsRouteLease?, Never>
    ...
    struct HistoryRequest {
        ...
        var visibleMessagesByID: [UUID: SteelEngineChatMessage]
    ...
    var pendingToolCallsById: [String: SteelEngineChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        ...
        haptics: SteelEngineChatHaptics = SteelEngineChatHaptics(),
    ...
    public func isDefaultModel(_ model: SteelEngineChatModelChoice) -> Bool {
    ...
    private var usesMutableAgentRouting: Bool {
        SteelEngineChatSessionKey.agentID(from: self.sessionKey) == nil
    }
    ...
    func usesMutableContractRouting(sessionKey: String, contract: String?) -> Bool {
        ...
        let contractMainKey = SteelEngineChatSessionRoutingContract.parse(contract)?.mainKey ?? ""
    ...
    static let baseThinkingLevelOptions: [SteelEngineChatThinkingLevelOption] = [
        SteelEngineChatThinkingLevelOption(id: "off", label: "off"),
        SteelEngineChatThinkingLevelOption(id: "minimal", label: "minimal"),
    ...
    public func removeAttachment(_ id: SteelEnginePendingAttachment.ID) {
...
extension SteelEngineChatViewModel {
    // MARK: - Internals
    ...
    func appendMessage(_ message: SteelEngineChatMessage) {
    ...
    func currentSessionSnapshot() -> SessionSnapshot {
        SessionSnapshot(
            ...
            deliveryAgentID: SteelEngineChatSessionKey.agentID(from: self.sessionKey) ?? self.activeAgentId,
    ...
    func fetchSessions(limit: Int?, sessionSnapshot: SessionSnapshot? = nil) async {
        ...
        while true {
            ...
            let res: SteelEngineChatSessionsListResponse
            do {
            ...
            let organized = SteelEngineChatSessionListOrganizer.organize(res.sessions)
    ...
    private static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "SteelEngineChatTransport"
    ...
    private func performSelectModel(
        ...
        routeLease: SteelEngineChatSessionSettingsRouteLease?) async
    {
        do {
            guard let routeLease else { throw SteelEngineChatTransportSendError.notDispatched }
            let patchResult = try await routeLease.patchSessionSettings(
                ...
                patch: SteelEngineChatSessionSettingsPatch(model: .some(request.modelRef)))
    ...
    func enqueueSessionSettingsPatch(
        ...
        operation: @escaping @MainActor (SteelEngineChatSessionSettingsRouteLease?) async -> Void)
    {
    ...
    func placeholderSession(key: String) -> SteelEngineChatSessionEntry {
    ...
    private func generatedNewSessionKey() -> String {
        ...
        guard let agentID = SteelEngineChatSessionKey.agentID(from: sessionKey) ??
            activeAgentId ??
            SteelEngineChatSessionKey.agentID(from: resolvedMainSessionKey) ??
            sessions.lazy.compactMap({ SteelEngineChatSessionKey.agentID(from: $0.key) }).first
        else {
            return baseKey
        }
    ...
    private func applySuccessfulModelSelection(
        ...
        syncSelection: Bool,
        patchResult: SteelEngineChatModelPatchResult? = nil)
    {
        self.recordSuccessfulModelPatch(
    ...
    private func recordSuccessfulModelPatch(
        selectionID: String,
        patchResult: SteelEngineChatModelPatchResult?,
        target: ModelPatchTarget)
    {
        ...
        if let patchResult {
            ...
            self.acceptedSettingsPatchResultsByTarget[target] = SteelEngineChatModelPatchResult(
