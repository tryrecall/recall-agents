...
private let transportEventsLogger = Logger(subsystem: "ai.steelengine", category: "SteelEngineChatUI")

@MainActor
private final class PendingRunOwnerReference {
    weak var value: SteelEngineChatViewModel?

    init(_ value: SteelEngineChatViewModel) {
        self.value = value
    }
}
...
extension SteelEngineChatViewModel {
    ...
    func handleTransportEvent(_ evt: SteelEngineChatTransportEvent) {
        switch evt {
        case let .health(ok):
    ...
    private func handleSessionMessageEvent(_ payload: SteelEngineSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: SteelEngineChatEventPayload) {
           ...
           let runId = Self.normalizedRunID(chat.runId)
        {
            ...
            self.adoptRun(
                runId: runId,
                bufferedText: SteelEngineChatEventText.assistantText(from: chat) ?? "")
            return
        }
        ...
        case "final", "aborted", "error":
            ...
            let hapticEvent: SteelEngineChatHaptics.Event? = switch chat.state {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: SteelEngineChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = SteelEngineChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: SteelEngineChatMessage.self)
        }
    ...
    static func isAssistantMessage(_ message: SteelEngineChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: SteelEngineChatMessage) -> SteelEngineChatMessage {
        guard message.timestamp == nil else { return message }
        return SteelEngineChatMessage(
            id: message.id,
            role: message.role,
    ...
    private func handleAgentEvent(_ evt: SteelEngineAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SteelEngineChatPendingToolCall(
    ...
    private func handleAgentLifecycleEvent(_ evt: SteelEngineAgentEventPayload, isPendingRun: Bool) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: SteelEngineAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
    ...
    func finishPendingRunAfterTerminalOkSendAck(_ response: SteelEngineChatSendResponse) {
    ...
    func finishPendingRunIfTerminalSendAck(_ response: SteelEngineChatSendResponse) -> Bool {
    ...
    private func refreshIfPending(
        ...
        terminalState: SteelEngineChatRunTerminalState? = nil,
    ...
    private func finishPendingRun(runId: String, terminalState: SteelEngineChatRunTerminalState) {
        let hapticEvent: SteelEngineChatHaptics.Event
    ...
    static func hasUnansweredLatestUser(in messages: [SteelEngineChatMessage]) -> Bool {
        self.latestUserTurn(in: messages) != nil && !self.hasAssistantMessageAfterLatestUser(in: messages)
    }

    static func latestUserTurn(in messages: [SteelEngineChatMessage]) -> LatestUserTurn? {
    ...
    static func userTurn(
        at userIndex: [SteelEngineChatMessage].Index,
        in messages: [SteelEngineChatMessage]) -> LatestUserTurn?
    {
    ...
    static func hasAnsweredUser(
        ...
        in messages: [SteelEngineChatMessage])
        -> Bool
    {
        ...
        var occurrence = 0
        var latestMatchingUserIndex: [SteelEngineChatMessage].Index?
    ...
    private static func hasAssistantMessage(
        after userIndex: [SteelEngineChatMessage].Index,
        in messages: [SteelEngineChatMessage]) -> Bool
    {
    ...
    private static func hasAssistantMessageAfterLatestUser(in messages: [SteelEngineChatMessage]) -> Bool {
        guard let lastUserIndex = messages.lastIndex(where: { $0.role.lowercased() == "user" }) else {
            return false
        }
    ...
    private static func assistantHapticEvent(
        for message: SteelEngineChatMessage) -> SteelEngineChatHaptics.Event?
    {
    ...
    func assistantHapticEventAfterLatestUser() -> SteelEngineChatHaptics.Event? {
    ...
    private func assistantHapticEvent(after timestamp: Double) -> SteelEngineChatHaptics.Event? {
    ...
    func refreshHistoriesAfterOutboxFlush(
        ...
        routeLease: SteelEngineChatTransportRouteLease) async
    {
    ...
    private nonisolated static func runPendingRunOwner(
        ...
        armID: UInt64,
        transport: any SteelEngineChatTransport) async
    {
    ...
    private nonisolated static func observePendingRunCompletion(
        ...
        transport: any SteelEngineChatTransport) async
    {
        var terminalState: SteelEngineChatRunTerminalState?
            ...
            armID: armID)
        {
            ...
            let effectiveObservation = terminalState.map(SteelEngineChatRunObservation.terminal) ?? observation
    ...
    private static func processPendingRunObservation(
        ...
        userMessageTimestamp: Double?,
        armID: UInt64,
        observation: SteelEngineChatRunObservation,
    ...
    func clearPendingRun(
        _ runId: String,
        hapticEvent: SteelEngineChatHaptics.Event? = nil)
    {
