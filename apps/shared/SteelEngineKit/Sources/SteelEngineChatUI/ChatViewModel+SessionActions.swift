...
private let chatSessionActionsLogger = Logger(
    subsystem: "ai.steelengine",
    category: "SteelEngineChat")
...
extension SteelEngineChatViewModel {
    ...
    public func fetchSessionList(search: String?, archived: Bool) async -> [SteelEngineChatSessionEntry] {
        ...
        do {
            let res = try await self.transport.listSessions(
                ...
                search: query,
                archived: archived)
            return SteelEngineChatSessionListOrganizer.organize(res.sessions)
        } catch {
            ...
            return SteelEngineChatSessionListOrganizer.filter(self.sessions, search: query)
        }
    }
    ...
    public func setSessionPinned(key: String, pinned: Bool) {
        ...
        if let index = self.sessions.firstIndex(where: { $0.key == key }) {
            ...
            self.sessions = SteelEngineChatSessionListOrganizer.organize(self.sessions)
        }
        Task {
            do {
                try await self.transport.patchSession(
    ...
    func markCurrentSessionReadAfterActivation(
        ...
        fallbackEntry: SteelEngineChatSessionEntry?) async
    {
