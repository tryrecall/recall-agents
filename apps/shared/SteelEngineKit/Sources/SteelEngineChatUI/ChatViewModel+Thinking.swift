...
extension SteelEngineChatViewModel {
    ...
    func performSelectThinkingLevel(_ level: String) {
        ...
        if acceptedThinkingLevelsByTarget[target] == nil {
            ...
            acceptedSettingsPatchResultsByTarget[target] = SteelEngineChatModelPatchResult(
    ...
    private func mergedThinkingPatchSuccess(
        _ patchResult: SteelEngineChatModelPatchResult?,
        acceptedLevel: String,
        target: ModelPatchTarget) -> SteelEngineChatModelPatchResult
    {
    ...
    func updateCurrentSessionThinkingLevels(
        _ thinkingLevels: [SteelEngineChatThinkingLevelOption],
        sessionKey: String,
    ...
    func inactiveSettingsStateKey(for target: ModelPatchTarget) -> String? {
        if target.agentID != nil || target.sessionRoutingContract != nil {
            guard SteelEngineChatSessionKey.agentID(from: target.canonicalSessionKey) != nil else { return nil }
        }
    ...
    private func thinkingLevelWithoutGatewayMetadata(
        _ level: String,
        target: ModelPatchTarget,
        session: SteelEngineChatSessionEntry?) -> String?
    {
    ...
    private func thinkingPickerIsAvailable(
        for session: SteelEngineChatSessionEntry?,
        modelChoice: SteelEngineChatModelChoice?) -> Bool
    {
        let resolved = self.resolvedThinkingLevelOptions(for: session)
    ...
    private struct ThinkingLevelOptionsResolution {
        let options: [SteelEngineChatThinkingLevelOption]
        let isGatewayMetadata: Bool
    }

    private func resolvedThinkingLevelOptions(
        for currentSession: SteelEngineChatSessionEntry?) -> ThinkingLevelOptionsResolution
    {
    ...
    private func selectedModelChoice(
        for currentSession: SteelEngineChatSessionEntry?) -> SteelEngineChatModelChoice?
    {
    ...
    private func modelChoice(modelID: String?, provider: String?) -> SteelEngineChatModelChoice? {
    ...
    private static func sessionModelMatchesDefaults(
        ...
        defaults: SteelEngineChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [SteelEngineChatThinkingLevelOption]?) -> [SteelEngineChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return SteelEngineChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [SteelEngineChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return SteelEngineChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    static func withCurrentThinkingOption(
        _ options: [SteelEngineChatThinkingLevelOption],
        current: String) -> [SteelEngineChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [SteelEngineChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [SteelEngineChatThinkingLevelOption]) -> [SteelEngineChatThinkingLevelOption]
    {
        var result: [SteelEngineChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    static func normalizedThinkingLevel(
        _ level: String?,
        options: [SteelEngineChatThinkingLevelOption],
