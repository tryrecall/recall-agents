...
struct ChatSessionSidebar: View {
    ...
    @State private var sessionPendingDeletion: SteelEngineChatSessionEntry?
    @State private var sessionPendingRename: SteelEngineChatSessionEntry?
        ...
        return HStack(spacing: 6) {
            VStack(alignment: .leading, spacing: 2) {
                Text(ChatSessionSidebarModel.displayName(for: session))
                    .font(SteelEngineChatTypography.body(size: 13, weight: .medium, relativeTo: .body))
                    .lineLimit(1)
    ...
    private func badges(for node: ChatSessionSidebarModel.Node) -> some View {
        ...
        if node.badges.failedCount > 0 {
            Image(systemName: "exclamationmark.triangle.fill")
                .foregroundStyle(SteelEngineChatTheme.warning)
    ...
    @ViewBuilder
    private func contextMenu(for session: SteelEngineChatSessionEntry) -> some View {
        Button {
    ...
    private func rowSubtitle(for session: SteelEngineChatSessionEntry) -> String? {
        var parts: [String] = []
