...
struct ChatMathBlockView: View {
    ...
    @ScaledMetric(relativeTo: .body) private var fontSize: CGFloat = SteelEngineChatTypography.bodySize
...
struct ChatMarkdownTableView: View {
    ...
    private func cell(_ text: String, isHeader: Bool) -> some View {
        Text(self.inlineMarkdown(text))
            .font(isHeader ? SteelEngineChatTypography.footnoteSemiBold : SteelEngineChatTypography.footnote)
            .foregroundStyle(SteelEngineChatTheme.assistantText)
            .textSelection(.enabled)
    }
