...
public enum SteelEngineChatSessionKey {
...
public enum SteelEngineChatGatewayPayloadCodec {
    ...
    public static func decodeAgentWaitObservation(_ data: Data) throws -> SteelEngineChatRunObservation {
        let decoded = try JSONDecoder().decode(AgentWaitResponse.self, from: data)
        return SteelEngineChatRunObservation.fromWaitResponse(
    ...
    public static func decodeModelChoices(_ data: Data) throws -> [SteelEngineChatModelChoice] {
        let decoded = try JSONDecoder().decode(ModelsListResult.self, from: data)
        return decoded.models.map(self.modelChoice)
    }

    public static func decodeSessionRoutingIdentity(_ data: Data) throws -> SteelEngineChatSessionRoutingIdentity {
        let decoded = try JSONDecoder().decode(AgentsListResult.self, from: data)
        guard let identity = SteelEngineChatSessionRoutingIdentity(
    ...
    public static func modelChoice(_ model: ModelChoice) -> SteelEngineChatModelChoice {
        let name = model.name.trimmingCharacters(in: .whitespacesAndNewlines)
        return SteelEngineChatModelChoice(
            modelID: model.id,
    ...
    public static func commandChoice(_ entry: CommandEntry) -> SteelEngineChatCommandChoice {
        let sourceValue = (entry.source.value as? String)?
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        let source: SteelEngineChatCommandChoice.Source = switch sourceValue {
        case "native":
            .command
        case "skill":
            .skill
        ...
        return SteelEngineChatCommandChoice(
            id: id,
            name: entry.name,
            textAliases: aliases,
    ...
    public static func event(from frame: EventFrame) -> SteelEngineChatTransportEvent? {
        switch frame.event {
        case "tick":
            return .tick
        case "sessions.changed":
            guard let payload = frame.payload,
                  let change = try? GatewayPayloadDecoding.decode(
                      payload,
                      as: SteelEngineChatSessionsChangedEvent.self)
            else { return nil }
        ...
        case "health":
            ...
            let ok = (try? GatewayPayloadDecoding.decode(
                payload,
                as: SteelEngineGatewayHealthOK.self))?.ok ?? true
            return .health(ok: ok)
        case "chat":
            guard let payload = frame.payload,
        ...
        case "session.message":
            guard let payload = frame.payload,
                  let message = try? GatewayPayloadDecoding.decode(
                      payload,
                      as: SteelEngineSessionMessageEventPayload.self)
            else { return nil }
        ...
        case "agent":
            guard let payload = frame.payload,
                  let agent = try? GatewayPayloadDecoding.decode(
                      payload,
                      as: SteelEngineAgentEventPayload.self)
            else { return nil }
            return .agent(agent)
        ...
        case "question.resolved":
            guard let payload = frame.payload,
                  let resolved = try? GatewayPayloadDecoding.decode(
                      payload,
                      as: SteelEngineQuestionResolvedEvent.self)
            else { return nil }
