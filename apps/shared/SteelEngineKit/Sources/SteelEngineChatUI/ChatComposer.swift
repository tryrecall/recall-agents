...
private struct CleanChatComposerSurface: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        content
            ...
            .overlay(
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .strokeBorder(SteelEngineChatTheme.composerBorder, lineWidth: 1))
        #else
...
@MainActor
struct SteelEngineChatComposer: View {
    @Bindable var viewModel: SteelEngineChatViewModel
    let style: SteelEngineChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: SteelEngineChatView.ComposerChrome
    let isComposerEnabled: Bool
    let isAttachmentInputEnabled: Bool
    let messagePlaceholder: String?
    let talkControl: SteelEngineChatTalkControl?
    let voiceNoteControl: SteelEngineChatVoiceNoteControl?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if let voiceNoteControl, voiceNoteControl.recorder.isRecording {
                SteelEngineVoiceNoteRecordingRow(recorder: voiceNoteControl.recorder)
            ...
            if self.composerChrome == .full {
                ...
                if self.style == .standard {
                    let shape = UnevenRoundedRectangle(
                        ...
                        style: .continuous)
                    shape
                        .fill(SteelEngineChatTheme.composerBackground)
                        .overlay(shape.strokeBorder(SteelEngineChatTheme.composerBorder, lineWidth: 1))
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 5) {
                    ...
                    if self.viewModel.showsModelPicker {
                        self.modelPicker
                        if self.viewModel.modelSelectionID != SteelEngineChatViewModel.defaultModelSelectionID {
                            self.modelPinButton
                        }
                    }
                }
    ...
    private func contextUsageIndicator(_ fraction: Double) -> some View {
        ...
        let color = fraction >= 0.8 ? SteelEngineChatTheme.warning : SteelEngineChatTheme.muted
        return ZStack {
            Circle()
                .stroke(SteelEngineChatTheme.muted.opacity(0.2), lineWidth: 2)
            Circle()
    ...
    private var thinkingPicker: some View {
            ...
            set: { next in self.viewModel.selectThinkingLevel(next) }))
        {
            Text(String(localized: "Default (inherited)"))
                ...
                .tag(SteelEngineChatViewModel.inheritedThinkingSelectionID)
    ...
    private var modelPicker: some View {
            ...
            set: { next in self.viewModel.selectModel(next) }))
        {
            Text(self.viewModel.defaultModelLabel)
                .font(SteelEngineChatTypography.captionSemiBold)
                .tag(SteelEngineChatViewModel.defaultModelSelectionID)
    ...
    private func modelOptions(_ models: [SteelEngineChatModelChoice]) -> some View {
            ...
            set: { next in self.viewModel.switchSession(to: next) }))
        {
            ForEach(self.viewModel.sessionChoices, id: \.key) { session in
                Text(session.displayName ?? session.key)
                    .font(SteelEngineChatTypography.mono(size: 12, relativeTo: .caption))
                    .tag(session.key)
            }
        } label: {
            Text("Session")
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \SteelEnginePendingAttachment.id)
                { (att: SteelEnginePendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            SteelEnginePlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
                            ...
                            if let durationSeconds = att.durationSeconds {
                                Text(steelEngineVoiceNoteDurationLabel(durationSeconds))
                                    .font(SteelEngineChatTypography.caption)
                                    .foregroundStyle(.secondary)
                            }
                        } else {
                    ...
                    .padding(.vertical, 5)
                    .background(SteelEngineChatTheme.accent.opacity(0.08))
                    .clipShape(Capsule())
                }
            }
        }
    ...
    private var fullEditor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(SteelEngineChatTheme.divider)
                .frame(height: 1)
    ...
    private func talkButton(_ talkControl: SteelEngineChatTalkControl) -> some View {
        Button {
    ...
    private func compactTalkButton(_ talkControl: SteelEngineChatTalkControl) -> some View {
        Button {
            talkControl.toggle(self.viewModel.sessionKey)
        } label: {
            ChatTalkButtonGlyph(control: talkControl)
                .font(SteelEngineChatTypography.body(size: 14, weight: .semibold, relativeTo: .subheadline))
                ...
                .background {
                    Circle()
                        .fill(talkControl.isEnabled
                            ...
                            : AnyShapeStyle(SteelEngineChatTheme.accent))
    ...
    private func talkButtonFill(_ talkControl: SteelEngineChatTalkControl) -> AnyShapeStyle {
        if talkControl.isEnabled {
            return AnyShapeStyle(SteelEngineChatTheme.userBubble)
        }
        if !talkControl.isGatewayConnected {
            return AnyShapeStyle(Color.secondary.opacity(0.12))
        }
        return SteelEngineChatTheme.subtleCard
    }

    private func talkButtonStroke(_ talkControl: SteelEngineChatTalkControl) -> Color {
        if talkControl.isEnabled {
            return Color.white.opacity(0.18)
        }
        return SteelEngineChatTheme.composerBorder
    }

    private func talkAccessibilityValue(_ talkControl: SteelEngineChatTalkControl) -> String {
    ...
    private func talkHelpText(_ talkControl: SteelEngineChatTalkControl) -> String {
    ...
    private var connectionPill: some View {
        HStack(spacing: 6) {
            ...
            Text(self.connectionStatusText)
                .font(SteelEngineChatTypography.caption2)
                .foregroundStyle(.secondary)
        }
        ...
        .background {
            if self.composerChrome == .full {
                Capsule()
                    .fill(SteelEngineChatTheme.subtleCard)
            }
        }
    }
...
extension SteelEngineChatComposer {
                      ...
                      self.viewModel.slashCommands.isEmpty
            {
                VStack(alignment: .leading, spacing: 10) {
                    Text("Commands unavailable")
                        .font(SteelEngineChatTypography.footnoteSemiBold)
                    Text(error)
                        .font(SteelEngineChatTypography.caption)
                        .foregroundStyle(.secondary)
                        .lineLimit(3)
    ...
    private func slashCommandRow(
        _ command: SteelEngineChatCommandChoice,
        isHighlighted: Bool) -> some View
    {
        HStack(alignment: .top, spacing: 0) {
            VStack(alignment: .leading, spacing: 3) {
                Text(command.displayInvocation)
                    .font(SteelEngineChatTypography.mono(
                        size: 15,
                        weight: .semibold,
    ...
    private func selectSlashCommand(_ command: SteelEngineChatCommandChoice) {
...
extension SteelEngineChatComposer {
            ...
            Button {
                ...
                if self.viewModel.isAborting {
                    ...
                    Image(systemName: "stop.fill")
                        .font(SteelEngineChatTypography.display(size: 13, weight: .semibold, relativeTo: .caption))
                }
            }
            .buttonStyle(.plain)
            .foregroundStyle(.white)
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? SteelEngineChatTheme.userBubble
    }
