...
public struct SteelEngineChatDisplayOptions: OptionSet, Sendable {
...
@MainActor
public struct SteelEngineChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    private let displayOptions: SteelEngineChatDisplayOptions
    ...
    private let talkControl: SteelEngineChatTalkControl?
    private let voiceNoteControl: SteelEngineChatVoiceNoteControl?
    ...
    public init(
        ...
        displayOptions: SteelEngineChatDisplayOptions? = nil,
        ...
        talkControl: SteelEngineChatTalkControl? = nil,
        voiceNoteControl: SteelEngineChatVoiceNoteControl? = nil,
        speech: SteelEngineChatSpeechController? = nil)
    {
    ...
    public var body: some View {
        ZStack {
            if self.drawsBackground, self.style == .standard {
                SteelEngineChatTheme.background
                    .ignoresSafeArea()
            }
    ...
    private var messageListRows: some View {
        ...
        SteelEngineQuestionCards(viewModel: self.viewModel)
    ...
    private func messageRow(
        for msg: SteelEngineChatMessage,
    ...
    private func isListenable(_ msg: SteelEngineChatMessage) -> Bool {
    ...
    private func speechChipIsPreparing(
        _ speech: SteelEngineChatSpeechController,
        messageID: UUID) -> Bool?
    {
        switch speech.phase {
    ...
    private var visibleMessages: [SteelEngineChatMessage] {
        let base: [SteelEngineChatMessage]
    ...
    private var jumpToLatestButton: some View {
        Button {
            ...
            Label("Jump to latest", systemImage: "arrow.down")
                .font(SteelEngineChatTypography.body(size: 16, weight: .semibold, relativeTo: .callout))
                .padding(.horizontal, 13)
                .padding(.vertical, 8)
        }
        .buttonStyle(.plain)
        .foregroundStyle(SteelEngineChatTheme.assistantText)
        .background(
            Capsule()
...
extension SteelEngineChatView {
    private func mergeToolResults(in messages: [SteelEngineChatMessage]) -> [SteelEngineChatMessage] {
        var result: [SteelEngineChatMessage] = []
        ...
        for message in messages {
            ...
            let merged = SteelEngineChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: SteelEngineChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: SteelEngineChatMessage) -> Bool {
    ...
    private func primaryText(in message: SteelEngineChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            let kind = (content.type ?? "text").lowercased()
            guard kind == "text" || kind.isEmpty else { return nil }
            return content.text
        }
        return SteelEngineChatMessage.displayText(
    ...
    private func hasInlineAttachments(in message: SteelEngineChatMessage) -> Bool {
    ...
    private func toolCalls(in message: SteelEngineChatMessage) -> [SteelEngineChatMessageContent] {
    ...
    private func inlineToolResults(in message: SteelEngineChatMessage) -> [SteelEngineChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: SteelEngineChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: SteelEngineChatMessage) -> String {
        self.primaryText(in: message)
    }

    @ViewBuilder
    private func copyMessageButton(for message: SteelEngineChatMessage) -> some View {
    ...
    @ViewBuilder
    private func openFullMessageButton(for message: SteelEngineChatMessage) -> some View {
    ...
    @ViewBuilder
    private func replyMessageButton(for message: SteelEngineChatMessage) -> some View {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
...
private struct ChatAssistantIntroCard: View {
    let text: String
    let prompts: [SteelEngineChatView.StarterPrompt]
    let onPrompt: (SteelEngineChatView.StarterPrompt) -> Void
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(self.text)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
                        .fill(SteelEngineChatTheme.assistantBubble))
            ...
            ForEach(self.prompts) { prompt in
                Button {
                    ...
                    HStack(spacing: 8) {
                        Text(prompt.title)
                            .font(SteelEngineChatTypography.body(size: 15, weight: .semibold, relativeTo: .callout))
                            .multilineTextAlignment(.leading)
                        Spacer(minLength: 8)
    ...
    var body: some View {
        ...
        ContentUnavailableView {
            Label(self.title, systemImage: self.systemImage)
                .font(SteelEngineChatTypography.headline)
        } description: {
            Text(self.message)
                .font(SteelEngineChatTypography.body)
        } actions: {
            if let actionTitle, let action {
                Button(action: action) {
                    Text(actionTitle)
                        .font(SteelEngineChatTypography.body(size: 15, weight: .semibold, relativeTo: .subheadline))
                }
