...
private let outboxLogger = Logger(subsystem: "ai.steelengine", category: "SteelEngineChatOutbox")

/// Display state for a transcript row backed by a durable outbox command.
public enum SteelEngineChatOutboxMessageState: Equatable, Sendable {
    case queued
    case sending
    case confirming
...
extension SteelEngineChatViewModel {
    ...
    public func outboxState(for messageID: UUID) -> SteelEngineChatOutboxMessageState? {
    ...
    func enqueueOutboxCommand(
        ...
        draftAttachments: [SteelEnginePendingAttachment] = [],
        session: SessionSnapshot) async -> Bool
    {
        ...
        let thinking = self.effectiveThinkingLevelForSend(
            ...
            agentID: agentID,
            sessionRoutingContract: routingContract)
        let command = SteelEngineChatOutboxCommand(
            id: UUID().uuidString,
            sessionKey: session.key,
            deliverySessionKey: deliverySessionKey,
            routingContract: routingContract,
            ...
            attachments: draftAttachments.map {
                SteelEngineChatOutboxAttachment(
                    type: $0.type,
                    mimeType: $0.mimeType,
                    fileName: $0.fileName,
                    data: $0.data,
    ...
    func confirmOutboxCommands(in messages: [SteelEngineChatMessage]) {
    ...
    func confirmOutboxCommandsNow(in messages: [SteelEngineChatMessage]) async {
        self.observeCanonicalOutboxMessageKeys(in: messages)
        guard let outbox else { return }
    ...
    private func persistCanonicalOutboxEvidence(
        _ message: SteelEngineChatMessage,
        for command: SteelEngineChatOutboxCommand) async
    {
        guard let transcriptCache = transcriptCache as? any SteelEngineChatCanonicalTranscriptMerging else { return }
    ...
    private func observeCanonicalOutboxMessageKeys(in messages: [SteelEngineChatMessage]) {
    ...
    private func presentOutboxCommands(_ commands: [SteelEngineChatOutboxCommand]) {
    ...
    private static func outboxUserMessage(for command: SteelEngineChatOutboxCommand) -> SteelEngineChatMessage {
        var content = [
    ...
    private func mapOutboxCommand(_ command: SteelEngineChatOutboxCommand, to messageID: UUID) {
    ...
    private func deliverOutboxCommand(
        _ command: SteelEngineChatOutboxCommand,
        outbox: any SteelEngineChatCommandOutbox,
        routeLease: SteelEngineChatTransportRouteLease) async -> OutboxFlushDisposition
    {
        do {
            let response = try await routeLease.sendMessage(
                ...
                attachments: command.attachments.map {
                    SteelEngineChatAttachmentPayload(
                        type: $0.type,
                        mimeType: $0.mimeType,
                        fileName: $0.fileName,
            ...
            if error.detailsReason == SteelEngineChatSessionRoutingContract.changedErrorReason {
    ...
    private func finishAcceptedOutboxCommand(
        _ command: SteelEngineChatOutboxCommand,
        outbox: any SteelEngineChatCommandOutbox) async -> OutboxFlushDisposition
    {
    ...
    private static func needsOutboxDeliveryReconciliation(_ command: SteelEngineChatOutboxCommand) -> Bool {
        command.status == .awaitingConfirmation ||
            (command.status == .failed &&
                command.lastError == SteelEngineChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }

    private static func deliveryTarget(for command: SteelEngineChatOutboxCommand) -> OutboxDeliveryTarget {
    ...
    private func parkOutboxCommandWithUnconfirmedDelivery(
        _ command: SteelEngineChatOutboxCommand,
        outbox: any SteelEngineChatCommandOutbox) async -> SteelEngineChatOutboxUpdateResult
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            retryCount: command.retryCount,
            lastError: SteelEngineChatSQLiteTranscriptCache.outboxUnconfirmedError)
        switch update {
        case .updated:
            self.setOutboxState(
                .failed(reason: SteelEngineChatSQLiteTranscriptCache.outboxUnconfirmedError),
    ...
    private func parkOutboxCommandForChangedTarget(
        _ command: SteelEngineChatOutboxCommand,
        outbox: any SteelEngineChatCommandOutbox) async -> Bool
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            retryCount: command.retryCount,
            lastError: SteelEngineChatSQLiteTranscriptCache.outboxChangedTargetError)
        guard update != .unavailable else {
            self.applyTransportHealth(false)
            return false
        }
    ...
    private func recordOutboxRejection(
        of command: SteelEngineChatOutboxCommand,
        outbox: any SteelEngineChatCommandOutbox,
    ...
    private func spliceSentCommandIntoCachedTranscript(_ command: SteelEngineChatOutboxCommand) async {
    ...
    private func outboxRoutingContract(for session: SessionSnapshot) -> String? {
        if !self.transport.outboxRequiresSessionRoutingContract {
            return SteelEngineChatOutboxCommand.legacyUnboundRoutingContract
        }
        ...
        agentID: String?) -> String?
    {
        ...
        if SteelEngineChatSessionKey.agentID(from: raw) != nil { return raw }
    ...
    private func setOutboxState(_ state: SteelEngineChatOutboxMessageState, forCommandID commandID: String) {
    ...
    func handleOutboxChange(_ change: SteelEngineChatOutboxChange) {
    ...
    private static func outboxDisplayState(for command: SteelEngineChatOutboxCommand)
        -> SteelEngineChatOutboxMessageState
    {
