...
enum SteelEngineChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("SteelEngineChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("SteelEngineChatTheme.onboardingAssistantBubble"),
...
enum SteelEnginePlatformImageFactory {
    static func image(_ image: SteelEnginePlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
