...
public enum SteelEngineMascotMood: String, CaseIterable, Equatable, Sendable {
...
enum SteelEngineMascotEffect: Equatable {
    case none
    case sparkles
    case hearts
    case zzz
...
final class SteelEngineMascotAnimator {
    ...
    private(set) var mood: SteelEngineMascotMood = .idle
    ...
    private var activeGesture: SteelEngineMascotGesture?
    private var activeGestureStart: TimeInterval = 0
    private var pendingGesture: SteelEngineMascotGesture?
    ...
    private var lastClickReaction: SteelEngineMascotGesture?
    ...
    private var requestedAccessory: SteelEngineMascotAccessory = .none
    ...
    func setMood(_ mood: SteelEngineMascotMood, at time: TimeInterval) {
    ...
    func setAccessory(_ accessory: SteelEngineMascotAccessory, at time: TimeInterval) {
    ...
    func handleTap(at time: TimeInterval) {
        ...
        var reactions: [SteelEngineMascotGesture] = [.hop, .wave, .wink]
    ...
    func pose(at time: TimeInterval) -> SteelEngineMascotPose {
        ...
        let effectiveMood: SteelEngineMascotMood = dozing ? .sleepy : self.mood
        ...
        if self.requestedAccessory != .none {
            ...
            pose.accessoryAmount = SteelEngineMascotGesture.easeOut(progress)
        }
    ...
    private func advanceSchedules(at time: TimeInterval) {
        ...
        self.blinkStarts.removeAll { time - $0 > SteelEngineMascotGesture.blinkDuration }
    ...
    private func randomQuirk() -> SteelEngineMascotGesture {
    ...
    private func startGesture(_ gesture: SteelEngineMascotGesture, at time: TimeInterval) {
        self.activeGesture = gesture
        self.activeGestureStart = time
    }

    private static func entranceGesture(for mood: SteelEngineMascotMood) -> SteelEngineMascotGesture? {
        switch mood {
        case .happy: .hop
    ...
    private func basePose(for mood: SteelEngineMascotMood, at time: TimeInterval) -> SteelEngineMascotPose {
        ...
        case .working:
            ...
            if phase < 0.05 {
                ...
                pose.rightClawDegrees = -6 - 28 * SteelEngineMascotGesture.easeInOut((phase - 0.05) / 0.55)
            } else if phase < 0.72 {
                let strike = ((phase - 0.60) / 0.12).clamped(to: 0...1)
                pose.rightClawDegrees = -34 + 46 * strike * strike
            } else {
                pose.rightClawDegrees = 12 - 18 * SteelEngineMascotGesture.easeInOut((phase - 0.72) / 0.28)
            }
            pose.leftClawDegrees = 4 + 2 * sin(2 * .pi * phase)
            let impact = SteelEngineMascotGesture.bell(((phase - 0.72) / 0.14).clamped(to: 0...1))
    ...
    private func applyGaze(
        _ pose: inout SteelEngineMascotPose,
        mood: SteelEngineMascotMood,
    ...
    private func applyBlinks(_ pose: inout SteelEngineMascotPose, at time: TimeInterval) {
        guard pose.happyEyes < 0.6 else { return }
        for start in self.blinkStarts {
            let progress = (time - start) / SteelEngineMascotGesture.blinkDuration
            guard progress >= 0, progress <= 1 else { continue }
            let closure = SteelEngineMascotGesture.bell(CGFloat(progress))
    ...
    private func applyDizzy(_ pose: inout SteelEngineMascotPose, at time: TimeInterval) {
...
enum SteelEngineMascotGesture: Equatable {
    case wave
    case hop
    case wink
    case celebrate
    ...
    func apply(to pose: inout SteelEngineMascotPose, progress p: CGFloat) {
        switch self {
        case .wave:
        ...
        case .sneeze:
            if p < 0.42 {
                let inhale = SteelEngineMascotGesture.easeInOut(p / 0.42)
                ...
                let recover = 1 - SteelEngineMascotGesture.easeInOut((p - 0.58) / 0.42)
        ...
        case .sigh:
            let rise = SteelEngineMascotGesture.easeInOut((p / 0.3).clamped(to: 0...1))
            let fall = SteelEngineMascotGesture.easeInOut(((p - 0.3) / 0.45).clamped(to: 0...1))
