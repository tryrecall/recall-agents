...
public struct ChatModelProviderSection: Identifiable, Sendable, Equatable {
    ...
    public let models: [SteelEngineChatModelChoice]
    public let isDefaultProvider: Bool
}

public struct ChatModelPickerSections: Sendable, Equatable {
    public let pinned: [SteelEngineChatModelChoice]
    public let recent: [SteelEngineChatModelChoice]
    public let providers: [ChatModelProviderSection]
}

@MainActor
public final class ChatModelPickerStore {
    private static let favoritesKey = "steelengine.chat.modelFavorites"
    private static let recentsKey = "steelengine.chat.modelRecents"
    ...
    public func recordRecent(_ selectionID: String) {
        guard !selectionID.isEmpty, selectionID != SteelEngineChatViewModel.defaultModelSelectionID else { return }
    ...
    static func isDefaultModel(
        _ choice: SteelEngineChatModelChoice,
    ...
    private static func effectiveProvider(_ choice: SteelEngineChatModelChoice) -> String {
        let metadataProvider = self.normalizedProvider(choice.provider)
        if metadataProvider != "other" {
            return metadataProvider
        }
