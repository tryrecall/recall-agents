...
public struct SteelEngineVoiceNoteRecording: Equatable, Sendable {
...
@MainActor
@Observable
public final class SteelEngineVoiceNoteRecorder {
    public enum State: Equatable {
        case idle
        case requestingPermission
        case recording(startedAt: Date, fileURL: URL)
        case finished(recording: SteelEngineVoiceNoteRecording)
        case staging(recording: SteelEngineVoiceNoteRecording)
    ...
    public convenience init() {
        self.init(capture: SteelEngineVoiceNoteAudioCapture())
    }

    /// Creates a recorder with an injectable audio capture seam.
    public init(
        capture: any VoiceNoteAudioCapture,
        durationLimit: TimeInterval = SteelEngineVoiceNoteRecorder.maximumDurationSeconds,
    ...
    public var completedRecording: SteelEngineVoiceNoteRecording? {
    ...
    func claimCompletedRecording() -> SteelEngineVoiceNoteRecording? {
        guard case let .finished(recording) = self.state else { return nil }
    ...
    func completeStaging(_ recording: SteelEngineVoiceNoteRecording) {
    ...
    @discardableResult
    public func finish() -> SteelEngineVoiceNoteRecording? {
        guard case let .recording(_, fileURL) = self.state else { return nil }

        self.timerTask?.cancel()
        self.timerTask = nil
        let duration = max(0, self.capture.stop())
        let recording = SteelEngineVoiceNoteRecording(fileURL: fileURL, durationSeconds: duration)
...
@MainActor
public final class SteelEngineVoiceNoteAudioCapture: NSObject, VoiceNoteAudioCapture, AVAudioRecorderDelegate {
    ...
    public func start(url: URL) throws {
        do {
            ...
            guard recorder.record() else {
                throw NSError(
                    domain: "SteelEngineVoiceNoteAudioCapture",
                    code: 1,
...
func steelEngineVoiceNoteDurationLabel(_ durationSeconds: TimeInterval) -> String {
