...
private let cacheLogger = Logger(subsystem: "ai.steelengine", category: "SteelEngineChatTranscriptCache")

private final class OutboxChangeHub: @unchecked Sendable {
    private let lock = NSLock()
    private var continuations: [UUID: AsyncStream<SteelEngineChatOutboxChange>.Continuation] = [:]

    func stream() -> AsyncStream<SteelEngineChatOutboxChange> {
        let id = UUID()
        let pair = AsyncStream<SteelEngineChatOutboxChange>.makeStream()
        self.lock.lock()
    ...
    func yield(_ change: SteelEngineChatOutboxChange) {
        self.lock.lock()
...
public actor SteelEngineChatSQLiteTranscriptCache: SteelEngineChatTranscriptCache,
    SteelEngineChatCanonicalTranscriptMerging,
    SteelEngineChatCommandOutbox
{
    ...
    public nonisolated static func loadSessionRoutingIdentity(
        databaseURL: URL,
        gatewayID: String) -> SteelEngineChatSessionRoutingIdentity?
    {
        ...
        return SteelEngineChatSessionRoutingIdentity(
    ...
    // MARK: - SteelEngineChatTranscriptCache

    public func loadSessions() async -> [SteelEngineChatSessionEntry] {
        ...
        guard let decoded = try? JSONDecoder().decode(
            [SteelEngineChatSessionEntry].self,
    ...
    public func loadTranscript(sessionKey: String) async -> [SteelEngineChatMessage] {
        await self.loadTranscript(sessionKey: sessionKey, agentID: nil)
    }

    public func loadTranscript(sessionKey: String, agentID: String?) async -> [SteelEngineChatMessage] {
        guard !self.isRetired else { return [] }
        guard let db = await handle() else { return [] }
    ...
    private func readTranscript(
        _ db: OpaquePointer,
        sessionKey: String,
        agentID: String?) -> [SteelEngineChatMessage]
    {
    ...
    public func storeSessions(_ sessions: [SteelEngineChatSessionEntry]) async {
        guard !self.isRetired else { return }
        guard let db = await handle() else { return }
    ...
    public func loadSessionRoutingIdentity() async -> SteelEngineChatSessionRoutingIdentity? {
    ...
    public func storeSessionRoutingIdentity(_ identity: SteelEngineChatSessionRoutingIdentity) async {
    ...
    public func storeTranscript(sessionKey: String, messages: [SteelEngineChatMessage]) async {
    ...
    public func storeTranscript(
        sessionKey: String,
        agentID: String?,
        messages: [SteelEngineChatMessage]) async
    {
...
extension SteelEngineChatSQLiteTranscriptCache {
    // MARK: - SteelEngineChatCommandOutbox

    public nonisolated func changes() -> AsyncStream<SteelEngineChatOutboxChange> {
        self.outboxChangeHub.stream()
    }

    public func enqueueCommand(_ command: SteelEngineChatOutboxCommand) async -> Bool {
    ...
    public func loadCommands() async -> [SteelEngineChatOutboxCommand] {
        await self.loadCommandsIfAvailable() ?? []
    }

    public func loadCommandsIfAvailable() async -> [SteelEngineChatOutboxCommand]? {
        guard !self.isRetired, let db = await handle() else { return nil }
    ...
    public func claimNextCommand() async -> SteelEngineChatOutboxCommand? {
        guard !self.isRetired, let db = await handle() else { return nil }
    ...
    public func markCommandAwaitingConfirmation(id: String) async -> SteelEngineChatOutboxUpdateResult {
        ...
        routingContract: String) async -> SteelEngineChatOutboxUpdateResult
    {
        ...
        let allowsUntargetedAgent = normalizedRoutingContract == SteelEngineChatOutboxCommand
    ...
    public func cancelCommand(id: String) async -> SteelEngineChatOutboxUpdateResult {
        guard !self.isRetired, let db = await handle() else { return .unavailable }
        guard self.execute(db, sql: "BEGIN IMMEDIATE", bindings: []) else { return .unavailable }
        var committed = false
        defer {
        ...
        let result: SteelEngineChatOutboxUpdateResult
        do {
            let isProven = self.canonicalMessageProofHub.lockProofDecision(for: messageKey)
            defer { self.canonicalMessageProofHub.unlockProofDecision() }
            if isProven {
    ...
    public func confirmCommand(id: String) async -> SteelEngineChatOutboxUpdateResult {
        guard !self.isRetired, let db = await handle() else { return .unavailable }
        guard self.execute(
            db,
    ...
    private func emitOutboxChange(_ change: SteelEngineChatOutboxChange) {
...
extension SteelEngineChatSQLiteTranscriptCache {
    ...
    static func cacheableMessages(_ messages: [SteelEngineChatMessage]) -> [SteelEngineChatMessage] {
    ...
    static func boundedSessions(_ sessions: [SteelEngineChatSessionEntry]) -> [SteelEngineChatSessionEntry] {
    ...
    private static func attachmentByteCount(_ attachments: [SteelEngineChatOutboxAttachment]) -> Int? {
        var total = 0
    ...
    private func readCommands(_ db: OpaquePointer) -> [SteelEngineChatOutboxCommand]? {
        var statement: OpaquePointer?
        let sql = """
        ...
        var commands: [SteelEngineChatOutboxCommand] = []
        ...
        while step == SQLITE_ROW {
               ...
               let text = sqlite3_column_text(statement, 5)
            {
                ...
                guard let attachments = try? JSONDecoder().decode(
                    [SteelEngineChatOutboxAttachment].self,
                ...
                if let status = SteelEngineChatOutboxCommand.Status(rawValue: statusRaw) {
                    commands.append(
