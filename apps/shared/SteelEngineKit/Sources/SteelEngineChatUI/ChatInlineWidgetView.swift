...
enum SteelEngineChatWidgetSurfaceRole: Sendable, Hashable {
    case node
    case operatorSurface
    case legacy
}

public struct SteelEngineChatWidgetResource: Sendable, Equatable {
    public let url: URL
    public let tlsFingerprintSHA256: String?
    let surfaceRole: SteelEngineChatWidgetSurfaceRole
    let attemptedSurfaceRoles: Set<SteelEngineChatWidgetSurfaceRole>
    ...
    init(
        ...
        surfaceRole: SteelEngineChatWidgetSurfaceRole,
        attemptedSurfaceRoles: Set<SteelEngineChatWidgetSurfaceRole>)
    {
        self.url = url
        self.tlsFingerprintSHA256 = tlsFingerprintSHA256
...
public enum SteelEngineChatWidgetURLResolver {
    private static let documentsPath = "/__steelengine__/canvas/documents"
    ...
    public static func resolveResource(
        target: String,
        replacing failedResource: SteelEngineChatWidgetResource?,
    ...
    private static func capabilitySurface(_ rawSurfaceURL: String?) -> URLComponents? {
        ...
        guard segments.count >= 3,
              segments[segments.count - 3] == "__steelengine__",
    ...
    private static func resolve(
        ...
        role: SteelEngineChatWidgetSurfaceRole,
        target: String,
        attemptedRoles: Set<SteelEngineChatWidgetSurfaceRole>) -> SteelEngineChatWidgetResource?
    {
        guard let url = resolve(surfaceURL: surface.url, target: target) else { return nil }
        let resource = SteelEngineChatWidgetResource(
            url: url,
    ...
    private static func resolvePreferred(
        ...
        target: String,
        excluding failedResource: SteelEngineChatWidgetResource?,
        blockedRoles: Set<SteelEngineChatWidgetSurfaceRole>,
        attemptedRoles: Set<SteelEngineChatWidgetSurfaceRole>) -> SteelEngineChatWidgetResource?
    {
        [
            (role: SteelEngineChatWidgetSurfaceRole.node, surface: surfaces.node),
            (role: SteelEngineChatWidgetSurfaceRole.operatorSurface, surface: surfaces.operatorSurface),
        ]
            .lazy
    ...
    private static func isReplacement(
        _ candidate: SteelEngineChatWidgetResource,
        for failedResource: SteelEngineChatWidgetResource?) -> Bool
    {
...
struct ChatInlineWidgetView: View {
    let preview: SteelEngineChatCanvasPreview
    ...
    @State private var resolvedResource: SteelEngineChatWidgetResource?
        ...
        VStack(alignment: .leading, spacing: 6) {
            if let title = self.preview.title?.trimmingCharacters(in: .whitespacesAndNewlines), !title.isEmpty {
                Text(title)
                    ...
                    .fontWeight(.semibold)
                    .foregroundStyle(SteelEngineChatTheme.muted)
            }
                ...
                ChatInlineWidgetWebView(
                    ...
                    .overlay {
                        RoundedRectangle(cornerRadius: 10, style: .continuous)
                            .stroke(SteelEngineChatTheme.muted.opacity(0.24), lineWidth: 1)
                    }
    ...
    private func handleLoadFailure(resource: SteelEngineChatWidgetResource) {
...
extension SteelEngineChatWidgetResource {
...
private final class ChatInlineWidgetNavigationDelegate: NSObject, WKNavigationDelegate {
    var resource: SteelEngineChatWidgetResource {
        didSet {
    ...
    init(resource: SteelEngineChatWidgetResource, onFailure: @escaping @MainActor @Sendable () -> Void) {
...
private func makeChatInlineWidgetWebView(
    resource: SteelEngineChatWidgetResource,
    allowsScripts: Bool,
