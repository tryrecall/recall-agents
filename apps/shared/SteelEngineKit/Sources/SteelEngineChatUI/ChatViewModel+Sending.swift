...
private let chatSendingLogger = Logger(subsystem: "ai.steelengine", category: "SteelEngineChatUI")
...
extension SteelEngineChatViewModel {
    ...
    public func slashCommandMatches(
        query: String,
        filter: SteelEngineChatCommandFilter) -> [SteelEngineChatCommandChoice]
    {
    ...
    public func applySlashCommandSelection(_ command: SteelEngineChatCommandChoice) {
    ...
    private static func isKnownSlashCommandText(
        _ text: String,
        commands: [SteelEngineChatCommandChoice]) -> Bool
    {
    ...
    private static func commands(
        _ commands: [SteelEngineChatCommandChoice],
        ...
        filter: SteelEngineChatCommandFilter) -> [SteelEngineChatCommandChoice]
    {
        ...
        let effectiveFilter: SteelEngineChatCommandFilter =
            self.queryTargetsSkills(trimmed) && filter == .all ? .skills : filter
        return commands.enumerated()
            .compactMap { index, command -> (Int, Int, SteelEngineChatCommandChoice)? in
    ...
    private static func command(
        _ command: SteelEngineChatCommandChoice,
        isIncludedIn filter: SteelEngineChatCommandFilter) -> Bool
    {
        switch filter {
        case .all:
            true
    ...
    private struct SendDraft {
        let input: String
        let attachments: [SteelEnginePendingAttachment]
        let trimmed: String
        let session: SessionSnapshot
        let replyTarget: SteelEngineChatReplyTarget?
    ...
    private struct LiveSendAttempt {
        ...
        let encodedAttachments: [SteelEngineChatAttachmentPayload]
        ...
        mustPreserveOutboxOrder: Bool) async -> AttachmentPersistenceDecision
    {
        ...
        guard case let .unavailable(reason) = routeResult,
              reason == SteelEngineChatTransportUpgradeMessage.routingContract
        else {
    ...
    private static func userContent(
        messageText: String,
        attachments: [SteelEnginePendingAttachment],
        encodedAttachments: [SteelEngineChatAttachmentPayload]) -> [SteelEngineChatMessageContent]
    {
        var content: [SteelEngineChatMessageContent] = [
    ...
    private func handleLiveSendResponse(
        _ response: SteelEngineChatSendResponse,
    ...
    private func handleLiveSendFailure(_ error: Error, attempt: LiveSendAttempt) async {
        ...
        if attempt.encodedAttachments.isEmpty, !(error is GatewayResponseError) {
            ...
            let deliveryIsAmbiguous = !(error is SteelEngineChatTransportSendError)
