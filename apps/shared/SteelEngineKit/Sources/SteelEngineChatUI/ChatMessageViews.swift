...
struct ChatAgentAvatar: View {
    ...
    var body: some View {
        Text(self.displayText)
            .font(SteelEngineChatTypography.avatar(size: self.fontSize))
            ...
            .background(
                Circle()
                    .fill(
                        LinearGradient(
                            colors: [
                                (self.tint ?? SteelEngineChatTheme.accent).opacity(0.95),
            ...
            .shadow(color: (self.tint ?? SteelEngineChatTheme.accent).opacity(0.18), radius: 8, y: 4)
...
struct ChatMessageBubble: View {
    ...
    let inlineWidgetResourceResolver: @MainActor @Sendable (
        String,
        SteelEngineChatWidgetResource?) async -> SteelEngineChatWidgetResource?
...
private struct ChatMessageBody: View {
    @Environment(\.steelEngineAssistantBubblesInCleanChrome) private var assistantBubblesInClean
    let message: SteelEngineChatMessage
    let isUser: Bool
    let style: SteelEngineChatView.Style
    let markdownVariant: ChatMarkdownVariant
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? SteelEngineChatTheme.userText : SteelEngineChatTheme.assistantText
    ...
    private var inlineAttachments: [SteelEngineChatMessageContent] {
    ...
    private var inlineWidgets: [SteelEngineChatCanvasPreview] {
    ...
    private var toolCalls: [SteelEngineChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [SteelEngineChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func usageTint(_ pressure: ChatMessageUsagePresentation.Pressure) -> Color {
        switch pressure {
        case .normal:
            SteelEngineChatTheme.muted
        case .warning:
            SteelEngineChatTheme.warning
        case .danger:
            SteelEngineChatTheme.danger
        }
    }
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? SteelEngineChatTheme.userBubble
        }
        if self.style == .onboarding {
            return SteelEngineChatTheme.onboardingAssistantBubble
        }
        return SteelEngineChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return SteelEngineChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: SteelEngineChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.isAudio ? "Voice note" : (self.att.fileName ?? "Attachment"))
                .font(SteelEngineChatTypography.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? SteelEngineChatTheme.userText : SteelEngineChatTheme.assistantText)
            if self.isAudio, let durationSeconds = self.att.durationSeconds {
                Text(steelEngineVoiceNoteDurationLabel(durationSeconds))
                    .font(SteelEngineChatTypography.footnote)
                    .foregroundStyle(
                        self.isUser
                            ? SteelEngineChatTheme.userText.opacity(0.72)
                            : SteelEngineChatTheme.assistantText.opacity(0.72))
            }
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: SteelEngineChatMessageContent
...
struct ChatOutboxStatusLabel: View {
    let state: SteelEngineChatOutboxMessageState
    ...
    var body: some View {
        ...
        .foregroundStyle(self.state.isFailed ? AnyShapeStyle(SteelEngineChatTheme.danger) : AnyShapeStyle(.secondary))
    ...
    private var title: LocalizedStringResource {
        ...
        case .confirming:
            "Confirming…"
        case let .failed(reason) where reason == SteelEngineChatSQLiteTranscriptCache.outboxUnconfirmedError:
            "Delivery unknown"
        case .failed:
            "Not sent"
        }
...
private struct SteelEngineAssistantBubblesInCleanChromeKey: EnvironmentKey {
...
extension EnvironmentValues {
    ...
    public var steelEngineAssistantBubblesInCleanChrome: Bool {
        get { self[SteelEngineAssistantBubblesInCleanChromeKey.self] }
        set { self[SteelEngineAssistantBubblesInCleanChromeKey.self] = newValue }
    }
}
...
private struct AssistantBubbleContainerStyle: ViewModifier {
    ...
    @Environment(\.steelEngineAssistantBubblesInCleanChrome) private var bubblesInClean
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [SteelEngineChatPendingToolCall]
    let isClean: Bool
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                let font = segment.kind == .thinking
                    ? SteelEngineChatTypography.callout.italic()
                    : SteelEngineChatTypography.body
