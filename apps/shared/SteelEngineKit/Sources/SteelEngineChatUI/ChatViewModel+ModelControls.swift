...
extension SteelEngineChatViewModel {
        ...
        self.enqueueSessionSettingsPatch(requestID: requestID, target: target) { [weak self] routeLease in
            ...
            do {
                ...
                let result = try await routeLease.patchSessionSettings(
                    ...
                    patch: SteelEngineChatSessionSettingsPatch(verboseLevel: .some(next)))
    ...
    func performSelectFastMode(_ selectionID: String) {
        let next: SteelEngineChatFastMode?
        switch selectionID {
        ...
        self.enqueueSessionSettingsPatch(requestID: requestID, target: target) { [weak self] routeLease in
            ...
            do {
                ...
                let result = try await routeLease.patchSessionSettings(
                    sessionKey: target.canonicalSessionKey,
                    agentID: target.agentID,
                    patch: SteelEngineChatSessionSettingsPatch(fastMode: .some(next)))
    ...
    func applyModelControlPatchResult(
        _ result: SteelEngineChatModelPatchResult,
        sessionKey: String,
    ...
    private func recordModelControlPatchSuccess(
        ...
        fastModeOverride: SteelEngineChatFastMode?? = nil,
        ...
        verboseLevelOverride: String?? = nil)
    {
        ...
        let recordedFastMode: SteelEngineChatFastMode? = if let fastModeOverride {
            fastModeOverride
        } else {
        ...
        self.lastSuccessfulSettingsPatchResultsByTarget[target] = SteelEngineChatModelPatchResult(
    ...
    private func updateCurrentSessionFastMode(
        _ mode: SteelEngineChatFastMode?,
        effective: SteelEngineChatFastMode?,
        sessionKey: String,
        exactMatchOnly: Bool = false)
    {
        let index = exactMatchOnly
