...
extension SteelEngineChatViewModel {
    static func decodeMessages(_ raw: [AnyCodable]) -> [SteelEngineChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SteelEngineChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    static func stripInboundMetadata(from message: SteelEngineChatMessage) -> SteelEngineChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> SteelEngineChatMessageContent in
    ...
    static func messageContentFingerprint(for message: SteelEngineChatMessage) -> String {
    ...
    static func finalMessageContentFingerprint(for message: SteelEngineChatMessage) -> String {
        message.content.map { item in
            let type = (item.type ?? "text").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            let text = (item.text ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    static func messageIdentityKey(for message: SteelEngineChatMessage) -> String? {
    ...
    static func userRefreshIdentityKey(for message: SteelEngineChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    static func finalMessageReconciliationKey(for message: SteelEngineChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    static func adoptingCanonicalMessage(
        _ incoming: SteelEngineChatMessage,
        over existing: SteelEngineChatMessage) -> SteelEngineChatMessage
    {
    ...
    private static func preservingLocalAudioDurations(
        in incoming: [SteelEngineChatMessageContent],
        from existing: [SteelEngineChatMessageContent]) -> [SteelEngineChatMessageContent]
    {
    ...
    static func messageRange(
        ...
        in messages: [SteelEngineChatMessage]) -> Range<[SteelEngineChatMessage].Index>
    {
    ...
    func adoptingProvisionalFinalMessageIDs(
        in incoming: [SteelEngineChatMessage]) -> [SteelEngineChatMessage]
    {
    ...
    func clearProvisionalFinalMarkersAdoptedByHistory(_ incoming: [SteelEngineChatMessage]) {
    ...
    func adoptCorrelatedUserMessage(incoming: SteelEngineChatMessage) -> Bool {
    ...
    func adoptProvisionalFinalMessage(incoming: SteelEngineChatMessage) -> Bool {
    ...
    static func reconcileMessageIDs(
        previous: [SteelEngineChatMessage],
        incoming: [SteelEngineChatMessage]) -> [SteelEngineChatMessage]
    {
        guard !previous.isEmpty, !incoming.isEmpty else { return incoming }

        var previousMessagesByKey: [String: [SteelEngineChatMessage]] = [:]
    ...
    static func reconcileRunRefreshMessages(
        previous: [SteelEngineChatMessage],
        incoming: [SteelEngineChatMessage],
        pendingLocalUserEchoIDs: Set<UUID>) -> [SteelEngineChatMessage]
    {
    ...
    static func dedupeMessages(_ messages: [SteelEngineChatMessage]) -> [SteelEngineChatMessage] {
    ...
    static func dedupeKey(for message: SteelEngineChatMessage) -> String? {
...
extension SteelEngineChatViewModel {
    ...
    func applyHistoryPayload(
        _ payload: SteelEngineChatHistoryPayload,
    ...
    private func provisionalFinalMessagesMissing(
        from incoming: [SteelEngineChatMessage]) -> [SteelEngineChatMessage]
    {
