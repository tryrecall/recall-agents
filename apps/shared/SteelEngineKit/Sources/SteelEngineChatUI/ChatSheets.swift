...
struct ChatSessionsSheet: View {
    ...
    @State private var scopedSessions: [SteelEngineChatSessionEntry] = []
    @State private var isLoadingScoped = false
    @State private var renameTarget: SteelEngineChatSessionEntry?
    ...
    private var displayedSessions: [SteelEngineChatSessionEntry] {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear {
                self.viewModel.refreshSessions(limit: SteelEngineChatViewModel.sessionListFetchLimit)
            }
            .alert(
                "Rename Session",
                isPresented: Binding(
    ...
    private func sessionRow(_ session: SteelEngineChatSessionEntry) -> some View {
            ...
            HStack(spacing: 8) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(session.displayName ?? session.key)
                        .font(SteelEngineChatTypography.mono(size: 17, relativeTo: .body))
                        .lineLimit(1)
        ...
        .swipeActions(edge: .leading, allowsFullSwipe: true) {
            if !session.isArchived {
                ...
                .tint(SteelEngineChatTheme.accent)
            }
        }
        .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                ...
                mainSessionKey: self.viewModel.resolvedMainSessionKey)
            {
                ...
                .tint(session.isArchived ? SteelEngineChatTheme.accent : SteelEngineChatTheme.danger)
            }
        }
        .contextMenu {
            Button {
