...
extension SteelEngineChatViewModel {
    nonisolated static func chatContextUsageFraction(for session: SteelEngineChatSessionEntry?) -> Double? {
    ...
    func currentSessionEntry() -> SteelEngineChatSessionEntry? {
    ...
    func sessionMutationIdentity(for key: String, listedKey: String? = nil) -> String {
        ...
        return self.modelPatchTarget(
            ...
            agentID: SteelEngineChatSessionKey.agentID(from: key) ?? self.activeAgentId,
            sessionRoutingContract: nil).canonicalSessionKey
    }

    func applyingLocalUnreadOverrides(
        to sessions: [SteelEngineChatSessionEntry]) -> [SteelEngineChatSessionEntry]
    {
        ...
        sessionRoutingContract: String?) -> ModelPatchTarget
    {
        ...
        if SteelEngineChatSessionKey.agentID(from: candidate) != nil || normalizedCandidate == "unknown" {
            targetKey = candidate
            targetAgentID = nil
    ...
    func sessionEntryForThinking(
        ...
        agentID: String?) -> SteelEngineChatSessionEntry?
    {
    ...
    func successfulModelPatchResult(
        ...
        session: SteelEngineChatSessionEntry?) -> SteelEngineChatModelPatchResult?
    {
        guard let session,
    ...
    public var sessionChoices: [SteelEngineChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = SteelEngineChatSessionListOrganizer.organize(sessions)
        let mainSessionKey = resolvedMainSessionKey

        var result: [SteelEngineChatSessionEntry] = []
