...
struct ChatTalkActivityStrip: View {
    let control: SteelEngineChatTalkControl
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            ...
                            if !model.partialTranscript.isEmpty {
                                Text(model.partialTranscript)
                                    .font(SteelEngineChatTypography.caption.italic())
    ...
    private func header(model: ChatTalkTranscriptStripModel, showsDisclosure: Bool) -> some View {
        HStack(spacing: 7) {
            Circle()
                .fill(SteelEngineChatTheme.accent)
            ...
            if let collapsedText = model.collapsedText, !self.isExpanded {
                Text(collapsedText)
                    .font(SteelEngineChatTypography.caption)
                    .foregroundStyle(model.partialTranscript.isEmpty ? .secondary : SteelEngineChatTheme.accent)
...
extension View {
    func chatTalkInputDeviceMenu(_ control: SteelEngineChatTalkControl) -> some View {
