...
struct ChatPlanPill: View {
    let steps: [SteelEngineChatPlanStep]
    ...
    private var currentStep: SteelEngineChatPlanStep? {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if self.isExpanded {
                Divider()
                    .overlay(SteelEngineChatTheme.divider)
                    .padding(.horizontal, 12)
    ...
    private func stepRow(_ step: SteelEngineChatPlanStep) -> some View {
            ...
            Text(step.step)
                ...
                .foregroundStyle(
                    step.status == .pending
                        ...
                        : SteelEngineChatTheme.assistantText)
                .fixedSize(horizontal: false, vertical: true)
        }
    ...
    private static func marker(for status: SteelEngineChatPlanStep.Status) -> String {
        switch status {
        case .completed: "✓"
        case .inProgress: "▸"
        case .pending: "▢"
    ...
    private static func markerColor(for status: SteelEngineChatPlanStep.Status) -> Color {
        switch status {
        case .completed, .inProgress: SteelEngineChatTheme.accent
        case .pending: SteelEngineChatTheme.muted
        }
    }

    private static func stepAccessibilityLabel(_ step: SteelEngineChatPlanStep) -> String {
        "\(self.accessibilityLabel(for: step.status)), \(step.step)"
    }

    private static func accessibilityLabel(for status: SteelEngineChatPlanStep.Status) -> String {
        switch status {
