...
extension SteelEngineChatViewModel {
    ...
    func replaceMessages(_ messages: [SteelEngineChatMessage]) {
    ...
    func persistSessionsToCache(_ sessions: [SteelEngineChatSessionEntry]) {
        guard let transcriptCache else { return }
        let previous = pendingCacheWriteTask
        pendingCacheWriteTask = Task.detached {
            await previous?.value
        ...
        if sessions.isEmpty, !hasAppliedLiveSessions {
            Task { [weak self] in
                ...
                self.sessions = self.applyingLocalUnreadOverrides(
                    to: SteelEngineChatSessionListOrganizer.organize(cached))
            }
        }
    ...
    static func transcriptCacheAgentID(sessionKey: String, agentID: String?) -> String? {
        guard SteelEngineChatSessionKey.agentID(from: sessionKey) == nil else { return nil }
