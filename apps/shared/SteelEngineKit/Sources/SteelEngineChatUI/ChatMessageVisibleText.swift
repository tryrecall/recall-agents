...
enum ChatMessageVisibleText {
    static func copyText(in message: SteelEngineChatMessage) -> String {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        return role == "assistant" ? self.visibleText(in: message) : self.primaryText(in: message)
    }

    static func visibleText(in message: SteelEngineChatMessage) -> String {
    ...
    static func hasVisibleText(in message: SteelEngineChatMessage) -> Bool {
        !self.visibleText(in: message)
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .isEmpty
    }

    private static func primaryText(in message: SteelEngineChatMessage) -> String {
