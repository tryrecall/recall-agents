        ...
        Button {
            ...
            HStack(spacing: 6) {
                Text(verbatim: String(
                    ...
                    .foregroundStyle(SteelEngineChatTheme.assistantText.opacity(0.65))
                    .lineLimit(1)
                Spacer(minLength: 0)
            ...
            .padding(.vertical, 6)
            .background(SteelEngineChatTheme.subtleCard)
            .clipShape(RoundedRectangle(cornerRadius: 8, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: 8, style: .continuous)
                    .strokeBorder(SteelEngineChatTheme.divider, lineWidth: 1))
        }
        .buttonStyle(.plain)
        .accessibilityLabel(
            String(
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                case .failed:
                    Text("No preview available")
                        .font(SteelEngineChatTypography.callout)
                        .foregroundStyle(SteelEngineChatTheme.assistantText.opacity(0.65))
                case let .loaded(metadata):
