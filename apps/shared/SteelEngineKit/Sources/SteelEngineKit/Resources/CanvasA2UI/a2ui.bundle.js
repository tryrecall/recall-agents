...
var SteelEngineA2UIHost = class extends i$7 {
	static properties = {
		surfaces: { state: true },
  ...
  `;
	connectedCallback() {
		...
		for (const eventName of ["steelengine:a2ui-action-status"]) globalThis.addEventListener(eventName, this.#statusListener);
		this.#syncSurfaces();
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		if (this.#statusListener) {
			for (const eventName of ["steelengine:a2ui-action-status"]) globalThis.removeEventListener(eventName, this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		const handler = globalThis.webkit?.messageHandlers?.steelengineCanvasA2UIAction ?? globalThis.steelengineCanvasA2UIAction;
		if (handler?.postMessage) try {
			if (handler === globalThis.steelengineCanvasA2UIAction) postNativeMessage(handler, JSON.stringify({ userAction }));
...
if (!customElements.get("steelengine-a2ui-host")) customElements.define("steelengine-a2ui-host", SteelEngineA2UIHost);
