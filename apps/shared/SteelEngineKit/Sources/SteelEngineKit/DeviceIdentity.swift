...
enum DeviceIdentityPaths {
    ...
    private static let appGroupStateDirAvailable =
        DeviceIdentityPaths.hasAppGroupEntitlement(SteelEngineAppGroup.identifier)
        ...
        temporaryDirectory: URL) -> URL
    {
        ...
        return temporaryDirectory.appendingPathComponent("steelengine", isDirectory: true)
    }

    private static func stateDirOverrideURL() -> URL? {
        // Test-scoped stores must win over the process environment. Parallel Swift tests
        // otherwise race whenever another suite temporarily swaps STEELENGINE_STATE_DIR.
    ...
    private static func legacyStateDirURL() -> URL? {
        if let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
            return appSupport.appendingPathComponent("SteelEngine", isDirectory: true)
        }
        return nil
    }
    ...
    private static func appGroupStateDirURL() -> URL? {
        guard
            let containerURL = FileManager.default
                .containerURL(forSecurityApplicationGroupIdentifier: SteelEngineAppGroup.identifier)
        else {
            return nil
        }
        return containerURL.appendingPathComponent("SteelEngine", isDirectory: true)
    }
    ...
    /// An explicit STEELENGINE_STATE_DIR override selects a caller-chosen store, not the legacy
