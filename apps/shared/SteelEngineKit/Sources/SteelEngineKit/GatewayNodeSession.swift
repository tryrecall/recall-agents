...
public actor GatewayNodeSession {
    ...
    private let logger = Logger(subsystem: "ai.steelengine", category: "node.gateway")
    ...
    // periphery:ignore - Shipped public refresh API; keep until a breaking SteelEngineKit window.
    @discardableResult
...
extension GatewayNodeSession {
        ...
        -> BridgeInvokeResponse
    {
        ...
        let requiresRouteScopedCancellation = request.command == "computer.act" ||
            SteelEngineTalkCommand(rawValue: request.command) != nil
    ...
    private func decodeInvokeRequest(from payload: SteelEngineProtocol.AnyCodable) throws -> NodeInvokeRequestPayload {
        try self.decodeEventPayload(from: payload)
    }

    private func decodeEventPayload<T: Decodable>(from payload: SteelEngineProtocol.AnyCodable) throws -> T {
        do {
