...
public enum SteelEngineNodeStorage {
    public static func appSupportDir() throws -> URL {
        let base = FileManager().urls(for: .applicationSupportDirectory, in: .userDomainMask).first
        guard let base else {
            throw NSError(domain: "SteelEngineNodeStorage", code: 1, userInfo: [
    ...
    public static func cachesDir() throws -> URL {
        ...
        guard let base else {
            throw NSError(domain: "SteelEngineNodeStorage", code: 2, userInfo: [
