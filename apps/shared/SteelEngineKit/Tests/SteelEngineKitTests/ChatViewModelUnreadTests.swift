...
private actor UnreadTestTransportState {
    ...
    var sessionOverride: [SteelEngineChatSessionEntry]?
...
private final class UnreadTestTransport: @unchecked Sendable, SteelEngineChatTransport {
    ...
    func setSessions(_ sessions: [SteelEngineChatSessionEntry]) async {
...
extension UnreadTestTransportState {
    ...
    fileprivate func setSessions(_ sessions: [SteelEngineChatSessionEntry]) {
...
struct ChatViewModelUnreadTests {
    ...
    @Test func `failed route lease preserves mutation queue ordering`() async throws {
        ...
        let firstLease = SteelEngineChatSessionMutationRouteLease { _, _, _, _, _, _ in
            await recorder.append("first-start")
            try await Task.sleep(for: .milliseconds(100))
            await recorder.append("first-end")
        }
        let thirdLease = SteelEngineChatSessionMutationRouteLease { _, _, _, _, _, _ in
            await recorder.append("third")
        }

        let first = queue.reserve(
            routeLease: Task<SteelEngineChatSessionMutationRouteLease?, Never> { firstLease },
            queueKey: "a",
            routeKey: "a",
            unread: false)
        let second = queue.reserve(
            routeLease: Task<SteelEngineChatSessionMutationRouteLease?, Never> { nil },
            queueKey: "a",
            routeKey: "a",
            unread: true)
        let third = queue.reserve(
            routeLease: Task<SteelEngineChatSessionMutationRouteLease?, Never> { thirdLease },
            queueKey: "a",
            routeKey: "a",
            unread: false)

        try await first.value
        await #expect(throws: SteelEngineChatTransportSendError.self) {
    ...
    private func viewModel(
        ...
        transport: UnreadTestTransport) -> SteelEngineChatViewModel
    {
        let defaults = UserDefaults(suiteName: "ChatViewModelUnreadTests.\(UUID().uuidString)") ?? .standard
        return SteelEngineChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
    ...
    private func entry(
        ...
        pinned: Bool? = nil) -> SteelEngineChatSessionEntry
    {
