    ...
    idempotencyKey: String? = nil) -> AnyCodable
{
    ...
    if let idempotencyKey {
        message["__steelengine"] = ["idempotencyKey": idempotencyKey]
    }
...
extension [SteelEngineChatMessage] {
...
private func historyPayload(
    ...
    inFlightRun: SteelEngineChatInFlightRun? = nil) -> SteelEngineChatHistoryPayload
{
...
private func sessionEntry(
    ...
    contextTokens: Int? = nil) -> SteelEngineChatSessionEntry
{
...
private func sessionsListResponse(_ sessions: [SteelEngineChatSessionEntry]) -> SteelEngineChatSessionsListResponse {
...
private func thinkingOption(_ id: String, label: String? = nil) -> SteelEngineChatThinkingLevelOption {
    SteelEngineChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    thinkingLevels: [SteelEngineChatThinkingLevelOption]? = nil,
    thinkingOptions: [String]? = nil,
    thinkingDefault: String? = nil,
    verboseLevel: String? = nil,
    fastMode: SteelEngineChatFastMode? = nil,
    effectiveFastMode: SteelEngineChatFastMode? = nil,
    totalTokens: Int? = nil,
    totalTokensFresh: Bool? = nil,
    contextTokens: Int? = nil) -> SteelEngineChatSessionEntry
{
...
private func modelChoice(
    ...
    reasoning: Bool? = nil) -> SteelEngineChatModelChoice
{
...
private func sessionsResponse(
    _ session: SteelEngineChatSessionEntry,
    defaults: SteelEngineChatSessionsDefaults? = nil) -> SteelEngineChatSessionsListResponse
{
    SteelEngineChatSessionsListResponse(
        ts: 1,
        path: nil,
...
private func commandChoice(
    ...
    aliases: [String],
    description: String = "",
    source: SteelEngineChatCommandChoice.Source = .command,
    acceptsArgs: Bool = false) -> SteelEngineChatCommandChoice
{
    SteelEngineChatCommandChoice(
...
private func makeViewModel(
    ...
    historyResponses: [SteelEngineChatHistoryPayload],
    sessionRoutingContract: String? = nil,
    sessionsResponses: [SteelEngineChatSessionsListResponse] = [],
    modelResponses: [[SteelEngineChatModelChoice]] = [],
    modelPatchResults: [SteelEngineChatModelPatchResult?] = [],
    thinkingPatchResults: [SteelEngineChatModelPatchResult?] = [],
    commandResponses: [[SteelEngineChatCommandChoice]] = [],
    requestHistoryHook: (@Sendable (String) async throws -> Void)? = nil,
    historyResponseHook: (@Sendable (String, Int, [String]) async throws -> SteelEngineChatHistoryPayload?)? = nil,
    ...
    sessionSettingsPatchHook: (
        @Sendable (SteelEngineChatSessionSettingsPatch) async throws -> SteelEngineChatModelPatchResult?)? = nil,
    ...
    listSessionsHook: (
        @Sendable (TestSessionListQuery) async throws -> SteelEngineChatSessionsListResponse?)? = nil,
    sendMessageHook: (@Sendable (String) async throws -> SteelEngineChatSendResponse)? = nil,
    sendMessageStatus: String = "ok",
    waitForRunCompletionHook: (@Sendable (String, Int) async -> SteelEngineChatRunObservation)? = nil,
    ...
    -> (TestChatTransport, SteelEngineChatViewModel)
{
...
private func sendUserMessage(_ vm: SteelEngineChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, SteelEngineChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SteelEngineChatHistoryPayload]
    private let sessionsResponses: [SteelEngineChatSessionsListResponse]
    private let modelResponses: [[SteelEngineChatModelChoice]]
    private let modelPatchResults: [SteelEngineChatModelPatchResult?]
    private let thinkingPatchResults: [SteelEngineChatModelPatchResult?]
    private let commandResponses: [[SteelEngineChatCommandChoice]]
    private let requestHistoryHook: (@Sendable (String) async throws -> Void)?
    private let historyResponseHook:
        (@Sendable (String, Int, [String]) async throws -> SteelEngineChatHistoryPayload?)?
    ...
    private let sessionSettingsPatchHook:
        (@Sendable (SteelEngineChatSessionSettingsPatch) async throws -> SteelEngineChatModelPatchResult?)?
    ...
    private let listSessionsHook:
        (@Sendable (TestSessionListQuery) async throws -> SteelEngineChatSessionsListResponse?)?
    private let sendMessageHook: (@Sendable (String) async throws -> SteelEngineChatSendResponse)?
    private let sendMessageStatus: String
    private let waitForRunCompletionHook:
        (@Sendable (String, Int) async -> SteelEngineChatRunObservation)?
    ...
    private let stream: AsyncStream<SteelEngineChatTransportEvent>
    private let continuation: AsyncStream<SteelEngineChatTransportEvent>.Continuation
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<SteelEngineChatTransportEvent>.Continuation!
        ...
        worktree _: Bool?) async throws -> SteelEngineChatCreateSessionResponse
    {
        ...
        return SteelEngineChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SteelEngineChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SteelEngineChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [SteelEngineChatAttachmentPayload]) async throws -> SteelEngineChatSendResponse
    {
        ...
        return SteelEngineChatSendResponse(runId: idempotencyKey, status: self.sendMessageStatus)
    }
        ...
        archived: Bool) async throws -> SteelEngineChatSessionsListResponse
    {
        ...
        return self.sessionsResponses.last ?? SteelEngineChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
        ...
        patch: SteelEngineChatSessionSettingsPatch) async throws -> SteelEngineChatModelPatchResult?
    {
        ...
        var result: SteelEngineChatModelPatchResult?
        ...
        if let thinkingLevelUpdate = patch.thinkingLevel {
            ...
            let thinkingResult = try await self.patchSessionThinking(
                sessionKey: sessionKey,
                thinkingLevel: thinkingLevel)
            result = SteelEngineChatModelPatchResult(
    ...
    private func patchSessionThinking(
        sessionKey: String,
        thinkingLevel: String) async throws -> SteelEngineChatModelPatchResult?
    {
        ...
        return SteelEngineChatModelPatchResult(
            key: sessionKey,
            modelProvider: nil,
            model: nil,
            thinkingLevel: thinkingLevel)
    ...
    func emit(_ evt: SteelEngineChatTransportEvent) {
...
struct ChatViewModelTests {
    @Test @MainActor func `locally expired question is evicted after terminal grace`() {
        let viewModel = SteelEngineChatViewModel(
            sessionKey: "main",
    ...
    @Test func `context usage fraction validates freshness and token bounds`() {
        func fraction(total: Int?, fresh: Bool? = true, context: Int?) -> Double? {
            SteelEngineChatViewModel.chatContextUsageFraction(
                for: sessionEntry(
                    key: "main",
                    updatedAt: 1,
                    totalTokens: total,
    ...
    @Test @MainActor func `event listener does not retain discarded view model`() async throws {
        ...
        var viewModel: SteelEngineChatViewModel? = SteelEngineChatViewModel(
            sessionKey: "main",
            transport: transport)
    ...
    @Test func `decodes in-flight run from chat history`() throws {
        ...
        let payload = try JSONDecoder().decode(SteelEngineChatHistoryPayload.self, from: data)

        #expect(payload.inFlightRun?.runId == "run-active")
        #expect(payload.inFlightRun?.text == "partial")
        #expect(payload.inFlightRun?.plan?.steps == [
            SteelEngineChatPlanStep(step: "Reconnect", status: .inProgress),
        ])
    ...
    @Test func `decodes agent scope from chat event`() throws {
        ...
        let payload = try JSONDecoder().decode(SteelEngineChatEventPayload.self, from: data)
    ...
    @Test func `bootstrap adopts active history run and consumes live events`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-active", text: "partial reply"))
    ...
    @Test func `adopts Codex history run without buffered text`() async throws {
        let history = historyPayload(
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-codex", text: ""))
    ...
    @Test func `bootstrap adopts in-flight plan snapshot`() async throws {
        let history = historyPayload(
            inFlightRun: SteelEngineChatInFlightRun(
                runId: "run-plan",
                text: "working",
                plan: SteelEngineChatPlanSnapshot(
                    steps: [
                        SteelEngineChatPlanStep(step: "Inspect", status: .completed),
                        SteelEngineChatPlanStep(step: "Reconnect", status: .inProgress),
                    ],
    ...
    @Test func `history plan reconciliation contract`() async {
        let retainedSteps = [SteelEngineChatPlanStep(step: "Retained", status: .inProgress)]
        let liveSteps = [SteelEngineChatPlanStep(step: "New live plan", status: .inProgress)]
        let cases: [(
            name: String,
            payload: SteelEngineChatHistoryPayload,
            expectedRunId: String?,
            expectedSteps: [SteelEngineChatPlanStep],
            ...
            (
                ...
                historyPayload(
                    inFlightRun: SteelEngineChatInFlightRun(
                        ...
                        text: "working",
                        plan: SteelEngineChatPlanSnapshot(
                            steps: [SteelEngineChatPlanStep(step: "Replacement", status: .completed)]))),
                "run-retained",
                [SteelEngineChatPlanStep(step: "Replacement", status: .completed)],
                false),
            (
                "legacy-preserve",
                historyPayload(
                    inFlightRun: SteelEngineChatInFlightRun(runId: "run-retained", text: "working")),
                "run-retained",
                retainedSteps,
                false),
            (
                ...
                historyPayload(
                    inFlightRun: SteelEngineChatInFlightRun(
                        ...
                        text: "next",
                        plan: SteelEngineChatPlanSnapshot(
                            steps: [SteelEngineChatPlanStep(step: "Next run", status: .inProgress)]))),
                "run-next",
                [SteelEngineChatPlanStep(step: "Next run", status: .inProgress)],
                false),
            (
                "active-preserve",
            ...
            (
                ...
                historyPayload(
                    inFlightRun: SteelEngineChatInFlightRun(
                        runId: "run-retained",
                        text: "working",
                        plan: SteelEngineChatPlanSnapshot(steps: []))),
                nil,
                [],
                false),
        ]
    ...
    @Test func `foreground history refreshes adopted run snapshot`() async throws {
        let firstHistory = historyPayload(
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-active", text: "first partial"))
        let resumedHistory = historyPayload(
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-active", text: "resumed partial"))
        let historyCalls = AsyncCounter()
        ...
        let (_, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, sentRunIds in
                ...
                return historyPayload(
                    ...
                    inFlightRun: SteelEngineChatInFlightRun(runId: runId, text: "working"))
            },
    ...
    @Test func `foreground discovers run started without local ownership`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-external", text: "external partial"))
    ...
    @Test func `foreground keeps active run with intermediate assistant history`() async throws {
        ...
        let activeHistory = historyPayload(
            ...
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-tool", text: "still working"))
        let (_, vm) = await makeViewModel(historyResponses: [idleHistory, activeHistory])
    ...
    @Test func `foreground synthesizes activity when no run snapshot or local run exists`() async throws {
        ...
        transport.emit(
            .sessionMessage(
                SteelEngineSessionMessageEventPayload(
                    sessionKey: "main",
    ...
    @Test func `foreground clears completed run without assistant output`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: SteelEngineChatInFlightRun(
                ...
                text: "",
                plan: SteelEngineChatPlanSnapshot(
                    steps: [SteelEngineChatPlanStep(step: "Finish", status: .inProgress)])))
        let completedHistory = historyPayload(
            messages: [chatTextMessage(role: "user", text: "quiet task", timestamp: 1)],
            hasActiveRun: false)
    ...
    @Test func `foreground missing snapshot does not clear an in-flight send`() async throws {
        ...
        let historyCalls = AsyncCounter()
        let activeHistory = historyPayload(
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-active", text: "accepted"))
        let (_, vm) = await makeViewModel(
            historyResponses: [historyPayload(), historyPayload(), activeHistory],
            requestHistoryHook: { _ in _ = await historyCalls.increment() },
            sendMessageHook: { _ in
                await sendGate.wait()
                return SteelEngineChatSendResponse(runId: "run-active", status: "pending")
            })
    ...
    @Test func `post-send history keeps active run with intermediate assistant output`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-active", text: "working"))
        let (_, vm) = await makeViewModel(
            historyResponses: [historyPayload(), activeHistory],
            sendMessageHook: { _ in
                SteelEngineChatSendResponse(runId: "run-active", status: "pending")
            })
    ...
    @Test func `post-send stale inactive history preserves newer live plan`() async throws {
        ...
        #expect(await MainActor.run { vm.planSteps == [
            SteelEngineChatPlanStep(step: "Finish", status: .inProgress),
        ] })
    }
    ...
    @Test func `live chat delta owns run while bootstrap history is pending`() async throws {
        ...
        let staleHistory = historyPayload(
            ...
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-stale", text: "stale partial"))
        let (transport, vm) = await makeViewModel(
    ...
    @Test func `global chat delta adopts only selected agent run`() async throws {
        let bareGlobalMatches = await MainActor.run {
            (
                SteelEngineChatViewModel.matchesCurrentSessionKey(
                    incoming: "global",
                    agentId: "work",
                    current: "global",
                    mainSessionKey: "main",
    ...
    @Test func `intermediate session message preserves pending recovery snapshot`() async throws {
        ...
        let historyCalls = AsyncCounter()
        let activeHistory = historyPayload(
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-active", text: "active partial"))
        let (transport, vm) = await makeViewModel(
    ...
    @Test func `manual refresh re-adopts active run after clearing local ownership`() async throws {
        ...
        let refreshedHistory = historyPayload(
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-active", text: "refreshed partial"))
    ...
    @Test func `older history cannot replace newer run snapshot`() async throws {
        ...
        let initialHistory = historyPayload(
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-initial", text: "initial"))
        let (_, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, _ in
                if index == 1 {
                    ...
                    return historyPayload(
                        inFlightRun: SteelEngineChatInFlightRun(runId: "run-older", text: "older"))
                }
                if index == 2 {
                    return historyPayload(
                        inFlightRun: SteelEngineChatInFlightRun(runId: "run-newer", text: "newer"))
                }
                return nil
            })
    ...
    @Test func `delayed history cannot overwrite newer live run text`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-active", text: "initial"))
        let (transport, vm) = await makeViewModel(
            historyResponses: [activeHistory],
            requestHistoryHook: { _ in _ = await historyCalls.increment() },
            historyResponseHook: { _, index, _ in
                ...
                return historyPayload(
                    inFlightRun: SteelEngineChatInFlightRun(runId: "run-active", text: "stale"))
            })
    ...
    @Test func `external terminal event protects current run from delayed snapshot`() async throws {
        ...
        let currentHistory = historyPayload(
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-current", text: "current"))
        let (transport, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, _ in
                if index == 1 {
                    ...
                    _ = await staleCompletions.increment()
                    return historyPayload(
                        inFlightRun: SteelEngineChatInFlightRun(runId: "run-finished", text: "stale"))
                }
                if index == 2 {
                    throw NSError(domain: "test", code: 1)
                }
    ...
    @Test func `sequence gap re-adopts active run from history`() async throws {
        ...
        let recoveredHistory = historyPayload(
            inFlightRun: SteelEngineChatInFlightRun(runId: "run-recovered", text: "recovered partial"))
    ...
    @Test func `displays error message fallback only for assistant error turns`() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> SteelEngineChatMessage {
        ...
        #expect(
            SteelEngineChatMessage.errorDisplayText(
        ...
        #expect(
            SteelEngineChatMessage.displayText(
                contentText: "",
                role: assistantError.role,
                stopReason: assistantError.stopReason,
                errorMessage: assistantError.errorMessage) == "stale provider failure")
    ...
    @Test func `duplicate final events append one provisional reply`() async throws {
        ...
        await sendUserMessage(vm, text: "hello")
        let runId = try await waitForLastSentRunId(transport)
        let final = SteelEngineChatEventPayload(
            runId: runId,
            sessionKey: "main",
            state: "final",
            message: chatTextMessage(
    ...
    @Test func `later identical session reply does not adopt prior turn provisional final`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { runId in
                SteelEngineChatSendResponse(runId: runId, status: "pending")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `plan event parses typed and legacy steps`() async throws {
        ...
        #expect(await MainActor.run { vm.planSteps } == [
            SteelEngineChatPlanStep(step: "Inspect state", status: .inProgress),
            SteelEngineChatPlanStep(step: "Write fix", status: .pending),
            SteelEngineChatPlanStep(step: "Verify", status: .completed),
        ])
    ...
    @Test func `plan snapshots replace and empty snapshot clears`() async throws {
        ...
        try await waitUntil("replacement plan snapshot applies") {
            await MainActor.run {
                vm.planSteps == [SteelEngineChatPlanStep(step: "Replacement", status: .pending)] &&
    ...
    @Test func `terminal event for another run preserves active plan`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: SteelEngineChatInFlightRun(
                ...
                text: "",
                plan: SteelEngineChatPlanSnapshot(
                    steps: [SteelEngineChatPlanStep(step: "Keep working", status: .inProgress)])))
        let (transport, vm) = await makeViewModel(
            historyResponses: [initialHistory, activeHistory, activeHistory],
            sendMessageHook: { _ in
                SteelEngineChatSendResponse(runId: activeRunId, status: "pending")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        ...
        #expect(await MainActor.run { vm.pendingRunCount } == 1)
        #expect(await MainActor.run { vm.planSteps } == [
            SteelEngineChatPlanStep(step: "Keep working", status: .inProgress),
        ])
    }
    ...
    @Test func `rekeys optimistic user message when gateway reuses active run`() async throws {
        ...
        let (_, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                ...
                return SteelEngineChatSendResponse(runId: remoteRunId, status: "in_flight")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `canonical projected reply after steering adopts reused provisional final`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                SteelEngineChatSendResponse(runId: remoteRunId, status: "in_flight")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `terminal timeout send ack surfaces error and allows next send`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { runId in
                ...
                return SteelEngineChatSendResponse(
                    runId: runId,
    ...
    @Test @MainActor func `bounded repeated same text reply invalidates older stale refresh`() async throws {
        ...
        transport.emit(SteelEngineChatTransportEvent.seqGap)
    ...
    @Test func `session choices prefer main and recent`() async throws {
        ...
        let history = historyPayload()
        let sessions = SteelEngineChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func `new trigger falls back to reset when create session is unsupported`() async throws {
        ...
        let unsupported = NSError(
            domain: "SteelEngineChatTransport",
            code: 0,
    ...
    @Test func `bootstraps model selection from session and defaults`() async throws {
        ...
        let sessions = SteelEngineChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: SteelEngineChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `selecting default model patches nil and updates selection`() async throws {
        ...
        await MainActor.run { vm.selectModel(SteelEngineChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == SteelEngineChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func `selecting provider qualified model disambiguates duplicate model I ds`() async throws {
        ...
        let sessions = SteelEngineChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: SteelEngineChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `distinct model patches are serialized in selection order`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            modelResponses: [models],
            modelPatchResults: [
                SteelEngineChatModelPatchResult(
            ...
            historyResponseHook: { _, index, sentRunIds in
                ...
                if (1...3).contains(index) {
                    ...
                    return historyPayload(
                        ...
                        sessionId: sessionId,
                        messages: [chatTextMessage(role: "user", text: "hello", timestamp: now)],
                        inFlightRun: SteelEngineChatInFlightRun(runId: runId, text: ""))
                }
    ...
    @Test func `late model completion does not replay current session selection into previous session`() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = SteelEngineChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = SteelEngineChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func `explicit thinking level wins over history and persists changes`() async throws {
        let history = SteelEngineChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test @MainActor func `Ultra is canonical while ultrathink remains a high alias`() {
        #expect(SteelEngineChatViewModel.normalizedThinkingLevel("ultra") == "ultra")
        #expect(SteelEngineChatViewModel.normalizedThinkingLevel("ULTRA") == "ultra")
        #expect(SteelEngineChatViewModel.normalizedThinkingLevel("ultrathink") == "high")
        #expect(
            SteelEngineChatViewModel.normalizedThinkingLevel(
                "ultra",
    ...
    @Test func `decodes authoritative model patch thinking state`() throws {
        ...
        let result = try JSONDecoder().decode(SteelEngineChatModelPatchResult.self, from: data)
    ...
    @Test func `decodes gateway thinking metadata from session list`() throws {
        ...
        let decoded = try JSONDecoder().decode(
            SteelEngineChatSessionsListResponse.self,
            from: Data(json.utf8))
        ...
        try await waitUntil("models loaded with default selection") {
            await MainActor.run {
                vm.modelChoices.count == 2 &&
                    vm.modelSelectionID == SteelEngineChatViewModel.defaultModelSelectionID
            }
        }
    ...
    @Test func `default settings patch returns accepted thinking state`() async throws {
        ...
        let result = try await transport.patchSessionSettings(
            sessionKey: "main",
            agentID: nil,
            patch: SteelEngineChatSessionSettingsPatch(thinkingLevel: .some("high")))
    ...
    @Test func `default thinking selection clears override and adopts resolved level`() async throws {
        ...
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        await MainActor.run {
            vm.selectThinkingLevel(SteelEngineChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")

        #expect(await MainActor.run { vm.thinkingSelectionID } ==
            SteelEngineChatViewModel.inheritedThinkingSelectionID)
        #expect(await MainActor.run { vm.thinkingLevel } == "medium")
        #expect(await MainActor.run { vm.sessions.first?.thinkingLevel } == nil)
    ...
    @Test func `inherited verbosity does not masquerade as persisted override`() async throws {
        ...
        #expect(await MainActor.run { vm.verboseLevel } == SteelEngineChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `fast and verbosity rejection restores inherited overrides`() async throws {
        ...
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.fastModeSelectionID } == SteelEngineChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `fast and verbosity default selections clear overrides`() async throws {
        ...
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        await MainActor.run {
            vm.selectFastMode(SteelEngineChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.sessions.first?.fastMode } == nil)
        ...
        await MainActor.run {
            vm.selectVerboseLevel(SteelEngineChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.sessions.first?.verboseLevel } == nil)
        #expect(await MainActor.run { vm.verboseLevel } == SteelEngineChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `strips inbound metadata from history messages`() async throws {
        ...
        let vm = await MainActor.run { SteelEngineChatViewModel(sessionKey: "main", transport: transport) }
...
struct ChatViewModelSessionManagementTests {
    @Test @MainActor func `session list organizer orders pinned first with key tiebreak`() {
        let organized = SteelEngineChatSessionListOrganizer.organize([
    ...
    @Test @MainActor func `session list organizer filters across display fields`() {
        ...
        let matched = SteelEngineChatSessionListOrganizer.filter(sessions, search: "TRIP")
        #expect(matched.map(\.key) == ["agent:main:topic-a", "agent:main:trip-notes"])
        #expect(SteelEngineChatSessionListOrganizer.filter(sessions, search: "  ") == sessions)
    }
