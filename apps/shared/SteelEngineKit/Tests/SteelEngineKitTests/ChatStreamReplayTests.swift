...
/// Replays scripted gateway traffic against `SteelEngineChatViewModel` with deterministic
...
private final class ScriptedChatTransport: @unchecked Sendable, SteelEngineChatTransport {
    private actor State {
        var history: SteelEngineChatHistoryPayload
        var historyRequestCount = 0
        var sentRunIds: [String] = []

        init(history: SteelEngineChatHistoryPayload) {
            self.history = history
        }

        func setHistory(_ payload: SteelEngineChatHistoryPayload) {
            self.history = payload
        }

        func recordHistoryRequest() -> SteelEngineChatHistoryPayload {
    ...
    func setHistory(_ payload: SteelEngineChatHistoryPayload) async {
    ...
    // MARK: SteelEngineChatTransport

    func requestHistory(sessionKey _: String) async throws -> SteelEngineChatHistoryPayload {
        ...
        attachments _: [SteelEngineChatAttachmentPayload]) async throws -> SteelEngineChatSendResponse
    {
        ...
        return SteelEngineChatSendResponse(runId: idempotencyKey, status: "pending")
    }
    ...
    {
        SteelEngineChatSessionsListResponse(ts: nil, path: nil, count: 0, defaults: nil, sessions: [])
    }
...
private func replayHistory(
    ...
    messages: [AnyCodable] = []) -> SteelEngineChatHistoryPayload
{
...
/// `__steelengine.idempotencyKey` metadata used for turn correlation.
...
private func replaySessionMessageEvent(
    ...
    messageId: String) -> SteelEngineChatTransportEvent
{
    .sessionMessage(
...
private func replayFinalEvent(
    runId: String,
    text: String,
    timestamp: Double) -> SteelEngineChatTransportEvent
{
    .chat(
...
private func replayAssistantDeltaEvent(
    ...
    seq: Int) -> SteelEngineChatTransportEvent
{
    .agent(
...
private struct StreamReplayHarness {
    ...
    static func bootstrapped(
        initialHistory: SteelEngineChatHistoryPayload = replayHistory()) async throws -> StreamReplayHarness
    {
        let transport = ScriptedChatTransport(history: initialHistory)
        let vm = await MainActor.run {
            SteelEngineChatViewModel(sessionKey: "main", transport: transport)
        }
    ...
    func converge(
        _ label: String,
        _ condition: @escaping @MainActor @Sendable (SteelEngineChatViewModel) -> Bool) async throws
    {
        let vm = self.vm
...
extension SteelEngineChatViewModel {
    fileprivate var replayAssistantRows: [SteelEngineChatMessage] {
        self.messages.filter { $0.role == "assistant" }
    }

    fileprivate func replayAssistantRows(text: String) -> [SteelEngineChatMessage] {
        self.replayAssistantRows.filter { message in
            message.content.compactMap(\.text).joined() == text
        }
    }

    fileprivate var replayUserRows: [SteelEngineChatMessage] {
