...
private final class ContextUsageTestTransport: @unchecked Sendable, SteelEngineChatTransport {
...
struct ChatContextUsageTests {
    private func message(
        ...
        usage: SteelEngineChatUsage? = nil) -> SteelEngineChatMessage
    {
    ...
    private func usage(
        ...
        cacheWrite: Int? = nil,
        total: Int? = nil,
        costTotal: Double? = nil) throws -> SteelEngineChatUsage
    {
        ...
        return try JSONDecoder().decode(SteelEngineChatUsage.self, from: data)
    }
    ...
    @Test @MainActor func `view model resolves context totals through a selected global alias`() {
        let vm = SteelEngineChatViewModel(
            ...
            activeAgentId: "ops")
        vm.sessions = [SteelEngineChatSessionEntry(
