...
@Suite struct DeepLinksSecurityTests {
    ...
    @Test func gatewayDeepLinkAllowsLoopbackWs() {
        let url = URL(
            string: "steelengine://gateway?host=127.0.0.1&port=18789&tls=0&token=abc")!
        #expect(
    ...
    @Test func setupCodeAllowsPrivateLanHostPayload() {
        let payload = #"{"host":"steelengine.local","port":18789,"tls":false,"bootstrapToken":"tok"}"#
        #expect(
            GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload)) == .init(
                host: "steelengine.local",
                port: 18789,
