...
private func pickerModel(_ selectionID: String) -> SteelEngineChatModelChoice {
...
@Suite struct ChatModelPickerStoreTests {
    ...
    @Test func `recents dedupe move to front cap and skip invalid ids`() throws {
        try withPickerStore { store, _ in
            store.recordRecent("")
            store.recordRecent(SteelEngineChatViewModel.defaultModelSelectionID)
    ...
    @Test func `missing default provider does not mark other as default`() {
        let unqualified = SteelEngineChatModelChoice(
            modelID: "local-model",
            name: "Local",
            provider: "",
            contextWindow: nil)
    ...
    @Test func `qualified global default matches provider model pair`() {
        ...
        let qualifiedModelID = SteelEngineChatModelChoice(
            modelID: "openai/gpt",
            name: "GPT",
            provider: "openai",
            contextWindow: nil)
