...
private final class HapticRecorder: @unchecked Sendable {
    ...
    private var recordedEvents: [SteelEngineChatHaptics.Event] = []

    var events: [SteelEngineChatHaptics.Event] {
        self.lock.lock()
        defer { self.lock.unlock() }
        return self.recordedEvents
    }

    func record(_ event: SteelEngineChatHaptics.Event) {
        self.lock.lock()
        defer { self.lock.unlock() }
...
private final class HapticsTestTransport: @unchecked Sendable, SteelEngineChatTransport {
    private let response: SteelEngineChatSendResponse
    ...
    init(status: String, historyMessages: @escaping @Sendable () -> [AnyCodable] = { [] }) {
        self.response = SteelEngineChatSendResponse(runId: "run-1", status: status)
        self.historyMessages = historyMessages
        var continuation: AsyncStream<SteelEngineChatTransportEvent>.Continuation!
    ...
    HapticRecorder)
{
    ...
    let haptics = SteelEngineChatHaptics(performer: recorder.record)
    let viewModel = await MainActor.run {
        SteelEngineChatViewModel(sessionKey: "main", transport: transport, haptics: haptics)
    }
    return (transport, viewModel, recorder)
}

private func sendHapticsTestMessage(_ viewModel: SteelEngineChatViewModel) async {
    await MainActor.run {
...
struct ChatHapticsTests {
    ...
    @Test func `completion fires once for duplicate terminal events`() async throws {
        ...
        let final = SteelEngineChatTransportEvent.chat(SteelEngineChatEventPayload(
            runId: "run-1",
            sessionKey: "main",
            state: "final",
            message: nil,
