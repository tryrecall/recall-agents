...
final class VoiceNoteRecorderTests: XCTestCase {
    ...
    func testDurationLabelBoundsMalformedHistoryValues() {
        XCTAssertEqual(steelEngineVoiceNoteDurationLabel(.infinity), "0:00")
        XCTAssertEqual(steelEngineVoiceNoteDurationLabel(.nan), "0:00")
        XCTAssertEqual(steelEngineVoiceNoteDurationLabel(1e100), "3:00")
        XCTAssertEqual(steelEngineVoiceNoteDurationLabel(-1), "0:00")
    }
    ...
    func testRecordButtonRequiresAttachmentInput() {
        let recorder = SteelEngineVoiceNoteRecorder(capture: FakeVoiceNoteAudioCapture())
        let control = SteelEngineChatVoiceNoteControl(recorder: recorder, isTalkActive: false)
        let button = SteelEngineVoiceNoteButton(
            control: control,
            compact: false,
