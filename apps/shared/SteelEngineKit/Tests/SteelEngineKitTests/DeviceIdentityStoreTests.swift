...
struct DeviceIdentityStoreTests {
    ...
    func `device auth owners preserve exact unicode bytes`() throws {
        ...
        let stateDirPath = try #require(getenv("STEELENGINE_STATE_DIR").map { String(cString: $0) })
    ...
    func `secondary profiles use separate identity and auth files`() throws {
        ...
        // getenv, not ProcessInfo: the trait pins STEELENGINE_STATE_DIR via setenv and
    ...
    // Regression: an explicit STEELENGINE_STATE_DIR override must never import the
