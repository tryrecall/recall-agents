...
struct SteelEngineMascotAnimatorTests {
    private func makeAnimator(seed: UInt64 = 7, interactive: Bool = false) -> SteelEngineMascotAnimator {
        SteelEngineMascotAnimator(seed: seed, hourOfDay: 12, allowsAutoSleep: interactive)
    }

    @Test func `poses stay inside drawable bounds for every mood`() {
        for mood in SteelEngineMascotMood.allCases {
    ...
    @Test func `sleepy mood wears the nightcap`() {
        ...
        let staticPose = SteelEngineMascotPose.staticPose(for: .sleepy)
    ...
    @Test func `static poses carry the mood signature`() {
        let sad = SteelEngineMascotPose.staticPose(for: .sad)
        #expect(sad.antennaDroop > 0)
        #expect(sad.mouthCurve < 0)
        let celebrating = SteelEngineMascotPose.staticPose(for: .celebrating)
        #expect(celebrating.leftClawDegrees > 0)
        #expect(celebrating.mouthCurve > 0)
        let idle = SteelEngineMascotPose.staticPose(for: .idle)
        #expect(idle == SteelEngineMascotPose())
        let working = SteelEngineMascotPose.staticPose(for: .working)
