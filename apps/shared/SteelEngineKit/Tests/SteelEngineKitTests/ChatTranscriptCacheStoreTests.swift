...
private func cacheSessionEntry(key: String, updatedAt: Double) -> SteelEngineChatSessionEntry {
...
private func messageTexts(_ messages: [SteelEngineChatMessage]) -> [String] {
...
private func outboxCommand(
    ...
    text: String,
    attachments: [SteelEngineChatOutboxAttachment] = [],
    thinking: String = "off",
    createdAt: Double = Date().timeIntervalSince1970) -> SteelEngineChatOutboxCommand
{
...
struct ChatTranscriptCacheStoreTests {
    @Test func `verified routing identity survives a cold store reopen`() async throws {
        ...
        let identity = try #require(SteelEngineChatSessionRoutingIdentity(
            ...
            mainSessionKey: " Work ",
            defaultAgentID: " Main "))
        let store = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")

        await store.storeSessionRoutingIdentity(identity)
        #expect(SteelEngineChatSQLiteTranscriptCache.loadSessionRoutingIdentity(
            databaseURL: url,
            gatewayID: "gw-a") == identity)

        let reopened = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
    ...
    @Test func `transcript keeps only most recent messages within bound`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = SteelEngineChatSQLiteTranscriptCache.maxCachedMessagesPerSession
    ...
    @Test func `sessions list is bounded to most recently updated`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = SteelEngineChatSQLiteTranscriptCache.maxCachedSessions
    ...
    @Test func `transcript eviction keeps most recent sessions`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = SteelEngineChatSQLiteTranscriptCache.maxCachedTranscripts
    ...
    @Test func `transcripts are scoped per gateway identity`() async throws {
        let url = try makeDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let storeA = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let storeB = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-b")
    ...
    @Test func `reset retirement permits physical removal of all gateway rows`() async throws {
        ...
        await storeA.retire()
        await storeB.retire()
        SteelEngineChatSQLiteTranscriptCache.removeDatabaseFiles(at: url)
        await storeA.storeTranscript(
            sessionKey: "main",
            messages: [cacheMessage(role: "user", text: "late write", timestamp: 3)])

        let readerA = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let readerB = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-b")
    ...
    @Test func `removing one gateway database preserves another`() async throws {
        ...
        let storeA = SteelEngineChatSQLiteTranscriptCache(databaseURL: urlA, gatewayID: "gw-a")
        let storeB = SteelEngineChatSQLiteTranscriptCache(databaseURL: urlB, gatewayID: "gw-b")
        ...
        await storeA.retire()
        SteelEngineChatSQLiteTranscriptCache.removeDatabaseFiles(at: urlA)

        let readerA = SteelEngineChatSQLiteTranscriptCache(databaseURL: urlA, gatewayID: "gw-a")
    ...
    @Test func `v1 transcript cache migrates to durable outbox schema`() async throws {
        ...
        let migrated = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
    ...
    @Test func `v2 migration parks rows without a routing contract`() async throws {
        ...
        #expect(commands.map(\.lastError) == [
            SteelEngineChatSQLiteTranscriptCache.outboxUnknownTargetError,
            SteelEngineChatSQLiteTranscriptCache.outboxUnknownTargetError,
            SteelEngineChatSQLiteTranscriptCache.outboxUnconfirmedError,
            SteelEngineChatSQLiteTranscriptCache.outboxUnconfirmedError,
            SteelEngineChatSQLiteTranscriptCache.outboxUnknownTargetError,
            SteelEngineChatSQLiteTranscriptCache.outboxUnknownTargetError,
        ])
    ...
    @Test func `unknown schema preserves durable outbox bytes and fails closed`() async throws {
        ...
        let blocked = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        ...
        #expect(sqlite3_exec(
            raw,
            "PRAGMA user_version = \(SteelEngineChatSQLiteTranscriptCache.schemaVersion)",
            nil,
            nil,
            nil) == SQLITE_OK)
        sqlite3_close_v2(raw)
        let recovered = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
    ...
    @Test func `undecodable row is dropped and treated as miss`() async throws {
        ...
        let reader = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
...
struct ChatCommandOutboxStoreTests {
    ...
    @Test func `interrupted sending rows fail closed on recovery`() async throws {
        ...
        #expect(recovered.map(\.lastError) == [SteelEngineChatSQLiteTranscriptCache.outboxUnconfirmedError])
    ...
    @Test func `queued commands expire to failed at the staleness boundary`() async throws {
        ...
        let store = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let now = Date().timeIntervalSince1970
        let maxAge = SteelEngineChatSQLiteTranscriptCache.outboxCommandMaxAge
        #expect(await store.enqueueCommand(
        ...
        #expect(stale.lastError == SteelEngineChatSQLiteTranscriptCache.outboxExpiredError)
        #expect(fresh.status == .queued)
        #expect(fresh.lastError == nil)
        #expect(unconfirmed.status == .failed)
        #expect(unconfirmed.lastError == SteelEngineChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }
    ...
    @Test func `enqueue refuses beyond the queue bound`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = SteelEngineChatSQLiteTranscriptCache.maxQueuedCommands
    ...
    @Test func `attachment byte admission enforces command and gateway bounds`() {
        let commandBound = SteelEngineChatSQLiteTranscriptCache.maxAttachmentBytesPerCommand
        let gatewayBound = SteelEngineChatSQLiteTranscriptCache.maxQueuedAttachmentBytes

        #expect(SteelEngineChatSQLiteTranscriptCache.canEnqueueAttachmentBytes(
    ...
    @Test func `enqueue persists attachment bytes and refuses a full byte budget`() async throws {
        let url = try makeDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let attachment = SteelEngineChatOutboxAttachment(
            type: "file",
            mimeType: "audio/mp4",
        ...
        #expect(sqlite3_exec(
            ...
            SET attachment_bytes = \(SteelEngineChatSQLiteTranscriptCache.maxQueuedAttachmentBytes)
        ...
        let fullStore = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
