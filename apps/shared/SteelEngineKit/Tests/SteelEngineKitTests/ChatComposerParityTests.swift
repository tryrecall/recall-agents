...
private final class ComposerParityTransport: @unchecked Sendable, SteelEngineChatTransport {
        ...
        attachments _: [SteelEngineChatAttachmentPayload]) async throws -> SteelEngineChatSendResponse
    {
        ...
        return SteelEngineChatSendResponse(runId: idempotencyKey, status: "started")
    }
...
struct ChatComposerStateTests {
    @Test func `late send does not consume newer reply selection for same message`() {
        let vm = SteelEngineChatViewModel(sessionKey: "main", transport: ComposerParityTransport())
        let messageID = UUID()
    ...
    @Test func `accepted normal send quotes and clears reply and stored draft`() async throws {
        ...
        let vm = SteelEngineChatViewModel(sessionKey: "main", transport: transport)
        vm.healthOK = true
        vm.input = "continue"
    ...
    @Test func `accepted attachment only send quotes and clears reply`() async throws {
        ...
        let vm = SteelEngineChatViewModel(sessionKey: "main", transport: transport)
        vm.healthOK = true
        vm.attachments = [SteelEnginePendingAttachment(
            url: nil,
            data: Data([1, 2, 3]),
            fileName: "image.png",
            mimeType: "image/png",
