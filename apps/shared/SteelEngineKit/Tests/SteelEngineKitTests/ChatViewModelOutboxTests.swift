...
private func outboxTestCommand(id: String, text: String, createdAt: Double) -> SteelEngineChatOutboxCommand {
    SteelEngineChatOutboxCommand(
        id: id,
        sessionKey: "main",
...
private func userTexts(_ vm: SteelEngineChatViewModel) async -> [String] {
    await MainActor.run {
        vm.messages
            .filter { $0.role == "user" }
...
private final class OutboxTestTransport: @unchecked Sendable, SteelEngineChatTransport {
    let state: OutboxTransportState
    private let sessions: [SteelEngineChatSessionEntry]
    ...
    init(
        ...
        sessions: [SteelEngineChatSessionEntry] = [],
    ...
    func emit(_ event: SteelEngineChatTransportEvent) {
    ...
    func listCommands(sessionKey _: String) async throws -> [SteelEngineChatCommandChoice] {
    ...
    private func requestHistory(
        sessionKey: String,
        agentID: String?,
        expectedRoute: Int?) async throws -> SteelEngineChatHistoryPayload
    {
        ...
        let durableUserRows = keys.indices.compactMap { index -> AnyCodable? in
            ...
            return AnyCodable([
                ...
                "__steelengine": ["idempotencyKey": "\(key):user"],
            ] as [String: Any])
        }
        return SteelEngineChatHistoryPayload(
            sessionKey: sessionKey,
    ...
    private func sendMessage(
        ...
        thinking: String,
        idempotencyKey: String,
        expectedRoute: Int?) async throws -> SteelEngineChatSendResponse
    {
        if let expectedRoute, await state.routeGeneration != expectedRoute {
        ...
        if await self.state.sendRoutingChanged {
            throw GatewayResponseError(
                ...
                details: [
                    "reason": AnyCodable(SteelEngineChatSessionRoutingContract.changedErrorReason),
                ])
        }
        if await self.state.sendRejects {
            // Gateway responded but refused to start the run.
            return SteelEngineChatSendResponse(runId: idempotencyKey, status: "error")
        }
        ...
        return SteelEngineChatSendResponse(runId: idempotencyKey, status: "accepted")
    }
...
private func outboxSessionEntry(
    key: String,
    thinkingLevels: [String]) -> SteelEngineChatSessionEntry
{
    SteelEngineChatSessionEntry(
        ...
        model: nil,
        contextTokens: nil,
        thinkingLevels: thinkingLevels.map { SteelEngineChatThinkingLevelOption(id: $0, label: $0) })
}

private func makeOutboxViewModel(
    transport: OutboxTestTransport,
    outbox: any SteelEngineChatCommandOutbox,
    transcriptCache: (any SteelEngineChatTranscriptCache)? = nil,
    retryDelaysMs: [UInt64] = [1, 1],
    sessionKey: String = "main",
    activeAgentID: String? = "main",
    sessionRoutingContract: String? = "per-sender|main|main") async -> SteelEngineChatViewModel
{
    await MainActor.run {
...
private func sendWhileOffline(_ vm: SteelEngineChatViewModel, text: String) async throws {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
@MainActor
private func queuedStateCount(_ vm: SteelEngineChatViewModel) -> Int {
...
private actor DelayingOutbox: SteelEngineChatCommandOutbox {
    private nonisolated let base: SteelEngineChatSQLiteTranscriptCache
    ...
    init(base: SteelEngineChatSQLiteTranscriptCache) {
        self.base = base
    }

    nonisolated func changes() -> AsyncStream<SteelEngineChatOutboxChange> {
        self.base.changes()
    }
    ...
    func enqueueCommand(_ command: SteelEngineChatOutboxCommand) async -> Bool {
    ...
    func loadCommands() async -> [SteelEngineChatOutboxCommand] {
    ...
    func loadCommandsIfAvailable() async -> [SteelEngineChatOutboxCommand]? {
        if self.loadDelayNanoseconds > 0 {
            try? await Task.sleep(nanoseconds: self.loadDelayNanoseconds)
        }
    ...
    func claimNextCommand() async -> SteelEngineChatOutboxCommand? {
    ...
    func markCommandAwaitingConfirmation(id: String) async -> SteelEngineChatOutboxUpdateResult {
    ...
    func markCommandFailedIfPresent(
        id: String,
        retryCount: Int,
        lastError: String?) async -> SteelEngineChatOutboxUpdateResult
    {
    ...
    func markCommandRetriedIfPresent(
        ...
        routingContract: String) async -> SteelEngineChatOutboxUpdateResult
    {
    ...
    func cancelCommand(id: String) async -> SteelEngineChatOutboxUpdateResult {
        await self.base.cancelCommand(id: id)
    }

    func confirmCommand(id: String) async -> SteelEngineChatOutboxUpdateResult {
...
private actor SnapshotHoldingOutbox: SteelEngineChatCommandOutbox {
    private nonisolated let base: SteelEngineChatSQLiteTranscriptCache
...
private actor CancellationHoldingOutbox: SteelEngineChatCommandOutbox {
    private nonisolated let base: SteelEngineChatSQLiteTranscriptCache
...
struct ChatViewModelOutboxTests {
    @Test func `offline send queues durably and renders queued row`() async throws {
        ...
        let store = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
    ...
    @Test func `unsupported gateway keeps queued work and surfaces upgrade action`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        let message = SteelEngineChatTransportUpgradeMessage.routingContract
    ...
    @Test func `inert outbox does not capability gate healthy live chat`() async throws {
        ...
        let transport = OutboxTestTransport(
            healthy: true,
            routeUnavailableReason: SteelEngineChatTransportUpgradeMessage.routingContract)
        let vm = await makeOutboxViewModel(transport: transport, outbox: store)
        ...
        parked.lastError = SteelEngineChatSQLiteTranscriptCache.outboxChangedTargetError
    ...
    @Test func `reconnect waits for canonical session metadata before flushing Ultra`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        #expect(await store.enqueueCommand(SteelEngineChatOutboxCommand(
            id: "alpha-ultra",
            sessionKey: "main",
    ...
    @Test func `changed default agent parks command visibly for retry`() async throws {
        ...
        #expect(await store.loadCommands().first?.lastError ==
            SteelEngineChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `atomic gateway routing rejection parks without retrying`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == SteelEngineChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `failed alias row remains reachable after owner change`() async throws {
        ...
        #expect(await store.enqueueCommand(SteelEngineChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 1,
            lastError: SteelEngineChatSQLiteTranscriptCache.outboxExpiredError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `ownerless global retry stays failed without a selected agent`() async throws {
        ...
        #expect(await store.enqueueCommand(SteelEngineChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 0,
            lastError: SteelEngineChatSQLiteTranscriptCache.outboxUnknownTargetError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `delete race preserves a turn already proven by canonical history`() async throws {
        ...
        let canonicalMessage = SteelEngineChatMessage(
            role: "user",
    ...
    @Test func `gateway rejections burn attempts then fail terminally and support tap retry`() async throws {
        ...
        #expect(failed.retryCount == SteelEngineChatViewModel.maxOutboxSendAttempts)
    ...
    @Test func `unavailable terminal write drops health instead of advancing FIFO`() async throws {
        ...
        #expect(await store.enqueueCommand(SteelEngineChatOutboxCommand(
            ...
            createdAt: Date().timeIntervalSince1970,
            status: .queued,
            retryCount: SteelEngineChatViewModel.maxOutboxSendAttempts - 1,
            lastError: "rejected")))
        let outbox = DelayingOutbox(base: store)
    ...
    @Test func `ambiguous live transport failure requires explicit retry`() async throws {
        ...
        #expect(preserved.lastError == SteelEngineChatSQLiteTranscriptCache.outboxUnconfirmedError)
    ...
    @Test func `tap retry refreshes createdAt so an expired command can resend`() async throws {
        ...
        let staleCreatedAt = Date().timeIntervalSince1970 -
            SteelEngineChatSQLiteTranscriptCache.outboxCommandMaxAge - 60
        ...
        #expect(await store.loadCommands().map(\.lastError) == [
            SteelEngineChatSQLiteTranscriptCache.outboxExpiredError,
        ])
    ...
    @Test func `background canonical alias event confirms by idempotency key`() async throws {
        ...
        transport.emit(.sessionMessage(SteelEngineSessionMessageEventPayload(
    ...
    @Test func `full queue refuses enqueue and keeps the draft`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = SteelEngineChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        for index in 0..<SteelEngineChatSQLiteTranscriptCache.maxQueuedCommands {
        ...
        #expect(await userTexts(vm).isEmpty)
        #expect(await store.loadCommands().count == SteelEngineChatSQLiteTranscriptCache.maxQueuedCommands)
    }
    ...
    @Test func `queued send transport failure fails closed until explicit retry`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == SteelEngineChatSQLiteTranscriptCache.outboxUnconfirmedError)
        #expect(command.retryCount == 0)
        #expect(await transport.state.sentIdempotencyKeys.isEmpty)
    ...
    @Test func `deleting a queued message removes bubble and durable row`() async throws {
        ...
        await store.storeTranscript(
            sessionKey: "main",
            messages: [SteelEngineChatMessage(
                role: "user",
                content: [SteelEngineChatMessageContent(
                    type: "text",
