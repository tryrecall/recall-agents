...
struct GatewayNodeSessionTests {
    @Test func `operator canvas refresh uses the operator surface method`() async throws {
        ...
        try task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__steelengine__/cap/operator-token",
                ],
            ])
    ...
    @Test func `canvas surface refresh is shared across callers with different timeouts`() async throws {
        ...
        task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__steelengine__/cap/new-token",
                ],
            ])
    ...
    @Test func `last timed out surface waiter releases stalled rotation for retry`() async throws {
        ...
        try task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__steelengine__/cap/retry-token",
                ],
            ])
    ...
    @Test func `lagging canvas refresh reuses the rotated capability`() async throws {
        ...
        try task.emitResponse(
            ...
            payload: [
                "surface": "canvas",
                "pluginSurfaceUrls": [
                    "canvas": "http://gateway.example.invalid/__steelengine__/cap/old-token",
                ],
            ])
    ...
    func `watch approval warning text is optional and round trips`() throws {
        let legacy = try JSONDecoder().decode(
            SteelEngineWatchExecApprovalItem.self,
    ...
    func `watch approval recovery schema carries exact resolution attempt identifiers`() throws {
        ...
        let prompt = SteelEngineWatchExecApprovalPromptMessage(
        ...
        let request = SteelEngineWatchExecApprovalSnapshotRequestMessage(
            requestId: "request",
            ...
            await gateway.invokeIfCurrentRoute(
                ...
                onInvoke: { request in
                    ...
                    return BridgeInvokeResponse(
                        id: request.id,
                        ok: false,
                        error: SteelEngineNodeError(code: .unavailable, message: "UNAVAILABLE: route changed"))
                })
        }
    ...
    func `replacement invoke fails promptly while disconnect lifecycle is blocked`() async throws {
        ...
        #expect(error["code"] as? String == SteelEngineNodeErrorCode.unavailable.rawValue)
    ...
    func `computer invoke receipts deduplicate in flight and after reconnect`() async throws {
        ...
        #expect(mismatchError["code"] as? String == SteelEngineNodeErrorCode.invalidRequest.rawValue)
        #expect(await probe.count() == 1)
    ...
    func `connect failure preserves protocol mismatch details`() async throws {
        ...
        do {
            ...
            #expect(problem?
                .message == "This app is older than the gateway. Update SteelEngine on this device, then retry.")
    ...
    func `failed device token write is not reported as an issued role`() async throws {
        ...
        setenv("STEELENGINE_STATE_DIR", blocker.path, 1)
    ...
    func `normalize canvas host url preserves explicit secure canvas port`() {
        let normalized = GatewayPluginSurfaceURL.canonicalize(
            raw: "https://canvas.example.com:9443/__steelengine__/cap/token",
            against: URL(string: "wss://gateway.example.com"))

        #expect(normalized == "https://canvas.example.com:9443/__steelengine__/cap/token")
    }

    @Test
    func `normalize canvas host url backfills gateway host for loopback canvas`() {
        let normalized = GatewayPluginSurfaceURL.canonicalize(
            raw: "http://127.0.0.1:18789/__steelengine__/cap/token",
            against: URL(string: "wss://gateway.example.com:7443"))

        #expect(normalized == "https://gateway.example.com:7443/__steelengine__/cap/token")
    }
    ...
    func `gateway TLS pin applies only to the same canvas endpoint`() {
        ...
        #expect(GatewayPluginSurfaceURL.tlsFingerprintForSurface(
            fingerprint,
            surfaceURL: "https://gateway.example.com:7443/__steelengine__/cap/token",
            gatewayURL: gateway) == fingerprint)
        #expect(GatewayPluginSurfaceURL.tlsFingerprintForSurface(
            fingerprint,
            surfaceURL: "https://canvas.example.com:7443/__steelengine__/cap/token",
            gatewayURL: gateway) == nil)
        #expect(GatewayPluginSurfaceURL.tlsFingerprintForSurface(
            fingerprint,
            surfaceURL: "https://gateway.example.com:9443/__steelengine__/cap/token",
            gatewayURL: gateway) == nil)
    }
