...
private func cachedMessage(role: String, text: String, timestamp: Double) -> SteelEngineChatMessage {
...
private func visibleTexts(_ vm: SteelEngineChatViewModel) async -> [String] {
    await MainActor.run { vm.messages.map { $0.content.compactMap(\.text).joined() } }
}

private func cachedSessionEntry(key: String, updatedAt: Double) -> SteelEngineChatSessionEntry {
...
private actor TestTranscriptCache: SteelEngineChatTranscriptCache {
    private var transcripts: [String: [SteelEngineChatMessage]]
    private var sessions: [SteelEngineChatSessionEntry]
    private let loadSessionsHook: (@Sendable () async -> Void)?
    private(set) var storedTranscriptSessionKeys: [String] = []
    private(set) var storedTranscripts: [[SteelEngineChatMessage]] = []
    private(set) var storedSessionsCallCount = 0

    init(
        transcripts: [String: [SteelEngineChatMessage]] = [:],
...
private final class GatedHistoryChatTransport: @unchecked Sendable, SteelEngineChatTransport {
    private let historyGate: AsyncStream<Void>.Continuation?
    private let historyGateStream: AsyncStream<Void>?
    private let historyResult: @Sendable (String, Int) throws -> SteelEngineChatHistoryPayload
    ...
    init(
        gated: Bool,
        historyResult: @escaping @Sendable (String, Int) throws -> SteelEngineChatHistoryPayload)
    {
        ...
        var eventCont: AsyncStream<SteelEngineChatTransportEvent>.Continuation!
    ...
    {
        SteelEngineChatSendResponse(runId: idempotencyKey, status: "accepted")
    }
...
struct ChatViewModelTranscriptCacheTests {
    @Test func `cold open paints cached transcript then live history replaces it`() async throws {
        ...
        let vm = await MainActor.run {
            SteelEngineChatViewModel(sessionKey: "main", transport: transport, transcriptCache: cache)
        }
