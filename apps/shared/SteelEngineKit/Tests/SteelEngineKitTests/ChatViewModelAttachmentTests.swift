...
private actor AttachmentSendCapture {
    private(set) var attachments: [SteelEngineChatAttachmentPayload] = []

    func store(_ attachments: [SteelEngineChatAttachmentPayload]) {
    ...
    func first() -> SteelEngineChatAttachmentPayload? {
...
private struct AttachmentProcessingTransport: SteelEngineChatTransport {
        ...
        attachments: [SteelEngineChatAttachmentPayload]) async throws -> SteelEngineChatSendResponse
    {
        ...
        return SteelEngineChatSendResponse(runId: idempotencyKey, status: self.responseStatus)
    }
...
private func makeAttachmentOutbox() -> SteelEngineChatSQLiteTranscriptCache {
    SteelEngineChatSQLiteTranscriptCache(
...
private func makeDurableAttachmentViewModel(
    ...
    outbox: SteelEngineChatSQLiteTranscriptCache) -> SteelEngineChatViewModel
{
...
final class ChatViewModelAttachmentTests: XCTestCase {
    func testImageAttachmentsAreProcessedBeforeStaging() async throws {
        ...
        let viewModel = await MainActor.run {
            SteelEngineChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
        }
    ...
    func testPartialIdentitySyncPreservesTheOtherDeferredComponent() {
        ...
        let contractViewModel = SteelEngineChatViewModel(
            sessionKey: "main",
            transport: AttachmentProcessingTransport(),
            activeAgentId: "main",
            sessionRoutingContract: oldContract)
        let contractAttachment = SteelEnginePendingAttachment(
            url: nil,
        ...
        let agentViewModel = SteelEngineChatViewModel(
            sessionKey: "main",
            transport: AttachmentProcessingTransport(),
            activeAgentId: "main",
            sessionRoutingContract: oldContract)
        let agentAttachment = SteelEnginePendingAttachment(
            url: nil,
    ...
    func testFailedAttachmentSendWithoutOutboxRestoresDraft() async throws {
        ...
        let attachmentID = await MainActor.run {
            let attachment = SteelEnginePendingAttachment(
                url: nil,
                data: attachmentData,
                fileName: "retry.m4a",
                mimeType: "audio/mp4",
    ...
    func testVoiceNoteSendKeepsCapturedDurationWhenDraftChangesDuringHealthCheck() async throws {
        ...
        let (viewModel, draftAttachmentID) = await MainActor.run {
            ...
            let draftAttachment = SteelEnginePendingAttachment(
                url: nil,
    ...
    func testAmbiguousVoiceNoteSurvivesViewModelRecreation() async throws {
        let outbox = makeAttachmentOutbox()
        var firstViewModel: SteelEngineChatViewModel? = await MainActor.run {
            let viewModel = makeDurableAttachmentViewModel(
    ...
    func testCanonicalVoiceNoteConfirmationPreservesDurationAndDeletesDurableBytes() async throws {
        ...
        let canonical = try JSONDecoder().decode(
            ...
            from: Data(
                """
                {"role":"user","content":"See attached.","__steelengine":{"idempotencyKey":"\(command
    ...
    func testCanonicalVoiceNotePreservesOptimisticDuration() throws {
        let localAudio = SteelEngineChatMessageContent(
            type: "file",
            text: nil,
            mimeType: "audio/mp4",
        ...
        let existing = SteelEngineChatMessage(
            role: "user",
            content: [localAudio],
            timestamp: nil,
        ...
        let incoming = try JSONDecoder().decode(
            SteelEngineChatMessage.self,
            from: Data(
                #"{"role":"user","content":"See attached.","__steelengine":{"idempotencyKey":"run:user"},"MediaPaths":["media/inbound/media-1.m4a"],"MediaTypes":["audio/mp4"]}"#
                    .utf8))

        let adopted = SteelEngineChatViewModel.adoptingCanonicalMessage(incoming, over: existing)
