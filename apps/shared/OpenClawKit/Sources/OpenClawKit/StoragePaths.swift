...
public enum RecallNodeStorage {
    public static func appSupportDir() throws -> URL {
        let base = FileManager().urls(for: .applicationSupportDirectory, in: .userDomainMask).first
        guard let base else {
            throw NSError(domain: "RecallNodeStorage", code: 1, userInfo: [
                NSLocalizedDescriptionKey: "Application Support directory unavailable",
            ])
        }
        return base.appendingPathComponent("Recall", isDirectory: true)
    }
    ...
    public static func cachesDir() throws -> URL {
        ...
        guard let base else {
            throw NSError(domain: "RecallNodeStorage", code: 2, userInfo: [
