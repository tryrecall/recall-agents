...
struct SteelEngineTypographyTests {
    @Test func `session controls use branded typography`() throws {
        ...
        #expect(support.contains(".font(SteelEngineType.subhead)"))
        #expect(support.contains(".font(SteelEngineType.subheadSemiBold)"))
        #expect(commandCenter.contains("Toggle(isOn: self.$showArchived)"))
        #expect(commandCenter.contains("Text(\"Show Archived\")"))
        #expect(commandCenter.contains(".font(SteelEngineType.captionMedium)"))
    }

    @Test func `bundled fonts load from app bundle`() {
        for name in SteelEngineType.registeredPostScriptNames {
    ...
    @Test func `app extensions register bundled branded fonts`() throws {
        ...
        for targetName in ["SteelEngineActivityWidget", "SteelEngineWatchApp"] {
            let target = try Self.extract(
                project,
                from: "  \(targetName):",
                to: targetName == "SteelEngineActivityWidget" ? "  SteelEngineWatchApp:" : "  SteelEngineTests:")
    ...
    @Test func `extension text surfaces use branded typography helpers`() throws {
        ...
        #expect(activitySource.contains("SteelEngineActivityType.subheadSemiBold"))
        #expect(activitySource.contains("SteelEngineActivityType.subheadBold"))
        #expect(activitySource.contains("SteelEngineActivityType.caption"))
    ...
    @Test func `UIKit typography refreshes when Dynamic Type changes`() throws {
        ...
        let typographySource = try String(
            contentsOf: Self.sourceURL("Design/SteelEngineTypography.swift"),
            encoding: .utf8)

        #expect(appSource.contains("UIContentSizeCategory.didChangeNotification"))
        #expect(appSource.contains("SteelEngineType.refreshUIKitAppearance(in: Self.connectedWindows())"))
    ...
    @Test func `listed iOS app surfaces enforce branded control typography`() throws {
        let proComponents = try String(
            contentsOf: Self.sourceURL("Design/SteelEngineProComponents.swift"),
            encoding: .utf8)
        ...
        let automations = try String(
            ...
            encoding: .utf8)
        let docs = try String(contentsOf: Self.sourceURL("Design/SteelEngineDocsScreen.swift"), encoding: .utf8)
        ...
        let chatTypography = try String(
            contentsOf: Self.iosRootURL()
                ...
                .appendingPathComponent("shared/SteelEngineKit/Sources/SteelEngineChatUI/ChatTypography.swift"),
            encoding: .utf8)
        let chatMessageViews = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/SteelEngineKit/Sources/SteelEngineChatUI/ChatMessageViews.swift"),
            encoding: .utf8)
        let chatMarkdownRenderer = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/SteelEngineKit/Sources/SteelEngineChatUI/ChatMarkdownRenderer.swift"),
            encoding: .utf8)

        #expect(automations.contains(".font(SteelEngineType.body)"))
        #expect(automations.contains(".font(SteelEngineType.headline)"))
        #expect(automations.contains(".font(SteelEngineType.subheadSemiBold)"))
        #expect(automations.contains(".font(SteelEngineType.caption)"))
        #expect(!automations.contains(".font(.body"))
        #expect(!automations.contains(".font(.headline"))
        #expect(!automations.contains(".font(.caption"))

        #expect(proComponents.contains(".font(SteelEngineType.subheadSemiBold)"))
        ...
        #expect(chatTab.contains(".font(SteelEngineType.body)"))
        ...
        #expect(quickSetup.contains(".font(SteelEngineType.subheadSemiBold)"))
        ...
        #expect(gatewayProblem.contains(".font(SteelEngineType.subheadSemiBold)"))
        ...
        #expect(onboardingSteps.contains(".font(SteelEngineType.subheadSemiBold)"))
        ...
        #expect(onboardingWizard.contains(".font(SteelEngineType.subheadSemiBold)"))
        ...
        let onboardingSecureOption = try Self.extract(
            ...
            to: ".tag(true)")
        #expect(onboardingUnencryptedOption.contains(".font(SteelEngineType.captionSemiBold)"))
        #expect(onboardingSecureOption.contains(".font(SteelEngineType.captionSemiBold)"))

        #expect(settingsSections.contains(".font(SteelEngineType.body)"))
        #expect(settingsSections.contains("Text(warningText)"))
        #expect(settingsSections.contains(".font(SteelEngineType.caption)"))
        #expect(approvalDialog.contains("Text(warningText)"))
        #expect(approvalDialog.contains(".font(SteelEngineType.footnote)"))
        ...
        #expect(settingsSections.contains("SteelEngineToggleIndicator(isOn: isOn.wrappedValue)"))
        #expect(settingsSections.contains("TextField(\"Default Share Instruction\""))
        #expect(settingsSections.contains(".font(SteelEngineType.subhead)"))
        ...
        #expect(settingsSections.contains(".font(SteelEngineType.subheadSemiBold)"))
        #expect(settingsSupport.contains("struct SettingsBuildMetadataStrip"))
        #expect(settingsSupport.contains(".font(SteelEngineType.caption2SemiBold)"))
        #expect(settingsSupport.contains(".font(SteelEngineType.monoSmall)"))
        ...
        let settingsSecureOption = try Self.extract(
            ...
            from: "Text(\"Secure (TLS)\")",
            to: ".tag(true)")
        #expect(settingsUnencryptedOption.contains(".font(SteelEngineType.captionSemiBold)"))
        #expect(settingsSecureOption.contains(".font(SteelEngineType.captionSemiBold)"))
        ...
        #expect(appleHealthAccess.contains(".font(SteelEngineType.footnote)"))
        ...
        #expect(permissionRow.contains(".font(SteelEngineType.footnoteSemiBold)"))
        ...
        #expect(skills.contains("Text(\"Gateway warning\").font(SteelEngineType.headline)"))
        #expect(skills.contains("Text(\"Acknowledge and install\").font(SteelEngineType.subheadSemiBold)"))
        #expect(skills.contains("prompt: Text(\"Search ClawHub\").font(SteelEngineType.body)"))

        for source in [agentDestinations, dreaming, instances, channels, skills, docs] {
            #expect(source.contains(".font(SteelEngineType.body)"))
        }

        #expect(chatMessageViews.contains("font: SteelEngineChatTypography.body"))
        #expect(chatMessageViews.contains("SteelEngineChatTypography.callout.italic()"))
    ...
    private static func activityWidgetSourceURL() -> URL {
        self.iosRootURL().appendingPathComponent("ActivityWidget/SteelEngineLiveActivity.swift")
    }

    private static func activityWidgetTypographySourceURL() -> URL {
        self.iosRootURL().appendingPathComponent("ActivityWidget/SteelEngineActivityTypography.swift")
    }
    ...
    private static func appSourceURL() -> URL {
        self.sourceURL("SteelEngineApp.swift")
    }
    ...
    private static func swiftSourcesForTypographyAudit() throws -> [URL] {
        let roots = [
            ...
            self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/SteelEngineKit/Sources/SteelEngineChatUI"),
        ]
    ...
    private static func unbrandedTextCallOffenders() throws -> [String] {
        let fontTokens = ["SteelEngineType", "SteelEngineChatTypography"]
    ...
    private static func hasAllowedBrandedFontParameter(_ window: String, line: String, in url: URL) -> Bool {
        ...
        case "apps/ios/Sources/Design/SteelEngineProComponents.swift":
        ...
        case "apps/shared/SteelEngineKit/Sources/SteelEngineChatUI/ChatMarkdownRenderer.swift":
