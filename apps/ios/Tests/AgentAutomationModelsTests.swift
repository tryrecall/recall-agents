...
struct AgentAutomationModelsTests {
    ...
    @Test func `semantic command formatting is not a change`() throws {
        let job = Self.job(
            payload: AnyCodable([
                ...
                "argv": AnyCodable([AnyCodable("steelengine"), AnyCodable("status")]),
                "cwd": AnyCodable("/tmp"),
            ]))
        var draft = try #require(AgentAutomationDraft(job: job))
        draft.payload = .command(argvJSON: "[ \"steelengine\", \"status\" ]", cwd: "/tmp")
    ...
    @Test func `detail source guards route and exact queued run`() throws {
        ...
        #expect(source.contains("SteelEngineType.subheadSemiBold"))
