...
@Suite(.serialized) struct SteelEngineAppDelegateTests {
    @Test @MainActor func `resolves registry model before view task assigns delegate model`() {
        let registryModel = NodeAppModel()
        SteelEngineAppModelRegistry.appModel = registryModel
        defer { SteelEngineAppModelRegistry.appModel = nil }

        let delegate = SteelEngineAppDelegate()
    ...
    @Test @MainActor func `derives background refresh task identifier from app bundle identifier`() {
        let delegate = SteelEngineAppDelegate()
        let bundleIdentifier = Bundle.main.bundleIdentifier ?? "ai.steelenginefoundation.app.tests"
    ...
    @Test @MainActor func `stages a gateway URL when the model is ready`() async throws {
        SteelEngineAppModelRegistry.appModel = nil
        defer { SteelEngineAppModelRegistry.appModel = nil }
        let model = NodeAppModel()
        let delegate = SteelEngineAppDelegate()
        delegate.appModel = model
        let url = try #require(URL(
            string: "steelengine://gateway?host=gateway.example.com&port=443&tls=1&token=tok"))
