...
struct SteelEngineBrandTests {
    @Test func `brand colors meet text contrast in both appearances`() {
        let foregroundColors = [
            ("accent", SteelEngineBrand.uiAccentForeground),
            ("accentHot", SteelEngineBrand.uiAccentHotForeground),
            ("ok", SteelEngineBrand.uiOK),
            ("warn", SteelEngineBrand.uiWarn),
            ("danger", SteelEngineBrand.uiDanger),
            ("info", SteelEngineBrand.uiInfo),
        ]
        ...
        for style in [UIUserInterfaceStyle.light, .dark] {
            ...
            let pillBackground = Self.composite(
                SteelEngineBrand.uiAccentForeground,
                alpha: style == .dark ? 0.12 : 0.08,
                over: .secondarySystemGroupedBackground,
                traits: traits)
            #expect(Self.contrastRatio(SteelEngineBrand.uiAccentForeground, pillBackground, traits: traits) >= 4.5)
            #expect(Self.contrastRatio(SteelEngineBrand.uiAccent, .white, traits: traits) >= 4.5)
        }
    }
