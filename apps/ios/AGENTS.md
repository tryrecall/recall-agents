...
- iOS SwiftUI text should use branded typography helpers, not bare system fonts. Use `SteelEngineType` in `apps/ios/Sources/**`, `SteelEngineChatTypography` in shared chat UI, `WatchClawType` in watch UI, and `SteelEngineActivityType` in Live Activity UI.
...
- When touching iOS text surfaces, update/keep `apps/ios/Tests/SteelEngineTypographyTests.swift` so bare text/control regressions are caught, then run the focused typography test.
...
- When adding, removing, or upgrading iOS dependencies, audit whether `apps/ios/Resources/Licenses/` needs updates. Exclude dependencies owned by SteelEngine Foundation from the published license list.
