...
Use these steps to exercise the live SteelEngine iOS App Review Gateway.

## Demo Account / Setup

Use the SteelEngine iOS app with the live review Gateway setup code included in
...
9. On `Connected`, tap `Open SteelEngine`.
10. Confirm the `Control` screen shows `Gateway Online`.
11. Tap `Settings`.
12. Tap `Approvals`.
13. Tap `Open Notifications`.
14. Tap `Enable Notifications`.
15. On `Enable SteelEngine Hosted Push Relay?`, tap `Continue`.
16. If iOS asks whether SteelEngine may send notifications, tap `Allow`.
...
command `printf 'SteelEngine App Review approval demo complete\n'`. Tap
...
9. While Talk is active, send SteelEngine to the background by returning to the
   Home Screen or locking the iPhone. Do not force quit the app.
10. Continue speaking then wait for assistant audio reply.

Expected result: realtime Talk audio continues while SteelEngine is backgrounded.
Reopen SteelEngine, confirm Talk is still active, then tap `Stop Talk`.
...
1. Tap `Settings`.
2. Tap `Reconnect`.
3. Immediately send SteelEngine to the background by returning to the Home Screen
   or locking the iPhone.
4. Watch the Lock Screen or Dynamic Island while the Gateway reconnects.

Expected result: while reconnecting, iOS can show an `SteelEngine` Live Activity
with connection status such as `Connecting...` or `Reconnecting...`. On a fast
network this status may be brief because SteelEngine ends the Live Activity after
...
4. Immediately send SteelEngine to the background and lock the iPhone. Do not
   force quit the app.

Expected result: the iPhone Lock Screen receives a visible `SteelEngine`
notification with this body:

```text
SteelEngine App Review push notification demo
```

Tap the notification and unlock the iPhone if prompted. If SteelEngine opens on
...
3. Immediately send SteelEngine to the background and lock the iPhone. Do not
   force quit the app.
4. Wait for the `SteelEngine` notification on the Lock Screen. It normally appears
   about 10 seconds after the message is sent.
5. Tap the notification and unlock the iPhone if prompted. If SteelEngine opens on
   `Control`, tap `Chat`.
...
permission sheet. SteelEngine requests read-only access to steps, sleep, resting
...
4. Tap `Share`.
5. Tap `SteelEngine`.
6. Confirm the SteelEngine share extension appears and shows
   `Edit text, then tap Send.` and `Send to SteelEngine`.
7. Tap `Send to SteelEngine`.

Expected result: the SteelEngine share extension sends the shared Safari page to
the live review Gateway and shows `Sent to SteelEngine.` Returning to SteelEngine
