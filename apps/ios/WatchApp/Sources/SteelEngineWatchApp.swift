...
enum WatchScreenshotMode {
    private static let defaultsKey = "steelengine.watch.screenshotMode"
    static let approvals = ProcessInfo.processInfo.arguments.contains(
        "--steelengine-watch-approval-screenshot-mode")
        || ProcessInfo.processInfo.environment["STEELENGINE_WATCH_APPROVAL_SCREENSHOT_MODE"] == "1"
    static let enabled = ProcessInfo.processInfo.arguments.contains("--steelengine-watch-screenshot-mode")
        || ProcessInfo.processInfo.environment["STEELENGINE_WATCH_SCREENSHOT_MODE"] == "1"
        || UserDefaults.standard.bool(forKey: WatchScreenshotMode.defaultsKey)
        || WatchScreenshotMode.approvals
}

@main
struct SteelEngineWatchApp: App {
    ...
    fileprivate func configureScreenshotFixture(includeApproval: Bool = false) {
        ...
        self.consume(
            appSnapshot: WatchAppSnapshotMessage(
                ...
                gatewayStableID: "watch-screenshot-gateway",
                talkStatus: SteelEngineWatchAppStatus(code: .talkReady),
                talkEnabled: true,
                talkListening: false,
                talkSpeaking: false,
