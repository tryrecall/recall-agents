...
SteelEngine iOS is the officially released iPhone app. It connects to an SteelEngine Gateway as a `role: node` for chat, voice, approvals, sharing, and device-aware automation.
...
- `xcodegen`
- `fastlane`
- Apple account signed into Xcode for the canonical SteelEngine team (`FWJYW4S8P8`)
- Fastlane Apple Developer Portal session for the canonical SteelEngine team when creating bundle IDs or enabling services
- Release-owner access to the encrypted signing repo password (`MATCH_PASSWORD`)
- App Store Connect app already created for `ai.steelenginefoundation.app`
...
- Local development uses the canonical `ai.steelenginefoundation.app*` bundle IDs when the SteelEngine team is available, and unique `ai.steelenginefoundation.app.test.*` bundle IDs only for non-canonical fallback teams.
- App Store release uses canonical `ai.steelenginefoundation.app*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/AppStoreRelease.xcconfig`.
...
- App Store release also switches the app to `SteelEnginePushMode=appStore`, which derives relay transport, official distribution, the canonical production relay, production APNs, production relay profile, `appleStrict` proof, and the App-Attest-capable entitlement file.
...
- App Store release builds use the canonical hosted relay at `https://ios-push-relay.steelengine.ai`.
...
3. Confirm the App Store Connect app and Apple Developer identifiers/capabilities exist for:
   - `ai.steelenginefoundation.app`
   - `ai.steelenginefoundation.app.share`
   - `ai.steelenginefoundation.app.activitywidget`
   - `ai.steelenginefoundation.app.watchkitapp`
...
8. Expected outputs after a successful run:
   - `apps/ios/build/app-store/SteelEngine-<version>.ipa`
   - `apps/ios/build/app-store/SteelEngine-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/SteelEngine.entitlements` derives `aps-environment` from the active build configuration/signing override.
- App Attest relay builds use `apps/ios/Sources/SteelEngineAppAttest.entitlements`; local/direct builds do not require App Attest provisioning.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual Debug builds default to `SteelEnginePushMode=localSandbox`, direct APNs registration, and a development `aps-environment` entitlement. Local/manual Release builds default to `SteelEnginePushMode=localProduction` and direct production APNs registration.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `STEELENGINE_APNS_TEAM_ID`, `STEELENGINE_APNS_KEY_ID`, and either `STEELENGINE_APNS_PRIVATE_KEY_P8` or `STEELENGINE_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.steelengine/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `STEELENGINE_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to sandbox APNs through `SteelEnginePushMode=localSandbox`; Release builds default to production APNs through `SteelEnginePushMode=localProduction`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `steelengine.json`. `STEELENGINE_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine SteelEngine official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as SteelEngine node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
