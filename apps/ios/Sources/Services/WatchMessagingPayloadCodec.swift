...
enum WatchMessagingPayloadCodec {
    private static let durableSnapshotTypes = [
        SteelEngineWatchPayloadType.appSnapshot.rawValue,
        SteelEngineWatchPayloadType.execApprovalSnapshot.rawValue,
    ]
        ...
        gatewayStableID: String?) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": SteelEngineWatchPayloadType.notify.rawValue,
            "id": id,
            "title": params.title,
            "body": params.body,
            "priority": params.priority?.rawValue ?? SteelEngineNotificationPriority.active.rawValue,
            "sentAtMs": self.nowMs(),
        ]
    ...
    static func encodeDirectNodeSetupPayload(setupCode: String) -> [String: Any] {
        [
            "type": SteelEngineWatchPayloadType.directNodeSetup.rawValue,
            "setupCode": setupCode,
            "sentAtMs": self.nowMs(),
        ]
    }

    static func encodeExecApprovalItem(_ item: SteelEngineWatchExecApprovalItem) -> [String: Any] {
        var payload: [String: Any] = [
            "id": item.id,
    ...
    static func encodeExecApprovalPromptPayload(
        _ message: SteelEngineWatchExecApprovalPromptMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": SteelEngineWatchPayloadType.execApprovalPrompt.rawValue,
    ...
    static func encodeExecApprovalResolvedPayload(
        _ message: SteelEngineWatchExecApprovalResolvedMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": SteelEngineWatchPayloadType.execApprovalResolved.rawValue,
    ...
    static func encodeExecApprovalExpiredPayload(
        _ message: SteelEngineWatchExecApprovalExpiredMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": SteelEngineWatchPayloadType.execApprovalExpired.rawValue,
            "approvalId": message.approvalId,
    ...
    static func encodeExecApprovalSnapshotPayload(
        _ message: SteelEngineWatchExecApprovalSnapshotMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": SteelEngineWatchPayloadType.execApprovalSnapshot.rawValue,
    ...
    static func encodeAppSnapshotPayload(
        _ message: SteelEngineWatchAppSnapshotMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
    ...
    private static func encodeAppStatus(_ status: SteelEngineWatchAppStatus) -> [String: Any] {
    ...
    static func encodeChatCompletionPayload(
        _ message: SteelEngineWatchChatCompletionMessage) -> [String: Any]
    {
        [
    ...
    {
        guard (payload["type"] as? String) == SteelEngineWatchPayloadType.reply.rawValue else {
            return nil
        }
    ...
    {
        guard (payload["type"] as? String) == SteelEngineWatchPayloadType.execApprovalResolve.rawValue else {
            return nil
        }
        guard let approvalId = ExecApprovalIdentifier.exact(payload["approvalId"] as? String),
              ...
              let decision = SteelEngineWatchExecApprovalDecision(rawValue: rawDecision)
        else {
            return nil
        }
    ...
    {
        guard (payload["type"] as? String) == SteelEngineWatchPayloadType.execApprovalSnapshotRequest.rawValue else {
            return nil
        }
    ...
    {
        guard (payload["type"] as? String) == SteelEngineWatchPayloadType.appSnapshotRequest.rawValue else {
            return nil
        }
    ...
    {
        guard (payload["type"] as? String) == SteelEngineWatchPayloadType.appCommand.rawValue else {
            return nil
        }
        guard let rawCommand = nonEmpty(payload["command"] as? String),
              let command = SteelEngineWatchAppCommand(rawValue: rawCommand)
        else {
            return nil
        }
