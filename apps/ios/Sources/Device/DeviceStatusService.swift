...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    private func batteryStatus() -> SteelEngineBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: SteelEngineBatteryState = switch device.batteryState {
        ...
        return SteelEngineBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> SteelEngineThermalStatusPayload {
        let state: SteelEngineThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return SteelEngineThermalStatusPayload(state: state)
    }

    private func storageStatus() -> SteelEngineStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return SteelEngineStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
