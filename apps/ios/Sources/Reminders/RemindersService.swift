...
final class RemindersService: RemindersServicing {
    ...
    func list(params: SteelEngineRemindersListParams) async throws -> SteelEngineRemindersListPayload {
        ...
        let payload: [SteelEngineReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return SteelEngineReminderPayload(
        ...
        return SteelEngineRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: SteelEngineRemindersAddParams) async throws -> SteelEngineRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = Self.date(fromDueComponents: reminder.dueDateComponents)
        let payload = SteelEngineReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return SteelEngineRemindersAddPayload(reminder: payload)
    }
