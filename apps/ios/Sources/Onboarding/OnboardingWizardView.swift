    ...
    private var authStep: some View {
        Section {
            ...
            if let problem = self.currentProblem {
                ...
                Text("SteelEngine is checking gateway and node access.")
                    .font(SteelEngineType.footnote)
                    .foregroundStyle(.secondary)
            }
        } header: {
        ...
        if self.issue.needsPairing {
            Section {
                ...
                let requestLine: String = {
                    ...
                    return String(localized: "Request ID: check `steelengine devices list`.")
                }()
                let commandLine = self.currentProblem?.actionCommand ?? "steelengine devices approve <requestId>"
...
extension OnboardingWizardView {
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
               ...
               let port = active.port
            {
                ...
                self.manualHost = "steelengine.local"
        ...
        if self.selectedMode == .developerLocal, self.manualHost == "steelengine.local" {
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "steelengine.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "steelengine.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "steelengine.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
