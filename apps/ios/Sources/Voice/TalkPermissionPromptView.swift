    ...
    var body: some View {
        VStack(alignment: .leading, spacing: self.style == .sheet ? 16 : 12) {
            HStack(alignment: .top, spacing: 12) {
                Image(systemName: self.iconSystemName)
                    ...
                    .foregroundStyle(self.requestIsPending ? SteelEngineBrand.warn : SteelEngineBrand.accent)
                    .frame(width: 28, height: 28)

                VStack(alignment: .leading, spacing: 6) {
                    Text(self.titleText)
                        .font(self.style == .sheet ? SteelEngineType.title3SemiBold : SteelEngineType.headline)
        ...
        .background {
            if self.style == .card || self.style == .sheet {
                RoundedRectangle(cornerRadius: SteelEngineRadius.md, style: .continuous)
                    .fill(.thinMaterial)
            }
        }
        .overlay {
            if self.style == .card || self.style == .sheet {
                RoundedRectangle(cornerRadius: SteelEngineRadius.md, style: .continuous)
                    .stroke(SteelEngineBrand.accent.opacity(0.20), lineWidth: 1)
            }
        }
