...
final class MotionService: MotionServicing {
    func activities(params: SteelEngineMotionActivityParams) async throws -> SteelEngineMotionActivityPayload {
        ...
        let mapped: [SteelEngineMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        SteelEngineMotionActivityEntry(
        ...
        return SteelEngineMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = SteelEnginePedometerPayload(
