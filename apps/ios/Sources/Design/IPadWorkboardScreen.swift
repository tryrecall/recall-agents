            ...
            Text(IPadWorkboardDefaults.label(for: status))
                ...
                .overlay {
                    Capsule()
                        .strokeBorder(
                            self.selectedStatus == status
                                ? SteelEngineBrand.accent.opacity(0.42)
                                : Color.primary.opacity(0.08),
                            lineWidth: 1)
                }
        }
        .buttonStyle(.plain)
        .foregroundStyle(self.selectedStatus == status ? SteelEngineBrand.accent : .primary)
        .accessibilityLabel(
            String(
...
private struct IPadWorkboardKanbanCard: View {
    ...
    private var color: Color {
        switch self.card.status {
        case "running": SteelEngineBrand.ok
        case "review": SteelEngineBrand.accentForeground
        case "blocked": SteelEngineBrand.warn
        case "done": .secondary
        default: SteelEngineBrand.accentHotForeground
        }
    }
    ...
    var body: some View {
        ...
        .swipeActions(edge: .trailing, allowsFullSwipe: false) {
            if let nextStatus {
                ...
                .tint(SteelEngineBrand.accentHot)
            }
            Button(action: self.archive) {
                Text(self.card.metadata?.archivedAt == nil
                    ? LocalizedStringKey("Archive")
