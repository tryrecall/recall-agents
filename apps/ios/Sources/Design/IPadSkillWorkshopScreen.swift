...
struct IPadSkillWorkshopScreen: View {
            ...
            NavigationStack {
                ScrollView {
                    self.presentedProposalDetail(proposalID: route.proposalID)
                        ...
                        .padding(.vertical, SteelEngineSpacing.space4)
                }
    ...
    private var compactFiltersCard: some View {
        ...
        return ProCard(radius: SteelEngineProMetric.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
...
private struct IPadSkillProposalKanbanCard: View {
    ...
    var body: some View {
                    ...
                    HStack(alignment: .top, spacing: 10) {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            Text(self.proposal.title)
                                .font(SteelEngineType.subheadSemiBold)
                                .foregroundStyle(self.isSelected ? SteelEngineBrand.accent : .primary)
                                .lineLimit(2)
        ...
        .padding(SteelEngineSpacing.space3)
        .background(
            self.isSelected ? SteelEngineBrand.accent.opacity(0.08) : Color.clear,
            in: RoundedRectangle(cornerRadius: SteelEngineRadius.xs, style: .continuous))
...
struct IPadSkillProposalRow: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 10)
        .background(
            self.isSelected ? SteelEngineBrand.danger.opacity(0.08) : Color.clear,
            in: RoundedRectangle(cornerRadius: SteelEngineRadius.xs, style: .continuous))
    }
}
...
struct IPadSkillProposal: Identifiable {
    ...
    var statusColor: Color {
        switch self.status {
        case "pending": SteelEngineBrand.warn
        case "applied": SteelEngineBrand.ok
        case "rejected": .secondary
        case "quarantined", "stale": SteelEngineBrand.warn
        default: SteelEngineBrand.accent
        }
    }
