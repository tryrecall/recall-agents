...
struct AgentProDreamingDestination: View {
            ...
            ProCard(padding: 0) {
                if let diary = self.overview?.dreamDiary {
                    if diary.found, let content = self.normalizedMultiline(diary.content) {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            HStack {
                                ProIconBadge(systemName: "book.pages", color: SteelEngineBrand.accent)
    ...
    private func dreamingEntriesList(
        ...
        emptyTitle: SteelEngineTextValue,
        emptyDetail: SteelEngineTextValue) -> some View
    {
        VStack(alignment: .leading, spacing: 8) {
    ...
    private func dreamingEntryRow(_ entry: DreamingEntryLite) -> some View {
        HStack(alignment: .top, spacing: 12) {
            ProIconBadge(systemName: "text.page", color: SteelEngineBrand.accent)
    ...
    private func dreamingPhaseRow(_ phase: DreamingPhaseRow) -> some View {
        HStack(alignment: .top, spacing: 12) {
            ProIconBadge(
                ...
                color: phase.status.enabled == false ? .secondary : SteelEngineBrand.accent)
            VStack(alignment: .leading, spacing: 4) {
                Text(phase.title)
                    .font(SteelEngineType.subheadSemiBold)
            ...
            Text(self.dreamingPhaseState(phase.status))
                .font(SteelEngineType.caption2SemiBold)
                .foregroundStyle(phase.status.managedCronPresent == true ? SteelEngineBrand.accent : .secondary)
                .lineLimit(1)
        }
        .padding(.vertical, 10)
        .padding(.horizontal, 14)
    ...
    private func emptyDetailRow(
        icon: String,
        title: SteelEngineTextValue,
        detail: SteelEngineTextValue) -> some View
    {
        HStack(spacing: 12) {
    ...
    private static func dreamDiaryInnerContent(_ content: String) -> String {
        let start = "<!-- steelengine:dreaming:diary:start -->"
        let end = "<!-- steelengine:dreaming:diary:end -->"
