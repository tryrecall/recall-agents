...
struct AgentProNodesDestination: View {
    ...
    private func nodeListCard(title: SteelEngineTextValue, values: [String]) -> some View {
        VStack(alignment: .leading, spacing: 8) {
    ...
    private func detailMetric(label: SteelEngineTextValue, value: String) -> some View {
        VStack(alignment: .leading, spacing: 3) {
            label.text
    ...
    private static func presenceColor(_ entry: PresenceEntry) -> Color {
        self.normalized(entry.reason) == nil ? SteelEngineBrand.accent : SteelEngineBrand.warn
    }
