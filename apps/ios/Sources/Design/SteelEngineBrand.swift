...
enum AppAppearancePreference: String, CaseIterable, Identifiable {
    ...
    static var launchArgumentPreference: AppAppearancePreference? {
        ...
        guard let flagIndex = arguments.firstIndex(of: "--steelengine-appearance") else {
            return nil
        }
...
struct SteelEngineActivationGlyph: View {
    let size: CGFloat
    ...
    var body: some View {
        SteelEngineMascotView(floats: false, mood: self.mood, interactive: self.interactive)
            .frame(width: self.size, height: self.size)
            .shadow(
                color: SteelEngineBrand.activationGlow.opacity(0.18),
...
extension View {
    func steelEngineSheetChrome() -> some View {
        self
            .tint(SteelEngineBrand.accent)
            .background {
                SteelEngineBrand.sheetBackground
                    .ignoresSafeArea()
            }
    }

    func steelEngineCraftSurface(cornerRadius: CGFloat = 24, shadow: Bool = true) -> some View {
        self.modifier(SteelEngineCraftSurfaceModifier(cornerRadius: cornerRadius, shadow: shadow))
    }
}

private struct SteelEngineCraftSurfaceModifier: ViewModifier {
        ...
        content
            .background {
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .fill(SteelEngineBrand.activationSurface)
                    .shadow(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .stroke(SteelEngineBrand.activationHairline, lineWidth: 0.5)
            }
    }
}

struct SteelEnginePrimaryActionButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .foregroundStyle(self.isEnabled ? SteelEngineBrand.activationPrimaryActionText : Color.secondary)
            .tint(self.isEnabled ? SteelEngineBrand.activationPrimaryActionText : Color.secondary)
            .frame(maxWidth: .infinity)
            .frame(height: self.height)
            .background {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .fill(self.isEnabled ? Self.primaryFill : SteelEngineBrand.activationDisabledGradient)
                    .shadow(
                        color: self.isEnabled ? SteelEngineBrand.activationPrimaryAction.opacity(0.08) : .clear,
                        radius: 1,
                        x: 0,
                        y: 1)
                    .shadow(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .stroke(
                        self.isEnabled
                            ? SteelEngineBrand.activationPrimaryAction.opacity(0.72)
                            : SteelEngineBrand.activationNeutralStroke,
                        lineWidth: 0.75)
            }
...
struct SteelEngineSecondaryActionButtonStyle: ButtonStyle {
    @Environment(\.isEnabled) private var isEnabled
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            .font(SteelEngineType.subheadSemiBold)
            .foregroundStyle(self.isEnabled ? SteelEngineBrand.activationPrimaryAction : .secondary)
            .frame(maxWidth: .infinity)
            .frame(height: self.height)
            .background {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .stroke(SteelEngineBrand.activationHairline, lineWidth: 0.75)
            }
            .scaleEffect(configuration.isPressed && self.isEnabled ? 0.98 : 1)
            .animation(.smooth(duration: 0.14), value: configuration.isPressed)
    }
    ...
    private static var secondaryFill: LinearGradient {
        LinearGradient(
            colors: [
                SteelEngineBrand.activationSecondaryActionTop,
                SteelEngineBrand.activationSecondaryActionBottom,
            ],
            startPoint: .top,
            endPoint: .bottom)
    }
...
struct SteelEngineCloseButtonStyle: ButtonStyle {
    @Environment(\.isEnabled) private var isEnabled
        ...
        configuration.label
            ...
            .background {
                Capsule(style: .continuous)
                    .fill(SteelEngineBrand.activationNeutralGradient)
                    .shadow(
            ...
            .overlay {
                Capsule(style: .continuous)
                    .stroke(SteelEngineBrand.activationNeutralStroke, lineWidth: 0.6)
            }
