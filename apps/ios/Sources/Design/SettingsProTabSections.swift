...
extension SettingsProTab {
    ...
    @ViewBuilder var settingsListSection: some View {
        Section {
            ...
            self.settingsListRow(
                icon: "sparkles",
                iconColor: SteelEngineBrand.accent,
                title: "Skills",
                route: .skills)
            self.settingsListRow(
                icon: "waveform",
        ...
        badgeValue: String? = nil) -> some View
    {
        ...
        .badge(badgeValue.map { Text($0).font(SteelEngineType.captionSemiBold) })
    }
                ...
                Section("Upgrade access") {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("This phone has limited Gateway access.")
                            .font(SteelEngineType.subheadSemiBold)
                        Text(
                            "Use a secure wss:// or Tailscale Serve Gateway, then scan a full-access setup code from the Control UI or steelengine qr and reconnect to enable settings and upgrades.") // swiftlint:disable:this line_length
                            .font(SteelEngineType.caption) // Keep the native localization key contiguous.
                            .foregroundStyle(.secondary)
    ...
    var approvalsDestination: some View {
        Group {
            self.detailStatusCard(
                ...
                color: self.notificationsNeedAttention ? SteelEngineBrand.warn :
                    (self.pendingApprovalCount == 0 ? SteelEngineBrand.ok : SteelEngineBrand.warn))
        ...
        Group {
            ...
            self.detailStatusCard(
                ...
                detail: .verbatim(watchStatus.appInstalled
                    ...
                    : String(
                        localized: "Install the SteelEngine watch app before enabling direct mode.")),
                value: .verbatim(
                ...
                color: watchStatus.appInstalled ? SteelEngineBrand.ok : SteelEngineBrand.warn)
        ...
        Section {
            VStack(alignment: .leading, spacing: 4) {
                Text("Notifications are off")
                    .font(SteelEngineType.subheadSemiBold)
                Text("Enable Notifications to receive approval alerts while SteelEngine is not open.")
                    .font(SteelEngineType.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
        ...
        if let pendingApproval {
            Section("Reviewing") {
                ...
                if let errorText = self.appModel.pendingExecApprovalPromptErrorText {
                    Text(errorText)
                        .font(SteelEngineType.caption)
                        .foregroundStyle(SteelEngineBrand.danger)
                }
    ...
    var voiceDestination: some View {
        Group {
            self.detailStatusCard(
                ...
                color: self.talkEnabled || self.voiceWakeEnabled ? SteelEngineBrand.accent : .secondary)
    ...
    var diagnosticsDestination: some View {
        Group {
            self.detailStatusCard(
                ...
                color: self.gatewayDiagnosticConnected ? SteelEngineBrand.ok : SteelEngineBrand.warn)
            ...
            self.detailListCard {
                ...
                SettingsDetailRow(
                    "App",
                    value: .verbatim(DeviceInfoHelper.steelEngineVersionString()))
    ...
    var notificationsSection: some View {
        Section("Notifications") {
            HStack(spacing: 12) {
                ...
                .accessibilityHint("Turns SteelEngine notification delivery on or off")
            }
    ...
    @ViewBuilder var licensesDestination: some View {
        ...
        if documents.isEmpty {
            ...
            Section {
                ...
                Text("SteelEngine appreciates its partners in the open-source community.")
                    .font(SteelEngineType.footnote)
            }
    ...
    var aboutDestination: some View {
        Group {
            Section {
                VStack(spacing: 12) {
                    SteelEngineProMark(size: 96, shadowRadius: 18, interactive: true)
            ...
            Section {
                self.aboutLinkRow(
                    ...
                    icon: "globe",
                    color: .blue,
                    url: URL(string: "https://steelengine.ai")!)
                self.aboutLinkRow(
                    title: "Docs",
                    icon: "book.fill",
                    color: .orange,
                    url: URL(string: "https://docs.steelengine.ai")!)
                self.aboutLinkRow(
                    title: "GitHub",
                    icon: "chevron.left.slash.chevron.right",
                    color: .gray,
                    url: URL(string: "https://github.com/steelengineai/recall-agents")!)
                self.aboutLinkRow(
                    title: "Discord",
                ...
                Text("© 2026 SteelEngine Foundation — MIT License.")
                    .font(SteelEngineType.footnote)
            }
        }
    }
    ...
    /// would bypass the typography audit and SteelEngineType styling.
    func aboutLinkRow(
        title: LocalizedStringKey,
        icon: String,
        color: Color,
            ...
            VStack(alignment: .leading, spacing: 12) {
                Button {
                    ...
                    HStack {
                        ...
                        Spacer(minLength: 8)
                        ZStack {
                            SteelEngineToggleIndicator(isOn: self.locationSettingsPresentation.sharingControlIsOn)
    ...
    var agentSelectionCard: some View {
        Section {
            Picker("Default Agent", selection: self.$selectedAgentPickerId) {
                Text("Default").font(SteelEngineType.body).tag("")
                ...
                ForEach(self.appModel.gatewayAgents.filter { $0.id != defaultId }, id: \.id) { agent in
                    ...
                    Text(name.isEmpty ? agent.id : name).font(SteelEngineType.body).tag(agent.id)
                }
            }
            .font(SteelEngineType.body)
        } footer: {
    ...
    var gatewaySetupCard: some View {
        Section {
            ...
            if let warning = self.tailnetWarningText {
                Text(warning).font(SteelEngineType.footnote).foregroundStyle(SteelEngineBrand.warn)
        ...
        Section {
            ...
            Picker("Speech Language", selection: self.$talkSpeechLocale) {
                ForEach(TalkSpeechLocale.supportedOptions()) { option in
                    Text(option.label).font(SteelEngineType.body).tag(option.id)
                }
            }
            .font(SteelEngineType.body)
            ...
            Section("Voice") {
                Picker("Provider", selection: self.talkProviderSelectionBinding) {
                    ForEach(TalkModeProviderSelection.allCases) { option in
                        Text(option.label).font(SteelEngineType.body).tag(option.rawValue)
                    }
                }
                .font(SteelEngineType.body)
                if self.shouldShowRealtimeVoicePicker {
                    Picker("Realtime Voice", selection: self.talkRealtimeVoiceSelectionBinding) {
                        Text("Gateway Default").font(SteelEngineType.body).tag("")
                        ForEach(TalkModeRealtimeVoiceSelection.voices, id: \.self) { voice in
                            Text(TalkModeRealtimeVoiceSelection.label(for: voice)).font(SteelEngineType.body).tag(voice)
                        }
                    }
                    .font(SteelEngineType.body)
                }
        ...
        Button {
            ...
            HStack {
                Text(title)
                    .font(SteelEngineType.body)
                Spacer(minLength: 8)
                SteelEngineToggleIndicator(isOn: isOn.wrappedValue)
            }
            .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
