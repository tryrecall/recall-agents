...
@MainActor
enum SteelEngineAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class SteelEngineAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.steelenginefoundation.app", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.steelenginefoundation.app", category: "BackgroundWake")
    ...
    private static var appBundleIdentifier: String {
        guard let bundleId = Bundle.main.bundleIdentifier?.trimmingCharacters(in: .whitespacesAndNewlines),
            ...
            return "ai.steelenginefoundation.app"
        }
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? SteelEngineAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "steelengine.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "steelengine.watch.promptId"
    static let sessionKeyKey = "steelengine.watch.sessionKey"
    static let gatewayStableIDKey = "steelengine.watch.gatewayStableID"
    static let actionPrimaryIDKey = "steelengine.watch.action.primary.id"
    static let actionPrimaryLabelKey = "steelengine.watch.action.primary.label"
    static let actionSecondaryIDKey = "steelengine.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "steelengine.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "steelengine.watch.action.primary"
    static let actionSecondaryIdentifier = "steelengine.watch.action.secondary"
    static let actionIdentifierPrefix = "steelengine.watch.action."
    static let actionIDKeyPrefix = "steelengine.watch.action.id."
    static let actionLabelKeyPrefix = "steelengine.watch.action.label."
    static let categoryPrefix = "steelengine.watch.prompt.category."
        ...
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> SteelEngineWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return SteelEngineWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "SteelEngine" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [SteelEngineWatchAction]) -> [UNNotificationAction] {
...
@main
struct SteelEngineApp: App {
    @State private var appearanceModel: AppAppearanceModel
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(SteelEngineAppDelegate.self) private var appDelegate
    @Environment(\.scenePhase) private var scenePhase

    init() {
        Self.installUncaughtExceptionLogger()
        GatewaySettingsStore.bootstrapPersistence()
        SteelEngineType.installUIKitAppearance()
        let appModel = NodeAppModel(audioAdmissionInitiallyAllowed: false)
        #if DEBUG
        if ProcessInfo.processInfo.arguments.contains("--steelengine-reset-onboarding") {
        ...
        #endif
        SteelEngineAppModelRegistry.appModel = appModel
        ...
        WindowGroup {
            RootTabs()
                ...
                .onReceive(
                    ...
                    perform: { _ in
                        SteelEngineType.refreshUIKitAppearance(in: Self.connectedWindows())
                    })
                .onOpenURL { url in
    ...
    private static var screenshotModeEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--steelengine-screenshot-mode")
        #else
        false
        #endif
    }
    ...
    private static var screenshotNotificationGuidanceEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--steelengine-screenshot-notification-guidance")
        #else
        false
        #endif
    }
    ...
    private func applyWindowTint() {
        for window in Self.connectedWindows() {
            window.tintColor = SteelEngineBrand.uiAccent
        }
    }
...
extension SteelEngineApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("SteelEngine: installing uncaught exception handler")
