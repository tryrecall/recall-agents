...
actor CameraController {
    ...
    func snap(params: SteelEngineCameraSnapParams) async throws -> (
        format: String,
        base64: String,
        width: Int,
        height: Int)
    ...
    func clip(params: SteelEngineCameraClipParams) async throws -> (
        format: String,
        base64: String,
        durationMs: Int,
        hasAudio: Bool)
    {
        ...
        let movURL = FileManager().temporaryDirectory
            .appendingPathComponent("steelengine-camera-\(UUID().uuidString).mov")
        let mp4URL = FileManager().temporaryDirectory
            .appendingPathComponent("steelengine-camera-\(UUID().uuidString).mp4")
        defer {
    ...
    private nonisolated static func pickCamera(
        facing: SteelEngineCameraFacing,
