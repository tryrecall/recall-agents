...
@MainActor
final class SteelEngineSnapshotUITests: XCTestCase {
    ...
    func testOnboardingExplainsCapabilitiesAndTrust() {
        ...
        app.launchArguments += ["--steelengine-reset-onboarding"]
        app.launch()
        self.app = app
    ...
    func testEmptyChatStarterPromptSendsMessage() throws {
        self.launchApp(
            ...
            additionalArguments: ["--steelengine-empty-chat-fixture"])
        ...
        starter.tap()
        let sentText = "Summarize the current SteelEngine status and tell me what needs attention."
    ...
    func testEmptyChatStarterPromptsLocalizeInGerman() throws {
        self.launchApp(
            ...
            additionalArguments: [
                "--steelengine-empty-chat-fixture",
                "-AppleLanguages",
                "(de)",
                "-AppleLocale",
                "de_DE",
        ...
        let starter = try XCTUnwrap(self.app?.buttons["SteelEngine-Status prüfen"])
        XCTAssertTrue(starter.exists)
        starter.tap()
        XCTAssertTrue(
            self.app?.staticTexts[
                "Fasse den aktuellen SteelEngine-Status zusammen und sage mir, was Aufmerksamkeit erfordert.",
    ...
    func testOnboardingPairCommandAndCompletionOpenChat() throws {
        ...
        setupSnapshot(app)
        app.launchArguments += [
            "--steelengine-reset-onboarding",
            "--steelengine-initial-tab",
            "settings",
            "--steelengine-initial-destination",
            "settings",
        ]
        app.launch()
        self.app = app
    ...
    func testChatReturnsToOriginatingControlDetail() throws {
        ...
        let returnButton = try XCTUnwrap(self.app?.buttons["SteelEngineChatBackToControlDetailButton"])
    ...
    func testLiveGatewayChatRoundTripAndControlOverview() throws {
        ...
        for index in 0..<3 {
            let seedMarker = "STEELENGINE_E2E_SEED_\(index)_\(Int(Date().timeIntervalSince1970 * 1000))"
        ...
        let replyMarker = "STEELENGINE_E2E_OK_\(Int(Date().timeIntervalSince1970 * 1000))"
        self.sendLiveGatewayMessage(
    ...
    func testManualAuthRetryUsesEditedToken() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["STEELENGINE_IOS_RETRY_E2E"] == "1",
            "Set STEELENGINE_IOS_RETRY_E2E=1 with a local token-auth Gateway on port 18920")
        let token = try XCTUnwrap(ProcessInfo.processInfo.environment["STEELENGINE_IOS_RETRY_TOKEN"])
    ...
    func testPhotosLimitedAccess() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["STEELENGINE_IOS_PHOTOS_E2E"] == "1",
            "Set STEELENGINE_IOS_PHOTOS_E2E=1 to exercise the system Photos prompt")
        ...
        additionalArguments: [String] = [])
    {
        ...
        app.launchArguments += [
            ...
            "--steelengine-sidebar-visibility",
            "hidden",
        ]
        if screenshotMode {
            app.launchArguments.append("--steelengine-screenshot-mode")
        }
        app.launchArguments += additionalArguments
        if let appearance {
            app.launchArguments += ["--steelengine-appearance", appearance]
        }
        app.launch()
        self.app = app
    ...
    {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["STEELENGINE_IOS_LIVE_GATEWAY"] == "1",
            "Set STEELENGINE_IOS_LIVE_GATEWAY=1 and provide a fresh setup code")

        if let setupCode = ProcessInfo.processInfo.environment["STEELENGINE_IOS_LIVE_SETUP_CODE"] {
