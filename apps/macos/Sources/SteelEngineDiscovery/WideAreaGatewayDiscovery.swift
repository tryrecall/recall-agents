...
enum WideAreaGatewayDiscovery {
        ...
        context: DiscoveryContext = .live) -> [WideAreaGatewayBeacon]
    {
        ...
        for raw in ptrLines {
            ...
            let suffix = "._steelengine-gw._tcp.\(domainTrimmed)"
    ...
    {
        guard let domain = SteelEngineBonjour.wideAreaGatewayServiceDomain else { return nil }
        let domainTrimmed = domain.trimmingCharacters(in: CharacterSet(charactersIn: "."))
        let probeName = "_steelengine-gw._tcp.\(domainTrimmed)"
