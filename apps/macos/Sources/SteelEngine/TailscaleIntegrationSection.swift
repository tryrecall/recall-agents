...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.steelengine/steelengine.json. Prefer STEELENGINE_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
        ...
        isPaused: Bool) -> GatewayTailscaleApplyMessages
    {
        ...
        let statusMessage = if connectionMode == .local, !isPaused {
            "Saved to ~/.steelengine/steelengine.json. Restarting gateway…"
        } else {
            "Saved to ~/.steelengine/steelengine.json. Restart the gateway to apply."
        }
        return GatewayTailscaleApplyMessages(
