...
private struct SkillRow: View {
    ...
    private var sourceLabel: String {
        ...
        case "steelengine-managed":
            "Managed"
        case "steelengine-workspace":
            "Workspace"
        case "steelengine-extra":
            "Extra"
        case "steelengine-plugin":
            "Plugin"
        default:
            self.skill.source
        }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            SecureField(self.editor.envKey, text: self.$value)
                .textFieldStyle(.roundedBorder)
            Text("Saved to steelengine.json under skills.entries.\(self.editor.skillKey)")
                .font(.caption2)
            ...
            do {
                if isPrimary {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        apiKey: value)
                    self.statusMessage = "Saved API key — stored in steelengine.json (skills.entries.\(skillKey))"
                } else {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        env: [envKey: value])
                    self.statusMessage = "Saved \(envKey) — stored in steelengine.json (skills.entries.\(skillKey).env)"
                }
            } catch {
                self.statusMessage = error.localizedDescription
            }
