...
enum SteelEngineLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = SteelEngineOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SteelEngineFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ...
        let label = SteelEngineLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SteelEngineOSLogHandler: AppLogLevelBackedHandler {
...
struct SteelEngineFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]

    func log(event: LogEvent) {
        guard AppLogSettings.fileLoggingEnabled() else { return }
        let (subsystem, category) = SteelEngineLogging.parseLabel(self.label)
