...
final class CLIInstallPrompter {
    ...
    private let logger = Logger(subsystem: "ai.steelengine", category: "cli.prompt")
    ...
    func installTargetForCurrentBuild(confirmStable: Bool = false) -> CLIInstaller.InstallTarget? {
            ...
            isDebug: CLIInstallBuild.isDebug)
        {
            ...
            let alert = NSAlert()
            alert.messageText = "Install SteelEngine CLI?"
    ...
    private func chooseChannel(suggested: CLIInstaller.Channel) -> CLIInstaller.Channel? {
        ...
        let alert = NSAlert()
        alert.messageText = "Choose SteelEngine CLI channel"
        alert.informativeText =
            "This is an unreleased SteelEngine build. " +
        ...
        restartManagedGateway: Bool = false) async -> Bool
    {
        ...
        if installed {
            ...
            if usesLocalGateway {
                await status.set("Starting SteelEngine Gateway…")
                if !showCompletionAlert {
                    self.logger.info("managed CLI repair: Starting SteelEngine Gateway…")
                }
            ...
            case .ready:
                "SteelEngine Gateway is ready."
            case .deferred:
                "SteelEngine is installed. The Gateway will start when This Mac is active and resumed."
            case .failed:
                "SteelEngine was installed, but the Gateway did not start. Open Settings to retry."
            case nil:
                "SteelEngine CLI is ready for the Mac node."
            }
            await status.set(message)
            if !showCompletionAlert {
                self.logger.info("managed CLI repair: \(message, privacy: .public)")
    ...
    private func openSettings(tab: SettingsTab) {
        ...
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: .steelengineSelectSettingsTab, object: tab)
        }
    }
