...
extension OnboardingView {
    ...
    func startExistingCLIActivationIfNeeded() {
        ...
        OnboardingController.shared.busyReason = "SteelEngine is starting the Gateway service."
        cliStatus = "Starting SteelEngine Gateway…"
    ...
    func finishExistingCLIActivation() async {
        ...
        case .ready:
            cliInstalled = true
            cliStatus = "SteelEngine Gateway is ready."
        case .deferred:
            cliInstalled = false
            cliStatus = "SteelEngine is paused. Resume it, then retry setup to start the Gateway."
        case .failed:
            cliInstalled = false
            cliStatus = "SteelEngine is installed, but the Gateway did not start. Retry setup."
        }
    }
    ...
    func startCLIInstall() {
        ...
        OnboardingController.shared.busyReason = "SteelEngine is installing the Gateway service."
    ...
    func runCLIInstall() async {
        ...
        if !Self.shouldActivateLocalGateway(afterCLIInstallFor: self.state.connectionMode) {
            cliStatus = "SteelEngine CLI is ready for the Mac node."
            cliInstalled = true
            return
        }
        cliStatus = "Starting SteelEngine Gateway…"
        ...
        case .ready:
            cliStatus = "SteelEngine Gateway is ready."
        case .deferred:
            cliStatus = "SteelEngine is installed. The Gateway will start when This Mac is active and resumed."
        case .failed:
            cliStatus = "SteelEngine was installed, but the Gateway did not start. Retry setup."
            return
        }
        cliInstalled = true
    }
    ...
    func refreshLocalGatewayProbe() async {
        ...
        await MainActor.run {
            ...
            let expectedTokens = ["node", "steelengine", "tsx", "pnpm", "bun"]
