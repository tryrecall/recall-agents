...
extension OnboardingAISetupModel {
    ...
    static func friendlyTransportError(_ raw: String) -> String {
        if raw.localizedCaseInsensitiveContains("unknown method") {
            return "The Gateway is running an older SteelEngine version that doesn’t support " +
                "app-guided setup. Update SteelEngine on the gateway, then try again."
        }
        return raw.isEmpty
            ...
            return code == "UNKNOWN_METHOD" ||
                (code == "INVALID_REQUEST" &&
                    (message.contains("unknown method") ||
                        message.contains("invalid steelengine.setup.activate params")))
        }
        return error is GatewayConnectAuthError ||
            error is GatewayTLSValidationError ||
            error is SteelEngineChatTransportSendError
    }
