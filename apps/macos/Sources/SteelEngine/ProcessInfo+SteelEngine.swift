...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.steelengine.mac`) even if the shipped
    ...
    {
        if environment["STEELENGINE_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "steelengine.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "steelengine.nixMode") { return true }
