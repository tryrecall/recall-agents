...
enum ApplicationRelocator {
    ...
    private static let logger = Logger(subsystem: "ai.steelengine", category: "app-relocation")
    ...
    private nonisolated static let replacementSourceBundleEnvironmentKey = "STEELENGINE_REPLACEMENT_SOURCE_BUNDLE"
    private nonisolated static let replacementParentPIDEnvironmentKey = "STEELENGINE_REPLACEMENT_PARENT_PID"
    private nonisolated static let replacementCodeHashEnvironmentKey = "STEELENGINE_REPLACEMENT_CODE_HASH"
    private nonisolated static let replacementReadyFDEnvironmentKey = "STEELENGINE_REPLACEMENT_READY_FD"
    private nonisolated static let replacementBootoutTargetEnvironmentKey = "STEELENGINE_REPLACEMENT_BOOTOUT_TARGET"
    private nonisolated static let replacementSupervisorLabelEnvironmentKey =
        "STEELENGINE_REPLACEMENT_SUPERVISOR_LABEL"
    private nonisolated static let replacementSupervisorPlistEnvironmentKey =
        "STEELENGINE_REPLACEMENT_SUPERVISOR_PLIST"
        ...
        processInfo: ProcessInfo = .processInfo) -> LaunchDisposition
    {
        ...
        case .continueLaunch:
            #if DEBUG
            let monitorDebugReplacement = processInfo.environment["STEELENGINE_MONITOR_APP_REPLACEMENT"] == "1"
            #else
        ...
        case let .offerInstall(destination, replacing):
            ...
            do {
                ...
                showFailure(
                    "SteelEngine couldn’t be installed in Applications. Move it there manually, then open that copy.")
                return .continueLaunch(startUpdater: false)
            }
        case .cannotInstall:
            let message =
                "SteelEngine is running from a temporary location. " +
...
extension ApplicationRelocator {
    ...
    private static func confirmInstall(replacing: Bool) -> Bool {
        ...
        alert.messageText = replacing
            ? "Replace the older SteelEngine in Applications?"
            : "Install SteelEngine in Applications?"
        alert.informativeText = replacing
            ? "This copy is newer than the installed app. SteelEngine will replace it and reopen from Applications."
            : "SteelEngine will copy itself to Applications and reopen there so updates and launch at login stay reliable."
    ...
    private static func relaunchAndTerminate(at destination: URL) -> LaunchDisposition {
        ...
        do {
            ...
            self.showFailure(
                "SteelEngine is installed in Applications, but couldn’t reopen automatically. Open it there manually.")
            return .continueLaunch(startUpdater: false)
        }
    }
    ...
    private static func startSupervisorRestorationWatcher(_ supervisor: KeepAliveSupervisor) -> Bool {
        ...
        watcher.arguments = [
            ...
            "steelengine-supervisor-restoration",
    ...
    private static func showFailure(_ message: String) {
        let alert = NSAlert()
        alert.messageText = "Move SteelEngine to Applications"
