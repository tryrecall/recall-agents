...
enum CommandResolver {
    private static let projectRootDefaultsKey = "steelengine.gatewayProjectRootPath"
    private static let helperName = "steelengine"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let steelengineEntry = root.appendingPathComponent("steelengine.mjs").path
        if FileManager().isReadableFile(atPath: steelengineEntry) { return steelengineEntry }
        let binEntry = root.appendingPathComponent("bin/steelengine.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/steelengine")
    ...
    static func preferredPaths() -> [String] {
        ...
        let validatedExecutable = self.validatedSteelEngineExecutable(
            defaults: .standard,
            fileManager: .default,
        ...
        validatedExecutable: String? = nil) -> [String]
    {
        ...
        let managedPaths = self.steelengineManagedPaths(home: home)
    ...
    static func validatedSteelEngineExecutable(
    ...
    private static func steelengineManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".steelengine"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func steelengineExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSteelEngineExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
        ...
        let sourceEntrypoint = root.appendingPathComponent("steelengine.mjs").path
    ...
    static func steelengineNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let steelenginePath = projectSteelEngineExecutable(projectRoot: root) {
            return [steelenginePath, subcommand] + extraArgs
        }
        if let steelenginePath = steelengineExecutable(searchPaths: searchPaths) {
            return [steelenginePath, subcommand] + extraArgs
        }
        ...
        if let pnpm = findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "steelengine", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            steelengine CLI not found. Install the CLI, or run pnpm build in an SteelEngine source checkout.
            """
    ...
    static func steelengineCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real steelengine CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/steelengine"
        ...
        if command -v steelengine >/dev/null 2>&1; then
          CLI="$(command -v steelengine)"
          steelengine \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/steelengine.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/steelengine.mjs"
            node "$PRJ/steelengine.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/steelengine.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/steelengine.js"
            node "$PRJ/bin/steelengine.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent steelengine"
          pnpm --silent steelengine \(quotedArgs);
        else
          echo "steelengine CLI missing on remote host"; exit 127;
        fi
        """
    ...
    {
        let root = configRoot ?? SteelEngineConfigFile.loadDict()
