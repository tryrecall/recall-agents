...
struct SettingsRootView: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .steelengineSelectSettingsTab)) { note in
        ...
        .onReceive(NotificationCenter.default.publisher(for: .steelengineConfigDidChange)) { _ in
    ...
    private var nixManagedBanner: some View {
        ...
        let configPath = self.snapshotPaths.configPath ?? SteelEnginePaths.configURL.path
        let stateDir = self.snapshotPaths.stateDir ?? SteelEnginePaths.stateDirURL.path
    ...
    private func scheduleInferenceRefresh(clearPrevious: Bool, resetSystemAgent: Bool = false) {
        if resetSystemAgent {
            // SteelEngine sessions are gateway-owned. Re-key the cached detail so a route
            // change cannot send old conversation state to a new endpoint.
            self.systemAgentChatIdentity = UUID()
        }
        if clearPrevious {
            // Preserve an active or pending SteelEngine request while config truth is revalidated.
...
struct SettingsTabGroup: Identifiable {
    ...
    static func defaultGroups(showDebug: Bool, showSystemAgent: Bool) -> [SettingsTabGroup] {
        ...
        var groups = [
            ...
            SettingsTabGroup(title: "SteelEngine", tabs: [.about]),
        ]
...
extension Notification.Name {
    static let steelengineSelectSettingsTab = Notification.Name("steelengineSelectSettingsTab")
}
