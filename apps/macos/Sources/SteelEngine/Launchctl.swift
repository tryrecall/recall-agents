...
enum LaunchAgentPlist {
        ...
        generatedEnvironmentWrapperURL: URL? = nil) -> LaunchAgentPlistSnapshot?
    {
        ...
        let token = env["STEELENGINE_GATEWAY_TOKEN"]?.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty
        let password = env["STEELENGINE_GATEWAY_PASSWORD"]?.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty
        ...
        wrapperURL: URL?) -> [String: String]
    {
        ...
        // Read only the canonical file when the LaunchAgent uses SteelEngine's generated wrapper.
