...
/// Onboarding hero mascot with the steelengine.ai hero treatment: the animated
...
struct GlowingSteelEngineIcon: View {
    @Environment(\.colorScheme) private var colorScheme

    let size: CGFloat
    let mood: SteelEngineMascotMood
    let accessory: SteelEngineMascotAccessory
    ...
    var body: some View {
        SteelEngineMascotView(mood: self.mood, accessory: self.accessory, interactive: true)
            .frame(width: self.size, height: self.size)
            .shadow(
                color: SteelEngineMascotView.heroGlowColor(for: self.colorScheme),
...
extension OnboardingView {
    ...
    var mascotMood: SteelEngineMascotMood {
    ...
    var mascotAccessory: SteelEngineMascotAccessory {
    ...
    static func mascotMood(for snapshot: MascotMoodSnapshot) -> SteelEngineMascotMood {
    ...
    static func mascotAccessory(for page: MascotPage) -> SteelEngineMascotAccessory {
        switch page {
        case .ready: .gradCap
    ...
    static func memoryImportMood(for phase: OnboardingMemoryImportModel.Phase) -> SteelEngineMascotMood {
        switch phase {
