...
extension Notification.Name {
    static let steelenginePermissionsChanged = Notification.Name("steelengine.permissions.changed")
}
...
enum PermissionManager {
    ...
    static func ensure(_ caps: [Capability], interactive: Bool) async -> [Capability: Bool] {
        ...
        if interactive {
            await MainActor.run {
                NotificationCenter.default.post(name: .steelenginePermissionsChanged, object: nil)
            }
        }
        return results
    }
...
enum AppleScriptPermission {
    private static let logger = Logger(subsystem: "ai.steelengine", category: "AppleScriptPermission")
