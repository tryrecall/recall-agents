...
enum GatewayDiscoverySelectionSupport {
        ...
        state: AppState)
    {
        ...
        if preferredTransport == .direct {
            SteelEngineConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.direct.rawValue)
            if !state.remoteUrl.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                SteelEngineConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
            } else {
                SteelEngineConfigFile.clearRemoteGatewayUrl()
            }
        } else {
            SteelEngineConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.ssh.rawValue)
            SteelEngineConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
        }
    }
