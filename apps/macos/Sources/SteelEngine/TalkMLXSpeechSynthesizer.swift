import Dispatch
import Foundation
import SteelEngineMLXTTSProtocol
import OSLog
...
actor TalkMLXSpeechSynthesizer {
    ...
    private let logger = Logger(subsystem: "ai.steelengine", category: "talk.mlx")
    ...
    fileprivate static func helperInvocation() -> HelperInvocation {
        ...
        if let override = ProcessInfo.processInfo.environment["STEELENGINE_MLX_TTS_BIN"], !override.isEmpty {
        ...
        if let executableDir = Bundle.main.executableURL?.deletingLastPathComponent() {
            let bundled = executableDir.appendingPathComponent("steelengine-mlx-tts")
        ...
        return HelperInvocation(
            ...
            argumentPrefix: ["steelengine-mlx-tts"],
            displayName: "steelengine-mlx-tts")
    }
