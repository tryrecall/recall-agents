        ...
        return HStack(alignment: .center, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(complete ? "All access granted" : "\(granted) of \(total) permissions granted")
                    .font(.headline)
                Text("SteelEngine only asks for macOS capabilities when a feature needs them.")
                    .font(.footnote)
...
private struct LocationAccessSettings: View {
    ...
    @AppStorage(locationModeKey) private var locationModeRaw: String = SteelEngineLocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = SteelEngineLocationMode.off.rawValue
        ...
        VStack(spacing: 0) {
                ...
                subtitle: "Allow agents to use device location when a tool asks for it.")
            {
                Picker("Location Access", selection: self.$locationModeRaw) {
                    Text("Off").tag(SteelEngineLocationMode.off.rawValue)
                    Text("While Using").tag(SteelEngineLocationMode.whileUsing.rawValue)
                    Text("Always").tag(SteelEngineLocationMode.always.rawValue)
                }
                .labelsHidden()
                .pickerStyle(.menu)
    ...
    private var locationMode: SteelEngineLocationMode {
        SteelEngineLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: SteelEngineLocationMode) async -> Bool {
