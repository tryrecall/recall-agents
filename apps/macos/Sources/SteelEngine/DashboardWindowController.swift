...
/// pinned at `--steelengine-native-titlebar-height`, resurrecting the traffic-light
...
final class DashboardWindowController: NSWindowController, WKNavigationDelegate, WKUIDelegate, NSWindowDelegate {
    private static let linkMessageHandlerName = "steelengineLink"
    private static let windowDragMessageHandlerName = "steelengineWindowDrag"
    private static let updateMessageHandlerName = "steelengineUpdate"
    ...
    func setUpdateBridgeEnabled(_ enabled: Bool) {
        ...
        self.webView.evaluateJavaScript(
            "window.dispatchEvent(new CustomEvent('steelengine:native-update-availability-changed'))")
    }
    ...
    private static func makeJavaScriptConfirmAlert(message: String, host: String?) -> NSAlert {
        let alert = NSAlert()
        alert.messageText = "SteelEngine Dashboard"
    ...
    fileprivate func receiveUpdateMessage(_ message: WKScriptMessage) {
        ...
        guard DashboardManager.updateBridgeEnabled(mode: AppStateStore.shared.connectionMode) else {
            ...
            self.webView.evaluateJavaScript(
                "window.dispatchEvent(new CustomEvent('steelengine:native-update-declined'))")
            return
        }
    ...
    private func publishNativeHistoryState() {
        ...
        self.webView.evaluateJavaScript(
            """
            window.__STEELENGINE_NATIVE_HISTORY__ = {canGoBack:\(canGoBack),canGoForward:\(canGoForward)};
            window.dispatchEvent(new CustomEvent('steelengine:native-history-state', \
            {detail:window.__STEELENGINE_NATIVE_HISTORY__}));
            """)
    }
    ...
    private static func makeWindow(contentView: NSView) -> NSWindow {
        ...
        // (--steelengine-native-titlebar-height); without it they hug the top edge.
    ...
    private static func installNativeChromeScript(into userContentController: WKUserContentController) {
        ...
        let capabilityScript = "window.__STEELENGINE_NATIVE_WEB_CHROME__ = true;"
        ...
        // `html.steelengine-native-macos` styles fold the titlebar clearance into
        ...
        @media (min-width: 700px) {
          ...
          html.steelengine-native-macos .sidebar-shell,
          html.steelengine-native-macos .settings-sidebar__header {
            padding-top: max(14px, var(--steelengine-native-titlebar-height)) !important;
          }
        }
        """
        let script = """
        (() => {
          try {
            if (document.getElementById("steelengine-native-macos-chrome")) return;
            const style = document.createElement("style");
            style.id = "steelengine-native-macos-chrome";
            style.textContent = \(Self.jsStringLiteral(css));
            document.documentElement.classList.add("steelengine-native-macos", "steelengine-native-web-chrome");
