...
extension Notification.Name {
    static let steelengineCLIInstalled = Notification.Name("steelengine.cli.installed")
}
...
enum CLIInstaller {
    ...
    enum Status: Equatable {
        ...
        var message: String {
            ...
            case let .ready(_, version):
                "SteelEngine Gateway \(version) is ready."
            case .missing:
                "SteelEngine Gateway is not installed yet."
            case .unusable:
                "The SteelEngine Gateway could not be verified. Setup will repair it."
        ...
        fileManager: FileManager) -> [String]
    {
        ...
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("steelengine").path
    ...
    static func managedExecutableLocation() -> String {
        URL(fileURLWithPath: self.installPrefix())
            .appendingPathComponent("bin/steelengine")
            .path
    }
        ...
        statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async -> Bool
    {
        ...
        await statusHandler("Installing SteelEngine CLI (\(target.selector))…")
        guard let installerURL = Bundle.main.url(forResource: "install-cli", withExtension: "sh") else {
            await statusHandler("Install failed: installer resource is missing. Reinstall SteelEngine.")
            return false
        }
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed steelengine \($0)." } ?? "Installed steelengine."
            self.rememberInstallPolicy(target)
            await statusHandler(summary)
            NotificationCenter.default.post(name: .steelengineCLIInstalled, object: nil)
            return true
        }
    ...
    private static func installPrefix() -> String {
        FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".steelengine")
            .path
    }
        ...
        -> ManagedCLIUpdateOutcome
    {
        ...
        await statusHandler(repair
            ? String(localized: "Repairing the SteelEngine Gateway update…")
            : String(localized: "Updating the SteelEngine Gateway to \(targetVersion)…"))
        ...
        NotificationCenter.default.post(name: .steelengineCLIInstalled, object: nil)
        await statusHandler(String(localized: "SteelEngine Gateway \(installedVersion) is installed."))
        return .success(
    ...
    private static func devCheckoutLocation(prefix: String) -> String {
        URL(fileURLWithPath: prefix)
            .appendingPathComponent("dev/steelengine")
            .path
    }
