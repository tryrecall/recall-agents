...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["STEELENGINE_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/steelengine")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("steelengine-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("steelengine-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/steelengine/ (rolling or stdout), or nil if none exist.
