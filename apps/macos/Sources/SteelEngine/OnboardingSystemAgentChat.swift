...
/// Onboarding talks to SteelEngine over the gateway `steelengine.chat` RPC.
...
final class SystemAgentOnboardingChatModel {
    ...
    var input = ""
    /// Set when SteelEngine hands off to the normal agent ("talk to agent").
    ...
    /// status/repair greeting (used by Settings → SteelEngine).
    ...
    func startIfNeeded() async {
        ...
        if Task.isCancelled, self.requestGeneration == generation {
            self.started = false
            self.errorMessage = "SteelEngine was interrupted. Restart to try again."
        }
    }
        ...
        do {
            ...
            if error is CancellationError || Task.isCancelled {
                ...
                self.errorMessage = Task.isCancelled
                    ? "SteelEngine was interrupted. Restart to try again."
                    : "The Gateway connection changed. Restart SteelEngine to reconnect."
                return
            }
            ...
            HStack(spacing: 8) {
                Group {
                    if self.model.expectsSensitiveReply {
                        ...
                        TextField(
                            "Reply to SteelEngine… (yes sets everything up)",
...
private struct SystemAgentChatBubble: View {
    ...
    private var attributedText: AttributedString {
        // SteelEngine replies use light markdown (headings, bold, backticks).
