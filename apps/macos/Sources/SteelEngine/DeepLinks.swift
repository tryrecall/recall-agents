...
private let deepLinkLogger = Logger(subsystem: "ai.steelengine", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        case let .agent(link):
            guard !AppStateStore.shared.isPaused else {
                self.presentAlert(title: "SteelEngine is paused", message: "Unpause SteelEngine to run agent actions.")
                return
            }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run SteelEngine agent?", message: body) else { return }
        }
