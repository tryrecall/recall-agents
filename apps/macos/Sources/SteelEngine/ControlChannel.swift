...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SteelEngineProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.steelengine", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SteelEngineKit.AnyCodable]()) {
                $0[$1.key] = SteelEngineKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking SteelEngine Local Network access.
                    Allow SteelEngine in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SteelEngineProtocol.AnyCodable?) -> [String: SteelEngineProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SteelEngineProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SteelEngineKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SteelEngineProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("steelengine.control.heartbeat")
}
