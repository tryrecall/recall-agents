...
enum ExecApprovalsStore {
    private static let logger = Logger(subsystem: "ai.steelengine", category: "exec-approvals")
    ...
    private static func trustedRootURL() -> URL {
        guard let configured = SteelEngineEnv.path("STEELENGINE_HOME") else {
    ...
    private static func stateDirURL() -> URL {
        guard let configured = SteelEngineEnv.path("STEELENGINE_STATE_DIR") else {
            return self.trustedRootURL().appendingPathComponent(".steelengine", isDirectory: true)
        }
...
extension ExecApprovalsStore {
    ...
    static func expandPath(_ raw: String) -> String {
        ...
        let configuredHome = SteelEngineEnv.path("STEELENGINE_HOME")
