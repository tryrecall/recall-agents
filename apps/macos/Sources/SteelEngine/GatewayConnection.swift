...
private let gatewayConnectionLogger = Logger(subsystem: "ai.steelengine", category: "gateway.connection")
...
private enum GatewayActivationBindingKeyStore {
    private static let service = "ai.steelengine.onboarding-route-binding"
...
actor GatewayConnection {
    ...
    nonisolated static let operatorClientCaps = [SteelEngineGatewayClientCapability.inlineWidgets]
...
extension GatewayConnection {
    ...
    func sessionRoutingIdentity(
        ifCurrentRoute route: Route) async throws -> SteelEngineChatSessionRoutingIdentity
    {
        let data = try await request(
            SteelEngineChatGatewayRequests.agentsList(),
            ifCurrentRoute: route)
        return try SteelEngineChatGatewayPayloadCodec.decodeSessionRoutingIdentity(data)
    }
...
extension GatewayConnection {
    ...
    private func sessionDefaultString(_ defaults: [String: SteelEngineProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(SteelEngineGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> SteelEngineSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return SteelEngineSessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        ...
        maxChars: Int? = nil,
        timeoutMs: Int? = nil,
        ifCurrentRoute route: Route? = nil) async throws -> SteelEngineChatHistoryPayload
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        ...
        if let route {
            let data = try await self.request(
                request,
                ifCurrentRoute: route)
            return try self.decoder.decode(SteelEngineChatHistoryPayload.self, from: data)
        }
        ...
        distinguishPreDispatchRouteChange: Bool = false) async throws -> SteelEngineChatSendResponse
    {
        ...
        if let route {
            let data = try await self.request(
                ...
                ifCurrentRoute: route,
                distinguishPreDispatchRouteChange: distinguishPreDispatchRouteChange)
            return try self.decoder.decode(SteelEngineChatSendResponse.self, from: data)
        }
        let data = try await self.request(request)
        return try self.decoder.decode(SteelEngineChatSendResponse.self, from: data)
    }
