...
/// (`~/Library/Application Support/SteelEngine/chat-cache.sqlite`), matching the
/// existing SteelEngine app-support layout. macOS has no per-file Data Protection
...
enum MacChatTranscriptCache {
    struct Context {
        let store: SteelEngineChatSQLiteTranscriptCache
    ...
    @MainActor private static var storesByGatewayID: [String: SteelEngineChatSQLiteTranscriptCache] = [:]
    ...
    ///   session data: distinct profiles (distinct `STEELENGINE_STATE_DIR`) never
    ...
    static func currentGatewayID() -> String? {
        ...
        return self.gatewayID(
            mode: mode,
            localStateDir: SteelEngineConfigFile.stateDirURL(),
    ...
    static func makeContext() -> Context? {
        ...
        let databaseURL = base.appendingPathComponent("SteelEngine/chat-cache.sqlite", isDirectory: false)
        return Context(
            store: self.store(databaseURL: databaseURL, gatewayID: gatewayID),
            routingIdentity: SteelEngineChatSQLiteTranscriptCache.loadSessionRoutingIdentity(
                databaseURL: databaseURL,
                gatewayID: gatewayID))
    }

    @MainActor
    static func store(databaseURL: URL, gatewayID: String) -> SteelEngineChatSQLiteTranscriptCache {
        if let store = storesByGatewayID[gatewayID] {
            return store
        }
        let store = SteelEngineChatSQLiteTranscriptCache(databaseURL: databaseURL, gatewayID: gatewayID)
