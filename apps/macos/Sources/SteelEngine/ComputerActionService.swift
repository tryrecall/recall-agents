...
final class ComputerActionExecutionQueue {
    typealias Operation = @MainActor (SteelEngineComputerActParams, UInt64) async throws
        -> SteelEngineComputerActResult
    typealias CancellationHop = @Sendable (
        @escaping @MainActor @Sendable () -> Void) -> Void

    private struct QueuedAction {
        let id: UUID
        let params: SteelEngineComputerActParams
        let lifecycleGeneration: UInt64
        let operation: Operation
        let continuation: CheckedContinuation<SteelEngineComputerActResult, Error>
    ...
    private var currentActionTask: Task<SteelEngineComputerActResult, Error>?
    ...
    func perform(
        ...
        operation: @escaping Operation) async throws -> SteelEngineComputerActResult
    {
        let actionID = UUID()
        let cancellationState = ComputerActionCancellationState()
    ...
    private func drain() async {
        while !self.pendingActions.isEmpty {
            ...
            let outcome: Result<SteelEngineComputerActResult, Error>
            do {
...
final class ComputerActionService {
        ...
        lifecycleGeneration: UInt64) async throws -> SteelEngineComputerActResult
    {
        ...
        return SteelEngineComputerActResult(ok: true, cursorX: cursor.x, cursorY: cursor.y)
    }
    ...
    private func peekabooClick(at point: CGPoint, action: SteelEngineComputerAction) async throws {
    ...
    private struct ResolvedDisplay {
        var geometry: SteelEngineComputerDisplayGeometry
    ...
    private func point(
        _ x: Double?,
        _ y: Double?,
        params: SteelEngineComputerActParams,
        display: ResolvedDisplay) throws -> CGPoint?
    {
        if x == nil, y == nil {
            return nil
        ...
        let capturedWidth = SteelEngineComputerInputGeometry.capturedWidth(
            refWidth: refWidth,
            sourceWidth: display.sourceWidth,
            sourceHeight: display.sourceHeight)
        let mapped = SteelEngineComputerInputGeometry.mapReferencePointToGlobal(
            x: x,
            y: y,
        ...
        let clamped = SteelEngineComputerInputGeometry.clampToDisplay(
            x: mapped.x,
            y: mapped.y,
            display: geometry)
    ...
    static func validatedCurrentCursorPoint(
        _ point: CGPoint?,
        display: SteelEngineComputerDisplayGeometry) throws -> CGPoint
    {
        guard let point,
    ...
    static func validateHeldButtonTransition(
        action: SteelEngineComputerAction,
    ...
    func typeTextForTesting(
        _ text: String,
        lifecycleGeneration: UInt64 = 0) async throws -> SteelEngineComputerActResult
    {
        let params = SteelEngineComputerActParams(action: .type, text: text)
    ...
    private func resolveDisplay(params: SteelEngineComputerActParams) async throws -> ResolvedDisplay {
        ...
        let geometry = SteelEngineComputerDisplayGeometry(
        ...
        guard SteelEngineComputerInputGeometry.isValidMappingGeometry(
        ...
        if Self.usesScreenshotCoordinates(params) {
            let refWidth = try Self.requiredReferenceWidth(params)
            let currentFrameId = SteelEngineComputerInputGeometry.displayFrameId(
    ...
    private static func usesScreenshotCoordinates(_ params: SteelEngineComputerActParams) -> Bool {
        params.x != nil || params.y != nil || params.fromX != nil || params.fromY != nil
    }

    private static func requiredReferenceWidth(_ params: SteelEngineComputerActParams) throws -> Int {
    ...
    private static func scrollDirection(
        _ direction: SteelEngineComputerScrollDirection) -> PeekabooFoundation.ScrollDirection
    {
        switch direction {
        case .up: .up
        case .down: .down
    ...
    private func rawScroll(direction: SteelEngineComputerScrollDirection, amount: Int, flags: CGEventFlags) throws {
