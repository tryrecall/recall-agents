...
final class MacNodeModeCoordinator: NSObject {
    ...
    private let logger = Logger(subsystem: "ai.steelengine", category: "mac-node")
    ...
    override private convenience init() {
        ...
        let nodeHostWorker = MacNodeHostWorker(session: session) {
            NotificationCenter.default.post(name: .steelengineNodeHostWorkerFailed, object: nil)
        }
        self.init(
            session: session,
        ...
        routeInvalidationHook: (@Sendable () async -> Void)? = nil)
    {
        ...
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.refreshNodeConfiguration),
            name: .steelenginePermissionsChanged,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostWorkerFailed),
            name: .steelengineNodeHostWorkerFailed,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostConfigurationChanged),
            name: .steelengineConfigDidChange,
            object: nil)
        self.notificationCenter.addObserver(
            self,
            selector: #selector(self.nodeHostConfigurationChanged),
            name: .steelengineCLIInstalled,
            object: nil)
    }
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = SteelEngineConfigFile.browserControlEnabled()
        ...
        claudeSessionCatalogEnabled: Bool) async throws -> ConnectionAttempt?
    {
        ...
        if !nativeCaps.contains(SteelEngineCapability.computer.rawValue) {
            await self.runtime.releaseHeldComputerInput()
        }
            ...
            onInvoke: { [weak self] req in
                guard let self else {
                    return BridgeInvokeResponse(
                        id: req.id,
                        ok: false,
                        error: SteelEngineNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                }
        ...
        claudeSessionCatalogEnabled: Bool) -> [String]
    {
        ...
        return Self.resolvedCaps(
            ...
            cameraEnabled: cameraEnabled,
            computerControlEnabled: computerControlEnabled,
            locationMode: SteelEngineLocationMode(rawValue: rawLocationMode) ?? .off,
            connectionMode: AppStateStore.shared.connectionMode,
            codexThreadCatalogEnabled: codexThreadCatalogEnabled,
            claudeSessionCatalogEnabled: claudeSessionCatalogEnabled)
    }
    ...
    private func startNodeHostWorkerIfConfigured() async throws -> MacNodeHostManifest? {
        ...
        if let projectExecutable = CommandResolver.projectSteelEngineExecutable() {
    ...
    private func buildSessionBox(url: URL, connectionMode: AppState.ConnectionMode) -> WebSocketSessionBox? {
        ...
        guard let params = Self.tlsParams(
            ...
            root: SteelEngineConfigFile.loadDict(),
...
extension MacNodeModeCoordinator {
    nonisolated static func resolvedCaps(
        ...
        locationMode: SteelEngineLocationMode,
        connectionMode: AppState.ConnectionMode,
        ...
        claudeSessionCatalogEnabled: Bool = false) -> [String]
    {
        ...
        if computerControlEnabled {
            caps.append(SteelEngineCapability.computer.rawValue)
        }
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        ...
        if capsSet.contains(SteelEngineCapability.camera.rawValue) {
        ...
        if capsSet.contains(SteelEngineCapability.location.rawValue) {
        ...
        if capsSet.contains(SteelEngineCapability.computer.rawValue) {
            commands.append(SteelEngineComputerCommand.act.rawValue)
        }
