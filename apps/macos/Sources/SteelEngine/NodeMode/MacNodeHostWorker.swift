...
extension Notification.Name {
    static let steelengineNodeHostWorkerFailed = Notification.Name("steelengine.node-host-worker.failed")
}
...
final class MacNodeHostWorker: MacNodeHostWorking, @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "ai.steelengine", category: "node-host-worker")
    private let queue = DispatchQueue(label: "ai.steelengine.node-host-worker")
    private let writerQueue = DispatchQueue(label: "ai.steelengine.node-host-worker.writer")
    ...
    private func startLocked(command: [String]) {
        ...
        environment["STEELENGINE_NODE_EXEC_HOST"] = "app"
        environment["STEELENGINE_NODE_EXEC_FALLBACK"] = "0"
    ...
    private static func decodeInvokeResponse(_ result: [String: Any], id: String) -> BridgeInvokeResponse {
        ...
        let code = SteelEngineNodeErrorCode(rawValue: rawError?["code"] as? String ?? "UNAVAILABLE") ?? .unavailable
        let error = ok ? nil : SteelEngineNodeError(
            code: code,
    ...
    private static func unavailableResponse(_ id: String, _ message: String) -> BridgeInvokeResponse {
        BridgeInvokeResponse(
            id: id,
            ok: false,
            error: SteelEngineNodeError(code: .unavailable, message: message))
    }
