...
enum MacNodeCodexThreadCatalog {
    ...
    private static let commandEnvironmentKey = "STEELENGINE_CODEX_APP_SERVER_BIN"
    private static let argumentsEnvironmentKey = "STEELENGINE_CODEX_APP_SERVER_ARGS"
    ...
    static func list(paramsJSON: String?) async throws -> String {
        try await self.list(paramsJSON: paramsJSON) {
            SteelEngineConfigFile.loadDict()
        }
    }
    ...
    static func shouldAdvertise(root: [String: Any]? = nil) -> Bool {
        let root = root ?? SteelEngineConfigFile.loadDict()
        guard SteelEngineConfigFile.configuredBundledPluginAllowed(
...
extension MacNodeCodexThreadCatalog {
    ...
    private static func configuredPlugin(root: [String: Any]) throws -> ConfiguredPlugin? {
        guard let entry = SteelEngineConfigFile.pluginEntry(
            MacNodeCodexThreadCatalogContract.pluginId,
            root: root)
        else { return nil }
...
private final class CodexAppServerThreadRequestSession: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "ai.steelengine.codex-thread-catalog")
    ...
    private static func initializeRequestData() throws -> Data {
        try self.jsonData([
            ...
            "params": [
                "clientInfo": [
                    "name": "steelengine_macos",
                    "title": "SteelEngine macOS Node",
