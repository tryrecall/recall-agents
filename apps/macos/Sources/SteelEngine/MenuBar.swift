...
struct SteelEngineApp: App {
    ...
    private static let logger = Logger(subsystem: "ai.steelengine", category: "app")
    ...
    init() {
        SteelEngineLogging.bootstrapIfNeeded()
    ...
    var body: some Scene {
        ...
        Window("SteelEngine Settings", id: SettingsWindowOpener.windowID) {
    ...
    private func applyStatusItemAppearance(paused _: Bool, sleeping _: Bool) {
        ...
        self.statusItem?.button?.toolTip = self.state.voiceWakeMeterActive
            ? "SteelEngine - Voice Wake live meter active"
            : "SteelEngine"
    }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    private static let dashboardURL = URL(string: "steelengine://dashboard")!
    private var state: AppState?
    private var terminationCleanupTask: Task<Void, Never>?
    private var terminationDeadlineTask: Task<Void, Never>?
    private var terminationCleanupFinished = false
    private let webChatAutoLogger = Logger(subsystem: "ai.steelengine", category: "Chat")
    ...
    func applicationDidFinishLaunching(_: Notification) {
        ...
        if ProcessInfo.processInfo.environment["STEELENGINE_DEBUG_PAIRING_DEMO"] == "1" {
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
    ...
    private func scheduleFirstRunOnboardingIfNeeded(gatewayConnected: Bool) async {
           ...
           gatewayConnected
        {
            ...
            // default-agent model cannot run SteelEngine and must stay in onboarding.
            do {
...
extension SparkleUpdaterController: SPUUpdaterDelegate {
    func allowedChannels(for _: SPUUpdater) -> Set<String> {
        allowedSparkleChannels(forGatewayUpdateChannel: SteelEngineConfigFile.gatewayUpdateChannel())
    }
