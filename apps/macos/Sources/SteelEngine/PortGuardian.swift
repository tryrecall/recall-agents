...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "ai.steelengine", category: "portguard")
    #if DEBUG
    private var testingDescriptors: [Int: Descriptor] = [:]
    #endif
    private nonisolated static let appSupportDir: URL = {
        let base = FileManager().urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return base.appendingPathComponent("SteelEngine", isDirectory: true)
    }()
    ...
    static func classifyTunnelRecord(_ record: Record, process: TunnelProcessInfo?) -> TunnelRecordAction {
        ...
        // live parent may be a concurrent SteelEngine instance (prod + dev), so hands off.
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "steelengine", "tsx", "pnpm", "bun"]
        ...
        managedGatewayPID: Int32? = nil) -> Bool
    {
        ...
        case .local:
            ...
            if full.contains("gateway-daemon") || full.contains("steelengine-gateway")
                || cmd.contains("steelengine-gateway")
            {
                return true
            }
            if self.isNodeSteelEngineGatewayCommand(full) { return true }
            // If args are unavailable, treat a CLI listener as expected.
            if cmd.contains("steelengine"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
    ...
    private static func isNodeSteelEngineGatewayCommand(_ fullCommand: String) -> Bool {
        ...
        return self.isSteelEngineDistEntrypointToken(tokens[1])
            && tokens[2].lowercased() == "gateway"
    }

    private static func isSteelEngineDistEntrypointToken(_ token: String) -> Bool {
