...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.steelengine", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SteelEngineSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SteelEngineSessionPreviewEntry,
    ...
    private static func cache(payload: SteelEngineSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SteelEngineChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SteelEngineKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SteelEngineKit.AnyCodable]) -> [SteelEngineChatMessage] {
    ...
    private static func previewText(for message: SteelEngineChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SteelEngineChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SteelEngineChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SteelEngineChatMessage) -> String? {
