...
/// Menu contents for the SteelEngine menu bar extra.
struct MenuContent: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            self.debugMenu
            Button("About SteelEngine") { self.open(tab: .about) }
    ...
    private var connectionLabel: String {
        ...
        case .unconfigured:
            "SteelEngine Not Configured"
        case .remote:
            "Remote SteelEngine Active"
        case .local:
            "SteelEngine Active"
        }
    }
    ...
    private var macNodeStatus: (label: String, color: Color)? {
        ...
        if let entry = self.nodesStore.nodes.first(where: { $0.nodeId == deviceId }) {
            ...
            let missingRequiredCommands = [
                ...
                SteelEngineSystemCommand.run.rawValue,
                SteelEngineSystemCommand.which.rawValue,
