...
final class AppState {
    private static let logger = Logger(subsystem: "ai.steelengine", category: "app-state")
        ...
        execApprovalsReadRetryDelay: Duration = .milliseconds(250))
    {
        ...
        let configRoot = SteelEngineConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            SteelEngineConfigFile.canonicalHostForComparison(host) ==
                SteelEngineConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = SteelEngineConfigFile.url()
    ...
    private func applyConfigFromDisk() {
        ...
        NotificationCenter.default.post(name: .steelengineConfigDidChange, object: nil)
    }
...
extension AppState {
    ...
    func syncGatewayConfigNow() -> Bool {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: SteelEngineConfigFile.loadDict(),
            draft: draft)
        guard synced.changed else {
            self.setGatewayConfigSyncState(.current)
            return true
        }
        guard SteelEngineConfigFile.saveDict(synced.root) else {
            self.setGatewayConfigSyncState(.failed)
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/steelengine"
        state.remoteCliPath = ""
        return state
    }
}
