...
struct SteelEngineMacCLI {
    static func main() async {
        ...
        case let .unknown(exitCode):
            fputs("steelengine-mac: unknown command\n", stderr)
            printUsage()
            exit(exitCode)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      steelengine-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      steelengine-mac discover [--timeout <ms>] [--json] [--include-local]
      steelengine-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      steelengine-mac connect
      steelengine-mac configure-remote --ssh-target user@gateway.local --remote-port 18789
      steelengine-mac connect --url ws://127.0.0.1:18789 --json
      steelengine-mac discover --timeout 3000 --json
      steelengine-mac wizard --mode local
    """)
}
