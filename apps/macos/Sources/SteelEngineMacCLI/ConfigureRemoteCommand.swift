...
private let appDefaultsSuites = ["ai.steelengine.mac", "ai.steelengine.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            ...
            Usage:
              steelengine-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
              ...
              steelengine-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote SteelEngine checkout for CLI commands.
              --cli-path <path>   Remote steelengine executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
    ...
    defaultsSuites: [String]) throws -> ConfigureRemoteOutput
{
    ...
    let configURL = steelEngineConfigURL()
...
private func steelEngineConfigURL() -> URL {
    if let raw = ProcessInfo.processInfo.environment["STEELENGINE_CONFIG_PATH"],
       !raw.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    {
        return URL(fileURLWithPath: NSString(string: raw).expandingTildeInPath)
    }
    return FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".steelengine/steelengine.json")
}
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String, suites: [String]) {
    for suite in suites {
        ...
        defaults.set("remote", forKey: "steelengine.connectionMode")
        setDefaultString(defaults, key: "steelengine.remoteTarget", value: target)
        defaults.set(true, forKey: "steelengine.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "steelengine.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "steelengine.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "steelengine.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "steelengine.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("SteelEngine macOS Remote Config")
