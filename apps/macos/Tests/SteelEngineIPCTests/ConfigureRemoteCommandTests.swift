import Foundation
import Testing
@testable import SteelEngineMacCLI
...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("steelengine-configure-remote-\(UUID().uuidString).json")
        ...
        try await TestIsolation.withIsolatedState(env: ["STEELENGINE_CONFIG_PATH": configURL.path]) {
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "steelengine.connectionMode") == "remote")
                #expect(defaults.string(forKey: "steelengine.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "steelengine.onboardingSeen") == true)
                #expect(defaults.string(forKey: "steelengine.remoteCliPath") == "/opt/homebrew/bin/steelengine")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("steelengine-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote defaults SSH host key policy to strict`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("steelengine-configure-remote-strict-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("steelengine-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("steelengine-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["STEELENGINE_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
