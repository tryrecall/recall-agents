...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: SteelEngineProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SteelEngineProtocol.AnyCodable] = [
            "presence": .init([SteelEngineProtocol.AnyCodable(entry)]),
        ]
        let payload = SteelEngineProtocol.AnyCodable(payloadMap)
