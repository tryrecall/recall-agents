...
struct MacNodeModeCoordinatorTests {
    ...
    @Test @MainActor func `config and CLI changes restart startup scoped node host worker`() async throws {
        ...
        notificationCenter.post(name: .steelengineConfigDidChange, object: nil)

        try await self.waitUntil("node-host worker restart", timeout: restartTimeout) {
            await worker.stops() == 1
        }

        notificationCenter.post(name: .steelengineCLIInstalled, object: nil)
    ...
    @Test func `native manifest excludes CLI-owned node commands`() {
        ...
        #expect(!caps.contains(SteelEngineCapability.browser.rawValue))
        #expect(!commands.contains(SteelEngineBrowserCommand.proxy.rawValue))
        #expect(commands.contains(SteelEngineCanvasCommand.present.rawValue))
        #expect(commands.contains(SteelEngineSystemCommand.notify.rawValue))
        #expect(!commands.contains(SteelEngineFileSystemCommand.listDir.rawValue))
    ...
    @Test func `Codex supervision activation respects the plugin flag and global policy`() {
        ...
        #expect(SteelEngineConfigFile.explicitlyEnabledPluginConfigFlag(
            "codex",
        ...
        #expect(SteelEngineConfigFile.configuredBundledPluginAllowed(
            "codex",
        ...
        #expect(SteelEngineConfigFile.pluginEntry("codex", root: ambiguousEntryAliases) == nil)
        #expect(!SteelEngineConfigFile.explicitlyEnabledPluginConfigFlag(
            "codex",
            path: ["supervision", "enabled"],
    ...
    @Test func `computer control cap gates the computer.act command`() {
        ...
        #expect(enabledCaps.contains(SteelEngineCapability.computer.rawValue))
        #expect(enabledCommands.contains(SteelEngineComputerCommand.act.rawValue))
        ...
        #expect(!disabledCaps.contains(SteelEngineCapability.computer.rawValue))
        #expect(!disabledCommands.contains(SteelEngineComputerCommand.act.rawValue))
    }
