...
@Suite(.serialized)
struct SteelEngineConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("steelengine-config-\(UUID().uuidString)")
            .appendingPathComponent("steelengine.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["STEELENGINE_CONFIG_PATH": override]) {
            #expect(SteelEngineConfigFile.url().path == override)
        }
    }
    ...
    func `browser control enabled reads config flag`() async {
        ...
        await TestIsolation.withEnvValues(["STEELENGINE_CONFIG_PATH": override]) {
            #expect(SteelEngineConfigFile.browserControlEnabled() == true)
            SteelEngineConfigFile.saveDict(["browser": ["enabled": false]])
            #expect(SteelEngineConfigFile.browserControlEnabled() == false)
            SteelEngineConfigFile.setBrowserControlEnabled(true)
            #expect(SteelEngineConfigFile.browserControlEnabled() == true)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["STEELENGINE_CONFIG_PATH": override]) {
            SteelEngineConfigFile.saveDict([
                "gateway": [
                    "remote": [
            ...
            #expect(SteelEngineConfigFile.remoteGatewayPort() == 19999)
            #expect(SteelEngineConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(SteelEngineConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(SteelEngineConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(SteelEngineConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(SteelEngineConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["STEELENGINE_CONFIG_PATH": override]) {
            ...
            SteelEngineConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:18789")
            let root = SteelEngineConfigFile.loadDict()
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["STEELENGINE_CONFIG_PATH": override]) {
            ...
            SteelEngineConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = SteelEngineConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["STEELENGINE_CONFIG_PATH": override]) {
            ...
            SteelEngineConfigFile.clearRemoteGatewayUrl()
            let root = SteelEngineConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("steelengine-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "STEELENGINE_CONFIG_PATH": nil,
            "STEELENGINE_STATE_DIR": dir,
        ]) {
            #expect(SteelEngineConfigFile.stateDirURL().path == dir)
            #expect(SteelEngineConfigFile.url().path == "\(dir)/steelengine.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("steelengine-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("steelengine.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "STEELENGINE_STATE_DIR": stateDir.path,
            "STEELENGINE_CONFIG_PATH": configPath.path,
        ]) {
            SteelEngineConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-steelengine-config-file")
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let allowedRoot = SteelEngineConfigFile.loadDict()
    ...
    func `load dict ignores legacy config health sidecar`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try SteelEngineConfigFile.withTestingFileLock {
                let loaded = SteelEngineConfigFile.loadDict()
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try SteelEngineConfigFile.withTestingFileLock {
                ...
                _ = SteelEngineConfigFile.loadDict()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = SteelEngineConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [
