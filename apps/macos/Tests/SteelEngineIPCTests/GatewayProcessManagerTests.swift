...
struct GatewayProcessManagerTests {
        ...
        _ body: () async throws -> T) async throws -> T
    {
        ...
        return try await TestIsolation.withEnvValues(["STEELENGINE_CONFIG_PATH": configPath], body)
    }
    ...
    @Test func `coalesces concurrent launch agent enable requests`() async throws {
        ...
        let marker = FileManager.default.temporaryDirectory
            .appendingPathComponent("steelengine-launchagent-marker-\(UUID().uuidString)")
        try await self.withLocalGatewayConfig {
            ...
            async let first: String? = manager._testEnableLaunchAgentIfNeeded(
                bundlePath: "/Applications/SteelEngine.app",
                port: port)
        ...
        try await self.withLocalGatewayConfig {
            ...
            GatewayLaunchAgentManager.setTestingDaemonStatusPayloads([
                ...
                {"ok":true,"service":{
                  ...
                  "command":{"programArguments":["steelengine","gateway","--port","\(secondPort)"]},
    ...
    @Test func `restart waits for disable before attaching`() async throws {
        ...
        let descriptor = PortGuardian.Descriptor(
            pid: 4242,
            command: "steelengine-gateway",
            executablePath: "/tmp/steelengine-gateway")
        ...
        try await self.withLocalGatewayConfig {
            ...
            GatewayLaunchAgentManager.setTestingDaemonStatusPayload(
                ...
                {"ok":true,"service":{
                  "loaded":true,
                  "runtime":{"status":"running","pid":4242},
                  "command":{"programArguments":["steelengine","gateway","--port","\(port)"]},
                  "configAudit":{"ok":true,"issues":[]}
                }}
                """)
            GatewayLaunchAgentManager.clearTestingDaemonCommandCalls()
        ...
        try await self.withLocalGatewayConfig {
            ...
            let staleStatuses = [
                ...
                {"ok":true,"service":{
                  "loaded":true,
                  "runtime":{"status":"running","pid":4242},
                  "command":{"programArguments":["steelengine","gateway","--port","19084"]},
                  "configAudit":{"ok":true,"issues":[]}
                }}
                """,
                """
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        ...
        do {
            ...
            do {
                ...
                #expect(details.contains("pid 4343 steelengine-gateway @ /tmp/steelengine-gateway"))
                #expect(manager._testControlChannelRefreshForces().last == true)
                await cleanup()
            } catch {
                await cleanup()
