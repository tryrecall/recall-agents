...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers open claw binary`() throws {
        ...
        let cmd = CommandResolver.steelengineCommand(
            subcommand: "gateway",
            defaults: defaults,
            configRoot: [:],
            searchPaths: searchPaths,
            projectRoot: tmp)
        #expect(cmd.prefix(2).elementsEqual([steelenginePath.path, "gateway"]))
    }

    @Test func `source checkout entrypoint wins when package bin link is absent`() throws {
        let defaults = self.makeLocalDefaults()
        let tmp = try makeTempDirForTests()
        let sourceEntrypoint = tmp.appendingPathComponent("steelengine.mjs")
        let staleGlobalBin = tmp.appendingPathComponent("global/bin")
        try makeExecutableForTests(at: sourceEntrypoint)
        try makeExecutableForTests(at: staleGlobalBin.appendingPathComponent("steelengine"))
    ...
    @Test func `falls back to node and script`() throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/steelengine.js")
    ...
    @Test func `prefers open claw binary over pnpm`() throws {
        ...
        let steelenginePath = binDir.appendingPathComponent("steelengine")
        ...
        #expect(cmd.prefix(2).elementsEqual([steelenginePath.path, "rpc"]))
    }
    ...
    @Test func `falls back to pnpm`() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "steelengine", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "steelengine", "health", "--json"]))
    ...
    @Test func `missing CLI explains install and source checkout paths`() throws {
        ...
        #expect(cmd.last?.contains("steelengine CLI not found") == true)
        #expect(cmd.last?.contains("Install the CLI") == true)
        #expect(cmd.last?.contains("run pnpm build in an SteelEngine source checkout") == true)
    }
    ...
    @Test func `managed install only precedes external installs after validation`() throws {
        ...
        let managedBin = home.appendingPathComponent(".steelengine/bin")
        try FileManager().createDirectory(at: managedBin, withIntermediateDirectories: true)
        let managedExecutable = managedBin.appendingPathComponent("steelengine")
    ...
    @Test func `validated CLI preference expires when the app requires a newer version`() throws {
        ...
        let executable = root.appendingPathComponent("steelengine")
        ...
        #expect(CommandResolver.validatedSteelEngineExecutable(
            defaults: defaults,
            fileManager: .default,
    ...
    @Test func `builds SSH command for remote mode`() {
        ...
        defaults.set("steelengine@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/steelengine", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "steelengine@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/steelengine'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("steelengine"))
    ...
    @Test func `remote SSH with explicit blank target fails closed`() throws {
        ...
        defaults.set("stale-gateway-alias", forKey: remoteTargetKey)
        let tmp = try makeTempDirForTests()
        let localSteelEngine = tmp.appendingPathComponent("node_modules/.bin/steelengine")
        try makeExecutableForTests(at: localSteelEngine)

        let command = CommandResolver.steelengineCommand(
            subcommand: "status",
            defaults: defaults,
            configRoot: [
                "gateway": [
    ...
    @Test func `direct remote route uses local CLI despite stale SSH defaults`() throws {
        ...
        #expect(command == [localSteelEngine.path, "status"])
    }
    ...
    @Test func `config root local overrides remote defaults`() throws {
        ...
        #expect(cmd.first == steelenginePath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
