...
struct MacGatewayChatTransportMappingTests {
    @Test func `mac chat advertises inline widgets`() {
        #expect(GatewayConnection.operatorClientCaps == [SteelEngineGatewayClientCapability.inlineWidgets])
    }
    ...
    @Test func `legacy trace preference migrates to independent defaults once`() throws {
        ...
        defaults.set(false, forKey: SteelEngineChatWindowShell.assistantTraceDefaultsKey)

        #expect(WebChatTracePreferences.displayOptions(defaults: defaults).isEmpty)
        #expect(defaults.object(forKey: SteelEngineChatWindowShell.assistantReasoningDefaultsKey) as? Bool == false)
        #expect(defaults.object(forKey: SteelEngineChatWindowShell.assistantToolActivityDefaultsKey) as? Bool == false)

        defaults.set(true, forKey: SteelEngineChatWindowShell.assistantReasoningDefaultsKey)
    ...
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: SteelEngineProtocol.AnyCodable(["ok": SteelEngineProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: SteelEngineProtocol.AnyCodable(["ok": SteelEngineProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `sessions changed event maps to authoritative refresh signal`() {
        let payload = SteelEngineProtocol.AnyCodable([
            "sessionKey": SteelEngineProtocol.AnyCodable("agent:main:main"),
            "agentId": SteelEngineProtocol.AnyCodable("main"),
            "reason": SteelEngineProtocol.AnyCodable("command-metadata"),
        ])
        let frame = EventFrame(
            type: "event",
    ...
    @Test func `chat event maps to chat`() {
        let payload = SteelEngineProtocol.AnyCodable([
            "runId": SteelEngineProtocol.AnyCodable("run-1"),
            "sessionKey": SteelEngineProtocol.AnyCodable("main"),
            "state": SteelEngineProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `session message event maps to session message`() {
        let payload = SteelEngineProtocol.AnyCodable([
            "sessionKey": SteelEngineProtocol.AnyCodable("agent:main:main"),
            "messageId": SteelEngineProtocol.AnyCodable("msg-1"),
            "messageSeq": SteelEngineProtocol.AnyCodable(7),
            "message": SteelEngineProtocol.AnyCodable([
                "role": SteelEngineProtocol.AnyCodable("user"),
                "content": SteelEngineProtocol.AnyCodable([
                    SteelEngineProtocol.AnyCodable([
                        "type": SteelEngineProtocol.AnyCodable("text"),
                        "text": SteelEngineProtocol.AnyCodable("spoken transcript"),
                    ]),
                ]),
                "timestamp": SteelEngineProtocol.AnyCodable(1234.5),
            ]),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: SteelEngineProtocol.AnyCodable(["a": SteelEngineProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
