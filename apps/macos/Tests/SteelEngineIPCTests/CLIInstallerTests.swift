...
struct CLIInstallerTests {
    @Test func `installed location finds executable`() throws {
        ...
        let root = fm.temporaryDirectory.appendingPathComponent(
            "steelengine-cli-installer-\(UUID().uuidString)")
        defer { try? fm.removeItem(at: root) }

        let binDir = root.appendingPathComponent("bin")
        try fm.createDirectory(at: binDir, withIntermediateDirectories: true)
        let cli = binDir.appendingPathComponent("steelengine")
    ...
    @Test func `installer command runs the signed bundled script without a shell pipeline`() {
        let command = CLIInstaller.installScriptCommand(
            ...
            prefix: "/Users/Test User/.steelengine",
            scriptPath: "/Applications/SteelEngine.app/Contents/Resources/install-cli.sh")

        #expect(command == [
            "/bin/bash",
            "/Applications/SteelEngine.app/Contents/Resources/install-cli.sh",
            "--json",
            "--no-onboard",
            "--prefix",
            "/Users/Test User/.steelengine",
            "--version",
            "2026.7.3-beta.1",
        ])
    ...
    @Test func `dev installer uses a managed git main checkout`() {
        ...
        #expect(command.suffix(6) == [
            ...
            "git",
            "--git-dir",
            "/Users/Test User/.steelengine/dev/steelengine",
        ])
    }
    ...
    @Test func `managed update uses the canonical updater without accepting downgrades`() {
        let command = CLIInstaller.managedUpdateCommand(
            executable: "/Users/Test User/.steelengine/bin/steelengine",
            targetVersion: "2026.7.4")

        #expect(command == [
            "/Users/Test User/.steelengine/bin/steelengine",
            "update",
            "--tag",
            "2026.7.4",
            "--json",
    ...
    @Test func `managed setup requires a parseable compatible version`() {
        let location = "/Users/test/.steelengine/bin/steelengine"

        #expect(CLIInstaller.classifyVersion(
            location: location,
            output: "SteelEngine 2026.7.3\n",
    ...
    @Test func `matching external CLI with unsupported Node is unusable`() async throws {
        let root = FileManager().temporaryDirectory.appendingPathComponent(
            "steelengine-old-node-cli-\(UUID().uuidString)")
        defer { try? FileManager().removeItem(at: root) }
        try FileManager().createDirectory(at: root, withIntermediateDirectories: true)
        let executable = root.appendingPathComponent("steelengine")
    ...
    @Test func `CLI probe preserves environment and resolves shebang tools beside executable`() {
        let location = "/custom/bin/steelengine"
    ...
    @Test func `managed CLI probe prefers its private runtime`() {
        let executable = "/Users/test/.steelengine/bin/steelengine"
        let environment = CLIInstaller.probeEnvironment(
            location: executable,
            processEnvironment: [:],
            preferredPaths: ["/Users/test/.nvm/versions/node/v20/bin", "/usr/bin"],
            managedExecutable: executable,
            managedRuntimeDirectory: "/Users/test/.steelengine/tools/node/bin")

        #expect(environment["PATH"] == [
            "/Users/test/.steelengine/bin",
            "/Users/test/.steelengine/tools/node/bin",
