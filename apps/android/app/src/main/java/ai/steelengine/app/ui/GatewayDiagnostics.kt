...
import ai.steelengine.app.gatewayConnectionStatusForDisplay
...
internal fun steelEngineAndroidVersionLabel(): String {
...
internal fun gatewayNodeApprovalCommand(
  ...
  when (state) {
    ...
    -> normalizeGatewayApprovalRequestId(requestId)?.let { "steelengine nodes approve $it" } ?: "steelengine nodes status"
    GatewayNodeApprovalState.Unapproved -> "steelengine nodes status"
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return nativeString(
    "Help diagnose this SteelEngine Android gateway connection failure.\n\n" +
      "Please:\n" +
      ...
      "- tell me whether `steelengine devices list` should show a pending pairing request\n" +
      "- if more signal is needed, ask for `steelengine qr --json`, `steelengine devices list`, and `steelengine nodes status`\n" +
      ...
      "- status/error: \$status",
    screen,
    steelEngineAndroidVersionLabel(),
    device,
    androidVersion,
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("SteelEngine gateway diagnostics", report))
