...
import ai.steelengine.app.mainActivityPendingIntent
...
private const val NOTIFICATION_CHANNEL_BASE_ID = "steelengine.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" ->
          Triple("passive", NotificationManager.IMPORTANCE_LOW, nativeString("SteelEngine Passive"))
        "timesensitive" ->
          Triple(
            "timesensitive",
            NotificationManager.IMPORTANCE_HIGH,
            nativeString("SteelEngine Time Sensitive"),
          )
        else ->
          Triple("active", NotificationManager.IMPORTANCE_DEFAULT, nativeString("SteelEngine Active"))
      }
