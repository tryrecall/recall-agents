...
class InvokeDispatcherTest {
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(SteelEngineSmsCommand.Search.rawValue, "not-json")
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(SteelEngineSmsCommand.Send.rawValue, """{"to":"+15551234567","message":"hi"}""")
  ...
  fun handleInvoke_blocksCameraCommandsWhenCameraDisabled() =
    runTest {
      val result = newDispatcher(cameraEnabled = false).handleInvoke(SteelEngineCameraCommand.List.rawValue, null)
  ...
  fun handleInvoke_blocksLocationCommandWhenLocationDisabled() =
    runTest {
      val result = newDispatcher(locationEnabled = false).handleInvoke(SteelEngineLocationCommand.Get.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(installedAppsSharingEnabled = false)
          .handleInvoke(SteelEngineDeviceCommand.Apps.rawValue, """{"limit":1}""")
    ...
    runTest {
      val result =
        newDispatcher(motionActivityAvailable = false)
          .handleInvoke(SteelEngineMotionCommand.Activity.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(motionPedometerAvailable = false)
          .handleInvoke(SteelEngineMotionCommand.Pedometer.rawValue, null)
  ...
  fun handleInvoke_blocksCallLogWhenUnavailable() =
    runTest {
      val result =
        newDispatcher(callLogAvailable = false).handleInvoke(SteelEngineCallLogCommand.Search.rawValue, null)
  ...
  fun handleInvoke_blocksPhotosWhenUnavailable() =
    runTest {
      val result = newDispatcher(photosAvailable = false).handleInvoke(SteelEnginePhotosCommand.Latest.rawValue, null)
  ...
  fun handleInvoke_routesTalkPttCommands() =
    runTest {
      ...
      val start = dispatcher.handleInvoke(SteelEngineTalkCommand.PttStart.rawValue, null)
      val stop = dispatcher.handleInvoke(SteelEngineTalkCommand.PttStop.rawValue, null)
      val cancel = dispatcher.handleInvoke(SteelEngineTalkCommand.PttCancel.rawValue, null)
      val once = dispatcher.handleInvoke(SteelEngineTalkCommand.PttOnce.rawValue, null)
  ...
  fun handleInvoke_presentAndHideDriveTheShellOwnedCanvasState() =
    runTest {
      ...
      val hide = dispatcher.handleInvoke(SteelEngineCanvasCommand.Hide.rawValue, null)
    ...
    runTest {
      ...
      val result =
        newDispatcher(isForeground = false, canvas = canvas)
          .handleInvoke(SteelEngineCanvasCommand.Present.rawValue, """{"url":"https://example.com"}""")
