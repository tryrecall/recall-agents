# fastlane setup (SteelEngine Android)
...
Create a Google Play service account JSON key with Google Play Developer API access, then grant that service account access to the SteelEngine app in Play Console.
...
```bash
GOOGLE_PLAY_PACKAGE_NAME=ai.steelengine.app
```
...
STEELENGINE_ANDROID_UPLOAD_KEYSTORE=<path-to-upload-keystore.jks> \
STEELENGINE_ANDROID_SIGNING_PROPERTIES=<path-to-android-signing.properties> \
...
```properties
STEELENGINE_ANDROID_STORE_PASSWORD=<store-password>
STEELENGINE_ANDROID_KEY_ALIAS=<upload-key-alias>
STEELENGINE_ANDROID_KEY_PASSWORD=<key-password>
```
...
The script creates and boots a retained `SteelEngine_Screenshots_API36` AVD from
...
- Stable GitHub Release APK publication is separate from Google Play: `SteelEngine Release Publish` dispatches `.github/workflows/android-release.yml`, whose protected `android-release` environment provides `MATCH_PASSWORD`; the repository GitHub App reads the encrypted signing repo.
