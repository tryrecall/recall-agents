## SteelEngine Android App

SteelEngine Android is the officially released Google Play app. It connects to an SteelEngine Gateway as a companion node for chat, voice, approvals, screen, and device-aware automation.
...
tasks still require explicit `steelengineBuildCommit` and
`steelengineBuildTimestamp` properties so signed artifacts remain reproducible.
...
The screenshot script defaults to a retained `SteelEngine_Screenshots_API36` AVD
...
- Play build: `steelengine-<version>-play-release.aab`
- Wear build: `steelengine-<version>-wear-release.aab`
- Third-party build: `steelengine-<version>-third-party-release.apk`

`pnpm android:bundle:release` is an alias for the same Fastlane archive lane.

Regular final and correction SteelEngine releases publish the signed third-party APK as `SteelEngine-Android.apk` with a checksum manifest and GitHub Actions provenance. `.github/workflows/android-release.yml` is the only automated GitHub Release upload path; `SteelEngine Release Publish` dispatches it while the canonical release is still a draft and blocks publication until the uploaded asset contract verifies.

The protected `android-release` environment supplies `MATCH_PASSWORD`; the repository's read-only GitHub App token checks out encrypted material from `steelengine/apps-signing`. The workflow builds the exact release tag, refuses to replace different existing bytes, and re-downloads the APK for checksum, certificate, and provenance verification.
...
./gradlew -PsteelengineBuildCommit="$commit" -PsteelengineBuildTimestamp="$built_at" :app:bundlePlayRelease
./gradlew -PsteelengineBuildCommit="$commit" -PsteelengineBuildTimestamp="$built_at" :wear:bundleRelease
./gradlew -PsteelengineBuildCommit="$commit" -PsteelengineBuildTimestamp="$built_at" :app:bundleThirdPartyRelease
```
...
```bash
pnpm steelengine gateway --port 18789 --verbose
```
...
- Canvas web content already supports live reload when loaded from Gateway `__steelengine__/canvas/` (see `docs/platforms/android.md`).
...
Current SteelEngine Android implication:
...
2) Android app is connected to that gateway and `steelengine nodes status` shows it as paired + connected.
3) App stays unlocked and in foreground for the whole run.
4) Open the app **Screen** tab and keep it active during the run (canvas/A2UI commands require the canvas WebView attached there).
5) Grant runtime permissions for capabilities you expect to pass (camera/mic/location/notification listener/location, etc.).
6) No interactive system dialogs should be pending before test start.
7) Canvas host is enabled and reachable from the device for remote Canvas checks (do not run gateway with `STEELENGINE_SKIP_CANVAS_HOST=1`; startup logs should include `canvas host mounted at .../__steelengine__/`).
...
```bash
steelengine devices list
steelengine devices approve --latest   # preview only; copy the requestId from output
steelengine devices approve <requestId>
```
...
- `STEELENGINE_ANDROID_GATEWAY_URL=ws://...` (default: from your local SteelEngine config)
- `STEELENGINE_ANDROID_GATEWAY_TOKEN=...`
- `STEELENGINE_ANDROID_GATEWAY_PASSWORD=...`
...
- `pairing required` before tests start:
  - list pending requests (`steelengine devices list`), then approve with the exact ID (`steelengine devices approve <requestId>`) and rerun.
