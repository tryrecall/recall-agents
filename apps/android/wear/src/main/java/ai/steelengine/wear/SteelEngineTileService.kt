...
class SteelEngineTileService : Material3TileService() {
  override suspend fun MaterialScope.tileResponse(requestParams: RequestBuilders.TileRequest): TileBuilders.Tile {
    val openAction = ActionBuilders.launchAction(ComponentName(this@SteelEngineTileService, MainActivity::class.java))
    val openClickable = clickable(action = openAction, id = "open_steelengine")
    val layout =
      primaryLayout(
