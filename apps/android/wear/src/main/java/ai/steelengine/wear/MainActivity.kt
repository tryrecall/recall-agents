...
@Composable
internal fun SteelEngineWearApp(
  ...
  SteelEngineWearTheme(themeMode = themeMode) {
    AppScaffold {
      SteelEngineWearScreens(
        snapshot = snapshot,
        failure = failure,
        ...
        onTalk = {
          if (!state.connected || snapshot?.activeSessionId == null) return@SteelEngineWearScreens
...
internal const val REPLY_RESULT_KEY = "steelengine_watch_message"
