...
const GATEWAY_SERVICE_TYPE: &str = "_steelengine-gw._tcp.local.";
...
impl GatewayDiscovery {
    fn start(&self) -> Result<(), String> {
        ...
        let thread = thread::Builder::new()
            .name("steelengine-gateway-discovery".to_string())
            .spawn(move || {
...
fn prettify_instance_name(name: &str) -> String {
    ...
    strip_ascii_suffix(without_conflict, " (SteelEngine)")
        .trim()
        .to_string()
}
...
mod tests {
    ...
    fn service() -> ResolvedService {
        let properties = [
            ("transport", "gateway"),
            ("displayName", "Studio (SteelEngine)"),
            ("gatewayTls", "yes"),
    ...
    fn decodes_and_prettifies_fallback_name() {
        assert_eq!(
            service_instance_name("Peter\\032Studio\\032(SteelEngine)._steelengine-gw._tcp.local."),
            "Peter Studio"
        );
        assert_eq!(prettify_instance_name("Studio (SteelEngine) (2)"), "Studio");
    }
}
