...
#[derive(Clone, Debug)]
pub struct SteelEngineCli {
    executable: PathBuf,
    steelengine_home: PathBuf,
}
...
impl fmt::Display for CliError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Missing => write!(formatter, "SteelEngine CLI not found"),
...
impl SteelEngineCli {
    pub fn discover() -> Result<Self, CliError> {
        let home = steelengine_home()?;
        if let Some(override_path) = env::var_os("STEELENGINE_DESKTOP_CLI") {
            let cli = Self::new(PathBuf::from(override_path), home);
            cli.verify()?;
            return Ok(cli);
        }

        let managed = home.join("bin/steelengine");
        if managed.is_file() {
        ...
        let cli = Self::new(PathBuf::from("steelengine"), home);
        match cli.verify() {
            Ok(()) => Ok(cli),
    ...
    fn new(executable: PathBuf, steelengine_home: PathBuf) -> Self {
        Self {
            executable,
            steelengine_home,
        }
        ...
        S: AsRef<std::ffi::OsStr>,
    {
        self.command(args)?
            .output()
            .map_err(|error| CliError::Spawn(format!("Failed to run SteelEngine CLI: {error}")))
    }
        ...
        S: AsRef<std::ffi::OsStr>,
    {
        ...
        let value = serde_json::from_slice(&output.stdout).map_err(|error| {
            CliError::InvalidJson(format!("SteelEngine CLI returned invalid JSON: {error}"))
        })?;
        Ok((value, output))
    }
    ...
    fn command_path(&self) -> Result<OsString, CliError> {
        let mut paths = vec![
            self.steelengine_home.join("bin"),
            self.steelengine_home.join("tools/node/bin"),
        ];
...
pub fn steelengine_home() -> Result<PathBuf, CliError> {
    ...
    Ok(PathBuf::from(home).join(".steelengine"))
}
