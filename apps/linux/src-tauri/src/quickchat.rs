...
fn resolve_selected_agent(
    ...
    agents
        ...
        .cloned()
        .ok_or_else(|| "SteelEngine did not report a default agent.".to_string())
}
...
fn build_agents(catalog: &AgentsListResult) -> Result<Vec<QuickChatAgent>, String> {
    ...
    if agents.iter().any(|agent| agent.is_default) {
        ...
        Err("SteelEngine did not report a default agent.".to_string())
    }
}
...
mod tests {
    ...
    fn test_directory(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "steelengine-quickchat-{label}-{}-{unique}",
            std::process::id()
        ))
    }
