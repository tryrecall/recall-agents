...
# Build and bundle Recall into a minimal .app we can open.
# Outputs to dist/Recall.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Recall.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Recall"
BUNDLE_ID="${BUNDLE_ID:-ai.tryrecall.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/recall/recall/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Recall/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :RecallBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :RecallGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Recall"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Recall"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Recall"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Recall" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Recall/Resources/Recall.icns" "$APP_ROOT/Contents/Resources/Recall.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Recall/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying RecallKit resources"
RECALLKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/RecallKit_RecallKit.bundle"
if [ -d "$RECALLKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/RecallKit_RecallKit.bundle"
  cp -R "$RECALLKIT_BUNDLE" "$APP_ROOT/Contents/Resources/RecallKit_RecallKit.bundle"
else
  echo "WARN: RecallKit resource bundle not found at $RECALLKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Recall"
killall -q Recall 2>/dev/null || true
