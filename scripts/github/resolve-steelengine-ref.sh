...
REMOTE_URL="${STEELENGINE_REF_REMOTE:-https://github.com/steelengineai/recall-agents.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-steelengine-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves SteelEngine branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve SteelEngine ref: ${REF}" >&2
exit 1
