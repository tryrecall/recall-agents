...
export const securitySensitiveGuardMarker = "<!-- steelengine:security-sensitive-guard -->";
...
export function renderSecuritySensitiveAwarenessComment(changes) {
  return [
    ...
    "- If this PR intentionally needs the change, a repository admin or member of `@steelengine/steelengine-secops` must approve the exact head SHA.",
  ].join("\n");
}
...
export function renderAuthorizedSecuritySensitiveComment(override) {
  const lines = [
    ...
    "",
    "This PR includes security-sensitive file changes. A repository admin or member of `@steelengine/steelengine-secops` authorized this exact head SHA with `/allow-security-sensitive-change`.",
    "",
...
export function renderTrustedSecuritySensitiveComment({ actor, headSha, changes }) {
  return [
    ...
    "",
    "This PR includes security-sensitive file changes. The guard is informational because the PR author is a repository admin or a member of `@steelengine/steelengine-secops`.",
    "",
...
export function renderBlockedSecuritySensitiveComment({ headSha, changes }) {
  return [
    ...
    "",
    "SteelEngine does not accept security-sensitive file changes through PRs unless a repository admin or security explicitly authorizes the current head SHA.",
    "",
    "Detected security-sensitive changes:",
    ...renderChangedFileLines(changes),
    "",
    "If this PR intentionally needs these changes, ask a repository admin or member of `@steelengine/steelengine-secops` to comment:",
    "",
    "```text",
...
export function githubApi(token, options = {}) {
  return createGitHubApi(token, {
    ...options,
    userAgent: "steelengine-security-sensitive-guard",
  });
}
...
async function main() {
  ...
  const trustedCommentAuthors = securitySensitiveGuardCommentAuthors(
    process.env.STEELENGINE_SECURITY_SENSITIVE_GUARD_COMMENT_BOTS,
  );
  ...
  const mode = process.env.STEELENGINE_SECURITY_SENSITIVE_GUARD_MODE ?? "enforce";
