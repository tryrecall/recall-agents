...
# - dist/SteelEngine-<version>.zip
# - dist/SteelEngine-<version>.dmg
...
export BUNDLE_ID="${BUNDLE_ID:-ai.steelengine.mac}"
...
canonical_sparkle_build() {
  ...
  stderr_file="$(mktemp "${TMPDIR:-/tmp}/steelengine-sparkle-build.XXXXXX")" || {
...
APP="$ROOT_DIR/dist/SteelEngine.app"
if [[ ! -d "$APP" ]]; then
...
ZIP="$ROOT_DIR/dist/SteelEngine-$VERSION.zip"
DMG="$ROOT_DIR/dist/SteelEngine-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/SteelEngine-$VERSION.notary.zip"
DSYM_ZIP="$ROOT_DIR/dist/SteelEngine-$VERSION.dSYM.zip"
