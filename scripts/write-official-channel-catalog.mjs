...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.recall) ? packageJson.recall : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.recall?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.recall?.channel?.id) || left.name;
    const rightId = trimString(right.recall?.channel?.id) || right.name;
