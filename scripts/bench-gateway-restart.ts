// Bench Gateway Restart script supports SteelEngine repository automation.
...
const GATEWAY_CASES: readonly GatewayBenchCase[] = [
  ...
  },
  {
    ...
    env: { STEELENGINE_ACPX_RUNTIME_STARTUP_PROBE: "1", STEELENGINE_SKIP_CHANNELS: "1" },
    config: BASE_CONFIG,
  },
  {
    id: "skipChannelsNoAcpxProbe",
    name: "gateway restart, skip channels, ACPX startup probe off",
    env: { STEELENGINE_ACPX_RUNTIME_STARTUP_PROBE: "0", STEELENGINE_SKIP_CHANNELS: "1" },
    config: BASE_CONFIG,
  },
  {
    id: "default",
...
function printUsage(): void {
  console.log(`SteelEngine Gateway restart benchmark
...
function sanitizedEnv(
  ...
  return {
    ...
    LOGNAME: process.env.LOGNAME ?? "steelengine-bench",
    NO_COLOR: "1",
    PATH: process.env.PATH,
    SHELL: process.env.SHELL,
    TMPDIR: process.env.TMPDIR,
    USER: process.env.USER ?? "steelengine-bench",
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "steelengine-gateway-restart-bench-"));
