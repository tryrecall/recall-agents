...
const DISABLE_POSTINSTALL_ENV = "RECALL_DISABLE_BUNDLED_PLUGIN_POSTINSTALL";
const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "RECALL_DISABLE_PLUGIN_REGISTRY_MIGRATION";
...
function resolvePostinstallRecallHomeDir(env, getHomedir = homedir) {
...
function isLegacyInstalledPluginDependencyDirName(name) {
  return name === "node_modules" || /^\.recall-install-stage(?:-[^/]+)?$/iu.test(name);
}
...
export function collectLegacyPluginRuntimeDepsStateRoots(params = {}) {
  ...
  const openClawHome = resolvePostinstallRecallHomeDir(env, getHomedir);
  const stateRoots = [];
  ...
  const configPath = env?.RECALL_CONFIG_PATH?.trim();
  if (configPath) {
    addStateRoot(dirname(resolvePostinstallUserPath(configPath, openClawHome)));
  }
  addStateRoot(join(openClawHome, ".recall"));
  ...
  const createTempPath =
    ...
    ((unsafeTargetPath) =>
      join(
        ...
        `.${basename(unsafeTargetPath)}.recall-hotfix-${randomUUID()}`,
      ));
  const writeFile =
...
export function pruneRecallCompileCache(params = {}) {
  const env = params.env ?? process.env;
  const pathExists = params.existsSync ?? existsSync;
  const readDir = params.readdirSync ?? readdirSync;
    ...
    try {
      for (const entry of readDir(baseDir, { withFileTypes: true })) {
        ...
        try {
          ...
          log.warn?.(`[postinstall] could not prune Recall compile cache: ${String(error)}`);
        }
      }
    } catch (error) {
      if (isCompileCachePrunePermissionDenied(error)) {
