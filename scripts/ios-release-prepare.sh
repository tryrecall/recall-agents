...
- regenerates apps/ios/SteelEngine.xcodeproj via xcodegen
EOF
}
...
if [[ "${TEAM_ID}" != "${CANONICAL_TEAM_ID}" ]]; then
  echo "iOS App Store release must use canonical SteelEngine Team ID ${CANONICAL_TEAM_ID}; got ${TEAM_ID}." >&2
  exit 1
fi

if [[ -n "${STEELENGINE_PUSH_RELAY_BASE_URL:-}" || -n "${IOS_PUSH_RELAY_BASE_URL:-}" ]]; then
...
RELEASE_GIT_COMMIT="$(STEELENGINE_REQUIRE_BUILD_METADATA=1 steelengine_resolve_git_commit "${ROOT_DIR}")"
...
STEELENGINE_APP_BUNDLE_ID = ai.steelenginefoundation.app
STEELENGINE_SHARE_BUNDLE_ID = ai.steelenginefoundation.app.share
STEELENGINE_ACTIVITY_WIDGET_BUNDLE_ID = ai.steelenginefoundation.app.activitywidget
STEELENGINE_WATCH_APP_BUNDLE_ID = ai.steelenginefoundation.app.watchkitapp
STEELENGINE_CODE_SIGN_ENTITLEMENTS = Sources/SteelEngineAppAttest.entitlements
STEELENGINE_APNS_ENTITLEMENT_ENVIRONMENT = production
STEELENGINE_APP_ATTEST_ENVIRONMENT = production
STEELENGINE_PUSH_MODE = appStore
STEELENGINE_PUSH_RELAY_BASE_URL =
EOF
