...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  RECALL_SKIP_ACPX_RUNTIME: "1",
  RECALL_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$RECALL_WATCH_PID_FILE"',
    'mkdir -p "$RECALL_HOME/.recall"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$RECALL_HOME/.tryrecall/recall-agents.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    RECALL_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    RECALL_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-gateway-watch-"));
