...
function formatCapturedWatchLog(text, truncated) {
  return truncated
    ? `[steelengine] log truncated to last ${WATCH_LOG_CAPTURE_MAX_CHARS} chars\n${text}`
    : text;
}
...
export function buildTimedWatchCommand(
  ...
  const isolatedStateDir = path.join(isolatedHomeDir, ".steelengine");
  const isolatedConfigPath = path.join(isolatedStateDir, "steelengine.json");
  ...
  const shellSource = [
    'echo "$$" > "$STEELENGINE_WATCH_PID_FILE"',
    'mkdir -p "$STEELENGINE_STATE_DIR"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$STEELENGINE_CONFIG_PATH"`,
  ...
  const env = {
    STEELENGINE_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    STEELENGINE_HOME: isolatedHomeDir,
    STEELENGINE_CONFIG_PATH: isolatedConfigPath,
    STEELENGINE_STATE_DIR: isolatedStateDir,
...
export async function runTimedWatch(options, outputDir, deps = {}) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-gateway-watch-"));
