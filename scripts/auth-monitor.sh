...
#
# Suggested cron: */30 * * * * /home/admin/recall/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Recall notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., recall-alerts)
...
STATE_FILE="$HOME/.recall/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Recall if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use recall
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Recall to $NOTIFY_PHONE..."
            recall send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Recall is down. Run: ssh l36 '~/recall/scripts/mobile-reauth.sh'" "urgent"
    exit 1
