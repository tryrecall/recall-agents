// Protocol Gen Kotlin script supports SteelEngine repository automation.
...
const gatewayOutputPath = path.join(
  repoRoot,
  "apps/android/app/src/main/java/ai/steelengine/app/gateway/GatewayProtocol.kt",
);
const constantsOutputPath = path.join(
  repoRoot,
  "apps/android/app/src/main/java/ai/steelengine/app/protocol/SteelEngineProtocolConstants.kt",
);
...
const androidEnums: EnumSpec[] = [
  enumSpec("SteelEngineCapability", "", [
    ["Canvas", "canvas"],
    ["Camera", "camera"],
    ["Sms", "sms"],
    ["Talk", "talk"],
  ...
  enumSpec("SteelEngineCanvasCommand", "canvas.", [
    ["Present", "present"],
    ["Hide", "hide"],
    ["Navigate", "navigate"],
    ["Eval", "eval"],
  ...
  enumSpec("SteelEngineCanvasA2UICommand", "canvas.a2ui.", [
    ["Push", "push"],
    ["PushJSONL", "pushJSONL"],
    ["Reset", "reset"],
  ]),
  enumSpec("SteelEngineCameraCommand", "camera.", [
    ["List", "list"],
    ["Snap", "snap"],
    ["Clip", "clip"],
  ]),
  enumSpec("SteelEngineSmsCommand", "sms.", [
    ["Send", "send"],
    ["Search", "search"],
  ]),
  enumSpec("SteelEngineTalkCommand", "talk.", [
    ["PttStart", "ptt.start"],
    ["PttStop", "ptt.stop"],
    ["PttCancel", "ptt.cancel"],
    ["PttOnce", "ptt.once"],
  ]),
  enumSpec("SteelEngineLocationCommand", "location.", [["Get", "get"]]),
  enumSpec("SteelEngineDeviceCommand", "device.", [
    ["Status", "status"],
    ["Info", "info"],
  ...
  enumSpec("SteelEngineNotificationsCommand", "notifications.", [
    ["List", "list"],
    ["Actions", "actions"],
  ]),
  enumSpec("SteelEngineSystemCommand", "system.", [["Notify", "notify"]]),
  enumSpec("SteelEnginePhotosCommand", "photos.", [["Latest", "latest"]]),
  enumSpec("SteelEngineContactsCommand", "contacts.", [
    ["Search", "search"],
    ["Add", "add"],
  ]),
  enumSpec("SteelEngineCalendarCommand", "calendar.", [
    ["Events", "events"],
    ["Add", "add"],
  ]),
  enumSpec("SteelEngineMotionCommand", "motion.", [
    ["Activity", "activity"],
    ["Pedometer", "pedometer"],
  ]),
  enumSpec("SteelEngineCallLogCommand", "callLog.", [["Search", "search"]]),
];
...
async function generate(): Promise<void> {
  ...
  const gatewayContent = [
    ...
    "package ai.steelengine.app.gateway",
    "",
