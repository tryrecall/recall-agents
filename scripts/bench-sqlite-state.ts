// SQLite state benchmark seeds SteelEngine DBs and reports hot-query proof lines.
...
function printUsage(): void {
  console.log(`SteelEngine SQLite state benchmark
...
function main(): void {
  ...
  const stateDir =
    options.stateDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sqlite-perf-"));
  ...
  try {
    ...
    const agentDatabases = Array.from({ length: config.agentCount }, (_, index) =>
      openSteelEngineAgentDatabase({ agentId: `perf-agent-${index}`, env }),
    );
