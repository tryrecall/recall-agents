...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "steelengine.plugin.json");
    ...
    if (packageJson.steelengine && "extensions" in packageJson.steelengine) {
      packageJson.steelengine = {
        ...packageJson.steelengine,
        extensions: rewritePackageExtensions(packageJson.steelengine.extensions),
        ...(typeof packageJson.steelengine.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.steelengine.setupEntry) }
          : {}),
      };
    }
