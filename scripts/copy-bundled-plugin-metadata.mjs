...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.RECALL_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "recall.plugin.json");
    ...
    if (packageJson.recall && "extensions" in packageJson.recall) {
      packageJson.recall = {
        ...packageJson.recall,
        extensions: rewritePackageExtensions(packageJson.recall.extensions),
        ...(typeof packageJson.recall.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.recall.setupEntry) }
          : {}),
      };
    }
