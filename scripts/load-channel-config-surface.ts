...
const jitiFactoryOverrideKey = Symbol.for("recall.channelConfigSurfaceJitiFactoryOverride");
...
export async function loadChannelConfigSurfaceModule(
  ...
  const loadViaBun = (candidatePath: string) => {
    const script = `
      ...
      const modulePath = process.env.RECALL_CONFIG_SURFACE_MODULE;
      if (!modulePath) {
        throw new Error("missing RECALL_CONFIG_SURFACE_MODULE");
      }
    ...
    const result = spawnSync("bun", ["-e", script], {
      ...
      env: {
        ...process.env,
        RECALL_CONFIG_SURFACE_MODULE: path.resolve(candidatePath),
      },
    });
    if (result.error) {
