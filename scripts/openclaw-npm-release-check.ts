...
const EXPECTED_REPOSITORY_URL = "https://github.com/tryrecall/recall-agents";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@recall/fs-safe";
...
const skipPackValidationEnv = "RECALL_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "recall") {
    errors.push(`package.json name must be "recall"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.recall !== "recall.mjs") {
    errors.push(
      `package.json bin.recall must be "recall.mjs"; found "${pkg.bin?.recall ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`recall-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `recall-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
