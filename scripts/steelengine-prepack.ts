...
// Openclaw Prepack script supports SteelEngine repository automation.
...
const PREPARED_RELEASE_ENV = "STEELENGINE_PREPACK_PREPARED";
const OCM_INTERNAL_NPM_BIN_ENV = "OCM_INTERNAL_NPM_BIN";
const OCM_WORKSPACE_DIRS_ENV = "STEELENGINE_OCM_WORKSPACE_DEPENDENCY_DIRS";
...
const SELF_CONTAINED_SOURCE_PACK_COMMAND =
  "node scripts/package-steelengine-for-docker.mjs --allow-unreleased-changelog";
...
export function collectSourcePackWorkspaceDependencyErrors(
  ...
  const aiDependency = packageJson.dependencies?.["@steelengine/ai"];
  if (typeof aiDependency !== "string" || !aiDependency.trim().startsWith("workspace:")) {
    return [];
  }
  if (ocmExternalizesWorkspacePackage("@steelengine/ai", env)) {
    return [];
  }
  return [
    `plain root packing cannot safely resolve @steelengine/ai from ${aiDependency}: pnpm rewrites the workspace dependency to an exact version without bundling the package`,
    `use \`${SELF_CONTAINED_SOURCE_PACK_COMMAND}\` for a self-contained source package; official npm release automation prepares and publishes @steelengine/ai separately`,
  ];
}
...
export function resolvePrepackCommandTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  return positiveEnvInt(
    "STEELENGINE_PREPACK_COMMAND_TIMEOUT_MS",
    env,
