...
const AI_PACKAGE_NAME = "@steelengine/ai";
const AI_LOCK_PATH = "node_modules/@steelengine/ai";
...
export function prepareSteelEngineNpmShrinkwrap(params: {
  aiIntegrity: string;
  aiManifest: PackageManifest;
  ...
  if (aiVersion !== rootVersion) {
    throw new Error(`AI package version ${aiVersion} does not match SteelEngine ${rootVersion}`);
  }
  ...
  if (rootLockVersion !== rootVersion) {
    throw new Error(
      `npm shrinkwrap root version ${rootLockVersion} does not match SteelEngine ${rootVersion}`,
    );
  }
...
export function assertPreparedSteelEngineNpmShrinkwrap(params: {
  aiIntegrity: string;
  ...
  if (rootManifest.dependencies?.[AI_PACKAGE_NAME] !== aiVersion) {
    throw new Error(
      `packed SteelEngine manifest must depend on exact ${AI_PACKAGE_NAME}@${aiVersion}`,
    );
  }
  const expected = prepareSteelEngineNpmShrinkwrap({
    aiIntegrity: params.aiIntegrity,
    aiManifest,
    rootManifest,
  ...
  if (
    ...
    throw new Error(
      `prepared SteelEngine npm shrinkwrap does not lock the exact ${AI_PACKAGE_NAME} tarball`,
    );
  }
}
...
function main(argv = process.argv.slice(2)): void {
  ...
  if (!aiTarballPath || argv.length > 3) {
    throw new Error(
      "Usage: node --import tsx scripts/prepare-steelengine-npm-shrinkwrap.ts <steelengine-ai.tgz> [npm-shrinkwrap.json] [package.json]",
    );
  }
