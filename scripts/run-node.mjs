...
const listRequiredBundledPluginMetadataOutputs = (pluginEntries, deps) =>
  pluginEntries.flatMap(({ id, hasManifest, hasPackageJson }) => {
    ...
    if (hasManifest) {
      requiredPaths.push(path.join(builtPluginDir, "recall.plugin.json"));
    }
...
const listRequiredRecallExtensionAliasOutputs = (deps) => {
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
  ...
  const aliasDir = path.join(distRoot, "extensions", "node_modules", "recall");
  return [
...
export const listRequiredRuntimePostBuildOutputs = (deps) => {
  ...
  return [
    ...
    ...listRequiredRecallExtensionAliasOutputs(deps),
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.RECALL_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.RECALL_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (deps) => {
  if (deps.env.RECALL_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by RECALL_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by RECALL_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "RECALL_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_CPU_PROF_DIR_ENV = "RECALL_RUN_NODE_CPU_PROF_DIR";
const RUN_NODE_CPU_PROF_MAX_FILES_ENV = "RECALL_RUN_NODE_CPU_PROF_MAX_FILES";
const RUN_NODE_FILTER_SYNC_IO_STDERR_ENV = "RECALL_RUN_NODE_FILTER_SYNC_IO_STDERR";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "RECALL_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "RECALL_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "RECALL_RUN_NODE_BUILD_LOCK_STALE_MS";
const RUN_NODE_SKIP_DTS_BUILD_ENV = "RECALL_RUN_NODE_SKIP_DTS_BUILD";
...
const logRunner = (message, deps) => {
  if (deps.env.RECALL_RUNNER_LOG === "0") {
    return;
  }
  const line = `[recall] ${message}\n`;
...
const shouldUseRunNodeProgress = (deps) =>
  ...
  deps.env.RECALL_RUNNER_PROGRESS !== "0" &&
  deps.env.CI !== "true" &&
  !deps.outputTee;
...
const createRunNodeProgress = (label, deps) => {
  ...
  const render = () => {
    ...
    frameIndex += 1;
    deps.stderr.write(`\r[recall] ${frame} ${label} (${elapsedSeconds}s)`);
    visible = true;
  };
...
const listRunNodeCpuProfiles = (deps, absoluteProfileDir, commandName) => {
  ...
  const prefix = `recall-${commandName}-`;
  return entries
    .filter(
      (entry) =>
...
const resolveRunNodeCpuProfileArgs = (deps) => {
  ...
  const profileName = `recall-${commandName}-${pid}-${timestamp}.cpuprofile`;
...
const resolveRunNodeDiagnosticArgs = (deps) => {
  ...
  if (deps.env.RECALL_TRACE_SYNC_IO === "1") {
...
const runRecall = async (deps) => {
  const diagnosticArgs = resolveRunNodeDiagnosticArgs(deps);
  const nodeProcess = deps.spawn(deps.execPath, [...diagnosticArgs, "recall.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[recall] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.RECALL_BUILD_PRIVATE_QA = "1";
    deps.env.RECALL_ENABLE_PRIVATE_QA_CLI = "1";
    deps.env.RECALL_DISABLE_BUNDLED_PLUGINS ??= "0";
  }
