...
const DEFAULT_OUTPUT_NAME = "steelengine-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "STEELENGINE_TRUSTED_PACKAGE_TOKEN";
...
export const STEELENGINE_PACKAGE_SPEC_RE =
  /^steelengine@(alpha|beta|extended-stable|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-steelengine-package-candidate.mjs --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
function resolvePackedSteelEngineTarballFilename(value) {
  ...
  if (
    ...
    throw new Error(
      `npm pack reported unsafe SteelEngine tarball filename: ${JSON.stringify(filename)}`,
    );
  }
  return filename;
}

export function validateSteelEnginePackageSpec(spec) {
  if (!STEELENGINE_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be steelengine@alpha, steelengine@beta, steelengine@extended-stable, steelengine@latest, or an exact SteelEngine release version; got: ${spec}`,
    );
  }
}

export function resolveNpmPackageCandidatePackRunner(packageSpec, outputDir, params = {}) {
  validateSteelEnginePackageSpec(packageSpec);
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an SteelEngine branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `steelengine-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (Array.isArray(parsed)) {
    ...
    if (packedFilename) {
      filename = resolvePackedSteelEngineTarballFilename(packedFilename);
    }
  }
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (
        ...
        (trimmed.startsWith("steelengine-") ||
          trimmed.includes(":") ||
          trimmed.includes("/") ||
          trimmed.includes("\\"))
      ) {
        filename = resolvePackedSteelEngineTarballFilename(trimmed);
      }
    }
  }
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => {
        try {
          return resolvePackedSteelEngineTarballFilename(entry) === entry;
        } catch {
          return false;
        }
      })
  ...
  if (!filename) {
    throw new Error(`npm pack produced no SteelEngine tarball in ${outputDir}`);
  }
...
export const cleanPackedSteelEngineTarballsForTest = cleanPackedSteelEngineTarballs;
...
async function resolveCandidate(options) {
  ...
  console.error(`Checking SteelEngine package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-steelengine-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
  ...
  if (pkg.name !== "steelengine") {
    throw new Error(`package candidate must be named "steelengine"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
