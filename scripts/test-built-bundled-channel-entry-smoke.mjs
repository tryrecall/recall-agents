...
process.env.RECALL_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

const { packageRoot } = parsePackageRootArg(
  process.argv.slice(2),
  "RECALL_BUNDLED_CHANNEL_SMOKE_ROOT",
);
...
const installedLayoutEnv = "RECALL_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";
...
function packageRootLooksInstalled(root) {
  return root.replaceAll("\\", "/").endsWith("/node_modules/recall");
}
...
function smokeInInstalledLayoutIfNeeded() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "recall-channel-entry-smoke-"));
  const nodeModulesRoot = path.join(tempRoot, "node_modules");
  const installedPackageRoot = path.join(nodeModulesRoot, "recall");
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    const extensionEntries =
      Array.isArray(packageJson.recall.extensions) && packageJson.recall.extensions.length > 0
    ...
    const setupEntry = packageJson.recall.setupEntry;
