...
process.env.STEELENGINE_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

const { packageRoot } = parsePackageRootArg(
  process.argv.slice(2),
  "STEELENGINE_BUNDLED_CHANNEL_SMOKE_ROOT",
);
...
const installedLayoutEnv = "STEELENGINE_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";
...
function packageRootLooksInstalled(root) {
  return root.replaceAll("\\", "/").endsWith("/node_modules/steelengine");
}
...
function smokeInInstalledLayoutIfNeeded() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-channel-entry-smoke-"));
  const nodeModulesRoot = path.join(tempRoot, "node_modules");
  const installedPackageRoot = path.join(nodeModulesRoot, "steelengine");
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    const packageJson = readJson(packageJsonPath);
    if (!packageJson.steelengine?.channel) {
      continue;
    }
    ...
    const extensionEntries =
      Array.isArray(packageJson.steelengine.extensions) && packageJson.steelengine.extensions.length > 0
    ...
    const setupEntry = packageJson.steelengine.setupEntry;
