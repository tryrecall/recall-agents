...
func (docFrontmatterTranslator) Translate(_ context.Context, text, _, _ string) (string, error) {
	replacer := strings.NewReplacer(
		"Step-by-step Fly.io deployment for SteelEngine with persistent storage and HTTPS", "在 Fly.io 上逐步部署 SteelEngine，包含持久化存储和 HTTPS",
		"Deploying SteelEngine on Fly.io", "在 Fly.io 上部署 SteelEngine",
...
func (docFrontmatterFallbackTranslator) Translate(_ context.Context, text, _, _ string) (string, error) {
	switch text {
	case "Step-by-step Fly.io deployment for SteelEngine with persistent storage and HTTPS":
		return strings.Join([]string{
			"<frontmatter>",
			"title: Fly.io",
			"summary: \"在 Fly.io 上部署 SteelEngine 的逐步指南，包含持久化存储和 HTTPS 设置\"",
			"read_when:",
			"  - 在 Fly.io 上部署 SteelEngine",
			"  - 设置 Fly 卷、机密和初始运行配置",
			"</frontmatter>",
			"",
			"<body>",
	...
	case "Deploying SteelEngine on Fly.io":
		return "在 Fly.io 上部署 SteelEngine", nil
...
func TestTranslateDocBodyChunkedFallsBackToSmallerChunks(t *testing.T) {
	...
	t.Setenv("STEELENGINE_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "4096")
...
func TestValidateDocChunkTranslationRejectsChangedCapitalizedPlaceholder(t *testing.T) {
	...
	source := "Run `steelengine pairing approve sms <CODE>`.\n"
	translated := "Ejecuta `steelengine pairing approve sms <TOKEN>`.\n"
...
func TestValidateDocChunkTranslationPreservesEmbeddedAnglePlaceholders(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "version token",
			source:     "```text\nSteelEngine v<version>\n```\n",
			translated: "```text\nSteelEngine v<versión>\n```\n",
		},
		{
...
func TestTranslateDocBodyChunkedPreservesMarkersAfterSplit(t *testing.T) {
	...
	t.Setenv("STEELENGINE_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "32")
...
func TestTranslateDocBodyChunkedPreservesListStructureAcrossSanitizedChunkBoundary(t *testing.T) {
	...
	t.Setenv("STEELENGINE_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "20")
...
func TestTranslateDocBodyChunkedPreSplitsOversizedPromptBudget(t *testing.T) {
	...
	t.Setenv("STEELENGINE_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "4096")
	t.Setenv("STEELENGINE_DOCS_I18N_DOC_CHUNK_PROMPT_BUDGET", "60")
...
func TestTranslateDocBodyChunkedSplitsOversizedSingletonBlock(t *testing.T) {
	...
	t.Setenv("STEELENGINE_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "24")
...
func TestTranslateDocBodyChunkedSplitsSingletonBlockWhenPromptBudgetExceeded(t *testing.T) {
	...
	t.Setenv("STEELENGINE_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "4096")
	t.Setenv("STEELENGINE_DOCS_I18N_DOC_CHUNK_PROMPT_BUDGET", strconv.Itoa(budget))
	translator := &oversizedBlockTranslator{}
	translated, err := translateDocBodyChunked(context.Background(), translator, "gateway/configuration-reference.md", body, "en", "zh-CN")
	if err != nil {
		t.Fatalf("translateDocBodyChunked returned error: %v", err)
...
func TestTranslateDocBodyChunkedMasksInlineCodeAndListMarkers(t *testing.T) {
	body := strings.Join([]string{
		"- Visible prose uses `steelengine config`.",
	...
	for _, input := range translator.rawInputs {
		if strings.Contains(input, "`steelengine config`") || strings.Contains(input, "``nested `ticks` ``") {
	...
	for _, exact := range []string{
		"- Видимый текст uses `steelengine config`.",
...
func TestTranslateDocBodyChunkedRetriesSingletonFenceAfterValidationFailure(t *testing.T) {
	...
	t.Setenv("STEELENGINE_DOCS_I18N_DOC_CHUNK_MAX_BYTES", "4096")
	t.Setenv("STEELENGINE_DOCS_I18N_DOC_CHUNK_PROMPT_BUDGET", "4096")
...
func TestProcessFileDocUsesFieldLevelFrontmatterTranslation(t *testing.T) {
	...
	text := string(output)
	if !strings.Contains(text, "在 Fly.io 上逐步部署 SteelEngine，包含持久化存储和 HTTPS") {
		t.Fatalf("expected translated summary in output:\n%s", text)
	}
	if !strings.Contains(text, "在 Fly.io 上部署 SteelEngine") {
...
func TestProcessFileDocRejectsSuspiciousFrontmatterScalarExpansion(t *testing.T) {
	...
	if !strings.Contains(text, "summary: Step-by-step Fly.io deployment for SteelEngine with persistent storage and HTTPS") {
...
func TestValidateDocChunkTranslationRejectsMovedProtectedProductLinkLabel(t *testing.T) {
	...
	source := "Deploy SteelEngine on [Render](https://render.com) using the Blueprint.\n"
	translated := "Render पर Blueprint का उपयोग करके [SteelEngine](https://render.com) परिनियोजित करें।\n"
...
func TestValidateDocBodyRejectsMovedProtectedReferenceLinkLabel(t *testing.T) {
	...
	translated := "Auf Render mit [SteelEngine][provider] bereitstellen.\n\n[provider]: https://render.com\n"
	err := validateDocBodyFencedLiterals(source, translated)
	if err == nil || !strings.Contains(err.Error(), "protected link label mismatch") {
