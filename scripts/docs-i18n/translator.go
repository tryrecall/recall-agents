...
const (
	...
	envDocsI18nPromptTimeout    = "STEELENGINE_DOCS_I18N_PROMPT_TIMEOUT"
	envDocsI18nCommandWaitDelay = "STEELENGINE_DOCS_I18N_COMMAND_WAIT_DELAY"
	envDocsI18nCodexExecutable  = "STEELENGINE_DOCS_I18N_CODEX_EXECUTABLE"
)
...
func runCodexExecPrompt(ctx context.Context, req codexPromptRequest) (string, error) {
	outputFile, err := os.CreateTemp("", "steelengine-docs-i18n-codex-*.txt")
	if err != nil {
		return "", err
	}
...
func isolatedCodexHomeBase() (string, error) {
	...
	base := filepath.Join(cacheDir, "steelengine-docs-i18n")
