...
var contextualProductDestinations = map[string]contextualProductDestinationRule{
	...
	"Lobster":     {routes: []string{"/tools/lobster", "/steelengine/lobster"}},
	"Mantis":      {routes: []string{"/concepts/mantis", "/steelengine/mantis"}},
	"Tokenjuice":  {routes: []string{"/tools/tokenjuice", "/steelengine/tokenjuice"}},
}
...
func docsI18nDocChunkMaxBytes() int {
	value := strings.TrimSpace(os.Getenv("STEELENGINE_DOCS_I18N_DOC_CHUNK_MAX_BYTES"))
	if value == "" {
...
func docsI18nDocChunkPromptBudget() int {
	value := strings.TrimSpace(os.Getenv("STEELENGINE_DOCS_I18N_DOC_CHUNK_PROMPT_BUDGET"))
	if value == "" {
