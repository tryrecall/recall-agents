#!/usr/bin/env node
// Test Env Mutation Report script supports SteelEngine repository automation.
...
const DEFAULT_ALLOWED_FILES = new Map([
  ["src/test-utils/steelengine-test-state.ts", "canonical SteelEngine test state helper"],
  ...
  ["test/setup-steelengine-runtime.ts", "global runtime-test setup owns process env isolation"],
  [
...
export function renderTestEnvMutationReport(
  ...
  const lines = [
    "SteelEngine test env mutation report",
...
function printHelp(): void {
  process.stdout.write(`SteelEngine test env mutation report
