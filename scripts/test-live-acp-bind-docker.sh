...
ACP_AGENT_LIST_RAW="${RECALL_LIVE_ACP_BIND_AGENTS:-${RECALL_LIVE_ACP_BIND_AGENT:-claude,codex,gemini}}"
TEMP_DIRS=()
...
openclaw_live_acp_bind_resolve_auth_provider() {
  case "${1:-}" in
    ...
    opencode) printf '%s\n' "opencode" ;;
    *)
      echo "Unsupported RECALL_LIVE_ACP_BIND agent: ${1:-} (expected claude, codex, droid, gemini, or opencode)" >&2
      return 1
      ;;
  esac
}
...
openclaw_live_acp_bind_resolve_agent_command() {
  case "${1:-}" in
    claude) printf '%s' "${RECALL_LIVE_ACP_BIND_AGENT_COMMAND_CLAUDE:-${RECALL_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    codex) printf '%s' "${RECALL_LIVE_ACP_BIND_AGENT_COMMAND_CODEX:-${RECALL_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    droid) printf '%s' "${RECALL_LIVE_ACP_BIND_AGENT_COMMAND_DROID:-${RECALL_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    gemini) printf '%s' "${RECALL_LIVE_ACP_BIND_AGENT_COMMAND_GEMINI:-${RECALL_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    opencode) printf '%s' "${RECALL_LIVE_ACP_BIND_AGENT_COMMAND_OPENCODE:-${RECALL_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    *) return 1 ;;
  esac
}
...
agent="${RECALL_LIVE_ACP_BIND_AGENT:-claude}"
case "$agent" in
  claude)
...
if [ -n "\${RECALL_LIVE_ACP_BIND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${RECALL_LIVE_ACP_BIND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${RECALL_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${RECALL_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD}"
fi
...
NODE
  ...
  opencode)
    ...
    export OPENCODE_CONFIG_CONTENT="$(
      node -e 'process.stdout.write(JSON.stringify({model: process.env.RECALL_LIVE_ACP_BIND_OPENCODE_MODEL || "opencode/kimi-k2.6"}))'
    )"
    ;;
  *)
    echo "Unsupported RECALL_LIVE_ACP_BIND_AGENT: $agent" >&2
    exit 1
    ;;
esac
tmp_dir="$(mktemp -d)"
...
cd "$tmp_dir"
export RECALL_LIVE_ACP_BIND_AGENT_COMMAND="${RECALL_LIVE_ACP_BIND_AGENT_COMMAND:-}"
...
if ((${#ACP_AGENTS[@]} == 0)); then
  echo "No ACP bind agents selected. Use RECALL_LIVE_ACP_BIND_AGENTS=claude,codex,droid,gemini,opencode." >&2
  exit 1
fi
...
for ACP_AGENT in "${ACP_AGENTS[@]}"; do
  ...
  DOCKER_RUN_ARGS=(docker run --rm -t \
    ...
    -e RECALL_LIVE_ACP_BIND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}" \
    -e RECALL_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}" \
    -e GEMINI_API_KEY \
    -e GOOGLE_API_KEY \
    -e FACTORY_API_KEY \
    -e OPENAI_API_KEY \
    ...
    -e RECALL_LIVE_ACP_BIND=1 \
    -e RECALL_LIVE_ACP_BIND_AGENT="$ACP_AGENT" \
    -e RECALL_LIVE_ACP_BIND_OPENCODE_MODEL="${RECALL_LIVE_ACP_BIND_OPENCODE_MODEL:-opencode/kimi-k2.6}" \
    -e RECALL_LIVE_ACP_BIND_AGENT_COMMAND="$AGENT_COMMAND")
