...
// Openclaw Npm Postpublish Verify script supports SteelEngine repository automation.
...
import { parseReleaseVersion, resolveNpmCommandInvocation } from "./steelengine-npm-release-check.ts";
...
type SteelEngineNpmPostpublishVerifyArgs =
  | {
      help: false;
      version: string;
    }
...
export function steelEngineNpmPostpublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/steelengine-npm-postpublish-verify.ts <version>";
}

export function parseSteelEngineNpmPostpublishVerifyArgs(
  argv: readonly string[],
): SteelEngineNpmPostpublishVerifyArgs {
  ...
  if (!version) {
    throw new Error(steelEngineNpmPostpublishVerifyUsage());
  }
  if (version.startsWith("-")) {
    throw new Error(`Unknown steelengine npm postpublish verifier option: ${version}`);
  }
  const extraArg = args[1]?.trim();
  if (extraArg) {
    throw new Error(`Unexpected steelengine npm postpublish verifier argument: ${extraArg}`);
  }
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  const exactSpec = `steelengine@${version}`;
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`steelengine@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
const NPM_PROVENANCE_REPOSITORY = "https://github.com/steelengineai/recall-agents";
const NPM_PROVENANCE_WORKFLOW_PATH = ".github/workflows/steelengine-npm-release.yml";
...
function resolveNpmProvenanceVerificationPolicy(
  ...
  if (
    ...
    throw new Error(
      `npm provenance attestation does not bind ${version} to the trusted SteelEngine GitHub release workflow.`,
    );
  }
...
function collectInstalledPluginSdkDeclarationErrors(packageRoot: string): string[] {
  ...
  const errors: string[] = [];
  const forbiddenPrivateWorkspaceSpecifiers = ["@steelengine/llm-core"];
...
export function resolveInstalledBinaryPath(prefixDir: string, platform = process.platform): string {
  return platform === "win32"
    ? pathWin32.join(prefixDir, "steelengine.cmd")
    : pathPosix.join(prefixDir, "bin", "steelengine");
}
...
async function verifyPublishedRegistryProvenanceOnce(version: string): Promise<void> {
  ...
  await verifyNpmProvenanceAttestation({
    ...
    integrity,
    attestations,
    expectedWorkflowRef: process.env.STEELENGINE_NPM_EXPECTED_WORKFLOW_REF,
    expectedWorkflowSha: process.env.STEELENGINE_NPM_EXPECTED_WORKFLOW_SHA,
  });
  console.log(
    `steelengine-npm-postpublish-verify: registry signature and provenance attestation verified (${version})`,
  );
}
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `steelengine-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    const packageRoot = join(globalRoot, "steelengine");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== scenario.expectedVersion) {
      errors.push(
        `installed steelengine binary version mismatch: expected ${scenario.expectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    ...
    console.log(`steelengine-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
async function main(argv = process.argv.slice(2)): Promise<void> {
  const args = parseSteelEngineNpmPostpublishVerifyArgs(argv);
  if (args.help) {
    console.log(steelEngineNpmPostpublishVerifyUsage());
    return;
  }
  ...
  console.log(
    `steelengine-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`steelengine-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
