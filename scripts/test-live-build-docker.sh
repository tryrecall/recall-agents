...
DOCKER_BUILD_EXTENSIONS="${RECALL_DOCKER_BUILD_EXTENSIONS:-${RECALL_EXTENSIONS:-}}"
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "RECALL_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi

if [[ "${RECALL_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  echo "==> Live-test image not available; building because RECALL_DOCKER_BUILD_ON_MISSING/RECALL_TESTBOX allows fallback."
fi
