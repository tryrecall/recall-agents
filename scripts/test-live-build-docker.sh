...
DOCKER_BUILD_EXTENSIONS="${STEELENGINE_DOCKER_BUILD_EXTENSIONS:-${STEELENGINE_EXTENSIONS:-}}"
LIVE_IMAGE_PULL_ATTEMPTS="${STEELENGINE_LIVE_DOCKER_PULL_ATTEMPTS:-3}"
LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS="${STEELENGINE_LIVE_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"

if ! [[ "$LIVE_IMAGE_PULL_ATTEMPTS" =~ ^[1-9][0-9]*$ ]]; then
  echo "STEELENGINE_LIVE_DOCKER_PULL_ATTEMPTS must be a positive integer, got: $LIVE_IMAGE_PULL_ATTEMPTS" >&2
  exit 2
fi
if ! [[ "$LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS" =~ ^[0-9]+$ ]]; then
  echo "STEELENGINE_LIVE_DOCKER_PULL_RETRY_DELAY_SECONDS must be a non-negative integer, got: $LIVE_IMAGE_PULL_RETRY_DELAY_SECONDS" >&2
  exit 2
fi
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "STEELENGINE_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi
...
if [[ "${STEELENGINE_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  echo "==> Live-test image not available; building because STEELENGINE_DOCKER_BUILD_ON_MISSING/STEELENGINE_TESTBOX allows fallback."
fi
