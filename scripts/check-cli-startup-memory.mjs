...
const tmpHome = mkdtempSync(path.join(os.tmpdir(), "recall-startup-memory-"));
...
const MAX_RSS_MARKER = "__RECALL_MAX_RSS_KB__=";
...
const cases = [
  {
    id: "help",
    label: "--help",
    args: ["recall.mjs", "--help"],
    limitMb: Number(process.env.RECALL_STARTUP_MEMORY_HELP_MB ?? DEFAULT_LIMITS_MB.help),
  },
  {
    id: "statusJson",
    label: "status --json",
    args: ["recall.mjs", "status", "--json"],
    limitMb: Number(
      process.env.RECALL_STARTUP_MEMORY_STATUS_JSON_MB ?? DEFAULT_LIMITS_MB.statusJson,
    ),
  },
  {
    id: "gatewayStatus",
    label: "gateway status",
    args: ["recall.mjs", "gateway", "status"],
    limitMb: Number(
      process.env.RECALL_STARTUP_MEMORY_GATEWAY_STATUS_MB ?? DEFAULT_LIMITS_MB.gatewayStatus,
    ),
  },
];
...
function formatFixGuidance(testCase, details) {
  ...
  const guidance = [
    ...
    "LLM prompt:",
    `"Recall startup-memory CI failed for '${testCase.label}'. Analyze this failure, identify the first runtime/import side effect that makes startup heavier or broken, and propose the smallest safe patch. Failure output:\n${details}"`,
  ];
