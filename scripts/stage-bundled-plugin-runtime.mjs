...
function ensureRecallExtensionAlias(params) {
  const pluginSdkDir = path.join(params.repoRoot, "dist", "plugin-sdk");
  if (!fs.existsSync(pluginSdkDir)) {
    return;
  }

  const aliasDir = path.join(params.distExtensionsRoot, "node_modules", "recall");
...
function shouldCopyRuntimeFile(relativePath) {
  return (
    ...
    isPathOrNestedPath(relativePath, "recall.plugin.json") ||
...
export function stageBundledPluginRuntime(params = {}) {
  ...
  ensureRecallExtensionAlias({ repoRoot, distExtensionsRoot });
