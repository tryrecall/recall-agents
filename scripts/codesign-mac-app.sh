...
APP_BUNDLE="dist/SteelEngine.app"
...
ENT_TMP_DIR=$(mktemp -d -t steelengine-entitlements.XXXXXX)
trap cleanup EXIT
...
MLX_TTS_HELPER="$APP_BUNDLE/Contents/MacOS/steelengine-mlx-tts"
if [ -f "$MLX_TTS_HELPER" ]; then
  echo "Signing MLX TTS helper"; sign_item "$MLX_TTS_HELPER" "$APP_ENTITLEMENTS"
fi

# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/SteelEngine" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/SteelEngine" "$APP_ENTITLEMENTS"
fi
