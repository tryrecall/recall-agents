...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.RECALL_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[recall] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[recall] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[recall] This usually means a file in node_modules is empty or truncated.");
  console.error("[recall] Recommended recovery:");
  console.error("[recall]   rm -rf node_modules");
  console.error("[recall]   pnpm store prune");
  console.error("[recall]   pnpm install");
  console.error("");
  console.error("[recall] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.RECALL_WATCH_MODE = "1";
  childEnv.RECALL_WATCH_SESSION = watchSession;
  ...
  childEnv.RECALL_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.RECALL_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `recall doctor --fix --non-interactive` once.",
        deps,
      );
