...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.STEELENGINE_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[steelengine] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[steelengine] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[steelengine] This usually means a file in node_modules is empty or truncated.");
  console.error("[steelengine] Recommended recovery:");
  console.error("[steelengine]   rm -rf node_modules");
  console.error("[steelengine]   pnpm store prune");
  console.error("[steelengine]   pnpm install");
  console.error("");
  console.error("[steelengine] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.STEELENGINE_WATCH_MODE = "1";
  childEnv.STEELENGINE_WATCH_SESSION = watchSession;
  ...
  childEnv.STEELENGINE_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.STEELENGINE_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `steelengine doctor --fix --non-interactive` once.",
        deps,
      );
