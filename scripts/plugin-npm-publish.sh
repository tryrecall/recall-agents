...
if [[ "${RECALL_NPM_PUBLISH_PROVENANCE:-1}" != "0" && "${RECALL_NPM_PUBLISH_PROVENANCE:-1}" != "false" ]]; then
...
if [[ "${RECALL_NPM_PUBLISH_AUTH_MODE:-}" == "trusted-publisher" ]]; then
  publish_auth_token=""
...
if [[ "${mode}" == "--pack-dry-run" ]]; then
  RECALL_PLUGIN_NPM_BUNDLE_DEPENDENCIES=1 \
