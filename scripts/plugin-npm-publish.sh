...
if [[ "${mode}" == "--pack" && -z "${STEELENGINE_PLUGIN_NPM_PACK_OUTPUT_DIR:-}" ]]; then
  echo "--pack requires STEELENGINE_PLUGIN_NPM_PACK_OUTPUT_DIR" >&2
  exit 2
fi
...
const plan = resolveNpmPublishPlan(
  ...
  process.env.STEELENGINE_PLUGIN_NPM_PUBLISH_TAG,
);
...
if [[ "${STEELENGINE_NPM_PUBLISH_PROVENANCE:-1}" != "0" && "${STEELENGINE_NPM_PUBLISH_PROVENANCE:-1}" != "false" ]]; then
...
if [[ "${STEELENGINE_NPM_PUBLISH_AUTH_MODE:-}" == "trusted-publisher" ]]; then
  publish_auth_token=""
...
if [[ "${mode}" == "--pack" || "${mode}" == "--pack-dry-run" ]]; then
  ...
  if [[ "${mode}" == "--pack-dry-run" ]]; then
    ...
    mkdir -p "${STEELENGINE_PLUGIN_NPM_PACK_OUTPUT_DIR}"
    pack_output_dir="$(cd "${STEELENGINE_PLUGIN_NPM_PACK_OUTPUT_DIR}" && pwd)"
    pack_args+=(--pack-destination "${pack_output_dir}")
  fi
  STEELENGINE_PLUGIN_NPM_BUNDLE_DEPENDENCIES=1 \
