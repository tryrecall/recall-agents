...
usage() {
  cat <<'EOF'
Usage:
  RECALL_PUSH_RELAY_BASE_URL=https://relay.example.com \
...
- regenerates apps/ios/Recall.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${RECALL_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid RECALL_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid RECALL_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid RECALL_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid RECALL_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing RECALL_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(RECALL_URL_SLASH)$(RECALL_URL_SLASH)#g'
)"
...
RECALL_CODE_SIGN_STYLE = Automatic
RECALL_DEVELOPMENT_TEAM = ${TEAM_ID}
RECALL_IOS_SELECTED_TEAM = ${TEAM_ID}
RECALL_APP_BUNDLE_ID = ai.tryrecall.client
RECALL_SHARE_BUNDLE_ID = ai.tryrecall.client.share
RECALL_ACTIVITY_WIDGET_BUNDLE_ID = ai.tryrecall.client.activitywidget
RECALL_WATCH_APP_BUNDLE_ID = ai.tryrecall.client.watchkitapp
RECALL_WATCH_EXTENSION_BUNDLE_ID = ai.tryrecall.client.watchkitapp.extension
RECALL_APP_PROFILE =
RECALL_SHARE_PROFILE =
RECALL_PUSH_TRANSPORT = relay
RECALL_PUSH_DISTRIBUTION = official
RECALL_URL_SLASH = /
RECALL_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
RECALL_PUSH_APNS_ENVIRONMENT = production
EOF
