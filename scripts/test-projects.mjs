...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.RECALL_VITEST_MAX_WORKERS || env.RECALL_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      RECALL_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(baseEnv.RECALL_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        !isCiLikeEnv(baseEnv) &&
        !baseEnv.RECALL_TEST_PROJECTS_PARALLEL &&
        !baseEnv.RECALL_VITEST_MAX_WORKERS &&
        !baseEnv.RECALL_TEST_WORKERS &&
