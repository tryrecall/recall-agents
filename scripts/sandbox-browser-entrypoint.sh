...
CDP_PORT="${RECALL_BROWSER_CDP_PORT:-9222}"
CDP_SOURCE_RANGE="${RECALL_BROWSER_CDP_SOURCE_RANGE:-}"
CDP_AUTH_TOKEN="${RECALL_BROWSER_CDP_AUTH_TOKEN:-}"
VNC_PORT="${RECALL_BROWSER_VNC_PORT:-5900}"
NOVNC_PORT="${RECALL_BROWSER_NOVNC_PORT:-6080}"
ENABLE_NOVNC="${RECALL_BROWSER_ENABLE_NOVNC:-1}"
HEADLESS="${RECALL_BROWSER_HEADLESS:-0}"
ALLOW_NO_SANDBOX="${RECALL_BROWSER_NO_SANDBOX:-0}"
NOVNC_PASSWORD="${RECALL_BROWSER_NOVNC_PASSWORD:-}"

DISABLE_GRAPHICS_FLAGS="${RECALL_BROWSER_DISABLE_GRAPHICS_FLAGS:-1}"
DISABLE_EXTENSIONS="${RECALL_BROWSER_DISABLE_EXTENSIONS:-1}"
RENDERER_PROCESS_LIMIT="${RECALL_BROWSER_RENDERER_PROCESS_LIMIT:-2}"
AUTO_START_TIMEOUT_MS="${RECALL_BROWSER_AUTO_START_TIMEOUT_MS:-12000}"
...
if [[ -z "${CDP_AUTH_TOKEN}" ]]; then
  ...
  RECALL_BROWSER_CHROME_CDP_PORT="${CHROME_CDP_PORT}" python3 - <<'PY' &
import base64
import hmac
import ipaddress
import os
...
LISTEN_PORT = int(os.environ["RECALL_BROWSER_CDP_PORT"])
UPSTREAM_PORT = int(os.environ["RECALL_BROWSER_CHROME_CDP_PORT"])
AUTH_TOKEN = os.environ["RECALL_BROWSER_CDP_AUTH_TOKEN"]
SOURCE_RANGE = os.environ.get("RECALL_BROWSER_CDP_SOURCE_RANGE", "").strip()
...
EXPECTED_BASIC = "Basic " + base64.b64encode(f"recall:{AUTH_TOKEN}".encode()).decode()
    ...
    def handle(self):
        ...
        if not has_auth(header_bytes):
            self.request.sendall(
                ...
                b'WWW-Authenticate: Basic realm="Recall CDP"\r\n'
                b"Connection: close\r\n"
                b"Content-Length: 0\r\n\r\n"
            )
            return
