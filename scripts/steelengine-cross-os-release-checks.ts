...
async function main(argv: string[]) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `steelengine@${previousVersion}` : "steelengine@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.STEELENGINE_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.STEELENGINE_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
