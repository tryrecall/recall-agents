...
- STEELENGINE_IOS_VERSION = exact canonical iOS version
- STEELENGINE_MARKETING_VERSION = short iOS/App Store version
- STEELENGINE_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    STEELENGINE_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    STEELENGINE_MARKETING_VERSION)
...
RESOLVED_GIT_COMMIT="$(steelengine_resolve_git_commit "${ROOT_DIR}")"
RESOLVED_BUILD_TIMESTAMP="$(steelengine_resolve_build_timestamp)"
...
STEELENGINE_IOS_VERSION = ${IOS_VERSION}
STEELENGINE_MARKETING_VERSION = ${MARKETING_VERSION}
STEELENGINE_BUILD_VERSION = ${BUILD_NUMBER}
STEELENGINE_GIT_COMMIT = ${RESOLVED_GIT_COMMIT}
STEELENGINE_BUILD_TIMESTAMP = ${RESOLVED_BUILD_TIMESTAMP}
EOF
