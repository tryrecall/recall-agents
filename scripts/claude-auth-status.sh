...
# Checks both Claude Code and Recall auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
RECALL_AUTH="$HOME/.recall/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    recall models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    recall_status=$(check_recall_auth 2>/dev/null || true)

    claude_expires=0
    recall_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        recall_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        recall_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$RECALL_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$recall_status" \
        --arg be "$recall_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            recall: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$recall_status" == EXPIRED* ]] || [[ "$recall_status" == MISSING* ]]; then
        echo "RECALL_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$recall_status" == EXPIRING* ]]; then
        echo "RECALL_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "Recall Auth (~/.recall/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$RECALL_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$RECALL_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'recall doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active recall >/dev/null 2>&1; then
    echo -e "Recall service: ${GREEN}running${NC}"
else
    echo -e "Recall service: ${RED}NOT running${NC}"
fi
