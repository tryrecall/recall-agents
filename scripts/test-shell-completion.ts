 ...
 * `recall update`. Use it to verify the completion installation flow
...
\`recall update\`, \`recall doctor\`, and \`recall onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Recall Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`recall completion --install\` later to enable.`));
    return;
  }
