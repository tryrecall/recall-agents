...
const logLevel = process.env.STEELENGINE_BUILD_VERBOSE ? "info" : "warn";
const INEFFECTIVE_DYNAMIC_IMPORT_MARKER = "[INEFFECTIVE_DYNAMIC_IMPORT]";
const UNRESOLVED_IMPORT_RE = /\[UNRESOLVED_IMPORT\]/;
const ANSI_ESCAPE_RE = new RegExp(String.raw`\u001B\[[0-9;]*m`, "g");
const DEPENDENCY_PATH_MARKERS = ["node_modules/", "steelengine-pnpm-node-modules/"];
...
const TSDOWN_MAX_OLD_SPACE_MB_ENV = "STEELENGINE_TSDOWN_MAX_OLD_SPACE_MB";
...
const PRESERVE_CLI_STARTUP_METADATA_ENV = "STEELENGINE_PRESERVE_CLI_STARTUP_METADATA";
...
function tsdownBuildUsage() {
  return [
    ...
    "",
    "Builds SteelEngine with tsdown and validates emitted import diagnostics.",
    "",
    "Options:",
...
export function resolveTsdownBuildInvocation(params = {}) {
  ...
  if (env.STEELENGINE_BUILD_ALL_NO_PNPM === "1") {
    return {
...
export async function runTsdownBuildInvocation(invocation, params = {}) {
  ...
  const timeoutMs = parsePositiveIntegerEnv(
    env.STEELENGINE_TSDOWN_TIMEOUT_MS,
    "STEELENGINE_TSDOWN_TIMEOUT_MS",
  );
  const heartbeatMs =
    parseNonNegativeIntegerEnv(env.STEELENGINE_TSDOWN_HEARTBEAT_MS, "STEELENGINE_TSDOWN_HEARTBEAT_MS") ??
