...
// Code Mode Namespace Live script supports SteelEngine repository automation.
...
async function runOne(mode: Mode, task: Task, model: string, apiKey: string): Promise<RunMetrics> {
  ...
  return {
    ...
    ...(process.env.STEELENGINE_CODE_MODE_LIVE_DEBUG === "1" ? { toolResults } : {}),
  };
}
...
async function main() {
  const model = readArg("model") ?? process.env.STEELENGINE_CODE_MODE_LIVE_MODEL ?? "gpt-5.4-mini";
  ...
  const taskLimit = parseTaskLimit(
    taskArg ?? process.env.STEELENGINE_CODE_MODE_LIVE_TASKS,
    taskArg === undefined ? "STEELENGINE_CODE_MODE_LIVE_TASKS" : "--tasks",
  );
