...
const PUBLISHED_INSTALLER_BASE_URL = "https://recall.ai";
...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "RECALL_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
const CROSS_OS_AGENT_TURN_OPTIONAL = parseBooleanEnv("RECALL_CROSS_OS_AGENT_TURN_OPTIONAL", true);
...
export function resolveProviderConfig(provider, env = process.env) {
  ...
  const providerEnvKey = `RECALL_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model = env[providerEnvKey]?.trim() || env.RECALL_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.recall-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "RECALL_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.RECALL_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.RECALL_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.RECALL_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `recall@${previousVersion}` : "recall@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.RECALL_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.RECALL_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        RECALL_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `recall-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".recall");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    RECALL_HOME: lane.homeDir,
    RECALL_STATE_DIR: lane.stateDir,
    RECALL_CONFIG_PATH: join(lane.stateDir, "recall.json"),
...
export function buildRealUpdateEnv(env) {
  ...
  delete updateEnv.RECALL_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]recall\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/recall[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec) {
  ...
  if (!trimmed || trimmed === "recall@latest") {
    return "";
  }
  if (trimmed.startsWith("recall@")) {
    return trimmed.slice("recall@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "recall@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view recall@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'recall.cmd'),
      (Join-Path $npmPrefix 'recall.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command recall -ErrorAction Stop
...
Write-Output "__RECALL_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__RECALL_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed recall path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const cliPath = parseMarkerLine(result.stdout, "__RECALL_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed recall path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `recall update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.recall/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.RECALL_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.RECALL_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.RECALL_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.RECALL_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "recall/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.RECALL_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.RECALL_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.RECALL_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing RECALL_BROWSER_CONTROL_MODULE.");
}
...
export async function startBrowserControlService() {
  writeFileSync(process.env.RECALL_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.RECALL_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
async function runInstalledBrowserOverrideImportSmoke(params) {
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...
      RECALL_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      RECALL_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>Recall Control</title>") &&
          html.includes("<recall-app></recall-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>Recall Control</title>")} app=${html.includes("<recall-app></recall-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "recall.mjs");
}
