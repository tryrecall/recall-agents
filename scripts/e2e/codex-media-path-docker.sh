...
IMAGE_NAME="$(docker_e2e_resolve_image "recall-codex-media-path-e2e" RECALL_CODEX_MEDIA_PATH_E2E_IMAGE)"
PORT="${RECALL_CODEX_MEDIA_PATH_PORT:-18790}"
TOKEN="codex-media-path-e2e-$$"
CODEX_PLUGIN_SPEC="${RECALL_CODEX_MEDIA_PATH_PLUGIN_SPEC:-npm:@recall/codex}"

docker_e2e_build_or_reuse "$IMAGE_NAME" codex-media-path "$ROOT_DIR/scripts/e2e/Dockerfile" "$ROOT_DIR"
RECALL_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 codex-media-path empty)"

echo "Running Codex media-path Docker E2E..."
docker_e2e_run_logged_with_harness codex-media-path \
  -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 \
  -e "RECALL_CODEX_MEDIA_PATH_PLUGIN_SPEC=$CODEX_PLUGIN_SPEC" \
  -e "RECALL_CODEX_MEDIA_PATH_TIMEOUT_SECONDS=${RECALL_CODEX_MEDIA_PATH_TIMEOUT_SECONDS:-180}" \
