...
// against the Recall npm tarball installed in the functional image.
...
export async function connectMcpClient(params: {
  ...
  const tokenDir = "/tmp/recall-mcp-client";
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      RECALL_STATE_DIR: "/tmp/recall-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[recall mcp] ${String(chunk)}`);
  });
