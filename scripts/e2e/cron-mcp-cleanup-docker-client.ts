// Cron Mcp Cleanup Docker Client script supports SteelEngine repository automation.
...
export function readCronMcpCleanupProbePidWaitMs(env: NodeJS.ProcessEnv = process.env): number {
  return readPositiveIntEnv("STEELENGINE_CRON_MCP_CLEANUP_PID_WAIT_MS", 120_000, env);
}
...
async function waitForProbeExit(params: {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    if (!args || !args.includes("steelengine-cron-mcp-cleanup-probe")) {
      return;
    }
    await delay(100);
  }
  ...
  while (Date.now() - startedAt < params.timeoutMs) {
    ...
    if (observed.length > 0) {
      ...
      for (const pid of observed) {
        const args = await describeProbePid(pid);
        if (args?.includes("steelengine-cron-mcp-cleanup-probe")) {
          allExited = false;
          break;
        }
      }
...
async function runCronCleanupScenario(params: {
  ...
  assert(
    initialArgs === undefined || initialArgs.includes("steelengine-cron-mcp-cleanup-probe"),
