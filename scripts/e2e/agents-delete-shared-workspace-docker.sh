...
IMAGE_NAME="$(docker_e2e_resolve_image "recall-agents-delete-shared-workspace-e2e:local" RECALL_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE)"
SKIP_BUILD="${RECALL_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD:-0}"
DOCKER_COMMAND_TIMEOUT="${RECALL_AGENTS_DELETE_SHARED_WORKSPACE_DOCKER_COMMAND_TIMEOUT:-300s}"
RECALL_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 agents-delete-shared-workspace empty)"
...
run_logged agents-delete-shared-workspace docker_e2e_docker_cmd run --rm \
  ...
  -e RECALL_SKIP_BROWSER_CONTROL_SERVER=1 \
...
run_openclaw() {
  ...
  if [ -f /app/recall.mjs ]; then
    node /app/recall.mjs "$@"
    return
  fi
  echo "recall CLI not found in Docker image" >&2
  exit 1
}
...
mkdir -p "$RECALL_STATE_DIR" "$SHARED_WORKSPACE"
