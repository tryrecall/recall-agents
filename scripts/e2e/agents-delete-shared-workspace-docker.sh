...
IMAGE_NAME="$(docker_e2e_resolve_image "steelengine-agents-delete-shared-workspace-e2e:local" STEELENGINE_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE)"
SKIP_BUILD="${STEELENGINE_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD:-0}"
DOCKER_COMMAND_TIMEOUT="${STEELENGINE_AGENTS_DELETE_SHARED_WORKSPACE_DOCKER_COMMAND_TIMEOUT:-300s}"
STEELENGINE_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 agents-delete-shared-workspace empty)"
...
run_logged agents-delete-shared-workspace docker_e2e_docker_cmd run --rm \
  ...
  -e STEELENGINE_SKIP_BROWSER_CONTROL_SERVER=1 \
  -e STEELENGINE_SKIP_ACPX_RUNTIME=1 \
  -e STEELENGINE_SKIP_ACPX_RUNTIME_PROBE=1 \
  -e STEELENGINE_GATEWAY_TOKEN=agents-delete-shared-workspace-token \
...
mkdir -p "$STEELENGINE_STATE_DIR" "$SHARED_WORKSPACE"
node scripts/e2e/lib/fixture.mjs agents-delete-config

run_steelengine agents delete ops --force --json > "$output_file"
