...
IMAGE_NAME="$(docker_e2e_resolve_image "recall-live-plugin-tool-e2e" RECALL_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${RECALL_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${RECALL_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${RECALL_CURRENT_PACKAGE_TGZ:-}"
AGENT_TURN_TIMEOUT_SECONDS="${RECALL_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}"
PROFILE_FILE="${RECALL_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${RECALL_TESTBOX_PROFILE_FILE:-$HOME/.recall-testbox-live.profile}}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${RECALL_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "RECALL_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires RECALL_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
RECALL_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e RECALL_LIVE_PLUGIN_TOOL_MODEL="${RECALL_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.5}" \
  -e "RECALL_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
MODEL_REF="${RECALL_LIVE_PLUGIN_TOOL_MODEL:?missing RECALL_LIVE_PLUGIN_TOOL_MODEL}"
PLUGIN_ID="e2e-slug-tool"
PLUGIN_NAME="@recall/e2e-slug-tool"
PLUGIN_VERSION="0.0.0-e2e.1"
TOOL_NAME="e2e_slug_probe"
SEED="Recall E2E Plugin Tool $(date +%s)-$RANDOM"
...
dump_debug_logs() {
  ...
  openclaw_e2e_dump_logs \
    ...
    /tmp/recall-plugin-install.log \
    /tmp/recall-plugin-enable.log \
    /tmp/recall-plugins-list.json \
    /tmp/recall-plugin-inspect.json \
...
fixture_dir="$(mktemp -d /tmp/recall-live-plugin-tool.XXXXXX)"
...
recall plugins install "npm-pack:$plugin_tgz" --force >/tmp/recall-plugin-install.log 2>&1
node scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
recall plugins enable "$PLUGIN_ID" >/tmp/recall-plugin-enable.log 2>&1
recall plugins list --json >/tmp/recall-plugins-list.json
recall plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/recall-plugin-inspect.json
...
recall agent --local \
  ...
  --thinking off \
  --timeout "${RECALL_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/recall-agent.json 2>/tmp/recall-agent.err
