...
IMAGE_NAME="$(docker_e2e_resolve_image "steelengine-live-plugin-tool-e2e" STEELENGINE_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${STEELENGINE_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${STEELENGINE_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${STEELENGINE_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${STEELENGINE_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${STEELENGINE_TESTBOX_PROFILE_FILE:-$HOME/.steelengine-testbox-live.profile}}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${STEELENGINE_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "STEELENGINE_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires STEELENGINE_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
STEELENGINE_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e STEELENGINE_LIVE_PLUGIN_TOOL_MODEL="${STEELENGINE_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.6-luna}" \
  ...
  -e "STEELENGINE_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
MODEL_REF="${STEELENGINE_LIVE_PLUGIN_TOOL_MODEL:?missing STEELENGINE_LIVE_PLUGIN_TOOL_MODEL}"
PLUGIN_ID="e2e-slug-tool"
PLUGIN_NAME="@steelengine/e2e-slug-tool"
PLUGIN_VERSION="0.0.0-e2e.1"
TOOL_NAME="e2e_slug_probe"
SEED="SteelEngine E2E Plugin Tool $(date +%s)-$RANDOM"
...
dump_debug_logs() {
  local status="$1"
  local agent_output_dump_bytes="${STEELENGINE_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES:-16384}"
  echo "Live plugin tool scenario failed with exit code $status" >&2
  if [ -f /tmp/steelengine-agent.json ]; then
    echo "--- /tmp/steelengine-agent.json (last ${agent_output_dump_bytes} bytes) ---" >&2
    tail -c "$agent_output_dump_bytes" /tmp/steelengine-agent.json >&2 || true
    echo >&2
  fi
  steelengine_e2e_dump_logs \
    /tmp/steelengine-install.log \
    /tmp/steelengine-plugin-install.log \
    /tmp/steelengine-plugin-enable.log \
    /tmp/steelengine-plugins-list.json \
    /tmp/steelengine-plugin-inspect.json \
    /tmp/steelengine-live-plugin-tool-pack.log \
...
fixture_dir="$(mktemp -d /tmp/steelengine-live-plugin-tool.XXXXXX)"
...
(cd "$plugin_dir" && npm pack --pack-destination "$fixture_dir" --silent) \
  >/tmp/steelengine-live-plugin-tool-pack.log 2>&1
plugin_tgzs=()
...
if [ "${#plugin_tgzs[@]}" -ne 1 ]; then
  ...
  steelengine_e2e_dump_logs /tmp/steelengine-live-plugin-tool-pack.log
  exit 1
fi
plugin_tgz="${plugin_tgzs[0]}"

echo "Installing fixture plugin from npm-pack: $plugin_tgz"
steelengine plugins install "npm-pack:$plugin_tgz" --force >/tmp/steelengine-plugin-install.log 2>&1
node --disable-warning=ExperimentalWarning scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
steelengine plugins enable "$PLUGIN_ID" >/tmp/steelengine-plugin-enable.log 2>&1
steelengine plugins list --json >/tmp/steelengine-plugins-list.json
steelengine plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/steelengine-plugin-inspect.json
...
steelengine agent --local \
  ...
  --thinking off \
  --timeout "${STEELENGINE_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/steelengine-agent.json 2>/tmp/steelengine-agent.err
