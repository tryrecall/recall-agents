...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".steelengine", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(
    runtimeContextMessage.customType === "steelengine.runtime-context",
  ...
  assert(
    !userText.includes("STEELENGINE_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function verifyDoctorRepair(root: string) {
  ...
  const databasePath = path.join(stateDir, "agents", "main", "agent", "steelengine-agent.sqlite");
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-session-runtime-context-"));
  const stateDir = path.join(root, ".steelengine");
  setEnvValue("HOME", root);
  setEnvValue("STEELENGINE_STATE_DIR", stateDir);
  setEnvValue("STEELENGINE_CONFIG_PATH", path.join(stateDir, "steelengine.json"));
  try {
    ...
    if (process.env.STEELENGINE_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
