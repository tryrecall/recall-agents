...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".recall", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(customEntry.customType === "recall.runtime-context", "unexpected custom message type");
  ...
  assert(
    !userText.includes("RECALL_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "recall-session-runtime-context-"));
  process.env.HOME = root;
  process.env.RECALL_STATE_DIR = path.join(root, ".recall");
  process.env.RECALL_CONFIG_PATH = path.join(process.env.RECALL_STATE_DIR, "recall.json");
  try {
    ...
    if (process.env.RECALL_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
