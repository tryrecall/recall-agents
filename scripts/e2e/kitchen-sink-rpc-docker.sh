...
IMAGE_NAME="$(docker_e2e_resolve_image "steelengine-kitchen-sink-rpc-e2e" STEELENGINE_KITCHEN_SINK_RPC_E2E_IMAGE)"
MAX_MEMORY_MIB="$(docker_e2e_read_nonnegative_decimal_env STEELENGINE_KITCHEN_SINK_MAX_MEMORY_MIB 2048)"
...
CONTAINER_NAME="steelengine-kitchen-sink-rpc-e2e-$$"
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/steelengine-kitchen-sink-rpc.XXXXXX")"
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/steelengine-kitchen-sink-rpc-stats.XXXXXX")"
...
DOCKER_ENV_ARGS=(
  ...
  -e STEELENGINE_ENTRY=/app/steelengine.mjs
)

for env_name in \
  STEELENGINE_KITCHEN_SINK_NPM_SPEC \
  STEELENGINE_KITCHEN_SINK_PLUGIN_ID \
  ...
  STEELENGINE_KITCHEN_SINK_RPC_PORT \
  ...
  STEELENGINE_KITCHEN_SINK_KEEP_TMP \
  STEELENGINE_KITCHEN_SINK_MAX_RSS_MIB \
  STEELENGINE_KITCHEN_SINK_COMMAND_MAX_RSS_MIB; do
  env_value="${!env_name:-}"
