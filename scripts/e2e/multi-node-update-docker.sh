...
# Recall under node-A, registers the gateway service pointing at node-A, then
# switches PATH so node-B comes first and runs `recall update`. Verifies that:
#
...
#
# Requires: Docker, a built recall-current.tgz (or will build one).
set -euo pipefail
...
IMAGE_NAME="recall-multi-node-update-e2e"
DOCKER_RUN_TIMEOUT="${RECALL_MULTI_NODE_DOCKER_TIMEOUT:-300s}"
ARTIFACT_DIR="${RECALL_MULTI_NODE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/multi-node-update}"

mkdir -p "$ARTIFACT_DIR"
chmod -R a+rwX "$ARTIFACT_DIR" || true

# Build the bare e2e image and prepare the package tarball.
docker_e2e_build_or_reuse "$IMAGE_NAME" multi-node-update "$ROOT_DIR/scripts/e2e/Dockerfile" "$ROOT_DIR" "bare" "${RECALL_SKIP_DOCKER_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz multi-node-update "${RECALL_CURRENT_PACKAGE_TGZ:-}")"
...
docker_e2e_run_with_harness \
  ...
  -e RECALL_SKIP_PROVIDERS=1 \
  -e RECALL_SKIP_CHANNELS=1 \
  -e RECALL_DISABLE_BONJOUR=1 \
...
echo ""
echo "── Step 2: Install Recall under node-A ──"

# Use node-A to install recall with npm prefix A.
...
echo "Installing Recall package under node-A prefix: $NPM_PREFIX_A"
npm install -g /tmp/recall-current.tgz --no-fund --no-audit >"$ARTIFACTS/install-a.log" 2>&1
echo "Installed. Checking recall location..."

RECALL_A="$(command -v recall)"
echo "recall binary: $RECALL_A"
echo "recall version: $(recall --version 2>/dev/null || echo unknown)"

# Record the package root for node-A install.
PACKAGE_ROOT_A="$NPM_PREFIX_A/lib/node_modules/recall"
...
GATEWAY_UNIT_PATH="/root/.config/systemd/user/recall-gateway.service"
...
recall gateway install --json >"$ARTIFACTS/gateway-install.json" 2>"$ARTIFACTS/gateway-install.err" || true
...
# but recall is NOT installed there.
...
echo "which recall: $(command -v recall)"
echo "process.execPath will be: $(node -e "console.log(process.execPath)")"

echo ""
echo "── Step 6: Run recall update (this is the bug) ──"
...
echo "Running recall update --yes --json..."
UPDATE_EXIT=0
recall update --yes --json \
  --tag /tmp/recall-current.tgz \
...
if [ "$UPDATE_EXIT" -ne 0 ] && ! grep -q "gateway" "$ARTIFACTS/update.err" 2>/dev/null; then
  echo "FAIL: recall update failed before reaching the package install step"
...
if [ "$BAKED_NODE_AFTER" = "$NODE_B" ] && [ "$BAKED_NODE_BEFORE" != "$NODE_B" ]; then
  ...
  echo "   but Recall package is still under node-A prefix ($PACKAGE_ROOT_A)."
...
# Check 2: Is the Recall package installed under node-B npm prefix?
if [ -f "$NPM_PREFIX_B/lib/node_modules/recall/package.json" ]; then
  echo "WARNING: Recall was ALSO installed under node-B prefix (split install)"
else
  echo "OK: Recall is NOT under node-B prefix (expected: only under node-A)"
fi
...
if [ -f "$ARTIFACTS/update.err" ]; then
  if grep -qi "Shell Recall root differs" "$ARTIFACTS/update.err" 2>/dev/null; then
...
if [ -f "$GATEWAY_UNIT_PATH" ]; then
  ...
  if [ -s "$GATEWAY_PID_FILE" ] && kill -0 "$(cat "$GATEWAY_PID_FILE" 2>/dev/null)" 2>/dev/null; then
    ...
    # Try a health probe.
    if recall gateway status --json >"$ARTIFACTS/status.json" 2>&1; then
