...
IMAGE_NAME="$(docker_e2e_resolve_image "steelengine-onboard-e2e" STEELENGINE_ONBOARD_E2E_IMAGE)"
STEELENGINE_TEST_STATE_FUNCTION_B64="$(docker_e2e_test_state_function_b64)"
MAX_MEMORY_MIB="$(docker_e2e_read_nonnegative_decimal_env STEELENGINE_ONBOARD_MAX_MEMORY_MIB 2048)"
MAX_CPU_PERCENT="$(docker_e2e_read_nonnegative_decimal_env STEELENGINE_ONBOARD_MAX_CPU_PERCENT 1200)"
DOCKER_RUN_TIMEOUT="${STEELENGINE_ONBOARD_DOCKER_RUN_TIMEOUT:-1200s}"
...
CONTAINER_NAME="steelengine-onboard-e2e-$$"
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/steelengine-onboard.XXXXXX")"
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/steelengine-onboard-stats.XXXXXX")"
...
DOCKER_COMMAND_TIMEOUT="$DOCKER_RUN_TIMEOUT" docker_e2e_docker_run_cmd run --name "$CONTAINER_NAME" "${DOCKER_E2E_HARNESS_ARGS[@]}" -t \
  ...
  -e "STEELENGINE_ONBOARD_GATEWAY_WAIT_ATTEMPTS=$GATEWAY_WAIT_ATTEMPTS" \
