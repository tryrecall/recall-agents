...
export async function runWindowsBackgroundPowerShell(
  ...
  const fileBase = `recall-parallels-${nonce}`;
...
if (Test-Path $logPath) {
  ...
  if ($bytes.Length -gt $offset) {
    "__RECALL_LOG_OFFSET__:$($bytes.Length)"
...
if (Test-Path $donePath) {
  ...
  "__RECALL_BACKGROUND_EXIT__:$backgroundExit"
  '__RECALL_BACKGROUND_DONE__'
  ...
  exit 0
    ...
    const offsetMatch = poll.stdout.match(/__RECALL_LOG_OFFSET__:(\d+)/);
    if (offsetMatch) {
      lastLogOffset = Number(offsetMatch[1]);
    }
    if (poll.stdout.includes("__RECALL_BACKGROUND_DONE__")) {
      const exitMatch = poll.stdout.match(/__RECALL_BACKGROUND_EXIT__:(\S+)/);
...
export class LinuxGuest {
  ...
  exec(args: string[], options: GuestExecOptions = {}): string {
    const result = run(
      "prlctl",
      ["exec", this.vmName, "/usr/bin/env", "HOME=/root", "RECALL_ALLOW_ROOT=1", ...args],
      {
        check: false,
        input: options.input,
        quiet: true,
  ...
  bash(script: string): string {
    const scriptPath = `/tmp/recall-parallels-${process.pid}-${Date.now()}.sh`;
    const write = run(
      "prlctl",
      [
        "exec",
...
export class WindowsGuest {
  ...
  powershell(script: string, options: GuestExecOptions = {}): string {
    const scriptName = `recall-parallels-${process.pid}-${Date.now()}.ps1`;
