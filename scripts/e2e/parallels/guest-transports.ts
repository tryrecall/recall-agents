// Guest Transports script supports SteelEngine repository automation.
...
function guestScriptName(extension: string): string {
  return `steelengine-parallels-${randomUUID()}.${extension}`;
}
...
export async function runWindowsBackgroundPowerShell(
  ...
  const guestRunDir = `steelengine-parallels\\${nonce}`;
  const windowsDonePath = `%WINDIR%\\Temp\\${guestRunDir}\\done`;
  const windowsLogPath = `%WINDIR%\\Temp\\${guestRunDir}\\run.log`;
  const backgroundExitPrefix = `__STEELENGINE_BACKGROUND_EXIT__:${nonce}:`;
  const backgroundDoneMarker = `__STEELENGINE_BACKGROUND_DONE__:${nonce}`;
  const deadline = Date.now() + options.timeoutMs;
  const pathsScript = `$runDir = Join-Path (Join-Path $env:WINDIR 'Temp\\steelengine-parallels') ${psSingleQuote(nonce)}
...
function Write-SteelEngineUtf8File([string]$Path, [string]$Value) {
  [System.IO.File]::WriteAllText($Path, $Value, [System.Text.UTF8Encoding]::new($false))
}`;
  const payload = `$ErrorActionPreference = 'Stop'
$PSNativeCommandUseErrorActionPreference = $false
${pathsScript}
Write-SteelEngineUtf8File $pidPath ([string]$PID)
$script:SteelEngineBackgroundLogBytes = 0
function Add-SteelEngineBackgroundLog {
  param([Parameter(ValueFromPipeline=$true)]$InputObject)
  process {
    $text = $InputObject | Out-String
    $bytes = [System.Text.Encoding]::UTF8.GetBytes($text)
    $remaining = [int64]${WINDOWS_BACKGROUND_LOG_MAX_BYTES} - $script:SteelEngineBackgroundLogBytes
    if ($remaining -le 0) {
      return
    }
    ...
    try {
      if ($count -gt 0) {
        ...
        $script:SteelEngineBackgroundLogBytes += $count
      }
      if ($needsBoundaryNewline) {
        $stream.WriteByte(10)
        $script:SteelEngineBackgroundLogBytes++
      }
    } finally {
      $stream.Dispose()
    }
...
try {
  & {
${options.script}
  } *>&1 | Add-SteelEngineBackgroundLog
  Write-SteelEngineUtf8File $exitPath '0'
} catch {
  $_ | Add-SteelEngineBackgroundLog
  Write-SteelEngineUtf8File $exitPath '1'
} finally {
  Write-SteelEngineUtf8File $donePath 'done'
}`;
  const writeArgs = [
    "exec",
    options.vmName,
...
function cleanupWindowsBackground(
  ...
  const stopProcessTree = options.stopProcessTree
    ? `function Stop-SteelEngineBackgroundProcessTree([int]$ProcessId) {
  Get-CimInstance Win32_Process -Filter "ParentProcessId=$ProcessId" -ErrorAction SilentlyContinue | ForEach-Object {
    Stop-SteelEngineBackgroundProcessTree ([int]$_.ProcessId)
  }
...
export class LinuxGuest {
  ...
  private transportArgs(args: string[]): string[] {
    return ["exec", this.vmName, "/usr/bin/env", "HOME=/root", "STEELENGINE_ALLOW_ROOT=1", ...args];
  }
