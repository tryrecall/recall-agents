// Filesystem script supports SteelEngine repository automation.
...
const STEELENGINE_VERSION_PATTERN = /SteelEngine\s+([0-9][^\s]*)/gi;
...
export async function extractLastSteelEngineVersionFromLog(
  logPath: string,
  pattern = STEELENGINE_VERSION_PATTERN,
  maxBytes = DEFAULT_TEXT_FILE_TAIL_BYTES,
): Promise<string> {
...
export async function makeTempDir(prefix: string): Promise<string> {
  const root =
    process.env.STEELENGINE_PARALLELS_ARTIFACT_ROOT || path.join(repoRoot, ".artifacts", "parallels");
