...
const defaultOptions = (): WindowsOptions => ({
  ...
  installUrl: "https://recall.ai/install.ps1",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
  ...
  snapshotHint: "pre-recall-native-e2e-2026-03-12",
  targetPackageSpec: "",
  upgradeFromPackedMain: false,
  vmName: "Windows 11",
});

const windowsPortableGitPathScript = `$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'Recall\\deps') 'portable-git') ''
...
Options:
  ...
  --upgrade-from-packed-main
                             ...
                             then run recall update --channel dev.
...
class WindowsSmoke {
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("recall-parallels-windows.");
    this.phases = new PhaseRunner(this.runDir);
    this.guest = new WindowsGuest(this.options.vmName, this.phases);
    this.tgzDir = await makeTempDir("recall-parallels-windows-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () => this.installMain("recall-main-fresh.tgz"));
    ...
    await this.phase(
      ...
      Number(process.env.RECALL_PARALLELS_WINDOWS_AGENT_TIMEOUT_S || 2700),
      () => this.verifyTurn(),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    await this.phase(
      "upgrade.update-dev",
      Number(process.env.RECALL_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S || 1200),
  ...
  private guestPowerShell(
    ...
    return this.guest.powershell(`${windowsRecallResolver}\n${script}`, options);
  }
  ...
  private logGuestPreflight(cleanRecall: boolean): void {
    const cleanScript = cleanRecall
      ? "npm.cmd uninstall -g recall --no-fund --no-audit --loglevel=error 2>$null; $global:LASTEXITCODE = 0"
      : "";
...
if ($LASTEXITCODE -ne 0) { throw "recall --version failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestPowerShell("Invoke-Recall --version");
...
Invoke-Recall onboard --non-interactive --mode local --auth-choice ${psSingleQuote(this.auth.authChoice)} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json
if ($LASTEXITCODE -ne 0) { throw "recall onboard failed with exit code $LASTEXITCODE" }`,
      720_000,
    );
  }
...
${windowsScopedEnvFunction}
$script:RecallUpdateExit = 0
Invoke-WithScopedEnv @{ RECALL_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'; RECALL_DISABLE_BUNDLED_PLUGINS = '1' } {
  Invoke-Recall update --channel dev --yes --json
  $script:RecallUpdateExit = $LASTEXITCODE
}
if ($script:RecallUpdateExit -ne 0) { throw "recall update failed with exit code $script:RecallUpdateExit" }
Invoke-Recall --version
Invoke-Recall update status --json`,
      { timeoutMs: Number(process.env.RECALL_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S || 1200) * 1000 },
    );
  }
...
if ($LASTEXITCODE -ne 0) { throw "gateway ${action} failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyGatewayReachable(): void {
    ...
    const recoveryAfter =
      Number(process.env.RECALL_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S || 180) * 1000;
    const start = Date.now();
    while (Date.now() < deadline) {
      const probe = this.guestPowerShell(
        "Invoke-Recall gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json",
      ...
      if (!recoveryTried && Date.now() - start >= recoveryAfter) {
        ...
        this.guestPowerShell("Invoke-Recall gateway start", {
          check: false,
          timeoutMs: 120_000,
        });
        recoveryTried = true;
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestPowerShell("Invoke-Recall gateway status --help", {
      check: false,
    });
    const suffix = help.includes("--require-rpc") ? "--deep --require-rpc" : "--deep";
    this.guestPowerShell(`Invoke-Recall gateway status ${suffix}`);
  }
...
if (-not $agentOk) { throw 'recall agent finished without OK response' }`,
      Number(process.env.RECALL_PARALLELS_WINDOWS_AGENT_TIMEOUT_S || 2700) * 1000,
    );
  }
