// Package Artifact script supports SteelEngine repository automation.
...
function resolveNpmPackTarballFilename(value: unknown): string {
  ...
  const result = Array.isArray(value)
    ? value.at(-1)
    : value && typeof value === "object" && "steelengine" in value
      ? value.steelengine
      : value;
  const filename =
    result &&
...
export function resolveSteelEngineRegistryVersion(specOrAlias: string): string {
  const rawValue = specOrAlias.trim();
  const value = rawValue.startsWith("steelengine@") ? rawValue.slice("steelengine@".length) : rawValue;
  if (!value) {
    return "";
  }
  if (value === "latest" || value === "beta" || /^\d/.test(value)) {
    return npmViewVersion(`steelengine@${value}`);
  }
  const betaMatch = /^beta(\d+)$/u.exec(value);
  if (betaMatch) {
    const betaSuffix = `-beta.${betaMatch[1]}`;
    const versions = JSON.parse(
      run("npm", ["view", "steelengine", "versions", "--json"], { quiet: true }).stdout,
    ) as string[];
    const match = versions
      ...
      .at(-1);
    if (!match) {
      die(`no steelengine registry version found for alias ${value}`);
    }
    return match;
  }
  return "";
...
export async function packSteelEngine(input: {
  destination: string;
  packageSpec?: string;
  requireControlUi?: boolean;
  ...
  return await withPackageLock(path.join(tmpdir(), "steelengine-parallels-build.lock"), async () => {
    ...
    const tgzPath = path.join(input.destination, `steelengine-main-${shortHead}.tgz`);
...
async function acquirePackageLock(
  ...
  const timeoutMs = readPositiveIntEnv("STEELENGINE_PARALLELS_PACKAGE_LOCK_TIMEOUT_MS", 30 * 60_000);
  const staleMs = readPositiveIntEnv("STEELENGINE_PARALLELS_PACKAGE_LOCK_STALE_MS", 2 * 60 * 60_000);
