...
// Macos Smoke script supports SteelEngine repository automation.
...
const guestSteelEngine = "steelengine";
const guestSteelEngineEntry = '"$(npm root -g)/steelengine/steelengine.mjs"';
const guestSteelEngineEntryRunner = `node ${guestSteelEngineEntry}`;
...
class MacosSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await makeTempDir("steelengine-parallels-macos.");
    ...
    this.tgzDir = await makeTempDir("steelengine-parallels-macos-tgz.");
    try {
  ...
  private guestSteelEngineEntryExec(
    args: string[],
    options: { check?: boolean; env?: Record<string, string> } = {},
  ): string {
    ...
    return this.guestSh(
      `set -e
entry="$(npm root -g)/steelengine/steelengine.mjs"
exec node "$entry" ${argv}`,
  ...
  private resetState(): void {
    this.guestSh(String.raw`/usr/bin/pkill -f 'steelengine.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'steelengine-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'steelengine.mjs gateway' >/dev/null 2>&1 || true
...
${guestNpm} uninstall -g steelengine >/dev/null 2>&1 || true
rm -rf "$HOME/.steelengine"
...
curl -fsSL --connect-timeout 10 --max-time 120 --retry 2 --retry-delay 2 ${shellQuote(
      ...
      )} -o /tmp/steelengine-install.sh
bash /tmp/steelengine-install.sh --version ${shellQuote(this.installVersion)}
${guestSteelEngine} --version`,
    );
  }
...
${guestSteelEngine} --version`);
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec([guestSteelEngine, "--version"]);
...
check_path "$root/steelengine"
check_path "$root/steelengine/extensions"
if [ -d "$root/steelengine/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "$extension_dir"
  done < <(/usr/bin/find "$root/steelengine/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi`);
  }
  ...
  private ensureGuestPnpm(): void {
    this.guestSh(String.raw`set -eu
bootstrap_root=/tmp/steelengine-smoke-pnpm-bootstrap
...
"$bootstrap_bin/pnpm" --version`);
  ...
  private runDevChannelUpdate(): void {
    ...
    this.guestSh(
      `set -eu
rm -rf ${shellQuote(`${home}/steelengine`)}
export PATH=${shellQuote(`/tmp/steelengine-smoke-pnpm-bootstrap/node_modules/.bin:${guestPath}`)}
${guestNode} - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || ${JSON.stringify(home)}, ".steelengine", "steelengine.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
config.update = { ...(config.update || {}), channel: "dev" };
fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + "\\n");
JS
/usr/bin/env NODE_OPTIONS=--max-old-space-size=8192 STEELENGINE_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 STEELENGINE_DISABLE_BUNDLED_PLUGINS=1 ${guestSteelEngineEntryRunner} update --channel dev --yes --json
${guestSteelEngineEntryRunner} --version
${guestSteelEngineEntryRunner} update status --json`,
    );
  }

  private verifyDevChannelUpdate(): void {
    const status = this.guestSteelEngineEntryExec(["update", "status", "--json"]);
  ...
  private startManualGatewayIfNeeded(): void {
    ...
    this.guestSh(
      `set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'steelengine.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'steelengine-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'steelengine.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env HOME=${shellQuote(home)} USER=${shellQuote(this.guestUser)} LOGNAME=${shellQuote(this.guestUser)} PATH=${shellQuote(guestPath)} ${shellQuote(
        `${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`,
      )} STEELENGINE_HOME=${shellQuote(home)} STEELENGINE_STATE_DIR=${shellQuote(`${home}/.steelengine`)} STEELENGINE_CONFIG_PATH=${shellQuote(
        `${home}/.steelengine/steelengine.json`,
      )} ${guestSteelEngineEntryRunner} gateway run --bind loopback --port 18789 --force </dev/null >/tmp/steelengine-parallels-macos-gateway.log 2>&1 &
sleep 1`,
    );
  }
  ...
  private verifyGateway(): void {
    for (let attempt = 1; attempt <= 8; attempt++) {
      const result = this.guestSteelEngine(
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestExec([guestSteelEngine, "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["gateway", "status", "--deep", "--require-rpc"]
      : ["gateway", "status", "--deep"];
    if (!this.guestSteelEngine(args, false)) {
      throw new Error("gateway status failed");
    }
  }

  private guestSteelEngine(args: string[], check: boolean): boolean {
    const result = run(
      "prlctl",
      [
        "exec",
    ...
    if (check && result.status !== 0) {
      throw new Error(`steelengine ${args.join(" ")} failed`);
    }
...
while [ $SECONDS -lt $deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 http://127.0.0.1:18789/ >/tmp/steelengine-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>SteelEngine Control</title>' /tmp/steelengine-dashboard-smoke.html >/dev/null &&
      grep -F '<steelengine-app></steelengine-app>' /tmp/steelengine-dashboard-smoke.html >/dev/null; then
      asset_paths="$(
        sed -nE 's/.*<(script|link)[^>]*(src|href)=["'"'"']([^"'"'"']+)["'"'"'].*/\3/p' /tmp/steelengine-dashboard-smoke.html |
          grep -E '(^|/)assets/' |
          grep -Ev '^(https?:)?//' |
          sort -u
      )"
...
exit 1`);
  ...
  private verifyTurn(): void {
    this.guestSteelEngineEntryExec(["models", "set", this.auth.modelId]);
...
JSON
${guestSteelEngineEntryRunner} config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    }
    this.guestSteelEngineEntryExec([
      "config",
      "set",
    ...
    this.guestSteelEngineEntryExec(["config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} ${guestSteelEngineEntryRunner} agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private ensureDiscordGatewayReady(): void {
    ...
    this.verifyGateway();
    const status = this.guestSteelEngineEntryExec(["channels", "status", "--probe", "--json"]);
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastSteelEngineVersionFromLog(path.join(this.runDir, `${phaseName}.log`));
  }
