// Powershell script supports SteelEngine repository automation.
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:STEELENGINE_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.steelengine\\steelengine.json' }
$agentTurnVersionText = Invoke-SteelEngine --version 2>$null | Out-String
$agentTurnRuntimePolicySupported = $false
if ($agentTurnVersionText -match 'SteelEngine\\s+(\\d{4})\\.(\\d{1,2})\\.(\\d{1,2})') {
...
$env:STEELENGINE_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:STEELENGINE_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$env:STEELENGINE_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED = if ($agentTurnRuntimePolicySupported) { '1' } else { '0' }
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'steelengine-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.STEELENGINE_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.STEELENGINE_PARALLELS_AGENT_CONFIG_PATCH || "{}");
const canWriteAgentRuntime = process.env.STEELENGINE_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED === "1";
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@steelengine", "codex"), { recursive: true, force: true });
...
if (selectedModelEntry && typeof selectedModelEntry === "object" && !Array.isArray(selectedModelEntry)) {
  if (canWriteAgentRuntime) {
    selectedModelEntry.agentRuntime = { id: "steelengine" };
  } else {
...
Remove-Item Env:STEELENGINE_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:STEELENGINE_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
Remove-Item Env:STEELENGINE_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsSteelEngineResolver = String.raw`$portableNode = if ($env:LOCALAPPDATA) { Join-Path $env:LOCALAPPDATA 'Programs\nodejs' } else { $null }
if ($portableNode -and (Test-Path (Join-Path $portableNode 'node.exe'))) {
  $env:PATH = "$portableNode;$env:PATH"
}
function Resolve-SteelEngineCommand {
  if ($script:SteelEngineResolvedCommand) { return $script:SteelEngineResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\steelengine.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\steelengine.ps1'
  }
  foreach ($name in @('steelengine.cmd', 'steelengine.ps1', 'steelengine')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'steelengine.cmd'
    $shimCandidates += Join-Path $npmPrefix 'steelengine.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:SteelEngineResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:SteelEngineResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
  ...
  if ($npmPrefix) {
    $entryCandidates += Join-Path $npmPrefix 'node_modules\steelengine\steelengine.mjs'
  }
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:SteelEngineResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:SteelEngineResolvedCommand
    }
  }
  throw 'steelengine command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-SteelEngine {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $SteelEngineArgs)
  $command = Resolve-SteelEngineCommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @SteelEngineArgs
    } else {
      & $command.Path @SteelEngineArgs
    }
  } finally {
