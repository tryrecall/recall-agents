...
export function windowsModelProviderTimeoutScript(modelId: string): string {
  ...
  return `$providerTimeoutBatchPath = Join-Path ([System.IO.Path]::GetTempPath()) 'recall-provider-timeout.batch.json'
@'
${batchJson}
'@ | Set-Content -Path $providerTimeoutBatchPath -Encoding UTF8
Invoke-Recall config set --batch-file $providerTimeoutBatchPath --strict-json
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:RECALL_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.recall\\recall.json' }
$agentTurnVersionText = Invoke-Recall --version 2>$null | Out-String
$agentTurnRuntimePolicySupported = $false
if ($agentTurnVersionText -match 'Recall\\s+(\\d{4})\\.(\\d{1,2})\\.(\\d{1,2})') {
...
$env:RECALL_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:RECALL_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$env:RECALL_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED = if ($agentTurnRuntimePolicySupported) { '1' } else { '0' }
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'recall-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.RECALL_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.RECALL_PARALLELS_AGENT_CONFIG_PATCH || "{}");
const canWriteAgentRuntime = process.env.RECALL_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED === "1";
...
Remove-Item Env:RECALL_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:RECALL_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
Remove-Item Env:RECALL_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsRecallResolver = String.raw`function Resolve-RecallCommand {
  if ($script:RecallResolvedCommand) { return $script:RecallResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\recall.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\recall.ps1'
  }
  foreach ($name in @('recall.cmd', 'recall.ps1', 'recall')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'recall.cmd'
    $shimCandidates += Join-Path $npmPrefix 'recall.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:RecallResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:RecallResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
    $entryCandidates += Join-Path $env:APPDATA 'npm\node_modules\recall\recall.mjs'
  }
  if ($npmPrefix) {
    $entryCandidates += Join-Path $npmPrefix 'node_modules\recall\recall.mjs'
  }
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:RecallResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:RecallResolvedCommand
    }
  }
  throw 'recall command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-Recall {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $RecallArgs)
  $command = Resolve-RecallCommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @RecallArgs
    } else {
      & $command.Path @RecallArgs
    }
  } finally {
