...
IMAGE_NAME="$(docker_e2e_resolve_image "recall-release-upgrade-user-journey-e2e" RECALL_RELEASE_UPGRADE_USER_JOURNEY_E2E_IMAGE)"
SKIP_BUILD="${RECALL_RELEASE_UPGRADE_USER_JOURNEY_E2E_SKIP_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz release-upgrade-user-journey "${RECALL_CURRENT_PACKAGE_TGZ:-}")"
...
RECALL_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 release-upgrade-user-journey empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e "RECALL_RELEASE_UPGRADE_BASELINE_SPEC=${RECALL_RELEASE_UPGRADE_BASELINE_SPEC:-recall@latest}" \
