#!/usr/bin/env bash
# Installs Recall from a prepared package tarball, installs @recall/codex
...
CANDIDATE_ROOT="${RECALL_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"
...
IMAGE_NAME="$(docker_e2e_resolve_image "recall-codex-npm-plugin-live-e2e" RECALL_CODEX_NPM_PLUGIN_E2E_IMAGE)"
DOCKER_TARGET="${RECALL_CODEX_NPM_PLUGIN_DOCKER_TARGET:-bare}"
HOST_BUILD="${RECALL_CODEX_NPM_PLUGIN_HOST_BUILD:-1}"
PACKAGE_TGZ="${RECALL_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${RECALL_CODEX_NPM_PLUGIN_PROFILE_FILE:-${RECALL_TESTBOX_PROFILE_FILE:-$HOME/.recall-testbox-live.profile}}"
CODEX_PLUGIN_SPEC="${RECALL_CODEX_NPM_PLUGIN_SPEC:-}"
CODEX_PLUGIN_MOUNT=()
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${RECALL_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "RECALL_CODEX_NPM_PLUGIN_HOST_BUILD=0 requires RECALL_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
prepare_codex_plugin_spec() {
  ...
  if [ -z "$CODEX_PLUGIN_SPEC" ]; then
    CODEX_PLUGIN_PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/recall-codex-plugin-pack.XXXXXX")"
    (
      cd "$CANDIDATE_ROOT"
    ...
    ) >/tmp/recall-codex-plugin-pack.log 2>&1
    pack_output=()
    ...
    if [ "${#pack_output[@]}" -ne 1 ]; then
      ...
      cat /tmp/recall-codex-plugin-pack.log >&2 || true
      exit 1
    fi
...
RECALL_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 codex-npm-plugin-live empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e RECALL_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS="${RECALL_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS:-0}" \
  -e RECALL_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL="${RECALL_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-1}" \
  -e RECALL_CODEX_NPM_PLUGIN_MODEL="${RECALL_CODEX_NPM_PLUGIN_MODEL:-codex/gpt-5.4}" \
  -e RECALL_CODEX_NPM_PLUGIN_SPEC="$CODEX_PLUGIN_SPEC" \
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
...
CODEX_PLUGIN_SPEC="${RECALL_CODEX_NPM_PLUGIN_SPEC:?missing RECALL_CODEX_NPM_PLUGIN_SPEC}"
MODEL_REF="${RECALL_CODEX_NPM_PLUGIN_MODEL:?missing RECALL_CODEX_NPM_PLUGIN_MODEL}"
SESSION_ID="codex-npm-plugin-live"
SUCCESS_MARKER="OPENCLAW-CODEX-NPM-PLUGIN-LIVE-OK"
PLUGIN_INSTALL_FLAGS=(--force)
if [ "${RECALL_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-0}" = "1" ]; then
...
dump_debug_logs() {
  ...
  openclaw_e2e_dump_logs \
    ...
    /tmp/recall-codex-plugin-install.log \
    /tmp/recall-codex-plugin-enable.log \
    /tmp/recall-codex-plugins-list.json \
    /tmp/recall-codex-plugin-inspect.json \
    /tmp/recall-codex-preflight.log \
    /tmp/recall-codex-agent.json \
    /tmp/recall-codex-agent.err \
    /tmp/recall-codex-agent-turn1.json \
    /tmp/recall-codex-agent-turn1.err \
    /tmp/recall-codex-agent-turn2.json \
    /tmp/recall-codex-agent-turn2.err \
    /tmp/recall-codex-plugin-uninstall.log \
    /tmp/recall-codex-plugins-list-after-uninstall.json \
    /tmp/recall-codex-agent-after-uninstall.json \
    /tmp/recall-codex-agent-after-uninstall.err
}
...
recall plugins install "$CODEX_PLUGIN_SPEC" "${PLUGIN_INSTALL_FLAGS[@]}" >/tmp/recall-codex-plugin-install.log 2>&1

node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs configure "$MODEL_REF"

echo "Enabling Codex plugin..."
recall plugins enable codex >/tmp/recall-codex-plugin-enable.log 2>&1

recall plugins list --json >/tmp/recall-codex-plugins-list.json
recall plugins inspect codex --runtime --json >/tmp/recall-codex-plugin-inspect.json
...
"$CODEX_BIN" exec \
  ...
  --color never \
  --skip-git-repo-check \
  "Reply exactly: ${SUCCESS_MARKER}-PREFLIGHT" >/tmp/recall-codex-preflight.log 2>&1 </dev/null
...
echo "Running Recall local agent turns through npm-installed Codex plugin..."
run_agent_turn \
  "turn1" \
  "${SUCCESS_MARKER}-TURN-1" \
  "Reply in one short sentence. Include token ${SUCCESS_MARKER}-TURN-1 and say hello from the Recall Codex plugin." \
  /tmp/recall-codex-agent-turn1.json \
  /tmp/recall-codex-agent-turn1.err
run_agent_turn \
  "turn2" \
...
recall plugins uninstall codex --force >/tmp/recall-codex-plugin-uninstall.log 2>&1
recall plugins list --json >/tmp/recall-codex-plugins-list-after-uninstall.json
node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs assert-uninstalled

if recall agent --local \
  --agent main \
  ...
  --thinking low \
  --timeout 120 \
  --json >/tmp/recall-codex-agent-after-uninstall.json 2>/tmp/recall-codex-agent-after-uninstall.err; then
  echo "Expected Recall agent to fail after Codex uninstall, got status 0" >&2
  exit 1
fi
if ! grep -Fq 'Requested agent harness "codex" is not registered' /tmp/recall-codex-agent-after-uninstall.err &&
  ! grep -Fq 'Unknown model: codex/' /tmp/recall-codex-agent-after-uninstall.err; then
  echo "Unexpected post-uninstall agent error:" >&2
  cat /tmp/recall-codex-agent-after-uninstall.err >&2 || true
  exit 1
fi
