#!/usr/bin/env bash
# Installs SteelEngine from a prepared package tarball, installs @steelengine/codex
...
CANDIDATE_ROOT="${STEELENGINE_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"
...
IMAGE_NAME="$(docker_e2e_resolve_image "steelengine-codex-npm-plugin-live-e2e" STEELENGINE_CODEX_NPM_PLUGIN_E2E_IMAGE)"
DOCKER_TARGET="${STEELENGINE_CODEX_NPM_PLUGIN_DOCKER_TARGET:-bare}"
HOST_BUILD="${STEELENGINE_CODEX_NPM_PLUGIN_HOST_BUILD:-1}"
PACKAGE_TGZ="${STEELENGINE_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${STEELENGINE_CODEX_NPM_PLUGIN_PROFILE_FILE:-${STEELENGINE_TESTBOX_PROFILE_FILE:-$HOME/.steelengine-testbox-live.profile}}"
CODEX_PLUGIN_SPEC="${STEELENGINE_CODEX_NPM_PLUGIN_SPEC:-}"
CODEX_PLUGIN_MOUNT=()
CODEX_PLUGIN_PACK_DIR=""
ASSERT_MAX_TEXT_FILE_BYTES="$(
  docker_e2e_read_positive_int_env STEELENGINE_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES 1048576
)"
ASSERT_MAX_ERROR_TAIL_BYTES="$(
  docker_e2e_read_positive_int_env STEELENGINE_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES 65536
)"
ASSERT_MAX_TRANSCRIPT_FILES="$(
  docker_e2e_read_positive_int_env STEELENGINE_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES 64
)"
ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES="$(
  docker_e2e_read_positive_int_env STEELENGINE_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES 4096
)"
ASSERT_MAX_TRANSCRIPT_SCAN_BYTES="$(
  docker_e2e_read_positive_int_env STEELENGINE_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES 2097152
)"
AGENT_TURN_TIMEOUT_SECONDS="$(
  docker_e2e_read_positive_int_env STEELENGINE_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS 420
)"
SESSION_STORE_CONTRACT="${STEELENGINE_CODEX_NPM_PLUGIN_SESSION_STORE_CONTRACT:-}"
...
BINDING_STORE_CONTRACT="${STEELENGINE_CODEX_NPM_PLUGIN_BINDING_STORE_CONTRACT:-}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${STEELENGINE_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "STEELENGINE_CODEX_NPM_PLUGIN_HOST_BUILD=0 requires STEELENGINE_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
prepare_codex_plugin_spec() {
  ...
  if [ -z "$CODEX_PLUGIN_SPEC" ]; then
    CODEX_PLUGIN_PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/steelengine-codex-plugin-pack.XXXXXX")"
    (
      cd "$CANDIDATE_ROOT"
    ...
    ) >/tmp/steelengine-codex-plugin-pack.log 2>&1
    pack_output=()
    ...
    if [ "${#pack_output[@]}" -ne 1 ]; then
      ...
      docker_e2e_print_log /tmp/steelengine-codex-plugin-pack.log >&2
      exit 1
    fi
...
STEELENGINE_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 codex-npm-plugin-live empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e STEELENGINE_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS="${STEELENGINE_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS:-0}" \
  -e STEELENGINE_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL="${STEELENGINE_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-1}" \
  -e STEELENGINE_CODEX_NPM_PLUGIN_MODEL="${STEELENGINE_CODEX_NPM_PLUGIN_MODEL:-openai/gpt-5.4}" \
  -e STEELENGINE_CODEX_NPM_PLUGIN_SPEC="$CODEX_PLUGIN_SPEC" \
  -e STEELENGINE_CODEX_NPM_PLUGIN_BINDING_STORE_CONTRACT="$BINDING_STORE_CONTRACT" \
  -e STEELENGINE_CODEX_NPM_PLUGIN_SESSION_STORE_CONTRACT="$SESSION_STORE_CONTRACT" \
  -e "STEELENGINE_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES=$ASSERT_MAX_TEXT_FILE_BYTES" \
  -e "STEELENGINE_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES=$ASSERT_MAX_ERROR_TAIL_BYTES" \
  -e "STEELENGINE_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES=$ASSERT_MAX_TRANSCRIPT_FILES" \
  -e "STEELENGINE_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES=$ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES" \
  -e "STEELENGINE_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES=$ASSERT_MAX_TRANSCRIPT_SCAN_BYTES" \
  -e "STEELENGINE_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
  -e "STEELENGINE_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT" \
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
...
CODEX_PLUGIN_SPEC="${STEELENGINE_CODEX_NPM_PLUGIN_SPEC:?missing STEELENGINE_CODEX_NPM_PLUGIN_SPEC}"
MODEL_REF="${STEELENGINE_CODEX_NPM_PLUGIN_MODEL:?missing STEELENGINE_CODEX_NPM_PLUGIN_MODEL}"
POST_UNINSTALL_MODEL_REF="$MODEL_REF"
SESSION_ID="codex-npm-plugin-live"
SUCCESS_MARKER="STEELENGINE-CODEX-NPM-PLUGIN-LIVE-OK"
AGENT_TURN_TIMEOUT_SECONDS="${STEELENGINE_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS:-420}"
PLUGIN_INSTALL_FLAGS=(--force)
if [ "${STEELENGINE_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-0}" = "1" ]; then
...
dump_debug_logs() {
  ...
  steelengine_e2e_dump_logs \
    ...
    /tmp/steelengine-codex-plugin-install.log \
    /tmp/steelengine-codex-plugin-enable.log \
    /tmp/steelengine-codex-plugins-list.json \
    /tmp/steelengine-codex-plugin-inspect.json \
    /tmp/steelengine-codex-preflight.log \
    /tmp/steelengine-codex-agent.json \
    /tmp/steelengine-codex-agent.err \
    /tmp/steelengine-codex-agent-turn1.json \
    /tmp/steelengine-codex-agent-turn1.err \
    /tmp/steelengine-codex-agent-turn2.json \
    /tmp/steelengine-codex-agent-turn2.err \
    /tmp/steelengine-codex-followthrough.json \
    /tmp/steelengine-codex-followthrough.log \
    /tmp/steelengine-codex-followthrough.err \
    /tmp/steelengine-codex-plugin-uninstall.log \
    /tmp/steelengine-codex-plugins-list-after-uninstall.json \
...
steelengine plugins install "$CODEX_PLUGIN_SPEC" "${PLUGIN_INSTALL_FLAGS[@]}" >/tmp/steelengine-codex-plugin-install.log 2>&1

node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs configure "$MODEL_REF"

echo "Enabling Codex plugin..."
steelengine plugins enable codex >/tmp/steelengine-codex-plugin-enable.log 2>&1

steelengine plugins list --json >/tmp/steelengine-codex-plugins-list.json
steelengine plugins inspect codex --runtime --json >/tmp/steelengine-codex-plugin-inspect.json
...
"$CODEX_BIN" exec \
  ...
  --color never \
  --skip-git-repo-check \
  "Reply exactly: ${SUCCESS_MARKER}-PREFLIGHT" >/tmp/steelengine-codex-preflight.log 2>&1 </dev/null
...
echo "Running SteelEngine local agent turns through npm-installed Codex plugin..."
run_agent_turn \
  "turn1" \
  "${SUCCESS_MARKER}-TURN-1" \
  "Reply in one short sentence. Include token ${SUCCESS_MARKER}-TURN-1 and say hello from the SteelEngine Codex plugin." \
  /tmp/steelengine-codex-agent-turn1.json \
  /tmp/steelengine-codex-agent-turn1.err
run_agent_turn \
  "turn2" \
...
FOLLOWTHROUGH_WORKSPACE="${STEELENGINE_STATE_DIR:?missing STEELENGINE_STATE_DIR}/workspace"
...
STEELENGINE_PACKAGE_ROOT="$(steelengine_e2e_package_root "$NPM_CONFIG_PREFIX")"
if node scripts/e2e/lib/codex-npm-plugin-live/followthrough-turn.mjs \
  ...
  >/tmp/steelengine-codex-followthrough.log \
  2>/tmp/steelengine-codex-followthrough.err </dev/null; then
  followthrough_status=0
else
  followthrough_status=$?
fi
echo "followthrough_agent_status: $followthrough_status stdout_bytes=$(wc -c </tmp/steelengine-codex-followthrough.json 2>/dev/null || printf 0) stderr_bytes=$(wc -c </tmp/steelengine-codex-followthrough.err 2>/dev/null || printf 0)"
...
steelengine plugins uninstall codex --force >/tmp/steelengine-codex-plugin-uninstall.log 2>&1
steelengine plugins list --json >/tmp/steelengine-codex-plugins-list-after-uninstall.json
...
if steelengine agent --local \
  ...
  --thinking low \
  --timeout 120 \
  --json >/tmp/steelengine-codex-agent-after-uninstall.json 2>/tmp/steelengine-codex-agent-after-uninstall.err; then
