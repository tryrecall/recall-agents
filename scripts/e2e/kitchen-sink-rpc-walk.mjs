...
const PLUGIN_SPEC =
  process.env.STEELENGINE_KITCHEN_SINK_NPM_SPEC || "npm:@steelengine/kitchen-sink@latest";
const PLUGIN_ID = process.env.STEELENGINE_KITCHEN_SINK_PLUGIN_ID || "steelengine-kitchen-sink-fixture";
...
Runs the external Kitchen Sink plugin RPC walk against a built SteelEngine entry.

Environment:
  STEELENGINE_ENTRY                         Built SteelEngine entrypoint. Defaults to dist/index.mjs or dist/index.js.
  STEELENGINE_KITCHEN_SINK_NPM_SPEC         Plugin package spec. Default: npm:@steelengine/kitchen-sink@latest.
  STEELENGINE_KITCHEN_SINK_PLUGIN_ID        Plugin id. Default: steelengine-kitchen-sink-fixture.
  STEELENGINE_KITCHEN_SINK_PERSONALITY      Plugin fixture personality. Default: conformance.
  STEELENGINE_KITCHEN_SINK_RPC_PORT         Gateway loopback port. Default: OS-selected free port.
  STEELENGINE_KITCHEN_SINK_RPC_READY_MS     Gateway readiness timeout.
  STEELENGINE_KITCHEN_SINK_RPC_COMMAND_MS   SteelEngine command timeout.
  STEELENGINE_KITCHEN_SINK_RPC_INSTALL_MS   Plugin install timeout.
  STEELENGINE_KITCHEN_SINK_RPC_CALL_MS      RPC call timeout.
  STEELENGINE_KITCHEN_SINK_RPC_FETCH_MS     HTTP readiness probe timeout.
  STEELENGINE_KITCHEN_SINK_RPC_FETCH_BODY_BYTES  HTTP readiness probe response ceiling.
  STEELENGINE_KITCHEN_SINK_MAX_RSS_MIB      Gateway RSS ceiling.
  STEELENGINE_KITCHEN_SINK_COMMAND_MAX_RSS_MIB  Install/CLI command RSS ceiling.
  STEELENGINE_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS  Per-command stdout/stderr capture ceiling.
  STEELENGINE_KITCHEN_SINK_KEEP_TMP=1       Preserve the isolated temp home.
`;
}
...
export function resolveKitchenSinkRpcConfig(env = process.env) {
  const commandTimeoutMs = readPositiveTimerMs(
    env.STEELENGINE_KITCHEN_SINK_RPC_COMMAND_MS,
    DEFAULT_COMMAND_TIMEOUT_MS,
    "STEELENGINE_KITCHEN_SINK_RPC_COMMAND_MS",
  );
  return {
    commandMaxRssMiB: readPositiveInt(
      env.STEELENGINE_KITCHEN_SINK_COMMAND_MAX_RSS_MIB,
      DEFAULT_MAX_COMMAND_RSS_MIB,
      "STEELENGINE_KITCHEN_SINK_COMMAND_MAX_RSS_MIB",
    ),
    commandTimeoutMs,
    fetchBodyMaxBytes: readPositiveInt(
      env.STEELENGINE_KITCHEN_SINK_RPC_FETCH_BODY_BYTES,
      DEFAULT_FETCH_BODY_MAX_BYTES,
      "STEELENGINE_KITCHEN_SINK_RPC_FETCH_BODY_BYTES",
    ),
    fetchTimeoutMs: readPositiveTimerMs(
      env.STEELENGINE_KITCHEN_SINK_RPC_FETCH_MS,
      DEFAULT_FETCH_TIMEOUT_MS,
      "STEELENGINE_KITCHEN_SINK_RPC_FETCH_MS",
    ),
    installTimeoutMs: readPositiveTimerMs(
      env.STEELENGINE_KITCHEN_SINK_RPC_INSTALL_MS,
      Math.max(commandTimeoutMs, DEFAULT_INSTALL_TIMEOUT_MS),
      "STEELENGINE_KITCHEN_SINK_RPC_INSTALL_MS",
    ),
    maxRssMiB: readPositiveInt(
      env.STEELENGINE_KITCHEN_SINK_MAX_RSS_MIB,
      DEFAULT_MAX_RSS_MIB,
      "STEELENGINE_KITCHEN_SINK_MAX_RSS_MIB",
    ),
    outputCaptureChars: readPositiveInt(
      env.STEELENGINE_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS,
      DEFAULT_OUTPUT_CAPTURE_CHARS,
      "STEELENGINE_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS",
    ),
    readyTimeoutMs: readPositiveTimerMs(
      env.STEELENGINE_KITCHEN_SINK_RPC_READY_MS,
      DEFAULT_READY_TIMEOUT_MS,
      "STEELENGINE_KITCHEN_SINK_RPC_READY_MS",
    ),
    rpcTimeoutMs: readPositiveTimerMs(
      env.STEELENGINE_KITCHEN_SINK_RPC_CALL_MS,
      DEFAULT_RPC_TIMEOUT_MS,
      "STEELENGINE_KITCHEN_SINK_RPC_CALL_MS",
    ),
  };
}
...
export async function resolveKitchenSinkRpcPort(env = process.env, options = {}) {
  const rawPort = (env.STEELENGINE_KITCHEN_SINK_RPC_PORT || "").trim();
  if (rawPort) {
    const port = readPositiveInt(rawPort, 0, "STEELENGINE_KITCHEN_SINK_RPC_PORT");
    if (port > 65535) {
      throw new Error(
        `STEELENGINE_KITCHEN_SINK_RPC_PORT must be a TCP port from 1 to 65535. Got: ${JSON.stringify(rawPort)}`,
      );
    }
    return port;
  }
...
export function makeEnv() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-kitchen-sink-rpc-"));
  ...
  return {
    ...
    env: {
      ...
      STEELENGINE_KITCHEN_SINK_PERSONALITY:
        process.env.STEELENGINE_KITCHEN_SINK_PERSONALITY || "conformance",
    },
  };
}
...
function resolveCommandParentSignalKillGraceMs(env) {
  const raw = env.VITEST && env.STEELENGINE_TEST_KITCHEN_SINK_PARENT_SIGNAL_KILL_GRACE_MS;
  if (!raw) {
...
async function runSteelEngine(runner, args, env, options = {}) {
  const config = resolveKitchenSinkRpcConfig(env);
  const command = await resolveSteelEngineCommand(runner, args, env, {
    stdio: ["ignore", "pipe", "pipe"],
  });
...
async function resolveSteelEngineCommand(runner, args, env, options = {}) {
  if (runner.pnpm) {
...
async function rpcCall(method, params, options) {
  ...
  const payload = module
    ? await module.callGateway({
        config: readJson(options.env.STEELENGINE_CONFIG_PATH),
        configPath: options.env.STEELENGINE_CONFIG_PATH,
...
async function loadCallGatewayModule(runner) {
  if (!usesBuiltSteelEngineEntry(runner)) {
    return null;
  }
...
export function usesBuiltSteelEngineEntry(runner, cwd = process.cwd(), env = process.env) {
  if (runner?.pnpm || !runner?.baseArgs?.[0]) {
    return false;
  }
  const entry = runner.baseArgs[0];
  if (env.STEELENGINE_ENTRY && entry === env.STEELENGINE_ENTRY) {
    return true;
  }
    ...
    entries: {
      ...
      [PLUGIN_ID]: {
        ...
        config: {
          ...
          personality: env.STEELENGINE_KITCHEN_SINK_PERSONALITY,
        },
        hooks: {
...
async function samplePosixProcessTree(pid, run, commandLineNeedles) {
  ...
  try {
    ...
    const gatewayTitleMatches = descendants.filter((row) =>
      row.command.toLowerCase().includes("steelengine-gateway"),
    );
...
export async function main() {
  const config = resolveKitchenSinkRpcConfig();
  let runner = resolveSteelEngineRunner();
  const port = await resolveKitchenSinkRpcPort();
  const { root, env } = makeEnv();
  const logPath = path.join(root, "gateway.log");
  const keepTmp = process.env.STEELENGINE_KITCHEN_SINK_KEEP_TMP === "1";
  let failed = false;
  let child;
  ...
  try {
    ...
    const inspect = parseJsonOutput(
      (
        await runSteelEngine(runner, ["plugins", "inspect", PLUGIN_ID, "--runtime", "--json"], env, {
          ...commandResourceOptions,
    ...
    assertKitchenSinkUiDescriptors(uiDescriptors, {
      expectDescriptor: env.STEELENGINE_KITCHEN_SINK_PERSONALITY !== "conformance",
    });
