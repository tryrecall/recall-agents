...
ALLOW_ENV="STEELENGINE_QA_ALLOW_UPDATE_RUN_SELF"
...
IMAGE_NAME="$(
  docker_e2e_resolve_image \
    "steelengine-update-run-package-self-upgrade-e2e" \
    STEELENGINE_UPDATE_RUN_SELF_UPGRADE_E2E_IMAGE
)"
SKIP_BUILD="${STEELENGINE_UPDATE_RUN_SELF_UPGRADE_E2E_SKIP_BUILD:-0}"
DOCKER_RUN_TIMEOUT="${STEELENGINE_UPDATE_RUN_SELF_UPGRADE_DOCKER_RUN_TIMEOUT:-1800s}"
ARTIFACT_DIR="${STEELENGINE_UPDATE_RUN_SELF_UPGRADE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/update-run-package-self-upgrade}"
QA_CHANNEL_FIXTURE_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/steelengine-update-run-qa-channel.XXXXXX")"
...
prepare_qa_channel_fixture() {
    ...
    node -e '
      ...
      fs.writeFileSync(
        ...
        `${JSON.stringify({
          ...
          buildCommand: "STEELENGINE_BUILD_PRIVATE_QA=1 corepack pnpm build:docker",
        }, null, 2)}\n`,
      );
  ...
  (
    ...
    COREPACK_ENABLE_DOWNLOAD_PROMPT=0 STEELENGINE_BUILD_PRIVATE_QA=1 corepack pnpm build:docker
  ) >>"$ARTIFACT_DIR/historical-qa-channel-build.log" 2>&1

  local compiled_plugin="$checkout_root/dist/extensions/qa-channel"
  for required_file in package.json steelengine.plugin.json index.js setup-entry.js; do
...
docker_e2e_run_with_harness \
  ...
  -e STEELENGINE_QA_ALLOW_UPDATE_RUN_SELF=1 \
  -e STEELENGINE_UPDATE_RUN_SELF_UPGRADE_ARTIFACT_DIR=/tmp/steelengine-update-run-artifacts \
  -e STEELENGINE_UPDATE_RUN_SELF_UPGRADE_SOURCE_VERSION="$SOURCE_VERSION" \
  -v "$ARTIFACT_DIR:/tmp/steelengine-update-run-artifacts" \
  -v "$QA_CHANNEL_FIXTURE_ROOT/checkout:/tmp/steelengine-update-run-build:ro" \
  "$IMAGE_NAME" \
