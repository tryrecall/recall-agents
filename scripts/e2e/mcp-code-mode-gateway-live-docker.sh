...
IMAGE_NAME="$(docker_e2e_resolve_image "steelengine-mcp-code-mode-gateway-live-e2e" STEELENGINE_IMAGE)"
PORT="$(docker_e2e_read_tcp_port_env STEELENGINE_MCP_CODE_MODE_LIVE_GATEWAY_PORT 18789)"
...
CONTAINER_NAME="steelengine-mcp-code-mode-live-e2e-$$"
PROFILE_FILE="${STEELENGINE_MCP_CODE_MODE_LIVE_PROFILE_FILE:-${STEELENGINE_TESTBOX_PROFILE_FILE:-$HOME/.steelengine-testbox-live.profile}}"

CLIENT_LOG="$(mktemp -t steelengine-mcp-code-mode-live-log.XXXXXX)"
...
STEELENGINE_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-code-mode-gateway-live empty)"

# The profile is only a credential source. Keep this lane's SteelEngine runtime
# isolated from host/testbox mode flags that can change packaged behavior.
unset STEELENGINE_TESTBOX
...
docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e "STEELENGINE_DOCKER_OPENAI_BASE_URL=${OPENAI_BASE_URL:-https://api.openai.com/v1}" \
  ...
  -e "GW_TOKEN=$TOKEN" \
  -e "STEELENGINE_MCP_CODE_MODE_CLIENT_TIMEOUT_MS=$CLIENT_TIMEOUT_MS" \
  -e "STEELENGINE_MCP_CODE_MODE_CLIENT_BODY_MAX_BYTES=$CLIENT_BODY_MAX_BYTES" \
  -e "STEELENGINE_ALLOW_INSECURE_PRIVATE_WS=1" \
  -e "STEELENGINE_MCP_CODE_MODE_MODEL=${STEELENGINE_MCP_CODE_MODE_LIVE_MODEL:-steelengine/main}" \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
  bash -lc "set -euo pipefail
    ...
    gateway_pid=\"\$(steelengine_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-code-mode-live-gateway.log)\"
    steelengine_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-code-mode-live-gateway.log 480 $PORT
