...
const maxBodyBytes = readPositiveIntEnv("STEELENGINE_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES", 1048576);
...
if (!Number.isFinite(timeoutSeconds) || timeoutSeconds <= 0) {
  throw new Error(`invalid STEELENGINE_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: ${timeoutSeconds}`);
}
if (!Number.isFinite(maxBodyBytes) || maxBodyBytes <= 0) {
  throw new Error(`invalid STEELENGINE_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES: ${maxBodyBytes}`);
}
  ...
  response = await Promise.race([
    fetch(`http://127.0.0.1:${port}/v1/chat/completions`, {
      ...
      headers: {
        ...
        "x-steelengine-model": backendModel,
      },
      body: JSON.stringify({
        model: "steelengine",
        stream: false,
