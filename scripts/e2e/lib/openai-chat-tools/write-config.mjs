...
const configPath = requireEnv("STEELENGINE_CONFIG_PATH");
const stateDir = requireEnv("STEELENGINE_STATE_DIR");
const workspaceDir = requireEnv("STEELENGINE_TEST_WORKSPACE_DIR");
const modelRef = requireEnv("STEELENGINE_OPENAI_CHAT_TOOLS_MODEL");
const token = requireEnv("STEELENGINE_GATEWAY_TOKEN");
const timeoutSeconds = readPositiveIntEnv("STEELENGINE_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS", 180);
const gatewayPort = readTcpPortEnv("PORT", 18789);
const [providerId, modelId] = modelRef.split("/");
if (providerId !== "openai" || !modelId) {
  throw new Error(`STEELENGINE_OPENAI_CHAT_TOOLS_MODEL must be openai/*, got ${modelRef}`);
}
