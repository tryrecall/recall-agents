...
function writeConfig(kind) {
  const configPath = requireArg(process.env.STEELENGINE_CONFIG_PATH, "STEELENGINE_CONFIG_PATH");
      ...
      : kind === "browser-cdp"
        ? {
            gateway: {
              ...
              auth: {
                mode: "token",
                token: requireArg(process.env.STEELENGINE_GATEWAY_TOKEN, "STEELENGINE_GATEWAY_TOKEN"),
              },
              controlUi: { enabled: false },
            },
            browser: {
...
function writeOpenAiWebSearchMinimalConfig() {
  writeJson(path.join(process.env.STEELENGINE_STATE_DIR, "steelengine.json"), {
    agents: {
      defaults: {
    ...
    gateway: { auth: { mode: "token", token: process.env.STEELENGINE_GATEWAY_TOKEN } },
  });
}

function writeOpenWebUiConfig([openaiApiKey]) {
  const batchPath = requireArg(
    process.env.STEELENGINE_CONFIG_BATCH_PATH,
    "STEELENGINE_CONFIG_BATCH_PATH",
  );
  writeJson(batchPath, [
    ...
    { path: "models.providers.openai.models", value: [] },
    {
      ...
      value: readPositiveIntEnv("STEELENGINE_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS", 900),
    },
    { path: "models.providers.openai.agentRuntime", value: { id: "steelengine" } },
    { path: "gateway.controlUi.enabled", value: false },
    { path: "gateway.mode", value: "local" },
    { path: "gateway.bind", value: "lan" },
    { path: "gateway.auth.mode", value: "token" },
    { path: "gateway.auth.token", value: process.env.STEELENGINE_GATEWAY_TOKEN },
    { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
    { path: "agents.defaults.model.primary", value: process.env.STEELENGINE_OPENWEBUI_MODEL },
  ]);
}
