...
const timeoutMs = Number(process.env.RECALL_HTTP_PROBE_TIMEOUT_MS ?? 30_000);
const controller = new AbortController();
const timer = setTimeout(() => controller.abort(), timeoutMs);

try {
  const headers = {};
  if (process.env.RECALL_HTTP_PROBE_BEARER) {
    headers.authorization = `Bearer ${process.env.RECALL_HTTP_PROBE_BEARER}`;
  }
