...
STEELENGINE_TEST_STATE_SCRIPT_B64="${STEELENGINE_TEST_STATE_SCRIPT_B64:-}"
steelengine_skill_install_owns_home=0
cleanup_clawhub_skill_install_home() {
  if [ "$steelengine_skill_install_owns_home" = "1" ] && [ -n "${HOME:-}" ]; then
    rm -rf "$HOME"
  fi
}
trap cleanup_clawhub_skill_install_home EXIT

if [ -n "$STEELENGINE_TEST_STATE_SCRIPT_B64" ]; then
  steelengine_e2e_eval_test_state_from_b64 "$STEELENGINE_TEST_STATE_SCRIPT_B64"
else
  export HOME="$(mktemp -d "${TMPDIR:-/tmp}/steelengine-skill-install-home.XXXXXX")"
  steelengine_skill_install_owns_home=1
  export USERPROFILE="$HOME"
  export STEELENGINE_HOME="$HOME"
...
if [ -n "${STEELENGINE_CURRENT_PACKAGE_TGZ:-}" ]; then
  ...
  steelengine_e2e_install_package /tmp/steelengine-skill-install-npm.log
fi

if [ -n "${STEELENGINE_CURRENT_PACKAGE_TGZ:-}" ] && command -v steelengine >/dev/null 2>&1; then
  STEELENGINE_CMD=(steelengine)
elif command -v pnpm >/dev/null 2>&1 && [ -f package.json ]; then
  if [ "${STEELENGINE_SKILL_INSTALL_E2E_BUILD_SOURCE:-0}" = "1" ]; then
    pnpm build >/tmp/steelengine-skill-install-build.log 2>&1
  fi
  STEELENGINE_CMD=(pnpm --silent steelengine)
elif command -v steelengine >/dev/null 2>&1; then
  STEELENGINE_CMD=(steelengine)
else
  echo "steelengine command not found; install package first or run from repo with pnpm" >&2
  exit 1
fi

mkdir -p "$(dirname "$STEELENGINE_CONFIG_PATH")"
node --input-type=module - "$STEELENGINE_CONFIG_PATH" <<'NODE'
...
query="${STEELENGINE_SKILL_INSTALL_E2E_QUERY:-homeassistant}"
requested_slug="${STEELENGINE_SKILL_INSTALL_E2E_SLUG:-}"
preferred_slug="${STEELENGINE_SKILL_INSTALL_E2E_PREFERRED_SLUG:-homeassistant-skill}"
search_json="/tmp/steelengine-skill-install-search.json"
resolve_json="/tmp/steelengine-skill-install-resolved.json"
install_log="/tmp/steelengine-skill-install.log"
info_json="/tmp/steelengine-skill-install-info.json"

echo "Searching live ClawHub skills for: $query"
"${STEELENGINE_CMD[@]}" skills search "$query" --limit 8 --json >"$search_json"
...
if ! "${STEELENGINE_CMD[@]}" skills install "$slug" --force >"$install_log" 2>&1; then
  echo "Skill install failed" >&2
  steelengine_e2e_dump_logs /tmp/steelengine-skill-install-npm.log "$search_json" "$resolve_json" "$install_log"
  exit 1
fi

workspace_dir="$HOME/.steelengine/workspace"
skill_dir="$workspace_dir/skills/$slug"
origin_json="$skill_dir/.clawhub/origin.json"
lock_json="$workspace_dir/.clawhub/lock.json"

steelengine_e2e_assert_file "$skill_dir/SKILL.md"
steelengine_e2e_assert_file "$origin_json"
steelengine_e2e_assert_file "$lock_json"

"${STEELENGINE_CMD[@]}" skills info "$slug" --json >"$info_json"
