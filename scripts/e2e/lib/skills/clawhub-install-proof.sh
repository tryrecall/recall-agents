...
source "$ROOT_DIR/scripts/lib/recall-e2e-instance.sh"

RECALL_TEST_STATE_SCRIPT_B64="${RECALL_TEST_STATE_SCRIPT_B64:-}"
if [ -n "$RECALL_TEST_STATE_SCRIPT_B64" ]; then
  openclaw_e2e_eval_test_state_from_b64 "$RECALL_TEST_STATE_SCRIPT_B64"
else
  export HOME="$(mktemp -d "${TMPDIR:-/tmp}/recall-skill-install-home.XXXXXX")"
  export USERPROFILE="$HOME"
  export RECALL_HOME="$HOME"
  export RECALL_STATE_DIR="$HOME/.recall"
...
if [ -n "${RECALL_CURRENT_PACKAGE_TGZ:-}" ]; then
  ...
  openclaw_e2e_install_package /tmp/recall-skill-install-npm.log
fi

if [ -n "${RECALL_CURRENT_PACKAGE_TGZ:-}" ] && command -v recall >/dev/null 2>&1; then
  RECALL_CMD=(recall)
elif command -v pnpm >/dev/null 2>&1 && [ -f package.json ]; then
  if [ "${RECALL_SKILL_INSTALL_E2E_BUILD_SOURCE:-0}" = "1" ]; then
    pnpm build >/tmp/recall-skill-install-build.log 2>&1
  fi
  RECALL_CMD=(pnpm --silent recall)
elif command -v recall >/dev/null 2>&1; then
  RECALL_CMD=(recall)
else
  echo "recall command not found; install package first or run from repo with pnpm" >&2
  exit 1
fi

mkdir -p "$(dirname "$RECALL_CONFIG_PATH")"
node --input-type=module - "$RECALL_CONFIG_PATH" <<'NODE'
...
query="${RECALL_SKILL_INSTALL_E2E_QUERY:-homeassistant}"
requested_slug="${RECALL_SKILL_INSTALL_E2E_SLUG:-}"
preferred_slug="${RECALL_SKILL_INSTALL_E2E_PREFERRED_SLUG:-homeassistant-skill}"
search_json="/tmp/recall-skill-install-search.json"
resolve_json="/tmp/recall-skill-install-resolved.json"
install_log="/tmp/recall-skill-install.log"
info_json="/tmp/recall-skill-install-info.json"

echo "Searching live ClawHub skills for: $query"
"${RECALL_CMD[@]}" skills search "$query" --limit 8 --json >"$search_json"
...
if ! "${RECALL_CMD[@]}" skills install "$slug" --force >"$install_log" 2>&1; then
  echo "Skill install failed" >&2
  openclaw_e2e_dump_logs /tmp/recall-skill-install-npm.log "$search_json" "$resolve_json" "$install_log"
  exit 1
fi

workspace_dir="$HOME/.recall/workspace"
...
"${RECALL_CMD[@]}" skills info "$slug" --json >"$info_json"

node --input-type=module - "$RECALL_CONFIG_PATH" "$skill_dir" "$origin_json" "$lock_json" "$info_json" "$slug" <<'NODE'
import fs from "node:fs";
