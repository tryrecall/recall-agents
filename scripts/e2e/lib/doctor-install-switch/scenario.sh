...
export PATH="/tmp/steelengine-bin:$PATH"
mkdir -p /tmp/steelengine-bin
cp scripts/e2e/lib/doctor-install-switch/shims/systemctl /tmp/steelengine-bin/systemctl
cp scripts/e2e/lib/doctor-install-switch/shims/loginctl /tmp/steelengine-bin/loginctl
chmod +x /tmp/steelengine-bin/systemctl /tmp/steelengine-bin/loginctl
...
(
  cd "$git_root"
  steelengine_e2e_maybe_timeout "${STEELENGINE_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit >/tmp/steelengine-git-install.log 2>&1
  git init -q
  git config user.email "docker-e2e@steelengine.local"
  git config user.name "SteelEngine Docker E2E"
  git add -A --
  git commit -qm "test fixture"
)
npm_log="/tmp/steelengine-doctor-switch-npm-install.log"
if ! steelengine_e2e_maybe_timeout "${STEELENGINE_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$package_tgz" >"$npm_log" 2>&1; then
  steelengine_e2e_print_log "$npm_log"
  exit 1
fi

npm_bin="/tmp/npm-prefix/bin/steelengine"
npm_root="/tmp/npm-prefix/lib/node_modules/steelengine"
...
git_cli="$git_root/steelengine.mjs"

package_version="$(node -p "require(\"$npm_root/package.json\").version")"
update_doctor_env="STEELENGINE_UPDATE_IN_PROGRESS=1"
update_doctor_env+=" STEELENGINE_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE=1"
update_doctor_env+=" STEELENGINE_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART=1"
update_doctor_env+=" STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR=1"
update_doctor_env+=" STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION=0"
...
run_flow() {
  ...
  local install_log="/tmp/steelengine-doctor-switch-${name}-install.log"
  local doctor_log="/tmp/steelengine-doctor-switch-${name}-doctor.log"
  local command_timeout="${STEELENGINE_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"

  echo "== Flow: $name =="
  steelengine_test_state_create "switch-${name}" empty
  export USER="testuser"

  if ! steelengine_e2e_maybe_timeout "$command_timeout" bash -c "$install_cmd" >"$install_log" 2>&1; then
    steelengine_e2e_print_log "$install_log"
    exit 1
  fi
  rm -f "$HOME/.zshrc" "$HOME/.bashrc" "$HOME/.bash_profile"
  rm -rf "$HOME/.config/fish" "$HOME/.config/powershell"

  unit_path="$HOME/.config/systemd/user/steelengine-gateway.service"
  if [ ! -f "$unit_path" ]; then
  ...
  if ! steelengine_e2e_maybe_timeout "$command_timeout" bash -c "$doctor_cmd" >"$doctor_log" 2>&1; then
    steelengine_e2e_print_log "$doctor_log"
    exit 1
  fi
...
run_cross_state_approval_flow() {
  ...
  local automated_log="/tmp/steelengine-doctor-switch-${name}-automated.log"
  local direct_log="/tmp/steelengine-doctor-switch-${name}-direct.log"
  local command_timeout="${STEELENGINE_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  local default_state_dir="$HOME/.steelengine"
  ...
  local state_database="$custom_state_dir/state/steelengine.sqlite"
  ...
  if ! steelengine_e2e_maybe_timeout "$command_timeout" env \
    STEELENGINE_STATE_DIR="$custom_state_dir" \
    STEELENGINE_CONFIG_PATH="$custom_state_dir/steelengine.json" \
    STEELENGINE_UPDATE_IN_PROGRESS=1 \
    "$npm_bin" doctor --repair --yes --non-interactive >"$automated_log" 2>&1; then
    steelengine_e2e_print_log "$automated_log"
    exit 1
  fi
  ...
  if ! steelengine_e2e_maybe_timeout "$command_timeout" env \
    -u STEELENGINE_UPDATE_IN_PROGRESS \
    STEELENGINE_STATE_DIR="$custom_state_dir" \
    STEELENGINE_CONFIG_PATH="$custom_state_dir/steelengine.json" \
    "$npm_bin" doctor --repair --yes --non-interactive >"$direct_log" 2>&1; then
    steelengine_e2e_print_log "$direct_log"
    exit 1
  fi
...
run_proxy_env_flow() {
  ...
  if ! steelengine_e2e_maybe_timeout "$command_timeout" env \
    ...
    STEELENGINE_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART=1 \
    STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR=1 \
    STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION=0 \
...
run_wrapper_flow() {
  ...
  local install_log="/tmp/steelengine-doctor-switch-${name}-install.log"
  local reinstall_log="/tmp/steelengine-doctor-switch-${name}-reinstall.log"
  local env_repair_log="/tmp/steelengine-doctor-switch-${name}-env-repair.log"
  local doctor_log="/tmp/steelengine-doctor-switch-${name}-doctor.log"
  local clear_log="/tmp/steelengine-doctor-switch-${name}-clear.log"
  local command_timeout="${STEELENGINE_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  local wrapper="$HOME/.local/bin/steelengine-wrapper"
  node scripts/e2e/lib/doctor-install-switch/write-wrapper.mjs \
    "$wrapper" \
    "$npm_bin" \
    "$HOME/steelengine-wrapper-argv.log"

  local unit_path="$HOME/.config/systemd/user/steelengine-gateway.service"

  if ! steelengine_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" --force >"$install_log" 2>&1; then
    steelengine_e2e_print_log "$install_log"
    exit 1
  fi
  assert_exec_arg "$unit_path" 1 "$wrapper"
  assert_exec_arg "$unit_path" 2 "gateway"
  assert_env_value "$unit_path" "STEELENGINE_WRAPPER" "$wrapper"

  if ! steelengine_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --force >"$reinstall_log" 2>&1; then
    steelengine_e2e_print_log "$reinstall_log"
    exit 1
  fi
  assert_exec_arg "$unit_path" 1 "$wrapper"
  assert_exec_arg "$unit_path" 2 "gateway"
  ...
  sed -i "/^Environment=STEELENGINE_WRAPPER=/d" "$unit_path"
  if ! steelengine_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
    steelengine_e2e_print_log "$env_repair_log"
    exit 1
  fi
  assert_exec_arg "$unit_path" 1 "$wrapper"
  assert_env_value "$unit_path" "STEELENGINE_WRAPPER" "$wrapper"

  sed -i "s#^Environment=STEELENGINE_WRAPPER=.*#Environment=STEELENGINE_WRAPPER=/tmp/stale-steelengine-wrapper#" "$unit_path"
  if ! steelengine_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
    steelengine_e2e_print_log "$env_repair_log"
    exit 1
  fi
  ...
  if ! steelengine_e2e_maybe_timeout "$command_timeout" node "$git_cli" doctor --repair --force --yes >"$doctor_log" 2>&1; then
    steelengine_e2e_print_log "$doctor_log"
    exit 1
  fi
  if ! grep -Fq "Gateway service invokes STEELENGINE_WRAPPER:" "$doctor_log"; then
  ...
  if ! steelengine_e2e_maybe_timeout "$command_timeout" env STEELENGINE_WRAPPER= "$npm_bin" gateway install --force >"$clear_log" 2>&1; then
    steelengine_e2e_print_log "$clear_log"
    exit 1
  fi
  assert_no_env_key "$unit_path" "STEELENGINE_WRAPPER"
