...
cp scripts/e2e/lib/doctor-install-switch/shims/systemctl /tmp/recall-bin/systemctl
cp scripts/e2e/lib/doctor-install-switch/shims/loginctl /tmp/recall-bin/loginctl
chmod +x /tmp/recall-bin/systemctl /tmp/recall-bin/loginctl

package_tgz="${RECALL_CURRENT_PACKAGE_TGZ:?missing RECALL_CURRENT_PACKAGE_TGZ}"
git_root="/tmp/recall-git"
mkdir -p "$git_root"
...
(
  cd "$git_root"
  npm install --omit=optional --no-fund --no-audit >/tmp/recall-git-install.log 2>&1
  git init -q
  git config user.email "docker-e2e@recall.local"
  git config user.name "Recall Docker E2E"
  git add -A
  git commit -qm "test fixture"
)
npm_log="/tmp/recall-doctor-switch-npm-install.log"
if ! npm install -g --prefix /tmp/npm-prefix --omit=optional "$package_tgz" >"$npm_log" 2>&1; then
  cat "$npm_log"
  exit 1
fi

npm_bin="/tmp/npm-prefix/bin/recall"
npm_root="/tmp/npm-prefix/lib/node_modules/recall"
...
run_flow() {
  ...
  local install_log="/tmp/recall-doctor-switch-${name}-install.log"
  local doctor_log="/tmp/recall-doctor-switch-${name}-doctor.log"
  local command_timeout="${RECALL_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  unit_path="$HOME/.config/systemd/user/recall-gateway.service"
  if [ ! -f "$unit_path" ]; then
...
run_flow \
  ...
  "$npm_entry" \
  "RECALL_UPDATE_IN_PROGRESS=1 node $git_cli doctor --repair --force --yes --non-interactive" \
  "$git_entry"
...
run_flow \
  ...
  "$git_entry" \
  "RECALL_UPDATE_IN_PROGRESS=1 $npm_bin doctor --repair --force --yes --non-interactive" \
  "$npm_entry"
...
run_proxy_env_flow() {
  ...
  if ! timeout "$command_timeout" env RECALL_UPDATE_IN_PROGRESS=1 \
...
run_wrapper_flow() {
  ...
  local install_log="/tmp/recall-doctor-switch-${name}-install.log"
  local reinstall_log="/tmp/recall-doctor-switch-${name}-reinstall.log"
  local env_repair_log="/tmp/recall-doctor-switch-${name}-env-repair.log"
  local doctor_log="/tmp/recall-doctor-switch-${name}-doctor.log"
  local clear_log="/tmp/recall-doctor-switch-${name}-clear.log"
  local command_timeout="${RECALL_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  local wrapper="$HOME/.local/bin/recall-wrapper"
  node scripts/e2e/lib/doctor-install-switch/write-wrapper.mjs \
    "$wrapper" \
    "$npm_bin" \
    "$HOME/recall-wrapper-argv.log"

  local unit_path="$HOME/.config/systemd/user/recall-gateway.service"
  ...
  assert_env_value "$unit_path" "RECALL_WRAPPER" "$wrapper"
  ...
  sed -i "/^Environment=RECALL_WRAPPER=/d" "$unit_path"
  ...
  sed -i "s#^Environment=RECALL_WRAPPER=.*#Environment=RECALL_WRAPPER=/tmp/stale-recall-wrapper#" "$unit_path"
  if ! timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
    cat "$env_repair_log"
    exit 1
  fi
  ...
  if ! grep -Fq "Gateway service invokes RECALL_WRAPPER:" "$doctor_log"; then
  ...
  if ! timeout "$command_timeout" env RECALL_WRAPPER= "$npm_bin" gateway install --force >"$clear_log" 2>&1; then
    cat "$clear_log"
    exit 1
  fi
  assert_no_env_key "$unit_path" "RECALL_WRAPPER"
