...
function clickClackHttpTimeoutMs() {
  return readPositiveInt(
    process.env.STEELENGINE_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS,
    5000,
    "STEELENGINE_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS",
  );
}

function clickClackHttpBodyMaxBytes() {
  return readPositiveInt(
    process.env.STEELENGINE_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES,
    1024 * 1024,
...
function assertOnboard() {
  ...
  assert(fs.existsSync(configPath()), "onboard did not write steelengine.json");
  const stateRaw =
    fs.readFileSync(configPath(), "utf8") +
    (fs.existsSync(authPath) ? fs.readFileSync(authPath, "utf8") : "");
  assert(
    !stateRaw.includes("sk-steelengine-release-user-journey"),
