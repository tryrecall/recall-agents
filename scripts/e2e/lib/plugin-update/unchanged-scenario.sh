...
openclaw_e2e_install_package /tmp/recall-install.log "mounted Recall package" /tmp/npm-prefix
...
RECALL_PACKAGE_ACCEPTANCE_LEGACY_COMPAT="$(node "$probe" legacy-compat "$package_version")"
...
node scripts/e2e/lib/plugin-update/registry-server.mjs >/tmp/recall-e2e-registry.log 2>&1 &
registry_pid=$!
...
if ! node "$probe" wait-registry; then
  ...
  cat /tmp/recall-e2e-registry.log || true
  exit 1
fi

before_config_hash=""
if [ "$RECALL_PACKAGE_ACCEPTANCE_LEGACY_COMPAT" != "1" ]; then
  before_config_hash="$(sha256sum "$RECALL_CONFIG_PATH" | awk '{print $1}')"
fi
plugin_update_timeout_seconds="${RECALL_PLUGIN_UPDATE_TIMEOUT_SECONDS:-180}"
...
if [ -n "$before_config_hash" ]; then
  after_config_hash="$(sha256sum "$RECALL_CONFIG_PATH" | awk '{print $1}')"
