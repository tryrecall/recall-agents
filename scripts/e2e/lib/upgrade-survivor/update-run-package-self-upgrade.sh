...
if [ "${STEELENGINE_QA_ALLOW_UPDATE_RUN_SELF:-0}" != "1" ]; then
  echo "blocked destructive package self-upgrade; set STEELENGINE_QA_ALLOW_UPDATE_RUN_SELF=1 to run" >&2
  exit 2
fi
...
SOURCE_VERSION="${STEELENGINE_UPDATE_RUN_SELF_UPGRADE_SOURCE_VERSION:-2026.4.26}"
SOURCE_SPEC="steelengine@$SOURCE_VERSION"
TARGET_TAG="latest"
RESTART_NOTE="QA-UPDATE-RUN-PACKAGE-SELF-UPGRADE"
PORT=18789
QA_BUS_PORT=43123
ARTIFACT_DIR="${STEELENGINE_UPDATE_RUN_SELF_UPGRADE_ARTIFACT_DIR:-/tmp/steelengine-update-run-artifacts}"
RUNTIME_ROOT="${STEELENGINE_UPDATE_RUN_SELF_UPGRADE_RUNTIME_ROOT:-/tmp/steelengine-update-run-runtime}"
export HOME="$RUNTIME_ROOT/home"
export STEELENGINE_STATE_DIR="$HOME/.steelengine"
export STEELENGINE_CONFIG_PATH="$STEELENGINE_STATE_DIR/steelengine.json"
export STEELENGINE_TEST_WORKSPACE_DIR="$HOME/workspace"
...
SERVICE_UNIT_ARTIFACT="$ARTIFACT_DIR/steelengine-gateway.service"
...
mkdir -p \
  ...
  "$STEELENGINE_TEST_WORKSPACE_DIR" \
  "$npm_config_prefix" \
  "$npm_config_cache"
rm -f \
  "$QA_BUS_READY_FILE" \
...
steelengine_e2e_maybe_timeout 600s \
...
if ! steelengine --version | grep -Fq "$SOURCE_VERSION"; then
  echo "source steelengine --version did not report $SOURCE_VERSION" >&2
  exit 1
fi

target_version="$(
  npm view "steelengine@$TARGET_TAG" version --json --prefer-online --cache "$npm_config_cache" |
    node -e '
      let raw = "";
...
qa_plugin_source="/tmp/steelengine-update-run-build/dist/extensions/qa-channel"
qa_plugin_dir="$qa_plugin_source"
if [ ! -f "$qa_plugin_source/steelengine.plugin.json" ] || [ ! -f "$qa_plugin_source/index.js" ]; then
...
'
steelengine_e2e_maybe_timeout 300s \
  steelengine plugins install "$qa_plugin_source" --link \
  >"$PLUGIN_INSTALL_LOG" 2>&1
steelengine plugins inspect qa-channel --json >"$SOURCE_PLUGIN_INSPECT_JSON"
node -e '
  const fs = require("node:fs");
...
' "$SOURCE_PLUGIN_INSPECT_JSON"
  ...
  node -e '
    const fs = require("node:fs");
    const indexPath = `${process.env.STEELENGINE_STATE_DIR}/plugins/installs.json`;
...
for _ in $(seq 1 100); do
  ...
  if ! kill -0 "$qa_bus_pid" 2>/dev/null; then
    steelengine_e2e_print_log "$QA_BUS_STDIO_LOG" >&2
    exit 1
  fi
  sleep 0.1
done
...
CONFIG_PATH="$STEELENGINE_CONFIG_PATH" \
...
steelengine config validate >"$ARTIFACT_DIR/config-validate.log" 2>&1

install_update_restart_systemctl_shim
if ! steelengine_e2e_maybe_timeout 120s \
  steelengine gateway install --force --json \
  >"$SERVICE_INSTALL_JSON" 2>"$SERVICE_INSTALL_ERR"; then
  echo "historical Gateway service install failed" >&2
  steelengine_e2e_print_log "$SERVICE_INSTALL_ERR" >&2
  exit 1
fi
service_unit="$HOME/.config/systemd/user/steelengine-gateway.service"
if [ ! -f "$service_unit" ] || ! grep -q '^ExecStart=' "$service_unit"; then
  echo "historical Gateway install did not create a service unit" >&2
  exit 1
fi
if grep -q 'STEELENGINE_SKIP_PROVIDERS' "$service_unit"; then
  echo "service-owned target environment unexpectedly suppresses providers" >&2
  exit 1
fi
if ! grep -q 'STEELENGINE_SYSTEMD_UNIT=steelengine-gateway.service' "$service_unit"; then
  echo "service-owned target environment omitted its systemd marker" >&2
  exit 1
fi
cp "$service_unit" "$SERVICE_UNIT_ARTIFACT"
systemctl --user stop steelengine-gateway.service
if systemctl --user is-active steelengine-gateway.service >/dev/null 2>&1; then
...
gateway_call() {
  ...
  steelengine gateway call "$method" \
...
if ! wait "$supervisor_monitor_pid"; then
  ...
  steelengine_e2e_print_log "$SUPERVISOR_MONITOR_LOG" >&2
  exit 1
fi
supervisor_monitor_pid=""
...
steelengine_e2e_wait_gateway_ready "$gateway_pid" "$SYSTEMCTL_SHIM_DAEMON_LOG" 180 "$PORT"
...
if [ ! -f "$UPDATE_STATUS_JSON" ]; then
  ...
  steelengine_e2e_print_log "$UPDATE_STATUS_ERR" >&2
...
TARGET_PLUGIN_INDEX_OUT="$TARGET_PLUGIN_INDEX_JSON" node --input-type=module -e '
  ...
  if (
    record?.source !== "path" ||
    record.installPath !== "/tmp/steelengine-update-run-build/dist/extensions/qa-channel"
  ) {
