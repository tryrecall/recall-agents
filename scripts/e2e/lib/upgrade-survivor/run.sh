...
ARTIFACT_ROOT="$(dirname "${STEELENGINE_UPGRADE_SURVIVOR_SUMMARY_JSON:-/tmp/steelengine-upgrade-survivor-artifacts/summary.json}")"
...
RUNTIME_ROOT="$STEELENGINE_UPGRADE_SURVIVOR_RUNTIME_ROOT"
STATE_HOME_ROOT="${STEELENGINE_UPGRADE_SURVIVOR_STATE_HOME_ROOT:-$RUNTIME_ROOT/state-home}"
mkdir -p "$ARTIFACT_ROOT"
mkdir -p "$RUNTIME_ROOT"
export TMPDIR="${STEELENGINE_UPGRADE_SURVIVOR_TMPDIR:-$RUNTIME_ROOT/tmp}"
export STEELENGINE_TEST_STATE_TMPDIR="${STEELENGINE_UPGRADE_SURVIVOR_TEST_STATE_TMPDIR:-$RUNTIME_ROOT/state-tmp}"
mkdir -p "$TMPDIR" "$STEELENGINE_TEST_STATE_TMPDIR"
...
SUMMARY_JSON="${STEELENGINE_UPGRADE_SURVIVOR_SUMMARY_JSON:-$ARTIFACT_ROOT/summary.json}"
PHASE_LOG="$ARTIFACT_ROOT/phases.jsonl"
BASELINE_RAW="${STEELENGINE_UPGRADE_SURVIVOR_BASELINE:?missing STEELENGINE_UPGRADE_SURVIVOR_BASELINE}"
CANDIDATE_KIND="${STEELENGINE_UPGRADE_SURVIVOR_CANDIDATE_KIND:-tarball}"
CANDIDATE_SPEC="${STEELENGINE_UPGRADE_SURVIVOR_CANDIDATE_SPEC:-${STEELENGINE_CURRENT_PACKAGE_TGZ:-}}"
...
export STEELENGINE_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON="$CONFIG_COVERAGE_JSON"
...
validate_baseline_package_spec() {
  ...
  echo "STEELENGINE_UPGRADE_SURVIVOR_BASELINE must be steelengine@latest, steelengine@beta, steelengine@alpha, an exact SteelEngine release version, or a bare release version; got: $spec" >&2
  return 1
}
...
normalize_baseline() {
  ...
  if [ -z "$raw" ]; then
    echo "STEELENGINE_UPGRADE_SURVIVOR_BASELINE cannot be empty" >&2
    return 1
  fi
  case "$raw" in
    steelengine@*)
      baseline_spec="$raw"
      baseline_version="${raw#steelengine@}"
      ;;
    *@*)
      echo "STEELENGINE_UPGRADE_SURVIVOR_BASELINE must be steelengine@<version> or a bare version" >&2
      return 1
      ;;
    *)
      baseline_version="$raw"
      baseline_spec="steelengine@$raw"
      ;;
  esac
  case "$baseline_version" in
    latest | beta | alpha)
      ...
      ;;
    dev | main | "")
      echo "STEELENGINE_UPGRADE_SURVIVOR_BASELINE must be steelengine@latest, steelengine@beta, steelengine@alpha, steelengine@<version>, or a bare version" >&2
      return 1
      ;;
    *)
...
validate_update_restart_mode() {
  case "$UPDATE_RESTART_MODE" in
    manual | auto-auth)
      ;;
    *)
      echo "STEELENGINE_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE must be manual or auto-auth; got: $UPDATE_RESTART_MODE" >&2
      return 1
      ;;
  esac
}
...
const summary = {
  ...
  candidate: {
    kind: process.env.STEELENGINE_UPGRADE_SURVIVOR_CANDIDATE_KIND || null,
    spec: process.env.STEELENGINE_UPGRADE_SURVIVOR_CANDIDATE_SPEC || process.env.STEELENGINE_CURRENT_PACKAGE_TGZ || null,
...
cleanup() {
  ...
  if [ -s "$SYSTEMCTL_SHIM_PID_FILE" ]; then
    ...
    if [[ "$shim_pid" =~ ^[0-9]+$ ]] && [ "$shim_pid" -gt 1 ]; then
      steelengine_e2e_terminate_gateways "$shim_pid"
    fi
  fi
}
...
package_root() {
  printf '%s/lib/node_modules/steelengine\n' "$npm_config_prefix"
}

legacy_runtime_deps_symlink_plugin() {
  local plugin="${STEELENGINE_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK:-}"
  if [ -z "$plugin" ]; then
    return 1
  fi
  case "$plugin" in
    *[!A-Za-z0-9._-]*)
      echo "STEELENGINE_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK must be a plugin id, got: $plugin" >&2
      return 2
      ;;
  esac
  printf '%s\n' "$plugin"
...
legacy_runtime_deps_symlink_target() {
  local plugin="$1"
  printf '%s/@steelengine-upgrade-survivor/%s-runtime-dep\n' "$(dirname "$(package_root)")" "$plugin"
}
...
plugin_deps_cleanup_plugins() {
  printf '%s\n' "${STEELENGINE_UPGRADE_SURVIVOR_PLUGIN_DEPS_CLEANUP_PLUGINS:-discord telegram}"
}
...
seed_source_only_plugin_shadow() {
  ...
  local shadow_root="$STEELENGINE_STATE_DIR/extensions/opik-steelengine"
  mkdir -p "$shadow_root/src"
  cat >"$shadow_root/package.json" <<'JSON'
{
  "name": "@opik/opik-steelengine",
...
JSON
  cat >"$shadow_root/steelengine.plugin.json" <<'JSON'
{
  "id": "opik-steelengine",
  "activation": {
    "onStartup": false
...
legacy_plugin_dependency_probe_paths() {
  ...
  while IFS= read -r plugin_dir; do
    printf '%s\n' \
      ...
      "$plugin_dir/.steelengine-runtime-deps.json" \
      "$plugin_dir/.steelengine-runtime-deps-stamp.json" \
      "$plugin_dir/.steelengine-runtime-deps-copy-upgrade-survivor" \
      "$plugin_dir/.steelengine-install-stage-upgrade-survivor" \
      "$plugin_dir/.steelengine-pnpm-store"
  done < <(plugin_deps_cleanup_plugin_dirs "$plugin")
  printf '%s\n' \
    "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$STEELENGINE_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$STEELENGINE_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor"
}
...
seed_legacy_plugin_dependency_debris() {
  ...
  for plugin in $(plugin_deps_cleanup_plugins); do
    ...
    found=1
    mkdir -p \
      "$plugin_dir/node_modules/steelengine-upgrade-survivor-dep" \
      "$plugin_dir/.steelengine-runtime-deps-copy-upgrade-survivor/node_modules/steelengine-upgrade-survivor-dep" \
      "$plugin_dir/.steelengine-install-stage-upgrade-survivor" \
      "$plugin_dir/.steelengine-pnpm-store" \
      "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/steelengine-upgrade-survivor-dep" \
      "$STEELENGINE_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/steelengine-upgrade-survivor-dep" \
      "$STEELENGINE_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/steelengine-upgrade-survivor-dep"
    printf '{"name":"steelengine-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/node_modules/steelengine-upgrade-survivor-dep/package.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup"}\n' "$plugin" \
      >"$plugin_dir/.steelengine-runtime-deps.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup","stale":true}\n' "$plugin" \
      >"$plugin_dir/.steelengine-runtime-deps-stamp.json"
    printf '{"name":"steelengine-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/.steelengine-runtime-deps-copy-upgrade-survivor/node_modules/steelengine-upgrade-survivor-dep/package.json"
    printf '{"name":"steelengine-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/steelengine-upgrade-survivor-dep/package.json"
    printf '{"name":"steelengine-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$STEELENGINE_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/steelengine-upgrade-survivor-dep/package.json"
    printf '{"name":"steelengine-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$STEELENGINE_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/steelengine-upgrade-survivor-dep/package.json"
...
seed_legacy_runtime_deps_symlink() {
  ...
  printf '{"name":"steelengine-upgrade-survivor-legacy-runtime-deps","version":"0.0.0"}\n' \
...
reset_run_state() {
  rm_rf_retry "$npm_config_prefix" "$TMPDIR" "$STEELENGINE_TEST_STATE_TMPDIR" "$STATE_HOME_ROOT"
  rm -f "$SYSTEMCTL_SHIM_PID_FILE" "$SYSTEMCTL_SHIM_DAEMON_LOG"
  mkdir -p "$npm_config_prefix" "$npm_config_cache" "$TMPDIR" "$STEELENGINE_TEST_STATE_TMPDIR"
}

install_baseline() {
  normalize_baseline
  echo "Installing baseline package: $baseline_spec"
  if ! steelengine_e2e_maybe_timeout "${STEELENGINE_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix "$npm_config_prefix" "$baseline_spec" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
    echo "baseline npm install failed" >&2
    steelengine_e2e_print_log "$BASELINE_INSTALL_LOG" >&2
    return 1
  fi
  if ! command -v steelengine >/dev/null; then
    echo "baseline install did not expose steelengine on PATH" >&2
    echo "PATH=$PATH" >&2
  ...
  if ! version_output="$(steelengine_e2e_maybe_timeout "$COMMAND_TIMEOUT" steelengine --version 2>&1)"; then
    echo "baseline steelengine --version failed" >&2
    echo "$version_output" >&2
    return 1
  fi
  if [[ "$version_output" != *"$baseline_version"* ]]; then
    echo "baseline steelengine --version mismatch: expected output to include $baseline_version" >&2
    echo "$version_output" >&2
    return 1
  fi
}
...
seed_state() {
  ...
  if [ "$ROOT_MANAGED_VPS" = "1" ]; then
    ...
    rm -rf /root/.steelengine /root/workspace
    steelengine_test_state_create /root minimal
  else
    steelengine_test_state_create "$STATE_HOME_ROOT" minimal
  fi
  export STEELENGINE_UPGRADE_SURVIVOR_BASELINE_VERSION="$baseline_version"
...
validate_baseline_config() {
  if ! steelengine_e2e_maybe_timeout "$COMMAND_TIMEOUT" steelengine config validate >"$BASELINE_CONFIG_VALIDATE_LOG" 2>&1; then
    echo "generated baseline config failed baseline validation" >&2
    steelengine_e2e_print_log "$BASELINE_CONFIG_VALIDATE_LOG" >&2
    return 1
  fi
}
...
log_file="${STEELENGINE_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_LOG:-/tmp/steelengine-systemctl-shim.log}"
pid_file="${STEELENGINE_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE:-/tmp/steelengine-systemctl-shim.pid}"
daemon_log="${STEELENGINE_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_DAEMON_LOG:-/tmp/steelengine-systemctl-shim-gateway.log}"
printf '%s\n' "$*" >>"$log_file"
...
unit_path() {
  printf '%s/.config/systemd/user/steelengine-gateway.service\n' "${HOME:?missing HOME}"
}
...
install_update_restart_service_unit() {
  if ! steelengine_e2e_maybe_timeout "$COMMAND_TIMEOUT" env -u STEELENGINE_GATEWAY_TOKEN -u STEELENGINE_GATEWAY_PASSWORD steelengine gateway install --force --json >"$BASELINE_SERVICE_INSTALL_JSON" 2>"$BASELINE_SERVICE_INSTALL_ERR"; then
    echo "baseline gateway service install failed" >&2
    steelengine_e2e_print_log "$BASELINE_SERVICE_INSTALL_ERR" >&2
    steelengine_e2e_print_log "$BASELINE_SERVICE_INSTALL_JSON" >&2
    return 1
  fi
}
...
if (!stateDir) {
  throw new Error("missing STEELENGINE_STATE_DIR");
}
...
write_update_restart_service_secretref_env() {
  ...
  local dotenv_path="$STEELENGINE_STATE_DIR/.env"
...
assert_baseline_state() {
  STEELENGINE_UPGRADE_SURVIVOR_ASSERT_STAGE=baseline \
...
resolve_candidate_version() {
  if [ -z "$CANDIDATE_SPEC" ]; then
    echo "missing STEELENGINE_UPGRADE_SURVIVOR_CANDIDATE_SPEC" >&2
    return 1
  fi
...
update_candidate() {
  ...
  local update_env=(
    env
    -u STEELENGINE_GATEWAY_TOKEN
    -u STEELENGINE_GATEWAY_PASSWORD
    -u STEELENGINE_ALLOW_ROOT
  )
  ...
  if [ "$ROOT_MANAGED_VPS" != "1" ]; then
    update_env+=(STEELENGINE_ALLOW_ROOT=1)
  fi
  if ! steelengine_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${update_env[@]}" steelengine "${update_args[@]}" >"$UPDATE_JSON" 2>"$UPDATE_ERR"; then
    echo "steelengine update failed" >&2
    steelengine_e2e_print_log "$UPDATE_ERR" >&2
    steelengine_e2e_print_log "$UPDATE_JSON" >&2
    return 1
  fi
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
assert_root_managed_vps_cli_usable() {
  ...
  steelengine_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" steelengine config file >"$ARTIFACT_ROOT/root-vps-config-file.out" 2>"$ARTIFACT_ROOT/root-vps-config-file.err"
  steelengine_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" steelengine plugins >"$ARTIFACT_ROOT/root-vps-plugins.out" 2>"$ARTIFACT_ROOT/root-vps-plugins.err"
}

run_doctor() {
  if ! steelengine_e2e_maybe_timeout "$COMMAND_TIMEOUT" steelengine doctor --fix --non-interactive >"$DOCTOR_LOG" 2>&1; then
    echo "steelengine doctor failed" >&2
    steelengine_e2e_print_log "$DOCTOR_LOG" >&2
    return 1
  fi
}

validate_post_doctor_config() {
  if ! steelengine_e2e_maybe_timeout "$COMMAND_TIMEOUT" steelengine config validate >>"$DOCTOR_LOG" 2>&1; then
...
probe_gateway_endpoint() {
  ...
  if [ -n "${STEELENGINE_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING:-}" ]; then
    args+=(--allow-failing "$STEELENGINE_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING")
  fi
...
start_gateway() {
  ...
  local ready_epoch
  start_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  env -u STEELENGINE_GATEWAY_TOKEN -u STEELENGINE_GATEWAY_PASSWORD steelengine gateway --port "$port" --bind loopback --allow-unconfigured >"$GATEWAY_LOG" 2>&1 &
  gateway_pid="$!"
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
check_gateway_status() {
  ...
  if ! steelengine_e2e_maybe_timeout "$COMMAND_TIMEOUT" steelengine gateway status --url "ws://127.0.0.1:$port" --token "$GATEWAY_AUTH_TOKEN_REF" --require-rpc --timeout 30000 --json >"$STATUS_JSON" 2>"$STATUS_ERR"; then
    echo "gateway status failed" >&2
    steelengine_e2e_print_log "$STATUS_ERR" >&2
    steelengine_e2e_print_log "$GATEWAY_LOG" >&2
    return 1
  fi
  ...
  if [ "$status_seconds" -gt "$budget" ]; then
    ...
    steelengine_e2e_print_log "$STATUS_JSON" >&2
    return 1
  fi
