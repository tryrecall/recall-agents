...
ARTIFACT_ROOT="$(dirname "${RECALL_UPGRADE_SURVIVOR_SUMMARY_JSON:-/tmp/recall-upgrade-survivor-artifacts/summary.json}")"
...
SUMMARY_JSON="${RECALL_UPGRADE_SURVIVOR_SUMMARY_JSON:-$ARTIFACT_ROOT/summary.json}"
PHASE_LOG="$ARTIFACT_ROOT/phases.jsonl"
BASELINE_RAW="${RECALL_UPGRADE_SURVIVOR_BASELINE:?missing RECALL_UPGRADE_SURVIVOR_BASELINE}"
CANDIDATE_KIND="${RECALL_UPGRADE_SURVIVOR_CANDIDATE_KIND:-tarball}"
CANDIDATE_SPEC="${RECALL_UPGRADE_SURVIVOR_CANDIDATE_SPEC:-${RECALL_CURRENT_PACKAGE_TGZ:-}}"
...
export RECALL_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON="$CONFIG_COVERAGE_JSON"
rm -f "$SUMMARY_JSON" "$CONFIG_COVERAGE_JSON"
: >"$PHASE_LOG"

validate_baseline_package_spec() {
  local spec="$1"
  if [[ "$spec" =~ ^recall@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$ ]]; then
    return 0
  fi
  echo "RECALL_UPGRADE_SURVIVOR_BASELINE must be recall@latest, recall@beta, recall@alpha, an exact Recall release version, or a bare release version; got: $spec" >&2
  return 1
}
...
normalize_baseline() {
  ...
  if [ -z "$raw" ]; then
    echo "RECALL_UPGRADE_SURVIVOR_BASELINE cannot be empty" >&2
    return 1
  fi
  case "$raw" in
    recall@*)
      baseline_spec="$raw"
      baseline_version="${raw#recall@}"
      ;;
    *@*)
      echo "RECALL_UPGRADE_SURVIVOR_BASELINE must be recall@<version> or a bare version" >&2
      return 1
      ;;
    *)
      baseline_version="$raw"
      baseline_spec="recall@$raw"
      ;;
  esac
  case "$baseline_version" in
    latest | beta | alpha)
      ...
      ;;
    dev | main | "")
      echo "RECALL_UPGRADE_SURVIVOR_BASELINE must be recall@latest, recall@beta, recall@alpha, recall@<version>, or a bare version" >&2
      return 1
      ;;
    *)
...
validate_update_restart_mode() {
  case "$UPDATE_RESTART_MODE" in
    manual | auto-auth)
      ;;
    *)
      echo "RECALL_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE must be manual or auto-auth; got: $UPDATE_RESTART_MODE" >&2
      return 1
      ;;
  esac
}
...
const summary = {
  ...
  candidate: {
    kind: process.env.RECALL_UPGRADE_SURVIVOR_CANDIDATE_KIND || null,
    spec: process.env.RECALL_UPGRADE_SURVIVOR_CANDIDATE_SPEC || process.env.RECALL_CURRENT_PACKAGE_TGZ || null,
...
legacy_runtime_deps_symlink_plugin() {
  local plugin="${RECALL_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK:-}"
  if [ -z "$plugin" ]; then
    return 1
  fi
  case "$plugin" in
    *[!A-Za-z0-9._-]*)
      echo "RECALL_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK must be a plugin id, got: $plugin" >&2
      return 2
      ;;
  esac
  printf '%s\n' "$plugin"
...
legacy_runtime_deps_symlink_target() {
  local plugin="$1"
  printf '%s/@recall-upgrade-survivor/%s-runtime-dep\n' "$(dirname "$(package_root)")" "$plugin"
}
...
plugin_deps_cleanup_plugins() {
  printf '%s\n' "${RECALL_UPGRADE_SURVIVOR_PLUGIN_DEPS_CLEANUP_PLUGINS:-discord telegram}"
}
...
seed_source_only_plugin_shadow() {
  ...
  local shadow_root="$RECALL_STATE_DIR/extensions/opik-recall"
  mkdir -p "$shadow_root/src"
  cat >"$shadow_root/package.json" <<'JSON'
{
  "name": "@opik/opik-recall",
...
JSON
  cat >"$shadow_root/recall.plugin.json" <<'JSON'
{
  "id": "opik-recall",
  "activation": {
    "onStartup": false
...
legacy_plugin_dependency_probe_paths() {
  ...
  while IFS= read -r plugin_dir; do
    printf '%s\n' \
      ...
      "$plugin_dir/.recall-runtime-deps.json" \
      "$plugin_dir/.recall-runtime-deps-stamp.json" \
      "$plugin_dir/.recall-runtime-deps-copy-upgrade-survivor" \
      "$plugin_dir/.recall-install-stage-upgrade-survivor" \
      "$plugin_dir/.recall-pnpm-store"
  done < <(plugin_deps_cleanup_plugin_dirs "$plugin")
  printf '%s\n' \
    "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$RECALL_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$RECALL_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor"
}
...
seed_legacy_plugin_dependency_debris() {
  ...
  for plugin in $(plugin_deps_cleanup_plugins); do
    ...
    found=1
    mkdir -p \
      "$plugin_dir/node_modules/recall-upgrade-survivor-dep" \
      "$plugin_dir/.recall-runtime-deps-copy-upgrade-survivor/node_modules/recall-upgrade-survivor-dep" \
      "$plugin_dir/.recall-install-stage-upgrade-survivor" \
      "$plugin_dir/.recall-pnpm-store" \
      "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/recall-upgrade-survivor-dep" \
      "$RECALL_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/recall-upgrade-survivor-dep" \
      "$RECALL_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/recall-upgrade-survivor-dep"
    printf '{"name":"recall-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/node_modules/recall-upgrade-survivor-dep/package.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup"}\n' "$plugin" \
      >"$plugin_dir/.recall-runtime-deps.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup","stale":true}\n' "$plugin" \
      >"$plugin_dir/.recall-runtime-deps-stamp.json"
    printf '{"name":"recall-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/.recall-runtime-deps-copy-upgrade-survivor/node_modules/recall-upgrade-survivor-dep/package.json"
    printf '{"name":"recall-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/recall-upgrade-survivor-dep/package.json"
    printf '{"name":"recall-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$RECALL_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/recall-upgrade-survivor-dep/package.json"
    printf '{"name":"recall-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$RECALL_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/recall-upgrade-survivor-dep/package.json"
...
seed_legacy_runtime_deps_symlink() {
  ...
  printf '{"name":"recall-upgrade-survivor-legacy-runtime-deps","version":"0.0.0"}\n' \
...
install_baseline() {
  ...
  if ! command -v recall >/dev/null; then
    echo "baseline install did not expose recall on PATH" >&2
    echo "PATH=$PATH" >&2
  ...
  if ! version_output="$(recall --version 2>&1)"; then
    echo "baseline recall --version failed" >&2
    echo "$version_output" >&2
    return 1
  fi
  if [[ "$version_output" != *"$baseline_version"* ]]; then
    echo "baseline recall --version mismatch: expected output to include $baseline_version" >&2
    echo "$version_output" >&2
    return 1
  fi
}
...
validate_baseline_config() {
  if ! recall config validate >"$BASELINE_CONFIG_VALIDATE_LOG" 2>&1; then
...
log_file="${RECALL_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_LOG:-/tmp/recall-systemctl-shim.log}"
pid_file="${RECALL_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE:-/tmp/recall-systemctl-shim.pid}"
daemon_log="${RECALL_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_DAEMON_LOG:-/tmp/recall-systemctl-shim-gateway.log}"
printf '%s\n' "$*" >>"$log_file"
...
unit_path() {
  printf '%s/.config/systemd/user/recall-gateway.service\n' "${HOME:?missing HOME}"
}
...
install_update_restart_service_unit() {
  if ! env -u RECALL_GATEWAY_TOKEN -u RECALL_GATEWAY_PASSWORD recall gateway install --force --json >"$BASELINE_SERVICE_INSTALL_JSON" 2>"$BASELINE_SERVICE_INSTALL_ERR"; then
...
if (!stateDir) {
  throw new Error("missing RECALL_STATE_DIR");
}
...
write_update_restart_service_secretref_env() {
  ...
  local dotenv_path="$RECALL_STATE_DIR/.env"
...
write_update_restart_service_auth_env() {
  ...
  printf 'GATEWAY_AUTH_TOKEN_REF=%s\n' "$GATEWAY_AUTH_TOKEN_REF" >>"$tmp_path"
  mv "$tmp_path" "$dotenv_path"
  local systemd_env_path="$RECALL_STATE_DIR/gateway.systemd.env"
...
assert_baseline_state() {
  RECALL_UPGRADE_SURVIVOR_ASSERT_STAGE=baseline \
...
resolve_candidate_version() {
  if [ -z "$CANDIDATE_SPEC" ]; then
    echo "missing RECALL_UPGRADE_SURVIVOR_CANDIDATE_SPEC" >&2
    return 1
  fi
...
update_candidate() {
  ...
  local update_env=(
    env
    -u RECALL_GATEWAY_TOKEN
    -u RECALL_GATEWAY_PASSWORD
    -u RECALL_ALLOW_ROOT
  )
  ...
  if [ "$ROOT_MANAGED_VPS" != "1" ]; then
    update_env+=(RECALL_ALLOW_ROOT=1)
  fi
  if ! "${update_env[@]}" recall "${update_args[@]}" >"$UPDATE_JSON" 2>"$UPDATE_ERR"; then
...
assert_root_managed_vps_cli_usable() {
  ...
  "${root_cli_env[@]}" recall config file >"$ARTIFACT_ROOT/root-vps-config-file.out" 2>"$ARTIFACT_ROOT/root-vps-config-file.err"
  "${root_cli_env[@]}" recall plugins >"$ARTIFACT_ROOT/root-vps-plugins.out" 2>"$ARTIFACT_ROOT/root-vps-plugins.err"
}

run_doctor() {
  if ! recall doctor --fix --non-interactive >"$DOCTOR_LOG" 2>&1; then
...
validate_post_doctor_config() {
  if ! recall config validate >>"$DOCTOR_LOG" 2>&1; then
...
probe_gateway_endpoint() {
  ...
  if [ -n "${RECALL_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING:-}" ]; then
    args+=(--allow-failing "$RECALL_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING")
  fi
  args+=(--out "$out_file")
...
start_gateway() {
  local port=18789
  local budget="${RECALL_UPGRADE_SURVIVOR_START_BUDGET_SECONDS:-90}"
  local start_epoch
  local ready_epoch
  start_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  env -u RECALL_GATEWAY_TOKEN -u RECALL_GATEWAY_PASSWORD recall gateway --port "$port" --bind loopback --allow-unconfigured >"$GATEWAY_LOG" 2>&1 &
  gateway_pid="$!"
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
check_gateway_probes() {
  ...
  export RECALL_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING="discord,telegram,whatsapp,feishu,matrix"
  readyz_seconds="$(probe_gateway_endpoint /readyz ready "$READYZ_JSON")"
  unset RECALL_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING
}

check_gateway_status() {
  local port=18789
  local budget="${RECALL_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS:-30}"
  local status_start
  local status_end
  status_start="$(node -e "process.stdout.write(String(Date.now()))")"
  if ! recall gateway status --url "ws://127.0.0.1:$port" --token "$GATEWAY_AUTH_TOKEN_REF" --require-rpc --timeout 30000 --json >"$STATUS_JSON" 2>"$STATUS_ERR"; then
