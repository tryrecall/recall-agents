    ...
    [
      {
        id: "logging-file",
        intent: "logging",
        argv: ["config", "set", "logging.file", "~/recall-upgrade-survivor/gateway.jsonl"],
      },
    ],
  ],
  [
...
function selectedScenario() {
  return process.env.RECALL_UPGRADE_SURVIVOR_SCENARIO || "base";
}
...
function runRecall(step) {
  const result = spawnSync("recall", step.argv, {
    encoding: "utf8",
    env: process.env,
  });
  return {
...
function applyRecipe() {
  ...
  for (const step of [...recipe.slice(0, -1), ...scenarioSteps, recipe.at(-1)]) {
    ...
    const outcome = runRecall(adaptedStep);
