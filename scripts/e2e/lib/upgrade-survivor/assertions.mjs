...
function getScenario() {
  const scenario = process.env.RECALL_UPGRADE_SURVIVOR_SCENARIO || "base";
  assert(SCENARIOS.has(scenario), `unknown upgrade survivor scenario: ${scenario}`);
  return scenario;
}

function getConfig() {
  return readJson(requireEnv("RECALL_CONFIG_PATH"));
}

function getCoverage() {
  const file = process.env.RECALL_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON;
...
function seedState() {
  ...
  const workspace = requireEnv("RECALL_TEST_WORKSPACE_DIR");
  ...
  for (const plugin of ["discord", "telegram", "whatsapp"]) {
    writeJson(path.join(runtimeRoot, plugin, ".recall-runtime-deps-stamp.json"), {
      version: 0,
      plugin,
      stale: true,
    });
    write(
      path.join(
        runtimeRoot,
        plugin,
        ".recall-runtime-deps-copy-stale",
        "node_modules",
        "stale-sentinel",
        "package.json",
      ),
  ...
  if (scenario === "versioned-runtime-deps") {
    const version = process.env.RECALL_UPGRADE_SURVIVOR_BASELINE_VERSION || "2026.4.24";
    for (const plugin of ["discord", "feishu", "telegram", "whatsapp"]) {
      writeJson(
        path.join(
          runtimeRoot,
          `recall-${version}-${plugin}`,
...
function assertConfigSurvived() {
  ...
  if (hasCoverage(coverage) && acceptsIntent(coverage, "logging")) {
    assert(
      config.logging?.file === "~/recall-upgrade-survivor/gateway.jsonl",
...
function assertStateSurvived() {
  ...
  const stage = process.env.RECALL_UPGRADE_SURVIVOR_ASSERT_STAGE || "survival";
  ...
  if (scenario === "stale-source-plugin-shadow") {
    const staleRoot = path.join(stateDir, "extensions", "opik-recall");
    assert(
  ...
  if (scenario === "versioned-runtime-deps") {
    ...
    const staleVersionedRoots = fs.existsSync(runtimeRoot)
      ? fs.readdirSync(runtimeRoot).filter((entry) => entry.startsWith(`recall-${version}-`))
      : [];
    assert(
...
function assertExternalPluginInstall(records, pluginId, packageName) {
  ...
  if (installedFromNpm) {
    const npmRoot = path.join(requireEnv("RECALL_STATE_DIR"), "npm", "node_modules");
    assert(
  ...
  const extensionsRoot = path.join(requireEnv("RECALL_STATE_DIR"), "extensions");
  assert(
...
function assertConfiguredPluginInstalls() {
  ...
  assertOptionalConfiguredPluginIndex(records, index.plugins ?? [], {
    packageName: "@recall/brave-plugin",
    pluginId: "brave",
  });
