...
export OPENAI_API_KEY="sk-steelengine-release-media-memory"
export STEELENGINE_QA_ALLOW_LOCAL_IMAGE_PROVIDER=1

PORT="18789"
MOCK_PORT="44200"
SUCCESS_MARKER="STEELENGINE_E2E_OK_MEDIA_MEMORY"
MEMORY_MARKER="release-media-memory-saffron-$(date +%s)"
media_root="$(mktemp -d /tmp/steelengine-release-media-memory.XXXXXX)"
...
entry="$(steelengine_e2e_package_entrypoint "$package_root")"
{
  ...
  printf 'entry=%s\n' "$entry"
  printf 'HOME=%s\n' "$HOME"
  printf 'STEELENGINE_HOME=%s\n' "$STEELENGINE_HOME"
  printf 'STEELENGINE_STATE_DIR=%s\n' "$STEELENGINE_STATE_DIR"
  printf 'STEELENGINE_CONFIG_PATH=%s\n' "$STEELENGINE_CONFIG_PATH"
} >"$ENV_LOG"
...
mock_pid="$(steelengine_e2e_start_mock_openai "$MOCK_PORT" "$MOCK_OPENAI_LOG")"
...
cp "$STEELENGINE_CONFIG_PATH" "$CONFIG_JSON"
steelengine plugins list --json >"$PLUGINS_JSON" 2>"$PLUGINS_STDERR_LOG"
...
mkdir -p "$STEELENGINE_STATE_DIR/workspace/memory"
...
cat >"$STEELENGINE_STATE_DIR/workspace/MEMORY.md" <<EOF
# Long-term memory

- The release media memory marker is $MEMORY_MARKER.
EOF

steelengine memory index --force >"$INDEX_LOG" 2>&1
steelengine memory search "$MEMORY_MARKER" --json >"$SEARCH_BEFORE_JSON" 2>"$SEARCH_BEFORE_STDERR_LOG"
...
steelengine memory search "$MEMORY_MARKER" --json >"$SEARCH_AFTER_JSON" 2>"$SEARCH_AFTER_STDERR_LOG"
