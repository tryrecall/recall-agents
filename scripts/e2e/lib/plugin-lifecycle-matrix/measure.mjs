...
const pollMs = clampTimerTimeoutMs(
  readPositiveIntEnv("STEELENGINE_PLUGIN_LIFECYCLE_METRIC_POLL_MS", 100),
);
const timeoutMs = clampTimerTimeoutMs(
  readPositiveIntEnv("STEELENGINE_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS", 300000),
);
const timeoutKillGraceMs = clampTimerTimeoutMs(
  readPositiveIntEnv("STEELENGINE_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS", 2000),
);
const maxRssKbThreshold = readPositiveIntEnv(
  "STEELENGINE_PLUGIN_LIFECYCLE_MAX_RSS_KB",
  4 * 1024 * 1024,
);
const maxWallMs = readPositiveIntEnv("STEELENGINE_PLUGIN_LIFECYCLE_MAX_WALL_MS", timeoutMs);
const maxCpuCoreRatio = readPositiveNumberEnv("STEELENGINE_PLUGIN_LIFECYCLE_MAX_CPU_CORE_RATIO", 16);
...
const pageSize = readPositiveIntEnvOrGetconf("STEELENGINE_PROC_PAGE_SIZE", "PAGESIZE");
const clockTicks = readPositiveIntEnvOrGetconf("STEELENGINE_PROC_CLK_TCK", "CLK_TCK");
