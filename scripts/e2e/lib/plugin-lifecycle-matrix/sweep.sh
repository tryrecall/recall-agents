...
openclaw_e2e_install_package /tmp/recall-plugin-lifecycle-install.log "mounted Recall package" /tmp/npm-prefix
...
pack_root="$(mktemp -d "/tmp/recall-plugin-lifecycle-pack.XXXXXX")"
registry_root="$(mktemp -d "/tmp/recall-plugin-lifecycle-registry.XXXXXX")"
