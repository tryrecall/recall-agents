...
const inspect = readJson("/tmp/recall-codex-inspect.json");
...
if (!String(codexRecord.spec || "").includes("@recall/codex")) {
  throw new Error(`expected @recall/codex install spec, got ${codexRecord.spec}`);
}
...
if (!fs.existsSync(codexPackageJson)) {
  throw new Error(`missing npm-installed @recall/codex package: ${codexPackageJson}`);
}
const codexPackage = readJson(codexPackageJson);
if (codexPackage.name !== "@recall/codex") {
...
if (authRaw.includes("sk-recall-codex-on-demand-e2e")) {
