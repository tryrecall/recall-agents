...
const STATUS_TEXT_MAX_BYTES = readPositiveIntEnv(
  "STEELENGINE_NPM_ONBOARD_STATUS_TEXT_MAX_BYTES",
  1024 * 1024,
);
...
function assertOnboardState() {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error("onboard did not write steelengine.json");
  }
  ...
  assertOpenAiEnvAuthProfileStore(authStoreText, {
    ...
    rawKeyNeedle: "sk-steelengine-npm-onboard-e2e",
  });
}
...
function assertMockModelConfig() {
  ...
  if (provider?.agentRuntime?.id !== "steelengine") {
  ...
  if (agent && agentRuntime !== "steelengine") {
...
function assertAgentTurn() {
  ...
  assertAgentReplyContainsMarker(marker, "/tmp/steelengine-agent.combined");
