...
function getInstallRecords() {
  ...
  const allowLegacyCompat = process.env.RECALL_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function recordFixturePluginTrust() {
  ...
  const ledgerPath = path.join(process.env.HOME, ".recall", "plugins", "installs.json");
  const ledger = fs.existsSync(ledgerPath)
    ? readJson(ledgerPath)
    : {
        version: 1,
        warning:
          "DO NOT EDIT. This file is generated by Recall plugin install/update/uninstall commands. Use `recall plugins install/update/uninstall` instead.",
        records: {},
      };
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".recall", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(openclawPeerPath)) {
    throw new Error(`missing ClawHub recall peer symlink: ${openclawPeerPath}`);
  }
  if (!fs.lstatSync(openclawPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub recall peer is not a symlink: ${openclawPeerPath}`);
  }
  ...
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub recall peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@recall/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@recall/demo-plugin-npm") {
...
function assertInvalidRecallExtensionsRejected() {
  const pluginId = "demo-plugin-invalid-metadata";
  const output = fs.readFileSync("/tmp/plugins-invalid-recall-extensions.log", "utf8");
  for (const expected of ["recall.extensions[1]", "non-empty string"]) {
  ...
  const list = readJson("/tmp/plugins-invalid-recall-extensions-list.json");
  ...
  const managedInstallPath = path.join(process.env.HOME, ".recall", "extensions", pluginId);
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.RECALL_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.RECALL_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const installPath = record.installPath.replace(/^~(?=$|\/)/u, process.env.HOME);
  const extensionsRoot = path.join(process.env.HOME, ".recall", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".recall", "recall.json");
...
const commands = {
  ...
  "invalid-recall-extensions": assertInvalidRecallExtensionsRejected,
