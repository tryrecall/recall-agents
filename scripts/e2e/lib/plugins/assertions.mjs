...
const scratchRoot = process.env.STEELENGINE_PLUGINS_TMP_DIR || os.tmpdir();
...
function readClawHubPreflightLimits() {
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "STEELENGINE_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES",
      1024 * 1024,
    ),
    timeoutMs: readPositiveIntEnv("STEELENGINE_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS", 30_000),
  };
}
...
function getInstallRecords() {
  const configPath = steelEngineConfigPath();
  const config = readSteelEngineConfig();
  const allowLegacyCompat = process.env.STEELENGINE_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function steelEngineConfigPath() {
  return path.join(process.env.HOME, ".steelengine", "steelengine.json");
}

function readSteelEngineConfig() {
  const configPath = steelEngineConfigPath();
  return fs.existsSync(configPath) ? readRequiredSteelEngineConfig() : {};
}

function readRequiredSteelEngineConfig() {
  const configPath = steelEngineConfigPath();
  try {
    return readJson(configPath);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`failed to read SteelEngine config ${configPath}: ${message}`, { cause: error });
  }
}
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".steelengine", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(steelenginePeerPath)) {
    throw new Error(`missing ClawHub steelengine peer symlink: ${steelenginePeerPath}`);
  }
  if (!fs.lstatSync(steelenginePeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub steelengine peer is not a symlink: ${steelenginePeerPath}`);
  }
  ...
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub steelengine peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@steelengine/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@steelengine/demo-plugin-npm") {
...
function assertInvalidSteelEngineExtensionsRejected() {
  const pluginId = "demo-plugin-invalid-metadata";
  for (const expected of ["steelengine.extensions[1]", "non-empty string"]) {
    assertTextFileIncludes(
      scratchFile("plugins-invalid-steelengine-extensions.log"),
      expected,
      "malformed metadata install output",
    );
  }

  const list = readJson(scratchFile("plugins-invalid-steelengine-extensions-list.json"));
  ...
  const managedInstallPath = path.join(process.env.HOME, ".steelengine", "extensions", pluginId);
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.STEELENGINE_CLAWHUB_URL ||
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".steelengine", "steelengine.json");
...
const commands = {
  ...
  "invalid-steelengine-extensions": assertInvalidSteelEngineExtensionsRejected,
