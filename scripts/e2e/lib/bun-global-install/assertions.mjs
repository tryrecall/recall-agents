...
const runWithTimeout = async (timeout, command, commandArgs) => {
  const killGrace = parsePositiveNumber(
    process.env.STEELENGINE_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS ??
      String(DEFAULT_TIMEOUT_KILL_GRACE_MS),
    "STEELENGINE_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS",
  );
...
if (mode === "assert-image-providers") {
  const raw = process.env.STEELENGINE_IMAGE_PROVIDERS_JSON ?? "";
  let parsed;
  try {
...
if (mode === "assert-release-versions") {
  ...
  const rootAiVersion = rootManifest.dependencies?.["@steelengine/ai"];
  if (
    ...
    throw new Error(
      `candidate version mismatch: steelengine=${String(rootVersion)}, dependency=${String(rootAiVersion)}, @steelengine/ai=${String(aiVersion)}`,
    );
  }
