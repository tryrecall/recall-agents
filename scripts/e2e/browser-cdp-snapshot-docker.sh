...
BASE_IMAGE="$(docker_e2e_resolve_image "recall-browser-cdp-base-e2e" RECALL_BROWSER_CDP_BASE_E2E_IMAGE)"
IMAGE_NAME="$(docker_e2e_resolve_image "recall-browser-cdp-snapshot-e2e" RECALL_BROWSER_CDP_SNAPSHOT_E2E_IMAGE)"
SKIP_BUILD="${RECALL_BROWSER_CDP_SNAPSHOT_E2E_SKIP_BUILD:-0}"
PORT="18789"
CDP_PORT="19222"
FIXTURE_PORT="18080"
TOKEN="browser-cdp-e2e-token"
CONTAINER_NAME="recall-browser-cdp-e2e-$$"
DOCKER_COMMAND_TIMEOUT="${RECALL_BROWSER_CDP_SNAPSHOT_DOCKER_COMMAND_TIMEOUT:-900s}"
...
if [ "${RECALL_SKIP_DOCKER_BUILD:-0}" = "1" ] || [ "$SKIP_BUILD" = "1" ]; then
  ...
  build_dir="$(mktemp -d "${TMPDIR:-/tmp}/recall-browser-cdp-build.XXXXXX")"
...
RECALL_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 browser-cdp-snapshot empty)"
...
mkdir -p /tmp/recall-browser-cdp/chrome
find dist -maxdepth 1 -type f -name 'pw-ai-*.js' ! -name 'pw-ai-state-*' -exec mv {} /tmp/recall-browser-cdp/ \;
PORT=$PORT CDP_PORT=$CDP_PORT node scripts/e2e/lib/fixture.mjs browser-cdp
chromium --headless=new --no-sandbox --disable-gpu --disable-dev-shm-usage \\
  --remote-debugging-address=127.0.0.1 \\
  --remote-debugging-port=$CDP_PORT \\
  --user-data-dir=/tmp/recall-browser-cdp/chrome \\
...
set -euo pipefail
source /tmp/recall-test-state-env
source scripts/lib/recall-e2e-instance.sh
entry=\"\$(openclaw_e2e_resolve_entrypoint)\"
