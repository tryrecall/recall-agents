#!/usr/bin/env bash
# Runs Open WebUI against a Dockerized Recall Gateway and verifies the proxied
...
IMAGE_NAME="$(docker_e2e_resolve_image "recall-openwebui-e2e" RECALL_OPENWEBUI_E2E_IMAGE)"
...
MODEL="${RECALL_OPENWEBUI_MODEL:-openai/gpt-5.5}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${RECALL_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${RECALL_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${RECALL_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${RECALL_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${RECALL_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="recall-openwebui-e2e-$$"
GW_NAME="recall-openwebui-gateway-$$"
OW_NAME="recall-openwebui-$$"
PROVIDER_TIMEOUT_SECONDS="${RECALL_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS:-900}"
PROBE_FETCH_TIMEOUT_MS="${RECALL_OPENWEBUI_FETCH_TIMEOUT_MS:-$((PROVIDER_TIMEOUT_SECONDS * 1000 + 60000))}"
DOCKER_COMMAND_TIMEOUT="${RECALL_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-$((PROVIDER_TIMEOUT_SECONDS + 90))s}"
DOCKER_PULL_TIMEOUT="${RECALL_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"
...
PROFILE_FILE="${RECALL_TESTBOX_PROFILE_FILE:-$HOME/.recall-testbox-live.profile}"
...
docker_e2e_docker_cmd run -d \
  ...
  -e "RECALL_OPENWEBUI_MODEL=$MODEL" \
  -e "RECALL_SKIP_CHANNELS=1" \
  -e "RECALL_SKIP_GMAIL_WATCHER=1" \
  -e "RECALL_SKIP_CRON=1" \
  -e "RECALL_SKIP_CANVAS_HOST=1" \
  -e "RECALL_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=$PROVIDER_TIMEOUT_SECONDS" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/recall-openwebui-config.XXXXXX.json)"
    RECALL_CONFIG_BATCH_PATH="$batch_file" node scripts/e2e/lib/fixture.mjs openwebui-config "$openai_api_key"
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 240 1 "RECALL_HTTP_PROBE_BEARER='$TOKEN' node scripts/e2e/lib/openwebui/http-probe.mjs 'http://127.0.0.1:$PORT/v1/models' 200"; then
...
docker_e2e_docker_cmd run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Recall E2E" \
  -e WEBUI_SECRET_KEY="recall-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Recall E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="recall/default" \
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 90 5 "RECALL_HTTP_PROBE_BEARER='$TOKEN' RECALL_HTTP_PROBE_TIMEOUT_MS=8000 node scripts/e2e/lib/openwebui/http-probe.mjs 'http://$GW_NAME:$PORT/v1/models' 200"; then
...
echo "Running Open WebUI -> Recall smoke..."
