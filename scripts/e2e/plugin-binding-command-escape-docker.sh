...
IMAGE_NAME="${RECALL_PLUGIN_BINDING_COMMAND_ESCAPE_E2E_IMAGE:-recall-plugin-binding-command-escape-e2e}"
CONTAINER_NAME="recall-plugin-binding-command-escape-e2e-$$"
RUN_LOG="$(mktemp -t recall-plugin-binding-command-escape-log.XXXXXX)"
...
docker run --rm \
  ...
  -e RECALL_VITEST_FS_MODULE_CACHE_PATH=/tmp/recall-vitest-cache \
  "$IMAGE_NAME" \
