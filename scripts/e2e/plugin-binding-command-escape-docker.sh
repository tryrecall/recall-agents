...
IMAGE_NAME="${STEELENGINE_PLUGIN_BINDING_COMMAND_ESCAPE_E2E_IMAGE:-steelengine-plugin-binding-command-escape-e2e}"
CONTAINER_NAME="steelengine-plugin-binding-command-escape-e2e-$$"
DOCKER_RUN_TIMEOUT="${STEELENGINE_PLUGIN_BINDING_COMMAND_ESCAPE_DOCKER_RUN_TIMEOUT:-900s}"
RUN_LOG="$(mktemp -t steelengine-plugin-binding-command-escape-log.XXXXXX)"
...
DOCKER_COMMAND_TIMEOUT="$DOCKER_RUN_TIMEOUT" docker_e2e_docker_run_cmd run --rm \
  ...
  -e STEELENGINE_VITEST_FS_MODULE_CACHE_PATH=/tmp/steelengine-vitest-cache \
  "$IMAGE_NAME" \
