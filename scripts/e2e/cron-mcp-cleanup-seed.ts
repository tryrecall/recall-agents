...
import { applyDockerOpenAiProviderConfig, type RecallConfig } from "./docker-openai-seed.ts";
...
process.title = "recall-cron-mcp-cleanup-probe";
...
async function main() {
  const stateDir = process.env.RECALL_STATE_DIR?.trim() || path.join(os.homedir(), ".recall");
  const configPath =
    process.env.RECALL_CONFIG_PATH?.trim() || path.join(stateDir, "recall.json");
