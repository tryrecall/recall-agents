...
export function resolveCompileConcurrency(
  ...
  const raw = env.STEELENGINE_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "STEELENGINE_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.STEELENGINE_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";
