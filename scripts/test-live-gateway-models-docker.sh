...
LIVE_GATEWAY_MAX_MODELS="$(steelengine_live_read_positive_int_env STEELENGINE_LIVE_GATEWAY_MAX_MODELS 8)"
LIVE_GATEWAY_STEP_TIMEOUT_MS="$(steelengine_live_read_positive_int_env STEELENGINE_LIVE_GATEWAY_STEP_TIMEOUT_MS 45000)"
LIVE_GATEWAY_MODEL_TIMEOUT_MS="$(steelengine_live_read_positive_int_env STEELENGINE_LIVE_GATEWAY_MODEL_TIMEOUT_MS 90000)"
TEMP_DIRS=()
DOCKER_HOME_MOUNT=()
...
elif [[ -n "${STEELENGINE_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(steelengine_live_collect_auth_dirs_from_csv "${STEELENGINE_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(steelengine_live_collect_auth_files_from_csv "${STEELENGINE_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
DOCKER_RUN_ARGS=()
steelengine_live_init_docker_run_args DOCKER_RUN_ARGS "${STEELENGINE_LIVE_GATEWAY_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e STEELENGINE_LIVE_GATEWAY_MODELS="${STEELENGINE_LIVE_GATEWAY_MODELS:-modern}" \
  -e STEELENGINE_LIVE_GATEWAY_PROVIDERS="${STEELENGINE_LIVE_GATEWAY_PROVIDERS:-}" \
  -e STEELENGINE_LIVE_GATEWAY_THINKING="${STEELENGINE_LIVE_GATEWAY_THINKING:-}" \
  -e STEELENGINE_LIVE_GATEWAY_SMOKE="${STEELENGINE_LIVE_GATEWAY_SMOKE:-1}" \
  -e STEELENGINE_LIVE_GATEWAY_MAX_MODELS="$LIVE_GATEWAY_MAX_MODELS" \
  -e STEELENGINE_LIVE_GATEWAY_HEARTBEAT_MS="${STEELENGINE_LIVE_GATEWAY_HEARTBEAT_MS:-}" \
  -e STEELENGINE_LIVE_GATEWAY_STEP_TIMEOUT_MS="$LIVE_GATEWAY_STEP_TIMEOUT_MS" \
  -e STEELENGINE_LIVE_GATEWAY_MODEL_TIMEOUT_MS="$LIVE_GATEWAY_MODEL_TIMEOUT_MS" \
