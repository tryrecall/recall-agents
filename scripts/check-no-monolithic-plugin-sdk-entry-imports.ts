...
import { discoverRecallPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']recall\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']recall\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']recall\/plugin-sdk\/channel-runtime["']/,
    label: "recall/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']recall\/plugin-sdk\/config-runtime["']/,
    label: "recall/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']recall\/plugin-sdk\/infra-runtime["']/,
    label: "recall/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic recall/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy recall/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused recall/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
