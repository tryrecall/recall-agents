#!/usr/bin/env node
// Builds the SteelEngine package artifact used by Docker E2E.
...
const AI_RUNTIME_PACKAGE = "@steelengine/ai";
const AI_RUNTIME_BACKUP_DIR = ".steelengine-ai-package-backup";
const ACTIVE_CHILD_KILLERS = new Set();
const PACKAGE_BUILD_PLUGIN_SELECTION_ENV_NAMES = [
  "STEELENGINE_EXTENSIONS",
  "STEELENGINE_DOCKER_BUILD_EXTENSIONS",
...
function resolvePackedSteelEngineFileName(value) {
  const filename = value.trim();
  if (
    !filename.endsWith(".tgz") ||
    (!filename.startsWith("steelengine-") &&
      !filename.includes(":") &&
      !filename.includes("/") &&
      !filename.includes("\\"))
  ) {
  ...
  if (
    !/^steelengine-[A-Za-z0-9._-]+\.tgz$/u.test(filename) ||
    filename.includes("\0") ||
    filename !== path.basename(filename) ||
    filename !== path.win32.basename(filename)
  ) {
    throw new Error(`npm pack reported unsafe SteelEngine tarball filename: ${filename}`);
  }
  return filename;
}
...
const PACKAGE_ARTIFACT_BUILD_STEPS = [
  {
    label: "Building SteelEngine package artifacts",
    command: "node",
    ...
    // STEELENGINE_RUN_NODE_SKIP_DTS_BUILD=0 this packaging path requires.
...
export async function buildPackageArtifacts(sourceDir, options = {}) {
  ...
  const buildEnv = {
    ...
    STEELENGINE_RUN_NODE_SKIP_DTS_BUILD: "0",
  };
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      ...
      timeoutMs: resolveTimeoutMs(
        "STEELENGINE_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
...
async function newestSteelEngineTarball(outputDir, packOutput) {
  let fromOutput = "";
  try {
    ...
    if (Array.isArray(parsed)) {
      for (const entry of parsed) {
        ...
        const filename = resolvePackedSteelEngineFileName(entry.filename);
        if (filename) {
          fromOutput = filename;
        }
      }
  ...
  for (const line of packOutput.split(/\r?\n/u)) {
    const filename = resolvePackedSteelEngineFileName(line);
    if (filename) {
      fromOutput = filename;
    }
  }
  ...
  const packed = entries
    .filter((entry) => {
      try {
        return resolvePackedSteelEngineFileName(entry) === entry;
      } catch {
        return false;
      }
    })
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed SteelEngine tarball in ${outputDir}`);
  }
...
async function cleanPackedSteelEngineTarballs(outputDir) {
  let entries;
  try {
...
function isPackedAiRuntimeTarball(filename) {
  return /^steelengine-ai-[A-Za-z0-9._-]+\.tgz$/u.test(filename);
}
...
export async function prepareBundledAiRuntimePackage(
  ...
  const aiRuntimePath = path.join(sourceDir, "node_modules", "@steelengine", "ai");
  ...
  const extractAiRuntime =
    ...
    ((tarballPath, destination) =>
      ...
      run("tar", ["-xzf", tarballPath, "-C", destination, "--strip-components=1"], destination, {
        timeoutMs: resolveTimeoutMs(
          "STEELENGINE_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
  ...
  if (!hasAiRuntimeWorkspace) {
    throw new Error("@steelengine/ai dependency requires the packages/ai workspace");
  }
  if (typeof aiRuntimeDependency !== "string") {
    throw new Error("root package.json must declare @steelengine/ai as a dependency");
  }
  ...
  try {
    ...
    if (packedAiTarballs.length !== 1) {
      throw new Error(
        `expected one packed @steelengine/ai tarball in ${outputDir}, found ${packedAiTarballs.length}`,
      );
    }
    ...
    if (typeof stagedPackageJson.version !== "string" || !stagedPackageJson.version) {
      throw new Error("packed @steelengine/ai package must declare a version");
    }
    for (const [name, version] of Object.entries(stagedPackageJson.dependencies ?? {})) {
      if (packageJson.dependencies[name] !== version) {
        throw new Error(
          `root package.json must declare ${name}@${version} to bundle @steelengine/ai without duplicate dependencies`,
        );
      }
    }
...
export async function packSteelEnginePackageForDocker(sourceDir, outputDir, options = {}) {
  ...
  console.error("==> Packing SteelEngine package");
  await prepareChangelog(sourceDir);
  let packOutput;
  let cleanupBundledAiRuntime = async () => {};
  try {
    await cleanPackedSteelEngineTarballs(outputDir);
  ...
  let tarball = await newestSteelEngineTarball(outputDir, options.pnpmPack ? "" : packOutput);
  if (options.outputName) {
...
export async function writePackageInventoryForDocker(sourceDir, runImpl = run) {
  ...
  await runImpl(
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "STEELENGINE_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
...
async function main() {
  ...
  console.error("==> Writing SteelEngine package inventory");
  ...
  console.error("==> Checking SteelEngine package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [
      path.join(ROOT_DIR, "scripts/check-steelengine-package-tarball.mjs"),
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "STEELENGINE_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",
        DEFAULT_PACKAGE_TARBALL_CHECK_TIMEOUT_MS,
      ),
    },
  );
  console.error(
    `==> SteelEngine package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
