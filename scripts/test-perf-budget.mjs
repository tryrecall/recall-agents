...
function parseArgs(argv, env = process.env) {
  const opts = parseFlagArgs(
    argv,
    {
      ...
      maxWallMs: readBudgetEnvNumber("STEELENGINE_TEST_PERF_MAX_WALL_MS", env),
      baselineWallMs: readBudgetEnvNumber("STEELENGINE_TEST_PERF_BASELINE_WALL_MS", env),
      maxRegressionPct: readBudgetEnvNumber("STEELENGINE_TEST_PERF_MAX_REGRESSION_PCT", env) ?? 10,
      reportOnly: readBooleanEnv("STEELENGINE_TEST_PERF_REPORT_ONLY", env),
    },
    [
...
function main() {
  ...
  const reportPath = runVitestJsonReport({
    config: opts.config,
    prefix: "steelengine-vitest-perf",
  });
