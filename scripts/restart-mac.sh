...
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.recall.mac.plist"
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.recall.mac 2>/dev/null || true
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.recall.gateway.plist' | head -n 40 || true"
fi
