#!/usr/bin/env bash
# Reset Recall like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${RECALL_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Recall.app/Contents/MacOS/Recall"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Recall"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Recall"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Recall"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.tryrecall.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/recall-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${RECALL_RESTART_LOG:-/tmp/recall-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${RECALL_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.recall/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  RECALL_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node recall.mjs daemon install --force --runtime node"
      log "  node recall.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.recall/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.tryrecall.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Recall instances"
kill_all_recall
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Recall"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Recall.app" ]]; then
    APP_BUNDLE="/Applications/Recall.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Recall.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Recall.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Recall.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set RECALL_APP_BUNDLE to your installed Recall.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node recall.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node recall.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".recall", "recall.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: Recall is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.tryrecall.gateway.plist' | head -n 40 || true"
fi
