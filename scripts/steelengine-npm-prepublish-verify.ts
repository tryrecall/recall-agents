...
// Openclaw Npm Prepublish Verify script supports SteelEngine repository automation.
...
import { resolveNpmCommandInvocation } from "./steelengine-npm-release-check.ts";
import {
  assertPreparedSteelEngineNpmShrinkwrap,
  npmTarballIntegrity,
  readTarballJson,
} from "./prepare-steelengine-npm-shrinkwrap.ts";
...
type SteelEngineNpmPrepublishVerifyArgs =
  | {
...
export function steelEngineNpmPrepublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/steelengine-npm-prepublish-verify.ts <tarball.tgz> [expected-version] [dependency.tgz ...]";
}

export function parseSteelEngineNpmPrepublishVerifyArgs(
  argv: readonly string[],
): SteelEngineNpmPrepublishVerifyArgs {
  ...
  if (!tarballPath) {
    throw new Error(steelEngineNpmPrepublishVerifyUsage());
  }
  if (tarballPath.startsWith("-")) {
    throw new Error(`Unknown steelengine npm prepublish verifier option: ${tarballPath}`);
  }

  const expectedVersion = args[1]?.trim();
  if (expectedVersion?.startsWith("-")) {
    throw new Error(`Unknown steelengine npm prepublish verifier option: ${expectedVersion}`);
  }
...
function main(argv = process.argv.slice(2)): void {
  const args = parseSteelEngineNpmPrepublishVerifyArgs(argv);
  if (args.help) {
    console.log(steelEngineNpmPrepublishVerifyUsage());
    return;
  }

  const workingDir = mkdtempSync(join(tmpdir(), "steelengine-prepublish-"));
  ...
  try {
    ...
    if (usesPreparedLocalDependencyInstall(args.dependencyTarballPaths.length)) {
        ...
        `${JSON.stringify(
          {
            private: true,
            dependencies: {
              "@steelengine/ai": pathToFileURL(aiTarballPath).href,
              steelengine: pathToFileURL(realpathSync(args.tarballPath)).href,
            },
          },
          null,
          2,
      ...
      packageRoot = join(prefixDir, "node_modules", "steelengine");
      const binaryPath = join(
        prefixDir,
        "node_modules",
        ".bin",
        process.platform === "win32" ? "steelengine.cmd" : "steelengine",
      );
      binaryInvocation =
      ...
      packageRoot = join(globalRoot, "steelengine");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== resolvedExpectedVersion) {
      errors.push(
        `installed steelengine binary version mismatch: expected ${resolvedExpectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    if (errors.length === 0) {
    ...
    console.log(
      `steelengine-npm-prepublish-verify: prepared tarball install OK (${resolvedExpectedVersion}).`,
    );
  } finally {
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`steelengine-npm-prepublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
