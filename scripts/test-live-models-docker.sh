...
if [[ -n "${RECALL_DOCKER_AUTH_DIRS:-}" ]]; then
  ...
  while IFS= read -r auth_dir; do
    ...
    {
      recall_live_collect_auth_dirs_from_csv "${RECALL_LIVE_PROVIDERS:-}"
      recall_live_collect_auth_dirs_from_csv "${RECALL_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
docker run --rm -t \
  ...
  -e RECALL_LIVE_MAX_MODELS="${RECALL_LIVE_MAX_MODELS:-48}" \
