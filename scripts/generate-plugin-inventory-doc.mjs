...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving Recall messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to Recall.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an Recall plugin.";
}
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.recall?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    return "included in Recall";
  }
  const install = packageJson.recall?.install;
  const release = packageJson.recall?.release;
  const clawhubSpec =
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.recall?.install?.clawhubSpec === "string" ||
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "recall.plugin.json");
...
function renderDocument() {
  ...
  return `---
summary: "Generated inventory of Recall plugins shipped in core, published externally, or kept source-only"
read_when:
...
This page is generated from \`extensions/*/package.json\`, \`recall.plugin.json\`,
...
- **Core npm package:** built into the \`recall\` npm package and available without a separate plugin install.
- **Official external package:** Recall-maintained plugin omitted from the core npm package, kept in this official inventory, and installed on demand through ClawHub and/or npm.
...
say \`included in Recall\` are already present in the core package. Official
external packages need one install, then a Gateway restart.

For example, Discord is an official external package:

\`\`\`bash
recall plugins install @recall/discord
recall gateway restart
recall plugins inspect discord --runtime --json
\`\`\`

During the launch cutover, ordinary bare package specs still install from npm.
Use \`clawhub:@recall/discord\` or \`npm:@recall/discord\` when you need an
