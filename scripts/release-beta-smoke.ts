...
// Release Beta Smoke script supports SteelEngine repository automation.
...
Options:
  ...
  --repo <owner/repo>          GitHub repo. Default: steelengine/steelengine
...
const DEFAULT_COMMAND_TIMEOUT_MS = readPositiveInt(
  process.env.STEELENGINE_RELEASE_BETA_SMOKE_COMMAND_MS,
  10 * 60_000,
  "STEELENGINE_RELEASE_BETA_SMOKE_COMMAND_MS",
);
const TELEGRAM_POLL_INTERVAL_MS = readPositiveInt(
  process.env.STEELENGINE_RELEASE_BETA_SMOKE_POLL_INTERVAL_MS,
  30_000,
  "STEELENGINE_RELEASE_BETA_SMOKE_POLL_INTERVAL_MS",
);
const TELEGRAM_POLL_TIMEOUT_MS = readPositiveInt(
  process.env.STEELENGINE_RELEASE_BETA_SMOKE_POLL_TIMEOUT_MS,
  4 * 60 * 60_000,
  "STEELENGINE_RELEASE_BETA_SMOKE_POLL_TIMEOUT_MS",
);
...
function resolveBetaVersion(beta: string): string {
  const value = beta.trim().replace(/^steelengine@/, "");
  if (/^\d{4}\.\d+\.\d+-beta\.\d+$/u.test(value)) {
    return value;
  }
  if (value === "beta") {
    return run("npm", ["view", "steelengine@beta", "version"], { capture: true }).trim();
  }
  const betaMatch = /^(?:beta)?(\d+)$/u.exec(value);
  if (!betaMatch) {
    return run("npm", ["view", `steelengine@${value}`, "version"], { capture: true }).trim();
  }
  const suffix = `-beta.${betaMatch[1]}`;
  const versions = JSON.parse(
    run("npm", ["view", "steelengine", "versions", "--json"], { capture: true }),
  ) as string[];
  const match = versions
    ...
    .at(-1);
  if (!match) {
    throw new Error(`no steelengine registry version found for ${beta}`);
  }
  return match;
}
  ...
  const result = spawnSync(
    ...
    [
      ...
      [
        "set -euo pipefail",
        'token="$(gh auth token)"',
        'curl -fsS -H "Authorization: Bearer ${token}" -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" "${STEELENGINE_GITHUB_REST_URL}"',
      ].join("\n"),
    ],
    {
      encoding: "utf8",
      env: { ...process.env, STEELENGINE_GITHUB_REST_URL: url },
      killSignal: "SIGKILL",
      maxBuffer: CAPTURE_MAX_BUFFER_BYTES,
...
function appendTelegramProofToRelease(repo: string, version: string, runId: string): void {
  ...
  const notesFile = path.join(
    "/tmp",
    `steelengine-${version.replace(/[^a-zA-Z0-9.-]/g, "-")}-release-notes-${process.pid}.md`,
  );
...
async function main(): Promise<void> {
  ...
  const packageSpec = `steelengine@${version}`;
