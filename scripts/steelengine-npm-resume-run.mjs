...
const WORKFLOW_PATH = ".github/workflows/steelengine-npm-release.yml";
...
function requiredString(value, label) {
  if (typeof value !== "string" || value.length === 0) {
    fail(`SteelEngine npm resume run is missing ${label}.`);
  }
  return value;
}
...
function requiredSha(value, label) {
  ...
  if (!SHA_PATTERN.test(sha)) {
    fail(`SteelEngine npm resume run has invalid ${label}.`);
  }
  return sha;
}
...
export function validateSteelEngineNpmResumeRun({
  ...
  if (!branchMatch) {
    fail(`SteelEngine npm resume run has an untrusted workflow ref: ${url}`);
  }
  ...
  if (
    ...
    fail(`SteelEngine npm resume run has an untrusted workflow identity: ${url}`);
  }

  const tagObjectSha = requiredSha(tagRef?.object?.sha, "tooling tag object SHA");
  if (tagRef?.object?.type !== "tag") {
    fail(`SteelEngine npm resume run tooling ref is not a signed annotated tag: ${url}`);
  }
  ...
  if (
    ...
    fail(
      `SteelEngine npm resume run is not bound to a real, main-reachable protected tooling tag: ${url}`,
    );
  }
  ...
  if (
    ...
    fail(`SteelEngine npm resume run lacks successful parent release approval validation: ${url}`);
  }
...
export function runSteelEngineNpmResumeGh(args, params = {}) {
...
export function resolveSteelEngineNpmResumeRun({ repo, runId, runGh = runSteelEngineNpmResumeGh }) {
  if (!/^[1-9][0-9]*$/u.test(runId)) {
    fail("SteelEngine npm resume run id must be a positive integer.");
  }
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/u.test(repo)) {
    fail("SteelEngine npm resume repository must be owner/name.");
  }
  ...
  return validateSteelEngineNpmResumeRun({
...
function main(argv = process.argv.slice(2)) {
  const result = resolveSteelEngineNpmResumeRun(parseArgs(argv));
