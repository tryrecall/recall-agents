...
BASE_IMAGE="${BASE_IMAGE:-steelengine-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-steelengine-sandbox-common:bookworm-slim}"
...
STEELENGINE_DOCKER_BUILD_USE_BUILDX="${STEELENGINE_DOCKER_BUILD_USE_BUILDX:-0}"
STEELENGINE_DOCKER_BUILD_CACHE_FROM="${STEELENGINE_DOCKER_BUILD_CACHE_FROM:-}"
STEELENGINE_DOCKER_BUILD_CACHE_TO="${STEELENGINE_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=steelengine.sandbox=1)
NOTE
