...
BASE_IMAGE="${BASE_IMAGE:-recall-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-recall-sandbox-common:bookworm-slim}"
...
RECALL_DOCKER_BUILD_USE_BUILDX="${RECALL_DOCKER_BUILD_USE_BUILDX:-0}"
RECALL_DOCKER_BUILD_CACHE_FROM="${RECALL_DOCKER_BUILD_CACHE_FROM:-}"
RECALL_DOCKER_BUILD_CACHE_TO="${RECALL_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${RECALL_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=recall.sandbox=1)
NOTE
