...
// metadata expected by SteelEngine.
...
export function collectPluginNpmPublishedRuntimeErrors(params) {
  ...
  const runtimeExtensionsResult = readPackageStringList(
    packageLabel,
    "steelengine.runtimeExtensions",
    packageJson.steelengine?.runtimeExtensions,
  );
  const setupEntryResult = readOptionalPackageString(
    packageLabel,
    "steelengine.setupEntry",
    packageJson.steelengine?.setupEntry,
  );
  const runtimeSetupEntryResult = readOptionalPackageString(
    packageLabel,
    "steelengine.runtimeSetupEntry",
    packageJson.steelengine?.runtimeSetupEntry,
  );
  errors.push(
  ...
  if (!hasPackedFile(packageFiles, "steelengine.plugin.json")) {
    errors.push(`${packageLabel} plugin npm package must include steelengine.plugin.json`);
    return errors;
  }
  ...
  if (runtimeExtensions.length > 0 && runtimeExtensions.length !== extensions.length) {
    errors.push(
      `${packageLabel} package.json steelengine.runtimeExtensions length (${runtimeExtensions.length}) must match steelengine.extensions length (${extensions.length})`,
    );
    return errors;
  }
  ...
  if (runtimeSetupEntry && !setupEntry) {
    errors.push(
      `${packageLabel} package.json steelengine.runtimeSetupEntry requires steelengine.setupEntry`,
    );
    return errors;
  }
...
export function readPluginNpmCommandOptions(env = process.env) {
  return {
    ...
    maxBuffer: readPositiveIntEnv(
      "STEELENGINE_PLUGIN_NPM_COMMAND_MAX_BUFFER_BYTES",
      DEFAULT_NPM_COMMAND_MAX_BUFFER_BYTES,
      env,
    ),
    stdio: ["ignore", "pipe", "pipe"],
    timeout: readPositiveIntEnv(
      "STEELENGINE_PLUGIN_NPM_COMMAND_TIMEOUT_MS",
...
async function packPublishedPackage(spec, destinationDir) {
  const attempts = readPositiveIntEnv("STEELENGINE_PLUGIN_NPM_VERIFY_ATTEMPTS", 90);
  const delayMs = readPositiveIntEnv("STEELENGINE_PLUGIN_NPM_VERIFY_DELAY_MS", 10000);
  let lastError;
...
async function verifyPublishedPackageReadme(spec) {
  const attempts = readPositiveIntEnv("STEELENGINE_PLUGIN_NPM_README_VERIFY_ATTEMPTS", 6);
  const delayMs = readPositiveIntEnv("STEELENGINE_PLUGIN_NPM_README_VERIFY_DELAY_MS", 10000);
  let lastError;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
...
async function verifyPublishedPluginRuntime(spec) {
  const workingDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-plugin-npm-runtime."));
  try {
