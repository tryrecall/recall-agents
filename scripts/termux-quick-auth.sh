...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/steelengine/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "SteelEngine Re-Auth" -c "After getting key, run: ssh $SERVER '~/steelengine/scripts/mobile-reauth.sh'" --id steelengine-auth
        ;;
    *)
