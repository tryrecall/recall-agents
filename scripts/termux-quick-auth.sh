...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/recall/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Recall Re-Auth" -c "After getting key, run: ssh $SERVER '~/recall/scripts/mobile-reauth.sh'" --id recall-auth
        ;;
    *)
