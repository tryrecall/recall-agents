...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${RECALL_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$RECALL_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.RECALL_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `RECALL_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${RECALL_INSTALL_SMOKE_IMAGE:-recall-install-smoke:local}"
NONROOT_IMAGE="${RECALL_INSTALL_NONROOT_IMAGE:-recall-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${RECALL_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${RECALL_INSTALL_URL:-https://recall.bot/install.sh}"
CLI_INSTALL_URL="${RECALL_INSTALL_CLI_URL:-https://recall.bot/install-cli.sh}"
PACKAGE_NAME="${RECALL_INSTALL_PACKAGE:-recall}"
SKIP_NONROOT="${RECALL_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${RECALL_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${RECALL_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${RECALL_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${RECALL_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
SKIP_FRESHNESS="${RECALL_INSTALL_SMOKE_SKIP_FRESHNESS:-0}"
FRESHNESS_INSTALL_URL="${RECALL_INSTALL_SMOKE_FRESHNESS_INSTALL_URL:-file:///tmp/recall-install.sh}"
...
UPDATE_BASELINE_VERSION="${RECALL_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"
UPDATE_PACKAGE_SPEC="${RECALL_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${RECALL_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${RECALL_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${RECALL_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${RECALL_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${RECALL_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
NPM_CACHE_DIR="${RECALL_INSTALL_SMOKE_NPM_CACHE_DIR:-}"
NPM_CACHE_OWNED=0
NPM_CACHE_PREPARED=0
NPM_CACHE_DOCKER_ARGS=()
INSTALL_SCRIPT_DOCKER_ARGS=(
  -v "$ROOT_DIR/scripts/install.sh:/tmp/recall-install.sh:ro"
  -v "$ROOT_DIR/scripts/install-cli.sh:/tmp/recall-install-cli.sh:ro"
)
...
' "$pack_json_file"
  ...
  if [[ -z "$UPDATE_PACKAGE_SPEC" ]]; then
    node scripts/check-recall-package-tarball.mjs "${UPDATE_DIR}/${UPDATE_TGZ_FILE}"
  fi
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (RECALL_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  prepare_npm_cache
  ...
  docker run --rm -t \
    ...
    -e RECALL_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e RECALL_INSTALL_METHOD=npm \
    -e RECALL_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e RECALL_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e RECALL_INSTALL_LATEST_OUT="/out/latest" \
    -e RECALL_NO_ONBOARD=1 \
    -e RECALL_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    "${NPM_CACHE_DOCKER_ARGS[@]}" \
    -e RECALL_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e RECALL_INSTALL_SMOKE_MODE=update \
    -e RECALL_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e RECALL_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e RECALL_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e RECALL_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e RECALL_NO_ONBOARD=1 \
    -e RECALL_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (RECALL_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    docker run --rm -t \
      ...
      "${NPM_CACHE_DOCKER_ARGS[@]}" \
      -e RECALL_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e RECALL_INSTALL_SMOKE_MODE=npm-global \
      -e RECALL_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e RECALL_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e RECALL_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e RECALL_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_FRESHNESS" == "1" ]]; then
  echo "==> Skip installer npm freshness smoke (RECALL_INSTALL_SMOKE_SKIP_FRESHNESS=1)"
else
  prepare_npm_cache
  ...
  docker run --rm -t \
    ...
    "${NPM_CACHE_DOCKER_ARGS[@]}" \
    "${INSTALL_SCRIPT_DOCKER_ARGS[@]}" \
    -e RECALL_INSTALL_URL="$FRESHNESS_INSTALL_URL" \
    -e RECALL_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e RECALL_INSTALL_SMOKE_MODE=freshness \
    -e RECALL_INSTALL_FRESHNESS_VERSION="${RECALL_INSTALL_FRESHNESS_VERSION:-latest}" \
    -e RECALL_INSTALL_FRESHNESS_MIN_RELEASE_AGE="$FRESHNESS_MIN_RELEASE_AGE" \
    -e RECALL_INSTALL_FRESHNESS_NPM_VERSION="$FRESHNESS_NPM_VERSION" \
    -e RECALL_NO_ONBOARD=1 \
    -e RECALL_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (RECALL_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e RECALL_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e RECALL_INSTALL_METHOD=npm \
    -e RECALL_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e RECALL_NO_ONBOARD=1 \
    -e RECALL_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${RECALL_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (RECALL_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  "${INSTALL_SCRIPT_DOCKER_ARGS[@]}" \
  -e RECALL_INSTALL_URL="$INSTALL_URL" \
  -e RECALL_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e RECALL_NO_ONBOARD=1 \
  -e RECALL_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
