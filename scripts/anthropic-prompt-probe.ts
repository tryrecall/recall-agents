// Anthropic Prompt Probe script supports SteelEngine repository automation.
import { spawn } from "node:child_process";
// Live prompt probe for Anthropic setup-token and Claude CLI prompt-path debugging.
// Usage:
// STEELENGINE_PROMPT_TRANSPORT=direct|gateway
// STEELENGINE_PROMPT_MODE=extra
// STEELENGINE_PROMPT_TEXT='...'
// STEELENGINE_PROMPT_CAPTURE=1
...
const TRANSPORT = process.env.STEELENGINE_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE = "extra";
const PROMPT_TEXT = process.env.STEELENGINE_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.STEELENGINE_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.STEELENGINE_USER_PROMPT?.trim() || "is clawd here?";
const ENABLE_CAPTURE = parseBooleanEnv({
  fallback: false,
  name: "STEELENGINE_PROMPT_CAPTURE",
  raw: process.env.STEELENGINE_PROMPT_CAPTURE,
});
const INCLUDE_RAW = parseBooleanEnv({
  fallback: false,
  name: "STEELENGINE_PROMPT_INCLUDE_RAW",
  raw: process.env.STEELENGINE_PROMPT_INCLUDE_RAW,
});
const KEEP_TMP = parseBooleanEnv({
  fallback: false,
  name: "STEELENGINE_PROMPT_KEEP_TMP",
  raw: process.env.STEELENGINE_PROMPT_KEEP_TMP,
});
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.STEELENGINE_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 45_000,
  label: "STEELENGINE_PROMPT_TIMEOUT_MS",
  min: 1,
  raw: process.env.STEELENGINE_PROMPT_TIMEOUT_MS,
});
const GATEWAY_TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 120_000,
  label: "STEELENGINE_PROMPT_GATEWAY_TIMEOUT_MS",
  min: 1,
  raw: process.env.STEELENGINE_PROMPT_GATEWAY_TIMEOUT_MS,
});
...
const CAPTURE_PROXY_MAX_BODY_BYTES = parseStrictIntegerOption({
  ...
  label: "STEELENGINE_PROMPT_CAPTURE_MAX_BODY_BYTES",
  min: 1,
  raw: process.env.STEELENGINE_PROMPT_CAPTURE_MAX_BODY_BYTES,
});
const GATEWAY_LOG_TAIL_BYTES = 256 * 1024;
const SETUP_TOKEN_RAW = process.env.STEELENGINE_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.STEELENGINE_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.STEELENGINE_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
...
async function resolveSetupTokenSource(): Promise<TokenSource> {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set STEELENGINE_LIVE_SETUP_TOKEN_VALUE or STEELENGINE_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["steelengine.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        ...
        STEELENGINE_AGENT_DIR: params.agentDir,
        ...
        STEELENGINE_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      detached: process.platform !== "win32",
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-gateway-prompt-probe-"));
...
async function main() {
  if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
    throw new Error("missing STEELENGINE_PROMPT_TEXT or STEELENGINE_PROMPT_LIST_JSON");
  }
