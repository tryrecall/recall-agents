#!/usr/bin/env bash
# ClawDock - Docker helpers for SteelEngine
# Inspired by Simon Willison's "Running SteelEngine in Docker"
# https://til.simonwillison.net/llms/steelengine-docker
#
# Installation:
...
# Common paths to check for SteelEngine
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/steelengine"
  "${HOME}/workspace/steelengine"
  "${HOME}/projects/steelengine"
  "${HOME}/dev/steelengine"
  "${HOME}/code/steelengine"
  "${HOME}/src/steelengine"
)
...
_clawdock_browser_url_for_gateway_url() {
  ...
  published=$(_clawdock_compose port steelengine-gateway 18789 2>/dev/null | head -n 1 | tr -d '\r')
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found SteelEngine at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/steelengine"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ SteelEngine not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/steelengineai/recall-agents.git ~/steelengine"
    echo "  cd ~/steelengine && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^STEELENGINE_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d steelengine-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart steelengine-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f steelengine-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.steelengine"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show steelengine.json
  if [[ -f "${config_dir}/steelengine.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/steelengine.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/steelengine.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No steelengine.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec steelengine-gateway \
    bash -c 'echo "alias steelengine=\"./steelengine.mjs\"" > /tmp/.bashrc_steelengine && bash --rcfile /tmp/.bashrc_steelengine'
}
...
clawdock-cli() {
  _clawdock_compose run --rm steelengine-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build steelengine-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d steelengine-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build steelengine-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "STEELENGINE_GATEWAY_TOKEN=$token" steelengine-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" steelengine-gateway \
    bash -c './steelengine.mjs config set gateway.remote.token "$TOKEN" && ./steelengine.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec steelengine-gateway \
    bash -c "./steelengine.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart steelengine-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm --no-deps steelengine-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec steelengine-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'steelengine config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for SteelEngine${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (steelengine alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to steelengine project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.steelengine)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'steelengine channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'steelengine status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.steelengine.ai${_CLR_RESET}"
  echo ""
}
