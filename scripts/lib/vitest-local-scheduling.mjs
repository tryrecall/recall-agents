...
function isSystemThrottleDisabled(env) {
  const normalized = env.STEELENGINE_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestEnv(env = process.env) {
  const normalizedLocalCheck = env.STEELENGINE_LOCAL_CHECK?.trim().toLowerCase();
