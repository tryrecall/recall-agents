// Codex App Server Protocol Source script supports SteelEngine repository automation.
...
export function resolveCodexProtocolMinFreeBytes(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.STEELENGINE_CODEX_PROTOCOL_MIN_FREE_BYTES;
  ...
  if (!Number.isFinite(parsed) || parsed < 0) {
    throw new Error(
      `STEELENGINE_CODEX_PROTOCOL_MIN_FREE_BYTES must be a non-negative byte count, got ${raw}`,
    );
  }
...
export function validateCodexProtocolGenerationHeadroom(params: {
  ...
  throw new Error(
    [
      ...
      "Run this check on Crabbox/Testbox, free local disk, or set STEELENGINE_CODEX_PROTOCOL_MIN_FREE_BYTES=0 to override intentionally.",
    ].join("\n"),
  );
}
...
export async function resolveCodexAppServerProtocolSource(repoRoot: string): Promise<{
  ...
  throw new Error(
    [
      ...
      "Set STEELENGINE_CODEX_REPO to a checkout of openai/codex, or keep a sibling `codex` checkout next to the primary SteelEngine checkout.",
...
async function collectCodexRepoCandidates(repoRoot: string): Promise<string[]> {
  const candidates = [
    process.env.STEELENGINE_CODEX_REPO,
