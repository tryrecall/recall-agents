...
async function runWriterChild(argv: string[]): Promise<void> {
  ...
  try {
    ...
    const next = database
      .prepare(
        "SELECT COALESCE(MAX(batch), -1) + 1 AS next_batch FROM steelengine_reliability_entries",
      )
    ...
    const insertSentinel = database.prepare(
      "INSERT INTO steelengine_reliability_sentinel (id, payload) VALUES (1, ?)",
    );
    const deleteExpired = database.prepare(
      "DELETE FROM steelengine_reliability_entries WHERE batch < ?",
    );
