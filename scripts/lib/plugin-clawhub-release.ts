...
const CLAWHUB_SHARED_RELEASE_INPUT_PATHS = [
  ...
  "scripts/recall-npm-release-check.ts",
...
export function collectClawHubPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.recall?.release?.publishToClawHub !== true) {
      continue;
    }
...
export function collectClawHubVersionGateErrors(params: {
  ...
  for (const plugin of changedPlugins) {
    ...
    if (baseManifest?.recall?.release?.publishToClawHub !== true) {
      continue;
    }
    const baseVersion =
...
export async function collectClawHubRecallOwnerErrors(params: {
  ...
  const requiredOwnerHandle = params.requiredOwnerHandle ?? "recall";
  const errors: string[] = [];

  await Promise.all(
    params.plugins.map(async (plugin) => {
      if (!plugin.packageName.startsWith("@recall/")) {
        return;
      }
      ...
      if (response.status === 404) {
        errors.push(
          `${plugin.packageName}: ClawHub package row must already exist under @${requiredOwnerHandle} before Recall release publish.`,
        );
        return;
      }
      if (!response.ok) {
