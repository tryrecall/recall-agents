// Plugin Clawhub Release script supports SteelEngine repository automation.
...
const STEELENGINE_PLUGIN_CLAWHUB_REPOSITORY = "steelengine/steelengine";
const STEELENGINE_PLUGIN_CLAWHUB_WORKFLOW_FILENAME = "plugin-clawhub-release.yml";
...
const CLAWHUB_SHARED_RELEASE_INPUT_PATHS = [
  ...
  "scripts/steelengine-npm-release-check.ts",
...
export function collectClawHubPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.steelengine?.release?.publishToClawHub !== true) {
      continue;
    }
...
export function collectClawHubVersionGateErrors(params: {
  ...
  for (const plugin of changedPlugins) {
    ...
    if (baseManifest?.steelengine?.release?.publishToClawHub !== true) {
      continue;
    }
    const baseVersion =
...
async function hasClawHubTrustedPublisher(
  ...
  try {
    ...
    return isSteelEnginePluginTrustedPublisher(trustedPublisherDetail.trustedPublisher);
  } finally {
    request.clearTimeout();
  }
}

function isSteelEnginePluginTrustedPublisher(value: unknown): boolean {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return false;
  }
  const trustedPublisher = value as ClawHubTrustedPublisherConfig;
  return (
    trustedPublisher.repository === STEELENGINE_PLUGIN_CLAWHUB_REPOSITORY &&
    trustedPublisher.workflowFilename === STEELENGINE_PLUGIN_CLAWHUB_WORKFLOW_FILENAME &&
