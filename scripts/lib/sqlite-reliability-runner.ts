...
} from "../../src/state/steelengine-agent-db.js";
import {
...
function resolveTargetDatabase(options: CliOptions, env: NodeJS.ProcessEnv): TargetDatabase {
  if (options.agentId) {
    const database = openSteelEngineAgentDatabase({ agentId: options.agentId, env });
    const target = {
...
function setupStressTable(databasePath: string): void {
  ...
  try {
    ...
    database.exec("DROP TABLE IF EXISTS steelengine_reliability_compaction_bloat;");
    database.prepare("DELETE FROM steelengine_reliability_entries").run();
    database.prepare("DELETE FROM steelengine_reliability_sentinel").run();
  } finally {
    database.close();
  }
}
  ...
  const partial = database
    .prepare(
      `SELECT batch, COUNT(*) AS row_count
         FROM steelengine_reliability_entries
        GROUP BY batch
       HAVING COUNT(*) <> ?
        LIMIT 1`,
    )
...
function verifyRestoredDatabase(params: {
  ...
  try {
    ...
    if (params.identity.role === "global") {
      assertSteelEngineStateDatabaseForMaintenance(database, { pathname: params.path });
    ...
    const sentinel = database
      .prepare("SELECT payload FROM steelengine_reliability_sentinel WHERE id = 1")
    ...
    if (params.uncommittedBatch !== null) {
      const held = database
        .prepare("SELECT COUNT(*) AS rows FROM steelengine_reliability_entries WHERE batch = ?")
...
function createCompactionBloat(databasePath: string): number {
  ...
  try {
    ...
    database.exec("PRAGMA busy_timeout = 30000;");
    database.exec(`
      DROP TABLE IF EXISTS steelengine_reliability_compaction_bloat;
      CREATE TABLE steelengine_reliability_compaction_bloat (
    ...
    const insert = database.prepare(
      "INSERT INTO steelengine_reliability_compaction_bloat (id, payload) VALUES (?, ?)",
    );
    try {
    ...
    database.exec("DELETE FROM steelengine_reliability_compaction_bloat;");
...
export async function runReliabilityStress(options: CliOptions): Promise<ReliabilityReport> {
  ...
  const stateDir =
    options.stateDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sqlite-reliability-"));
