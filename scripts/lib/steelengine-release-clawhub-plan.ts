// SteelEngine release ClawHub plan script supports release workflow routing.
...
type SteelEngineReleaseClawHubPlanArgs = {
...
type SteelEngineReleaseClawHubPlan = {
  bootstrapWorkflowSha: string;
...
type SteelEngineReleaseClawHubRuntimeStateArgs = {
  repository: string;
...
type SteelEngineReleaseClawHubRuntimeState = {
  verifierArgs: string[];
  proofLines: {
    normal: string;
    bootstrap: string;
...
export function buildSteelEngineReleaseClawHubRuntimeState(
  args: SteelEngineReleaseClawHubRuntimeStateArgs,
): SteelEngineReleaseClawHubRuntimeState {
...
export function parseSteelEngineReleaseClawHubPlanArgs(
  argv: string[],
): SteelEngineReleaseClawHubPlanArgs {
  const values = [...argv];
  if (values[0] === "--") {
    values.shift();
  }
...
export async function buildSteelEngineReleaseClawHubPlan(
  args: SteelEngineReleaseClawHubPlanArgs,
  options: {
    rootDir?: string;
    fetchImpl?: typeof fetch;
    registryBaseUrl?: string;
  } = {},
): Promise<SteelEngineReleaseClawHubPlan> {
