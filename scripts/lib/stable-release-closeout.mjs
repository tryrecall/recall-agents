...
function isCloseoutEvidenceAsset(assetName, tag) {
  ...
  return (
    assetName === `steelengine-${releaseVersion}-stable-main-closeout.json` ||
    assetName === `steelengine-${releaseVersion}-stable-main-closeout.json.sha256`
  );
}
...
export function verifyStableMainCloseout(params) {
  ...
  if (mainVersion && !isStableMainVersionAtLeast(mainVersion, version)) {
    errors.push(
      `main package.json version is ${mainVersion}, expected shipped version ${version} or a later stable SteelEngine CalVer.`,
    );
  }
  ...
  const expectedMacAssets = [
    `SteelEngine-${macAssetVersion}.zip`,
    `SteelEngine-${macAssetVersion}.dmg`,
    `SteelEngine-${macAssetVersion}.dSYM.zip`,
  ];
  ...
  if (params.requireCompletePlatformAssets) {
    const requiredPlatformFamilies = [
      {
        label: "Android",
        prefix: "SteelEngine-Android",
        expected: ["SteelEngine-Android-SHA256SUMS.txt", "SteelEngine-Android.apk"],
      },
      {
        label: "Windows",
        prefix: "SteelEngineCompanion-",
        expected: [
          "SteelEngineCompanion-SHA256SUMS.txt",
