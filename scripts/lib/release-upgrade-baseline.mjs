...
export function compareSteelEngineVersions(leftVersion, rightVersion) {
  const left = parseVersion(leftVersion);
  const right = parseVersion(rightVersion);
  if (!left || !right) {
    throw new Error(`cannot compare SteelEngine versions: ${leftVersion} ${rightVersion}`);
  }
...
function normalizePublishedVersions(publishedVersions) {
  return [...new Set(publishedVersions.map((version) => String(version).trim()).filter(Boolean))]
    ...
    .toSorted((left, right) => compareSteelEngineVersions(right, left));
}

export function resolveDefaultReleaseUpgradeBaseline(candidateVersion, publishedVersions) {
  const candidate = parseVersion(candidateVersion);
  if (!candidate) {
    throw new Error(`invalid candidate SteelEngine version: ${candidateVersion}`);
  }

  const versions = normalizePublishedVersions(publishedVersions);
  const older = versions.find((version) => compareSteelEngineVersions(version, candidate.version) < 0);
  if (older) {
    return `steelengine@${older}`;
  }

  const same = versions.find(
    (version) => compareSteelEngineVersions(version, candidate.version) === 0,
  );
  if (same) {
    return `steelengine@${same}`;
  }

  throw new Error(`no published SteelEngine baseline is <= candidate ${candidate.version}`);
}
...
function readPublishedVersions(args) {
  ...
  const raw = execFileSync("npm", ["view", "steelengine", "versions", "--json", "--silent"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "inherit"],
  });
  const parsed = JSON.parse(raw);
  if (!Array.isArray(parsed)) {
    throw new Error("npm returned a non-array steelengine versions payload");
  }
  return parsed;
}
