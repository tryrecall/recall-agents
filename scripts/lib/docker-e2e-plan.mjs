...
export const DEFAULT_E2E_BARE_IMAGE = "steelengine-docker-e2e-bare:local";
export const DEFAULT_E2E_FUNCTIONAL_IMAGE = "steelengine-docker-e2e-functional:local";
...
const LEGACY_UPGRADE_SURVIVOR_SCENARIO_CATALOGS = new Map([
  ...
  [
    ...
    "base acpx-steelengine-tools-bridge feishu-channel bootstrap-persona channel-post-core-restore plugin-deps-cleanup configured-plugin-installs stale-source-plugin-shadow tilde-log-path versioned-runtime-deps",
  ],
  [
    "d9c9edcb27aca88a0b11c72e85592001cba732cf0f96bb8a73c1c0243c8f3678",
    "base acpx-steelengine-tools-bridge feishu-channel bootstrap-persona channel-post-core-restore codex-allowlist-survival plugin-deps-cleanup configured-plugin-installs stale-source-plugin-shadow tilde-log-path versioned-runtime-deps",
  ],
  [
...
export function normalizeUpgradeSurvivorBaselineSpec(raw) {
  ...
  const spec = value.startsWith("steelengine@") ? value : `steelengine@${value}`;
  if (
    !/^steelengine@(?:alpha|beta|latest|[0-9]{4}\.[0-9]+\.[0-9]+(?:-(?:[0-9]+|alpha\.[0-9]+|beta\.[0-9]+))?)$/u.test(
      spec,
    )
  ) {
    throw new Error(
      `invalid published upgrade survivor baseline: ${JSON.stringify(
        value,
      )}. Expected steelengine@latest, steelengine@beta, steelengine@alpha, or steelengine@YYYY.M.PATCH.`,
    );
  }
  return spec;
}
...
function parsePublishedReleaseVersion(spec) {
  const match = /^steelengine@([0-9]{4})\.([0-9]+)\.([0-9]+)/u.exec(String(spec ?? ""));
  if (!match) {
    return null;
  }
  return {
...
function supportsUpgradeSurvivorScenarioAtBaseline(scenario, baselineSpec) {
  return (
    ...
    (scenario !== "acpx-steelengine-tools-bridge" ||
      ...
      return matrixBaselines.flatMap((baselineSpec) =>
        matrixScenarios
          ...
          .map((scenario) => {
            ...
            const commandPrefix = [
              `STEELENGINE_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/${name}"`,
              baselineSpec
                ? `STEELENGINE_UPGRADE_SURVIVOR_BASELINE_SPEC=${shellQuote(baselineSpec)}`
                : "",
              scenario ? `STEELENGINE_UPGRADE_SURVIVOR_SCENARIO=${shellQuote(scenario)}` : "",
            ]
              .filter(Boolean)
              .join(" ");
...
export function parseLiveMode(raw) {
  ...
  throw new Error(
    `STEELENGINE_DOCKER_ALL_LIVE_MODE must be one of: all, skip, only. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function parseProfile(raw) {
  ...
  throw new Error(
    `STEELENGINE_DOCKER_ALL_PROFILE must be one of: ${DEFAULT_PROFILE}, ${RELEASE_PATH_PROFILE}. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function lanesNeedSteelEnginePackage(poolLanes) {
...
export function findLaneByName(name) {
  return dedupeLanes(
    expandUpgradeSurvivorBaselineLanes(
      ...
      process.env.STEELENGINE_UPGRADE_SURVIVOR_BASELINE_SPECS,
      undefined,
      process.env.STEELENGINE_UPGRADE_SURVIVOR_SCENARIOS,
    ).lanes,
...
function buildPlanJson(params) {
  ...
  return {
    ...
    needs: {
      ...
      package: lanesNeedSteelEnginePackage(scheduledLanes),
    },
  ...
  const selectedLanes =
    options.selectedLaneNames.length > 0
      ? options.selectedLaneNames.flatMap((selectedName) => {
          ...
          selectNamedLanes(unfilteredSelectableLanes, [selectedName], "STEELENGINE_DOCKER_ALL_LANES");
          return [];
        })
      : undefined;
