...
export function buildDeprecatedPluginSdkModuleSpecifiers(
  ...
  const unscoped = [
    ...
    ...deprecatedSubpaths.map((subpath) => `steelengine/plugin-sdk/${subpath}`),
  ];
  // tsconfig aliases the scoped @steelengine/plugin-sdk package to the same
...
export const BANNED_INTERNAL_PLUGIN_SDK_FACADE_MODULES = [
  // Reply facades: canonical seams are steelengine/plugin-sdk/channel-inbound and
  // steelengine/plugin-sdk/channel-outbound (defineChannelMessageAdapter family).
  {
    modulePath: "src/plugin-sdk/channel-envelope",
    canonical: "steelengine/plugin-sdk/channel-inbound",
  },
  {
    modulePath: "src/plugin-sdk/channel-message",
    canonical: "steelengine/plugin-sdk/channel-outbound",
  },
  {
