...
steelengine_live_stage_source_tree() {
  local dest_dir="${1:?destination directory required}"
  local stage_mode="${STEELENGINE_LIVE_DOCKER_SOURCE_STAGE_MODE:-copy}"

  if [ "$stage_mode" = "symlink" ]; then
    echo "STEELENGINE_LIVE_DOCKER_SOURCE_STAGE_MODE=symlink is disabled; using copy staging." >&2
  fi
  ...
  tar -C /src \
    ...
    --exclude=.worktrees \
    --exclude=__steelengine_vitest__ \
    --exclude=relay.sock \
    --exclude='*.sock' \
    --exclude='*/*.sock' \
    --exclude='apps/*/.build' \
...
steelengine_live_link_runtime_tree() {
  local dest_dir="${1:?destination directory required}"
...
steelengine_live_stage_node_modules() {
  local dest_dir="${1:?destination directory required}"
...
steelengine_live_scrub_staged_plugin_index() {
  local dest_dir="${1:?destination directory required}"
  local db_path="$dest_dir/state/steelengine.sqlite"
...
steelengine_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.steelengine"
  ...
  if [ -d "$source_dir" ]; then
    ...
    steelengine_live_scrub_staged_plugin_index "$dest_dir"
    if [ -d "$source_dir/workspace" ] && [ ! -e "$dest_dir/workspace" ]; then
      ln -s "$source_dir/workspace" "$dest_dir/workspace"
    fi
  fi

  export STEELENGINE_STATE_DIR="$dest_dir"
  export STEELENGINE_CONFIG_PATH="$dest_dir/steelengine.json"
}
