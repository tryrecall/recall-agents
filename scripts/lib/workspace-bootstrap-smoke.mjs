...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "recall-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "recall.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".recall", "workspace");
