// Verifies installed packages can bootstrap the default SteelEngine workspace files.
...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "steelengine-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "steelengine.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".steelengine", "workspace");
