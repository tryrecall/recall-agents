...
export async function resolveInstallerTargetVersion(params: {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "steelengine@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view steelengine@latest version did not return a version.");
  }
...
export function buildInstallerSmokeScript(
  ...
  if ((params.platform ?? process.platform) === "win32") {
    return `
$installerPath = Join-Path ([System.IO.Path]::GetTempPath()) ("steelengine-installer-" + [guid]::NewGuid().ToString("N") + ".ps1")
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "curl.exe failed to download the SteelEngine installer (exit $LASTEXITCODE)"
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'steelengine.cmd'),
      (Join-Path $npmPrefix 'steelengine.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command steelengine -ErrorAction Stop
...
Write-Output "__STEELENGINE_PATH__=$commandPath"
...
export async function verifyFreshShellCommand(params: {
  ...
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__STEELENGINE_PATH__=") ?? "",
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed steelengine path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v steelengine >/dev/null 2>&1",
    'printf "__STEELENGINE_PATH__=%s\\n" "$(command -v steelengine)"',
    "steelengine --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__STEELENGINE_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed steelengine path from fresh POSIX shell.");
  }
...
export async function ensureDevUpdateGitInstall(params: {
  ...
  // The dev-update lane must prove that `steelengine update --channel dev` landed on
