...
export function resolvePackageCandidatePackCommand(sourceDir: string, packDir: string) {
  const packageHelper = join(sourceDir, "scripts", "package-steelengine-for-docker.mjs");
...
function resolvePackedCandidateFromOutput(params: {
  ...
  if (params.packCommand.kind === "docker-helper") {
    ...
    const packedTarball = resolvePackDestinationTarball(
      ...
      params.packDir,
      "package-steelengine-for-docker",
    );
    return {
...
export async function runBundledPluginPostinstall(params: LaneCommandParams) {
  ...
  delete installEnv.STEELENGINE_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "steelengine/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.STEELENGINE_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.STEELENGINE_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.STEELENGINE_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing STEELENGINE_BROWSER_CONTROL_MODULE.");
}
...
export async function startBrowserControlService() {
  writeFileSync(process.env.STEELENGINE_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.STEELENGINE_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
export async function runInstalledBrowserOverrideImportSmoke(
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...
      STEELENGINE_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      STEELENGINE_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
...
export function installedEntryPath(prefixDir: string) {
  return join(installedPackageRoot(prefixDir), "steelengine.mjs");
}
