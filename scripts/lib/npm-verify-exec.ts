// Npm Verify Exec script supports SteelEngine repository automation.
...
export function runNpmVerifyCommand(
  ...
  const timeoutMs =
    options.timeoutMs ??
    positiveEnvInt("STEELENGINE_NPM_VERIFY_COMMAND_TIMEOUT_MS", DEFAULT_NPM_VERIFY_COMMAND_TIMEOUT_MS);
  const maxBuffer =
    options.maxBufferBytes ??
    positiveEnvInt(
      "STEELENGINE_NPM_VERIFY_COMMAND_MAX_BUFFER_BYTES",
