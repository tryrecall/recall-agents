...
const INCLUDE_FILE_ENV_KEY = "RECALL_VITEST_INCLUDE_FILE";
...
const BROAD_CHANGED_FALLBACK_PATTERNS = [
  /^package\.json$/u,
  /^pnpm-lock\.yaml$/u,
  /^test\/setup(?:\.shared|\.extensions|-recall-runtime)?\.ts$/u,
...
const TOOLING_SOURCE_TEST_TARGETS = new Map([
  ...
  ["scripts/lib/recall-test-state.mjs", ["test/scripts/recall-test-state.test.ts"]],
...
const TOOLING_TEST_TARGETS = new Map([
  ...
  ["test/scripts/recall-test-state.test.ts", ["test/scripts/recall-test-state.test.ts"]],
  [
...
const SOURCE_TEST_TARGETS = new Map([
  ...
  ["src/test-utils/recall-test-state.ts", ["src/test-utils/recall-test-state.test.ts"]],
  [
...
const BROAD_CHANGED_ENV_KEY = "RECALL_TEST_CHANGED_BROAD";
const VITEST_NO_OUTPUT_TIMEOUT_ENV_KEY = "RECALL_VITEST_NO_OUTPUT_TIMEOUT_MS";
const VITEST_NO_OUTPUT_RETRY_ENV_KEY = "RECALL_VITEST_NO_OUTPUT_RETRY";
...
export function buildFullSuiteVitestRunPlans(args, cwd = process.cwd()) {
  ...
  const parallelShardCount = Number.parseInt(process.env.RECALL_TEST_PROJECTS_PARALLEL ?? "", 10);
...
export function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return (
    env.RECALL_TEST_PROJECTS_SERIAL !== "1" && env.CI !== "true" && env.GITHUB_ACTIONS !== "true"
  );
}
...
function hasConservativeVitestWorkerBudget(env) {
  const workerBudget = parsePositiveInt(
    env.RECALL_VITEST_MAX_WORKERS ?? env.RECALL_TEST_WORKERS,
  );
...
export function resolveParallelFullSuiteConcurrency(specCount, env, hostInfo) {
  env ??= process.env;
  const override = parsePositiveInt(env.RECALL_TEST_PROJECTS_PARALLEL);
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.RECALL_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.RECALL_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
  ...
  return plans.map((plan, index) => {
    const includeFilePath = plan.includePatterns
      ? path.join(
          ...
          `recall-vitest-include-${process.pid}-${Date.now()}-${index}.json`,
        )
      : null;
    return {
      config: plan.config,
...
export function shouldAcquireLocalHeavyCheckLock(runSpecs, env = process.env) {
  if (env.RECALL_TEST_HEAVY_CHECK_LOCK_HELD === "1") {
    return false;
  }

  if (env.RECALL_TEST_PROJECTS_FORCE_LOCK === "1") {
    return true;
  }
