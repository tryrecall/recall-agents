...
EXTRA_MOUNTS="${STEELENGINE_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${STEELENGINE_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${STEELENGINE_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${STEELENGINE_DOCKER_SOCKET:-}"
TIMEZONE="${STEELENGINE_TZ:-}"
RAW_SKIP_ONBOARDING="${STEELENGINE_SKIP_ONBOARDING:-}"
SKIP_ONBOARDING=""
DOCKER_PULL_TIMEOUT="${STEELENGINE_DOCKER_SETUP_PULL_TIMEOUT:-600s}"
OFFLINE_MODE=""
DEFAULT_SANDBOX_IMAGE="steelengine-sandbox:bookworm-slim"
DEFAULT_SANDBOX_BROWSER_IMAGE="steelengine-sandbox-browser:bookworm-slim"
...
read_config_gateway_token() {
  local config_path="$STEELENGINE_CONFIG_DIR/steelengine.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == STEELENGINE_GATEWAY_TOKEN=* ]]; then
      token="${line#STEELENGINE_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${STEELENGINE_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$STEELENGINE_GATEWAY_PORT" "$STEELENGINE_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$STEELENGINE_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$STEELENGINE_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  # During setup, avoid the shared-network steelengine-cli service because it
  ...
  # Host STEELENGINE_* paths are Compose bind-mount sources. Setup-time CLI writes
  ...
  run_prestart_gateway \
    ...
    --entrypoint node steelengine-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" steelengine-cli "$@"
}
...
run_gateway_up() {
  ...
  docker compose "${compose_args[@]}" "${up_args[@]}" steelengine-gateway
}
...
resolve_offline_sandbox_images() {
  ...
  printf '%s' "$agents_json" | run_prestart_gateway \
    -T --entrypoint node steelengine-gateway -e '
const fs = require("node:fs");
...
const matchesBrowser = (rawPattern) => {
  ...
  if (pattern === "group:steelengine" || pattern === "group:ui") {
    return true;
  }
  if (!pattern) {
    return false;
...
validate_offline_sandbox_prerequisites() {
      ...
      browser)
        if ! browser_contract="$(
          docker --host "unix://$DOCKER_SOCKET_PATH" image inspect \
            -f '{{ index .Config.Labels "org.steelengine.sandbox-browser.contract" }}' \
  ...
  if ! run_compose_one_off --rm --entrypoint docker steelengine-gateway --version >/dev/null 2>&1; then
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "STEELENGINE_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "STEELENGINE_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
STEELENGINE_CONFIG_DIR="${STEELENGINE_CONFIG_DIR:-$HOME/.steelengine}"
STEELENGINE_WORKSPACE_DIR="${STEELENGINE_WORKSPACE_DIR:-$HOME/.steelengine/workspace}"
STEELENGINE_AUTH_PROFILE_SECRET_DIR="${STEELENGINE_AUTH_PROFILE_SECRET_DIR:-$HOME/.steelengine-auth-profile-secrets}"

validate_mount_path_value "STEELENGINE_CONFIG_DIR" "$STEELENGINE_CONFIG_DIR"
validate_mount_path_value "STEELENGINE_WORKSPACE_DIR" "$STEELENGINE_WORKSPACE_DIR"
validate_mount_path_value "STEELENGINE_AUTH_PROFILE_SECRET_DIR" "$STEELENGINE_AUTH_PROFILE_SECRET_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "STEELENGINE_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "STEELENGINE_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "STEELENGINE_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "STEELENGINE_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "STEELENGINE_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "STEELENGINE_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$STEELENGINE_CONFIG_DIR"
mkdir -p "$STEELENGINE_WORKSPACE_DIR"
mkdir -p "$STEELENGINE_AUTH_PROFILE_SECRET_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$STEELENGINE_CONFIG_DIR/identity"
mkdir -p "$STEELENGINE_CONFIG_DIR/agents/main/agent"
mkdir -p "$STEELENGINE_CONFIG_DIR/agents/main/sessions"

export STEELENGINE_CONFIG_DIR
export STEELENGINE_WORKSPACE_DIR
export STEELENGINE_AUTH_PROFILE_SECRET_DIR
export STEELENGINE_GATEWAY_PORT="${STEELENGINE_GATEWAY_PORT:-18789}"
export STEELENGINE_BRIDGE_PORT="${STEELENGINE_BRIDGE_PORT:-18790}"
export STEELENGINE_GATEWAY_BIND="${STEELENGINE_GATEWAY_BIND:-lan}"
export STEELENGINE_DISABLE_BONJOUR="${STEELENGINE_DISABLE_BONJOUR:-}"
export STEELENGINE_IMAGE="$IMAGE_NAME"
export STEELENGINE_IMAGE_APT_PACKAGES="${STEELENGINE_IMAGE_APT_PACKAGES-${STEELENGINE_DOCKER_APT_PACKAGES:-}}"
export STEELENGINE_IMAGE_PIP_PACKAGES="${STEELENGINE_IMAGE_PIP_PACKAGES:-}"
export STEELENGINE_EXTENSIONS="${STEELENGINE_EXTENSIONS:-}"
export STEELENGINE_DOCKER_BUILD_NODE_OPTIONS="${STEELENGINE_DOCKER_BUILD_NODE_OPTIONS---max-old-space-size=8192}"
export STEELENGINE_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB="${STEELENGINE_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB:-}"
export STEELENGINE_DOCKER_BUILD_SKIP_DTS="${STEELENGINE_DOCKER_BUILD_SKIP_DTS:-1}"
export STEELENGINE_INSTALL_BROWSER="${STEELENGINE_INSTALL_BROWSER:-}"
export STEELENGINE_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export STEELENGINE_HOME_VOLUME="$HOME_VOLUME_NAME"
export STEELENGINE_ALLOW_INSECURE_PRIVATE_WS="${STEELENGINE_ALLOW_INSECURE_PRIVATE_WS:-}"
export STEELENGINE_SANDBOX="$SANDBOX_ENABLED"
export STEELENGINE_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export STEELENGINE_DOCKER_SETUP=1
export STEELENGINE_TZ="$TIMEZONE"
...
export STEELENGINE_OTEL_PRELOADED="${STEELENGINE_OTEL_PRELOADED:-}"
export STEELENGINE_SKIP_ONBOARDING="$SKIP_ONBOARDING"
...
if [[ -z "${STEELENGINE_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    STEELENGINE_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $STEELENGINE_CONFIG_DIR/steelengine.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      STEELENGINE_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      STEELENGINE_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      STEELENGINE_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
export STEELENGINE_GATEWAY_TOKEN
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${STEELENGINE_CONFIG_DIR}:/home/node/.steelengine"
    gateway_workspace_mount="${STEELENGINE_WORKSPACE_DIR}:/home/node/.steelengine/workspace"
    gateway_auth_profile_secret_mount="${STEELENGINE_AUTH_PROFILE_SECRET_DIR}:/home/node/.config/steelengine"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${STEELENGINE_INSTALL_DOCKER_CLI:-}" ]]; then
    export STEELENGINE_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  STEELENGINE_AUTH_PROFILE_SECRET_DIR \
  STEELENGINE_GATEWAY_PORT \
  STEELENGINE_BRIDGE_PORT \
  STEELENGINE_GATEWAY_BIND \
  STEELENGINE_DISABLE_BONJOUR \
  STEELENGINE_GATEWAY_TOKEN \
  STEELENGINE_IMAGE \
  STEELENGINE_EXTRA_MOUNTS \
  STEELENGINE_HOME_VOLUME \
  STEELENGINE_IMAGE_APT_PACKAGES \
  STEELENGINE_IMAGE_PIP_PACKAGES \
  STEELENGINE_EXTENSIONS \
  STEELENGINE_DOCKER_BUILD_NODE_OPTIONS \
  STEELENGINE_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB \
  STEELENGINE_DOCKER_BUILD_SKIP_DTS \
  STEELENGINE_INSTALL_BROWSER \
  STEELENGINE_SANDBOX \
  STEELENGINE_DOCKER_SOCKET \
  DOCKER_GID \
  STEELENGINE_INSTALL_DOCKER_CLI \
  ...
  STEELENGINE_OTEL_PRELOADED \
  STEELENGINE_SKIP_ONBOARDING

if [[ -n "$OFFLINE_MODE" ]]; then
  require_local_docker_image "$IMAGE_NAME"
  echo "==> Using preloaded Docker image: $IMAGE_NAME"
elif [[ "$IMAGE_NAME" == "steelengine:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  BUILD_GIT_COMMIT="$(steelengine_resolve_git_commit "$ROOT_DIR")"
  BUILD_TIMESTAMP="$(steelengine_resolve_build_timestamp)"
  PROVENANCE_BUILD_ARGS=(--build-arg "STEELENGINE_BUILD_TIMESTAMP=${BUILD_TIMESTAMP}")
  if [[ "$BUILD_GIT_COMMIT" =~ ^[0-9a-fA-F]{40}$ ]]; then
    PROVENANCE_BUILD_ARGS+=(--build-arg "GIT_COMMIT=${BUILD_GIT_COMMIT}")
  fi
  run_docker_build \
    "${PROVENANCE_BUILD_ARGS[@]}" \
    --build-arg "STEELENGINE_IMAGE_APT_PACKAGES=${STEELENGINE_IMAGE_APT_PACKAGES}" \
    --build-arg "STEELENGINE_IMAGE_PIP_PACKAGES=${STEELENGINE_IMAGE_PIP_PACKAGES}" \
    --build-arg "STEELENGINE_EXTENSIONS=${STEELENGINE_EXTENSIONS}" \
    --build-arg "STEELENGINE_DOCKER_BUILD_NODE_OPTIONS=${STEELENGINE_DOCKER_BUILD_NODE_OPTIONS}" \
    --build-arg "STEELENGINE_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=${STEELENGINE_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB}" \
    --build-arg "STEELENGINE_DOCKER_BUILD_SKIP_DTS=${STEELENGINE_DOCKER_BUILD_SKIP_DTS}" \
    --build-arg "STEELENGINE_INSTALL_BROWSER=${STEELENGINE_INSTALL_BROWSER}" \
    --build-arg "STEELENGINE_INSTALL_DOCKER_CLI=${STEELENGINE_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the SteelEngine metadata subdirectory
# (.steelengine/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh steelengine-gateway -c \
  'PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin; export PATH; \
   /usr/bin/find -P /home/node/.steelengine -xdev -execdir /usr/bin/chown -h node:node {} +; \
   /usr/bin/chown -h node:node /home/node/.config; \
   /usr/bin/find -P /home/node/.config/steelengine -xdev -execdir /usr/bin/chown -h node:node {} +; \
   if [ -d /home/node/.steelengine/workspace/.steelengine ] && [ ! -L /home/node/.steelengine/workspace/.steelengine ]; then \
     /usr/bin/find -P /home/node/.steelengine/workspace/.steelengine -xdev -execdir /usr/bin/chown -h node:node {} +; \
   fi || true'

echo ""
if [[ -n "$SKIP_ONBOARDING" ]]; then
  echo "==> Skipping onboarding (STEELENGINE_SKIP_ONBOARDING is set)"
else
  echo "==> Onboarding (interactive)"
  echo "Docker setup pins Gateway mode to local."
  echo "Gateway runtime bind comes from STEELENGINE_GATEWAY_BIND (default: lan)."
  echo "Current runtime bind: $STEELENGINE_GATEWAY_BIND"
  if is_truthy_value "$STEELENGINE_DISABLE_BONJOUR"; then
    echo "Bonjour/mDNS advertising: force disabled (STEELENGINE_DISABLE_BONJOUR=$STEELENGINE_DISABLE_BONJOUR)."
  elif [[ -z "$STEELENGINE_DISABLE_BONJOUR" ]]; then
    echo "Bonjour/mDNS advertising: auto (disabled inside the Gateway container unless explicitly enabled)."
  else
    echo "Bonjour/mDNS advertising: explicitly enabled (STEELENGINE_DISABLE_BONJOUR=$STEELENGINE_DISABLE_BONJOUR)."
  fi
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm steelengine-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm steelengine-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm steelengine-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.steelengine.ai/channels"
...
# --- Sandbox setup (opt-in via STEELENGINE_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -z "$OFFLINE_MODE" && ! -S "$DOCKER_SOCKET_PATH" ]]; then
    echo "WARNING: STEELENGINE_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
  ...
  if [[ -n "$SANDBOX_ENABLED" && -z "$OFFLINE_MODE" ]] &&
    ! run_compose_one_off --rm --entrypoint docker steelengine-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg STEELENGINE_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (STEELENGINE_IMAGE=steelengine:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in SteelEngine config.
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.steelengine.ai/gateway/sandboxing"
...
echo "Config: $STEELENGINE_CONFIG_DIR"
echo "Workspace: $STEELENGINE_WORKSPACE_DIR"
echo "Token: stored in Docker environment/config (not printed)."
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f steelengine-gateway"
echo "  ${COMPOSE_HINT} exec steelengine-gateway sh -lc 'node dist/index.js health --token \"\$STEELENGINE_GATEWAY_TOKEN\"'"
