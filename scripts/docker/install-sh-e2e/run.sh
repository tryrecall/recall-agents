...
EXPECTED_VERSION="$(npm view "recall@${INSTALL_TAG}" version)"
...
const versions = JSON.parse(execSync("npm view recall versions --json", { encoding: "utf8" }));
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  ...
  npm install -g "recall@${PREVIOUS_VERSION}"
fi

echo "==> Run official installer one-liner"
if [[ "$INSTALL_TAG" == "beta" ]]; then
  RECALL_BETA=1 curl -fsSL "$INSTALL_URL" | bash
...
INSTALLED_VERSION="$(extract_recall_semver "$INSTALLED_VERSION")"
...
run_profile() {
  ...
  SESSION_JSONL="/root/.recall-${profile}/agents/main/sessions/${SESSION_ID}.jsonl"
  ...
  for _ in $(seq 1 60); do
    if recall --profile "$profile" health --timeout 2000 --json >/dev/null 2>&1; then
      break
    fi
    sleep 0.25
  done
  recall --profile "$profile" health --timeout 10000 --json >/dev/null
  ...
  if [[ ! -f "$SESSION_JSONL" ]]; then
    ...
    ls -la "/root/.recall-${profile}/agents/main/sessions" >&2 || true
    exit 1
  fi
