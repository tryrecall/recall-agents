...
const WINDOWS_NODE_REPO = "steelengine/steelengine-windows-node";
const WINDOWS_NODE_REQUIRED_ASSETS = [
  "SteelEngineCompanion-Setup-x64.exe",
  "SteelEngineCompanion-Setup-arm64.exe",
];
...
SteelEngine Release Publish command only after everything is green.
...
Options:
  ...
  --npm-preflight-run <id>            Reuse successful SteelEngine NPM Release preflight run.
...
export function parseArgs(argv) {
  ...
  if (options.pluginPublishScope === "selected") {
    throw new Error(
      "--plugin-publish-scope selected is only for plugin-only repair publishes; release candidates publish SteelEngine with --plugin-publish-scope all-publishable",
    );
  }
...
function githubApiTimeoutMs() {
  const raw = process.env.STEELENGINE_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS;
  if (!raw) {
    return DEFAULT_GITHUB_API_TIMEOUT_MS;
  }
  if (!/^[1-9]\d*$/u.test(raw)) {
    throw new Error("STEELENGINE_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS must be a positive integer");
  }
...
function runFromTrustedTooling(argv, { targetRoot, workflowRef }) {
  ...
  const tempRoot = mkdtempSync(join(tmpdir(), "steelengine-release-tooling-"));
...
export function buildPublishCommand(options) {
  ...
  return [
    ...
    "workflow",
    "run",
    "steelengine-release-publish.yml",
    "--repo",
    options.repo,
    "--ref",
    workflowRef,
...
async function runParallelsIfNeeded(options, tarballPath, dependencyTarballPaths) {
  ...
  run("bash", ["-lc", command], {
    env: {
      STEELENGINE_PARALLELS_ARTIFACT_ROOT: join(process.cwd(), ".artifacts", "parallels"),
    },
  });
  return {
    status: "passed",
...
async function runTelegramIfNeeded(options, artifact, manifest, runAttempt, sourceSha) {
  ...
  const runId = dispatchWorkflow(options.repo, workflowFile, options.workflowRef, {
    package_spec: `steelengine@${options.tag.replace(/^v/u, "")}`,
...
async function main() {
  ...
  if (!options.npmPreflightRunId && !options.skipDispatch) {
    const workflowFile = "steelengine-npm-release.yml";
  ...
  const npmRun = await waitForSuccessfulRun(options.repo, options.npmPreflightRunId, {
    workflowName: "SteelEngine NPM Release",
    workflowRef: options.workflowRef,
  });
  ...
  const npmArtifact = await downloadResolvedArtifact(
    ...
    `steelengine-npm-preflight-${options.tag}`,
