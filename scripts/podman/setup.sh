#!/usr/bin/env bash
# One-time host setup for rootless Recall in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.recall by default, and uses the repo-local
# launch script at ./scripts/run-recall-podman.sh.
#
...
#   systemctl --user start recall.service
set -euo pipefail
...
RECALL_USER="$(id -un)"
RECALL_HOME="${HOME:-}"
RECALL_CONFIG_DIR="${RECALL_CONFIG_DIR:-}"
RECALL_WORKSPACE_DIR="${RECALL_WORKSPACE_DIR:-}"
RECALL_IMAGE="${RECALL_PODMAN_IMAGE:-${RECALL_IMAGE:-recall:local}}"
RECALL_CONTAINER_NAME="${RECALL_PODMAN_CONTAINER:-recall}"
PLATFORM_NAME="$(uname -s 2>/dev/null || echo unknown)"
HOST_GATEWAY_PORT="${RECALL_PODMAN_GATEWAY_HOST_PORT:-${RECALL_GATEWAY_PORT:-18789}}"
...
if [[ "$RECALL_IMAGE" == "recall:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
...
if [[ -z "$RECALL_HOME" ]]; then
  RECALL_HOME="$(resolve_user_home "$RECALL_USER")"
fi
if [[ -z "$RECALL_HOME" ]]; then
  echo "Unable to resolve HOME for user $RECALL_USER." >&2
  exit 1
fi
if [[ -z "$RECALL_CONFIG_DIR" ]]; then
  RECALL_CONFIG_DIR="$RECALL_HOME/.recall"
fi
if [[ -z "$RECALL_WORKSPACE_DIR" ]]; then
  RECALL_WORKSPACE_DIR="$RECALL_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$RECALL_HOME"
validate_mount_source_path "config directory" "$RECALL_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$RECALL_WORKSPACE_DIR"
validate_container_name "$RECALL_CONTAINER_NAME"
validate_image_name "$RECALL_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$RECALL_CONFIG_DIR" "$RECALL_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$RECALL_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$RECALL_WORKSPACE_DIR"

RECALL_IMAGE_APT_PACKAGES="${RECALL_IMAGE_APT_PACKAGES-${RECALL_DOCKER_APT_PACKAGES:-}}"
RECALL_IMAGE_PIP_PACKAGES="${RECALL_IMAGE_PIP_PACKAGES:-}"
BUILD_ARGS=()
if [[ -n "$RECALL_IMAGE_APT_PACKAGES" ]]; then
  BUILD_ARGS+=(--build-arg "RECALL_IMAGE_APT_PACKAGES=${RECALL_IMAGE_APT_PACKAGES}")
fi
if [[ -n "$RECALL_IMAGE_PIP_PACKAGES" ]]; then
  BUILD_ARGS+=(--build-arg "RECALL_IMAGE_PIP_PACKAGES=${RECALL_IMAGE_PIP_PACKAGES}")
fi
if [[ -n "${RECALL_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "RECALL_EXTENSIONS=${RECALL_EXTENSIONS}")
fi
if [[ -n "${RECALL_INSTALL_BROWSER:-}" ]]; then
  BUILD_ARGS+=(--build-arg "RECALL_INSTALL_BROWSER=${RECALL_INSTALL_BROWSER}")
fi

if [[ "$RECALL_IMAGE" == "recall:local" ]]; then
  echo "Building image $RECALL_IMAGE ..."
  podman build -t "$RECALL_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$RECALL_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $RECALL_IMAGE"
  else
    echo "Pulling image $RECALL_IMAGE ..."
    podman pull "$RECALL_IMAGE"
  fi
fi

ENV_FILE="$RECALL_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
...
EOF
  ...
  echo "Generated RECALL_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "RECALL_PODMAN_CONTAINER" "$RECALL_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "RECALL_PODMAN_IMAGE" "$RECALL_IMAGE"

CONFIG_JSON="$RECALL_CONFIG_DIR/recall.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  ...
  RECALL_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$RECALL_CONFIG_DIR")"
  RECALL_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$RECALL_WORKSPACE_DIR")"
  RECALL_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$RECALL_IMAGE")"
  RECALL_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$RECALL_CONTAINER_NAME")"
  sed \
    -e "s|{{RECALL_HOME}}|$RECALL_HOME_ESCAPED|g" \
    -e "s|{{RECALL_CONFIG_DIR}}|$RECALL_CONFIG_ESCAPED|g" \
    -e "s|{{RECALL_WORKSPACE_DIR}}|$RECALL_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$RECALL_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$RECALL_CONTAINER_ESCAPED|g" \
    "$QUADLET_TEMPLATE" | write_file_atomically "$QUADLET_DST" 644

  if command -v systemctl >/dev/null 2>&1; then
    echo "Reloading and starting user service..."
    if systemctl --user daemon-reload && systemctl --user start recall.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start recall.service" >&2
...
echo "  recall --container $RECALL_CONTAINER_NAME dashboard --no-open"
