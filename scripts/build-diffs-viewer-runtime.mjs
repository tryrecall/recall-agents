...
const pierreDiffsEmptySideEffectNamespace = "steelengine-diffs-empty-side-effect";
...
export function createPierreDiffsSideEffectImportPlugin() {
  return {
    name: "steelengine-diffs-pierre-side-effect-imports",
    setup(buildContext) {
...
async function buildDiffsViewerRuntime(targetName) {
  ...
  const result = await build({
    ...
    minify: true,
    define: {
      __STEELENGINE_DIFFS_LANGUAGE_PACK__: String(target.languagePackAvailable),
      NaN: "Number.NaN",
    },
    legalComments: "none",
    outfile: outputPath,
    ...
    plugins: [
      ...
      ...(target.shikiAlias
        ? [
            {
              name: "steelengine-diffs-curated-shiki",
              setup(buildContext) {
