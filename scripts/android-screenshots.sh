...
DEFAULT_SCREENSHOT_AVD="SteelEngine_Screenshots_API36"
...
boot_emulator() {
  ...
  emulator="$(emulator_bin)"
  EMULATOR_LOG="$(mktemp "${TMPDIR:-/tmp}/steelengine-android-screenshot-emulator.XXXXXX.log")"
...
"$ADB_BIN" -s "$ADB_SERIAL" shell pm clear ai.steelengine.app >/dev/null
"$ADB_BIN" -s "$ADB_SERIAL" shell pm grant ai.steelengine.app android.permission.RECORD_AUDIO >/dev/null
"$ADB_BIN" -s "$ADB_SERIAL" logcat -c >/dev/null 2>&1 || true

for scene in "${SCENES[@]}"; do
  output_path="${OUTPUT_DIR}/steelengine-${scene}.jpg"
  raw_path="${OUTPUT_DIR}/steelengine-${scene}.raw.png"
  artifact_path="${ARTIFACT_DIR}/screenshots/steelengine-${scene}.jpg"
  ui_dump_path="${ARTIFACT_DIR}/ui-dumps/steelengine-${scene}.xml"
  activity_start_path="${ARTIFACT_DIR}/activity-start/steelengine-${scene}.txt"
  "$ADB_BIN" -s "$ADB_SERIAL" shell am force-stop ai.steelengine.app >/dev/null
  "$ADB_BIN" -s "$ADB_SERIAL" shell am start -W \
    -n ai.steelengine.app/.MainActivity \
    --ez steelengine.screenshotMode true \
    --es steelengine.screenshotScene "$scene" >"$activity_start_path"
