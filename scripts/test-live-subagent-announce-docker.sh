...
if [[ -n "${OPENAI_API_KEY:-}" || -n "${OPENAI_BASE_URL:-}" || -n "${GEMINI_API_KEY:-}" || -n "${GOOGLE_API_KEY:-}" ]]; then
  docker_env_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/steelengine-subagent-live-env.XXXXXX")"
  TEMP_DIRS+=("$docker_env_dir")
  docker_env_file="$docker_env_dir/provider.env"
  {
    if [[ -n "${OPENAI_API_KEY:-}" ]]; then
      printf 'STEELENGINE_DOCKER_LIVE_OPENAI_API_KEY=%s\n' "${OPENAI_API_KEY}"
    fi
    if [[ -n "${OPENAI_BASE_URL:-}" ]]; then
      printf 'STEELENGINE_DOCKER_LIVE_OPENAI_BASE_URL=%s\n' "${OPENAI_BASE_URL}"
    fi
    if [[ -n "${GEMINI_API_KEY:-}" ]]; then
      printf 'STEELENGINE_DOCKER_LIVE_GEMINI_API_KEY=%s\n' "${GEMINI_API_KEY}"
    fi
    if [[ -n "${GOOGLE_API_KEY:-}" ]]; then
      printf 'STEELENGINE_DOCKER_LIVE_GOOGLE_API_KEY=%s\n' "${GOOGLE_API_KEY}"
    fi
  } >"$docker_env_file"
  DOCKER_EXTRA_ENV_FILES+=(--env-file "$docker_env_file")
fi

CONTAINER_NODE_OPTIONS="$(steelengine_live_container_node_options)"

read -r -d '' LIVE_TEST_CMD <<'EOF' || true
set -euo pipefail
[ -f "$HOME/.profile" ] && [ -r "$HOME/.profile" ] && source "$HOME/.profile" || true
if [ -n "${STEELENGINE_DOCKER_LIVE_OPENAI_API_KEY:-}" ]; then
  export OPENAI_API_KEY="$STEELENGINE_DOCKER_LIVE_OPENAI_API_KEY"
  unset STEELENGINE_DOCKER_LIVE_OPENAI_API_KEY
fi
if [ -n "${STEELENGINE_DOCKER_LIVE_OPENAI_BASE_URL:-}" ]; then
  export OPENAI_BASE_URL="$STEELENGINE_DOCKER_LIVE_OPENAI_BASE_URL"
  unset STEELENGINE_DOCKER_LIVE_OPENAI_BASE_URL
fi
if [ -n "${STEELENGINE_DOCKER_LIVE_GEMINI_API_KEY:-}" ]; then
  export GEMINI_API_KEY="$STEELENGINE_DOCKER_LIVE_GEMINI_API_KEY"
  unset STEELENGINE_DOCKER_LIVE_GEMINI_API_KEY
fi
if [ -n "${STEELENGINE_DOCKER_LIVE_GOOGLE_API_KEY:-}" ]; then
  export GOOGLE_API_KEY="$STEELENGINE_DOCKER_LIVE_GOOGLE_API_KEY"
  unset STEELENGINE_DOCKER_LIVE_GOOGLE_API_KEY
fi
...
STEELENGINE_LIVE_SUBAGENT_E2E=1 \
...
echo "==> Model: ${STEELENGINE_LIVE_SUBAGENT_E2E_MODEL:-openai/gpt-5.6-luna}"
echo "==> Profile file: $PROFILE_STATUS"
DOCKER_RUN_ARGS=()
steelengine_live_init_docker_run_args DOCKER_RUN_ARGS "${STEELENGINE_LIVE_SUBAGENT_DOCKER_RUN_TIMEOUT:-1200s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e STEELENGINE_LIVE_TEST_QUIET="${STEELENGINE_LIVE_TEST_QUIET:-}" \
  -e STEELENGINE_LIVE_WRAPPER_HEARTBEAT_MS="${STEELENGINE_LIVE_WRAPPER_HEARTBEAT_MS:-}" \
  -e STEELENGINE_LIVE_SUBAGENT_E2E=1 \
  -e STEELENGINE_LIVE_SUBAGENT_E2E_MODEL="${STEELENGINE_LIVE_SUBAGENT_E2E_MODEL:-}" \
  -e STEELENGINE_VITEST_FS_MODULE_CACHE=0 \
  -e STEELENGINE_VITEST_MAX_WORKERS="${STEELENGINE_VITEST_MAX_WORKERS:-1}")
