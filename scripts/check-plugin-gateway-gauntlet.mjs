...
export function parseArgs(argv) {
  ...
  const options = {
    ...
    pluginIds: [],
    shardTotal: readOptionalPositiveIntEnv("STEELENGINE_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("STEELENGINE_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    limit: undefined,
    skipPrebuild: false,
    skipLifecycle: false,
    skipQa: false,
    ...
    qaTimeoutMs: 900_000,
    allowEmpty: false,
    failOnObservation: process.env.STEELENGINE_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION === "1",
    keepRunRoot: process.env.STEELENGINE_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT === "1",
  };
  const envIds = normalizeCsv(process.env.STEELENGINE_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
Environment:
  STEELENGINE_PLUGIN_GATEWAY_GAUNTLET_IDS   Comma-separated plugin ids to include
  STEELENGINE_PLUGIN_GATEWAY_GAUNTLET_TOTAL Total plugin shards
  STEELENGINE_PLUGIN_GATEWAY_GAUNTLET_INDEX Zero-based shard index
  STEELENGINE_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION=1
  STEELENGINE_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT=1
  STEELENGINE_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES  QA summary read ceiling
`);
}
...
function steelengineCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function createIsolatedEnv(repoRoot, runRoot) {
  ...
  return {
    ...
    STEELENGINE_LOG_DIR: path.join(runRoot, "logs"),
    STEELENGINE_QA_SUITE_PROGRESS: process.env.STEELENGINE_QA_SUITE_PROGRESS ?? "1",
...
function buildSlashHelpProbe(params) {
  ...
  return {
    ...
    ...steelengineCommand(params.repoRoot, [command, "--help"]),
...
async function runPluginLifecycleCommand(params) {
  ...
  params.rows.push(
    await runMeasuredCommand({
      ...
      ...steelengineCommand(params.repoRoot, ["plugins", ...params.args]),
      label: params.label,
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-plugin-gauntlet-"));
