...
usage() {
  echo "usage: bash scripts/steelengine-npm-publish.sh --publish [package.tgz]"
}
...
publish_plan="$(
  PACKAGE_VERSION="${package_version}" REQUESTED_PUBLISH_TAG="${STEELENGINE_NPM_PUBLISH_TAG:-}" \
    BYPASS_EXTENDED_STABLE_GUARD="${BYPASS_EXTENDED_STABLE_GUARD:-}" \
    node scripts/steelengine-npm-extended-stable-release.mjs publish-plan
)"
