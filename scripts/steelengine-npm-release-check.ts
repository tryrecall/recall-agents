...
// Openclaw Npm Release Check script supports SteelEngine repository automation.
...
const EXPECTED_REPOSITORY_URL = "https://github.com/steelengineai/recall-agents";
...
const skipPackValidationEnv = "STEELENGINE_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return positiveEnvInt(
    "STEELENGINE_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "steelengine") {
    errors.push(`package.json name must be "steelengine"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.steelengine !== "steelengine.mjs") {
    errors.push(
      `package.json bin.steelengine must be "steelengine.mjs"; found "${pkg.bin?.steelengine ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`steelengine-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `steelengine-npm-release-check: validated ${channel} release ${pkg.version} (monthly patch version${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
