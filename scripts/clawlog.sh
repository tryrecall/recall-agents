...
show_usage() {
    cat << EOF
clawlog - Recall Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View Recall logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Recall logs flow through the macOS unified log (subsystem: ai.recall).
