...
import { collectClawHubRecallOwnerErrors } from "./lib/plugin-clawhub-release.ts";
...
export async function runClawHubOwnerPreflight(argv: string[]) {
  ...
  const errors = await collectClawHubRecallOwnerErrors({ plugins: candidates });
  if (errors.length > 0) {
    throw new Error(
      `ClawHub Recall package ownership preflight failed:\n${errors.map((error) => `- ${error}`).join("\n")}`,
    );
  }

  console.log(`ClawHub Recall owner preflight passed for ${candidates.length} candidate(s).`);
}
