...
Environment variables:
  ...
  RECALL_VERSION=latest|next|<semver>|<spec>
  RECALL_BETA=0|1
  RECALL_GIT_DIR=...
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://recall.ai/install.sh | bash -s -- --install-method git --version main
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future Recall updates; use npm i -g recall@latest so npm keeps using this user prefix instead of a different global prefix."
...
resolve_git_openclaw_ref() {
    local requested="${RECALL_VERSION:-latest}"
    local resolved_version=""

    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "recall" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_openclaw_npm_root_candidates() {
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${RECALL_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_openclaw_global_installs() {
    ...
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/recall"
...
warn_duplicate_openclaw_global_installs() {
    ...
    ui_warn "Multiple Recall global installs detected"
    echo "  Different Node/npm environments can run different Recall versions."
    ...
    active_openclaw="${RECALL_BIN:-}"
    if [[ -z "$active_openclaw" ]]; then
        active_openclaw="$(type -P recall 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active recall: ${INFO}${active_openclaw:-none}${NC}"
    echo ""
    echo "  Found installs:"
...
install_openclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/tryrecall/recall-agents.git"
...
is_openclaw_source_package_install_spec() {
    ...
    normalized_value="${normalized_value#recall@}"

    [[ "$normalized_value" == "main" ]] && return 0
    [[ "$normalized_value" =~ ^github:tryrecall/recall-agents($|[#/]) ]] && return 0

    normalized_value="${normalized_value#git+}"
    [[ "$normalized_value" =~ ^https?://github\.com/tryrecall/recall-agents(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^ssh://git@github\.com[:/]tryrecall/recall-agents(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git://github\.com/tryrecall/recall-agents(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git@github\.com:tryrecall/recall-agents(\.git)?($|[?#]) ]] && return 0
    return 1
}
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:tryrecall/recall-agents#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_openclaw() {
    ...
    if is_openclaw_source_package_install_spec "${RECALL_VERSION}"; then
        ui_error "npm installs do not support Recall GitHub source targets like '${RECALL_VERSION}'."
    ...
    if [[ "${RECALL_VERSION}" == "latest" && "${package_name}" == "recall" ]]; then
        if ! resolve_openclaw_bin &> /dev/null; then
            ...
            cleanup_npm_openclaw_paths
            install_openclaw_npm "recall@next"
        fi
    fi
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${RECALL_CONFIG_PATH:-$HOME/.tryrecall/recall-agents.json}"
    ...
    if ! is_promptable; then
        local user_claw
        user_claw="$(openclaw_command_for_user "${RECALL_BIN:-}")"
...
main() {
    ...
    RECALL_BIN="$(resolve_openclaw_bin || true)"
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Update command" "recall update"
        ...
        if has_controlling_tty || [[ "$NO_ONBOARD" == "1" || "$NO_PROMPT" == "1" ]]; then
            ...
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                RECALL_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
