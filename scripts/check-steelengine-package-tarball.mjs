...
function usage() {
  return "Usage: node scripts/check-steelengine-package-tarball.mjs [--require-bundled-workspace-deps] <steelengine.tgz>";
}
...
function parseArgs(argv) {
  ...
  for (const rawArg of args) {
    ...
    if (arg.startsWith("-")) {
      throw new Error(`Unknown SteelEngine package tarball check option: ${arg}`);
    }
    if (tarball) {
      throw new Error(`Unexpected SteelEngine package tarball check argument: ${arg}`);
    }
    tarball = arg;
  }
  if (!tarball) {
...
if (!fs.existsSync(tarball)) {
  fail(`SteelEngine package tarball does not exist: ${tarball}`);
}
...
const REQUIRED_BUNDLED_WORKSPACE_DEPENDENCIES = ["@steelengine/ai"];
...
const REQUIRED_BUNDLED_WORKSPACE_RUNTIME_ENTRIES = new Map([
  [
    "@steelengine/ai",
    [
      { specifier: "@steelengine/ai", entry: "dist/index.mjs" },
      { specifier: "@steelengine/ai/providers", entry: "dist/providers.mjs" },
      {
        specifier: "@steelengine/ai/internal/runtime",
...
function collectRequiredBundledWorkspaceDependencyErrors(
  ...
  for (const name of REQUIRED_BUNDLED_WORKSPACE_DEPENDENCIES) {
    ...
    if (!bundledDependencies.has(name)) {
      errors.push(
        `package.json dependencies.${name} must be listed in bundleDependencies because it is private to the SteelEngine workspace`,
      );
    }
...
const phaseTimingsEnabled = process.env.STEELENGINE_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-steelengine-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-package-tarball-"));
try {
...
const PACKAGE_INSTALL_GUARD_RELATIVE_PATH = "dist/steelengine-install-guard";
...
if (!entrySet.has("npm-shrinkwrap.json")) {
  ...
  try {
    ...
    if (shrinkwrap.name !== "steelengine") {
      errors.push("npm-shrinkwrap.json root name must be steelengine");
    }
    ...
    if (!rootPackage || rootPackage.name !== "steelengine") {
      errors.push("npm-shrinkwrap.json packages root must name steelengine");
    }
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`SteelEngine package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`SteelEngine package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("SteelEngine package tarball integrity passed.");
