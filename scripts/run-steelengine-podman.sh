#!/usr/bin/env bash
# Rootless SteelEngine in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-steelengine-podman.sh launch        # Start gateway
#   ./scripts/run-steelengine-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   steelengine --container steelengine dashboard --no-open
#   steelengine --container steelengine channels login
#
...
run_podman_detached() {
  steelengine_host_timeout_cmd "$PODMAN_RUN_TIMEOUT" podman run "$@"
}
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      STEELENGINE_GATEWAY_TOKEN|STEELENGINE_PODMAN_CONTAINER|STEELENGINE_PODMAN_IMAGE|STEELENGINE_IMAGE|STEELENGINE_PODMAN_PULL|STEELENGINE_PODMAN_RUN_TIMEOUT|STEELENGINE_PODMAN_GATEWAY_HOST_PORT|STEELENGINE_GATEWAY_PORT|STEELENGINE_PODMAN_BRIDGE_HOST_PORT|STEELENGINE_BRIDGE_PORT|STEELENGINE_GATEWAY_BIND|STEELENGINE_PODMAN_USERNS|STEELENGINE_BIND_MOUNT_OPTIONS|STEELENGINE_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-steelengine-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${STEELENGINE_CONFIG_DIR:-$EFFECTIVE_HOME/.steelengine}"
ENV_FILE="${STEELENGINE_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${STEELENGINE_CONFIG_DIR:-$EFFECTIVE_HOME/.steelengine}"
ENV_FILE="${STEELENGINE_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${STEELENGINE_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${STEELENGINE_PODMAN_CONTAINER:-steelengine}"
STEELENGINE_IMAGE="${STEELENGINE_PODMAN_IMAGE:-${STEELENGINE_IMAGE:-steelengine:local}}"
PODMAN_PULL="${STEELENGINE_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${STEELENGINE_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${STEELENGINE_PODMAN_GATEWAY_HOST_PORT:-${STEELENGINE_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${STEELENGINE_PODMAN_BRIDGE_HOST_PORT:-${STEELENGINE_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${STEELENGINE_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$STEELENGINE_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  STEELENGINE_CONTAINER="" STEELENGINE_CONFIG_DIR="$config_dir" \
    steelengine config get gateway.bind 2>/dev/null || true
}
...
# STEELENGINE_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${STEELENGINE_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'STEELENGINE_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v steelengine >/dev/null 2>&1; then
    echo "Warning: steelengine not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    STEELENGINE_CONTAINER="" STEELENGINE_CONFIG_DIR="$config_dir" \
      steelengine config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    STEELENGINE_CONTAINER="" STEELENGINE_CONFIG_DIR="$config_dir" \
      steelengine config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  STEELENGINE_CONTAINER="" STEELENGINE_CONFIG_DIR="$config_dir" \
    steelengine config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${STEELENGINE_GATEWAY_TOKEN:-}" ]]; then
  export STEELENGINE_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "STEELENGINE_GATEWAY_TOKEN" "$STEELENGINE_GATEWAY_TOKEN"
  echo "Generated STEELENGINE_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/steelengine.json"
...
PODMAN_USERNS="${STEELENGINE_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported STEELENGINE_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (STEELENGINE_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${STEELENGINE_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${STEELENGINE_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$STEELENGINE_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.steelengine/.npm \
    -e STEELENGINE_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.steelengine:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.steelengine/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$STEELENGINE_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME steelengine dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME steelengine devices approve --latest  # if pairing required"
