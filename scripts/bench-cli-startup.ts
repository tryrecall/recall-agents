// Bench Cli Startup script supports SteelEngine repository automation.
...
const DEFAULT_ENTRY = "steelengine.mjs";
const MAX_RSS_MARKER = "__STEELENGINE_MAX_RSS_KB__=";

function resolveTimeoutKillGraceMs(env: NodeJS.ProcessEnv): number {
  const raw = env.VITEST ? env.STEELENGINE_TEST_CLI_STARTUP_TIMEOUT_KILL_GRACE_MS : undefined;
  if (!raw || !/^\d+$/u.test(raw)) {
...
function parseGatewayPortEnv(raw: string | undefined): number {
  ...
  if (bracketHostMatch) {
    return parsePositiveInt(bracketHostMatch[1], 32123, "STEELENGINE_GATEWAY_PORT");
  }
  ...
  return parsePositiveInt(portRaw, 32123, "STEELENGINE_GATEWAY_PORT");
}
...
function buildConfigFixture(commandCase: CommandCase): Record<string, unknown> | null {
  ...
  const port = parseGatewayPortEnv(process.env.STEELENGINE_GATEWAY_PORT);
  return {
    gateway: {
      auth: { mode: "none" },
      bind: "loopback",
...
async function runSample(params: {
  ...
  const runRoot = mkdtempSync(path.join(os.tmpdir(), "steelengine-cli-bench-home-"));
  const stateDir = path.join(runRoot, ".steelengine");
...
function printUsage(): void {
  console.log(`SteelEngine CLI benchmark
...
Options:
  ...
  --entry <path>               Primary entry file (default: steelengine.mjs)
...
async function main(): Promise<void> {
  ...
  const tmpDir = mkdtempSync(path.join(os.tmpdir(), "steelengine-cli-bench-"));
