...
const DEFAULT_ENTRY = "recall.mjs";
...
async function runSample(params: {
  ...
  const runRoot = mkdtempSync(path.join(os.tmpdir(), "recall-cli-bench-home-"));
  const stateDir = path.join(runRoot, ".recall");
...
function printUsage(): void {
  console.log(`Recall CLI benchmark
...
Options:
  ...
  --entry <path>               Primary entry file (default: recall.mjs)
...
async function main(): Promise<void> {
  ...
  const tmpDir = mkdtempSync(path.join(os.tmpdir(), "recall-cli-bench-"));
