// Realtime Talk Live Smoke script supports SteelEngine repository automation.
...
const OPENAI_REALTIME_MODEL =
  process.env.STEELENGINE_REALTIME_OPENAI_MODEL?.trim() || "gpt-realtime-2.1";
const OPENAI_REALTIME_VOICE = process.env.STEELENGINE_REALTIME_OPENAI_VOICE?.trim() || "alloy";
const DEFAULT_OPENAI_HTTP_TIMEOUT_MS = 30_000;
const OPENAI_HTTP_RESPONSE_MAX_BYTES = 256 * 1024;
const GOOGLE_REALTIME_MODEL =
  process.env.STEELENGINE_REALTIME_GOOGLE_MODEL?.trim() || "gemini-3.1-flash-live-preview";
const GOOGLE_REALTIME_VOICE = process.env.STEELENGINE_REALTIME_GOOGLE_VOICE?.trim() || "Kore";
...
type OpenAIWebRtcSmokeGlobal = typeof globalThis & {
  steelengineReadBoundedRealtimeResponseText?: OpenAIRealtimeBrowserResponseReader;
};
...
function resolveOpenAIHttpTimeoutMs(
  raw = process.env.STEELENGINE_REALTIME_OPENAI_HTTP_TIMEOUT_MS,
): number {
  return parseStrictIntegerOption({
    fallback: DEFAULT_OPENAI_HTTP_TIMEOUT_MS,
    label: "STEELENGINE_REALTIME_OPENAI_HTTP_TIMEOUT_MS",
    min: 1,
    raw,
  });
}
...
function openAIRealtimeBrowserResponseReaderInitScript(): string {
  return `globalThis.steelengineReadBoundedRealtimeResponseText = ${readOpenAIRealtimeBrowserResponseText.toString()};`;
}
...
async function smokeOpenAIBackendBridge(apiKey: string): Promise<SmokeResult> {
  ...
  const bridge = provider.createBridge({
    ...
    instructions: "SteelEngine backend realtime live smoke. Do not speak yet.",
    onAudio: () => {},
    onClearAudio: () => {},
    onEvent: (event) => {
      ...
      const result = await page.evaluate(
        async ({ clientSecret: secret, sdpAnswerMaxBytes, timeoutMs }) => {
          const readBoundedTextLocal = (globalThis as OpenAIWebRtcSmokeGlobal)
            .steelengineReadBoundedRealtimeResponseText;
...
async function smokeGoogleLiveBrowserWs(browser: Browser, apiKey: string): Promise<SmokeResult> {
  try {
    ...
    const session = await provider.createBrowserSession?.({
      ...
      voice: GOOGLE_REALTIME_VOICE,
      instructions:
        "SteelEngine browser Video Talk live smoke. After receiving a visual frame and request, call describe_view exactly once.",
...
async function smokeGatewayRelayBrowser(browser: Browser): Promise<SmokeResult> {
  ...
  const dir = await mkdtemp(path.join(tmpdir(), "steelengine-realtime-talk-"));
  try {
