// Ios Node E2E script supports SteelEngine repository automation.
...
function usage(): string {
  return [
    ...
    "Or set env: STEELENGINE_GATEWAY_URL / STEELENGINE_GATEWAY_TOKEN",
    "",
    "Options:",
...
const urlRaw = getArg("--url") ?? process.env.STEELENGINE_GATEWAY_URL;
const token = getArg("--token") ?? process.env.STEELENGINE_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.STEELENGINE_RUN_DANGEROUS === "1";
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "steelengine ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "steelengine-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "SteelEngine E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",
