// Gateway Smoke script supports SteelEngine repository automation.
...
function parseGatewaySmokeCli(
  ...
  return {
    help: has("--help") || has("-h"),
    token: getArg("--token") ?? env.STEELENGINE_GATEWAY_TOKEN,
    urlRaw: getArg("--url") ?? env.STEELENGINE_GATEWAY_URL,
  };
}
...
export async function runGatewaySmoke(
  ...
  try {
    ...
    const connectRes = await request("connect", {
      ...
      client: {
        id: "steelengine-ios",
        displayName: "steelengine gateway smoke test",
        version: "dev",
        platform: "dev",
        mode: "ui",
        instanceId: "steelengine-dev-smoke",
      },
      locale: "en-US",
