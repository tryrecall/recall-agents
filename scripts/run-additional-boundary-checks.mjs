...
export function parseCliArgs(args, env = process.env) {
  let shardSpec = env.STEELENGINE_ADDITIONAL_BOUNDARY_SHARD ?? "";
  let help = false;
  ...
  try {
    ...
    if (cliArgs.help) {
      ...
      const concurrencyRaw =
        process.env.STEELENGINE_ADDITIONAL_BOUNDARY_CONCURRENCY ??
        process.env.STEELENGINE_EXTENSION_BOUNDARY_CONCURRENCY;
      const concurrencyLabel =
        process.env.STEELENGINE_ADDITIONAL_BOUNDARY_CONCURRENCY === undefined
          ? "STEELENGINE_EXTENSION_BOUNDARY_CONCURRENCY"
          : "STEELENGINE_ADDITIONAL_BOUNDARY_CONCURRENCY";
      const concurrency = resolveConcurrency(concurrencyRaw, 4, concurrencyLabel);
      const checkTimeoutMs = resolvePositiveInteger(
        process.env.STEELENGINE_ADDITIONAL_BOUNDARY_TIMEOUT_MS,
        DEFAULT_CHECK_TIMEOUT_MS,
        "STEELENGINE_ADDITIONAL_BOUNDARY_TIMEOUT_MS",
      );
      const outputMaxBytes = resolvePositiveInteger(
        process.env.STEELENGINE_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES,
        DEFAULT_OUTPUT_MAX_BYTES,
        "STEELENGINE_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES",
      );
