#!/usr/bin/env bash
# Rootless Recall in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-recall-podman.sh launch        # Start gateway
#   ./scripts/run-recall-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   recall --container recall dashboard --no-open
#   recall --container recall channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      RECALL_GATEWAY_TOKEN|RECALL_PODMAN_CONTAINER|RECALL_PODMAN_IMAGE|RECALL_IMAGE|RECALL_PODMAN_PULL|RECALL_PODMAN_GATEWAY_HOST_PORT|RECALL_GATEWAY_PORT|RECALL_PODMAN_BRIDGE_HOST_PORT|RECALL_BRIDGE_PORT|RECALL_GATEWAY_BIND|RECALL_PODMAN_USERNS|RECALL_BIND_MOUNT_OPTIONS|RECALL_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-recall-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${RECALL_CONFIG_DIR:-$EFFECTIVE_HOME/.recall}"
ENV_FILE="${RECALL_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${RECALL_CONFIG_DIR:-$EFFECTIVE_HOME/.recall}"
ENV_FILE="${RECALL_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${RECALL_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${RECALL_PODMAN_CONTAINER:-recall}"
RECALL_IMAGE="${RECALL_PODMAN_IMAGE:-${RECALL_IMAGE:-recall:local}}"
PODMAN_PULL="${RECALL_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${RECALL_PODMAN_GATEWAY_HOST_PORT:-${RECALL_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${RECALL_PODMAN_BRIDGE_HOST_PORT:-${RECALL_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${RECALL_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$RECALL_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  RECALL_CONTAINER="" RECALL_CONFIG_DIR="$config_dir" \
    recall config get gateway.bind 2>/dev/null || true
}
...
# RECALL_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${RECALL_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v recall >/dev/null 2>&1; then
    echo "Warning: recall not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    RECALL_CONTAINER="" RECALL_CONFIG_DIR="$config_dir" \
      recall config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    RECALL_CONTAINER="" RECALL_CONFIG_DIR="$config_dir" \
      recall config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  RECALL_CONTAINER="" RECALL_CONFIG_DIR="$config_dir" \
    recall config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${RECALL_GATEWAY_TOKEN:-}" ]]; then
  export RECALL_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "RECALL_GATEWAY_TOKEN" "$RECALL_GATEWAY_TOKEN"
  echo "Generated RECALL_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/recall.json"
...
PODMAN_USERNS="${RECALL_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported RECALL_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (RECALL_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${RECALL_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${RECALL_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$RECALL_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.recall/.npm \
    -e RECALL_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.recall:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.recall/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$RECALL_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME recall dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME recall devices approve --latest  # if pairing required"
