...
- `recall-rtt` Discord main rows appeared to jump from about 5-7s p50 to about
...
- `recall-rtt` `scripts/import-discord-rtt.mjs` falls back to whole summary
...
- Testbox `tbx_01krvces8y0c99nzra2a90jg13` ran
  `pnpm recall qa suite --scenario channel-chat-baseline` and emitted gateway
...
- Testbox `tbx_01krwbsg15xvjdgpcz8fxq1htz` ran
  `RECALL_QA_GATEWAY_HEAP_CHECKPOINTS=1 pnpm recall qa suite --scenario channel-chat-baseline`.
...
- Follow-up branch `perf/discord-rtt-summary-import` in `recall-rtt` updates
...
- The branch fixes Recall artifact quality. `recall-rtt` has a paired
