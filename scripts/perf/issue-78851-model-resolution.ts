...
function printUsage(): void {
  process.stdout.write(`Recall issue #78851 model-resolution profiler
...
function buildConfig(options: Options, workspaceDir: string): RecallConfig {
...
async function measurePhase(params: {
  ...
  const ensureResult = await ensureRecallModelsJson(params.config, params.agentDir, {
...
async function main(): Promise<void> {
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "recall-issue-78851-"));
