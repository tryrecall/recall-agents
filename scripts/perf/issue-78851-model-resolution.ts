// Issue 78851 Model Resolution script supports SteelEngine repository automation.
...
import { ensureSteelEngineModelsJson } from "../../src/agents/models-config.js";
...
function buildConfig(options: Options, workspaceDir: string): SteelEngineConfig {
  const providers: NonNullable<NonNullable<SteelEngineConfig["models"]>["providers"]> = {};
...
async function measurePhase(params: {
  ...
  const ensureResult = await ensureSteelEngineModelsJson(params.config, params.agentDir, {
...
async function main(): Promise<void> {
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "steelengine-issue-78851-"));
