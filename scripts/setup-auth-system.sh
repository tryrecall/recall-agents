#!/bin/bash
# Setup Recall Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'recall-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Recall message: Send warning via Recall itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/recall-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/recall-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/recall-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now recall-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status recall-auth-monitor.timer"
echo ""
echo "Quick commands:"
