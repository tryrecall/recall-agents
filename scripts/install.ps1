# Recall Installer for Windows (PowerShell)
# Usage: iwr -useb https://recall.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://recall.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\recall",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 Recall Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one Recall.$NC" -Level info
    Write-Host ""
}
...
function Install-RecallGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Recall from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/recall/recall.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\recall\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\recall.cmd" -Encoding ASCII -Force
...
function Resolve-PackageInstallSpec {
    ...
    if ([string]::IsNullOrWhiteSpace($trimmed)) {
        return "recall@latest"
    }
    if ($trimmed.ToLowerInvariant() -eq "main") {
        return "github:recall/recall#main"
    }
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Recall from git to $GitDir" -Level info
        } else {
            Install-RecallGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Recall via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-RecallNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'recall onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 Recall installed successfully!" -Level success
}
