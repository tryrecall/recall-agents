# SteelEngine Installer for Windows
# Usage: powershell -c "irm https://steelengine.ai/install.ps1 | iex"
#        powershell -c "& ([scriptblock]::Create((irm https://steelengine.ai/install.ps1))) -Tag beta -NoOnboard -DryRun"
...
function Complete-Install {
    ...
    throw "SteelEngine installation failed with exit code $($script:InstallExitCode)."
}
...
Write-Host ""
Write-Host "  SteelEngine Installer" -ForegroundColor Cyan
Write-Host ""
...
if (-not $PSBoundParameters.ContainsKey("InstallMethod")) {
    if (-not [string]::IsNullOrWhiteSpace($env:STEELENGINE_INSTALL_METHOD)) {
        $InstallMethod = $env:STEELENGINE_INSTALL_METHOD
    }
}
if (-not $PSBoundParameters.ContainsKey("GitDir")) {
    if (-not [string]::IsNullOrWhiteSpace($env:STEELENGINE_GIT_DIR)) {
        $GitDir = $env:STEELENGINE_GIT_DIR
    }
}
if (-not $PSBoundParameters.ContainsKey("NoOnboard")) {
    if ($env:STEELENGINE_NO_ONBOARD -eq "1") {
        $NoOnboard = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("NoGitUpdate")) {
    if ($env:STEELENGINE_GIT_UPDATE -eq "0") {
        $NoGitUpdate = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("DryRun")) {
    if ($env:STEELENGINE_DRY_RUN -eq "1") {
        $DryRun = $true
    }
}
...
if ([string]::IsNullOrWhiteSpace($GitDir)) {
    $userHome = [Environment]::GetFolderPath("UserProfile")
    $GitDir = (Join-Path $userHome "steelengine")
}
...
function Get-SteelEngineDepsRoot {
    ...
    return (Join-Path $localAppData "SteelEngine\deps")
}

function Get-PortableNodeRoot {
    return (Join-Path (Get-SteelEngineDepsRoot) "portable-node")
}
...
function Ensure-PortableNodeOnUserPath {
    ...
    if (Add-ToUserPath $nodeDir) {
        Write-Host "[!] Added $nodeDir to user PATH (restart terminal if node or steelengine is not found)" -ForegroundColor Yellow
    }
}
...
function Check-ExistingSteelEngine {
    if (Get-SteelEngineCommandPath) {
        Write-Host "[*] Existing SteelEngine installation detected" -ForegroundColor Yellow
        return $true
    }
    return $false
}
...
function Get-PortableGitRoot {
    return (Join-Path (Get-SteelEngineDepsRoot) "portable-git")
}
...
function Resolve-PortableGitDownload {
    ...
    $headers = @{
        "User-Agent" = "steelengine-installer"
...
function Install-PortableGit {
    ...
    $portableRoot = Get-PortableGitRoot
    $portableParent = Split-Path -Parent $portableRoot
    $tempName = "steelengine-portable-git-" + [guid]::NewGuid().ToString("N")
...
function Ensure-Git {
    ...
    Write-Host ""
    Write-Host "Error: Git is required to install SteelEngine." -ForegroundColor Red
...
function Get-SteelEngineCommandPath {
    $steelengineCmd = Get-Command steelengine.cmd -ErrorAction SilentlyContinue
    if ($steelengineCmd -and $steelengineCmd.Source) {
        return $steelengineCmd.Source
    }

    $steelengine = Get-Command steelengine -ErrorAction SilentlyContinue
    if ($steelengine -and $steelengine.Source) {
        return $steelengine.Source
    }

    return $null
}

function Invoke-SteelEngineCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )

    $commandPath = Get-SteelEngineCommandPath
    if (-not $commandPath) {
        throw "steelengine command not found on PATH."
    }

    & $commandPath @Arguments
}

function Invoke-InteractiveSteelEngineCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )
    ...
    if ($process.ExitCode -ne 0) {
        throw "steelengine $($Arguments -join ' ') failed with exit code $($process.ExitCode)."
    }
}
...
function Ensure-SteelEngineOnPath {
    if (Get-SteelEngineCommandPath) {
        return $true
    }
    ...
    foreach ($npmBin in $npmBins) {
        if (-not (Test-Path (Join-Path $npmBin "steelengine.cmd"))) {
            continue
        }
    ...
    Write-Host "[!] steelengine is not on PATH yet." -ForegroundColor Yellow
...
function Resolve-NpmSteelEngineInstallSpec {
    param(
        [string]$PackageName,
        [string]$RequestedTag
    )
...
function Test-SteelEngineSourcePackageInstallSpec {
    ...
    if ($normalizedTag.StartsWith("steelengine@")) {
        $normalizedTag = $normalizedTag.Substring("steelengine@".Length)
    }

    if ($normalizedTag -eq "main") {
        return $true
    }
    if ($normalizedTag -match '^github:steelengine/steelengine($|[#/])') {
        return $true
    }
    ...
    return (
        $normalizedTag -match '^https?://github\.com/steelengine/steelengine(\.git)?($|[?#])' -or
        $normalizedTag -match '^ssh://git@github\.com[:/]steelengine/steelengine(\.git)?($|[?#])' -or
        $normalizedTag -match '^git://github\.com/steelengine/steelengine(\.git)?($|[?#])' -or
        $normalizedTag -match '^git@github\.com:steelengine/steelengine(\.git)?($|[?#])'
    )
}
...
function Install-SteelEngine {
    if ([string]::IsNullOrWhiteSpace($Tag)) {
        $Tag = "latest"
    }
    if (Test-SteelEngineSourcePackageInstallSpec -RequestedTag $Tag) {
        Write-Host "Error: npm installs do not support SteelEngine GitHub source targets like '$Tag'." -ForegroundColor Red
    ...
    # Use steelengine package for beta, steelengine for stable
    $packageName = "steelengine"
    if ($Tag -eq "beta" -or $Tag -match "^beta\.") {
        $packageName = "steelengine"
    }
    $installSpec = Resolve-NpmSteelEngineInstallSpec -PackageName $packageName -RequestedTag $Tag
    Write-Host "[*] Installing SteelEngine ($installSpec)..." -ForegroundColor Yellow
    ...
    try {
        ...
        if ($npmInstallStatus -ne 0) {
            ...
            if ($npmOutput -match "spawn git" -or $npmOutput -match "ENOENT.*git") {
                ...
                Write-Host '  powershell -c "irm https://steelengine.ai/install.ps1 | iex"' -ForegroundColor Cyan
            }
    ...
    Write-Host "[OK] SteelEngine installed" -ForegroundColor Green
    return $true
}

# Install SteelEngine from GitHub
function Install-SteelEngineFromGit {
    param(
        [string]$RepoDir,
        [switch]$SkipUpdate
    )
    ...
    $repoUrl = "https://github.com/steelengineai/recall-agents.git"
    Write-Host "[*] Installing SteelEngine from GitHub ($repoUrl)..." -ForegroundColor Yellow
    ...
    if (-not (Test-Path $entryPath)) {
        Write-Host "[!] SteelEngine build did not produce $entryPath" -ForegroundColor Red
        return $false
    }
    ...
    $cmdPath = Join-Path $binDir "steelengine.cmd"
    ...
    Write-Host "[OK] SteelEngine wrapper installed to $cmdPath" -ForegroundColor Green
...
function Run-Doctor {
    ...
    try {
        Invoke-SteelEngineCommand doctor --non-interactive
    } catch {
...
function Test-GatewayServiceLoaded {
    try {
        $statusJson = (Invoke-SteelEngineCommand daemon status --json 2>$null)
...
function Refresh-GatewayServiceIfLoaded {
    if (-not (Get-SteelEngineCommandPath)) {
        return
    }
    ...
    try {
        Invoke-SteelEngineCommand gateway install --force | Out-Null
    } catch {
    ...
    try {
        Invoke-SteelEngineCommand gateway restart | Out-Null
        Invoke-SteelEngineCommand gateway status --json | Out-Null
        Write-Host "[OK] Gateway service refreshed" -ForegroundColor Green
    } catch {
        Write-Host "[!] Gateway service restart failed; continuing. Run: steelengine gateway restart" -ForegroundColor Yellow
    }
}

function Get-LegacyRepoDir {
    if (-not [string]::IsNullOrWhiteSpace($env:STEELENGINE_GIT_DIR)) {
        return $env:STEELENGINE_GIT_DIR
    }
    $userHome = [Environment]::GetFolderPath("UserProfile")
    return (Join-Path $userHome "steelengine")
}
...
function Main {
    ...
    $isUpgrade = Check-ExistingSteelEngine
    ...
    if ($InstallMethod -eq "git") {
        try {
            ...
            if ($npmCommand) {
                Invoke-NpmCommand -Arguments @("uninstall", "-g", "steelengine") 2>$null | Out-Null
                Write-Host "[OK] Removed npm global install if present" -ForegroundColor Green
            }
        } catch { }
        $finalGitDir = $GitDir
        $gitInstallResults = @(Install-SteelEngineFromGit -RepoDir $GitDir -SkipUpdate:$NoGitUpdate)
        if (-not (Test-BooleanSuccessResult -Results $gitInstallResults)) {
            return (Fail-Install)
        }
    } else {
        $gitWrapper = Join-Path (Join-Path $env:USERPROFILE ".local\\bin") "steelengine.cmd"
        ...
        $npmInstallResults = @(Install-SteelEngine)
        if (-not (Test-BooleanSuccessResult -Results $npmInstallResults)) {
            return (Fail-Install)
        }
    }

    if (-not (Ensure-SteelEngineOnPath)) {
        Write-Host "Install completed, but SteelEngine is not on PATH yet." -ForegroundColor Yellow
        Write-Host "Open a new terminal, then run: steelengine doctor" -ForegroundColor Cyan
        return
    }
    ...
    try {
        $installedVersion = (Invoke-SteelEngineCommand --version 2>$null).Trim()
    } catch {
        $installedVersion = $null
    }
    if (-not $installedVersion) {
        try {
            ...
            if ($npmList -and $npmList.dependencies -and $npmList.dependencies.steelengine -and $npmList.dependencies.steelengine.version) {
                $installedVersion = $npmList.dependencies.steelengine.version
            }
        } catch {
            $installedVersion = $null
        }
    ...
    Write-Host ""
    if ($installedVersion) {
        Write-Host "SteelEngine installed successfully ($installedVersion)!" -ForegroundColor Green
    } else {
        Write-Host "SteelEngine installed successfully!" -ForegroundColor Green
    }
    Write-Host ""
    if ($isUpgrade) {
        $updateMessages = @(
    ...
    if ($InstallMethod -eq "git") {
        ...
        Write-Host "Wrapper: $env:USERPROFILE\\.local\\bin\\steelengine.cmd" -ForegroundColor Cyan
        Write-Host ""
    }

    if ($isUpgrade) {
        Write-Host "Upgrade complete. Run " -NoNewline
        Write-Host "steelengine doctor" -ForegroundColor Cyan -NoNewline
        Write-Host " to check for additional migrations."
    } else {
        if ($NoOnboard) {
            Write-Host "Skipping onboard (requested). Run " -NoNewline
            Write-Host "steelengine onboard" -ForegroundColor Cyan -NoNewline
            Write-Host " later."
        } else {
            Write-Host "Starting setup..." -ForegroundColor Cyan
            Write-Host ""
            Invoke-InteractiveSteelEngineCommand onboard
        }
    }
