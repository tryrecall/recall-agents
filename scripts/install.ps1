...
# Usage: powershell -c "irm https://recall.ai/install.ps1 | iex"
#        powershell -c "& ([scriptblock]::Create((irm https://recall.ai/install.ps1))) -Tag beta -NoOnboard -DryRun"
...
function Complete-Install {
    ...
    throw "Recall installation failed with exit code $($script:InstallExitCode)."
}
...
if (-not $PSBoundParameters.ContainsKey("InstallMethod")) {
    if (-not [string]::IsNullOrWhiteSpace($env:RECALL_INSTALL_METHOD)) {
        $InstallMethod = $env:RECALL_INSTALL_METHOD
    }
}
if (-not $PSBoundParameters.ContainsKey("GitDir")) {
    if (-not [string]::IsNullOrWhiteSpace($env:RECALL_GIT_DIR)) {
        $GitDir = $env:RECALL_GIT_DIR
    }
}
if (-not $PSBoundParameters.ContainsKey("NoOnboard")) {
    if ($env:RECALL_NO_ONBOARD -eq "1") {
        $NoOnboard = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("NoGitUpdate")) {
    if ($env:RECALL_GIT_UPDATE -eq "0") {
        $NoGitUpdate = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("DryRun")) {
    if ($env:RECALL_DRY_RUN -eq "1") {
        $DryRun = $true
    }
}
...
if ([string]::IsNullOrWhiteSpace($GitDir)) {
    ...
    $GitDir = (Join-Path $userHome "recall")
}
...
function Get-RecallDepsRoot {
    ...
    return (Join-Path $localAppData "Recall\deps")
}

function Get-PortableNodeRoot {
    return (Join-Path (Get-RecallDepsRoot) "portable-node")
}
...
function Ensure-PortableNodeOnUserPath {
    ...
    if (Add-ToUserPath $nodeDir) {
        Write-Host "[!] Added $nodeDir to user PATH (restart terminal if node or recall is not found)" -ForegroundColor Yellow
    }
}
...
function Check-ExistingRecall {
    if (Get-RecallCommandPath) {
        Write-Host "[*] Existing Recall installation detected" -ForegroundColor Yellow
        return $true
    }
    return $false
}
...
function Get-PortableGitRoot {
    return (Join-Path (Get-RecallDepsRoot) "portable-git")
}
...
function Install-PortableGit {
    ...
    $portableParent = Split-Path -Parent $portableRoot
    $tmpZip = Join-Path $env:TEMP $download.Name
    $tmpExtract = Join-Path $env:TEMP ("recall-portable-git-" + [guid]::NewGuid().ToString("N"))
...
function Ensure-Git {
    ...
    Write-Host ""
    Write-Host "Error: Git is required to install Recall." -ForegroundColor Red
...
function Get-RecallCommandPath {
    $openclawCmd = Get-Command recall.cmd -ErrorAction SilentlyContinue
    if ($openclawCmd -and $openclawCmd.Source) {
        return $openclawCmd.Source
    }

    $recall = Get-Command recall -ErrorAction SilentlyContinue
    if ($recall -and $recall.Source) {
...
function Invoke-RecallCommand {
    ...
    $commandPath = Get-RecallCommandPath
    if (-not $commandPath) {
        throw "recall command not found on PATH."
    }

    & $commandPath @Arguments
}

function Invoke-InteractiveRecallCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )
...
function Ensure-RecallOnPath {
    ...
    foreach ($npmBin in $npmBins) {
        if (-not (Test-Path (Join-Path $npmBin "recall.cmd"))) {
            continue
        }
    ...
    Write-Host "[!] recall is not on PATH yet." -ForegroundColor Yellow
...
function Resolve-NpmRecallInstallSpec {
    param(
        [string]$PackageName,
        [string]$RequestedTag
    )
...
function Test-RecallSourcePackageInstallSpec {
    ...
    if ($normalizedTag.StartsWith("recall@")) {
        $normalizedTag = $normalizedTag.Substring("recall@".Length)
    }

    if ($normalizedTag -eq "main") {
        return $true
    }
    if ($normalizedTag -match '^github:tryrecall/recall-agents($|[#/])') {
        return $true
    }
    ...
    return (
        $normalizedTag -match '^https?://github\.com/tryrecall/recall-agents(\.git)?($|[?#])' -or
        $normalizedTag -match '^ssh://git@github\.com[:/]tryrecall/recall-agents(\.git)?($|[?#])' -or
        $normalizedTag -match '^git://github\.com/tryrecall/recall-agents(\.git)?($|[?#])' -or
        $normalizedTag -match '^git@github\.com:tryrecall/recall-agents(\.git)?($|[?#])'
    )
}
...
function Install-Recall {
    ...
    if (Test-RecallSourcePackageInstallSpec -RequestedTag $Tag) {
        Write-Host "Error: npm installs do not support Recall GitHub source targets like '$Tag'." -ForegroundColor Red
    ...
    # Use recall package for beta, recall for stable
    $packageName = "recall"
    if ($Tag -eq "beta" -or $Tag -match "^beta\.") {
        $packageName = "recall"
    }
    $installSpec = Resolve-NpmRecallInstallSpec -PackageName $packageName -RequestedTag $Tag
    Write-Host "[*] Installing Recall ($installSpec)..." -ForegroundColor Yellow
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if ($npmOutput -match "spawn git" -or $npmOutput -match "ENOENT.*git") {
                ...
                Write-Host '  powershell -c "irm https://recall.ai/install.ps1 | iex"' -ForegroundColor Cyan
            }
    ...
    Write-Host "[OK] Recall installed" -ForegroundColor Green
    return $true
}

# Install Recall from GitHub
function Install-RecallFromGit {
    ...
    $repoUrl = "https://github.com/tryrecall/recall-agents.git"
    Write-Host "[*] Installing Recall from GitHub ($repoUrl)..." -ForegroundColor Yellow
    ...
    if (-not (Test-Path $entryPath)) {
        Write-Host "[!] Recall build did not produce $entryPath" -ForegroundColor Red
        return $false
    }
    ...
    $cmdPath = Join-Path $binDir "recall.cmd"
    ...
    Write-Host "[OK] Recall wrapper installed to $cmdPath" -ForegroundColor Green
...
function Run-Doctor {
    ...
    try {
        Invoke-RecallCommand doctor --non-interactive
    } catch {
...
function Test-GatewayServiceLoaded {
    try {
        $statusJson = (Invoke-RecallCommand daemon status --json 2>$null)
...
function Refresh-GatewayServiceIfLoaded {
    ...
    try {
        Invoke-RecallCommand gateway install --force | Out-Null
    } catch {
    ...
    try {
        Invoke-RecallCommand gateway restart | Out-Null
        Invoke-RecallCommand gateway status --json | Out-Null
...
function Get-LegacyRepoDir {
    if (-not [string]::IsNullOrWhiteSpace($env:RECALL_GIT_DIR)) {
        return $env:RECALL_GIT_DIR
    }
    $userHome = [Environment]::GetFolderPath("UserProfile")
    return (Join-Path $userHome "recall")
}
...
function Main {
    ...
    $isUpgrade = Check-ExistingRecall
    ...
    if ($InstallMethod -eq "git") {
        try {
            ...
            if ($npmCommand) {
                Invoke-NpmCommand -Arguments @("uninstall", "-g", "recall") 2>$null | Out-Null
                Write-Host "[OK] Removed npm global install if present" -ForegroundColor Green
            }
        } catch { }
        $finalGitDir = $GitDir
        $gitInstallResults = @(Install-RecallFromGit -RepoDir $GitDir -SkipUpdate:$NoGitUpdate)
        if (-not (Test-BooleanSuccessResult -Results $gitInstallResults)) {
            return (Fail-Install)
        }
    } else {
        $gitWrapper = Join-Path (Join-Path $env:USERPROFILE ".local\\bin") "recall.cmd"
        ...
        $npmInstallResults = @(Install-Recall)
        if (-not (Test-BooleanSuccessResult -Results $npmInstallResults)) {
            return (Fail-Install)
        }
    }

    if (-not (Ensure-RecallOnPath)) {
        Write-Host "Install completed, but Recall is not on PATH yet." -ForegroundColor Yellow
        Write-Host "Open a new terminal, then run: recall doctor" -ForegroundColor Cyan
        return
    }
    ...
    try {
        $installedVersion = (Invoke-RecallCommand --version 2>$null).Trim()
    } catch {
        $installedVersion = $null
    }
    if (-not $installedVersion) {
        try {
            ...
            if ($npmList -and $npmList.dependencies -and $npmList.dependencies.recall -and $npmList.dependencies.recall.version) {
                $installedVersion = $npmList.dependencies.recall.version
            }
        } catch {
            $installedVersion = $null
        }
    ...
    Write-Host ""
    if ($installedVersion) {
        Write-Host "Recall installed successfully ($installedVersion)!" -ForegroundColor Green
    } else {
        Write-Host "Recall installed successfully!" -ForegroundColor Green
    }
    Write-Host ""
    if ($isUpgrade) {
        $updateMessages = @(
    ...
    if ($InstallMethod -eq "git") {
        ...
        Write-Host "Wrapper: $env:USERPROFILE\\.local\\bin\\recall.cmd" -ForegroundColor Cyan
        Write-Host ""
    }

    if ($isUpgrade) {
        Write-Host "Upgrade complete. Run " -NoNewline
        Write-Host "recall doctor" -ForegroundColor Cyan -NoNewline
        Write-Host " to check for additional migrations."
    } else {
        if ($NoOnboard) {
            Write-Host "Skipping onboard (requested). Run " -NoNewline
            Write-Host "recall onboard" -ForegroundColor Cyan -NoNewline
            Write-Host " later."
        } else {
            Write-Host "Starting setup..." -ForegroundColor Cyan
            Write-Host ""
            Invoke-InteractiveRecallCommand onboard
        }
    }
