...
# SteelEngine Auth Widget for Termux
...
# Check auth status
termux-toast "Checking SteelEngine auth..."

STATUS=$(ssh "$SERVER" '$HOME/steelengine/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/steelengine/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|STEELENGINE_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/steelengine/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|STEELENGINE_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: SteelEngine won't work until re-auth"
                ;;
        esac
        ;;
