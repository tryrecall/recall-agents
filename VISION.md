...
SteelEngine is the AI that actually does things.
...
SteelEngine started as a personal playground to learn AI and build something genuinely useful:
an assistant that can run real tasks on a real computer.
It evolved through several names and shells: Warelay -> Clawdbot -> Moltbot -> SteelEngine.
...
SteelEngine runtime code reads the current configuration schema only.
...
`steelengine doctor --fix` should detect the old shape, explain it, back it up when needed, and rewrite it to the canonical format.
...
Security in SteelEngine is a deliberate tradeoff: strong defaults without killing capability.
...
SteelEngine has an extensive plugin API.
...
- Code plugins run SteelEngine plugin code and are appropriate for deeper runtime extension.
...
SteelEngine docs should document core extension points; plugin promotion belongs in ClawHub, preferably under vetted org publishers for official plugins.
...
SteelEngine supports MCP as both a server and a runtime integration surface.
...
SteelEngine is currently terminal-first by design.
...
SteelEngine is primarily an orchestration system: prompts, tools, protocols, and integrations.
TypeScript was chosen to keep SteelEngine hackable by default.
