  ...
  it("loads explicit extensionPath without importing bundled sqlite-vec", async () => {
    ...
    await expect(
      loadSqliteVecExtension({
        db: db as never,
        extensionPath: "/opt/recall/sqlite-vec.so",
      }),
    ).resolves.toEqual({ ok: true, extensionPath: "/opt/recall/sqlite-vec.so" });
    expect(db.enableLoadExtension).toHaveBeenCalledWith(true);
    expect(db.loadExtension).toHaveBeenCalledWith("/opt/recall/sqlite-vec.so");
  });
