...
import type { SteelEngineConfig } from "./config-utils.js";
...
const rootMemoryConfig = (workspaceDir: string): SteelEngineConfig =>
  ({
    agents: {
...
describe("resolveMemoryBackendConfig", () => {
  it("defaults to builtin backend when config missing", () => {
    const cfg = { agents: { defaults: { workspace: "/tmp/memory-test" } } } as SteelEngineConfig;
  ...
  it("preserves unquoted Windows absolute qmd command paths", () => {
    ...
    const cfg = {
      agents: { defaults: { workspace: String.raw`C:\Users\penny\.steelengine\workspace` } },
      memory: {
        backend: "qmd",
        qmd: {
          command,
