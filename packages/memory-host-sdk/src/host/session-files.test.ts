...
beforeEach(() => {
  ...
  Reflect.set(process.env, "STEELENGINE_STATE_DIR", tmpDir);
  clearRuntimeConfigSnapshot();
  clearConfigCache();
});
...
describe("memory session sync targets", () => {
  it("parses deprecated canonical SteelEngine transcript paths into sync identity", () => {
...
describe("buildSessionEntry", () => {
  it("returns lineMap tracking original JSONL line numbers", async () => {
    ...
    const jsonlLines = [
      ...
      JSON.stringify({ type: "custom", customType: "steelengine.cache-ttl", data: {} }),
  ...
  it("drops every assistant response in a provenance-marked heartbeat turn", async () => {
    const jsonlLines = [
      JSON.stringify({
        ...
        message: {
          role: "user",
          content: "[SteelEngine heartbeat poll]",
