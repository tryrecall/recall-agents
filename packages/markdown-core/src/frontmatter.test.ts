...
`;
    const result = parseFrontmatterBlock(content);
    expect(result.metadata).toBe('{"steelengine":{"emoji":"disk","events":["command:new"]}}');

    const parsed = JSON5.parse(expectDefined(result.metadata, "result.metadata test invariant"));
    expect(parsed.steelengine?.emoji).toBe("disk");
  });
...
---`;
    const result = parseFrontmatterBlock(content);
    expect(result.metadata).toBe('{"steelengine":true}');
  });
