...
/** Resolve the underlying OS home before applying SteelEngine overrides. */
...
/** Resolve raw home with STEELENGINE_HOME tilde expansion. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  return explicitHome ? { home, prefix: "$STEELENGINE_HOME" } : { home, prefix: "~" };
}
...
/** Replace the effective home path with "~" or "$STEELENGINE_HOME" for terminal display. */
