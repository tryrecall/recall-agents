...
import type { SteelEngineConfig, TtsConfig } from "steelengine/plugin-sdk/config-contracts";
...
vi.mock("steelengine/plugin-sdk/channel-targets", () => ({
...
vi.mock("steelengine/plugin-sdk/speech-core", async () => {
  const actual = await vi.importActual("steelengine/plugin-sdk/speech-core");
...
function createTtsConfig(prefsName: string): SteelEngineConfig {
  return {
    messages: {
      tts: {
        enabled: true,
...
describe("speech-core native voice-note routing", () => {
  ...
  it("tells generic TTS guidance to defer to MEMORY voice-delivery instructions", () => {
    const hint = buildTtsSystemPromptHint(createTtsConfig("steelengine-speech-core-tts-hint-test"));
  ...
  it("marks Discord auto TTS replies as native voice messages", async () => {
    await expectTtsPayloadResult({
      channel: "discord",
      prefsName: "steelengine-speech-core-tts-test",
  ...
  it("keeps compatible audio-file synthesis deliverable as a voice memo", async () => {
    await expectTtsPayloadResult({
      ...
      prefsName: "steelengine-speech-core-tts-voice-memo-mp3-test",
  ...
  it("does not mark unsupported audio-file output as a voice memo", async () => {
    await expectTtsPayloadResult({
      channel: "voice-memo-chat",
      prefsName: "steelengine-speech-core-tts-voice-memo-ogg-test",
  ...
  it("pre-transcodes synthesized mp3 to opus-in-CAF when the host can satisfy preferAudioFileFormat", async () => {
    ...
    await expectTtsPayloadResult({
      channel: "voice-memo-chat",
      prefsName: "steelengine-speech-core-tts-voice-memo-caf-transcode-test",
  ...
  it("falls back to the original mp3 buffer when the host transcoder fails", async () => {
    ...
    await expectTtsPayloadResult({
      channel: "voice-memo-chat",
      prefsName: "steelengine-speech-core-tts-voice-memo-caf-fallback-test",
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/steelengine-speech-core-voice-model-default-test.json",
          },
        },
      } as SteelEngineConfig,
      disableFallback: true,
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            provider: "openrouter",
            prefsPath: "/tmp/steelengine-speech-core-explicit-model-alias-test.json",
            providers: {
              openrouter: {
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/steelengine-speech-core-voice-model-fallback-test.json",
          },
        },
      } as SteelEngineConfig,
    });
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/steelengine-speech-core-same-provider-voice-model-fallback-test.json",
          },
        },
      } as SteelEngineConfig,
    });
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            provider: "openai",
            prefsPath: "/tmp/steelengine-speech-core-realtime-voice-model-ignored-test.json",
          },
        },
      } as SteelEngineConfig,
      disableFallback: true,
    ...
    const result = await synthesizeSpeech({
      ...
      cfg: {
        ...
        messages: {
          tts: {
            enabled: true,
            prefsPath: "/tmp/steelengine-speech-core-supported-voice-model-provider-test.json",
          },
        },
      } as SteelEngineConfig,
    });
  ...
  it.each(["feishu", "whatsapp"] as const)(
    ...
    async (channel) => {
      ...
      await expectTtsPayloadResult({
        channel,
        prefsName: `steelengine-speech-core-tts-${channel}-mp3-test`,
  ...
  it("keeps non-native voice-note channels as regular audio files", async () => {
    await expectTtsPayloadResult({
      channel: "slack",
      prefsName: "steelengine-speech-core-tts-slack-test",
  ...
  it("synthesizes explicitly tagged short hidden TTS text", async () => {
    const cfg = createTtsConfig("steelengine-speech-core-short-hidden-tts-test");
    let mediaDir: string | undefined;
    try {
      const result = await maybeApplyTtsToPayload({
        payload: {
  ...
  it("truncates long TTS text on a UTF-16 boundary", async () => {
    const prefsName = "steelengine-speech-core-utf16-truncate-test";
  ...
  it("skips block delivery kind in final mode (accumulated final tail synthesizes instead)", async () => {
    synthesizeMock.mockClear();
    const cfg = createTtsConfig("steelengine-speech-core-block-kind-tts-test");
    const result = await maybeApplyTtsToPayload({
  ...
  it("skips tool delivery kind in final mode", async () => {
    synthesizeMock.mockClear();
    const cfg = createTtsConfig("steelengine-speech-core-tool-kind-tts-test");
    const result = await maybeApplyTtsToPayload({
  ...
  it("keeps skipping untagged short TTS text", async () => {
    const cfg = createTtsConfig("steelengine-speech-core-short-plain-tts-test");
    const result = await maybeApplyTtsToPayload({
      payload: {
        text: "hello",
        audioAsVoice: true,
  ...
  it("skips auto TTS for legacy final media directives", async () => {
    synthesizeMock.mockClear();
    const cfg = createTtsConfig("steelengine-speech-core-media-directive-tts-test");
    const result = await maybeApplyTtsToPayload({
  ...
  it("keeps skipping explicit tagged TTS text that strips to empty markdown", async () => {
    const cfg = createTtsConfig("steelengine-speech-core-empty-hidden-tts-test");
    const result = await maybeApplyTtsToPayload({
      payload: {
  ...
  it("selects persona preferred provider before config fallback", () => {
    ...
    const prefsPath = "/tmp/steelengine-speech-core-persona-provider.json";
  ...
  it("treats provider configuration errors as unconfigured", () => {
    ...
    const cfg = {
      messages: {
        tts: {
          providers: { broken: {} },
          prefsPath: "/tmp/steelengine-speech-core-invalid-provider.json",
        },
      },
    } as SteelEngineConfig;
    const config = resolveTtsConfig(cfg);
    ...
    const cfg: SteelEngineConfig = {
      messages: {
        tts: {
          enabled: true,
          provider: "mock",
          prefsPath: "/tmp/steelengine-speech-core-persona-merge.json",
          providers: {
            mock: {
              model: "base-model",
              voice: "base-voice",
