...
function resolveGatewayUrl(options: RecallOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `Recall Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current Recall Gateway yet`);
}
...
function readChatProjection(event: RecallEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: RecallEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: RecallEvent): boolean {
  return (
...
export class Recall {
  ...
  private readonly transport: RecallTransport;
  private readonly normalizedEvents = new EventHub<RecallEvent>({
    replayLimit: MAX_NORMALIZED_REPLAY_EVENTS,
  });
  private readonly replayByRunId = new Map<string, RecallEvent[]>();
  ...
  events(filter?: (event: RecallEvent) => boolean): AsyncIterable<RecallEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: RecallEvent) => boolean,
  ): AsyncIterable<RecallEvent> {
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: RecallEvent): RecallEvent | undefined => {
    ...
    const matches = (event: RecallEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: RecallEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): RecallEvent[] {
...
export class Agent {
  constructor(
    private readonly client: Recall,
    readonly id: string,
  ) {}
...
export class AgentsNamespace {
  constructor(private readonly client: Recall) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<RecallEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: Recall,
