// SteelEngine SDK module implements client behavior.
...
// High-level SteelEngine SDK client. Namespaces below translate friendly SDK calls
...
/** Connection and transport options for the SteelEngine SDK client. */
export type SteelEngineOptions = {
  gateway?: "auto" | (string & {});
  url?: string;
  token?: string;
  password?: string;
...
function resolveGatewayUrl(options: SteelEngineOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `SteelEngine Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current SteelEngine Gateway yet`);
}
...
function readChatProjection(event: SteelEngineEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: SteelEngineEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: SteelEngineEvent): boolean {
  return (
...
export class SteelEngine {
  ...
  private readonly transport: SteelEngineTransport;
  private readonly normalizedEvents = new EventHub<SteelEngineEvent>({
    replayLimit: MAX_NORMALIZED_REPLAY_EVENTS,
  });
  private readonly replayByRunId = new Map<string, SteelEngineEvent[]>();
  ...
  constructor(options: SteelEngineOptions = {}) {
    this.transport =
      options.transport ??
  ...
  events(filter?: (event: SteelEngineEvent) => boolean): AsyncIterable<SteelEngineEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: SteelEngineEvent) => boolean,
  ): AsyncIterable<SteelEngineEvent> {
  ...
  private assertOpen(): void {
    if (this.closed) {
      throw new Error("SteelEngine SDK client is closed");
    }
  }
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: SteelEngineEvent): SteelEngineEvent | undefined => {
    ...
    const matches = (event: SteelEngineEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: SteelEngineEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): SteelEngineEvent[] {
...
export class Agent {
  constructor(
    private readonly client: SteelEngine,
    readonly id: string,
  ) {}
...
export class AgentsNamespace {
  constructor(private readonly client: SteelEngine) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<SteelEngineEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: SteelEngine,
...
export class TasksNamespace extends RpcNamespace {
  constructor(client: SteelEngine) {
    super(client, "tasks");
  }
