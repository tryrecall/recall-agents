// SteelEngine SDK helper module supports normalize behavior.
import type { GatewayEvent, JsonObject, SteelEngineEvent, SteelEngineEventType } from "./types.js";
...
function normalizeLifecycleEndEventType(data: JsonObject): SteelEngineEventType {
  const status = readLowerString(data.status);
  const stopReason = readLowerString(data.stopReason);
...
function normalizeAgentEventType(payload: JsonObject): SteelEngineEventType {
...
function normalizeNamedEventType(event: GatewayEvent): SteelEngineEventType {
...
export function normalizeGatewayEvent(event: GatewayEvent): SteelEngineEvent {
  const payload = asRecord(event.payload);
