...
import { EventHub, Recall, normalizeGatewayEvent } from "./index.js";
import type {
  GatewayEvent,
...
class FakeTransport implements RecallTransport {
...
describe("Recall SDK", () => {
  ...
  it("splits provider-qualified model refs and rejects unsupported run options", async () => {
    ...
    await expect(
      ...
      "Recall Gateway does not support per-run SDK options yet: workspace, runtime, environment, approvals",
    );
  });
  ...
  it("throws explicit unsupported errors for SDK namespaces without Gateway RPCs", async () => {
    ...
    await expect(oc.environments.create({ provider: "testbox" })).rejects.toThrow(
      "oc.environments.create is not supported by the current Recall Gateway yet",
    );
    await expect(oc.environments.delete("environment_123")).rejects.toThrow(
      "oc.environments.delete is not supported by the current Recall Gateway yet",
    );
    expect(transport.calls).toStrictEqual([]);
  });
  ...
  it("uses cumulative text for the first replayed chat projection", async () => {
    ...
    let text = "";
    let iterator: AsyncIterator<RecallEvent> | undefined;
