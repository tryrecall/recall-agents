...
import { EventHub, SteelEngine, normalizeGatewayEvent } from "./index.js";
import type {
  GatewayEvent,
...
class FakeTransport implements SteelEngineTransport {
...
class EventsOnlyTransport implements SteelEngineTransport {
...
describe("SteelEngine SDK", () => {
  ...
  it("splits provider-qualified model refs and rejects unsupported run options", async () => {
    ...
    await expect(
      ...
      "SteelEngine Gateway does not support per-run SDK options yet: workspace, runtime, environment, approvals",
    );
  });
  ...
  it("manages environments through current Gateway methods", async () => {
    ...
    await expect(oc.environments.delete("worker_123")).rejects.toThrow(
      "oc.environments.delete is not supported by the current SteelEngine Gateway yet",
    );
    expect(transport.calls).toEqual([
  ...
  it("keeps close terminal when it races a pending connect", async () => {
    ...
    await expect(connect).rejects.toThrow("SteelEngine SDK client is closed");
    await close;
    await expect(oc.agents.list()).rejects.toThrow("SteelEngine SDK client is closed");
    await expect(oc.events()[Symbol.asyncIterator]().next()).rejects.toThrow(
      "SteelEngine SDK client is closed",
    );
    expect(() => oc.rawEvents()).toThrow("SteelEngine SDK client is closed");
  ...
  it("rejects normalized event streams when the event pump fails before yielding", async () => {
    ...
    let futureIterator: AsyncIterator<SteelEngineEvent> | undefined;
  ...
  it("uses cumulative text for the first replayed chat projection", async () => {
    ...
    let text = "";
    let iterator: AsyncIterator<SteelEngineEvent> | undefined;
