...
import { GatewayClientTransport, Recall } from "./index.js";
...
describe("Recall SDK websocket e2e", () => {
  afterEach(async () => {
    await Promise.all(
...
describe("Recall SDK real Gateway e2e", () => {
...
const liveGatewayUrl = process.env.RECALL_SDK_LIVE_GATEWAY_URL;
const liveGatewayToken = process.env.RECALL_SDK_LIVE_GATEWAY_TOKEN;
...
liveGatewayDescribe("Recall SDK live Gateway e2e", () => {
  it("connects to a configured Gateway, streams a real run, and waits for completion", async () => {
    ...
    try {
      ...
      const agent = await oc.agents.get(process.env.RECALL_SDK_LIVE_AGENT_ID ?? "main");
      const run = await agent.run({
        input: "Reply with exactly: RECALL_SDK_LIVE_OK",
      ...
      expect(events.text).toContain("RECALL_SDK_LIVE_OK");
    } finally {
      await oc.close();
    }
  }, 240_000);
