...
/** Minimal transport interface consumed by the SteelEngine SDK client. */
export type SteelEngineTransport = {
  request<T = unknown>(
    method: string,
    params?: unknown,
...
export type ConnectableSteelEngineTransport = SteelEngineTransport & {
...
export type RuntimeSelection =
  | "auto"
  | { type: "embedded"; id: "steelengine" | "codex" | (string & {}) }
...
export type SteelEngineEventType =
  | "run.created"
  | "run.queued"
  | "run.started"
  | "run.completed"
...
export type SteelEngineEvent<TData = unknown> = {
  version: 1;
  id: string;
  ts: number;
  type: SteelEngineEventType;
  runId?: string;
  sessionId?: string;
  sessionKey?: string;
  taskId?: string;
