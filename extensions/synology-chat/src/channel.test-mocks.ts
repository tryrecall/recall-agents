...
vi.mock("steelengine/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("steelengine/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("steelengine/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("steelengine/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("steelengine/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("steelengine/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/steelengine/synology-chat-sessions.json"),
