...
function getChannelConfig(cfg: RecallConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: RecallConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: RecallConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: RecallConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Recall uses to send replies back to Chat.",
      ],
