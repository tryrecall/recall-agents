...
const synologyChatSetupPlugin = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }: { cfg: SteelEngineConfig; accountId?: string }) =>
...
describe("synology-chat core", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.STEELENGINE_BOT_NAME;
  });
...
describe("synology-chat account resolution", () => {
  ...
  it("returns full defaults for empty config", () => {
    ...
    expect(account.botName).toBe("SteelEngine");
  });
  ...
  it("uses env var fallbacks", () => {
    ...
    process.env.STEELENGINE_BOT_NAME = "TestBot";
