...
const synologyChatSetupPlugin = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }: { cfg: RecallConfig; accountId?: string }) =>
...
describe("synology-chat core", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.RECALL_BOT_NAME;
  });
...
describe("synology-chat account resolution", () => {
  ...
  it("returns full defaults for empty config", () => {
    ...
    expect(account.botName).toBe("Recall");
  });
  ...
  it("uses env var fallbacks", () => {
    ...
    process.env.RECALL_BOT_NAME = "TestBot";
