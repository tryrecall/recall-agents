import { clearLiveCatalogCacheForTests } from "recall/plugin-sdk/provider-catalog-shared";
import {
...
const SUPPORTED_DURATIONS_HINT = Symbol.for("recall.videoGeneration.supportedDurations");
...
vi.mock("recall/plugin-sdk/provider-http", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/provider-http")>(
    "recall/plugin-sdk/provider-http",
  );
  return {
    ...actual,
