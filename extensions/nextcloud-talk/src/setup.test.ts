...
describe("resolveNextcloudTalkAccount", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-nextcloud-talk-"));
  ...
  it("does not fall through from a missing explicit bot secret file", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-nextcloud-talk-missing-"));
