...
- Extension production code should import from `steelengine/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `steelengine.plugin.json` and the package
  `steelengine` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `steelengine/plugin-sdk/*`.
