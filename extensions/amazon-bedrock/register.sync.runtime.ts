...
import { resolvePluginConfigObject } from "recall/plugin-sdk/plugin-config-runtime";
import type { RecallPluginApi } from "recall/plugin-sdk/plugin-entry";
import {
  ANTHROPIC_BY_MODEL_REPLAY_HOOKS,
  normalizeProviderId,
} from "recall/plugin-sdk/provider-model-shared";
import { streamWithPayloadPatch } from "recall/plugin-sdk/provider-stream-shared";
 ...
 * IDs that may not contain the model name). When Recall's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if Recall identifies this as an Anthropic model via the ARN heuristic.
 ...
 * the Recall config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: RecallPluginApi): void {
