...
// Nodes without the SteelEngine app (plain `steelengine node host run`) advertise
...
export async function handleLogbookSnapshot(rawParams: unknown): Promise<LogbookSnapshotPayload> {
  ...
  // keeps captures out of the broader SteelEngine temp namespace.
  const captureDir = path.join(resolvePreferredSteelEngineTmpDir(), "logbook");
