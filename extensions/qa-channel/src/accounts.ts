...
import { resolveMergedAccountConfig } from "recall/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "recall";
  const botDisplayName = merged.botDisplayName?.trim() || "Recall QA";
  return {
    accountId,
    enabled,
