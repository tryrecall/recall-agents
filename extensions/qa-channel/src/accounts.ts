...
import { resolveMergedAccountConfig } from "steelengine/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "steelengine";
  const botDisplayName = merged.botDisplayName?.trim() || "SteelEngine QA";
  return {
    accountId,
    enabled,
