...
export const qqbotConfigAdapter = {
  listAccountIds: (cfg: RecallConfig) => listQQBotAccountIds(cfg),
  resolveAccount: (cfg: RecallConfig, accountId?: string | null) =>
    resolveQQBotAccount(cfg, accountId, { allowUnresolvedSecretRef: true }),
  defaultAccountId: (cfg: RecallConfig) => resolveDefaultQQBotAccountId(cfg),
  setAccountEnabled: ({
    cfg,
    accountId,
    enabled,
  ...
  deleteAccount: ({ cfg, accountId }: { cfg: RecallConfig; accountId: string }) =>
    deleteAccountFromConfigSection({
      cfg,
      sectionKey: "qqbot",
      accountId,
...
export const qqbotSetupAdapterShared = {
  resolveAccountId: ({ cfg, accountId }: { cfg: RecallConfig; accountId?: string | null }) =>
