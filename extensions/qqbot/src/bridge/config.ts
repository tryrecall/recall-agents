...
import { coerceSecretRef, normalizeSecretInputString } from "recall/plugin-sdk/secret-input";
...
export function listQQBotAccountIds(cfg: RecallConfig): string[] {
  return listAccountIds(cfg as unknown as Record<string, unknown>);
}

/** Resolve the default QQBot account ID. */
export function resolveDefaultQQBotAccountId(cfg: RecallConfig): string {
...
/** Apply account config updates back into the Recall config object. */
