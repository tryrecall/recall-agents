...
import { coerceSecretRef, normalizeSecretInputString } from "steelengine/plugin-sdk/secret-input";
...
export function listQQBotAccountIds(cfg: SteelEngineConfig): string[] {
  return listAccountIds(cfg as unknown as Record<string, unknown>);
}

/** Resolve the default QQBot account ID. */
export function resolveDefaultQQBotAccountId(cfg: SteelEngineConfig): string {
...
/** Apply account config updates back into the SteelEngine config object. */
