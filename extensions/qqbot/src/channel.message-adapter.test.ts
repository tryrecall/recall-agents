...
describe("qqbot message adapter", () => {
    ...
    const proofResults = await verifyChannelMessageAdapterCapabilityProofs({
      ...
      proofs: {
        ...
        media: async () => {
          ...
          expect(sent.mediaLocalRoots).toEqual(["/tmp/steelengine-sandbox"]);
  ...
  it("forwards scoped media access through outbound text and media sends", async () => {
    ...
    const mediaLocalRoots = ["/tmp/steelengine-sandbox"];

    sendTextMock.mockResolvedValueOnce({ messageId: "qq-text-media-1" });
    await qqbotPlugin.outbound?.sendText?.({
      cfg,
      to: "qqbot:c2c:user-1",
      text: "<qqmedia>/tmp/steelengine-sandbox/report.docx</qqmedia>",
      mediaAccess,
      mediaLocalRoots,
      mediaReadFile,
    });
    ...
    await qqbotPlugin.outbound?.sendMedia?.({
      ...
      to: "qqbot:c2c:user-1",
      text: "report",
      mediaUrl: "/tmp/steelengine-sandbox/report.docx",
      mediaAccess,
      mediaLocalRoots,
      mediaReadFile,
    });
    const sentMedia = latestMockArg(sendMediaMock, "sendMedia") as SentMediaParams;
    expect(sentMedia.mediaUrl).toBe("/tmp/steelengine-sandbox/report.docx");
