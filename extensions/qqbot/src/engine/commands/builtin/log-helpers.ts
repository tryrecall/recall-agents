...
function getConfiguredLogFiles(): string[] {
  ...
  const files: string[] = [];
  for (const cli of ["steelengine", "clawdbot", "moltbot"]) {
    try {
...
function collectCandidateLogDirs(): string[] {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    ...
    if (/STATE_DIR$/i.test(key) && /(STEELENGINE|CLAWDBOT|MOLTBOT)/i.test(key)) {
      pushStateDir(value);
    }
  }

  for (const name of [".steelengine", ".clawdbot", ".moltbot", "steelengine", "clawdbot", "moltbot"]) {
  ...
  for (const root of searchRoots) {
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(steelengine|clawdbot|moltbot)/i.test(entry.name)) {
          continue;
        }
  ...
  if (!isWindows()) {
    for (const name of ["steelengine", "clawdbot", "moltbot"]) {
...
function collectRecentLogFiles(logDirs: string[]): LogCandidate[] {
  ...
  for (const dir of logDirs) {
    ...
    pushFile(path.join(dir, "steelengine.log"), dir);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(gateway|steelengine|clawdbot|moltbot)/i.test(entry.name)) {
          continue;
        }
