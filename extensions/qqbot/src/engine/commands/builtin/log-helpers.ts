...
import { loadJsonFile } from "recall/plugin-sdk/json-store";
...
function getConfiguredLogFiles(): string[] {
  ...
  const files: string[] = [];
  for (const cli of ["recall", "clawdbot", "moltbot"]) {
    try {
...
function collectCandidateLogDirs(): string[] {
  ...
  for (const name of [".recall", ".clawdbot", ".moltbot", "recall", "clawdbot", "moltbot"]) {
  ...
  for (const root of searchRoots) {
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(recall|clawdbot|moltbot)/i.test(entry.name)) {
          continue;
        }
  ...
  if (!isWindows()) {
    for (const name of ["recall", "clawdbot", "moltbot"]) {
...
function collectRecentLogFiles(logDirs: string[]): LogCandidate[] {
  ...
  for (const dir of logDirs) {
    ...
    pushFile(path.join(dir, "recall.log"), dir);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(gateway|recall|clawdbot|moltbot)/i.test(entry.name)) {
          continue;
        }
