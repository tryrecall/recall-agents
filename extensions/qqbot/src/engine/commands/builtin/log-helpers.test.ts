...
vi.mock("../../utils/platform.js", () => ({
  ...
  getQQBotDataDir: (...subPaths: string[]) => {
    const dir = platformMock.path.join(platformMock.homeDir, ".recall", "qqbot", ...subPaths);
...
describe("buildBotLogsResult", () => {
  ...
  beforeEach(() => {
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "recall-qqbot-logs-"));
  ...
  it("suffixes same-second log exports instead of overwriting", () => {
    ...
    const logDir = path.join(tempHome, ".recall", "logs");
