...
import { loadOutboundMediaFromUrl } from "steelengine/plugin-sdk/outbound-media";
import { resolveLocalPathFromRootsSync } from "steelengine/plugin-sdk/security-runtime";
import {
...
vi.mock("steelengine/plugin-sdk/security-runtime", { spy: true });
...
function makeCtx() {
  return {
    ...
    mediaAccess: {
      localRoots: ["/tmp/steelengine-sandbox"],
      workspaceDir: "/tmp/workspace",
      readFile: async () => Buffer.from("report"),
    },
    mediaLocalRoots: ["/tmp/steelengine-sandbox"],
...
beforeEach(async () => {
  ...
  steelengineHome = await fs.realpath(
...
describe("resolveOutboundMediaPath", () => {
  ...
  it("does not treat workspaceDir as an allowed host absolute root", () => {
    ...
    ).toEqual(["/tmp/steelengine-sandbox"]);
  });

  it("maps only authorized virtual workspace roots for host-read loading", () => {
    expect(
      resolveWorkspaceScopedLocalRoots(
        ["/workspace/attachments", "/tmp/steelengine-sandbox", "/workspace/../media"],
        "/tmp/agent-workspace",
      ),
    ).toEqual(["/tmp/agent-workspace/attachments", "/tmp/steelengine-sandbox"]);
  });
...
describe("trySendViaHostRead error handling", () => {
  it("returns OutboundResult.error when loadOutboundMediaFromUrl rejects", async () => {
    ...
    const result = await sendPhoto(makeCtx(), "/tmp/steelengine-sandbox/report.docx");
  ...
  it("falls back to normal local sends for trusted media paths outside host-read roots", async () => {
    const trustedMediaDir = path.join(steelengineHome, ".steelengine", "media", "qqbot");
  ...
  it("returns OutboundResult.error when senderSendMedia rejects", async () => {
    ...
    const result = await sendPhoto(makeCtx(), "/tmp/steelengine-sandbox/chart.png");
  ...
  it("loads virtual-root workspace media through the real outbound loader", async () => {
    const actualOutboundMedia = await vi.importActual<
      typeof import("steelengine/plugin-sdk/outbound-media")
    >("steelengine/plugin-sdk/outbound-media");
    mockedLoadOutboundMediaFromUrl.mockImplementation(actualOutboundMedia.loadOutboundMediaFromUrl);
    mockedSenderSendMedia.mockResolvedValue({ id: "media-1", timestamp: 123 });
    const workspaceDir = path.join(steelengineHome, "agent-workspace");
