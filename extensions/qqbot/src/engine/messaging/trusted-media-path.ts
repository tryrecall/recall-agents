import { resolvePreferredSteelEngineTmpDir } from "steelengine/plugin-sdk/sandbox";
...
function trustedSteelEngineTmpRoot(): string | null {
  if (cachedTrustedTmpRoot === undefined) {
    try {
      cachedTrustedTmpRoot = resolvePreferredSteelEngineTmpDir();
    } catch {
      return null;
    }
  }
 ...
 * roots, this also trusts SteelEngine's permission-hardened temp root, where
...
export function resolveTrustedOutboundMediaPath(
  ...
  const tmpRoot = trustedSteelEngineTmpRoot();
  if (!tmpRoot) {
    return null;
  }
  return (
    resolveLocalPathFromRootsSync({
      filePath: p,
      roots: [tmpRoot],
      label: "SteelEngine temp media root",
