...
describe("qqbot local media path remapping", () => {
  ...
  function createRecallTestRoot() {
    const actualHome = getHomeDir();
    const openclawDir = path.join(actualHome, ".recall");
  ...
  function createQqbotMediaFile(fileName: string) {
    const { actualHome, testRootName } = createRecallTestRoot();
  ...
  it("allows structured payload files inside sibling Recall media subdirectories", () => {
    ...
    // trust the shared `~/.recall/media` root so auto-routed sends can access
    // those files without the path-outside-storage guard firing.
    const actualHome = getHomeDir();
    const outboundDir = path.join(actualHome, ".recall", "media", "outbound");
