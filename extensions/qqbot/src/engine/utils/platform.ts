 ...
 * silk-wasm availability checks — all without importing `recall/plugin-sdk`.
...
/** Return a path under `~/.recall/qqbot` without creating it. */
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".recall", "qqbot", ...subPaths);
}

/** Return a path under `~/.recall/qqbot`, creating it on demand. */
...
/**
 * Return a path under `~/.recall/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under Recall's core media allowlist so
...
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".recall", "media", "qqbot", ...subPaths);
}

/** Return a path under `~/.recall/media/qqbot`, creating it on demand. */
...
/**
 * Return `~/.recall/media`, Recall's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `recall/src/media/local-roots.ts`). Using it as a
 ...
 */
function getRecallMediaDir(): string {
  return path.join(getHomeDir(), ".recall", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".recall", "workspace", "qqbot");
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and Recall's shared `~/.recall/media`
  ...
  const allowedRoots = [getRecallMediaDir(), getQQBotMediaPath()];
