 ...
 * silk-wasm availability checks — all without importing `steelengine/plugin-sdk`.
 ...
 * `STEELENGINE_HOME`. QQ Bot still checks this tree for legacy state imports and
...
/**
 * Resolve the effective SteelEngine home directory.
 *
 * Mirrors the contract from core (`src/infra/home-dir.ts::resolveEffectiveHomeDir`)
 * so QQ Bot media roots live under the same tree the rest of SteelEngine treats as
 * `~`. The extension cannot import the core helper directly (it is a separate
 * package with `steelengine` as a peer dependency), so this re-implements the
 * minimal contract:
 *
 * 1. `STEELENGINE_HOME` when set (with `~` / `~/...` expanded against the OS home).
 ...
 */
function resolveSteelEngineHome(): string {
  const raw = process.env.STEELENGINE_HOME?.trim();
...
/**
 * Return a legacy path under `~/.steelengine/qqbot` without creating it.
 *
...
function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".steelengine", "qqbot", ...subPaths);
}

/** Return a path under `~/.steelengine/qqbot`, creating it on demand. */
...
/**
 * Return a path under `<steelengine-home>/.steelengine/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under SteelEngine's core media allowlist
 * so downloaded images and audio can be accessed by framework media tooling.
 * The base honors `STEELENGINE_HOME` (when set) so files written by agents into
 * the SteelEngine-managed media tree are reachable by this plugin even when
 * `HOME` and `STEELENGINE_HOME` differ (Docker, multi-user hosts). Fixes #83562.
 */
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(resolveSteelEngineHome(), ".steelengine", "media", "qqbot", ...subPaths);
}

/** Return a path under `<steelengine-home>/.steelengine/media/qqbot`, creating it on demand. */
...
/**
 * Return `<steelengine-home>/.steelengine/media`, SteelEngine's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `steelengine/src/media/local-roots.ts`). Using it as a
 ...
 * {@link getQQBotMediaPath}, the base honors `STEELENGINE_HOME`.
 */
function getSteelEngineMediaDir(): string {
  return path.join(resolveSteelEngineHome(), ".steelengine", "media");
}
...
function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const steelengineHomeDir = resolveSteelEngineHome();
  const mediaRoot = getQQBotMediaPath();
  const dataRoot = getQQBotDataPath();
  // When STEELENGINE_HOME differs from HOME we have to consider workspace roots
  // under both trees: agents may be configured with `~`-relative paths (HOME)
  // or with the SteelEngine-managed home tree. Deduplicate when they match.
  const workspaceRoots = Array.from(
    new Set([
      path.join(osHomeDir, ".steelengine", "workspace", "qqbot"),
      path.join(steelengineHomeDir, ".steelengine", "workspace", "qqbot"),
    ]),
  );
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and SteelEngine's shared `~/.steelengine/media`
  ...
  const allowedRoots = [getSteelEngineMediaDir(), getQQBotMediaPath()];
