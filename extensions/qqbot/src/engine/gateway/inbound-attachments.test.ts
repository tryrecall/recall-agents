...
vi.mock("../utils/platform.js", () => ({
  getQQBotMediaDir: () => "/tmp/steelengine-qqbot-downloads",
}));
...
describe("engine/gateway/inbound-attachments", () => {
  ...
  it("uses remote image URL when image download fails", async () => {
    ...
    expect(downloadFileMock).toHaveBeenCalledWith(
      ...
      "/tmp/steelengine-qqbot-downloads",
      "a.png",
    );
  ...
  it("classifies image content types case-insensitively when download succeeds", async () => {
    downloadFileMock.mockResolvedValueOnce("/tmp/steelengine-qqbot-downloads/a.png");
    downloadFileMock.mockResolvedValueOnce("/tmp/steelengine-qqbot-downloads/b.png");
    ...
    expect(result.imageUrls).toEqual([
      "/tmp/steelengine-qqbot-downloads/a.png",
      "/tmp/steelengine-qqbot-downloads/b.png",
    ]);
  ...
  it("does not classify a mixed-case non-image content type as an image", async () => {
    downloadFileMock.mockResolvedValue("/tmp/steelengine-qqbot-downloads/doc.pdf");
    ...
    expect(result.imageUrls).toEqual([]);
    expect(result.attachmentInfo).toBe("\n[Attachment: /tmp/steelengine-qqbot-downloads/doc.pdf]");
  });

  it("prefers voice_wav_url for voice downloads and transcribes with configured STT", async () => {
    downloadFileMock.mockResolvedValue("/tmp/steelengine-qqbot-downloads/voice.wav");
    ...
    expect(transcribeAudioMock).toHaveBeenCalledWith("/tmp/steelengine-qqbot-downloads/voice.wav", {
      channels: { qqbot: { stt: {} } },
    });
    expect(result.voiceAttachmentPaths).toEqual(["/tmp/steelengine-qqbot-downloads/voice.wav"]);
