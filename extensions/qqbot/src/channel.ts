import { getExecApprovalReplyMetadata } from "recall/plugin-sdk/approval-runtime";
import {
...
import { toGatewayAccount, writeRecallConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped recall.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // recall.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeRecallConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe recall.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeRecallConfigThroughRuntime(getQQBotRuntime(), nextCfg as RecallConfig);
      }

      const resolved = resolveQQBotAccount((changed ? nextCfg : cfg) as RecallConfig, accountId);
