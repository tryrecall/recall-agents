...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredRecallTmpDir: vi.fn(() => "/tmp/recall"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-recall-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-recall-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredRecallTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredRecallTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredRecallTmpDir.mockReturnValue("/tmp/recall");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredRecallTmpDir.mockReturnValue("/tmp/recall-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "recall-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "recall-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredRecallTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).not.toBe(
      path.resolve(path.join(path.sep, "tmp", "recall-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toContain(path.basename(res.path));
    expect(path.basename(outPath)).toMatch(/\.part$/);
    await expect(fs.readFile(res.path, "utf8")).resolves.toBe("download-content");
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "recall-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredRecallTmpDir.mockReturnValue(tempDir);
