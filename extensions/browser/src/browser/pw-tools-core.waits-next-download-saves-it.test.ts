...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredSteelEngineTmpDir: vi.fn(() => "/tmp/steelengine"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-steelengine-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-steelengine-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredSteelEngineTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredSteelEngineTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredSteelEngineTmpDir.mockReturnValue("/tmp/steelengine");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredSteelEngineTmpDir.mockReturnValue("/tmp/steelengine-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "steelengine-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "steelengine-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredSteelEngineTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).not.toBe(
      path.resolve(path.join(path.sep, "tmp", "steelengine-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toContain(path.basename(res.path));
    expect(path.basename(outPath)).toMatch(/\.part$/);
    await expect(fs.readFile(res.path, "utf8")).resolves.toBe("download-content");
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "steelengine-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredSteelEngineTmpDir.mockReturnValue(tempDir);
