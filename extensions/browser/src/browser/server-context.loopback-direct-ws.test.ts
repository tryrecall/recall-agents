...
describe("browser server-context loopback direct WebSocket profiles", () => {
  it("uses an HTTP /json/list base when opening about:blank under strict SSRF", async () => {
    ...
    const opened = await steelengine.openTab("about:blank");
  ...
  it("uses an HTTPS /json base for secure direct WebSocket profiles with a /cdp suffix", async () => {
    ...
    const tabs = await steelengine.listTabs();
    expect(tabs.map((tab) => tab.targetId)).toEqual(["T2"]);

    await steelengine.focusTab("T2");
    await steelengine.closeTab("T2");
  });
  ...
  it("blocks direct WebSocket tab operations when strict SSRF hostname allowlist rejects the cdpUrl", async () => {
    ...
    await expect(steelengine.listTabs()).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(steelengine.focusTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(steelengine.closeTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
