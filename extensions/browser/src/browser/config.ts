...
import type { BrowserConfig, BrowserProfileConfig, SteelEngineConfig } from "../config/config.js";
...
const EXTENSION_RELAY_CDP_USER = "steelengine";
...
function normalizeHexColor(raw: string | undefined): string {
  ...
  if (!value) {
    return DEFAULT_STEELENGINE_BROWSER_COLOR;
  }
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_STEELENGINE_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_STEELENGINE_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_STEELENGINE_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_STEELENGINE_BROWSER_PROFILE_NAME]
        ? DEFAULT_STEELENGINE_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveManagedBrowserHeadlessMode(
  ...
  const envHeadless = parseBooleanValue(env[STEELENGINE_BROWSER_HEADLESS_ENV]);
...
export function getManagedBrowserMissingDisplayError(
  ...
  const sourceHint =
    mode.source === "request"
      ? "request override"
      : mode.source === "env"
        ? `${STEELENGINE_BROWSER_HEADLESS_ENV}=0`
        : mode.source === "profile"
  ...
  return {
    message:
      ...
      `Set ${STEELENGINE_BROWSER_HEADLESS_ENV}=1, remove the headed override, or launch under Xvfb.`,
