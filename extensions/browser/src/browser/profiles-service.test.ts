...
import { resolveRecallUserDataDir } from "./chrome.js";
import type { BrowserRouteContext, BrowserServerState } from "./server-context.js";
import { movePathToTrash } from "./trash.js";

const configMocks = vi.hoisted(() => ({
  getRuntimeConfig: vi.fn<() => RecallConfig>(),
  writeConfigFile: vi.fn<(cfg: RecallConfig) => Promise<void>>(async (_cfg) => {}),
  mutateConfigFile: vi.fn(
    async (params: {
      mutate: (draft: RecallConfig, context: { snapshot: { path: string } }) => unknown;
    }) => {
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    replaceConfigFile: vi.fn(async ({ nextConfig }: { nextConfig: RecallConfig }) => {
...
vi.mock("./chrome.js", () => ({
  resolveRecallUserDataDir: vi.fn(() => "/tmp/recall-test/recall/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "recall-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveRecallUserDataDir).mockReturnValue(userDataDir);
