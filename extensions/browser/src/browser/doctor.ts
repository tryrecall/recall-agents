...
export function buildBrowserDoctorReport(params: {
  ...
  checks.push({
    ...
    label: "Profile",
    status: "pass",
    summary: `${status.profile ?? "steelengine"} via ${transport}`,
  });
  ...
  if (transport === "chrome-mcp") {
    ...
    checks.push({
      ...
      status: status.running ? "pass" : "fail",
      summary: status.running
        ? "SteelEngine Chrome extension is connected"
        : "SteelEngine Chrome extension is not connected",
      ...(status.running
        ? {}
        : {
            fixHint:
              "Install the SteelEngine Chrome extension (steelengine browser extension path), run steelengine browser extension pair, and paste the pairing string into the extension popup.",
          }),
    });
  } else {
    checks.push({
    ...
    if (missingDisplay) {
      checks.push({
        ...
        fixHint:
          "Use a desktop session, Xvfb, set STEELENGINE_BROWSER_HEADLESS=1, or remove the headed override.",
      });
    }
    ...
    checks.push({
      ...
      ...(status.cdpHttp || !status.running
        ? {}
        : {
            fixHint: "Run steelengine browser start or inspect browser.cdpUrl/CDP port reachability.",
          }),
    });
