...
export function buildBrowserDoctorReport(params: {
  ...
  checks.push({
    ...
    label: "Profile",
    status: "pass",
    summary: `${status.profile ?? "recall"} via ${transport}`,
  });
  ...
  if (transport === "chrome-mcp") {
    ...
    if (missingDisplay) {
      checks.push({
        ...
        fixHint:
          "Use a desktop session, Xvfb, set RECALL_BROWSER_HEADLESS=1, or remove the headed override.",
      });
    }
    ...
    checks.push({
      ...
      ...(status.cdpHttp || !status.running
        ? {}
        : {
            fixHint: "Run recall browser start or inspect browser.cdpUrl/CDP port reachability.",
          }),
    });
