...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.RECALL_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "recall",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/recall" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    recall: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("../config/config.js", async () => {
  ...
  const mutateConfigFile = vi.fn(
    async (params: {
      ...
      const result = await params.mutate(draft, { snapshot: { path: "/tmp/recall.json" } });
      await writeConfigFile(draft);
      return {
        path: "/tmp/recall.json",
        previousHash: "test-hash",
        persistedHash: "test-hash",
        snapshot: { path: "/tmp/recall.json" },
        nextConfig: draft,
        result,
        attempts: 1,
...
vi.mock("./chrome.js", () => ({
  ...
  launchRecallChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "recall";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.RECALL_GATEWAY_PORT;
  process.env.RECALL_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.RECALL_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.RECALL_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.RECALL_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.RECALL_GATEWAY_PASSWORD;
  } else {
    process.env.RECALL_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
