...
function setupEnsureBrowserAvailableHarness() {
  ...
  const launchRecallChrome = vi.mocked(chromeModule.launchRecallChrome);
  const stopRecallChrome = vi.mocked(chromeModule.stopRecallChrome);
  ...
  const profile = ctx.forProfile("recall");

  return { launchRecallChrome, stopRecallChrome, isChromeCdpReady, profile, state };
}
...
function requireFirstLaunchOptions(launchRecallChrome: {
  mock: { calls: unknown[][] };
}): unknown {
  const [call] = launchRecallChrome.mock.calls;
  if (!call) {
...
describe("browser server-context ensureBrowserAvailable", () => {
  it("waits for CDP readiness after launching to avoid follow-up PortInUseError races (#21149)", async () => {
    const { launchRecallChrome, stopRecallChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValueOnce(false).mockResolvedValue(true);
    mockLaunchedChrome(launchRecallChrome, 123);

    const promise = profile.ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(promise).resolves.toBeUndefined();

    expect(launchRecallChrome).toHaveBeenCalledTimes(1);
    expect(isChromeCdpReady).toHaveBeenCalled();
    expect(stopRecallChrome).not.toHaveBeenCalled();
  });
  ...
  it("stops launched chrome when CDP readiness never arrives", async () => {
    ...
    mockLaunchedChrome(launchRecallChrome, 321);
    ...
    expect(launchRecallChrome).toHaveBeenCalledTimes(1);
    expect(stopRecallChrome).toHaveBeenCalledTimes(1);
  });

  it("uses configured local CDP readiness timeout after launching", async () => {
    const { launchRecallChrome, stopRecallChrome, isChromeCdpReady, profile, state } =
      setupEnsureBrowserAvailableHarness();
    state.resolved.localCdpReadyTimeoutMs = 250;
    isChromeCdpReady.mockResolvedValue(false);
    mockLaunchedChrome(launchRecallChrome, 322);
  ...
  it("deduplicates concurrent lazy-start calls to prevent PortInUseError", async () => {
    ...
    mockLaunchedChrome(launchRecallChrome, 456);
  ...
  it("deduplicates concurrent lazy-start calls across fresh profile contexts", async () => {
    const { launchRecallChrome, stopRecallChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    mockLaunchedChrome(launchRecallChrome, 457);

    const firstCtx = createBrowserRouteContext({ getState: () => state });
    const secondCtx = createBrowserRouteContext({ getState: () => state });
    const first = firstCtx.forProfile("recall").ensureBrowserAvailable();
    const second = secondCtx.forProfile("recall").ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(Promise.all([first, second])).resolves.toEqual([undefined, undefined]);
  ...
  it("passes request-local headless override to initial launch", async () => {
    const { launchRecallChrome, stopRecallChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    mockLaunchedChrome(launchRecallChrome, 654);
    ...
    expect(launchRecallChrome).toHaveBeenCalledTimes(1);
    expect(requireFirstLaunchOptions(launchRecallChrome)).toEqual({ headlessOverride: true });
    expect(stopRecallChrome).not.toHaveBeenCalled();
  });
  ...
  it("passes request-local headless override to the owned restart path", async () => {
    ...
    mockLaunchedChrome(launchRecallChrome, 987);
  ...
  it("does not share inflight lazy-start promises across different headless overrides", async () => {
    const { launchRecallChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
  ...
  it("clears the concurrent lazy-start guard after launch failure", async () => {
    const { launchRecallChrome, stopRecallChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchRecallChrome.mockRejectedValueOnce(
    ...
    mockLaunchedChrome(launchRecallChrome, 789);
    const retry = profile.ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(retry).resolves.toBeUndefined();

    expect(launchRecallChrome).toHaveBeenCalledTimes(2);
    expect(stopRecallChrome).not.toHaveBeenCalled();
  });
  ...
  it("cools down repeated managed Chrome launch failures across route contexts", async () => {
    const { launchRecallChrome, stopRecallChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchRecallChrome.mockRejectedValue(new Error("Failed to start Chrome CDP"));
    ...
    await expect(cooledDownCtx.forProfile("recall").ensureBrowserAvailable()).rejects.toThrow(
      'Browser launch for profile "recall" is cooling down after 3 consecutive managed Chrome launch failures.',
    );
    await expect(cooledDownCtx.forProfile("recall").ensureBrowserAvailable()).rejects.toThrow(
      "set browser.enabled=false if the browser tool is not needed",
    );

    expect(launchRecallChrome).toHaveBeenCalledTimes(3);
    expect(stopRecallChrome).not.toHaveBeenCalled();
  });

  it("allows one managed Chrome launch attempt after the cooldown expires", async () => {
    const { launchRecallChrome, isChromeCdpReady, state } = setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchRecallChrome.mockRejectedValue(new Error("Failed to start Chrome CDP"));
    ...
    await expect(retryCtx.forProfile("recall").ensureBrowserAvailable()).rejects.toThrow(
      "Failed to start Chrome CDP",
    );

    expect(launchRecallChrome).toHaveBeenCalledTimes(4);
  });
  ...
  it("reuses a pre-existing loopback browser after an initial short probe miss", async () => {
    ...
    expect(launchRecallChrome).not.toHaveBeenCalled();
    expect(stopRecallChrome).not.toHaveBeenCalled();
  });
  ...
  it("explains attachOnly for externally managed loopback CDP services", async () => {
    ...
    await expect(promise).rejects.toThrow(
      'Port 18800 is in use for profile "recall" but not by recall.',
    );
    await expect(promise).rejects.toThrow(
      "set browser.profiles.recall.attachOnly=true so Recall attaches without trying to manage the local process",
    );
    await expect(promise).rejects.toThrow(
      "For Browserless Docker, set EXTERNAL to the same WebSocket endpoint Recall can reach via browser.profiles.<name>.cdpUrl.",
    );
  ...
  it("retries remote CDP websocket reachability once before failing", async () => {
    const { launchRecallChrome, stopRecallChrome, isChromeCdpReady } =
      setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
  ...
  it("treats attachOnly loopback CDP as local control with remote-class probe timeouts", async () => {
    const { launchRecallChrome, stopRecallChrome } = setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
    const isChromeCdpReady = vi.mocked(chromeModule.isChromeCdpReady);
