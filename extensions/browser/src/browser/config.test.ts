...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("recall");
    expect(profile?.driver).toBe("recall");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const recall = resolveProfile(resolved, "recall");
    expect(recall?.driver).toBe("recall");
    expect(recall?.cdpPort).toBe(18800);
    expect(recall?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from RECALL_GATEWAY_PORT when unset", () => {
    withEnv({ RECALL_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const recall = resolveProfile(resolved, "recall");
      expect(recall?.cdpPort).toBe(19012);
      expect(recall?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ RECALL_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const recall = resolveProfile(resolved, "recall");
      expect(recall?.cdpPort).toBe(19022);
      expect(recall?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(recall?.cdpPort).toBe(19000);
    expect(recall?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "recall");
  ...
  describe("managed browser headless mode", () => {
    const noDisplayEnv = {
      ...
      [RECALL_BROWSER_HEADLESS_ENV]: undefined,
    };
    ...
    it("lets explicit profile headless=false beat the Linux no-display fallback", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        profiles: {
          recall: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
      });
      const profile = resolveProfile(resolved, "recall")!;
    ...
    it("lets RECALL_BROWSER_HEADLESS override profile/global config", () => {
      const resolved = resolveBrowserConfig({
        profiles: {
          recall: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
      ...
      expect(
        resolveManagedBrowserHeadlessMode(resolved, profile, {
          platform: "linux",
          env: { ...noDisplayEnv, [RECALL_BROWSER_HEADLESS_ENV]: "1" },
        }),
    ...
    it("lets request-local headless override beat env and profile/global config", () => {
      ...
      expect(
        resolveManagedBrowserHeadlessMode(resolved, profile, {
          ...
          platform: "linux",
          env: { ...noDisplayEnv, [RECALL_BROWSER_HEADLESS_ENV]: "0" },
        }),
    ...
    it("returns an actionable error only when headed mode is explicitly selected", () => {
      ...
      const defaultProfile = resolveProfile(defaultResolved, "recall")!;
      expect(
      ...
      const profile = resolveProfile(profileResolved, "recall")!;
      expect(
        getManagedBrowserMissingDisplayError(profileResolved, profile, {
          platform: "linux",
          env: noDisplayEnv,
        }),
      ).toContain("browser.profiles.recall.headless=false");
    });
  });
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "recall")!;
  ...
  describe("default profile preference", () => {
    it("defaults to recall profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("recall");
    });

    it("keeps recall default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("recall");
    ...
    it("keeps recall default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("recall");
    ...
    it("keeps recall default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
