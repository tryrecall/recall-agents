...
function formatLocalPortOwnershipHint(profile: ResolvedBrowserProfile): string {
  const resetHint =
    `If Recall should own this local profile, run action=reset-profile profile=${profile.name} ` +
  ...
  return (
    ...
    `set browser.profiles.${profile.name}.attachOnly=true so Recall attaches without trying ` +
    "to manage the local process. For Browserless Docker, set EXTERNAL to the same WebSocket " +
    "endpoint Recall can reach via browser.profiles.<name>.cdpUrl."
  );
}
...
export function createProfileAvailability({
  ...
  const formatChromeMcpAttachFailure = (lastError: unknown): string => {
    ...
    if (message.includes("DevToolsActivePort") || message.includes("Could not connect to Chrome")) {
      return (
        ...
        'If you do not need your signed-in browser session, use the managed "recall" profile instead.' +
        detail
      );
    }
    return (
  ...
  const reconcileProfileRuntime = async (): Promise<void> => {
    ...
    if (profileState.running) {
      await stopRecallChrome(profileState.running).catch(() => {});
      setProfileRunning(null);
    }
  ...
  const waitForCdpReadyAfterLaunch = async (): Promise<void> => {
    // launchRecallChrome() can return before Chrome is fully ready to serve /json/version + CDP WS.
  ...
  const launchManagedChrome = async (
    ...
    try {
      return await launchRecallChrome(current.resolved, profile, launchOptions);
    } catch (err) {
  ...
  const ensureBrowserAvailableOnce = async (options?: BrowserEnsureOptions): Promise<void> => {
    ...
    if (!httpReachable) {
      ...
      // Browser control service can restart while a loopback Recall browser is still
      ...
      try {
        ...
        await stopRecallChrome(launched).catch(() => {});
        setProfileRunning(null);
        recordManagedLaunchFailure(profileState, err);
        throw err;
      }
    ...
    if (!profileState.running) {
      const detail = await describeCdpFailure(PROFILE_ATTACH_RETRY_TIMEOUT_MS);
      throw new BrowserProfileUnavailableError(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by recall. ` +
          `${formatLocalPortOwnershipHint(profile)} ${detail}`,
      );
    }

    await stopRecallChrome(profileState.running);
    setProfileRunning(null);
