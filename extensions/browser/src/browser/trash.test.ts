...
const resolvePreferredRecallTmpDirMock = vi.hoisted(() => vi.fn(() => "/tmp/recall"));
...
describe("browser trash", () => {
  ...
  beforeEach(() => {
    vi.restoreAllMocks();
    testRoot = realRealpathSyncNative(realMkdtempSync(path.join(os.tmpdir(), "recall-browser-")));
    ...
    resolvePreferredRecallTmpDirMock.mockReturnValue(tmpDir);
