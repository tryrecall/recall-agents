...
vi.mock("../infra/tmp-recall-dir.js", () => ({
  resolvePreferredRecallTmpDir: () => "/tmp/recall-browser-test",
}));
...
import {
  buildRecallChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchRecallChrome,
  resolveRecallUserDataDir,
  stopRecallChrome,
} from "./chrome.js";
 ...
 * not exercise: launchRecallChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopRecallChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildRecallChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveRecallUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveRecallUserDataDir();
      expect(dir.endsWith(path.join("recall", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveRecallUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildRecallChromeLaunchArgs branches", () => {
    ...
    it("toggles headless args", () => {
      const args = buildRecallChromeLaunchArgs({
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchRecallChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchRecallChrome(resolved, profile);
  ...
  describe("launchRecallChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "recall-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchRecallChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchRecallChrome(makeResolved(), profile);
          expect(running.pid).toBe(4242);
    ...
    it("accepts a ready CDP diagnostic after the launch HTTP probe expires", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          const port = new URL(baseUrl).port;
          const profile = makeProfile(Number(port));
          const running = await launchRecallChrome(
    ...
    it("clears stale singleton locks and retries once after profile-in-use launch failure", async () => {
      const configPath = path.join(tmpDir, "recall.json");
      await fsp.writeFile(
        configPath,
        JSON.stringify({
          logging: {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const userDataDir = resolveRecallUserDataDir(profile.name);
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchRecallChrome(resolved, profile)).rejects.toThrow(
  ...
  describe("stopRecallChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopRecallChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopRecallChrome>[0],
        1,
      );
  ...
  describe("launchRecallChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "recall-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveRecallUserDataDir(profileName));
        ...
        const staged = resolveRecallUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_RECALL_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      const configDir = await fsp.mkdtemp(path.join(os.tmpdir(), "recall-redact-off-"));
      const configPath = path.join(configDir, "recall.json");
      ...
      let message = "";
      try {
        await launchRecallChrome(resolved, profile);
      } catch (err) {
    ...
    it("logs a warning when decorateRecallProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateRecallProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateRecallProfile }, "decorateRecallProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".recall-profile-decorated") || s.endsWith("Preferences")) {
