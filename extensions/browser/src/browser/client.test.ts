...
describe("browser client", () => {
  ...
  it("uses the expected endpoints + methods for common calls", async () => {
    ...
    expect(doctorResult.profile).toBe("recall");
    ...
    expect(deepDoctorResult.profile).toBe("recall");
    ...
    expect(urls.some((url) => url.endsWith("/doctor?profile=recall&deep=true"))).toBe(true);
    ...
    const deepDoctor = calls.find((c) => c.url.endsWith("/doctor?profile=recall&deep=true"));
