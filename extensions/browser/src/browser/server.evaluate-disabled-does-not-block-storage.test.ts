...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        steelengine: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.STEELENGINE_GATEWAY_PORT;
    process.env.STEELENGINE_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.STEELENGINE_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.STEELENGINE_GATEWAY_PASSWORD;
