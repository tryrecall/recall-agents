...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-recall-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-recall-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-recall-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function resolveBrowserFetchOperatorHint(
  ...
  if (opts?.ownership === "external-browser") {
    return (
      "The browser profile is external to Recall; make sure its browser/CDP endpoint " +
      "is running and reachable. Restarting the Recall gateway will not launch it."
    );
  }
  const isLocal = !isAbsoluteHttp(url);
  return isLocal
    ? `Restart the Recall gateway (Recall.app menubar, or \`${formatCliCommand("recall gateway")}\`).`
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  if (kind === "timeout") {
    return new Error(
      `Can't reach the Recall browser control service (timed out after ${timeoutMs}ms). ${operatorHint}`,
  ...
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the Recall browser control service. ${operatorHint} (${msg})`,
    ),
    err instanceof Error ? { cause: err } : undefined,
  );
}
