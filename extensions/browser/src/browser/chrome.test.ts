...
type StopChromeTarget = Parameters<typeof stopRecallChrome>[0];
...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "recall-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateRecallProfile(userDataDir, { color: DEFAULT_RECALL_BROWSER_COLOR });

    const expectedSignedArgb = ((0xff << 24) | 0xff4500) >> 0;

    const def = await readDefaultProfileFromLocalState(userDataDir);

    expect(def.name).toBe(DEFAULT_RECALL_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_RECALL_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".recall-profile-decorated"),
      "utf-8",
    );
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateRecallProfile(userDataDir, { color: "lobster-orange" });
    const def = await readDefaultProfileFromLocalState(userDataDir);
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_RECALL_BROWSER_PROFILE_NAME);
  });
...
describe("browser chrome helpers", () => {
  ...
  it("finds Playwright-managed Linux Chromium", () => {
    const browserPath = fs.mkdtempSync(path.join(os.tmpdir(), "recall-ms-playwright-"));
  ...
  it("stopRecallChrome no-ops when process is already killed", async () => {
    const proc = makeChromeTestProc({ killed: true });
    await stopChromeWithProc(proc, 10);
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopRecallChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopRecallChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
...
describe("browser chrome launch args", () => {
  it("does not force an about:blank tab at startup", () => {
    const args = buildRecallChromeLaunchArgs({
      ...
      userDataDir: "/tmp/recall-test-user-data",
    });

    expect(args).not.toContain("about:blank");
    expect(args).toContain("--remote-debugging-port=18800");
    expect(args).toContain("--user-data-dir=/tmp/recall-test-user-data");
  });
});
