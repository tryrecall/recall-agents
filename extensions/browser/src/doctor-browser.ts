...
import { DEFAULT_STEELENGINE_BROWSER_PROFILE_NAME, resolveBrowserConfig } from "./browser/config.js";
...
function collectChromeMcpProfiles(cfg: SteelEngineConfig): ExistingSessionProfile[] {
...
function collectManagedProfiles(cfg: SteelEngineConfig): ManagedProfile[] {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return [];
  }
  ...
  for (const [profileName, rawProfile] of Object.entries(configuredProfiles)) {
    const profile = asRecord(rawProfile);
    const driver = normalizeOptionalString(profile?.driver) ?? "steelengine";
    if (driver !== "existing-session") {
...
function isLegacyClawdProfileConfigured(cfg: SteelEngineConfig, legacyProfileDir: string): boolean {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return false;
  }
...
export function detectLegacyClawdBrowserProfileResidue(
  ...
  if (
    resolved.defaultProfile !== DEFAULT_STEELENGINE_BROWSER_PROFILE_NAME ||
...
function formatLegacyClawdBrowserProfileResidueNote(
  ...
  return [
    ...
    `- The canonical SteelEngine-managed browser profile is ${residue.canonicalUserDataDir}.`,
    `- If no browser is using the legacy profile, run ${formatCliCommand("steelengine doctor --fix")} to archive it safely instead of deleting it in place.`,
  ].join("\n");
}
...
export async function noteChromeMcpBrowserReadiness(
  ...
  if (!browserExecutable && managedProfiles.length > 0) {
    noteFn(
      [
        `- SteelEngine-managed browser profile(s) are configured: ${managedProfileLabel}.`,
  ...
  if (missingDisplay || shouldWarnRootNoSandbox) {
    const lines = [`- SteelEngine-managed browser profile(s) are configured: ${managedProfileLabel}.`];
    if (missingDisplay) {
      lines.push(
  ...
  if (autoConnectProfiles.length === 0) {
    noteFn(
      [
        ...
        "- Keep the browser running and accept the attach consent prompt the first time SteelEngine connects.",
      ].join("\n"),
      "Browser",
    );
    return;
  ...
  if (!chrome) {
    const lines = [
      `- Chrome MCP existing-session is configured for profile(s): ${profileLabel}.`,
      `- Google Chrome was not found on this host for auto-connect profile(s): ${autoProfileLabel}. SteelEngine does not bundle Chrome.`,
