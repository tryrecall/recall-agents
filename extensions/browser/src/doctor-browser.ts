...
import { DEFAULT_RECALL_BROWSER_PROFILE_NAME, resolveBrowserConfig } from "./browser/config.js";
...
function collectChromeMcpProfiles(cfg: RecallConfig): ExistingSessionProfile[] {
...
function collectManagedProfiles(cfg: RecallConfig): ManagedProfile[] {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return [];
  }
  ...
  for (const [profileName, rawProfile] of Object.entries(configuredProfiles)) {
    const profile = asRecord(rawProfile);
    const driver = normalizeOptionalString(profile?.driver) ?? "recall";
    if (driver !== "existing-session") {
...
function isLegacyClawdProfileConfigured(cfg: RecallConfig, legacyProfileDir: string): boolean {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return false;
  }
...
export function detectLegacyClawdBrowserProfileResidue(
  ...
  if (
    resolved.defaultProfile !== DEFAULT_RECALL_BROWSER_PROFILE_NAME ||
...
function formatLegacyClawdBrowserProfileResidueNote(
  ...
  return [
    ...
    `- The canonical Recall-managed browser profile is ${residue.canonicalUserDataDir}.`,
    `- If no browser is using the legacy profile, run ${formatCliCommand("recall doctor --fix")} to archive it safely instead of deleting it in place.`,
  ].join("\n");
}
...
export async function noteChromeMcpBrowserReadiness(
  ...
  if (!browserExecutable && managedProfiles.length > 0) {
    noteFn(
      [
        `- Recall-managed browser profile(s) are configured: ${managedProfileLabel}.`,
        "- No Chromium-based browser executable was found on this host for Recall-managed launch.",
  ...
  if (missingDisplay || shouldWarnRootNoSandbox) {
    const lines = [`- Recall-managed browser profile(s) are configured: ${managedProfileLabel}.`];
    if (missingDisplay) {
      lines.push(
  ...
  if (autoConnectProfiles.length === 0) {
    noteFn(
      [
        ...
        "- Keep the browser running and accept the attach consent prompt the first time Recall connects.",
      ].join("\n"),
      "Browser",
    );
    return;
  ...
  if (!chrome) {
    const lines = [
      `- Chrome MCP existing-session is configured for profile(s): ${profileLabel}.`,
      `- Google Chrome was not found on this host for auto-connect profile(s): ${autoProfileLabel}. Recall does not bundle Chrome.`,
