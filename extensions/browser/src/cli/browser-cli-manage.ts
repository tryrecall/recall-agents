...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "steelengine";
...
function formatBrowserDoctorGatewayError(error: unknown): string {
  ...
  return "Gateway auth SecretRef is unavailable in this command path; browser doctor cannot reach the admin-scoped browser.request endpoint. Set STEELENGINE_GATEWAY_TOKEN or STEELENGINE_GATEWAY_PASSWORD, then retry.";
}
...
async function runBrowserDoctor(parent: BrowserParentOpts, profile?: string, deep?: boolean) {
  ...
  checks.push({
    name: "profile",
    ok: true,
    detail: `${status.profile ?? "steelengine"} (${usesChromeMcpTransport(status) ? "chrome-mcp" : (status.transport ?? "cdp")})`,
  });
  checks.push({
    name: "browser",
    ok: status.running,
    detail: status.running
      ...
      : "not running; run `steelengine browser start`",
  });
  if (status.graphics) {
    checks.push({
      name: "graphics",
...
type BrowserProfileDriver = "steelengine" | "existing-session" | "extension";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "steelengine"}`,
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "steelengine" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (steelengine|existing-session). Default: steelengine")
    .action(
      async (
        opts: {
          name: string;
