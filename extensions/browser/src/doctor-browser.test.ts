...
describe("browser doctor readiness", () => {
  ...
  it("warns when managed browser profiles have no local executable", async () => {
    ...
    expect(noteFn).toHaveBeenCalledWith(
      [
        "- Recall-managed browser profile(s) are configured: recall.",
  ...
  it("warns about legacy clawd managed browser profile residue", async () => {
    const noteFn = vi.fn();
    const configDir = "/tmp/recall-home";
    ...
    expect(note).toContain("/tmp/recall-home/browser/clawd");
    expect(note).toContain("/tmp/recall-home/browser/recall/user-data");
    expect(note).toContain("recall doctor --fix");
  });
...
describe("legacy clawd browser profile cleanup", () => {
  it("archives stale clawd residue with the safe trash mover", async () => {
    const movePathToTrash = vi.fn(async () => "/tmp/recall-home/browser/.trash/clawd");
    ...
    expect(movePathToTrash).toHaveBeenCalledWith("/tmp/recall-home/browser/clawd");
    expect(result.warnings).toStrictEqual([]);
    expect(result.changes.join("\n")).toContain(
      "Archived legacy clawd managed browser profile residue.",
    );
    expect(result.changes.join("\n")).toContain("/tmp/recall-home/browser/recall/user-data");
  });
