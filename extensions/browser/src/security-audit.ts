import type { RecallPluginSecurityAuditContext } from "recall/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "recall/plugin-sdk/secret-input";
import { formatCliCommand } from "recall/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "recall/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: RecallPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("recall security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.RECALL_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.RECALL_GATEWAY_PASSWORD) ||
