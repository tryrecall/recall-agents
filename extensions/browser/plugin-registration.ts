...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage SteelEngine's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
};
...
function createBrowserToolOptions(ctx: SteelEnginePluginToolContext): {
  sandboxBridgeUrl?: string;
  allowHostControl?: boolean;
  agentSessionKey?: string;
  agentDir?: string;
...
export const browserPluginNodeHostCommands: SteelEnginePluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: SteelEnginePluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): SteelEnginePluginService {
  let service: SteelEnginePluginService | null = null;
...
export function registerBrowserPlugin(api: SteelEnginePluginApi) {
  configureSystemProfileImportStateStore(
  ...
  api.registerTool(((ctx: SteelEnginePluginToolContext) =>
    createLazyBrowserTool(createBrowserToolOptions(ctx))) as SteelEnginePluginToolFactory);
  api.registerCli(
    async ({ program }) => {
  ...
  api.registerHttpRoute({
    ...
    handler: (_req: IncomingMessage, res: ServerResponse) => {
      ...
      res.end("Upgrade Required: connect the SteelEngine Chrome extension over WebSocket.");
    },
