...
describe("maybeCreateDiscordAutoThread autoThreadName", () => {
  it("renames created thread when generated mode is enabled", async () => {
    ...
    const cfg = { agents: { defaults: { model: "anthropic/claude-opus-4-6" } } } as SteelEngineConfig;
    const result = await maybeCreateDiscordAutoThread(
      createBaseParams({
        baseText: "Need help with deploy rollout",
        combinedBody: "Need help with deploy rollout",
        channelName: "steelengine",
        channelDescription: "SteelEngine development coordination and release planning",
    ...
    expectGeneratedTitleField("channelName", "steelengine");
    expectGeneratedTitleField(
      "channelDescription",
      "SteelEngine development coordination and release planning",
    );
