...
vi.mock("steelengine/plugin-sdk/channel-outbound", async () => {
  const actual = await vi.importActual<typeof import("steelengine/plugin-sdk/channel-outbound")>(
    "steelengine/plugin-sdk/channel-outbound",
  );
  return {
    ...actual,
...
describe("deliverDiscordReply", () => {
      ...
      replies: [
        {
          text: [
            "[tool:exec]",
            "<parameter=command>",
            'cat /proc/mounts 2>/dev/null | grep -i "libra|rav|steelengine" | head -20',
            "</parameter>",
            "",
            "<function=exec>",
            "<parameter=command>",
  ...
  it("passes media roots and explicit off-mode payload reply tags to shared outbound", async () => {
    ...
    await deliverDiscordReply({
      ...
      textLimit: 2000,
      replyToMode: "off",
      mediaLocalRoots: ["/tmp/steelengine-media"],
      kind: "final",
    });
    ...
    expect(params.mediaAccess).toEqual({ localRoots: ["/tmp/steelengine-media"] });
  });
