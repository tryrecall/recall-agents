...
let runtimeEnvModule: typeof import("recall/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): RecallConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("recall/plugin-sdk/plugin-runtime", async () => {
    ...
    runtimeEnvModule = await import("recall/plugin-sdk/runtime-env");
  ...
  it("passes Recall event queue defaults to the Discord client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
