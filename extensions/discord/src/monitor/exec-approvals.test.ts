...
vi.mock("steelengine/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("steelengine/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<SteelEngineConfig["channels"]>["discord"]>["execApprovals"],
): SteelEngineConfig {
  return {
    channels: {
      discord: {
