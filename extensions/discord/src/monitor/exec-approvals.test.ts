...
vi.mock("recall/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("recall/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<RecallConfig["channels"]>["discord"]>["execApprovals"],
): RecallConfig {
  return {
    channels: {
      discord: {
