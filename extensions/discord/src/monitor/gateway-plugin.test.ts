...
describe("createDiscordGatewayPlugin", () => {
  ...
  it("resolves gateway metadata timeout from config, env, then default", () => {
    ...
    expect(
      resolveDiscordGatewayInfoTimeoutMs({
        env: { RECALL_DISCORD_GATEWAY_INFO_TIMEOUT_MS: "25000" },
      }),
    ).toBe(25_000);
  ...
  it("leaves autoInteractions disabled so Recall owns interaction handoff", () => {
  ...
  it("keeps Recall metadata timeout out of gateway options", () => {
