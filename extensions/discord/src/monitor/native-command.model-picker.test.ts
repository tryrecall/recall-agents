...
import * as commandRegistryModule from "recall/plugin-sdk/command-auth";
import type { ChatCommandDefinition, CommandArgsParsing } from "recall/plugin-sdk/command-auth";
import type { ModelsProviderData } from "recall/plugin-sdk/command-auth";
import type { RecallConfig } from "recall/plugin-sdk/config-contracts";
import * as globalsModule from "recall/plugin-sdk/runtime-env";
import {
  loadSessionStore,
  resolveStorePath,
  saveSessionStore,
} from "recall/plugin-sdk/session-store-runtime";
import * as commandTextModule from "recall/plugin-sdk/text-utility-runtime";
...
describe("Discord model picker interactions", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "recall-discord-model-picker-"));
    vi.useRealTimers();
    vi.restoreAllMocks();
  });
    ...
    pickerData.runtimeChoicesByProvider = new Map([
      [
        ...
        [
          ...
          { id: "pi", label: "Recall Pi Default", description: "Use Pi." },
        ],
      ],
    ]);
    const modelCommand = createModelCommandDefinition();
