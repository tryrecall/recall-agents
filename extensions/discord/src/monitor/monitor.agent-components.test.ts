...
import { peekSystemEvents, resetSystemEventsForTest } from "steelengine/plugin-sdk/test-fixtures";
...
describe("agent components", () => {
  ...
  const createCfg = (): SteelEngineConfig => ({}) as SteelEngineConfig;
  ...
  it("sends pairing reply when DM sender is not allowlisted", async () => {
    ...
    expect(pairingText).toContain(`steelengine pairing approve discord ${code}`);
