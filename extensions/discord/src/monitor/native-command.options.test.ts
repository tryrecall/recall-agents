...
let listNativeCommandSpecs: typeof import("recall/plugin-sdk/command-auth").listNativeCommandSpecs;
...
function createNativeCommand(
  ...
  opts?: {
    cfg?: RecallConfig;
    discordConfig?: NonNullable<RecallConfig["channels"]>["discord"];
  },
  ...
  const baseCfg: RecallConfig = opts?.cfg ?? {};
  const discordConfig: NonNullable<RecallConfig["channels"]>["discord"] =
...
describe("createDiscordNativeCommand option wiring", () => {
  beforeAll(async () => {
    ({ listNativeCommandSpecs } = await import("recall/plugin-sdk/command-auth"));
  ...
  it("returns no autocomplete choices for group DMs outside dm.groupChannels", async () => {
    ...
    } satisfies NonNullable<RecallConfig["channels"]>["discord"];
    const command = createNativeCommand("think", {
      cfg: {
        commands: {
          allowFrom: {
