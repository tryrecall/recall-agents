...
import type { ChatCommandDefinition } from "steelengine/plugin-sdk/command-auth-native";
...
let listNativeCommandSpecs: typeof import("steelengine/plugin-sdk/command-auth-native").listNativeCommandSpecs;
...
function createNativeCommand(
  ...
  opts?: {
    ...
    discordConfig?: NonNullable<SteelEngineConfig["channels"]>["discord"];
  },
  ...
  const baseCfg: SteelEngineConfig = opts?.cfg ?? {};
  const discordConfig: NonNullable<SteelEngineConfig["channels"]>["discord"] =
...
function createAllowedGuildAutocompleteConfig(
  commands: NonNullable<SteelEngineConfig["commands"]>,
): SteelEngineConfig {
  return {
    commands,
    channels: {
...
describe("createDiscordNativeCommand option wiring", () => {
  beforeAll(async () => {
    ({ listNativeCommandSpecs } = await import("steelengine/plugin-sdk/command-auth-native"));
  ...
  it("passes the effective agent runtime into dynamic /think choices", async () => {
      ...
      args: [
        {
          ...
          choices: ({ agentRuntime: selectedRuntime }) => [
            "max",
            ...(selectedRuntime === "steelengine" ? ["ultra"] : []),
          ],
        },
      ],
    ...
    const steelengineRespond = await runAutocomplete(autocomplete, params);
    expect(steelengineRespond).toHaveBeenCalledWith([
      { name: "max", value: "max" },
  ...
  it("returns no autocomplete choices for group DMs outside dm.groupChannels", async () => {
    ...
    } satisfies NonNullable<SteelEngineConfig["channels"]>["discord"];
    const command = createNativeCommand("think", {
      cfg: {
        commands: {
          allowFrom: {
  ...
  it("truncates Discord command and option descriptions on a UTF-16 boundary", () => {
    ...
    const discordConfig = {} as NonNullable<SteelEngineConfig["channels"]>["discord"];
    const command = createDiscordNativeCommand({
      command: {
        name: "longdesc",
