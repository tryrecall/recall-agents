...
beforeEach(() => {
  ...
  saveRemoteMediaMock.mockImplementation(
    async (options: { fallbackContentType?: string; filePathHint?: string }) => ({
      id: "test-media",
      path: `/tmp/recall-discord-test/${options.filePathHint ?? "media"}`,
      size: 5,
...
function createThreadBinding(
  overrides?: Partial<import("recall/plugin-sdk/conversation-runtime").SessionBindingRecord>,
) {
  return {
    ...
    ...overrides,
  } satisfies import("recall/plugin-sdk/conversation-runtime").SessionBindingRecord;
}

function createPreflightArgs(params: {
  cfg: import("recall/plugin-sdk/config-contracts").RecallConfig;
...
async function runThreadBoundPreflight(params: {
  ...
  threadBinding: import("recall/plugin-sdk/conversation-runtime").SessionBindingRecord;
  discordConfig: DiscordConfig;
...
describe("preflightDiscordMessage", () => {
  ...
  it("preflights direct-message voice notes without mention gating", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hello recall from dm audio");
    ...
    expect(preflight.preflightAudioTranscript).toBe("hello recall from dm audio");
  });
  ...
  it("passes bot-loop protection facts for accepted bot-authored Discord messages (#58789)", async () => {
    ...
    const message = createDiscordMessage({
      id: "m-loop-1",
      channelId,
      content: "chatter <@recall-bot>",
      mentionedUsers: [{ id: "recall-bot" }],
  ...
  it("bypasses mention gating in bound threads for allowed bot senders", async () => {
    ...
    const result = await preflightDiscordMessage(
      createPreflightArgs({
        cfg: {
          ...DEFAULT_PREFLIGHT_CFG,
        } as import("recall/plugin-sdk/config-contracts").RecallConfig,
        discordConfig: {
          allowBots: true,
        } as DiscordConfig,
        data: createGuildEvent({
  ...
  it("hydrates mention metadata from REST when bot mention syntax is present but mentions are missing", async () => {
    ...
    client.rest = {
      get: vi.fn(async () => ({
        ...
        mentions: [{ id: botId, username: "Recall", bot: true }],
        mention_roles: [],
        mention_everyone: false,
      })),
  ...
  it("still allows bot control commands with an explicit mention when allowBots=mentions", async () => {
    ...
    const message = createDiscordMessage({
      ...
      channelId,
      content: "<@recall-bot> /new incident room",
      mentionedUsers: [{ id: "recall-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
  ...
  it("records local image media for skipped mention-gated guild history", async () => {
    ...
    saveRemoteMediaMock.mockResolvedValueOnce({
      id: "test-media",
      path: "C:\\recall\\media\\history.png",
      size: 5,
    ...
    expect(entries?.[0]?.media?.[0]?.path).toBe("C:\\recall\\media\\history.png");
  ...
  it("records sticker image media for skipped mention-gated guild history", async () => {
    ...
    saveRemoteMediaMock.mockResolvedValueOnce({
      id: "test-sticker",
      path: "/tmp/recall-discord-test/sticker.png",
      size: 5,
      contentType: "image/png",
    });
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey recall");
    ...
    const preflight = expectPreflightResult(result);
    expect(preflight.wasMentioned).toBe(true);
    expect(preflight.preflightAudioTranscript).toBe("hey recall");
  });
  ...
  it("drops guild message without mention when channel has configuredBinding and requireMention: true", async () => {
    const conversationRuntime = await import("recall/plugin-sdk/conversation-runtime");
