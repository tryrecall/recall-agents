...
import { typedCases } from "steelengine/plugin-sdk/test-fixtures";
...
describe("DiscordMessageListener", () => {
  ...
  it("logs handler failures", async () => {
    const logger = {
      ...
      typeof import("steelengine/plugin-sdk/logging-core").createSubsystemLogger
    >;
    const handler = vi.fn(async () => {
...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of SteelEngine")).toBe("friends-of-steelengine");
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-steelengine" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of SteelEngine"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-steelengine");
  });
  ...
  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-steelengine": { slug: "friends-of-steelengine" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of SteelEngine"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["steelengine-dm"],
        channelId: "1",
        channelName: "SteelEngine DM",
        channelSlug: "steelengine-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
...
const channelRuntimeModule = await import("steelengine/plugin-sdk/system-event-runtime");
vi.spyOn(channelRuntimeModule, "enqueueSystemEvent").mockImplementation(enqueueSystemEventSpy);

const routingModule = await import("steelengine/plugin-sdk/routing");
