...
import { asBoolean } from "steelengine/plugin-sdk/string-coerce-runtime";
import {
...
const DISCORD_REALTIME_FORCED_CONSULT_REASON =
  "provider_final_transcript_without_steelengine_agent_consult";
...
function buildDiscordSpeakExactUserMessage(text: string): string {
  return [
    "Internal SteelEngine voice playback result.",
...
function extractDiscordExactSpeechConsultText(args: unknown): string | undefined {
  ...
  if (
    !message.includes("Speak this exact SteelEngine answer") &&
...
function buildDiscordRealtimeInstructions(params: {
  ...
  const base =
    params.instructions ??
    [
      "You are SteelEngine's Discord voice interface.",
  ...
  if (isDiscordAgentProxyVoiceMode(params.mode)) {
    return [
      ...
      "Mode: SteelEngine agent proxy.",
      "You are the realtime voice surface for the same SteelEngine agent the user can message directly.",
      "Do not mention a backend, supervisor, helper, or separate system. Present the result as your own work.",
      "Delegate substantive requests, actions, tool work, current facts, memory, workspace context, and user-specific context with steelengine_agent_consult.",
      "Do not block, refuse, or downscope at the voice layer. Delegate to SteelEngine and treat its result as authoritative.",
      "Answer directly only for greetings, acknowledgements, brief latency tests, or filler while waiting.",
      'While waiting for SteelEngine data or tool results, use at most one short natural backchannel such as "yeah", "mm-hmm", "got it", or "one sec"; vary it and do not treat it as the final answer.',
      "When SteelEngine sends an internal exact answer to speak, do not call tools. Say only that answer.",
      buildRealtimeVoiceAgentConsultPolicyInstructions({
