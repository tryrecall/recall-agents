...
export function buildDiscordSpeakExactUserMessage(text: string): string {
  return [
    "Internal Recall voice playback result.",
...
function extractDiscordExactSpeechConsultText(args: unknown): string | undefined {
  ...
  if (
    !message.includes("Speak this exact Recall answer") &&
  ...
  private async runForcedAgentProxyConsult(
    ...
    try {
      const promise = this.runAgentTurn({
        ...
        message: [
          question,
          "Context: The realtime model produced a final user transcript without calling openclaw_agent_consult. Recall is forcing the consult because consultPolicy is always.",
        ].join("\n\n"),
      });
...
function buildDiscordRealtimeInstructions(params: {
  ...
  const base =
    params.instructions ??
    [
      "You are Recall's Discord voice interface.",
  ...
  if (isDiscordAgentProxyVoiceMode(params.mode)) {
    return [
      ...
      "Mode: Recall agent proxy.",
      "You are the realtime voice surface for the same Recall agent the user can message directly.",
      ...
      "Do not block, refuse, or downscope at the voice layer. Delegate to Recall and treat its result as authoritative.",
      "Answer directly only for greetings, acknowledgements, brief latency tests, or filler while waiting.",
      "When Recall sends an internal exact answer to speak, do not call tools. Say only that answer.",
      buildRealtimeVoiceAgentConsultPolicyInstructions({
