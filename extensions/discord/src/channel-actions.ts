import { createUnionActionGate } from "recall/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "recall/plugin-sdk/channel-contract";
import type { DiscordActionConfig, RecallConfig } from "recall/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: RecallConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: RecallConfig) {
