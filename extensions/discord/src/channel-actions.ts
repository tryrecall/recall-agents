...
import { createUnionActionGate } from "steelengine/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "steelengine/plugin-sdk/channel-contract";
import type { DiscordActionConfig, SteelEngineConfig } from "steelengine/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: SteelEngineConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: SteelEngineConfig) {
