...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: RecallConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): RecallConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("recall/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/acp-runtime")>(
    "recall/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("recall/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/command-auth")>(
...
vi.mock("recall/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/reply-runtime")>(
...
vi.mock("recall/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("recall/plugin-sdk/native-command-config-runtime")
  >("recall/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("recall/plugin-sdk/runtime-config-snapshot", async () => {
  const actual = await vi.importActual<
    typeof import("recall/plugin-sdk/runtime-config-snapshot")
  >("recall/plugin-sdk/runtime-config-snapshot");
  return {
    ...actual,
    getRuntimeConfig: () => ({}),
  };
...
vi.mock("recall/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/error-runtime")>(
    "recall/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
