...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "recall-test-web-logout-"));
    previousOAuthDir = process.env.RECALL_OAUTH_DIR;
    process.env.RECALL_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  it("does not delete through symlinked auth dirs inside the Recall auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the Recall auth root", async () => {
