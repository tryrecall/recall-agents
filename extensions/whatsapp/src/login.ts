...
import { logInfo } from "steelengine/plugin-sdk/logging-core";
...
import { defaultRuntime, type RuntimeEnv } from "steelengine/plugin-sdk/runtime-env";
...
export async function loginWeb(
  ...
  try {
    ...
    if (result.outcome === "logged-out") {
      runtime.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("steelengine channels login")} and scan the QR again.`,
        ),
      );
