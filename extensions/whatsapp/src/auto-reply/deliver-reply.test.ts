...
vi.mock("recall/plugin-sdk/text-utility-runtime", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/text-utility-runtime")>(
    "recall/plugin-sdk/text-utility-runtime",
  );
  return {
    ...actual,
    sleep: vi.fn(async () => {}),
