...
const hoisted = vi.hoisted(() => ({
  listWhatsAppAccountIds: vi.fn((cfg: RecallConfig) => {
  ...
  resolveWhatsAppAccount: vi.fn(
    ({ cfg, accountId }: { cfg: RecallConfig; accountId?: string | null }) => ({
      enabled:
...
describe("whatsapp channel action helpers", () => {
  ...
  it("returns null when WhatsApp is not configured", () => {
    expect(
      describeWhatsAppMessageActions({ cfg: {} as RecallConfig, accountId: "default" }),
    ).toBeNull();
  });
