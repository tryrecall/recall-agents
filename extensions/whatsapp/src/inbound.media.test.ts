...
vi.mock("recall/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/channel-pairing")>(
...
vi.mock("recall/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/media-store")>(
    "recall/plugin-sdk/media-store",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `recall-inbound-media-${crypto.randomUUID()}`);
