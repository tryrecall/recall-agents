...
import { info, success } from "steelengine/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("steelengine channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
