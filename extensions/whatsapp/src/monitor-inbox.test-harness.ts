...
const pluginRuntimeMocks = vi.hoisted(() => {
  ...
  let stateDir = `/tmp/steelengine-whatsapp-ingress-${Date.now()}-${Math.random()}`;
  ...
  return {
    ...
    reset: () => {
      ...
      stateDir = `/tmp/steelengine-whatsapp-ingress-${Date.now()}-${Math.random()}`;
    },
  };
});
...
vi.mock("steelengine/plugin-sdk/channel-activity-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("steelengine/plugin-sdk/channel-activity-runtime")
  >("steelengine/plugin-sdk/channel-activity-runtime");
  return {
    ...actual,
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    resetWebInboundDedupe();
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "steelengine-auth-"));
    } else {
      authDir = undefined;
    }
  });
