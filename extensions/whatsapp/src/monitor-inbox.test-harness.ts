...
vi.mock("recall/plugin-sdk/channel-activity-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("recall/plugin-sdk/channel-activity-runtime")
  >("recall/plugin-sdk/channel-activity-runtime");
  return {
    ...actual,
...
function expectInboxPairingReplyText(
  ...
  expect(text).toContain("Recall: access not configured.");
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "recall-auth-"));
    } else {
      authDir = undefined;
    }
  });
