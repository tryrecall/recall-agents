...
import { formatCliCommand, formatDocsLink } from "recall/plugin-sdk/setup-tools";
...
function mergeWhatsAppConfig(
  cfg: RecallConfig,
  patch: Partial<NonNullable<NonNullable<RecallConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: RecallConfig, dmPolicy: DmPolicy): RecallConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: RecallConfig, allowFrom?: string[]): RecallConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: RecallConfig, selfChatMode: boolean): RecallConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: RecallConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so Recall can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Recall" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("recall channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
