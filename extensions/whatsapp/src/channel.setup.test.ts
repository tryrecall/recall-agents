...
import { createQueuedWizardPrompter } from "steelengine/plugin-sdk/plugin-test-runtime";
...
vi.mock("steelengine/plugin-sdk/setup", async () => {
  ...
  return {
    ...
    setSetupChannelEnabled: (cfg: SteelEngineConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<SteelEngineConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
