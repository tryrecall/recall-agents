...
import { stripAnsi } from "recall/plugin-sdk/channel-test-helpers";
...
const testState = vi.hoisted(() => ({
  authDir: `${(process.env.TMPDIR ?? "/tmp").replace(/\/+$/, "")}/recall-wa-creds-${process.pid}-${Math.random().toString(16).slice(2)}`,
}));
