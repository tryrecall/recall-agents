...
import { VERSION } from "steelengine/plugin-sdk/cli-runtime";
import {
  createHttp1EnvHttpProxyAgent,
  createHttp1ProxyAgent,
  createNodeProxyAgent,
} from "steelengine/plugin-sdk/fetch-runtime";
import { danger, success } from "steelengine/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "steelengine/plugin-sdk/runtime-env";
...
function resolveEnvWaWebSocketUrl(): string | undefined {
  const value = resolveWaWebSocketUrl(process.env[STEELENGINE_WHATSAPP_WEB_SOCKET_URL_ENV]);
  if (!value) {
    return undefined;
  }
  let url: URL;
  try {
    ...
    throw new Error(`${STEELENGINE_WHATSAPP_WEB_SOCKET_URL_ENV} must be a valid URL.`);
  }
  if (url.protocol !== "ws:" && url.protocol !== "wss:") {
    throw new Error(`${STEELENGINE_WHATSAPP_WEB_SOCKET_URL_ENV} must use ws:// or wss://.`);
  }
  return url.toString();
}
...
async function createWaSocketInternal(
  ...
  const sock = makeWASocket({
    ...
    browser: ["steelengine", "cli", VERSION],
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("steelengine channels login")}`,
              ),
            );
          }
        }
