...
import { withMockedWindowsPlatform, withRestoredMocks } from "steelengine/plugin-sdk/test-node-mocks";
import { optimizeImageToPng } from "steelengine/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub STEELENGINE_STATE_DIR.
    // Also keep it outside the SteelEngine temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["STEELENGINE_STATE_DIR"]);
    process.env.STEELENGINE_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "steelengine-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredSteelEngineTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/steelengine` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredSteelEngineTmpDir();
  ...
  it("allows default SteelEngine state workspace and sandbox roots", async () => {
  ...
  it("rejects default SteelEngine state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
