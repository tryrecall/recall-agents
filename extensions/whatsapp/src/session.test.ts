...
import { resetLogger, setLoggerOverride } from "recall/plugin-sdk/runtime-env";
...
const TEST_UNDICI_RUNTIME_DEPS_KEY = "__RECALL_TEST_UNDICI_RUNTIME_DEPS__";
...
function createTempCaFile(contents: string): string {
  const dir = createTempAuthDir("recall-wa-proxy-ca");
...
describe("web session", () => {
  ...
  it("creates WA socket with QR handler", async () => {
    const authDir = createTempAuthDir("recall-wa-creds-test");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("recall-wa-creds-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("recall-wa-authdir-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("recall-wa-auth-parent-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("recall-wa-creds-symlink-save");
      const targetPath = path.join(authDir, "target-creds.json");
      const credsPath = path.join(authDir, "creds.json");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("recall-wa-creds-parent-symlink-save");
      const targetBaseDir = path.join(rootDir, "target-base");
      const linkedBaseDir = path.join(rootDir, "linked-base");
      const authDir = path.join(linkedBaseDir, "default");
      fsSync.mkdirSync(targetBaseDir);
  ...
  it("adds managed proxy CA trust to WhatsApp env proxy agents", async () => {
    ...
    vi.stubEnv("RECALL_PROXY_ACTIVE", "1");
    vi.stubEnv("RECALL_PROXY_CA_FILE", caFile);
  ...
  it("logWebSelfId prints cached E.164 when creds exist", () => {
    const authDir = createTempAuthDir("recall-wa-log-self");
    fsSync.writeFileSync(
  ...
  it("logWebSelfId prints cached lid details when creds include a lid", () => {
    const authDir = createTempAuthDir("recall-wa-log-self-lid");
    fsSync.writeFileSync(
      path.join(authDir, "creds.json"),
      JSON.stringify({
        me: {
  ...
  it("does not clobber creds backup when creds.json is corrupted", async () => {
    const authDir = createTempAuthDir("recall-wa-corrupt-backup");
  ...
  it("serializes creds.update saves to avoid overlapping writes", async () => {
    ...
    const authDir = createTempAuthDir("recall-wa-queue");
    const openMock = mockFsOpenForCredsWrites({
  ...
  it("lets different authDir queues flush independently", async () => {
    ...
    const authDirA = createTempAuthDir("recall-wa-a");
    const authDirB = createTempAuthDir("recall-wa-b");
    const onError = vi.fn();
  ...
  it("rotates creds backup when creds.json is valid JSON", async () => {
    const authDir = createTempAuthDir("recall-wa-rotate-backup");
    const credsPath = path.join(authDir, "creds.json");
    const backupPath = path.join(authDir, "creds.json.bak");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("recall-wa-rotate-backup-symlink");
      const credsPath = path.join(authDir, "creds.json");
      const backupPath = path.join(authDir, "creds.json.bak");
  ...
  it("writes creds.json atomically via temp file and rename", async () => {
    ...
    try {
      await writeCredsJsonAtomically("/tmp/recall-oauth/whatsapp/default", {
        me: { id: "123@s.whatsapp.net" },
      });

      const write = firstWriteFileCall(openMock.writeFileSpy);
      expect(write.path).toContain(
        path.join("/tmp", "recall-oauth", "whatsapp", "default", ".creds."),
      );
      expect(typeof write.data).toBe("string");
      expect(write.options.mode).toBe(0o600);
      expect(write.options.flag).toBe("wx");
      ...
      expect(chmodSpy).toHaveBeenCalledWith(
        path.join("/tmp", "recall-oauth", "whatsapp", "default", "creds.json"),
        0o600,
      );
  ...
  it("keeps the previous creds.json valid if the atomic rename fails", async () => {
    const authDir = createTempAuthDir("recall-wa-creds-atomic");
    const credsPath = path.join(authDir, "creds.json");
