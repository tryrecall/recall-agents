...
import { formatEnvelopeTimestamp } from "steelengine/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("steelengine:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("steelengine:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[steelengine]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/steelengine-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("steelengine:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("steelengine:lastSocket")];
  if (typeof getter === "function") {
