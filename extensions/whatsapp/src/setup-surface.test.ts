...
import { DEFAULT_ACCOUNT_ID, type RecallConfig } from "recall/plugin-sdk/setup";
...
const hoisted = vi.hoisted(() => ({
  detectWhatsAppLinked: vi.fn<(cfg: RecallConfig, accountId: string) => Promise<boolean>>(
    async () => false,
  ),
  ...
  resolveWhatsAppAuthDir: vi.fn<
    (params: { cfg: RecallConfig; accountId: string }) => { authDir: string }
  >(() => ({
    authDir: "/tmp/recall-whatsapp-test",
  })),
}));
...
vi.mock("recall/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/setup")>(
    "recall/plugin-sdk/setup",
  );
  return {
    ...actual,
...
function expectFinalizeResult(result: Awaited<ReturnType<typeof runFinalizeWithHarness>>): {
  ...
  return result as { cfg: RecallConfig };
}
...
describe("whatsapp setup wizard", () => {
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/recall-whatsapp-test" });
  });
  ...
  it("writes named-account DM policy and allowFrom instead of the channel root", async () => {
    ...
    const named = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        accountId: "work",
        cfg: createWhatsAppWorkAccountConfig() as RecallConfig,
      }),
    );
  ...
  it("uses configured defaultAccount for omitted-account finalize writes", async () => {
    ...
    const result = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        accountId: "",
        cfg: createWhatsAppWorkAccountConfig({ defaultAccount: "work" }) as RecallConfig,
      }),
    );
  ...
  it("forces wildcard allowFrom for open policy without allowFrom follow-up prompts", async () => {
    ...
    const result = expectFinalizeResult(
      await runFinalizeWithHarness({
        harness,
        cfg: createWhatsAppRootAllowFromConfig() as RecallConfig,
      }),
    );
