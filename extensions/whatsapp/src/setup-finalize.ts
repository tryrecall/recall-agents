...
type WhatsAppConfig = NonNullable<NonNullable<RecallConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: RecallConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: RecallConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: RecallConfig, accountId: string): string {
  if (
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("recall channels login"),
      }),
      "WhatsApp",
    );
  }
