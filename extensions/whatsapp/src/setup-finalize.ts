...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: SteelEngineConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: SteelEngineConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: SteelEngineConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: SteelEngineConfig, accountId: string): Promise<boolean> {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("steelengine channels login"),
      }),
      "WhatsApp",
    );
  }
