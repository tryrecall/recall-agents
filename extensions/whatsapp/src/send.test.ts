...
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS } from "recall/plugin-sdk/media-runtime";
...
let resetLogger: typeof import("recall/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("recall/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: RecallConfig = {
  channels: { whatsapp: {} },
};
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("recall/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `recall-outbound-${crypto.randomUUID()}.log`);
