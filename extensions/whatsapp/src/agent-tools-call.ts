...
import { createActionGate, stringEnum } from "steelengine/plugin-sdk/channel-actions";
import type {
  AnyAgentTool,
  SteelEnginePluginApi,
  SteelEnginePluginToolContext,
} from "steelengine/plugin-sdk/core";
import { mulawToPcm } from "steelengine/plugin-sdk/realtime-voice";
...
async function resolveRequesterE164(params: {
  accountId: string;
  cfg: NonNullable<SteelEnginePluginToolContext["config"]>;
...
function resolveRuntimeConfig(api: SteelEnginePluginApi, context: SteelEnginePluginToolContext) {
    ...
    async execute(_toolCallId, rawParams, signal) {
      ...
      if (params.action === "status") {
        return jsonResult({
          ...
          note: "MeowCaller uses a separate WhatsApp linked-device session; it cannot reuse SteelEngine's Baileys credentials.",
        });
      }
      ...
      if (linkedSelf === target) {
        throw new Error(
          "WhatsApp cannot call the linked account itself; use a dedicated SteelEngine WhatsApp number",
        );
      }
      ...
      try {
        ...
        const tempDir = await fs.mkdtemp(
          path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-whatsapp-call-"),
        );
...
export function registerWhatsAppCallTool(api: SteelEnginePluginApi): void {
