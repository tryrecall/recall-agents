...
vi.mock("steelengine/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("steelengine/plugin-sdk/logging-core")>(
    "steelengine/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("SteelEngine WhatsApp listener close");
