...
describe("WhatsApp auth dir profile resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["STEELENGINE_STATE_DIR", "STEELENGINE_OAUTH_DIR"]);
    tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-wa-profile-"));
  ...
  it("resolves the default web auth dir from STEELENGINE_STATE_DIR at call time", async () => {
