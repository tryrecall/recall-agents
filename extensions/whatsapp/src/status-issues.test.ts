...
describe("collectWhatsAppStatusIssues", () => {
  beforeEach(() => {
    vi.stubEnv("STEELENGINE_CONTAINER_HINT", "");
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Run: steelengine channels login (scan QR on the gateway host).",
      },
    ]);
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Run: steelengine doctor (or restart the gateway). If it persists, relink via channels login and check logs.",
      },
    ]);
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Watch: steelengine logs --follow and run steelengine channels status --probe if disconnects continue. If it keeps flapping, restart the gateway or relink via channels login.",
      },
    ]);
  });
