...
import { buildRealtimeVoiceAgentConsultPolicyInstructions } from "steelengine/plugin-sdk/realtime-voice";
...
export async function buildRealtimeVoiceInstructions(params: {
  ...
  const capsule: string[] = [
    "SteelEngine agent voice context:",
    `- Agent id: ${agentId}`,
    "- Use this context to match the SteelEngine agent's personality and standing preferences on fast voice turns.",
    "- Treat this as compact context only; call steelengine_agent_consult when the caller needs the full agent brain, tools, memory, or workspace state.",
  ];
