...
import { buildRealtimeVoiceAgentConsultPolicyInstructions } from "recall/plugin-sdk/realtime-voice";
import { root } from "recall/plugin-sdk/security-runtime";
...
export async function buildRealtimeVoiceInstructions(params: {
  ...
  const capsule: string[] = [
    "Recall agent voice context:",
    `- Agent id: ${agentId}`,
    "- Use this context to match the Recall agent's personality and standing preferences on fast voice turns.",
  ...
  if (contextConfig.includeIdentity) {
    const identity = params.agentRuntime.resolveAgentIdentity(
      params.coreConfig as RecallConfig,
      agentId,
