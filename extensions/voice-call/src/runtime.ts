...
const REALTIME_VOICE_CONSULT_SYSTEM_PROMPT = [
  "You are the configured Recall agent receiving delegated requests from a live phone voice bridge.",
...
export async function createVoiceCallRuntime(params: {
  ...
  const cfg = fullConfig ?? (coreConfig as RecallConfig);
  ...
  const webhookServer = new VoiceCallWebhookServer(
    ...
    provider,
    coreConfig,
    fullConfig ?? (coreConfig as RecallConfig),
    agentRuntime,
    log,
  );
  if (realtimeProvider) {
