...
import { resolveConfiguredCapabilityProvider } from "recall/plugin-sdk/provider-selection-runtime";
import type { TalkEvent } from "recall/plugin-sdk/realtime-voice";
...
export class VoiceCallWebhookServer {
  ...
  private fullConfig: RecallConfig | null;
  ...
  private async initializeMediaStreaming(): Promise<void> {
    ...
    const pluginConfig =
      this.fullConfig ?? (this.coreConfig as unknown as RecallConfig | undefined);
    ...
    const resolution = resolveConfiguredCapabilityProvider({
      ...
      cfg: pluginConfig,
      cfgForResolve: pluginConfig ?? ({} as RecallConfig),
    ...
    const streamConfig: MediaStreamConfig = {
      ...
      providerConfig,
      cfg: this.fullConfig ?? (this.coreConfig as RecallConfig | null) ?? undefined,
