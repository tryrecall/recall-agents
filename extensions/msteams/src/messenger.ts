...
import { sleep } from "steelengine/plugin-sdk/text-utility-runtime";
import { loadWebMedia } from "steelengine/plugin-sdk/web-media";
import type { MarkdownTableMode, MSTeamsReplyStyle, SteelEngineConfig } from "../runtime-api.js";
...
export function renderReplyPayloadsToMessages(
  ...
  const tableMode =
    ...
    getMSTeamsRuntime().channel.text.resolveMarkdownTableMode({
      cfg: getMSTeamsRuntime().config.current() as SteelEngineConfig,
      channel: "msteams",
    });
