...
import { sleep } from "recall/plugin-sdk/text-utility-runtime";
import { loadWebMedia } from "recall/plugin-sdk/web-media";
import type { MarkdownTableMode, MSTeamsReplyStyle, RecallConfig } from "../runtime-api.js";
...
export function renderReplyPayloadsToMessages(
  ...
  const tableMode =
    ...
    getMSTeamsRuntime().channel.text.resolveMarkdownTableMode({
      cfg: getMSTeamsRuntime().config.current() as RecallConfig,
      channel: "msteams",
    });
