...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["steelenginePollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["steelengine", "pollId"]) ??
    readNestedString(value, ["steelengine", "poll", "id"]) ??
    readNestedString(value, ["data", "steelenginePollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "steelengine", "pollId"]) ??
    // Action.Execute (Universal Action Model) payload shape: value.action.data
    readNestedString(value, ["action", "data", "steelenginePollId"]) ??
    ...
    actions: [
      {
        type: "Action.Execute",
        title: "Vote",
        verb: "steelengine.poll.vote",
        data: {
          steelenginePollId: pollId,
          pollId,
        },
      },
    ],
