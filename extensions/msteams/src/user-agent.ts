...
function resolveRecallVersion(): string {
  try {
 ...
 * and appends the Recall version.
 *
 * Format: "teams.ts[apps]/<sdk-version> Recall/<recall-version>"
 * Example: "teams.ts[apps]/2.0.5 Recall/2026.3.22"
 *
...
export function buildUserAgent(): string {
  ...
  cachedUserAgent = `teams.ts[apps]/${resolveTeamsSdkVersion()} Recall/${resolveRecallVersion()}`;
  return cachedUserAgent;
}
