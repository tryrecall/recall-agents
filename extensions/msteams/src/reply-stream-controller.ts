...
/**
 * Bridges steelengine's reply pipeline callbacks to the SDK's `ctx.stream`.
...
export function createTeamsReplyStreamController(params: {
  ...
  // steelengine's reply pipeline calls onPartialReply with the cumulative text on
  ...
  return {
    ...
    onPartialReply(payload: { text?: string }): void {
      ...
      // mode, steelengine's pipeline doesn't deliver tokens — the model output
