...
function createConfig(port: number): RecallConfig {
  return {
    channels: {
      msteams: {
        enabled: true,
...
function updateMSTeamsConfig(
  cfg: RecallConfig,
  patch: NonNullable<NonNullable<RecallConfig["channels"]>["msteams"]>,
): void {
...
function requireRegisteredMSTeamsConfig(): RecallConfig {
  const registered = readMockCallArg(registerMSTeamsHandlers, 0, 1) as { cfg?: RecallConfig };
  if (!registered?.cfg) {
