...
describe("Hermes migration config mapping", () => {
  ...
  it("keeps built-in Hermes provider overrides on SteelEngine's canonical provider IDs", async () => {
    const root = await makeTempRoot();
    const source = path.join(root, "hermes");
    await writeFile(
      path.join(source, "config.yaml"),
  ...
  it("maps the Hermes MiniMax China route to SteelEngine's canonical provider", async () => {
    const root = await makeTempRoot();
    const source = path.join(root, "hermes");
    await writeFile(
      path.join(source, "config.yaml"),
