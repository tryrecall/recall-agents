...
import { MIGRATION_REASON_TARGET_EXISTS } from "steelengine/plugin-sdk/migration";
...
describe("Hermes migration file and skill items", () => {
  ...
  it("maps supported OAuth model providers and requests fresh SteelEngine authentication", async () => {
    const root = await makeTempRoot();
    const source = path.join(root, "hermes");
    ...
    expect(reauthItems.map((item) => item.reason)).toEqual([
      "Authenticate anthropic in SteelEngine after migration.",
      "Authenticate nous in SteelEngine after migration.",
      "Authenticate qwen with an API key after migration: steelengine onboard --auth-choice qwen-api-key.",
      "Authenticate minimax-portal in SteelEngine after migration.",
      "Authenticate xai in SteelEngine after migration.",
    ]);
  });
  ...
  it("applies files, appended memories, item backups, reports, and opt-in API keys", async () => {
    ...
    const previousAgentDir = process.env.STEELENGINE_AGENT_DIR;
    ...
    try {
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.STEELENGINE_AGENT_DIR = previousAgentDir;
      }
    }
  });
