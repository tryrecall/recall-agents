...
import { createCapturedPluginRegistration } from "recall/plugin-sdk/plugin-test-runtime";
...
describe("Hermes migration provider", () => {
  ...
  it("resolves tilde source paths against the OS home when RECALL_HOME is set", () => {
    const previous = process.env.RECALL_HOME;
    process.env.RECALL_HOME = path.join(path.sep, "tmp", "recall-home");
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.RECALL_HOME = previous;
      }
    }
  });
