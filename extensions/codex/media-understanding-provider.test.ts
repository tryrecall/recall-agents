...
describe("codex media understanding provider", () => {
  ...
  it("preserves configured WebSocket transport for media turns", async () => {
    ...
    expect(requests[1]?.params).toEqual(expect.objectContaining({ cwd: "/tmp/steelengine-agent" }));
    expect(requests[2]?.params).toEqual(expect.objectContaining({ cwd: "/tmp/steelengine-agent" }));
  });
  ...
  it("runs structured extraction through the same bounded Codex app-server path", async () => {
    ...
    expect(turnParams?.cwd).toBe("/tmp/steelengine-agent");
