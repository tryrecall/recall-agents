...
async function createBindingMigrationFixture(options: {
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-codex-doctor-"));
...
describe("codex doctor contract", () => {
  ...
  it.each([
    ...
    const externalDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-codex-store-"));
