...
vi.mock("steelengine/plugin-sdk/gateway-runtime", async () => {
  const actual = await vi.importActual<typeof import("steelengine/plugin-sdk/gateway-runtime")>(
    "steelengine/plugin-sdk/gateway-runtime",
  );
  return {
    ...actual,
      ...
      sessions: [
        {
          ...
          cwd: "/Users/test/Projects/steelengine",
          status: "idle",
          activeFlags: [],
...
describe("registerCodexSessionCli", () => {
  ...
  describe("sessions", () => {
    ...
    it("renders connected sessions, metadata, pagination, and offline host errors", async () => {
      ...
      expect(output).toContain("/Users/test/Projects/steelengine");
  ...
  describe("continue", () => {
    it("continues a Gateway-local thread and sanitizes the human-readable session key", async () => {
      ...
      expect(output).toBe("SteelEngine session: harness:codex:supervision:branch\n");
      expect(output).not.toContain("\u001b");
    });
