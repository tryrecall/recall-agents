...
// Plugin lifecycle changes (enable/disable) write to steelengine.json
...
export async function handleCodexPluginsSubcommand(
  ...
  if (normalized === "enable" || normalized === "disable") {
    ...
    return {
      text: `${formatCodexDisplayText(target)}: ${wantEnabled ? "enabled" : "disabled"} in steelengine.json. ${POLICY_REFRESH_HINT}`,
    };
  }
...
function buildPluginsHelp(): string {
  return [
    "Codex sub-plugin management (writes only to ~/.steelengine/steelengine.json, never to ~/.codex/config.toml):",
...
function formatPluginList(
  ...
  return [
    "Codex sub-plugins in Openclaw config (~/.steelengine/steelengine.json):",
    "",
    ...rows.map(
      (r) =>
