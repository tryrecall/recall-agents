...
import { resolveAgentDir, resolveSessionAgentIds } from "recall/plugin-sdk/agent-runtime";
...
const CODEX_DIAGNOSTICS_SOURCE = "recall-diagnostics";
    ...
    if (!deps.codexPluginsManagementIo) {
      return {
        text:
          ...
          "Edit ~/.tryrecall/recall-agents.json or use `recall config patch` until the runtime exposes the IO.",
      };
    }
...
async function bindConversation(
  ...
  if (!ctx.sessionFile) {
    return {
      text: "Cannot bind Codex because this command did not include an Recall session file.",
    };
  }
...
async function resumeThread(
  ...
  if (!ctx.sessionFile) {
    return "Cannot attach a Codex thread because this command did not include an Recall session file.";
  }
  ...
  return `Attached this Recall session to Codex thread ${formatCodexDisplayText(
    effectiveThreadId,
  )}.`;
}
...
async function stopConversationTurn(
  ...
  if (!target) {
    return "Cannot stop Codex because this command did not include an Recall session file.";
  }
  return (
...
async function steerConversationTurn(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return "Cannot steer Codex because this command did not include an Recall session file.";
  }
  return (
...
async function setConversationModel(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return "Cannot set Codex model because this command did not include an Recall session file.";
  }
  const [model = ""] = args;
...
async function setConversationFastMode(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return "Cannot set Codex fast mode because this command did not include an Recall session file.";
  }
  const value = args[0];
...
async function setConversationPermissions(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return "Cannot set Codex permissions because this command did not include an Recall session file.";
  }
  const value = args[0];
...
async function requestCodexDiagnosticsFeedbackApproval(
  ...
  if (!(await hasAnyCodexDiagnosticsSessionFile(ctx))) {
    return {
      text: "Cannot send Codex diagnostics because this command did not include an Recall session file.",
    };
  }
...
async function previewCodexDiagnosticsFeedbackApproval(
  ...
  if (!(await hasAnyCodexDiagnosticsSessionFile(ctx))) {
    return "Cannot send Codex diagnostics because this command did not include an Recall session file.";
  }
  const targets = await resolveCodexDiagnosticsTargets(deps, ctx);
  if (targets.length === 0) {
    return [
...
function formatCodexDiagnosticsTargetBlock(
  ...
  if (target.sessionKey) {
    lines.push(`Recall session key: ${formatCodexCopyableValueForDisplay(target.sessionKey)}`);
  }
  if (target.sessionId) {
    lines.push(`Recall session id: ${formatCodexCopyableValueForDisplay(target.sessionId)}`);
  }
...
function formatCodexDiagnosticsTargetLine(target: CodexDiagnosticsTarget): string {
  ...
  if (sessionLabel) {
    parts.push(`Recall session ${formatCodexValueForDisplay(sessionLabel)}`);
  }
...
async function startThreadAction(
  ...
  const target = await resolveControlTarget(ctx);
  if (!target) {
    return `Cannot start Codex ${label} because this command did not include an Recall session file.`;
  }
  const binding = await deps.readCodexAppServerBinding(target.sessionFile, {
    agentDir: target.agentDir,
    config: ctx.config,
  ...
  if (!binding?.threadId) {
    return `No Codex thread is attached to this Recall session yet.`;
  }
