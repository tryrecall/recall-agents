...
describe("native Codex thread tool", () => {
  ...
  async function withFixture(run: () => void | Promise<void>): Promise<void> {
    await withTempDir("steelengine-codex-threads-", async (tempRoot) => {
      root = tempRoot;
  ...
  function createTool(params?: {
    ...
    const context: SteelEnginePluginToolContext = {
      config: {},
      agentId: "main",
  ...
  it("forks a native thread and attaches the fork to the SteelEngine session", () =>
    withFixture(async () => {
      const request = vi.fn(async (_config, method: string) =>
        method === CODEX_CONTROL_METHODS.readThread
  ...
  it("rejects archive when another SteelEngine session owns the thread", () =>
    withFixture(async () => {
      await writeCodexAppServerBinding("session-id", {
      ...
      ).rejects.toThrow("owned by another SteelEngine session");
  ...
  it("rejects archive when a spawned descendant is owned by an SteelEngine session", () =>
    withFixture(async () => {
      await writeCodexAppServerBinding("session-id", {
        threadId: "current-thread",
        cwd: "/tmp/project",
      ...
      ).rejects.toThrow("spawned descendant is owned by an SteelEngine session");
