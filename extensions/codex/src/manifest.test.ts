...
describe("codex package manifest", () => {
  it("keeps runtime dependencies in the package manifest", () => {
    ...
    expect(packageJson.devDependencies).toHaveProperty("@steelengine/plugin-sdk");
    expect(packageJson.dependencies?.["@openai/codex"]).toBe(MAX_CODEX_APP_SERVER_VERSION);
    expect(packageJson.steelengine?.release?.requireLatestDependencies).toEqual(["@openai/codex"]);
    expect(packageJson.steelengine?.install?.requiredPlatformPackages).toEqual([
