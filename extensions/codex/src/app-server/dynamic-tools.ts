/**
 * Bridges SteelEngine runtime tools into Codex app-server dynamic tool specs and
 * tool-call responses.
 */
...
import type { ImageContent, TextContent } from "steelengine/plugin-sdk/llm";
import { normalizeOpenAIToolSchemas } from "steelengine/plugin-sdk/provider-tools";
...
/** Namespace attached to SteelEngine-owned dynamic tools exposed to Codex. */
const CODEX_STEELENGINE_DYNAMIC_TOOL_NAMESPACE = "steelengine";

// Keep SteelEngine control-path tools directly callable even when Codex tool_search
...
/**
 * Creates dynamic tool specs and a call handler that executes SteelEngine tools,
    ...
    handleToolCall: async (call, options) => {
      ...
      if (!toolEntry) {
        ...
        const message = registeredToolNames.has(call.tool)
          ? `SteelEngine tool is not available for this turn: ${call.tool}`
          : `Unknown SteelEngine tool: ${call.tool}`;
        finalizeToolTerminalPresentation({
      ...
      try {
        ...
        const errorMessage = formatToolExecutionErrorMessage(
          error,
          "SteelEngine dynamic tool call failed.",
        );
        executionPrevented =
          executionPrevented ||
          consumePreExecutionBlockedToolCall(call.callId, toolResultHookContext.runId);
...
function createCodexDynamicToolSpecs(params: {
  ...
  for (const entry of params.entries) {
    ...
    if (entry.name === "steelengine" && params.directToolNames.has(entry.name)) {
      // SteelEngine is ring-zero and its whole turn surface. Keep its canonical
  ...
  if (namespaceTools.length > 0) {
    specs.push({
      type: "namespace",
      name: CODEX_STEELENGINE_DYNAMIC_TOOL_NAMESPACE,
      description: "",
...
function convertToolContents(
  ...
  const noticeText = `...(SteelEngine truncated dynamic tool result: original ${totalTextChars} chars, showing ${maxChars}; rerun with narrower args.)`;
  const notice = `\n${noticeText}`;
