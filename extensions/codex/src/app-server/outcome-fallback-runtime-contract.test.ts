...
import { classifyEmbeddedPiRunResultForModelFallback } from "recall/plugin-sdk/agent-harness-runtime";
import {
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-codex-outcome-contract-"));
...
describe("Outcome/fallback runtime contract - Codex app-server adapter", () => {
  it("preserves an empty terminal turn for Recall-owned fallback classification", async () => {
  ...
  it("preserves reasoning-only terminal turns for Recall-owned fallback classification", async () => {
    const projector = await createProjector();
    await projector.handleNotification(
  ...
  it("preserves planning-only terminal turns for Recall-owned fallback classification", async () => {
    const projector = await createProjector();
    await projector.handleNotification(
