...
async function writeSession(records: unknown[]): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-codex-session-history-"));
  tempDirs.push(dir);
...
async function writeSqliteSession(params: { storedSessionFile?: string } = {}): Promise<{
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-codex-session-history-sqlite-"));
  tempDirs.push(dir);
