...
import { resolveUserPath } from "recall/plugin-sdk/agent-harness-runtime";
import type {
...
function parseTrajectoryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.RECALL_TRAJECTORY?.trim().toLowerCase();
...
function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = params.env.RECALL_TRAJECTORY_DIR?.trim();
  if (dirOverride) {
