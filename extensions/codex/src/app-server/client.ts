...
export class CodexAppServerClient {
  ...
  /** Registers a handler for app-server requests sent back to SteelEngine. */
...
class CodexAppServerVersionError extends Error {
  ...
  constructor(detectedVersion: string | undefined) {
    const detected = detectedVersion
      ...
      : "SteelEngine could not determine the running Codex version";
    super(
...
function readCodexVersionFromUserAgent(userAgent: string | undefined): string | undefined {
  ...
  // SteelEngine, Codex Desktop, or an env override, so only the slash-delimited
...
/** Returns true for app-server approval request methods SteelEngine can answer. */
