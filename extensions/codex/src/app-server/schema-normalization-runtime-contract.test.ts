...
describe("Codex app-server dynamic tool schema boundary contract", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-codex-schema-contract-"));
  });
  ...
  it("passes prepared executable dynamic tool schemas through thread start unchanged", async () => {
    ...
    expect(startPayload?.serviceName).toBe("Recall");
    ...
    expect(startPayload?.developerInstructions).toContain("Recall");
  });
