...
vi.mock("steelengine/plugin-sdk/session-transcript-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("steelengine/plugin-sdk/session-transcript-runtime")>();
  return {
    ...actual,
...
describe("importCodexThreadHistoryToTranscript", () => {
  it("imports only bounded user-visible conversation items with stable identities", async () => {
    const target = await createSqliteMirrorTarget("steelengine-codex-history-", {
  ...
  it("keeps the newest 200 visible messages and deduplicates a retried import", async () => {
    const target = await createSqliteMirrorTarget("steelengine-codex-bounded-history-", {
  ...
  it("assigns canonical assistant attribution and numeric fallback timestamps", async () => {
    const target = await createSqliteMirrorTarget("steelengine-codex-fallback-history-", {
...
describe("mirrorCodexAppServerTranscript", () => {
  it("mirrors user, assistant, and tool result messages by SQLite identity", async () => {
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-basic-");
  ...
  it("preserves gateway user-turn identity across Codex transcript mirroring", async () => {
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-user-identity-");
  ...
  it("emits message-bearing updates for newly appended mirrored messages only", async () => {
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-live-updates-");
  ...
  it("emits stable sequence numbers for multi-message mirror batches", async () => {
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-seq-");
  ...
  it("keeps assistant ownership when live update publication fails", async () => {
    ...
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-publish-failure-");
  ...
  it("deduplicates app-server turn mirrors by idempotency scope", async () => {
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-dedupe-");
    const messages = [
      makeAgentUserMessage({
        content: [{ type: "text", text: "hello" }],
        timestamp: Date.now(),
  ...
  it("reports final assistant ownership for new and idempotent mirrors", async () => {
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-assistant-owned-");
    const assistantMessage = attachCodexMirrorIdentity(
      makeAgentAssistantMessage({
  ...
  it("runs before_message_write before appending mirrored transcript messages", async () => {
    ...
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-hook-");
  ...
  it("returns the persisted user message for duplicate mirror hits", async () => {
    ...
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-duplicates-");
  ...
  it("preserves the computed idempotency key when hooks rewrite message keys", async () => {
    ...
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-key-hook-");
    const sourceMessage = makeAgentAssistantMessage({
      content: [{ type: "text", text: "hello" }],
      timestamp: Date.now(),
    });
  ...
  it("respects before_message_write blocking decisions", async () => {
    ...
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-blocked-");
  ...
  it("leaves the assistant unowned when transcript persistence fails", async () => {
    const root = await makeRoot("steelengine-codex-transcript-failure-");
    const assistantMessage = attachCodexMirrorIdentity(
      makeAgentAssistantMessage({
  ...
  it("dedupes mirrored messages despite snapshot positional shifts", async () => {
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-shift-");
    const userMessage = attachCodexMirrorIdentity(
      makeAgentUserMessage({
        content: [{ type: "text", text: "hello" }],
        timestamp: Date.now(),
  ...
  it("keeps repeated same-content turns distinct", async () => {
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-repeat-");
  ...
  it("dedupes prior-turn entries re-emitted into a later turn's snapshot", async () => {
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-reemit-");
    const userTurn1 = attachCodexMirrorIdentity(
  ...
  it("uses the role+content fingerprint when no identity is attached", async () => {
    const target = await createSqliteMirrorTarget("steelengine-codex-mirror-fingerprint-");
    const userMessage = makeAgentUserMessage({
      content: [{ type: "text", text: "hello" }],
      timestamp: Date.now(),
    });
