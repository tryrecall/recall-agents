...
async function createCodexSideToolBridge(input: {
  ...
  if (supportsModelTools(runtimeModel)) {
    const createRecallCodingTools = (await import("recall/plugin-sdk/agent-harness"))
...
async function handleSideDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    return failedSideDynamicToolResponse("Recall dynamic tool call aborted before execution.");
  }
  ...
  const timeoutPromise = new Promise<CodexDynamicToolCallResponse>((resolve) => {
    ...
    timeout = setTimeout(() => {
      controller.abort(new Error(`Recall dynamic tool call timed out after ${timeoutMs}ms.`));
      resolve(
        failedSideDynamicToolResponse(`Recall dynamic tool call timed out after ${timeoutMs}ms.`),
      );
    }, timeoutMs);
    timeout.unref?.();
  });
