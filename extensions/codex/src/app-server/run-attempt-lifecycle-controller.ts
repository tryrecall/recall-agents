...
export function createCodexAttemptLifecycleController(
  ...
  const { state, activeTurnItemIds, pendingSteelEngineDynamicToolCompletionIds, turnWatches } =
    turnRuntime;
  const releaseTurnAfterTerminalDynamicTool = (value: {
    ...
    if (
      !shouldReleaseTurnAfterTerminalDynamicTool({
        ...
        pendingSteelEngineDynamicToolCompletionIdsCount: pendingSteelEngineDynamicToolCompletionIds.size,
      })
    ) {
      return;
    }
