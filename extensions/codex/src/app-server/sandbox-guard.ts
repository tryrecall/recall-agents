...
const DIRECT_METHOD_POLICIES = new Map<string, DirectMethodPolicy>([
  ...
  ["thread/start", "requires-recall-environment"],
...
export function resolveCodexAppServerDirectSandboxBypassBlock(params: {
  ...
  if (
    policy === "requires-recall-environment" &&
    hasRecallSandboxEnvironmentSelection(params.requestParams)
  ) {
    return undefined;
  }
  return sandboxBlock;
...
function hasRecallSandboxEnvironmentSelection(value: unknown): boolean {
  ...
  return (
    ...
    environments.every((entry) => {
      ...
      return (
        ...
        environment.environmentId.startsWith("recall-sandbox-") &&
...
function formatCodexNativeSandboxBlock(params: { surface: string }): string {
  return [
    `Codex-native ${params.surface} is unavailable because Recall sandboxing is active for this session.`,
    "This mode cannot route execution through the Recall sandbox backend.",
