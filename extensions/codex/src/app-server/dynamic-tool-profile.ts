...
/** Tool names owned by Codex app-server and normally excluded from SteelEngine dynamic tools. */
...
type CodexDynamicToolProfileEnv = {
  STEELENGINE_BUILD_PRIVATE_QA?: string;
  STEELENGINE_QA_FORCE_RUNTIME?: string;
};

/** Normalizes SteelEngine/Codex tool names before filtering and allowlist checks. */
export function normalizeCodexDynamicToolName(name: string): string {
  const normalized = name.trim().toLowerCase();
  return DYNAMIC_TOOL_NAME_ALIASES[normalized] ?? normalized;
}

/** True only for the host-scoped SteelEngine run's exact tool contract. */
export function isSystemAgentOnlyCodexDynamicToolAllowlist(
  toolsAllow: readonly string[] | undefined,
): boolean {
  return (
    toolsAllow?.length === 1 && normalizeCodexDynamicToolName(toolsAllow[0] ?? "") === "steelengine"
  );
}
...
export function isForcedPrivateQaCodexRuntime(
  ...
  return (
    ...
    env.STEELENGINE_QA_FORCE_RUNTIME?.trim().toLowerCase() === "codex"
  );
}
...
/** Filters SteelEngine tools that Codex owns natively or config explicitly excludes. */
export function filterCodexDynamicTools<T extends { name: string }>(
  ...
  return filterCodexDynamicToolsWithOptions(tools, config, env, {
    preserveSteelEngineShell: false,
  });
}

/** Keeps exec/process only when Codex cannot advertise an environment-backed native shell. */
export function filterCodexDynamicToolsWithSteelEngineShell<T extends { name: string }>(
  tools: T[],
  config: Pick<CodexPluginConfig, "codexDynamicToolsExclude">,
  env: CodexDynamicToolProfileEnv = process.env,
): T[] {
  return filterCodexDynamicToolsWithOptions(tools, config, env, {
    preserveSteelEngineShell: true,
  });
}
...
function filterCodexDynamicToolsWithOptions<T extends { name: string }>(
  ...
  options: { preserveSteelEngineShell: boolean },
): T[] {
  ...
  if (!isForcedPrivateQaCodexRuntime(env)) {
    for (const name of CODEX_APP_SERVER_OWNED_DYNAMIC_TOOL_EXCLUDES) {
      if (options.preserveSteelEngineShell && CODEX_APP_SERVER_OWNED_SHELL_TOOL_EXCLUDES.has(name)) {
        continue;
      }
      excludes.add(name);
    }
