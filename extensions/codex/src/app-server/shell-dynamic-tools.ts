...
export function createNodeExecDynamicTool(
  execTool: SteelEngineDynamicTool,
  ...
  return {
    ...
    description: pinnedNode
      ? "Run a shell command on the SteelEngine configured remote node for this session. This tool always uses SteelEngine host=node internally and follows the existing node exec approval and allowlist policy. Use node_process for follow-up on backgrounded node_exec sessions. Use Codex's native shell for local app-server work."
      : "Run a shell command on an SteelEngine remote node. Select the node by name or id when multiple nodes are available. This tool always uses SteelEngine host=node internally and follows the existing node exec approval and allowlist policy. Use node_process for follow-up on backgrounded node_exec sessions. Use Codex's native shell for local app-server work.",
...
export function createNodeProcessDynamicTool(
  processTool: SteelEngineDynamicTool,
): SteelEngineDynamicTool {
  return {
    ...processTool,
    name: CODEX_NODE_PROCESS_DYNAMIC_TOOL_NAME,
    description:
      "Manage node_exec sessions that were started on SteelEngine remote nodes: list, poll, log, write, send-keys, submit, paste, kill, clear, or remove. Use only for node_exec follow-up; use Codex's native shell session handling for local app-server work.",
  };
}
...
function hideNodeExecDynamicToolParameters(
  parameters: SteelEngineDynamicTool["parameters"],
