...
import { upsertAuthProfile } from "steelengine/plugin-sdk/provider-auth";
...
vi.mock("steelengine/plugin-sdk/agent-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("steelengine/plugin-sdk/agent-runtime")>();
  return {
    ...actual,
    resolveApiKeyForProfile: async (
      ...
      if (params.forceRefresh || (oauthCredential.expires ?? 0) <= Date.now()) {
        ...
        if (refreshed?.access) {
          ...
          if (params.agentDir || process.env.STEELENGINE_STATE_DIR) {
...
describe("bridgeCodexAppServerStartOptions", () => {
  it("never overlays persisted profiles onto a supplied runtime store", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-codex-app-server-"));
  ...
  it("uses the native user Codex home for coexistence mode", async () => {
    await withTempDir("steelengine-codex-user-home-", async (root) => {
  ...
  it("places the ephemeral auth-store override after configured root overrides", async () => {
    await withTempDir("steelengine-codex-auth-store-", async (agentDir) => {
      const startOptions = createStartOptions({
  ...
  it("does not mistake an option value for the app-server subcommand", async () => {
    await withTempDir("steelengine-codex-profile-name-", async (agentDir) => {
      const startOptions = createStartOptions({
  ...
  it("preserves custom stdio backend arguments", async () => {
    await withTempDir("steelengine-codex-custom-backend-", async (agentDir) => {
      const startOptions = createStartOptions({
  ...
  it("fingerprints supplied token stores with the same profile id independently", async () => {
    const resolveKey = async (token: string) =>
      await resolveCodexAppServerAuthAccountCacheKey({
        agentDir: "/tmp/steelengine-codex-prepared-auth",
        authProfileId: "openai:work",
        authProfileStore: {
          version: 1,
          profiles: {
  ...
  it("selects ordered Codex OAuth before an OpenAI API-key backup", async () => {
    ...
    await applyCodexAppServerAuthProfile({
      client: { request } as never,
      agentDir: "/tmp/steelengine-codex-auth-product-proof",
      authProfileStore,
    });
  ...
  it("applies native Codex CLI OAuth when no SteelEngine auth profile exists", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-codex-app-server-"));
    const agentDir = path.join(root, "agent");
  ...
  it("finds native Codex OAuth in the OS home when SteelEngine uses an isolated home", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-codex-app-server-"));
    const osHome = path.join(root, "os-home");
    const steelEngineHome = path.join(root, "steelengine-home");
    const agentDir = path.join(steelEngineHome, "agents", "main", "agent");
    const request = vi.fn(async () => ({ type: "chatgptAuthTokens" }));
  ...
  it("answers refresh from native Codex CLI OAuth without persisting an SteelEngine profile", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-codex-app-server-"));
    const agentDir = path.join(root, "agent");
    const codexHome = path.join(root, "codex-cli");
  ...
  it("refreshes inherited main Codex OAuth without cloning it into the child store", async () => {
    ...
    vi.stubEnv("STEELENGINE_AGENT_DIR", "");
    oauthMocks.refreshOpenAICodexToken.mockResolvedValueOnce({
