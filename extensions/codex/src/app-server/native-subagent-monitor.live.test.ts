...
const LIVE =
  process.env.STEELENGINE_LIVE_TEST === "1" && process.env.STEELENGINE_LIVE_CODEX_NATIVE_SUBAGENT === "1";
...
describeLive("codex native subagent monitor live", () => {
  it("delivers spawned subagent results live and recovers them from history", async () => {
    ...
    await withTempDir("steelengine-codex-native-subagent-", async (root) => {
      ...
      try {
        ...
        // the parent turn completes first, like an SteelEngine run cleaning up
