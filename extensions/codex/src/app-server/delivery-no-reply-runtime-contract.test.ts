...
import { isSilentReplyPayloadText } from "steelengine/plugin-sdk/reply-chunking";
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-codex-delivery-contract-"));
