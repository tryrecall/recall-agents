...
import { resolveAgentWorkspaceDir } from "steelengine/plugin-sdk/agent-runtime";
import {
...
export async function buildCodexWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    const developerInstructionFiles = shouldInjectCodexSteelEnginePromptContext(params.params)
      ? selectCodexWorkspaceInheritedDeveloperInstructionFiles(contextFiles)
      : [];
    const turnScopedDeveloperInstructionFiles = shouldInjectCodexSteelEnginePromptContext(
      params.params,
    )
    ...
    return {
      ...
      memoryCollaborationInstructions: shouldInjectCodexSteelEnginePromptContext(params.params)
...
/**
 * Builds SteelEngine-provided workspace prompt context for the current Codex turn.
 */
export function buildCodexSteelEnginePromptContext(params: {
  params: EmbeddedRunAttemptParams;
  workspacePromptContext?: string;
}): string | undefined {
  if (!shouldInjectCodexSteelEnginePromptContext(params.params)) {
    return undefined;
  }
  const sections = [
    params.workspacePromptContext?.trim()
      ? ["## SteelEngine Workspace Context", "", params.workspacePromptContext.trim()].join("\n")
      : undefined,
  ].filter(isNonEmptyString);
...
function shouldInjectCodexSteelEnginePromptContext(params: EmbeddedRunAttemptParams): boolean {
...
/** Renders loaded SteelEngine skill prompts as Codex collaboration instructions. */
export function renderCodexSkillsCollaborationInstructions(params: {
  attempt: EmbeddedRunAttemptParams;
  skillsPrompt?: string;
}): string | undefined {
  if (!shouldInjectCodexSteelEnginePromptContext(params.attempt)) {
    return undefined;
  }
  return params.skillsPrompt?.trim()
    ? ["## SteelEngine Skills", "", params.skillsPrompt.trim()].join("\n")
    : undefined;
}

/**
 * Prepends SteelEngine context while preserving leading delivery metadata as
 * routing guidance instead of user request text.
 */
export function prependCodexSteelEnginePromptContext(
  prompt: string,
...
function renderCodexWorkspaceThreadDeveloperInstructions(
  ...
  return renderCodexWorkspaceDeveloperInstructions({
    files,
    header: "## SteelEngine Workspace Instructions",
    preamble:
...
function renderCodexWorkspaceMemoryReference(params: {
  ...
  const lines = [
    "## SteelEngine Workspace Memory",
    "",
    `MEMORY.md exists in the active agent workspace as a memory file, not an instruction file. SteelEngine does not paste its contents into native Codex turns; use ${toolNames.join(" or ")} when durable memory is relevant and the tools are available.`,
    "",
  ];
