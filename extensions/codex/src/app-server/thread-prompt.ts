...
import { listRegisteredPluginAgentPromptGuidance } from "steelengine/plugin-sdk/plugin-runtime";
...
function buildDeferredDynamicToolManifest(
  ...
  return `Deferred searchable SteelEngine dynamic tools available: ${deferredToolNames.join(", ")}. Use \`tool_search\` to load exact callable specs before use.`;
}
...
function buildVisibleReplyInstruction(
  ...
  if (messageToolAvailable) {
    return "For the current source conversation, reply normally in your final assistant message; SteelEngine will deliver it through the active source conversation. Use `message` only for explicit out-of-band sends, media/file sends, or sends to a different target.";
  }
  return "For the current source conversation, reply normally in your final assistant message; SteelEngine will deliver it through the active source conversation.";
}
