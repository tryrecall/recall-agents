/**
 * Bridges Codex app-server approval requests into SteelEngine policy hooks and
 * plugin approval UX.
 */
import {
...
export async function handleCodexAppServerApprovalRequest(params: {
  ...
  try {
    const policyOutcome = await runSteelEngineToolPolicyForApprovalRequest({
      method: params.method,
      requestParams,
      paramsForRun: params.paramsForRun,
      context,
...
/** Converts an SteelEngine approval outcome into the app-server method response. */
...
async function runSteelEngineToolPolicyForApprovalRequest(params: {
  method: string;
  requestParams: JsonObject | undefined;
  paramsForRun: EmbeddedRunAttemptParams;
  ...
  const policyRequest = buildSteelEngineToolPolicyRequest(params.method, params.requestParams);
  if (!policyRequest) {
    return undefined;
  }
...
> {
  ...
  try {
    ...
    return {
      handled: true,
      blocked: true,
      reason: `SteelEngine native hook relay unavailable for Codex app-server approval: ${formatCodexDisplayText(
        formatErrorMessage(error),
      )}`,
...
function readNativeRelayPreToolUseDecision(response: NativeHookRelayProcessResponse | undefined):
  ...
  if (!response || response.exitCode !== 0) {
    return {
      ...
      reason:
        ...
        "SteelEngine native hook relay failed for Codex app-server approval.",
  ...
  if (output?.permissionDecision === "deny") {
    return {
      ...
      reason:
        ...
        "SteelEngine native hook policy denied Codex app-server approval.",
  ...
  return {
    blocked: true,
    reason: output
      ? "SteelEngine native hook relay returned a non-deny Codex app-server approval decision."
...
function buildSteelEngineToolPolicyRequest(
  method: string,
  requestParams: JsonObject | undefined,
