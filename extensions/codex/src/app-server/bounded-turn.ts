...
import type { AuthProfileStore } from "steelengine/plugin-sdk/agent-runtime";
...
const STEELENGINE_CODEX_APP_SERVER_ARGS_ENV_VAR = "STEELENGINE_CODEX_APP_SERVER_ARGS";
...
function buildPrivateCodexAppServerStartOptions(
  ...
  const privateEnv = Object.fromEntries(
    Object.entries(start.env ?? {}).filter(
      ([name]) => name.trim().toUpperCase() !== STEELENGINE_CODEX_APP_SERVER_ARGS_ENV_VAR,
    ),
  );
  const clearEnv = (start.clearEnv ?? []).filter((name) => {
    const normalized = name.trim().toUpperCase();
    return normalized !== "CODEX_HOME" && normalized !== STEELENGINE_CODEX_APP_SERVER_ARGS_ENV_VAR;
  });
  return {
    ...start,
    ...
    clearEnv: [...clearEnv, STEELENGINE_CODEX_APP_SERVER_ARGS_ENV_VAR],
  };
}
...
function createCodexBoundedApprovalHandler(taskLabel: string) {
  return (request: { method: string }): JsonValue | undefined => {
    if (
      ...
      return {
        decision: "decline",
        reason: `SteelEngine Codex ${taskLabel} does not grant tool or file approvals.`,
      };
    }
    ...
    if (request.method.includes("requestApproval")) {
      return {
        decision: "decline",
        reason: `SteelEngine Codex ${taskLabel} does not grant native approvals.`,
      };
    }
