...
import { resolveSandboxRuntimeStatus } from "recall/plugin-sdk/sandbox";
import { getSessionEntry, type SessionEntry } from "recall/plugin-sdk/session-store-runtime";
...
export function resolveCodexNativeExecutionPolicy(params: {
  ...
  return {
    ...
    node,
    blockReason:
      "Recall exec host=node is active for this session. Codex app-server native execution cannot route shell, filesystem, MCP, or app-backed work through the selected Recall node.",
  };
}
...
export function formatCodexNativeNodeExecBlock(params: {
  ...
  return [
    `Codex-native ${params.surface} is unavailable because Recall exec host=node is active for this session.`,
    params.reason ??
      "Codex app-server native execution cannot route execution through the selected Recall node.",
    "Use a normal Codex harness turn so Recall exec/process tools run on the node, or switch exec host to gateway for native Codex app-server execution.",
  ].join(" ");
}
...
function listAgentEntries(config: RecallConfig): AgentEntry[] {
