...
import type { ManagedProcess, SteelEngineExecServer, ProcessChunk } from "./types.js";
...
async function runProcess(
  ...
  if (!backend) {
    throw new Error("SteelEngine sandbox backend is unavailable.");
  }
  ...
  try {
    ...
    if (!command) {
      throw new Error("SteelEngine sandbox exec spec did not provide a command.");
    }
