 ...
 * access through the active SteelEngine sandbox backend.
 */
...
import { SsrFBlockedError, isBlockedHostnameOrIp } from "steelengine/plugin-sdk/ssrf-runtime";
...
import type { HttpHeader, SteelEngineExecServer } from "./types.js";
...
async function runStreamingSandboxHttpRequest(
  ...
  try {
    ...
    if (!command) {
      throw new Error("SteelEngine sandbox HTTP exec spec did not provide a command.");
    }
...
tmp=$(mktemp "$TMPDIR/steelengine-http.XXXXXX.py" 2>/dev/null || mktemp "/tmp/steelengine-http.XXXXXX.py") || exit 1
trap 'rm -f "$tmp"' EXIT
cat > "$tmp" <<'PY'
import base64
import json
