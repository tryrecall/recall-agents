...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-codex-trajectory-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("Codex trajectory recorder", () => {
  ...
  it("rejects a SQLite marker for a different session identity", () => {
    ...
    const recorder = createCodexTrajectoryRecorder({
      cwd: tmpDir,
      attempt: {
        sessionFile: "sqlite:main:other:/tmp/steelengine-agent.sqlite",
        sessionId: "session-1",
        model: { api: "responses" },
      } as never,
  ...
  it("warns when the SQLite host recorder is unavailable", () => {
    ...
    const recorder = createCodexTrajectoryRecorder({
      cwd: makeTempDir(),
      attempt: {
        sessionFile: "sqlite:main:session-1:/tmp/steelengine-agent.sqlite",
        sessionId: "session-1",
        model: { api: "responses" },
      } as never,
      env: {},
