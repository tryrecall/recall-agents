      ...
      createMigrationItem({
        ...
        reason: collides
          ...
          : targetExists && !params.overwrite
            ? MIGRATION_REASON_TARGET_EXISTS
            : undefined,
        message: `Copy ${item.skill.sourceLabel} into this Recall agent workspace.`,
        details: {
          skillName: item.name,
    ...
    if (
      ...
      items.push(
        createMigrationItem({
          ...
          message: `Install Codex plugin "${plugin.pluginName}" in the Recall-managed Codex app-server runtime.`,
          details: {
            configKey,
    ...
    items.push(
      createMigrationManualItem({
        ...
        message:
          ...
          `Codex native plugin "${plugin.name}" was found but not activated automatically.`,
        recommendation:
          "Review the plugin bundle first, then install trusted compatible plugins with recall plugins install <path>.",
      }),
    );
  }
  return items;
...
function buildPluginConfigItem(
  ...
  return createMigrationItem({
    ...
    message:
      "Enable Recall's Codex plugin integration and record migrated source-installed curated plugins.",
    details: {
...
export async function buildCodexMigrationPlan(
  ...
  return {
    ...
    warnings,
    nextSteps: [
      "Run recall doctor after applying the migration.",
      "Review skipped or auth-required Codex plugin/config/hook items before exposing them in Recall sessions.",
    ],
    metadata: {
