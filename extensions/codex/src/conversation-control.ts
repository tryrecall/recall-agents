...
import { ModelSelectionLockedError } from "steelengine/plugin-sdk/model-session-runtime";
...
const CODEX_CONVERSATION_CONTROL_STATE = Symbol.for("steelengine.codex.conversationControl");
...
async function requireThreadBinding(
  ...
  if (!binding?.threadId) {
    throw new Error("No Codex thread is attached to this SteelEngine session yet.");
  }
  return binding;
}
