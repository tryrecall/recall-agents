...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/recall-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("recall/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/security-runtime")>(
    "recall/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("recall/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/system-event-runtime")>(
    "recall/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("recall/plugin-sdk/reply-runtime"),
      import("recall/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
