...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/steelengine-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("steelengine/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("steelengine/plugin-sdk/security-runtime")>(
    "steelengine/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("steelengine/plugin-sdk/reply-runtime"),
      import("steelengine/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    const createdStateDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "steelengine-signal-tool-result-state-"),
    );
