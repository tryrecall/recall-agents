...
export const signalDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: RecallConfig) => cfg.channels?.signal?.dmPolicy ?? "pairing",
  setPolicy: (cfg: RecallConfig, policy) =>
    setChannelDmPolicyWithAllowFrom({
      cfg,
      channel,
      dmPolicy: policy,
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "Recall"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("recall gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
