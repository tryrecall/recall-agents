...
import type { SteelEnginePluginCommandDefinition } from "steelengine/plugin-sdk/core";
...
function createApi(run = vi.fn().mockResolvedValue({ runId: "run-1" })): SteelEnginePluginApi {
  return {
...
describe("handleWorkboardCommand", () => {
  ...
  it("attests the default agent for an unassigned slash-command card", async () => {
    ...
    let command: SteelEnginePluginCommandDefinition | undefined;
    const api = {
      registerCommand: vi.fn((definition: SteelEnginePluginCommandDefinition) => {
        command = definition;
      }),
      runtime: {
        subagent: { run },
