...
import { createSendCfgThreadingRuntime } from "recall/plugin-sdk/channel-test-helpers";
...
vi.mock("recall/plugin-sdk/plugin-config-runtime", async () => {
  const original = (await vi.importActual("recall/plugin-sdk/plugin-config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("recall/plugin-sdk/text-chunking", async () => {
  const original = (await vi.importActual("recall/plugin-sdk/text-chunking")) as Record<
    string,
    unknown
  >;
  return {
...
afterAll(() => {
  ...
  vi.doUnmock("recall/plugin-sdk/plugin-config-runtime");
  vi.doUnmock("recall/plugin-sdk/text-chunking");
  vi.resetModules();
});
