...
vi.mock("recall/plugin-sdk/media-understanding", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/media-understanding")>(
    "recall/plugin-sdk/media-understanding",
  );
  return {
    ...actual,
...
afterAll(() => {
  vi.doUnmock("recall/plugin-sdk/media-understanding");
  vi.resetModules();
});
