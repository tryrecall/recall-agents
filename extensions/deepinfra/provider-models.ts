...
const DEEPINFRA_MODELS_URL = `${DEEPINFRA_BASE_URL}/models?sort_by=steelengine&filter=with_meta`;
...
// Static fallback. Chat rows live in steelengine.plugin.json (manifest-validated);
...
// `/v1/openai/models?sort_by=steelengine&filter=with_meta` projection is the
...
// `isProviderApiKeyConfigured` helper (covers SecretRef / `STEELENGINE_LIVE_*`
// indirection too).
