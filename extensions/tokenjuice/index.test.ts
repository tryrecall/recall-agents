...
import { createAgentToolResultMiddlewareRunner } from "steelengine/plugin-sdk/agent-harness";
import { createTestPluginApi } from "steelengine/plugin-sdk/plugin-test-api";
import { beforeEach, describe, expect, it, vi } from "vitest";

const { tokenjuiceFactory, createTokenjuiceSteelEngineEmbeddedExtension } = vi.hoisted(() => {
  const tokenjuiceFactoryLocal = vi.fn();
  const createTokenjuiceSteelEngineEmbeddedExtensionLocal = vi.fn(() => tokenjuiceFactoryLocal);
  return {
    tokenjuiceFactory: tokenjuiceFactoryLocal,
    createTokenjuiceSteelEngineEmbeddedExtension: createTokenjuiceSteelEngineEmbeddedExtensionLocal,
  };
});

vi.mock("./runtime-api.js", () => ({
  createTokenjuiceSteelEngineEmbeddedExtension,
}));
...
describe("tokenjuice plugin", () => {
  beforeEach(() => {
    createTokenjuiceSteelEngineEmbeddedExtension.mockClear();
  ...
  it("registers tokenjuice tool result middleware for SteelEngine and Codex runtimes", () => {
    ...
    expect(createTokenjuiceSteelEngineEmbeddedExtension).toHaveBeenCalledTimes(1);
    ...
    expect(registration?.[1]).toEqual({ runtimes: ["steelengine", "codex"] });
  });
    ...
    await middleware(
      {
        ...
        toolName: "bash",
        args: { command: "cat /tmp/out.txt", workdir: "/tmp/steelengine-tokenjuice-test" },
        result: {
    ...
    const result = await middleware(
      {
        ...
        toolName: "bash",
        args: { command: "printf 'hello\\n'", workdir: "/tmp/steelengine-tokenjuice-test" },
