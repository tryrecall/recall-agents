...
import type { RecallPluginApi, RecallPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<RecallPluginApi> = {}): RecallPluginApi {
...
function fakeCtx(overrides: Partial<RecallPluginToolContext> = {}): RecallPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", () => {
    const api = fakeApi();
    const factoryTool = (ctx: RecallPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
