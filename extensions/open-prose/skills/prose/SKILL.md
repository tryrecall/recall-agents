...
metadata: { "steelengine": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == SteelEngine `sessions_spawn`
- **File I/O** == SteelEngine `read`/`write`
- **Remote fetch** == SteelEngine `web_fetch` (or `exec` with curl when POST is required)
