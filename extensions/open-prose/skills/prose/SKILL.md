...
metadata: { "recall": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Recall `sessions_spawn`
- **File I/O** == Recall `read`/`write`
- **Remote fetch** == Recall `web_fetch` (or `exec` with curl when POST is required)
