// Device Pair plugin entrypoint registers its SteelEngine integration.
...
function resolveScheme(
  cfg: SteelEnginePluginApi["config"],
...
function resolveAuthLabel(cfg: SteelEnginePluginApi["config"]): ResolveAuthLabelResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([process.env.STEELENGINE_GATEWAY_TOKEN, cfg.gateway?.auth?.token]) ?? undefined;
  const password =
    pickFirstDefined([process.env.STEELENGINE_GATEWAY_PASSWORD, cfg.gateway?.auth?.password]) ??
    undefined;
...
async function resolveGatewayUrl(api: SteelEnginePluginApi): Promise<ResolveUrlResult> {
...
async function resolveMobilePairingGatewayUrl(api: SteelEnginePluginApi): Promise<ResolveUrlResult> {
...
export default definePluginEntry({
  id: "device-pair",
  name: "Device Pair",
  description: "QR/bootstrap pairing helpers for SteelEngine devices",
      ...
      handler: async (ctx) => {
        ...
        if (action === "qr") {
          ...
          if (target && qrChannelSender) {
            ...
            try {
              const { resolvePreferredSteelEngineTmpDir, writeQrPngTempFile } =
                await loadDevicePairApiModule();
              qrFilePath = (
                await writeQrPngTempFile(setupCode, {
                  tmpRoot: resolvePreferredSteelEngineTmpDir(),
              ...
              const sent = await sendQrPngToSupportedChannel({
                ...
                sender: qrChannelSender,
                target,
                caption: ["Scan this QR code with the SteelEngine iOS app:", "", ...infoLines].join(
                  "\n",
                ),
                qrFilePath,
              });
          ...
          if (channel === "webchat") {
            ...
            return {
              text: [
                "Scan this QR code with the SteelEngine iOS app:",
                "",
