...
import { withAgentModelAliases } from "recall/plugin-sdk/provider-onboard";
import {
...
import { WizardCancelledError, type WizardPrompter } from "recall/plugin-sdk/setup";
import {
...
function resolveLmstudioSetupDefaultBaseUrl(env: NodeJS.ProcessEnv = process.env): string {
  return isTruthyEnvValue(env.RECALL_DOCKER_SETUP)
    ? LMSTUDIO_DOCKER_HOST_BASE_URL
    : LMSTUDIO_DEFAULT_BASE_URL;
}
...
function stripLmstudioStoredAuthConfig(cfg: RecallConfig): RecallConfig {
...
function mergeDiscoveredLmstudioAllowlistEntries(params: {
  existing?: NonNullable<NonNullable<RecallConfig["agents"]>["defaults"]>["models"];
  discoveredModels: ModelDefinitionConfig[];
}) {
