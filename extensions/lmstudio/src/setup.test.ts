import { CUSTOM_LOCAL_AUTH_MARKER } from "recall/plugin-sdk/provider-auth";
...
import { resolveAgentModelPrimaryValue } from "recall/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "recall/plugin-sdk/provider-setup";
import type { WizardPrompter } from "recall/plugin-sdk/setup";
...
vi.mock("recall/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("recall/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("recall/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("recall/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("recall/plugin-sdk/provider-auth");
  vi.doUnmock("recall/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
function buildConfig(): RecallConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
function buildDiscoveryContext(params?: {
  ...
  return {
    config: params?.config ?? ({} as RecallConfig),
    env: params?.env ?? {},
...
describe("lmstudio setup", () => {
  ...
  it("non-interactive setup fails when requested model is missing", async () => {
    ...
    const dockerSetup = ["1", "true", "yes", "on"].includes(
      process.env.RECALL_DOCKER_SETUP?.trim().toLowerCase() ?? "",
    );
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("RECALL_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([
