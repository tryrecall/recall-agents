...
vi.mock("steelengine/plugin-sdk/sandbox", async (importOriginal) => {
  const actual = await importOriginal<typeof import("steelengine/plugin-sdk/sandbox")>();
  return {
    ...actual,
...
describe("openshell backend exec workdir validation", () => {
  beforeEach(() => {
    ...
    sdkMocks.runSshSandboxCommand.mockImplementation(async ({ remoteCommand }) => ({
      stdout: String(remoteCommand).includes("steelengine-validate-workdir")
