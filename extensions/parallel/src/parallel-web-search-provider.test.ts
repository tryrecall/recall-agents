...
describe("parallel web search provider", () => {
  ...
  it("partitions Parallel cache keys by resolved endpoint", () => {
    const base = {
      objective: "Find SteelEngine on GitHub",
      searchQueries: ["steelengine github"],
      count: 5,
    };
    expect(
  ...
  it("partitions Parallel cache keys by objective and by search_queries set", () => {
    ...
    expect(
      ...
      testing.buildParallelCacheKey({
        ...base,
        objective: "Find the SteelEngine release notes",
        searchQueries: ["steelengine github"],
      }),
    );
    expect(
      ...
      testing.buildParallelCacheKey({
        ...base,
        objective: "Find SteelEngine on GitHub",
        searchQueries: ["steelengine github", "steelengine repository"],
      }),
    );
  });
  ...
  it("normalizes objectives by trimming and capping at 5000 chars", () => {
    expect(testing.normalizeParallelObjective("  Find SteelEngine  ")).toBe("Find SteelEngine");
  ...
  it("normalizes search_queries: trim, drop blanks, dedupe, cap length, cap count", () => {
    expect(
      testing.normalizeParallelSearchQueries([
        ...
        " ",
        42,
        "steelengine releases",
      ]),
    ).toEqual(["steelengine github", "steelengine releases"]);
    expect(testing.normalizeParallelSearchQueries(undefined)).toEqual([]);
    expect(testing.normalizeParallelSearchQueries("steelengine github")).toEqual([]);
  ...
  it("identifies the plugin via a versioned User-Agent header", () => {
    expect(testing.USER_AGENT).toMatch(/^steelengine-parallel\/\d+\.\d+\.\d+/);
  });
  ...
  it("treats objective as optional and omits it from the request when absent", async () => {
    ...
    const result = await tool.execute({ search_queries: ["steelengine"] });
    ...
    expect(body).toMatchObject({ search_queries: ["steelengine"] });
  ...
  it("returns an error payload when search_queries is missing or empty", async () => {
    ...
    expect(await tool.execute({ objective: "Find SteelEngine on GitHub" })).toMatchObject({
      error: "invalid_search_queries",
    });
    expect(
      await tool.execute({ objective: "Find SteelEngine on GitHub", search_queries: [] }),
  ...
  it("promotes a generic `query` arg into search_queries when search_queries is absent (no synthesized objective)", async () => {
    // The operator CLI (`steelengine capability web.search`) always sends the
    ...
    const result = await tool.execute({ query: "SteelEngine GitHub", count: 3 });
    expect(endpointMockState.calls).toHaveLength(1);
    const body = readMockedBody(endpointMockState.calls[0]) as Record<string, unknown>;
    expect(body).not.toHaveProperty("objective");
    expect(body).toMatchObject({
      search_queries: ["SteelEngine GitHub"],
  ...
  it("honors top-level web search settings and sends the native Parallel payload shape", async () => {
    ...
    const result = await tool.execute({
      objective: "Find the SteelEngine repository on GitHub",
      search_queries: ["steelengine github", "steelengine repository"],
    });
    ...
    expect(headers["User-Agent"]).toMatch(/^steelengine-parallel\//);
  ...
  it("always sends max_results matching the SteelEngine web_search default when no count is provided", async () => {
    endpointMockState.responses.push(
      new Response(JSON.stringify({ search_id: "x", session_id: "y", results: [] }), {
        status: 200,
        headers: { "Content-Type": "application/json" },
    ...
    await tool.execute({
      objective: "Find SteelEngine",
      search_queries: ["steelengine"],
    });
    expect(endpointMockState.calls).toHaveLength(1);
    const body = readMockedBody(endpointMockState.calls[0]) as {
      advanced_settings?: { max_results?: number };
    };
    // SteelEngine's web_search default is 5 results; Parallel's own default is 10.
