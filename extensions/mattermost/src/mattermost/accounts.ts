...
import type { BlockStreamingCoalesceConfig } from "steelengine/plugin-sdk/config-contracts";
...
export function listMattermostAccountIds(cfg: SteelEngineConfig): string[] {
  return mattermostAccountHelpers.listAccountIds(cfg);
}

export function resolveDefaultMattermostAccountId(cfg: SteelEngineConfig): string {
