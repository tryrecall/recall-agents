...
function buildButtonAttachmentsForTest(params: ButtonPropsInput): ButtonAttachments {
  const signedChannelId = params.buttons[0]?.context?.["__steelengine_channel_id"];
  ...
  it("uses interactions.callbackBaseUrl when configured", () => {
    const url = resolveInteractionCallbackUrl("default", {
      channels: {
        mattermost: {
          interactions: {
            callbackBaseUrl: "https://gateway.example.com/steelengine",
          },
        },
      },
    });
    expect(url).toBe("https://gateway.example.com/steelengine/mattermost/interactions/default");
  });
...
describe("createMattermostInteractionHandler", () => {
  ...
  function createActionContext(actionId = "approve", channelId = "chan-1") {
    const context = { action_id: actionId, __steelengine_channel_id: channelId };
