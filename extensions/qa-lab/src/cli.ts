...
export function registerQaLabCli(program: Command) {
  ...
  qa.command("suite")
    ...
    .option("--runtime-pair <pair>", "Run each scenario under both runtimes, e.g. steelengine,codex")
    .option(
  ...
  qa.command("jsonl-replay")
    ...
    .option("--runtime-pair <pair>", "Runtime pair label, e.g. steelengine,codex", "steelengine,codex")
    .option(
      "--provider-mode <mode>",
  ...
  credentials
    ...
    .option("--site-url <url>", "Override STEELENGINE_QA_CONVEX_SITE_URL")
    .option("--endpoint-prefix <path>", "Override STEELENGINE_QA_CONVEX_ENDPOINT_PREFIX")
  ...
  qa.command("docker-scaffold")
    ...
    .option("--image <name>", "Prebaked image name", "steelengine:qa-local-prebaked")
  ...
  qa.command("docker-build-image")
    ...
    .option("--repo-root <path>", "Repository root to target when running from a neutral cwd")
    .option("--image <name>", "Image tag", "steelengine:qa-local-prebaked")
