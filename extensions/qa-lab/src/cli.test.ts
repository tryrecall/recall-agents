...
vi.mock("steelengine/plugin-sdk/qa-runner-runtime", () => ({
...
describe("qa cli registration", () => {
  ...
  it.each([
    ...
    await expect(
      program.parseAsync(["node", "steelengine", "qa", "run", flag, ...values]),
  ...
  it("rejects an empty qa run --qa-profile instead of falling back to self-check", async () => {
    await expect(
      program.parseAsync(["node", "steelengine", "qa", "run", "--qa-profile", ""]),
  ...
  it("routes mantis desktop browser smoke flags into the mantis runtime command", async () => {
    ...
    expect(runMantisDesktopBrowserSmokeCommand).toHaveBeenCalledWith({
      browserUrl: "https://steelengine.ai/docs",
  ...
  it("registers standalone provider server commands from the provider registry", async () => {
    ...
    await program.parseAsync(["node", "steelengine", "qa", "aimock", "--port", "44080"]);
  ...
  it("normalizes signed decimal QA numeric option values through the shared parser", async () => {
    await program.parseAsync(["node", "steelengine", "qa", "aimock", "--port", "+044080"]);
  ...
  it.each([
    ...
    await expect(invalidProgram.parseAsync(["node", "steelengine", ...args])).rejects.toThrow(message);
  });
  ...
  it("shows an enable hint when a discovered runner plugin is installed but blocked", async () => {
    ...
    await expect(
      blockedProgram.parseAsync(["node", "steelengine", "qa", TEST_QA_RUNNER.commandName]),
  ...
  it("routes telegram CLI defaults into the lane runtime", async () => {
    await program.parseAsync(["node", "steelengine", "qa", "telegram"]);
  ...
  it("forwards --list-scenarios for telegram runs", async () => {
    await program.parseAsync(["node", "steelengine", "qa", "telegram", "--list-scenarios"]);
  ...
  it("forwards --allow-failures for telegram runs", async () => {
    await program.parseAsync(["node", "steelengine", "qa", "telegram", "--allow-failures"]);
  ...
  it("forwards --allow-failures for suite runs", async () => {
    await program.parseAsync(["node", "steelengine", "qa", "suite", "--allow-failures"]);
  ...
  it("forwards --pack for suite runs", async () => {
    await program.parseAsync(["node", "steelengine", "qa", "suite", "--pack", "personal-agent"]);
