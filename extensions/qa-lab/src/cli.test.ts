...
import type { QaRunnerCliContribution } from "recall/plugin-sdk/qa-runner-runtime";
...
vi.mock("recall/plugin-sdk/qa-runner-runtime", () => ({
...
describe("qa cli registration", () => {
  ...
  it("routes mantis desktop browser smoke flags into the mantis runtime command", async () => {
    ...
    expect(runMantisDesktopBrowserSmokeCommand).toHaveBeenCalledWith({
      browserUrl: "https://recall.ai/docs",
  ...
  it("registers standalone provider server commands from the provider registry", async () => {
    ...
    await program.parseAsync(["node", "recall", "qa", "aimock", "--port", "44080"]);
  ...
  it("shows an enable hint when a discovered runner plugin is installed but blocked", async () => {
    ...
    await expect(
      blockedProgram.parseAsync(["node", "recall", "qa", TEST_QA_RUNNER.commandName]),
  ...
  it("routes telegram CLI defaults into the lane runtime", async () => {
    await program.parseAsync(["node", "recall", "qa", "telegram"]);
  ...
  it("forwards --list-scenarios for telegram runs", async () => {
    await program.parseAsync(["node", "recall", "qa", "telegram", "--list-scenarios"]);
  ...
  it("forwards --allow-failures for telegram runs", async () => {
    await program.parseAsync(["node", "recall", "qa", "telegram", "--allow-failures"]);
  ...
  it("forwards --allow-failures for suite runs", async () => {
    await program.parseAsync(["node", "recall", "qa", "suite", "--allow-failures"]);
  ...
  it("forwards --pack for suite runs", async () => {
    await program.parseAsync(["node", "recall", "qa", "suite", "--pack", "personal-agent"]);
