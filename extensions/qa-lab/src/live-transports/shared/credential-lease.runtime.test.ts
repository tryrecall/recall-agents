...
describe("credential lease runtime", () => {
  ...
  it("acquires, heartbeats, and releases convex credentials", async () => {
    ...
    const lease = await acquireQaCredentialLease({
      ...
      role: "maintainer",
      env: {
        STEELENGINE_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
        STEELENGINE_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
      },
      fetchImpl,
    ...
    try {
      await expect(
        acquireQaCredentialLease({
          ...
          role: "maintainer",
          env: {
            STEELENGINE_QA_CONVEX_SITE_URL: broker.url,
            STEELENGINE_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
    ...
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          ...
          payload: {
            __steelengineQaCredentialPayloadChunksV1: true,
    ...
    await expect(
      acquireQaCredentialLease({
        ...
        env: {
          STEELENGINE_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
          STEELENGINE_QA_CONVEX_SECRET_CI: "ci-secret",
          STEELENGINE_QA_CREDENTIAL_PAYLOAD_MAX_CHUNKS: "2",
        },
        fetchImpl,
        resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
        parsePayload: (payload) =>
    ...
    await expect(
      acquireQaCredentialLease({
        ...
        env: {
          STEELENGINE_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
          STEELENGINE_QA_CONVEX_SECRET_CI: "ci-secret",
          STEELENGINE_QA_CREDENTIAL_PAYLOAD_MAX_BYTES: "32",
        },
        fetchImpl,
        resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
        parsePayload: (payload) =>
  ...
  it("retries convex acquire while the pool is exhausted", async () => {
    ...
    const lease = await acquireQaCredentialLease({
      ...
      env: {
        STEELENGINE_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
        STEELENGINE_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
        STEELENGINE_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS: "90000",
      },
      fetchImpl,
      randomImpl: () => 0,
      timeImpl: () => nowMs,
  ...
  it("retries transient convex acquire transport failures", async () => {
    ...
    const lease = await acquireQaCredentialLease({
      ...
      env: {
        STEELENGINE_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
        STEELENGINE_QA_CONVEX_SECRET_MAINTAINER: "test",
        STEELENGINE_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS: "90000",
      },
      fetchImpl,
      randomImpl: () => 0,
  ...
  it("rejects non-https convex site URLs unless local insecure opt-in is enabled", async () => {
    await expect(
      acquireQaCredentialLease({
        ...
        source: "convex",
        env: {
          STEELENGINE_QA_CONVEX_SITE_URL: "http://qa-cred.example.convex.site",
          STEELENGINE_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
        },
        resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
        parsePayload: (payload) =>
  ...
  it("allows loopback http URLs when STEELENGINE_QA_ALLOW_INSECURE_HTTP is enabled", async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValueOnce(
      jsonResponse({
        status: "ok",
    ...
    await acquireQaCredentialLease({
      ...
      role: "maintainer",
      env: {
        STEELENGINE_QA_CONVEX_SITE_URL: "http://127.0.0.1:3210",
        STEELENGINE_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
        STEELENGINE_QA_ALLOW_INSECURE_HTTP: "1",
      },
      fetchImpl,
    ...
    await expect(
      acquireQaCredentialLease({
        ...
        env: {
          STEELENGINE_QA_CONVEX_SITE_URL: "https://qa-cred.example.convex.site",
          STEELENGINE_QA_CONVEX_SECRET_MAINTAINER: "maintainer-secret",
          STEELENGINE_QA_CONVEX_ENDPOINT_PREFIX: "//evil.example",
        },
        resolveEnvPayload: () => ({ groupId: "-1", driverToken: "unused", sutToken: "unused" }),
        parsePayload: (payload) =>
          payload as { groupId: string; driverToken: string; sutToken: string },
      }),
    ).rejects.toThrow("STEELENGINE_QA_CONVEX_ENDPOINT_PREFIX must be an absolute path");
  });
  ...
  it("fails convex mode when auth secret is missing", async () => {
    ...
    ).rejects.toThrow("STEELENGINE_QA_CONVEX_SECRET_MAINTAINER");
  });
