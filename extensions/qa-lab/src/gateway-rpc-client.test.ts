...
vi.mock("recall/plugin-sdk/gateway-runtime", () => ({
  ...
  it("calls the in-process gateway cli helper without mutating process.env", async () => {
    ...
    try {
      ...
      if (originalHome === undefined) {
        ...
        process.env.RECALL_HOME = originalHome;
      }
    }

    expect(process.env.RECALL_HOME).toBe(originalHome);
  });
