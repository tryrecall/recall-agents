...
const QA_LAB_UI_OVERLAY_DIR = "/opt/recall-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        RECALL_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${params.bindUiDist ? `      - ${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro\n` : ""}    healthcheck:
    ...
    command:
      - sh
      - -lc
      - RECALL_QA_CONTROL_UI_PROXY_TOKEN="$(node -e 'const fs=require("node:fs");const cfg=JSON.parse(fs.readFileSync("/opt/recall-scaffold/recall.json","utf8"));process.stdout.write(cfg.gateway?.auth?.token ?? "")')" exec node dist/index.js qa ui --host 0.0.0.0 --port ${QA_LAB_INTERNAL_PORT} --advertise-host 127.0.0.1 --advertise-port ${params.qaLabPort} --control-ui-url http://127.0.0.1:${params.gatewayPort}/ --control-ui-proxy-target http://recall-qa-gateway:18789/${params.bindUiDist ? ` --ui-dist-dir ${QA_LAB_UI_OVERLAY_DIR}` : ""} --auto-kickoff-target direct --send-kickoff-on-start --embedded-gateway disabled
    depends_on:
      qa-mock-openai:
...
${imageBlock}    pull_policy: never
    ...
    environment:
      RECALL_CONFIG_PATH: /tmp/tryrecall/recall-agents.json
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/recall/workspace /tmp/recall/state && cp /opt/recall-scaffold/recall.json /tmp/tryrecall/recall-agents.json && cp -R /opt/recall-scaffold/seed-workspace/. /tmp/recall/workspace/ && ln -snf /opt/recall-repo /tmp/recall/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t recall:qa-local-prebaked --build-arg RECALL_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "recall:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "recall.json"),
  ];
