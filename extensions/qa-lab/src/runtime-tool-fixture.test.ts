...
describe("runtime tool fixture", () => {
  ...
  it("does not fail Codex-native fixtures solely because SteelEngine dynamic exposure is absent", async () => {
    const env = await makeEnv({
      mock: { baseUrl: "http://127.0.0.1:9999" },
      gateway: {
        baseUrl: "http://127.0.0.1:1",
        tempRoot: "",
        workspaceDir: "",
        runtimeEnv: { STEELENGINE_QA_FORCE_RUNTIME: "codex" },
        call: vi.fn(),
      },
    });
    ...
    expect(details).toContain("SteelEngine dynamic exposure is intentionally omitted");
  ...
  it.each([
    ...
    },
    {
      ...
      toolName: "web_search",
      happyArgs: { query: "SteelEngine runtime parity fixed query" },
      happyOutput: "result",
  ...
  it("still fails required SteelEngine dynamic fixtures when the tool is absent", async () => {
    const env = await makeEnv();
