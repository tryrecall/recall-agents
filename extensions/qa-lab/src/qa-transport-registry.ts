...
async function createBuiltInQaTransport(
  ...
  if (context.driver === "crabline") {
    const { resolveSteelEngineCrablineChannelDriverSelection } = await import("@openclaw/crabline");
    const selection = resolveSteelEngineCrablineChannelDriverSelection({ channel: context.channelId });
