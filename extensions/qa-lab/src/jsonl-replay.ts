...
type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { steelengine: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { steelengine: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      steelengine: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.steelengine.push(parity.cells.steelengine);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# SteelEngine JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
