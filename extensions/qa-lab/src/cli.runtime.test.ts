...
describe("qa cli runtime", () => {
  ...
  beforeEach(async () => {
    ...
    writeQaDockerHarnessFiles.mockResolvedValue({
      outputDir: "/tmp/steelengine-repo/.artifacts/qa-docker",
    });
  ...
  it("dispatches a taxonomy-backed profile category through the suite runner", async () => {
    ...
    process.env.STEELENGINE_QA_PROFILE = "release";
    try {
      runQaSuite.mockImplementationOnce(async () => {
        expect(process.env.STEELENGINE_QA_PROFILE).toBe("smoke-ci");
        await fs.writeFile(
          suiteEvidencePath,
          JSON.stringify(
            makeQaEvidence([
      ...
      expectFields(suiteArgs, {
        repoRoot: path.resolve("/tmp/steelengine-repo"),
        outputDir: path.resolve("/tmp/steelengine-repo", ".artifacts/qa-e2e/smoke-ci"),
        transportId: "qa-channel",
      ...
      expect(process.env.STEELENGINE_QA_PROFILE).toBe("release");
  ...
  it("resolves suite repo-root-relative paths before dispatching", async () => {
    ...
    expect(runQaSuite).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/steelengine-repo"),
      outputDir: path.resolve("/tmp/steelengine-repo", ".artifacts/qa/frontier"),
      transportId: "qa-channel",
      channelDriver: undefined,
      channelDriverSelection: undefined,
      providerMode: "live-frontier",
  ...
  it("uses the Crabline default channel when selected scenarios do not request one", async () => {
    ...
    expect(runQaSuite).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/steelengine-repo"),
      outputDir: path.resolve("/tmp/steelengine-repo", ".artifacts/qa/multipass-telegram"),
      transportId: "qa-channel",
      channelDriver: "crabline",
  ...
  it("rejects unknown runtime-pair ids at the CLI boundary", async () => {
    ...
    ).rejects.toThrow('--runtime-pair only supports "steelengine" and "codex".');
    expect(runQaSuite).not.toHaveBeenCalled();
  });
  ...
  it("resolves telegram qa repo-root-relative paths before dispatching", async () => {
    ...
    expect(runQaFlowSuiteFromRuntime).toHaveBeenCalledWith(
      expect.objectContaining({
        repoRoot: path.resolve("/tmp/steelengine-repo"),
        outputDir: path.resolve("/tmp/steelengine-repo", ".artifacts/qa/telegram"),
        providerMode: "live-frontier",
        primaryModel: "openai/gpt-5.6-luna",
        alternateModel: "openai/gpt-5.6-luna",
        fastMode: true,
  ...
  it("rejects output dirs that escape the repo root", () => {
    expect(() => resolveRepoRelativeOutputDir("/tmp/steelengine-repo", "../outside")).toThrow(
      "--output-dir must stay within the repo root.",
    );
    expect(() => resolveRepoRelativeOutputDir("/tmp/steelengine-repo", "/tmp/outside")).toThrow(
  ...
  it("uses the trusted Telegram launcher for the shared suite gateway", async () => {
    ...
    const launcherPath = path.join(telegramArtifactsDir, "steelengine-telegram-sut-launcher");
    const runtimeRoot = path.join(telegramArtifactsDir, "runtime");
    const runtimeTempParent = path.join(runtimeRoot, "tmp");
    const preloadPath = path.join(runtimeRoot, "steelengine-telegram-preentry.mjs");
    ...
    vi.stubEnv("STEELENGINE_QA_TELEGRAM_SUT_FORWARDED_ENV_KEYS", "HOME,PATH");
    vi.stubEnv("STEELENGINE_QA_TELEGRAM_SUT_CLEANUP_TIMEOUT_MS", "60000");
    vi.stubEnv("STEELENGINE_QA_TELEGRAM_SUT_GID", "1002");
    vi.stubEnv("STEELENGINE_QA_TELEGRAM_SUT_STEELENGINE_COMMAND", launcherPath);
    vi.stubEnv("STEELENGINE_QA_TELEGRAM_SUT_PRELOAD_PATH", preloadPath);
    vi.stubEnv("STEELENGINE_QA_TELEGRAM_SUT_PROCESS_BOUNDARY_DIR", boundaryDir);
    vi.stubEnv("STEELENGINE_QA_TELEGRAM_SUT_RUNTIME_EXECUTABLE", process.execPath);
    vi.stubEnv("STEELENGINE_QA_TELEGRAM_SUT_UID", "1001");
    await runQaTelegramCommand({
    ...
    expect(runQaFlowSuiteFromRuntime).toHaveBeenCalledWith(
      expect.objectContaining({ sutSteelEngineCommand }),
    );
  });

  it("rejects relative Telegram launcher paths before starting a gateway", async () => {
    vi.stubEnv("STEELENGINE_QA_TELEGRAM_SUT_STEELENGINE_COMMAND", "relative-launcher");
    await expect(
      runQaTelegramCommand({
        repoRoot: "/tmp/steelengine-repo",
        scenarioIds: ["telegram-help-command"],
      }),
    ).rejects.toThrow("STEELENGINE_QA_TELEGRAM_SUT_STEELENGINE_COMMAND must be an absolute file path.");
  ...
  it("rejects non-executable Telegram launcher files before starting a gateway", async () => {
    ...
    await expect(
      ...
      `STEELENGINE_QA_TELEGRAM_SUT_STEELENGINE_COMMAND must point to an executable regular file: ${launcherPath}`,
    );
  ...
  it("prints telegram scenario catalog without resolving the SUT launcher", async () => {
    ...
    expectWriteContains(
      stdoutWrite,
      "telegram-status-command\tdefault\tTelegram status command reply\tVerify Telegram status returns model, session, and activation details. refs=steelengine/steelengine#74698",
    );
  });
  ...
  it("prints a markdown tool coverage report from runtime tool fixtures", async () => {
    ...
    expectWriteContains(stdoutWrite, "# SteelEngine Runtime Tool Coverage");
  ...
  it("writes a curated mock JSONL replay report and summary", async () => {
    ...
    try {
      ...
      expect(report).toContain("# SteelEngine JSONL Replay Report - steelengine vs codex");
    ...
    try {
      await fs.writeFile(
        ...
        JSON.stringify({
          ...
          run: { runtimePair: ["steelengine", "codex"] },
        }),
        "utf8",
      );
  ...
  it("resolves character eval paths and passes model refs through", async () => {
    ...
    expectFields(characterEvalArgs, {
      repoRoot: path.resolve("/tmp/steelengine-repo"),
      outputDir: path.resolve("/tmp/steelengine-repo", ".artifacts/qa/character"),
  ...
  it("routes suite runs through multipass when the runner is selected", async () => {
    ...
    expect(runQaMultipass).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/steelengine-repo"),
      outputDir: path.resolve("/tmp/steelengine-repo", ".artifacts/qa-multipass"),
      transportId: "qa-channel",
      providerMode: "mock-openai",
      primaryModel: undefined,
      alternateModel: undefined,
  ...
  it("resolves self-check repo-root-relative paths before starting the lab server", async () => {
    ...
    expect(startQaLabServer).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/steelengine-repo"),
      outputPath: path.resolve("/tmp/steelengine-repo", ".artifacts/qa/self-check.md"),
    });
  });
  ...
  it("rejects oversized credential payload files before broker setup", async () => {
    const previousMaxBytes = process.env.STEELENGINE_QA_CREDENTIAL_PAYLOAD_MAX_BYTES;
    ...
    process.env.STEELENGINE_QA_CREDENTIAL_PAYLOAD_MAX_BYTES = "32";
    ...
    try {
      await expect(
        ...
        "Payload file exceeds STEELENGINE_QA_CREDENTIAL_PAYLOAD_MAX_BYTES (32 bytes).",
      );
    } finally {
      if (previousMaxBytes === undefined) {
        delete process.env.STEELENGINE_QA_CREDENTIAL_PAYLOAD_MAX_BYTES;
      } else {
        process.env.STEELENGINE_QA_CREDENTIAL_PAYLOAD_MAX_BYTES = previousMaxBytes;
      }
    }
  });
  ...
  it("resolves docker scaffold paths relative to the explicit repo root", async () => {
    ...
    expect(writeQaDockerHarnessFiles).toHaveBeenCalledWith({
      outputDir: path.resolve("/tmp/steelengine-repo", ".artifacts/qa-docker"),
      repoRoot: path.resolve("/tmp/steelengine-repo"),
  ...
  it("passes the explicit repo root into docker image builds", async () => {
    await runQaDockerBuildImageCommand({
      repoRoot: "/tmp/steelengine-repo",
      image: "steelengine:qa-local-prebaked",
    });
  ...
  it("resolves docker up paths relative to the explicit repo root", async () => {
    ...
    expect(runQaDockerUp).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/steelengine-repo"),
      outputDir: path.resolve("/tmp/steelengine-repo", ".artifacts/qa-up"),
      gatewayPort: undefined,
      qaLabPort: undefined,
