...
import { appendRegularFile } from "recall/plugin-sdk/security-runtime";
...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/recall-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/recall";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/recall-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `recall-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "RECALL_CONFIG_PATH" &&
          key !== "RECALL_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `RECALL_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `RECALL_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm recall qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredRecallTmpDir(), `${plan.vmName}-qa-suite-`),
  );
