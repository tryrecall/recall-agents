...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    for (const expectedFile of [
      ...
      path.join(outputDir, "state", "recall.json"),
    ...
    expect(compose).toContain("image: recall:qa-local-prebaked");
    ...
    expect(compose).toContain(":/opt/recall-qa-lab-ui:ro");
    expect(compose).toContain("      - sh");
    expect(compose).toContain("      - -lc");
    expect(compose).toContain(
      '        - fetch("http://127.0.0.1:18789/healthz").then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))',
    );
    expect(compose).toContain("--control-ui-proxy-target http://recall-qa-gateway:18789/");
    ...
    expect(compose).toContain("--ui-dist-dir /opt/recall-qa-lab-ui");
    expect(compose).toContain(":/opt/recall-repo:ro");
    expect(compose).toContain("./state:/opt/recall-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/recall-scaffold/seed-workspace/. /tmp/recall/workspace/",
    );
    expect(compose).toContain("RECALL_CONFIG_PATH: /tmp/tryrecall/recall-agents.json");
    expect(compose).toContain("RECALL_STATE_DIR: /tmp/recall/state");
    expect(compose).toContain('RECALL_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("RECALL_GATEWAY_TOKEN=qa-token");
    ...
    const config = await readFile(path.join(outputDir, "state", "recall.json"), "utf8");
    ...
    expect(config).toContain('"/tmp/recall/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    expect(result.imageName).toBe("recall:qa-local-prebaked");
    expect(calls).toEqual([
      "docker build -t recall:qa-local-prebaked --build-arg RECALL_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/recall",
    ]);
  });
});
