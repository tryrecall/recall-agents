...
export const QA_EVIDENCE_SUMMARY_KIND = "steelengine.qa.evidence-summary";
...
export function resolveQaEvidenceProfile(params: {
  ...
  const envProfiles = [
    ["STEELENGINE_E2E_PROFILE", params.env?.STEELENGINE_E2E_PROFILE],
    ["STEELENGINE_QA_PROFILE", params.env?.STEELENGINE_QA_PROFILE],
  ] as const;
...
function resolveQaEvidenceRunner(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  return params.env?.STEELENGINE_QA_RUNNER?.trim() || params.fallback || "host";
}

function resolveQaEvidenceChannelDriver(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  const id =
    params.fallback?.trim() ||
    params.env?.STEELENGINE_QA_CHANNEL_DRIVER?.trim() ||
    params.env?.STEELENGINE_E2E_CHANNEL_DRIVER?.trim();
  return id ? { id } : undefined;
}

function resolveQaEvidencePackageSource(env: NodeJS.ProcessEnv | undefined) {
  const spec = env?.STEELENGINE_QA_PACKAGE_SOURCE?.trim() || undefined;
  const sha = env?.STEELENGINE_QA_PACKAGE_SOURCE_SHA?.trim() || undefined;
  const explicitKind = env?.STEELENGINE_QA_PACKAGE_SOURCE_KIND?.trim();
  const kind =
    explicitKind ||
