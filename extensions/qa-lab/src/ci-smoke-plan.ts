...
const QA_SMOKE_CI_CHANNELS = ["matrix", STEELENGINE_CRABLINE_DEFAULT_CHANNEL] as const;
...
export function createQaSmokeCiPart(partId: string): QaSmokeCiPart {
  ...
  const scenarios = scenarioPack.scenarios.filter(
    (scenario) =>
      ...
      scenarioMatchesQaProviderLane({
        ...
        channel: scenario.execution.channel ?? STEELENGINE_CRABLINE_DEFAULT_CHANNEL,
      }),
  );
  ...
  const unsupportedChannels = new Set(
    scenarios
      .map((scenario) => scenario.execution.channel ?? STEELENGINE_CRABLINE_DEFAULT_CHANNEL)
  ...
  const defaultChannelScenarios = scenarios
    .filter(
      (scenario) =>
        (scenario.execution.channel ?? STEELENGINE_CRABLINE_DEFAULT_CHANNEL) ===
  ...
  const runs: QaSmokeCiRun[] = [
    {
      channel: STEELENGINE_CRABLINE_DEFAULT_CHANNEL,
      slug: "primary",
