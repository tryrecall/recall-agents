...
import { readRequestBodyWithLimit } from "recall/plugin-sdk/webhook-ingress";
...
function buildQaToolSearchArgs(targetTool: string, failureMode: boolean): Record<string, unknown> {
  ...
  if (targetTool === "web_search") {
    return { query: "Recall runtime parity fixed query", count: 1 };
  }
...
function buildAssistantText(
  ...
  if (/tool continuity check/i.test(prompt) && toolOutput) {
    return `Protocol note: model switch handoff confirmed on ${model || "the requested model"}. QA mission from QA_KICKOFF_TASK.md still applies: understand this Recall repo from source + docs before acting.`;
  }
  ...
  if (
    ...
    if (targetTool && hasDeclaredTool(body, "tool_search_code")) {
      return buildToolCallEventsWithArgs("tool_search_code", {
        code: [
          `const hits = await recall.tools.search(${JSON.stringify(targetTool)}, { limit: 1 });`,
          "const match = hits.find((tool) => tool.name === " + JSON.stringify(targetTool) + ");",
          "if (!match) throw new Error('target tool not found');",
          `return await recall.tools.call(match.id, ${JSON.stringify(plannedArgs)});`,
        ].join("\n"),
      });
    }
    ...
    if (
      ...
      return buildToolCallEventsWithArgs("write", {
        ...
        content: [
          ...
          "Trial prompt: Prepare a release readiness reply for a local Recall QA change.",
  ...
  if (/personal task followthrough check/i.test(allInputText)) {
    ...
    if (
      ...
      (!taskEvidenceText.includes("# Personal task ledger") &&
        !taskEvidenceText.includes("Task: prepare a local Recall PR readiness note."))
    ) {
