...
const QA_LIVE_ENV_ALIASES = Object.freeze([
  {
    liveVar: "STEELENGINE_LIVE_OPENAI_KEY",
    providerVar: "OPENAI_API_KEY",
  },
  {
    liveVar: "STEELENGINE_LIVE_ANTHROPIC_KEY",
    providerVar: "ANTHROPIC_API_KEY",
  },
  {
    liveVar: "STEELENGINE_LIVE_GEMINI_KEY",
    providerVar: "GEMINI_API_KEY",
  },
]);

export const QA_LIVE_PROVIDER_CONFIG_PATH_ENV = "STEELENGINE_QA_LIVE_PROVIDER_CONFIG_PATH";
const QA_LIVE_CLI_BACKEND_PRESERVE_ENV = "STEELENGINE_LIVE_CLI_BACKEND_PRESERVE_ENV";
const QA_LIVE_CLI_BACKEND_AUTH_MODE_ENV = "STEELENGINE_LIVE_CLI_BACKEND_AUTH_MODE";
...
export const QA_PROVIDER_SECRET_ENV_KEY_PATTERNS = Object.freeze([
  /^STEELENGINE_LIVE_[A-Z0-9_]+_KEYS?$/u,
]);
...
const QA_LIVE_ALLOWED_ENV_PATTERNS = Object.freeze([
  ...
  /^STEELENGINE_LIVE_[A-Z0-9_]+_KEY$/u,
  /^STEELENGINE_LIVE_[A-Z0-9_]+_KEYS$/u,
]);
...
export function resolveQaLiveCliAuthEnv(
  ...
  const hasAnthropicKey = Boolean(
    baseEnv.ANTHROPIC_API_KEY?.trim() || baseEnv.STEELENGINE_LIVE_ANTHROPIC_KEY?.trim(),
  );
  if (opts?.forwardHostHomeForClaudeCli && authMode === "api-key" && !hasAnthropicKey) {
    throw new Error(
      "Claude CLI API-key QA mode requires ANTHROPIC_API_KEY or STEELENGINE_LIVE_ANTHROPIC_KEY",
    );
  }
...
export function resolveQaLiveProviderConfigPath(env: NodeJS.ProcessEnv = process.env) {
  const explicit =
    env[QA_LIVE_PROVIDER_CONFIG_PATH_ENV]?.trim() || env.STEELENGINE_CONFIG_PATH?.trim();
  return explicit
    ? { path: resolveUserPath(explicit, env), explicit: true }
    : { path: path.join(os.homedir(), ".steelengine", "steelengine.json"), explicit: false };
}
