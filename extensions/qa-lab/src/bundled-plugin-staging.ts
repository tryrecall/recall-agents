...
function findQaBundledPluginDirsByManifestId(params: {
  ...
  for (const sourceRoot of resolveQaBundledPluginScanRoots(params.repoRoot)) {
    for (const entry of readdirSync(sourceRoot, { withFileTypes: true }).toSorted((left, right) =>
      ...
      const manifestId = readQaBundledManifestId(path.join(candidate, "steelengine.plugin.json"));
...
function resolveQaBundledPluginManifestPath(params: {
  ...
  return manifestDir ? path.join(manifestDir, "steelengine.plugin.json") : null;
}
...
export async function resolveQaOwnerPluginIdsForProviderIds(params: {
  ...
  for (const sourceRoot of resolveQaBundledPluginScanRoots(params.repoRoot)) {
    for (const entry of await fs.readdir(sourceRoot, { withFileTypes: true })) {
      ...
      const manifestPath = path.join(sourceRoot, entry.name, "steelengine.plugin.json");
...
export async function resolveQaRuntimeHostVersion(params: {
  ...
  for (const pluginId of stagedPluginIds) {
    ...
    const candidate = parseStableSemverFloor(packageJson.steelengine?.install?.minHostVersion);
...
export async function createQaBundledPluginsDir(params: {
  ...
  const stagedSteelEnginePackageDir = path.join(stagedRoot, "node_modules", "steelengine");
  await fs.mkdir(stagedSteelEnginePackageDir, { recursive: true });
  await fs.copyFile(
    path.join(params.repoRoot, "package.json"),
    path.join(stagedSteelEnginePackageDir, "package.json"),
  );
  ...
  for (const pluginId of stagedPluginIds) {
    ...
    if (manifestPath) {
      await fs.copyFile(manifestPath, path.join(targetDir, "steelengine.plugin.json"));
    }
  }
  await symlinkQaStagedDirEntry({
    sourcePath: path.join(stagedRoot, "dist"),
    targetPath: path.join(stagedSteelEnginePackageDir, "dist"),
    directory: true,
  });
  return {
    bundledPluginsDir,
