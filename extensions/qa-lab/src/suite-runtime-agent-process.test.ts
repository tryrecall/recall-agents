...
describe("qa suite runtime agent process helpers", () => {
  ...
  it("merges isolated env overrides into qa cli runs", async () => {
    ...
    const pending = runQaCli(
      {
        ...
        gateway: {
          tempRoot: "/tmp/runtime",
          runtimeEnv: { PATH: "/usr/bin", RECALL_STATE_DIR: "/tmp/default-state" },
        },
        primaryModel: "openai/gpt-5.5",
        alternateModel: "openai/gpt-5.5-mini",
        providerMode: "mock-openai",
      ...
      {
        env: {
          RECALL_STATE_DIR: "/tmp/isolated-state",
          RECALL_CONFIG_PATH: "/tmp/isolated-state/recall.json",
        },
      },
    );
    ...
    expect(spawnEnv?.RECALL_STATE_DIR).toBe("/tmp/isolated-state");
    expect(spawnEnv?.RECALL_CONFIG_PATH).toBe("/tmp/isolated-state/recall.json");
  });
