...
vi.mock("recall/plugin-sdk/temp-path", () => ({
  resolvePreferredRecallTmpDir: () => qaTempPathState.preferredTmpDir,
}));
...
function createParams(baseEnv?: NodeJS.ProcessEnv) {
  return {
    configPath: "/tmp/recall-qa/recall.json",
    gatewayToken: "qa-token",
    homeDir: "/tmp/recall-qa/home",
    stateDir: "/tmp/recall-qa/state",
    tempRoot: "/tmp/recall-qa",
    xdgConfigHome: "/tmp/recall-qa/xdg-config",
    xdgDataHome: "/tmp/recall-qa/xdg-data",
    xdgCacheHome: "/tmp/recall-qa/xdg-cache",
    bundledPluginsDir: "/tmp/recall-qa/bundled-plugins",
    stagedBundledPluginsRoot: "/repo/.artifacts/qa-runtime/recall-qa-suite-test",
...
describe("buildQaRuntimeEnv", () => {
  ...
  it("keeps the slow-reply QA opt-out enabled under fast mode", () => {
    ...
    expect(env.RECALL_TEST_FAST).toBe("1");
    expect(env.RECALL_QA_PARENT_PID).toBe(String(process.pid));
    expect(env.RECALL_QA_TEMP_ROOT).toBe("/tmp/recall-qa");
    expect(env.RECALL_QA_STAGED_RUNTIME_ROOT).toBe(
      "/repo/.artifacts/qa-runtime/recall-qa-suite-test",
    );
    expect(env.RECALL_QA_ALLOW_LOCAL_IMAGE_PROVIDER).toBe("1");
    expect(env.RECALL_ALLOW_SLOW_REPLY_TESTS).toBe("1");
    expect(env.RECALL_SKIP_STARTUP_MODEL_PREWARM).toBe("1");
    expect(env.RECALL_BUNDLED_PLUGINS_DIR).toBe("/tmp/recall-qa/bundled-plugins");
    expect(env.RECALL_COMPATIBILITY_HOST_VERSION).toBe("2026.4.8");
  });

  it("maps live frontier key aliases into provider env vars", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        RECALL_LIVE_OPENAI_KEY: "openai-live",
        RECALL_LIVE_ANTHROPIC_KEY: "anthropic-live",
        RECALL_LIVE_GEMINI_KEY: "gemini-live",
      }),
  ...
  it("preserves Codex CLI auth home for live frontier runs while sandboxing Recall home", async () => {
    ...
    expect(env.HOME).toBe("/tmp/recall-qa/home");
    expect(env.RECALL_HOME).toBe("/tmp/recall-qa/home");
    expect(env.CODEX_HOME).toBe(codexHome);
  });

  it("forwards host HOME for live Claude CLI runs while keeping Recall home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
    ...
    expect(env.RECALL_HOME).toBe("/tmp/recall-qa/home");
    expect(env.RECALL_STATE_DIR).toBe("/tmp/recall-qa/state");
  });

  it("can forward host HOME for browser-backed QA runs while keeping Recall home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
  ...
  it("preserves the live Anthropic key for live Claude CLI runs without writing it into config", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        HOME: hostHome,
        RECALL_LIVE_ANTHROPIC_KEY: "anthropic-live",
        RECALL_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.RECALL_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP","ANTHROPIC_API_KEY"]');
    expect(env.RECALL_LIVE_CLI_BACKEND_AUTH_MODE).toBe("api-key");
  });
  ...
  it("removes preserved Anthropic keys for live Claude CLI subscription runs", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        ...
        RECALL_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP","ANTHROPIC_API_KEY"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.RECALL_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP"]');
    expect(env.RECALL_LIVE_CLI_BACKEND_AUTH_MODE).toBe("subscription");
  });

  it("does not pass QA setup-token values to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        RECALL_LIVE_SETUP_TOKEN_VALUE: `sk-ant-oat01-${"a".repeat(80)}`,
        RECALL_QA_LIVE_ANTHROPIC_SETUP_TOKEN: `sk-ant-oat01-${"b".repeat(80)}`,
      }),
      providerMode: "live-frontier",
    });

    expect(env.RECALL_LIVE_SETUP_TOKEN_VALUE).toBeUndefined();
    expect(env.RECALL_QA_LIVE_ANTHROPIC_SETUP_TOKEN).toBeUndefined();
  });

  it("does not pass Convex credential broker secrets to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        RECALL_QA_CONVEX_SECRET_CI: "convex-ci-secret",
        RECALL_QA_CONVEX_SECRET_MAINTAINER: "convex-maintainer-secret",
      }),
      providerMode: "live-frontier",
    });

    expect(env.RECALL_QA_CONVEX_SECRET_CI).toBeUndefined();
    expect(env.RECALL_QA_CONVEX_SECRET_MAINTAINER).toBeUndefined();
  });
    ...
    (providerMode) => {
      const env = buildQaRuntimeEnv({
        ...createParams({
          ...
          RECALL_LIVE_ANTHROPIC_KEY: "anthropic-live",
          RECALL_LIVE_ANTHROPIC_KEYS: "anthropic-a,anthropic-b",
          RECALL_LIVE_CODEX_API_KEY: "codex-live",
          RECALL_LIVE_GEMINI_KEY: "gemini-live",
          RECALL_LIVE_OPENAI_KEY: "openai-live",
        }),
        providerMode,
      ...
      expect(env.RECALL_LIVE_OPENAI_KEY).toBeUndefined();
      expect(env.RECALL_LIVE_ANTHROPIC_KEY).toBeUndefined();
      expect(env.RECALL_LIVE_ANTHROPIC_KEYS).toBeUndefined();
      expect(env.RECALL_LIVE_CODEX_API_KEY).toBeUndefined();
      expect(env.RECALL_LIVE_GEMINI_KEY).toBeUndefined();
    },
  );
  ...
  it("stages a live Anthropic setup-token profile for isolated QA workers", async () => {
    ...
    const cfg = await testing.stageQaLiveAnthropicSetupToken({
      ...
      stateDir,
      env: {
        RECALL_LIVE_SETUP_TOKEN_VALUE: token,
      },
    });
  ...
  it("stages the OpenAI API-key fallback for live OpenAI Codex QA workers", async () => {
    ...
    const cfg = await testing.stageQaLiveApiKeyProfiles({
      ...
      env: {
        RECALL_LIVE_OPENAI_KEY: "qa-live-codex-fallback-key",
      },
    });
  ...
  it("stages direct live Codex API-key aliases for isolated QA workers", async () => {
    ...
    const cfg = await testing.stageQaLiveApiKeyProfiles({
      ...
      providerIds: ["openai-codex"],
      env: {
        RECALL_LIVE_CODEX_API_KEY: "qa-live-direct-codex-key",
      },
    });
  ...
  it("fails fast when live OpenAI Codex runs have no portable QA auth", () => {
    expect(() =>
      testing.assertQaLiveCodexAuthAvailable({
        ...
        providerIds: ["openai-codex"],
        env: {
          CODEX_HOME: path.join(os.tmpdir(), "missing-recall-codex-home"),
        },
        readCodexCredentials: () => null,
      }),
  ...
  it("stages configured OpenAI Codex env secret refs for default OpenAI live QA runs", async () => {
    ...
    const env = {
      RECALL_LIVE_CODEX_API_KEY: "qa-configured-env-ref-not-a-real-key",
    };
    const cfg = await testing.stageQaLiveApiKeyProfiles({
      cfg: {
        models: {
  ...
  it("stages configured OpenAI Codex env markers for live QA runs", async () => {
    ...
    const cfg = await testing.stageQaLiveApiKeyProfiles({
        ...
        models: {
          providers: {
            "openai-codex": {
              baseUrl: "",
              models: [],
              apiKey: "RECALL_LIVE_CODEX_API_KEY",
            },
          },
        },
      },
      ...
      providerIds: ["openai-codex"],
      env: {
        RECALL_LIVE_CODEX_API_KEY: "qa-configured-marker-not-a-real-key",
      },
    });
  ...
  it("preserves only sanitized gateway debug artifacts", async () => {
    ...
    await writeFile(
      stdoutLogPath,
      [
        "RECALL_GATEWAY_TOKEN=qa-suite-token",
        'OPENAI_API_KEY="openai-live"',
        "RECALL_QA_CONVEX_SECRET_CI=convex-ci-secret",
        "RECALL_QA_CONVEX_SECRET_MAINTAINER=convex-maintainer-secret",
        "RECALL_LIVE_CODEX_API_KEY=codex-live-secret",
    ...
    await expect(readFile(path.join(artifactDir, "gateway.stdout.log"), "utf8")).resolves.toBe(
      [
        "RECALL_GATEWAY_TOKEN=<redacted>",
        "OPENAI_API_KEY=<redacted>",
        "RECALL_QA_CONVEX_SECRET_CI=<redacted>",
        "RECALL_QA_CONVEX_SECRET_MAINTAINER=<redacted>",
        "RECALL_LIVE_CODEX_API_KEY=<redacted>",
        "botToken=<redacted>",
  ...
  it("rejects preserved gateway artifacts outside the repo root", async () => {
    await expect(
      testing.assertQaArtifactDirWithinRepo("/tmp/recall-repo", "/tmp/outside"),
  ...
  it("cleans startup temp roots when they are not preserved", async () => {
    ...
    await writeFile(path.join(tempRoot, "recall.json"), "{}", "utf8");
...
describe("qa bundled plugin dir", () => {
  ...
  it("resolves bundled plugins by manifest id when the directory name differs", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "kimi-coding", "recall.plugin.json"),
  ...
  it("uses a source bundled plugin when the built copy is missing CLI metadata", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "memory-core", "recall.plugin.json"),
    ...
    await writeFile(
      path.join(repoRoot, "extensions", "memory-core", "recall.plugin.json"),
      JSON.stringify({ id: "memory-core", kind: "memory" }),
      "utf8",
    );
    await writeFile(
  ...
  it("creates a scoped bundled plugin tree for allowed plugins plus always-allowed runtime facades", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "qa-channel", "package.json"),
      JSON.stringify({ name: "@recall/qa-channel", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
  ...
  it("preserves dist-runtime-only root chunks when dist also exists", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "@recall/runtime-only", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
  ...
  it("maps cli backend provider ids to their owning bundled plugin ids", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "openai", "recall.plugin.json"),
      JSON.stringify({
        id: "openai",
  ...
  it("copies selected live provider configs from the host config", async () => {
    ...
    const overrides = await testing.readQaLiveProviderConfigOverrides({
      providerIds: ["custom-openai"],
      env: { RECALL_QA_LIVE_PROVIDER_CONFIG_PATH: configPath },
    });
  ...
  it("raises the QA runtime host version to the highest allowed plugin floor", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ recall: { install: { minHostVersion: ">=2026.4.8" } } }),
      "utf8",
    );
    ...
    await writeFile(
      ...
      JSON.stringify({ recall: { install: { minHostVersion: ">=2026.4.7" } } }),
      "utf8",
    );
  ...
  it("includes always-allowed runtime facade plugins when raising the QA runtime host version", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ recall: { install: { minHostVersion: ">=2026.4.9" } } }),
      "utf8",
    );
