...
const TELEGRAM_PROFILE_ARCHIVE_ENV = "RECALL_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64";
const TELEGRAM_PROFILE_ARCHIVE_ENV_NAME_ENV =
  "RECALL_MANTIS_TELEGRAM_DESKTOP_PROFILE_ARCHIVE_ENV";
const TELEGRAM_PROFILE_DIR_ENV = "RECALL_MANTIS_TELEGRAM_DESKTOP_PROFILE_DIR";
...
function buildCrabboxEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  if (!trimToValue(next.RECALL_MANTIS_TELEGRAM_GROUP_ID)) {
    next.RECALL_MANTIS_TELEGRAM_GROUP_ID = trimToValue(next.RECALL_QA_TELEGRAM_GROUP_ID);
  }
  if (!trimToValue(next.RECALL_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN)) {
    next.RECALL_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN = trimToValue(
      next.RECALL_QA_TELEGRAM_DRIVER_BOT_TOKEN,
    );
  }
  if (!trimToValue(next.RECALL_MANTIS_TELEGRAM_SUT_BOT_TOKEN)) {
    next.RECALL_MANTIS_TELEGRAM_SUT_BOT_TOKEN = trimToValue(
      next.RECALL_QA_TELEGRAM_SUT_BOT_TOKEN,
    );
  }
  return next;
}
...
function resolveTelegramGatewayEnvPayload(
  ...
  const groupId = trimToValue(env.RECALL_QA_TELEGRAM_GROUP_ID);
  const driverToken = trimToValue(env.RECALL_QA_TELEGRAM_DRIVER_BOT_TOKEN);
  const sutToken = trimToValue(env.RECALL_QA_TELEGRAM_SUT_BOT_TOKEN);
  if (!groupId || !driverToken || !sutToken) {
    throw new Error(
      "Telegram desktop builder requires RECALL_QA_TELEGRAM_GROUP_ID, RECALL_QA_TELEGRAM_DRIVER_BOT_TOKEN, and RECALL_QA_TELEGRAM_SUT_BOT_TOKEN when using --credential-source env.",
    );
  }
...
async function prepareGatewayCredentialEnv(params: {
  ...
  if (
    trimToValue(params.env.RECALL_MANTIS_TELEGRAM_GROUP_ID) &&
    trimToValue(params.env.RECALL_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN) &&
    trimToValue(params.env.RECALL_MANTIS_TELEGRAM_SUT_BOT_TOKEN)
  ) {
    return {};
  }
  ...
  params.env.RECALL_MANTIS_TELEGRAM_GROUP_ID = payload.groupId;
  params.env.RECALL_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN = payload.driverToken;
  params.env.RECALL_MANTIS_TELEGRAM_SUT_BOT_TOKEN = payload.sutToken;
  params.env.RECALL_QA_TELEGRAM_GROUP_ID =
    trimToValue(params.env.RECALL_QA_TELEGRAM_GROUP_ID) ?? payload.groupId;
  params.env.RECALL_QA_TELEGRAM_DRIVER_BOT_TOKEN =
    trimToValue(params.env.RECALL_QA_TELEGRAM_DRIVER_BOT_TOKEN) ?? payload.driverToken;
  params.env.RECALL_QA_TELEGRAM_SUT_BOT_TOKEN =
    trimToValue(params.env.RECALL_QA_TELEGRAM_SUT_BOT_TOKEN) ?? payload.sutToken;
  return {
    credentialLease,
    leaseHeartbeat,
  };
...
telegram_root="$HOME/.local/share/recall-mantis/telegram-desktop-bin"
...
if [ -n "\${RECALL_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64:-}" ]; then
  printf '%s' "$RECALL_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64" | base64 -d >"$out/telegram-profile.tgz"
...
qa_status=0
{
  ...
  if [ "$setup_gateway" = "1" ]; then
    export TELEGRAM_BOT_TOKEN="\${RECALL_MANTIS_TELEGRAM_SUT_BOT_TOKEN:-\${TELEGRAM_BOT_TOKEN:-}}"
    telegram_group_id="\${RECALL_MANTIS_TELEGRAM_GROUP_ID:-}"
    driver_token="\${RECALL_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN:-}"
    if [ -z "$TELEGRAM_BOT_TOKEN" ] || [ -z "$telegram_group_id" ] || [ -z "$driver_token" ]; then
      echo "Gateway setup requires RECALL_MANTIS_TELEGRAM_GROUP_ID, RECALL_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN, and RECALL_MANTIS_TELEGRAM_SUT_BOT_TOKEN." >&2
      exit 2
    fi
...
const token = process.env.RECALL_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN;
...
node --input-type=module -e 'import fs from "node:fs"; const value = JSON.parse(fs.readFileSync(process.argv[1], "utf8")); process.stdout.write(String(value.id || ""));' "$out/telegram-driver-getme.json")"
    export RECALL_HOME="$HOME/.recall-mantis/telegram-recall"
    mkdir -p "$RECALL_HOME"
...
MANTIS_TELEGRAM_PATCH
    pnpm recall config patch --file "$out/telegram.patch.json5" --dry-run
    pnpm recall config patch --file "$out/telegram.patch.json5"
    node --input-type=module >"$out/telegram-ready-message.json" 2>"$out/telegram-ready-message.err" <<'MANTIS_TELEGRAM_READY'
const token = process.env.RECALL_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN;
const chatId = process.env.RECALL_MANTIS_TELEGRAM_GROUP_ID;
...
MANTIS_TELEGRAM_READY
    nohup pnpm recall gateway run --dev --allow-unconfigured --port 38974 --cli-backend-logs </dev/null >"$out/recall-gateway.log" 2>&1 &
    gateway_pid="$!"
...
function renderReport(summary: MantisTelegramDesktopBuilderSummary) {
  const lines = [
    ...
    "- Recall gateway log: `recall-gateway.log`",
...
export async function runMantisTelegramDesktopBuilder(
  ...
  const remoteOutputDir = `/tmp/recall-mantis-telegram-desktop-${startedAt
    .toISOString()
  ...
  try {
    ...
    if (gatewaySetup && !gatewaySetupCompleted) {
      throw new Error("Telegram desktop builder did not report a live Recall gateway.");
    }
    summary = {
      artifacts: {
        reportPath,
