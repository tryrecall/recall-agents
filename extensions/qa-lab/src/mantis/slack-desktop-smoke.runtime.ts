...
const CRABBOX_MARKET_ENV = "STEELENGINE_MANTIS_CRABBOX_MARKET";
...
const SLACK_URL_ENV = "STEELENGINE_MANTIS_SLACK_URL";
const SLACK_CHANNEL_ID_ENV = "STEELENGINE_MANTIS_SLACK_CHANNEL_ID";
...
function buildCrabboxEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  if (!trimToValue(next.STEELENGINE_MANTIS_SLACK_BOT_TOKEN) && trimToValue(next.SLACK_BOT_TOKEN)) {
    next.STEELENGINE_MANTIS_SLACK_BOT_TOKEN = next.SLACK_BOT_TOKEN;
  }
  if (
    !trimToValue(next.STEELENGINE_MANTIS_SLACK_BOT_TOKEN) &&
    trimToValue(next.STEELENGINE_QA_SLACK_SUT_BOT_TOKEN)
  ) {
    next.STEELENGINE_MANTIS_SLACK_BOT_TOKEN = next.STEELENGINE_QA_SLACK_SUT_BOT_TOKEN;
  }
  if (!trimToValue(next.STEELENGINE_MANTIS_SLACK_APP_TOKEN) && trimToValue(next.SLACK_APP_TOKEN)) {
    next.STEELENGINE_MANTIS_SLACK_APP_TOKEN = next.SLACK_APP_TOKEN;
  }
  if (
    !trimToValue(next.STEELENGINE_MANTIS_SLACK_APP_TOKEN) &&
    trimToValue(next.STEELENGINE_QA_SLACK_SUT_APP_TOKEN)
  ) {
    next.STEELENGINE_MANTIS_SLACK_APP_TOKEN = next.STEELENGINE_QA_SLACK_SUT_APP_TOKEN;
  }
  if (
    !trimToValue(next.STEELENGINE_MANTIS_SLACK_CHANNEL_ID) &&
    trimToValue(next.STEELENGINE_QA_SLACK_CHANNEL_ID)
  ) {
    next.STEELENGINE_MANTIS_SLACK_CHANNEL_ID = next.STEELENGINE_QA_SLACK_CHANNEL_ID;
  }
  return next;
}

function resolveSlackGatewayEnvPayload(env: NodeJS.ProcessEnv): SlackGatewayCredentialPayload {
  const channelId = trimToValue(env.STEELENGINE_QA_SLACK_CHANNEL_ID);
  const sutBotToken = trimToValue(env.STEELENGINE_QA_SLACK_SUT_BOT_TOKEN);
  const sutAppToken = trimToValue(env.STEELENGINE_QA_SLACK_SUT_APP_TOKEN);
  if (!channelId || !sutBotToken || !sutAppToken) {
    throw new Error(
      "Gateway setup requires STEELENGINE_QA_SLACK_CHANNEL_ID, STEELENGINE_QA_SLACK_SUT_BOT_TOKEN, and STEELENGINE_QA_SLACK_SUT_APP_TOKEN when using --credential-source env.",
    );
  }
  return {
    channelId,
...
async function prepareGatewayCredentialEnv(params: {
  ...
  if (
    trimToValue(params.env.STEELENGINE_MANTIS_SLACK_BOT_TOKEN) &&
    trimToValue(params.env.STEELENGINE_MANTIS_SLACK_APP_TOKEN)
  ) {
    return {};
  }
  ...
  params.env.STEELENGINE_MANTIS_SLACK_BOT_TOKEN = payload.sutBotToken;
  params.env.STEELENGINE_MANTIS_SLACK_APP_TOKEN = payload.sutAppToken;
  params.env.STEELENGINE_MANTIS_SLACK_CHANNEL_ID =
    trimToValue(params.env.STEELENGINE_MANTIS_SLACK_CHANNEL_ID) ?? payload.channelId;
  params.env.STEELENGINE_QA_SLACK_CHANNEL_ID =
    trimToValue(params.env.STEELENGINE_QA_SLACK_CHANNEL_ID) ?? payload.channelId;
  params.env.STEELENGINE_QA_SLACK_SUT_BOT_TOKEN =
    trimToValue(params.env.STEELENGINE_QA_SLACK_SUT_BOT_TOKEN) ?? payload.sutBotToken;
  params.env.STEELENGINE_QA_SLACK_SUT_APP_TOKEN =
    trimToValue(params.env.STEELENGINE_QA_SLACK_SUT_APP_TOKEN) ?? payload.sutAppToken;
  return {
    credentialLease,
    leaseHeartbeat,
  };
...
remote_command_timeout_seconds="\${STEELENGINE_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS:-${remoteCommandTimeoutSeconds}}"
if [ -z "\${STEELENGINE_QA_SLACK_CHANNEL_ID:-}" ] && [ -n "$slack_channel_id" ]; then
  export STEELENGINE_QA_SLACK_CHANNEL_ID="$slack_channel_id"
fi
case "$remote_command_timeout_seconds" in
  ''|*[!0-9]*)
    echo "STEELENGINE_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS must be an integer number of seconds." >&2
    exit 2
    ;;
esac
if [ "$remote_command_timeout_seconds" -le 0 ]; then
  echo "STEELENGINE_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS must be greater than zero." >&2
  exit 2
fi
rm -rf "$out"
mkdir -p "$out"
...
team_id="\${STEELENGINE_QA_SLACK_TEAM_ID:-}"
auth_test_token="\${STEELENGINE_QA_SLACK_SUT_BOT_TOKEN:-\${STEELENGINE_MANTIS_SLACK_BOT_TOKEN:-}}"
...
const token = process.env.STEELENGINE_QA_SLACK_SUT_BOT_TOKEN || process.env.STEELENGINE_MANTIS_SLACK_BOT_TOKEN;
...
if [ -z "$slack_url" ] && [ -n "$team_id" ] && [ -n "\${STEELENGINE_QA_SLACK_CHANNEL_ID:-}" ]; then
  slack_url="https://app.slack.com/client/$team_id/$STEELENGINE_QA_SLACK_CHANNEL_ID"
fi
profile="\${STEELENGINE_MANTIS_SLACK_BROWSER_PROFILE_DIR:-$HOME/.config/steelengine-mantis/slack-chrome-profile}"
mkdir -p "$profile"
if [ "$setup_gateway" = "1" ]; then
  export SLACK_BOT_TOKEN="\${STEELENGINE_MANTIS_SLACK_BOT_TOKEN:-\${SLACK_BOT_TOKEN:-}}"
  export SLACK_APP_TOKEN="\${STEELENGINE_MANTIS_SLACK_APP_TOKEN:-\${SLACK_APP_TOKEN:-}}"
  if [ -z "$SLACK_BOT_TOKEN" ] || [ -z "$SLACK_APP_TOKEN" ]; then
    echo "Gateway setup requires STEELENGINE_MANTIS_SLACK_BOT_TOKEN and STEELENGINE_MANTIS_SLACK_APP_TOKEN." >&2
    exit 2
  fi
...
run_mantis_remote_body() {
  ...
  if ! node_supports_type_stripping; then
    ...
    node_root="$HOME/.cache/steelengine-mantis/node-v$node_version-linux-$node_arch"
...
')
  ...
  if [ "$setup_gateway" = "1" ]; then
    export STEELENGINE_HOME="$HOME/.steelengine-mantis/slack-steelengine"
...
MANTIS_SLACK_PATCH
    pnpm steelengine config patch --file "$out/slack.socket.patch.json5" --dry-run
    pnpm steelengine config patch --file "$out/slack.socket.patch.json5"
    nohup pnpm steelengine gateway run --dev --allow-unconfigured --port 38973 --cli-backend-logs </dev/null >"$out/steelengine-gateway.log" 2>&1 &
    gateway_pid="$!"
    ...
    qa_args=(steelengine qa slack --repo-root . --output-dir "$slack_qa_output_dir" --provider-mode "$provider_mode" --model "$primary_model" --alt-model "$alternate_model" --credential-source "$credential_source" --credential-role "$credential_role")
    if [ "$fast_mode" = "1" ]; then
      qa_args+=(--fast)
    fi
    if [ "$approval_checkpoints" = "1" ]; then
      ...
      export STEELENGINE_QA_SLACK_APPROVAL_CHECKPOINT_DIR="$checkpoint_dir"
      export STEELENGINE_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS="\${STEELENGINE_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS:-${approvalCheckpointTimeoutMs}}"
      export STEELENGINE_MANTIS_APPROVAL_CHECKPOINT_SCENARIOS_JSON="$approval_checkpoint_scenarios_json"
      export STEELENGINE_MANTIS_APPROVAL_BROWSER_BIN="$browser_bin"
      cat >"$out/approval-checkpoint-watcher.mjs" <<'MANTIS_APPROVAL_WATCHER'
	import { spawn } from "node:child_process";
	import fs from "node:fs/promises";
	import path from "node:path";

const checkpointDir = process.env.STEELENGINE_QA_SLACK_APPROVAL_CHECKPOINT_DIR;
const timeoutMs = Number.parseInt(
  process.env.STEELENGINE_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS || "${approvalCheckpointTimeoutMs}",
  10,
);
	const scenarioIds = JSON.parse(
	  process.env.STEELENGINE_MANTIS_APPROVAL_CHECKPOINT_SCENARIOS_JSON || "[]",
	);
	const browserBin = process.env.STEELENGINE_MANTIS_APPROVAL_BROWSER_BIN;

if (!checkpointDir) {
  throw new Error("STEELENGINE_QA_SLACK_APPROVAL_CHECKPOINT_DIR is required.");
}
if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
  throw new Error("STEELENGINE_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS must be a positive integer.");
}
  ...
  throw new Error(\`Timed out waiting for approval checkpoint: \${filePath}\`);
	...
	function renderCheckpointHtml(checkpoint) {
	  ...
	  return '<!doctype html><html><head><meta charset="utf-8">' +
	    ...
	    '<div class="meta"><span class="name">steelengine</span><span class="app">APP</span><span class="state">' +
	...
	async function captureScreenshot(screenshotPath, checkpoint) {
	  if (!browserBin) {
	    throw new Error("STEELENGINE_MANTIS_APPROVAL_BROWSER_BIN is required to render approval checkpoint screenshots.");
	  }
...
export async function runMantisSlackDesktopSmoke(
  ...
  const slackChannelId =
    ...
    trimToValue(env.STEELENGINE_QA_SLACK_CHANNEL_ID) ??
  ...
  const remoteOutputDir = `/tmp/steelengine-mantis-slack-desktop-${startedAt
    .toISOString()
  ...
  try {
    ...
    if (gatewaySetup && !gatewaySetupCompleted) {
      throw new Error("Slack desktop gateway setup did not report a live SteelEngine gateway.");
    }
