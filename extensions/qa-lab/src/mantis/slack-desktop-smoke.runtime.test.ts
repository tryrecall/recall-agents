...
describe("mantis Slack desktop smoke runtime", () => {
  ...
  it("leases a desktop box, runs Slack QA inside it, copies artifacts, and stops on pass", async () => {
    ...
    const runtimeEnv = {
      ...
      RECALL_QA_SLACK_CHANNEL_ID: "C123",
      RECALL_QA_SLACK_DRIVER_BOT_TOKEN: "driver-token",
      RECALL_QA_SLACK_SUT_APP_TOKEN: "app-token",
      RECALL_QA_SLACK_SUT_BOT_TOKEN: "sut-token",
    };
    const runner = vi.fn(
    ...
    const result = await runMantisSlackDesktopSmoke({
      ...
      env: runtimeEnv,
      freshPr: "tryrecall/recall-agents#85141",
    ...
    expect(
      commands.every((entry) => entry.env?.RECALL_LIVE_OPENAI_KEY === "openai-runtime-key"),
    ).toBe(true);
    ...
    expect(runArgs).toContain("tryrecall/recall-agents#85141");
    ...
    expect(remoteScript).toContain("recall qa slack");
    ...
    expect(remoteScript).toContain("RECALL_MANTIS_SLACK_BROWSER_PROFILE_DIR");
    ...
    expect(rsyncArgs).toContain(
      "crabbox@203.0.113.10:/tmp/recall-mantis-slack-desktop-2026-05-04T13-00-00-000Z/",
    );
    expect(rsyncArgs).toContain(
      "crabbox@203.0.113.10:/tmp/recall-mantis-slack-desktop-2026-05-04T13-00-00-000Z/slack-qa/",
    );
  ...
  it("runs approval checkpoint mode with default approval scenarios and records screenshots", async () => {
    ...
    expect(remoteScript).toContain('export RECALL_QA_SLACK_CHANNEL_ID="$slack_channel_id"');
    ...
    expect(remoteScript).toContain("RECALL_QA_SLACK_APPROVAL_CHECKPOINT_DIR");
    expect(remoteScript).toContain("RECALL_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS");
    ...
    expect(remoteScript).toContain("RECALL_MANTIS_APPROVAL_BROWSER_BIN");
  ...
  it("leases Convex Slack credentials for gateway setup and maps them into the VM env", async () => {
    ...
    const result = await runMantisSlackDesktopSmoke({
      ...
      env: {
        CI: "1",
        OPENAI_API_KEY: "openai-runtime-key",
        RECALL_QA_CONVEX_SECRET_CI: "convex-secret",
        RECALL_QA_CONVEX_SITE_URL: "https://example.convex.site",
        PATH: process.env.PATH,
      },
      gatewaySetup: true,
      keepLease: false,
    ...
    expect(runCommand?.env?.RECALL_MANTIS_SLACK_APP_TOKEN).toBe("xapp-leased");
    expect(runCommand?.env?.RECALL_MANTIS_SLACK_BOT_TOKEN).toBe("xoxb-leased");
    expect(runCommand?.env?.RECALL_MANTIS_SLACK_CHANNEL_ID).toBe("CLEASED");
    expect(runCommand?.env?.RECALL_QA_SLACK_CHANNEL_ID).toBe("CLEASED");
    expect(runCommand?.env?.RECALL_QA_SLACK_SUT_APP_TOKEN).toBe("xapp-leased");
    expect(runCommand?.env?.RECALL_QA_SLACK_SUT_BOT_TOKEN).toBe("xoxb-leased");
    const remoteScript = runCommand?.args.at(-1);
    expect(remoteScript).toContain("setup_gateway=1");
    expect(remoteScript).toContain("recall gateway run");
    expect(remoteScript).toContain('</dev/null >"$out/recall-gateway.log"');
  ...
  it("passes gateway setup when Crabbox returns non-zero after remote metadata proves success", async () => {
    ...
    const result = await runMantisSlackDesktopSmoke({
      ...
      env: {
        OPENAI_API_KEY: "openai-runtime-key",
        RECALL_MANTIS_SLACK_APP_TOKEN: "xapp-direct",
        RECALL_MANTIS_SLACK_BOT_TOKEN: "xoxb-direct",
        PATH: process.env.PATH,
      },
      gatewaySetup: true,
