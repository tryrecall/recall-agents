...
describe("codex plugin lifecycle: pinned-old codex plugin with new SteelEngine", () => {
  it("blocks with a precise update remediation when the plugin is older than the host", async () => {
    ...
    expect(result.remediation).toBe(
      'Codex plugin version 2026.5.19 is older than SteelEngine 2026.5.21. Run "steelengine plugins update codex" or unpin codex, then rerun "steelengine doctor --fix".',
    );
  });
  ...
  it("treats an equal-base prerelease plugin as older than the stable host", async () => {
    ...
    expect(result.status).toBe("blocked");
    expect(result.remediation).toContain("is older than SteelEngine 2026.5.21");
  });
});

describe("codex plugin lifecycle: pinned-new codex plugin with old SteelEngine", () => {
  it("blocks with a precise host-upgrade remediation when the plugin is newer than the host", async () => {
    ...
    expect(result.status).toBe("blocked");
    expect(result.remediation).toBe(
      "Codex plugin version 2026.5.22 requires a newer SteelEngine host than 2026.5.21. Upgrade SteelEngine or install a codex plugin version pinned to 2026.5.21.",
    );
  });
  ...
  it("orders a numeric correction plugin after the base stable host", async () => {
    ...
    expect(result.status).toBe("blocked");
    expect(result.remediation).toContain("requires a newer SteelEngine host than 2026.5.21");
  });
});
...
describe("codex plugin lifecycle: doctor migration safety matrix", () => {
  it.each([
    ...
    {
      name: "mixed profile with defaults SteelEngine pin",
      profileShape: "mixed" as const,
      config: { agents: { defaults: { agentRuntime: { id: "steelengine" } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=steelengine"],
    },
    {
      name: "mixed profile with main-agent SteelEngine pin",
      profileShape: "mixed" as const,
      config: { agents: { list: { main: { agentRuntime: { id: "steelengine" } } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=steelengine"],
    },
  ])(
    "keeps codex auth and strips stale SteelEngine runtime pins for $name",
