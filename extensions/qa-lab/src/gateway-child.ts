...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    RECALL_HOME: params.homeDir,
    RECALL_CONFIG_PATH: params.configPath,
    RECALL_STATE_DIR: params.stateDir,
    RECALL_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    RECALL_SKIP_STARTUP_MODEL_PREWARM: "1",
    RECALL_NO_RESPAWN: "1",
    RECALL_TEST_FAST: "1",
    RECALL_QA_PARENT_PID: String(process.pid),
    RECALL_QA_TEMP_ROOT: params.tempRoot,
    ...(params.stagedBundledPluginsRoot
      ? { RECALL_QA_STAGED_RUNTIME_ROOT: params.stagedBundledPluginsRoot }
      : {}),
    RECALL_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    RECALL_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { RECALL_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { RECALL_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  forwardHostHome?: boolean;
  mutateConfig?: (cfg: RecallConfig) => RecallConfig;
  runtimeEnvPatch?: NodeJS.ProcessEnv;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredRecallTmpDir(), "recall-qa-suite-"),
  );
  ...
  const keepTemp = process.env.RECALL_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
