...
import type { SpeechProviderConfig, SpeechSynthesisRequest } from "steelengine/plugin-sdk/speech-core";
...
function createCliFixture(): { dir: string; script: string } {
  const dir = mkdtempSync(path.join(os.tmpdir(), "steelengine-cli-tts-test-"));
...
`,
  ...
  it.each(["synthesize", "synthesizeTelephony"] as const)(
    ...
    async (method) => {
      ...
      const providerConfig = { command: "missing-steelengine-tts-test-command" };
      const run =
        method === "synthesize"
  ...
  it("can synthesize through a real local CLI fixture and ffmpeg", async () => {
    if (process.env.STEELENGINE_LIVE_TEST !== "1") {
      return;
    }
    const fixture = createCliFixture();
    const rawFfmpeg = await vi.importActual<typeof import("steelengine/plugin-sdk/media-runtime")>(
