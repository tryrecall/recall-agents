...
export function scanPolicyGatewayExposure(
  ...
  if (bind === "custom" && hasCustomBindHost) {
    entries.push({
      ...
      kind: "bind",
      source: "oc://steelengine.config/gateway/customBindHost",
  ...
  entries.push({
    ...
    kind: "auth",
    source: "oc://steelengine.config/gateway/auth/mode",
  ...
  entries.push({
    ...
    kind: "authRateLimit",
    source: "oc://steelengine.config/gateway/auth/rateLimit",
  ...
  pushGatewayBooleanEvidence(
    ...
    "controlUi",
    controlUi.enabled,
    "oc://steelengine.config/gateway/controlUi/enabled",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://steelengine.config/gateway/controlUi/allowInsecureAuth",
  );
  pushGatewayBooleanEvidence(
    entries,
  ...
  if (typeof tailscale.mode === "string") {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://steelengine.config/gateway/tailscale/mode",
  ...
  if (tailscale.mode === "serve" && tailscale.preserveFunnel === true) {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://steelengine.config/gateway/tailscale/preserveFunnel",
      value: "funnel",
    });
  }
  ...
  if (gateway.mode === "remote") {
    entries.push({
      ...
      kind: "remote",
      source: "oc://steelengine.config/gateway/mode",
      value: "remote",
    });
    if (typeof remote.url === "string" && remote.url.trim() !== "") {
      entries.push({
        ...
        kind: "remote",
        source: "oc://steelengine.config/gateway/remote/url",
        value: true,
      });
    }
  }
...
function pushGatewayHttpEndpointEvidence(
  ...
  const source = `oc://steelengine.config/gateway/http/endpoints/${endpoint}`;
...
function pushGatewayNodeCommandEvidence(
  ...
  if (Array.isArray(nodes.denyCommands)) {
    nodes.denyCommands.forEach((command, index) => {
      ...
      entries.push({
        ...
        source: `oc://steelengine.config/gateway/nodes/denyCommands/#${index}`,
        value: normalized,
        command: normalized,
      });
    });
  ...
  nodes.allowCommands.forEach((command, index) => {
    ...
    entries.push({
      ...
      kind: "nodeCommand",
      source: `oc://steelengine.config/gateway/nodes/allowCommands/#${index}`,
      value: normalized,
      command: normalized,
    });
  });
