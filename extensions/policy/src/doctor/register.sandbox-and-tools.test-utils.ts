...
describe("registerPolicyDoctorChecks", () => {
    ...
    expect(runnerEvidence).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          value: "none",
          source: "oc://steelengine.config/agents/defaults/sandbox/docker/network",
        }),
        expect.objectContaining({
          kind: "browserCdpSourceRange",
          value: "172.21.0.1/32",
          source: "oc://steelengine.config/agents/defaults/sandbox/browser/cdpSourceRange",
        }),
        expect.objectContaining({
          kind: "containerMount",
          bind: "/shared:/shared:ro",
          source: "oc://steelengine.config/agents/defaults/sandbox/docker/binds/#0",
        }),
        expect.objectContaining({
          kind: "containerMount",
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          ocPath: "oc://steelengine.config/agents/list/#0/sandbox/browser/cdpSourceRange",
        }),
      ]),
    );
  });
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          ocPath: "oc://steelengine.config/agents/defaults/sandbox/backend",
    ...
    expect(runnerEvidence).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "containerNetwork",
          value: "host",
          source: "oc://steelengine.config/agents/list/#0/sandbox/docker/network",
        }),
        expect.objectContaining({
          kind: "containerMount",
          bind: "/var/run/docker.sock:/var/run/docker.sock:rw",
          source: "oc://steelengine.config/agents/list/#0/sandbox/docker/binds/#0",
        }),
        expect.objectContaining({
          kind: "containerMount",
          bind: "/browser:/browser:rw",
          source: "oc://steelengine.config/agents/list/#0/sandbox/browser/binds/#0",
        }),
      ]),
    );
  });
  ...
  it("applies agent-scoped sandbox claims only to matching agents", async () => {
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          ocPath: "oc://steelengine.config/agents/list/#0/sandbox/mode",
          requirement: "oc://policy.jsonc/sandbox/requireMode",
        }),
        expect.objectContaining({
          checkId: "policy/sandbox-backend-unapproved",
          ocPath: "oc://steelengine.config/agents/list/#0/sandbox/backend",
    ...
    expect(result.findings).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ocPath: "oc://steelengine.config/agents/list/#1/sandbox/backend",
          requirement: "oc://policy.jsonc/scopes/sebby/sandbox/allowBackends",
        }),
      ]),
    );
    ...
    expect(evidence.toolPosture).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          kind: "profile",
          value: "coding",
          source: "oc://steelengine.config/tools/profile",
        }),
        expect.objectContaining({
          id: "reviewer-exec-security",
          kind: "execSecurity",
          value: "deny",
          source: "oc://steelengine.config/agents/list/#0/tools/exec/security",
        }),
        expect.objectContaining({
          id: "tools-elevated-allow-from-whatsapp",
          kind: "elevatedAllowFrom",
          entries: ["+15550000001", "15550000002"],
          source: "oc://steelengine.config/tools/elevated/allowFrom/whatsapp",
        }),
      ]),
    );
    expect(result.findings).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          ocPath: "oc://steelengine.config/tools/elevated/enabled",
  ...
  it("reports global and agent-scoped tool claims independently", async () => {
    ...
    expect(result.findings).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ocPath: "oc://steelengine.config/agents/list/#1/tools/exec/host",
        }),
      ]),
    );
  });
  ...
  it("reports global and agent-scoped alsoAllow drift", async () => {
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          ocPath: "oc://steelengine.config/tools/alsoAllow",
        ...
        expect.objectContaining({
          checkId: "policy/tools-also-allow-missing",
          ocPath: "oc://steelengine.config/agents/list/#0/tools/alsoAllow",
    ...
    expect(result.findings).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          requirement: "oc://policy.jsonc/scopes/sebby/tools/alsoAllow/expected",
          ocPath: "oc://steelengine.config/agents/list/#1/tools/alsoAllow",
        }),
      ]),
    );
  });
    ...
    expect(evidence.toolPosture).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          value: false,
          source: "oc://steelengine.config/tools/elevated/enabled",
        }),
      ]),
    );
    expect(result.findings).toEqual([]);
    ...
    expect(evidence.toolPosture).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          kind: "execSecurity",
          value: "deny",
          source: "oc://steelengine.config/tools/exec/security",
        }),
      ]),
    );
    expect(result.findings).toEqual([]);
  ...
  it("reports preserved Tailscale Funnel routes when policy denies Funnel exposure", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/gateway-tailscale-funnel",
        severity: "error",
        ocPath: "oc://steelengine.config/gateway/tailscale/preserveFunnel",
        requirement: "oc://policy.jsonc/gateway/exposure/allowTailscaleFunnel",
      }),
    ]);
  });
  ...
  it("reports valid non-loopback custom bind hosts", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/gateway-non-loopback-bind",
        severity: "error",
        ocPath: "oc://steelengine.config/gateway/customBindHost",
        requirement: "oc://policy.jsonc/gateway/exposure/allowNonLoopbackBind",
      }),
    ]);
  });
  ...
  it("reports configured gateway remote URLs when remote mode is active", async () => {
    ...
    expect(result.findings).toEqual([
      ...
      expect.objectContaining({
        checkId: "policy/gateway-remote-enabled",
        severity: "error",
        ocPath: "oc://steelengine.config/gateway/remote/url",
        requirement: "oc://policy.jsonc/gateway/remote/allow",
      }),
    ]);
  });
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          checkId: "policy/gateway-http-url-fetch-unrestricted",
          severity: "error",
          ocPath: "oc://steelengine.config/gateway/http/endpoints/responses/files/allowUrl",
          requirement: "oc://policy.jsonc/gateway/http/requireUrlAllowlists",
        }),
        expect.objectContaining({
          checkId: "policy/gateway-http-url-fetch-unrestricted",
          severity: "error",
          ocPath: "oc://steelengine.config/gateway/http/endpoints/responses/images/allowUrl",
          requirement: "oc://policy.jsonc/gateway/http/requireUrlAllowlists",
        }),
      ]),
    );
