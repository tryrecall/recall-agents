...
type RepairPatch = {
  readonly config: SteelEngineConfig;
...
function mergeRequiredDenyTools(
  ...
  for (const finding of findings) {
    ...
    if (
      ...
      finding.ocPath === "oc://steelengine.config/tools/deny"
    ) {
      warnings.push(
  ...
  return changes.length > 0
    ? { config: next as SteelEngineConfig, changes: uniqueStrings(changes), warnings }
...
function disableElevatedTools(
  ...
  if (
    !findings.some((finding) => finding.ocPath === "oc://steelengine.config/tools/elevated/enabled")
  ) {
    return { config: cfg, changes: [] };
  }
  const next = cloneConfig(cfg);
  ...
  return {
    config: next as SteelEngineConfig,
...
function disableInsecureControlUi(
  ...
  const changes: string[] = [];
  const fields = [
    ["allowInsecureAuth", "oc://steelengine.config/gateway/controlUi/allowInsecureAuth"],
    [
      "dangerouslyDisableDeviceAuth",
      "oc://steelengine.config/gateway/controlUi/dangerouslyDisableDeviceAuth",
    ],
    [
      "dangerouslyAllowHostHeaderOriginFallback",
      "oc://steelengine.config/gateway/controlUi/dangerouslyAllowHostHeaderOriginFallback",
    ],
  ] as const;
  ...
  return changes.length > 0
    ? { config: next as SteelEngineConfig, changes }
    : { config: cfg, changes };
}
...
function disableRemoteGatewayMode(
  ...
  if (!findings.some((finding) => finding.ocPath === "oc://steelengine.config/gateway/mode")) {
    return { config: cfg, changes: [] };
  }
  const next = cloneConfig(cfg);
  const gateway = ensureRecord(next, "gateway");
...
function enableSensitiveLoggingRedaction(cfg: SteelEngineConfig): RepairPatch {
  const next = cloneConfig(cfg);
...
function disableTelemetryContentCapture(cfg: SteelEngineConfig): RepairPatch {
  const next = cloneConfig(cfg);
...
function cloneConfig(cfg: SteelEngineConfig): ConfigRecord {
...
function configPathSegments(ocPath: string): readonly string[] {
  const prefix = "oc://steelengine.config/";
...
function skippedUnsafeScopedRepair(cfg: SteelEngineConfig, warning: string): RepairPatch {
...
function isScopedInheritedChannelDefaultFinding(finding: HealthFinding): boolean {
  return (
    hasScopedPolicyRequirement([finding]) &&
    finding.ocPath?.startsWith("oc://steelengine.config/channels/defaults/") === true
  );
}
