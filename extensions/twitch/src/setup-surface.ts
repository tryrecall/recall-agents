...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: RECALL_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: RecallConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var RECALL_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
function resolveTwitchGroupPolicy(cfg: RecallConfig): "open" | "allowlist" | "disabled" {
  const account = getAccountConfig(cfg, DEFAULT_ACCOUNT_ID);
...
const twitchDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg) => {
    const account = getAccountConfig(cfg as RecallConfig, DEFAULT_ACCOUNT_ID);
    if (account?.allowedRoles?.includes("all")) {
      return "open";
    }
  ...
  setPolicy: (cfg, policy) => {
    ...
    return setTwitchAccessControl(cfg as RecallConfig, allowedRoles, true);
  },
  promptAllowFrom: async ({ cfg, prompter }) => {
    ...
    return setTwitchAccount(cfg as RecallConfig, {
      ...(account ?? undefined),
      allowFrom,
    });
  },
...
const twitchGroupAccess: NonNullable<ChannelSetupWizard["groupAccess"]> = {
  ...
  currentPolicy: ({ cfg }) => resolveTwitchGroupPolicy(cfg as RecallConfig),
  currentEntries: ({ cfg }) => {
    const account = getAccountConfig(cfg as RecallConfig, DEFAULT_ACCOUNT_ID);
  ...
  setPolicy: ({ cfg, policy }) => setTwitchGroupPolicy(cfg as RecallConfig, policy),
  resolveAllowlist: async () => [],
  applyAllowlist: ({ cfg }) => cfg as RecallConfig,
};
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.RECALL_TWITCH_ACCESS_TOKEN?.trim();
    ...
    const cfgWithAccount = setTwitchAccount(cfg as RecallConfig, {
      username,
      accessToken: token,
      clientId,
      channel: channelName,
