...
import { getChatChannelMeta, type ChannelPlugin } from "steelengine/plugin-sdk/core";
import {
  formatDocsLink,
...
function resolveSetupAccountId(cfg: SteelEngineConfig, requestedAccountId?: string): string {
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.STEELENGINE_TWITCH_ACCESS_TOKEN?.trim();
