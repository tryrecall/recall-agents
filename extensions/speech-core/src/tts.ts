...
import { resolveChannelTtsVoiceDelivery } from "recall/plugin-sdk/channel-targets";
import type {
  RecallConfig,
  ResolvedTtsPersona,
  TtsAutoMode,
...
import { transcodeAudioBuffer } from "recall/plugin-sdk/media-runtime";
import {
...
import { isVerbose, logVerbose } from "recall/plugin-sdk/runtime-env";
import { tempWorkspaceSync, resolvePreferredRecallTmpDir } from "recall/plugin-sdk/sandbox";
...
import { stripMarkdown } from "recall/plugin-sdk/text-chunking";
import { resolveConfigDir, resolveUserPath } from "recall/plugin-sdk/text-utility-runtime";
import {
...
function sortSpeechProvidersForAutoSelection(cfg?: RecallConfig) {
...
function resolveTtsRuntimeConfig(cfg: RecallConfig): RecallConfig {
  return (
    selectApplicableRuntimeConfig({
      inputConfig: cfg,
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: RecallConfig): TtsProvider[] {
