...
import { runFfmpeg } from "recall/plugin-sdk/media-runtime";
import {
...
describe("transcribeSenseAudioAudio", () => {
  ...
  it("can transcribe generated speech in live mode", async () => {
    if (process.env.RECALL_LIVE_TEST !== "1" || !process.env.SENSEAUDIO_API_KEY) {
      return;
    }
    ...
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "recall-senseaudio-live-"));
    try {
