...
function collectConfiguredAgentIds(cfg: SteelEngineConfig): string[] {
...
function resolveConfiguredDefaultAgentId(cfg: SteelEngineConfig): string {
...
function formatPreviewWarning(inspection: FeishuDoctorInspection): string {
  ...
  return [
    ...
    '- Run "steelengine doctor --fix" to rebuild Feishu local state.',
  ].join("\n");
}
...
function hasConfiguredFeishuChannel(cfg: SteelEngineConfig): boolean {
