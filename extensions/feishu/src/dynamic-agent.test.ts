...
beforeEach(async () => {
  tempRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "steelengine-feishu-agent-"));
});
...
function createRuntime(
  currentCfg?: SteelEngineConfig,
  persistedCfg?: SteelEngineConfig,
  mutationCfg?: SteelEngineConfig,
) {
  let runtimeCfg = structuredClone(currentCfg ?? ({} as SteelEngineConfig));
  const commitConfig = vi.fn();
  const mutateConfigFile = vi.fn(
    async (params: {
      mutate: (draft: SteelEngineConfig, context: { snapshot: never; previousHash: null }) => unknown;
    }) => {
...
describe("maybeCreateDynamicAgent", () => {
  ...
  it("rechecks current ingress inside the config mutation lock", async () => {
    ...
    const canCreateForConfig = vi
      .fn<(cfg: SteelEngineConfig) => Promise<boolean>>()
  ...
  it("preserves concurrent runtime config when creating from a stale request snapshot", async () => {
    ...
    const result = await maybeCreateDynamicAgent({
      cfg: { agents: { list: [] }, bindings: [] } as SteelEngineConfig,
      runtime,
      accountId: "default",
      senderOpenId: "ou_sender",
      canCreateForConfig: async () => true,
