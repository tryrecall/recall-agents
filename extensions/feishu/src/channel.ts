...
import { ToolAuthorizationError } from "steelengine/plugin-sdk/channel-actions";
import {
...
import { createComputedAccountStatusAdapter } from "steelengine/plugin-sdk/status-helpers";
import {
...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount, FeishuProbeResult> =
  createChatChannelPlugin({
    base: {
      ...
      auth: {
        login: async ({ cfg }) => {
          const { createClackPrompter } = await import("steelengine/plugin-sdk/setup-runtime");
          const { replaceConfigFile } = await import("steelengine/plugin-sdk/config-mutation");
