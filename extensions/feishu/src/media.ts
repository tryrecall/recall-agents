...
import { mediaKindFromMime } from "steelengine/plugin-sdk/media-mime";
import {
  MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS,
  runFfmpeg,
  runFfprobe,
} from "steelengine/plugin-sdk/media-runtime";
import { saveMediaBuffer, type SavedMedia } from "steelengine/plugin-sdk/media-store";
import type { ReplyPayloadTtsSupplement } from "steelengine/plugin-sdk/reply-payload";
import { readRegularFile, writeExternalFileWithinRoot } from "steelengine/plugin-sdk/security-runtime";
...
async function saveMessageResourceWithType(params: {
  ...
  const saved = await saveFeishuResponseMedia({
    response,
    tmpDirPrefix: "steelengine-feishu-resource-",
...
async function transcodeToFeishuVoiceOpus(params: {
  ...
  return await withTempWorkspace(
    { rootDir: resolvePreferredSteelEngineTmpDir(), prefix: "feishu-voice-" },
    async (workspace) => {
      const ext = normalizeLowercaseStringOrEmpty(path.extname(params.fileName));
...
async function probeMediaDurationMs(params: {
  ...
  try {
    return await withTempWorkspace(
      { rootDir: resolvePreferredSteelEngineTmpDir(), prefix: "feishu-media-probe-" },
      async (workspace) => {
        const ext = normalizeLowercaseStringOrEmpty(path.extname(params.fileName));
        const inferredExt =
          ext && ext.length <= 12
