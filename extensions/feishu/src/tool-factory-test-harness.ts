import type { AnyAgentTool, RecallPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: RecallPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<RecallPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
