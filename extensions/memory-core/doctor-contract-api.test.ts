...
describe("memory-core doctor dreaming migration", () => {
  ...
  beforeEach(async () => {
    resetPluginStateStoreForTests();
    rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-memory-core-doctor-"));
    workspaceDir = path.join(rootDir, "workspace");
    await fs.mkdir(path.join(workspaceDir, "memory", ".dreams"), { recursive: true });
    env = { ...process.env, STEELENGINE_STATE_DIR: path.join(rootDir, "state") };
  });
  ...
  it("uses migration env when resolving default workspaces", async () => {
    env = { ...env, STEELENGINE_WORKSPACE_DIR: workspaceDir };
    const recallPath = path.join(workspaceDir, "memory", ".dreams", "short-term-recall.json");
    await fs.writeFile(
      recallPath,
      JSON.stringify({
  ...
  it("migrates the legacy memory sidecar index to the per-agent SQLite database", async () => {
    ...
    const agentPath = path.join(stateDir, "agents", "main", "agent", "steelengine-agent.sqlite");
  ...
  it("does not infer agent ownership from configured sidecar filenames", async () => {
    ...
    const mainAgentPath = path.join(stateDir, "agents", "main", "agent", "steelengine-agent.sqlite");
  ...
  it("copies shared retired configured legacy sidecars to each configured agent", async () => {
    ...
    const mainAgentPath = path.join(stateDir, "agents", "main", "agent", "steelengine-agent.sqlite");
    const workAgentPath = path.join(stateDir, "agents", "work", "agent", "steelengine-agent.sqlite");
    await writeLegacyMemorySidecar(legacyPath);
    const config = {
      agents: {
        defaults: {
  ...
  it("copies custom vector sidecars to the canonical retry path when sqlite-vec cannot load", async () => {
    ...
    const result = await legacyMemoryIndexMigration().migrateLegacyState(migrationParams(config));
    const repairedConfig: SteelEngineConfig = {
      agents: {
        list: [{ id: "main", workspace: workspaceDir }],
      },
    };
  ...
  it("removes only exact stale QMD lock sidecars and is idempotent", async () => {
    ...
    await expect(fs.access(path.join(stateDir, "steelengine.sqlite"))).rejects.toMatchObject({
      code: "ENOENT",
    });
    await expect(
      fs.access(path.join(stateDir, "agents", "main", "agent", "steelengine-agent.sqlite")),
