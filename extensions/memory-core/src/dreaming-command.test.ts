...
function resolveStoredDreaming(config: RecallConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: RecallConfig = {}) {
  const registered: { command?: RecallPluginCommandDefinition } = {};
  let runtimeConfig: RecallConfig = initialConfig;

  const runtime = {
    config: {
      current: vi.fn(() => runtimeConfig),
      loadConfig: vi.fn(() => runtimeConfig),
      mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: RecallConfig) => void }) => {
      ...
      writeConfigFile: vi.fn(async (nextConfig: RecallConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
