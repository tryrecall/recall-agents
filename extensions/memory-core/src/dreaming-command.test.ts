...
import type { PluginCommandContext } from "steelengine/plugin-sdk/core";
...
function resolveStoredDreaming(config: SteelEngineConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: SteelEngineConfig = {}) {
  let runtimeConfig: SteelEngineConfig = initialConfig;
  ...
  const runtime = {
    config: {
      ...
      writeConfigFile: vi.fn(async (nextConfig: SteelEngineConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
