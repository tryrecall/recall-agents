...
import type { MemoryEmbeddingProbeResult } from "recall/plugin-sdk/memory-core-host-engine-storage";
import {
  resolveMemoryDreamingConfig,
  resolveMemoryRemDreamingConfig,
} from "recall/plugin-sdk/memory-core-host-status";
...
function resolveMemoryPluginConfig(cfg: RecallConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredRecallTmpDir(), "recall-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: RecallConfig): string {
...
function resolveAgent(cfg: RecallConfig, agent?: string) {
...
function resolveAgentIds(cfg: RecallConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: recall memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredRecallTmpDir(), "recall-rem-backfill-"),
      );
      try {
