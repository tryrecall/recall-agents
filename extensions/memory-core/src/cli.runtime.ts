...
import { isUsageCountedSessionTranscriptFileName } from "steelengine/plugin-sdk/memory-core-host-engine-qmd";
import type { MemoryEmbeddingProbeResult } from "steelengine/plugin-sdk/memory-core-host-engine-storage";
import {
...
function formatMemoryIndexIdentityWarning(
  ...
  return {
    reason,
    fix: `Run: steelengine memory status --index --agent ${agentId}`,
  };
}
...
function resolveMemoryPluginConfig(cfg: SteelEngineConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: SteelEngineConfig): string {
...
function resolveAgent(cfg: SteelEngineConfig, agent?: string) {
...
function resolveAgentIds(cfg: SteelEngineConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: steelengine memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-rem-backfill-"),
      );
      try {
