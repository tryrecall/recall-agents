...
import { appendSqliteSessionTranscriptEventForTest } from "steelengine/plugin-sdk/sqlite-runtime-testing";
...
vi.mock("steelengine/plugin-sdk/memory-core-host-runtime-core", { spy: true });

const { createTempWorkspace } = createMemoryCoreTestHarness();
const DREAMS_FILE_LOCKS_KEY = Symbol.for("steelengine.memoryCore.dreamingNarrative.fileLocks");
...
const originalNarrativeStateDir = process.env.STEELENGINE_STATE_DIR;
...
function restoreNarrativeTestEnv(): void {
  if (originalNarrativeStateDir === undefined) {
    ...
    Reflect.set(process.env, "STEELENGINE_STATE_DIR", originalNarrativeStateDir);
  }
}
...
describe("dream diary file behavior", () => {
  it("writes, reads, deduplicates, and removes backfill entries", async () => {
    ...
    const startMarker = "<!-- steelengine:dreaming:diary:start -->";
    const endMarker = "<!-- steelengine:dreaming:diary:end -->";
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("generates narrative and writes diary entry", async () => {
    const workspaceDir = await createTempWorkspace("steelengine-dreaming-narrative-");
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("steelengine-dreaming-narrative-");
    const stateDir = await createTempWorkspace("steelengine-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("steelengine-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("steelengine-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDirs = await Promise.all(
      Array.from({ length: 5 }, () => createTempWorkspace("steelengine-dreaming-detach-")),
    );
    const logger = createMockLogger();
  ...
  it("serializes detached narratives that reuse a workspace and phase session", async () => {
    ...
    const workspaceDir = await createTempWorkspace("steelengine-dreaming-detach-");
    const logger = createMockLogger();
