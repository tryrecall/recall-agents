...
import * as memoryCoreHostRuntimeCoreModule from "recall/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "recall/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "recall/plugin-sdk/session-store-runtime";
...
const DREAMS_FILE_LOCKS_KEY = Symbol.for("recall.memoryCore.dreamingNarrative.fileLocks");
const NARRATIVE_SESSION_LOCKS_KEY = Symbol.for(
  "recall.memoryCore.dreamingNarrative.sessionLocks",
);
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("recall:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("recall-dreaming-backfill-");
    ...
    expect(content.match(/recall:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("recall-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- recall:dreaming:diary:start -->");
    expect(content).toContain("<!-- recall:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- recall:dreaming:diary:start -->");
    const end = content.indexOf("<!-- recall:dreaming:diary:end -->");
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("recall-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("recall-dreaming-narrative-");
    const stateDir = await createTempWorkspace("recall-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("recall-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("recall-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDirs = await Promise.all(
      Array.from({ length: 5 }, () => createTempWorkspace("recall-dreaming-detach-")),
    );
    const logger = createMockLogger();
  ...
  it("serializes detached narratives that reuse a workspace and phase session", async () => {
    ...
    const workspaceDir = await createTempWorkspace("recall-dreaming-detach-");
    const logger = createMockLogger();
