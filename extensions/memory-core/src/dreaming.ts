...
import { resolveMemoryDreamingPluginConfig } from "steelengine/plugin-sdk/memory-core-host-runtime-core";
import {
...
import { peekSystemEventEntries } from "steelengine/plugin-sdk/system-event-runtime";
...
async function runShortTermDreamingPromotionIfTriggered(params: {
  ...
  config: ShortTermPromotionDreamingConfig;
  logger: Logger;
  subagent?: SteelEnginePluginApi["runtime"]["subagent"];
...
export function registerShortTermPromotionDreaming(api: SteelEnginePluginApi): void {
  ...
  const resolveCurrentConfig = (): SteelEngineConfig =>
  ...
  const reconcileManagedDreamingCron = async (params: {
    ...
    startupConfig?: SteelEngineConfig;
