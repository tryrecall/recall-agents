...
import { peekSystemEventEntries } from "recall/plugin-sdk/system-event-runtime";
...
type Logger = Pick<RecallPluginApi["logger"], "info" | "warn" | "error">;
...
export function registerShortTermPromotionDreaming(api: RecallPluginApi): void {
  ...
  const resolveCurrentConfig = (): RecallConfig =>
    (api.runtime.config?.current?.() ?? api.config) as RecallConfig;
