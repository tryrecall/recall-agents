...
import {
  ...
  formatHelpExamples,
  theme,
} from "steelengine/plugin-sdk/memory-core-host-runtime-cli";
import {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["steelengine memory status", "Show index and provider status."],
          [
            "steelengine memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["steelengine memory status --deep", "Probe embedding provider readiness."],
          ["steelengine memory index --force", "Force a full reindex."],
          ['steelengine memory search "meeting notes"', "Quick search using positional query."],
          [
            'steelengine memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `steelengine memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
            "Review weighted short-term candidates for long-term memory.",
          ],
          [
            "steelengine memory promote --apply",
            "Append top-ranked short-term candidates into MEMORY.md.",
          ],
          [
            'steelengine memory promote-explain "router vlan"',
            "Explain why a specific candidate would or would not promote.",
          ],
          [
            "steelengine memory rem-harness --json",
            "Preview REM reflections, candidate truths, and deep promotion output.",
          ],
          [
            "steelengine memory rem-backfill --path ./memory",
          ...
          ["steelengine memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.steelengine.ai/cli/memory")}\n`,
    );
