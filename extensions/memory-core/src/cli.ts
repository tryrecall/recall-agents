    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["recall memory status", "Show index and provider status."],
          [
            "recall memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["recall memory status --deep", "Probe embedding provider readiness."],
          ["recall memory index --force", "Force a full reindex."],
          ['recall memory search "meeting notes"', "Quick search using positional query."],
          [
            'recall memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `recall memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "recall memory rem-backfill --path ./memory",
          ...
          ["recall memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.recall.ai/cli/memory")}\n`,
    );
