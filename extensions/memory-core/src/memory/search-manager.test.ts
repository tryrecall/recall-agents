...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "steelengine/plugin-sdk/memory-core-host-engine-qmd";
import type {
...
function createBuiltinCfg(agentId: string): SteelEngineConfig {
  return {
    agents: {
      defaults: {
        workspace: "/tmp/workspace",
...
async function expectPendingQmdReplacement(params: {
  agentId: string;
  firstCfg: SteelEngineConfig;
  secondCfg: SteelEngineConfig;
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("steelengine.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-qmd-workspace-"));
