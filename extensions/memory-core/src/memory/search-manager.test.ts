...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "recall/plugin-sdk/memory-core-host-engine-qmd";
...
vi.mock("recall/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createBuiltinCfg(agentId: string): RecallConfig {
  return {
    agents: {
      defaults: {
...
async function expectPendingQmdReplacement(params: {
  agentId: string;
  firstCfg: RecallConfig;
  secondCfg: RecallConfig;
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("recall.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-qmd-workspace-"));
