...
import { withMockedWindowsPlatform } from "recall/plugin-sdk/test-node-mocks";
...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("recall.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("recall.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("recall.qmdEmbedQueueTail");
...
vi.mock("recall/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("recall/plugin-sdk/memory-core-host-engine-foundation")
  >("recall/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("recall/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/file-lock")>(
    "recall/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { RecallConfig } from "recall/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
