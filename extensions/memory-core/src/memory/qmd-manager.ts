...
import { withFileLock } from "recall/plugin-sdk/file-lock";
import {
...
const QMD_UPDATE_QUEUE_KEY = Symbol.for("recall.qmdUpdateQueueState");
...
function normalizeHanBm25Query(query: string): string {
  ...
  // Keep Han/CJK BM25 queries intact so Recall search semantics match direct qmd search.
  return trimmed;
}
