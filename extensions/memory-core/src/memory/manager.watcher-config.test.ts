...
const { createdWatchers, memoryLoggerWarn, watchMock } = vi.hoisted(() => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("recall.test.memoryWatchFactory")] =
    result.watchMock;
  return result;
});

vi.mock("recall/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("recall/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
} from "recall/plugin-sdk/memory-core-host-engine-embeddings";
import {
...
describe("memory watcher config", () => {
  ...
  afterAll(() => {
    Reflect.deleteProperty(globalThis, Symbol.for("recall.test.memoryWatchFactory"));
  });
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): RecallConfig {
    const defaults: NonNullable<NonNullable<RecallConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: RecallConfig) {
