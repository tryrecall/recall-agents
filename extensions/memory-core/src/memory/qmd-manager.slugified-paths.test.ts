...
import { resolveMemoryBackendConfig } from "steelengine/plugin-sdk/memory-core-host-engine-storage";
...
describe("QmdMemoryManager slugified path resolution", () => {
  ...
  async function createManager(params?: { cfg?: SteelEngineConfig }) {
  ...
  beforeEach(async () => {
    ...
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-qmd-slugified-"));
