...
import { resolveMemoryBackendConfig } from "recall/plugin-sdk/memory-core-host-engine-storage";
...
describe("QmdMemoryManager slugified path resolution", () => {
  ...
  async function createManager(params?: { cfg?: RecallConfig }) {
  ...
  beforeEach(async () => {
    ...
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-qmd-slugified-"));
