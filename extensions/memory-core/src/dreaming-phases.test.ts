...
import { RequestScopedSubagentRuntimeError } from "steelengine/plugin-sdk/error-runtime";
import {
...
import { formatSqliteSessionFileMarker } from "steelengine/plugin-sdk/sqlite-runtime-testing";
...
const LIGHT_SLEEP_EVENT_TEXT = "__steelengine_memory_core_light_sleep__";
const REM_SLEEP_EVENT_TEXT = "__steelengine_memory_core_rem_sleep__";
const originalDreamingTestFast = process.env.STEELENGINE_TEST_FAST;
const originalDreamingStateDir = process.env.STEELENGINE_STATE_DIR;
const EMPTY_SESSION_CONTENT_HASH =
  "75a11da44c802486bc6f65640aa48a730f0f684c5c07a42ba3cd1735eb3fb070";
const LIGHT_DREAMING_TEST_CONFIG: SteelEngineConfig = {
  plugins: {
    entries: {
      "memory-core": {
        config: {
...
function setDreamingTestEnv(stateDir: string): void {
  Reflect.set(process.env, "STEELENGINE_TEST_FAST", "1");
...
function restoreDreamingTestEnv(): void {
  if (originalDreamingTestFast === undefined) {
    Reflect.deleteProperty(process.env, "STEELENGINE_TEST_FAST");
  } else {
    Reflect.set(process.env, "STEELENGINE_TEST_FAST", originalDreamingTestFast);
  }
  if (originalDreamingStateDir === undefined) {
    Reflect.deleteProperty(process.env, "STEELENGINE_STATE_DIR");
  } else {
    Reflect.set(process.env, "STEELENGINE_STATE_DIR", originalDreamingStateDir);
  }
  clearRuntimeConfigSnapshot();
}
...
async function createDreamingWorkspace(): Promise<string> {
  const workspaceDir = await createTempWorkspace("steelengine-dreaming-phases-");
...
async function triggerLightDreaming(
  ...
  await beforeAgentReply(
    { cleanedBody: "__steelengine_memory_core_light_sleep__" },
...
describe("memory-core dreaming phases", () => {
      ...
      await beforeAgentReply(
        {
          cleanedBody: [
            "System: rotate logs",
            "System: __steelengine_memory_core_light_sleep__",
            "",
            "A scheduled reminder has been triggered. The reminder content is:",
            "",
            "rotate logs",
            "__steelengine_memory_core_light_sleep__",
            "",
  ...
  it("stops stripping a malformed managed block at the next section boundary", async () => {
    ...
    await withDreamingTestClock(async () => {
      await writeDailyNote(workspaceDir, [
        ...
        "",
        "## Light Sleep",
        "<!-- steelengine:dreaming:light:start -->",
        ...
        "<!-- steelengine:dreaming:light:end -->",
      ]);
  ...
  it("records light/rem signals that reinforce deep promotion ranking", async () => {
    ...
    await withDreamingTestClock(async () => {
      ...
      await beforeAgentReply(
        { cleanedBody: "__steelengine_memory_core_rem_sleep__" },
        { trigger: "heartbeat", workspaceDir },
      );
    });
  ...
  it("increments dailyCount when the same daily file is re-ingested on a later day", async () => {
    ...
    const configForTest: SteelEngineConfig = {
      plugins: {
        entries: {
          "memory-core": {
            config: {
