...
function scanTelegramMalformedGroupsConfig(cfg: SteelEngineConfig): TelegramMalformedGroupsHit[] {
...
function scanTelegramInvalidAllowFromEntries(cfg: SteelEngineConfig): TelegramAllowFromInvalidHit[] {
...
function scanTelegramBotEndpointApiRoots(cfg: SteelEngineConfig): TelegramApiRootBotEndpointHit[] {
...
function formatTelegramAccountConfigPath(cfg: SteelEngineConfig, accountId: string): string {
  const telegram = asObjectRecord((cfg.channels as Record<string, unknown> | undefined)?.telegram);
  const accounts = asObjectRecord(telegram?.accounts);
...
function maybeRepairTelegramApiRoots(cfg: SteelEngineConfig): {
...
async function repairTelegramConfig(params: { cfg: SteelEngineConfig }): Promise<{
  config: SteelEngineConfig;
  changes: string[];
}> {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: SteelEngineConfig): Promise<{
  config: SteelEngineConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("steelengine/plugin-sdk/runtime");
