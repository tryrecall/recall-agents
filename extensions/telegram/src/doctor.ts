...
function formatTelegramAccountConfigPath(cfg: RecallConfig, accountId: string): string {
  const telegram = asObjectRecord((cfg.channels as Record<string, unknown> | undefined)?.telegram);
  const accounts = asObjectRecord(telegram?.accounts);
...
export function maybeRepairTelegramApiRoots(cfg: RecallConfig): {
  config: RecallConfig;
  changes: string[];
} {
...
async function repairTelegramConfig(params: { cfg: RecallConfig }): Promise<{
  config: RecallConfig;
  changes: string[];
}> {
...
export async function maybeRepairTelegramAllowFromUsernames(cfg: RecallConfig): Promise<{
  config: RecallConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("recall/plugin-sdk/runtime");
