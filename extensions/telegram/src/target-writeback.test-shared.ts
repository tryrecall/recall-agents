...
vi.mock("recall/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/config-mutation")>(
    "recall/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("recall/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/cron-store-runtime")>(
    "recall/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
...
export function installMaybePersistResolvedTelegramTargetTests(params?: {
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/recall.json");
