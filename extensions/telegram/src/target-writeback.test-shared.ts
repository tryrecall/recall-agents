...
vi.mock("steelengine/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("steelengine/plugin-sdk/config-mutation")>(
    "steelengine/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("steelengine/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("steelengine/plugin-sdk/cron-store-runtime")>(
    "steelengine/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
...
export function installMaybePersistResolvedTelegramTargetTests(params?: {
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/steelengine.json");
      expect(saveCronStore).toHaveBeenCalledTimes(1);
