...
import type { RoutePeer } from "steelengine/plugin-sdk/routing";
import {
...
const telegramMessageAdapter = createChannelMessageAdapterFromOutbound<SteelEngineConfig>({
  id: "telegram",
  live: {
    capabilities: {
      draftPreview: true,
...
function resolveTelegramNativeTopicThreadId(
  ...
  // Keep the chat-scoped canonical id inside SteelEngine state; translate it back
  ...
  base: {
      ...
      inboundFormattingHints: ({ cfg, accountId }) => {
        ...
        if (richMessages) {
          return {
            ...
            rules: [
              "Telegram rich ON (Bot API 10.2 blocks; SteelEngine maps markdown + these HTML islands to typed blocks).",
    ...
    gateway: {
      ...
      logoutAccount: async ({ accountId, cfg }) => {
        ...
        const nextCfg = { ...cfg } as SteelEngineConfig;
