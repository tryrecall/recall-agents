import type { TelegramNetworkConfig } from "recall/plugin-sdk/config-contracts";
...
let isWSL2Sync: typeof import("recall/plugin-sdk/runtime-env").isWSL2Sync;
...
async function loadModule() {
  ({ isWSL2Sync } = await import("recall/plugin-sdk/runtime-env"));
  ({
  ...
  it.each([
    {
      ...
      expected: {
        value: true,
        source: "env:RECALL_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "uses env disable when set",
      env: { RECALL_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1" },
      expected: {
        value: false,
        source: "env:RECALL_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "prefers env enable over config",
      env: { RECALL_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1" },
  ...
  it.each([
    {
      ...
      env: { RECALL_TELEGRAM_DNS_RESULT_ORDER: "verbatim" },
      nodeMajor: 22,
      expected: {
        value: "verbatim",
        source: "env:RECALL_TELEGRAM_DNS_RESULT_ORDER",
      },
    },
    {
      name: "normalizes trimmed env values",
      env: { RECALL_TELEGRAM_DNS_RESULT_ORDER: "  IPV4FIRST  " },
      nodeMajor: 20,
      expected: {
        value: "ipv4first",
        source: "env:RECALL_TELEGRAM_DNS_RESULT_ORDER",
    ...
    },
    {
      ...
      env: { RECALL_TELEGRAM_DNS_RESULT_ORDER: "bogus" },
      network: { dnsResultOrder: "ipv4first" },
      nodeMajor: 20,
      expected: { value: "ipv4first", source: "config" },
    },
