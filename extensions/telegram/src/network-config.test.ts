...
import type { TelegramNetworkConfig } from "steelengine/plugin-sdk/config-contracts";
...
let isWSL2Sync: typeof import("steelengine/plugin-sdk/runtime-env").isWSL2Sync;
...
async function loadModule() {
  const { isWSL2Sync: loadedIsWSL2Sync } = await import("steelengine/plugin-sdk/runtime-env");
  ...
  it.each([
    {
      ...
      env: {
        STEELENGINE_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1",
        STEELENGINE_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1",
      },
      expected: {
        value: true,
        source: "env:STEELENGINE_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "uses env disable when set",
      env: { STEELENGINE_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1" },
      expected: {
        value: false,
        source: "env:STEELENGINE_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "prefers env enable over config",
      env: { STEELENGINE_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1" },
  ...
  it.each([
    {
      ...
      env: { STEELENGINE_TELEGRAM_DNS_RESULT_ORDER: "verbatim" },
      nodeMajor: 22,
      expected: {
        value: "verbatim",
        source: "env:STEELENGINE_TELEGRAM_DNS_RESULT_ORDER",
      },
    },
    {
      name: "normalizes trimmed env values",
      env: { STEELENGINE_TELEGRAM_DNS_RESULT_ORDER: "  IPV4FIRST  " },
      nodeMajor: 20,
      expected: {
        value: "ipv4first",
        source: "env:STEELENGINE_TELEGRAM_DNS_RESULT_ORDER",
    ...
    },
    {
      ...
      env: { STEELENGINE_TELEGRAM_DNS_RESULT_ORDER: "bogus" },
      network: { dnsResultOrder: "ipv4first" },
      nodeMajor: 20,
      expected: { value: "ipv4first", source: "config" },
    },
