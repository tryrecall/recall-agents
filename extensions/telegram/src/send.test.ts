...
const { resetLogger, setLoggerOverride } = await import("recall/plugin-sdk/runtime-env");
const {
  buildInlineKeyboard,
...
function captureInfoLogs(): string {
  logCaptureCounter += 1;
  const logFile = `/tmp/recall-telegram-send-log-${process.pid}-${logCaptureCounter}.jsonl`;
...
describe("sent-message-cache", () => {
  ...
  it("keeps sent-message ownership across restart", async () => {
    const persistedStorePath = `/tmp/recall-telegram-send-tests-${process.pid}-restart.json`;
  ...
  it("keeps expired custom-store cleanup away from the default store", () => {
    const customStorePath = `/tmp/recall-telegram-send-tests-${process.pid}-custom-cleanup.json`;
  ...
  it("keeps default and custom stores isolated while both are loaded", () => {
    const customStorePath = `/tmp/recall-telegram-send-tests-${process.pid}-custom-isolated.json`;
    const customCfg = { session: { store: customStorePath } };
