...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("steelengine/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
type DispatchReplyHarnessParams = Parameters<DispatchReplyWithBufferedBlockDispatcherFn>[0];
type ReadRemoteMediaBufferFn =
  typeof import("steelengine/plugin-sdk/media-runtime").readRemoteMediaBuffer;
...
function ensureMediaHarnessStoreRoot(): string {
  mediaHarnessStoreRoot ??= mkdtempSync(path.join(os.tmpdir(), "steelengine-telegram-media-e2e-"));
...
const defaultRuntimeConfig = (() =>
  ...
  }) as SteelEngineConfig) as TelegramBotDeps["getRuntimeConfig"];
