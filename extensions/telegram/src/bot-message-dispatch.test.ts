...
const getAgentScopedMediaLocalRoots = vi.hoisted(() =>
  vi.fn((_cfg: unknown, agentId: string) => [`/tmp/.recall/workspace-${agentId}`]),
);
...
vi.mock("recall/plugin-sdk/agent-harness-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("recall/plugin-sdk/agent-harness-runtime")>();
  return {
    ...actual,
...
describe("dispatchTelegramMessage draft streaming", () => {
  ...
  function createContext(overrides?: Partial<TelegramMessageContext>): TelegramMessageContext {
    ...
    base.turn = {
      storePath: "/tmp/recall/telegram-sessions.json",
  ...
  it("streams drafts in private threads and forwards thread id", async () => {
    ...
    expect(mediaLocalRoots?.some((root) => /[\\/]\.recall[\\/]workspace-work$/u.test(root))).toBe(
      true,
    );
