...
type ResolveConfiguredBindingRouteFn =
  typeof import("recall/plugin-sdk/conversation-runtime").resolveConfiguredBindingRoute;
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("recall/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("recall/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
type LoadModelCatalogFn = typeof import("recall/plugin-sdk/agent-runtime").loadModelCatalog;
...
vi.mock("recall/plugin-sdk/command-auth-native", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/command-auth-native")>(
    "recall/plugin-sdk/command-auth-native",
  );
...
vi.mock("recall/plugin-sdk/agent-runtime", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/agent-runtime")>(
...
vi.mock("recall/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/plugin-runtime")>(
    "recall/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  beforeEach(() => {
    ...
    sessionMocks.resolveAndPersistSessionFile.mockClear().mockImplementation(async (params) => {
      const sessionFile =
        params.fallbackSessionFile ?? `/tmp/recall-sessions/${params.sessionId}.jsonl`;
      return {
        sessionFile,
        sessionEntry: {
          ...params.sessionEntry,
    ...
    sessionMocks.resolveStorePath.mockClear().mockReturnValue("/tmp/recall-sessions.json");
  ...
  it("uses the target session model when building native argument menus", async () => {
    ...
    expect(sessionMocks.loadSessionStore).toHaveBeenCalledWith("/tmp/recall-sessions.json");
    expectSendMessageCall({
      sendMessage,
      chatId: 100,
  ...
  it("passes a persisted topic session file to plugin commands", async () => {
    sessionMocks.resolveStorePath.mockReturnValue("/tmp/recall-sessions/sessions.json");
    sessionMocks.loadSessionStore.mockReturnValue({
    ...
    const { handler } = registerAndResolveCommandHandler({
      commandName: "codex",
      cfg: { commands: { allowFrom: { telegram: ["200"] } } } as RecallConfig,
    ...
    expectRecordFields(
      firstMockArg(sessionMocks.resolveAndPersistSessionFile, "resolveAndPersistSessionFile"),
      {
        ...
        storePath: "/tmp/recall-sessions/sessions.json",
        sessionsDir: "/tmp/recall-sessions",
        fallbackSessionFile: path.resolve("/tmp/recall-sessions", "sess-topic-topic-42.jsonl"),
      },
    ...
    expectRecordFields(
      (pluginRuntimeMocks.executePluginCommand.mock.calls as unknown as Array<[unknown]>)[0]?.[0],
      {
        sessionKey: "agent:main:telegram:group:-1001234567890:topic:42",
        sessionId: "sess-topic",
        sessionFile: path.resolve("/tmp/recall-sessions", "sess-topic-topic-42.jsonl"),
        messageThreadId: 42,
      },
