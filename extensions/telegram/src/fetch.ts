...
import { resolveRequestUrl } from "recall/plugin-sdk/request-url";
...
// fix for the leak observed in recall#68128 is the transport lifecycle that
...
function resolveRecallProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.RECALL_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveRecallProxyUrlForTelegram() : undefined;
