...
import { resolveRequestUrl } from "steelengine/plugin-sdk/request-url";
...
// fix for the leak observed in steelengine#68128 is the transport lifecycle that
...
function resolveSteelEngineProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.STEELENGINE_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveSteelEngineProxyUrlForTelegram() : undefined;
