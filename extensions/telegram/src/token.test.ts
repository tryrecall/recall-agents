...
describe("resolveTelegramToken", () => {
  ...
  function createTempDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-telegram-token-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  function createUnknownAccountConfig(): RecallConfig {
    return {
      channels: {
        telegram: {
  ...
  function expectNoTokenForUnknownAccount(cfg: RecallConfig) {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked tokenFile paths", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile: tokenLink } } } as RecallConfig;
  ...
  // Regression: https://github.com/tryrecall/recall-agents/issues/53876
...
describe("telegram update offset store", () => {
  it("persists and reloads the last update id", async () => {
    await withStateDirEnv("recall-telegram-", async () => {
