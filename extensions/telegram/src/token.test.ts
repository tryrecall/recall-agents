...
describe("resolveTelegramToken", () => {
  ...
  function createTempDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-telegram-token-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  function createUnknownAccountConfig(): SteelEngineConfig {
    return {
      channels: {
        telegram: {
  ...
  function expectNoTokenForUnknownAccount(cfg: SteelEngineConfig) {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const cfg = { channels: { telegram: { tokenFile: tokenLink } } } as SteelEngineConfig;
  ...
  // Regression: https://github.com/steelengineai/recall-agents/issues/53876
