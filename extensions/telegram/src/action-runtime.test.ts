...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): SteelEngineConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as SteelEngineConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): SteelEngineConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  function topicCacheScopeFor(cfg: SteelEngineConfig, accountId: string): string {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["STEELENGINE_STATE_DIR", "TELEGRAM_BOT_TOKEN"]);
    resetTelegramTopicNameCacheForTest();
  ...
  it("persists sendMessage action deliveries before Telegram platform send", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-telegram-action-durable-"));
    const {
      createOutboundTestPlugin,
      createTestRegistry,
      readQueuedDeliveryEntriesForTest,
      setActivePluginRegistry,
    } = await import("steelengine/plugin-sdk/plugin-test-runtime");
  ...
  it("stores created forum topic names in the account-scoped cache", async () => {
    ...
    const cfg = {
      ...
      session: { store: path.join(os.tmpdir(), "steelengine-telegram-action-sessions.json") },
    } as SteelEngineConfig;
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: SteelEngineConfig, accountId = "media") {
    await handleTelegramAction(
