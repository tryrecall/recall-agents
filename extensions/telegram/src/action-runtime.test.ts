...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): RecallConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as RecallConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): RecallConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: RecallConfig, accountId = "media") {
    await handleTelegramAction(
