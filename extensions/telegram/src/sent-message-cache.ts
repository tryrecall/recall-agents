...
import { replaceFileAtomicSync } from "recall/plugin-sdk/security-runtime";
import { resolveStorePath } from "recall/plugin-sdk/session-store-runtime";

const TTL_MS = 24 * 60 * 60 * 1000;
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("recall.telegramSentMessagesState");
...
function resolveSentMessageStorePath(cfg?: Pick<RecallConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<RecallConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<RecallConfig, "session">): SentMessageStore {
