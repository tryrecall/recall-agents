...
const { recordInboundSessionMock, resolveStorePathMock } = vi.hoisted(() => ({
  ...
  resolveStorePathMock: vi.fn<ResolveStorePathFn>(() => "/tmp/recall-session-store.json"),
}));
...
const { clearRuntimeConfigSnapshot, setRuntimeConfigSnapshot } =
  await import("recall/plugin-sdk/runtime-config-snapshot");
...
afterEach(() => {
  ...
  resolveStorePathMock.mockReturnValue("/tmp/recall-session-store.json");
});
  ...
  it("uses thread session key for DM topics when dm.threadReplies is inbound", async () => {
    const ctx = await buildContext(
      ...
      },
      {
        cfg: {
          agents: {
            defaults: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/recall" },
          },
          channels: {
            telegram: {
              dmPolicy: "open",
...
describe("buildTelegramMessageContext group sessions without forum", () => {
  ...
  it("does not add a topic-cache store lookup for non-forum group reply threads", async () => {
    const resolveStorePath = vi.fn(() => "/tmp/recall/session-store.json");
  ...
  it("reloads topic name from disk after cache reset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-telegram-topic-name-"));
