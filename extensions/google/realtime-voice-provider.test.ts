...
import type { RealtimeVoiceTool } from "steelengine/plugin-sdk/realtime-voice";
...
describe("buildGoogleRealtimeVoiceProvider", () => {
  ...
  it("uses Gemini 3.1 Live-compatible defaults", async () => {
    ...
    const bridge = provider.createBridge({
      ...
      tools: [createRealtimeTool("steelengine_agent_consult")],
      onAudio: vi.fn(),
      onClearAudio: vi.fn(),
    });
  ...
  it("connects with Google Live setup config and tool declarations", async () => {
    ...
    expect(declarations[1]?.name).toBe("steelengine_agent_consult");
    expect(declarations[1]?.description).toBe("Ask SteelEngine");
  ...
  it("creates constrained browser sessions for Google Live Talk", async () => {
    ...
    expect(declaration?.name).toBe("steelengine_agent_consult");
  ...
  it("keeps Google Live consult calls open after continuing tool responses", async () => {
    ...
    lastConnectParams().callbacks.onmessage({
      ...
      toolCall: {
        functionCalls: [
          { id: "consult-call", name: "steelengine_agent_consult", args: { prompt: "hi" } },
        ],
      },
    });
