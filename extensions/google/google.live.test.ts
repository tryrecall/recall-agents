...
import { normalizeTranscriptForMatch } from "recall/plugin-sdk/provider-test-contracts";
...
describeLive("google plugin live", () => {
  it("synthesizes speech through the registered provider", async () => {
    ...
    const audioFile = await provider.synthesize({
      text: "Recall Google text to speech integration test OK.",
  ...
  it("transcodes speech to Opus for voice-note targets", async () => {
    ...
    const audioFile = await provider.synthesize({
      text: "Recall Google voice note integration test OK.",
      cfg: { plugins: { enabled: true } } as never,
      providerConfig: { apiKey: GOOGLE_API_KEY },
      target: "voice-note",
      timeoutMs: 90_000,
  ...
  it("runs Gemini web search through the registered provider tool", async () => {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        result = await tool?.execute({ query: "Recall GitHub", count: 1 });
        lastError = undefined;
        break;
      } catch (error) {
        lastError = error;
  ...
  it("runs Gemini web search through the Google model provider config fallback", async () => {
    await withGoogleApiEnvUnset(async () => {
      ...
      const result = await tool?.execute({ query: "Recall GitHub", count: 1 });
