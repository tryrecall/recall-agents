...
vi.mock("steelengine/plugin-sdk/provider-transport-runtime", async (importOriginal) => ({
...
const MODEL_PROVIDER_REQUEST_TRANSPORT_SYMBOL = Symbol.for(
  "steelengine.modelProviderRequestTransport",
);
...
describe("google transport stream", () => {
  ...
  afterAll(() => {
    vi.doUnmock("steelengine/plugin-sdk/provider-transport-runtime");
  ...
  it("uses the guarded fetch transport and parses Gemini SSE output", async () => {
    ...
    expect(new Headers(init.headers).get("x-goog-api-client")).toMatch(/^steelengine\//u);
  ...
  it("rotates Gemini LLM API keys when a pre-stream request is rate limited", async () => {
    vi.stubEnv("STEELENGINE_LIVE_GEMINI_KEY", "");
  ...
  it("retries Gemini 3 requests with lean thinking when the first attempt has no first response", async () => {
    vi.stubEnv("STEELENGINE_GOOGLE_GEMINI_FIRST_RESPONSE_RETRY_MS", "10");
    guardedFetchMock
  ...
  it.each([
    ...
    async (location, origin) => {
      const tempDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-google-vertex-region-"));
  ...
  it("resolves non-file Vertex ADC through google-auth-library without OAuth refresh fetch", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-google-vertex-authlib-"));
    vi.stubEnv("GOOGLE_APPLICATION_CREDENTIALS", "");
    vi.stubEnv("HOME", path.join(tempDir, "home"));
    vi.stubEnv("APPDATA", "");
  ...
  it("bounds Google Vertex ADC files before google-auth-library reads them", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-google-vertex-adc-file-"));
  ...
  it("bounds google-auth-library ADC token resolution at the Vertex owner", async () => {
    const tempDir = await mkdtemp(
      path.join(os.tmpdir(), "steelengine-google-vertex-authlib-timeout-"),
    );
    vi.stubEnv("GOOGLE_APPLICATION_CREDENTIALS", "");
    vi.stubEnv("HOME", path.join(tempDir, "home"));
    vi.stubEnv("APPDATA", "");
  ...
  it("does not cache google-auth ADC tokens when fallback expiry would exceed Date range", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-google-vertex-authlib-expiry-"));
    vi.useFakeTimers();
  ...
  it("uses google-auth-library bearer auth for Google Vertex credential marker requests", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-google-vertex-authlib-stream-"));
    vi.stubEnv("GOOGLE_APPLICATION_CREDENTIALS", "");
    vi.stubEnv("HOME", path.join(tempDir, "home"));
    vi.stubEnv("APPDATA", "");
  ...
  it("strips redundant google provider prefixes from Google Vertex model paths", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-google-vertex-prefix-"));
    vi.stubEnv("HOME", path.join(tempDir, "home"));
    vi.stubEnv("APPDATA", "");
    vi.stubEnv("GOOGLE_CLOUD_PROJECT", "vertex-project");
    vi.stubEnv("GOOGLE_CLOUD_LOCATION", "us-central1");
  ...
  it("times out an authorized_user ADC token refresh", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-google-vertex-adc-timeout-"));
    const credentialsPath = path.join(tempDir, "application_default_credentials.json");
    await writeFile(
      credentialsPath,
      JSON.stringify({
  ...
  it("refreshes authorized_user ADC from a compressed token response", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-google-vertex-adc-gzip-"));
    const credentialsPath = path.join(tempDir, "application_default_credentials.json");
    await writeFile(
      credentialsPath,
      JSON.stringify({
  ...
  it("rejects oversized authorized_user ADC token responses", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-google-vertex-adc-large-"));
    const credentialsPath = path.join(tempDir, "application_default_credentials.json");
    await writeFile(
      credentialsPath,
      JSON.stringify({
  ...
  it("rejects authorized_user ADC gzip responses that expand past the limit", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-google-vertex-adc-bomb-"));
    const credentialsPath = path.join(tempDir, "application_default_credentials.json");
    await writeFile(
      credentialsPath,
      JSON.stringify({
  ...
  it("does not reuse authorized_user ADC tokens with unsafe expiry lifetimes", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-google-vertex-unsafe-adc-"));
    const credentialsPath = path.join(tempDir, "application_default_credentials.json");
    await writeFile(
      credentialsPath,
      JSON.stringify({
  ...
  it("refreshes authorized_user ADC from the Windows APPDATA fallback for Google Vertex requests", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-google-vertex-appdata-adc-"));
  ...
  it("treats the Google transport alias as the same route for signature replay", () => {
    ...
    } as Model<"steelengine-google-generative-ai-transport">;
