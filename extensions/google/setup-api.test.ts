...
import type { CliBackendPlugin } from "steelengine/plugin-sdk/cli-backend";
...
describe("google gemini cli backend auth bridge", () => {
  it("materializes selected SteelEngine OAuth credentials into a persistent profile-scoped Gemini CLI home", async () => {
    const backend = buildGoogleGeminiCliBackend();
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-test-workspace-"));
    ...
    try {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify({
          ...
          mcp: { allowed: ["steelengine"] },
          mcpServers: { steelengine: { url: "http://127.0.0.1:23119/mcp" } },
        })}\n`,
        "utf8",
      );
      ...
      expect(
        path.relative(resolvePreferredSteelEngineTmpDir(), path.dirname(systemSettingsPath ?? "")),
      ).toMatch(/^steelengine-gemini-cli-/);
  ...
  it("stages Gemini CLI JSON through same-directory atomic renames", async () => {
    await withTempDir("steelengine-test-workspace-", async (workspaceDir) => {
      const backend = buildGoogleGeminiCliBackend();
