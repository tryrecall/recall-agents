...
import { asObject, trimToUndefined } from "steelengine/plugin-sdk/speech-core";
...
function resolveGoogleTtsModelProviderApiKey(cfg?: SteelEngineConfig): string | undefined {
...
function isSteelEngineGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isSteelEngineGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
