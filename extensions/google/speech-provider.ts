...
function resolveGoogleTtsModelProviderApiKey(cfg?: RecallConfig): string | undefined {
...
function isRecallGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isRecallGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
