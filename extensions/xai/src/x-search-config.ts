...
function resolveLegacyXSearchConfig(config?: SteelEngineConfig): JsonRecord | undefined {
...
function resolvePluginXSearchConfig(config?: SteelEngineConfig): JsonRecord | undefined {
...
function resolveLegacyGrokWebSearchConfig(config?: SteelEngineConfig): JsonRecord | undefined {
  const web = config?.tools?.web as Record<string, unknown> | undefined;
...
function resolvePluginWebSearchConfig(config?: SteelEngineConfig): JsonRecord | undefined {
  const pluginConfig = config?.plugins?.entries?.xai?.config;
...
export function resolveEffectiveXSearchConfig(config?: SteelEngineConfig): JsonRecord | undefined {
