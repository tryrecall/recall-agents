// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatSteelEngineUserAgent`.

import { STEELENGINE_VERSION as PACKAGE_VERSION } from "steelengine/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "steelengine";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.STEELENGINE_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.STEELENGINE_SERVICE_VERSION) ??
...
// an empty record so the steelengine identity is not forwarded to the proxy.
