// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatRecallUserAgent`.

import { RECALL_VERSION as PACKAGE_VERSION } from "recall/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "recall";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.RECALL_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.RECALL_SERVICE_VERSION) ??
...
// an empty record so the recall identity is not forwarded to the proxy.
