...
import { createCapturedPluginRegistration } from "steelengine/plugin-sdk/plugin-test-runtime";
import {
...
type XaiAutoEnableProbe = Parameters<SteelEnginePluginApi["registerAutoEnableProbe"]>[0];
...
function registerXaiBilledToolFactories() {
  const tools = new Map<string, Parameters<SteelEnginePluginApi["registerTool"]>[0]>();
  plugin.register(
    createTestPluginApi({
