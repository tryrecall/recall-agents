...
const {
  ...
  resolveProviderHttpRequestConfigMock: vi.fn((params: Record<string, unknown>) => {
    ...
    if (params.provider === "xai") {
      const version = process.env.RECALL_VERSION?.trim() || "unknown";
      headers.set("User-Agent", `recall/${version}`);
      headers.set("originator", "recall");
...
vi.mock("recall/plugin-sdk/provider-http", () => ({
...
describe("xai image generation provider", () => {
  ...
  it("builds provider with correct models, default, and capabilities", () => {
    ...
    expect(isConfigured({ agentDir: "/tmp/recall-xai-test" })).toBe(true);
    expect(isProviderApiKeyConfiguredMock).toHaveBeenCalledWith({
      provider: "xai",
      agentDir: "/tmp/recall-xai-test",
    });
  });
  ...
  it("forwards xAI attribution User-Agent through the SDK image request", async () => {
    ...
    const request = requirePostJsonCall();
    expect(request.headers?.get("user-agent")).toBe("recall/2026.3.22");
    expect(request.headers?.get("originator")).toBe("recall");
