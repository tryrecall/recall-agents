...
async function withMockedOpenAiMemoryPlugin<T>(params: {
  ...
  vi.resetModules();
  vi.doMock("recall/plugin-sdk/runtime-env", () => ({
...
describe("memory plugin e2e", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "recall-memory-test-" });
  ...
  test("uses provider adapter auth when embedding apiKey is omitted", async () => {
    ...
    vi.resetModules();
    vi.doMock("recall/plugin-sdk/memory-core-host-engine-embeddings", () => ({
    ...
    try {
      ...
      const mockApi = {
        ...
        runtime: {
          ...
          agent: {
            resolveAgentDir: vi.fn(() => "/tmp/recall-agent"),
          },
        },
        logger: {
          info: vi.fn(),
      ...
      expect(providerOptions.agentDir).toBe("/tmp/recall-agent");
      ...
      vi.doUnmock("recall/plugin-sdk/memory-core-host-engine-embeddings");
      vi.doUnmock("openai");
      vi.doUnmock("./lancedb-runtime.js");
      vi.resetModules();
    }
