...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.RECALL_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("recall-gw");
  ...
  it("honors truthy RECALL_DISABLE_BONJOUR values", async () => {
    enableAdvertiserUnitMode();
    process.env.RECALL_DISABLE_BONJOUR = "true";
  ...
  it("auto-disables Bonjour on Fly Machines without Docker sentinel files", async () => {
    ...
    process.env.FLY_APP_NAME = "recall-clawcks-test";
  ...
  it("honors explicit Bonjour opt-in inside detected containers", async () => {
    enableAdvertiserUnitMode();
    process.env.RECALL_DISABLE_BONJOUR = "0";
    vi.spyOn(fs, "existsSync").mockImplementation((filePath) => String(filePath) === "/.dockerenv");
  ...
  it("logs ciao handler classifications at the bonjour caller", async () => {
    ...
    expect(
      handler?.(
        new Error(
          "Can't probe for a service which is announced already. Received announcing for service Recall Gateway._openclaw._tcp.local.",
        ),
      ),
    ).toBe(true);
  ...
  it("defers probing recovery while a name conflict is still settling", async () => {
    ...
    await vi.advanceTimersByTimeAsync(10_000);
    listenerMap.get("name-change")?.("test-host (Recall) (2)");

    await vi.advanceTimersByTimeAsync(15_000);

    expect(createService).toHaveBeenCalledTimes(1);
    expectWarnContaining('name conflict resolved; newName="test-host (Recall) (2)"');
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("Mac (Recall)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
  ...
  it("falls back to recall when system hostname is invalid for DNS", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.RECALL_MDNS_HOSTNAME;
    ...
    const [gatewayCall] = createService.mock.calls as Array<[ServiceCall]>;
    expect(gatewayCall?.[0]?.hostname).toBe("recall");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("recall.local");
  ...
  it("truncates reported Kubernetes service name at the DNS label byte limit", async () => {
    ...
    expectDnsLabelByteLength(`${reportedHostname} (Recall)`, 64);
  ...
  it("truncates multi-byte hostname within DNS label byte limit", async () => {
    // 21 CJK characters = 63 bytes in UTF-8, adding " (Recall)" pushes over
  ...
  it("uses system hostname when RECALL_MDNS_HOSTNAME is unset", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.RECALL_MDNS_HOSTNAME;
