...
/** Input data used to publish SteelEngine gateway Bonjour records. */
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.STEELENGINE_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "SteelEngine"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "SteelEngine";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(SteelEngine\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.STEELENGINE_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "steelengine";
    ...
    const hostnameLabel = hostnameWithoutLocal.slice(0, labelEnd).trim() || "steelengine";
    const hostname = truncateToDnsLabel(hostnameLabel, "steelengine");
    const instanceName =
      typeof opts.instanceName === "string" && opts.instanceName.trim()
        ? opts.instanceName.trim()
        : `${hostname} (SteelEngine)`;
