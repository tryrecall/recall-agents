...
import type { PluginLogger } from "recall/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "recall/plugin-sdk/runtime-env";
...
// See https://github.com/tryrecall/recall-agents/issues/72481
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.RECALL_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "Recall"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "Recall";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Recall\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.RECALL_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "recall";
    ...
    const recreateAdvertiser = async (reason: string, opts?: { stuckState?: boolean }) => {
      ...
      recreatePromise = (async () => {
        ...
        if (tooManyConsecutive || tooManyStuckStates || tooManyInWindow) {
          ...
          logger.warn(
            `bonjour: disabling advertiser after ${detail} (${reason}); set discovery.mdns.mode="off" or RECALL_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );
