...
type RegisteredMemoryEmbeddingProvider = Parameters<
  RecallPluginApi["registerMemoryEmbeddingProvider"]
>[0];
type GithubCopilotTestProvider = {
  auth: Array<{
    run: (ctx: unknown) => Promise<ProviderAuthResult | null>;
    runNonInteractive: (ctx: unknown) => Promise<RecallConfig | null>;
  }>;
  catalog: {
...
async function createAgentDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-github-copilot-test-"));
  tempDirs.push(dir);
  return dir;
}
...
function registerProviderAndCatalogWithPluginConfig(pluginConfig: Record<string, unknown>) {
  const registerProviderMock = vi.fn<RecallPluginApi["registerProvider"]>();
  const registerModelCatalogProviderMock =
    vi.fn<RecallPluginApi["registerModelCatalogProvider"]>();
...
describe("github-copilot plugin", () => {
  it("registers embedding provider", () => {
    const registerMemoryEmbeddingProviderMock =
      vi.fn<RecallPluginApi["registerMemoryEmbeddingProvider"]>();
