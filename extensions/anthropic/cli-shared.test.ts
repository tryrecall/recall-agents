...
describe("resolveClaudeCliAutoCompactEnv", () => {
  it("maps the effective SteelEngine context budget into Claude Code compaction", () => {
...
describe("resolveClaudeCliExecutionArgs", () => {
  it("isolates SteelEngine from Claude user customizations while preserving exact MCP", () => {
    expect(
      resolveClaudeCliExecutionArgs({
        workspaceDir: "/tmp",
        provider: "claude-cli",
        ...
        baseArgs: [
          ...
          "--strict-mcp-config",
          "--mcp-config",
          "/tmp/steelengine-steelengine-mcp.json",
          "--resume",
          "native-session",
          "--tools",
          "Bash,Edit",
      ...
      "",
      "--allowedTools",
      "mcp__steelengine__steelengine",
    ]);
  });

  it("leaves non-SteelEngine customization args intact under generic tool availability", () => {
    expect(
      resolveClaudeCliExecutionArgs({
        workspaceDir: "/tmp",
        provider: "claude-cli",
    ...
    expect(
      resolveClaudeCliExecutionArgs({
        ...
        baseArgs: [
          ...
          "--output-format",
          "stream-json",
          "--allowedTools=mcp__steelengine__*",
          "--allowedTools",
          "Read",
          "Grep",
          "--permission-mode",
...
describe("normalizeClaudeBackendConfig", () => {
  ...
  it("derives Claude bypass from SteelEngine YOLO policy and disables it for safer policy", () => {
  ...
  it("derives Claude bypass from per-agent SteelEngine exec policy", () => {
    expect(
      normalizeClaudeArgs(["-p"], {
        backendId: "claude-cli",
        agentId: "safe-agent",
  ...
  it("passes system prompt on every turn (issue #80374 — systemPromptWhen must be 'always')", () => {
    ...
    // SteelEngine system prompt context on resumed / compacted claude-cli sessions.
    const backend = buildAnthropicCliBackend();
  ...
  it("passes the effective context budget to Claude Code's native compactor", () => {
    ...
    expect(
      backend.prepareExecution?.({
        workspaceDir: "/tmp/steelengine-claude-cli",
        provider: "claude-cli",
        modelId: "claude-opus-4-7",
