 ...
 */
import { formatCliCommand, parseDurationMs } from "steelengine/plugin-sdk/cli-runtime";
...
import {
  ...
  type SteelEngineConfig as ProviderAuthConfig,
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("steelengine models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("steelengine models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("steelengine doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: SteelEnginePluginApi): void {
