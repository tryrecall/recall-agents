import { formatCliCommand, parseDurationMs } from "recall/plugin-sdk/cli-runtime";
import type {
  RecallPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type RecallConfig as ProviderAuthConfig,
...
import { fetchClaudeUsage } from "recall/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("recall models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("recall models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("recall doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: RecallPluginApi): void {
