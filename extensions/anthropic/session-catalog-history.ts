...
import { withSessionTranscriptWriteLock } from "steelengine/plugin-sdk/session-transcript-runtime";
...
function importedClaudeMessage(
  ...
  if (item.type === "userMessage") {
    // Imported native rows are not SteelEngine-authored; mirrorOrigin excludes them
    // from self-echo provenance so a repeated native prompt stays observable.
    return {
      role: "user",
      content: text,
      timestamp,
      __steelengine: { mirrorOrigin: "claude-catalog-import" },
    } as AgentMessage;
  }
  const prefix =
    item.type === "reasoning"
