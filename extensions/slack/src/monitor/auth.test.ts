...
vi.mock("steelengine/plugin-sdk/channel-ingress-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("steelengine/plugin-sdk/channel-ingress-runtime")
  >("steelengine/plugin-sdk/channel-ingress-runtime");
  return {
    ...actual,
...
describe("authorizeSlackSystemEventSender", () => {
  ...
  beforeEach(() => {
    readChannelIngressStoreAllowFromForDmPolicyMock.mockReset();
    delete process.env.STEELENGINE_SLACK_CHANNEL_MEMBERS_CACHE_TTL_MS;
  });
  ...
  it("ignores non-decimal channel member cache ttl env values", async () => {
    process.env.STEELENGINE_SLACK_CHANNEL_MEMBERS_CACHE_TTL_MS = "0x0";
