...
import type { SlackSlashCommandConfig } from "steelengine/plugin-sdk/config-contracts";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(
    normalizeOptionalString(raw?.name) ?? "steelengine",
  );
  const name = normalizedName || "steelengine";
  return {
