import type { SlackSlashCommandConfig } from "recall/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "recall");
  const name = normalizedName || "recall";
  return {
