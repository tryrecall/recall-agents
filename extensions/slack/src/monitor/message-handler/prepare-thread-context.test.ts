...
import { resolveEnvelopeFormatOptions } from "recall/plugin-sdk/channel-inbound";
...
describe("resolveSlackThreadContextData", () => {
  const storeFixture = createSlackSessionStoreFixture("recall-slack-thread-context-");
  ...
  async function resolveAllowlistedThreadContext(params: {
    ...
    const result = await resolveSlackThreadContextData({
      ...
      envelopeOptions: resolveEnvelopeFormatOptions({} as RecallConfig),
