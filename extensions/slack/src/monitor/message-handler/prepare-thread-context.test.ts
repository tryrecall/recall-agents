...
import { resolveEnvelopeFormatOptions } from "steelengine/plugin-sdk/channel-inbound";
...
describe("resolveSlackThreadContextData", () => {
  const storeFixture = createSlackSessionStoreFixture("steelengine-slack-thread-context-");
  ...
  async function resolveAllowlistedThreadContext(params: {
    ...
    const result = await resolveSlackThreadContextData({
      ...
      envelopeOptions: resolveEnvelopeFormatOptions({} as SteelEngineConfig),
