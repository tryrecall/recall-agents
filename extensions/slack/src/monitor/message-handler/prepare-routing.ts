...
type SlackRouteBinding = NonNullable<SteelEngineConfig["bindings"]>[number];
type SlackRouteBindingPeer = NonNullable<SlackRouteBinding["match"]["peer"]>;

const slackRouteBindingConfigCache = new WeakMap<
  SteelEngineConfig,
  { bindingsRef: SteelEngineConfig["bindings"]; normalizedCfg: SteelEngineConfig }
>();
...
function normalizeSlackRouteBindingConfig(cfg: SteelEngineConfig): SteelEngineConfig {
  ...
  const normalizedCfg = changed
    ? ({ ...cfg, bindings: normalizedBindings } as SteelEngineConfig)
    : cfg;
