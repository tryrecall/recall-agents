...
type SlackRouteBinding = NonNullable<RecallConfig["bindings"]>[number];
type SlackRouteBindingPeer = NonNullable<SlackRouteBinding["match"]["peer"]>;

const slackRouteBindingConfigCache = new WeakMap<
  RecallConfig,
  { bindingsRef: RecallConfig["bindings"]; normalizedCfg: RecallConfig }
>();
...
function normalizeSlackRouteBindingConfig(cfg: RecallConfig): RecallConfig {
  ...
  const normalizedCfg = changed
    ? ({ ...cfg, bindings: normalizedBindings } as RecallConfig)
    : cfg;
