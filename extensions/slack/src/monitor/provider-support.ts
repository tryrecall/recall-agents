...
const RECALL_SLACK_CLIENT_PING_TIMEOUT_MS = 15_000;
const RECALL_SLACK_SOCKET_START_FAILED_EVENT = "unable_to_socket_mode_start";
const RECALL_SLACK_NATIVE_RECONNECT_OBSERVER_KEY = "__openclawNativeReconnectFailureObserver";
...
function installSlackNativeReconnectFailureObserver(receiver: unknown) {
  ...
  if (Reflect.get(client, RECALL_SLACK_NATIVE_RECONNECT_OBSERVER_KEY)) {
    return;
  }
  ...
  Reflect.set(client, RECALL_SLACK_NATIVE_RECONNECT_OBSERVER_KEY, true);
  Reflect.set(
    client,
    ...
    function patchedDelayReconnectAttempt(this: object, callback: unknown) {
      ...
      const delayMs =
        (Number.isFinite(pingTimeoutMs) && pingTimeoutMs >= 0
          ? pingTimeoutMs
          : RECALL_SLACK_CLIENT_PING_TIMEOUT_MS) * nextFailureCount;
        ...
        setTimeout(() => {
          ...
          Promise.resolve(callback.call(this)).then(resolve, (error: unknown) => {
            if (callback === Reflect.get(this, "start")) {
              emit.call(this, RECALL_SLACK_SOCKET_START_FAILED_EVENT, error);
              resolve(undefined);
              return;
            }
            reject(error);
...
export function shouldSkipRecallSlackSelfEvent(args: SlackSelfFilterArgs): boolean {
...
export function createSlackBoltApp(params: {
  ...
  const socketModeReceiverOptions: SlackSocketModeReceiverOptions = {
    ...
    clientPingTimeout:
      params.socketMode?.clientPingTimeout ?? RECALL_SLACK_CLIENT_PING_TIMEOUT_MS,
  ...
  const app = new params.interop.App({
    ...
    // consumes that promise, tripping Recall's fatal unhandled-rejection path.
    tokenVerificationEnabled: false,
  });
  app.use(async (args) => {
    if (shouldSkipRecallSlackSelfEvent(args)) {
      return;
    }
    await args.next();
  });
