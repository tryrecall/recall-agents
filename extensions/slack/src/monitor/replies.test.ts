...
vi.mock("steelengine/plugin-sdk/hook-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("steelengine/plugin-sdk/hook-runtime")>();
  return {
    ...actual,
    triggerInternalHook,
  };
...
vi.mock("steelengine/plugin-sdk/plugin-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("steelengine/plugin-sdk/plugin-runtime")>();
  return {
    ...actual,
...
describe("deliverReplies identity passthrough", () => {
  ...
  it("delivers media before native chart blocks with the same reply context", async () => {
    ...
    const metadata = { event_type: "steelengine_test", event_payload: { source: "chart" } };
  ...
  it("renders interactive replies into Slack blocks during delivery", async () => {
    ...
    expect(blocks[1]?.elements?.[0]?.action_id).toBe("steelengine:reply_button:1:1");
