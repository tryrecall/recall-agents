...
// Prefix for Recall-generated action IDs to scope our handler
const RECALL_ACTION_PREFIX = "recall:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${RECALL_ACTION_PREFIX}`);

  // Handle Recall modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
