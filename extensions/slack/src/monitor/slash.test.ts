...
function createArgMenusHarness(
  cfg: SteelEngineConfig = { commands: { native: true, nativeSkills: false } },
) {
...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, /^steelengine_cmdarg/, "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "steelengine_cmdarg", "arg-menu options");
  });
  ...
  it("prefers the configured slash command over native commands", async () => {
    ...
    expect(
      [...configuredHarness.commands.keys()].some(
        (command) => command instanceof RegExp && command.test("/steelengine"),
      ),
    ).toBe(true);
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(
      Array.from(testHarness.actions.keys()).some(
        (key) => key instanceof RegExp && String(key) === String(/^steelengine_cmdarg/),
      ),
    ).toBe(true);
    expect(testHarness.options.has("steelengine_cmdarg")).toBe(true);
    expect(testHarness.optionsReceiverContexts[0]).toBe(testHarness.app);
  });

  it.each(["codex", "steelengine"] as const)(
  ...
  it("shows a button menu when required args are omitted", async () => {
    ...
    expect(actions?.elements?.[0]?.action_id).toBe("steelengine_cmdarg_0_0");
    expect(actions?.elements?.[1]?.action_id).toBe("steelengine_cmdarg_0_1");
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("steelengine_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("steelengine_cmdarg");
    expect(blockId).toContain("steelengine_cmdarg_ext:");
    const token = (blockId ?? "").slice("steelengine_cmdarg_ext:".length);
...
function createPolicyHarness(overrides?: {
  ...
  const ctx = {
    ...
    slashCommand: {
      ...
      name: overrides?.slashCommandName ?? "steelengine",
...
describe("slack slash command session metadata", () => {
  ...
  it("refreshes slash routing config between invocations", async () => {
    ...
    const sourceCfg = (harness.ctx as { cfg: SteelEngineConfig }).cfg;
    const runtimeCfg = {
      ...sourceCfg,
      session: { dmScope: "per-channel-peer" },
    } as SteelEngineConfig;
    resolveAgentRouteMock.mockImplementation((params: { cfg: SteelEngineConfig }) => ({
      agentId: "main",
      accountId: "acct",
      sessionKey:
