...
import { patchChannelConfigForAccount } from "recall/plugin-sdk/setup-runtime";
import { formatDocsLink } from "recall/plugin-sdk/setup-tools";
import type { ResolvedSlackAccount } from "./accounts.js";
import type { RecallConfig } from "./channel-api.js";

export const SLACK_CHANNEL = "slack" as const;

export function buildSlackManifest(botName = "Recall") {
  const safeName = botName.trim() || "Recall";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Recall`,
    },
    features: {
      bot_user: {
      ...
      assistant_view: {
        assistant_description: `${safeName} connects Slack assistant threads to Recall agents.`,
        suggested_prompts: [
          {
