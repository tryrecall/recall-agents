...
import { expectPairingReplyText } from "steelengine/plugin-sdk/channel-test-helpers";
import { HISTORY_CONTEXT_MARKER } from "steelengine/plugin-sdk/reply-history";
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\bsteelengine\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("steelengine: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("steelengine: hello <@U2>");
  });
