...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): RecallConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as RecallConfig;
  ...
  function expectLastSlackSend(content: string, cfg: RecallConfig, threadTs?: string) {
  ...
  async function resolveReadToken(cfg: RecallConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: RecallConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
  ...
  it("returns non-image downloadFile results as file metadata instead of image content", async () => {
    downloadSlackFile.mockResolvedValueOnce({
      path: "/tmp/recall-media/report.pdf",
    ...
    expect(String(firstContent.text)).toContain("/tmp/recall-media/report.pdf");
    ...
    expect(details.media).toEqual({
      mediaUrl: "/tmp/recall-media/report.pdf",
      contentType: "application/pdf",
    });
  });
