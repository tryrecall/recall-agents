...
async function getSlackConfiguredState(cfg: RecallConfig) {
...
describe("slackPlugin status", () => {
  it("uses the direct Slack probe helper when runtime is not initialized", async () => {
    const probeSpy = vi.spyOn(probeModule, "probeSlack").mockResolvedValueOnce({
      ok: true,
      status: 200,
      bot: { id: "B1", name: "recall-bot" },
      team: { id: "T1", name: "Recall" },
    });
    clearSlackRuntime();
    const cfg = {
      channels: {
  ...
  it("keeps Slack MPIM outbound routing as group", async () => {
    ...
    const route = await resolveRoute({
      cfg: { channels: { slack: { botToken: "xoxb-test" } } } as RecallConfig,
      agentId: "main",
      target: "G123",
    });
...
describe("slackPlugin agentPrompt", () => {
  it("tells agents interactive replies are disabled by default", () => {
    ...
    expect(hints).toContain(
      "- Slack plain text sends: write standard Markdown; Recall converts it to Slack mrkdwn, including `**bold**`, headings, lists, and `[label](url)` links.",
    );
    expect(hints).toContain(
