...
//
// `steelengine/plugin-sdk/channel-policy` is intentionally NOT lazy here —
...
const EXTENSION_SHARED_MODULE_ID = "steelengine/plugin-sdk/extension-shared";
const TARGET_RESOLVER_RUNTIME_MODULE_ID = "steelengine/plugin-sdk/target-resolver-runtime";
  ...
  base: {
    ...
    conversationBindings: {
      isCurrentConversationBindingSupported: ({ accountId }) => {
        const cfg = getOptionalSlackRuntime()?.config.current() as SteelEngineConfig | undefined;
    ...
    actions: createSlackActions(SLACK_CHANNEL, {
      invoke: async (action, cfg, toolContext) =>
        ...
        )(action, cfg as SteelEngineConfig, toolContext as SlackActionContext | undefined),
    }),
