...
const {
  ...
  resolveMatrixQaRecallCliEntryPath,
  runMatrixQaRecallCli,
  startMatrixQaRecallCli,
} = vi.hoisted(() => ({
  formatMatrixQaCliCommand: (args: string[]) => `recall ${args.join(" ")}`,
  redactMatrixQaCliOutput: (text: string) => text,
  resolveMatrixQaRecallCliEntryPath: (cwd: string) => `${cwd}/dist/index.js`,
  runMatrixQaRecallCli: vi.fn(),
  startMatrixQaRecallCli: vi.fn(),
}));
...
describe("matrix live qa scenarios", () => {
  beforeEach(() => {
    ...
    runMatrixQaRecallCli.mockReset();
    startMatrixQaRecallCli.mockReset();
  ...
  it("merges default and scenario-requested Matrix topology once per run", () => {
    expect(
      scenarioTesting.buildMatrixQaTopologyForScenarios({
        defaultRoomName: "Recall Matrix QA run",
        scenarios: [
  ...
  it("runs the Matrix E2EE state_after regression scenario through the gateway fault proxy", async () => {
    ...
    try {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverDeviceId: "DRIVERDEVICE",
        gatewayRuntimeEnv: {
          RECALL_CONFIG_PATH: gatewayConfigPath,
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      startMatrixQaRecallCli.mockReturnValue({
        args: ["matrix", "verify", "self", "--account", "cli"],
        endStdin,
        kill,
      ...
      runMatrixQaRecallCli.mockImplementation(async ({ args, env, stdin }) => {
        if (!cliAccountConfigDuringRun && env.RECALL_CONFIG_PATH) {
          const cliConfig = JSON.parse(
            await readFile(String(env.RECALL_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
              matrix?: {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverPassword: "driver-password",
        gatewayRuntimeEnv: {
          RECALL_CONFIG_PATH: "/tmp/gateway-config.json",
          RECALL_STATE_DIR: "/tmp/gateway-state",
          PATH: process.env.PATH,
        },
        outputDir,
      });
      ...
      expect(startMatrixQaRecallCli).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(startMatrixQaRecallCli, "startMatrixQaRecallCli").args).toEqual([
        "matrix",
        "verify",
        "self",
        "--account",
      ...
      expect(mockObjectArg(startMatrixQaRecallCli, "startMatrixQaRecallCli").timeoutMs).toBe(
        16_000,
      );
      ...
      expect(registrationRequest?.deviceName).toBe(
        "Recall Matrix QA CLI Self Verification Owner",
      );
      if (
      ...
      expect(loginWithPassword).toHaveBeenCalledWith({
        deviceName: "Recall Matrix QA CLI Self Verification Device",
        password: "cli-owner-password",
        userId: "@cli-owner:matrix-qa.test",
      });
      const e2eeClientOptions = mockObjectArg(
      ...
      expect(runMatrixQaRecallCli).toHaveBeenCalledTimes(2);
      expect(runMatrixQaRecallCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          "matrix",
          "verify",
          "backup",
      ...
      expect(mockObjectArg(runMatrixQaRecallCli, "runMatrixQaRecallCli").stdin).toBe(
        "encoded-recovery-key\n",
      );
      const cliEnv = mockObjectArg(startMatrixQaRecallCli, "startMatrixQaRecallCli")
        .env as Record<string, unknown>;
      expect(cliEnv?.RECALL_STATE_DIR).toContain("recall-matrix-cli-qa-");
      expect(cliEnv?.RECALL_CONFIG_PATH).toContain("recall-matrix-cli-qa-");
      const configPath = String(cliEnv?.RECALL_CONFIG_PATH);
      ...
      await expectPathMissing(String(cliEnv?.RECALL_STATE_DIR));
  ...
  it("runs Matrix account add --enable-e2ee through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaRecallCli.mockImplementation(async ({ args, env }) => {
        if (env.RECALL_CONFIG_PATH) {
      ...
      expect(runMatrixQaRecallCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          ...
          "cli-add-password",
          "--device-name",
          "Recall Matrix QA CLI Account Add E2EE",
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Recall Matrix QA CLI Account Add Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaRecallCli.mockImplementation(async ({ args, env }) => {
        if (!initialAccountConfig && env.RECALL_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.RECALL_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Recall Matrix QA CLI Encryption Setup Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup idempotency through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Recall Matrix QA CLI Encryption Idempotent Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      const wait = vi
        .fn()
        .mockRejectedValue(new Error("recall matrix encryption setup exited 1"));
      const kill = vi.fn();
      startMatrixQaRecallCli.mockReturnValue({
      ...
      expect(
        (
          mockObjectArg(startMatrixQaRecallCli, "startMatrixQaRecallCli").env as Record<
            string,
            unknown
          >
        ).RECALL_CONFIG_PATH,
      ).toContain("recall-matrix-e2ee-setup-qa-");
      expect(output).toHaveBeenCalledTimes(1);
      expect(wait).toHaveBeenCalledTimes(1);
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Recall Matrix QA CLI Encryption Failure Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Recall Matrix QA CLI Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix invalid recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Recall Matrix QA CLI Invalid Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaRecallCli.mockImplementation(async ({ args, env }) => {
        const configPath = String(env.RECALL_CONFIG_PATH);
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Recall Matrix QA CLI Multi Account Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
    ...
    try {
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "Recall Matrix QA CLI Setup Gateway" &&
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "Recall Matrix QA CLI Setup Driver" &&
            request.registrationToken === "registration-token",
        ),
      ).toBe(true);
      expect(createPrivateRoom).toHaveBeenCalledWith({
