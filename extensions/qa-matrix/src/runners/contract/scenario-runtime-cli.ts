import { spawn as startRecallCliProcess } from "node:child_process";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `recall ${redactMatrixQaCliArgs(args).join(" ")}`;
}

export function resolveMatrixQaRecallCliEntryPath(cwd: string): string {
...
export function startMatrixQaRecallCli(params: {
  ...
  const distEntryPath = resolveMatrixQaRecallCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startRecallCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
    env: params.env,
...
export async function runMatrixQaRecallCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaRecallCli(params).wait();
}
...
export async function createMatrixQaRecallCliRuntime(params: {
  accountId: string;
  accessToken: string;
