...
- Extension production code should import from `recall/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `recall.plugin.json` and the package
  `recall` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `recall/plugin-sdk/*`.
