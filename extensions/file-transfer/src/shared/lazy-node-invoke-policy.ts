import type { RecallPluginNodeInvokePolicy } from "recall/plugin-sdk/plugin-entry";
import { FILE_TRANSFER_NODE_INVOKE_COMMANDS } from "./node-invoke-policy-commands.js";

type LoadFileTransferNodeInvokePolicy = () => Promise<RecallPluginNodeInvokePolicy>;
...
export function createLazyFileTransferNodeInvokePolicy(
  ...
  let policyPromise: Promise<RecallPluginNodeInvokePolicy> | undefined;

  return {
    commands: [...FILE_TRANSFER_NODE_INVOKE_COMMANDS],
    async handle(ctx) {
      let policy: RecallPluginNodeInvokePolicy;
      try {
