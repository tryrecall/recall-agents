...
function createPolicyContext(
  overrides: Partial<RecallPluginNodeInvokePolicyContext> = {},
): RecallPluginNodeInvokePolicyContext {
  return {
    nodeId: "node-1",
    command: "file.fetch",
    ...
    client: null,
    invokeNode: vi.fn<RecallPluginNodeInvokePolicyContext["invokeNode"]>(async () => ({
      ok: true,
      payload: { ok: true },
      payloadJSON: null,
    })),
...
describe("lazy file-transfer node invoke policy", () => {
  it("exposes command metadata without loading the delegate", () => {
    const loadPolicy = vi.fn<() => Promise<RecallPluginNodeInvokePolicy>>();
  ...
  it("loads and caches the delegate on first handle", async () => {
    const invokeNode = vi.fn<RecallPluginNodeInvokePolicyContext["invokeNode"]>(async () => ({
      ok: true,
      payload: { ok: true },
      payloadJSON: null,
    }));
    const delegateHandle = vi.fn<RecallPluginNodeInvokePolicy["handle"]>(async (ctx) => {
      await ctx.invokeNode();
      return { ok: true, payload: { delegated: true } };
    });
    const loadPolicy = vi.fn<() => Promise<RecallPluginNodeInvokePolicy>>(async () => ({
      commands: ["file.fetch"],
