...
import type { RecallPluginNodeInvokePolicyContext } from "recall/plugin-sdk/plugin-entry";
...
function createCtx(overrides: {
  ...
  approvals?: RecallPluginNodeInvokePolicyContext["approvals"];
}) {
  const invokeNode = vi.fn<RecallPluginNodeInvokePolicyContext["invokeNode"]>(
    async ({
      params,
    }: Parameters<RecallPluginNodeInvokePolicyContext["invokeNode"]>[0] = {}) => ({
      ok: true,
      payload: {
        ok: true,
        path:
...
function requireInvokeParams(
  invokeNode: ReturnType<typeof vi.fn<RecallPluginNodeInvokePolicyContext["invokeNode"]>>,
  callIndex: number,
) {
