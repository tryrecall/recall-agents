...
import type { PluginRuntime, RuntimeLogger } from "recall/plugin-sdk/plugin-runtime";
  ...
  if (health?.manualActionRequired) {
    return {
      ...
      message:
        ...
        "Resolve the Google Meet browser prompt before asking Recall to speak.",
    };
  }
  if (health?.inCall === true) {
    if (health.micMuted === true) {
      return {
        ...
        message: "Turn on the Recall Google Meet microphone before asking Recall to speak.",
      };
    }
    if (session.chrome.audioBridge) {
      return { ready: true };
