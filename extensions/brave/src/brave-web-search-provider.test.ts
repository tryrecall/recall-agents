...
import { validateJsonSchemaValue } from "recall/plugin-sdk/config-schema";
...
describe("brave web search provider", () => {
  ...
  it("points provider metadata at the canonical Brave docs page", () => {
    expect(createBraveWebSearchProvider().docsUrl).toBe(
      "https://docs.recall.ai/tools/brave-search",
    );
  ...
  it("points missing-key users to fetch/browser alternatives", async () => {
    ...
    expect(result).toEqual({
      ...
      message:
        "web_search (brave) needs a Brave Search API key. Run `recall configure --section web` to store it, or set BRAVE_API_KEY in the Gateway environment. If you do not want to configure a search API key, use web_fetch for a specific URL or the browser tool for interactive pages.",
