...
function missingBraveKeyPayload() {
  return {
    ...
    message: `web_search (brave) needs a Brave Search API key. Run \`${formatCliCommand("recall configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
...
export function createBraveWebSearchProvider(): WebSearchProviderPlugin {
  return {
    ...
    docsUrl: "https://docs.recall.ai/brave-search",
    autoDetectOrder: 10,
