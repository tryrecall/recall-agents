...
import { canResolveEnvSecretRefInReadOnlyPath } from "recall/plugin-sdk/extension-shared";
import { resolveSecretInputString, normalizeSecretInput } from "recall/plugin-sdk/secret-input";
...
type WebFetchConfig = NonNullable<RecallConfig["tools"]>["web"] extends infer Web
...
function resolveFetchConfig(cfg?: RecallConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: RecallConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: RecallConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: RecallConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: RecallConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: RecallConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: RecallConfig, override?: number): number {
