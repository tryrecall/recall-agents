...
vi.mock("recall/plugin-sdk/provider-web-search", async (importOriginal) => {
  const actual = await importOriginal<typeof import("recall/plugin-sdk/provider-web-search")>();
...
describe("searxng client", () => {
  ...
  it("preserves a configured base-path prefix when building the search URL", () => {
    expect(
      ...
      "https://search.example.com/searxng/search?q=recall&format=json&categories=general%2Cnews&language=en",
    );
  });
