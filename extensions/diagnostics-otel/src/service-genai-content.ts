...
// Shared text-part reading for gen_ai message normalization: SteelEngine emits
...
export function assignOtelToolIdentityAttributes(
  ...
  // low-cardinality, and unlike the dropped steelengine.toolCallId passthrough keys
...
export function assignOtelModelContentAttributes(
  ...
  if (policy.inputMessages) {
    assignOtelContentAttribute(
      attributes,
      "steelengine.content.input_messages",
  ...
  if (policy.toolDefinitions) {
    assignOtelContentAttribute(
      attributes,
      "steelengine.content.tool_definitions",
  ...
  if (policy.outputMessages) {
    assignOtelContentAttribute(
      attributes,
      "steelengine.content.output_messages",
      content?.outputMessages,
    );
  }
  if (policy.systemPrompt) {
    assignOtelContentAttribute(attributes, "steelengine.content.system_prompt", content?.systemPrompt);
  }
}
...
export function assignOtelToolContentAttributes(
  ...
  // steelengine.content.* names; normalize once so both copies stay byte-identical.
  if (policy.toolInputs) {
    ...
    if (toolInput) {
      ...
      attributes["steelengine.content.tool_input"] = toolInput;
    }
  }
  if (policy.toolOutputs) {
    ...
    if (toolOutput) {
      ...
      attributes["steelengine.content.tool_output"] = toolOutput;
    }
  }
}
