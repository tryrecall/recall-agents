...
import type { RecallPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/recall-diagnostics-otel-test";
...
const ORIGINAL_RECALL_OTEL_PRELOADED = process.env.RECALL_OTEL_PRELOADED;
...
type OtelContextFlags = {
  ...
  captureContent?: NonNullable<
    NonNullable<RecallPluginServiceContext["config"]["diagnostics"]>["otel"]
  >["captureContent"];
};
...
function createTraceOnlyContext(endpoint: string): RecallPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.RECALL_OTEL_PRELOADED;
  ...
  afterEach(() => {
    resetDiagnosticEventsForTest();
    if (ORIGINAL_RECALL_OTEL_PRELOADED === undefined) {
      delete process.env.RECALL_OTEL_PRELOADED;
    } else {
      process.env.RECALL_OTEL_PRELOADED = ORIGINAL_RECALL_OTEL_PRELOADED;
    }
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("recall.webhook.received")?.add).toHaveBeenCalledWith(1, {
      "recall.channel": "telegram",
      "recall.webhook": "telegram-post",
    });
    expect(
      telemetryState.histograms.get("recall.webhook.duration_ms")?.record,
    ).toHaveBeenCalledWith(120, {
      "recall.channel": "telegram",
      "recall.webhook": "telegram-post",
    });
    expect(telemetryState.counters.get("recall.message.queued")?.add).toHaveBeenCalledWith(1, {
      "recall.channel": "telegram",
      "recall.source": "telegram",
    });
    expect(telemetryState.histograms.get("recall.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("recall.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "recall.channel": "telegram",
      "recall.source": "telegram",
    });
    expect(telemetryState.histograms.get("recall.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "recall.lane": "main",
    });
    expect(telemetryState.counters.get("recall.message.processed")?.add).toHaveBeenCalledWith(1, {
      "recall.channel": "telegram",
      "recall.outcome": "completed",
    });
    expect(telemetryState.counters.get("recall.message.received")?.add).toHaveBeenCalledWith(1, {
      "recall.channel": "telegram",
      "recall.source": "webhook",
    });
    expect(telemetryState.counters.get("recall.message.received")?.add).toHaveBeenCalledWith(1, {
      "recall.channel": "unknown",
      "recall.source": "unknown",
    });
    expect(
      telemetryState.counters.get("recall.message.dispatch.started")?.add,
    ...
    expect(
      telemetryState.counters.get("recall.message.dispatch.completed")?.add,
    ...
    expect(
      telemetryState.histograms.get("recall.message.dispatch.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "recall.channel": "telegram",
    ...
    expect(
      telemetryState.histograms.get("recall.message.duration_ms")?.record,
    ).toHaveBeenCalledWith(55, {
      "recall.channel": "telegram",
      "recall.outcome": "completed",
    });
    expect(telemetryState.histograms.get("recall.queue.wait_ms")?.record).toHaveBeenCalledWith(
      10,
      {
        "recall.lane": "main",
      },
    );
    expect(telemetryState.counters.get("recall.session.stuck")?.add).toHaveBeenCalledTimes(1);
    expect(telemetryState.counters.get("recall.session.stuck")?.add).toHaveBeenCalledWith(1, {
      "recall.state": "processing",
    });
    expect(
      telemetryState.histograms.get("recall.session.stuck_age_ms")?.record,
    ).toHaveBeenCalledWith(125_000, {
      "recall.state": "processing",
    });
    expect(telemetryState.counters.get("recall.run.attempt")?.add).toHaveBeenCalledWith(1, {
      "recall.attempt": 2,
    });
    ...
    expect(telemetryState.counters.get("recall.session.turn.created")?.add).toHaveBeenCalledWith(
      1,
      {
        "recall.agent": "agent.default",
        "recall.channel": "telegram",
        "recall.trigger": "user",
      },
    );
    ...
    expect(spanNames).toContain("recall.webhook.processed");
    expect(spanNames).toContain("recall.message.processed");
    expect(spanNames).toContain("recall.session.stuck");
    const webhookSpanOptions = startedSpanOptions("recall.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("recall.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("recall.message.processed");
    expect(messageSpanOptions?.attributes?.["recall.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["recall.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["recall.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("recall.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("recall.messageId");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.RECALL_OTEL_PRELOADED = "1";
    const service = createDiagnosticsOtelService();
    const ctx = createOtelContext(OTEL_TEST_ENDPOINT, { traces: true, metrics: true, logs: true });
    await service.start(ctx);
    ...
    const runDurationRecordCall = lastHistogramRecord("recall.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["recall.provider"]).toBe("openai");
    expect(runDurationAttributes?.["recall.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("recall.run");
    expect(runSpanOptions?.attributes?.["recall.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("recall.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "recall.exporter": "diagnostics-otel",
      "recall.signal": "logs",
      "recall.status": "started",
      "recall.reason": "configured",
    });
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("recall.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "recall.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("recall.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "recall.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("recall.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "recall.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpanOptions = startedSpanOptions("recall.liveness.warning");
    expect(livenessSpanOptions?.attributes?.["recall.liveness.reason"]).toBe(
      "event_loop_delay:cpu",
    );
    expect(livenessSpanOptions?.attributes?.["recall.liveness.active"]).toBe(2);
    expect(livenessSpanOptions?.attributes?.["recall.liveness.queued"]).toBe(4);
    const span = telemetryState.spans.find((item) => item.name === "recall.liveness.warning");
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "recall.exporter": "diagnostics-otel",
      "recall.signal": "logs",
      "recall.status": "failure",
      "recall.reason": "emit_failed",
      "recall.errorCategory": "TypeError",
    });
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    await service.start(ctx);
    telemetryState.counters.get("recall.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("recall.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["recall.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["recall.blocked_by"]).toBe("policy-plugin");
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("recall.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["recall.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["recall.token"];
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "recall.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "recall.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "recall.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/recall/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/recall/src/private.ts:42",
      },
    ...
    expect(String(emitCall.attributes["recall.good"])).toMatch(/^y+/);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(String(emitCall.attributes["recall.good"]).length).toBeLessThanOrEqual(4200);
    expect(Object.hasOwn(emitCall.attributes, `recall.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "recall.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "recall.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "recall.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "recall.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "recall.code.location")).toBe(false);
    await service.stop?.(ctx);
  });
  ...
  test("does not parent diagnostic event spans from plugin-emittable trace context", async () => {
    ...
    const modelUsageCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "recall.model.usage",
    );
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("recall.tokens");
    expect(tokens?.add).toHaveBeenCalledWith(12, {
      "recall.channel": "webchat",
      "recall.agent": "ops",
      "recall.provider": "openai",
      "recall.model": "gpt-5.4",
      "recall.token": "input",
    });
  ...
  test("bounds agent identifiers on model usage metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("recall.tokens")?.add).toHaveBeenCalledWith(2, {
      "recall.channel": "unknown",
      "recall.agent": "unknown",
      "recall.provider": "openai",
      "recall.model": "gpt-5.4",
      "recall.token": "input",
    });
    expect(
      JSON.stringify(telemetryState.counters.get("recall.tokens")?.add.mock.calls),
  ...
  test("drops session-shaped queue lane metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("recall.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "recall.lane": "session",
      },
    ...
    expect(
      JSON.stringify(telemetryState.counters.get("recall.queue.lane.enqueue")?.add.mock.calls),
    ).not.toContain("Agent:qa:otel-trace-smoke");
    await service.stop?.(ctx);
  });
  ...
  test("keeps only the bounded prefix from scoped queue lane metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("recall.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "recall.lane": "dreaming-narrative",
      },
    );
    expect(
      JSON.stringify(telemetryState.counters.get("recall.queue.lane.enqueue")?.add.mock.calls),
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("recall.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "recall.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "recall.sessionId")).toBe(false);
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("recall.run");
    expect(spanNames).toContain("recall.model.call");
    expect(spanNames).toContain("recall.harness.run");
    expect(spanNames).toContain("recall.tool.execution");

    const runOptions = startedSpanOptions("recall.run");
    expect(runOptions?.attributes?.["recall.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["recall.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["recall.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["recall.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "recall.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "recall.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "recall.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("recall.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "recall.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "recall.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "recall.sessionKey")).toBe(false);
    expect(modelOptions?.startTime).toBeTypeOf("number");
    expect(modelCall?.[2]).toBeUndefined();

    const harnessCall = startedSpanCall("recall.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["recall.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["recall.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["recall.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["recall.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["recall.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["recall.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["recall.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["recall.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["recall.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["recall.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["recall.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "recall.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "recall.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "recall.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "recall.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("recall.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["recall.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["recall.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["recall.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["recall.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "recall.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "recall.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "recall.sessionKey")).toBe(false);
    expect(toolOptions?.startTime).toBeTypeOf("number");
    expect(toolCall?.[2]).toBeUndefined();

    const modelCallDuration = lastHistogramRecord("recall.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(80);
    expect(modelCallDuration?.[1]?.["recall.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["recall.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("recall.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["recall.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["recall.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("recall.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["recall.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["recall.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("recall.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["recall.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["recall.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("recall.model.call");
    expect(modelSpanAttributes["recall.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["recall.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["recall.model_call.time_to_first_byte_ms"]).toBe(45);
    const runDuration = lastHistogramRecord("recall.run.duration_ms");
    expect(runDuration?.[0]).toBe(100);
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "recall.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("recall.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["recall.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["recall.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["recall.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "recall.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "recall.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("recall.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "recall.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "recall.runId")).toBe(false);

    const toolSpan = spanByName("recall.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("recall.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    await service.stop?.(ctx);
  });
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("recall.model.failover");
    expect(failoverOptions?.attributes?.["recall.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["recall.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["recall.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["recall.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["recall.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["recall.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["recall.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["recall.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "recall.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "recall.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("recall.model.failover")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("drops session-shaped queue lanes from model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("recall.model.failover");
    expect(failoverOptions?.attributes?.["recall.lane"]).toBe("session");
  ...
  test("maps model call APIs to GenAI operation names and error type", async () => {
    ...
    const modelCallAttrs = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "recall.model.call")
  ...
  test("uses latest GenAI provider attribute only when semconv opt-in is set", async () => {
    ...
    const modelCallOptions = startedSpanOptions("recall.model.call");
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    const modelCallOptions = startedSpanOptions("recall.model.call");
    expect(modelCallOptions?.attributes?.["recall.failureKind"]).toBe("terminated");
    expect(
      Object.hasOwn(modelCallOptions?.attributes ?? {}, "recall.upstreamRequestIdHash"),
    ).toBe(false);
    expect(modelCallOptions?.startTime).toBeTypeOf("number");
    const span = telemetryState.spans.find((candidate) => candidate.name === "recall.model.call");
    expect(span?.addEvent).toHaveBeenCalledWith("recall.provider.request", {
      "recall.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("recall.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(40);
    expect(modelCallDuration?.[1]?.["recall.failureKind"]).toBe("terminated");
    expect(Object.hasOwn(modelCallDuration?.[1] ?? {}, "recall.upstreamRequestIdHash")).toBe(
      false,
    );
    await service.stop?.(ctx);
  });
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("recall.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "recall.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["recall.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["recall.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["recall.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["recall.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["recall.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["recall.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["recall.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["recall.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["recall.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["recall.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["recall.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["recall.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["recall.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("recall.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "recall.toolName": "process",
      "recall.loop.level": "critical",
      "recall.loop.action": "block",
      "recall.loop.detector": "known_poll_no_progress",
      "recall.loop.count": 20,
      "recall.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("recall.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["recall.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["recall.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["recall.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["recall.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["recall.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["recall.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "recall.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("recall.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("recall.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "recall.memory.level": "critical",
        "recall.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("recall.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "recall.memory.level": "critical",
      "recall.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("recall.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["recall.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["recall.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["recall.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["recall.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["recall.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["recall.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["recall.memory.array_buffers_bytes"]).toBe(6);
    expect(pressureOptions?.attributes?.["recall.memory.threshold_bytes"]).toBe(512);
    expect(pressureOptions?.attributes?.["recall.memory.rss_growth_bytes"]).toBe(256);
    expect(pressureOptions?.attributes?.["recall.memory.window_ms"]).toBe(60_000);
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "recall.memory.pressure",
    );
  ...
  test("records async diagnostic queue drop summaries", async () => {
    ...
    const counter = telemetryState.counters.get("recall.diagnostic.async_queue.dropped");
    expect(counter?.add).toHaveBeenCalledWith(4, {
      "recall.diagnostic.async_queue.drop_class": "total",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "recall.diagnostic.async_queue.drop_class": "trusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(2, {
      "recall.diagnostic.async_queue.drop_class": "untrusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "recall.diagnostic.async_queue.drop_class": "priority",
    });
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "recall.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "recall.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "recall.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["recall.run"]).toBeUndefined();
    expect(parentBySpanName["recall.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["recall.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("keeps trusted run spans alive long enough for post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("recall.run")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("does not force remote parents for completed-only trusted lifecycle spans", async () => {
    ...
    expect(parentBySpanName["recall.run"]).toBeUndefined();
    expect(parentBySpanName["recall.model.call"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["recall.run"]).toBeUndefined();
    expect(parentBySpanName["recall.model.call"]).toBeUndefined();
    expect(parentBySpanName["recall.tool.execution"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter((call) => call[0] === "recall.run"),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "recall.model.call",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "recall.tool.execution",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "recall.harness.run",
      ),
    ).toHaveLength(1);
    await service.stop?.(ctx);
  });
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("recall.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["recall.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["recall.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["recall.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["recall.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("recall.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["recall.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["recall.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["recall.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["recall.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["recall.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["recall.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["recall.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "recall.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "recall.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "recall.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("recall.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("recall.exec")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("recall.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "recall.channel": "matrix",
      "recall.delivery.kind": "text",
    });
    const deliveryDurationRecords = telemetryState.histograms.get(
      "recall.message.delivery.duration_ms",
    )?.record.mock.calls as Array<[unknown, Record<string, unknown>]>;
    expect(deliveryDurationRecords[0]?.[0]).toBe(25);
    expect(deliveryDurationRecords[0]?.[1]["recall.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["recall.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["recall.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["recall.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["recall.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["recall.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["recall.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "recall.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["recall.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["recall.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["recall.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["recall.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["recall.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["recall.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["recall.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["recall.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "recall.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "recall.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "recall.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "recall.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "recall.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "recall.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "recall.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("recall.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["recall.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["recall.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["recall.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("recall.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["recall.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["recall.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["recall.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["recall.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("recall.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["recall.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["recall.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["recall.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("recall.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["recall.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["recall.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["recall.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("recall.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["recall.status"]).toBe("released");
    expect(telemetryState.counters.get("recall.talk.event")?.add).toHaveBeenCalledWith(1, {
      "recall.talk.brain": "agent-consult",
      "recall.talk.event_type": "input.audio.delta",
      "recall.talk.mode": "realtime",
      "recall.talk.provider": "openai",
      "recall.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("recall.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "recall.talk.brain": "agent-consult",
        "recall.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("recall.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "recall.talk.brain": "agent-consult",
      "recall.talk.event_type": "latency.metrics",
      "recall.talk.mode": "realtime",
      "recall.talk.provider": "openai",
      "recall.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("recall.talk.event")?.add.mock.calls,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    const modelOptions = startedSpanOptions("recall.model.call");
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "recall.content.input_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "recall.content.output_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "recall.content.system_prompt")).toBe(
      false,
    );
    expect(modelOptions?.startTime).toBeTypeOf("number");
    const toolOptions = startedSpanOptions("recall.tool.execution");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "recall.content.tool_input")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "recall.content.tool_output")).toBe(
      false,
    );
    expect(toolOptions?.startTime).toBeTypeOf("number");
    await service.stop?.(ctx);
  ...
  test("exports bounded redacted content when capture fields are opted in", async () => {
    ...
    expect(modelAttrs?.["recall.content.output_messages"]).toBe("model reply");
    expect(modelAttrs?.["recall.content.system_prompt"]).toBe("system prompt");
    expect(String(modelAttrs?.["recall.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["recall.content.tool_input"]).toBe("tool input");
    expect(String(toolAttrs?.["recall.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["recall.content.tool_output"])).not.toContain("a".repeat(11));
    await service.stop?.(ctx);
  });
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("recall.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["recall.reason"])).toContain("…");
    expect(typeof attrs?.["recall.reason"]).toBe("string");
    expect(String(attrs?.["recall.reason"])).not.toContain(
