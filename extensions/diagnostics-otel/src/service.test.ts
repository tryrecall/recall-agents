...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("recall.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("recall.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("recall.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("recall.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("recall.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("recall.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("recall.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("recall.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("recall.run.attempt")?.add).toHaveBeenCalled();
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("recall.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "recall.reason": expect.stringContaining("…"),
      }),
    );
