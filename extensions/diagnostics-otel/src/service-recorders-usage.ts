...
export function createUsageRecorders(runtime: DiagnosticsRecorderRuntime) {
  ...
  const recordModelUsage = (
    ...
    const attrs = {
      "steelengine.channel": evt.channel ?? "unknown",
      "steelengine.agent": lowCardinalityAttr(evt.agentId),
    ...
    if (usage.input) {
      tokensCounter.add(usage.input, { ...attrs, "steelengine.token": "input" });
    ...
    if (usage.output) {
      tokensCounter.add(usage.output, { ...attrs, "steelengine.token": "output" });
    ...
    if (usage.cacheRead) {
      tokensCounter.add(usage.cacheRead, { ...attrs, "steelengine.token": "cache_read" });
    }
    if (usage.cacheWrite) {
      tokensCounter.add(usage.cacheWrite, { ...attrs, "steelengine.token": "cache_write" });
    }
    if (usage.promptTokens) {
      tokensCounter.add(usage.promptTokens, { ...attrs, "steelengine.token": "prompt" });
    }
    if (usage.total) {
      tokensCounter.add(usage.total, { ...attrs, "steelengine.token": "total" });
    }
    ...
    if (evt.context?.limit) {
      contextHistogram.record(evt.context.limit, {
        ...attrs,
        "steelengine.context": "limit",
      });
    }
    if (evt.context?.used) {
      contextHistogram.record(evt.context.used, {
        ...attrs,
        "steelengine.context": "used",
      });
    }
    ...
    const spanAttrs: Record<string, string | number> = {
      ...attrs,
      "steelengine.tokens.input": usage.input ?? 0,
      "steelengine.tokens.output": usage.output ?? 0,
      "steelengine.tokens.cache_read": usage.cacheRead ?? 0,
      "steelengine.tokens.cache_write": usage.cacheWrite ?? 0,
      "steelengine.tokens.total": usage.total ?? 0,
    };
    ...
    const span = spanWithDuration("steelengine.model.usage", spanAttrs, evt.durationMs, {
  ...
  const recordWebhookReceived = (
    ...
    const attrs = {
      "steelengine.channel": evt.channel ?? "unknown",
      "steelengine.webhook": evt.updateType ?? "unknown",
    };
  ...
  const recordWebhookProcessed = (
    ...
    const attrs = {
      "steelengine.channel": lowCardinalityAttr(evt.channel),
      "steelengine.webhook": lowCardinalityAttr(evt.updateType),
    };
    ...
    const span = spanWithDuration("steelengine.webhook.processed", spanAttrs, evt.durationMs);
    span.end();
  };
  ...
  const recordWebhookError = (evt: Extract<DiagnosticEventPayload, { type: "webhook.error" }>) => {
    ...
    const spanAttrs: Record<string, string | number> = {
      ...attrs,
      "steelengine.error": redactedError,
    };
    const span = tracer.startSpan("steelengine.webhook.error", {
  ...
  const recordMessageQueued = (
    ...
    const attrs = {
      "steelengine.channel": lowCardinalityAttr(evt.channel),
      "steelengine.source": lowCardinalityAttr(evt.source),
    };
  ...
  const recordMessageDispatchStarted = (
    ...
    trackInternalOrTrustedSpan(
      evt,
      metadata,
      spanWithDuration("steelengine.message.processed", attrs, undefined, {
  ...
  const recordMessageProcessed = (
    ...
    const attrs = {
      "steelengine.channel": lowCardinalityAttr(evt.channel),
      "steelengine.outcome": evt.outcome ?? "unknown",
    };
    ...
    const spanAttrs: Record<string, string | number> = { ...attrs };
    if (evt.reason) {
      spanAttrs["steelengine.reason"] = lowCardinalityAttr(evt.reason, "unknown");
    }
    const trackedSpan = getTrackedInternalOrTrustedSpan(evt, metadata);
    const span =
      trackedSpan ??
      spanWithDuration("steelengine.message.processed", spanAttrs, evt.durationMs, {
        parentContext: internalOrTrustedExplicitParentContext(evt, metadata),
        endTimeMs: evt.ts,
      });
  ...
  const messageDeliveryAttrs = (evt: MessageDeliveryDiagnosticEvent): Record<string, string> => ({
    "steelengine.channel": lowCardinalityAttr(evt.channel),
    "steelengine.delivery.kind": lowCardinalityAttr(evt.deliveryKind, "other"),
  });
  ...
  const recordMessageDeliveryCompleted = (
    ...
    const span = spanWithDuration(
      ...
      {
        ...attrs,
        "steelengine.delivery.result_count": evt.resultCount,
      },
      evt.durationMs,
  ...
  const recordMessageDeliveryError = (
    ...
    const span = spanWithDuration("steelengine.message.delivery", attrs, evt.durationMs, {
  ...
  const recordRunStarted = (
    ...
    const span = trackTrustedSpan(
      evt,
      metadata,
      spanWithDuration("steelengine.run", spanAttrs, undefined, {
        parentContext: activeTrustedParentContext(evt, metadata),
        startTimeMs: evt.ts,
      }),
    );
