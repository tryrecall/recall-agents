...
const DEFAULT_SERVICE_NAME = "recall";
...
const PRELOADED_OTEL_SDK_ENV = "RECALL_OTEL_PRELOADED";
...
function assignModelCallSizeTimingAttrs(
  ...
  assignPositiveNumberAttr(attrs, "recall.model_call.request_bytes", evt.requestPayloadBytes);
  assignPositiveNumberAttr(attrs, "recall.model_call.response_bytes", evt.responseStreamBytes);
  assignPositiveNumberAttr(
    attrs,
    "recall.model_call.time_to_first_byte_ms",
...
function addUpstreamRequestIdSpanEvent(
  ...
  span.addEvent?.("recall.provider.request", {
    "recall.upstreamRequestIdHash": boundedHash,
  });
}
...
function assignOtelModelContentAttributes(
  ...
  if (policy.inputMessages) {
    assignOtelContentAttribute(attributes, "recall.content.input_messages", event.inputMessages);
  }
  if (policy.outputMessages) {
    assignOtelContentAttribute(
      attributes,
      "recall.content.output_messages",
      event.outputMessages,
    );
  }
  if (policy.systemPrompt) {
    assignOtelContentAttribute(attributes, "recall.content.system_prompt", event.systemPrompt);
  }
}
...
function assignOtelToolContentAttributes(
  ...
  if (policy.toolInputs) {
    assignOtelContentAttribute(attributes, "recall.content.tool_input", event.toolInput);
  }
  if (policy.toolOutputs) {
    assignOtelContentAttribute(attributes, "recall.content.tool_output", event.toolOutput);
  }
}
...
function assignOtelLogEventAttributes(
  ...
  for (const rawKey in eventAttributes) {
    ...
    assignOtelLogAttribute(attributes, `recall.${key}`, eventAttributes[rawKey]);
  }
}
...
function addTraceAttributes(
  ...
  attributes["recall.traceId"] = normalized.traceId;
  if (normalized.spanId) {
    attributes["recall.spanId"] = normalized.spanId;
  }
  if (normalized.parentSpanId) {
    attributes["recall.parentSpanId"] = normalized.parentSpanId;
  }
  if (normalized.traceFlags) {
    attributes["recall.traceFlags"] = normalized.traceFlags;
  }
}

export function createDiagnosticsOtelService(): RecallPluginService {
  ...
  return {
    ...
    async start(ctx) {
      ...
      const meter = metrics.getMeter("recall");
      const tracer = trace.getTracer("recall");
      ...
      const tokensCounter = meter.createCounter("recall.tokens", {
        unit: "1",
      ...
      const costCounter = meter.createCounter("recall.cost.usd", {
        unit: "1",
        description: "Estimated model cost (USD)",
      });
      const durationHistogram = meter.createHistogram("recall.run.duration_ms", {
        unit: "ms",
        description: "Agent run duration",
      });
      const harnessDurationHistogram = meter.createHistogram("recall.harness.duration_ms", {
        unit: "ms",
        description: "Agent harness lifecycle duration",
      });
      const contextHistogram = meter.createHistogram("recall.context.tokens", {
        unit: "1",
        description: "Context window size and usage",
      });
      const webhookReceivedCounter = meter.createCounter("recall.webhook.received", {
        unit: "1",
        description: "Webhook requests received",
      });
      const webhookErrorCounter = meter.createCounter("recall.webhook.error", {
        unit: "1",
        description: "Webhook processing errors",
      });
      const webhookDurationHistogram = meter.createHistogram("recall.webhook.duration_ms", {
        unit: "ms",
        description: "Webhook processing duration",
      });
      const messageQueuedCounter = meter.createCounter("recall.message.queued", {
        unit: "1",
        description: "Messages queued for processing",
      });
      const messageReceivedCounter = meter.createCounter("recall.message.received", {
        unit: "1",
        description: "Inbound messages received",
      });
      const messageDispatchStartedCounter = meter.createCounter(
        "recall.message.dispatch.started",
        {
          unit: "1",
      ...
      const messageDispatchCompletedCounter = meter.createCounter(
        "recall.message.dispatch.completed",
        {
          unit: "1",
      ...
      const messageDispatchDurationHistogram = meter.createHistogram(
        "recall.message.dispatch.duration_ms",
        {
          unit: "ms",
          description: "Inbound message dispatch duration",
        },
      );
      const messageProcessedCounter = meter.createCounter("recall.message.processed", {
        unit: "1",
        description: "Messages processed by outcome",
      });
      const messageDurationHistogram = meter.createHistogram("recall.message.duration_ms", {
        unit: "ms",
        description: "Message processing duration",
      });
      const messageDeliveryStartedCounter = meter.createCounter(
        "recall.message.delivery.started",
        {
          unit: "1",
      ...
      const queueDepthHistogram = meter.createHistogram("recall.queue.depth", {
        unit: "1",
        description: "Queue depth on enqueue/dequeue",
      });
      const queueWaitHistogram = meter.createHistogram("recall.queue.wait_ms", {
        unit: "ms",
        description: "Queue wait time before execution",
      });
      const laneEnqueueCounter = meter.createCounter("recall.queue.lane.enqueue", {
        unit: "1",
        description: "Command queue lane enqueue events",
      });
      const laneDequeueCounter = meter.createCounter("recall.queue.lane.dequeue", {
        unit: "1",
        description: "Command queue lane dequeue events",
      });
      const sessionStateCounter = meter.createCounter("recall.session.state", {
        unit: "1",
        description: "Session state transitions",
      });
      const sessionTurnCreatedCounter = meter.createCounter("recall.session.turn.created", {
        unit: "1",
        description: "Agent session turns created",
      });
      const sessionStuckCounter = meter.createCounter("recall.session.stuck", {
        unit: "1",
        description: "Sessions stuck in processing",
      });
      const sessionStuckAgeHistogram = meter.createHistogram("recall.session.stuck_age_ms", {
        unit: "ms",
        description: "Age of stuck sessions",
      });
      const sessionRecoveryRequestedCounter = meter.createCounter(
        "recall.session.recovery.requested",
        {
          unit: "1",
      ...
      const sessionRecoveryCompletedCounter = meter.createCounter(
        "recall.session.recovery.completed",
        {
          unit: "1",
      ...
      const sessionRecoveryAgeHistogram = meter.createHistogram(
        "recall.session.recovery.age_ms",
        {
          unit: "ms",
          description: "Age of sessions selected for recovery",
        },
      );
      const talkEventCounter = meter.createCounter("recall.talk.event", {
        unit: "1",
        description: "Talk events emitted by type",
      });
      const talkEventDurationHistogram = meter.createHistogram("recall.talk.event.duration_ms", {
        unit: "ms",
        description: "Talk event duration when reported",
      });
      const talkAudioBytesHistogram = meter.createHistogram("recall.talk.audio.bytes", {
        unit: "By",
        description: "Talk audio frame byte lengths",
      });
      const runAttemptCounter = meter.createCounter("recall.run.attempt", {
        unit: "1",
        description: "Run attempts",
      });
      const toolLoopCounter = meter.createCounter("recall.tool.loop", {
        unit: "1",
        description: "Detected repetitive tool-call loop events",
      });
      const modelCallDurationHistogram = meter.createHistogram("recall.model_call.duration_ms", {
        unit: "ms",
        description: "Model call duration",
      });
      const modelCallRequestBytesHistogram = meter.createHistogram(
        "recall.model_call.request_bytes",
        {
          unit: "By",
      ...
      const modelCallResponseBytesHistogram = meter.createHistogram(
        "recall.model_call.response_bytes",
        {
          unit: "By",
      ...
      const toolExecutionDurationHistogram = meter.createHistogram(
        "recall.tool.execution.duration_ms",
        {
          unit: "ms",
          description: "Tool execution duration",
        },
      );
      const execProcessDurationHistogram = meter.createHistogram("recall.exec.duration_ms", {
        unit: "ms",
        description: "Exec process duration",
      });
      const memoryRssHistogram = meter.createHistogram("recall.memory.rss_bytes", {
        unit: "By",
        description: "Resident set size reported by diagnostic memory samples",
      });
      const memoryHeapUsedHistogram = meter.createHistogram("recall.memory.heap_used_bytes", {
        unit: "By",
        description: "Heap used bytes reported by diagnostic memory samples",
      });
      const memoryHeapTotalHistogram = meter.createHistogram("recall.memory.heap_total_bytes", {
        unit: "By",
        description: "Heap total bytes reported by diagnostic memory samples",
      });
      const memoryExternalHistogram = meter.createHistogram("recall.memory.external_bytes", {
        unit: "By",
      ...
      const livenessEventLoopDelayMaxHistogram = meter.createHistogram(
        "recall.liveness.event_loop_delay_max_ms",
        {
          unit: "ms",
      ...
      const livenessEventLoopUtilizationHistogram = meter.createHistogram(
        "recall.liveness.event_loop_utilization",
        {
          unit: "1",
      ...
      const livenessCpuCoreRatioHistogram = meter.createHistogram(
        "recall.liveness.cpu_core_ratio",
        {
          unit: "1",
          description: "CPU core ratio reported by diagnostic liveness warnings",
        },
      );
      const telemetryExporterCounter = meter.createCounter("recall.telemetry.exporter.events", {
        unit: "1",
      ...
      if (logsEnabled) {
        ...
        const otelLogger = logProvider.getLogger("recall");
        recordLogRecord = (evt, metadata) => {
          try {
            ...
            assignOtelLogAttribute(attributes, "recall.log.level", logLevelName);
            if (evt.loggerName) {
              assignOtelLogAttribute(attributes, "recall.logger", evt.loggerName);
            }
            if (evt.loggerParents?.length) {
              assignOtelLogAttribute(
                attributes,
                "recall.logger.parents",
      ...
      const addRunAttrs = (
        ...
        if (evt.provider) {
          spanAttrs["recall.provider"] = evt.provider;
        }
        if (evt.model) {
          spanAttrs["recall.model"] = evt.model;
        }
        if (evt.channel) {
          spanAttrs["recall.channel"] = evt.channel;
        }
        if (evt.trigger) {
          spanAttrs["recall.trigger"] = evt.trigger;
        }
      };
      ...
      const paramsSummaryAttrs = (
        ...
        return {
          "recall.tool.params.kind": summary.kind,
          ...("length" in summary ? { "recall.tool.params.length": summary.length } : {}),
        };
      };
      ...
      const recordModelUsage = (
        ...
        const attrs = {
          "recall.channel": evt.channel ?? "unknown",
          "recall.agent": lowCardinalityAttr(evt.agentId),
          "recall.provider": evt.provider ?? "unknown",
          "recall.model": evt.model ?? "unknown",
        };
        ...
        if (usage.input) {
          tokensCounter.add(usage.input, { ...attrs, "recall.token": "input" });
        ...
        if (usage.output) {
          tokensCounter.add(usage.output, { ...attrs, "recall.token": "output" });
        ...
        if (usage.cacheRead) {
          tokensCounter.add(usage.cacheRead, { ...attrs, "recall.token": "cache_read" });
        }
        if (usage.cacheWrite) {
          tokensCounter.add(usage.cacheWrite, { ...attrs, "recall.token": "cache_write" });
        }
        if (usage.promptTokens) {
          tokensCounter.add(usage.promptTokens, { ...attrs, "recall.token": "prompt" });
        }
        if (usage.total) {
          tokensCounter.add(usage.total, { ...attrs, "recall.token": "total" });
        }
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "recall.tokens.input": usage.input ?? 0,
          "recall.tokens.output": usage.output ?? 0,
          "recall.tokens.cache_read": usage.cacheRead ?? 0,
          "recall.tokens.cache_write": usage.cacheWrite ?? 0,
          "recall.tokens.total": usage.total ?? 0,
        };
        ...
        const span = spanWithDuration("recall.model.usage", spanAttrs, evt.durationMs, {
      ...
      const recordWebhookReceived = (
        ...
        const attrs = {
          "recall.channel": evt.channel ?? "unknown",
          "recall.webhook": evt.updateType ?? "unknown",
        };
      ...
      const recordWebhookProcessed = (
        ...
        const attrs = {
          "recall.channel": lowCardinalityAttr(evt.channel),
          "recall.webhook": lowCardinalityAttr(evt.updateType),
        };
        ...
        const span = spanWithDuration("recall.webhook.processed", spanAttrs, evt.durationMs);
        span.end();
      };
      ...
      const recordWebhookError = (
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "recall.error": redactedError,
        };
        const span = tracer.startSpan("recall.webhook.error", {
      ...
      const recordMessageQueued = (
        ...
        const attrs = {
          "recall.channel": lowCardinalityAttr(evt.channel),
          "recall.source": lowCardinalityAttr(evt.source),
        };
      ...
      const recordMessageDispatchCompleted = (
        ...
        const attrs = {
          "recall.channel": lowCardinalityAttr(evt.channel),
          "recall.outcome": evt.outcome,
          "recall.reason": lowCardinalityAttr(evt.reason, "none"),
          "recall.source": lowCardinalityAttr(evt.source),
        };
      ...
      const recordMessageProcessed = (
        ...
        const attrs = {
          "recall.channel": lowCardinalityAttr(evt.channel),
          "recall.outcome": evt.outcome ?? "unknown",
        };
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        if (evt.reason) {
          spanAttrs["recall.reason"] = lowCardinalityAttr(evt.reason, "unknown");
        }
        const span = spanWithDuration("recall.message.processed", spanAttrs, evt.durationMs);
      ...
      const messageDeliveryAttrs = (
        ...
        "recall.channel": lowCardinalityAttr(evt.channel),
        "recall.delivery.kind": lowCardinalityAttr(evt.deliveryKind, "other"),
      });
      ...
      const recordMessageDeliveryCompleted = (
        ...
        const span = spanWithDuration(
          ...
          {
            ...attrs,
            "recall.delivery.result_count": evt.resultCount,
          },
          evt.durationMs,
          { endTimeMs: evt.ts },
        );
      ...
      const recordMessageDeliveryError = (
        ...
        const attrs = {
          ...messageDeliveryAttrs(evt),
          "recall.outcome": "error",
          "recall.errorCategory": lowCardinalityAttr(evt.errorCategory, "other"),
        };
        messageDeliveryDurationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        }
        const span = spanWithDuration("recall.message.delivery", attrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
        span.setStatus({
      ...
      const recordRunStarted = (
        ...
        const span = trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("recall.run", spanAttrs, undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordLaneEnqueue = (
        ...
        const attrs = { "recall.lane": lowCardinalityQueueLaneAttr(evt.lane) };
      ...
      const recordSessionState = (
        ...
        const attrs: Record<string, string> = { "recall.state": evt.state };
        if (evt.reason) {
          attrs["recall.reason"] = redactSensitiveText(evt.reason);
        }
      ...
      const recordSessionTurnCreated = (
        ...
        sessionTurnCreatedCounter.add(1, {
          "recall.agent": lowCardinalityAttr(evt.agentId, "unknown"),
          "recall.channel": lowCardinalityAttr(evt.channel, "unknown"),
          "recall.trigger": evt.trigger,
        });
      };
      ...
      const recordSessionStuck = (
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        spanAttrs["recall.queueDepth"] = evt.queueDepth ?? 0;
        spanAttrs["recall.ageMs"] = evt.ageMs;
        const span = tracer.startSpan("recall.session.stuck", { attributes: spanAttrs });
      ...
      const sessionRecoveryAttrs = (evt: SessionRecoveryDiagnosticEvent) => {
        ...
        if (evt.activeWorkKind) {
          attrs["recall.active_work_kind"] = evt.activeWorkKind;
        }
        return attrs;
      };
      ...
      const recordSessionRecoveryRequested = (
        ...
        attrs["recall.action"] = evt.allowActiveAbort ? "abort" : "recover";
      ...
      const recordSessionRecoveryCompleted = (
        ...
        const attrs = sessionRecoveryAttrs(evt);
        attrs["recall.status"] = evt.status;
        attrs["recall.action"] = lowCardinalityAttr(evt.action, "unknown");
        if (evt.outcomeReason) {
          attrs["recall.reason"] = redactSensitiveText(evt.outcomeReason);
        }
      ...
      const talkEventAttrs = (evt: TalkDiagnosticEvent): Record<string, string> => ({
        "recall.talk.brain": lowCardinalityAttr(evt.brain),
        "recall.talk.event_type": lowCardinalityAttr(evt.talkEventType),
        "recall.talk.mode": lowCardinalityAttr(evt.mode),
        "recall.talk.provider": lowCardinalityAttr(evt.provider),
        "recall.talk.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordRunAttempt = (evt: Extract<DiagnosticEventPayload, { type: "run.attempt" }>) => {
        runAttemptCounter.add(1, { "recall.attempt": evt.attempt });
      };

      const toolLoopAttrs = (
        evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>,
      ): Record<string, string | number> => ({
        "recall.toolName": lowCardinalityAttr(evt.toolName, "tool"),
        "recall.loop.level": evt.level,
        "recall.loop.action": evt.action,
        "recall.loop.detector": evt.detector,
        "recall.loop.count": evt.count,
        ...(evt.pairedToolName
          ? { "recall.loop.paired_tool": lowCardinalityAttr(evt.pairedToolName, "tool") }
          : {}),
      });
      ...
      const recordToolLoop = (evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>) => {
        ...
        const span = spanWithDuration("recall.tool.loop", attrs, 0, { endTimeMs: evt.ts });
      ...
      const recordMemoryPressure = (
        ...
        const attrs = {
          "recall.memory.level": evt.level,
          "recall.memory.reason": evt.reason,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "recall.memory.rss_bytes": evt.memory.rssBytes,
          "recall.memory.heap_used_bytes": evt.memory.heapUsedBytes,
          "recall.memory.heap_total_bytes": evt.memory.heapTotalBytes,
          "recall.memory.external_bytes": evt.memory.externalBytes,
          "recall.memory.array_buffers_bytes": evt.memory.arrayBuffersBytes,
          ...(evt.thresholdBytes !== undefined
            ? { "recall.memory.threshold_bytes": evt.thresholdBytes }
            : {}),
          ...(evt.rssGrowthBytes !== undefined
            ? { "recall.memory.rss_growth_bytes": evt.rssGrowthBytes }
            : {}),
          ...(evt.windowMs !== undefined ? { "recall.memory.window_ms": evt.windowMs } : {}),
        };
        const span = spanWithDuration("recall.memory.pressure", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
      ...
      const recordRunCompleted = (
        ...
        if (evt.channel) {
          attrs["recall.channel"] = evt.channel;
        }
        if (evt.blockedBy) {
          attrs["recall.blocked_by"] = lowCardinalityAttr(evt.blockedBy, "unknown");
        }
        durationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.blockedBy) {
          spanAttrs["recall.blocked_by"] = lowCardinalityAttr(evt.blockedBy, "unknown");
        }
        if (evt.errorCategory) {
          spanAttrs["recall.errorCategory"] = lowCardinalityAttr(evt.errorCategory, "other");
        }
        ...
        const span =
          trackedSpan ??
          spanWithDuration("recall.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
      ...
      const harnessRunMetricAttrs = (evt: HarnessRunDiagnosticEvent) => ({
        "recall.harness.id": lowCardinalityAttr(evt.harnessId, "unknown"),
        "recall.harness.plugin": lowCardinalityAttr(evt.pluginId),
        ...(evt.type === "harness.run.started"
          ? {}
          : {
              "recall.outcome": evt.type === "harness.run.error" ? "error" : evt.outcome,
            }),
        "recall.provider": lowCardinalityAttr(evt.provider, "unknown"),
        "recall.model": lowCardinalityAttr(evt.model, "unknown"),
        ...(evt.channel ? { "recall.channel": lowCardinalityAttr(evt.channel) } : {}),
      });
      ...
      const recordHarnessRunStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("recall.harness.run", harnessRunMetricAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordHarnessRunCompleted = (
        ...
        if (evt.resultClassification) {
          spanAttrs["recall.harness.result_classification"] = lowCardinalityAttr(
            evt.resultClassification,
          );
        }
        if (typeof evt.yieldDetected === "boolean") {
          spanAttrs["recall.harness.yield_detected"] = evt.yieldDetected;
        }
        if (evt.itemLifecycle) {
          spanAttrs["recall.harness.items.started"] = evt.itemLifecycle.startedCount;
          spanAttrs["recall.harness.items.completed"] = evt.itemLifecycle.completedCount;
          spanAttrs["recall.harness.items.active"] = evt.itemLifecycle.activeCount;
        }
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("recall.harness.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordHarnessRunError = (
        ...
        const attrs = {
          ...harnessRunMetricAttrs(evt),
          "recall.harness.phase": evt.phase,
          "recall.errorCategory": errorType,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...
          ...(evt.cleanupFailed ? { "recall.harness.cleanup_failed": true } : {}),
        };
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("recall.harness.run", spanAttrs, evt.durationMs, {
      ...
      const recordContextAssembled = (
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          "recall.context.message_count": evt.messageCount,
          "recall.context.history_text_chars": evt.historyTextChars,
          "recall.context.history_image_blocks": evt.historyImageBlocks,
          "recall.context.max_message_text_chars": evt.maxMessageTextChars,
          "recall.context.system_prompt_chars": evt.systemPromptChars,
          "recall.context.prompt_chars": evt.promptChars,
          "recall.context.prompt_images": evt.promptImages,
        };
        addRunAttrs(spanAttrs, evt);
        if (evt.contextTokenBudget !== undefined) {
          spanAttrs["recall.context.token_budget"] = evt.contextTokenBudget;
        }
        if (evt.reserveTokens !== undefined) {
          spanAttrs["recall.context.reserve_tokens"] = evt.reserveTokens;
        }
        const span = spanWithDuration("recall.context.assembled", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      ...
      const recordModelFailover = (
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          "recall.failover.reason": lowCardinalityAttr(evt.reason, "unknown"),
        };
        if (evt.fromProvider) {
          spanAttrs["recall.provider"] = evt.fromProvider;
        }
        if (evt.fromModel) {
          spanAttrs["recall.model"] = evt.fromModel;
        }
        if (evt.toProvider) {
          spanAttrs["recall.failover.to_provider"] = evt.toProvider;
        }
        if (evt.toModel) {
          spanAttrs["recall.failover.to_model"] = evt.toModel;
        }
        if (evt.lane) {
          spanAttrs["recall.lane"] = lowCardinalityQueueLaneAttr(evt.lane, "unknown");
        }
        if (evt.suspended !== undefined) {
          spanAttrs["recall.failover.suspended"] = evt.suspended;
        }
        if (evt.cascadeDepth !== undefined) {
          spanAttrs["recall.failover.cascade_depth"] = evt.cascadeDepth;
        }
        const span = spanWithDuration("recall.model.failover", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      ...
      const modelCallMetricAttrs = (evt: ModelCallLifecycleDiagnosticEvent) => ({
        "recall.provider": evt.provider,
        "recall.model": evt.model,
        "recall.api": lowCardinalityAttr(evt.api),
        "recall.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordModelCallStarted = (
        ...
        if (evt.api) {
          spanAttrs["recall.api"] = evt.api;
        }
        if (evt.transport) {
          spanAttrs["recall.transport"] = evt.transport;
        }
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("recall.model.call", spanAttrs, undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordModelCallCompleted = (
        ...
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("recall.model.call", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordModelCallError = (
        ...
        const metricAttrs = {
          ...
          "recall.errorCategory": errorType,
          ...(evt.failureKind
            ? { "recall.failureKind": lowCardinalityAttr(evt.failureKind, "other") }
            : {}),
        };
        modelCallDurationHistogram.record(evt.durationMs, metricAttrs);
        recordModelCallSizeTimingMetrics(evt, metricAttrs);
        ...
        if (evt.failureKind) {
          spanAttrs["recall.failureKind"] = lowCardinalityAttr(evt.failureKind, "other");
        }
        assignGenAiModelCallAttrs(spanAttrs, evt);
        if (evt.api) {
          spanAttrs["recall.api"] = evt.api;
      ...
      const recordToolExecutionStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("recall.tool.execution", toolExecutionBaseAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordToolExecutionCompleted = (
        ...
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("recall.tool.execution", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordToolExecutionError = (
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.errorCode) {
          spanAttrs["recall.errorCode"] = lowCardinalityAttr(evt.errorCode, "other");
        }
        assignOtelToolContentAttributes(
          spanAttrs,
          evt as unknown as Record<string, unknown>,
      ...
      const recordToolExecutionBlocked = (
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...toolExecutionBaseAttrs(evt),
          "recall.outcome": "blocked",
          "recall.deniedReason": lowCardinalityAttr(evt.deniedReason, "other"),
        };
        addRunAttrs(spanAttrs, evt);
        const span = spanWithDuration("recall.tool.execution", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordExecProcessCompleted = (
        ...
        const attrs: Record<string, string | number> = {
          "recall.exec.target": evt.target,
          "recall.exec.mode": evt.mode,
          "recall.outcome": evt.outcome,
        };
        if (evt.failureKind) {
          attrs["recall.failureKind"] = evt.failureKind;
        }
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "recall.exec.command_length": evt.commandLength,
        };
        if (typeof evt.exitCode === "number") {
          spanAttrs["recall.exec.exit_code"] = evt.exitCode;
        }
        if (evt.exitSignal) {
          spanAttrs["recall.exec.exit_signal"] = lowCardinalityAttr(evt.exitSignal, "other");
        }
        if (evt.timedOut !== undefined) {
          spanAttrs["recall.exec.timed_out"] = evt.timedOut;
        }

        const span = spanWithDuration("recall.exec", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
      ...
      const recordHeartbeat = (
        ...
        queueDepthHistogram.record(evt.queued, { "recall.channel": "heartbeat" });
      };
      ...
      const recordLivenessWarning = (
        ...
        const attrs = {
          "recall.liveness.reason": lowCardinalityAttr(reason, "unknown"),
        };
        livenessWarningCounter.add(1, attrs);
        queueDepthHistogram.record(evt.queued, { "recall.channel": "liveness" });
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "recall.liveness.active": evt.active,
          "recall.liveness.waiting": evt.waiting,
          "recall.liveness.queued": evt.queued,
          "recall.liveness.interval_ms": evt.intervalMs,
          ...(evt.eventLoopDelayP99Ms !== undefined
            ? { "recall.liveness.event_loop_delay_p99_ms": evt.eventLoopDelayP99Ms }
            : {}),
          ...(evt.eventLoopDelayMaxMs !== undefined
            ? { "recall.liveness.event_loop_delay_max_ms": evt.eventLoopDelayMaxMs }
            : {}),
          ...(evt.eventLoopUtilization !== undefined
            ? { "recall.liveness.event_loop_utilization": evt.eventLoopUtilization }
            : {}),
          ...(evt.cpuUserMs !== undefined
            ? { "recall.liveness.cpu_user_ms": evt.cpuUserMs }
            : {}),
          ...(evt.cpuSystemMs !== undefined
            ? { "recall.liveness.cpu_system_ms": evt.cpuSystemMs }
            : {}),
          ...(evt.cpuTotalMs !== undefined
            ? { "recall.liveness.cpu_total_ms": evt.cpuTotalMs }
            : {}),
          ...(evt.cpuCoreRatio !== undefined
            ? { "recall.liveness.cpu_core_ratio": evt.cpuCoreRatio }
            : {}),
        };
        const span = spanWithDuration("recall.liveness.warning", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
        span.setStatus({
          code: SpanStatusCode.ERROR,
      ...
      const recordDiagnosticPhaseCompleted = (
        ...
        const spanAttrs: Record<string, string | number> = {
          "recall.phase": lowCardinalityAttr(evt.name, "unknown"),
          ...(evt.cpuUserMs !== undefined ? { "recall.phase.cpu_user_ms": evt.cpuUserMs } : {}),
          ...(evt.cpuSystemMs !== undefined
            ? { "recall.phase.cpu_system_ms": evt.cpuSystemMs }
            : {}),
          ...(evt.cpuTotalMs !== undefined
            ? { "recall.phase.cpu_total_ms": evt.cpuTotalMs }
            : {}),
          ...(evt.cpuCoreRatio !== undefined
            ? { "recall.phase.cpu_core_ratio": evt.cpuCoreRatio }
            : {}),
        };
        for (const [key, value] of Object.entries(evt.details ?? {})) {
          spanAttrs[`recall.phase.detail.${key}`] =
            typeof value === "boolean" ? String(value) : value;
        }
        const span = spanWithDuration("recall.diagnostic.phase", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      };
      ...
      const recordTelemetryExporter = (
        ...
        telemetryExporterCounter.add(1, {
          "recall.exporter": lowCardinalityAttr(evt.exporter, "unknown"),
          "recall.signal": evt.signal,
          "recall.status": evt.status,
          ...(evt.reason ? { "recall.reason": evt.reason } : {}),
          ...(evt.errorCategory
            ? { "recall.errorCategory": lowCardinalityAttr(evt.errorCategory, "other") }
            : {}),
        });
