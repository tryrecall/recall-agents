...
export function createDiagnosticsLogExporter(params: {
  ...
  if (logsEnabled) {
    ...
    if (logsToOtlp) {
      ...
      otelLogger = logProvider.getLogger("steelengine");
    }
    ...
    const buildDiagnosticLogRecord = (
      ...
      assignOtelLogAttribute(attributes, "steelengine.log.level", logLevelName);
      if (evt.loggerName) {
        assignOtelLogAttribute(attributes, "steelengine.logger", evt.loggerName);
      }
      if (evt.loggerParents?.length) {
        assignOtelLogAttribute(attributes, "steelengine.logger.parents", evt.loggerParents.join("."));
      }
    ...
    const buildSecurityLogRecord = (
      ...
      const traceContext = normalizedTrustedTraceContext(evt, metadata);
      const logRecord: LogRecord = {
        body: "steelengine.security.event",
        severityText,
