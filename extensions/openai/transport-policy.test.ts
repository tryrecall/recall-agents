import type { ProviderRuntimeModel } from "recall/plugin-sdk/plugin-entry";
...
describe("openai transport policy", () => {
  ...
  it("builds native turn state for direct OpenAI routes", () => {
    ...
    expect(state?.headers?.["x-recall-session-id"]).toBe("session-123");
    expect(state?.headers?.["x-recall-turn-id"]).toBe("turn-123");
    expect(state?.headers?.["x-recall-turn-attempt"]).toBe("2");
  ...
  it("returns websocket session headers and cooldown for native routes", () => {
    ...
    expect(policy?.headers?.["x-recall-session-id"]).toBe("session-123");
