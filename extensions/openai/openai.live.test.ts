...
import type { ResolvedTtsConfig } from "recall/plugin-sdk/agent-runtime";
...
import { runRealtimeSttLiveTest } from "recall/plugin-sdk/provider-test-contracts";
...
const LIVE_MODEL_ID = process.env.RECALL_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.5";
const LIVE_IMAGE_MODEL = process.env.RECALL_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
...
describeLive("openai plugin live", () => {
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const cfg = createLiveConfig();
    const ttsConfig = createLiveTtsConfig();
    const phrase = "Testing Recall OpenAI realtime transcription integration test OK.";
