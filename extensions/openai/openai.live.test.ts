...
import type { ResolvedTtsConfig } from "steelengine/plugin-sdk/agent-runtime";
...
import { runRealtimeSttLiveTest } from "steelengine/plugin-sdk/provider-test-contracts";
...
const LIVE_MODEL_ID = process.env.STEELENGINE_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.6-luna";
const LIVE_IMAGE_MODEL = process.env.STEELENGINE_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.STEELENGINE_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-5.4-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.STEELENGINE_LIVE_TEST === "1";
...
describeLive("openai plugin live", () => {
  ...
  it("lists voices and synthesizes audio through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "SteelEngine integration test OK.",
      cfg,
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const cfg = createLiveConfig();
    const ttsConfig = createLiveTtsConfig();
    const phrase = "Testing SteelEngine OpenAI realtime transcription integration test OK.";
