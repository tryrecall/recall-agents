...
} from "recall/plugin-sdk/proxy-capture";
...
describe("openai tts", () => {
  ...
  describe("openaiTTS diagnostics", () => {
    it("adds Recall attribution headers to native OpenAI speech requests", async () => {
      ...
      expect(headers?.originator).toBe("recall");
      expect(headers?.version).toBe("2026.3.22");
      expect(headers?.["User-Agent"]).toBe("recall/2026.3.22");
    });
    ...
    it("records TTS exchanges in debug proxy capture mode", async () => {
      ...
      process.env.RECALL_DEBUG_PROXY_ENABLED = "1";
      ...
      process.env.RECALL_DEBUG_PROXY_SESSION_ID = "tts-session";
      ...
      store.upsertSession({
        ...
        dbPath: process.env.RECALL_DEBUG_PROXY_DB_PATH,
        blobDir: process.env.RECALL_DEBUG_PROXY_BLOB_DIR,
      });
    ...
    it("does not double-capture TTS exchanges when the global fetch patch is installed", async () => {
      ...
      process.env.RECALL_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
      process.env.RECALL_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
      process.env.RECALL_DEBUG_PROXY_SESSION_ID = "tts-patched-session";
