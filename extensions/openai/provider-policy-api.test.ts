...
describe("OpenAI provider policy artifact", () => {
  ...
  it.each([
    ...
    ["gpt-5.6-sol", "steelengine", "medium"],
    ["gpt-5.6-terra", "codex", "medium"],
    ["gpt-5.6-terra", "steelengine", "medium"],
    ["gpt-5.6-luna", "codex", "medium"],
    ["gpt-5.6-luna", "steelengine", "medium"],
  ...
  it.each(["gpt-5.6", "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"])(
    "exposes logical Ultra for %s on the SteelEngine runtime",
    (modelId) => {
  ...
  it("preserves explicit official completions and keeps them on SteelEngine", () => {
    expect(
      resolveModelRoutes({
        provider: "openai",
        modelId: "gpt-5.5",
  ...
  it("canonicalizes equivalent Platform URLs and keeps unknown variants single-route", () => {
    ...
    ).toEqual({ kind: "indeterminate", defaultRuntimeId: "steelengine" });
  });
