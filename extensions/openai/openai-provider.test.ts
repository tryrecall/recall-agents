...
vi.mock("recall/plugin-sdk/provider-stream-family", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("recall/plugin-sdk/provider-stream-family")>();
...
describe("buildOpenAIProvider", () => {
  ...
  it("owns Codex wrapper composition for responses payloads", () => {
    ...
    expect(String(headers?.["User-Agent"]).startsWith("recall/")).toBe(true);
