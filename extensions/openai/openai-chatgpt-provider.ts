...
import { CODEX_CLI_PROFILE_ID, type OAuthCredential } from "steelengine/plugin-sdk/provider-auth";
 ...
 * entirely when they were written by older SteelEngine versions. When that row wins
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.steelengine.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `steelengine models auth login --provider openai` or `steelengine configure`.";
}
