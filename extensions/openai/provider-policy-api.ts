...
} from "steelengine/plugin-sdk/provider-model-types";
import {
...
const OPENAI_AGENT_RUNTIME_ID = "steelengine";
const CODEX_AGENT_RUNTIME_ID = "codex";
const STEELENGINE_RUNTIME_COMPATIBLE_IDS = [OPENAI_AGENT_RUNTIME_ID] as const;
  ...
  return {
    ...
    runtimePolicy: {
      compatibleIds: codexCanReproduceRoute(candidate, sourceBaseUrl)
        ...
        : STEELENGINE_RUNTIME_COMPATIBLE_IDS,
    },
  };
}
...
function resolveSingleObservedModelRoute(
  ...
  // and let the SteelEngine runtime own the request.
  const platformApi =
...
export function resolveModelRoutes(
  ...
  return {
    ...
    defaultRuntimeId: resolutions.some(
      (resolution) => resolution.kind === "routes" && resolution.defaultRuntimeId === "steelengine",
    )
      ? OPENAI_AGENT_RUNTIME_ID
