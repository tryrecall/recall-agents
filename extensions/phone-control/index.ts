// Phone Control plugin entrypoint registers its SteelEngine integration.
...
function openArmStateStore(api: SteelEnginePluginApi) {
...
async function readStoredArmState(api: SteelEnginePluginApi): Promise<StoredArmState | null> {
...
async function readArmState(api: SteelEnginePluginApi): Promise<ArmStateFile | null> {
  return (await readStoredArmState(api))?.state ?? null;
}

async function registerArmState(api: SteelEnginePluginApi, state: ArmStateFileV3): Promise<void> {
...
async function consumeArmState(api: SteelEnginePluginApi, expected: StoredArmState): Promise<boolean> {
...
export default definePluginEntry({
  ...
  register(api: SteelEnginePluginApi) {
    ...
    const timerService: SteelEnginePluginService = {
