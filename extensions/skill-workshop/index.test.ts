...
import type { AnyAgentTool } from "recall/plugin-sdk/agent-runtime";
import type { PluginTrustedToolPolicyRegistration } from "recall/plugin-sdk/core";
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-skill-workshop-test-"));
  tempDirs.push(dir);
  return dir;
}
