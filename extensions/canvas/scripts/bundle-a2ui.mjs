...
async function main() {
  ...
  if (!hasA2uiPackage || !hasAppDir) {
    ...
    if (process.env.RECALL_SPARSE_PROFILE || process.env.RECALL_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI package missing; skipping bundle because RECALL_A2UI_SKIP_MISSING=1 or RECALL_SPARSE_PROFILE is set.",
      );
      return;
    }
