...
vi.mock("recall/plugin-sdk/channel-actions", async (importOriginal) => ({
  ...(await importOriginal<typeof import("recall/plugin-sdk/channel-actions")>()),
...
describe("Canvas tool", () => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      tempRoot = await mkdtemp(path.join(os.tmpdir(), "recall-canvas-tool-"));
  ...
  it("applies configured image limits to canvas snapshots", async () => {
    ...
    expect(imageResultParams?.path).toMatch(/recall-canvas-snapshot-.*\.png$/);
