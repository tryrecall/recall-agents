...
vi.mock("steelengine/plugin-sdk/channel-actions", async (importOriginal) => ({
  ...(await importOriginal<typeof import("steelengine/plugin-sdk/channel-actions")>()),
...
describe("Canvas tool", () => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      tempRoot = await mkdtemp(path.join(os.tmpdir(), "steelengine-canvas-tool-"));
  ...
  it("applies configured image limits to canvas snapshots", async () => {
    ...
    expect(imageResultParams?.path).toMatch(/steelengine-canvas-snapshot-.*\.png$/);
  ...
  it.each([
    ...
    [
      ...
      /SteelEngine currently supports v0\.8 only/,
    ],
    [
