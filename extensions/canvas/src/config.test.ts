...
describe("Canvas plugin config", () => {
  const originalSkipCanvasHost = process.env.RECALL_SKIP_CANVAS_HOST;

  afterEach(() => {
    if (originalSkipCanvasHost === undefined) {
      delete process.env.RECALL_SKIP_CANVAS_HOST;
    } else {
      process.env.RECALL_SKIP_CANVAS_HOST = originalSkipCanvasHost;
    }
  });
  ...
  it("honors truthy skip-canvas env values before host registration", () => {
    for (const value of ["1", "true", " yes ", "ON"]) {
      process.env.RECALL_SKIP_CANVAS_HOST = value;
