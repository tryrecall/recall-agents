...
import { detectMime } from "steelengine/plugin-sdk/media-mime";
import { isTruthyEnvValue, type RuntimeEnv } from "steelengine/plugin-sdk/runtime-env";
import { resolveStateDir } from "steelengine/plugin-sdk/state-paths";
import {
  lowercasePreservingWhitespace,
  normalizeOptionalString,
} from "steelengine/plugin-sdk/string-coerce-runtime";
import { ensureDir, resolveUserPath } from "steelengine/plugin-sdk/text-utility-runtime";
...
<title>SteelEngine Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.steelengineCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.steelengineCanvasA2UIAction &&
        typeof window.steelengineCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.steelengineSendUserAction === "function";
  ...
  window.addEventListener("steelengine:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android SteelEngine node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.steelengineSendUserAction === "function"
        ? window.steelengineSendUserAction
        : undefined;
...
function shouldIgnoreCanvasWatchPath(rootReal: string, candidatePath: string): boolean {
  ...
  // policy below the root so the default ~/.steelengine parent is still watched.
  return relative
    .split(/[\\/]/u)
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>SteelEngine Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
