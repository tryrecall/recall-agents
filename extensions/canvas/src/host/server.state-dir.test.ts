...
describe("canvas host state dir defaults", () => {
  ...
  it("uses STEELENGINE_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("steelengine-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("SteelEngine Canvas");
      } finally {
        await handler.close();
      }
    });
