 ...
 * SteelEngine-owned wrapper trees after validating paths, packages, and lease ids.
 */
...
const STEELENGINE_PLUGIN_DEPS_MARKER = "/plugin-runtime-deps/";
...
const OWNED_ACP_PACKAGE_NAMES = [
  ...
  // recognition until their SteelEngine-owned wrapper/process tree is gone.
...
type AcpxProcessCleanupResult = {
  ...
  skippedReason?: "missing-root" | "not-steelengine-owned" | "unverified-root";
};
...
function resolveSteelEngineInstallRoot(pluginRoot: string): string {
  if (
...
function resolveOwnedAcpPackageRootCandidates(packageName: string): string[] {
  ...
  const steelEngineRoot = resolveSteelEngineInstallRoot(pluginRoot);
  return [
    resolvePackageRoot(packageName),
    path.join(pluginRoot, "node_modules", packageName),
    path.join(steelEngineRoot, "node_modules", packageName),
...
/** Check whether a command references an SteelEngine-generated ACPX wrapper path. */
export function isSteelEngineLeaseAwareAcpxProcessCommand(params: {
...
function liveCommandMatchesLeaseIdentity(params: {
  ...
  return (
    commandOptionEquals(parts, STEELENGINE_ACPX_LEASE_ID_ARG, params.expectedLeaseId) &&
    commandOptionEquals(parts, STEELENGINE_GATEWAY_INSTANCE_ID_ARG, params.expectedGatewayInstanceId)
  );
}

/** Check whether a command is owned by SteelEngine ACPX runtime packages or wrappers. */
function isSteelEngineOwnedAcpxProcessCommand(params: {
  command: string | undefined;
  wrapperRoot?: string;
}): boolean {
  const command = params.command?.trim();
  ...
  if (!normalized.includes(STEELENGINE_PLUGIN_DEPS_MARKER)) {
    return false;
  }
...
/** Terminate one validated SteelEngine-owned ACPX wrapper process tree. */
export async function cleanupSteelEngineOwnedAcpxProcessTree(params: {
  rootPid?: number;
  rootCommand?: string;
  expectedLeaseId?: string;
  expectedGatewayInstanceId?: string;
  ...
  // that this PID still belongs to an SteelEngine-owned wrapper, fail closed to
  ...
  if (!liveCommandWasGeneratedWrapper && storedCommandWasGeneratedWrapper) {
    return {
      ...
      terminatedPids: [],
      skippedReason: "not-steelengine-owned",
    };
  }
  if (
  ...
  if (
    !isSteelEngineOwnedAcpxProcessCommand({
      command: rootCommand,
      wrapperRoot: params.wrapperRoot,
    })
  ) {
...
/** Reap orphaned SteelEngine-owned ACPX wrapper trees during runtime startup. */
export async function reapStaleSteelEngineOwnedAcpxOrphans(params: {
  wrapperRoot: string;
  deps?: AcpxProcessCleanupDeps;
