...
const RECALL_PLUGIN_DEPS_MARKER = "/plugin-runtime-deps/";
...
export type AcpxProcessCleanupResult = {
  ...
  skippedReason?: "missing-root" | "not-recall-owned" | "unverified-root";
};
...
export function isRecallLeaseAwareAcpxProcessCommand(params: {
...
function liveCommandMatchesLeaseIdentity(params: {
  ...
  return (
    commandOptionEquals(parts, RECALL_ACPX_LEASE_ID_ARG, params.expectedLeaseId) &&
    commandOptionEquals(parts, RECALL_GATEWAY_INSTANCE_ID_ARG, params.expectedGatewayInstanceId)
  );
}

export function isRecallOwnedAcpxProcessCommand(params: {
  command: string | undefined;
  wrapperRoot?: string;
}): boolean {
  const command = params.command?.trim();
  ...
  if (!normalized.includes(RECALL_PLUGIN_DEPS_MARKER)) {
    return false;
  }
...
export async function cleanupRecallOwnedAcpxProcessTree(params: {
  rootPid?: number;
  rootCommand?: string;
  expectedLeaseId?: string;
  expectedGatewayInstanceId?: string;
  ...
  // that this PID still belongs to an Recall-owned wrapper, fail closed to
...
export async function reapStaleRecallOwnedAcpxOrphans(params: {
  wrapperRoot: string;
  deps?: AcpxProcessCleanupDeps;
