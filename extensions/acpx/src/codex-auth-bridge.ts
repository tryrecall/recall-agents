...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--recall-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @recall/acpx manifest`);
  }
  return version;
}
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep Recall's isolated CODEX_HOME wrapper, but launch the plugin-local
...
const openClawWrapperArgs = new Set([
  ${quoteCommandPart(RECALL_ACPX_LEASE_ID_ARG)},
  ${quoteCommandPart(RECALL_GATEWAY_INSTANCE_ID_ARG)},
]);

function readRecallWrapperArg(args, name) {
...
function resolveStderrLogPath(args) {
  ...
  const leaseId =
    process.env[${JSON.stringify(RECALL_ACPX_LEASE_ID_ENV)}] ||
    readRecallWrapperArg(args, ${quoteCommandPart(RECALL_ACPX_LEASE_ID_ARG)}) ||
    "pid-" + process.pid;
...
function stripRecallWrapperArgs(args) {
  const stripped = [];
...
const configuredArgs = stripRecallWrapperArgs(rawConfiguredArgs);
...
if (!command) {
  console.error("[recall] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[recall] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in Recall; fallback must not float to an unpatched newer release.
