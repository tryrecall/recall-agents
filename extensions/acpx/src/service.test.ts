...
const { cleanupSteelEngineOwnedAcpxProcessTreeMock } = vi.hoisted(() => ({
  cleanupSteelEngineOwnedAcpxProcessTreeMock: vi.fn(
    async (): Promise<{
...
const { reapStaleSteelEngineOwnedAcpxOrphansMock } = vi.hoisted(() => ({
  reapStaleSteelEngineOwnedAcpxOrphansMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
vi.mock("./process-reaper.js", () => ({
  cleanupSteelEngineOwnedAcpxProcessTree: cleanupSteelEngineOwnedAcpxProcessTreeMock,
  reapStaleSteelEngineOwnedAcpxOrphans: reapStaleSteelEngineOwnedAcpxOrphansMock,
}));

import { getAcpRuntimeBackend } from "../runtime-api.js";
import type { SteelEnginePluginServiceContext } from "../runtime-api.js";
...
const previousEnv = {
  STEELENGINE_ACPX_RUNTIME_STARTUP_PROBE: process.env.STEELENGINE_ACPX_RUNTIME_STARTUP_PROBE,
  STEELENGINE_SKIP_ACPX_RUNTIME: process.env.STEELENGINE_SKIP_ACPX_RUNTIME,
  STEELENGINE_SKIP_ACPX_RUNTIME_PROBE: process.env.STEELENGINE_SKIP_ACPX_RUNTIME_PROBE,
};
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-acpx-service-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  cleanupSteelEngineOwnedAcpxProcessTreeMock.mockClear();
  reapStaleSteelEngineOwnedAcpxOrphansMock.mockClear();
  acpxRuntimeConstructorMock.mockClear();
  createAgentRegistryMock.mockClear();
  createFileSessionStoreMock.mockClear();
  restoreEnv("STEELENGINE_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("STEELENGINE_SKIP_ACPX_RUNTIME");
  restoreEnv("STEELENGINE_SKIP_ACPX_RUNTIME_PROBE");
  for (const dir of tempDirs.splice(0)) {
    await fs.rm(dir, { recursive: true, force: true });
  }
});

function createServiceContext(workspaceDir: string): SteelEnginePluginServiceContext {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".steelengine-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
function createOpenKeyedStore(ctx: SteelEnginePluginServiceContext) {
  const env = { ...process.env, STEELENGINE_STATE_DIR: ctx.stateDir };
...
function createAcpxRuntimeService(
  ctx: SteelEnginePluginServiceContext,
...
function openGatewayInstanceStore(ctx: SteelEnginePluginServiceContext) {
...
function openProcessLeaseStore(ctx: SteelEnginePluginServiceContext) {
...
describe("createAcpxRuntimeService", () => {
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.STEELENGINE_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.STEELENGINE_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = await makeTempDir();
  ...
  it("waits for the embedded runtime startup probe before resolving by default", async () => {
    delete process.env.STEELENGINE_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
  ...
  it("reaps stale ACPX process leases from the generated wrapper root at startup", async () => {
    ...
    cleanupSteelEngineOwnedAcpxProcessTreeMock.mockResolvedValueOnce({
    ...
    expect(cleanupSteelEngineOwnedAcpxProcessTreeMock).toHaveBeenCalledWith({
      rootPid: 101,
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale SteelEngine-owned ACPX processes");
  ...
  it("runs wrapper-root orphan cleanup before dropping pending ACPX leases", async () => {
    ...
    await openProcessLeaseStore(ctx).register(lease.leaseId, lease);
    reapStaleSteelEngineOwnedAcpxOrphansMock.mockResolvedValueOnce({
    ...
    expect(cleanupSteelEngineOwnedAcpxProcessTreeMock).not.toHaveBeenCalled();
    expect(reapStaleSteelEngineOwnedAcpxOrphansMock).toHaveBeenCalledWith({
      wrapperRoot,
      deps: processCleanupDeps,
    });
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale SteelEngine-owned ACPX processes");
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.STEELENGINE_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.STEELENGINE_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.STEELENGINE_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.STEELENGINE_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (STEELENGINE_SKIP_ACPX_RUNTIME=1)",
    );
  });
});
