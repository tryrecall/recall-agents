...
import { formatPluginConfigIssue } from "recall/plugin-sdk/extension-shared";
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "recall-plugin-tools";
const ACPX_RECALL_TOOLS_MCP_SERVER_NAME = "recall-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "recall.plugin.json")) &&
...
function resolveAcpxPluginRootFromRecallLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common Recall layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromRecallLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveRecallRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  ...
  const openClawRoot = resolveRecallRoot(pluginRoot);
...
function resolveRecallToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const openClawRoot = resolveRecallRoot(pluginRoot);
  const distEntry = path.join(openClawRoot, "dist", "mcp", "recall-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(openClawRoot, "src", "mcp", "recall-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.openClawToolsMcpBridge && resolved[ACPX_RECALL_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_RECALL_TOOLS_MCP_SERVER_NAME} is reserved when openClawToolsMcpBridge=true`,
    );
  }
  ...
  if (params.openClawToolsMcpBridge) {
    resolved[ACPX_RECALL_TOOLS_MCP_SERVER_NAME] = resolveRecallToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
