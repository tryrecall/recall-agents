...
import { formatPluginConfigIssue } from "steelengine/plugin-sdk/extension-shared";
...
function resolveAcpxPluginRootFromSteelEngineLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common SteelEngine layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromSteelEngineLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveSteelEngineRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  ...
  const steelEngineRoot = resolveSteelEngineRoot(pluginRoot);
  const distEntry = path.join(steelEngineRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(steelEngineRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
function resolveSteelEngineToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const steelEngineRoot = resolveSteelEngineRoot(pluginRoot);
  const distEntry = path.join(steelEngineRoot, "dist", "mcp", "steelengine-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(steelEngineRoot, "src", "mcp", "steelengine-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.steelEngineToolsMcpBridge && resolved[ACPX_STEELENGINE_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_STEELENGINE_TOOLS_MCP_SERVER_NAME} is reserved when steelEngineToolsMcpBridge=true`,
    );
  }
  ...
  if (params.steelEngineToolsMcpBridge) {
    resolved[ACPX_STEELENGINE_TOOLS_MCP_SERVER_NAME] = resolveSteelEngineToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
...
/** Convert SteelEngine MCP server config into ACPX runtime MCP server entries. */
...
export function resolveAcpxPluginConfig(params: {
  ...
  const steelEngineToolsMcpBridge = normalized.steelEngineToolsMcpBridge === true;
  const mcpServers = resolveConfiguredMcpServers({
    mcpServers: normalized.mcpServers,
    pluginToolsMcpBridge,
    steelEngineToolsMcpBridge,
