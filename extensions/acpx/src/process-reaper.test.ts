...
vi.mock("steelengine/plugin-sdk/process-runtime", async (importOriginal) => ({
  ...(await importOriginal<typeof import("steelengine/plugin-sdk/process-runtime")>()),
  runExec: runExecMock,
}));
...
import {
  ...
  reapStaleSteelEngineOwnedAcpxOrphans,
} from "./process-reaper.js";

const WRAPPER_ROOT = "/tmp/steelengine-state/acpx";
const CODEX_WRAPPER_COMMAND = `node ${WRAPPER_ROOT}/codex-acp-wrapper.mjs`;
const CODEX_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_WRAPPER_COMMAND} ${STEELENGINE_ACPX_LEASE_ID_ARG} lease-1 ${STEELENGINE_GATEWAY_INSTANCE_ID_ARG} gateway-1`;
...
const PLUGIN_DEPS_CODEX_APP_SERVER_COMMAND =
  "node /tmp/steelengine/plugin-runtime-deps/node_modules/@openai/codex/bin/codex.js app-server";
const PLUGIN_DEPS_CODEX_PLATFORM_COMMAND =
  "/tmp/steelengine/plugin-runtime-deps/node_modules/@openai/codex-linux-x64/vendor/codex app-server";
...
type CleanupDeps = NonNullable<Parameters<typeof cleanupSteelEngineOwnedAcpxProcessTree>[0]["deps"]>;
...
describe("process reaper", () => {
  ...
  it("only treats generated wrappers as launch-lease aware", () => {
    ...
    expect(
      isSteelEngineLeaseAwareAcpxProcessCommand({ command: LOCAL_NODE_MODULES_CODEX_COMMAND }),
    ).toBe(false);
    expect(isSteelEngineLeaseAwareAcpxProcessCommand({ command: PLUGIN_DEPS_CODEX_COMMAND })).toBe(
      false,
    );
  });
    ...
    const { deps, killed } = cleanupDeps([
      {
        pid: 113,
        ppid: 1,
        command: `${CODEX_WRAPPER_COMMAND} ${STEELENGINE_ACPX_LEASE_ID_ARG} other-lease ${STEELENGINE_GATEWAY_INSTANCE_ID_ARG} gateway-1`,
      },
    ]);
  ...
  it("does not kill a reused pid when the live command is not SteelEngine-owned", async () => {
  ...
  it("skips non-owned recorded process trees", async () => {
    ...
    expect(result.skippedReason).toBe("not-steelengine-owned");
    expect(killed).toStrictEqual([]);
  });

  it("reaps stale SteelEngine-owned wrapper and adapter orphans on startup", async () => {
    const { deps, killed } = cleanupDeps([
    ...
    const result = await reapStaleSteelEngineOwnedAcpxOrphans({
      wrapperRoot: WRAPPER_ROOT,
      deps,
    });
