...
const DOCUMENTED_STEELENGINE_BRIDGE_COMMAND =
  "env STEELENGINE_HIDE_BANNER=1 STEELENGINE_SUPPRESS_NOTES=1 steelengine acp --url ws://127.0.0.1:18789 --token-file ~/.steelengine/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @agentclientprotocol/codex-acp@1.1.2";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/steelengine/acpx/codex-acp-wrapper.mjs"`;
const CODEX_ACP_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_ACP_WRAPPER_COMMAND} ${STEELENGINE_ACPX_LEASE_ID_ARG} lease-close ${STEELENGINE_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  const runtime = new AcpxRuntime(
    {
      ...
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "steelengine" ? "steelengine acp" : agentName),
        list: () => ["codex", "steelengine"],
      },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("adds the SteelEngine session key to both managed tools MCP bridges", () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    const { runtime } = makeRuntime(baseStore, {
      ...
      steelengineToolsMcpBridgeEnabled: true,
      mcpServers: [
        {
          name: "steelengine-plugin-tools",
          command: "node",
          args: ["dist/mcp/plugin-tools-serve.js"],
          env: [],
        },
        {
          name: "steelengine-tools",
          command: "node",
          args: ["dist/mcp/steelengine-tools-serve.js"],
          env: [],
        },
      ],
    });
    ...
    expect(readScopedMcpEnv("agent:worker:main", "steelengine-plugin-tools")).toContainEqual({
      name: "STEELENGINE_TOOLS_MCP_AGENT_SESSION_KEY",
      value: "agent:worker:main",
    });
    expect(readScopedMcpEnv("agent:research:main", "steelengine-tools")).toContainEqual({
      name: "STEELENGINE_TOOLS_MCP_AGENT_SESSION_KEY",
      value: "agent:research:main",
    });
  });

  it("keeps managed SteelEngine tools MCP delegates reachable for fresh sessions", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("uses the no-MCP delegate for startup probes when the SteelEngine tools bridge is enabled", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("normalizes SteelEngine Codex model ids for ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("strips the SteelEngine Anthropic provider prefix for Claude ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("keeps Claude ACP model ids intact after stripping the SteelEngine provider prefix", () => {
  ...
  it.each([
    ...
    const wrapperRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-acpx-runtime-"));
    ...
    const { runtime, delegate } = makeRuntime(baseStore, {
      steelengineGatewayInstanceId: "gateway-test",
      steelengineProcessLeaseStore: leaseStore.store,
      steelengineWrapperRoot: wrapperRoot,
      agentRegistry: {
  ...
  it("adds Codex wrapper stderr tail to generic terminal turn error events", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        steelengineLeaseId: "lease-turn-event",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("adds Codex wrapper stderr tail to generic startTurn failure results", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        steelengineLeaseId: "lease-start-turn",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("adds Codex wrapper stderr tail when startTurn creation throws", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        steelengineLeaseId: "lease-start-turn-create",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("disables delegate prompt timeout for SteelEngine-managed turns", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(
      ...
      `npx @agentclientprotocol/codex-acp@1.1.2 ${STEELENGINE_CODEX_CONFIG_ARG} '{"model":"gpt-5.4","model_reasoning_effort":"medium"}'`,
    );
    expect(testing.isCodexAcpCommand("steelengine acp")).toBe(false);
  });
  ...
  it("recognizes claude-agent-acp commands", () => {
    ...
    expect(
      testing.isClaudeAcpCommand(`node "/tmp/steelengine/acpx/claude-agent-acp-wrapper.mjs"`),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
        `node.exe "C:/Users/runner/AppData/Local/Temp/steelengine/acpx/claude-agent-acp-wrapper.mjs"`,
      ),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
    ...
    expect(testing.isClaudeAcpCommand("steelengine acp")).toBe(false);
  ...
  it("releases managed SteelEngine tools MCP delegates after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("cleans up SteelEngine-owned ACPX process trees after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/steelengine/acpx/codex-acp-wrapper.mjs"',
        pid: 900,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate } = makeRuntime(
      baseStore,
      {
        steelengineWrapperRoot: "/tmp/steelengine/acpx",
      },
      {
        steelengineProcessCleanup: {
          listProcesses: vi.fn(async () => [
            ...
            },
            {
              ...
              ppid: 900,
              command:
                "node /tmp/steelengine/plugin-runtime-deps/node_modules/@agentclientprotocol/codex-acp/dist/index.js",
            },
          ]),
  ...
  it("records ACPX process leases without persisting lease-specific agent commands", async () => {
    ...
    expect(lease?.wrapperPath).toBe("/tmp/steelengine/acpx/codex-acp-wrapper.mjs");
    expect(launchCommands[0]).toContain("STEELENGINE_ACPX_LEASE_ID=");
    expect(launchCommands[0]).toContain("STEELENGINE_GATEWAY_INSTANCE_ID=gateway-test");
    expect(savedRecords[0]?.agentCommand).toBe(CODEX_ACP_WRAPPER_COMMAND);
    expect(savedRecords[0]?.steelengineGatewayInstanceId).toBe("gateway-test");
    expect(savedRecords[0]?.steelengineLeaseId).toBe(lease?.leaseId);
  });
  ...
  it("merges sidecar lease ids into loaded ACPX session records", async () => {
    ...
    expect(loadedRecord?.steelengineGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.steelengineLeaseId).toBe("lease-loaded");
  });
  ...
  it("merges the lease for the current ACPX session process when old leases exist", async () => {
    ...
    const loadedRecord = await wrappedStore.load("agent:codex:acp:binding:test");
    expect(loadedRecord?.steelengineGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.steelengineLeaseId).toBe("lease-current");
  });
  ...
  it("uses matching leases before legacy pid cleanup on close", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/steelengine/acpx/codex-acp-wrapper.mjs"',
        steelengineLeaseId: "lease-close",
        pid: 930,
      })),
      save: vi.fn(async () => {}),
    };
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 930,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${STEELENGINE_ACPX_LEASE_ID_ARG} lease-old ${STEELENGINE_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
            {
              pid: 940,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${STEELENGINE_ACPX_LEASE_ID_ARG} lease-current ${STEELENGINE_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
  ...
  it("uses session lease metadata for fallback close cleanup identity checks", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        ...
        agentCommand: 'node "/tmp/steelengine/acpx/codex-acp-wrapper.mjs"',
        steelengineGatewayInstanceId: "gateway-test",
        steelengineLeaseId: "lease-record",
        pid: 920,
      })),
      save: vi.fn(async () => {}),
    };
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 920,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${STEELENGINE_ACPX_LEASE_ID_ARG} other-lease ${STEELENGINE_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
          ]),
          killProcess: vi.fn((pid, signal) => {
            killed.push({ pid, signal });
  ...
  it("routes steelengine ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:steelengine:acp:test",
      backend: "acpx",
  ...
  it("routes non-steelengine sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for steelengine sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:steelengine:acp:test",
      backend: "acpx",
      runtimeSessionName: "steelengine-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the steelengine agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "steelengine" ? "codex" : agentName),
        list: () => ["codex", "steelengine"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the steelengine bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "steelengine acp" : agentName),
        list: () => ["codex", "steelengine"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped steelengine bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "steelengine" ? DOCUMENTED_STEELENGINE_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "steelengine"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node steelengine entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "steelengine" ? "env STEELENGINE_HIDE_BANNER=1 node steelengine.mjs acp" : agentName,
        list: () => ["codex", "steelengine"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:steelengine:acp:test",
        agentCommand: DOCUMENTED_STEELENGINE_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:steelengine:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:steelengine:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to steelengine bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
