...
describe("embedded acpx plugin config", () => {
  it("resolves workspace stateDir and cwd by default", () => {
    const workspaceDir = path.resolve("/tmp/recall-acpx");
  ...
  it("injects the built-in plugin-tools MCP server only when explicitly enabled", () => {
    ...
    const server = resolved.mcpServers["recall-plugin-tools"];
  ...
  it("injects the built-in Recall tools MCP server only when explicitly enabled", () => {
    const resolved = resolveAcpxPluginConfig({
      rawConfig: {
    ...
    const server = resolved.mcpServers["recall-tools"];
    expect(server).toEqual({
      command: process.execPath,
      args: expectedSourceMcpServerArgs("src/mcp/recall-tools-serve.ts"),
    });
  });
  ...
  it("keeps the runtime json schema in sync with the manifest config schema", () => {
    ...
    const manifest = JSON.parse(
      fs.readFileSync(path.join(pluginRoot, "recall.plugin.json"), "utf8"),
