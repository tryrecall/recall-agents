/**
 * SteelEngine ACPX runtime adapter. It wraps the upstream acpx runtime with
 * SteelEngine session metadata, lease tracking, model scoping, and cleanup policy.
 */
...
import { redactSensitiveText } from "steelengine/plugin-sdk/security-runtime";
...
import { CODEX_ACP_PACKAGE, STEELENGINE_CODEX_CONFIG_ARG } from "./codex-adapter.js";
...
import {
  cleanupSteelEngineOwnedAcpxProcessTree,
  isSteelEngineLeaseAwareAcpxProcessCommand,
...
type SteelEngineAcpxRuntimeOptions = AcpRuntimeOptions & {
  steelengineWrapperRoot?: string;
  steelengineGatewayInstanceId?: string;
  steelengineProcessLeaseStore?: AcpxProcessLeaseStore;
  pluginToolsMcpBridgeEnabled?: boolean;
  steelengineToolsMcpBridgeEnabled?: boolean;
};
type AcpxRuntimeTestOptions = Record<string, unknown> & {
  steelengineProcessCleanup?: AcpxProcessCleanupDeps;
};
type SteelEngineRuntimeTurnInput = Parameters<NonNullable<AcpRuntime["startTurn"]>>[0];
type SteelEngineRuntimeEnsureInput = Parameters<AcpRuntime["ensureSession"]>[0];
type SteelEngineRuntimeHandle = Awaited<ReturnType<AcpRuntime["ensureSession"]>>;
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "steelengine-plugin-tools";
const ACPX_STEELENGINE_TOOLS_MCP_SERVER_NAME = "steelengine-tools";
const STEELENGINE_TOOLS_MCP_AGENT_SESSION_KEY_ENV = "STEELENGINE_TOOLS_MCP_AGENT_SESSION_KEY";

type ResetAwareSessionStore = AcpSessionStore & {
  markFresh: (sessionKey: string) => void;
};

type SteelEngineLeaseSessionMetadata = {
  steelengineLeaseId: string;
  steelengineGatewayInstanceId: string;
};

function withSteelEngineManagedTurnTimeout<T extends object>(input: T): T & { timeoutMs: 0 } {
  // SteelEngine owns ACP turn deadlines. acpx treats timeout after partial agent
...
function withSteelEngineLeaseSessionMetadata<T extends object>(
  record: T,
  metadata: SteelEngineLeaseSessionMetadata,
): T & SteelEngineLeaseSessionMetadata {
  return {
    ...record,
    steelengineLeaseId: metadata.steelengineLeaseId,
    steelengineGatewayInstanceId: metadata.steelengineGatewayInstanceId,
  };
}
...
function readSteelEngineLeaseIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { steelengineLeaseId } = record as { steelengineLeaseId?: unknown };
  return typeof steelengineLeaseId === "string" ? steelengineLeaseId.trim() || undefined : undefined;
}

function readSteelEngineGatewayInstanceIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { steelengineGatewayInstanceId } = record as { steelengineGatewayInstanceId?: unknown };
  return typeof steelengineGatewayInstanceId === "string"
    ? steelengineGatewayInstanceId.trim() || undefined
    : undefined;
}
...
function createResetAwareSessionStore(
  ...
  return {
    async load(sessionId: string): Promise<AcpLoadedSessionRecord> {
      ...
      return withSteelEngineLeaseSessionMetadata(record, {
        steelengineLeaseId: lease.leaseId,
        steelengineGatewayInstanceId: lease.gatewayInstanceId,
      });
    },
    async save(record: AcpSessionRecord): Promise<void> {
      ...
      if (
        ...
        recordToSave = withSteelEngineLeaseSessionMetadata(
          {
            ...record,
          ...
          {
            steelengineLeaseId: launch.leaseId,
            steelengineGatewayInstanceId: launch.gatewayInstanceId,
          },
        );
      }
...
const STEELENGINE_BRIDGE_EXECUTABLE = "steelengine";
const STEELENGINE_BRIDGE_SUBCOMMAND = "acp";
const CODEX_ACP_AGENT_ID = "codex";
const CODEX_ACP_STEELENGINE_PREFIX = "openai/";
const CLAUDE_ACP_STEELENGINE_PREFIX = "anthropic/";
...
function isSteelEngineBridgeCommand(command: string | undefined): boolean {
  if (!command) {
    return false;
  }
  const parts = unwrapEnvCommand(splitCommandParts(command.trim()));
  if (basename(parts[0] ?? "") === STEELENGINE_BRIDGE_EXECUTABLE) {
    return parts[1] === STEELENGINE_BRIDGE_SUBCOMMAND;
  }
  if (basename(parts[0] ?? "") !== "node") {
    return false;
  }
  const scriptName = basename(parts[1] ?? "");
  return /^steelengine(?:\.[cm]?js)?$/i.test(scriptName) && parts[2] === STEELENGINE_BRIDGE_SUBCOMMAND;
}
...
// See steelengine/steelengine#73071.
...
function classifyCodexAcpModelRequest(
  ...
  if (value.toLowerCase().startsWith(CODEX_ACP_STEELENGINE_PREFIX)) {
    value = value.slice(CODEX_ACP_STEELENGINE_PREFIX.length);
...
function normalizeClaudeAcpModelOverride(rawModel: string | undefined): string | undefined {
  ...
  if (!raw.toLowerCase().startsWith(CLAUDE_ACP_STEELENGINE_PREFIX)) {
    return raw;
  }
  return raw.slice(CLAUDE_ACP_STEELENGINE_PREFIX.length).trim() || undefined;
}

function withAcpxSessionOptions(input: SteelEngineRuntimeEnsureInput): AcpxDelegateEnsureInput {
...
async function ensureDelegateSessionWithModelFallback(
  ...
  input: SteelEngineRuntimeEnsureInput,
...
function appendCodexAcpConfigOverrides(command: string, override: CodexAcpModelOverride): string {
  ...
  return `${command} ${STEELENGINE_CODEX_CONFIG_ARG} ${quoteShellArg(JSON.stringify(config))}`;
}
...
function shouldUseBridgeSafeDelegateForCommand(command: string | undefined): boolean {
  return isSteelEngineBridgeCommand(command);
}
...
function withManagedToolsMcpSessionEnv(params: {
  ...
  steelengineToolsEnabled: boolean;
  mcpServers: AcpRuntimeOptions["mcpServers"];
  sessionKey: string;
}): AcpRuntimeOptions["mcpServers"] {
  const sessionKey = params.sessionKey.trim();
  if (
    (!params.pluginToolsEnabled && !params.steelengineToolsEnabled) ||
    !sessionKey ||
  ...
  const nextServers = params.mcpServers.map((server): AcpxMcpServer => {
    ...
    const isManagedSteelEngineTools =
      params.steelengineToolsEnabled && server.name === ACPX_STEELENGINE_TOOLS_MCP_SERVER_NAME;
    if ((!isManagedPluginTools && !isManagedSteelEngineTools) || !("command" in server)) {
      return server;
    }
    changed = true;
    const env = [
      ...server.env.filter((entry) => entry.name !== STEELENGINE_TOOLS_MCP_AGENT_SESSION_KEY_ENV),
      {
        name: STEELENGINE_TOOLS_MCP_AGENT_SESSION_KEY_ENV,
        value: sessionKey,
      },
    ];
    return { ...server, env };
...
/** SteelEngine-managed ACP runtime implementation backed by the upstream acpx runtime. */
export class AcpxRuntime implements AcpRuntime {
  ...
  private readonly steelengineToolsMcpBridgeEnabled: boolean;
  ...
  constructor(options: SteelEngineAcpxRuntimeOptions, testOptions?: AcpxRuntimeTestOptions) {
    const { steelengineProcessCleanup, ...delegateTestOptions } = testOptions ?? {};
    this.processCleanupDeps = steelengineProcessCleanup;
    this.wrapperRoot = options.steelengineWrapperRoot;
    this.gatewayInstanceId = options.steelengineGatewayInstanceId;
    this.processLeaseStore = options.steelengineProcessLeaseStore;
    this.pluginToolsMcpBridgeEnabled = options.pluginToolsMcpBridgeEnabled === true;
    this.steelengineToolsMcpBridgeEnabled = options.steelengineToolsMcpBridgeEnabled === true;
    this.managedToolsMcpBridgeEnabled =
      this.pluginToolsMcpBridgeEnabled || this.steelengineToolsMcpBridgeEnabled;
    ...
    const delegate = new BaseAcpxRuntime(
      {
        ...
        mcpServers: withManagedToolsMcpSessionEnv({
          ...
          steelengineToolsEnabled: this.steelengineToolsMcpBridgeEnabled,
  ...
  private async readCodexTurnFailureStderr(params: { handle: AcpRuntimeHandle }): Promise<string> {
    ...
    return readCodexWrapperStderrTail({
      wrapperRoot: this.wrapperRoot,
      leaseId: readSteelEngineLeaseIdFromRecord(record),
    });
  }
  ...
  private async cleanupProcessTreeForRecord(
    ...
    const leaseId = readSteelEngineLeaseIdFromRecord(record);
    ...
    if (lease && lease.gatewayInstanceId === this.gatewayInstanceId && lease.rootPid > 0) {
      ...
      const result = await cleanupSteelEngineOwnedAcpxProcessTree({
    ...
    const expectedGatewayInstanceId = readSteelEngineGatewayInstanceIdFromRecord(record);
    await cleanupSteelEngineOwnedAcpxProcessTree({
      rootPid,
      rootCommand,
  ...
  async ensureSession(
  ...
  ): Promise<SteelEngineRuntimeHandle> {
    ...
    const appliedModel: SteelEngineRuntimeHandle["appliedModel"] =
  ...
  async *runTurn(input: Parameters<AcpRuntime["runTurn"]>[0]): AsyncIterable<AcpRuntimeEvent> {
    ...
    try {
      for await (const event of delegate.runTurn(withSteelEngineManagedTurnTimeout(input))) {
        if (
          event.type !== "error" ||
          !isCodexAcpCommand(command) ||
  ...
  startTurn(input: SteelEngineRuntimeTurnInput): AcpRuntimeTurn {
    ...
    const turnPromise = Promise.all([
      ...
      try {
        return {
          command,
          turn: delegate.startTurn(withSteelEngineManagedTurnTimeout(input)),
        };
      } catch (error) {
        if (!isCodexAcpCommand(command) || !isGenericInternalAcpError(error)) {
          throw error;
