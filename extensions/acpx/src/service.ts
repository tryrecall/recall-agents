...
const ENABLE_STARTUP_PROBE_ENV = "STEELENGINE_ACPX_RUNTIME_STARTUP_PROBE";
const SKIP_RUNTIME_PROBE_ENV = "STEELENGINE_SKIP_ACPX_RUNTIME_PROBE";
  ...
  async function resolveRuntime(): Promise<AcpxRuntimeLike> {
    ...
    runtimePromise ??= loadRuntimeModule().then((module) => {
      runtime = new module.AcpxRuntime({
        ...
        steelengineGatewayInstanceId: params.gatewayInstanceId,
        steelengineProcessLeaseStore: params.processLeaseStore,
        steelengineWrapperRoot: params.wrapperRoot,
        ...
        steelengineToolsMcpBridgeEnabled: params.pluginConfig.steelEngineToolsMcpBridge,
...
function resolveAllowedAgentsProbeAgent(ctx: SteelEnginePluginServiceContext): string | undefined {
...
async function reapOpenAcpxProcessLeases(params: {
  ...
  for (const lease of leases) {
    if (lease.rootPid <= 0) {
      ...
      if (!result) {
        result = await reapStaleSteelEngineOwnedAcpxOrphans({
...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: SteelEnginePluginServiceContext): Promise<void> {
      if (process.env.STEELENGINE_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (STEELENGINE_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (startupReap.terminatedPids.length > 0) {
        ctx.logger.info(
          `reaped ${startupReap.terminatedPids.length} stale SteelEngine-owned ACPX process${startupReap.terminatedPids.length === 1 ? "" : "es"}`,
        );
      }
    ...
    async stop(_ctx: SteelEnginePluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
