...
import type { SteelEnginePluginService, SteelEnginePluginServiceContext } from "steelengine/plugin-sdk/core";
...
type DeferredServiceState = {
  ctx: SteelEnginePluginServiceContext | null;
  params: CreateAcpxRuntimeServiceParams;
  realRuntime: AcpRuntime | null;
  realService: SteelEnginePluginService | null;
...
async function startRealService(state: DeferredServiceState): Promise<AcpRuntime> {
  ...
  state.startPromise ??= (async () => {
    ...
    await service.start(state.ctx as SteelEnginePluginServiceContext);
