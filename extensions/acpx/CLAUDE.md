...
The ACPX extension is a thin Recall wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `recall/acpx`, not in this extension.
...
Use this flow when Recall needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `recall/acpx` repo first.
2. In Recall, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Recall and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Recall ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Recall-specific glue, move it to `recall/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
