...
vi.mock("steelengine/plugin-sdk/acp-runtime-backend", () => ({
...
const previousSkipRuntime = process.env.STEELENGINE_SKIP_ACPX_RUNTIME;

function restoreEnv(): void {
  if (previousSkipRuntime === undefined) {
    delete process.env.STEELENGINE_SKIP_ACPX_RUNTIME;
  } else {
    process.env.STEELENGINE_SKIP_ACPX_RUNTIME = previousSkipRuntime;
  }
}

function createServiceContext() {
  return {
    workspaceDir: "/tmp/steelengine-acpx-register-test",
    stateDir: "/tmp/steelengine-acpx-register-test/state",
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
