...
The ACPX extension is a thin SteelEngine wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `steelengine/acpx`, not in this extension.
...
Use this flow when SteelEngine needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `steelengine/acpx` repo first.
2. In SteelEngine, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild SteelEngine and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for SteelEngine ACP validation.
...
If a change feels like shared ACP runtime behavior instead of SteelEngine-specific glue, move it to `steelengine/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
