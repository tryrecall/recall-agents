...
function resolveQianfanPreset(cfg: RecallConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: RecallConfig) => {
...
export function applyQianfanProviderConfig(cfg: RecallConfig): RecallConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: RecallConfig): RecallConfig {
