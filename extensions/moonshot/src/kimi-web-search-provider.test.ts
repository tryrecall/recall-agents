...
describe("kimi web search provider", () => {
  ...
  it("points missing-key users to fetch/browser alternatives", async () => {
    await withEnvAsync({ KIMI_API_KEY: undefined, MOONSHOT_API_KEY: undefined }, async () => {
      ...
      const result = await tool.execute({ query: "SteelEngine docs" });
  ...
  it("accepts final responses backed by Kimi web search tool replay", async () => {
    const toolArguments = JSON.stringify({
      query: "SteelEngine GitHub repository",
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          choices: [
            {
              finish_reason: "stop",
              message: { content: "SteelEngine is available on GitHub." },
            },
          ],
        }),
      );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "SteelEngine is available on GitHub.");
  ...
  it("accepts final responses with search result citations", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        search_results: [{ title: "SteelEngine", url: "https://github.com/steelengineai/recall-agents" }],
        choices: [
          {
            finish_reason: "stop",
            message: { content: "SteelEngine is on GitHub." },
          },
        ],
      }),
    );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "SteelEngine is on GitHub.");
      expect(result.citations).toEqual(["https://github.com/steelengineai/recall-agents"]);
      expect(result).not.toHaveProperty("error");
    });
  });
