...
function resolveKimiBaseUrl(kimi?: KimiConfig, steelEngineConfig?: SteelEngineConfig): string {
  ...
  const moonshotBaseUrl = steelEngineConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: SteelEngineConfig; searchConfig?: SearchConfigRecord },
  ...
  if (!result.grounded) {
    return {
      ...
      provider: "kimi",
      model,
      docs: "https://docs.steelengine.ai/tools/kimi-search",
