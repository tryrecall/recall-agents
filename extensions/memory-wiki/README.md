...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **Recall**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.recall-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
- The compiled agent digests in `.recall-wiki/cache/agent-digest.json` and `.recall-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
