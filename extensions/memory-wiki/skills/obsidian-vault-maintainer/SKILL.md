...
- Start from `steelengine wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `steelengine wiki obsidian status` before shelling out, then prefer the dedicated helpers like `steelengine wiki obsidian search`, `steelengine wiki obsidian open`, `steelengine wiki obsidian command`, and `steelengine wiki obsidian daily`.
