...
import type { OpenBlobStoreOptions } from "steelengine/plugin-sdk/plugin-state-runtime";
import {
...
describe("Memory Wiki compiled cache lifecycle", () => {
  beforeEach(async () => {
    ...
    blobStoreEnv = { ...process.env, STEELENGINE_STATE_DIR: blobStateDir };
    configureMemoryWikiCompiledCacheStore(createCacheStore());
  });
  ...
  it("ignores legacy files and rebuilds only on compile", async () => {
    ...
    const legacyPath = path.join(rootDir, ".steelengine-wiki", "cache", "agent-digest.json");
  ...
  it("rejects claims newer than a restored vault after lifecycle refresh", async () => {
    ...
    const logPath = path.join(rootDir, ".steelengine-wiki", "log.jsonl");
  ...
  it("rejects a predecessor snapshot when a vault path is reused", async () => {
    ...
    await Promise.all([
      ...
      fs.writeFile(
        path.join(rootDir, ".steelengine-wiki", "log.jsonl"),
        `${JSON.stringify({
