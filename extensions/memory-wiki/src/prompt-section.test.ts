...
async function seedCompiledDigest(params: {
  ...
  await fs.mkdir(path.join(params.config.vault.path, ".steelengine-wiki"), { recursive: true });
  await Promise.all([
    fs.writeFile(path.join(params.config.vault.path, "WIKI.md"), "# Memory Wiki\n", "utf8"),
    fs.writeFile(path.join(params.config.vault.path, ".steelengine-wiki", "log.jsonl"), "", "utf8"),
  ]);
  activateMemoryWikiCompiledCacheOwner(
    params.config,
