...
import type { AnyAgentTool, SteelEnginePluginToolFactory } from "steelengine/plugin-sdk/plugin-entry";
...
// Full plugin shards can push this real multi-vault integration past the
// generic 120-second unit-test budget; assertions still fail immediately.
const AGENT_VAULT_ISOLATION_TIMEOUT_MS = 240_000;
...
): Map<string, SteelEnginePluginToolFactory> {
...
function createMemoryCoreTool(params: {
  factories: Map<string, SteelEnginePluginToolFactory>;
...
describe("agent-scoped memory-wiki tools", () => {
  it(
    "keeps apply, search, and get behavior isolated by configured agent",
    async () => {
    ...
    AGENT_VAULT_ISOLATION_TIMEOUT_MS,
  );
});
