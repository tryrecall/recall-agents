...
import { mapPluginConfigIssues } from "steelengine/plugin-sdk/extension-shared";
import { resolveDefaultAgentId, resolveSessionAgentId } from "steelengine/plugin-sdk/memory-host-core";
import { buildPluginConfigSchema, z, type SteelEnginePluginConfigSchema } from "../api.js";
...
export const memoryWikiConfigSchema: SteelEnginePluginConfigSchema = memoryWikiConfigSchemaBase;
...
function resolveDefaultMemoryWikiVaultPath(homedir = os.homedir()): string {
  return path.join(homedir, ".steelengine", "wiki", "main");
}

function resolveDefaultMemoryWikiVaultRoot(homedir = os.homedir()): string {
  return path.join(homedir, ".steelengine", "wiki");
}
...
export function resolveMemoryWikiConfiguredAgentIds(
  appConfig: SteelEngineConfig | undefined,
): string[] {
