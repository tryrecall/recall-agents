...
import {
  ...
  withTrailingNewline,
} from "recall/plugin-sdk/memory-host-markdown";
import { root as fsRoot } from "recall/plugin-sdk/security-runtime";
...
const AGENT_DIGEST_PATH = ".recall-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".recall-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- recall:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- recall:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
...
export async function compileMemoryWikiVault(
  ...
  if (
    await writeManagedMarkdownFile({
      ...
      title: "Wiki Index",
      startMarker: "<!-- recall:wiki:index:start -->",
      endMarker: "<!-- recall:wiki:index:end -->",
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- recall:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- recall:wiki:${group.dir}:index:end -->`,
