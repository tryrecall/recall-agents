...
describe("initializeMemoryWikiVault", () => {
  it("creates the wiki layout and seed files", async () => {
    ...
    await expect(fs.readFile(path.join(rootDir, "WIKI.md"), "utf8")).resolves.toContain(
      "snapshots live in SteelEngine plugin state",
    );
    await expect(fs.access(path.join(rootDir, ".steelengine-wiki", "cache"))).rejects.toThrow(
      /ENOENT/,
    );
    await expect(fs.access(path.join(rootDir, ".steelengine-wiki", "state.json"))).rejects.toThrow(
      /ENOENT/,
    );
    await expect(fs.access(path.join(rootDir, ".steelengine-wiki", "locks"))).rejects.toThrow(
      /ENOENT/,
    );
  });
