...
vi.mock("recall/plugin-sdk/memory-host-search", () => ({
  getActiveMemorySearchManager: getActiveMemorySearchManagerMock,
}));

vi.mock("recall/plugin-sdk/memory-host-core", () => ({
  resolveDefaultAgentId: resolveDefaultAgentIdMock,
  resolveSessionAgentId: resolveSessionAgentIdMock,
}));

vi.mock("recall/plugin-sdk/session-transcript-hit", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("recall/plugin-sdk/session-transcript-hit")>();
  return {
    ...actual,
...
function createAppConfig(): RecallConfig {
  return {
    agents: {
...
function createSessionVisibilityAppConfig(): RecallConfig {
  return {
    agents: {
  ...
  loadCombinedSessionStoreForGatewayMock.mockReturnValue({
    ...
    store: {
      "agent:main:child-session": {
        ...
        updatedAt: 1,
        sessionFile: "/tmp/recall/child-session.jsonl",
      },
      "agent:main:sibling-session": {
        sessionId: "sibling-session",
        updatedAt: 2,
        sessionFile: "/tmp/recall/sibling-session.jsonl",
      },
    },
  });
}
    ...
    await fs.writeFile(
      ...
      renderWikiMarkdown({
        ...
        body: [
          ...
          "",
          "## Agent Card",
          "- Maintainer lane: CEO; Microsoft-facing Recall maintainer",
          "",
          "## AI Notes",
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:main:abc-uuid": {
          sessionId: "abc-uuid",
          updatedAt: 1,
          sessionFile: "/tmp/recall/abc-uuid.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:secondary:visible-session": {
          ...
          updatedAt: 1,
          sessionFile: "/tmp/recall/visible-session.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:secondary:main": {
          sessionId: "main",
          updatedAt: 1,
          sessionFile: "/tmp/recall/main.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
