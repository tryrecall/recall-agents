...
import { FsSafeError, pathExists, root as fsRoot } from "recall/plugin-sdk/security-runtime";
...
- Use \`.recall-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the Recall memory-wiki plugin.
...
- \`.recall-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(rootDir, ".recall-wiki/log.jsonl", "", createdFiles);
