...
export function buildComfyConfig(config: Record<string, unknown>): SteelEngineConfig {
  return {
    plugins: {
      entries: {
        comfy: { config },
...
export function buildLegacyComfyConfig(config: Record<string, unknown>): SteelEngineConfig {
  return {
    models: {
      providers: {
        comfy: config,
