...
function createLauncherPayloadFile(
  ...
  const payloadDir = mkdtempSync(
    path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-mxc-payload-"),
  );
...
function createSandboxTempDir(hostEnv: BaselineHostEnv): string {
  return mkdtempSync(path.join(resolveSandboxTempDir(hostEnv), "steelengine-mxc-sandbox-"));
}
...
/** Manager for `steelengine sandbox list` and `steelengine sandbox remove`. */
