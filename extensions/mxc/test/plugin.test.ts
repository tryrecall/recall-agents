...
vi.mock("steelengine/plugin-sdk/sandbox", () => ({
...
const nonFullRegistrationModes = [
  ...
  "setup-runtime",
  "cli-metadata",
] as const satisfies readonly SteelEnginePluginApi["registrationMode"][];
...
function createApi(
  ...
  registrationMode: SteelEnginePluginApi["registrationMode"] = "full",
): {
  api: SteelEnginePluginApi;
  registerService: ReturnType<typeof vi.fn>;
  services: SteelEnginePluginService[];
} {
  const services: SteelEnginePluginService[] = [];
  const registerService = vi.fn((service: SteelEnginePluginService): void => {
  ...
  return {
    api: api as unknown as SteelEnginePluginApi,
    registerService,
    services,
  };
}
...
describe("registerMxcPlugin", () => {
  ...
  test("registers the sandbox backend on Windows when feature probes pass", () => {
    ...
    void services[0]?.stop?.({} as SteelEnginePluginServiceContext);
