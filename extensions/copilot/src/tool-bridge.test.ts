...
import type { AnyAgentTool, SandboxContext } from "steelengine/plugin-sdk/agent-harness-runtime";
...
async function convertSteelEngineToolToSdkToolForTest(
  ...
  const bridge = await createCopilotToolBridge({
    ...
    createSteelEngineCodingTools: async () => [sourceTool],
    modelId: "gpt-test",
...
describe("createCopilotToolBridge", () => {
  it("returns empty arrays for unsupported providers without calling the seam", async () => {
    const createSteelEngineCodingTools = vi.fn(async () => [makeTool()]);
    ...
    expect(createSteelEngineCodingTools).toHaveBeenCalledTimes(0);
  });

  it("allows vetted BYOK providers to expose model tools", async () => {
    const sourceTools = [makeTool()];
    const createSteelEngineCodingTools = vi.fn(async () => sourceTools);
    ...
    expect(createSteelEngineCodingTools).toHaveBeenCalledWith(
  ...
  it("forwards supported fields to injected createSteelEngineCodingTools", async () => {
    ...
    expect(createSteelEngineCodingTools).toHaveBeenCalledTimes(1);
  ...
  it("returns sdkTools and sourceTools with matching lengths", async () => {
    ...
    const result = await createCopilotToolBridge({
      agentId: "agent-1",
      createSteelEngineCodingTools: async () => sourceTools,
      modelId: "gpt-4o",
      modelProvider: "github-copilot",
      sessionId: "session-1",
    });
  ...
  it("preserves direct-only SteelEngine through the exact Copilot allowlist", async () => {
    ...
    const result = await createCopilotToolBridge({
      agentId: "steelengine",
      attemptParams: {
        runId: "steelengine-turn-1",
        sessionKey: "agent:steelengine:main",
        toolsAllow: ["steelengine"],
      } as never,
      createSteelEngineCodingTools: async () => [systemAgentTool],
      modelId: "gpt-4.1",
      modelProvider: "github-copilot",
    ...
    expect(result.sdkTools.map((tool) => tool.name)).toEqual(["steelengine"]);
  });

  it("compacts the Copilot tool surface behind tool_search controls when enabled", async () => {
    const createSteelEngineCodingTools = vi.fn(async (opts: unknown) => {
  ...
  it("compacts the Copilot tool surface behind code-mode exec/wait when enabled", async () => {
    const createSteelEngineCodingTools = vi.fn(async () => [
      makeTool({ name: "fake_hidden" }),
      makeTool({ name: "read" }),
    ]);
  ...
  it("throws when createSteelEngineCodingTools returns a non-array", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createSteelEngineCodingTools: async () => ({ tools: [] }) as never,
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      }),
    ).rejects.toThrow("createSteelEngineCodingTools must return an array");
  });

  it("throws when createSteelEngineCodingTools rejects and includes the cause", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createSteelEngineCodingTools: async () => {
  ...
  // F6: PI-parity tool context. The bridged SteelEngine tools register
  ...
  describe("PI-parity attempt context (F6)", () => {
    function captureCall() {
      ...
      return {
        createSteelEngineCodingTools,
        getOpts: () =>
          (createSteelEngineCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as Record<
            string,
            unknown
          >,
      };
    ...
    it("forwards identity, owner/policy, and channel/routing fields from attemptParams", async () => {
      const { createSteelEngineCodingTools, getOpts } = captureCall();
    ...
    it("prefers the unscoped toolAuthProfileStore when building SteelEngine tools", async () => {
      const { createSteelEngineCodingTools, getOpts } = captureCall();
  ...
  describe("sandbox forwarding (PR #86155 [P1])", () => {
    ...
    it("defaults sandbox to undefined and derives spawnWorkspaceDir from workspaceDir when no sandbox is passed (back-compat)", async () => {
      ...
      const opts = (createSteelEngineCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        sandbox?: unknown;
    ...
    it("forwards an explicit sandbox and spawnWorkspaceDir verbatim to createSteelEngineCodingTools", async () => {
  ...
  // SteelEngine attempt would suppress. These tests pin the contract.
  describe("tool-surface gating (PR #86155 [P1] round-6)", () => {
    it("short-circuits when attemptParams.disableTools is true and never calls createSteelEngineCodingTools", async () => {
      const createSteelEngineCodingTools = vi.fn(async () => [makeTool()]);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
    ...
    it("leaves the tool list unchanged when toolsAllow is undefined", async () => {
      ...
      const createSteelEngineCodingTools = vi.fn(async () => tools);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
  ...
  describe("tool-name aliases (PR #86155 [P1] round-7)", () => {
    ...
    it("does not discard lean-mode overrides after tool construction", async () => {
      const result = await createCopilotToolBridge({
        ...
        createSteelEngineCodingTools: async () => [makeTool({ name: "image_generate" })],
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      });
    ...
    it("keeps core tools available for glob allowlists", async () => {
      ...
      const options = (createSteelEngineCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        toolConstructionPlan?: { includeSteelEngineTools?: boolean };
      };
      expect(options?.toolConstructionPlan?.includeSteelEngineTools).toBe(true);
    });
...
describe("createCopilotToolBridge tool conversion", () => {
  it("throws on empty and non-string names", async () => {
    await expect(
      convertSteelEngineToolToSdkToolForTest(makeTool({ name: "" as never }), {}),
    ).rejects.toThrow("tool name must be a non-empty string");
    await expect(
      convertSteelEngineToolToSdkToolForTest(makeTool({ name: 42 as never }), {}),
    ).rejects.toThrow("tool name must be a non-empty string");
  });

  it("throws on non-function execute", async () => {
    await expect(
      convertSteelEngineToolToSdkToolForTest(makeTool({ execute: "nope" as never }), {}),
  ...
  it("preserves name, description, and parameters exactly", async () => {
    ...
    const result = await convertSteelEngineToolToSdkToolForTest(sourceTool, {});
  ...
  it("sets skipPermission: true so SteelEngine's wrapped-tool internal enforcement handles permission decisions (PI-parity model)", async () => {
    // Per the harness docs: every bridged SteelEngine tool comes from
    // `createSteelEngineCodingTools`, which already wraps each tool with
    ...
    // for kind: "custom-tool" would either short-circuit SteelEngine's
    // richer enforcement (allow-all) or block every call (reject-all).
    // Setting `skipPermission: true` lets the wrapped execute() run
    // SteelEngine's hook with the right context — mirrors codex
    // (`extensions/codex/src/app-server/dynamic-tools.ts`).
    const result = (await convertSteelEngineToolToSdkToolForTest(makeTool(), {})) as SdkTool & {
  ...
  it("marks every bridged tool as overridesBuiltInTool so SteelEngine owns names that collide with Copilot CLI built-ins (edit/read/write/bash/...)", async () => {
    // Real-world dogfood found that steelengine's createSteelEngineCodingTools
    ...
    for (const name of ["edit", "read", "write", "bash", "live_echo"]) {
      const result = (await convertSteelEngineToolToSdkToolForTest(
        makeTool({ name }),
        {},
  ...
  it("returns a failure result when the signal is already aborted", async () => {
    ...
    controller.abort();
    const sourceTool = makeTool();
    const sdkTool = await convertSteelEngineToolToSdkToolForTest(sourceTool, {
      abortSignal: controller.signal,
    });
  ...
  it("calls beforeExecute with the invocation context before execute", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = await convertSteelEngineToolToSdkToolForTest(sourceTool, { beforeExecute });
  ...
  it("calls prepareArguments and passes the prepared args and toolCallId to execute", async () => {
    ...
    const sdkTool = await convertSteelEngineToolToSdkToolForTest(sourceTool, { onToolCompleted });
  ...
  it("converts single text content to an exact textResultForLlm", async () => {
    ...
    const sdkTool = await convertSteelEngineToolToSdkToolForTest(makeTool({}, sourceResult), {
      onAgentToolResult,
    });
  ...
  it("reports thrown tool failures to the private result observer", async () => {
    ...
    const onAgentToolResult = vi.fn();
    const sdkTool = await convertSteelEngineToolToSdkToolForTest(
      makeTool({
        execute: vi.fn(async () => {
          throw error;
        }),
  ...
  it("reports returned SteelEngine error results to both tool observers", async () => {
    const onAgentToolResult = vi.fn();
    const onToolCompleted = vi.fn();
    const sourceResult = {
  ...
  it("reports catalog tool failures to the host terminal observer", async () => {
    ...
    await createCopilotToolBridge({
      ...
      createSteelEngineCodingTools: async (options: unknown) => {
