 ...
 *     use cases.
 *   - Falls back to STEELENGINE_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *     (`<steelEngineHome>/.steelengine/agents/<agentId>/copilot`, or
 *     `<agentDir>/copilot` when an agent directory is supplied) that
 *     respects `STEELENGINE_HOME` for the home directory root.
 ...
 *   4. STEELENGINE_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  // "copilot" subdir so it cannot collide with SteelEngine's own files
  ...
  const steelEngineHome = readString(args.env.STEELENGINE_HOME);
  const rootHome = steelEngineHome ? resolve(steelEngineHome) : safeHomeDir(args.homeDir);
  // Per-agent isolation per proposal section 3.6:
  //   <steelEngineHome>/.steelengine/agents/<agentId>/copilot
  return resolve(join(rootHome, ".steelengine", "agents", args.agentId, "copilot"));
}
...
function readEnvTokenFallback(
  ...
  // STEELENGINE_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "STEELENGINE_GITHUB_TOKEN", value: readString(env.STEELENGINE_GITHUB_TOKEN) },
