...
describe("resolveCopilotAuth - copilotHome resolution", () => {
  ...
  it("synthesises per-agent default from homeDir when no path is given", () => {
    ...
    expect(result.copilotHome).toBe(
      resolve(join(FAKE_HOME, ".steelengine", "agents", "agent-1", "copilot")),
    );
  });

  it("respects STEELENGINE_HOME env var as the home root", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: { STEELENGINE_HOME: "/custom/steelengine" } as NodeJS.ProcessEnv,
      homeDir: fakeHomeDir,
    });
    expect(result.copilotHome).toBe(
      resolve(join("/custom/steelengine", ".steelengine", "agents", "agent-1", "copilot")),
    );
  });
  ...
  it("uses the default agent id when agentId is invalid/missing", () => {
    ...
    expect(result.copilotHome).toBe(
      resolve(join(FAKE_HOME, ".steelengine", "agents", COPILOT_DEFAULT_AGENT_ID, "copilot")),
    );
  });
...
describe("resolveCopilotAuth - contract-resolved auth (resolvedApiKey + authProfileId)", () => {
  ...
  it("contract resolvedApiKey takes precedence over env fallback", () => {
    const result = resolveCopilotAuth({
      ...
      authProfileId: "p",
      env: {
        STEELENGINE_GITHUB_TOKEN: "env-should-be-ignored",
...
describe("resolveCopilotAuth - env var fallbacks", () => {
  ...
  it("STEELENGINE_GITHUB_TOKEN takes precedence over GITHUB_TOKEN", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: {
        STEELENGINE_GITHUB_TOKEN: "steelengine-tok",
        GITHUB_TOKEN: "github-tok",
      } as NodeJS.ProcessEnv,
      homeDir: fakeHomeDir,
    });
    expect(result.gitHubToken).toBe("steelengine-tok");
    expect(result.authProfileId).toBe("env:STEELENGINE_GITHUB_TOKEN");
    expect(result.authProfileVersion).toBe(tokenFingerprint("steelengine-tok"));
  });
  ...
  it("STEELENGINE_GITHUB_TOKEN takes precedence over COPILOT_GITHUB_TOKEN, GH_TOKEN and GITHUB_TOKEN", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: {
        STEELENGINE_GITHUB_TOKEN: "steelengine-tok",
  ...
  it("explicit auth.useLoggedInUser=true wins over env tokens", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      auth: { useLoggedInUser: true },
      env: { STEELENGINE_GITHUB_TOKEN: "env-tok" } as NodeJS.ProcessEnv,
      homeDir: fakeHomeDir,
    });
    expect(result.authMode).toBe("useLoggedInUser");
  });
...
describe("resolveCopilotAuth - defaults wiring", () => {
  ...
  it("uses os.homedir() when homeDir is not injected", () => {
    ...
    expect(result.copilotHome.endsWith(join(".steelengine", "agents", "agent-1", "copilot"))).toBe(
      true,
    );
  });
  ...
  it("falls back to process.cwd() if homeDir throws", () => {
    ...
    expect(result.copilotHome.includes(join(".steelengine", "agents", "agent-1", "copilot"))).toBe(
      true,
    );
  });
});
