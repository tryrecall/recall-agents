...
// Filtering them out of the SteelEngine bootstrap injection avoids
...
/**
 * Loads SteelEngine workspace bootstrap files (IDENTITY.md, SOUL.md,
...
export async function resolveCopilotWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    // SOUL.md / IDENTITY.md / .steelengine conventions live) onto the
...
function renderCopilotWorkspaceBootstrapInstructions(
  ...
  const lines: string[] = [
    "SteelEngine loaded these user-editable workspace files. Treat them as project/user context. The Copilot SDK loads AGENTS.md natively from its instruction directories, so AGENTS.md is not repeated here.",
    "",
    "# Project Context",
    "",
