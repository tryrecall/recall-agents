...
import { attachModelProviderRequestTransport } from "steelengine/plugin-sdk/agent-harness-runtime";
import type {
...
describe("createCopilotAgentHarness", () => {
  ...
  describe("reset", () => {
    ...
    it("does not invoke deleteSession for a session belonging to a different steelengineSessionId", async () => {
      const pool = makePoolMock();
      const deleteSession = vi.fn().mockResolvedValue(undefined);
      const client = createMockCopilotClient({ deleteSession });
      mocks.runCopilotAttempt.mockImplementation(async (params, deps) => {
  ...
  it("aborts deferred compaction cleanup when the SteelEngine session resets", async () => {
    const cleanup = createDeferred<"aborted" | "completed" | "deadline">();
    const abort = vi.fn(() => cleanup.resolve("aborted"));
    mocks.runCopilotAttempt.mockImplementation(async (_params, deps) => {
      deps.onSessionEstablished?.({
  ...
  describe("session reuse across turns (dogfood finding #4)", () => {
    ...
    // `runAttempt` calls within the same SteelEngine session should pass
