...
export function createCopilotAgentHarness(
  ...
  >();
  // Maps SteelEngine session id (from AgentHarnessAttemptParams.sessionId) to
  ...
  return {
    ...
    async runAttempt(params: AgentHarnessAttemptParams): Promise<AgentHarnessAttemptResult> {
      const attemptPromise = (async () => {
        ...
        const steelengineSessionId =
          typeof params.sessionId === "string" ? params.sessionId : undefined;

        // Dogfood finding #4: reuse the SDK session across turns within
        // the same SteelEngine session so that the GitHub Copilot agent runtime's prompt
        ...
        const compactionCleanupPending =
          steelengineSessionId !== undefined && hasPendingDeferredCompactionCleanup(steelengineSessionId);
        const replayBlocked =
          steelengineSessionId !== undefined &&
          (compactionCleanupPending || resetBlockedStoredSessions.has(steelengineSessionId));
        const tracked =
          steelengineSessionId && !replayBlocked ? trackedSessions.get(steelengineSessionId) : undefined;
        const stored = steelengineSessionId
          ? replayBlocked
            ? undefined
            : lookupStoredBinding(options?.sessionStore, steelengineSessionId)
          : undefined;
        ...
        return runCopilotAttempt(effectiveParams, {
          pool,
          onSessionEstablished: steelengineSessionId
            ? ({
                ...
                trackedSessions.set(steelengineSessionId, {
                  sdkSessionId,
                ...
                registerStoredBinding(options?.sessionStore, steelengineSessionId, {
                  schemaVersion: 2,
                  sdkSessionId,
                  compatKey: currentCompatKey,
                  compactKey: currentCompactKey,
                ...
                resetBlockedStoredSessions.delete(steelengineSessionId);
              }
            : undefined,
          onDeferredCompaction: steelengineSessionId
            ? ({
                abort,
                cleanup,
                sdkSessionId,
                ...
                const trackedBinding = trackedSessions.get(steelengineSessionId);
                const storedBinding = lookupStoredBinding(options?.sessionStore, steelengineSessionId);
                ...
                resetBlockedStoredSessions.add(steelengineSessionId);
                void cleanup.then((outcome) => {
                  const currentTracked = trackedSessions.get(steelengineSessionId);
                  const currentStored = lookupStoredBinding(
                    options?.sessionStore,
                    steelengineSessionId,
                  );
                  ...
                  if (stillOwnsTrackedSession) {
                    trackedSessions.delete(steelengineSessionId);
                  }
                  if (stillOwnsStoredSession) {
                    deleteStoredBinding(options?.sessionStore, steelengineSessionId);
                  }
                  if (stillOwnsTrackedSession || stillOwnsStoredSession) {
                    resetBlockedStoredSessions.add(steelengineSessionId);
                  }
    ...
    async reset(params: AgentHarnessResetParams): Promise<void> {
      const steelengineSessionId = typeof params.sessionId === "string" ? params.sessionId : undefined;
      if (!steelengineSessionId) {
        return;
      }
      ...
      const tracked = trackedSessions.get(steelengineSessionId);
      const stored = lookupStoredBinding(options?.sessionStore, steelengineSessionId);
      resetBlockedStoredSessions.add(steelengineSessionId);
      await abortDeferredCompactionCleanups(steelengineSessionId);
      const currentStored = lookupStoredBinding(options?.sessionStore, steelengineSessionId);
      const stillOwnsStoredSession =
        stored !== undefined && currentStored?.sdkSessionId === stored.sdkSessionId;
      if (stillOwnsStoredSession) {
        if (deleteStoredBinding(options?.sessionStore, steelengineSessionId)) {
          resetBlockedStoredSessions.delete(steelengineSessionId);
        }
      } else {
        resetBlockedStoredSessions.delete(steelengineSessionId);
      ...
      if (trackedSessions.get(steelengineSessionId)?.sdkSessionId === tracked.sdkSessionId) {
        trackedSessions.delete(steelengineSessionId);
      }
      try {
    ...
    async compact(
      ...
      // The SDK owns Copilot history compaction. SteelEngine only resumes
      // the tracked SDK session and calls the session-scoped RPC; durable
      // SteelEngine session/transcript state stays in SQLite, with no marker
      ...
      if (hasPendingDeferredCompactionCleanup(steelengineSessionId)) {
        return {
          ok: false,
          compacted: false,
      ...
      if (!compatibleTracked) {
        ...
        // needs the live SessionConfig with SteelEngine hooks/tools, so preserve the
