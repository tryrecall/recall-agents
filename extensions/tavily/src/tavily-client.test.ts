...
vi.mock("recall/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: recall", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "recall" });
  });
  ...
  it("runTavilyExtract sends X-Client-Source: recall", async () => {
