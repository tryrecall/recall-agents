...
const LIVE = process.env.STEELENGINE_LIVE_TEST === "1" && process.env.STEELENGINE_LIVE_OLLAMA === "1";
const OLLAMA_BASE_URL =
  process.env.STEELENGINE_LIVE_OLLAMA_BASE_URL?.trim() || "http://127.0.0.1:11434";
const CHAT_MODEL = process.env.STEELENGINE_LIVE_OLLAMA_MODEL?.trim() || "llama3.2:latest";
const EMBEDDING_MODEL =
  process.env.STEELENGINE_LIVE_OLLAMA_EMBED_MODEL?.trim() || "embeddinggemma:latest";
const PROVIDER_ID = process.env.STEELENGINE_LIVE_OLLAMA_PROVIDER_ID?.trim() || "ollama-live-custom";
const RUN_WEB_SEARCH = process.env.STEELENGINE_LIVE_OLLAMA_WEB_SEARCH !== "0";
const RUN_EMBEDDINGS =
  process.env.STEELENGINE_LIVE_OLLAMA_EMBEDDINGS === "1" ||
  (process.env.STEELENGINE_LIVE_OLLAMA_EMBEDDINGS !== "0" && !isOllamaCloudBaseUrl(OLLAMA_BASE_URL));
...
function requireOllamaRuntimeApiKey(): string | undefined {
  ...
  if (!apiKey) {
    throw new Error(
      "STEELENGINE_LIVE_OLLAMA_BASE_URL points at a remote Ollama host; set OLLAMA_API_KEY.",
    );
  }
  return apiKey;
}
...
async function withTempSteelEngineState<T>(run: (paths: { root: string }) => Promise<T>): Promise<T> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-ollama-cli-live-"));
  try {
    await fs.writeFile(
      path.join(root, "steelengine.json"),
      JSON.stringify(
        {
          models: {
            providers: {
...
async function runSteelEngineCli(args: string[], env: NodeJS.ProcessEnv) {
  ...
  const commandArgs = sourceRunnerAvailable
    ...
    : ["steelengine.mjs", ...args];
  const outputRoot = fsSync.mkdtempSync(path.join(os.tmpdir(), "steelengine-ollama-cli-output-"));
...
function buildCliEnv(root: string): NodeJS.ProcessEnv {
  ...
  return {
    ...
    STEELENGINE_LIVE_OLLAMA: "1",
    STEELENGINE_LIVE_OLLAMA_WEB_SEARCH: "0",
...
describe.skipIf(!LIVE)("ollama live", () => {
  it("runs infer model run through the local CLI path without static model discovery", async () => {
    await withTempSteelEngineState(async ({ root }) => {
      const result = await runSteelEngineCli(
        [
          "infer",
          "model",
          "run",
  ...
  it.skipIf(!RUN_WEB_SEARCH)(
    ...
    async () => {
      ...
      const result = (await tool.execute({
        query: "SteelEngine documentation",
        count: 1,
      })) as {
        provider?: string;
