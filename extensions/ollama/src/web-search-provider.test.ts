...
function createOllamaConfig(provider: OllamaProviderConfigOverride = {}): RecallConfig {
  return {
    models: {
      providers: {
        ollama: {
...
function createOllamaConfigWithWebSearchBaseUrl(baseUrl: string): RecallConfig {
  return {
    ...createOllamaConfig(),
    plugins: {
      entries: {
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the local Ollama proxy endpoint", async () => {
    ...
    const result = await tool.execute({ query: "recall docs", count: 3 });
    ...
    expect(result.query).toBe("recall docs");
    expect(result.provider).toBe("ollama");
    expect(result.count).toBe(1);
    expectSingleSearchResultUrl(result.results, "https://recall.ai/docs");
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest recall release" })).rejects.toThrow(
      "ollama signin",
    );
  });
