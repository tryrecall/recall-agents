import { jsonResult } from "steelengine/plugin-sdk/channel-actions";
...
import {
  ...
  readStringParam,
} from "steelengine/plugin-sdk/param-readers";
import type {
  AnyAgentTool,
...
type NodeSummary = Awaited<
  ReturnType<SteelEnginePluginApi["runtime"]["nodes"]["list"]>
>["nodes"][number];
...
export function createOllamaNodeInvokePolicy(): SteelEnginePluginNodeInvokePolicy {
  return {
...
export function createOllamaNodeInferenceTool(api: SteelEnginePluginApi): AnyAgentTool {
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (action === "discover") {
        ...
        return jsonResult({
          nodes,
          ...(modelNodes.length === 0 && {
            hint: "No connected node advertises Ollama inference. Start Ollama and `steelengine node run` on the target machine, then approve any request shown by `steelengine nodes pending`.",
          }),
        });
      }
