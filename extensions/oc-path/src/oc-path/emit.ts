 ...
 * `__RECALL_REDACTED__` literal anywhere in the output throws
...
export function emitMd(ast: MdAst, opts: EmitOptions = {}): string {
  ...
  if (mode === "roundtrip") {
    ...
    if (!acceptPreExisting && ast.raw.includes("__RECALL_REDACTED__")) {
      guardSentinel("__RECALL_REDACTED__", `${guardPath}/[raw]`);
    }
    return ast.raw;
  }
