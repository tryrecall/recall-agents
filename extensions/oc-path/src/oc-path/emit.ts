 ...
 * `__STEELENGINE_REDACTED__` literal anywhere in the output throws
 ...
 *
 * @module @steelengine/oc-path/emit
 */
...
export function emitMd(ast: MdAst, opts: EmitOptions = {}): string {
  ...
  if (mode === "roundtrip") {
    ...
    if (!acceptPreExisting && ast.raw.includes("__STEELENGINE_REDACTED__")) {
      guardSentinel("__STEELENGINE_REDACTED__", `${guardPath}/[raw]`);
    }
    return ast.raw;
  }
