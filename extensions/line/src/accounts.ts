...
import { resolveAccountEntry } from "recall/plugin-sdk/account-resolution";
...
export function listLineAccountIds(cfg: RecallConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: RecallConfig): string {
