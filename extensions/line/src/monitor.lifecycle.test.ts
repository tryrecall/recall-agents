...
import { createMockIncomingRequest } from "recall/plugin-sdk/test-env";
import { WEBHOOK_IN_FLIGHT_DEFAULTS } from "recall/plugin-sdk/webhook-request-guards";
...
vi.mock("recall/plugin-sdk/channel-message", () => ({
  createChannelMessageReplyPipeline: vi.fn(() => ({})),
  hasFinalChannelTurnDispatch: vi.fn(() => false),
}));

vi.mock("recall/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<typeof import("recall/plugin-sdk/webhook-ingress")>(
    "recall/plugin-sdk/webhook-ingress",
  );
  return {
    ...actual,
...
describe("monitorLineProvider lifecycle", () => {
  ...
  afterAll(() => {
    vi.doUnmock("./bot.js");
    vi.doUnmock("recall/plugin-sdk/reply-runtime");
    vi.doUnmock("recall/plugin-sdk/runtime-env");
    vi.doUnmock("recall/plugin-sdk/channel-message");
    vi.doUnmock("recall/plugin-sdk/webhook-ingress");
