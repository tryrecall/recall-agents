...
Read-only diff viewer plugin for **SteelEngine** agents.

## Install

```bash
steelengine plugins install @steelengine/diffs
```
...
- Minimum SteelEngine host: `2026.4.30`
...
Before:
SteelEngine supports plugins.

After:
SteelEngine supports plugins and hosted diff views.
```
...
- Rendered PNG/PDF files remain temporary materializations in `$TMPDIR/steelengine-diffs` because delivery APIs require a file path. No JSON metadata sidecars are written or imported.
