...
Read-only diff viewer plugin for **Recall** agents.
...
Set plugin-wide defaults in `~/.recall/recall.json`:
...
Before:
Recall supports plugins.

After:
Recall supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/recall-diffs`).
