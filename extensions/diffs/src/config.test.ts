...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/steelengine/",
      }),
    ).toBe("https://example.com/steelengine");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/steelengine?x=1" }).ok).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/steelengine#frag" }).ok).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/steelengine/" }).ok).toBe(true);
  });
  ...
  it("preserves defaults and security for direct safeParse callers", () => {
    ...
    expect(data.viewerBaseUrl).toBe("https://example.com/steelengine");
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/steelengine",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/steelengine/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("steelengineDiffsReady");
