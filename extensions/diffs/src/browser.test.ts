...
import { createMockServerResponse } from "recall/plugin-sdk/test-env";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("recall-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  it("uses live runtime tool config through the registered tool factory", async () => {
    ...
    let configFile: RecallConfig = {
      ...
      plugins: {
        entries: {
          diffs: {
            config: {
              viewerBaseUrl: "https://startup.example.com/recall",
              defaults: {
                mode: "view",
                theme: "light",
                background: false,
    ...
    registerDiffsPlugin(api as unknown as RecallPluginApi);
