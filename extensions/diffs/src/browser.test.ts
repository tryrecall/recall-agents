...
import type { SteelEnginePluginApi, SteelEnginePluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    ...
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("steelengine-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  it("uses live runtime tool config through the registered tool factory", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<SteelEnginePluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: SteelEnginePluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    let registeredHttpRouteHandler: HttpRouteHandler | undefined;
    let configFile: SteelEngineConfig = {
      gateway: {
        port: 18789,
        bind: "loopback",
      },
      plugins: {
        entries: {
          diffs: {
            config: {
              viewerBaseUrl: "https://startup.example.com/steelengine",
              defaults: {
                mode: "view",
                theme: "light",
                background: false,
    ...
    const api = createTestPluginApi({
      ...
      registerTool(tool: Parameters<SteelEnginePluginApi["registerTool"]>[0]) {
    ...
    registerDiffsPlugin(api as unknown as SteelEnginePluginApi);
