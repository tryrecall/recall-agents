...
declare const __STEELENGINE_DIFFS_LANGUAGE_PACK__: boolean | undefined;

// Build-time esbuild define; typeof guard keeps the module loadable where the
// define is absent (vitest/node), matching the __STEELENGINE_VERSION__ pattern.
function readInjectedLanguagePackFlag(): boolean | undefined {
  return typeof __STEELENGINE_DIFFS_LANGUAGE_PACK__ === "boolean"
    ? __STEELENGINE_DIFFS_LANGUAGE_PACK__
    : undefined;
}
...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-steelengine-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-steelengine-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.steelengineDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.steelengineDiffsError = "true";
