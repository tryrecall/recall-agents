...
import { resolvePreferredSteelEngineTmpDir } from "../api.js";
...
export async function createTempDiffRoot(prefix: string): Promise<{
  ...
  const rootDir = await fs.mkdtemp(path.join(resolvePreferredSteelEngineTmpDir(), prefix));
  return {
    rootDir,
    cleanup: async () => {
...
export async function createDiffStoreHarness(prefix: string): Promise<{
  ...
  const env = {
    ...process.env,
    STEELENGINE_STATE_DIR: path.join(harnessRoot, "state"),
  };
  const openBlobStore = () =>
