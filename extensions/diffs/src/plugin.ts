...
export function registerDiffsPlugin(api: SteelEnginePluginApi): void {
  const store = new DiffArtifactStore({
    rootDir: path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-diffs"),
  ...
  const resolveCurrentAccessConfig = () => {
    const currentConfig = (api.runtime.config?.current?.() ?? api.config) as SteelEngineConfig;
...
function resolveDiffsLanguagePackAvailability(api: SteelEnginePluginApi): boolean {
  const currentConfig = (api.runtime.config?.current?.() ?? api.config) as SteelEngineConfig;
...
function hasSiblingLanguagePackRuntime(rootDir: string | undefined): boolean {
  ...
  return (
    fs.existsSync(path.join(languagePackRoot, "steelengine.plugin.json")) &&
