...
import type { SteelEngineConfig, SteelEnginePluginApi, SteelEnginePluginToolContext } from "../api.js";
...
describe("diffs plugin language-pack discovery", () => {
  it.each(["assets", "dist/assets"])(
    ...
    (assetDir) => {
      ...
      const root = fs.mkdtempSync(join(os.tmpdir(), "steelengine-diffs-language-pack-"));
      try {
        ...
        fs.writeFileSync(
          join(languagePackRoot, "steelengine.plugin.json"),
          '{"id":"diffs-language-pack"}\n',
        );
        const config = { plugins: {} } as SteelEngineConfig;
        ...
        } satisfies SteelEnginePluginToolContext;
