...
import { createReplyToFanout } from "recall/plugin-sdk/outbound-runtime";
import { resolvePayloadMediaUrls } from "recall/plugin-sdk/reply-payload";
...
const MATRIX_RECALL_PRESENTATION_KEY = "com.recall.presentation" as const;
const MATRIX_RECALL_PRESENTATION_TYPE = "message.presentation" as const;
...
function buildMatrixPresentationContent(presentation: MessagePresentation) {
  return {
    ...presentation,
    version: 1,
    type: MATRIX_RECALL_PRESENTATION_TYPE,
  };
}
...
function resolveMatrixPresentationContent(
  ...
  const presentation = toRecord(extraContent?.[MATRIX_RECALL_PRESENTATION_KEY]);
  if (
    !presentation ||
    presentation.version !== 1 ||
    presentation.type !== MATRIX_RECALL_PRESENTATION_TYPE
  ) {
    return undefined;
  }
  return presentation;
  ...
  return {
    ...
    channelData: {
      ...
      matrix: {
        ...matrixData,
        extraContent: {
          [MATRIX_RECALL_PRESENTATION_KEY]: buildMatrixPresentationContent(params.presentation),
        },
      },
    },
  };
...
function resolveMatrixExtraContent(payload: ReplyPayload): MatrixExtraContentFields | undefined {
  ...
  return presentation ? { [MATRIX_RECALL_PRESENTATION_KEY]: presentation } : undefined;
}
