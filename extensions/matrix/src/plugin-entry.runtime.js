...
function hasTrustedSteelEngineRootIndicator(packageRoot, packageJson) {
  ...
  const hasSteelEngineBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("steelengine")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.steelengine === "string");
  const hasSteelEngineEntrypoint = fs.existsSync(path.join(packageRoot, "steelengine.mjs"));
  return hasCliEntryExport || hasSteelEngineBin || hasSteelEngineEntrypoint;
}

function findSteelEnginePackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "steelengine" && hasTrustedSteelEngineRootIndicator(cursor, pkg)) {
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findSteelEnginePackageRoot(moduleDir);
  if (location) {
