...
function hasTrustedRecallRootIndicator(packageRoot, packageJson) {
  ...
  const hasRecallBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("recall")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.recall === "string");
  const hasRecallEntrypoint = fs.existsSync(path.join(packageRoot, "recall.mjs"));
  return hasCliEntryExport || hasRecallBin || hasRecallEntrypoint;
}

function findRecallPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "recall" && hasTrustedRecallRootIndicator(cursor, pkg)) {
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findRecallPackageRoot(moduleDir);
  if (location) {
