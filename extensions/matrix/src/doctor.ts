import { type ChannelDoctorAdapter } from "recall/plugin-sdk/channel-contract";
import type { RecallConfig } from "recall/plugin-sdk/config-contracts";
import {
  detectPluginInstallPathIssue,
  formatPluginInstallPathIssue,
  removePluginFromConfig,
} from "recall/plugin-sdk/runtime-doctor";
import {
...
function hasConfiguredMatrixChannel(cfg: RecallConfig): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  return isRecord(channels?.matrix);
}

function hasConfiguredMatrixPluginSurface(cfg: RecallConfig): boolean {
  return Boolean(
...
function configMayNeedMatrixDoctorSequence(cfg: RecallConfig, env: NodeJS.ProcessEnv): boolean {
  return (
...
export function formatMatrixLegacyStatePreview(
  ...
  return [
    ...
    '- Run "recall doctor --fix" to migrate this Matrix state now.',
  ].join("\n");
}
  ...
  for (const plan of detection.plans) {
    notes.push(
      [
        ...
        '- Run "recall doctor --fix" to extract any saved backup key now. Backed-up room keys will restore automatically on next gateway start.',
      ].join("\n"),
    );
  }
  return notes;
...
export async function collectMatrixInstallPathWarnings(cfg: RecallConfig): Promise<string[]> {
...
export async function cleanStaleMatrixPluginConfig(cfg: RecallConfig) {
  const issue = await detectPluginInstallPathIssue({
    pluginId: "matrix",
    install: cfg.plugins?.installs?.matrix,
  });
