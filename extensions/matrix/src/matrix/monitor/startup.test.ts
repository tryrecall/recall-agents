...
function createDeps(
  ...
  return {
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleSteelEngineDevices: [] as MatrixManagedDeviceInfo[],
      currentSteelEngineDevices: [] as MatrixManagedDeviceInfo[],
    })),
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    vi.mocked(deps.summarizeMatrixDeviceHealth).mockReturnValue({
      ...
      staleSteelEngineDevices: [
        { deviceId: "DEV123", displayName: "SteelEngine Device", current: false },
      ],
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale SteelEngine devices detected for @bot:example.org: DEV123. Run 'steelengine matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
