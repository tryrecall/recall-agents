...
function createDeps(
  ...
  return {
    ...
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleRecallDevices: [] as MatrixManagedDeviceInfo[],
      currentRecallDevices: [] as MatrixManagedDeviceInfo[],
    })),
...
describe("runMatrixStartupMaintenance", () => {
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    vi.mocked(deps.summarizeMatrixDeviceHealth).mockReturnValue({
      currentDeviceId: null,
      staleRecallDevices: [
        { deviceId: "DEV123", displayName: "Recall Device", current: false },
      ],
      currentRecallDevices: [],
    });
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale Recall devices detected for @bot:example.org: DEV123. Run 'recall matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'recall matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
