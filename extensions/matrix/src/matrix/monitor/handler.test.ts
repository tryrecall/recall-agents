...
import { getSessionEntry, upsertSessionEntry } from "steelengine/plugin-sdk/session-store-runtime";
...
function expectFinalizedPreviewEdit(eventId: string, text: string) {
  ...
  expect(options.extraContent).toEqual({ [MATRIX_STEELENGINE_FINALIZED_PREVIEW_KEY]: true });
}
...
describe("matrix monitor handler draft streaming", () => {
  ...
  it("replaces recovered Matrix command progress instead of leaving stale failed text", async () => {
    ...
    await opts.onItemEvent?.({
      ...
      phase: "end",
      status: "failed",
      progressText: "run steelengine cron -> run jq (agent) failed",
    });
    await opts.onItemEvent?.({
      itemId: "command-1",
      kind: "command",
    ...
    expect(recoveredEdit?.[2]).not.toContain("run steelengine cron -> run jq");
    vi.useRealTimers();
  });
