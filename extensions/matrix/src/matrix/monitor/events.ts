...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'steelengine matrix verify status --verbose --account ${accountId}' and 'steelengine matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another SteelEngine runtime is using the same account. " +
    `Check 'steelengine matrix verify status --verbose --account ${accountId}' and 'steelengine matrix devices list --account ${accountId}'.`
  );
}
