  ...
  it("classifies repeated fresh post-healthy-sync decrypt failures separately", async () => {
    ...
    try {
      ...
      expectWarnContextFields(
        logger,
        4,
        "matrix: repeated fresh encrypted messages are still failing to decrypt after Matrix resumed healthy sync. This device may still be missing new room keys. Check 'recall matrix verify status --verbose --account ops' and 'recall matrix devices list --account ops'.",
        {
          failureCount: 3,
          roomCount: 3,
          senderCount: 3,
