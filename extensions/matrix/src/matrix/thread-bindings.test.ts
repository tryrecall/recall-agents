...
import { getSessionBindingService, testing } from "steelengine/plugin-sdk/session-binding-runtime";
...
describe("matrix thread bindings", () => {
  ...
  async function readPersistedBindings(bindingsPath: string) {
    const store = createPluginStateKeyedStoreForTests<{
      ...
      maxEntries: 10_000,
      env: { ...process.env, STEELENGINE_STATE_DIR: path.dirname(bindingsPath) },
    });
    return {
      version: 1,
