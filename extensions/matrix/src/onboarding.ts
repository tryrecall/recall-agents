...
import type { DmPolicy } from "steelengine/plugin-sdk/config-contracts";
import {
...
import { isPrivateNetworkOptInEnabled } from "steelengine/plugin-sdk/ssrf-policy";
import {
...
async function configureMatrixInviteAutoJoin(params: {
  ...
  await params.prompter.note(
    [
      ...
      "SteelEngine agents will not join invited rooms or fresh DM-style invites unless you set autoJoin.",
  ...
  const deviceName =
    normalizeStringifiedOptionalString(
      await params.prompter.text({
        ...
        initialValue: existing.deviceName ?? "SteelEngine Gateway",
      }),
    ) ?? "";
