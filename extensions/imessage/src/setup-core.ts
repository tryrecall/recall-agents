...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: SteelEngineConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: SteelEngineConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: SteelEngineConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "For the usual setup, run SteelEngine on the Mac signed into Messages.",
    ...
    "After Private API setup, run `imsg launch`, then `steelengine channels status --probe`.",
    "Ensure SteelEngine has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: SteelEngineConfig; accountId?: string }) =>
