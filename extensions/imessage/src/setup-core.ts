...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: RecallConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: RecallConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: RecallConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "Run Recall on the Mac signed into Messages, or set cliPath to an SSH wrapper that runs imsg on that Mac.",
    "Linux/Windows hosts cannot run the default local imsg path directly.",
    "Run `imsg launch`, then `recall channels status --probe` to verify private API actions.",
    "Ensure Recall has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: RecallConfig; accountId?: string }) =>
