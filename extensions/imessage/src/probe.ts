...
import { detectBinary } from "steelengine/plugin-sdk/setup";
import {
...
function resolveIMessageNonMacHostError(
  ...
  return "iMessage via the default imsg CLI must run on macOS. Run SteelEngine on the signed-in Messages Mac, or set channels.imessage.cliPath to an SSH wrapper that runs imsg on that Mac.";
}
...
// subcommand (added by steelengine/imsg#114, which lets a single bridge call
...
export async function probeIMessagePrivateApi(
  ...
  try {
    ...
    // steelengine/imsg#114. We do this even when the bridge is unavailable
