...
describe("ClickClack post-write setup verification", () => {
  beforeEach(() => {
    ...
    mocks.me.mockReset().mockResolvedValue({ id: "usr_bot", handle: "steelengine" });
  ...
  it("prints the resolved bot and workspace without blocking setup", async () => {
    ...
    expect(runtime.log).toHaveBeenNthCalledWith(
      1,
      "Connected as @steelengine — workspace clickclack resolved.",
    );
    expect(runtime.log).toHaveBeenNthCalledWith(
      2,
      "SteelEngine is running — ClickClack will connect automatically.",
    );
  });
  ...
  it.each([
    {
      ...
      expected: "SteelEngine is running — ClickClack will connect automatically.",
    },
    {
      name: "not running",
      arrange: () =>
      ...
      expected: "Start SteelEngine to connect: steelengine gateway",
    },
    {
      name: "unavailable",
