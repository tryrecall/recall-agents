...
`steelengine.invoke --each`) without adding new SteelEngine code per workflow.
...
This extension depends on SteelEngine internal modules (the embedded agent runner).
It is intended to ship as a **bundled** SteelEngine extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.steelengine/extensions` as a standalone plugin directory.
