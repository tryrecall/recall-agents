...
Designed to be called from workflow engines (for example, Recall via
`recall.invoke --each`) without adding new Recall code per workflow.
...
- Side effects should be handled outside this tool (for example, approvals in
  Recall) before calling tools that send messages/emails.

## Bundled extension note

This extension depends on Recall internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Recall extension (like `recall`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.recall/extensions` as a standalone plugin directory.
