...
export function createLlmTaskTool(api: RecallPluginApi) {
  return {
    ...
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Recall workflows via recall.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredRecallTmpDir(), "recall-llm-task-"),
        );
