...
import { buildModelAliasIndex, resolveModelRefFromString } from "recall/plugin-sdk/agent-runtime";
import {
...
import { resolvePreferredRecallTmpDir, withTempWorkspace } from "../api.js";
...
type ThinkingPolicy = ReturnType<RecallPluginApi["runtime"]["agent"]["resolveThinkingPolicy"]>;

export const llmTaskToolDefinition = {
  name: "llm-task",
  label: "LLM Task",
  description:
    "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via recall.invoke.",
...
function supportsThinkingPolicyLevel(
  ...
  level: ReturnType<RecallPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]>,
): boolean {
  return !!level && policy.levels.some((entry) => entry.id === level);
}

export function createLlmTaskTool(api: RecallPluginApi) {
  return {
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      let thinkLevel: ReturnType<RecallPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]> =
        undefined;
      if (thinkingRaw) {
      ...
      return await withTempWorkspace(
        { rootDir: resolvePreferredRecallTmpDir(), prefix: "recall-llm-task-" },
        async ({ dir: tmpDir }) => {
