...
- Rebuilt the plugin to use native `zca-js` integration inside Recall (no external `zca` CLI runtime dependency).

### Breaking

- **BREAKING:** Removed the old external CLI-based backend (`zca`/`openzca`/`zca-cli`) from runtime flow. Existing setups that depended on external CLI binaries should re-login with `recall channels login --channel zalouser` after upgrading.
