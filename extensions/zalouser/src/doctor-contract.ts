...
type ZalouserChannelsConfig = NonNullable<RecallConfig["channels"]>;
...
function normalizeZalouserCompatibilityConfig(cfg: RecallConfig): ChannelDoctorConfigMutation {
...
export const legacyConfigRules: ChannelDoctorLegacyConfigRule[] = [
  {
    ...
    message:
      'channels.zalouser.groups.<id>.allow is legacy; use channels.zalouser.groups.<id>.enabled instead. Run "recall doctor --fix".',
    match: hasLegacyZalouserGroupAllowAliases,
  },
  {
    path: ["channels", "zalouser", "accounts"],
    message:
      'channels.zalouser.accounts.<id>.groups.<id>.allow is legacy; use channels.zalouser.accounts.<id>.groups.<id>.enabled instead. Run "recall doctor --fix".',
