...
function resolveZalouserOutboundChunkMode(cfg: SteelEngineConfig, accountId?: string) {
  return getZalouserRuntime().channel.text.resolveChunkMode(cfg, "zalouser", accountId);
}

function resolveZalouserOutboundTextChunkLimit(cfg: SteelEngineConfig, accountId?: string) {
...
export const zalouserPairingTextAdapter = {
  ...
  notify: async ({ cfg, id, message }: { cfg: SteelEngineConfig; id: string; message: string }) => {
    const { sendMessageZalouser } = await loadZalouserChannelRuntime();
