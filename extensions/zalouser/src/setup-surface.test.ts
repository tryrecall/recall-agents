...
describe("zalouser setup wizard", () => {
  ...
  it("checks setup status without persisting refreshed session credentials", async () => {
    ...
    await expect(
      zalouserSetupWizard.status.resolveConfigured({ cfg: {} as SteelEngineConfig }),
    ).resolves.toBe(true);
  ...
  it("reports account-scoped config keys for named accounts", () => {
    expect(zalouserSetupWizard.dmPolicy?.resolveConfigKeys?.({} as SteelEngineConfig, "work")).toEqual(
      {
