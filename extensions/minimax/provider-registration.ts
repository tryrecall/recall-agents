...
import { buildOauthProviderAuthResult } from "steelengine/plugin-sdk/provider-auth";
...
import { buildProviderStreamFamilyHooks } from "steelengine/plugin-sdk/provider-stream-family";
import { fetchMinimaxUsage } from "steelengine/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: SteelEngineConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: SteelEngineConfig): string | undefined {
...
export function registerMinimaxProviders(api: SteelEnginePluginApi) {
