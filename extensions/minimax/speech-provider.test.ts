...
describe("buildMinimaxSpeechProvider", () => {
  ...
  describe("isConfigured", () => {
    ...
    beforeEach(async () => {
      tempStateDir = await mkdtemp(path.join(tmpdir(), "recall-minimax-tts-auth-"));
  ...
  describe("synthesize", () => {
    ...
    beforeEach(async () => {
      tempStateDir = await mkdtemp(path.join(tmpdir(), "recall-minimax-tts-synth-"));
      tempAgentDir = path.join(tempStateDir, "agents", "main", "agent");
      await mkdir(tempAgentDir, { recursive: true });
      process.env = {
        ...savedEnv,
        RECALL_AGENT_DIR: tempAgentDir,
        RECALL_STATE_DIR: tempStateDir,
      };
      clearMinimaxAuthEnv();
