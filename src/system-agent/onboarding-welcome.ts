...
export async function loadAuthoredSetupConfig(params: {
  ...
  authoredConfig?: import("../config/types.steelengine.js").SteelEngineConfig;
...
export async function buildOnboardingWelcome(params: {
  ...
  if (!defaultModel) {
    throw new Error(
      "SteelEngine onboarding requires working inference first. Run `steelengine onboard` to configure and verify a default model.",
    );
  }
  ...
  const welcome = [
    "## Hi, I'm SteelEngine — let's hatch your agent.",
    "",
    ...
    "",
    "Heads up: your agent gets real access to this machine — https://docs.steelengine.ai/security",
