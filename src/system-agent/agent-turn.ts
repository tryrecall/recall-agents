// SteelEngine agent turns run the real embedded agent loop with the ring-zero tool.
...
/**
 * SteelEngine is a real agent: same loop, session transcript, and tool pipeline
 * as regular agents — restricted to the single ring-zero `steelengine` tool.
...
const SYSTEM_AGENT_MCP_TOOL_NAME = "mcp__steelengine__steelengine";
...
export function createSystemAgentSession(
  ...
  return {
    sessionId: `steelengine-${randomUUID()}`,
    verifiedInference,
    proposalRef: {},
  };
}
...
async function ensureSystemAgentDirs(
  ...
  const base = path.join(resolveStateDir(), "steelengine");
...
function resolveSystemAgentCliBackend(
  ...
  // auth come from the configured default agent. SteelEngine also forces a
...
function resolveSystemAgentCliToolAvailability(
  ...
  throw new Error(`CLI backend ${backendId} cannot enforce SteelEngine's exact tool availability`);
}

/**
 * CLI harnesses run the steelengine tool in a stdio MCP subprocess, so the
...
async function mirrorSystemAgentToolStateFromEvents(params: {
  ...
  return onAgentEvent((evt) => {
    ...
    // CLI harnesses report MCP tools with transport prefixes (mcp__steelengine__steelengine).
    if (name !== "steelengine" && !name.endsWith("__steelengine")) {
      return;
    }
    const args =
...
/**
 * Run one SteelEngine turn through the embedded agent loop. Route, runner, and
...
async function runSystemAgentTurnWithDeps(
  ...
  const runId = `steelengine-turn-${randomUUID()}`;
  const shared = {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("steelengine.systemAgentTurnTestApi")] = {
