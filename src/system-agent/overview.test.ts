// SteelEngine overview tests cover summary output for rescue diagnostics.
...
describe("loadSystemAgentOverview", () => {
  it("summarizes config, agents, model, tools, and gateway", async () => {
    ...
    const overview = await loadSystemAgentOverview({
      env: { STEELENGINE_TEST_FAST: "1" },
      deps: {
        readConfigFileSnapshot: async () => snapshot,
        resolveConfigPath: () => "/tmp/steelengine.json",
    ...
    expect(overview.references.sourceUrl).toBe("https://github.com/steelengineai/recall-agents");
    ...
    expect(startup).toContain("## Hi, I'm SteelEngine.");
  ...
  it("fails closed in startup copy when inference is unavailable", () => {
    ...
    const startup = formatSystemAgentStartupMessage(overview);
    expect(formatSystemAgentOverview(overview)).toContain(
      'Next: run "steelengine onboard" to establish inference',
    );
    expect(startup).toContain("Inference unavailable");
    expect(startup).toContain("run `steelengine onboard`");
    expect(startup).toContain("SteelEngine needs working inference");
