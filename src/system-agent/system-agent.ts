// SteelEngine CLI runner selects JSON, one-shot, or interactive setup-helper mode.
...
/**
 * CLI entry point for SteelEngine.
 *
...
/** Options accepted by the SteelEngine command runner. */
...
/** Run SteelEngine in JSON, one-shot message, or interactive TUI mode. */
export async function runSystemAgent(
  ...
  if (boundOpts.message?.trim()) {
    ...
    const overview = await withProgress(
      {
        label: "Loading SteelEngine overview…",
        indeterminate: true,
        delayMs: 0,
        fallback: "none",
      },
  ...
  if (!inputIsTty || !outputIsTty) {
    // Without a TTY, SteelEngine cannot safely ask for confirmation; require --message instead.
