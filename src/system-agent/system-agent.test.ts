// SteelEngine tests cover main rescue and audit command behavior.
...
describe("runSystemAgent", () => {
  ...
  it("uses the assistant planner only to choose typed operations", async () => {
    ...
    expect(lines.join("\n")).toContain("[steelengine] planner: openai/gpt-5.5");
    expect(lines.join("\n")).toContain("[steelengine] interpreted: restart gateway");
    expect(lines.join("\n")).toContain("Plan: restart the Gateway. Say yes to apply.");
    expect(lines.indexOf("Default model: openai/gpt-5.5")).toBeLessThan(
      lines.findIndex((line) => line.includes("[steelengine] planner:")),
    );
  });
  ...
  it("starts interactive SteelEngine in the TUI shell", async () => {
    const { runtime, lines } = createSystemAgentTestRuntime();
