...
describe("commitment extraction", () => {
  ...
  async function createConfig(): Promise<RecallConfig> {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-commitments-"));
    tmpDirs.push(tmpDir);
    vi.stubEnv("RECALL_STATE_DIR", tmpDir);
    return {
      commitments: {
        enabled: true,
      },
  ...
  it("rejects disabled, low-confidence, and non-future candidates", () => {
    const cfg: RecallConfig = { commitments: { enabled: true } };
