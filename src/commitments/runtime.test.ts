...
function requireFirstEmbeddedAgentRequest(): {
  ...
  if (!call) {
    throw new Error("expected embedded SteelEngine agent extraction request");
  }
  const [request] = call;
...
describe("commitment extraction runtime", () => {
  ...
  async function createConfig(): Promise<SteelEngineConfig> {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-commitment-runtime-"));
    tmpDirs.push(tmpDir);
