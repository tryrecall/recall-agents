...
describe("trajectory metadata", () => {
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/steelengine.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.steelengine/steelengine.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          STEELENGINE_STATE_DIR: "/Users/tester/.steelengine",
        },
      ...
      expect(harness.invocation).toEqual([
        "node",
        "~/project/steelengine.js",
        "--api-key",
        "<redacted>",
        "--config=$STEELENGINE_STATE_DIR/steelengine.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/steelengine.js");
  ...
  it("redactPathForSupport returns empty string for null/undefined input", () => {
    const ctx: SupportRedactionContext = { env: {}, stateDir: "/tmp/.steelengine" };
