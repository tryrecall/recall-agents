...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "recall-trajectory-"));
let tempDirId = 0;
...
describe("exportTrajectoryBundle", () => {
  it("sanitizes session ids in default export directory names", () => {
    ...
    expect(outputDir).toBe(
      path.join(
        ...
        ".recall",
        "trajectory-exports",
        "recall-trajectory-___evil_-2026-04-22T08-00-00",
      ),
    );
  });
  ...
  it("uses the recorded runtime pointer before current environment overrides", async () => {
    ...
    fs.writeFileSync(
      ...
      `${JSON.stringify({
        traceSchema: "recall-trajectory-pointer",
        schemaVersion: 1,
        sessionId: "session-1",
    ...
    const previous = process.env.RECALL_TRAJECTORY_DIR;
    process.env.RECALL_TRAJECTORY_DIR = envRuntimeDir;
    try {
      const bundle = await exportTrajectoryBundle({
        outputDir,
        sessionFile,
      ...
      if (previous === undefined) {
        delete process.env.RECALL_TRAJECTORY_DIR;
      } else {
        process.env.RECALL_TRAJECTORY_DIR = previous;
      }
    }
  });
  ...
  it("exports merged runtime and transcript events plus convenience files", async () => {
    ...
    const runtimeEvents: TrajectoryEvent[] = [
      ...
      },
      {
        ...
        sessionId: "session-1",
        data: {
          harness: { type: "recall", version: "0.1.0" },
