...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "recall-version-" });
...
describe("version resolution", () => {
  it("keeps bundled version injection as a direct define identifier", async () => {
    ...
    expect(source).toContain("typeof __RECALL_VERSION__");
    expect(source).toContain("? __RECALL_VERSION__");
  });

  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "recall", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "recall", version: "2.3.4" });
  ...
  it("ignores non-recall package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers RECALL_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        RECALL_VERSION: "9.9.9",
        RECALL_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over stale RECALL_VERSION for compatibility checks", () => {
    const previous = process.env.RECALL_VERSION;
    const previousService = process.env.RECALL_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.RECALL_VERSION = "2026.3.25";
      process.env.RECALL_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("RECALL_VERSION", previous);
      restoreEnvValue("RECALL_SERVICE_VERSION", previousService);
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        ...
        RECALL_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        RECALL_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        RECALL_VERSION: "2026.3.99",
        RECALL_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
