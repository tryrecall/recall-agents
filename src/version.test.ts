...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "steelengine-version-" });
...
describe("version resolution", () => {
  it("keeps bundled version injection as a direct define identifier", async () => {
    ...
    expect(source).toContain("typeof __STEELENGINE_VERSION__");
    expect(source).toContain("? __STEELENGINE_VERSION__");
  });

  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "steelengine", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "steelengine", version: "2.3.4" });
  ...
  it("ignores non-steelengine package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers STEELENGINE_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        STEELENGINE_VERSION: "9.9.9",
        STEELENGINE_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over stale STEELENGINE_VERSION for compatibility checks", () => {
    const previousCompatibility = process.env.STEELENGINE_COMPATIBILITY_HOST_VERSION;
    const previous = process.env.STEELENGINE_VERSION;
    const previousService = process.env.STEELENGINE_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      delete process.env.STEELENGINE_COMPATIBILITY_HOST_VERSION;
      process.env.STEELENGINE_VERSION = "2026.3.25";
      process.env.STEELENGINE_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("STEELENGINE_COMPATIBILITY_HOST_VERSION", previousCompatibility);
      restoreEnvValue("STEELENGINE_VERSION", previous);
      restoreEnvValue("STEELENGINE_SERVICE_VERSION", previousService);
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        STEELENGINE_VERSION: "2026.3.99",
        STEELENGINE_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        STEELENGINE_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        STEELENGINE_VERSION: "2026.3.99",
        STEELENGINE_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
  ...
  it("prefers runtime VERSION over service/package markers and ignores unusable env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        STEELENGINE_VERSION: "   ",
        STEELENGINE_SERVICE_VERSION: "  2.0.0  ",
    ...
    expect(
      resolveRuntimeServiceVersion({
        ...
        STEELENGINE_SERVICE_VERSION: "null",
        npm_package_version: "1.0.0-package",
      }),
    ).toBe(VERSION);
  });
