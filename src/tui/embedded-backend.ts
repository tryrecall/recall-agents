...
function hasProviderWildcardModelAllowlist(cfg: SteelEngineConfig) {
  const modelMaps = [
...
function resolveConfiguredReplaceModeCatalog(cfg: SteelEngineConfig) {
...
function shouldLoadFullGatewayCatalogForReplaceMode(cfg: SteelEngineConfig) {
...
async function loadEmbeddedTuiModelCatalog(cfg: SteelEngineConfig) {
      ...
      const result = await agentCommandFromIngress(
        {
          ...
          // bytes on the wire. See: https://github.com/steelengineai/recall-agents/issues/3658
          message,
          sessionKey: canonicalKey,
          ...(params.agentId ? { agentId: params.agentId } : {}),
