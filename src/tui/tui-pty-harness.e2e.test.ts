...
async function writeTuiPtyFixtureScript(dir: string) {
  ...
  await writeFile(
    ...
    `
      ...
      const actionLogPath = process.env.STEELENGINE_TUI_PTY_LOG_PATH;
      const gatewayStatus = process.env.STEELENGINE_TUI_PTY_GATEWAY_STATUS ?? "fixture gateway ok";
      const startupDelayMs = Number(process.env.STEELENGINE_TUI_PTY_STARTUP_DELAY_MS ?? 0);
      const footerModel = process.env.STEELENGINE_TUI_PTY_MODEL;
      const footerThinkingLevel = process.env.STEELENGINE_TUI_PTY_THINKING_LEVEL;
      ...
      let fastMode = process.env.STEELENGINE_TUI_PTY_FAST_MODE === "true";
      ...
      async function main() {
        await runTui({
          ...
          deliver: false,
          historyLimit: 5,
          title: "steelengine tui pty fixture",
        });
      }
...
async function startTuiFixture(opts: { env?: NodeJS.ProcessEnv } = {}) {
  const tempDir = await mkdtemp(path.join(tmpdir(), "steelengine-tui-pty-"));
  ...
  const run = startPty(process.execPath, ["--import", "tsx", scriptPath], {
    ...
    env: {
      ...
      STEELENGINE_TUI_PTY_LOG_PATH: logPath,
      NO_COLOR: undefined,
      ...opts.env,
    },
...
describe.sequential("TUI PTY harness", () => {
  ...
  beforeAll(async () => {
    ...
    const boots = await Promise.allSettled([
      ...
      startTuiFixture({
        env: {
          STEELENGINE_TUI_PTY_MODEL: "gpt-5.6-sol@openai:setup-64cddea3-938c-431e-be3b-aa47090577c7",
          STEELENGINE_TUI_PTY_THINKING_LEVEL: "high",
        },
      }),
      startTuiFixture({
        env: { STEELENGINE_TUI_PTY_STARTUP_DELAY_MS: "400" },
      }),
    ]);
