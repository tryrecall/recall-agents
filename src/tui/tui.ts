...
const STEELENGINE_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../steelengine.mjs", import.meta.url));
const STEELENGINE_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const STEELENGINE_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const STEELENGINE_DIST_ENTRY_MJS_PATH = fileURLToPath(
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(STEELENGINE_DIST_ENTRY_JS_PATH) || existsSync(STEELENGINE_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(STEELENGINE_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? STEELENGINE_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? STEELENGINE_RUN_NODE_SCRIPT_PATH;
...
export function resolveLocalAuthSpawnCwd(params: { args: string[]; defaultCwd?: string }): string {
  const defaultCwd =
    params.defaultCwd ?? tryProcessCwd() ?? path.dirname(STEELENGINE_CLI_WRAPPER_PATH);
  const entryArg = params.args[0]?.trim();
  if (!entryArg) {
    return defaultCwd;
  }
  const entryBase = path.basename(entryArg).toLowerCase();
  if (entryBase === "steelengine.mjs") {
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required|scope upgrade/i.test(reasonLabel)) {
    return {
      ...
      pairingHint:
        "Device approval needed. Run `steelengine devices approve --latest` to preview the pending request, " +
        "then rerun the printed `steelengine devices approve <requestId>` command " +
...
export function scheduleProcessExitAfterTuiReturn(
  ...
  const timer = setTimeoutFn(() => {
    try {
      writeStderr("steelengine tui forcing process exit after return\n");
    } catch {
...
function resolveEmptySessionInfoDefaults(config: SteelEngineConfig): SessionInfo {
  return {
...
export async function runTui(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const fallbackCwd = path.dirname(STEELENGINE_CLI_WRAPPER_PATH);
  ...
  const previousConsoleSubsystemFilter = isLocalMode
    ...
    : null;
  if (isLocalMode) {
    setConsoleSubsystemFilter(["__steelengine_tui_quiet__"]);
  }
  ...
  const updateHeader = () => {
    ...
    const title = opts.title ?? "steelengine tui";
    header.setText(
      theme.header(
  ...
  const forceExit = () => {
    try {
      process.stderr.write("steelengine tui forcing exit\n");
    } catch {
    ...
    void Promise.resolve()
      ...
      .catch((err: unknown) => {
        if (!isTuiTerminalLossError(err)) {
          try {
            process.stderr.write(`steelengine tui shutdown failed: ${String(err)}\n`);
          } catch {
