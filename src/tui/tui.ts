...
import { getRuntimeConfig, type RecallConfig } from "../config/config.js";
...
const RECALL_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../recall.mjs", import.meta.url));
const RECALL_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const RECALL_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const RECALL_DIST_ENTRY_MJS_PATH = fileURLToPath(
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(RECALL_DIST_ENTRY_JS_PATH) || existsSync(RECALL_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(RECALL_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? RECALL_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? RECALL_RUN_NODE_SCRIPT_PATH;
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run recall devices list",
      pairingHint:
        "Pairing required. Run `recall devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export function scheduleProcessExitAfterTuiReturn(
  ...
  const timer = setTimeoutFn(() => {
    try {
      writeStderr("recall tui forcing process exit after return\n");
    } catch {
...
export async function runTui(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const updateHeader = () => {
    ...
    const title = opts.title ?? "recall tui";
    header.setText(
      theme.header(
  ...
  const forceExit = () => {
    try {
      process.stderr.write("recall tui forcing exit\n");
    } catch {
    ...
    void Promise.resolve()
      ...
      .catch((err) => {
        if (!isTuiTerminalLossError(err)) {
          try {
            process.stderr.write(`recall tui shutdown failed: ${String(err)}\n`);
          } catch {
