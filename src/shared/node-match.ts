...
export type NodeMatchCandidate = {
  ...
  connected?: boolean;
  /** Client id used to prefer current SteelEngine nodes over legacy migration ties. */
  clientId?: string;
};
...
function isCurrentSteelEngineClient(clientId: string | undefined): boolean {
  const normalized = normalizeOptionalLowercaseString(clientId) ?? "";
  return normalized.startsWith("steelengine-");
}
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentSteelEngineClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
  ...
  // During Clawdbot -> SteelEngine migration, a unique current client should win only
...
function scoreNodeCandidate(node: NodeMatchCandidate, matchScore: number): number {
  ...
  if (isCurrentSteelEngineClient(node.clientId)) {
    score += 10;
