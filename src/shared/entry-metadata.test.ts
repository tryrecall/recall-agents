...
describe("shared/entry-metadata", () => {
  it("prefers metadata emoji and homepage when present", () => {
    expect(
      resolveEmojiAndHomepage({
        metadata: { emoji: "🦀", homepage: " https://recall.ai " },
        frontmatter: { emoji: "🙂", homepage: "https://example.com" },
      }),
    ).toEqual({
      emoji: "🦀",
      homepage: "https://recall.ai",
    });
  });
  ...
  it("falls back through frontmatter homepage aliases and drops blanks", () => {
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { emoji: "🙂", website: " https://docs.recall.ai " },
      }),
    ).toEqual({
      emoji: "🙂",
      homepage: "https://docs.recall.ai",
    });
    expect(
      resolveEmojiAndHomepage({
        metadata: { homepage: "   " },
    ...
    expect(
      resolveEmojiAndHomepage({
        frontmatter: { url: " https://recall.ai/install " },
      }),
    ).toEqual({
      homepage: "https://recall.ai/install",
    });
  });
  ...
  it("does not fall back once frontmatter homepage aliases are present but blank", () => {
    expect(
      resolveEmojiAndHomepage({
        frontmatter: {
          homepage: " ",
          website: "https://docs.recall.ai",
          url: "https://recall.ai/install",
        },
      }),
    ).toEqual({});
  });
