...
const tempRoot = createSuiteTempRootTracker({ prefix: "steelengine-fleet-doctor-" });
...
function healthyInspection(): Extract<FleetContainerInspectResult, { kind: "ok" }> {
  return {
    ...
    environment: { STEELENGINE_GATEWAY_TOKEN: "secret" },
...
function healthyNetwork(): Extract<FleetNetworkInspectResult, { kind: "ok" }> {
  return {
    ...
    attachedContainers: [{ id: "cell", name: "steelengine-cell-acme" }],
    internal: false,
  };
}
...
describe("fleet doctor", () => {
  ...
  it.each([
    ...
    [
      ...
      "fail",
      (inspection: ReturnType<typeof healthyInspection>) => {
        inspection.labels["steelengine.fleet.owner"] = "foreign";
      },
    ],
    [
      "gateway-token-env",
    ...
    [
      ...
      (inspection: ReturnType<typeof healthyInspection>) => {
        ...
        inspection.labels["steelengine.fleet.disk-limit"] = "10g";
    ...
    [
      ...
      (inspection: ReturnType<typeof healthyInspection>) => {
        ...
        inspection.labels["steelengine.fleet.disk-limit"] = "not-a-size";
