...
describe("fleet service", () => {
  ...
  it("creates a bootable token-only cell config and returns the secret-bearing result", async () => {
    ...
    expect(containers.createNetwork).toHaveBeenCalledWith(
      ...
      "steelengine-cell-acme-net",
      {
        ...
        "steelengine.fleet.owner": cellOwnerId(path.join(root, "fleet", "cells", "acme")),
        "steelengine.fleet.attempt": expect.stringMatching(/^[a-f0-9]{32}$/u),
      },
      { internal: false },
    );
    expect(containers.start).toHaveBeenCalledWith("docker", "steelengine-cell-acme");
    expect(profile?.networkName).toBe("steelengine-cell-acme-net");
    ...
    expect(profile?.environment).toMatchObject({
      STEELENGINE_GATEWAY_TOKEN: "gw-token",
      FEATURE: "a=b",
    });

    const dataDir = path.join(root, "fleet", "cells", "acme");
    const config = JSON.parse(await fs.readFile(path.join(dataDir, "steelengine.json"), "utf8")) as {
      gateway?: {
        mode?: string;
        bind?: string;
  ...
  it("keeps an unhealthy created cell and its runtime evidence", async () => {
    ...
    await expect(service.create({ tenant: "sick", gatewayToken: "token" })).rejects.toThrow(
      "Fleet cell sick was created but did not become healthy within 60s; inspect it with `steelengine fleet status sick` or `steelengine fleet logs sick`, or remove it with `steelengine fleet rm sick --force`.",
    );
  ...
  it.each(["start", "stop", "restart"] as const)("runs the %s lifecycle action", async (action) => {
    ...
    expect(containers[action]).toHaveBeenCalledWith("docker", "steelengine-cell-acme");
  });
  ...
  it("carries inspected environment and resources through upgrade", async () => {
    ...
    const diskLabels = { ...fleetLabels(), "steelengine.fleet.disk-limit": "10g" };
    containers.inspect
    ...
    const result = await service.upgrade("acme", "ghcr.io/steelengine/steelengine:v2");

    expect(result).toEqual({
      tenant: "acme",
      action: "upgrade",
      image: "ghcr.io/steelengine/steelengine:v2",
    });
    expect(containers.pull).toHaveBeenCalledWith("docker", "ghcr.io/steelengine/steelengine:v2");
    expect(containers.stop).toHaveBeenCalledWith("docker", "steelengine-cell-acme");
    expect(containers.remove).toHaveBeenCalledWith("docker", "steelengine-cell-acme", false);
    expect(containers.inspectNetwork).toHaveBeenCalledWith("docker", "steelengine-cell-acme-net");
    const [profile, start] = containers.run.mock.calls[0] ?? [];
    ...
    expect(profile).toMatchObject({
      ...
      pidsLimit: 512,
      diskSize: "10g",
      networkName: "steelengine-cell-acme-net",
      environment: {
        HOME: "/home/node",
        STEELENGINE_GATEWAY_TOKEN: "old-token",
        FEATURE: "enabled",
    ...
    expect(getFleetCell(env, "acme")?.image).toBe("ghcr.io/steelengine/steelengine:v2");
  });

  it("passes digest-pinned images verbatim to create and upgrade", async () => {
    const containers = createContainerMock();
    const digest = `ghcr.io/steelengine/steelengine@sha256:${"a".repeat(64)}`;
    const service = createFleetService({
      env,
      containers: containers.runtime,
      generateAttemptId: () => NEXT_ATTEMPT_ID,
  ...
  it("restores the immutable old image when replacement fails", async () => {
    ...
    expect(getFleetCell(env, "acme")?.image).toBe("ghcr.io/steelengine/steelengine:latest");
  });
  ...
  it("refuses upgrade when an unexpected container is attached to the cell network", async () => {
    ...
    containers.inspectNetwork.mockResolvedValue({
      ...
      labels: fleetLabels(),
      attachedContainers: [
        { id: "cell-id", name: "steelengine-cell-acme" },
