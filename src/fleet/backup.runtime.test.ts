...
const tempRoot = createSuiteTempRootTracker({ prefix: "steelengine-fleet-backup-test-" });
...
function inspection(running = false): Extract<FleetContainerInspectResult, { kind: "ok" }> {
  return {
    ...
    labels: {
      ...
      "steelengine.fleet.owner": cellOwnerId(record.dataDir),
      "steelengine.fleet.attempt": ATTEMPT,
      "steelengine.fleet.env-keys": "",
      "steelengine.fleet.disk-limit": "10g",
    },
    environment: { STEELENGINE_GATEWAY_TOKEN: "old-token" },
...
async function createArchive(
  ...
  await fs.writeFile(
    ...
    JSON.stringify({
      schemaVersion: 1,
      kind: "steelengine-fleet-cell-backup",
...
describe("fleet restore runtime", () => {
  ...
  it("rejects archives inside cell state and foreign cell networks before mutation", async () => {
    ...
    containers.inspectNetwork.mockResolvedValue({
      kind: "ok",
      labels: { "steelengine.fleet.tenant": "acme", "steelengine.fleet.owner": "foreign" },
      attachedContainers: [],
      internal: false,
    });
    await expect(restoreFleetCell(restoreParams(containers, archive))).rejects.toThrow(
  ...
  it.each([
    ...
    await fs.writeFile(
      path.join(source, "manifest.json"),
      JSON.stringify({ schemaVersion: 1, kind: "steelengine-fleet-cell-backup", tenant: "acme" }),
    );
    await mutate(source);
    const archive = path.join(root, `${path.basename(source)}.tgz`);
  ...
  it("fails preflight on a drifted container before any destructive step", async () => {
    ...
    delete drifted.labels["steelengine.fleet.env-keys"];
  ...
  it("swaps state, repins config, and rotates the token", async () => {
    ...
    const config = JSON.parse(
      await fs.readFile(path.join(record.dataDir, "steelengine.json"), "utf8"),
    ) as { gateway?: { controlUi?: { allowedOrigins?: string[] } } };
    expect(config.gateway?.controlUi?.allowedOrigins).toContain("http://127.0.0.1:19100");
    expect(containers.run.mock.calls[0]?.[0].environment.STEELENGINE_GATEWAY_TOKEN).toBe("new-token");
    ...
    expect(containers.run.mock.calls[0]?.[0].environment.STEELENGINE_GATEWAY_TOKEN).not.toBe(
      "old-token",
    );
  ...
  it("stops an unhealthy started replacement so its undelivered token cannot serve", async () => {
    ...
    containers.run.mockImplementation(async () => {
      ...
      running.labels["steelengine.fleet.attempt"] = NEXT_ATTEMPT;
    });
    let clock = 0;
    let message = "";
    try {
