...
describe("renderQrPngBase64", () => {
  const tmpRoot = path.join(os.tmpdir(), "steelengine-qr-image-tests");
  ...
  it("delegates PNG rendering to qrcode", async () => {
    await expect(renderQrPngBase64("steelengine", { scale: 8, marginModules: 2 })).resolves.toBe(
      MOCK_PNG_BASE64,
    );
    expect(toBuffer).toHaveBeenCalledWith("steelengine", {
      margin: 2,
      scale: 8,
    });
  });
  ...
  it("uses the default PNG rendering options", async () => {
    await renderQrPngBase64("steelengine");
    expect(toBuffer).toHaveBeenCalledWith("steelengine", {
      margin: 4,
      scale: 6,
    });
  ...
  it("floors finite PNG rendering options before delegating", async () => {
    await renderQrPngBase64("steelengine", { scale: 8.9, marginModules: 2.9 });
    expect(toBuffer).toHaveBeenCalledWith("steelengine", {
      margin: 2,
      scale: 8,
    });
  ...
  it.each([
    ...
    await expect(renderQrPngBase64("steelengine", { scale, marginModules })).rejects.toThrow(message);
    expect(toBuffer).not.toHaveBeenCalled();
  });

  it("formats QR PNG data URLs", async () => {
    await expect(renderQrPngDataUrl("steelengine")).resolves.toBe(
  ...
  it("writes QR PNGs to a scoped temp file", async () => {
    ...
    const result = await writeQrPngTempFile("steelengine", {
      tmpRoot,
      dirPrefix: "pair-",
  ...
  it.each([
    ...
    await expect(
      writeQrPngTempFile("steelengine", {
        tmpRoot,
