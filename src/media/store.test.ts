...
describe("media store", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("steelengine-test-home-");
    home = tempHome.home;
  ...
  it.each([
    {
      ...
      run: async () => {
        await withTempStore(async (storeLocal17, homeLocal4) => {
          ...
          expect(path.normalize(dir)).toContain(`${path.sep}.steelengine${path.sep}media`);
    ...
    },
    {
      ...
      run: async () => {
        await withTempStore(async (storeLocal9, homeInner) => {
          ...
          const targetDir = path.join(homeInner, ".steelengine", "media", "oversized-stream");
  ...
  it("prefers header mime extension when sniffed mime lacks mapping", async () => {
    await withTempStore(async (_store, homeLocal) => {
      vi.doMock("@steelengine/media-core/mime", async () => {
        const actual = await vi.importActual<typeof import("@steelengine/media-core/mime")>(
          "@steelengine/media-core/mime",
        );
        return {
          ...actual,
      ...
      try {
        ...
        vi.doUnmock("@steelengine/media-core/mime");
      }
    });
  });
