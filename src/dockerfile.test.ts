...
import { BUNDLED_PLUGIN_ROOT_DIR } from "steelengine/plugin-sdk/test-fixtures";
...
describe("Dockerfile", () => {
  ...
  it("uses full bookworm for build stages and slim bookworm for runtime", async () => {
    ...
    expect(dockerfile).toContain("FROM ${STEELENGINE_NODE_BOOKWORM_IMAGE} AS workspace-deps");
    expect(dockerfile).toContain("FROM ${STEELENGINE_NODE_BOOKWORM_IMAGE} AS build");
    expect(dockerfile).toContain("FROM ${STEELENGINE_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime");
    ...
    expect(dockerfile).not.toContain("STEELENGINE_VARIANT");
  });
  ...
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG STEELENGINE_INSTALL_BROWSER");
  ...
  it("uses portable copies for workspace dependency inputs", async () => {
    ...
    const workspaceDepsEnd = dockerfile.indexOf("FROM ${STEELENGINE_BUN_IMAGE} AS bun-binary");
  ...
  it("copies install workspace manifests before pnpm install", async () => {
    ...
    expect(dockerfile).toContain("done < /out/steelengine-selected-plugin-dirs");
  ...
  it("keeps validated plugin selection outside the build-context copy destination", async () => {
    ...
    expect(dockerfile).not.toContain("/app/.steelengine-selected-plugin-dirs");
    expect(dockerfile).not.toContain("./.steelengine-selected-plugin-dirs");
    expect(dockerfile).toContain("grep -qx 'matrix' /tmp/steelengine-selected-plugin-dirs");
    expect(dockerfile).toContain(
      'selected_plugin_dirs="$(cat /tmp/steelengine-selected-plugin-dirs)"',
    );
    expect(dockerfile).toContain('STEELENGINE_EXTENSIONS="$(cat /tmp/steelengine-selected-plugin-dirs)"');
  });
  ...
  it("does not let pnpm resync the full source workspace during Docker build scripts", async () => {
    ...
    expect(collapsed).not.toContain(
      'STEELENGINE_DOCKER_BUILD_EXTENSIONS="$STEELENGINE_EXTENSIONS" STEELENGINE_RUN_NODE_SKIP_DTS_BUILD=',
    );
  ...
  it("shares public source provenance across backend and Control UI builds", async () => {
    ...
    const timestampArgIndex = dockerfile.indexOf('ARG STEELENGINE_BUILD_TIMESTAMP=""');
    ...
    expect(dockerfile).toContain("STEELENGINE_BUILD_TIMESTAMP=${STEELENGINE_BUILD_TIMESTAMP}");
    expect(dockerfile).toContain('STEELENGINE_BUILD_TIMESTAMP="$(date -u +%Y-%m-%dT%H:%M:%SZ)"');
  ...
  it("documents provenance arguments for manual source builds", async () => {
    ...
    expect(docs).toContain('--build-arg "STEELENGINE_BUILD_TIMESTAMP=${BUILD_TIMESTAMP}"');
  ...
  it("prunes runtime dependencies and omitted plugin packages after the build stage", async () => {
    ...
    expect(dockerfile).toContain("ARG STEELENGINE_EXTENSIONS");
    expect(dockerfile).toContain(
    ...
    expect(dockerfile).toContain("ARG STEELENGINE_BUNDLED_PLUGIN_DIR");
    expect(dockerfile).toContain(
    ...
    expect(dockerfile).toContain("readlink -f /app/node_modules/@steelengine/ai");
    expect(dockerfile).toContain('mv "$ai_runtime_tmp/ai" /app/node_modules/@steelengine/ai');
  ...
  it("keeps the Codex plugin in official Docker release images", async () => {
    ...
    const releaseKeepList = "STEELENGINE_EXTENSIONS=diagnostics-otel,codex";

    expect(workflow.match(new RegExp(releaseKeepList, "g"))).toHaveLength(4);
    expect(workflow).not.toContain("STEELENGINE_EXTENSIONS=diagnostics-otel\n");
  });
  ...
  it("uses one source commit and timestamp for every official Docker artifact", async () => {
    ...
    expect(
      workflow.split(
        "STEELENGINE_BUILD_TIMESTAMP=${{ needs.resolve_build_provenance.outputs.built_at }}",
      ).length - 1,
    ).toBe(4);
  });
  ...
  it("publishes official Docker browser images with baked Chromium", async () => {
    ...
    expect(workflow).toContain("STEELENGINE_INSTALL_BROWSER=1");
    ...
    expect(workflow).toContain("grep -q '^ARG STEELENGINE_INSTALL_BROWSER' Dockerfile");
  ...
  it("publishes official Docker releases to GHCR and Docker Hub", async () => {
    ...
    expect(workflow).toContain("DOCKERHUB_IMAGE_NAME: steelengine/steelengine");
  ...
  it("smokes runtime workspace templates before Docker release manifests publish", async () => {
    ...
    expect(workflow).toContain('test -f "${temp_root}/home/.steelengine/workspace/HEARTBEAT.md"');
  });
  ...
  it("keeps only the runtime-assets prune proof in full release validation", async () => {
    ...
    expect(workflow).not.toContain('test -f "${temp_root}/home/.steelengine/workspace/HEARTBEAT.md"');
  ...
  it("does not override bundled plugin discovery in runtime images", async () => {
    const dockerfile = collapseDockerContinuations(await readFile(dockerfilePath, "utf8"));
    expect(dockerfile).toContain(`ARG STEELENGINE_BUNDLED_PLUGIN_DIR=${BUNDLED_PLUGIN_ROOT_DIR}`);
    expect(dockerfile).not.toMatch(/^\s*ENV\b[^\n]*\bSTEELENGINE_BUNDLED_PLUGINS_DIR\b/m);
  });
  ...
  it("Docker GPG fingerprint awk uses correct quoting for STEELENGINE_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
  ...
  it("pre-creates named-volume mount points before switching to the node user", async () => {
    ...
    // before the leaf .config/steelengine dir (issue #85968).
    ...
    expect(dockerfile).not.toContain("mkdir -p /home/node/.steelengine");
    expect(dockerfile).toContain("/home/node/.steelengine/workspace");
    expect(dockerfile).toContain("/home/node/.config/steelengine");
    expect(dockerfile).toContain(
