...
let listedConfigs: Array<RecallConfig | undefined> = [];
...
describe("video-generation runtime", () => {
    ...
    providers = [
      {
        ...
        async generateVideo(req) {
          seenSupportedDurationHint = (req as Record<symbol, readonly number[] | undefined>)[
            Symbol.for("recall.videoGeneration.supportedDurations")
          ];
          seenRequest = {
  ...
  it("lists runtime video-generation providers through the provider registry", () => {
    ...
    expect(
      listRuntimeVideoGenerationProviders({ config: {} as RecallConfig }, runtimeDeps),
    ).toEqual(registryProviders);
    expect(listedConfigs).toEqual([{} as RecallConfig]);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    await expect(
      runGenerateVideo({ cfg: {} as RecallConfig, prompt: "animate a cat" }),
    ).rejects.toThrow(
