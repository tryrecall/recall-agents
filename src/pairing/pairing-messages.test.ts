...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["STEELENGINE_CONTAINER_HINT", "STEELENGINE_PROFILE"]);
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:steelengine|steelengine) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
