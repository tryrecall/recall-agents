// Generates setup codes used to pair external channels with SteelEngine.
...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.STEELENGINE_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.STEELENGINE_GATEWAY_PASSWORD);
  const token =
