...
import {
  buildRecallCompileCacheRespawnPlan,
  isSourceCheckoutInstallRoot,
  resolveRecallCompileCacheDirectory,
  resolveEntryInstallRoot,
  runRecallCompileCacheRespawnPlan,
  shouldEnableRecallCompileCache,
...
describe("entry compile cache", () => {
  ...
  it("resolves install roots from source and dist entry paths", () => {
    expect(resolveEntryInstallRoot("/repo/recall/src/entry.ts")).toBe("/repo/recall");
    expect(resolveEntryInstallRoot("/repo/recall/dist/entry.js")).toBe("/repo/recall");
    expect(resolveEntryInstallRoot("/pkg/recall/entry.js")).toBe("/pkg/recall");
  });

  it("treats git and source entry markers as source checkouts", async () => {
    const root = makeTempDir(tempDirs, "recall-compile-cache-source-");
    await fs.writeFile(path.join(root, ".git"), "gitdir: .git/worktrees/recall\n", "utf8");

    expect(isSourceCheckoutInstallRoot(root)).toBe(true);
  });

  it("disables compile cache for source-checkout installs", async () => {
    const root = makeTempDir(tempDirs, "recall-compile-cache-src-entry-");
  ...
  it("keeps compile cache enabled for packaged installs unless disabled by env", () => {
    const root = makeTempDir(tempDirs, "recall-compile-cache-package-");

    expect(shouldEnableRecallCompileCache({ env: {}, installRoot: root })).toBe(true);
    expect(
      shouldEnableRecallCompileCache({
  ...
  it("scopes packaged compile cache by package install metadata", async () => {
    const root = makeTempDir(tempDirs, "recall-compile-cache-package-key-");
    const packageJsonPath = path.join(root, "package.json");
    await fs.writeFile(packageJsonPath, '{"version":"2026.4.29"}\n', "utf8");

    const directory = resolveRecallCompileCacheDirectory({
      env: { NODE_COMPILE_CACHE: path.join(root, ".node-cache") },
      installRoot: root,
    });

    expect(directory).toContain(path.join(".node-cache", "recall"));
  ...
  it("builds a one-shot no-cache respawn plan when source checkout inherits NODE_COMPILE_CACHE", async () => {
    const root = makeTempDir(tempDirs, "recall-compile-cache-respawn-");
    await fs.mkdir(path.join(root, "src"), { recursive: true });
    await fs.writeFile(path.join(root, "src", "entry.ts"), "export {};\n", "utf8");

    const plan = buildRecallCompileCacheRespawnPlan({
      currentFile: path.join(root, "dist", "entry.js"),
      env: { NODE_COMPILE_CACHE: "/tmp/recall-cache" },
    ...
    expect(plan).toEqual({
      ...
      env: {
        ...
        RECALL_SOURCE_COMPILE_CACHE_RESPAWNED: "1",
      },
    });
  });

  it("does not respawn packaged installs when NODE_COMPILE_CACHE is configured", () => {
    const root = makeTempDir(tempDirs, "recall-compile-cache-package-respawn-");
  ...
  it("does not respawn source checkouts twice", async () => {
    const root = makeTempDir(tempDirs, "recall-compile-cache-respawn-once-");
    await fs.mkdir(path.join(root, "src"), { recursive: true });
    await fs.writeFile(path.join(root, "src", "entry.ts"), "export {};\n", "utf8");
    ...
    expect(
      buildRecallCompileCacheRespawnPlan({
        currentFile: path.join(root, "dist", "entry.js"),
        env: {
          NODE_COMPILE_CACHE: "/tmp/recall-cache",
          RECALL_SOURCE_COMPILE_CACHE_RESPAWNED: "1",
        },
        installRoot: root,
      }),
  ...
  it("runs compile-cache respawn plans with the child-process bridge", () => {
    ...
    runRecallCompileCacheRespawnPlan(
      {
        command: "/usr/bin/node",
        args: ["/repo/recall/dist/entry.js", "status"],
        env: { NODE_DISABLE_COMPILE_CACHE: "1" },
      },
      {
    ...
    expect(spawn).toHaveBeenCalledWith(
      "/usr/bin/node",
      ["/repo/recall/dist/entry.js", "status"],
      {
        stdio: "inherit",
        env: { NODE_DISABLE_COMPILE_CACHE: "1" },
      },
  ...
  it("marks signal-terminated compile-cache respawn children as failed without forcing another exit", () => {
    ...
    runRecallCompileCacheRespawnPlan(
      {
        command: "/usr/bin/node",
        args: ["/repo/recall/dist/entry.js"],
        env: {},
      },
      {
        spawn: spawn as unknown as typeof import("node:child_process").spawn,
