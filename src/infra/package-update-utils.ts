...
import { readRootJsonObjectSync } from "@recall/fs-safe/json";
...
export function installedPackageNeedsRecallPeerLinkRepair(dir: string): boolean {
  ...
  try {
    fsSync.statSync(path.join(dir, "node_modules", "recall"));
    return false;
  } catch (error) {
