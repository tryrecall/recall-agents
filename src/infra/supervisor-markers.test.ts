...
describe("SUPERVISOR_HINT_ENV_VARS", () => {
  it("includes the cross-platform supervisor hint env vars", () => {
    ...
    expect(envVars.has("RECALL_WINDOWS_TASK_NAME")).toBe(true);
    expect(envVars.has("RECALL_SERVICE_MARKER")).toBe(true);
    expect(envVars.has("RECALL_SERVICE_KIND")).toBe(true);
  });
});

describe("detectRespawnSupervisor", () => {
  it("detects launchd and systemd only from non-blank platform-specific hints", () => {
    expect(detectRespawnSupervisor({ LAUNCH_JOB_LABEL: " ai.recall.gateway " }, "darwin")).toBe(
      "launchd",
    );
  ...
  it("ignores service markers on non-Windows platforms and unknown platforms", () => {
    ...
    expect(
      detectRespawnSupervisor({ LAUNCH_JOB_LABEL: "ai.recall.gateway" }, "freebsd"),
    ).toBeNull();
  });
});
