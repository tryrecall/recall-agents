...
const canCreateDirectorySymlinks = (() => {
  ...
  try {
    probeDir = fsSync.mkdtempSync(
      path.join(os.tmpdir(), "steelengine-install-safe-dir-symlink-probe-"),
    );
...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@steelengine/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@steelengine/matrix", id: "matrix", expected: true },
    { packageName: "@steelengine/matrix", id: "@steelengine/matrix", expected: true },
    { packageName: "@steelengine/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@steelengine/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      resolveSafeInstallDir({
        baseDir: "/tmp/plugins",
        id: "@steelengine/matrix",
        invalidNameMessage: "invalid plugin name",
      }),
    ).toEqual({
      ok: true,
      path: path.join("/tmp/plugins", "@steelengine__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "steelengine-install-safe-" }, async (baseDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTempDir({ prefix: "steelengine-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "steelengine-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTempDir({ prefix: "steelengine-install-safe-" }, async (parentDir) => {
