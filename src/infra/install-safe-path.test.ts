...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@recall/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@recall/matrix", id: "matrix", expected: true },
    { packageName: "@recall/matrix", id: "@recall/matrix", expected: true },
    { packageName: "@recall/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@recall/matrix", id: " ", expected: false },
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "recall-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "recall-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "recall-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "recall-install-safe-" }, async (parentDir) => {
