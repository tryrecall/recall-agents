...
import { resolveRecallPackageRoot, resolveRecallPackageRootSync } from "./recall-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveRecallPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "recall" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/recall -> .../dist/index.js)
