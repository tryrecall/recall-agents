...
// update.
//
// `runGatewayUpdate` (git mode) runs `steelengine doctor --fix` with
// `STEELENGINE_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE=1`, which makes the doctor
// pass DEFER configured-plugin repair to a later convergence step (see
// `shouldDeferConfiguredPluginInstallRepair`). The `steelengine update` CLI resumes
...
// binary's hidden `steelengine update finalize` entrypoint — the designed
...
// re-enters the full `steelengine update` CLI. Re-run convergence on no-op retries:
...
export async function runPostCoreFinalizeAfterGatewayUpdate(params: {
  ...
  try {
    ...
    if (params.preUpdateConfig) {
      sourceConfigDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-update-post-core-"));
  ...
  return {
    ...
    steps: [
      ...result.steps,
      {
        ...
        command: "steelengine update finalize",
