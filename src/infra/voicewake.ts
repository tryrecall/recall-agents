...
const DEFAULT_TRIGGERS = ["steelengine", "claude", "computer"];
const VOICEWAKE_CONFIG_KEY = "default";

type VoiceWakeDatabase = Pick<SteelEngineStateKyselyDatabase, "voicewake_triggers">;
...
export async function setVoiceWakeTriggers(
  ...
  runSteelEngineStateWriteTransaction(
    ...
    baseDir ? { env: { ...process.env, STEELENGINE_STATE_DIR: baseDir } } : {},
  );
  return {
    triggers: sanitized,
    updatedAtMs,
