...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "RECALL_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "recall", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("recall channels login --channel whatsapp");
  });
