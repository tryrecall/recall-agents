...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "STEELENGINE_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "steelengine", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("steelengine channels login --channel whatsapp");
  });
