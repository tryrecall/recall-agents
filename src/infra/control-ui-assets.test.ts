...
vi.mock("./steelengine-root.js", () => ({
  resolveSteelEnginePackageRoot: vi.fn(async () => null),
  resolveSteelEnginePackageRootSync: vi.fn(() => null),
}));
...
let steelengineRoot: typeof import("./steelengine-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    steelengineRoot = await import("./steelengine-root.js");
  });
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", () => {
    const pkgRoot = abs("fixtures/steelengine-bundle");
    (
      steelengineRoot.resolveSteelEnginePackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/SteelEngine.app/Contents/MacOS/SteelEngine");
    const bundledUiDir = abs("fixtures/SteelEngine.app/Contents/Resources/control-ui");
  ...
  it("resolves control-ui root for symlinked argv1 via realpath", () => {
    const pkgRoot = abs("fixtures/bun-global/steelengine");
    const wrapperArgv1 = abs("fixtures/bin/steelengine");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/steelengine-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
