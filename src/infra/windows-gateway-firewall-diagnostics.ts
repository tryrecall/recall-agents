...
$targetPort = ${port}
${sourceTypeSetup}
function Test-SteelEnginePortMatch($value) {
...
foreach ($rule in $rules) {
  foreach ($portFilter in @($rule | Get-NetFirewallPortFilter)) {
    ...
    if (($protocol -eq 'Any' -or $protocol -eq 'TCP') -and (Test-SteelEnginePortMatch $portFilter.LocalPort)) {
...
function Resolve-SteelEngineProgramScope($rule) {
...
foreach ($rule in $policy.Rules) {
  ...
  if (($protocol -ne 'TCP' -and $protocol -ne 'Any') -or -not (Test-SteelEnginePortMatch $rule.LocalPorts)) { continue }
  [void]$matchingRules.Add([pscustomobject]@{
    ...
    Program = (Resolve-SteelEngineProgramScope $rule)
...
function Get-SteelEngineManagedRules {
  try {
...
$activeProfiles = Get-NetFirewallProfile -PolicyStore ActiveStore | Select-Object Name, @{Name='Enabled';Expression={$_.Enabled.ToString()}}, @{Name='DefaultInboundAction';Expression={$_.DefaultInboundAction.ToString()}}, @{Name='AllowInboundRules';Expression={$_.AllowInboundRules.ToString()}}, @{Name='AllowLocalFirewallRules';Expression={$_.AllowLocalFirewallRules.ToString()}}
$localProfiles = Get-NetFirewallProfile -PolicyStore localhost | Select-Object Name, @{Name='Enabled';Expression={$_.Enabled.ToString()}}, @{Name='DefaultInboundAction';Expression={$_.DefaultInboundAction.ToString()}}, @{Name='AllowInboundRules';Expression={$_.AllowInboundRules.ToString()}}, @{Name='AllowLocalFirewallRules';Expression={$_.AllowLocalFirewallRules.ToString()}}
$managedMatchingRules = @(Get-SteelEngineManagedRules)
$policy = New-Object -ComObject HNetCfg.FwPolicy2
$matchingRules = New-Object System.Collections.ArrayList
foreach ($rule in $policy.Rules) {
  if (-not $rule.Enabled -or $rule.Direction -ne 1 -or $rule.Action -ne 1) { continue }
...
export async function inspectWindowsGatewayFirewall(
  ...
  if (mode === "quick") {
    ...
    if (quickJson === null) {
      return {
        ...
        message: "SteelEngine could not quickly inspect Windows Firewall LAN Gateway policy.",
        details: [
          "Run `steelengine gateway status --deep` again, or verify the advertised LAN URL from another device.",
        ],
      };
    }
    ...
    if (!quickPayload || typeof quickPayload !== "object" || Array.isArray(quickPayload)) {
      return {
        ...
        severity: "warning",
        code: "windows_firewall_inspection_failed",
        message: "SteelEngine could not parse Windows Firewall LAN Gateway policy.",
        details: [
          "Run `steelengine gateway status --deep` again, or verify the advertised LAN URL from another device.",
        ],
      };
  ...
  if (stateJson === null || rulesJson === null) {
    return {
      ...
      severity: "warning",
      code: "windows_firewall_inspection_failed",
      message: "SteelEngine could not inspect Windows Firewall policy for LAN Gateway reachability.",
      details: [
        "Run `steelengine gateway status --deep` from a normal PowerShell session and verify the advertised LAN URL from another device.",
      ],
    };
  }
  ...
  if (!policyState) {
    return {
      ...
      severity: "warning",
      code: "windows_firewall_inspection_failed",
      message: "SteelEngine could not parse Windows Firewall policy for LAN Gateway reachability.",
      details: [
        "Run `steelengine gateway status --deep` from a normal PowerShell session and verify the advertised LAN URL from another device.",
      ],
    };
  ...
  if (shouldProbeManagedActiveRules(initialDiagnostic)) {
    ...
    if (managedRulesJson !== null) {
      ...
      return {
        ...
        code: "windows_firewall_inspection_failed",
        message:
          "SteelEngine could not inspect managed Windows Firewall rules for LAN Gateway reachability.",
        details: [
          "Run `steelengine gateway status --deep` from a normal PowerShell session and verify Group Policy or administrator-managed allow rules for the Gateway port.",
        ],
      };
    }
  }
