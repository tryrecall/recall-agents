...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "recall-device-pairing-churn-" });
...
describe("device pairing requestId churn", () => {
  ...
  test("supersedes a stale read repair request when a devices-first flow reconnects to approve the repair", async () => {
    ...
    // `recall devices approve <requestId>` reconnects with the caller scopes
