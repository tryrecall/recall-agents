...
const stateDir = "/opt/recall/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/recall/state/sessions/../config.jsonl` resolves to
    // `/opt/recall/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("normalizes Windows-style separators before anchor checks", () => {
    const winStateDir = "C:\\recall\\state";
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/recall/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
