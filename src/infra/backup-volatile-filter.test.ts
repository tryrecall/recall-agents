...
const stateDir = "/opt/steelengine/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/steelengine/state/sessions/../config.jsonl` resolves to
    // `/opt/steelengine/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("normalizes Windows-style separators before anchor checks", () => {
    const winStateDir = "C:\\steelengine\\state";
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/steelengine/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
