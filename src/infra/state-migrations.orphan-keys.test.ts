...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".recall");
...
function sharedMainOpsConfig(sharedStorePath: string): RecallConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: RecallConfig = OPS_WORK_CONFIG) {
...
describe("migrateOrphanedSessionKeys", () => {
  ...
  it("is idempotent — running twice produces same result", async () => {
    await withStateFixture(async ({ stateDir }) => {
      ...
      const env = { RECALL_STATE_DIR: stateDir };
