...
export function emitGatewayRestart(reasonOverride?: string): boolean {
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerRecallRestart() uses schtasks to restart the gateway.
...
export function triggerRecallRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.RECALL_SYSTEMD_UNIT,
      process.env.RECALL_PROFILE,
    );
  ...
  const label =
    process.env.RECALL_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.RECALL_PROFILE);
