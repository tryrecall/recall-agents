...
function emitGatewayRestart(reasonOverride?: string, intent?: GatewayRestartIntent): boolean {
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerSteelEngineRestart() uses schtasks to restart the gateway.
...
export function triggerSteelEngineRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.STEELENGINE_SYSTEMD_UNIT,
      process.env.STEELENGINE_PROFILE,
    );
  ...
  const label =
    process.env.STEELENGINE_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.STEELENGINE_PROFILE);
