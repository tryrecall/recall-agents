/**
 * High-level lifecycle management for Recall's operator-managed network
 * proxy routing.
 *
 * Recall does not spawn or configure the filtering proxy. When enabled, it
...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    RECALL_PROXY_ACTIVE: process.env["RECALL_PROXY_ACTIVE"],
    RECALL_PROXY_LOOPBACK_MODE: process.env["RECALL_PROXY_LOOPBACK_MODE"],
    RECALL_PROXY_CA_FILE: process.env["RECALL_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["RECALL_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["RECALL_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["RECALL_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["RECALL_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or RECALL_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["RECALL_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["RECALL_PROXY_CA_FILE"],
  });
