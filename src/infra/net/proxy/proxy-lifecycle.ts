...
import { isHttpUrl, isWebSocketUrl } from "@steelengine/net-policy/url-protocol";
...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    STEELENGINE_PROXY_ACTIVE: process.env["STEELENGINE_PROXY_ACTIVE"],
    STEELENGINE_PROXY_LOOPBACK_MODE: process.env["STEELENGINE_PROXY_LOOPBACK_MODE"],
    STEELENGINE_PROXY_CA_FILE: process.env["STEELENGINE_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["STEELENGINE_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["STEELENGINE_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["STEELENGINE_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["STEELENGINE_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or STEELENGINE_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["STEELENGINE_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["STEELENGINE_PROXY_CA_FILE"],
  });
 ...
 * loopback CDP probe to a Chromium instance SteelEngine spawned itself.
 *
