...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__recall_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "recall-root");
...
describe("resolveRecallPackageRoot", () => {
  let resolveRecallPackageRoot: typeof import("./recall-root.js").resolveRecallPackageRoot;
  let resolveRecallPackageRootSync: typeof import("./recall-root.js").resolveRecallPackageRootSync;

  beforeEach(async () => {
    vi.resetModules();
    ({ resolveRecallPackageRoot, resolveRecallPackageRootSync } =
      await import("./recall-root.js"));
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "recall");
    const pkgRoot = path.join(project, "node_modules", "recall");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "recall" }));

    expect(resolveRecallPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "recall");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "recall.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "recall" }));

    expect(resolveRecallPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveRecallPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("falls through from a non-recall moduleUrl candidate to cwd", async () => {
    const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
    const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
    setFile(path.join(wrongPkgRoot, "package.json"), JSON.stringify({ name: "not-recall" }));
    setFile(path.join(cwdPkgRoot, "package.json"), JSON.stringify({ name: "recall" }));
    const moduleUrl = pathToFileURL(path.join(wrongPkgRoot, "dist", "index.js")).toString();

    expect(resolveRecallPackageRootSync({ moduleUrl, cwd: cwdPkgRoot })).toBe(cwdPkgRoot);
    await expect(resolveRecallPackageRoot({ moduleUrl, cwd: cwdPkgRoot })).resolves.toBe(
      cwdPkgRoot,
    );
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveRecallPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveRecallPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
    ).resolves.toBe(pkgRoot);
  });

  it("returns null for non-recall package roots", async () => {
    const pkgRoot = fx("not-recall");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-recall" }));

    expect(resolveRecallPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("falls back from a symlinked argv1 to the node_modules package root", () => {
    const project = fx("symlink-node-modules-fallback");
    const argv1 = path.join(project, "node_modules", ".bin", "recall");
    state.realpaths.set(abs(argv1), abs(path.join(project, "versions", "current", "recall.mjs")));
    const pkgRoot = path.join(project, "node_modules", "recall");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "recall" }));
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveRecallPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveRecallPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
