...
function setPackageRoot(root: string, name = "recall") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./recall-root.js").resolveRecallPackageRootSync,
  asyncResolver: typeof import("./recall-root.js").resolveRecallPackageRoot,
  opts: Parameters<typeof import("./recall-root.js").resolveRecallPackageRootSync>[0],
...
vi.mock("./recall-root.fs.runtime.js", () => ({
...
describe("resolveRecallPackageRoot", () => {
  ...
  let clearRecallPackageRootCaches: typeof import("./recall-root.js").testing.clearRecallPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveRecallPackageRoot,
      resolveRecallPackageRootSync,
      testing: { clearRecallPackageRootCaches },
    } = await import("./recall-root.js"));
  });

  beforeEach(() => {
    clearRecallPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    ...
    {
      name: "falls through from a non-recall moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-recall");
    ...
    {
      name: "returns null for non-recall package roots",
      setup: () => {
        const pkgRoot = fx("not-recall");
        setPackageRoot(pkgRoot, "not-recall");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "recall.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "recall");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
