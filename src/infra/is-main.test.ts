...
describe("isMainModule", () => {
  ...
  it("returns true for configured wrapper-to-entry pairs", () => {
    expect(
      isMainModule({
        ...
        argv: ["node", "/repo/steelengine.mjs"],
        cwd: "/repo",
        env: {},
        wrapperEntryPairs: [{ wrapperBasename: "steelengine.mjs", entryBasename: "entry.js" }],
      }),
    ).toBe(true);
  });
  ...
  it("returns false when this module is only imported under PM2", () => {
    expect(
      isMainModule({
        currentFile: "/repo/node_modules/steelengine/dist/index.js",
