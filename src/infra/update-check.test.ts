...
describe("compareSemverStrings", () => {
  ...
  it("treats Recall stable correction releases as newer than their base release", () => {
...
describe("checkDepsStatus", () => {
  it("reports unknown, missing, stale, and ok states from lockfile markers", async () => {
    await withTempDir({ prefix: "recall-update-check-" }, async (base) => {
...
describe("checkUpdateStatus", () => {
  ...
  it("detects package installs for non-git roots", async () => {
    await withTempDir({ prefix: "recall-update-check-" }, async (root) => {
      await fs.writeFile(
  ...
  it("treats symlinked git installs as git roots", async () => {
    await withTempDir({ prefix: "recall-update-check-git-" }, async (base) => {
      const repoRoot = path.join(base, "repo");
      const linkedRoot = path.join(base, "linked-recall");
      await fs.mkdir(repoRoot, { recursive: true });
      await fs.writeFile(
        path.join(repoRoot, "package.json"),
        JSON.stringify({ name: "recall", packageManager: "pnpm@10.0.0" }),
        "utf8",
      );
