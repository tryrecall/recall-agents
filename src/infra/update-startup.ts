...
import { resolveRecallPackageRoot } from "./recall-root.js";
...
function resolveAutoUpdatePolicy(cfg: RecallConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: RecallConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.RECALL_NO_AUTO_UPDATE);
  ...
  const root = await resolveRecallPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("recall update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by RECALL_NO_AUTO_UPDATE", {
