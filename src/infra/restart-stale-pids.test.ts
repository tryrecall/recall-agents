...
function createSteelEngineBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "steelengine-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation((command: unknown) => {
    ...
    call += 1;
    if (call === 1) {
      return createSteelEngineBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses steelengine-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "steelengine-gateway" },
          { pid: process.pid, cmd: "steelengine-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("verifies argv when lsof reports the node process name instead of steelengine", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        if (command === "ps") {
          return {
            error: null,
            status: 0,
            stdout: "node /opt/steelengine/dist/entry.js gateway\n",
            stderr: "",
          };
        }
        return {
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: steelengine-weixin sidecar (child of the gateway) invoked
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "steelengine-gateway" },
          { pid: unrelatedStalePid, cmd: "steelengine-gateway" },
        ]),
        stderr: "",
      });
      ...
      () => {
        ...
        mockReadFileSync.mockImplementation((path: unknown): string => {
          if (path === `/proc/${directParentPid}/status`) {
            return `Name:\tsteelengine-gateway\nPid:\t${directParentPid}\nPPid:\t${grandparentPid}\n`;
          }
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "steelengine-gateway" },
            { pid: grandparentPid, cmd: "steelengine-gateway" },
            { pid: benignStalePid, cmd: "steelengine-gateway" },
          ]),
          stderr: "",
        });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "steelengine-gateway" },
            { pid: gatewayGrandparentPid, cmd: "steelengine-gateway" },
          ]),
          stderr: "",
        });
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            ...
            status: 0,
            stdout: lsofOutput([
              { pid: toolHostPid, cmd: "steelengine-gateway" },
              { pid: gatewayGrandparentPid, cmd: "steelengine-gateway" },
              { pid: benignStalePid, cmd: "steelengine-gateway" },
            ]),
            stderr: "",
    ...
    it("excludes pids whose command does not include 'steelengine'", () => {
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: process.pid + 3152, cmd: "steelengine-gateway" }]),
            stderr: "",
          };
        });
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["steelengine", "gateway"]);
        expect(findGatewayPidsOnPortSync(18789)).toEqual([stalePid]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(18789, undefined);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple steelengine pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "steelengine-gateway" },
          { pid: pid2, cmd: "steelengine-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-steelengine pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks steelengine processes — non-steelengine listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty steelengine stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty steelengine stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + steelengine pid in stdout — container-restricted lsof reports partial results
          return createSteelEngineBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "steelengine-gateway" }]),
          stderr: "",
        };
      });
    ...
    it("continues cleanup and port polling when individual process signals fail", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        ...
        return lsofCall === 1
          ? createLsofResult({
              stdout: lsofOutput([
                { pid: termDeniedPid, cmd: "steelengine-gateway" },
                { pid: killDeniedPid, cmd: "steelengine-gateway" },
              ]),
            })
          : createLsofResult({ status: 1 });
      });
    ...
    it("does not kill a protected gateway pid after reparenting", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        return lsofCall === 1
          ? createLsofResult({
              stdout: lsofOutput([
                { pid: protectedPid, cmd: "steelengine-gateway" },
                { pid: stalePid, cmd: "steelengine-gateway" },
              ]),
            })
          : createLsofResult({ status: 1 });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'steelengine'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("steelengine")
      // inside the mid-loop flush: a non-steelengine cmd between two entries must not
      // be pushed, but the following steelengine entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-steelengine entry first, then steelengine entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — parse false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid steelengine entry must still be found.
      const stdout = `p0\ncsteelengine-gateway\np${stalePid}\ncsteelengine-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  describe("pollPortOnce — status 1 + non-empty stdout", () => {
    it("keeps polling when status 1 includes a non-steelengine listener record", () => {
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          if (command === "lsof") {
            ...
            return lsofCall === 2
              ? createLsofResult({
                  stdout: lsofOutput([{ pid: gatewayParentPid, cmd: "steelengine-gateway" }]),
                })
              : createLsofResult({ status: 1 });
          }
          return createLsofResult();
