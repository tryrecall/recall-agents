...
function createRecallBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "recall-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation((command: unknown) => {
    ...
    call += 1;
    if (call === 1) {
      return createRecallBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses recall-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "recall-gateway" },
          { pid: process.pid, cmd: "recall-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("verifies argv when lsof reports the node process name instead of recall", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        if (command === "ps") {
          return {
            error: null,
            status: 0,
            stdout: "node /opt/recall/dist/entry.js gateway\n",
            stderr: "",
          };
        }
        return {
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: recall-weixin sidecar (child of the gateway) invoked
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "recall-gateway" },
          { pid: unrelatedStalePid, cmd: "recall-gateway" },
        ]),
        stderr: "",
      });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "recall-gateway" },
            { pid: grandparentPid, cmd: "recall-gateway" },
            { pid: benignStalePid, cmd: "recall-gateway" },
          ]),
          stderr: "",
        });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "recall-gateway" },
            { pid: gatewayGrandparentPid, cmd: "recall-gateway" },
          ]),
          stderr: "",
        });
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            ...
            status: 0,
            stdout: lsofOutput([
              { pid: toolHostPid, cmd: "recall-gateway" },
              { pid: gatewayGrandparentPid, cmd: "recall-gateway" },
              { pid: benignStalePid, cmd: "recall-gateway" },
            ]),
            stderr: "",
    ...
    it("excludes pids whose command does not include 'recall'", () => {
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: process.pid + 3152, cmd: "recall-gateway" }]),
            stderr: "",
          };
        });
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["recall", "gateway"]);
        expect(findGatewayPidsOnPortSync(18789)).toEqual([stalePid]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(18789, undefined);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple recall pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "recall-gateway" },
          { pid: pid2, cmd: "recall-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-recall pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks recall processes — non-recall listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty recall stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty recall stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + recall pid in stdout — container-restricted lsof reports partial results
          return createRecallBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "recall-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'recall'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("recall")
      // inside the mid-loop flush: a non-recall cmd between two entries must not
      // be pushed, but the following recall entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-recall entry first, then recall entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid recall entry must still be found.
      const stdout = `p0\ncopenclaw-gateway\np${stalePid}\ncopenclaw-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero recall pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-recall stdout (line 145)", () => {
    it("treats status 1 + non-recall stdout as port-free (not an recall process)", () => {
      // status 1 + non-empty stdout where no recall pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill recall pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-recall output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // No recall pids in status-1 output means the port is free for this cleanup.
      expect(cleanStaleGatewayProcessesSync()).toContain(stalePid);
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          if (command === "lsof") {
            ...
            return createLsofResult({
              stdout: lsofOutput([{ pid: gatewayParentPid, cmd: "recall-gateway" }]),
            });
          }
          return createLsofResult();
        });
