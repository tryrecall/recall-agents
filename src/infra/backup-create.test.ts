...
describe("formatBackupCreateSummary", () => {
  const backupArchiveLine = "Backup archive: /tmp/recall-backup.tar.gz";
...
describe("buildExtensionsNodeModulesFilter", () => {
  it("excludes dependency trees only under state extensions", () => {
    ...
    expect(filter("/state/extensions/demo/recall.plugin.json")).toBe(true);
  ...
  it("normalizes Windows path separators", () => {
    const filter = buildExtensionsNodeModulesFilter("C:\\Users\\me\\.recall\\");

    expect(filter(String.raw`C:\Users\me\.recall\extensions\demo\index.js`)).toBe(true);
    expect(
      filter(String.raw`C:\Users\me\.recall\extensions\demo\node_modules\dep\index.js`),
    ).toBe(false);
  });
});
...
describe("createBackupArchive", () => {
  ...
  it("omits installed plugin node_modules from the real archive while keeping plugin files", async () => {
    await withRecallTestState(
      {
        layout: "state-only",
        prefix: "recall-backup-plugin-deps-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        await fs.writeFile(
          path.join(stateDir, "extensions", "demo", "recall.plugin.json"),
          '{"id":"demo"}\n',
          "utf8",
        );
        await fs.writeFile(
        ...
        expect(entrySuffixes).toContain("/state/extensions/demo/recall.plugin.json");
  ...
  it("does not duplicate the root manifest when the system tempdir lives inside the state dir", async () => {
    await withRecallTestState(
      {
        layout: "state-only",
        prefix: "recall-backup-tmp-overlap-",
        scenario: "minimal",
      },
      async (state) => {
        const stateDir = state.stateDir;
  ...
  it("does not duplicate the root manifest when the system tempdir is the state dir itself", async () => {
    await withRecallTestState(
      {
        layout: "state-only",
        prefix: "recall-backup-tmp-equals-state-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
