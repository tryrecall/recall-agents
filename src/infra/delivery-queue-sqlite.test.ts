...
describe("delivery-queue-sqlite corrupt JSON resilience", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-dq-case-"));
...
describe("countFailedDeliveryQueueEntries", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-dq-count-"));
    stateDir = path.join(tmpDir, "state");
    fs.mkdirSync(stateDir, { recursive: true });
  });
