...
async function createStagedNpmInstall(
  ...
  const prefix = await fs.mkdtemp(path.join(targetLayout.globalRoot, ".recall-update-stage-"));
...
async function prepareNpmGitSourceInstallSpec(params: {
  ...
  const packDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-update-pack-"));
...
async function replaceNpmBinShims(params: {
  ...
  const names = new Set([params.packageName, "recall"]);
  ...
  const backup: NpmBinShimBackup = {
    backupDir: await fs.mkdtemp(
      path.join(params.targetLayout.globalRoot, ".recall-shim-backup-"),
    ),
...
async function swapStagedNpmInstall(params: {
  ...
  const backupRoot = path.join(targetLayout.globalRoot, `.recall-${process.pid}-${Date.now()}`);
