...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers RECALL_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for RECALL_HOME", () => {
    const result = resolveRequiredHomeDir(
      { RECALL_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when RECALL_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ RECALL_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("resolveOsHomeDir", () => {
  it("ignores RECALL_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          RECALL_HOME: "/srv/recall-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/recall-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/recall-home " },
      expected: "/srv/recall-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/recall-home/docs"));
  });
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of RECALL_HOME", () => {
    expect(
