...
const SUPERVISOR_HINTS = {
  launchd: ["STEELENGINE_LAUNCHD_LABEL"],
  systemd: ["STEELENGINE_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["STEELENGINE_WINDOWS_TASK_NAME"],
} as const;
...
interface DetectRespawnSupervisorOptions {
  includeLinuxSteelEngineGatewayServiceMarker?: boolean;
}
...
function hasSteelEngineGatewayServiceMarker(env: NodeJS.ProcessEnv): boolean {
  return (
    env.STEELENGINE_SERVICE_MARKER?.trim() === "steelengine" &&
    env.STEELENGINE_SERVICE_KIND?.trim() === "gateway"
  );
}

function isCurrentGatewayLaunchdJob(env: NodeJS.ProcessEnv): boolean {
  const expectedLabel = resolveGatewayLaunchAgentLabel(env.STEELENGINE_PROFILE);
  if (
...
export function detectRespawnSupervisor(
  ...
  if (platform === "linux") {
    return hasAnyHint(env, SUPERVISOR_HINTS.systemd) ||
      (options.includeLinuxSteelEngineGatewayServiceMarker === true &&
        hasSteelEngineGatewayServiceMarker(env))
      ? "systemd"
      : null;
  }
  if (platform === "win32") {
    ...
    const marker = env.STEELENGINE_SERVICE_MARKER?.trim();
