...
import { triggerRecallRestart } from "./restart.js";
 ...
 * - RECALL_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
...
export function restartGatewayProcessWithFreshPid(
  ...
  if (isTruthy(process.env.RECALL_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    if (supervisor === "schtasks") {
      const restart = triggerRecallRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
...
export function respawnGatewayProcessForUpdate(
  ...
  if (isTruthy(process.env.RECALL_NO_RESPAWN)) {
    return { mode: "disabled", detail: "RECALL_NO_RESPAWN" };
  }
  const supervisor = detectRespawnSupervisor(process.env);
  if (supervisor) {
    if (supervisor === "schtasks") {
