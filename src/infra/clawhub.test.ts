...
describe("clawhub helpers", () => {
  ...
  afterEach(() => {
    delete process.env.STEELENGINE_CLAWHUB_URL;
  ...
  it("treats SteelEngine release correction versions as stable plugin API hosts", () => {
  ...
  it("checks min gateway versions with loose host labels", () => {
    ...
    expect(satisfiesGatewayMinimum("SteelEngine 2026.3.22", "2026.3.0")).toBe(true);
  ...
  it("loads ClawHub request auth from config.json", async () => {
    await withTempDir({ prefix: "steelengine-clawhub-config-" }, async (configRoot) => {
  ...
  it("loads ClawHub request auth from the legacy config path override", async () => {
    await withTempDir({ prefix: "steelengine-clawdhub-config-" }, async (configRoot) => {
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await withTempDir({ prefix: "steelengine-clawhub-home-" }, async (fakeHome) => {
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await withTempDir({ prefix: "steelengine-clawhub-home-" }, async (fakeHome) => {
        await withTempDir({ prefix: "steelengine-clawhub-xdg-" }, async (xdgRoot) => {
  ...
  it("preserves the configured ClawHub base URL path prefix", async () => {
    process.env.STEELENGINE_CLAWHUB_URL = "https://internal.example.com/clawhub";
    let requestedUrl = "";
  ...
  it("fetches typed package artifact resolver reports", async () => {
    ...
    expect(new URL(requestedUrl).pathname).toBe(
      "/api/v1/packages/%40steelengine%2Fdiagnostics-otel/versions/2026.3.22/artifact",
    );
  });
  ...
  it("fetches typed package security reports", async () => {
    ...
    expect(new URL(requestedUrl).pathname).toBe(
      "/api/v1/packages/%40steelengine%2Fdiagnostics-otel/versions/2026.3.22/security",
    );
  });
  ...
  it("annotates 429 errors with the reset hint and a sign-in hint when unauthenticated", async () => {
    process.env.CLAWHUB_CONFIG_PATH = path.join(os.tmpdir(), "steelengine-no-clawhub-config");
    await expect(
      searchClawHubSkills({
        query: "calendar",
        fetchImpl: async () =>
