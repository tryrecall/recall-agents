...
describe("clawhub helpers", () => {
  ...
  afterEach(() => {
    delete process.env.RECALL_CLAWHUB_URL;
  ...
  it("treats Recall CalVer correction versions as stable plugin API hosts", () => {
  ...
  it("resolves ClawHub auth token from config.json", async () => {
    await withTempDir({ prefix: "recall-clawhub-config-" }, async (configRoot) => {
  ...
  it("resolves ClawHub auth token from the legacy config path override", async () => {
    await withTempDir({ prefix: "recall-clawdhub-config-" }, async (configRoot) => {
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await withTempDir({ prefix: "recall-clawhub-home-" }, async (fakeHome) => {
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await withTempDir({ prefix: "recall-clawhub-home-" }, async (fakeHome) => {
        await withTempDir({ prefix: "recall-clawhub-xdg-" }, async (xdgRoot) => {
  ...
  it("preserves the configured ClawHub base URL path prefix", async () => {
    process.env.RECALL_CLAWHUB_URL = "https://internal.example.com/clawhub";
    let requestedUrl = "";
    ...
    await expect(
      fetchClawHubPackageReadiness({
        ...
        fetchImpl: async (input) => {
          ...
          return new Response(
            JSON.stringify({
              package: { name: "@recall/diagnostics-otel", isOfficial: true },
  ...
  it("annotates 429 errors with the reset hint and a sign-in hint when unauthenticated", async () => {
    process.env.RECALL_CLAWHUB_CONFIG_PATH = path.join(os.tmpdir(), "recall-no-clawhub-config");
    await expect(
      searchClawHubSkills({
        query: "calendar",
        fetchImpl: async () =>
