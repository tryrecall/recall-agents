...
async function createTimelineEnv() {
  const dir = await mkdtemp(join(tmpdir(), "recall-diagnostics-timeline-"));
  tempDirs.push(dir);
  return {
    env: {
      RECALL_DIAGNOSTICS: "timeline",
      RECALL_DIAGNOSTICS_RUN_ID: "run-1",
      RECALL_DIAGNOSTICS_ENV: "env-1",
      RECALL_DIAGNOSTICS_TIMELINE_PATH: join(dir, "nested", "timeline.jsonl"),
...
describe("diagnostics timeline", () => {
  it("detects when timeline output is enabled", async () => {
    ...
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, RECALL_DIAGNOSTICS: "1" } })).toBe(true);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, RECALL_DIAGNOSTICS: "yes" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, RECALL_DIAGNOSTICS: "on" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, RECALL_DIAGNOSTICS: "all" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, RECALL_DIAGNOSTICS: "*" } })).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, RECALL_DIAGNOSTICS: "diagnostics.timeline" },
      }),
    ).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({ env: { ...env, RECALL_DIAGNOSTICS: "telegram.http" } }),
    ).toBe(false);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, RECALL_DIAGNOSTICS: "0" } })).toBe(
      false,
    );
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, RECALL_DIAGNOSTICS_TIMELINE_PATH: "" },
      }),
    ).toBe(false);
  });
  ...
  it("honors config diagnostics flags after config is available", async () => {
    ...
    delete envWithoutFlag.RECALL_DIAGNOSTICS;
    const configWithTimeline = { diagnostics: { flags: ["timeline"] } } as RecallConfig;
    const configWithWildcard = { diagnostics: { flags: ["*"] } } as RecallConfig;
    const configWithoutTimeline = { diagnostics: { flags: ["telegram.http"] } } as RecallConfig;
  ...
  it("writes JSONL diagnostic events with the stable envelope", async () => {
    ...
    expect(event?.schemaVersion).toBe("recall.diagnostics.v1");
  ...
  it("records span start and end events around successful work", async () => {
    ...
    delete configOnlyEnv.RECALL_DIAGNOSTICS;

    await expect(
      measureDiagnosticsTimelineSpan("runtimeDeps.stage", () => "ok", {
        phase: "startup",
        attributes: { pluginCount: 3 },
        config: { diagnostics: { flags: ["timeline"] } } as RecallConfig,
        env: configOnlyEnv,
      }),
    ).resolves.toBe("ok");
