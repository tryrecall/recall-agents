...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.RECALL_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*recall-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "recall";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "recall");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "recall-gw");
  ...
  records.push(`_recall-gw._tcp IN PTR ${instanceLabel}._recall-gw._tcp`);
  records.push(`${instanceLabel}._recall-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._recall-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; recall-content-hash: ${contentHash}\n${contentBody}`;
}
