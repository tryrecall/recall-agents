...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.STEELENGINE_WIDE_AREA_DOMAIN ?? null;
  try {
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*steelengine-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "steelengine";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "steelengine");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "steelengine-gw");
  ...
  records.push(`_steelengine-gw._tcp IN PTR ${instanceLabel}._steelengine-gw._tcp`);
  records.push(`${instanceLabel}._steelengine-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._steelengine-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; steelengine-content-hash: ${contentHash}\n${contentBody}`;
}
