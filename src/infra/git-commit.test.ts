...
async function makeTempDir(label: string): Promise<string> {
  return await tempDirs.make(`steelengine-${label}-`);
}
...
async function makeFakeSteelEnginePackage(root: string) {
...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the steelengine package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "steelengine");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "steelengine", version: "2026.3.10" }),
      "utf-8",
    );
