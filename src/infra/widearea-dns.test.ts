...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  ...
  gatewayPort: 18789,
  displayName: "Mac Studio (SteelEngine)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "steelengine.internal", expected: "steelengine.internal." },
    { value: "steelengine.internal.", expected: "steelengine.internal." },
    { value: "  steelengine.internal  ", expected: "steelengine.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { STEELENGINE_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { STEELENGINE_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
    ...
    },
    {
      ...
      params: {
        env: { STEELENGINE_WIDE_AREA_DOMAIN: "foo/bar" } as NodeJS.ProcessEnv,
      },
      expected: null,
    },
  ...
  it("builds valid zone paths under the DNS config directory", () => {
    ...
    const zonePath = getWideAreaZonePath("steelengine.internal.");

    expect(zonePath).toBe(path.join(dnsDir, "steelengine.internal.db"));
    expect(path.relative(dnsDir, zonePath)).toBe("steelengine.internal.db");
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN steelengine.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_steelengine-gw._tcp IN PTR studio-london._steelengine-gw._tcp`,
      `studio-london._steelengine-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (SteelEngine)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/steelengine`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN steelengine.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._steelengine-gw._tcp IN TXT`,
        `displayName=Mac Studio (SteelEngine)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("steelengine.internal."),
      changed: false,
    });
    expect(writeSpy).not.toHaveBeenCalled();
  });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("steelengine.internal."),
      expectedZoneText,
      "utf-8",
    );
  });
