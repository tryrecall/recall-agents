...
const matrixChunkConfig: RecallConfig = {
  channels: { matrix: { textChunkLimit: 4000 } } as RecallConfig["channels"],
};

const expectedPreferredTmpRoot = resolvePreferredRecallTmpDir();
...
async function runChunkedMatrixDelivery(params?: {
  ...
  const cfg: RecallConfig = {
    channels: { matrix: { textChunkLimit: 2 } } as RecallConfig["channels"],
  };
...
describe("deliverOutboundPayloads", () => {
  ...
  it("chunks direct adapter text and preserves delivery overrides across sends", async () => {
    ...
    const results = await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 2 } } } as RecallConfig,
      channel: "matrix",
      to: "!room",
      accountId: "default",
  ...
  it("passes delivery config and account context to adapter payload normalization", async () => {
    ...
    const cfg = { channels: { matrix: { enabled: true } } } as unknown as RecallConfig;
    setActivePluginRegistry(
      createTestRegistry([
        {
          pluginId: "matrix",
  ...
  it("uses adapter-provided formatted senders and scoped media roots when available", async () => {
    ...
    const textResults = await deliverOutboundPayloads({
      cfg: { channels: { line: {} } } as RecallConfig,
      channel: "line",
      to: "U123",
      accountId: "default",
    ...
    const cfg = { channels: { line: {} } } as RecallConfig;
    await deliverOutboundPayloads({
      cfg,
      channel: "line",
      to: "U123",
    ...
    expect(
      sendFormattedMediaCall?.mediaLocalRoots?.some((root) =>
        root.endsWith(path.join(".recall", "workspace-work")),
      ),
    ).toBe(true);
    expect(sendMedia).not.toHaveBeenCalled();
  });

  it("includes Recall tmp root in plugin mediaLocalRoots", async () => {
    const sendMatrix = vi.fn().mockResolvedValue({ messageId: "m-media", roomId: "!room" });

    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as RecallConfig,
      channel: "matrix",
      to: "!room:example",
  ...
  it("lets explicit formatting options override configured chunking", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 4000 } } } as RecallConfig,
      channel: "matrix",
      to: "!room",
      payloads: [{ text: "abcd" }],
  ...
  it("bails out without sending when a concurrent drain already claimed the queue entry", async () => {
    // Regression for tryrecall/recall-agents#70386: if a reconnect or startup drain
