  ...
  resolveMissingOfficialExternalChannelPluginRepairHint: ({ channelId }: { channelId: string }) =>
    mocks.missingOfficialExternalChannels.has(channelId)
      ? {
          ...
          installSpec: `@steelengine/${channelId}`,
          installCommand: `steelengine plugins install @steelengine/${channelId}`,
          doctorFixCommand: "steelengine doctor --fix",
          repairHint: `Install the official external plugin with: steelengine plugins install @steelengine/${channelId}, or run: steelengine doctor --fix.`,
        }
      : null,
}));
...
describe("resolveMessageChannelSelection", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedMessage:
        "Channel is unavailable: feishu. Install the official external plugin with: steelengine plugins install @steelengine/feishu, or run: steelengine doctor --fix.",
    },
    {
      params: { cfg: {} as never },
      expectedMessage:
        "Channel is required (no configured channels detected). Run steelengine channels add to configure one",
    },
    {
      setup: () => {
        mocks.resolveOutboundChannelPlugin.mockReturnValue(undefined);
      ...
      expectedMessage:
        "Channel is required (no available channels detected). Configured official external channel WhatsApp is missing its plugin. Install the official external plugin with: steelengine plugins install @steelengine/whatsapp, or run: steelengine doctor --fix.",
    },
    {
      setup: () => {
        mocks.listChannelPlugins.mockReturnValue([
