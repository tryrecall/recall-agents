...
function resolveAvailableKnownChannel(params: {
  ...
  // been eagerly loaded (e.g. `steelengine agent --local`). Already-loaded and
...
export function isConfiguredChannel(cfg: SteelEngineConfig, channelId: string): boolean {
...
function formatMissingOfficialExternalChannelsMessage(
  ...
  return `Configured official external channels ${labels} are missing their plugins. Run: steelengine doctor --fix, or install individually: ${installCommands}.`;
}

function formatNoConfiguredChannelsMessage(): string {
  return [
    "Channel is required (no configured channels detected).",
    "Run steelengine channels add to configure one, or pass --channel <channel> after enabling a channel.",
    "Use steelengine channels list --all to see available channel ids.",
  ].join(" ");
}
...
async function isPluginConfigured(plugin: ChannelPlugin, cfg: SteelEngineConfig): Promise<boolean> {
