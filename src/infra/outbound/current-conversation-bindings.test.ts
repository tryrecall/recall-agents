...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.RECALL_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-current-bindings-"));
    process.env.RECALL_STATE_DIR = testStateDir;
