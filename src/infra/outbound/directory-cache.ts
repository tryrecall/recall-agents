...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: RecallConfig | null = null;
  ...
  get(key: string, cfg: RecallConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: RecallConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: RecallConfig): void {
