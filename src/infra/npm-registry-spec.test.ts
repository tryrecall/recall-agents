...
describe("npm registry spec validation", () => {
  it.each([
    ...
    "@steelengine/voice-call@1.2.3",
    "@steelengine/voice-call@1.2.3-beta.4",
    "@steelengine/voice-call@latest",
    "@steelengine/voice-call@beta",
  ])("accepts %s", (spec) => {
  ...
  it.each([
    {
      spec: "@steelengine/voice-call@^1.2.3",
    ...
    {
      spec: "git+ssh://github.com/steelengine/steelengine",
      expected: "URLs are not allowed",
    },
    {
...
describe("npm registry spec parsing helpers", () => {
  it.each([
    ...
    },
    {
      ...
      expected: {
        name: "@steelengine/voice-call",
        raw: "@steelengine/voice-call@beta",
        selector: "beta",
        selectorKind: "tag",
        selectorIsPrerelease: false,
      },
    ...
    {
      spec: "@steelengine/voice-call@2026.5.3-1",
      expected: {
        name: "@steelengine/voice-call",
        raw: "@steelengine/voice-call@2026.5.3-1",
        selector: "2026.5.3-1",
    ...
    {
      spec: "@steelengine/voice-call@1.2.3-beta.1",
      expected: {
        name: "@steelengine/voice-call",
        raw: "@steelengine/voice-call@1.2.3-beta.1",
  ...
  it.each([
    { spec: "@steelengine/voice-call", expected: true },
    { spec: "@steelengine/voice-call@1.2.3", expected: true },
    { spec: "@other/voice-call", expected: false },
    { spec: "voice-call", expected: false },
    { spec: "npm:@steelengine/voice-call", expected: false },
    { spec: undefined, expected: false },
  ])("detects SteelEngine-org npm specs for %s", ({ spec, expected }) => {
    expect(isSteelEngineOrgNpmSpec(spec)).toBe(expected);
  });
  ...
  ])("compares SteelEngine release versions for %s and %s", ({ left, right, expected }) => {
    expect(compareSteelEngineReleaseVersions(left, right)).toBe(expected);
  });
});
...
describe("npm prerelease resolution policy", () => {
  it.each([
    ...
    {
      spec: "@steelengine/voice-call@latest",
  ...
  it.each([
    {
      spec: "@steelengine/voice-call",
      resolvedVersion: "1.2.3-beta.1",
      expected: `Use "@steelengine/voice-call@beta"`,
    },
    {
      spec: "@steelengine/voice-call@beta",
      resolvedVersion: "1.2.3-rc.1",
