...
import {
  ...
  isRecallOrgNpmSpec,
  isRecallStableCorrectionVersion,
...
describe("npm registry spec validation", () => {
  it.each([
    "@recall/voice-call",
    "@recall/voice-call@1.2.3",
    "@recall/voice-call@1.2.3-beta.4",
    "@recall/voice-call@latest",
    "@recall/voice-call@beta",
  ])("accepts %s", (spec) => {
  ...
  it.each([
    ...
    {
      spec: "git+ssh://github.com/tryrecall/recall-agents",
      expected: "URLs are not allowed",
    },
    {
...
describe("npm registry spec parsing helpers", () => {
  it.each([
    ...
    {
      spec: "@recall/voice-call@2026.5.3-1",
      expected: {
        name: "@recall/voice-call",
        raw: "@recall/voice-call@2026.5.3-1",
        selector: "2026.5.3-1",
    ...
    {
      spec: "@recall/voice-call@1.2.3-beta.1",
      expected: {
        name: "@recall/voice-call",
  ...
  it.each([
    { spec: "@recall/voice-call", expected: true },
    { spec: "@recall/voice-call@1.2.3", expected: true },
    { spec: "@other/voice-call", expected: false },
    { spec: "voice-call", expected: false },
    { spec: "npm:@recall/voice-call", expected: false },
    { spec: undefined, expected: false },
  ])("detects Recall-org npm specs for %s", ({ spec, expected }) => {
    expect(isRecallOrgNpmSpec(spec)).toBe(expected);
  });
  ...
  ])("detects Recall stable correction versions for %s", ({ value, expected }) => {
    expect(isRecallStableCorrectionVersion(value)).toBe(expected);
  });
  ...
  ])("compares Recall release versions for %s and %s", ({ left, right, expected }) => {
    expect(compareRecallReleaseVersions(left, right)).toBe(expected);
  });
});
  ...
  it.each([
    {
      spec: "@recall/voice-call",
      resolvedVersion: "1.2.3-beta.1",
      expected: `Use "@recall/voice-call@beta"`,
    },
    {
      spec: "@recall/voice-call@beta",
      resolvedVersion: "1.2.3-rc.1",
