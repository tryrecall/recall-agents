...
describe("command explainer tree-sitter runtime", () => {
  ...
  it("reports missing parser packages and wasm files with explainer context", () => {
    expect(() =>
      resolvePackageFileForCommandExplanation(
        "definitely-missing-recall-parser-package",
        "parser.wasm",
      ),
    ).toThrow("Unable to resolve definitely-missing-recall-parser-package");

    expect(() =>
      resolvePackageFileForCommandExplanation("web-tree-sitter", "missing-recall-parser.wasm"),
    ).toThrow("Unable to locate missing-recall-parser.wasm in web-tree-sitter");
  });
  ...
  it("detects eval, source, aliases, and carrier shell wrappers", async () => {
    const evalCommand = await explainShellCommand('eval "$RECALL_CMD"');
