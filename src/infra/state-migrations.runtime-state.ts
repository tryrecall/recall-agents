...
>;
type LegacyConfigHealthImportDatabase = Pick<SteelEngineStateKyselyDatabase, "config_health_entries">;
...
const DEFAULT_VOICEWAKE_TRIGGERS = ["steelengine", "claude", "computer"];
...
export function migrateLegacyVoiceWakeSettings(params: {
  ...
  const env = { ...process.env, STEELENGINE_STATE_DIR: params.stateDir };
...
export function migrateLegacyConfigHealth(params: {
  ...
  let shouldArchive = false;
  try {
    const result = runSteelEngineStateWriteTransaction(
      ({ db }) => {
