...
let ensureRecallCliOnPath: typeof import("./path-env.js").ensureRecallCliOnPath;

describe("ensureRecallCliOnPath", () => {
  const envKeys = [
    "PATH",
    "RECALL_PATH_BOOTSTRAPPED",
  ...
  beforeAll(async () => {
    ({ ensureRecallCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/recall-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "recall");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureRecallCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling recall exists", () => {
    const { tmp, appBinDir, appCli } = setupAppCliRoot("case-bundled");
    process.env.PATH = "/usr/bin";
    delete process.env.RECALL_PATH_BOOTSTRAPPED;
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "recall");
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.RECALL_PATH_BOOTSTRAPPED;
      delete process.env.RECALL_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.RECALL_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/recall-path/case-linuxbrew");
