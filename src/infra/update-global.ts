...
/** Supported package managers for SteelEngine global install and update flows. */
...
const PRIMARY_PACKAGE_NAME = "steelengine";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
/** npm-compatible spec used when the user asks to install the moving main branch. */
const STEELENGINE_MAIN_PACKAGE_SPEC = "github:steelengine/steelengine#main";
const COREPACK_ENABLE_DOWNLOAD_PROMPT_DEFAULT = "0";
const NPM_GLOBAL_INSTALL_QUIET_FLAGS = ["--no-fund", "--no-audit", "--loglevel=error"] as const;
const PNPM_STEELENGINE_BUILD_ALLOWLIST_FLAG = `--allow-build=${PRIMARY_PACKAGE_NAME}`;
const BUN_STEELENGINE_TRUST_FLAG = "--trust";
...
/**
 * Extracts a pinned installed version from package specs like `steelengine@1.2.3`.
...
/**
 * Verifies that a global package root looks like a packaged SteelEngine install
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "steelengine.plugin.json")))
      ) {
...
async function resolvePortableGitPathPrepend(): Promise<string[]> {
  ...
  const portableGitRoot = path.join(localAppData, "SteelEngine", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.STEELENGINE_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.STEELENGINE_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return STEELENGINE_MAIN_PACKAGE_SPEC;
  }
 ...
 * Used for staged installs where SteelEngine creates the prefix itself.
 */
...
function resolveBunGlobalInstallSpec(spec: string): string {
  ...
  // tarball is added beside it and can form an steelengine dependency loop.
...
/**
 * Detects an installed global SteelEngine package by probing package-manager roots
...
/**
 * Builds the primary package-manager argv for a global SteelEngine install.
 * npm receives quiet/freshness-bypass flags; pnpm and Bun approve SteelEngine's lifecycle.
 */
  ...
  if (resolved.manager === "pnpm") {
    return [
      ...
      ...(resolved.pnpmIsolated?.globalBinDir
        ...
        : []),
      PNPM_STEELENGINE_BUILD_ALLOWLIST_FLAG,
      spec,
    ];
  }
  if (resolved.manager === "bun") {
