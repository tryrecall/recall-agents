...
type GatewayBootLifecycleTestDatabase = Pick<SteelEngineStateKyselyDatabase, "gateway_boot_lifecycle">;
...
function createLifecycleDb() {
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-gateway-boot-"));
