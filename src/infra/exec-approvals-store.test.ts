...
afterEach(() => {
  vi.restoreAllMocks();
  if (originalRecallHome === undefined) {
...
function approvalsFilePath(homeDir: string): string {
  return path.join(homeDir, ".recall", "exec-approvals.json");
}
...
describe("exec approvals store helpers", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    expect(path.normalize(resolveExecApprovalsPath())).toBe(
      path.normalize(path.join(dir, ".recall", "exec-approvals.json")),
    );
    expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
      path.normalize(path.join(dir, ".recall", "exec-approvals.sock")),
    );
  });
  ...
  it("accepts a symlinked RECALL_HOME as the trusted approvals root", () => {
    ...
    process.env.RECALL_HOME = linkedHome;

    saveExecApprovals({ version: 1, defaults: { security: "full" }, agents: {} });

    expect(
      fs.readFileSync(path.join(realHome, ".recall", "exec-approvals.json"), "utf8"),
  ...
  it("refuses to traverse symlinked approvals components below a symlinked home", () => {
    ...
    fs.symlinkSync(realHome, linkedHome, "dir");
    fs.symlinkSync(linkedStateTarget, path.join(realHome, ".recall"), "dir");
    process.env.RECALL_HOME = linkedHome;
