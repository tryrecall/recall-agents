...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.RECALL_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Recall";
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw = process.env.RECALL_MDNS_HOSTNAME?.trim() || "recall";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
