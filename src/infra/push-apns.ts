...
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "steelengine.exec-approval";
const PLUGIN_APPROVAL_NOTIFICATION_CATEGORY = "steelengine.plugin-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.STEELENGINE_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.STEELENGINE_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
  ...
  const inlineKeyRaw =
    normalizeNonEmptyString(env.STEELENGINE_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.STEELENGINE_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.STEELENGINE_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set STEELENGINE_APNS_PRIVATE_KEY_P8 or STEELENGINE_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading STEELENGINE_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
