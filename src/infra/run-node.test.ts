...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "recall.plugin.json");
...
const DIST_RUNTIME_EXTENSION_MANIFEST = "dist-runtime/extensions/demo/recall.plugin.json";
...
const DIST_RECALL_ALIAS_PACKAGE = "dist/extensions/node_modules/recall/package.json";
const DIST_RECALL_ALIAS_PLUGIN_SDK_INDEX =
  "dist/extensions/node_modules/recall/plugin-sdk/index.js";
...
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "recall.plugin.json");
...
async function writeRuntimePostBuildScaffold(tmp: string): Promise<void> {
  ...
  await writeProjectFiles(tmp, {
    ...
    [DIST_RECALL_ALIAS_PACKAGE]:
      '{"name":"recall","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js"}}\n',
  });
...
function statusCommandSpawn() {
  return [process.execPath, "recall.mjs", "status"];
}

function gatewayCallStatusCommandSpawn() {
  return [process.execPath, "recall.mjs", "gateway", "call", "status", "--json"];
}
...
describe("run-node script", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
  ...
  it("skips DTS generation only for launcher-triggered local builds", async () => {
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      ...
      expect(spawnCalls[2]?.args).toEqual(["recall.mjs", "status"]);
      expect(spawnCalls[0]?.env.RECALL_RUN_NODE_SKIP_DTS_BUILD).toBeUndefined();
      expect(spawnCalls[1]?.env.RECALL_RUN_NODE_SKIP_DTS_BUILD).toBe("1");
      expect(spawnCalls[2]?.env.RECALL_RUN_NODE_SKIP_DTS_BUILD).toBeUndefined();
    });
  });
    ...
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[], options?: unknown) => {
        ...
        return createPipedExitedProcess({
          stdout: args[0] === "recall.mjs" ? "child stdout\n" : "",
          stderr: args[0] === "recall.mjs" ? "child stderr\n" : "",
        });
      };
      const mutedStream = {
        write: () => true,
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...
          RECALL_FORCE_BUILD: "1",
          RECALL_RUNNER_LOG: "1",
          RECALL_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr: mutedStream,
        stdout: mutedStream,
      ...
      await expect(fs.readFile(outputPath, "utf-8")).resolves.toContain("[recall]");
      expect(spawnCalls.at(-1)?.args).toEqual(["recall.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.RECALL_RUN_NODE_OUTPUT_LOG).toBe(outputPath);
    ...
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[]) =>
        createPipedExitedProcess({
          stderr: args[0] === "recall.mjs" ? childStderr : "",
        });
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          RECALL_RUNNER_LOG: "0",
          RECALL_RUN_NODE_FILTER_SYNC_IO_STDERR: "1",
          RECALL_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr,
  ...
  it("adds Node CPU profiling flags to the launched Recall child when requested", async () => {
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          RECALL_RUNNER_LOG: "0",
          RECALL_RUN_NODE_CPU_PROF_DIR: ".artifacts/profiles",
        },
        spawn,
        spawnSync,
        execPath: process.execPath,
      ...
      expect(childArgs[2]).toMatch(
        /^--cpu-prof-name=recall-status-4242-\d{4}-\d{2}-\d{2}T.*\.cpuprofile$/,
      );
      expect(childArgs.slice(3)).toEqual(["recall.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.RECALL_RUN_NODE_CPU_PROF_DIR).toBe(profileDir);
  ...
  it("rotates old Node CPU profiles when a retention cap is set", async () => {
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      ...
      const oldProfiles = [
        "recall-status-oldest.cpuprofile",
        "recall-status-middle.cpuprofile",
        "recall-status-newest.cpuprofile",
      ];
      ...
      fsSync.writeFileSync(path.join(profileDir, "recall-models-old.cpuprofile"), "{}");
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...
          RECALL_RUNNER_LOG: "0",
          RECALL_RUN_NODE_CPU_PROF_DIR: ".artifacts/profiles",
          RECALL_RUN_NODE_CPU_PROF_MAX_FILES: "2",
        },
        spawn,
        spawnSync,
        execPath: process.execPath,
      ...
      expect(fsSync.existsSync(path.join(profileDir, "recall-models-old.cpuprofile"))).toBe(true);
    });
  });

  it("adds Node sync I/O tracing flag to the launched Recall child when requested", async () => {
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
      ...
      expect(exitCode).toBe(0);
      expect(spawnCalls.at(-1)).toEqual(["--trace-sync-io", "recall.mjs", "gateway", "--force"]);
    });
  });
  ...
  it("does not mutate Matrix QA args when no generic output log is requested", async () => {
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      ...
      expect(exitCode).toBe(0);
      const childArgs = spawnCalls.at(-1)?.args ?? [];
      expect(childArgs).toEqual(["recall.mjs", "qa", "matrix"]);
      expect(spawnCalls.at(-1)?.env.RECALL_RUN_NODE_OUTPUT_LOG).toBeUndefined();
    });
  });
  ...
  it("passes the synthesized private QA env into runtime postbuild staging", async () => {
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      ...
      expect(postBuildParams?.env?.RECALL_BUILD_PRIVATE_QA).toBe("1");
      expect(postBuildParams?.env?.RECALL_ENABLE_PRIVATE_QA_CLI).toBe("1");
      expect(postBuildParams?.env?.RECALL_DISABLE_BUNDLED_PLUGINS).toBe("0");
    });
  });
  ...
  it("preserves an explicit bundled plugin disable flag for QA runs", async () => {
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      ...
      const exitCode = await runQaCommand({
        ...
        spawnSync,
        runRuntimePostBuild,
        env: { RECALL_DISABLE_BUNDLED_PLUGINS: "1" },
      });
      ...
      const postBuildParams = firstMockCall(runRuntimePostBuild)?.[0] as
        | { cwd?: string; env?: Record<string, string | undefined> }
        | undefined;
      expect(postBuildParams?.env?.RECALL_DISABLE_BUNDLED_PLUGINS).toBe("1");
    });
  });
  ...
  it("reruns runtime postbuild in watch mode when required outputs are missing with no runtime stamp", async () => {
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      ...
      await fs.rm(resolvePath(tmp, DIST_RECALL_ALIAS_PACKAGE));
  ...
  it("reruns runtime postbuild for dirty extension package metadata in watch mode", async () => {
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [EXTENSION_PACKAGE]: '{"recall":{"extensions":["./index.ts"]}}\n',
    ...
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      ...
      const runs = Promise.all([
        runStatusCommand({
          ...
          spawnSync,
          env: {
            RECALL_RUN_NODE_BUILD_LOCK_POLL_MS: "1",
          },
          runRuntimePostBuild,
        }),
        runStatusCommand({
  ...
  it("forwards wrapper SIGTERM to the active recall child and returns 143", async () => {
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
      ...
      expect(spawnCall?.[1]).toEqual(["recall.mjs", "status"]);
    ...
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [EXTENSION_INDEX]: "export default {};\n",
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"name":"demo","recall":{"extensions":["./index.ts"]}}\n',
          [ROOT_TSDOWN]: "export default {};\n",
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_PACKAGE]: '{"name":"demo","recall":{"extensions":["./stale.js"]}}\n',
        },
    ...
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"recall":{"extensions":["./src/index.ts","./src/extra.ts"]}}\n',
    ...
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_SRC]: "export default {};\n",
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"recall":{"extensions":["./src/index.ts"]}}\n',
        },
        buildPaths: [
          ROOT_SRC,
          EXTENSION_SRC,
    ...
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_PACKAGE]: '{"recall":{"extensions":["./src/index.js"]}}\n',
          [DIST_EXTENSION_RUNTIME_SRC]: "export default {};\n",
          [DIST_RUNTIME_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_RUNTIME_EXTENSION_PACKAGE]: '{"recall":{"extensions":["./src/index.js"]}}\n',
          [RUNTIME_POSTBUILD_STAMP]: '{"head":"abc123"}\n',
        },
        buildPaths: [
          ROOT_SRC,
    ...
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_EXTENSION_PACKAGE]: '{"recall":{"extensions":["./index.js"]}}\n',
          [DIST_RUNTIME_EXTENSION_INDEX]: "export default {};\n",
          [DIST_RUNTIME_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_RUNTIME_EXTENSION_PACKAGE]: '{"recall":{"extensions":["./index.js"]}}\n',
          [RUNTIME_POSTBUILD_STAMP]: '{"head":"abc123"}\n',
        },
        buildPaths: [
          ROOT_SRC,
  ...
  it("does not require Recall SDK alias outputs when dist extensions are absent", async () => {
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
  ...
  it("reports missing Recall SDK alias outputs when runtime stamps match HEAD", async () => {
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          ...
          [DIST_RECALL_ALIAS_PACKAGE]:
            '{"name":"recall","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js"}}\n',
          [DIST_RECALL_ALIAS_PLUGIN_SDK_INDEX]:
      ...
      await fs.rm(resolvePath(tmp, DIST_RECALL_ALIAS_PLUGIN_SDK_INDEX));
    ...
    await withTempDir({ prefix: "recall-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [DIFFS_PACKAGE]:
            '{"recall":{"build":{"staticAssets":[{"source":"./assets/viewer-runtime.js","output":"assets/viewer-runtime.js"}]}}}\n',
