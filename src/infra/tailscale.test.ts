...
describe("tailscale helpers", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["STEELENGINE_TEST_TAILSCALE_BINARY", "NODE_ENV", "VITEST"]);
    process.env.STEELENGINE_TEST_TAILSCALE_BINARY = "tailscale";
  ...
  it("enableTailscaleServe passes a configured service name", async () => {
    ...
    await enableTailscaleServe(3000, exec as never, "svc:steelengine");
    ...
    expectExecCall(
      ...
      1,
      tailscaleBin,
      ["serve", "--service=svc:steelengine", "--bg", "--yes", "3000"],
      {
        maxBuffer: 200_000,
        timeoutMs: 15_000,
      },
  ...
  it("disableTailscaleServe disables only the configured service name", async () => {
    ...
    await disableTailscaleServe(exec as never, "svc:steelengine");

    expect(exec).toHaveBeenCalledTimes(1);
    expectExecCall(exec, 1, tailscaleBin, ["serve", "clear", "svc:steelengine"], {
      maxBuffer: 200_000,
      timeoutMs: 15_000,
    });
  });
