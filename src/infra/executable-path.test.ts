...
describe("executable path helpers", () => {
  it("detects regular files and rejects directories", async () => {
    await withTempDir({ prefix: "steelengine-exec-path-" }, async (base) => {
  ...
  it("does not treat drive-less rooted windows paths as cwd-relative executables", () => {
    ...
    expect(
      resolveExecutablePath(String.raw`:\Users\demo\AI\system\steelengine\git.exe`, {
        cwd: String.raw`C:\Users\demo\AI\system\steelengine`,
      }),
    ).toBeUndefined();
    expect(
      resolveExecutablePath(String.raw`:/Users/demo/AI/system/steelengine/git.exe`, {
