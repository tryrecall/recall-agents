// Resolves the SteelEngine package root from runtime and package metadata.
import path from "node:path";
import { fileURLToPath } from "node:url";
import { steelEngineRootFs, steelEngineRootFsSync } from "./steelengine-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["steelengine"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  ...
  try {
    const name = parsePackageName(await steelEngineRootFs.readFile(packageJsonPath, "utf-8"));
...
function readPackageNameSync(dir: string): string | null {
  ...
  try {
    const name = parsePackageName(steelEngineRootFsSync.readFileSync(packageJsonPath, "utf-8"));
    packageNameCache.set(packageJsonPath, name);
    return name;
  } catch {
    packageNameCache.set(packageJsonPath, null);
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  // the target so a launcher nested under another SteelEngine checkout keeps its own package root.
  try {
    const resolved = steelEngineRootFsSync.realpathSync(normalized);
...
export async function resolveSteelEnginePackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
// Every distinct SteelEngine package root among the runtime hints, in candidate order (symlinked
...
export function resolveSteelEnginePackageRootsSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string[] {
...
export function resolveSteelEnginePackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
  return resolveSteelEnginePackageRootsSync(opts)[0] ?? null;
}
