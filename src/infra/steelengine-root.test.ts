// Covers SteelEngine package root resolution.
...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__steelengine_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "steelengine-root");
...
function setPackageRoot(root: string, name = "steelengine") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./steelengine-root.js").resolveSteelEnginePackageRootSync,
  asyncResolver: typeof import("./steelengine-root.js").resolveSteelEnginePackageRoot,
  opts: Parameters<typeof import("./steelengine-root.js").resolveSteelEnginePackageRootSync>[0],
...
vi.mock("./steelengine-root.fs.runtime.js", () => ({
  steelEngineRootFsSync: mockFsModule(),
  steelEngineRootFs: mockFsPromisesModule(),
}));

describe("resolveSteelEnginePackageRoot", () => {
  let resolveSteelEnginePackageRoot: typeof import("./steelengine-root.js").resolveSteelEnginePackageRoot;
  let resolveSteelEnginePackageRootSync: typeof import("./steelengine-root.js").resolveSteelEnginePackageRootSync;

  beforeAll(async () => {
    ({ resolveSteelEnginePackageRoot, resolveSteelEnginePackageRootSync } =
      await import("./steelengine-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "steelengine");
        const pkgRoot = path.join(project, "node_modules", "steelengine");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "steelengine");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "steelengine.mjs")));
        setPackageRoot(realPkg);
        return { opts: { argv1: bin }, expected: realPkg };
      },
    },
    {
      name: "prefers a symlink target nested under another steelengine package",
      setup: () => {
        const sourceRoot = fx("nested-symlink-scenario");
        const bin = path.join(sourceRoot, ".artifacts", "prefix", "bin", "steelengine");
        ...
        state.realpaths.set(abs(bin), abs(path.join(installedRoot, "steelengine.mjs")));
    ...
    {
      name: "falls through from a non-steelengine moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-steelengine");
    ...
    {
      name: "returns null for non-steelengine package roots",
      setup: () => {
        const pkgRoot = fx("not-steelengine");
        setPackageRoot(pkgRoot, "not-steelengine");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "steelengine.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "steelengine");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    {
      name: "still resolves the steelengine package below a node_modules boundary",
      setup: () => {
