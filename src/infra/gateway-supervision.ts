...
export function formatExternalSupervisorActionRequired(action: string): string {
  return [
    `SteelEngine gateway lifecycle is managed by an external supervisor (${GATEWAY_SUPERVISOR_MODE_ENV}=external).`,
...
export function formatExternalSupervisorUpdateRequired(): string {
  return [
    `SteelEngine self-update is disabled while gateway lifecycle is managed by an external supervisor (${GATEWAY_SUPERVISOR_MODE_ENV}=external).`,
