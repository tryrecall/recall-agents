...
describe("runGatewayUpdate", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-update-"));
  });
  ...
  function createGlobalNpmUpdateRunner(params: {
    ...
    const baseInstallKey = "npm i -g recall@latest --no-fund --no-audit --loglevel=error";
    const omitOptionalInstallKey =
      "npm i -g recall@latest --omit=optional --no-fund --no-audit --loglevel=error";
  ...
  it("falls back to npm when pnpm is unavailable for git installs", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "recall", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
    const uiIndexPath = path.join(tempDir, "dist", "control-ui", "index.html");
    await fs.mkdir(path.dirname(uiIndexPath), { recursive: true });
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (
        key === `${process.execPath} ${path.join(tempDir, "recall.mjs")} doctor --non-interactive`
      ) {
        return { stdout: "", stderr: "", code: 0 };
      }
      return { stdout: "", stderr: "", code: 0 };
  ...
  it.each([
    {
      ...
      expectedInstallCommand: "npm i -g recall@latest --no-fund --no-audit --loglevel=error",
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: "npm i -g recall@beta --no-fund --no-audit --loglevel=error",
  ...
  it("updates global npm installs from the GitHub main package spec", async () => {
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand:
        "npm i -g github:recall/recall#main --no-fund --no-audit --loglevel=error",
      tag: "main",
    });
  ...
  it("falls back to global npm update when git is missing from PATH", async () => {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
      installCommand: "npm i -g recall@latest --no-fund --no-audit --loglevel=error",
      gitRootMode: "missing",
    ...
    expect(result.status).toBe("ok");
    expect(result.mode).toBe("npm");
    expect(calls).toContain("npm i -g recall@latest --no-fund --no-audit --loglevel=error");
  });
  ...
  it("uses RECALL_UPDATE_PACKAGE_SPEC for global package updates", async () => {
    ...
    const expectedInstallCommand =
      "npm i -g http://10.211.55.2:8138/recall-next.tgz --no-fund --no-audit --loglevel=error";
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
  ...
  it("updates global bun installs when detected", async () => {
    ...
    await withEnvAsync({ BUN_INSTALL: bunInstall }, async () => {
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g recall@latest")).toBe(true);
    });
  });
  ...
  it("fails with a clear reason when recall.mjs is missing", async () => {
    ...
    expect(result.steps.at(-1)?.name).toBe("recall doctor entry");
  });
