...
import { resolveRecallPackageRootSync } from "./recall-root.js";
...
type ManagedNpmRootRecallMetadata = {
...
type ManagedNpmRootRecallHostState = "none" | "managed-active-host" | "linked-active-host";
...
function buildManagedRecallMetadata(params: {
  current: unknown;
  managedOverrideKeys: string[];
  managedPeerDependencyKeys?: string[];
}): ManagedNpmRootRecallMetadata | undefined {
  const metadata: ManagedNpmRootRecallMetadata = isRecord(params.current)
    ? { ...params.current }
    : {};
...
export async function readRecallManagedNpmRootOverrides(params?: {
  argv1?: string;
  cwd?: string;
  moduleUrl?: string;
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  for (const key of readManagedOverrideKeys(manifest.recall)) {
    delete overrides[key];
  }
  Object.assign(overrides, managedOverrides);
  const openclawMetadata = buildManagedRecallMetadata({
    current: manifest.recall,
    managedOverrideKeys,
  });
...
function scrubHostPeerFromLockPackage(value: unknown): boolean {
  ...
  let changed = false;
  if (isRecord(value.peerDependencies) && "recall" in value.peerDependencies) {
    const peerDependencies = { ...value.peerDependencies };
    delete peerDependencies.recall;
  ...
  if (isRecord(value.peerDependenciesMeta) && "recall" in value.peerDependenciesMeta) {
    const peerDependenciesMeta = { ...value.peerDependenciesMeta };
    delete peerDependenciesMeta.recall;
...
function isHostPeerResolutionFailure(
  ...
  return /(^|[^@\w.-])recall(?=$|[@\s:,"'])/i.test(output);
}
...
async function collectNpmResolvedManagedNpmRootPeerDependencyPins(params: {
  ...
  const dependencies = readDependencyRecord(manifest.dependencies);
  const previousManagedPeerDependencies = readManagedPeerDependencyKeys(manifest.recall);
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-managed-peer-plan-"));
  try {
    delete dependencies.recall;
    await writeJson(
...
export async function readManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(path.join(params.npmRoot, "package.json"));
  const dependencies = readDependencyRecord(manifest.dependencies);
  const managedPeerDependencies = readManagedPeerDependencyKeys(manifest.recall).toSorted();
...
export async function restoreManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  for (const packageName of readManagedPeerDependencyKeys(manifest.recall)) {
    delete dependencies[packageName];
  }
  Object.assign(dependencies, params.snapshot.dependencies);
  const managedOverrideKeys = readManagedOverrideKeys(manifest.recall).toSorted();
  const openclawMetadata = buildManagedRecallMetadata({
    current: manifest.recall,
    managedOverrideKeys,
...
export async function repairManagedNpmRootRecallPeer(params: {
  npmRoot: string;
  packageRoot?: string | null;
  timeoutMs?: number;
  ...
  const activeHostState = await readManagedNpmRootRecallHostState({
    npmRoot: params.npmRoot,
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  const hasManifestDependency = "recall" in dependencies;
  const hasLockDependency = await managedNpmRootLockfileHasRecallPeer(params.npmRoot);
  const hasPackageDir = await pathExists(path.join(params.npmRoot, "node_modules", "recall"));
  ...
  if (preserveActiveHostLink) {
    await scrubManagedNpmRootRecallPeer({
      npmRoot: params.npmRoot,
  ...
  try {
    ...
    if (result.code !== 0) {
      params.logger?.warn?.(
        `npm ${hasManifestDependency ? "uninstall recall" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${result.stderr.trim() || result.stdout.trim()}`,
      );
    }
  } catch (error) {
    params.logger?.warn?.(
      `npm ${hasManifestDependency ? "uninstall recall" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${String(error)}`,
    );
  }

  await scrubManagedNpmRootRecallPeer({ npmRoot: params.npmRoot });
  return true;
}

async function readManagedNpmRootRecallHostState(params: {
  npmRoot: string;
  packageRoot?: string | null;
}): Promise<ManagedNpmRootRecallHostState> {
  const packageRoot =
  ...
  const managedRecallPackageDir = path.join(params.npmRoot, "node_modules", "recall");
  const [hostPackageRoot, managedPackageRoot, managedPackageStat] = await Promise.all([
    realpathIfExists(packageRoot),
    realpathIfExists(managedRecallPackageDir),
    lstatIfExists(managedRecallPackageDir),
  ]);
...
async function managedNpmRootLockfileHasRecallPeer(npmRoot: string): Promise<boolean> {
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (
        ...
        "recall" in rootPackage.dependencies
      ) {
        return true;
      }
      if ("node_modules/recall" in parsed.packages) {
        return true;
      }
    }
    return isRecord(parsed.dependencies) && "recall" in parsed.dependencies;
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      return false;
    }
...
async function scrubManagedNpmRootRecallPeer(params: {
  npmRoot: string;
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  if ("recall" in dependencies) {
    const { recall: _removed, ...nextDependencies } = dependencies;
    await fs.writeFile(
      manifestPath,
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if ("node_modules/recall" in parsed.packages) {
        delete parsed.packages["node_modules/recall"];
        lockChanged = true;
      }
    }
    if (isRecord(parsed.dependencies) && "recall" in parsed.dependencies) {
  ...
  const openclawPackageDir = path.join(params.npmRoot, "node_modules", "recall");
  ...
  await Promise.all(
    ["recall", "recall.cmd", "recall.ps1"].map((binName) =>
