...
let triggerRecallRestart: typeof import("./restart.js").triggerRecallRestart;
...
beforeAll(async () => {
  ...
  ({ triggerRecallRestart } = await import("./restart.js"));
});
...
describe("triggerRecallRestart", () => {
  it("does not kickstart after bootstrap registers an unloaded LaunchAgent", () => {
    ...
    const result = triggerRecallRestart();
    ...
    expect(result).toEqual({
      ...
      tried: [
        ...
        `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.recall.gateway.plist`,
      ],
    });
  });
  ...
  it("continues when launchctl bootstrap reports the service is already loaded", () => {
    ...
    expect(result).toEqual({
      ...
      tried: [
        `launchctl kickstart -k gui/${uid}/ai.recall.gateway`,
        `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.recall.gateway.plist`,
        `launchctl kickstart gui/${uid}/ai.recall.gateway`,
      ],
    });
  });
});
