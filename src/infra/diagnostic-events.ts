...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __recallDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__recallDiagnosticEventsState) {
    globalStore.__recallDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__recallDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: RecallConfig): boolean {
