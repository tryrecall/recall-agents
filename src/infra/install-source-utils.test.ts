...
const TEMP_DIR_PREFIX = "steelengine-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: {
  ...
  const cwd = await createTempDir("steelengine-install-source-utils-");
  const archivePath = path.join(cwd, "steelengine-plugin-1.2.3.tgz");
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("steelengine-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
  ...
  it.each([
    {
      ...
      path: async () => "/tmp/does-not-exist-steelengine-archive.tgz",
...
describe("resolveNpmSpecMetadata", () => {
  ...
  it.each([
    ...
    const result = await resolveNpmSpecMetadata({ spec: "@steelengine/codex" });
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "@steelengine/codex",
        version: "2026.6.11",
        resolvedSpec: "@steelengine/codex@2026.6.11",
  ...
  it("selects the newest multi-version entry satisfying the requested range", async () => {
    ...
    await expect(resolveNpmSpecMetadata({ spec: "@steelengine/codex@^2026.6.0" })).resolves.toEqual({
      ok: true,
  ...
  it("uses the last multi-version entry when the selector is not a semver range", async () => {
    ...
    const result = await resolveNpmSpecMetadata({ spec: "@steelengine/codex@latest" });
  ...
  it("accepts metadata without an steelengine block", async () => {
    const { steelengine: _steelengine, ...withoutSteelEngine } = npmViewMetadata;
    mockPackCommandResult({ stdout: JSON.stringify(withoutSteelEngine) });
  ...
  it("reports which required metadata fields are missing", async () => {
    ...
    await expect(resolveNpmSpecMetadata({ spec: "@steelengine/codex" })).resolves.toEqual({
      ok: false,
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "steelengine-plugin@1.2.3",
          name: "steelengine-plugin",
          version: "1.2.3",
          filename: "steelengine-plugin-1.2.3.tgz",
    ...
    const result = await runPack("steelengine-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "steelengine-plugin",
        version: "1.2.3",
        resolvedSpec: "steelengine-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "steelengine-plugin@1.2.3", "--ignore-scripts", "--json"],
      {
        cwd,
        timeoutMs: 300_000,
        env: {
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "steelengine-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\nsteelengine-plugin-1.2.3.tgz\n",
    });
  ...
  it.each([
    ...
    },
    {
      ...
      stdout:
        'npm notice using cache\n[{"id":"steelengine-plugin@1.2.3","name":"steelengine-plugin","version":"1.2.3","integrity":"sha512-test-integrity","shasum":"abc123"}]\n',
      expectedMetadata: {
        name: "steelengine-plugin",
        version: "1.2.3",
        resolvedSpec: "steelengine-plugin@1.2.3",
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@steelengine/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@steelengine/whatsapp", cwd);
    expectPackError(result, [
      "Package not found on npm",
      "@steelengine/whatsapp",
      "docs.steelengine.ai/tools/plugin",
    ]);
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("steelengine-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "steelengine-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@steelengine/plugin-demo@2.0.0",
            filename: "steelengine-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@steelengine/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "steelengine-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@steelengine/plugin-demo@2.0.0",
      },
    });
  });
