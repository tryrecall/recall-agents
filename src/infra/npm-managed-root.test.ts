...
const fixtureRootTracker = createSuiteTempRootTracker({
  prefix: "steelengine-npm-managed-root-",
});
...
describe("managed npm root", () => {
  ...
  it("syncs SteelEngine-owned overrides without dropping unrelated local overrides", async () => {
    const npmRoot = await makeTempRoot();
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
  ...
  it("reads workspace pnpm overrides for managed plugin installs", async () => {
    ...
    await expect(readSteelEngineManagedNpmRootOverrides()).resolves.toEqual(expectedOverrides);
  });
  ...
  it("resolves npm override dependency references from the host package manifest", async () => {
    ...
    await expect(readSteelEngineManagedNpmRootOverrides({ packageRoot })).resolves.toEqual({
      "managed-runtime": "3.1024.0",
      nested: {
        "optional-runtime": "2.0.0",
    ...
    expect(
      resolveManagedNpmRootDependencySpec({
        parsedSpec: {
          ...
          raw: "@steelengine/discord@stable",
          selector: "stable",
          selectorKind: "tag",
      ...
      `${JSON.stringify(
        {
          lockfileVersion: 3,
          packages: {
            "node_modules/@steelengine/discord": {
              version: "2026.5.2",
              resolved: "https://registry.npmjs.org/@steelengine/discord/-/discord-2026.5.2.tgz",
    ...
    const runCommand = vi.fn(async (_args: string[], optionsOrTimeout: number | CommandOptions) => {
      ...
      if (runCommand.mock.calls.length === 1) {
        return {
          code: 1,
          stdout: "",
          stderr: "npm ERR! notarget No matching version found for steelengine@2026.5.99-beta.1",
          signal: null,
          killed: false,
          termination: "exit" as const,
        };
      ...
      `${JSON.stringify(
        {
          ...
          dependencies: {
            "@steelengine/discord": "2026.5.2",
            "@steelengine/voice-call": "2026.5.2",
          },
          devDependencies: {
            fixture: "1.0.0",
          },
  ...
  it("repairs stale managed steelengine peer state without dropping plugin packages", async () => {
    const npmRoot = await makeTempRoot();
    ...
    await fs.writeFile(
      path.join(npmRoot, "node_modules", "steelengine", "package.json"),
      `${JSON.stringify({ name: "steelengine", version: "2026.5.4" })}\n`,
    );
    await fs.mkdir(path.join(npmRoot, "node_modules", ".bin"), { recursive: true });
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "steelengine"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "steelengine.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "steelengine.ps1"), "ps1 shim");
    await fs.writeFile(
    ...
    await expect(repairManagedNpmRootSteelEnginePeer({ npmRoot, runCommand })).resolves.toBe(true);
    expect(runCommand).toHaveBeenCalledTimes(1);
    ...
    expect(lockfile.packages?.["node_modules/@steelengine/discord"]?.version).toBe("2026.5.4");
    expect(lockfile.dependencies?.steelengine).toBeUndefined();
    await expectPathMissing(path.join(npmRoot, "node_modules", "steelengine"));
    for (const binName of ["steelengine", "steelengine.cmd", "steelengine.ps1"]) {
  ...
  it("does not repair the active SteelEngine host package in a root-managed install", async () => {
    const npmRoot = await makeTempRoot();
      ...
      `${JSON.stringify(
        {
          ...
          dependencies: {
            ...
            "@xdarkicex/steelengine-memory-libravdb": "1.4.69",
          },
        },
        null,
        2,
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({ name: "steelengine", version: "2026.5.12-beta.6" })}\n`,
    );
  ...
  it("scrubs managed ownership metadata without deleting a linked active host package", async () => {
    const npmRoot = await makeTempRoot();
    const hostPackageRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-host-package-"));
    ...
    await fs.symlink(hostPackageRoot, path.join(npmRoot, "node_modules", "steelengine"), "dir");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "steelengine"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "steelengine.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "steelengine.ps1"), "ps1 shim");
    await fs.writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/@xdarkicex/steelengine-memory-libravdb": {
              version: "1.4.69",
            },
          },
          dependencies: {
    ...
    expect(runCommand).not.toHaveBeenCalled();
    await expect(fs.realpath(path.join(npmRoot, "node_modules", "steelengine"))).resolves.toBe(
    ...
    expect(lockfile.packages?.["node_modules/steelengine"]).toBeUndefined();
    expect(lockfile.packages?.["node_modules/@xdarkicex/steelengine-memory-libravdb"]?.version).toBe(
      "1.4.69",
    );
    expect(lockfile.dependencies?.steelengine).toBeUndefined();
    for (const binName of ["steelengine", "steelengine.cmd", "steelengine.ps1"]) {
