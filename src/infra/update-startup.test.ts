...
vi.mock("./recall-root.js", async () => {
  const actual = await vi.importActual<typeof import("./recall-root.js")>("./recall-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "recall-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    if (!loaded) {
      ({ resolveRecallPackageRoot } = await import("./recall-root.js"));
    ...
    vi.mocked(resolveRecallPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveRecallPackageRoot).mockResolvedValue("/opt/recall");
  ...
  it.each([
    ...
    expect(log.info).toHaveBeenCalledWith(
      `update available (latest): v2.0.0 (current v1.0.0). Run: ${formatCliCommand("recall update")}`,
    );
  ...
  it("honors RECALL_NO_AUTO_UPDATE for configured auto-updates", async () => {
    mockPackageUpdateStatus("beta", "2.0.0-beta.1");
    process.env.RECALL_NO_AUTO_UPDATE = "1";
    const log = { info: vi.fn() };
    const runAutoUpdate = createAutoUpdateSuccessMock();
    ...
    const disabledLogCall = log.info.mock.calls.find(
      ([message]) => message === "auto-update disabled by RECALL_NO_AUTO_UPDATE",
    );
    expect(disabledLogCall).toEqual([
      "auto-update disabled by RECALL_NO_AUTO_UPDATE",
      {
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/recall/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
