...
// Keep aligned with @recall/plugin-package-contract ExternalPluginCompatibility.
...
function normalizeBaseUrl(baseUrl?: string): string {
  const envValue =
    normalizeOptionalString(process.env.RECALL_CLAWHUB_URL) ||
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    normalizeOptionalString(process.env.RECALL_CLAWHUB_CONFIG_PATH) ||
...
export async function resolveClawHubAuthToken(): Promise<string | undefined> {
  const envToken =
    normalizeOptionalString(process.env.RECALL_CLAWHUB_TOKEN) ||
...
const RECALL_CALVER_STABLE_CORRECTION_PATTERN = /^[vV]?(\d{4}\.\d{1,2}\.\d{1,2})-\d+$/;

function normalizeCalVerCorrectionForPluginApi(pluginApiVersion: string): string {
  const match = RECALL_CALVER_STABLE_CORRECTION_PATTERN.exec(pluginApiVersion.trim());
...
export async function downloadClawHubPackageArchive(params: {
  ...
  if (params.artifact === "clawpack") {
    ...
    const target = await createTempDownloadTarget({
      prefix: "recall-clawhub-clawpack",
