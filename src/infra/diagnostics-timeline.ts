...
const RECALL_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "recall.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.RECALL_DIAGNOSTICS)) &&
    typeof env.RECALL_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.RECALL_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: RECALL_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.RECALL_DIAGNOSTICS_RUN_ID ? { runId: env.RECALL_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.RECALL_DIAGNOSTICS_ENV ? { envName: env.RECALL_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.RECALL_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
