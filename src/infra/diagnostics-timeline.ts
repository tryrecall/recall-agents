...
const STEELENGINE_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "steelengine.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.STEELENGINE_DIAGNOSTICS)) &&
    typeof env.STEELENGINE_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.STEELENGINE_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: STEELENGINE_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.STEELENGINE_DIAGNOSTICS_RUN_ID ? { runId: env.STEELENGINE_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.STEELENGINE_DIAGNOSTICS_ENV ? { envName: env.STEELENGINE_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.STEELENGINE_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
