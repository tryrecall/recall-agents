...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/steelengine-state", "devices")).toEqual({
      dir: path.join("/tmp/steelengine-state", "devices"),
      pendingPath: path.join("/tmp/steelengine-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/steelengine-state", "devices", "paired.json"),
    });
  });
