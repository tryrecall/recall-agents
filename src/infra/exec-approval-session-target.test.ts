...
function resolveSlackPluginOriginTarget(params: { cfg: SteelEngineConfig; turnSourceTo: string }) {
...
describe("exec approval session target", () => {
  ...
  it("returns null for blank session keys, missing entries, and unresolved targets", async () => {
    await withTempDir({ prefix: "steelengine-exec-approval-session-target-" }, async (tmpDir) => {
