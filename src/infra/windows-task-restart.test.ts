...
const resolveTaskScriptPathMock = vi.hoisted(() =>
  vi.fn((env: Record<string, string | undefined>) => {
    ...
    return path.join(home, ".recall", "gateway.cmd");
  }),
);
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    expect(result.tried).toContain('schtasks /Run /TN "Recall Gateway (work)"');
    ...
    expect(script).toContain(
      'recall restart attempt source=windows-task-handoff target="Recall Gateway (work)"',
    );
    expect(script).toContain(
      `powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "(Get-ScheduledTask -TaskName 'Recall Gateway (work)' -ErrorAction SilentlyContinue).State" 2>nul | findstr /I /C:"Running" >nul 2>&1`,
    );
    expect(script).toContain('schtasks /Run /TN "Recall Gateway (work)" >>');
    expect(script.indexOf("powershell.exe -NoProfile")).toBeLessThan(
      script.indexOf('schtasks /Run /TN "Recall Gateway (work)"'),
    );
  ...
  it("prefers RECALL_WINDOWS_TASK_NAME overrides", () => {
    ...
    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "Recall Gateway (custom)" >>');
  });
  ...
  it("escapes custom task names in the PowerShell running-task probe", () => {
    ...
    relaunchGatewayScheduledTask({
      RECALL_WINDOWS_TASK_NAME: "Recall Gateway (Bob's work)",
    });

    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain(
      "-Command \"(Get-ScheduledTask -TaskName 'Recall Gateway (Bob''s work)' -ErrorAction SilentlyContinue).State\"",
    );
  });
  ...
  it("includes startup fallback", () => {
    const taskScriptDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-state-"));
