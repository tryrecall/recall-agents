...
describe("resolvePreferredRecallTmpDir", () => {
  ...
  it("prefers /tmp/recall when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_RECALL_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/recall when /tmp/recall is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/recall when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/recall exists but is not writable", () => {
    const accessSync = vi.fn((target: string) => {
      if (target === POSIX_RECALL_TMP_DIR) {
  ...
  it("falls back when /tmp/recall is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/recall is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/recall is group/other writable", () => {
