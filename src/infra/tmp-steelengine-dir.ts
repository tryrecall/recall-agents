// Creates temporary SteelEngine directories for runtime scratch work.
import fs from "node:fs";
import { tmpdir as getOsTmpDir } from "node:os";
import path from "node:path";

/** Preferred shared SteelEngine temp root on POSIX systems when ownership and permissions are safe. */
export const POSIX_STEELENGINE_TMP_DIR = "/tmp/steelengine";
...
export type ResolvePreferredSteelEngineTmpDirOptions = {
...
/** Resolves a safe SteelEngine temp root, falling back to user-scoped os.tmpdir paths when needed. */
export function resolvePreferredSteelEngineTmpDir(
  options: ResolvePreferredSteelEngineTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "steelengine" : `steelengine-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[steelengine] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback SteelEngine temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback SteelEngine temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_STEELENGINE_TMP_DIR;
