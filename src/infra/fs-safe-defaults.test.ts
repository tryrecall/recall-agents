...
describe("fs-safe defaults", () => {
  afterEach(() => {
    ...
    delete process.env.RECALL_FS_SAFE_PYTHON_MODE;
  });

  it("disables the Python helper by default in Recall", async () => {
  ...
  it("honors the Recall-specific env mode override", async () => {
    process.env.RECALL_FS_SAFE_PYTHON_MODE = "auto";
