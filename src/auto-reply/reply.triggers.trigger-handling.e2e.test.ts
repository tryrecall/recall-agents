...
async function expectNextRunUsesTargetSession(
  ...
  const runParams = firstMockCallArg(params.runEmbeddedAgentMock, "embedded SteelEngine agent");
...
describe("trigger handling", () => {
    ...
    await withTempHome(async (home) => {
      ...
      for (const testCase of thinkCases) {
        ...
        if (testCase.assertPrompt) {
          const prompt =
            firstMockCallArg(runEmbeddedAgentMock, "embedded SteelEngine agent").prompt ?? "";
  ...
  it("resolves heartbeat model selection from overrides", async () => {
    await withTempHome(async (home) => {
      ...
      for (const testCase of modelCases) {
        ...
        const call = firstMockCallArg(runEmbeddedAgentMock, "embedded SteelEngine agent");
  ...
  it("compacts worker sessions via the agent session file", async () => {
    await withTempHome(async (home) => {
      ...
      const sessionFile = firstMockCallArg(
        getCompactEmbeddedAgentSessionMock(),
        "embedded SteelEngine compaction",
      ).sessionFile;
      if (typeof sessionFile !== "string") {
        throw new Error("expected embedded SteelEngine compaction sessionFile");
      }
  ...
  it("applies native model auth profile overrides to the target session", async () => {
    await withTempHome(async (home) => {
      ...
      runEmbeddedAgentMock.mockReset();
      const storePath = requireSessionStorePath(cfg);
      const authDir = join(home, ".steelengine", "agents", "main", "agent");
