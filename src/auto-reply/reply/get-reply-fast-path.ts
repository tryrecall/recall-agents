...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("recall.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("recall.reply.full-runtime");

type ReplyConfigWithMarker = RecallConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    env.RECALL_ALLOW_SLOW_REPLY_TESTS === "1" || env.RECALL_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function markCompleteReplyConfig<T extends RecallConfig>(
  config: T,
...
export function withFastReplyConfig<T extends RecallConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends RecallConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

function isCompleteReplyConfig(config: unknown): config is RecallConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set RECALL_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  ...
  return applyMergePatch(params.getRuntimeConfig(), configOverride) as RecallConfig;
}
