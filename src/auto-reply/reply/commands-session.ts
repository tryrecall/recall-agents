...
import { scheduleGatewaySigusr1Restart, triggerRecallRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Recall in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerRecallRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Recall via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
