...
const TEST_SESSION_STORE_PATH = path.join(
  os.tmpdir(),
  `recall-commands-subagents-info-${process.pid}.json`,
);

function buildCommandTestConfig(): RecallConfig {
  return {
    ...baseCommandTestConfig,
    session: {
...
function buildInfoContext(params: { cfg: RecallConfig; runs: object[]; restTokens: string[] }) {
  return {
    params: {
      cfg: params.cfg,
...
describe("subagents info", () => {
  ...
  it("sanitizes leaked task details in /subagents info", () => {
    ...
    expect(text).not.toContain("Recall runtime context (internal):");
