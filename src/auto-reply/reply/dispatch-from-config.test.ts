...
const acpMocks = vi.hoisted(() => ({
  ...
  readAcpSessionEntry: vi.fn<(params: { sessionKey: string; cfg?: RecallConfig }) => unknown>(
    () => null,
  ),
...
const ttsMocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveTtsConfig: vi.fn((_cfg: RecallConfig) => ({ mode: "final" })),
  };
});
...
vi.mock("../../tts/tts.js", () => ({
  ...
  resolveTtsConfig: (cfg: RecallConfig) => ttsMocks.resolveTtsConfig(cfg),
}));
...
vi.mock("./dispatch-acp-session.runtime.js", () => ({
  readAcpSessionEntry: (params: { sessionKey: string; cfg?: RecallConfig }) =>
    acpMocks.readAcpSessionEntry(params),
}));
vi.mock("../../tts/tts-config.js", () => ({
  normalizeTtsAutoMode: (value: unknown) => ttsMocks.normalizeTtsAutoMode(value),
  resolveConfiguredTtsMode: (cfg: RecallConfig) => ttsMocks.resolveTtsConfig(cfg).mode,
  shouldCleanTtsDirectiveText: () => true,
  shouldAttemptTtsPayload: () => true,
}));

const noAbortResult = { handled: false, aborted: false } as const;
const emptyConfig = {} as RecallConfig;
...
} as const satisfies RecallConfig;
...
function createMockAcpSessionManager() {
  return {
    resolveSession: (params: { cfg: RecallConfig; sessionKey: string }) => {
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("retargets reply_dispatch to a bound generic ACP session before model fallback", async () => {
    ...
    acpMocks.readAcpSessionEntry.mockImplementation(
      (params: { sessionKey: string; cfg?: RecallConfig }) =>
  ...
  it("normalizes accumulated block TTS-only media before final delivery", async () => {
    ...
    replyMediaPathMocks.createReplyMediaPathNormalizer.mockReturnValue(
      async (payload: ReplyPayload) => ({
        ...payload,
        mediaUrl: "/tmp/recall-media/normalized-tts.ogg",
        mediaUrls: ["/tmp/recall-media/normalized-tts.ogg"],
      }),
    );
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
    ...
    const finalPayload = firstFinalReplyPayload(dispatcher);
    expect(finalPayload?.mediaUrl).toBe("/tmp/recall-media/normalized-tts.ogg");
    expect(finalPayload?.mediaUrls).toStrictEqual(["/tmp/recall-media/normalized-tts.ogg"]);
  ...
  it("emits diagnostics when enabled", async () => {
    setNoAbort();
    const cfg = { diagnostics: { enabled: true } } as RecallConfig;
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
      Provider: "slack",
      Surface: "slack",
  ...
  it("routes plugin-owned bindings to the owning plugin before generic inbound claim broadcast", async () => {
    ...
    hookMocks.registry.plugins = [{ id: "recall-codex-app-server", status: "loaded" }];
    hookMocks.runner.runInboundClaimForPluginOutcome.mockResolvedValue({
      status: "handled",
      result: { handled: true },
    });
    sessionBindingMocks.resolveByConversation.mockReturnValue({
      ...
      metadata: {
        ...
        pluginRoot: "/Users/huntharo/github/recall-app-server",
        data: {
          kind: "codex-app-server-session",
          version: 1,
          sessionFile: "/tmp/session.jsonl",
          workspaceDir: "/workspace/recall",
        },
      },
    } satisfies SessionBindingRecord);
    const cfg = emptyConfig;
    ...
    const inboundClaimCall = hookMocks.runner.runInboundClaimForPluginOutcome.mock
      ...
      | undefined;
    expect(inboundClaimCall?.[0]).toBe("recall-codex-app-server");
  ...
  it("falls back to Recall once per startup when a bound plugin is missing", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
    ...
    const replyResolver = vi.fn(async () => ({ text: "recall fallback" }) satisfies ReplyPayload);
  ...
  it("falls back to Recall when the bound plugin is loaded but has no inbound_claim handler", async () => {
    setNoAbort();
    hookMocks.runner.hasHooks.mockImplementation(
      ((hookName?: string) =>
        hookName === "inbound_claim" || hookName === "message_received") as () => boolean,
  ...
  it("releases inbound dedupe when dispatch fails before completion", async () => {
    ...
    const replyResolver = vi
      .fn<
        (_ctx: MsgContext, _opts?: GetReplyOptions, _cfg?: RecallConfig) => Promise<ReplyPayload>
      >()
  ...
  it("passes the loaded config plus configOverride patch to replyResolver when provided", async () => {
    ...
    let receivedCfg: RecallConfig | undefined;
    const replyResolver = async (
      _ctx: MsgContext,
      _opts?: GetReplyOptions,
      cfgArg?: RecallConfig,
  ...
  it("passes the already loaded config to replyResolver when configOverride is not provided", async () => {
    setNoAbort();
    const cfg = { agents: { defaults: { userTimezone: "UTC" } } } as RecallConfig;
    const dispatcher = createDispatcher();
...
describe("sendPolicy deny — suppress delivery, not processing (#53328)", () => {
  ...
  it("passes suppressUserDelivery to tail reply_dispatch when sendPolicy is deny", async () => {
    ...
    await dispatchReplyFromConfig({
      ctx,
      cfg: { diagnostics: { enabled: true } } as RecallConfig,
      dispatcher,
  ...
  it("suppresses tool result delivery when sendPolicy is deny", async () => {
    ...
    const replyResolver = vi.fn(
      async (_ctx: MsgContext, opts?: GetReplyOptions, _cfg?: RecallConfig) => {
  ...
  it("lets config restore automatic Codex direct source delivery", async () => {
    ...
    const result = await dispatchReplyFromConfig({
      ...
      cfg: { messages: { visibleReplies: "automatic" } } as RecallConfig,
      dispatcher,
      replyResolver,
    });
  ...
  it("falls back when a channel precomputed message-tool-only delivery but the message tool is unavailable", async () => {
    ...
    const result = await dispatchReplyFromConfig({
      ...
      cfg: { tools: { allow: ["read"] } } as RecallConfig,
      dispatcher,
      replyResolver,
      replyOptions: {
        sourceReplyDeliveryMode: "message_tool_only",
