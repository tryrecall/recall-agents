...
import {
  ...
  resolveSteelEngineTrustedNpmPackageInstall,
...
> {
  ...
  if (clawhubSpec) {
    ...
    if (!result.ok) {
      ...
      if (result.code === CLAWHUB_INSTALL_ERROR_CODE.CLAWHUB_RISK_ACKNOWLEDGEMENT_REQUIRED) {
        return {
          ok: false,
          error: `${warningPrefix}${result.error} The /plugins chat command cannot acknowledge ClawHub risk; run the local steelengine plugins install command with --acknowledge-clawhub-risk from a trusted shell after reviewing the warning.`,
        };
      }
  ...
  const trustedNpmInstall = resolveSteelEngineTrustedNpmPackageInstall(npmSpec);
