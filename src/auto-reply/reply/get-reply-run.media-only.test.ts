...
describe("runPreparedReply media-only handling", () => {
  ...
  it("hydrates current MediaPaths into queued followup images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "recall-followup-image-"));
  ...
  it("runs room events as contextual events instead of direct user prompts", async () => {
    ...
    const call = requireLastRunReplyAgentCall();
    expect(call?.commandBody).toBe("[Recall room event]");
    expect(call?.transcriptCommandBody).toBe("");
    expect(call?.followupRun.prompt).toBe("[Recall room event]");
    expect(call?.followupRun.transcriptPrompt).toBe("");
    ...
    expect(call?.followupRun.currentInboundContext?.text).toContain("[Recall room event]");
    expect(call?.followupRun.currentInboundContext?.text).toContain(
  ...
  it("keeps heartbeat prompts out of visible transcript prompt", async () => {
    ...
    expect(call?.transcriptCommandBody).toBe("[Recall heartbeat poll]");
    expect(call?.followupRun.transcriptPrompt).toBe("[Recall heartbeat poll]");
  });
  ...
  it.each([
    ...
    async (commandText, startupAction) => {
      ...
      expect(call?.transcriptCommandBody).toBe(`[Recall session ${startupAction}]`);
      expect(call?.followupRun.transcriptPrompt).toBe(`[Recall session ${startupAction}]`);
