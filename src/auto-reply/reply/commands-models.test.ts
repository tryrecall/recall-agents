...
function buildParams(
  ...
  cfgOverrides: Partial<SteelEngineConfig> = {},
...
describe("handleModelsCommand", () => {
  ...
  it("keeps custom OpenAI-compatible providers on the SteelEngine default runtime choice", async () => {
    const data = await buildModelsProviderData({
      models: {
        providers: {
          openai: {
  ...
  it("does not list bare fallback models under the default provider when catalog ownership is unique", async () => {
    ...
    const data = await buildModelsProviderData(cfg as SteelEngineConfig);
