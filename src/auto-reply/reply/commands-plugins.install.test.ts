...
const workspaceHarness = createCommandWorkspaceHarness("recall-command-plugins-install-");
...
describe("handleCommands /plugins install", () => {
  ...
  it("installs from an explicit clawhub: spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.2.3",
      extensions: ["index.js"],
      packageName: "@recall/clawhub-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@recall/clawhub-demo",
    ...
    await withTempHome("recall-command-plugins-home-", async () => {
      ...
      const params = buildPluginsParams(
        "/plugins install clawhub:@recall/clawhub-demo@1.2.3",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
      ...
      expect(mockFirstObjectArg(installPluginFromClawHubMock).spec).toBe(
        "clawhub:@recall/clawhub-demo@1.2.3",
      );
      expectPersistedInstall("clawhub-demo", {
        source: "clawhub",
        spec: "clawhub:@recall/clawhub-demo@1.2.3",
  ...
  it("refuses plugin installs in Nix mode before package installer side effects", async () => {
    const previousNixMode = process.env.RECALL_NIX_MODE;
    ...
    try {
      await withTempHome("recall-command-plugins-home-", async () => {
        ...
        expect(result.reply?.text).toContain("RECALL_NIX_MODE=1");
        expect(result.reply?.text).toContain("nix-recall#quick-start");
      ...
      if (previousNixMode === undefined) {
        ...
        process.env.RECALL_NIX_MODE = previousNixMode;
      }
    }
  });
  ...
  it("treats /plugin add as an install alias", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.0.0",
      extensions: ["index.js"],
      packageName: "@recall/alias-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@recall/alias-demo",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        version: "1.0.0",
    ...
    await withTempHome("recall-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugin add clawhub:@recall/alias-demo@1.0.0",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
      ...
      expect(mockFirstObjectArg(installPluginFromClawHubMock).spec).toBe(
        "clawhub:@recall/alias-demo@1.0.0",
      );
    });
  });
  ...
  it("trusts catalog npm package installs with alternate selectors", async () => {
    ...
    await withTempHome("recall-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugins install @wecom/wecom-recall-plugin@latest",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
