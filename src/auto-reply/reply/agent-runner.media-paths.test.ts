...
describe("runReplyAgent media path normalization", () => {
  ...
  it("reuses the provided media context inside runAgentTurnWithFallback", async () => {
    // Regression test for steelengine/steelengine#68056.
  ...
  it("passes current inbound media paths as native SteelEngine images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-native-agent-media-"));
  ...
  it("does not pass recent history images as unlabeled native SteelEngine images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-native-agent-history-"));
    cleanupPaths.push(tmpDir);
  ...
  it("falls back to prompt refs instead of forwarding partial current media", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "steelengine-native-agent-partial-"));
    cleanupPaths.push(tmpDir);
