...
const mocks = vi.hoisted(() => ({
  ...
  triggerSteelEngineRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerSteelEngineRestart: mocks.triggerSteelEngineRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerSteelEngineRestart.mockReset();
    mocks.triggerSteelEngineRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(mocks.triggerSteelEngineRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerSteelEngineRestart).not.toHaveBeenCalled();
  ...
  it("clears the success sentinel when fallback restart fails", async () => {
    mocks.triggerSteelEngineRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
