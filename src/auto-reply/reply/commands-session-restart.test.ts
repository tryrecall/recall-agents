...
const mocks = vi.hoisted(() => ({
  ...
  formatDoctorNonInteractiveHint: vi.fn(() => "Run: recall doctor --non-interactive"),
  ...
  triggerRecallRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerRecallRestart: mocks.triggerRecallRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerRecallRestart.mockReset();
    mocks.triggerRecallRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(sentinelPayload?.doctorHint).toBe("Run: recall doctor --non-interactive");
    expect(sentinelPayload?.stats).toEqual({
      mode: "gateway.restart",
      reason: "/restart",
    });
    expect(mocks.triggerRecallRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerRecallRestart).not.toHaveBeenCalled();
  ...
  it("removes the success sentinel when fallback restart fails", async () => {
    mocks.triggerRecallRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
