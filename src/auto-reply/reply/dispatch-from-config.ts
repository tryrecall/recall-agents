...
import { isParentOwnedBackgroundAcpSession } from "@steelengine/acp-core/session-interaction-mode";
import {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("steelengine.dispatchFromConfigTestApi")] = {
...
async function dispatchReplyFromConfigInner(
  ...
  try {
    ...
    const replyConfig = withFullRuntimeReplyConfig(
      params.configOverride ? (applyMergePatch(cfg, params.configOverride) as SteelEngineConfig) : cfg,
    );
