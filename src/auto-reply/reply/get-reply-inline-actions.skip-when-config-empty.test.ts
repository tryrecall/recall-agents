...
vi.mock("../../agents/steelengine-tools.runtime.js", () => ({
  createSteelEngineTools: (...args: unknown[]) => createSteelEngineToolsMock(...args),
}));
...
function mockToolDispatchedSkillCommand() {
  ...
  createSteelEngineToolsMock.mockReturnValue([
    {
      name: "send_status",
      execute: toolExecute,
    },
...
describe("handleInlineActions", () => {
  beforeEach(() => {
    ...
    createSteelEngineToolsMock.mockReset();
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(
      mockObjectArg(createSteelEngineToolsMock, "createSteelEngineTools").requesterAgentIdOverride,
    ).toBe("named-worker");
  ...
  it("passes sender identity into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    const toolsArgs = mockObjectArg(createSteelEngineToolsMock, "createSteelEngineTools");
  ...
  it("applies subagent policy to ACP envelope inline dispatch sessions", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-inline-acp-policy-"));
    try {
  ...
  it("passes sandboxed runtime state into inline tool construction", async () => {
    ...
    expect(createSteelEngineToolsMock).toHaveBeenCalledWith(
