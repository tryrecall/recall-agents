...
describe("buildExportSessionReply", () => {
  ...
  it("injects scripts and session data through the real export template", async () => {
    ...
    expect(html).not.toContain("data-recall-export-placeholder");
  ...
  it("suffixes colliding default export filenames instead of overwriting", async () => {
    ...
    const expectedBase = path.join(
      "/tmp/workspace",
      "recall-session-session--2026-05-05T10-11-12.html",
    );
    const expectedSuffix = path.join(
      "/tmp/workspace",
      "recall-session-session--2026-05-05T10-11-12-2.html",
    );
    ...
    expect(reply.text).toContain("📄 File: recall-session-session--2026-05-05T10-11-12-2.html");
  });
  ...
  it("preserves replacement text with dollar sequences", async () => {
    hoisted.exportHtmlTemplateContents.set(
      "template.html",
      [
        '<style data-recall-export-placeholder="CSS"></style>',
        '<script id="session-data" type="application/json" data-recall-export-placeholder="SESSION_DATA"></script>',
        '<script data-recall-export-placeholder="MARKED_JS"></script>',
        '<script data-recall-export-placeholder="HIGHLIGHT_JS"></script>',
        '<script data-recall-export-placeholder="JS"></script>',
      ].join(""),
    );
