...
function makeWorkspace(): string {
  return tempDirs.make("steelengine-session-export-");
}
...
describe("writeSessionExportFile", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = makeWorkspace();
      const aliasParent = tempDirs.make("steelengine-session-export-alias-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = makeWorkspace();
      const outsideDir = tempDirs.make("steelengine-session-export-outside-");
