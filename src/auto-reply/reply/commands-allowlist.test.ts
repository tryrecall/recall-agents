...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-allowlist-config-"));
  const configPath = path.join(dir, "steelengine.json");
  const previous = process.env.STEELENGINE_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      setTestEnvValue("STEELENGINE_CONFIG_PATH", previous);
    }
        ...
        run: async () => {
          await withTempConfigPath(
            ...
            async (configPath) => {
              ...
              const written = await readJsonFile<SteelEngineConfig>(configPath);
