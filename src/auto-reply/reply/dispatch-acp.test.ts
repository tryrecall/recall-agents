...
const policyMocks = vi.hoisted(() => ({
  resolveAcpDispatchPolicyError: vi.fn<(cfg: RecallConfig) => AcpRuntimeError | null>(() => null),
  resolveAcpAgentPolicyError: vi.fn<(cfg: RecallConfig, agent: string) => AcpRuntimeError | null>(
    () => null,
  ),
}));
...
const sessionMetaMocks = vi.hoisted(() => ({
  readAcpSessionEntry: vi.fn<
    (params: { sessionKey: string; cfg?: RecallConfig }) => AcpSessionStoreEntry | null
  >(() => null),
}));
...
vi.mock("../../acp/policy.js", () => ({
  resolveAcpDispatchPolicyError: (cfg: RecallConfig) =>
    policyMocks.resolveAcpDispatchPolicyError(cfg),
  resolveAcpAgentPolicyError: (cfg: RecallConfig, agent: string) =>
...
function createAcpConfigWithVisibleToolTags(): RecallConfig {
...
describe("tryDispatchAcpReply", () => {
  ...
  it("preserves chat.send inline image attachments over recent history images", async () => {
    ...
    const historyPath = "/tmp/recall-history-inline.png";
  ...
  it("marks accumulated ACP block TTS finals as trusted local media for WebChat", async () => {
    ...
    ttsMocks.resolveTtsConfig.mockReturnValue({ mode: "final" });
    queueTtsReplies({
      mediaUrl: "/tmp/recall-media/acp-tts.ogg",
      audioAsVoice: true,
    } as MockTtsReply);
    ...
    expect(finalPayload.mediaUrl).toBe("/tmp/recall-media/acp-tts.ogg");
