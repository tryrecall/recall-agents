...
describe("runMemoryFlushIfNeeded", () => {
  ...
  beforeEach(async () => {
    rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-memory-unit-"));
  ...
  it("skips SteelEngine preflight compaction for explicit Codex runtime overrides", async () => {
    registerMemoryFlushPlanResolverForTest(() => ({
      softThresholdTokens: 4_000,
      forceFlushTranscriptBytes: 1_000_000_000,
      reserveTokensFloor: 0,
  ...
  it("keeps the OpenAI API context window for persisted SteelEngine runtime overrides", async () => {
    registerMemoryFlushPlanResolverForTest(() => ({
      softThresholdTokens: 4_000,
      forceFlushTranscriptBytes: 1_000_000_000,
      reserveTokensFloor: 0,
