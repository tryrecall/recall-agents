...
import { type FastMode, normalizeOptionalString } from "@steelengine/normalization-core/string-coerce";
import {
...
type AgentDefaults = NonNullable<SteelEngineConfig["agents"]>["defaults"];
...
export async function runPreparedReply(
  ...
  const useFastReplyRuntime = shouldUseReplyFastTestRuntime({
    cfg,
    isFastTestEnv: process.env.STEELENGINE_TEST_FAST === "1",
  });
  ...
  const userTurnInput =
    userTurnTranscriptText !== undefined || userTurnMediaForPersistence.length > 0
      ? {
          ...
          // replayed as history. See: https://github.com/steelengineai/recall-agents/issues/3658
