...
function requireResolveSessionAgentIdCall(index = 0) {
  ...
  )?.[0] as { sessionKey?: string; config?: SteelEngineConfig } | undefined;
  if (!call) {
...
function requireResolveAgentDirCall(index = 0) {
  const call = resolveAgentDirMock.mock.calls[index] as [SteelEngineConfig, string] | undefined;
  if (!call) {
...
describe("handleCompactCommand", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    ...
    const result = await handleCompactCommand(
      {
        ...buildCompactParams("/compact", {
          commands: { text: true },
          channels: { whatsapp: { allowFrom: ["*"] } },
          session: { store: "/tmp/steelengine-session-store.json" },
        } as SteelEngineConfig),
        ctx: {
          Provider: "whatsapp",
          Surface: "whatsapp",
        ...
        agentDir: "/tmp/steelengine-agent-compact",
    ...
    expect(call.agentDir).toBe("/tmp/steelengine-agent-compact");
