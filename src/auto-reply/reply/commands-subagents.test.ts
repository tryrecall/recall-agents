...
describe("subagents info", () => {
  const TEST_SESSION_STORE_PATH = path.join(
    os.tmpdir(),
    `steelengine-commands-subagents-info-${process.pid}.json`,
  );

  function buildCommandTestConfig(): SteelEngineConfig {
    return {
      ...baseCommandTestConfig,
      session: {
  ...
  function buildInfoContext(params: { cfg: SteelEngineConfig; runs: object[]; restTokens: string[] }) {
    return {
      params: {
        cfg: params.cfg,
  ...
  it("sanitizes leaked task details in /subagents info", () => {
    ...
    expect(text).not.toContain("SteelEngine runtime context (internal):");
