...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "recall-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop recall!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@recall_bot", { botUsername: "recall_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@recall_bot", { botUsername: "recall_bot" })).toBe(true);
