...
type HomeEnvSnapshot = {
  ...
  RECALL_STATE_DIR: string | undefined;
  RECALL_AGENT_DIR: string | undefined;
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".recall", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.RECALL_STATE_DIR = path.join(home, ".recall");
    process.env.RECALL_AGENT_DIR = path.join(home, ".recall", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".recall", "agent");
