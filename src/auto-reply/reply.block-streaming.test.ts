...
function createReplyConfig(home: string, streamMode?: "block"): RecallConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("recall-stream-", async (home) => {
  ...
  it("still parses media directives for direct block payloads", async () => {
    await withTempHome(async (home) => {
      ...
      expect(onBlockReply.mock.calls[0][0]).toMatchObject({
        text: "Result",
        mediaUrls: [path.join(home, "recall", "image.png")],
      });
    });
  });
});
