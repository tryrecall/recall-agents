import { isHttpsUrl, isHttpUrl } from "@steelengine/net-policy/url-protocol";
...
export const TailscaleServiceNameSchema = z
  ...
  .regex(/^svc:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/, {
    message:
      'Tailscale serviceName must use the "svc:<dns-label>" format, for example "svc:steelengine"',
  });
    ...
    if (Object.hasOwn(data, "disabled")) {
      ...
      const replacement =
        typeof disabled === "boolean"
          ? `"enabled: ${!disabled}" instead, then run "steelengine doctor --fix" to migrate existing config`
