...
describe("validateConfigObjectRawWithPlugins channel metadata", () => {
    ...
    if (!result.ok) {
      expect(result.issues).toContainEqual(
        expect.objectContaining({
          ...
          message:
            'invalid config for plugin steelengine-lark: must not have additional properties: "unsupportedField"',
        }),
      );
    }
  });
  ...
  it("sanitizes the schema owner in validation diagnostics", () => {
    const unsafeId = `steelengine${String.fromCharCode(10)}${String.fromCharCode(27)}[31m-lark`;
    const registry = createExternalFeishuSchemaRegistry();
  ...
  it("keeps raw channel validation diagnostics plugin-agnostic", () => {
    ...
    if (!result.ok) {
      ...
      expect(result.issues[0]?.message).not.toContain("steelengine doctor --fix");
    }
  });
});
