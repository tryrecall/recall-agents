...
import type { RecallConfig, ConfigValidationIssue } from "./types.js";
...
function materializeBundledModelProviderOverlays(config: RecallConfig): RecallConfig {
...
const SECRETREF_POLICY_DOC_URL = "https://docs.recall.ai/reference/secretref-credential-surface";
...
function formatMissingOfficialExternalPluginWarning(
  ...
  if (pluginId === "memory-lancedb" && opts?.selectedMissingMemorySlot) {
    return `plugin not installed: ${pluginId} — gateway will run without persistent memory until installed; install the official external plugin with: recall plugins install ${installSpec}`;
  }
  return `plugin not installed: ${pluginId} — install the official external plugin with: recall plugins install ${installSpec}`;
}
...
function collectRawBundledChannelConfigIssues(config: RecallConfig): ConfigValidationIssue[] {
...
function validateIdentityAvatar(config: RecallConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: RecallConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
...
): { ok: true; config: RecallConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = RecallSchema.safeParse(normalizedRaw);
  ...
  const validatedConfig = materializeBundledModelProviderOverlays(validated.data as RecallConfig);
  const channelIssues =
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: RecallConfig | null | undefined;
  ...
  const ensureCompatConfig = (): RecallConfig => {
  ...
  const validateWebSearchProvider = () => {
    ...
    if (installCatalogEntry) {
      const issue = {
        path,
        message: `web_search provider is not available: ${trimmed} (install or enable plugin "${installCatalogEntry.pluginId}", then run recall doctor --fix)`,
      ...
      if (hasPluginEvidenceForWebSearchProvider(trimmed, installCatalogEntry.pluginId)) {
        warnings.push({
          ...issue,
          message: `web_search provider is not available: ${trimmed} (configured plugin "${installCatalogEntry.pluginId}" is unavailable; Gateway will ignore this optional provider until the plugin is installed/enabled or recall doctor --fix repairs the config)`,
        });
        return;
      }
      issues.push(issue);
    ...
    if (hasStalePluginEvidenceForUnknownWebSearchProvider(trimmed)) {
      warnings.push({
        ...issue,
        message: `${issue.message} (stale web search plugin config ignored; run recall doctor --fix to remove stale config, or install the plugin)`,
      });
      return;
    }
    issues.push(issue);
    ...
    for (const key of Object.keys(config.channels)) {
      ...
      if (!allowedChannels.has(trimmed)) {
        ...
        if (hasStalePluginEvidenceForUnknownChannel(trimmed)) {
          warnings.push({
            ...issue,
            message: `${issue.message} (stale channel plugin config ignored; run recall doctor --fix to remove stale config, or install the plugin)`,
          });
        } else {
          issues.push(issue);
        }
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Recall config schema.
