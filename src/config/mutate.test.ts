...
type MockValidationResult =
  | { ok: true; config: SteelEngineConfig; warnings: MockValidationIssue[] }
...
const validationMocks = vi.hoisted(() => ({
  validateConfigObjectWithPlugins: vi.fn(
    (config: SteelEngineConfig): MockValidationResult => ({
      ok: true,
      config,
      warnings: [],
    }),
...
function createSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/steelengine.json",
    exists: true,
...
describe("config mutate helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "steelengine-config-mutate-" });
  const originalNixMode = process.env.STEELENGINE_NIX_MODE;
  ...
  afterAll(async () => {
    if (originalNixMode === undefined) {
      ...
      process.env.STEELENGINE_NIX_MODE = originalNixMode;
    }
  ...
  it("preserves config path ownership across transform retries", async () => {
    const initial = createSnapshot({
      hash: "hash-1",
      path: "/tmp/first-steelengine.json",
      sourceConfig: { agents: { list: [] } },
    });
    const fresh = createSnapshot({
      hash: "hash-2",
      path: "/tmp/second-steelengine.json",
      sourceConfig: { agents: { list: [] } },
    });
    ioMocks.readConfigFileSnapshotForWrite
      .mockResolvedValueOnce({
    ...
    const transform = vi.fn((config: SteelEngineConfig) => ({ nextConfig: config }));
  ...
  it("does not retry transform mutations after config path ownership changes", async () => {
    ...
    await expect(
      transformConfigFileWithRetry({
        transform(config) {
          activeConfigPath = "/tmp/second-steelengine.json";
  ...
  it("rejects replace attempts when the active config path changed", async () => {
    ...
    await expect(
      replaceConfigFile({
        baseHash: snapshot.hash,
        nextConfig: { gateway: { port: 19002 } },
        writeOptions: { expectedConfigPath: "/tmp/first-steelengine.json" },
      }),
    ).rejects.toThrow("config path changed since last load");
    expect(ioMocks.writeConfigFile).not.toHaveBeenCalled();
  });
  ...
  it("refuses mutate writes in Nix mode before touching disk", async () => {
    ...
    ).rejects.toThrow("SteelEngine Nix overview: https://docs.steelengine.ai/install/nix");
  ...
  it("repairs invalid config through a single-file top-level plugins include", async () => {
    ...
    const pluginsPath = path.join(home, ".steelengine", "config", "plugins.json5");
      ...
      `${JSON.stringify(
        {
          entries: {
            old: {
              enabled: true,
              config: { token: "${STEELENGINE_TEST_PLUGIN_TOKEN}" },
            },
          },
        },
        null,
    ...
    ioMocks.readConfigFileSnapshotForWrite
      .mockResolvedValueOnce({
        ...
        writeOptions: {
          ...
          envSnapshotForRestore: { STEELENGINE_TEST_PLUGIN_TOKEN: "plugin-token-runtime" },
    ...
    try {
      await replaceConfigFile({
        ...
        io: {
          env: { STEELENGINE_TEST_PLUGIN_TOKEN: "plugin-token-after-read" },
    ...
    expect(persistedPlugins.entries?.old?.config?.token).toBe("${STEELENGINE_TEST_PLUGIN_TOKEN}");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const linkPath = path.join(home, ".steelengine", "link");
  ...
  it("rejects direct mutations to external include roots", async () => {
    ...
    await expect(
      replaceConfigFile({
        ...
        nextConfig,
        io: {
          env: { STEELENGINE_INCLUDE_ROOTS: "~/shared" },
          readConfigFileSnapshotForWrite: ioMocks.readConfigFileSnapshotForWrite,
          writeConfigFile: ioMocks.writeConfigFile,
        },
      }),
  ...
  it("preserves auth-store refresh scope for managed top-level include writes", async () => {
    ...
    const preflight = vi.fn(
      async (sourceConfig: SteelEngineConfig, refreshOptions?: { includeAuthStoreRefs?: boolean }) => {
  ...
  it("uses the published restart env source for isolated managed include writes", async () => {
    ...
    const configPath = path.join(home, ".steelengine", "steelengine.json");
    const envPath = path.join(home, ".steelengine", "config", "env.json5");
    const envKey = "OC";
    ...
    let preflightSource: SteelEngineConfig | undefined;
  ...
  it("does not write an include after the active config path changes during preflight", async () => {
    ...
    const firstConfigPath = path.join(home, "first", "steelengine.json");
    const secondConfigPath = path.join(home, "second", "steelengine.json");
  ...
  it("rolls back an include write when config path ownership changes during commit", async () => {
    ...
    const assertConfigPathForWrite = () => {
      if (fsNode.readFileSync(pluginsPath, "utf-8") !== initialPluginsRaw) {
        activeConfigPath = "/tmp/other-steelengine.json";
      }
  ...
  it.each(["active path", "refreshed snapshot path"] as const)(
    ...
    async (changeKind) => {
      ...
      const configPath = path.join(home, "first", "steelengine.json");
      const otherConfigPath = path.join(home, "second", "steelengine.json");
      const pluginsPath = path.join(home, "first", "plugins.json5");
      const rootConfig = { plugins: { $include: "./plugins.json5" } };
      const initialPluginsRaw = `${JSON.stringify({ entries: {} }, null, 2)}\n`;
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const includeDir = path.join(home, ".steelengine", "config");
      const movedIncludeDir = path.join(home, ".steelengine", "config-original");
  ...
  it("preflights the restored include payload with the current environment", async () => {
    ...
    const initialPluginsRaw = `${JSON.stringify(
      {
        entries: {
          old: { enabled: true, config: { token: "${STEELENGINE_TEST_INCLUDE_TOKEN}" } },
        },
      },
      null,
      2,
    ...
    const observedSources: SteelEngineConfig[] = [];
    ...
    try {
      ...
      await expect(
        replaceConfigFile({
          ...
          writeOptions: {
            expectedConfigPath: snapshot.path,
            envSnapshotForRestore: { STEELENGINE_TEST_INCLUDE_TOKEN: "old-token" },
            assertConfigPathForWrite: allowConfigPathWrite,
            includeFileTargetsForWrite: { [pluginsPath]: await resolveIncludeTarget(pluginsPath) },
          },
          nextConfig: {
          ...
          io: {
            env: { STEELENGINE_TEST_INCLUDE_TOKEN: "new-token" },
            readConfigFileSnapshotForWrite: ioMocks.readConfigFileSnapshotForWrite,
            writeConfigFile: ioMocks.writeConfigFile,
          },
        }),
  ...
  it("rolls back single-file top-level include writes when runtime refresh fails", async () => {
    ...
    const envKey = "STEELENGINE_TEST_INCLUDE_ROLLBACK_ENV";
    await fs.mkdir(path.dirname(pluginsPath), { recursive: true });
    await fs.writeFile(
      configPath,
      `${JSON.stringify({ plugins: { $include: "./config/plugins.json5" } }, null, 2)}\n`,
  ...
  it("preflights injected root writers before persisting", async () => {
    ...
    const configPath = path.join(home, ".steelengine", "steelengine.json");
    await fs.mkdir(path.dirname(configPath), { recursive: true });
    const initialConfig = { gateway: { mode: "local" } } satisfies SteelEngineConfig;
    ...
    const injectedWrite = vi.fn(async (config: SteelEngineConfig, options?: ConfigWriteOptions) => {
