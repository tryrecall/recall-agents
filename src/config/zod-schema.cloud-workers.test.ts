...
function parseCloudWorkers(value: unknown) {
  const result = SteelEngineSchema.safeParse({ cloudWorkers: value });
  if (!result.success) {
...
describe("SteelEngineSchema cloudWorkers config", () => {
  it("is absent by default and accepts an empty opt-in block", () => {
    expect(SteelEngineSchema.parse({}).cloudWorkers).toBeUndefined();
  ...
  it.each([
    ...
    expect(SteelEngineSchema.safeParse({ cloudWorkers }).success).toBe(false);
  });
});
