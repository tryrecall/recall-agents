...
describe("config io paths", () => {
  ...
  it("uses ~/.steelengine/steelengine.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".steelengine", 19001);
      const io = createIoForHome(home);
      expect(io.configPath).toBe(configPath);
    });
  });

  it("defaults to ~/.steelengine/steelengine.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".steelengine", "steelengine.json"));
    });
  });

  it("uses STEELENGINE_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { STEELENGINE_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".steelengine", "steelengine.json"));
    });
  });

  it("honors explicit STEELENGINE_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".steelengine", 20002, "custom.json");
      const io = createIoForHome(home, { STEELENGINE_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("explains what to check when config was written by a newer SteelEngine", async () => {
    await withTempHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        [
          `Your SteelEngine config was written by version 9999.1.1, but this command is running ${VERSION}.`,
  ...
  it("does not warn about newer config during internal update handoff reads", async () => {
    await withTempHome(async (home) => {
      ...
      const io = createConfigIO({
        configPath,
        env: { HOME: home, STEELENGINE_UPDATE_POST_CORE: "1" } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger,
      });
      io.loadConfig();
