// Defines the top-level SteelEngine configuration type.
...
/** Top-level SteelEngine config as read from user/project config files. */
export type SteelEngineConfig = {
  /** JSON schema URL used by editors and generated config files. */
  $schema?: string;
  meta?: {
    /** Last SteelEngine version that wrote this config. */
  ...
  wizard?: {
    ...
    lastRunAt?: string;
    /** SteelEngine version used by the last completed wizard run. */
  ...
  ui?: {
    /** Accent color for SteelEngine UI chrome (hex). */
    seamColor?: string;
    assistant?: {
...
export type SteelEngineConfigInput = Omit<SteelEngineConfig, "models"> & {
  models?: ModelsConfigInput;
};

declare const steelEngineConfigStateBrand: unique symbol;

type BrandedConfigState<TState extends string> = SteelEngineConfig & {
  readonly [steelEngineConfigStateBrand]?: TState;
};
