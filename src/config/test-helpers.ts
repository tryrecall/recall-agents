...
import { resetConfigRuntimeState, type RecallConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        RECALL_MPM_CATALOG_PATHS: undefined,
        RECALL_LOAD_SHELL_ENV: undefined,
        RECALL_DEFER_SHELL_ENV_FALLBACK: undefined,
        RECALL_SHELL_ENV_TIMEOUT_MS: undefined,
...
export async function writeRecallConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.RECALL_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected RECALL_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeRecallConfig(home, config);
