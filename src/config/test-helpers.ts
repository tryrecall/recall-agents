...
import { resetConfigRuntimeState, type SteelEngineConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        STEELENGINE_MPM_CATALOG_PATHS: undefined,
...
export async function writeSteelEngineConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.STEELENGINE_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected STEELENGINE_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeSteelEngineConfig(home, config);
