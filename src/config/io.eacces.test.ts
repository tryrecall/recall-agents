...
describe("config io EACCES handling", () => {
  it("returns a helpful error message when config file is not readable (EACCES)", async () => {
    const configPath = "/data/.steelengine/steelengine.json";
    const errors: string[] = [];
  ...
  it("includes configPath in the chown hint for the correct remediation command", async () => {
    const configPath = "/home/myuser/.steelengine/steelengine.json";
    const io = createConfigIO({
      configPath,
      fs: makeEaccesFs(configPath),
      logger: { error: () => {}, warn: () => {} },
...
describe("config write guard after unreadable config", () => {
  ...
  it.each([
    ...
    async ({ writeOptions }) => {
      const home = fsNode.mkdtempSync(path.join(os.tmpdir(), "steelengine-unreadable-"));
      tempRoots.push(home);
      ...
      const skeletal: SteelEngineConfig = { channels: { telegram: { enabled: true } } };
      ...
      const rejectedArtifacts = fsNode
        ...
        .filter((name) => name.startsWith("steelengine.json.rejected."));
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        ...
        await withEnvAsync(
          { STEELENGINE_CONFIG_PATH: configPath, STEELENGINE_TEST_FAST: "1" },
          async () => {
            await expect(
