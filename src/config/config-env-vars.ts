...
function collectConfigEnvVarsByTarget(cfg?: RecallConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: RecallConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: RecallConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: RecallConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.RECALL_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
