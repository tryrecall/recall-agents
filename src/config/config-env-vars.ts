...
function isBlockedConfigEnvVar(key: string): boolean {
  return (
    ...
    key.toUpperCase() === "STEELENGINE_INCLUDE_ROOTS" ||
...
function collectConfigEnvVarsByTarget(cfg?: SteelEngineConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: SteelEngineConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** Collects config env vars safe to persist into managed service environments. */
export function collectConfigServiceEnvVars(cfg?: SteelEngineConfig): Record<string, string> {
...
type PublishedConfigRuntimeEnvState = {
  ...
  sourceConfig: SteelEngineConfig | null;
};
...
export function createConfigRuntimeEnvBase(
  activeConfig: SteelEngineConfig,
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.STEELENGINE_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
