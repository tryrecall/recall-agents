...
export const ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV =
  "RECALL_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS";
...
type FutureConfigGuardParams = {
  ...
  config?: Pick<RecallConfig, "meta"> | null;
  currentVersion?: string;
...
export function resolveFutureConfigActionBlock(
  ...
  return {
    ...
    currentVersion,
    touchedVersion,
    message: `Refusing to ${params.action} because this Recall binary (${currentVersion}) is older than the config last written by Recall ${touchedVersion}.`,
    hints: [
