...
function expectRecallSchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = RecallSchema.safeParse(params.config);
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectRecallSchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = RecallSchema.safeParse(config);
