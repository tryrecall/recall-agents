...
import { withTempHome, writeRecallConfig } from "./test-helpers.js";
...
describe("readBestEffortConfig", () => {
  it("can read snapshots without updating config observation state", async () => {
    await withTempHome(async (home) => {
      ...
      const healthPath = `${home}/.recall/logs/config-health.json`;
  ...
  it("does not restore suspicious direct edits from .bak during ordinary reads", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeRecallConfig(home, {
      ...
      const entries = await fs.readdir(`${home}/.recall`);
      expect(entries.some((entry) => entry.startsWith("recall.json.clobbered."))).toBe(false);
    });
  });
