...
export const FIELD_HELP: Record<string, string> = {
  ...
  "agents.list[].runtime.acp.agent":
    "Optional ACP harness agent id to use for this Recall agent (for example codex, claude).",
  ...
  "browser.profiles.*.driver":
    'Per-profile browser driver mode. Use "recall" (or legacy "clawd") for CDP-based profiles, or use "existing-session" for host-local Chrome DevTools MCP attachment.',
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `recall plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.recall/extensions/<id>).",
  ...
  "session.parentForkMaxTokens":
    "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, Recall starts a fresh thread session instead of forking; set 0 to disable this protection.",
  "session.mainKey":
  ...
  "channels.irc":
    "IRC channel provider configuration and compatibility settings for classic IRC transport workflows. Use this section when bridging legacy chat infrastructure into Recall.",
  "channels.defaults":
  ...
  "channels.discord.eventQueue.listenerTimeout": `Canonical Discord listener timeout control in ms for gateway normalization/enqueue handlers. Default is ${DISCORD_DEFAULT_LISTENER_TIMEOUT_MS} in Recall; set per account via channels.discord.accounts.<id>.eventQueue.listenerTimeout.`,
