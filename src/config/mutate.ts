...
async function tryWriteSingleTopLevelIncludeMutation(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
}): Promise<{ persistedHash: string | null; persistedConfig: RecallConfig } | null> {
...
function resolveConfigWriteResult(
  ...
  fallbackConfig: RecallConfig,
): { persistedHash: string | null; persistedConfig: RecallConfig } {
  if (result) {
    return {
...
export async function mutateConfigFile<T = void>(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
  mutate: (draft: RecallConfig, context: ConfigMutationContext) => Promise<T | void> | T | void;
}): Promise<ConfigMutationResult<T>> {
