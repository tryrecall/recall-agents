...
import type { ConfigFileSnapshot, SteelEngineConfig } from "./types.steelengine.js";
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "steelengine-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { STEELENGINE_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("writes health state to SQLite through public config reads", async () => {
    await withSuiteHome(async (home) => {
      ...
      const healthPath = path.join(home, ".steelengine", "logs", "config-health.json");
  ...
  it("loads shipped plugin install config records without mutating config or plugin index", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const pluginDir = path.join(home, ".steelengine", "plugins", "demo");
  ...
  it("migrates shipped plugin install config records during explicit writes even when the manifest is missing", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const pluginDir = path.join(home, ".steelengine", "plugins", "missing");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps shipped plugin install config records when index migration fails", async () => {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const unwritableStatePath = path.join(home, ".steelengine");
  ...
  it("dedupes validation warnings across writes and reloads until config becomes clean", async () => {
    await withSuiteHome(async (home) => {
      const warn = vi.fn();
      const io = createConfigIO({
        env: { HOME: home, STEELENGINE_TEST_FAST: "1" } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: { warn, error: vi.fn() },
      });
      const staleConfig = {
  ...
  it("keeps writes inside an STEELENGINE_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".steelengine", "steelengine.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "steelengine.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "steelengine.json"), "utf-8"),
      ) as {
  ...
  it("drops keys that exist only on the next-config prototype", async () => {
    await withSuiteHome(async (home) => {
      ...
      await io.writeConfigFile(nextConfig as SteelEngineConfig);
  ...
  it("prints missing-meta write anomalies when anomaly logging is requested", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          STEELENGINE_CONFIG_WRITE_ANOMALY_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("suppresses overwrite audit output when skipOutputLogs is set", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          VITEST: "true",
          STEELENGINE_TEST_CONFIG_OVERWRITE_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/steelengine-main" }] },
  ...
  it("keeps authored agent provider params during narrowed internal agent writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as SteelEngineConfig;
  ...
  it("returns the read-time environment snapshot for invalid config repairs", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          STEELENGINE_GATEWAY_TOKEN: "gateway-token-at-read",
          STEELENGINE_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
      ...
      expect(result.writeOptions.envSnapshotForRestore?.STEELENGINE_GATEWAY_TOKEN).toBe(
  ...
  it("returns the snapshot-time hash when an included file is malformed", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const includePath = path.join(home, ".steelengine", "plugins.json5");
  ...
  it("returns a write guard that rejects a changed active config path", async () => {
    await withSuiteHome(async (home) => {
      const firstConfigPath = path.join(home, ".steelengine", "first.json");
      const secondConfigPath = path.join(home, ".steelengine", "second.json");
      ...
      const env = {
        STEELENGINE_CONFIG_PATH: firstConfigPath,
        STEELENGINE_TEST_FAST: "1",
      } as NodeJS.ProcessEnv;
      const io = createConfigIO({ env, homedir: () => home, logger: silentLogger });

      const result = await io.readConfigFileSnapshotForWrite();
      env.STEELENGINE_CONFIG_PATH = secondConfigPath;
  ...
  it("rejects local write ownership when config env changes path selection during the read", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const configuredNextPath = path.join(home, ".steelengine", "next.json");
      const sourceConfig = {
        env: { STEELENGINE_CONFIG_PATH: configuredNextPath },
        gateway: { mode: "local" },
  ...
  it("follows config env path selection before returning global write ownership", async () => {
    await withSuiteHome(async (home) => {
      ...
      const initialConfig = JSON.parse(await fs.readFile(configPath, "utf-8")) as SteelEngineConfig;
  ...
  it("does not use expectedConfigPath as the write destination", async () => {
    await withSuiteHome(async (home) => {
      const expectedConfigPath = path.join(home, ".steelengine", "expected.json");
      const activeConfigPath = path.join(home, ".steelengine", "active.json");
      ...
      await withEnvAsync(
        {
          STEELENGINE_CONFIG_PATH: activeConfigPath,
          STEELENGINE_TEST_FAST: "1",
        },
        async () => {
          await writeConfigFile(
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const includePath = path.join(home, ".steelengine", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://steelengine.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
  ...
  it("rejects invalid include-backed repairs instead of persisting substituted secrets", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const includePath = path.join(home, ".steelengine", "gateway.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify(
      ...
      const io = createConfigIO({
        env: {
          STEELENGINE_GATEWAY_TOKEN: "gateway-token-runtime",
          STEELENGINE_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
  ...
  it("repairs invalid root-authored siblings without flattening included config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const includePath = path.join(home, ".steelengine", "agent-defaults.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
  ...
  it("rejects repairs that would flatten a valid outer include with a broken nested include", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const pluginsPath = path.join(home, ".steelengine", "plugins.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        pluginsPath,
  ...
  it("preserves escaped root literals before validating unrelated includes", async () => {
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/steelengine-test-literal-plugin",
          source: "/tmp/steelengine-test-literal-plugin/index.ts",
          manifestPath: "/tmp/steelengine-test-literal-plugin/steelengine.plugin.json",
          configSchema: {
            type: "object",
            properties: {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const agentsPath = path.join(home, ".steelengine", "agents.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        agentsPath,
  ...
  it("repairs invalid config without flattening array-nested includes", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const includePath = path.join(home, ".steelengine", "main-agent.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ id: "main", workspace: "${STEELENGINE_AGENT_WORKSPACE}" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      const io = createConfigIO({
        env: {
          STEELENGINE_AGENT_WORKSPACE: "/resolved/agent-workspace",
          STEELENGINE_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
      ...
      await expect(fs.readFile(includePath, "utf-8")).resolves.toContain(
        '"workspace": "${STEELENGINE_AGENT_WORKSPACE}"',
      );
    });
  });
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/steelengine-test-required-plugin",
          source: "/tmp/steelengine-test-required-plugin/index.ts",
          manifestPath: "/tmp/steelengine-test-required-plugin/steelengine.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
  ...
  it("writes runtime-derived edits back to source SecretRef markers", async () => {
    await withSuiteHome(async (home) => {
      ...
      await withEnvAsync({ STEELENGINE_CONFIG_PATH: configPath }, async () => {
        setRuntimeConfigSnapshot(
          {
            gateway: { mode: "local" },
            models: {
      ...
      try {
        await withEnvAsync(
          ...
          async () => {
            ...
            expect(persisted.gateway?.auth?.token).toBe("${STEELENGINE_GATEWAY_TOKEN}");
  ...
  it("stages managed root-write config env until the owner accepts it", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const envKey = "STEELENGINE_TEST_MANAGED_ROOT_ENV";
      const initialAuthoredConfig = {
        gateway: {
          mode: "local" as const,
          auth: { mode: "token" as const, token: "${STEELENGINE_TEST_MANAGED_ROOT_ENV}" },
        },
        env: { vars: { [envKey]: "old" } },
      } satisfies SteelEngineConfig;
      const initialConfig = {
      ...
      let notifiedSource: SteelEngineConfig | undefined;
      const unsubscribe = registerConfigWriteListener(
        (event) => {
      ...
      try {
        await withEnvAsync({ STEELENGINE_CONFIG_PATH: configPath, [envKey]: "old" }, async () => {
          setRuntimeConfigSnapshot(initialConfig, initialConfig);
  ...
  it("resolves watcher candidates after removing the accepted config env layer", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const envKey = "STEELENGINE_TEST_WATCHER_ENV";
      const activeConfig = {
        env: { vars: { [envKey]: "old" } },
      ...
      const candidate = {
        env: { vars: { [envKey]: "new" } },
        gateway: { auth: { mode: "token" as const, token: "${STEELENGINE_TEST_WATCHER_ENV}" } },
      } satisfies SteelEngineConfig;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
  ...
  it("rereads a managed write against an env transaction accepted during preflight", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const envKey = "STEELENGINE_TEST_INTERLEAVED_WRITE_ENV";
      const makeConfig = (value: string, token: string): SteelEngineConfig => ({
      ...
      try {
        await withEnvAsync({ STEELENGINE_CONFIG_PATH: configPath, [envKey]: "a" }, async () => {
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/steelengine-test-demo",
          source: "/tmp/steelengine-test-demo/index.ts",
          manifestPath: "/tmp/steelengine-test-demo/steelengine.plugin.json",
          configSchema: {
            type: "object",
            properties: {
      ...
      try {
        await withEnvAsync({ STEELENGINE_CONFIG_PATH: configPath }, async () => {
          ...
          const postWriteSnapshot = await createConfigIO({
            env: { STEELENGINE_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
            homedir: () => home,
            logger: silentLogger,
  ...
  it("does not delete an existing root config when rollback has no previous raw payload", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies SteelEngineConfig;
      await fs.writeFile(configPath, `${JSON.stringify(initialConfig, null, 2)}\n`, "utf-8");
      const baseSnapshot = {
        path: configPath,
        exists: true,
  ...
  it("blocks runtime preflight failures before committing root writes", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const initialRaw = `${JSON.stringify({ gateway: { mode: "local" } }, null, 2)}\n`;
      let observedSource: SteelEngineConfig | undefined;
  ...
  it("restores config env vars when post-write runtime refresh rollback succeeds", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const envKey = "STEELENGINE_TEST_RUNTIME_ROLLBACK_ENV";
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies SteelEngineConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
  ...
  it("rolls back root writes when canonical reread changes config path ownership", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const nextConfigPath = path.join(home, ".steelengine", "next.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies SteelEngineConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        async () => {
          ...
          await expect(
            writeConfigFile(
              {
                gateway: { mode: "local", port: 19001 },
                env: { STEELENGINE_CONFIG_PATH: nextConfigPath },
              },
              {
                baseSnapshot: prepared.snapshot,
                ...prepared.writeOptions,
  ...
  it("uses injected filesystem operations when rolling back ownership loss", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".steelengine", "steelengine.json");
      const otherConfigPath = path.join(home, ".steelengine", "other.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies SteelEngineConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        promises: {
          ...
          rename: async (from, to) => {
            ...
            if (!committed && to === configPath) {
              committed = true;
              env.STEELENGINE_CONFIG_PATH = otherConfigPath;
            }
          },
        },
      } as typeof fsNode;
    ...
    await withSuiteHome(async (home) => {
        ...
        plugins: [
          {
            ...
            skills: [],
            hooks: [],
            rootDir: "/tmp/steelengine-test-strict-plugin",
            source: "/tmp/steelengine-test-strict-plugin/index.ts",
            manifestPath: "/tmp/steelengine-test-strict-plugin/steelengine.plugin.json",
            configSchema: {
              type: "object",
      ...
      try {
        ...
        await withEnvAsync({ STEELENGINE_CONFIG_PATH: configPath }, async () => {
          ...
          await expect(
            writeConfigFile({ agents: { list: "not-array" } } as unknown as SteelEngineConfig, {
  ...
  it("preserves authored tilde paths when runtime-shaped writes hand back absolute paths", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            logging: { file: "~/steelengine-upgrade-survivor/gateway.jsonl" },
          },
          null,
          2,
        )}\n`,
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "steelengine-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
        { baseSnapshot: snapshot },
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as SteelEngineConfig;
      expect(persisted.logging?.file).toBe("~/steelengine-upgrade-survivor/gateway.jsonl");
