...
describe("config io write", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-config-io-"));
  });
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".recall", "recall.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".recall", "logs", "config-audit.jsonl");
  ...
  it('shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withSuiteHome(async (home) => {
      ...
      const invalidConfig: RecallConfig = {
        channels: {
          telegram: {
            dmPolicy: "open",
            allowFrom: [],
      ...
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        "recall config set channels.telegram.allowFrom '[\"*\"]'",
      );
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        'recall config set channels.telegram.dmPolicy "pairing"',
      );
    });
  });
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    await withSuiteHome(async (home) => {
      ...
      expect(lines.length).toBeGreaterThan(0);
      await expect(
        fs.stat(path.join(home, ".recall", "logs", "config-audit.jsonl")),
      ).resolves.toBeDefined();
      await expect(
        fs.stat(path.resolve("undefined", ".recall", "logs", "config-audit.jsonl")),
      ).rejects.toThrow();
    });
  });
