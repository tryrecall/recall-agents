...
export const AGENT_FIELD_HELP: Record<string, string> = {
  ...
  "agents.list.*.models.*.agentRuntime.id":
    'Per-agent model runtime id: "steelengine", "auto", a registered plugin harness id such as "codex", or a supported CLI backend alias such as "claude-cli".',
  ...
  "agents.defaults.compaction.reserveTokensFloor":
    "Minimum floor enforced for reserveTokens in embedded SteelEngine compaction paths (0 disables the floor guard). Use a non-zero floor to avoid over-aggressive compression under fluctuating token estimates.",
  ...
  "agents.defaults.compaction.midTurnPrecheck":
    "Optional embedded SteelEngine tool-loop precheck that detects context pressure after a tool result is appended and before the next model call. When enabled, SteelEngine reuses existing precheck recovery to truncate tool results or compact before retrying.",
  "agents.defaults.compaction.midTurnPrecheck.enabled":
    "Enable structured mid-turn context pressure checks for embedded SteelEngine tool loops. Default: false. Keep disabled unless long tool-heavy sessions hit context overflow before normal turn-end compaction can run.",
  ...
  "agents.defaults.runRetries":
    "Outer run loop retry iteration boundaries for the embedded SteelEngine runner to prevent infinite execution loops during failure recovery.",
  "agents.defaults.runRetries.base":
    "Base number of run retry iterations for the embedded SteelEngine runner's outer run loop (default: 24).",
  ...
  "agents.list[].runRetries":
    "Optional per-agent override for the embedded SteelEngine runner's outer run loop retry iteration boundaries.",
  ...
  "agents.defaults.embeddedAgent":
    "Embedded SteelEngine runner hardening controls for how workspace-local agent settings are trusted and applied in SteelEngine sessions.",
  "agents.defaults.embeddedAgent.projectSettingsPolicy":
    'How embedded SteelEngine handles workspace-local `.steelengine/settings.json`: "sanitize" (default) strips shellPath/shellCommandPrefix, "ignore" disables project settings entirely, and "trusted" applies project settings as-is.',
  "agents.defaults.embeddedAgent.executionContract":
    'Embedded SteelEngine execution contract: "default" keeps the standard runner behavior, while "strict-agentic" enables structured plan tracking and non-visible turn recovery for supported OpenAI/OpenAI Codex GPT-5-family runs.',
  "agents.list[].embeddedAgent":
    "Optional per-agent embedded SteelEngine overrides. Use this to opt specific agents into stricter GPT-5 execution behavior without changing the global default.",
  "agents.list[].embeddedAgent.executionContract":
    'Optional per-agent embedded SteelEngine execution contract override. Set "strict-agentic" to enable structured plan tracking and non-visible turn recovery for that agent on supported OpenAI/OpenAI Codex GPT-5-family runs, or "default" to inherit the standard runner behavior.',
  ...
  "commands.mcp":
    "Allow /mcp chat command to manage SteelEngine MCP server config under mcp.servers (default: false).",
  "commands.plugins":
  ...
  mcp: "Global MCP server definitions managed by SteelEngine. Embedded SteelEngine and other runtime adapters can consume these servers without storing them inside runtime-owned project settings.",
  "mcp.servers":
    "Named MCP server definitions. SteelEngine stores them in its own config and runtime adapters decide which transports are supported at execution time.",
  "mcp.servers.*.codex":
    "SteelEngine projection metadata for Codex app-server threads only. It does not affect ACP sessions or generic Codex harness config. Omit this block to keep the server available to every Codex app-server agent with Codex's default MCP approval behavior.",
  ...
  "mcp.servers.*.oauth.authProfileId":
    "Refresh-capable auth profile id used to inject the current bearer token into this remote MCP server. When set, SteelEngine resolves and refreshes the profile at runtime and does not project refresh material downstream.",
  "mcp.servers.*.codex.agents":
    "Optional non-empty SteelEngine agent ids that should receive this MCP server in Codex app-server thread config. Empty, blank, or invalid lists fail closed; when omitted, the server is projected for all Codex app-server agents.",
  ...
  "mcp.servers.*.codex.default_tools_approval_mode":
    "Codex-native spelling for the same per-server MCP tool approval mode. Prefer defaultToolsApprovalMode in SteelEngine config.",
