...
import { isSensitiveUrlConfigPath } from "@steelengine/net-policy/redact-sensitive-url";
...
describe("mapSensitivePaths", () => {
  ...
  it("main schema yields correct hints (samples)", () => {
    ...
    schema.title = "SteelEngineConfig";
    const hints = mapSensitivePaths(SteelEngineSchema, "", {});
...
describe("collectMatchingSchemaPaths", () => {
  it("finds base-config URL fields that may embed secrets", () => {
    const paths = collectMatchingSchemaPaths(SteelEngineSchema, "", isSensitiveUrlConfigPath);
