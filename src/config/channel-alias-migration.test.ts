...
function cfgWith(channelId: string, entry: Record<string, unknown>): SteelEngineConfig {
...
describe("defineChannelAliasMigration message generation", () => {
  it("generates preview-chunk channel messages (discord shape)", () => {
    ...
    expect(migration.legacyConfigRules.map((rule) => rule.message)).toEqual([
      'channels.discord.streamMode, channels.discord.streaming (scalar), chunkMode, blockStreaming, draftChunk, and blockStreamingCoalesce are legacy; use channels.discord.streaming.{mode,chunkMode,preview.chunk,block.enabled,block.coalesce}. Run "steelengine doctor --fix".',
      'channels.discord.accounts.<id>.streamMode, streaming (scalar), chunkMode, blockStreaming, draftChunk, and blockStreamingCoalesce are legacy; use channels.discord.accounts.<id>.streaming.{mode,chunkMode,preview.chunk,block.enabled,block.coalesce}. Run "steelengine doctor --fix".',
    ]);
  ...
  it("generates native-transport channel messages (slack shape)", () => {
    ...
    expect(migration.legacyConfigRules.map((rule) => rule.message)).toEqual([
      'channels.slack.streamMode, channels.slack.streaming (scalar), chunkMode, blockStreaming, blockStreamingCoalesce, and nativeStreaming are legacy; use channels.slack.streaming.{mode,chunkMode,block.enabled,block.coalesce,nativeTransport}. Run "steelengine doctor --fix".',
      'channels.slack.accounts.<id>.streamMode, streaming (scalar), chunkMode, blockStreaming, blockStreamingCoalesce, and nativeStreaming are legacy; use channels.slack.accounts.<id>.streaming.{mode,chunkMode,block.enabled,block.coalesce,nativeTransport}. Run "steelengine doctor --fix".',
    ]);
  });
  ...
  it("generates delivery-only channel messages (imessage shape)", () => {
    ...
    expect(migration.legacyConfigRules.map((rule) => rule.message)).toEqual([
      'channels.imessage.chunkMode, blockStreaming, and blockStreamingCoalesce are legacy; use channels.imessage.streaming.{chunkMode,block.enabled,block.coalesce}. Run "steelengine doctor --fix".',
      'channels.imessage.accounts.<id>.chunkMode, blockStreaming, and blockStreamingCoalesce are legacy; use channels.imessage.accounts.<id>.streaming.{chunkMode,block.enabled,block.coalesce}. Run "steelengine doctor --fix".',
    ]);
  });
  ...
  it("generates plain mode channel messages (msteams shape)", () => {
    ...
    expect(migration.legacyConfigRules[0]?.message).toBe(
      'channels.msteams.streamMode, channels.msteams.streaming (scalar), chunkMode, blockStreaming, and blockStreamingCoalesce are legacy; use channels.msteams.streaming.{mode,chunkMode,block.enabled,block.coalesce}. Run "steelengine doctor --fix".',
    );
  });
});
