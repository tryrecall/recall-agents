...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ RECALL_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-recall",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-recall#general");
  });
  ...
  it("derives session transcripts dir from RECALL_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { RECALL_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when RECALL_STATE_DIR differs from stored paths", () => {
