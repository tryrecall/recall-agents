...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ STEELENGINE_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-steelengine",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-steelengine#general");
  });
  ...
  it("derives session transcripts dir from STEELENGINE_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { STEELENGINE_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when STEELENGINE_STATE_DIR differs from stored paths", () => {
