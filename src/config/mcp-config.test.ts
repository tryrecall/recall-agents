...
const mockReadSourceConfigSnapshot = vi.hoisted(() => async () => {
  ...
  const configPath = pathValue.join(process.env.STEELENGINE_STATE_DIR ?? "", "steelengine.json");
  try {
...
const mockReplaceConfigFile = vi.hoisted(() => async ({ nextConfig }: { nextConfig: unknown }) => {
  ...
  const configPath = pathLocal.join(process.env.STEELENGINE_STATE_DIR ?? "", "steelengine.json");
