...
import { normalizeConfiguredProviderCatalogModelId } from "@steelengine/model-catalog-core/provider-model-id-normalization";
...
export function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  steelengine config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  steelengine config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function coerceConfig(value: unknown): SteelEngineConfig {
...
function unsetPathForWrite(
  root: SteelEngineConfig,
  pathSegments: string[],
): { changed: boolean; next: SteelEngineConfig } {
