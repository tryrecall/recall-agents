...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: RecallConfig): boolean {
...
function hasConfiguredEmbeddedHarnessRuntime(cfg: RecallConfig, env: NodeJS.ProcessEnv): boolean {
...
function isProviderConfigured(cfg: RecallConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: RecallConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: RecallConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: RecallConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: RecallConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: RecallConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: RecallConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: RecallConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: RecallConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: RecallConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: RecallConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: RecallConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: RecallConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: RecallConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: RecallConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: RecallConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: RecallConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: RecallConfig): boolean {
  return (
...
function hasPluginEntries(cfg: RecallConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: RecallConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: RecallConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: RecallConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: RecallConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: RecallConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: RecallConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: RecallConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: RecallConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
