...
function isProviderConfigured(cfg: SteelEngineConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: SteelEngineConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: SteelEngineConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: SteelEngineConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: SteelEngineConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: SteelEngineConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: SteelEngineConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredSpeechProviderSelection(cfg: SteelEngineConfig): boolean {
  return collectConfiguredSpeechProviderIds(cfg).size > 0;
}

function hasConfiguredPluginConfigEntry(cfg: SteelEngineConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: SteelEngineConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: SteelEngineConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: SteelEngineConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: SteelEngineConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: SteelEngineConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: SteelEngineConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: SteelEngineConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: SteelEngineConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: SteelEngineConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: SteelEngineConfig): boolean {
  return (
...
function hasPluginEntries(cfg: SteelEngineConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: SteelEngineConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: SteelEngineConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: SteelEngineConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: SteelEngineConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: SteelEngineConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: SteelEngineConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: SteelEngineConfig, pluginId: string): boolean {
...
function disableImplicitPreferredOverPlugin(params: {
  config: SteelEngineConfig;
  originalConfig: SteelEngineConfig;
  pluginId: string;
...
function isBuiltInChannelAlreadyEnabled(cfg: SteelEngineConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
