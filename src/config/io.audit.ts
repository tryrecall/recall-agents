...
export function createConfigWriteAuditRecordBase(params: {
  ...
  return {
    ...
    watchMode: params.env.RECALL_WATCH_MODE === "1",
    watchSession: normalizeAuditLabel(params.env.RECALL_WATCH_SESSION),
    watchCommand: normalizeAuditLabel(params.env.RECALL_WATCH_COMMAND),
...
export type ConfigAuditScrubResult = {
  ...
  // to swap the file. Caller should re-run `recall doctor --fix` once the
