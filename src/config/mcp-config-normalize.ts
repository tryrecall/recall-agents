...
type RecallMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_RECALL_TRANSPORT: Record<string, RecallMcpHttpTransport | "stdio"> = {
...
export function resolveRecallMcpTransportAlias(
  value: unknown,
): RecallMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_RECALL_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_RECALL_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveRecallMcpTransportAlias(next.type);
