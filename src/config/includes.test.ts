...
const ETC_STEELENGINE_DIR = path.join(ROOT_DIR, "etc", "steelengine");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "steelengine.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcSteelEnginePath(...parts: string[]) {
  return path.join(ETC_STEELENGINE_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcSteelEnginePath("agents.json");
    ...
    expectResolveIncludeError(
      () =>
        resolve(
          ...
          configPath("sub", "steelengine.json"),
        ),
...
describe("collectIncludePathsRecursive", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "steelengine-include-scan-" }, async (tempRoot) => {
        ...
        const includePaths = await collectIncludePathsRecursive({
          configPath: path.join(configDir, "steelengine.json"),
          parsed: {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "steelengine-include-root-swap-" }, async (tempRoot) => {
        ...
        expect(() =>
          resolveConfigIncludes(
            { $include: "./outer.json5" },
            path.join(configLink, "steelengine.json"),
            resolver,
          ),
  ...
  it("honors explicitly allowed include roots", async () => {
    await withTempDir({ prefix: "steelengine-include-scan-roots-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
  ...
  it("continues past rejected nested includes to later safe siblings", async () => {
    await withTempDir({ prefix: "steelengine-include-scan-nested-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
  ...
  it("revisits an include reached later at a shallower depth", async () => {
    await withTempDir({ prefix: "steelengine-include-scan-depth-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
...
describe("resolveConfigIncludeWritePath", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "steelengine-include-write-path-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      await withTempDir({ prefix: "steelengine-includes-symlink-" }, async (tempRoot) => {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "steelengine.json"),
        );
    ...
    it("rejects include files that are hardlinked aliases", async () => {
      ...
      await withTempDir({ prefix: "steelengine-includes-hardlink-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
        const outsideDir = path.join(tempRoot, "outside");
        await fs.mkdir(configDir, { recursive: true });
        await fs.mkdir(outsideDir, { recursive: true });
    ...
    it("rejects include files larger than the guarded read limit", async () => {
      await withTempDir({ prefix: "steelengine-includes-big-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
        await fs.mkdir(configDir, { recursive: true });
        await fs.writeFile(
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "steelengine.json")),
...
describe("STEELENGINE_INCLUDE_ROOTS allowlist", () => {
  ...
  it("still rejects include paths that fall outside every allowed root", () => {
    const obj = { $include: etcSteelEnginePath("agents.json") };
    expect(() =>
  ...
  it("resolves a symlinked include whose realpath lands inside an allowed root", async () => {
    await withTempDir({ prefix: "steelengine-includes-allowed-symlink-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
      const sharedDir = path.join(tempRoot, "shared");
      await fs.mkdir(configDir, { recursive: true });
      await fs.mkdir(sharedDir, { recursive: true });
  ...
  it("rejects a symlinked include that escapes both the config directory and every allowed root", async () => {
    await withTempDir({ prefix: "steelengine-includes-allowed-escape-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
      const allowedDir = path.join(tempRoot, "allowed");
