...
async function makeSettings() {
  testRoot = await mkdtemp(join(tmpdir(), "recall-debug-proxy-managed-proxy-"));
...
describe("debug proxy managed-proxy direct upstream policy", () => {
  const originalProxyActive = process.env["RECALL_PROXY_ACTIVE"];
  const originalAllowDirect =
    process.env["RECALL_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"];

  beforeEach(async () => {
    await cleanupTestDirs();
    delete process.env["RECALL_PROXY_ACTIVE"];
    delete process.env["RECALL_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"];
  });
  ...
  afterEach(async () => {
    if (originalProxyActive === undefined) {
      ...
      process.env["RECALL_PROXY_ACTIVE"] = originalProxyActive;
    }
  ...
  it("uses shared truthy parsing for managed proxy mode", () => {
    process.env["RECALL_PROXY_ACTIVE"] = "true";
  ...
  it("allows direct upstreams with explicit diagnostic override", () => {
    process.env["RECALL_PROXY_ACTIVE"] = "1";
    process.env["RECALL_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY"] = "1";
