...
import { normalizeNullableString as normalizeObservedValue } from "@steelengine/normalization-core/string-coerce";
...
class DebugProxyCaptureStoreImpl {
  ...
  constructor(
    ...
    const database = openSteelEngineStateDatabase({ env: optionsOrDbPath.env });
    this.db = database.db;
...
function resolveDebugProxyCaptureStoreKey(
  ...
  return typeof optionsOrDbPath === "string"
    ...
    : `shared:${openSteelEngineStateDatabase({ env: optionsOrDbPath.env }).path}`;
}
