...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[recall proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[recall proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`recall proxy coverage\` for details.`,
  );
}
