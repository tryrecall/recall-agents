...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[steelengine proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[steelengine proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`steelengine proxy coverage\` for details.`,
  );
}
