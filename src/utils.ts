...
export { sliceUtf16Safe, truncateUtf16Safe } from "@steelengine/normalization-core/utf16-slice";

/** Resolves the SteelEngine config directory from state/config env overrides or home. */
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".steelengine");
  try {
...
/** Resolves the effective SteelEngine home directory, if one can be determined. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.STEELENGINE_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$STEELENGINE_HOME" };
  }
  return { home, prefix: "~" };
}

/** Replaces the leading home directory in a path with `~` or `$STEELENGINE_HOME`. */
