...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".recall");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.RECALL_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$RECALL_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via RECALL_STATE_DIR.
