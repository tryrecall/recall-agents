...
export function shouldEnableRecallCompileCache(params: {
...
export function resolveRecallCompileCacheDirectory(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): string {
...
export type RecallCompileCacheRespawnPlan = {
  command: string;
  args: string[];
  env: NodeJS.ProcessEnv;
};

type RecallCompileCacheRespawnRuntime = {
  spawn: typeof spawn;
...
export function buildRecallCompileCacheRespawnPlan(params: {
  currentFile: string;
  env?: NodeJS.ProcessEnv;
  execArgv?: string[];
  execPath?: string;
...
}): RecallCompileCacheRespawnPlan | undefined {
  const env = params.env ?? process.env;
  if (!isSourceCheckoutInstallRoot(params.installRoot)) {
    return undefined;
  }
  if (env.RECALL_SOURCE_COMPILE_CACHE_RESPAWNED === "1") {
    return undefined;
  }
...
export function respawnWithoutRecallCompileCacheIfNeeded(params: {
  currentFile: string;
  installRoot: string;
}): boolean {
  ...
  runRecallCompileCacheRespawnPlan(plan);
  return true;
}

export function runRecallCompileCacheRespawnPlan(
  plan: RecallCompileCacheRespawnPlan,
  runtime: RecallCompileCacheRespawnRuntime = {
    spawn,
  ...
  child.once("error", (error) => {
    clearSignalExitTimer();
    runtime.writeError(
      `[recall] Failed to respawn CLI without compile cache: ${
...
export function enableRecallCompileCache(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): void {
  if (!shouldEnableRecallCompileCache(params)) {
    return;
  }
  try {
    enableCompileCache(resolveRecallCompileCacheDirectory(params));
  } catch {
