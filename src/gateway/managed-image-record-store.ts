...
function stateDatabaseOptions(stateDir?: string): SteelEngineStateDatabaseOptions {
  return stateDir
...
export function readManagedImageRecord(
  ...
  const database = openSteelEngineStateDatabase(stateDatabaseOptions(stateDir));
