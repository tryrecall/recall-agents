...
describe("startGatewayTailscaleExposure preserveFunnel", () => {
  ...
  it("passes serviceName through to Tailscale Serve setup and cleanup", async () => {
    ...
    expect(mocks.enableTailscaleServe).toHaveBeenCalledWith(18789, undefined, "svc:steelengine");
    expect(logTailscale.info).toHaveBeenCalledWith(
      "serve enabled for svc:steelengine: https://steelengine.tailnet.ts.net/ (WS via wss://steelengine.tailnet.ts.net)",
    );

    await cleanup?.();

    expect(mocks.disableTailscaleServe).toHaveBeenCalledWith(undefined, "svc:steelengine");
  });
