...
export function attachRecallTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__recall && typeof record.__recall === "object" && !Array.isArray(record.__recall)
      ? (record.__recall as Record<string, unknown>)
      : {};
  return {
    ...record,
    __recall: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachRecallTranscriptMeta(parsed.message, {
...
export function resolveSessionTranscriptCandidates(
  ...
  const legacyDir = path.join(home, ".recall", "sessions");
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "recall" && model === "delivery-mirror";
