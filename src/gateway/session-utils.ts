...
import { type RecallConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: RecallConfig): string[] {
...
export function listAgentsForGateway(cfg: RecallConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: RecallConfig): string {
...
function resolveSessionStoreAgentId(cfg: RecallConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: RecallConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: RecallConfig): GatewaySessionsDefaults {
