...
describe("gateway/node-command-policy", () => {
  ...
  it("keeps canvas commands out of core defaults when the canvas plugin is not active", () => {
    const allowlist = resolveNodeCommandAllowlist({} as SteelEngineConfig, {
      platform: "windows",
  ...
  it("uses the app-recommendation kill switch for gateway device.apps access", () => {
    ...
    expect(resolveNodeCommandAllowlist({} as SteelEngineConfig, macNode).has("device.apps")).toBe(
      true,
    );
    expect(
      resolveNodeCommandAllowlist(
        { wizard: { appRecommendations: false } } as SteelEngineConfig,
        macNode,
      ).has("device.apps"),
    ).toBe(false);
  });
  ...
  it("allows approved node-host MCP calls while denyCommands still wins", () => {
    ...
    const allowlist = resolveNodeCommandAllowlist({} as SteelEngineConfig, node);
    expect(
      resolveNodePairingCommandAllowlist({} as SteelEngineConfig, node).has("mcp.tools.call.v1"),
    ).toBe(true);
    ...
    const denied = resolveNodeCommandAllowlist(
      { gateway: { nodes: { denyCommands: ["mcp.tools.call.v1"] } } } as SteelEngineConfig,
      node,
    );
  ...
  it("does not grandfather approved non-default commands after config removal", () => {
    const staleApproval = resolveNodeCommandAllowlist({} as SteelEngineConfig, {
      platform: "macos",
      deviceFamily: "Mac",
  ...
  it("keeps computer.act out of the runtime allowlist until explicitly allowed", () => {
    ...
    const unarmed = resolveNodeCommandAllowlist({} as SteelEngineConfig, macNode);
    expect(unarmed.has("computer.act")).toBe(false);
    expect(
      resolveNodeCommandAllowlist({} as SteelEngineConfig, {
        ...macNode,
    ...
    const armed = resolveNodeCommandAllowlist(
      { gateway: { nodes: { allowCommands: ["computer.act"] } } } as SteelEngineConfig,
      macNode,
    );
  ...
  it("keeps computer.act declarable through the macOS pairing allowlist only", () => {
    const pairing = resolveNodePairingCommandAllowlist({} as SteelEngineConfig, {
      platform: "macos",
      deviceFamily: "Mac",
    ...
    const windowsPairing = resolveNodePairingCommandAllowlist({} as SteelEngineConfig, {
      platform: "windows",
      deviceFamily: "Windows",
      commands: ["computer.act"],
    });
  ...
  it("requires explicit gateway opt-in for iOS health summaries", () => {
    ...
    expect(resolveNodeCommandAllowlist({} as SteelEngineConfig, node).has("health.summary")).toBe(
      false,
    );
