...
import type { ModelsConfig, RecallConfig, ModelProviderConfig } from "../config/types.js";
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.RECALL_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.RECALL_LIVE_MAX_MODELS, 0));
}
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveRecallAgentDir();
