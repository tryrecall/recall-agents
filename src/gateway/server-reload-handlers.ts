...
type GatewayReloadHandlerParams = {
  ...
  createHealthMonitor: (config: RecallConfig) => ChannelHealthMonitor | null;
...
> & {
  ...
  initialConfig: RecallConfig;
  initialCompareConfig?: RecallConfig;
  ...
  resolveSharedGatewaySessionGenerationForConfig: (config: RecallConfig) => string | undefined;
...
export function createGatewayReloadHandlers(params: GatewayReloadHandlerParams) {
  ...
  const markActiveMainSessionsForRestart = async (nextConfig: RecallConfig, reason: string) => {
  ...
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: RecallConfig) => {
            ...
            await startGmailWatcherWithLogs({
              ...
              onSkipped: () =>
                params.logHooks.info(
                  "skipping gmail watcher restart (RECALL_SKIP_GMAIL_WATCHER=1)",
                ),
            });
          }
        }
    ...
    if (channelsToRestart.size > 0) {
      if (shouldSkipChannelRestart()) {
        params.logChannels.info(
          "skipping channel reload (RECALL_SKIP_CHANNELS=1 or RECALL_SKIP_PROVIDERS=1)",
        );
      } else {
        if (!plan.reloadPlugins) {
          await waitForActiveWorkBeforeChannelReload(channelsToRestart, nextConfig);
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: RecallConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
