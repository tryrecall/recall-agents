...
describe("createLazyGatewayCronState", () => {
  ...
  it("resolves its default store path from the prepared env", () => {
    const stateRoot = "/tmp/steelengine-candidate-state";
    const lazy = createLazyGatewayCronState({
      ...createParams(),
      env: { ...process.env, STEELENGINE_STATE_DIR: stateRoot },
    });
  ...
  it("preserves the startup cron enabled flag without loading cron runtime", () => {
    vi.stubEnv("STEELENGINE_SKIP_CRON", "1");
...
function createParams(overrides: Partial<SteelEngineConfig> = {}) {
  return {
    cfg: {
      ...overrides,
...
function createCronState(cron: GatewayCronServiceContract): GatewayCronState {
  return {
    cron,
    storePath: "/tmp/steelengine-cron.json",
    cronEnabled: true,
  } as GatewayCronState;
}
