...
function createSingleAgentAvatarConfig(workspace: string): SteelEngineConfig {
  return {
...
describe("gateway session utils", () => {
  ...
  test("session list search includes direct-session origin display labels", () => {
    ...
    const listed = listSessionsFromStore({
      ...
      storePath: "",
      store,
      opts: { search: "steelengine-tui" },
    });

    expect(listed.sessions.map((session) => session.key)).toEqual([
      "agent:main:telegram:direct:42",
    ]);
    expect(listed.sessions[0]?.displayName).toBe("steelengine-tui");
  });
  ...
  test("preserves persisted Ultra while projecting picker levels without a catalog", () => {
    providerArtifactMocks.resolveBundledProviderPolicySurface.mockReturnValue({
      resolveThinkingProfile: ({ modelId, agentRuntime }) => ({
        levels: [
          ...
          (agentRuntime === "steelengine" || !modelId.startsWith("gpt-5.6-luna"))
            ? [{ id: "ultra" as const }]
            : []),
        ],
      }),
    ...
    const steelEngineOverride = row({
      sessionId: "steelengine",
      thinkingLevel: "ultra",
      agentRuntimeOverride: "steelengine",
    } as SessionEntry);
    const legacyObservedSteelEngine = row({
      sessionId: "legacy-observed-steelengine",
      thinkingLevel: "ultra",
    ...
    expect(steelEngineOverride.thinkingLevel).toBe("ultra");
    expect(steelEngineOverride.agentRuntime?.id).toBe("steelengine");
    expect(legacyObservedSteelEngine.thinkingLevel).toBe("ultra");
    expect(legacyObservedSteelEngine.agentRuntime?.id).toBe("codex");
    expect(legacyObservedSteelEngine.thinkingLevels?.map((level) => level.id)).not.toContain("ultra");
  ...
  test("session rows expose estimated context budget status", () => {
    const row = buildGatewaySessionRow({
      ...
      entry: {
        sessionId: "session-1",
        sessionFile: "/tmp/steelengine/agents/main/sessions/session-1.jsonl",
        updatedAt: 1,
        contextBudgetStatus: {
          schemaVersion: 1,
  ...
  test("buildGatewaySessionRow displayName falls through to origin label for direct sessions", () => {
    ...
    expect(row.displayName).toBe("steelengine-tui");
  });
  ...
  test("buildGatewaySessionRow projects worktree and execNode bindings", () => {
    ...
    const entry = {
      ...
      worktree: { id: "wt-id", branch: "steelengine/wt-1234", repoRoot: "/repo" },
      execNode: "macbook",
    ...
    expect(row.worktree).toEqual({ id: "wt-id", branch: "steelengine/wt-1234", repoRoot: "/repo" });
    expect(row.execNode).toBe("macbook");
    expect(row.execCwd).toBe("/Users/peter/Projects/steelengine");
  });
  ...
  test("listAgentsForGateway keeps explicit agents.list scope over disk-only agents (scope boundary)", async () => {
    await withStateDirEnv("steelengine-agent-list-scope-", async ({ stateDir }) => {
  ...
  test("listAgentsForGateway reports whether each workspace is a git checkout", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-agent-workspace-git-"));
...
describe("listSessionsFromStore selected model display", () => {
  test("async list yields during bulk transcript title and last-message hydration", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sessions-list-yield-"));
    try {
  ...
  test("caps transcript title and last-message hydration for bulk list responses", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sessions-list-cap-"));
    try {
      const storePath = path.join(tmpDir, "sessions.json");
      const store: Record<string, SessionEntry> = {};
      const now = Date.now();
