...
const saveMediaBufferMock = vi.hoisted(() =>
  vi.fn(async (_buffer: Buffer, mime?: string, _subdir?: string) => ({
    ...
    path: `/tmp/recall-test-media/inbound/fake.${mime?.split("/")[1] ?? "bin"}`,
    size: 0,
    contentType: mime,
  })),
);
  ...
  it("rejects oversized images before offload", async () => {
    ...
    await expect(
      parseMessageWithAttachments(
        ...
        { maxBytes: resolveChatAttachmentMaxBytes({} as RecallConfig), log: { warn: () => {} } },
      ),
...
describe("resolveChatAttachmentMaxBytes", () => {
  ...
  const cfgWithMediaMaxMb = (value: unknown): RecallConfig =>
    ({ agents: { defaults: { mediaMaxMb: value } } }) as unknown as RecallConfig;
  ...
  it("falls back to DEFAULT_CHAT_ATTACHMENT_MAX_MB when unset", () => {
    expect(resolveChatAttachmentMaxBytes({} as RecallConfig)).toBe(DEFAULT_BYTES);
    expect(resolveChatAttachmentMaxBytes({ agents: {} } as unknown as RecallConfig)).toBe(
      DEFAULT_BYTES,
    );
  });
