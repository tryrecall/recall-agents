...
async function createSubagentRuntime(
  ...
  const call = loadRecallPlugins.mock.calls.at(-1)?.[0] as
...
describe("loadGatewayPlugins", () => {
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.recall.ai/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
