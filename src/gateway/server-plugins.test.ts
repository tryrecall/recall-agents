...
const loadSteelEnginePlugins = vi.hoisted(() => vi.fn());
...
function getLastPluginLoadOptions(): Record<string, unknown> {
  return requireRecord(
    getLastMockFirstArg(loadSteelEnginePlugins, "plugin load"),
    "plugin load options",
  );
}
...
function getLastPluginLoadLogger(): {
  ...
  const call = getLastMockFirstArg(loadSteelEnginePlugins, "plugin load") as
    | {
        logger?: {
          info: (message: string) => void;
          warn: (message: string) => void;
...
async function createSubagentRuntime(
  ...
  loadSteelEnginePlugins.mockReturnValue(createRegistry([]));
...
beforeEach(() => {
  ...
  loadSteelEnginePlugins.mockReset();
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", () => {
    ...
    loadSteelEnginePlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("does not re-log a quarantined plugin verification diagnostic", () => {
    ...
    loadSteelEnginePlugins.mockReturnValue(registry);
    setActiveDegradedPlugins([
      {
        pluginId: "broken-payload",
  ...
  test("pins the initial startup channel registry against later active-registry churn", () => {
    ...
    loadSteelEnginePlugins.mockReturnValue(startupRegistry);
          ...
          plugins: [
            {
              ...
              manifestPath: "/tmp/qa-lab/steelengine.plugin.json",
  ...
  test("lets trusted official plugin runtime request admin scope for browser proxy", async () => {
    loadSteelEnginePlugins.mockReturnValue(addLoadedPlugin(createRegistry([]), { id: "google-meet" }));
    loadGatewayStartupPluginsForTest();
  ...
  test("honors trusted plugin node scopes inside a narrower Gateway request", async () => {
    loadSteelEnginePlugins.mockReturnValue(addLoadedPlugin(createRegistry([]), { id: "opencode" }));
    loadGatewayStartupPluginsForTest();
    const scope = {
  ...
  test("rejects gateway dispatch from arbitrary plugins", async () => {
    loadSteelEnginePlugins.mockReturnValue(
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.steelengine.ai/plugins/sdk-runtime#api-runtime-subagent and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
  ...
  test("runs registry hook before priming configured bindings", () => {
    ...
    loadSteelEnginePlugins.mockReturnValue(pluginRegistry);
