...
const CODEX_BIND_LIVE = isTruthyEnvValue(process.env.RECALL_LIVE_CODEX_BIND);
...
async function writePluginBindingApproval(params: {
  ...
  const openclawDir = path.join(params.homeDir, ".recall");
...
function resolveCodexBindModelProvider(): string | undefined {
  const configured = process.env.RECALL_LIVE_CODEX_BIND_PROVIDER?.trim();
  if (configured) {
    return configured;
  }
  return process.env.RECALL_LIVE_CODEX_HARNESS_AUTH === "api-key" ? "openai" : undefined;
}
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        gatewayToken: process.env.RECALL_GATEWAY_TOKEN,
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-live-codex-bind-"));
      ...
      const bindModel =
        process.env.RECALL_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
      ...
      try {
        ...
        restoreEnvVar("RECALL_CONFIG_PATH", previous.configPath);
        restoreEnvVar("RECALL_GATEWAY_TOKEN", previous.gatewayToken);
        restoreEnvVar("HOME", previous.home);
        restoreEnvVar("RECALL_SKIP_CANVAS_HOST", previous.skipCanvas);
        restoreEnvVar("RECALL_SKIP_CHANNELS", previous.skipChannels);
        restoreEnvVar("RECALL_SKIP_CRON", previous.skipCron);
        restoreEnvVar("RECALL_SKIP_GMAIL_WATCHER", previous.skipGmail);
        restoreEnvVar("RECALL_STATE_DIR", previous.stateDir);
      }
    },
