...
function asPluginInstallConfig(records: PluginInstallRecords): RecallConfig {
  return {
    plugins: {
      installs: records,
    },
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: RecallConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: RecallConfig) => void | Promise<void>;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: RecallConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (
    ...
    nextCompareConfig: RecallConfig,
    afterWrite?: ConfigWriteNotification["afterWrite"],
  ) => {
