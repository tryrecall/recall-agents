...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.RECALL_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: RecallConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: RecallConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: RecallConfig, requestedAgentId: string) => {
