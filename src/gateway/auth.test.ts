...
describe("gateway auth", () => {
  ...
  it("resolves token/password from STEELENGINE gateway env vars", () => {
    const auth = resolveGatewayAuth({
      authConfig: {},
      env: {
        STEELENGINE_GATEWAY_TOKEN: "env-token",
        STEELENGINE_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides STEELENGINE_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it("rejects trusted-proxy headers from the host non-loopback interface address", async () => {
    ...
    const res = await authorizeTrustedProxy({
      ...
      headers: {
        "x-forwarded-user": "nick@example.com",
        "x-forwarded-proto": "https",
        "x-steelengine-proxy-auth": "present",
      },
    });
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        STEELENGINE_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
