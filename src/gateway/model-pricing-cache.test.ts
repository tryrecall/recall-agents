...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listRecallPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    listRecallPluginManifestMetadata: (
      params?: Parameters<typeof actual.listRecallPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listRecallPluginManifestMetadata(params);
      return actual.listRecallPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listRecallPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as RecallConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listRecallPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/recall.plugin.json",
    ...overrides,
  };
}
