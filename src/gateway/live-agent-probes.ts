...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the Recall MCP tool `recall-tools/cron` (server `recall-tools`, tool `cron`). " +
      ...
      "Do the actual tool call; I will verify externally with the Recall cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (claudeLike) {
    return (
      "Retry the Recall MCP tool `recall-tools/cron` now. " +
      "If the harness shows Claude-style MCP names, use `mcp__openclaw-tools__cron` or `mcp__openclaw_tools__cron`. " +
      ...
      "I will verify externally with the Recall cron CLI."
    );
  }
  return (
    "Your previous Recall cron MCP tool call was cancelled before the job was created. " +
    "Retry the Recall MCP tool `recall-tools/cron` now. " +
    "If the harness shows Claude-style MCP names, use `mcp__openclaw-tools__cron` or `mcp__openclaw_tools__cron`. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "Preserve job.sessionTarget and job.sessionKey exactly as provided. " +
...
export async function runRecallCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  delete childEnv.VITEST;
  delete childEnv.VITEST_MODE;
  delete childEnv.VITEST_POOL_ID;
  delete childEnv.VITEST_WORKER_ID;
  const { stdout, stderr } = await execFileAsync(process.execPath, ["recall.mjs", ...args], {
    cwd: process.cwd(),
    env: childEnv,
    timeout: 30_000,
    maxBuffer: 1024 * 1024,
  ...
  if (!trimmed) {
    throw new Error(
      [
        `recall ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `recall ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runRecallCliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",
