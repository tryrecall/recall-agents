...
import { clearRuntimeConfigSnapshot, type RecallConfig } from "../config/config.js";
...
const CLI_DEBUG = isTruthyEnvValue(process.env.RECALL_LIVE_CLI_BACKEND_DEBUG);
const CLI_CI_SAFE_CODEX_CONFIG = isTruthyEnvValue(
  process.env.RECALL_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG,
);
const CLI_MCP_SCHEMA_PROBE = isTruthyEnvValue(
  process.env.RECALL_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE,
);
...
const CLI_BACKEND_REQUEST_TIMEOUT_MS = parsePositiveIntegerEnv(
  "RECALL_LIVE_CLI_BACKEND_REQUEST_TIMEOUT_MS",
  15 * 60_000,
);
...
function openAiProviderConfigForCodexCli(
  modelKey: string,
): NonNullable<NonNullable<RecallConfig["models"]>["providers"]>["openai"] {
  ...
  it(
    ...
    async () => {
      const preservedEnv = new Set(
        parseJsonStringArray(
          "RECALL_LIVE_CLI_BACKEND_PRESERVE_ENV",
          process.env.RECALL_LIVE_CLI_BACKEND_PRESERVE_ENV,
        ) ?? [],
      );
      ...
      const cliImageArg =
        process.env.RECALL_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || providerDefaults?.imageArg;
      const cliImageMode =
        parseImageMode(process.env.RECALL_LIVE_CLI_BACKEND_IMAGE_MODE) ??
      ...
      const cfgWithCliBackends = cfg as RecallConfig & {
        agents?: {
          defaults?: {
