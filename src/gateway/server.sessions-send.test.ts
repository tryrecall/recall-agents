...
const { createRecallTools } = await import("../agents/recall-tools.js");
...
type SessionSendTool = ReturnType<typeof createRecallTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createRecallTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["RECALL_GATEWAY_PORT", "RECALL_GATEWAY_TOKEN"]);
  ...
  process.env.RECALL_GATEWAY_PORT = String(gatewayPort);
  process.env.RECALL_GATEWAY_TOKEN = gatewayToken;
