...
const { createSteelEngineTools } = await import("../agents/steelengine-tools.js");
...
type SessionSendTool = ReturnType<typeof createSteelEngineTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createSteelEngineTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["STEELENGINE_GATEWAY_PORT", "STEELENGINE_GATEWAY_TOKEN"]);
  ...
  process.env.STEELENGINE_GATEWAY_PORT = String(gatewayPort);
  process.env.STEELENGINE_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  ...
  it(
    ...
    async () => {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-sessions-send-route-"));
  ...
  it(
    ...
    { timeout: SESSION_SEND_E2E_TIMEOUT_MS },
    async () => {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-sessions-send-mirror-"));
