...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("steelengine doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(async () => {
    ...
    delete process.env.STEELENGINE_PROXY_ACTIVE;
    delete process.env.STEELENGINE_PROXY_LOOPBACK_MODE;
  ...
  it("allows ws:// hostnames with STEELENGINE_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.STEELENGINE_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://steelengine-gateway.ai:18789",
      onConnectError,
    });
...
describe("GatewayClient close handling", () => {
  ...
  it("clears stale token on device token mismatch close", () => {
    const onClose = vi.fn();
    const env = { STEELENGINE_HOME: "/tmp/custom-steelengine-home" };
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      STEELENGINE_STATE_DIR: "/tmp/steelengine-client-service-state",
