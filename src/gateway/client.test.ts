...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("recall doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(async () => {
    ...
    delete process.env.RECALL_PROXY_ACTIVE;
    delete process.env.RECALL_PROXY_LOOPBACK_MODE;
  ...
  it("allows ws:// hostnames with RECALL_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.RECALL_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://recall-gateway.ai:18789",
      onConnectError,
    });
...
describe("GatewayClient close handling", () => {
  ...
  it("clears stale token on device token mismatch close", () => {
    const onClose = vi.fn();
    const env = { RECALL_HOME: "/tmp/custom-recall-home" };
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      RECALL_STATE_DIR: "/tmp/recall-client-service-state",
