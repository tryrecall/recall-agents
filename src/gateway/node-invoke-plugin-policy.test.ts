...
import type { SteelEnginePluginNodeInvokePolicyContext } from "../plugins/types.js";
...
function createApprovalRequestPolicy(params?: {
  ...
  return createDemoPolicy(async (ctx: SteelEnginePluginNodeInvokePolicyContext) => {
...
describe("applyPluginNodeInvokePolicy", () => {
  ...
  it("uses a matching plugin policy when one is registered", async () => {
    setDangerousDemoCommandRegistry([
      createDemoPolicy((ctx: SteelEnginePluginNodeInvokePolicyContext) => ctx.invokeNode()),
    ]);
    const { context, invoke } = createContext();
  ...
  it("fails closed before routing an unrenderable persistent policy approval", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-node-policy-approval-"));
