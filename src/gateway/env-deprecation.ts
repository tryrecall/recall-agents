...
// Emits a one-shot notice for ignored pre-SteelEngine environment prefixes.
...
export function warnLegacySteelEngineEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but SteelEngine only reads STEELENGINE_* names now.`,
      "Rename them by replacing the legacy prefix with STEELENGINE_; the old names are ignored.",
    ].join("\n"),
    { code: "STEELENGINE_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}
