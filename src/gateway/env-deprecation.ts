...
export function warnLegacyRecallEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but Recall only reads RECALL_* names now.`,
      "Rename them by replacing the legacy prefix with RECALL_; the old names are ignored.",
    ].join("\n"),
    { code: "RECALL_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}

export function resetLegacyRecallEnvWarningForTest(): void {
  warned = false;
}
