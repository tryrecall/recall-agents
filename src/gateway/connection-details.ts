...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: SteelEngineConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride =
    cliUrlOverride || options.ignoreEnvUrlOverride || options.localPortOverride !== undefined
      ? undefined
      : normalizeOptionalString(process.env.STEELENGINE_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.STEELENGINE_ALLOW_INSECURE_PRIVATE_WS === "1";
