...
const mocks = vi.hoisted(() => ({
  ...
  resolveWideAreaDiscoveryDomain: vi.fn(() => "recall.internal."),
  writeWideAreaGatewayZone: vi.fn<WriteWideAreaGatewayZone>(async () => ({
    changed: true,
    zonePath: "/tmp/recall.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (Recall)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/recall"),
...
describe("startGatewayDiscovery", () => {
  ...
  it("starts registered local discovery services with gateway advertisement context", async () => {
    ...
    process.env.RECALL_SSH_PORT = "2222";
  ...
  it("continues startup when a local discovery service never settles", async () => {
    ...
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.RECALL_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = "10";
  ...
  it("skips local discovery services for truthy RECALL_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.RECALL_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "recall.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
    ...
    expect(zoneParams.domain).toBe("recall.internal.");
    expect(zoneParams.gatewayPort).toBe(18789);
    expect(zoneParams.gatewayDirectReachable).toBe(true);
    expect(zoneParams.displayName).toBe("Lab Mac (Recall)");
    ...
    expect(logs.info.mock.calls).toEqual([
      ["wide-area DNS-SD updated (recall.internal. → /tmp/recall.internal.db)"],
    ]);
    expect(result.bonjourStop).toBeNull();
  });
