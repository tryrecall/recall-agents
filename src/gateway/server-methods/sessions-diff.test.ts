...
function initRepo(root: string): void {
  ...
  git(root, "config", "user.email", "test@steelengine.test");
...
describe("loadSessionDiff", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    repoRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sessions-diff-")));
  ...
  it("withholds patch content for hardlinked files pointing outside the checkout", async () => {
    const secretDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-secret-")));
