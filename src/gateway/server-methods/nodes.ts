...
function resolveRelayNodePushConfig(cfg: RecallConfig) {
...
export async function maybeWakeNodeWithApns(
  nodeId: string,
  opts?: { force?: boolean; wakeReason?: string; cfg?: RecallConfig },
  ...
  try {
    ...
    if (registration.transport === "relay") {
      ...
      result = await sendApnsAlert({
        registration,
        nodeId,
        title: "Recall needs a quick reopen",
        body: "Tap to reopen Recall and restore the node connection.",
        relayConfig: relay.relayConfig,
      });
    } else {
      const auth = await resolveDirectNodePushConfig();
