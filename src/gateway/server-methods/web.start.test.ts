...
describe("webHandlers web.login.start", () => {
  ...
  it("surfaces the missing official external plugin hint when no web-login provider is loaded", async () => {
    ...
    mocks.resolveMissingOfficialExternalChannelPluginRepairHint.mockReturnValue({
      ...
      repairHint:
        "Install the official external plugin with: steelengine plugins install clawhub:@steelengine/whatsapp, or run: steelengine doctor --fix.",
    });
    const respond = vi.fn();
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        message:
          "web login provider is not available. Install the official external plugin with: steelengine plugins install clawhub:@steelengine/whatsapp, or run: steelengine doctor --fix.",
      }),
    );
  ...
  it("joins multiple missing official external plugin hints when more than one configured channel is missing", async () => {
        ...
        channelId === "whatsapp"
          ...
          : channelId === "signal"
            ? {
                ...
                channelId: "signal",
                label: "Signal",
                installSpec: "clawhub:@steelengine/signal",
                installCommand: "steelengine plugins install clawhub:@steelengine/signal",
                doctorFixCommand: "steelengine doctor --fix",
                repairHint:
                  "Install the official external plugin with: steelengine plugins install clawhub:@steelengine/signal, or run: steelengine doctor --fix.",
              }
            : null,
    );
    const respond = vi.fn();
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        code: "INVALID_REQUEST",
        message:
          "web login provider is not available. Configured official external channel plugins are missing for WhatsApp, Signal. Install them with: steelengine plugins install clawhub:@steelengine/whatsapp; steelengine plugins install clawhub:@steelengine/signal, or run: steelengine doctor --fix.",
      }),
    );
  });
