...
import {
  ...
  fetchSteelEngineSkillSecurityVerdicts,
...
export const skillsHandlers: GatewayRequestHandlers = {
  ...
  "skills.securityVerdicts": async ({ params, respond, context }) => {
    ...
    try {
      ...
      if (targets.length === 0) {
        respond(true, { schema: "steelengine.skills.security-verdicts.v1", items: [] }, undefined);
        return;
      }
      const items = await fetchSteelEngineSkillSecurityVerdicts(targets);
      respond(true, { schema: "steelengine.skills.security-verdicts.v1", items }, undefined);
    } catch (err) {
