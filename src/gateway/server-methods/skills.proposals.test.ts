...
vi.mock("../../skills/security/clawhub-verdicts.js", () => ({
  ...
  fetchSteelEngineSkillSecurityVerdicts: vi.fn(),
}));
...
describe("skills proposal gateway handlers", () => {
  beforeEach(async () => {
    testState = await createSteelEngineTestState({
      layout: "state-only",
      prefix: "steelengine-skills-proposals-gateway-state-",
    });
    ...
    mocks.workspaceDir = await tempDirs.make("steelengine-skills-proposals-gateway-");
  ...
  it("scopes list and inspect to the resolved agent workspace", async () => {
    ...
    const secondWorkspaceDir = await tempDirs.make("steelengine-skills-proposals-gateway-second-");
