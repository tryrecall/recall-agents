...
const getRuntimeConfig = vi.hoisted(() => vi.fn(() => ({}) as SteelEngineConfig));
const resolveDefaultAgentId = vi.hoisted(() => vi.fn(() => "main"));
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg: SteelEngineConfig, _agentId: string) => "/tmp/steelengine"),
);
const resolveMemorySearchConfig = vi.hoisted(() =>
  vi.fn<(_cfg: SteelEngineConfig, _agentId: string) => { enabled: boolean } | null>(() => ({
    enabled: true,
  })),
);
...
describe("doctor.memory.status", () => {
  beforeEach(() => {
    ...
    resolveAgentWorkspaceDir.mockReset().mockReturnValue("/tmp/steelengine");
  ...
  it("includes dreaming counts and managed cron status when workspace data is available", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((cfg: SteelEngineConfig, agentId: string) => {
      if (agentId === "alpha") {
  ...
  it("scopes dreaming status to the requested agent workspace", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((_cfg: SteelEngineConfig, agentId: string) => {
      if (agentId === "alpha") {
        return alphaWorkspaceDir;
      }
      return mainWorkspaceDir;
...
describe("doctor.memory dream actions", () => {
  it("clears grounded-only staged short-term entries without touching the diary", async () => {
    resolveAgentWorkspaceDir.mockReturnValue("/tmp/steelengine");
    removeGroundedShortTermCandidates.mockResolvedValue({
      removed: 3,
      storePath: "/tmp/steelengine/memory/.dreams/short-term-recall.json",
    });
    const respond = vi.fn();
  ...
  it("repairs contaminated dreaming artifacts for control-ui callers", async () => {
    ...
    repairDreamingArtifacts.mockResolvedValue({
      changed: true,
      archiveDir: "/tmp/steelengine/.steelengine-repair/dreaming/2026-04-11T22-00-00-000Z",
  ...
  it("dedupes exact dream diary duplicates for control-ui callers", async () => {
    ...
    dedupeDreamDiaryEntries.mockResolvedValue({
      dreamsPath: "/tmp/steelengine/DREAMS.md",
      removed: 2,
      kept: 7,
    });
    const respond = vi.fn();
...
describe("doctor.memory.dreamDiary", () => {
  ...
  it("reads DREAMS.md for the requested agent", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((_cfg, agentId) =>
      agentId === "research-analyst" ? workspaceDir : "/tmp/steelengine",
    );
    const respond = vi.fn();
...
describe("doctor.memory.remHarness", () => {
  const makeHarnessPreview = (
    ...
    workspaceDir: overrides.workspaceDir ?? "/tmp/steelengine",
    nowMs: 0,
    remConfig: {
      enabled: true,
      lookbackDays: 7,
  ...
  beforeEach(() => {
    getRuntimeConfig.mockClear().mockReturnValue({} as SteelEngineConfig);
