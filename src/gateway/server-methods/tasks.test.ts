...
beforeEach(async () => {
  stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-gateway-tasks-"));
...
describe("tasks gateway handlers", () => {
  ...
  it("sanitizes task text before exposing SDK summaries", async () => {
    const task = createTaskRecord({
      ...
      runId: "run-sanitized",
      label:
        "Compile artifact\nSteelEngine runtime context (internal): Keep internal details private.",
    ...
    expect(JSON.stringify(calls[0]?.[1])).not.toContain("SteelEngine runtime context");
  });
