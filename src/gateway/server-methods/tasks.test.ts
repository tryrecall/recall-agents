...
beforeEach(async () => {
  stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-gateway-tasks-"));
...
describe("tasks gateway handlers", () => {
  ...
  it("sanitizes task text before exposing SDK summaries", async () => {
    const task = createTaskRecord({
      ...
      runId: "run-sanitized",
      label:
        "Compile artifact\nRecall runtime context (internal): Keep internal details private.",
    ...
    expect(JSON.stringify(calls[0]?.[1])).not.toContain("Recall runtime context");
  });
