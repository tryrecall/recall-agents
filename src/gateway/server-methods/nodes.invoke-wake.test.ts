...
describe("plugin surface refresh", () => {
  ...
  it("refreshes generic plugin surface capability urls", async () => {
    ...
    await expectDefined(
      ...
      params: {
        surface: "canvas",
        observedUrl: "https://gateway.example/__steelengine__/cap/old-token",
      },
    ...
    expect(parsedCanvasUrl.pathname.startsWith("/__steelengine__/cap/")).toBe(true);
    const capabilityToken = parsedCanvasUrl.pathname.slice("/__steelengine__/cap/".length);
  ...
  it("reuses a capability rotated after the caller observed its surface", async () => {
    ...
    vi.setSystemTime(1_000);
    const respond = vi.fn();
    const currentUrl = "http://127.0.0.1:18789/__steelengine__/cap/current-token";
    const client = {
      connect: {
        client: { id: "node-1", mode: "node" },
      },
...
describe("node.invoke APNs wake path", () => {
  ...
  it("explains when a declared node command surface awaits approval", async () => {
    ...
    expect(call[0]).toBe(false);
    expect(call[2]?.message).toBe(
      "node command not allowed: the node's declared command surface is pending approval; run `steelengine nodes pending`, then `steelengine nodes approve <requestId>`",
    );
    expect(nodeRegistry.invoke).not.toHaveBeenCalled();
  });
