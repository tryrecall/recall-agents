...
describe("gateway agent handler", () => {
  ...
  it("does not pass a text-only user-turn recorder for offloaded image agent runs", async () => {
    await withTempDir({ prefix: "steelengine-gateway-agent-offloaded-image-" }, async (root) => {
  ...
  it("drops a stale transcript path when a stale session rotates ids", async () => {
    ...
    const staleEntry = {
      ...
      sessionFile: "/tmp/steelengine/agents/main/sessions/old-session-id.jsonl",
      updatedAt: 0,
      sessionStartedAt: 0,
    };
    mockMainSessionEntry(staleEntry);
  ...
  it("rotates a failed session instead of resuming when its transcript is missing", async () => {
    ...
    const missingTranscriptEntry = {
      ...
      sessionFile: "/tmp/steelengine/missing/failed-missing-session-id.jsonl",
      status: "failed",
      updatedAt: now,
  ...
  it.each([
    ...
    async (scenario) => {
      ...
      await withTempDir({ prefix: "steelengine-gateway-terminal-main-newer-" }, async (root) => {
  ...
  it("reuses terminal main sessions when the fresh store row has the transcript marker", async () => {
    ...
    await withTempDir({ prefix: "steelengine-gateway-terminal-main-fresh-marker-" }, async (root) => {
      const sessionsDir = `${root}/sessions`;
  ...
  it("honors explicit gateway session-id resumes for terminal main rows", async () => {
    ...
    await withTempDir(
      { prefix: "steelengine-gateway-terminal-main-explicit-resume-" },
      async (root) => {
        const sessionsDir = `${root}/sessions`;
        await fs.mkdir(sessionsDir, { recursive: true });
        const sessionFile = "terminal-main-session.jsonl";
  ...
  it.each(["heartbeat", "cron"] as const)(
    ...
    async (runKind) => {
      ...
      await withTempDir(
        { prefix: `steelengine-gateway-terminal-main-${runKind}-reuse-` },
        async (root) => {
          const sessionsDir = `${root}/sessions`;
          await fs.mkdir(sessionsDir, { recursive: true });
