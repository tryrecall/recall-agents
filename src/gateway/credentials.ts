  ...
  constructor(path: string) {
    super(
      [
        ...
        "Fix: set RECALL_GATEWAY_TOKEN/RECALL_GATEWAY_PASSWORD, pass explicit --token/--password,",
...
export function resolveGatewayCredentialsFromValues(params: {
  ...
  const envPassword = trimToUndefined(env.RECALL_GATEWAY_PASSWORD);
...
export function resolveGatewayCredentialsFromConfig(params: {
  ...
  const localTokenPrecedence =
    ...
    (env.RECALL_SERVICE_KIND === "gateway" ? "config-first" : "env-first");
