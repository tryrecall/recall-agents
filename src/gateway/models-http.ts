...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([RECALL_MODEL_ID, RECALL_DEFAULT_MODEL_ID]);
  ids.add(`recall/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`recall/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== RECALL_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
