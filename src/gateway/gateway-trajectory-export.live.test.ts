...
const CODEX_HARNESS_LIVE = process.env.STEELENGINE_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.STEELENGINE_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["STEELENGINE_TRAJECTORY", "STEELENGINE_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-steelengine-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.STEELENGINE_TRAJECTORY = "1";
      process.env.STEELENGINE_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".steelengine", "trajectory-exports", "bundle");
