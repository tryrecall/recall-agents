...
const CODEX_HARNESS_LIVE = process.env.RECALL_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.RECALL_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["RECALL_TRAJECTORY", "RECALL_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-recall-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.RECALL_TRAJECTORY = "1";
      process.env.RECALL_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".recall", "trajectory-exports", "bundle");
