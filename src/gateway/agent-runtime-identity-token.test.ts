...
const envSnapshot = captureEnv(["HOME", "STEELENGINE_HOME", "STEELENGINE_STATE_DIR"]);

const tempHomes: string[] = [];

function useTempHome(): string {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-agent-runtime-"));
  tempHomes.push(home);
  setTestEnvValue("HOME", home);
  setTestEnvValue("STEELENGINE_HOME", home);
  setTestEnvValue("STEELENGINE_STATE_DIR", "");
  return home;
}
