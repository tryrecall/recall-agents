...
const ROOT_PREFIX = "/";
const CONTROL_UI_ASSISTANT_MEDIA_PREFIX = "/__steelengine__/assistant-media";
...
const CONTROL_UI_NAMESPACE_PREFIX = "/__steelengine__/";
...
// `/__steelengine__/`, so a browser that opens the default entry infers
// `/__steelengine__` as its base path (see `inferBasePathFromPathname`) and fetches
// `/__steelengine__/control-ui-config.json`. Accept that namespaced alias so the
...
// Single-underscore `/__steelengine` prefix used by the pre-base-path-relative
...
// `/__steelengine/control-ui-config.json`, so current main and the v2026.6.1
// release serve and document that exact path under an empty base path.
const LEGACY_CONTROL_UI_NAMESPACE_PREFIX = "/__steelengine";

// The old documented no-base-path bootstrap endpoint
// (`/__steelengine/control-ui-config.json`, single underscore). It is derived from
// the legacy `/__steelengine` namespace joined with the canonical config constant
 ...
 * single-underscore suffix `${basePath}/__steelengine/control-ui-config.json` that
 ...
 * the default-namespace alias `/__steelengine__/control-ui-config.json`, which is
 * what the default `/__steelengine__/` entry requests after inferring its base path
...
function matchesControlUiBootstrapConfigPath(pathname: string, basePath: string): boolean {
  ...
  // `/__steelengine` namespace, so joining it with the base path yields
  // `${basePath}/__steelengine/control-ui-config.json` (or the bare legacy path
  ...
  // The default `/__steelengine__/` namespace alias only applies when no base path
