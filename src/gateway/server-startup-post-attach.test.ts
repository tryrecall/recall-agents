...
const hoisted = vi.hoisted(() => {
  ...
  const resolveDefaultAgentDir = vi.fn(() => "/tmp/recall-state/agents/default/agent");
  ...
  const ensureRecallModelsJson = vi.fn(async () => {});
...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/recall-state",
    resolveConfigPath: vi.fn(() => "/tmp/recall-state/recall.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn(() => "/tmp/recall-state"),
  };
});
...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: vi.fn(() => "/tmp/recall-workspace"),
...
vi.mock("../agents/models-config.js", () => ({
  ensureRecallModelsJson: hoisted.ensureRecallModelsJson,
}));
...
function firstEnsureModelsJsonCall(): [
  ...
  const call = hoisted.ensureRecallModelsJson.mock.calls[0];
  if (!call || call.length < 3) {
    throw new Error("expected ensureRecallModelsJson call");
  }
...
describe("startGatewayPostAttachRuntime", () => {
  beforeEach(() => {
    vi.stubEnv("RECALL_SKIP_CHANNELS", "0");
    vi.stubEnv("RECALL_SKIP_PROVIDERS", "0");
    ...
    hoisted.ensureRecallModelsJson.mockReset();
    hoisted.ensureRecallModelsJson.mockResolvedValue(undefined);
  ...
  it("skips heavy restart sentinel refresh when no sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-no-sentinel-"));
  ...
  it("refreshes the restart sentinel when the sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-sentinel-"));
  ...
  it("expands tilde-based restart sentinel state paths", async () => {
    const osHome = fs.mkdtempSync(path.join(os.tmpdir(), "recall-home-"));
    try {
      const openclawHome = path.join(osHome, "recall-home");
      const stateDirFromHome = path.join(openclawHome, ".recall");
      ...
      const backslashStateDir = path.resolve(`${osHome}\\recall-state`);
  ...
  it("avoids sync filesystem probes while checking restart sentinel presence", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-async-sentinel-"));
    try {
      fs.writeFileSync(path.join(stateDir, "restart-sentinel.json"), "{}\n");
  ...
  it("defers qmd memory backend startup refresh until the idle delay elapses", async () => {
    ...
    try {
      await startGatewaySidecars({
        ...
        defaultWorkspaceDir: "/tmp/recall-workspace",
        deps: {} as never,
  ...
  it("prewarms models.json in the configured default agent dir", async () => {
    ...
    hoisted.resolveDefaultAgentDir.mockReturnValue("/tmp/recall-state/agents/ops/agent");
    ...
    expect(hoisted.ensureRecallModelsJson).toHaveBeenCalledTimes(1);
    const ensureCall = firstEnsureModelsJsonCall();
    expect(ensureCall[0]).toBe(cfg);
    expect(ensureCall[1]).toBe("/tmp/recall-state/agents/ops/agent");
    const options = ensureCall[2];
    expect(options?.workspaceDir).toBe("/tmp/recall-workspace");
  ...
  it("starts channels without waiting for primary model prewarm completion", async () => {
    await withEnvAsync(
      { RECALL_SKIP_CHANNELS: undefined, RECALL_SKIP_PROVIDERS: undefined },
      async () => {
  ...
  it("emits a startup trace span when channel startup is skipped", async () => {
    ...
    await withEnvAsync(
      { RECALL_SKIP_CHANNELS: "1", RECALL_SKIP_PROVIDERS: undefined },
      async () => {
        await startGatewaySidecars({
    ...
    expect(logChannels.info).toHaveBeenCalledWith(
      "skipping channel start (RECALL_SKIP_CHANNELS=1 or RECALL_SKIP_PROVIDERS=1)",
    );
  });
