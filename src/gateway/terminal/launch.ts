...
type TerminalLaunchPolicy = {
  ...
  isEnabled: () => boolean;
  prepareConfig: (config: SteelEngineConfig, options: { restartPending: boolean }) => void;
  commitConfig: () => void;
...
export function createTerminalLaunchPolicy(initialConfig: SteelEngineConfig): TerminalLaunchPolicy {
  let activeConfig = initialConfig;
  let hasPendingRestart = false;
  let terminalDisabledUntilRestart = false;
  let preparedConfig: SteelEngineConfig | null = null;
  let appliedConfigWhileRestartPending: SteelEngineConfig | null = null;
  ...
  const preserveTerminalConfig = (config: SteelEngineConfig, owner: SteelEngineConfig) => {
  ...
  const resolveForConfig = (config: SteelEngineConfig, agentId?: string) => {
  ...
  const accumulateRestartRestrictions = (config: SteelEngineConfig) => {
  ...
  const accumulateCommitRestrictions = (config: SteelEngineConfig) => {
    if (config.gateway?.terminal?.enabled !== true) {
...
export function buildTerminalEnv(baseEnv: NodeJS.ProcessEnv): Record<string, string> {
  ...
  // Lets shells and prompts detect that they are inside an SteelEngine terminal.
  env.STEELENGINE_TERMINAL = "1";
  return env;
}
