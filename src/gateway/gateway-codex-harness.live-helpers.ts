export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current Recall session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsRecallStatus =
    normalized.includes("recall is running on") ||
    /recall\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("recall status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsRecallStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from recall status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live recall config shows") ||
