...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "recall-test-empty-bundled-plugins");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".recall", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["RECALL_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  const workspaceDir = path.join(tempHome, "recall");
  await fs.mkdir(workspaceDir, { recursive: true });
  process.env.RECALL_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const configDir = path.join(tempHome, ".recall");
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "recall-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "recall-wizard-home-"));
      const configPath = path.join(tempHome, ".recall", "recall.json");
      process.env.HOME = tempHome;
      process.env.RECALL_STATE_DIR = path.join(tempHome, ".recall");
      process.env.RECALL_CONFIG_PATH = configPath;
      process.env.RECALL_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "recall-minimal-gateway-home-"));
      const configPath = path.join(tempHome, ".recall", "recall.json");
      const bundledPluginsDir = path.join(tempHome, "recall-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.RECALL_STATE_DIR = path.join(tempHome, ".recall");
      process.env.RECALL_CONFIG_PATH = configPath;
      process.env.RECALL_SKIP_CHANNELS = "1";
      ...
      process.env.RECALL_SKIP_BROWSER_CONTROL_SERVER = "1";
      process.env.RECALL_SKIP_PROVIDERS = "1";
      process.env.RECALL_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
      process.env.RECALL_TEST_MINIMAL_GATEWAY = "1";
