...
vi.mock("../agents/models-config.js", () => ({
  ensureRecallModelsJson: (config: unknown, agentDir: unknown, options?: unknown) =>
    ensureRecallModelsJsonMock(config, agentDir, options),
}));
...
function expectModelsJsonPrewarmCall(cfg: RecallConfig) {
  expect(ensureRecallModelsJsonMock).toHaveBeenCalledTimes(1);
  const [calledConfig, agentDir, options] = ensureRecallModelsJsonMock.mock.calls.at(0) ?? [];
...
describe("gateway startup primary model warmup", () => {
  ...
  it("honors the startup model prewarm skip env", () => {
    ...
    expect(
      shouldSkipStartupModelPrewarm({
        RECALL_SKIP_STARTUP_MODEL_PREWARM: "true",
      }),
    ).toBe(true);
  });
  ...
  it("warns when scoped models.json preparation fails", async () => {
    ensureRecallModelsJsonMock.mockRejectedValueOnce(new Error("models write failed"));
    const warn = vi.fn();
