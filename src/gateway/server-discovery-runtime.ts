...
function resolveDiscoveryAdvertiseTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw = env.RECALL_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS?.trim();
  if (!raw) {
...
export async function startGatewayDiscovery(params: {
  ...
  const localDiscoveryEnabled =
    mdnsMode !== "off" &&
    !isTruthyEnvValue(process.env.RECALL_DISABLE_BONJOUR) &&
  ...
  const tailnetDns = needsTailnetDns
    ...
    : undefined;
  const sshPortEnv = mdnsMinimal ? undefined : process.env.RECALL_SSH_PORT?.trim();
