...
describe("native Gateway protocol levels", () => {
  it("match the TypeScript source of truth", async () => {
    ...
    const androidPath = "apps/android/app/src/main/java/ai/recall/app/gateway/GatewayProtocol.kt";
  ...
  it("uses the min constant for native connect compatibility ranges", async () => {
    const swiftConnectFiles = [
      ...
      "apps/macos/Sources/RecallMacCLI/WizardCommand.swift",
    ];
    ...
    const androidPath = "apps/android/app/src/main/java/ai/recall/app/gateway/GatewaySession.kt";
    const android = await readRepoFile(androidPath);
    assertPattern(
      android,
      androidPath,
