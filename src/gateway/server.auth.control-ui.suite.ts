...
export function registerControlUiAndPairingSuite(): void {
  ...
  const startServerWithOperatorIdentity = async (identityPrefix = "recall-device-scope-") => {
      ...
      await withGatewayServer(async ({ port }) => {
        ...
        const { device } = await createSignedDevice({
          ...
          identityPath: path.join(
            os.tmpdir(),
            `recall-controlui-device-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}-${controlUiIdentityPathSeq++}.json`,
          ),
          nonce: String(nonce),
        });
        const res = await connectReq(ws, {
