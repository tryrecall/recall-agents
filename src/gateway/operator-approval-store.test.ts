...
function createDatabaseOptions(): SteelEngineStateDatabaseOptions {
  const stateDir = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-operator-approval-")),
  );
...
function rawApprovalRow(options: SteelEngineStateDatabaseOptions, id: string) {
...
describe("operator approval store", () => {
  ...
  it("reads the default clock after waiting for the SQLite write lock", async () => {
    ...
    const databasePath = openSteelEngineStateDatabase(databaseOptions).path;
