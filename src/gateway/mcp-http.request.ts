...
function resolveScopedSessionKey(cfg: SteelEngineConfig, rawSessionKey: string | undefined): string {
...
function resolveMcpSender(params: {
  ...
  const captureKey = normalizeOptionalString(getHeader(params.req, "x-steelengine-cli-capture-key"));
  const clientGrant =
...
export function resolveMcpHttpBodyTimeoutMs(): number {
  return readPositiveIntEnv("STEELENGINE_MCP_LOOPBACK_BODY_TIMEOUT_MS", DEFAULT_MCP_BODY_TIMEOUT_MS);
}
...
export function resolveMcpCliCaptureKey(
  ...
  return normalizeOptionalString(getHeader(req, "x-steelengine-cli-capture-key"));
}
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    sessionId: normalizeOptionalString(getHeader(req, "x-steelengine-session-id")),
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-steelengine-message-channel")) ?? undefined,
    ...
    clientCaps: normalizeMcpClientCapsHeader(getHeader(req, "x-steelengine-client-caps")),
    currentChannelId: normalizeOptionalString(getHeader(req, "x-steelengine-current-channel-id")),
    currentThreadTs: normalizeOptionalString(getHeader(req, "x-steelengine-current-thread-ts")),
    currentMessageId: normalizeOptionalString(getHeader(req, "x-steelengine-current-message-id")),
    currentInboundAudio: normalizeMcpBooleanHeader(
      getHeader(req, "x-steelengine-current-inbound-audio"),
    ),
    accountId: normalizeOptionalString(getHeader(req, "x-steelengine-account-id")),
    inboundEventKind: normalizeMcpInboundEventKind(getHeader(req, "x-steelengine-inbound-event-kind")),
    sourceReplyDeliveryMode: normalizeMcpSourceReplyDeliveryMode(
      getHeader(req, "x-steelengine-source-reply-delivery-mode"),
    ),
    taskSuggestionDeliveryMode: normalizeMcpTaskSuggestionDeliveryMode(
      getHeader(req, "x-steelengine-task-suggestion-delivery-mode"),
    ),
    requireExplicitMessageTarget: normalizeMcpBooleanHeader(
      getHeader(req, "x-steelengine-require-explicit-message-target"),
    ),
    senderIsOwner: auth.senderIsOwner,
  };
}
