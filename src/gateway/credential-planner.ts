...
type GatewaySecretDefaults = NonNullable<SteelEngineConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${STEELENGINE_GATEWAY_TOKEN}` from being
...
export function hasGatewayTokenEnvCandidate(env: NodeJS.ProcessEnv = process.env): boolean {
  return Boolean(trimToUndefined(env.STEELENGINE_GATEWAY_TOKEN));
}
...
export function hasGatewayPasswordEnvCandidate(env: NodeJS.ProcessEnv = process.env): boolean {
  return Boolean(trimToUndefined(env.STEELENGINE_GATEWAY_PASSWORD));
}
...
export function createGatewayCredentialPlan(params: {
  ...
  const envPassword = trimToUndefined(env.STEELENGINE_GATEWAY_PASSWORD);
