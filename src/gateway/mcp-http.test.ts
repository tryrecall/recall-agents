  ...
  it("passes session, account, message channel, and inbound event headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-recall-account-id": "work",
        "x-recall-message-channel": "telegram",
        "x-recall-inbound-event-kind": "room_event",
      },
    ...
    const sendToolsList = async (inboundEventKind: string) =>
      await sendRaw({
        ...
        headers: {
          ...
          "x-session-key": "agent:main:telegram:group:chat123",
          "x-recall-message-channel": "telegram",
          "x-recall-inbound-event-kind": inboundEventKind,
        },
        body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      });
    ...
    const sendToolsList = async (token?: string) =>
      await sendRaw({
        ...
        headers: {
          ...
          "x-recall-message-channel": "matrix",
        },
        body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.recall?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.recall?.headers?.Authorization).toBe(
      "Bearer ${RECALL_MCP_TOKEN}",
    );
    expect(config.mcpServers?.recall?.headers?.["x-recall-message-channel"]).toBe(
      "${RECALL_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.recall?.headers).not.toHaveProperty("x-recall-sender-is-owner");
  });
});
