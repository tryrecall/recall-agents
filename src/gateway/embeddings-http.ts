...
function resolveAutoExplicitProviders(cfg: RecallConfig): Set<string> {
  return new Set(
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== RECALL_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-recall-model")) ||
