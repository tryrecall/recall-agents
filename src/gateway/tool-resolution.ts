...
import { createSteelEngineTools } from "../agents/steelengine-tools.js";
...
export function resolveGatewayScopedTools(params: {
  ...
  const steelEngineTools = createSteelEngineTools({
  ...
  const baseTools = nodeExecSurface
    ? steelEngineTools.filter((tool) => tool.name.trim().toLowerCase() !== "exec")
    : steelEngineTools;
  const allTools = includeNodeExecTool
    ? [
        ...
        createLazyExecTool(
          ...
          {
            description:
              "Execute a shell command on a connected SteelEngine node. This tool is node-only; use the CLI native shell for Gateway-local commands. Commands run synchronously. Set node when multiple nodes are available.",
