...
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("steelengine.gatewayTestHelpers.configRoot");
...
const gatewayTestHoisted = vi.hoisted(() => {
  const key = Symbol.for("steelengine.gatewayTestHelpers.hoisted");
...
export const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `steelengine-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

/** Updates the config root used by gateway config-module mocks. */
export function setTestConfigRoot(root: string): void {
  testConfigRoot.value = root;
  process.env.STEELENGINE_CONFIG_PATH = path.join(root, "steelengine.json");
}
