...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("recall doctor")}" to repair, then retry.`
    : "";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.RECALL_TEST_MINIMAL_GATEWAY === "1";
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `recall config set gateway.auth.mode token` and `recall config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  const transcriptUnsub = minimalTestGateway
    ...
    : onSessionTranscriptUpdate((update) => {
        ...
        const sessionSnapshot = sessionRow
          ...
          : {};
        const message = attachRecallTranscriptMeta(update.message, {
