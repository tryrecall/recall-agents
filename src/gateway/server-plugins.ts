...
import { clearActivatedPluginRuntimeState, loadSteelEnginePlugins } from "../plugins/loader.js";
...
const PLUGIN_SUBAGENT_POLICY_STATE_KEY: unique symbol = Symbol.for(
  "steelengine.pluginSubagentOverridePolicyState",
);
...
export function setPluginSubagentOverridePolicies(cfg: SteelEngineConfig): void {
  ...
  if (!policy?.allowModelOverride) {
    return {
      ...
      reason:
        ...
        "See https://docs.steelengine.ai/plugins/sdk-runtime#api-runtime-subagent and search for: " +
...
export function loadGatewayPlugins(params: {
  ...
  const pluginRegistry = loadSteelEnginePlugins({
    config: resolvedConfig,
