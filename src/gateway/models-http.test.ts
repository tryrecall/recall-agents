...
const READ_SCOPE_HEADER = { "x-recall-scopes": "operator.read" };
...
describe("OpenAI-compatible models HTTP API (e2e)", () => {
  it("serves /v1/models when compatibility endpoints are enabled", async () => {
    ...
    expect(json.data?.map((entry) => entry.id)).toContain("recall");
    expect(json.data?.map((entry) => entry.id)).toContain("recall/default");
    expect(
      json.data?.every((entry) => typeof entry.id === "string" && entry.id?.startsWith("recall")),
    ).toBe(true);
  });
  ...
  it("rejects operator scopes that lack read access", async () => {
    const res = await getModels("/v1/models", { "x-recall-scopes": "operator.approvals" });
    await expectMissingReadScope(res);
  });

  it("rejects requests with no declared operator scopes", async () => {
    const res = await getModels("/v1/models", { "x-recall-scopes": "" });
    await expectMissingReadScope(res);
  });
