...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-steelengine-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-steelengine-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-steelengine-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-steelengine-thread-id"));
