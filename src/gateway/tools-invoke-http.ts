...
import { createRecallTools } from "../agents/recall-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const accountId = getHeader(req, "x-recall-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-recall-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-recall-thread-id")?.trim() || undefined;
  ...
  const allTools = createRecallTools({
