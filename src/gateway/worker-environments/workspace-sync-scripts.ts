...
const leaseDirectory = path.join(os.homedir(), ".steelengine-worker", "quiescence");
...
const leasePath = path.join(os.homedir(), ".steelengine-worker", "quiescence", crypto.createHash("sha256").update(root).digest("hex") + "." + nonce + ".json");
...
git update-ref refs/heads/steelengine-worker "$base"
git symbolic-ref HEAD refs/heads/steelengine-worker
git read-tree "$base"
...
function eligiblePaths() {
  ...
  selected.delete(".steelengine-base.pack");
  ...
  for (const priorManifestDigest of priorManifestDigests) {
    ...
    const priorPath = path.join(process.env.HOME, ".steelengine-worker", "manifests", priorManifestDigest + ".json");
    ...
    for (const entry of prior.entries) {
      ...
      if (entry.path !== ".steelengine-base.pack" && !isDerivedWorkspacePath(entry.path)) {
...
async function main() {
  ...
  const workerRoot = path.join(process.env.HOME, ".steelengine-worker");
