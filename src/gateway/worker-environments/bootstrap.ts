...
const BOOTSTRAP_ROOT = ".steelengine-worker";
...
const NODE_MISSING_MARKER = "STEELENGINE_WORKER_NODE_MISSING";
const NODE_UNSUPPORTED_MARKER = "STEELENGINE_WORKER_NODE_UNSUPPORTED";
const NPM_MISSING_MARKER = "STEELENGINE_WORKER_NPM_MISSING";
const BOOTSTRAP_OUTPUT_TAG = "STEELENGINE_WORKER_BOOTSTRAP_V1";
...
try {
  ...
  const shapeMatches =
    Object.keys(actual).sort().join(",") === "bundleHash,steelengineVersion,protocolFeatures";
  const featuresMatch =
  ...
  process.exit(
    shapeMatches &&
      ...
      actual.steelengineVersion === expected.steelengineVersion &&
      featuresMatch
      ? 0
      : 1,
  );
...
function addFile(relative) {
  ...
  const mode = relative === "steelengine.mjs" || (stats.mode & 0o111) !== 0 ? 0o700 : 0o600;
...
find "$incoming" -type f -name 'steelengine-upload-*.tgz.*' -mmin +60 -exec rm -f -- {} + 2>/dev/null || true
upload=$(mktemp "$incoming/steelengine-upload-$hash.tgz.XXXXXXXX")
...
case "$install" in
  ...
  npm)
    ...
    STEELENGINE_DISABLE_PLUGIN_REGISTRY_MIGRATION=1 npm pack "$package_spec" --pack-destination "$staging" --ignore-scripts --json --registry=https://registry.npmjs.org/ > "$npm_pack_json"
    ...
    STEELENGINE_DISABLE_PLUGIN_REGISTRY_MIGRATION=1 npm install --global --prefix "$npm_prefix" --ignore-scripts --omit=dev --no-audit --no-fund "$package_archive"
    package_dir=$npm_prefix/lib/node_modules/steelengine
    if [ ! -f "$package_dir/steelengine.mjs" ]; then
      printf '%s\n' 'npm did not install the SteelEngine package root' >&2
      exit 2
    fi
...
if [ "$install" = bundle ]; then
  ...
  STEELENGINE_DISABLE_PLUGIN_REGISTRY_MIGRATION=1 npm install --prefix "$staging" --ignore-scripts --omit=dev --no-audit --no-fund >&2
fi
...
function normalizeHandshake(artifact: WorkerInstallationArtifact): WorkerAdmissionHandshake {
  ...
  const steelengineVersion = artifact.steelengineVersion.trim();
  ...
  if (!steelengineVersion) {
    throw new Error("Worker SteelEngine version must be non-empty");
  }
  if (
  ...
  if (artifact.install === "npm") {
    if (
      !isExactSemverVersion(steelengineVersion) ||
      artifact.packageSpec !== `steelengine@${steelengineVersion}`
    ) {
      throw new Error(`Worker npm install must use exact package steelengine@${steelengineVersion}`);
    }
  ...
  return { bundleHash, steelengineVersion, protocolFeatures };
}
...
function parseReceiptJson(
  ...
  if (
    ...
    parsed.steelengineVersion !== expected.steelengineVersion ||
...
export async function bootstrapWorker(
  ...
  const prepared = await prepareWorkerSsh({
    ...
    temporaryDirectoryPrefix: "steelengine-worker-bootstrap-",
  });
  try {
