...
async function writeFixture(
  ...
  await fs.writeFile(path.join(packageRoot, "steelengine.mjs"), "import './dist/entry.js';\n", {
    encoding: "utf8",
    mode: 0o755,
  });
...
function bundleArtifact(overrides: Partial<WorkerBundleArtifact> = {}): WorkerBundleArtifact {
  return {
    ...
    steelengineVersion: "1.2.3",
    protocolFeatures: [],
    tarballSha256: "b".repeat(64),
    tarballPath: "/tmp/steelengine-worker.tgz",
    ...overrides,
  };
}
...
describe("worker bundle producer", () => {
  it("hashes the same file manifest deterministically", async () => {
    await withTempDir({ prefix: "steelengine-worker-bundle-" }, async (root) => {
  ...
  it("prunes workspace deps and lifecycle fields from dev manifests", async () => {
    await withTempDir({ prefix: "steelengine-worker-bundle-prune-" }, async (root) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify({
          ...
          type: "module",
          files: ["dist/"],
          dependencies: { json5: "2.2.3", "@steelengine/gateway-protocol": "workspace:*" },
  ...
  it("vendors workspace packages that the shipped dist imports", async () => {
    await withTempDir({ prefix: "steelengine-worker-bundle-vendor-" }, async (root) => {
      const packageRoot = path.join(root, "package");
      await writeFixture(packageRoot, [
        ["dist/entry.js", 'import { fake } from "@steelengine/fake-pkg";\nexport { fake };\n'],
      ]);
      await fs.writeFile(
        path.join(packageRoot, "package.json"),
        `${JSON.stringify({
          ...
          dependencies: {
            json5: "2.2.3",
            "@steelengine/fake-pkg": "workspace:*",
            "@steelengine/gateway-protocol": "workspace:*",
          },
        })}\n`,
        "utf8",
      );
      const vendorSource = path.join(packageRoot, "node_modules/@steelengine/fake-pkg");
      await fs.mkdir(path.join(vendorSource, "dist"), { recursive: true });
      await fs.writeFile(
        path.join(vendorSource, "package.json"),
        `${JSON.stringify({
          name: "@steelengine/fake-pkg",
          version: "1.2.3",
          type: "module",
          main: "./dist/index.js",
      ...
      await expect(listTarball(bundle.tarballPath)).resolves.toEqual([
        ...
        "steelengine.mjs",
        "package.json",
        "vendor/steelengine-fake-pkg/dist/index.js",
        "vendor/steelengine-fake-pkg/package.json",
      ]);
      const extractRoot = path.join(root, "extract");
      await fs.mkdir(extractRoot, { recursive: true });
      await tar.extract({ file: bundle.tarballPath, cwd: extractRoot });
      ...
      expect(staged.dependencies).toEqual({
        json5: "2.2.3",
        "@steelengine/fake-pkg": "file:./vendor/steelengine-fake-pkg",
      });
      const vendored = JSON.parse(
        await fs.readFile(path.join(extractRoot, "vendor/steelengine-fake-pkg/package.json"), "utf8"),
      ) as Record<string, unknown>;
  ...
  it("fails closed when a dist-referenced workspace package is not installed", async () => {
    await withTempDir({ prefix: "steelengine-worker-bundle-vendor-missing-" }, async (root) => {
      const packageRoot = path.join(root, "package");
      await writeFixture(packageRoot, [
        ["dist/entry.js", 'import "@steelengine/fake-pkg";\nexport {};\n'],
      ]);
      await fs.writeFile(
        path.join(packageRoot, "package.json"),
        `${JSON.stringify({
          ...
          type: "module",
          files: ["dist/"],
          dependencies: { "@steelengine/fake-pkg": "workspace:*" },
        })}\n`,
        "utf8",
      );
      ...
      ).rejects.toThrow("cannot resolve workspace dependency @steelengine/fake-pkg");
    });
  });

  it("changes the hash when file contents change", async () => {
    await withTempDir({ prefix: "steelengine-worker-bundle-change-" }, async (root) => {
      const packageRoot = path.join(root, "package");
  ...
  it("archives the staged bytes when the source changes during packaging", async () => {
    await withTempDir({ prefix: "steelengine-worker-bundle-mutation-" }, async (root) => {
  ...
  it("owns one immutable build snapshot for its lifecycle", async () => {
    await withTempDir({ prefix: "steelengine-worker-bundle-cache-" }, async (root) => {
      const packageRoot = path.join(root, "package");
  ...
  it("retries after a failed preparation without polling a successful snapshot", async () => {
    await withTempDir({ prefix: "steelengine-worker-bundle-retry-" }, async (root) => {
      const packageRoot = path.join(root, "package");
      const producer = createWorkerBundleProducer({
        packageRoot,
        cacheDir: path.join(root, "cache"),
  ...
  it("replaces a corrupt content-addressed cache entry", async () => {
    await withTempDir({ prefix: "steelengine-worker-bundle-corrupt-" }, async (root) => {
      const packageRoot = path.join(root, "package");
      const cacheDir = path.join(root, "cache");
      await writeFixture(packageRoot, [["dist/entry.js", "export {};\n"]]);
      const first = await createWorkerBundleProducer({ packageRoot, cacheDir }).prepare();
  ...
  it.skipIf(process.platform === "win32")("rejects symlinked runtime files", async () => {
    await withTempDir({ prefix: "steelengine-worker-bundle-symlink-" }, async (root) => {
      const packageRoot = path.join(root, "package");
      await writeFixture(packageRoot, [["dist/entry.js", "export {};\n"]]);
      await fs.rename(
        path.join(packageRoot, "steelengine.mjs"),
        path.join(packageRoot, "launcher-target.mjs"),
      );
      await fs.symlink("launcher-target.mjs", path.join(packageRoot, "steelengine.mjs"));

      await expect(
        createWorkerBundleProducer({ packageRoot, cacheDir: path.join(root, "cache") }).prepare(),
      ).rejects.toThrow("Unsafe worker bundle path: steelengine.mjs");
    });
  });
});
...
describe("worker npm installation artifact", () => {
  it("uses an exact registry-proven gateway package", async () => {
    await withTempDir({ prefix: "steelengine-worker-npm-release-" }, async (packageRoot) => {
      await writeFixture(packageRoot, [["dist/entry.js", "export {};\n"]]);
  ...
  it("rejects dev and packages that fail release verification", async () => {
    const verifyRelease = vi.fn(async (): Promise<string> => {
      throw new Error("SteelEngine 1.2.3 is not published; use the worker bundle install");
    });
    await expect(
      resolveWorkerNpmInstallationArtifact({
        bundle: bundleArtifact({ steelengineVersion: "dev" }),
  ...
  it("rejects a source checkout even when its version is published", async () => {
    await withTempDir({ prefix: "steelengine-worker-npm-source-" }, async (packageRoot) => {
      await writeFixture(packageRoot, [["dist/entry.js", "export {};\n"]]);
      await fs.writeFile(path.join(packageRoot, "npm-shrinkwrap.json"), "{}\n", "utf8");
