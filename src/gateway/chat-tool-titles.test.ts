...
import { closeSteelEngineAgentDatabases } from "../state/steelengine-agent-db.js";
...
function mockPreparedModel(): void {
  prepareSimpleCompletionModelForAgent.mockResolvedValue({
    selection: { provider: "openai", modelId: "gpt-test", agentDir: "/tmp/steelengine-agent" },
...
describe("generateToolCallTitles", () => {
  ...
  beforeEach(() => {
    ...
    stateDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-tool-titles-")));
