...
function expectMessageFields(
  message: unknown,
  fields: { role?: string; content?: unknown; steelengine?: Record<string, unknown> },
) {
  ...
  if (fields.steelengine) {
    const metadata = requireRecord(record["__steelengine"], "message metadata");
    for (const [key, value] of Object.entries(fields.steelengine)) {
...
describe("readSessionTitleFieldsFromTranscript cache", () => {
  ...
  registerTempSessionStore("steelengine-session-fs-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    const marker = out[1] as {
      ...
      __steelengine?: { kind?: string; id?: string };
      timestamp?: number;
    };
    expect(marker.role).toBe("system");
    expect(marker.content?.[0]?.text).toBe("Compaction");
    expect(marker["__steelengine"]?.kind).toBe("compaction");
    expect(marker["__steelengine"]?.id).toBe("comp-1");
  ...
  test("preserves real sequence metadata for async bounded recent-message reads", async () => {
    ...
    try {
      ...
      expectMessageFields(result.messages[0], { content: "recent", steelengine: { seq: 3 } });
      expectMessageFields(result.messages[1], { content: "latest", steelengine: { seq: 4 } });
  ...
  test("forwards the outer JSONL record timestamp to __steelengine.recordTimestampMs (#85648)", async () => {
    ...
    expectMessageFields(result[0], {
      content: "old turn",
      steelengine: { recordTimestampMs: Date.parse(t1) },
    });
    expectMessageFields(result[1], {
      content: "fresh turn",
      steelengine: { recordTimestampMs: Date.parse(t2) },
    });
  });

  test("surfaces persisted user idempotency keys in __steelengine metadata (#79844)", async () => {
    ...
    expectMessageFields(result[0], {
      content: "pending optimistic turn",
      steelengine: { id: "entry-user-1", idempotencyKey: "client-turn-1" },
    });
  });
  ...
  test("reads active tree branch asynchronously without SessionManager.open", async () => {
    ...
    try {
      ...
      expectMessageFields(messages[2], { steelengine: { id: "user-2", seq: 3 } });
  ...
  test("chooses the newest reset archive across candidate roots", async () => {
    ...
    const legacySessionsDir = path.join(tmpDir, ".steelengine", "sessions");
    ...
    await withEnvAsync({ STEELENGINE_HOME: tmpDir }, async () => {
      const fullMessages = await readSessionMessagesAsync(sessionId, storePath, undefined, {
        mode: "full",
  ...
  test("keeps async rows when imported parent links are incomplete without leaf control", async () => {
    ...
    expectMessageFields(messages[0], { steelengine: { id: "legacy-user", seq: 1 } });
    expectMessageFields(messages[1], { steelengine: { id: "tree-assistant", seq: 2 } });
    expectMessageFields(messages[2], { steelengine: { id: "orphan-tail", seq: 3 } });
  });
  ...
  test("reads only the active branch when transcript rewrites abandon older entries", () => {
    ...
    try {
      ...
      expect(out).toHaveLength(2);
      expectMessageFields(out[0], { role: "user", content: "clean prompt", steelengine: { seq: 1 } });
  ...
  test.each([
    ...
    ({ sessionId, sessionFileParts, wrongStorePathParts, message }) => {
      ...
      expect((out[0] as { __steelengine?: { seq?: number } })["__steelengine"]?.seq).toBe(1);
    },
  );
  ...
  test("keeps compaction markers when reading only the active SessionManager branch", () => {
    ...
    expect(
      out.map((message) => ({
        role: (message as { role?: string }).role,
        content: (message as { content?: unknown }).content,
        kind: (message as { __steelengine?: { kind?: string } })["__steelengine"]?.kind,
      })),
    ).toEqual([
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  registerTempSessionStore("steelengine-session-preview-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("readLatestSessionUsageFromTranscript", () => {
  ...
  registerTempSessionStore("steelengine-session-usage-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
...
describe("resolveSessionTranscriptCandidates", () => {
  test("fallback candidate uses STEELENGINE_HOME instead of os.homedir()", () => {
    withEnv({ STEELENGINE_HOME: "/srv/steelengine-home", HOME: "/home/other" }, () => {
      const candidates = resolveSessionTranscriptCandidates("sess-1", undefined);
      const fallback = candidates[candidates.length - 1];
      expect(fallback).toBe(
        path.join(path.resolve("/srv/steelengine-home"), ".steelengine", "sessions", "sess-1.jsonl"),
      );
    });
  });
});
...
describe("resolveSessionTranscriptCandidates safety", () => {
  test.each([
    {
      storePath: "/tmp/steelengine/agents/main/sessions/sessions.json",
      sessionFile: "/tmp/steelengine/agents/ops/sessions/sess-safe.jsonl",
    },
    {
  ...
  test("drops unsafe session IDs instead of producing traversal paths", () => {
    const candidates = resolveSessionTranscriptCandidates(
      "../etc/passwd",
      "/tmp/steelengine/agents/main/sessions/sessions.json",
    );
  ...
  test("drops unsafe sessionFile candidates and keeps safe fallbacks", () => {
    const storePath = "/tmp/steelengine/agents/main/sessions/sessions.json";
    const candidates = resolveSessionTranscriptCandidates(
      "sess-safe",
      storePath,
      "../../etc/passwd",
  ...
  test("prefers the current sessionId transcript before a stale sessionFile candidate", () => {
    ...
    const candidates = resolveSessionTranscriptCandidates(
      ...
      storePath,
      "/tmp/steelengine/agents/main/sessions/22222222-2222-4222-8222-222222222222.jsonl",
    );

    expect(candidates[0]).toBe(
      path.resolve("/tmp/steelengine/agents/main/sessions/11111111-1111-4111-8111-111111111111.jsonl"),
    );
    expect(candidates).toContain(
      path.resolve("/tmp/steelengine/agents/main/sessions/22222222-2222-4222-8222-222222222222.jsonl"),
    );
  });

  test("keeps explicit custom sessionFile ahead of synthesized fallback", () => {
    const storePath = "/tmp/steelengine/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/steelengine/agents/main/sessions/custom-transcript.jsonl";
    const candidates = resolveSessionTranscriptCandidates(
      "11111111-1111-4111-8111-111111111111",
      storePath,
      sessionFile,
  ...
  test("keeps custom topic-like transcript paths ahead of synthesized fallback", () => {
    const storePath = "/tmp/steelengine/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/steelengine/agents/main/sessions/custom-topic-notes.jsonl";
    const candidates = resolveSessionTranscriptCandidates(
      "11111111-1111-4111-8111-111111111111",
      storePath,
      sessionFile,
  ...
  test("keeps forked transcript paths ahead of synthesized fallback", () => {
    ...
    const sessionFile =
      "/tmp/steelengine/agents/main/sessions/2026-03-23T16-30-00-000Z_11111111-1111-4111-8111-111111111111.jsonl";
  ...
  test("keeps timestamped custom transcript paths ahead of synthesized fallback", () => {
    const storePath = "/tmp/steelengine/agents/main/sessions/sessions.json";
    const sessionId = "11111111-1111-4111-8111-111111111111";
    const sessionFile = "/tmp/steelengine/agents/main/sessions/2026-03-23T16-30-00-000Z_notes.jsonl";
    const candidates = resolveSessionTranscriptCandidates(sessionId, storePath, sessionFile);
  ...
  test("still treats generated topic transcripts from another session as stale", () => {
    ...
    const staleSessionFile =
      "/tmp/steelengine/agents/main/sessions/22222222-2222-4222-8222-222222222222-topic-thread.jsonl";
...
describe("oversized transcript line guards", () => {
  ...
  registerTempSessionStore("steelengine-session-fs-oversized-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
  ...
  test("oversized line metadata extraction preserves id and parentId", async () => {
    ...
    // to the __steelengine metadata. Both must be correct for the record to
    ...
    // id is preserved in __steelengine transcript metadata
    const meta = (oversized as Record<string, Record<string, unknown>>)["__steelengine"];
...
describe("short read resilience", () => {
  ...
  registerTempSessionStore("steelengine-short-read-test-", (nextTmpDir, nextStorePath) => {
    tmpDir = nextTmpDir;
    storePath = nextStorePath;
  });
