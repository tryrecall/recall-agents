...
import type { ConfigFileSnapshot, SteelEngineConfig } from "../config/config.js";
import { resolveConfigPath, resolveStateDir } from "../config/paths.js";
import type { CliBackendConfig } from "../config/types.agent-defaults.js";
import type { GatewayAuthConfig } from "../config/types.gateway.js";
import type { SecurityAuditSuppression } from "../config/types.steelengine.js";
import {
...
async function materializeAuditGatewayAuthRefs(params: {
  ...
  const materializeParams = {
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(params.env.STEELENGINE_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(params.env.STEELENGINE_GATEWAY_PASSWORD)),
  };
...
function shouldMaterializeHooksGatewayAuthRefs(cfg: SteelEngineConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your SteelEngine state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your SteelEngine state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectLoggingFindings(cfg: SteelEngineConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: SteelEngineConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectYoloExecScopeIds(cfg: SteelEngineConfig, approvals: ExecApprovalsFile): string[] {
...
function collectExecRuntimeFindings(cfg: SteelEngineConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  if (claudePermissionModeHits.length > 0 && yoloExecScopeIds.length > 0) {
    findings.push({
      ...
      detail: `claude-cli sets ${claudePermissionModeHits.map((hit) => `${hit.argSet}=${hit.mode}`).join(", ")}, but SteelEngine exec is YOLO for: ${yoloExecScopeIds.join(", ")}. Managed Claude live sessions use --permission-mode bypassPermissions.`,
      remediation:
        "Restrict SteelEngine tools.exec.security/tools.exec.ask, or remove the Claude --permission-mode override.",
    });
  }
...
function listConfiguredMcpServerNames(cfg: SteelEngineConfig): string[] {
...
function collectAgentSkillMcpBoundaryScopes(cfg: SteelEngineConfig): AgentSkillMcpBoundaryScope[] {
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  return [
    {
      ...
      detail:
        ...
        `\nMCP server registries visible to the gateway configuration/state:\n${sources
          ...
          .join("\n")}\n` +
        "agents.*.skills filters SteelEngine skill visibility and snapshots; it is not a shell-time authorization boundary. " +
...
function collectOpenExecSurfacePaths(cfg: SteelEngineConfig): string[] {
