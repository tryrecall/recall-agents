...
import type { ConfigFileSnapshot, RecallConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: RecallConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Recall state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Recall state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.RECALL_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.RECALL_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("recall security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.RECALL_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(env.RECALL_GATEWAY_PASSWORD) ||
...
function collectLoggingFindings(cfg: RecallConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: RecallConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: RecallConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: RecallConfig): string[] {
  const channels = asRecord(cfg.channels);
  if (!channels) {
    return [];
  }
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("recall status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("recall security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set RECALL_GATEWAY_TOKEN/RECALL_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("recall security audit --deep")}".`,
    });
  }
