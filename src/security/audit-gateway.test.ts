...
describe("security audit gateway config findings", () => {
  ...
  it("warns when STEELENGINE_GATEWAY_TOKEN shadows a different configured token source", () => {
    const cfg: SteelEngineConfig = {
  ...
  it("does not warn when gateway.auth.token resolves from STEELENGINE_GATEWAY_TOKEN", () => {
    const cfg: SteelEngineConfig = {
