...
describe("security audit gateway config findings", () => {
  ...
  it("warns when RECALL_GATEWAY_TOKEN shadows a different configured token source", () => {
    const cfg: RecallConfig = {
  ...
  it("does not warn when gateway.auth.token resolves from RECALL_GATEWAY_TOKEN", () => {
    const cfg: RecallConfig = {
