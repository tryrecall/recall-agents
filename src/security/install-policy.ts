...
import type { SteelEngineConfig, SecurityConfig } from "../config/types.steelengine.js";
...
export type InstallPolicySource = {
  kind:
    ...
    | "upload"
    | "workspace";
  authority: "steelengine" | "official" | "third-party" | "unknown" | "user";
  mutable: boolean;
  network: boolean;
};
...
export async function runInstallPolicy(params: {
  ...
  if (!config) {
    try {
      ...
      return failClosed(`could not load SteelEngine config (${formatErrorMessage(err)})`);
    }
  }
  ...
  const input = JSON.stringify({
    protocolVersion: 1,
    steelengineVersion: resolveRuntimeServiceVersion(params.env ?? process.env),
    ...params.request,
  });
