...
describe("security audit install metadata findings", () => {
  ...
  const runInstallMetadataAudit = async (cfg: SteelEngineConfig, stateDir: string) => {
  ...
  const writePluginIndexInstallRecords = async (
    ...
    const index: InstalledPluginIndex = {
      ...
      plugins: Object.keys(records).map((pluginId) => ({
        pluginId,
        manifestPath: path.join(stateDir, "extensions", pluginId, "steelengine.plugin.json"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-security-install-"));
  });
              ...
              hooks: {
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@steelengine/test-hooks@1.2.3",
        ...
        run: async () => {
          ...
          await writePluginIndexInstallRecords(stateDir, {
            "voice-call": {
              source: "npm",
              spec: "@steelengine/voice-call",
              resolvedSpec: "@steelengine/voice-call@1.2.3",
              integrity: "sha512-plugin",
            },
          });
          return runInstallMetadataAudit(
                ...
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@steelengine/test-hooks",
                      resolvedSpec: "@steelengine/test-hooks@1.2.3",
                      integrity: "sha512-hook",
                    },
                  },
                },
...
describe("security audit extension tool reachability findings", () => {
  ...
  const runSharedExtensionsAudit = async (config: SteelEngineConfig) => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-security-extensions-"));
    isolatedHome = path.join(fixtureRoot, "home");
    const isolatedEnv = createPathResolutionEnv(isolatedHome, { STEELENGINE_HOME: isolatedHome });
    ...
    const cases = [
      {
        ...
        cfg: {} satisfies SteelEngineConfig,
