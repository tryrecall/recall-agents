...
describe("security audit install metadata findings", () => {
  ...
  const runInstallMetadataAudit = async (cfg: RecallConfig, stateDir: string) => {
  ...
  const writePluginIndexInstallRecords = async (
    ...
    const index: InstalledPluginIndex = {
      ...
      plugins: Object.keys(records).map((pluginId) => ({
        pluginId,
        manifestPath: path.join(stateDir, "extensions", pluginId, "recall.plugin.json"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-security-install-"));
  });
...
describe("security audit extension tool reachability findings", () => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-security-extensions-"));
    isolatedHome = path.join(fixtureRoot, "home");
    const isolatedEnv = createPathResolutionEnv(isolatedHome, { RECALL_HOME: isolatedHome });
