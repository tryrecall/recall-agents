...
describe("security audit exec surface findings", () => {
  // Redirect the Recall home (RECALL_HOME wins over HOME/USERPROFILE in
  // `resolveRawHomeDir`) to a per-test tempdir so `saveExecApprovals` never
  // touches the real `~/.recall/exec-approvals.json` on the host running
  // the suite.
  ...
  beforeEach(async () => {
    ...
    // RECALL_HOME takes precedence over HOME/USERPROFILE in resolveRawHomeDir,
