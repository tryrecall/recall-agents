...
function isGatewayRemotelyExposed(cfg: SteelEngineConfig): boolean {
...
function formatHooksTokenReuseRemediation(reuse: GatewayAuthSharedSecretReuse): string {
  ...
  return `Run ${formatCliCommand("steelengine doctor --fix")} to rotate a persisted hooks.token, then update external hook senders to use the new hook token.`;
}
...
function listKnownNodeCommands(cfg: SteelEngineConfig): Set<string> {
...
function listOpenInboundPolicies(cfg: SteelEngineConfig): string[] {
  const out: string[] = [];
...
function listPotentialMultiUserSignals(cfg: SteelEngineConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function listAuditAgentToolContexts(cfg: SteelEngineConfig): AuditAgentToolContext[] {
...
function collectRiskyToolExposureContexts(cfg: SteelEngineConfig): {
...
function collectControlPlaneToolExposureContexts(cfg: SteelEngineConfig): string[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep STEELENGINE_STATE_DIR on a local-only volume and re-run "${formatCliCommand("steelengine security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: SteelEngineConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer STEELENGINE_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-steelengine-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: SteelEngineConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: SteelEngineConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: SteelEngineConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: SteelEngineConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: SteelEngineConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: SteelEngineConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: SteelEngineConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "SteelEngine's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway. For multiple users or organizations, run one isolated Gateway cell per tenant: https://docs.steelengine.ai/gateway/multi-tenant-hosting",
    remediation:
