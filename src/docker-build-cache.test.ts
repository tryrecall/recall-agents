...
describe("docker build cache layout", () => {
  ...
  it("uses pnpm cache mounts in Dockerfiles that install repo dependencies", async () => {
    for (const path of [
      ...
      expect(
        ...
        /--mount=type=cache,id=steelengine-pnpm-store,target=\/(?:root|home\/appuser)\/\.local\/share\/pnpm\/store,sharing=locked/,
      );
    }
  });
  ...
  it("keeps the shared e2e image on the packaged tarball install path", async () => {
    ...
    expect(dockerfile).toMatch(
      /^COPY --from=steelengine_package --chown=appuser:appuser steelengine-current\.tgz \/tmp\/steelengine-current\.tgz$/m,
    );
    ...
    expect(dockerfile).toContain(
      "COPY --from=functional-manifest --chown=appuser:appuser /tmp/steelengine-deps /tmp/steelengine-deps",
    );
    expect(dockerfile).toContain("npm install --omit=dev --no-fund --no-audit");
    expect(dockerfile).not.toContain("npm install -g --prefix");
    expect(dockerfile).toContain(
      "COPY --from=functional-deps --chown=appuser:appuser /tmp/steelengine-deps/node_modules /app/node_modules",
    );
    // Packaged postinstall must run before the self-link exists so its prune
    // walks cannot cycle through /app/node_modules/steelengine -> /app.
    const postinstallIndex = dockerfile.indexOf(
      "node /app/scripts/postinstall-bundled-plugins.mjs",
    );
    const selfLinkIndex = dockerfile.indexOf("ln -sfn /app /app/node_modules/steelengine");
