...
function createDatabaseOptions() {
  const stateDir = makeTempDir(tempDirs, "steelengine-session-upstream-monitor-");
...
describe("session upstream monitor", () => {
  it("records watched activity once and advances its marker", async () => {
    ...
    const dedupeRow = openSteelEngineStateDatabase(database)
  ...
  it("clamps skewed upstream event times without touching bookkeeping clocks", async () => {
    ...
    const cursor = openSteelEngineStateDatabase(database)
  ...
  it("skips recording and marker writes when the link was refreshed mid-scan", async () => {
    ...
    const row = openSteelEngineStateDatabase(database)
  ...
  it("records an external prompt five seconds after SteelEngine activity", async () => {
    const database = createDatabaseOptions();
    ...
    await runSessionUpstreamMonitorTick({
      ...
      isRunActive: () => false,
      loadOwnRecentUserTexts: async () => ["SteelEngine prompt"],
    });
