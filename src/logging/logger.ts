...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredSteelEngineTmpDir() : POSIX_STEELENGINE_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "steelengine.log")
    : `${POSIX_STEELENGINE_TMP_DIR}/steelengine.log`;
}
...
type LoggerConfigLoader = () => SteelEngineConfig["logging"] | undefined;
...
function resolveDefaultActiveLogFile(): string {
  if (process.env.VITEST === "true" && process.env.STEELENGINE_TEST_FILE_LOG === "1") {
    return path.join(
      process.cwd(),
      ".artifacts",
      "test-logs",
...
function resolveSettings(): ResolvedSettings {
  ...
  const cfg: SteelEngineConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? loadLoggerConfig();
  const defaultLevel =
    process.env.VITEST === "true" && process.env.STEELENGINE_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (currentFileBytes > 0 && nextBytes > settings.maxFileBytes) {
        if (rotateLogFile(activeFile)) {
          ...
          process.stderr.write(
            `[steelengine] log file rotation failed; continuing writes file=${activeFile} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
      }
