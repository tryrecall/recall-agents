...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredRecallTmpDir() : POSIX_RECALL_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "recall.log")
    : `${POSIX_RECALL_TMP_DIR}/recall.log`;
}
...
type LoggerConfigLoader = () => RecallConfig["logging"] | undefined;
...
function resolveSettings(): ResolvedSettings {
  ...
  const cfg: RecallConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? loadLoggerConfig();
  const defaultLevel =
    process.env.VITEST === "true" && process.env.RECALL_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (currentFileBytes > 0 && nextBytes > settings.maxFileBytes) {
        if (rotateLogFile(activeFile)) {
          ...
          process.stderr.write(
            `[recall] log file rotation failed; continuing writes file=${activeFile} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
      }
