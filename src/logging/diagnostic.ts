...
function resolveDiagnosticSessionStorePaths(config?: RecallConfig): string[] | undefined {
  if (!config) {
    return undefined;
  }
  try {
...
function shouldWriteCriticalMemoryPressureBundle(config?: RecallConfig): boolean {
...
export function resolveStuckSessionWarnMs(config?: RecallConfig): number {
