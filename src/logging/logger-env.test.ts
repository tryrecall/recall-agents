...
const logPathTracker = createSuiteLogPathTracker("recall-test-env-log-level-");

describe("RECALL_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    originalEnv = process.env.RECALL_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.RECALL_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.RECALL_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("RECALL_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid RECALL_LOG_LEVEL="nope"');
  });
});
