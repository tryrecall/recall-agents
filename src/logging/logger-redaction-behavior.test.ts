...
const logPathTracker = createSuiteLogPathTracker("recall-log-redaction-");
const originalConfigPath = process.env.RECALL_CONFIG_PATH;
const originalHome = process.env.HOME;
const originalTestFileLog = process.env.RECALL_TEST_FILE_LOG;
...
describe("file log redaction", () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    expect(loggerTest.resolveActiveLogFile("~/custom-recall.log")).toBe(
      path.join(home, "custom-recall.log"),
    );
  });
