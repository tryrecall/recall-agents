...
const logPathTracker = createSuiteLogPathTracker("steelengine-log-redaction-");
...
describe("file log redaction", () => {
  ...
  it("uses logging.file from the active config path", () => {
    ...
    withEnv({ STEELENGINE_CONFIG_PATH: configPath, STEELENGINE_TEST_FILE_LOG: "1" }, () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    withEnv({ HOME: home }, () => {
      expect(loggerTest.resolveActiveLogFile("~/custom-steelengine.log")).toBe(
        path.join(home, "custom-steelengine.log"),
      );
    });
  });
