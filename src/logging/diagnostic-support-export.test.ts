...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "steelengine.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["steelengine", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
              STEELENGINE_GATEWAY_TOKEN: fakeToken,
            },
          },
        },
        gateway: {
    ...
    expect(combined).not.toContain("__STEELENGINE_REDACTED__");
    ...
    expect(combined).toContain("$STEELENGINE_STATE_DIR");
    ...
    const sanitizedLogs = entries["logs/steelengine-sanitized.jsonl"];
    ...
    expect(status.data?.service?.command?.environment?.STEELENGINE_GATEWAY_TOKEN).toBe("<redacted>");
  ...
  it("truncates support strings without splitting UTF-16 surrogate pairs", () => {
    ...
    const redactedPathPrefix = `$STEELENGINE_STATE_DIR${path.sep}`;
    expect(
  ...
  it("redacts Windows USERPROFILE paths when HOME is unset", () => {
    ...
    const stateDir = `${userProfile}\\AppData\\Roaming\\steelengine`;
    const redaction = {
      env: {
    ...
    expect(redactSupportString(`${stateDir}\\logs\\gateway.log`, redaction)).toBe(
      "$STEELENGINE_STATE_DIR\\logs\\gateway.log",
    );
    expect(
    ...
    const status = sanitizeSupportSnapshotValue(
      {
        service: {
          command: {
            programArguments: [
              "node",
              `${userProfile}\\steelengine\\dist\\index.js`,
              "--config",
              `${stateDir}\\steelengine.json`,
            ],
            sourcePath: "c:\\users\\support-user\\AppData\\Local\\steelengine\\gateway-service.json",
          },
        },
      },
      redaction,
    ...
    expect(serialized).toContain("~\\\\steelengine\\\\dist\\\\index.js");
    expect(serialized).toContain("$STEELENGINE_STATE_DIR\\\\steelengine.json");
    expect(serialized).toContain("~\\\\AppData\\\\Local\\\\steelengine\\\\gateway-service.json");
  });
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/steelengine.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/steelengine-sanitized.jsonl");
