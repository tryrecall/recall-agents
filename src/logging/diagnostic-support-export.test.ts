...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "recall.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["recall", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
              RECALL_GATEWAY_TOKEN: fakeToken,
            },
          },
        },
        gateway: {
    ...
    expect(combined).toContain("$RECALL_STATE_DIR");
    ...
    const sanitizedLogs = entries["logs/recall-sanitized.jsonl"];
    ...
    expect(status.data?.service?.command?.environment?.RECALL_GATEWAY_TOKEN).toBe("<redacted>");
  ...
  it("redacts Windows USERPROFILE paths when HOME is unset", () => {
    ...
    const stateDir = `${userProfile}\\AppData\\Roaming\\recall`;
    const redaction = {
      env: {
    ...
    expect(redactSupportString(`${stateDir}\\logs\\gateway.log`, redaction)).toBe(
      "$RECALL_STATE_DIR\\logs\\gateway.log",
    );
    expect(
    ...
    const status = sanitizeSupportSnapshotValue(
      {
        service: {
          command: {
            programArguments: [
              "node",
              `${userProfile}\\recall\\dist\\index.js`,
              "--config",
              `${stateDir}\\recall.json`,
            ],
            sourcePath: "c:\\users\\support-user\\AppData\\Local\\recall\\gateway-service.json",
          },
        },
      },
      redaction,
    ...
    expect(serialized).toContain("~\\\\recall\\\\dist\\\\index.js");
    expect(serialized).toContain("$RECALL_STATE_DIR\\\\recall.json");
    expect(serialized).toContain("~\\\\AppData\\\\Local\\\\recall\\\\gateway-service.json");
  });
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/recall.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/recall-sanitized.jsonl");
