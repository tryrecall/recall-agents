...
function writeConfig(source: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-redact-config-"));
  tempDirs.push(dir);
...
describe("redactSensitiveText", () => {
  ...
  it("masks named Gateway security headers", () => {
    ...
    expect(output).toContain("X-SteelEngine-Token: supers…7890");
    expect(output).toContain("x-pomerium-jwt-assertion: eyJhea…3456");
    expect(output).toContain("X-Api-Key=***");
    expect(output).not.toContain(steelEngineToken);
  ...
  it("masks URL userinfo and database connection-string passwords", () => {
    const input = [
      ...
      "postgres://dbuser:database-password-1234567890@db.example.test/steelengine",
      "postgres://secret:secret@db.example.test/steelengine",
    ...
    expect(output).toContain("postgres://dbuser:databa…7890@db.example.test/steelengine");
    expect(output).toContain("postgres://secret:***@db.example.test/steelengine");
  ...
  it("does not redact ordinary identifiers containing short token-prefix substrings", () => {
    const input = [
      "npm_telegram_package_spec ask_steelengine_query_patterns team_management risk_assessment glpat-docs dapi-example sbp_short nfp_site CCIPAT_docs ATATT-example fw-tooshort fw_tooshort fpk_tooshort",
  ...
  it("masks connection-string passwords through the default options path", () => {
    expect(
      redactSensitiveText("postgres://dbuser:opaquepw12345@db.example.test/steelengine", {
        mode: "tools",
      }),
    ).toBe("postgres://dbuser:***@db.example.test/steelengine");
  });
