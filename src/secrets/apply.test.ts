...
async function writeJsonFile(filePath: string, value: unknown): Promise<void> {
  if (path.basename(filePath) === "steelengine-agent.sqlite") {
...
async function createApplyFixture(): Promise<ApplyFixture> {
  const paths = buildFixturePaths(
    await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-secrets-apply-")),
  );
  ...
  return {
    ...paths,
    env: {
      STEELENGINE_STATE_DIR: paths.stateDir,
      STEELENGINE_CONFIG_PATH: paths.configPath,
...
describe("secrets apply", () => {
  ...
  it("atomically deletes a newly created auth store when a later auth write fails", async () => {
    ...
    const firstDatabase = openSteelEngineAgentDatabase({
      agentId: "first",
      path: firstStorePath,
    });
    ...
    const secondDatabase = openSteelEngineAgentDatabase({
      agentId: "second",
  ...
  it("scrubs .env in legacy .clawdbot state directory via automatic fallback", async () => {
    // Do NOT set STEELENGINE_STATE_DIR — rely on resolveStateDir's automatic
    // legacy-directory fallback. A controlled HOME that contains only
    // .clawdbot (no .steelengine) exercises the scrub path so the old
    // resolveConfigDir call (which always returns $HOME/.steelengine) would
    // miss the .env inside .clawdbot.
    const homeDir = tempDirs.make("steelengine-secrets-apply-legacy-");
  ...
  it("uses the same resolved stateDir for .env scrubbing as for auth stores", async () => {
    ...
    //
    // Set up a HOME where both .steelengine and .clawdbot exist.
    // resolveStateDir returns .steelengine when both exist because it checks
    // .steelengine first. The apply must use that same root for .env.
    const homeDir = tempDirs.make("steelengine-secrets-apply-root-");
    const steelengineDir = path.join(homeDir, ".steelengine");
    const clawdbotDir = path.join(homeDir, ".clawdbot");
    const configPath = path.join(steelengineDir, "steelengine.json");
    const agentDir = path.join(steelengineDir, "agents", "main", "agent");
    const steelengineEnvPath = path.join(steelengineDir, ".env");
    ...
    // .env in the canonical .steelengine dir — this is the one that should be scrubbed
    await fs.writeFile(
      steelengineEnvPath,
      "OPENAI_API_KEY=sk-openai-plaintext\nUNRELATED=value\n", // pragma: allowlist secret
      "utf8",
    ...
    try {
      ...
      // Canonical .steelengine/.env was scrubbed
      const nextOpenclawEnv = await fs.readFile(steelengineEnvPath, "utf8");
