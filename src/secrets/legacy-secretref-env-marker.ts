...
function toCandidate(
  ...
  defaults: NonNullable<RecallConfig["secrets"]>["defaults"] | undefined,
...
export function migrateLegacySecretRefEnvMarkers(config: RecallConfig): {
  config: RecallConfig;
  changes: string[];
} {
  ...
  const next = structuredClone(config) as RecallConfig & Record<string, unknown>;
