...
function createPluginSteelEngineConfigSecretTargetEntry(
  pluginId: string,
  configPath: string,
  ...
  for (const record of bundledPlugins) {
    for (const config of WEB_PROVIDER_SECRET_CONFIGS) {
      if (
        ...
        entries.push(createPluginSteelEngineConfigSecretTargetEntry(record.id, config.configPath));
      }
    }
  }
...
function listBundledPluginConfigSecretTargetRegistryEntries(
  ...
  for (const record of bundledPlugins) {
    ...
    for (const secretInput of secretInputs) {
      const entry = createPluginSteelEngineConfigSecretTargetEntry(record.id, secretInput.path);
...
export function getSecretTargetRegistry(params?: {
  ...
  if (params?.sourceTree) {
    ...
    return loadSecretTargetRegistryFromPluginMetadata({
      env: {
        ...process.env,
        STEELENGINE_BUNDLED_PLUGINS_DIR: process.env.STEELENGINE_BUNDLED_PLUGINS_DIR ?? "extensions",
      },
