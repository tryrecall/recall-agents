...
function setConfiguredFetchProviderKey(configTarget: RecallConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: RecallConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: RecallConfig, provider: ProviderUnderTest): unknown {
...
describe("runtime web tools resolution", () => {
  ...
  describe("when brave is installed as an external plugin and explicitly configured", () => {
    ...
    beforeEach(() => {
      loadInstalledPluginIndexInstallRecordsSyncMock.mockReturnValue({
        brave: { source: "npm", spec: "@recall/brave-search" },
      });
