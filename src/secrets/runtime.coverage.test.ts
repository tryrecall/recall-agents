...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: SteelEngineConfig): unknown =>
...
const DEBUG_COVERAGE_BATCHES = process.env.STEELENGINE_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_STEELENGINE_COVERAGE_EXCLUSIONS = new Set([
...
function addCoveragePluginLoadPath(config: SteelEngineConfig, pluginId: string): void {
...
function collectSteelEngineCoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "steelengine.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_STEELENGINE_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForSteelEngineTarget(
...
async function expectSteelEngineCoverageBatchResolved(
  label: string,
  ...
  for (const [index, entry] of batch.entries()) {
    const envId = toCoverageEnvRefId("STEELENGINE_SECRET_TARGET", entry.id);
    ...
    applyConfigForSteelEngineTarget(config, entry, envId, wildcardToken);
  }
...
const STEELENGINE_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectSteelEngineCoverageEntries({ includePluginEntries: false }),
);
const STEELENGINE_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
  collectSteelEngineCoverageEntries({ includePluginEntries: true }),
);
...
describe("secrets runtime target coverage", () => {
  beforeAll(async () => {
    ...
    const googleChatBatch = STEELENGINE_CORE_COVERAGE_BATCHES.find((batch) =>
      batch.some((entry) => entry.id === "channels.googlechat.serviceAccount"),
    );
    if (googleChatBatch) {
      await expectSteelEngineCoverageBatchResolved("steelengine.json core", googleChatBatch);
    }
    const webProviderBatch = STEELENGINE_PLUGIN_COVERAGE_BATCHES.find((batch) =>
    ...
    if (webProviderBatch) {
      ...
      await expectSteelEngineCoverageBatchResolved("steelengine.json plugins", webProviderBatch);
    }
  });

  describe("steelengine.json core and channel registry targets", () => {
    test.each(STEELENGINE_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectSteelEngineCoverageBatchResolved("steelengine.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("steelengine.json plugin registry targets", () => {
    test.each(STEELENGINE_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectSteelEngineCoverageBatchResolved("steelengine.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = toCoverageEnvRefId("STEELENGINE_AUTH_SECRET_TARGET", entry.id);
