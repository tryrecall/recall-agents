...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: RecallConfig): unknown =>
...
type SecretRegistryEntry = {
  id: string;
  configFile: "recall.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.RECALL_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_RECALL_COVERAGE_EXCLUSIONS = new Set([
...
function collectRecallCoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "recall.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_RECALL_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForRecallTarget(
  config: RecallConfig,
  entry: SecretRegistryEntry,
  envId: string,
  wildcardToken: string,
...
async function expectRecallCoverageBatchResolved(
  label: string,
  ...
  for (const [index, entry] of batch.entries()) {
    const envId = `RECALL_SECRET_TARGET_${entry.id}`;
    ...
    applyConfigForRecallTarget(config, entry, envId, wildcardToken);
  }
...
const RECALL_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectRecallCoverageEntries({ includePluginEntries: false }),
);
const RECALL_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
  collectRecallCoverageEntries({ includePluginEntries: true }),
);
...
describe("secrets runtime target coverage", () => {
  ...
  describe("recall.json core and channel registry targets", () => {
    test.each(RECALL_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectRecallCoverageBatchResolved("recall.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("recall.json plugin registry targets", () => {
    test.each(RECALL_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectRecallCoverageBatchResolved("recall.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = `RECALL_AUTH_SECRET_TARGET_${entry.id}`;
