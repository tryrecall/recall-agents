...
function buildConfigForRecallTarget(entry: SecretRegistryEntry, envId: string): RecallConfig {
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every recall.json registry target when configured as active", async () => {
    const entries = listSecretTargetRegistryEntries().filter(
      (entry) => entry.configFile === "recall.json",
    );
    for (const [index, entry] of entries.entries()) {
      const envId = `RECALL_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
        config: buildConfigForRecallTarget(entry, envId),
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const [index, entry] of entries.entries()) {
      const envId = `RECALL_AUTH_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
