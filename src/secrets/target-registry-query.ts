...
const RECALL_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of RECALL_COMPILED_SECRET_TARGETS) {
...
const RECALL_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: RECALL_COMPILED_SECRET_TARGETS,
    entriesById: RECALL_TARGETS_BY_ID,
  });
