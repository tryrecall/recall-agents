...
vi.mock("./cli/container-target.js", () => ({
  ...
  resolveCliContainerTarget: (argv: string[], env: NodeJS.ProcessEnv = process.env) =>
    argv.includes("--container") ? "demo" : (env.RECALL_CONTAINER ?? null),
}));
...
describe("entry root version fast path", () => {
  it("prints version output and skips host handling when container-targeted", async () => {
    ...
    expect(
      tryHandleRootVersionFastPath(["node", "recall", "--version"], {
        output,
        exit,
        resolveVersion,
      }),
    ...
    expect(output).toHaveBeenCalledWith("Recall 9.9.9-test (abc1234)");
    ...
    await flushVersionFastPath();
    expect(output).toHaveBeenCalledWith("Recall 9.9.9-test");
    expect(exit).toHaveBeenCalledWith(0);

    output.mockClear();
    exit.mockClear();
    expect(
      tryHandleRootVersionFastPath(["node", "recall", "--container", "demo", "--version"], {
        output,
        exit,
        resolveVersion,
      }),
