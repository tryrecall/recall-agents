...
const MISSING_WEB_SEARCH_PROVIDER_DIAGNOSTIC_MESSAGE =
  "web_search tool requested in toolsAllow but no web search provider is selected. Configure one with: steelengine configure --section web, or set tools.web.search.provider.";
...
function requireEmbeddedAgentCall(): {
  ...
  const call = runEmbeddedAgentMock.mock.calls[0]?.[0] as
    ...
    | undefined;
  if (!call) {
    throw new Error("Expected embedded SteelEngine agent call for toolsAllow passthrough");
  }
  return call;
}
...
describe("runCronIsolatedAgentTurn toolsAllow passthrough", () => {
  ...
  beforeEach(() => {
    previousFastTestEnv = process.env.STEELENGINE_TEST_FAST;
  ...
  afterEach(() => {
    ...
    vi.stubEnv("STEELENGINE_TEST_FAST", previousFastTestEnv);
  });
