      ...
      },
      {
        ...
        plugin: {
          ...
          config: {
            listAccountIds: () => [],
            resolveAccount: () => ({}),
            resolveAllowFrom: ({ cfg }: { cfg: RecallConfig }) =>
...
function makeCfg(overrides?: Partial<RecallConfig>): RecallConfig {
  return {
    bindings: [],
    channels: {},
    ...overrides,
...
function makeForumBoundCfg(accountId = "account-b"): RecallConfig {
  return makeCfg({
    bindings: [
      {
        agentId: AGENT_ID,
...
async function resolveLastTarget(cfg: RecallConfig) {
