...
/** Runs a test callback with an isolated SteelEngine home for cron tests. */
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "steelengine-cron-" });
}
...
export async function writeSessionStoreEntries(
  ...
  const dir = path.join(home, ".steelengine", "sessions");
