...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "recall-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".recall", "sessions");
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<RecallConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<RecallConfig>,
