...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  const envSnapshot = captureEnv(["STEELENGINE_HOME", "HOME"]);

  afterEach(() => {
    envSnapshot.restore();
  });

  it("uses STEELENGINE_HOME for tilde expansion", () => {
    setTestEnvValue("STEELENGINE_HOME", "/srv/steelengine-home");
    setTestEnvValue("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/steelengine-home", "cron", "jobs.json"));
  });
});
...
describe("cron store", () => {
  ...
  it("stores queued reservations separately from active run markers", async () => {
    ...
    const queuedRow = openSteelEngineStateDatabase()
