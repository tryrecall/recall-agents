...
const { makeStorePath } = createCronStoreHarness({ prefix: "recall-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses RECALL_HOME for tilde expansion", () => {
    vi.stubEnv("RECALL_HOME", "/srv/recall-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/recall-home", "cron", "jobs.json"));
  });
});
