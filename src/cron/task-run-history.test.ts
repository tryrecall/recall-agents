...
describe("cron task run history", () => {
  it("reads executions produced by the cron service from the ledger", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-cron-task-service-history-" },
      async (state) => {
  ...
  it("round-trips outcomes and telemetry through task detail", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-cron-task-history-" },
      async (state) => {
  ...
  it("preserves paging and text-query filtering", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-cron-task-history-page-" },
      async (state) => {
  ...
  it("keeps same-job histories and totals scoped to one cron store", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-cron-task-history-store-scope-" },
      async (state) => {
