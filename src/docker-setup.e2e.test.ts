...
const sandboxRootTracker = createSuiteTempRootTracker({ prefix: "recall-docker-setup-" });
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    RECALL_AUTH_PROFILE_SECRET_DIR: join(sandbox.rootDir, "auth-profile-secrets"),
  };
...
function isGatewayStartLine(line: string) {
  return line.includes("compose") && line.includes(" up -d") && line.includes("recall-gateway");
}
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      RECALL_DOCKER_APT_PACKAGES: "curl wget",
      RECALL_EXTRA_MOUNTS: undefined,
      RECALL_HOME_VOLUME: "recall-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("RECALL_IMAGE_APT_PACKAGES=curl wget");
    ...
    expect(envFile).toContain("RECALL_DISABLE_BONJOUR=");
    expect(envFile).toContain(
      `RECALL_AUTH_PROFILE_SECRET_DIR=${join(activeSandbox.rootDir, "auth-profile-secrets")}`,
    );
    ...
    expect(extraCompose).toContain(
      `${join(activeSandbox.rootDir, "auth-profile-secrets")}:/home/node/.config/recall`,
    );
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("recall-home:");
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg RECALL_IMAGE_APT_PACKAGES=curl wget");
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node recall-gateway dist/index.js onboard --mode local --no-install-daemon --gateway-auth token --gateway-token-ref-env RECALL_GATEWAY_TOKEN --skip-ui --suppress-gateway-token-output`,
    );
    ...
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node recall-gateway dist/index.js config set --batch-json [{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]`,
    );
    expect(log).not.toContain("run --rm recall-cli onboard --mode local --no-install-daemon");
  });
  ...
  it("persists explicit Docker Bonjour opt-in overrides", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      RECALL_DISABLE_BONJOUR: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("RECALL_DISABLE_BONJOUR=0");
  });

  it("normalizes legacy RECALL_DOCKER_APT_PACKAGES into RECALL_IMAGE_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("RECALL_IMAGE_APT_PACKAGES=curl wget");
    expect(envFile).not.toContain("RECALL_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg RECALL_IMAGE_APT_PACKAGES=curl wget");
    expect(log).not.toContain("--build-arg RECALL_DOCKER_APT_PACKAGES");
  });

  it("prefers RECALL_IMAGE_APT_PACKAGES over legacy RECALL_DOCKER_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      RECALL_IMAGE_APT_PACKAGES: "curl wget httpie",
      RECALL_DOCKER_APT_PACKAGES: "curl wget",
    });
    expect(result.status).toBe(0);

    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("RECALL_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(envFile).not.toContain("RECALL_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg RECALL_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(log).not.toMatch(/--build-arg RECALL_IMAGE_APT_PACKAGES=curl wget(?! httpie)/);
  });

  it("explicitly empty RECALL_IMAGE_APT_PACKAGES suppresses legacy fallback", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("RECALL_IMAGE_APT_PACKAGES=");
    expect(envFile).not.toContain("curl wget");

    const log = await readDockerLog(activeSandbox);
    expect(log).not.toContain("--build-arg RECALL_IMAGE_APT_PACKAGES=curl wget");
  });

  it("avoids shared-network recall-cli before the gateway is started", async () => {
    const activeSandbox = requireSandbox(sandbox);
  ...
  it("pins setup-time CLI state paths inside the container", async () => {
    ...
    await resetDockerLog(activeSandbox);
    const result = runDockerSetup(activeSandbox, {
      RECALL_HOME: "/mnt/c/Users/Trevor",
      RECALL_STATE_DIR: "/mnt/c/Users/Trevor/.recall",
      RECALL_CONFIG_PATH: "/mnt/c/Users/Trevor/.tryrecall/recall-agents.json",
      RECALL_SKIP_ONBOARDING: "1",
    });
    expect(result.status).toBe(0);
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain("run --rm --no-deps --user root --entrypoint sh recall-gateway -c");
  });
  ...
  it("precreates auth profile secret key dir outside the mounted state dir", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      RECALL_CONFIG_DIR: configDir,
      RECALL_WORKSPACE_DIR: workspaceDir,
      RECALL_AUTH_PROFILE_SECRET_DIR: secretDir,
    });
    ...
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("find /home/node/.config/recall -xdev");
  });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      ...
      expect(forceRecreateLine).toBe(
        `compose compose -f ${join(activeSandbox.rootDir, "docker-compose.yml")} up -d --force-recreate recall-gateway`,
      );
  ...
  it("rejects injected multiline RECALL_EXTRA_MOUNTS values", () => {
    const activeSandbox = requireSandbox(sandbox);
  ...
  it("rejects invalid RECALL_EXTRA_MOUNTS mount format", () => {
    const activeSandbox = requireSandbox(sandbox);
  ...
  it("rejects invalid RECALL_HOME_VOLUME names", () => {
    const activeSandbox = requireSandbox(sandbox);
  ...
  it("rejects RECALL_TZ values that are not present in zoneinfo", () => {
    const activeSandbox = requireSandbox(sandbox);
  ...
  it("skips onboarding when RECALL_SKIP_ONBOARDING is set", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("RECALL_SKIP_ONBOARDING=1");
  });

  it("treats RECALL_SKIP_ONBOARDING=0 as disabled and runs onboarding", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      RECALL_SKIP_ONBOARDING: "0",
    });

    expect(result.status).toBe(0);
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain(
      "onboard --mode local --no-install-daemon --gateway-auth token --gateway-token-ref-env RECALL_GATEWAY_TOKEN --skip-ui --suppress-gateway-token-output",
    );
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toMatch(/RECALL_SKIP_ONBOARDING=\n/);
  });
  ...
  it("keeps docker-compose gateway Bonjour advertising in auto mode by default", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(
      compose.match(/RECALL_DISABLE_BONJOUR: \$\{RECALL_DISABLE_BONJOUR:-\}/g),
    ).toHaveLength(1);
  });
  ...
  it("keeps docker-compose auth profile secret key source durable outside state", async () => {
    ...
    expect(
      compose.split(
        "${RECALL_AUTH_PROFILE_SECRET_DIR:-${HOME:-/tmp}/.recall-auth-profile-secrets}:/home/node/.config/recall",
      ),
    ).toHaveLength(3);
  });
  ...
  it("pins container-side state, workspace, and config dirs on both services so host .env paths cannot leak (#77436)", async () => {
    ...
    expect(compose.match(/RECALL_HOME: \/home\/node$/gm)).toHaveLength(2);
    expect(compose.match(/RECALL_STATE_DIR: \/home\/node\/\.recall$/gm)).toHaveLength(2);
    expect(
      compose.match(/RECALL_CONFIG_PATH: \/home\/node\/\.recall\/recall\.json$/gm),
    ).toHaveLength(2);
    expect(compose.match(/RECALL_CONFIG_DIR: \/home\/node\/\.recall$/gm)).toHaveLength(2);
    expect(
      compose.match(/RECALL_WORKSPACE_DIR: \/home\/node\/\.recall\/workspace$/gm),
    ).toHaveLength(2);
  });

  it("Dockerfile ARG RECALL_IMAGE_APT_PACKAGES must not have a default value", async () => {
    // If the ARG has a default (e.g. ARG RECALL_IMAGE_APT_PACKAGES=""), Docker treats it as
    // "set" even when no --build-arg is passed. That breaks the RUN fallback expression
    // ${RECALL_IMAGE_APT_PACKAGES-$RECALL_DOCKER_APT_PACKAGES} because the variable is
    // never truly unset, so legacy-only callers using --build-arg RECALL_DOCKER_APT_PACKAGES
    ...
    const argLine = dockerfile
      .split("\n")
      .find((line) => line.startsWith("ARG RECALL_IMAGE_APT_PACKAGES"));
    expect(argLine).toBeDefined();
    // Must be bare `ARG RECALL_IMAGE_APT_PACKAGES` with no default assignment
    expect(argLine).toBe("ARG RECALL_IMAGE_APT_PACKAGES");
  });
});
