...
describe("canvas host", () => {
  ...
  it("injects live reload script", () => {
    ...
    expect(out).toContain("recallCanvasA2UIAction");
    expect(out).toContain("recallSendUserAction");
  });
  ...
  it("creates a default index.html when missing", async () => {
    ...
    try {
      ...
      expect(html).toContain("recallSendUserAction");
  ...
  it("serves A2UI scaffold and blocks traversal/symlink escapes", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.recallA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      ...
      const res = await realFetch(`http://127.0.0.1:${server.port}/__recall__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("recall-a2ui-host");
      expect(html).toContain("recallCanvasA2UIAction");

      const bundleRes = await realFetch(
        `http://127.0.0.1:${server.port}/__recall__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("recallA2UI");
