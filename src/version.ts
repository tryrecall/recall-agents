...
const CORE_PACKAGE_NAME = "recall";
...
function readInjectedVersion(): string | undefined {
  return typeof __RECALL_VERSION__ === "string" ? __RECALL_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["RECALL_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["RECALL_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["RECALL_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.RECALL_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current Recall version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: readInjectedVersion(),
  bundledVersion: process.env.RECALL_BUNDLED_VERSION,
});
