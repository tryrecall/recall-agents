...
declare const __STEELENGINE_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "steelengine";
...
function readInjectedVersion(): string | undefined {
  return typeof __STEELENGINE_VERSION__ === "string" ? __STEELENGINE_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["STEELENGINE_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["STEELENGINE_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["STEELENGINE_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.STEELENGINE_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current SteelEngine version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: readInjectedVersion(),
  bundledVersion: process.env.STEELENGINE_BUNDLED_VERSION,
});
