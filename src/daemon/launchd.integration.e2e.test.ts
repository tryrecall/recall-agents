...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `steelengine-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      STEELENGINE_LAUNCHD_LABEL: `ai.steelengine.launchd-int-${testId}`,
      STEELENGINE_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
