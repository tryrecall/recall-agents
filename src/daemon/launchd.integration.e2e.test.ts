...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `recall-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      RECALL_LAUNCHD_LABEL: `ai.tryrecall.launchd-int-${testId}`,
      RECALL_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
