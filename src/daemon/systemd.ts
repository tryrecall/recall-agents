...
function resolveSystemdServiceName(env: GatewayServiceEnv): string {
  ...
  return resolveGatewaySystemdServiceName(env.STEELENGINE_PROFILE);
}
...
function sanitizeSystemdUnitBackupContent(params: {
  ...
  // Backups should not retain file-managed secrets that SteelEngine moved into the
...
function resolveSystemdEnvironmentFilePath(params: {
  ...
  const serviceKind = params.environment?.STEELENGINE_SERVICE_KIND?.trim();
  const filename =
...
function resolveLegacyNodeSystemdEnvironmentFilePath(params: {
  ...
  if (params.environment?.STEELENGINE_SERVICE_KIND?.trim() !== "node") {
    return null;
  }
...
function isNodeSystemdEnvironment(env: GatewayServiceEnv): boolean {
  return env.STEELENGINE_SERVICE_KIND?.trim() === "node";
}
...
async function writeSystemdGatewayEnvironmentFile(params: {
  ...
  /** SteelEngine-managed keys that must not be preserved from an old env file; stale file values
  ...
  fileManagedKeys?: ReadonlySet<string>;
  /** State-dir .env keys SteelEngine previously managed but is now skipping (unresolved shell
  ...
  // SteelEngine-managed keys (identified by inlineManagedKeys) are excluded: a stale
...
async function removeNodeSystemdManagedEnvironmentKeys(env: GatewayServiceEnv): Promise<void> {
  ...
  const managedKeys = new Set(["STEELENGINE_GATEWAY_TOKEN", "STEELENGINE_GATEWAY_PASSWORD"]);
