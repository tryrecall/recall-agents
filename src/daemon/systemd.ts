...
function resolveSystemdServiceName(env: GatewayServiceEnv): string {
  const override = env.RECALL_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override.slice(0, -".service".length) : override;
  }
  return resolveGatewaySystemdServiceName(env.RECALL_PROFILE);
}
...
function resolveSystemdEnvironmentFilePath(params: {
  ...
  const serviceKind = params.environment?.RECALL_SERVICE_KIND?.trim();
  const filename =
...
function resolveLegacyNodeSystemdEnvironmentFilePath(params: {
  ...
  if (params.environment?.RECALL_SERVICE_KIND?.trim() !== "node") {
    return null;
  }
...
function isNodeSystemdEnvironment(env: GatewayServiceEnv): boolean {
  return env.RECALL_SERVICE_KIND?.trim() === "node";
}
...
async function writeSystemdGatewayEnvironmentFile(params: {
  ...
  /** Recall-managed keys that must not be preserved from an old env file; stale file values
  ...
  // Recall-managed keys (identified by inlineManagedKeys) are excluded: a stale
...
async function removeNodeSystemdManagedEnvironmentKeys(env: GatewayServiceEnv): Promise<void> {
  ...
  const managedKeys = new Set([normalizeSystemdEnvironmentKey("RECALL_GATEWAY_TOKEN")]);
