...
const RECALL_UPDATE_LAUNCHD_LABEL_PREFIX = "ai.recall.update.";

export type StaleRecallUpdateLaunchdJob = {
  label: string;
  pid?: number;
  lastExitStatus?: number;
};

function normalizeRecallUpdateLaunchdLabel(label: unknown): string | null {
  if (typeof label !== "string") {
    return null;
  }
  const trimmed = label.trim();
  return trimmed.startsWith(RECALL_UPDATE_LAUNCHD_LABEL_PREFIX) ? trimmed : null;
}

function isCurrentGatewayLaunchdLabel(label: string, env: NodeJS.ProcessEnv): boolean {
  const gatewayProfileLabel = resolveGatewayLaunchAgentLabel(env.RECALL_PROFILE);
  if (label === gatewayProfileLabel) {
    return true;
  }
  if (
    env.RECALL_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER ||
    env.RECALL_SERVICE_KIND?.trim() !== GATEWAY_SERVICE_KIND
  ) {
    return false;
  }
  const configuredLabel = env.RECALL_LAUNCHD_LABEL?.trim();
  return Boolean(configuredLabel && label === configuredLabel);
}

export function isRecallUpdateLaunchdLabel(label: unknown): label is string {
  return normalizeRecallUpdateLaunchdLabel(label) !== null;
}

function resolveCurrentRecallUpdateLaunchdJobLabel(
  ...
  for (const label of [
    ...
    env.RECALL_LAUNCHD_LABEL,
  ]) {
    const normalized = normalizeRecallUpdateLaunchdLabel(label);
    if (normalized) {
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.RECALL_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.RECALL_PROFILE));
}
...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by Recall. Do not edit while the gateway service is installed.",
...
export function parseLaunchctlListRecallUpdateJobs(
  output: string,
): StaleRecallUpdateLaunchdJob[] {
  const jobs: StaleRecallUpdateLaunchdJob[] = [];
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    if (!label.startsWith(RECALL_UPDATE_LAUNCHD_LABEL_PREFIX)) {
      continue;
    }
...
export async function findStaleRecallUpdateLaunchdJobs(): Promise<
  StaleRecallUpdateLaunchdJob[]
> {
  ...
  return parseLaunchctlListRecallUpdateJobs(result.stdout);
}

export async function removeRecallUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeRecallUpdateLaunchdLabel(label);
...
export async function disableRecallUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeRecallUpdateLaunchdLabel(label);
  if (process.platform !== "darwin" || !normalizedLabel) {
    return false;
  }
...
export async function disableCurrentRecallUpdateLaunchdJob(
  env: NodeJS.ProcessEnv = process.env,
): Promise<boolean> {
  const label = resolveCurrentRecallUpdateLaunchdJobLabel(env);
  if (!label) {
    return false;
  }
  return await disableRecallUpdateLaunchdJob(label);
}
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromServiceEnv = parseStrictPositiveInteger(
    command?.environment?.RECALL_GATEWAY_PORT ?? "",
  );
  if (fromServiceEnv !== undefined) {
    return fromServiceEnv;
  }
  const fromEnv = parseStrictPositiveInteger(env.RECALL_GATEWAY_PORT ?? "");
  return fromEnv ?? null;
}
...
function throwBootstrapGuiSessionError(params: {
  ...
  throw new Error(
    [
      ...
      "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.recall.ai/gateway",
    ].join("\n"),
  );
}
...
export async function stopLaunchAgent({
  ...
  if (!persistDisable) {
    ...
    // `recall gateway start` re-enables cleanly without a manual `launchctl enable`.
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.RECALL_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "recall gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "recall gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `recall gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    await bootstrapLaunchAgentOrThrow({
      ...
      serviceTarget,
      plistPath,
      actionHint: "recall gateway restart",
    });
