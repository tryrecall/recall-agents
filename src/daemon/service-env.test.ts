...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.STEELENGINE_GATEWAY_PORT).toBe("18789");
    expect(env.STEELENGINE_GATEWAY_TOKEN).toBeUndefined();
    expect(env.STEELENGINE_SERVICE_MARKER).toBe("steelengine");
    expect(env.STEELENGINE_SERVICE_KIND).toBe("gateway");
    expect(typeof env.STEELENGINE_SERVICE_VERSION).toBe("string");
    expect(env.STEELENGINE_SYSTEMD_UNIT).toBe("steelengine-gateway.service");
    expect(env.STEELENGINE_WINDOWS_TASK_NAME).toBe("SteelEngine Gateway");
    expect(env.STEELENGINE_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    if (process.platform === "darwin") {
      expect(env.STEELENGINE_LAUNCHD_LABEL).toBe("ai.steelengine.gateway");
    }
  });

  it("sets the SteelEngine-owned launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user" },
      port: 18789,
      platform: "darwin",
  ...
  it("passes through STEELENGINE_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
    ...
    expect(env.STEELENGINE_WRAPPER).toBe("/usr/local/bin/steelengine-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".steelengine", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", STEELENGINE_PROFILE: "work" },
      port: 18789,
    });
    expect(env.STEELENGINE_SYSTEMD_UNIT).toBe("steelengine-gateway-work.service");
    expect(env.STEELENGINE_WINDOWS_TASK_NAME).toBe("SteelEngine Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.STEELENGINE_LAUNCHD_LABEL).toBe("ai.steelengine.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        ...
        STEELENGINE_SYSTEMD_UNIT: "steelengine-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.STEELENGINE_SYSTEMD_UNIT).toBe("steelengine-gateway-maintenance.service");
  });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", STEELENGINE_PROFILE: "work" },
      port: 18789,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    ...
    expect(env.STEELENGINE_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the SteelEngine-owned launchd marker for macOS node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/Users/user" },
      platform: "darwin",
    });

    expect(env.STEELENGINE_LAUNCHD_LABEL).toBe("ai.steelengine.node");
  });

  it("passes through STEELENGINE_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", STEELENGINE_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.STEELENGINE_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through STEELENGINE_GATEWAY_PASSWORD for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", STEELENGINE_GATEWAY_PASSWORD: " node-password " },
    });
    expect(env.STEELENGINE_GATEWAY_PASSWORD).toBe("node-password");
  });

  it("passes through STEELENGINE_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", STEELENGINE_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.STEELENGINE_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits STEELENGINE_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.STEELENGINE_WINDOWS_TASK_NAME).toBe("SteelEngine Node");
    expect(env.STEELENGINE_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.STEELENGINE_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".steelengine"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", STEELENGINE_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".steelengine-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", STEELENGINE_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".steelengine"));
  });

  it("uses STEELENGINE_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", STEELENGINE_STATE_DIR: "/var/lib/steelengine" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/steelengine"));
  });

  it("expands ~ in STEELENGINE_STATE_DIR", () => {
    const env = { HOME: "/Users/test", STEELENGINE_STATE_DIR: "~/steelengine-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/steelengine-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { STEELENGINE_STATE_DIR: "C:\\State\\steelengine" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\steelengine");
  });
});
