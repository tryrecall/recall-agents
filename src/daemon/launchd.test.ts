...
import {
  ...
  disableRecallUpdateLaunchdJob,
  findStaleRecallUpdateLaunchdJobs,
  isLaunchAgentListed,
  isRecallUpdateLaunchdLabel,
  parseLaunchctlPrint,
  parseLaunchctlListRecallUpdateJobs,
  readLaunchAgentProgramArguments,
  readLaunchAgentRuntime,
  repairLaunchAgentBootstrap,
  removeRecallUpdateLaunchdJob,
  restartLaunchAgent,
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.recall.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", RECALL_PROFILE: "default" },
    });
  ...
  it("parses stale Recall updater jobs from launchctl list", () => {
    const jobs = parseLaunchctlListRecallUpdateJobs(
      [
        "123 0 ai.recall.gateway",
        "- 127 ai.recall.update.2026.5.12",
        "8142 0 ai.recall.update.2026.5.13-beta.1",
        "- 0 com.example.other",
      ].join("\n"),
    );
    ...
    expect(jobs).toEqual([
      ...
      {
        label: "ai.recall.update.2026.5.13-beta.1",
        pid: 8142,
        lastExitStatus: 0,
      },
    ]);
  ...
  it.runIf(process.platform === "darwin")(
    "finds stale Recall updater jobs via launchctl list",
    async () => {
      state.listOutput = "- 127 ai.recall.update.2026.5.12\n";

      const jobs = await findStaleRecallUpdateLaunchdJobs();
  ...
  it("recognizes only legacy Recall update launchd labels", () => {
    expect(isRecallUpdateLaunchdLabel("ai.recall.update.2026.5.12")).toBe(true);
    expect(isRecallUpdateLaunchdLabel("ai.recall.gateway")).toBe(false);
    expect(isRecallUpdateLaunchdLabel("com.example.update")).toBe(false);
  });

  it.runIf(process.platform === "darwin")("removes legacy updater launchd jobs", async () => {
    await expect(removeRecallUpdateLaunchdJob(" ai.recall.update.2026.5.12 ")).resolves.toBe(
      true,
    );

    expect(state.launchctlCalls).toContainEqual(["remove", "ai.recall.update.2026.5.12"]);
  });
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentRecallUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.recall.update.2026.5.12",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.recall.update.2026.5.12`,
      ]);
  ...
  it.runIf(process.platform === "darwin")(
    "disables the current legacy updater launchd job from Recall label env",
    async () => {
      await expect(
        disableCurrentRecallUpdateLaunchdJob({
          RECALL_LAUNCHD_LABEL: "ai.recall.update.2026.5.12",
        }),
      ).resolves.toBe(true);
  ...
  it.runIf(process.platform === "darwin")(
    "does not let non-update launchd markers mask the Recall update label",
    async () => {
      await expect(
        disableCurrentRecallUpdateLaunchdJob({
          XPC_SERVICE_NAME: "0",
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentRecallUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.recall.gateway",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentRecallUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.recall.update.2026.5.12",
          RECALL_PROFILE: "update.2026.5.12",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")("disables explicit legacy updater jobs", async () => {
    await expect(disableRecallUpdateLaunchdJob("ai.recall.update.2026.5.12")).resolves.toBe(
      true,
    );
...
describe("launchd install", () => {
  ...
  it("writes LaunchAgent environment to an owner-only env file when provided", async () => {
    const env = createDefaultLaunchdEnv();
    const tmpDir = "/Users/test/.recall/tmp";
    ...
    const plistPath = resolveLaunchAgentPlistPath(env);
    const envFilePath = "/Users/test/.recall/service-env/ai.recall.gateway.env";
    const wrapperPath = "/Users/test/.recall/service-env/ai.recall.gateway-env-wrapper.sh";
    ...
    expect(state.dirModes.get("/Users/test/.recall/service-env")).toBe(0o700);
  ...
  it("repairs a mangled label-derived service-env wrapper path on restart", async () => {
    ...
    const serviceEnv = {
      ...callerEnv,
      RECALL_STATE_DIR: "/Users/test/service-env/custom-state",
    };
    await installLaunchAgent({
      env: serviceEnv,
      stdout: new PassThrough(),
      ...
      environment: {
        ...
        RECALL_STATE_DIR: serviceEnv.RECALL_STATE_DIR,
      },
    });

    const plistPath = resolveLaunchAgentPlistPath(callerEnv);
    const envFilePath = "/Users/test/service-env/custom-state/service-env/ai.recall.gateway.env";
    const wrapperPath =
      "/Users/test/service-env/custom-state/service-env/ai.recall.gateway-env-wrapper.sh";
    const callerEnvFilePath = "/Users/test/.recall/service-env/ai.recall.gateway.env";
    const callerWrapperPath =
      "/Users/test/.recall/service-env/ai.recall.gateway-env-wrapper.sh";
    const mangledEnvFilePath =
      "/Users/test/service-env/custom-state/service-env/[ai.recall.gateway.env](http:/ai.recall.gateway.env)";
    const mangledWrapperPath =
      "/Users/test/service-env/custom-state/service-env/[ai.recall.gateway-env-wrapper.sh](http:/ai.recall.gateway-env-wrapper.sh)";
    state.files.set(
      plistPath,
    ...
    expect(command?.programArguments).toEqual(defaultProgramArguments);
    expect(command?.environment?.RECALL_GATEWAY_PORT).toBe("18789");
    expect(command?.environment?.RECALL_STATE_DIR).toBe(serviceEnv.RECALL_STATE_DIR);
    expect(command?.environmentValueSources?.RECALL_GATEWAY_PORT).toBe("file");
    ...
    expect(rewrittenEnv).toContain("export RECALL_GATEWAY_PORT='18789'");
    expect(rewrittenEnv).toContain(
      "export RECALL_STATE_DIR='/Users/test/service-env/custom-state'",
    );
  });
  ...
  it("writes KeepAlive=true policy with shutdown and throttle limits", async () => {
    ...
    expect(plist).toContain("<string>/Users/test/Library/Logs/recall/gateway.log</string>");
  ...
  it("rewrites the plist before bootstrap during restart fallback", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "  <dict>",
        "    <key>Label</key>",
        "    <string>ai.recall.gateway</string>",
  ...
  it("stops LaunchAgent via bootout by default, preserving KeepAlive for future crashes", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.recall.gateway`;
  ...
  it("resolves the stop postcondition port from the stored LaunchAgent environment", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { RECALL_GATEWAY_PORT: "19006" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("reloads launchd after rewriting an existing plist", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "    <string>/Users/test/.recall-default/logs/gateway.log</string>",
        "  </dict>",
        "</plist>",
      ].join("\n"),
    );
  ...
  it("uses the stored LaunchAgent environment port for restart stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { RECALL_GATEWAY_PORT: "19007" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("hands plist reload off when current LaunchAgent needs rewritten paths", async () => {
    ...
    expect(state.files.get(plistPath)).toContain("/Users/test/Library/Logs/recall/gateway.log");
    expect(state.launchctlCalls).toStrictEqual([]);
  });
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.recall.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when RECALL_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.recall.gateway.plist",
    },
    {
      name: "uses profile-specific label when RECALL_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", RECALL_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.recall.jbphoenix.plist",
    },
    {
      name: "prefers RECALL_LAUNCHD_LABEL over RECALL_PROFILE",
      env: {
        HOME: "/Users/test",
        RECALL_PROFILE: "jbphoenix",
        RECALL_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from RECALL_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        RECALL_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty RECALL_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        RECALL_PROFILE: "myprofile",
        RECALL_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.recall.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
  ...
  it("rejects invalid launchd labels that contain path separators", () => {
    expect(() =>
      resolveLaunchAgentPlistPath({
        HOME: "/Users/test",
        RECALL_LAUNCHD_LABEL: "../evil/label",
      }),
