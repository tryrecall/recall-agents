...
async function withPreparedGatewayTask(
  ...
  await withWindowsEnv("steelengine-win-stop-", async ({ env }) => {
...
describe("Scheduled Task stop/restart cleanup", () => {
  it("suspends a task whose Settings.Enabled value uses the default", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toEqual([
        ["/Query", "/TN", "SteelEngine Gateway", "/XML"],
        ["/Change", "/TN", "SteelEngine Gateway", "/DISABLE"],
      ]);
    });
  });
  ...
  it("preserves an already-disabled task", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toEqual([["/Query", "/TN", "SteelEngine Gateway", "/XML"]]);
    });
  });
  ...
  it("restores an enabled task after an ambiguous disable failure", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toEqual([
        ["/Query", "/TN", "SteelEngine Gateway", "/XML"],
        ["/Change", "/TN", "SteelEngine Gateway", "/DISABLE"],
        ["/Change", "/TN", "SteelEngine Gateway", "/ENABLE"],
      ]);
    });
  });
  ...
  it("reenables a task after the update window", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toEqual([["/Change", "/TN", "SteelEngine Gateway", "/ENABLE"]]);
    });
  });
  ...
  it("starts a registered task and ignores audit observer failures", async () => {
    await withPreparedGatewayTask(async ({ env }) => {
      ...
      expect(schtasksCalls).toContainEqual(["/Run", "/TN", "SteelEngine Gateway"]);
  ...
  it("kills lingering verified gateway listeners and waits for port release before restart", async () => {
    await withPreparedGatewayTask(async ({ env, stdout }) => {
      ...
      expect(schtasksCalls).toEqual([
        ["/Query"],
        ["/Query", "/TN", "SteelEngine Gateway"],
        ["/End", "/TN", "SteelEngine Gateway"],
        ["/Run", "/TN", "SteelEngine Gateway"],
        ["/Query"],
        ["/Query", "/TN", "SteelEngine Gateway", "/V", "/FO", "LIST"],
        ["/Query"],
        ["/Query", "/TN", "SteelEngine Gateway", "/V", "/FO", "LIST"],
      ]);
    });
  ...
  it("does not wait on or force-kill the gateway port when restarting a node Scheduled Task", async () => {
    await withPreparedGatewayTask(async ({ env, stdout }) => {
      ...
      expect(schtasksCalls).toEqual([
        ...
        ["/Query", "/TN", "SteelEngine Node"],
        ["/End", "/TN", "SteelEngine Node"],
        ["/Run", "/TN", "SteelEngine Node"],
        ["/Query"],
        ["/Query", "/TN", "SteelEngine Node", "/V", "/FO", "LIST"],
        ["/Query"],
        ["/Query", "/TN", "SteelEngine Node", "/V", "/FO", "LIST"],
      ]);
    });
  ...
  it("throws when /Run fails during restart", async () => {
    await withPreparedGatewayTask(async ({ env, stdout }) => {
      ...
      expect(schtasksCalls.at(-1)).toEqual(["/Run", "/TN", "SteelEngine Gateway"]);
    });
  });
});
