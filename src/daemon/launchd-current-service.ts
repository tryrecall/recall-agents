...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  const configuredLabel = normalizeOptionalString(env.STEELENGINE_LAUNCHD_LABEL);
  if (!configuredLabel || configuredLabel !== label) {
    return false;
  }
  if (
    normalizeOptionalString(env.STEELENGINE_SERVICE_MARKER) === "steelengine" &&
    Boolean(normalizeOptionalString(env.STEELENGINE_SERVICE_KIND))
  ) {
