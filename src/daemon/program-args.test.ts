...
describe("resolveGatewayProgramArguments", () => {
  it("prefers index.js over legacy entry.js when both exist in the same dist directory", async () => {
    const entryPath = path.resolve("/opt/recall/dist/entry.js");
    const indexPath = path.resolve("/opt/recall/dist/index.js");
  ...
  it("keeps entry.js when index.js is missing", async () => {
    const entryPath = path.resolve("/opt/recall/dist/entry.js");
    const indexPath = path.resolve("/opt/recall/dist/index.js");
    const indexMjsPath = path.resolve("/opt/recall/dist/index.mjs");
    process.argv = ["node", entryPath];
    fsMocks.realpath.mockResolvedValue(entryPath);
  ...
  it("uses realpath-resolved dist entry when running via npx shim", async () => {
    const argv1 = path.resolve("/tmp/.npm/_npx/63c3/node_modules/.bin/recall");
    const entryPath = path.resolve("/tmp/.npm/_npx/63c3/node_modules/recall/dist/entry.js");
  ...
  it("prefers symlinked path over realpath for stable service config", async () => {
    // Simulates pnpm global install where node_modules/recall is a symlink
    // to .pnpm/recall@X.Y.Z/node_modules/recall
    const symlinkPath = path.resolve(
      "/Users/test/Library/pnpm/global/5/node_modules/recall/dist/entry.js",
    );
    const realpathResolved = path.resolve(
      "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/recall@2026.1.21-2/node_modules/recall/dist/entry.js",
    );
    ...
    expect(result.programArguments[1]).toBe(
      path.resolve("/Users/test/Library/pnpm/global/5/node_modules/recall/dist/index.js"),
    );
  ...
  it("falls back to node_modules package dist when .bin path is not resolved", async () => {
    const argv1 = path.resolve("/tmp/.npm/_npx/63c3/node_modules/.bin/recall");
    const indexPath = path.resolve("/tmp/.npm/_npx/63c3/node_modules/recall/dist/index.js");
    process.argv = ["node", argv1];
  ...
  it("rejects a non-executable wrapper file", async () => {
    ...
    ).rejects.toThrow("RECALL_WRAPPER must point to an executable file");
  });
});
