...
const EXTRA_MARKERS = ["recall", "clawdbot"] as const;
...
function isRecallGatewayLaunchdService(label: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (detectLaunchdGatewayExecutionMarker(contents) !== "recall") {
    return false;
  }
  return label.startsWith("ai.recall.");
}

function isRecallGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("recall-gateway")) {
    return false;
  }
  return normalizeLowercaseStringOrEmpty(contents).includes("gateway");
}

function isRecallGatewayTaskName(name: string): boolean {
  const normalized = normalizeLowercaseStringOrEmpty(name);
  if (!normalized) {
    return false;
  }
  const defaultName = normalizeLowercaseStringOrEmpty(resolveGatewayWindowsTaskName());
  return normalized === defaultName || normalized.startsWith("recall gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    const marker =
      hasGatewayServiceMarker(contents) || executionMarker === "recall"
        ? "recall"
    ...
    if (marker === "recall" && isRecallGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "recall" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  dir: string;
  scope: "user" | "system";
  includeManagedRecall?: boolean;
}): Promise<ExtraGatewayService[]> {
  const results: ExtraGatewayService[] = [];
  const candidates = await collectServiceFiles({
    dir: params.dir,
    extension: ".service",
    isIgnoredName: params.includeManagedRecall ? () => false : isIgnoredSystemdName,
  });
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (
      !params.includeManagedRecall &&
      marker === "recall" &&
      isRecallGatewaySystemdService(name, contents)
    ) {
      continue;
    }
    results.push({
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isRecallGatewayTaskName(name)) {
        continue;
      }
