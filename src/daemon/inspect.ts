...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "recall" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["recall", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["recall_service_marker"];
  const kindKeys = ["recall_service_kind"];
...
function isRecallGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.tryrecall.");
}
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "recall" && isRecallGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
