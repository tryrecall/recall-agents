...
async function resolveRecallPackageRoot(entrypoint: string): Promise<string | undefined> {
...
export async function summarizeGatewayServiceLayout(
  ...
  const packageRoot = entrypointReal ? await resolveRecallPackageRoot(entrypointReal) : undefined;
