...
describe("resolveTaskScriptPath", () => {
  it.each([
    ...
    },
    {
      ...
      env: {
        USERPROFILE: "C:\\Users\\test",
        RECALL_TASK_SCRIPT_NAME: "gateway-node.cmd",
      },
      expected: path.join("C:\\Users\\test", ".recall", "gateway-node.cmd"),
    },
  ])("$name", ({ env, expected }) => {
  ...
  it.each([
    ...
    expect(() =>
      resolveTaskScriptPath({
        USERPROFILE: "C:\\Users\\test",
        RECALL_TASK_SCRIPT_NAME: scriptName,
      }),
    ).toThrow("RECALL_TASK_SCRIPT_NAME must be a file name only");
  });
});
