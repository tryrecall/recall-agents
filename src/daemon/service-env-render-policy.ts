...
function isLaunchAgentServiceEnvironment(params: {
  ...
  return (
    ...
    Boolean(params.serviceEnvironment.RECALL_LAUNCHD_LABEL?.trim())
  );
}
...
export function applyManagedServiceEnvRenderPolicy(params: {
  ...
  if (params.plan.environment.RECALL_SERVICE_MANAGED_ENV_KEYS) {
    params.plan.environmentValueSources.RECALL_SERVICE_MANAGED_ENV_KEYS = "inline";
  }
  if (!isLaunchAgentServiceEnvironment(params)) {
    return;
  }
  const managedKeys = readManagedServiceEnvKeysFromEnvironment({
    RECALL_SERVICE_MANAGED_ENV_KEYS: params.managedServiceEnvKeys,
  });
