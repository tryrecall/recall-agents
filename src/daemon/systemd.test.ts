...
vi.mock("node:child_process", async () => {
  const { mockNodeChildProcessExecFile } = await import("recall/plugin-sdk/test-node-mocks");
...
const TEST_MANAGED_HOME = "/tmp/recall-test-home";
const GATEWAY_SERVICE = "recall-gateway.service";
const NODE_SERVICE = "recall-node.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/recall gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/recall", "gateway", "run"]);
...
describe("systemd availability", () => {
  ...
  it("does not let stale SUDO_USER override a sudo-u target user scope", async () => {
    ...
    await expect(
      isSystemdUserServiceAvailable({ USER: "recall", SUDO_USER: "admin" }),
    ).resolves.toBe(true);
    expect(execFileMock).toHaveBeenCalledTimes(1);
  });
});
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/recall-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "recall-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "recall-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/recall-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled recall-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled recall-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when RECALL_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/recall-gateway.service",
    },
    {
      name: "uses profile-specific service name when RECALL_PROFILE is set to a custom value",
      env: { HOME: "/home/test", RECALL_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/recall-gateway-jbphoenix.service",
    },
    {
      name: "prefers RECALL_SYSTEMD_UNIT over RECALL_PROFILE",
      env: {
        HOME: "/home/test",
        RECALL_PROFILE: "jbphoenix",
        RECALL_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles RECALL_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        RECALL_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from RECALL_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        RECALL_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/recall gateway start --name "My Bot"')).toEqual([
      "/usr/bin/recall",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('recall --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["recall", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('recall --path "C:\\\\Program Files\\\\Recall"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["recall", "--path", "C:\\\\Program Files\\\\Recall"]);

    expect(
      splitArgsPreservingQuotes('recall --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["recall", "--label", 'My "Quoted" Name']);
  });
});
...
describe("parseSystemdEnvAssignments", () => {
  it("parses single-quoted whole assignments", () => {
    expect(
      parseSystemdEnvAssignments("'RECALL_GATEWAY_TOKEN=single quoted token' FOO=bar"),
    ).toEqual([
      { key: "RECALL_GATEWAY_TOKEN", value: "single quoted token" },
      { key: "FOO", value: "bar" },
    ]);
  });

  it("keeps apostrophes inside unquoted assignment values literal", () => {
    expect(parseSystemdEnvAssignments("FOO=can't RECALL_GATEWAY_TOKEN=token")).toEqual([
      { key: "FOO", value: "can't" },
      { key: "RECALL_GATEWAY_TOKEN", value: "token" },
    ]);
  });
});
...
describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/recall gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads RECALL_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/recall gateway run", "EnvironmentFile=%h/.recall/.env"],
      { [`${TEST_SERVICE_HOME}/.recall/.env`]: "RECALL_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.RECALL_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/recall gateway run",
        "EnvironmentFile=%h/.recall/.env",
        'Environment="RECALL_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.recall/.env`]: "RECALL_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.RECALL_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.RECALL_GATEWAY_TOKEN).toBe("inline-and-file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.recall/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/recall-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/recall gateway run",
          'EnvironmentFile=%h/.recall/first.env "%h/.recall/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.recall/first.env") {
        return "RECALL_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.recall/second env.env") {
        return 'RECALL_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      ...
      RECALL_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "RECALL_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "RECALL_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "RECALL_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      RECALL_GATEWAY_TOKEN: "override-token",
      RECALL_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/recall-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/recall gateway run",
          "EnvironmentFile=%h/.recall/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.recall/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'RECALL_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "RECALL_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      RECALL_GATEWAY_TOKEN: "quoted token",
      RECALL_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      RECALL_GATEWAY_TOKEN: "file",
      RECALL_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("stageSystemdService", () => {
  async function withStageFixture(
    ...
    const tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-systemd-stage-"));
    const home = path.join(tempHomeRoot, "home");
    const stateDir = path.join(home, ".recall");
    const env = {
      HOME: home,
      RECALL_STATE_DIR: stateDir,
      RECALL_SYSTEMD_UNIT: "recall-gateway-stage-test",
    };
  ...
  it("writes dotenv-backed values to a separate env file and keeps inline env minimal", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      await fs.writeFile(
        ...
        ["RECALL_GATEWAY_TOKEN=dotenv-token", "LLM_API_KEY=dotenv-key"].join("\n"),
        "utf8",
      );
      ...
      await stageSystemdService({
        ...
        environment: {
          RECALL_GATEWAY_TOKEN: "dotenv-token",
      ...
      expect(unit).not.toContain("Environment=RECALL_GATEWAY_TOKEN=dotenv-token");
      expect(unit).not.toContain("Environment=LLM_API_KEY=dotenv-key");
      expect(envFile).toBe("RECALL_GATEWAY_TOKEN=dotenv-token\nLLM_API_KEY=dotenv-key\n");
  ...
  it("writes node file-backed managed values to the node env file instead of the unit", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath, nodeEnvFilePath }) => {
      ...
      await stageSystemdService({
        env,
        stdout: { write: vi.fn() } as unknown as NodeJS.WritableStream,
        programArguments: ["/usr/bin/recall", "node", "run"],
        workingDirectory: "/tmp",
        environment: {
          RECALL_GATEWAY_TOKEN: "file-backed-token",
          RECALL_GATEWAY_PORT: "18789",
          RECALL_SERVICE_KIND: "node",
        },
      ...
      expect(unit).toContain("Environment=RECALL_GATEWAY_PORT=18789");
      expect(unit).not.toContain("Environment=RECALL_GATEWAY_TOKEN=file-backed-token");
      expect(envFile).toBe("RECALL_GATEWAY_TOKEN=file-backed-token\n");
      expect(envFileStat.mode & 0o777).toBe(0o600);
  ...
  it("migrates operator entries from the legacy gateway env file when writing node env files", async () => {
    await withStageFixture(async ({ env, unitPath, envFilePath, nodeEnvFilePath }) => {
      const legacyGatewayEnvFile =
        ["RECALL_GATEWAY_TOKEN=legacy-node-token", "OPENROUTER_API_KEY=operator-key"].join("\n") +
        "\n";
      ...
      await stageSystemdService({
        ...
        workingDirectory: "/tmp",
        environment: {
          RECALL_GATEWAY_TOKEN: "fresh-file-token",
          RECALL_GATEWAY_PORT: "18789",
          RECALL_SERVICE_KIND: "node",
        },
        environmentValueSources: {
      ...
      expect(unit).toContain(`EnvironmentFile=-${nodeEnvFilePath}`);
      expect(unit).not.toContain("RECALL_GATEWAY_TOKEN=fresh-file-token");
  ...
  it("clears stale node file-backed managed keys without touching the gateway env file", async () => {
    await withStageFixture(async ({ env, unitPath, envFilePath, nodeEnvFilePath }) => {
      await fs.writeFile(envFilePath, "RECALL_GATEWAY_TOKEN=stale-token\n", {
        encoding: "utf8",
        mode: 0o600,
      });
      await fs.writeFile(nodeEnvFilePath, "RECALL_GATEWAY_TOKEN=stale-node-token\n", {
        encoding: "utf8",
        mode: 0o600,
      });
      ...
      await expect(fs.readFile(envFilePath, "utf8")).resolves.toBe(
        "RECALL_GATEWAY_TOKEN=stale-token\n",
      );
    });
  });
  ...
  it("sanitizes file-backed managed values out of the backup unit on re-stage", async () => {
    await withStageFixture(async ({ env, unitPath }) => {
      ...
      await fs.writeFile(
        ...
        [
          "[Service]",
          "ExecStart=/usr/bin/recall node run",
          "Environment=FOO=bar RECALL_GATEWAY_TOKEN=inline-token BAZ=qux",
          "Environment=RECALL_GATEWAY_TOKEN=token-only-line",
          "Environment='RECALL_GATEWAY_TOKEN=single-quoted-token' FROM_SINGLE=kept",
          "Environment=RECALL_GATEWAY_PORT=18789",
        ].join("\n"),
      ...
      await stageSystemdService({
        ...
        workingDirectory: "/tmp",
        environment: {
          RECALL_GATEWAY_TOKEN: "fresh-token",
          RECALL_GATEWAY_PORT: "18789",
          RECALL_SERVICE_KIND: "node",
        },
        environmentValueSources: {
      ...
      expect(unit).not.toContain("Environment=RECALL_GATEWAY_TOKEN=fresh-token");
      expect(backupUnit).not.toContain("Environment=RECALL_GATEWAY_TOKEN=inline-token");
      expect(backupUnit).not.toContain("Environment=RECALL_GATEWAY_TOKEN=token-only-line");
      ...
      expect(backupUnit).toContain("Environment=RECALL_GATEWAY_PORT=18789");
  ...
  it("keeps inline overrides out of the generated env file", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      await fs.writeFile(
        path.join(stateDir, ".env"),
        ["RECALL_GATEWAY_TOKEN=stale-token", "LLM_API_KEY=dotenv-key"].join("\n"),
        "utf8",
      );
      ...
      expect(unit).toContain(`EnvironmentFile=-${envFilePath}`);
      expect(unit).toContain("Environment=RECALL_GATEWAY_TOKEN=fresh-token");
  ...
  it("clears stale inline-managed keys from env file on re-stage (#76860)", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      // Existing env file carries a stale RECALL_GATEWAY_TOKEN that the
      // operator previously wrote there but staging now supplies inline.
      await fs.writeFile(
        envFilePath,
        ["RECALL_GATEWAY_TOKEN=stale-gateway-token", "OPENROUTER_API_KEY=or-operator-key"].join(
          "\n",
        ) + "\n",
        { encoding: "utf8", mode: 0o600 },
      );
      ...
      await stageSystemdService({
        ...
        programArguments: ["/usr/bin/recall", "gateway", "run"],
        workingDirectory: "/tmp",
        // Staging manages RECALL_GATEWAY_TOKEN inline; RECALL_SERVICE_MANAGED_ENV_KEYS
        // marks it as an Recall-managed key so the stale env-file copy is cleared.
        environment: {
          RECALL_GATEWAY_TOKEN: "fresh-gateway-token",
          LLM_API_KEY: "dotenv-key",
          OPENROUTER_API_KEY: "or-operator-key",
          RECALL_SERVICE_MANAGED_ENV_KEYS: "RECALL_GATEWAY_TOKEN",
        },
        environmentValueSources: {
          RECALL_GATEWAY_TOKEN: "inline-and-file",
          LLM_API_KEY: "inline",
          OPENROUTER_API_KEY: "file",
          RECALL_SERVICE_MANAGED_ENV_KEYS: "inline",
        },
      });
      ...
      expect(envFile).not.toContain("RECALL_GATEWAY_TOKEN");
      ...
      expect(unit).toContain("Environment=RECALL_GATEWAY_TOKEN=fresh-gateway-token");
...
describe("systemd service install and uninstall", () => {
  async function withNodeSystemdFixture(
    ...
    const tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-node-systemd-"));
    const home = path.join(tempHomeRoot, "home");
    const stateDir = path.join(home, ".recall");
    const env = {
      HOME: home,
      RECALL_STATE_DIR: stateDir,
      RECALL_SYSTEMD_UNIT: "recall-node",
      RECALL_SERVICE_KIND: "node",
    };
    const unitPath = resolveSystemdUserUnitPath(env);
    const nodeEnvFilePath = path.join(stateDir, "node.systemd.env");
  ...
  it("activates the RECALL_SYSTEMD_UNIT override during install", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath }) => {
      ...
      const unit = await fs.readFile(unitPath, "utf8");
      expect(unitPath).toMatch(/recall-node\.service$/);
      expect(unit).toContain("recall node run");
      ...
      execFileMock
        ...
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          ...
          cb(
            ...
            "",
            "Unit file recall-node.service does not exist.",
          );
        })
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertUserSystemctlArgs(args, "daemon-reload");
  ...
  it("uses the sudo-u target user for install activation machine-scope retry", async () => {
    await withNodeSystemdFixture(async ({ env }) => {
      mockEffectiveUid(1000);
      const installEnv = { ...env, USER: "recall", SUDO_USER: "admin" };
      execFileMock
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertUserSystemctlArgs(args, "status");
          cb(null, "", "");
        ...
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertMachineUserSystemctlArgs(args, "recall", "enable", NODE_SERVICE);
          cb(null, "", "");
        })
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertUserSystemctlArgs(args, "restart", NODE_SERVICE);
  ...
  it("disables the RECALL_SYSTEMD_UNIT override during uninstall", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath, nodeEnvFilePath }) => {
      await fs.mkdir(path.dirname(unitPath), { recursive: true });
      await fs.writeFile(unitPath, "[Unit]\nDescription=Recall Node\n", "utf8");
      await fs.writeFile(
        nodeEnvFilePath,
        "RECALL_GATEWAY_TOKEN=stale-node-token\nOPENROUTER_API_KEY=operator-key\n",
        { encoding: "utf8", mode: 0o600 },
      );
  ...
  it("preserves node env file values when unit removal fails during uninstall", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath, nodeEnvFilePath }) => {
      ...
      await expect(fs.readFile(unitPath, "utf8")).resolves.toContain("Recall Node");
      await expect(fs.readFile(nodeEnvFilePath, "utf8")).resolves.toBe(
        "RECALL_GATEWAY_TOKEN=stale-node-token\nOPENROUTER_API_KEY=operator-key\n",
      );
      expect(execFileMock).toHaveBeenCalledTimes(2);
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "recall-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ RECALL_PROFILE: "work" });
  });
