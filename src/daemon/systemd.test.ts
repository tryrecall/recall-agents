...
const TEST_MANAGED_HOME = "/tmp/steelengine-test-home";
const GATEWAY_SERVICE = "steelengine-gateway.service";
const NODE_SERVICE = "steelengine-node.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/steelengine gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/steelengine", "gateway", "run"]);
...
describe("systemd availability", () => {
  ...
  it("does not let stale SUDO_USER override a sudo-u target user scope", async () => {
    ...
    await expect(
      isSystemdUserServiceAvailable({ USER: "steelengine", SUDO_USER: "admin" }),
    ).resolves.toBe(true);
    expect(execFileMock).toHaveBeenCalledTimes(1);
  });
});
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/steelengine-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "steelengine-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "steelengine-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/steelengine-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled steelengine-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("system-scope gateway unit detection (steelengine#87577)", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    execFileMock.mockReset();
  });
  ...
  it("findInstalledSystemdGatewayScope prefers user scope when both exist", async () => {
    mockUnitFileLayout({
      user: true,
      system: "/etc/systemd/system/steelengine-gateway.service",
    });
    ...
    expect(result?.unitPath).toContain("/.config/systemd/user/steelengine-gateway.service");
  });

  it("findInstalledSystemdGatewayScope detects system-scope unit in /etc/systemd/system", async () => {
    mockUnitFileLayout({ system: "/etc/systemd/system/steelengine-gateway.service" });
    const result = await findInstalledSystemdGatewayScope({ HOME: TEST_MANAGED_HOME });
  ...
  it("findInstalledSystemdGatewayScope falls back to /usr/lib/systemd/system", async () => {
    mockUnitFileLayout({ system: "/usr/lib/systemd/system/steelengine-gateway.service" });
    const result = await findInstalledSystemdGatewayScope({ HOME: TEST_MANAGED_HOME });
    expect(result?.scope).toBe("system");
    expect(result?.unitPath).toBe("/usr/lib/systemd/system/steelengine-gateway.service");
  });
  ...
  it("findInstalledSystemdGatewayScope falls back to marker-owned system unit with custom name", async () => {
    ...
    findSystemGatewayServicesMock.mockResolvedValueOnce([
      {
        platform: "linux",
        label: "steelengine.service",
        detail: "unit: /etc/systemd/system/steelengine.service",
        scope: "system",
        marker: "steelengine",
      },
    ]);
  ...
  it("isSystemdServiceEnabled queries the marker-owned custom system unit name", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, args, _opts, cb) => {
      expect(args).toEqual(["is-enabled", "steelengine.service"]);
      cb(null, "enabled\n", "");
    });
  ...
  it("restartSystemdService surfaces sudo guidance using the marker-owned custom unit name", async () => {
    ...
    await expect(
      ...
      /steelengine\.service is a system-scope unit \(\/etc\/systemd\/system\/steelengine\.service\); run `sudo systemctl restart steelengine\.service`/,
    );
    expect(execFileMock).not.toHaveBeenCalled();
  ...
  it("readSystemdServiceRuntime queries the system manager for system-scope units", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, args, _opts, cb) => {
      ...
      cb(
        null,
        [
          "Id=steelengine-gateway.service",
          "ActiveState=active",
          "SubState=running",
          "MainPID=4242",
        ].join("\n"),
    ...
    expect(runtime.systemd?.unit).toBe("steelengine-gateway.service");
  });
  ...
  it("restartSystemdService refuses to use the user manager when the unit is system-scope and the caller is not root", async () => {
    ...
    await expect(
      ...
      /system-scope unit .* run `sudo systemctl restart steelengine-gateway\.service`/,
    );
    expect(execFileMock).not.toHaveBeenCalled();
    expect(write).not.toHaveBeenCalled();
  });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled steelengine-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("readSystemdServiceRuntime", () => {
  ...
  // wedged systemd socket cannot hang `steelengine status` (which advertises --timeout).
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when STEELENGINE_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/steelengine-gateway.service",
    },
    {
      name: "uses profile-specific service name when STEELENGINE_PROFILE is set to a custom value",
      env: { HOME: "/home/test", STEELENGINE_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/steelengine-gateway-jbphoenix.service",
    },
    {
      name: "prefers STEELENGINE_SYSTEMD_UNIT over STEELENGINE_PROFILE",
      env: {
        HOME: "/home/test",
        STEELENGINE_PROFILE: "jbphoenix",
        STEELENGINE_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles STEELENGINE_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        STEELENGINE_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from STEELENGINE_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        STEELENGINE_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/steelengine gateway start --name "My Bot"')).toEqual([
      "/usr/bin/steelengine",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('steelengine --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["steelengine", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('steelengine --path "C:\\\\Program Files\\\\SteelEngine"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["steelengine", "--path", "C:\\\\Program Files\\\\SteelEngine"]);

    expect(
      splitArgsPreservingQuotes('steelengine --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["steelengine", "--label", 'My "Quoted" Name']);
  });
});
...
describe("parseSystemdEnvAssignments", () => {
  it("parses single-quoted whole assignments", () => {
    expect(
      parseSystemdEnvAssignments("'STEELENGINE_GATEWAY_TOKEN=single quoted token' FOO=bar"),
    ).toEqual([
      { key: "STEELENGINE_GATEWAY_TOKEN", value: "single quoted token" },
      { key: "FOO", value: "bar" },
    ]);
  });

  it("keeps apostrophes inside unquoted assignment values literal", () => {
    expect(parseSystemdEnvAssignments("FOO=can't STEELENGINE_GATEWAY_TOKEN=token")).toEqual([
      { key: "FOO", value: "can't" },
      { key: "STEELENGINE_GATEWAY_TOKEN", value: "token" },
    ]);
  });
});
...
describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/steelengine gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads STEELENGINE_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/steelengine gateway run", "EnvironmentFile=%h/.steelengine/.env"],
      { [`${TEST_SERVICE_HOME}/.steelengine/.env`]: "STEELENGINE_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.STEELENGINE_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/steelengine gateway run",
        "EnvironmentFile=%h/.steelengine/.env",
        'Environment="STEELENGINE_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.steelengine/.env`]: "STEELENGINE_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.STEELENGINE_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.STEELENGINE_GATEWAY_TOKEN).toBe("inline-and-file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.steelengine/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/steelengine-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/steelengine gateway run",
          'EnvironmentFile=%h/.steelengine/first.env "%h/.steelengine/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.steelengine/first.env") {
        return "STEELENGINE_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.steelengine/second env.env") {
        return 'STEELENGINE_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      ...
      STEELENGINE_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "STEELENGINE_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "STEELENGINE_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "STEELENGINE_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      STEELENGINE_GATEWAY_TOKEN: "override-token",
      STEELENGINE_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/steelengine-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/steelengine gateway run",
          "EnvironmentFile=%h/.steelengine/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.steelengine/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'STEELENGINE_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          'STEELENGINE_GATEWAY_PASSWORD="symbol \\" \\\\ \\$ \\`"', // pragma: allowlist secret
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      STEELENGINE_GATEWAY_TOKEN: "quoted token",
      STEELENGINE_GATEWAY_PASSWORD: 'symbol " \\ $ `', // pragma: allowlist secret
...
describe("stageSystemdService", () => {
  async function withStageFixture(
    ...
    const tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-systemd-stage-"));
    const home = path.join(tempHomeRoot, "home");
    const stateDir = path.join(home, ".steelengine");
    const env = {
      HOME: home,
      STEELENGINE_STATE_DIR: stateDir,
      STEELENGINE_SYSTEMD_UNIT: "steelengine-gateway-stage-test",
    };
  ...
  it("writes dotenv-backed values to a separate env file and keeps inline env minimal", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      await fs.writeFile(
        ...
        ["STEELENGINE_GATEWAY_TOKEN=dotenv-token", "LLM_API_KEY=dotenv-key"].join("\n"),
        "utf8",
      );
      ...
      await stageSystemdService({
        ...
        environment: {
          STEELENGINE_GATEWAY_TOKEN: "dotenv-token",
      ...
      expect(unit).not.toContain("Environment=STEELENGINE_GATEWAY_TOKEN=dotenv-token");
      expect(unit).not.toContain("Environment=LLM_API_KEY=dotenv-key");
      expect(envFile).toBe("STEELENGINE_GATEWAY_TOKEN=dotenv-token\nLLM_API_KEY=dotenv-key\n");
  ...
  it("writes node file-backed managed values to the node env file instead of the unit", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath, nodeEnvFilePath }) => {
      ...
      await stageSystemdService({
        env,
        stdout: { write: vi.fn() } as unknown as NodeJS.WritableStream,
        programArguments: ["/usr/bin/steelengine", "node", "run"],
        workingDirectory: "/tmp",
        environment: {
          STEELENGINE_GATEWAY_TOKEN: "file-backed-token",
          STEELENGINE_GATEWAY_PASSWORD: gatewayPassword,
          STEELENGINE_GATEWAY_PORT: "18789",
          STEELENGINE_SERVICE_MANAGED_ENV_KEYS: "STEELENGINE_GATEWAY_PASSWORD,STEELENGINE_GATEWAY_TOKEN", // pragma: allowlist secret
      ...
      expect(unit).toContain("Environment=STEELENGINE_GATEWAY_PORT=18789");
      expect(unit).not.toContain("Environment=STEELENGINE_GATEWAY_TOKEN=file-backed-token");
      expect(unit).not.toContain("Environment=STEELENGINE_GATEWAY_PASSWORD=");
      expect(envFile).toBe(
        'STEELENGINE_GATEWAY_TOKEN=file-backed-token\nSTEELENGINE_GATEWAY_PASSWORD="symbol \\" \\\\ \\$ \\`"\n',
      );
      expect(envFileStat.mode & 0o777).toBe(0o600);
  ...
  it("migrates operator entries from the legacy gateway env file when writing node env files", async () => {
    await withStageFixture(async ({ env, unitPath, envFilePath, nodeEnvFilePath }) => {
      const legacyGatewayEnvFile =
        ["STEELENGINE_GATEWAY_TOKEN=legacy-node-token", "OPENROUTER_API_KEY=operator-key"].join("\n") +
        "\n";
      ...
      await stageSystemdService({
        ...
        workingDirectory: "/tmp",
        environment: {
          STEELENGINE_GATEWAY_TOKEN: "fresh-file-token",
          STEELENGINE_GATEWAY_PORT: "18789",
          STEELENGINE_SERVICE_KIND: "node",
        },
        environmentValueSources: {
      ...
      expect(unit).toContain(`EnvironmentFile=-${nodeEnvFilePath}`);
      expect(unit).not.toContain("STEELENGINE_GATEWAY_TOKEN=fresh-file-token");
  ...
  it("clears stale node file-backed managed keys without touching the gateway env file", async () => {
    await withStageFixture(async ({ env, unitPath, envFilePath, nodeEnvFilePath }) => {
      await fs.writeFile(envFilePath, "STEELENGINE_GATEWAY_TOKEN=stale-token\n", {
        encoding: "utf8",
        mode: 0o600,
      });
      await fs.writeFile(nodeEnvFilePath, "STEELENGINE_GATEWAY_TOKEN=stale-node-token\n", {
        encoding: "utf8",
        mode: 0o600,
      });
      ...
      await expect(fs.readFile(envFilePath, "utf8")).resolves.toBe(
        "STEELENGINE_GATEWAY_TOKEN=stale-token\n",
      );
    });
  });
  ...
  it("sanitizes file-backed managed values out of the backup unit on re-stage", async () => {
    await withStageFixture(async ({ env, unitPath }) => {
      ...
      await fs.writeFile(
        ...
        [
          "[Service]",
          "ExecStart=/usr/bin/steelengine node run",
          "Environment=FOO=bar STEELENGINE_GATEWAY_TOKEN=inline-token BAZ=qux",
          "Environment=STEELENGINE_GATEWAY_TOKEN=token-only-line",
          "Environment='STEELENGINE_GATEWAY_TOKEN=single-quoted-token' FROM_SINGLE=kept",
      ...
      expect(unit).not.toContain("Environment=STEELENGINE_GATEWAY_TOKEN=fresh-token");
      expect(backupUnit).not.toContain("Environment=STEELENGINE_GATEWAY_TOKEN=inline-token");
      expect(backupUnit).not.toContain("Environment=STEELENGINE_GATEWAY_TOKEN=token-only-line");
      ...
      expect(backupUnit).toContain("Environment=STEELENGINE_GATEWAY_PORT=18789");
  ...
  it("keeps inline overrides out of the generated env file", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      await fs.writeFile(
        path.join(stateDir, ".env"),
        ["STEELENGINE_GATEWAY_TOKEN=stale-token", "LLM_API_KEY=dotenv-key"].join("\n"),
        "utf8",
      );
      ...
      expect(unit).toContain(`EnvironmentFile=-${envFilePath}`);
      expect(unit).toContain("Environment=STEELENGINE_GATEWAY_TOKEN=fresh-token");
  ...
  it("clears stale inline-managed keys from env file on re-stage (#76860)", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      // Existing env file carries a stale STEELENGINE_GATEWAY_TOKEN that the
      // operator previously wrote there but staging now supplies inline.
      await fs.writeFile(
        envFilePath,
        ["STEELENGINE_GATEWAY_TOKEN=stale-gateway-token", "OPENROUTER_API_KEY=or-operator-key"].join(
          "\n",
        ) + "\n",
        { encoding: "utf8", mode: 0o600 },
      );
      ...
      await stageSystemdService({
        ...
        programArguments: ["/usr/bin/steelengine", "gateway", "run"],
        workingDirectory: "/tmp",
        // Staging manages STEELENGINE_GATEWAY_TOKEN inline; STEELENGINE_SERVICE_MANAGED_ENV_KEYS
        // marks it as an SteelEngine-managed key so the stale env-file copy is cleared.
        environment: {
          STEELENGINE_GATEWAY_TOKEN: "fresh-gateway-token",
          LLM_API_KEY: "dotenv-key",
          OPENROUTER_API_KEY: "or-operator-key",
          STEELENGINE_SERVICE_MANAGED_ENV_KEYS: "STEELENGINE_GATEWAY_TOKEN",
        },
        environmentValueSources: {
      ...
      expect(envFile).not.toContain("STEELENGINE_GATEWAY_TOKEN");
      ...
      expect(unit).toContain("Environment=STEELENGINE_GATEWAY_TOKEN=fresh-gateway-token");
...
describe("systemd service install and uninstall", () => {
  async function withNodeSystemdFixture(
    ...
    const tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-node-systemd-"));
    const home = path.join(tempHomeRoot, "home");
    const stateDir = path.join(home, ".steelengine");
    const env = {
      HOME: home,
      STEELENGINE_STATE_DIR: stateDir,
      STEELENGINE_SYSTEMD_UNIT: "steelengine-node",
      STEELENGINE_SERVICE_KIND: "node",
    };
    const unitPath = resolveSystemdUserUnitPath(env);
    const nodeEnvFilePath = path.join(stateDir, "node.systemd.env");
  ...
  it("activates the STEELENGINE_SYSTEMD_UNIT override during install", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath }) => {
      ...
      const unit = await fs.readFile(unitPath, "utf8");
      expect(unitPath).toMatch(/steelengine-node\.service$/);
      expect(unit).toContain("steelengine node run");
      ...
      execFileMock
        ...
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          ...
          cb(
            ...
            "",
            "Unit file steelengine-node.service does not exist.",
          );
        })
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertUserSystemctlArgs(args, "daemon-reload");
  ...
  it("uses the sudo-u target user for install activation machine-scope retry", async () => {
    await withNodeSystemdFixture(async ({ env }) => {
      mockEffectiveUid(1000);
      const installEnv = { ...env, USER: "steelengine", SUDO_USER: "admin" };
      execFileMock
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertUserSystemctlArgs(args, "status");
          cb(null, "", "");
        ...
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertMachineUserSystemctlArgs(args, "steelengine", "enable", NODE_SERVICE);
          cb(null, "", "");
        })
        .mockImplementationOnce((_cmd, args, _opts, cb) => {
          assertUserSystemctlArgs(args, "restart", NODE_SERVICE);
  ...
  it("disables the STEELENGINE_SYSTEMD_UNIT override during uninstall", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath, nodeEnvFilePath }) => {
      await fs.mkdir(path.dirname(unitPath), { recursive: true });
      await fs.writeFile(unitPath, "[Unit]\nDescription=SteelEngine Node\n", "utf8");
      await fs.writeFile(
        nodeEnvFilePath,
        [
          "STEELENGINE_GATEWAY_TOKEN=stale-node-token",
          "STEELENGINE_GATEWAY_PASSWORD=stale-password",
          "OPENROUTER_API_KEY=operator-key",
          "LLM_API_KEY=$SECRET_FROM_SHELL",
  ...
  it("removes a password-only node environment file during uninstall", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath, nodeEnvFilePath }) => {
      await fs.mkdir(path.dirname(unitPath), { recursive: true });
      await fs.writeFile(unitPath, "[Unit]\nDescription=SteelEngine Node\n", "utf8");
      await fs.writeFile(nodeEnvFilePath, "STEELENGINE_GATEWAY_PASSWORD=stale-password\n", {
        encoding: "utf8",
        mode: 0o600,
      });
  ...
  it("preserves node env file values when unit removal fails during uninstall", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath, nodeEnvFilePath }) => {
      ...
      await expect(fs.readFile(unitPath, "utf8")).resolves.toContain("SteelEngine Node");
      await expect(fs.readFile(nodeEnvFilePath, "utf8")).resolves.toBe(
        "STEELENGINE_GATEWAY_TOKEN=stale-node-token\nOPENROUTER_API_KEY=operator-key\n",
      );
      expect(execFileMock).toHaveBeenCalledTimes(2);
...
describe("systemd service control", () => {
  ...
  it("runs reset-failed before restarting a profile-specific user unit", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "reset-failed", "steelengine-gateway-work.service");
        // args[0] is the "--user" scope flag; the systemctl verb is args[1].
        restartSequence.push(args[1] ?? "");
        cb(null, "", "");
      })
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "steelengine-gateway-work.service");
        restartSequence.push(args[1] ?? "");
        cb(null, "", "");
      });
    await assertRestartSuccess({ STEELENGINE_PROFILE: "work" });
