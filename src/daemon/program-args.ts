...
export const RECALL_WRAPPER_ENV_KEY = "RECALL_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/recall -> .pnpm/recall@X.Y.Z/...
...
export async function resolveRecallWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${RECALL_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveRecallWrapperPath(params.wrapperPath);
  if (wrapperPath) {
