...
export const STEELENGINE_WRAPPER_ENV_KEY = "STEELENGINE_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/steelengine -> .pnpm/steelengine@X.Y.Z/...
...
function appendNodeModulesBinCandidates(
  ...
  // steelengine from node_modules/.bin points at the package root sibling.
...
export async function resolveSteelEngineWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${STEELENGINE_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveSteelEngineWrapperPath(params.wrapperPath);
  if (wrapperPath) {
