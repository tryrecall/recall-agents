...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds RECALL_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `recall gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service RECALL_GATEWAY_TOKEN does not match gateway.auth.token in recall.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.RECALL_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.RECALL_GATEWAY_TOKEN);
}
