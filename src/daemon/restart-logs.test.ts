...
describe("restart log conventions", () => {
  it("resolves profile-aware gateway logs and restart attempts together", () => {
    ...
    expect(resolveGatewayLogPaths(env)).toEqual({
      logDir: "/Users/test/.steelengine-work/logs",
      stdoutPath: "/Users/test/.steelengine-work/logs/gateway.log",
      stderrPath: "/Users/test/.steelengine-work/logs/gateway.err.log",
    });
    expect(resolveGatewayRestartLogPath(env)).toBe(
      "/Users/test/.steelengine-work/logs/gateway-restart.log",
    );
  });

  it("honors STEELENGINE_STATE_DIR for restart attempts", () => {
    const env = {
      HOME: "/Users/test",
      STEELENGINE_STATE_DIR: "/tmp/steelengine-state",
    };

    expect(resolveGatewayRestartLogPath(env)).toBe("/tmp/steelengine-state/logs/gateway-restart.log");
  });

  it("keeps macOS LaunchAgent stdout outside the state directory", () => {
    const env = {
      HOME: "/Users/test",
      STEELENGINE_STATE_DIR: "/Volumes/External/steelengine",
    };
    ...
    expect(resolveGatewayRestartLogPath(env)).toBe(
      "/Volumes/External/steelengine/logs/gateway-restart.log",
    );
  });
  ...
  it("keeps macOS LaunchAgent logs profile-aware in the shared user log directory", () => {
    ...
    expect(resolveGatewaySupervisorLogPaths(env, { platform: "darwin" })).toEqual({
      logDir: "/Users/test/Library/Logs/steelengine",
      stdoutPath: "/Users/test/Library/Logs/steelengine/gateway-work.log",
      stderrPath: "/Users/test/Library/Logs/steelengine/gateway-work.err.log",
    });
  });
  ...
  it("renders best-effort POSIX log setup with escaped paths", () => {
    ...
    expect(setup).toContain(
      "if mkdir -p '/Users/test'\\''s/.steelengine/logs' 2>/dev/null && : >>'/Users/test'\\''s/.steelengine/logs/gateway-restart.log' 2>/dev/null; then",
    );
    expect(setup).toContain("exec >>'/Users/test'\\''s/.steelengine/logs/gateway-restart.log' 2>&1");
  });
  ...
  it("renders CMD log setup with quoted paths", () => {
    ...
    expect(setup.quotedLogPath).toBe('"C:\\Users\\Test User/.steelengine/logs/gateway-restart.log"');
    expect(setup.lines).toContain(
      'if not exist "C:\\Users\\Test User/.steelengine/logs" mkdir "C:\\Users\\Test User/.steelengine/logs" >nul 2>&1',
    );
  });

  it("appends a profile-aware lifecycle audit line with stable key-value fields", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-lifecycle-audit-"));
    ...
    expect(line).toMatch(/^\[[^\]]+\] steelengine gateway lifecycle /);
  ...
  it("does not throw when lifecycle audit logging fails", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-lifecycle-audit-fail-"));
    tempDirs.push(stateDir);
    ...
    expect(() =>
      appendGatewayLifecycleAuditLog(
        { STEELENGINE_STATE_DIR: path.join(blocker, "state") },
        {
          action: "stop",
          source: "cli",
          mode: "bootout",
