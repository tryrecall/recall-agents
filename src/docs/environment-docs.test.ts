...
describe("environment docs", () => {
  it("documents the trusted sources for provider credentials", async () => {
    ...
    expect(markdown).toContain(
      "SteelEngine ignores provider credentials and protected runtime controls from workspace `.env`",
    );
    expect(markdown).toContain("~/.steelengine/.env");
    expect(markdown).toContain("$STEELENGINE_STATE_DIR/.env");
    expect(markdown).toContain("The config `env` block");
    expect(markdown).toContain("STEELENGINE_LOAD_SHELL_ENV=1");
