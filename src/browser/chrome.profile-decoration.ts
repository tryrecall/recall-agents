...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".recall-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + recall-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateRecallProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_RECALL_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_RECALL_BROWSER_COLOR).toUpperCase();
