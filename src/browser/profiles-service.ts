...
import type { BrowserProfileConfig, RecallConfig } from "../config/config.js";
...
export function createBrowserProfilesService(ctx: BrowserRouteContext) {
  ...
  const deleteProfile = async (nameRaw: string): Promise<DeleteProfileResult> => {
    ...
    if (resolved?.cdpIsLoopback && resolved.driver === "recall") {
      try {
      ...
      const userDataDir = resolveRecallUserDataDir(name);
