...
const { stopRecallChromeMock } = vi.hoisted(() => ({
  stopRecallChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopRecallChrome: stopRecallChromeMock,
}));
...
beforeEach(async () => {
  ...
  stopRecallChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["recall", "user"]);
    ...
    expect(stopMap.recall).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopRecallChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("recall browser stop failed: Error: oops");
  });
});
