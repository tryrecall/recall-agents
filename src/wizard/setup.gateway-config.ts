...
import type { GatewayBindMode, GatewayTailscaleMode, SteelEngineConfig } from "../config/config.js";
...
export async function configureGatewayForSetup(
  ...
  if (authMode === "token") {
    ...
    if (tokenMode === "ref") {
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.STEELENGINE_GATEWAY_TOKEN)) ||
        randomToken();
