...
export async function finalizeSetupWizard(
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.recall.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.recall/recall.json (gateway.auth.token) or RECALL_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("recall config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("recall doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("recall dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control Recall."
            : "Copy/paste this URL in a browser on this machine to control Recall.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("recall dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.recall.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.recall.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("recall configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.recall.ai/tools/web"}`,
          "Docs: https://docs.recall.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("recall configure --section web")}`,
          "",
          "Docs: https://docs.recall.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://recall.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control Recall."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control Recall.",
  );
