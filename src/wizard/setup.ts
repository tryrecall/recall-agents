...
async function writeWizardConfigFile(config: RecallConfig): Promise<RecallConfig> {
...
export async function runSetupWizard(
  ...
  let baseConfig: RecallConfig = snapshot.valid
    ? snapshot.exists
  ...
  const quickstartHint = t("wizard.setup.flowQuickstartHint", {
    command: formatCliCommand("recall configure"),
  });
  ...
  if (
    ...
    runtime.error(
      "Invalid --flow. Use quickstart, manual, advanced, or import. Example: recall onboard --flow quickstart",
    );
    runtime.exit(1);
    return;
  }
