...
const resolveExistingKey = vi.hoisted(() =>
  vi.fn<(config: RecallConfig, provider: string) => string | undefined>(() => undefined),
);
const hasExistingKey = vi.hoisted(() =>
  vi.fn<(config: RecallConfig, provider: string) => boolean>(() => false),
);
...
const listConfiguredWebSearchProviders = vi.hoisted(() =>
  vi.fn<(params?: { config?: RecallConfig }) => PluginWebSearchProviderEntry[]>(() => []),
);
...
function createEnabledFirecrawlSearchConfig(): RecallConfig {
  return {
    tools: {
      web: {
        search: {
...
describe("finalizeSetupWizard", () => {
  ...
  it("resolves gateway password SecretRef for probe but omits auth from TUI hatch", async () => {
    ...
    process.env.RECALL_GATEWAY_PASSWORD = "resolved-gateway-password"; // pragma: allowlist secret
  ...
  it("uses the resolved setup password for health checks", async () => {
    vi.stubEnv("RECALL_GATEWAY_PASSWORD", "env-password");
