        ...
        plugins: {
          ...
          installs: {
            ...
            [entry.pluginId]: {
              ...
              installPath: `/tmp/recall-plugins/${entry.pluginId}`,
            },
          },
        },
      },
...
describe("runSearchSetupFlow", () => {
    ...
    expect(select).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: expect.arrayContaining([
          expect.objectContaining({
            label: "暂时跳过",
            hint: "稍后可用 recall configure --section web 配置",
          }),
        ]),
      }),
    );
  ...
  it("installs an external catalog search provider before enabling it", async () => {
    ...
    expect(installRequest.entry?.install?.npmSpec).toBe("@recall/brave-plugin");
    ...
    expect(next.plugins?.installs?.brave?.spec).toBe("@recall/brave-plugin");
  });
