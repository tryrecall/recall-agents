...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn(() => ({ label: "steelengine-gateway" })),
...
vi.mock("../agents/agent-tools.js", () => ({
  createSteelEngineCodingTools: mocks.createSteelEngineCodingTools,
}));
...
describe("doctor runtime tool schema checks", () => {
  beforeEach(() => {
    mocks.createSteelEngineCodingTools.mockReset().mockReturnValue([]);
    ...
    mocks.readGatewayServiceState.mockReset().mockResolvedValue({
      ...
      running: true,
      env: {},
      command: { programArguments: ["steelengine", "gateway"], sourcePath: "/tmp/gateway.service" },
  ...
  it("preserves direct OpenAI catalog transport while building doctor runtime models", async () => {
    ...
    mocks.createSteelEngineCodingTools.mockReturnValueOnce([
  ...
  it("reports unsupported schemas exposed only to a non-default configured agent", async () => {
    mocks.createSteelEngineCodingTools.mockImplementation((options) =>
    ...
    expect(mocks.createSteelEngineCodingTools).toHaveBeenCalledWith(
  ...
  it("skips ACP-only agents because they do not use embedded tool projection", async () => {
    ...
    expect(mocks.createSteelEngineCodingTools).toHaveBeenCalledTimes(1);
    expect(mocks.createSteelEngineCodingTools).toHaveBeenCalledWith(
  ...
  it("loads bundled MCP runtime once per distinct agent workspace", async () => {
    mocks.createSteelEngineCodingTools.mockReturnValue([]);
    mocks.createBundleMcpToolRuntime.mockImplementation(
      async (options: { workspaceDir: string }) => ({
...
describe("doctor gateway runtime checks", () => {
  beforeEach(() => {
    ...
    mocks.resolveGatewayService.mockReset().mockReturnValue({ label: "steelengine-gateway" });
  });
  ...
  it("reports unreachable gateway health probes", async () => {
    ...
    await expect(
      ...
      fixHint:
        "Start the Gateway service or run `steelengine doctor --fix` for service repair prompts.",
    });
  });
  ...
  it("reports missing local gateway daemon service", async () => {
    ...
    await expect(
      ...
      path: "gateway.mode",
      target: "steelengine-gateway",
      fixHint: "Run `steelengine doctor --fix` or `steelengine gateway install` to install it.",
    });
  });
