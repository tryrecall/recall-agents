...
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg?: unknown, _agentId?: unknown) => "/tmp/recall-workspace"),
);
...
describe("setupChannels workspace shadow exclusion", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resolveAgentWorkspaceDir.mockReturnValue("/tmp/recall-workspace");
  ...
  it("preloads configured external plugins from the trusted catalog boundary", async () => {
    ...
    expect(trustedInput.workspaceDir).toBe("/tmp/recall-workspace");
    ...
    expect(registryInput.workspaceDir).toBe("/tmp/recall-workspace");
  });
  ...
  it("loads the selected bundled catalog plugin without writing explicit plugin enablement", async () => {
    ...
    expect(firstRegistryInput.workspaceDir).toBe("/tmp/recall-workspace");
    ...
    expect(secondRegistryInput.workspaceDir).toBe("/tmp/recall-workspace");
  ...
  it(
    ...
    async () => {
      ...
      expect(catalogLookupCall[1]?.workspaceDir).toBe("/tmp/recall-workspace");
      expect(ensureChannelSetupPluginInstalled).toHaveBeenCalledTimes(1);
      expectExternalCatalogInstallCall();
      expect(note).not.toHaveBeenCalledWith("external-chat plugin not available.", "Channel setup");
      expect(configure).toHaveBeenCalledTimes(1);
