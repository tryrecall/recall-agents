...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "/tmp/fake-recall.json",
...
describe("doctor health contributions", () => {
  ...
  it("keeps legacy parent writable release repairs old-parent-readable", async () => {
    ...
    const ctx = {
      ...
      options: {},
      cfgForPersistence: {},
      configPath: "/tmp/fake-recall.json",
      env: {
        RECALL_UPDATE_IN_PROGRESS: "1",
        RECALL_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE: "1",
      },
    } as Parameters<(typeof contribution)["run"]>[0];
  ...
  describe("config size drops during update", () => {
    ...
    it("allows config size drops when RECALL_UPDATE_IN_PROGRESS=1", async () => {
    ...
    it("points update-time config rewrites at the pre-update backup", async () => {
      ...
      expect(ctx.runtime.log).toHaveBeenCalledWith(
        "Update changed config; pre-update backup: /tmp/fake-recall.json.pre-update",
      );
    });
