...
type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<SteelEngineConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<SteelEngineConfig["tools"]>["web"]>["search"]>;
...
const WEB_SEARCH_DOCS_URL = "https://docs.steelengine.ai/tools/web";
...
function defaultModelUsesCodexRuntime(config: SteelEngineConfig): boolean {
...
function rawKeyValue(config: SteelEngineConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: SteelEngineConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: SteelEngineConfig, provider: SearchProvider): SecretRef {
  const entry =
...
export function applySearchKey(
  ...
  const nextBase: SteelEngineConfig = {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: SteelEngineConfig, result: SteelEngineConfig): SteelEngineConfig {
...
export async function runSearchSetupFlow(
  ...
  const finalizeSelection = (nextConfig: SteelEngineConfig) =>
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "SteelEngine will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.steelengine.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
