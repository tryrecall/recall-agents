...
import type { ConfigValidationIssue, SteelEngineConfig } from "../config/types.steelengine.js";
...
export type CoreHealthCheckDeps = {
  readonly detectUnavailableSkills: (cfg: SteelEngineConfig) => Promise<readonly SkillStatusEntry[]>;
  readonly collectSecurityWarnings: (cfg: SteelEngineConfig) => Promise<readonly string[]>;
...
async function collectSecurityWarningsWithRuntime(cfg: SteelEngineConfig): Promise<readonly string[]> {
...
const gatewayConfigCheck: HealthCheck = {
  ...
  kind: "core",
  description: "steelengine.jsonc gateway block is set and unambiguous.",
  source: "doctor",
  async detect(ctx) {
    ...
    if (!ctx.cfg.gateway?.mode) {
      findings.push({
        ...
        path: "gateway.mode",
        fixHint:
          "Run `steelengine configure` and set Gateway mode (local/remote), or `steelengine config set gateway.mode local`.",
      });
    }
    if (ctx.cfg.gateway?.mode !== "remote" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
      findings.push({
        ...
        fixHint:
          "Set an explicit mode: `steelengine config set gateway.auth.mode token` or `... password`.",
      });
    }
    return findings;
  },
    ...
    return [
      {
        ...
        fixHint:
          "Set commands.ownerAllowFrom to your channel user id, e.g. `steelengine config set commands.ownerAllowFrom '[\"telegram:123456789\"]'`.",
      },
    ];
  },
};
...
function resolveDoctorMode(cfg: SteelEngineConfig): "local" | "remote" {
...
export function buildGatewayTokenSecretRefFixHint(ref: SecretRef): string {
  if (ref.source === "exec") {
    return "Run `steelengine doctor --allow-exec` to verify exec SecretRefs during doctor, or `steelengine secrets audit --allow-exec` to audit all exec SecretRefs.";
  }
    ...
    return [
      {
        ...
        path: "gateway.auth",
        fixHint: "Run `steelengine doctor --fix --generate-gateway-token` to generate a token.",
      },
    ];
  },
};
    ...
    return [
      ...detected.preview.map(
        (line): HealthFinding => ({
          ...
          fixHint: "Run `steelengine doctor --fix` to migrate legacy state.",
        }),
      ),
      ...detected.warnings.map(
        (warning): HealthFinding => ({
          ...
          message: warning,
          path: detected.stateDir,
          fixHint: "Resolve the warning, then rerun `steelengine doctor --fix`.",
        }),
      ),
    ];
  },
      ...
      (issue): HealthFinding => ({
        ...
        fixHint: issue.repairBlocked
          ? [
              ...
              "or set the affected OpenAI models to an SteelEngine runtime policy.",
            ].join(" ")
          : [
              "Run `steelengine doctor --fix`: it enables plugins.entries.codex,",
              "or set the affected OpenAI models to an SteelEngine runtime policy.",
            ].join(" "),
      }),
    );
...
function unavailableSkillToFinding(skill: SkillStatusEntry): HealthFinding {
  return {
    ...
    fixHint:
      "Install/configure the missing requirement, or run `steelengine doctor --fix` to disable unused unavailable skills.",
  };
}
...
function browserResidueFinding(residue: LegacyClawdBrowserProfileResidue): HealthFinding {
  return {
    ...
    fixHint:
      "Run `steelengine doctor --fix` to archive the stale clawd profile safely instead of deleting it in place.",
  };
}
...
const browserClawdProfileResidueCheck: HealthCheck = {
  ...
  kind: "core",
  description:
    "Legacy clawd managed browser profile residue has been archived after the SteelEngine rename.",
  source: "doctor",
  async detect(ctx, scope) {
...
const finalConfigValidationCheck: HealthCheck = {
  ...
  kind: "core",
  description: "Active steelengine.jsonc parses and conforms to the config schema.",
  source: "doctor",
  async detect() {
