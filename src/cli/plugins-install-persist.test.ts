...
describe("persistPluginInstall", () => {
  ...
  it("adds installed plugins to restrictive allowlists before enabling", async () => {
    ...
    enablePluginInConfig.mockImplementation((...args: unknown[]) => {
      const [cfg, pluginId] = args as [RecallConfig, string];
  ...
  it("removes a replaced managed install directory before refreshing the registry", async () => {
    ...
    setInstalledPluginIndexInstallRecords({
      codex: {
        source: "clawhub",
        spec: "clawhub:@recall/codex",
        installPath: "/tmp/recall/extensions/codex",
      },
    });
    planPluginUninstall.mockReturnValueOnce({
      ...
      directoryRemoval: {
        target: "/tmp/recall/extensions/codex",
      },
    });
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@recall/codex",
        installPath: "/tmp/recall/npm/node_modules/@recall/codex",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "Removed previous plugin install directory: /tmp/recall/extensions/codex",
    );
  });
  ...
  it("warns when an installed npm plugin remains shadowed by a config-selected source", async () => {
    ...
    const next = await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@recall/discord",
        installPath: "/tmp/recall/npm/node_modules/@recall/discord/index.ts",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "active config source: /tmp/recall-upstream/extensions/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain(
      "installed npm source: /tmp/recall/npm/node_modules/@recall/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain("recall plugins doctor");
  });
  ...
  it("does not warn when the config-selected source is inside the npm install path", async () => {
      ...
      plugins: [
        {
          id: "discord",
          origin: "config",
          source: "/tmp/recall/npm/node_modules/@recall/discord/dist/index.js",
          status: "loaded",
        },
      ],
      diagnostics: [],
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@recall/discord",
        installPath: "/tmp/recall/npm/node_modules/@recall/discord",
      },
    });
