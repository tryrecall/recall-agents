...
async function spawnSteelEngine(
  args: string[],
...
describe("steelengine skills install ClawHub GitHub-backed E2E", () => {
  it("installs from the install resolver and reports install telemetry", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-clawhub-cli-e2e-"));
    try {
      const result = await spawnSteelEngine(
        ["skills", "install", "@demo-owner/aiq-deploy", "--global"],
        {
          ...
          env: {
            ...
            STEELENGINE_CLAWHUB_URL: registry,
