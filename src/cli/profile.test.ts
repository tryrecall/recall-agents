...
describe("parseCliProfileArgs", () => {
  ...
  it("parses interleaved --profile after the command token", () => {
    const res = parseCliProfileArgs(["node", "recall", "status", "--profile", "work", "--deep"]);
    if (!res.ok) {
      throw new Error(res.error);
    }
    expect(res.profile).toBe("work");
    expect(res.argv).toEqual(["node", "recall", "status", "--deep"]);
  });
  ...
  it("preserves Matrix QA --profile after leading root options", () => {
    ...
    expect(res.profile).toBeNull();
    expect(res.argv).toEqual(["node", "recall", "--no-color", "qa", "matrix", "--profile=fast"]);
  });
  ...
  it("still parses root --profile before Matrix QA", () => {
    ...
    expect(res.profile).toBe("work");
    expect(res.argv).toEqual(["node", "recall", "qa", "matrix", "--fail-fast"]);
  });

  it("parses interleaved --dev after the command token", () => {
    const res = parseCliProfileArgs(["node", "recall", "status", "--dev"]);
    if (!res.ok) {
      throw new Error(res.error);
    }
    expect(res.profile).toBe("dev");
  ...
  it.each([
    ...
    ["interleaved after command", ["node", "recall", "status", "--profile", "work", "--dev"]],
...
describe("formatCliCommand", () => {
  ...
  it("trims whitespace from profile", () => {
    expect(formatCliCommand("recall doctor --fix", { RECALL_PROFILE: "  jbopenclaw  " })).toBe(
      "recall --profile jbopenclaw doctor --fix",
    );
  });
  ...
  it("inserts --container when a container hint is set", () => {
    expect(
      formatCliCommand("recall gateway status --deep", { RECALL_CONTAINER_HINT: "demo" }),
    ).toBe("recall --container demo gateway status --deep");
  });

  it("ignores unsafe container hints", () => {
    expect(
      formatCliCommand("recall gateway status --deep", {
        RECALL_CONTAINER_HINT: "demo; rm -rf /",
      }),
    ).toBe("recall gateway status --deep");
  });

  it("preserves both --container and --profile hints", () => {
    expect(
      formatCliCommand("recall doctor", {
        RECALL_CONTAINER_HINT: "demo",
        RECALL_PROFILE: "work",
      }),
    ).toBe("recall --container demo doctor");
  });

  it("does not prepend --container for update commands", () => {
    expect(formatCliCommand("recall update", { RECALL_CONTAINER_HINT: "demo" })).toBe(
      "recall update",
    );
    expect(
      formatCliCommand("pnpm recall update --channel beta", { RECALL_CONTAINER_HINT: "demo" }),
    ).toBe("pnpm recall update --channel beta");
  });
});
