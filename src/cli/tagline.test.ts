...
const EXPECTED_DEFAULT_TAGLINE = "All your chats, one SteelEngine.";
...
describe("pickTagline", () => {
  ...
  it("keeps STEELENGINE_TAGLINE_INDEX behavior in random mode", () => {
    const value = pickTagline({
      mode: "random",
      env: { STEELENGINE_TAGLINE_INDEX: "0" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(
  ...
  it("ignores partial STEELENGINE_TAGLINE_INDEX values", () => {
    expect(
      pickTagline({
        mode: "random",
        env: { STEELENGINE_TAGLINE_INDEX: "1abc" } as NodeJS.ProcessEnv,
        random: () => 0,
      }),
