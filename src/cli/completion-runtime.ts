...
export const COMPLETION_SKIP_PLUGIN_COMMANDS_ENV = "STEELENGINE_COMPLETION_SKIP_PLUGIN_COMMANDS";
...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# SteelEngine Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# SteelEngine Completion\n${sourceLine}`;
  const next = trimmed ? `${trimmed}\n\n${block}\n` : `${block}\n`;
  return { next, changed: next !== content, hadExisting };
}

/** Resolves the shell startup profile path that should contain the SteelEngine completion block. */
...
/** Returns whether a shell profile already contains an SteelEngine completion block or source line. */
 ...
 * Returns true if profile has `source <(steelengine completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "steelengine") {
