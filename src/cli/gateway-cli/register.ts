      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["recall gateway run", "Run the gateway in the foreground."],
            ["recall gateway status", "Show service status plus connectivity/capability."],
            ["recall gateway discover", "Find local and wide-area gateway beacons."],
            ["recall gateway stability", "Show recent stability diagnostics."],
            ["recall gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.recall.ai/cli/gateway")}\n`,
      ),
  );
