...
>(() => ({ mode: "disabled", detail: "RECALL_NO_RESPAWN" }));
...
describe("runGatewayLoop", () => {
  ...
  it("releases the lock before exiting on spawned restart", async () => {
    vi.clearAllMocks();
    peekGatewaySigusr1RestartReason.mockReturnValue(undefined);
    const originalTraceEnv = process.env.RECALL_GATEWAY_RESTART_TRACE;
    process.env.RECALL_GATEWAY_RESTART_TRACE = "1";
    ...
    try {
      await withIsolatedSignals(async ({ captureSignal }) => {
        ...
        expect(respawnOpts?.env?.RECALL_GATEWAY_RESTART_TRACE_STARTED_AT_MS).toMatch(/^\d/u);
        expect(respawnOpts?.env?.RECALL_GATEWAY_RESTART_TRACE_LAST_AT_MS).toMatch(/^\d/u);
        expect(writeGatewayRestartHandoffSync).not.toHaveBeenCalled();
      });
    } finally {
      if (originalTraceEnv === undefined) {
        delete process.env.RECALL_GATEWAY_RESTART_TRACE;
      } else {
        process.env.RECALL_GATEWAY_RESTART_TRACE = originalTraceEnv;
      }
    }
  });
