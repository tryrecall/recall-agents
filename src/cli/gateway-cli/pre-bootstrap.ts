...
function applyInvocationDestructiveOverride(value: string | undefined): void {
  if (process.env.STEELENGINE_SERVICE_MARKER?.trim() || value === undefined) {
    delete process.env[ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV];
  } else {
...
async function guardGatewayRunSelectedConfig(
  ...
  for (;;) {
    ...
    if (!snapshot.valid) {
      ...
      // off-root $include self-authorize STEELENGINE_INCLUDE_ROOTS on the next read. Only explicit dev
