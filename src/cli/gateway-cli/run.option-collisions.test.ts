...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/recall-stability.json",
  path: "/tmp/recall-stability.json",
}));
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/recall-control-ui" as string | null,
}));
...
vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/recall-test-missing-config.json",
...
vi.mock("../../config/paths.js", () => ({
  CONFIG_PATH: "/tmp/recall-test-missing-config.json",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/recall-control-ui";
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    const previousMarker = process.env.RECALL_SERVICE_MARKER;
    process.env.RECALL_SERVICE_MARKER = "gateway";
    try {
      ...
      if (previousMarker === undefined) {
        ...
        process.env.RECALL_SERVICE_MARKER = previousMarker;
      }
    }
  ...
  it.each([
    ...
    delete process.env.RECALL_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.RECALL_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `recall onboard --mode local` or `recall setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("blocks invalid startup config without automatic recovery", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/recall-test-missing-config.json",
      config: {},
      parsed: null,
