...
function createGatewayCliStartupTrace() {
  const enabled = isTruthyEnvValue(process.env.STEELENGINE_GATEWAY_STARTUP_TRACE);
...
function getGatewayStartGuardErrors(params: {
  ...
  if (!params.configExists) {
    return [
      `Missing config. Run \`${formatCliCommand("steelengine setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
    ];
  }
  if (params.mode === undefined) {
    return [
      [
        ...
        `Re-run \`${formatCliCommand("steelengine onboard --mode local")}\` or \`${formatCliCommand("steelengine setup")}\`, set gateway.mode=local manually, or pass --allow-unconfigured.`,
      ].join(" "),
...
async function readGatewayStartupConfig(params: {
  ...
  let blockedRecoveryConfig: SteelEngineConfig | null = null;
...
async function runGatewayCommandOnce(opts: GatewayRunOpts, hooks: GatewayRunRuntimeHooks = {}) {
  ...
  const isDevProfile = normalizeOptionalLowercaseString(process.env.STEELENGINE_PROFILE) === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.STEELENGINE_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.STEELENGINE_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.STEELENGINE_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (snapshot) {
    ...
    const finalConfigEnteredServiceMode = Boolean(process.env.STEELENGINE_SERVICE_MARKER?.trim());
  ...
  if (process.env.STEELENGINE_SERVICE_MARKER?.trim()) {
  ...
  if (opts.force) {
    ...
    try {
      ...
      defaultRuntime.error(
        `Could not free port ${port}: ${formatErrorMessage(err)}. Run ${formatCliCommand("steelengine gateway status --deep")} to inspect the listener.`,
      );
      defaultRuntime.exit(1);
      return;
    }
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or STEELENGINE_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
      ...
      [
        ...
        ...(isContainerEnvironment()
          ? [
              ...
              "Set STEELENGINE_GATEWAY_TOKEN or STEELENGINE_GATEWAY_PASSWORD, or pass --token/--password to start with auth.",
            ]
          : [
              "Set gateway.auth.token/password (or STEELENGINE_GATEWAY_TOKEN/STEELENGINE_GATEWAY_PASSWORD) or pass --token/--password.",
            ]),
        ...authHints,
      ]
        .filter(Boolean)
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("steelengine gateway stop")}`,
      );
      try {
      ...
      // the SteelEngine liveness contract. Arbitrary HTTP listeners must fail.
      const gatewayTls =
    ...
    defaultRuntime.error(
      `Gateway failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("steelengine gateway status --deep")} for diagnostics.`,
    );
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("steelengine.gatewayRunTestApi")] = testing;
}
