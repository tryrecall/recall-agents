...
function createGatewayCliStartupTrace() {
  const enabled = isTruthyEnvValue(process.env.RECALL_GATEWAY_STARTUP_TRACE);
...
function warnInlinePasswordFlag() {
  defaultRuntime.error(
    "Warning: --password can be exposed via process listings. Prefer --password-file or RECALL_GATEWAY_PASSWORD.",
  );
}
...
async function maybeLogPendingControlUiBuild(cfg: RecallConfig): Promise<void> {
...
function getGatewayStartGuardErrors(params: {
  ...
  if (!params.configExists) {
    return [
      `Missing config. Run \`${formatCliCommand("recall setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
    ];
  }
  if (params.mode === undefined) {
    return [
      [
        ...
        `Re-run \`${formatCliCommand("recall onboard --mode local")}\` or \`${formatCliCommand("recall setup")}\`, set gateway.mode=local manually, or pass --allow-unconfigured.`,
      ].join(" "),
...
export async function runGatewayCommand(opts: GatewayRunOpts) {
  ...
  if (process.env.RECALL_SERVICE_MARKER?.trim()) {
    process.env[GATEWAY_SERVICE_RUNTIME_PID_ENV] = String(process.pid);
  }
  const isDevProfile = normalizeOptionalLowercaseString(process.env.RECALL_PROFILE) === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.RECALL_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.RECALL_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.RECALL_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (futureStartupBlock && process.env.RECALL_SERVICE_MARKER?.trim()) {
  ...
  if (opts.force) {
    try {
      ...
      defaultRuntime.error(
        `Could not free port ${port}: ${formatErrorMessage(err)}. Run ${formatCliCommand("recall gateway status --deep")} to inspect the listener.`,
      );
      defaultRuntime.exit(1);
      return;
    }
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or RECALL_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
      ...
      [
        ...
        ...(isContainerEnvironment()
          ? [
              ...
              "Set RECALL_GATEWAY_TOKEN or RECALL_GATEWAY_PASSWORD, or pass --token/--password to start with auth.",
            ]
          : [
              "Set gateway.auth.token/password (or RECALL_GATEWAY_TOKEN/RECALL_GATEWAY_PASSWORD) or pass --token/--password.",
            ]),
        ...authHints,
      ]
        .filter(Boolean)
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("recall gateway stop")}`,
      );
      try {
    ...
    defaultRuntime.error(
      `Gateway failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("recall gateway status --deep")} for diagnostics.`,
    );
    defaultRuntime.exit(1);
  }
}
