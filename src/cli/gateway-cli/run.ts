...
async function runGatewayCommand(opts: GatewayRunOpts) {
  const isDevProfile = process.env.RECALL_PROFILE?.trim().toLowerCase() === "dev";
  ...
  if (opts.claudeCliLogs) {
    ...
    process.env.RECALL_CLAUDE_CLI_LOG_OUTPUT = "1";
  }
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: RECALL_GATEWAY_TOKEN env if set)",
    )
