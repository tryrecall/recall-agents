...
const resolveDevWorkspaceDir = (env: NodeJS.ProcessEnv = process.env): string => {
  ...
  const profile = normalizeOptionalLowercaseString(env.STEELENGINE_PROFILE);
  if (profile === "dev") {
    return baseDir;
  }
...
async function ensureDevWorkspace(dir: string) {
  ...
  const [agents, soul, tools, identity, user] = await Promise.all([
    loadDevTemplate(
      "AGENTS.dev.md",
      `# AGENTS.md - SteelEngine Dev Workspace\n\nDefault dev workspace for steelengine gateway --dev.\n`,
    ),
    loadDevTemplate(
      "SOUL.dev.md",
