...
describe("plugins cli list", () => {
  ...
  it("reports stale plugin config in doctor output without claiming full plugin health", async () => {
    ...
    expect(output).toContain(
      'Run "recall doctor --fix" to remove stale plugin ids and dangling channel references.',
    );
    expect(output).toContain(
  ...
  it("reports missing configured Codex runtime plugin in doctor output", async () => {
    ...
    expect(output).toContain("recall doctor --fix");
    expect(output).toContain("recall plugins install @recall/codex");
    expect(output).toContain(
      "No plugin install-tree issues detected; configuration warnings remain.",
    );
    expect(output).not.toContain("No plugin issues detected.");
  ...
  it("reports missing configured ACPX runtime plugin in doctor output", async () => {
    ...
    expect(output).toContain("recall doctor --fix");
    expect(output).toContain("recall plugins install @recall/acpx");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports blocked configured ACPX runtime with ACP-specific guidance", async () => {
    ...
    expect(output).not.toContain("recall plugins install @recall/acpx");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports configured Codex runtime when the plugin record is disabled", async () => {
    ...
    expect(output).not.toContain("recall plugins install @recall/codex");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports blocked configured Codex runtime without install advice", async () => {
    ...
    expect(output).not.toContain('Run "recall doctor --fix" to install');
    expect(output).not.toContain("recall plugins install @recall/codex");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports config-selected plugin source shadowing in doctor output", async () => {
    buildPluginDiagnosticsReport.mockReturnValue({
      plugins: [
        createPluginRecord({
          id: "discord",
          origin: "config",
          source: "/tmp/recall-upstream/extensions/discord/index.ts",
          status: "error",
      ...
      diagnostics: [
        {
          level: "warn",
          pluginId: "discord",
          source: "/tmp/recall/npm/node_modules/@recall/discord/index.ts",
          message:
            "duplicate plugin id resolved by explicit config-selected plugin; global plugin will be overridden by config plugin (/tmp/recall-upstream/extensions/discord/index.ts)",
        },
      ],
    });
    ...
    expect(output).toContain("active: /tmp/recall-upstream/extensions/discord/index.ts");
    expect(output).toContain("shadowed: /tmp/recall/npm/node_modules/@recall/discord/index.ts");
    expect(output).toContain("recall plugins registry --refresh");
  });
  ...
  it("reports persisted plugin registry state without refreshing", async () => {
    ...
    expect(runtimeLogs.join("\n")).toContain("recall plugins registry --refresh");
  });
  ...
  it("keeps inspect on the static snapshot by default", async () => {
    setInstalledPluginIndexInstallRecords({
      "recall-mem0": {
        source: "clawhub",
        spec: "clawhub:recall-mem0",
        installPath: "/plugins/recall-mem0",
        version: "2026.5.1",
        clawhubPackage: "recall-mem0",
        clawhubChannel: "official",
        artifactKind: "npm-pack",
        artifactFormat: "tgz",
        npmIntegrity: "sha512-clawpack",
        npmShasum: "1".repeat(40),
        npmTarballName: "recall-mem0-2026.5.1.tgz",
    ...
    buildPluginSnapshotReport.mockReturnValue({
      plugins: [createPluginRecord({ id: "recall-mem0", name: "Mem0" })],
      diagnostics: [],
    });
    buildPluginInspectReport.mockReturnValue({
      workspaceDir: "/workspace",
      plugin: createPluginRecord({ id: "recall-mem0", name: "Mem0" }),
      shape: "hook-only",
    ...
    await runPluginsCommand(["plugins", "inspect", "recall-mem0"]);

    expect(buildPluginDiagnosticsReport).not.toHaveBeenCalled();
    expect(runtimeLogs.join("\n")).toContain("Policy");
    expect(runtimeLogs.join("\n")).toContain("allowConversationAccess: true");
    expect(runtimeLogs.join("\n")).toContain("ClawHub package: recall-mem0");
  ...
  it("runtime-inspects without repairing deps", async () => {
    ...
    await runPluginsCommand(["plugins", "inspect", "recall-mem0", "--runtime"]);

    expect(buildPluginDiagnosticsReport).toHaveBeenCalledWith({
      config: {},
      onlyPluginIds: ["recall-mem0"],
    });
  });
