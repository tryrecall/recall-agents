...
describe("plugins cli list", () => {
  ...
  it("reports missing configured Codex runtime plugin in doctor output", async () => {
    ...
    expect(output).toContain("steelengine doctor --fix");
    expect(output).toContain("steelengine plugins install @steelengine/codex");
    expect(output).toContain(
      "No plugin install-tree issues detected; configuration warnings remain.",
    );
    expect(output).not.toContain("No plugin issues detected.");
  ...
  it("reports missing configured ACPX runtime plugin in doctor output", async () => {
    ...
    expect(output).toContain("steelengine doctor --fix");
    expect(output).toContain("steelengine plugins install @steelengine/acpx");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports blocked configured ACPX runtime with ACP-specific guidance", async () => {
    ...
    expect(output).not.toContain('runtime policy to "steelengine"');
    expect(output).not.toContain("steelengine plugins install @steelengine/acpx");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports configured Codex runtime when the plugin record is disabled", async () => {
    ...
    expect(output).not.toContain("steelengine plugins install @steelengine/codex");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports blocked configured Codex runtime without install advice", async () => {
    ...
    expect(output).not.toContain('Run "steelengine doctor --fix" to install');
    expect(output).not.toContain("steelengine plugins install @steelengine/codex");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports config-selected plugin source shadowing in doctor output", async () => {
      ...
      diagnostics: [
        {
          level: "warn",
          pluginId: "discord",
          source: "/tmp/steelengine/npm/node_modules/@steelengine/discord/index.ts",
          message:
            "duplicate plugin id resolved by explicit config-selected plugin; global plugin will be overridden by config plugin (/tmp/steelengine-upstream/extensions/discord/index.ts)",
        },
      ],
    });
    ...
    expect(output).toContain("active: /tmp/steelengine-upstream/extensions/discord/index.ts");
    expect(output).toContain("shadowed: /tmp/steelengine/npm/node_modules/@steelengine/discord/index.ts");
    expect(output).toContain("steelengine plugins registry --refresh");
  });
  ...
  it("reports persisted plugin registry state without refreshing", async () => {
    ...
    expect(runtimeLogs.join("\n")).toContain("steelengine plugins registry --refresh");
  });
  ...
  it("keeps inspect on the static snapshot by default", async () => {
    setInstalledPluginIndexInstallRecords({
      "steelengine-mem0": {
        source: "clawhub",
        spec: "clawhub:steelengine-mem0",
        installPath: "/plugins/steelengine-mem0",
        version: "2026.5.1",
        clawhubPackage: "steelengine-mem0",
        clawhubChannel: "official",
        artifactKind: "npm-pack",
        artifactFormat: "tgz",
        npmIntegrity: "sha512-clawpack",
        npmShasum: "1".repeat(40),
        npmTarballName: "steelengine-mem0-2026.5.1.tgz",
    ...
    buildPluginSnapshotReport.mockReturnValue({
      plugins: [createPluginRecord({ id: "steelengine-mem0", name: "Mem0" })],
      diagnostics: [],
    });
    buildPluginInspectReport.mockReturnValue({
      workspaceDir: "/workspace",
      plugin: createPluginRecord({ id: "steelengine-mem0", name: "Mem0" }),
      shape: "hook-only",
    ...
    await runPluginsCommand(["plugins", "inspect", "steelengine-mem0"]);

    expect(buildPluginDiagnosticsReport).not.toHaveBeenCalled();
    expect(runtimeLogs.join("\n")).toContain("Policy");
    expect(runtimeLogs.join("\n")).toContain("allowConversationAccess: true");
    expect(runtimeLogs.join("\n")).toContain("ClawHub package: steelengine-mem0");
  ...
  it("runtime-inspects without repairing deps", async () => {
    ...
    await runPluginsCommand(["plugins", "inspect", "steelengine-mem0", "--runtime"]);

    expect(buildPluginDiagnosticsReport).toHaveBeenCalledWith({
      config: {},
      onlyPluginIds: ["steelengine-mem0"],
    });
  });
