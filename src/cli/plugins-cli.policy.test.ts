...
describe("plugins cli policy mutations", () => {
  ...
  function requireFirstWrittenConfig(): RecallConfig {
  ...
  function requirePluginEntries(
    config: RecallConfig,
  ): NonNullable<NonNullable<RecallConfig["plugins"]>["entries"]> {
  ...
  it("refreshes the persisted plugin registry after enabling a plugin", async () => {
    const sourceConfig = {} as RecallConfig;
    const enabledConfig = {
      plugins: {
        entries: {
          alpha: { enabled: true },
  ...
  it.each(["enable", "disable"] as const)(
    ...
    async (command) => {
      ...
      expect(runtimeErrors).toContain(
        "Plugin not found: missing-plugin. Run `recall plugins list` to see installed plugins, or `recall plugins search missing-plugin` to look for installable plugins.",
      );
      expect(enablePluginInConfig).not.toHaveBeenCalled();
      expect(writeConfigFile).not.toHaveBeenCalled();
