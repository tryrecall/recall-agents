    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["steelengine nodes status", "List known nodes with live status."],
          ["steelengine nodes pairing pending", "Show pending node pairing requests."],
          ["steelengine nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'steelengine nodes invoke --node <id> --command system.which --params \'{"bins":["uname"]}\'',
            "Invoke a node command directly.",
          ],
          ["steelengine nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.steelengine.ai/cli/nodes")}\n`,
    );
...
function shouldRegisterNodesPluginCommands(nodes: Command, argv: readonly string[]): boolean {
  ...
  if (!requestedSubcommand) {
    // Bare `steelengine nodes` listing should still surface plugin-provided subcommands.
    return true;
  }
