...
describe("shouldRegisterPrimarySubcommand", () => {
  it("skips eager primary registration for help/version invocations", () => {
    expect(shouldRegisterPrimarySubcommand(["node", "recall", "status", "--help"])).toBe(false);
    expect(shouldRegisterPrimarySubcommand(["node", "recall", "-V"])).toBe(false);
    expect(shouldRegisterPrimarySubcommand(["node", "recall", "-v"])).toBe(false);
  });

  it("keeps eager primary registration for regular command runs", () => {
    expect(shouldRegisterPrimarySubcommand(["node", "recall", "status"])).toBe(true);
    expect(shouldRegisterPrimarySubcommand(["node", "recall", "acp", "-v"])).toBe(true);
  });
});
...
describe("shouldSkipPluginCommandRegistration", () => {
  ...
  it("skips plugin registration for builtin command runs", () => {
    expect(
      shouldSkipPluginCommandRegistration({
        argv: ["node", "recall", "sessions", "--json"],
        primary: "sessions",
        hasBuiltinPrimary: true,
      }),
    ).toBe(true);
  ...
  it("keeps plugin registration for non-builtin command runs", () => {
    expect(
      shouldSkipPluginCommandRegistration({
        argv: ["node", "recall", "voicecall", "status"],
        primary: "voicecall",
        hasBuiltinPrimary: false,
      }),
    ).toBe(false);
