...
describe("completion-runtime", () => {
  ...
  it("installs PowerShell completion into the concrete profile path", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-completion-state-bob's-"));

    try {
      await withEnvAsync({ HOME: homeDir, STEELENGINE_STATE_DIR: stateDir }, async () => {
        const cachePath = resolveCompletionCachePath("powershell", "steelengine");
        await fs.mkdir(path.dirname(cachePath), { recursive: true });
        await fs.writeFile(cachePath, "# powershell completion\n", "utf-8");

        await installCompletion("powershell", true, "steelengine");

        const profilePath = resolveCompletionProfilePath("powershell");
        const profile = await fs.readFile(profilePath, "utf-8");
        expect(profile).toBe(`# SteelEngine Completion\n. '${cachePath.replace(/'/g, "''")}'\n`);
      });
    } finally {
  ...
  it("rejects install when the completion cache is missing", async () => {
    ...
    try {
      await withEnvAsync({ HOME: homeDir, STEELENGINE_STATE_DIR: stateDir }, async () => {
        await expect(installCompletion("zsh", true, "steelengine")).rejects.toThrow(
