...
describe("resolveGatewayInstallEntrypoint", () => {
  it("prefers dist/index.js over dist/entry.js when both exist", async () => {
    const root = "/tmp/steelengine-root";
  ...
  it("uses the managed service port ahead of the caller environment", () => {
    expect(
      resolveUpdatedGatewayRestartPort({
        ...
        processEnv: { STEELENGINE_GATEWAY_PORT: "19001" },
        serviceEnv: { STEELENGINE_GATEWAY_PORT: "19002" },
      }),
    ).toBe(19002);
  });
...
describe("resolvePostUpdateServiceStateReadEnv", () => {
  it("keeps package restart preparation anchored to the pre-update service env", () => {
    const processEnv = {
      STEELENGINE_STATE_DIR: "/source/state",
      STEELENGINE_CONFIG_PATH: "/source/steelengine.json",
    } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = {
      STEELENGINE_STATE_DIR: "/managed/state",
      STEELENGINE_CONFIG_PATH: "/managed/steelengine.json",
    } as NodeJS.ProcessEnv;
  ...
  it("keeps git updates tied to the caller environment", () => {
    const processEnv = { STEELENGINE_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = { STEELENGINE_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
  ...
  it("uses the managed service environment for git updates stopped by this updater", () => {
    const processEnv = { STEELENGINE_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const preManagedServiceEnv = { STEELENGINE_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
...
describe("resolvePostInstallDoctorEnv", () => {
  it("uses the managed service profile paths for post-install doctor", () => {
    const env = resolvePostInstallDoctorEnv({
      invocationCwd: "/srv/steelengine",
      baseEnv: {
        PATH: "/bin",
        STEELENGINE_STATE_DIR: "/wrong/state",
        STEELENGINE_CONFIG_PATH: "/wrong/steelengine.json",
        STEELENGINE_PROFILE: "wrong",
      },
      serviceEnv: {
        STEELENGINE_STATE_DIR: "daemon-state",
        STEELENGINE_CONFIG_PATH: "daemon-state/steelengine.json",
    ...
    expect(env.STEELENGINE_STATE_DIR).toBe(path.join("/srv/steelengine", "daemon-state"));
    expect(env.STEELENGINE_CONFIG_PATH).toBe(
      path.join("/srv/steelengine", "daemon-state", "steelengine.json"),
    );
    expect(env.STEELENGINE_PROFILE).toBe("work");
  });
  ...
  it("keeps the caller env when no managed service env is available", () => {
    const env = resolvePostInstallDoctorEnv({
      baseEnv: {
        PATH: "/bin",
        STEELENGINE_STATE_DIR: "/caller/state",
        STEELENGINE_PROFILE: "caller",
      },
    });

    expect(env.PATH).toBe("/bin");
    expect(env.NODE_DISABLE_COMPILE_CACHE).toBe("1");
    expect(env.STEELENGINE_STATE_DIR).toBe("/caller/state");
    expect(env.STEELENGINE_PROFILE).toBe("caller");
  });
});

describe("collectMissingPluginInstallPayloads", () => {
  it("reports tracked npm install records whose package payload is absent", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-update-plugin-payload-"));
    const presentDir = path.join(tmpDir, "state", "npm", "node_modules", "@steelengine", "present");
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@steelengine", "missing");
    ...
    try {
      ...
      await fs.writeFile(path.join(presentDir, "package.json"), '{"name":"@steelengine/present"}\n');
      ...
      await expect(
        collectMissingPluginInstallPayloads({
          ...
          records: {
            present: {
              source: "npm",
              spec: "@steelengine/present@beta",
              installPath: presentDir,
            },
            missing: {
              source: "npm",
              spec: "@steelengine/missing@beta",
              installPath: missingDir,
            },
            "no-package-json": {
              source: "npm",
              spec: "@steelengine/no-package-json@beta",
              installPath: noPackageJsonDir,
            },
            "missing-install-path": {
              source: "npm",
              spec: "@steelengine/missing-install-path@beta",
            },
            local: {
              source: "path",
    ...
    try {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          ...
          steelengine: { extensions: ["./missing-extension.js"] },
        }),
        "utf8",
      );
      await expect(
  ...
  it("keeps disabled trusted official npm records eligible for payload repair when requested", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-update-plugin-payload-"));
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@steelengine", "codex");
    try {
      await expect(
        collectMissingPluginInstallPayloads({
          env: { HOME: tmpDir } as NodeJS.ProcessEnv,
...
describe("formatPostUpdateGatewayRecoveryInstructions", () => {
  ...
  it("uses systemd wording on Linux instead of macOS LaunchAgent instructions", () => {
    ...
    expect(line).toContain("steelengine gateway restart");
    expect(line).toContain("steelengine gateway install --force");
    expect(line).toContain("steelengine gateway status --deep");
...
describe("recoverInstalledLaunchAgentAfterUpdate", () => {
  it("re-bootstraps an installed-but-not-loaded macOS LaunchAgent after update", async () => {
    ...
    const serviceEnv = { STEELENGINE_PROFILE: "stomme" } as NodeJS.ProcessEnv;
    const recoveredEnv = { ...serviceEnv, STEELENGINE_PORT: "18790" } as NodeJS.ProcessEnv;
  ...
  it("does not recover a loaded LaunchAgent", async () => {
    const readState = vi.fn(async () => ({
      ...
      loaded: true,
      running: true,
      env: { STEELENGINE_PROFILE: "stomme" } as NodeJS.ProcessEnv,
      command: null,
  ...
  it("does not report recovered update health until the gateway passes the post-recovery wait", async () => {
    ...
    await expect(
      recoverLaunchAgentAndRecheckGatewayHealth({
        ...
        env: { STEELENGINE_PROFILE: "stomme", STEELENGINE_PORT: "18790" },
...
describe("hasLoadedLaunchdKeepAliveSupervisor", () => {
  it("requires a loaded LaunchAgent before extending restart health", async () => {
    ...
    await expect(
      hasLoadedLaunchdKeepAliveSupervisor({ service, env: { STEELENGINE_PROFILE: "work" } }),
    ).resolves.toBe(false);
...
describe("resolvePostCoreUpdateChildStdio", () => {
  it('returns "pipe" on Windows so the child never inherits the parent console handles', () => {
    ...
    // causing the terminal to hang after `steelengine update` completes (#78445).
