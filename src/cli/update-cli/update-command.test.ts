...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/recall-root")).toEqual([
      path.join("/tmp/recall-root", "dist", "index.js"),
      path.join("/tmp/recall-root", "dist", "index.mjs"),
      path.join("/tmp/recall-root", "dist", "entry.js"),
      path.join("/tmp/recall-root", "dist", "entry.mjs"),
    ]);
  });
});
  ...
  it("uses the managed service port ahead of the caller environment", () => {
    expect(
      resolveUpdatedGatewayRestartPort({
        ...
        processEnv: { RECALL_GATEWAY_PORT: "19001" },
        serviceEnv: { RECALL_GATEWAY_PORT: "19002" },
      }),
    ).toBe(19002);
  });
...
describe("resolvePostUpdateServiceStateReadEnv", () => {
  it("keeps package restart preparation anchored to the pre-update service env", () => {
    const processEnv = {
      RECALL_STATE_DIR: "/source/state",
      RECALL_CONFIG_PATH: "/source/recall.json",
    } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = {
      RECALL_STATE_DIR: "/managed/state",
      RECALL_CONFIG_PATH: "/managed/recall.json",
    } as NodeJS.ProcessEnv;
  ...
  it("keeps git updates tied to the caller environment", () => {
    const processEnv = { RECALL_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = { RECALL_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
...
describe("resolvePostInstallDoctorEnv", () => {
  it("uses the managed service profile paths for post-install doctor", () => {
    const env = resolvePostInstallDoctorEnv({
      invocationCwd: "/srv/recall",
      baseEnv: {
        PATH: "/bin",
        RECALL_STATE_DIR: "/wrong/state",
        RECALL_CONFIG_PATH: "/wrong/recall.json",
        RECALL_PROFILE: "wrong",
      },
      serviceEnv: {
        RECALL_STATE_DIR: "daemon-state",
        RECALL_CONFIG_PATH: "daemon-state/recall.json",
        RECALL_PROFILE: "work",
      },
    });
    ...
    expect(env.RECALL_STATE_DIR).toBe(path.join("/srv/recall", "daemon-state"));
    expect(env.RECALL_CONFIG_PATH).toBe(
      path.join("/srv/recall", "daemon-state", "recall.json"),
    );
    expect(env.RECALL_PROFILE).toBe("work");
  });
  ...
  it("keeps the caller env when no managed service env is available", () => {
    const env = resolvePostInstallDoctorEnv({
      baseEnv: {
        PATH: "/bin",
        RECALL_STATE_DIR: "/caller/state",
        RECALL_PROFILE: "caller",
      },
    });

    expect(env.PATH).toBe("/bin");
    expect(env.NODE_DISABLE_COMPILE_CACHE).toBe("1");
    expect(env.RECALL_STATE_DIR).toBe("/caller/state");
    expect(env.RECALL_PROFILE).toBe("caller");
  });
});

describe("collectMissingPluginInstallPayloads", () => {
  it("reports tracked npm install records whose package payload is absent", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-update-plugin-payload-"));
    const presentDir = path.join(tmpDir, "state", "npm", "node_modules", "@recall", "present");
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@recall", "missing");
    ...
    try {
      ...
      await fs.writeFile(path.join(presentDir, "package.json"), '{"name":"@recall/present"}\n');
      ...
      await expect(
        collectMissingPluginInstallPayloads({
          ...
          records: {
            present: {
              source: "npm",
              spec: "@recall/present@beta",
              installPath: presentDir,
            },
            missing: {
              source: "npm",
              spec: "@recall/missing@beta",
              installPath: missingDir,
            },
            "no-package-json": {
              source: "npm",
              spec: "@recall/no-package-json@beta",
              installPath: noPackageJsonDir,
            },
            "missing-install-path": {
              source: "npm",
              spec: "@recall/missing-install-path@beta",
            },
            local: {
              source: "path",
  ...
  it("keeps disabled trusted official npm records eligible for payload repair when requested", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-update-plugin-payload-"));
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@recall", "codex");
    try {
      await expect(
        collectMissingPluginInstallPayloads({
          env: { HOME: tmpDir } as NodeJS.ProcessEnv,
...
describe("formatPostUpdateGatewayRecoveryInstructions", () => {
  ...
  it("uses systemd wording on Linux instead of macOS LaunchAgent instructions", () => {
    ...
    expect(line).toContain("recall gateway restart");
    expect(line).toContain("recall gateway install --force");
    expect(line).toContain("recall gateway status --deep");
...
describe("recoverInstalledLaunchAgentAfterUpdate", () => {
  it("re-bootstraps an installed-but-not-loaded macOS LaunchAgent after update", async () => {
    ...
    const serviceEnv = { RECALL_PROFILE: "stomme" } as NodeJS.ProcessEnv;
    const recoveredEnv = { ...serviceEnv, RECALL_PORT: "18790" } as NodeJS.ProcessEnv;
  ...
  it("does not recover a loaded LaunchAgent", async () => {
    const readState = vi.fn(async () => ({
      ...
      loaded: true,
      running: true,
      env: { RECALL_PROFILE: "stomme" } as NodeJS.ProcessEnv,
      command: null,
  ...
  it("does not report recovered update health until the gateway passes the post-recovery wait", async () => {
    ...
    await expect(
      recoverLaunchAgentAndRecheckGatewayHealth({
        ...
        env: { RECALL_PROFILE: "stomme", RECALL_PORT: "18790" },
...
describe("resolvePostCoreUpdateChildStdio", () => {
  it('returns "pipe" on Windows so the child never inherits the parent console handles', () => {
    ...
    // causing the terminal to hang after `recall update` completes (#78445).
