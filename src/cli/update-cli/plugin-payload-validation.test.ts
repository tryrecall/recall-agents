...
describe("runPluginPayloadSmokeCheck", () => {
  let tmpRoot: string;
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-payload-smoke-"));
  });
  afterEach(async () => {
  ...
  async function linkRecallPeerToHost(dir: string): Promise<void> {
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(resolveTestHostRoot(), path.join(dir, "node_modules", "recall"), "junction");
  }
  ...
  it("reports ok for a record whose package.json + main file exist", async () => {
    ...
    await writePackage(
      dir,
      { name: "@recall/discord", main: "dist/index.js" },
      "module.exports = {};",
    );
  ...
  it("reports a failure when the main entry file is missing on disk", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, { name: "@recall/brave", main: "dist/index.js" });
    const result = await runPluginPayloadSmokeCheck({
      records: { brave: { source: "npm", installPath: dir } },
      env: {},
    });
  ...
  it("accepts a manifest with no main field (Recall plugins commonly use `exports` or `recall.extensions`)", async () => {
    const dir = path.join(tmpRoot, "matrix");
    await writePackage(dir, { name: "@recall/plugin-matrix" });
    const result = await runPluginPayloadSmokeCheck({
  ...
  it("accepts a manifest that declares an existing `recall.extensions` entry and no `main`", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
  ...
  it("reports a failure when `recall.extensions` contains invalid entries", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@recall/brave-plugin",
      recall: { extensions: ["./index.js", " "] },
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        detail:
          "Plugin extension entry validation failed: package.json recall.extensions[1] must be a non-empty string",
      },
    ]);
  });
  ...
  it("reports a failure when an recall peer link is missing", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
        name: "@recall/codex",
        main: "dist/index.js",
        peerDependencies: { recall: ">=2026.5.18-beta.1" },
      },
      "export default {};\n",
    );
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        detail: `Plugin declares peerDependency "recall" but peer link audit failed: missing ${path.join(
          dir,
          "node_modules",
          "recall",
        )}.`,
  ...
  it("reports a failure when an recall peer link is a stale real directory", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const stalePeerDir = path.join(dir, "node_modules", "recall");
  ...
  it("reports a failure when an recall peer link points at the wrong package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const wrongHostRoot = path.join(tmpRoot, "old-recall");
    await fs.mkdir(wrongHostRoot, { recursive: true });
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(wrongHostRoot, path.join(dir, "node_modules", "recall"), "junction");
  ...
  it("accepts an recall peer link when it resolves to the host package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    await linkRecallPeerToHost(dir);
  ...
  it("reports a failure when an `recall.extensions` entry file is missing", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@recall/brave-plugin",
