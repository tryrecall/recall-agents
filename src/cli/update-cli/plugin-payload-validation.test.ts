...
describe("runPluginPayloadSmokeCheck", () => {
  let tmpRoot: string;
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-payload-smoke-"));
  });
  afterEach(async () => {
    vi.restoreAllMocks();
  ...
  async function linkSteelEnginePeerToHost(dir: string): Promise<void> {
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(resolveTestHostRoot(), path.join(dir, "node_modules", "steelengine"), "junction");
  }
  ...
  it("reports ok for a record whose package.json + main file exist", async () => {
    ...
    await writePackage(
      dir,
      { name: "@steelengine/discord", main: "dist/index.js" },
      "module.exports = {};",
    );
  ...
  it("checks a selected manifest root without an installed-index record", async () => {
    ...
    await writePackage(
      dir,
      { name: "@steelengine/codex", steelengine: { extensions: ["./index.js"] } },
      "export default {};",
    );
  ...
  it("reports a failure when the main entry file is missing on disk", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, { name: "@steelengine/brave", main: "dist/index.js" });
    const result = await runPluginPayloadSmokeCheck({
      records: { brave: { source: "npm", installPath: dir } },
      env: {},
    });
  ...
  it("accepts a manifest with no main field (SteelEngine plugins commonly use `exports` or `steelengine.extensions`)", async () => {
    const dir = path.join(tmpRoot, "matrix");
    await writePackage(dir, { name: "@steelengine/plugin-matrix" });
    const result = await runPluginPayloadSmokeCheck({
  ...
  it("accepts a manifest that declares an existing `steelengine.extensions` entry and no `main`", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
  ...
  it("reports a failure when `steelengine.extensions` contains invalid entries", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@steelengine/brave-plugin",
      steelengine: { extensions: ["./index.js", " "] },
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        reason: "missing-extension-entry",
        detail:
          "Plugin extension entry validation failed: package.json steelengine.extensions[1] must be a non-empty string",
      },
    ]);
  });
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        reason: "missing-extension-entry",
        detail:
          "Plugin extension entry validation failed: package.json steelengine.extensions is empty",
      },
    ]);
  });
  ...
  it("accepts a packaged TypeScript extension entry when compiled runtime output exists", async () => {
    ...
    await writePackage(dir, {
      ...
      steelengine: { extensions: ["./index.ts"] },
    });
  ...
  it("reports a failure when an steelengine peer link is missing", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
        name: "@steelengine/codex",
        main: "dist/index.js",
        peerDependencies: { steelengine: ">=2026.5.18-beta.1" },
      },
      "export default {};\n",
    );
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        detail: `Plugin declares peerDependency "steelengine" but peer link audit failed: missing ${path.join(
          dir,
          "node_modules",
          "steelengine",
        )}.`,
  ...
  it("reports a failure when an steelengine peer link is a stale real directory", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const stalePeerDir = path.join(dir, "node_modules", "steelengine");
  ...
  it("reports a failure when an steelengine peer link points at the wrong package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const wrongHostRoot = path.join(tmpRoot, "old-steelengine");
    await fs.mkdir(wrongHostRoot, { recursive: true });
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(wrongHostRoot, path.join(dir, "node_modules", "steelengine"), "junction");
  ...
  it("accepts an steelengine peer link when it resolves to the host package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    await linkSteelEnginePeerToHost(dir);
  ...
  it("reports a failure when an `steelengine.extensions` entry file is missing", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@steelengine/brave-plugin",
