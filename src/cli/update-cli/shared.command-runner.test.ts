...
describe("createGlobalCommandRunner", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "steelengine-update-root-" }, async (base) => {
        const storeRoot = path.join(base, "store", "steelengine");
        const packageRoot = path.join(base, "global", "v11", "install", "node_modules", "steelengine");
        ...
        process.argv[1] = path.join(packageRoot, "steelengine.mjs");
        try {
