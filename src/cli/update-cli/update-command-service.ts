...
async function hasLoadedLaunchdKeepAliveSupervisor(params: {
  ...
  // SteelEngine's loaded LaunchAgent has canonical KeepAlive policy. Read this once before
...
function formatPostUpdateGatewayRecoveryLine(platform: NodeJS.Platform): string {
  const restartCommand = replaceCliName(formatCliCommand("steelengine gateway restart"), CLI_NAME);
  const installCommand = replaceCliName(
    formatCliCommand("steelengine gateway install --force"),
    CLI_NAME,
  );
...
function formatPostUpdateGatewayRecoveryInstructions(
  ...
  if (isPackageManagerUpdateMode(result.mode) && beforeVersion) {
    lines.push(
      `Rollback: reinstall SteelEngine ${beforeVersion} with the same package manager, then rerun \`${replaceCliName(formatCliCommand("steelengine gateway install --force"), CLI_NAME)}\`.`,
    );
  }
  return lines;
}
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("steelengine.updateCommandServiceTestApi")] =
    {
...
export class UpdateCommandAbort extends Error {
  constructor() {
    super("steelengine-update-abort");
...
function formatGatewayAncestryBlockMessage(pid: number): string {
  return `steelengine update detected it is running inside the gateway process tree.
Gateway PID ${pid} is an ancestor of this process, so this updater cannot safely stop or restart the gateway that owns it.
Run \`${replaceCliName(formatCliCommand("steelengine update"), CLI_NAME)}\` from a shell outside the gateway service, or stop the gateway service first and then update.`;
}
...
export async function maybeStopManagedServiceBeforeMutableUpdate(params: {
  ...
  if (serviceMatchesMutationRoot === false) {
    if (!params.jsonMode) {
      defaultRuntime.log(
        theme.muted(
          `Managed gateway service points at a different SteelEngine root; leaving it running during this ${params.updateInstallKind} update.`,
        ),
      );
    }
    return {
...
function isRunningInsideGatewayService(
  ...
  if (env.STEELENGINE_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER) {
    return false;
  }
  const serviceKind = env.STEELENGINE_SERVICE_KIND?.trim();
...
export async function resolvePackageRuntimePreflight(params: {
  ...
  return resultError(
    [
      `${runtimeLabel} is too old for steelengine@${targetVersion}.`,
      `The requested package requires ${status.nodeEngine}.`,
      runtime.nodeRunner
        ? "Upgrade the Node runtime that owns the managed Gateway service, then rerun `steelengine update`."
        : "Upgrade to Node 22.22.3+, Node 24.15.0+, or Node 25.9.0+, then rerun `steelengine update`.",
      "Bare `npm i -g steelengine` can silently install an older compatible release.",
      "After upgrading Node, use `npm i -g steelengine@latest`.",
    ].join("\n"),
  );
}
...
export function stripGatewayServiceMarkerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  delete resolvedEnv.STEELENGINE_SERVICE_MARKER;
  delete resolvedEnv.STEELENGINE_SERVICE_KIND;
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("steelengine completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
export async function maybeRestartService(params: {
  ...
  const verifyRestartedGateway = async (
    ...
    const diagnosticLines = [
      ...
      `Run \`${replaceCliName(formatCliCommand("steelengine gateway status --deep"), CLI_NAME)}\` for details.`,
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("steelengine gateway restart"), CLI_NAME)}`,
          ),
        );
      }
      if (
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("steelengine doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("steelengine gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("steelengine gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
