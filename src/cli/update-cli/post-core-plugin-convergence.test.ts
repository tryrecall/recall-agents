...
const mocks = vi.hoisted(() => ({
  ...
  relinkRecallPeerDependenciesInManagedNpmRoot: vi.fn(),
...
vi.mock("../../plugins/plugin-peer-link.js", () => ({
  relinkRecallPeerDependenciesInManagedNpmRoot:
    mocks.relinkRecallPeerDependenciesInManagedNpmRoot,
}));
...
describe("runPostCorePluginConvergence", () => {
  ...
  beforeEach(() => {
    ...
    mocks.relinkRecallPeerDependenciesInManagedNpmRoot.mockResolvedValue({
      checked: 0,
      attempted: 0,
      repaired: 0,
      skipped: 0,
  ...
  function makeTempDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-post-core-convergence-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  it("calls repair with RECALL_UPDATE_POST_CORE_CONVERGENCE=1 set", async () => {
    const cfg = { plugins: { entries: {} } } as unknown as RecallConfig;
    await runPostCorePluginConvergence({
      cfg,
      env: { RECALL_UPDATE_IN_PROGRESS: "1" },
    });
  ...
  it("uses the candidate runtime version over a stale inherited host version", async () => {
    ...
    await runPostCorePluginConvergence({
      cfg,
      env: { RECALL_COMPATIBILITY_HOST_VERSION: "2026.5.12" },
    });
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      cfg,
      env: {
  ...
  it("returns the post-repair install records so callers can re-seed pluginConfig", async () => {
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { discord: { enabled: true } } } } as unknown as RecallConfig,
      env: {},
    });
  ...
  it("repairs managed npm recall peer links before payload smoke checks", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [],
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { codex: { enabled: true } } } } as unknown as RecallConfig,
      env: { RECALL_STATE_DIR: "/tmp/recall-state" },
    });

    expect(mocks.relinkRecallPeerDependenciesInManagedNpmRoot).toHaveBeenCalledWith({
      npmRoot: "/tmp/recall-state/npm",
      logger: {},
    });
    expect(result.changes).toEqual([
      "Repaired Recall host peer link(s) for 1 managed npm plugin package(s).",
    ]);
    expect(
      mocks.relinkRecallPeerDependenciesInManagedNpmRoot.mock.invocationCallOrder[0],
  ...
  it("flags errored=true and surfaces actionable guidance when repair warns", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "discord" from @recall/discord: ENETUNREACH.',
      ],
      records: {},
    });
    const result = await runPostCorePluginConvergence({
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        message:
          'Failed to install missing configured plugin "discord" from @recall/discord: ENETUNREACH.',
        guidance: ["Run `recall doctor --fix` to retry plugin repair."],
      },
    ]);
  });
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        guidance: [
          "Run `recall doctor --fix` to retry plugin repair.",
          "Run `recall plugins inspect brave --runtime --json` for details.",
        ],
      },
    ]);
  });
      ...
      warnings: [
        {
          ...
          guidance: ["Run `recall doctor --fix`."],
        },
        {
