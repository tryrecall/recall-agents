...
const mocks = vi.hoisted(() => ({
  ...
  relinkSteelEnginePeerDependenciesInManagedNpmRoot: vi.fn(),
...
vi.mock("../../plugins/plugin-peer-link.js", () => ({
  relinkSteelEnginePeerDependenciesInManagedNpmRoot:
    mocks.relinkSteelEnginePeerDependenciesInManagedNpmRoot,
}));
...
describe("runPostCorePluginConvergence", () => {
  ...
  beforeEach(() => {
    ...
    mocks.relinkSteelEnginePeerDependenciesInManagedNpmRoot.mockResolvedValue({
      checked: 0,
      attempted: 0,
      repaired: 0,
      skipped: 0,
  ...
  function makeTempDir(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-post-core-convergence-"));
    tempDirs.push(dir);
    return dir;
  }
  ...
  it("calls repair with STEELENGINE_UPDATE_POST_CORE_CONVERGENCE=1 set", async () => {
    const cfg = { plugins: { entries: {} } } as unknown as SteelEngineConfig;
  ...
  it("checks active payloads without running repair or peer-link convergence", async () => {
    ...
    await runActivePluginPayloadSmokeCheck({ cfg, records, env: { STEELENGINE_STATE_DIR: "/state" } });

    expect(mocks.runPluginPayloadSmokeCheck).toHaveBeenCalledWith({
      records: { active: records.active },
      env: { STEELENGINE_STATE_DIR: "/state" },
    });
    expect(mocks.repairMissingConfiguredPluginInstalls).not.toHaveBeenCalled();
    expect(mocks.relinkSteelEnginePeerDependenciesInManagedNpmRoot).not.toHaveBeenCalled();
  });
  ...
  it("uses the candidate runtime version over a stale inherited host version", async () => {
    ...
    await runPostCorePluginConvergence({
      cfg,
      env: { STEELENGINE_COMPATIBILITY_HOST_VERSION: "2026.5.12" },
    });
    expect(mocks.repairMissingConfiguredPluginInstalls).toHaveBeenCalledWith({
      cfg,
      env: {
  ...
  it("returns the post-repair install records so callers can re-seed pluginConfig", async () => {
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { discord: { enabled: true } } } } as unknown as SteelEngineConfig,
      env: {},
    });
  ...
  it("repairs managed npm steelengine peer links in every managed npm project before payload smoke checks", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [],
    ...
    mocks.listManagedPluginNpmRoots.mockResolvedValue([
      "/tmp/steelengine-state/npm",
      "/tmp/steelengine-state/npm/projects/codex",
    ]);
    mocks.relinkSteelEnginePeerDependenciesInManagedNpmRoot
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: { codex: { enabled: true } } } } as unknown as SteelEngineConfig,
      env: { STEELENGINE_STATE_DIR: "/tmp/steelengine-state" },
    });

    expect(mocks.relinkSteelEnginePeerDependenciesInManagedNpmRoot).toHaveBeenNthCalledWith(1, {
      npmRoot: "/tmp/steelengine-state/npm",
      logger: {},
      onPackageReadError: expect.any(Function),
    });
    expect(mocks.relinkSteelEnginePeerDependenciesInManagedNpmRoot).toHaveBeenNthCalledWith(2, {
      npmRoot: "/tmp/steelengine-state/npm/projects/codex",
      logger: {},
      onPackageReadError: expect.any(Function),
    });
    expect(result.changes).toEqual([
      "Repaired SteelEngine host peer link(s) for 1 managed npm plugin package(s).",
    ]);
    expect(
      mocks.relinkSteelEnginePeerDependenciesInManagedNpmRoot.mock.invocationCallOrder[0],
    ).toBeLessThan(
      expectDefined(
  ...
  it("keeps repair warnings nonblocking with actionable guidance", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "discord" from @steelengine/discord: ENETUNREACH.',
      ],
      records: {},
    });
    const result = await runPostCorePluginConvergence({
  ...
  it("keeps failed configured-plugin repair fetches nonblocking", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "matrix" from clawhub:@steelengine/matrix@beta: ClawHub ClawPack download for @steelengine/matrix@2026.6.1-beta.1 body stalled after 30000ms.',
      ],
  ...
  it("surfaces repair notices without marking convergence errored", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      ...
      notices: [
        'ClawHub trust warning for "@steelengine/discord@1.2.3": ClawHub has not completed a fresh clean security check for this release. Status: security scan is pending. Review the package before enabling it.',
      ],
      warnings: [],
    ...
    expect(result.warnings).toStrictEqual([
      {
        ...
        guidance: [
          ...
          "Run `steelengine plugins inspect brave --runtime --json` for details.",
        ],
      },
    ]);
  });
  ...
  it("uses ownership guidance and a coherent update outcome for unreadable package.json", async () => {
    ...
    const guidance = [
      "Fix file access for /p/brave/package.json so it is readable by the user running SteelEngine. For EACCES or EPERM, correct its ownership or permissions; otherwise resolve the reported filesystem I/O error, then retry.",
      "Run `steelengine plugins inspect brave --runtime --json` for details.",
    ];
    expect(result.warnings).toStrictEqual([
      {
  ...
  it("does not duplicate a package-scoped repair error owned by a smoke failure", async () => {
    const installPath = "/tmp/steelengine-state/npm/projects/brave/node_modules/brave";
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValue({
      changes: [],
      warnings: [],
      records: { brave: { source: "npm", installPath } },
    });
    mocks.relinkSteelEnginePeerDependenciesInManagedNpmRoot.mockImplementation(
  ...
  it("keeps an unowned package read error visible for startup to block", async () => {
    const packageDir = "/tmp/steelengine-state/npm/node_modules/untracked";
    mocks.relinkSteelEnginePeerDependenciesInManagedNpmRoot.mockImplementation(
      async (params: { onPackageReadError?: (error: unknown, packageDir: string) => void }) => {
    ...
    const result = await runPostCorePluginConvergence({
      cfg: { plugins: { entries: {} } } as unknown as SteelEngineConfig,
      env: { STEELENGINE_STATE_DIR: "/tmp/steelengine-state" },
    });

    expect(result.warnings).toStrictEqual([
      {
        reason: "Failed to repair managed npm SteelEngine host peer links: EACCES: permission denied",
        message: "Failed to repair managed npm SteelEngine host peer links: EACCES: permission denied",
        guidance: ["Run `steelengine update repair` to retry plugin repair."],
      },
    ]);
    expect(result.errored).toBe(false);
