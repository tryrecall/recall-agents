...
const STEP_LABELS: Record<string, string> = {
  ...
  "recall doctor entry": "Checking doctor entrypoint",
  "recall doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g recall@latest");
    hints.push(
      "System install outline: recall gateway stop -> sudo <system-npm> i -g recall@latest -> recall gateway install --force -> recall gateway restart.",
    );
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g recall@latest --omit=optional");
  }
