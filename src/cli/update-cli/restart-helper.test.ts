...
`,
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18789) {
    const stateCheck = "$taskState = Get-RecallScheduledTaskState -TaskName $taskName";
    const runningGuard = 'if ($taskState -eq "Running")';
    const endCommand =
      'Invoke-RecallSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10';
    const skipEndLog = "recall restart skipped schtasks end";
    const pollLoop = "for ($attempt = 1; $attempt -le 10; $attempt++)";
    const pollCall = `Get-RecallListenerPids -Port $port`;
    const forceKillBranch = "if ($attempt -eq 10)";
    const forceKillCommand = "Stop-Process -Id $listenerPid -Force";
    const runCommand =
      'Invoke-RecallSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30';
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'recall-gateway.service'");
    ...
    it("creates restart scripts in a private temp directory with exclusive creation", async () => {
      ...
      const oldCandidatePath = path.join(os.tmpdir(), `recall-restart-${timestamp}.sh`);
      const victimDir = await makeTempDir("recall-restart-helper-victim-");
      ...
      try {
        ...
        expect(path.basename(scriptDir)).toMatch(/^recall-restart-/);
    ...
    it("uses RECALL_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        RECALL_PROFILE: "default",
        RECALL_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("fails with sudo systemd guidance when the gateway unit is system-scoped", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const tmpDir = await makeTempDir("recall-restart-helper-");
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        ...
        RECALL_STATE_DIR: path.join(tmpDir, "state"),
      });
      const result = await executeScript(scriptPath, {
        PATH: `${fakeBinDir}:${process.env.PATH ?? ""}`,
        RECALL_SYSTEMCTL_CALLS: callsPath,
      });
      const calls = await fs.readFile(callsPath, "utf-8");

      expect(result.code).toBe(78);
      expect(result.stderr).toContain("system-scoped recall gateway unit detected");
      expect(result.stderr).toContain("sudo systemctl restart recall-gateway.service");
      expect(calls).toContain("--user is-active --quiet recall-gateway.service");
      expect(calls).toContain("is-active --quiet recall-gateway.service");
      expect(calls).not.toContain("--user restart recall-gateway.service");
    });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.recall.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.recall.gateway'");
    ...
    it("captures macOS launchctl stderr to ~/.recall/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.recall/logs/gateway-restart.log' 2>&1");
    ...
    it("uses RECALL_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/recall-state/logs' 2>/dev/null && : >>'/tmp/recall-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/recall-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
...
`,
      ...
      expect(log).toContain("recall restart attempt source=update target=ai.recall.gateway");
      expect(log).toContain("launchctl kickstart -k gui/501/ai.recall.gateway");
      expect(log).toContain("recall restart failed source=update status=42");
      expect(log).not.toContain("recall restart done source=update");
    });
...
`,
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        RECALL_LAUNCHD_LABEL: "ai.recall.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        RECALL_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.recall.$(echo injected)");
      expect(log).not.toContain("target=ai.recall.injected");
    });

    it("uses RECALL_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        RECALL_PROFILE: "default",
        RECALL_LAUNCHD_LABEL: "com.custom.recall",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.recall'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'Recall Gateway'");
      expect(content).toContain("function Invoke-RecallSchtasksWithTimeout");
      expect(content).toContain("function Get-RecallScheduledTaskState");
      expect(content).toContain("function Invoke-RecallStartupLauncher");
      expect(content).toContain("Get-ScheduledTask -TaskName $TaskName");
      expect(content).toContain("recall restart skipped schtasks end");
      expect(content).toContain(
        '$launcherPath = Join-Path $env:USERPROFILE ".recall\\gateway.cmd"',
      );
      expect(content).toContain("recall restart launched startup fallback");
      expectWindowsRestartWaitOrdering(content);
      expect(content).toContain('del "%~f0" >nul 2>&1');
      expect(content).toContain('rmdir "%RECALL_RESTART_SCRIPT_DIR%" >nul 2>&1');
      await cleanupScript(scriptPath);
    });

    it("uses RECALL_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      ...
      expect(content).toContain("$taskName = 'Recall Gateway (custom)'");
      expect(content).toContain("Get-RecallScheduledTaskState -TaskName $taskName");
      expect(content).toContain(
        'Invoke-RecallSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10',
      );
      expect(content).toContain("$status = Invoke-RecallStartupLauncher");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        RECALL_PROFILE: "production",
      });
      expect(content).toContain("recall-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        RECALL_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.recall.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain("$taskName = 'Recall Gateway (production)'");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        RECALL_LAUNCHD_LABEL: "ai.recall.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.recall.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
