...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.tryrecall.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.tryrecall.gateway'");
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "Recall Gateway"');
      expect(content).toContain('schtasks /Run /TN "Recall Gateway"');
    ...
    it("uses RECALL_WINDOWS_TASK_NAME override on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "Recall Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "Recall Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      expect(content).toContain("gui/502/ai.tryrecall.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "Recall Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        RECALL_LAUNCHD_LABEL: "ai.tryrecall.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.tryrecall.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
