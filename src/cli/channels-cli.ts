...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["recall channels list", "List configured channels."],
          ["recall channels list --all", "Show configured, bundled, and installable channels."],
          ["recall channels add", "Open guided channel setup."],
          ["recall channels status --probe", "Run channel status checks and probes."],
          [
            "recall channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["recall channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.recall.ai/cli/channels",
        )}\n`,
    );
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["recall channels add", "Open guided setup for available chat channels."],
          [
            "recall channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["recall channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)
