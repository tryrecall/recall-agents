...
function createCompletionProgram(): Command {
  ...
  program.option(
    "--status-json",
    "Output JSON (alias for `models status --json`) in $STEELENGINE_STATE_DIR",
  );
...
describe("completion-cli", () => {
  it("generates zsh functions for nested subcommands", () => {
    ...
    expect(script).toContain("_steelengine_gateway()");
    expect(script).toContain("(status) _steelengine_gateway_status ;;");
    expect(script).toContain("(restart) _steelengine_gateway_restart ;;");
    expect(script).toContain("--force[Force the action]");
    expect(script).toContain("\\`models status --json\\`");
    expect(script).toContain("\\$STEELENGINE_STATE_DIR");
  });

  it("escapes zsh option descriptions for double-quoted arguments specs", () => {
    const program = new Command()
      .name("steelengine")
      .option("--literal", "Use $STEELENGINE_STATE_DIR with `model/list` and John's profile");

    const script = getCompletionScript("zsh", program);

    expect(script).toContain(
      "--literal[Use \\$STEELENGINE_STATE_DIR with \\`model/list\\` and John's profile]",
    );
  ...
  it("defers zsh registration until compinit is available", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-zsh-completion-"));
    try {
      const scriptPath = path.join(tempDir, "steelengine.zsh");
      ...
      const result = spawnSync(
        ...
        [
          ...
          `
            ...
            [[ -z "\${_comps[steelengine]-}" ]] || exit 10
            [[ "\${precmd_functions[(r)_steelengine_register_completion]}" = "_steelengine_register_completion" ]] || exit 11
            autoload -Uz compinit
            compinit -C
            _steelengine_register_completion
            [[ -z "\${precmd_functions[(r)_steelengine_register_completion]}" ]] || exit 12
            [[ "\${_comps[steelengine]-}" = "_steelengine_root_completion" ]]
          `,
        ],
        {
          encoding: "utf8",
  ...
  it("generates PowerShell command paths without the executable prefix", () => {
    ...
    expect(script).not.toContain("if ($commandPath -eq 'steelengine gateway') {");
  ...
  it("generates valid PowerShell root arrays when commands or options are empty", () => {
    const commandsOnly = new Command().name("steelengine");
    commandsOnly.command("status");
    const optionsOnly = new Command().name("steelengine").option("--json", "JSON output");
    const empty = new Command().name("steelengine");
  ...
  it("generates fish completions for root and nested command contexts", () => {
    ...
    expect(script).toContain(
      'complete -c steelengine -n "__fish_use_subcommand" -a "gateway" -d \'Gateway commands\'',
    );
    expect(script).toContain(
      'complete -c steelengine -n "__steelengine_command_path_matches gateway -- -t --token" -a "status" -d \'Show gateway status\'',
    );
    expect(script).toContain(
      "complete -c steelengine -n \"__steelengine_command_path_matches gateway -- -t --token\" -l force -d 'Force the action'",
    );
    expect(script).toContain(
      "complete -c steelengine -n \"__steelengine_command_path_matches gateway status -- -t --token\" -l json -d 'JSON output'",
    );
    expect(script).toContain("__steelengine_command_path_matches gateway -- -t --token");
  ...
  it("scopes fish value-taking option skips to the active command path", () => {
    ...
    expect(script).toContain("__steelengine_command_path_matches agent -- --verbose");
    expect(script).toContain("__steelengine_command_path_matches sessions cleanup --");
    expect(script).not.toContain("__steelengine_command_path_matches sessions cleanup -- --verbose");
    expect(script).toContain(
      "complete -c steelengine -n \"__steelengine_command_path_matches sessions cleanup --\" -l dry-run -d 'Preview cleanup'",
    );
  });
  ...
  it("uses Commander's parsed flags instead of value placeholder syntax", () => {
    ...
    expect(fishScript).toContain(
      "complete -c steelengine -n \"__fish_use_subcommand\" -l trigger-script -d 'Condition script file, or - for stdin'",
    );
  ...
  it("generates valid Bash completion without subcommands", () => {
    ...
    const script = getCompletionScript("bash", new Command().name("steelengine"));
...
// Commander aliases are typeable commands (`steelengine capability` == `steelengine infer`),
...
describe("completion-cli command aliases", () => {
  it("completes root and nested aliases in zsh lists and dispatch", () => {
    ...
    expect(script).toContain("(infer|capability) _steelengine_infer ;;");
    expect(script).toContain("'create[Add a job]'");
    expect(script).toContain("(add|create) _steelengine_cron_add ;;");
  });
  ...
  it("offers options after a nested alias in bash", () => {
    ...
    const result = spawnSync(
      ...
      [
        ...
        "-c",
        `${script}
COMP_WORDS=(steelengine --profile work cron create --a)
COMP_CWORD=5
_steelengine_completion
printf '%s\\n' "\${COMPREPLY[@]}"
`,
      ],
      { encoding: "utf8" },
  ...
  it("completes aliases and their subtrees in fish", () => {
    ...
    expect(script).toContain(
      'complete -c steelengine -n "__fish_use_subcommand" -a "capability" -d \'Run inference\'',
    );
    expect(script).toContain(
      'complete -c steelengine -n "__steelengine_command_path_matches capability -- --profile" -a "embed" -d \'Embed text\'',
    );
    expect(script).toContain(
      'complete -c steelengine -n "__steelengine_command_path_matches cron -- --profile" -a "create" -d \'Add a job\'',
    );
    expect(script).toContain(
      "complete -c steelengine -n \"__steelengine_command_path_matches cron create -- --profile --at\" -l at -d 'Schedule time'",
    );
  });
