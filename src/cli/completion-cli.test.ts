...
describe("completion-cli", () => {
  ...
  it("defers zsh registration until compinit is available", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-zsh-completion-"));
    try {
      const scriptPath = path.join(tempDir, "recall.zsh");
      ...
      const result = spawnSync(
        ...
        [
          ...
          `
            ...
            [[ "\${_comps[recall]-}" = "_openclaw_root_completion" ]]
          `,
        ],
        {
          encoding: "utf8",
  ...
  it("generates PowerShell command paths without the executable prefix", () => {
    ...
    expect(script).not.toContain("if ($commandPath -eq 'recall gateway') {");
  ...
  it("generates fish completions for root and nested command contexts", () => {
    ...
    expect(script).toContain(
      'complete -c recall -n "__fish_use_subcommand" -a "gateway" -d \'Gateway commands\'',
    );
    expect(script).toContain(
      'complete -c recall -n "__fish_seen_subcommand_from gateway" -a "status" -d \'Show gateway status\'',
    );
    expect(script).toContain(
      "complete -c recall -n \"__fish_seen_subcommand_from gateway\" -l force -d 'Force the action'",
    );
  });
});
