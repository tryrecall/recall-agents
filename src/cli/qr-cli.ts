...
function readDevicePairPublicUrlFromConfig(cfg: RecallConfig): string | undefined {
...
function shouldResolveLocalGatewayPasswordSecret(
  ...
  if (trimToUndefined(env.RECALL_GATEWAY_PASSWORD)) {
    return false;
  }
  ...
  const envToken = trimToUndefined(env.RECALL_GATEWAY_TOKEN);
...
async function resolveLocalGatewayPasswordSecretIfNeeded(cfg: RecallConfig): Promise<void> {
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.recall.ai/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the Recall mobile app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("recall devices list")}`,
          `  ${theme.command("recall devices approve <requestId>")}`,
        );
