...
function readDevicePairPublicUrlFromConfig(cfg: SteelEngineConfig): string | undefined {
...
function shouldResolveLocalGatewayPasswordSecret(
  ...
  if (trimToUndefined(env.STEELENGINE_GATEWAY_PASSWORD)) {
    return false;
  }
...
async function resolveLocalGatewayPasswordSecretIfNeeded(cfg: SteelEngineConfig): Promise<void> {
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.steelengine.ai/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the SteelEngine mobile app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("steelengine devices list")}`,
          `  ${theme.command("steelengine devices approve <requestId>")}`,
        );
