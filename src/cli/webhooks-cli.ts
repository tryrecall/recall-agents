...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.recall.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Recall hooks")
    ...
    .option("--hook-url <url>", "Recall hook URL")
    .option("--hook-token <token>", "Recall hook token")
...
function parseGmailSetupOptions(raw: Record<string, unknown>): GmailSetupOptions {
  ...
  if (!account) {
    throw new Error(
      `--account is required. Example: ${formatCliCommand("recall webhooks gmail setup --account default")}.`,
    );
  }
