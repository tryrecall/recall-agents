    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "recall node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["recall node status", "Check node host service status."],
          ["recall node install", "Install the node host service."],
          ["recall node start", "Start the installed node host service."],
          ["recall node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.recall.ai/cli/node")}\n`,
    );
