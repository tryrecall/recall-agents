...
describe("cli program (nodes media)", () => {
  ...
  it("runs nodes camera snap and prints two MEDIA paths", async () => {
    ...
    expect(mediaPaths[0]).toContain("steelengine-camera-snap-");
    expect(mediaPaths[1]).toContain("steelengine-camera-snap-");
  ...
  it("runs one unknown-position camera snap for a Linux node", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /steelengine-camera-snap-unknown-.*\.jpg$/,
    });
  });
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /steelengine-camera-clip-front-.*\.mp4$/,
    });
  });
  ...
  it("runs an unknown-position camera clip for a Linux node", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /steelengine-camera-clip-unknown-.*\.mp4$/,
    });
  });
    ...
    it.each([
      {
        ...
        expectedPathPattern: /steelengine-camera-snap-front-.*\.jpg$/,
      },
      {
