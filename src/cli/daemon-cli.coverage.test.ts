...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      RECALL_GATEWAY_PORT: String(params.port),
      ...(params.wrapperPath ? { RECALL_WRAPPER: params.wrapperPath } : {}),
      ...(params.token ? { RECALL_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-daemon-cli-"));
    ...
    process.env.RECALL_STATE_DIR = tmpDir;
    process.env.RECALL_CONFIG_PATH = path.join(tmpDir, "recall.json");
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        RECALL_PROFILE: "dev",
        RECALL_STATE_DIR: "/tmp/recall-daemon-state",
        RECALL_CONFIG_PATH: "/tmp/recall-daemon-state/recall.json",
        RECALL_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.recall.gateway.plist",
    });
  ...
  it("passes the existing service environment into the install plan on forced reinstall", async () => {
    ...
    expect((installPlanParams.env as NodeJS.ProcessEnv).RECALL_WRAPPER).toBe(
      "/usr/local/bin/recall-doppler",
    );
  });
