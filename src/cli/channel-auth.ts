...
import { getRuntimeConfig, readConfigFileSnapshot, type SteelEngineConfig } from "../config/config.js";
...
function isConfiguredAuthPlugin(plugin: ChannelPlugin, cfg: SteelEngineConfig): boolean {
  const key = plugin.id;
...
function resolveConfiguredAuthChannelInput(cfg: SteelEngineConfig, mode: ChannelAuthMode): string {
  ...
  if (configured.length === 0) {
    throw new Error(
      `No configured channel supports ${mode}. Run ${formatCliCommand("steelengine channels status")} to inspect channels or ${formatCliCommand("steelengine channels add --channel <channel>")} to add one.`,
    );
  }
...
async function resolveChannelPluginForMode(
  ...
  if (!channelId) {
    throw new Error(
      `Unsupported channel "${channelInput}". Run ${formatCliCommand("steelengine channels list")} to see available channels.`,
    );
  }
  ...
  if (!plugin || !supportsChannelAuthMode(plugin, mode)) {
    throw new Error(
      formatUnsupportedChannelActionMessage({
        channel: channelId,
        action: mode,
        inspectCommand: "steelengine channels status --channel " + channelId,
      }),
    );
  }
  return {
  ...
  if (!login) {
    throw new Error(
      formatUnsupportedChannelActionMessage({
        ...
        action: "login",
        inspectCommand: "steelengine channels status --channel " + channelInput,
      }),
    );
  }
