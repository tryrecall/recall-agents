...
describe("proxy cli runtime", () => {
  const envKeys = [
    ...
    "STEELENGINE_DEBUG_PROXY_CERT_DIR",
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "steelengine-proxy-cli-runtime-"));
    process.env.STEELENGINE_STATE_DIR = tempDir;
    process.env.STEELENGINE_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.STEELENGINE_DEBUG_PROXY_ENABLED;
    delete process.env.STEELENGINE_DEBUG_PROXY_SESSION_ID;
  ...
  it("prints actionable disabled proxy config output", async () => {
    ...
    expect(process.stdout["write"]).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Enable proxy.enabled with proxy.proxyUrl or STEELENGINE_PROXY_URL, or pass --proxy-url for an explicit one-off validation.\n",
    );
  });
  ...
  it("redacts malformed proxy URLs in text output", async () => {
    ...
    expect(process.stdout["write"]).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Fix proxy.proxyUrl, STEELENGINE_PROXY_URL, or --proxy-url so it uses a reachable http:// or https:// proxy.\n",
    );
  });
  ...
  it("prints proxy validation JSON and sets exit code on failure", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        enabled: true,
        source: "missing",
        errors: ["proxy validation requires proxy.proxyUrl, --proxy-url, or STEELENGINE_PROXY_URL"],
      },
      checks: [],
    });
    const { runProxyValidateCommand } = await import("./proxy-cli.runtime.js");
