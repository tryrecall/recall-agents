...
export function buildToolPluginPackageManifest(params: {
  ...
  const steelengine =
    params.packageManifest.steelengine &&
    typeof params.packageManifest.steelengine === "object" &&
    !Array.isArray(params.packageManifest.steelengine)
      ? { ...(params.packageManifest.steelengine as JsonObject) }
      : {};
  const existingExtensions = Array.isArray(steelengine.extensions)
    ? steelengine.extensions.filter((entry): entry is string => typeof entry === "string")
    : [];
...
export function validateToolPluginProject(params: {
  ...
  if (JSON.stringify(params.manifest) !== JSON.stringify(expectedManifest)) {
    errors.push("steelengine.plugin.json generated metadata is stale. Run steelengine plugins build.");
  }
  if (params.manifest.id !== params.metadata.id) {
    errors.push(
      `steelengine.plugin.json id (${String(params.manifest.id)}) must match entry id (${params.metadata.id})`,
    );
  }
  if (!params.manifest.configSchema || typeof params.manifest.configSchema !== "object") {
    errors.push("steelengine.plugin.json must include object configSchema");
  }
  ...
  if (missing.length > 0) {
    errors.push(`steelengine.plugin.json contracts.tools is missing: ${missing.join(", ")}`);
  }
  if (extra.length > 0) {
    errors.push(
      `steelengine.plugin.json contracts.tools has no matching defineToolPlugin tool: ${extra.join(
        ", ",
      )}`,
    );
  }
  ...
  if (extensionResolution.status !== "ok") {
    errors.push(
      extensionResolution.status === "missing" || extensionResolution.status === "empty"
        ? "package.json must include steelengine.extensions"
        : extensionResolution.error,
    );
  } else if (!extensionResolution.entries.includes(params.entry)) {
    errors.push(`package.json steelengine.extensions must include ${params.entry}`);
  }
  return errors;
}
...
export async function runPluginsBuildCommand(opts: PluginsBuildOptions): Promise<void> {
  ...
  if (opts.check) {
    ...
    if (
      ...
      defaultRuntime.error("Generated plugin metadata is out of date. Run steelengine plugins build.");
...
function writeToolPluginScaffold(params: { rootDir: string; id: string; name: string }): void {
  const packageManifest = {
    name: `steelengine-plugin-${params.id}`,
    version: "0.1.0",
    type: "module",
    private: true,
    scripts: {
  ...
  const description = `Add ${params.name} tools to SteelEngine.`;
...
import { getToolPluginMetadata } from "steelengine/plugin-sdk/tool-plugin";
...
Simple SteelEngine tool plugin.
...
function writeProviderPluginScaffold(params: { rootDir: string; id: string; name: string }): void {
  const packageName = `steelengine-plugin-${params.id}`;
  ...
  const description = `Add ${params.name} models to SteelEngine.`;
  const packageManifest = {
    name: packageName,
    version: "0.1.0",
    description: `SteelEngine provider plugin for ${params.name}.`,
    type: "module",
    scripts: {
      build: "tsc -p tsconfig.json",
      test: "vitest run --config ./vitest.config.ts",
  ...
  const indexSource = `import { definePluginEntry } from "steelengine/plugin-sdk/plugin-entry";
import { createProviderApiKeyAuthMethod } from "steelengine/plugin-sdk/provider-auth-api-key";
import { buildSingleProviderApiKeyCatalog } from "steelengine/plugin-sdk/provider-catalog-shared";
...
`;
  const testSource = `import { describe, expect, it } from "vitest";
import type { SteelEnginePluginApi, ProviderPlugin } from "steelengine/plugin-sdk/plugin-entry";
import entry from "./index.js";
...
describe(${idLiteral}, () => {
  it("registers the provider", () => {
    ...
    } as Partial<SteelEnginePluginApi>;

    entry.register(api as SteelEnginePluginApi);
...
SteelEngine provider plugin for ${params.name}.
...
jobs:
  publish:
    permissions:
      ...
      contents: read
      id-token: write
    uses: steelengine/clawhub/.github/workflows/package-publish.yml@${CLAWHUB_PACKAGE_PUBLISH_WORKFLOW_REF}
    with:
