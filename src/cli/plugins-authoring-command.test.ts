...
describe("plugin authoring commands", () => {
  beforeAll(async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-plugin-source-warm-"));
    try {
      const entryPath = writeSourceToolPluginProject({
        tmpDir,
        packageName: "steelengine-plugin-source-warm",
  ...
  it("preserves manifest-owned metadata while updating generated fields", () => {
    ...
    expect(
      validateToolPluginProject({
        ...
        entry: "./src/index.ts",
        manifest,
        packageManifest: { version: "1.2.3", steelengine: { extensions: ["./src/index.ts"] } },
      }),
    ).toEqual([]);
  });
  ...
  it("drops stale manifest-owned tool metadata when no generated metadata remains", () => {
    const metadata = createDemoMetadata();
    const packageManifest = { version: "1.2.3", steelengine: { extensions: ["./src/index.ts"] } };
    const manifest = buildToolPluginManifest({
      metadata,
      packageManifest,
      existingManifest: {
  ...
  it("aligns package metadata with the selected runtime extension entry", () => {
    expect(
      buildToolPluginPackageManifest({
        packageManifest: {
          name: "demo",
          steelengine: { setupEntry: "./setup.ts", extensions: ["./src/other.ts"] },
        },
        entry: "./src/index.ts",
      }),
    ).toEqual({
  ...
  it("reports stale manifest contracts", () => {
    ...
    expect(
      validateToolPluginProject({
        ...
        packageManifest: { steelengine: { extensions: ["./src/index.ts"] } },
      }),
    ).toEqual([
      "steelengine.plugin.json generated metadata is stale. Run steelengine plugins build.",
      "steelengine.plugin.json contracts.tools is missing: demo_echo",
      "steelengine.plugin.json contracts.tools has no matching defineToolPlugin tool: other_tool",
    ]);
  });

  it("reports missing entries with an author-facing path", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-plugin-missing-"));
  ...
  it("throws a user-friendly error when package.json is malformed JSON", async () => {
    const tmpDir = tempDirs.make("steelengine-plugin-bad-json-");
    const packagePath = path.join(tmpDir, "package.json");
    const entryPath = writeSourceToolPluginProject({
      tmpDir,
      packageName: "steelengine-plugin-bad-json",
      pluginId: "bad-json",
  ...
  it("loads source entries that import the SteelEngine plugin SDK package subpath", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-plugin-source-"));
    const entryPath = writeSourceToolPluginProject({
      tmpDir,
      packageName: "steelengine-plugin-source-demo",
  ...
  it("finishes a build from an absolute root after the launch directory is removed", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-plugin-deleted-cwd-build-"));
    const packagePath = path.join(tmpDir, "package.json");
    const entryPath = writeSourceToolPluginProject({
      tmpDir,
      packageName: "steelengine-plugin-deleted-cwd-build",
    ...
    try {
      ...
      expect(fs.existsSync(path.join(tmpDir, "steelengine.plugin.json"))).toBe(true);
      expect(log).toHaveBeenCalledWith(`Wrote ${path.join(tmpDir, "steelengine.plugin.json")}`);
  ...
  it("finishes init with an absolute directory after the launch directory is removed", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-plugin-deleted-cwd-init-"));
  ...
  it("scaffolds a dist-entry tool plugin project", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-plugin-init-"));
    ...
    expect(
      ...
      scripts: {
        "plugin:build": "npm run build && steelengine plugins build --entry ./dist/index.js",
        "plugin:validate": "npm run build && steelengine plugins validate --entry ./dist/index.js",
    ...
    expect(
      JSON.parse(fs.readFileSync(path.join(projectDir, "steelengine.plugin.json"), "utf8")),
    ).toMatchObject({
      id: "stock-quotes",
      name: 'Stock "Quotes"',
      configSchema: {
  ...
  it("scaffolds a provider plugin project with ClawHub validation and release metadata", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-provider-init-"));
    ...
    expect(packageManifest).toMatchObject({
      name: "steelengine-plugin-plugin-init-test",
      scripts: {
      ...
      steelengine: {
        extensions: ["./dist/index.js"],
        install: {
          clawhubSpec: "clawhub:steelengine-plugin-plugin-init-test",
    ...
    const manifest = JSON.parse(
      fs.readFileSync(path.join(projectDir, "steelengine.plugin.json"), "utf8"),
    );
    expect(manifest).toMatchObject({
      id: "plugin-init-test",
      name: "Plugin Init Test",
    ...
    expect(workflow).toContain(
      "steelengine/clawhub/.github/workflows/package-publish.yml@9d49df109d4ad3dc8a6ecf05d26b39f46d294721",
    );
  });
});
