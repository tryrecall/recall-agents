...
export async function runPluginsListCommand(
  ...
  if (list.length === 0) {
    runtime.log(
      theme.muted(
        `No plugins found. Run ${formatCliCommand("recall plugins install <plugin>")} to add one, or ${formatCliCommand("recall plugins list --json")} to inspect raw discovery state.`,
      ),
    );
    return;
  }
