...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["steelengine security audit", "Run a local security audit."],
          [
            "steelengine security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["steelengine security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
          ...
          ["steelengine security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["steelengine security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.steelengine.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("SteelEngine security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("steelengine security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("steelengine security audit --fix")}`));
        if (!fixResult) {
