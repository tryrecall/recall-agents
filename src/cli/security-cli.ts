...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["recall security audit", "Run a local security audit."],
          ["recall security audit --deep", "Include best-effort live Gateway probe checks."],
          ["recall security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "recall security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["recall security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["recall security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.recall.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Recall security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("recall security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("recall security audit --fix")}`));
        if (!fixResult) {
