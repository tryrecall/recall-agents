...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "recall", "terminal"] },
    { argv: ["node", "recall", "chat"] },
    { argv: ["node", "recall", "gateway"] },
    { argv: ["node", "recall", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "recall", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "recall", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "recall", "gateway", "status"] },
    { argv: ["node", "recall", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "recall", "--help"] },
    { argv: ["node", "recall", "gateway"] },
    { argv: ["node", "recall", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "recall.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("recall.internal");
    } finally {
      log.mockRestore();
    }
  });
