...
function expectLoadSteelEnginePluginsCall(
  callIndex: number,
  expected: {
    config?: unknown;
  ...
  const params = mocks.loadSteelEnginePlugins.mock.calls[callIndex]?.[0] as
    | {
        config?: unknown;
        activationSourceConfig?: unknown;
        autoEnabledReasons?: unknown;
...
const mocks = vi.hoisted(() => ({
  loadSteelEnginePlugins: vi.fn<typeof import("../plugins/loader.js").loadSteelEnginePlugins>(),
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("uses the resolved runtime load context for configured channel scope", () => {
    ...
    expectLoadSteelEnginePluginsCall(0, {
      config: autoEnabledConfig,
  ...
  it("reloads when escalating from configured-channels to channels", () => {
    ...
    expectLoadSteelEnginePluginsCall(1, {
