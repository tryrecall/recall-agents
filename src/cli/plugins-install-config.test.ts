import { bundledPluginRootAt, repoInstallSpec } from "recall/plugin-sdk/test-fixtures";
import { beforeEach, describe, expect, it, vi } from "vitest";
import type { RecallConfig } from "../config/config.js";
import type { ConfigFileSnapshot } from "../config/types.recall.js";
import {
...
vi.mock("../commands/doctor/shared/channel-doctor.js", () => ({
  collectChannelDoctorStaleConfigMutations: (cfg: RecallConfig) =>
...
function makeSnapshot(overrides: Partial<ConfigFileSnapshot> = {}): ConfigFileSnapshot {
  return {
    ...
    resolved: { plugins: {} } as RecallConfig,
    valid: false,
    runtimeConfig: { plugins: {} } as ConfigFileSnapshot["runtimeConfig"],
    config: { plugins: {} } as RecallConfig,
    hash: "abc",
...
describe("loadConfigForInstall", () => {
  const discordNpmRequest = {
    rawSpec: "@recall/discord",
    normalizedSpec: "@recall/discord",
  ...
  beforeEach(() => {
    ...
    collectChannelDoctorStaleConfigMutationsMock.mockImplementation(async (cfg: RecallConfig) => [
      {
        config: cfg,
        changes: [],
      },
  ...
  it("returns the source config and base hash when the snapshot is valid", async () => {
    const cfg = { plugins: { entries: { discord: { enabled: true } } } } as RecallConfig;
  ...
  it("does not run stale Discord cleanup on the happy path", async () => {
    const cfg = { plugins: {} } as RecallConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        valid: true,
        sourceConfig: cfg,
      ...
      makeSnapshot({
        ...
        issues: [
          ...
          },
          {
            ...
            message:
              'web_search provider is not available: brave (install or enable plugin "brave", then run recall doctor --fix)',
          },
        ],
      }),
    );
  ...
  it("allows explicit repo-checkout bundled-plugin reinstall recovery", async () => {
    const snapshotCfg = { plugins: {} } as RecallConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        config: snapshotCfg,
  ...
  it("rejects non-Discord install requests when config is invalid", async () => {
    ...
    ).rejects.toThrow("Config invalid; run `recall doctor --fix` before installing plugins.");
  });
