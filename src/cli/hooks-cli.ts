...
function buildHooksReport(config: RecallConfig): HookStatusReport {
...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  if (hooks.length === 0) {
    const message = opts.eligible
      ? `No eligible hooks found. Run \`${formatCliCommand("recall hooks list")}\` to see all hooks.`
      : "No hooks found.";
    return message;
  }
...
export function formatHookInfo(
  ...
  if (!hook) {
    ...
    return `Hook "${hookName}" not found. Run \`${formatCliCommand("recall hooks list")}\` to see available hooks.`;
  }
...
export async function enableHook(hookName: string): Promise<void> {
  ...
  const config = (snapshot.sourceConfig ?? snapshot.config) as RecallConfig;
...
export function registerHooksCli(program: Command): void {
  const hooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/hooks", "docs.recall.ai/cli/hooks")}\n`,
    );
  ...
  hooks
    .command("install")
    .description("Deprecated: install a hook pack via `recall plugins install`")
    ...
    .action(async (raw: string, opts: { link?: boolean; pin?: boolean }) => {
      defaultRuntime.log(
        theme.warn("`recall hooks install` is deprecated; use `recall plugins install`."),
      );
  ...
  hooks
    .command("update")
    .description("Deprecated: update hook packs via `recall plugins update`")
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      defaultRuntime.log(
        theme.warn("`recall hooks update` is deprecated; use `recall plugins update`."),
      );
