...
function buildHooksReport(config: SteelEngineConfig): HookStatusReport {
...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  if (hooks.length === 0) {
    const message = opts.eligible
      ? `No eligible hooks found. Run \`${formatCliCommand("steelengine hooks list")}\` to see all hooks.`
      : "No hooks found.";
    return message;
  }
...
export function formatHookInfo(
  ...
  if (!hook) {
    ...
    return `Hook "${hookName}" not found. Run \`${formatCliCommand("steelengine hooks list")}\` to see available hooks.`;
  }
...
async function enableHook(hookName: string): Promise<void> {
  ...
  const config = (snapshot.sourceConfig ?? snapshot.config) as SteelEngineConfig;
...
export function registerHooksCli(program: Command): void {
  const hooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/hooks", "docs.steelengine.ai/cli/hooks")}\n`,
    );
  ...
  hooks
    .command("install")
    .description("Deprecated: install a hook pack via `steelengine plugins install`")
    ...
    .action(async (raw: string, opts: { force?: boolean; link?: boolean; pin?: boolean }) => {
      defaultRuntime.log(
        theme.warn("`steelengine hooks install` is deprecated; use `steelengine plugins install`."),
      );
  ...
  hooks
    .command("update")
    .description("Deprecated: update hook packs via `steelengine plugins update`")
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      defaultRuntime.log(
        theme.warn("`steelengine hooks update` is deprecated; use `steelengine plugins update`."),
      );
