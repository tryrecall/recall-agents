...
import type { SteelEngineConfig, ConfigFileSnapshot } from "../config/types.steelengine.js";
...
const loadInstalledPluginIndexInstallRecords = vi.fn(
  async (params: { config?: SteelEngineConfig } = {}) => params.config?.plugins?.installs ?? {},
);
...
const launchdUpdateCleanupMocks = vi.hoisted(() => ({
  disableCurrentSteelEngineUpdateLaunchdJob: vi.fn(async () => false),
}));
const databasePreflightMocks = vi.hoisted(() => ({
  preflightSteelEngineDatabaseSchemas: vi.fn(),
}));
...
vi.mock("../state/steelengine-database-preflight.js", () => ({
  STEELENGINE_DATABASE_SCHEMA_DOCS_URL: "https://docs.steelengine.ai/reference/database-schemas",
  preflightSteelEngineDatabaseSchemas: databasePreflightMocks.preflightSteelEngineDatabaseSchemas,
}));

vi.mock("../infra/steelengine-root.js", () => ({
  resolveSteelEnginePackageRoot: vi.fn(),
  resolveSteelEnginePackageRootSync: vi.fn(() => process.cwd()),
}));
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/steelengine-config",
...
vi.mock("../daemon/launchd.js", async (importOriginal) => ({
  ...
  disableCurrentSteelEngineUpdateLaunchdJob:
    launchdUpdateCleanupMocks.disableCurrentSteelEngineUpdateLaunchdJob,
}));
...
const { resolveSteelEnginePackageRoot } = await import("../infra/steelengine-root.js");
const {
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/steelengine-update-tests";
  let fixtureCount = 0;
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveSteelEnginePackageRoot).mockResolvedValue(root);
  ...
  const stripSteelEnginePackageAlias = (spec: string) => {
    const trimmed = spec.trim();
    return trimmed.toLowerCase().startsWith("steelengine@")
      ? trimmed.slice("steelengine@".length)
      : trimmed;
  };

  const isNpmGitPackageSpec = (spec: string) => {
    const target = stripSteelEnginePackageAlias(spec);
  ...
  const syncPluginCall = (index = 0) => {
    const calls = syncPluginsForUpdateChannel.mock.calls as unknown as Array<
      [Record<string, unknown> & { channel?: string; config?: SteelEngineConfig }]
    >;
  ...
  const npmPluginUpdateCall = (index = 0) => {
    const calls = updateNpmInstalledPlugins.mock.calls as unknown as Array<
      [Record<string, unknown> & { config?: SteelEngineConfig; timeoutMs?: number }]
    >;
    return calls[index]?.[0];
  };
  ...
  const setupConfigMutationWithRetryMock = () => {
    vi.mocked(mutateConfigFileWithRetry).mockImplementation(async (params) => {
      ...
      const nextConfig = structuredClone(snapshot.sourceConfig) as SteelEngineConfig;
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("steelengine-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("steelengine-updated-root");
    ...
    for (const entrypoint of entrypoints) {
      ...
      fsSync.writeFileSync(
        packageJsonPath,
        JSON.stringify({ name: "steelengine", version: "2026.4.24" }),
        "utf8",
      );
  ...
  beforeEach(() => {
    ...
    databasePreflightMocks.preflightSteelEngineDatabaseSchemas.mockReturnValue({
      incompatible: [],
      indeterminate: [],
    });
    vi.mocked(resolveSteelEnginePackageRoot).mockResolvedValue(process.cwd());
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        if (destination) {
          await fs.writeFile(path.join(destination, "steelengine-9999.0.0.tgz"), "packed\n", "utf8");
        }
      }
      return {
        stdout: "",
    ...
    readPackageName.mockResolvedValue("steelengine");
    ...
    serviceReadCommand.mockImplementation(async () =>
      (await serviceLoaded()) ? { programArguments: ["steelengine", "gateway", "run"] } : null,
    );
    ...
    prepareRestartScript.mockResolvedValue("/tmp/steelengine-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "steelengine-gateway" }],
      hints: [],
    });
    ...
    checkShellCompletionStatus.mockResolvedValue({
      ...
      cacheExists: false,
      cachePath: "/tmp/steelengine-completion.zsh",
    ...
    launchdUpdateCleanupMocks.disableCurrentSteelEngineUpdateLaunchdJob.mockReset();
    launchdUpdateCleanupMocks.disableCurrentSteelEngineUpdateLaunchdJob.mockResolvedValue(false);
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("steelengine-completion-timeout");
    ...
    expect(call?.[1]).toEqual([path.join(root, "steelengine.mjs"), "completion", "--write-state"]);
    expect(call?.[2]?.env?.STEELENGINE_COMPLETION_SKIP_PLUGIN_COMMANDS).toBe("1");
  ...
  it("disarms legacy launchd updater jobs before refusing mutating updates in Nix mode", async () => {
    await withEnvAsync({ STEELENGINE_NIX_MODE: "1" }, async () => {
      await expect(updateCommand({ yes: true })).rejects.toThrow("STEELENGINE_NIX_MODE=1");
    });

    expect(launchdUpdateCleanupMocks.disableCurrentSteelEngineUpdateLaunchdJob).toHaveBeenCalledOnce();
    expect(runGatewayUpdate).not.toHaveBeenCalled();
  ...
  it("delegates mutating updates when an external supervisor owns gateway lifecycle", async () => {
    await withEnvAsync({ STEELENGINE_SUPERVISOR_MODE: "external" }, async () => {
  ...
  it("logs friendly hint with manual refresh command when completion cache write times out", async () => {
    const root = createCaseDir("steelengine-completion-timeout-msg");
    pathExists.mockResolvedValue(true);
    ...
    expect(logOutput).toContain("steelengine completion --write-state");
  ...
  it("keeps update completion refresh best-effort when profile install fails", async () => {
    ...
    expect(installCompletion).toHaveBeenCalledWith("zsh", true, "steelengine");
    const logOutput = vi
      .mocked(runtimeCapture.log)
      .mock.calls.map((call) => String(call[0]))
      .join("\n");
  ...
  it("respawns into the updated package root before running post-update tasks", async () => {
    ...
    expect(call?.[2]?.env?.STEELENGINE_UPDATE_IN_PROGRESS).toBe("1");
    expect(call?.[2]?.env?.STEELENGINE_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.STEELENGINE_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.STEELENGINE_COMPATIBILITY_HOST_VERSION).toBe("1.0.0");
  ...
  it("finishes package updates when the post-core process writes a result but keeps handles open", async () => {
    ...
    spawn.mockImplementationOnce((_command: unknown, _argv: unknown, options: unknown) => {
      const resultPath = (options as { env?: NodeJS.ProcessEnv }).env
        ?.STEELENGINE_UPDATE_POST_CORE_RESULT_PATH;
      if (!resultPath) {
  ...
  it("does not restart a stopped managed gateway after post-core plugin errors", async () => {
    ...
    const root = createCaseDir("steelengine-update");
            ...
            warnings: [
              {
                ...
                guidance: ["Run steelengine update repair to retry post-update plugin repair."],
              },
            ],
            sync: {
              changed: false,
  ...
  it("does not carry gateway service markers into the post-core update process", async () => {
    ...
    expect(spawnEnv?.STEELENGINE_SERVICE_MARKER).toBeUndefined();
    expect(spawnEnv?.STEELENGINE_SERVICE_KIND).toBeUndefined();
  ...
  it("passes pre-update plugin install records into the post-core update process", async () => {
    ...
    const pluginInstallRecords = {
      demo: {
        ...
        installPath: "/tmp/steelengine-demo-plugin",
      },
    } as const;
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      const recordsPath = env?.STEELENGINE_UPDATE_POST_CORE_INSTALL_RECORDS_PATH;
      const sourceConfigPath = env?.STEELENGINE_UPDATE_POST_CORE_SOURCE_CONFIG_PATH;
      if (!recordsPath) {
  ...
  it("clears stale npm resolution metadata before post-core downgrade resume", async () => {
    ...
    const pluginInstallRecords = {
      msteams: {
        source: "npm",
        spec: "@steelengine/msteams",
        installPath: "/tmp/steelengine-msteams-plugin",
        version: "1.0.0",
        resolvedName: "@steelengine/msteams",
        resolvedVersion: "1.0.0",
        resolvedSpec: "@steelengine/msteams@1.0.0",
  ...
  it("respawns into the updated git root before requested channel persistence", async () => {
    ...
    expect(call?.[2]?.env?.STEELENGINE_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.STEELENGINE_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.STEELENGINE_UPDATE_POST_CORE_REQUESTED_CHANNEL).toBe("dev");
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(syncPluginsForUpdateChannel).not.toHaveBeenCalled();
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
  });
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    const downgradedRoot = createCaseDir("steelengine-downgraded-root");
    setupUpdatedRootRefresh({
  ...
  it("pins the compatibility host version to the downgraded target during current-process post-core plugin convergence (#87914)", async () => {
    const downgradedRoot = createCaseDir("steelengine-downgraded-compat-root");
    setupUpdatedRootRefresh({
      gatewayUpdateImpl: async () =>
        makeOkUpdateResult({
          mode: "npm",
    ...
    updateNpmInstalledPlugins.mockImplementation(async () => {
      hostVersionDuringPluginUpdate = process.env.STEELENGINE_COMPATIBILITY_HOST_VERSION;
    ...
    try {
      ...
      expect(process.env.STEELENGINE_COMPATIBILITY_HOST_VERSION).toBeUndefined();
    } finally {
      delete process.env.STEELENGINE_COMPATIBILITY_HOST_VERSION;
    }
  });
    ...
    await withEnvAsync(
      {
        ...
        STEELENGINE_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it("post-core resume children exit after writing a plugin update result", async () => {
    const resultDir = createCaseDir("steelengine-post-core-result");
    ...
    await withEnvAsync(
      {
        STEELENGINE_UPDATE_POST_CORE: "1",
        STEELENGINE_UPDATE_POST_CORE_CHANNEL: "stable",
        STEELENGINE_UPDATE_POST_CORE_RESULT_PATH: resultPath,
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("post-core resume mode uses the parent install records snapshot for missing payload warnings", async () => {
    const resultDir = createCaseDir("steelengine-post-core-records");
    ...
    await withEnvAsync(
      {
        STEELENGINE_UPDATE_POST_CORE: "1",
        STEELENGINE_UPDATE_POST_CORE_CHANNEL: "stable",
        STEELENGINE_UPDATE_POST_CORE_INSTALL_RECORDS_PATH: recordsPath,
      },
      async () => {
  ...
  it("post-core resume mode prefers post-doctor disk install records over the stale parent snapshot", async () => {
    const resultDir = createCaseDir("steelengine-post-core-disk-records");
    const recordsPath = path.join(resultDir, "plugin-install-records.json");
    await fs.mkdir(resultDir, { recursive: true });
    await fs.writeFile(
      recordsPath,
    ...
    const postDoctorRecords = {
      codex: {
        source: "npm",
        spec: "@steelengine/codex@2026.5.17",
  ...
  it("post-core resume mode persists the requested update channel with the updated process", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...
      resolved: { update: { channel: "stable" } } as SteelEngineConfig,
      sourceConfig: { update: { channel: "stable" } } as SteelEngineConfig,
      runtimeConfig: { update: { channel: "stable" } } as SteelEngineConfig,
      config: { update: { channel: "stable" } } as SteelEngineConfig,
      hash: "stable-hash",
    });
    ...
    await withEnvAsync(
      {
        STEELENGINE_UPDATE_POST_CORE: "1",
        STEELENGINE_UPDATE_POST_CORE_CHANNEL: "dev",
        STEELENGINE_UPDATE_POST_CORE_REQUESTED_CHANNEL: "dev",
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("keeps json update output successful when post-core plugin updates warn", async () => {
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @steelengine/demo@1.0.0"
    ...
    expect(pluginWarning(jsonOutput)?.guidance).toEqual([
      ...
      "Run steelengine plugins inspect demo --runtime --json for details.",
    ]);
  ...
  it("does not print duplicate failed ClawHub sync trust warnings in human post-core output", async () => {
    ...
    syncPluginsForUpdateChannel.mockImplementationOnce(
      async (params: { config: SteelEngineConfig; logger?: { warn?: (message: string) => void } }) => {
        params.logger?.warn?.(trustWarning);
        return {
          changed: false,
          config: params.config,
  ...
  it("does not print duplicate ClawHub update trust warnings in human post-core output", async () => {
    ...
    expect(output).toContain("Run steelengine update repair to retry post-update plugin repair.");
    expect(output).toContain("Run steelengine plugins inspect demo --runtime --json for details.");
  });

  it("detects missing plugin payloads from persisted records before npm updates", async () => {
    const installPath = createCaseDir("steelengine-missing-plugin-payload");
  ...
  it("prints non-fatal plugin warnings in human update output", async () => {
    ...
    expect(logs).toContain("Run steelengine update repair to retry post-update plugin repair.");
    expect(logs).toContain("Run steelengine plugins inspect demo --runtime --json for details.");
  });
      ...
      outcomes: [
        {
          ...
          status: "skipped",
          message:
            'Disabled "demo" after plugin update failure; SteelEngine will continue without it. Failed to update demo: registry timeout',
        },
      ],
    });
    vi.mocked(defaultRuntime.writeJson).mockClear();
  ...
  it("marks blocked ClawHub update skips as post-update warnings", async () => {
    ...
    expect(pluginOutcome(jsonOutput)?.pluginId).toBe("demo");
    expect(pluginOutcome(jsonOutput)?.status).toBe("skipped");
    expect(pluginOutcome(jsonOutput)?.message).toContain("Run steelengine update repair");
  });
  ...
  it("preserves fresh-process plugin warning details in parent json output", async () => {
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      queueMicrotask(() => {
        void (async () => {
          const resultPath = env?.STEELENGINE_UPDATE_POST_CORE_RESULT_PATH;
          if (resultPath) {
            await fs.writeFile(
              resultPath,
              JSON.stringify({
                ...
                warnings: [
                  {
                    ...
                    message:
                      'Plugin "demo" could not be processed after the core update: Failed to update demo: registry timeout Run steelengine update repair to retry post-update plugin repair. Run steelengine plugins inspect demo --runtime --json for details.',
                    guidance: [
                      "Run steelengine update repair to retry post-update plugin repair.",
                      "Run steelengine plugins inspect demo --runtime --json for details.",
                    ],
  ...
  it("refuses an incompatible package target before service stop or install", async () => {
    mockPackageInstallStatus(createCaseDir("steelengine-schema-refusal"));
    ...
    databasePreflightMocks.preflightSteelEngineDatabaseSchemas.mockReturnValue({
      incompatible: [
        {
          kind: "agent",
          path: "/tmp/steelengine/agents/main/agent/steelengine-agent.sqlite",
          agentId: "main",
          foundVersion: 11,
          supportedVersion: 9,
    ...
    expect(databasePreflightMocks.preflightSteelEngineDatabaseSchemas).toHaveBeenCalledWith({
      env: process.env,
  ...
  it("skips package schema preflight when target metadata is missing", async () => {
    mockPackageInstallStatus(createCaseDir("steelengine-schema-missing"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "9999.0.0",
      version: "9999.0.0",
      nodeEngine: ">=22.19.0",
    ...
    expect(databasePreflightMocks.preflightSteelEngineDatabaseSchemas).not.toHaveBeenCalled();
    expect(packageInstallCommandCall()).toBeDefined();
  });

  it("refuses a package update when exact target metadata lookup fails", async () => {
    mockPackageInstallStatus(createCaseDir("steelengine-schema-metadata-failure"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "9999.0.0",
      version: null,
      nodeEngine: null,
    ...
    expect(packageInstallCommandCall()).toBeUndefined();
    expect(defaultRuntime.error).toHaveBeenCalledWith(
      expect.stringContaining("could not inspect exact package target steelengine@9999.0.0"),
    );
    expect(defaultRuntime.exit).toHaveBeenCalledWith(1);
  });

  it("continues a package update when target schemas are compatible", async () => {
    mockPackageInstallStatus(createCaseDir("steelengine-schema-compatible"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "9999.0.0",
      version: "9999.0.0",
      nodeEngine: ">=22.19.0",
    ...
    expect(databasePreflightMocks.preflightSteelEngineDatabaseSchemas).toHaveBeenCalledTimes(2);
    expect(packageInstallCommandCall()?.[0]).toContain("steelengine@9999.0.0");
  });

  it("reports an incompatible package target during dry-run", async () => {
    mockPackageInstallStatus(createCaseDir("steelengine-schema-dry-run"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "9999.0.0",
      version: "9999.0.0",
      nodeEngine: ">=22.19.0",
    ...
    databasePreflightMocks.preflightSteelEngineDatabaseSchemas.mockReturnValue({
      incompatible: [
        {
          kind: "state",
          path: "/tmp/steelengine/state/steelengine.sqlite",
          foundVersion: 3,
          supportedVersion: 2,
        },
      ],
    ...
    expect(logs).toContain("https://docs.steelengine.ai/reference/database-schemas");
    expect(serviceStop).not.toHaveBeenCalled();
    expect(packageInstallCommandCall()).toBeUndefined();
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
  });
  ...
  it("reports indeterminate package databases during dry-run", async () => {
    mockPackageInstallStatus(createCaseDir("steelengine-schema-indeterminate-dry-run"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "9999.0.0",
      version: "9999.0.0",
      nodeEngine: ">=22.19.0",
    ...
    databasePreflightMocks.preflightSteelEngineDatabaseSchemas.mockReturnValue({
      incompatible: [],
      indeterminate: [
        { kind: "state", path: "/tmp/steelengine/state/steelengine.sqlite", reason: "database busy" },
      ],
    });
    ...
    expect(logs).toContain(
      "could not inspect state database /tmp/steelengine/state/steelengine.sqlite: database busy; retry once the gateway releases it",
    );
  });
  ...
  it("refuses a package target that changes after the service stops", async () => {
    ...
    serviceReadCommand.mockResolvedValue({
      ...
      environment: { STEELENGINE_STATE_DIR: "/managed/state" },
    });
    serviceLoaded.mockResolvedValue(true);
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "9999.0.0",
    ...
    databasePreflightMocks.preflightSteelEngineDatabaseSchemas
    ...
    expect(databasePreflightMocks.preflightSteelEngineDatabaseSchemas.mock.calls[1]?.[0].env).toEqual(
      expect.objectContaining({ STEELENGINE_STATE_DIR: "/managed/state" }),
    );
    expect(packageInstallCommandCall()).toBeUndefined();
  ...
  it("refuses a git target that changes after the service stops", async () => {
    ...
    databasePreflightMocks.preflightSteelEngineDatabaseSchemas
      ...
      .mockReturnValueOnce({
        incompatible: [],
        indeterminate: [
          { kind: "agent", path: "/tmp/steelengine-agent.sqlite", reason: "database busy" },
        ],
      });
  ...
  it("fails a post-stop git refusal when no managed service was running", async () => {
    ...
    databasePreflightMocks.preflightSteelEngineDatabaseSchemas
      ...
      .mockReturnValueOnce({
        incompatible: [],
        indeterminate: [{ kind: "state", path: "/tmp/steelengine.sqlite", reason: "database busy" }],
      });
  ...
  it.each([
    {
      ...
      assert: () => {
        ...
        expect(logs.join("\n")).toContain("SteelEngine update status");
      },
    },
    {
      name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "steelengine", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as SteelEngineConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expectPackageInstallSpec("steelengine@9999.0.0");
      }
  ...
  it("falls back to latest when beta tag is older than release", async () => {
    ...
    expectPackageInstallSpec("steelengine@1.2.3-1");
  });
  ...
  it("installs the verified exact package and persists an explicit extended-stable channel", async () => {
    ...
    expectPackageInstallSpec("steelengine@2026.6.33");
  ...
  it("uses the same exact resolver for a bare update with stored extended-stable", async () => {
    ...
    mockPackageInstallStatus(tempDir);
    readPackageVersion.mockResolvedValue("2026.6.33");
    const config = { update: { channel: "extended-stable" } } as SteelEngineConfig;
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...baseSnapshot,
      parsed: config,
      sourceConfig: config,
  ...
  it("fails closed without config or package mutation when extended-stable resolution fails", async () => {
    ...
    expect(packageInstallCommandCall()).toBeUndefined();
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(launchdUpdateCleanupMocks.disableCurrentSteelEngineUpdateLaunchdJob).not.toHaveBeenCalled();
      ...
      warnings: [
        {
          ...
          guidance: ["Run steelengine update repair."],
        },
      ],
      errored: true,
      smokeFailures: [],
  ...
  it("runs the package update when latest target lookup is unresolved", async () => {
    ...
    expectPackageInstallSpec("steelengine@latest");
  });
  ...
  it("allows package updates from inherited gateway service env when the managed gateway is not running", async () => {
    mockPackageInstallStatus(createCaseDir("steelengine-update"));
    ...
    expect(defaultRuntime.error).not.toHaveBeenCalledWith(
      [
        ...
        "That path replaces the active SteelEngine dist tree while the live gateway may still lazy-load old chunks.",
        "Run `steelengine update` from a shell outside the gateway service, or stop the gateway service first and then update.",
      ].join("\n"),
    );
    expectPackageInstallSpec("steelengine@9999.0.0");
  });
  ...
  it("refuses package updates from inside the active gateway process tree", async () => {
    ...
    const errors = vi.mocked(defaultRuntime.error).mock.calls.map((call) => String(call[0]));
    expect(errors.join("\n")).toContain(
      "steelengine update detected it is running inside the gateway process tree.",
    );
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g steelengine` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    {
      ...
      expectedSpec: "steelengine@9999.0.0",
    },
    {
      name: "main shorthand",
      run: async () => {
      ...
      expectedSpec: "github:steelengine/steelengine#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("steelengine-update"));
        await updateCommand({ yes: true, tag: "github:steelengine/steelengine#main" });
      },
      expectedSpec: "github:steelengine/steelengine#main",
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("steelengine-update"));
        await updateCommand({ yes: true, tag: "SteelEngine@github:steelengine/steelengine#main" });
      },
      expectedSpec: "SteelEngine@github:steelengine/steelengine#main",
    },
    {
      name: "full git URL package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("steelengine-update"));
        await updateCommand({ yes: true, tag: "https://github.com/steelengineai/recall-agents.git#main" });
      },
      expectedSpec: "https://github.com/steelengineai/recall-agents.git#main",
    },
    {
      name: "hosted GitHub URL package spec without git suffix",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("steelengine-update"));
        await updateCommand({ yes: true, tag: "https://github.com/steelengineai/recall-agents#main" });
      },
      expectedSpec: "https://github.com/steelengineai/recall-agents#main",
    },
    {
      ...
      run: async () => {
        ...
        await updateCommand({
          yes: true,
          tag: "steelengine@https://github.com/steelengineai/recall-agents#main",
        });
      },
      expectedSpec: "https://github.com/steelengineai/recall-agents#main",
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("steelengine-update"));
        await updateCommand({ yes: true, tag: "steelengine/steelengine#main" });
      },
      expectedSpec: "steelengine/steelengine#main",
    },
    {
      name: "SCP-style SSH package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("steelengine-update"));
        await updateCommand({ yes: true, tag: "git@github.com:steelengineai/recall-agents.git#main" });
      },
      expectedSpec: "git@github.com:steelengineai/recall-agents.git#main",
    },
    {
      name: "STEELENGINE_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("steelengine-update"));
        await withEnvAsync(
      ...
      expectedSpec: "http://10.211.55.2:8138/steelengine-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "steelengine", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("stops package post-update work when staged npm install verification fails", async () => {
    const tempDir = await createTrackedTempDir("steelengine-update-staged-fail-");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "steelengine", version: "2026.4.20" }),
      "utf-8",
    );
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        const stageRoot = path.join(stagePrefix, "lib", "node_modules", "steelengine");
        await fs.mkdir(path.join(stageRoot, "dist"), { recursive: true });
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "steelengine", version: "2026.4.25" }),
          "utf-8",
        );
  ...
  it("runs old package doctors without fix mode when service ownership is unknown", async () => {
    const tempDir = await createTrackedTempDir("steelengine-update-package-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "steelengine");
    ...
    await writePackageDistInventory(pkgRoot);
    serviceReadCommand.mockResolvedValue({
      programArguments: ["steelengine-wrapper", "gateway", "run"],
    });
    serviceLoaded.mockResolvedValue(true);
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.STEELENGINE_UPDATE_IN_PROGRESS,
    ).toBe("1");
    expect(
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.STEELENGINE_SERVICE_REPAIR_POLICY,
    ).toBeUndefined();
  ...
  it("continues package post-core work for explicit post-update doctor advisories", async () => {
    const tempDir = await createTrackedTempDir("steelengine-update-package-doctor-warning-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "steelengine");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    await withEnvAsync({ STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION: "1" }, async () => {
    ...
    expect(postCoreCall?.[2]?.env?.STEELENGINE_UPDATE_POST_CORE).toBe("1");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
    const jsonOutput = lastWriteJsonCall() as UpdateRunResult | undefined;
    const doctorStep = jsonOutput?.steps.find((step) => step.name === "steelengine doctor");
    expect(jsonOutput?.status).toBe("ok");
  ...
  it("fails package updates when the post-update doctor is killed after verification", async () => {
    const tempDir = await createTrackedTempDir("steelengine-update-package-doctor-timeout-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "steelengine");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
  ...
  it("runs package post-update doctor from the verified package root after a staged swap", async () => {
    const tempDir = await createTrackedTempDir("steelengine-update-staged-doctor-");
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (argv[0] === "npm" && argv[1] === "i" && argv.includes("--prefix")) {
        ...
        await fs.writeFile(
          ...
          JSON.stringify({ name: "steelengine", version: "2026.5.14" }),
          "utf-8",
        );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.STEELENGINE_COMPATIBILITY_HOST_VERSION,
    ).toBe("2026.5.14");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
  });
  ...
  it("stops a running managed gateway before package replacement", async () => {
    ...
    suspendScheduledTaskAutoStartForUpdate.mockResolvedValue(true);
    resumeScheduledTaskAutoStartAfterUpdate.mockResolvedValue(true);
    const tempDir = await createTrackedTempDir("steelengine-update-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "steelengine");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    const serviceStopCall = serviceStop.mock.calls[0]?.[0] as
      ...
      | undefined;
    expect(serviceStopCall?.env?.STEELENGINE_SERVICE_MARKER).toBe("steelengine");
    expect(serviceStopCall?.env?.STEELENGINE_SERVICE_KIND).toBe("gateway");
  ...
  it("restores Windows Scheduled Task autostart when service stop fails", async () => {
    const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    mockPackageInstallStatus(createCaseDir("steelengine-update-stop-failure"));
    serviceReadCommand.mockResolvedValue({
      programArguments: ["steelengine", "gateway", "run"],
      environment: {
        STEELENGINE_SERVICE_MARKER: "steelengine",
  ...
  it("preserves both the update and Scheduled Task recovery failures", async () => {
    const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    mockPackageInstallStatus(createCaseDir("steelengine-update-recovery-failure"));
    serviceReadCommand.mockResolvedValue({
      programArguments: ["steelengine", "gateway", "run"],
      environment: {
        STEELENGINE_SERVICE_MARKER: "steelengine",
  ...
  it.each(["SIGINT", "SIGBREAK"] as const)(
    ...
    async (signal) => {
      ...
      resumeScheduledTaskAutoStartAfterUpdate.mockResolvedValue(true);
      mockPackageInstallStatus(createCaseDir("steelengine-update-suspension-signal"));
      serviceReadCommand.mockResolvedValue({
        programArguments: ["steelengine", "gateway", "run"],
        environment: {
          STEELENGINE_SERVICE_MARKER: "steelengine",
  ...
  it.each(["running", "stopped"] as const)(
    ...
    async (runtimeStatus) => {
      const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
      mockPackageInstallStatus(createCaseDir("steelengine-update-stopped-task"));
      serviceReadCommand.mockResolvedValue({
        programArguments: ["steelengine", "gateway", "run"],
        environment: {
          STEELENGINE_SERVICE_MARKER: "steelengine",
  ...
  it("stops a running managed git gateway when wrapper commands hide the service root", async () => {
    const wrapperPath = path.join(
      createCaseDir("steelengine-update-wrapper-service"),
      "gateway-wrapper",
    );
    serviceReadCommand.mockResolvedValue({
  ...
  it("stops a managed gateway rooted at the git checkout when switching package installs to dev", async () => {
    const packageRoot = createCaseDir("steelengine-update-package-root");
    const gitRoot = await createTrackedTempDir("steelengine-update-git-service-root-");
    ...
    await withEnvAsync({ STEELENGINE_GIT_DIR: gitRoot }, async () => {
  ...
  it("stops a managed gateway rooted at the package install when switching package installs to dev", async () => {
    const packageRoot = await createTrackedTempDir("steelengine-update-package-service-root-");
  ...
  it("does not stop or restart a managed gateway owned by another git checkout", async () => {
    const otherRoot = await createTrackedTempDir("steelengine-update-other-service-root-");
  ...
  it("keeps managed service stop output off stdout during json package updates", async () => {
    const tempDir = await createTrackedTempDir("steelengine-update-json-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "steelengine");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    ...
    serviceStop.mockImplementationOnce(async (params: { stdout?: NodeJS.WritableStream }) => {
      params.stdout?.write("Stopped systemd service: steelengine-gateway.service\n");
    });
    pathExists.mockImplementation(async (candidate: string) => {
      try {
        await fs.access(candidate);
  ...
  it("disarms legacy launchd updater jobs before stopping the gateway", async () => {
    const tempDir = await createTrackedTempDir("steelengine-update-launchd-loop-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "steelengine");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    launchdUpdateCleanupMocks.disableCurrentSteelEngineUpdateLaunchdJob.mockResolvedValue(true);
    pathExists.mockImplementation(async (candidate: string) => {
      try {
        await fs.access(candidate);
        return true;
    ...
    const cleanupOrder =
      launchdUpdateCleanupMocks.disableCurrentSteelEngineUpdateLaunchdJob.mock.invocationCallOrder[0];
  ...
  it("refreshes package installs even when the current version already matches the target", async () => {
    const tempDir = await createTrackedTempDir("steelengine-update-current-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "steelengine");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "steelengine", version: "2026.4.23" }),
      "utf-8",
    );
    await fs.writeFile(entryPath, "export {};\n", "utf-8");
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
    ...
    expectPackageInstallSpec("steelengine@2026.4.23");
    const doctorCall = doctorCommandCall();
    expect(doctorCall?.[0][0]).toContain("node");
    expect(doctorCall?.[0].slice(1)).toEqual([entryPath, "doctor", "--non-interactive"]);
    ...
    expect(postCoreSpawn?.[2].env?.STEELENGINE_UPDATE_POST_CORE).toBe("1");
    expect(postCoreSpawn?.[2].env?.STEELENGINE_UPDATE_POST_CORE_CHANNEL).toBe("stable");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(
      vi
        .mocked(defaultRuntime.log)
  ...
  it("retries package updates without optional deps when npm global update fails", async () => {
    const tempDir = await createTrackedTempDir("steelengine-update-optional-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "steelengine");
    mockPackageInstallStatus(pkgRoot);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "steelengine", version: "9999.0.0" }),
      "utf-8",
    );
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
      const absolutePath = path.join(pkgRoot, relativePath);
    ...
    const installCall = vi
      ...
      .mock.calls.find(
        ([argv]) =>
          ...
          argv[1] === "i" &&
          argv[2] === "-g" &&
          argv.includes("steelengine@9999.0.0"),
      );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    const tempDir = createCaseDir("steelengine-update");
    const localAppData = createCaseDir("steelengine-localappdata");
  ...
  it("warns when a package update targets a managed service root outside the shell root", async () => {
    const shellRoot = createCaseDir("steelengine-shell-root");
    const serviceRoot = await createTrackedTempDir("steelengine-service-root-");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "steelengine", version: "2026.5.18" }),
      "utf-8",
    );
    ...
    expect(logs).toContain(
      `Shell SteelEngine root differs from the managed gateway service root: ${shellRoot}`,
    );
    expect(logs).toContain("make sure `steelengine` on PATH resolves to the managed service root");
  ...
  it("runs managed service package follow-up commands with the service Node", async () => {
    const shellRoot = createCaseDir("steelengine-shell-root");
    const servicePrefix = await createTrackedTempDir("steelengine-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const serviceRoot = path.join(nodeModules, "steelengine");
  ...
  it("uses the managed service Node when package roots match but node binaries differ", async () => {
    const root = createCaseDir("steelengine-same-root");
  ...
  it("refreshes the managed service to current Node when its baked Node cannot run the target", async () => {
    const servicePrefix = await createTrackedTempDir("steelengine-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "steelengine");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const serviceNpm = path.join(servicePrefix, "bin", "npm");
    const entrypoint = path.join(root, "dist", "index.js");
    await fs.mkdir(path.dirname(entrypoint), { recursive: true });
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "steelengine", version: "2026.7.1" }),
          "utf-8",
        );
        await fs.writeFile(stageEntryPoint, "export {};\n", "utf-8");
        await writePackageDistInventory(stageRoot);
  ...
  it("pins package install to the service root when nodes differ and no owning npm exists at the prefix", async () => {
    const servicePrefix = await createTrackedTempDir("steelengine-no-npm-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "steelengine");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const entrypoint = path.join(root, "dist", "index.js");
  ...
  it("refreshes post-doctor config before post-update plugin sync", async () => {
    const tempDir = createCaseDir("steelengine-update");
    mockPackageInstallStatus(tempDir);
    const preUpdateConfig = { update: { channel: "stable" } } as SteelEngineConfig;
    ...
    syncPluginsForUpdateChannel.mockImplementation(async ({ config }) => ({
      ...
      config: {
        ...
        plugins: {
          ...config.plugins,
          load: { paths: ["/tmp/steelengine-updated-plugin"] },
        },
      },
      summary: {
        switchedToBundled: [],
    ...
    const syncConfig = syncPluginCall()?.config as
      | (SteelEngineConfig & { meta?: { lastTouchedVersion?: string } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          baseHash?: string;
          nextConfig?: SteelEngineConfig & { meta?: { lastTouchedVersion?: string } };
        }
      | undefined;
    ...
    await withEnvAsync(
      {
        STEELENGINE_UPDATE_POST_CORE: "1",
        STEELENGINE_UPDATE_POST_CORE_CHANNEL: "stable",
        STEELENGINE_UPDATE_POST_CORE_STARTED_AT_MS: String(updateStartedAtMs),
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
  ...
  it("persists authored channel values when post-core restore input is resolved", async () => {
    ...
    await withEnvAsync(
      {
        STEELENGINE_UPDATE_POST_CORE: "1",
        STEELENGINE_UPDATE_POST_CORE_CHANNEL: "stable",
        STEELENGINE_UPDATE_POST_CORE_SOURCE_CONFIG_PATH: sourceConfigPath,
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
    ...
    const syncConfig = syncPluginCall()?.config as
      | (SteelEngineConfig & { channels?: { whatsapp?: { token?: string } } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          nextConfig?: SteelEngineConfig & {
  ...
  it("persists channel and runs post-update work after switching from package to git", async () => {
    const tempDir = createCaseDir("steelengine-update");
    const gitRoot = path.join(tempDir, "..", "steelengine");
    ...
    const syncCall = syncPluginCall() as
      | { channel?: string; config?: SteelEngineConfig; workspaceDir?: string }
      | undefined;
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `steelengine update`.",
    );
    expect(serviceStop).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).toHaveBeenCalledWith(0);
  });
  ...
  it("restores an unknown package service without rewriting its missing updated entrypoint", async () => {
    ...
    expect(runDaemonInstall).not.toHaveBeenCalled();
    expect(runRestartScript).toHaveBeenCalledWith("/tmp/steelengine-restart-test.sh");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
  });

  it("tries the updated install restart when package service refresh fails", async () => {
    const updatedRoot = createCaseDir("steelengine-updated-root");
  ...
  it("restores a same-version service without rewriting when its root is ambiguous", async () => {
    const oldRoot = createCaseDir("steelengine-old-root");
    const updatedRoot = createCaseDir("steelengine-updated-root");
  ...
  it("writes the control-plane update sentinel after managed package restart health passes", async () => {
    const stateDir = await createTrackedTempDir("steelengine-update-sentinel-state-");
    const metaDir = await createTrackedTempDir("steelengine-update-sentinel-meta-");
  ...
  it.each([
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            STEELENGINE_CONFIG_PATH: "./config/steelengine.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedEnv: () => ({
        STEELENGINE_STATE_DIR: path.resolve("./state"),
        STEELENGINE_CONFIG_PATH: path.resolve("./config/steelengine.json"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
    },
    {
      ...
      expectedEnv: (context?: { originalCwd: string }) => ({
        STEELENGINE_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ STEELENGINE_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.STEELENGINE_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("marks the whole update command as update-in-progress", async () => {
    await withEnvAsync({ STEELENGINE_UPDATE_IN_PROGRESS: undefined }, async () => {
      ...
      vi.mocked(runGatewayUpdate).mockImplementationOnce(async () => {
        observedUpdateEnv = process.env.STEELENGINE_UPDATE_IN_PROGRESS;
  ...
  it("updateFinalizeCommand runs doctor and plugin convergence with full update env", async () => {
    await withEnvAsync(
      {
        STEELENGINE_UPDATE_IN_PROGRESS: undefined,
        STEELENGINE_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR: undefined,
        STEELENGINE_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE: undefined,
      },
      async () => {
        ...
        expect(process.env.STEELENGINE_UPDATE_IN_PROGRESS).toBeUndefined();
        expect(process.env.STEELENGINE_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBeUndefined();
        expect(process.env.STEELENGINE_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBeUndefined();
  ...
  it("updateFinalizeCommand repairs doctor by default and refreshes plugin state after doctor", async () => {
    ...
    loadInstalledPluginIndexInstallRecords.mockResolvedValueOnce(postDoctorRecords);
    syncPluginsForUpdateChannel.mockImplementationOnce(
      async (params: { config?: SteelEngineConfig }) => ({
        changed: true,
  ...
  it("updateFinalizeCommand restores channels from the RPC pre-update config payload", async () => {
    const tempDir = createCaseDir("steelengine-rpc-finalize");
    const sourceConfigPath = path.join(tempDir, "source-config.json");
    const preUpdateConfig = {
      channels: {
        whatsapp: {
  ...
  it("updateFinalizeCommand reapplies requested channel against post-doctor config", async () => {
    const preDoctorConfig = { update: { channel: "stable" } } as SteelEngineConfig;
    const postDoctorConfig = { update: { channel: "beta" } } as SteelEngineConfig;
    const preDoctorSnapshot: ConfigFileSnapshot = {
      ...baseSnapshot,
      sourceConfig: preDoctorConfig,
      resolved: preDoctorConfig,
  ...
  it("updateFinalizeCommand converges on the effective channel from env without persisting update.channel", async () => {
    const noChannelConfig = {} as SteelEngineConfig;
    ...
    const priorEffective = process.env.STEELENGINE_UPDATE_EFFECTIVE_CHANNEL;
    // Simulate a no-config git/source update whose effective channel is dev.
    process.env.STEELENGINE_UPDATE_EFFECTIVE_CHANNEL = "dev";
    try {
      await updateFinalizeCommand({ json: true, restart: false });
    } finally {
      if (priorEffective === undefined) {
        delete process.env.STEELENGINE_UPDATE_EFFECTIVE_CHANNEL;
      } else {
        process.env.STEELENGINE_UPDATE_EFFECTIVE_CHANNEL = priorEffective;
      }
    }
  ...
  it.each([
    ...
    },
    {
      ...
      requireTty: false,
      expectedError:
        "Update wizard requires a TTY. Use `steelengine update --channel <stable|extended-stable|beta|dev>` instead.",
    },
  ] as const)(
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("steelengine-update-wizard");
    await withEnvAsync({ STEELENGINE_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/steelengine as the default dev checkout directory", async () => {
    ...
    try {
      await withEnvAsync(
        ...
        async () => {
          expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "steelengine"));
        },
      );
    } finally {
  ...
  it("uses STEELENGINE_HOME for the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    try {
      await withEnvAsync(
        { STEELENGINE_GIT_DIR: undefined, STEELENGINE_HOME: "/srv/steelengine-home" },
        async () => {
          expect(resolveGitInstallDir()).toBe(path.posix.join("/srv/steelengine-home", "steelengine"));
        },
      );
    } finally {
      homedirSpy.mockRestore();
  ...
  it("creates the parent directory before cloning the default dev checkout", async () => {
    const root = await createTrackedTempDir("steelengine-update-home-");
    const home = path.join(root, "custom-steelengine-home");
    const checkoutDir = path.join(home, "steelengine");

    await withEnvAsync({ STEELENGINE_GIT_DIR: undefined, STEELENGINE_HOME: home }, async () => {
    ...
    expect(cloneCall?.[0]).toEqual([
      "git",
      "clone",
      "https://github.com/steelengineai/recall-agents.git",
      checkoutDir,
    ]);
  });
});
