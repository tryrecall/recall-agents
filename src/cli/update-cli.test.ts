...
import type { RecallConfig, ConfigFileSnapshot } from "../config/types.recall.js";
...
const loadInstalledPluginIndexInstallRecords = vi.fn(
  async (params: { config?: RecallConfig } = {}) => params.config?.plugins?.installs ?? {},
);
...
const launchdUpdateCleanupMocks = vi.hoisted(() => ({
  disableCurrentRecallUpdateLaunchdJob: vi.fn(async () => false),
}));
...
vi.mock("../infra/recall-root.js", () => ({
  resolveRecallPackageRoot: vi.fn(),
  resolveRecallPackageRootSync: vi.fn(() => process.cwd()),
}));

vi.mock("../config/config.js", () => ({
  assertConfigWriteAllowedInCurrentMode: () => {
    if (process.env.RECALL_NIX_MODE === "1") {
      throw new Error(
        [
          ...
          "Recall Nix overview: https://docs.recall.ai/install/nix",
        ].join("\n"),
      );
    }
  },
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/recall-config",
  };
});
...
vi.mock("../daemon/launchd.js", async (importOriginal) => ({
  ...
  disableCurrentRecallUpdateLaunchdJob:
    launchdUpdateCleanupMocks.disableCurrentRecallUpdateLaunchdJob,
}));
...
const { resolveRecallPackageRoot } = await import("../infra/recall-root.js");
const {
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/recall-update-tests";
  let fixtureCount = 0;
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveRecallPackageRoot).mockResolvedValue(root);
  ...
  const stripRecallPackageAlias = (spec: string) => {
    const trimmed = spec.trim();
    return trimmed.toLowerCase().startsWith("recall@")
      ? trimmed.slice("recall@".length)
      : trimmed;
  };

  const isNpmGitPackageSpec = (spec: string) => {
    const target = stripRecallPackageAlias(spec);
  ...
  const syncPluginCall = (index = 0) => {
    const calls = syncPluginsForUpdateChannel.mock.calls as unknown as Array<
      [{ channel?: string; config?: RecallConfig }]
    >;
  ...
  const npmPluginUpdateCall = (index = 0) => {
    const calls = updateNpmInstalledPlugins.mock.calls as unknown as Array<
      [{ config?: RecallConfig; timeoutMs?: number }]
    >;
    return calls[index]?.[0];
  };
  ...
  const setupConfigMutationWithRetryMock = () => {
    vi.mocked(mutateConfigFileWithRetry).mockImplementation(async (params) => {
      ...
      const nextConfig = structuredClone(snapshot.sourceConfig) as RecallConfig;
  ...
  const expectPackageInstallSpec = (spec: string) => {
    ...
    if (isNpmGitPackageSpec(spec)) {
      ...
      installSpec = path.join(packDir, "recall-9999.0.0.tgz");
    } else {
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("recall-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("recall-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveRecallPackageRoot).mockResolvedValue(process.cwd());
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        if (destination) {
          await fs.writeFile(path.join(destination, "recall-9999.0.0.tgz"), "packed\n", "utf8");
        }
      }
      return {
        stdout: "",
    ...
    readPackageName.mockResolvedValue("recall");
    ...
    serviceReadCommand.mockImplementation(async () =>
      (await serviceLoaded()) ? { programArguments: ["recall", "gateway", "run"] } : null,
    );
    ...
    prepareRestartScript.mockResolvedValue("/tmp/recall-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "recall-gateway" }],
      hints: [],
    });
    ...
    launchdUpdateCleanupMocks.disableCurrentRecallUpdateLaunchdJob.mockReset();
    launchdUpdateCleanupMocks.disableCurrentRecallUpdateLaunchdJob.mockResolvedValue(false);
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("recall-completion-timeout");
    ...
    expect(call?.[1]).toEqual([path.join(root, "recall.mjs"), "completion", "--write-state"]);
    expect(call?.[2]?.env?.RECALL_COMPLETION_SKIP_PLUGIN_COMMANDS).toBe("1");
  ...
  it("disarms legacy launchd updater jobs before refusing mutating updates in Nix mode", async () => {
    await withEnvAsync({ RECALL_NIX_MODE: "1" }, async () => {
      await expect(updateCommand({ yes: true })).rejects.toThrow("RECALL_NIX_MODE=1");
    });

    expect(launchdUpdateCleanupMocks.disableCurrentRecallUpdateLaunchdJob).toHaveBeenCalledOnce();
    expect(runGatewayUpdate).not.toHaveBeenCalled();
  ...
  it("logs friendly hint with manual refresh command when completion cache write times out", async () => {
    const root = createCaseDir("recall-completion-timeout-msg");
    pathExists.mockResolvedValue(true);
    ...
    expect(logOutput).toContain("recall completion --write-state");
  ...
  it("respawns into the updated package root before running post-update tasks", async () => {
    ...
    expect(call?.[2]?.env?.RECALL_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.RECALL_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.RECALL_COMPATIBILITY_HOST_VERSION).toBe("1.0.0");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
  ...
  it("finishes package updates when the post-core process writes a result but keeps handles open", async () => {
    ...
    spawn.mockImplementationOnce((_command: unknown, _argv: unknown, options: unknown) => {
      const resultPath = (options as { env?: NodeJS.ProcessEnv }).env
        ?.RECALL_UPDATE_POST_CORE_RESULT_PATH;
      if (!resultPath) {
  ...
  it("does not restart a stopped managed gateway after post-core plugin errors", async () => {
    const root = createCaseDir("recall-update");
            ...
            warnings: [
              {
                ...
                guidance: ["Run recall doctor --fix to attempt automatic repair."],
              },
            ],
            sync: {
              changed: false,
  ...
  it("does not carry gateway service markers into the post-core update process", async () => {
    ...
    await withEnvAsync(
      {
        RECALL_SERVICE_MARKER: "recall",
        RECALL_SERVICE_KIND: "gateway",
    ...
    expect(spawnEnv?.RECALL_SERVICE_MARKER).toBeUndefined();
    expect(spawnEnv?.RECALL_SERVICE_KIND).toBeUndefined();
  ...
  it("passes pre-update plugin install records into the post-core update process", async () => {
    ...
    const pluginInstallRecords = {
      demo: {
        source: "npm",
        spec: "@recall/demo@1.0.0",
        installPath: "/tmp/recall-demo-plugin",
      },
    } as const;
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      const recordsPath = env?.RECALL_UPDATE_POST_CORE_INSTALL_RECORDS_PATH;
      const sourceConfigPath = env?.RECALL_UPDATE_POST_CORE_SOURCE_CONFIG_PATH;
      if (!recordsPath) {
  ...
  it("respawns into the updated git root before requested channel persistence", async () => {
    ...
    expect(call?.[2]?.env?.RECALL_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.RECALL_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.RECALL_UPDATE_POST_CORE_REQUESTED_CHANNEL).toBe("dev");
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(syncPluginsForUpdateChannel).not.toHaveBeenCalled();
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
  });
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    const downgradedRoot = createCaseDir("recall-downgraded-root");
    setupUpdatedRootRefresh({
    ...
    await withEnvAsync(
      {
        ...
        RECALL_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it("post-core resume children exit after writing a plugin update result", async () => {
    const resultDir = createCaseDir("recall-post-core-result");
    ...
    await withEnvAsync(
      {
        RECALL_UPDATE_POST_CORE: "1",
        RECALL_UPDATE_POST_CORE_CHANNEL: "stable",
        RECALL_UPDATE_POST_CORE_RESULT_PATH: resultPath,
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("post-core resume mode uses the parent install records snapshot for missing payload warnings", async () => {
    const resultDir = createCaseDir("recall-post-core-records");
    ...
    await withEnvAsync(
      {
        RECALL_UPDATE_POST_CORE: "1",
        RECALL_UPDATE_POST_CORE_CHANNEL: "stable",
        RECALL_UPDATE_POST_CORE_INSTALL_RECORDS_PATH: recordsPath,
      },
      async () => {
  ...
  it("post-core resume mode prefers post-doctor disk install records over the stale parent snapshot", async () => {
    const resultDir = createCaseDir("recall-post-core-disk-records");
    const recordsPath = path.join(resultDir, "plugin-install-records.json");
    await fs.mkdir(resultDir, { recursive: true });
    await fs.writeFile(
      recordsPath,
  ...
  it("post-core resume mode persists the requested update channel with the updated process", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...
      resolved: { update: { channel: "stable" } } as RecallConfig,
      sourceConfig: { update: { channel: "stable" } } as RecallConfig,
      runtimeConfig: { update: { channel: "stable" } } as RecallConfig,
      config: { update: { channel: "stable" } } as RecallConfig,
      hash: "stable-hash",
    });
    ...
    await withEnvAsync(
      {
        RECALL_UPDATE_POST_CORE: "1",
        RECALL_UPDATE_POST_CORE_CHANNEL: "dev",
        RECALL_UPDATE_POST_CORE_REQUESTED_CHANNEL: "dev",
      },
      async () => {
        await updateCommand({ restart: false });
      },
  ...
  it("uses a fail-closed integrity policy for post-core plugin updates", async () => {
    ...
    await expect(
      onIntegrityDrift({
        pluginId: "demo",
        spec: "@recall/demo@1.0.0",
        resolvedSpec: "@recall/demo@1.0.0",
  ...
  it("keeps json update output successful when post-core plugin updates warn", async () => {
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @recall/demo@1.0.0"
    ...
    expect(pluginWarning(jsonOutput)?.guidance).toEqual([
      "Run recall doctor --fix to attempt automatic repair.",
      "Run recall plugins inspect demo --runtime --json for details.",
    ]);
  ...
  it("detects missing plugin payloads from persisted records before npm updates", async () => {
    const installPath = createCaseDir("recall-missing-plugin-payload");
  ...
  it("prints non-fatal plugin warnings in human update output", async () => {
    ...
    expect(logs).toContain("Run recall doctor --fix to attempt automatic repair.");
    expect(logs).toContain("Run recall plugins inspect demo --runtime --json for details.");
  });
      ...
      outcomes: [
        {
          ...
          status: "skipped",
          message:
            'Disabled "demo" after plugin update failure; Recall will continue without it. Failed to update demo: registry timeout',
        },
      ],
    });
    vi.mocked(defaultRuntime.writeJson).mockClear();
  ...
  it("preserves fresh-process plugin warning details in parent json output", async () => {
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      queueMicrotask(async () => {
        const resultPath = env?.RECALL_UPDATE_POST_CORE_RESULT_PATH;
        if (resultPath) {
          await fs.writeFile(
            resultPath,
            JSON.stringify({
              ...
              warnings: [
                {
                  ...
                  message:
                    'Plugin "demo" could not be processed after the core update: Failed to update demo: registry timeout Run recall doctor --fix to attempt automatic repair. Run recall plugins inspect demo --runtime --json for details.',
                  guidance: [
                    "Run recall doctor --fix to attempt automatic repair.",
                    "Run recall plugins inspect demo --runtime --json for details.",
                  ],
  ...
  it.each([
    {
      ...
      assert: () => {
        ...
        expect(logs.join("\n")).toContain("Recall update status");
      },
    },
    {
      name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "recall", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as RecallConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expectPackageInstallSpec("recall@latest");
      }
  ...
  it("allows package updates from inherited gateway service env when the managed gateway is not running", async () => {
    mockPackageInstallStatus(createCaseDir("recall-update"));
    ...
    expect(defaultRuntime.error).not.toHaveBeenCalledWith(
      [
        ...
        "Run `recall update` from a shell outside the gateway service, or stop the gateway service first and then update.",
      ].join("\n"),
    );
    expectPackageInstallSpec("recall@latest");
  });
  ...
  it("refuses package updates from inherited gateway service env when --no-restart leaves the gateway running", async () => {
    ...
    serviceReadCommand.mockResolvedValue({
      programArguments: ["recall", "gateway", "run"],
      environment: {
        RECALL_SERVICE_MARKER: "recall",
        RECALL_SERVICE_KIND: "gateway",
      },
  ...
  it("refuses package updates from inside the active gateway process tree", async () => {
    ...
    expect(errors.join("\n")).toContain(
      "recall update detected it is running inside the gateway process tree.",
    );
  ...
  it.each([
    ...
    },
    {
      ...
      expectedSpec: "github:tryrecall/recall-agents#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("recall-update"));
        await updateCommand({ yes: true, tag: "github:tryrecall/recall-agents#main" });
      },
      expectedSpec: "github:tryrecall/recall-agents#main",
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("recall-update"));
        await updateCommand({ yes: true, tag: "Recall@github:tryrecall/recall-agents#main" });
      },
      expectedSpec: "Recall@github:tryrecall/recall-agents#main",
    },
    {
      name: "full git URL package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("recall-update"));
        await updateCommand({ yes: true, tag: "https://github.com/tryrecall/recall-agents.git#main" });
      },
      expectedSpec: "https://github.com/tryrecall/recall-agents.git#main",
    },
    {
      name: "hosted GitHub URL package spec without git suffix",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("recall-update"));
        await updateCommand({ yes: true, tag: "https://github.com/tryrecall/recall-agents#main" });
      },
      expectedSpec: "https://github.com/tryrecall/recall-agents#main",
    },
    {
      ...
      run: async () => {
        ...
        await updateCommand({
          yes: true,
          tag: "recall@https://github.com/tryrecall/recall-agents#main",
        });
      },
      expectedSpec: "https://github.com/tryrecall/recall-agents#main",
    },
    {
      ...
      run: async () => {
        mockPackageInstallStatus(createCaseDir("recall-update"));
        await updateCommand({ yes: true, tag: "tryrecall/recall-agents#main" });
      },
      expectedSpec: "tryrecall/recall-agents#main",
    },
    {
      name: "SCP-style SSH package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("recall-update"));
        await updateCommand({ yes: true, tag: "git@github.com:tryrecall/recall-agents.git#main" });
      },
      expectedSpec: "git@github.com:tryrecall/recall-agents.git#main",
    },
    {
      name: "RECALL_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("recall-update"));
        await withEnvAsync(
          { RECALL_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/recall-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/recall-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "recall");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "recall", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("stops package post-update work when staged npm install verification fails", async () => {
    const tempDir = await createTrackedTempDir("recall-update-staged-fail-");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "recall", version: "2026.4.20" }),
      "utf-8",
    );
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        const stageRoot = path.join(stagePrefix, "lib", "node_modules", "recall");
        await fs.mkdir(path.join(stageRoot, "dist"), { recursive: true });
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "recall", version: "2026.4.25" }),
          "utf-8",
        );
  ...
  it("marks package post-update doctor as update-in-progress", async () => {
    const tempDir = await createTrackedTempDir("recall-update-package-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "recall");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "recall", version: "2026.4.21" }),
      "utf-8",
    );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.RECALL_UPDATE_IN_PROGRESS,
    ).toBe("1");
  });

  it("runs package post-update doctor from the verified package root after a staged swap", async () => {
    const tempDir = await createTrackedTempDir("recall-update-staged-doctor-");
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (argv[0] === "npm" && argv[1] === "i" && argv.includes("--prefix")) {
        ...
        await fs.writeFile(
          ...
          JSON.stringify({ name: "recall", version: "2026.5.14" }),
          "utf-8",
        );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.RECALL_COMPATIBILITY_HOST_VERSION,
    ).toBe("2026.5.14");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
  });

  it("stops a running managed gateway before package replacement", async () => {
    const tempDir = await createTrackedTempDir("recall-update-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "recall");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    const serviceStopCall = serviceStop.mock.calls[0]?.[0] as
      ...
      | undefined;
    expect(serviceStopCall?.env?.RECALL_SERVICE_MARKER).toBe("recall");
    expect(serviceStopCall?.env?.RECALL_SERVICE_KIND).toBe("gateway");
  ...
  it("keeps managed service stop output off stdout during json package updates", async () => {
    const tempDir = await createTrackedTempDir("recall-update-json-stop-service-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "recall");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    ...
    serviceStop.mockImplementationOnce(async (params: { stdout?: NodeJS.WritableStream }) => {
      params.stdout?.write("Stopped systemd service: recall-gateway.service\n");
    });
    pathExists.mockImplementation(async (candidate: string) => {
      try {
        await fs.access(candidate);
  ...
  it("disarms legacy launchd updater jobs before stopping the gateway", async () => {
    const tempDir = await createTrackedTempDir("recall-update-launchd-loop-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "recall");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    launchdUpdateCleanupMocks.disableCurrentRecallUpdateLaunchdJob.mockResolvedValue(true);
    pathExists.mockImplementation(async (candidate: string) => {
      try {
        await fs.access(candidate);
        return true;
    ...
    const cleanupOrder =
      launchdUpdateCleanupMocks.disableCurrentRecallUpdateLaunchdJob.mock.invocationCallOrder[0];
  ...
  it("refreshes package installs even when the current version already matches the target", async () => {
    const tempDir = await createTrackedTempDir("recall-update-current-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "recall");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "recall", version: "2026.4.23" }),
      "utf-8",
    );
    await fs.writeFile(entryPath, "export {};\n", "utf-8");
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
    ...
    expect(postCoreSpawn?.[2].env?.RECALL_UPDATE_POST_CORE).toBe("1");
    expect(postCoreSpawn?.[2].env?.RECALL_UPDATE_POST_CORE_CHANNEL).toBe("stable");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(
      vi
        .mocked(defaultRuntime.log)
  ...
  it("retries package updates without optional deps when npm global update fails", async () => {
    const tempDir = await createTrackedTempDir("recall-update-optional-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "recall");
    mockPackageInstallStatus(pkgRoot);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "recall", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses the owning npm binary for package updates when PATH npm points elsewhere", async () => {
    ...
    const pkgRoot = path.join(brewRoot, "recall");
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("recall-update");
    const localAppData = createCaseDir("recall-localappdata");
  ...
  it("warns when a package update targets a managed service root outside the shell root", async () => {
    const shellRoot = createCaseDir("recall-shell-root");
    const serviceRoot = await createTrackedTempDir("recall-service-root-");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "recall", version: "2026.5.18" }),
      "utf-8",
    );
    ...
    expect(logs).toContain(
      `Shell Recall root differs from the managed gateway service root: ${shellRoot}`,
    );
    expect(logs).toContain("make sure `recall` on PATH resolves to the managed service root");
  ...
  it("runs managed service package follow-up commands with the service Node", async () => {
    const shellRoot = createCaseDir("recall-shell-root");
    const servicePrefix = await createTrackedTempDir("recall-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const serviceRoot = path.join(nodeModules, "recall");
  ...
  it("uses the managed service Node when package roots match but node binaries differ", async () => {
    const root = createCaseDir("recall-same-root");
  ...
  it("uses the managed service Node for follow-up commands when roots match but nodes differ", async () => {
    const servicePrefix = await createTrackedTempDir("recall-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "recall");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const serviceNpm = path.join(servicePrefix, "bin", "npm");
    const entrypoint = path.join(root, "dist", "index.js");
    await fs.mkdir(path.dirname(entrypoint), { recursive: true });
  ...
  it("pins package install to the service root when nodes differ and no owning npm exists at the prefix", async () => {
    const servicePrefix = await createTrackedTempDir("recall-no-npm-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "recall");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const entrypoint = path.join(root, "dist", "index.js");
  ...
  it("does not repair legacy config during a dry run", async () => {
    ...
    expect(replaceConfigFile).not.toHaveBeenCalled();
    expect(runCommandWithTimeout).not.toHaveBeenCalled();
    expect(launchdUpdateCleanupMocks.disableCurrentRecallUpdateLaunchdJob).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it("refreshes post-doctor config before post-update plugin sync", async () => {
    const tempDir = createCaseDir("recall-update");
    mockPackageInstallStatus(tempDir);
    const preUpdateConfig = { update: { channel: "stable" } } as RecallConfig;
    ...
    syncPluginsForUpdateChannel.mockImplementation(async ({ config }) => ({
      ...
      config: {
        ...
        plugins: {
          ...config.plugins,
          load: { paths: ["/tmp/recall-updated-plugin"] },
        },
      },
      summary: {
        switchedToBundled: [],
    ...
    const syncConfig = syncPluginCall()?.config as
      | (RecallConfig & { meta?: { lastTouchedVersion?: string } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          baseHash?: string;
          nextConfig?: RecallConfig & { meta?: { lastTouchedVersion?: string } };
        }
      | undefined;
    ...
    await withEnvAsync(
      {
        RECALL_UPDATE_POST_CORE: "1",
        RECALL_UPDATE_POST_CORE_CHANNEL: "stable",
        RECALL_UPDATE_POST_CORE_STARTED_AT_MS: String(updateStartedAtMs),
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
  ...
  it("persists authored channel values when post-core restore input is resolved", async () => {
    ...
    await withEnvAsync(
      {
        RECALL_UPDATE_POST_CORE: "1",
        RECALL_UPDATE_POST_CORE_CHANNEL: "stable",
        RECALL_UPDATE_POST_CORE_SOURCE_CONFIG_PATH: sourceConfigPath,
      },
      async () => {
        await updateCommand({ yes: true, restart: false });
      },
    ...
    const syncConfig = syncPluginCall()?.config as
      | (RecallConfig & { channels?: { whatsapp?: { token?: string } } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          nextConfig?: RecallConfig & {
  ...
  it("persists channel and runs post-update work after switching from package to git", async () => {
    const tempDir = createCaseDir("recall-update");
    const gitRoot = path.join(tempDir, "..", "recall");
    ...
    const syncCall = syncPluginCall() as
      | { channel?: string; config?: RecallConfig; workspaceDir?: string }
      | undefined;
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `recall update`.",
    );
    expect(defaultRuntime.exit).toHaveBeenCalledWith(0);
  });
  it.each([
  ...
  it("fails a JSON package update when fallback restart leaves the old gateway running", async () => {
    const updatedRoot = createCaseDir("recall-updated-root");
  ...
  it("writes the control-plane update sentinel after managed package restart health passes", async () => {
    const stateDir = await createTrackedTempDir("recall-update-sentinel-state-");
    const metaDir = await createTrackedTempDir("recall-update-sentinel-meta-");
  ...
  it.each([
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            RECALL_CONFIG_PATH: "./config/recall.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedEnv: () => ({
        RECALL_STATE_DIR: path.resolve("./state"),
        RECALL_CONFIG_PATH: path.resolve("./config/recall.json"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
    },
    {
      ...
      expectedEnv: (context?: { originalCwd: string }) => ({
        RECALL_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ RECALL_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.RECALL_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateFinalizeCommand runs doctor and plugin convergence with full update env", async () => {
    await withEnvAsync(
      {
        RECALL_UPDATE_IN_PROGRESS: undefined,
        RECALL_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR: undefined,
        RECALL_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE: undefined,
      },
      async () => {
        ...
        expect(doctorEnv?.RECALL_UPDATE_IN_PROGRESS).toBe("1");
        expect(doctorEnv?.RECALL_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBe("1");
        expect(doctorEnv?.RECALL_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBe("1");
        expect(process.env.RECALL_UPDATE_IN_PROGRESS).toBeUndefined();
        expect(process.env.RECALL_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBeUndefined();
        expect(process.env.RECALL_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBeUndefined();
  ...
  it("updateFinalizeCommand repairs doctor by default and refreshes plugin state after doctor", async () => {
    ...
    syncPluginsForUpdateChannel.mockImplementationOnce(
      async (params: { config?: RecallConfig }) => ({
        changed: true,
  ...
  it("updateFinalizeCommand reapplies requested channel against post-doctor config", async () => {
    const preDoctorConfig = { update: { channel: "stable" } } as RecallConfig;
    const postDoctorConfig = { update: { channel: "beta" } } as RecallConfig;
    const preDoctorSnapshot: ConfigFileSnapshot = {
      ...baseSnapshot,
      sourceConfig: preDoctorConfig,
      resolved: preDoctorConfig,
  ...
  it.each([
    ...
    },
    {
      ...
      requireTty: false,
      expectedError:
        "Update wizard requires a TTY. Use `recall update --channel <stable|beta|dev>` instead.",
    },
  ] as const)(
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("recall-update-wizard");
    await withEnvAsync({ RECALL_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/recall as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ RECALL_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "recall"));
    });
