...
vi.mock("../infra/path-env.js", () => ({
  ensureRecallCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  beforeEach(() => {
    ...
    delete process.env.RECALL_DISABLE_CLI_STARTUP_HELP_FAST_PATH;
    delete process.env.RECALL_HIDE_BANNER;
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "recall", "status"]);
  ...
  it("disposes registered harnesses after full CLI command completion", async () => {
    ...
    await runCli(["node", "recall", "agent", "--local"]);

    expect(parseAsync).toHaveBeenCalledWith(["node", "recall", "agent", "--local"]);
  ...
  it("shows the standard spinner while loading the full CLI", async () => {
    ...
    await runCli(["node", "recall", "config"]);

    expect(createCliProgressMock).toHaveBeenCalledWith({
      label: "Loading Recall CLI…",
      indeterminate: true,
      delayMs: 0,
    });
  ...
  it("pauses non-tty stdin after full CLI command completion", async () => {
    ...
    try {
      await runCli(["node", "recall", "channels"]);

      expect(parseAsync).toHaveBeenCalledWith(["node", "recall", "channels"]);
  ...
  it("emits the startup banner before gateway foreground fast-path startup", async () => {
    await runCli(["node", "recall", "gateway", "--force"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).toHaveBeenCalledWith("9.9.9-test", {
      argv: ["node", "recall", "gateway", "--force"],
    });
  ...
  it("honors banner suppression on the gateway foreground fast path", async () => {
    process.env.RECALL_HIDE_BANNER = "1";

    await runCli(["node", "recall", "gateway"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).not.toHaveBeenCalled();
    expect(commanderParseAsyncMock).toHaveBeenCalledWith(["node", "recall", "gateway"]);
  });
  ...
  it("renders browser help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "recall", "browser", "--help"]);
  ...
  it("renders secrets help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "recall", "secrets", "--help"]);
  ...
  it("renders nodes help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "recall", "nodes", "--help"]);
  ...
  it("defers nodes help startup metadata when plugin config can change command metadata", async () => {
    const argv = ["node", "recall", "nodes", "--help"];
    const parseAsync = vi.fn().mockResolvedValueOnce(undefined);
    const program = {
  ...
  it("renders selected subcommand help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "recall", "doctor", "--help"]);
  ...
  it("renders setup/onboard/configure help without building the full program", async () => {
    await runCli(["node", "recall", "setup", "--help"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "recall", "--help"]);
    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(loadRootHelpRenderOptionsForConfigSensitivePluginsMock).toHaveBeenCalledTimes(1);
    expect(outputPrecomputedRootHelpTextMock).toHaveBeenCalledTimes(1);
  ...
  it.each([
    ["gateway runtime", ["node", "recall", "gateway", "run"]],
    ["bare gateway runtime", ["node", "recall", "gateway"]],
    ["node runtime", ["node", "recall", "node", "run"]],
    ["local agent runtime", ["node", "recall", "agent", "--local"]],
    ["provider inference", ["node", "recall", "infer", "web", "fetch", "https://example.com"]],
    ["model command", ["node", "recall", "models", "auth", "login", "openai"]],
    ["plugin command", ["node", "recall", "plugins", "marketplace", "list"]],
    ["skill command", ["node", "recall", "skills", "search", "browser"]],
    ["update command", ["node", "recall", "update", "check"]],
    ["channel probe", ["node", "recall", "channels", "status", "--probe"]],
    ["channel capabilities probe", ["node", "recall", "channels", "capabilities"]],
    ["directory plugin command", ["node", "recall", "directory", "peers", "list"]],
    ["message plugin command", ["node", "recall", "message", "send", "--to", "demo"]],
    ["metadata-owned plugin command", ["node", "recall", "googlemeet", "login"]],
  ])("starts managed proxy routing for %s", (_name, argv) => {
    expect(shouldStartProxyForCli(argv)).toBe(true);
  });

  it.each([
    ["root help", ["node", "recall", "--help"]],
    ["root version", ["node", "recall", "--version"]],
    ["gateway help", ["node", "recall", "gateway", "--help"]],
    ["gateway run help", ["node", "recall", "gateway", "run", "--help"]],
    ["status", ["node", "recall", "status"]],
    ["health", ["node", "recall", "health"]],
    ["gateway status", ["node", "recall", "gateway", "status"]],
    ["gateway health", ["node", "recall", "gateway", "health"]],
    ["remote agent control-plane", ["node", "recall", "agent", "run"]],
    ["chat control-plane", ["node", "recall", "chat"]],
    ["terminal control-plane", ["node", "recall", "terminal"]],
    ["config", ["node", "recall", "config", "get", "proxy.enabled"]],
    ["channels parent help", ["node", "recall", "channels"]],
    ["completion", ["node", "recall", "completion", "zsh"]],
    ["debug proxy cli", ["node", "recall", "proxy", "start"]],
    ["agents list", ["node", "recall", "agents", "list"]],
    ["models list", ["node", "recall", "models", "list"]],
    ["models status without live probe", ["node", "recall", "models", "status"]],
    ["skills check", ["node", "recall", "skills", "check"]],
    ["skills info", ["node", "recall", "skills", "info", "weather"]],
    ["skills list", ["node", "recall", "skills", "list"]],
    ["tasks list", ["node", "recall", "tasks", "list"]],
    ["legacy singular tool namespace", ["node", "recall", "tool", "image_generate"]],
    ["gateway tools namespace typo", ["node", "recall", "tools", "effective"]],
    ["migrate", ["node", "recall", "migrate"]],
  ...
  it("starts the managed proxy for network-capable commands by default", async () => {
    ...
    await runCli(["node", "recall", "plugins", "marketplace", "list"]);
  ...
  it("starts the managed proxy for metadata-owned plugin commands by default", async () => {
    ...
    await runCli(["node", "recall", "googlemeet", "login"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it("rejects unowned command roots before proxy and plugin runtime registration", async () => {
    await expect(runCli(["node", "recall", "foo"])).rejects.toThrow(
  ...
  it("does not suggest plugins.allow for unknown command roots before proxy startup", async () => {
    ...
    let error: unknown;
    try {
      await runCli(["node", "recall", "totally-unknown"]);
    } catch (caught) {
      error = caught;
    }
  ...
  it("preserves plugins.allow diagnostics for roots owned only by CLI metadata", async () => {
    ...
    await expect(runCli(["node", "recall", "qa"])).rejects.toThrow(
  ...
  it("reports plugin tool command mistakes before proxy startup", async () => {
    ...
    await expect(runCli(["node", "recall", "lcm_recent"])).rejects.toThrow(
  ...
  it("does not install the env proxy dispatcher for bypassed skills inspection commands", async () => {
    ...
    await runCli(["node", "recall", "skills", "check"]);
  ...
  it.each([
    ["auth", ["node", "recall", "auth", "--help"]],
    ["tool", ["node", "recall", "tool", "image_generate"]],
    ["tools", ["node", "recall", "tools", "effective"]],
  ...
  it("routes lazy plugin registration logs to stderr only during --json registration", async () => {
    ...
    await runCli(["node", "recall", "memory", "search", "query", "--json"]);
  ...
  it("does not route lazy plugin registration logs for pass-through --json after terminator", async () => {
    ...
    await runCli(["node", "recall", "memory", "--", "--json"]);
  ...
  it("fails protected commands when managed proxy activation fails", async () => {
    ...
    await expect(runCli(["node", "recall", "gateway", "run"])).rejects.toThrow(
  ...
  it("stops the managed proxy after normal gateway runtime completion", async () => {
    ...
    await runCli(["node", "recall", "gateway", "run"]);
  ...
  it("stops the managed proxy and exits after SIGINT", async () => {
    ...
    try {
      const runPromise = runCli(["node", "recall", "plugins", "marketplace", "list"]);
      await vi.waitFor(() => {
        expect(
  ...
  it("starts onboarding for bare root invocations when config only has metadata", async () => {
    readConfigFileSnapshotMock.mockResolvedValueOnce({
      ...
      valid: true,
      sourceConfig: {
        $schema: "https://recall.ai/config.json",
  ...
  it("points noninteractive fresh bare root invocations to onboarding automation", async () => {
    ...
    try {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        "Onboarding needs an interactive TTY. Use `recall onboard --non-interactive --accept-risk ...` for automation.",
      );
  ...
  it("bootstraps env proxy before modern onboard Crestodian startup", async () => {
    ...
    await runCli(["node", "recall", "onboard", "--modern", "--json"]);
  ...
  it("does not fail the command when memory cleanup is unavailable", async () => {
    ...
    await expect(runCli(["node", "recall", "status"])).resolves.toBeUndefined();
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "recall", "--container", "demo", "status"]);
  ...
  it("swallows Commander parse exits after recording the exit code", async () => {
    ...
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "status", ["node", "recall", "status"]],
    ]);
    expect(process.exitCode).toBe(1);
    process.exitCode = exitCode;
  });
  ...
  it("loads the real primary command before rendering command help", async () => {
    ...
    expect(registerCoreCliByNameMock.mock.calls).toEqual([
      [program, ctx, "doctor", ["node", "recall", "doctor", "--help"]],
    ]);
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "doctor", ["node", "recall", "doctor", "--help"]],
    ]);
  });
  ...
  it("restores terminal state before uncaught CLI exits", async () => {
    ...
    try {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "[recall] Recall hit an unexpected runtime error.",
      );
      expect(consoleErrorSpy).toHaveBeenCalledWith("[recall] Reason: boom");
  ...
  it("does not exit for transient uncaught CLI exceptions", async () => {
    ...
    try {
      ...
      expect(consoleWarnSpy.mock.calls).toEqual([
        ["[recall] Non-fatal uncaught exception (continuing):", hostUnreachable.stack],
      ]);
