...
function createConfigureInteractiveResult(options?: {
  ...
  return {
    ...
    preflight: {
      ...
      changedFiles: options?.changed ? ["/tmp/recall.json"] : [],
      checks: {
        resolvability: true,
...
describe("secrets CLI", () => {
  ...
  it("explains Gateway reload failures without duplicate doctor noise", async () => {
    callGatewayFromCli.mockRejectedValue(
      new Error(
        "gateway closed (1006 abnormal closure). Gateway target: ws://127.0.0.1:18789 Source: local loopback Config: /tmp/recall.json Bind: loopback Possible causes: - Gateway not yet ready. Run `recall doctor` for diagnostics.",
      ),
    );
    ...
    expect(runtimeErrors.at(-1)).toContain("recall gateway status --deep");
