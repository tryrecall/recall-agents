...
import { serveRecallChannelMcp } from "../mcp/channel-server.js";
...
export function registerMcpCli(program: Command) {
  const mcp = program.command("mcp").description("Manage Recall MCP config and channel bridge");

  mcp
    .command("serve")
    .description("Expose Recall channels over MCP stdio")
    ...
    .action(async (opts) => {
      try {
        ...
        defaultRuntime.error(
          `MCP server failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("recall mcp list")} to inspect configured servers.`,
        );
        defaultRuntime.exit(1);
      }
    });
  ...
  mcp
    ...
    .action(async (opts: { json?: boolean }) => {
      ...
      if (names.length === 0) {
        defaultRuntime.log(
          `No MCP servers configured in ${loaded.path}. Add one with ${formatCliCommand('recall mcp set <name> \'{"command":"uvx","args":["context7-mcp"]}\'')}.`,
        );
        return;
      }
  ...
  mcp
    ...
    .action(async (name: string | undefined, opts: { json?: boolean }) => {
      ...
      if (name && !value) {
        fail(
          `No MCP server named "${name}" in ${loaded.path}. Run ${formatCliCommand("recall mcp list")} to see configured servers.`,
        );
      }
      if (opts.json) {
        printJson(value ?? {});
  ...
  mcp
    ...
    .action(async (name: string) => {
      ...
      if (!result.removed) {
        fail(
          `No MCP server named "${name}" in ${result.path}. Run ${formatCliCommand("recall mcp list")} to see configured servers.`,
        );
      }
