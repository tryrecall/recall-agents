...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-bundled-skills-test-"));
    process.env.RECALL_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
