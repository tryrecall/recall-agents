...
import { installedPluginRoot } from "recall/plugin-sdk/test-fixtures";
...
const CLI_STATE_ROOT = "/tmp/recall-state";
const ORIGINAL_RECALL_STATE_DIR = process.env.RECALL_STATE_DIR;
const ORIGINAL_RECALL_NIX_MODE = process.env.RECALL_NIX_MODE;
const PROFILE_STATE_ROOT = "/tmp/recall-ledger-profile";
...
function useProfileExtensionsDir(): string {
  process.env.RECALL_STATE_DIR = PROFILE_STATE_ROOT;
  return path.resolve(PROFILE_STATE_ROOT, "extensions");
}

function createEnabledPluginConfig(pluginId: string): RecallConfig {
  return {
    plugins: {
      entries: {
        [pluginId]: {
...
function createEmptyPluginConfig(): RecallConfig {
  return {
    plugins: {
      entries: {},
    },
...
function createNpmPackPluginInstallResult(
  ...
  return {
    ...
    manifestName: `@recall/${pluginId}`,
    npmTarballName: `recall-${pluginId}-1.2.3.tgz`,
    npmResolution: {
      name: `@recall/${pluginId}`,
      version: "1.2.3",
      resolvedSpec: `@recall/${pluginId}@1.2.3`,
...
function createPathHookPackInstalledConfig(tmpRoot: string): RecallConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function createNpmHookPackInstalledConfig(): RecallConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function replaceConfigCall(callIndex = 0): { baseHash?: string; nextConfig?: RecallConfig } {
...
describe("plugins cli install", () => {
  ...
  afterEach(() => {
    if (ORIGINAL_RECALL_STATE_DIR === undefined) {
      delete process.env.RECALL_STATE_DIR;
    } else {
      process.env.RECALL_STATE_DIR = ORIGINAL_RECALL_STATE_DIR;
    }
    if (ORIGINAL_RECALL_NIX_MODE === undefined) {
      delete process.env.RECALL_NIX_MODE;
    } else {
      process.env.RECALL_NIX_MODE = ORIGINAL_RECALL_NIX_MODE;
    }
  });
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `recall doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(writeConfigFile).not.toHaveBeenCalled();
  ...
  it("resolves exact official external plugin ids through their npm package", async () => {
    ...
    expect(npmInstallCall().spec).toBe("@recall/brave-plugin");
    ...
    expect(record.spec).toBe("@recall/brave-plugin");
  ...
  it("passes third-party external catalog integrity with catalog install trust", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("wecom-recall-plugin");
    loadConfig.mockReturnValue(cfg);
    findBundledPluginSourceMock.mockReturnValue(undefined);
    installPluginFromNpmSpec.mockResolvedValue(
      createNpmPluginInstallResult("wecom-recall-plugin"),
    );
    enablePluginInConfig.mockReturnValue({ config: enabledCfg });
    applyExclusiveSlotSelection.mockReturnValue({
      config: enabledCfg,
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-recall-plugin@2026.5.7");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-recall-plugin");
  ...
  it("passes third-party external catalog integrity to hook-pack fallback", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing recall.extensions",
      code: "missing_openclaw_extensions",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "aborted: npm package integrity drift detected for @wecom/wecom-recall-plugin@2026.5.7",
    });

    await expect(runPluginsCommand(["plugins", "install", "wecom"])).rejects.toThrow("__exit__:1");

    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(hookNpmInstallCall().spec).toBe("@wecom/wecom-recall-plugin@2026.5.7");
  ...
  it("installs npm-pack archives through npm install semantics", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
    const archivePath = "/tmp/recall-demo-1.2.3.tgz";
    ...
    const record = persistedInstallRecord("demo");
    expect(record.source).toBe("npm");
    expect(record.spec).toBe("@recall/demo@1.2.3");
    ...
    expect(record.npmTarballName).toBe("recall-demo-1.2.3.tgz");
    expect(writeConfigFile).toHaveBeenCalledWith(enabledCfg);
  });
  ...
  it("marks explicit official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "npm:@recall/discord"]);
  ...
  it("marks scoped official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@recall/discord"]);
  ...
  it("marks catalog npm package installs with alternate selectors as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@wecom/wecom-recall-plugin@latest"]);
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-recall-plugin@latest");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-recall-plugin");
    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(npmInstallCall().expectedIntegrity).toBeUndefined();
    expect(installPluginFromClawHub).not.toHaveBeenCalled();
  ...
  it("reports npm install failures without trying ClawHub when npm: prefix is used", async () => {
    ...
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing recall.hooks",
    });
  ...
  it("adds a Git PATH hint when npm plugin dependency install cannot spawn git", async () => {
    ...
    await expect(
      runPluginsCommand(["plugins", "install", "npm:@recall/whatsapp"]),
    ).rejects.toThrow("__exit__:1");
  ...
  it("does not resolve npm: prefixed bundled plugin ids through bundled installs", async () => {
    loadConfig.mockReturnValue({ plugins: { load: { paths: [] } } } as RecallConfig);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
  ...
  it("rejects --pin for git installs and points at git refs", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain("recall plugins install git:<repo>@<ref>");
  });
  ...
  it("passes dangerous force unsafe install to linked path probe installs", async () => {
    ...
    const enabledCfg = createEnabledPluginConfig("demo");
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "recall-plugin-link-"));
  ...
  it("passes dangerous force unsafe install to linked hook-pack probe fallback", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "recall-hook-link-"));
  ...
  it("does not fall back to hook pack for linked path when a no-flag security scan blocks", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-link-plugin-"));
  ...
  it("passes dangerous force unsafe install to local hook-pack fallback installs", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "recall-hook-install-"));
    primeHookPackPathFallback({
      tmpRoot,
  ...
  it("passes the active profile extensions dir to local path installs", async () => {
    const extensionsDir = useProfileExtensionsDir();
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-local-plugin-"));
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
  ...
  it("suggests update or --force when npm plugin install target already exists", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "plugin already exists: /home/recall/.recall/extensions/lossless-claw (delete it first)",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing recall.hooks",
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Use `recall plugins update <id-or-npm-spec>` to upgrade the tracked plugin, or rerun install with `--force` to replace it.",
    );
    expect(runtimeErrors.at(-1)).not.toContain("Also not a valid hook pack");
  });
  ...
  it("still falls back to local hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    const localHookDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-local-hook-pack-"));
    const cfg = {} as RecallConfig;
    const installedCfg = {
      hooks: {
        internal: {
