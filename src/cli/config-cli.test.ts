...
const mockWriteConfigFile = vi.fn<
  (cfg: RecallConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});
const mockResolveSecretRefValue = vi.fn();

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: RecallConfig, options?: { unsetPaths?: string[][] }) =>
...
describe("config cli", () => {
  ...
  describe("config set parsing flags", () => {
    ...
    it("shows --strict-json and keeps --json as a legacy alias in help", async () => {
      ...
      expect(helpText).toContain("recall config set gateway.port 19001 --strict-json");
      expect(helpText).toContain(
        "recall config set channels.discord.token --ref-provider default --ref-source",
      );
  ...
  describe("config file", () => {
    ...
    it("handles config file path with home directory", async () => {
      ...
      snapshot.path = "/home/user/.recall/recall.json";
      mockReadConfigFileSnapshot.mockResolvedValueOnce(snapshot);

      await runConfigCommand(["config", "file"]);

      expect(mockLog).toHaveBeenCalledWith("/home/user/.recall/recall.json");
    });
  });
});
