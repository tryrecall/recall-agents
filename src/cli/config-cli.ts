...
function normalizeModelProviderRefsForConfigMutation(
  providers: NonNullable<RecallConfig["models"]>["providers"] | undefined,
): unknown {
...
function normalizeConfigMutationModelRefs(cfg: RecallConfig): RecallConfig {
...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "recall config set secrets.providers.vault --provider-source file --provider-path /etc/recall/secrets.json --provider-mode json",
);
...
const CONFIG_PATCH_EXAMPLE_STDIN = formatCliCommand("recall config patch --stdin");
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("recall doctor --fix")}\` ${message}`;
}
...
async function loadValidConfig(runtime: RuntimeEnv = defaultRuntime) {
  ...
  runtime.error(`Recall config is invalid: ${shortenHomePath(snapshot.path)}`);
...
function formatPluginInstallConfigSetError(): string {
  return [
    ...
    `  ${formatCliCommand("recall plugins install <spec>")}`,
    `  ${formatCliCommand("recall plugins update <plugin-id>")}`,
    `  ${formatCliCommand("recall plugins uninstall <plugin-id>")}`,
  ].join("\n");
}
...
function formatAutoManagedMetaError(paths: readonly PathSegment[][]): string {
  ...
  return [
    `${subject} is auto-managed by Recall and cannot be edited; the value would be overwritten on the next config write.`,
    "",
    "These fields are stamped on every config write to record the Recall version and timestamp that produced the file.",
  ].join("\n");
}
...
function collectDryRunSchemaErrors(params: { config: RecallConfig }): ConfigSetDryRunError[] {
...
async function runConfigOperations(params: {
  ...
  const nextConfig = normalizeConfigMutationModelRefs(next as RecallConfig);
...
export async function runConfigGet(opts: { path: string; json?: boolean; runtime?: RuntimeEnv }) {
  ...
  try {
    ...
    if (!res.found) {
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Run ${formatCliCommand("recall config validate")} to inspect config shape.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigUnset(opts: {
  ...
  try {
    ...
    if (!unsetResult.removed) {
      ...
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Nothing was changed. Run ${formatCliCommand("recall config get <path>")} first if you are unsure of the path.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "recall.json";
  ...
  try {
    ...
    if (!snapshot.exists) {
      if (opts.json) {
        ...
        runtime.error(
          `Create one with ${formatCliCommand("recall onboard")} or run ${formatCliCommand("recall doctor --fix")}.`,
        );
      }
      runtime.exit(1);
      return;
    ...
    if (!snapshot.valid) {
      ...
      if (opts.json) {
        ...
        runtime.error(danger(`Recall config is invalid: ${shortPath}`));
        ...
        runtime.error(`Inspect with ${formatCliCommand("recall config validate")}.`);
      }
      runtime.exit(1);
      return;
    }
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.recall.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing recall.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing recall.json (checks schema and SecretRef resolvability; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for recall.json")
    .action(async () => {
