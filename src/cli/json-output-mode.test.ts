...
describe("json output mode", () => {
  ...
  it("detects json output flags before argv terminators", () => {
    expect(hasJsonOutputFlag(["node", "recall", "nodes", "list", "--json"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "recall", "nodes", "list", "--json=true"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "recall", "nodes", "--", "--json"])).toBe(false);
  });
