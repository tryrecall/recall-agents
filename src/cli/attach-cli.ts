...
export function writeClaudeMcpConfig(mcpConfig: AttachGrant["mcpConfig"]): {
  ...
  const dir = mkdtempSync(join(tmpdir(), "steelengine-attach-"));
...
export async function registerAttachCli(program: Command, _argv: string[] = process.argv) {
  program
    ...
    .addHelpText(
      "after",
      "\nExamples:\n  steelengine attach                       Attach Claude Code to the main session\n  steelengine attach --session agent:main:telegram:123 --ttl 600000\n  steelengine attach --print-config        Set up the grant + config and print how to launch it yourself\n",
    )
