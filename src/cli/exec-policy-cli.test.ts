...
const mocks = vi.hoisted(() => {
  ...
  let configState: SteelEngineConfig = {
    tools: {
      exec: {
        host: "auto",
        security: "allowlist",
  ...
  return {
    ...
    setConfig: (next: SteelEngineConfig) => {
      configState = next;
    },
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: SteelEngineConfig) => void }) => {
    ...
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: SteelEngineConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: SteelEngineConfig }>
    >(async () => ({
      path: "/tmp/steelengine.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: SteelEngineConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/steelengine.json");
    ...
    expect(output).not.toContain("/tmp/steelengine.json\nforged");
