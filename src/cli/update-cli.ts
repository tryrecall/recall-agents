...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Recall and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["recall update", "Update a source checkout (git)"],
        ["recall update --channel beta", "Switch to beta channel (git + npm)"],
        ["recall update --channel dev", "Switch to dev channel (git + npm)"],
        ["recall update --tag beta", "One-off update to a dist-tag or version"],
        ["recall update --tag main", "One-off package update from GitHub main"],
        ["recall update --dry-run", "Preview actions without changing anything"],
        ["recall update --no-restart", "Update without restarting the service"],
        ["recall update --json", "Output result as JSON"],
        ["recall update --yes", "Non-interactive (accept downgrade prompts)"],
        ["recall update wizard", "Interactive update wizard"],
        ["recall --update", "Shorthand for recall update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run recall update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.recall.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .description("Run Recall update finalization after an external core runtime change")
    .option("--json", "Output result as JSON", false)
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["recall update status", "Show channel + version status."],
          ["recall update status --json", "JSON output."],
          ["recall update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.recall.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
