...
const DEFAULT_CLI_NAME = "steelengine";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(steelengine)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `steelengine`. */
...
/** Replace a leading `steelengine` command prefix with the active CLI name. */
