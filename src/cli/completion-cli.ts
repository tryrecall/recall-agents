...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Recall Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(recall completion --shell zsh)
  return (
 ...
 * Returns true if profile has `source <(recall completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.recall.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $RECALL_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "recall") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
