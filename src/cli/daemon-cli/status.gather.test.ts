...
import type { StaleSteelEngineUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const findStaleSteelEngineUpdateLaunchdJobs = vi.fn<
  (env?: NodeJS.ProcessEnv) => Promise<StaleSteelEngineUpdateLaunchdJob[]>
>(async () => []);
...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    ...
    STEELENGINE_CONFIG_PATH: "/tmp/steelengine-daemon/steelengine.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.STEELENGINE_STATE_DIR ?? "/tmp/steelengine-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.STEELENGINE_CONFIG_PATH ?? `${stateDir}/steelengine.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({
    ...
    const isDaemon = configPath.includes("/steelengine-daemon/");
...
vi.mock("../../daemon/launchd.js", () => ({
  findStaleSteelEngineUpdateLaunchdJobs: (env?: NodeJS.ProcessEnv) =>
    findStaleSteelEngineUpdateLaunchdJobs(env),
}));
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    setTestEnvValue("STEELENGINE_STATE_DIR", "/tmp/steelengine-cli");
    setTestEnvValue("STEELENGINE_CONFIG_PATH", "/tmp/steelengine-cli/steelengine.json");
    ...
    findStaleSteelEngineUpdateLaunchdJobs.mockReset();
    findStaleSteelEngineUpdateLaunchdJobs.mockResolvedValue([]);
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(probeInput.configPath).toBe("/tmp/steelengine-daemon/steelengine.json");
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/steelengine-cli/steelengine.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.STEELENGINE_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.STEELENGINE_GATEWAY_PORT ?? "missing-port",
    }));
    ...
    expect(
      serviceReadRuntime.mock.calls.some(([env]) => env?.STEELENGINE_GATEWAY_PORT === "19001"),
    ).toBe(true);
  ...
  it("surfaces recent service restart handoffs only during deep status", async () => {
    ...
    expect(handoffInput.STEELENGINE_STATE_DIR).toBe("/tmp/steelengine-daemon");
    expect(handoffInput.STEELENGINE_CONFIG_PATH).toBe("/tmp/steelengine-daemon/steelengine.json");
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      ...
      findStaleSteelEngineUpdateLaunchdJobs.mockResolvedValueOnce([
        {
      ...
      const staleScanEnv = findStaleSteelEngineUpdateLaunchdJobs.mock.calls[0]?.[0];
      expect(staleScanEnv?.STEELENGINE_STATE_DIR).toBe("/tmp/steelengine-daemon");
      expect(staleScanEnv?.STEELENGINE_CONFIG_PATH).toBe("/tmp/steelengine-daemon/steelengine.json");
      expect(staleScanEnv?.STEELENGINE_LAUNCHD_LABEL).toBe("ai.steelengine.manual-update.gateway");
  ...
  it("does not read restart handoffs during normal status", async () => {
    ...
    expect(findStaleSteelEngineUpdateLaunchdJobs).not.toHaveBeenCalled();
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-status-config-"));
    const configPath = path.join(tmp, "steelengine.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        gateway: {
    ...
    setTestEnvValue("STEELENGINE_STATE_DIR", tmp);
    setTestEnvValue("STEELENGINE_CONFIG_PATH", configPath);
    serviceReadCommand.mockResolvedValueOnce({
      programArguments: ["/bin/node", "cli", "gateway", "--port", "19001"],
      environment: {
        STEELENGINE_STATE_DIR: tmp,
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    setTestEnvValue("STEELENGINE_GATEWAY_TOKEN", "env-token");
    setTestEnvValue("STEELENGINE_GATEWAY_PASSWORD", "env-password"); // pragma: allowlist secret
  ...
  it("includes the last gateway error when the service is listening but the RPC probe fails", async () => {
    inspectPortUsage.mockResolvedValueOnce({
      port: 19001,
      status: "busy",
      listeners: [{ pid: 8000, ppid: 1, commandLine: "steelengine gateway" }],
      hints: [],
    });
    callGatewayStatusProbe.mockResolvedValueOnce({
      ok: false,
  ...
  it("reads install records from the merged daemon service environment, not the CLI process env", async () => {
    ...
    // The mock daemon service command sets STEELENGINE_STATE_DIR=/tmp/steelengine-daemon,
    // distinct from the CLI process STEELENGINE_STATE_DIR=/tmp/steelengine-cli. Drift
