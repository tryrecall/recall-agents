...
describe("renderGatewayServiceStartHints", () => {
  ...
  it("resolves daemon container context from either env key", () => {
    expect(
      resolveDaemonContainerContext({
        STEELENGINE_CONTAINER: "steelengine-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("steelengine-demo-container");
    expect(
      resolveDaemonContainerContext({
        STEELENGINE_CONTAINER_HINT: "steelengine-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("steelengine-demo-container");
  });

  it("prepends a single container restart hint when STEELENGINE_CONTAINER is set", () => {
    expect(
      renderGatewayServiceStartHints({
        STEELENGINE_CONTAINER: "steelengine-demo-container",
      } as NodeJS.ProcessEnv),
  ...
  it("prepends a single container restart hint when STEELENGINE_CONTAINER_HINT is set", () => {
    expect(
      renderGatewayServiceStartHints({
        STEELENGINE_CONTAINER_HINT: "steelengine-demo-container",
      } as NodeJS.ProcessEnv),
...
describe("filterContainerGenericHints", () => {
  it("drops the generic container foreground hint when STEELENGINE_CONTAINER is set", () => {
    expect(
      filterContainerGenericHints(
        [
        ...
        { STEELENGINE_CONTAINER: "steelengine-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });

  it("drops the generic container foreground hint when STEELENGINE_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
        ...
        { STEELENGINE_CONTAINER_HINT: "steelengine-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });
});
