...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("steelengine-daemon-install-int-");
    configPath = path.join(tempHome, "steelengine.json");
    process.env.HOME = tempHome;
    process.env.STEELENGINE_STATE_DIR = tempHome;
  ...
  it("refuses service install when config was written by a newer SteelEngine", async () => {
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.STEELENGINE_GATEWAY_TOKEN).toBeUndefined();
  });
});
