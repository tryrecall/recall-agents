...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("recall gateway install", env)}`);
    if (fileLog) {
  ...
  if (runtime.missingSupervision) {
    hints.push(
      `LaunchAgent installed but not loaded. Run: ${formatCliCommand("recall gateway restart", env)}`,
    );
    if (fileLog) {
      hints.push(`File logs: ${fileLog}`);
    }
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.RECALL_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.RECALL_PROFILE),
      }),
    );
  }
  return hints;
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  const hints = buildPlatformServiceStartHints({
    installCommand: formatCliCommand("recall gateway install", env),
    startCommand: formatCliCommand("recall gateway", env),
