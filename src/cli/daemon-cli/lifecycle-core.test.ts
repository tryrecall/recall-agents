...
const loadConfig = vi.fn<() => RecallConfig>(() => ({
  gateway: {
    auth: {
      token: "config-token",
    },
...
function stubServiceGatewayTokenEnv() {
  service.readCommand.mockResolvedValue({
    programArguments: [],
    environment: {
      RECALL_GATEWAY_TOKEN: "service-token",
...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { RECALL_GATEWAY_TOKEN: "service-token" },
    });
    stubEmptyGatewayEnv();
  });
  ...
  it("prints the container restart hint when restart is requested for a not-loaded service", async () => {
    ...
    vi.stubEnv("RECALL_CONTAINER_HINT", "recall-demo-container");
    ...
    expect(runtimeLogs).toContain(
      "Start with: Restart the container or the service that manages it for recall-demo-container.",
    );
  });
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { RECALL_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("RECALL_GATEWAY_TOKEN", "env-token");
  ...
  it("repairs stale loaded services during start before reporting success", async () => {
    service.readCommand.mockResolvedValue({
      programArguments: ["recall", "gateway"],
      environment: { RECALL_SERVICE_VERSION: "2026.4.24" },
    });
    const repairLoadedService = vi.fn(async () => ({
      result: "started" as const,
      message: "Gateway service definition repaired and started.",
      warnings: ["service was installed by Recall 2026.4.24, current CLI is 2026.5.2"],
      loaded: true,
    }));
    ...
    expect(payload.warnings?.[0]).toContain("service was installed by Recall");
    expect(payload.service?.loaded).toBe(true);
  });
  ...
  it("fails start with an install hint when a stale loaded service has no repair callback", async () => {
    ...
    expect(payload.hints).toEqual(["recall gateway install --force"]);
    expect(service.restart).not.toHaveBeenCalled();
  });
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    await runServiceStart({
      serviceNoun: "Gateway",
      service,
      renderStartHints: () => ["recall gateway install"],
      opts: { json: true },
    });
    ...
    expect(payload.hints?.includes("recall gateway install")).toBe(true);
    expect(
      payload.hintItems?.some(
        (item) => item.kind === "install" && item.text === "recall gateway install",
      ),
    ).toBe(true);
    expect(service.restart).not.toHaveBeenCalled();
  });
