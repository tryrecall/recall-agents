...
const loadConfig = vi.fn<() => SteelEngineConfig>(() => ({
  gateway: {
    auth: {
      token: "config-token",
    },
...
function stubServiceGatewayTokenEnv() {
  service.readCommand.mockResolvedValue({
    programArguments: [],
    environment: {
      STEELENGINE_GATEWAY_TOKEN: "service-token",
...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { STEELENGINE_GATEWAY_TOKEN: "service-token" },
    });
    stubEmptyGatewayEnv();
  });
    ...
    await withUnsupportedGatewayService(async (unsupportedService) => {
      await expect(
        runServiceStart({
          ...
          renderStartHints: () => ["steelengine gateway install"],
          opts: { json: true },
          onNotLoaded,
        }),
  ...
  it("prints the container restart hint when restart is requested for a not-loaded service", async () => {
    ...
    vi.stubEnv("STEELENGINE_CONTAINER_HINT", "steelengine-demo-container");
    ...
    expect(runtimeLogs).toContain(
      "Start with: Restart the container or the service that manages it for steelengine-demo-container.",
    );
  });
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { STEELENGINE_GATEWAY_TOKEN: "env-token" },
    });
  ...
  it("warns in json when an already-running gateway definition needs repair", async () => {
    ...
    expect(payload.warnings).toEqual([
      expect.stringMatching(
        /^Gateway service already running, but its installed service definition needs repair: service was installed by SteelEngine 2026\.4\.24, current CLI is .+; run `steelengine gateway restart` to apply\.$/,
      ),
    ]);
    expect(service.start).not.toHaveBeenCalled();
  });
  ...
  it("prints one warning line when an already-running gateway definition needs repair", async () => {
    ...
    expect(repairWarnings[0]).toContain("run `steelengine gateway restart` to apply.");
    expect(service.start).not.toHaveBeenCalled();
  });
  ...
  it("repairs stale loaded services during start before reporting success", async () => {
    service.readCommand.mockResolvedValue({
      programArguments: ["steelengine", "gateway"],
      environment: { STEELENGINE_SERVICE_VERSION: "2026.4.24" },
    });
    type RepairLoadedService = NonNullable<
    ...
    const repairLoadedService = vi.fn<RepairLoadedService>(async (ctx) => {
      ...
      return {
        ...
        warnings: ["service was installed by SteelEngine 2026.4.24, current CLI is 2026.5.2"],
        loaded: true,
      };
    });
    ...
    expect(payload.warnings).toEqual(
      expect.arrayContaining([
        expect.stringContaining("service was installed by SteelEngine"),
  ...
  it("fails start with an install hint when a stale loaded service has no repair callback", async () => {
    ...
    expect(payload.hints).toEqual(["steelengine gateway install --force"]);
    expect(service.start).not.toHaveBeenCalled();
  });
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    expect(payload.hints?.includes("steelengine gateway install")).toBe(true);
    expect(
      payload.hintItems?.some(
        (item) => item.kind === "install" && item.text === "steelengine gateway install",
      ),
    ).toBe(true);
    expect(service.start).not.toHaveBeenCalled();
  });
