...
export async function repairLoadedGatewayServiceForStart(
  ...
  const wrapperPath = await resolveSteelEngineWrapperPath(installEnv[STEELENGINE_WRAPPER_ENV_KEY]);
  const installedPort =
    parseTcpPortFromArgs(params.state.command?.programArguments) ??
    parseTcpPort(params.state.command?.environment?.STEELENGINE_GATEWAY_PORT);
  ...
  return {
    ...
    message:
      params.action === "restart"
        ...
        : "Gateway service definition repaired and started. Reopen the Control UI with `steelengine dashboard` or copy a fresh auth URL with `steelengine dashboard --no-open`.",
