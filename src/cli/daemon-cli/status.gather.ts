...
function resolveSnapshotRuntimeConfig(snapshot: ConfigFileSnapshot | null): RecallConfig | null {
...
function coerceStatusConfig(value: unknown): RecallConfig {
...
export type DaemonStatus = {
  ...
  service: {
    ...
    staleUpdateLaunchdJobs?: StaleRecallUpdateLaunchdJob[];
  };
  config?: {
    cli: ConfigSummary;
  ...
  const staleUpdateLaunchdJobs =
    opts.deep && process.platform === "darwin"
      ? await loadLaunchdModule()
          .then(({ findStaleRecallUpdateLaunchdJobs }) => findStaleRecallUpdateLaunchdJobs())
          .catch(() => [])
      : [];
