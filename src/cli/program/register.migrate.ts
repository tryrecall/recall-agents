...
export function registerMigrateCommand(program: Command) {
  const migrate = addVerifyPluginAppsOption(
    program
      ...
      .option("--no-backup", "Skip the pre-migration Recall backup")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["recall migrate list", "Show available migration providers."],
          ["recall migrate hermes", "Preview Hermes migration, then prompt before applying."],
          ["recall migrate hermes --dry-run", "Preview Hermes migration only."],
          [
  ...
  addMigrationOptions(
    migrate
      ...
      .description("Preview a migration without changing Recall state"),
  ).action(async (provider, opts) => {
    await runCommandWithRuntime(defaultRuntime, async () => {
