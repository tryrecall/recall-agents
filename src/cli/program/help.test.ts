...
describe("configureProgramHelp", () => {
  ...
  beforeEach(() => {
    ...
    originalSuppressHelpBanner = process.env.RECALL_SUPPRESS_HELP_BANNER;
  ...
  afterEach(() => {
    ...
    if (originalSuppressHelpBanner === undefined) {
      ...
      process.env.RECALL_SUPPRESS_HELP_BANNER = originalSuppressHelpBanner;
    }
  });
  ...
  it("includes banner and docs/examples in root help output", () => {
    ...
    expect(help).toContain("https://docs.recall.ai/cli");
  });

  it("suppresses banner formatting when parent default help requests it", () => {
    process.argv = ["node", "recall", "channels"];
  ...
  it("prints version and exits immediately when version flags are present", () => {
    process.argv = ["node", "recall", "--version"];
    expectVersionExit({ expectedVersion: "Recall 9.9.9-test (abc1234)" });
  });

  it("prints version and exits immediately without commit metadata", () => {
    process.argv = ["node", "recall", "--version"];
    resolveCommitHashMock.mockReturnValue(null);
    expectVersionExit({ expectedVersion: "Recall 9.9.9-test" });
  });
});
