...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["steelengine status", "Show channel health + session summary."],
          ["steelengine status --all", "Full diagnosis (read-only)."],
          ["steelengine status --json", "Machine-readable output."],
          ["steelengine status --usage", "Show model provider usage/quota snapshots."],
          [
            "steelengine status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["steelengine status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.steelengine.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.steelengine.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
  ...
  const sessionsCmd = addSessionsListOptions(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["steelengine sessions", "List all sessions."],
          ["steelengine sessions --agent work", "List sessions for one agent."],
          ["steelengine sessions --all-agents", "Aggregate sessions across agents."],
          ["steelengine sessions --active 120", "Only last 2 hours."],
          ["steelengine sessions --limit 25", "Show the newest 25 sessions."],
          ["steelengine sessions --json", "Machine-readable output."],
          ["steelengine sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.steelengine.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["steelengine sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "steelengine sessions cleanup --dry-run --fix-missing",
          ...
          ["steelengine sessions cleanup --enforce", "Apply maintenance now."],
          ["steelengine sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["steelengine sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "steelengine sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .steelengine/trajectory-exports")
  ...
  sessionsCmd
    ...
    .action(async (key: string, opts, command) => {
      ...
      // command, e.g. `steelengine sessions --agent work compact <key>`. Merge those
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["steelengine commitments", "List pending inferred follow-ups."],
          ["steelengine commitments --all", "List all inferred follow-ups."],
          ["steelengine commitments --agent work", "List one agent's inferred follow-ups."],
          ["steelengine commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
