...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.RECALL_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.RECALL_DISABLE_LAZY_SUBCOMMANDS);
};

export const loadValidatedConfigForPluginRegistration =
  async (): Promise<RecallConfig | null> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live Recall docs",
    hasSubcommands: false,
    register: async (program) => {
  ...
  {
    name: "plugins",
    description: "Manage Recall plugins and extensions",
    hasSubcommands: true,
    register: async (program) => {
