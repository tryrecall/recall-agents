...
export function registerOnboardCommand(program: Command): void {
  const command = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/onboard", "docs.steelengine.ai/cli/onboard")}\n`,
    )
    .option(
      "--workspace <dir>",
    ...
    .option("--modern", "Open inference-gated SteelEngine (kept for compatibility)", false)
        ...
        if (unsupportedOptions.length > 0) {
          defaultRuntime.error(
            [
              ...
              "Run those setup options without --modern, or remove them to open SteelEngine.",
            ].join("\n"),
          );
        ...
        if (opts.nonInteractive && opts.acceptRisk !== true) {
          defaultRuntime.error(
            [
              ...
              `Re-run with: ${formatCliCommand("steelengine onboard --modern --non-interactive --accept-risk ...")}`,
            ].join("\n"),
          );
          defaultRuntime.exit(1);
          return;
