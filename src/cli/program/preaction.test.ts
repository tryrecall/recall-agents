...
import { repoInstallSpec } from "recall/plugin-sdk/test-fixtures";
...
beforeEach(() => {
  ...
  originalHideBanner = process.env.RECALL_HIDE_BANNER;
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.RECALL_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  it("handles debug mode and config-only command preaction", async () => {
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "recall", "status", "--debug"],
    });
    ...
    expect(processTitleSetSpy).toHaveBeenCalledWith("recall-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "recall", "agents", "list"],
    });
  ...
  it("loads plugins for text local agent runs", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "recall", "agent", "--local", "--message", "hi"],
    });
  ...
  it("loads plugins for json local agent runs", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "recall", "agent", "--local", "--message", "hi", "--json"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "recall", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "recall", "channels", "add"],
    });
  ...
  it("skips startup bootstrap for parent default help actions", async () => {
    await runPreAction({
      parseArgv: ["channels"],
      processArgv: ["node", "recall", "channels"],
    });
  ...
  it("lets configure own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["configure"],
      processArgv: ["node", "recall", "configure"],
    });
  ...
  it("lets bare config own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["config"],
      processArgv: ["node", "recall", "config"],
    });
  ...
  it("lets guided config sections own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["config"],
      processArgv: ["node", "recall", "config", "--section", "models"],
    });
  ...
  it("skips the config guard and plugin loading for doctor lint", async () => {
    await runPreAction({
      parseArgv: ["doctor"],
      processArgv: ["node", "recall", "doctor", "--lint"],
    });
  ...
  it("only allows invalid config for explicit official recovery reinstall requests", async () => {
    await runPreAction({
      parseArgv: ["plugins", "install", "@recall/discord"],
      processArgv: ["node", "recall", "plugins", "install", "@recall/discord"],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@recall/brave-plugin"],
      processArgv: ["node", "recall", "plugins", "install", "@recall/brave-plugin"],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@recall/slack"],
      processArgv: ["node", "recall", "plugins", "install", "@recall/slack"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "recall", "plugins", "install", "alpha"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "recall", "plugins", "install", DISCORD_REPO_INSTALL_SPEC],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@recall/discord", "--marketplace", "local/repo"],
      processArgv: [
        "node",
        "recall",
        "plugins",
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "recall", "--version"],
    });
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "recall", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "recall", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "recall", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "recall", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "recall", "channels", "send", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "recall", "config", "set", "gateway.auth.mode", "local", "--json"],
    });
  ...
  it("does not preload plugins for agents list JSON output", async () => {
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "recall", "agents", "list", "--json"],
    });
  ...
  it("does not preload plugins for remote agent JSON output", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "recall", "agent", "--message", "hi", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "recall", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "recall", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for config schema", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "recall", "config", "schema"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "recall", "backup", "create", "--json"],
    });
