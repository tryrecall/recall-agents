...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.RECALL_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.RECALL_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.RECALL_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.RECALL_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.RECALL_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.RECALL_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.RECALL_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "recall", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "recall"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "recall", "nodes", "list"], "recall");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "recall", "infer"], "recall");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "recall", "acp", "--help"], "recall");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "recall", "gateway", "--force"];
    process.argv = argv;
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "recall", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("recall");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "recall", "completion", "--write-state"];
    const program = new Command().name("recall");
  ...
  it("does not preload plugin CLI registrations for bare plugin parent help", async () => {
    process.argv = ["node", "recall", "plugins"];
    const program = new Command().name("recall");
