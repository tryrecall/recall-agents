...
describe("private-qa-cli", () => {
  ...
  it("loads the private QA CLI from a source checkout path", async () => {
    ...
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-private-qa-source-"));
  ...
  it("loads the private QA CLI from a raw synced source checkout path", async () => {
    process.env.STEELENGINE_ENABLE_PRIVATE_QA_CLI = "1";
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-private-qa-raw-source-"));
    tempDirs.push(repoRoot);
    const expectedPaths = new Set([
  ...
  it("rejects non-source package roots even when private QA is enabled", () => {
    process.env.STEELENGINE_ENABLE_PRIVATE_QA_CLI = "1";
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-private-qa-"));
    tempDirs.push(root);
    fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "steelengine" }), "utf8");
    const importModule = vi.fn(async () => ({}));
    ...
    ).toThrow("Private QA CLI is only available from an SteelEngine source checkout.");
  ...
  it("rejects when the private QA env flag is disabled", () => {
    ...
    expect(() => loadPrivateQaCliModule({ importModule })).toThrow(
      "Private QA CLI is only available from an SteelEngine source checkout.",
    );
    expect(importModule).not.toHaveBeenCalled();
  });
});
