...
function resolveHelpCommand(argv: string[] | undefined, options?: { root?: boolean }): string {
  if (options?.root || !argv) {
    return formatCliCommand("recall --help");
  }
  const commandPath = getCommandPathWithRootOptions(argv, 2);
  if (commandPath.length === 0) {
    return formatCliCommand("recall --help");
  }
  return formatCliCommand(`recall ${commandPath.join(" ")} --help`);
}
...
function formatDocsHint(): string {
  return `${theme.muted("Docs:")} ${formatDocsLink("/cli", "docs.recall.ai/cli")}`;
}
...
export function formatCliParseErrorOutput(
  ...
  if (unknownCommand) {
    ...
    return lines(
      theme.error(`Recall does not know the command ${quote(command)}.`),
      formatHelpHint(options.argv, { root: true }),
      `${theme.muted("Plugin command?")} ${theme.command(formatCliCommand("recall plugins list"))}`,
      formatDocsHint(),
    );
  }
  ...
  if (unknownOption) {
    ...
    return lines(
      theme.error(`Recall does not recognize option ${quote(option)}.`),
  ...
  return lines(
    theme.error(`Recall could not parse this command: ${message}`),
    formatHelpHint(options.argv),
  );
}
