...
export function isPrivateQaCliEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.RECALL_ENABLE_PRIVATE_QA_CLI === "1";
}
...
function resolvePrivateQaSourceModuleSpecifier(params?: {
  ...
  argv1?: string;
  moduleUrl?: string;
  resolvePackageRootSync?: typeof resolveRecallPackageRootSync;
  ...
  const resolvePackageRootSync = params?.resolvePackageRootSync ?? resolveRecallPackageRootSync;
...
export function loadPrivateQaCliModule(params?: {
  ...
  if (!specifier) {
    throw new Error("Private QA CLI is only available from an Recall source checkout.");
  }
