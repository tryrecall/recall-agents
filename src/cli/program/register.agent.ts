...
${formatHelpExamples([
  ['recall agent --to +15555550123 --message "status update"', "Start a new session."],
  ['recall agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['recall agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'recall agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.recall.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.recall.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['recall agents set-identity --agent main --name "Recall" --emoji "🤖"', "Set name + emoji."],
  ["recall agents set-identity --agent main --avatar avatars/recall.png", "Set avatar path."],
  [
    "recall agents set-identity --workspace ~/.recall/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "recall agents set-identity --identity-file ~/.recall/workspace/IDENTITY.md --agent main",
