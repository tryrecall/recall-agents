...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.recall${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.RECALL_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.RECALL_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.RECALL_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.RECALL_CONFIG_PATH)) {
    env.RECALL_CONFIG_PATH = path.join(stateDir, "recall.json");
  }

  if (profile === "dev" && !env.RECALL_GATEWAY_PORT?.trim()) {
    env.RECALL_GATEWAY_PORT = "19001";
  }
}
