...
describe("plugins search command", () => {
  ...
  it("searches ClawHub code and bundle plugin families", async () => {
      ...
      .mockResolvedValueOnce([
        {
          score: 10,
          package: {
            name: "steelengine-calendar-bundle",
    ...
    expect(mocks.logs.join("\n")).toContain("steelengine-calendar");
    expect(mocks.logs.join("\n")).toContain(
      "Install: steelengine plugins install clawhub:steelengine-calendar",
    );
  });
