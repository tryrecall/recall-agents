...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "recall", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "recall", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "recall", "--profile", "work", "gateway", "status"],
