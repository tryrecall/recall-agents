...
function useBundledPluginDirOverrideForTest(dir: string): void {
  process.env.RECALL_BUNDLED_PLUGINS_DIR = dir;
...
afterEach(() => {
  ...
  if (originalBundledPluginsDir === undefined) {
    ...
    process.env.RECALL_BUNDLED_PLUGINS_DIR = originalBundledPluginsDir;
  }
...
describe("plugin-sdk facade runtime", () => {
  it("honors trusted bundled plugin dir overrides", () => {
    const overrideA = createBundledPluginDir("recall-facade-runtime-a-", "override-a");
    const overrideB = createBundledPluginDir("recall-facade-runtime-b-", "override-b");
  ...
  it("falls back to package source surfaces when an override dir is partial", () => {
    const overrideDir = createTrustedBundledFixtureRoot("recall-facade-runtime-empty-");
  ...
  it("returns the same object identity on repeated calls (sentinel consistency)", () => {
    const dir = createBundledPluginDir("recall-facade-identity-", "identity-check");
  ...
  it("breaks circular facade re-entry during module evaluation", () => {
    const dir = createBundledPluginDir("recall-facade-circular-", "circular-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("back-fills the sentinel before post-load facade tracking re-enters", () => {
    const dir = createBundledPluginDir("recall-facade-post-load-", "post-load-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("clears the cache on load failure so retries re-execute", () => {
    const dir = createThrowingPluginDir("recall-facade-throw-");
    useBundledPluginDirOverrideForTest(dir);
  ...
  it("allows runtime-api facade loads when the bundled plugin is explicitly enabled", () => {
    const dir = createTempDirSync("recall-facade-runtime-enabled-");
  ...
  it("resolves a globally-installed plugin whose rootDir basename matches the dirName", () => {
    const lineDir = createTempDirSync("recall-facade-global-line-");
    ...
    fs.writeFileSync(
      path.join(lineDir, "recall.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
  ...
  it("resolves a globally-installed plugin public surface from package dist", () => {
    const lineDir = createTempDirSync("recall-facade-global-line-dist-");
  ...
  it("resolves a globally-installed plugin with an encoded scoped rootDir basename", () => {
    const encodedDir = createTempDirSync("recall-facade-encoded-line-");
    ...
    fs.writeFileSync(
      path.join(encodedDir, "recall.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
        configSchema: { type: "object", additionalProperties: false, properties: {} },
  ...
  it("prefers the source runtime snapshot for facade activation checks", () => {
    const dir = createTempDirSync("recall-facade-source-snapshot-");
    ...
    fs.writeFileSync(
      path.join(dir, "demo", "recall.plugin.json"),
      JSON.stringify({
        id: "demo",
      }),
      "utf8",
