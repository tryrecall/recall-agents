...
export { VERSION as STEELENGINE_VERSION } from "../version.js";
...
export async function detectAndLoadAgentHarnessPromptImages(params: {
  ...
  const [{ resolveImageSanitizationLimits }, { detectAndLoadPromptImages }, { MAX_IMAGE_BYTES }] =
    await Promise.all([
      ...
      import("@steelengine/media-core/constants"),
    ]);
...
/**
 * Derive the same compact user-facing tool detail that embedded SteelEngine uses for progress logs.
 */
 ...
 * preserve the same SteelEngine-owned fallback signals as the built-in SteelEngine path
