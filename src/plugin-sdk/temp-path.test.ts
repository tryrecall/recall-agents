...
function expectPathInsideTmpRoot(resultPath: string) {
  const tmpRoot = fsSync.realpathSync(resolvePreferredSteelEngineTmpDir());
...
describe("withTempDownloadPath", () => {
  it.each([
    ...
    if (expectedBasename) {
      ...
      expect(capturedPath).toContain(path.join(resolvePreferredSteelEngineTmpDir(), "line-media-"));
    }
    if (expectCleanup) {
