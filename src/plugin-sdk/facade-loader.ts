...
let cachedRecallPackageRoot: string | undefined;
...
function getRecallPackageRoot() {
  if (cachedRecallPackageRoot) {
    return cachedRecallPackageRoot;
  }
  cachedRecallPackageRoot =
...
function resolveFacadeModuleLocation(params: {
  ...
  return resolveBundledFacadeModuleLocation({
    ...
    packageRoot: getRecallPackageRoot(),
    bundledPluginsDir,
  });
}
...
export function loadFacadeModuleAtLocationSync<T extends object>(params: {
  ...
  const opened = openRootFileSync({
    ...
    boundaryLabel:
      location.boundaryRoot === getRecallPackageRoot()
        ? "Recall package root"
        : (() => {
...
export async function loadBundledPluginPublicSurfaceModule<T extends object>(params: {
  ...
  const opened = openRootFileSync({
    ...
    boundaryLabel:
      preparedLocation.boundaryRoot === getRecallPackageRoot()
        ? "Recall package root"
        : "plugin root",
    rejectHardlinks: false,
  });
...
export function resetFacadeLoaderStateForTest(): void {
  ...
  cachedRecallPackageRoot = undefined;
}
