...
export function requireRuntimeConfig(config: SteelEngineConfig, context: string): SteelEngineConfig {
  if (config) {
    return config;
  }
  throw new Error(
...
export function resolveLivePluginConfigObject(
  runtimeConfigLoader: (() => SteelEngineConfig | undefined) | undefined,
  pluginId: string,
