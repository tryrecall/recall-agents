...
const STEELENGINE_SOURCE_EXTENSIONS_ROOT = path.resolve(STEELENGINE_PACKAGE_ROOT, "extensions");
...
function resolveFacadeModuleLocationUncached(params: {
  ...
  if (!areBundledPluginsDisabled(env)) {
    ...
    const bundledLocation = resolveBundledFacadeModuleLocation({
      ...
      packageRoot: STEELENGINE_PACKAGE_ROOT,
      bundledPluginsDir,
    });
    if (bundledLocation) {
...
function buildFacadeActivationCheckParams(
  ...
  return {
    ...params,
    location,
    sourceExtensionsRoot: STEELENGINE_SOURCE_EXTENSIONS_ROOT,
    resolutionKey: createFacadeResolutionKey(params),
  };
}
