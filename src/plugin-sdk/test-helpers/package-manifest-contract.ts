    ...
    if (minHostVersionBaseline) {
      it("declares a parseable minHostVersion floor at or above the baseline", () => {
        ...
        const requirement = parseMinHostVersionRequirement(
          manifest.steelengine?.install?.minHostVersion ?? null,
        );

        expect(
          requirement,
          `${packagePath} should declare steelengine.install.minHostVersion`,
        ).not.toBeNull();
        if (!requirement) {
          return;
        }
        ...
        expect(
          ...
          `${packagePath} should require at least SteelEngine ${minHostVersionBaseline}`,
        ).toBe(true);
      });
    }
  });
