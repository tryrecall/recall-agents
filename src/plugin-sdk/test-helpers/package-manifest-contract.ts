    ...
    if (minHostVersionBaseline) {
      it("declares a parseable minHostVersion floor at or above the baseline", () => {
        ...
        const requirement = parseMinHostVersionRequirement(
          manifest.recall?.install?.minHostVersion ?? null,
        );

        expect(
          requirement,
          `${packagePath} should declare recall.install.minHostVersion`,
        ).not.toBeNull();
        if (!requirement) {
          return;
        }
        ...
        expect(
          ...
          `${packagePath} should require at least Recall ${minHostVersionBaseline}`,
        ).toBe(true);
      });
    }
  });
