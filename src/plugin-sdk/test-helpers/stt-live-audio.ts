...
export const RECALL_LIVE_TRANSCRIPT_MARKER_RE = /open(?:claw|cl|flaw|clar|core)/;

export function expectRecallLiveTranscriptMarker(value: string): void {
  expect(normalizeTranscriptForMatch(value)).toMatch(RECALL_LIVE_TRANSCRIPT_MARKER_RE);
}
...
export async function runRealtimeSttLiveTest(params: {
  ...
  const errors: Error[] = [];
  const expected = params.expectedNormalizedText ?? RECALL_LIVE_TRANSCRIPT_MARKER_RE;
