...
const ORIGINAL_ENV = {
  ...
  RECALL_CONFIG_PATH: process.env.RECALL_CONFIG_PATH,
  RECALL_STATE_DIR: process.env.RECALL_STATE_DIR,
  RECALL_TEST_FAST: process.env.RECALL_TEST_FAST,
} as const;
...
describe("plugin-sdk qa-runner-runtime linked plugin smoke", () => {
  ...
  it("loads an activated qa runner from a linked plugin path without a bundled install fallback", async () => {
    const stateDir = makeTempDir("recall-qa-runner-state-");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@recall/qa-linked",
        type: "module",
        recall: {
          extensions: ["./index.js"],
          install: {
            npmSpec: "@recall/qa-linked",
          },
        },
      }),
      "utf8",
