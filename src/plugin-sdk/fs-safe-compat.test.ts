...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "steelengine/plugin-sdk/core";
import {
  readFileWithinRoot,
  removePathWithinRoot,
  writeFileWithinRoot,
} from "steelengine/plugin-sdk/file-access-runtime";
import {
  loadSecretFileSync,
  type SecretFileReadResult,
} from "steelengine/plugin-sdk/secret-file-runtime";
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTempDir({ prefix: "steelengine-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps root-bounded file-access helpers on file-access-runtime", async () => {
    await withTempDir({ prefix: "steelengine-sdk-file-access-compat-" }, async (root) => {
