...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "recall/plugin-sdk/core";
import { readFileWithinRoot, writeFileWithinRoot } from "recall/plugin-sdk/file-access-runtime";
import {
  loadSecretFileSync,
  type SecretFileReadResult,
} from "recall/plugin-sdk/secret-file-runtime";
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTempDir({ prefix: "recall-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps deprecated root-bounded read/write helpers on file-access-runtime", async () => {
    await withTempDir({ prefix: "recall-sdk-file-access-compat-" }, async (root) => {
