...
const resolveRecallPackageRootSync = vi.hoisted(() => vi.fn());
...
describe("plugin-sdk qa-runner-runtime", () => {
  ...
  const originalPrivateQaCli = process.env.RECALL_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    ...
    resolveRecallPackageRootSync.mockReset().mockReturnValue(null);
  ...
  it("loads bundled plugin test APIs with the private QA source tree override", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "recall-qa-test-api-root-");
    resolveRecallPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(testApiCall?.env?.RECALL_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(testApiCall?.env?.RECALL_BUNDLED_PLUGINS_DIR).toBe(
  ...
  it("prefers the source bundled tree for private qa discovery in repo checkouts", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "recall-qa-runner-root-");
    resolveRecallPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(manifestCall?.env?.RECALL_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(manifestCall?.env?.RECALL_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
    ...
    expect(publicSurfaceCall?.env?.RECALL_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(publicSurfaceCall?.env?.RECALL_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
  });
  ...
  it("fails when runtime registrations include an undeclared command", async () => {
    ...
    expect(() => module.listQaRunnerCliContributions()).toThrow(
      'QA runner plugin "qa-matrix" exported "extra" from runtime-api.js but did not declare it in recall.plugin.json',
    );
  });
});
