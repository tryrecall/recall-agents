...
const LIVE =
  isLiveTestEnabled(["STEELENGINE_LIVE_SKILL_HISTORY_SCAN"]) &&
...
function liveConfig(): SteelEngineConfig {
  const modelId = process.env.STEELENGINE_LIVE_SKILL_HISTORY_MODEL ?? "gpt-5.6-luna";
  return {
    models: {
      providers: {
        openai: {
...
describeLive("Skill Workshop history scan live OpenAI eval", () => {
  beforeAll(async () => {
    testState = await createSteelEngineTestState({
      layout: "state-only",
      prefix: "steelengine-live-skill-history-state-",
    });
    workspaceDir = await tempDirs.make("steelengine-live-skill-history-workspace-");
  });
