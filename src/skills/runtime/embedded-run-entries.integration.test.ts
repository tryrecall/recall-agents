...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("steelengine-bundled-");
  const workspaceDir = await createTempDir("steelengine-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: SteelEngineConfig) {
