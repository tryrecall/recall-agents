    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "steelengine.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "steelengine.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "steelengine.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("steelengine-");
  const acpxRoot = await tempDirs.make("steelengine-acpx-plugin-");
  const helperRoot = await tempDirs.make("steelengine-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("steelengine-");
  const pluginRoot = await tempDirs.make("steelengine-plugin-");
  const outsideDir = await tempDirs.make("steelengine-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("steelengine-");
    const pluginRoot = await tempDirs.make("steelengine-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("steelengine-");
    const pluginRoot = await tempDirs.make("steelengine-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
