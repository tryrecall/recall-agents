...
describe("buildWorkspaceSkillStatus", () => {
  it("reports blank env requirements as missing", () => {
    const envName = "STEELENGINE_TEST_BLANK_SKILL_STATUS";
  ...
  it("surfaces valid ClawHub linkage and local Skill Card metadata", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-skill-status-"));
    try {
  ...
  it("links a discovered global ClawHub skill only through the managed lockfile", async () => {
    const managedParentDir = tempDirs.make("steelengine-managed-");
    const workspaceDir = tempDirs.make("steelengine-skill-status-");
    ...
    expect(skill).toMatchObject({ source: "steelengine-managed" });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const managedParentDir = tempDirs.make("steelengine-managed-");
      const externalSkillDir = tempDirs.make("steelengine-skill-target-");
      const workspaceDir = tempDirs.make("steelengine-skill-status-");
      const managedSkillsDir = path.join(managedParentDir, "skills");
  ...
  it("classifies a mixed broken skill pack without flattening visibility reasons", () => {
    const missingBin = "steelengine-test-definitely-missing-skill-bin";
    const report = buildWorkspaceSkillStatus("/tmp/ws", {
      agentId: "specialist",
      config: {
        agents: {
      ...
      entries: [
        ...
        createEntry("needs-bin", {
          metadata: {
            ...
            install: [
              {
                kind: "node",
                package: "@steelengine/missing-skill-bin",
                bins: [missingBin],
              },
            ],
          },
        ...
        createEntry("needs-env", {
          metadata: {
            primaryEnv: "STEELENGINE_TEST_MISSING_SKILL_KEY",
            requires: { env: ["STEELENGINE_TEST_MISSING_SKILL_KEY"] },
          },
        }),
        ...
        createEntry("bundled-blocked", { source: "steelengine-bundled" }),
      ],
    });
    ...
    expect(needsBin.install).toStrictEqual([
      {
        kind: "node",
        id: "node-0",
        label: "Install @steelengine/missing-skill-bin (pnpm)",
        bins: [missingBin],
      },
    ]);
    const needsEnv = requireSkillStatus(byName, "needs-env");
    expect(needsEnv.eligible).toBe(false);
    expect(needsEnv.primaryEnv).toBe("STEELENGINE_TEST_MISSING_SKILL_KEY");
    expect(needsEnv.missing).toStrictEqual({
      anyBins: [],
      bins: [],
      config: [],
      env: ["STEELENGINE_TEST_MISSING_SKILL_KEY"],
      os: [],
    });
