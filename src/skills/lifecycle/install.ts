...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "steelengine");
  }
  return path.join(homedir(), ".steelengine");
}
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const scanResult = await evaluateSkillInstallPolicy({
    ...
    source:
      skillSource === "steelengine-bundled"
        ? { kind: "bundled", authority: "steelengine", mutable: false, network: false }
        : skillSource === "steelengine-managed" || skillSource === "steelengine-extra"
          ? { kind: "managed", authority: "steelengine", mutable: false, network: false }
  ...
  const trustedInstallSources = new Set(["steelengine-bundled", "steelengine-managed", "steelengine-extra"]);
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("steelengine.skillsInstallTestApi")] =
    testing;
}
