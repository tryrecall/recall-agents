...
async function installArchiveResolution(params: {
  ...
  archivePath: string;
  registry: string;
  authority: "official" | "steelengine" | "third-party";
  force?: boolean;
  logger?: Logger;
  config?: SteelEngineConfig;
}) {
  return await withExtractedArchiveRoot({
    ...
    tempDirPrefix: "steelengine-skill-clawhub-",
    timeoutMs: 120_000,
...
async function installGitHubResolution(params: {
  ...
  return await withExtractedArchiveRoot({
    archivePath: params.archivePath,
    tempDirPrefix: "steelengine-skill-clawhub-github-",
    timeoutMs: 120_000,
...
function assertInstallResolutionAllowed(
  ...
  if (!resolution.ok) {
    if (resolution.reason === "ambiguous_slug") {
      ...
      throw new Error(
        `Skill "${resolution.slug}" is ambiguous on ClawHub. Install an owner-qualified skill, for example: steelengine skills install @owner/${resolution.slug}.${message}`,
      );
    }
