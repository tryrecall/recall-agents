...
import { loadRecallProviderIndex, normalizeRecallProviderIndex } from "./index.js";

describe("Recall provider index", () => {
  it("normalizes provider preview catalog rows through model catalog validation", () => {
    const index = normalizeRecallProviderIndex({
      version: 1,
      providers: {
        Moonshot: {
          id: "moonshot",
          ...
          plugin: {
            id: "moonshot",
            package: " @recall/plugin-moonshot ",
            install: {
              clawhubSpec: " clawhub:recall/moonshot@2026.5.2 ",
              npmSpec: " @recall/plugin-moonshot@1.2.3 ",
  ...
  it("loads the bundled provider index without runtime plugin loading", () => {
    const index = loadRecallProviderIndex();
