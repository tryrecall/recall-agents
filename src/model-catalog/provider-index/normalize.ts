...
const RECALL_PROVIDER_INDEX_VERSION = 1;
...
function normalizeInstall(value: unknown): RecallProviderIndexPluginInstall | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function normalizePlugin(value: unknown): RecallProviderIndexPlugin | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function normalizeOnboardingScopes(
  value: unknown,
): RecallProviderIndexProviderAuthChoice["onboardingScopes"] | undefined {
...
function normalizeAssistantVisibility(
  value: unknown,
): RecallProviderIndexProviderAuthChoice["assistantVisibility"] | undefined {
...
function normalizeAuthChoice(params: {
  ...
  providerName: string;
  value: unknown;
}): RecallProviderIndexProviderAuthChoice | undefined {
...
function normalizeAuthChoices(params: {
  ...
  providerName: string;
  value: unknown;
}): readonly RecallProviderIndexProviderAuthChoice[] | undefined {
  if (!Array.isArray(params.value)) {
    return undefined;
  }
  const choices = params.value
    .map((value) => normalizeAuthChoice({ ...params, value }))
    .filter((choice): choice is RecallProviderIndexProviderAuthChoice => Boolean(choice));
...
function normalizeProvider(
  ...
  value: unknown,
): RecallProviderIndexProvider | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
export function normalizeRecallProviderIndex(value: unknown): RecallProviderIndex | undefined {
  if (!isRecord(value) || value.version !== RECALL_PROVIDER_INDEX_VERSION) {
    return undefined;
  }
  if (!isRecord(value.providers)) {
    return undefined;
  }
  const providers: Record<string, RecallProviderIndexProvider> = {};
  ...
  return {
    version: RECALL_PROVIDER_INDEX_VERSION,
