...
import type {
  ...
  SteelEngineProviderIndexPluginInstall,
  SteelEngineProviderIndexPlugin,
  SteelEngineProviderIndexProviderAuthChoice,
  SteelEngineProviderIndexProvider,
} from "./types.js";
...
const STEELENGINE_PROVIDER_INDEX_VERSION = 1;
...
function normalizeInstall(value: unknown): SteelEngineProviderIndexPluginInstall | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function normalizePlugin(value: unknown): SteelEngineProviderIndexPlugin | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function normalizeOnboardingScopes(
  value: unknown,
): SteelEngineProviderIndexProviderAuthChoice["onboardingScopes"] | undefined {
...
function normalizeAssistantVisibility(
  value: unknown,
): SteelEngineProviderIndexProviderAuthChoice["assistantVisibility"] | undefined {
...
function normalizeAuthChoice(params: {
  ...
  providerName: string;
  value: unknown;
}): SteelEngineProviderIndexProviderAuthChoice | undefined {
...
function normalizeAuthChoices(params: {
  ...
  providerName: string;
  value: unknown;
}): readonly SteelEngineProviderIndexProviderAuthChoice[] | undefined {
  if (!Array.isArray(params.value)) {
    return undefined;
  }
  const choices = params.value
    .map((value) => normalizeAuthChoice({ ...params, value }))
    .filter((choice): choice is SteelEngineProviderIndexProviderAuthChoice => Boolean(choice));
...
function normalizeProvider(
  ...
  value: unknown,
): SteelEngineProviderIndexProvider | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
export function normalizeSteelEngineProviderIndex(value: unknown): SteelEngineProviderIndex | undefined {
  if (!isRecord(value) || value.version !== STEELENGINE_PROVIDER_INDEX_VERSION) {
    return undefined;
  }
  if (!isRecord(value.providers)) {
    return undefined;
  }
  const providers: Record<string, SteelEngineProviderIndexProvider> = {};
  ...
  return {
    version: STEELENGINE_PROVIDER_INDEX_VERSION,
