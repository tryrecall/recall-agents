...
function listAgents(cfg: RecallConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: RecallConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<RecallConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: RecallConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: RecallConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: RecallConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<RecallConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  RecallConfig,
  {
    bindingsRef: RecallConfig["bindings"];
    agentsRef: RecallConfig["agents"];
    sessionRef: RecallConfig["session"];
...
function resolveRouteCacheForConfig(cfg: RecallConfig): Map<string, ResolvedAgentRoute> {
