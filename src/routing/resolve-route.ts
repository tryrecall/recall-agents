...
function listAgents(cfg: SteelEngineConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: SteelEngineConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<SteelEngineConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: SteelEngineConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: SteelEngineConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: SteelEngineConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<SteelEngineConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  SteelEngineConfig,
  {
    bindingsRef: SteelEngineConfig["bindings"];
    agentsRef: SteelEngineConfig["agents"];
    sessionRef: SteelEngineConfig["session"];
...
function resolveRouteCacheForConfig(cfg: SteelEngineConfig): Map<string, ResolvedAgentRoute> {
