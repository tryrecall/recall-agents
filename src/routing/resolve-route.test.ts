...
const resolveRoute = (
  params: Omit<Parameters<typeof resolveAgentRoute>[0], "cfg"> & { cfg?: SteelEngineConfig },
) =>
  resolveAgentRoute({
    cfg: params.cfg ?? {},
    ...params,
...
describe("resolveAgentRoute", () => {
  ...
  const expectRouteResolutionCase = (params: {
    routeParams: Omit<Parameters<typeof resolveRoute>[0], "cfg"> & { cfg: SteelEngineConfig };
    expected: ResolvedRouteExpectation;
  }) => {
  ...
  test.each([
    ...
    },
    {
      ...
      cfg: {
        agents: {
          list: [{ id: "home", default: true, workspace: "~/steelengine-home" }],
        },
      } satisfies SteelEngineConfig,
      channel: "whatsapp" as const,
      accountId: "biz",
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<SteelEngineConfig["bindings"]>[number];
