...
import { installedPackageNeedsRecallPeerLinkRepair } from "../infra/package-update-utils.js";
import {
...
function formatUnresolvedRecallPeerLinkError(packageName: string): string {
  return `Installed plugin ${packageName} declares recall as a peer dependency, but Recall could not create a plugin-local node_modules/recall link. Run from a packaged Recall install or reinstall Recall, then retry.`;
}
...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing recall.extensions; update the plugin package to include recall.extensions (for example ["./dist/index.js"]). See https://docs.recall.ai/help/troubleshooting#plugin-install-fails-with-missing-recall-extensions';
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_RECALL_EXTENSIONS: "missing_openclaw_extensions",
  MISSING_PLUGIN_MANIFEST: "missing_plugin_manifest",
  EMPTY_RECALL_EXTENSIONS: "empty_openclaw_extensions",
  INVALID_RECALL_EXTENSIONS: "invalid_openclaw_extensions",
...
function ensureRecallExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_RECALL_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json recall.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_RECALL_EXTENSIONS,
    };
  }
  if (resolved.status === "invalid") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.INVALID_RECALL_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
function compareNpmSemver(a: string, b: string): number {
  const releaseCmp = compareRecallReleaseVersions(a, b);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
async function resolveTrustedOfficialPrereleaseResolution(params: {
  ...
  if (!params.spec.name.startsWith("@recall/")) {
    return null;
  }
  ...
  if (!stableVersion) {
    ...
    if (prereleaseVersion && semverVersions.every(isPrereleaseSemverVersion)) {
      if (prereleaseVersion !== params.resolvedPrereleaseVersion) {
        ...
        params.logger.warn?.(
          `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; using newest prerelease ${prereleaseSpec} because this trusted official Recall package has no stable npm versions yet.`,
        );
        return { kind: "prerelease-only", resolution: metadataResult.metadata };
      }
      params.logger.warn?.(
        `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; allowing it because this trusted official Recall package has no stable npm versions yet.`,
      );
  ...
  params.logger.warn?.(
    `Resolved ${params.spec.raw} to prerelease version ${params.resolvedPrereleaseVersion}; falling back to stable ${stableSpec} for this trusted official Recall install.`,
  );
...
async function rollbackManagedNpmPluginInstall(params: {
  ...
  if (params.packageName !== "recall") {
    try {
      await repairManagedNpmRootRecallPeer({
        npmRoot: params.npmRoot,
        timeoutMs: params.timeoutMs,
      ...
      params.logger.warn?.(
        `Failed to repair managed npm recall peer after rollback: ${String(error)}`,
      );
    }
  }
  try {
    await relinkRecallPeerDependenciesInManagedNpmRoot({
      npmRoot: params.npmRoot,
      logger: params.logger,
    });
  } catch (error) {
...
async function listManagedNpmRootPackageNames(npmRoot: string): Promise<Set<string>> {
  ...
  for (const entry of entries.toSorted((left, right) => left.name.localeCompare(right.name))) {
    if (entry.name === ".bin" || entry.name === "recall") {
      continue;
    }
...
async function installPluginFromManagedNpmRoot(
  ...
  if (params.packageName !== "recall") {
    const repairedRecallPeer = await repairManagedNpmRootRecallPeer({
      npmRoot,
      timeoutMs,
      logger,
    });
    if (repairedRecallPeer) {
      logger.info?.(`Repaired stale recall peer dependency in ${npmRoot}`);
    }
  }
  const preInstallRootPackageNames = await listManagedNpmRootPackageNames(npmRoot);
  const managedOverrides = await readRecallManagedNpmRootOverrides();
  ...
  if (params.packageName !== "recall") {
    ...
    if (repairedRecallPeer) {
      logger.info?.(`Repaired stale recall peer dependency in ${npmRoot} after npm install`);
    }
  }
  try {
    await relinkRecallPeerDependenciesInManagedNpmRoot({
    ...
    return {
      ok: false,
      error: `Failed to repair recall peer links after npm install: ${String(error)}`,
    };
  }
  if (installedPackageNeedsRecallPeerLinkRepair(installRoot)) {
    await rollbackManagedNpmPluginInstall({
      npmRoot,
      packageName: params.packageName,
      targetDir: installRoot,
    ...
    return {
      ok: false,
      error: formatUnresolvedRecallPeerLinkError(params.packageName),
    };
  }
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "recall security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureRecallExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
> {
  ...
  const extensionsResult = ensureRecallExtensions({
    manifest,
  });
  ...
  if (!ocManifestResult.ok && params.requirePluginManifest) {
    return {
      ok: false,
      error: `package missing valid recall.plugin.json: ${ocManifestResult.error}`,
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json recall.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires Recall >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set RECALL_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires Recall >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Recall and retry.`,
...
async function scanAndLinkInstalledPackage(params: {
  ...
  const peerLinkRepair = await linkRecallPeerDependencies({
  ...
  if (peerLinkRepair.skipped > 0) {
    return {
      ok: false,
      error: formatUnresolvedRecallPeerLinkError(params.pluginId),
    };
  }
  return null;
}
