...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_RECALL_EXTENSIONS: "missing_recall_extensions",
  EMPTY_RECALL_EXTENSIONS: "empty_recall_extensions",
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    logger.warn?.(
      `Bundle "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "recall security audit --deep" after install.`,
    );
  }
...
async function installPluginFromPackageDir(
  ...
  // Prefer the canonical `id` from recall.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-recall"), the plugin registry
  ...
  try {
    ...
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "recall security audit --deep" after install.`,
    );
  }
