...
import type { RecallPluginDefinition, RecallPluginModule } from "./types.js";
...
export function buildVitestCapabilityShimAliasMap(): Record<string, string> {
  return Object.fromEntries(
    CAPABILITY_VITEST_SHIM_ALIASES.flatMap(({ subpath, target }) => {
      ...
      return [
        [`recall/plugin-sdk/${subpath}`, targetPath],
        [`@recall/plugin-sdk/${subpath}`, targetPath],
      ];
    }),
  );
...
function applyVitestCapabilityAliasOverrides(params: {
  ...
  const {
    "recall/plugin-sdk": _ignoredLegacyRootAlias,
    "@recall/plugin-sdk": _ignoredScopedRootAlias,
    ...scopedAliasMap
  } = params.aliasMap;
  return {
    ...scopedAliasMap,
...
function resolvePluginModuleExport(moduleExport: unknown): {
  ...
  if (resolved && typeof resolved === "object") {
    const definition = resolved as RecallPluginDefinition;
    return {
      definition,
...
export function loadBundledCapabilityRuntimeRegistry(params: {
  ...
  const discovery = params.discovery ?? discoverRecallPlugins({ env });
  ...
  for (const candidate of discovery.candidates) {
    ...
    try {
      mod = getModuleLoader(safeSource)(safeSource) as RecallPluginModule;
    } catch (error) {
