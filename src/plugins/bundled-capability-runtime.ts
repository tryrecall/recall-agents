...
import type { SteelEnginePluginDefinition, SteelEnginePluginModule } from "./types.js";
...
function buildVitestCapabilityShimAliasMap(): Record<string, string> {
  return Object.fromEntries(
    CAPABILITY_VITEST_SHIM_ALIASES.flatMap(({ subpath, target }) => {
      ...
      return [
        [`steelengine/plugin-sdk/${subpath}`, targetPath],
        [`@steelengine/plugin-sdk/${subpath}`, targetPath],
      ];
    }),
  );
...
function applyVitestCapabilityAliasOverrides(params: {
  ...
  const {
    "steelengine/plugin-sdk": _ignoredLegacyRootAlias,
    "@steelengine/plugin-sdk": _ignoredScopedRootAlias,
    ...scopedAliasMap
  } = params.aliasMap;
  return {
    ...scopedAliasMap,
...
function resolvePluginModuleExport(moduleExport: unknown): {
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as SteelEnginePluginDefinition["register"],
    };
  }
...
export function loadBundledCapabilityRuntimeRegistry(params: {
  ...
  const devSourceRoot = resolveSteelEngineDevSourceRoot(env);
  ...
  const discovery = params.discovery ?? discoverSteelEnginePlugins({ env });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: SteelEnginePluginModule | null;
    try {
      mod = getModuleLoader(safeSource)(safeSource) as SteelEnginePluginModule;
    } catch (error) {
