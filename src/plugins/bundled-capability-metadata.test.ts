  ...
  return listExtensionPackagePaths(extensionsDir)
    .filter((packagePath) => {
      const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf-8")) as {
        recall?: RecallPackageManifest;
      };
      return normalizeBundledPluginStringList(packageJson.recall?.extensions).length > 0;
    })
    .map(
      (packagePath) =>
        JSON.parse(
          fs.readFileSync(path.join(path.dirname(packagePath), "recall.plugin.json"), "utf-8"),
        ) as PluginManifest,
    )
