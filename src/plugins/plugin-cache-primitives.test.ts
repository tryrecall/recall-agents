...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as SteelEngineConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as SteelEngineConfig;
    const load = vi.fn(async (config?: SteelEngineConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: SteelEngineConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
