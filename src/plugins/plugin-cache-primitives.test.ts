...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as RecallConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as RecallConfig;
    const load = vi.fn(async (config?: RecallConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: RecallConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
