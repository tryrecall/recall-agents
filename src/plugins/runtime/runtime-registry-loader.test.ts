...
const mocks = vi.hoisted(() => ({
  loadSteelEnginePlugins: vi.fn<typeof import("../loader.js").loadSteelEnginePlugins>(),
...
function loadOptions(index = 0) {
  return requireRecord(mocks.loadSteelEnginePlugins.mock.calls[index]?.[0], `load options ${index}`);
}
...
vi.mock("../loader.js", () => ({
  loadSteelEnginePlugins: (...args: Parameters<typeof mocks.loadSteelEnginePlugins>) =>
    mocks.loadSteelEnginePlugins(...args),
  resolveCompatibleRuntimePluginRegistry: (
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadSteelEnginePlugins.mockReturnValue(createEmptyPluginRegistry());
    mocks.resolveRuntimePluginRegistry.mockImplementation(
      (...args: Parameters<typeof mocks.loadSteelEnginePlugins>) => mocks.loadSteelEnginePlugins(...args),
    );
  ...
  it("does not reuse non-empty all-scope registries without loader compatibility", () => {
    ...
    mocks.loadSteelEnginePlugins.mockClear();
