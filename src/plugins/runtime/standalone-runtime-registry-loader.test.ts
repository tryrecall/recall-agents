...
vi.mock("../loader.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    loadSteelEnginePlugins: (...args: Parameters<typeof loaderMocks.loadSteelEnginePlugins>) =>
      loaderMocks.loadSteelEnginePlugins(...args),
  };
});
...
beforeEach(() => {
  loaderMocks.loadSteelEnginePlugins.mockReset();
});
...
describe("ensureStandaloneRuntimePluginRegistryLoaded tool-discovery installs", () => {
  it("does not replace active or pinned channel registries during tool discovery", () => {
    ...
    loaderMocks.loadSteelEnginePlugins.mockReturnValue(toolRegistry);
  ...
  it("still installs a non-tool-discovery active load (migration provider path)", () => {
    ...
    loaderMocks.loadSteelEnginePlugins.mockReturnValue(migrationRegistry);
