...
const REQUIRED_REVIEWED_PUBLISHABLE_CRITICAL_FINDINGS = new Set([
  "@recall/acpx:dangerous-exec:src/codex-auth-bridge.ts",
  "@recall/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.mjs",
  "@recall/codex:dangerous-exec:src/app-server/sandbox-exec-server/http.ts",
  "@recall/codex:dangerous-exec:src/app-server/sandbox-exec-server/processes.ts",
  "@recall/codex:dangerous-exec:src/app-server/transport-stdio.ts",
  "@recall/codex:dangerous-exec:src/node-cli-sessions.ts",
  "@recall/google-meet:dangerous-exec:src/node-host.ts",
  "@recall/google-meet:dangerous-exec:src/realtime.ts",
  "@recall/matrix:dangerous-exec:src/matrix/deps.ts",
  "@recall/voice-call:dangerous-exec:src/tunnel.ts",
  "@recall/voice-call:dangerous-exec:src/webhook/tailscale.ts",
]);

const OPTIONAL_REVIEWED_PUBLISHABLE_DIST_CRITICAL_FINDINGS = new Set([
  "@recall/acpx:dangerous-exec:dist/mcp-proxy.mjs",
  "@recall/acpx:dangerous-exec:dist/service-<hash>.js",
  "@recall/codex:dangerous-exec:dist/client-<hash>.js",
  "@recall/google-meet:dangerous-exec:dist/index.js",
  "@recall/slack:dynamic-code-execution:dist/outbound-payload.test-harness-<hash>.js",
  "@recall/voice-call:dangerous-exec:dist/runtime-entry-<hash>.js",
]);
...
function stageScannerRelevantPackedFiles(
  ...
  const stageDir = mkdtempSync(join(tmpdir(), "recall-plugin-npm-scan-"));
...
function collectPublishablePluginPackages(): PublishablePluginPackage[] {
  return listPublishablePluginPackageDirs()
    .flatMap((packageDir) => {
      ...
      let packageJson: {
        name?: unknown;
        recall?: { release?: { publishToNpm?: unknown } };
      };
      try {
