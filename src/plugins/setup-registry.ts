...
function collectConfiguredPluginEntryIds(config: SteelEngineConfig): string[] {
...
function resolveRegister(mod: SteelEnginePluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: SteelEnginePluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as SteelEnginePluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: SteelEnginePluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so SteelEngine ignored the plugin setup runtime entry. Remove setup-api/steelengine.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
