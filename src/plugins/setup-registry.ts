...
function collectConfiguredPluginEntryIds(config: RecallConfig): string[] {
...
function resolveRegister(mod: RecallPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: RecallPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as RecallPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: RecallPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so Recall ignored the plugin setup runtime entry. Remove setup-api/recall.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
