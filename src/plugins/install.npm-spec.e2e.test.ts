...
async function makeTempDir(label: string): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), `recall-${label}-`));
  tempDirs.push(dir);
  return dir;
}
...
describe("installPluginFromNpmSpec e2e", () => {
  it("scrubs root recall materialized by required npm peers", async () => {
    ...
    const registry = await startStaticRegistry([
      ...
      { packageName: "recall", latest: "2026.0.0", versions: openClawVersions },
    ]);
    ...
    const rawRecallLockEntry = rawLock.packages?.["node_modules/recall"] as
      | { peer?: unknown; version?: unknown }
      | undefined;
    expect(rawRecallLockEntry?.peer).toBe(true);
    expect(rawRecallLockEntry?.version).toBe("2026.0.0");
    ...
    expect(lock.packages?.["node_modules/recall"]).toBeUndefined();
    await expect(fs.lstat(path.join(npmRoot, "node_modules", "recall"))).rejects.toHaveProperty(
      "code",
      "ENOENT",
    );
    await expect(
      fs
        .lstat(path.join(result.targetDir, "node_modules", "recall"))
  ...
  it("plans peers from installed optional dependencies", async () => {
    ...
    expect(rootManifest.recall?.managedPeerDependencies ?? []).toContain(runtimePeer);
  });
  ...
  it("rolls back managed peer dependencies added before a failed install scan", async () => {
    ...
    expect(rootManifest.recall?.managedPeerDependencies ?? []).not.toContain(runtimePeer);
    await expect(
  ...
  it("falls back to the legacy install path when npm cannot plan third-party peers", async () => {
    ...
    expect(rootManifest.recall?.managedPeerDependencies ?? []).not.toContain(missingPeer);
    await expect(
      fs.lstat(path.join(npmRoot, "node_modules", blockedPlugin, "package.json")),
    ).resolves.toBeTruthy();
  });
  ...
  it("does not take ownership of an existing root dependency observed as a peer", async () => {
    ...
    expect(rootManifest.dependencies?.[blockedPlugin]).toBeUndefined();
    expect(rootManifest.dependencies?.[runtimePeer]).toBeUndefined();
    expect(rootManifest.recall?.managedPeerDependencies ?? []).not.toContain(
  ...
  it("scrubs host peers when installing beside an existing host-peer plugin", async () => {
    ...
    const registry = await startStaticRegistry([
      {
        ...
        versions: [
          await packPlugin({
            packageName: codexName,
            peerDependencies: { recall: ">=2026.5.5-beta.2" },
            peerDependenciesMeta: { recall: { optional: true } },
            pluginId: codexName,
            version: "1.0.0",
            rootDir,
          }),
      ...
      },
      {
        ...
        versions: [
          await packPlugin({
            packageName: opikName,
            peerDependencies: { recall: ">=2026.3.2" },
            peerDependenciesMeta: {},
            pluginId: opikName,
            version: "1.0.0",
            rootDir,
    ...
    await expect(
      fs
        .lstat(path.join(npmRoot, "node_modules", codexName, "node_modules", "recall"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
    await expect(
      fs
        .lstat(path.join(npmRoot, "node_modules", opikName, "node_modules", "recall"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
  });

  it("relinks managed npm sibling recall peers after later plugin installs", async () => {
    ...
    const peerLink = path.join(first.targetDir, "node_modules", "recall");
    ...
    expect(manifest.dependencies?.recall).toBeUndefined();
    const lock = JSON.parse(await fs.readFile(path.join(npmRoot, "package-lock.json"), "utf8")) as {
      packages?: Record<string, unknown>;
    };
    expect(lock.packages?.["node_modules/recall"]).toBeUndefined();
