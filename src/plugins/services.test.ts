...
import type { SteelEnginePluginService, SteelEnginePluginServiceContext } from "./types.js";
...
function createRegistry(
  services: SteelEnginePluginService[],
...
function expectServiceLogger(ctx: SteelEnginePluginServiceContext) {
...
function expectServiceContexts(
  contexts: SteelEnginePluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: SteelEnginePluginServiceContext[] = [];
  ...
  it("rejects unsafe event names, scopes, and payloads", async () => {
    let context: SteelEnginePluginServiceContext | undefined;
    const broadcastPluginEvent = vi.fn();
    await startPluginServices({
      registry: createRegistry([
        {
  ...
  it("grants internal diagnostics only to trusted diagnostics exporter services", async () => {
    ...
    const prometheusContexts: SteelEnginePluginServiceContext[] = [];
    ...
    const officialDiagnosticsOtelContexts: SteelEnginePluginServiceContext[] = [];
    ...
    const officialInstallContexts: SteelEnginePluginServiceContext[] = [];
    ...
    const untrustedContexts: SteelEnginePluginServiceContext[] = [];
    ...
    const spoofedContexts: SteelEnginePluginServiceContext[] = [];
