...
function collectExplicitEffectivePluginIds(config: SteelEngineConfig): string[] {
...
function collectSelectedContextEnginePluginIds(config: SteelEngineConfig): string[] {
  const plugins = normalizePluginsConfig(config.plugins);
  if (!plugins.enabled) {
    return [];
  }
