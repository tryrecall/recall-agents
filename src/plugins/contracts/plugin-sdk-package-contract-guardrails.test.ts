...
const PLUGIN_SDK_SUBPATH_PATTERN = /steelengine\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
const DEPRECATED_EXTENSION_SDK_SPECIFIERS = new Set([
  ...
  "steelengine/plugin-sdk/compat",
  "steelengine/plugin-sdk/test-utils",
]);
const DEPRECATED_TEST_ALIAS_SPECIFIERS = new Set(["steelengine/plugin-sdk/test-utils"]);
...
const LEGACY_MEMORY_EMBEDDING_PROVIDER_MANIFEST_FILES = new Set([
  "extensions/amazon-bedrock/steelengine.plugin.json",
  "extensions/deepinfra/steelengine.plugin.json",
  "extensions/github-copilot/steelengine.plugin.json",
  "extensions/google/steelengine.plugin.json",
  "extensions/lmstudio/steelengine.plugin.json",
  "extensions/memory-core/steelengine.plugin.json",
  "extensions/mistral/steelengine.plugin.json",
  "extensions/ollama/steelengine.plugin.json",
  "extensions/openai/steelengine.plugin.json",
  "extensions/voyage/steelengine.plugin.json",
]);
...
function collectDeprecatedExtensionSdkImports(): Array<{ file: string; specifier: string }> {
  const leaks: Array<{ file: string; specifier: string }> = [];
  const importPatterns = [
    /\b(?:import|export)\b[\s\S]*?\bfrom\s*["'](steelengine\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']/g,
    /\bimport\s*\(\s*["'](steelengine\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']\s*\)/g,
    /\bvi\.(?:mock|doMock)\s*\(\s*["'](steelengine\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']/g,
  ];
  for (const file of collectExtensionFiles(resolve(REPO_ROOT, "extensions"))) {
    const repoRelativePath = toRepoRelativePath(file);
    const source = fs.readFileSync(file, "utf8");
...
function collectNewDeprecatedMemoryEmbeddingProviderManifestFiles(): string[] {
  ...
  const manifestFiles =
    listGitTrackedFiles({
      repoRoot: REPO_ROOT,
      pathspecs: "extensions/**/steelengine.plugin.json",
    }) ?? [];
...
function collectDeprecatedTestAliasImports(): string[] {
  const leaks: Array<{ file: string; specifier: string }> = [];
  const importPatterns = [
    /\b(?:import|export)\b[\s\S]*?\bfrom\s*["'](steelengine\/plugin-sdk\/test-utils)["']/g,
    /\bimport\s*\(\s*["'](steelengine\/plugin-sdk\/test-utils)["']\s*\)/g,
    /\bvi\.(?:mock|doMock)\s*\(\s*["'](steelengine\/plugin-sdk\/test-utils)["']/g,
  ];
...
function collectCrossOwnerReservedSdkImports(): Array<{
  ...
  for (const file of collectExtensionFiles(resolve(REPO_ROOT, "extensions"))) {
    ...
    for (const match of source.matchAll(importPattern)) {
      ...
      leaks.push({
        file: repoRelativePath,
        specifier: `steelengine/plugin-sdk/${subpath}`,
        owner,
      });
    }
  }
    ...
    for (const importPattern of importPatterns) {
      for (const match of source.matchAll(importPattern)) {
        ...
        if (subpath && subpaths.has(subpath)) {
          imports.add(`${repoRelativePath}: steelengine/plugin-sdk/${subpath}`);
        }
      }
    }
  }
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps curated public plugin-sdk references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references steelengine/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }
  ...
  it("keeps Matrix dependencies local to the Matrix plugin", () => {
    ...
    expect(rootRuntimeDeps.has("@steelengine/plugin-package-contract")).toBe(false);
  });
