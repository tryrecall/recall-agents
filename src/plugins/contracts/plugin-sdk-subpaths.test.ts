...
import type {
  ...
  RecallPluginApi as CoreRecallPluginApi,
  PluginRuntime as CorePluginRuntime,
} from "recall/plugin-sdk/core";
import * as providerEntrySdk from "recall/plugin-sdk/provider-entry";
...
import type {
  ...
  RecallPluginApi as SharedRecallPluginApi,
...
const BROWSER_HELPER_EXPORT_PARITY_CONTRACTS: readonly BrowserHelperExportParityContract[] = [
  ...
  },
  {
    ...
    expectedExports: [
      ...
      "DEFAULT_RECALL_BROWSER_ENABLED",
...
describe("plugin-sdk subpath exports", () => {
  ...
  it("keeps the deprecated channel-runtime shim unused in repo imports", () => {
    const matches = findRepoFilesContaining({
      ...
      pattern:
        /(?:from\s+|import\s+(?:type\s+)?|import\s*\(\s*)["']recall\/plugin-sdk\/channel-runtime(?=["'])/u,
      exclude: [
  ...
  it("keeps shared plugin-sdk types aligned", () => {
    ...
    expectTypeOf<CoreRecallPluginApi>().toMatchTypeOf<RecallPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<PluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<ChannelMessageActionContext>();
    expectTypeOf<CoreRecallPluginApi>().toMatchTypeOf<SharedRecallPluginApi>();
    expectTypeOf<CorePluginRuntime>().toMatchTypeOf<SharedPluginRuntime>();
    expectTypeOf<CoreChannelMessageActionContext>().toMatchTypeOf<SharedChannelMessageActionContext>();
  });

  it("keeps runtime entry subpaths importable", async () => {
    const coreSdk = await importResolvedPluginSdkSubpath("recall/plugin-sdk/core");
    ...
    const globalSingletonSdk = await importResolvedPluginSdkSubpath(
      "recall/plugin-sdk/global-singleton",
    );
    const pluginEntrySdk = await importResolvedPluginSdkSubpath("recall/plugin-sdk/plugin-entry");
    ...
    for (const id of representativeRuntimeSmokeSubpaths) {
      representativeModules.push(await importResolvedPluginSdkSubpath(`recall/plugin-sdk/${id}`));
    }
  ...
  it("keeps the Zalouser command-auth compatibility facade importable", async () => {
    const commandAuthSdk = await importResolvedPluginSdkSubpath("recall/plugin-sdk/command-auth");
    const zalouserSdk = await importResolvedPluginSdkSubpath("recall/plugin-sdk/zalouser");
