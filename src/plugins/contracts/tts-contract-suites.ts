...
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "recall/plugin-sdk/speech-core";
...
type TtsRuntimeModule = typeof import("recall/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("recall/plugin-sdk/speech-core");
...
const speechCoreRuntimeApiModuleId = resolveWorkspacePackagePublicModuleUrl({
  packageName: "@recall/speech-core",
...
function asLegacyTtsConfig(value: unknown): RecallConfig {
  return value as RecallConfig;
}

function asLegacyRecallConfig(value: Record<string, unknown>): RecallConfig {
  return value as unknown as RecallConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): RecallConfig {
...
async function loadTtsCore(): Promise<TtsCoreModule> {
  ttsCorePromise ??= import("recall/plugin-sdk/speech-core");
...
function createResolvedSummarizationConfig(cfg: RecallConfig): ResolvedTtsConfig {
  const rawConfig =
    ...
    describe("getTtsProvider", () => {
      ...
      it("passes cfg into auto-selection so model-provider Google keys can configure TTS", () => {
        withIsolatedSpeechProviderEnv(
          ...
          () => {
            const cfg = asLegacyRecallConfig({
              agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
              models: {
                providers: {
                  google: {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: RecallConfig = asLegacyRecallConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.RECALL_TTS_PREFS;
      process.env.RECALL_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.RECALL_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: RecallConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
