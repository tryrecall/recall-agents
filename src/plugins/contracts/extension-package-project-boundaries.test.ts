...
const MEMORY_HOST_SDK_ALLOWED_CORE_BRIDGE_FILES = [
  "packages/memory-host-sdk/src/host/recall-runtime-auth.ts",
  "packages/memory-host-sdk/src/host/recall-runtime-network.ts",
  "packages/memory-host-sdk/src/host/recall-runtime.ts",
] as const;
const MEMORY_HOST_SDK_RUNTIME_ADAPTER_FILES = [
  "packages/memory-host-sdk/src/host/recall-runtime-agent.ts",
  "packages/memory-host-sdk/src/host/recall-runtime-cli.ts",
  "packages/memory-host-sdk/src/host/recall-runtime-config.ts",
  "packages/memory-host-sdk/src/host/recall-runtime-io.ts",
  "packages/memory-host-sdk/src/host/recall-runtime-memory.ts",
  "packages/memory-host-sdk/src/host/recall-runtime-session.ts",
] as const;
...
function collectRecallRuntimeDirectImportFiles(relativeDir: string): string[] {
  return collectCodeFiles(relativeDir).filter((file) => {
    const source = fs.readFileSync(resolve(REPO_ROOT, file), "utf8");
    return source.includes('"./recall-runtime.js"');
  });
}
...
describe("opt-in extension package boundaries", () => {
  ...
  it("keeps every opt-in extension rooted inside its package and on the package sdk", () => {
    ...
    for (const extensionName of optInExtensions) {
      ...
      expect(packageJson.devDependencies?.["@recall/plugin-sdk"]).toBe("workspace:*");
    }
  });
  ...
  it("keeps plugin-sdk package types generated from the package build, not a hand-maintained types bridge", () => {
    ...
    expect(packageJson.name).toBe("@recall/plugin-sdk");
  ...
  it("keeps memory-host-sdk as a private package-owned contract surface", () => {
    ...
    expect(packageJson.name).toBe("@recall/memory-host-sdk");
    ...
    expect(collectRecallRuntimeDirectImportFiles("packages/memory-host-sdk/src")).toEqual([
