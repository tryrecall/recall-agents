...
export function createNetworkRegistrars(state: PluginRegistryState) {
  ...
  const registerHttpRoute = (record: PluginRecord, params: SteelEnginePluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: SteelEnginePluginChannelRegistration | ChannelPlugin,
    ...
    const normalized =
      typeof (registration as SteelEnginePluginChannelRegistration).plugin === "object"
        ? (registration as SteelEnginePluginChannelRegistration)
