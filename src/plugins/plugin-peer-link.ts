...
export type SteelEnginePeerLinkAuditIssue = {
  packageName: string;
  packageDir: string;
  reason: string;
};
...
type AuditManagedNpmRootResult = {
  checked: number;
  broken: number;
  issues: SteelEnginePeerLinkAuditIssue[];
};

type SteelEnginePeerLinkResult = "linked" | "skipped" | "unchanged";
...
async function auditSteelEnginePeerDependency(params: {
  hostRoot: string;
  packageDir: string;
  npmRoot?: string;
  packageName?: string;
}): Promise<SteelEnginePeerLinkAuditIssue | null> {
  const packageName =
    params.packageName ??
    (params.npmRoot
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      return {
        packageName,
        packageDir: params.packageDir,
        reason: `missing ${path.join(nodeModulesDir, "steelengine")}`,
      };
    }
    throw error;
  }

  const linkPath = path.join(nodeModulesDir, "steelengine");
...
export async function auditSteelEnginePeerDependencyLink(params: {
  packageDir: string;
  packageName?: string;
}): Promise<SteelEnginePeerLinkAuditIssue | null> {
  ...
  if (!hostRoot) {
    return {
      packageName,
      packageDir: params.packageDir,
      reason: "could not locate steelengine package root",
    };
  }
  return await auditSteelEnginePeerDependency({
    hostRoot,
    packageDir: params.packageDir,
    packageName,
  });
...
async function ensureRealNodeModulesDir(params: {
  ...
  try {
    ...
    if (!existing.isDirectory() || existing.isSymbolicLink()) {
      params.logger.warn?.(
        `Skipping steelengine peerDependency link because ${nodeModulesDir} is not a real directory.`,
      );
      return null;
    }
...
async function linkSteelEnginePeerDependency(params: {
  hostRoot: string;
  installedDir: string;
  peerName: string;
  logger: PluginPeerLinkLogger;
}): Promise<SteelEnginePeerLinkResult> {
  ...
  try {
    ...
    if (existing) {
      if (!existing.isSymbolicLink()) {
        if (params.peerName === "steelengine" && existing.isDirectory()) {
          const existingPackageName = await readPackageName(linkPath);
          if (existingPackageName === "steelengine") {
        ...
        params.logger.warn?.(
          `Skipping steelengine peerDependency link because ${linkPath} already exists and is not a symlink.`,
        );
        return "skipped";
      }
...
/**
 * Symlink the host steelengine package for plugins that declare it as a peer.
 ...
 */
export async function linkSteelEnginePeerDependencies(params: {
  installedDir: string;
  peerDependencies: Record<string, string>;
  logger: PluginPeerLinkLogger;
}): Promise<{ repaired: number; skipped: number }> {
  const peers = Object.keys(params.peerDependencies).filter((name) => name === "steelengine");
  if (peers.length === 0) {
  ...
  if (!hostRoot) {
    params.logger.warn?.(
      "Could not locate steelengine package root to symlink peerDependencies; plugin may fail to resolve steelengine at runtime.",
    );
  ...
  for (const peerName of peers) {
    const result = await linkSteelEnginePeerDependency({
      hostRoot,
      installedDir: params.installedDir,
      peerName,
      logger: params.logger,
...
export async function relinkSteelEnginePeerDependenciesInManagedNpmRoot(params: {
  npmRoot: string;
  logger: PluginPeerLinkLogger;
...
export async function auditSteelEnginePeerDependenciesInManagedNpmRoot(params: {
  npmRoot: string;
  onPackageReadError?: (error: unknown, packageDir: string) => void;
  ...
  let checked = 0;
  const issues: SteelEnginePeerLinkAuditIssue[] = [];
  for (const packageDir of await listManagedNpmRootPackageDirs(params.npmRoot)) {
    let peerDependencies: Record<string, string>;
    try {
      peerDependencies = await readPackagePeerDependencies(packageDir);
    ...
    checked += 1;
    const issue = await auditSteelEnginePeerDependency({
      hostRoot,
      npmRoot: params.npmRoot,
      packageDir,
    });
