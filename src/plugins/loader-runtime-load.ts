...
export function loadSteelEnginePlugins(options: PluginLoadOptions = {}): PluginRegistry {
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'steelengine plugins inspect <id> --runtime --json' for runtime diagnostics, 'steelengine plugins list' for registry state, and restart the Gateway after plugin code or load-path changes.`,
        );
      }
    }
    if (cacheEnabled) {
