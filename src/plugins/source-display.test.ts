...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "steelengine", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".steelengine", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".steelengine", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "steelengine-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".steelengine", "extensions"),
      },
    });
  });
});
