...
export function resolvePluginVersionDriftUpdateCommand(entry: PluginVersionDriftEntry): string {
  ...
  if (exactNpmPackageName) {
    ...
    if (parseRegistryNpmSpec(exactNpmTarget)?.selectorKind === "exact-version") {
      return `steelengine plugins update ${exactNpmTarget}`;
    }
  }
  return `steelengine plugins update ${entry.pluginId}`;
}
...
function isPluginEnabled(config: SteelEngineConfig | undefined, pluginId: string): boolean {
 ...
 *   `version` field of the installed steelengine package.json).
 ...
 * @param params.config The merged daemon-side SteelEngineConfig (optional).
