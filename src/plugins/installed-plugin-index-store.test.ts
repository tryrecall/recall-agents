...
function makeTempDir() {
  return makeTrackedTempDir("steelengine-installed-plugin-index-store", tempDirs);
}
  ...
  it("preserves startup config paths across persisted index roundtrips", async () => {
    ...
    const index = createIndex({
      plugins: [
        {
          pluginId: "browser",
          manifestPath: "/plugins/browser/steelengine.plugin.json",
      ...
      plugins: [
        {
          ...
          manifestPath: "/plugins/provider-owner/steelengine.plugin.json",
