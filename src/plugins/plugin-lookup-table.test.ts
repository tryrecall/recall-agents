...
vi.mock("../channels/config-presence.js", () => ({
  ...
  listExplicitlyDisabledChannelIdsForConfig: (config: RecallConfig) =>
...
function createManifestRecord(
  ...
  return {
    ...
    manifestPath: `/plugins/${plugin.id}/recall.plugin.json`,
    ...plugin,
  };
}
...
const indexDiagnostic = {
  level: "warn",
  source: "/plugins/demo/recall.plugin.json",
...
describe("loadPluginLookUpTable", () => {
  beforeEach(() => {
    ...
    listPotentialConfiguredChannelIds
      .mockReset()
      .mockImplementation((config: RecallConfig) => Object.keys(config.channels ?? {}));
  ...
  it("rebuilds when a provided metadata snapshot has stale plugin paths", async () => {
    ...
    const requestedPlugins = [
      createManifestRecord({
        ...
        manifestPath: "/plugins-moved/telegram/recall.plugin.json",
      }),
    ];
    const config = {
      channels: {
