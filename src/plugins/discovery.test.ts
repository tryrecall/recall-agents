...
import { discoverSteelEnginePlugins } from "./discovery.js";
...
function makeTempDir() {
  return makeTrackedTempDir("steelengine-plugins", tempDirs);
}
...
function withSteelEnginePackageArgv<T>(packageRoot: string, fn: () => T): T {
  mkdirSafe(path.join(packageRoot, "bin"));
  fs.writeFileSync(path.join(packageRoot, "package.json"), '{"name":"steelengine"}\n', "utf-8");
  const originalArgv = process.argv;
  process.argv = [originalArgv[0] ?? "node", path.join(packageRoot, "bin", "steelengine")];
  try {
    return fn();
  } finally {
...
const canCreateDirectorySymlinks = (() => {
  const probeDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-symlink-probe-"));
...
function buildDiscoveryEnvWithOverrides(
  ...
  const enablesBundledOverride =
    Object.hasOwn(overrides, "STEELENGINE_BUNDLED_PLUGINS_DIR") &&
    overrides.STEELENGINE_BUNDLED_PLUGINS_DIR !== undefined;
  return {
    ...buildDiscoveryEnv(stateDir),
    ...(enablesBundledOverride ? { STEELENGINE_DISABLE_BUNDLED_PLUGINS: undefined } : {}),
    ...overrides,
  };
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverSteelEnginePlugins>[0],
) {
  return discoverSteelEnginePlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}

function discoverWithEnv(params: Parameters<typeof discoverSteelEnginePlugins>[0]) {
  return discoverSteelEnginePlugins(params);
}
...
function expectCandidatePresence(
  result: Awaited<ReturnType<typeof discoverSteelEnginePlugins>>,
...
describe("discoverSteelEnginePlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@steelengine/alpha",
      pluginId: "alpha",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceDir, ".steelengine", "extensions", "beta"),
      packageName: "@steelengine/beta",
      pluginId: "beta",
    });
  ...
  it("ignores standalone helper scripts in auto-discovered extension roots", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".steelengine", "extensions");
    mkdirSafe(globalExt);
  ...
  it("warns without blocking when a plugin requires a missing plugin", async () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@steelengine/diffs-language-pack",
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      ...
      createPackagePluginWithEntry({
        ...
        packageName: "@steelengine/linked-plugin",
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      ...
      createPackagePluginWithEntry({
        packageDir: linkedPluginDir,
        packageName: "@steelengine/workspace-linked-plugin",
  ...
  it("does not recurse arbitrary workspace directories for plugin auto-discovery", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@steelengine/workspace-plugin",
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@steelengine/stray-workspace-plugin",
    });

    const result = discoverSteelEnginePlugins({
      workspaceDir,
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceRoot, ".steelengine", "extensions", "tilde-workspace"),
      packageName: "@steelengine/tilde-workspace",
  ...
  it("does not warn about source checkout deps when bundled plugins are disabled", () => {
    ...
    fs.writeFileSync(path.join(extensionDir, "steelengine.plugin.json"), '{"id":"twitch"}\n', "utf-8");

    const result = withSteelEnginePackageArgv(packageRoot, () =>
      discoverSteelEnginePlugins({ env: buildDiscoveryEnv(stateDir) }),
    );
  ...
  it("does not treat repo-level live or test files as plugin entrypoints", () => {
    const stateDir = makeTempDir();
    const packageRoot = path.join(stateDir, "node_modules", "steelengine");
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@steelengine/real-plugin",
      pluginId: "real-plugin",
    });

    const { candidates, diagnostics } = withSteelEnginePackageArgv(packageRoot, () =>
  ...
  it("discovers bind-mounted bundled source overlays before packaged dist bundles", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@steelengine/synology-chat",
  ...
  it("discovers untracked global package plugins that point at TypeScript source", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@steelengine/local-source-pack",
      extensions: ["./index.ts"],
    });
  ...
  it("allows linked local install records to point at TypeScript source entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@steelengine/linked-source-pack",
  ...
  it("still requires compiled runtime output for tracked installed package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@steelengine/source-only-pack",
      extensions: ["./src/index.ts"],
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
    ...
    expect(
      result.diagnostics.some(
        (entry) =>
          entry.pluginId === "source-only-pack" && entry.message.includes("steelengine doctor --fix"),
      ),
    ).toBe(false);
  ...
  it("treats install record sourcePath dirs as managed during global scans", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@steelengine/source-path-pack",
      extensions: ["./src/index.ts"],
    });
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      ...
      writePluginPackageManifest({
        ...
        packageName: "@steelengine/source-path-symlink-pack",
        extensions: ["./src/index.ts"],
      });
  ...
  it("discovers global plugin directories with package metadata but no package entries", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "@steelengine/metadata-only-pack", version: "0.0.1" }),
      "utf-8",
    );
  ...
  it("warns on legacy npm declaration stubs without loading workspace node_modules", async () => {
    ...
    mkdirSafe(pluginDir);
    fs.writeFileSync(
      path.join(pluginDir, "steelengine.extension.json"),
      JSON.stringify({
    ...
    expectDiagnostic({
      ...
      source: path.join(pluginDir, "steelengine.extension.json"),
      messageIncludes: 'run "steelengine doctor --fix"',
    });
  });
  ...
  it("keeps explicit runtime extension entries strict for untracked global packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@steelengine/missing-runtime-pack",
      extensions: ["./index.ts"],
  ...
  it("reuses one filesystem realpath lookup per package root within a discovery run", () => {
    ...
    const { candidates } = discoverSteelEnginePlugins({
      env: buildDiscoveryEnv(stateDir),
    });
  ...
  it("uses explicit runtime extension entries for installed package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@steelengine/runtime-pack",
      extensions: ["./src/index.ts"],
      runtimeExtensions: ["./dist/index.js"],
      setupEntry: "./src/setup-entry.ts",
  ...
  it("rejects missing explicit runtime setup entries for installed package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@steelengine/missing-runtime-setup-pack",
  ...
  it("reports missing declared setup entries for package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@steelengine/missing-setup-pack",
      extensions: ["./dist/index.js"],
      setupEntry: "./src/setup-entry.ts",
    });
    writePluginEntry(path.join(pluginDir, "dist", "index.js"));
  ...
  it("rejects package runtimeExtensions that do not match extension entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@steelengine/runtime-mismatch-pack",
      extensions: ["./src/one.ts", "./src/two.ts"],
  ...
  it("rejects blank package runtimeExtensions before falling back to inferred entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@steelengine/runtime-blank-pack",
      extensions: ["./src/index.ts"],
      runtimeExtensions: [" "],
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
    ...
    expect(
      result.diagnostics.some(
        (entry) =>
          entry.level === "error" &&
          entry.message.includes("steelengine.runtimeExtensions[0]") &&
  ...
  it("rejects blank package extensions instead of falling back to inferred entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@steelengine/extension-blank-pack",
      extensions: ["./dist/index.js", " "],
    });
    writePluginEntry(path.join(pluginDir, "dist", "index.js"));
    ...
    expect(
      result.diagnostics.some(
        (entry) =>
          entry.level === "error" &&
          entry.message.includes("steelengine.extensions[1]") &&
          entry.message.includes("non-empty string"),
      ),
    ).toBe(true);
  });
  ...
  it("infers built dist entries for installed TypeScript package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@steelengine/built-peer-pack",
      extensions: ["src/index.ts"],
      setupEntry: "src/setup-entry.ts",
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
  ...
  it("preserves nested entry paths when inferring installed dist entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@steelengine/nested-pack",
  ...
  it("keeps workspace package TypeScript entries unless runtime entries are explicit", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const pluginDir = path.join(workspaceDir, ".steelengine", "extensions", "workspace-pack");
    mkdirSafe(path.join(pluginDir, "src"));
    mkdirSafe(path.join(pluginDir, "dist"));

    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@steelengine/workspace-pack",
      extensions: ["./src/index.ts"],
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
    writePluginEntry(path.join(pluginDir, "dist", "index.js"));
  ...
  it("skips incompatible non-bundled package plugin API candidates during discovery", () => {
    ...
    createPackagePluginWithEntry({
      packageDir: pluginDir,
      packageName: "@steelengine/future-channel",
      pluginId: "future-channel",
      compatPluginApi: ">=2026.5.27-beta.2",
    });

    const { candidates, diagnostics } = discoverSteelEnginePlugins({
      env: buildDiscoveryEnvWithOverrides(stateDir, {
        STEELENGINE_COMPATIBILITY_HOST_VERSION: "2026.5.27-beta.1",
      }),
    });
  ...
  it("skips malformed non-bundled package plugin API candidates during discovery", () => {
    ...
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@steelengine/malformed-channel",
        steelengine: {
          extensions: ["./index.js"],
    ...
    const { candidates, diagnostics } = discoverSteelEnginePlugins({
      env: buildDiscoveryEnvWithOverrides(stateDir, {
        STEELENGINE_COMPATIBILITY_HOST_VERSION: "2026.5.27",
      }),
    });
    ...
    expectDiagnostic({
      ...
      source: path.join(pluginDir, "package.json"),
      messageIncludes:
        "invalid package plugin API metadata: package.json steelengine.compat.pluginApi must be a string; skipping discovery",
    });
  });
  ...
  it("checks non-bundled package plugin API before package entry validation", () => {
    ...
    expectNoDiagnostic({ diagnostics, messageIncludes: "steelengine.extensions" });
  });
  ...
  it("discovers same-floor beta non-bundled package plugin API candidates", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@steelengine/current-channel",
  ...
  it("discovers source-checkout-only bundled plugins alongside built bundled plugins", () => {
    const stateDir = makeTempDir();
    const packageRoot = path.join(stateDir, "steelengine");
    const bundledDir = path.join(packageRoot, "dist", "extensions");
    ...
    writePluginPackageManifest({
      ...
      packageName: "@steelengine/shipped",
      extensions: ["./index.js"],
    });
  ...
  it("does not discover nested node_modules copies under installed plugins", async () => {
    const stateDir = makeTempDir();
    const pluginDir = path.join(stateDir, "extensions", "opik-steelengine");
    ...
    writePluginManifest({ pluginDir, id: "opik-steelengine" });
    fs.writeFileSync(
    ...
    writePluginPackageManifest({
      packageDir: path.join(pluginDir, "node_modules", "steelengine"),
    ...
    const { candidates } = await discoverWithStateDir(stateDir, {});
    expectCandidateOrder(candidates, ["opik-steelengine"]);
  });

  it("skips dependency and build directories while scanning workspace roots", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const workspaceRoot = path.join(workspaceDir, ".steelengine", "extensions");
    const workspacePluginDir = path.join(workspaceRoot, "workspace-plugin");
    const nestedNodeModulesDir = path.join(workspaceRoot, "node_modules", "steelengine");
  ...
  it.each([
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        for (const [dirName, packageName, pluginId] of [
          ["elevenlabs-speech-pack", "@steelengine/elevenlabs-speech", "elevenlabs"],
          ["microsoft-speech-pack", "@steelengine/microsoft-speech", "microsoft"],
        ] as const) {
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@steelengine/demo-plugin-dir",
          entryPath: "index.js",
        });
  ...
  it.each([
    ...
    expect(legacy?.format).toBe("steelengine");
  ...
  it.each([
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@steelengine/escape-pack",
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@steelengine/missing-entry-pack",
  ...
  it("ignores package manifests that are hardlinked aliases", async () => {
    ...
    fs.writeFileSync(
      outsideManifest,
      JSON.stringify({
        name: "@steelengine/pack",
        steelengine: { extensions: ["./entry.ts"] },
      }),
      "utf-8",
    );
    try {
  ...
  it.runIf(process.platform !== "win32")("blocks world-writable plugin paths", async () => {
    ...
    createPackagePluginWithEntry({
      packageDir: pluginDir,
      packageName: "@steelengine/world-open",
      pluginId: "world-open",
    });
  ...
  it.runIf(process.platform !== "win32" && typeof process.getuid === "function")(
    ...
    async () => {
      ...
      createPackagePluginWithEntry({
        ...
        packageName: "@steelengine/owner-mismatch",
  ...
  it.runIf(process.platform !== "win32")("deduplicates blocked candidate diagnostics", () => {
    ...
    try {
      const result = discoverSteelEnginePlugins({
        env: {
          ...buildDiscoveryEnv(stateDir),
          STEELENGINE_PLUGINS_PATHS: blockedDir,
        },
      });
  ...
  it("discovers bundled and global plugins for each workspace-specific scan", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@steelengine/bundled-plugin",
      pluginId: "bundled-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(globalExt, "global-plugin"),
      packageName: "@steelengine/global-plugin",
      pluginId: "global-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceA, ".steelengine", "extensions", "workspace-a-plugin"),
      packageName: "@steelengine/workspace-a-plugin",
      pluginId: "workspace-a-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceB, ".steelengine", "extensions", "workspace-b-plugin"),
      packageName: "@steelengine/workspace-b-plugin",
    ...
    const first = withSteelEnginePackageArgv(packageRoot, () =>
    ...
    const second = withSteelEnginePackageArgv(packageRoot, () =>
