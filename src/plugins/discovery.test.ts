...
import { bundledDistPluginFile } from "recall/plugin-sdk/test-fixtures";
import { afterEach, describe, expect, it, vi } from "vitest";
import type { PluginInstallRecord } from "../config/types.plugins.js";
import { discoverRecallPlugins } from "./discovery.js";
...
vi.mock("./bundled-dir.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveBundledPluginsDir: (env: NodeJS.ProcessEnv = process.env) =>
      env.RECALL_BUNDLED_PLUGINS_DIR ?? actual.resolveBundledPluginsDir(env),
  };
});
...
function makeTempDir() {
  return makeTrackedTempDir("recall-plugins", tempDirs);
}
...
function withRecallPackageArgv<T>(packageRoot: string, fn: () => T): T {
  mkdirSafe(path.join(packageRoot, "bin"));
  fs.writeFileSync(path.join(packageRoot, "package.json"), '{"name":"recall"}\n', "utf-8");
  const originalArgv = process.argv;
  process.argv = [originalArgv[0] ?? "node", path.join(packageRoot, "bin", "recall")];
  try {
    return fn();
  } finally {
...
const canCreateDirectorySymlinks = (() => {
  const probeDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-symlink-probe-"));
...
function buildDiscoveryEnvWithOverrides(
  ...
  const enablesBundledOverride =
    Object.prototype.hasOwnProperty.call(overrides, "RECALL_BUNDLED_PLUGINS_DIR") &&
    overrides.RECALL_BUNDLED_PLUGINS_DIR !== undefined;
  return {
    ...buildDiscoveryEnv(stateDir),
    ...(enablesBundledOverride ? { RECALL_DISABLE_BUNDLED_PLUGINS: undefined } : {}),
    ...overrides,
  };
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverRecallPlugins>[0],
) {
  return discoverRecallPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}

function discoverWithEnv(params: Parameters<typeof discoverRecallPlugins>[0]) {
  return discoverRecallPlugins(params);
}
...
function expectCandidatePresence(
  result: Awaited<ReturnType<typeof discoverRecallPlugins>>,
...
describe("discoverRecallPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".recall", "extensions");
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      ...
      createPackagePluginWithEntry({
        ...
        packageName: "@recall/linked-plugin",
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      ...
      createPackagePluginWithEntry({
        packageDir: linkedPluginDir,
        packageName: "@recall/workspace-linked-plugin",
  ...
  it("does not recurse arbitrary workspace directories for plugin auto-discovery", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@recall/workspace-plugin",
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@recall/stray-workspace-plugin",
    });

    const result = discoverRecallPlugins({
      workspaceDir,
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".recall", "extensions");
    mkdirSafe(workspaceExt);
  ...
  it("does not warn about source checkout deps when bundled plugins are disabled", () => {
    ...
    fs.writeFileSync(path.join(extensionDir, "recall.plugin.json"), '{"id":"twitch"}\n', "utf-8");

    const result = withRecallPackageArgv(packageRoot, () =>
      discoverRecallPlugins({ env: buildDiscoveryEnv(stateDir) }),
    );
  ...
  it("does not treat repo-level live or test files as plugin entrypoints", () => {
    const stateDir = makeTempDir();
    const packageRoot = path.join(stateDir, "node_modules", "recall");
    ...
    const { candidates, diagnostics } = withRecallPackageArgv(packageRoot, () =>
    ...
    const { candidates, diagnostics } = withRecallPackageArgv(packageRoot, () =>
      discoverRecallPlugins({
        ...
        env: {
          ...buildDiscoveryEnv(stateDir),
          RECALL_DISABLE_BUNDLED_PLUGINS: undefined,
          RECALL_BUNDLED_PLUGINS_DIR: bundledRoot,
        },
      }),
    );
  ...
  it("discovers bind-mounted bundled source overlays before packaged dist bundles", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@recall/synology-chat",
  ...
  it("discovers untracked global package plugins that point at TypeScript source", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@recall/local-source-pack",
      extensions: ["./index.ts"],
    });
  ...
  it("still requires compiled runtime output for tracked installed package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@recall/source-only-pack",
  ...
  it("treats install record sourcePath dirs as managed during global scans", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@recall/source-path-pack",
      extensions: ["./src/index.ts"],
    });
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      ...
      writePluginPackageManifest({
        ...
        packageName: "@recall/source-path-symlink-pack",
        extensions: ["./src/index.ts"],
      });
  ...
  it("discovers global plugin directories with package metadata but no package entries", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "@recall/metadata-only-pack", version: "0.0.1" }),
      "utf-8",
    );
  ...
  it("warns on legacy npm declaration stubs without loading workspace node_modules", async () => {
    ...
    mkdirSafe(pluginDir);
    fs.writeFileSync(
      path.join(pluginDir, "recall.extension.json"),
      JSON.stringify({
    ...
    expectDiagnostic({
      ...
      source: path.join(pluginDir, "recall.extension.json"),
      messageIncludes: 'run "recall doctor --fix"',
    });
  });
  ...
  it("keeps explicit runtime extension entries strict for untracked global packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@recall/missing-runtime-pack",
      extensions: ["./index.ts"],
  ...
  it("reuses one filesystem realpath lookup per package root within a discovery run", () => {
    ...
    const { candidates } = discoverRecallPlugins({
      env: buildDiscoveryEnv(stateDir),
    });
  ...
  it("uses explicit runtime extension entries for installed package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@recall/runtime-pack",
      extensions: ["./src/index.ts"],
      runtimeExtensions: ["./dist/index.js"],
  ...
  it("rejects missing explicit runtime setup entries for installed package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@recall/missing-runtime-setup-pack",
  ...
  it("reports missing declared setup entries for package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@recall/missing-setup-pack",
      extensions: ["./dist/index.js"],
      setupEntry: "./src/setup-entry.ts",
    });
    writePluginEntry(path.join(pluginDir, "dist", "index.js"));
  ...
  it("rejects package runtimeExtensions that do not match extension entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@recall/runtime-mismatch-pack",
      extensions: ["./src/one.ts", "./src/two.ts"],
  ...
  it("rejects blank package runtimeExtensions before falling back to inferred entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@recall/runtime-blank-pack",
      extensions: ["./src/index.ts"],
      runtimeExtensions: [" "],
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
    ...
    expect(
      result.diagnostics.some(
        (entry) =>
          entry.level === "error" &&
          entry.message.includes("recall.runtimeExtensions[0]") &&
  ...
  it("rejects blank package extensions instead of falling back to inferred entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@recall/extension-blank-pack",
      extensions: ["./dist/index.js", " "],
    });
    writePluginEntry(path.join(pluginDir, "dist", "index.js"));
    ...
    expect(
      result.diagnostics.some(
        (entry) =>
          entry.level === "error" &&
          entry.message.includes("recall.extensions[1]") &&
          entry.message.includes("non-empty string"),
      ),
    ).toBe(true);
  });
  ...
  it("infers built dist entries for installed TypeScript package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@recall/built-peer-pack",
      extensions: ["src/index.ts"],
      setupEntry: "src/setup-entry.ts",
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
  ...
  it("preserves nested entry paths when inferring installed dist entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@recall/nested-pack",
  ...
  it("keeps workspace package TypeScript entries unless runtime entries are explicit", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const pluginDir = path.join(workspaceDir, ".recall", "extensions", "workspace-pack");
    mkdirSafe(path.join(pluginDir, "src"));
    mkdirSafe(path.join(pluginDir, "dist"));

    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@recall/workspace-pack",
      extensions: ["./src/index.ts"],
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
    writePluginEntry(path.join(pluginDir, "dist", "index.js"));
  ...
  it("keeps trusted bundled package fallback from missing TypeScript metadata to JavaScript", () => {
    ...
    const { candidates, diagnostics } = discoverRecallPlugins({
      env: buildDiscoveryEnvWithOverrides(stateDir, {
        RECALL_BUNDLED_PLUGINS_DIR: bundledDir,
      }),
    });
  ...
  it("discovers source-checkout-only bundled plugins alongside built bundled plugins", () => {
    const stateDir = makeTempDir();
    const packageRoot = path.join(stateDir, "recall");
    const bundledDir = path.join(packageRoot, "dist", "extensions");
  ...
  it("does not discover nested node_modules copies under installed plugins", async () => {
    const stateDir = makeTempDir();
    const pluginDir = path.join(stateDir, "extensions", "opik-recall");
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@opik/opik-recall",
      extensions: ["./src/index.ts"],
    });
    writePluginManifest({ pluginDir, id: "opik-recall" });
    fs.writeFileSync(
    ...
    writePluginPackageManifest({
      packageDir: path.join(pluginDir, "node_modules", "recall"),
      packageName: "recall",
    ...
    const { candidates } = await discoverWithStateDir(stateDir, {});
    expectCandidateOrder(candidates, ["opik-recall"]);
  });

  it("skips dependency and build directories while scanning workspace roots", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const workspaceRoot = path.join(workspaceDir, ".recall", "extensions");
    const workspacePluginDir = path.join(workspaceRoot, "workspace-plugin");
    const nestedNodeModulesDir = path.join(workspaceRoot, "node_modules", "recall");
  ...
  it.each([
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        for (const [dirName, packageName, pluginId] of [
          ["elevenlabs-speech-pack", "@recall/elevenlabs-speech", "elevenlabs"],
          ["microsoft-speech-pack", "@recall/microsoft-speech", "microsoft"],
        ] as const) {
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@recall/demo-plugin-dir",
          entryPath: "index.js",
        });
  ...
  it.each([
    ...
    expect(legacy?.format).toBe("recall");
  ...
  it.each([
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@recall/escape-pack",
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@recall/missing-entry-pack",
  ...
  it.runIf(process.platform !== "win32")("deduplicates blocked candidate diagnostics", () => {
    ...
    try {
      const result = discoverRecallPlugins({
        env: {
          ...buildDiscoveryEnv(stateDir),
          RECALL_PLUGINS_PATHS: blockedDir,
        },
      });
  ...
  it("discovers bundled and global plugins for each workspace-specific scan", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@recall/bundled-plugin",
      pluginId: "bundled-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(globalExt, "global-plugin"),
      packageName: "@recall/global-plugin",
      pluginId: "global-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceA, ".recall", "extensions", "workspace-a-plugin"),
      packageName: "@recall/workspace-a-plugin",
      pluginId: "workspace-a-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceB, ".recall", "extensions", "workspace-b-plugin"),
      packageName: "@recall/workspace-b-plugin",
    ...
    const first = withRecallPackageArgv(packageRoot, () =>
    ...
    const second = withRecallPackageArgv(packageRoot, () =>
