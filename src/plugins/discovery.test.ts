...
import { clearPluginDiscoveryCache, discoverRecallPlugins } from "./discovery.js";
import {
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      recall: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverRecallPlugins", () => {
  ...
  it("strips provider suffixes from package-derived ids", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@recall/ollama-provider",
      extensions: ["./src/index.ts"],
    });
    fs.writeFileSync(
      path.join(globalExt, "src", "index.ts"),
  ...
  it("normalizes bundled speech package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@recall/elevenlabs-speech",
      extensions: ["./src/index.ts"],
    });
    writePluginPackageManifest({
      packageDir: microsoftDir,
      packageName: "@recall/microsoft-speech",
      extensions: ["./src/index.ts"],
    });
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverRecallPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        RECALL_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    });
    ...
    const second = discoverRecallPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        RECALL_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverRecallPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        RECALL_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
