...
vi.mock("../infra/recall-root.js", () => ({
  resolveRecallPackageRootSync: vi.fn(),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("recall-plugin-install");
...
function setPluginMinHostVersion(pluginDir: string, minHostVersion: string) {
  const packageJsonPath = path.join(pluginDir, "package.json");
  const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
    recall?: { install?: Record<string, unknown> };
  };
  manifest.recall = {
    ...manifest.recall,
    install: {
      ...manifest.recall?.install,
      minHostVersion,
    },
  };
  fs.writeFileSync(packageJsonPath, JSON.stringify(manifest), "utf-8");
...
async function ensureDynamicArchiveTemplate(params: {
  ...
  if (params.writePluginManifest !== false) {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "recall.plugin.json"),
      JSON.stringify({
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "recall.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs package archive runtime dependencies", async () => {
    ...
    expect(commandOptions.cwd).toContain(".recall-install-stage-");
  });
  ...
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@recall/voice-call" });
  ...
  it("rejects native plugin zip archives without recall.plugin.json", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package missing valid recall.plugin.json");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_PLUGIN_MANIFEST);
    }
    expect(fs.existsSync(resolvePluginInstallDir("@recall/zipper", extensionsDir))).toBe(false);
  });
  ...
  it("rejects packages without recall.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@recall/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("recall.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_RECALL_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when recall.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@recall/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing recall.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_RECALL_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without recall.extensions");
  });

  it("rejects package installs when recall.extensions entries escape the package", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_RECALL_EXTENSIONS);
  ...
  it("rejects package installs when recall.extensions contains a blank entry", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_RECALL_EXTENSIONS);
      expect(result.error).toContain("recall.extensions[1]");
  ...
  it("rejects package installs when runtimeExtensions contains a blank entry", async () => {
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_RECALL_EXTENSIONS);
      expect(result.error).toContain("recall.runtimeExtensions[0]");
      expect(result.error).toContain("non-empty string");
    }
  });
  ...
  it("rejects package installs when an extension entry is a symlink escape", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        recall: { extensions: ["./linked/escape.js"] },
      }),
    );
  ...
  it("still scans declared package entrypoints when they live under test-looking paths", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        recall: { extensions: ["tests/runtime.test.js"] },
      }),
    );
    fs.mkdirSync(path.join(pluginDir, "tests"), { recursive: true });
    fs.writeFileSync(
  ...
  it.runIf(process.platform !== "win32")(
    "allows package installs when node_modules/recall points at the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-recall");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"recall"}\n');
      vi.mocked(resolveRecallPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "recall-peer-plugin");

      const nodeModulesDir = path.join(pluginDir, "node_modules");
      fs.mkdirSync(nodeModulesDir, { recursive: true });
      fs.symlinkSync(hostRoot, path.join(nodeModulesDir, "recall"), "junction");
  ...
  it.runIf(process.platform !== "win32")(
    "allows package installs when node_modules/.bin/recall points inside the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-recall");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"recall"}\n');
      const hostBin = path.join(hostRoot, "recall.mjs");
      fs.writeFileSync(hostBin, "#!/usr/bin/env node\n");
      vi.mocked(resolveRecallPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "recall-bin-peer-plugin");

      const binDir = path.join(pluginDir, "node_modules", ".bin");
      fs.mkdirSync(binDir, { recursive: true });
      fs.symlinkSync(hostBin, path.join(binDir, "recall"), "file");
  ...
  it.runIf(process.platform !== "win32")(
    "fails package installs when node_modules/recall points outside the host package root",
    async () => {
      const { pluginDir, extensionsDir, tmpDir } = setupPluginInstallDirs();
      const hostRoot = path.join(tmpDir, "host-recall");
      const spoofedRoot = path.join(tmpDir, "spoofed-recall");
      fs.mkdirSync(hostRoot, { recursive: true });
      fs.mkdirSync(spoofedRoot, { recursive: true });
      fs.writeFileSync(path.join(hostRoot, "package.json"), '{"name":"recall"}\n');
      fs.writeFileSync(path.join(spoofedRoot, "package.json"), '{"name":"recall"}\n');
      vi.mocked(resolveRecallPackageRootSync).mockReturnValue(hostRoot);
      writeMinimalPackagePlugin(pluginDir, "spoofed-recall-peer-plugin");

      const nodeModulesDir = path.join(pluginDir, "node_modules");
      fs.mkdirSync(nodeModulesDir, { recursive: true });
      fs.symlinkSync(spoofedRoot, path.join(nodeModulesDir, "recall"), "junction");
      ...
      if (!result.ok) {
        expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED);
        expect(result.error).toContain("node_modules/recall");
      }
    },
  );

  it.runIf(process.platform !== "win32")(
    "fails package installs for nested or non-exact recall node_modules symlinks",
    async () => {
      const cases = [
        {
          pluginName: "nested-recall-peer-plugin",
          relativePath: path.join("node_modules", "vendor", "node_modules", "recall"),
        },
        {
          pluginName: "uppercase-recall-peer-plugin",
          relativePath: path.join("node_modules", "Recall"),
        },
        {
          pluginName: "trailing-space-recall-peer-plugin",
          relativePath: path.join("node_modules", "recall "),
        },
      ] as const;
  ...
  it("fails package installs when manifest traversal exceeds the directory cap", async () => {
    vi.stubEnv("RECALL_INSTALL_SCAN_MAX_DIRECTORIES", "4");
  ...
  it("fails package installs when manifest traversal exceeds the depth cap", async () => {
    vi.stubEnv("RECALL_INSTALL_SCAN_MAX_DEPTH", "2");
  ...
  it("does not flag the real qa-matrix plugin as dangerous install code", async () => {
    ...
    vi.mocked(resolveRecallPackageRootSync).mockReturnValue(process.cwd());
  ...
  it("scans extension entry files in hidden directories", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        recall: { extensions: [".hidden/index.js"] },
      }),
    );
    fs.writeFileSync(
...
describe("installPluginFromDir", () => {
  ...
  it("preserves local package manifests without dependency surgery", async () => {
    ...
    expect(manifest.devDependencies?.recall).toBe("workspace:*");
  ...
  it.each([
    {
      ...
      expectedMessageIncludes: ["requires Recall >=2026.3.22, but this host is 2026.3.21"],
    },
    {
      name: "rejects plugins with invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedCode: PLUGIN_INSTALL_ERROR_CODE.INVALID_MIN_HOST_VERSION,
      expectedMessageIncludes: ["invalid package.json recall.install.minHostVersion"],
    },
    {
  ...
  it("uses recall.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@recall/cognee-recall"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedPluginId: "@recall/test-plugin",
      install: (pluginDir: string, extensionsDir: string) =>
        installPluginFromDir({
          dirPath: pluginDir,
          extensionsDir,
...
describe("linkRecallPeerDependencies (via installPluginFromDir)", () => {
  const resolveRootMock = vi.mocked(resolveRecallPackageRootSync);
  ...
  it("creates a node_modules/recall symlink when peerDependencies declares recall", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    const run = vi.mocked(runCommandWithTimeout);
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { recall: "*" });
    ...
    const symlinkPath = path.join(result.targetDir, "node_modules", "recall");
  ...
  it("keeps the recall peer symlink when a local plugin already has dependencies", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { recall: "*" }, { "is-number": "7.0.0" });
  ...
  it("does not create a symlink when peerDependencies is empty", async () => {
    ...
    const symlinkPath = path.join(nodeModulesDir, "recall");
  ...
  it("is idempotent - re-installing replaces an existing symlink without error", async () => {
    ...
    const symlinkPath = path.join(second.targetDir, "node_modules", "recall");
    expect(fs.lstatSync(symlinkPath).isSymbolicLink()).toBe(true);
  });

  it("rejects when resolveRecallPackageRootSync returns null", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    ...
    expectWarningIncludes(warnings, "Could not locate recall package root");
  });
});
