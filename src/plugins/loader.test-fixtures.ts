...
import { clearActivatedPluginRuntimeState, loadSteelEnginePlugins } from "./loader.js";
import { resetPluginRuntimeStateForTest } from "./runtime.js";

export { loadSteelEnginePlugins };

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadSteelEnginePlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadSteelEnginePlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "steelengine-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.STEELENGINE_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.STEELENGINE_DISABLE_BUNDLED_PLUGINS;
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".steelengine", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ STEELENGINE_STATE_DIR: stateDir, ...params.env }, () =>
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.STEELENGINE_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.STEELENGINE_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.STEELENGINE_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
