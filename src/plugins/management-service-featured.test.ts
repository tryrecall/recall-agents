...
function metadataSnapshot(params: {
  ...
  const packageName =
    params.packageName === null ? undefined : (params.packageName ?? `@steelengine/${id}`);
  const manifest = {
    id,
...
describe("plugin management Featured authority", () => {
  it("projects listing metadata from a top-level hosted feed entry", async () => {
    ...
    const officialCatalog = {
      entries: [
        hostedFeedEntry({
          packageName: "@expediagroup/expedia-steelengine",
    ...
    expect(catalog.plugins[0]).toMatchObject({
      id: "@expediagroup/expedia-steelengine",
      name: "Expedia Travel",
      description: "Search flights, stays, and travel options.",
      featured: true,
      order: 10,
  ...
  it("surfaces a newly featured live official package without static fallback metadata", async () => {
    ...
    mocks.officialCatalog.mockResolvedValue(
      hostedCatalog([
        hostedFeedEntry({
          packageName: "@steelengine/new-tool",
          title: "New Tool",
          featured: true,
        }),
      ]),
    ...
    expect(catalog.plugins).toEqual([
      expect.objectContaining({
        id: "@steelengine/new-tool",
        name: "New Tool",
        featured: true,
        install: { source: "official", pluginId: "@steelengine/new-tool" },
      }),
    ]);
  });
  ...
  it("orders live featured packages by when they were featured", async () => {
    ...
    mocks.officialCatalog.mockResolvedValue(
      hostedCatalog([
        hostedFeedEntry({
          packageName: "@steelengine/older-popular",
          title: "Older Popular",
          featured: true,
          featuredAt: 100,
          order: 1,
        ...
        hostedFeedEntry({
          packageName: "@steelengine/newest-featured",
        ...
        hostedFeedEntry({
          packageName: "@steelengine/legacy-featured",
    ...
    expect(catalog.plugins.map((plugin) => plugin.id)).toEqual([
      "@steelengine/newest-featured",
      "@steelengine/older-popular",
      "@steelengine/legacy-featured",
    ]);
  ...
  it("clears stale embedded curation on a matched package without bundled curation", async () => {
    ...
    mocks.officialCatalog.mockResolvedValue(
      hostedCatalog([
        hostedFeedEntry({
          packageName: "@steelengine/copilot",
          title: "Copilot",
          featured: false,
          pluginId: "copilot",
          catalogFeatured: true,
  ...
  it("lets a live unfeature override an installed published plugin manifest", async () => {
    mocks.metadata.mockReturnValue(
      metadataSnapshot({
        ...
        name: "Diffs",
        origin: "global",
        installRecord: { source: "npm", spec: "@steelengine/diffs" },
      }),
    );
    mocks.officialCatalog.mockResolvedValue(
      hostedCatalog([{ ...hostedFeedDiffsEntry, featured: false }]),
  ...
  it.each([
    { id: "workboard", name: "Workboard", packageName: "@steelengine/workboard" },
    { id: "open-prose", name: "OpenProse", packageName: "@steelengine/open-prose" },
    { id: "memory-wiki", name: "Memory Wiki", packageName: "@steelengine/memory-wiki" },
  ...
  it("applies hosted curation to the exact published package for bundled FireCrawl", async () => {
    ...
    mocks.metadata.mockReturnValue(
      metadataSnapshot({
        id: "firecrawl",
        name: "firecrawl",
        packageName: "@steelengine/firecrawl-plugin",
        featured: false,
        description: "Optional SteelEngine capability.",
  ...
  it("preserves npm-only bundled curation outside the hosted producer identity", async () => {
    mocks.metadata.mockReturnValue(
      metadataSnapshot({
        ...
        installRecord: { source: "npm", spec: "@steelengine/acpx" },
      }),
    );
    mocks.officialCatalog.mockResolvedValue(hostedCatalog([]));
    ...
    mocks.metadata.mockReturnValue(
      metadataSnapshot({
        ...
        installRecord: {
          ...
          clawhubPackage: "@steelengine/new-tool",
        },
      }),
    );
    mocks.officialCatalog.mockResolvedValue(hostedCatalog([]));
