...
import type {
  RecallPluginHookOptions,
  RecallPluginToolFactory,
  RecallPluginToolOptions,
} from "./tool-types.js";
...
export type {
  RecallPluginActiveModelContext,
  RecallPluginHookOptions,
 ...
 */
export type RecallPluginConfigSchema = {
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Recall uses this to expose
 ...
 *
 * Runs after Recall resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Recall auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Recall wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Recall builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. Recall no
...
/**
 * Provider-owned stream wrapper hook after Recall applies its generic
 ...
 *
 * Return `true` or `false` to override Recall's built-in provider cache TTL
 ...
 *
 * Runs only after Recall exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Recall should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Recall loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Recall falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Recall calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Recall performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Recall registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Recall wrappers.
   *
   ...
   *
   * Called after Recall resolves the raw configured credential but before the
   ...
   * hint than Recall's generic auth-store guidance.
   */
   ...
   * Recall's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Recall should surface a provider-specific hint for
   ...
   * merging. Recall deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. Recall stores the
   ...
   * without replacing the full Recall-owned system prompt.
   */
   ...
   *
   * Runs after Recall's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep Recall's default fallback policy.
   */
   ...
   * rewrites after Recall has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Recall handles
   ...
   *
   * Recall uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `recall doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `recall doctor` should offer.
   *
   ...
   *
   * Recall calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Recall's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Recall falls back to that stored profile.
   */
...
export type RecallPluginGatewayMethod = {
  method: string;
...
export type PluginCommandDiagnosticsSession = {
  ...
  sessionKey?: string;
  /** Ephemeral Recall session id when available. */
  sessionId?: string;
  /** Transcript file for this Recall session when available. */
  sessionFile?: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active Recall session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current Recall configuration */
  config: RecallConfig;
...
export type RecallPluginCommandDefinition = {
...
export type RecallPluginHttpRouteAuth = "gateway" | "plugin";
export type RecallPluginHttpRouteMatch = "exact" | "prefix";
export type RecallPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type RecallPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type RecallPluginHttpRouteUpgradeHandler = (
  req: IncomingMessage,
  socket: Duplex,
  head: Buffer,
) => Promise<boolean | void> | boolean | void;

export type RecallPluginHttpRouteParams = {
  path: string;
...
export type RecallPluginHostedMediaResolver = (
  mediaUrl: string,
) => string | null | undefined | Promise<string | null | undefined>;

export type RecallPluginCliContext = {
  /**
   * Command object where this plugin should register its commands.
   *
   * For root CLI registrations this is the root `recall` program. For nested
...
export type RecallPluginCliRegistrar = (ctx: RecallPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want Recall to keep a plugin command lazy-loaded while still
 ...
 */
export type RecallPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type RecallPluginNodeCliFeatureOptions = {
  /** Explicit node feature command names owned under `recall nodes`. */
  commands?: string[];
  /**
   * Parse-time command descriptors for lazy node feature CLI registration.
   *
   * Descriptors are registered under `recall nodes`, so a descriptor named
   * `"camera"` exposes `recall nodes camera`.
   */
  descriptors?: RecallPluginCliCommandDescriptor[];
};

export type RecallPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type RecallPluginNodeHostCommand = {
  command: string;
  cap?: string;
  dangerous?: boolean;
...
export type RecallPluginNodeInvokeTransportResult =
  | {
      ok: true;
      payload?: unknown;
...
export type RecallPluginNodeInvokeApprovalDecision = "allow-once" | "allow-always" | "deny";

export type RecallPluginNodeInvokePolicyApprovalRuntime = {
  request: (input: {
    title: string;
    description: string;
    ...
    id?: string;
    decision?: RecallPluginNodeInvokeApprovalDecision | null;
  }>;
};

export type RecallPluginNodeInvokePolicyContext = {
  nodeId: string;
  command: string;
  params: unknown;
  timeoutMs?: number;
  ...
  approvals?: RecallPluginNodeInvokePolicyApprovalRuntime;
  invokeNode: (input?: {
    params?: unknown;
    timeoutMs?: number;
    idempotencyKey?: string;
...
export type RecallPluginNodeInvokePolicyResult =
  | {
      ok: true;
      payload?: unknown;
      payloadJSON?: string | null;
...
export type RecallPluginNodeInvokePolicy = {
  commands: string[];
  /**
  ...
  handle: (
    ctx: RecallPluginNodeInvokePolicyContext,
  ) => Promise<RecallPluginNodeInvokePolicyResult> | RecallPluginNodeInvokePolicyResult;
};

export type RecallPluginSecurityAuditContext = {
  config: RecallConfig;
...
export type RecallPluginSecurityAuditCollector = (
  ctx: RecallPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

export type RecallGatewayDiscoveryAdvertiseContext = {
...
export type RecallGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: RecallGatewayDiscoveryAdvertiseContext,
  ) => void | Promise<void | { stop?: () => void | Promise<void> }>;
};

/** Context passed to long-lived plugin services. */
export type RecallPluginServiceContext = {
  config: RecallConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type RecallPluginService = {
  id: string;
  start: (ctx: RecallPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: RecallPluginServiceContext) => void | Promise<void>;
};

export type RecallPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type RecallPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  reload?: RecallPluginReloadRegistration;
  nodeHostCommands?: RecallPluginNodeHostCommand[];
  securityAuditCollectors?: RecallPluginSecurityAuditCollector[];
  register?: (api: RecallPluginApi) => void;
  activate?: (api: RecallPluginApi) => void;
};

export type RecallPluginModule = RecallPluginDefinition | ((api: RecallPluginApi) => void);
...
export type PluginConfigMigration = (config: RecallConfig) =>
  | {
      config: RecallConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `recall migrate`. */
...
export type RecallPluginSessionStateApi = {
...
export type RecallPluginSessionWorkflowApi = {
...
export type RecallPluginSessionControlsApi = {
...
export type RecallPluginSessionApi = {
  state: RecallPluginSessionStateApi;
  workflow: RecallPluginSessionWorkflowApi;
  controls: RecallPluginSessionControlsApi;
};

export type RecallPluginAgentEventsApi = {
...
export type RecallPluginAgentApi = {
  events: RecallPluginAgentEventsApi;
};

export type RecallPluginRunContextApi = {
...
export type RecallPluginLifecycleApi = {
...
export type RecallPluginApi = {
   ...
   */
  session: RecallPluginSessionApi;
  /** Grouped facade for agent-event workflow seams. */
  agent: RecallPluginAgentApi;
  /** Grouped facade for run-scoped plugin scratch state. */
  runContext: RecallPluginRunContextApi;
  /** Grouped facade for plugin-owned lifecycle cleanup hooks. */
  lifecycle: RecallPluginLifecycleApi;
  registerTool: (
    tool: AnyAgentTool | RecallPluginToolFactory,
    opts?: RecallPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: RecallPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: RecallPluginHttpRouteParams) => void;
  /** Register a plugin-owned resolver for browser-style hosted media URLs. */
  registerHostedMediaResolver: (resolver: RecallPluginHostedMediaResolver) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: RecallPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: RecallPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every command exposed at `parentPath`, Recall
  ...
  /**
   * Register a plugin-owned node feature command group under `recall nodes`.
   *
  ...
  registerNodeCliFeature: (
    registrar: RecallPluginCliRegistrar,
    opts?: RecallPluginNodeCliFeatureOptions,
  ) => void;
  registerReload: (registration: RecallPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: RecallPluginNodeHostCommand) => void;
  registerNodeInvokePolicy: (policy: RecallPluginNodeInvokePolicy) => void;
  registerSecurityAuditCollector: (collector: RecallPluginSecurityAuditCollector) => void;
  registerService: (service: RecallPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: RecallGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `recall migrate` (migration capability). */
   ...
   */
  registerCommand: (command: RecallPluginCommandDefinition) => void;
