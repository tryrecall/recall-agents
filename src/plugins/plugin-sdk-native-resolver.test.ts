...
function writeFakeSteelEnginePackage(root: string): { distRoot: string; loaderModulePath: string } {
  ...
  fs.writeFileSync(path.join(root, "steelengine.mjs"), "#!/usr/bin/env node\n", "utf8");
...
describe("installSteelEnginePluginSdkNativeResolver", () => {
  it("resolves installed plugin SDK imports to the dev source root", () => {
    const stableRoot = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sdk-native-stable-"));
    const devRoot = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sdk-native-dev-source-"));
    const { loaderModulePath } = writeFakeSteelEnginePackage(stableRoot);
    writeFakeSteelEnginePackage(devRoot);
    ...
    const previousDevSourceRoot = process.env.STEELENGINE_DEV_SOURCE_ROOT;
    process.env.STEELENGINE_DEV_SOURCE_ROOT = devRoot;

    try {
      const installedAliases = installSteelEnginePluginSdkNativeResolver({
        modulePath: loaderModulePath,
        pluginModulePath: externalPluginEntry,
      });

      expect(installedAliases).toContain("steelengine/plugin-sdk/agent-runtime");
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(fs.realpathSync(requireFromPlugin.resolve("steelengine/plugin-sdk/agent-runtime"))).toBe(
        fs.realpathSync(path.join(devRoot, "dist", "plugin-sdk", "agent-runtime.js")),
      );
    } finally {
      if (previousDevSourceRoot === undefined) {
        delete process.env.STEELENGINE_DEV_SOURCE_ROOT;
      } else {
        process.env.STEELENGINE_DEV_SOURCE_ROOT = previousDevSourceRoot;
      }
    }
  });
  ...
  it("removes stale native SDK aliases when a later dev root omits a subpath", () => {
    ...
    expect(fs.realpathSync(requireFromPlugin.resolve("steelengine/plugin-sdk/stable-extra"))).toBe(
    ...
    expect(() => requireFromPlugin.resolve("steelengine/plugin-sdk/stable-extra")).toThrow();
  });

  it("keeps native aliases on JS dist artifacts when source files exist", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sdk-native-source-resolver-"));
    const { loaderModulePath } = writeFakeSteelEnginePackage(root);
    ...
    expect(installedAliases).toContain("steelengine/plugin-sdk/channel-outbound");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(fs.realpathSync(requireFromPlugin.resolve("steelengine/plugin-sdk/channel-outbound"))).toBe(
      fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")),
    );
  });

  it("lets built external plugins resolve SteelEngine SDK subpaths with createRequire", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sdk-native-resolver-"));
    const { distRoot, loaderModulePath } = writeFakeSteelEnginePackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    ...
    try {
      ...
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(
        fs.realpathSync(requireFromPlugin.resolve("steelengine/plugin-sdk/channel-outbound")),
      ).toBe(fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")));
      const sdk = requireFromPlugin("steelengine/plugin-sdk/channel-outbound") as {
        defineChannelMessageAdapter?: () => string;
      };

      expect(sdk.defineChannelMessageAdapter?.()).toBe("adapter");
      expect(() => requireFromPlugin.resolve("steelengine/not-plugin-sdk/channel-message")).toThrow();
    } finally {
      if (process.platform !== "win32") {
  ...
  beforeAll(() => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sdk-native-esm-resolver-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'import path from "node:path";',
        'import { pathToFileURL } from "node:url";',
        `import { installSteelEnginePluginSdkNativeResolver } from ${JSON.stringify(resolverModuleUrl)};`,
        ...
        "fs.writeFileSync(",
        "  entryPath,",
        '  "import { defineChannelMessageAdapter } from \\"steelengine/plugin-sdk/channel-outbound\\"; export const eager = defineChannelMessageAdapter(); export const loadLazy = () => import(\\"./lazy.js\\");\\n",',
        '  "utf8",',
        ");",
        "fs.writeFileSync(",
        "  lazyPath,",
        '  "import { defineChannelMessageAdapter } from \\"steelengine/plugin-sdk/channel-outbound\\"; export const lazy = defineChannelMessageAdapter();\\n",',
        '  "utf8",',
        ");",
        "installSteelEnginePluginSdkNativeResolver({",
        "  modulePath: loaderModulePath,",
  ...
  it("does not resolve SDK aliases for parents outside registered plugin roots", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sdk-native-guard-"));
    const { loaderModulePath } = writeFakeSteelEnginePackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    const unrelatedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sdk-native-outside-"));
    ...
    expect(requireFromPlugin.resolve("steelengine/plugin-sdk/channel-outbound")).toBeTruthy();
    expect(() => requireFromOutside.resolve("steelengine/plugin-sdk/channel-outbound")).toThrow();
  });

  it("resolves internal core packages only for SteelEngine-owned source parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sdk-native-core-internal-"));
    const { loaderModulePath } = writeFakeSteelEnginePackage(root);
    ...
    expect(installedAliases).toContain("@steelengine/normalization-core/string-coerce");
    expect(installedAliases).toContain("@steelengine/normalization-core/boolean-coercion");
    expect(installedAliases).toContain("@steelengine/normalization-core/result");
    expect(installedAliases).toContain("@steelengine/normalization-core/agent-id");
    expect(installedAliases).toContain("@steelengine/media-core/mime");
    expect(installedAliases).toContain("@steelengine/markdown-core/code-spans");
    expect(installedAliases).toContain("@steelengine/ai/internal/retry-after");
    expect(installedAliases).toContain("@steelengine/ai/internal/runtime");
    expect(installedAliases).toContain("@steelengine/acp-core/runtime/types");
    expect(installedAliases).toContain("@steelengine/llm-core");
    const requireFromCoreSource = createRequire(coreSourceParent);
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@steelengine/normalization-core/string-coerce")),
    ).toBe(fs.realpathSync(normalizationSource));
    expect(
      fs.realpathSync(
        requireFromCoreSource.resolve("@steelengine/normalization-core/boolean-coercion"),
      ),
    ).toBe(fs.realpathSync(booleanCoercionSource));
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@steelengine/normalization-core/result")),
    ).toBe(fs.realpathSync(resultSource));
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@steelengine/normalization-core/agent-id")),
    ).toBe(fs.realpathSync(agentIdSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@steelengine/media-core/mime"))).toBe(
      fs.realpathSync(mediaCoreSource),
    );
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@steelengine/markdown-core/code-spans")),
    ).toBe(fs.realpathSync(markdownCoreSource));
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@steelengine/ai/internal/retry-after")),
    ).toBe(fs.realpathSync(aiRetryAfterSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@steelengine/ai/internal/runtime"))).toBe(
      fs.realpathSync(aiRuntimeSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@steelengine/acp-core/runtime/types"))).toBe(
      fs.realpathSync(acpCoreSource),
    );
    expect(fs.realpathSync(requireFromCoreSource.resolve("@steelengine/llm-core"))).toBe(
      fs.realpathSync(llmCoreSource),
    );
    expect(() => requireFromPlugin.resolve("@steelengine/normalization-core/string-coerce")).toThrow();
    expect(() =>
      requireFromPlugin.resolve("@steelengine/normalization-core/boolean-coercion"),
    ).toThrow();
    expect(() => requireFromPlugin.resolve("@steelengine/normalization-core/result")).toThrow();
    expect(() => requireFromPlugin.resolve("@steelengine/media-core/mime")).toThrow();
    expect(() => requireFromPlugin.resolve("@steelengine/markdown-core/code-spans")).toThrow();
    expect(() => requireFromPlugin.resolve("@steelengine/ai/internal/retry-after")).toThrow();
    expect(() => requireFromPlugin.resolve("@steelengine/ai/internal/runtime")).toThrow();
    expect(() => requireFromPlugin.resolve("@steelengine/acp-core/runtime/types")).toThrow();
    expect(() => requireFromPlugin.resolve("@steelengine/llm-core")).toThrow();
  });

  it("does not register source-only SDK subpaths for native resolution", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sdk-native-source-only-"));
    const { loaderModulePath } = writeFakeSteelEnginePackage(root);
    ...
    expect(installedAliases).toContain("steelengine/plugin-sdk/channel-outbound");
    expect(installedAliases).not.toContain("steelengine/plugin-sdk/source-only");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(() => requireFromPlugin.resolve("steelengine/plugin-sdk/source-only")).toThrow();
  });

  it("scopes private SSRF SDK aliases to bundled local IPC native parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sdk-native-ssrf-"));
    const { loaderModulePath } = writeFakeSteelEnginePackage(root);
    ...
    expect(installedAliases).toContain("steelengine/plugin-sdk/ssrf-runtime-internal");
    const requireFromOllama = createRequire(ollamaEntry);
    expect(
      fs.realpathSync(requireFromOllama.resolve("steelengine/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeOllama = createRequire(runtimeOllamaEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeOllama.resolve("steelengine/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromBrowser = createRequire(browserEntry);
    expect(
      fs.realpathSync(requireFromBrowser.resolve("steelengine/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeBrowser = createRequire(runtimeBrowserEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeBrowser.resolve("steelengine/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromOther = createRequire(otherEntry);
    expect(() => requireFromOther.resolve("steelengine/plugin-sdk/ssrf-runtime-internal")).toThrow();
  });
});
