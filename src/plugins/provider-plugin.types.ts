...
export type ProviderPlugin = {
   ...
   *
   * SteelEngine calls this only from async model resolution paths. After it
   ...
   *
   * Runs after SteelEngine performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before SteelEngine registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic SteelEngine wrappers.
   *
   ...
   *
   * Called after SteelEngine resolves the raw configured credential but before the
   ...
   * hint than SteelEngine's generic auth-store guidance.
   */
   ...
   * SteelEngine's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when SteelEngine should surface a provider-specific hint for
   ...
   * merging. SteelEngine deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. SteelEngine stores the
   ...
   * without replacing the full SteelEngine-owned system prompt.
   */
   ...
   *
   * Runs after SteelEngine's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep SteelEngine's default fallback policy.
   */
   ...
   * rewrites after SteelEngine has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before SteelEngine handles
  ...
  ) => SteelEngineConfig | null | undefined;
  /**
   ...
   *
   * SteelEngine uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `steelengine doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `steelengine doctor` should offer.
   *
   ...
   *
   * SteelEngine calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep SteelEngine's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * SteelEngine falls back to that stored profile.
   */
