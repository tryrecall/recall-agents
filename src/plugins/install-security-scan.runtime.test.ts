...
describe("install security scan official bypass", () => {
  it("bypasses plugin install friction for bundled SteelEngine sources", async () => {
    const result = await scanBundleInstallSourceRuntime({
      logger: {},
      pluginId: "steelengine/kitchen-sink",
      sourceDir: "/tmp/steelengine-bundled-plugin",
      source: { kind: "bundled", authority: "steelengine", mutable: false, network: false },
    });
  ...
  it("bypasses plugin install friction for official ClawHub sources", async () => {
    const result = await scanBundleInstallSourceRuntime({
      logger: {},
      pluginId: "@steelengine/matrix",
      sourceDir: "/tmp/steelengine-official-clawhub-plugin",
  ...
  it("bypasses skill install friction for bundled SteelEngine sources", async () => {
    const result = await evaluateSkillInstallPolicyRuntime({
      installId: "node",
      logger: {},
      origin: {
        type: "steelengine-bundled",
        skillName: "peekaboo",
        installId: "node",
      },
      source: { kind: "bundled", authority: "steelengine", mutable: false, network: false },
      skillName: "peekaboo",
      sourceDir: "/tmp/steelengine-bundled-skill/peekaboo",
    });
  ...
  it("runs only operator policy for official immutable npm sources", async () => {
    const result = await preflightPluginNpmInstallPolicyRuntime({
      ...
      requestedSpecifier: "@steelengine/matrix@latest",
      source: { kind: "npm", authority: "official", mutable: false, network: true },
      sourcePath: "/tmp/steelengine-official-npm",
