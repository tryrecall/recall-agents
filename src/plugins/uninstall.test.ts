...
type PluginConfig = NonNullable<RecallConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: RecallConfig["channels"];
}): RecallConfig {
...
function createSinglePluginWithEmptySlotsConfig(): RecallConfig {
...
function createSingleNpmInstallConfig(installPath: string): RecallConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("removePluginFromConfig", () => {
  ...
  it("removes absolute load path for a workspace-relative install source path", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tempRoot, "recall-uninstall-portable-source-"));
    try {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as RecallConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
...
describe("uninstallPlugin", () => {
      ...
      config: createPluginConfig({
        installs: {
          "missing-linked-plugin": createPathInstallRecord(
            "/missing/recall/plugin",
            "/missing/recall/plugin",
          ),
        },
        loadPaths: ["/missing/recall/plugin", "/keep/this/plugin"],
      }),
      expectedActions: {
        entry: false,
        install: true,
  ...
  it("uninstalls npm-managed packages through npm before deleting the package directory", async () => {
    ...
    const pluginDir = path.join(npmRoot, "node_modules", "@recall", "kitchen-sink");
    ...
    const plan = planPluginUninstall({
      config: createPluginConfig({
        entries: createSinglePluginEntries("recall-kitchen-sink-fixture"),
        installs: {
          "recall-kitchen-sink-fixture": {
            source: "npm",
            spec: "@recall/kitchen-sink@1.0.0",
            installPath: pluginDir,
          },
        },
      }),
    ...
    expect(plan.directoryRemoval).toEqual({
      ...
      cleanup: {
        kind: "npm",
        npmRoot,
        packageName: "@recall/kitchen-sink",
      },
    });
    ...
    expect(applied).toEqual({ directoryRemoved: true, warnings: [] });
    expectNpmUninstallCommand({ packageName: "@recall/kitchen-sink", npmRoot });
    await expectPathAccessState(pluginDir, "missing");
  });

  it("repairs remaining npm plugin recall peer links after npm uninstall prunes them", async () => {
    const stateDir = path.join(tempDir, "state");
    const npmRoot = path.join(stateDir, "npm");
    ...
    await expectPathAccessState(path.join(npmRoot, "node_modules", "recall"), "missing");
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("returns managed install path for copied path installs", () => {
    const extensionsDir = path.join(os.tmpdir(), "recall-uninstall-safe");
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    ...
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-recall-plugin-install/my-plugin",
      },
      extensionsDir,
    });
  ...
  it("uses configured installPath when npm installed it under the managed npm root", () => {
    const stateDir = path.join(os.tmpdir(), "recall-uninstall-safe");
    const extensionsDir = path.join(stateDir, "extensions");
    const installPath = path.join(stateDir, "npm", "node_modules", "@recall", "kitchen-sink");
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@recall/kitchen-sink@latest",
          installPath,
        },
        extensionsDir,
      }),
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@recall/kitchen-sink@latest",
          installPath: path.join(os.tmpdir(), "npm", "node_modules", "@recall", "kitchen-sink"),
        },
        extensionsDir,
      }),
    ).toBe(resolvePluginInstallDir("recall-kitchen-sink-fixture", extensionsDir));
  });

  it("uses configured installPath when it is under the recorded managed extensions root", () => {
    const currentExtensionsDir = path.join(os.tmpdir(), "recall-uninstall-current", "extensions");
    const recordedExtensionsDir = path.join(
      os.tmpdir(),
      "recall-uninstall-recorded",
      "extensions",
    );
    const installPath = resolvePluginInstallDir("my-plugin", recordedExtensionsDir);
