...
import { bundledPluginRootAt } from "recall/plugin-sdk/test-fixtures";
...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-recall",
    targetDir: params?.targetDir ?? "/tmp/opik-recall",
...
function createSuccessfulClawHubUpdateResult(params?: {
  ...
  return {
    ...
    targetDir: params?.targetDir ?? "/tmp/recall-plugins/legacy-chat",
...
function createInstalledPackageDir(params: {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-plugin-update-test-"));
  tempDirs.push(dir);
  fs.writeFileSync(
...
function createRecallPeerLinkFixtures(plugins: Array<{ pluginId: string; packageName: string }>) {
  const peerTarget = fs.mkdtempSync(path.join(os.tmpdir(), "recall-peer-target-"));
  ...
  const installPaths = Object.fromEntries(
    plugins.map(({ pluginId, packageName }) => [
      ...
      createInstalledPackageDir({
        name: packageName,
        version: "2026.5.4",
        peerDependencies: { recall: ">=2026.5.4" },
      }),
    ]),
  );
  const peerLinkPath = (pluginId: string) =>
    path.join(installPaths[pluginId], "node_modules", "recall");
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@recall/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  const install = params.result.config.plugins?.installs?.["recall-codex-app-server"];
  expect(install?.source).toBe("npm");
  expect(install?.spec).toBe(params.spec);
  expect(install?.installPath).toBe("/tmp/recall-codex-app-server");
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-recall",
        spec: "@opik/opik-recall",
        integrity: "sha512-old",
        installPath: "/tmp/opik-recall",
      }),
      pluginIds: ["opik-recall"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-recall",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        ...
        spec: "recall-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/recall-codex-app-server",
      }),
      ...
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "recall-codex-app-server",
        targetDir: "/tmp/recall-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "recall-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it("trusts official catalog npm updates when the installed package matches the catalog", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        spec: "@recall/acpx",
        installPath,
        resolvedName: "@recall/acpx",
        resolvedSpec: "@recall/acpx@2026.5.2-beta.1",
        resolvedVersion: "2026.5.2-beta.1",
      }),
      pluginIds: ["acpx"],
    });

    expect(npmInstallCall()?.spec).toBe("@recall/acpx");
  ...
  it("does not skip trusted official default updates when latest resolves to the installed prerelease", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@recall/acpx",
          version: "2026.5.2",
          resolvedSpec: "@recall/acpx@2026.5.2",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        shasum: "beta",
        resolvedName: "@recall/acpx",
        resolvedSpec: "@recall/acpx@2026.5.2-beta.2",
  ...
  it("updates trusted official npm plugins when latest resolves to a stable correction release", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@recall/acpx",
          version: "2026.5.3-1",
          resolvedSpec: "@recall/acpx@2026.5.3-1",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        installPath,
        resolvedName: "@recall/acpx",
        resolvedSpec: "@recall/acpx@2026.5.3",
  ...
  it("repairs missing recall peer links before skipping unchanged npm plugins", async () => {
    const installPath = createInstalledPackageDir({
      name: "@recall/codex",
      version: "2026.5.3",
      peerDependencies: { recall: ">=2026.5.3" },
    });
    mockNpmViewMetadata({
      name: "@recall/codex",
      version: "2026.5.3",
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@recall/codex",
          version: "2026.5.3",
          resolvedSpec: "@recall/codex@2026.5.3",
        },
      }),
    );
    const config: RecallConfig = {
    ...
    expect(npmInstallCall()?.spec).toBe("@recall/codex");
  ...
  it("skips unchanged npm plugins when the recall peer link already resolves", async () => {
    const installPath = createInstalledPackageDir({
      name: "@recall/codex",
      version: "2026.5.3",
      peerDependencies: { recall: ">=2026.5.3" },
    });
    fs.mkdirSync(path.join(installPath, "node_modules", "recall"), { recursive: true });
    mockNpmViewMetadata({
      name: "@recall/codex",
      version: "2026.5.3",
      integrity: "sha512-same",
  ...
  it("repairs recall peer links after batch npm updates prune earlier plugin links", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@recall/brave-plugin" },
      { pluginId: "codex", packageName: "@recall/codex" },
      { pluginId: "discord", packageName: "@recall/discord" },
    ];
    const { installPaths, peerLinkPath, linkPeer } = createRecallPeerLinkFixtures(plugins);
  ...
  it("repairs sibling recall peer links after a targeted npm update prunes the shared install tree", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@recall/brave-plugin" },
      { pluginId: "codex", packageName: "@recall/codex" },
      { pluginId: "discord", packageName: "@recall/discord" },
      ...
      return Promise.resolve(
        createSuccessfulNpmUpdateResult({
          ...
          npmResolution: {
            name: "@recall/codex",
            version: "2026.5.4",
            resolvedSpec: "@recall/codex@2026.5.4",
          },
        }),
      );
    });
  ...
  it("continues repairing sibling recall peer links after one recorded npm install cannot be relinked", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@recall/brave-plugin" },
      { pluginId: "codex", packageName: "@recall/codex" },
    ];
      ...
      config: {
        plugins: {
          installs: {
            broken: {
              source: "npm",
              spec: "@recall/broken-plugin",
              installPath: brokenInstallPath,
              resolvedName: "@recall/broken-plugin",
              resolvedVersion: "2026.5.4",
              resolvedSpec: "@recall/broken-plugin@2026.5.4",
            },
            ...Object.fromEntries(
              plugins.map(({ pluginId, packageName }) => [
                pluginId,
    ...
    expect(warnMessages).toEqual([
      `Could not repair recall peer link for "broken" at ${brokenInstallPath}: Skipping recall peerDependency link because ${path.join(brokenInstallPath, "node_modules")} is not a real directory.`,
    ]);
  });
  ...
  it("expands home-relative install paths before checking installed npm versions", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "recall-plugin-update-home-"));
    tempDirs.push(home);
    const installPath = path.join(home, ".recall", "extensions", "lossless-claw");
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "lossless-claw",
        spec: "@martian-engineering/lossless-claw",
        installPath: "~/.recall/extensions/lossless-claw",
        resolvedName: "@martian-engineering/lossless-claw",
        resolvedVersion: "0.9.0",
        resolvedSpec: "@martian-engineering/lossless-claw@0.9.0",
        integrity: "sha512-same",
  ...
  it("updates disabled trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@recall/diagnostics-otel",
      }),
    );
    ...
    const config = createClawHubInstallConfig({
      ...
      clawhubFamily: "code-plugin",
      clawhubChannel: "official",
      spec: "clawhub:@recall/diagnostics-otel@2026.5.3",
    });
    const result = await updateNpmInstalledPlugins({
      config: {
        ...config,
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@recall/diagnostics-otel");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-otel"], {
      source: "clawhub",
      spec: "clawhub:@recall/diagnostics-otel",
      version: "2026.5.4",
      clawhubPackage: "@recall/diagnostics-otel",
      clawhubChannel: "official",
    });
  ...
  it("updates bare trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@recall/diagnostics-prometheus",
      }),
    );
        ...
        plugins: {
          installs: {
            "diagnostics-prometheus": {
              source: "clawhub",
              spec: "clawhub:@recall/diagnostics-prometheus@2026.5.3",
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@recall/diagnostics-prometheus");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-prometheus"], {
      source: "clawhub",
      spec: "clawhub:@recall/diagnostics-prometheus",
      version: "2026.5.4",
      clawhubPackage: "@recall/diagnostics-prometheus",
      clawhubChannel: "official",
    });
  ...
  it("clears stale plugin policy and slot references when disabling failed updates", async () => {
    ...
    const message =
      'Disabled "demo" after plugin update failure; Recall will continue without it. Failed to update demo: security scan blocked install';
    expect(warn).toHaveBeenCalledWith(message);
    expect(result.changed).toBe(true);
    expect(result.config.plugins?.entries?.demo).toEqual({
      enabled: false,
  ...
  it("aborts exact pinned npm plugin updates on integrity drift by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockImplementation(
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            integrity: "sha512-new",
            resolvedSpec: "@opik/opik-recall@0.2.5",
            version: "0.2.5",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @opik/opik-recall@0.2.5",
          };
        }
    ...
    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for "opik-recall" (@opik/opik-recall@0.2.5): expected sha512-old, got sha512-new',
    );
    expect(result.changed).toBe(false);
    expect(result.config).toBe(config);
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update opik-recall: aborted: npm package integrity drift detected for @opik/opik-recall@0.2.5",
      },
    ]);
  });
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @recall/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@recall/missing",
      ...
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @recall/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        ...
        resolvedSpec: "recall-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "recall-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "recall-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "recall-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
      ...
      expectedSpec: "recall-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "recall-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "recall-codex-app-server",
      });
  ...
  it("falls back to the default npm spec when a beta tag is unavailable", async () => {
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error:
          "npm ERR! code ETARGET\nnpm ERR! No matching version found for recall-codex-app-server@beta.",
      })
      .mockResolvedValueOnce(
        createSuccessfulNpmUpdateResult({
          pluginId: "recall-codex-app-server",
          ...
          npmResolution: {
            name: "recall-codex-app-server",
            version: "0.2.6",
            resolvedSpec: "recall-codex-app-server@0.2.6",
          },
        }),
      );
    ...
    expect(npmInstallCall(0)?.spec).toBe("recall-codex-app-server@beta");
    expect(npmInstallCall(1)?.spec).toBe("recall-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "recall-codex-app-server" has no beta npm release for recall-codex-app-server@beta; using recall-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "recall-codex-app-server",
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Updated recall-codex-app-server: unknown -> 0.2.6. (warning: beta channel fallback used recall-codex-app-server because recall-codex-app-server@beta could not be used).",
    );
  });
  ...
  it("falls back to the default npm spec when the beta package exists but is invalid", async () => {
    ...
    expect(npmInstallCall(1)?.spec).toBe("recall-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "recall-codex-app-server" failed beta npm update for recall-codex-app-server@beta; using recall-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "recall-codex-app-server",
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update recall-codex-app-server: npm package not found for recall-codex-app-server.",
      },
    ]);
  });
  ...
  it("preserves explicit npm tags when updating on the beta channel", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createCodexAppServerInstallConfig({
        spec: "recall-codex-app-server@rc",
      }),
      pluginIds: ["recall-codex-app-server"],
      updateChannel: "beta",
      dryRun: true,
  ...
  it("falls back to npm for trusted official ClawHub artifact blocks", async () => {
    ...
    installPluginFromClawHubMock.mockResolvedValueOnce({
      ...
      error:
        'ClawHub artifact download for "@recall/discord@2026.5.16-beta.5" is not available yet (ClawHub /api/v1/packages/%40openclaw%2Fdiscord/versions/2026.5.16-beta.5/artifact/download failed (403): Blocked: this package release has been flagged as malicious and cannot be downloaded.). Use "npm:@recall/discord@2026.5.16-beta.5" for launch installs while ClawHub artifact routing is being rolled out.',
    });
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        pluginId: "discord",
        targetDir: "/tmp/recall-plugins/discord",
        version: "2026.5.16-beta.5",
        npmResolution: {
          name: "@recall/discord",
          version: "2026.5.16-beta.5",
          resolvedSpec: "@recall/discord@2026.5.16-beta.5",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createClawHubInstallConfig({
        ...
        installPath,
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@recall/discord",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        spec: "clawhub:@recall/discord",
      }),
      pluginIds: ["discord"],
      updateChannel: "beta",
      disableOnFailure: true,
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@recall/discord@beta");
    expect(npmInstallCall()?.spec).toBe("@recall/discord@beta");
    ...
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
      spec: "@recall/discord",
      installPath: "/tmp/recall-plugins/discord",
      version: "2026.5.16-beta.5",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        message:
          "Updated discord: 2026.5.12 -> 2026.5.16-beta.5. (warning: official ClawHub artifact fallback used @recall/discord@beta).",
      },
    ]);
    expect(warnMessages).toEqual([
      'Plugin "discord" could not download official ClawHub artifact for clawhub:@recall/discord@beta; using npm @recall/discord@beta instead. Core update can still complete.',
    ]);
  });
  ...
  it("uses the default npm spec when beta ClawHub falls back before an artifact block", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@recall/discord",
          version: "2026.5.16",
          resolvedSpec: "@recall/discord@2026.5.16",
        },
      }),
    );
    ...
    expect(clawHubInstallCall(0)?.spec).toBe("clawhub:@recall/discord@beta");
    expect(clawHubInstallCall(1)?.spec).toBe("clawhub:@recall/discord");
    expect(npmInstallCall()?.spec).toBe("@recall/discord");
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
      spec: "@recall/discord",
      installPath: "/tmp/recall-plugins/discord",
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Updated discord: 2026.5.12 -> 2026.5.16. (warning: official ClawHub artifact fallback used @recall/discord).",
    );
    expect(warnMessages).toEqual([
      'Plugin "discord" has no beta ClawHub release for clawhub:@recall/discord@beta; using clawhub:@recall/discord instead. Core update can still complete.',
      'Plugin "discord" could not download official ClawHub artifact for clawhub:@recall/discord; using npm @recall/discord instead. Core update can still complete.',
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        nextVersion: "2026.5.16-beta.5",
        message:
          "discord is up to date (2026.5.16-beta.5). (warning: official ClawHub artifact fallback would use @recall/discord@beta).",
      },
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update discord: artifact unavailable (ClawHub clawhub:@recall/discord@beta).",
      },
    ]);
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@recall/voice-call",
      targetDir: "/tmp/recall-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(npmInstallCall()?.spec).toBe("@recall/voice-call");
    expect(npmInstallCall()?.expectedPluginId).toBe("voice-call");
    expect(result.config.plugins?.allow).toEqual(["@recall/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@recall/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@recall/voice-call");
    expect(result.config.plugins?.entries?.["@recall/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expectRecordFields(result.config.plugins?.installs?.["@recall/voice-call"], {
      source: "npm",
      spec: "@recall/voice-call",
      installPath: "/tmp/recall-voice-call",
      version: "0.0.2",
    });
  ...
  it("migrates context engine slot when a plugin id changes during update", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@recall/context-engine",
      targetDir: "/tmp/recall-context-engine",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.slots?.contextEngine).toBe("@recall/context-engine");
    expectRecordFields(result.config.plugins?.installs?.["@recall/context-engine"], {
      source: "npm",
      spec: "@recall/context-engine",
      installPath: "/tmp/recall-context-engine",
      version: "0.0.2",
    });
  ...
  it("forwards dangerous force unsafe install to plugin update installers", async () => {
    ...
    expect(npmInstallCall()?.spec).toBe("recall-codex-app-server@beta");
    expect(npmInstallCall()?.dangerouslyForceUnsafeInstall).toBe(true);
    expect(npmInstallCall()?.expectedPluginId).toBe("recall-codex-app-server");
  });

  it("reuses the recorded managed extensions root when updating external plugins", async () => {
    const installPath = "/var/recall/extensions/demo";
    const extensionsDir = "/var/recall/extensions";
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "demo",
        targetDir: installPath,
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/recall-home";
    mockBundledSources(
      createBundledSource({
  ...
  it("installs an externalized bundled plugin and rewrites its old bundled path plugin index", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/recall-plugins/legacy-chat",
        version: "2.0.0",
        npmResolution: {
          name: "@recall/legacy-chat",
          version: "2.0.0",
          resolvedSpec: "@recall/legacy-chat@2.0.0",
        },
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@recall/legacy-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
      spec: "@recall/legacy-chat",
      installPath: "/tmp/recall-plugins/legacy-chat",
      version: "2.0.0",
      resolvedName: "@recall/legacy-chat",
  ...
  it("marks official externalized bundled npm installs as trusted", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "voice-call",
        targetDir: "/tmp/recall-plugins/voice-call",
  ...
  it("falls back from ClawHub to npm only when the ClawHub package is absent", async () => {
    ...
    expect(result.summary.warnings).toEqual([
      "ClawHub clawhub:legacy-chat@2026.5.1-beta.2 unavailable for legacy-chat; falling back to npm @recall/legacy-chat.",
    ]);
    expect(result.summary.errors).toStrictEqual([]);
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
  ...
  it("does not fall back from ClawHub to non-Recall npm packages", async () => {
    resolveBundledPluginSourcesMock.mockReturnValue(new Map());
    installPluginFromClawHubMock.mockResolvedValue({
      ok: false,
      code: "package_not_found",
      ...
      externalizedBundledPluginBridges: [
        {
          bundledPluginId: "voice-call",
          preferredSource: "clawhub",
          clawhubSpec: "clawhub:@recall/voice-call",
          npmSpec: "@recall/voice-call",
          channelIds: ["voice-call"],
        },
      ],
  ...
  it("externalizes bundled plugins that were enabled by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/recall-plugins/default-chat",
        version: "2.0.0",
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@recall/default-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["default-chat"], {
      source: "npm",
      spec: "@recall/default-chat",
      installPath: "/tmp/recall-plugins/default-chat",
      version: "2.0.0",
    });
  });
      ...
      config: {
        ...
        plugins: {
          ...
          installs: {
            "legacy-chat": {
              source: "npm",
              spec: "@recall/legacy-chat@1.2.3",
              resolvedSpec: "@recall/legacy-chat@1.2.3",
              installPath: "/tmp/recall-plugins/legacy-chat",
            },
          },
        },
