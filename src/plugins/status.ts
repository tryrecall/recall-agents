...
import { normalizeSteelEngineVersionBase } from "../config/version.js";
...
function resolveReportedPluginVersion(
  ...
  return (
    normalizeSteelEngineVersionBase(resolveCompatibilityHostVersion(env)) ??
    normalizeSteelEngineVersionBase(plugin.version) ??
    plugin.version
  );
}
