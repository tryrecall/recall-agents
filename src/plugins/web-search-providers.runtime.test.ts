    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/recall.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/recall.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
...
function expectLoaderCallCount(count: number) {
  expect(loadRecallPluginsMock).toHaveBeenCalledTimes(count);
}
...
function expectScopedWebSearchCandidates(pluginIds: readonly string[]) {
  ...
  expect(
    requireLastCallFirstArg(loadRecallPluginsMock, "loadRecallPlugins").onlyPluginIds,
  ).toEqual([...pluginIds]);
}
...
function expectAutoEnabledWebSearchLoad(params: {
  ...
  const loaderParams = requireLastCallFirstArg(loadRecallPluginsMock, "loadRecallPlugins");
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  it("resolves current config contents when config changes in place", () => {
    ...
    const env = createWebSearchEnv({ RECALL_HOME: "/tmp/recall-home-a" });
