...
/** Result of reading package.json steelengine.compat.pluginApi metadata. */
...
export function resolvePackagePluginApiRange(
  ...
  if (!isRecord(compat)) {
    return { ok: false, error: "package.json steelengine.compat must be an object" };
  }
  ...
  if (typeof pluginApi !== "string") {
    return { ok: false, error: "package.json steelengine.compat.pluginApi must be a string" };
  }
  const range = pluginApi.trim();
  if (!range) {
    return { ok: false, error: "package.json steelengine.compat.pluginApi must not be empty" };
  }
