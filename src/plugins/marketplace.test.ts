...
async function listMarketplaceDownloadTempDirs(): Promise<string[]> {
  ...
  return entries
    .filter(
      (entry) => entry.isDirectory() && entry.name.startsWith("recall-marketplace-download-"),
    )
...
describe("marketplace plugins", () => {
  ...
  it("lists plugins from a local marketplace root", async () => {
    await withTempDir("recall-marketplace-test-", async (rootDir) => {
  ...
  it("resolves Claude-style plugin@marketplace shortcuts from known_marketplaces.json", async () => {
    await withTempDir("recall-marketplace-test-", async (homeDir) => {
      const openClawHome = path.join(homeDir, "recall-home");
      ...
      const shortcut = await withEnvAsync(
        { HOME: homeDir, RECALL_HOME: openClawHome },
