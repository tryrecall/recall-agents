...
describe("loadSteelEnginePlugins", () => {
  ...
  it("can scope bundled provider loads without hanging", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@steelengine/scoped-provider",
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@steelengine/unscoped-provider",
        steelengine: { extensions: ["./index.cjs"] },
      }),
      "utf-8",
    );
  ...
  it("allows bundled plugins to supply system.notify without opening the command to external plugins", () => {
    ...
    const bundledRegistry = loadSteelEnginePlugins({
      cache: false,
    ...
    const externalRegistry = loadSteelEnginePlugins({
      cache: false,
  ...
  it("restores cached command and interactive handler registrations on cache hits", () => {
    ...
    const registry = loadSteelEnginePlugins(loadOptions);
  ...
  it("restores cached memory capability public artifacts on cache hits", async () => {
    ...
    const first = loadSteelEnginePlugins(options);
    ...
    const second = loadSteelEnginePlugins(options);
  ...
  it("uses discovery registration mode for non-activating loads", () => {
    useNoBundledPlugins();
    const marker = "__steelengineDiscoveryModeTest";
    const plugin = writePlugin({
    ...
    const snapshot = loadSteelEnginePlugins({
      activate: false,
      cache: false,
      workspaceDir: plugin.dir,
      config,
  ...
  it("caches non-activating snapshots without restoring global side effects", () => {
    useNoBundledPlugins();
    clearPluginCommands();
    const marker = "__steelengineSnapshotCacheRegisterCount";
    const plugin = writePlugin({
      id: "snapshot-cache",
  ...
  it("does not re-register non-bundled plugins after gateway-bindable boot loads", () => {
    useNoBundledPlugins();
    const marker = "__steelengineGatewayBootRegisterCount";
    const plugin = writePlugin({
  ...
  it("reuses a gateway-bindable cache entry for later default-mode loads", () => {
    useNoBundledPlugins();
    const marker = "__steelengineGatewayBindableCacheRegisterCount";
    const plugin = writePlugin({
    ...
    const gatewayBindable = loadSteelEnginePlugins({
      ...options,
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
      },
    });
    const defaultMode = loadSteelEnginePlugins(options);
  ...
  it("keeps pinned gateway hooks and later default-mode hooks dispatchable together", () => {
    ...
    const gatewayRegistry = loadSteelEnginePlugins({
    ...
    try {
      ...
      const defaultRegistry = loadSteelEnginePlugins({
        ...
        return {
          ...
          loadFirst: () =>
            loadSteelEnginePlugins({
              ...
              env: {
                ...process.env,
                STEELENGINE_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadSteelEnginePlugins({
              ...
              env: {
                ...process.env,
                STEELENGINE_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
