...
export type PluginCliCommandGroupEntry = {
  ...
  placeholders: readonly SteelEnginePluginCliCommandDescriptor[];
  names: readonly string[];
  register: (program: SteelEnginePluginCliContext["program"]) => Promise<void>;
};
...
async function loadPluginCliMetadataRegistryWithContext(
  ...
  return {
    ...context,
    registry: await loadSteelEnginePluginCliRegistry(
...
async function loadPluginCliCommandRegistryWithContext(params: {
  ...
  return {
    ...params.context,
    registry: loadSteelEnginePlugins(
