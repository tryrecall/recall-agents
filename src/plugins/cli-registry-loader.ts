...
export type PluginCliCommandGroupEntry = {
  ...
  placeholders: readonly RecallPluginCliCommandDescriptor[];
  names: readonly string[];
  register: (program: RecallPluginCliContext["program"]) => Promise<void>;
};
...
export async function loadPluginCliMetadataRegistryWithContext(
  ...
  return {
    ...context,
    registry: await loadRecallPluginCliRegistry(
...
): Promise<RecallPluginCliCommandDescriptor[]> {
  try {
