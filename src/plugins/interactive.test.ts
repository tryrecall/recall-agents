...
describe("plugin interactive handlers", () => {
  ...
  it("hydrates legacy interactive state shapes before clearing handlers", async () => {
    ...
    const stateKey = Symbol.for("steelengine.pluginInteractiveState");
  ...
  it("resolves active registry handlers without retaining them after retirement", async () => {
    ...
    registry.plugins.push({
      id: "steelengine-code-agent",
      name: "SteelEngine Code Agent",
      status: "loaded",
    } as never);
    registry.interactiveHandlers = [
      {
        ...
        pluginId: "steelengine-code-agent",
        pluginName: "SteelEngine Code Agent",
        pluginRoot: "/plugins/steelengine-code-agent",
        handler: handler as never,
      },
    ];
    expect(
      registerRegistryPluginInteractiveHandler(
        "steelengine-code-agent",
        {
          channel: "telegram",
          namespace: "code-agent",
          handler: handler as never,
