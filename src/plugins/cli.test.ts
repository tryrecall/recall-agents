...
const mocks = vi.hoisted(() => ({
  ...
  loadRecallPluginCliRegistry: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadRecallPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadRecallPluginCliRegistry(...args),
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: RecallConfig;
  autoEnabledConfig: RecallConfig;
  ...
  const loadOptions = getMockCallObject(mocks.loadRecallPlugins);
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadRecallPluginCliRegistry.mockReset();
    mocks.loadRecallPluginCliRegistry.mockResolvedValue(createCliRegistry());
    ...
    mocks.loadConfig.mockReturnValue({} as RecallConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as RecallConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as RecallConfig, env);
  ...
  it("injects gateway-backed node runtime into plugin CLI commands", async () => {
    await registerPluginCliCommands(createProgram(), {} as RecallConfig);

    const loadOptions = getMockCallObject(mocks.loadRecallPlugins) as {
  ...
  it("reuses loaded plugin CLI entries on repeat calls for the same program", async () => {
    ...
    expect(mocks.loadRecallPlugins).toHaveBeenCalledTimes(1);
  });

  it("reloads plugin CLI entries when the requested primary command changes", async () => {
    const program = createProgram();

    await registerPluginCliCommands(program, {} as RecallConfig, undefined, undefined, {
      primary: "memory",
    });
    await registerPluginCliCommands(program, {} as RecallConfig);
  ...
  it("reloads plugin CLI entries when config or environment identity changes", async () => {
    const program = createProgram();
    const configA = {} as RecallConfig;
    const configB = { plugins: {} } as RecallConfig;
    const envA = { RECALL_HOME: "/tmp/a" } as NodeJS.ProcessEnv;
    const envB = { RECALL_HOME: "/tmp/b" } as NodeJS.ProcessEnv;
    ...
    expect(mocks.loadRecallPlugins).toHaveBeenCalledTimes(3);
  });
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadRecallPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    const registryOptions = getMockCallObject(mocks.loadRecallPluginCliRegistry);
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadRecallPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as RecallConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("scopes full CLI loading through CLI metadata when manifest planning finds no plugin match", async () => {
    ...
    expect(mocks.loadRecallPluginCliRegistry).toHaveBeenCalled();
    const loadOptions = getMockCallObject(mocks.loadRecallPlugins);
    expect(loadOptions.onlyPluginIds).toEqual(["memory-core"]);
  });
  ...
  it("scopes nested CLI loading through CLI metadata parent paths", async () => {
    ...
    mocks.loadRecallPluginCliRegistry.mockResolvedValue(nestedRegistry);
    mocks.loadRecallPlugins.mockReturnValue(nestedRegistry);
    const program = createProgram("nodes");
    program.exitOverride();
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as RecallConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
