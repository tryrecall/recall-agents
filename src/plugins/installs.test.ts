  ...
  it.each([
    {
      ...
      input: {
        ...
        version: "1.2.3",
        resolvedSpec: "@steelengine/demo@1.2.3",
...
describe("resolveNpmInstallRecordSpec", () => {
  it("uses an exact resolved registry spec when managed installs request pinning", () => {
    expect(
      resolveNpmInstallRecordSpec({
        requestedSpec: "@steelengine/codex",
        resolution: {
          name: "@steelengine/codex",
          version: "2026.5.30-beta.1",
          resolvedSpec: "@steelengine/codex@2026.5.30-beta.1",
        },
        pinResolvedRegistrySpec: true,
      }),
    ).toBe("@steelengine/codex@2026.5.30-beta.1");
  });
    ...
    expect(
      resolveNpmInstallRecordSpec({
        ...
        resolution: {
          name: "@steelengine/codex",
          version: "2026.5.30-beta.1",
          resolvedSpec: "@steelengine/codex@beta",
        },
        pinResolvedRegistrySpec: true,
      }),
    ).toBe("@steelengine/codex");
  ...
  it("replaces local npm-pack provenance on a registry reinstall", () => {
    const localInstall = recordPluginInstall(
      {},
      {
        pluginId: "diffs",
        source: "npm",
        spec: "/tmp/steelengine-diffs.tgz",
        sourcePath: "/tmp/steelengine-diffs.tgz",
        installPath: "/tmp/steelengine/plugins/diffs-local",
        artifactKind: "npm-pack",
        artifactFormat: "tgz",
        npmTarballName: "steelengine-diffs-1.0.0.tgz",
    ...
    const registryInstall = recordPluginInstall(localInstall, {
      ...
      installPath: "/tmp/steelengine/plugins/diffs",
      resolvedName: "@steelengine/diffs",
      resolvedVersion: "1.1.0",
      resolvedSpec: "@steelengine/diffs@1.1.0",
      installedAt: "2026-05-15T00:00:03.000Z",
    });
