...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/recall-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "recall.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): RecallConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    RECALL_HOME: path.join(homeDir, "home"),
    RECALL_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
