...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/steelengine-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "steelengine.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): SteelEngineConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    STEELENGINE_HOME: path.join(homeDir, "home"),
    STEELENGINE_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
