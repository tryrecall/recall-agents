...
describe("loadEnabledBundleMcpConfig", () => {
  it("loads enabled Claude bundle MCP config and absolutizes relative args", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "RECALL_HOME", "RECALL_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("recall-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("recall-bundle-mcp-workspace-");
  ...
  it("merges inline bundle MCP servers and skips disabled bundles", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "RECALL_HOME", "RECALL_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("recall-bundle-inline-home-");
      const workspaceDir = await tempHarness.createTempDir("recall-bundle-inline-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.RECALL_HOME;
      delete process.env.RECALL_STATE_DIR;

      const enabledRoot = path.join(homeDir, ".recall", "extensions", "inline-enabled");
      const disabledRoot = path.join(homeDir, ".recall", "extensions", "inline-disabled");
  ...
  it("resolves inline Claude MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "RECALL_HOME", "RECALL_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("recall-bundle-inline-placeholder-home-");
      const workspaceDir = await tempHarness.createTempDir(
        "recall-bundle-inline-placeholder-workspace-",
      );
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.RECALL_HOME;
      ...
      const pluginRoot = path.join(homeDir, ".recall", "extensions", "inline-claude");
