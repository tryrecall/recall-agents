...
describe("web-provider-runtime-shared", () => {
  ...
  it("caches setup web provider plugin loads by default", () => {
    ...
    const mapRegistryProviders = vi.fn(() => ["provider"]);
    mocks.loadSteelEnginePlugins.mockReturnValue(loadedRegistry as never);
    ...
    expect(providers).toEqual(["provider"]);
    expect(mockArg(mocks.loadSteelEnginePlugins).cache).toBe(true);
    expect(mockArg(mocks.loadSteelEnginePlugins).onlyPluginIds).toEqual(["brave"]);
  });
  ...
  it("falls back to plugin loading when bundled runtime artifacts do not cover the scope", () => {
    ...
    mocks.loadSteelEnginePlugins.mockReturnValue(fallbackRegistry as never);
  ...
  it("loads the plugin registry when runtime activation is explicitly requested", () => {
    ...
    expect(mocks.loadSteelEnginePlugins).toHaveBeenCalledTimes(1);
    expect(mockArg(mocks.loadSteelEnginePlugins).activate).toBe(true);
  });
  ...
  it("keeps explicit setup web provider cache opt-outs", () => {
    ...
    expect(mockArg(mocks.loadSteelEnginePlugins).cache).toBe(false);
    expect(mockArg(mocks.loadSteelEnginePlugins).onlyPluginIds).toEqual(["brave"]);
  });
});
