...
const fixtureRoot = makeTrackedTempDir("recall-sdk-alias-root", fixtureTempDirs);
let tempDirIndex = 0;
...
function createBundledPluginPackagePublicSurfaceAliasFixture() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "@recall/slack", type: "module" }, null, 2),
    "utf-8",
  );
...
function createUserInstalledPluginSdkAliasFixture() {
  ...
  const externalPluginRoot = path.join(makeTempDir(), ".recall", "extensions", "demo");
...
function expectPluginSdkAliasTargets(
  ...
  expect(fs.realpathSync(aliases["recall/plugin-sdk"] ?? "")).toBe(
  ...
  if (params.channelRuntimePath) {
    expect(fs.realpathSync(aliases["recall/plugin-sdk/channel-runtime"] ?? "")).toBe(
  ...
  if (params.pluginEntryPath) {
    expect(fs.realpathSync(aliases["recall/plugin-sdk/plugin-entry"] ?? "")).toBe(
...
describe("plugin sdk alias helpers", () => {
  ...
  it("adds private qa plugin-sdk subpaths for trusted local checkouts when enabled", () => {
    ...
    const subpaths = withEnv({ RECALL_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it("adds non-QA private Codex helper subpaths only for trusted Codex plugins", () => {
    ...
    const { packageRoot: installedCodexRoot, pluginEntry: installedCodexEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".recall", "npm"),
        packageName: "@recall/codex",
      });
    const { packageRoot: installedOtherRoot, pluginEntry: installedOtherEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".recall", "npm"),
        packageName: "@recall/demo",
      });
    const shadowCodexRoot = path.join(makeTempDir(), ".recall", "extensions", "codex-shadow");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "@recall/codex", type: "module" }, null, 2),
      "utf-8",
    );
    fs.writeFileSync(shadowCodexEntry, 'export const plugin = "shadow";\n', "utf-8");

    const codexSubpaths = withEnv({ RECALL_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceCodexEntry,
      }),
    );
    const otherSubpaths = withEnv({ RECALL_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceOtherEntry,
      }),
    );
    const installedCodexSubpaths = withCwd(installedCodexRoot, () =>
      withEnv({ RECALL_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
        listPluginSdkExportedSubpaths({
          modulePath: installedCodexEntry,
          argv1: path.join(fixture.root, "recall.mjs"),
        }),
      ),
    );
  ...
  it("does not reuse a non-private cached subpath list after private qa gets enabled", () => {
    ...
    const privateSubpaths = withEnv({ RECALL_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it.each([
    {
      name: "does not derive plugin-sdk subpaths from cwd fallback when package root is not an Recall root",
      fixture: () =>
        createPluginSdkAliasFixture({
  ...
  it("adds private qa plugin-sdk aliases for source plugins when enabled", () => {
    ...
    const aliases = withEnv({ RECALL_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined }, () =>
      buildPluginLoaderAliasMap(sourcePluginEntry),
    );
    ...
    expect(fs.realpathSync(aliases["recall/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
    expect(fs.realpathSync(aliases["recall/plugin-sdk/qa-channel"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelPath),
    );
    expect(fs.realpathSync(aliases["recall/plugin-sdk/qa-channel-protocol"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelProtocolPath),
    );
    expect(fs.realpathSync(aliases["recall/plugin-sdk/qa-lab"] ?? "")).toBe(
  ...
  it("aliases non-QA private plugin-sdk subpaths for trusted Codex runtime loading", () => {
    ...
    const aliases = withEnv(
      { RECALL_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined },
      () => buildPluginLoaderAliasMap(sourcePluginEntry),
    );
    ...
    const installedAliases = withCwd(installedCodexRoot, () =>
      withEnv({ RECALL_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(
          installedCodexEntry,
          path.join(fixture.root, "recall.mjs"),
          undefined,
          "dist",
        ),
      ),
    ...
    expect(fs.realpathSync(aliases["recall/plugin-sdk/codex-mcp-projection"] ?? "")).toBe(
      fs.realpathSync(sourceCodexMcpProjectionPath),
    );
    expect(fs.realpathSync(aliases["recall/plugin-sdk/codex-native-task-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceCodexNativeTaskRuntimePath),
    );
    expect(
      fs.realpathSync(installedAliases["recall/plugin-sdk/codex-mcp-projection"] ?? ""),
    ).toBe(fs.realpathSync(distCodexMcpProjectionPath));
    expect(
      fs.realpathSync(installedAliases["recall/plugin-sdk/codex-native-task-runtime"] ?? ""),
    ).toBe(fs.realpathSync(distCodexNativeTaskRuntimePath));
    expect(aliases["recall/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(otherAliases["recall/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(otherAliases["recall/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
    expect(installedOtherAliases["recall/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(installedOtherAliases["recall/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
    expect(shadowCodexAliases["recall/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(shadowCodexAliases["recall/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
  });
  ...
  it("aliases bundled plugin package public surfaces for source plugin transforms", () => {
    ...
    expect(fs.realpathSync(aliases["@recall/slack/api.js"] ?? "")).toBe(
      fs.realpathSync(sourceApiPath),
    );
    expect(fs.realpathSync(aliases["@recall/slack/runtime-api.js"] ?? "")).toBe(
      fs.realpathSync(sourceRuntimeApiPath),
    );
    expect(aliases["@recall/slack/test-api.js"]).toBeUndefined();
    expect(aliases["@recall/slack/internal.js"]).toBeUndefined();
  });
  ...
  it("aliases bundled plugin package test surfaces only in private QA mode", () => {
    ...
    expect(fs.realpathSync(aliases["@recall/slack/test-api.js"] ?? "")).toBe(
  ...
  it("falls back to source plugin-sdk subpath aliases when dist chunks are stale", () => {
    ...
    expect(fs.realpathSync(distAliases["recall/plugin-sdk/provider-entry"] ?? "")).toBe(
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via the running recall argv hint", () => {
    const {
      externalPluginEntry,
      externalPluginRoot,
      fixture,
    ...
    const aliases = withCwd(externalPluginRoot, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(externalPluginEntry, path.join(fixture.root, "recall.mjs")),
      ),
    );
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via moduleUrl hint", () => {
    ...
    // This covers installations where argv1 does not resolve to the recall root
    // (e.g. single-binary distributions or custom process launchers).
    // Use recall.mjs which is created by createPluginSdkAliasFixture (bin+marker mode).
    // Use fixture.root as cwd so process.cwd() fallback also resolves to fixture, not the
    // real recall repo root in the test runner environment.
    const loaderModuleUrl = pathToFileURL(path.join(fixture.root, "recall.mjs")).href;
    ...
    // runner binary, inside the recall repo), which resolves before moduleUrl is checked.
    // An empty string is falsy so resolveTrustedRecallRootFromArgvHint returns null,
  ...
  it.each([
    {
      name: "does not resolve plugin-sdk alias files from cwd fallback when package root is not an Recall root",
      fixture: () =>
        createPluginSdkAliasFixture({
          srcFile: "channel-runtime.ts",
          distFile: "channel-runtime.js",
  ...
  it("scopes plugin loader module config by plugin-sdk resolution", () => {
    ...
    expect(fs.realpathSync(auto.aliasMap["recall/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(dist.aliasMap["recall/plugin-sdk"] ?? "")).toBe(
...
`,
    ...
    const withAlias = createJiti(sourceLoaderBaseUrl, {
      ...buildPluginLoaderJitiOptions({
        "recall/plugin-sdk/outbound-send-deps": copiedChannelRuntimeShim,
        "@recall/plugin-sdk/outbound-send-deps": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
...
describe("buildPluginLoaderAliasMap memoization", () => {
  ...
  it("returns different references for different modulePath inputs", () => {
    ...
    expect(aliasA["recall/plugin-sdk"]).not.toBe(aliasB["recall/plugin-sdk"]);
  });
  ...
  it("does not reuse a public alias map after private qa aliases are enabled", () => {
    ...
    const publicAliases = withEnv({ RECALL_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      buildPluginLoaderAliasMap(entry),
    );
    const privateAliases = withEnv({ RECALL_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      buildPluginLoaderAliasMap(entry),
    );

    expect(publicAliases).not.toBe(privateAliases);
    expect(publicAliases["recall/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(fs.realpathSync(privateAliases["recall/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
  });
  ...
  it("does not reuse a development alias map in production mode", () => {
    ...
    expect(fs.realpathSync(developmentAliases["recall/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(productionAliases["recall/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(distRootAlias),
    );
  });
...
describe("buildPluginLoaderJitiOptions", () => {
  it("pre-normalizes and marks alias maps for source transforms", () => {
    ...
    const aliasMap = {
      "recall/plugin-sdk/core": "/repo/src/plugin-sdk/core.ts",
      "recall/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
      "@recall/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
    };
