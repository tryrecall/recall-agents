...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "steelengine-doctor-contract-load-paths-"),
  );
  tempDirs.push(dir);
  return dir;
}
...
function createDoctorPluginConfig(pluginRoot: string, pluginId: string): SteelEngineConfig {
  return {
    plugins: {
...
function readPluginLlmPolicy(config: SteelEngineConfig, pluginId: string): Record<string, unknown> {
