...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "recall-doctor-contract-load-paths-"),
  );
  tempDirs.push(dir);
  return dir;
}
...
function createDoctorPluginConfig(pluginRoot: string, pluginId: string): RecallConfig {
  return {
    plugins: {
...
function readPluginLlmPolicy(config: RecallConfig, pluginId: string): Record<string, unknown> {
