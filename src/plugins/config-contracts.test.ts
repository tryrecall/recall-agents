...
const mocks = vi.hoisted(() => {
  ...
  return {
    discoverRecallPlugins: vi.fn(() => ({ candidates: [], diagnostics: [] })),
...
vi.mock("./discovery.js", () => ({
  discoverRecallPlugins: mocks.discoverRecallPlugins,
}));
...
function createPluginRecord(
  ...
  return {
    ...
    hooks: [],
    source: `/tmp/${overrides.id}/recall.plugin.json`,
...
describe("resolvePluginConfigContractsById", () => {
  beforeEach(() => {
    mocks.discoverRecallPlugins.mockReset();
    mocks.discoverRecallPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
