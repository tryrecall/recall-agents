...
function makeStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-plugin-index-records-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin index install records store", () => {
    ...
    await writePersistedInstalledPluginIndexInstallRecords(
      {
        twitch: {
          source: "npm",
          spec: "@recall/plugin-twitch@1.0.0",
          installPath: "plugins/npm/@recall/plugin-twitch",
        },
      },
      {
        stateDir,
  ...
  it("recovers managed npm plugin records when the persisted ledger is empty", async () => {
    ...
    expectRecordFields(loaded.discord, {
      source: "npm",
      spec: "@recall/discord@2026.5.2",
      installPath: discordDir,
      version: "2026.5.2",
      resolvedName: "@recall/discord",
      resolvedVersion: "2026.5.2",
  ...
  it("recovers managed npm metadata when the persisted record points at an older package version", async () => {
    ...
    await writePersistedInstalledPluginIndexInstallRecords(
      {
        codex: {
          source: "npm",
          spec: "@recall/codex@2026.5.16-beta.1",
          installPath: codexDir,
          version: "2026.5.16-beta.1",
          resolvedName: "@recall/codex",
          resolvedVersion: "2026.5.16-beta.1",
          resolvedSpec: "@recall/codex@2026.5.16-beta.1",
    ...
    const record = expectRecordFields(loaded.codex, {
      source: "npm",
      spec: "@recall/codex@2026.5.18-beta.1",
      installPath: codexDir,
      version: "2026.5.18-beta.1",
      resolvedName: "@recall/codex",
      resolvedVersion: "2026.5.18-beta.1",
      resolvedSpec: "@recall/codex@2026.5.18-beta.1",
    });
