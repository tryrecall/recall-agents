...
function listExternalBundledPluginDirs(): string[] | null {
  ...
  for (const file of files) {
    const match = /^extensions\/([^/]+)\/(steelengine\.plugin\.json|package\.json)$/u.exec(file);
    if (!match) {
      continue;
    }
  ...
  return [...metadataByDir.entries()]
    .filter(
      ([, metadataFiles]) =>
        metadataFiles.has("package.json") && metadataFiles.has("steelengine.plugin.json"),
    )
    .map(([dirName]) => dirName)
    .toSorted();
}
...
function listGitPluginMetadataFiles(): string[] | null {
  return listGitTrackedFiles({
    pathspecs: ["extensions/*/package.json", "extensions/*/steelengine.plugin.json"],
  });
}
...
function readBundledPluginRecords(): BundledPluginRecord[] {
  return listBundledPluginDirs().flatMap((dirName) => {
    ...
    const pkg = readJsonFile(packagePath) as SteelEnginePackageShape;
    ...
    return [
      {
        ...
        packageName,
        manifestId,
        installNpmSpec: normalizeText(pkg.steelengine?.install?.npmSpec),
        channelId: normalizeText(pkg.steelengine?.channel?.id),
      },
    ];
  });
}
...
function resolveAllowedPackageNamesForId(pluginId: string): string[] {
  return ALLOWED_PACKAGE_SUFFIXES.map((suffix) => `@steelengine/${pluginId}${suffix}`);
}
  ...
  it.each([
    {
      ...
      message: `Bundled extension package names must stay anchored to the manifest id via @steelengine/<id> or an approved suffix (${ALLOWED_PACKAGE_SUFFIXES.join(", ")}). Update the plugin naming docs and this invariant before adding a new naming form.`,
    ...
    },
    {
      ...
      message:
        "Bundled extension directory names should match steelengine.plugin.json:id. If a legacy exception is unavoidable, add it to DIR_ID_EXCEPTIONS with a comment.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    {
      name: "keeps bundled steelengine.install.npmSpec aligned with the package name",
      message:
        "Bundled steelengine.install.npmSpec values must match the package name so install/update paths stay deterministic.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    },
    {
      ...
      message:
        "Non-packaged bundled plugins are source-only/private and must not advertise steelengine.install.npmSpec.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
    ...
    },
    {
      ...
      message:
        "Bundled steelengine.channel.id values must match steelengine.plugin.json:id for the owning plugin.",
      collectMismatches: (records: BundledPluginRecord[]) =>
        records
          .filter(
