...
import type { SteelEnginePluginModule } from "./types.js";
...
export async function loadSteelEnginePluginCliRegistry(
  ...
  for (const candidate of orderedCandidates) {
    ...
    try {
      mod = withProfile(
        ...
        "cli-metadata",
        () => loadPluginModule(safeSource) as SteelEnginePluginModule,
      );
    } catch (error) {
      recordPluginError({
        logger,
