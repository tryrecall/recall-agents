...
function isTrustedSourceLinkedOfficialPackage(pkg: NonNullable<ClawHubPackageDetail["package"]>) {
  ...
  return (
    ...
    (sourceRepo === "steelengine/steelengine" ||
      sourceRepo === "github.com/steelengine/steelengine" ||
      sourceRepo === "https://github.com/steelengineai/recall-agents")
  );
}
...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    ...
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "steelengine skills install ${installRef}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this SteelEngine runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires SteelEngine >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
...
> {
  ...
  try {
    ...
    const clawhubAuthority = isDefaultClawHubBaseUrl(params.baseUrl) ? "steelengine" : "third-party";
    params.logger?.info?.(
