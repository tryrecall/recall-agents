// Shared policy and messaging for installs outside SteelEngine's trusted plugin sources.
...
export function resolveSteelEngineTrustedNpmPackageInstall(
  npmSpec: string,
...
export function isSteelEngineTrustedPluginInstallSpec(
  spec: string,
  bundledSources: ReadonlyMap<string, BundledPluginSource> = getProcessBundledPluginSources(),
): boolean {
  ...
  if (explicitNpm) {
    return resolveSteelEngineTrustedNpmPackageInstall(npmSpec, bundledSources) !== null;
  }
  ...
  return Boolean(
    bundled ??
    resolveSteelEngineTrustedNpmPackageInstall(npmSpec, bundledSources) ??
