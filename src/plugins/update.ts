...
import {
  ...
  installedPackageNeedsRecallPeerLinkRepair,
...
import { linkRecallPeerDependencies } from "./plugin-peer-link.js";
...
function isBundledVersionNewer(bundledVersion: string, installedVersion: string): boolean {
  const releaseCmp = compareRecallReleaseVersions(bundledVersion, installedVersion);
  if (releaseCmp !== null) {
    return releaseCmp > 0;
  }
...
function migratePluginConfigId(cfg: RecallConfig, fromId: string, toId: string): RecallConfig {
  if (fromId === toId) {
    return cfg;
  }
...
function withoutPluginInstallRecord(cfg: RecallConfig, pluginId: string): RecallConfig {
  const installs = cfg.plugins?.installs;
...
function resetDisabledPluginSlots(
  slots: NonNullable<RecallConfig["plugins"]>["slots"] | undefined,
  pluginId: string,
): NonNullable<RecallConfig["plugins"]>["slots"] | undefined {
  if (!slots) {
    return slots;
  }
  let next = slots;
...
function disablePluginConfigEntry(config: RecallConfig, pluginId: string): RecallConfig {
...
async function repairRecallPeerLinksForNpmInstalls(params: {
  config: RecallConfig;
  logger: PluginUpdateLogger;
}): Promise<boolean> {
  let repaired = false;
  for (const [pluginId, record] of Object.entries(params.config.plugins?.installs ?? {})) {
    ...
    try {
      ...
      params.logger.warn?.(
        `Could not repair recall peer link for "${pluginId}" due to invalid install path: ${String(err)}`,
      );
      continue;
    }

    if (!installedPackageNeedsRecallPeerLinkRepair(installPath)) {
      continue;
    }
    ...
    try {
      ...
      if (peerLinkRepair.skipped > 0) {
        params.logger.warn?.(
          `Could not repair recall peer link for "${pluginId}" at ${installPath}: ${warnings.join("; ") || "peer link repair was skipped"}`,
        );
        continue;
      }
      repaired = !installedPackageNeedsRecallPeerLinkRepair(installPath) || repaired;
    } catch (err) {
      params.logger.warn?.(
        `Could not repair recall peer link for "${pluginId}" at ${installPath}: ${String(err)}`,
      );
    }
  }
  return repaired;
...
export async function updateNpmInstalledPlugins(params: {
  ...
  const recordFailure = (pluginId: string, message: string) => {
    if (params.disableOnFailure && !params.dryRun) {
      const disabledMessage =
        `Disabled "${pluginId}" after plugin update failure; Recall will continue without it. ` +
        message;
    ...
    if (!params.dryRun && record.source === "npm" && currentVersion) {
      ...
      if (metadataResult.ok) {
        if (
          ...
          !installedPackageNeedsRecallPeerLinkRepair(installPath) &&
  ...
  if (ranNpmInstaller) {
    changed =
      (await repairRecallPeerLinksForNpmInstalls({
        config: next,
        logger,
      })) || changed;
  }
