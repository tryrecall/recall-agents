...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("recall-bundle-commands-home-");
      const workspaceDir = await tempHarness.createTempDir("recall-bundle-commands-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.RECALL_HOME;
      delete process.env.RECALL_STATE_DIR;

      const pluginRoot = path.join(homeDir, ".recall", "extensions", "compound-bundle");
