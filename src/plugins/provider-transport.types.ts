...
/**
 * Provider-owned stream wrapper hook after SteelEngine applies its generic
 ...
 *
 * Return `true` or `false` to override SteelEngine's built-in provider cache TTL
 ...
 *
 * Runs only after SteelEngine exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that SteelEngine should append to the generic `Unknown model`
 * error.
 */
