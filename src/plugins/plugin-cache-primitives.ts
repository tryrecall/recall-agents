...
export type ConfigScopedRuntimeCache<T> = WeakMap<SteelEngineConfig, Map<string, T>>;
...
type ConfigScopedPromiseLoader<T> = {
  load(config?: SteelEngineConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: SteelEngineConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<SteelEngineConfig, Promise<T>>();

  const createPromise = (config?: SteelEngineConfig): Promise<T> => {
  ...
  return {
    async load(config?: SteelEngineConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<SteelEngineConfig, Promise<T>>();
    },
  };
}
