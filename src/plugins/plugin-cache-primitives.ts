...
export type ConfigScopedRuntimeCache<T> = WeakMap<RecallConfig, Map<string, T>>;

export type ConfigScopedPromiseLoader<T> = {
  load(config?: RecallConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: RecallConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<RecallConfig, Promise<T>>();

  const createPromise = (config?: RecallConfig): Promise<T> => {
  ...
  return {
    async load(config?: RecallConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<RecallConfig, Promise<T>>();
    },
  };
}
