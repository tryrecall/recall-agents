...
function makePluginDir() {
  return makeTrackedTempDir("steelengine-manifest-model-catalog", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "steelengine.plugin.json"), JSON.stringify(manifest), "utf8");
}
