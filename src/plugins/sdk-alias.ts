...
function hasTrustedRecallRootIndicator(params: {
  packageRoot: string;
  ...
  const hasRecallBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("recall")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.recall === "string");
  const hasRecallEntrypoint = fs.existsSync(path.join(params.packageRoot, "recall.mjs"));
  return hasCliEntryExport || hasRecallBin || hasRecallEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedRecallRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedRecallRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  return hasTrustedRecallRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveRecallPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedRecallRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["recall/plugin-sdk", "@recall/plugin-sdk"] as const;
const OFFICIAL_CODEX_PLUGIN_PACKAGE_NAME = "@recall/codex";
...
function readBundledPluginPackageName(packageJsonPath: string): string | null {
  ...
  return name.startsWith("@recall/") ? name : null;
}
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.RECALL_ENABLE_PRIVATE_QA_CLI === "1";
}
...
function isOfficialInstalledCodexPluginPackageRoot(packageRoot: string) {
  ...
  return last === "codex" && scope === "@recall" && nodeModules === "node_modules";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.prototype.hasOwnProperty.call(aliasMap, `recall/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "recall/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
