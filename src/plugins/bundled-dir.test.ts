...
const originalBundledDir = process.env.STEELENGINE_BUNDLED_PLUGINS_DIR;
...
function createSteelEngineRoot(params: {
  prefix: string;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "steelengine" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function seedBundledPluginTree(rootDir: string, relativeDir: string, pluginId = "discord") {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: `@steelengine/${pluginId}` }, null, 2)}\n`,
    "utf8",
  );
  fs.writeFileSync(
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.STEELENGINE_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
  if (params.disableBundledPlugins === undefined) {
    delete process.env.STEELENGINE_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.STEELENGINE_DISABLE_BUNDLED_PLUGINS = params.disableBundledPlugins;
  }
...
function expectResolvedBundledDirFromRoot(params: {
  ...
  expectResolvedBundledDir({
    ...
    argv1: params.argv1 ?? path.join(params.repoRoot, "steelengine.mjs"),
...
afterEach(() => {
  ...
  if (originalBundledDir === undefined) {
    ...
    process.env.STEELENGINE_BUNDLED_PLUGINS_DIR = originalBundledDir;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "steelengine-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "steelengine-bundled-dir-dist-",
        hasDistExtensions: true,
      },
      {
        expectedRelativeDir: path.join("dist", "extensions"),
    ...
    [
      ...
      {
        prefix: "steelengine-bundled-dir-git-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "steelengine-bundled-dir-vitest-",
        hasExtensions: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
      },
    ...
    [
      ...
      {
        prefix: "steelengine-bundled-dir-tsx-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "steelengine-bundled-dir-git-",
        hasExtensions: true,
        hasSrc: true,
        hasGitCheckout: true,
        hasPnpmWorkspace: true,
    ...
    const repoRoot = createSteelEngineRoot(layout);
  ...
  it("falls back to source extensions when dist trees exist but do not contain real plugin manifests", () => {
    const repoRoot = createSteelEngineRoot({
      prefix: "steelengine-bundled-dir-incomplete-built-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("uses source extensions in pnpm workspace mirrors without git metadata", () => {
    const repoRoot = createSteelEngineRoot({
      prefix: "steelengine-bundled-dir-source-mirror-",
      hasExtensions: true,
      hasSrc: true,
      hasPnpmWorkspace: true,
    });
  ...
  it("keeps built bundled plugins for git-looking trees without pnpm workspace metadata", () => {
    const repoRoot = createSteelEngineRoot({
      prefix: "steelengine-bundled-dir-git-no-pnpm-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("reports missing pnpm workspace deps for source checkouts", () => {
    const repoRoot = createSteelEngineRoot({
      prefix: "steelengine-bundled-dir-source-deps-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
      hasPnpmWorkspace: true,
    ...
    process.argv[1] = path.join(repoRoot, "steelengine.mjs");
  ...
  it("returns a stable empty bundled plugin directory when bundled plugins are disabled", () => {
    const repoRoot = createSteelEngineRoot({
      prefix: "steelengine-bundled-dir-disabled-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("separates tilde override cache entries by STEELENGINE_HOME", () => {
    const homeA = makeRepoRoot("steelengine-bundled-dir-home-a-");
    const homeB = makeRepoRoot("steelengine-bundled-dir-home-b-");
    ...
    const bundledA = resolveBundledPluginsDir({ ...envBase, STEELENGINE_HOME: homeA });
    const bundledB = resolveBundledPluginsDir({ ...envBase, STEELENGINE_HOME: homeB });
  ...
  it("ignores an existing override under an argv1-derived fake package root", () => {
    const installedRoot = createSteelEngineRoot({
      prefix: "steelengine-bundled-dir-argv-override-reject-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));

    vi.spyOn(process, "cwd").mockReturnValue(installedRoot);
    process.argv[1] = path.join(installedRoot, "steelengine.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.STEELENGINE_BUNDLED_PLUGINS_DIR = path.join(installedRoot, "dist", "extensions");
    delete process.env.STEELENGINE_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST relax existing override trust checks", () => {
    const overrideRoot = makeRepoRoot("steelengine-bundled-dir-vitest-override-reject-");
    ...
    process.env.STEELENGINE_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
  ...
  it("does not let VITEST add cwd to bundled plugin resolution candidates", () => {
    const cwdRepoRoot = createSteelEngineRoot({
      prefix: "steelengine-bundled-dir-vitest-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("falls back from a missing override instead of returning an untrusted future path", () => {
    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("steelengine-bundled-dir-missing-cwd-"));
    process.argv[1] = "/usr/bin/env";
    process.execArgv.length = 0;
    delete process.env.VITEST;
    const missingOverride = path.join(
      makeRepoRoot("steelengine-bundled-dir-missing-override-"),
      "extensions",
    );
    process.env.STEELENGINE_BUNDLED_PLUGINS_DIR = missingOverride;
    delete process.env.STEELENGINE_DISABLE_BUNDLED_PLUGINS;
  ...
  it("falls back to argv root when an existing rejected override is unrelated", () => {
    const installedRoot = createSteelEngineRoot({
      prefix: "steelengine-bundled-dir-rejected-override-argv-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
    const overrideRoot = makeRepoRoot("steelengine-bundled-dir-rejected-override-");
    seedBundledPluginTree(overrideRoot, "extensions", "memory-core");

    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("steelengine-bundled-dir-rejected-cwd-"));
    process.argv[1] = path.join(installedRoot, "steelengine.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.STEELENGINE_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
  ...
  it("ignores an enclosing checkout reached through node_modules tooling argv1", () => {
    ...
    const outerRoot = createSteelEngineRoot({
      prefix: "steelengine-bundled-dir-enclosing-",
      hasExtensions: true,
      hasSrc: true,
      hasDistExtensions: true,
      hasGitCheckout: true,
  ...
  it("does not resolve bundled plugins from cwd when argv1 is not a package root", () => {
    const cwdRepoRoot = createSteelEngineRoot({
      prefix: "steelengine-bundled-dir-untrusted-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createSteelEngineRoot({
          prefix: "steelengine-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        const cwdRepoRoot = createSteelEngineRoot({
          prefix: "steelengine-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "steelengine.mjs"),
        };
      },
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createSteelEngineRoot({
          prefix: "steelengine-bundled-dir-override-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        return {
