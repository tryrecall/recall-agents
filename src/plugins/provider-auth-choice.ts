...
function resolveConfiguredDefaultModelPrimary(cfg: SteelEngineConfig): string | undefined {
...
async function applyDefaultModelFromAuthChoice(params: {
  config: SteelEngineConfig;
  configBeforeProviderAuth?: SteelEngineConfig;
  selectedModel: string;
  selectedModelDisplay?: string;
  preserveExistingDefaultModel: boolean | undefined;
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  workspaceDir?: string;
  runSelectedModelHook: (config: SteelEngineConfig) => Promise<void>;
...
export async function runProviderPluginAuthMethod(params: {
  ...
  allowSecretRefPrompt?: boolean;
  opts?: Partial<ProviderAuthOptionBag>;
}): Promise<{ config: SteelEngineConfig; defaultModel?: string }> {
...
export async function applyAuthChoiceLoadedPluginProvider(
  ...
  const resolveScopedRuntimeProviders = (config: SteelEngineConfig): ProviderPlugin[] =>
    resolvePluginProviders({
      config,
      workspaceDir,
      env: params.env,
