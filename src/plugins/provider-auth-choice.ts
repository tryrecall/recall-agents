...
function resolveConfiguredDefaultModelPrimary(cfg: RecallConfig): string | undefined {
...
async function applyDefaultModelFromAuthChoice(params: {
  config: RecallConfig;
  configBeforeProviderAuth?: RecallConfig;
  selectedModel: string;
  selectedModelDisplay?: string;
  preserveExistingDefaultModel: boolean | undefined;
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  workspaceDir?: string;
  runSelectedModelHook: (config: RecallConfig) => Promise<void>;
...
export async function runProviderPluginAuthMethod(params: {
  ...
  opts?: Partial<ProviderAuthOptionBag>;
}): Promise<{ config: RecallConfig; defaultModel?: string }> {
...
export async function applyAuthChoiceLoadedPluginProvider(
  ...
  const resolveScopedRuntimeProviders = (config: RecallConfig): ProviderPlugin[] =>
    resolvePluginProviders({
      config,
      workspaceDir,
      env: params.env,
...
export async function applyAuthChoicePluginProvider(
  ...
  if (!provider) {
    await params.prompter.note(
      `${options.label} auth plugin is not available. Install or enable the plugin, then rerun onboarding. If this started after an update, run "recall doctor --fix" first.`,
      options.label,
    );
    return { config: nextConfig };
  }
