...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "recall-install-sh-"));
  const cliPath = path.join(root, "recall");
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Recall 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Recall dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("Recall dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "recall-install-stdin-"));
