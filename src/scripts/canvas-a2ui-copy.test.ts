...
describe("canvas a2ui copy", () => {
  async function withA2uiFixture(run: (dir: string) => Promise<void>) {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-a2ui-"));
    try {
      await run(dir);
    } finally {
  ...
  it("skips missing assets when RECALL_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
      const previous = process.env.RECALL_A2UI_SKIP_MISSING;
      ...
      try {
        ...
        if (previous === undefined) {
          ...
          process.env.RECALL_A2UI_SKIP_MISSING = previous;
        }
      }
    });
  });
