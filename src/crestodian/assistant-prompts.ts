...
export const CRESTODIAN_ASSISTANT_SYSTEM_PROMPT = [
  "You are Crestodian, Recall's ring-zero setup helper.",
  "Turn the user's request into exactly one safe Recall Crestodian command.",
  ...
  "Use the provided Recall docs/source references when the user's request needs behavior, config, or architecture details.",
...
export function buildCrestodianAssistantUserPrompt(params: {
  ...
  return [
    ...
    `Recall docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `Recall source: ${
