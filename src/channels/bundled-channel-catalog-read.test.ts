...
// resolveRecallPackageRootSync. Isolate from the real repo by mocking
// moduleUrl/argv1 resolution to null and deriving only from the tmp cwd.
vi.mock("../infra/recall-root.js", () => ({
  resolveRecallPackageRootSync: (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
  resolveRecallPackageRoot: async (opts: { cwd?: string; argv1?: string; moduleUrl?: string }) =>
    opts.cwd ?? null,
}));
...
const originalTrustBundledPluginsDir = process.env.RECALL_TEST_TRUST_BUNDLED_PLUGINS_DIR;
...
afterEach(() => {
  ...
  if (originalTrustBundledPluginsDir === undefined) {
    ...
    process.env.RECALL_TEST_TRUST_BUNDLED_PLUGINS_DIR = originalTrustBundledPluginsDir;
  }
...
function useBundledPluginsDir(extensionsRoot: string | undefined): void {
  if (extensionsRoot) {
    process.env.RECALL_BUNDLED_PLUGINS_DIR = extensionsRoot;
...
function seedRoot(prefix: string): string {
  ...
  writeJsonFile(path.join(root, "package.json"), { name: "recall" });
...
function seedChannelPkg(
  ...
  writeJsonFile(path.join(pluginDir, "recall.plugin.json"), {
    id: opts.id,
...
describe("listBundledChannelCatalogEntries", () => {
  ...
  it("falls back to dist/channel-catalog.json when the resolver returns undefined", () => {
    // RECALL_DISABLE_BUNDLED_PLUGINS, missing bundled tree, or an unresolvable
  ...
  it("falls back to dist/channel-catalog.json when the resolved dir has no plugin package.jsons", () => {
    // A stale staged dir or an RECALL_BUNDLED_PLUGINS_DIR override pointing at
