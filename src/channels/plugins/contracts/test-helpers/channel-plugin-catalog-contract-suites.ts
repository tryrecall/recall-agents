    ...
    it.each([
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-catalog-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@steelengine/demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-channel-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-catalog-rich-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
            JSON.stringify({
              ...
              schemaVersion: 1,
              description:
                "Extension manifest. Declares plugin packages that SteelEngine can discover during onboarding and install on demand via `steelengine plugins install`.",
              entries: [
                {
                  name: "@wecom/wecom-steelengine-plugin",
                  description:
                    "SteelEngine WeCom (企业微信) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  steelengine: {
                    channel: {
          ...
          return {
            ...
            expected: {
              ...
              installSource: {
                defaultChoice: "npm",
                npm: {
                  spec: "@wecom/wecom-steelengine-plugin@1.2.3",
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-catalog-prerelease-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(catalogPath, {
            ...createCatalogEntry({
              packageName: "@steelengine/prerelease-demo-channel",
          ...
          return {
            ...
            catalogPaths: [catalogPath],
            expected: {
              install: { npmSpec: "@steelengine/prerelease-demo-channel@2026.5.3-beta.1" },
              installSource: {
                npm: {
                  spec: "@steelengine/prerelease-demo-channel@2026.5.3-beta.1",
                  packageName: "@steelengine/prerelease-demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-catalog-clawhub-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
          ...
          return {
            ...
            expected: {
              ...
              installSource: {
                defaultChoice: "clawhub",
                clawhub: {
                  spec: "clawhub:steelengine/clawhub-chat@2026.5.2",
                  packageName: "steelengine/clawhub-chat",
                  version: "2026.5.2",
                  exactVersion: true,
                },
                warnings: [],
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-catalog-yuanbao-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
              ...
              entries: [
                {
                  ...
                  description:
                    "SteelEngine Yuanbao (元宝) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  steelengine: {
                    channel: {
                    ...
                    install: {
                      npmSpec: "steelengine-plugin-yuanbao@1.0.0",
                      defaultChoice: "npm",
                      minHostVersion: ">=2026.4.10",
          ...
          return {
            channelId: "steelengine-plugin-yuanbao",
            catalogPaths: [catalogPath],
            expected: {
              id: "steelengine-plugin-yuanbao",
              meta: {
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(
            path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-catalog-home-"),
          );
          const catalogPath = path.join(home, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@steelengine/env-demo-channel",
          ...
          return {
            env: {
              ...process.env,
              STEELENGINE_PLUGIN_CATALOG_PATHS: "~/catalog.json",
      ...
      },
      {
        ...
        setup: () => {
          ...
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@steelengine/default-env-demo",
