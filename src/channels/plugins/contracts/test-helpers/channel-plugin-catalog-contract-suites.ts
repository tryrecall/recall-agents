    ...
    it.each([
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredRecallTmpDir(), "recall-catalog-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@recall/demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredRecallTmpDir(), "recall-channel-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredRecallTmpDir(), "recall-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredRecallTmpDir(), "recall-catalog-rich-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
            JSON.stringify({
              ...
              schemaVersion: 1,
              description:
                "Extension manifest. Declares plugin packages that Recall can discover during onboarding and install on demand via `recall plugins install`.",
              entries: [
                {
                  name: "@wecom/wecom-recall-plugin",
                  description:
                    "Recall WeCom (企业微信) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  recall: {
                    channel: {
          ...
          return {
            ...
            expected: {
              ...
              installSource: {
                defaultChoice: "npm",
                npm: {
                  spec: "@wecom/wecom-recall-plugin@1.2.3",
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredRecallTmpDir(), "recall-catalog-prerelease-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(catalogPath, {
            ...createCatalogEntry({
              packageName: "@recall/prerelease-demo-channel",
          ...
          return {
            ...
            catalogPaths: [catalogPath],
            expected: {
              install: { npmSpec: "@recall/prerelease-demo-channel@2026.5.3-beta.1" },
              installSource: {
                npm: {
                  spec: "@recall/prerelease-demo-channel@2026.5.3-beta.1",
                  packageName: "@recall/prerelease-demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredRecallTmpDir(), "recall-catalog-clawhub-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
          ...
          return {
            ...
            expected: {
              ...
              installSource: {
                defaultChoice: "clawhub",
                clawhub: {
                  spec: "clawhub:recall/clawhub-chat@2026.5.2",
                  packageName: "recall/clawhub-chat",
                  version: "2026.5.2",
                  exactVersion: true,
                },
                warnings: [],
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredRecallTmpDir(), "recall-catalog-yuanbao-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
              ...
              entries: [
                {
                  ...
                  description:
                    "Recall Yuanbao (元宝) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  recall: {
                    channel: {
                    ...
                    install: {
                      npmSpec: "recall-plugin-yuanbao@1.0.0",
                      defaultChoice: "npm",
                      minHostVersion: ">=2026.4.10",
          ...
          return {
            channelId: "recall-plugin-yuanbao",
            catalogPaths: [catalogPath],
            expected: {
              id: "recall-plugin-yuanbao",
              meta: {
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(
            path.join(resolvePreferredRecallTmpDir(), "recall-catalog-home-"),
          );
          const catalogPath = path.join(home, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@recall/env-demo-channel",
          ...
          return {
            env: {
              ...process.env,
              RECALL_PLUGIN_CATALOG_PATHS: "~/catalog.json",
              RECALL_HOME: home,
              HOME: home,
            },
      ...
      },
      {
        ...
        setup: () => {
          ...
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@recall/default-env-demo",
