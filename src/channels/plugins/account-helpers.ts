...
export function createAccountListHelpers(
  ...
  options?: {
    ...
    hasImplicitDefaultAccount?: (cfg: SteelEngineConfig) => boolean;
  },
) {
  function hasImplicitDefaultAccount(cfg: SteelEngineConfig): boolean {
  ...
  function resolveConfiguredDefaultAccountId(cfg: SteelEngineConfig): string | undefined {
    const channel = cfg.channels?.[channelKey] as Record<string, unknown> | undefined;
  ...
  function listAccountIds(cfg: SteelEngineConfig): string[] {
  ...
  function resolveDefaultAccountId(cfg: SteelEngineConfig): string {
