...
export function createAccountListHelpers(
  ...
  options?: {
    ...
    hasImplicitDefaultAccount?: (cfg: RecallConfig) => boolean;
  },
) {
  function hasImplicitDefaultAccount(cfg: RecallConfig): boolean {
  ...
  function resolveConfiguredDefaultAccountId(cfg: RecallConfig): string | undefined {
    const channel = cfg.channels?.[channelKey] as Record<string, unknown> | undefined;
  ...
  function listAccountIds(cfg: RecallConfig): string[] {
  ...
  function resolveDefaultAccountId(cfg: RecallConfig): string {
