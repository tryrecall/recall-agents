...
describe("channel action capability matrix", () => {
  ...
  function getCapabilities(plugin: Pick<ChannelPlugin, "actions">, cfg: SteelEngineConfig) {
  ...
  it("forwards Telegram action capabilities through the channel wrapper", () => {
    ...
    const result = getCapabilities(telegramPlugin, {} as SteelEngineConfig);
    ...
    const discordResult = getCapabilities(discordPlugin, {} as SteelEngineConfig);
