  ...
  return fs
    ...
    .filter(
      (entryPath) =>
        ...
        fs.existsSync(path.join(entryPath, "recall.plugin.json")),
    );
}
...
function listGitSourceBundledPluginRoots(extensionsDir: string): string[] | null {
  const result = spawnSync(
    "git",
    ["ls-files", "--", "extensions/*/package.json", "extensions/*/recall.plugin.json"],
    {
      cwd: process.cwd(),
      encoding: "utf8",
...
describe("bundled channel entry shape guards", () => {
  ...
  it("fills sparse bundled channel plugin metadata from package metadata", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "recall-bundled-metadata-"));
    ...
    try {
      process.env.RECALL_BUNDLED_PLUGINS_DIR = path.join(tempRoot, "dist", "extensions");
  ...
  it("uses the active bundled plugin root override for channel entry loading", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "recall-bundled-override-"));
    const previousBundledPluginsDir = process.env.RECALL_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(tempRoot, "dist", "extensions", "alpha");
    fs.mkdirSync(pluginDir, { recursive: true });
    fs.writeFileSync(
  ...
  it("treats direct bundled plugin-tree overrides as scan roots", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "recall-bundled-direct-override-"));
    const previousBundledPluginsDir = process.env.RECALL_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.RECALL_BUNDLED_PLUGINS_DIR = pluginsRoot;
  ...
  it("partitions bundled channel lazy caches by active bundled root without re-importing", async () => {
    const rootA = fs.mkdtempSync(path.join(os.tmpdir(), "recall-bundled-root-a-"));
    const rootB = fs.mkdtempSync(path.join(os.tmpdir(), "recall-bundled-root-b-"));
    const previousBundledPluginsDir = process.env.RECALL_BUNDLED_PLUGINS_DIR;
    const testGlobal = globalThis as typeof globalThis & {
    ...
    try {
      ...
      process.env.RECALL_BUNDLED_PLUGINS_DIR = path.join(rootA, "dist", "extensions");
      ...
      process.env.RECALL_BUNDLED_PLUGINS_DIR = path.join(rootB, "dist", "extensions");
  ...
  it("uses dist-runtime as the boundary root for packaged setup entries", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "recall-bundled-runtime-root-"));
  ...
  it("loads setup-entry feature plugins without loading the main channel entry", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "recall-bundled-setup-only-"));
    const previousBundledPluginsDir = process.env.RECALL_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.RECALL_BUNDLED_PLUGINS_DIR = path.join(root, "dist", "extensions");
  ...
  it("swallows and caches bundled plugin and setup load failures", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "recall-bundled-load-failure-"));
    const previousBundledPluginsDir = process.env.RECALL_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(root, "dist", "extensions", "alpha");
    const testGlobal = globalThis as typeof globalThis & {
  ...
  it("caches undefined bundled plugin loads as unavailable", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "recall-bundled-null-load-"));
    const previousBundledPluginsDir = process.env.RECALL_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(root, "dist", "extensions", "alpha");
    const testGlobal = globalThis as typeof globalThis & {
  ...
  it("keeps channel entrypoints on the dedicated entry-contract SDK surface", () => {
    const offenders = collectBundledChannelEntrypointOffenders(
      bundledPluginRoots,
      (source) =>
        !source.includes('from "recall/plugin-sdk/channel-entry-contract"') ||
        source.includes('from "recall/plugin-sdk/core"') ||
        source.includes('from "recall/plugin-sdk/channel-core"'),
    );
  ...
  it("keeps setup-entry legacy feature hints mirrored in package metadata", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      ...
      for (const feature of ["legacyStateMigrations", "legacySessionSurfaces"]) {
        ...
        const hasHint = packageJson.recall?.setupFeatures?.[feature] === true;
  ...
  it("keeps channel implementations off the broad core SDK surface", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      for (const relativePath of ["src/channel.ts", "src/plugin.ts"]) {
        ...
        if (source.includes('from "recall/plugin-sdk/core"')) {
          offenders.push(path.relative(process.cwd(), filePath));
        }
      }
    }
  ...
  it("keeps bundled hot runtime barrels off the broad core SDK surface", () => {
    const offenders = [
      ...
      fs.readFileSync(path.resolve(filePath), "utf8").includes("recall/plugin-sdk/core"),
    );
  ...
  it("keeps bundled doctor surfaces off the broad runtime barrel", () => {
    const offenders = [
      ...
      fs
        ...
        .includes('from "recall/plugin-sdk/runtime"'),
    );
  ...
  it("breaks reentrant bundled channel discovery cycles with an empty fallback", async () => {
    const pluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-bundled-reentrant-"));
