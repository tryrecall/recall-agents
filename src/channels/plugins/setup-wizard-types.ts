...
export type ChannelOnboardingPostWriteHook = {
  channel: ChannelId;
  accountId: string;
  run: (ctx: { cfg: RecallConfig; runtime: RuntimeEnv }) => Promise<void> | void;
};
...
export type ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: RecallConfig, accountId?: string) => DmPolicy;
  setPolicy: (cfg: RecallConfig, policy: DmPolicy, accountId?: string) => RecallConfig;
