...
import { discoverRecallPlugins } from "../../plugins/discovery.js";
import { loadPluginManifest } from "../../plugins/manifest.js";
import type { RecallPackageManifest } from "../../plugins/manifest.js";
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = [
    resolveRecallPackageRootSync({ cwd: process.cwd() }),
    resolveRecallPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<RecallPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: RecallPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function loadBundledMetadataCatalogEntries(options: CatalogOptions): ChannelPluginCatalogEntry[] {
  ...
  for (const dirent of fs.readdirSync(bundledDir, { withFileTypes: true })) {
    ...
    const entry = buildCatalogEntry({
      ...
      packageManifest: packageJson.recall,
    });
    if (entry) {
      entries.push(entry);
    }
