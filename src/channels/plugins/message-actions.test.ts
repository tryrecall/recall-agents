...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(listChannelMessageCapabilities({} as RecallConfig).toSorted()).toEqual([
      "buttons",
      "cards",
      "interactive",
    ]);
    expect(channelSupportsMessageCapability({} as RecallConfig, "interactive")).toBe(true);
    expect(channelSupportsMessageCapability({} as RecallConfig, "buttons")).toBe(true);
    expect(channelSupportsMessageCapability({} as RecallConfig, "cards")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as RecallConfig, channel: "discord" },
        "interactive",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as RecallConfig, channel: "telegram" },
        "interactive",
      ),
    ).toBe(false);
    expect(
    ...
    expect(channelSupportsMessageCapabilityForChannel({ cfg: {} as RecallConfig }, "cards")).toBe(
      false,
    );
  });
  ...
  it("uses unified message tool discovery for actions, capabilities, and schema", () => {
    ...
    expect(listChannelMessageActions({} as RecallConfig)).toEqual(["send", "broadcast", "react"]);
    expect(listChannelMessageCapabilities({} as RecallConfig)).toEqual(["interactive"]);
    expect(
      resolveChannelMessageToolSchemaProperties({
        cfg: {} as RecallConfig,
        channel: "discord",
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(listChannelMessageActions({} as RecallConfig)).toEqual(["send", "broadcast"]);
    expect(listChannelMessageCapabilities({} as RecallConfig)).toEqual([]);
