...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(channelSupportsMessageCapability({} as RecallConfig, "presentation")).toBe(true);
    expect(channelSupportsMessageCapability({} as RecallConfig, "delivery-pin")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as RecallConfig, channel: "demo-buttons" },
        "presentation",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as RecallConfig, channel: "demo-cards" },
        "presentation",
      ),
    ).toBe(false);
    expect(
    ...
    expect(
      channelSupportsMessageCapabilityForChannel({ cfg: {} as RecallConfig }, "delivery-pin"),
    ).toBe(false);
  });
  ...
  it("uses unified message tool discovery for actions, capabilities, and schema", () => {
    ...
    expect(listChannelMessageCapabilities({} as RecallConfig)).toEqual(["presentation"]);
    expect(
      resolveChannelMessageToolSchemaProperties({
        cfg: {} as RecallConfig,
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(listChannelMessageCapabilities({} as RecallConfig)).toStrictEqual([]);
