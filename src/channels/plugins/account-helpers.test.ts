...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): SteelEngineConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as SteelEngineConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: SteelEngineConfig) => string[];
  input: SteelEngineConfig;
  expected: string[];
}) {
  expect(params.resolve(params.input)).toEqual(params.expected);
}

function expectResolvedDefaultAccountCase(input: SteelEngineConfig, expected: string) {
