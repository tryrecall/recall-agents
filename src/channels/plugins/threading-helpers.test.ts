...
describe("createStaticReplyToModeResolver", () => {
  it("always returns the configured mode", () => {
    expect(createStaticReplyToModeResolver("off")({ cfg: {} as RecallConfig })).toBe("off");
    expect(createStaticReplyToModeResolver("all")({ cfg: {} as RecallConfig })).toBe("all");
  });
});
...
describe("createTopLevelChannelReplyToModeResolver", () => {
  it("reads the top-level channel config", () => {
    ...
    expect(
      resolver({
        cfg: { channels: { discord: { replyToMode: "first" } } } as RecallConfig,
      }),
    ).toBe("first");
  });
  ...
  it("falls back to off", () => {
    const resolver = createTopLevelChannelReplyToModeResolver("discord");
    expect(resolver({ cfg: {} as RecallConfig })).toBe("off");
  });
});
...
describe("createScopedAccountReplyToModeResolver", () => {
  ...
  it("passes chatType through", () => {
    ...
    expect(resolver({ cfg: {} as RecallConfig, chatType: "group" })).toBe("first");
