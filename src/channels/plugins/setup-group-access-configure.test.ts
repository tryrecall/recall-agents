...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: RecallConfig, policy: ChannelAccessPolicy) => RecallConfig;
  resolveAllowlist: (params: { cfg: RecallConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: RecallConfig; resolved: TResolved }) => RecallConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: RecallConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: RecallConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: RecallConfig, policy: ChannelAccessPolicy): RecallConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: RecallConfig, policy: ChannelAccessPolicy): RecallConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: RecallConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: RecallConfig; resolved: string[] }) => {
      calls.push("apply");
