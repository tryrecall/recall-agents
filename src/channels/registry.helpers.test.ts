...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://recall.ai");
  });

  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    const startupRegistry = createRegistryWithRegisteredChannel("recall-weixin", ["weixin"]);
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["recall-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("recall-weixin");
