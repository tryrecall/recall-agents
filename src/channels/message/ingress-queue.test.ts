...
type ChannelIngressTestDatabase = Pick<SteelEngineStateKyselyDatabase, "channel_ingress_events">;

async function withTempState<T>(fn: (stateDir: string) => Promise<T>): Promise<T> {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-ingress-queue-"));
  try {
  ...
  describe("corrupt JSON resilience", () => {
    function insertCorruptRow(
      ...
      executeSqliteQuerySync(
        ...
        } as Insertable<SteelEngineStateKyselyDatabase["channel_ingress_events"]>),
      );
    }
