...
async function writeExecApprovalsConfig(config: Record<string, unknown>) {
  const approvalsPath = path.join(process.env.HOME ?? "", ".recall", "exec-approvals.json");
...
describe("exec approvals", () => {
  ...
  beforeAll(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-exec-approvals-"));
  });

  beforeEach(async () => {
    previousHome = process.env.HOME;
    previousUserProfile = process.env.USERPROFILE;
    previousBundledPluginsDir = process.env.RECALL_BUNDLED_PLUGINS_DIR;
    previousDisableBundledPlugins = process.env.RECALL_DISABLE_BUNDLED_PLUGINS;
  ...
  afterEach(() => {
    ...
    if (previousDisableBundledPlugins === undefined) {
      ...
      process.env.RECALL_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
    }
  });
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-test-bin-"));
  ...
  it("runs a direct skill wrapper command without prompting when the wrapper is allowlisted", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-skill-wrapper-"));
    try {
      const binDir = path.join(tempDir, "bin");
  ...
  it("requires approval for the legacy skill display prelude even when the wrapper is allowlisted", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-skill-prelude-"));
    try {
      const skillDir = path.join(tempDir, ".recall", "skills", "gog");
