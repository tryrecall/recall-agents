...
const PERSISTED_DETAIL_REDACTION_BOUNDARY = "\u0000RECALL_PERSISTED_DETAIL_BOUNDARY\u0000";
...
function redactPersistedDetailString(
  ...
  const redactedPrefix =
    boundaryIndex >= 0
      ...
      : "[Recall persisted detail redacted: boundary marker removed]";
  ...
  const persistedPrefix =
    ...
    PARTIAL_PRIVATE_KEY_BLOCK_RE.test(initialPersistedPrefix)
      ? "[Recall persisted detail redacted: partial secret span omitted]"
      : initialPersistedPrefix;
  const boundaryNotice = "[Recall persisted detail redacted: boundary overlap omitted]";
  return `${persistedPrefix}${persistedPrefix ? "\n" : ""}${boundaryNotice}\n\n[Recall persisted detail truncated: ${Math.max(
    0,
...
function redactPersistedDetailValue(
  ...
  if (depth >= 8) {
    return "[Recall persisted detail redacted: max depth exceeded]";
  }
...
function isTranscriptOnlyRecallAssistantMessage(message: AgentMessage): boolean {
  ...
  return provider === "recall" && (model === "delivery-mirror" || model === "gateway-injected");
}
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage) => {
    ...
    const transcriptOnlyAssistant =
      nextRole === "assistant" &&
      toolCalls.length === 0 &&
      isTranscriptOnlyRecallAssistantMessage(nextMessage);
    if (
