...
function asRecallConfig(value: unknown): RecallConfig | undefined {
  return value !== null && typeof value === "object" && !Array.isArray(value)
    ? (value as RecallConfig)
    : undefined;
}
...
function resolveProviderRuntimeHandleForPlugins(params: {
  ...
  return resolveProviderRuntimePluginHandle({
    ...
    config: asRecallConfig(params.runtimeHandle?.config) ?? params.config,
...
export function buildAgentRuntimeDeliveryPlan(
  ...
  const config = asRecallConfig(params.config);
    ...
    prompt: {
      ...
      resolveSystemPromptContribution(context) {
        return resolveProviderSystemPromptContribution({
          ...
          context: {
            ...context,
            config: asRecallConfig(context.config),
          },
        });
      },
