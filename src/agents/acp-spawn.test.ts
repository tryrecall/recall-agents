...
function createDefaultSpawnConfig(): SteelEngineConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function replaceSpawnConfig(next: SteelEngineConfig): void {
...
async function createCrossAgentWorkspaceFixture(options?: {
  ...
  const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-acp-spawn-"));
...
describe("spawnAcpDirect", () => {
  ...
  it("rejects SteelEngine config agent ids when runtime=acp targets a native agent", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      acp: {
        enabled: true,
    ...
    expect(result).toHaveProperty(
      "error",
      'agentId "pleres" is an SteelEngine config agent, not an ACP harness. Use runtime="subagent" or omit runtime for SteelEngine config agents. Use runtime="acp" only with external ACP harness ids such as codex, claude, droid, gemini, or opencode, or configure agents.list[].runtime.type="acp" with runtime.acp.agent.',
    );
    expect(hoisted.initializeSessionMock).not.toHaveBeenCalled();
  ...
  it("maps SteelEngine ACP runtime agent aliases to their configured harness id", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      agents: {
        list: [
