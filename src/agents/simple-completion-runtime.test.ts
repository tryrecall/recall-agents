...
describe("prepareSimpleCompletionModel", () => {
  ...
  it("applies provider runtime auth before storing simple-completion credentials", async () => {
    ...
    expect(runtimeAuthInput.workspaceDir).toBe("/tmp/recall-agent");
...
describe("completeWithPreparedSimpleCompletionModel", () => {
  ...
  it("normalizes Recall-only thinking levels before using pi-ai simple completion", async () => {
    const model = {
      provider: "openai",
      id: "gpt-5.4",
      name: "gpt-5.4",
