...
describe("runExecProcess POSIX command wrapper", () => {
  ...
  it("wraps command with PATH export if STEELENGINE_PREPEND_PATH is present", async () => {
    ...
    expect(commandStr).toContain(
      'export PATH="${STEELENGINE_PREPEND_PATH}${PATH:+:$PATH}"; unset STEELENGINE_PREPEND_PATH; echo test',
    );
  });
