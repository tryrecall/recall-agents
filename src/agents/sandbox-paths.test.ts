...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-managed-media-"));
  try {
    return await withEnvAsync({ STEELENGINE_STATE_DIR: stateDir }, async () => {
...
async function withOutsideHardlinkInSteelEngineTmp<T>(
  params: {
    steelEngineTmpDir: string;
  ...
  const hardlinkPath = path.join(params.steelEngineTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.steelEngineTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.steelEngineTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.steelEngineTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxPath", () => {
  it("keeps home-sibling roots absolute in escape diagnostics", async () => {
    ...
    await withEnvAsync({ HOME: home, STEELENGINE_HOME: undefined }, async () => {
  ...
  it("still shortens roots beneath the home directory", async () => {
    const home = path.join(os.homedir(), "test-home");
    await withEnvAsync({ HOME: home, STEELENGINE_HOME: undefined }, async () => {
      const root = path.join(home, "steelengine-sandbox");
      const outside = path.dirname(root);

      expect(() => resolveSandboxPath({ filePath: outside, cwd: root, root })).toThrow(
        `Path escapes sandbox root (~${path.sep}steelengine-sandbox): ${outside}`,
      );
    });
  });
});
...
describe("resolveSandboxedMediaSource", () => {
  const steelEngineTmpDir = resolvePreferredSteelEngineTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred SteelEngine tmp root",
      media: path.join(steelEngineTmpDir, "image.png"),
      expected: path.join(steelEngineTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred SteelEngine tmp root",
      media: pathToFileURL(path.join(steelEngineTmpDir, "photo.png")).href,
      expected: path.join(steelEngineTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred SteelEngine tmp root",
      media: path.join(steelEngineTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(steelEngineTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(steelEngineTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside steelengine tmp root",
      media: path.join(os.tmpdir(), "outside-steelengine", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked SteelEngine tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(steelEngineTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(steelEngineTmpDir, { recursive: true });
      const symlinkPath = path.join(steelEngineTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked SteelEngine tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInSteelEngineTmp(
      {
        steelEngineTmpDir,
  ...
  it("rejects symlinked SteelEngine tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInSteelEngineTmp(
