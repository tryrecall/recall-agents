...
describe("normalizeProviders", () => {
  const createModel = (
    overrides: Partial<
      NonNullable<NonNullable<RecallConfig["models"]>["providers"]>[string]["models"][number]
    > = {},
  ) => ({
    id: "config-model",
    name: "Config model",
  ...
  it("trims provider keys so image models remain discoverable for custom providers", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-agent-"));
    try {
      const providers: NonNullable<NonNullable<RecallConfig["models"]>["providers"]> = {
        " dashscope-vision ": {
  ...
  it("normalizes retired Google Gemini model ids before emitting provider config", async () => {
    ...
    try {
      const providers: NonNullable<NonNullable<RecallConfig["models"]>["providers"]> = {
        ...
        "google-gemini-cli": {
          baseUrl: "recall://google-gemini-cli",
          models: [
            createModel({
              id: "gemini-3-pro-preview",
  ...
  it("replaces resolved env var value with env var name to prevent plaintext persistence", async () => {
    ...
    const env = {
      ...
      RECALL_SKIP_PROVIDERS: undefined,
      RECALL_TEST_MINIMAL_GATEWAY: undefined,
    };
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    } as unknown as NonNullable<NonNullable<RecallConfig["models"]>["providers"]>;

    const sourceProviders: NonNullable<NonNullable<RecallConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
