...
describe("normalizeProviders", () => {
  const createModel = (
    overrides: Partial<
      NonNullable<NonNullable<SteelEngineConfig["models"]>["providers"]>[string]["models"][number]
    > = {},
  ) => ({
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    const sourceProviders: NonNullable<NonNullable<SteelEngineConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
