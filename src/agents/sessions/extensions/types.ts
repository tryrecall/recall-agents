...
export interface ExtensionUIContext {
   ...
   * extend `CustomEditor` from `steelengine/plugin-sdk/agent-sessions` and call
   * `super.handleInput(data)` for keys you don't handle.
   *
   * @example
   * ```ts
   * import { CustomEditor } from "steelengine/plugin-sdk/agent-sessions";
   *
...
export interface ExtensionContext {
  ...
  /** Gracefully shut down SteelEngine and exit. Available in all contexts. */
  shutdown(): void;
...
interface ProviderModelConfig {
  ...
  reasoning: boolean;
  /** Maps SteelEngine thinking levels to provider/model-specific values; null marks a level unsupported. */
