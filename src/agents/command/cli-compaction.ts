...
import { STEELENGINE_AGENT_RUNTIME_ID } from "../agent-runtime-id.js";
...
type CliCompactionDeps = {
  ...
  resolveContextEngine: (cfg: SteelEngineConfig) => Promise<ContextEngine>;
...
function isNativeHarnessCompactionSession(
  ...
  if (!harnessId || normalizeOptionalAgentRuntimeId(harnessId) === STEELENGINE_AGENT_RUNTIME_ID) {
    return false;
  }
...
export async function runCliTurnCompactionLifecycle(params: {
  ...
  if (
    params.sessionEntry?.modelSelectionLocked === true &&
    lockedHarnessRuntime !== STEELENGINE_AGENT_RUNTIME_ID &&
