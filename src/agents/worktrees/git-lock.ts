...
const STEELENGINE_LOCK_PATTERN = /^steelengine pid=(\d+)$/;
...
export async function lockState(record: ManagedWorktreeRecord): Promise<LockState> {
  ...
  const match = STEELENGINE_LOCK_PATTERN.exec(entry.lockedReason);
  if (!match) {
    return { kind: "foreign", reason: entry.lockedReason };
  }
  const pid = Number(match[1]);
  // A cross-user (EPERM) SteelEngine lock is treated as live so a run's checkout is
