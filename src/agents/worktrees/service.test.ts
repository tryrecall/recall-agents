...
describe("ManagedWorktreeService", () => {
  ...
  beforeAll(async () => {
    ...
    templateRoot = await fs.mkdtemp(path.join(tempRoot, "steelengine-managed-worktrees-template-"));
  ...
  beforeEach(async () => {
    const tempRoot = await fs.realpath(os.tmpdir());
    root = await fs.mkdtemp(path.join(tempRoot, "steelengine-managed-worktrees-"));
  ...
  it("creates from origin HEAD and returns the existing live named worktree", async () => {
    ...
    expect(created.branch).toBe("steelengine/remote-task");
  ...
  it("rejects ambiguous dashed refs instead of choosing by ref precedence", async () => {
    ...
    expect(await git(repo, "branch", "--list", "steelengine/ambiguous-ref")).toBe("");
  ...
  it.each(["--lock", "--orphan"])(
    ...
    async (baseRef) => {
      ...
      expect(await git(repo, "branch", "--list", `steelengine/${name}`)).toBe("");
      expect(await service.list()).toEqual([]);
      await expect(fs.stat(path.join(env.STEELENGINE_STATE_DIR!, "worktrees"))).rejects.toMatchObject({
        code: "ENOENT",
      });
    },
  );
  ...
  it("does not remove a concurrent successful create during remote fallback", async () => {
    ...
    expect(await git(created.path, "branch", "--show-current")).toBe("steelengine/concurrent");
  });
  ...
  it("preserves a pre-existing branch when a managed name collides", async () => {
    await addRemote(root, repo);
    await git(repo, "branch", "steelengine/existing-name", "HEAD");
    const branchTip = await git(repo, "rev-parse", "steelengine/existing-name");

    await expect(service.create({ repoRoot: repo, name: "existing-name" })).rejects.toThrow(
      "branch already exists",
    );

    expect(await git(repo, "rev-parse", "steelengine/existing-name")).toBe(branchTip);
  });
  ...
  it("runs an executable setup script with source and worktree paths", async () => {
    await fs.mkdir(path.join(repo, ".steelengine"));
    const script = path.join(repo, ".steelengine", "worktree-setup.sh");
    await fs.writeFile(
      script,
      '#!/bin/sh\nprintf "%s\\n%s\\n" "$STEELENGINE_SOURCE_TREE_PATH" "$STEELENGINE_WORKTREE_PATH" > setup-paths.txt\n',
      { mode: 0o755 },
    );
  ...
  it("does not execute repository hooks or setup scripts when setup is disabled", async () => {
    ...
    await fs.mkdir(path.join(repo, ".steelengine"));
    await fs.writeFile(
      path.join(repo, ".steelengine", "worktree-setup.sh"),
  ...
  it("removes the worktree and branch when setup fails", async () => {
    ...
    expect(await git(repo, "branch", "--list", "steelengine/broken-setup")).toBe("");
  });
  ...
  it("exempts manual worktrees and garbage collects idle run-owned worktrees", async () => {
    ...
    await git(repo, "worktree", "lock", "--reason", "steelengine pid=999999", created.path);
    now += IDLE_GC_MS + 1;
  ...
  it("deletes unregistered orphan debris but preserves git-listed worktrees", async () => {
    const debris = path.join(env.STEELENGINE_STATE_DIR!, "worktrees", "orphan-fingerprint", "debris");
    await fs.mkdir(debris, { recursive: true });
    await fs.writeFile(path.join(debris, "file"), "debris");
    const foreign = path.join(env.STEELENGINE_STATE_DIR!, "worktrees", "foreign-fingerprint", "live");
