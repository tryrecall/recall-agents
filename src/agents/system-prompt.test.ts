...
describe("buildAgentSystemPrompt", () => {
  it("resolves helper session keys to scoped prompt surfaces", () => {
    ...
    expect(resolveAgentPromptSurfaceForSessionKey("agent:main")).toBe("steelengine_main");
    expect(resolveAgentPromptSurfaceForSessionKey(undefined)).toBe("steelengine_main");
  });
  ...
  it("includes an SteelEngine control section", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/steelengine",
    });

    expect(prompt).toContain("## SteelEngine Control");
    expect(prompt).toContain("Config read: `gateway`");
    expect(prompt).not.toContain("steelengine gateway status|restart|start|stop");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    ...
    expect(prompt).toContain(
      '[embed url="/__steelengine__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain("Never local/file:// or arbitrary URL");
    expect(prompt).toContain("URL must start `/__steelengine__/canvas/`; else use `ref`");
  ...
  it("keeps the SteelEngine empty-tool fallback on the main prompt surface", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/steelengine",
      toolNames: [],
    });

    expect(prompt).toContain("SteelEngine lists the standard tools above");
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/steelengine/docs");
    expect(prompt).toContain(
      "SteelEngine behavior questions: docs first via `Read`/local search. AGENTS/project/workspace/profile/memory = instructions/user memory, not product design truth.",
    );
  });
  ...
  it("includes docs guidance when docsPath is provided", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/steelengine/docs");
    expect(prompt).toContain("Source: /tmp/steelengine");
    expect(prompt).toContain(
      "SteelEngine behavior questions: docs first via `read`/local search. AGENTS/project/workspace/profile/memory = instructions/user memory, not product design truth.",
    );
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Docs: https://docs.steelengine.ai");
    expect(prompt).toContain("Source: https://github.com/steelengineai/recall-agents");
    expect(prompt).toContain(
      "SteelEngine behavior questions: docs mirror first when web exists. AGENTS/project/workspace/profile/memory = instructions/user memory, not product design truth.",
    );
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/steelengine",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/steelengine/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/steelengine/BOOTSTRAP.md");
    expect(prompt).toContain("Ask who I am.");
    expect(prompt.match(/## \/tmp\/steelengine\/BOOTSTRAP\.md/g)).toHaveLength(1);
  ...
  it("delegates system changes when steelengine tool is present", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/steelengine",
      toolNames: ["steelengine", "sessions_spawn"],
    });

    expect(prompt).toContain(
      "Config, channels, plugins, new agents, model/provider, updates: ask `steelengine`.",
    );
    expect(prompt).toContain("Never write own config; SteelEngine is system expert.");
    expect(prompt).toContain("`visible:true` only web/app user or asked.");
  });

  it("omits steelengine delegation guidance without the tool", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/steelengine",
      toolNames: ["gateway"],
    });

    expect(prompt).not.toContain("ask `steelengine`");
  });
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "File tools use host workspace /tmp/steelengine. exec uses container /workspace or relative workdir paths; never host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("exec steelengine/acpx");
    expect(prompt).toContain("`agents_list`/`subagents` = SteelEngine runtime=subagent only");
