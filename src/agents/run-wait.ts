...
import {
  isSteelEngineInternalSourceReplyMirrorAssistantMessage,
  isSteelEngineMessageToolMirrorAssistantMessage,
  isTranscriptOnlySteelEngineAssistantMessage,
...
function isWaitedReplyTranscriptArtifact(message: unknown): boolean {
  return (
    ...
    isSteelEngineMessageToolMirrorAssistantMessage(message) ||
...
function readTranscriptMessageSeq(message: unknown): number | undefined {
  ...
  const meta = (message as { __steelengine?: unknown })["__steelengine"];
...
function resolveLatestAssistantReplySnapshot(
  ...
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    ...
    if (
      ...
      isSteelEngineInternalSourceReplyMirrorAssistantMessage(candidate)
    ) {
