...
function createSessionFile(params?: { history?: Array<{ role: "user"; content: string }> }) {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-cli-hooks-"));
...
function buildPreparedContext(params?: {
  ...
  return {
    ...
    ...(params?.steelEngineHistoryPrompt
      ? { steelEngineHistoryPrompt: params.steelEngineHistoryPrompt }
      : {}),
    authEpochVersion: 2,
  };
}
...
describe("runCliAgent reliability", () => {
  ...
  it("preserves fresh retry for direct CLI callers without a pre-clear hook", async () => {
    ...
    const context = buildPreparedContext({
      ...
      provider: "claude-cli",
      model: "opus",
      steelEngineHistoryPrompt: CLI_RESEED_PROMPT,
    });
    ...
    const stateDir = autoCleanupTempDirs.make("steelengine-cli-retry-images-");
  ...
  it("preserves first-turn delivery through cleanup without binding the SteelEngine session id", async () => {
    supervisorSpawnMock.mockClear();
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      const input = args[0] as Parameters<ReturnType<typeof getProcessSupervisor>["spawn"]>[0];
      const captureHandle = markMcpLoopbackToolCallStarted({
  ...
  it("bounds admitted requests that have not finished uploading", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      const captureHandle = markMcpLoopbackRequestStarted(
        input.env?.STEELENGINE_MCP_CLI_CAPTURE_KEY ?? "",
      );
      if (!captureHandle) {
  ...
  it("keeps non-capture live-session artifacts through fresh recovery retry", async () => {
    ...
    const artifactDir = autoCleanupTempDirs.make("steelengine-live-retry-artifacts-");
  ...
  it("seeds fresh CLI sessions from the SteelEngine transcript", async () => {
    supervisorSpawnMock.mockResolvedValueOnce(
      createManagedRun({
        reason: "exit",
        exitCode: 0,
    ...
    const result = await runPreparedCliAgent(
      buildPreparedContext({
        steelEngineHistoryPrompt:
          "Continue this conversation using the SteelEngine transcript below.\n\nUser: earlier ask\n\nAssistant: earlier answer\n\n<next_user_message>\nhi\n</next_user_message>",
      }),
    );
  ...
  it("keeps resumed CLI sessions on native resume history", async () => {
    ...
    const result = await runPreparedCliAgent(
      buildPreparedContext({
        ...
        steelEngineHistoryPrompt: "User: earlier ask",
      }),
    );
  ...
  it("records transformed fresh Claude reseed prompts with durable local proof", async () => {
    ...
    try {
      ...
      const context = buildPreparedContext({
        ...
        steelEngineHistoryPrompt: historyPrompt,
      });
  ...
  it("passes cwd to approved CLI user-turn persistence", async () => {
    ...
    const { dir, sessionFile } = createSessionFile();
    const taskDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-cli-persist-cwd-"));
  ...
  it("does not execute the CLI when approved user turn persistence fails", async () => {
    supervisorSpawnMock.mockClear();
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-cli-persist-fail-"));
    const onUserMessagePersisted = vi.fn();
  ...
  it("blocks CLI runs before llm_input and model execution when before_agent_run blocks", async () => {
    ...
    try {
      ...
      expect(blockedLine.message["__steelengine"].beforeAgentRunBlocked.blockedBy).toBe(
        "policy-plugin",
      );
      expect(blockedLine.message["__steelengine"].beforeAgentRunBlocked).not.toHaveProperty("reason");
      expect(Object.hasOwn(blockedLine.message["__steelengine"], "beforeAgentRunBlocked")).toBe(true);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("builds fresh-session history reseed prompts from hook-mutated prompts", async () => {
    ...
    try {
      ...
      expect(context.steelEngineHistoryPrompt).toContain("Compaction summary: compacted earlier ask");
      expect(context.steelEngineHistoryPrompt).toContain("hook context");
      expect(context.steelEngineHistoryPrompt).toContain("current ask");
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
