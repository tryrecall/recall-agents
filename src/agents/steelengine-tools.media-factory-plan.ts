...
function hasExplicitImageModelConfig(config: SteelEngineConfig | undefined): boolean {
  return hasExplicitToolModelConfig(config?.agents?.defaults?.imageModel);
}

function hasExplicitPdfModelConfig(config: SteelEngineConfig | undefined): boolean {
  return (
