...
async function expectVertexAdcEnvApiKey(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), params.tempPrefix ?? "steelengine-adc-"));
...
vi.mock("./model-auth-env-vars.js", () => {
  ...
  const resolveMockProviderAuthEvidence = (params?: { config?: SteelEngineConfig }) => {
    const evidence = {
      "google-vertex": [
        {
  ...
  return {
    ...
    resolveProviderEnvAuthLookupMaps: (params?: { config?: SteelEngineConfig }) => ({
      aliasMap,
...
function buildDemoLocalProviderCfg(apiKey: string): SteelEngineConfig {
  return {
    models: {
      providers: {
        "demo-local": {
...
describe("getApiKeyForModel", () => {
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    await withSteelEngineTestState(
      ...
      async (state) => {
        ...
        const store = ensureAuthProfileStore(process.env.STEELENGINE_AGENT_DIR, {
  ...
  it("uses the config default agent dir when resolving provider profiles", async () => {
    await withSteelEngineTestState(
      {
        layout: "state-only",
        prefix: "steelengine-auth-agent-dir-",
        agentEnv: "clear",
        env: {
  ...
  it("reports the config default agent dir when provider auth is missing", async () => {
    await withSteelEngineTestState(
      {
        layout: "state-only",
        prefix: "steelengine-auth-missing-agent-dir-",
        agentEnv: "clear",
        env: {
          XAI_API_KEY: undefined,
        },
  ...
  it("reads Claude CLI credentials when the Claude CLI provider is resolved", async () => {
    ...
    await withSteelEngineTestState(
      {
        layout: "state-only",
        prefix: "steelengine-auth-claude-cli-",
        agentEnv: "main",
      },
      async () => {
  ...
  it("uses trusted workspace manifest auth evidence in runtime auth checks", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-workspace-cloud-auth-"));
  ...
  it("resolveEnvApiKey('google-vertex') accepts ADC credentials from the provided env snapshot", async () => {
    await expectVertexAdcEnvApiKey({
      ...
      credentialsJson: "{}",
      tempPrefix: "steelengine-google-adc-",
      env: {
  ...
  it("resolveEnvApiKey('google-vertex') accepts Unicode explicit ADC credential paths", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-google-adc-unicode-"));
  ...
  it("resolveEnvApiKey('google-vertex') accepts Unicode ADC fallback home paths", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-google-adc-home-"));
  ...
  it("resolveEnvApiKey('google-vertex') rejects GOOGLE_CLOUD_PROJECT_ID-only ADC auth evidence", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-google-adc-project-id-"));
    const credentialsPath = path.join(tempDir, "adc.json");
    await fs.writeFile(credentialsPath, "{}", "utf8");
  ...
  it("resolveEnvApiKey('google-vertex') accepts Windows APPDATA ADC fallback evidence", async () => {
    const appDataDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-google-adc-appdata-"));
  ...
  it("resolveEnvApiKey('google-vertex') does not synthesize APPDATA from USERPROFILE", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-google-adc-home-"));
    const userProfileDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "steelengine-google-adc-userprofile-"),
    );
  ...
  it("resolveEnvApiKey('google-vertex') keeps ADC fallback when manifest env candidates are empty", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-google-adc-candidates-"));
    const credentialsPath = path.join(tempDir, "adc.json");
    await fs.writeFile(credentialsPath, "{}", "utf8");
