...
function mockContextDeps(params: {
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveRecallAgentDir: () => "/tmp/recall-agent",
  }));
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real recall startup commands that need model metadata", async () => {
    ...
    try {
      for (const scenario of [
        ...
        {
          argv: ["node", "recall", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "recall", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureRecallModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureRecallModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
