...
describe("buildCliArgs", () => {
  ...
  it("passes Codex system prompts via a model instructions file config override", () => {
    expect(
      buildCliArgs({
        ...
        systemPromptFilePath: "/tmp/recall/system-prompt.md",
        useResume: false,
      }),
    ).toEqual(["exec", "--json", "-c", 'model_instructions_file="/tmp/recall/system-prompt.md"']);
  });
  ...
  it("passes Claude system prompts through its file flag", () => {
    ...
    ).toEqual(["-p", "--append-system-prompt-file", "/tmp/recall/system-prompt.md"]);
  });
...
describe("writeCliImages", () => {
  it("uses stable hashed file paths so repeated image hydration reuses the same path", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredRecallTmpDir(), "recall-cli-write-images-"),
    );
    ...
    try {
      expect(first.paths).toStrictEqual([
        expect.stringMatching(
          new RegExp(
            `^${escapeRegExp(`${resolvePreferredRecallTmpDir()}/recall-cli-images/`)}.*\\.png$`,
          ),
        ),
      ]);
  ...
  it("uses the shared media extension map for image formats beyond the tiny builtin list", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(resolvePreferredRecallTmpDir(), "recall-cli-write-heic-"),
    );
    const image: ImageContent = {
      type: "image",
      data: "aGVpYy1pbWFnZQ==",
  ...
  it("hydrates prompt media refs into codex image args through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredRecallTmpDir(), "recall-cli-prompt-image-"),
    );
    ...
    try {
      ...
      expect(argv).toStrictEqual([
        ...
        "--image",
        expect.stringContaining("recall-cli-images"),
      ]);
  ...
  it("appends hydrated prompt media refs for stdin backends through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredRecallTmpDir(), "recall-cli-prompt-image-generic-"),
    );
    ...
    try {
      ...
      expect(promptWithImages).toContain("recall-cli-images");
  ...
  it("appends Gemini prompt refs with @-prefixed image paths", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredRecallTmpDir(), "recall-cli-prompt-image-gemini-"),
    );
    ...
    try {
      ...
      expect(prepared.imagePaths?.[0]?.startsWith(path.join(tempDir, ".recall-cli-images"))).toBe(
        true,
      );
  ...
  it("prefers explicit images over prompt refs through the helper seams", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredRecallTmpDir(), "recall-cli-explicit-images-"),
    );
    ...
    try {
      ...
      expect(argv[argv.indexOf("--image") + 1]).toContain("recall-cli-images");
...
describe("writeCliSystemPromptFile", () => {
  it("writes stripped system prompts to a private temp file", async () => {
    ...
    try {
      expect(written.filePath).toContain("recall-cli-system-prompt-");
