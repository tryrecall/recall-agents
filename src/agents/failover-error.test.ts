...
// https://github.com/tryrecall/recall-agents/issues/33785
...
describe("failover-error", () => {
  ...
  it("403 OpenRouter 'Key limit exceeded' returns billing (model fallback trigger)", () => {
    // GitHub: tryrecall/recall-agents#53849 — OpenRouter returns 403 with "Key limit exceeded"
  ...
  describe("isNonProviderRuntimeCoordinationError", () => {
    ...
    const makeEmbeddedTakeoverError = () => {
      const err = new Error(
        "session file changed while embedded prompt lock was released: /tmp/recall/session.jsonl",
      );
