...
import type { AssistantMessage, Message, Tool } from "steelengine/plugin-sdk/llm";
...
describeCacheLive("embedded agent runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.STEELENGINE_CACHE_TRACE,
      file: process.env.STEELENGINE_CACHE_TRACE_FILE,
      messages: process.env.STEELENGINE_CACHE_TRACE_MESSAGES,
      prompt: process.env.STEELENGINE_CACHE_TRACE_PROMPT,
      system: process.env.STEELENGINE_CACHE_TRACE_SYSTEM,
    };
    setTestEnvValue("STEELENGINE_CACHE_TRACE", "1");
    setTestEnvValue("STEELENGINE_CACHE_TRACE_FILE", liveCacheTraceFile);
    setTestEnvValue("STEELENGINE_CACHE_TRACE_MESSAGES", "0");
    setTestEnvValue("STEELENGINE_CACHE_TRACE_PROMPT", "0");
    setTestEnvValue("STEELENGINE_CACHE_TRACE_SYSTEM", "0");
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("STEELENGINE_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("STEELENGINE_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("STEELENGINE_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("STEELENGINE_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("STEELENGINE_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
