...
function makeFallbacksOnlyCfg(): RecallConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): RecallConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "recall-auth-suite-mock");
...
// https://github.com/tryrecall/recall-agents/issues/23440
...
describe("runWithModelFallback", () => {
  ...
  it("aborts the fallback chain on session write-lock timeout instead of trying every model (#83510)", async () => {
    ...
    const lockError = new SessionWriteLockTimeoutError({
      timeoutMs: 10_000,
      owner: "pid=37121",
      lockPath: "/tmp/recall/session.jsonl.lock",
    });
  ...
  it("warns when falling back due to model_not_found", async () => {
    const warnLogs = createWarnLogCapture("recall-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
