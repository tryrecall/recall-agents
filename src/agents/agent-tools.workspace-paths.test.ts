...
describe("workspace path resolution", () => {
  it("uses cwd for coding filesystem tools while workspaceDir remains the agent workspace", async () => {
    await withTempDir("steelengine-agent-ws-", async (workspaceDir) => {
      await withTempDir("steelengine-task-cwd-", async (cwd) => {
        const tools = createSteelEngineCodingTools({ workspaceDir, cwd });
  ...
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("steelengine-ws-", async (workspaceDir) => {
      await withTempDir("steelengine-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createSteelEngineCodingTools({ workspaceDir });
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "steelengine" }],
          });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("steelengine-ws-", async (workspaceDir) => {
      await withTempDir("steelengine-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("steelengine-ws-", async (workspaceDir) => {
      const cfg: SteelEngineConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createSteelEngineCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-steelengine.txt");
      await expect(
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("steelengine-ws-symlink-write-", async (workspaceDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("steelengine-ws-symlink-edit-", async (workspaceDir) => {
        const realDir = path.join(workspaceDir, "oc_system", "memory");
        const aliasDir = path.join(workspaceDir, "memory");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("steelengine-ws-symlink-escape-", async (rootDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("steelengine-ws-symlink-leaf-", async (workspaceDir) => {
  ...
  it("allows workspaceOnly reads for resolved skill roots without allowing other filesystem access", async () => {
    await withTempDir("steelengine-skill-read-", async (rootDir) => {
      const workspaceDir = path.join(rootDir, "workspace");
  ...
  it("rejects symlink escapes inside resolved skill roots", async () => {
    ...
    await withTempDir("steelengine-skill-read-symlink-", async (rootDir) => {
      const workspaceDir = path.join(rootDir, "workspace");
      const skillDir = path.join(rootDir, "global-skills", "demo");
      await fs.mkdir(workspaceDir, { recursive: true });
      await fs.mkdir(skillDir, { recursive: true });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("steelengine-sandbox-", async (sandboxDir) => {
      await withTempDir("steelengine-workspace-", async (workspaceDir) => {
        ...
        const tools = createSteelEngineCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
