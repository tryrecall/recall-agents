...
import {
  ...
  ModelRegistry,
  SessionManager,
} from "steelengine/plugin-sdk/agent-sessions";
...
describe("buildEmbeddedExtensionFactories", () => {
  it("bridges middleware mutations with unique fallback tool call ids", async () => {
    ...
    expect(seenToolCallIds[0]).toMatch(/^steelengine-/);
    expect(seenToolCallIds[1]).toMatch(/^steelengine-/);
