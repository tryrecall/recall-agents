...
describe("subagent registry state read cache", () => {
  const previousReadSqliteFlag = process.env.STEELENGINE_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE;
  ...
  afterEach(() => {
    ...
    if (previousReadSqliteFlag === undefined) {
      ...
      process.env.STEELENGINE_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE = previousReadSqliteFlag;
    }
    vi.useRealTimers();
  });
