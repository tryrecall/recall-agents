...
function resolveAttachmentLimits(config: RecallConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".recall", "attachments");
  const relDir = path.posix.join(".recall", "attachments", attachmentId);
