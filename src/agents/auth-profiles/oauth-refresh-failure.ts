...
export function buildOAuthRefreshFailureLoginCommand(
  ...
  if (sanitizedProvider === "claude-cli") {
    ...
    // first, then re-register that auth method with SteelEngine.
    const claudeLoginCommand = formatCliCommand("claude auth login");
    const steelengineLoginCommand = formatCliCommand(
      sanitizedProfileId
        ? `steelengine models auth login --provider anthropic --method cli --profile-id ${quoteShellArg(sanitizedProfileId)}`
        : "steelengine models auth login --provider anthropic --method cli",
    );
    return `${claudeLoginCommand} && ${steelengineLoginCommand}`;
  }
  return sanitizedProvider
    ? formatCliCommand(
        sanitizedProfileId
          ? `steelengine models auth login --provider ${sanitizedProvider} --profile-id ${quoteShellArg(sanitizedProfileId)}`
          : `steelengine models auth login --provider ${sanitizedProvider}`,
      )
    : formatCliCommand("steelengine models auth login");
}
