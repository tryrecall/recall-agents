...
let prepareSteelEngineModelsJsonSourceMock: ReturnType<typeof vi.fn>;
...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ...
    prepareSteelEngineModelsJsonSourceMock = vi.fn().mockResolvedValue({
      agentDir: "/tmp/steelengine",
    ...
    vi.doMock("./models-config.js", () => ({
      ...
      prepareSteelEngineModelsJsonSource: prepareSteelEngineModelsJsonSourceMock,
    }));
    ...
    vi.doMock("./agent-scope.js", () => ({
      resolveAgentWorkspaceDir: (cfg: SteelEngineConfig, agentId: string) => {
        const entry = cfg.agents?.list?.find((entryEntry) => entryEntry.id === agentId);
        return entry?.workspace ?? cfg.agents?.defaults?.workspace ?? "/tmp/steelengine-workspace";
      },
      resolveDefaultAgentDir: () => "/tmp/steelengine",
  ...
  beforeEach(() => {
    ...
    prepareSteelEngineModelsJsonSourceMock.mockReset();
    prepareSteelEngineModelsJsonSourceMock.mockResolvedValue({
      agentDir: "/tmp/steelengine",
      fingerprint: "source-fingerprint",
      workspaceDir: "/tmp/steelengine-workspace",
      wrote: false,
  ...
  it("uses the state cached catalog before runtime discovery", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as SteelEngineConfig });
    ...
    expect(prepareSteelEngineModelsJsonSourceMock).not.toHaveBeenCalled();
  ...
  it("bypasses the state cached catalog when a refresh is requested", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as SteelEngineConfig, useCache: false });
  ...
  it("does not publish a catalog load from an invalidated generation", async () => {
    ...
    const staleLoad = loadModelCatalog({ config: {} as SteelEngineConfig });
    resetModelCatalogCache();
    await expect(loadModelCatalog({ config: {} as SteelEngineConfig })).resolves.toBe(freshCatalog);
  ...
  it("misses the state cached catalog when source freshness changes", async () => {
    ...
    await expect(loadModelCatalog({ config: {} as SteelEngineConfig })).resolves.toEqual([
      { id: "cached-stale", name: "Cached Stale", provider: "openai" },
    ]);
    resetModelCatalogCacheForTest();
    mockAgentDiscoveryModels([{ id: "fresh-fast", name: "Fresh Fast", provider: "openai" }]);
  ...
  it("reloads dynamic registry entries after clearing the cache", async () => {
    ...
    const first = await loadModelCatalog({ config: {} as SteelEngineConfig });
    expect(first).toStrictEqual([
      {
        id: "existing",
        name: "Existing",
    ...
    const second = await loadModelCatalog({ config: {} as SteelEngineConfig });
  ...
  it("filters suppressed built-ins from persisted read-only catalog rows", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as SteelEngineConfig, readOnly: true });
  ...
  it("loads generated plugin catalog rows in read-only mode", async () => {
    const catalogPath = "/tmp/steelengine/plugins/read-only-shard/catalog.json";
    mkdirSync("/tmp/steelengine/plugins/read-only-shard", { recursive: true });
    ...
    try {
      ...
      rmSync("/tmp/steelengine/plugins/read-only-shard", { recursive: true, force: true });
    }
  });

  it("preserves sidecar and manifest physical routes in the read-only catalog", async () => {
    const catalogPath = "/tmp/steelengine/plugins/openai/catalog.json";
    mkdirSync("/tmp/steelengine/plugins/openai", { recursive: true });
    writeFileSync(catalogPath, "{}");
    ...
    try {
      ...
      rmSync("/tmp/steelengine/plugins/openai", { recursive: true, force: true });
    }
  });
  ...
  it("loads manifest catalog rows from the current metadata snapshot without provider runtime", () => {
    ...
    const result = loadManifestModelCatalog({ config: {} as SteelEngineConfig });
  ...
  it("passes explicit env when checking current manifest catalog snapshot compatibility", () => {
    const env = { HOME: "/tmp/steelengine-model-catalog-env" } as NodeJS.ProcessEnv;
