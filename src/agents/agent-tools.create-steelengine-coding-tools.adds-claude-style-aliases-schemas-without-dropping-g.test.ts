...
import type { AgentTool, AgentToolResult } from "steelengine/plugin-sdk/agent-core";
...
describe("createSteelEngineCodingTools read behavior", () => {
  ...
  it("uses host decoding only for host-backed sandbox paths", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-sbx-encoding-"));
  ...
  it("applies sandbox path guards to canonical path", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "steelengine-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-read-autopage-"));
  ...
  it("maps inbound media refs to sandbox-staged media for reads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-read-media-sbx-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-read-cap-"));
  ...
  it("returns empty content for explicit offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-read-offset-eof-"));
  ...
  it("returns empty content for adaptive offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-read-offset-adaptive-"));
    await fs.writeFile(path.join(tmpDir, "notes.txt"), "one\ntwo\nthree\n", "utf8");
    try {
      const readTool = createSandboxedReadTool({
        root: tmpDir,
  ...
  it("returns already-read adaptive content when pagination reaches EOF", async () => {
    ...
    const readTool = createSteelEngineReadTool({
      name: "read",
      label: "read",
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createSteelEngineReadTool(
      baseRead as unknown as Parameters<typeof createSteelEngineReadTool>[0],
    );
