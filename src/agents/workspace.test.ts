...
beforeEach(async () => {
  ...
  testState = await createSteelEngineTestState({
    layout: "state-only",
    prefix: "steelengine-workspace-state-",
  });
});
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses STEELENGINE_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      STEELENGINE_HOME: "/srv/steelengine-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/steelengine-home"), ".steelengine", "workspace"));
  });

  it("prefers STEELENGINE_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      STEELENGINE_WORKSPACE_DIR: "/srv/steelengine-workspace",
      STEELENGINE_HOME: "/srv/steelengine-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/steelengine-workspace"));
  });
});
...
describe("ensureAgentWorkspace", () => {
  ...
  it("does not overwrite a foreign root workspace-state.json file", async () => {
    ...
    const foreignState = "not steelengine state\n";
  ...
  it("requires Doctor before using legacy JSON setup state", async () => {
    ...
    ).rejects.toThrow(/run steelengine doctor --fix/u);
    await expect(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-workspace-hardlink-"));
    try {
