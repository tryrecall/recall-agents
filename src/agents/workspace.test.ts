...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses RECALL_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      RECALL_HOME: "/srv/recall-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/recall-home"), ".recall", "workspace"));
  });

  it("prefers RECALL_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      RECALL_WORKSPACE_DIR: "/srv/recall-workspace",
      RECALL_HOME: "/srv/recall-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/recall-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".recall", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("recall-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("recall-workspace-");
    await fs.mkdir(path.join(tempDir, ".recall"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-workspace-hardlink-"));
    try {
