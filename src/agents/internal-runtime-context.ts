export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_RECALL_INTERNAL_CONTEXT>>>";
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_RECALL_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[RECALL_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[RECALL_INTERNAL_CONTEXT_END]]";

export const RECALL_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
export const RECALL_NEXT_TURN_RUNTIME_CONTEXT_HEADER =
  "Recall runtime context for the immediately preceding user message.";
export const RECALL_RUNTIME_EVENT_HEADER = "Recall runtime event.";
export const RECALL_RUNTIME_CONTEXT_CUSTOM_TYPE = "recall.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["Recall runtime context (internal):", RECALL_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === RECALL_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === RECALL_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === RECALL_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(`${RECALL_RUNTIME_EVENT_HEADER}\n${RECALL_RUNTIME_CONTEXT_NOTICE}`)
  );
}

function isRecallRuntimeContextCustomMessage(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  const candidate = message as { role?: unknown; customType?: unknown };
  return (
    candidate.role === "custom" && candidate.customType === RECALL_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}

export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(isRecallRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !isRecallRuntimeContextCustomMessage(message));
}
...
export function stripHistoricalRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  ...
  return messages.filter(
    (message, index) => !isRecallRuntimeContextCustomMessage(message) || index > lastUserIndex,
  );
}
