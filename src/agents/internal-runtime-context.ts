 ...
 */
/** Opening delimiter for protected SteelEngine runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_STEELENGINE_INTERNAL_CONTEXT>>>";
/** Closing delimiter for protected SteelEngine runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_STEELENGINE_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[STEELENGINE_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[STEELENGINE_INTERNAL_CONTEXT_END]]";

/** Notice inserted into runtime-generated context blocks. */
export const STEELENGINE_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
/** Header for context attached to the immediately preceding user message. */
export const STEELENGINE_NEXT_TURN_RUNTIME_CONTEXT_HEADER =
  "SteelEngine runtime context for the immediately preceding user message.";
/** Header for runtime events passed as prompt context. */
export const STEELENGINE_RUNTIME_EVENT_HEADER = "SteelEngine runtime event.";
/** Custom message type used for structured runtime-context messages. */
export const STEELENGINE_RUNTIME_CONTEXT_CUSTOM_TYPE = "steelengine.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["SteelEngine runtime context (internal):", STEELENGINE_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === STEELENGINE_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === STEELENGINE_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === STEELENGINE_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(
      `${STEELENGINE_NEXT_TURN_RUNTIME_CONTEXT_HEADER}\n${STEELENGINE_RUNTIME_CONTEXT_NOTICE}`,
    ) ||
    text.includes(`${STEELENGINE_RUNTIME_EVENT_HEADER}\n${STEELENGINE_RUNTIME_CONTEXT_NOTICE}`)
  );
}

function isSteelEngineRuntimeContextCustomMessage(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  const candidate = message as { role?: unknown; customType?: unknown };
  return (
    candidate.role === "custom" && candidate.customType === STEELENGINE_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}

/** Remove all structured runtime-context custom messages. */
export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(isSteelEngineRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !isSteelEngineRuntimeContextCustomMessage(message));
}
...
export function stripHistoricalRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  ...
  for (let index = lastUserIndex - 1; index >= 0; index -= 1) {
    if (!isSteelEngineRuntimeContextCustomMessage(messages[index])) {
      break;
    }
    currentRuntimeContextIndexes.add(index);
  }
  return messages.filter((message, index) => {
    if (!isSteelEngineRuntimeContextCustomMessage(message)) {
      return true;
    }
...
export function relocateCurrentRuntimeContextCarrierToTail<T>(messages: T[]): T[] {
  ...
  if (lastIndex < 0 || !messages.some(isSteelEngineRuntimeContextCustomMessage)) {
    return messages;
  }
  ...
  while (
    ...
    isSteelEngineRuntimeContextCustomMessage(messages[firstNonCarrierFromEnd])
  ) {
    firstNonCarrierFromEnd -= 1;
  }
  const rest = messages.filter((message) => !isSteelEngineRuntimeContextCustomMessage(message));
  ...
  const carriers = messages.filter(isSteelEngineRuntimeContextCustomMessage);
