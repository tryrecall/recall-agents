...
} from "./steelengine-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./steelengine-tools.nodes-workspace-guard.js";
import {
  collectPresentSteelEngineTools,
...
import { createSteelEngineDelegateToolsForRun } from "./tools/steelengine-delegate-tool.js";
...
export function createSteelEngineTools(
  options?: {
  ...
  options?.recordToolPrepStage?.("steelengine-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("steelengine-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("steelengine-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("steelengine-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("steelengine-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("steelengine-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("steelengine-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("steelengine-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("steelengine-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("steelengine-tools:nodes-tool");
  ...
  const includeUpdatePlanTool = shouldIncludeUpdatePlanToolForSteelEngineTools({
    config: resolvedConfig,
    agentSessionKey: options?.agentSessionKey,
    agentId: options?.requesterAgentIdOverride,
  ...
  const includeAskUserTool = shouldIncludeAskUserToolForSteelEngineTools({
    config: resolvedConfig,
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentSteelEngineTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentSteelEngineTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool(),
          ...createSteelEngineDelegateToolsForRun({ ...options, sessionAgentId }),
        ]),
    ...
    ...collectPresentSteelEngineTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("steelengine-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    options?.recordToolPrepStage?.("steelengine-tools:plugin-tools");
  }

  allTools = filterToolsByClientCaps(allTools, options?.clientCaps);
  options?.recordToolPrepStage?.("steelengine-tools:client-capabilities");
  ...
  options?.recordToolPrepStage?.("steelengine-tools:tool-hooks");
  return allTools
    .map((tool) =>
