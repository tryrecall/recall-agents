...
import { markRecallExecEnv } from "../../infra/recall-exec-env.js";
...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    throw new Error(
      `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; Recall will not substitute plain debian:bookworm-slim.`,
    );
  }
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "recall.sandbox=1");
  args.push("--label", `recall.sessionKey=${params.scopeKey}`);
  args.push("--label", `recall.createdAtMs=${createdAtMs}`);
  args.push("--label", `recall.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `recall.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markRecallExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "recall.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`recall sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`recall sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("recall sandbox recreate --all");
}
