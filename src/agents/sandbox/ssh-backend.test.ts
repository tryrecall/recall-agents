...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "steelengine-test-ssh-config"),
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "steelengine-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "steelengine-ssh-worker-abcd1234",
    ...
    expect(sessionSettings.workspaceRoot).toBe("/remote/steelengine");
    const commandParams = requireSshRunCommandParams();
    expect(commandParams.remoteCommand).toContain("/remote/steelengine/steelengine-ssh-agent-worker");
  });
  ...
  it("rejects unmaterialized shared SSH refs even when no active owner inherited them", async () => {
    ...
    await expect(
      sshSandboxBackendManager.removeRuntime({
        entry: {
          containerName: "steelengine-ssh-shared-abcd1234",
          backendId: "ssh",
          runtimeLabel: "steelengine-ssh-shared-abcd1234",
          sessionKey: "shared",
          createdAtMs: 1,
          lastUsedAtMs: 1,
          image: "peter@example.com:2222",
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const skillsWorkspaceDir = await createTempDir("steelengine-ssh-skills-");
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        browser: {
          enabled: false,
          image: "steelengine-browser",
          containerPrefix: "steelengine-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/steelengine/steelengine-ssh-agent-worker");
    ...
    expect(skillsUploadParams.remoteDir).toContain("/workspace/.steelengine/sandbox-skills");
  ...
  it("validates remote workdirs before exec accepts backend-owned cwd", async () => {
    sshMocks.runSshSandboxCommand
      ...
      .mockResolvedValueOnce({
        stdout: Buffer.from("/remote/steelengine/steelengine-ssh-agent-worker-abcd1234/workspace/src\n"),
        stderr: Buffer.alloc(0),
        code: 0,
      })
      .mockResolvedValueOnce({
      ...
      .mockResolvedValueOnce({
        stdout: Buffer.from("/remote/steelengine/steelengine-ssh-agent-worker-abcd1234/agent/src\n"),
        stderr: Buffer.alloc(0),
        code: 0,
      });
    ...
    await expect(
      backend.validateWorkdir?.(
        "/remote/steelengine/steelengine-ssh-agent-worker-abcd1234/workspace/src",
      ),
    ).resolves.toBe("/remote/steelengine/steelengine-ssh-agent-worker-abcd1234/workspace/src");
    await expect(
      backend.validateWorkdir?.(
        "/remote/steelengine/steelengine-ssh-agent-worker-abcd1234/workspace/missing",
      ),
    ).resolves.toBeNull();
    await expect(
      backend.validateWorkdir?.("/remote/steelengine/steelengine-ssh-agent-worker-abcd1234/agent/src"),
    ).resolves.toBe("/remote/steelengine/steelengine-ssh-agent-worker-abcd1234/agent/src");

    const validationCommand = String(requireSshRunCommandParams(1).remoteCommand);
    expect(validationCommand).toContain("steelengine-validate-workdir");
    ...
    expect(agentValidationCommand).toContain(
      "/remote/steelengine/steelengine-ssh-agent-worker-abcd1234/agent",
    );
  });
  ...
  it("refreshes materialized skills before validating a skills workdir", async () => {
    ...
    const runtimePaths = resolveSshRuntimePaths("/remote/steelengine", "agent:worker");
  ...
  it("clears stale remote materialized skills when the local copy is missing", async () => {
    const tmpDir = await createTempDir("steelengine-ssh-skills-");
    ...
    expect(commandParams.remoteCommand).toContain("steelengine-sandbox-clear");
    expect(commandParams.remoteCommand).toContain("/workspace/.steelengine/sandbox-skills");
    await backend.finalizeExec?.({
      status: "completed",
      exitCode: 0,
      timedOut: false,
