...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "recall-test-ssh-config"),
    host: "recall-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "recall-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "recall-ssh-worker-abcd1234",
    ...
    expect(sessionSettings.workspaceRoot).toBe("/remote/recall");
    const commandParams = requireSshRunCommandParams();
    expect(commandParams.remoteCommand).toContain("/remote/tryrecall/recall-agents-ssh-agent-worker");
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/tryrecall/recall-agents-ssh-agent-worker");
