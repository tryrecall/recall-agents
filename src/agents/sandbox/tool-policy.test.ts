...
describe("sandbox/tool-policy", () => {
  ...
  it("keeps blocked-tool guidance glob-aware and shell-safe", () => {
    ...
    expect(message).toContain(
      "steelengine sandbox explain --session 'agent:main:weird session;rm -rf /'",
    );
  });
  ...
  it.each([
    ...
    ({ sessionKey, expectedLabel }) => {
      ...
      expect(message).toContain(`steelengine sandbox explain --session '${sessionKey}'`);
    },
  );
  ...
  it("avoids terminal injection for control-character session keys", () => {
    ...
    expect(message).toContain("steelengine sandbox explain --agent main");
