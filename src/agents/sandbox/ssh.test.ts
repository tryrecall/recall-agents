...
describe("sandbox ssh helpers", () => {
  it("materializes inline ssh auth data into a temp config", async () => {
    ...
    expect(config).toContain("Host steelengine-sandbox");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-ssh-real-"));
      ...
      const target = path.join(root, "workspace", ".steelengine", "sandbox-skills");
      await execFileAsync("/bin/sh", [
        "-c",
        ENSURE_REMOTE_REAL_DIRECTORY_SCRIPT,
        "steelengine-remote-dir",
        target,
        root,
      ]);

      await expect(
        fs.stat(path.join(realParent, "runtime", "workspace", ".steelengine", "sandbox-skills")),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = makeTempDir(tempDirs, "steelengine-ssh-real-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = makeTempDir(tempDirs, "steelengine-ssh-workdir-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = makeTempDir(tempDirs, "steelengine-ssh-root-");
  ...
  it("builds remote workdir validation commands with quoted literal paths", () => {
    ...
    expect(command).toContain("steelengine-validate-workdir");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.symlink(realParent, path.join(root, "workspace", ".steelengine"));
      ...
      await expect(
        execFileAsync("/bin/sh", [
          ...
          ENSURE_REMOTE_REAL_DIRECTORY_SCRIPT,
          "steelengine-remote-dir",
          path.join(root, "workspace", ".steelengine", "sandbox-skills"),
          root,
        ]),
      ).rejects.toThrow(/unsafe remote directory symlink/);
    },
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-ssh-upload-"));
