...
function formatSandboxRecreateHint(params: { scope: SandboxScope; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`steelengine sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`steelengine sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("steelengine sandbox recreate --all");
}
