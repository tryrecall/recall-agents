...
describe("ssh subprocess env sanitization", () => {
  ...
  it("filters blocked secrets before spawning ssh commands", async () => {
    ...
    await runSshSandboxCommand({
      session: {
        command: "ssh",
        configPath: "/tmp/recall-test-ssh-config",
        host: "recall-sandbox",
      },
      remoteCommand: "true",
    });
  ...
  it("filters blocked secrets before spawning ssh uploads", async () => {
    ...
    const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-ssh-upload-env-"));
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-ssh-upload-safe-"));
      tempDirs.push(localDir);
