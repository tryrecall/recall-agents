...
const {
  TEST_STATE_DIR,
  PREVIOUS_STEELENGINE_STATE_DIR,
  ...
  const baseDir = mkdtempSync(nodePath.join(tmpdir(), "steelengine-sandbox-registry-"));
  const previousStateDir = process.env.STEELENGINE_STATE_DIR;
  Reflect.set(process.env, "STEELENGINE_STATE_DIR", baseDir);

  return {
    TEST_STATE_DIR: baseDir,
    PREVIOUS_STEELENGINE_STATE_DIR: previousStateDir,
...
afterAll(async () => {
  ...
  if (PREVIOUS_STEELENGINE_STATE_DIR === undefined) {
    deleteTestEnvValue("STEELENGINE_STATE_DIR");
  } else {
    setTestEnvValue("STEELENGINE_STATE_DIR", PREVIOUS_STEELENGINE_STATE_DIR);
  }
});
