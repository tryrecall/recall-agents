...
describe("docker sandbox backend manager", () => {
  ...
  it("matches ordinary sandbox runtimes against sandbox.docker.image", async () => {
    dockerMocks.execDocker.mockResolvedValueOnce({
      code: 0,
      stdout: "recall-sandbox:bookworm-slim\n",
      stderr: "",
    });
    ...
    expect(result).toEqual({
      running: true,
      actualConfigLabel: "recall-sandbox:bookworm-slim",
  ...
  it("matches browser runtimes against sandbox.browser.image", async () => {
    dockerMocks.execDocker.mockResolvedValueOnce({
      code: 0,
      stdout: "recall-sandbox-browser:bookworm-slim\n",
      stderr: "",
    });
    ...
    expect(result).toEqual({
      running: true,
      actualConfigLabel: "recall-sandbox-browser:bookworm-slim",
      configLabelMatch: true,
    });
  });
