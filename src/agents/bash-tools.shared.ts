 ...
 */
import { parseStrictInteger } from "@steelengine/normalization-core/number-coercion";
...
export function buildDockerExecArgs(params: {
  ...
  for (const [key, value] of Object.entries(params.env)) {
    ...
    // PATH is handled separately via STEELENGINE_PREPEND_PATH below.
    if (key === "PATH") {
      continue;
    }
  ...
  if (hasCustomPath) {
    ...
    args.push("-e", `STEELENGINE_PREPEND_PATH=${params.env.PATH}`);
  }
  ...
  const pathExport = hasCustomPath
    ? 'export PATH="${STEELENGINE_PREPEND_PATH}:$PATH"; unset STEELENGINE_PREPEND_PATH; '
    : "";
