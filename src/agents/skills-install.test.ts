...
import { resolveRecallMetadata, resolveSkillInvocationPolicy } from "./skills/frontmatter.js";
...
metadata: {"recall":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
const workspaceSuite = createFixtureSuite("recall-skills-install-");
...
describe("installSkill code safety scanning", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("RECALL_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an Recall-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["RECALL_STATE_DIR", "RECALL_CONFIG_PATH"]);
    try {
      process.env.RECALL_STATE_DIR = "/tmp/untrusted-state";
      process.env.RECALL_CONFIG_PATH = "/tmp/untrusted-config/recall.json";
  ...
  it("surfaces plugin scanner findings from before_install", async () => {
    ...
    await withWorkspaceCase(async ({ workspaceDir }) => {
      ...
      expect(payload?.origin).toBe("recall-workspace");
