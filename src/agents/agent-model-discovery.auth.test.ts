...
async function createAgentDir(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-agent-auth-storage-"));
}
...
describe("discoverAuthStorage", () => {
  ...
  it("includes env-backed provider auth when no auth profile exists", () => {
    ...
    const previousDisableBundledPlugins = process.env.STEELENGINE_DISABLE_BUNDLED_PLUGINS;
    ...
    try {
      ...
      if (previousDisableBundledPlugins === undefined) {
        ...
        process.env.STEELENGINE_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
      }
    }
  });
