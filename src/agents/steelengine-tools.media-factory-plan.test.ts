...
import { resolveOptionalMediaToolFactoryPlan } from "./steelengine-tools.media-factory-plan.js";
...
type CreateSteelEngineToolsOptions = Parameters<
  typeof import("./steelengine-tools.js").createSteelEngineTools
>[0];
let createSteelEngineToolsForTestModule: typeof import("./steelengine-tools.js").createSteelEngineTools;
let legacyComfyToolNames: string[];

async function createSteelEngineToolsForTest(options?: CreateSteelEngineToolsOptions) {
  return createSteelEngineToolsForTestModule(options);
}
...
function legacyModelProviderConfig(provider: Record<string, unknown>): SteelEngineConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  beforeAll(async () => {
    ({ createSteelEngineTools: createSteelEngineToolsForTestModule } =
      await import("./steelengine-tools.js"));
    ...
    legacyComfyToolNames = (
      await createSteelEngineToolsForTest({
        config,
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: SteelEngineConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createSteelEngineToolsForTest({
      config,
      agentDir: "/tmp/steelengine-agent-main",
  ...
  it("does not register the image tool without cheap vision availability evidence", async () => {
    const config: SteelEngineConfig = {};
    const workspaceDir = "/tmp/steelengine-workspace";
