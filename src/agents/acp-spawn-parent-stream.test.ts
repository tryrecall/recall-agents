...
function progressModeConfig(acp?: SteelEngineConfig["acp"]): SteelEngineConfig {
  return {
    ...(acp ? { acp } : {}),
    channels: {
      forum: {
...
describe("startAcpSpawnParentStreamRelay", () => {
  ...
  it("resolves ACP spawn stream log path from session metadata", () => {
    readAcpSessionEntryMock.mockReturnValue({
      storePath: "/tmp/steelengine/agents/codex/sessions/sessions.json",
      entry: {
        sessionId: "sess-123",
        sessionFile: "/tmp/steelengine/agents/codex/sessions/sess-123.jsonl",
      },
    });
    resolveSessionFilePathMock.mockReturnValue(
      "/tmp/steelengine/agents/codex/sessions/sess-123.jsonl",
    );
    ...
    expect(resolved).toBe("/tmp/steelengine/agents/codex/sessions/sess-123.acp-stream.jsonl");
    ...
    expect(options.storePath).toBe("/tmp/steelengine/agents/codex/sessions/sessions.json");
  });
});
