...
import "./test-helpers/fast-steelengine-tools-sessions.js";
...
function createSteelEngineTools(options?: {
...
describe("sessions tools", () => {
  ...
  it("sessions_list forwards mailbox filters and includes messages", async () => {
    ...
    const tool = createSteelEngineTools().find((candidate) => candidate.name === "sessions_list");
    if (!tool) {
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-sessions-list-preview-"));
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createSteelEngineTools().find((candidate) => candidate.name === "sessions_history");
    if (!tool) {
    ...
    expect(details.messages).toContainEqual(
      expect.objectContaining({ provider: "steelengine", model: "gateway-injected" }),
    );
    expect(details.messages).toContainEqual(
  ...
  it("sessions_history caps oversized payloads and strips heavy fields", async () => {
    ...
    expect(thinkingBlock?.steelengineReasoningReplay).toBeUndefined();
  });
