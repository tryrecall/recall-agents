...
import { resolveSteelEngineMcpTransportAlias } from "../config/mcp-config-normalize.js";
...
import type {
  ...
  SteelEnginePluginMcpServerConnectionResolver,
} from "../plugins/types.js";
import { isMcpConfigRecord } from "./mcp-config-shared.js";

export type { McpServerConnectionResolved };

type McpServerConnectionResolverEntry = SteelEnginePluginMcpServerConnectionResolver & {
  pluginId: string;
};
...
const MCP_CONNECTION_RESOLVER_TEST_STATE_KEY = Symbol.for(
  "steelengine.mcpServerConnectionResolverTestState",
);
...
export function applyMcpConnectionOverride(
  ...
  const fromTransport =
    typeof base.transport === "string"
      ? resolveSteelEngineMcpTransportAlias(base.transport)
      : undefined;
  const fromType = resolveSteelEngineMcpTransportAlias(base.type);
...
export const testing = {
  setMcpServerConnectionResolversForTest(
    resolvers?: Iterable<SteelEnginePluginMcpServerConnectionResolver & { pluginId?: string }> | null,
  ): void {
    if (!resolvers) {
