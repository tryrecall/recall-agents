...
function createClaudeCliOverrideConfig(config: CliBackendConfig): RecallConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from Recall's default YOLO exec policy", () => {
  ...
  it("keeps Claude permission mode unset when Recall exec policy is not YOLO", () => {
    const resolved = requireCliBackendConfig("claude-cli", {
