...
function resolveAnthropicOpusThinking(cfg: RecallConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: RecallConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveConfiguredRefForTest(cfg: Partial<RecallConfig>) {
...
describe("model-selection", () => {
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<RecallConfig> = {
        agents: {
          defaults: {
            models: {
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("should fall back to the configured default provider and warn if provider is missing for non-alias", async () => {
      const warnLogs = createWarnLogCapture("recall-model-selection-test");
      try {
        const cfg: Partial<RecallConfig> = {
          agents: {
            defaults: {
