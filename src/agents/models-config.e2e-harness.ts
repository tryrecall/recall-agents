...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousRecallAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousRecallAgentDir = process.env.RECALL_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousRecallAgentDir === undefined) {
      delete process.env.RECALL_AGENT_DIR;
    } else {
      process.env.RECALL_AGENT_DIR = previousRecallAgentDir;
    }
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "RECALL_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export const CUSTOM_PROXY_MODELS_CONFIG: RecallConfig = {
  models: {
    providers: {
      "custom-proxy": {
