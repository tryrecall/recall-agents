...
function writeExecApprovalsFixture(root: string, file: Record<string, unknown>): void {
  const stateDir = process.env.STEELENGINE_STATE_DIR ?? path.join(root, "state");
...
describe("exec security floor", () => {
  ...
  beforeEach(() => {
    ...
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-exec-security-floor-"));
    setTestEnvValue("HOME", tempRoot);
    setTestEnvValue("USERPROFILE", tempRoot);
    setTestEnvValue("STEELENGINE_HOME", tempRoot);
    setTestEnvValue("STEELENGINE_STATE_DIR", path.join(tempRoot, "state"));
