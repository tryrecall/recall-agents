...
describe("provider transport stream contracts", () => {
  it("covers the supported transport api alias matrix", () => {
    // Supported APIs can be projected to SteelEngine transport aliases when needed.
    const cases = [
      {
        ...
        alias: "steelengine-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "steelengine-openai-completions-transport",
      },
      {
        ...
        alias: "steelengine-azure-openai-responses-transport",
      },
      {
        ...
        alias: "steelengine-anthropic-messages-transport",
      },
      {
        ...
        alias: "steelengine-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on SteelEngine transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createSteelEngineTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the SteelEngine simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("steelengine-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the SteelEngine transport until SteelEngine preserves attribution", () => {
