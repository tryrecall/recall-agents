...
describe("provider transport stream contracts", () => {
    ...
    const cases = [
      {
        ...
        alias: "recall-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "recall-openai-completions-transport",
      },
      {
        ...
        alias: "recall-azure-openai-responses-transport",
      },
      {
        ...
        alias: "recall-anthropic-messages-transport",
      },
      {
        ...
        alias: "recall-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on Recall transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createRecallTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the Recall simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("recall-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the Recall transport until PI preserves attribution", () => {
