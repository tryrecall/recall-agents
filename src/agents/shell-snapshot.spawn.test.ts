...
describe.skipIf(process.platform === "win32")("shell snapshot subprocesses", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["HOME", "STEELENGINE_EXEC_SHELL_SNAPSHOT", "STEELENGINE_STATE_DIR"]);
    spawnMock.mockReset();
  ...
  it("does not create output pipes for status-only shell commands", async () => {
    ...
    const home = tempDirs.make("steelengine-snapshot-spawn-home-");
    const stateDir = tempDirs.make("steelengine-snapshot-spawn-state-");
