...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("recall-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("recall-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("recall-tilde-test-mkdir-");
  ...
  it("ignores RECALL_HOME for write operations", async () => {
    const openclawHome = await createTempDir("recall-home-override-", os.tmpdir());
    const dir = await createTempDir("recall-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("RECALL_HOME", openclawHome);
  ...
  it("ignores RECALL_HOME for mkdir operations", async () => {
    const openclawHome = await createTempDir("recall-home-override-", os.tmpdir());
    const dir = await createTempDir("recall-tilde-test-mkdir-");
  ...
  it("ignores RECALL_HOME for readFile operations", async () => {
    const openclawHome = await createTempDir("recall-home-override-", os.tmpdir());
    const dir = await createTempDir("recall-tilde-test-edit-");
  ...
  it("ignores RECALL_HOME for access operations", async () => {
    const openclawHome = await createTempDir("recall-home-override-", os.tmpdir());
    const dir = await createTempDir("recall-tilde-test-edit-");
