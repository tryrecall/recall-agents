...
type SteelEngineReadToolOptions = {
...
function resolveAdaptiveReadMaxBytes(options?: SteelEngineReadToolOptions): number {
...
function withWorkspaceSafeTempHint(error: unknown): unknown {
  ...
  const message = error.message.includes(".steelengine/tmp/")
    ? error.message
    : `${error.message}. Use a relative path under \`.steelengine/tmp/\` inside the workspace for scratch/temp/meta files that file tools need to read or write later.`;
...
/** Create a sandbox-backed read tool with SteelEngine result normalization. */
export function createSandboxedReadTool(params: SandboxToolParams) {
  const base = createReadTool(params.root, {
    operations: createSandboxReadOperations(params),
  }) as unknown as AnyAgentTool;
  return createSteelEngineReadTool(base, {
...
/** Wrap the base read tool with SteelEngine paging, MIME, and image handling. */
export function createSteelEngineReadTool(
  base: AnyAgentTool,
  options?: SteelEngineReadToolOptions,
): AnyAgentTool {
  return {
    ...base,
...
export function wrapReadToolWithSkillContent(
  ...
  const virtualRead = createSteelEngineReadTool(virtualBase, options);
  return {
    ...tool,
    execute: async (toolCallId, args, signal, onUpdate) => {
      const record = getToolParamsRecord(args);
