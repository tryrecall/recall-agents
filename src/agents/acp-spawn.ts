...
function resolveHeartbeatConfigForAgent(params: {
  cfg: RecallConfig;
  agentId: string;
}): NonNullable<NonNullable<RecallConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an Recall config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for Recall config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: RecallConfig, agentId: string): boolean {
...
function resolveConfiguredAcpSubagentTargetIds(cfg: RecallConfig): string[] {
