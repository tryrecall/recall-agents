    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "recall.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "recall.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "recall.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("recall-");
  const acpxRoot = await tempDirs.make("recall-acpx-plugin-");
  const helperRoot = await tempDirs.make("recall-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("recall-");
  const pluginRoot = await tempDirs.make("recall-plugin-");
  const outsideDir = await tempDirs.make("recall-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("recall-");
    const pluginRoot = await tempDirs.make("recall-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("recall-");
    const pluginRoot = await tempDirs.make("recall-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
