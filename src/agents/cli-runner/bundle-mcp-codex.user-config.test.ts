...
describe("buildCodexUserMcpServersThreadConfigPatch", () => {
  it("returns undefined when cfg has no mcp.servers (regression: #80814)", () => {
    ...
    expect(buildCodexUserMcpServersThreadConfigPatch({} as RecallConfig)).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: {} } as RecallConfig),
    ).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: { servers: {} } } as RecallConfig),
    ).toBeUndefined();
  });
  ...
  it("filters Codex-scoped user MCP servers by Recall agent id", () => {
    const cfg = {
      mcp: {
        servers: {
          atlas: {
  ...
  it("omits scoped Codex MCP servers when no Recall agent id is available", () => {
    const patch = buildCodexUserMcpServersThreadConfigPatch({
      mcp: {
        servers: {
          atlas: {
