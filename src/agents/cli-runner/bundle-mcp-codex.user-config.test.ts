/** Tests projecting SteelEngine user MCP servers into Codex app-server config. */
...
describe("buildCodexUserMcpServersThreadConfigPatch", () => {
  ...
  it("returns undefined when cfg has no mcp.servers (regression: #80814)", () => {
    ...
    expect(buildCodexUserMcpServersThreadConfigPatch({} as SteelEngineConfig)).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: {} } as SteelEngineConfig),
    ).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: { servers: {} } } as SteelEngineConfig),
    ).toBeUndefined();
  });
  ...
  it("projects exact SteelEngine MCP tool filters into Codex-native tool filters", () => {
    const patch = buildCodexUserMcpServersThreadConfigPatch({
      mcp: {
        servers: {
          docs: {
  ...
  it("rejects wildcard SteelEngine MCP tool filters that Codex cannot project exactly", () => {
    expect(() =>
      buildCodexUserMcpServersThreadConfigPatch({
        mcp: {
          servers: {
  ...
  it("filters Codex-scoped user MCP servers by SteelEngine agent id", () => {
    // Agent-scoped MCP servers should follow the active SteelEngine agent, while
  ...
  it("omits scoped Codex MCP servers when no SteelEngine agent id is available", () => {
    const patch = buildCodexUserMcpServersThreadConfigPatch({
      mcp: {
        servers: {
          atlas: {
