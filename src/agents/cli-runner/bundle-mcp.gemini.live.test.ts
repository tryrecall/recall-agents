...
const LIVE = isLiveTestEnabled(["RECALL_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "recall-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("openclaw_live_probe", "Recall Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an Recall-configured streamable-http server", async () => {
    const geminiCommand = process.env.RECALL_LIVE_GEMINI_COMMAND ?? "gemini";
