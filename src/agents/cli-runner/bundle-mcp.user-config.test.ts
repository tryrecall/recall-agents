/** Tests merging user SteelEngine MCP server config into Claude bundle-MCP overlays. */
...
describe("prepareCliBundleMcpConfig user mcp.servers", () => {
  ...
  it("merges user-configured mcp.servers from SteelEngine config", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "steelengine-cli-bundle-mcp-user-servers-",
    );
  ...
  it("translates SteelEngine transport field on user mcp.servers into Claude type", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "steelengine-cli-bundle-mcp-user-servers-transport-",
    );
  ...
  it("preserves explicit type and still strips transport on user mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "steelengine-cli-bundle-mcp-user-servers-transport-explicit-",
    );
  ...
  it("omits unavailable OAuth servers without blocking the CLI agent", async () => {
    ...
    const warn = vi.fn();
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "steelengine-cli-bundle-mcp-user-servers-oauth-",
    );
  ...
  it("user mcp.servers do not override the loopback additionalConfig", async () => {
    // The SteelEngine loopback server is generated runtime state and must win over
    // user config with the same server name.
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "steelengine-cli-bundle-mcp-user-servers-loopback-",
    );
      ...
      additionalConfig: {
        mcpServers: {
          steelengine: {
            ...
            headers: { Authorization: "Bearer ${STEELENGINE_MCP_TOKEN}" },
          },
        },
      },
    });
  ...
  it("replaces overlapping bundle server entries with user-configured mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "steelengine-cli-bundle-mcp-user-servers-replace-",
    );
