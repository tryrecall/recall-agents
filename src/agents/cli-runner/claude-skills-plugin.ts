...
const RECALL_CLAUDE_PLUGIN_NAME = "recall-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredRecallTmpDir(), "recall-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, RECALL_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: RECALL_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Recall skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
