...
describe("prepareCliBundleMcpConfig", () => {
  ...
  it("loads workspace bundle MCP plugins from the configured workspace root", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "recall-cli-bundle-mcp-workspace-root-",
    );
    const pluginRoot = path.join(workspaceDir, ".recall", "extensions", "workspace-probe");
  ...
  it("merges loopback overlay config with bundle MCP servers", async () => {
    ...
    expect(Object.keys(raw.mcpServers ?? {}).toSorted()).toEqual(["bundleProbe", "recall"]);
    expect(raw.mcpServers?.recall?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.recall?.headers?.Authorization).toBe("Bearer ${RECALL_MCP_TOKEN}");
  ...
  it("preserves extra env values alongside generated MCP config", async () => {
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      config: { plugins: { enabled: false } },
      env: {
        RECALL_MCP_TOKEN: "loopback-token-123",
        RECALL_MCP_SESSION_KEY: "agent:main:telegram:group:chat123",
      },
    });
  ...
  it("leaves args untouched when bundle MCP is disabled", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/recall-bundle-mcp-disabled",
    });
