...
describe("buildCliAgentSystemPrompt", () => {
  ...
  it("uses CLI backend tool fallback instead of SteelEngine tool assumptions", () => {
    const prompt = buildCliAgentSystemPrompt({
      workspaceDir: "/tmp/steelengine",
      tools: [],
      modelDisplay: "test/model",
    ...
    expect(prompt).not.toContain("Larger work: use `sessions_spawn`");
    expect(prompt).not.toContain("Do not poll `subagents list` / `sessions_list` in a loop");
    expect(prompt).toContain("No SteelEngine tool list is injected");
  });
  ...
  it("uses cwd, not bootstrap workspace, for CLI workspace guidance", () => {
    const prompt = buildCliAgentSystemPrompt({
      workspaceDir: "/tmp/steelengine-agent",
      cwd: "/tmp/task-repo",
      tools: [],
      modelDisplay: "test/model",
    });
    ...
    expect(prompt).not.toContain("Working directory: /tmp/steelengine-agent");
  });
  ...
  it("includes CLI-scoped plugin command guidance", () => {
    ...
    // SteelEngine-main command text into external CLI backends.
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "SteelEngine-only command guidance.",
    ...
    expect(prompt).not.toContain("SteelEngine-only command guidance.");
  });
