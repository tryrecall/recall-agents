...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    envSnapshot = captureEnv(["RECALL_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("recall-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("recall-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("recall-cli-bundle-mcp-bundled-");
    process.env.RECALL_BUNDLED_PLUGINS_DIR = emptyBundledDir;
...
function createEnabledBundleProbeConfig(): RecallConfig {
  return {
    plugins: {
      entries: {
