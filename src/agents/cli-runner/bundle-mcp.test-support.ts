...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    ...
    envSnapshot = captureEnv(["STEELENGINE_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("steelengine-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("steelengine-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("steelengine-cli-bundle-mcp-bundled-");
    setTestEnvValue("STEELENGINE_BUNDLED_PLUGINS_DIR", emptyBundledDir);
...
function createEnabledBundleProbeConfig(): SteelEngineConfig {
  return {
    plugins: {
      entries: {
