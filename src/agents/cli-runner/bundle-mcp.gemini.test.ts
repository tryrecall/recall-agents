...
describe("prepareCliBundleMcpConfig gemini", () => {
  it("writes Gemini system settings for bundle MCP servers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/recall-bundle-mcp-gemini",
    ...
    expect(prepared.env?.RECALL_MCP_TOKEN).toBe("loopback-token-123");
    ...
    expect(raw.mcp?.allowed).toEqual(["recall"]);
    expect(raw.mcpServers?.recall?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.recall?.headers?.Authorization).toBe("Bearer loopback-token-123");
