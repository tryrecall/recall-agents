...
const CLI_BACKEND_PRESERVE_ENV = "RECALL_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.RECALL_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.RECALL_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.RECALL_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.RECALL_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.RECALL_MCP_MESSAGE_CHANNEL || "<empty>"}`,
  ].join(" ");
}
    ...
    return await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (env.RECALL_MCP_TOKEN || env.RECALL_MCP_SESSION_KEY || env.RECALL_MCP_AGENT_ID) {
