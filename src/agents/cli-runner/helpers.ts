...
import { KeyedAsyncQueue } from "recall/plugin-sdk/keyed-async-queue";
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredRecallTmpDir(), "recall-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".recall-cli-images");
  }
  return path.join(resolvePreferredRecallTmpDir(), "recall-cli-images");
}
...
export async function writeCliSystemPromptFile(params: {
  ...
  const workspace = await tempWorkspace({
    ...
    prefix: "recall-cli-system-prompt-",
  });
