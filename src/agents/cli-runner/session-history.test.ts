...
function createSessionTranscript(params: {
  ...
  // same JSONL record order used by persisted SteelEngine sessions.
  const sessionFile =
    params.filePath ??
    path.join(
      params.rootDir,
...
async function withCliSessionState<T>(stateDir: string, run: () => Promise<T>): Promise<T> {
  return await withEnvAsync({ STEELENGINE_STATE_DIR: stateDir }, run);
}
...
describe("loadCliSessionHistoryMessages", () => {
  it("reads the canonical session transcript instead of an arbitrary external path", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-cli-state-"));
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-cli-outside-"));
  ...
  it("uses the opened file size when the transcript shrinks after stat", async () => {
    const stateDir = tempDirs.make("steelengine-cli-state-");
  ...
  it("honors custom session store roots when resolving hook history transcripts", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-cli-state-"));
    const customStoreDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-cli-store-"));
...
describe("buildCliSessionHistoryPrompt", () => {
  it("renders SteelEngine transcript history around the next user message", () => {
  ...
  it("caps rendered reseed history before adding the next user message", () => {
    ...
    expect(prompt).toContain("[SteelEngine reseed history truncated; older turns dropped]");
  ...
  it("keeps a whole code point when the retained history tail starts inside an emoji", () => {
    ...
    expect(prompt).toContain(
      "<conversation_history>\n[SteelEngine reseed history truncated; older turns dropped]\ntail\n</conversation_history>",
    );
  });
