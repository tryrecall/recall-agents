...
describe("loadCliSessionHistoryMessages", () => {
  ...
  it("reads the canonical session transcript instead of an arbitrary external path", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-cli-state-"));
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-cli-outside-"));
  ...
  it("honors custom session store roots when resolving hook history transcripts", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-cli-state-"));
    const customStoreDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-cli-store-"));
    vi.stubEnv("RECALL_STATE_DIR", stateDir);
...
describe("buildCliSessionHistoryPrompt", () => {
  it("renders Recall transcript history around the next user message", () => {
  ...
  it("caps rendered reseed history before adding the next user message", () => {
    ...
    expect(prompt).toContain("[Recall reseed history truncated]");
