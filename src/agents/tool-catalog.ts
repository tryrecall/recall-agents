...
function buildCoreToolGroupMap() {
  ...
  const openclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInRecallGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:recall": openclawTools,
