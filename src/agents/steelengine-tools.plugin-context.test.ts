...
import { resolveSteelEnginePluginToolInputs } from "./steelengine-tools.plugin-context.js";
import { applyPluginToolDeliveryDefaults } from "./plugin-tool-delivery-defaults.js";
import type { AnyAgentTool } from "./tools/common.js";

describe("steelengine plugin tool context", () => {
  it("forwards trusted requester sender identity", () => {
    const result = resolveSteelEnginePluginToolInputs({
      options: {
        config: {} as never,
  ...
  it("defaults missing and unknown conversation-read origins to delegated", () => {
    const missing = resolveSteelEnginePluginToolInputs({
      options: { config: {} as never },
    });
    const unknown = resolveSteelEnginePluginToolInputs({
      options: {
        config: {} as never,
