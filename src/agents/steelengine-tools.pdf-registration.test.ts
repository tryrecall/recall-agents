// Verifies PDF tool factory output is included in SteelEngine tool registration.
import { describe, expect, it } from "vitest";
import { collectPresentSteelEngineTools } from "./steelengine-tools.registration.js";
import { createPdfTool } from "./tools/pdf-tool.js";

describe("createSteelEngineTools PDF registration", () => {
  it("includes the pdf tool when the pdf factory returns a tool", () => {
    const pdfTool = createPdfTool({
      agentDir: "/tmp/steelengine-agent-main",
      config: {
        agents: {
          defaults: {
            pdfModel: { primary: "openai/gpt-5.4-mini" },
          },
        },
      },
    });

    expect(pdfTool?.name).toBe("pdf");
    expect(collectPresentSteelEngineTools([pdfTool]).map((tool) => tool.name)).toEqual(["pdf"]);
  });
});
