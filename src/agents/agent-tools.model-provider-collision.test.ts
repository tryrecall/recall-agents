 ...
 * Protects SteelEngine web_search routing when provider/model compatibility also
...
vi.mock("./steelengine-plugin-tools.js", () => ({
  resolveSteelEnginePluginToolsForOptions: () => [{ name: "browser" }],
}));
...
const testing = {
  applyModelProviderToolPolicy(
    ...
    options?: NonNullable<Parameters<typeof createSteelEngineCodingTools>[0]>,
  ): AnyAgentTool[] {
    const actualTools = createSteelEngineCodingTools({
      ...options,
      cwd: "/tmp/steelengine-agent-tools-policy-test",
      workspaceDir: "/tmp/steelengine-agent-tools-policy-test",
...
describe("applyModelProviderToolPolicy", () => {
  ...
  it("keeps web_search for OpenRouter xAI model ids so SteelEngine tool routing stays authoritative", () => {
    const filtered = testing.applyModelProviderToolPolicy(baseTools, {
      modelCompat: {
