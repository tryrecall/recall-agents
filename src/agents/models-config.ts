...
import { planRecallModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: RecallConfig): {
  config: RecallConfig;
  sourceConfigForSecrets: RecallConfig;
} {
...
export async function ensureRecallModelsJson(
  config?: RecallConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveRecallAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planRecallModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
