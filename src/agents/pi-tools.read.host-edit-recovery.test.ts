...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-edit-recovery-"));
  ...
  it("recovers post-write failures when edit calls use file_path", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-edit-recovery-"));
    const workspaceDir = path.join(tmpDir, ".recall", "workspace");
  ...
  it("recovers tilde paths against the OS home even when RECALL_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const openclawHome = path.join(tmpDir, "recall-home");
    await fs.mkdir(osHome, { recursive: true });
    await fs.mkdir(openclawHome, { recursive: true });

    const previousHome = process.env.HOME;
    const previousUserProfile = process.env.USERPROFILE;
    const previousOpenclawHome = process.env.RECALL_HOME;
    process.env.HOME = osHome;
    process.env.USERPROFILE = osHome;
    process.env.RECALL_HOME = openclawHome;
    ...
    try {
      ...
      if (previousOpenclawHome === undefined) {
        ...
        process.env.RECALL_HOME = previousOpenclawHome;
      }
    }
  });
