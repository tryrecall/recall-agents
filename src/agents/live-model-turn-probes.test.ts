...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "RECALL_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { RECALL_LIVE_MODEL_IMAGE_PROBE: "false" },
        "RECALL_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { RECALL_LIVE_MODEL_IMAGE_PROBE: "1" },
        "RECALL_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
