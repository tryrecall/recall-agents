...
const STEELENGINE_TRANSPORT_TO_CLI_BUNDLE_TYPE: Record<string, string> = {
...
/**
 * User config stores SteelEngine MCP transport names, while CLI backends such as
 * Claude Code and Gemini expect a downstream `type` field. Keep this adapter
 * out of the generic merge path because embedded SteelEngine still consumes the raw
 * SteelEngine `transport` shape directly.
 */
export function toCliBundleMcpServerConfig(server: BundleMcpServerConfig): BundleMcpServerConfig {
  ...
  if (typeof rawTransport === "string") {
    const mapped = STEELENGINE_TRANSPORT_TO_CLI_BUNDLE_TYPE[rawTransport];
    if (mapped) {
      next.type = mapped;
    }
  }
...
export function loadMergedBundleMcpConfig(params: {
  ...
  return {
    config: {
      // SteelEngine config is the owner-managed layer, so it overrides bundle defaults.
      mcpServers: {
        ...enabledBundleMcp,
        ...Object.fromEntries(
