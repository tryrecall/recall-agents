...
vi.mock("../process/supervisor/index.js", () => {
  ...
  const applySegmentShellEffects = (segment: string, env: NodeJS.ProcessEnv) => {
    if (segment === 'export PATH="${RECALL_PREPEND_PATH}${PATH:+:$PATH}"') {
      const prepend = env.RECALL_PREPEND_PATH ?? "";
      const current = readEnvPath(env);
      writeEnvPath(env, `${prepend}${current ? `:${current}` : ""}`);
      return;
    }
    if (segment === "unset RECALL_PREPEND_PATH") {
      delete env.RECALL_PREPEND_PATH;
    }
  };
...
const defaultShell = isWin
  ? undefined
  : process.env.RECALL_TEST_SHELL || resolveShellFromPath("bash") || process.env.SHELL || "sh";
...
const notifyCfg = {} as RecallConfig;
