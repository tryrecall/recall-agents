...
vi.mock("../process/supervisor/index.js", () => {
  ...
  const applySegmentShellEffects = (segment: string, env: NodeJS.ProcessEnv) => {
    if (segment === 'export PATH="${STEELENGINE_PREPEND_PATH}${PATH:+:$PATH}"') {
      const prepend = env.STEELENGINE_PREPEND_PATH ?? "";
      const current = readEnvPath(env);
      writeEnvPath(env, `${prepend}${current ? `:${current}` : ""}`);
      return;
    }
    if (segment === "unset STEELENGINE_PREPEND_PATH") {
      delete env.STEELENGINE_PREPEND_PATH;
    }
  };
...
const SHELL_ENV_KEYS = ["STEELENGINE_EXEC_SHELL_SNAPSHOT", "SHELL"] as const;
const PATH_SHELL_ENV_KEYS = ["STEELENGINE_EXEC_SHELL_SNAPSHOT", "PATH", "SHELL"] as const;
...
const notifyCfg = {} as SteelEngineConfig;
