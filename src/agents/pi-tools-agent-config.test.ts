...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: RecallConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<RecallConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<RecallConfig["agents"]>["list"]>[number]["tools"];
  }): RecallConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createRecallCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createRecallCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createRecallCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createRecallCodingTools({
      config: cfg,
    ...
    const bobTools = createRecallCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createRecallCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createRecallCodingTools({
      config: cfg,
