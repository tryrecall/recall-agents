...
vi.mock("../plugins/provider-runtime.js", () => ({
  applyProviderConfigDefaultsWithPlugin: (config: RecallConfig) => config,
...
let planRecallModelsJsonWithDeps: typeof import("./models-config.plan.js").planRecallModelsJsonWithDeps;
let readGeneratedModelsJson: typeof import("./models-config.test-utils.js").readGeneratedModelsJson;
const fixtureSuite = createFixtureSuite("recall-models-runtime-source-");

beforeAll(async () => {
  await fixtureSuite.setup();
  ({ clearConfigCache, clearRuntimeConfigSnapshot, setRuntimeConfigSnapshot } =
    await import("../config/io.js"));
  ({ ensureRecallModelsJson, resetModelsJsonReadyCacheForTest } =
    await import("./models-config.js"));
  ({ planRecallModelsJsonWithDeps } = await import("./models-config.plan.js"));
...
function createOpenAiApiKeySourceConfig(): RecallConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiApiKeyRuntimeConfig(): RecallConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderSourceConfig(): RecallConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderRuntimeConfig(): RecallConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiSourceConfigWithHeadersAndApiKey(): RecallConfig {
...
function createOpenAiRuntimeConfigWithHeadersAndApiKey(): RecallConfig {
...
function withGatewayTokenMode(config: RecallConfig): RecallConfig {
  return {
    ...config,
    gateway: {
      auth: {
...
async function planGeneratedProviders(params: {
  config: RecallConfig;
  sourceConfigForSecrets: RecallConfig;
}) {
  const plan = await planRecallModelsJsonWithDeps(
    {
      cfg: params.config,
      sourceConfigForSecrets: params.sourceConfigForSecrets,
      agentDir: "/tmp/recall-models-plan",
      env: {},
      existingRaw: "",
      existingParsed: null,
    },
...
describe("models-config runtime source snapshot", () => {
  it("uses runtime source snapshot markers when passed the active runtime config", () => {
    ...
    const runtimeConfig: RecallConfig = {
      models: {
        providers: {
  ...
  it("projects cloned runtime configs onto source snapshot when preserving provider auth", async () => {
    ...
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      const clonedRuntimeConfig: RecallConfig = {
        ...runtimeConfig,
        agents: {
          defaults: {
      ...
      try {
        ...
        await ensureRecallModelsJson(clonedRuntimeConfig, agentDir);
  ...
  it("invalidates cached readiness when projected config changes under the same runtime snapshot", async () => {
    ...
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      const sourceConfig = createOpenAiApiKeySourceConfig();
      const runtimeConfig = createOpenAiApiKeyRuntimeConfig();
      const firstCandidate: RecallConfig = {
        ...runtimeConfig,
        models: {
          providers: {
            openai: {
      ...
      const secondCandidate: RecallConfig = {
        ...runtimeConfig,
        models: {
          providers: {
            openai: {
      ...
      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureRecallModelsJson(firstCandidate, agentDir);
        ...
        expect(parsed.providers.openai?.headers?.["X-Recall-Test"]).toBe("one");

        // Header changes still rewrite models.json, but merge mode preserves the existing baseUrl.
        await ensureRecallModelsJson(secondCandidate, agentDir);
        ...
        expect(parsed.providers.openai?.baseUrl).toBe("https://api.openai.com/v1");
        expect(parsed.providers.openai?.apiKey).toBe("OPENAI_API_KEY"); // pragma: allowlist secret
        expect(parsed.providers.openai?.headers?.["X-Recall-Test"]).toBe("two");
      } finally {
        clearRuntimeConfigSnapshot();
        clearConfigCache();
      }
