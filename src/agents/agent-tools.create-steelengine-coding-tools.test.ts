/**
 * Broad coverage for createSteelEngineCodingTools.
...
import * as steelEnginePluginTools from "./steelengine-plugin-tools.js";
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createSteelEngineCodingTools>) {
...
type SteelEngineCodingTool = ReturnType<typeof createSteelEngineCodingTools>[number];
type SteelEngineToolsOptions = NonNullable<Parameters<typeof createSteelEngineTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: SteelEngineCodingTool[], name: string): SteelEngineCodingTool {
...
function requireToolExecute(tool: SteelEngineCodingTool): NonNullable<SteelEngineCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateSteelEngineToolsOptions(): SteelEngineToolsOptions {
  const calls = vi.mocked(createSteelEngineTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createSteelEngineTools call");
  }
  return options;
}
...
function cronCreatorToolNames(
  list: SteelEngineToolsOptions["cronCreatorToolAllowlist"] | undefined,
): string[] | undefined {
  return list?.map((entry) => (typeof entry === "string" ? entry : entry.name));
}

describe("createSteelEngineCodingTools", () => {
  ...
  it("exposes only gateway config reads to owner sessions", () => {
    const tools = createSteelEngineCodingTools({ config: testConfig });
  ...
  it("passes explicit hook channel ids to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-hook-channel-"));
  ...
  it("re-wraps existing before_tool_call hooks once with the current context", async () => {
    ...
    vi.mocked(createSteelEngineTools).mockReturnValueOnce([wrapped as never]);

    const tools = createSteelEngineCodingTools({ agentId: "main", sessionId: "session-new" });
  ...
  it("keeps Tool Search controls when core SteelEngine tools are not materialized", () => {
    const createSteelEngineToolsMock = vi.mocked(createSteelEngineTools);
    createSteelEngineToolsMock.mockClear();
    ...
    expect(createSteelEngineToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("keeps the injected ring-zero tool under policy and rejects a same-name replacement", () => {
    ...
    vi.mocked(createSteelEngineTools).mockReturnValueOnce([duplicateTool]);

    const tools = runWithAgentRingZeroTools([injectedTool], () =>
      createSteelEngineCodingTools({
        config: { tools: { allow: ["read"], deny: ["steelengine"] } },
    ...
    expect(tools[0]?.name).toBe("steelengine");
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createSteelEngineToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateSteelEngineToolsOptions();
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:steelengine"], ["*"]]) {
      const tools = createSteelEngineCodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("passes source reply delivery mode to SteelEngine tool construction", () => {
    const createSteelEngineToolsMock = vi.mocked(createSteelEngineTools);
    createSteelEngineToolsMock.mockClear();
    ...
    expect(createSteelEngineToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateSteelEngineToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });

  it("passes configured filesystem policy to SteelEngine tool construction", () => {
    const createSteelEngineToolsMock = vi.mocked(createSteelEngineTools);
    createSteelEngineToolsMock.mockClear();
    ...
    expect(latestCreateSteelEngineToolsOptions().fsPolicy).toEqual({ workspaceOnly: true });
  });
  ...
  it("uses the canonical spawn workspace for follow-up task suggestions", () => {
    ...
    expect(latestCreateSteelEngineToolsOptions()).toMatchObject({
      workspaceDir: "/agent/workspace",
      spawnWorkspaceDir: "/host/project",
      cwd: "/host/project",
    });
  ...
  it("keeps an unsandboxed task repo as the follow-up suggestion cwd", () => {
    ...
    expect(latestCreateSteelEngineToolsOptions().cwd).toBe("/task/repo");
  });
  ...
  it("passes plugin suppression into SteelEngine tool construction plans", () => {
    const createSteelEngineToolsMock = vi.mocked(createSteelEngineTools);
    createSteelEngineToolsMock.mockClear();
    ...
    expect(createSteelEngineToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateSteelEngineToolsOptions().disablePluginTools).toBe(true);
  });

  it("forwards trusted conversation recall to SteelEngine tool construction", () => {
    const createSteelEngineToolsMock = vi.mocked(createSteelEngineTools);
    createSteelEngineToolsMock.mockClear();
    ...
    expect(latestCreateSteelEngineToolsOptions().conversationRecall).toEqual(conversationRecall);
  });

  it("keeps plugin-only construction off the SteelEngine core factory", () => {
    const createSteelEngineToolsMock = vi.mocked(createSteelEngineTools);
    createSteelEngineToolsMock.mockClear();
  ...
  it("forwards prepared run facts to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(steelEnginePluginTools, "resolveSteelEnginePluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("forwards the native channel id through standard tool construction", () => {
    ...
    expect(latestCreateSteelEngineToolsOptions().nativeChannelId).toBe("oc_native_chat");
    expect(latestCreateSteelEngineToolsOptions().currentChatType).toBe("group");
    expect(latestCreateSteelEngineToolsOptions().messageActionTurnCapability).toBe(
      "turn-capability-1",
    );
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createSteelEngineToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateSteelEngineToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
  ...
  it("materializes additive runtime tools while preserving normal deny policy", () => {
    ...
    expect(createSteelEngineToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateSteelEngineToolsOptions().pluginToolAllowlist).not.toContain(
      "workboard_heartbeat",
    );
    expect(
      isPluginToolAllowed(
        new Set(latestCreateSteelEngineToolsOptions().pluginToolAllowlist),
        "workboard",
        "workboard_heartbeat",
      ),
    ).toBe(true);
    expect(latestCreateSteelEngineToolsOptions()).not.toHaveProperty("runtimePluginToolGrant");
    expectListIncludes(latestCreateSteelEngineToolsOptions().pluginToolDenylist, ["workboard_block"]);
  });

  it("passes explicit denylist entries to SteelEngine tool factory planning", () => {
    const createSteelEngineToolsMock = vi.mocked(createSteelEngineTools);
    createSteelEngineToolsMock.mockClear();
    ...
    expect(createSteelEngineToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateSteelEngineToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to SteelEngine plugin discovery", async () => {
    const createSteelEngineToolsMock = vi.mocked(createSteelEngineTools);
    createSteelEngineToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `steelengine-session-store-${agentId}-{agentId}.json`,
    );
    ...
    expect(createSteelEngineToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateSteelEngineToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes effective allow-list-restricted tool surface to spawned sessions", () => {
    ...
    expect(createSteelEngineToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateSteelEngineToolsOptions().inheritedToolAllowlist;
  ...
  it("passes group-restricted tool surface to cron-created agent turns", () => {
    ...
    expect(createSteelEngineToolsMock).toHaveBeenCalledTimes(1);
    const cronAllow = latestCreateSteelEngineToolsOptions().cronCreatorToolAllowlist;
  ...
  it("lets embedded attempts refresh a caller-owned cron creator tool surface", () => {
    ...
    const cronCreatorToolAllowlistRef: NonNullable<
      SteelEngineToolsOptions["cronCreatorToolAllowlist"]
    > = [];
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("steelengine-tools:test-helper")).toBeLessThan(
      stages.indexOf("steelengine-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createSteelEngineCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createSteelEngineCodingTools({
      config: testConfig,
    ...
    const codexTools = createSteelEngineCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: SteelEngineConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createSteelEngineCodingTools({
    ...
    const anthropicTools = createSteelEngineCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: SteelEngineConfig = {
      tools: {
        exec: {
    ...
    const allowed = createSteelEngineCodingTools({
    ...
    const denied = createSteelEngineCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createSteelEngineCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createSteelEngineCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("separates the canonical message provider from transport tool policy", () => {
    vi.mocked(createSteelEngineTools).mockClear();
    ...
    expect(latestCreateSteelEngineToolsOptions().agentChannel).toBe("discord");
  });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createSteelEngineCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createSteelEngineCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createSteelEngineCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createSteelEngineCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createSteelEngineCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createSteelEngineCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createSteelEngineCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createSteelEngineCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createSteelEngineCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-read-"));
    try {
      ...
      const contents = "Hello from steelengine read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "steelengine-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "steelengine-workspace"),
    ...
    const tools = createSteelEngineCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-canonical-"));
    try {
      const tools = createSteelEngineCodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-memory-cwd-"));
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-legacy-alias-"));
    try {
      const tools = createSteelEngineCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-structured-write-"));
    try {
      const tools = createSteelEngineCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-structured-edit-"));
    try {
