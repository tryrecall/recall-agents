...
function hasExplicitVideoGenerationModelConfig(cfg?: RecallConfig): boolean {
...
export function createVideoGenerateTool(options?: {
  ...
  const cfg: RecallConfig = options?.config ?? getRuntimeConfig();
  if (
    !hasGenerationToolAvailability({
      cfg,
