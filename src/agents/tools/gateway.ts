...
function resolveLocalGatewayUrlKeys(cfg: SteelEngineConfig): Set<string> {
...
function resolveConfiguredRemoteGatewayKey(cfg: SteelEngineConfig): string | undefined {
...
export function resolveGatewayOptions(opts?: GatewayCallOptions) {
  ...
  const timeoutMs =
    typeof opts?.timeoutMs === "number" && Number.isFinite(opts.timeoutMs)
      ...
      : 30_000;
  const envGatewayUrl = trimToUndefined(process.env.STEELENGINE_GATEWAY_URL);
  const target =
  ...
  try {
    ...
    if (isNodeApprovalReplay) {
      throw new Error(
        [
          ...
          "Fix the SteelEngine state directory permissions and retry the approval.",
        ].join(" "),
        { cause: error },
      );
    }
    ...
    throw new Error(
      [
        ...
        "Fix the SteelEngine state directory permissions or use the local approval-runtime gateway.",
      ].join(" "),
      { cause: error },
    );
  }
...
function staleGatewayAgentRuntimeIdentityError(cause: unknown): Error {
  return new Error(
    [
      "The running Gateway is from an older SteelEngine build and rejected current agent runtime connection metadata.",
      "Restart the Gateway with `steelengine gateway restart`, then retry.",
    ].join(" "),
    { cause },
  );
}
