...
describe("skill_workshop tool", () => {
  it("describes action selection and pending-proposal discovery in its schema", () => {
    const tool = createSkillWorkshopTool({ workspaceDir: "/tmp/steelengine" });
  ...
  it("documents that proposal_content must be final skill body content, not a plan or change description", () => {
    ...
    const proposalOnlySchema = JSON.stringify(
      createSkillWorkshopTool({ workspaceDir: "/tmp/steelengine", proposalOnly: true }).parameters,
    );
  ...
  it("is exposed in the SteelEngine tool set", async () => {
    const workspaceDir = await tempDirs.make("steelengine-skill-workshop-tool-");
  ...
  it("keeps proposal state inside an injected state directory", async () => {
    const workspaceDir = await tempDirs.make("steelengine-skill-workshop-isolated-workspace-");
    const isolatedStateDir = await tempDirs.make("steelengine-skill-workshop-isolated-state-");
    const env = { ...process.env, STEELENGINE_STATE_DIR: isolatedStateDir };
  ...
  it("restricts internal review runs to one pending proposal mutation", async () => {
    const workspaceDir = await tempDirs.make("steelengine-skill-workshop-review-");
  ...
  it("does not refund the review mutation budget after a failed mutation", async () => {
    const workspaceDir = await tempDirs.make("steelengine-skill-workshop-review-failure-");
    const proposalMutationBudget: SkillWorkshopProposalMutationBudget = { remaining: 1 };
    const tool = createSkillWorkshopTool({
      workspaceDir,
      proposalOnly: true,
  ...
  it("durably completes a proposal review and blocks later work", async () => {
    const workspaceDir = await tempDirs.make("steelengine-skill-workshop-review-completion-");
    let completions = 0;
  ...
  it("honors a larger internal review mutation budget", async () => {
    const workspaceDir = await tempDirs.make("steelengine-skill-workshop-history-review-");
  ...
  it("counts repeated revisions as one distinct proposal idea", async () => {
    const workspaceDir = await tempDirs.make("steelengine-skill-workshop-distinct-review-");
    const proposalMutationBudget: SkillWorkshopProposalMutationBudget = { remaining: 3 };
    const tool = createSkillWorkshopTool({
      workspaceDir,
      proposalOnly: true,
  ...
  it("is not exposed from sandboxed SteelEngine tool sets", async () => {
    const workspaceDir = await tempDirs.make("steelengine-skill-workshop-tool-");
    const tools = createSteelEngineTools({
      workspaceDir,
      config: {},
  ...
  it("scopes proposal discovery to the tool workspace", async () => {
    const firstWorkspaceDir = await tempDirs.make("steelengine-skill-workshop-tool-first-");
    const secondWorkspaceDir = await tempDirs.make("steelengine-skill-workshop-tool-second-");
