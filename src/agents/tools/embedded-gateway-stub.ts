...
interface EmbeddedGatewayRuntime {
  resolveSessionAgentId: (opts: { sessionKey: string; config: RecallConfig }) => string;
  ...
  resolveEffectiveChatHistoryMaxChars: (cfg: RecallConfig) => number;
  ...
  loadCombinedSessionStoreForGateway: (cfg: RecallConfig) => {
    storePath: string;
    store: unknown;
  };
