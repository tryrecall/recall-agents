...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `recall security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as RecallConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as RecallConfig);
