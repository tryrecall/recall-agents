...
import { createSteelEngineDelegateToolsForRun } from "./steelengine-delegate-tool.js";
...
describe("steelengine delegation tool", () => {
  it("relays context and surfaces pending approval", async () => {
    ...
    const tool = createSteelEngineDelegateToolsForRun({
      sessionAgentId: "main",
      runSessionKey: "agent:main:dm:one",
      agentChannel: "webchat",
    })[0];
    if (!tool) {
      throw new Error("expected SteelEngine delegation tool");
    }

    const result = await tool.execute("call-1", { message: "Add channel." });

    expect(callGateway).toHaveBeenCalledWith("steelengine.chat", {
  ...
  it("uses the owner-only core gate", () => {
    expect(GATEWAY_OWNER_ONLY_CORE_TOOLS).toContain("steelengine");
  });
});
