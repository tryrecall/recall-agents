...
describe("createAgentToolsSandboxContext", () => {
  it("provides stable defaults for agent-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("steelengine-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("steelengine-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("steelengine-sbx-");
