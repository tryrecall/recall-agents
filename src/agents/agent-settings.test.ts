...
describe("isSilentOverflowProneModel", () => {
  ...
  // steelengine#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable SteelEngine runtime's secondary compaction path;
  // SteelEngine's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of steelengine#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering SteelEngine runtime's
  ...
  // state.messages intact; SteelEngine's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep SteelEngine runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove SteelEngine runtime's
