...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-agent-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: SteelEngineConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<SteelEngineConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<SteelEngineConfig["agents"]>["list"]>[number]["tools"];
  }): SteelEngineConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("keeps core tools for owner WebChat while restricting non-owners", () => {
    ...
    expect(ownerTools).toContain("steelengine");
    ...
    expect(nonOwnerTools).not.toContain("steelengine");
