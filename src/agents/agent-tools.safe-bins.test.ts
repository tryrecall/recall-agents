...
let createSteelEngineCodingTools: typeof import("./agent-tools.js").createSteelEngineCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      STEELENGINE_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "steelengine-test-no-bundled-extensions"),
    },
    async () => {
      ({ createSteelEngineCodingTools } = await import("./agent-tools.js"));
    },
  );
});
...
async function withSafeBinsExecTool(
  ...
  try {
    await withEnvAsync(
      {
        STEELENGINE_SHELL_ENV_TIMEOUT_MS: "1",
        PATH: "/usr/bin:/bin",
        SHELL: "/bin/sh",
      },
      async () => {
...
describe("createSteelEngineCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "steelengine-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "steelengine-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "steelengine-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "steelengine-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "steelengine-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "steelengine-safe-bins-grep-",
        safeBins: ["grep"],
