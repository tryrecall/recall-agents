...
const pluginEnvSnapshot = captureEnv(["RECALL_DISABLE_BUNDLED_PLUGINS"]);
...
function rawSkillApiKeyRefConfig(skillName: string): RecallConfig {
  return {
    skills: {
      entries: {
        [skillName]: {
...
function resolvedSkillApiKeyConfig(skillName: string, apiKey: string): RecallConfig {
  return {
    skills: {
      entries: {
        [skillName]: {
...
describe("buildWorkspaceSkillCommandSpecs", () => {
  ...
  it("includes enabled Claude bundle markdown commands as native Recall slash commands", async () => {
    ...
    const pluginRoot = path.join(workspaceDir, ".recall", "extensions", "compound-bundle");
