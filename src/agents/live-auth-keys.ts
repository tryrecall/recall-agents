...
const GOOGLE_LIVE_SINGLE_KEY = "RECALL_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "RECALL_LIVE_ANTHROPIC_KEY",
    listVar: "RECALL_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "RECALL_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `RECALL_LIVE_${base}_KEY`;
