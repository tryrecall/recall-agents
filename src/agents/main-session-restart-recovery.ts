...
export async function markRestartAbortedMainSessions(params: {
  cfg?: RecallConfig;
  additionalCfgs?: Iterable<RecallConfig | undefined>;
  stateDir?: string;
  ...
  const env =
    params.stateDir === undefined
      ? process.env
      : { ...process.env, RECALL_STATE_DIR: params.stateDir };
  const stateDir = resolveStateDir(env);
  const configs = [params.cfg, ...(params.additionalCfgs ?? [])].filter(
    (cfg): cfg is RecallConfig => Boolean(cfg),
  );
...
function buildResumeMessage(pendingFinalDeliveryText?: string | null): string {
  const base =
    ...
    "Recall was waiting on tool/model work. Continue from the existing " +
...
async function resolveRestartRecoveryStorePaths(params: {
  ...
  if (params.cfg) {
    const env = { ...process.env, RECALL_STATE_DIR: stateDir };
