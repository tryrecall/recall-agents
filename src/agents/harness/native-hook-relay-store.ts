...
type NativeHookRelayBridgeDatabase = Pick<SteelEngineStateKyselyDatabase, "native_hook_relay_bridges">;

type NativeHookRelayBridgeRow = SteelEngineStateKyselyDatabase["native_hook_relay_bridges"];
...
export function pruneNativeHookRelayBridgeRecords(params: {
  ...
  const database = openSteelEngineStateDatabase({ path: params.stateDbPath });
  const db = getNodeSqliteKysely<NativeHookRelayBridgeDatabase>(database.db);
