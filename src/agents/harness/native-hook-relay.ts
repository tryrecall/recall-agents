        ...
        hookSpecificOutput: {
          ...
          decision:
            decision === "allow"
              ...
              : {
                  behavior: "deny",
                  message: message?.trim() || "Denied by Recall",
                },
        },
      })}\n`,
      stderr: "",
...
export function buildNativeHookRelayCommand(params: {
  ...
  const executable = params.executable ?? resolveRecallCliExecutable();
  const argv =
    executable === "recall"
      ? ["recall"]
...
function nativeHookRelayBridgeDir(): string {
  ...
  return path.join(tmpdir(), `recall-native-hook-relays-${uid}`);
}
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  hash.update("recall:native-hook-relay:permission-allow-always:v2");
  hash.update("\0");
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `recall-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveRecallCliExecutable(): string {
  const envPath = process.env.RECALL_CLI_PATH?.trim();
  ...
  if (packageRoot) {
    for (const candidate of [
      path.join(packageRoot, "recall.mjs"),
  ...
  throw new Error("Cannot resolve Recall CLI executable path for native hook relay");
}
