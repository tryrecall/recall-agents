...
type RegisterNativeHookRelayParams = {
  ...
  channelId?: string;
  allowedEvents?: readonly NativeHookRelayEvent[];
  /** Whether this relay should run SteelEngine loop detection from native PreToolUse hooks. */
...
const NATIVE_HOOK_RELAY_STATE_SYMBOL = Symbol.for("steelengine.nativeHookRelay.state");
        ...
        hookSpecificOutput: {
          ...
          decision:
            decision === "allow"
              ...
              : {
                  behavior: "deny",
                  message: message?.trim() || "Denied by SteelEngine",
                },
        },
      })}\n`,
      stderr: "",
...
function buildNativeHookRelayCommandWithStateDatabase(params: {
  ...
  const executable = params.executable ?? resolveSteelEngineCliExecutable();
  const argv =
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  hash.update("steelengine:native-hook-relay:permission-allow-always:v2");
  hash.update("\0");
...
function readNativeHookRelayApprovalMode(rawPayload: JsonValue): "report" | undefined {
  const payload = isJsonObject(rawPayload) ? rawPayload : {};
  return payload.steelengine_approval_mode === "report" ? "report" : undefined;
}
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `steelengine-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveSteelEngineCliExecutable(): string {
  const envPath = process.env.STEELENGINE_CLI_PATH?.trim();
  ...
  throw new Error("Cannot resolve SteelEngine CLI executable path for native hook relay");
}
