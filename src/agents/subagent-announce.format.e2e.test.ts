...
let configOverride: SteelEngineConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: SteelEngineConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/steelengineai/recall-agents/issues/31298
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // STEELENGINE_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("SteelEngine runtime context (internal):");
