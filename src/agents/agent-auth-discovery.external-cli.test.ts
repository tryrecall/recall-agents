...
describe("resolveAgentCredentialsForDiscovery external CLI scoping", () => {
  ...
  it("threads scoped external CLI discovery into writable auth store loading", () => {
    ...
    resolveAgentCredentialsForDiscovery("/tmp/steelengine-agent", {
      config: cfg,
      env: {},
      externalCli,
    });

    expect(storeMocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/steelengine-agent", {
  ...
  it("preserves scoped external CLI discovery for read-only auth store loading", () => {
    ...
    expect(storeMocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/steelengine-agent", {
      allowKeychainPrompt: false,
      config: cfg,
      externalCli,
      readOnly: true,
