...
vi.mock("../model-suppression.js", () => {
  ...
  // extensions/qwen/recall.plugin.json and src/plugins/manifest-model-suppression.ts.
...
import type { RecallConfig, RecallConfigInput } from "../../config/config.js";
...
describe("resolveModel", () => {
  ...
  it("invalidates PI discovery stores when inherited default auth changes", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-model-cache-"));
  ...
  it("invalidates PI discovery stores when implicit main auth changes without config", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-model-cache-state-"));
    vi.stubEnv("RECALL_STATE_DIR", rootDir);
  ...
  it("does not create fallback models from provider overlays alone", () => {
    ...
    } satisfies RecallConfigInput;

    const result = resolveModelForTest(
      "typoProvider",
      "typoed-model",
      "/tmp/agent",
      cfg as unknown as RecallConfig,
    );
