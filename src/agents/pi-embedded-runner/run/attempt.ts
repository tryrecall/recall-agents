...
import { resolveRecallReferencePaths } from "../../docs-path.js";
  ...
  try {
    ...
    const toolsRaw = !shouldConstructTools
      ...
      : (() => {
          ...
          corePluginToolStages.mark("attempt:create-recall-coding-tools");
    ...
    const openClawReferences = await resolveRecallReferencePaths({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop Recall
      // compaction overrides applied in createPreparedEmbeddedPiSettingsManager — same
      // rehydration also restores Pi's auto-compaction (recall#75799), so re-apply
      // both guards.
      applyPiCompactionSettingsFromConfig({
        settingsManager,
        cfg: params.config,
      ...
      // exact Recall-managed registrations so custom tools survive startup and
      ...
      if (isRawModelRun) {
        ...
        // normal Recall agent/tool prompt when `session.prompt()` starts.
      ...
      try {
        ...
        await sessionLockController.withSessionWriteLock(async () => {
          ...
          // double-compaction. See: https://github.com/tryrecall/recall-agents/issues/9282
          ...
          if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
            try {
              activeSessionManager.appendCustomEntry("recall:prompt-error", {
                timestamp: Date.now(),
                runId: params.runId,
                sessionId: params.sessionId,
                provider: params.provider,
      ...
      // See: https://github.com/tryrecall/recall-agents/issues/8643
