...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createRecallCodingToolsMock: UnknownMock;
...
const hoisted = vi.hoisted((): AttemptSpawnWorkspaceHoisted => {
  ...
  const createRecallCodingToolsMock = vi.fn(() => []);
...
vi.mock("../../docs-path.js", () => ({
  resolveRecallReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../pi-tools.js", () => ({
  createRecallCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createRecallCodingToolsMock(options),
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("recall.cache-ttl", data),
  ...
  readLastCacheTtlTimestamp: (
    ...
    for (let index = calls.length - 1; index >= 0; index -= 1) {
      ...
      if (customType !== "recall.cache-ttl") {
        continue;
      }
      const entry = data as
        | {
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createRecallCodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.RECALL_TRAJECTORY;
  if (params.trajectory !== true) {
    process.env.RECALL_TRAJECTORY = "0";
  }
  try {
    return await (
    ...
    if (previousTrajectoryEnv === undefined) {
      delete process.env.RECALL_TRAJECTORY;
    } else {
      process.env.RECALL_TRAJECTORY = previousTrajectoryEnv;
    }
  }
}
