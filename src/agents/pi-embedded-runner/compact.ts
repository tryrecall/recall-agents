...
import { createRecallCodingTools, resolveProcessToolScopeKey } from "../pi-tools.js";
...
async function compactEmbeddedPiSessionDirectOnce(
  ...
  await ensureRecallModelsJson(params.config, agentDir, {
    workspaceDir: resolvedWorkspace,
  });
  ...
  try {
    ...
    const toolsRaw = createRecallCodingTools({
      exec: {
    ...
    const filteredBundledTools = applyFinalEffectiveToolPolicy({
      ...
      // createRecallCodingTools(...) also omit it, so both paths resolve
    ...
    try {
      ...
      // exact Recall-managed registrations so custom tools survive startup.
