...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("recall-bundled-");
  const workspaceDir = await createTempDir("recall-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: RecallConfig) {
