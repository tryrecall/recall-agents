// Verifies SteelEngine-owned tool hooks preserve adjusted params and telemetry.
import type { AgentTool } from "steelengine/plugin-sdk/agent-core";
import {
  installSteelEngineOwnedToolHooks,
  resetSteelEngineOwnedToolHooks,
  textToolResult,
...
describe("SteelEngine-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetSteelEngineOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installSteelEngineOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installSteelEngineOwnedToolHooks();
    ...
    const originalParams = {
      ...
      mediaUrl: "/tmp/steelengine-reply.png",
      provider: "telegram",
      to: "chat-1",
    };
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/steelengine-reply.png"]);
    expect(
    ...
    ).toEqual([
      {
        ...
        mediaUrls: ["/tmp/steelengine-reply.png"],
      },
    ]);
    const [afterPayload, afterContext] = await waitForAfterToolCall(hooks);
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installSteelEngineOwnedToolHooks({ blockReason: "blocked by policy" });
