...
function cfg(mode: "tools" | "off", patterns?: string[]): RecallConfig {
  return {
    logging: {
...
describe("redactTranscriptMessage", () => {
  ...
  it("redacts nested strings in assistant tool-call arguments", () => {
    ...
    expect(argumentsValue.command).toBe("OPENAI_API_KEY=sk-abc…0xyz recall health");
    expect(argumentsValue.env.nested[0]).toBe("token sk-abc…0xyz");
    expect(argumentsValue.count).toBe(1);
    expect(serializedArguments).toContain("recall health");
  ...
  it("redacts structured tool-use input payloads", () => {
    ...
    expect(inputValue.command).toBe("OPENAI_API_KEY=sk-abc…0xyz recall health");
