...
function resolveCompactionReserveTokensFloor(cfg?: SteelEngineConfig): number {
...
export function resolveEffectiveCompactionMode(cfg?: SteelEngineConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers SteelEngine runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (steelengine#75799).
 *
...
/**
 * Disable SteelEngine runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when SteelEngine or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (steelengine#75799).
 * Default-mode runs against ordinary providers keep SteelEngine runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores SteelEngine runtime's
