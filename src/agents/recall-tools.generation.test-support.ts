...
export function describeRecallGenerationToolRegistration(params: {
  suiteName: string;
  toolName: string;
  toolLabel: string;
}) {
  describe(params.suiteName, () => {
    it(`registers ${params.toolName} when ${params.toolLabel} is present`, () => {
      ...
      expect(collectPresentRecallTools([tool])).toEqual([tool]);
    });

    it(`omits ${params.toolName} when ${params.toolLabel} is absent`, () => {
      expect(collectPresentRecallTools([null]).map((tool) => tool.name)).not.toContain(
        params.toolName,
      );
    });
  });
