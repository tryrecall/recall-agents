...
function createAnthropicAuthConfig(params: {
  order?: string[];
  models?: NonNullable<NonNullable<SteelEngineConfig["agents"]>["defaults"]>["models"];
}): SteelEngineConfig {
...
describe("resolveCliRuntimeExecutionProvider", () => {
  ...
  it("does not override an explicit SteelEngine model-runtime policy with CLI auth", () => {
    // Runtime policy is more explicit than profile order, so CLI auth cannot
    // force a model onto the CLI harness when config says SteelEngine.
    expect(
      resolveCliRuntimeExecutionProvider({
        cfg: createAnthropicAuthConfig({
          order: ["anthropic:claude-cli"],
          models: {
            "anthropic/opus-4.7": { agentRuntime: { id: "steelengine" } },
          },
        }),
        provider: "anthropic",
        modelId: "opus-4.7",
