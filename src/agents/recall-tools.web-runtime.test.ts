...
function findTool(name: string, config: RecallConfig) {
  const allTools = createRecallTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: RecallConfig; env?: NodeJS.ProcessEnv }) {
...
describe("recall tools runtime web metadata wiring", () => {
