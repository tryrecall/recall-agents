...
describeCacheLive("pi embedded runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.RECALL_CACHE_TRACE,
      file: process.env.RECALL_CACHE_TRACE_FILE,
      messages: process.env.RECALL_CACHE_TRACE_MESSAGES,
      prompt: process.env.RECALL_CACHE_TRACE_PROMPT,
      system: process.env.RECALL_CACHE_TRACE_SYSTEM,
    };
    process.env.RECALL_CACHE_TRACE = "1";
    process.env.RECALL_CACHE_TRACE_FILE = liveCacheTraceFile;
    process.env.RECALL_CACHE_TRACE_MESSAGES = "0";
    process.env.RECALL_CACHE_TRACE_PROMPT = "0";
    process.env.RECALL_CACHE_TRACE_SYSTEM = "0";
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("RECALL_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("RECALL_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("RECALL_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("RECALL_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("RECALL_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "RECALL_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "RECALL_LIVE_ANTHROPIC_CACHE_MODEL",
