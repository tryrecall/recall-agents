...
const ensureRecallModelsJsonMock = vi.fn(async () => ({ wrote: false }));
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureRecallModelsJson: (...args: Parameters<typeof ensureRecallModelsJsonMock>) =>
        ensureRecallModelsJsonMock(...args),
    };
  });
};
...
beforeAll(async () => {
  ...
  e2eWorkspace = await createEmbeddedPiRunnerTestWorkspace("recall-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureRecallModelsJsonMock.mockReset();
  ensureRecallModelsJsonMock.mockResolvedValue({ wrote: false });
