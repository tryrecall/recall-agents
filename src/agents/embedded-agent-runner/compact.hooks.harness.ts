...
export const resolveAgentHarnessPolicyMock = vi.fn(() => ({ runtime: "steelengine" }));
function createSelectedAgentHarnessMock(params: {
  ...
  const id =
    params.agentHarnessId ?? params.agentHarnessRuntimeOverride ?? configured.runtime ?? "steelengine";
  return {
    id,
...
export const createSteelEngineCodingToolsMock = vi.fn(() => []);
...
export async function loadCompactHooksHarness(): Promise<{
  ...
  vi.doMock("../models-config.js", () => ({
    ensureSteelEngineModelsJson: vi.fn(async () => {}),
  }));
