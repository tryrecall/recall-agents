...
vi.mock("../model-suppression.js", () => {
  ...
  // extensions/qwen/steelengine.plugin.json and src/plugins/manifest-model-suppression.ts.
  ...
  return {
    ...
    buildSuppressedBuiltInModelError: ({
      ...
      if (isUnsupportedXaiMultiAgentModel(provider, id)) {
        return "Unknown model: xai/grok-4.20-multi-agent-0309. SteelEngine does not currently support xAI multi-agent models; choose another xAI model. See https://docs.steelengine.ai/providers/xai.";
      }
      return undefined;
    },
  };
...
import type { SteelEngineConfig, SteelEngineConfigInput } from "../../config/config.js";
...
describe("resolveModel", () => {
  ...
  it("invalidates agent discovery stores when generated plugin catalogs change", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-model-cache-plugin-"));
  ...
  it("invalidates agent discovery stores when inherited default auth changes", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-model-cache-"));
    const agentDir = path.join(rootDir, "agent");
  ...
  it("uses the resolved default agent workspace for cached model discovery", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-model-workspace-"));
    const agentDir = path.join(rootDir, "agent");
  ...
  it("invalidates agent discovery stores when implicit main auth changes without config", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-model-cache-state-"));
    const agentDir = path.join(rootDir, "agents", "worker", "agent");
    const mainAgentDir = path.join(rootDir, "agents", "main", "agent");
    fs.mkdirSync(agentDir, { recursive: true });
    fs.mkdirSync(mainAgentDir, { recursive: true });
    try {
      await withEnvAsync({ STEELENGINE_STATE_DIR: rootDir }, async () => {
        mockDiscoveredModel(discoverModels, {
          provider: "openai",
          modelId: "gpt-5.5",
          templateModel: {
  ...
  it("skips SteelEngine auth and model discovery during dynamic model resolution", async () => {
  ...
  it("prefers user steelengine.json config over the Fireworks manifest for the same id", () => {
  ...
  it("defaults missing model cost before handing models to SteelEngine", () => {
  ...
  it("leaves maxTokens undefined when no configured or catalog value is available (regression: #98295)", () => {
    // Regression for https://github.com/steelengineai/recall-agents/issues/98295.
  ...
  it("does not create fallback models from provider overlays alone", () => {
    ...
    } satisfies SteelEngineConfigInput;
  ...
  it("does not build a configured fallback for unsupported xAI multi-agent models", () => {
    ...
    expect(result.model).toBeUndefined();
    expect(result.error).toBe(
      "Unknown model: xai/grok-4.20-multi-agent-0309. SteelEngine does not currently support xAI multi-agent models; choose another xAI model. See https://docs.steelengine.ai/providers/xai.",
    );
  });
