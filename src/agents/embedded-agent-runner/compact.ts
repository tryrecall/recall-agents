...
import { createSteelEngineCodingTools, resolveProcessToolScopeKey } from "../agent-tools.js";
...
export async function compactEmbeddedAgentSessionDirect(
  ...
  if (paramsBase.modelSelectionLocked === true && lockedHarnessRuntime !== "steelengine") {
    return {
      ok: false,
      compacted: false,
...
async function compactEmbeddedAgentSessionDirectOnce(
  ...
  await ensureSteelEngineModelsJson(params.config, agentDir, {
    workspaceDir: resolvedWorkspace,
  });
  ...
  try {
    ...
    const steelEngineReferences = await resolveSteelEngineReferencePaths({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop SteelEngine
      // compaction overrides applied in createPreparedEmbeddedAgentSettingsManager — same
      // rehydration also restores SteelEngine runtime's auto-compaction (steelengine#75799), so re-apply
      ...
      // exact SteelEngine-managed registrations so custom tools survive startup.
