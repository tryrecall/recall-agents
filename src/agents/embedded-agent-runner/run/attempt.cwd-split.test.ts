...
describe("runEmbeddedAttempt cwd/workspace split", () => {
  ...
  it("uses workspace for bootstrap and cwd for runtime tools", async () => {
    ...
    const taskRepo = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-task-repo-"));
    ...
    const toolsCall = hoisted.createSteelEngineCodingToolsMock.mock.calls[0]?.[0] as
  ...
  it("skips runtime tool construction when the selected model does not support tools", async () => {
    ...
    expect(hoisted.createSteelEngineCodingToolsMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects cwd overrides for sandboxed runs instead of silently ignoring them", async () => {
    ...
    hoisted.resolveSandboxContextMock.mockResolvedValueOnce({
      enabled: true,
      workspaceAccess: "ro",
      workspaceDir: "/tmp/steelengine-sandbox-copy",
    });
  ...
  it("runs a managed worktree when sandbox workspace and cwd match", async () => {
    const worktree = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-sandbox-worktree-"));
    ...
    expect(hoisted.createSteelEngineCodingToolsMock).toHaveBeenCalledWith(
