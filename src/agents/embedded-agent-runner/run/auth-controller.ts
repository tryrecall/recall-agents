...
export function createEmbeddedRunAuthController(params: {
  ...
  const applyApiKeyInfo = async (candidate?: string, attemptIndex?: number): Promise<void> => {
    ...
    if (!apiKeyInfo.apiKey) {
      ...
      // SteelEngine runtime's authStorage considers the provider authenticated.  Try
      ...
      // sentinel so SteelEngine runtime's hasConfiguredAuth() passes and the AWS SDK default
