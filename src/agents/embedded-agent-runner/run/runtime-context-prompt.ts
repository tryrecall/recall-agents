...
import {
  ...
  STEELENGINE_RUNTIME_EVENT_HEADER,
} from "../../internal-runtime-context.js";
import type { CurrentInboundPromptContext } from "./params.js";

const STEELENGINE_RUNTIME_EVENT_USER_PROMPT = "Continue the SteelEngine runtime event.";
...
export type RuntimeContextCustomMessage = {
  ...
  content: string;
  display: false;
  details: { source: "steelengine-runtime-context"; runtimeContextCarrier: true };
  timestamp: number;
};
...
export function resolveRuntimeContextPromptParts(params: {
  ...
  if (!prompt.trim()) {
    return runtimeContext
      ? {
          prompt: STEELENGINE_RUNTIME_EVENT_USER_PROMPT,
          ...(modelPromptText.trim() && modelPromptText !== STEELENGINE_RUNTIME_EVENT_USER_PROMPT
            ? { modelPrompt: modelPromptText }
            : {}),
          runtimeContext,
          runtimeOnly: true,
