  ...
  it("injects workspace identity context", () => {
    ...
    const result = buildAttemptSystemPrompt({
      ...
      embeddedSystemPrompt: {
        ...
        userTimezone: "UTC",
        contextFiles: [
          { path: "/tmp/steelengine/SOUL.md", content: "SOUL_CONTEXT_MARKER" },
          { path: "/tmp/steelengine/IDENTITY.md", content: "IDENTITY_CONTEXT_MARKER" },
          { path: "/tmp/steelengine/USER.md", content: "USER_CONTEXT_MARKER" },
        ],
      },
    ...
    expect(result.systemPrompt).toContain("## /tmp/steelengine/SOUL.md");
    expect(result.systemPrompt).toContain("SOUL_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/steelengine/IDENTITY.md");
    expect(result.systemPrompt).toContain("IDENTITY_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/steelengine/USER.md");
  ...
  it("preserves bootstrap Project Context", () => {
    const result = buildAttemptSystemPrompt({
      ...
      embeddedSystemPrompt: {
        ...
        contextFiles: [
          {
            path: "/tmp/steelengine/BOOTSTRAP.md",
            content: "Reply with BOOTSTRAP_OK.",
          },
          {
            path: "/tmp/steelengine/SOUL.md",
            content: "SOUL_CONTEXT_MARKER",
          },
          {
            path: "/tmp/steelengine/IDENTITY.md",
            content: "IDENTITY_CONTEXT_MARKER",
          },
          {
            path: "/tmp/steelengine/USER.md",
    ...
    expect(result.systemPrompt).toContain("## /tmp/steelengine/USER.md");
    expect(result.systemPrompt).toContain("USER_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/steelengine/BOOTSTRAP.md");
