...
describe("append-only late media (issue #99495)", () => {
  it("keeps every sent fingerprint stable and appends one late-media turn", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-99495-boundary-"));
    const target = {
      agentId: "main",
      cwd: dir,
...
function isCarrier(message: unknown): boolean {
  return Boolean(
    ...
    (message as { customType?: unknown }).customType === STEELENGINE_RUNTIME_CONTEXT_CUSTOM_TYPE,
  );
}
