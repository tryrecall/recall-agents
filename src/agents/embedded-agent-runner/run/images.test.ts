...
describe("detectImageReferences", () => {
  ...
  it("ignores SteelEngine CLI image cache paths from prior prompt transcripts", () => {
    ...
    const refs = detectImageReferences(
      [
        '<system-reminder>Called the Read tool with {"file_path":"/Users/ada/.steelengine/workspace/.steelengine-cli-images/stale.png"}</system-reminder>',
  ...
  it("ignores temporary SteelEngine CLI image cache paths", () => {
    expectNoImageReferences(
      `Prior turn wrote ${path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-cli-images", "stale.jpg")}`,
    );
    expectNoImageReferences(
      `[media attached: ${path.join(resolvePreferredSteelEngineTmpDir(), "steelengine-cli-images", "stale.jpg")} (image/jpeg)]`,
    );
    expectNoImageReferences(
      `Prior turn wrote ${path.join(os.tmpdir(), "steelengine", "steelengine-cli-images", "stale.jpg")}`,
    );
    expectNoImageReferences(
      `Prior turn wrote ${path.join(os.tmpdir(), "steelengine-501", "steelengine-cli-images", "stale.jpg")}`,
    );
  });

  it("ignores file URLs into the SteelEngine CLI image cache", () => {
    const stalePath = path.join(os.tmpdir(), "steelengine", "steelengine-cli-images", "stale.png");
  ...
  it("detects normal user image paths in similarly named directories", () => {
    expect(detectImageReferences("/workspace/steelengine-cli-images/current.png")).toStrictEqual([
      {
        raw: "/workspace/steelengine-cli-images/current.png",
        type: "path",
        resolved: "/workspace/steelengine-cli-images/current.png",
      },
    ]);
  });
...
[media attached 1/2: /Users/tyleryust/.steelengine/media/IMG_6430.jpeg (image/jpeg)]
[media attached 2/2: /Users/tyleryust/.steelengine/media/IMG_6431.jpeg (image/jpeg)]
what about these images?`,
    ...
    expect(refs).toStrictEqual([
      {
        raw: "/Users/tyleryust/.steelengine/media/IMG_6430.jpeg",
        type: "path",
        resolved: "/Users/tyleryust/.steelengine/media/IMG_6430.jpeg",
      },
      {
        raw: "/Users/tyleryust/.steelengine/media/IMG_6431.jpeg",
        type: "path",
        resolved: "/Users/tyleryust/.steelengine/media/IMG_6431.jpeg",
      },
    ]);
  });
...
what is this?`);
  ...
  it("handles paths with spaces in filename", () => {
    ...
    const ref =
      expectSingleImageReference(`[media attached: /Users/test/.steelengine/media/ChatGPT Image Apr 21, 2025.png (image/png) | https://example.com/same.png]
what is this?`);

    expect(ref).toStrictEqual({
      raw: "/Users/test/.steelengine/media/ChatGPT Image Apr 21, 2025.png",
      type: "path",
      resolved: "/Users/test/.steelengine/media/ChatGPT Image Apr 21, 2025.png",
    });
  });
...
describe("loadImageFromRef", () => {
  it("hydrates managed inbound media URIs before workspace path resolution", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-native-image-uri-"));
  ...
  it("hydrates sandbox-staged inbound media URIs", async () => {
    const sandboxRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-native-image-sbx-uri-"));
  ...
  it("allows sandbox-validated host paths outside default media roots", async () => {
    ...
    const sandboxParent = await fs.mkdtemp(path.join(homeDir, "steelengine-sandbox-image-"));
    try {
...
describe("detectAndLoadPromptImages", () => {
  ...
  it("skips generated media-note refs already supplied inline", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-native-image-dedupe-"));
  ...
  it("classifies prompt and attachment refs while preserving mixed attachment order", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-native-image-order-"));
    const inboundDir = path.join(stateDir, "media", "inbound");
    await fs.mkdir(inboundDir, { recursive: true });
  ...
  it("blocks prompt image refs outside workspace when sandbox workspaceOnly is enabled", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-native-image-sandbox-"));
  ...
  it("loads managed inbound absolute paths when workspaceOnly is enabled", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-native-image-managed-"));
    const workspaceDir = path.join(stateDir, "workspace-agent");
    const inboundDir = path.join(stateDir, "media", "inbound");
    await fs.mkdir(workspaceDir, { recursive: true });
    await fs.mkdir(inboundDir, { recursive: true });
