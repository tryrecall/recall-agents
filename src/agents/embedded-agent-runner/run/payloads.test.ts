...
describe("buildEmbeddedRunPayloads tool-error warnings", () => {
  ...
  it("does not replay raw-looking accumulated tool output when final answer text is available", () => {
    const payloads = buildPayloads({
      assistantTexts: [
        "/root/steelengine/packages/gateway-protocol/src/schema/protocol-schemas.ts:181:  PluginControlUiDescriptorSchema,",
  ...
  it("surfaces heartbeat exec tool output details when the task run fails", () => {
    const payloads = buildPayloads({
      lastToolError: {
        toolName: "exec",
        meta: "show last 20 lines of ~/.steelengine/workspace/memory/2026-06-04.md",
        error:
          "tail: cannot open '/home/user/.steelengine/workspace/memory/2026-06-04.md' for reading: No such file or directory",
      },
  ...
  it("strips NO_REPLY text but keeps voice media directives", () => {
    const payloads = buildPayloads({
      assistantTexts: ["NO_REPLY\nMEDIA:/tmp/steelengine/tts-a/voice-a.opus\n[[audio_as_voice]]"],
    });

    expect(payloads).toHaveLength(1);
    expect(payloads[0]?.mediaUrl).toBe("/tmp/steelengine/tts-a/voice-a.opus");
    expect(payloads[0]?.mediaUrls).toEqual(["/tmp/steelengine/tts-a/voice-a.opus"]);
