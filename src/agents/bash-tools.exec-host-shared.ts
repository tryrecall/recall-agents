...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.recall/exec-approvals.json both apply.",
    "Fix one of these:",
    ...
    'Tip: run "recall doctor" and "recall approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
