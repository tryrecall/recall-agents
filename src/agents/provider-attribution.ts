...
import { listSteelEnginePluginManifestMetadata } from "../plugins/manifest-metadata-scan.js";
...
const STEELENGINE_ATTRIBUTION_PRODUCT = "SteelEngine";
const STEELENGINE_ATTRIBUTION_ORIGINATOR = "steelengine";
...
function formatSteelEngineUserAgent(version: string): string {
  return `${STEELENGINE_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
function collectManifestProviderEndpoints(): ManifestProviderEndpointCacheEntry[] {
  const entries: ManifestProviderEndpointCacheEntry[] = [];
  for (const { manifest } of listSteelEnginePluginManifestMetadata()) {
...
function resolveProviderAttributionIdentity(
  ...
  return {
    product: STEELENGINE_ATTRIBUTION_PRODUCT,
...
function buildOpenRouterAttributionPolicy(
  ...
  return {
    ...
    reviewNote: "Documented app attribution headers. Verified in SteelEngine runtime wrapper.",
    ...identity,
    headers: {
...
function buildNvidiaAttributionPolicy(
  ...
  return {
    ...
    headers: {
      "X-BILLING-INVOKE-ORIGIN": STEELENGINE_ATTRIBUTION_PRODUCT,
    },
  };
}
...
function buildGoogleAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      "x-goog-api-client": `${STEELENGINE_ATTRIBUTION_ORIGINATOR}/${identity.version}`,
    },
  };
}
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: STEELENGINE_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatSteelEngineUserAgent(identity.version),
    },
  };
}
...
function buildXaiAttributionPolicy(
  ...
  return {
    ...
    hook: "request-headers",
    reviewNote:
      "xAI api.x.ai accepts a standard steelengine User-Agent. Companion originator/version headers mirror the OpenAI attribution shape for consistency; they are not validated against an xAI-specific spec and are expected to be ignored by xAI's OpenAI-compatible surface.",
    ...identity,
    headers: {
      originator: STEELENGINE_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
