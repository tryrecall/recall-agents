...
describe("tool display details", () => {
  ...
  it("summarizes tool-search code targets from described tool ids", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: "const tool = await steelengine.tools.describe('steelengine:core:exec'); return await steelengine.tools.call(tool.id, { command: 'echo hi' });",
      }),
    ).toEqual({
      toolName: "steelengine:core:exec",
  ...
  it("normalizes direct tool-search catalog ids to native display names and args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await steelengine.tools.call("steelengine:core:exec", { command: "echo hi" });',
      }),
    ).toEqual({
      toolName: "steelengine:core:exec",
      displayToolName: "exec",
  ...
  it("preserves JS numeric literals in tool-search call args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await steelengine.tools.call("web_search", { query: "SteelEngine", count: 1e3, limit: +3, threshold: .5 });',
      })?.displayArgs,
    ).toEqual({
      query: "SteelEngine",
      count: 1000,
  ...
  it("formats web_search query with quotes", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        name: "web_search",
        args: { query: "SteelEngine docs", count: 3 },
      }),
    );

    expect(detail).toBe('for "SteelEngine docs" (top 3)');
  });
  ...
  it("formats Parallel's native objective + search_queries shape", () => {
    expect(
      ...
      'for "Find the SteelEngine repository on GitHub", "steelengine github", "steelengine repository" (top 5)',
    );
  });
  ...
  it("summarizes exec commands with context", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        ...
        args: {
          command:
            "set -euo pipefail\ngit -C /Users/adityasingh/.steelengine/workspace status --short | head -n 3",
          workdir: "/Users/adityasingh/.steelengine/workspace",
        },
      }),
    );
  ...
  it("uses compact workspace markers for common workspace paths", () => {
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git fetch", workdir: "/Users/peter/mantis-workspace/steelengine" },
          detailMode: "explain",
        }),
      ),
    ...
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git status", workdir: "/Users/peter/Projects/steelengine" },
          detailMode: "explain",
        }),
      ),
      ...
      formatToolDetail(
        resolveToolDisplay({
          ...
          args: {
            ...
            workdir: "/root/.steelengine/sandboxes/agent-clawsweeper-sandbox-discor-766423d0",
          },
          detailMode: "explain",
        }),
      ),
  ...
  it("omits bash and exec names from compact tool summaries", () => {
    ...
    expect(
      formatToolSummary(
        resolveToolDisplay({
          name: "web_search",
          args: { query: "SteelEngine docs" },
        }),
      ),
    ).toBe('🔎 Web Search: for "SteelEngine docs"');
  });
    ...
    const detail = formatToolDetail(
      resolveToolDisplay({
        ...
        args: {
          ...
          workdir: "/Users/example/.steelengine/workspace",
        },
        detailMode: "explain",
      }),
    );
  ...
  it("keeps heredoc body separators out of top-level stage splitting", () => {
    const stages = splitTopLevelStages(
      [
        "mkdir -p .steelengine/tmp/farm-notices",
        "cat > .steelengine/tmp/farm-notices/ventura.txt <<'EOF'",
