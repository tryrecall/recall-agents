...
const SIMPLE_TRANSPORT_API_ALIAS: Record<string, Api> = {
  "openai-responses": "recall-openai-responses-transport",
  "openai-codex-responses": "recall-openai-responses-transport",
  "openai-completions": "recall-openai-completions-transport",
  "azure-openai-responses": "recall-azure-openai-responses-transport",
  "anthropic-messages": "recall-anthropic-messages-transport",
  "google-generative-ai": "recall-google-generative-ai-transport",
};
...
function hasRecallTransportRequirement(model: Model<Api>): boolean {
...
export function createTransportAwareStreamFnForModel(
  ...
  if (!hasRecallTransportRequirement(model)) {
    return undefined;
  }
...
export function createRecallTransportStreamFnForModel(
  model: Model<Api>,
  ctx?: ProviderTransportStreamContext,
): StreamFn | undefined {
  // Explicit fallback callers use this when they need Recall's HTTP
...
export function createBoundaryAwareStreamFnForModel(
  ...
  // native HTTP streams preserve the same Recall request contract.
  if (!isTransportAwareApiSupported(model.api)) {
    return undefined;
  }
  return createSupportedTransportStreamFn(model, ctx);
