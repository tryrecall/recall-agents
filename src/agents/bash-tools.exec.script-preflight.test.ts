...
import { __setFsSafeTestHooksForTest } from "@recall/fs-safe/test-hooks";
...
const parseRecallChannelsLoginShellCommand = testing.parseRecallChannelsLoginShellCommand;
...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("recall-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive Recall channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", () => {
    expect(
      parseRecallChannelsLoginShellCommand("recall channels login --channel whatsapp"),
    ).toBe(true);
    expect(
      parseRecallChannelsLoginShellCommand(
        "pnpm exec recall channels login --channel whatsapp --verbose",
      ),
    ).toBe(true);
    expect(parseRecallChannelsLoginShellCommand("recall channels status --deep")).toBe(false);
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-recall-channel-login", {
        command: "recall channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive Recall channel login commands/);
    await expect(
      tool.execute("call-wrapped-recall-channel-login", {
        command: "sudo -u recall bash -lc 'recall channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive Recall channel login commands/);
    await expect(
      tool.execute("call-clustered-sudo-channel-login", {
        command: "sudo -EH bash -lc 'recall channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive Recall channel login commands/);
    await expect(
      tool.execute("call-deep-env-channel-login", {
        command: "env env env env env env recall channels login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive Recall channel login commands/);
    await expect(
      tool.execute("call-env-s-trailing-channel-login", {
        command: "env -S 'recall channels' login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive Recall channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("recall-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("recall-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("recall-exec-preflight-nonblock-", async (tmp) => {
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("recall-exec-preflight-win-", async (tmp) => {
      await fs.writeFile(path.join(tmp, "bad.py"), "payload = $DM_JSON", "utf-8");
