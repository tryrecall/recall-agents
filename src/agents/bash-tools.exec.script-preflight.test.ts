...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("steelengine-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive SteelEngine channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", async () => {
    await expect(
      detectUnsafeExecControlShellCommand("steelengine channels login --channel whatsapp"),
    ).resolves.toBe("channel-login");
    expect(
      await detectUnsafeExecControlShellCommand(
        "pnpm exec steelengine channels login --channel whatsapp --verbose",
      ),
    ).toBe("channel-login");
    await expect(
      detectUnsafeExecControlShellCommand("steelengine channels status --deep"),
    ).resolves.toBeNull();
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-steelengine-channel-login", {
        command: "steelengine channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive SteelEngine channel login commands/);
    await expect(
      tool.execute("call-wrapped-steelengine-channel-login", {
        command: "sudo -u steelengine bash -lc 'steelengine channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive SteelEngine channel login commands/);
    await expect(
      tool.execute("call-clustered-sudo-channel-login", {
        command: "sudo -EH bash -lc 'steelengine channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive SteelEngine channel login commands/);
    await expect(
      tool.execute("call-deep-env-channel-login", {
        command: "env env env env env env steelengine channels login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive SteelEngine channel login commands/);
    await expect(
      tool.execute("call-env-s-trailing-channel-login", {
        command: "env -S 'steelengine channels' login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive SteelEngine channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("steelengine-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("steelengine-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("steelengine-exec-preflight-nonblock-", async (tmp) => {
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("steelengine-exec-preflight-win-", async (tmp) => {
      await fs.writeFile(path.join(tmp, "bad.py"), "payload = $DM_JSON", "utf-8");
...
describe("exec interpreter heuristics ReDoS guard", () => {
  it("does not hang on long commands with VAR=value assignments and whitespace-heavy text", async () => {
    ...
    const command = `ACCESS_TOKEN=$(__steelengine_missing_redos_guard__)\nprintf '%s' '${htmlBlock}' >/dev/null`;
