...
export function readEmbeddedGatewayTokenForTest(
  ...
  return isEnvironmentFileOnlySource(command?.environmentValueSources?.RECALL_GATEWAY_TOKEN)
    ? undefined
    : normalizeOptionalString(command?.environment?.RECALL_GATEWAY_TOKEN);
}
