...
import { resolvePrimaryStringValue } from "@steelengine/normalization-core/string-coerce";
...
function isUpdateInProgress(): boolean {
  const value = process.env.STEELENGINE_UPDATE_IN_PROGRESS;
 ...
 */
export function stripUnknownConfigKeys(config: SteelEngineConfig): {
  ...
  const parsed = SteelEngineSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }
...
export function noteOpencodeProviderOverrides(cfg: SteelEngineConfig): void {
...
export function noteImplicitFallbackClobberWarnings(cfg: SteelEngineConfig): void {
