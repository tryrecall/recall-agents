...
describe("buildGatewayRuntimeHints", () => {
  ...
  it("surfaces suspicious systemd cgroup hygiene with inspection commands", () => {
    expect(
      ...
      "Run: systemctl --user show steelengine-gateway.service -p KillMode -p TasksCurrent -p MemoryCurrent -p MainPID",
      "Run: systemd-cgls --user-unit steelengine-gateway.service",
      "After reviewing service settings, run: steelengine gateway restart",
    ]);
  });
  ...
  it("guides recovery when systemd hit its restart start limit (crash loop)", () => {
    ...
    expect(text).toContain("steelengine gateway restart");
