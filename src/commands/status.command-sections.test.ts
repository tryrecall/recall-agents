...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:recall security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:recall security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:recall status --all",
      "  Need to debug live? cmd:recall logs --follow",
      "  Fix reachability first: cmd:recall gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:recall devices approve req-123)",
      "muted(Fallback: cmd:recall devices approve --latest)",
      "muted(Inspect: cmd:recall devices list)",
    ]);
  });
