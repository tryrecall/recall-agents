...
import { readConfigFileSnapshot, writeConfigFile, type RecallConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("recall config set gateway.auth.mode token")} or ${formatCliCommand("recall config set gateway.auth.mode password")}.`,
  ].join(" ");
}
