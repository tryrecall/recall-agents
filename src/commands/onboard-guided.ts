...
export type GuidedOnboardingDeps = {
  ...
  persistRiskAcknowledgement?: (config: SteelEngineConfig) => Promise<void>;
   ...
   * "chat" preserves the legacy handoff into the SteelEngine system-agent chat —
...
async function persistRiskAcknowledgement(config: SteelEngineConfig): Promise<void> {
...
async function runGuidedOnboardingFlow(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      t("wizard.guided.invalidConfigRepair", {
        fixCommand: formatCliCommand("steelengine doctor --fix"),
        inspectCommand: formatCliCommand("steelengine config validate"),
      }),
    );
    runtime.exit(1);
    return null;
  ...
  // Inference is the only prerequisite for SteelEngine. Use the caller's or
  // current default workspace as isolated probe context; SteelEngine owns any
  ...
  if (alreadyConfigured) {
    ...
    // would, then hand off to the hatch instead of parking in the SteelEngine chat.
    const applySetup =
    ...
    try {
      ...
      if (!appliedSnapshot.valid) {
        throw new Error("Setup wrote an invalid SteelEngine config.");
      }
  ...
  if (wantsDiscovery) {
    ...
    if (recommendedConfig !== persistedConfig) {
      ...
      if (!latestSnapshot.valid) {
        throw new Error("App recommendations could not update an invalid SteelEngine config.");
      }
