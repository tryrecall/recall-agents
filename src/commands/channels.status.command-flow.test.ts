...
const mocks = vi.hoisted(() => ({
  ...
  readConfigFileSnapshot: vi.fn(async () => ({ path: "/tmp/recall.json" })),
  ...
  resolveMissingOfficialExternalChannelPluginRepairHint: ({ channelId }: { channelId: string }) =>
    mocks.missingOfficialExternalChannels.has(channelId)
      ? {
          ...
          channelId,
          label: "Feishu",
          installSpec: "@recall/feishu",
          installCommand: "recall plugins install @recall/feishu",
          doctorFixCommand: "recall doctor --fix",
          repairHint:
            "Install the official external plugin with: recall plugins install @recall/feishu, or run: recall doctor --fix.",
        }
      : null,
}));
...
describe("channelsStatusCommand SecretRef fallback flow", () => {
  ...
  it("shows missing official external plugin repair hints in config-only output", async () => {
    ...
    expect(joined).toContain(
      "Feishu: Install the official external plugin with: recall plugins install @recall/feishu, or run: recall doctor --fix.",
    );
  });
