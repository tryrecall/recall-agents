...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByRecall ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByRecall ? "installed · " : "";
  ...
  runtime.log(theme.heading("Recall status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`recall devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("recall devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("recall devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("recall security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("recall security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.recall.ai/faq");
  runtime.log("Troubleshooting: https://docs.recall.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("recall status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("recall logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("recall status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("recall gateway probe")}`);
  }
}
