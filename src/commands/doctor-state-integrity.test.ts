...
type EnvSnapshot = {
  HOME?: string;
  RECALL_HOME?: string;
  RECALL_STATE_DIR?: string;
  RECALL_OAUTH_DIR?: string;
  RECALL_AGENT_DIR?: string;
...
function captureEnv(): EnvSnapshot {
  return {
    ...
    RECALL_OAUTH_DIR: process.env.RECALL_OAUTH_DIR,
...
function setupSessionState(cfg: RecallConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: RecallConfig) {
...
async function runStateIntegrityText(cfg: RecallConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "recall-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.RECALL_HOME = tempHome;
    ...
    fs.mkdirSync(process.env.RECALL_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when RECALL_OAUTH_DIR is explicitly configured", async () => {
    process.env.RECALL_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: RecallConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(hasRepairPromptMessage(confirmRuntimeRepair, "Create OAuth dir at")).toBe(true);
  ...
  it("does not warn when RECALL_AGENT_DIR points at the live compatibility agent dir", async () => {
    ...
    process.env.RECALL_AGENT_DIR = legacyAgentDir;
  ...
  it("warns about tombstoned subagent restart recovery sessions", async () => {
    ...
    expect(text).toContain("recall tasks maintenance --apply");
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.RECALL_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        ...
        process.env.RECALL_HOME = symlinkHome;
        process.env.RECALL_STATE_DIR = path.join(symlinkHome, ".recall");
  ...
  it("prints recall-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: RecallConfig = {};
    writeSessionStore(cfg, {
      "agent:main:main": {
    ...
    expect(text).toMatch(/recall sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/recall sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /recall sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
  ...
  it("does not let synthetic heartbeat metadata override mixed transcript history", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-heartbeat-main-mixed-"));
    try {
  ...
  it("does not let heartbeat-looking routing metadata skip mixed transcript checks", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-heartbeat-main-route-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("does not classify transcripts with real user activity after 400 heartbeat messages", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-heartbeat-main-cap-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
  ...
  it("keeps the heartbeat main-session helper conservative", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-heartbeat-main-helper-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("moves store entries and clears matching TUI pointers without touching others", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-tui-pointer-clear-"));
    try {
