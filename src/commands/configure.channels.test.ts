...
function expectUnknownChannelRemovalPrompt(unsafeChannel: string, label: string) {
  ...
  expect(confirmArg().message).toBe(
    `Delete ${label} configuration from ~/.steelengine/steelengine.json?`,
  );
...
describe("removeChannelConfigWizard", () => {
  ...
  it("lists configured channels from steelengine.json even when no plugins are loaded", async () => {
  ...
  it("deletes the selected channel block from steelengine.json", async () => {
    ...
    expect(confirmArg().message).toBe(
      "Delete Telegram configuration from ~/.steelengine/steelengine.json?",
    );
  ...
  it("deletes a real channel block named done", async () => {
    ...
    expect(confirmArg().message).toBe("Delete done configuration from ~/.steelengine/steelengine.json?");
  ...
  it("sanitizes known channel labels before rendering prompts", async () => {
    ...
    expect(confirmArg().message).toBe(
      "Delete Telegram\\nBot configuration from ~/.steelengine/steelengine.json?",
    );
    expect(note).toHaveBeenCalledWith(
