...
function expectUnknownChannelRemovalPrompt(unsafeChannel: string, label: string) {
  ...
  expect(confirmArg().message).toBe(
    `Delete ${label} configuration from ~/.tryrecall/recall-agents.json?`,
  );
...
describe("removeChannelConfigWizard", () => {
  ...
  it("lists configured channels from recall.json even when no plugins are loaded", async () => {
  ...
  it("deletes the selected channel block from recall.json", async () => {
    ...
    expect(confirmArg().message).toBe(
      "Delete Telegram configuration from ~/.tryrecall/recall-agents.json?",
    );
  ...
  it("deletes a real channel block named done", async () => {
    ...
    expect(confirmArg().message).toBe("Delete done configuration from ~/.tryrecall/recall-agents.json?");
  ...
  it("sanitizes known channel labels before rendering prompts", async () => {
    ...
    expect(confirmArg().message).toBe(
      "Delete Telegram\\nBot configuration from ~/.tryrecall/recall-agents.json?",
    );
    expect(note).toHaveBeenCalledWith(
