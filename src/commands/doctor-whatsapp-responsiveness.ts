...
function isLocalTuiCommand(command: string): boolean {
  ...
  if (executable === "recall-tui") {
    return true;
  }
  return executable === "recall" && LOCAL_TUI_SUBCOMMANDS.has(argv[1] ?? "");
}
...
function hasWhatsappEnabled(cfg: RecallConfig): boolean {
...
export async function noteWhatsappResponsivenessHealth(params: {
  ...
  if (gatewayDegraded && tuiProcesses.length > 0) {
    ...
    if (params.shouldRepair) {
      ...
      warnings.push(
        `Fix: close those TUI sessions, or run ${formatCliCommand("recall doctor --fix")}.`,
      );
    }
  }
