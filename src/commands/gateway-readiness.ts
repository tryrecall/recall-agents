...
function printGatewayNotReadyHints(runtime: RuntimeEnv, reason: string): void {
  runtime.log(reason);
  runtime.log("Run `steelengine gateway status --deep` for details.");
  runtime.log("Run `steelengine gateway start` to start a managed gateway.");
  runtime.log("Run `steelengine gateway run` for a foreground gateway.");
}
...
export async function ensureGatewayReadyForOperation(
  ...
  const prompt = shouldInstall
    ? `Gateway is not installed. Install and start it now so SteelEngine can ${options.operation}?`
    : `Gateway is not running. Start it now so SteelEngine can ${options.operation}?`;
