...
describe("status.scan.config-shared", () => {
  beforeEach(() => {
    ...
    mocks.resolveConfigPath.mockReturnValue(
      `/tmp/recall-status-scan-config-shared-missing-${process.pid}.json`,
    );
  });
  ...
  it("adds a status diagnostic for gateway token source conflicts", async () => {
    ...
    const result = await loadStatusScanCommandConfig({
      ...
      readBestEffortConfig,
      resolveConfig,
      env: { VITEST: "true", RECALL_GATEWAY_TOKEN: "env-token" },
      allowMissingConfigFastPath: true,
    });

    expect(result.secretDiagnostics).toEqual([
      "RECALL_GATEWAY_TOKEN conflicts with gateway.auth.token: Remove RECALL_GATEWAY_TOKEN from the shell, ~/.recall/.env, or launchctl env if gateway.auth.token is intended, or point gateway.auth.token at ${RECALL_GATEWAY_TOKEN} if the env var should be canonical.",
    ]);
  });
  ...
  it("does not add a status diagnostic when config uses RECALL_GATEWAY_TOKEN", async () => {
    const sourceConfig = {
