...
const mocks = vi.hoisted(() => ({
  ...
  resolveSteelEngineWrapperPath: vi.fn(),
...
vi.mock("../daemon/program-args.js", () => ({
  ...
  resolveSteelEngineWrapperPath: mocks.resolveSteelEngineWrapperPath,
}));
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { STEELENGINE_PORT: "3000" },
  } = params;
  const workingDirectory = Object.hasOwn(params, "workingDirectory")
    ? params.workingDirectory
    : "/Users/me";
  mocks.resolvePreferredNodePath.mockResolvedValue("/opt/node");
  mocks.resolveSteelEngineWrapperPath.mockImplementation(async (value: string | undefined) =>
...
async function buildPluginConfigExecSecretRefPlan(home: string) {
  mockNodeGatewayPlanFixture({ serviceEnvironment: { STEELENGINE_PORT: "3000" } });
...
describe("buildGatewayInstallPlan", () => {
  ...
  // Prevent tests from reading the developer's real ~/.steelengine/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ STEELENGINE_PORT: "3000" });
  ...
  it("adds the active steelengine command bin directory to the managed service PATH", async () => {
    mockNodeGatewayPlanFixture();
    const originalArgv = process.argv;
    const steelengineBinPath = path.join(isolatedHome, ".npm-global", "bin", "steelengine");
    process.argv = ["node", steelengineBinPath, "gateway", "install"];
    ...
    ).toStrictEqual(["/opt/homebrew/opt/node/bin", path.dirname(steelengineBinPath)]);
  });
  ...
  it("uses the state dir as the default macOS launchd working directory", async () => {
    ...
    expect(plan.workingDirectory).toBe(path.join(isolatedHome, ".steelengine"));
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
  ...
  it("passes STEELENGINE_WRAPPER through program args and managed service env", async () => {
    const wrapperPath = path.resolve("/usr/local/bin/steelengine-doppler");
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        STEELENGINE_PORT: "3000",
    ...
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
    expect(
      firstMockArg(mocks.buildServiceEnvironment, "buildServiceEnvironment").env?.STEELENGINE_WRAPPER,
    ).toBe(wrapperPath);
    expect(plan.environment.STEELENGINE_WRAPPER).toBe(wrapperPath);
  });

  it("clears a Windows wrapper env that points at the generated gateway.cmd script", async () => {
    const selfWrapperPath = path.join(isolatedHome, ".steelengine", "gateway.cmd");
    const warn = vi.fn();
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        STEELENGINE_PORT: "3000",
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        STEELENGINE_WRAPPER: selfWrapperPath,
      }),
      port: 3000,
      runtime: "node",
      platform: "win32",
    ...
    expect(plan.environment.STEELENGINE_WRAPPER).toBeUndefined();
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Ignoring STEELENGINE_WRAPPER because it points to the Windows task script",
      ),
    );
  });
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.STEELENGINE_PORT).toBe("3000");
    expect(plan.environment.STEELENGINE_SERVICE_MANAGED_ENV_KEYS).toBe(
  ...
  it("renders config env SecretRefs as file-backed managed values on Linux", async () => {
    ...
    expect(plan.environment.STEELENGINE_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
  ...
  it("keeps config env SecretRefs managed when auth profiles reuse the key", async () => {
    ...
    expect(plan.environment.STEELENGINE_SERVICE_MANAGED_ENV_KEYS).toBe("OPENAI_API_KEY");
  });
  ...
  it("includes passEnv values for configured exec SecretRef providers", async () => {
    ...
    expect(plan.environment.STEELENGINE_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("allows safe inherited passEnv names while blocking dangerous exec SecretRef env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        BASH_ENV: "/tmp/steelengine-test-bashenv",
        XDG_CONFIG_HOME: "/tmp/steelengine-test-xdg-home",
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        STEELENGINE_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
    ...
    expect(plan.environment.STEELENGINE_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.STEELENGINE_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("does not inline config env SecretRef values already backed by state-dir dotenv", async () => {
    await writeStateDirDotEnv("DISCORD_BOT_TOKEN=discord-dotenv-token\n", {
      stateDir: path.join(isolatedHome, ".steelengine"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        STEELENGINE_PORT: "3000",
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("tracks .env vars with config while preserving service precedence", async () => {
    await writeStateDirDotEnv(
      ...
      {
        stateDir: path.join(tmpDir, ".steelengine"),
      },
    );
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
  ...
  it("retains .env values for macOS LaunchAgent env SecretRefs", async () => {
    ...
    expect(plan.environment.STEELENGINE_SERVICE_MANAGED_ENV_KEYS).toBe("MINIMAX_API_KEY");
  });
  ...
  it("retains config SecretRef env values for macOS LaunchAgent env files", async () => {
    ...
    expect(plan.environment.STEELENGINE_SERVICE_MANAGED_ENV_KEYS).toBe("TELEGRAM_DEFAULT_BOTTOKEN");
  });
  ...
  it("retains existing generated env-file SecretRef values for macOS LaunchAgent regeneration", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        STEELENGINE_SERVICE_MANAGED_ENV_KEYS:
  ...
  it("does not retain config env values for macOS LaunchAgent env files", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        STEELENGINE_SERVICE_MANAGED_ENV_KEYS: "BRAVE_API_KEY",
      },
      existingEnvironmentValueSources: {
        BRAVE_API_KEY: "file",
        STEELENGINE_SERVICE_MANAGED_ENV_KEYS: "inline",
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.STEELENGINE_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        STEELENGINE_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.STEELENGINE_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    expect(plan.environment.STEELENGINE_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.STEELENGINE_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:steelengine|steelengine)( --profile isolated)? gateway install/,
    );
  });
});
...
describe("collectPreservedExistingServiceEnvVars — operator opt-in allowlist", () => {
  ...
  it("continues to drop stale STEELENGINE_ALLOW_ROOT", async () => {
    const result = await buildEnvironment({ STEELENGINE_ALLOW_ROOT: "1" });
    expect(result.STEELENGINE_ALLOW_ROOT).toBeUndefined();
  });

  it("preserves STEELENGINE_CLI_CONTAINER_BYPASS and STEELENGINE_CONTAINER_HINT", async () => {
    const result = await buildEnvironment({
      STEELENGINE_CLI_CONTAINER_BYPASS: "1",
      STEELENGINE_CONTAINER_HINT: "ci",
    });
    expect(result.STEELENGINE_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.STEELENGINE_CONTAINER_HINT).toBe("ci");
  });

  it("still drops arbitrary STEELENGINE_FOO", async () => {
    const result = await buildEnvironment({ STEELENGINE_FOO: "bar" });
    expect(result.STEELENGINE_FOO).toBeUndefined();
  });

  it("preserves container opt-ins while dropping unrelated STEELENGINE_* keys", async () => {
    const result = await buildEnvironment({
      STEELENGINE_CLI_CONTAINER_BYPASS: "1",
      STEELENGINE_CONTAINER_HINT: "ci",
      STEELENGINE_BAZ: "qux",
    ...
    expect(result.STEELENGINE_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.STEELENGINE_CONTAINER_HINT).toBe("ci");
    expect(result.STEELENGINE_BAZ).toBeUndefined();
  });
});
