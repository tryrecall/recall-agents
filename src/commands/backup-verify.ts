...
function assertCanonicalSqlitePathCasing(relativePath: string, archivePath: string): void {
  ...
  const portablePath = resolvePortableArchivePathKey(relativePath);
  const isGlobalAlias =
    portablePath === "state/steelengine.sqlite" && relativePath !== "state/steelengine.sqlite";
  const isAgentAlias =
    segments.length === 4 &&
    segments[0]?.toLowerCase() === "agents" &&
    Boolean(segments[1]) &&
    segments[2]?.toLowerCase() === "agent" &&
    segments[3]?.toLowerCase() === "steelengine-agent.sqlite" &&
    (segments[0] !== "agents" ||
      segments[2] !== "agent" ||
...
function resolveExpectedSqliteRoleFromRelativePath(
  ...
  if (relativePath === "state/steelengine.sqlite") {
    return "global";
  }
  const segments = relativePath.split("/");
  if (
...
async function verifySqliteSnapshots(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(tempRoot, "steelengine-backup-verify-sqlite-"));
  try {
