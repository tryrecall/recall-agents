...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(steelengine completion ...)` */
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: "Run `steelengine doctor --fix` to upgrade to cached completion.",
      },
    ];
  }
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: `Run \`steelengine completion --write-state\` or \`steelengine doctor --fix\` to regenerate ${status.cachePath}.`,
      },
    ];
  }
  return [];
