...
import type { SteelEngineConfig, GatewayAuthConfig } from "../config/config.js";
...
function hasConfiguredProviderModels(cfg: SteelEngineConfig, provider: string | undefined): boolean {
  if (!provider) {
    return false;
  }
...
function hasStaticManifestCatalogRows(cfg: SteelEngineConfig, provider: string | undefined): boolean {
  if (!provider) {
    return false;
  }
  return (
...
function listConfiguredModelProviders(cfg: SteelEngineConfig): string[] {
  return Object.entries(cfg.models?.providers ?? {})
    .filter(([, provider]) => (provider.models?.length ?? 0) > 0)
    .map(([provider]) => provider);
}

function resolveSingleConfiguredProvider(cfg: SteelEngineConfig): string | undefined {
...
function resolveConfiguredProviderFromAuthChange(params: {
  before: SteelEngineConfig;
  after: SteelEngineConfig;
...
export function buildGatewayAuthConfig(params: {
  ...
  if (params.mode === "trusted-proxy") {
    if (!params.trustedProxy) {
      throw new Error(
        `trustedProxy config is required when mode is trusted-proxy. Run ${formatCliCommand("steelengine configure --section gateway")} to configure Gateway auth interactively.`,
      );
    }
