...
export async function requireValidConfigFileSnapshot(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`SteelEngine config is invalid: ${snapshot.path}\n${issues}`);
    runtime.error(
      isPluginPackagingRuntimeOutputInvalidConfigSnapshot(snapshot)
        ? `Fix: ${formatPluginPackagingRuntimeOutputRecoveryHint()}`
        : `Fix: ${formatCliCommand("steelengine doctor --fix")}`,
    );
    runtime.error(`Inspect: ${formatCliCommand("steelengine config validate")}`);
    runtime.exit(1);
    return null;
  }
...
/** Read and return a valid SteelEngine config, or null after reporting validation errors. */
