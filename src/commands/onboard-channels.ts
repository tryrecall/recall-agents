...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("recall pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelSetupDmPolicy) => {
    ...
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`recall pairing approve ${policy.channel} <code>`)}`,
      ...
      if (adapter) {
        await prompter.note(
          `${channel} plugin not available (continuing with setup). If the channel still doesn't work after setup, run \`${formatCliCommand(
            "recall plugins list",
          )}\` and \`${formatCliCommand("recall plugins enable " + channel)}\`, then restart the gateway.`,
          "Channel setup",
        );
        await refreshStatus(channel);
        return true;
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("recall channels add")}\``,
        },
      ],
