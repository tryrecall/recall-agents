...
function bindGatewayConfig(target: RemoteGatewayInferenceTarget): SteelEngineConfig {
  return {
    ...target.config,
    gateway: {
 ...
 * Gateway's SteelEngine finish setup before handing off to its normal TUI.
...
export async function runRemoteGatewayInferenceOnboarding(
  ...
  await runGuidedOnboarding({}, runtime, {
    ...
    runSystemAgentChat: async () => {
      ...
      await prompter.intro("SteelEngine");
      ...
      try {
        for (;;) {
          await prompter.note(reply.reply, "SteelEngine");
          if (reply.action === "exit") {
            await prompter.outro("SteelEngine setup finished.");
            return;
          }
        ...
        if (error instanceof WizardCancelledError) {
          await prompter.outro("SteelEngine setup paused.");
          return;
        }
        throw error;
      }
