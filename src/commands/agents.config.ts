...
function resolveAgentName(cfg: RecallConfig, agentId: string) {
...
function resolveAgentModel(cfg: RecallConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: RecallConfig): AgentSummary[] {
