// SteelEngine command gate: prove inference before starting conversational setup.
...
/**
 * Start SteelEngine only after the configured default model completes a real
...
export async function runSystemAgentWithInference(
  ...
  if (opts.yes && !opts.message?.trim()) {
    failOneShotExecution(
      opts,
      runtime,
      new Error("SteelEngine --yes requires --message so approval is limited to one request."),
    );
    return;
  }
  const oneShot = isOneShotRequest(opts);
  if (!oneShot && !hasInteractiveTty(opts)) {
    runtime.error("SteelEngine needs an interactive TTY. Use --message for one command.");
    runtime.exit(1);
    return;
  }
  ...
  if (oneShot) {
    const guidance = "Run `steelengine onboard` to connect and live-test AI first.";
    if (opts.json) {
      writeRuntimeJson(runtime, {
        ok: false,
        status: inference.status,
        error: `SteelEngine requires working inference: ${inference.error}`,
        guidance,
      });
    } else {
      runtime.error(
        [`SteelEngine requires working inference: ${inference.error}`, guidance].join("\n"),
      );
    }
    if (!requestExitAfterOneShotOutput(runtime, 1)) {
      runtime.exit(1);
  ...
  runtime.log("SteelEngine requires working inference. Starting guided AI setup…");
