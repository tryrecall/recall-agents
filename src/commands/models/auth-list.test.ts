...
const mocks = vi.hoisted(() => ({
  ...
  resolveModelsTargetAgent: vi.fn((_cfg: SteelEngineConfig, rawAgentId?: string) => {
    const agentId = rawAgentId ?? "main";
    return { agentDir: `/tmp/steelengine/agents/${agentId}`, agentId };
  }),
}));

vi.mock("../../agents/agent-scope.js", () => ({
  resolveAgentDir: (_cfg: SteelEngineConfig, agentId: string) => `/tmp/steelengine/agents/${agentId}`,
...
vi.mock("../../agents/auth-profiles.js", () => ({
  ...
  resolveAuthStatePathForDisplay: (agentDir: string) => `${agentDir}/steelengine-agent.sqlite`,
}));
...
describe("modelsAuthListCommand", () => {
  beforeEach(() => {
    mocks.loadModelsConfig.mockReset().mockResolvedValue({} as SteelEngineConfig);
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "coder",
        authStatePath: "/tmp/steelengine/agents/coder/steelengine-agent.sqlite",
        profiles: [
          {
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "main",
        authStatePath: "/tmp/steelengine/agents/main/steelengine-agent.sqlite",
        profiles: [
          {
  ...
  it("prints an empty profile list without failing", async () => {
    ...
    expect(runtime.logs).toEqual([
      "Agent: main",
      "Auth state store: /tmp/steelengine/agents/main/steelengine-agent.sqlite",
      "Profiles: (none)",
    ]);
  });
