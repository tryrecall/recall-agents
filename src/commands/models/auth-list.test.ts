...
vi.mock("../../agents/agent-scope.js", () => ({
  resolveAgentDir: (_cfg: RecallConfig, agentId: string) => `/tmp/recall/agents/${agentId}`,
...
describe("modelsAuthListCommand", () => {
  beforeEach(() => {
    mocks.loadModelsConfig.mockReset().mockResolvedValue({} as RecallConfig);
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "coder",
        authStatePath: "/tmp/recall/agents/coder/auth-state.json",
        profiles: [
          {
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "main",
        authStatePath: "/tmp/recall/agents/main/auth-state.json",
        profiles: [
          {
  ...
  it("prints an empty profile list without failing", async () => {
    ...
    expect(runtime.logs).toEqual([
      "Agent: main",
      "Auth state file: /tmp/recall/agents/main/auth-state.json",
      "Profiles: (none)",
    ]);
  });
});
