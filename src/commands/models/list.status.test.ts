...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveRecallAgentDir: vi.fn().mockReturnValue("/tmp/recall-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/recall-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveRecallAgentDir: mocks.resolveRecallAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveRecallAgentDir).toHaveBeenCalled();
