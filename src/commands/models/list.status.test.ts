...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/recall-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/recall-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/recall-agent"}/auth-profiles.json`,
    ),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/recall-dev/recall.json",
    }),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/recall-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/recall-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/recall-dev/recall.json");
    expect(payload.auth.storePath).toBe("/tmp/recall-agent/auth-profiles.json");
  ...
  it("honors RECALL_AGENT_DIR when no --agent override is provided", async () => {
    const localRuntime = createRuntime();
    const previous = process.env.RECALL_AGENT_DIR;
    process.env.RECALL_AGENT_DIR = "/tmp/recall-isolated-agent";
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.RECALL_AGENT_DIR = previous;
      }
    }

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/recall-isolated-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/recall-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/recall-isolated-agent/auth-profiles.json");
  });
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/recall-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/recall-agent-custom");
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/recall-agent-custom/auth-profiles.json",
        });
      },
    );
  });
      ...
      expect(payload.auth.runtimeAuthRoutes).toEqual([
        {
          ...
          effective: {
            kind: "profiles",
            detail: "/tmp/recall-agent/auth-profiles.json",
          },
        },
      ]);
      expect(localRuntime.exit).not.toHaveBeenCalledWith(1);
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        provider === "workspace-cloud" && options?.workspaceDir === "/tmp/recall-agent/workspace"
          ? {
