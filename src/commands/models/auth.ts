...
function validateOpenAICodexApiKeyInput(value: string): string | undefined {
  ...
  if (looksLikeJwtToken(trimmed) || looksLikeStructuredCredential(trimmed)) {
    return `That looks like token or OAuth material, not an OpenAI API key. Use ${formatCliCommand("recall models auth paste-token --provider openai-codex")} for token auth material.`;
  }
...
function resolveRequestedProviderOrThrow(
  ...
  throw new Error(
    `Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("recall plugins list --json")}\`.`,
  );
}
...
export async function modelsAuthSetupTokenCommand(
  ...
  if (!process.stdin.isTTY) {
    throw new Error(
      `setup-token requires an interactive TTY. In automation, use ${formatCliCommand("recall models auth paste-token --provider <provider>")} instead.`,
    );
  }
  ...
  if (tokenProviders.length === 0) {
    throw new Error(
      `No provider token-auth plugins found. Install one via \`${formatCliCommand("recall plugins install")}\`.`,
    );
  }
  ...
  if (!provider) {
    throw new Error(
      `No token-capable provider is available. Run ${formatCliCommand("recall plugins list")} to verify provider plugins are installed.`,
    );
  }
...
export async function modelsAuthPasteTokenCommand(
  ...
  if (!rawProvider) {
    throw new Error(
      `Missing --provider. Run ${formatCliCommand("recall models status")} or ${formatCliCommand("recall plugins list")} to choose a provider.`,
    );
  }
  ...
  const tokenInput = await readPastedSecret({
    ...
    validate: (value) => {
      ...
      if (isOpenAICodexProvider(provider) && looksLikeOpenAIApiKey(trimmed)) {
        return `That looks like an OpenAI API key. Use ${formatCliCommand("recall models auth paste-api-key --provider openai-codex")} for API-key auth.`;
      }
      return undefined;
    },
  });
  ...
  if (provider === "anthropic") {
    runtime.log("Anthropic setup-token auth is supported in Recall.");
    runtime.log("Recall prefers Claude CLI reuse when it is available on the host.");
    runtime.log("Anthropic staff told us this Recall path is allowed again.");
  }
}
...
export async function modelsAuthAddCommand(opts: { agent?: string }, runtime: RuntimeEnv) {
  ...
  if (providerPlugin) {
    ...
    if (methodId !== "paste") {
      ...
      if (!method) {
        throw new Error(
          `Unknown token auth method "${methodId}". Run ${formatCliCommand("recall models auth login --provider " + providerPlugin.id)} to choose interactively.`,
        );
      }
      await runProviderAuthMethod({
        config,
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  if (!process.stdin.isTTY) {
    throw new Error(
      `models auth login requires an interactive TTY. In automation, use ${formatCliCommand("recall models auth paste-token --provider <provider>")} when token auth is available.`,
    );
  }
  ...
  if (authProviders.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("recall plugins install")}\`.`,
    );
  }
  ...
  if (!selectedProvider) {
    throw new Error(
      `Unknown provider. Run ${formatCliCommand("recall models status")} or ${formatCliCommand("recall plugins list")} to see available provider plugins.`,
    );
  }
  ...
  if (!chosenMethod) {
    throw new Error(
      `Unknown auth method. Run ${formatCliCommand("recall models auth login --provider " + selectedProvider.id)} without --method to choose interactively.`,
    );
  }
