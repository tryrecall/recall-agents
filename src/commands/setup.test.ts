  ...
  return {
    ...
    ensureAgentWorkspace: vi.fn(
      async (params?: { dir?: string; skipOptionalBootstrapFiles?: string[] }) => ({
        dir: params?.dir ?? path.join(home, ".recall", "workspace"),
      }),
    ),
    ...
    mkdir: vi.fn(async () => {}),
    resolveSessionTranscriptsDir: vi.fn(() => path.join(home, ".recall", "sessions")),
...
describe("setupCommand", () => {
  it("writes gateway.mode=local on first run", async () => {
    await withTempHome(async (home) => {
      ...
      const workspace = path.join(home, ".recall", "workspace");
  ...
  it("explains that plain setup only initializes local files", async () => {
    await withTempHome(async (home) => {
      ...
      expect(runtime.log.mock.calls.map((call) => String(call[0])).slice(-5)).toStrictEqual([
        ...
        "Next guided path: recall onboard.",
        "Next targeted changes: recall configure for models, channels, Gateway, plugins, skills, and health checks.",
        "Add a chat channel later: recall channels add.",
      ]);
    });
  });
