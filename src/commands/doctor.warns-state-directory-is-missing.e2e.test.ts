...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    await withEnvAsync({ STEELENGINE_STATE_DIR: missingDir }, async () => {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
      });
  ...
  it("skips gateway auth warning when STEELENGINE_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.STEELENGINE_GATEWAY_TOKEN = "env-token-1234567890";
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
  ...
  it("warns when token and password are both configured and gateway.auth.mode is unset", async () => {
    ...
    expect(String(gatewayAuthNote[0])).toContain("steelengine config set gateway.auth.mode token");
    expect(String(gatewayAuthNote[0])).toContain("steelengine config set gateway.auth.mode password");
  });
  ...
  it("keeps doctor read-only when gateway token is SecretRef-managed but unresolved", async () => {
    ...
    const previousToken = process.env.STEELENGINE_GATEWAY_TOKEN;
    delete process.env.STEELENGINE_GATEWAY_TOKEN;
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
      ...
      if (previousToken === undefined) {
        ...
        process.env.STEELENGINE_GATEWAY_TOKEN = previousToken;
      }
    }
  ...
  it("does not let STEELENGINE_GATEWAY_TOKEN hide an unresolved SecretRef-managed token", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: {
          mode: "local",
    ...
    const previousFallbackToken = process.env.STEELENGINE_GATEWAY_TOKEN;
    const previousRefToken = process.env.STEELENGINE_MISSING_GATEWAY_REF_TOKEN;
    process.env.STEELENGINE_GATEWAY_TOKEN = "fallback-token-1234567890";
    delete process.env.STEELENGINE_MISSING_GATEWAY_REF_TOKEN;
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
      ...
      if (previousFallbackToken === undefined) {
        ...
        process.env.STEELENGINE_GATEWAY_TOKEN = previousFallbackToken;
      }
      if (previousRefToken === undefined) {
        delete process.env.STEELENGINE_MISSING_GATEWAY_REF_TOKEN;
      } else {
        process.env.STEELENGINE_MISSING_GATEWAY_REF_TOKEN = previousRefToken;
      }
    }
  ...
  it("skips token-mode exec token probes even when env password is set", async () => {
    ...
    const previousPassword = process.env.STEELENGINE_GATEWAY_PASSWORD;
    process.env.STEELENGINE_GATEWAY_PASSWORD = "fallback-password";
    try {
      callGateway.mockClear();
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
      ...
      if (previousPassword === undefined) {
        ...
        process.env.STEELENGINE_GATEWAY_PASSWORD = previousPassword;
      }
    }
  ...
  it("skips gateway auth warning when SecretRef-managed token resolves", async () => {
    ...
    const previousToken = process.env.STEELENGINE_GATEWAY_TOKEN;
    process.env.STEELENGINE_GATEWAY_TOKEN = "resolved-token-1234567890";
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
