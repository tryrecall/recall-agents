...
function formatTaskLookupMiss(lookup: string): string {
  return formatLookupMiss({
    noun: "Task",
    value: lookup,
    listCommand: "steelengine tasks list",
    valueLabel: "task id",
  });
}
...
export async function tasksListCommand(
  ...
  if (tasks.length === 0) {
    runtime.log(
      `No background tasks found. Run ${formatCliCommand("steelengine tasks audit")} to check for stale task state.`,
    );
    return;
  }
  const rich = isRich();
...
export async function tasksMaintenanceCommand(
  ...
  if (!opts.apply) {
    runtime.log("Dry run only. Re-run with `steelengine tasks maintenance --apply` to write changes.");
  }
}
