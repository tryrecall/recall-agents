...
async function withTempHome<T>(
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-doctor-claude-cli-"));
  ...
  it("matches Claude's sanitized workspace project dir shape", () => {
    expect(
      resolveClaudeCliProjectDirForWorkspace({
        workspaceDir: "/Users/vincentkoc/GIT/_Perso/steelengine/.steelengine/workspace",
        homeDir: "/Users/vincentkoc",
      }),
    ).toBe(
      "/Users/vincentkoc/.claude/projects/-Users-vincentkoc-GIT--Perso-steelengine--steelengine-workspace",
    );
  });
});
...
describe("noteClaudeCliHealth", () => {
  ...
  it("explains the exact bad wiring when the claude-cli auth profile is missing", async () => {
    await withTempHome(({ homeDir, workspaceDir }) => {
      ...
      expect(body).toContain(`SteelEngine auth profile: missing (${CLAUDE_CLI_PROFILE_ID})`);
