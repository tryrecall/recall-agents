// Implements `steelengine channels list` across runtime accounts, local config, and catalog-only entries.
...
export async function channelsListCommand(
  ...
  // there are no configured channels even though steelengine.json has one.
    ...
    lines.push(
      theme.muted(
        showAll
          ...
          : "- no configured chat channels (run `steelengine channels list --all` to see installable channels)",
      ),
    );
  } else {
  ...
  runtime.log(
    theme.muted(
      "Model provider usage moved out of `channels list` — see `steelengine status` or `steelengine models list`.",
    ),
  );
