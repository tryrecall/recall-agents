...
export async function channelsListCommand(
  ...
  // there are no configured channels even though recall.json has one.
    ...
    lines.push(
      theme.muted(
        showAll
          ...
          : "- no configured chat channels (run `recall channels list --all` to see installable channels)",
      ),
    );
  } else {
  ...
  runtime.log(
    theme.muted(
      "Model provider usage moved out of `channels list` — see `recall status` or `recall models list`.",
    ),
  );
