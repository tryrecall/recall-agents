...
async function resolveCatalogChannelEntry(raw: string, cfg: RecallConfig | null) {
...
async function channelsAddCommandImpl(
  ...
  const cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as RecallConfig;
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run recall doctor --fix, then retry recall channels add.`
  ...
  if (!plugin?.setup?.applyAccountConfig) {
    runtime.error(
      ...
      })} Run ${formatCliCommand("recall channels add")} with no flags for guided setup.`,
    );
    runtime.exit(1);
    return;
  }
