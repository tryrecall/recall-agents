...
describe("resetCommand", () => {
  ...
  beforeEach(() => {
    ...
    resolveCleanupPlanFromDisk.mockReturnValue({
      stateDir: "/tmp/.recall",
      configPath: "/tmp/.recall/recall.json",
      oauthDir: "/tmp/.recall/credentials",
      configInsideState: true,
      oauthInsideState: true,
      workspaceDirs: ["/tmp/.recall/workspace"],
    });
    removePath.mockResolvedValue({ ok: true });
    listAgentSessionDirs.mockResolvedValue(["/tmp/.recall/agents/main/sessions"]);
  ...
  it("recommends creating a backup before state-destructive reset scopes", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(expect.stringContaining("recall backup create"));
  });
  ...
  it("does not recommend backup for config-only reset", async () => {
    ...
    expect(runtime.log).not.toHaveBeenCalledWith(expect.stringContaining("recall backup create"));
  });
});
