...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("recall doctor --fix")} or ${formatCliCommand("recall memory status --fix")}`
    : `Verify: ${formatCliCommand("recall memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: RecallConfig): Promise<void> {
  try {
...
function hasActiveAlternateMemoryPluginSlot(cfg: RecallConfig): boolean {
 ...
 * Runs as part of `recall doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
      ...
      note(
        [
          ...
          workspaceProbeFailed
            ...
            : `- Set an explicit binary path: ${formatCliCommand("recall config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("recall config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("recall memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`recall config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    if (isKeyOptionalMemoryProvider(resolved.provider)) {
      ...
      // not warn. A skipped probe means the user ran `recall doctor` without
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("recall configure --section model")}`,
        `- To disable: ${formatCliCommand("recall config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("recall memory status --deep")}`,
      ].join("\n"),
      "Memory search",
