 ...
 * Runs as part of `recall doctor` — config-only, no network calls.
 */
    ...
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("recall config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("recall memory status --deep")}`,
        ].join("\n"),
        "Memory search",
