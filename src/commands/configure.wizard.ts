...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.STEELENGINE_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.STEELENGINE_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from steelengine.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<SteelEngineConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "SteelEngine update wizard" : "SteelEngine configure");
    ...
    const baseConfig: SteelEngineConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("steelengine doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const promptGatewayRunMode = async (): Promise<OnboardMode> => {
      ...
      const localProbePromise = (async () => {
        ...
        return probeGatewayReachable({
          url: localUrl,
          token: process.env.STEELENGINE_GATEWAY_TOKEN ?? baseLocalProbeToken,
          password: process.env.STEELENGINE_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    ...
    if (shouldSkipGatewaySummary) {
      ...
      if (remoteUrl) {
        note(
          ["Remote Gateway:", remoteUrl, "Docs: https://docs.steelengine.ai/gateway/remote"].join(
            "\n",
          ),
          "Gateway",
        );
    ...
    note(
      [
        ...
        "Docs: https://docs.steelengine.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
