...
function isRecallCommandBasename(basename: string, platform: NodeJS.Platform): boolean {
  if (basename === "recall") {
    return true;
  }
  if (platform === "win32") {
    return (
      basename === "recall.cmd" || basename === "recall.ps1" || basename === "recall.exe"
    );
  }
  return false;
}
...
export function resolveDaemonRecallBinDir(
  params: {
    argv?: string[];
    env?: Record<string, string | undefined>;
  ...
  if (
    ...
    isRecallCommandBasename(path.basename(argv1), platform)
  ) {
  ...
  for (const rawSegment of (env.PATH ?? "").split(path.delimiter)) {
    ...
    const candidate = path.join(segment, platform === "win32" ? "recall.cmd" : "recall");
...
export function resolveDaemonServicePathDirs(params: {
  ...
  for (const dir of resolveDaemonRecallBinDir(params) ?? []) {
    addUniquePathDir(dirs, dir);
  }
  return dirs.length > 0 ? dirs : undefined;
}
