...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-steelengine-backup";
...
async function createArchiveWithManifestContent(
  ...
  const payloadArchivePath =
    options.payloadArchivePath ?? `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.steelengine/payload.txt`;
  try {
...
async function createSqlitePayload(setup: (database: DatabaseSync) => void): Promise<Buffer> {
  const tempDir = tempDirs.make("steelengine-backup-verify-sqlite-db-");
...
describe("backupVerifyCommand", () => {
  ...
  it("verifies a valid backup archive", async () => {
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-backup-verify-out-"));
    try {
      ...
      const payloadArchivePath = `${archiveRoot}/payload/posix/tmp/.steelengine/state.txt`;
      await fs.writeFile(
        manifestPath,
  ...
  it("verifies SQLite integrity and the canonical shared-state role", async () => {
    const stateAssetArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.steelengine`;
    const sqliteArchivePath = `${stateAssetArchivePath}/state/steelengine.sqlite`;
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "steelengine-backup-valid-sqlite-",
  ...
  it("rejects canonical SQLite snapshots with foreign-key violations", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "steelengine-backup-foreign-key-",
        manifestAssetArchivePath: stateAssetArchivePath,
        payloads: [
          {
            fileName: "steelengine.sqlite",
  ...
  it("does not interpret plugin-owned SQLite schemas without their owner runtime", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "steelengine-backup-plugin-owned-sqlite-",
        manifestAssetArchivePath: stateAssetArchivePath,
        payloads: [
          {
  ...
  it("rejects a structurally valid archive containing a malformed SQLite snapshot", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "steelengine-backup-invalid-sqlite-",
        manifestAssetArchivePath: stateAssetArchivePath,
        payloads: [
          {
            fileName: "steelengine.sqlite",
      ...
      async (archivePath) => {
        const verificationTempRoot = tempDirs.make("steelengine-backup-verify-cleanup-");
        const tmpdirSpy = vi.spyOn(os, "tmpdir").mockReturnValue(verificationTempRoot);
        try {
          const runtime = createBackupVerifyRuntime();
          await expect(backupVerifyCommand(runtime, { archive: archivePath })).rejects.toThrow(
            /Backup SQLite snapshot failed verification.*steelengine\.sqlite/iu,
          );
  ...
  it("rejects an empty SQLite snapshot instead of accepting a new empty database", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "steelengine-backup-empty-sqlite-",
        manifestAssetArchivePath: stateAssetArchivePath,
        payloads: [
          {
  ...
  it.each(["-wal", "-WAL"])(
    ...
    async (sidecarSuffix) => {
      ...
      await withBrokenArchiveFixture(
        {
          tempPrefix: "steelengine-backup-sqlite-sidecar-",
          manifestAssetArchivePath: stateAssetArchivePath,
          payloads: [
            {
              fileName: "steelengine.sqlite",
            ...
            {
              fileName: "steelengine.sqlite-wal",
        ...
        async (archivePath) => {
          const runtime = createBackupVerifyRuntime();
          await expect(backupVerifyCommand(runtime, { archive: archivePath })).rejects.toThrow(
            /contains a SQLite snapshot sidecar.*steelengine\.sqlite-wal/iu,
          );
        },
      );
    },
  ...
  it("rejects case-mangled canonical SQLite paths", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "steelengine-backup-sqlite-case-alias-",
        manifestAssetArchivePath: stateAssetArchivePath,
        payloads: [
          {
            fileName: "steelengine.sqlite",
      ...
      async (archivePath) => {
        const runtime = createBackupVerifyRuntime();
        await expect(backupVerifyCommand(runtime, { archive: archivePath })).rejects.toThrow(
          /case-mangled canonical SQLite path.*State\/SteelEngine\.SQLITE/u,
        );
      },
    );
  });
  ...
  it("rejects case-mangled aliases of the state asset root", async () => {
    ...
    const aliasSidecarArchivePath = `${TEST_ARCHIVE_ROOT}/PAYLOAD/posix/tmp/.steelengine/plugins/dedicated/custom.sqlite-wal`;

    await withBrokenArchiveFixture(
      {
        tempPrefix: "steelengine-backup-state-root-case-alias-",
        manifestAssetArchivePath: stateAssetArchivePath,
        payloads: [
          {
  ...
  it("rejects a truncated SQLite snapshot with a valid database header", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "steelengine-backup-corrupt-sqlite-",
        manifestAssetArchivePath: stateAssetArchivePath,
        payloads: [
          {
  ...
  it("rejects a page-aligned truncated plugin SQLite snapshot", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "steelengine-backup-page-truncated-sqlite-",
        manifestAssetArchivePath: stateAssetArchivePath,
        payloads: [
          {
            fileName: "corrupt.sqlite",
  ...
  it("rejects a canonical SQLite snapshot with the wrong database role", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "steelengine-backup-wrong-sqlite-role-",
        manifestAssetArchivePath: stateAssetArchivePath,
        payloads: [
          {
            fileName: "steelengine.sqlite",
  ...
  it("validates a canonical agent database whose agent id is node_modules", async () => {
    const stateAssetArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.steelengine`;
    const sqliteArchivePath = `${stateAssetArchivePath}/agents/node_modules/agent/steelengine-agent.sqlite`;
    const sqlitePayload = await createSqlitePayload((database) => {
      database.exec(`
        CREATE TABLE schema_meta (
          meta_key TEXT NOT NULL PRIMARY KEY,
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "steelengine-backup-agent-node-modules-",
        manifestAssetArchivePath: stateAssetArchivePath,
        payloads: [
          {
            fileName: "steelengine-agent.sqlite",
            contents: sqlitePayload,
            archivePath: sqliteArchivePath,
          },
        ],
  ...
  it("rejects a state asset root that does not encode its declared source path", async () => {
    ...
    const sqliteArchivePath = `${declaredStateAssetRoot}/posix/tmp/.steelengine/state/steelengine.sqlite`;
    const sqlitePayload = await createSqlitePayload((database) => {
      database.exec(`
        CREATE TABLE schema_meta (
          meta_key TEXT NOT NULL PRIMARY KEY,
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "steelengine-backup-state-root-bypass-",
  ...
  it("rejects SQLite extraction before writing when temporary space is insufficient", () => {
    expect(() =>
      testApi.assertSqliteExtractionBudget({
        entries: [
          {
            raw: "backup/payload/state/steelengine.sqlite",
            normalized: "backup/payload/state/steelengine.sqlite",
  ...
  it("ignores package-owned and transient SQLite-shaped state files", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "steelengine-backup-excluded-sqlite-",
        manifestAssetArchivePath: stateAssetArchivePath,
        payloads: [
          {
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-steelengine-backup";
        ...
        assets: [
          {
            ...
            archivePath: `${rootName}/payload/posix/tmp/.steelengine`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("reports malformed manifest JSON without leaking parser internals", async () => {
    await createArchiveWithManifestContent(
      {
        tempPrefix: "steelengine-backup-bad-manifest-json-",
  ...
  it("rejects oversized manifest entries without retaining the full body", async () => {
    await createArchiveWithManifestContent(
      {
        tempPrefix: "steelengine-backup-huge-manifest-",
  ...
  it("rejects unsafe hardlink targets", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-backup-linkpath-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.steelengine/target.txt`;
    const hardlinkArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.steelengine/hardlink.txt`;
    try {
  ...
  it("accepts root-relative internal hardlink targets from older backups", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-backup-rootless-linkpath-"));
    const archivePath = path.join(tempDir, "backup.tar.gz");
    const rootRelativeTargetPath = "payload/posix/tmp/.steelengine/target.txt";
  ...
  it("rejects hardlink targets missing from archive entries", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-backup-missing-linkpath-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.steelengine/target.txt`;
    const hardlinkArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.steelengine/hardlink.txt`;
    const missingTargetPath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.steelengine/missing-target.txt`;
    try {
      const archive = gzipSync(
        Buffer.concat([
          encodeTarEntry({
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    ...
    try {
      ...
      const stateArchivePath = `${archiveRoot}/payload/posix/tmp/.steelengine/state.txt`;
  ...
  it("rejects duplicate manifest and payload entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.steelengine/payload.txt`;
    for (const options of [
      {
        tempPrefix: "steelengine-backup-duplicate-manifest-",
        payloads: [{ fileName: "payload.txt", contents: "payload\n" }],
        buildTarEntries: ({
          manifestPath,
          payloadPaths,
      ...
      {
        tempPrefix: "steelengine-backup-duplicate-payload-",
        payloads: [
      ...
      {
        tempPrefix: "steelengine-backup-portable-path-collision-",
        payloads: [
          { fileName: "payload-a.txt", contents: "payload-a\n", archivePath: payloadArchivePath },
          {
