...
function makeTrustedBundledPluginsDir() {
  ...
  const dir = fs.mkdtempSync(path.join(fixturesRoot, "recall-doctor-plugin-manifests-"));
  tempDirs.push(dir);
  return dir;
}

function configWithPluginLoadPath(pluginRoot: string): RecallConfig {
  return {
    plugins: {
      load: {
        paths: [pluginRoot],
...
describe("doctor plugin manifest legacy contract repair", () => {
  ...
  it("collects legacy top-level capability keys for migration", () => {
    ...
    const manifestPath = path.join(root, "recall.plugin.json");
  ...
  it("rewrites legacy top-level capability keys into contracts", async () => {
    ...
    const next = JSON.parse(fs.readFileSync(path.join(root, "recall.plugin.json"), "utf-8")) as {
