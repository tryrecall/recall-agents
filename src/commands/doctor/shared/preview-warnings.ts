...
function listAgentRecords(cfg: RecallConfig): Record<string, unknown>[] {
  return Array.isArray(cfg.agents?.list) ? cfg.agents.list.filter(hasRecord) : [];
}

function hasChannels(cfg: RecallConfig): boolean {
  return hasRecord(cfg.channels);
}

function hasPlugins(cfg: RecallConfig): boolean {
  return hasRecord(cfg.plugins);
}

function hasPluginLoadPaths(cfg: RecallConfig): boolean {
...
function hasSubagentAllowlistConfig(cfg: RecallConfig): boolean {
...
function hasExplicitChannelPluginBlockerConfig(cfg: RecallConfig): boolean {
...
function hasConfiguredSafeBins(cfg: RecallConfig): boolean {
...
function sourceReplyRuntimeMayAllowMessageTool(cfg: RecallConfig): boolean {
...
function resolveGroupVisibleReplyProvenance(cfg: RecallConfig): {
...
export function collectVisibleReplyToolPolicyWarnings(cfg: RecallConfig): string[] {
  const groupPolicy = resolveGroupVisibleReplyProvenance(cfg);
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    warnings.push(
      `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; Recall falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
    );
  }
  ...
  if (globalVisibleReplies === "message_tool" && groupPolicy.path !== "messages.visibleReplies") {
    ...
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; Recall falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  }
  return warnings;
}
...
export function collectChannelBoundMessageToolPolicyWarnings(cfg: RecallConfig): string[] {
