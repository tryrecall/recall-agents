...
function currentSteelEngineReleaseBase(): string {
...
const mocks = vi.hoisted(() => ({
  ...
  getOfficialExternalPluginCatalogManifest: vi.fn(
    (entry: { steelengine?: unknown }) => entry.steelengine,
  ),
  ...
  resolveDefaultPluginExtensionsDir: vi.fn(() => "/tmp/steelengine-plugins"),
  resolveDefaultPluginNpmDir: vi.fn(() => "/tmp/steelengine-npm"),
  resolvePluginNpmProjectsDir: vi.fn((npmDir = "/tmp/steelengine-npm") =>
  ...
  resolvePluginInstallDir: vi.fn(
    (pluginId: string, extensionsDir = "/tmp/steelengine-plugins") => `${extensionsDir}/${pluginId}`,
  ),
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-plugin-stub-repair-"));
  tempDirs.push(dir);
  return dir;
}
...
function writeLegacyNpmDeclarationStub(params: {
  ...
  fs.writeFileSync(
    path.join(params.pluginDir, "steelengine.extension.json"),
    JSON.stringify({
...
vi.mock("../../../plugins/doctor-contract-registry.js", async (importOriginal) => ({
  ...
  applyPluginDoctorCompatibilityMigrations: (cfg: SteelEngineConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
describe("repairMissingConfiguredPluginInstalls", () => {
  ...
  beforeEach(() => {
    ...
    mocks.resolveDefaultPluginExtensionsDir.mockReturnValue("/tmp/steelengine-plugins");
    mocks.resolveDefaultPluginNpmDir.mockReturnValue("/tmp/steelengine-npm");
    ...
    mocks.resolveOfficialExternalProviderContractPluginIds.mockImplementation(
      ({ contract, providerIds }: { contract: string; providerIds: ReadonlySet<string> }) => {
        ...
        return entries.flatMap((entry) => {
          ...
          const pluginId = candidate.steelengine?.plugin?.id ?? candidate.id;
          const ownedProviderIds = candidate.steelengine?.contracts?.[contract];
          if (
            !pluginId ||
    ...
    mocks.resolveOfficialExternalProviderPluginIds.mockImplementation(
      ({ providerIds }: { providerIds: ReadonlySet<string> }) => {
        ...
        return entries.flatMap((entry) => {
          ...
          const pluginId = candidate.steelengine?.plugin?.id ?? candidate.id;
          const ownsConfiguredProvider = candidate.steelengine?.providers?.some((provider) =>
    ...
    mocks.installPluginFromClawHub.mockResolvedValue({
      ok: true,
      pluginId: "matrix",
      targetDir: "/tmp/steelengine-plugins/matrix",
      version: "1.2.3",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@steelengine/plugin-matrix",
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValue({
      ...
      version: "1.2.3",
      npmResolution: {
        name: "@steelengine/plugin-matrix",
        version: "1.2.3",
        resolvedSpec: "@steelengine/plugin-matrix@1.2.3",
  ...
  it("maps a missing configured plugin install to a structured finding and dry-run effect", async () => {
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          npmSpec: "@steelengine/plugin-matrix@1.2.3",
    ...
    expect(issue).toEqual({
      ...
      pluginId: "matrix",
      installSpec: "@steelengine/plugin-matrix@1.2.3",
    });
    expect(
      ...
      severity: "warning",
      target: "matrix",
      fixHint: "Run `steelengine doctor --fix` to install @steelengine/plugin-matrix@1.2.3.",
    });
    expect(
    ...
    expect(issues).toEqual([
      {
        ...
        pluginId: "discord",
        installPath: missingDiscordPath,
        installSpec: "@steelengine/discord",
      },
    ]);
  });

  it("installs a missing configured SteelEngine channel plugin from npm by default", async () => {
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        id: "matrix",
        pluginId: "matrix",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: "@steelengine/plugin-matrix@1.2.3",
      extensionsDir: "/tmp/steelengine-plugins",
    ...
    expectRecordFields((records as Record<string, unknown>).matrix, {
      source: "npm",
      spec: "@steelengine/plugin-matrix@1.2.3",
      installPath: "/tmp/steelengine-plugins/matrix",
      version: "1.2.3",
    });
    ...
    expect(result.changes).toEqual([
      'Installed missing configured plugin "matrix" from @steelengine/plugin-matrix@1.2.3.',
    ]);
  ...
  it("uses an explicit ClawHub install spec before npm", async () => {
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          clawhubSpec: "clawhub:@steelengine/plugin-matrix@stable",
          npmSpec: "@steelengine/plugin-matrix@1.2.3",
          expectedIntegrity: "sha512-test",
        },
      },
    ...
    const clawHubCall = expectRecordFields(mockCallArg(mocks.installPluginFromClawHub), {
      spec: "clawhub:@steelengine/plugin-matrix@stable",
      expectedPluginId: "matrix",
    });
    expect(clawHubCall.logger).toEqual(expect.objectContaining({ terminalLinks: false }));
    expect(mocks.installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(result.changes).toEqual([
      'Installed missing configured plugin "matrix" from clawhub:@steelengine/plugin-matrix@stable.',
    ]);
  ...
  it("adds actionable acknowledgement guidance for risky ClawHub candidate failures", async () => {
    mocks.installPluginFromClawHub.mockResolvedValueOnce({
      ...
      error:
        'ClawHub release "@steelengine/plugin-matrix@stable" has trust warnings. Review the package and rerun with --acknowledge-clawhub-risk to continue.',
    });
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        id: "matrix",
    ...
    expect(result.warnings[0]).toContain(
      "steelengine plugins install clawhub:@steelengine/plugin-matrix@stable --acknowledge-clawhub-risk",
    );
  });
  ...
  it("adds repair warnings for blocked ClawHub update outcomes", async () => {
    const records = {
      demo: {
        source: "clawhub",
        spec: "clawhub:@steelengine/plugin-demo@stable",
      ...
      outcomes: [
        {
          ...
          message:
            'Skipped demo ClawHub update: ClawHub release "@steelengine/plugin-demo@1.2.4" cannot be installed because ClawHub flagged it as blocked or malicious. Review the security details above or choose a different version. Existing installed plugin left unchanged.',
        },
      ],
    });
  ...
  it("sanitizes and shell-quotes ClawHub acknowledgement guidance specs before rendering commands", async () => {
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "Matrix" },
        install: {
          clawhubSpec: "clawhub:@steelengine/plugin-matrix\n\u001b[31m@stable;$(touch /tmp/pwned)",
        },
      },
    ]);
    ...
    expect(warning).toContain(
      "steelengine plugins install 'clawhub:@steelengine/plugin-matrix\\n@stable;$(touch /tmp/pwned)' --acknowledge-clawhub-risk",
    );
  ...
  it("falls back to npm when an SteelEngine channel plugin artifact is unavailable on ClawHub", async () => {
    mocks.installPluginFromClawHub.mockResolvedValueOnce({
      ok: false,
    ...
    expect(result.changes).toEqual([
      'ClawHub clawhub:@steelengine/plugin-matrix@stable unavailable for "matrix"; falling back to npm @steelengine/plugin-matrix@1.2.3.',
      'Installed missing configured plugin "matrix" from @steelengine/plugin-matrix@1.2.3.',
    ]);
    expect(result.warnings).toStrictEqual([]);
  });
  ...
  it("does not fall back from ClawHub to non-SteelEngine npm packages", async () => {
    ...
    expect(result.warnings).toEqual([
      'Failed to install missing configured plugin "matrix" from clawhub:@steelengine/plugin-matrix@stable: ClawHub artifact download is not available yet.',
    ]);
  });

  it("honors npm-first catalog metadata for missing SteelEngine channel plugins", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "twitch",
      targetDir: "/tmp/steelengine-plugins/twitch",
      version: "2026.5.2",
      npmResolution: {
        name: "@steelengine/twitch",
        version: "2026.5.2",
        resolvedSpec: "@steelengine/twitch@2026.5.2",
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@steelengine/twitch"),
      expectedPluginId: "twitch",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "twitch" from ${expectedNpmInstallSpec("@steelengine/twitch")}.`,
    ]);
  });
  ...
  it("installs missing configured non-channel plugins from the official external catalog", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      targetDir: "/tmp/steelengine-plugins/diagnostics-otel",
      version: "2026.5.2",
      npmResolution: {
        name: "@steelengine/diagnostics-otel",
        version: "2026.5.2",
        resolvedSpec: "@steelengine/diagnostics-otel@2026.5.2",
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@steelengine/diagnostics-otel"),
      expectedPluginId: "diagnostics-otel",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "diagnostics-otel" from ${expectedNpmInstallSpec("@steelengine/diagnostics-otel")}.`,
    ]);
  });
  ...
  it("repairs official plugins at the exact extended-stable core version", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@steelengine/diagnostics-otel",
        version: VERSION,
        resolvedSpec: `@steelengine/diagnostics-otel@${VERSION}`,
      },
    });
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: `@steelengine/diagnostics-otel@${VERSION}`,
      expectedPluginId: "diagnostics-otel",
      trustedSourceLinkedOfficialInstall: true,
    });
  ...
  it("installs the official llama.cpp plugin for configured local memory embeddings", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "llama-cpp",
      targetDir: "/tmp/steelengine-plugins/llama-cpp",
      version: "2026.6.2",
      npmResolution: {
        name: "@steelengine/llama-cpp-provider",
        version: "2026.6.2",
        resolvedSpec: "@steelengine/llama-cpp-provider@2026.6.2",
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        ...
        steelengine: {
          ...
          install: {
            npmSpec: "@steelengine/llama-cpp-provider",
            defaultChoice: "npm",
          },
        },
        install: {
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@steelengine/llama-cpp-provider"),
      expectedPluginId: "llama-cpp",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "llama-cpp" from ${expectedNpmInstallSpec("@steelengine/llama-cpp-provider")}.`,
    ]);
  });
  ...
  it("does not let runtime fallback metadata override official catalog install specs", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "acpx",
      targetDir: "/tmp/steelengine-plugins/acpx",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@steelengine/acpx"),
      expectedPluginId: "acpx",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "acpx" from ${expectedNpmInstallSpec("@steelengine/acpx")}.`,
    ]);
  });
  ...
  it("uses current bundled discovery to remove records before stale snapshots can reinstall official plugins", async () => {
    const records = {
      "google-meet": {
        ...
        resolvedName: "@steelengine/google-meet",
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        id: "google-meet",
        label: "Google Meet",
        install: { npmSpec: "@steelengine/google-meet" },
        steelengine: {
          id: "google-meet",
          install: { npmSpec: "@steelengine/google-meet" },
        },
  ...
  it.each([
    [
      ...
      {
        source: "npm",
        spec: "@steelengine/matrix-fork",
        resolvedName: "@steelengine/matrix-fork",
        resolvedSpec: "@steelengine/matrix-fork@1.2.3",
    ...
    [
      ...
      {
        source: "clawhub",
        spec: "clawhub:@steelengine/matrix-fork@stable",
        clawhubPackage: "@steelengine/matrix-fork",
        installPath: "/missing/matrix-fork",
      },
    ],
  ])(
  ...
  it("defers missing external payload repair during the package update doctor pass", async () => {
    ...
    expect(result).toEqual({
      changes: [
        'Skipped package-manager repair for configured plugin "discord" during package update; rerun "steelengine doctor --fix" after the update completes.',
      ],
      warnings: [],
  ...
  it("updates an existing npm target when stale baseline records miss an installed package", async () => {
    ...
    const packageDir = path.join(npmRoot, "node_modules", "@steelengine", "discord");
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValue({
      ...
      npmResolution: {
        ...
        version: "1.2.3",
        resolvedSpec: "@steelengine/discord@1.2.3",
    ...
    const result = await repairMissingConfiguredPluginInstalls({
      ...
      env: {
        STEELENGINE_UPDATE_POST_CORE_CONVERGENCE: "1",
      },
    });

    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@steelengine/discord"),
      expectedPluginId: "discord",
      npmDir: npmRoot,
      mode: "update",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "discord" from ${expectedNpmInstallSpec("@steelengine/discord")}.`,
    ]);
    expect(result.warnings).toEqual([]);
  ...
  it("prefers an existing npm payload over ClawHub during post-core repair", async () => {
    const npmRoot = makeTempDir();
    const packageDir = path.join(npmRoot, "node_modules", "@steelengine", "matrix");
    fs.mkdirSync(packageDir, { recursive: true });
    fs.writeFileSync(
      path.join(packageDir, "package.json"),
      JSON.stringify({ name: "@steelengine/matrix", version: "1.2.3" }),
    );
    mocks.resolveDefaultPluginNpmDir.mockReturnValue(npmRoot);
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
    ...
    mocks.installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Plugin "@steelengine/matrix" requires plugin API >=2026.5.18.',
    });
    mocks.installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "matrix",
      ...
      npmResolution: {
        ...
        version: "1.2.3",
        resolvedSpec: "@steelengine/matrix@1.2.3",
        integrity: "sha512-matrix",
        resolvedAt: "2026-05-01T00:00:00.000Z",
      },
    });
    ...
    expectRecordFields(result.records.matrix, {
      ...
      version: "1.2.3",
      resolvedName: "@steelengine/matrix",
  ...
  it("passes the post-core compatibility host version to ClawHub repair", async () => {
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "WhatsApp" },
        install: {
          clawhubSpec: "clawhub:@steelengine/whatsapp",
    ...
    mocks.installPluginFromClawHub.mockResolvedValue({
      ok: true,
      pluginId: "whatsapp",
      targetDir: "/tmp/steelengine-plugins/whatsapp",
      version: "1.2.3",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
    ...
    const result = await repairMissingConfiguredPluginInstalls({
      ...
      env: {
        STEELENGINE_COMPATIBILITY_HOST_VERSION: "2026.5.19",
        STEELENGINE_UPDATE_POST_CORE_CONVERGENCE: "1",
      },
    });

    expectRecordFields(mockCallArg(mocks.installPluginFromClawHub), {
      spec: expectedClawHubInstallSpec("clawhub:@steelengine/whatsapp"),
      env: {
        STEELENGINE_COMPATIBILITY_HOST_VERSION: "2026.5.19",
        STEELENGINE_UPDATE_POST_CORE_CONVERGENCE: "1",
      },
    ...
    expectRecordFields(result.records.whatsapp, {
      ...
      installPath: "/tmp/steelengine-plugins/whatsapp",
      clawhubPackage: "@steelengine/whatsapp",
    });
  });

  it("repairs missing external payload during post-core convergence even with STEELENGINE_UPDATE_IN_PROGRESS=1", async () => {
    const records = {
      discord: {
        source: "npm",
        spec: "@steelengine/discord",
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        pluginId: "discord",
        meta: { label: "Discord" },
        install: { npmSpec: "@steelengine/discord" },
      },
    ]);
  ...
  it("installs channel-selected external plugins during a legacy package update doctor pass", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@steelengine/discord",
        version: "2026.5.17",
        resolvedSpec: "@steelengine/discord@2026.5.17",
        integrity: "sha512-discord",
  ...
  it("prefers npm over ClawHub during a legacy package update doctor pass", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      version: "2026.5.17",
      npmResolution: {
        name: "@steelengine/whatsapp",
        version: "2026.5.17",
        resolvedSpec: "@steelengine/whatsapp@2026.5.17",
    ...
    expect(mocks.installPluginFromClawHub).not.toHaveBeenCalled();
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@steelengine/whatsapp"),
      expectedPluginId: "whatsapp",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "whatsapp" from ${expectedNpmInstallSpec("@steelengine/whatsapp")}.`,
    ]);
    expectRecordFields(result.records.whatsapp, {
      source: "npm",
      installPath: "/tmp/steelengine-plugins/whatsapp",
  ...
  it("installs a missing third-party downloadable plugin from npm only", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "wecom",
      targetDir: "/tmp/steelengine-plugins/wecom",
      version: "2026.4.23",
      npmResolution: {
        name: "@wecom/wecom-steelengine-plugin",
        version: "2026.4.23",
        resolvedSpec: "@wecom/wecom-steelengine-plugin@2026.4.23",
    ...
    mocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        ...
        meta: { label: "WeCom" },
        install: {
          npmSpec: "@wecom/wecom-steelengine-plugin@2026.4.23",
        },
      },
    ]);
    ...
    expectRecordFields(installArg, {
      spec: "@wecom/wecom-steelengine-plugin@2026.4.23",
      expectedPluginId: "wecom",
    });
    expect(installArg).not.toHaveProperty("trustedSourceLinkedOfficialInstall", true);
    expect(result.changes).toEqual([
      'Installed missing configured plugin "wecom" from @wecom/wecom-steelengine-plugin@2026.4.23.',
    ]);
  });
  ...
  it("upgrades v2026.7.1-beta.3 Codex Supervisor config and installs Codex", async () => {
    ...
    const cfg = migration.next as SteelEngineConfig;
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "codex",
      targetDir: "/tmp/steelengine-plugins/codex",
      version: "2026.7.2",
      npmResolution: {
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@steelengine/codex"),
    ...
    expectRecordFields((records as Record<string, unknown>).codex, {
      ...
      installPath: "/tmp/steelengine-plugins/codex",
      version: "2026.7.2",
    ...
    expect(result.changes).toEqual([
      `Installed missing configured plugin "codex" from ${expectedNpmInstallSpec("@steelengine/codex")}.`,
    ]);
    expect(result.warnings).toEqual([]);
  ...
  it("refreshes a stale managed Codex runtime plugin selected by the OpenAI Codex route", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "@steelengine/codex", version: "2026.5.6" }),
    );
    const records = {
      codex: {
        source: "npm",
        spec: "@steelengine/codex",
        resolvedName: "@steelengine/codex",
        resolvedSpec: "@steelengine/codex@2026.5.6",
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@steelengine/codex",
        version: VERSION,
        resolvedSpec: `@steelengine/codex@${VERSION}`,
    ...
    expect(result.changes).toEqual([
      `Refreshed stale configured plugin "codex" from ${expectedNpmInstallSpec("@steelengine/codex")}.`,
    ]);
  ...
  it("does not refresh a converged beta Codex runtime plugin on the second doctor pass", async () => {
    const codexBetaVersion = `${currentSteelEngineReleaseBase()}-beta.4`;
    const installDir = makeTempDir();
    fs.writeFileSync(
      path.join(installDir, "package.json"),
      JSON.stringify({ name: "@steelengine/codex", version: "2026.5.6" }),
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@steelengine/codex",
        version: codexBetaVersion,
        resolvedSpec: `@steelengine/codex@${codexBetaVersion}`,
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: "@steelengine/codex@beta",
      expectedPluginId: "codex",
      trustedSourceLinkedOfficialInstall: true,
      mode: "update",
    });
    expect(firstPass.changes).toEqual([
      'Refreshed stale configured plugin "codex" from @steelengine/codex@beta.',
    ]);
  ...
  it("does not downgrade a newer managed Codex runtime plugin", async () => {
    ...
    fs.writeFileSync(
      path.join(installDir, "package.json"),
      JSON.stringify({ name: "@steelengine/codex", version: "9999.1.1" }),
    );
    const records = {
      codex: {
        source: "npm",
        spec: "@steelengine/codex",
        resolvedName: "@steelengine/codex",
        resolvedSpec: "@steelengine/codex@9999.1.1",
  ...
  it.each([
    ...
    ["environment runtime override", {}, { STEELENGINE_AGENT_RUNTIME: "codex" }],
  ...
  it("still installs a channel catalog plugin when the configured owner is blocked by the allowlist", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "feishu",
      targetDir: "/tmp/steelengine-plugins/feishu",
      version: "2026.5.2",
      npmResolution: {
        name: "@steelengine/feishu",
        version: "2026.5.2",
        resolvedSpec: "@steelengine/feishu@2026.5.2",
      },
    });
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@steelengine/feishu"),
      expectedPluginId: "feishu",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "feishu" from ${expectedNpmInstallSpec("@steelengine/feishu")}.`,
    ]);
  });
  ...
  it("reinstalls a missing configured plugin from its persisted install record", async () => {
    const records = {
      demo: {
        source: "npm",
        spec: "@steelengine/plugin-demo@1.0.0",
        installPath: "/missing/demo",
      },
    };
    mocks.loadInstalledPluginIndexInstallRecords.mockResolvedValue(records);
        ...
        plugins: {
          installs: {
            demo: {
              source: "npm",
              spec: "@steelengine/plugin-demo@1.0.0",
              installPath: "/tmp/steelengine-plugins/demo",
            },
          },
        },
      },
  ...
  it("keeps non-ClawHub updater warnings as persisted-record repair warnings", async () => {
    ...
    const repairWarning =
      'Could not repair steelengine peer link for "demo" at /tmp/steelengine-plugins/demo: permission denied';
    mocks.loadInstalledPluginIndexInstallRecords.mockResolvedValue(records);
  ...
  it("adds actionable acknowledgement guidance for risky persisted ClawHub repair failures", async () => {
      ...
      outcomes: [
        {
          ...
          message:
            'Skipped demo ClawHub update: ClawHub release "@steelengine/plugin-demo@1.0.0" has trust warnings. Review the package and rerun with --acknowledge-clawhub-risk to continue. Existing installed plugin left unchanged.',
        },
      ],
    });
    ...
    expect(result.warnings[0]).toContain(
      "steelengine plugins install clawhub:@steelengine/plugin-demo@1.0.0 --acknowledge-clawhub-risk",
    );
  });
  ...
  it("prefixes legacy persisted ClawHub package records in acknowledgement guidance", async () => {
      ...
      outcomes: [
        {
          ...
          code: CLAWHUB_INSTALL_ERROR_CODE.CLAWHUB_RISK_ACKNOWLEDGEMENT_REQUIRED,
          message:
            'Skipped demo ClawHub update: ClawHub release "@steelengine/plugin-demo@latest" has trust warnings. Review the package and rerun with --acknowledge-clawhub-risk to continue. Existing installed plugin left unchanged.',
        },
      ],
    });
    ...
    expect(result.warnings[0]).toContain(
      "steelengine plugins install clawhub:@steelengine/plugin-demo --acknowledge-clawhub-risk",
    );
  });
  ...
  it("repairs a broken managed package entry from its attributed registry diagnostic", async () => {
    const records = {
      demo: {
        source: "npm",
        spec: "@steelengine/plugin-demo@1.0.0",
        resolvedName: "@steelengine/plugin-demo",
        resolvedSpec: "@steelengine/plugin-demo@1.0.0",
  ...
  it("reinstalls a known configured plugin from the catalog when its recorded install path is missing", async () => {
    const records = {
      discord: {
        source: "npm",
        spec: "@steelengine/discord",
        installPath: "/tmp/steelengine-missing-discord-install-record",
      },
    };
    mocks.loadInstalledPluginIndexInstallRecords.mockResolvedValue(records);
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      ...
      diagnostics: [
        {
          ...
          pluginId: "discord",
          message:
            "channel plugin manifest declares discord without channelConfigs metadata; add steelengine.plugin.json#channelConfigs so config schema and setup surfaces work before runtime loads",
        },
      ],
    });
    mocks.updateNpmInstalledPlugins.mockResolvedValue({
  ...
  it("reinstalls a recorded external web search plugin from provider-only config", async () => {
    const records = {
      brave: {
        source: "npm",
        spec: "@steelengine/brave-plugin@beta",
  ...
  it("replaces a configured official web search plugin when its installed package is source-only", async () => {
    ...
    const records = {
      brave: {
        source: "clawhub",
        spec: "clawhub:@steelengine/brave-plugin@2026.5.1-beta.1",
        installPath: installDir,
        clawhubPackage: "@steelengine/brave-plugin",
        clawhubChannel: "official",
        clawhubUrl: "https://clawhub.ai",
      },
    };
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "brave",
      targetDir: "/tmp/steelengine-plugins/brave",
      version: "2026.5.12",
      npmResolution: {
        name: "@steelengine/brave-plugin",
        version: "2026.5.12",
        resolvedSpec: "@steelengine/brave-plugin@2026.5.12",
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@steelengine/brave-plugin"),
    ...
    expect(result).toEqual({
      changes: [
        `Installed missing configured plugin "brave" from ${expectedNpmInstallSpec("@steelengine/brave-plugin")}.`,
      ],
      warnings: [],
  ...
  it("replaces a configured official channel plugin when only its channel is configured", async () => {
    ...
    const records = {
      slack: {
        source: "clawhub",
        spec: "clawhub:@steelengine/slack@2026.5.12-beta.1",
        installPath: installDir,
        clawhubPackage: "@steelengine/slack",
        clawhubChannel: "official",
        clawhubUrl: "https://clawhub.ai",
      },
    };
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "slack",
      targetDir: "/tmp/steelengine-npm/node_modules/@steelengine/slack",
      version: "2026.5.12",
      npmResolution: {
        name: "@steelengine/slack",
        version: "2026.5.12",
        resolvedSpec: "@steelengine/slack@2026.5.12",
    ...
    expect(fs.existsSync(installDir)).toBe(false);
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@steelengine/slack"),
      expectedPluginId: "slack",
      trustedSourceLinkedOfficialInstall: true,
      mode: "update",
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "slack" from ${expectedNpmInstallSpec("@steelengine/slack")}.`,
    ]);
  });
  ...
  it("keeps a broken official install record when replacement install fails", async () => {
    ...
    expect(result).toEqual({
      changes: [],
      warnings: [
        `Failed to install missing configured plugin "brave" from ${expectedNpmInstallSpec("@steelengine/brave-plugin")}: network unavailable`,
      ],
  ...
  it("installs a configured external web search plugin from provider-only config", async () => {
    ...
    mocks.resolveOfficialExternalPluginId.mockImplementation(
      (entry: { id?: string; steelengine?: { plugin?: { id?: string } } }) =>
        entry.steelengine?.plugin?.id ?? entry.id,
    );
    mocks.resolveOfficialExternalPluginInstall.mockImplementation(
      (entry: { install?: unknown; steelengine?: { install?: unknown } }) =>
        entry.steelengine?.install ?? entry.install ?? null,
    );
    mocks.resolveOfficialExternalPluginLabel.mockImplementation(
      (entry: { label?: string; steelengine?: { plugin?: { label?: string } } }) =>
        entry.steelengine?.plugin?.label ?? entry.label ?? "plugin",
    );
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "brave",
      ...
      npmResolution: {
        name: "@steelengine/brave-plugin",
        version: "2026.5.2",
        resolvedSpec: "@steelengine/brave-plugin@2026.5.2",
      },
    });
  ...
  it("installs configured external speech and web-fetch plugins from selected providers", async () => {
    ...
    for (const [pluginId, npmSpec] of packages) {
      mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
        ok: true,
        pluginId,
        targetDir: `/tmp/steelengine-plugins/${pluginId}`,
        version: "2026.6.8",
        npmResolution: {
          name: npmSpec,
          version: "2026.6.8",
  ...
  it("installs a configured external model provider without an auth choice", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "groq",
      targetDir: "/tmp/steelengine-plugins/groq",
      version: "2026.6.8",
      npmResolution: {
        name: "@steelengine/groq-provider",
        version: "2026.6.8",
        resolvedSpec: "@steelengine/groq-provider@2026.6.8",
      },
    });
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@steelengine/groq-provider"),
      expectedPluginId: "groq",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "groq" from ${expectedNpmInstallSpec("@steelengine/groq-provider")}.`,
    ]);
  });
  ...
  it("installs an external speech provider selected only by voiceModel", async () => {
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        ...
        label: "Gradium",
        install: {
          npmSpec: "@steelengine/gradium-speech",
          defaultChoice: "npm",
        },
        steelengine: {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "gradium",
      targetDir: "/tmp/steelengine-plugins/gradium",
      version: "2026.6.8",
      npmResolution: {
        name: "@steelengine/gradium-speech",
        version: "2026.6.8",
        resolvedSpec: "@steelengine/gradium-speech@2026.6.8",
      },
    });
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@steelengine/gradium-speech"),
      expectedPluginId: "gradium",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "gradium" from ${expectedNpmInstallSpec("@steelengine/gradium-speech")}.`,
    ]);
  });

  it("installs env-only web provider plugins before auto-detection", async () => {
    const packages = [
      ["exa", "@steelengine/exa-plugin", "EXA_API_KEY"],
      ["firecrawl", "@steelengine/firecrawl-plugin", "FIRECRAWL_API_KEY"],
    ] as const;
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue(
  ...
  it("installs configured external web search plugins from beta on the beta channel", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@steelengine/brave-plugin",
        version: "2026.5.4-beta.1",
        resolvedSpec: "@steelengine/brave-plugin@2026.5.4-beta.1",
      },
    });
    ...
    expect(result.changes).toEqual([
      'Installed missing configured plugin "brave" from @steelengine/brave-plugin@beta.',
    ]);
  });
  ...
  it("repairs a configured plugin from a legacy npm declaration stub", async () => {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "guardrail-bridge",
      targetDir: "/tmp/steelengine-plugins/guardrail-bridge",
      version: "1.0.0",
      npmResolution: {
    ...
    expectRecordFields((records as Record<string, unknown>)["guardrail-bridge"], {
      source: "npm",
      spec: "@guardrail-bridge/guardrail-bridge@1.0.0",
      installPath: "/tmp/steelengine-plugins/guardrail-bridge",
      version: "1.0.0",
  ...
  it("installs Firecrawl for env-only web fetch when search is disabled", async () => {
    ...
    mocks.listOfficialExternalPluginCatalogEntries.mockReturnValue([
      {
        ...
        label: "Firecrawl",
        install: {
          npmSpec: "@steelengine/firecrawl-plugin",
          defaultChoice: "npm",
        },
        steelengine: {
    ...
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce({
      ok: true,
      pluginId: "firecrawl",
      targetDir: "/tmp/steelengine-plugins/firecrawl",
      version: "2026.6.8",
      npmResolution: {
        name: "@steelengine/firecrawl-plugin",
        version: "2026.6.8",
        resolvedSpec: "@steelengine/firecrawl-plugin@2026.6.8",
      },
    });
    ...
    expectRecordFields(mockCallArg(mocks.installPluginFromNpmSpec), {
      spec: expectedNpmInstallSpec("@steelengine/firecrawl-plugin"),
      expectedPluginId: "firecrawl",
      trustedSourceLinkedOfficialInstall: true,
    });
    expect(result.changes).toEqual([
      `Installed missing configured plugin "firecrawl" from ${expectedNpmInstallSpec("@steelengine/firecrawl-plugin")}.`,
    ]);
  });
