...
function runtimeHostCandidate(params: {
  ...
  if (runtimeId === "steelengine" || runtimeId === "auto") {
    return { runtimeId, host: STEELENGINE_EMBEDDED_CONTEXT_ENGINE_HOST, paths: params.paths };
  }
  if (runtimeId === "codex") {
...
function selectedContextEngineSlotId(cfg: SteelEngineConfig): string {
...
export async function maybeRepairContextEngineHostCompatibility(params: {
  ...
  doctorFixCommand: string;
  env?: NodeJS.ProcessEnv;
}): Promise<{ config: SteelEngineConfig; changes: string[]; warnings?: string[] }> {
  const resolved = await resolveSelectedContextEngineInfo(params);
  if (!resolved.info) {
