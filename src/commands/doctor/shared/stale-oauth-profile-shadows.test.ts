...
describe("stale OAuth profile shadow doctor repair", () => {
  const envSnapshot = captureEnv(["RECALL_STATE_DIR", "RECALL_HOME"]);
  let tempRoot = "";
  let stateDir = "";

  beforeEach(async () => {
    clearRuntimeAuthProfileStoreSnapshots();
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-stale-oauth-shadow-"));
    stateDir = path.join(tempRoot, "state");
    process.env.RECALL_STATE_DIR = stateDir;
    process.env.RECALL_HOME = stateDir;
  });
  ...
  it("warns about stale local OAuth shadows without modifying the child store", async () => {
    ...
    expect(warnings[0]).toContain("recall doctor --fix");
  ...
  it("uses the injected env for the main auth store", async () => {
    ...
    const injectedEnv = {
      ...process.env,
      RECALL_STATE_DIR: injectedStateDir,
      RECALL_HOME: injectedStateDir,
    };
    saveAuthProfileStore(
      storeWith(
        profileId,
  ...
  it("removes stale child OAuth shadows and local cooldown state", async () => {
    ...
    const result = await repairStaleOAuthProfileShadows({
      cfg: { agents: { list: [{ id: "telegram" }] } } satisfies RecallConfig,
      now,
    });
