...
function isRuntimeDependencyMarkerName(name: string): boolean {
  return (
    name === ".recall-runtime-deps.json" ||
    name === ".recall-runtime-deps-stamp.json" ||
    name.startsWith(".recall-runtime-deps-")
  );
}

function isInstallStageDebrisName(name: string): boolean {
  return /^\.recall-install-stage(?:-.+)?$/u.test(name);
}

function isLegacyDependencyDebrisName(name: string): boolean {
  return (
    isRuntimeDependencyMarkerName(name) ||
    name === ".recall-pnpm-store" ||
    name === ".recall-install-backups" ||
...
function collectExplicitStageTargets(env: NodeJS.ProcessEnv): CleanupTarget[] {
  return splitPathList(env.RECALL_PLUGIN_STAGE_DIR).map((entry) => ({
    kind: "explicit-stage",
...
async function hasRecallRenameResidue(root: string): Promise<boolean> {
  const nodeModulesRoot = path.join(root, "node_modules");
  if (await isFile(path.join(nodeModulesRoot, ".recall-rename-tmp"))) {
    return true;
  }
  ...
  for (const entry of entries) {
    ...
    if (await isFile(path.join(entryPath, ".recall-rename-tmp"))) {
      return true;
    }
    ...
    for (const scopedEntry of scopedEntries) {
      ...
      if (await isFile(path.join(entryPath, scopedEntry.name, ".recall-rename-tmp"))) {
        return true;
      }
    }
  }
...
async function hasExplicitStageDebrisProof(root: string): Promise<boolean> {
  ...
  return await hasRecallRenameResidue(root);
}
...
function filterLegacyStaleRootCandidates(
  ...
  for (const target of targets) {
    ...
    if (!cleanupRootPaths.some((rootPath) => isPathInsideRoot(targetPath, rootPath))) {
      warnings.push(
        `Skipped legacy plugin dependency state ${targetPath}: outside Recall cleanup roots`,
      );
      continue;
    }
    safeTargets.push({ ...target, path: targetPath });
...
async function resolveSafeRemovalTarget(
  ...
  if (!cleanupRoots.some((root) => isPathInsideRoot(realPath, root.realPath))) {
    return {
      warning: `Skipped legacy plugin dependency state ${targetPath}: resolved outside Recall cleanup roots`,
    };
  }
  return { target: targetPath };
}
