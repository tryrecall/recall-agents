...
import {
  ...
  resolveSteelEngineMcpTransportAlias,
...
const MCP_SERVER_TYPE_RULE: LegacyConfigRule = {
  path: ["mcp", "servers"],
  message:
    'mcp.servers entries use SteelEngine transport names; CLI-native type aliases are legacy here. Run "steelengine doctor --fix".',
  match: (value) =>
    isRecord(value) &&
...
const MCP_SERVER_DISABLED_RULES: LegacyConfigRule[] = [
  ...
  message:
    ...
    'Run "steelengine doctor --fix" to migrate it.',
  match: (value) =>
    isRecord(value) &&
  ...
  defineLegacyConfigMigration({
    ...
    apply: (raw, changes) => {
      ...
      for (const [serverName, rawServer] of Object.entries(servers)) {
        ...
        const alias = resolveSteelEngineMcpTransportAlias(rawServer.type);
