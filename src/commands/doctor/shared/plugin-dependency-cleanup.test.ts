...
describe("cleanupLegacyPluginDependencyState", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "recall-plugin-deps-cleanup-"));
  });
  ...
  it("collects and removes legacy plugin dependency state roots", async () => {
    ...
    const explicitStageDir = path.join(stateDir, ".recall-install-stage-explicit");
    ...
    const legacyExtensionStamp = path.join(
      ...
      "extensions",
      "demo",
      ".recall-runtime-deps-stamp.json",
    );
    const legacyManifest = path.join(
      packageRoot,
      "extensions",
      "demo",
      ".recall-runtime-deps.json",
    );
    ...
    const env = {
      ...
      RECALL_PLUGIN_STAGE_DIR: explicitStageDir,
  ...
  it("removes configured plugin stage roots outside Recall roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    ...
    await fs.writeFile(
      path.join(stageRoot, "node_modules", "ansi-escapes", ".recall-rename-tmp"),
    ...
    const result = await cleanupLegacyPluginDependencyState({
      env: {
        RECALL_STATE_DIR: stateDir,
        RECALL_PLUGIN_STAGE_DIR: stageRoot,
      },
      packageRoot,
    });
  ...
  it("refuses arbitrary explicit plugin stage roots outside Recall roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
  ...
  it("refuses explicit plugin stage paths with parent segments", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    const dotDotStage = `${stateDir}${path.sep}..${path.sep}.recall-install-stage-dotdot`;
    ...
    const result = await cleanupLegacyPluginDependencyState({
      env: {
        RECALL_STATE_DIR: stateDir,
        RECALL_PLUGIN_STAGE_DIR: dotDotStage,
      },
      packageRoot,
    });
  ...
  it("does not follow symlinked extension roots outside Recall roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    ...
    await fs.writeFile(path.join(externalPlugin, ".recall-runtime-deps.json"), "{}");
  ...
  it("refuses legacy roots that resolve outside Recall roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "package");
    const legacyRuntimeRoot = path.join(stateDir, "plugin-runtime-deps");
    ...
    expect(result.changes).toStrictEqual([]);
    expect(result.warnings).toContain(
      `Skipped legacy plugin dependency state ${legacyRuntimeRoot}: resolved outside Recall cleanup roots`,
    );
  ...
  it("does not unlink global runtime symlinks through unsafe cleanup roots", async () => {
    const stateDir = path.join(tempDir, "state");
    const packageRoot = path.join(tempDir, "prefix", "lib", "node_modules", "recall");
