...
import { compareSteelEngineVersions } from "../../../config/version.js";
import {
...
function isPluginsGloballyDisabled(cfg: SteelEngineConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: SteelEngineConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: SteelEngineConfig): string[] {
...
function isPluginEntryDisabled(cfg: SteelEngineConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: SteelEngineConfig, channelId: string): boolean {
  const channels = asObjectRecord(cfg.channels);
  const entry = asObjectRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: SteelEngineConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: SteelEngineConfig): string[] {
  const entries = asObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: SteelEngineConfig): string[] {
...
function collectConfiguredChannelIds(cfg: SteelEngineConfig, env: NodeJS.ProcessEnv): string[] {
  const ids = new Set<string>();
  const channels = asObjectRecord(cfg.channels);
  if (channels) {
...
function collectWebSearchPluginIds(cfg: SteelEngineConfig): string[] {
...
function collectEnvWebSearchPluginIds(cfg: SteelEngineConfig, env: NodeJS.ProcessEnv): string[] {
  if (cfg.tools?.web?.search?.enabled === false) {
    return [];
  }
...
function collectWebFetchPluginIds(cfg: SteelEngineConfig): string[] {
...
function collectEnvWebFetchPluginIds(cfg: SteelEngineConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectSpeechPluginIds(cfg: SteelEngineConfig): string[] {
  return resolveOfficialExternalProviderContractPluginIds({
...
function collectAcpRuntimePluginIds(cfg: SteelEngineConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: SteelEngineConfig): string[] {
...
function addEligiblePluginId(cfg: SteelEngineConfig, pluginIds: Set<string>, pluginId: string): void {
...
function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareSteelEngineVersions(
  ...
  const touchedComparedToRelease = compareSteelEngineVersions(params.touchedVersion, releaseVersion);
