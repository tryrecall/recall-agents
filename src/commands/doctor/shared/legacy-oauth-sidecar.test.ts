...
async function writeLegacySidecarThatNeedsKeychain(): Promise<{
  ...
  const state = await createSteelEngineTestState({
    layout: "state-only",
    prefix: "steelengine-legacy-oauth-keychain-warn-",
    env: {
      STEELENGINE_AGENT_DIR: undefined,
      STEELENGINE_AUTH_PROFILE_SECRET_KEY: undefined,
    },
  });
  states.push(state);
...
describe("loadLegacyOAuthSidecarMaterial keychain-only headless warning", () => {
  ...
  function envWithoutVitestSignals(state: SteelEngineTestState): NodeJS.ProcessEnv {
  ...
  it("emits a single doctor-pointer warning when only Keychain can decrypt and prompts are disabled", async () => {
    ...
    expect(String(firstMessage)).toContain("steelengine doctor --fix");
