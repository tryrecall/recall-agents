...
function resolveBundledWorkspaceDir(cfg: SteelEngineConfig): string | undefined {
  return resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg)) ?? undefined;
}

function isSteelEngineNodeModulesPackageRoot(packageRoot: string): boolean {
  ...
  return packageDir === "steelengine" && parentDir === "node_modules";
}

/** Find configured plugin load paths that alias bundled plugins already shipped by SteelEngine. */
  ...
  for (const rawPath of rawPaths) {
    ...
    if (!match) {
      ...
      const oldPackageMatch =
        // Only rewrite paths rooted in the installed SteelEngine package; user plugin paths stay intact.
        oldPackageRoot && oldBundledLeaf && isSteelEngineNodeModulesPackageRoot(oldPackageRoot)
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; SteelEngine loads the packaged bundled plugin from "${hit.toPath}".`,
  );
