...
function buildLegacyRealtimeTalkCompat(
  talk: Record<string, unknown>,
  normalizedTalk: NonNullable<SteelEngineConfig["talk"]>,
): Record<string, unknown> | undefined {
  ...
  return normalizeTalkSection({ realtime: compat } as SteelEngineConfig["talk"])?.realtime;
}
...
export function normalizeLegacyTalkConfig(cfg: SteelEngineConfig, changes: string[]): SteelEngineConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as SteelEngineConfig["talk"]) ?? {};
