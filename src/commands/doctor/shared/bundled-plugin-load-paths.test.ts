...
import { bundledDistPluginRootAt, bundledPluginRootAt } from "steelengine/plugin-sdk/test-fixtures";
...
describe("bundled plugin load path repair", () => {
  beforeEach(() => {
    const packageRoot = "/app/node_modules/steelengine";
  ...
  it("removes stale bundled paths from old versioned SteelEngine package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "steelengine");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
      "steelengine@2026.3.28_@napi-rs+canvas@0.1.97",
      "node_modules",
      "steelengine",
    );
  ...
  it("removes stale legacy bundled paths from old versioned SteelEngine package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "steelengine");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
  ...
  it("formats a doctor hint for legacy bundled plugin paths", () => {
    ...
    expect(warnings).toEqual([
      `- plugins.load.paths: bundled plugin path "${legacyPath}" still aliases feishu; SteelEngine loads the packaged bundled plugin from "${bundledPath}".`,
      '- Run "steelengine doctor --fix" to remove these redundant bundled plugin paths.',
    ]);
  });

  it("ignores bundled plugins that already resolve to source extensions", () => {
    const sourcePath = path.resolve("repo", "steelengine", "extensions", "feishu");
    vi.spyOn(bundledSources, "resolveBundledPluginSources").mockReturnValue(
