...
function repairBindingsForTest(config: SteelEngineConfig) {
  ...
  it("removes sidecar memory search index paths", () => {
    const res = migrateLegacyConfigForTest({
      memorySearch: {
        provider: "openai",
        store: {
          path: "/tmp/steelengine-memory-{agentId}.sqlite",
        ...
        providers: {
          codex: {
            models: [
              ...
              { id: "gpt-steelengine", agentRuntime: { id: "steelengine" } },
            ],
          },
        },
      },
    ...
    const res = migrateLegacyConfigForTest({
      agents: {
        ...
        list: [
          {
            id: "reviewer",
            agentRuntime: { fallback: "steelengine" },
            embeddedHarness: {
              runtime: "codex",
              fallback: "none",
            },
...
describe("legacy migrate MCP server type aliases", () => {
  it("moves CLI-native http type to SteelEngine streamable HTTP transport", () => {
    const res = migrateLegacyConfigForTest({
      mcp: {
        servers: {
          silo: {
