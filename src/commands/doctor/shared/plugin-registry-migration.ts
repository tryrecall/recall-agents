...
export const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "RECALL_DISABLE_PLUGIN_REGISTRY_MIGRATION";
export const FORCE_PLUGIN_REGISTRY_MIGRATION_ENV = "RECALL_FORCE_PLUGIN_REGISTRY_MIGRATION";
...
export type PluginRegistryInstallMigrationParams = LoadInstalledPluginIndexParams &
  InstalledPluginIndexStoreOptions & {
    ...
    readConfig?: () => Promise<RecallConfig> | RecallConfig;
  };
...
function listConfiguredChannelIds(config: RecallConfig): Set<string> {
...
function listConfiguredModelProviderIds(config: RecallConfig): Set<string> {
...
export async function migratePluginRegistryForInstall(
  ...
  const config = stripShippedPluginInstallConfigRecords(rawConfig) as RecallConfig;
