...
const mocks = vi.hoisted(() => ({
  ...
  maybeRepairManagedNpmSteelEnginePeerLinks: vi.fn(),
...
vi.mock("../doctor-plugin-registry.js", () => ({
  maybeRepairManagedNpmSteelEnginePeerLinks: mocks.maybeRepairManagedNpmSteelEnginePeerLinks,
...
vi.mock("./shared/channel-doctor.js", () => ({
  collectChannelDoctorRepairMutations: ({ cfg }: { cfg: SteelEngineConfig }) => {
...
vi.mock("./shared/empty-allowlist-scan.js", () => ({
  scanEmptyAllowlistPolicyWarnings: (cfg: SteelEngineConfig) =>
    cfg.channels?.signal
...
vi.mock("./shared/allowlist-policy-repair.js", () => ({
  maybeRepairAllowlistPolicyAllowFrom: async (cfg: SteelEngineConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/bundled-plugin-load-paths.js", () => ({
  maybeRepairBundledPluginLoadPaths: (cfg: SteelEngineConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/invalid-plugin-config.js", () => ({
  maybeRepairInvalidPluginConfig: (cfg: SteelEngineConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/legacy-tools-by-sender.js", () => ({
  maybeRepairLegacyToolsBySenderKeys: (cfg: SteelEngineConfig) => {
...
vi.mock("./shared/exec-safe-bins.js", () => ({
  maybeRepairExecSafeBinProfiles: (cfg: SteelEngineConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
describe("doctor repair sequencing", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.applyPluginAutoEnable.mockImplementation((params: { config: SteelEngineConfig }) => ({
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementation((cfg: SteelEngineConfig) => ({
      config: cfg,
      changes: [],
    }));
    mocks.maybeRepairManagedNpmSteelEnginePeerLinks.mockResolvedValue(false);
    ...
    mocks.maybeRepairOpenAICodexAuthConfig.mockImplementation((cfg: SteelEngineConfig) => ({
      changes: [],
      config: cfg,
      warnings: [],
    }));
    ...
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementation((cfg: SteelEngineConfig) => ({
      config: cfg,
      changes: [],
    }));
    ...
    mocks.maybeRepairStaleConfiguredAuthOrders.mockImplementation(
      ({ cfg }: { cfg: SteelEngineConfig }) => ({ config: cfg, changes: [] }),
    );
    ...
    mocks.maybeRepairStalePluginConfig.mockImplementation((cfg: SteelEngineConfig) => ({
      config: cfg,
      changes: [],
    }));
  });
  ...
  it("repairs managed npm plugin drift before missing plugin install repair", async () => {
    ...
    mocks.maybeRepairManagedNpmSteelEnginePeerLinks.mockImplementation(async () => {
      events.push("steelengine-peer-links");
      return true;
    });
    ...
    expect(events).toEqual(["bundled-shadow-cleanup", "steelengine-peer-links", "missing-installs"]);
    ...
    expect(mocks.maybeRepairManagedNpmSteelEnginePeerLinks).toHaveBeenCalledOnce();
    const peerLinkCall = mocks.maybeRepairManagedNpmSteelEnginePeerLinks.mock.calls[0]?.[0];
  ...
  it("repairs stale OAuth shadows before importing and removing auth JSON", async () => {
    ...
    const result = await runDoctorRepairSequence({
      state: {
        ...
        candidate: {} as SteelEngineConfig,
        pendingChanges: false,
        fixHints: [],
      },
      doctorFixCommand: "steelengine doctor --fix",
  ...
  it("auto-enables newly installed configured plugins after doctor repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: ['Installed missing configured plugin "brave" from @steelengine/brave-plugin.'],
      warnings: [],
    });
    mocks.applyPluginAutoEnable.mockImplementationOnce((params: { config: SteelEngineConfig }) => ({
      config: {
        ...params.config,
        plugins: {
    ...
    expect(result.changeNotes).toStrictEqual([
      'Installed missing configured plugin "brave" from @steelengine/brave-plugin.',
  ...
  it("explicitly enables plugins repaired from env-only configuration", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: ['Installed missing configured plugin "exa" from @steelengine/exa-plugin.'],
      warnings: [],
    ...
    expect(result.changeNotes).toStrictEqual([
      'Installed missing configured plugin "exa" from @steelengine/exa-plugin.',
  ...
  it("surfaces ClawHub notices from successful missing configured plugin repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      ...
      warnings: [],
      notices: [
        'ClawHub trust warning for "@steelengine/brave-plugin@1.2.3": scan=pending; reasons=pending.',
      ],
    });
    mocks.maybeRepairStalePluginConfig.mockImplementationOnce((cfg: SteelEngineConfig) => ({
      config: {
        ...cfg,
        plugins: {
          ...cfg.plugins,
  ...
  it("moves legacy Codex routes to canonical OpenAI before missing plugin install repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockImplementationOnce(
      async (params: { cfg: SteelEngineConfig }) => {
  ...
  it("runs group allowFrom fallback migration after open-policy allowFrom repair", async () => {
    const events: string[] = [];
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementationOnce((cfg: SteelEngineConfig) => {
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementationOnce((cfg: SteelEngineConfig) => {
  ...
  it("does not remove deferred configured plugins during the package update doctor pass", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [
        'Skipped package-manager repair for configured plugin "brave" during package update; rerun "steelengine doctor --fix" after the update completes.',
      ],
      warnings: [],
    });
    const result = await runDoctorRepairSequence({
  ...
  it("preserves configured plugins when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "brave" from @steelengine/brave-plugin: package install failed',
      ],
  ...
  it("preserves configured channels when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured channel plugin "whatsapp" from @steelengine/whatsapp: package install failed',
      ],
