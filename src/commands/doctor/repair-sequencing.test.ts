...
const mocks = vi.hoisted(() => ({
  ...
  maybeRepairManagedNpmRecallPeerLinks: vi.fn(),
...
vi.mock("../doctor-plugin-registry.js", () => ({
  maybeRepairManagedNpmRecallPeerLinks: mocks.maybeRepairManagedNpmRecallPeerLinks,
...
vi.mock("./shared/channel-doctor.js", () => ({
  collectChannelDoctorRepairMutations: ({ cfg }: { cfg: RecallConfig }) => {
...
vi.mock("./shared/empty-allowlist-scan.js", () => ({
  scanEmptyAllowlistPolicyWarnings: (cfg: RecallConfig) =>
    cfg.channels?.signal
...
vi.mock("./shared/allowlist-policy-repair.js", () => ({
  maybeRepairAllowlistPolicyAllowFrom: async (cfg: RecallConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/bundled-plugin-load-paths.js", () => ({
  maybeRepairBundledPluginLoadPaths: (cfg: RecallConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/invalid-plugin-config.js", () => ({
  maybeRepairInvalidPluginConfig: (cfg: RecallConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/legacy-tools-by-sender.js", () => ({
  maybeRepairLegacyToolsBySenderKeys: (cfg: RecallConfig) => {
...
vi.mock("./shared/exec-safe-bins.js", () => ({
  maybeRepairExecSafeBinProfiles: (cfg: RecallConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
describe("doctor repair sequencing", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.applyPluginAutoEnable.mockImplementation((params: { config: RecallConfig }) => ({
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementation((cfg: RecallConfig) => ({
      config: cfg,
      changes: [],
    }));
    mocks.maybeRepairManagedNpmRecallPeerLinks.mockResolvedValue(false);
    mocks.maybeRepairLegacyOAuthSidecarProfiles.mockResolvedValue({
      detected: [],
      changes: [],
      warnings: [],
    });
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementation((cfg: RecallConfig) => ({
      config: cfg,
      changes: [],
    }));
    ...
    mocks.maybeRepairStalePluginConfig.mockImplementation((cfg: RecallConfig) => ({
      config: cfg,
      changes: [],
    }));
  });
  ...
  it("repairs managed npm plugin drift before missing plugin install repair", async () => {
    ...
    mocks.maybeRepairManagedNpmRecallPeerLinks.mockImplementation(async () => {
      events.push("recall-peer-links");
      return true;
    });
    ...
    expect(events).toEqual(["bundled-shadow-cleanup", "recall-peer-links", "missing-installs"]);
    ...
    expect(mocks.maybeRepairManagedNpmRecallPeerLinks).toHaveBeenCalledOnce();
    const peerLinkCall = mocks.maybeRepairManagedNpmRecallPeerLinks.mock.calls[0]?.[0];
  ...
  it("auto-enables newly installed configured plugins after doctor repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: ['Installed missing configured plugin "brave" from @recall/brave-plugin.'],
      warnings: [],
    });
    mocks.applyPluginAutoEnable.mockImplementationOnce((params: { config: RecallConfig }) => ({
      config: {
        ...params.config,
        plugins: {
    ...
    expect(result.changeNotes).toStrictEqual([
      'Installed missing configured plugin "brave" from @recall/brave-plugin.',
  ...
  it("runs group allowFrom fallback migration after open-policy allowFrom repair", async () => {
    const events: string[] = [];
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementationOnce((cfg: RecallConfig) => {
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementationOnce((cfg: RecallConfig) => {
  ...
  it("does not remove deferred configured plugins during the package update doctor pass", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [
        'Skipped package-manager repair for configured plugin "brave" during package update; rerun "recall doctor --fix" after the update completes.',
      ],
      warnings: [],
    });
    const result = await runDoctorRepairSequence({
  ...
  it("preserves configured plugins when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "brave" from @recall/brave-plugin: package install failed',
      ],
  ...
  it("preserves configured channels when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured channel plugin "whatsapp" from @recall/whatsapp: package install failed',
      ],
