...
export function noteCronModelOverrides(params: {
  ...
  lines.push(
    `Review with ${formatCliCommand("steelengine cron list")} and ${formatCliCommand("steelengine cron show <job-id>")}; remove \`payload.model\` from jobs that should inherit the default.`,
  );
...
function collectCronDeliveryTargetAdvisory(params: {
  ...
  return [
    ...
    `Reactivate the channel plugin or update the job's \`delivery.channel\` after reviewing with ${formatCliCommand("steelengine cron list")} and ${formatCliCommand("steelengine cron show <job-id>")}.`,
  ].join("\n");
}
...
export async function collectLegacyWhatsAppCrontabHealthWarning(
  ...
  return [
    ...
    "`~/.steelengine/bin/ensure-whatsapp.sh` is not maintained by current SteelEngine and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
    `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("steelengine channels status --probe")}, ${formatCliCommand("steelengine doctor")}, and ${formatCliCommand("steelengine gateway status")} for current health checks.`,
