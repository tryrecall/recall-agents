...
describe("exportTrajectoryCommand", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.resolveDefaultSessionStorePath.mockReturnValue("/tmp/recall/sessions.json");
  ...
  it("points missing session key users at the sessions command", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      "--session-key is required. Run recall sessions to choose a session.",
    );
  ...
  it("preserves direct options when an encoded request omits them", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      "Session not found: agent:main:telegram:direct:123. Run recall sessions to see available sessions.",
    );
    expect(runtime.exit).toHaveBeenCalledWith(1);
  });
