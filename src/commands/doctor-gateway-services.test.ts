...
const originalParentSupportsGatewayRestart =
  process.env.STEELENGINE_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART;
const originalParentAllowsGatewayServiceRepair =
  process.env.STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR;
const originalParentAllowsGatewayActivation =
  process.env.STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION;
...
async function runRepair(cfg: SteelEngineConfig, options: { allowExecSecretRefs?: boolean } = {}) {
    ...
    issues: [
      {
        ...
        message: "Gateway service STEELENGINE_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    ...
    mocks.readWindowsProcessArgsSync.mockReturnValue(["node", "steelengine.mjs", "update"]);
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: SteelEngineConfig, env) => {
      const configToken =
        typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
      const envToken = env.STEELENGINE_GATEWAY_TOKEN?.trim() || undefined;
  ...
  afterEach(() => {
    ...
    if (originalParentSupportsGatewayRestart === undefined) {
      delete process.env.STEELENGINE_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART;
    } else {
      process.env.STEELENGINE_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART =
        originalParentSupportsGatewayRestart;
    }
    if (originalParentAllowsGatewayServiceRepair === undefined) {
      delete process.env.STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR;
    } else {
      process.env.STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR =
        originalParentAllowsGatewayServiceRepair;
    }
    if (originalParentAllowsGatewayActivation === undefined) {
      delete process.env.STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION;
    } else {
      process.env.STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION =
  ...
  it("reports the installed Gateway heap limit and derivation", async () => {
    const command = createGatewayCommand("/opt/steelengine/dist/index.js");
  ...
  it("does not duplicate gateway runtime warnings already emitted by the node install plan", async () => {
    ...
    mocks.readCommand.mockResolvedValue({
      programArguments: [nvmNode, "/usr/local/bin/steelengine", "gateway", "--port", "18789"],
      environment: {},
    });
  ...
  it("repairs gateway services whose pinned port differs from current config", async () => {
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      programArguments: ["/usr/bin/node", "/usr/local/bin/steelengine", "gateway", "--port", "18888"],
      workingDirectory: "/tmp",
      environment: {},
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
  ...
  it("uses STEELENGINE_GATEWAY_TOKEN when config token is missing", async () => {
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/steelengine/dist/index.js",
      installEntrypoint:
        "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/steelengine@2026.3.12/node_modules/steelengine/dist/index.js",
      realpath: async (value: string) => {
        const normalized = value.replaceAll("\\", "/").replace(/^[A-Z]:/i, "");
        if (normalized.includes("/global/5/node_modules/steelengine/")) {
          return normalized.replace(
            "/global/5/node_modules/steelengine/",
            "/global/5/node_modules/.pnpm/steelengine@2026.3.12/node_modules/steelengine/",
          );
        }
        return normalized;
      },
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/opt/steelengine/../steelengine/dist/index.js",
      installEntrypoint: "/opt/steelengine/dist/index.js",
  ...
  it("keeps wrapper-managed gateway services aligned during entrypoint drift checks", async () => {
    ...
    mocks.buildGatewayInstallPlan.mockImplementation(async ({ env }) => ({
      programArguments: [env.STEELENGINE_WRAPPER, "gateway", "--port", "18789"],
      environment: {
        STEELENGINE_WRAPPER: env.STEELENGINE_WRAPPER,
      },
    }));
    ...
    expect(requireRecord(installPlanOptions.env, "install env").STEELENGINE_WRAPPER).toBe(wrapperPath);
    expect(
    ...
    expect(mocks.note).toHaveBeenCalledWith(
      "Gateway service invokes STEELENGINE_WRAPPER: /usr/local/bin/steelengine-doppler",
      "Gateway",
    );
    expect(mocks.stage).not.toHaveBeenCalled();
    expect(mocks.install).not.toHaveBeenCalled();
  ...
  it("still flags entrypoint mismatch when canonicalized paths differ", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint:
        "/Users/test/.nvm/versions/node/v22.0.0/lib/node_modules/steelengine/dist/index.js",
      installEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/steelengine/dist/index.js",
    });
  ...
  it("skips entrypoint rewrites for an active systemd unit", async () => {
    ...
    mocks.readCommand.mockResolvedValue({
      ...createGatewayCommand("/opt/old-steelengine/dist/index.js"),
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      ...createGatewayCommand("/opt/new-steelengine/dist/index.js"),
      workingDirectory: "/tmp",
    });
  ...
  it("leaves all service metadata unchanged when an active unit has command drift plus other issues", async () => {
    mockProcessPlatform("linux");
    mocks.readCommand.mockResolvedValue({
      programArguments: ["/usr/bin/steelengine", "run"],
      environment: {},
      sourcePath: "/home/test/.config/systemd/user/steelengine-gateway.service",
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
      issues: [
  ...
  it("skips entrypoint rewrite in non-interactive fix mode", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/npm/node_modules/steelengine/dist/entry.js",
      installEntrypoint: "/Users/test/Library/npm/node_modules/steelengine/dist/index.js",
    ...
    expectNoteContaining("steelengine gateway install --force", "Gateway service config");
    expect(mocks.stage).not.toHaveBeenCalled();
    expect(mocks.install).not.toHaveBeenCalled();
  });
  ...
  it("defers systemd service config rewrites during non-interactive update repairs", async () => {
    mockProcessPlatform("linux");
    process.env.STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "1";
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/npm/node_modules/steelengine/dist/entry.js",
      installEntrypoint: "/Users/test/Library/npm/node_modules/steelengine/dist/index.js",
      installWorkingDirectory: "/tmp",
  ...
  it.each([
    ["update command", ["node", "steelengine.mjs", "update"]],
    ["--update shorthand", ["node", "steelengine.mjs", "--update"]],
    ["doctor update prompt", ["node", "steelengine.mjs", "doctor"]],
    ...
    mocks.readCommand.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      environment: {
        STEELENGINE_SERVICE_VERSION: "2026.5.25",
        STEELENGINE_WINDOWS_TASK_NAME: "SteelEngine Gateway Work",
      },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
      issues: [
        {
          ...
          message: "Gateway service was installed by an older SteelEngine version.",
          level: "recommended",
        },
      ],
    });
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      ...
      workingDirectory: "/tmp",
      environment: {
        STEELENGINE_SERVICE_VERSION: "2026.5.26",
      },
    });
    ...
    expectNoteContaining(
      "Gateway service was installed by an older SteelEngine version.",
      "Gateway service config",
    );
    expect(mocks.stage).not.toHaveBeenCalled();
    expect(mocks.install).not.toHaveBeenCalled();
  ...
  it("stages running Windows repairs when the update parent forbids activation", async () => {
    ...
    process.env.STEELENGINE_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE = "1";
    process.env.STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "1";
    process.env.STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION = "0";
    mocks.readCommand.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      environment: {
        STEELENGINE_SERVICE_VERSION: "2026.5.25",
  ...
  it("does not rewrite a service when the update parent rejects ownership", async () => {
    mockProcessPlatform("win32");
    process.env.STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "0";
    process.env.STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION = "0";
    mocks.readCommand.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      environment: {
  ...
  it.each([
    {
      ...
      args: ["node", "steelengine.mjs", "update", "--no-restart"],
    },
    {
    ...
    },
    {
      ...
      args: ["node", "steelengine.mjs", "update", "wizard"],
    },
    {
    ...
    },
    {
      ...
      args: ["node", "steelengine.mjs", "gateway"],
    },
    ...
    mocks.readCommand.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      environment: { STEELENGINE_SERVICE_VERSION: "2026.5.25" },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
      issues: [
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      workingDirectory: "/tmp",
      environment: { STEELENGINE_SERVICE_VERSION: "2026.5.26" },
    });
    mocks.readRuntime.mockResolvedValue({ status: "running" });
  ...
  it("persists embedded service tokens before Windows update repairs rewrite the task", async () => {
    ...
    process.env.STEELENGINE_UPDATE_IN_PROGRESS = "1";
    process.env.STEELENGINE_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE = "1";
    process.env.STEELENGINE_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART = "1";
    process.env.STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "1";
    process.env.STEELENGINE_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION = "1";
  ...
  it("reports service config drift but skips service rewrite when service repair policy is external", async () => {
    await withEnvAsync({ STEELENGINE_SERVICE_REPAIR_POLICY: "external" }, async () => {
      setupGatewayEntrypointRepairScenario({
        currentEntrypoint: "/Users/test/Library/npm/node_modules/steelengine/dist/entry.js",
        installEntrypoint: "/Users/test/Library/npm/node_modules/steelengine/dist/index.js",
        installWorkingDirectory: "/tmp",
  ...
  it("warns when the gateway service entrypoint resolves to a source checkout", async () => {
    await withEnvAsync({}, async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-doctor-service-layout-"));
      try {
  ...
  it("does not duplicate Gateway service config panels for a source-checkout entrypoint with audit findings", async () => {
    await withEnvAsync({}, async () => {
      const root = await fs.mkdtemp(
        path.join(os.tmpdir(), "steelengine-doctor-service-config-dedup-"),
      );
      try {
        await fs.mkdir(path.join(root, ".git"), { recursive: true });
        await fs.mkdir(path.join(root, "src"), { recursive: true });
        ...
        const installEntrypoint = "/usr/local/lib/node_modules/steelengine/dist/index.js";
        setupGatewayEntrypointRepairScenario({
        ...
        const forceMatches = consolidated.match(/steelengine gateway install --force/g) ?? [];
  ...
  it("keeps the gateway install force hint when a source-checkout warning is suppressed and repair is declined", async () => {
    await withEnvAsync({}, async () => {
      const root = await fs.mkdtemp(
        path.join(os.tmpdir(), "steelengine-doctor-service-config-force-hint-"),
      );
      try {
        await fs.mkdir(path.join(root, ".git"), { recursive: true });
        await fs.mkdir(path.join(root, "src"), { recursive: true });
        ...
        expect(gatewayServiceConfigNotes[1]?.[0]).toContain("steelengine gateway install --force");
      } finally {
        await fs.rm(root, { recursive: true, force: true });
      }
    });
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("keeps legacy gateway services warning-level", () => {
    expect(
      extraGatewayServiceToHealthFinding({
        platform: "linux",
        label: "steelengine-gateway.service",
        detail: "legacy unit",
        scope: "user",
        legacy: true,
      }),
      ...
      expect.objectContaining({
        ...
        severity: "warning",
        source: "linux",
        target: "steelengine-gateway.service",
      }),
    );
  });
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing SteelEngine gateway next.",
    );
  });
