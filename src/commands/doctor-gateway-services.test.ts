...
vi.mock("../daemon/service-audit.js", () => ({
  ...
  readEmbeddedGatewayToken: (
    ...
    command?.environmentValueSources?.RECALL_GATEWAY_TOKEN === "file"
      ? undefined
      : command?.environment?.RECALL_GATEWAY_TOKEN?.trim() || undefined,
  SERVICE_AUDIT_CODES: {
...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/Library/pnpm/global/5/node_modules/recall/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/opt/recall/../recall/dist/index.js",
        "gateway",
        "--port",
        "18789",
      ],
