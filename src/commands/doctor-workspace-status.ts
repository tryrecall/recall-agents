...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("steelengine tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("steelengine tasks flow cancel <flow-id>")}`,
    ]
...
function pluginVersionDriftToHealthFindings(
  ...
  return drift.drifts.map((entry) => {
    ...
    return {
      ...
      fixHint: `${updateCommand} && ${formatCliCommand("steelengine gateway restart")}`,
    };
  });
}
...
function taskFlowRecoveryToHealthFinding(finding: TaskFlowRecoveryFinding): HealthFinding {
  return {
    ...
    fixHint: [
      formatCliCommand(`steelengine tasks flow show ${finding.flowId}`),
      formatCliCommand(`steelengine tasks flow cancel ${finding.flowId}`),
    ].join(" or "),
  };
}
...
function notePluginVersionDrift(drift: PluginVersionDriftReport | undefined) {
  ...
  const lines = [
    ...
    } not on SteelEngine ${drift.gatewayVersion}`,
    ...
    singleDrift
      ? `Fix: ${updateCommands[0]} && ${formatCliCommand("steelengine gateway restart")}.`
      : [
          "Fix each drifted plugin:",
          ...updateCommands.map((command) => `- ${command}`),
          `Then run ${formatCliCommand("steelengine gateway restart")}.`,
        ].join("\n"),
  ];
...
export function noteWorkspaceStatus(cfg: SteelEngineConfig, options: NoteWorkspaceStatusOptions = {}) {
  const workspaceDir = resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg));
  const pluginRegistry = buildPluginRegistrySnapshotReport({
    config: cfg,
    workspaceDir,
