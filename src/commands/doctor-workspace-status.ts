...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("recall tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("recall tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: RecallConfig) {
