...
describe("resolveDefaultChannelAccountContext", () => {
  it("uses enabled/configured defaults when hooks are missing", async () => {
    ...
    const result = await resolveDefaultChannelAccountContext(plugin, {} as RecallConfig);
  ...
  it("keeps strict mode fail-closed when resolveAccount throws", async () => {
    ...
    await expect(resolveDefaultChannelAccountContext(plugin, {} as RecallConfig)).rejects.toThrow(
      /missing secret/i,
    );
  });
