...
describe("maybeRepairSandboxImages", () => {
  ...
  function createSandboxConfig(mode: "off" | "all" | "non-main"): SteelEngineConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
  ...
  function createSandboxConfigWithDockerNetwork(network: string): SteelEngineConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
...
describe("maybeRepairSandboxRegistryFiles", () => {
  ...
  it("warns about legacy registry files without migrating outside doctor --fix", async () => {
    inspectLegacySandboxRegistryFiles.mockResolvedValue([
      {
        kind: "containers",
        registryPath: "/tmp/steelengine/sandbox/containers.json",
        shardedDir: "/tmp/steelengine/sandbox/containers",
        source: "monolithic",
        exists: true,
        valid: true,
        entries: 2,
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "- containers monolithic: /tmp/steelengine/sandbox/containers.json (2 entries)",
        "Run steelengine doctor --fix to migrate them to SQLite.",
      ].join("\n"),
      "Sandbox",
    );
  });
  ...
  it("maps legacy registry files to structured findings and dry-run effects", () => {
    ...
    expect(legacySandboxRegistryInspectionToHealthFinding(monolithicFile)).toEqual(
      expect.objectContaining({
        ...
        severity: "warning",
        path: "/tmp/steelengine/sandbox/containers.json",
    ...
    expect(legacySandboxRegistryInspectionToRepairEffect(monolithicFile)).toEqual({
      ...
      target: "/tmp/steelengine/sandbox/containers.json",
      dryRunSafe: false,
    });
    expect(legacySandboxRegistryInspectionToHealthFinding(shardedFile)).toEqual(
      expect.objectContaining({
        path: "/tmp/steelengine/sandbox/containers",
        message: expect.stringContaining(
          "- containers sharded: /tmp/steelengine/sandbox/containers (2 entries)",
        ),
      }),
    );
    expect(legacySandboxRegistryInspectionToRepairEffect(shardedFile)).toEqual(
      expect.objectContaining({
        target: "/tmp/steelengine/sandbox/containers",
      }),
    );
  });
  ...
  it("maps invalid legacy registry files to quarantine effects", () => {
    expect(
      legacySandboxRegistryInspectionToRepairEffect({
        kind: "browsers",
        registryPath: "/tmp/steelengine/sandbox/browsers.json",
        shardedDir: "/tmp/steelengine/sandbox/browsers",
        source: "monolithic",
        exists: true,
        valid: false,
        entries: 0,
      ...
      expect.objectContaining({
        ...
        target: "/tmp/steelengine/sandbox/browsers.json",
      }),
    );
  });
