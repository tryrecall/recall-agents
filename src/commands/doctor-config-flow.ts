...
function collectConfiguredChannelIds(cfg: RecallConfig): string[] {
  const channels =
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: RecallConfig = baseCfg;
  let candidate = structuredClone(baseCfg);
  let pendingChanges = false;
  let fixHints: string[] = [];
  const doctorFixCommand = formatCliCommand("recall doctor --fix");
  ...
  if (hasLegacyInternalHookHandlers(snapshot.parsed)) {
    note(
      [
        ...
        "- recall doctor --fix does not rewrite this shape automatically.",
      ].join("\n"),
      "Legacy config keys detected",
    );
  }
