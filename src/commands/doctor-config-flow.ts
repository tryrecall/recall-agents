...
function collectUnsupportedInternalHookEntryWarnings(cfg: SteelEngineConfig): string[] {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: SteelEngineConfig = baseCfg;
  let candidate = structuredClone(baseCfg);
  let pendingChanges = false;
  let fixHints: string[] = [];
  let shouldRepairCronCodexModelRefsAfterConfigWrite = false;
  const doctorFixCommand = formatCliCommand("steelengine doctor --fix");
  ...
  if (hasLegacyInternalHookHandlers(snapshot.parsed)) {
    note(
      [
        ...
        "- steelengine doctor --fix does not rewrite this shape automatically.",
      ].join("\n"),
      "Legacy config keys detected",
    );
  }
