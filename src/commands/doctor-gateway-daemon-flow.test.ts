...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.steelengine.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.steelengine.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        STEELENGINE_STATE_DIR: "/tmp/steelengine-service",
        STEELENGINE_CONFIG_PATH: "/tmp/steelengine-service/steelengine.json",
      },
    });
    ...
    const [handoffEnv] = readGatewayRestartHandoffSync.mock.calls[0] as unknown as [
      { STEELENGINE_STATE_DIR?: string; STEELENGINE_CONFIG_PATH?: string },
    ];
    expect(handoffEnv?.STEELENGINE_STATE_DIR).toBe("/tmp/steelengine-service");
    expect(handoffEnv?.STEELENGINE_CONFIG_PATH).toBe("/tmp/steelengine-service/steelengine.json");
    expect(note).toHaveBeenCalledWith(
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "steelengine-gateway", address: "0.0.0.0:18789" }];
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners,
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      status: "busy",
      listeners: [
        { pid: 5001, commandLine: "steelengine-gateway", address: "0.0.0.0:18789" },
        { pid: 5002, commandLine: "steelengine-gateway", address: "127.0.0.1:18789" },
      ],
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      `Run ${formatCliCommand("steelengine gateway install")} when you want to install the gateway service.`,
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system SteelEngine gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        ...
        detail: "unit: /etc/systemd/system/steelengine-gateway.service",
        scope: "system",
        marker: "steelengine",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        "System-level SteelEngine gateway service detected while the user gateway service is not installed.",
        "- steelengine-gateway.service (unit: /etc/systemd/system/steelengine-gateway.service)",
        "SteelEngine will not install a second user-level gateway service automatically.",
        "Run `steelengine gateway status --deep` or `steelengine doctor --deep` to inspect duplicate services.",
