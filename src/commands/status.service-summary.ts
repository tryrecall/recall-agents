...
export async function readServiceStatusSummary(
  ...
  try {
    ...
    const managedBySteelEngine = state.installed;
    // A running unmanaged process still counts as installed for status display.
    const externallyManaged = !managedBySteelEngine && state.running;
    const installed = managedBySteelEngine || externallyManaged;
