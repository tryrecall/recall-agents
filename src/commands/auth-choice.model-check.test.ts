...
describe("warnIfModelConfigLooksOff", () => {
  ...
  it("skips catalog validation when requested while keeping auth checks", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      'No auth configured for provider "openai". The agent may fail until credentials are added. Run `steelengine models auth login --provider openai`, `steelengine configure`, or set an API key env var.',
      "Model check",
    );
  });
  ...
  it("accepts subscription auth but not key sources for gpt-5.3-codex-spark", async () => {
    ...
    expect(warning).toContain("steelengine models auth login --provider openai");
