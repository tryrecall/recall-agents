/**
 * Top-level `steelengine onboard` command entrypoint.
 *
 ...
 * dispatch; the command layer routes it through the inference-gated SteelEngine.
 */
...
export async function setupWizardCommand(
  ...
  if (
    ...
    runtime.error(
      `Invalid --secret-input-mode. Use "plaintext" or "ref", or run ${formatCliCommand("steelengine onboard")} for the interactive setup.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (normalizedOpts.resetScope && !VALID_RESET_SCOPES.has(normalizedOpts.resetScope)) {
    runtime.error(
      `Invalid --reset-scope. Use "config", "config+creds+sessions", or "full". Run ${formatCliCommand("steelengine onboard --reset --reset-scope config")} for a config-only reset.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    ...
    runtime.error(
      [
        ...
        "Read: https://docs.steelengine.ai/security",
        `Re-run with: ${formatCliCommand("steelengine onboard --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    runtime.exit(1);
    return;
  ...
  if (process.platform === "win32") {
    runtime.log(
      [
        "Windows detected - SteelEngine runs great on WSL2!",
        "Native Windows might be trickier.",
        "Quick setup: wsl --install (one command, one reboot)",
        "Guide: https://docs.steelengine.ai/windows",
      ].join("\n"),
    );
  }
