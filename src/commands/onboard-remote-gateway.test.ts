...
function makeLocalConfig(): SteelEngineConfig {
  return {
...
describe("runRemoteGatewayInferenceOnboarding", () => {
  it.each([
    ...
    "pins $label across detect, activate, verify, SteelEngine, and in-process TUI",
    async ({ auth, secret }) => {
  ...
  ])("fails closed on $label before SteelEngine", async ({ verification, error }) => {
    const localConfig = makeLocalConfig();
    const localConfigBefore = structuredClone(localConfig);
  ...
  it("does not advance or fall back locally after an ambiguous activation request failure", async () => {
    ...
    expect(methods).toEqual(["steelengine.setup.detect", "steelengine.setup.activate"]);
    expect(runTui).not.toHaveBeenCalled();
  });

  it("treats a cancelled remote SteelEngine conversation as a pause without opening the agent", async () => {
    const methods: string[] = [];
    const callGatewayMock = vi.fn(async (options: CallGatewayCliOptions): Promise<unknown> => {
      methods.push(options.method);
      if (options.method === "steelengine.setup.detect") {
    ...
    expect(prompter.outro).toHaveBeenCalledWith("SteelEngine setup paused.");
    expect(runTui).not.toHaveBeenCalled();
  });
});
