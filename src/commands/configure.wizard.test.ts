...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    promptRemoteGatewayConfig: vi.fn(async (cfg: RecallConfig) => ({
      ...cfg,
      gateway: { mode: "remote", remote: { url: "wss://gateway.example.test" } },
    })),
    isCodexNativeWebSearchRelevant: vi.fn(({ config }: { config: RecallConfig }) =>
      Boolean(config.auth?.profiles?.["openai-codex:default"]),
    ),
    setupChannels: vi.fn(async (cfg: RecallConfig) => cfg),
  };
});
...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.tryrecall/recall-agents.json",
...
function setupBaseWizardState(config: RecallConfig = {}) {
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.setupSearch.mockImplementation(async (cfg: RecallConfig) => cfg);
    mocks.promptAuthConfig.mockReset();
    mocks.promptAuthConfig.mockImplementation(async (cfg: RecallConfig) => cfg);
    mocks.promptGatewayConfig.mockReset();
    mocks.promptGatewayConfig.mockImplementation(async (cfg: RecallConfig) => ({
      config: cfg,
      port: 18789,
    }));
  });
  ...
  it("persists provider-owned web search config changes returned by setupSearch", async () => {
    setupBaseWizardState();
    mocks.setupSearch.mockImplementation(async (cfg: RecallConfig) =>
  ...
  it("retries without dropping nested plugin config written during wizard flow (issue #64188)", async () => {
    ...
    expect(String(defaults.workspace)).toContain("/.recall/workspace");
