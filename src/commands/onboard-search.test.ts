...
function getWebSearchConfig(config: RecallConfig | undefined, pluginId: string) {
  return (config as WebSearchConfigRecord | undefined)?.plugins?.entries?.[pluginId]?.config
    ?.webSearch;
}

function ensureWebSearchConfig(config: RecallConfig, pluginId: string) {
...
function createSearchProviderEntry(id: string): PluginWebSearchProviderEntry {
  ...
  const entry: PluginWebSearchProviderEntry = {
    ...
    applySelectionConfig: (config) => {
      const next: RecallConfig = { ...config, plugins: { ...config.plugins } };
      const entries = { ...next.plugins?.entries } as NonNullable<
        NonNullable<RecallConfig["plugins"]>["entries"]
      >;
...
function createPerplexityConfig(apiKey: string, enabled?: boolean): RecallConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: RecallConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): RecallConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: RecallConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: RecallConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
