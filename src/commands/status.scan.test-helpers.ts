...
type StatusScanSharedMocks = {
  ...
  resolveGatewayPort: Mock<(cfg?: SteelEngineConfig) => number>;
...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/steelengine-${configPathLabel}-missing-${process.pid}.json`),
    resolveGatewayPort: vi.fn((cfg?: SteelEngineConfig) => cfg?.gateway?.port ?? 18789),
...
export function createStatusScanConfig<T extends object = SteelEngineConfig>(
  overrides: T = {} as T,
): SteelEngineConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): SteelEngineConfig {
