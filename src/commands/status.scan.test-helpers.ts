...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/recall-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = RecallConfig>(
  overrides: T = {} as T,
): RecallConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): RecallConfig {
