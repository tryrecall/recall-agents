...
beforeAll(() => {
  envSnapshot = captureEnv(["RECALL_PROFILE"]);
  process.env.RECALL_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByRecall: Boolean(command),
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/recall/pnpm-lock.yaml",
        markerPath: "/tmp/recall/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.RECALL_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.recall.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.recall.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.RECALL_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/recall-root.js", () => ({
  resolveRecallPackageRoot: vi.fn().mockResolvedValue("/tmp/recall"),
  resolveRecallPackageRootSync: vi.fn(() => "/tmp/recall"),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expectLogsMatch(logs, /recall (?:--profile isolated )?status --all/);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("recall --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("RECALL_GATEWAY_TOKEN", "abcd1234", async () => {
