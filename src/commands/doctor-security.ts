...
import type { RecallConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: RecallConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: RecallConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Recall default (full)";
  const defaultRequestedAskSource = "Recall default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("recall approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: RecallConfig): string[] {
  void cfg;
  return [];
}

function collectExecFilesystemPolicyWarnings(cfg: RecallConfig): string[] {
...
function collectPlaintextConfigSecretWarnings(cfg: RecallConfig): string[] {
  ...
  return [
    "- WARNING: recall.json contains plaintext secret-bearing config fields.",
    `  Paths: ${pathLine}`,
    "  Agents or workspace tools that can read config files may see these API keys/tokens.",
    `  Migrate them to SecretRefs with ${formatCliCommand("recall secrets configure")} or ${formatCliCommand("recall secrets apply")}, then verify with ${formatCliCommand("recall secrets audit --check")}.`,
  ];
}
...
export async function collectSecurityWarnings(
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      "  Host exec gating still comes from ~/.recall/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("recall approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.recall.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("recall configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("recall config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("recall doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("recall config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('recall config set session.dmScope "per-channel-peer"') +
...
export async function noteSecurityWarnings(cfg: RecallConfig) {
  const warnings = await collectSecurityWarnings(cfg);
  const auditHint = `- Run: ${formatCliCommand("recall security audit --deep")}`;
