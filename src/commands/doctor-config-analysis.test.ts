...
describe("doctor config analysis helpers", () => {
  ...
  it("preserves user-authored model and agent metadata during unknown-key cleanup", () => {
    ...
    expect(RecallSchema.safeParse({ defaultModel: "minimax/MiniMax-M2.7" }).success).toBe(false);
  ...
  describe("stripUnknownConfigKeys during update", () => {
    const originalEnv = process.env.RECALL_UPDATE_IN_PROGRESS;
    ...
    afterEach(() => {
      if (originalEnv !== undefined) {
        process.env.RECALL_UPDATE_IN_PROGRESS = originalEnv;
      } else {
        delete process.env.RECALL_UPDATE_IN_PROGRESS;
      }
    });

    it("returns input unchanged when RECALL_UPDATE_IN_PROGRESS=1", () => {
    ...
    it("returns input unchanged when RECALL_UPDATE_IN_PROGRESS=true", () => {
      process.env.RECALL_UPDATE_IN_PROGRESS = "true";
      const input = { hooks: {}, unexpected: true } as never;
      const result = stripUnknownConfigKeys(input);
      expect(result.config).toBe(input);
      expect(result.removed).toEqual([]);
...
describe("collectImplicitFallbackClobberWarnings", () => {
  function buildConfig(overrides: { defaults?: unknown; list?: unknown[] }): RecallConfig {
    return {
      agents: {
