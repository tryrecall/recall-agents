...
function collectImplicitFallbackClobberWarnings(cfg: SteelEngineConfig): string[] {
  noteMock.mockClear();
...
describe("doctor config analysis helpers", () => {
  ...
  it("preserves user-authored model and agent metadata during unknown-key cleanup", () => {
    ...
    expect(SteelEngineSchema.safeParse({ defaultModel: "minimax/MiniMax-M2.7" }).success).toBe(false);
  ...
  describe("stripUnknownConfigKeys during update", () => {
    const originalEnv = process.env.STEELENGINE_UPDATE_IN_PROGRESS;
    ...
    afterEach(() => {
      if (originalEnv !== undefined) {
        process.env.STEELENGINE_UPDATE_IN_PROGRESS = originalEnv;
      } else {
        delete process.env.STEELENGINE_UPDATE_IN_PROGRESS;
      }
    });

    it("returns input unchanged when STEELENGINE_UPDATE_IN_PROGRESS=1", () => {
    ...
    it("returns input unchanged when STEELENGINE_UPDATE_IN_PROGRESS=true", () => {
      process.env.STEELENGINE_UPDATE_IN_PROGRESS = "true";
      const input = { hooks: {}, unexpected: true } as never;
      const result = stripUnknownConfigKeys(input);
      expect(result.config).toBe(input);
      expect(result.removed).toEqual([]);
...
describe("collectImplicitFallbackClobberWarnings", () => {
  function buildConfig(overrides: { defaults?: unknown; list?: unknown[] }): SteelEngineConfig {
    return {
      agents: {
