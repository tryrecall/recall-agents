...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("recall security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("recall status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("recall tasks maintenance --apply")}`,
