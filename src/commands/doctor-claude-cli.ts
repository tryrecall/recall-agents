...
function usesClaudeCliModelSelection(cfg: RecallConfig): boolean {
...
function resolveClaudeCliCommand(cfg: RecallConfig): string {
...
function formatWorkspaceHealthLine(
  ...
  if (health === "missing") {
    return `- ${label}: ${display} (missing; Recall will create it on first run).`;
  }
...
function resolveClaudeCliAgentIds(cfg: RecallConfig): string[] {
...
export function noteClaudeCliHealth(
  ...
  if (!storedProfile) {
    lines.push(`- Recall auth profile: missing (${CLAUDE_CLI_PROFILE_ID}) in ${authStorePath}.`);
    fixHints.push(
    ...
    lines.push(
      `- Recall auth profile: ${CLAUDE_CLI_PROFILE_ID} is wired to provider "${storedProfile.provider}" instead of "${CLAUDE_CLI_PROVIDER}".`,
    );
    fixHints.push(
    ...
    lines.push(
      `- Recall auth profile: ${CLAUDE_CLI_PROFILE_ID} (provider ${CLAUDE_CLI_PROVIDER}).`,
    );
  }
