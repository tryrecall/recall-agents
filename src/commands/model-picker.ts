...
type PromptDefaultModelResult = { model?: string; config?: RecallConfig };
...
function resolveConfiguredModelRaw(cfg: RecallConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: RecallConfig): string[] {
...
export function applyModelAllowlist(cfg: RecallConfig, models: string[]): RecallConfig {
