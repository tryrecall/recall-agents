...
describe("setupChannels", () => {
  ...
  it("renders the QuickStart channel picker without requiring the LINE runtime", async () => {
    ...
    await expect(
      runSetupChannels({} as RecallConfig, prompter, {
        quickstartDefaults: true,
      }),
    ).resolves.toEqual({} as RecallConfig);
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@recall/msteams-plugin": { enabled: true },
          },
        },
      } as RecallConfig,
      prompter,
