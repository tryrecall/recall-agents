...
export function hasExplicitMemorySearchConfig(cfg: RecallConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: RecallConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: RecallConfig, agentId: string) => { store: { path: string } } | null;
