...
const testConfigStore = new Map<string, RecallConfig>();
...
function resolveTestConfigPath() {
  const override = process.env.RECALL_CONFIG_PATH?.trim();
  if (override) {
    return override;
  }
  const stateDir = process.env.RECALL_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("RECALL_STATE_DIR must be set before config IO in this test");
  }
  return path.join(stateDir, "recall.json");
}
...
function readTestConfig<T = RecallConfig>(): T {
...
vi.mock("../config/config.js", () => ({
  replaceConfigFile: async ({ nextConfig }: { nextConfig: RecallConfig }) => {
    testConfigStore.set(resolveTestConfigPath(), nextConfig);
  },
  resolveGatewayPort: (cfg: RecallConfig) => cfg.gateway?.port ?? 18789,
}));
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  it("classifies daemon health ECONNREFUSED failures with a recovery command", async () => {
    await withStateDir("state-local-daemon-health-refused-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Fix: run `recall gateway restart`.");
    });
  }, 60_000);
