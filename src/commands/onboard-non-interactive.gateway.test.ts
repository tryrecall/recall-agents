...
const testConfigStore = new Map<string, SteelEngineConfig>();
...
function resolveTestConfigPath() {
  const override = process.env.STEELENGINE_CONFIG_PATH?.trim();
  if (override) {
    return override;
  }
  const stateDir = process.env.STEELENGINE_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("STEELENGINE_STATE_DIR must be set before config IO in this test");
  }
  return path.join(stateDir, "steelengine.json");
}
...
function readTestConfig<T = SteelEngineConfig>(): T {
...
vi.mock("../config/config.js", () => ({
  ...
  resolveGatewayPort: (cfg: SteelEngineConfig) => cfg.gateway?.port ?? 18789,
}));
    ...
    runNonInteractiveSetup(
      {
        nonInteractive: true,
        mode: "local",
        workspace: path.join(stateDir, "steelengine"),
        authChoice: "skip",
        skipSkills: true,
        skipHealth: false,
        installDaemon: true,
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("steelengine-onboard-");
  ...
  it("preserves existing agents.list and bindings on onboard rerun (steelengine#84692)", async () => {
    await withStateDir("state-preserve-agents-", async (stateDir) => {
      const workspace = path.join(stateDir, "steelengine");
      const seededAgents = [
  ...
  it("preserves existing agents.list and bindings on remote onboard rerun (steelengine#84692)", async () => {
  ...
  it("explains local health failure when no daemon was requested", async () => {
    await withStateDir("state-local-health-hint-", async (stateDir) => {
      ...
      await expect(
        ...
        /only waits for an already-running gateway unless you pass `--install-daemon` to `steelengine onboard`[\s\S]*steelengine onboard --install-daemon[\s\S]*steelengine onboard --skip-health/,
      );
    });
  }, 60_000);
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `steelengine gateway status --deep` for more detail.");
  ...
  it("classifies daemon health ECONNREFUSED failures with a recovery command", async () => {
    await withStateDir("state-local-daemon-health-refused-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Fix: run `steelengine gateway restart`.");
    });
  }, 60_000);
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      setTestEnvValue("STEELENGINE_STATE_DIR", stateDir);
      setTestEnvValue("STEELENGINE_CONFIG_PATH", path.join(stateDir, "steelengine.json"));
