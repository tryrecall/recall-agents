...
import { listSteelEngineRegisteredAgentDatabases } from "../state/steelengine-agent-db.js";
...
function collectSqliteBloatWarnings(deps?: { env?: NodeJS.ProcessEnv }): string[] {
  ...
  const statePath = resolveSteelEngineStateSqlitePath(env);
  ...
  for (const registered of listSteelEngineRegisteredAgentDatabases({ env })) {
