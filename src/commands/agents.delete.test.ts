...
function resolveFixtureStoreAgentId(cfg: SteelEngineConfig, deletedAgentId: string): string {
...
describe("agents delete command", () => {
  ...
  it("routes deletion through the Gateway when reachable", async () => {
    await withStateDirEnv("steelengine-agents-delete-gateway-", async ({ stateDir }) => {
      const now = Date.now();
  ...
  it("falls back to local deletion when the optional Gateway probe needs credentials", async () => {
    await withStateDirEnv("steelengine-agents-delete-gateway-auth-", async ({ stateDir }) => {
      const now = Date.now();
      const cfg: SteelEngineConfig = {
        agents: {
          list: [
  ...
  it("purges deleted agent entries from the session store", async () => {
    await withStateDirEnv("steelengine-agents-delete-", async ({ stateDir }) => {
      const now = Date.now();
      const cfg: SteelEngineConfig = {
        agents: {
          list: [
  ...
  it("deletes workspace state after local workspace removal", async () => {
    await withStateDirEnv("steelengine-agents-delete-workspace-state-", async ({ stateDir }) => {
  ...
  it("finishes agent-directory cleanup when workspace state deletion fails", async () => {
    await withStateDirEnv("steelengine-agents-delete-state-failure-", async ({ stateDir }) => {
      const opsWorkspace = path.join(stateDir, "workspace-ops");
  ...
  it("purges legacy main-alias entries owned by the deleted default agent", async () => {
    await withStateDirEnv("steelengine-agents-delete-main-alias-", async ({ stateDir }) => {
      const now = Date.now();
      const cfg: SteelEngineConfig = {
        agents: {
  ...
  it("preserves shared-store legacy default keys when deleting another agent", async () => {
    await withStateDirEnv("steelengine-agents-delete-shared-store-", async ({ stateDir }) => {
      const now = Date.now();
      const cfg: SteelEngineConfig = {
  ...
  it("skips workspace removal when another agent shares the same workspace (#70890)", async () => {
    await withStateDirEnv("steelengine-agents-delete-shared-workspace-", async ({ stateDir }) => {
  ...
  it("skips workspace removal when another agent workspace overlaps a child path (#70890)", async () => {
    await withStateDirEnv("steelengine-agents-delete-overlapping-workspace-", async ({ stateDir }) => {
      const sharedWorkspace = path.join(stateDir, "workspace-shared");
  ...
  it("skips workspace removal when deleting a parent workspace that contains another agent workspace (#70890)", async () => {
    await withStateDirEnv("steelengine-agents-delete-parent-workspace-", async ({ stateDir }) => {
      const sharedWorkspace = path.join(stateDir, "workspace-shared");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withStateDirEnv("steelengine-agents-delete-symlink-workspace-", async ({ stateDir }) => {
  ...
  it("trashes workspace when no other agent shares it", async () => {
    await withStateDirEnv("steelengine-agents-delete-unique-workspace-", async ({ stateDir }) => {
      const opsWorkspace = path.join(stateDir, "workspace-ops");
  ...
  it("retains workspace state when workspace trash fails", async () => {
    await withStateDirEnv("steelengine-agents-delete-trash-failure-", async ({ stateDir }) => {
      const opsWorkspace = path.join(stateDir, "workspace-ops");
      const cfg: SteelEngineConfig = {
        agents: {
          list: [
