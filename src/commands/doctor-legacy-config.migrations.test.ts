...
vi.mock("../plugins/setup-registry.js", () => ({
  runPluginSetupConfigMigrations: ({ config }: { config: RecallConfig }) => ({
    config,
    changes: [],
  }),
}));
...
vi.mock("./doctor/shared/channel-legacy-config-migrate.js", () => ({
  applyChannelDoctorCompatibilityMigrations: (cfg: RecallConfig) => ({
    next: cfg,
    changes: [],
  }),
}));
...
vi.mock("../secrets/target-registry.js", () => {
  ...
  return {
    discoverConfigSecretTargets: (cfg: RecallConfig) => {
      const targets: Array<{
        entry: typeof entry;
        path: string;
        pathSegments: string[];
...
describe("normalizeCompatibilityConfigValues", () => {
  ...
  beforeAll(() => {
    ...
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-oauth-"));
    process.env.RECALL_OAUTH_DIR = tempOauthDir;
  });
