...
function formatUnavailableSkillDoctorLines(skills: SkillStatusEntry[]): string[] {
  ...
  lines.push(`Disable unused skills: ${formatCliCommand("steelengine doctor --fix")}`);
  lines.push(
    `Inspect details: ${formatCliCommand("steelengine skills check --agent <id>")} or ${formatCliCommand("steelengine skills info <name> --agent <id>")}`,
  );
  return lines;
}
