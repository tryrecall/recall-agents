...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: RecallConfig = { plugins: { allow: ["other"] } };
    ...
    expect(result.cfg.plugins?.installs?.zalo?.spec).toBe("@recall/zalo");
    expect(result.cfg.plugins?.installs?.zalo?.installPath).toBe("/tmp/zalo");
    expect(installPluginFromNpmSpec).toHaveBeenCalledWith(
      expect.objectContaining({ spec: "@recall/zalo" }),
    );
  });
  ...
  it("uses the catalog plugin id for local-path installs", async () => {
    ...
    const result = await ensureChannelSetupPluginInstalled({
      ...
      entry: {
        ...baseEntry,
        id: "teams",
        pluginId: "@recall/msteams-plugin",
      },
      prompter,
      runtime,
    });
    ...
    expect(result.installed).toBe(true);
    expect(result.pluginId).toBe("@recall/msteams-plugin");
    expect(result.cfg.plugins?.entries?.["@recall/msteams-plugin"]?.enabled).toBe(true);
  });
    ...
    resolveBundledPluginSources.mockReturnValue(
      new Map([
        [
          ...
          {
            pluginId: "zalo",
            localPath: "/opt/recall/extensions/zalo",
            npmSpec: "@recall/zalo",
          },
        ],
      ]),
    ...
    expect(select).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: expect.arrayContaining([
          expect.objectContaining({
            value: "local",
            hint: "/opt/recall/extensions/zalo",
          }),
        ]),
      }),
    );
    ...
    resolveBundledPluginSources.mockReturnValue(
      new Map([
        [
          ...
          {
            pluginId: "whatsapp",
            localPath: "/opt/recall/extensions/whatsapp",
  ...
  it("clears discovery cache before reloading the setup plugin registry", () => {
    ...
    expect(loadRecallPlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        config: cfg,
        workspaceDir: "/tmp/recall-workspace",
        cache: false,
    ...
    expect(clearPluginDiscoveryCache.mock.invocationCallOrder[0]).toBeLessThan(
      vi.mocked(loadRecallPlugins).mock.invocationCallOrder[0] ?? Number.POSITIVE_INFINITY,
    );
  });
  ...
  it("scopes snapshots by plugin id when channel and plugin ids differ", () => {
    ...
    expect(loadRecallPlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        workspaceDir: "/tmp/recall-workspace",
        cache: false,
        onlyPluginIds: ["@recall/msteams-plugin"],
        includeSetupOnlyChannelPlugins: true,
        activate: false,
      }),
    );
