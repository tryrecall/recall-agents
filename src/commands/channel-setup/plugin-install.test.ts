...
import { bundledPluginRoot, bundledPluginRootAt } from "recall/plugin-sdk/test-fixtures";
...
const discoverRecallPlugins = vi.fn((_args?: unknown) => ({ candidates: [], diagnostics: [] }));
vi.mock("../../plugins/discovery.js", () => ({
  discoverRecallPlugins: (args: unknown) => discoverRecallPlugins(args),
}));
...
const bundledChatNpmSpec = "@recall/bundled-chat@1.2.3";
  ...
  resolveBundledPluginSources.mockReturnValue(
    new Map([
      [
        ...
        {
          pluginId: "bundled-chat",
          localPath: bundledPluginRootAt("/opt/recall", "bundled-chat"),
          npmSpec: bundledChatNpmSpec,
        },
      ],
    ]),
...
function createManifestRecord(
  ...
  return {
    ...
    hooks: [],
    origin: "bundled",
    rootDir: `/tmp/recall-test/${id}`,
    source: `/tmp/recall-test/${id}/index.ts`,
    manifestPath: `/tmp/recall-test/${id}/recall.plugin.json`,
    ...rest,
  };
}
...
function expectSetupSnapshotDoesNotScopeToPlugin(params: {
  ...
  expect(loadRecallPlugins).toHaveBeenCalledTimes(1);
  expect(requireMockCallArg(vi.mocked(loadRecallPlugins), 0).onlyPluginIds).toStrictEqual([]);
}
...
beforeEach(() => {
  ...
  discoverRecallPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const select = vi.fn((async <T extends string>() => "skip" as T) as WizardPrompter["select"]);
  const prompter = makePrompter({ select: select as unknown as WizardPrompter["select"] });
  const cfg: RecallConfig = { update: { channel } };
...
function expectLoadRecallPluginFields(expected: Record<string, unknown>, callIndex = 0) {
  expectRecordFields(
    requireMockCallArg(vi.mocked(loadRecallPlugins), callIndex),
    "loadRecallPlugins args",
    expected,
  );
}
...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: RecallConfig = { plugins: { allow: ["bundled-chat"] } };
  ...
  it("installs npm channel plugins into the active profile extensions dir", async () => {
    ...
    const profileStateDir = "/tmp/recall-ledger-channel";
    process.env.RECALL_STATE_DIR = profileStateDir;
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "bundled-chat",
  ...
  it("installs npm beta on the beta channel without persisting the beta tag", async () => {
    ...
    const cfg: RecallConfig = { update: { channel: "beta" } };
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "wecom-recall-plugin",
      targetDir: "/tmp/wecom-recall-plugin",
      version: "2026.5.4-beta.1",
      npmResolution: {
        name: "@recall/wecom",
        version: "2026.5.4-beta.1",
        resolvedSpec: "@recall/wecom@2026.5.4-beta.1",
      },
    });
    ...
    expectRecordFields(requireMockCallArg(installPluginFromNpmSpec, 0), "npm install args", {
      spec: "@recall/wecom@beta",
      expectedPluginId: "wecom-recall-plugin",
    });
    expect(result.cfg.plugins?.installs?.["wecom-recall-plugin"]?.spec).toBe("@recall/wecom");
  });
  ...
  it("defaults to bundled local path on beta channel when available", async () => {
    ...
    expectRecordFields(
      ...
      {
        value: "local",
        hint: bundledPluginRootAt("/opt/recall", "bundled-chat"),
      },
    );
  });
  ...
  it("offers ClawHub as the first-class install source for channel catalog entries", async () => {
    ...
    await ensureChannelSetupPluginInstalled({
      ...
      entry: {
        ...
        install: {
          clawhubSpec: "clawhub:recall/clawhub-chat@2026.5.2",
    ...
    expectRecordFields(options[0], "clawhub option", {
      value: "clawhub",
      label: "Download from ClawHub (clawhub:recall/clawhub-chat@2026.5.2)",
    });
    expectRecordFields(options[1], "skip option", {
      value: "skip",
    });
  ...
  it("skips the install prompt when autoConfirmSingleSource is set and only npm is available", async () => {
    ...
    const npmOnlyEntry: ChannelPluginCatalogEntry = {
      ...
      install: {
        npmSpec: "@recall/wecom@2026.4.23",
      },
    };
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "wecom-recall-plugin",
      installPath: "/tmp/wecom-recall-plugin",
    });
    vi.mocked(fs.existsSync).mockReturnValue(false);
    resolveBundledPluginSources.mockReturnValue(new Map());
    ...
    expect(select).not.toHaveBeenCalled();
    expect(result.installed).toBe(true);
    expect(result.pluginId).toBe("wecom-recall-plugin");
  });
