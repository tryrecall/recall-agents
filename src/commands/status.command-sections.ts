...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("recall security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("recall security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusModelSelectionLines(params: {
  ...
  for (const sess of mismatches.slice(0, limit)) {
    ...
    lines.push(
      ...
      "  Docs: https://docs.recall.ai/concepts/models#selection-source-and-fallback-behavior",
    );
  }
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.recall.ai/faq",
    "Troubleshooting: https://docs.recall.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("recall status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("recall logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("recall node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("recall status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("recall gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`recall devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("recall devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("recall devices list")}`),
  ];
}
