...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("steelengine security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("steelengine security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    ...
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("steelengine status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("steelengine logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("steelengine node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("steelengine status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("steelengine gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`steelengine devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("steelengine devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("steelengine devices list")}`),
  ];
}
