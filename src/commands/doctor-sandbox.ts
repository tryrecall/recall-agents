...
import { resolveSteelEnginePackageRootsSync } from "../infra/steelengine-root.js";
...
function resolveSandboxScript(
  ...
  // Scan every steelengine package root the shared resolver finds (symlinked launcher via realpath,
  ...
  const roots = resolveSteelEnginePackageRootsSync({
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("steelengine.doctorSandboxTestApi")] = {
...
function codexBwrapNeedsNetworkNamespaceProbe(cfg: SteelEngineConfig): boolean {
  const network = cfg.agents?.defaults?.sandbox?.docker?.network?.trim().toLowerCase();
  return network === undefined || network === "" || network === "none";
}

async function probeCodexBwrapNamespaces(cfg: SteelEngineConfig): Promise<CodexBwrapNamespaceProbe> {
...
async function noteCodexBwrapNamespaceWarning(cfg: SteelEngineConfig): Promise<void> {
  ...
  const lines = [
    ...
    "",
    "Fix the host namespace policy for the SteelEngine service user, then restart the gateway.",
    "Prefer an AppArmor profile that grants the required namespaces to the SteelEngine service process.",
...
function resolveSandboxDockerImage(cfg: SteelEngineConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_IMAGE;
}

function resolveSandboxBackend(cfg: SteelEngineConfig): string {
  const backend = cfg.agents?.defaults?.sandbox?.backend?.trim();
  return backend || "docker";
}

function resolveSandboxBrowserImage(cfg: SteelEngineConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.browser?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_BROWSER_IMAGE;
}

function updateSandboxDockerImage(cfg: SteelEngineConfig, image: string): SteelEngineConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
function updateSandboxBrowserImage(cfg: SteelEngineConfig, image: string): SteelEngineConfig {
  return {
    ...cfg,
    agents: {
      ...cfg.agents,
...
export async function maybeRepairSandboxImages(
  ...
  if (!dockerAvailable) {
    const lines = [
      ...
      "- Disable sandbox mode: steelengine config set agents.defaults.sandbox.mode off",
    ];
    note(lines.join("\n"), "Sandbox");
    return cfg;
  }
...
${formatLegacyRegistryInspectionLine(file)}`,
    ...
    fixHint: `Run ${formatCliCommand("steelengine doctor --fix")} to migrate valid entries to SQLite.`,
  };
}
  ...
  if (!prompter.shouldRepair) {
    note(
      [
        ...
        `Run ${formatCliCommand("steelengine doctor --fix")} to migrate them to SQLite.`,
      ].join("\n"),
      "Sandbox",
    );
    return;
...
export function noteSandboxScopeWarnings(cfg: SteelEngineConfig) {
