...
function listAuthProfileRepairCandidates(
  ...
  const envAgentDir = readNonEmptyString(env.STEELENGINE_AGENT_DIR);
  if (envAgentDir) {
    ...
    note(
      [
        ...
        ...(unreferencedSidecars.length > 0
          ...
          : []),
        `- ${formatCliCommand("steelengine doctor --fix")} migrates active profiles back to inline OAuth credentials and removes only sidecar files it successfully migrated.`,
      ].join("\n"),
      "Auth profiles",
    );
  }
