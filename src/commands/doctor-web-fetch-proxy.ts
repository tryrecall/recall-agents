...
const DIRECT_PROBE_HOST = "docs.steelengine.ai";
...
async function collectWebFetchProxyDiagnostic(params: {
  ...
  return [
    ...
    `  ${formatCliCommand("steelengine config set tools.web.fetch.useTrustedEnvProxy true")}`,
    "- Keep the opt-in disabled for untrusted proxies; enabling it lets the proxy resolve DNS after SteelEngine's hostname checks.",
  ].join("\n");
}
