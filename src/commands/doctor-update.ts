...
async function detectSteelEngineGitCheckout(root: string): Promise<"git" | "not-git" | "unknown"> {
...
async function restartRunningGatewayServiceAfterUpdate(
  ...
  try {
    ...
    note("Restarted the running gateway service after updating SteelEngine.", "Update");
    return true;
  } catch (err) {
...
/** Offers to update SteelEngine before doctor when running interactively from an updatable install. */
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const updateInProgress = isTruthyEnvValue(process.env.STEELENGINE_UPDATE_IN_PROGRESS);
  ...
  const git = await detectSteelEngineGitCheckout(params.root);
  if (git === "git") {
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("steelengine update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
