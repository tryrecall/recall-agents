...
import {
  RECALL_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveRecallWrapperPath,
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperPath = await resolveRecallWrapperPath(
    params.wrapperPath ?? params.env[RECALL_WRAPPER_ENV_KEY],
  );
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [RECALL_WRAPPER_ENV_KEY]: wrapperPath }
    : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.RECALL_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("recall gateway install")}\` after fixing the error.`;
}
