...
import {
  STEELENGINE_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveSteelEngineWrapperPath,
...
function configContainsSecretRef(config: SteelEngineConfig | undefined): boolean {
  if (!config) {
    return false;
  }
...
// they share the STEELENGINE_ prefix that is otherwise stripped from preserved
...
const PRESERVED_STEELENGINE_OPERATOR_OPT_IN_ENV_KEYS = new Set([
  "STEELENGINE_CLI_CONTAINER_BYPASS",
...
function collectPreservedExistingServiceEnvVars(
  ...
  for (const [rawKey, rawValue] of Object.entries(existingEnvironment)) {
    ...
    if (
      ...
      upper === "PATH" ||
      upper === "TMPDIR" ||
      (upper.startsWith("STEELENGINE_") && !PRESERVED_STEELENGINE_OPERATOR_OPT_IN_ENV_KEYS.has(upper))
    ) {
      continue;
    }
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperInput = params.wrapperPath ?? params.env[STEELENGINE_WRAPPER_ENV_KEY];
  ...
  if (wrapperPointsAtWindowsTaskScript) {
    params.warn?.(
      `Ignoring ${STEELENGINE_WRAPPER_ENV_KEY} because it points to the Windows task script; using the SteelEngine gateway entrypoint directly to avoid a recursive gateway.cmd wrapper.`,
    );
  }
  const wrapperPath = wrapperPointsAtWindowsTaskScript
    ? undefined
    : await resolveSteelEngineWrapperPath(wrapperInput);
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [STEELENGINE_WRAPPER_ENV_KEY]: wrapperPath }
    : wrapperPointsAtWindowsTaskScript
      ? omitEnvKey(params.env, STEELENGINE_WRAPPER_ENV_KEY)
      : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.STEELENGINE_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("steelengine gateway install")}\` after fixing the error.`;
}
