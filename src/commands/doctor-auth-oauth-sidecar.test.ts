...
async function makeTestState(seed = "legacy-oauth-seed"): Promise<SteelEngineTestState> {
  const state = await createSteelEngineTestState({
    layout: "state-only",
    prefix: "steelengine-doctor-oauth-sidecar-",
    env: {
      STEELENGINE_AGENT_DIR: undefined,
      STEELENGINE_AUTH_PROFILE_SECRET_KEY: seed,
    },
  });
  states.push(state);
  return state;
...
function encryptLegacySidecarMaterial(params: {
  ref: { source: "steelengine-credentials"; provider: "openai-codex"; id: string };
  profileId: string;
  provider: string;
  seed: string;
...
describe("maybeRepairLegacyOAuthSidecarProfiles", () => {
  ...
  it("scans STEELENGINE_AGENT_DIR before treating sidecars as unreferenced", async () => {
    const state = await makeTestState();
