...
async function makeTestState(seed = "legacy-oauth-seed"): Promise<RecallTestState> {
  const state = await createRecallTestState({
    layout: "state-only",
    prefix: "recall-doctor-oauth-sidecar-",
    env: {
      RECALL_AGENT_DIR: undefined,
      RECALL_AUTH_PROFILE_SECRET_KEY: seed,
    },
  });
  states.push(state);
  return state;
...
function encryptLegacySidecarMaterial(params: {
  ref: { source: "recall-credentials"; provider: "openai-codex"; id: string };
  profileId: string;
  provider: string;
  seed: string;
...
describe("maybeRepairLegacyOAuthSidecarProfiles", () => {
  ...
  it("scans RECALL_AGENT_DIR before treating sidecars as unreferenced", async () => {
    const state = await makeTestState();
