...
describe("resolveSandboxScript", () => {
  ...
  // Create a repo checkout that the shared resolver will recognize: it keys off an steelengine
  ...
  function mkRepo(prefix: string): string {
    ...
    fs.writeFileSync(path.join(repo, "package.json"), JSON.stringify({ name: "steelengine" }));
    return repo;
  }
  ...
  it("falls back to cwd when the launcher path does not resolve to a repo", () => {
    ...
    const result = resolveSandboxScript(scriptRel, {
      argv1: "/nonexistent-ocsbx/bin/steelengine",
      cwd: repo,
    });
  ...
  it("keeps searching cwd when the launcher resolves to a package root without the script", () => {
    // Installed/published steelengine package root: it carries the package.json marker but not
    ...
    fs.writeFileSync(path.join(installed, "package.json"), JSON.stringify({ name: "steelengine" }));
    const entry = path.join(installed, "steelengine.mjs");
    fs.writeFileSync(entry, "");
