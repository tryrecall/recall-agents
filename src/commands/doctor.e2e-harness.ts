...
const resolveSteelEnginePackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
function createLegacyStateMigrationDetectionResult(params?: {
  ...
  return {
    ...
    rescuePending: {
      sourcePaths: ["/tmp/state/crestodian/rescue-pending", "/tmp/state/steelengine/rescue-pending"],
      hasLegacy: false,
    },
    channelPairing: {
      sourceDir: "/tmp/oauth",
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadSteelEnginePlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/steelengine.json",
    createConfigIO,
...
vi.mock("steelengine/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("steelengine/plugin-sdk/provider-stream-shared", () => ({
...
vi.mock("../infra/steelengine-root.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../infra/steelengine-root.js")>();
  return {
    ...actual,
...
beforeEach(() => {
  ...
  resolveSteelEnginePackageRoot.mockReset().mockResolvedValue(null);
  ...
  originalUpdateInProgress = process.env.STEELENGINE_UPDATE_IN_PROGRESS;
  process.env.STEELENGINE_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-doctor-state-"));
