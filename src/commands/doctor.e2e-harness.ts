...
export const resolveRecallPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadRecallPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("recall/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("recall/plugin-sdk/provider-stream-shared", () => ({
...
beforeEach(() => {
  ...
  resolveRecallPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.RECALL_STATE_DIR;
  originalUpdateInProgress = process.env.RECALL_UPDATE_IN_PROGRESS;
  process.env.RECALL_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "recall-doctor-state-"));
