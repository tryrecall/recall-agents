...
describe("detectLinuxVolatileStateDir", () => {
  const TMPFS_MOUNT_INFO = [
    ...
    "35 22 0:35 / /home/user/.steelengine rw - tmpfs tmpfs rw,size=1048576k",
  ].join("\n");
  const RAMFS_MOUNT_INFO = [
    "22 1 0:21 / / rw,relatime - ext4 /dev/sda1 rw",
    "35 22 0:35 / /home/user/.steelengine rw - ramfs ramfs rw",
  ].join("\n");
  ...
  it.each([
    ...
    const result = detectLinuxVolatileStateDir("/home/user/.steelengine", {
      platform: "linux",
      mountInfo,
    ...
    expect(result).toMatchObject({
      ...
      mountPoint: "/home/user/.steelengine",
      fsType,
    });
  });
  ...
  it("uses the most specific matching mount", () => {
    const mountInfo = [
      ...
      "35 30 0:35 / /home/user/.steelengine rw - tmpfs tmpfs rw",
    ].join("\n");

    expect(
      detectLinuxVolatileStateDir("/home/user/.steelengine", {
        platform: "linux",
        mountInfo,
        resolveRealPath: (targetPath) => targetPath,
      }),
  ...
  it("detects a missing state directory through an existing symlink", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-doctor-volatile-"));
    try {
      ...
      const result = detectLinuxVolatileStateDir(path.join(stateLink, "steelengine"), {
        platform: "linux",
        mountInfo: [
          "22 1 0:21 / / rw,relatime - ext4 /dev/sda1 rw",
      ...
      expect(result).toMatchObject({
        path: path.join(resolvedVolatileMount, "steelengine"),
...
describe("formatLinuxVolatileStateDirWarning", () => {
  it("covers all SQLite state and sidecar files under the volatile state directory", () => {
    const warning = formatLinuxVolatileStateDirWarning("~/.steelengine", {
      path: "/home/user/.steelengine",
      mountPoint: "/home/user/.steelengine",
      fsType: "tmpfs",
    });
    ...
    expect(warning).toContain("STEELENGINE_STATE_DIR");
  });
});
