...
import { resolveSteelEngineAgentDir } from "../plugin-sdk/agent-dir-compat.js";
...
function listOrphanAgentDirs(cfg: SteelEngineConfig, stateDir: string): OrphanAgentDir[] {
  ...
  const liveCompatibilityAgentDir = resolveSteelEngineAgentDir();
  try {
...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".steelengine"].map((dir) => path.resolve(root, entry.name, dir));
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for STEELENGINE_STATE_DIR.",
  ].join("\n");
}
...
export function formatLinuxVolatileStateDirWarning(
  ...
  return [
    ...
    "- Move STEELENGINE_STATE_DIR to a persistent filesystem to avoid data loss.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // STEELENGINE_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
function shouldRequireOAuthDir(cfg: SteelEngineConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.STEELENGINE_OAUTH_DIR?.trim()) {
    return true;
  }
...
function shouldSuppressOrphanTranscriptWarning(cfg: SteelEngineConfig, agentId: string): boolean {
  ...
  switch (issue.kind) {
    case "mac-cloud-state-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Move STEELENGINE_STATE_DIR to local non-synced storage such as ~/.steelengine.",
      };
    case "linux-sd-state-dir":
      return {
        ...
        fixHint: "Move STEELENGINE_STATE_DIR to SSD/NVMe-backed storage.",
      };
    case "linux-volatile-state-dir":
      return {
        ...
        path: issue.path,
        target: issue.mountPoint,
        fixHint: "Move STEELENGINE_STATE_DIR to persistent local storage.",
      };
    case "missing-state-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Run `steelengine doctor --fix` to create the state directory.",
      };
    case "state-dir-not-writable":
      return {
        ...
        path: issue.path,
        fixHint: "Run `steelengine doctor --fix` to repair state directory permissions.",
      };
    case "state-dir-too-open":
      return {
        ...
        path: issue.path,
        fixHint: "Run `steelengine doctor --fix` to tighten state directory permissions.",
      };
    case "config-file-too-open":
      return {
        ...
        path: issue.path,
        fixHint: "Run `steelengine doctor --fix` to tighten config file permissions.",
      };
    case "missing-runtime-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Run `steelengine doctor --fix` to create missing runtime state directories.",
      };
    case "runtime-dir-not-writable":
      return {
        ...
        path: issue.path,
        fixHint: "Run `steelengine doctor --fix` to repair runtime state directory permissions.",
      };
  }
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.steelengine).",
        `  Set locally: STEELENGINE_STATE_DIR=~/.steelengine ${formatCliCommand("steelengine doctor")}`,
      ].join("\n"),
    );
  }
  if (linuxSdBackedStateDir) {
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`steelengine sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`steelengine sessions cleanup --store "${absoluteStorePath}" --dry-run --fix-missing`)}`,
          `  Prune missing entries: ${formatCliCommand(`steelengine sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
    ...
    if (wedgedSubagentSessions.length > 0) {
      ...
      warnings.push(
        [
          ...
          "  SteelEngine will not auto-resume these child sessions on restart; reconcile their task records instead.",
          `  Examples: ${wedgedSubagentSessions
            .slice(0, 3)
            .map(([key]) => key)
            .join(", ")}`,
          `  Fix: ${formatCliCommand("steelengine tasks maintenance --apply")}`,
        ].join("\n"),
      );
...
export function collectWorkspaceBackupTip(workspaceDir: string): string | null {
  ...
  return "- Tip: back up the agent workspace in a private git repo; keep ~/.steelengine out of git (credentials, sessions). Details: /concepts/agent-workspace#git-backup-recommended";
}
