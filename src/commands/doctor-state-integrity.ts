...
import { resolveRecallAgentDir } from "../plugin-sdk/agent-dir-compat.js";
...
function listOrphanAgentDirs(cfg: RecallConfig, stateDir: string): OrphanAgentDir[] {
  ...
  const liveCompatibilityAgentDir = resolveRecallAgentDir();
  try {
...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".recall"].map((dir) => path.resolve(root, entry.name, dir));
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for RECALL_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // RECALL_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
function shouldRequireOAuthDir(cfg: RecallConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.RECALL_OAUTH_DIR?.trim()) {
    return true;
  }
...
function shouldSuppressOrphanTranscriptWarning(cfg: RecallConfig, agentId: string): boolean {
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.recall).",
        `  Set locally: RECALL_STATE_DIR=~/.recall ${formatCliCommand("recall doctor")}`,
      ].join("\n"),
    );
  }
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`recall sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`recall sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`,
          `  Prune missing entries: ${formatCliCommand(`recall sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
    ...
    if (wedgedSubagentSessions.length > 0) {
      ...
      warnings.push(
        [
          ...
          "  Recall will not auto-resume these child sessions on restart; reconcile their task records instead.",
          `  Examples: ${wedgedSubagentSessions
            .slice(0, 3)
            .map(([key]) => key)
            .join(", ")}`,
          `  Fix: ${formatCliCommand("recall tasks maintenance --apply")}`,
        ].join("\n"),
      );
