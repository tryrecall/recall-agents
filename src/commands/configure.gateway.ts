...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "RECALL_GATEWAY_TOKEN",
          placeholder: "RECALL_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like RECALL_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Recall will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Recall trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.recall.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
