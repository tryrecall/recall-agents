...
/** Applies a local non-interactive auth choice to the pending SteelEngine config. */
export async function applyNonInteractiveAuthChoice(params: {
  ...
  if (opts.secretInputMode && !requestedSecretInputMode) {
    runtime.error(
      `Invalid --secret-input-mode. Use "plaintext" or "ref", or run ${formatCliCommand("steelengine onboard")} for interactive setup.`,
    );
    runtime.exit(1);
    return null;
  }
