...
const SAMPLE_SECRET_REF = {
  source: "env" as const,
  provider: "default",
  id: "RECALL_GATEWAY_TOKEN_REF",
};

function createTokenConfig(token: unknown): RecallConfig {
  return {
...
describe("applyNonInteractiveGatewayConfig token resolution chain", () => {
  const originalEnvToken = process.env.RECALL_GATEWAY_TOKEN;
  ...
  afterEach(() => {
    if (originalEnvToken === undefined) {
      ...
      process.env.RECALL_GATEWAY_TOKEN = originalEnvToken;
    }
  ...
  it("prefers existing plaintext token over ambient RECALL_GATEWAY_TOKEN on re-onboard", () => {
    // A stale shell/launchd RECALL_GATEWAY_TOKEN must not rotate a
    // persisted token — that would break already-paired clients.
    process.env.RECALL_GATEWAY_TOKEN = "stale-env-token";
    const nextConfig = createTokenConfig("existing-user-token");
  ...
  it("uses RECALL_GATEWAY_TOKEN to fill an empty config on first-run", () => {
  ...
  it("preserves an existing SecretRef even when ambient RECALL_GATEWAY_TOKEN is set", () => {
  ...
  it("overrides an existing SecretRef when --gateway-token-ref-env is provided", () => {
    const newRefId = "RECALL_GATEWAY_TOKEN_NEW_REF";
  ...
  it("fails when --gateway-token-ref-env points to a missing env var", () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      'Environment variable "MISSING_GATEWAY_TOKEN_ENV" is missing or empty. Export it first, then rerun recall onboard --non-interactive.',
    );
