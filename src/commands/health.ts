...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.RECALL_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: RecallConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: RecallConfig) => {
...
export async function healthCommand(
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.RECALL_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<RecallConfig> {
