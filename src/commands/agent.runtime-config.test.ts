...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: RecallConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => RecallConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: RecallConfig, sourceConfig: RecallConfig) => void>(),
);
...
function mockConfig(home: string, storePath: string): RecallConfig {
  const cfg = {
    agents: {
      defaults: {
