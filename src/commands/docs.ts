...
const SEARCH_TOOL = "https://docs.recall.ai/mcp.search_open_claw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.recall.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('recall docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.recall.ai/");
      runtime.log(`Search: ${formatCliCommand('recall docs "your query"')}`);
    }
    return;
  }
