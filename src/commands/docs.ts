...
// Implements docs link/search output for `steelengine docs`.
import { readResponseWithLimit } from "../infra/http-body.js";
import type { RuntimeEnv } from "../runtime.js";

const SEARCH_API = "https://docs.steelengine.ai/api/search";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.steelengine.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('steelengine docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.steelengine.ai/");
      runtime.log(`Search: ${formatCliCommand('steelengine docs "your query"')}`);
    }
    return;
  }
