...
describe("buildProviderStatusIndex", () => {
  ...
  it("prefers inspectAccount for read-only status surfaces", async () => {
    ...
    const map = await buildProviderStatusIndex({} as SteelEngineConfig);
  ...
  it("records accounts that throw during read-only resolution as not configured", async () => {
    ...
    await expect(buildProviderStatusIndex({} as SteelEngineConfig)).resolves.toEqual(
      new Map([
        [
          "quietchat:default",
          {
  ...
  it("rethrows unexpected read-only account resolution errors", async () => {
    ...
    await expect(buildProviderStatusIndex({} as SteelEngineConfig)).rejects.toThrow("plugin crash");
  });
  ...
  it("uses repair hints instead of unknown for bound missing external channels", () => {
    ...
    expect(lines).toEqual([
      "Feishu default: missing plugin - Install the official external plugin with: steelengine plugins install @steelengine/feishu, or run: steelengine doctor --fix.",
    ]);
  });
