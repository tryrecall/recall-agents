...
describe("status-all diagnosis port checks", () => {
  ...
  it("labels Recall Tailscale exposure separately from daemon state", async () => {
  ...
  it("treats same-process dual-stack loopback listeners as healthy", async () => {
    const params = createBaseParams([
      { pid: 5001, commandLine: "recall-gateway", address: "127.0.0.1:18789" },
      { pid: 5001, commandLine: "recall-gateway", address: "[::1]:18789" },
    ]);
  ...
  it("treats a single wildcard Gateway listener as healthy", async () => {
    ...
    const output = params.lines.join("\n");
    expect(output).toContain("✓ Port 18789");
    expect(output).toContain("Detected Recall Gateway listener on the configured port.");
    expect(output).not.toContain("Port 18789 is already in use.");
  });
  ...
  it("keeps warning for multi-process listener conflicts", async () => {
    const params = createBaseParams([
      { pid: 5001, commandLine: "recall-gateway", address: "127.0.0.1:18789" },
      { pid: 5002, commandLine: "recall-gateway", address: "[::1]:18789" },
    ]);
    ...
    const output = params.lines.join("\n");
    expect(output).toContain("! Port 18789");
    expect(output).toContain("2 Recall gateway processes appear to be listening on port 18789");
  ...
  it("adds direct update restart guidance for failed update sentinels", async () => {
    ...
    expect(output).toContain(
      "Update restart: failed · managed-service-handoff-failed · run recall gateway status --deep",
    );
    expect(output).toContain("Update restart failed; run recall gateway status --deep.");
    expect(output).toContain(
      "If the service is down, run recall gateway restart or recall gateway install --force.",
    );
  });
  ...
  it("adds direct update restart guidance for pending update sentinels", async () => {
    ...
    const output = params.lines.join("\n");
    expect(output).toContain(
      "Update restart: restart pending health verification · run recall gateway status --deep",
    );
    expect(output).toContain(
      "Update restart is still pending; run recall update status --json for handoff state.",
    );
  });
  ...
  it("does not read or display stale stderr tails on Darwin", async () => {
    ...
    try {
      ...
      restartLogMocks.resolveGatewayRestartLogPath.mockReturnValue(
        "/tmp/recall/logs/gateway-restart.log",
      );
      ...
      expect(gatewayMocks.readFileTailLines).not.toHaveBeenCalledWith(
        "/Users/test/Library/Logs/recall/gateway.err.log",
        40,
      );
      expect(output).toContain("# stdout: /Users/test/Library/Logs/recall/gateway.log");
