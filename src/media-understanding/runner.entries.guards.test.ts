...
describe("media-understanding missing provider errors", () => {
  it("includes the catalog repair hint for a media provider contract", async () => {
    ...
    expect(message).toMatch(/^Media provider not available: groq .*steelengine plugins install/);
    expect(message).toContain("@steelengine/groq-provider");
    expect(message).toContain("steelengine plugins registry --refresh");
    expect(message).toContain("stop and start the gateway service");
    expect(message).toContain("steelengine doctor --fix");
  });
