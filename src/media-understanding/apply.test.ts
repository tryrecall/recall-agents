...
const TEMP_MEDIA_PREFIX = "steelengine-media-";
...
function createGroqAudioConfig(): SteelEngineConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): SteelEngineConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): SteelEngineConfig {
  return {
  ...
  return await withEnvAsync(
    {
      ...
      STEELENGINE_ANTIGRAVITY_CLI: undefined,
...
async function setupAudioAutoDetectCase(stdout?: string): Promise<{
  ...
  const cfg: SteelEngineConfig = { tools: { media: { audio: {} } } };
  if (stdout !== undefined) {
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredSteelEngineTmpDir();
  ...
  it("uses Antigravity CLI as the last auto image fallback", async () => {
    ...
    const cfg: SteelEngineConfig = { tools: { media: { image: {} } } };
    mockedResolveApiKey.mockResolvedValue({
      source: "none",
      mode: "api-key",
    });
    ...
    expect(probeOptions).toEqual({
      timeoutMs: 3000,
      cwd: expect.stringContaining("steelengine-antigravity-probe-"),
    });
