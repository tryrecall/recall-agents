...
const TEMP_MEDIA_PREFIX = "recall-media-";
...
function createGroqAudioConfig(): RecallConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): RecallConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): RecallConfig {
  return {
  ...
  return await withEnvAsync(
    {
      ...
      RECALL_ANTIGRAVITY_CLI: undefined,
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: RecallConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredRecallTmpDir();
  ...
  it("uses Antigravity CLI as the last auto image fallback", async () => {
    ...
    const cfg: RecallConfig = { tools: { media: { image: {} } } };
    mockedResolveApiKey.mockResolvedValue({
      source: "none",
      mode: "api-key",
    });
    ...
    expect(probeOptions).toEqual({
      timeoutMs: 3000,
      cwd: expect.stringContaining("recall-antigravity-probe-"),
    });
