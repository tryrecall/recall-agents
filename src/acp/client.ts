/** Interactive stdio ACP client used to connect a terminal session to an SteelEngine ACP server. */
...
async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const defaultServerCommand = entryPath ? process.execPath : "steelengine";
  ...
  await client.initialize({
    ...
    clientInfo: { name: "steelengine-acp-client", version: "1.0.0" },
  });
...
export async function runAcpClientInteractive(opts: AcpClientOptions = {}): Promise<void> {
  ...
  console.log("SteelEngine ACP client");
