...
const createTempDir = () => tempDirs.make("steelengine-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets STEELENGINE_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "steelengine",
    });

    expect(env.STEELENGINE_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("steelengine");
  });

  it("overrides pre-existing STEELENGINE_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      STEELENGINE_SHELL: "wrong",
    });
    expect(env.STEELENGINE_SHELL).toBe("acp-client");
  });
  ...
  it("preserves STEELENGINE_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        STEELENGINE_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["STEELENGINE_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default SteelEngine bridge", () => {
    ...
    expect(env.STEELENGINE_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.STEELENGINE_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "steelengine",
        serverArgs: ["acp"],
        defaultServerCommand: "steelengine",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("STEELENGINE_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "steelengine", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "steelengine", "dist", "entry.js");
    const shimPath = path.join(dir, "steelengine.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\steelengine\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
  ...
  it("prompts for search when rawInput path escapes cwd", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/steelengine-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
  ...
  it("auto-approves search when query-like title text contains a path label", async () => {
    await expectAutoAllowWithoutPrompt({
      ...
      cwd: "/tmp/steelengine-acp-cwd/workspace",
    });
  });
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/steelengine-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/steelengine-acp-cwd",
    });
