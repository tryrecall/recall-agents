...
const createTempDir = () => tempDirs.make("recall-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets RECALL_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "recall",
    });

    expect(env.RECALL_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("recall");
  });

  it("overrides pre-existing RECALL_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      RECALL_SHELL: "wrong",
    });
    expect(env.RECALL_SHELL).toBe("acp-client");
  ...
  it("preserves RECALL_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        RECALL_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["RECALL_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Recall bridge", () => {
    ...
    expect(env.RECALL_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.RECALL_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("RECALL_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "recall", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "recall", "dist", "entry.js");
    const shimPath = path.join(dir, "recall.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\recall\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/recall-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/recall-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/recall-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
