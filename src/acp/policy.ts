...
import { AcpRuntimeError } from "@steelengine/acp-core/runtime/errors";
...
export function isAcpEnabledByPolicy(cfg: SteelEngineConfig): boolean {
  return cfg.acp?.enabled !== false;
}

/** Resolves the effective dispatch policy state for inbound ACP routing. */
function resolveAcpDispatchPolicyState(cfg: SteelEngineConfig): AcpDispatchPolicyState {
...
export function resolveAcpDispatchPolicyMessage(cfg: SteelEngineConfig): string | null {
...
export function resolveAcpDispatchPolicyError(cfg: SteelEngineConfig): AcpRuntimeError | null {
...
export function resolveAcpExplicitTurnPolicyError(cfg: SteelEngineConfig): AcpRuntimeError | null {
  if (isAcpEnabledByPolicy(cfg)) {
    return null;
  }
...
function isAcpAgentAllowedByPolicy(cfg: SteelEngineConfig, agentId: string): boolean {
