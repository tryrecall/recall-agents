...
describe("ACP session metadata SQLite store", () => {
  ...
  it("persists ACP metadata in SQLite without writing sessions.json acp blocks", async () => {
    await withTempDir({ prefix: "steelengine-acp-meta-" }, async (dir) => {
      const storePath = path.join(dir, "sessions.json");
      const databasePath = path.join(dir, "state", "steelengine.sqlite");
  ...
  it("honors STEELENGINE_STATE_DIR when joining listed SQLite rows to session stores", async () => {
    await withTempDir({ prefix: "steelengine-acp-meta-" }, async (dir) => {
      const env = { ...process.env, STEELENGINE_STATE_DIR: dir } as NodeJS.ProcessEnv;
