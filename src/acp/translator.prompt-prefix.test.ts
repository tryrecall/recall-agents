...
describe("acp prompt cwd prefix", () => {
  ...
  it("redacts home directory in prompt prefix", async () => {
    ...
    expect(requestSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        message: expect.stringMatching(/\[Working directory: ~[\\/]recall-test\]/),
      }),
      { expectFinal: true },
    );
  });
  ...
  it("keeps backslash separators when cwd uses them", async () => {
    ...
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringContaining("[Working directory: ~\\recall-test]"),
      }),
      { expectFinal: true },
    );
  });
  ...
  it("injects a system provenance receipt when requested", async () => {
    ...
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        systemProvenanceReceipt: expect.stringContaining("bridge=recall-acp"),
      }),
      { expectFinal: true },
    );
    expect(requestSpy).toHaveBeenCalledWith(
