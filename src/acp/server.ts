#!/usr/bin/env node
/** ACP stdio server that bridges Agent Client Protocol clients to the SteelEngine Gateway. */
  ...
  const gateway = new GatewayClient({
    ...
    onEvent: (evt) => {
      ...
      void agent?.handleGatewayEvent(evt).catch((err: unknown) => {
        process.stderr.write(`steelengine acp: gateway event ${evt.event} failed\n`);
        if (opts.verbose) {
          process.stderr.write(`steelengine acp: gateway event ${evt.event} error: ${String(err)}\n`);
        }
      });
    },
    onHelloOk: () => {
...
function printHelp(): void {
  console.log(`Usage: steelengine acp [options]
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
  ...
  if (argv.includes("--token") || argv.includes("--gateway-token")) {
    console.error(
      "Warning: --token can be exposed via process listings. Prefer --token-file or STEELENGINE_GATEWAY_TOKEN.",
    );
  }
