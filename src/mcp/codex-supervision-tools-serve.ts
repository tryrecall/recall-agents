...
function withCodexSupervisionEnabled(config: SteelEngineConfig): SteelEngineConfig {
  const next = structuredClone(normalizePluginTargetConfig(config, "codex")) as SteelEngineConfig &
  ...
  if (process.env.STEELENGINE_CODEX_SUPERVISOR_ALLOW_RAW_TRANSCRIPTS === "1") {
    supervision.allowRawTranscripts = true;
  }
  if (process.env.STEELENGINE_CODEX_SUPERVISOR_ALLOW_WRITE_CONTROLS === "1") {
    supervision.allowWriteControls = true;
  }
  return next;
}

function resolveCodexSupervisionTools(config: SteelEngineConfig): AnyAgentTool[] {
  const context = {
    config,
...
function requireCompleteCodexSupervisionToolSet(tools: readonly AnyAgentTool[]): void {
  ...
  throw new Error(
    `Codex supervision MCP could not load the official @steelengine/codex plugin tools (missing: ${missing.join(", ") || "none"}). Install or update @steelengine/codex, then enable Codex supervision.`,
  );
}

export function createCodexSupervisionToolsMcpServer(
  params: { config?: SteelEngineConfig; tools?: AnyAgentTool[] } = {},
): Server {
  ...
  return createToolsMcpServer({ name: "steelengine-codex-supervisor", tools });
}
