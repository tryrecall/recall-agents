...
vi.mock("./channel-bridge.js", () => ({
  RecallChannelBridge: class MockRecallChannelBridge {
...
describe("serveRecallChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveRecallChannelMcp } = await import("./channel-server.js");

    const servePromise = serveRecallChannelMcp({ verbose: false });
