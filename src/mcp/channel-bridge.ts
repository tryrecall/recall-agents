...
export class RecallChannelBridge {
  ...
  constructor(
    private readonly cfg: RecallConfig,
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`recall mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `recall mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
