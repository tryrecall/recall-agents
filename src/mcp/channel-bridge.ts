...
/**
 * Runtime bridge between MCP tools and the SteelEngine Gateway channel APIs.
 *
...
export class SteelEngineChannelBridge {
  ...
  constructor(
    private readonly cfg: SteelEngineConfig,
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClientCtor({
      ...
      clientDisplayName: "SteelEngine MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`steelengine mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
  ...
  private async sendNotification(notification: ServerNotification): Promise<void> {
    ...
    try {
      ...
      process.stderr.write(`steelengine mcp: notification ${notification.method} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `steelengine mcp: notification ${notification.method} error: ${String(error)}\n`,
        );
      }
    }
  }
  ...
  private async dispatchGatewayEvent(event: EventFrame): Promise<void> {
    try {
      ...
      process.stderr.write(`steelengine mcp: gateway event ${event.event} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `steelengine mcp: gateway event ${event.event} error: ${String(error)}\n`,
        );
      }
    }
  }
