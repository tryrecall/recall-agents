/**
 * Standalone MCP server that exposes Recall plugin-registered tools
...
function resolvePluginToolPolicy(config: RecallConfig): {
...
function resolveTools(config: RecallConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "recall-plugin-tools", tools });
}
