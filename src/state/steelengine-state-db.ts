// SteelEngine state database manages shared persisted state and migrations.
...
import * as operatorApprovalMigration from "./steelengine-state-db-operator-approval-migration.js";
import {
  ensureColumn,
  tableExists,
  tableHasColumn,
  tablePrimaryKeyColumns,
} from "./steelengine-state-db-schema-helpers.js";
import * as sessionWatchMigration from "./steelengine-state-db-session-watch-migration.js";
import type { DB as SteelEngineStateKyselyDatabase } from "./steelengine-state-db.generated.js";
import {
  resolveSteelEngineStateSqliteDir,
  resolveSteelEngineStateSqlitePath,
} from "./steelengine-state-db.paths.js";
import { STEELENGINE_STATE_SCHEMA_SQL } from "./steelengine-state-schema.generated.js";

/**
 * Shared SteelEngine SQLite state database lifecycle and metadata writers.
 *
...
export const STEELENGINE_STATE_SCHEMA_VERSION = 4;
const STEELENGINE_STATE_STRICT_SCHEMA_VERSION = 3;
/** Maximum time one synchronous SQLite call may wait for a lock. */
export const STEELENGINE_SQLITE_BUSY_TIMEOUT_MS = 5_000;
/** User-facing guide for schema refusals; lives here so error sites avoid import cycles. */
export const STEELENGINE_DATABASE_SCHEMA_DOCS_URL =
  "https://docs.steelengine.ai/reference/database-schemas";
const STEELENGINE_STATE_DIR_MODE = 0o700;
const STEELENGINE_STATE_FILE_MODE = 0o600;
const STEELENGINE_STATE_CANONICAL_UNIQUE_INDEXES = [
  {
...
const STEELENGINE_STATE_MAINTENANCE_SCHEMA_COMPATIBILITY = {
...
export type SteelEngineStateDatabase = {
  db: DatabaseSync;
  path: string;
  walMaintenance: SqliteWalMaintenance;
};
/** Options for resolving or overriding the shared state database path. */
export type SteelEngineStateDatabaseOptions = {
  env?: NodeJS.ProcessEnv;
  path?: string;
};
export type SteelEngineStateDatabaseSchemaMigration = {
  kind:
...
const cachedDatabases = new Map<string, SteelEngineStateDatabase>();
...
export function recordSteelEngineStateDatabaseOpenFailure(pathname: string, error: Error): void {
  terminalOpenLatch.record(pathname, error);
}

/** Clear a terminal open failure after doctor rewrites the database file. */
export function clearSteelEngineStateDatabaseOpenFailure(pathname: string): void {
  terminalOpenLatch.clear(pathname);
}
type SteelEngineStateMetadataDatabase = Pick<SteelEngineStateKyselyDatabase, "schema_meta">;

/** Build the shared refusal used by state and agent database opens. */
export function createSteelEngineDatabaseVerificationError(
  kind: "agent" | "state",
  pathname: string,
  ...
  const error = new Error(
    `SteelEngine ${kind} database ${pathname} is quarantined after integrity verification failed: ${storedError ?? "unknown integrity error"}. Restore the database from a backup or repair it, then run steelengine doctor --fix to clear the quarantine. See ${STEELENGINE_DATABASE_SCHEMA_DOCS_URL}.`,
  );
...
export function assertSteelEngineStateDatabaseForMaintenance(
  ...
  if (userVersion !== STEELENGINE_STATE_SCHEMA_VERSION) {
    throw new Error(
      `SteelEngine state database ${options.pathname} uses schema version ${userVersion}; run steelengine doctor --fix before compacting it.`,
    );
  }
  ...
  if (metadata?.role !== "global") {
    ...
    throw new Error(
      `SteelEngine state database ${options.pathname} has schema role ${role}; expected global.`,
    );
  }
  if (metadata.schema_version !== STEELENGINE_STATE_SCHEMA_VERSION) {
    const schemaVersion =
      typeof metadata.schema_version === "number" ? metadata.schema_version : "invalid";
    throw new Error(
      `SteelEngine state database ${options.pathname} metadata schema version ${schemaVersion} does not match ${STEELENGINE_STATE_SCHEMA_VERSION}; run steelengine doctor --fix before compacting it.`,
    );
  }
  assertSqliteSchemaContains(
    database,
    ...
    STEELENGINE_STATE_MAINTENANCE_SCHEMA_COMPATIBILITY,
  );
}
...
export function ensureSteelEngineStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  const dir = path.dirname(pathname);
  const defaultDir = resolveSteelEngineStateSqliteDir(env);
  const isDefaultStateDatabase =
    path.resolve(pathname) === path.resolve(resolveSteelEngineStateSqlitePath(env));
  if (isDefaultStateDatabase && dir !== defaultDir) {
    throw new Error(`SteelEngine state database path resolved outside its state dir: ${pathname}`);
  }
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: STEELENGINE_STATE_DIR_MODE });
  ...
  if (isDefaultStateDatabase || !dirExisted) {
    bestEffortChmodSync(dir, STEELENGINE_STATE_DIR_MODE);
  }
  for (const candidate of resolveSqliteDatabaseFilePaths(pathname)) {
    if (existsSync(candidate)) {
      bestEffortChmodSync(candidate, STEELENGINE_STATE_FILE_MODE);
    }
  }
}
...
function markCurrentStateSchemaVersion(db: DatabaseSync): void {
  ...
  db.exec(`PRAGMA user_version = ${STEELENGINE_STATE_SCHEMA_VERSION};`);
  if (
    ...
    ).run(STEELENGINE_STATE_SCHEMA_VERSION, Date.now());
  }
}
...
function assertCanonicalStateSchemaShape(db: DatabaseSync, pathname: string): void {
  ...
  if (!hasCanonicalAuditEventsSchema(db)) {
    if (canRepairLegacyAuditEventsSchema(db)) {
      throw new Error(
        `SteelEngine state database ${pathname} has a legacy audit event schema; run steelengine doctor --fix to migrate it.`,
      );
    }
    throw new Error(
      `SteelEngine state database ${pathname} has a noncanonical audit event schema that cannot be repaired automatically; restore the canonical audit_events shape before retrying.`,
    );
  }
}
export function detectSteelEngineStateDatabaseSchemaMigrations(
  options: SteelEngineStateDatabaseOptions = {},
): SteelEngineStateDatabaseSchemaMigration[] {
  ...
  try {
    const migrations: SteelEngineStateDatabaseSchemaMigration[] = [];
    const userVersion = readSqliteUserVersion(db);
    if (!hasCanonicalAgentDatabasesPrimaryKey(db)) {
    ...
    if (tableExists(db, "audit_events") && userVersion < STEELENGINE_STATE_STRICT_SCHEMA_VERSION) {
...
export function repairSteelEngineStateDatabaseSchema(options: SteelEngineStateDatabaseOptions = {}): {
  changes: string[];
  warnings: string[];
} {
  ...
  ensureSteelEngineStatePermissions(pathname, env);
  const sqlite = requireNodeSqlite();
  ...
  try {
    ...
    const changes = runSqliteImmediateTransactionSync(
      ...
      () => {
        ...
        if (tableExists(db, "audit_events")) {
          ...
          db.exec(STEELENGINE_STATE_SCHEMA_SQL);
    ...
    const quarantineCleared = clearSteelEngineDatabaseQuarantine(pathname, { env });
    clearSteelEngineStateDatabaseOpenFailure(pathname);
    return {
      changes,
      warnings: quarantineCleared
        ? []
        : [
            `Persisted quarantine record for ${pathname} could not be cleared; rerun steelengine doctor --fix so the repaired database is not refused again.`,
          ],
    };
  } catch (err) {
    // Reaching this catch inside doctor means repair itself refused or failed,
    // so the runtime asserts' "run steelengine doctor --fix" advice is circular here.
    const reason = String(err).replace(
      /has a legacy ([a-z ]+) schema; run steelengine doctor --fix to migrate it\./u,
...
export function withSteelEngineStateStartupMigrationCheckpointDatabase<T>(
...
function ensureAdditiveStateColumns(db: DatabaseSync): void {
  ...
  ensureColumn(db, "worker_environments", "bootstrap_steelengine_version TEXT");
...
function ensureSchema(db: DatabaseSync, pathname: string): void {
  const now = Date.now();
  const kysely = getNodeSqliteKysely<SteelEngineStateMetadataDatabase>(db);
  ...
  try {
    runSqliteImmediateTransactionSync(
      ...
      () => {
        ...
        if (previousVersion < STEELENGINE_STATE_STRICT_SCHEMA_VERSION) {
          migrateSqliteSchemaToStrictInTransaction(db, STEELENGINE_STATE_SCHEMA_SQL, {
            databaseLabel: pathname,
          });
        }
        repairCanonicalSqliteUniqueIndexes(db, pathname, STEELENGINE_STATE_CANONICAL_UNIQUE_INDEXES);
        db.exec(`PRAGMA user_version = ${STEELENGINE_STATE_SCHEMA_VERSION};`);
        executeSqliteQuerySync(
          db,
          kysely
            ...
            .values({
              meta_key: "primary",
              role: "global",
              schema_version: STEELENGINE_STATE_SCHEMA_VERSION,
              agent_id: null,
              app_version: VERSION,
              created_at: now,
              updated_at: now,
...
function resolveDatabasePath(options: SteelEngineStateDatabaseOptions = {}): string {
  return path.resolve(options.path ?? resolveSteelEngineStateSqlitePath(options.env ?? process.env));
}
...
function assertStateDatabaseIntegrityBeforeMutation(
  ...
  if (
    ...
    (userVersion > 0 && userVersion < STEELENGINE_STATE_SCHEMA_VERSION)
  ) {
    ...
    stateDbLog.info("state database schema migration pending; verifying integrity first", {
      ...
      path: pathname,
      toVersion: STEELENGINE_STATE_SCHEMA_VERSION,
    });
...
export function openSteelEngineStateDatabase(
  options: SteelEngineStateDatabaseOptions = {},
  ...
  try {
    const quarantine = readSteelEngineDatabaseQuarantine(pathname, { env });
    if (quarantine) {
      quarantineFailure = createSteelEngineDatabaseVerificationError(
        "state",
        pathname,
        quarantine.reason,
      );
  ...
  const walMaintenance = (() => {
    ...
    try {
      ...
      if (
        ...
        recordSteelEngineStateDatabaseOpenFailure(pathname, err);
      }
      throw err;
    }
  })();
...
export function runSteelEngineStateWriteTransaction<T>(
  operation: (database: SteelEngineStateDatabase) => T,
  options: SteelEngineStateDatabaseOptions = {},
  ...
  try {
    ensureSteelEngineStatePermissions(database.path, options.env ?? process.env);
  } catch {
...
export function closeSteelEngineStateDatabase(): void {
...
export function isSteelEngineStateDatabaseOpen(): boolean {
  return Array.from(cachedDatabases.values()).some((database) => database.db.isOpen);
}

/** Close shared state handles and clear terminal failure latches for test isolation. */
export function closeSteelEngineStateDatabaseForTest(): void {
