// SteelEngine agent database tests cover agent-scoped DB storage and migrations.
...
import {
  ...
  disposeSteelEngineAgentDatabaseByPath,
  ensureSteelEngineAgentDatabaseSchema,
  inspectSteelEngineAgentDatabaseOwner,
  listSteelEngineRegisteredAgentDatabases,
...
function createTempStateDir(): string {
  return makeTempDir(agentDbTempDirs, "steelengine-agent-db-");
}
...
function readRegisteredAgentDatabaseLastSeenAt(params: {
  ...
  const row = openSteelEngineStateDatabase({ env: params.env })
...
function launchAgentSchemaOpener(params: {
  ...
  const agentModuleUrl = new URL("./steelengine-agent-db.ts", import.meta.url).href;
  const stateModuleUrl = new URL("./steelengine-state-db.ts", import.meta.url).href;
  const child = spawn(
    process.execPath,
    [
      "--import",
      ...
      `
        ...
        const db = new DatabaseSync(process.env.STEELENGINE_AGENT_DB_RACE_PATH);
        ...
        process.stdin.once("data", () => {
          let result;
          try {
            ensureSteelEngineAgentDatabaseSchema(observedDb, {
              agentId: process.env.STEELENGINE_AGENT_DB_RACE_AGENT,
              env: { STEELENGINE_STATE_DIR: process.env.STEELENGINE_AGENT_DB_RACE_STATE_DIR },
              path: process.env.STEELENGINE_AGENT_DB_RACE_PATH,
              register: true,
            });
            result = { agentId: process.env.STEELENGINE_AGENT_DB_RACE_AGENT, ok: true };
          } catch (error) {
            result = {
              agentId: process.env.STEELENGINE_AGENT_DB_RACE_AGENT,
              ok: false,
    ...
    ],
    {
      env: {
        ...process.env,
        STEELENGINE_AGENT_DB_RACE_AGENT: params.agentId,
        STEELENGINE_AGENT_DB_RACE_PATH: params.databasePath,
        STEELENGINE_AGENT_DB_RACE_STATE_DIR: params.stateDir,
      },
...
describe("steelengine agent database", () => {
  it("resolves under the per-agent state directory", () => {
    ...
    ).toBe(path.join(stateDir, "agents", "worker-1", "agent", "steelengine-agent.sqlite"));
  });
  ...
  it("lists a missing registry without creating the shared state database", () => {
    ...
    expect(listSteelEngineRegisteredAgentDatabases({ env })).toEqual([]);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      ...
      expect(() => listSteelEngineRegisteredAgentDatabases({ env })).toThrow("is unavailable");
    },
  );
  ...
  it("lists the registry without updating shared schema metadata", () => {
    const stateDir = createTempStateDir();
    const env = { STEELENGINE_STATE_DIR: stateDir };
    const stateDatabase = openSteelEngineStateDatabase({ env });
    stateDatabase.db
  ...
  it("creates the per-agent schema and registers it globally", () => {
    ...
    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./steelengine-agent-schema.sql", import.meta.url)),
    );
    expect(
      database.db
        .prepare(
    ...
    expect(database.path).toBe(
      path.join(stateDir, "agents", "worker-1", "agent", "steelengine-agent.sqlite"),
    );

    const registered = listSteelEngineRegisteredAgentDatabases({
      env: { STEELENGINE_STATE_DIR: stateDir },
    ...
    expect(registered).toMatchObject({
      agentId: "worker-1",
      path: database.path,
      schemaVersion: STEELENGINE_AGENT_SCHEMA_VERSION,
    });
    expect(registered?.sizeBytes).toBeGreaterThan(0);
  });

  it("folds additive heartbeat storage into schema version 11", () => {
    expect(STEELENGINE_AGENT_SCHEMA_VERSION).toBe(11);
    const stateDir = createTempStateDir();
    const env = { STEELENGINE_STATE_DIR: stateDir };
    const opened = openSteelEngineAgentDatabase({ agentId: "worker-1", env });
    ...
    const reopened = openSteelEngineAgentDatabase({ agentId: "worker-1", env });
    expect(
      reopened.db
  ...
  it("upgrades version 10 with agent state intact and adds lease storage", () => {
    ...
    const migrated = openSteelEngineAgentDatabase({ agentId: "worker-1", env });
  ...
  it("migrates version 8 tables to STRICT without losing agent state", () => {
    ...
    expect(readSqliteNumberPragma(migrated.db, "user_version")).toBe(STEELENGINE_AGENT_SCHEMA_VERSION);
    expect(
      migrated.db
        .prepare("SELECT schema_version FROM schema_meta WHERE meta_key = 'primary'")
        .get(),
  ...
  it("migrates version 1 memory source identities before registering version 2", () => {
    ...
    const database = openSteelEngineAgentDatabase({ agentId: "worker-1", env });

    expect(readSqliteNumberPragma(database.db, "user_version")).toBe(STEELENGINE_AGENT_SCHEMA_VERSION);
    expect(
    ...
    expect(
      listSteelEngineRegisteredAgentDatabases({ env }).find((entry) => entry.agentId === "worker-1"),
    ).toMatchObject({ path: databasePath, schemaVersion: STEELENGINE_AGENT_SCHEMA_VERSION });
  ...
  it("keeps multiple registered paths for the same agent", () => {
    ...
    const relocated = openSteelEngineAgentDatabase({
      agentId: "worker-1",
      env,
      path: relocatedPath,
    });
    const defaultDatabase = openSteelEngineAgentDatabase({
      agentId: "worker-1",
      env,
    });

    expect(
      listSteelEngineRegisteredAgentDatabases({ env })
  ...
  it("does not refresh global registry metadata on cached opens", () => {
    ...
    try {
      const first = openSteelEngineAgentDatabase({
        agentId: "worker-1",
        env,
      });
      expect(
      ...
      const second = openSteelEngineAgentDatabase({
        agentId: "worker-1",
        env,
      });
  ...
  it("rejects the legacy agent registry primary key with a doctor repair hint", () => {
    ...
    expect(() => listSteelEngineRegisteredAgentDatabases({ env })).toThrow(
      /run steelengine doctor --fix/,
    );
    ...
    ).toThrow(/run steelengine doctor --fix/);

    fs.rmSync(stateDatabasePath);
    const reopened = openSteelEngineAgentDatabase({
      agentId: "worker-1",
      env,
    });
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-agent-db-state-"));
          const env = { STEELENGINE_STATE_DIR: stateDir };
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-agent-db-relative-"));
          ...
          try {
            ...
            console.log(JSON.stringify({
              ...
              registeredPaths: listSteelEngineRegisteredAgentDatabases({ env })
                .filter((entry) => entry.agentId === "worker-1")
                .map((entry) => entry.path),
  ...
  it("closes only the cached database at the exact resolved path", () => {
    ...
    const first = openSteelEngineAgentDatabase({ agentId: "worker-1", env, path: firstPath });
    const second = openSteelEngineAgentDatabase({ agentId: "worker-2", env, path: secondPath });

    expect(closeSteelEngineAgentDatabaseByPath(path.join(stateDir, "missing.sqlite"))).toBe(false);
    ...
    expect(second.db.isOpen).toBe(true);
    expect(closeSteelEngineAgentDatabaseByPath(firstPath)).toBe(false);

    const reopened = openSteelEngineAgentDatabase({ agentId: "worker-1", env, path: firstPath });
  ...
  it("disposes only its exact cached owner and unregisters that registry row", () => {
    const stateDir = createTempStateDir();
    const env = { STEELENGINE_STATE_DIR: stateDir };
    const first = openSteelEngineAgentDatabase({ agentId: "worker-1", env });
    const second = openSteelEngineAgentDatabase({ agentId: "worker-2", env });

    expect(disposeSteelEngineAgentDatabaseByPath(first.path, { env })).toBe(true);
    expect(first.db.isOpen).toBe(false);
    expect(second.db.isOpen).toBe(true);
    expect(listSteelEngineRegisteredAgentDatabases({ env })).toEqual([
      expect.objectContaining({ agentId: "worker-2", path: second.path }),
    ]);
    expect(disposeSteelEngineAgentDatabaseByPath(first.path, { env })).toBe(false);
    expect(second.db.isOpen).toBe(true);
  ...
  it("serializes concurrent ownership claims for one unowned database", async () => {
    ...
    try {
      ...
      expect(inspectSteelEngineAgentDatabaseOwner(databasePath)).toEqual({
        status: "owned",
        agentId: winner?.agentId,
      });
      expect(
        listSteelEngineRegisteredAgentDatabases({ env: { STEELENGINE_STATE_DIR: stateDir } }),
      ).toEqual([
        expect.objectContaining({
          agentId: winner?.agentId,
          path: databasePath,
  ...
  it("rechecks schema version after waiting for a concurrent writer", async () => {
    ...
    const futureVersion = STEELENGINE_AGENT_SCHEMA_VERSION + 1;
    try {
      await opener.ready;
      opener.child.stdin.end("go\n");
  ...
  it("rejects explicit paths that point at the global state database", () => {
    ...
    const stateDatabase = openSteelEngineStateDatabase({
      env,
      path: databasePath,
    });
    closeSteelEngineStateDatabaseForTest();
    ...
    const reopenedStateDatabase = openSteelEngineStateDatabase({
      env,
      path: databasePath,
    });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    ...
    const env = { STEELENGINE_STATE_DIR: parentDir };
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      runSteelEngineAgentWriteTransaction(() => {
        runSteelEngineAgentWriteTransaction(() => undefined, options);
  ...
  it("repairs a same-name transcript uniqueness index before accepting writes", () => {
    const stateDir = createTempStateDir();
    const env = { STEELENGINE_STATE_DIR: stateDir };
    const created = openSteelEngineAgentDatabase({ agentId: "worker-1", env });
  ...
  it("rejects same-name transcript index drift when duplicate rows block repair", () => {
    ...
    try {
      ...
      expect(
        after
          .prepare(
            "SELECT name FROM sqlite_schema WHERE type = 'index' AND name LIKE 'steelengine_probe_%'",
          )
          .all(),
      ).toEqual([]);
    } finally {
  ...
  it("adds transcript watermarks and session provenance to v4 session tables", () => {
    ...
    const currentSchema = fs.readFileSync(
      new URL("./steelengine-agent-schema.sql", import.meta.url),
      "utf8",
    );
  ...
  it("adds durable conversation delivery state when upgrading the canonical v10 schema", () => {
    ...
    expect(() =>
      assertSteelEngineAgentDatabaseForMaintenance(database.db, {
        agentId: "worker-1",
  ...
  it("rejects stale schema_meta indexes before writable initialization", () => {
    const stateDir = createTempStateDir();
    const env = { STEELENGINE_STATE_DIR: stateDir };
    const databasePath = openSteelEngineAgentDatabase({ agentId: "worker-1", env }).path;
    closeSteelEngineAgentDatabasesForTest();
    closeSteelEngineStateDatabaseForTest();
    ...
    try {
      expect(() =>
        ensureSteelEngineAgentDatabaseSchema(independentlyManaged, {
          agentId: "worker-1",
          env,
        }),
  ...
  it("runs full integrity before a pending agent schema migration", () => {
    ...
    try {
      before.exec(`PRAGMA user_version = ${STEELENGINE_AGENT_SCHEMA_VERSION - 1};`);
    } finally {
      before.close();
    }
  ...
  it("latches newer per-agent schema failures before integrity scans", () => {
    ...
    const { DatabaseSync } = requireNodeSqlite();
    const db = new DatabaseSync(databasePath);
    db.exec(`PRAGMA user_version = ${STEELENGINE_AGENT_SCHEMA_VERSION + 1};`);
    db.close();
    ...
    clearSteelEngineAgentDatabaseOpenFailure(databasePath);
    expect(openSteelEngineAgentDatabase({ agentId: "worker-1", env }).db.isOpen).toBe(true);
  });
  ...
  it("closes cached handles on normal process exit so no stale WAL remains", () => {
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          import fs from "node:fs";
          import { openSteelEngineAgentDatabase } from ${JSON.stringify(agentModuleUrl)};

          const database = openSteelEngineAgentDatabase({
            agentId: "worker-1",
            env: { STEELENGINE_STATE_DIR: process.env.STEELENGINE_AGENT_DB_EXIT_TEST_DIR },
          });
      ...
      {
        encoding: "utf8",
        env: { ...process.env, STEELENGINE_AGENT_DB_EXIT_TEST_DIR: stateDir },
      },
    );
    const result = JSON.parse(output) as {
