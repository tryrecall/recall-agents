// SteelEngine agent database stores agent-scoped persisted runtime state.
...
} from "./steelengine-agent-db-session-migrations.js";
import {
  addSessionProvenanceColumns,
  backfillSessionEntryProvenance,
  backfillTranscriptMutationWatermarks,
} from "./steelengine-agent-db-session-provenance.js";
import type { DB as SteelEngineAgentKyselyDatabase } from "./steelengine-agent-db.generated.js";
import { resolveSteelEngineAgentSqlitePath } from "./steelengine-agent-db.paths.js";
...
import {
  createSteelEngineDatabaseVerificationError,
...
export { resolveSteelEngineAgentSqlitePath } from "./steelengine-agent-db.paths.js";
 ...
 * SteelEngine state database for discovery and maintenance.
 */
...
export const STEELENGINE_AGENT_SCHEMA_VERSION = 11;
const STEELENGINE_AGENT_DB_DIR_MODE = 0o700;
const STEELENGINE_AGENT_DB_FILE_MODE = 0o600;
const STEELENGINE_AGENT_DB_SLOW_OPEN_MS = 1_000;
const STEELENGINE_AGENT_CANONICAL_UNIQUE_INDEXES = [
  {
...
const STEELENGINE_AGENT_MAINTENANCE_SCHEMA_COMPATIBILITY = {
...
export type SteelEngineAgentDatabase = {
  agentId: string;
  db: DatabaseSync;
  path: string;
...
export type SteelEngineAgentDatabaseOptions = SteelEngineStateDatabaseOptions & {
  agentId: string;
};

/** Shared-state registry row describing an agent database seen by this process. */
export type SteelEngineRegisteredAgentDatabase = {
  agentId: string;
  path: string;
...
type SteelEngineAgentMetadataDatabase = Pick<SteelEngineAgentKyselyDatabase, "schema_meta">;
type SteelEngineAgentRegistryDatabase = Pick<SteelEngineStateKyselyDatabase, "agent_databases">;

const cachedDatabases = new Map<string, SteelEngineAgentDatabase>();
const terminalOpenLatch = createSqliteTerminalOpenLatch({
  closeByPath: closeSteelEngineAgentDatabaseByPath,
});

/** Latch background verification damage so later opens fail without rescanning. */
export function recordSteelEngineAgentDatabaseOpenFailure(pathname: string, error: Error): void {
 ...
 */
export function clearSteelEngineAgentDatabaseOpenFailure(
  pathname: string,
  options: SteelEngineStateDatabaseOptions = {},
): boolean {
  const resolvedPath = path.resolve(pathname);
  const cleared = clearSteelEngineDatabaseQuarantine(resolvedPath, { env: options.env });
...
function logSlowAgentDatabaseOpen(params: {
  ...
  if (params.elapsedMs < STEELENGINE_AGENT_DB_SLOW_OPEN_MS) {
    return;
  }
  agentDbLog.warn("slow SteelEngine agent database open", {
    agentId: params.agentId,
    elapsedMs: params.elapsedMs,
    path: params.path,
    thresholdMs: STEELENGINE_AGENT_DB_SLOW_OPEN_MS,
  });
}

function assertSupportedAgentSchemaVersion(db: DatabaseSync, pathname: string): void {
  const userVersion = readSqliteUserVersion(db);
  if (userVersion > STEELENGINE_AGENT_SCHEMA_VERSION) {
    throw createNewerSqliteSchemaVersionError(
      "SteelEngine agent database",
      pathname,
      userVersion,
...
function migrateSteelEngineAgentSchema(db: DatabaseSync): void {
  const userVersion = readSqliteUserVersion(db);
  if (userVersion >= STEELENGINE_AGENT_SCHEMA_VERSION) {
    return;
  }
...
function backfillSteelEngineAgentSchema(db: DatabaseSync, previousVersion: number): void {
...
export function ensureSteelEngineAgentDatabasePermissions(
  pathname: string,
  options: SteelEngineAgentDatabaseOptions,
): void {
  const dir = path.dirname(pathname);
  const defaultPath = resolveSteelEngineAgentSqlitePath({
    agentId: options.agentId,
    env: options.env,
  });
  const isDefaultAgentDatabase = path.resolve(pathname) === path.resolve(defaultPath);
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: STEELENGINE_AGENT_DB_DIR_MODE });
  ...
  if (isDefaultAgentDatabase || !dirExisted) {
    chmodSync(dir, STEELENGINE_AGENT_DB_DIR_MODE);
  }
  for (const candidate of resolveSqliteDatabaseFilePaths(pathname)) {
    try {
      chmodSync(candidate, STEELENGINE_AGENT_DB_FILE_MODE);
    } catch (error) {
...
function assertExistingSchemaOwner(
  ...
  if (existing.role !== "agent") {
    throw new Error(
      `SteelEngine agent database ${pathname} has schema role ${existing.role ?? "unknown"}; expected agent.`,
    );
  }
  if (!existing.agentId) {
    throw new Error(`SteelEngine agent database ${pathname} has no agent owner.`);
  }
  if (normalizeAgentId(existing.agentId) !== agentId) {
    throw new Error(
      `SteelEngine agent database ${pathname} belongs to agent ${existing.agentId}; requested agent ${agentId}.`,
    );
  }
}

/** Require the exact agent owner and schema before offline file maintenance. */
export function assertSteelEngineAgentDatabaseForMaintenance(
  ...
  if (!metadata) {
    throw new Error(
      `SteelEngine agent database ${options.pathname} has no schema ownership metadata.`,
    );
  }
  ...
  if (userVersion !== STEELENGINE_AGENT_SCHEMA_VERSION) {
    throw new Error(
      `SteelEngine agent database ${options.pathname} uses schema version ${userVersion}; run steelengine doctor --fix before compacting it.`,
    );
  }
  if (metadata.schemaVersion !== STEELENGINE_AGENT_SCHEMA_VERSION) {
    throw new Error(
      `SteelEngine agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match ${STEELENGINE_AGENT_SCHEMA_VERSION}; run steelengine doctor --fix before compacting it.`,
    );
  }
  assertSqliteSchemaContains(
    database,
    ...
    STEELENGINE_AGENT_MAINTENANCE_SCHEMA_COMPATIBILITY,
  );
}

/** Upgrade a supported older owned schema before strict offline maintenance. */
export function migrateSteelEngineAgentDatabaseForMaintenance(options: {
  agentId: string;
  pathname: string;
}): void {
  const agentId = normalizeAgentId(options.agentId);
  ...
  try {
    ...
    const hasSupportedOlderVersion =
      ...
      metadataVersion >= 1 &&
      metadataVersion < STEELENGINE_AGENT_SCHEMA_VERSION;
    if (!hasSupportedOlderVersion) {
      return;
    }
    ensureSteelEngineAgentDatabaseSchema(database, {
      agentId,
...
function ensureAgentSchema(db: DatabaseSync, agentId: string, pathname: string): void {
  ...
  try {
    runSqliteImmediateTransactionSync(db, () => {
      ...
      migrateSteelEngineAgentSchema(db);
      db.exec(STEELENGINE_AGENT_SCHEMA_SQL);
      migrateConversationDeliveryTargetColumn(db);
      migrateSessionTranscriptActiveProjection(db, previousVersion);
      if (previousVersion < 11) {
        migrateSqliteSchemaToStrictInTransaction(db, STEELENGINE_AGENT_SCHEMA_SQL, {
          databaseLabel: pathname,
        });
      }
      repairCanonicalSqliteUniqueIndexes(db, pathname, STEELENGINE_AGENT_CANONICAL_UNIQUE_INDEXES);
      backfillSteelEngineAgentSchema(db, previousVersion);
      if (previousVersion < 11) {
        backfillSessionConversations(db);
      }
      backfillSessionEntryProvenance(db, previousVersion);
      const kysely = getNodeSqliteKysely<SteelEngineAgentMetadataDatabase>(db);
      db.exec(`PRAGMA user_version = ${STEELENGINE_AGENT_SCHEMA_VERSION};`);
      const now = Date.now();
      executeSqliteQuerySync(
        db,
        kysely
...
export function ensureSteelEngineAgentDatabaseSchema(
  db: DatabaseSync,
  options: SteelEngineAgentDatabaseOptions & { register?: boolean },
): void {
  const agentId = normalizeAgentId(options.agentId);
  const databaseOptions = { ...options, agentId };
  const pathname = resolveSteelEngineAgentSqlitePath(databaseOptions);
  ensureSteelEngineAgentDatabasePermissions(pathname, databaseOptions);
  ...
  configureSqlitePreSchemaPragmas(db, {
    busyTimeoutMs: STEELENGINE_SQLITE_BUSY_TIMEOUT_MS,
  });
...
function registerAgentDatabase(params: {
  ...
  runSteelEngineStateWriteTransaction(
    (database) => {
      const db = getNodeSqliteKysely<SteelEngineAgentRegistryDatabase>(database.db);
      executeSqliteQuerySync(
        database.db,
        db
...
/** List agent databases recorded in the shared SteelEngine state registry. */
export function listSteelEngineRegisteredAgentDatabases(
  options: SteelEngineStateDatabaseOptions = {},
): SteelEngineRegisteredAgentDatabase[] {
  ...
  if (!existsSync(pathname)) {
    if (hasUnavailableMissingSqlitePath(pathname)) {
      throw new Error(`SteelEngine state database ${pathname} is unavailable.`);
    }
    return [];
  }
  if (detectSteelEngineStateDatabaseSchemaMigrations(options).length > 0) {
    throw new Error(
      `SteelEngine state database ${pathname} has a legacy agent database registry schema; run steelengine doctor --fix to migrate it.`,
    );
  }
  ...
  try {
    database.exec(`PRAGMA busy_timeout = ${STEELENGINE_SQLITE_BUSY_TIMEOUT_MS};`);
    if (readSqliteUserVersion(database) > STEELENGINE_STATE_SCHEMA_VERSION) {
      throw new Error(
        `SteelEngine state database ${pathname} uses a newer schema than this SteelEngine build.`,
      );
    }
    ...
    if (registryTable.type !== "table") {
      throw new Error(`SteelEngine state database ${pathname} has an invalid agent registry.`);
    }
    const db = getNodeSqliteKysely<SteelEngineAgentRegistryDatabase>(database);
...
export type SteelEngineAgentDatabaseOwnerInspection =
  | { status: "owned"; agentId: string }
  | { status: "unowned" }
  | { status: "unreadable" };

/** Read a database's durable role and agent owner without mutating it. */
export function inspectSteelEngineAgentDatabaseOwner(
  pathname: string,
): SteelEngineAgentDatabaseOwnerInspection {
  const sqlite = requireNodeSqlite();
...
function assertAgentDatabaseIntegrityBeforeMutation(
  ...
  if (
    ...
    (userVersion > 0 && userVersion < STEELENGINE_AGENT_SCHEMA_VERSION)
  ) {
    ...
    agentDbLog.info("agent database schema migration pending; verifying integrity first", {
      ...
      path: pathname,
      toVersion: STEELENGINE_AGENT_SCHEMA_VERSION,
    });
...
export function openSteelEngineAgentDatabase(
  options: SteelEngineAgentDatabaseOptions,
): SteelEngineAgentDatabase {
  const agentId = normalizeAgentId(options.agentId);
  const databaseOptions = { ...options, agentId };
  const pathname = resolveSteelEngineAgentSqlitePath(databaseOptions);
  ...
  if (cached?.db.isOpen) {
    if (cached.agentId !== agentId) {
      throw new Error(
        `SteelEngine agent database ${pathname} is already open for agent ${cached.agentId}; requested agent ${agentId}.`,
      );
    }
    return cached;
  }
  ...
  try {
    const quarantine = readSteelEngineDatabaseQuarantine(pathname, { env: databaseOptions.env });
    if (quarantine) {
      persistedFailure = createSteelEngineDatabaseVerificationError(
        "agent",
        pathname,
        quarantine.reason,
      );
  ...
  if (persistedFailure) {
    recordSteelEngineAgentDatabaseOpenFailure(pathname, persistedFailure);
  ...
  const walMaintenance = (() => {
    ...
    try {
      ...
      maintenance = configureSqliteConnectionPragmas(db, {
        busyTimeoutMs: STEELENGINE_SQLITE_BUSY_TIMEOUT_MS,
        databaseLabel: `steelengine-agent:${agentId}`,
      ...
      if (
        ...
        recordSteelEngineAgentDatabaseOpenFailure(pathname, err);
      }
      throw err;
    }
  })();
  ...
  try {
    ...
    closeCachedSteelEngineAgentDatabase(database);
    throw error;
  }
  ...
  unregisterExitClose ??= registerSqliteCacheExitClose(closeSteelEngineAgentDatabases);
...
const postCommitPublications = new WeakMap<SteelEngineAgentDatabase, Array<() => void>>();

/** Queue a non-throwing runtime publication on the outer database commit edge. */
export function deferSteelEngineAgentPostCommitPublication(
...
export function runSteelEngineAgentWriteTransaction<T>(
    ...
    result = runSqliteImmediateTransactionSync(
      ...
      () => {
        ...
        if (!enteredNestedTransaction) {
          ...
          ensureSteelEngineAgentDatabasePermissions(database.path, options);
        }
        return operationResult;
      },
      {
        busyTimeoutMs: transactionOptions.busyTimeoutMs ?? STEELENGINE_SQLITE_BUSY_TIMEOUT_MS,
...
function closeCachedSteelEngineAgentDatabase(database: SteelEngineAgentDatabase): void {
...
export function isSteelEngineAgentDatabaseOpen(pathname: string): boolean {
...
export function closeSteelEngineAgentDatabaseByPath(pathname: string): boolean {
...
export function disposeSteelEngineAgentDatabaseByPath(
  pathname: string,
  ...
  try {
    ...
    closeSteelEngineAgentDatabaseByPath(resolvedPath);
  }
  return true;
}

/** Close all cached agent database handles. */
export function closeSteelEngineAgentDatabases(): void {
  unregisterExitClose?.();
  unregisterExitClose = null;
...
export function closeSteelEngineAgentDatabasesForTest(): void {
  closeSteelEngineAgentDatabases();
