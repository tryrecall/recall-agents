// SteelEngine state database tests cover state DB migrations and persistence.
...
import {
  ...
  withSteelEngineStateStartupMigrationCheckpointDatabase,
...
function createTempStateDir(): string {
  return makeTempDir(stateDbTempDirs, "steelengine-state-db-");
}
...
function seedLegacySessionWatchCursorSchema(stateDir: string): {
  ...
  const databasePath = openSteelEngineStateDatabase(options).path;
  closeSteelEngineStateDatabaseForTest();
...
function runHotRollbackJournalRecoveryProbe(params: { moduleUrl: string; rootDir: string }): {
  ...
  const probeSource = `
    ...
    const initial = openSteelEngineStateDatabase({ path: databasePath });
    initial.db.exec(\`
    ...
    const writerSource = \`
      ...
      const database = new DatabaseSync(process.env.STEELENGINE_HOT_JOURNAL_DATABASE_PATH);
      ...
      database
        ...
        .run("uncommitted");
      fs.writeFileSync(process.env.STEELENGINE_HOT_JOURNAL_READY_PATH, "ready");
      setInterval(() => {}, 1_000);
    \`;
    const writer = spawn(
      process.execPath,
      ["--input-type=module", "-e", writerSource],
      {
        env: {
          ...process.env,
          STEELENGINE_HOT_JOURNAL_DATABASE_PATH: databasePath,
          STEELENGINE_HOT_JOURNAL_READY_PATH: readyPath,
        },
    ...
    try {
      ...
      const reopened = openSteelEngineStateDatabase({ path: databasePath });
...
function expectNoncanonicalAuditSchemaRejected(stateDir: string, databasePath: string): void {
  const options = { env: { STEELENGINE_STATE_DIR: stateDir } };
  expect(detectSteelEngineStateDatabaseSchemaMigrations(options)).toEqual([
    { kind: "audit-events-v2", path: databasePath },
  ]);
  expect(() => openSteelEngineStateDatabase(options)).toThrow(/noncanonical audit event schema/);
  expect(repairSteelEngineStateDatabaseSchema(options)).toEqual({
    changes: [],
...
function runConcurrentSchemaProbe(params: {
  ...
  const workerSource = `
    ...
    const pageBarrierDir = process.env.STEELENGINE_SCHEMA_TEST_PAGE_BARRIER_DIR;
    if (pageBarrierDir) {
      const pageReadyPath = process.env.STEELENGINE_SCHEMA_TEST_PAGE_READY_PATH;
      const workerCount = Number(process.env.STEELENGINE_SCHEMA_TEST_WORKER_COUNT);
    ...
    const {
      closeSteelEngineStateDatabaseForTest,
      openSteelEngineStateDatabase,
    } = await import(process.env.STEELENGINE_SCHEMA_TEST_MODULE_URL);
    const databasePath = process.env.STEELENGINE_SCHEMA_TEST_DATABASE_PATH;
    const readyPath = process.env.STEELENGINE_SCHEMA_TEST_READY_PATH;
    const startPath = process.env.STEELENGINE_SCHEMA_TEST_START_PATH;
    ...
    try {
      const database = openSteelEngineStateDatabase({ path: databasePath });
  ...
  const orchestratorSource = `
    ...
    for (let round = 0; round < roundCount; round += 1) {
      ...
      if (mode === "upgrade") {
        ...
        openSteelEngineStateDatabase({ path: databasePath });
        closeSteelEngineStateDatabaseForTest();
        ...
        legacy.exec(\`
          ...
          ALTER TABLE worker_environments DROP COLUMN bootstrap_steelengine_version;
      ...
      const workers = Array.from({ length: workerCount }, (_, index) => {
        ...
        return spawn(
          ...
          ["--import", "tsx", "--input-type=module", "-e", workerSource],
          {
            env: {
              ...process.env,
              STEELENGINE_SCHEMA_TEST_DATABASE_PATH: databasePath,
              STEELENGINE_SCHEMA_TEST_MODULE_URL: moduleUrl,
              STEELENGINE_SCHEMA_TEST_READY_PATH: readyPath,
              STEELENGINE_SCHEMA_TEST_START_PATH: startPath,
              ...(mode === "fresh"
                ? {
                    STEELENGINE_SCHEMA_TEST_PAGE_BARRIER_DIR: barrierDir,
                    STEELENGINE_SCHEMA_TEST_PAGE_READY_PATH: pageReadyPath,
                    STEELENGINE_SCHEMA_TEST_WORKER_COUNT: String(workerCount),
                  }
                : {}),
            },
...
describe("steelengine state database", () => {
  it("resolves under the shared state database directory", () => {
    const stateDir = createTempStateDir();

    expect(resolveSteelEngineStateSqlitePath({ STEELENGINE_STATE_DIR: stateDir })).toBe(
      path.join(stateDir, "state", "steelengine.sqlite"),
    );
  });
  ...
  it("keeps test default state under a worker-sharded temp directory", () => {
    expect(
      ...
      path.join(os.tmpdir(), "steelengine-test-state", `${process.pid}-7`, "state", "steelengine.sqlite"),
    );
  });
  ...
  it("creates the shared state schema from the committed SQL shape", () => {
    ...
    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./steelengine-state-schema.sql", import.meta.url)),
    );
    expect(database.path).toBe(path.join(stateDir, "state", "steelengine.sqlite"));
    expect(
      database.db
        .prepare(
  ...
  it("drops unreleased transient verification history on open", () => {
    ...
    const reopened = openSteelEngineStateDatabase(options);
    expect(
      reopened.db
  ...
  it("doctor migrates existing APNs tombstone tables to STRICT without losing rows", () => {
    ...
    const migrated = openSteelEngineStateDatabase(options);
    expect(
      migrated.db
        .prepare("SELECT strict FROM pragma_table_list WHERE name = 'apns_registration_tombstones'")
        .get(),
  ...
  it("doctor migrates version 2 tables to STRICT without losing rows", () => {
    const stateDir = createTempStateDir();
    const options = { env: { STEELENGINE_STATE_DIR: stateDir } };
    const opened = openSteelEngineStateDatabase(options);
  ...
  it("doctor migrates version 3 ambient watch sentinels into cursor provenance", () => {
    ...
    expect(readSqliteNumberPragma(migrated.db, "user_version")).toBe(STEELENGINE_STATE_SCHEMA_VERSION);
    expect(
      migrated.db
        .prepare("SELECT schema_version FROM schema_meta WHERE meta_key = 'primary'")
        .get(),
    ).toEqual({ schema_version: STEELENGINE_STATE_SCHEMA_VERSION });
    closeSteelEngineStateDatabaseForTest();
    expect(repairSteelEngineStateDatabaseSchema(options)).toEqual({ changes: [], warnings: [] });
  });
  ...
  it("rejects a placement turn claim tuple without an owner", () => {
    const database = openSteelEngineStateDatabase({
      env: { STEELENGINE_STATE_DIR: createTempStateDir() },
    });
  ...
  it("repairs a same-name shared-state uniqueness index", () => {
    ...
    const created = openSteelEngineStateDatabase({ env });
    ...
    const reopened = openSteelEngineStateDatabase({ env });
    expect(
      reopened.db
        .prepare(
  ...
  it("migrates the released audit ledger to message-compatible attribution exactly once", () => {
    ...
    expect(() => openSteelEngineStateDatabase(options)).toThrow(/legacy audit event schema/);
    ...
    try {
      ...
      expect(readSqliteNumberPragma(db, "user_version")).toBe(STEELENGINE_STATE_SCHEMA_VERSION);
      expect(
  ...
  it("preserves an empty audit ledger's sequence high-water mark", () => {
    ...
    expect(repairSteelEngineStateDatabaseSchema(options).warnings).toEqual([]);
  ...
  it("lets normal open create an audit ledger for a pre-v2 database", () => {
    ...
    expect(readSqliteNumberPragma(opened.db, "user_version")).toBe(STEELENGINE_STATE_SCHEMA_VERSION);
  });
  ...
  it("refuses to rebuild a noncanonical audit table with unknown data columns", () => {
    ...
    const result = repairSteelEngineStateDatabaseSchema(options);
  ...
  it.runIf(process.platform === "linux")("closes the database when initialization fails", () => {
    const databasePath = path.join(createTempStateDir(), "steelengine.sqlite");
    fs.writeFileSync(databasePath, "not a sqlite database");

    expect(() => openSteelEngineStateDatabase({ path: databasePath })).toThrow(
  ...
  it("rejects stale schema_meta indexes before writable initialization", () => {
    ...
    expect(() => openSteelEngineStateDatabase(options)).toThrow(
  ...
  it("defers unrelated current-schema index corruption but keeps doctor scans full", () => {
    ...
    expect(openSteelEngineStateDatabase(options).db.isOpen).toBe(true);
    closeSteelEngineStateDatabaseForTest();
    expect(repairSteelEngineStateDatabaseSchema(options)).toEqual({
      changes: [],
      warnings: [
    ...
    expect(() =>
      withSteelEngineStateStartupMigrationCheckpointDatabase(checkpointCallback, options),
  ...
  it("runs full integrity before a pending state schema migration", () => {
    ...
    try {
      before.exec(`PRAGMA user_version = ${STEELENGINE_STATE_SCHEMA_VERSION - 1};`);
    } finally {
      before.close();
    }
    ...
    () => {
      expect(
        runHotRollbackJournalRecoveryProbe({
          moduleUrl: new URL("./steelengine-state-db.ts", import.meta.url).href,
  ...
  it("backfills durable approval transport references in databases created by PR 1", () => {
    ...
    const reopened = openSteelEngineStateDatabase({ env: { STEELENGINE_STATE_DIR: stateDir } });
    expect(
      reopened.db
  ...
  it("migrates operator approvals to accept system-agent records", () => {
    ...
    expect(detectSteelEngineStateDatabaseSchemaMigrations(options)).toContainEqual({
      kind: "operator-approvals-system-agent",
      path: databasePath,
    });
    expect(repairSteelEngineStateDatabaseSchema(options)).toEqual({
      changes: ["Migrated shared state operator approvals → SteelEngine system changes"],
      warnings: [],
    });
  ...
  it("does not recursively recommend doctor when operator approval repair refuses a shape", () => {
    ...
    expect(result.warnings[0]).not.toContain("run steelengine doctor --fix");
  });
  ...
  it("adds managed-image typed columns before creating canonical indexes", () => {
    ...
    assertSteelEngineStateDatabaseForMaintenance(reopened.db, { pathname: reopened.path });
  ...
  it("serializes concurrent additive schema upgrades across processes", () => {
    ...
    const moduleUrl = new URL("./steelengine-state-db.ts", import.meta.url).href;
    ...
    const expectedShape = createSqliteSchemaShapeFromSql(
      new URL("./steelengine-state-schema.sql", import.meta.url),
    );
    const { DatabaseSync } = requireNodeSqlite();
  ...
  it("migrates requester and executor attribution for existing cross-agent tasks", () => {
    ...
    const currentReopened = openSteelEngineStateDatabase({
      env: { STEELENGINE_STATE_DIR: stateDir },
    });
    expect(
      currentReopened.db
  ...
  it("normalizes obsolete task delivery statuses in existing state databases", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-state-task-delivery-status-" },
      async ({ stateDir }) => {
        const database = openSteelEngineStateDatabase({
          env: { STEELENGINE_STATE_DIR: stateDir },
        });
  ...
  it("configures durable SQLite connection pragmas", () => {
    ...
    expect(readSqliteNumberPragma(database.db, "user_version")).toBe(STEELENGINE_STATE_SCHEMA_VERSION);
  ...
  it("latches newer global schema failures before integrity scans", () => {
    ...
    try {
      openSteelEngineStateDatabase(options);
    } catch (error) {
      firstFailure = error;
    }
    expect(firstFailure).toMatchObject({
      name: "SqliteSchemaVersionError",
      message: expect.stringContaining("https://docs.steelengine.ai/reference/database-schemas"),
    });
    ...
    clearSteelEngineStateDatabaseOpenFailure(databasePath);
    expect(openSteelEngineStateDatabase(options).db.isOpen).toBe(true);
  });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    const databasePath = path.join(
      os.tmpdir(),
      `steelengine-explicit-state-${process.pid}-${Date.now()}.sqlite`,
    );

    expect(() => openSteelEngineStateDatabase({ path: databasePath })).not.toThrow();
  ...
  it("keeps cached handles open when another state path is opened", () => {
    ...
    const first = openSteelEngineStateDatabase({ path: firstPath });
    const second = openSteelEngineStateDatabase({ path: secondPath });

    expect(first.db.isOpen).toBe(true);
    expect(second.db.isOpen).toBe(true);
    expect(openSteelEngineStateDatabase({ path: firstPath })).toBe(first);
    expect(readSqliteNumberPragma(first.db, "user_version")).toBe(STEELENGINE_STATE_SCHEMA_VERSION);
  });
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "steelengine-state-db-relative-"));
          ...
          try {
            ...
            const first = openSteelEngineStateDatabase({ path: "state.sqlite" });
            first.db
            ...
            const second = openSteelEngineStateDatabase({ path: "state.sqlite" });
            second.db
              .prepare("INSERT INTO diagnostic_events (scope, event_key, payload_json, created_at) VALUES (?, ?, ?, ?)")
  ...
  it("uses savepoints for nested write transaction rollback", () => {
    ...
    runSteelEngineStateWriteTransaction((database) => {
      ...
      expect(() =>
        runSteelEngineStateWriteTransaction((inner) => {
  ...
  it("rejects Promise-returning write transactions", () => {
    ...
    expect(() =>
      runSteelEngineStateWriteTransaction(async () => {
        return "not sync";
      }, options),
