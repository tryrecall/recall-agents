...
import {
  ...
  recordSteelEngineAgentDatabaseOpenFailure,
} from "./steelengine-agent-db.js";
import type {
  SteelEngineDatabaseVerifyResult,
  SteelEngineDatabaseVerifyTarget,
} from "./steelengine-database-verify.worker.js";
import { recordSteelEngineDatabaseQuarantine } from "./steelengine-quarantine-store.js";
import { recordSteelEngineStateDatabaseOpenFailure } from "./steelengine-state-db.js";
import { resolveSteelEngineStateSqlitePath } from "./steelengine-state-db.paths.js";

export const STEELENGINE_DATABASE_VERIFY_INITIAL_DELAY_MS = 5 * 60_000;
export const STEELENGINE_DATABASE_VERIFY_INTERVAL_MS = 24 * 60 * 60_000;
...
function resolveDatabaseVerifyWorkerUrl(currentModuleUrl = import.meta.url): URL {
  ...
  if (distIndex >= 0) {
    ...
    return pathToFileURL(path.join(distRoot, "state", "steelengine-database-verify.worker.js"));
  }
  const extension = path.extname(currentPath) || ".js";
  return new URL(`./steelengine-database-verify.worker${extension}`, currentModuleUrl);
}

function isVerifyResult(value: unknown): value is SteelEngineDatabaseVerifyResult {
...
): Promise<SteelEngineDatabaseVerifyResult[]> {
...
export function collectSteelEngineDatabaseVerifyTargets(options: {
  env: NodeJS.ProcessEnv;
}): SteelEngineDatabaseVerifyTarget[] {
  const targets = new Map<string, SteelEngineDatabaseVerifyTarget>();
  const statePath = path.resolve(resolveSteelEngineStateSqlitePath(options.env));
  if (existsSync(statePath)) {
    targets.set(statePath, { kind: "state", label: "SteelEngine state database", path: statePath });
  }
  let registeredDatabases: ReturnType<typeof listSteelEngineRegisteredAgentDatabases> = [];
  try {
    registeredDatabases = listSteelEngineRegisteredAgentDatabases({ env: options.env });
  } catch (error) {
  ...
  for (const registered of registeredDatabases) {
    ...
    targets.set(agentPath, {
      kind: "agent",
      label: `SteelEngine agent database ${registered.agentId}`,
      path: agentPath,
    });
  }
...
function createVerificationFailure(result: SteelEngineDatabaseVerifyResult): Error {
...
export function applySteelEngineDatabaseVerificationResults(options: {
  env: NodeJS.ProcessEnv;
  results: readonly SteelEngineDatabaseVerifyResult[];
  targets: readonly SteelEngineDatabaseVerifyTarget[];
}): void {
  ...
  for (const result of options.results) {
    ...
    const recorded = recordSteelEngineDatabaseQuarantine({
      env: options.env,
      kind: target.kind,
      path: result.path,
    ...
    if (target.kind === "state") {
      recordSteelEngineStateDatabaseOpenFailure(result.path, error);
    } else {
      recordSteelEngineAgentDatabaseOpenFailure(result.path, error);
    }
