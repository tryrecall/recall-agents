...
type SteelEngineAgentSqlitePathOptions = {
  agentId: string;
...
export function resolveSteelEngineAgentSqlitePath(options: SteelEngineAgentSqlitePathOptions): string {
  const agentId = normalizeAgentId(options.agentId);
  return path.resolve(
    options.path ??
      path.join(
        path.dirname(resolveSteelEngineStateSqliteDir(options.env ?? process.env)),
        "agents",
        agentId,
        "agent",
