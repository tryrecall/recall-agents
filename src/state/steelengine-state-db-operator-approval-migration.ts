...
import { tableExists } from "./steelengine-state-db-schema-helpers.js";
...
export function assertCanonicalOperatorApprovalKinds(db: DatabaseSync, pathname: string): void {
  if (!hasCanonicalOperatorApprovalKinds(db)) {
    throw new Error(
      `SteelEngine state database ${pathname} has a legacy operator approval schema; run steelengine doctor --fix to migrate it.`,
    );
  }
}
...
function canonicalOperatorApprovalCreateSql(): string {
  ...
  const start = STEELENGINE_STATE_SCHEMA_SQL.indexOf(marker);
  const end = STEELENGINE_STATE_SCHEMA_SQL.indexOf(
  ...
  return STEELENGINE_STATE_SCHEMA_SQL.slice(start, end + tableTerminator.length);
}
...
function operatorApprovalIndexSql(): string {
  const statements = STEELENGINE_STATE_SCHEMA_SQL.split(";")
...
export function repairOperatorApprovalSchema(db: DatabaseSync): string[] {
  return repairOperatorApprovalKinds(db)
    ? ["Migrated shared state operator approvals → SteelEngine system changes"]
    : [];
}
