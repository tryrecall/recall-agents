...
type AgentLeaseDatabase = Pick<SteelEngineAgentKyselyDatabase, "state_leases">;
type LeaseKysely = ReturnType<typeof getNodeSqliteKysely<LeaseDatabase>>;

type SteelEngineStateLeaseDatabase = { scope: "shared" } | { scope: "agent"; agentId: string };

type SteelEngineStateLeaseOptions = {
  scope: string;
  key: string;
  database: SteelEngineStateLeaseDatabase;
  leaseMs: number;
  waitMs: number;
  signal?: AbortSignal;
...
export type SteelEngineStateLeaseContext = {
  signal: AbortSignal;
...
export type SteelEngineStateLeaseErrorCode =
  | "STEELENGINE_STATE_LEASE_INVALID_INPUT"
  | "STEELENGINE_STATE_LEASE_TIMEOUT"
  | "STEELENGINE_STATE_LEASE_ABORTED"
  | "STEELENGINE_STATE_LEASE_LOST"
  | "STEELENGINE_STATE_LEASE_STORAGE_FAILED";

export class SteelEngineStateLeaseError extends Error {
  readonly code: SteelEngineStateLeaseErrorCode;

  constructor(message: string, options: { code: SteelEngineStateLeaseErrorCode; cause?: unknown }) {
    super(message, { cause: options.cause });
    this.name = "SteelEngineStateLeaseError";
...
function leaseError(
  code: SteelEngineStateLeaseErrorCode,
  message: string,
  cause?: unknown,
): SteelEngineStateLeaseError {
  return new SteelEngineStateLeaseError(message, {
    code,
    ...(cause === undefined ? {} : { cause }),
  });
}

function invalidInput(message: string): SteelEngineStateLeaseError {
  return leaseError("STEELENGINE_STATE_LEASE_INVALID_INPUT", message);
}
...
function validateOptions(options: SteelEngineStateLeaseOptions) {
...
function withLeaseWriteTransaction<T>(
  ...
  const agentDatabase = openSteelEngineAgentDatabase({ agentId: database.agentId });
  const run = () =>
    runSteelEngineAgentWriteTransaction(
...
function withLeaseRead<T>(
  ...
  const sqlite =
    database.scope === "shared"
      ? openSteelEngineStateDatabase().db
      : openSteelEngineAgentDatabase({ agentId: database.agentId }).db;
...
function verifyLeaseOwnership(
  params: LeaseIdentity & { database?: SteelEngineStateLeaseDatabase; transaction?: DatabaseSync },
): void {
  try {
...
export async function withSteelEngineStateLease<T>(
  options: SteelEngineStateLeaseOptions,
  ...
  const abortLost = (cause?: unknown) => {
    if (!leaseLost.signal.aborted) {
      leaseLost.abort(
        cause instanceof SteelEngineStateLeaseError
          ? cause
          : leaseError(
              "STEELENGINE_STATE_LEASE_LOST",
  ...
  const heartbeat = setInterval(() => {
    try {
      ...
      if (error instanceof SteelEngineStateLeaseError && error.code === "STEELENGINE_STATE_LEASE_LOST") {
        abortLost(error);
