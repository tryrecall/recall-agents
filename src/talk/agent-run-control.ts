/**
 * Runtime adapter for realtime voice control of active SteelEngine agent runs.
 *
...
export async function controlRealtimeVoiceAgentRun(
  ...
  if (mode === "cancel") {
    ...
    const message = aborted
      ? "Cancelled the active SteelEngine run."
      : "SteelEngine could not cancel the active run.";
    return {
      ok: aborted,
      mode,
      sessionKey,
  ...
  return {
    ...
    message:
      mode === "followup"
        ? "Queued that follow-up for the active SteelEngine run."
