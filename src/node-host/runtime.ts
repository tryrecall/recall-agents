...
import type { SteelEnginePluginNodeHostCommandContext } from "../plugins/types.node-host.js";
...
function ensureNodePathEnv(): string {
  ensureSteelEngineCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
export async function prepareNodeHostRuntime(params?: {
  ...
  return {
    ...
    start({ client, onInventoryChanged, onManifestChanged }) {
      ...
      >();
      const pluginCommandContext: SteelEnginePluginNodeHostCommandContext = {
      ...
      return {
        async invoke(frame) {
          ...
          const pluginCommandIo: SteelEnginePluginNodeHostCommandIo | undefined =
