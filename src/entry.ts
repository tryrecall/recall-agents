#!/usr/bin/env node
// Boots the SteelEngine CLI entry point under Node.
// CLI process entrypoint for SteelEngine command execution.
...
import {
  enableSteelEngineCompileCache,
  resolveEntryInstallRoot,
  respawnWithoutSteelEngineCompileCacheIfNeeded,
} from "./entry.compile-cache.js";
import { buildCliRespawnPlan, runCliRespawnPlan } from "./entry.respawn.js";
import { tryHandleRootVersionFastPath } from "./entry.version-fast-path.js";
import { normalizeEnv } from "./infra/env.js";
import { isMainModule } from "./infra/is-main.js";
import { ensureSteelEngineExecMarkerOnProcess } from "./infra/steelengine-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "steelengine.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "steelengine.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  const waitingForCompileCacheRespawn = respawnWithoutSteelEngineCompileCacheIfNeeded({
    currentFile: entryFile,
    installRoot,
  });
  if (!waitingForCompileCacheRespawn) {
    process.title = "steelengine";
    ...
    if (!ensureCliRespawnReady()) {
      ...
      if (!parsedContainer.ok) {
        console.error(`[steelengine] ${parsedContainer.error}`);
        process.exit(2);
      }
      ...
      if (!parsed.ok) {
        ...
        console.error(`[steelengine] ${parsed.error}`);
        process.exit(2);
      }

      const containerTargetName = resolveCliContainerTarget(process.argv);
      if (containerTargetName && parsed.profile) {
        console.error("[steelengine] --container cannot be combined with --profile/--dev");
        process.exit(2);
      }
...
export async function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[steelengine] Failed to display help:",
