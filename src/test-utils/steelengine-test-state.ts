// Creates isolated SteelEngine state directories for integration-style tests.
...
type SteelEngineTestStateLayout = "home" | "state-only" | "split";

type SteelEngineTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
type SteelEngineTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: SteelEngineTestStateLayout;
  scenario?: SteelEngineTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
export type SteelEngineTestState = {
  root: string;
  home: string;
  stateDir: string;
  configPath: string;
...
const DEFAULT_PREFIX = "steelengine-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "steelengine.json"),
...
function scenarioConfig(options: SteelEngineTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "steelengine-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: SteelEngineTestStateOptions): Record<string, string | undefined> {
...
export async function createSteelEngineTestState(
  options: SteelEngineTestStateOptions = {},
): Promise<SteelEngineTestState> {
  ...
  const state: SteelEngineTestState = {
    root,
    ...paths,
    env,
    envVars,
    ...
    applyEnv: () => {
      ...
      for (const [key, value] of Object.entries(envVars)) {
        // Test fixtures apply a fixed SteelEngine env set, not plugin-provided host env.
        if (value === undefined) {
...
export async function withSteelEngineTestState<T>(
  options: SteelEngineTestStateOptions,
  fn: (state: SteelEngineTestState) => Promise<T>,
): Promise<T> {
  const state = await createSteelEngineTestState(options);
  try {
