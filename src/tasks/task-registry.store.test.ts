...
const ORIGINAL_ENV = captureEnv(["STEELENGINE_STATE_DIR"]);
...
describe("task-registry store runtime", () => {
  ...
  it("logs restore parser failures and keeps the failure sticky", async () => {
    const warnLogs = createWarnLogCapture("steelengine-task-registry-restore-test");
  ...
  it("rejects corrupt persisted task rows during sqlite restore", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-task-store-corrupt-" },
      async () => {
        resetTaskRegistryForTests();
  ...
  it("drops invalid requester origins during sqlite restore", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-task-store-invalid-origin-" },
      async () => {
        resetTaskRegistryForTests();
        const created = createTaskRecord({
          runtime: "acp",
  ...
  it("round-trips runtime-owned task detail through sqlite", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-task-store-detail-" },
      async () => {
  ...
  it("preserves explicit null task detail through sqlite", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-task-store-null-detail-" },
      async () => {
        const task: TaskRecord = {
          ...createStoredTask(),
          detail: null,
  ...
  it("loads task and delivery rows from one sqlite read snapshot", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-task-store-read-snapshot-" },
      async () => {
        resetTaskRegistryForTests();
        const created = createTaskRecord({
          runtime: "acp",
  ...
  it("bypasses stale owner indexes for complete fresh results", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-task-store-owner-index-" },
      async () => {
        resetTaskRegistryForTests();
  ...
  it("persists executor and requester agent ids in sqlite task rows", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-task-agent-id-" },
      async () => {
        const created = createTaskRecord({
          runtime: "subagent",
  ...
  it("persists tool activity across sqlite restore", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-task-tool-activity-" },
      async () => {
        const created = createTaskRecord({
          runtime: "subagent",
          ownerKey: "agent:main:main",
  ...
  it("persists requester origin atomically when creating sqlite tasks", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-task-create-origin-" },
      async () => {
        const created = createTaskRecord({
          runtime: "acp",
  ...
  it("prunes stale sqlite delivery state while retaining current rows", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-task-delivery-prune-" },
      async () => {
  ...
  it("prunes large sqlite snapshots without binding every task id at once", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-task-large-prune-" },
      async () => {
  ...
  it("reopens after the shared state database is closed", async () => {
    await withSteelEngineTestState(
      { layout: "state-only", prefix: "steelengine-task-store-" },
      async () => {
