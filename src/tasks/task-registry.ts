...
const TASK_REGISTRY_DELIVERY_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "steelengine.taskRegistry.deliveryRuntimeOverride",
);
const TASK_REGISTRY_CONTROL_RUNTIME_OVERRIDE_KEY = Symbol.for(
  "steelengine.taskRegistry.controlRuntimeOverride",
);
  ...
  try {
    ...
    if (isBackgroundExecTask(task)) {
      ...
      if (task.runtime === "cron") {
        ...
        // not have SteelEngine child sessions to terminate. Cancellation clears
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("steelengine.taskRegistryTestApi")] = {
