...
describe("shouldAttemptTtsPayload", () => {
  ...
  beforeAll(() => {
    root = mkdtempSync(path.join(tmpdir(), "steelengine-tts-config-"));
  });
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["STEELENGINE_TTS_PREFS"]);
    dir = path.join(root, `case-${caseId++}`);
    mkdirSync(dir, { recursive: true });
    prefsPath = path.join(dir, "tts.json");
    process.env.STEELENGINE_TTS_PREFS = prefsPath;
  });
  ...
  it("skips TTS when config, prefs, and session state leave auto mode off", () => {
    expect(shouldAttemptTtsPayload({ cfg: {} as SteelEngineConfig })).toBe(false);
  });
  ...
  it("honors session auto state before prefs and config", () => {
    ...
    const cfg = { messages: { tts: { auto: "off" } } } as SteelEngineConfig;
  ...
  it("uses local prefs before config auto mode", () => {
    ...
    writeFileSync(prefsPath, JSON.stringify({ tts: { auto: "off" } }));
    expect(
      shouldAttemptTtsPayload({ cfg: { messages: { tts: { enabled: true } } } as SteelEngineConfig }),
    ).toBe(false);
  });
