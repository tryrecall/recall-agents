...
homepage: https://docs.recall.ai/automation/hooks#command-logger
metadata:
  {
    "recall":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.recall/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.recall/logs/commands.log ~/.recall/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/recall`:
   ```
   /home/username/.recall/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
