...
homepage: https://docs.steelengine.ai/automation/hooks#command-logger
metadata:
  {
    "steelengine":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.steelengine/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.steelengine/logs/commands.log ~/.steelengine/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/steelengine`:
   ```
   /home/username/.steelengine/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.steelengine/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.steelengine/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.steelengine/logs/commands.log | jq .
```
