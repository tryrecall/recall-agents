...
This directory contains hooks that ship with SteelEngine. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.steelengine/workspace`)
...
```bash
steelengine hooks enable bootstrap-extra-files
```
...
**Output**: `~/.steelengine/logs/commands.log`

**Enable**:

```bash
steelengine hooks enable command-logger
```
...
homepage: https://docs.steelengine.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.steelengine/hooks/` (shared across workspaces)
...
```bash
steelengine hooks enable session-memory
steelengine hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.steelengine/steelengine.json`:
...
2. Restart gateway: `pkill -9 -f 'steelengine.*gateway' && pnpm steelengine gateway`
3. Enable the hook: `steelengine hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.steelengine.ai/automation/hooks
