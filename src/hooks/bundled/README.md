...
This directory contains hooks that ship with Recall. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
- **Managed hooks**: `~/.recall/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.tryrecall/recall-agents.json`:
...
2. Restart gateway: `pkill -9 -f 'recall.*gateway' && pnpm recall gateway`
3. Enable the hook: `recall hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.recall.ai/automation/hooks
