...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: vi.fn(() => "/tmp/steelengine-agent"),
  resolveAgentDir: vi.fn(() => "/tmp/steelengine-agent/.steelengine-agent"),
}));
...
function requireFirstRunOptions(): Record<string, unknown> {
  ...
  if (!call) {
    throw new Error("expected embedded SteelEngine agent run");
  }
  const [options] = call;
  if (!options || typeof options !== "object") {
    throw new Error("expected embedded SteelEngine agent run options");
  }
