...
type CanonicalInboundMessageHookContext = {
  ...
  // `mediaPath(s)` are files SteelEngine has already staged locally. `mediaUrl(s)`
...
): MessageTranscribedHookContext & { cfg: SteelEngineConfig } {
...
export function toInternalMessagePreprocessedContext(
  canonical: CanonicalInboundMessageHookContext,
  cfg: SteelEngineConfig,
): MessagePreprocessedHookContext & { cfg: SteelEngineConfig } {
  const shared = toInternalInboundMessageHookContextBase(canonical);
  return {
    ...shared,
    transcript: canonical.transcript,
