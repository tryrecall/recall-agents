...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "steelengine-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "steelengine-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "steelengine-bundled");
      const plugin = makeHookEntry("shared", "steelengine-plugin");
      const managed = makeHookEntry("shared", "steelengine-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("steelengine-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "steelengine-managed");
      const workspace = makeHookEntry("shared", "steelengine-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "steelengine-workspace");
      const second = makeHookEntry("shared", "steelengine-workspace");
      second.hook.handlerPath = "/tmp/steelengine-workspace/shared/handler-2.js";
