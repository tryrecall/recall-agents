...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "recall-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "recall-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "recall-bundled");
      const plugin = makeHookEntry("shared", "recall-plugin");
      const managed = makeHookEntry("shared", "recall-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("recall-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "recall-managed");
      const workspace = makeHookEntry("shared", "recall-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "recall-workspace");
      const second = makeHookEntry("shared", "recall-workspace");
      second.hook.handlerPath = "/tmp/recall-workspace/shared/handler-2.js";
