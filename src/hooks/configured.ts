...
export function hasConfiguredInternalHooks(config: RecallConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: RecallConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
