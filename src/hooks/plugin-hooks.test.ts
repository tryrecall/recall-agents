...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "steelengine-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.STEELENGINE_BUNDLED_HOOKS_DIR;
    process.env.STEELENGINE_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.STEELENGINE_BUNDLED_HOOKS_DIR;
    } else {
      process.env.STEELENGINE_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".steelengine", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): SteelEngineConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entry.hook.source).toBe("steelengine-plugin");
  ...
  it("does not treat Claude hooks.json bundles as SteelEngine hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".steelengine", "extensions", "claude-bundle");
