...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "recall-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.RECALL_BUNDLED_HOOKS_DIR;
    process.env.RECALL_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });

  afterEach(() => {
    clearInternalHooks();
    if (previousBundledHooksDir === undefined) {
      delete process.env.RECALL_BUNDLED_HOOKS_DIR;
    } else {
      process.env.RECALL_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    ...
    await fsp.writeFile(
      ...
      [
        ...
        'metadata: {"recall":{"events":["command:new"]}}',
        "---",
        "",
        "# Bundle hook",
        "",
  ...
  function createConfig(enabled: boolean): RecallConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("recall-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Recall hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".recall", "extensions", "claude-bundle");
