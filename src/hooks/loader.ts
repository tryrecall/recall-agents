...
const LOADED_INTERNAL_HOOK_REGISTRATIONS_KEY = Symbol.for(
  "steelengine.loadedInternalHookRegistrations",
);
 ...
 *
 * @param cfg - SteelEngine configuration
...
export async function loadInternalHooks(
        ...
        if (unknownEvents.length > 0) {
          log.warn(
            `Hook '${safeLogValue(entry.hook.name)}' subscribes to event${unknownEvents.length === 1 ? "" : "s"} ` +
              `${unknownEvents.map((event) => safeLogValue(event)).join(", ")} not emitted by SteelEngine core — ` +
              `likely a typo; unless a plugin emits it, the hook never fires. ` +
              `Known events: https://docs.steelengine.ai/automation/hooks`,
          );
        }
  ...
  for (const handlerConfig of handlers) {
    try {
      ...
      const importUrl = buildImportUrl(safeModulePath, "steelengine-workspace");
      const mod = (await import(importUrl)) as Record<string, unknown>;
      ...
      if (!isKnownInternalHookEventKey(handlerConfig.event)) {
        log.warn(
          `Legacy hook handler ${safeLogValue(rawModule)} subscribes to event ` +
            `${safeLogValue(handlerConfig.event)} not emitted by SteelEngine core — ` +
            `likely a typo; unless a plugin emits it, the hook never fires. ` +
            `Known events: https://docs.steelengine.ai/automation/hooks`,
        );
      }
