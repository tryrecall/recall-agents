...
import {
  parseFrontmatter,
  resolveRecallMetadata,
  resolveHookInvocationPolicy,
} from "./frontmatter.js";
import type { RecallHookMetadata } from "./types.js";
...
function requireRecallMetadata(metadata: RecallHookMetadata | undefined): RecallHookMetadata {
  if (!metadata) {
    throw new Error("expected recall metadata");
  }
  return metadata;
}
...
`;
    ...
    expect(parsed.recall.emoji).toBe("💾");
    expect(parsed.recall.events).toEqual(["command:new"]);
  });
...
`;
    ...
    expect(parsed.recall.emoji).toBe("📝");
    expect(parsed.recall.events).toEqual(["command"]);
    expect(parsed.recall.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.recall.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"recall": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"recall": {"events": ["test"]}}');
  });
...
describe("resolveRecallMetadata", () => {
  it("extracts recall metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveRecallMetadata(frontmatter);
    const recall = requireRecallMetadata(result);
    expect(recall.emoji).toBe("🔥");
    expect(recall.events).toEqual(["command:new", "command:reset"]);
    expect(recall.requires?.config).toEqual(["workspace.dir"]);
    expect(recall.requires?.bins).toEqual(["git"]);
  });
  ...
  it("returns undefined when recall key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        recall: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Recall" },
            { id: "npm", kind: "npm", package: "@recall/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@recall/hook");
  });
...
`;
    ...
    const recall = requireRecallMetadata(resolveRecallMetadata(frontmatter));
    expect(recall.emoji).toBe("💾");
    expect(recall.events).toEqual(["command:new", "command:reset"]);
    expect(recall.requires?.config).toEqual(["workspace.dir"]);
    expect(recall.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const recall = resolveRecallMetadata(frontmatter);
    expect(recall?.emoji).toBe("disk");
    expect(recall?.events).toEqual(["command:new"]);
  });
});
