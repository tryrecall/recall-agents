...
const fixtureRoot = path.join(process.cwd(), ".tmp", `recall-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@recall/test-hooks",
      version: "0.0.1",
      recall: { hooks: params.hooks },
...
async function createZipHookPackBuffer(params: {
  ...
  const packageJson = JSON.stringify({
    ...
    version: "0.0.1",
    recall: { hooks: [`./hooks/${params.hookName}`] },
  });
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "recall.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "recall.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.npmResolution?.resolvedSpec).toBe("@recall/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@recall/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = runCommandWithTimeoutMock;
    mockNpmPackMetadataResult(run, {
      id: "@recall/test-hooks@0.0.2-beta.1",
      name: "@recall/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@recall/test-hooks@beta"');
    }
  });
});
