...
const fixtureRoot = path.join(process.cwd(), ".tmp", `steelengine-hook-install-${randomUUID()}`);
...
const zipHooksBuffer = await createZipHookPackBuffer({
  packageName: "@steelengine/zip-hooks",
  hookName: "zip-hook",
...
const npmPackHooksBuffer = await createTarGzHookPackBuffer({
  packageName: "@steelengine/test-hooks",
  hookName: "one-hook",
...
async function createZipHookPackBuffer(params: {
  ...
  const packageJson = JSON.stringify({
    ...
    version: "0.0.1",
    steelengine: { hooks: [`./hooks/${params.hookName}`] },
  });
...
describe("installHooksFromPath", () => {
  it.each([
    {
      ...
      code: HOOK_INSTALL_ERROR_CODE.MISSING_STEELENGINE_HOOKS,
    },
    {
      steelengine: { hooks: [] },
      error: "package.json steelengine.hooks is empty",
      code: HOOK_INSTALL_ERROR_CODE.EMPTY_STEELENGINE_HOOKS,
    },
  ])("returns a stable code for $error", async ({ steelengine, error, code }) => {
    const pkgDir = makeTempDir();
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@steelengine/test-hooks", steelengine }),
    );
  ...
  it("blocks a staged single hook before publishing the target", async () => {
    ...
    expect(scanCall.packageDir).toContain(".steelengine-install-stage-");
  ...
  it.each([".codex-plugin/plugin.json", "hooks/hooks.json", "steelengine.plugin.json"])(
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "steelengine.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "steelengine.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  it("forwards npm install policy metadata through extracted archive validation", async () => {
          ...
          expect(params.archiveInstallParams).toEqual(
            expect.objectContaining({
              installPolicyRequest: {
                kind: "plugin-npm",
                requestedSpecifier: "@steelengine/test-hooks@0.0.1",
                source: {
                  kind: "npm",
    ...
    try {
      const result = await installHooksFromNpmSpec({
        spec: "@steelengine/test-hooks@0.0.1",
      });
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@steelengine/test-hooks@0.0.1",
              name: "@steelengine/test-hooks",
              version: "0.0.1",
              filename: packedName,
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.packageKind).toBe("hook-only");
    expect(result.npmResolution?.resolvedSpec).toBe("@steelengine/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@steelengine/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = runCommandWithTimeoutMock;
    mockNpmPackMetadataResult(run, {
      id: "@steelengine/test-hooks@0.0.2-beta.1",
      name: "@steelengine/test-hooks",
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@steelengine/test-hooks",
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("prerelease version 0.0.2-beta.1");
      expect(result.error).toContain('"@steelengine/test-hooks@beta"');
    }
  });
});
