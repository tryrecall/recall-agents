...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "recall-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["RECALL_BUNDLED_HOOKS_DIR"]);
  ...
  describe("loadInternalHooks", () => {
    it("detects configured internal hook surfaces", () => {
      expect(hasConfiguredInternalHooks({} satisfies RecallConfig)).toBe(false);
      expect(
    ...
    const expectNoCommandHookRegistration = async (cfg: RecallConfig) => {
    ...
    it("skips hook discovery until internal hooks are configured", async () => {
      for (const cfg of [
        {} satisfies RecallConfig,
        { hooks: {} } satisfies RecallConfig,
        { hooks: { internal: {} } } satisfies RecallConfig,
      ]) {
        const count = await loadInternalHooks(cfg, tmpDir);
        expect(count).toBe(0);
      }
