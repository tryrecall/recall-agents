...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "steelengine-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["STEELENGINE_BUNDLED_HOOKS_DIR"]);
  ...
  async function writeDiscoveredHook(params: {
    ...
    await fs.writeFile(
      ...
      [
        ...
        `metadata: {"steelengine":{"events":${JSON.stringify(events)}}}`,
        "---",
        "",
        `# ${params.hookName}`,
      ].join("\n"),
  ...
  describe("loadInternalHooks", () => {
    it("detects configured internal hook surfaces", () => {
      expect(hasConfiguredInternalHooks({} satisfies SteelEngineConfig)).toBe(false);
      expect(
    ...
    const expectNoCommandHookRegistration = async (cfg: SteelEngineConfig) => {
    ...
    it("skips hook discovery until internal hooks are configured", async () => {
      for (const cfg of [
        {} satisfies SteelEngineConfig,
        { hooks: {} } satisfies SteelEngineConfig,
        { hooks: { internal: {} } } satisfies SteelEngineConfig,
      ]) {
        const count = await loadInternalHooks(cfg, tmpDir);
        expect(count).toBe(0);
      }
