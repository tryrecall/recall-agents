...
function resolveSteelEngineHooks(
...
async function installHookPackageFromDir(
  ...
  const hookManifest = resolveSteelEngineHooks(manifest);
  if (!hookManifest.ok) {
    return hookManifest;
  }
  ...
  for (const entry of hookEntries) {
    ...
    if (!runtime.isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `steelengine.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `steelengine.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
async function installHooksFromArchive(
  ...
  return await runtime.withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "steelengine-hook-",
    timeoutMs,
    logger,
...
export async function installHooksFromNpmSpec(
  ...
  return await runtime.installFromValidatedNpmSpecArchive({
    tempDirPrefix: "steelengine-hook-pack-",
    spec,
    timeoutMs,
