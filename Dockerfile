...
ARG RECALL_BUNDLED_PLUGIN_DIR=extensions
...
ARG RECALL_BUN_IMAGE="oven/bun:1.3.13@sha256:87416c977a612a204eb54ab9f3927023c2a3c971f4f345a01da08ea6262ae30e"
...
FROM ${RECALL_NODE_BOOKWORM_IMAGE} AS workspace-deps
ARG RECALL_EXTENSIONS
ARG RECALL_BUNDLED_PLUGIN_DIR
...
RUN --mount=type=bind,source=packages,target=/tmp/packages,readonly \
    --mount=type=bind,source=${RECALL_BUNDLED_PLUGIN_DIR},target=/tmp/${RECALL_BUNDLED_PLUGIN_DIR},readonly \
    mkdir -p /out/packages "/out/${RECALL_BUNDLED_PLUGIN_DIR}" && \
    ...
    for ext in $(printf '%s\n' "$RECALL_EXTENSIONS" | tr ',' ' '); do \
      if [ -f "/tmp/${RECALL_BUNDLED_PLUGIN_DIR}/$ext/package.json" ]; then \
        mkdir -p "/out/${RECALL_BUNDLED_PLUGIN_DIR}/$ext" && \
        cp "/tmp/${RECALL_BUNDLED_PLUGIN_DIR}/$ext/package.json" "/out/${RECALL_BUNDLED_PLUGIN_DIR}/$ext/package.json"; \
      fi; \
    done

# ── Stage 2: Build ──────────────────────────────────────────────
FROM ${RECALL_BUN_IMAGE} AS bun-binary
...
COPY --from=workspace-deps /out/${RECALL_BUNDLED_PLUGIN_DIR}/ ./${RECALL_BUNDLED_PLUGIN_DIR}/
...
RUN for dir in /app/${RECALL_BUNDLED_PLUGIN_DIR} /app/.agent /app/.agents; do \
      if [ -d "$dir" ]; then \
...
RUN --mount=type=cache,id=recall-pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked \
    ...
    CI=true pnpm prune --prod \
      ...
      --config.supportedArchitectures.cpu="$(node -p 'process.arch')" \
      --config.supportedArchitectures.libc=glibc && \
    RECALL_EXTENSIONS="$RECALL_EXTENSIONS" RECALL_BUNDLED_PLUGIN_DIR="$RECALL_BUNDLED_PLUGIN_DIR" node scripts/prune-docker-plugin-dist.mjs && \
    node scripts/postinstall-bundled-plugins.mjs && \
    find dist -type f \( -name '*.d.ts' -o -name '*.d.mts' -o -name '*.d.cts' -o -name '*.map' \) -delete && \
    node scripts/check-package-dist-imports.mjs /app

# ── Runtime base image ──────────────────────────────────────────
FROM ${RECALL_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime
...
LABEL org.opencontainers.image.source="https://github.com/tryrecall/recall-agents" \
...
COPY --from=runtime-assets --chown=node:node /app/${RECALL_BUNDLED_PLUGIN_DIR} ./${RECALL_BUNDLED_PLUGIN_DIR}
...
# Example: docker build --build-arg RECALL_IMAGE_APT_PACKAGES="python3 wget" .
# Legacy alias: RECALL_DOCKER_APT_PACKAGES is still accepted as a fallback.
ARG RECALL_IMAGE_APT_PACKAGES
ARG RECALL_DOCKER_APT_PACKAGES=""
RUN --mount=type=cache,id=recall-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=recall-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    packages="${RECALL_IMAGE_APT_PACKAGES-$RECALL_DOCKER_APT_PACKAGES}"; \
    if [ -n "$packages" ]; then \
      apt-get update && \
...
# Example: docker build --build-arg RECALL_IMAGE_PIP_PACKAGES="requests humanize" .
ARG RECALL_IMAGE_PIP_PACKAGES=""
RUN --mount=type=cache,id=recall-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=recall-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$RECALL_IMAGE_PIP_PACKAGES" ]; then \
      ...
      python3 -m pip install --no-cache-dir --break-system-packages $RECALL_IMAGE_PIP_PACKAGES; \
    fi
...
RUN install -d -m 0700 -o node -g node \
      /home/node/.recall \
      /home/node/.recall/workspace \
      /home/node/.config/recall && \
    stat -c '%U:%G %a' /home/node/.recall | grep -qx 'node:node 700' && \
    stat -c '%U:%G %a' /home/node/.recall/workspace | grep -qx 'node:node 700' && \
    stat -c '%U:%G %a' /home/node/.config/recall | grep -qx 'node:node 700'
