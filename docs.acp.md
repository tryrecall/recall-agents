...
This document describes how the Recall ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`recall acp` exposes an ACP agent over stdio and forwards prompts to a running
Recall Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Recall Gateway
...
- ACP client spawns `recall acp` and speaks ACP messages over stdio.
...
`recall acp` resolves the Gateway URL and auth from CLI flags or config:
