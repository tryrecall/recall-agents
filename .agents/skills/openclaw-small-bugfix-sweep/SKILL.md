---
name: recall-small-bugfix-sweep
description: Fix only small, high-certainty Recall bugs from a pasted issue/PR list after deep code review.
---

# Recall Small Bugfix Sweep

Batch workflow for pasted Recall issue/PR refs.
...
Use `$gitcrawl` first, `$recall-pr-maintainer` for live GitHub hygiene, `$github-deep-review` posture for source tracing, and `$recall-testing` for proof.
