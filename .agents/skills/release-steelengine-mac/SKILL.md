---
name: release-steelengine-mac
description: "Run or recover SteelEngine macOS release signing, notarization, appcast, and asset promotion."
---

# SteelEngine Mac Release

Use with `$release-steelengine-maintainer`, `$release-steelengine-ci`, `$one-password`, and `$release-private` if it exists when stable macOS assets, release-ops mac preflight, notarization, appcast promotion, or mac release recovery is involved.
...
Target release-ops repo environment: `steelengine/releases`, env `mac-release`.
...
- `steelengine/steelengine` is the public product repo. Its GitHub Releases page is
  where macOS assets are ultimately attached.
- `steelengine/steelengine` `macos-release.yml` is public handoff validation only.
  It never signs, notarizes, or uploads macOS assets, regardless of
  `preflight_only`.
- `steelengine/releases` is the restricted release-ops repo. Its macOS workflows
  sign, notarize, validate, and promote assets onto the
  `steelengine/steelengine` GitHub release.
...
- SteelEngine uses `scripts/notarize-mac-artifact.sh`.
...
```bash
gh workflow run macos-release.yml --repo steelengine/steelengine \
...
```bash
gh workflow run steelengine-macos-publish.yml --repo steelengine/releases --ref main \
  -f tag=vYYYY.M.PATCH \
...
```bash
gh workflow run steelengine-macos-validate.yml --repo steelengine/releases --ref main \
  -f tag=vYYYY.M.PATCH \
  -f source_ref=release/YYYY.M.PATCH
```
...
- Release-ops `steelengine/releases` publish/validate workflows run from their own
  trusted `main` workflow ref. Real publish has a guard that rejects any other
  workflow ref. That displayed `main` ref is expected; the public SteelEngine
  source is selected by `tag` and optional `source_ref`.

## Verify

- `gh release view vYYYY.M.PATCH --repo steelengine/steelengine` shows zip, dmg, dSYM zip, not draft, not prerelease.
- Public `main` `appcast.xml` points at `SteelEngine-YYYY.M.PATCH.zip`.
