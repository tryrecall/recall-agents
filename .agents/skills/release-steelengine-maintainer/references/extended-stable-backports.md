...
- Read `docs/reference/RELEASING.md`,
  `scripts/steelengine-npm-extended-stable-release.mjs`, and the relevant release
...
- Target npm `extended-stable` and the canonical
  ...
  npm `latest`.
- Cover the core `steelengine` package and every npm-publishable official plugin
...
- Read `SECURITY.md` and use `$security-triage` for security candidates. Route
  unpublished advisory work through `$steelengine-ghsa-maintainer`; never expose
  private details before the security owner authorizes disclosure.
- Use `$steelengine-testing` for proof selection, `$autoreview` before handoff,
  and `$steelengine-pr-maintainer` for GitHub operations.
...
dist_tags=$(npm view steelengine dist-tags --json --userconfig "$npm_userconfig")
published_version=$(printf '%s' "$dist_tags" | jq -r '."extended-stable" // empty')
if [[ -n "$published_version" ]]; then
  npm view "steelengine@${published_version}" version \
...
`$steelengine-ghsa-maintainer` to:
...
- mode, published `steelengine@extended-stable` version or approved bootstrap
...
core `steelengine` selector-repair command. Repair missing or stale official-
