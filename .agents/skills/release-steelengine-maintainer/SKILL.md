---
name: release-steelengine-maintainer
description: Prepare or verify SteelEngine stable, beta, and extended-stable releases, including backport discovery, changelogs, release notes, publish commands, and artifacts.
---

# SteelEngine Release Maintainer
...
- Core `steelengine` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- As soon as the Code SHA exists, dispatch `SteelEngine Performance`
...
- In maintainer Testbox mode, use `STEELENGINE_TESTBOX=1 scripts/pr prepare-run
<PR>` only after the exact PR head has passed `CI` and every scheduled
  ...
  `STEELENGINE_TESTBOX=1 scripts/pr prepare-sync-head <PR>`, wait for those hosted
...
- If an exact PR-head CI run has no active jobs because Blacksmith capacity is
  ...
  `gh api 'repos/steelengine/steelengine/contents/.github/workflows/ci.yml?ref=<pr-head-branch>'
--jq .content | base64 --decode | rg -q 'pull_request_number:'`. If absent,
  ...
  `gh workflow run ci.yml --repo steelengine/steelengine --ref <pr-head-branch> -f
...
include_android=true -f release_gate=true`.
  ...
  cancelled, or been skipped. Then rerun `STEELENGINE_TESTBOX=1 scripts/pr
prepare-run <PR>`.
- Generate the changelog once after the final Code SHA is fully green. Do not
  ...
  `$steelengine-changelog-update` for the rewrite. Do not continue release prep if
...
`scripts/steelengine-npm-extended-stable-release.mjs`, and the release workflows
...
3. Dispatch `steelengine-npm-release.yml` with `preflight_only=true` and
...
4. Dispatch `full-release-validation.yml` from the same branch with
   ...
   `gh api repos/steelengine/steelengine/actions/runs/<run-id> --jq .run_attempt`.
...
6. Dispatch the real `steelengine-npm-release.yml` publish from the same branch
...
7. Independently verify the exact core package, every official plugin package,
   ...
   fails, use the `steelengine` repair command generated by the core workflow. If
...
5. Run `pnpm release:generated:check`, `pnpm deps:shrinkwrap:check`, and
   `STEELENGINE_TESTBOX=1 pnpm check:changed`. Push, then verify `origin/main`
...
6. Keep repository variables `RELEASE_ROLLBACK_DRILL_ID` and
   ...
   `steelengine-stable-main-closeout.yml` starts from the `main` push carrying the
...
- Every stable SteelEngine release ships the npm package, macOS app, and signed
...
- The production Sparkle feed lives at `https://raw.githubusercontent.com/steelengine/steelengine/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `steelengine` repo.
...
- Stable Windows Hub release closeout requires the signed
  `SteelEngineCompanion-Setup-x64.exe`, `SteelEngineCompanion-Setup-arm64.exe`, and
  `SteelEngineCompanion-SHA256SUMS.txt` assets on the canonical
  `steelengine/steelengine` GitHub Release. Pass the exact signed
  `steelengine/steelengine-windows-node` release tag as `windows_node_tag` to
  `SteelEngine Release Publish`, together with the candidate-approved
  ...
  dispatches the public `Windows Node Release` workflow while the SteelEngine
  release is still a draft, carries those pinned source asset digests
  unchanged, verifies the expected SteelEngine Foundation Authenticode signer on
  ...
  `SteelEngineCompanion-*` target asset names, then replaces the expected contract
  assets with the pinned source bytes.
- Website Windows Hub download links should target exact canonical
  `steelengine/steelengine/releases/download/vYYYY.M.PATCH/...` assets for the current
  ...
  is visible from both the GitHub release page and steelengine.ai.
...
- Generate `$steelengine-changelog-update`'s full contribution manifest before
...
- Before publishing or closing a release, run
  `$steelengine-changelog-update`'s `verify-release-notes.mjs` with every stable
...
- To update an existing GitHub Release body, resolve the numeric release id and
  ...
  `gh api repos/steelengine/steelengine/releases/tags/vYYYY.M.PATCH --jq .id`, then
  `gh api -X PATCH repos/steelengine/steelengine/releases/<id> -F body=@/tmp/notes.md`.
  Do not trust `gh release edit --notes-file` or `--input` JSON if verification
  disagrees; verify with `gh api repos/steelengine/steelengine/releases/<id>` because
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `steelengine YYYY.M.PATCH-beta.N`
...
Use the SteelEngine account's existing release-post style:

- Format: `SteelEngine YYYY.M.PATCH 🦞` or `🦞 SteelEngine YYYY.M.PATCH is live`, blank line,
  then 3-4 emoji-led bullets, blank line, one short punchline, then the release
  link.
- For beta: say `SteelEngine YYYY.M.PATCH-beta.N 🦞` or `SteelEngine YYYY.M.PATCH beta N is
...
- Before tagging, diff publishable plugin package manifests against the last
  ...
  (`steelengine.release.publishToNpm: true` or `publishToClawHub: true`) whose
...
- Bootstrap a new ClawHub package only from the trusted workflow source:
  `gh workflow run plugin-clawhub-new.yml --ref main -f plugins=@steelengine/name -f ref=<full-release-sha> -f pretag_validation=true -f dry_run=true`.
...
```bash
  STEELENGINE_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/steelengine-npm-postpublish-verify.ts <published-version>
```
...
- Use `pnpm test:live:media video` for bounded video-provider smoke when video
  ...
  each provider operation with `STEELENGINE_LIVE_VIDEO_GENERATION_TIMEOUT_MS`
  (`180000` by default).
...
- Set `STEELENGINE_LIVE_VIDEO_GENERATION_FULL_MODES=1` only when intentionally
  validating the slower image-to-video and video-to-video transform lanes.

## Check all relevant release builds

- Always validate the SteelEngine npm release path before creating the tag.
...
- Full pre-npm beta test roster:
  ...
  - all Parallels install/update tests:
    ...
    rerun lanes from `steelengine-parallels-smoke`
...
- Post-published beta verification roster:
  - `node --import tsx scripts/steelengine-npm-postpublish-verify.ts <beta-version>`
  ...
  - published npm Telegram proof: dispatch Actions > `NPM Telegram Beta E2E`
    from `main` with `package_spec=steelengine@<beta-version>` and
    ...
    `STEELENGINE_NPM_TELEGRAM_PACKAGE_SPEC` and Convex CI env only as a fallback
    or debugging path.
...
- For beta-style full e2e batteries, hard-cap top-level long lanes instead of letting them run indefinitely. Use host `timeout --foreground`/`gtimeout --foreground` caps such as:
  - `45m` for `STEELENGINE_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`
  ...
  - `180m` for local full QA live OpenAI + Anthropic rosters when explicitly
    ...
    `QA-Lab - All Lanes`
  - Parallels caps from the `steelengine-parallels-smoke` skill
...
- Actual npm install/update phases are capped at 5 minutes. If `npm install -g`, installer package install, or `steelengine update` takes longer than 300s in release e2e, stop treating the run as healthy progress and debug the installer/updater or harness.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `steelengine/steelengine` and the release-ops mac preflight in
  `steelengine/releases` for every release.
...
- SteelEngine publish uses GitHub trusted publishing.
- Stable npm promotion from `beta` to `latest` uses the restricted release-ops
  `steelengine/releases/.github/workflows/steelengine-npm-dist-tags.yml` workflow
...
- Prefer fixing the release-ops workflow token path over any local 1Password
  ...
  repo's `NPM_TOKEN` secret, scoped to the `steelengine` package with read/write
...
- If the release-ops dist-tag workflow cannot promote because `NPM_TOKEN` is
  ...
  - Promote with a fresh OTP:
    `npm dist-tag add steelengine@YYYY.M.PATCH latest --otp "$OTP"`.
  - Verify with a cache-bypassed registry read, for example:
    `npm view steelengine dist-tags --json --prefer-online --cache /tmp/steelengine-npm-cache-verify-$$`
    and `npm view steelengine@latest version dist.tarball --json --prefer-online`.
...
- npm registry metadata is eventually consistent immediately after trusted
  ...
  steps instead of reading the registry again. If the SteelEngine npm child
...
- `.github/workflows/macos-release.yml` in `steelengine/steelengine` is now a
  ...
  artifacts.
- `steelengine/releases/.github/workflows/steelengine-macos-validate.yml` is the
...
- Real mac preflight and real mac publish both use
  `steelengine/releases/.github/workflows/steelengine-macos-publish.yml`.
...
- Focused plugin-only repairs use `plugin_publish_scope=selected` with a nonempty
  ...
  is disabled.
- Dispatch regular beta and stable `SteelEngine Release Publish` runs from trusted
...
- The `npm-release` environment must be approved by `@steelengine/steelengine-release-managers` before publish continues.
- Mac publish uses
  `steelengine/releases/.github/workflows/steelengine-macos-publish.yml` for
...
- Real release-ops mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `steelengine/steelengine`
  automatically when `STEELENGINE_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for the SteelEngine package
...
- `@steelengine/*` plugin publishes use a separate maintainer-only flow.
...
7. Immediately dispatch Actions > `SteelEngine Performance` from the pinned
...
20. For stable releases, start `.github/workflows/macos-release.yml` in
    `steelengine/steelengine` and wait for the public validation-only run to pass.
21. For stable releases, start
    `steelengine/releases/.github/workflows/steelengine-macos-validate.yml` with the
    same tag and wait for the release-ops mac validation lane to pass.
22. For stable releases, start
    `steelengine/releases/.github/workflows/steelengine-macos-publish.yml` with
...
24. Start `.github/workflows/steelengine-release-publish.yml` from the exact pinned
    ...
    `steelengine/steelengine-windows-node` tag as `windows_node_tag` and its
    candidate-approved installer digest map as `windows_node_installer_digests`.
25. Wait for `npm-release` approval from `@steelengine/steelengine-release-managers`.
26. Wait for the real publish workflow to run postpublish verification,
    ...
    waited plugin publish or Windows Hub promotion fails after SteelEngine npm
    succeeds, the workflow keeps the release draft with SteelEngine npm evidence
...
32. For stable releases, start
    `steelengine/releases/.github/workflows/steelengine-macos-publish.yml` for the
...
- Use `steelengine-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
