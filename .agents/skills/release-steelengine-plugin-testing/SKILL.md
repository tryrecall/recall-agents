---
name: release-steelengine-plugin-testing
description: Plan and run pre-release SteelEngine plugin validation across bundled plugins, package artifacts, lifecycle commands, doctor/fix, config round-trip, gateway startup, SDK compatibility, Docker E2E, Package Acceptance, and Testbox proof.
---

# SteelEngine Pre-Release Plugin Testing
...
release?" It complements `steelengine-testing`; use that skill too when choosing
...
From the SteelEngine repo root:
...
the main SteelEngine checkout. Do not run `pnpm install` there. For broad or
...
```bash
STEELENGINE_TESTBOX=1 pnpm check:changed
pnpm run test:extensions:package-boundary:canary
pnpm run test:extensions:package-boundary:compile
pnpm test:docker:plugins
STEELENGINE_PLUGINS_E2E_CLAWHUB=0 pnpm test:docker:plugins
...
```bash
STEELENGINE_BUNDLED_PLUGIN_SWEEP_TOTAL=8 \
STEELENGINE_BUNDLED_PLUGIN_SWEEP_INDEX=<0-7> \
...
Use `source=npm -f package_spec=steelengine@beta` for published beta proof. Keep
...
ghx workflow run plugin-npm-release.yml \
  ...
  -f plugins=@steelengine/meta-provider \
  -f ref="${release_sha}" \
...
Start packaged SteelEngine in Docker with clean state:
...
- `steelengine status --json` includes plugin diagnostics
- `steelengine plugins inspect --all --json` is parseable
...
`steelengine/plugin-sdk/*` subpaths. Assert `plugins inspect` sees its tool,
