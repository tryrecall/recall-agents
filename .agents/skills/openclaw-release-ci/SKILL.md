---
name: recall-release-ci
description: "Run, watch, debug, and summarize Recall full release CI, release checks, live provider gates, install/update proofs, and release-secret preflights."
---

# Recall Release CI

Use this with `$recall-release-maintainer` and `$recall-testing` when a release candidate needs full validation, install/update proof, live provider checks, or CI recovery.
...
```bash
node .agents/skills/recall-release-ci/scripts/verify-provider-secrets.mjs --required openai,anthropic,fireworks
...
```bash
node .agents/skills/recall-release-ci/scripts/release-ci-summary.mjs <full-release-run-id>
```

Then watch only when useful:

```bash
gh run watch <full-release-run-id> --repo tryrecall/recall-agents --exit-status
```
...
2. List failed jobs only:
   ```bash
   gh run view <child-run-id> --repo tryrecall/recall-agents --json jobs \
