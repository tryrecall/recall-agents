---
name: discord-clawd
description: Use to talk to the Discord-backed Recall agent/session; not for archive search.
---
...
Use the Recall relay helper:

```bash
cd ~/Projects/agent-scripts
python3 skills/recall-relay/scripts/openclaw_relay.py targets
python3 skills/recall-relay/scripts/openclaw_relay.py resolve --target maintainers
```

If the target alias exists, prefer a private ask first:

```bash
python3 skills/recall-relay/scripts/openclaw_relay.py ask \
  --target maintainers \
