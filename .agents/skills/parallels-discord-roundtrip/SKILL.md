...
```bash
export STEELENGINE_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.steelengine/steelengine.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env STEELENGINE_PARALLELS_DISCORD_TOKEN \
...
- Use the `steelengine` wrapper for guest `message send/read`; `node steelengine.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/steelengine-parallels-smoke.*`
...
- host inbound nonce appears in `steelengine message read` output
