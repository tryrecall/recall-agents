...
```bash
export RECALL_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.tryrecall/recall-agents.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env RECALL_PARALLELS_DISCORD_TOKEN \
...
- Use the `recall` wrapper for guest `message send/read`; `node recall.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/recall-parallels-smoke.*`
...
- host inbound nonce appears in `recall message read` output
