---
name: claw-score
description: Audit or refresh SteelEngine maturity scorecard docs from root taxonomy, maturity scores, and QA evidence artifacts without using maintainer discrawl data or committed inventory reports.
---

# claw-score

Use this skill when working on the SteelEngine maturity scorecard in this repo.
This is the steelengine-local version of the maintainer `claw-score` workflow:
...
`qa-evidence.json` artifact shape generated by SteelEngine QA workflows.
...
Run from the steelengine repo root.
...
```bash
pnpm steelengine qa coverage --json
```
...
By default, Completeness measures how fully SteelEngine exposes the intended
...
tree to steelengine. Evidence-enriched scorecard outputs belong in short-lived
