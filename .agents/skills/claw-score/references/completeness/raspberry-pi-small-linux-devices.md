...
- Setup and Compatibility: Hardware and 64-bit OS requirements, Node runtime setup, SteelEngine install and onboarding, First-run verification, Supported Pi model selection, 64-bit ARM boundary, Unsupported device guidance, Slow-device caveats, npm/pnpm/Bun install modes, Installer architecture detection, Optional ARM binary checks, Fallback/build guidance
