...
const DEFAULT_CHECKOUT = "/Users/steipete/steelengine";
const DEFAULT_EXPECTED_ORIGIN = "steelengine/steelengine";
...
export function classifyActions(
  ...
  const generatedMacProtocolChanged = changedPaths.some((changedPath) =>
    /^apps\/shared\/SteelEngineKit\/Sources\/SteelEngineProtocol\//u.test(changedPath),
  );
  const runMacos =
  ...
  const macUiVerification =
    ...
    changedPaths.some((changedPath) =>
      /^(?:apps\/macos\/Sources\/|apps\/shared\/SteelEngineKit\/Sources\/|apps\/swabble\/Sources\/)/u.test(
        changedPath,
      ),
    );
...
function defaultLockPath(checkout) {
  ...
  return path.join(tmpdir(), `steelengine-live-updater-${key}.lock`);
}

function defaultStatePath(checkout) {
  return path.join(realpathSync(checkout), ".git", "steelengine-live-updater-state.json");
}
...
function writeMaintenanceState(statePath, state) {
  ...
  const temporary = path.join(directory, `.steelengine-live-updater-${process.pid}-${randomUUID()}`);
...
function resolveManagedGatewayCommand(
  ...
  const defaultEnvDir = path.join(stateDir ?? path.join(home, ".steelengine"), "service-env");
...
function readManagedGatewayLaunchAgent(checkout) {
  ...
  const plistPath = path.join(home, "Library/LaunchAgents/ai.steelengine.gateway.plist");
  let plistStat;
  try {
  ...
  const stateDir =
    typeof environmentVariables?.STEELENGINE_STATE_DIR === "string"
      ? environmentVariables.STEELENGINE_STATE_DIR
      : path.join(home, ".steelengine");
  ...
  const configPath =
    typeof plist?.EnvironmentVariables?.STEELENGINE_CONFIG_PATH === "string"
      ? plist.EnvironmentVariables.STEELENGINE_CONFIG_PATH
      : path.join(gatewayCommand.stateDir, "steelengine.json");
  return {
    configPath,
...
function inspectManagedGatewayDeployment(checkout) {
  ...
  const home = process.env.HOME;
  if (!home || !existsSync(path.join(home, "Library/LaunchAgents/ai.steelengine.gateway.plist"))) {
    return null;
  }
...
function replaceLaunchAgentEntrypoint(deployment, entrypoint) {
  const temporaryPath = `${deployment.plistPath}.steelengine-live-updater-${randomUUID()}`;
...
export function runBuiltGatewayCli(checkout, args, deployment, options = {}) {
  ...
  delete baseEnv.STEELENGINE_GATEWAY_URL;
  delete baseEnv.STEELENGINE_GATEWAY_TOKEN;
  delete baseEnv.STEELENGINE_GATEWAY_PASSWORD;
  delete baseEnv.STEELENGINE_CONFIG_PATH;
  delete baseEnv.STEELENGINE_GATEWAY_PORT;
  ...
  if (wrapperPath && envFilePath) {
    ...
    try {
      const configuredPath = execFileSync(
        executable,
        [...wrapperPrefix, "/usr/bin/printenv", "STEELENGINE_CONFIG_PATH"],
  ...
  const overlayPath = path.join(
    ...
    `.steelengine-live-updater-config-${randomUUID()}.json`,
  );
  writeFileSync(
    overlayPath,
    `${JSON.stringify({
  ...
  const envOverrides = [`STEELENGINE_CONFIG_PATH=${overlayPath}`, `STEELENGINE_GATEWAY_PORT=${port}`];
  const env = Object.assign(baseEnv, {
    STEELENGINE_CONFIG_PATH: overlayPath,
...
export function prepareGatewaySuspension(
  ...
  const requestId = `steelengine-live-updater-${randomUUID()}`;
  let result;
  try {
    result = JSON.parse(
...
function runBuildWithPreservedMacApp(runCommand, checkout, sleep = defaultSleep) {
  const appBundle = path.join(checkout, "dist/SteelEngine.app");
  ...
  const preservedBundle = path.join(
    checkout,
    ".git",
    `.steelengine-live-mac-${process.pid}-${randomUUID()}.app`,
  );
...
function restartGateway(
  ...
  assertExactBuild(checkout, expectedSha);
  if (!deployment) {
    runCommand("pnpm", ["steelengine", "gateway", "restart"], checkout);
    return startedAtMs;
  }
  if (bootstrap) {
...
function verifyGateway(runCommand, checkout, expectedSha, deployment = null) {
  ...
  runCommand(
    "pnpm",
    ["steelengine", "gateway", "status", "--deep", "--require-rpc", "--json"],
    checkout,
  );
  runCommand("pnpm", ["steelengine", "health", "--verbose", "--json"], checkout);
}
...
function isCurrentGatewayLogSource(source, sourceRoot, managedSourceRoots) {
  ...
  while (candidate !== path.dirname(candidate)) {
    ...
    if (existsSync(packagePath) && existsSync(gitPath)) {
      try {
        if (JSON.parse(readFileSync(packagePath, "utf8")).name === "steelengine") {
...
function readFallbackGatewayLogs(sinceMs) {
  ...
  const directories = new Set(["/tmp/steelengine", path.join(tmpdir(), "steelengine")]);
  const contents = [];
  for (const directory of directories) {
    for (const date of dates) {
      const logPath = path.join(directory, `steelengine-${date}.log`);
...
function defaultVerifyMacTarget(checkout) {
  ...
  const executable = path.join(checkout, "dist/SteelEngine.app/Contents/MacOS/SteelEngine");
