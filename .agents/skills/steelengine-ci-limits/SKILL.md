---
name: steelengine-ci-limits
description: Manage SteelEngine GitHub Actions and Blacksmith CI capacity, runner-registration budgets, fanout caps, main-push single-flight, shard sizing, hosted-runner offload, queue health, and safe ramp-down/ramp-up changes. Use when tuning `.github/workflows/*`, `docs/ci.md`, CI runner labels, matrix `max-parallel`, ClawSweeper/Blacksmith burst protection, CodeQL runner placement, or investigating slow/queued SteelEngine CI.
---

# SteelEngine CI Limits

Use this skill for CI capacity changes, not ordinary test failure triage. The
goal is to keep SteelEngine fast while staying below GitHub's self-hosted runner
...
- The scarce resource is Blacksmith runner registrations, not Blacksmith vCPU
  capacity.
- GitHub runner registrations for `steelengine` currently report a 10,000 per
  5-minute bucket in `actions_runner_registration`. Verify the live bucket
  before each tuning pass because GitHub can change it. The `steelengine`
...
ghx run list -R steelengine/steelengine --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
ghx run list -R steelengine/clawsweeper --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
curl -fsS https://clawsweeper.steelengine.ai/api/status | jq '{generated_at,fleet,diagnostics:{errors:.diagnostics.errors}}'
curl -fsS https://clawsweeper.steelengine.ai/api/exact-review-queue | jq '.'
...
- **SteelEngine test runtime:** jobs start quickly but one lane dominates wall time.
  Use `$steelengine-test-performance` instead of runner tuning.
- **Real failing CI:** one job fails after starting. Use `$github:gh-fix-ci` or
  `$steelengine-testing`, not this skill.
- **ClawSweeper backlog:** exact-review queue grows while CI is healthy. Tune
  ClawSweeper workers in `steelengine/clawsweeper`, not SteelEngine CI.
...
ClawSweeper, ClawHub, Clownfish, SteelEngine RTT, and Clawbench.
...
7. Split truly slow tests with `$steelengine-test-performance`; do not hide a slow
...
./node_modules/.bin/oxfmt --check .github/workflows/ci.yml .github/workflows/codeql-critical-quality.yml docs/ci.md test/scripts/ci-workflow-guards.test.ts .agents/skills/steelengine-ci-limits/SKILL.md .agents/skills/steelengine-ci-limits/agents/openai.yaml
git diff --check
```
...
ghx pr checks <pr> -R steelengine/steelengine --watch --interval 15
```
...
STEELENGINE_TESTBOX=1 scripts/pr prepare-run <pr>
```
...
```bash
ghx run list -R steelengine/steelengine --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
for repo in steelengine/clawsweeper steelengine/clawhub steelengine/clownfish steelengine/steelengine-rtt steelengine/clawbench; do
