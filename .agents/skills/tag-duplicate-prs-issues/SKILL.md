...
description: Use gitcrawl to search duplicate SteelEngine PRs/issues, group related work in prtags, and sync duplicate state to GitHub.
---
...
Use `gitcrawl` first because it is the local SteelEngine history and clustering source.
...
```bash
gitcrawl threads steelengine/steelengine --numbers <issue-or-pr-number> --include-closed --json
gitcrawl neighbors steelengine/steelengine --number <issue-or-pr-number> --limit 20 --json
```

Then search key phrases and subsystem terms:

```bash
gitcrawl search steelengine/steelengine --query "<key phrase from title or body>" --mode hybrid --limit 20 --json
gitcrawl search steelengine/steelengine --query "<subsystem or error phrase>" --mode hybrid --limit 20 --json
```

Inspect likely clusters:

```bash
gitcrawl cluster-detail steelengine/steelengine --id <cluster-id> --member-limit 20 --body-chars 280 --json
```
...
```bash
gh search prs --repo steelengine/steelengine --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo steelengine/steelengine --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo steelengine/steelengine --match comments --limit 50 -- "<error or maintainer phrase>"
```
...
```bash
prtags search text -R steelengine/steelengine "<problem phrase>" --types group --limit 10
prtags search similar -R steelengine/steelengine "<problem summary>" --types group --limit 10
prtags group list -R steelengine/steelengine
```
...
```bash
prtags group create -R steelengine/steelengine \
  --kind mixed \
...
```bash
prtags field ensure -R steelengine/steelengine --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R steelengine/steelengine --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R steelengine/steelengine --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R steelengine/steelengine --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R steelengine/steelengine --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R steelengine/steelengine --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R steelengine/steelengine --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R steelengine/steelengine --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R steelengine/steelengine --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R steelengine/steelengine <pr-number> \
...
```bash
prtags annotation issue set -R steelengine/steelengine <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R steelengine/steelengine
```
