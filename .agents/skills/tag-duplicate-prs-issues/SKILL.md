...
description: Use gitcrawl to search duplicate Recall PRs/issues, group related work in prtags, and sync duplicate state to GitHub.
---
...
Use `gitcrawl` first because it is the local Recall history and clustering source.
...
gitcrawl neighbors tryrecall/recall-agents --number <issue-or-pr-number> --limit 20 --json
```

Then search key phrases and subsystem terms:

```bash
gitcrawl search tryrecall/recall-agents --query "<key phrase from title or body>" --mode hybrid --limit 20 --json
gitcrawl search tryrecall/recall-agents --query "<subsystem or error phrase>" --mode hybrid --limit 20 --json
```
...
```bash
gh search prs --repo tryrecall/recall-agents --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo tryrecall/recall-agents --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo tryrecall/recall-agents --match comments --limit 50 -- "<error or maintainer phrase>"
```
...
```bash
prtags search text -R tryrecall/recall-agents "<problem phrase>" --types group --limit 10
prtags search similar -R tryrecall/recall-agents "<problem summary>" --types group --limit 10
prtags group list -R tryrecall/recall-agents
```
...
```bash
prtags group create -R tryrecall/recall-agents \
  --kind mixed \
...
```bash
prtags field ensure -R tryrecall/recall-agents --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R tryrecall/recall-agents --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R tryrecall/recall-agents --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R tryrecall/recall-agents --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R tryrecall/recall-agents --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R tryrecall/recall-agents --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R tryrecall/recall-agents --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R tryrecall/recall-agents --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R tryrecall/recall-agents --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R tryrecall/recall-agents <pr-number> \
...
```bash
prtags annotation issue set -R tryrecall/recall-agents <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R tryrecall/recall-agents
```
