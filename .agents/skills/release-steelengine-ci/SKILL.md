---
name: release-steelengine-ci
description: "Run, watch, debug, and summarize SteelEngine full release CI, release checks, live provider gates, install/update proofs, and release-secret preflights."
---

# SteelEngine Release CI

Use this with `$release-steelengine-maintainer` and `$steelengine-testing` when a release candidate needs full validation, install/update proof, live provider checks, or CI recovery.
...
- Reused Testboxes are provenance-gated after their first successful run.
  ...
  `STEELENGINE_TESTBOX_ALLOW_STALE=1` for release evidence.
...
```bash
node .agents/skills/release-steelengine-ci/scripts/verify-provider-secrets.mjs --required openai,anthropic,fireworks
...
```bash
gh workflow run steelengine-performance.yml \
...
Publish with `steelengine-release-publish.yml` using `release_profile=from-validation`
...
2. List failed jobs only:
   ```bash
   gh run view <child-run-id> --repo steelengine/steelengine --json jobs \
