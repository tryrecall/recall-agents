---
name: crabbox
description: "Crabbox/Testbox remote proof for SteelEngine: trusted-source routing, untrusted isolation, Linux/macOS/Windows/WSL2, live E2E, desktop, diagnostics, cleanup."
---

# Crabbox

Remote SteelEngine proof. Heavy tests. Builds. Typecheck/lint fan-out. Docker.
...
Need direct AWS semantics? Pass `--provider aws`. Need normal trusted SteelEngine
...
node scripts/crabbox-wrapper.mjs run \
  ...
  STEELENGINE_TEST_PROJECTS_PARALLEL=6 \
  STEELENGINE_VITEST_MAX_WORKERS=1 \
  STEELENGINE_TESTBOX=1 STEELENGINE_TESTBOX_REMOTE_RUN=1 \
  pnpm check:changed
```
...
```sh
TMPDIR=/tmp STEELENGINE_TESTBOX=1 "$AUTOREVIEW" \
...
```sh
cd <clean-trusted-steelengine-main>
...
node scripts/crabbox-wrapper.mjs run \
  ...
  --no-hydrate --fresh-pr steelengine/steelengine#123 \
  --timing-json --shell -- \
...
Isolate mutable state: `STEELENGINE_STATE_DIR=$(mktemp -d)`. Test-only local plugin
artifacts may use `STEELENGINE_ALLOW_PLUGIN_INSTALL_OVERRIDES=1`; never call them
...
SteelEngine setup belongs hydration workflow/repo scripts.
