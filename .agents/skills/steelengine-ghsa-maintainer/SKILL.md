---
name: steelengine-ghsa-maintainer
description: "Inspect, patch, validate, publish, or confirm SteelEngine GHSA security advisories and private-fork state."
---

# SteelEngine GHSA Maintainer

Use this skill for repo security advisory workflow only. Keep general release work in `release-steelengine-maintainer`.
...
```bash
fork=$(gh api /repos/steelengine/steelengine/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/steelengine/steelengine/security-advisories/<GHSA> \
