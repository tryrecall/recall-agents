---
name: steelengine-changelog-update
description: Regenerate SteelEngine release changelog sections from git history before beta or stable releases.
---

# SteelEngine Changelog Update
...
Use it with `release-steelengine-maintainer`; this skill owns changelog content,
...
3. Generate the complete contribution record and editorial manifest before
   ...
   node .agents/skills/steelengine-changelog-update/scripts/verify-release-notes.mjs \
     ...
     --manifest /tmp/steelengine-release-<YYYY.M.PATCH>.json \
     --write-ledger
   ```
...
6. Preserve attribution:
   ...
   - never thank bots, `@claude`, `@steelengine`, `@clawsweeper`, or `@steipete`
...
- unqualified `#NNN` references resolve against `steelengine/steelengine`;
  cross-repository references such as `steelengine/imsg#141` remain literal
