...
function printUsage() {
  console.log(`Usage:
  node .agents/skills/steelengine-changelog-update/scripts/verify-release-notes.mjs \\
...
export function defaultGithubSnapshotPath(base, target, gitCommonDir) {
  ...
  return path.resolve(gitCommonDir, "steelengine-release-cache", defaultName);
}
...
function closingReferencesIn(text) {
  const references = [];
  for (const match of text.matchAll(
    /\b(?:fix(?:es|ed)?|closes?|closed|resolves?|resolved)\s+(?:(?:steelengine\/steelengine)?#\d+)(?:\s*(?:,|and)\s*(?:(?:steelengine\/steelengine)?#\d+))*/gi,
  )) {
...
function resolveAssociatedPullRequests(commitHashes, targetTimestamp) {
  ...
  for (let index = 0; index < commitHashes.length; index += commitAssociationQueryBatchSize) {
    ...
    const fields = chunk
      .map(
        (hash, offset) =>
          `c${index + offset}: repository(owner: "steelengine", name: "steelengine") {
  ...
  while (pending.length > 0) {
    ...
    const fields = chunk
      .map(
        (item, offset) =>
          `c${offset}: repository(owner: "steelengine", name: "steelengine") {
  ...
  while (pending.length > 0) {
    ...
    const fields = chunk
      .map((item, offset) => {
        ...
        return `n${offset}: repository(owner: "steelengine", name: "steelengine") {
...
function resolveReferences(numbers) {
  ...
  for (let index = 0; index < numbers.length; index += 40) {
    ...
    const fields = chunk
      .map(
        (number) => `n${number}: repository(owner: "steelengine", name: "steelengine") {
