---
name: steelengine-debugging
description: Debug SteelEngine model, provider, tool-surface, code-mode, streaming, and live/Crabbox behavior by choosing the right logs, probes, and proof path before changing code.
---

# SteelEngine Debugging

Use this skill when SteelEngine behavior differs between local tests, live models,
providers, code mode, Tool Search, Crabbox, or CI, and the next move should be a
debug signal rather than a guess.

## Read First

- `docs/logging.md` for log files, `steelengine logs`, and targeted debug flags.
- `docs/reference/test.md` for local test commands.
- `docs/reference/code-mode.md` for code-mode exec/wait and tool catalog rules.
- Use `$steelengine-testing` for choosing test lanes.
...
STEELENGINE_DEBUG_MODEL_PAYLOAD=full-redacted STEELENGINE_DEBUG_SSE=peek steelengine gateway
```

Useful flags:

- `STEELENGINE_DEBUG_MODEL_TRANSPORT=1`: request start, fetch response, SDK
  headers, first SSE event, stream done, and transport errors at `info`.
- `STEELENGINE_DEBUG_MODEL_PAYLOAD=summary`: bounded payload summary.
- `STEELENGINE_DEBUG_MODEL_PAYLOAD=tools`: all model-facing tool names.
- `STEELENGINE_DEBUG_MODEL_PAYLOAD=full-redacted`: capped, redacted JSON payload.
  Use only while debugging; prompts/message text may still appear.
- `STEELENGINE_DEBUG_SSE=events`: first-event and stream-completion timing.
- `STEELENGINE_DEBUG_SSE=peek`: first five redacted SSE events.
- `STEELENGINE_DEBUG_CODE_MODE=1`: code-mode tool-surface diagnostics.
