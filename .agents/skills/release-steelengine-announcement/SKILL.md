---
name: release-steelengine-announcement
description: "Draft or post SteelEngine beta/stable Discord release announcements from changelog, GitHub release, registry, and validation evidence. Use when announcing a beta, stable release, release candidate, or asking what users should test after an SteelEngine release."
---

# SteelEngine Release Announcement

Use with `release-steelengine-maintainer` after a beta or stable release is live.
...
- exact version, e.g. `SteelEngine 2026.5.25-beta.1`
...
- update command promoted near the top:
  ```sh
  steelengine update --channel beta --yes
  steelengine --version
  ```
- fresh install path:
  `Install from https://steelengine.ai`
- GitHub release link
...
```sh
steelengine update --channel stable --yes
steelengine --version
```

Fresh installs still point to `https://steelengine.ai`.
...
sending it. Never use SteelEngine channel sends, bots, webhooks, relays, or tokens.
