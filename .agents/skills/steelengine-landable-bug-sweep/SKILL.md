---
name: steelengine-landable-bug-sweep
description: "Find or repair a requested batch of small high-confidence non-SDK-boundary SteelEngine bugfix PRs until they are landable."
---

# SteelEngine Landable Bug Sweep

Autonomous maintainer workflow for producing a requested batch of landable SteelEngine bugfix PR URLs.
...
Use `$gitcrawl` for discovery/clustering, `$steelengine-pr-maintainer` for live GitHub mutation rules, `$github-author-context` when contributor trust matters, `$steelengine-testing` for proof choice, `$autoreview` before publishing/landing, and `$crabbox` for broad/E2E/live proof.
