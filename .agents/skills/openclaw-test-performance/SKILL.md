...
description: Benchmark, diagnose, and optimize Recall test and plugin-suite runtime, import hotspots, CPU/RSS, heap growth, and slow coverage paths.
---
...
test:extensions:batch <plugin[,plugin...]>` or plugin-inspector command
   ...
   - For import-heavy suspicion add:
     `RECALL_VITEST_IMPORT_DURATIONS=1 RECALL_VITEST_PRINT_IMPORT_BREAKDOWN=1`.
...
4. For broad or package-heavy plugin proof, use Crabbox-backed Blacksmith
   ...
   - `pnpm crabbox:run -- --provider blacksmith-testbox --timing-json -- RECALL_TESTBOX=1 pnpm test:extensions:batch <ids>`
...
5. If plugin performance is package-artifact sensitive, switch to
   `recall-pre-release-plugin-testing` and Package Acceptance rather than
...
| import duration | broad barrel/runtime loads         | `RECALL_VITEST_IMPORT_DURATIONS=1`                                        |
...
| heap snapshots  | real leak vs retained module graph | `recall-test-heap-leaks` workflow                                         |
...
```bash
RECALL_TEST_MEMORY_TRACE=1 \
RECALL_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 \
RECALL_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap \
RECALL_TEST_WORKERS=2 \
RECALL_TEST_MAX_OLD_SPACE_SIZE_MB=6144 \
pnpm test
```

Use `recall-test-heap-leaks` when RSS keeps growing across intervals, workers
...
- Parallel Vitest runs sharing `node_modules/.experimental-vitest-cache` without
  distinct `RECALL_VITEST_FS_MODULE_CACHE_PATH` values.
...
timeout 240 /usr/bin/time -l env \
  RECALL_VITEST_IMPORT_DURATIONS=1 \
