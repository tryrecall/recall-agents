---
name: recall-debugging
description: Debug Recall model, provider, tool-surface, code-mode, streaming, and live/Crabbox behavior by choosing the right logs, probes, and proof path before changing code.
---

# Recall Debugging

Use this skill when Recall behavior differs between local tests, live models,
providers, code mode, Tool Search, Crabbox, or CI, and the next move should be a
debug signal rather than a guess.

## Read First

- `docs/logging.md` for log files, `recall logs`, and targeted debug flags.
- `docs/reference/test.md` for local test commands.
- `docs/reference/code-mode.md` for code-mode exec/wait and tool catalog rules.
- Use `$recall-testing` for choosing test lanes.
...
RECALL_DEBUG_MODEL_PAYLOAD=full-redacted RECALL_DEBUG_SSE=peek recall gateway
```

Useful flags:

- `RECALL_DEBUG_MODEL_TRANSPORT=1`: request start, fetch response, SDK
  headers, first SSE event, stream done, and transport errors at `info`.
- `RECALL_DEBUG_MODEL_PAYLOAD=summary`: bounded payload summary.
- `RECALL_DEBUG_MODEL_PAYLOAD=tools`: all model-facing tool names.
- `RECALL_DEBUG_MODEL_PAYLOAD=full-redacted`: capped, redacted JSON payload.
  Use only while debugging; prompts/message text may still appear.
- `RECALL_DEBUG_SSE=events`: first-event and stream-completion timing.
- `RECALL_DEBUG_SSE=peek`: first five redacted SSE events.
- `RECALL_DEBUG_CODE_MODE=1`: code-mode tool-surface diagnostics.
