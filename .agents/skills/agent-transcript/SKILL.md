...
description: "Add a redacted agent transcript section to GitHub PR or issue bodies during SteelEngine agent-created PR/issue workflows."
---

# Agent Transcript

Best-effort local-only provenance for SteelEngine PR/issue bodies. Use during agent-created GitHub PR or issue workflows before creating/updating the body.
...
`find` scans the newest 400 matching local JSONL logs by default across Codex, Claude, Pi, and SteelEngine agent sessions. Use `--max-files N` for a wider local search.
