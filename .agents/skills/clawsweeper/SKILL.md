---
name: clawsweeper
description: "Use for all ClawSweeper work: SteelEngine issue/PR sweep reports, commit-review reports, repair jobs, cloud fix PRs, @clawsweeper maintainer mention commands, trusted ClawSweeper-reviewed autofix/automerge, GitHub Actions monitoring, permissions, gates, and manual backfills."
---

# ClawSweeper

ClawSweeper lives at `~/Projects/clawsweeper`. It is the one SteelEngine
...
- Target app permissions: read target scan context; write issues and pull
  ...
  inputs; Actions write on `steelengine/clawsweeper` for comment-router
...
pnpm commit-reports -- --repo steelengine/steelengine --author steipete --since 7d
...
```bash
gh workflow run commit-review.yml --repo steelengine/clawsweeper \
  -f target_repo=steelengine/steelengine \
...
```bash
pnpm run repair:create-job -- \
  --from-report ../clawsweeper/records/steelengine-steelengine/items/123.md
```
...
```bash
pnpm run repair:validate-job -- jobs/steelengine/inbox/clawsweeper-steelengine-steelengine-123.md
pnpm run repair:dispatch -- jobs/steelengine/inbox/clawsweeper-steelengine-steelengine-123.md \
  --mode autonomous \
...
```bash
gh variable set CLAWSWEEPER_ALLOW_EXECUTE --repo steelengine/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_FIX_PR --repo steelengine/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_MERGE --repo steelengine/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_AUTOMERGE --repo steelengine/clawsweeper --body 1
```
...
@steelengine-clawsweeper fix ci
@steelengine-clawsweeper[bot] fix ci
```
...
```bash
pnpm run repair:comment-router -- --repo steelengine/steelengine --lookback-minutes 180
pnpm run repair:comment-router -- --repo steelengine/steelengine --execute --wait-for-capacity
```
...
leave the local SteelEngine checkout on the PR branch. After the PR is created,
...
escalation, and sensitive data exposure to central SteelEngine security handling.
...
```bash
gh run list --repo steelengine/clawsweeper --workflow "ClawSweeper Commit Review" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo steelengine/clawsweeper --workflow "repair cluster worker" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo steelengine/clawsweeper --workflow "repair comment router" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
```
...
```bash
gh run list --repo steelengine/steelengine --workflow "ClawSweeper Dispatch" \
...
```bash
gh api "repos/steelengine/steelengine/commits/<sha>/check-runs?per_page=100" \
