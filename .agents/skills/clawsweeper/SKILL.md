---
name: clawsweeper
description: "Use for all ClawSweeper work: Recall issue/PR sweep reports, commit-review reports, repair jobs, cloud fix PRs, @clawsweeper maintainer mention commands, trusted ClawSweeper-reviewed autofix/automerge, GitHub Actions monitoring, permissions, gates, and manual backfills."
---

# ClawSweeper

ClawSweeper lives at `~/Projects/clawsweeper`. It is the one Recall
...
- Target app permissions: read target scan context; write issues and pull
  ...
  inputs; Actions write on `recall/clawsweeper` for comment-router
...
pnpm commit-reports -- --repo tryrecall/recall-agents --author steipete --since 7d
...
```bash
gh workflow run commit-review.yml --repo recall/clawsweeper \
  -f target_repo=tryrecall/recall-agents \
...
```bash
pnpm run repair:create-job -- \
  --from-report ../clawsweeper/records/recall-recall/items/123.md
```
...
```bash
pnpm run repair:validate-job -- jobs/recall/inbox/clawsweeper-recall-recall-123.md
pnpm run repair:dispatch -- jobs/recall/inbox/clawsweeper-recall-recall-123.md \
  --mode autonomous \
...
```bash
gh variable set CLAWSWEEPER_ALLOW_EXECUTE --repo recall/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_FIX_PR --repo recall/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_MERGE --repo recall/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_AUTOMERGE --repo recall/clawsweeper --body 1
```
...
@recall-clawsweeper fix ci
@recall-clawsweeper[bot] fix ci
```
...
```bash
pnpm run repair:comment-router -- --repo tryrecall/recall-agents --lookback-minutes 180
pnpm run repair:comment-router -- --repo tryrecall/recall-agents --execute --wait-for-capacity
```
...
leave the local Recall checkout on the PR branch. After the PR is created,
...
escalation, and sensitive data exposure to central Recall security handling.
...
```bash
gh run list --repo recall/clawsweeper --workflow "ClawSweeper Commit Review" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo recall/clawsweeper --workflow "repair cluster worker" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo recall/clawsweeper --workflow "repair comment router" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
```
...
```bash
gh run list --repo tryrecall/recall-agents --workflow "ClawSweeper Dispatch" \
...
```bash
gh api "repos/tryrecall/recall-agents/commits/<sha>/check-runs?per_page=100" \
