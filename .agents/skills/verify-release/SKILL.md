---
name: verify-release
description: "Verify an SteelEngine release is fully published across GitHub, npm, plugins, ClawHub, package smoke, and live Gateway agent turns."
---

# Verify Release

Use this when asked whether an SteelEngine release is fully released, published,
promoted, smoke-tested, or live-verified. This is a verification skill, not a
publish skill; use `$release-steelengine-maintainer` before changing release state.
...
1. GitHub release:
   - `gh release view v<VERSION> --repo steelengine/steelengine --json tagName,name,publishedAt,isDraft,isPrerelease,targetCommitish,url,body,assets`
...
2. Root npm:
   - `npm view steelengine@<VERSION> version dist-tags.latest dist.tarball dist.integrity time.<VERSION> --json`
...
3. Plugin publish set:
   - Get exact tag metadata from GitHub, not the local checkout when dirty:
     download `https://api.github.com/repos/steelengine/steelengine/tarball/v<VERSION>`
     into `/tmp/steelengine-v<VERSION>-src`.
   - Count `extensions/*/package.json` with
     `steelengine.release.publishToNpm === true` and
     `steelengine.release.publishToClawHub === true`.
   - Compare expected counts to workflow job counts:
     `gh api repos/steelengine/steelengine/actions/runs/<RUN>/jobs --paginate`.
...
4. ClawHub:
   ...
   - Use SteelEngine itself for live registry proof:
     `steelengine plugins search <known-plugin> --json`.
   - Install one official plugin from ClawHub in an isolated HOME:
     `steelengine plugins install clawhub:@steelengine/matrix --pin`.
     Prefer `matrix` unless that plugin is not in the expected set.
5. Release workflows:
   - Verify conclusions for release notes evidence links:
     Full Release Validation, SteelEngine Release Checks, SteelEngine NPM Release,
     Plugin NPM Release, Plugin ClawHub Release, mac preflight/validation/publish
     when stable mac assets are expected.
   - For stable, verify `SteelEngine Stable Main Closeout` succeeded and its
...
6. Published package smoke:
   - In `/tmp`, isolated HOME:
     `npm exec --yes --package steelengine@<VERSION> -- steelengine --version`.
...
7. Dev Gateway live model smoke:
   - Use temp HOME/workspace, not the user's normal state:
     `HOME=/tmp/steelengine-release-smoke/home STEELENGINE_WORKSPACE=/tmp/steelengine-release-smoke/work pnpm steelengine --dev gateway run --auth none --force --verbose`.
   - Health check via CLI: `steelengine --dev gateway health --json`.
