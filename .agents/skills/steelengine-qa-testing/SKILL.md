---
name: steelengine-qa-testing
description: Run, watch, debug, extend, or explain SteelEngine qa-lab and qa-channel scenarios, artifacts, and live lanes.
---

# SteelEngine QA Testing
...
5. If the user wants to watch the live UI, find the current `steelengine-qa` listen port and report `http://127.0.0.1:<port>`.
...
`pnpm steelengine qa matrix` defaults to the full `all` profile. Use explicit
profiles for faster CI/release proof:

```bash
pnpm steelengine qa matrix --profile release
```
...
- That item is the first place to look for:
  ...
  - `STEELENGINE_QA_PROVIDER_MODE`
  - `STEELENGINE_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `SteelEngine`
  - item: `STEELENGINE_QA_CONVEX_SITE_URL`
  - item: `STEELENGINE_QA_CONVEX_SECRET_MAINTAINER`
  - item: `STEELENGINE_QA_CONVEX_SECRET_CI`
- Additional related notes/login items seen during QA credential work:
  - vault: `Private`
  - items: `STEELENGINE QA`, `Convex`, `Telegram`
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `STEELENGINE_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `STEELENGINE_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `SteelEngine/STEELENGINE_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
STEELENGINE_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
STEELENGINE_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
STEELENGINE_QA_PROVIDER_MODE="mock-openai" \
STEELENGINE_NPM_TELEGRAM_PACKAGE_SPEC="steelengine@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `STEELENGINE_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `STEELENGINE_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `STEELENGINE_QA_PROVIDER_MODE`, map it explicitly to `STEELENGINE_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
  ...
  - a real run of `pnpm test:docker:npm-telegram-live` passed with:
    ...
    - `STEELENGINE_NPM_TELEGRAM_PROVIDER_MODE=mock-openai`
...
```bash
gh workflow run "NPM Telegram Beta E2E" --repo steelengine/steelengine --ref main \
  -f package_spec=steelengine@YYYY.M.D-beta.N \
  -f package_label=steelengine@YYYY.M.D-beta.N \
...
```bash
gh api repos/steelengine/steelengine/actions/runs/<run-id>/artifacts
```
...
- Use two dedicated WhatsApp-capable test numbers: one driver account and one SUT account. Do not use personal numbers or personal SteelEngine WhatsApp accounts in the shared pool.
...
- Validate with `pnpm steelengine qa whatsapp --credential-source convex --credential-role maintainer --provider-mode mock-openai` and preserve artifact paths plus redacted pass/fail summaries.
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal SteelEngine identity combines with the character.
...
```bash
pnpm steelengine qa manual \
  --model codex-cli/<codex-model> \
  --message "Reply exactly: CODEX_OK"
```

- Treat the concrete Codex model name as user/config input; do not hardcode it in source, docs examples, or scenarios.
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `STEELENGINE_HOME` sandboxed.
