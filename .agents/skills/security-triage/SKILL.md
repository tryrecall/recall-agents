...
description: Triage Recall security advisories, drafts, and GHSA reports with shipped-tag and trust-model proof.
---
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/tryrecall/recall-agents/security-advisories/<GHSA>`.
...
gh api /repos/tryrecall/recall-agents/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view recall version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo tryrecall/recall-agents --match title,body,comments -- "<terms>"
gh search prs --repo tryrecall/recall-agents --match title,body,comments -- "<terms>"
```
