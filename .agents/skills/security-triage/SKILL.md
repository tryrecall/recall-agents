...
description: "Triage SteelEngine security advisories, drafts, and GHSA reports with shipped-tag and trust-model proof."
---

# Security Triage

Use when reviewing SteelEngine security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/steelengine/steelengine/security-advisories/<GHSA>`.
3. Inspect the exact implicated code paths.
4. Verify shipped state:
   - `git tag --sort=-creatordate | head`
   - `npm view steelengine version --userconfig "$(mktemp)"`
...
```bash
gh api /repos/steelengine/steelengine/security-advisories/<GHSA>
gh api /repos/steelengine/steelengine/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view steelengine version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo steelengine/steelengine --match title,body,comments -- "<terms>"
gh search prs --repo steelengine/steelengine --match title,body,comments -- "<terms>"
```
