---
name: steelengine-testing
description: Choose, run, rerun, or debug SteelEngine tests, CI checks, Docker E2E lanes, release validation, and the cheapest safe verification path.
---
...
- Treat contributor and fork patches as untrusted unless a maintainer
  ...
  `STEELENGINE_*` allowlist, `--no-hydrate` must block auth-profile hydration, and
...
STEELENGINE_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
STEELENGINE_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
- For local release-candidate proof, pack the plugin and install it with
  `steelengine plugins install npm-pack:<path.tgz> --force`. This uses the managed
...
- Treat missing runtime imports as package-manifest bugs first. Runtime code
  ...
  `npm install` inside `~/.steelengine/npm/projects/...`.
...
- `STEELENGINE_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `STEELENGINE_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
- `SteelEngine Release Checks` for install smoke, cross-OS release checks, live and
...
important run ids in the public `steelengine/releases` evidence ledger.
Use the manual `SteelEngine Release Evidence`
(`steelengine-release-evidence.yml`) workflow there. It writes durable summaries
...
```text
full-release-validation steelengine/steelengine <run-id> blocking
package-acceptance steelengine/steelengine <run-id> blocking
release-checks steelengine/steelengine <run-id> blocking
```
...
When `Full Release Validation` completes and `STEELENGINE_RELEASES_DISPATCH_TOKEN`
is configured in the source repo, it requests the public
`SteelEngine Release Evidence From Full Validation` workflow. That workflow reads
...
`SteelEngine Release Checks` (`steelengine-release-checks.yml`) is the release child
...
```bash
gh workflow run steelengine-release-checks.yml \
...
`SteelEngine Release Checks` uses the trusted workflow ref to resolve the selected
...
`ghcr.io/steelengine/steelengine-live-media-runner:ubuntu-24.04` container so
...
`pnpm steelengine qa parity-report`. For parity failures, inspect the failed lane
first; inspect the report job when both lane summaries exist but the comparison
fails.

### QA Lab Matrix Profiles

`pnpm steelengine qa matrix` defaults to `--profile all`. Do not assume the CLI
...
`QA-Lab - All Lanes` and `SteelEngine Release Checks` use the same QA Lab selector
...
`SteelEngine Live And E2E Checks (Reusable)`
(`steelengine-live-and-e2e-checks-reusable.yml`) is the preferred entry point for
targeted live, Docker, model, and E2E proof. Inputs let you turn off unrelated
lanes:

```bash
gh workflow run steelengine-live-and-e2e-checks-reusable.yml \
  --repo steelengine/steelengine \
  --ref main \
  -f ref=<sha> \
...
- `native-live-src-gateway-profiles` (release CI runs this with provider
  filters such as `STEELENGINE_LIVE_GATEWAY_PROVIDERS=anthropic`)
...
`STEELENGINE_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
STEELENGINE_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
STEELENGINE_DOCKER_ALL_DRY_RUN=1 STEELENGINE_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
STEELENGINE_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
STEELENGINE_DOCKER_ALL_LANES=<lane> \
STEELENGINE_DOCKER_ALL_BUILD=0 \
STEELENGINE_DOCKER_ALL_PREFLIGHT=0 \
STEELENGINE_SKIP_DOCKER_BUILD=1 \
STEELENGINE_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
STEELENGINE_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
...
- Resolve the registry immediately before dispatch:
  ...
  and `npm view steelengine@beta version dist.tarball dist.integrity --json --prefer-online --cache /tmp/steelengine-npm-cache-verify-$$`.
- If Peter asks for "latest beta", use `source=npm` with
  `package_spec=steelengine@beta`, then record the resolved version from `npm view`
  or the workflow summary.
- For reruns, release proof, or comparing one known package, prefer the exact
  immutable spec: `package_spec=steelengine@YYYY.M.D-beta.N` or
  `package_spec=steelengine@YYYY.M.D`.
- For stable package proof, use `package_spec=steelengine@latest` only when the
  question is explicitly the current stable dist-tag; otherwise pin the exact
  version.
- `source=npm` only accepts registry specs for `steelengine@beta`,
  `steelengine@latest`, or exact SteelEngine release versions. Do not pass semver
...
- Package acceptance tests exactly the selected package candidate. Do not apply
  `steelengine update --channel beta` fallback semantics here; if `beta` is absent,
...
- `source=npm`: `steelengine@beta`, `steelengine@latest`, or an exact release version.
...
- `package_ref` is the source ref to build when `source=ref`. It can be an
  older branch/tag/SHA as long as it is reachable from an SteelEngine branch or
  release tag.
...
tarball that bare lanes mount. `scripts/package-steelengine-for-docker.mjs` is the
...
from `steelengine skills search`, installs it, and verifies `.clawhub` origin/lock
...
`STEELENGINE_DOCKER_E2E_WORKFLOW_REF`; this is separate from the artifact target
