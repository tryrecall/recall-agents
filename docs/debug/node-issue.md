...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Recall
...
Running Recall via Node with `tsx` fails at startup with:

```
[recall] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `recall status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch recall.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node recall.mjs status` works on Node 25.
