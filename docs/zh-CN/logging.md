...
你可以在 `~/.recall/recall.json` 中覆盖此设置：
...
所有日志配置都在 `~/.recall/recall.json` 的 `logging` 下。
...
- Recall 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `recall plugins enable diagnostics-otel` 启用插件。
...
- `recall.tokens`（计数器，属性：`recall.token`、`recall.channel`、`recall.provider`、`recall.model`）
- `recall.cost.usd`（计数器，属性：`recall.channel`、`recall.provider`、`recall.model`）
- `recall.run.duration_ms`（直方图，属性：`recall.channel`、`recall.provider`、`recall.model`）
- `recall.context.tokens`（直方图，属性：`recall.context`、`recall.channel`、`recall.provider`、`recall.model`）

消息流：

- `recall.webhook.received`（计数器，属性：`recall.channel`、`recall.webhook`）
- `recall.webhook.error`（计数器，属性：`recall.channel`、`recall.webhook`）
- `recall.webhook.duration_ms`（直方图，属性：`recall.channel`、`recall.webhook`）
- `recall.message.queued`（计数器，属性：`recall.channel`、`recall.source`）
- `recall.message.processed`（计数器，属性：`recall.channel`、`recall.outcome`）
- `recall.message.duration_ms`（直方图，属性：`recall.channel`、`recall.outcome`）

队列 + 会话：

- `recall.queue.lane.enqueue`（计数器，属性：`recall.lane`）
- `recall.queue.lane.dequeue`（计数器，属性：`recall.lane`）
- `recall.queue.depth`（直方图，属性：`recall.lane` 或 `recall.channel=heartbeat`）
- `recall.queue.wait_ms`（直方图，属性：`recall.lane`）
- `recall.session.state`（计数器，属性：`recall.state`、`recall.reason`）
- `recall.session.stuck`（计数器，属性：`recall.state`）
- `recall.session.stuck_age_ms`（直方图，属性：`recall.state`）
- `recall.run.attempt`（计数器，属性：`recall.attempt`）
...
- `recall.webhook.error`
  - `recall.channel`、`recall.webhook`、`recall.chatId`、`recall.error`
- `recall.message.processed`
  - `recall.channel`、`recall.outcome`、`recall.chatId`、`recall.messageId`、`recall.sessionKey`、`recall.sessionId`、`recall.reason`
- `recall.session.stuck`
  - `recall.state`、`recall.ageMs`、`recall.queueDepth`、`recall.sessionKey`、`recall.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `recall doctor`。
