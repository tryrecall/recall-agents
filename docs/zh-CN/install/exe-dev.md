...
summary: 在 exe.dev 上运行 Recall Gateway 网关（VM + HTTPS 代理）以实现远程访问
title: exe.dev
x-i18n:
...
目标：让 Recall Gateway 网关运行在 exe.dev VM 上，并且可通过你的笔记本电脑访问：`https://<vm-name>.exe.xyz`
...
Shelley 是 [exe.dev](https://exe.dev) 的智能体，可以使用我们的提示词立即安装 Recall。
使用的提示词如下：
...
提示：请让这个 VM 保持**有状态**。Recall 会将状态存储在 `~/.recall/` 和 `~/.recall/workspace/` 下。
...
## 4）设置 nginx，将 Recall 代理到端口 8000
...
`gateway.auth.token` 中的令牌（可通过 `recall config get gateway.auth.token` 获取，或使用
`recall doctor --generate-gateway-token` 生成）。使用 `recall devices list` 和
`recall devices approve <requestId>` 批准设备。如果拿不准，请在浏览器中使用 Shelley！
