...
通过一键模板在 Railway 上部署 Recall，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/recall` — 控制面板 UI
...
- `RECALL_STATE_DIR=/data/.recall`（推荐）
- `RECALL_WORKSPACE_DIR=/data/workspace`（推荐）
- `RECALL_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 Recall 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
