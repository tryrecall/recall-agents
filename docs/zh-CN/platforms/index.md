...
Recall 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 服务状态：`recall gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均受支持）：

- 向导（推荐）：`recall onboard --install-daemon`
- 直接安装：`recall gateway install`
- 配置流程：`recall configure` → 选择 **Gateway 服务**
- 修复/迁移：`recall doctor`（会提供安装或修复服务的选项）

服务目标取决于操作系统：

- macOS：LaunchAgent（`ai.tryrecall.gateway` 或 `ai.tryrecall.<profile>`；旧版为 `com.recall.*`）
- Linux/WSL2：systemd 用户服务（`recall-gateway[-<profile>].service`）
