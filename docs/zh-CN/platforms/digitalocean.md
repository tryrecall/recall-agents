---
read_when:
  - 在 DigitalOcean 上设置 Recall
  - 寻找适合 Recall 的低价 VPS 托管
summary: 在 DigitalOcean 上运行 Recall（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 Recall

## 目标

在 DigitalOcean 上以 **每月 6 美元**（或预留定价时每月 4 美元）运行一个持久化的 Recall Gateway 网关。
...
- `~/.recall/` — 配置、凭证、会话数据
- `~/.recall/workspace/` — 工作区（`SOUL.md`、memory 等）
