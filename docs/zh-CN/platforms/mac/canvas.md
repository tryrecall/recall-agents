...
- `~/Library/Application Support/Recall/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `recall-canvas://<session>/<path>`

示例：

- `recall-canvas://main/` → `<canvasRoot>/main/index.html`
- `recall-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `recall-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
recall nodes canvas present --node <id>
recall nodes canvas navigate --node <id> --url "/"
recall nodes canvas eval --node <id> --js "document.title"
recall nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__recall__/a2ui/
```
...
recall nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
recall nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "recall://agent?message=Review%20this%20design";
```
