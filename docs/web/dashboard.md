...
- If clipboard and browser delivery fail, `recall dashboard` still prints the
  clean URL and tells you to use the token from `RECALL_GATEWAY_TOKEN` or
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `RECALL_GATEWAY_TOKEN`); `recall dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `recall dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `recall dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `RECALL_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `recall config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `RECALL_GATEWAY_TOKEN` in this shell, then rerun `recall dashboard`
  - No shared secret configured: `recall doctor --generate-gateway-token`
