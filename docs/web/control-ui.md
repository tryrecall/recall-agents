...
created. Re-run `recall devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `recall devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.recall/recall.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Recall
...
Recall **blocks** Control UI connections without device identity.
