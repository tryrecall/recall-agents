...
If the page fails to load, start the Gateway first: `steelengine gateway`.

<Note>
On native Windows LAN binds, Windows Firewall or organization-managed Group Policy can still block the advertised LAN URL even when `127.0.0.1` works on the Gateway host. Run `steelengine gateway status --deep` on the Windows host; it reports likely-blocked ports, profile mismatches, and local firewall rules that policy may ignore.
</Note>
...
If the browser retries pairing with changed auth details (role/scopes/public key), the previous pending request is superseded and a new `requestId` is created; re-run `steelengine devices list` before approving.

Switching an already-paired browser from read access to write/admin access is treated as an approval upgrade, not a silent reconnect: SteelEngine keeps the old approval active, blocks the broader reconnect, and asks you to approve the new scope set explicitly.

Once approved, the device is remembered and won't require re-approval unless you revoke it with `steelengine devices revoke --device <id> --role <role>`. See [Devices CLI](/cli/devices) for token rotation, revocation, and the Paperclip / `steelengine_gateway` first-run approval flow.
...
<Steps>
  ...
  <Step title="Connect the phone">
    In the SteelEngine mobile app, open **Settings** → **Gateway** and scan the QR code. You can copy and paste the setup code instead.
  </Step>
...
The Control UI fetches its runtime settings from `/control-ui-config.json`, resolved relative to the gateway's Control UI base path (for example `/__steelengine__/control-ui-config.json` under base path `/__steelengine__/`). That endpoint is gated by the same gateway auth as the rest of the HTTP surface: unauthenticated browsers cannot fetch it, and a successful fetch requires a valid gateway token/password, Tailscale Serve identity, or a trusted-proxy identity.
...
the Control UI. For example, a base path of `/steelengine` uses
`/steelengine/settings/plugins`. The page is always available, even when every
...
included with SteelEngine, official external plugins, and one-click MCP connectors
...
SteelEngine but disabled by default, so its action is **Enable**. Bundled plugins
...
`steelengine mcp login <name>` from the CLI after they are added.

The page intentionally focuses on inventory, discovery, install, enablement,
and removal. Use [`steelengine plugins`](/cli/plugins) for arbitrary npm, git, or
...
SteelEngine companion surface: the [iOS](/platforms/ios) and
...
<AccordionGroup>
  ...
  <Accordion title="Config">
    - View/edit `~/.steelengine/steelengine.json` (`config.get`, `config.set`).
  ...
  <Accordion title="Automations panel notes">
    ...
    - `cron.webhook` is a deprecated legacy fallback: run `steelengine doctor --fix` to migrate stored jobs that still use `notify: true` to explicit per-job webhook or completion delivery.
...
into an SteelEngine agent. The Gateway discovers supported local memory on its own
...
SteelEngine backup when state exists, writes a redacted migration report, and keeps
...
The dedicated MCP page is an operator view for SteelEngine-managed MCP servers under `mcp.servers`. It does not start MCP transports by itself; use it to inspect and edit saved config, then use `steelengine mcp doctor --probe` when you need live server proof.
...
7. Run `steelengine mcp status --verbose`, `steelengine mcp doctor --probe`, or `steelengine mcp reload` from a terminal for static diagnostics, live proof, or cached-runtime disposal.
...
<Warning>
The terminal is an unconfined host shell and inherits the Gateway process environment. Enable it only for trusted operator deployments. SteelEngine refuses terminal sessions for agents with `sandbox.mode: "all"`; changing an active agent to that mode closes its existing and in-flight terminal sessions.
</Warning>
...
Codex and Claude Code sessions discovered in the sessions sidebar can open in their native CLI inside the same terminal panel. In **Settings › Chat**, set **Open Codex/Claude sessions in** to **Terminal** to make a normal row click open `codex resume` or `claude --resume`; the default remains the read-only SteelEngine viewer. A row's right-click or kebab menu always offers both choices, and the viewer header includes **Open in terminal** when that session is eligible.
...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - In the macOS app, the SteelEngine mark uses the otherwise-empty native titlebar strip next to the window controls instead of consuming a sidebar row.
  ...
  <Accordion title="Stop and abort">
    ...
    - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop steelengine`, `please stop`) to abort out-of-band.
...
token), first opens and reloads show a small animated SteelEngine mark while the connection is
...
If the page shows **Protocol mismatch** right after an SteelEngine update, first reopen the dashboard with `steelengine dashboard` and hard-refresh. If it still fails, clear site data for the dashboard origin or test in a private browser window; an old tab or browser service-worker cache can keep running a pre-update Control UI bundle against the newer Gateway.
...
| `state/steelengine.sqlite` → `web_push_vapid_keys`    | Auto-generated VAPID keypair used to sign Web Push payloads.                 |
| `state/steelengine.sqlite` → `web_push_subscriptions` | Persisted browser subscription endpoints, keys, and registration timestamps. |

Upgrades from the retired `push/vapid-keys.json` and `push/web-push-subscriptions.json` stores are imported by `steelengine doctor --fix`. Stop the Gateway before running that repair so an older process cannot recreate retired state during import. Run the repair before using Web Push after an upgrade; registration, delivery, deletion, and key resolution refuse to proceed while either retired source or an interrupted Doctor claim remains. The Gateway runtime reads and writes SQLite only.
...
- `STEELENGINE_VAPID_SUBJECT` (defaults to `https://steelengine.ai`)
...
<Tabs>
  ...
  <Tab title="Bind to tailnet + token">
    ```bash
    steelengine gateway --bind tailnet --token "$(openssl rand -hex 32)"
    ```
...
If you open the dashboard over plain HTTP (`http://<lan-ip>` or `http://<tailscale-ip>`), the browser runs in a **non-secure context** and blocks WebCrypto. By default, SteelEngine **blocks** Control UI connections without device identity.
...
- `GET /__steelengine__/assistant-media?meta=1&source=<path>` requires the normal Control UI operator auth; the browser sends the gateway token as a bearer header when checking availability.
...
Operator approval notifications can deep-link to a standalone approval document served under the reserved `${controlUiBasePath}/approve/{approvalId}` namespace (for example `/approve/<approvalId>`, or `/steelengine/approve/<approvalId>` with a configured base path). The URL is stable for the lifetime of the approval and safe to forward between your own devices: it identifies the approval, never authorizes it.
...
```bash
STEELENGINE_CONTROL_UI_BASE_PATH=/steelengine/ pnpm ui:build
```
...
If the browser loads a blank dashboard and DevTools shows no useful error, an extension or early content script may have prevented the JavaScript module app from evaluating. The static page includes a plain HTML recovery panel that appears when `<steelengine-app>` is not registered after startup.
