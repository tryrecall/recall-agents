...
read_when:
  ...
  - You need to map a third-party CLI into Recall's text fallback runner
---

CLI backend plugins let Recall call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points Recall at the plugin runtime module              |
| Manifest ownership   | `recall.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ...
    entry is `./src/index.ts`, add `recall.runtimeExtensions` that points at
  ...
  <Step title="Declare backend ownership">
    ```json recall.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through Recall",
    ...
    `cliBackends` is the runtime ownership list. It lets Recall auto-load the
  ...
  <Step title="Register the backend">
    ...
    } from "recall/plugin-sdk/cli-backend";
...
`CliBackendConfig` describes how Recall should launch and parse the CLI:
...
| `modelAliases`                            | Map Recall model ids to CLI-native ids                    |
| `sessionArg` / `sessionArgs`              | How to pass a session id                                    |
| `sessionMode`                             | `always`, `existing`, or `none`                             |
| `sessionIdFields`                         | JSON fields Recall reads from CLI output                  |
...
| `defaultAuthProfileId`             | Prefer a specific Recall auth profile                |
| `authEpochMode`                    | Decide how auth changes invalidate stored CLI sessions |
| `nativeToolMode`                   | Declare whether the CLI has always-on native tools     |
| `bundleMcp` / `bundleMcpMode`      | Opt into Recall's loopback MCP tool bridge           |
...
CLI backends do not receive Recall tools by default. If the CLI can consume an
...
"always-on"` so Recall can fail closed when a caller requires no native tools.
...
```bash
recall plugins inspect acme-cli --runtime --json
recall agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `recall.extensions` and built runtime entries for published packages</Check>
<Check>`recall.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
