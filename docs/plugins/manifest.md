...
read_when:
  - You are building an SteelEngine plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page covers the **native SteelEngine plugin manifest**, `steelengine.plugin.json`. For compatible bundle layouts (Codex, Claude, Cursor), see [Plugin bundles](/plugins/bundles).
...
SteelEngine auto-detects those layouts but does not validate them against the `steelengine.plugin.json` schema below. For a compatible bundle, SteelEngine reads bundle metadata, declared skill roots, Claude command roots, Claude `settings.json` defaults, Claude LSP defaults, and supported hook packs, when the layout matches SteelEngine's runtime expectations.

Every native SteelEngine plugin **must** ship `steelengine.plugin.json` in the **plugin root**. SteelEngine reads it to validate configuration **without executing plugin code**. A missing or invalid manifest blocks config validation and is treated as a plugin error.
...
`steelengine.plugin.json` is metadata SteelEngine reads **before loading your plugin code**. Everything in it must be cheap enough to inspect without booting plugin runtime.
...
- QA runner metadata the shared `steelengine qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`   | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; SteelEngine still reads this during the deprecation window.                                                                                        |
| `providerUsageAuthEnvVars`           | No       | `Record<string, string[]>`   | Usage/billing-only provider credentials. SteelEngine uses these names for usage discovery and secret scrubbing but never for inference auth.                                                                                                                                  |
| `providerAuthAliases`                | No       | `Record<string, string>`     | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                                                                 |
| `channelEnvVars`                     | No       | `Record<string, string[]>`   | Cheap channel env metadata that SteelEngine can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                                                                   |
...
| `qaRunners`                          | No       | `object[]`                   | Cheap QA runner descriptors used by the shared `steelengine qa` host before plugin runtime loads.                                                                                                                                                                             |
...
The generation provider metadata fields describe static auth signals for providers declared in the matching `contracts.*GenerationProviders` list. SteelEngine reads these fields before provider runtime loads so core tools can decide whether a generation provider is available without importing every provider plugin.
...
`toolMetadata` uses the same `configSignals` and `authSignals` shapes as generation provider metadata, keyed by tool name. `contracts.tools` declares ownership. `toolMetadata` declares cheap availability evidence so SteelEngine can avoid importing a plugin runtime just to have its tool factory return `null`.
...
If a tool has no `toolMetadata`, SteelEngine preserves the existing behavior and loads the owning plugin when the tool contract matches policy. For hot-path tools whose factory depends on auth/config, plugin authors should declare `toolMetadata` instead of making core import runtime to ask.

## providerAuthChoices reference

Each `providerAuthChoices` entry describes one onboarding or auth choice. SteelEngine reads this before provider runtime loads. Provider setup lists use these manifest choices, descriptor-derived setup choices, and install-catalog metadata without loading provider runtime.
...
| `choiceLabel`         | No       | `string`                                                              | User-facing label. If omitted, SteelEngine falls back to `choiceId`.                                         |
...
the exact selected model and returns a config proposal; SteelEngine live-tests that
proposal in isolation and commits it only after success.

## commandAliases reference

Use `commandAliases` when a plugin owns a runtime command name that users may mistakenly put in `plugins.allow` or try to run as a root CLI command. SteelEngine uses this metadata for diagnostics without importing plugin runtime code.
...
the shared `steelengine qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `steelengine qa`, for example `matrix`.    |
...
SteelEngine also includes `setup.providers[].envVars` in generic provider auth and env-var lookups. `providerAuthEnvVars` remains supported through a compatibility adapter during the deprecation window, but non-bundled plugins that still use it receive a manifest diagnostic. New plugins should put setup/status env metadata on `setup.providers[].envVars`.
...
SteelEngine can also derive simple setup choices from `setup.providers[].authMethods` when no setup entry is available, or when `setup.requiresRuntime: false` declares setup runtime unnecessary. Explicit `providerAuthChoices` entries stay preferred for custom labels, CLI flags, onboarding scope, and assistant metadata.

Set `requiresRuntime: false` only when those descriptors are sufficient for the setup surface. SteelEngine treats explicit `false` as a descriptor-only contract and will not execute `setup-api` or `steelengine.setupEntry` for setup lookup. If a descriptor-only plugin still ships one of those setup runtime entries, SteelEngine reports an additive diagnostic and continues ignoring it. Omitted `requiresRuntime` keeps legacy fallback behavior so existing plugins that added descriptors without the flag do not break.
...
Use `contracts` only for static capability ownership metadata that SteelEngine can read without importing the plugin runtime.
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `steelengine migrate`.                                                                         |
...
| `dangerousFlags`              | No       | `object[]` | Config literals that `steelengine doctor` should flag as insecure or dangerous when enabled. See below.                                                                                                                                   |
...
`channelConfigs` is plugin manifest metadata, not a new top-level user config section. Users still configure channel instances under `channels.<channel-id>`. SteelEngine reads manifest metadata to decide which plugin owns that configured channel before plugin runtime code executes.
...
Non-bundled plugins that declare `channels[]` should also declare matching `channelConfigs` entries. Without them, SteelEngine can still load the plugin, but cold-path config schema, setup, and Control UI surfaces cannot know the channel-owned option shape until plugin runtime executes.

`channelConfigs.<channel-id>.commands.nativeCommandsAutoEnabled` and `nativeSkillsAutoEnabled` can declare static `auto` defaults for command config checks that run before channel runtime loads. Bundled channels can also publish the same defaults through `package.json#steelengine.channel.commands` alongside their other package-owned channel catalog metadata.
...
When `channels.chat` is configured, SteelEngine considers both the channel id and the preferred plugin id. If the lower-priority plugin was only selected because it is bundled or enabled by default, SteelEngine disables it in the effective runtime config so one plugin owns the channel and its tools. Explicit user selection still wins: if the user explicitly enables both plugins (via `plugins.allow` or a material `plugins.entries` config), SteelEngine preserves that choice and reports duplicate channel/tool diagnostics instead of silently changing the requested plugin set.
...
Use `modelSupport` when SteelEngine should infer your provider plugin from shorthand model ids like `gpt-5.6-sol` or `claude-sonnet-4.6` before plugin runtime loads.
...
SteelEngine applies this precedence:
...
Use `modelCatalog` when SteelEngine should know provider model metadata before loading plugin runtime. This is the manifest-owned source for fixed catalog rows, provider aliases, suppression rules, and discovery mode. Runtime refresh still belongs in provider runtime code, but the manifest tells core when runtime is required.
...
| `compat`           | `object`                                                       | Optional compatibility flags matching SteelEngine model config compatibility.  |
...
Do not put runtime-only data in `modelCatalog`. Use `static` only when manifest rows are complete enough for provider-filtered list and picker surfaces to skip registry/runtime discovery. Use `refreshable` when manifest rows are useful listable seeds or supplements but a refresh/cache can add more rows later; refreshable rows are not authoritative by themselves. Use `runtime` when SteelEngine must load provider runtime to know the list.
...
The map key is the integration id. If `providerAlias` is omitted, SteelEngine uses the integration id as the SecretRef provider alias. Provider aliases must match the normal SecretRef provider alias pattern, for example `team-secrets` or `onepassword-work`.

When an operator selects the preset, SteelEngine writes a provider reference like:
...
At startup/reload, SteelEngine resolves that provider by loading current plugin manifest metadata, checking that the owning plugin is installed and active, and materializing the exec command from the manifest. Disabling or removing the plugin revokes the provider for active SecretRefs. Operators who want standalone exec configuration can still write manual `command`/`args` providers directly.

Only `source: "exec"` presets are currently supported. `command` must be `${node}`, and `args[0]` must be a `./` plugin-root-relative resolver script. SteelEngine materializes it at startup/reload to the current Node executable and the absolute in-plugin script path. Node options such as `--require`, `--import`, `--loader`, `--env-file`, `--eval`, and `--print` are not part of the manifest preset contract. Operators who need non-Node commands can configure standalone manual exec providers directly.

SteelEngine derives `trustedDirs` for manifest presets from the plugin root and, for `${node}` presets, the current Node executable directory. Manifest-authored `trustedDirs` are ignored. Other exec provider options such as `timeoutMs`, `noOutputTimeoutMs`, `maxOutputBytes`, `jsonOnly`, `env`, `passEnv`, and `allowInsecurePath` pass through to the normal SecretRef exec provider config.
...
| Field                      | Type               | What it means                                                                                                        |
| -------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the SteelEngine provider id, for example `z-ai` for a `zai` provider. |
...
4. SteelEngine Provider Index preview rows.
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `steelengine.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `steelengine` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if SteelEngine must know it before loading plugin code, put it in `steelengine.plugin.json`
...
Some pre-runtime plugin metadata intentionally lives in `package.json` under the `steelengine` block instead of `steelengine.plugin.json`. `steelengine.bundle` and `steelengine.bundle.json` are not SteelEngine plugin contracts; native plugins must use `steelengine.plugin.json` plus the supported `package.json#steelengine` fields below.

Important examples:

| Field                                                                                      | What it means                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `steelengine.extensions`                                                                      | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `steelengine.runtimeExtensions`                                                               | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `steelengine.setupEntry`                                                                      | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `steelengine.runtimeSetupEntry`                                                               | Declares the built JavaScript setup entrypoint for installed packages. Requires `setupEntry`, must exist, and must stay inside the plugin package directory.                         |
| `steelengine.channel`                                                                         | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `steelengine.channel.commands`                                                                | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `steelengine.channel.configuredState`                                                         | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `steelengine.channel.persistedAuthState`                                                      | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `steelengine.install.clawhubSpec` / `steelengine.install.npmSpec` / `steelengine.install.localPath` | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `steelengine.install.defaultChoice`                                                           | Preferred install path when multiple install sources are available.                                                                                                                  |
| `steelengine.install.minHostVersion`                                                          | Minimum supported SteelEngine host version, using a semver floor like `>=2026.3.22` or `>=2026.5.1-beta.1`.                                                                             |
| `steelengine.compat.pluginApi`                                                                | Minimum SteelEngine plugin API range required by this package, using a semver floor like `>=2026.5.27`.                                                                                 |
| `steelengine.install.expectedIntegrity`                                                       | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `steelengine.install.allowInvalidConfigRecovery`                                              | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `steelengine.install.requiredPlatformPackages`                                                | npm package aliases that must materialize when their lockfile platform constraints match the current host.                                                                           |
| `steelengine.startup.deferConfiguredChannelFullLoadUntilAfterListen`                          | Lets setup-runtime channel surfaces load before listen, then defers the full configured channel plugin until post-listen activation.                                                 |

Manifest metadata decides which provider/channel/setup choices appear in onboarding before runtime loads. `package.json#steelengine.install` tells onboarding how to fetch or enable that plugin when the user picks one of those choices. Do not move install hints into `steelengine.plugin.json`.

`steelengine.install.minHostVersion` is enforced during install and manifest registry loading for non-bundled plugin sources. Invalid values are rejected; newer-but-valid values skip external plugins on older hosts. Bundled source plugins are assumed to be co-versioned with the host checkout.

`steelengine.install.requiredPlatformPackages` is for npm packages that expose required native binaries through optional, platform-specific aliases. List the bare npm package name for every supported platform alias. During npm install, SteelEngine verifies only the declared alias whose lockfile constraints match the current host. If npm reports success but omits that alias, SteelEngine retries once with a fresh cache and rolls back the install if the alias is still missing.

`steelengine.compat.pluginApi` is enforced during package install for non-bundled plugin sources. Use it for the SteelEngine plugin SDK/runtime API floor that the package was built against. It can be stricter than `minHostVersion` when a plugin package needs a newer API but still keeps a lower install hint for other flows. Official SteelEngine release sync bumps existing official plugin API floors to the SteelEngine release version by default, but plugin-only releases can keep a lower floor when the package intentionally supports older hosts. Do not use the package version alone as the compatibility contract. `peerDependencies.steelengine` remains npm package metadata; SteelEngine uses the `steelengine.compat.pluginApi` contract for install compatibility decisions.
...
Exact npm version pinning already lives in `npmSpec`, for example `"npmSpec": "@wecom/wecom-steelengine-plugin@1.2.3"`. Official external catalog entries should pair exact specs with `expectedIntegrity` so update flows fail closed if the fetched npm artifact no longer matches the pinned release. Interactive onboarding still offers trusted registry npm specs, including bare package names and dist-tags, for compatibility. Catalog diagnostics can distinguish exact, floating, integrity-pinned, missing-integrity, package-name mismatch, and invalid default-choice sources. They also warn when `expectedIntegrity` is present but there is no valid npm source it can pin. When `expectedIntegrity` is present, install/update flows enforce it; when it is omitted, the registry resolution is recorded without an integrity pin.

Channel plugins should provide `steelengine.setupEntry` when status, channel list, or SecretRef scans need to identify configured accounts without loading the full runtime. The setup entry should expose channel metadata plus setup-safe config, status, and secrets adapters; keep network clients, gateway listeners, and transport runtimes in the main extension entrypoint.

Runtime entrypoint fields do not override package-boundary checks for source entrypoint fields. For example, `steelengine.runtimeExtensions` cannot make an escaping `steelengine.extensions` path loadable.

`steelengine.install.allowInvalidConfigRecovery` is intentionally narrow. It does not make arbitrary broken configs installable. Today it only allows install flows to recover from specific stale bundled-plugin upgrade failures, such as a missing bundled plugin path or a stale `channels.<id>` entry for that same bundled plugin. Unrelated config errors still block install and send operators to `steelengine doctor --fix`.

`steelengine.channel.persistedAuthState` is package metadata for a tiny checker module:
...
`steelengine.channel.configuredState` supports cheap configured checks. Prefer declarative env metadata when environment variables are sufficient:
...
Use `env.allOf` when every listed variable is required and `env.anyOf` when any one non-empty variable is enough. If a tiny non-runtime check needs more than environment metadata, use `specifier` plus `exportName` as shown for `persistedAuthState`; when `env` is present, SteelEngine uses it without loading that module. If the check needs full config resolution or the real channel runtime, keep that logic in the plugin `config.hasConfiguredState` hook instead.

## Discovery precedence (duplicate plugin ids)

SteelEngine discovers plugins from three roots, checked in this order: bundled plugins shipped with SteelEngine, the global install root (`~/.steelengine/extensions`), and the current workspace root (`<workspace>/.steelengine/extensions`), plus any explicit `plugins.load.paths` entries.
...
2. **Global install matching a tracked install record** — a plugin installed via `steelengine plugin install`/`steelengine plugin update` that SteelEngine's install tracking recognizes for that same id, even when the id also belongs to a bundled plugin
3. **Bundled** — plugins shipped with SteelEngine
...
- To override a bundled plugin, either run `steelengine plugin install` for that id so the tracked global install outranks the bundled copy, or pin a specific path via `plugins.entries.<id>` so it wins by config-selected precedence.
...
- Unknown `channels.*` keys are **errors**, unless the channel id is declared by a plugin manifest. If the same id also appears in `plugins.allow`, `plugins.entries`, or `plugins.installs` (a plugin that is referenced but not currently discoverable), SteelEngine downgrades this to a **warning** instead.
...
- The manifest is **required for native SteelEngine plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `SteelEnginePluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.
