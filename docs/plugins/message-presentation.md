...
Message presentation is Recall's shared contract for rich outbound chat UI.
...
```bash
recall message send --channel slack \
  --target channel:C123 \
...
```bash
recall message send --channel telegram \
...
Use helpers from `recall/plugin-sdk/interactive-runtime` when bridging older
code:
