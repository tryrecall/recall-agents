---
summary: "Testing utilities and patterns for SteelEngine plugins"
...
Reference for test utilities, patterns, and lint enforcement for SteelEngine
plugins.
...
These subpaths are repo-local source entrypoints for SteelEngine's own bundled
...
import { installCommonResolveTargetErrorCases } from "steelengine/plugin-sdk/channel-target-testing";
import { AUTH_PROFILE_RUNTIME_CONTRACT } from "steelengine/plugin-sdk/agent-runtime-test-contracts";
...
import { describePluginRegistrationContract } from "steelengine/plugin-sdk/plugin-test-contracts";
...
import { getProviderHttpMocks } from "steelengine/plugin-sdk/provider-http-test-mocks";
import { withEnv, withFetchPreconnect, withServer } from "steelengine/plugin-sdk/test-env";
import { isLiveTestEnabled } from "steelengine/plugin-sdk/test-live";
import { createRequestCaptureJsonFetch } from "steelengine/plugin-sdk/test-media-understanding";
import {
  bundledPluginRoot,
  createCliRuntimeCapture,
  typedCases,
} from "steelengine/plugin-sdk/test-fixtures";
import { mockNodeBuiltinModule } from "steelengine/plugin-sdk/test-node-mocks";
```

Use these focused subpaths for bundled plugin tests. The former
`steelengine/plugin-sdk/testing` barrel was repo-local, excluded from shipped
packages, and has been removed. The legacy `steelengine/plugin-sdk/test-utils`
...
Core-only suites that depend on bundled SteelEngine inventory stay under
...
import type { MockFn, PluginRuntime, RuntimeEnv } from "steelengine/plugin-sdk/plugin-test-runtime";
```
...
exercise SteelEngine's loader acceptance gates. Add at least one loader-backed
...
Prefer the shared plugin runtime mock from `steelengine/plugin-sdk/plugin-test-runtime`.
...
| Command                                                        | Enforces                                                                                    |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| `pnpm run lint:plugins:no-monolithic-plugin-sdk-entry-imports` | Bundled plugins cannot import the monolithic `steelengine/plugin-sdk` root barrel.             |
...
SteelEngine uses Vitest 4 with informational V8 coverage reporting. For plugin tests:
