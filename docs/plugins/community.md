---
summary: "Find and publish community-maintained Recall plugins"
read_when:
...
Community plugins are third-party packages that extend Recall with channels,
...
```bash
recall plugins install npm:<package-name>
```

Use [Manage plugins](/plugins/manage-plugins) for common install, update,
inspect, and uninstall examples. Use [`recall plugins`](/cli/plugins) for the
full command reference and source-selection rules.

## Publish plugins

Publish public community plugins on ClawHub when you want Recall users to
...
| Requirement          | Why                                                 |
| -------------------- | --------------------------------------------------- |
| Published on ClawHub | Users need `recall plugins install` hints to work |
