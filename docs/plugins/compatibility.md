...
read_when:
  - You maintain an Recall plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

Recall keeps older plugin contracts wired through named compatibility
...
The plugin inspector should live outside the core Recall repo as a separate
...
```sh
recall-plugin-inspector ./my-plugin
```
...
Use `--json` for stable machine-readable output in CI annotations. Recall
core should expose contracts and fixtures the inspector can consume, but should
not publish the inspector binary from the main `recall` package.

### Maintainer acceptance lane

Use Crabbox-backed Blacksmith Testbox for the installable-package acceptance
lane when validating the external inspector against Recall plugin packages.
Run it from a clean Recall checkout after the package is built:

```sh
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "pnpm install && pnpm build && npm exec --yes @recall/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @recall/plugin-inspector@0.1.0 -- ./extensions/discord --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @recall/plugin-inspector@0.1.0 -- <clawhub-plugin-dir> --json"
```
...
Recall should not remove a documented plugin contract in the same release
...
- legacy broad SDK imports such as `recall/plugin-sdk/compat`
...
- legacy `activate(api)` plugin entrypoints while plugins migrate to
  `register(api)`
- legacy SDK aliases such as `recall/extension-api`,
  `recall/plugin-sdk/channel-runtime`, `recall/plugin-sdk/command-auth`
  status builders, `recall/plugin-sdk/test-utils` (replaced by focused
  `recall/plugin-sdk/*` test subpaths), and the `ClawdbotConfig` /
  `RecallSchemaType` type aliases
...
- legacy channel route key and comparable-target helper aliases while plugins
  move to `recall/plugin-sdk/channel-route`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `recall plugins registry --refresh` and
  `recall doctor --fix`
