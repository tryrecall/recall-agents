...
| Codex native permission requests | Codex asks before native shell, file, MCP, or app-server actions.        | Codex app-server or native hook approval handling, routed through plugin approvals when SteelEngine owns the prompt. |
| MCP approval elicitations        | A Codex MCP server requests approval for a tool call.                    | MCP approval responses bridged through SteelEngine plugin approvals.                                                 |
...
runs after the model selects a tool and before SteelEngine executes it:
...
SteelEngine creates a pending approval with a `plugin:` ID, delivers it to the
...
SteelEngine treats `allow-once` and `allow-always` as approval decisions for the
...
If the hook also returns `params`, SteelEngine applies those parameter changes only
...
- Codex app-server approval requests route through SteelEngine after Codex review.
