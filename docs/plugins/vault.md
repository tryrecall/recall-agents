...
read_when:
  - You want SteelEngine to read API keys from HashiCorp Vault
...
The bundled Vault plugin lets SteelEngine resolve `exec` SecretRefs from
HashiCorp Vault at Gateway startup and reload time. SteelEngine stores Vault
references in config, keeps resolved values in the in-memory secrets snapshot,
and does not write the resolved API keys back to `steelengine.json`.

Use this when you already run Vault or want model provider keys to live outside
SteelEngine config files. For the SecretRef runtime model, see
...
Enable the bundled plugin before running the `steelengine vault` commands:

```bash
steelengine plugins enable vault
```

## Store a provider key in Vault

SteelEngine defaults to KV v2 mounted at `secret`, matching Vault dev-server
examples. For production Vault, set `STEELENGINE_VAULT_KV_MOUNT` to your actual KV
mount path before creating SecretRef ids. With the SteelEngine defaults, this
SecretRef id:
...
Use a scoped client token for SteelEngine, not a root token. For the default KV v2
...
that starts SteelEngine. The default auth method reads a Vault client token from
`VAULT_TOKEN`:
...
```bash
export VAULT_ADDR=https://vault.example.com
export STEELENGINE_VAULT_AUTH_METHOD=token_file
...
These variables must be present when SteelEngine starts. The Vault plugin forwards
...
```bash
export VAULT_ADDR=https://vault.example.com
export STEELENGINE_VAULT_AUTH_METHOD=jwt
export STEELENGINE_VAULT_AUTH_MOUNT=jwt
export STEELENGINE_VAULT_AUTH_ROLE=steelengine
export STEELENGINE_VAULT_JWT_FILE=/var/run/secrets/tokens/vault
```
...
```bash
export VAULT_ADDR=https://vault.example.com
export STEELENGINE_VAULT_AUTH_METHOD=kubernetes
export STEELENGINE_VAULT_AUTH_ROLE=steelengine
```

Set `STEELENGINE_VAULT_AUTH_MOUNT` only when Vault mounted Kubernetes auth somewhere
other than `auth/kubernetes`. Set `STEELENGINE_VAULT_JWT_FILE` only when the service
...
export STEELENGINE_VAULT_KV_MOUNT=secret
export STEELENGINE_VAULT_KV_VERSION=2
```

Check what the current shell can see:

```bash
steelengine vault status
```
...
```bash
steelengine vault status --provider-alias corp-vault
```

`steelengine vault status` never prints `VAULT_TOKEN`; it reports only whether the
...
```bash
steelengine secrets apply --from ./vault-secrets-plan.json --dry-run --allow-exec
steelengine secrets apply --from ./vault-secrets-plan.json --allow-exec
steelengine secrets audit --check --allow-exec
steelengine secrets reload
```

Use `--allow-exec` because the Vault plugin resolves through an SteelEngine-managed
exec SecretRef provider.

If the Gateway is not running yet, start it normally after applying the plan
instead of running `steelengine secrets reload`.
...
```bash
steelengine vault setup --openai-id providers/openai/apiKey
steelengine vault setup --anthropic-id providers/anthropic/apiKey
steelengine vault setup --openrouter-id providers/openrouter/apiKey
```
...
Bare target paths apply to `steelengine.json`. Use
`auth-profiles:<agentId>:<path>` for existing `auth-profiles.json` targets.
The target path must be a registered SteelEngine SecretRef target. The setup
command does not create arbitrary named secrets in SteelEngine; Vault remains the
secret store, and SteelEngine stores SecretRefs only on supported config fields.
...
```bash
export STEELENGINE_VAULT_KV_VERSION=1
```
...
- one auth source:
  - `VAULT_TOKEN`
  - `STEELENGINE_VAULT_AUTH_METHOD=token_file` plus `VAULT_TOKEN_FILE`
  - `STEELENGINE_VAULT_AUTH_METHOD=jwt` plus `STEELENGINE_VAULT_AUTH_MOUNT`,
    `STEELENGINE_VAULT_AUTH_ROLE`, and `STEELENGINE_VAULT_JWT_FILE`
  - `STEELENGINE_VAULT_AUTH_METHOD=kubernetes` plus `STEELENGINE_VAULT_AUTH_ROLE`; optionally
    override `STEELENGINE_VAULT_AUTH_MOUNT` or `STEELENGINE_VAULT_JWT_FILE`
- optional `VAULT_NAMESPACE`, `STEELENGINE_VAULT_KV_MOUNT`, and
  `STEELENGINE_VAULT_KV_VERSION`

When using Kubernetes, prefer `STEELENGINE_VAULT_AUTH_METHOD=kubernetes`
when Vault has Kubernetes auth configured for the cluster. Use
`STEELENGINE_VAULT_AUTH_METHOD=jwt` only when Vault is configured to treat the cluster
...
deployment config. SteelEngine does not require a fixed mount, role, or path: each
Gateway environment can set its own `STEELENGINE_VAULT_KV_MOUNT`,
`STEELENGINE_VAULT_AUTH_ROLE`, and SecretRef ids. If one shared Gateway must resolve
...
- [Secrets management](/gateway/secrets)
- [`steelengine secrets`](/cli/secrets)
