...
read_when:
  ...
  - You are changing plugin behavior across SteelEngine and Codex harness turns
---
...
continuation, and native compaction. SteelEngine owns channel routing, session
files, visible message delivery, SteelEngine dynamic tools, approvals, media
...
SteelEngine compatibility route keeps the normal SteelEngine system prompt even when
it uses Codex-flavored OpenAI auth or transport.

SteelEngine starts and resumes native Codex threads with Codex's built-in
personality disabled (`personality: "none"`) so workspace personality files
and SteelEngine agent identity stay authoritative. Native Codex keeps Codex-owned
base/model instructions and project-doc loading otherwise. Lightweight
SteelEngine runs (for example cron) still suppress project-doc loading.

SteelEngine developer instructions cover SteelEngine runtime concerns: source-channel
delivery, SteelEngine dynamic tools, ACP delegation, adapter context, and the
...
Most SteelEngine dynamic tools use the searchable `steelengine` namespace. Tools
marked `catalogMode: "direct-only"` use `steelengine_direct`, which Codex keeps
directly model-visible as `DirectModelOnly` instead of exposing it to nested
Code Mode execution.

## Thread bindings and model changes

When an SteelEngine session is attached to an existing Codex thread, the next
...
Supervised bindings are the exception. The SteelEngine model picker stays locked,
...
model-difference warning; the outer SteelEngine model and fallback chain never
...
not resume a thread, subscribe SteelEngine to its live events, or answer its
approvals.
...
On the same supervision connection, SteelEngine starts the canonical
...
resumed. The canonical thread has the full SteelEngine harness tool surface;
...
SteelEngine allows archive for a local `idle` or `notLoaded` row only after explicit
...
prove that another process is not using the thread. SteelEngine blocks a known
...
contract: the agent replies normally and SteelEngine posts the final text to the
...
Codex heartbeat turns get `heartbeat_respond` in the searchable SteelEngine tool
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| SteelEngine plugin hooks                 | SteelEngine                 | Product/plugin compatibility across SteelEngine and Codex harnesses.   |
| Codex app-server extension middleware | SteelEngine bundled plugins | Per-turn adapter behavior around SteelEngine dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

SteelEngine does not use project or global Codex `hooks.json` files to route
plugin behavior. For the native tool and permission bridge, SteelEngine injects
...
so Codex's app-server reviewer and SteelEngine's approval bridge handle real
...
controls; they are not exposed as SteelEngine plugin hooks in the v1 contract.

For SteelEngine dynamic tools, SteelEngine executes the tool after Codex asks for
the call, so plugin and middleware behavior runs in the harness adapter. For
Codex-native tools, Codex owns the canonical tool record; SteelEngine can mirror
...
matching app-server approval. If an SteelEngine `before_tool_call` hook returns
`requireApproval` while the native payload sets `steelengine_approval_mode:
...
request for the same tool use, SteelEngine opens the plugin approval prompt and
maps the decision back to Codex. Codex `PermissionRequest` events are a
separate approval path and can still route through SteelEngine approvals when
...
notifications and SteelEngine adapter state, not native Codex hook commands.
...
debugging. They do not invoke SteelEngine plugin hooks.
...
| SteelEngine channel routing and delivery         | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                                                                                                                                                                                                                                                                                                    |
| SteelEngine dynamic tools                        | Supported                                                                        | Codex asks SteelEngine to execute these tools, so SteelEngine stays in the execution path.                                                                                                                                                                                                                                                                                                                                                                                                |
| Prompt and context plugins                    | Supported                                                                        | SteelEngine projects SteelEngine-specific prompt/context into the Codex turn while leaving Codex-owned base, model, and configured project-doc prompts in the native Codex lane. SteelEngine disables Codex's built-in personality for native threads so agent workspace personality files remain authoritative. Native Codex developer instructions accept only command guidance explicitly scoped to `codex_app_server`; legacy global command hints remain for non-Codex prompt surfaces. |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around SteelEngine-owned dynamic tools.                                                                                                                                                                                                                                                                                                                                                                          |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through SteelEngine after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review.                                                                                                                                                                                                                                                                          |
| App-server trajectory capture                 | Supported                                                                        | SteelEngine records the request it sent to app-server and the app-server notifications it receives.                                                                                                                                                                                                                                                                                                                                                                                    |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but SteelEngine does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. SteelEngine owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms SteelEngine-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | SteelEngine can request native compaction, but does not receive a stable kept/dropped list, token delta, completion summary, or summary payload.   | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | SteelEngine does not let plugins or context engines veto, rewrite, or replace native Codex compaction.                                             | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | SteelEngine can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, SteelEngine only returns explicit allow or deny
...
request, SteelEngine remembers that exact provider/session/tool input/cwd
...
Codex MCP tool approval elicitations route through SteelEngine's plugin approval
...
resolve the same gateway record before SteelEngine returns the app-server answer.
...
default `messages.queue.mode: "steer"`, SteelEngine batches steer-mode chat
...
that case, SteelEngine waits for the active run to finish before starting the
...
harness, SteelEngine also calls Codex app-server `feedback/upload` for relevant
...
SteelEngine session ids, Codex thread ids, and local `codex resume <thread-id>`
commands for the threads that were sent.

If you deny or ignore the approval, SteelEngine does not print those Codex ids
...
belongs to Codex app-server. SteelEngine does not run preflight compaction for
Codex turns, replace Codex compaction with context-engine compaction, or fall
back to SteelEngine or public OpenAI summarization when native compaction cannot
be started. SteelEngine keeps a transcript mirror for channel history, search,
...
SteelEngine keeps the request and shared-client lease open until Codex emits the
...
timeout, SteelEngine requests a native turn interrupt. The lease and per-thread
...
period, SteelEngine retires the connection before releasing the fence. Remote
...
compaction is Codex's job; SteelEngine only starts native compaction for manually
requested triggers.

When a context engine requests Codex thread-bootstrap projection, SteelEngine
...
Codex reasoning or plan records when the app-server emits them. SteelEngine
...
not rewrite Codex-native tool result records. It only applies when SteelEngine
writes an SteelEngine-owned session transcript tool result.

## Media and delivery

SteelEngine continues to own media delivery and media provider selection. Image,
...
through the normal SteelEngine delivery path; media generation does not require
the legacy runtime. When Codex emits a native image-generation item with a
`savedPath`, SteelEngine forwards that exact file through the normal reply-media
