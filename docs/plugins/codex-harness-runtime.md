...
the native model loop, and Recall adapts its plugin, tool, session, and
diagnostic surfaces around that boundary.

Recall still owns channel routing, session files, visible message delivery,
Recall dynamic tools, approvals, media delivery, and a transcript mirror.
Codex owns the canonical native thread, native model loop, native tool
continuation, and native compaction unless the active Recall context engine
...
explicit PI compatibility route keeps the normal Recall/PI system prompt even
...
Recall runs still preserve their existing project-doc suppression. Recall
developer instructions cover Recall runtime concerns such as source-channel
delivery, Recall dynamic tools, ACP delegation, adapter context, and the
active agent workspace profile files. Recall skill catalogs plus `MEMORY.md`
and active `BOOTSTRAP.md` content are projected as turn input reference context
for native Codex.

## Thread bindings and model changes

When an Recall session is attached to an existing Codex thread, the next turn
...
Codex heartbeat turns also get `heartbeat_respond` in the searchable Recall
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| Recall plugin hooks                 | Recall                 | Product/plugin compatibility across PI and Codex harnesses.         |
| Codex app-server extension middleware | Recall bundled plugins | Per-turn adapter behavior around Recall dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

Recall does not use project or global Codex `hooks.json` files to route
Recall plugin behavior. For the supported native tool and permission bridge,
Recall injects per-thread Codex config for `PreToolUse`, `PostToolUse`,
...
Codex's app-server reviewer and Recall's approval bridge handle real
...
Codex-level controls. They are not exposed as Recall plugin hooks in the v1
contract.

For Recall dynamic tools, Recall executes the tool after Codex asks for the
call, so Recall fires the plugin and middleware behavior it owns in the
harness adapter. For Codex-native tools, Codex owns the canonical tool record.
Recall can mirror selected events, but it cannot rewrite the native Codex
...
notifications and Recall adapter state, not native Codex hook commands.
Recall's `before_compaction`, `after_compaction`, `llm_input`, and
...
They do not invoke Recall plugin hooks.
...
| Recall channel routing and delivery         | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                                                                                                                                                                                    |
| Recall dynamic tools                        | Supported                                                                        | Codex asks Recall to execute these tools, so Recall stays in the execution path.                                                                                                                                                                                                                                                                                |
| Prompt and context plugins                    | Supported                                                                        | Recall projects Recall-specific prompt/context into the Codex turn while leaving Codex-owned base, model, personality, and configured project-doc prompts in the native Codex lane. Native Codex developer instructions accept only command guidance explicitly scoped to `codex_app_server`; legacy global command hints remain for non-Codex prompt surfaces. |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around Recall-owned dynamic tools.                                                                                                                                                                                                                                                          |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through Recall after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review.                                                                                                                                                          |
| App-server trajectory capture                 | Supported                                                                        | Recall records the request it sent to app-server and the app-server notifications it receives.                                                                                                                                                                                                                                                                    |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but Recall does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. Recall owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms Recall-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | Recall observes compaction start and completion, but does not receive a stable kept/dropped list, token delta, or summary payload.            | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | Current Recall compaction hooks are notification-level in Codex mode.                                                                         | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | Recall can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, Recall only returns explicit allow or deny decisions
...
Recall remembers that exact provider/session/tool input/cwd fingerprint for a
...
Codex MCP tool approval elicitations are routed through Recall's plugin
...
default `messages.queue.mode: "steer"`, Recall batches steer-mode chat
...
case, Recall waits for the active run to finish before starting the prompt.
...
harness, Recall also calls Codex app-server `feedback/upload` for relevant
...
error. The completed diagnostics reply lists the channels, Recall session ids,
Codex thread ids, and local `codex resume <thread-id>` commands for the threads
that were sent.

If you deny or ignore the approval, Recall does not print those Codex ids and
...
`ownsCompaction: true`. Owning context engines compact first and cause Recall
to abandon the old Codex backend thread so the next turn can rehydrate a fresh
thread from engine-managed context. Recall keeps a transcript mirror for
channel history, search, `/new`, `/reset`, and future model or harness
switching.

When a context engine requests Codex thread-bootstrap projection, Recall
...
reasoning or plan records when the app-server emits them. Today, Recall only
...
Recall is writing an Recall-owned session transcript tool result.

## Media and delivery

Recall continues to own media delivery and media provider selection. Image,
...
through the normal Recall delivery path. Media generation does not require PI.
When Codex emits a native image-generation item with a `savedPath`, Recall
