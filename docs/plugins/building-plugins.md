---
summary: "Create your first Recall plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new Recall plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend Recall without changing core. A plugin can add a messaging
channel, model provider, local CLI backend, agent tool, hook, media provider,
or another plugin-owned capability.

You do not need to add an external plugin to the Recall repository. Publish
the package to [ClawHub](/clawhub) and users install it with:

```bash
recall plugins install clawhub:<package-name>
```
...
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because Recall loads bundled
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Recall to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through Recall model fallback.
  </Card>
...
```json recall.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to Recall",
  "contracts": {
    "tools": ["my_tool"]
  },
  "activation": {
...
```
    ...
    must appear in `contracts.tools` so Recall can discover ownership without
  ...
  <Step title="Test the runtime">
    ...
    ```bash
    recall plugins inspect my-plugin --runtime --json
    ```
    ...
    `recall demo-plugin ping`.

    For a bundled plugin in this repository, Recall discovers source-checkout
  ...
  <Step title="Install">
    ...
    ```bash
    recall plugins install clawhub:your-org/your-plugin
    ```
...
operator, installed plugin code, or a modified Recall runtime. Sensitive local
...
aligned with `api.registerTool(..., { optional: true })` so Recall can avoid
...
```typescript
import { definePluginEntry } from "recall/plugin-sdk";
```
...
`recall/plugin-sdk/gateway-method-runtime` bridge is reserved for plugin HTTP
...
<Check>**package.json** has correct `recall` metadata</Check>
<Check>**recall.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [tryrecall/recall-agents](https://github.com/tryrecall/recall-agents/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official Recall X account [@recall](https://x.com/recall) for release announcements.
