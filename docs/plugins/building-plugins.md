---
summary: "Create your first SteelEngine plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new SteelEngine plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend SteelEngine without changing core. A plugin can add a messaging
channel, model provider, local CLI backend, agent tool, hook, media provider,
or another plugin-owned capability.

You do not need to add an external plugin to the SteelEngine repository. Publish
...
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because SteelEngine discovers
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect SteelEngine to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through SteelEngine model fallback.
  </Card>
...
```json steelengine.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to SteelEngine",
  "contracts": {
    "tools": ["my_tool"]
  },
  "activation": {
...
```
    ...
    appear in `contracts.tools` so SteelEngine can discover ownership without
  ...
  <Step title="Register the tool">
    ...
    `defineChannelPluginEntry` from `steelengine/plugin-sdk/core` instead.
  ...
  <Step title="Test the runtime">
    ...
    ```bash
    steelengine plugins inspect my-plugin --runtime --json
    ```

    If the plugin registers a CLI command, run that command too and confirm
    output, for example `steelengine demo-plugin ping`.

    For a bundled plugin in this repository, SteelEngine discovers source-checkout
  ...
  <Step title="Test the package install">
    ...
    `steelengine.extensions` at built JavaScript like `./dist/index.js`, and make
    ...
    steelengine plugins install npm-pack:/tmp/<plugin-package>.tgz --force
    steelengine plugins inspect my-plugin --runtime --json
    ```

    `npm-pack:` uses SteelEngine's managed per-plugin npm project, so it catches
  ...
  <Step title="Install">
    ...
    ```bash
    steelengine plugins install clawhub:your-org/your-plugin
    ```
...
plugin is enabled. Optional tools need explicit user opt-in before SteelEngine
...
operator, installed plugin code, or a modified SteelEngine runtime. Sensitive
...
aligned with `api.registerTool(..., { optional: true })` so SteelEngine can avoid
...
```typescript
import { definePluginEntry } from "steelengine/plugin-sdk";
```
...
`steelengine/plugin-sdk/gateway-method-runtime` bridge is reserved for plugin HTTP
...
<Check>**package.json** has correct `steelengine` metadata</Check>
<Check>**steelengine.plugin.json** manifest is present and valid</Check>
...
1. Watch [steelengine/steelengine](https://github.com/steelengineai/recall-agents/releases) releases (`Watch` > `Releases`). Beta tags look like `v2026.3.N-beta.1`. You can also follow [@steelengine](https://x.com/steelengine) on X for release announcements.
