---
summary: "Manage SteelEngine plugins from the Control UI or CLI"
read_when:
...
rules, and edge cases, see [`steelengine plugins`](/cli/plugins).
...
configured Control UI base path. For example, a base path of `/steelengine` uses
`/steelengine/settings/plugins`. The page has two tabs:
...
- **Discover** is the store: featured plugins included with SteelEngine, official
...
or **Disable**. Workboard, for example, is included with SteelEngine and disabled
...
OAuth-backed MCP connectors still need a one-time `steelengine mcp login <name>`
...
`plugins list` is a cold inventory check: what SteelEngine can discover from
...
an install hint (`steelengine plugins install clawhub:<package>`) per result.

## Enable and disable plugins

```bash
steelengine plugins enable <plugin-id>
steelengine plugins disable <plugin-id>
```
...
steelengine plugins install clawhub:<package>@1.2.3
steelengine plugins install clawhub:<package>@beta

# Install from npm.
steelengine plugins install npm:<package>
steelengine plugins install npm:@scope/steelengine-plugin@1.2.3
steelengine plugins install npm:@steelengine/codex

# Install from a local npm-pack artifact.
steelengine plugins install npm-pack:<path.tgz>

# Install from git or a local development checkout.
steelengine plugins install git:github.com/acme/steelengine-plugin@v1.0.0
...
name matches a bundled or official plugin id, in which case SteelEngine uses
that local/official copy instead. Use `clawhub:`, `npm:`, `git:`, or
`npm-pack:` for deterministic source selection. SteelEngine's bundled and official
...
ClawHub, or hook-pack install, use `steelengine plugins update` instead. With
...
```bash
steelengine plugins update <plugin-id>
steelengine plugins update <npm-package-or-spec>
steelengine plugins update --all
steelengine plugins update <plugin-id> --dry-run
```
...
`steelengine plugins update --all` is the bulk maintenance path. It still
respects ordinary tracked install specs, but trusted official SteelEngine
...
```bash
steelengine plugins update @scope/steelengine-plugin@beta
steelengine plugins update @scope/steelengine-plugin
```
...
See [`steelengine plugins`](/cli/plugins#update) for the exact fallback and
pinning rules.

## Uninstall plugins

```bash
steelengine plugins uninstall <plugin-id> --dry-run
steelengine plugins uninstall <plugin-id>
steelengine plugins uninstall <plugin-id> --keep-files
```
...
In Nix mode (`STEELENGINE_NIX_MODE=1`), plugin install, update, uninstall,
...
| ----------- | --------------------------------------------------------------------------- | -------------------------------------------------------------- |
| ClawHub     | You want SteelEngine-native discovery, scan summaries, versions, and hints     | `steelengine plugins install clawhub:<package>`                   |
| git         | You want a branch, tag, or commit from a repository                         | `steelengine plugins install git:github.com/<owner>/<repo>@<ref>` |
...
| npm pack    | You are proving a local package artifact through npm install semantics      | `steelengine plugins install npm-pack:<path.tgz>`                 |
| npmjs.com   | You already ship JavaScript packages or need npm dist-tags/private registry | `steelengine plugins install npm:@acme/steelengine-plugin`           |
...
ClawHub is the primary public discovery surface for SteelEngine plugins. Publish
...
Native npm plugins must ship a plugin manifest (`steelengine.plugin.json`) plus
`package.json` metadata before publishing:

```json package.json
{
  "name": "@acme/steelengine-plugin",
  "version": "1.0.0",
  "type": "module",
  "steelengine": {
...
steelengine plugins install npm:@acme/steelengine-plugin
steelengine plugins install npm:@acme/steelengine-plugin@beta
steelengine plugins install npm:@acme/steelengine-plugin@1.0.0
```
...
- [Building plugins](/plugins/building-plugins) shows the full plugin
  package shape (including `steelengine.plugin.json`) and first publish
  workflow.
