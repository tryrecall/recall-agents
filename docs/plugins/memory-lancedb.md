...
```bash
recall plugins install @recall/memory-lancedb
```

The plugin is published to npm and is not bundled into the Recall runtime image.
...
Set `dimensions` for non-standard embedding models. Recall knows the
...
`memory_forget` query path, and `recall ltm search`. Auto-recall prefers the
...
```bash
recall ltm list
recall ltm search "project preferences"
recall ltm stats
```
...
```bash
recall ltm query --cols id,text,createdAt --limit 20
recall ltm query --filter "category = 'preference'" --order-by createdAt:desc
```
...
By default, LanceDB data lives under `~/.recall/memory/lancedb`. Override the
path with `dbPath`:
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "s3://memory-bucket/recall",
          storageOptions: {
...
Recall treats that package as part of the plugin package. Gateway startup
...
`@lancedb/lancedb` error during plugin load, upgrade Recall and restart the
Gateway.
...
```bash
recall ltm stats
recall ltm search "recent preference"
```
