...
channels, slots, and auto-enable rules. If that derived set is empty, Recall
...
Recall does not cache plugin discovery results or direct manifest registry
...
Compatibility-only provider fields that Recall no longer calls, such as
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of Recall's static catalog.
  </Accordion>
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  product plugins own their routes and schemas while Recall owns the
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-recall-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-recall-scopes` only when the header is explicitly present
  - if `x-recall-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-recall-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `recall/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `recall/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `recall/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `recall/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `recall/plugin-sdk/config-schema` | Root `recall.json` Zod schema (`RecallSchema`) |
...
<Info>
`recall/plugin-sdk/channel-runtime`, `recall/plugin-sdk/config-runtime`,
and `recall/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `recall/plugin-sdk/*` subpaths. Never
...
Security note: `recall plugins install` installs plugin dependencies with a
...
Useful `recall.channel` fields beyond the minimal example:
...
contain `{ "entries": [ { "name": "@scope/pkg", "recall": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `recall.install` block. The
