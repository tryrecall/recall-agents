...
An **agent harness** is the low level executor for one prepared SteelEngine agent
...
runtime and the normal SteelEngine provider transport is the wrong abstraction:
...
- a model runtime that needs its own resume id in addition to the SteelEngine
  session transcript
...
Core still forwards a compatible, explicitly selected or ordered SteelEngine auth
...
Verified SteelEngine continuations also pass `params.expectedRuntimeArtifact`.
...
The prepared attempt also includes `params.runtimePlan`, an SteelEngine-owned
policy bundle for runtime decisions that must stay shared across SteelEngine and
native harnesses:
...
Harnesses may use the plan for decisions that need to match SteelEngine behavior,
...
all of them before dispatch. Implicit selection uses SteelEngine if no plugin can
own the full set; an explicit or persisted plugin selection fails closed.

## Register a harness

**Import:** `steelengine/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "steelengine/plugin-sdk/agent-harness";
...
Delegates receive only work admission and embedded execution. SteelEngine requires
...
SteelEngine chooses a harness after provider/model resolution:
...
4. If no registered harness matches, SteelEngine uses its embedded runtime.
...
provider/model. Once a plugin harness has claimed a run, SteelEngine does not
...
SteelEngine. The harness then claims that provider in `supports(...)`.
...
- app-server request: SteelEngine sends the bare model id to Codex and lets the
...
behavior stay on SteelEngine. Plaintext official HTTP endpoints are rejected. Older `codex/gpt-*`
...
blocks older or unversioned servers, so SteelEngine only runs against the protocol
...
seam is for async tool-result transforms that must run before SteelEngine or
...
`steelengine/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so SteelEngine's fallback policy can decide whether to retry on a
different model. `planning-only` requires the harness's explicit `planText`
field; SteelEngine does not infer it from assistant prose. The helper
...
`steelengine/plugin-sdk/agent-harness-runtime` after they finalize an attempt. It
dispatches the portable `agent_end` hook and SteelEngine's research capture
...
user-input helpers from `steelengine/plugin-sdk/agent-harness-runtime` to format
the prompt, deliver it through SteelEngine's blocking reply path, and normalize
...
`steelengine/plugin-sdk/agent-harness-tool-runtime`. It owns
...
The bundled `codex` harness is the native Codex mode for embedded SteelEngine
...
refs should be repaired with `steelengine doctor --fix`, and legacy `codex/*`
model refs remain compatibility aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. SteelEngine still owns the chat channel,
...
By default, SteelEngine uses `auto` provider/model runtime policy: registered
...
token. Keep that binding explicitly associated with the SteelEngine session, and
keep mirroring user-visible assistant/tool output into the SteelEngine
transcript.

The SteelEngine transcript remains the compatibility layer for:

- channel-visible session history
- transcript search and indexing
- switching back to the built-in SteelEngine harness on a later turn
- generic `/new`, `/reset`, and session deletion behavior

If your harness stores a sidecar binding, implement `reset(...)` so SteelEngine
can clear it when the owning SteelEngine session is reset.

## Tool and media results

Core constructs the SteelEngine tool list and passes it into the prepared
...
outputs on the same delivery path as SteelEngine-backed runs.

### Terminal tool outcomes

`AgentHarnessAttemptParams.observeToolTerminal` is the host-owned terminal
outcome accumulator. A harness that executes SteelEngine dynamic tools or native
...
- Report `outcome: "success"` or `outcome: "failure"`. Include the structured
  ...
  display text.
- Use `nativeMutation` only for native tools that do not use an SteelEngine tool
  definition. Supply protocol-owned mutation and replay facts there; do not
  copy SteelEngine's mutation classifier into the harness.
...
without terminal reports, SteelEngine cannot preserve mutating-tool failure truth
