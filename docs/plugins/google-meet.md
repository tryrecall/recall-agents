...
read_when:
  - You want an Recall agent to join a Google Meet call
  - You want an Recall agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for Recall — the plugin is explicit by design:
...
- `agent` is the default talk-back mode: realtime transcription listens, the
  configured Recall agent answers, and regular Recall TTS speaks into Meet.
...
provider plus regular Recall TTS. OpenAI is the default transcription
...
```bash
recall googlemeet setup --transport chrome-node --mode transcribe
```
...
before asking an agent to join. Use `recall googlemeet setup --json` for
...
```bash
recall googlemeet setup --transport twilio
```
...
```bash
recall googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
recall googlemeet create --transport chrome-node --mode agent
```
...
```bash
recall googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
- Browser fallback: used when OAuth credentials are absent. Recall uses the
  ...
  the Recall Chrome profile on the node to already be signed in to Google.
...
Recall's microphone/camera permission grant and avoid the Meet **Use
...
Use `recall googlemeet test-listen <meet-url> --transport chrome-node` when
...
Local Chrome joins through the signed-in Recall browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by Recall. For
...
You do **not** need a full Recall Gateway or model API key inside a macOS VM
...
- Gateway host: Recall Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: Recall CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update Recall in the VM, then enable the bundled plugin there:

```bash
recall plugins enable google-meet
```

Start the node host in the VM:

```bash
recall node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
RECALL_ALLOW_INSECURE_PRIVATE_WS=1 \
  recall node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
RECALL_ALLOW_INSECURE_PRIVATE_WS=1 \
  recall node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
recall node restart
```

`RECALL_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`recall.json` setting. `recall node install` stores it in the LaunchAgent
...
```bash
recall googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, Recall browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, Recall auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `recall node run` in the VM,
  approve pairing, and make sure `recall plugins enable google-meet` and
  `recall plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses Recall
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. Recall
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by Recall; use separate virtual devices or Loopback-style routing
...
Recall does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with Recall, review BlackHole's
...
Chrome transport opens the Meet URL through Recall browser control and joins
as the signed-in Recall browser profile. On macOS, the plugin checks for
...
```bash
recall googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
recall googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local Recall audio
...
meeting; Recall does not discover those from the Meet page.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an Recall agent. Be brief.",
...
recall plugins list | grep -E 'google-meet|voice-call'
recall googlemeet setup
```
...
`RECALL_GOOGLE_MEET_*` environment variables.
...
4. Add the scopes Recall requests:
...
`meetings.space.readonly` lets Recall resolve Meet URLs/codes to spaces.
`meetings.space.settings` lets Recall pass `SpaceConfig` settings such as
...
```bash
recall googlemeet auth login --json
```
...
```bash
RECALL_GOOGLE_MEET_CLIENT_ID="your-client-id" \
RECALL_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
recall googlemeet auth login --json
```
...
RECALL_GOOGLE_MEET_CLIENT_ID="your-client-id" \
RECALL_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
recall googlemeet auth login --json --manual
```
...
support existed, rerun `recall googlemeet auth login --json` so the refresh
...
```bash
recall googlemeet doctor --oauth --json
```
...
```bash
recall googlemeet doctor --oauth --create-space --json
recall googlemeet create --no-join --json
```
...
```bash
recall googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
recall googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `recall googlemeet auth login
...
- `RECALL_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `RECALL_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `RECALL_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `RECALL_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `RECALL_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `RECALL_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `RECALL_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
recall googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
recall googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
recall googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
recall googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
recall googlemeet latest --today
recall googlemeet calendar-events --today --json
recall googlemeet artifacts --event "Weekly sync"
recall googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
recall googlemeet latest --meeting https://meet.google.com/abc-defg-hij
recall googlemeet artifacts --conference-record conferenceRecords/abc123 --json
recall googlemeet attendance --conference-record conferenceRecords/abc123 --json
```
...
```bash
recall googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```
...
Recall accepts a Meet URL, meeting code, or `spaces/{id}` input and resolves it
to the API space resource before ending the active conference.
It is separate from `googlemeet leave`: `leave` stops Recall's local/session
...
```bash
recall googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
recall googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
recall googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
recall googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
RECALL_LIVE_TEST=1 \
RECALL_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
...
```bash
recall googlemeet setup --transport chrome-node --mode transcribe
recall googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

Live smoke environment:

- `RECALL_LIVE_TEST=1` enables guarded live tests.
- `RECALL_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `RECALL_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `RECALL_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `RECALL_GOOGLE_MEET_CLIENT_SECRET`,
  `RECALL_GOOGLE_MEET_ACCESS_TOKEN`, and
  `RECALL_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `RECALL_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the Recall browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the Recall browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, Recall reports `manualActionRequired: true` or a
...
realtime transcription provider key, and a configured Recall TTS provider.
...
- `chrome.guestName: "Recall Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through Recall browser automation on `chrome-node`
...
- `mode: "agent"`: default talk-back mode. Participant speech is transcribed by
  ...
  Recall agent in a per-meeting sub-agent session, and spoken back through the
  normal Recall TTS runtime.
...
- `realtime.agentId`: optional Recall agent id for
...
the realtime transcription provider handles listening, the configured Recall
agent produces the answer, and regular Recall TTS speaks it into Meet. Use
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: whether
  managed Chrome speech is allowed now. `speechReady: false` means Recall did
...
transcripts are routed through the configured Recall agent, and the answer is
spoken through the normal Recall TTS runtime. Set `mode: "bidi"` when you want
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured Recall agent | Normal Recall TTS runtime            | You want "my agent is in the meeting" behavior        |
...
information, or normal Recall tools, it can call `openclaw_agent_consult`.

The consult tool runs the regular Recall agent behind the scenes with recent
meeting transcript context and returns a concise spoken answer. In `agent` mode,
Recall sends that answer directly to the TTS runtime; in `bidi` mode, the
...
Meet lane should consult a dedicated Recall agent workspace, model defaults,
...
```bash
recall googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
- `recall logs --follow` shows DTMF TwiML served before realtime TwiML, then a
...
```bash
recall plugins list | grep google-meet
recall googlemeet setup
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; Recall
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, Recall
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `RECALL_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `recall googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the Recall Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. Recall should click **Use microphone** or, for
...
```bash
recall googlemeet setup
recall googlemeet doctor
```

Use `mode: "agent"` for the normal STT -> Recall agent -> TTS talk-back path,
...
run `recall googlemeet status --json <session-id>` after participants speak
...
utterance but Recall did not see new output bytes reach the Chrome audio
bridge.
...
- Meet microphone and speaker are routed through the virtual audio path used by
  Recall. `doctor` should show `meet output routed: yes` for local Chrome
  realtime joins.
...
```bash
recall googlemeet recover-tab
recall googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
recall googlemeet setup --transport twilio
recall voicecall setup
recall voicecall smoke
```

`voicecall smoke` is readiness-only by default. To dry-run a specific number:

```bash
recall voicecall smoke --to "+15555550123"
```
...
```bash
recall voicecall smoke --to "+15555550123" --yes
```
...
- Run `recall googlemeet doctor <session-id>` to confirm the delegated Twilio
  call ID, whether DTMF was queued, and whether the intro greeting was requested.
- Run `recall voicecall status --call-id <id>` and confirm the call is still
  active.
- Run `recall voicecall tail` and check that Twilio webhooks are arriving at
  the Gateway.
- Run `recall logs --follow` and look for the Twilio Meet sequence: Google
...
- Re-run `recall googlemeet setup --transport twilio`; a green setup check is
...
- If the participant joins but you do not hear the greeting, check
  `recall logs --follow` for the post-DTMF `voicecall.speak` request and
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: Recall owns the
