...
read_when:
  ...
  - You want a reference for all registration methods on SteelEnginePluginApi
...
<Note>
  This page is for plugin authors using `steelengine/plugin-sdk/*` inside
  SteelEngine. For external apps, scripts, dashboards, CI jobs, and IDE extensions
...
prefer `steelengine/plugin-sdk/channel-core`; keep `steelengine/plugin-sdk/core` for
...
`steelengine.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. SteelEngine's
...
<Warning>
  ...
  `steelengine/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`steelengine/plugin-sdk/discord` and `steelengine/plugin-sdk/telegram-account` are
...
The `register(api)` callback receives an `SteelEnginePluginApi` object with these
methods:
...
| `api.registerNodeHostCommand(command)` | Command handled by `steelengine node run`; optional `agentTool` metadata can expose it as an agent-visible tool while the node is connected |
...
agentPromptGuidance: [
  "Global command hint.",
  { text: "Only show this in the main SteelEngine prompt.", surfaces: ["steelengine_main"] },
];
```

Structured `surfaces` may include `steelengine_main`, `codex_app_server`,
`cli_backend`, `acp_backend`, or `subagent`. `pi_main` remains a deprecated alias
for `steelengine_main`. Omit `surfaces` for intentional all-surface guidance. Do
...
| `api.registerNodeCliFeature(registrar, opts?)`  | Node feature CLI under `steelengine nodes`                                |
...
| `api.registerMigrationProvider(provider)`       | Importer for `steelengine migrate`                                        |
...
| `api.registerSecurityAuditCollector(collector)` | Findings collector for `steelengine security audit`                       |
...
- Unauthenticated requesters on a shared-thread harness still see the advertised
  ...
  requester. SteelEngine never falls back to another requester's credentials.
...
state, then return only lines for that run. SteelEngine freezes those lines and
...
Telegram's normal inbound agent path after the handler succeeds. SteelEngine keeps
...
plugin trust boundary; SteelEngine does not treat installed plugins as mutually
...
runtime, for example `["steelengine", "codex"]`. Installed plugins without that
contract, or without explicit enablement, cannot register this middleware; keep
normal SteelEngine plugin hooks for work that does not need pre-model tool-result
timing. The old
...
Gateway on a local discovery transport such as mDNS/Bonjour. SteelEngine calls the
...
`steelengine nodes canvas` explicit plugin-owned node features.
...
- User config still wins. SteelEngine merges `agents.defaults.cliBackends.<id>` over the
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping SteelEngine thinking levels to a native effort
...
- Backends that can disable all native tools for a specific run may declare
  ...
  SteelEngine fails closed if the backend cannot do so.
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `steelengine/plugin-sdk/memory-host-core` instead of reaching into a specific
...
Keep SteelEngine as the source of truth for due checks and execution.
...
| `api.config`             | `SteelEngineConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `steelengine/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when SteelEngine is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through SteelEngine's plugin
...
- **`@steelengine/openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@steelengine/openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `steelengine/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `steelengine/plugin-sdk/speech`, `.../provider-model-shared`, or another
