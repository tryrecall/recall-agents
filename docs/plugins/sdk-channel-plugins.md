---
summary: "Step-by-step guide to building a messaging channel plugin for Recall"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Recall to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects Recall to a
...
<Info>
  If you have not built any Recall plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Recall keeps one
...
`defineChannelMessageAdapter` from `recall/plugin-sdk/channel-message`. The
...
receive/send lifecycle helpers on `recall/plugin-sdk/channel-message`.

Channels migrating inbound authorization can use the experimental
`recall/plugin-sdk/channel-ingress-runtime` subpath from runtime receive
...
Use `recall/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `recall/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `recall/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `recall/plugin-sdk/approval-auth-runtime`
- `recall/plugin-sdk/approval-client-runtime`
- `recall/plugin-sdk/approval-delivery-runtime`
- `recall/plugin-sdk/approval-gateway-runtime`
- `recall/plugin-sdk/approval-handler-adapter-runtime`
- `recall/plugin-sdk/approval-handler-runtime`
- `recall/plugin-sdk/approval-native-runtime`
- `recall/plugin-sdk/approval-reply-runtime`
- `recall/plugin-sdk/channel-runtime-context`

Likewise, prefer `recall/plugin-sdk/setup-runtime`,
`recall/plugin-sdk/setup-runtime`,
`recall/plugin-sdk/reply-runtime`,
`recall/plugin-sdk/reply-dispatch-runtime`,
`recall/plugin-sdk/reply-reference`, and
`recall/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `recall/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `recall/plugin-sdk/setup-runtime` includes the env-aware adapter seam for
  `createEnvPatchedAccountSetupAdapter`
- `recall/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `recall.setupEntry` in
...
- use the broader `recall/plugin-sdk/setup` seam only when you also need the
...
- `recall/plugin-sdk/account-core`,
  `recall/plugin-sdk/account-id`,
  `recall/plugin-sdk/account-resolution`, and
  `recall/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `recall/plugin-sdk/inbound-envelope` and
  `recall/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `recall/plugin-sdk/messaging-targets` for target parsing/matching
- `recall/plugin-sdk/outbound-media` and
  `recall/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `recall/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `recall/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `recall/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `recall/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `recall/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `recall/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`recall/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#recall-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/recall-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "recall": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Recall to Acme Chat."
        }
      }
    }
    ```

    ```json recall.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { RecallConfig } from "recall/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "recall/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Recall
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "recall/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    Recall loads this instead of the full entry when the channel is disabled
    ...
    `recall/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    Recall. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Recall.
...
├── package.json              # recall.channel metadata
├── recall.plugin.json      # Manifest with config schema
