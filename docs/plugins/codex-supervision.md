---
summary: "Browse non-archived native Codex sessions and paginated transcripts across SteelEngine nodes"
title: "Supervise Codex sessions"
sidebarTitle: "Codex supervision"
read_when:
  - You want Codex Desktop or CLI sessions to appear in SteelEngine
...
- A stored or idle local session can create a model-locked SteelEngine Chat from
  ...
  supervised binding prevents SteelEngine from substituting another runtime,
...
- Install the official `@steelengine/codex` plugin on the Gateway. The SteelEngine
  macOS app can install it when you enable Codex features; CLI installations can
  run `steelengine plugins install @steelengine/codex`.
- Install and sign in to Codex Desktop or the Codex CLI on each computer whose
  sessions you want to list.
- Pair remote computers as SteelEngine nodes. Each computer must opt in locally;
...
Guided `steelengine onboard` and macOS first-run setup attempt to install and
...
Enable the `codex` plugin and its supervision capability in `steelengine.json`:
...
sessions visible in both apps without making ordinary SteelEngine turns share
...
Codex clients. SteelEngine does not assume that another client shares the same live
...
The native SteelEngine macOS app reads the same local setting when it advertises
...
`NODE_LIST_FAILED`, compare `steelengine nodes list` and **Settings > Devices**;
...
```bash
steelengine codex sessions [--search <text>] [--host <id>] [--limit <count>] [--cursor <cursor>] [--json] [--url <url>] [--token <token>] [--timeout <ms>] [--expect-final]
steelengine codex continue <thread-id> [--json] [--url <url>] [--token <token>] [--timeout <ms>] [--expect-final]
steelengine codex archive <thread-id> --confirm-no-other-runner [--json] [--url <url>] [--token <token>] [--timeout <ms>] [--expect-final]
```

`steelengine codex sessions` options:
...
SteelEngine creates a normal Chat entry, mirrors bounded user and assistant history
...
connection, SteelEngine starts the canonical `appServer`-source full harness thread
...
has the full SteelEngine harness tool surface. This is a visible-history branch, not
...
rather than another SteelEngine model runtime or the ordinary agent-home harness.
...
last turn, Codex emits its normal model-difference warning. SteelEngine uses the
...
SteelEngine omits model and provider overrides. If the canonical thread is changed
through a separate native Codex control, SteelEngine accepts Codex's persisted
selection. SteelEngine never substitutes its outer model or fallback chain.
...
SteelEngine does not subscribe to or answer approval requests while merely listing
...
clients and the SteelEngine catalog. The canonical branch is stored as a native
...
An active row reported by SteelEngine's App Server cannot start a new branch. Wait
...
other Codex client or SteelEngine runner is using that thread or its spawned
descendants. SteelEngine freshly reads the process-local status, proceeds only for
...
Archive is also blocked when SteelEngine knows that an active binding owns the
exact target thread or any non-archived spawned descendant. SteelEngine follows the
...
`steelengine doctor --fix` moves a retired `codex-supervisor` entry, its endpoint
...
**No sessions appear:** verify that `@steelengine/codex` is installed, both the
...
**Old `codex-supervisor` config remains:** run `steelengine doctor --fix`. Doctor
