...
read_when:
  - You want an SteelEngine agent to join a Microsoft Teams meeting
  - You are configuring Chrome, BlackHole, or SoX for Teams meeting talk-back
title: "Microsoft Teams meetings plugin"
---

The `teams-meetings` plugin joins Microsoft Teams links as a guest in the SteelEngine Chrome profile. It accepts work links under `teams.microsoft.com/l/meetup-join/...` and consumer links under `teams.live.com/meet/...`. It does not create meetings, dial in, call Microsoft Graph, or record meetings.
  ...
  plugins: {
    entries: {
      "teams-meetings": {
        ...
        config: {
          defaultMode: "agent",
          chrome: { guestName: "SteelEngine Agent" },
        },
      },
    },
  },
...
```bash
steelengine teamsmeetings setup
steelengine teamsmeetings join 'https://teams.microsoft.com/l/meetup-join/...'
```
...
| Mode         | Behavior                                                                    |
| ------------ | --------------------------------------------------------------------------- |
| `agent`      | Realtime transcription consults the configured SteelEngine agent; TTS replies. |
...
Teams tenant policy can require sign-in, email verification, or organizer admission. Complete that step in the SteelEngine Chrome profile, then retry status or speech. The plugin does not bypass tenant policy.
