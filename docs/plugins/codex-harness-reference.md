...
By default, Recall starts the managed Codex binary shipped with the bundled
plugin:
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after Recall builds its inherited environment.                                                                                                                                                                                                                   |
| `requestTimeoutMs`                            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                                                                                                                                                                           |
| `turnCompletionIdleTimeoutMs`                 | `60000`                                                | Quiet window after Codex accepts a turn or after a turn-scoped app-server request while Recall waits for `turn/completed`.                                                                                                                                                                                                                          |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active Recall sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows Recall sandbox egress.                                                                                                                                             |
...
unattended Recall turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, Recall treats the implicit default as guardian
...
When an Recall sandbox is active, the local Codex app-server process still
runs on the Gateway host. Recall therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the Recall sandbox
backend. Shell access is exposed through Recall sandbox-backed dynamic tools
...
`workspace-write` without active Recall sandboxing. If you see
`bwrap: setting up uid map: Permission denied` or
`bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted`, run
`recall doctor` and fix the reported host namespace policy for the Recall
...
The stable default is fail-closed: active Recall sandboxing disables native
...
try Codex's remote environment support with Recall's sandbox backend. This
...
When the flag is on and the current Recall session is sandboxed, Recall
...
Recall-owned environment. If the app-server cannot register the environment,
...
loopback exec-server unless it is running on the same host, so Recall rejects
that combination.
...
1. An explicit Recall Codex auth profile for the agent.
...
Stdio app-server launches inherit Recall's process environment by default.
Recall owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's Recall state. That keeps Codex config,
accounts, plugin cache/data, and thread state scoped to the Recall agent
instead of leaking in from the operator's personal `~/.codex` home.

Recall does not rewrite `HOME` for normal local app-server launches. Codex-run
subprocesses such as `recall`, `gh`, `git`, cloud CLIs, and shell commands see
...
Recall plugins and Recall skill snapshots still flow through Recall's own
...
part of an Recall agent, inventory them explicitly:

```bash
recall migrate codex --dry-run
recall migrate apply codex --yes
```
...
Most remaining Recall integration tools, such as messaging, media, cron,
browser, nodes, gateway, `heartbeat_respond`, and `web_search`, are available
through Codex tool search under the `recall` namespace. This keeps the initial
...
surface, while explicit Recall or ACP delegation is still available through
the `recall` dynamic tool namespace.
...
Dynamic tool budgets are capped at 600000 ms. On timeout, Recall aborts the
...
Codex then goes quiet without `turn/completed`, Recall best-effort interrupts
...
availability is owned by Codex app-server, so the list can change when Recall
...
If discovery fails or times out, Recall uses a bundled fallback catalog for:
...
Codex handles `AGENTS.md` itself through native project-doc discovery. Recall
...
For Recall workspace parity, the Codex harness resolves the other bootstrap
files. `SOUL.md`, `IDENTITY.md`, `TOOLS.md`, and `USER.md` are forwarded as
Recall Codex developer instructions because they define the active agent,
...
forwarded as Recall turn input reference context.
