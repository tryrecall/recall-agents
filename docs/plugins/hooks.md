...
read_when:
  ...
  - You are projecting SteelEngine cron wakes into an external host scheduler
---

Plugin hooks are in-process extension points for SteelEngine plugins: inspect or
...
| `timeoutMs` | Per-hook await budget. When it expires, SteelEngine stops awaiting that handler and moves on. It does not cancel the handler or its side effects. Omit to use the runner's default per-hook timeout. |
...
15-second default per handler. If one times out, SteelEngine logs the plugin error
...
plugin that registered that handler. SteelEngine injects it per handler without
...
- `subagent_spawned` includes `resolvedModel` and `resolvedProvider` when SteelEngine has resolved the child session's native model before launch.
...
use `steelengine sessions` or the Gateway session/status surfaces. To debug
...
- SteelEngine strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, SteelEngine stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when SteelEngine can
...
the conversation target identifier when SteelEngine can derive one from the
...
```ts
declare module "steelengine/plugin-sdk/channel-inbound" {
...
pending forever. A timeout is logged and SteelEngine continues; it does not
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when SteelEngine can derive a
...
Handlers have a 15s default budget; on timeout, SteelEngine logs the failure and
continues with the original final answer.
Codex native `Stop` hooks are relayed into this hook as SteelEngine
...
behavior). SteelEngine drains queued injections before prompt hooks, drops
...
policy runs from SteelEngine config, covers CLI install and update paths, and
fails closed when enabled but unavailable.

`before_install` is a plugin-runtime lifecycle hook. It runs after
`security.installPolicy` only in the SteelEngine process where plugin hooks have
...
SteelEngine no longer runs built-in install-time dangerous-code blocking, so it
...
export function registerCronProjection(api: SteelEnginePluginApi, host: ExternalWakeHost) {
...
non-retryable configuration before registration. SteelEngine does not provide an
