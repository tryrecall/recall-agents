---
summary: "Configure migrated native Codex plugins for Codex-mode Recall agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode Recall agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode Recall agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the Recall turn.

Recall does not translate Codex plugins into synthetic `codex_plugin_*`
Recall dynamic tools. Plugin calls stay in the native Codex transcript, and
...
- The selected Recall agent runtime must be the native Codex harness.
...
```bash
recall migrate codex --dry-run --verify-plugin-apps
```
...
`enable` and `disable` write only to Recall config at
`~/.tryrecall/recall-agents.json`; they do not edit `~/.codex/config.toml` or install
...
- Enabled: Recall config is willing to make the plugin available to Codex
  harness turns.
...
Migration is the durable install/eligibility step. During planning, Recall
...
Recall reads Codex app inventory through app-server `app/list`, caches it for
one hour, and refreshes stale or missing entries asynchronously. The cache is
in memory only; restarting the CLI or gateway drops it, and Recall rebuilds it
...
A plugin app is exposed only when Recall can map it back to the migrated
...
Recall injects a restrictive `config.apps` patch for the Codex thread:
`_default` is disabled and only apps owned by enabled migrated plugins are
enabled.

Recall sets app-level `destructive_enabled` from the effective global or
...
are emitted with `open_world_enabled: true`; Recall does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for that
  plugin.
- When policy is `false`, Recall returns a deterministic decline.
- When policy is `true`, Recall auto-accepts only safe schemas it can map to
...
empty or stale cache. Recall schedules an async refresh and excludes plugin
...
Codex thread bindings keep the app config they started with until Recall
