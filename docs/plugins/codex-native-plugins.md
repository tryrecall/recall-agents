---
summary: "Configure native Codex plugins for Codex-mode SteelEngine agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode SteelEngine agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are configuring an existing workspace-directory Codex plugin
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode SteelEngine agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the SteelEngine turn. Plugin calls stay in the native Codex transcript;
Codex app-server owns app-backed MCP execution. SteelEngine does not translate
Codex plugins into synthetic `codex_plugin_*` SteelEngine dynamic tools.
...
`codexPlugins` has no effect on SteelEngine-provider runs, ACP conversation
...
```bash
steelengine migrate codex --dry-run --verify-plugin-apps
```
...
SteelEngine does not call `plugin/install` or start authentication for a
`workspace-directory` plugin. Install, enable, and authenticate it in Codex
before adding or enabling the SteelEngine policy. SteelEngine keeps apps hidden when
...
SteelEngine reports `marketplace_missing` for each enabled workspace plugin and
...
`enable`/`disable` write only to `~/.steelengine/steelengine.json`; they never edit
...
`auth_required`, reauthorize the app in Codex before enabling it in SteelEngine.
For a `workspace-directory` entry, enabling it here changes only SteelEngine
...
| Enabled    | Codex reports the plugin enabled, and SteelEngine config allows it for Codex harness turns.                                           |
...
- During planning, SteelEngine reads source Codex `plugin/read` details and
...
For `workspace-directory` plugins, setup happens outside SteelEngine. SteelEngine
...
disabled, inaccessible, or unauthenticated plugin exposes no apps; SteelEngine
...
SteelEngine reads Codex app inventory through app-server `app/list`, caches it
...
drops it, and SteelEngine rebuilds it from the next `app/list` read.
...
A plugin app is exposed only when SteelEngine can map it back to the configured
...
SteelEngine injects a restrictive `config.apps` patch for the Codex thread:
...
get `open_world_enabled: true`. SteelEngine does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for
  that plugin.
- `false`: SteelEngine returns a deterministic decline.
- `true`: SteelEngine auto-accepts only safe schemas it can map to an approval
  response, such as a boolean approve field.
- `"auto"`: SteelEngine exposes destructive plugin actions to Codex, then
  turns ownership-proven MCP approval elicitations into SteelEngine plugin
  approvals before returning the Codex approval response.
- `"ask"`: SteelEngine uses the same Codex write/destructive gating as
  ...
  admitted app using `"ask"`, SteelEngine selects Codex's human approvals
  reviewer for that app so Codex sends its approval elicitations to
  SteelEngine; other apps and non-app thread approvals keep their configured
  reviewer and policy.
...
| Code                                              | Meaning                                                                                                                              | Fix                                                                                                                    |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| `auth_required`                                   | Migration installed the plugin, but one of its apps still needs authentication. The entry is written disabled until you reauthorize. | Reauthorize the app in Codex, then enable the plugin in SteelEngine.                                                      |
...
| `plugin_detail_unavailable`                       | SteelEngine could not read plugin ownership details.                                                                                    | Inspect the target app-server's `plugin/list` and `plugin/read` responses.                                             |
...
| `app_inventory_missing`, `app_inventory_stale`    | App readiness came from an empty or stale cache.                                                                                     | SteelEngine schedules an async refresh automatically; plugin apps stay excluded until ownership and readiness are known.  |
...
account. SteelEngine can enable an accessible app for the thread even when the
...
`/new` or `/reset`. SteelEngine does not repair or authenticate workspace plugins.
...
`remotePluginId`; SteelEngine keeps owned apps hidden when that selector or the
...
not already active; install, enable, and authenticate it outside SteelEngine.
...
Codex thread bindings keep the app config they started with until SteelEngine
