...
when Recall loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, Recall will use a
...
**Import:** `recall/plugin-sdk/tool-plugin`
...
schemas, wraps plain return values in the Recall tool-result format, and
exposes static metadata that `recall plugins build` writes into the plugin
manifest.
...
- `configSchema` is optional. When omitted, Recall uses a strict empty object
...
- Tool names are static. `recall plugins build` derives `contracts.tools`
...
- Runtime loading stays strict. Installed plugins still need
  `recall.plugin.json` and `package.json` `recall.extensions`; Recall does
  not execute plugin code to infer missing manifest data.

## `definePluginEntry`

**Import:** `recall/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | -                   |
| `configSchema` | `RecallPluginConfigSchema \| () => RecallPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: RecallPluginApi) => void`                               | Yes      | -                   |

- `id` must match your `recall.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- Recall resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `recall/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: RecallPluginApi) => void`                               | No       | -                   |
| `registerFull`        | `(api: RecallPluginApi) => void`                               | No       | -                   |
...
- Discovery registration is non-activating, not import-free. Recall may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and Recall
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `recall nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; Recall resolves it to the
...
Recall loads this instead of the full entry when a channel is disabled,
...
- `recall/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
...
- `recall/plugin-sdk/channel-setup` for optional-install setup surfaces
- `recall/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`recall/plugin-sdk/channel-entry-contract` instead. That contract lets the
setup entry keep setup-safe plugin/secrets exports while still exposing a
runtime setter:

```typescript
import { defineBundledChannelSetupEntry } from "recall/plugin-sdk/channel-entry-contract";
...
the plugin entry and the channel plugin object so Recall can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Recall to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; Recall rejects descriptor names outside
...
Recall classifies loaded plugins by their registration behavior:
...
Use `recall plugins inspect <id>` to see a plugin's shape.
