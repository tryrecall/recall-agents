...
Installed plugins point `package.json` `steelengine` fields at both source and
built entries:
...
- If a `runtimeExtensions`/`runtimeSetupEntry` artifact is declared but
  missing, install/discovery fails with a packaging error; SteelEngine does not
  silently fall back to source. Source fallback (below) only applies when no
  runtime entry is declared at all.
- If an installed package declares only a TypeScript source entry, SteelEngine
...
**Import:** `steelengine/plugin-sdk/tool-plugin`
...
the SteelEngine tool-result format, and exposes static metadata that
`steelengine plugins build` writes into the plugin manifest (`contracts.tools`,
`configSchema`).
...
- For custom tool results, `steelengine/plugin-sdk/tool-results` exports
  `textResult` and `jsonResult`.
- Tool names are static, so `steelengine plugins build` derives
...
- Runtime loading stays strict: installed plugins still need
  `steelengine.plugin.json` and `package.json` `steelengine.extensions`. SteelEngine
  never executes plugin code to infer missing manifest data.

## `definePluginEntry`

**Import:** `steelengine/plugin-sdk/plugin-entry`
...
| `configSchema`            | `SteelEnginePluginConfigSchema \| () => SteelEnginePluginConfigSchema` | No       | Empty object schema |
| `reload`                  | `SteelEnginePluginReloadRegistration`                               | No       | -                   |
| `nodeHostCommands`        | `SteelEnginePluginNodeHostCommand[]`                                | No       | -                   |
| `securityAuditCollectors` | `SteelEnginePluginSecurityAuditCollector[]`                         | No       | -                   |
| `register`                | `(api: SteelEnginePluginApi) => void`                               | Yes      | -                   |

- `id` must match your `steelengine.plugin.json` manifest.
- External session catalogs use
  `steelengine/plugin-sdk/session-catalog` and
...
- `kind` is deprecated: declare an exclusive slot (`"memory"` or
  `"context-engine"`) in the `steelengine.plugin.json` manifest `kind` field
  instead. Runtime-entry `kind` remains only as a compatibility fallback for
  older plugins.
- `configSchema` can be a function for lazy evaluation. SteelEngine resolves and
...
- A `nodeHostCommands` descriptor can define `isAvailable({ config, env })`.
  ...
  node's Gateway declaration. SteelEngine evaluates it against the node-local
  startup config; command handlers should still validate availability when
  invoked.

## `defineChannelPluginEntry`

**Import:** `steelengine/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: SteelEnginePluginApi) => void`                               | No       | -                   |
| `registerFull`        | `(api: SteelEnginePluginApi) => void`                               | No       | -                   |
...
- `registerFull` runs only for `"full"` and `"tool-discovery"`. For
  `"tool-discovery"` it runs _instead of_ channel registration: SteelEngine
  ...
  channel setup.
- Discovery registration is non-activating, not import-free: SteelEngine may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory; SteelEngine
...
- Use `api.registerCli(..., { descriptors: [...] })` for plugin-owned root
  ...
  underscore, starting with a letter or number; SteelEngine rejects other
...
- Use `api.registerNodeCliFeature(...)` for paired-node feature commands so
  they land under `steelengine nodes` (equivalent to
  `registerCli(registrar, { parentPath: ["nodes"], ... })`).
- For other nested plugin commands, add `parentPath` and register commands
  on the `program` object passed to the registrar; SteelEngine resolves it to
...
SteelEngine loads this instead of the full entry when a channel is disabled,
...
| Import                              | Use for                                                                                                                                                                            |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `steelengine/plugin-sdk/setup-runtime` | Runtime-safe setup helpers: `createSetupTranslator`, import-safe setup patch adapters, lookup-note output, `promptResolvedAllowFrom`, `splitSetupEntries`, delegated setup proxies |
| `steelengine/plugin-sdk/channel-setup` | Optional-install setup surfaces                                                                                                                                                    |
| `steelengine/plugin-sdk/setup-tools`   | Setup/install CLI, archive, and docs helpers                                                                                                                                       |
...
`steelengine/plugin-sdk/channel-entry-contract` instead. It lets the setup
...
SteelEngine namespaces this as `plugin.<plugin-id>.changed`. Event names are one
...
evaluate the plugin entry and the channel plugin object so SteelEngine can
...
SteelEngine classifies loaded plugins by their registration behavior:
...
Use `steelengine plugins inspect <id>` to see a plugin's shape.
