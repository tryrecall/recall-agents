---
summary: "How Recall installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged Recall installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

Recall keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the Recall
package directory.
...
- SDK/core imports are peer or supplied Recall imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into Recall-owned package roots

Recall owns only the plugin lifecycle:
...
Recall uses stable per-source roots:

- npm packages install under `~/.recall/npm`
- git packages clone under `~/.recall/git`
...
`recall plugins install npm-pack:<path.tgz>` uses that same managed npm root
for a local npm-pack tarball. Recall reads the tarball's npm metadata, adds it
...
npm may hoist transitive dependencies to `~/.recall/npm/node_modules` beside
the plugin package. Recall scans the managed npm root before trusting the
...
publishable lockfile during install, and Recall's managed npm root supports it
through the normal npm install path. Recall-owned publishable plugin packages
...
Recall does not require it for community packages, but npm will respect it
when present.

Recall-owned npm plugin packages can also publish with explicit
...
with `recall.release.bundleRuntimeDependencies: false`; those packages still
...
`recall` package does not bundle its full dependency tree.

Plugins that import `recall/plugin-sdk/*` declare `recall` as a peer
dependency. Recall does not let npm install a separate registry copy of the
...
resolution/materialization for the shared root and Recall reasserts
plugin-local `node_modules/recall` links for installed packages that declare
...
Local plugins are treated as developer-controlled directories. Recall does not
...
recall plugins install <source>
recall doctor --fix
```

`doctor --fix` can clean legacy Recall-generated dependency state and recover
...
Lightweight and core-critical bundled plugins are shipped as part of Recall.
...
In source checkouts, Recall treats the repository as a pnpm monorepo. After
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g recall`        | Built runtime tree inside the package | Recall package and explicit plugin install/update/doctor flows     |
...
| `recall plugins install ...`   | Managed npm/git/ClawHub plugin root   | The plugin install/update flow                                       |

## Legacy cleanup

Older Recall versions generated bundled-plugin dependency roots at startup or
...
`.recall-runtime-deps*` manifests, generated plugin `node_modules`, install
