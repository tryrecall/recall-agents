---
summary: "How SteelEngine installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged SteelEngine installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

SteelEngine handles plugin dependencies at install/update time only. Runtime
loading never runs a package manager, repairs a dependency tree, or mutates
the SteelEngine package directory.
...
- SDK/core imports are peer or supplied SteelEngine imports.
- Local development plugins bring their own already-installed dependencies.
- npm and git plugins install into SteelEngine-owned package roots.

SteelEngine owns only the plugin lifecycle:
...
SteelEngine uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.steelengine/npm/projects/<encoded-package>`.
- git packages clone under `~/.steelengine/git`.
...
```bash
cd ~/.steelengine/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`steelengine plugins install npm-pack:<path.tgz>` uses the same per-plugin npm
project root for a local npm-pack tarball: SteelEngine reads the tarball's npm
...
`~/.steelengine/npm/projects/<encoded-package>` can unblock a temporary
...
`node_modules` beside the plugin package. SteelEngine scans the managed project
...
publishable lockfile during install, and SteelEngine's managed npm project root
supports it through the normal install path. SteelEngine-owned publishable
...
`steelengine.release.publishToNpm: true`. Third-party plugin packages may also
ship a shrinkwrap; SteelEngine does not require one for community packages, but
...
SteelEngine-owned npm plugin packages can also publish with explicit
...
`steelengine.release.bundleRuntimeDependencies: false`; they still ship a
shrinkwrap, but npm resolves runtime dependencies during install instead of
embedding every platform binary in the plugin tarball. The root `steelengine`
package does not bundle its full dependency tree.

Plugins that import `steelengine/plugin-sdk/*` declare `steelengine` as a peer
dependency. SteelEngine does not let npm install a separate registry copy of the
...
resolution/materialization, and SteelEngine reasserts plugin-local
`node_modules/steelengine` links for installed packages that declare the host
...
Local plugins are developer-controlled directories. SteelEngine never runs
...
steelengine plugins install <source>
steelengine doctor --fix
```

`doctor --fix` cleans legacy SteelEngine-generated dependency state and can
...
Lightweight and core-critical bundled plugins ship as part of SteelEngine. They
...
In source checkouts, SteelEngine treats the repository as a pnpm monorepo.
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g steelengine`        | Built runtime tree inside the package | SteelEngine package and explicit plugin install/update/doctor flows     |
...
| `steelengine plugins install ...`   | Managed npm project/git/ClawHub root  | The plugin install/update flow                                       |

## Legacy cleanup

Older SteelEngine versions generated bundled-plugin dependency roots at startup
...
`plugin-runtime-deps` targets, `.steelengine-runtime-deps*` manifests, generated
...
Older npm installs also used a shared `~/.steelengine/npm/node_modules` root.
