---
summary: "Step-by-step guide to building a model provider plugin for SteelEngine"
title: "Building provider plugins"
sidebarTitle: "Provider plugins"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to SteelEngine
  - You need to understand provider auth, catalogs, and runtime hooks
---

Build a provider plugin to add a model provider (LLM) to SteelEngine: a model
...
<Tip>
  Provider plugins add models to SteelEngine's normal inference loop. If the
...
<Steps>
  <Step title="Package and manifest">
    ...
    ```json package.json
    {
      "name": "@myorg/steelengine-acme-ai",
      "version": "1.0.0",
      "type": "module",
      "steelengine": {
        "extensions": ["./index.ts"],
    ...
    `setup.providers[].envVars` lets SteelEngine detect credentials without
    ...
    optional and lets SteelEngine auto-load your provider plugin from shorthand
    model ids like `acme-large` before runtime hooks exist. `steelengine.compat`
    and `steelengine.build` in `package.json` are required for ClawHub
    publishing (`steelengine.compat.pluginApi` and `steelengine.build.steelengineVersion`
    are the two required fields; `minGatewayVersion` falls back to
    `steelengine.install.minHostVersion` when omitted).
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "steelengine/plugin-sdk/provider-auth";
    ...
    calls and response mapping in the plugin; SteelEngine owns the shared row
    shape, source labels, and help rendering.

    That is a working provider. Users can now run
    `steelengine onboard --acme-ai-api-key <key>` and select
    ...
    `steelengine/plugin-sdk/provider-catalog-live-runtime` for the shared fetch
    ...
    putting provider policy in SteelEngine core.
    ...
    metadata and the plugin needs to project rows into SteelEngine model
    definitions itself:
    ...
    If the upstream provider uses different control tokens than SteelEngine, add a
    ...
    SteelEngine parses its own control markers or channel delivery.
    ...
    `buildProvider` is the live catalog path used when SteelEngine can resolve real
    ...
    SteelEngine's `models list --all` display currently executes static catalogs
    ...
    `steelengine/plugin-sdk/provider-onboard`. The narrowest helpers are
    ...
    `steelengine/plugin-sdk/provider-catalog-shared` instead of hardcoding
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    import { buildProviderStreamFamilyHooks } from "steelengine/plugin-sdk/provider-stream";
    ...
    <Accordion title="SDK seams powering the family builders">
      ...
      - `steelengine/plugin-sdk/provider-model-shared` - `ProviderReplayFamily`, `buildProviderReplayFamilyHooks(...)`, and the raw replay builders (`buildOpenAICompatibleReplayPolicy`, `buildAnthropicReplayPolicyForModel`, `buildGoogleGeminiReplayPolicy`, `buildHybridAnthropicOrOpenAIReplayPolicy`). Also exports Gemini replay helpers (`sanitizeGoogleGeminiReplayHistory`, `resolveTaggedReasoningOutputMode`) and endpoint/model helpers (`resolveProviderEndpoint`, `normalizeProviderId`, `normalizeGooglePreviewModelId`).
      - `steelengine/plugin-sdk/provider-stream` - `ProviderStreamFamily`, `buildProviderStreamFamilyHooks(...)`, `composeProviderStreamWrappers(...)`, plus the shared OpenAI/Codex wrappers (`createOpenAIAttributionHeadersWrapper`, `createOpenAIFastModeWrapper`, `createOpenAIServiceTierWrapper`, `createOpenAIResponsesContextManagementWrapper`, `createCodexNativeWebSearchWrapper`), DeepSeek V4 OpenAI-compatible wrapper (`createDeepSeekV4OpenAICompatibleThinkingWrapper`), Anthropic Messages thinking prefill cleanup (`createAnthropicThinkingPrefillPayloadWrapper`), plain-text tool-call compat (`createPlainTextToolCallCompatWrapper`), and shared proxy/provider wrappers (`createOpenRouterWrapper`, `createToolStreamWrapper`, `createMinimaxFastModeWrapper`).
      - `steelengine/plugin-sdk/provider-stream-shared` - lightweight payload and event wrappers for hot provider paths, including `createOpenAICompatibleCompletionsThinkingOffWrapper`, `createPayloadPatchStreamWrapper`, `createPlainTextToolCallCompatWrapper`, `normalizeOpenAICompatibleReasoningPayload(...)`, and `setQwenChatTemplateThinking(...)`.
      - `steelengine/plugin-sdk/provider-tools` - `ProviderToolCompatFamily`, `buildProviderToolCompatFamilyHooks("deepseek" | "gemini" | "openai")`, and underlying provider schema helpers.
      ...
      reasoning output so SteelEngine consumes native thought parts without adding
    ...
    <Tabs>
      ...
      <Tab title="Usage and billing">
        ...
        auth but has no usable usage token, and SteelEngine must skip generic
        API-key/OAuth fallback. Return `null` or `undefined` when the provider did
        not handle the request and SteelEngine should continue with generic fallback.

        Declare the provider id in `contracts.usageProviders`. When that manifest
        contract and **both** hooks are present, SteelEngine automatically includes
        ...
        is not auto-discovered, because SteelEngine cannot resolve its usage credential.
      </Tab>
    </Tabs>

    <Accordion title="Common provider hooks">
      SteelEngine calls hooks in roughly this order for model/provider plugins.
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    web fetch, and web search alongside text inference. SteelEngine classifies this as a
    **hybrid-capability** plugin - the recommended pattern for company plugins
    ...
    <Tabs>
      ...
      <Tab title="Realtime transcription">
        ...
        `steelengine/plugin-sdk/provider-http`. The helper normalizes upload
      ...
      <Tab title="Realtime voice">
        ...
        honor `options.suppressResponse`. SteelEngine then avoids suppression for
        ...
        the flag use SteelEngine's local input-audio fallback detection.
      </Tab>
      <Tab title="Media understanding">
        ...
        SteelEngine still keeps the normal auth gate for providers that do not
...
├── package.json              # steelengine.providers metadata
├── steelengine.plugin.json      # Manifest with provider auth metadata
