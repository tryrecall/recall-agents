...
read_when:
  - You want to place an outbound voice call from Recall
...
Voice calls for Recall via a plugin. Supports outbound notifications,
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    `streaming.*` OpenAI keys are rewritten by `recall doctor --fix`.
    Runtime fallback still accepts the old voice-call keys for now, but
    the rewrite path is `recall doctor --fix` and the compat shim is
    temporary.
...
- Voice Call exposes the shared `openclaw_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal Recall tools.
...
configured Recall agent without paying a full agent-consult round trip on
...
`openclaw_agent_consult` still run the full Recall agent and should be used
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `recall doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
recall voicecall call --to "+15555550123" --message "Hello from Recall"
recall voicecall start --to "+15555550123"   # alias for call
recall voicecall continue --call-id <id> --message "Any questions?"
recall voicecall speak --call-id <id> --message "One moment"
recall voicecall dtmf --call-id <id> --digits "ww123456#"
recall voicecall end --call-id <id>
recall voicecall status --call-id <id>
recall voicecall tail
recall voicecall latency                      # summarize turn latency from logs
...
Provider signatures are checked against the public URL Recall reconstructs
...
Use `recall logs --follow` for the live phase trace. A healthy Twilio Meet
...
`recall voicecall tail` still shows persisted call records; it is useful for
...
- `recall logs --follow` shows realtime TwiML served, the realtime bridge
