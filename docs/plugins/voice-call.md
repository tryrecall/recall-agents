...
read_when:
  - You want to place an outbound voice call from SteelEngine
...
Voice calls for SteelEngine via a plugin: outbound notifications, multi-turn
...
<Steps>
  ...
  <Step title="Verify setup">
    ...
    steelengine voicecall setup --json
    ```
...
| `store`                         | unset        | Overrides the default `$STEELENGINE_STATE_DIR/voice-calls` path (normally `~/.steelengine/voice-calls`). |
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    release (`2026.6.0`), so run `steelengine doctor --fix` to rewrite committed
...
- Voice Call exposes the shared `steelengine_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal SteelEngine tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `steelengine_agent_consult`.
...
configured SteelEngine agent without paying a full agent-consult round trip on
...
`steelengine_agent_consult` still run the full SteelEngine agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call steelengine_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `steelengine doctor --fix`; committed config should use `tts.providers.<provider>`.
...
Provider signatures are checked against the public URL SteelEngine reconstructs
...
Use `steelengine logs --follow` for the live phase trace. A healthy Twilio Meet
...
`steelengine voicecall tail` still shows persisted call records; useful for
...
- `steelengine logs --follow` shows realtime TwiML served, the realtime bridge started, and the initial greeting queued.
