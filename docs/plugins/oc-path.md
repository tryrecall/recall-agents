---
summary: "Bundled `oc-path` plugin: ships the `recall path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`recall path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the Recall repo under
...
- **Local automation**: shell scripts can resolve or update one workspace value
  with `recall path … --json` instead of carrying separate markdown, JSONC,
...
recall path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
recall path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `recall` CLI** on the host where you
...
The plugin metadata lives in `extensions/oc-path/recall.plugin.json`:
...
`recall path …`, so installs that never use the verb pay no cost.
...
state. Bare `recall path` invocations work immediately on the same host —
the CLI loads the plugin on demand.

Disable with:

```bash
recall plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `recall path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__RECALL_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`recall path` CLI reference](/cli/path)
