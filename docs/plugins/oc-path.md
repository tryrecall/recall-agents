---
summary: "Bundled `oc-path` plugin: ships the `steelengine path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`steelengine path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the SteelEngine repo under
...
- **Local automation**: shell scripts resolve or update one workspace value
  with `steelengine path … --json` instead of carrying separate markdown, JSONC,
...
steelengine path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
steelengine path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `steelengine` CLI** on the host where you
...
Plugin metadata lives in `extensions/oc-path/steelengine.plugin.json`:
...
lazily the first time you run `steelengine path …`, so installs that never use
...
state. Bare `steelengine path` invocations work immediately on the same host;
the CLI loads the plugin on demand.

Disable with:

```bash
steelengine plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `steelengine path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__STEELENGINE_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`steelengine path` CLI reference](/cli/path)
