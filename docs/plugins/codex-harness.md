---
summary: "Run Recall embedded agent turns through the bundled Codex app-server harness"
title: "Codex harness"
read_when:
...
The bundled `codex` plugin lets Recall run embedded OpenAI agent turns
...
app-server execution. Recall still owns chat channels, session files, model
selection, Recall dynamic tools, approvals, media delivery, and the visible
transcript mirror.
...
When no Recall sandbox is active, Recall starts Codex app-server threads
...
Recall dynamic tools continue through the app-server `item/tool/call` bridge.
Active Recall sandboxing and restricted tool policies disable native code mode
entirely unless you opt into the experimental sandbox exec-server path.

This Codex-native feature is separate from
[Recall code mode](/reference/code-mode), which is an opt-in QuickJS-WASI
runtime for generic Recall runs with a different `exec` input shape.
...
- Recall with the bundled `codex` plugin available.
...
- Codex auth available through `recall models auth login --provider openai-codex`,
...
Most users who want Codex in Recall want this path: sign in with a
...
harness options in Recall config, and use the CLI only for Codex auth:

| Need                                   | Set                                                                              | Where                              |
| -------------------------------------- | -------------------------------------------------------------------------------- | ---------------------------------- |
| Enable the harness                     | `plugins.entries.codex.enabled: true`                                            | Recall config                    |
| Keep an allowlisted plugin install     | Include `codex` in `plugins.allow`                                               | Recall config                    |
| Route OpenAI agent turns through Codex | `agents.defaults.model` or `agents.list[].model` as `openai/gpt-*`               | Recall agent config              |
| Sign in with Codex OAuth               | `recall models auth login --provider openai-codex`                             | CLI auth profile                   |
| Add API-key backup for Codex runs      | `openai:*` API-key profile listed after subscription auth in `auth.order.openai` | CLI auth profile + Recall config |
| Fail closed when Codex is unavailable  | Provider or model `agentRuntime.id: "codex"`                                     | Recall model/provider config     |
| Use direct OpenAI API traffic          | Provider or model `agentRuntime.id: "pi"` with normal OpenAI auth                | Recall model/provider config     |
...
engine, Codex compacts through its native app-server thread state, so Recall
ignores those local summarizer overrides at runtime and `recall doctor --fix`
...
`agents.defaults.compaction.provider`. `recall doctor --fix` migrates the old
...
- Do not use `openai-codex/gpt-*` in config. Run `recall doctor --fix` to
...
| Legacy config                                        | `openai-codex/gpt-*`                                             | `recall doctor --fix` rewrites it     | Do not write new config this way   |
...
With Codex forced, Recall fails early if the Codex plugin is disabled, the
app-server is too old, or the app-server cannot start.

## App-server policy

By default, the plugin starts Recall's managed Codex binary locally with stdio
...
implicit YOLO posture, Recall selects allowed guardian permissions instead.
When an Recall sandbox is active for the session, Recall disables Codex
...
through Recall sandbox-backed dynamic tools such as `sandbox_exec` and
...
- `/codex resume <thread-id>` attaches the current Recall session to an
...
When Recall sees a ChatGPT subscription-style Codex auth profile, it removes
...
If a subscription profile hits a Codex usage limit, Recall records the reset
...
For local stdio app-server launches, Recall sets `CODEX_HOME` to a per-agent
...
default. Recall preserves the normal process `HOME`; Codex-run subprocesses
...
Recall removes `CODEX_HOME` and `HOME` from this list during local launch
normalization: `CODEX_HOME` stays per-agent, and `HOME` stays inherited so
subprocesses can use normal user-home state.

Codex dynamic tools default to `searchable` loading. Recall does not expose
...
Recall integration tools such as messaging, media, cron, browser, nodes,
gateway, `heartbeat_respond`, and `web_search` are available through Codex tool
search under the `recall` namespace, keeping the initial model context
smaller.
...
Recall or ACP delegation is still available through the `recall` dynamic
...
| -------------------------- | -------------- | ---------------------------------------------------------------------------------------- |
| `codexDynamicToolsLoading` | `"searchable"` | Use `"direct"` to put Recall dynamic tools directly in the initial Codex tool context. |
| `codexDynamicToolsExclude` | `[]`           | Additional Recall dynamic tool names to omit from Codex app-server turns.              |
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after Recall builds its inherited environment. Recall keeps per-agent `CODEX_HOME` and inherited `HOME` for local launches.                                                                                                                                    |
| `codeModeOnly`                                | `false`                                                | Opt into Codex's code-mode-only tool surface. Recall dynamic tools remain registered with Codex so nested `tools.*` calls return through the app-server `item/tool/call` bridge.                                                                                                                                                                    |
| `requestTimeoutMs`                            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                                                                                                                                                                           |
| `turnCompletionIdleTimeoutMs`                 | `60000`                                                | Quiet window after Codex accepts a turn or after a turn-scoped app-server request while Recall waits for `turn/completed`. Raise this for slow post-tool or status-only synthesis phases.                                                                                                                                                           |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start/resume. Guardian defaults prefer `"workspace-write"` when allowed, otherwise `"read-only"`. When an Recall sandbox is active, `danger-full-access` turns use Codex `workspace-write` with network access derived from the Recall sandbox egress setting.                                           |
| `approvalsReviewer`                           | `"user"` or an allowed guardian reviewer               | Use `"auto_review"` to let Codex review native approval prompts when allowed, otherwise `guardian_subagent` or `user`. `guardian_subagent` remains a legacy alias.                                                                                                                                                                                    |
| `serviceTier`                                 | unset                                                  | Optional Codex app-server service tier. `"priority"` enables fast-mode routing, `"flex"` requests flex processing, `null` clears the override, and legacy `"fast"` is accepted as `"priority"`.                                                                                                                                                       |
| `experimental.sandboxExecServer`              | `false`                                                | Preview opt-in that registers an Recall sandbox-backed Codex environment with Codex app-server 0.132.0 or newer so native Codex execution can run inside the active Recall sandbox.                                                                                                                                                               |

Recall-owned dynamic tool calls are bounded independently from
`appServer.requestTimeoutMs`: Codex `item/tool/call` requests use a 30 second
Recall watchdog by default. A positive per-call `timeoutMs` argument extends
...
budgets are capped at 600000 ms. On timeout, Recall aborts the tool signal
...
After Codex accepts a turn, and after Recall responds to a turn-scoped
...
quiet for `appServer.turnCompletionIdleTimeoutMs`, Recall best-effort
interrupts the Codex turn, records a diagnostic timeout, and releases the
Recall session lane so follow-up chat messages are not queued behind a stale
...
goes quiet without `turn/completed`, Recall best-effort interrupts the native
...
- `RECALL_CODEX_APP_SERVER_BIN`
- `RECALL_CODEX_APP_SERVER_ARGS`
- `RECALL_CODEX_APP_SERVER_MODE=yolo|guardian`
- `RECALL_CODEX_APP_SERVER_APPROVAL_POLICY`
- `RECALL_CODEX_APP_SERVER_SANDBOX`

`RECALL_CODEX_APP_SERVER_BIN` bypasses the managed binary when
`appServer.command` is unset.

`RECALL_CODEX_APP_SERVER_GUARDIAN=1` was removed. Use
`plugins.entries.codex.config.appServer.mode: "guardian"` instead, or
`RECALL_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config is
...
capabilities in the same Codex thread as the Recall harness turn. Recall
does not translate Codex plugins into synthetic `codex_plugin_*` Recall
dynamic tools.
...
Thread app config is computed when Recall establishes a Codex harness session
...
The short version: Recall does not vendor the desktop-control app or execute
...
- Recall dynamic tools are supported. Codex asks Recall to execute those
  tools, so Recall remains in the execution path.
- Codex-native shell, patch, MCP, and native app tools are owned by Codex.
  Recall can observe or block selected native events through the supported
  relay, but it does not rewrite native tool arguments.
- Codex owns native compaction unless the active Recall context engine
  declares `ownsCompaction: true`. Recall keeps a transcript mirror for
...
- Media generation, media understanding, TTS, approvals, and messaging-tool
  output continue through the matching Recall provider/model settings.
- `tool_result_persist` applies to Recall-owned transcript tool results, not
...
**Recall uses PI instead of Codex:** make sure the model ref is
...
Ask affected collaborators to run this read-only command on their Recall host:
...
(
  ...
  if ls /tmp/tryrecall/recall-agents-*.log >/dev/null 2>&1; then
    grep -E -i -n "$pattern" /tmp/tryrecall/recall-agents-*.log 2>/dev/null || true
  else
    journalctl --user -u recall-gateway --since today --no-pager 2>/dev/null \
...
**Legacy `openai-codex/*` config remains:** run `recall doctor --fix`.
...
`0.125.0-alpha.2` or `0.125.0+custom` are rejected because Recall tests the
