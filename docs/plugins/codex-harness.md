---
summary: "Run SteelEngine embedded agent turns through the official Codex app-server harness"
title: "Codex harness"
read_when:
  - You want to use the official Codex app-server harness
  - You need Codex harness config examples
  - You want Codex-only deployments to fail instead of falling back to SteelEngine
---

The official `codex` plugin runs embedded OpenAI agent turns through Codex
app-server instead of the built-in SteelEngine harness. Codex owns the
low-level agent session: native thread resume, native tool continuation,
native compaction, and app-server execution. SteelEngine still owns chat
channels, session files, model selection, SteelEngine dynamic tools, approvals,
...
repaired by `steelengine doctor --fix`.
...
If Codex owns auth before Platform versus ChatGPT routing is known, SteelEngine
...
When no SteelEngine sandbox is active, SteelEngine starts Codex app-server threads
with Codex native code mode enabled (code-mode-only stays off by default), so
native workspace/code capabilities remain available alongside SteelEngine
dynamic tools routed through the app-server `item/tool/call` bridge. An
active SteelEngine sandbox or restricted tool policy disables native code mode
entirely unless you opt into the experimental sandbox exec-server path.

With the default `tools.exec.host: "auto"` and no active SteelEngine sandbox,
...
name or id and keeps SteelEngine's node approval policy in force. If a finite
runtime allowlist disables native Code Mode and leaves the turn without an
execution environment, SteelEngine keeps its policy-filtered `exec` and `process`
tools available instead for direct, unsandboxed execution.

This Codex-native feature is separate from
[SteelEngine Code Mode](/tools/code-mode), an opt-in QuickJS-WASI runtime
for generic SteelEngine runs with a different `exec` input shape. For the
...
- The official `@steelengine/codex` plugin installed. Include `codex` in
...
- A stable Codex app-server from `0.143.0` through `0.144.6`. The plugin manages a compatible
  ...
  startup.
- Codex auth through `steelengine models auth login --provider openai`, an
...
```bash
steelengine plugins install @steelengine/codex
...
The default `appServer.homeScope: "agent"` isolates each SteelEngine agent from
...
that home's native Codex auth, config, plugins, and thread store. SteelEngine does
not inject an SteelEngine auth profile into this app-server.
...
SteelEngine; the fork attaches to the current SteelEngine session and stays
...
`sessionCatalog.enabled: false` to remove it from the SteelEngine sidebar without
...
native configuration; the outer SteelEngine model and fallback chain never replace
...
| Need                                                | Set                                                                                              | Where                              |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------- |
| Enable the harness                                  | `plugins.entries.codex.enabled: true`                                                            | SteelEngine config                    |
| Hide native Codex session discovery                 | `plugins.entries.codex.config.sessionCatalog.enabled: false`                                     | Codex plugin config                |
| Keep an allowlisted plugin install                  | Include `codex` in `plugins.allow`                                                               | SteelEngine config                    |
...
| Sign in with ChatGPT/Codex OAuth                    | `steelengine models auth login --provider openai`                                                   | CLI auth profile                   |
| Add API-key backup for Codex runs                   | `openai:*` API-key profile listed after subscription auth in `auth.order.openai`                 | CLI auth profile + SteelEngine config |
| Fail closed when Codex is unavailable               | Provider or model `agentRuntime.id: "codex"`                                                     | SteelEngine model/provider config     |
| Use direct OpenAI API traffic                       | Provider or model `agentRuntime.id: "steelengine"` with normal OpenAI auth                          | SteelEngine model/provider config     |
...
SteelEngine ignores those local summarizer overrides at runtime, and
`steelengine doctor --fix` removes them when the agent uses Codex.
...
`agents.defaults.compaction.provider`. `steelengine doctor --fix` migrates the
...
compaction on the bound thread. SteelEngine does not wait for completion,
impose an SteelEngine timeout, restart the shared app-server, or fall back to a
...
- With runtime unset or `auto`, only an exact official HTTPS Platform Responses
  ...
  implicitly.
- Do not use legacy Codex GPT refs in config; run `steelengine doctor --fix` to
...
- `agentRuntime.id: "steelengine"` opts a provider or model into the embedded
  SteelEngine runtime when that is intentional.
...
| Direct OpenAI API-key traffic through SteelEngine  | Provider or model `agentRuntime.id: "steelengine"` and normal OpenAI auth                                      | `/status` shows SteelEngine runtime        | Use only when SteelEngine is intentional      |
| Legacy config                                   | legacy Codex GPT refs                                                                                       | `steelengine doctor --fix` rewrites it     | Do not write new config this way           |
...
With Codex forced, SteelEngine fails early if the effective route is not declared
Codex-compatible, the plugin is disabled, the app-server is too old, or the
app-server cannot start.

## App-server policy

By default, the plugin starts SteelEngine's managed Codex binary locally with
...
implicit YOLO posture, SteelEngine selects allowed guardian permissions
instead. When an SteelEngine sandbox is active for the session, SteelEngine
...
Shell access instead goes through SteelEngine sandbox-backed dynamic tools such
as `sandbox_exec` and `sandbox_process` when the normal exec/process tools
are available.

Use normalized SteelEngine exec mode for Codex native auto-review before
...
SteelEngine does not preserve legacy unsafe Codex `approvalPolicy: "never"` or
...
works, but `tools.exec.mode: "auto"` is the normalized SteelEngine surface.
...
supports SteelEngine text commands.
...
- `/codex goal` reads or updates the attached thread's native Codex goal. Codex automatic goal continuation stays disabled; SteelEngine does not own autonomous follow-on turns yet.
- `/codex resume <thread-id>` attaches the current SteelEngine session to an
...
When SteelEngine sees a ChatGPT subscription-style Codex auth profile, it
...
If a subscription profile hits a Codex usage limit, SteelEngine records the
...
When native Codex plugins are configured, SteelEngine installs or refreshes
...
ids, accessibility, and metadata, but SteelEngine owns the per-thread
enablement decision: if policy allows a listed accessible app, SteelEngine
...
installation for unknown ids; SteelEngine only activates marketplace plugins
with `plugin/install` and then refreshes inventory.

### Environment isolation

For local stdio app-server launches, SteelEngine sets `CODEX_HOME` to a
...
`~/.codex` by default. SteelEngine preserves the normal process `HOME`;
...
`appServer.homeScope: "user"`, SteelEngine instead uses the native user Codex
home and its existing account without injecting an SteelEngine auth profile.
...
process. SteelEngine removes `CODEX_HOME` and `HOME` from this list during
...
Codex dynamic tools default to `searchable` loading. SteelEngine normally does
...
so SteelEngine does not project a second goal store into Codex turns. Most
remaining SteelEngine integration tools, such as messaging, media, cron,
browser, nodes, gateway, and `heartbeat_respond`, are available through
Codex tool search under the `steelengine` namespace, keeping the initial model
...
Tools marked `catalogMode: "direct-only"`, including the SteelEngine `computer`
tool, use the `steelengine_direct` namespace instead. Codex treats that namespace
...
SteelEngine's managed `web_search` dynamic tool are mutually exclusive so
managed search cannot bypass native domain restrictions. SteelEngine uses the
...
while explicit SteelEngine or ACP delegation is still available through the
`steelengine` dynamic tool namespace. Heartbeat collaboration instructions
...
| -------------------------- | -------------- | ---------------------------------------------------------------------------------------- |
| `codexDynamicToolsLoading` | `"searchable"` | Use `"direct"` to put SteelEngine dynamic tools directly in the initial Codex tool context. |
| `codexDynamicToolsExclude` | `[]`           | Additional SteelEngine dynamic tool names to omit from Codex app-server turns.              |
...
| `homeScope`                                   | `"agent"`                                              | `"agent"` isolates ordinary harness state per SteelEngine agent. `"user"` is an explicit opt-in that shares the native `$CODEX_HOME` or `~/.codex`, uses native auth, and enables owner-only thread management. User scope supports local stdio or Unix transport. For the separate supervision connection, an unset value resolves to `"user"` for stdio or Unix and `"agent"` for WebSocket.     |
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after SteelEngine builds its inherited environment. SteelEngine keeps the selected `CODEX_HOME` and inherited `HOME` for local launches.                                                                                                                                                                           |
| `codeModeOnly`                                | `false`                                                | Opt into Codex's code-mode-only tool surface. Ordinary SteelEngine dynamic tools remain available through nested `tools.*` calls; `steelengine_direct` tools stay directly model-visible.                                                                                                                                                                                                             |
| `remoteWorkspaceRoot`                         | unset                                                  | Remote Codex app-server workspace root. When set, SteelEngine infers the local workspace root from the resolved SteelEngine workspace, preserves the current cwd suffix under this remote root, and sends only the final app-server cwd to Codex. If the cwd is outside the resolved SteelEngine workspace root, SteelEngine fails closed instead of sending a gateway-local path to the remote app-server. |
| `requestTimeoutMs`                            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                                                                                                                                                                                                                     |
| `turnCompletionIdleTimeoutMs`                 | `60000`                                                | Quiet window after Codex accepts a turn or after a turn-scoped app-server request while SteelEngine waits for `turn/completed`.                                                                                                                                                                                                                                                                    |
| `turnAssistantCompletionIdleTimeoutMs`        | `10000`                                                | Quiet window after a final/non-commentary assistant item or pre-tool raw assistant completion arms the assistant-output release while SteelEngine still waits for `turn/completed`. Raising it gives Codex more time to emit `turn/completed` before SteelEngine interrupts and releases the session lane.                                                                                            |
| `postToolRawAssistantCompletionIdleTimeoutMs` | `300000`                                               | Completion-idle and progress guard used after a tool handoff, native tool completion, post-tool raw assistant progress, raw reasoning completion, or reasoning progress while SteelEngine waits for `turn/completed`. Use this for trusted or heavy workloads where post-tool synthesis can legitimately stay quiet longer than the final assistant release budget.                                |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start/resume. Guardian defaults prefer `"workspace-write"` when allowed, otherwise `"read-only"`. When an SteelEngine sandbox is active, `danger-full-access` turns use Codex `workspace-write` with network access derived from the SteelEngine sandbox egress setting.                                                                                     |
| `approvalsReviewer`                           | `"user"` or an allowed guardian reviewer               | Use `"auto_review"` to let Codex review native approval prompts when allowed, otherwise `guardian_subagent` or `user`. `guardian_subagent` remains a legacy alias.                                                                                                                                                                                                                              |
| `serviceTier`                                 | unset                                                  | Optional Codex app-server service tier. `"priority"` enables fast-mode routing, `"flex"` requests flex processing, `null` clears the override, and legacy `"fast"` is accepted as `"priority"`.                                                                                                                                                                                                 |
| `networkProxy`                                | disabled                                               | Opt into Codex permissions-profile networking for app-server commands. SteelEngine defines the selected `permissions.<profile>.network` config and selects it with `default_permissions` instead of sending `sandbox`.                                                                                                                                                                             |
| `experimental.sandboxExecServer`              | `false`                                                | Preview opt-in that registers an SteelEngine sandbox-backed Codex environment with the supported Codex app-server so native Codex execution can run inside the active SteelEngine sandbox.                                                                                                                                                                                                            |

`appServer.networkProxy` is explicit because it changes the Codex sandbox
contract. When enabled, SteelEngine also sets `features.network_proxy.enabled`
and `default_permissions` in the Codex thread config so the generated
permission profile can start Codex managed networking. By default, SteelEngine
generates a collision-resistant `steelengine-network-<fingerprint>` profile
...
SteelEngine-owned dynamic tool calls are bounded independently from
`appServer.requestTimeoutMs`: Codex `item/tool/call` requests use a 90
second SteelEngine watchdog by default. A positive per-call `timeoutMs`
...
reduced by earlier preparation work. On timeout, SteelEngine aborts the tool
...
After Codex accepts a turn, and after SteelEngine responds to a turn-scoped
...
app-server goes quiet for `appServer.turnCompletionIdleTimeoutMs`, SteelEngine
best-effort interrupts the Codex turn, records a diagnostic timeout, and
releases the SteelEngine session lane so follow-up chat messages are not
...
Tool handoffs use a longer post-tool idle budget: after SteelEngine returns an
...
quiet without `turn/completed`, SteelEngine best-effort interrupts the native
turn and releases the session lane. If another turn watch wins that release
race, SteelEngine still accepts the completed final assistant item once no
...
stuck app-server client and release the SteelEngine session lane; they also
...
- `STEELENGINE_CODEX_APP_SERVER_BIN` bypasses the managed binary when
  `appServer.command` is unset.
- `STEELENGINE_CODEX_APP_SERVER_ARGS`
- `STEELENGINE_CODEX_APP_SERVER_MODE=yolo|guardian`
- `STEELENGINE_CODEX_APP_SERVER_APPROVAL_POLICY`
- `STEELENGINE_CODEX_APP_SERVER_SANDBOX`

`STEELENGINE_CODEX_APP_SERVER_GUARDIAN=1` was removed. Use
`plugins.entries.codex.config.appServer.mode: "guardian"` instead, or
`STEELENGINE_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config
...
capabilities in the same Codex thread as the SteelEngine harness turn. SteelEngine
does not translate Codex plugins into synthetic `codex_plugin_*` SteelEngine
dynamic tools.
...
Thread app config is computed when SteelEngine establishes a Codex harness
...
Short version: SteelEngine does not vendor the desktop-control app or execute
...
- SteelEngine dynamic tools are supported. Codex asks SteelEngine to execute
  those tools, so SteelEngine remains in the execution path.
- Codex-native shell, patch, MCP, and native app tools are owned by Codex.
  SteelEngine can observe or block selected native events through the
  supported relay, but it does not rewrite native tool arguments.
- Codex owns native compaction. SteelEngine keeps a transcript mirror for
  channel history, search, `/new`, `/reset`, and future model or harness
  switching, but does not replace Codex compaction with an SteelEngine or
  context-engine summarizer.
- Media generation, media understanding, TTS, approvals, and messaging-tool
  output continue through the matching SteelEngine provider/model settings.
- `tool_result_persist` applies to SteelEngine-owned transcript tool results,
...
**SteelEngine uses the built-in harness instead of Codex:** confirm the effective
...
(
  ...
  if ls /tmp/steelengine/steelengine-*.log >/dev/null 2>&1; then
    grep -E -i -n "$pattern" /tmp/steelengine/steelengine-*.log 2>/dev/null || true
  else
    journalctl --user -u steelengine-gateway --since today --no-pager 2>/dev/null \
...
**Legacy Codex model refs config remains:** run `steelengine doctor --fix`.
...
unvalidated releases are rejected because SteelEngine validates generated schemas
...
id that SteelEngine no longer has registered. This is a native Codex hook
...
SteelEngine Gateway so old threads are dropped and native hook registrations
are recreated.
...
used for SteelEngine loop detection and its no-policy marker. Required
