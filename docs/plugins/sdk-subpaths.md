...
`recall/plugin-sdk/`. This page catalogs the commonly used subpaths grouped by
...
| `plugin-sdk/config-schema`     | `RecallSchema`                                                                                                                                                       |
...
These subpaths remain package exports for older plugins and Recall test suites,
...
These broad re-export barrels remain buildable for Recall source and
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `recall.json` Zod schema export (`RecallSchema`) |
    ...
    | `plugin-sdk/bundled-channel-config-schema` | Bundled Recall channel config schemas for maintained bundled plugins only |
    ...
    | `plugin-sdk/discord` | Deprecated Discord compatibility facade for published `@recall/discord@2026.3.13` and tracked owner compatibility; new plugins should use generic channel SDK subpaths |
  ...
  <Accordion title="Provider subpaths">
    ...
    | `plugin-sdk/provider-auth` | `createProviderApiKeyAuthMethod`, `ensureApiKeyFromOptionEnvOrPrompt`, `upsertAuthProfile`, `upsertApiKeyProfile`, `writeOAuthCredentials`, deprecated `resolveRecallAgentDir` compatibility export |
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/config-contracts` | Focused type-only config surface for plugin config shapes such as `RecallConfig` and channel/provider config types |
    ...
    | `plugin-sdk/agent-harness` | Experimental trusted-plugin surface for low-level agent harnesses: harness types, active-run steer/abort helpers, Recall tool bridge helpers, runtime-plan tool policy helpers, terminal outcome classification, tool progress formatting/detail helpers, and attempt result utilities |
    ...
    | `plugin-sdk/agent-runtime` | Agent dir/identity/workspace helpers, including `resolveAgentDir`, `resolveDefaultAgentDir`, and deprecated `resolveRecallAgentDir` compatibility export |
  ...
  <Accordion title="Capability and testing subpaths">
    | Subpath | Key exports |
    | --- | --- |
    | `plugin-sdk/media-runtime` | Shared media fetch/transform/store helpers including `saveRemoteMedia`, `saveResponseMedia`, `readRemoteMediaBuffer`, and deprecated `fetchRemoteMedia`; prefer store helpers before buffer reads when a URL should become Recall media |
    ...
    | `plugin-sdk/testing` | Repo-local deprecated compatibility barrel for legacy Recall tests. New repo tests should import focused local test subpaths such as `plugin-sdk/agent-runtime-test-contracts`, `plugin-sdk/plugin-test-runtime`, `plugin-sdk/channel-test-helpers`, `plugin-sdk/test-env`, or `plugin-sdk/test-fixtures` instead |
  ...
  <Accordion title="Reserved bundled-helper subpaths">
    ...
    | `plugin-sdk/codex-native-task-runtime` | Bundled Codex plugin helper for mirroring Codex app-server native subagents into Recall task state |
