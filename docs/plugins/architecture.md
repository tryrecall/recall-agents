...
This page covers the internal architecture of the Recall plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Recall. Every
native Recall plugin registers against one or more capability types:
...
Recall classifies every loaded plugin into a shape based on its actual
...
Use `recall plugins inspect <id>` to see a plugin's shape and capability
...
When you run `recall doctor` or `recall plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `recall status --all` and `recall plugins doctor`.
...
1. **Manifest + discovery**
   Recall finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `recall.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Recall plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Recall reads the registry to expose tools, channels, provider
...
That split lets Recall validate config, explain missing/disabled plugins, and
...
normal chat actions. Recall keeps one shared `message` tool in core, and
...
Recall treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Recall-facing
  surfaces
...
So if Recall adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Recall has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { RecallPluginDefinition } from "recall/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "recall/plugin-sdk";
...
Recall already treats image/audio/video understanding as one shared
...
If Recall adds a new domain later, such as video generation, use the same
...
`RecallPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Recall can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Recall knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `RecallPluginApi` or
  `api.runtime`
...
Native Recall plugins run **in-process** with the Gateway. They are not
...
Compatible bundles are safer by default because Recall currently treats them
...
name: `@recall/<id>` by default, or an approved typed suffix such as
...
At startup, Recall does roughly this:
...
The manifest is the control-plane source of truth. Recall uses it to:
...
Recall keeps short in-process caches for:
...
- Set `RECALL_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `RECALL_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `RECALL_PLUGIN_DISCOVERY_CACHE_MS` and
  `RECALL_PLUGIN_MANIFEST_CACHE_MS`.
...
Recall still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Recall calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | Recall tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on Recall's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Recall's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Recall adds those
...
- If Recall adds a new capability such as video generation later, define the
...
- Recall only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Recall plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `recall/plugin-sdk` import when
authoring plugins:

- `recall/plugin-sdk/plugin-entry` for plugin registration primitives.
- `recall/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `recall/plugin-sdk/channel-setup`,
  `recall/plugin-sdk/channel-pairing`,
  `recall/plugin-sdk/channel-contract`,
  `recall/plugin-sdk/channel-feedback`,
  `recall/plugin-sdk/channel-inbound`,
  `recall/plugin-sdk/channel-lifecycle`,
  `recall/plugin-sdk/channel-reply-pipeline`,
  `recall/plugin-sdk/command-auth`,
  `recall/plugin-sdk/secret-input`, and
  `recall/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `recall/plugin-sdk/channel-config-helpers`,
  `recall/plugin-sdk/allow-from`,
  `recall/plugin-sdk/channel-config-schema`,
  `recall/plugin-sdk/channel-policy`,
  `recall/plugin-sdk/config-runtime`,
  `recall/plugin-sdk/infra-runtime`,
  `recall/plugin-sdk/agent-runtime`,
  `recall/plugin-sdk/lazy-runtime`,
  `recall/plugin-sdk/reply-history`,
  `recall/plugin-sdk/routing`,
  `recall/plugin-sdk/status-helpers`,
  `recall/plugin-sdk/runtime-store`, and
  `recall/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `recall/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `recall/plugin-sdk/*` subpaths. Recall core/test code may use the repo
...
- Avoid the root `recall/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `recall/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `recall/plugin-sdk/channel-actions`.
...
`recall/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Recall writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Recall's
...
- if both `catalog` and `discovery` are registered, Recall uses `catalog`
...
- Read-only command paths such as `recall status`, `recall status --all`,
  `recall channels status`, `recall channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `recall.extensions`:
...
Security guardrail: every `recall.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `recall plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `recall.setupEntry` can point at a lightweight setup-only module.
When Recall needs setup surfaces for a disabled channel plugin, or
...
Optional: `recall.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Recall load the
...
Channel plugins can advertise setup/discovery metadata via `recall.channel` and
install hints via `recall.install`. This keeps the core catalog data-free.
...
Recall can also merge **external channel catalogs** (for example, an MPM
...
Or point `RECALL_PLUGIN_CATALOG_PATHS` (or `RECALL_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "recall": { "channel": {...}, "install": {...} } } ] }`.
...
```ts
import { delegateCompactionToRuntime } from "recall/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `RecallPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Recall stays opinionated without becoming hardcoded to one
