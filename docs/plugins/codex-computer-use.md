---
summary: "Set up Codex Computer Use for Codex-mode Recall agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode Recall agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. Recall
...
Use this page when Recall is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## Recall.app and Peekaboo

Recall.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want Recall.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode Recall agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an Recall node and exposes mobile
...
Recall-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through Recall's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the Recall command:

```bash
cua-driver mcp-config --client recall
```

or register the stdio server yourself:

```bash
recall mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
```
...
available as a normal Recall MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. Recall
...
With this config, Recall checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, Recall asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, Recall also tries to
...
plugin command surface is available. These are Recall chat/runtime commands,
not `recall codex ...` CLI subcommands:
...
Recall uses the same app-server API that Codex itself exposes. The
...
During install, Recall polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, Recall prefers
...
`computer-use` is registered, Recall tries to add the standard bundled
...
Recall reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If Recall says Computer Use
...
Recall intentionally fails closed when `computerUse.enabled` is true. A
...
fresh Recall session.

**A Computer Use tool says `Native hook relay unavailable`.** The Codex-native
tool hook could not reach an active Recall relay through the local bridge or
Gateway fallback. Start a fresh Recall session with `/new` or `/reset`. If it
