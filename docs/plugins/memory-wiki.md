...
If the active memory plugin exposes shared recall artifacts, Recall can search
...
currently exposing public bridge inputs yet. Run `recall wiki doctor` first,
...
`recall wiki status`, `recall wiki doctor`, and `recall wiki bridge
...
recall wiki apply synthesis "Alpha Summary" --body "..." --source-id source.alpha
