---
summary: "Build simple typed agent tools with defineToolPlugin and recall plugins init/build/validate"
title: "Tool plugins"
sidebarTitle: "Tool Plugins"
read_when:
  - You want to build a simple Recall plugin that only adds agent tools
...
Tool plugins add agent-callable tools to Recall without adding a channel,
model provider, hook, service, or setup backend. Use `defineToolPlugin` when the
plugin owns a fixed list of tools and you want Recall to generate the manifest
metadata that keeps those tools discoverable without loading runtime code.

The recommended flow is:

1. Scaffold a package with `recall plugins init`.
2. Write tools with `defineToolPlugin`.
3. Build JavaScript.
4. Generate `recall.plugin.json` and `package.json` metadata with
   `recall plugins build`.
...
- `recall >=2026.5.17`, the first Recall version that exports
  `recall/plugin-sdk/tool-plugin`.
- A package root that can ship `dist/`, `recall.plugin.json`, and
  `package.json`.
...
- `recall.plugin.json`: generated manifest metadata for the initial tool.
...
`recall plugins build` writes the matching `toolMetadata.<tool>.optional`
manifest entry, so Recall can discover the tool without loading plugin
runtime code.
...
`defineToolPlugin` wraps plain return values into the Recall tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and Recall to keep the original value in `details`.
...
`configSchema` is optional. If you omit it, Recall uses a strict empty object
...
Recall reads plugin config from the plugin entry in the Gateway config. Do not
...
Recall discovers installed plugins from cold metadata. It must be able to read
the plugin manifest before importing plugin runtime code. `defineToolPlugin`
therefore exposes static metadata, and `recall plugins build` writes that
...
`contracts.tools` is the important discovery contract. It tells Recall which
...
For the simple tool-plugin workflow, `recall plugins build` aligns
...
- `recall.plugin.json` exists and passes the normal manifest loader.
...
- `package.json` points `recall.extensions` at the selected runtime entry.

## Install and inspect locally

From a separate Recall checkout or installed CLI, install the package path:

```bash
recall plugins install ./stock-quotes
recall plugins inspect stock-quotes --runtime
```
...
```bash
npm pack
recall plugins install npm-pack:./recall-plugin-stock-quotes-0.1.0.tgz
recall plugins inspect stock-quotes --runtime --json
```
...
```bash
recall plugins install clawhub:your-org/stock-quotes
```

Bare npm package specs remain supported during the launch cutover, but ClawHub
is the preferred discovery and distribution surface for Recall plugins.
...
`recall plugins build --entry ./dist/index.js` or
`recall plugins validate --entry ./dist/index.js`.
...
### `recall.plugin.json generated metadata is stale`
...
Commit both `recall.plugin.json` and `package.json` changes.

### `package.json recall.extensions must include ./dist/index.js`

The package metadata points at a different runtime entry. Run
`recall plugins build --entry ./dist/index.js` so the generator aligns the
...
1. `recall plugins inspect <plugin-id> --runtime`
2. `recall plugins validate --root <plugin-root> --entry ./dist/index.js`
3. `recall.plugin.json` has `contracts.tools` with the expected tool names.
4. `package.json` has `recall.extensions: ["./dist/index.js"]`.
