---
summary: "Build simple typed agent tools with defineToolPlugin and steelengine plugins init/build/validate"
title: "Tool plugins"
sidebarTitle: "Tool Plugins"
read_when:
  - You want to build a simple SteelEngine plugin that only adds agent tools
...
manifest metadata SteelEngine needs to discover tools without loading plugin
runtime code.
...
- `steelengine >=2026.5.17`, the first version that exports
  `steelengine/plugin-sdk/tool-plugin`.
- A package root that ships `dist/`, `steelengine.plugin.json`, and
  `package.json`.
...
| `package.json`         | Scripts, runtime deps, `steelengine.extensions: ["./dist/index.js"]` |
| `steelengine.plugin.json` | Generated manifest metadata for the initial tool                  |

`npm run plugin:build` runs `npm run build` (tsc) then
`steelengine plugins build --entry ./dist/index.js`. `npm run plugin:validate`
rebuilds and runs `steelengine plugins validate --entry ./dist/index.js`.
...
`steelengine plugins init <id>` options:
...
is sent to a model. `steelengine plugins build` writes the matching
`toolMetadata.<tool>.optional` manifest entry, so SteelEngine can see that the
...
`defineToolPlugin` wraps plain return values into the SteelEngine tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and SteelEngine to keep the original value in `details`.
...
SteelEngine compiles the schema before executing a catalog call, then validates the
...
`configSchema` is optional. Omit it and SteelEngine applies a strict empty object
...
SteelEngine reads plugin config from the plugin's entry in the Gateway config. Do
...
SteelEngine must read the plugin manifest before importing plugin runtime code.
`defineToolPlugin` exposes static metadata for this, and
`steelengine plugins build` writes it into the package. Rerun the generator after
changing plugin id, name, description, config schema, activation, or tool
names:

```bash
npm run build
steelengine plugins build --entry ./dist/index.js
```
...
`contracts.tools` is the important discovery contract: it tells SteelEngine which
...
`steelengine plugins build` also aligns `package.json` to the selected runtime
entry:

```json
{
  "type": "module",
  "files": ["dist", "steelengine.plugin.json", "README.md"],
  "dependencies": {
    "typebox": "^1.1.38"
  },
  "peerDependencies": {
    "steelengine": ">=2026.5.17"
  },
  "steelengine": {
...
```bash
npm run build
steelengine plugins build --entry ./dist/index.js --check
steelengine plugins validate --entry ./dist/index.js
npm test
```

`plugins validate` checks that:

- `steelengine.plugin.json` exists and passes the normal manifest loader.
...
- `package.json` points `steelengine.extensions` at the selected runtime entry.

## Install and inspect locally

From a separate SteelEngine checkout or installed CLI, install the package path:

```bash
steelengine plugins install ./stock-quotes
steelengine plugins inspect stock-quotes --runtime
```
...
```bash
npm pack
steelengine plugins install npm-pack:./steelengine-plugin-stock-quotes-0.1.0.tgz
steelengine plugins inspect stock-quotes --runtime --json
```
...
```bash
steelengine plugins install clawhub:your-org/stock-quotes
```

Bare npm package specs still install from npm during the launch cutover, but
ClawHub is the preferred discovery and distribution surface for SteelEngine
...
`steelengine plugins build --entry ./dist/index.js` or
`steelengine plugins validate --entry ./dist/index.js`.
...
### `steelengine.plugin.json generated metadata is stale`
...
Commit both `steelengine.plugin.json` and `package.json` changes.

### `package.json steelengine.extensions must include ./dist/index.js`

The package metadata points at a different runtime entry. Run
`steelengine plugins build --entry ./dist/index.js` so the generator aligns
...
1. `steelengine plugins inspect <plugin-id> --runtime`
2. `steelengine plugins validate --root <plugin-root> --entry ./dist/index.js`
3. `steelengine.plugin.json` has `contracts.tools` with the expected tool names.
4. `package.json` has `steelengine.extensions: ["./dist/index.js"]`.
