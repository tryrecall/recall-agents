...
[`recall plugins install`](/cli/plugins).
...
```bash
export RECALL_ALLOW_PLUGIN_INSTALL_OVERRIDES=1
export RECALL_PLUGIN_INSTALL_OVERRIDES='{
  "codex": "npm-pack:/tmp/recall-codex-2026.5.8.tgz",
  "recall-web-search": "npm:@recall/web-search@2026.5.8"
}'
```
...
Recall uses the override source instead of the catalog, bundled, or default
...
entry normally represents an Recall-owned package, an override is treated as
operator-supplied test input.

Workspace `.env` files cannot enable install overrides. Set these variables in
the trusted shell, CI job, or remote test command that launches Recall.

## Package E2E

Use an isolated state directory so package installs and install records do not
touch your normal Recall state:

```bash
npm pack extensions/codex --pack-destination /tmp

RECALL_STATE_DIR="$(mktemp -d)" \
RECALL_ALLOW_PLUGIN_INSTALL_OVERRIDES=1 \
RECALL_PLUGIN_INSTALL_OVERRIDES='{"codex":"npm-pack:/tmp/recall-codex-2026.5.8.tgz"}' \
pnpm recall onboard --mode local
```

Verify the installed package under the state directory:

```bash
find "$RECALL_STATE_DIR/npm/node_modules" -maxdepth 3 -name package.json -print
grep -R '"@recall/codex"' "$RECALL_STATE_DIR/npm/package-lock.json"
```
