...
The plugin is included with Recall, but it is off by default. When disabled, the route is not registered. When enabled, it adds:
...
Use the WebSocket RPC path for Recall clients and interactive tools that can keep a Gateway WebSocket connection open.
...
<Tabs>
  <Tab title="CLI">
    ```bash
    recall plugins enable admin-http-rpc
...
```bash
recall plugins disable admin-http-rpc
recall gateway restart
```
...
- For `token` and `password` auth, narrower `x-recall-scopes` headers are ignored and the normal full operator defaults are restored.
- Trusted identity-bearing HTTP modes honor `x-recall-scopes` when present.
...
The normal Gateway WebSocket RPC path remains the preferred control-plane API for Recall clients. Use admin HTTP RPC only for host tooling that needs a request/response HTTP surface.
