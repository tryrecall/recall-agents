...
RECALL_DIAGNOSTICS_TIMELINE_PATH=/tmp/recall-timeline.jsonl \
recall gateway run
```
...
`RECALL_DIAGNOSTICS_TIMELINE_PATH`. When `timeline` is enabled only from
config, the earliest config-loading spans are not emitted because Recall has
not read config yet; subsequent startup spans use the config flag.

`RECALL_DIAGNOSTICS=1`, `RECALL_DIAGNOSTICS=all`, and
`RECALL_DIAGNOSTICS=*` also enable the timeline because they enable every
diagnostics flag. Prefer `timeline` when you only want the JSONL timing
artifact.

Timeline records use the `recall.diagnostics.v1` envelope. Events can include
...
```bash
ls -t /tmp/tryrecall/recall-agents-*.log | head -n 1
```

Filter for Telegram HTTP diagnostics:

```bash
rg "telegram http error" /tmp/tryrecall/recall-agents-*.log
```

Filter for Brave Search HTTP diagnostics:

```bash
rg "brave http" /tmp/tryrecall/recall-agents-*.log
```

Or tail while reproducing:

```bash
tail -f /tmp/tryrecall/recall-agents-$(date +%F).log | rg "telegram http error"
```

For remote gateways, you can also use `recall logs --follow` (see [/cli/logs](/cli/logs)).
