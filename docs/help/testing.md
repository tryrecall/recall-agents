...
Recall has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
- Pool note:
  ...
  - Opt back into Vitest file isolation with `RECALL_TEST_ISOLATE=1 pnpm test`.
  - `RECALL_TEST_NO_ISOLATE=0` or `RECALL_TEST_NO_ISOLATE=false` also force isolated runs.
- Fast-local iteration note:
  ...
  - Opt out with `RECALL_VITEST_FS_MODULE_CACHE=0` or `RECALL_VITEST_FS_MODULE_CACHE=false` if you suspect stale transform cache behavior.
- Perf-debug note:
...
- How to select models:
  - `RECALL_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet/Haiku 4.5, GPT-5.x + Codex, Gemini 3, GLM 4.7, MiniMax M2.7, Grok 4)
  - `RECALL_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `RECALL_LIVE_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,..."` (comma allowlist)
...
- Enable:
  - `pnpm test:live` (or `RECALL_LIVE_TEST=1` if invoking Vitest directly)
  - `RECALL_LIVE_SETUP_TOKEN=1`
- Token sources (pick one):
  - Profile: `RECALL_LIVE_SETUP_TOKEN_PROFILE=anthropic:setup-token-test`
  - Raw token: `RECALL_LIVE_SETUP_TOKEN_VALUE=sk-ant-oat01-...`
- Model override (optional):
  - `RECALL_LIVE_SETUP_TOKEN_MODEL=anthropic/claude-opus-4-6`

Setup example:

```bash
recall models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
RECALL_LIVE_SETUP_TOKEN=1 RECALL_LIVE_SETUP_TOKEN_PROFILE=anthropic:setup-token-test pnpm test:live src/agents/anthropic.setup-token.live.test.ts
```
...
- Overrides (optional):
  - `RECALL_LIVE_CLI_BACKEND_MODEL="claude-cli/claude-opus-4-6"`
  - `RECALL_LIVE_CLI_BACKEND_MODEL="codex-cli/gpt-5.4"`
  - `RECALL_LIVE_CLI_BACKEND_COMMAND="/full/path/to/claude"`
  - `RECALL_LIVE_CLI_BACKEND_ARGS='["-p","--output-format","json","--permission-mode","bypassPermissions"]'`
  - `RECALL_LIVE_CLI_BACKEND_CLEAR_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'`
  - `RECALL_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt).
...
- `RECALL_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG=0` to keep Claude Code CLI MCP config enabled (default disables MCP config with a temporary empty file).
...
- Single model, direct (no gateway):
  - `RECALL_LIVE_MODELS="openai/gpt-5.2" pnpm test:live src/agents/models.profiles.live.test.ts`

- Single model, gateway smoke:
  - `RECALL_LIVE_GATEWAY_MODELS="openai/gpt-5.2" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `RECALL_LIVE_GATEWAY_MODELS="openai/gpt-5.2,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,zai/glm-4.7,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Profile store: `~/.recall/credentials/` (preferred; what “profile keys” means in the tests)
- Config: `~/.recall/recall.json` (or `RECALL_CONFIG_PATH`)
...
- Current bundled providers covered:
  ...
  - `google`
- Optional narrowing:
  - `RECALL_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google"`
  - `RECALL_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-1,google/gemini-3.1-flash-image-preview"`
  - `RECALL_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit"`
...
- `RECALL_PROFILE_FILE=...` (default: `~/.profile`) mounted to `/home/node/.profile` and sourced before running tests
- External CLI auth dirs under `$HOME` are mounted read-only under `/host-auth/...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs inferred from `RECALL_LIVE_PROVIDERS` / `RECALL_LIVE_GATEWAY_PROVIDERS`
