---
summary: "Frequently asked questions about Recall setup, configuration, and usage"
read_when:
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/recall/recall-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   recall health --json
   recall health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://recall.ai/install.sh | bash -s -- --install-method git
    ```

    This installs Recall **from a git checkout**, so the agent can read the code + docs and
    ...
    [https://github.com/recall/recall/issues](https://github.com/recall/recall/issues)
    [https://github.com/recall/recall/pulls](https://github.com/recall/recall/pulls)
    ...
    - `recall status`: quick snapshot of gateway/agent health + basic config.
    - `recall models status`: checks provider auth + model availability.
    - `recall doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `recall status --all`, `recall logs --follow`,
    `recall gateway status`, `recall health --verbose`.
  ...
  <Accordion title="Recommended way to install and set up Recall">
    ...
    If you don't have a global install yet, run it via `pnpm recall onboard`.
  ...
  <Accordion title="How do I authenticate the dashboard (token) on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - If it asks for auth, paste the token from `gateway.auth.token` (or `RECALL_GATEWAY_TOKEN`) into Control UI settings.
    - Retrieve it from the gateway host: `recall config get gateway.auth.token` (or generate one: `recall doctor --generate-gateway-token`).

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `recall gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
    - **Tailnet bind**: run `recall gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    1. Install Recall on the new machine.
    2. Copy `$RECALL_STATE_DIR` (default: `~/.recall`) from the old machine.
    3. Copy your workspace (default: `~/.recall/workspace`).
    4. Run `recall doctor` and restart the Gateway service.
    ...
    **Important:** if you only commit/push your workspace to GitHub, you're backing
    ...
    under `~/.recall/` (for example `~/.recall/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/recall/recall/blob/main/CHANGELOG.md](https://github.com/recall/recall/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.recall.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.recall.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.recall.ai`, then retry. More
    detail: [Troubleshooting](/help/faq#cannot-access-docsrecall-ai-ssl-error).
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/recall/recall/tree/main/docs](https://github.com/recall/recall/tree/main/docs)
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://recall.ai/install.sh | bash -s -- --beta
    ```
    ...
    [https://recall.ai/install.ps1](https://recall.ai/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://recall.ai/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://recall.ai/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://recall.ai/install.sh | bash -s -- --install-method git --verbose
    ```
  ...
  <Accordion title="Windows install says git not found or recall not recognized">
    ...
    **2) recall is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest Recall, track/report it in:

    - [Issue #30640](https://github.com/recall/recall/issues/30640)
  ...
  <Accordion title="How do I install Recall on Linux?">
  ...
  <Accordion title="How do I install Recall on a VPS?">
  ...
  <Accordion title="Can I ask Recall to update itself?">
    ...
    recall update --channel stable|beta|dev
    recall update --tag <dist-tag|version>
    recall update --no-restart
    ```

    If you must automate from an agent:

    ```bash
    recall update --yes --no-restart
    recall gateway restart
    ```
  ...
  <Accordion title="What does onboarding actually do?">
    `recall onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. You can run Recall with **API keys** (Anthropic/OpenAI/others) or with
    **local-only models** so your data stays on your device. Subscriptions (Claude
    ...
    OpenAI Codex OAuth is explicitly supported for external tools like Recall.
  ...
  <Accordion title="How does Anthropic setup-token auth work?">
    `claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in onboarding or paste it with `recall models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
  </Accordion>
  ...
  <Accordion title="Where do I find an Anthropic setup-token?">
    ...
    Copy the token it prints, then choose **Anthropic token (paste setup-token)** in onboarding. If you want to run it on the gateway host, use `recall models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `recall models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

  </Accordion>

  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    Yes - via **setup-token**. Recall no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
  ...
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Tip: set a **fallback model** so Recall can keep replying while a provider is rate-limited.
  ...
  <Accordion title="How does Codex auth work?">
    Recall supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). Onboarding can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Onboarding (CLI)](/start/wizard).
  </Accordion>

  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. Recall fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like Recall. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `recall.json`.

    Steps:

    1. Enable the plugin: `recall plugins enable google`
    2. Login: `recall models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Recall needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. Recall runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
  ...
  <Accordion title="If I buy a Mac mini to run Recall, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `recall nodes status` / `recall nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Onboarding accepts `@username` input and resolves it to a numeric ID, but Recall authorization uses numeric IDs only.

    Safer (no third-party bot):

    - DM your bot, then run `recall logs --follow` and read `from.id`.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Recall instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    If you run Recall via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    ...
    This **does not delete your data** - it only changes the Recall code install. Your state
    (`~/.recall`) and workspace (`~/.recall/workspace`) stay untouched.
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Recall-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run Recall on a dedicated machine?">
  ...
  <Accordion title="What are the minimum VPS requirements and recommended OS?">
    Recall is lightweight. For a basic Gateway + one chat channel:
  ...
  <Accordion title="Can I run Recall in a VM and what are the requirements?">
...
<AccordionGroup>
  <Accordion title="What is Recall, in one paragraph?">
    Recall is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.
  </Accordion>

  <Accordion title="Value proposition">
    Recall is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
  ...
  <Accordion title="What are the top five everyday use cases for Recall?">
  ...
  <Accordion title="Can Recall help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    Recall draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    Recall is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use Recall when you
    ...
    Showcase: [https://recall.ai/showcase](https://recall.ai/showcase)
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.recall/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.recall/recall.json`). Precedence is `<workspace>/skills` > `~/.recall/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.recall/recall.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.recall/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Recall treats as `<workspace>/skills` on the next session.
  </Accordion>
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `RECALL_SKIP_CRON` is not set.
    ...
    ```bash
    recall cron run <jobId> --force
    recall cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `recall skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://clawhub.com](https://clawhub.com).

    ```bash
    recall skills search "calendar"
    recall skills install <skill-slug>
  ...
  <Accordion title="Can Recall run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.recall.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH).**
    Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. Recall can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.recall/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "recall": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.recall/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with Recall?">
    Use the built-in `user` browser profile, which attaches through Chrome DevTools MCP:

    ```bash
    recall browser --browser-profile user tabs
    recall browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Persist `/home/node` with `RECALL_HOME_VOLUME` so caches survive.
    - Bake system deps into the image with `RECALL_DOCKER_APT_PACKAGES`.
  ...
  <Accordion title="How does memory work?">
    Recall memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    Recall also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, Recall auto-selects a provider when it
...
<AccordionGroup>
  <Accordion title="Is all data used with Recall saved locally?">
    No - **Recall's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.recall` + your workspace directory).
  ...
  <Accordion title="Where does Recall store its data?">
    Everything lives under `$RECALL_STATE_DIR` (default: `~/.recall`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$RECALL_STATE_DIR/recall.json`                             | Main config (JSON5)                                                |
    | `$RECALL_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
    | `$RECALL_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
    | `$RECALL_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
    | `$RECALL_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
    | `$RECALL_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
    | `$RECALL_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
    | `$RECALL_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
    | `$RECALL_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

    Legacy single-agent path: `~/.recall/agent/*` (migrated by `recall doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.recall/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.recall`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md` (or legacy fallback `memory.md` when `MEMORY.md` is absent),
      `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
    - **State dir (`~/.recall`)**: config, credentials, auth profiles, sessions, logs,
      and shared skills (`~/.recall/skills`).

    Default workspace is `~/.recall/workspace`, configurable via:
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.recall` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall Recall?">
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The Recall repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Recall reads an optional **JSON5** config from `$RECALL_CONFIG_PATH` (default: `~/.recall/recall.json`):

    ```
    $RECALL_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.recall/workspace`).

  </Accordion>

  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `RECALL_GATEWAY_TOKEN`).
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Recall enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

    If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `recall doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one Recall.` every time.
    - `random`: rotating funny/seasonal taglines (default behavior).
    - If you want no banner at all, set env `RECALL_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `recall configure --section web` and choose a provider.
    ...
    - Daemons read env vars from `~/.recall/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    - Restore from backup (git or a copied `~/.recall/recall.json`).
    - If you have no backup, re-run `recall doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `recall config set` for small changes.
    - Use `recall configure` for interactive edits.
  ...
  <Accordion title="Can the Recall browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `recall gateway status`
    - Gateway health: `recall status`
    - Channel health: `recall channels status`
  ...
  <Accordion title="Can two Recall instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `recall agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    recall agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
...
<AccordionGroup>
  <Accordion title="How does Recall load environment variables?">
    Recall reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.recall/.env` (aka `$RECALL_STATE_DIR/.env`)
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.recall/.env` so they're picked up even when the service doesn't inherit your shell env.
    ...
    `RECALL_LOAD_SHELL_ENV=1`, `RECALL_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `recall models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means Recall won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.recall/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Recall instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Recall but keep it installed?">
    ...
    - If you used profiles (`--profile` / `RECALL_PROFILE`), reset each state dir (defaults are `~/.recall-<profile>`).
    - Dev reset: `recall gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    headers like `# Heading`), Recall skips the heartbeat run to save API calls.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. Recall runs on **your own account**, so if you're in the group, Recall can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    recall directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does Recall not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.recall/agents/<agentId>/sessions/`.
    ...
    - Use `recall doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    Recall's default model is whatever you set as:

    ```
    agents.defaults.model.primary
    ```

    Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, Recall currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    - `recall models set ...` (updates just model config)
    - `recall configure --section model` (interactive)
    - edit `agents.defaults.model` in `~/.recall/recall.json`

    Avoid `config.apply` with a partial object unless you intend to replace the whole config.
    If you did overwrite config, restore from backup or re-run `recall doctor` to repair.
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `recall onboard` and choose `Ollama`
    ...
    - for manual switching, use `recall models list` and `recall models set ollama/<model>`
  ...
  <Accordion title="What do Recall, Flawd, and Krill use for models?">
    - These deployments can differ and may change over time; there is no fixed provider recommendation.
    - Check the current runtime setting on each gateway with `recall models status`.
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Upgrade to a current Recall release (or run from source `main`), then restart the gateway.
  ...
  <Accordion title="Are opus / sonnet / gpt built-in shortcuts?">
    Yes. Recall ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
  ...
  <Accordion title="How do I add models from other providers like OpenRouter or Z.AI?">
    ...
    **No API key found for provider after adding a new agent**
    ...
    - Run `recall agents add <id>` and configure auth during the wizard.
...
<AccordionGroup>
  <Accordion title="How does failover work?">
    ...
    Cooldowns apply to failing profiles (exponential backoff), so Recall can keep responding even when a provider is rate-limited or temporarily failing.
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist:**
    ...
    - **Confirm where auth profiles live** (new vs legacy paths)
      - Current: `~/.recall/agents/<agentId>/agent/auth-profiles.json`
      - Legacy: `~/.recall/agent/*` (migrated by `recall doctor`)
    - **Confirm your env var is loaded by the Gateway**
      - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.recall/.env` or enable `env.shellEnv`.
    - **Make sure you're editing the correct agent**
      - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
    - **Sanity-check model/auth status**
      - Use `recall models status` to see configured models and whether providers are authenticated.
    ...
    **Fix checklist for "No credentials found for profile anthropic"**
    ...
    - **Use a setup-token**
      - Run `claude setup-token`, then paste it with `recall models auth setup-token --provider anthropic`.
      - If the token was created on another machine, use `recall models auth paste-token --provider anthropic`.
    - **If you want to use an API key instead**
      - Put `ANTHROPIC_API_KEY` in `~/.recall/.env` on the **gateway host**.
      - Clear any pinned order that forces a missing profile:

        ```bash
        recall models auth order clear --provider anthropic
        ```
  ...
  <Accordion title="Why did it also try Google Gemini and fail?">
    If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Recall will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
    ...
    **LLM request rejected: thinking signature required (Google Antigravity)**
    ...
    Fix: Recall now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
<AccordionGroup>
  ...
  <Accordion title="What are typical profile IDs?">
    Recall uses provider-prefixed IDs like:
  ...
  <Accordion title="Can I control which auth profile is tried first?">
    ...
    Recall may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `recall models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.

    You can also set a **per-agent** order override (stored in that agent's `auth-profiles.json`) via the CLI:

    ```bash
    # Defaults to the configured default agent (omit --agent)
    recall models auth order get --provider anthropic

    # Lock rotation to a single profile (only try this one)
    recall models auth order set --provider anthropic anthropic:default

    # Or set an explicit order (fallback within provider)
    recall models auth order set --provider anthropic anthropic:work anthropic:default
    ...
    ```bash
    recall models auth order set --provider anthropic --agent main anthropic:default
    ```
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```
    --port > RECALL_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does recall gateway status say "Runtime: running" but "RPC probe: failed"?'>
    Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

    Use `recall gateway status` and trust these lines:
  ...
  <Accordion title='Why does recall gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `RECALL_STATE_DIR` mismatch).
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    Recall enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `recall gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run Recall in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `recall gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `recall dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `recall doctor --generate-gateway-token`.
    - If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
    - Set `gateway.auth.token` (or `RECALL_GATEWAY_TOKEN`) on the gateway host.
    - In the Control UI settings, paste the same token.
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `recall devices list`
      - `recall devices rotate --device <id> --role operator`
    - Still stuck? Run `recall status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - `RECALL_CONFIG_PATH` (per-instance config)
    - `RECALL_STATE_DIR` (per-instance state)
    - `agents.defaults.workspace` (workspace isolation)
    - `gateway.port` (unique ports)

    Quick setup (recommended):

    - Use `recall --profile <name> ...` per instance (auto-creates `~/.recall-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `recall --profile <name> gateway install`.

    Profiles also suffix service names (`ai.tryrecall.<profile>`; legacy `com.recall.*`, `recall-gateway-<profile>.service`, `Recall Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    recall tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - macOS: `$RECALL_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.recall/logs/...`; profiles use `~/.recall-<profile>/logs/...`)
    - Linux: `journalctl --user -u recall-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "Recall Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `recall gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart Recall?">
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `recall gateway status`
    2. Is the Gateway healthy? `recall status`
    3. Does the UI have the right token? `recall dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    recall channels status
    recall channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Recall already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: recall gateway restart vs recall gateway">
    - `recall gateway restart`: restarts the **background service** (launchd/systemd).
    - `recall gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `recall gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Recall assistant setup](/start/recall) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    recall message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Recall to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `recall pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `recall pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `recall doctor` to surface risky DM policies.
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Recall only replies to chats it receives or to explicit sends you trigger.

    Approve pairing with:

    ```bash
    recall pairing approve whatsapp <code>
    ```

    List pending requests:

    ```bash
    recall pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    Recall blocks **cross-provider** messaging by default. If a tool call is bound
