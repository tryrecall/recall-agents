...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/tryrecall/recall-agents-*.log | head -1)"
   ```
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.recall/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.tryrecall/recall-agents.json`). Precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.recall/skills` → bundled → `skills.load.extraDirs`, so managed overrides still win over bundled skills without touching git. If you need the skill installed globally but only visible to some agents, keep the shared copy in `~/.recall/skills` and control visibility with `agents.defaults.skills` and `agents.list[].skills`. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.tryrecall/recall-agents.json` (lowest precedence). Default precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.recall/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Recall treats as `<workspace>/skills` on the next session. If the skill should only be visible to certain agents, pair that with `agents.defaults.skills` or `agents.list[].skills`.
  </Accordion>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, Recall falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is the exact silent token `NO_REPLY` / `no_reply`, or exactly `ANNOUNCE_SKIP`, Recall intentionally suppresses the announce instead of posting stale earlier progress.
    ...
    ```bash
    recall tasks show <runId-or-sessionKey>
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    ...
    recall skills search --limit 20
    recall skills install <skill-slug>
    recall skills install <skill-slug> --version <version>
    recall skills install <skill-slug> --force
    recall skills install <skill-slug> --global
    ...
    Native `recall skills install` writes into the active workspace `skills/`
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across all local agents, use `recall skills install <slug> --global` (or place them manually in `~/.recall/skills/<name>/SKILL.md`). If only some agents should see a shared install, configure `agents.defaults.skills` or `agents.list[].skills`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills), [Skills config](/tools/skills-config), and [ClawHub](/tools/clawhub).
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Bake system deps into the image with `RECALL_IMAGE_APT_PACKAGES`.
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    Recall validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor. That means symlink-parent escapes still fail closed even when the last path segment does not exist yet, and allowed-root checks still apply after symlink resolution.
...
<AccordionGroup>
  ...
  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    ...
    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      ...
      Lowercase root `memory.md` is legacy repair input only; `recall doctor --fix`
      can merge it into `MEMORY.md` when both files exist.
    - **State dir (`~/.recall`)**: config, channel/provider state, auth profiles, sessions, logs,
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Recall reads an optional **JSON5** config from `$RECALL_CONFIG_PATH` (default: `~/.tryrecall/recall-agents.json`):
  ...
  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    ...
    - For password auth, set `gateway.auth.mode: "password"` plus `gateway.auth.password` (or `RECALL_GATEWAY_PASSWORD`) instead.
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Recall enforces gateway auth by default, including loopback. In the normal default path that means token auth: if no explicit auth path is configured, gateway startup resolves to token mode and generates a runtime-only token for that startup, so **local WS clients must authenticate**. Configure `gateway.auth.token`, `gateway.auth.password`, `RECALL_GATEWAY_TOKEN`, or `RECALL_GATEWAY_PASSWORD` explicitly when clients need a stable secret across restarts. This blocks other local processes from calling the Gateway.

    If you prefer a different auth path, you can explicitly choose password mode (or, for identity-aware reverse proxies, `trusted-proxy`). If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `recall doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `recall configure --section web` and choose a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, Recall auto-detects the first ready fetch fallback provider from available credentials. Today the bundled provider is Firecrawl.
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current Recall protects many accidental clobbers:

    - Recall-owned config writes validate the full post-change config before writing.
    - Invalid or destructive Recall-owned writes are rejected and saved as `recall.json.rejected.*`.
    - If a direct edit breaks startup or hot reload, Gateway fails closed or skips the reload; it does not rewrite `recall.json`.
    - `recall doctor --fix` owns repair and can restore last-known-good while saving the rejected file as `recall.json.clobbered.*`.

    Recover:

    - Check `recall logs --follow` for `Invalid config at`, `Config write rejected:`, or `config reload skipped (invalid config)`.
    - Inspect the newest `recall.json.clobbered.*` or `recall.json.rejected.*` beside the active config.
    - Run `recall config validate` and `recall doctor --fix`.
    - Copy only the intended keys back with `recall config set` or `config.patch`.
    - If you have no last-known-good or rejected payload, restore from backup, or re-run `recall doctor` and reconfigure channels/models.
...
<AccordionGroup>
  ...
  <Accordion title='Why does recall gateway status say "Runtime: running" but "Connectivity probe: failed"?'>
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Shared-secret mode: set `gateway.auth.token` / `RECALL_GATEWAY_TOKEN` or `gateway.auth.password` / `RECALL_GATEWAY_PASSWORD`, then paste the matching secret in Control UI settings.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    Profiles also suffix service names (`ai.recall.<profile>`; legacy `com.recall.*`, `recall-gateway-<profile>.service`, `Recall Gateway (<profile>)`).
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    ```
    /tmp/tryrecall/recall-agents-YYYY-MM-DD.log
    ```
    ...
    - macOS launchd stdout: `~/Library/Logs/recall/gateway.log` (profiles use `gateway-<profile>.log`; stderr is suppressed)
...
<AccordionGroup>
  ...
  <Accordion title="Is Recall less safe because it uses TypeScript/Node instead of Rust/WASM?">
    Language and runtime matter, but they are not the main risk for a personal
    agent. The practical Recall risks are gateway exposure, who can message the
    ...
    - run `recall security audit --deep` after config changes

    Details: [Security](/gateway/security), [Sandboxing](/gateway/sandboxing).

  </Accordion>

  <Accordion title="I saw reports about exposed Recall instances. What should I check?">
  ...
  <Accordion title="Are ClawHub skills and third-party plugins safe to install?">
    ...
    ClawHub skill pages expose scan state before install, and Recall plugin
...
Still stuck? Ask in [Discord](https://discord.com/invite/clawd) or open a [GitHub discussion](https://github.com/tryrecall/recall-agents/discussions).
