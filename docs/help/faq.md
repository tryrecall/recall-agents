---
summary: "Frequently asked questions about SteelEngine setup, configuration, and usage"
read_when:
...
<Steps>
  ...
  <Step title="Tail the latest log">
    ...
    ```bash
    tail -f "$(ls -t /tmp/steelengine/steelengine-*.log | head -1)"
    ```
  ...
  <Step title="Gateway snapshot (WS-only)">
    ...
    steelengine health --verbose   # shows the target URL + config path on errors
    ```
...
<AccordionGroup>
  ...
  <Accordion title="What are the top five everyday use cases for SteelEngine?">
  ...
  <Accordion title="Can SteelEngine help with lead gen, outreach, ads, and blogs for a SaaS?">
    ...
    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and platform policies, and review anything before it sends. Let SteelEngine draft; you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    SteelEngine is a **personal assistant** and coordination layer, not an IDE replacement. Use Claude Code or Codex for the fastest direct coding loop inside a repo. Use SteelEngine for durable memory, cross-device access, and tool orchestration.
    ...
    Showcase: [https://steelengine.ai/showcase](https://steelengine.ai/showcase).
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put changes in `~/.steelengine/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.steelengine/steelengine.json`). Precedence: `<workspace>/skills` -> `<workspace>/.agents/skills` -> `~/.agents/skills` -> `~/.steelengine/skills` -> bundled -> `skills.load.extraDirs`, so managed overrides win over bundled skills without touching git. To install globally but limit visibility to some agents, keep the shared copy in `~/.steelengine/skills` and control visibility with `agents.defaults.skills` / `agents.list[].skills`. Only upstream-worthy edits should go out as PRs against the repo copy.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes: add directories via `skills.load.extraDirs` in `~/.steelengine/steelengine.json` (lowest precedence in the order above). `clawhub` installs into `./skills` by default, which SteelEngine treats as `<workspace>/skills` on the next session. To limit visibility to certain agents, pair with `agents.defaults.skills` or `agents.list[].skills`.
  </Accordion>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, SteelEngine falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is exactly `NO_REPLY` / `no_reply` or `ANNOUNCE_SKIP`, SteelEngine intentionally suppresses the announce instead of posting stale earlier progress.

    Debug: `steelengine tasks show <lookup>` where `<lookup>` is a task id, run id, or session key.
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `STEELENGINE_SKIP_CRON` is not set.
    - Confirm the Gateway is running 24/7 (no sleep/restarts).
    - Verify job timezone (`--tz` vs host timezone).

    Debug:
    ```bash
    steelengine cron run <jobId>
    steelengine cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `steelengine skills` commands or drop skills into your workspace; the macOS Skills UI is not available on Linux. Browse skills at [https://clawhub.ai](https://clawhub.ai).

    ```bash
    steelengine skills search "calendar"
    steelengine skills search --limit 20
    steelengine skills install @owner/<skill-slug>
    steelengine skills install @owner/<skill-slug> --version <version>
    steelengine skills install @owner/<skill-slug> --force
    steelengine skills install @owner/<skill-slug> --global
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.steelengine.os` plus required binaries, and only load when eligible on the **Gateway host**. On Linux, `darwin`-only skills (`apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH)**. Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. SteelEngine treats macOS-only skills as eligible when required binaries exist on the node; the agent runs them via the `nodes` tool. With "Always Ask," approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced)**. Keep the Gateway on Linux, but make the required CLI binaries resolve to SSH wrappers that run on a Mac, then override the skill to allow Linux so it stays eligible.
    ...
    3. Override the skill metadata (workspace or `~/.steelengine/skills`) to allow Linux:
       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "steelengine": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="How do I use my existing signed-in Chrome with SteelEngine?">
    ...
    steelengine browser --browser-profile user snapshot
    ```
    ...
    Current limits on `existing-session` / `user` profiles versus the managed `steelengine` profile:
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Persist `/home/node` with `STEELENGINE_HOME_VOLUME` so caches survive.
    - Bake system deps into the image with `STEELENGINE_IMAGE_APT_PACKAGES`.
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    SteelEngine validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor, so symlink-parent escapes fail closed even when the final path segment does not exist yet.
  ...
  <Accordion title="How does memory work?">
    SteelEngine memory is Markdown files in the agent workspace: daily notes in `memory/YYYY-MM-DD.md`, curated long-term notes in `MEMORY.md` (main/private sessions only).

    SteelEngine also runs a silent **pre-compaction memory flush** before compaction summarizes the conversation, reminding the model to write durable notes first. It only runs when the workspace is writable (read-only sandboxes skip it); disable with `agents.defaults.compaction.memoryFlush.enabled: false`. See [Memory](/concepts/memory).
...
<AccordionGroup>
  <Accordion title="Is all data used with SteelEngine saved locally?">
    No: **SteelEngine's own state is local**, but **external services still see what you send them**.

    - **Local by default**: sessions, memory files, config, and workspace live on the Gateway host (`~/.steelengine` plus your workspace directory).
  ...
  <Accordion title="Where does SteelEngine store its data?">
    Everything lives under `$STEELENGINE_STATE_DIR` (default: `~/.steelengine`):

    | Path                                                               | Purpose                                                            |
    | ------------------------------------------------------------------ | ------------------------------------------------------------------ |
    | `$STEELENGINE_STATE_DIR/steelengine.json`                                 | Main config (JSON5)                                                 |
    | `$STEELENGINE_STATE_DIR/credentials/oauth.json`                        | Legacy OAuth import (copied into auth profiles on first use)        |
    | `$STEELENGINE_STATE_DIR/agents/<agentId>/agent/auth-profiles.json`     | Auth profiles (OAuth, API keys, optional `keyRef`/`tokenRef`)        |
    | `$STEELENGINE_STATE_DIR/secrets.json`                                  | Optional file-backed secret payload for `file` SecretRef providers   |
    | `$STEELENGINE_STATE_DIR/agents/<agentId>/agent/auth.json`              | Legacy compatibility file (static `api_key` entries scrubbed)        |
    | `$STEELENGINE_STATE_DIR/credentials/`                                  | Provider state (for example `whatsapp/<accountId>/creds.json`)      |
    | `$STEELENGINE_STATE_DIR/agents/`                                       | Per-agent state (agentDir + legacy/archive session artifacts)        |
    | `$STEELENGINE_STATE_DIR/agents/<agentId>/agent/steelengine-agent.sqlite`  | Per-agent SQLite state, including session rows and transcripts      |
    | `$STEELENGINE_STATE_DIR/agents/<agentId>/sessions/`                    | Legacy session migration sources and archive/support artifacts      |

    Legacy single-agent path `~/.steelengine/agent/*` is migrated by `steelengine doctor`.

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate, configured via `agents.defaults.workspace` (default: `~/.steelengine/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These live in the **agent workspace**, not `~/.steelengine`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`, `MEMORY.md`, `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`. Lowercase root `memory.md` is legacy repair input only; `steelengine doctor --fix` can merge it into `MEMORY.md` when both exist.
    - **State dir (`~/.steelengine`)**: config, channel/provider state, auth profiles, sessions, logs, shared skills (`~/.steelengine/skills`).

    Default workspace is `~/.steelengine/workspace`, configurable:
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.steelengine` (credentials, sessions, tokens, encrypted secrets payloads). For a full restore, back up the workspace and state directory separately.

    Docs: [Agent workspace](/concepts/agent-workspace).

  </Accordion>

  <Accordion title="How do I completely uninstall SteelEngine?">
    See [Uninstall](/install/uninstall).
  </Accordion>

  <Accordion title="Can agents work outside the workspace?">
    Yes. The workspace is the **default cwd** and memory anchor, not a hard sandbox. Relative paths resolve inside the workspace; absolute paths can access other host locations unless sandboxing is enabled. For isolation, use [`agents.defaults.sandbox`](/gateway/sandboxing) or per-agent sandbox settings. To make a repo the default working directory, point that agent's `workspace` at the repo root - the SteelEngine repo itself is just source code, so keep the workspace separate unless you intentionally want the agent to work inside it.
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    SteelEngine reads an optional **JSON5** config from `$STEELENGINE_CONFIG_PATH` (default: `~/.steelengine/steelengine.json`). If the file is missing, it uses safe-ish defaults, including a default workspace of `~/.steelengine/workspace`.
  </Accordion>
  ...
  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    ...
    - For password auth, set `gateway.auth.mode: "password"` plus `gateway.auth.password` (or `STEELENGINE_GATEWAY_PASSWORD`).
  ...
  <Accordion title="Why do I need a token on localhost now?">
    SteelEngine enforces gateway auth by default, including loopback. If no explicit auth path is configured, startup resolves to token mode and generates a runtime-only token for that startup, so local WS clients must authenticate. This blocks other local processes from calling the Gateway.

    Configure `gateway.auth.token`, `gateway.auth.password`, `STEELENGINE_GATEWAY_TOKEN`, or `STEELENGINE_GATEWAY_PASSWORD` explicitly when clients need a stable secret across restarts. You can also choose password mode, or `trusted-proxy` for identity-aware reverse proxies. For open loopback, set `gateway.auth.mode: "none"` explicitly. `steelengine doctor --generate-gateway-token` generates a token any time.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: always uses `All your chats, one SteelEngine.`.
    - `random`: rotating funny/seasonal taglines (default behavior).
    - For no banner at all, set env `STEELENGINE_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    Grok can also reuse xAI OAuth from model auth (`steelengine onboard --auth-choice xai-oauth`).

    **Recommended**: `steelengine configure --section web` and pick a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, SteelEngine auto-detects the first ready fetch fallback provider from available credentials; the official Firecrawl plugin provides that fallback.
    - Daemons read env vars from `~/.steelengine/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current SteelEngine protects most accidental clobbers:

    - SteelEngine-owned config writes validate the full post-change config before writing.
    - Invalid or destructive SteelEngine-owned writes are rejected and saved as `steelengine.json.rejected.*`.
    - A direct edit that breaks startup or hot reload makes the Gateway fail closed or skip the reload; it does not rewrite `steelengine.json`.
    - `steelengine doctor --fix` owns repair, can restore last-known-good, and saves the rejected file as `steelengine.json.clobbered.*`.

    Recover:

    - Check `steelengine logs --follow` for `Invalid config at`, `Config write rejected:`, or `config reload skipped (invalid config)`.
    - Inspect the newest `steelengine.json.clobbered.*` or `steelengine.json.rejected.*` beside the active config.
    - Run `steelengine config validate` and `steelengine doctor --fix`.
    - Copy only the intended keys back with `steelengine config set` or `config.patch`.
    - No last-known-good or rejected payload: restore from backup, or re-run `steelengine doctor` and reconfigure channels/models.
  ...
  <Accordion title="Can the SteelEngine browser run headless?">
    Yes:
...
<AccordionGroup>
  ...
  <Accordion title="Can two SteelEngine instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic)**: run a script that calls the other Gateway with `steelengine agent --message ... --deliver`, targeting a chat where the other bot listens. If one bot is on a remote VPS, point your CLI at that remote Gateway via SSH/Tailscale (see [Remote access](/gateway/remote)):

    ```bash
    steelengine agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
  ...
  <Accordion title="How do I set up Tailscale on a VPS and connect from my Mac?">
    ...
    ```bash
    steelengine gateway --tailscale serve
    ```
...
<AccordionGroup>
  <Accordion title="How does SteelEngine load environment variables?">
    SteelEngine reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory.
    - a global fallback `.env` from `~/.steelengine/.env` (`$STEELENGINE_STATE_DIR/.env`).

    Neither `.env` file overrides existing env vars. Provider credential and endpoint-routing keys are an exception for workspace `.env`: keys such as `GEMINI_API_KEY`, `XAI_API_KEY`, `MISTRAL_API_KEY`, or any key ending in `_ENDPOINT` (and other bundled-provider auth or endpoint env vars) are ignored from workspace `.env` and should live in the process environment, `~/.steelengine/.env`, or config `env`.
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.steelengine/.env` so they load even when the service does not inherit your shell env.
    2. Enable shell import (opt-in convenience):
       ...
       ```
       This runs your login shell and imports only missing expected keys (never overrides). Env var equivalents: `STEELENGINE_LOAD_SHELL_ENV=1`, `STEELENGINE_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `steelengine models status` reports whether **shell env import** is enabled. "Shell env: off" does **not** mean your env vars are missing - it just means SteelEngine will not load your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it will not inherit your shell environment. Fix by putting the token in `~/.steelengine/.env`, enabling `env.shellEnv.enabled: true`, or adding it to config `env` (applies only if missing), then restarting the gateway and rechecking:

    ```bash
    steelengine models status
    ```

    Copilot tokens resolve in this order: `STEELENGINE_GITHUB_TOKEN`, then `COPILOT_GITHUB_TOKEN`, then `GH_TOKEN`, then `GITHUB_TOKEN`.
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of SteelEngine instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset SteelEngine but keep it installed?">
    ```bash
    steelengine reset
    ```
    ...
    Onboarding also offers **Reset** if it detects an existing config; see [Onboarding (CLI)](/start/wizard). If you used profiles (`--profile` / `STEELENGINE_PROFILE`), reset each state dir (default `~/.steelengine-<profile>`). Dev-only reset: `steelengine gateway --dev --reset` wipes dev config, credentials, sessions, and workspace.
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    If `HEARTBEAT.md` exists but is effectively empty (only blank lines, Markdown/HTML comments, ATX headings, fence markers, or empty list-item stubs), SteelEngine skips the heartbeat run to save API calls. If the file is missing, the heartbeat still runs and the model decides what to do.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. SteelEngine runs on **your own account** - if you are in the group, SteelEngine can see it. By default, group replies are blocked until you allow senders (`groupPolicy: "allowlist"`).
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    steelengine directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does SteelEngine not reply in a group?">
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth**: active sessions and transcripts live in the per-agent SQLite database; legacy/archive artifacts can still accumulate under `~/.steelengine/agents/<agentId>/sessions/`.
    - **Token cost**: more agents means more concurrent model usage.
    - **Ops overhead**: per-agent auth profiles, workspaces, and channel routing.

    Keep one **active** workspace per agent (`agents.defaults.workspace`), prune old sessions with `steelengine sessions cleanup` if disk grows (do not edit active SQLite state by hand), and use `steelengine doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```text
    --port > STEELENGINE_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does steelengine gateway status say "Runtime: running" but "Connectivity probe: failed"?'>
    "Running" is the **supervisor's** view (launchd/systemd/schtasks); the connectivity probe is the CLI actually connecting to the gateway WebSocket. Trust these lines from `steelengine gateway status`: `Probe target:` (the URL the probe used), `Listening:` (what is actually bound on the port), `Last gateway error:` (common root cause when the process is alive but the port is not listening).
  </Accordion>

  <Accordion title='Why does steelengine gateway status show "Config (cli)" and "Config (service)" different?'>
    You are editing one config file while the service runs another (often a `--profile` / `STEELENGINE_STATE_DIR` mismatch).
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    SteelEngine enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` ("another gateway instance is already listening").

    Fix: stop the other instance, free the port, or run with `steelengine gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run SteelEngine in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `steelengine gateway` only starts when `gateway.mode` is `local` (or you pass an override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `steelengine dashboard` (prints + copies the dashboard URL, tries to open; shows an SSH hint if headless).
    - No token yet: `steelengine doctor --generate-gateway-token`.
    - Remote: tunnel first with `ssh -N -L 18789:127.0.0.1:18789 user@host`, then open `http://127.0.0.1:18789/`.
    - Shared-secret mode: set `gateway.auth.token` / `STEELENGINE_GATEWAY_TOKEN` or `gateway.auth.password` / `STEELENGINE_GATEWAY_PASSWORD`, then paste the matching secret in Control UI settings.
    ...
    - Mismatch persists after the one retry: rotate/re-approve the paired device token:
      ```bash
      steelengine devices list
      steelengine devices rotate --device <id> --role operator
      ```
    ...
    - Still stuck: `steelengine status --all` plus [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    Usually no - one Gateway can run multiple messaging channels and agents. Use multiple Gateways only for redundancy (for example a rescue bot) or hard isolation, and isolate each with its own `STEELENGINE_CONFIG_PATH`, `STEELENGINE_STATE_DIR`, `agents.defaults.workspace`, and unique `gateway.port`.

    Recommended: `steelengine --profile <name> ...` per instance (auto-creates `~/.steelengine-<name>`), a unique `gateway.port` per profile config (or `--port` for manual runs), and a per-profile service with `steelengine --profile <name> gateway install`.

    Profiles also suffix service names: launchd `ai.steelengine.<profile>`, systemd `steelengine-gateway-<profile>.service`, Windows `SteelEngine Gateway (<profile>)`. The unqualified `steelengine-gateway` systemd unit only exists for the default profile; the legacy pre-rename systemd unit name `clawdbot-gateway` is migrated automatically.
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```bash
    steelengine tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    File logs (structured): `/tmp/steelengine/steelengine-YYYY-MM-DD.log`. Set a stable path via `logging.file`; file log level via `logging.level`; console verbosity via `--verbose` and `logging.consoleLevel`.
    ...
    - macOS launchd stdout: `~/Library/Logs/steelengine/gateway.log` (profiles use `gateway-<profile>.log`; stderr is suppressed).
    - Linux: `journalctl --user -u steelengine-gateway[-<profile>].service -n 200 --no-pager`.
    - Windows: `schtasks /Query /TN "SteelEngine Gateway (<profile>)" /V /FO LIST`.
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `steelengine gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart SteelEngine?">
    Three Windows install modes:

    **1) Windows Hub local setup**: the native app manages a local app-owned WSL Gateway. Open **SteelEngine Companion** from the Start menu or tray, then use **Gateway Setup** or the Connections tab.
    ...
    **2) Manual WSL2 Gateway**: the Gateway runs inside Linux.
    ...
    steelengine gateway restart
    ```
    If you never installed the service, start it in the foreground: `steelengine gateway run`.
    ...
    **3) Native Windows CLI/Gateway**: runs directly in Windows.
    ...
    steelengine gateway restart
    ```
    If you run it manually (no service): `steelengine gateway run`.
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    Usually means the UI lost the WebSocket connection. Check: is the Gateway running (`steelengine gateway status`)? Is it healthy (`steelengine status`)? Does the UI have the right token (`steelengine dashboard`)? If remote, is the tunnel/Tailscale link up?
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ```bash
    steelengine channels status
    steelengine channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. SteelEngine already trims to the Telegram limit and retries with fewer commands, but some menu entries may still be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="How do I completely stop then start the Gateway?">
    ...
    In the foreground, stop with Ctrl-C, then `steelengine gateway run`.

    Docs: [Gateway service runbook](/gateway).

  </Accordion>

  <Accordion title="ELI5: steelengine gateway restart vs steelengine gateway">
    `steelengine gateway restart` restarts the **background service** (launchd/systemd). `steelengine gateway` runs the gateway **in the foreground** for this terminal session. Use the gateway subcommands if you installed the service; use the bare foreground run for a one-off.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must use structured media fields such as `media`, `mediaUrl`, `path`, or `filePath`. See [SteelEngine assistant setup](/start/steelengine) and [Agent send](/tools/agent-send).

    ```bash
    steelengine message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose SteelEngine to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults reduce risk:

    - Default behavior on DM-capable channels is **pairing**: unknown senders receive a pairing code and their message is not processed. Approve with `steelengine pairing approve --channel <channel> [--account <id>] <code>`. Pending requests are capped at **3 per channel**; check `steelengine pairing list --channel <channel> [--account <id>]` if a code did not arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `steelengine doctor` to surface risky DM policies.
  ...
  <Accordion title="Is SteelEngine less safe because it uses TypeScript/Node instead of Rust/WASM?">
  ...
  <Accordion title="I saw reports about exposed SteelEngine instances. What should I check?">
    ...
    Public binds without auth, open DMs/groups with tools, and exposed browser control are the findings to fix first. Details: [steelengine security audit](/gateway/security#steelengine-security-audit).

  </Accordion>

  <Accordion title="Are ClawHub skills and third-party plugins safe to install?">
    Treat third-party skills and plugins as code you are choosing to trust. ClawHub skill pages expose scan state before install, but scans are not a complete security boundary. SteelEngine does not run built-in local dangerous-code blocking during plugin/skill install or update; use operator-owned `security.installPolicy` for local allow/block decisions.
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code; their message is **not processed**. SteelEngine only replies to chats it receives or to explicit sends you trigger.

    ```bash
    steelengine pairing approve whatsapp <code>
    steelengine pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    SteelEngine blocks **cross-provider** messaging by default. If a tool call is bound to Telegram, it will not send to Discord unless you explicitly allow it - and this takes effect immediately, no gateway restart needed:
...
Still stuck? Ask in [Discord](https://discord.com/invite/clawd) or open a [GitHub discussion](https://github.com/steelengineai/recall-agents/discussions).
