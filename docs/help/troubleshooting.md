---
summary: "Symptom first troubleshooting hub for Recall"
read_when:
  - Recall is not working and you need the fastest path to a fix
...
- `recall status` → shows configured channels and no obvious auth errors.
- `recall status --all` → full report is present and shareable.
- `recall gateway probe` → expected gateway target is reachable (`Reachable: yes`). `Capability: ...` tells you what auth level the probe could prove, and `Read probe: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `recall gateway status` → `Runtime: running`, `Connectivity probe: ok`, and a plausible `Capability: ...` line. Use `--require-rpc` if you need read-scope RPC proof too.
- `recall doctor` → no blocking config/service errors.
- `recall channels status --probe` → reachable gateway returns live per-account
...
- `recall logs --follow` → steady activity, no repeating fatal errors.
...
and run `recall status --all` again. See [Tools](/tools) for the profile
...
## Local OpenAI-compatible backend works directly but fails in Recall

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `recall infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on Recall agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger Recall prompts crash the
...
## Plugin install fails with missing recall extensions

If install fails with `package.json missing recall.extensions`, the plugin package
is using an old shape that Recall no longer accepts.

Fix in the plugin package:

1. Add `recall.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `recall plugins install <package>` again.

Example:

```json
{
  "name": "@recall/my-plugin",
  "version": "1.2.3",
  "recall": {
...
If `recall doctor`, setup, or startup warnings show:
...
them. Do not remove the plugin config. Fix the file ownership or run Recall as
...
```mermaid
flowchart TD
  A[Recall is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `recall gateway status`
    - `Connectivity probe: ok`
    - `Capability: read-only`, `write-capable`, or `admin-capable`
  ...
  <Accordion title="Exec suddenly asks for approval">
    ```bash
    recall config get tools.exec.host
    recall config get tools.exec.security
    recall config get tools.exec.ask
    ...
    ```bash
    recall config set tools.exec.host gateway
    recall config set tools.exec.security full
    recall config set tools.exec.ask off
    recall gateway restart
    ```
  ...
  <Accordion title="Browser tool fails">
    ...
    - `recall` starts, or `user` can see local Chrome tabs.
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `recall browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
