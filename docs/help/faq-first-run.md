...
read_when:
  ...
  - Cannot access docs.steelengine.ai, cannot open dashboard, install stuck
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    **local config or environment issues** a remote helper cannot inspect, so this beats
    ...
    | Command | Shows |
    | --- | --- |
    | `steelengine status` | Gateway/agent health + basic config snapshot |
    | `steelengine status --all` | Full read-only diagnosis, pasteable |
    | `steelengine models status` | Provider auth + model availability |
    | `steelengine doctor` | Validates and repairs common config/state issues |
    | `steelengine logs --follow` | Live log tail |
    | `steelengine gateway status --deep` | Deep gateway/config/plugin health check |
    | `steelengine health --verbose` | Detailed health report |

    Found a real bug or fix? File an issue or send a PR:
    [Issues](https://github.com/steelengineai/recall-agents/issues) /
    [Pull requests](https://github.com/steelengineai/recall-agents/pulls).
  ...
  <Accordion title="Recommended way to install and set up SteelEngine">
    ```bash
    ...
    No global install yet? Run `pnpm steelengine onboard` instead. If Control UI assets are
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `STEELENGINE_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `STEELENGINE_GATEWAY_PASSWORD`).
    - No shared secret configured yet? Run `steelengine doctor --generate-gateway-token` (or `steelengine doctor --fix --generate-gateway-token`).

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `steelengine gateway --tailscale serve`, open `https://<magicdns>/`. With `gateway.auth.allowTailscale: true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes a trusted gateway host); HTTP APIs still need shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `steelengine gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - When a supported native channel can infer approvers safely, SteelEngine auto-enables DM-first native approvals if `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="What runtime do I need?">
    ...
    Bun can install dependencies and run package scripts, but it cannot run the SteelEngine CLI or Gateway because it lacks `node:sqlite`.
  </Accordion>
  ...
  <Accordion title="It is stuck on wake up my friend / onboarding will not hatch. What now?">
    ...
    TUI without sending anything — add a provider with `steelengine configure --section model`.
  ...
  <Accordion title="Can I migrate my setup to a new machine without redoing onboarding?">
    ...
    1. Install SteelEngine on the new machine.
    2. Copy `$STEELENGINE_STATE_DIR` (default: `~/.steelengine`) from the old machine.
    3. Copy your workspace (default: `~/.steelengine/workspace`).
    4. Run `steelengine doctor` and restart the Gateway service.
    ...
    **memory + bootstrap files**, but not session history or auth. Those live under
    `~/.steelengine/` (for example `~/.steelengine/agents/<agentId>/agent/steelengine-agent.sqlite`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/steelengineai/recall-agents/blob/main/CHANGELOG.md](https://github.com/steelengineai/recall-agents/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.steelengine.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.steelengine.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.steelengine.ai`, then retry. Help us
    ...
    [https://github.com/steelengineai/recall-agents/tree/main/docs](https://github.com/steelengineai/recall-agents/tree/main/docs)
  ...
  <Accordion title="Difference between stable and beta">
    ...
    **same version** after promotion.
    ...
    See what changed: [CHANGELOG.md](https://github.com/steelengineai/recall-agents/blob/main/CHANGELOG.md).
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published to npm it uses dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://steelengine.ai/install.sh | bash -s -- --beta
    ```
    ...
    Windows installer (PowerShell): `iwr -useb https://steelengine.ai/install.ps1 | iex`
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://steelengine.ai/install.sh | bash -s -- --verbose
    curl -fsSL --proto '=https' --tlsv1.2 https://steelengine.ai/install.sh | bash -s -- --beta --verbose
    curl -fsSL --proto '=https' --tlsv1.2 https://steelengine.ai/install.sh | bash -s -- --install-method git --verbose
    ```
  ...
  <Accordion title="Windows install says git not found or steelengine not recognized">
    ...
    **2) steelengine is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    Still reproducing this on latest SteelEngine? Track/report it: [Issue #30640](https://github.com/steelengineai/recall-agents/issues/30640).
  ...
  <Accordion title="How do I install SteelEngine on Linux?">
  ...
  <Accordion title="How do I install SteelEngine on a VPS?">
  ...
  <Accordion title="Can I ask SteelEngine to update itself?">
    ...
    steelengine update --channel stable|extended-stable|beta|dev
    steelengine update --tag <dist-tag|version>
    steelengine update --no-restart
    ```

    Automating from an agent:

    ```bash
    steelengine update --yes --no-restart
    steelengine gateway restart
    ```
  ...
  <Accordion title="What does onboarding actually do?">
    `steelengine onboard` is the recommended setup path. In **local mode** it walks through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. Run SteelEngine with **API keys** (Anthropic/OpenAI/others) or **local-only models**
    ...
    SteelEngine also supports hosted subscription-style options including **Qwen Cloud
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    Yes. SteelEngine supports Claude CLI reuse for Pro/Max/Team/Enterprise plans. Anthropic
    currently treats the `claude -p` path SteelEngine uses as subscription-plan usage subject
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic setup-token auth is also still a supported token path, but SteelEngine prefers
...
<AccordionGroup>
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Set a **fallback model** so SteelEngine keeps replying while a provider is rate-limited.
  ...
  <Accordion title="Is AWS Bedrock supported?">
    Yes. SteelEngine has a bundled **Amazon Bedrock (Converse)** provider. With AWS env
    markers present (`AWS_ACCESS_KEY_ID`, `AWS_PROFILE`, `AWS_BEARER_TOKEN_BEDROCK`),
    SteelEngine auto-enables the implicit Bedrock provider for model discovery; otherwise
  ...
  <Accordion title="How does Codex auth work?">
    SteelEngine supports **OpenAI Codex** via OAuth (ChatGPT sign-in). A fresh
    ...
    `openai/gpt-5.5` explicitly; SteelEngine does not silently downgrade. Legacy
    Codex-prefixed model refs are legacy config repaired by `steelengine doctor
  ...
  <Accordion title="Why does SteelEngine still mention legacy OpenAI Codex prefix?">
    ...
    - Legacy `openai-codex/*` model refs = legacy route repaired by `steelengine doctor --fix`.
    - `openai/gpt-5.5` plus an ordered `openai` API-key profile = API-key auth for an OpenAI agent model.
    - Legacy `openai-codex` auth profile ids = legacy ids migrated by `steelengine doctor --fix`.

    Want direct OpenAI Platform billing? Set `OPENAI_API_KEY`. Want ChatGPT/Codex
    subscription auth? Run `steelengine models auth login --provider openai`. Keep
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    `steelengine models status` shows the currently visible provider usage/quota windows, but
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    ...
    tools/workflows like SteelEngine. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `steelengine.json`.

    1. Install Gemini CLI locally so `gemini` is on `PATH`:
       - Homebrew: `brew install gemini-cli`
       - npm: `npm install -g @google/gemini-cli`
    2. Enable the plugin: `steelengine plugins enable google`
    3. Login: `steelengine models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. SteelEngine needs large context + strong safety; small cards truncate context
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. SteelEngine runs on macOS or Linux (Windows via WSL2). A Mac mini is a popular
  ...
  <Accordion title="If I buy a Mac mini to run SteelEngine, can I connect it to my MacBook Pro?">
    ...
    node host and pairs to the Gateway. Check with `steelengine nodes status` / `steelengine nodes list`.
  ...
  <Accordion title="Can I use Bun?">
    You can use Bun to install dependencies or run package scripts. The SteelEngine CLI and
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    not the bot username. Setup asks for numeric user IDs only; `steelengine doctor --fix`
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different SteelEngine instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    Running SteelEngine via systemd: make sure the service PATH includes
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    Yes, with `steelengine update --channel ...` on an existing install. This does **not
    delete your data** - only the SteelEngine code install changes. State (`~/.steelengine`) and
    workspace (`~/.steelengine/workspace`) stay untouched.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://steelengine.ai/install.sh | bash -s -- --install-method git
    curl -fsSL --proto '=https' --tlsv1.2 https://steelengine.ai/install.sh | bash -s -- --install-method npm
    ```
  ...
  <Accordion title="How important is it to run SteelEngine on a dedicated machine?">
  ...
  <Accordion title="Can I run SteelEngine in a VM and what are the requirements?">
