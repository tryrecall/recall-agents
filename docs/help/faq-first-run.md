...
read_when:
  ...
  - Cannot access docs.recall.ai, cannot open dashboard, install stuck
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    [https://github.com/tryrecall/recall-agents/issues](https://github.com/tryrecall/recall-agents/issues)
    [https://github.com/tryrecall/recall-agents/pulls](https://github.com/tryrecall/recall-agents/pulls)
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `RECALL_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `RECALL_GATEWAY_PASSWORD`).
    - If no shared secret is configured yet, generate a token with `recall doctor --generate-gateway-token`.

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `recall gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes trusted gateway host); HTTP APIs still require shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `recall gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, then paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - If a supported native channel can infer approvers safely, Recall now auto-enables DM-first native approvals when `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/tryrecall/recall-agents/blob/main/CHANGELOG.md](https://github.com/tryrecall/recall-agents/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.recall.ai (SSL error)">
    ...
    Advanced Security. Disable it or allowlist `docs.recall.ai`, then retry.
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/tryrecall/recall-agents/tree/main/docs](https://github.com/tryrecall/recall-agents/tree/main/docs)
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    - [Issue #30640](https://github.com/tryrecall/recall-agents/issues/30640)
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    ...
    **local-only models** so your data stays on your device. Subscriptions (Claude
    ...
    For Anthropic in Recall, the practical split is:

    - **Anthropic API key**: normal Anthropic API billing
    - **Claude CLI / Claude subscription auth in Recall**: Anthropic staff
      told us this usage is allowed again, and Recall is treating `claude -p`
    ...
    Recall also supports other hosted subscription-style options including
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    ...
    Recall treats Claude subscription auth and `claude -p` usage as sanctioned
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic staff told us this usage is allowed again, so Recall treats
    Claude CLI reuse and `claude -p` usage as sanctioned for this integration
    unless Anthropic publishes a new policy.

    Anthropic setup-token is still available as a supported Recall token path, but Recall now prefers Claude CLI reuse and `claude -p` when available.
    ...
    options in Recall, see [OpenAI](/providers/openai), [Qwen / Model
...
<AccordionGroup>
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Recall Claude-login path with Extra Usage enabled).
  ...
  <Accordion title="How does Codex auth work?">
    ...
    legacy config repaired by `recall doctor --fix`. Direct OpenAI API-key
  ...
  <Accordion title="Why does Recall still mention openai-codex?">
    ...
    - `openai-codex/gpt-5.5` = legacy model route repaired by `recall doctor --fix`
    ...
    `recall models auth login --provider openai-codex`. Keep the model ref as
    `openai/gpt-5.5`; `openai-codex/*` model refs are legacy config that
    `recall doctor --fix` rewrites.
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    Recall can show the currently visible provider usage/quota windows in
    `recall models status`, but it does not invent or normalize ChatGPT-web
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    ...
    2. Enable the plugin: `recall plugins enable google`
    3. Login: `recall models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Recall needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** model build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Setup asks for numeric user IDs only. If you already have legacy `@username` entries in config, `recall doctor --fix` can try to resolve them.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    Yes. Use `recall update --channel ...` when Recall is already installed.
    This **does not delete your data** - it only changes the Recall code install.
    Your state (`~/.recall`) and workspace (`~/.recall/workspace`) stay untouched.
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Recall-specific note:** WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).
