...
This is handy when you need to toggle obscure settings without editing `recall.json`.
...
Use `RECALL_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
RECALL_PLUGIN_LIFECYCLE_TRACE=1 recall plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm recall ...`
...
`RECALL_RUN_NODE_CPU_PROF_DIR`:

```bash
RECALL_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm recall status
```
...
```bash
RECALL_TRACE_SYNC_IO=1 pnpm recall gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched
Gateway child. Set `RECALL_TRACE_SYNC_IO=1` when you explicitly want Node
...
`recall-gateway-watch-main` (or a profile/port-specific variant such as
`recall-gateway-watch-dev-19001`) and auto-attaches from interactive terminals.
...
```bash
tmux attach -t recall-gateway-watch-main
```
...
# or
RECALL_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Disable auto-attach while keeping tmux management:

```bash
RECALL_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
`RECALL_TRACE_SYNC_IO=1` with `--benchmark` when you explicitly want both CPU
...
`RECALL_PROFILE`, `RECALL_CONFIG_PATH`, `RECALL_STATE_DIR`,
`RECALL_GATEWAY_PORT`, and `RECALL_SKIP_CHANNELS` into the pane. Put
...
`recall doctor --fix --non-interactive` once and restarts the Gateway child.
Use `RECALL_GATEWAY_WATCH_AUTO_DOCTOR=0` when you want the original startup
...
extension `package.json` and `recall.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.recall-dev` and
...
```bash
pnpm gateway:dev
RECALL_PROFILE=dev recall tui
```

If you don't have a global install yet, run the CLI via `pnpm recall ...`.
...
1. **Profile isolation** (global `--dev`)
   ...
   - `RECALL_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`RECALL_SKIP_CHANNELS=1`).
...
```bash
RECALL_PROFILE=dev recall gateway --dev --reset
```
...
## Raw stream logging (Recall)

Recall can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.recall/logs/raw-stream.jsonl
```
...
```bash
RECALL_RAW_STREAM=1
RECALL_RAW_STREAM_PATH=~/.recall/logs/raw-stream.jsonl
```

Default file:

`~/.recall/logs/raw-stream.jsonl`
...
- If you need to use the built Recall CLI for other tasks (i.e. `dashboard --no-open` if your debug session spawns a new auth token), you can execute it in another terminal as `node ./recall.mjs` or create a shell alias like `alias recall-build="node $(pwd)/recall.mjs"`
