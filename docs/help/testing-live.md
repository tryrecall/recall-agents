...
```bash
pnpm recall infer tts convert --local --json \
  --text "Recall live smoke." \
  --output /tmp/recall-live-smoke.mp3
```

Safe voice-call readiness smoke:

```bash
pnpm recall voicecall setup --json
pnpm recall voicecall smoke --to "+15555550123"
```
...
- Optional target overrides:
  - `RECALL_ANDROID_NODE_ID` or `RECALL_ANDROID_NODE_NAME`.
  - `RECALL_ANDROID_GATEWAY_URL` / `RECALL_ANDROID_GATEWAY_TOKEN` / `RECALL_ANDROID_GATEWAY_PASSWORD`.
...
- How to enable:
  - `pnpm test:live` (or `RECALL_LIVE_TEST=1` if invoking Vitest directly)
- Set `RECALL_LIVE_MODELS=modern` (or `all`, alias for modern) to actually run this suite; otherwise it skips to keep `pnpm test:live` focused on gateway smoke
- How to select models:
  - `RECALL_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet 4.6+, GPT-5.2 + Codex, Gemini 3, DeepSeek V4, GLM 4.7, MiniMax M2.7, Grok 4.3)
  - `RECALL_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `RECALL_LIVE_MODELS="openai/gpt-5.5,openai-codex/gpt-5.5,anthropic/claude-opus-4-6,..."` (comma allowlist)
  - Modern/all sweeps default to a curated high-signal cap; set `RECALL_LIVE_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
  - Exhaustive sweeps use `RECALL_LIVE_TEST_TIMEOUT_MS` for the whole direct-model test timeout. Default: 60 minutes.
  - Direct-model probes run with 20-way parallelism by default; set `RECALL_LIVE_MODEL_CONCURRENCY` to override.
- How to select providers:
  - `RECALL_LIVE_PROVIDERS="google,google-antigravity,google-gemini-cli"` (comma allowlist)
- Where keys come from:
  - By default: profile store and env fallbacks
  - Set `RECALL_LIVE_REQUIRE_PROFILE_KEYS=1` to enforce **profile store** only
- Why this exists:
  - Separates "provider API is broken / key is invalid" from "gateway agent pipeline is broken"
  - Contains small, isolated regressions (example: OpenAI Responses/Codex Responses reasoning replay + tool-call flows)

### Layer 2: Gateway + dev agent smoke (what "@recall" actually does)
...
- How to select models:
  ...
  - `RECALL_LIVE_GATEWAY_MODELS=all` is an alias for the modern allowlist
  - Or set `RECALL_LIVE_GATEWAY_MODELS="provider/model"` (or comma list) to narrow
  - Modern/all gateway sweeps default to a curated high-signal cap; set `RECALL_LIVE_GATEWAY_MAX_MODELS=0` for an exhaustive modern sweep or a positive number for a smaller cap.
- How to select providers (avoid "OpenRouter everything"):
  - `RECALL_LIVE_GATEWAY_PROVIDERS="google,google-antigravity,google-gemini-cli,openai,anthropic,zai,minimax"` (comma allowlist)
...
- Overrides (optional):
  - `RECALL_LIVE_CLI_BACKEND_MODEL="claude-cli/claude-sonnet-4-6"`
  - `RECALL_LIVE_CLI_BACKEND_COMMAND="/full/path/to/claude"`
  - `RECALL_LIVE_CLI_BACKEND_ARGS='["-p","--output-format","json"]'`
  - `RECALL_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt). Docker recipes default this off unless explicitly requested.
  - `RECALL_LIVE_CLI_BACKEND_IMAGE_ARG="--image"` to pass image file paths as CLI args instead of prompt injection.
  - `RECALL_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"` (or `"list"`) to control how image args are passed when `IMAGE_ARG` is set.
  - `RECALL_LIVE_CLI_BACKEND_RESUME_PROBE=1` to send a second turn and validate resume flow.
  - `RECALL_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1` to opt into the Claude Sonnet -> Opus same-session continuity probe when the selected model supports a switch target. Docker recipes default this off for aggregate reliability.
  - `RECALL_LIVE_CLI_BACKEND_MCP_PROBE=1` to opt into the MCP/tool loopback probe. Docker recipes default this off unless explicitly requested.
...
settings Recall gives Gemini, then runs `gemini --debug mcp list` to prove a
...
- It resolves CLI smoke metadata from the owning extension, then installs the matching Linux CLI package (`@anthropic-ai/claude-code` or `@google/gemini-cli`) into a cached writable prefix at `RECALL_DOCKER_CLI_TOOLS_DIR` (default: `~/.cache/recall/docker-cli-tools`).
...
- Enable:
  - `RECALL_LIVE_TEST=1 RECALL_LIVE_APNS_REACHABILITY=1 pnpm test:live src/infra/push-apns-http2.live.test.ts`
- Optional timeout:
  - `RECALL_LIVE_APNS_TIMEOUT_MS=30000`
...
- Defaults:
  ...
  - ACP backend: `acpx`
- Overrides:
  - `RECALL_LIVE_ACP_BIND_AGENT=claude`
  - `RECALL_LIVE_ACP_BIND_AGENT=codex`
  - `RECALL_LIVE_ACP_BIND_AGENT=droid`
  - `RECALL_LIVE_ACP_BIND_AGENT=gemini`
  - `RECALL_LIVE_ACP_BIND_AGENT=opencode`
  - `RECALL_LIVE_ACP_BIND_AGENTS=claude,codex,gemini`
  - `RECALL_LIVE_ACP_BIND_AGENT_COMMAND='npx -y @agentclientprotocol/claude-agent-acp@<version>'`
  - `RECALL_LIVE_ACP_BIND_CODEX_MODEL=gpt-5.5`
  - `RECALL_LIVE_ACP_BIND_OPENCODE_MODEL=opencode/kimi-k2.6`
  - `RECALL_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1`
  - `RECALL_LIVE_ACP_BIND_REQUIRE_CRON=1`
  - `RECALL_LIVE_ACP_BIND_PARENT_MODEL=openai/gpt-5.5`
- Notes:
  - This lane uses the gateway `chat.send` surface with admin-only synthetic originating-route fields so tests can attach message-channel context without pretending to deliver externally.
  - When `RECALL_LIVE_ACP_BIND_AGENT_COMMAND` is unset, the test uses the embedded `acpx` plugin's built-in agent registry for the selected ACP harness agent.
  - Bound-session cron MCP creation is best-effort by default because external ACP harnesses can cancel MCP calls after the bind/image proof has passed; set `RECALL_LIVE_ACP_BIND_REQUIRE_CRON=1` to make that post-bind cron probe strict.
...
- Use `RECALL_LIVE_ACP_BIND_AGENTS=claude`, `RECALL_LIVE_ACP_BIND_AGENTS=codex`, `RECALL_LIVE_ACP_BIND_AGENTS=droid`, `RECALL_LIVE_ACP_BIND_AGENTS=gemini`, or `RECALL_LIVE_ACP_BIND_AGENTS=opencode` to narrow the matrix.
...
- The OpenCode Docker variant is a strict single-agent regression lane. It writes a temporary `OPENCODE_CONFIG_CONTENT` default model from `RECALL_LIVE_ACP_BIND_OPENCODE_MODEL` (default `opencode/kimi-k2.6`), and `pnpm test:docker:live-acp-bind:opencode` requires a bound assistant transcript instead of accepting the generic post-bind skip.
- Direct `acpx` CLI calls are only a manual/workaround path for comparing behavior outside the Gateway. The Docker ACP bind smoke exercises Recall's embedded `acpx` runtime backend.
...
- Goal: validate the plugin-owned Codex harness through the normal gateway
  ...
  - send a second turn to the same Recall session and verify the app-server
    thread can resume
...
- Enable: `RECALL_LIVE_CODEX_HARNESS=1`
- Default model: `openai/gpt-5.5`
- Optional image probe: `RECALL_LIVE_CODEX_HARNESS_IMAGE_PROBE=1`
- Optional MCP/tool probe: `RECALL_LIVE_CODEX_HARNESS_MCP_PROBE=1`
- Optional Guardian probe: `RECALL_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1`
...
```bash
RECALL_LIVE_CODEX_HARNESS=1 \
  RECALL_LIVE_CODEX_HARNESS_IMAGE_PROBE=1 \
  RECALL_LIVE_CODEX_HARNESS_MCP_PROBE=1 \
  RECALL_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1 \
  RECALL_LIVE_CODEX_HARNESS_MODEL=openai/gpt-5.5 \
...
- Docker enables the image, MCP/tool, and Guardian probes by default. Set
  `RECALL_LIVE_CODEX_HARNESS_IMAGE_PROBE=0` or
  `RECALL_LIVE_CODEX_HARNESS_MCP_PROBE=0` or
  `RECALL_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0` when you need a narrower debug
  run.
...
- Single model, direct (no gateway):
  - `RECALL_LIVE_MODELS="openai/gpt-5.5" pnpm test:live src/agents/models.profiles.live.test.ts`

- Single model, gateway smoke:
  - `RECALL_LIVE_GATEWAY_MODELS="openai/gpt-5.5" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `RECALL_LIVE_GATEWAY_MODELS="openai/gpt-5.5,openai-codex/gpt-5.5,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,deepseek/deepseek-v4-flash,zai/glm-5.1,minimax/MiniMax-M2.7" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google focus (Gemini API key + Antigravity):
  - Gemini (API key): `RECALL_LIVE_GATEWAY_MODELS="google/gemini-3-flash-preview" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
  - Antigravity (OAuth): `RECALL_LIVE_GATEWAY_MODELS="google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-pro-high" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Google adaptive thinking smoke:
  - Gemini 3 dynamic default: `pnpm recall qa manual --provider-mode live-frontier --model google/gemini-3.1-pro-preview --alt-model google/gemini-3.1-pro-preview --message '/think adaptive Reply exactly: GEMINI_ADAPTIVE_OK' --timeout-ms 180000`
  - Gemini 2.5 dynamic budget: `pnpm recall qa manual --provider-mode live-frontier --model google/gemini-2.5-flash --alt-model google/gemini-2.5-flash --message '/think adaptive Reply exactly: GEMINI25_ADAPTIVE_OK' --timeout-ms 180000`
...
- Gemini API vs Gemini CLI:
  - API: Recall calls Google's hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by "Gemini".
  - CLI: Recall shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
Include at least one image-capable model in `RECALL_LIVE_GATEWAY_MODELS` (Claude/Gemini/OpenAI vision-capable variants, etc.) to exercise the image probe.

### Aggregators / alternate gateways

If you have keys enabled, we also support testing via:

- OpenRouter: `openrouter/...` (hundreds of models; use `recall models scan` to find tool+image capable candidates)
...
- If a live test says "no creds", debug the same way you'd debug `recall models list` / model selection.

- Per-agent auth profiles: `~/.recall/agents/<agentId>/agent/auth-profiles.json` (this is what "profile keys" means in the live tests)
- Config: `~/.tryrecall/recall-agents.json` (or `RECALL_CONFIG_PATH`)
- Legacy state dir: `~/.recall/credentials/` (copied into the staged live home when present, but not the main profile-key store)
...
Docker runners below with an explicit `RECALL_PROFILE_FILE`.
...
- Enable: `RECALL_LIVE_TEST=1 COMFY_LIVE_TEST=1 pnpm test:live -- extensions/comfy/comfy.live.test.ts`
- Scope:
...
- Current bundled providers covered:
  ...
  - `xai`
- Optional narrowing:
  - `RECALL_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google,openrouter,xai"`
  - `RECALL_LIVE_IMAGE_GENERATION_PROVIDERS="deepinfra"`
  - `RECALL_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-2,google/gemini-3.1-flash-image-preview,openrouter/google/gemini-3.1-flash-image-preview,xai/grok-imagine-image"`
  - `RECALL_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit,openrouter:generate,xai:default-generate,xai:default-edit"`
- Optional auth behavior:
  - `RECALL_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides

For the shipped CLI path, add an `infer` smoke after the provider/runtime live
test passes:

```bash
RECALL_LIVE_TEST=1 RECALL_LIVE_INFER_CLI_TEST=1 pnpm test:live -- test/image-generation.infer-cli.live.test.ts
recall infer image providers --json
recall infer image generate \
  --model google/gemini-3.1-flash-image-preview \
  --prompt "Minimal flat test image: one blue square on a white background, no text." \
  --output ./recall-infer-image-smoke.png \
  --json
```
...
- Enable: `RECALL_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts`
...
- Optional narrowing:
  - `RECALL_LIVE_MUSIC_GENERATION_PROVIDERS="google,minimax"`
  - `RECALL_LIVE_MUSIC_GENERATION_MODELS="google/lyria-3-clip-preview,minimax/music-2.6"`
- Optional auth behavior:
  - `RECALL_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Enable: `RECALL_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts`
- Harness: `pnpm test:live:media video`
- Scope:
  - Exercises the shared bundled video-generation provider path
  - Defaults to the release-safe smoke path: non-FAL providers, one text-to-video request per provider, one-second lobster prompt, and a per-provider operation cap from `RECALL_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default)
  - Skips FAL by default because provider-side queue latency can dominate release time; pass `--video-providers fal` or `RECALL_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` to run it explicitly
  - Uses already-exported provider env vars before probing
  - Uses live/env API keys ahead of stored auth profiles by default, so stale test keys in `auth-profiles.json` do not mask real shell credentials
  - Skips providers with no usable auth/profile/model
  - Runs only `generate` by default
  - Set `RECALL_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared transform modes when available:
  ...
  - Provider-specific Vydra coverage:
    - `RECALL_LIVE_TEST=1 RECALL_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
...
- Optional narrowing:
  - `RECALL_LIVE_VIDEO_GENERATION_PROVIDERS="deepinfra,google,openai,runway"`
  - `RECALL_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
  - `RECALL_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS=""` to include every provider in the default sweep, including FAL
  - `RECALL_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000` to reduce each provider operation cap for an aggressive smoke run
- Optional auth behavior:
  - `RECALL_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
