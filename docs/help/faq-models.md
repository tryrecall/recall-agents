...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    ...
    Models are referenced as `provider/model` (example: `openai/gpt-5.5` or `anthropic/claude-sonnet-4-6`). If you omit the provider, Recall first tries an alias, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider as a deprecated compatibility path. If that provider no longer exposes the configured default model, Recall falls back to the first configured provider/model instead of surfacing a stale removed-provider default. You should still **explicitly** set `provider/model`.
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    - edit `agents.defaults.model` in `~/.tryrecall/recall-agents.json`
  ...
  <Accordion title="If two providers expose the same model id, which one does /model use?">
    ...
    For example, `qianfan/deepseek-v4-flash` and `deepseek/deepseek-v4-flash` are different model refs even though both contain `deepseek-v4-flash`. Recall should not silently switch from one provider to the other just because the bare model id matches.
    ...
    If a run that started from a non-session override is allowed to use fallback, Recall tries the requested provider/model first, then configured fallbacks, and only then the configured primary. That prevents duplicate bare model ids from jumping directly back to the default provider.
  ...
  <Accordion title="Can I use GPT 5.5 for daily tasks and Codex 5.5 for coding?">
    ...
    - **Native Codex coding agent:** set `agents.defaults.model.primary` to `openai/gpt-5.5`. Sign in with `recall models auth login --provider openai-codex` when you want ChatGPT/Codex subscription auth.
  ...
  <Accordion title='Why do I see "Model ... is not allowed" and then no reply?'>
    ...
    Add it with: recall config set agents.defaults.models '{"provider/model":{}}' --strict-json --merge
    ```
  ...
  <Accordion title="How do I add models from other providers like OpenRouter or Z.AI?">
    ...
    **No API key found for provider after adding a new agent**
    ...
    - For OAuth profiles, sign in from the new agent when it needs its own account; otherwise Recall can read through to the default/main agent without cloning refresh tokens.
...
<AccordionGroup>
  <Accordion title="How does failover work?">
    ...
    The rate-limit bucket includes more than plain `429` responses. Recall
    ...
    explicit billing text on `401` or `403`, Recall can still keep that in
    ...
    `organization spending limit exceeded`), Recall treats it as
    ...
    unknown/error in it". Recall does treat provider-scoped transient shapes
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist for "No credentials found for profile anthropic"**
    ...
    - **Use Claude CLI**
      - Run `recall models auth login --provider anthropic --method cli --set-default` on the gateway host.
...
<AccordionGroup>
  <Accordion title="What is an auth profile?">
    ...
    To inspect saved profiles without dumping secrets, run `recall models auth list` (optionally `--provider <id>` or `--json`). See [Models CLI](/cli/models#auth-profiles) for details.
