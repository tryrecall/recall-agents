...
summary: "Build, test, and publish custom SKILL.md workspace skills for your SteelEngine agents."
read_when:
...
SteelEngine loads skills from several roots in a defined [precedence order](/tools/skills#loading-order).
...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.steelengine/workspace/skills/hello-world
    ```
    ...
    ```bash
    mkdir -p ~/.steelengine/workspace/skills/personal/hello-world
  ...
  <Step title="Verify the skill loaded">
    ...
    SteelEngine watches `SKILL.md` files under skills roots by default. If the
  ...
  <Step title="Test it">
    ```bash
    steelengine agent --message "give me a greeting"
    ```
...
metadata: { "steelengine": { "requires": { "bins": ["gemini"] }, "primaryEnv": "GEMINI_API_KEY" } }
---
```
...
<AccordionGroup>
  <Accordion title="Gating options">
    ...
    | `requires.config` | Each `steelengine.json` path must be truthy |
  ...
  <Accordion title="Environment and API keys">
    Wire an API key to a skill entry in `steelengine.json`:
...
steelengine skills workshop propose-update hello-world \
  --proposal ./PROPOSAL.md \
...
<Steps>
  <Step title="Ensure your SKILL.md is complete">
    Make sure `name`, `description`, and any `metadata.steelengine` gating fields
...
<Tip>
  ...
  - **Test locally** — use `steelengine agent --message "..."` before sharing.
