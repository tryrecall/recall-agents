...
read_when:
  - Running Recall Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, Recall Gateway runs inside WSL2, Chrome runs on Windows, and browser control must cross the WSL2 and Windows boundary. The layered failure pattern from [issue #39369](https://github.com/tryrecall/recall-agents/issues/39369) means several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Recall and Chrome are on the same machine
...
- Recall points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Recall is not the problem yet.
...
Fix that before touching Recall config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Recall at the address that is reachable from WSL2:
...
- use HTTP(S) when you want Recall to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting Recall to succeed
...
```bash
recall browser open https://example.com --browser-profile remote
recall browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `recall browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `recall browser stop --browser-profile remote`
...
3. Recall config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Recall config or Control UI auth
