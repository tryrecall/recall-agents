...
Recall also includes `x_search` for X (formerly Twitter) posts and
...
Direct OpenAI Responses models use OpenAI's hosted `web_search` tool automatically when Recall web search is enabled and no managed provider is pinned. This is provider-owned behavior in the bundled OpenAI plugin and only applies to native OpenAI API traffic, not OpenAI-compatible proxy base URLs or Azure routes. Set `tools.web.search.provider` to another provider such as `brave` to keep the managed `web_search` tool for OpenAI models, or set `tools.web.search.enabled: false` to disable both managed search and native OpenAI search.

## Native Codex web search

Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of Recall's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, Recall keeps the normal managed `web_search` behavior.

## Network safety

Managed `web_search` provider calls use Recall's guarded fetch path. For
trusted provider API hosts, Recall allows Surge, Clash, and sing-box fake-IP
...
If no `provider` is set, Recall checks providers in this order and uses the
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, Recall resolves only the
...
Grok can also reuse an xAI OAuth auth profile from `recall models auth login
...
Recall keeps startup resilient and reports a warning so you can reinstall the
plugin or run `recall doctor --fix` to clean up the stale config.

`web_fetch` fallback provider selection is separate:

- choose it with `tools.web.fetch.provider`
- or omit that field and let Recall auto-detect the first ready web-fetch
...
When you choose **Kimi** during `recall onboard` or
`recall configure --section web`, Recall can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `recall doctor --fix`.
When you choose Grok during `recall onboard` or `recall configure --section web`,
Recall can also offer optional `x_search` setup with the same credential.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, Recall does not
...
optional structured filters. Recall only enables the built-in xAI `x_search`
