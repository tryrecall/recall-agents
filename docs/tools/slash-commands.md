...
the SteelEngine command handler, and `/status` plus `/unfocus` stay local whenever
...
<ParamField path="commands.config" type="boolean" default="false">
  Enables `/config` (reads/writes `steelengine.json`). Owner-only.
</ParamField>

<ParamField path="commands.mcp" type="boolean" default="false">
  Enables `/mcp` (reads/writes SteelEngine-managed MCP config under `mcp.servers`). Owner-only.
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Discovery and status">
    ...
    | `/steelengine <request>` | Run the SteelEngine setup and repair helper from an owner DM |
  ...
  <Accordion title="Owner-only writes and admin">
    ...
    | --- | --- | --- |
    | `/config show\|get\|set\|unset` | `commands.config: true` | Read or write `steelengine.json`. Owner-only |
    | `/mcp show\|get\|set\|unset` | `commands.mcp: true` | Read or write SteelEngine-managed MCP server config. Owner-only |
    ...
    | `/restart` | `commands.restart: true` (default) | Restart SteelEngine |
...
/config set messages.responsePrefix="[steelengine]"
...
`/mcp` stores config in SteelEngine config, not embedded-agent project settings.
...
/plugins install npm:@steelengine/<official-package>
...
<AccordionGroup>
  ...
  <Accordion title="Slack specifics">
    `channels.slack.slashCommand` supports a single `/steelengine`-style command.
...
- **Execution vs runtime:** `/status` reports `Execution` for the effective sandbox path and `Runtime` for who is running the session: `SteelEngine Default`, `OpenAI Codex`, a CLI backend, or an ACP backend.
