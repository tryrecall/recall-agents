...
read_when:
  ...
  - Debugging why steelengine is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (SteelEngine-managed)"
---

SteelEngine can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls. It runs through a small local control service inside the Gateway (loopback only) and is isolated from your personal browser.

- Think of it as a **separate, agent-only browser**. The `steelengine` profile never touches your personal browser profile.
...
- A separate browser profile named **steelengine** (orange accent by default).
...
If `steelengine browser` is missing entirely, or the agent says the browser tool
...
Defaults need both `plugins.entries.browser.enabled` **and** `browser.enabled=true`. Disabling only the plugin removes the `steelengine browser` CLI, `browser.request` gateway method, agent tool, and control service as one unit; your `browser.*` config stays intact for a replacement.
...
If `steelengine browser` is unknown after an upgrade, `browser.request` is missing, or the agent reports the browser tool as unavailable, the usual cause is a `plugins.allow` list that omits `browser` and no root `browser` config block exists. Add it:
...
## Profiles: `steelengine`, `user`, `chrome`

- `steelengine`: managed, isolated browser (no extension required).
- `user`: built-in Chrome DevTools MCP attach profile for your **real
  ...
  prompt the first time SteelEngine attaches, so someone must be at the computer.
- `chrome`: built-in [Chrome extension](/tools/chrome-extension) profile for
  ...
  desk because it drives tabs through the SteelEngine browser extension instead of
  the remote-debugging port, so there is no "Allow remote debugging?" prompt.

For agent browser tool calls:

- Default: use the isolated `steelengine` browser.
...
Set `browser.defaultProfile: "steelengine"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.steelengine/steelengine.json`.
...
but SteelEngine does not automatically attach them to channel replies. To share a
...
understanding model is configured, SteelEngine keeps the normal image result so the
...
- Control service binds to loopback on a port derived from `gateway.port` (default `18791` = gateway + 2). `STEELENGINE_GATEWAY_PORT` takes priority over `gateway.port`; either shifts the derived ports in the same family.
- Local `steelengine` profiles auto-assign `cdpPort`/`cdpUrl` from a range starting 9 ports above the control port (default `18800`-`18899`); set those only for
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, SteelEngine pauses new launch
...
- `POST /start?headless=true` and `steelengine browser start --headless` request a
  ...
  remote CDP profiles reject the override because SteelEngine does not launch those
  browser processes.
- On Linux hosts without `DISPLAY` or `WAYLAND_DISPLAY`, local managed profiles
  ...
  `steelengine browser --json status`; trailing `steelengine browser status --json`
...
- `STEELENGINE_BROWSER_HEADLESS=1` forces local managed launches headless for the
  current process. `STEELENGINE_BROWSER_HEADLESS=0` forces headed mode for ordinary
...
- For a running local managed profile, status and doctor query Chrome's
  ...
  `steelengine browser --json status`. A passive status call does not launch Chrome.
...
- Default profile is `steelengine` (managed standalone). Use `defaultProfile: "user"` to opt into the signed-in user browser.
...
- `driver: "extension"` drives your signed-in Chrome through the [SteelEngine Chrome extension](/tools/chrome-extension). The relay owns its loopback endpoint, so these profiles do not accept `cdpUrl`. This is the only signed-in-browser mode that works with nobody at the computer.
...
SteelEngine uses it automatically. Set `browser.executablePath` to override
auto-detection. Top-level and per-profile `executablePath` values accept `~`
for your OS home directory:

```bash
steelengine config set browser.executablePath "/usr/bin/google-chrome"
steelengine config set browser.profiles.work.executablePath "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
```
...
Per-profile `executablePath` only affects local managed profiles that SteelEngine
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, SteelEngine will not launch a local browser.
- For externally managed CDP services on loopback (for example Browserless in
  ...
  without `attachOnly` is treated as a local SteelEngine-managed browser profile.
- `headless` only affects local managed profiles that SteelEngine launches. It does not restart or change existing-session or remote CDP browsers.
...
- local managed profiles: `steelengine browser stop` stops the browser process that
  SteelEngine launched
- attach-only and remote CDP profiles: `steelengine browser stop` closes the active
  ...
  though no browser process was launched by SteelEngine
...
SteelEngine preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, SteelEngine can
...
- If you set `nodeHost.browserProxy.allowProfiles`, SteelEngine treats it as a least-privilege boundary limiting which profile names the proxy will target.
...
CDP connection URLs over HTTPS and WebSocket. SteelEngine can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let SteelEngine
  discover `/json/version`.

### Browserless Docker on the same host

When Browserless is self-hosted in Docker and SteelEngine runs on the host, treat
...
SteelEngine process. Browserless must also advertise a matching reachable endpoint;
set Browserless `EXTERNAL` to that same public-to-SteelEngine WebSocket base, such
...
an address SteelEngine cannot reach, CDP HTTP can look healthy while the WebSocket
attach still fails.

Do not leave `attachOnly` unset for a loopback Browserless profile. Without
`attachOnly`, SteelEngine treats the loopback port as a local managed browser
profile and may report that the port is in use but not owned by SteelEngine.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). SteelEngine accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** - `http://host[:port]` or `https://host[:port]`.
  SteelEngine calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** - `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. SteelEngine connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** - `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). SteelEngine tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise SteelEngine
  ...
  accepts it, SteelEngine falls back to that root as well. This lets a bare `ws://`
...
`steelengine browser doctor` uses the same discovery-first, WebSocket-fallback
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-steelengine-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, SteelEngine
...
- Configure `gateway.auth.token`, `gateway.auth.password`, `STEELENGINE_GATEWAY_TOKEN`, or
  `STEELENGINE_GATEWAY_PASSWORD` explicitly if you want a stable secret you control
...
SteelEngine supports multiple named profiles (routing configs). Profiles can be:

- **steelengine-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `steelengine` profile is auto-created if missing.
...
SteelEngine can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when SteelEngine attaches.
...
```bash
steelengine browser --browser-profile user start
steelengine browser --browser-profile user status
steelengine browser --browser-profile user tabs
steelengine browser --browser-profile user snapshot --format ai
```
...
- `steelengine doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `steelengine` profile because it can
  act inside your signed-in browser session.
- SteelEngine does not launch the browser for this driver; it only attaches.
- SteelEngine uses the official Chrome DevTools MCP `--autoConnect` flow here. If
...
When `cdpUrl` is set on an existing-session profile, SteelEngine skips
...
Compared to the managed `steelengine` profile, existing-session drivers are more constrained:
...
When launching locally, SteelEngine picks the first available:
...
control API** plus a matching `steelengine browser` CLI (snapshots, refs, wait
...
- **CDP startup or readiness failure** means SteelEngine cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "steelengine" is not reachable after start`
  - `Remote CDP for profile "<name>" is not reachable at <cdpUrl>`
  - `Port <port> is in use for profile "<name>" but not by steelengine` when a
...
- For the local loopback `steelengine` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for SteelEngine's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (steelengine, chrome, or remote CDP).
