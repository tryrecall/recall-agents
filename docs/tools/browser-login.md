...
When a site requires login, sign in manually in the host browser's `steelengine`
...
SteelEngine controls a dedicated Chrome profile named `steelengine` (orange-tinted
UI), separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent uses its isolated `steelengine` browser.
...
Two ways to access the `steelengine` profile:
...
steelengine browser open https://x.com
```

For a non-default profile, put `--browser-profile <name>` before the
subcommand (default is `steelengine`):

```bash
steelengine browser --browser-profile <name> open https://x.com
```
...
```bash
steelengine browser --browser-profile steelengine open https://x.com
```
