...
Recall supports Grok as a `web_search` provider, using xAI web-grounded
...
Recall reuses it as a fallback for the bundled xAI model provider too.
...
Recall can use an existing xAI OAuth profile without prompting for a separate
web-search key. If OAuth is not available, it falls back to xAI API-key setup.
Recall can also show a separate follow-up step to enable `x_search` with the
...
<Steps>
  <Step title="Use xAI OAuth">
    ...
    ```bash
    recall onboard --auth-choice xai-oauth
...
**Credential alternatives:** sign in with `recall models auth login
...
route through an operator proxy or xAI-compatible Responses endpoint. Recall
