...
read_when:
  ...
  - Comparing SteelEngine exec approvals with ACPX harness permissions
...
```bash
steelengine config set tools.exec.mode auto
...
Set ACPX permissions separately from SteelEngine exec approvals:
...
| Use Codex/SteelEngine auto-review before humans  | `tools.exec.mode: "auto"`                                   |
...
Host exec uses the stricter result of SteelEngine config and the host-local approvals file. ACPX harness permissions do not loosen host exec approvals, and host exec approvals do not loosen ACPX harness prompts.
