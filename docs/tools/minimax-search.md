...
Recall supports MiniMax as a `web_search` provider through the MiniMax
...
Recall also accepts `MINIMAX_CODING_API_KEY`, `MINIMAX_OAUTH_TOKEN`, and
...
If `plugins.entries.minimax.config.webSearch.region` is unset, Recall resolves
...
| `count`   | integer | 1-10        | Number of results to return. Recall trims the returned list to this size. |
