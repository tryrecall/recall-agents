...
<Steps>
  <Step title="Install the plugin">
    ```bash
    steelengine plugins install diffs
    ```
  </Step>
...
```bash
steelengine plugins install clawhub:@steelengine/diffs-language-pack
```
...
Set plugin-wide defaults in `~/.steelengine/steelengine.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/steelengine",
        },
      },
    },
  },
...
- Viewer HTML and metadata live in the shared `state/steelengine.sqlite` database under the Diffs plugin blob namespace. HTML is gzip-compressed; SQLite stores only a SHA-256 hash of the random URL token, not the token itself.
- Rendered PNG/PDF files remain temporary materializations under `$TMPDIR/steelengine-diffs` because channel delivery requires a file path. SQLite owns their expiry metadata; no JSON sidecars are written.
...
<Steps>
  <Step title="Config">
    `browser.executablePath` in SteelEngine config.
  </Step>
