---
summary: "Typed workflow runtime for SteelEngine with resumable approval gates."
title: Lobster
read_when:
...
see [Task Flow](/automation/taskflow) (`steelengine tasks flow`); for the task
...
→ steelengine calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ steelengine calls gmail.send
...
SteelEngine runs Lobster workflows **in-process** using the bundled
...
[Lobster repo](https://github.com/steelengine/lobster) and put `lobster` on
`PATH`.
...
gog.gmail.search --query 'newer_than:1d' \
  | steelengine.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
### Important limitation: embedded Lobster vs `steelengine.invoke`

The bundled Lobster plugin runs workflows **in-process** inside the gateway.
In that embedded mode, `steelengine.invoke` does **not** automatically inherit a
gateway URL/auth context for nested SteelEngine CLI tool calls.

That means this pattern is **not currently reliable in the embedded runner**:

```lobster
steelengine.invoke --tool llm-task --action json --args-json '{ ... }'
```

Use the example below only when running the **standalone Lobster CLI** in an
environment where `steelengine.invoke` is already configured with the correct
...
- non-`steelengine.invoke` steps inside the Lobster pipeline until a supported
...
a bare envelope: SteelEngine creates or resumes a durable flow record, applies the
...
- **No secrets** - Lobster doesn't manage OAuth; it calls SteelEngine tools that
  do.
