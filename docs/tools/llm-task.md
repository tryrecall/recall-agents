...
`steelengine.invoke` already has the correct gateway URL/auth context.

For the bundled **embedded** Lobster runner inside SteelEngine, this nested CLI
...
- Lobster steps that do not rely on nested `steelengine.invoke` calls.
