...
The example below assumes the **standalone Lobster CLI** is running in an environment where `recall.invoke` already has the correct gateway URL/auth context.

For the bundled **embedded** Lobster runner inside Recall, this nested CLI pattern is **not currently reliable**:
...
- Lobster steps that do not rely on nested `recall.invoke` calls.
