...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let Recall run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask Recall in plain language to "run this in Codex" or "start Claude Code in a thread", Recall should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want Recall-native delegated runs.
...
Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, Recall returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning Recall agent id.
...
- If `agentId` is omitted, Recall uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, Recall may default to persistent behavior per runtime path
...
ACP sessions currently run on the host runtime, not inside the Recall sandbox.
...
If no target resolves, Recall returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, Recall returns a clear unsupported-control error.
...
```bash
recall plugins install ./extensions/acpx
```
...
- Relative paths resolve from Recall workspace directory.
...
- Recall startup remains non-blocking while the backend health check runs.
...
> **Important:** Recall currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
