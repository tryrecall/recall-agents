      ...
      parallel: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://api.parallel.ai", // optional; SteelEngine appends /v1/search
          },
        },
      },
    },
...
environment. For a gateway install, put it in `~/.steelengine/.env`.
...
Cloudflare AI Gateway). SteelEngine normalizes bare hosts by prepending
...
- Both providers return a `session_id`; SteelEngine surfaces it as `sessionId` in
...
- `searchId`, `warnings`, and `usage` from Parallel are passed through when
  present.
- SteelEngine always forwards a resolved result count to Parallel as
  ...
  SteelEngine's generic `web_search` default (5) -- Parallel's own API defaults
  to 10.
