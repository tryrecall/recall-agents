...
- `The profile appears to be in use by another Chromium process` means Chrome
  found stale `Singleton*` lock files in the managed profile directory. Recall
...
- `Missing X server or $DISPLAY` means a visible browser was explicitly
  ...
  `WAYLAND_DISPLAY` are both unset. If you set `RECALL_BROWSER_HEADLESS=0`,
  `browser.headless: false`, or `browser.profiles.<name>.headless: false`,
  remove that headed override, set `RECALL_BROWSER_HEADLESS=1`, start `Xvfb`,
  run `recall browser start --headless` for a one-shot managed launch, or run
  Recall in a real desktop session.
...
Then update your Recall config (`~/.tryrecall/recall-agents.json`):
...
| `RECALL_BROWSER_HEADLESS`      | Per-process override for local managed browser headless mode         | unset                                                       |
...
`recall browser start` still reports `not reachable after start`. Values must
