...
read_when:
  ...
  - Enabling the plugin-tools or SteelEngine-tools MCP bridge
...
Codex has two SteelEngine routes:
...
| `steelengine`   | SteelEngine ACP bridge (native `steelengine acp`)                                                                     |
...
When SteelEngine uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal SteelEngine `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by SteelEngine before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, SteelEngine/acpx cannot force a model selection.
...
Packaged installs use the official `@steelengine/acpx` runtime plugin for ACP.
...
```bash
steelengine plugins install ./path/to/local/acpx-plugin
```
...
signal. Set `STEELENGINE_ACPX_RUNTIME_STARTUP_PROBE=0` or
`STEELENGINE_SKIP_ACPX_RUNTIME_PROBE=1` only for scripts or environments that
...
- `agents.<id>.args` is optional. Each array item is shell-quoted before SteelEngine passes it through the current acpx command-string registry.
...
manually, and there is no separate postinstall step for SteelEngine itself. If an
adapter download or spawn fails, `/acp doctor` reports the failure.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose SteelEngine plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
SteelEngine plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
steelengine config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `steelengine-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled SteelEngine
  plugins.
...
### SteelEngine tools MCP bridge

By default, ACPX sessions also do **not** expose built-in SteelEngine tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
steelengine config set plugins.entries.acpx.config.steelEngineToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `steelengine-tools` into ACPX session
  bootstrap.
- Exposes selected built-in SteelEngine tools. The initial server exposes `cron`.
...
```bash
steelengine config set plugins.entries.acpx.config.timeoutSeconds 180
```

Runtime turns use SteelEngine agent/run timeouts, including `/acp timeout`.
...
```bash
steelengine config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from SteelEngine exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.

For the broader comparison between SteelEngine `tools.exec.mode`, Codex Guardian
...
```bash
steelengine config set plugins.entries.acpx.config.permissionMode approve-all
steelengine config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
SteelEngine defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `PermissionPromptUnavailableError: Permission prompt unavailable in non-interactive mode`.
