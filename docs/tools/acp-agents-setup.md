...
read_when:
  ...
  - Enabling the plugin-tools or Recall-tools MCP bridge
...
When Recall uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Recall `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by Recall before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, Recall/acpx cannot force a model selection.
...
Packaged installs use the official `@recall/acpx` runtime plugin for ACP.
...
```bash
recall plugins install ./path/to/local/acpx-plugin
```
...
`ready` signal. Set `RECALL_ACPX_RUNTIME_STARTUP_PROBE=0` or
`RECALL_SKIP_ACPX_RUNTIME_PROBE=1` only for scripts or environments that
...
- `command` accepts an absolute path, relative path (resolved from the Recall workspace), or command name.
...
- `agents.<id>.args` is optional. Each array item is shell-quoted before Recall passes it through the current acpx command-string registry.

See [Plugins](/tools/plugin).

### Automatic dependency install

When you install Recall globally with `npm install -g recall`, the acpx
...
normally and reports the missing dependency through `recall acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose Recall plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
Recall plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
recall config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `recall-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled Recall
  plugins.
...
### Recall tools MCP bridge

By default, ACPX sessions also do **not** expose built-in Recall tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
recall config set plugins.entries.acpx.config.openClawToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `recall-tools` into ACPX session
  bootstrap.
- Exposes selected built-in Recall tools. The initial server exposes `cron`.
...
```bash
recall config set plugins.entries.acpx.config.timeoutSeconds 180
```

Runtime turns use Recall agent/run timeouts, including `/acp timeout` and
...
```bash
recall config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from Recall exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
...
```bash
recall config set plugins.entries.acpx.config.permissionMode approve-all
recall config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
Recall defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
