...
`https://api.moonshot.cn/v1`), Recall reuses that same host for Kimi
...
If you omit `baseUrl`, Recall defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Recall defaults to `kimi-k2.6`.
...
Recall treats Kimi `web_search` as successful only after Moonshot returns
...
Recall returns a structured `kimi_web_search_ungrounded` error instead of
