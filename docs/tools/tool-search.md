---
summary: "Tool Search: compact large SteelEngine tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want SteelEngine agents to use a large tool catalog without adding every tool schema to the prompt
  - You want SteelEngine tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for SteelEngine runs
---

Tool Search is an experimental SteelEngine agent runtime feature. It gives agents one
...
This page documents SteelEngine Tool Search. It is not the Codex-native tool
...
For the generic SteelEngine runtime that exposes a QuickJS-WASI `exec`/`wait`
surface instead of Tool Search controls, see [Code Mode](/tools/code-mode).

When enabled for SteelEngine runs, the model receives one `tool_search_code` tool
...
Node subprocess with an `steelengine.tools` bridge:

```js
const hits = await steelengine.tools.search("create a GitHub issue");
const tool = await steelengine.tools.describe(hits[0].id);
return await steelengine.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include catalog-eligible SteelEngine tools, plugin tools, MCP
...
tool when it needs the exact schema, and calls that tool through SteelEngine.
Direct-only tools remain model-visible and are not added to the catalog.

Codex harness runs do not receive these experimental SteelEngine Tool Search
controls. SteelEngine passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the SteelEngine embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible SteelEngine and plugin tools.
...
6. Expose the SteelEngine code bridge, the structured fallback tools, or the
   compact directory surface alongside those direct-only tools.

At execution time every real tool call returns to SteelEngine. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`steelengine.tools.call(...)` crosses the bridge back into the Gateway, where the
...
- `directory`: exposes `tool_search`, `tool_describe`, and `tool_call` plus a
  ...
  providers that should see tool names without every full schema. SteelEngine can
...
All modes use the same policy-filtered catalog and normal SteelEngine execution
...
for the current run while SteelEngine tools, plugin tools, and MCP tools can be
...
All modes are experimental. Prefer direct tool exposure for small SteelEngine tool
...
catalog includes catalog-eligible SteelEngine, MCP, and client tools after normal
...
`steelengine.tools.search(query, options?)`
...
SteelEngine core or plugin tool may also include a compact `output` hint, such as
...
```js
const hits = await steelengine.tools.search("calendar event", { limit: 5 });
```

`steelengine.tools.describe(id)`
...
```js
const calendarCreate = await steelengine.tools.describe("mcp:calendar:create_event");
```

`steelengine.tools.call(id, args)`

Calls a selected tool through SteelEngine and returns the raw `{ tool, result }`
envelope. JSON-returning tools normally place their value in
`result.details`. If a trusted tool declares `outputSchema`, SteelEngine compiles
the schema before execution and validates final `details` after normal tool
hooks before returning the catalog call.

```js
await steelengine.tools.call(calendarCreate.id, {
  summary: "Planning",
...
tool name directly, SteelEngine hydrates it from the authorized catalog before
normal execution.
Directory-mode client tool names must not collide with SteelEngine, plugin, or MCP
...
network grants, and no child-process or worker grants. SteelEngine enforces a
...
- `steelengine.tools.search`
- `steelengine.tools.describe`
- `steelengine.tools.call`

Normal SteelEngine behavior still applies to final calls:
...
Enable Tool Search for SteelEngine runs with the default code bridge:

```bash
steelengine config set tools.toolSearch true
```
...
Use the structured fallback tools instead for SteelEngine runs:
...
Use the compact directory surface instead for SteelEngine runs:
...
- final tool calls executed through SteelEngine
...
- whether the result came from SteelEngine, MCP, or a client tool

## E2E validation

The QA Lab gateway scenario proves both paths with the SteelEngine runtime:

```bash
pnpm steelengine qa suite --provider-mode mock-openai --scenario tool-search-gateway-e2e
```
