...
- For shell scripts and direct interpreter/runtime file invocations, Recall also tries to bind
...
When `tools.exec.strictInlineEval=true`, Recall treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
`~/.recall/exec-approvals.json` directly.

CLI: `recall approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Recall does not treat
