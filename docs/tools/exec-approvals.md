...
host-local `ask: "always"` in `~/.recall/exec-approvals.json` keeps
...
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `recall approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                       |
| `recall exec-policy show`                                      | Local-machine merged view.                                                             |
| `recall exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals file in one step. |
...
- **Gateway host** → `recall` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, Recall also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.recall/exec-approvals.sock",
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, Recall treats inline code-eval forms as approval-only
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  ...
  Recall may attach parser-derived command spans so Web approval
...
**both** policy layers - requested exec policy in Recall config
...
- In YOLO mode, Recall does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when Recall's requested exec
...
<Steps>
  ...
  <Step title="Match the host approvals file">
    ```bash
    recall approvals set --stdin <<'EOF'
    {
      version: 1,
      defaults: {
        security: "full",
...
```bash
recall exec-policy preset yolo
```

That local shortcut updates both:

- Local `tools.exec.host/security/ask`.
- Local `~/.recall/exec-approvals.json` defaults.

It is intentionally local-only. To change gateway-host or node-host
approvals remotely, use `recall approvals set --gateway` or
`recall approvals set --node <id|name|ip>`.
...
```bash
recall approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `recall exec-policy` does not synchronize node approvals.
- `recall exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `recall approvals --node ...`.
...
specific argument shape. Recall evaluates the regular expression
...
entries instead of hand-editing the encoded value. If Recall cannot
...
edit its local `~/.recall/exec-approvals.json` directly.

CLI: `recall approvals` supports gateway or node editing - see
...
Denied exec approvals are terminal: Recall can report the denial to the
...
When an async exec approval is denied, Recall treats the request as terminal.
