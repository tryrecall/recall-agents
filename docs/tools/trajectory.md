---
summary: "Export redacted trajectory bundles for debugging an SteelEngine agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an SteelEngine session
...
Trajectory capture is SteelEngine's per-session flight recorder. It records a
...
SteelEngine writes the bundle under the workspace:

```text
.steelengine/trajectory-exports/steelengine-trajectory-<session>-<timestamp>/
```
...
The name resolves inside `.steelengine/trajectory-exports/`. Absolute paths and
`~` paths are rejected.
...
bundle; do not use allow-all. In group chats, SteelEngine sends the approval
...
`.steelengine/trajectory-exports`), `--store <path>` (session store override),
...
Trajectory capture is on by default for SteelEngine agent runs.
...
| `metadata.json`       | SteelEngine version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
```bash
export STEELENGINE_TRAJECTORY=0
```

This disables runtime trajectory capture before starting SteelEngine.
...
SteelEngine flushes runtime trajectory rows during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`STEELENGINE_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting SteelEngine:

```bash
export STEELENGINE_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when SteelEngine logs an `steelengine-trajectory-flush` timeout and
...
`STEELENGINE_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are for support and debugging, not public posting. SteelEngine
...
- confirm SteelEngine was started without `STEELENGINE_TRAJECTORY=0`
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.steelengine/trajectory-exports/`
