---
summary: "Export redacted trajectory bundles for debugging an Recall agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an Recall session
...
Trajectory capture is Recall's per-session flight recorder. It records a
...
Recall writes the bundle under the workspace:

```text
.recall/trajectory-exports/recall-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.recall/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, Recall sends the
...
```bash
recall sessions export-trajectory --session-key "agent:main:telegram:direct:123" --workspace .
```
...
Trajectory capture is on by default for Recall agent runs.
...
| `metadata.json`       | Recall version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
Recall also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `RECALL_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export RECALL_TRAJECTORY_DIR=/var/lib/recall/trajectories
```

When this variable is set, Recall writes one JSONL file per session id in that
directory.
...
Set `RECALL_TRAJECTORY=0` before starting Recall:

```bash
export RECALL_TRAJECTORY=0
```
...
Recall flushes runtime trajectory sidecars during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`RECALL_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting Recall:

```bash
export RECALL_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when Recall logs a `pi-trajectory-flush` timeout and continues.
It does not change the trajectory size caps. To tune all agent cleanup steps
that do not pass an explicit timeout, set `RECALL_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are designed for support and debugging, not public posting.
Recall redacts sensitive values before writing export files:
...
- confirm Recall was started without `RECALL_TRAJECTORY=0`
- check whether `RECALL_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.recall/trajectory-exports/`
