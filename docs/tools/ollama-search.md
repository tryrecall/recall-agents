...
SteelEngine supports **Ollama Web Search** as a bundled `web_search` provider,
...
<Note>
  SteelEngine never auto-selects Ollama Web Search over a higher-priority
...
config examples. If nothing is set, SteelEngine defaults to
...
- If the resolved host is `https://ollama.com`, SteelEngine calls
  `https://ollama.com/api/web_search` directly with the API key as bearer
  auth.
- Otherwise SteelEngine calls the local proxy endpoint
  ...
  the local host.
- SteelEngine warns during setup if Ollama is unreachable or not signed in, but
