...
Recall supports **Ollama Web Search** as a bundled `web_search` provider. It
...
- an Ollama host that is reachable from Recall
- `ollama signin`
...
If no explicit Ollama base URL is set, Recall uses `http://127.0.0.1:11434`.

If your Ollama host expects bearer auth, Recall reuses
...
- If the Ollama host is auth-protected, Recall reuses the normal Ollama
  provider API key when present.
- If `baseUrl` is `https://ollama.com`, Recall calls
...
- If the configured host does not expose web search and `OLLAMA_API_KEY` is set,
  Recall can fall back to `https://ollama.com/api/web_search` without sending
  that env key to the local host.
- Recall warns during setup if Ollama is unreachable or not signed in, but
