...
<AccordionGroup>
  <Accordion title="Non-blocking, push-based completion">
    ...
    - On completion, SteelEngine best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.

  </Accordion>
  <Accordion title="Completion delivery">
    - SteelEngine hands completions back to the requester session through an `agent` turn with a stable idempotency key.
    - If the requester run is still active, SteelEngine first tries to wake/steer that run instead of starting a second visible reply path.
    - If an active requester cannot be woken, SteelEngine falls back to a requester-agent handoff with the same completion context instead of dropping the announce.
    ...
    - Delivery keeps the resolved requester route: thread-bound or conversation-bound completion routes win when available. If the completion origin only provides a channel, SteelEngine fills the missing target/account from the requester session's resolved route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery still works.
  ...
  <Accordion title="Modes and ACP runtime">
    ...
    - SteelEngine hides `runtime: "acp"` until ACP is enabled, the requester is not sandboxed, and a backend plugin such as `acpx` is loaded. `runtime: "acp"` expects an external ACP harness id, or an `agents.list[]` entry with `runtime.type="acp"`; use the default sub-agent runtime for normal SteelEngine config agents from `agents_list`.
...
- **Run timeout:** SteelEngine uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout). `sessions_spawn` does not accept per-call timeout overrides.
- **Process lifetime:** a detached SteelEngine sub-agent has its own run lifecycle. A background task created inside an external CLI backend is different: it shares the parent CLI subprocess and stops if that parent reaches `agents.defaults.timeoutSeconds`.
...
When active children exist, SteelEngine injects a compact runtime-generated
...
<Steps>
  ...
  <Step title="Bind">
    SteelEngine creates or binds a thread to that session target in the active channel.
  </Step>
...
model and embedded runtime metadata so callers can distinguish SteelEngine, Codex
...
and `agents_list` omits it. Run `steelengine doctor --fix` to clean stale
...
- If a nested requester subagent session is gone, SteelEngine falls back to that session's requester when available.
...
target agent first. After that, SteelEngine applies the sub-agent restriction
layer.
...
SteelEngine does not treat `endedAt` absence as permanent proof that a
...
rapid re-wedge window, SteelEngine persists a recovery tombstone on that
session and stops auto-resuming it on later restarts. Run
`steelengine tasks maintenance --apply` to reconcile the task record, or
`steelengine doctor --fix` to clear stale aborted recovery flags on
tombstoned sessions.
