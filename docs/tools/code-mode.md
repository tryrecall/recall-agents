---
summary: "Use SteelEngine Code Mode to discover, call, and combine large tool catalogs in compact JavaScript or TypeScript workflows"
title: "Code Mode"
sidebarTitle: "Code Mode"
read_when:
  - You want to enable SteelEngine code mode for an agent run
...
Code mode is an experimental, opt-in SteelEngine agent-runtime feature. When
...
This page documents SteelEngine code mode, not Codex Code Mode. The two features
...
- SteelEngine code mode runs in the generic SteelEngine agent runtime and is
...
- Every catalog-eligible enabled tool (SteelEngine core, plugin, MCP, client) is hidden as a
  standalone model tool and exposed inside the guest program through `ALL_TOOLS`
  and `tools`.
- The `exec` description carries a bounded quick index of exact SteelEngine/plugin
...
- Provider neutral: works for SteelEngine, plugin, MCP, and client tools without
...
`STEELENGINE_DEBUG_MODEL_PAYLOAD=full-redacted` for a short debugging session.
...
| Stability           | experimental SteelEngine surface (Codex Code Mode is a separate, stable Codex harness surface) |
| Target surface      | generic SteelEngine agent runs                                                                 |
...
- **Code mode**: the SteelEngine runtime mode that hides catalog-compatible model
...
If code mode is enabled but QuickJS-WASI cannot load, SteelEngine fails closed
...
1. Resolve the agent, model, provider, sandbox, channel, sender, and run
   policy.
2. Build the effective SteelEngine tool list, adding eligible plugin, MCP, and
   client tools.
...
when `tools.codeMode.enabled: true` is configured. Code mode and SteelEngine Tool
...
- `command` is accepted as an exec-compatible alias for hook policies and
  trusted rewrites (the normal SteelEngine shell exec tool also uses a `command`
...
- If `language` is `"typescript"`, SteelEngine transpiles before evaluation.
...
final value is JSON-compatible after SteelEngine's output adapter runs.
...
`wait` exists because nested SteelEngine tools can be slow, interactive, approval
...
2. On suspension, SteelEngine snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot and
   re-registers host callbacks by stable names.
4. SteelEngine delivers nested tool results into the restored VM and drains
...
bounded, deterministic subset of exact SteelEngine/plugin ids, compact input
...
present only for a complete compact hint derived from a trusted SteelEngine core
or plugin `outputSchema`. MCP and client output-schema claims are not promoted
into this trusted catalog hint.

Plugin tools use `source: "steelengine"` with `sourceName` set to the owning
...
const hits = await tools.web_search({ query: "SteelEngine code mode" });
```
...
SteelEngine tools can declare `outputSchema` for the structured value placed in
...
import { defineToolPlugin } from "steelengine/plugin-sdk/tool-plugin";
...
- SteelEngine compiles the schema before running the tool, then validates final
...
directory. For each code-mode `exec` call, SteelEngine builds the run-scoped tool
...
catalog for arbitrary enabled SteelEngine, plugin, and client tools; use `MCP`
...
ordinary SteelEngine tools, not through the code-mode bridge.

## Tool catalog

The hidden catalog includes tools after effective policy filtering, in this
order: SteelEngine core tools, bundled plugin tools, external plugin tools, MCP
...
where `<source>` is `steelengine`, `mcp`, or `client` (plugin tools use
`steelengine` with the plugin id as `<owner>`; core tools use `steelengine:core:*`).
Examples:

```text
steelengine:core:message
steelengine:browser:browser_request
...
Code mode supersedes the SteelEngine Tool Search model surface for runs where it
is active.

When `tools.codeMode.enabled` is true and code mode activates:

- SteelEngine does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- MCP calls use the generated `MCP` namespace and its `$api()` headers instead
  of `tools.call(...)`.
- Nested calls dispatch through the same SteelEngine executor path that Tool
  Search uses.

See [Tool Search](/tools/tool-search) for the SteelEngine compact catalog bridge
that code mode supersedes for active runs.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal SteelEngine
shell `exec` tool is enabled, it is hidden from the model and cataloged like
any other tool.

Inside the guest runtime:

- `tools.call("steelengine:core:exec", input)` can call the shell exec tool if
  policy allows it.
...
If two tools normalize to the same safe convenience name, SteelEngine omits the
convenience function and requires `tools.call(id, input)`.

## Nested tool execution

Every nested tool call crosses the host bridge and re-enters SteelEngine,
...
- For read-only work, `exec` can set `restartSafe: true`. SteelEngine then rejects
...
- SteelEngine caps the number of concurrently suspended runs per process (64) and
...
SteelEngine loads `quickjs-wasi` as a direct dependency in the owning package; it
...
The runtime executes in a Node.js worker thread, outside SteelEngine's main
...
breakdown (`steelengine`/`mcp`/`client` counts), cumulative search/describe/call
...
tool inputs beyond existing SteelEngine trajectory policy.
...
For payload-shape debugging, use `STEELENGINE_DEBUG_MODEL_PAYLOAD=full-redacted`.
...
For stream debugging, use `STEELENGINE_DEBUG_SSE=peek` to log the first five
...
- `tools.search`, `tools.describe`, `tools.callValue`, and `tools.call` work for SteelEngine tools
...
5. Send an agent turn with SteelEngine, plugin, MCP, and client test tools.
...
8. In `exec`, call SteelEngine/plugin/client tools through `tools.search`,
