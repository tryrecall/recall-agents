...
Recall can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `recall configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `recall doctor --fix`.
...
when you run Firecrawl yourself. Recall accepts `http://` only for loopback,
...
Recall always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, Recall
