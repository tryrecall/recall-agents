...
For session-backed agent runs, Recall starts music generation as a
...
audio is still missing from message-tool delivery, Recall sends an
...
<Note>
Not all providers support all parameters. Recall still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, Recall
...
Provider request timeouts are operator configuration only. Recall uses
...
- **Status lookup:** `recall tasks list` or `recall tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** Recall injects an internal completion event back
...
```bash
RECALL_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
