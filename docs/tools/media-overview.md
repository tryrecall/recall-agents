...
read_when:
  - Looking for an overview of Recall's media capabilities
...
Recall generates images, videos, and music, understands inbound media
...
For async tools, Recall submits the request to the provider, returns a task
...
Recall wakes the agent with the generated media paths so it can tell the
...
delivery, Recall sends an idempotent direct fallback with only the missing
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    registered until Recall has dedicated provider contracts for those
    categories.
  </Accordion>
  <Accordion title="xAI">
    ...
    not registered in Recall until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
