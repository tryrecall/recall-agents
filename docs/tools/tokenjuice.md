...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in SteelEngine
...
Today this applies to SteelEngine embedded runs and SteelEngine dynamic tools in the Codex
app-server harness. Tokenjuice hooks SteelEngine's tool-result middleware and
...
```bash
steelengine plugins install clawhub:@steelengine/tokenjuice
```
...
```bash
steelengine config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
steelengine plugins disable tokenjuice
```
