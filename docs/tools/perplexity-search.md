...
Recall supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Recall also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `recall configure --section web`. It stores the key in
`~/.tryrecall/recall-agents.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.recall/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
