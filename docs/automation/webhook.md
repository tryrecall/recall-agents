...
read_when:
  ...
  - Wiring external systems into Recall
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Recall config directory (typically `~/.recall/hooks/transforms`).
...
- `recall webhooks gmail setup` writes `hooks.gmail` config for `recall webhooks gmail run`.
