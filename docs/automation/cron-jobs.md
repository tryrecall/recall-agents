...
- Jobs persist under `~/.recall/cron/` so restarts don’t lose schedules.
...
- For upgrades, `recall doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
Cron jobs are persisted on the Gateway host at `~/.recall/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `recall cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Recall applies a
...
If `delivery` is omitted for isolated jobs, Recall defaults to `announce`.
...
- CLI equivalents: `recall cron add --light-context ...` and `recall cron edit --light-context`.
...
- `schedule.at` accepts ISO 8601. Tool/API values without a timezone are treated as UTC; the CLI also accepts `recall cron add|edit --at "<offset-less-iso>" --tz <iana>` for local wall-clock one-shots.
...
- Job store: `~/.recall/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.recall/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Recall classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Recall archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `recall cron runs` and adjust retention before logs become large
...
recall cron edit <jobId> --agent ops
...
```bash
recall system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`recall system event`](/cli/system).

## Troubleshooting

### "Nothing runs"

- Check cron is enabled: `cron.enabled` and `RECALL_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Recall applies exponential retry backoff for recurring jobs after consecutive errors:
