...
recall system heartbeat last
recall logs --follow
recall config get agents.defaults.heartbeat
recall channels status --probe
```
...
```bash
recall config get agents.defaults.heartbeat.activeHours
recall config get agents.defaults.heartbeat.activeHours.timezone
recall config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
recall cron list
recall logs --follow
```
