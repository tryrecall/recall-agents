...
Recall can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Recall can send audio; Telegram gets a round voice-note bubble.
...
Recall defaults to Microsoft (unless disabled via
...
TTS config lives under `messages.tts` in `recall.json`.
...
- If `provider` is **unset**, Recall prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `microsoft`.
...
`~/.recall/settings/tts.json`, override with `RECALL_TTS_PREFS` or
...
Discord note: `/tts` is a built-in Discord command, so Recall registers
...
reply delivery. When the result is Telegram-compatible, Recall marks it for
