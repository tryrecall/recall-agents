...
summary: "Specification for making the bundled Codex app-server harness honor Recall context-engine plugins"
read_when:
...
Make the bundled Codex app-server harness honor the same Recall context-engine
...
Recall-level compaction policy as far as the Codex app-server boundary allows.
...
That means the required hook point is in Recall-controlled code. The external
boundary is the Codex app-server protocol itself: Recall can control what it
...
For Codex harness turns, Recall should preserve this lifecycle:

1. Read the mirrored Recall session transcript.
...
8. Mirror the Codex result back into the Recall transcript.
...
11. Preserve Codex native compaction signals and Recall compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. Recall should
not try to mutate the app-server's internal history except through supported
protocol calls.

Recall's transcript mirror remains the source for Recall features:
...
The context-engine interface returns Recall `AgentMessage[]`, not a Codex
...
- Label it clearly as Recall assembled context.
...
```text
Recall assembled context for this turn:
...
inside Recall and preserves context-engine semantics.
...
1. Recall context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit Recall compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
Recall compaction should prefer the context engine's `compact()` result for
the Recall transcript mirror and plugin state.
...
the Recall session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
Recall session lifecycle paths. Do not add Codex-specific cleanup unless the
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole Recall compaction when the context engine is primary.
...
Avoid requiring lossless-claw in Recall core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for Recall/plugin state.
