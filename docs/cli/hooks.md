...
Running `recall hooks` with no subcommand is equivalent to `recall hooks list`.
...
Enable a specific hook by adding it to your config (`~/.tryrecall/recall-agents.json` by default).
...
- `recall hooks list --json`, `info --json`, and `check --json` write structured JSON directly to stdout.
...
**Output:** `~/.recall/workspace/memory/YYYY-MM-DD-HHMM.md` by default. Set `hooks.internal.entries.session-memory.llmSlug: true` for model-generated filename slugs.
