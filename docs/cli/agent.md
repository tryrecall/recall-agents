---
summary: "CLI reference for `recall agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
recall agent --to +15555550123 --message "status update" --deliver
recall agent --agent ops --message "Summarize logs"
recall agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
...
recall agent --agent ops --message "Run locally" --local
```
