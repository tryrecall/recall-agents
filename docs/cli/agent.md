---
summary: "CLI reference for `steelengine agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
steelengine agent --to +15555550123 --message "status update" --deliver
...
steelengine agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
steelengine agent --agent ops --message "Run locally" --local
```
...
- Slash commands (for example `/compact`) cannot run through `--message`. The CLI rejects them and points you at the first-class command instead (`steelengine sessions compact <key>` for compaction).
