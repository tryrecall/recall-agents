---
summary: "CLI reference for `steelengine setup` (system-agent chat with onboarding fallback)"
read_when:
  - You want to chat with SteelEngine for setup or repair
...
`steelengine setup` is the system-agent entry point. On a configured system, bare
`steelengine setup` opens an interactive SteelEngine chat. On a fresh system, it
...
1. Any onboarding option (`--wizard`, `--baseline`, workspace, reset,
   ...
   options) runs onboarding exactly as `steelengine onboard` does.
2. `-m`/`--message` or `--yes` runs the system agent.
3. With no routing option, a configured interactive system opens SteelEngine. A
...
In guided mode, `--workspace <dir>` is the workspace proposed to SteelEngine;
...
candidates; when several local models are available, SteelEngine prefers the
...
`setup` accepts the same onboarding flags as `steelengine onboard`, including
...
non-interactive examples. `steelengine onboard --modern` remains a compatibility
entry for the same inference-gated SteelEngine assistant.

<Note>
`steelengine setup` is for mutable config installs. In Nix mode (`STEELENGINE_NIX_MODE=1`) SteelEngine refuses setup writes because the config file is managed by Nix. Use the first-party [nix-steelengine Quick Start](https://github.com/steelengine/nix-steelengine#quick-start) or the equivalent source config for another Nix package.
</Note>
...
| Flag                       | Description                                                                                           |
| -------------------------- | ----------------------------------------------------------------------------------------------------- |
| `-m, --message <text>`     | Run one SteelEngine request.                                                                             |
...
| `--json`                   | Configured system: SteelEngine overview. Onboarding route: onboarding summary.                           |
...
`steelengine setup --baseline` preserves the older baseline-only behavior: it
...
```bash
steelengine setup
steelengine setup -m "status"
steelengine setup -m "restart gateway" --yes
steelengine setup --json
steelengine setup --wizard
steelengine setup --baseline
steelengine setup --workspace ~/.steelengine/workspace
steelengine setup --import-from hermes --import-source ~/.hermes
steelengine setup --non-interactive --accept-risk --mode remote --remote-url wss://gateway-host:18789 --remote-token <token>
```

## Notes

- After baseline setup, run `steelengine onboard` for the full guided journey, `steelengine configure` for targeted changes, or `steelengine channels add` to add channel accounts.
