---
summary: "CLI reference for `recall config` (get/set/patch/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `recall.json`: get/set/patch/unset/file/schema/validate values by path and print the active config file. Run without a subcommand to open the configure wizard (same as `recall configure`).

<Note>
When `RECALL_NIX_MODE=1`, Recall treats `recall.json` as immutable. Read-only commands such as `config get`, `config file`, `config schema`, and `config validate` still work, but config writers refuse. Agents should edit the Nix source for the install instead; for the first-party nix-recall distribution, use [nix-recall Quick Start](https://github.com/recall/nix-recall#quick-start) and set values under `programs.recall.config` or `instances.<name>.config`.
</Note>

## Root options

<ParamField path="--section <section>" type="string">
  Repeatable guided-setup section filter when you run `recall config` without a subcommand.
</ParamField>
...
```bash
recall config file
recall config --section model
recall config --section gateway --section daemon
recall config schema
...
recall config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
...
recall config patch --file ./recall.patch.json5 --dry-run
...
Print the generated JSON schema for `recall.json` to stdout as JSON.
...
```bash
recall config schema > recall.schema.json
```

### Paths

Paths use dot or bracket notation. Quote bracket-notation paths in shell examples so shells such as zsh do not expand `[0]` as a glob before Recall receives the path:
...
```bash
recall config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
recall config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
```
...
```bash
recall config patch --file ./recall.patch.json5 --dry-run
recall config patch --file ./recall.patch.json5
```

You can also pipe a patch over stdin, which is useful for remote setup scripts:

```bash
ssh recall-host 'recall config patch --stdin --dry-run' < ./recall.patch.json5
ssh recall-host 'recall config patch --stdin' < ./recall.patch.json5
```
...
```bash
recall config patch --file ./discord.patch.json5 --replace-path 'channels.discord.guilds["123"].channels'
```
...
<Tabs>
  <Tab title="Success example">
    ```json
    {
      "ok": true,
      "operations": 1,
      "configPath": "~/.tryrecall/recall-agents.json",
      "inputModes": ["builder"],
      "checks": {
        "schema": false,
        "resolvability": true,
...
`recall config set` and other Recall-owned config writers validate the full post-change config before committing it to disk. If the new payload fails schema validation or looks like a destructive clobber, the active config is left alone and the rejected payload is saved beside it as `recall.json.rejected.*`.

<Warning>
The active config path must be a regular file. Symlinked `recall.json` layouts are unsupported for writes; use `RECALL_CONFIG_PATH` to point directly at the real file instead.
</Warning>

Prefer CLI writes for small edits:

```bash
recall config set gateway.reload.mode hybrid --dry-run
recall config set gateway.reload.mode hybrid
recall config validate
```
...
Direct editor writes are still allowed, but the running Gateway treats them as untrusted until they validate. Invalid direct edits fail startup or are skipped by hot reload; Gateway does not rewrite `recall.json`. Run `recall doctor --fix` to repair prefixed/clobbered config or restore the last-known-good copy. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-rejected-invalid-config).
...
- `config file`: Print the active config file path (resolved from `RECALL_CONFIG_PATH` or default location). The path should name a regular file, not a symlink.
...
After `recall config validate` is passing, you can use the local TUI to have an embedded agent compare the active config against the docs while you validate each change from the same terminal:

<Note>
If validation is already failing, start with `recall configure` or `recall doctor --fix`. `recall chat` does not bypass the invalid-config guard.
</Note>
...
<Steps>
  ...
  <Step title="Apply targeted edits">
    Apply targeted edits with `recall config set` or `recall configure`.
  </Step>
  <Step title="Re-validate">
    Rerun `recall config validate` after each change.
  </Step>
  <Step title="Doctor for runtime issues">
    If validation passes but the runtime is still unhealthy, run `recall doctor` or `recall doctor --fix` for migration and repair help.
  </Step>
</Steps>
