---
summary: "CLI reference for `steelengine config` (get/set/patch/unset/file/schema/validate)"
read_when:
...
Non-interactive helpers for `steelengine.json`: get/set/patch/unset a value by path, print the schema, validate, or print the active file path. Run `steelengine config` with no subcommand to open the same guided wizard as `steelengine configure`.

<Note>
When `STEELENGINE_NIX_MODE=1`, SteelEngine treats `steelengine.json` as immutable. Read-only commands (`config get`, `config file`, `config schema`, `config validate`) still work; config writers refuse. Edit the Nix source for the install instead; for the first-party nix-steelengine distribution, use the [nix-steelengine Quick Start](https://github.com/steelengine/nix-steelengine#quick-start) and set values under `programs.steelengine.config` or `instances.<name>.config`.
</Note>

## Root options

<ParamField path="--section <section>" type="string">
  Repeatable guided-setup section filter when you run `steelengine config` without a subcommand.
</ParamField>
...
steelengine config --section model
steelengine config --section gateway --section daemon
steelengine config schema
steelengine config get browser.executablePath
...
steelengine config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
steelengine config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
steelengine config set secrets.providers.vaultfile --provider-source file --provider-path /etc/steelengine/secrets.json --provider-mode json
...
steelengine config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
steelengine config validate
steelengine config validate --json
```
...
steelengine config get 'agents.list[0].id'
steelengine config get agents.list
steelengine config set 'agents.list[1].tools.exec.node' "node-id-or-name"
```
...
Prints the active config file path, resolved from `STEELENGINE_CONFIG_PATH` or the default location. The path names a regular file, not a symlink; see [Write safety](#write-safety).

### `config schema`

Prints the generated JSON schema for `steelengine.json` to stdout.
...
```bash
steelengine config schema
steelengine config schema > steelengine.schema.json
```
...
<Note>
If validation is already failing, start with `steelengine configure` or `steelengine doctor --fix`. `steelengine chat` does not bypass the invalid-config guard.
</Note>
...
```bash
steelengine config set agents.defaults.heartbeat.every "0m"
steelengine config set gateway.port 19001 --strict-json
steelengine config set channels.whatsapp.groups '["*"]' --strict-json
```
...
```bash
steelengine config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
steelengine config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
```
...
<Tabs>
  <Tab title="Value mode">
    ```bash
    steelengine config set <path> <value>
    ```
  </Tab>
  <Tab title="SecretRef builder mode">
    ```bash
    steelengine config set channels.discord.token \
  ...
  <Tab title="Provider builder mode">
    ...
    ```bash
    steelengine config set secrets.providers.vault \
      --provider-source exec \
      --provider-command /usr/local/bin/steelengine-vault \
      --provider-arg read \
  ...
  <Tab title="Batch mode">
    ```bash
    steelengine config set --batch-json '[
      {
...
steelengine config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/steelengine/secrets.json","mode":"json"}' \
  --strict-json
```
...
```bash
steelengine config patch --file ./steelengine.patch.json5 --dry-run
steelengine config patch --file ./steelengine.patch.json5
```

Pipe a patch over stdin for remote setup scripts:

```bash
ssh user@gateway-host 'steelengine config patch --stdin --dry-run' < ./steelengine.patch.json5
ssh user@gateway-host 'steelengine config patch --stdin' < ./steelengine.patch.json5
```
...
```bash
steelengine config patch --file ./discord.patch.json5 --replace-path 'channels.discord.guilds["123"].channels'
```
...
`--dry-run` validates changes without writing `steelengine.json`. Available on `config set`, `config patch`, and `config unset`.
...
`steelengine config set` and other SteelEngine-owned config writers validate the full post-change config before committing it to disk. If the new payload fails schema validation or looks like a destructive clobber, the active config is left alone and the rejected payload is saved beside it as `steelengine.json.rejected.*`.

SteelEngine-owned writes reserialize JSON5 as standard JSON. When the source contains comments, the writer warns immediately before removing them; use a direct editor when preserving comments matters.

<Warning>
The active config path must be a regular file. Symlinked `steelengine.json` layouts are unsupported for writes; use `STEELENGINE_CONFIG_PATH` to point directly at the real file instead.
</Warning>

Prefer CLI writes for small edits:

```bash
steelengine config set gateway.reload.mode hybrid --dry-run
steelengine config set gateway.reload.mode hybrid
steelengine config validate
```
...
Direct editor writes are still allowed, but the running Gateway treats them as untrusted until they validate. Invalid direct edits fail startup or are skipped by hot reload; Gateway does not rewrite `steelengine.json`. Run `steelengine doctor --fix` to repair prefixed/clobbered config or restore the last-known-good copy. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-rejected-invalid-config).
...
After `steelengine config validate` passes, use the local TUI to have an embedded agent compare the active config against the docs while you validate each change from the same terminal:
...
<Steps>
  ...
  <Step title="Apply targeted edits">
    Apply targeted edits with `steelengine config set` or `steelengine configure`.
  </Step>
  <Step title="Re-validate">
    Rerun `steelengine config validate` after each change.
  </Step>
  <Step title="Doctor for runtime issues">
    If validation passes but the runtime is still unhealthy, run `steelengine doctor` or `steelengine doctor --fix` for migration and repair help.
  </Step>
</Steps>
