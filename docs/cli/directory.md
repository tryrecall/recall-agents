---
summary: "CLI reference for `steelengine directory` (self, peers, groups)"
read_when:
...
# `steelengine directory`

Directory lookups for channels that support them: contacts/peers, groups, and "me" (self).

Results are meant to be pasted into other commands, especially `steelengine message send --target ...`.
...
```bash
steelengine directory peers list --channel slack --query "U0"
steelengine message send --channel slack --target user:U012ABCDEF --message "hello"
```
...
```bash
steelengine directory peers list --channel zalouser
steelengine directory peers list --channel zalouser --query "name"
steelengine directory peers list --channel zalouser --limit 50
```

## Groups

```bash
steelengine directory groups list --channel zalouser
