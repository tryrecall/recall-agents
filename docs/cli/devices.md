---
summary: "CLI reference for `steelengine devices` (device pairing + token rotation/revocation)"
read_when:
...
### `steelengine devices approve [requestId] [--latest]`
...
```bash
steelengine devices approve
steelengine devices approve <requestId>
steelengine devices approve --latest
```

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), SteelEngine supersedes the previous pending entry with a new `requestId`. Run `steelengine devices list` right before approval to get the current id.
</Note>

Approval behavior:

- If the device is already paired and requests broader scopes or role, SteelEngine keeps the existing approval and creates a new pending upgrade request. Compare `Requested` vs `Approved` in `steelengine devices list`, or preview with `--latest`, before approving.
...
### `steelengine devices remove <deviceId>`

Remove one paired device entry.

```bash
steelengine devices remove <deviceId>
steelengine devices remove <deviceId> --json
```
...
### `steelengine devices rename --device <id> --name <label>`
...
```bash
steelengine devices rename --device <deviceId> --name "Kitchen Mac"
steelengine devices rename --device <deviceId> --name "Kitchen Mac" --json
```
...
### `steelengine devices clear --yes [--pending]`

Clear paired devices in bulk. Gated by `--yes`.

```bash
steelengine devices clear --yes
steelengine devices clear --yes --pending
steelengine devices clear --yes --pending --json
```

`--pending` also rejects all pending pairing requests.

### `steelengine devices rotate --device <id> --role <role> [--scope <scope...>]`

Rotate a device token for a role, optionally updating its scopes.

```bash
steelengine devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `steelengine devices revoke --device <id> --role <role>`

Revoke a device token for a role.

```bash
steelengine devices revoke --device <deviceId> --role node
```
...
3. Rotate the operator token for the affected device:
   ...
   ```bash
   steelengine devices rotate --device <deviceId> --role operator
   ```
...
## Paperclip / `steelengine_gateway` first-run approval

Paperclip agents connecting through the `steelengine_gateway` adapter go through the same first-run device pairing approval as any other new client. If Paperclip reports `steelengine_gateway_pairing_required`, approve the pending device and retry.

```bash
steelengine devices approve --latest
```

The preview prints the exact `steelengine devices approve <requestId>` command; verify the details, then rerun that command with the request ID to approve it. For a remote gateway or explicit credentials, pass the same options while previewing and approving:

```bash
steelengine devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```
...
If approval keeps failing, run `steelengine devices list` first to confirm a pending request exists.
