---
summary: "CLI reference for `recall doctor` (health checks + guided repairs)"
read_when:
...
`recall doctor` is the Recall health surface. Use it when the gateway,
...
| ------- | ------------------------ | ------------------------------------------------------------------------------- |
| Inspect | `recall doctor`        | Human-oriented checks and guided prompts.                                       |
| Repair  | `recall doctor --fix`  | Applies supported repairs, using prompts unless non-interactive repair is safe. |
| Lint    | `recall doctor --lint` | Read-only structured findings for CI, preflight, and review gates.              |
...
recall doctor --deep
recall doctor --fix
recall doctor --fix --non-interactive
...
```bash
recall channels capabilities --channel discord --target channel:<channel-id>
...
`recall doctor --lint` is the read-only automation posture for doctor checks.
...
doctor --lint: ran 6 check(s), 1 finding(s)
  [warning] core/doctor/gateway-config gateway.mode - gateway.mode is unset; gateway start will be blocked.
    fix: Run `recall configure` and set Gateway mode (local/remote), or `recall config set gateway.mode local`.
```
  ...
  "findings": [
    {
      ...
      "path": "gateway.mode",
      "fixHint": "Run `recall configure` and set Gateway mode (local/remote), or `recall config set gateway.mode local`."
    }
  ]
}
```
...
example, `recall doctor --lint --severity-min error` can print no findings and
...
health path. The `recall/plugin-sdk/health` subpath exposes the same
...
```bash
recall doctor --lint --only core/doctor/gateway-config --json
recall doctor --lint --skip core/doctor/skills-readiness
```
...
- In Nix mode (`RECALL_NIX_MODE=1`), read-only doctor checks still work, but `doctor --fix`, `doctor --repair`, `doctor --yes`, and `doctor --generate-gateway-token` are disabled because `recall.json` is immutable. Edit the Nix source for this install instead; for nix-recall, use the agent-first [Quick Start](https://github.com/recall/nix-recall#quick-start).
...
- `--fix` (alias for `--repair`) writes a backup to `~/.tryrecall/recall-agents.json.bak` and drops unknown config keys, listing each removal.
...
- `doctor --fix --non-interactive` reports missing or stale gateway service definitions but does not install or rewrite them outside update repair mode. Run `recall gateway install` for a missing service, or `recall gateway install --force` when you intentionally want to replace the launcher.
...
- Doctor also scans `~/.recall/cron/jobs.json` (or `cron.store`) for legacy cron job shapes and can rewrite them in place before the scheduler has to auto-normalize them at runtime.
...
- On Linux, doctor warns when the user's crontab still runs legacy `~/.recall/bin/ensure-whatsapp.sh`; that script is no longer maintained and can log false WhatsApp gateway outages when cron lacks the systemd user-bus environment.
- When WhatsApp is enabled, doctor checks for a degraded Gateway event loop with local `recall-tui` clients still running. `doctor --fix` stops only verified local TUI clients so WhatsApp replies are not queued behind stale TUI refresh loops.
...
- Doctor cleans legacy plugin dependency staging state created by older Recall versions and relinks the host `recall` package for managed npm plugins that declare it as a peer dependency. It also repairs missing downloadable plugins that are referenced by config, such as `plugins.entries`, configured channels, configured provider/search settings, or configured agent runtimes. During package updates, doctor skips package-manager plugin repair until the package swap is complete; rerun `recall doctor --fix` afterward if a configured plugin still needs recovery. If the download fails, doctor reports the install error and preserves the configured plugin entry for the next repair attempt.
...
- Set `RECALL_SERVICE_REPAIR_POLICY=external` when another supervisor owns the gateway lifecycle. Doctor still reports gateway/service health and applies non-service repairs, but skips service install/start/restart/bootstrap and legacy service cleanup.
- On Linux, doctor ignores inactive extra gateway-like systemd units and does not rewrite command/entrypoint metadata for a running systemd gateway service during repair. Stop the service first or use `recall gateway install --force` when you intentionally want to replace the active launcher.
...
- Doctor includes a memory-search readiness check and can recommend `recall configure --section model` when embedding credentials are missing.
...
- Doctor reports an info note when Codex-mode agents are configured and personal Codex CLI assets exist in the operator's Codex home. Local Codex app-server launches use isolated per-agent homes, so use `recall migrate plan codex` to inventory assets that should be promoted deliberately.
...
- If sandbox mode is enabled but Docker is unavailable, doctor reports a high-signal warning with remediation (`install Docker` or `recall config set agents.defaults.sandbox.mode off`).
- If legacy sandbox registry files (`~/.recall/sandbox/containers.json` or `~/.recall/sandbox/browsers.json`) are present, doctor reports them; `recall doctor --fix` migrates valid entries into sharded registry directories and quarantines invalid legacy files.
...
If you previously ran `launchctl setenv RECALL_GATEWAY_TOKEN ...` (or `...PASSWORD`), that value overrides your config file and can cause persistent "unauthorized" errors.

```bash
launchctl getenv RECALL_GATEWAY_TOKEN
launchctl getenv RECALL_GATEWAY_PASSWORD

launchctl unsetenv RECALL_GATEWAY_TOKEN
launchctl unsetenv RECALL_GATEWAY_PASSWORD
```
