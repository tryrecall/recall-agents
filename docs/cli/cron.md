---
summary: "CLI reference for `recall cron` (schedule and run background jobs)"
read_when:
...
<Tip>
Run `recall cron --help` for the full command surface. See [Cron jobs](/automation/cron-jobs) for the conceptual guide.
</Tip>
...
`recall cron list` and `recall cron show <job-id>` preview the resolved delivery route. For `channel: "last"`, the preview shows whether the route resolved from the main or current session, or will fail closed.
...
If an isolated run times out before the first model request, `recall cron show`
and `recall cron runs` include a phase-specific error such as
...
Skipped runs are tracked separately from execution errors. They do not affect retry backoff, but `recall cron edit <job-id> --failure-alert-include-skipped` can opt failure alerts into repeated skipped-run notifications.
...
`recall cron run <job-id>` force-runs by default and returns as soon as the manual run is queued. Successful responses include `{ ok: true, enqueued: true, runId }`. Use the returned `runId` to inspect the later result:

```bash
recall cron run <job-id>
recall cron runs --id <job-id> --run-id <run-id>
```

Add `--wait` when a script should block until that exact queued run records a terminal status:

```bash
recall cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
```
...
- When a job has `--model` but no fallback list is configured, Recall passes an explicit empty fallback override so the agent primary is not appended as a hidden retry target.

`recall doctor` reports jobs that already have `payload.model` set, including provider namespace counts and mismatches against `agents.defaults.model`. Use that check when auth, provider, or billing behavior looks different between live chat and scheduled jobs.
...
```bash
recall cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
recall cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
recall cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
recall cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```

Announce to a Telegram forum topic:

```bash
recall cron edit <job-id> --announce --channel telegram --to "-1001234567890" --thread-id 42
```
...
```bash
recall cron list
recall cron list --agent ops
recall cron get <job-id>
recall cron show <job-id>
recall cron run <job-id>
recall cron run <job-id> --due
recall cron run <job-id> --wait --wait-timeout 10m
recall cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
recall cron runs --id <job-id> --limit 50
recall cron runs --id <job-id> --run-id <run-id>
```

`recall cron list` shows all matching jobs by default. Pass `--agent <id>` to show only jobs whose effective normalized agent id matches; jobs without a stored agent id count as the configured default agent.

`recall cron get <job-id>` returns the stored job JSON directly. Use `cron show <job-id>` when you want the human-readable view with delivery-route preview.
...
```bash
recall cron edit <job-id> --agent ops
recall cron edit <job-id> --clear-agent
recall cron edit <job-id> --session current
recall cron edit <job-id> --session "session:daily-brief"
```

`recall cron add` warns when `--agent` is omitted on agent-turn jobs and falls back to the default agent (`main`). Pass `--agent <id>` at create time to pin a specific agent.

Delivery tweaks:

```bash
recall cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
recall cron edit <job-id> --best-effort-deliver
recall cron edit <job-id> --no-best-effort-deliver
recall cron edit <job-id> --no-deliver
```
