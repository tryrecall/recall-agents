---
summary: "Recall Gateway CLI (`recall gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is Recall's WebSocket server (channels, nodes, sessions, hooks). Subcommands in this page live under `recall gateway …`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How Recall advertises and finds gateways.
  </Card>
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    - By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.tryrecall/recall-agents.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
    - `recall onboard --mode local` and `recall setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
<ParamField path="--token <token>" type="string">
  Token override (also sets `RECALL_GATEWAY_TOKEN` for the process).
</ParamField>
...
<ParamField path="--bind custom + gateway.customBindHost" type="string">
  Expects an IPv4 address today. For IPv6-only BYOH, place an IPv4 sidecar or proxy in front of the Gateway and point Recall at that IPv4 endpoint.
</ParamField>
...
recall gateway restart --safe
recall gateway restart --safe --skip-deferral
recall gateway restart --force
```

`recall gateway restart --safe` asks the running Gateway to preflight active Recall work before restarting. If queued operations, reply delivery, embedded runs, or task runs are active, the Gateway reports the blockers, coalesces duplicate safe restart requests, and restarts once the active work drains. Plain `restart` keeps the existing service-manager behavior for compatibility. Use `--force` only when you explicitly want the immediate override path.

`recall gateway restart --safe --skip-deferral` runs the same Recall-aware coordinated restart as `--safe`, but bypasses the active-work deferral gate so the Gateway emits the restart immediately even when blockers are reported. Use it as the operator escape hatch when a deferral has been pinned by a stuck task run and `--safe` alone would wait indefinitely. `--skip-deferral` requires `--safe`.
...
- Set `RECALL_GATEWAY_STARTUP_TRACE=1` to log phase timings during Gateway startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings for installed-index, manifest registry, startup planning, and owner-map work.
- Set `RECALL_GATEWAY_RESTART_TRACE=1` to log restart-scoped `restart trace:` lines for restart signal handling, active-work drain, shutdown phases, next start, ready timing, and memory metrics.
- Set `RECALL_DIAGNOSTICS=timeline` with `RECALL_DIAGNOSTICS_TIMELINE_PATH=<path>` to write a best-effort JSONL startup diagnostics timeline for external QA harnesses. You can also enable the flag with `diagnostics.flags: ["timeline"]` in config; the path is still env-provided. Add `RECALL_DIAGNOSTICS_EVENT_LOOP=1` to include event-loop samples.
...
```bash
recall gateway health --url ws://127.0.0.1:18789
```
...
```bash
recall gateway usage-cost
recall gateway usage-cost --days 7
recall gateway usage-cost --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - On fatal Gateway exits, shutdown timeouts, and restart startup failures, Recall writes the same diagnostic snapshot to `~/.recall/logs/stability/recall-stability-*.json` when the recorder has events. Inspect the newest bundle with `recall gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` also apply to bundle output.
...
recall gateway status --require-rpc
```
...
```bash
recall gateway call status
recall gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
responsible for eventually exec'ing `recall` or Node with those args.

```bash
cat > ~/.local/bin/recall-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- recall "$@"
EOF
chmod +x ~/.local/bin/recall-doppler

recall gateway install --wrapper ~/.local/bin/recall-doppler --force
recall gateway restart
```
...
`RECALL_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor
repairs.

```bash
RECALL_WRAPPER="$HOME/.local/bin/recall-doppler" recall gateway install --force
recall doctor
```

To remove a persisted wrapper, clear `RECALL_WRAPPER` while reinstalling:

```bash
RECALL_WRAPPER= recall gateway install --force
recall gateway restart
```
...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - `gateway restart --safe` asks the running Gateway to preflight active Recall work and defer the restart until reply delivery, embedded runs, and task runs drain. `--safe` cannot be combined with `--force` or `--wait`.
    ...
    - `gateway restart --safe --skip-deferral` runs the Recall-aware safe restart but bypasses the deferral gate so the Gateway emits the restart immediately even when blockers are reported. Operator escape hatch for stuck-task-run deferrals; requires `--safe`.
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `RECALL_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `RECALL_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `recall.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
...
```bash
recall gateway discover --timeout 4000
recall gateway discover --json | jq '.beacons[].wsUrl'
```
