...
```bash
recall dns setup
recall dns setup --domain recall.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `recall.internal`)
...
- zone file path
- current tailnet IPs
- recommended `recall.json` discovery config
...
- If `--domain` is omitted, Recall uses `discovery.wideArea.domain` from config.
