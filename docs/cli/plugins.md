---
summary: "CLI reference for `recall plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
Bundled plugins ship with Recall but start disabled. Use `plugins enable` to
activate them.

Native Recall plugins must ship `recall.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: recall` or `Format: bundle`. Verbose list/info
...
`recall.hooks` in `package.json`. Use `recall hooks` for filtered hook
...
If a bare install spec matches a bundled plugin id (for example `diffs`), Recall
...
Recall now also prefers ClawHub for bare npm-safe plugin specs. It only falls
...
Recall downloads the package archive from ClawHub, checks the advertised
...
inside the cloned marketplace repo. Recall accepts relative path sources from
...
state dir extensions root (`$RECALL_STATE_DIR/extensions/<id>`). Use
...
recall plugins update @recall/voice-call@beta
```

Updates apply to tracked installs in `plugins.installs` and tracked hook-pack
installs in `hooks.internal.installs`.

When you pass a plugin id, Recall reuses the recorded install spec for that
...
or exact version. Recall resolves that package name back to the tracked plugin
