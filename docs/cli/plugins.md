---
summary: "CLI reference for `steelengine plugins` (init, build, validate, list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
```bash
steelengine plugins list [--enabled] [--verbose] [--json]
steelengine plugins search <query> [--limit <n>] [--json]
steelengine plugins install <path-or-spec> [--link] [--force] [--pin] [--marketplace <source>]
steelengine plugins inspect <id> [--runtime] [--json]
steelengine plugins inspect --all [--runtime] [--json]
steelengine plugins info <id>                    # alias for inspect
steelengine plugins enable <id>
steelengine plugins disable <id>
steelengine plugins uninstall <id> [--dry-run] [--keep-files] [--force]
steelengine plugins update <id-or-npm-spec> | --all [--dry-run]
steelengine plugins registry [--refresh] [--json]
steelengine plugins doctor
steelengine plugins init <id> [--name <name>] [--type tool|provider] [--directory <path>]
steelengine plugins build [--entry <path>] [--check]
steelengine plugins validate [--entry <path>]
steelengine plugins marketplace entries [--offline] [--feed-profile <name>] [--json]
steelengine plugins marketplace list <source> [--json]
steelengine plugins marketplace refresh [--feed-profile <name>] [--expected-sha256 <sha256>] [--json]
```

For slow install, inspect, uninstall, or registry-refresh investigation, run the
command with `STEELENGINE_PLUGIN_LIFECYCLE_TRACE=1`. The trace writes phase timings
to stderr and keeps JSON output parseable. See [Debugging](/help/debugging#plugin-lifecycle-trace).

<Note>
In Nix mode (`STEELENGINE_NIX_MODE=1`), `steelengine.json` is immutable. `install`, `update`, `uninstall`, `enable`, and `disable` all refuse to run. Edit the Nix source for this install instead (`programs.steelengine.config` or `instances.<name>.config` for nix-steelengine), then rebuild. See the agent-first [Quick Start](https://github.com/steelengine/nix-steelengine#quick-start).
</Note>

<Note>
Bundled plugins ship with SteelEngine. Some are enabled by default (for example bundled model providers, bundled speech providers, and the bundled browser plugin); others require `plugins enable`.

Native SteelEngine plugins ship `steelengine.plugin.json` with an inline JSON Schema (`configSchema`, even if empty). Compatible bundles use their own bundle manifests instead.

`plugins list` shows `Format: steelengine` or `Format: bundle`. Verbose list/info output also shows the bundle subtype (`codex`, `claude`, or `cursor`) plus detected bundle capabilities.
</Note>

## Author

```bash
steelengine plugins init stock-quotes --name "Stock Quotes"
cd stock-quotes
...
argument is the plugin id; `--name` sets the display name. SteelEngine uses the
...
`plugin:validate` that build then call `steelengine plugins build`/`validate`.

`plugins build` imports the built entry, reads its static tool metadata, writes
`steelengine.plugin.json`, and keeps `package.json`'s `steelengine.extensions` aligned.
...
```bash
steelengine plugins init acme-models --name "Acme Models" --type provider
cd acme-models
npm install
npm run build
npm test
...
`steelengine plugins build`/`validate`; those commands are for the tool
...
```bash
steelengine plugins search "calendar"                      # search ClawHub plugins
steelengine plugins install @steelengine/<package>            # trusted official catalog
steelengine plugins install <package>                       # arbitrary npm package
steelengine plugins install clawhub:<package>                # ClawHub only
steelengine plugins install npm:<package>                    # npm only
steelengine plugins install npm-pack:<path.tgz>               # local npm-pack tarball
steelengine plugins install git:github.com/<owner>/<repo>     # git repo
steelengine plugins install git:github.com/<owner>/<repo>@<ref>
steelengine plugins install <path>                            # local path or archive
steelengine plugins install -l <path>                         # link instead of copy
steelengine plugins install <plugin>@<marketplace>             # marketplace shorthand
steelengine plugins install <plugin> --marketplace <name>      # marketplace (explicit)
steelengine plugins install <package> --force                  # confirm source / overwrite existing
steelengine plugins install <package> --pin                    # pin resolved npm version
steelengine plugins install clawhub:<package> --acknowledge-clawhub-risk
steelengine plugins install <package> --dangerously-force-unsafe-install
```
...
<Warning>
Bare package names install from npm by default during the launch cutover, unless they match a bundled or official plugin id, in which case SteelEngine uses that local/official copy instead of hitting the npm registry. Use `npm:<package>` when you deliberately want an external npm package instead. Use `clawhub:<package>` for ClawHub. Treat plugin installs like running code; prefer pinned versions.
</Warning>

<Warning>
ClawHub packages and SteelEngine's bundled/official catalog are trusted install
...
`bundle-plugin` packages (not skills; use `steelengine skills search` for those).
...
summary, and an install hint such as `steelengine plugins install clawhub:<package>`.

<Note>
ClawHub is the primary distribution and discovery surface for most plugins. Npm
remains a supported fallback and direct-install path. SteelEngine-owned
`@steelengine/*` plugin packages are published on npm again; see the current list
on [npmjs.com/org/steelengine](https://www.npmjs.com/org/steelengine) or the
...
<AccordionGroup>
  <Accordion title="Config includes and invalid-config repair">
    If your `plugins` section is backed by a single-file `$include`, `plugins install/update/enable/disable/uninstall` write through to that included file and leave `steelengine.json` untouched. Root includes, include arrays, and includes with sibling overrides fail closed instead of flattening. See [Config includes](/gateway/configuration) for the supported shapes.

    If config is invalid during install, `plugins install` normally fails closed and tells you to run `steelengine doctor --fix` first. During Gateway startup and hot reload, invalid plugin config fails closed like any other invalid config; `steelengine doctor --fix` can quarantine the invalid plugin entry. The only documented install-time exception is a narrow bundled-plugin recovery path for plugins that explicitly opt into `steelengine.install.allowInvalidConfigRecovery`.

  </Accordion>
  <Accordion title="--force confirmation and reinstall vs update">
    `--force` confirms a non-ClawHub source without prompting. It does not bypass `security.installPolicy` or remaining install safety checks. When the plugin or hook pack is already installed, it also reuses the existing target and overwrites it in place. Use it after reviewing an arbitrary npm, local, archive, git, or marketplace source, or when intentionally reinstalling the same id. For routine upgrades of an already tracked npm plugin, prefer `steelengine plugins update <id-or-npm-spec>`.

    If you run `plugins install` for a plugin id that is already installed, SteelEngine stops and points you at `plugins update <id-or-npm-spec>` for a normal upgrade, or at `plugins install <package> --force` when you genuinely want to overwrite the current install from a different source. Arbitrary sources still show the interactive provenance warning; noninteractive installs must pass `--force` after review. Trusted ClawHub and SteelEngine-catalog sources do not need it. With `--link`, `--force` confirms the source but does not change the linked-path install mode.
  ...
  <Accordion title="--dangerously-force-unsafe-install">
    `--dangerously-force-unsafe-install` is deprecated and is now a no-op. SteelEngine no longer runs built-in install-time dangerous-code blocking for plugin installs.
  ...
  <Accordion title="--acknowledge-clawhub-risk">
    Community ClawHub installs check the selected release's trust record before downloading. If ClawHub disables download for the release, reports malicious scan findings, or puts the release in a blocking moderation state (quarantined, revoked), SteelEngine refuses it outright regardless of this flag. For non-blocking risky scan statuses or moderation states, SteelEngine shows the trust details and asks for confirmation before continuing.

    Use `--acknowledge-clawhub-risk` only after reviewing the ClawHub warning and deciding to continue without an interactive prompt. Pending or stale (not-yet-clean) scan results warn but do not require acknowledgement. Official ClawHub packages and bundled SteelEngine plugin sources bypass this release-trust check entirely.

  </Accordion>
  <Accordion title="Hook packs and npm specs">
    `plugins install` is also the install surface for hook packs that expose `steelengine.hooks` in `package.json`. Use `steelengine hooks` for filtered hook visibility and per-hook enablement, not package installation.

    Npm specs are **registry-only** (package name plus optional **exact version** or **dist-tag**). Git/URL/file specs and semver ranges are rejected. Dependency installs run in one managed npm project per plugin with `--ignore-scripts` for safety, even when your shell has global npm install settings. Managed plugin npm projects inherit SteelEngine's package-level npm `overrides`, so host security pins apply to hoisted plugin dependencies too.
    ...
    Raw `@steelengine/*` specs that match bundled plugins resolve to the image-owned bundled copy before npm fallback. For example, `steelengine plugins install @steelengine/discord@2026.5.20 --pin` uses the bundled Discord plugin from the current SteelEngine build instead of creating a managed npm override. To force the external npm package, use `steelengine plugins install npm:@steelengine/discord@2026.5.20 --pin`.

    Bare specs and `@latest` stay on the stable track. SteelEngine date-stamped correction versions such as `2026.5.3-1` count as stable for this check. If npm resolves either form to a prerelease, SteelEngine stops and asks you to opt in explicitly with a prerelease tag (`@beta`/`@rc`) or an exact prerelease version (`@1.2.3-beta.4`).

    For npm installs without an exact version (`npm:<package>` or `npm:<package>@latest`), SteelEngine checks the resolved package metadata before install. If the latest stable package requires a newer SteelEngine plugin API or minimum host version, SteelEngine inspects older stable versions and installs the newest compatible release instead. Exact versions and explicit dist-tags stay strict: an incompatible selection fails and asks you to upgrade SteelEngine or choose a compatible version.

    If a bare install spec matches an official plugin id (for example `diffs`), SteelEngine installs the catalog entry directly. To install an npm package with the same name, use an explicit scoped spec (for example `@scope/diffs`).
  ...
  <Accordion title="Git repositories">
    ...
    Git installs clone into a temporary directory, check out the requested ref when present, then use the normal plugin directory installer, so manifest validation, operator install policy, package-manager install work, and install records behave like npm installs. Recorded git installs include the source URL/ref plus the resolved commit so `steelengine plugins update` can re-resolve the source later.

    After installing from git, use `steelengine plugins inspect <id> --runtime --json` to verify runtime registrations such as gateway methods and CLI commands. If the plugin registered a CLI root with `api.registerCli`, run that command directly through the SteelEngine root CLI, for example `steelengine demo-plugin ping`.

  </Accordion>
  <Accordion title="Archives">
    Supported archives: `.zip`, `.tgz`, `.tar.gz`, `.tar`. Native SteelEngine plugin archives must contain a valid `steelengine.plugin.json` at the extracted plugin root; archives that only contain `package.json` are rejected before SteelEngine writes install records.
...
```bash
steelengine plugins install clawhub:steelengine-codex-app-server
steelengine plugins install clawhub:steelengine-codex-app-server@1.2.3
```

Bare npm-safe plugin specs install from npm by default during the launch cutover unless they match an official plugin id:

```bash
steelengine plugins install steelengine-codex-app-server
```

Use `npm:` to make npm-only resolution explicit:

```bash
steelengine plugins install npm:steelengine-codex-app-server
steelengine plugins install npm:@steelengine/discord@2026.5.20
steelengine plugins install npm:@scope/plugin-name@1.0.1
```

SteelEngine checks the advertised plugin API / minimum gateway compatibility before install. When the selected ClawHub version publishes a ClawPack artifact, SteelEngine downloads the versioned npm-pack `.tgz`, verifies the ClawHub digest header and the artifact digest, then installs it through the normal archive path. Older ClawHub versions without ClawPack metadata still install through the legacy package archive verification path. Recorded installs keep their ClawHub source metadata, artifact kind, npm integrity, npm shasum, tarball name, and ClawPack digest facts for later updates.
Unversioned ClawHub installs keep an unversioned recorded spec so `steelengine plugins update` can follow newer ClawHub releases; explicit version or tag selectors such as `clawhub:pkg@1.2.3` and `clawhub:pkg@beta` remain pinned to that selector.
...
```bash
steelengine plugins marketplace list <marketplace-name>
steelengine plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` to pass the marketplace source explicitly:

```bash
steelengine plugins install <plugin-name> --marketplace <marketplace-name>
steelengine plugins install <plugin-name> --marketplace <owner/repo>
steelengine plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
steelengine plugins install <plugin-name> --marketplace ./my-marketplace
```
...
<Tabs>
  ...
  <Tab title="Remote marketplace rules">
    For remote marketplaces loaded from GitHub or git, plugin entries must stay inside the cloned marketplace repo. SteelEngine accepts relative path sources from that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path plugin sources from remote manifests.
  </Tab>
</Tabs>

For local paths and archives, SteelEngine auto-detects:

- native SteelEngine plugins (`steelengine.plugin.json`)
...
`~/.steelengine/extensions` or `<workspace>/.steelengine/extensions`; those
...
```bash
steelengine plugins install -l ./my-plugin
```
...
run `steelengine plugins enable <plugin-id>` or set
...
steelengine plugins list --enabled
steelengine plugins list --verbose
...
run `steelengine plugins list --enabled --verbose` or
`steelengine plugins inspect <id>` with a listed plugin id to confirm the plugin
ids and copy trusted ids into `plugins.allow` in `steelengine.json`. When the
...
so SteelEngine records install provenance.
...
`/app/extensions/synology-chat`. SteelEngine discovers that mounted source overlay
...
- `steelengine plugins inspect <id> --runtime --json` shows registered hooks and diagnostics from a module-loaded inspection pass. Runtime inspection never installs dependencies; use `steelengine doctor --fix` to clean legacy dependency state or recover missing downloadable plugins that are referenced by config.
- `steelengine gateway status --deep --require-rpc` confirms the reachable Gateway URL/profile, service/process hints, config path, and RPC health.
...
Plugin install metadata is machine-managed state, not user config. Installs and updates write it to the shared SQLite state database under the active SteelEngine state directory. The `installed_plugin_index` row stores durable `installRecords` metadata, including records for broken or missing plugin manifests, plus a manifest-derived cold registry cache used by `steelengine plugins update`, uninstall, diagnostics, and the cold plugin registry.

When SteelEngine sees shipped legacy `plugins.installs` records in config, runtime reads treat them as compatibility input without rewriting `steelengine.json`. Explicit plugin writes and `steelengine doctor --fix` move those records into the plugin index and remove the config key when config writes are allowed; if either write fails, the config records are kept so the install metadata is not lost.

## Uninstall

```bash
steelengine plugins uninstall <id>
steelengine plugins uninstall <id> --dry-run
steelengine plugins uninstall <id> --keep-files
steelengine plugins uninstall <id> --force
```

`uninstall` removes plugin records from `plugins.entries`, the persisted plugin index, plugin allow/deny list entries, and linked `plugins.load.paths` entries when applicable. Unless `--keep-files` is set, uninstall also removes the tracked managed install directory, but only when it resolves inside SteelEngine's plugin extensions root. If the plugin currently owns the `memory` or `contextEngine` slot, that slot resets to its default (`memory-core` for memory, `legacy` for context engine).
...
steelengine plugins update <id-or-npm-spec> --dry-run
steelengine plugins update @steelengine/voice-call
steelengine plugins update @acme/demo
steelengine plugins update steelengine-codex-app-server --acknowledge-clawhub-risk
steelengine plugins update steelengine-codex-app-server --dangerously-force-unsafe-install
```
...
<AccordionGroup>
  <Accordion title="Resolving plugin id vs npm spec">
    When you pass a plugin id, SteelEngine reuses the recorded install spec for that plugin. That means previously stored dist-tags such as `@beta` and exact pinned versions continue to be used on later `update <id>` runs.

    During `update <id> --dry-run`, exact pinned npm installs stay pinned. If SteelEngine can also resolve the package's registry default line and that default line is newer than the installed pinned version, the dry run reports the pin and prints the explicit `@latest` package update command to follow the registry default line.

    That targeted-update rule differs from the bulk `steelengine plugins update --all` maintenance path. Bulk updates still respect ordinary tracked install specs, but trusted official SteelEngine plugin records can sync to the current official catalog target instead of staying on a stale exact official package. Use targeted `update <id>` when you intentionally want to keep an exact or tagged official spec untouched.

    For npm installs, you can also pass an explicit npm package spec with a dist-tag or exact version. SteelEngine resolves that package name back to the tracked plugin record, updates that installed plugin, and records the new npm spec for future id-based updates.
  ...
  <Accordion title="--acknowledge-clawhub-risk on update">
    Community ClawHub-backed plugin updates run the same exact-release trust check as installs before downloading the replacement package. Use `--acknowledge-clawhub-risk` for reviewed automation that should continue when the selected ClawHub release has a risky trust warning. Official ClawHub packages and bundled SteelEngine plugin sources bypass this release-trust prompt.
  </Accordion>
</AccordionGroup>
...
steelengine plugins inspect <id> --runtime
steelengine plugins inspect <id> --json
steelengine plugins inspect --all
```

Inspect shows identity, load status, source, manifest capabilities, policy flags, diagnostics, install metadata, bundle capabilities, and any detected MCP or LSP server support without importing plugin runtime by default. JSON output includes the plugin manifest contracts, such as `contracts.agentToolResultMiddleware` and `contracts.trustedToolPolicies`, so operators can audit trusted-surface declarations before enabling or restarting a plugin. Add `--runtime` to load the plugin module and include registered hooks, tools, commands, services, gateway methods, and HTTP routes. Runtime inspection reports missing plugin dependencies directly; installs and repairs stay in `steelengine plugins install`, `steelengine plugins update`, and `steelengine doctor --fix`.

Plugin-owned CLI commands are usually installed as root `steelengine` command groups, but plugins may also register nested commands under a core parent such as `steelengine nodes`. After `inspect --runtime` shows a command under `cliCommands`, run it at the listed path; for example a plugin that registers `demo-git` can be verified with `steelengine demo-git ping`.
...
For module-shape failures such as missing `register`/`activate` exports, rerun with `STEELENGINE_PLUGIN_LOAD_DEBUG=1` to include a compact export-shape summary in the diagnostic output.

## Registry

```bash
steelengine plugins registry
steelengine plugins registry --refresh
steelengine plugins registry --json
```

The local plugin registry is SteelEngine's persisted cold read model for installed plugin identity, enablement, source metadata, and contribution ownership. Normal startup, provider owner lookup, channel setup classification, and plugin inventory can read it without importing plugin runtime modules.

Use `plugins registry` to inspect whether the persisted registry is present, current, or stale. Use `--refresh` to rebuild it from the persisted plugin index, config policy, and manifest/package metadata. This is a repair path, not a runtime activation path.

`steelengine doctor --fix` also repairs registry-adjacent managed npm drift: if an orphaned or recovered `@steelengine/*` package under a managed plugin npm project or the legacy flat managed npm root shadows a bundled plugin, doctor removes that stale package and rebuilds the registry so startup validates against the bundled manifest. Doctor also relinks the host `steelengine` package into managed npm plugins that declare `peerDependencies.steelengine`, so package-local runtime imports such as `steelengine/plugin-sdk/*` resolve after updates or npm repairs.

<Warning>
`STEELENGINE_DISABLE_PERSISTED_PLUGIN_REGISTRY=1` is a deprecated break-glass compatibility switch for registry read failures. Prefer `plugins registry --refresh` or `steelengine doctor --fix`; the env fallback is only for emergency startup recovery while the migration rolls out.
</Warning>

## Marketplace

```bash
steelengine plugins marketplace entries
steelengine plugins marketplace entries --offline
steelengine plugins marketplace entries --json
steelengine plugins marketplace entries --feed-profile <name>
steelengine plugins marketplace entries --feed-url <url>
steelengine plugins marketplace list <source>
steelengine plugins marketplace list <source> --json
steelengine plugins marketplace refresh
steelengine plugins marketplace refresh --feed-profile <name>
steelengine plugins marketplace refresh --feed-url <url>
steelengine plugins marketplace refresh --expected-sha256 <sha256> --json
```

`plugins marketplace entries` lists entries from the configured SteelEngine marketplace feed. By default it attempts the hosted feed and falls back to the latest accepted snapshot or bundled data. Use `--feed-profile <name>` to read a specific configured profile, `--feed-url <url>` to read an explicit hosted feed URL, and `--offline` to read the latest accepted snapshot without fetching the feed.

`plugins marketplace refresh` refreshes the configured hosted feed snapshot and reports whether SteelEngine accepted hosted data, a hosted snapshot, or bundled fallback data. Use `--expected-sha256` when a caller needs the command to fail unless a fresh hosted payload matches a pinned checksum.
...
Marketplace refresh loads a hosted SteelEngine marketplace feed and persists the
...
refreshes fail if SteelEngine cannot persist the validated snapshot.
