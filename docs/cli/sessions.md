---
summary: "CLI reference for `recall sessions` (list stored sessions + usage)"
read_when:
...
until a message is processed. Use `recall channels status --probe`,
`recall status --deep`, or `recall health --verbose` when you need live
channel connectivity.

`recall sessions` and Gateway `sessions.list` responses are bounded by
...
```bash
recall sessions
recall sessions --agent work
recall sessions --all-agents
recall sessions --active 120
recall sessions --limit 25
recall sessions --verbose
recall sessions --json
```
...
recall sessions export-trajectory --session-key "agent:main:telegram:direct:123" --output bug-123 --json
```
...
inside `.recall/trajectory-exports/` under the selected workspace.

`recall sessions --all-agents` reads configured agent stores. Gateway and ACP
...
`recall sessions --all-agents --json`:

```json
{
  "path": null,
  "stores": [
    { "agentId": "main", "path": "/home/user/.recall/agents/main/sessions/sessions.json" },
    { "agentId": "work", "path": "/home/user/.recall/agents/work/sessions/sessions.json" }
  ],
  "allAgents": true,
  "count": 2,
  "totalCount": 2,
...
recall sessions cleanup --agent work --dry-run
recall sessions cleanup --all-agents --dry-run
recall sessions cleanup --enforce
recall sessions cleanup --enforce --active-key "agent:main:telegram:direct:123"
recall sessions cleanup --dry-run --fix-dm-scope
recall sessions cleanup --json
```

`recall sessions cleanup` uses `session.maintenance` settings from config:

- Scope note: `recall sessions cleanup` maintains session stores, transcripts, and trajectory sidecars. It does not prune cron run logs (`cron/runs/<jobId>.jsonl`), which are managed by `cron.runLog.maxBytes` and `cron.runLog.keepLines` in [Cron configuration](/automation/cron-jobs#configuration) and explained in [Cron maintenance](/automation/cron-jobs#maintenance).
...
`recall sessions cleanup --all-agents --dry-run --json`:
...
```json
{
  ...
  "stores": [
    {
      "agentId": "main",
      "storePath": "/home/user/.recall/agents/main/sessions/sessions.json",
      "beforeCount": 120,
      "afterCount": 80,
      "missing": 0,
      "dmScopeRetired": 0,
    ...
    {
      "agentId": "work",
      "storePath": "/home/user/.recall/agents/work/sessions/sessions.json",
      "beforeCount": 18,
      "afterCount": 18,
      "missing": 0,
      "dmScopeRetired": 0,
