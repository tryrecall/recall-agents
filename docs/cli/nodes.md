---
summary: "CLI reference for `recall nodes` (list/status/approve/invoke, camera/canvas/screen)"
read_when:
...
recall nodes run --node <id|name|ip> <command...>
recall nodes run --raw "git status"
recall nodes run --agent main --node <id|name|ip> --raw "git status"
```
