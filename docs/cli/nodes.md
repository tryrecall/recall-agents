---
summary: "CLI reference for `steelengine nodes` (status, pairing, invoke, camera/canvas/screen/location/notify)"
read_when:
...
steelengine nodes status --last-connected 24h
steelengine nodes list
...
steelengine nodes reject <requestId>
steelengine nodes remove --node <id|name|ip>
steelengine nodes rename --node <id|name|ip> --name <displayName>
```

These commands drive the gateway-owned `node.pair.*` store, separate from device pairing (`steelengine devices approve`) that gates the node's WS `connect` handshake. See [Nodes](/nodes) for how the two relate.
...
```bash
steelengine nodes invoke --node <id> --command system.which --params '{"bins":["uname"]}'
```
...
```bash
steelengine nodes notify --node <id> --title "Build" --body "Done" --priority timeSensitive
steelengine nodes push --node <id> --title "SteelEngine" --environment sandbox
steelengine nodes location get --node <id> --accuracy precise
steelengine nodes screen record --node <id> --duration 10s --fps 10 --out ./clip.mp4
```

- `notify` sends a local notification on a node that declares `system.notify`, including macOS, iOS, Android, and direct watchOS nodes. Direct watchOS delivery requires SteelEngine to be active. Requires `--title` or `--body`. Options: `--sound <name>`, `--priority <passive|active|timeSensitive>`, `--delivery <system|overlay|auto>` (default `system`), `--invoke-timeout <ms>` (default `15000`).
- `push` sends an APNs test push to an iOS node. Options: `--title <text>` (default `SteelEngine`), `--body <text>`, `--environment <sandbox|production>` to override the detected APNs environment.
...
Camera and Canvas commands have their own docs: [Camera nodes](/nodes/camera), [Canvas](/platforms/mac/canvas). Canvas is implemented by the bundled experimental Canvas plugin; core keeps `steelengine nodes canvas` as a compatibility mount point.
