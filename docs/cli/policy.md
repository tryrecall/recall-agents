---
summary: "CLI reference for `recall policy` conformance checks"
read_when:
  - You want to check Recall settings against an authored policy.jsonc
...
`recall policy` is provided by the bundled Policy plugin. Policy is an
enterprise conformance layer over existing Recall settings. It does not add a
second configuration system. `policy.jsonc` defines authored requirements,
Recall observes the active workspace as evidence, and policy health checks
...
Recall config secret provider/auth profile posture, and governed tool
...
posture, require Recall config SecretRefs to use managed providers, require
...
repeatable way to prove that Recall still conforms to that statement. Use
...
exposure, agent workspace posture, Recall config secret provider/auth profile
...
when a concrete rule is present. Recall reads current `channels.*` settings
`mcp.servers.*`, `models.providers.*`, selected agent model refs, network SSRF
settings, Gateway bind/auth/Control UI/Tailscale/remote/HTTP posture, Recall
...
`edit`, and `apply_patch`; Recall config `group:fs` covers file mutation tools
...
```bash
recall policy check
recall policy check --json
recall policy check --severity-min error
```

`policy check` runs only the policy check set and emits evidence, findings, and
attestation hashes. The same findings also appear in `recall doctor --lint`
...
```json
{
  ...
  "evidence": {
    ...
    "mcpServers": [
      {
        "id": "docs",
        "transport": "stdio",
        "source": "oc://recall.config/mcp/servers/docs",
        "command": "npx"
      }
    ],
    "modelProviders": [
      {
        "id": "openai",
        "source": "oc://recall.config/models/providers/openai"
      }
    ],
    "modelRefs": [
      {
        ...
        "provider": "openai",
        "model": "gpt-5.5",
        "source": "oc://recall.config/agents/defaults/model"
      }
    ],
    "network": [
      {
        "id": "browser-private-network",
        "source": "oc://recall.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
        "value": false
      }
    ],
    "gatewayExposure": [
    ...
    "agentWorkspace": [
      {
        ...
        "value": "ro",
        "sandboxMode": "all",
        "sandboxModeSource": "oc://recall.config/agents/defaults/sandbox/mode",
        "sandboxEnabled": true,
        "explicit": true
      },
      {
        ...
        "kind": "toolDeny",
        "source": "oc://recall.config/tools/deny",
        "scope": "defaults",
        "tool": "exec",
        "denied": true,
        "explicit": true
    ...
    "secrets": [
      {
        "id": "vault",
        "kind": "provider",
        "source": "oc://recall.config/secrets/providers/vault",
        "providerSource": "env"
      },
      {
        "id": "oc://recall.config/models/providers/openai/apiKey",
        "kind": "input",
    ...
    "authProfiles": [
      {
        "id": "github",
        "source": "oc://recall.config/auth/profiles/github",
        "validMetadata": true,
        "provider": "github",
        "mode": "token"
      }
...
records the observed Recall state used by the policy checks. The
...
2. Run `recall policy check --json`.
...
5. Re-run `recall doctor --lint` in CI or release gates.
...
`recall policy watch` runs the same check repeatedly and reports when the
current evidence no longer matches `expectedAttestationHash`:

```bash
recall policy watch --json
```
...
```json
{
  ...
  "target": "oc://recall.config/mcp/servers/remote",
...
```json
{
  ...
  "target": "oc://recall.config/agents/defaults/model/fallbacks/#0",
...
```json
{
  ...
  "target": "oc://recall.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
...
```json
{
  ...
  "target": "oc://recall.config/gateway/bind",
...
```json
{
  ...
  "target": "oc://recall.config/agents/defaults/sandbox/workspaceAccess",
...
In this version, repair can disable channels that are enabled in Recall config
