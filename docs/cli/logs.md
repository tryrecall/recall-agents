...
`recall logs` also accepts the standard Gateway client flags:
...
recall logs
recall logs --follow
recall logs --follow --interval 2000
recall logs --limit 500 --max-bytes 500000
recall logs --json
recall logs --plain
...
recall logs --url ws://127.0.0.1:18789 --token "$RECALL_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the implicit local loopback Gateway asks for pairing, closes during connect, or times out before `logs.tail` answers, `recall logs` falls back to the configured Gateway file log automatically. Explicit `--url` targets do not use this fallback.
