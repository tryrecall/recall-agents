---
summary: "CLI reference for `recall configure` (interactive configuration prompts)"
read_when:
...
Use `recall onboard` for the full guided first-run journey, `recall setup` for the baseline config/workspace only, and `recall channels add` when you only need channel account setup.
...
<Tip>
`recall config` without a subcommand opens the same wizard. Use `recall config get|set|unset` for non-interactive edits.
</Tip>

For web search, `recall configure --section web` lets you choose a provider
...
recall configure --section model --section channels
recall configure --section gateway --section daemon
```
