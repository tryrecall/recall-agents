---
summary: "CLI reference for `steelengine configure` (interactive configuration prompts)"
read_when:
...
Use `steelengine onboard` or `steelengine setup` for the full guided first-run journey, `steelengine setup --baseline` for the baseline config/workspace only, and `steelengine channels add` when you only need channel account setup.

<Tip>
`steelengine config` with no subcommand opens the same wizard. Use `steelengine config get|set|unset` for non-interactive edits.
</Tip>
...
steelengine configure --section model --section channels
steelengine configure --section gateway --section daemon
```
...
<Note>
`steelengine configure` requires an interactive terminal (both stdin and stdout must be TTYs). Without one it prints the equivalent non-interactive `steelengine config get|set|patch|validate` commands and exits with an error instead of partially running.
</Note>
...
Re-running provider auth from configure preserves an existing `agents.defaults.model.primary`, even when the provider's auth step returns a config patch with its own recommended default model. Adding or reauthing a provider makes its models available without taking over your current primary model. Use `steelengine models auth login --provider <id> --set-default` or `steelengine models set <model>` to intentionally change the default model.
</Note>
...
`steelengine configure --section web` picks a web-search provider and configures its credentials. Some providers show provider-specific follow-ups:
