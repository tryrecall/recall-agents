---
summary: "CLI reference for `recall path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
recall plugins enable oc-path
```

## Why use it

Recall state is spread across human-edited markdown, commented JSONC config,
...
`recall path` gives those callers a stable address instead of a one-off grep,
...
You probably do not need `recall path` for ordinary whole-file edits, rich
...
```bash
recall path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
recall path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
recall path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
recall path resolve 'oc://AGENTS.md/runtime-safety/recall-gateway'
```

Validate a path in CI or a preflight script before the script reads or writes:

```bash
recall path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
# Read a leaf
recall path resolve 'oc://gateway.jsonc/version'

# Wildcard search
recall path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
recall path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
recall path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
recall path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
recall path emit ./AGENTS.md
```
...
recall path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Predicate search over JSONC children
recall path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
recall path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
recall path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
recall path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
recall path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
recall path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
recall path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
recall path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
recall path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
recall path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
recall path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ recall path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ recall path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ recall path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ recall path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ recall path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ recall path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ recall path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: recall.invoke
```

```bash
$ recall path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ recall path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
recall path resolve 'oc://AGENTS.md/tools/gh/risk' --human
recall path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
recall path find 'oc://AGENTS.md/tools/**/risk'
recall path find 'oc://session.jsonl/[event=action]/userId'
recall path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
recall path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
recall path set 'oc://gateway.jsonc/version' '2.0'
recall path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ recall path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
recall path emit ./AGENTS.md
recall path emit ./gateway.jsonc --json
```
...
`recall path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__RECALL_REDACTED__` (verbatim or as a substring) is refused at write
  time.
