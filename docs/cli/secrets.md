---
summary: "CLI reference for `recall secrets` (reload, audit, configure, apply)"
read_when:
...
Use `recall secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
recall secrets reload
recall secrets reload --json
recall secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan Recall state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `recall.json` refs)
...
```bash
recall secrets audit
recall secrets audit --check
recall secrets audit --json
...
```bash
recall secrets configure
recall secrets configure --plan-out /tmp/recall-secrets-plan.json
recall secrets configure --apply --yes
recall secrets configure --providers-only
recall secrets configure --skip-provider-setup
recall secrets configure --agent ops
recall secrets configure --json
```
...
- `configure` targets secret-bearing fields in `recall.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Recall fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
recall secrets apply --from /tmp/recall-secrets-plan.json --json
```
...
- `recall.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.recall/.env` known secret keys whose values were migrated
