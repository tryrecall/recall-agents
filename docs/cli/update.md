---
summary: "CLI reference for `recall update` (safe-ish source update + gateway auto-restart)"
read_when:
  - You want to update a source checkout safely
  - You are debugging `recall update` output or options
...
Safely update Recall and switch between stable/beta/dev channels.
...
- `--tag <dist-tag|version|spec>`: override the package target for this update only. For package installs, `main` maps to `github:tryrecall/recall-agents#main`; GitHub/git source specs are packed into a temporary tarball before the staged global npm install.
...
`recall update` does not have a `--verbose` flag. Use `--dry-run` to preview
the planned channel/tag/install/restart actions, `--json` for machine-readable
results, and `recall update status --json` when you only need channel and
...
<Note>
In Nix mode (`RECALL_NIX_MODE=1`), mutating `recall update` runs are disabled. Update the Nix source or flake input for this install instead; for nix-recall, use the agent-first [Quick Start](https://github.com/recall/nix-recall#quick-start). `recall update status` and `recall update --dry-run` remain read-only.
</Note>
...
recall update status --timeout 10
```
...
When you switch channels explicitly (`--channel ...`), Recall also keeps the
install method aligned:

- `dev` → ensures a git checkout (default: `~/recall`, override with `RECALL_GIT_DIR`),
...
and the helper runs the normal `recall update --yes --json` CLI path from
...
For package-manager installs, `recall update` resolves the target package
version before invoking the package manager. npm global installs use a staged
install: Recall installs the new package into a temporary npm prefix, verifies
...
installed Recall build while leaving full plugin-command completion rebuilds to
explicit `recall completion --write-state` runs.
...
not supervising it, Recall re-bootstraps the LaunchAgent automatically, then
...
manual `recall gateway restart` hint. With `--no-restart`,
...
- `ok: true`, `result.status: "skipped"`,
  ...
  `recall update --yes --json` outside the live service process.
- `ok: false`, `result.reason: "managed-service-handoff-unavailable"`, and
  `handoff.status: "unavailable"` mean Recall could not find a supervising
...
result. `recall status` and `recall status --all` show an `Update restart`
...
<Steps>
  ...
  <Step title="Preflight build (dev only)">
    Runs the TypeScript build in a temp worktree. If the tip fails, walks back up to 10 commits to find the newest buildable commit. Set `RECALL_UPDATE_PREFLIGHT_LINT=1` to also run lint during this preflight; lint runs in constrained serial mode because user update hosts are often smaller than CI runners.
  </Step>
  <Step title="Rebase">
  ...
  <Step title="Run doctor">
    `recall doctor` runs as the final safe-update check.
  </Step>
...
beta release, Recall falls back to the recorded default/latest spec and reports
that as a warning. For npm plugins, Recall also falls back when the beta
...
<Warning>
If an exact pinned npm plugin update resolves to an artifact whose integrity differs from the stored install record, `recall update` aborts that plugin artifact update instead of installing it. Reinstall or update the plugin explicitly only after verifying that you trust the new artifact.
</Warning>

<Note>
Post-update plugin sync failures that are scoped to a managed plugin and that the sync path can route around (e.g. an unreachable npm registry for a non-essential plugin) are reported as warnings after the core update succeeds. The JSON result keeps the top-level update `status: "ok"` and reports `postUpdate.plugins.status: "warning"` with `recall doctor --fix` and `recall plugins inspect <id> --runtime --json` guidance. Unexpected updater or sync exceptions still fail the update result. Fix the plugin install or update error, then rerun `recall doctor --fix` or `recall update`.

After the per-plugin sync step, `recall update` runs a mandatory **post-core convergence** pass before the gateway is restarted: it repairs missing configured plugin payloads, validates each _active_ tracked install record on disk, and statically verifies its `package.json` is parseable (and any explicitly-declared `main` exists). Failures from this pass — and an invalid Recall config snapshot — return `postUpdate.plugins.status: "error"` and flip the top-level update `status` to `"error"`, so `recall update` exits non-zero and the gateway is _not_ restarted with an unverified plugin set. The error includes structured `postUpdate.plugins.warnings[].guidance` lines pointing at `recall doctor --fix` and `recall plugins inspect <id> --runtime --json` for follow-up. Disabled plugin entries and records that are not trusted-source-linked official sync targets are skipped here, mirroring the `skipDisabledPlugins` policy used by the missing-payload check, so a stale disabled plugin record cannot block an otherwise valid update.
...
`recall --update` rewrites to `recall update` (useful for shells and launcher scripts).

## Related

- `recall doctor` (offers to run update first on git checkouts)
