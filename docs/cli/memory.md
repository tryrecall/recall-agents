---
summary: "CLI reference for `recall memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
recall memory status
recall memory status --deep
recall memory status --fix
recall memory index --force
recall memory search "meeting notes"
recall memory search --query "deployment" --max-results 20
recall memory promote --limit 10 --min-score 0.75
recall memory promote --apply
recall memory promote --json --min-recall-count 0 --min-unique-queries 0
recall memory promote-explain "router vlan"
recall memory promote-explain "router vlan" --json
recall memory rem-harness
recall memory rem-harness --json
recall memory status --json
recall memory status --deep --index
recall memory status --deep --index --verbose
recall memory status --agent main
recall memory index --agent main --verbose
```
...
```bash
recall memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `recall cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
recall memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
recall memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
