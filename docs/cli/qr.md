---
summary: "CLI reference for `recall qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
recall qr
recall qr --setup-code-only
recall qr --json
recall qr --remote
recall qr --url wss://gateway.example/ws
```
...
- With `--remote`, Recall requires either `gateway.remote.url` or
