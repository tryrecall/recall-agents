---
summary: "CLI reference for `recall migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into Recall
...
```bash
recall migrate list
recall migrate claude --dry-run
recall migrate codex --dry-run
recall migrate codex --skill gog-vault77-google-workspace
recall migrate codex --plugin google-calendar --dry-run
recall migrate codex --plugin google-calendar --verify-plugin-apps --dry-run
recall migrate hermes --dry-run
recall migrate hermes
recall migrate apply codex --yes --skill gog-vault77-google-workspace
recall migrate apply codex --yes --plugin google-calendar
recall migrate apply codex --yes
recall migrate apply claude --yes
recall migrate apply hermes --yes
recall migrate apply hermes --include-secrets --yes
recall onboard --flow import
recall onboard --import-from claude --import-source ~/.claude
recall onboard --import-from hermes --import-source ~/.hermes
```

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `recall migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local Recall state exists.
</ParamField>
...
`recall migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `recall migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an Recall backup before applying the migration. If no local Recall state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the Recall agent workspace.
...
- Claude command Markdown files converted into Recall skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. Recall does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the Recall Codex harness and you want to
...
Running `recall migrate codex` in an interactive terminal previews the full
...
start checked unless the target Recall Codex plugin config already has that
...
```bash
recall migrate codex --dry-run --skill gog-vault77-google-workspace
recall migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
```bash
recall migrate codex --dry-run --plugin google-calendar
recall migrate apply codex --yes --plugin google-calendar
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  Recall agent workspace when you want per-agent ownership.
...
- `SOUL.md` and `AGENTS.md` into the Recall agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for Recall file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that Recall cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live Recall config or credentials. This preserves opaque or unsafe state without pretending Recall can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `recall.plugin.json`:
...
Provider plugins can use `recall/plugin-sdk/migration` for item construction and summary counts, plus `recall/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `recall onboard --flow import` and `recall setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh Recall setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move Recall to a new machine.
