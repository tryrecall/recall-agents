---
summary: "CLI reference for `steelengine node` (headless node host)"
read_when:
...
node connection and adds native Mac capabilities. Use `steelengine node run` on a
...
`steelengine node run` can publish plugin or MCP-backed tools after it connects.
...
`nodeHost.mcp.servers` in `steelengine.json` on the node machine, then restart the
...
- `--context-path <path>`: Gateway WebSocket context path (e.g. `/steelengine-gw`). Appended to the WebSocket URL.
...
`steelengine node run` and `steelengine node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `STEELENGINE_GATEWAY_TOKEN` / `STEELENGINE_GATEWAY_PASSWORD` are checked first.
...
trusted private-DNS names, set `STEELENGINE_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `steelengine.json` config
key.
`steelengine node install` persists it into the supervised node service when it is
...
```bash
steelengine node install --host <gateway-host> --port 18789
```
...
steelengine node stop
steelengine node restart
steelengine node uninstall
```

Use `steelengine node run` for a foreground node host (no service).
...
runs `steelengine node identity --json` on the node host over SSH and approves on
...
```bash
steelengine node identity --json
```
...
Run `steelengine devices list` again before approval.
...
SteelEngine state directory (`~/.steelengine` by default, or `$STEELENGINE_STATE_DIR`
when set):

| State                                        | Purpose                                                                                                                          |
| -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `state/steelengine.sqlite` (`node_host_config`) | Client instance ID, display name, and Gateway connection metadata. The client sends this ID as `instanceId`.                     |
...
`node.json` with `steelengine doctor --fix` likewise does not reset pairing. To
revoke and re-pair a node:

1. On the Gateway, run `steelengine nodes remove --node <id|name|ip>`.
2. On the node, restart the installed service with `steelengine node restart`, or
   stop and rerun the foreground `steelengine node run` command. This starts the
   device-pairing flow. If `steelengine devices list` does not show a request
...
3. On the Gateway, run `steelengine devices list`, then
...
5. On the Gateway, run `steelengine nodes pending`, then
...
Older SteelEngine releases stored node-host state in `node.json` and could leave an
obsolete `token` field there. Stop the node host and run `steelengine doctor --fix`
...
- `$STEELENGINE_STATE_DIR/exec-approvals.json`, or
  `~/.steelengine/exec-approvals.json` when the variable is unset
- [Exec approvals](/tools/exec-approvals)
- `steelengine approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, SteelEngine prepares a canonical `systemRunPlan`
