---
summary: "CLI reference for `recall onboard` (interactive onboarding)"
read_when:
...
Full guided onboarding for local or remote Gateway setup. Use this when you want Recall to walk through model auth, workspace, gateway, channels, skills, and health in one flow.
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How Recall onboarding fits together.
  </Card>
...
recall onboard --skip-bootstrap
recall onboard --mode remote --remote-url wss://gateway-host:18789
```

`--flow import` uses plugin-owned migration providers such as Hermes. It only runs against a fresh Recall setup; if existing config, credentials, sessions, or workspace memory/identity files are present, reset or choose a fresh setup before importing.

`--modern` starts the Crestodian conversational onboarding preview. Without
`--modern`, `recall onboard` keeps the classic onboarding flow.

On a fresh install where the active config file is missing or has no authored
settings (empty or metadata-only), bare `recall` also starts the classic
onboarding flow. Once a config file has authored settings, bare `recall`
...
`RECALL_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
```bash
RECALL_LOCALE=zh-CN recall onboard
```
...
Recall marks common vision model IDs as image-capable automatically. Pass `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `recall gateway run`.
...
<AccordionGroup>
  ...
  <Accordion title="Other behaviors">
    ...
    - Fastest first chat: `recall dashboard` (Control UI, no channel setup).
...
Use `recall setup` instead when you only need the baseline config/workspace. Use `recall configure` later for targeted changes and `recall channels add` for channel-only setup.
