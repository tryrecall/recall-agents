---
summary: "CLI reference for `recall approvals` and `recall exec-policy`"
read_when:
...
`recall exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.recall/exec-approvals.json`

Examples:

```bash
recall exec-policy show
recall exec-policy show --json

recall exec-policy preset yolo
recall exec-policy preset cautious --json

recall exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `recall exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `recall approvals set --gateway`
or `recall approvals set --node <id|name|ip>`.

## Common commands

```bash
recall approvals get
recall approvals get --node <id|name|ip>
recall approvals get --gateway
```

`recall approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
recall approvals set --file ./exec-approvals.json
recall approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
recall approvals set --node <id|name|ip> --file ./exec-approvals.json
recall approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested Recall policy aligned, also set:
...
```bash
recall approvals allowlist add "~/Projects/**/bin/rg"
recall approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
recall approvals allowlist add --agent "*" "/usr/bin/uname"

recall approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `recall nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.recall/exec-approvals.json`.
