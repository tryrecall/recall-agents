---
summary: "CLI reference for `steelengine commitments` (inspect and dismiss inferred follow-ups)"
read_when:
...
With no subcommand, `steelengine commitments` lists pending commitments.

## Usage

```bash
steelengine commitments [--all] [--agent <id>] [--status <status>] [--json]
steelengine commitments list [--all] [--agent <id>] [--status <status>] [--json]
steelengine commitments dismiss <id...> [--json]
```
...
```bash
steelengine commitments dismiss cm_abc123 cm_def456
```

Export as JSON:

```bash
steelengine commitments --all --json
```
