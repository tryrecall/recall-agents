---
summary: "CLI reference for `recall health` (gateway health snapshot via RPC)"
read_when:
...
- Default `recall health` asks the running gateway for its health snapshot. When the
