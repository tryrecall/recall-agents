---
summary: "CLI reference for `steelengine agents` (list/add/delete/bindings/bind/unbind/set identity)"
read_when:
...
Manage isolated agents (workspaces + auth + routing). Running `steelengine agents` with no subcommand is equivalent to `steelengine agents list`.
...
```bash
steelengine agents list
steelengine agents list --bindings
steelengine agents add work --workspace ~/.steelengine/workspace-work
steelengine agents add work --workspace ~/.steelengine/workspace-work --bind telegram:*
steelengine agents add ops --workspace ~/.steelengine/workspace-ops --bind telegram:ops --non-interactive
steelengine agents bindings
steelengine agents bind --agent work --bind telegram:ops
steelengine agents unbind --agent work --bind telegram:ops
steelengine agents set-identity --workspace ~/.steelengine/workspace --from-identity
steelengine agents set-identity --agent main --avatar avatars/steelengine.png
steelengine agents delete work
```
...
If you also want different visible skills per agent, configure `agents.defaults.skills` and `agents.list[].skills` in `steelengine.json`. See [Skills config](/tools/skills-config) and [Configuration reference](/gateway/config-agents#agentsdefaultsskills).

List bindings:

```bash
steelengine agents bindings
steelengine agents bindings --agent work
steelengine agents bindings --json
```

Add bindings:

```bash
steelengine agents bind --agent work --bind telegram:ops --bind discord:guild-a
```

You can also add bindings when creating an agent:

```bash
steelengine agents add work --workspace ~/.steelengine/workspace-work --bind telegram:* --bind discord:*
```

If you omit `accountId` (`--bind <channel>`), SteelEngine resolves it from plugin setup hooks, forced account binding, or the channel's configured account count.

If you omit `--agent` for `bind` or `unbind`, SteelEngine targets the current default agent.
...
- If the same agent already has a matching channel binding without `accountId`, and you later bind with an explicit or resolved `accountId`, SteelEngine upgrades that existing binding in place instead of adding a duplicate.

Examples:

```bash
# match all accounts on the channel
steelengine agents bind --agent work --bind telegram:*
...
steelengine agents bind --agent work --bind telegram:alerts
```
...
```bash
steelengine agents unbind --agent work --bind telegram:ops
steelengine agents unbind --agent work --all
```
...
- Example path: `~/.steelengine/workspace/IDENTITY.md`
...
```bash
steelengine agents set-identity --agent main --name "SteelEngine" --emoji "🦞" --avatar avatars/steelengine.png
```
