...
```bash
steelengine sandbox list
steelengine sandbox list --browser  # browser containers only
steelengine sandbox list --json
```

### `steelengine sandbox recreate`
...
steelengine sandbox recreate --agent mybot        # includes agent:mybot:* sub-sessions
steelengine sandbox recreate --session "agent:main:main"
steelengine sandbox recreate --browser --all      # only browser containers
...
```bash
steelengine sandbox explain
steelengine sandbox explain --session agent:main:main
steelengine sandbox explain --agent work
steelengine sandbox explain --json
```
...
Updating sandbox config does not affect running containers: existing runtimes keep their old settings, and idle runtimes are only pruned after `prune.idleHours` (default 24h). Regularly used agents can keep stale runtimes alive indefinitely. `steelengine sandbox recreate` removes the old runtime so the next use rebuilds it from current config.

<Tip>
Prefer `steelengine sandbox recreate` over manual backend-specific cleanup. It uses the Gateway's runtime registry and avoids mismatches when scope or session keys change.
</Tip>
...
| Change                                                                                                                                                         | Command                                                             |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Docker image update (`agents.defaults.sandbox.docker.image`)                                                                                                   | `steelengine sandbox recreate --all`                                   |
| Sandbox config (`agents.defaults.sandbox.*`)                                                                                                                   | `steelengine sandbox recreate --all`                                   |
| SSH target/auth (`agents.defaults.sandbox.ssh.{target,workspaceRoot,identityFile,certificateFile,knownHostsFile,identityData,certificateData,knownHostsData}`) | `steelengine sandbox recreate --all`                                   |
| OpenShell source/policy/mode (`plugins.entries.openshell.config.{from,mode,policy}`)                                                                           | `steelengine sandbox recreate --all`                                   |
| `setupCommand`                                                                                                                                                 | `steelengine sandbox recreate --all` (or `--agent <id>` for one agent) |
...
- `~/.steelengine/sandbox/containers.json`
- `~/.steelengine/sandbox/browsers.json`
- one JSON shard per container/browser under `~/.steelengine/sandbox/containers/` or `~/.steelengine/sandbox/browsers/`

Run `steelengine doctor --fix` to migrate valid legacy entries into SQLite. Invalid legacy files are quarantined so a corrupt old registry cannot hide current runtime entries.

## Configuration

Sandbox settings live in `~/.steelengine/steelengine.json` under `agents.defaults.sandbox` (per-agent overrides go in `agents.list[].sandbox`):
