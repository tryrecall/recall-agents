...
- `recall pairing approve --channel <channel> <code>`
- `recall pairing approve <code>` when exactly one pairing-capable channel is configured
...
- If `commands.ownerAllowFrom` is empty when you approve a pairing code, Recall also records the approved sender as the command owner, using a channel-scoped entry such as `telegram:123456789`.
...
- If you approved a sender before this bootstrap existed, run `recall doctor`; it warns when no command owner is configured and shows the `recall config set commands.ownerAllowFrom ...` command to fix it.
