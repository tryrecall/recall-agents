...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Recall Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`recall acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Recall channel
conversations instead of hosting an ACP harness session, use
[`recall mcp serve`](/cli/mcp) instead.
...
- Recall acts as an ACP server
- an IDE or ACP client connects to Recall
- Recall forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Recall runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Recall: use `recall acp`
- Recall should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include Recall parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                                |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Recall gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
recall acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
recall acp --url wss://gateway-host:18789 --token-file ~/.recall/gateway.token

# Attach to an existing session key
recall acp --session agent:main:main

# Attach by label (must already exist)
recall acp --session-label "support inbox"

# Reset the session key before the first prompt
recall acp --session agent:main:main --reset-session
```
...
recall acp client --server-args --url wss://gateway-host:18789 --token-file ~/.recall/gateway.token

# Override the server command (default: recall)
recall acp client --server "node" --server-args recall.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Recall through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`recall acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `recall gateway call sessions.list` as the only ACP proof. That
...
it to drive an Recall Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `recall acp` over stdio.

Example config (persisted):

```bash
recall config set gateway.remote.url wss://gateway-host:18789
recall config set gateway.remote.token <token>
```
...
```bash
recall acp --session agent:main:main
recall acp --session agent:design:main
recall acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Recall plugin tools or selected
...
[Recall tools MCP bridge](/tools/acp-agents-setup#recall-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
Recall bot over ACP, use `acpx` with its built-in `recall` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx recall` at `recall acp`.
3. Target the Recall session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Recall ACP session
acpx recall exec "Summarize the active Recall session state."

# Persistent named session for follow-up turns
acpx recall sessions ensure --name codex-bridge
acpx recall -s codex-bridge --cwd /path/to/repo \
  "Ask my Recall work agent for recent context relevant to this repo."
```

If you want `acpx recall` to target a specific Gateway and session key every
time, override the `recall` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "recall": {
      "command": "env RECALL_HIDE_BANNER=1 RECALL_SUPPRESS_NOTES=1 recall acp --url ws://127.0.0.1:18789 --token-file ~/.recall/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Recall checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env RECALL_HIDE_BANNER=1 RECALL_SUPPRESS_NOTES=1 node recall.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Recall agent without scraping a terminal.
...
In Zed, open the Agent panel and select "Recall ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`RECALL_GATEWAY_TOKEN`, `RECALL_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`RECALL_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `RECALL_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `recall acp client` sets `RECALL_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `recall`).
