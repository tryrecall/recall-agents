...
read_when:
  - You want Recall to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, Recall can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. Recall may check in afterward.
- You say you are exhausted. Recall may ask later whether you slept.
- The agent says it will follow up after something changes. Recall may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: Recall remembers a
...
```bash
recall config set commitments.enabled true
recall config set commitments.maxPerDay 3
```

Equivalent `recall.json`:
...
After an agent reply, Recall may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, Recall stores a commitment with:
...
without Recall tools.

Recall never delivers an inferred commitment immediately after writing it.
...
recall commitments dismiss cm_abc123
```

See [`recall commitments`](/cli/commitments) for the command reference.
...
Stored commitments are local Recall state. They are operational memory, not
long-term memory. Disable the feature with:

```bash
recall config set commitments.enabled false
```
...
- Check `recall commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`recall commitments`](/cli/commitments)
