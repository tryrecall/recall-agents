...
read_when:
  - You want SteelEngine to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, SteelEngine can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. SteelEngine may check in afterward.
- You say you are exhausted. SteelEngine may ask later whether you slept.
- The agent says it will follow up after something changes. SteelEngine may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: SteelEngine remembers a
...
```bash
steelengine config set commitments.enabled true
steelengine config set commitments.maxPerDay 3
```

Equivalent `steelengine.json`:
...
After an agent reply, SteelEngine may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, SteelEngine stores a commitment with:
...
metadata, and due-commitment heartbeat turns run without SteelEngine tools.

SteelEngine never delivers an inferred commitment immediately after writing it.
...
```bash
steelengine commitments
steelengine commitments --all
steelengine commitments --agent main
steelengine commitments --status snoozed
steelengine commitments dismiss cm_abc123
```

See [`steelengine commitments`](/cli/commitments) for the full command reference.
...
Stored commitments are local SteelEngine operational memory in the shared SQLite
state database, not long-term memory. Disable the feature with:

```bash
steelengine config set commitments.enabled false
```
...
- Check `steelengine commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`steelengine commitments`](/cli/commitments)
