...
Paste this into `recall.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, Recall can show:
...
case-insensitive. If `allowedChatIds` is non-empty and Recall cannot resolve a
...
`config.promptOverride` replaces the default Active Memory prompt. Recall
...
5. Verify memory search itself works with `recall memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, Recall auto-detects the first
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `recall memory status --deep` to inspect the memory-search backend
      and index health.
