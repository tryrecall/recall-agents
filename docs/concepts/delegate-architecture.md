---
summary: "Delegate architecture: running Recall as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run Recall as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Recall agent that:
...
Recall's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Recall's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.recall/workspace-delegate",
  tools: {
...
- Cron run history: `~/.recall/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.recall/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Recall's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
recall agents add delegate
```

This creates:

- Workspace: `~/.recall/workspace-delegate`
- State: `~/.recall/agents/delegate/agent`
- Sessions: `~/.recall/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.recall/workspace" },
      {
        id: "delegate",
        workspace: "~/.recall/workspace-delegate",
        tools: {
...
~/.recall/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.recall/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.recall/workspace-org",
        agentDir: "~/.recall/agents/org-assistant/agent",
...
recall view. Recall redacts credential/token-like text, truncates long
