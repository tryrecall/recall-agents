...
The private QA stack exercises SteelEngine in a realistic, channel-shaped way that
a unit test cannot.
...
Every QA flow runs under `pnpm steelengine qa <subcommand>`. Many have `pnpm qa:*`
...
| `qa parity-report`                                  | Compare two `qa-suite-summary.json` files for a model-axis parity gate, or use `--runtime-axis --token-efficiency` to write Codex-vs-SteelEngine runtime parity and token-efficiency reports.                                                                          |
...
command also needs an SteelEngine root profile, put the root profile before the
QA command:

```bash
pnpm steelengine --profile work qa run --qa-profile smoke-ci
```
...
```bash
pnpm steelengine qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`steelengine.run`, `steelengine.harness.run`, a latest GenAI semantic-convention
model-call span, `steelengine.context.assembled`, and `steelengine.message.delivery`
...
IDs and `steelengine.content.*` attributes must stay out of the trace. The scenario
...
```bash
pnpm steelengine qa matrix --provider-mode mock-openai --profile release
```
...
```bash
STEELENGINE_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
  pnpm steelengine qa matrix --provider-mode live-frontier --profile release
```

Plain `pnpm steelengine qa matrix` runs the full `all` profile and continues after
...
| `e2ee-cli`   | 9         | `steelengine matrix encryption setup`, recovery-key, multi-account, gateway round-trip, and self-verification commands through the harness. |
...
`STEELENGINE_QA_MATRIX_TUWUNEL_IMAGE`; tune negative no-reply assertions with
`STEELENGINE_QA_MATRIX_NO_REPLY_WINDOW_MS` (default `8000`, clamped to the active
...
`STEELENGINE_QA_MATRIX_DISABLE_FORCE_EXIT=1` only for a direct test harness that
...
E2EE CLI profile also drives `steelengine matrix encryption setup` and
...
```bash
pnpm steelengine qa discord
pnpm steelengine qa slack
pnpm steelengine qa telegram
pnpm steelengine qa whatsapp
```
...
SteelEngine Slack gateway running inside the VM on port `38973`; without it, the
...
```bash
pnpm steelengine qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `steelengine infer image
...
The doctor checks Convex broker env (`STEELENGINE_QA_CONVEX_SITE_URL`,
`STEELENGINE_QA_CONVEX_ENDPOINT_PREFIX`), validates endpoint settings, reports
only set/missing status for `STEELENGINE_QA_CONVEX_SECRET_CI` and
`STEELENGINE_QA_CONVEX_SECRET_MAINTAINER`, and verifies admin/list reachability
...
```bash
pnpm steelengine qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds SteelEngine
...
- `STEELENGINE_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
...
of the `/usage` footer on tool-only replies. Use `pnpm steelengine qa telegram
...
```bash
STEELENGINE_QA_CREDENTIAL_SOURCE=convex \
pnpm test:docker:npm-telegram-live
```

When `STEELENGINE_QA_CREDENTIAL_SOURCE=convex` is set, the package live wrapper
...
`STEELENGINE_NPM_TELEGRAM_RTT_SAMPLES`, `STEELENGINE_NPM_TELEGRAM_RTT_TIMEOUT_MS`,
or `STEELENGINE_NPM_TELEGRAM_RTT_MAX_FAILURES` to tune RTT measurement without
...
controlled by the harness and a SUT bot started by the child SteelEngine gateway
...
- `STEELENGINE_QA_DISCORD_GUILD_ID`
- `STEELENGINE_QA_DISCORD_CHANNEL_ID`
- `STEELENGINE_QA_DISCORD_DRIVER_BOT_TOKEN`
- `STEELENGINE_QA_DISCORD_SUT_BOT_TOKEN`
- `STEELENGINE_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id
  returned by Discord (the lane fails fast otherwise).

Optional:

- `STEELENGINE_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for
...
- `STEELENGINE_QA_SLACK_APPROVAL_CHECKPOINT_DIR` enables visual approval
...
- `STEELENGINE_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS` overrides the checkpoint
...
- `slack-codex-approval-exec-native` - opt-in Codex Guardian command approval
  ...
  `steelengine-codex-app-server`, resolves it, and verifies the Codex turn
...
- `approval-checkpoints/` - only when Mantis sets
  `STEELENGINE_QA_SLACK_APPROVAL_CHECKPOINT_DIR`; contains checkpoint JSON,
...
{
  "display_information": {
    "name": "SteelEngine QA Driver",
    "description": "Test driver bot for SteelEngine QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "SteelEngine QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
{
  "display_information": {
    "name": "SteelEngine QA SUT",
    "description": "SteelEngine QA SUT connector for SteelEngine"
  },
  "features": {
    "bot_user": {
      "display_name": "SteelEngine QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#steelengine-qa`) and invite both
bots from inside the channel:

```text
/invite @SteelEngine QA Driver
/invite @SteelEngine QA SUT
```
...
`STEELENGINE_QA_SLACK_*` variables and pass `--credential-source env`), or seed
...
With `STEELENGINE_QA_CONVEX_SITE_URL` and `STEELENGINE_QA_CONVEX_SECRET_MAINTAINER`
exported in your shell, register and verify:

```bash
pnpm steelengine qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm steelengine qa credentials list --kind slack --status all --json
```
...
the harness and a SUT account started by the child SteelEngine gateway through
the bundled WhatsApp plugin.

Required env when `--credential-source env`:

- `STEELENGINE_QA_WHATSAPP_DRIVER_PHONE_E164`
- `STEELENGINE_QA_WHATSAPP_SUT_PHONE_E164`
- `STEELENGINE_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64`
- `STEELENGINE_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64`

Optional:

- `STEELENGINE_QA_WHATSAPP_GROUP_JID` enables group scenarios such as
...
`@steelengine/whatsapp/api.js` package surface instead of reaching into private
...
`--credential-source convex` (or set `STEELENGINE_QA_CREDENTIAL_SOURCE=convex`);
...
- `mock-openai` is the scenario-aware SteelEngine mock. It remains the default
...
runs SteelEngine's normal channel plugins against them. `live` is reserved for
...
- the `steelengine qa` command root
...
- how `steelengine qa <runner>` is mounted beneath the shared `qa` root
...
3. Keep transport-specific mechanics inside the runner plugin or channel
   harness.
4. Mount the runner as `steelengine qa <runner>` instead of registering a
   competing root command. Runner plugins should declare `qaRunners` in
   `steelengine.plugin.json` and export a matching `qaRunnerCliRegistrations`
...
or wiring a new transport - run `pnpm steelengine qa coverage` (add `--json`
for machine-readable output). When choosing focused proof for a touched
behavior or file path, run `pnpm steelengine qa coverage --match <query>`. The
...
```bash
pnpm steelengine qa character-eval \
