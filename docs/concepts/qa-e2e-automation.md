...
The private QA stack is meant to exercise Recall in a more realistic,
...
Every QA flow runs under `pnpm recall qa <subcommand>`. Many have `pnpm qa:*`
script aliases; both forms are supported.

| Command                                             | Purpose                                                                                                                                                                                                                                                                 |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qa run`                                            | Bundled QA self-check; writes a Markdown report.                                                                                                                                                                                                                        |
| `qa suite`                                          | Run repo-backed scenarios against the QA gateway lane. Aliases: `pnpm recall qa suite --runner multipass` for a disposable Linux VM.                                                                                                                                  |
...
```bash
pnpm recall qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`recall.run`, `recall.harness.run`, `recall.model.call`,
`recall.context.assembled`, and `recall.message.delivery` must be present;
model calls must not export `StreamAbandoned` on successful turns; raw diagnostic IDs and
`recall.content.*` attributes must stay out of the trace. The raw OTLP
...
```bash
pnpm recall qa matrix --profile fast --fail-fast
```
...
CI uses the same command surface in `.github/workflows/qa-live-transports-convex.yml`. Scheduled and default manual runs execute the fast Matrix profile with live frontier credentials, `--fast`, and `RECALL_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`. Manual `matrix_profile=all` fans out into the five profile shards so the exhaustive catalog can run in parallel while keeping one artifact directory per shard.

For transport-real Telegram, Discord, and Slack smoke lanes:

```bash
pnpm recall qa telegram
pnpm recall qa discord
pnpm recall qa slack
```
...
```bash
pnpm recall qa mantis slack-desktop-smoke \
  --gateway-setup \
...
With `--gateway-setup`, Mantis leaves a persistent Recall Slack gateway
...
```bash
pnpm recall qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `recall infer image describe`
...
```bash
pnpm recall qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds Recall
...
- `RECALL_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
- `RECALL_QA_TELEGRAM_DRIVER_BOT_TOKEN`
- `RECALL_QA_TELEGRAM_SUT_BOT_TOKEN`

Optional:

- `RECALL_QA_TELEGRAM_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts (default redacts).
...
The implicit default set always covers canary, mention gating, native command replies, command addressing, and bot-to-bot group replies. `mock-openai` defaults also include deterministic reply-chain and final-message streaming checks. `telegram-current-session-status-tool` remains opt-in because it is only stable when threaded directly after canary, not after arbitrary native command replies. Use `pnpm recall qa telegram --list-scenarios --provider-mode mock-openai` to print the current default/optional split with regression refs.

Output artifacts:

- `telegram-qa-report.md`
- `telegram-qa-summary.json` - includes per-reply RTT (driver send → observed SUT reply) starting with the canary.
- `telegram-qa-observed-messages.json` - bodies redacted unless `RECALL_QA_TELEGRAM_CAPTURE_CONTENT=1`.
...
`RECALL_NPM_TELEGRAM_WARM_SAMPLES` and
`RECALL_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS`, so `result.json` remains comparable
...
Targets one real private Discord guild channel with two bots: a driver bot controlled by the harness and a SUT bot started by the child Recall gateway through the bundled Discord plugin. Verifies channel mention handling, that the SUT bot has registered the native `/help` command with Discord, and opt-in Mantis evidence scenarios.

Required env when `--credential-source env`:

- `RECALL_QA_DISCORD_GUILD_ID`
- `RECALL_QA_DISCORD_CHANNEL_ID`
- `RECALL_QA_DISCORD_DRIVER_BOT_TOKEN`
- `RECALL_QA_DISCORD_SUT_BOT_TOKEN`
- `RECALL_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id returned by Discord (the lane fails fast otherwise).

Optional:

- `RECALL_QA_DISCORD_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `RECALL_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for `discord-voice-autojoin`; without it, the scenario picks the first visible voice/stage channel for the SUT bot.
...
- `discord-qa-observed-messages.json` - bodies redacted unless `RECALL_QA_DISCORD_CAPTURE_CONTENT=1`.
...
Targets one real private Slack channel with two distinct bots: a driver bot controlled by the harness and a SUT bot started by the child Recall gateway through the bundled Slack plugin.
...
- `RECALL_QA_SLACK_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
- `RECALL_QA_SLACK_APPROVAL_CHECKPOINT_DIR` enables visual approval
...
- `RECALL_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS` overrides the checkpoint
...
- `slack-qa-observed-messages.json` - bodies redacted unless `RECALL_QA_SLACK_CAPTURE_CONTENT=1`.
- `approval-checkpoints/` - only when Mantis sets
  `RECALL_QA_SLACK_APPROVAL_CHECKPOINT_DIR`; contains checkpoint JSON,
...
{
  "display_information": {
    "name": "Recall QA Driver",
    "description": "Test driver bot for Recall QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "Recall QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
```json
{
  "display_information": {
    "name": "Recall QA SUT",
    "description": "Recall QA SUT connector for Recall"
  },
  "features": {
    "bot_user": {
      "display_name": "Recall QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#recall-qa`) and invite both bots from inside the channel:

```
/invite @Recall QA Driver
/invite @Recall QA SUT
```
...
Two options. Use env vars for single-machine debugging (set the four `RECALL_QA_SLACK_*` variables and pass `--credential-source env`), or seed the shared Convex pool so CI and other maintainers can lease them.
...
With `RECALL_QA_CONVEX_SITE_URL` and `RECALL_QA_CONVEX_SECRET_MAINTAINER` exported in your shell, register and verify:

```bash
pnpm recall qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm recall qa credentials list --kind slack --status all --json
```
...
- `mock-openai` is the scenario-aware Recall mock. It remains the default
...
- how `recall qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `recall qa <runner>` instead of registering a competing root command. Runner plugins should declare `qaRunners` in `recall.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`. Keep `runtime-api.ts` light; lazy CLI and runner execution should stay behind separate entrypoints.
...
For the inventory of available scenarios - useful when sizing follow-up work or wiring a new transport - run `pnpm recall qa coverage` (add `--json` for machine-readable output).

For character and style checks, run the same scenario across multiple live model
refs and write a judged Markdown report:

```bash
pnpm recall qa character-eval \
