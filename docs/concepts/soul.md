---
summary: "Use SOUL.md to give your Recall agent an actual voice instead of generic assistant sludge"
read_when:
...
Recall injects it on normal sessions, so it has real weight. If your agent
...
For Recall, `SOUL.md` is that layer.
...
Path fixed for Recall workspaces: use `SOUL.md`, not `http://SOUL.md`.
...
<CardGroup cols={2}>
  <Card title="Agent workspace" href="/concepts/agent-workspace" icon="folder-open">
    Workspace files Recall injects into model context.
  </Card>
  <Card title="System prompt" href="/concepts/system-prompt" icon="message-lines">
    How `SOUL.md` is composed into Recall and Codex runtime context.
  </Card>
