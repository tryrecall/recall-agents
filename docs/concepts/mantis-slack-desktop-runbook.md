...
Linux desktop, VNC rescue, Slack Web, a real Recall gateway, screenshots,
...
- Mantis artifacts: owned by the Recall run. They live under
...
- remote logs such as `slack-desktop-command.log`, `recall-gateway.log`,
...
- `crabbox.remote_run`: sync, browser launch, Recall install/build or
...
remote status after Mantis has copied metadata proving that either the Recall
