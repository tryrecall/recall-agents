...
- Channel overrides: `*.streaming.block.enabled` and `*.streaming.block.coalesce` on bundled channels; stale flat keys are migrated by `steelengine doctor --fix`. Block streaming is off unless explicitly enabled, on every channel including Telegram. QQ Bot is the exception: it has no `streaming.block` keys and streams block replies unless `channels.qqbot.streaming.mode` is `"off"`.
...
The silent token `NO_REPLY` (case-insensitive, so `no_reply` also matches) means "do not deliver a user-visible reply." When a turn also has pending tool media, such as generated TTS audio, SteelEngine strips the silent text but still delivers the media attachment.

Silence policy resolves by conversation type:

- Direct conversations never receive `NO_REPLY` prompt guidance. If a direct run accidentally returns a bare silent token, SteelEngine suppresses it instead of rewriting or delivering it.
