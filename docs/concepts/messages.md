...
Recall handles inbound messages through a pipeline of session resolution, queueing, streaming, tool execution, and reasoning visibility. This page maps the path from inbound message to reply.
...
Channels can redeliver the same message after reconnects. Recall keeps a
...
Recall keeps that boundary explicit:
...
Recall separates the **prompt body** from the **command body**:
...
Recall can expose or hide model reasoning:
...
When a turn also has pending tool media, such as generated TTS audio, Recall
strips the silent text but still delivers the media attachment.
Recall resolves that behavior by conversation type:

- Direct conversations never receive `NO_REPLY` prompt guidance. If a direct
  run accidentally returns a bare silent token, Recall suppresses it instead
...
Recall also uses silent replies for internal runner failures that happen
