...
- **Session store**: chat history and routing state in `~/.steelengine/agents/<agentId>/agent/steelengine-agent.sqlite`.
...
<Warning>
Never reuse `agentDir` across agents — it causes auth/session state collisions. When a secondary agent's local OAuth credential is expired or its refresh fails, SteelEngine reads through to the default/main agent's credential for the same profile id and adopts whichever token is freshest, without copying the refresh token into the secondary agent's store. If you want a fully independent OAuth account, sign in from that agent. If you copy credentials manually, copy only portable static `api_key` or `token` profiles — OAuth refresh material is not portable by default (`copyToAgents` can opt a profile in explicitly).
</Warning>

Skills load from each agent workspace plus shared roots such as `~/.steelengine/skills`, then filter by the effective agent skill allowlist. Use `agents.defaults.skills` for a shared baseline and `agents.list[].skills` for a per-agent replacement (explicit entries replace the default, they do not merge). See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) and [Skills: agent allowlists](/tools/skills#agent-allowlists).
...
| -------------------------------- | -------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| Config                           | `~/.steelengine/steelengine.json`                                                            | `STEELENGINE_CONFIG_PATH`                                                                   |
| State dir                        | `~/.steelengine`                                                                          | `STEELENGINE_STATE_DIR`                                                                     |
| Default agent's workspace        | `~/.steelengine/workspace` (or `workspace-<profile>` when `STEELENGINE_PROFILE` is set)      | `agents.list[].workspace`, then `agents.defaults.workspace`, or `STEELENGINE_WORKSPACE_DIR` |
| Other agents' workspace          | `<stateDir>/workspace-<agentId>` (or `<agents.defaults.workspace>/<agentId>` when set) | `agents.list[].workspace`                                                                |
| Agent dir                        | `~/.steelengine/agents/<agentId>/agent`                                                   | `agents.list[].agentDir`                                                                 |
...
If you configure nothing, SteelEngine runs one agent:

- `agentId` defaults to `main`.
- Sessions key as `agent:main:<mainKey>` (default `mainKey` is `main`).
- Workspace defaults to `~/.steelengine/workspace` (or `workspace-<profile>` when `STEELENGINE_PROFILE` is set to something other than `default`).
- State defaults to `~/.steelengine/agents/main/agent`.
...
```bash
steelengine agents list --bindings
```
...
<Steps>
  <Step title="Create each agent workspace">
    ```bash
    steelengine agents add coding
    steelengine agents add social
    ```

    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.steelengine/agents/<agentId>`.
...
The configured path is the parent directory. SteelEngine appends the normalized
agent id, producing paths such as `~/.steelengine/wiki/support` and
`~/.steelengine/wiki/marketing`. Agent-scoped CLI and Gateway operations require
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.steelengine/workspace-alex" },
      { id: "mia", workspace: "~/.steelengine/workspace-mia" },
    ],
  },
  bindings: [
    {
...
Set `channels.<channel>.defaultAccount` to choose the account used when `accountId` is omitted. When unset, SteelEngine falls back to `default` if present, otherwise the first configured account id (sorted).
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.steelengine/workspace-main" },
          { id: "coding", workspace: "~/.steelengine/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.steelengine/workspace-main" },
          { id: "alerts", workspace: "~/.steelengine/workspace-alerts" },
        ],
      },
      bindings: [
  ...
  <Accordion title="WhatsApp numbers per agent">
    ...
    ```bash
    steelengine channels login --channel whatsapp --account personal
    steelengine channels login --channel whatsapp --account biz
    ```

    `~/.steelengine/steelengine.json` (JSON5):
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.steelengine/workspace-home",
            agentDir: "~/.steelengine/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.steelengine/workspace-work",
            agentDir: "~/.steelengine/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.steelengine/credentials/whatsapp/personal
              // authDir: "~/.steelengine/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.steelengine/credentials/whatsapp/biz
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.steelengine/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.steelengine/workspace-opus",
