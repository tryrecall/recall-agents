...
An **agent** here is the full per-persona scope: workspace files, auth profiles, model registry, and session store. `agentDir` is the on-disk state directory that holds this per-agent config at `~/.recall/agents/<agentId>/`. A **binding** maps a channel account (e.g. a Slack workspace or a WhatsApp number) to one of those agents.
...
- **Session store** (chat history + routing state) under `~/.recall/agents/<agentId>/sessions`.
...
a local profile, but Recall does not clone OAuth refresh tokens into the
...
Skills are loaded from each agent workspace plus shared roots such as `~/.recall/skills`, then filtered by the effective agent skill allowlist when configured. Use `agents.defaults.skills` for a shared baseline and `agents.list[].skills` for per-agent replacement. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) and [Skills: agent skill allowlists](/tools/skills#agent-skill-allowlists).
...
- State dir: `~/.recall` (or `RECALL_STATE_DIR`)
- Workspace: `~/.recall/workspace` (or `~/.recall/workspace-<agentId>`)
- Agent dir: `~/.recall/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.recall/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Recall runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.recall/workspace` (or `~/.recall/workspace-<profile>` when `RECALL_PROFILE` is set).
- State defaults to `~/.recall/agents/main/agent`.
...
<Steps>
  <Step title="Create each agent workspace">
    ...
    ```bash
    recall agents add coding
    recall agents add social
    ```

    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.recall/agents/<agentId>`.
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.recall/workspace-alex" },
      { id: "mia", workspace: "~/.recall/workspace-mia" },
    ],
  },
  bindings: [
    {
...
<AccordionGroup>
  ...
  <Accordion title="Account-scope detail">
    ...
    - If you later add the same binding for the same agent with an explicit account id, Recall upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
If you want a channel-wide default account when `accountId` is omitted, set `channels.<channel>.defaultAccount` (optional). When unset, Recall falls back to `default` if present, otherwise the first configured account id (sorted).
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.recall/workspace-main" },
          { id: "coding", workspace: "~/.recall/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.recall/workspace-main" },
          { id: "alerts", workspace: "~/.recall/workspace-alerts" },
        ],
      },
      bindings: [
  ...
  <Accordion title="WhatsApp numbers per agent">
    ...
    ```bash
    recall channels login --channel whatsapp --account personal
    recall channels login --channel whatsapp --account biz
    ```

    `~/.tryrecall/recall-agents.json` (JSON5):
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.recall/workspace-home",
            agentDir: "~/.recall/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.recall/workspace-work",
            agentDir: "~/.recall/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.recall/credentials/whatsapp/personal
              // authDir: "~/.recall/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.recall/credentials/whatsapp/biz
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.recall/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.recall/workspace-opus",
