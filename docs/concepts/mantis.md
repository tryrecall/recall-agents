---
summary: "Mantis is the visual end-to-end verification system for reproducing Recall bugs on live transports, capturing before and after evidence, and attaching artifacts to PRs."
title: "Mantis"
read_when:
  - Building or running live visual QA for Recall bugs
...
Mantis is the Recall end-to-end verification system for bugs that need a real
...
Mantis lives in the Recall QA stack.

- Recall owns the scenario runtime, transport adapters, evidence schema, and
  local CLI under `pnpm recall qa mantis`.
...
- Recall agents drive Mantis through Codex when a scenario needs agentic setup,
  debugging, or stuck-state reporting.

This boundary keeps transport knowledge in Recall, machine scheduling in
...
```bash
pnpm recall qa mantis discord-smoke \
...
thread, calls Recall's `message.thread-reply` action with a repo-local
...
```bash
pnpm recall qa mantis desktop-browser-smoke \
...
`RECALL_MANTIS_CRABBOX_PROVIDER` when running against another Crabbox fleet.

Useful desktop smoke flags:

- `--lease-id <cbx_...>` or `RECALL_MANTIS_CRABBOX_LEASE_ID` reuses a warmed desktop.
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome user-data-dir archive from the named environment variable before launching the browser. Use this for logged-in witnesses such as Discord Web. The default env var is `RECALL_MANTIS_BROWSER_PROFILE_TGZ_B64`.
...
- `--keep-lease` or `RECALL_MANTIS_KEEP_VM=1` keeps a newly created passing lease open for VNC inspection. Failed runs keep the lease by default when one was created so an operator can reconnect.
...
REST. When `RECALL_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also
writes a Discord Web URL artifact. When `RECALL_QA_DISCORD_KEEP_THREADS=1` is
...
the VM, runs `pnpm recall qa slack` inside that VM, opens Slack Web in the VNC
...
shape where the SUT Recall gateway and the browser both live inside the same
Linux desktop VM.

With `--gateway-setup`, the command prepares a persistent disposable Recall
home at `$HOME/.recall-mantis/slack-recall`, patches Slack Socket Mode
configuration for the selected channel, starts `recall gateway run` on port
...
- `RECALL_LIVE_OPENAI_KEY` for the remote model lane. If only
  `OPENAI_API_KEY` is set locally, Mantis maps it to `RECALL_LIVE_OPENAI_KEY`
  before invoking Crabbox so Crabbox's `RECALL_*` env forwarding can carry it
  into the VM.
...
channel id, Socket Mode app token, and bot token as the `RECALL_MANTIS_SLACK_*`
...
- `--gateway-setup` starts a persistent Recall Slack gateway in the VM instead of only running the bot-to-bot QA lane.
...
- `RECALL_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome profile inside the VM. The default is `$HOME/.config/recall-mantis/slack-chrome-profile`, so a manual Slack Web login survives reruns on the same lease.
...
Crabbox CLI from
`recall/crabbox` main so it can use the current desktop/browser lease flags
...
runs `pnpm recall qa mantis slack-desktop-smoke --gateway-setup` against that
...
worktree, runs `pnpm recall qa telegram --credential-source convex
...
`@recall-mantis telegram desktop proof`, from the Actions UI with freeform
...
Recall with the leased Telegram SUT bot token, starts `recall gateway run`
...
token never logs Telegram Desktop in; it only configures Recall. The desktop
...
- `--no-gateway-setup` installs and opens Telegram Desktop without configuring Recall.
...
comment or creates one. The workflows publish to `recall-crabbox-artifacts`
with public URLs under `https://artifacts.recall.ai`. They provide bucket,
...
```text
@recall-mantis discord status reactions
```
...
```text
@recall-mantis discord status reactions baseline=origin/main candidate=HEAD
```
...
6. Start a child Recall Gateway with an isolated state directory.
...
- It exercises a real Recall Gateway, Discord bot auth, message dispatch,
...
- `pnpm recall qa discord` already runs a live Discord lane with driver and
  SUT bots.
...
- **Headless automation**: default for CI. Chrome runs with CDP enabled, and
  Playwright or Recall browser control captures screenshots.
...
- Node 22 and pnpm
- Recall checkout and dependency cache
- Playwright Chromium browser cache when Playwright is used
- enough CPU and memory for one Recall Gateway, one browser, and one model run
...
- `RECALL_QA_DISCORD_MANTIS_BOT_TOKEN`
- `RECALL_QA_DISCORD_DRIVER_BOT_TOKEN`
- `RECALL_QA_DISCORD_SUT_BOT_TOKEN`
- `RECALL_QA_DISCORD_GUILD_ID`
- `RECALL_QA_DISCORD_CHANNEL_ID`
- `RECALL_QA_DISCORD_NOTIFY_CHANNEL_ID`
- `RECALL_QA_REDACT_PUBLIC_METADATA=1` for public GitHub artifact uploads
- `RECALL_QA_CONVEX_SITE_URL`
- `RECALL_QA_CONVEX_SECRET_CI`
- `RECALL_QA_MANTIS_CRABBOX_COORDINATOR`
- `RECALL_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN`
...
`RECALL_QA_REDACT_PUBLIC_METADATA=1` for this reason.
