---
summary: "SteelEngine capabilities across channels, routing, media, and UX."
read_when:
  - You want a full list of what SteelEngine supports
title: "Features"
---
...
- iMessage, Telegram, and WebChat ship with the core install; every other channel is an
  official plugin installed with `steelengine plugins install @steelengine/<id>` (or on demand
  during `steelengine onboard` / `steelengine channels add`)
...
- External plugin channels maintained outside the SteelEngine repo: WeChat, Yuanbao, and Zalo ClawBot
...
<CardGroup cols={2}>
  ...
  <Card title="Plugins" href="/tools/plugin" icon="plug">
    Official and external plugins that extend SteelEngine.
  </Card>
</CardGroup>
