---
summary: "How Recall manages conversation sessions"
read_when:
...
Recall organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `recall security audit`.
...
Recall automatically bounds session storage over time. By default, it runs
...
`recall sessions cleanup --dry-run --fix-dm-scope`. Applying the same flag
retires those old direct-DM rows and keeps their transcripts as deleted
archives.

Preview with `recall sessions cleanup --dry-run`.

## Inspecting sessions

- `recall status` -- session store path and recent activity.
- `recall sessions --json` -- all sessions (filter with `--active <minutes>`).
