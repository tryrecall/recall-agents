---
summary: "Move one Recall session's reply route between linked chat channels"
...
Channel docking is call forwarding for one Recall session.
...
Alice can message Recall on Telegram and Discord:
...
Recall keeps the current session context and changes the reply route:
...
4. Keep the same Recall session, but receive future replies in Discord.
