...
- Provider plugins can inject model catalogs via `registerProvider({ catalog })`;
  Recall merges that output into `models.providers` before writing
  `models.json`.
...
Provider plugins can now own most provider-specific logic while Recall keeps
the generic inference loop.

Typical split:

- `auth[].run` / `auth[].runNonInteractive`: provider owns onboarding/login
  flows for `recall onboard`, `recall models auth`, and headless setup
...
That covers providers that still fit Recall's normal transports. A provider
...
Recall ships with the pi‑ai catalog. These providers require **no**
...
- `openai/gpt-5.3-codex-spark` is intentionally suppressed in Recall because the live OpenAI API rejects it; Spark is treated as Codex-only
...
- CLI: `recall onboard --auth-choice token` (paste setup-token) or `recall models auth paste-token --provider anthropic`
- Direct API-key models support the shared `/fast` toggle and `params.fastMode`; Recall maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
...
- Caution: Gemini CLI OAuth in Recall is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
- Gemini CLI OAuth is shipped as part of the bundled `google` plugin.
  - Enable: `recall plugins enable google`
  - Login: `recall models auth login --provider google-gemini-cli --set-default`
  - Note: you do **not** paste a client id or secret into `recall.json`. The CLI login flow stores
...
- CLI: `recall onboard --kilocode-api-key <key>`
...
- CLI: `recall onboard --auth-choice mistral-api-key`
- Groq: `groq` (`GROQ_API_KEY`)
- Cerebras: `cerebras` (`CEREBRAS_API_KEY`)
  - GLM models on Cerebras use ids `zai-glm-4.7` and `zai-glm-4.6`.
  - OpenAI-compatible base URL: `https://api.cerebras.ai/v1`.
- GitHub Copilot: `github-copilot` (`COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN`)
- Hugging Face Inference example model: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `recall onboard --auth-choice huggingface-api-key`. See [Hugging Face (Inference)](/providers/huggingface).
...
```bash
recall models auth login --provider qwen-portal --set-default
```
...
`recall onboard` and the model picker. See [/providers/ollama](/providers/ollama)
...
- For custom providers, `reasoning`, `input`, `cost`, `contextWindow`, and `maxTokens` are optional.
  When omitted, Recall defaults to:
  - `reasoning: false`
  - `input: ["text"]`
  - `cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }`
  - `contextWindow: 200000`
