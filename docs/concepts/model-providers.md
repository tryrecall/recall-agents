...
<AccordionGroup>
  <Accordion title="Model refs and CLI helpers">
    ...
    - CLI helpers: `steelengine onboard`, `steelengine models list`, `steelengine models set <provider/model>`.
  ...
  <Accordion title="Adding provider auth does not change your primary model">
    `steelengine configure` preserves an existing `agents.defaults.model.primary` when you add or reauth a provider. `steelengine models auth login` does the same unless you pass `--set-default`. Provider plugins may still return a recommended default model in their auth config patch, but SteelEngine treats that as "make this model available" when a primary model already exists, not "replace the current primary model."

    To intentionally switch the default model, use `steelengine models set <provider/model>` or `steelengine models auth login --provider <id> --set-default`.
  ...
  <Accordion title="OpenAI provider/runtime split">
    ...
    - Authored Completions adapters, custom endpoints, and routes with authored request behavior stay on SteelEngine. Plaintext official HTTP endpoints are rejected.
    - legacy Codex model refs are legacy config that doctor rewrites to `openai/<model>`.
    - Provider/model `agentRuntime.id: "steelengine"` explicitly keeps an otherwise eligible route on SteelEngine. `agentRuntime.id: "codex"` requires Codex and fails closed when the effective route is not Codex-compatible.
  ...
  <Accordion title="CLI runtimes">
    ...
    Legacy `claude-cli/*` and `google-gemini-cli/*` refs migrate back to canonical provider refs with the runtime recorded separately. Legacy `codex-cli/*` refs migrate to `openai/*` and use the Codex app-server route; SteelEngine no longer keeps a bundled Codex CLI backend.
...
<AccordionGroup>
  <Accordion title="Key sources and priority">
    ...
    - `STEELENGINE_LIVE_<PROVIDER>_KEY` (single live override, highest priority)
...
- Provider: `openai`
- Auth: `OPENAI_API_KEY`
- Optional rotation: `OPENAI_API_KEYS`, `OPENAI_API_KEY_1`, `OPENAI_API_KEY_2`, plus `STEELENGINE_LIVE_OPENAI_KEY` (single override)
...
- Verify account/model availability with `steelengine models list --provider openai` if a specific install or API key behaves differently.
- CLI: `steelengine onboard --auth-choice openai-api-key`
- Default transport is `auto`; SteelEngine passes the transport choice to the shared model runtime.
...
- Hidden SteelEngine attribution headers (`originator`, `version`, `User-Agent`) apply only on native OpenAI traffic to `api.openai.com`, not generic OpenAI-compatible proxies
...
- Optional rotation: `ANTHROPIC_API_KEYS`, `ANTHROPIC_API_KEY_1`, `ANTHROPIC_API_KEY_2`, plus `STEELENGINE_LIVE_ANTHROPIC_KEY` (single override)
- Example model: `anthropic/claude-opus-4-6`
- CLI: `steelengine onboard --auth-choice apiKey`
- Direct public Anthropic requests support the shared `/fast` toggle and `params.fastMode`, including API-key and OAuth-authenticated traffic sent to `api.anthropic.com`; SteelEngine maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
...
<Note>
Claude CLI reuse (`claude -p`) is a sanctioned SteelEngine integration path. Anthropic setup-token auth remains supported, but SteelEngine prefers Claude CLI reuse when available.
</Note>
...
- CLI: `steelengine onboard --auth-choice openai` or `steelengine models auth login --provider openai`
- SteelEngine's embedded ChatGPT Responses transport defaults to `auto` (WebSocket-first, SSE fallback).
- `agents.defaults.models["openai/<model>"].params.transport`, `params.serviceTier`, and `params.fastMode` are authored embedded-request settings. They keep implicit runtime selection on SteelEngine; native Codex owns its app-server transport and service tier.
- Hidden SteelEngine attribution headers (`originator`, `version`, `User-Agent`) are only attached on native Codex traffic to `chatgpt.com/backend-api`, not generic OpenAI-compatible proxies
...
- Use provider/model `agentRuntime.id: "steelengine"` to keep an otherwise eligible route on the built-in runtime. With runtime unset or `auto`, only an exact official HTTPS Responses/ChatGPT-compatible route with no authored request override may select Codex implicitly.
- Legacy Codex GPT refs are legacy state, not a live provider route. Use canonical `openai/*` refs for new agent config, and run `steelengine doctor --fix` to migrate `codex/*` and `openai-codex/*` refs while preserving their native Codex semantics with model-scoped `agentRuntime.id: "codex"`. Existing explicit canonical `openai/gpt-5.5` selections are not upgraded.
...
- CLI: `steelengine onboard --auth-choice opencode-zen` or `steelengine onboard --auth-choice opencode-go`
...
- Provider: `google`
- Auth: `GEMINI_API_KEY`
- Optional rotation: `GEMINI_API_KEYS`, `GEMINI_API_KEY_1`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY` fallback, and `STEELENGINE_LIVE_GEMINI_KEY` (single override)
- Example models: `google/gemini-3.1-pro-preview`, `google/gemini-3.5-flash`
- Compatibility: legacy SteelEngine config using `google/gemini-3.1-flash-preview` is normalized to `google/gemini-3-flash-preview`
- Alias: `google/gemini-3.1-pro` is accepted and normalized to Google's live Gemini API id, `google/gemini-3.1-pro-preview`
- CLI: `steelengine onboard --auth-choice gemini-api-key`
...
- Direct Gemini runs also accept `agents.defaults.models["google/<model>"].params.cachedContent` (or legacy `cached_content`) to forward a provider-native `cachedContents/...` handle; Gemini cache hits surface as SteelEngine `cacheRead`
...
<Warning>
Gemini CLI OAuth in SteelEngine is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
</Warning>
...
<Steps>
  ...
  <Step title="Enable plugin">
    ```bash
    steelengine plugins enable google
    ```
  </Step>
  <Step title="Login">
    ```bash
    ...
    Default model: `google-gemini-cli/gemini-3-flash-preview`. You do **not** paste a client id or secret into `steelengine.json`. The CLI login flow stores tokens in auth profiles on the gateway host.
...
Gemini CLI uses `stream-json` by default. SteelEngine reads assistant stream
...
- CLI: `steelengine onboard --auth-choice zai-api-key`
...
- CLI: `steelengine onboard --auth-choice ai-gateway-api-key`
...
Install `@steelengine/moonshot-provider` before onboarding. Add an explicit `models.providers.moonshot` entry only when you need to override the base URL or model metadata:

- Provider: `moonshot`
- Auth: `MOONSHOT_API_KEY`
- Example model: `moonshot/kimi-k3`
- CLI: `steelengine onboard --auth-choice moonshot-api-key` or `steelengine onboard --auth-choice moonshot-api-key-cn`
...
- CLI: `steelengine onboard --auth-choice volcengine-api-key`
...
In onboarding/configure model pickers, the Volcengine auth choice prefers both `volcengine/*` and `volcengine-plan/*` rows. If those models are not loaded yet, SteelEngine falls back to the unfiltered catalog instead of showing an empty provider-scoped picker.
...
- CLI: `steelengine onboard --auth-choice byteplus-api-key`
...
In onboarding/configure model pickers, the BytePlus auth choice prefers both `byteplus/*` and `byteplus-plan/*` rows. If those models are not loaded yet, SteelEngine falls back to the unfiltered catalog instead of showing an empty provider-scoped picker.
...
- CLI: `steelengine onboard --auth-choice synthetic-api-key`
...
<Note>
On MiniMax's Anthropic-compatible streaming path, SteelEngine disables thinking by default for the M2.x family unless you explicitly set it; MiniMax-M3 (and M3.x) stays on the provider's omitted/adaptive thinking path by default. `/fast on` rewrites `MiniMax-M2.7` to `MiniMax-M2.7-highspeed`.
</Note>
...
SteelEngine uses LM Studio's native `/api/v1/models` and `/api/v1/models/load` for discovery + auto-load, with `/v1/chat/completions` for inference by default. If you want LM Studio JIT loading, TTL, and auto-evict to own model lifecycle, set `models.providers.lmstudio.params.preload: false`. See [/providers/lmstudio](/providers/lmstudio) for setup and troubleshooting.
...
Ollama is detected locally at `http://127.0.0.1:11434` when you opt in with `OLLAMA_API_KEY`, and the bundled provider plugin adds Ollama directly to `steelengine onboard` and the model picker. See [/providers/ollama](/providers/ollama) for onboarding, cloud/local mode, and custom configuration.
...
<AccordionGroup>
  <Accordion title="Default optional fields">
    For custom providers, `reasoning`, `input`, `cost`, `contextWindow`, and `maxTokens` are optional. When omitted, SteelEngine defaults to:
  ...
  <Accordion title="Proxy-route shaping rules">
    - For `api: "openai-completions"` on non-native endpoints (any non-empty `baseUrl` whose host is not `api.openai.com`), SteelEngine forces `compat.supportsDeveloperRole: false` to avoid provider 400 errors for unsupported `developer` roles.
    - Proxy-style OpenAI-compatible routes also skip native OpenAI-only request shaping: no `service_tier`, no Responses `store`, no Completions `store`, no prompt-cache hints, no OpenAI reasoning-compat payload shaping, and no hidden SteelEngine attribution headers.
...
steelengine models set opencode/claude-opus-4-6
