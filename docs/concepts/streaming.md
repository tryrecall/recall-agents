...
Recall has two separate streaming layers:
...
media block early, Recall remembers that delivery for the turn. If the final
...
distinct text around media that was already streamed, Recall still sends the
...
When block streaming is enabled, Recall can **merge consecutive block chunks**
...
- Discord: `streamMode` + boolean `streaming` remain runtime aliases for the `streaming` enum; run `recall doctor --fix` to rewrite persisted config.
- Slack: `streamMode` remains a runtime alias for `streaming.mode`; boolean `streaming` remains a runtime alias for `streaming.mode` plus `streaming.nativeTransport`; legacy `nativeStreaming` remains a runtime alias for `streaming.nativeTransport`. Run `recall doctor --fix` to rewrite persisted config.
