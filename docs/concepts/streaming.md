...
SteelEngine has two independent streaming layers, and there is **no true
...
legacy on every bundled channel: `steelengine doctor --fix` migrates them into
...
streaming sends media early, SteelEngine remembers that delivery for the turn. If
...
distinct text around media that was already streamed, SteelEngine still sends the
...
When block streaming is enabled, SteelEngine can **merge consecutive block
...
top-level boolean/string spellings are rewritten by `steelengine doctor --fix`).
...
| -------- | ----------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Telegram | `streamMode`, scalar/boolean `streaming`                    | Rewritten to `streaming.mode` by `steelengine doctor --fix`; not read at runtime                                                                        |
| Discord  | `streamMode`, boolean `streaming`                           | Rewritten to `streaming.mode` by `steelengine doctor --fix`; not read at runtime                                                                        |
| Slack    | `streamMode`; boolean `streaming`; legacy `nativeStreaming` | Rewritten to `streaming.mode` (and `streaming.nativeTransport` for the boolean/legacy forms) by `steelengine doctor --fix`; not read at runtime         |
| Matrix   | scalar/boolean `streaming`                                  | Rewritten to `streaming.mode` (including Matrix's `"quiet"` mode) by `steelengine doctor --fix`; not read at runtime                                    |
| Feishu   | boolean `streaming`                                         | Rewritten to `streaming.mode` by `steelengine doctor --fix`; not read at runtime                                                                        |
| QQ Bot   | boolean `streaming`; `streaming.c2cStreamApi`               | Rewritten to `streaming.mode` (and `streaming.nativeTransport` for the boolean/`c2cStreamApi` forms) by `steelengine doctor --fix`; not read at runtime |
...
- If the final edit fails before the completed text is confirmed, SteelEngine uses
...
- Telegram selected quote replies are an exception: when `replyToMode` is not
  `"off"` and selected quote text is present, SteelEngine skips the answer preview
...
- To keep preview streaming but hide tool-progress lines, set
  ...
  that use SteelEngine's compact progress renderer, including Discord, Matrix,
