...
[Honcho](https://honcho.dev) adds AI-native memory to Recall. It persists
...
```bash
recall plugins install @honcho-ai/recall-honcho
recall honcho setup
...
Settings live under `plugins.entries["recall-honcho"].config`:
    ...
    entries: {
      "recall-honcho": {
        config: {
          ...
          workspaceId: "recall", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `recall honcho setup` detects and
...
```bash
recall honcho setup                        # Configure API key and migrate files
recall honcho status                       # Check connection status
recall honcho ask <question>               # Query Honcho about the user
recall honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/recall-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Recall integration guide](https://docs.honcho.dev/v3/guides/integrations/recall)
- [Memory](/concepts/memory) -- Recall memory overview
