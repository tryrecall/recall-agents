...
SteelEngine uses OpenAI embeddings by default. To use another provider, set it
explicitly:
...
SteelEngine runs two retrieval paths in parallel and merges the results:
...
**No results?** Run `steelengine memory status` to check the index. If empty, run
`steelengine memory index --force`.

**Only keyword matches?** Your embedding provider may not be configured. Check
`steelengine memory status --deep`.
