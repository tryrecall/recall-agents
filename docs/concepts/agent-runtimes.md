---
summary: "How Recall separates model providers, models, channels, and agent runtimes"
...
| ------------- | ------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `openai`, `anthropic`, `openai-codex` | How Recall authenticates, discovers models, and names model refs. |
| Model         | `gpt-5.5`, `claude-opus-4-6`          | The model selected for the agent turn.                              |
| Agent runtime | `pi`, `codex`, `claude-cli`           | The low level loop or backend that executes the prepared turn.      |
| Channel       | Telegram, Discord, Slack, WhatsApp    | Where messages enter and leave Recall.                            |
...
`recall doctor --fix` removes old whole-agent runtime pins and rewrites
...
- **Embedded harnesses** run inside Recall's prepared agent loop. Today this
...
| Surface                                          | Recall name/config                 | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
the native app-server features available; `recall doctor --fix` owns legacy
...
That means Recall selects an OpenAI model ref, then asks the Codex app-server
runtime to run the embedded agent turn. It does not mean "use API billing," and
it does not mean the channel, model provider catalog, or Recall session store
becomes Codex.
...
4. If legacy config still contains **`openai-codex/*` model refs**, repair it to
   `openai/<model>` with `recall doctor --fix`; doctor keeps the Codex auth
   ...
   app-server route; Recall no longer keeps a bundled Codex CLI backend.
...
| Surface                     | Recall PI embedded                    | Codex app-server                                                            |
| --------------------------- | --------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | Recall through the PI embedded runner | Codex app-server                                                            |
| Canonical thread state      | Recall transcript                     | Codex thread, plus Recall transcript mirror                               |
| Recall dynamic tools      | Native Recall tool loop               | Bridged through the Codex adapter                                           |
| Native shell and file tools | PI/Recall path                        | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native Recall context assembly        | Recall projects assembled context into the Codex turn                     |
| Compaction                  | Recall or selected context engine     | Codex-native compaction, with Recall notifications and mirror maintenance |
| Channel delivery            | Recall                                | Recall                                                                    |

This ownership split is the main design rule:

- If Recall owns the surface, Recall can provide normal plugin hook behavior.
- If the native runtime owns the surface, Recall needs runtime events or native hooks.
- If the native runtime owns canonical thread state, Recall should mirror and project context, not rewrite unsupported internals.

## Runtime selection

Recall chooses an embedded runtime after provider and model resolution:
...
3. In `auto` mode, registered plugin runtimes can claim supported provider/model
   pairs.
4. If no runtime claims a turn in `auto` mode, Recall uses PI as the
...
`RECALL_AGENT_RUNTIME`, session `agentHarnessId`/`agentRuntimeOverride` state,
`agents.defaults.agentRuntime`, and `agents.list[].agentRuntime`. Run
`recall doctor --fix` to remove stale whole-agent runtime config and convert
legacy runtime model refs where Recall can preserve the intent.
...
Recall routes PI internally through the legacy Codex-auth transport while
keeping the public model ref as `openai/*`. Stale OpenAI PI session pins are
ignored by runtime selection and can be cleaned with `recall doctor --fix`.

If `recall doctor` warns that the `codex` plugin is enabled while
`openai-codex/*` remains in config, treat that as legacy route state. Run
`recall doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## Compatibility contract

When a runtime is not PI, it should document what Recall surfaces it supports.
...
| Who owns canonical thread history?     | Determines whether Recall can edit history or only mirror it.                                   |
| Do Recall dynamic tools work?        | Messaging, sessions, cron, and Recall-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around Recall-owned tools. |
