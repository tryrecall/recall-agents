---
summary: "Where timezones show up in SteelEngine — envelopes, tool payloads, system prompt"
read_when:
...
SteelEngine standardizes timestamps so the model sees a **single reference time** instead of a mix of provider-local clocks. Three surfaces show timezones, each with its own purpose:
...
If `userTimezone` is unset, SteelEngine resolves the host timezone at runtime via `Intl.DateTimeFormat().resolvedOptions().timeZone` (no config write). `agents.defaults.timeFormat` (`auto` | `12` | `24`) controls 12h/24h rendering in envelopes and downstream surfaces, not in the system prompt section.
