...
read_when:
  ...
  - Explaining how Recall updates one channel message while work is in progress
...
When progress drafts are enabled, Recall creates one visible work-in-progress
...
That is usually enough. Recall will pick an automatic one-word label, wait
...
Recall sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from Recall's built-in
...
Recall uses the same formatter for progress drafts and `/verbose`:
...
Recall truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, Recall still suppresses the older standalone
...
When the final answer is ready, Recall tries to keep the chat clean:

- If the draft can safely become the final answer, Recall edits it in place.
- If the channel uses native progress streaming, Recall finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, Recall sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, Recall keeps the
