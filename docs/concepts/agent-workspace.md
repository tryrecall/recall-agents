...
This is separate from `~/.steelengine/`, which stores config, credentials, and sessions.
...
When sandboxing is enabled and `workspaceAccess` is not `"rw"`, tools operate inside a sandbox workspace under `~/.steelengine/sandboxes`, not your host workspace.
</Warning>

## Default location

- Default: `~/.steelengine/workspace`
- If `STEELENGINE_PROFILE` is set and not `"default"`, the default becomes `~/.steelengine/workspace-<profile>`.
- `STEELENGINE_WORKSPACE_DIR` overrides both of the above when set.
...
Override in `~/.steelengine/steelengine.json`:
...
`steelengine onboard`, `steelengine configure`, or `steelengine setup` create the workspace and seed the bootstrap files if they are missing.
...
Older installs may have created `~/steelengine`. Keeping multiple workspace directories around can cause confusing auth or state drift, since only one workspace is active at a time.

<Note>
**Recommendation:** keep a single active workspace. If you no longer use the extra folders, archive or move them to Trash (for example `trash ~/steelengine`). If you intentionally keep multiple workspaces, make sure `agents.defaults.workspace` (or the per-agent `workspace` key) points to the active one.
</Note>

## Workspace file map

Standard files SteelEngine expects inside the workspace:
...
<Note>
If a bootstrap file is missing, SteelEngine injects a "missing file" marker into the session and continues. Large bootstrap files are truncated when injected; adjust limits with `agents.defaults.bootstrapMaxChars` (default: `20000`) and `agents.defaults.bootstrapTotalMaxChars` (default: `60000`). `steelengine setup` can recreate missing defaults without overwriting existing files.
</Note>

## What is NOT in the workspace

These live under `~/.steelengine/` and should NOT be committed to the workspace repo:

- `~/.steelengine/steelengine.json` (config)
- `~/.steelengine/state/steelengine.sqlite` (shared workspace setup state and attestations)
- `~/.steelengine/agents/<agentId>/agent/auth-profiles.json` (model auth profiles: OAuth + API keys)
- `~/.steelengine/agents/<agentId>/agent/steelengine-agent.sqlite` (session rows, transcripts, and per-agent runtime state)
- `~/.steelengine/agents/<agentId>/agent/codex-home/` (per-agent Codex runtime account, config, skills, plugins, and native thread state)
- `~/.steelengine/credentials/` (channel/provider state plus legacy OAuth import data)
- `~/.steelengine/agents/<agentId>/sessions/` (legacy migration sources and archive/support artifacts)
- `~/.steelengine/skills/` (managed skills)

If you need to migrate sessions or config, copy them separately and keep them out of version control.

Older SteelEngine releases wrote `steelengine-workspace-state.json`,
`.steelengine/workspace-state.json`, and `.attested` workspace sidecars. Current
runtime uses only the shared SQLite database for that state. If Doctor reports
one of these files, run `steelengine doctor --fix`; Doctor imports valid legacy
  ...
  <Step title="Add a private remote">
    <Tabs>
      ...
      <Tab title="GitHub CLI (gh)">
        ```bash
        gh auth login
        gh repo create steelengine-workspace --private --source . --remote origin --push
        ```
      </Tab>
...
- Anything under `~/.steelengine/`.
- Raw dumps of chats or sensitive attachments.

If you must store sensitive references, use placeholders and keep the real secret elsewhere (password manager, environment variables, or `~/.steelengine/`).
</Warning>
...
<Steps>
  <Step title="Clone the repo">
    Clone the repo to the desired path (default `~/.steelengine/workspace`).
  </Step>
  <Step title="Update config">
    Set `agents.defaults.workspace` to that path in `~/.steelengine/steelengine.json`.
  </Step>
  <Step title="Seed missing files">
    Run `steelengine setup --workspace <path>` to seed any missing files.
  </Step>
  <Step title="Copy sessions (optional)">
    If you need sessions, copy `~/.steelengine/agents/<agentId>/agent/steelengine-agent.sqlite`
    from the old machine separately. Copy `~/.steelengine/agents/<agentId>/sessions/`
