...
This is separate from `~/.recall/`, which stores config, credentials, and sessions.
...
When sandboxing is enabled and `workspaceAccess` is not `"rw"`, tools operate inside a sandbox workspace under `~/.recall/sandboxes`, not your host workspace.
</Warning>

## Default location

- Default: `~/.recall/workspace`
- If `RECALL_PROFILE` is set and not `"default"`, the default becomes `~/.recall/workspace-<profile>`.
- Override in `~/.tryrecall/recall-agents.json`:
...
`recall onboard`, `recall configure`, or `recall setup` will create the workspace and seed the bootstrap files if they are missing.
...
Older installs may have created `~/recall`. Keeping multiple workspace directories around can cause confusing auth or state drift, because only one workspace is active at a time.

<Note>
**Recommendation:** keep a single active workspace. If you no longer use the extra folders, archive or move them to Trash (for example `trash ~/recall`). If you intentionally keep multiple workspaces, make sure `agents.defaults.workspace` points to the active one.

`recall doctor` warns when it detects extra workspace directories.
</Note>

## Workspace file map

These are the standard files Recall expects inside the workspace:
...
<Note>
If any bootstrap file is missing, Recall injects a "missing file" marker into the session and continues. Large bootstrap files are truncated when injected; adjust limits with `agents.defaults.bootstrapMaxChars` (default: 12000) and `agents.defaults.bootstrapTotalMaxChars` (default: 60000). `recall setup` can recreate missing defaults without overwriting existing files.
</Note>

## What is NOT in the workspace

These live under `~/.recall/` and should NOT be committed to the workspace repo:

- `~/.tryrecall/recall-agents.json` (config)
- `~/.recall/agents/<agentId>/agent/auth-profiles.json` (model auth profiles: OAuth + API keys)
- `~/.recall/agents/<agentId>/agent/codex-home/` (per-agent Codex runtime account, config, skills, plugins, and native thread state)
- `~/.recall/credentials/` (channel/provider state plus legacy OAuth import data)
- `~/.recall/agents/<agentId>/sessions/` (session transcripts + metadata)
- `~/.recall/skills/` (managed skills)
...
- Anything under `~/.recall/`.
- Raw dumps of chats or sensitive attachments.

If you must store sensitive references, use placeholders and keep the real secret elsewhere (password manager, environment variables, or `~/.recall/`).
</Warning>
...
<Steps>
  <Step title="Clone the repo">
    Clone the repo to the desired path (default `~/.recall/workspace`).
  </Step>
  <Step title="Update config">
    Set `agents.defaults.workspace` to that path in `~/.tryrecall/recall-agents.json`.
  </Step>
  <Step title="Seed missing files">
    Run `recall setup --workspace <path>` to seed any missing files.
  </Step>
  <Step title="Copy sessions (optional)">
    If you need sessions, copy `~/.recall/agents/<agentId>/sessions/` from the old machine separately.
  </Step>
</Steps>
