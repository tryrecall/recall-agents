---
summary: "What the Recall system prompt contains and how it is assembled"
read_when:
...
Recall builds a custom system prompt for every agent run. The prompt is **Recall-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by Recall and injected into each agent run.
...
the full Recall-owned prompt. The provider runtime can:
...
- **Recall Control**: tells the model to prefer the `gateway` tool for
  config/restart work and to avoid inventing CLI commands.
- **Recall Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to Recall docs/source and when to read them.
...
Recall keeps large stable content, including **Project Context**, above the
...
Recall can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Memory Recall**, **Recall
...
For channel auto-reply runs, Recall omits the generic **Silent Replies**
...
Recall keeps committed prompt snapshots for the Codex runtime happy path under
...
Recall developer instructions, turn-scoped collaboration-mode instructions
when Recall provides them, user turn input, and references to the dynamic tool
specs.
...
collaboration-mode instructions inside the Codex runtime after Recall sends
...
On the native Codex harness, Recall avoids repeating stable workspace files
...
Recall prompt according to their existing gates. `HEARTBEAT.md` is omitted on
...
occurs, Recall can inject a concise system-prompt warning notice; control this with
...
When eligible skills exist, Recall injects a compact **available skills list**
...
points to the local Recall docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.recall.ai](https://docs.recall.ai).

The same section also includes the Recall source location. Git checkouts expose the local
...
consult docs first for Recall behavior, commands, configuration, or architecture, and to
run `recall status` itself when possible (asking the user only when it lacks access).
