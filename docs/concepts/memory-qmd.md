...
alongside Recall. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Recall falls back to the
...
Recall creates a self-contained QMD home under
`~/.recall/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- Recall creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so Recall skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, Recall retries with `qmd query`.
- With QMD releases that advertise multi-collection filters, Recall groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, Recall falls back to the builtin SQLite engine.
  ...
  `recall memory status` and one-shot CLI probes still recheck QMD directly.
...
Recall keeps the QMD search path compatible with both current and older QMD
installs.

On startup, Recall checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, Recall searches all
...
Older QMD builds only accept one collection filter. When Recall detects one
...
process, so you can tune QMD globally without adding new Recall config:
...
collection under `~/.recall/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Recall logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Recall
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but Recall still reports
...
with `recall memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Recall uses.

**Many QMD subprocesses during search?** Update QMD if possible. Recall uses
...
`memory.qmd.searchMode = "search"`. Recall treats that mode as lexical-only,
...
the next startup reconciliation. Recall recreates stale managed collections
...
Recall's builtin symlink rules. Keep temporary monorepo checkouts under
