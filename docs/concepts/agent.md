...
SteelEngine ships one **embedded agent runtime**: a built-in agent loop, tool
...
Recommended: use `steelengine setup` to create `~/.steelengine/steelengine.json` if missing and initialize the workspace files.
...
Inside the workspace, SteelEngine expects these user-editable files:
...
On the first turn of a new session, SteelEngine injects the contents of these files into the system prompt's Project Context. `MEMORY.md` is only injected when it exists at the workspace root.

Blank files are skipped. Large files are trimmed and truncated with a marker so prompts stay lean (read the file for full content). A missing file (other than `MEMORY.md`) injects a single "missing file" marker line instead; `steelengine setup` creates a safe default template for it.

`BOOTSTRAP.md` is only created for a **brand new workspace** (no other bootstrap files present). While it is pending, SteelEngine keeps it in Project Context and adds system-prompt bootstrap guidance for the initial ritual instead of copying it into the user message. If you delete it after completing the ritual, it is not recreated on later restarts.

After a workspace has been observed, SteelEngine stores its setup state and
attestation in the shared SQLite database at
`~/.steelengine/state/steelengine.sqlite`. If a recently attested workspace
...
not read those files. Run `steelengine doctor --fix` to validate them, import their
...
SteelEngine loads skills from these locations (highest precedence first):

- Workspace: `<workspace>/skills`
- Project agent skills: `<workspace>/.agents/skills`
- Personal agent skills: `~/.agents/skills`
- Managed/local: `~/.steelengine/skills`
...
The embedded agent runtime is SteelEngine-owned: model discovery, tool wiring,
...
`~/.steelengine/agents/<agentId>/sessions/` as legacy migration inputs, deleted or
...
SteelEngine. SteelEngine does not read session folders from other tools.
...
- If you omit the provider, SteelEngine tries an alias first, then a unique
  ...
  configured default model, SteelEngine falls back to the first configured
