...
- `/usage cost` in chats: local cost summary aggregated from Recall session logs.
- CLI: `recall status --usage` prints a full per-provider breakdown.
- CLI: `recall channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Recall treats
  ...
  quota, so Recall inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Recall prefers the
...
can supply plugin-specific usage auth logic; otherwise Recall falls back to
