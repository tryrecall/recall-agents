...
- `agents.defaults.models` is the allowlist/catalog of models Recall can use (plus aliases).
...
`recall models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.recall/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: Recall writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever Recall regenerates `models.json`, including command-driven paths like `recall agent`.
