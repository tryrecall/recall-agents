...
| Local             | `local`             | `@steelengine/llama-cpp-provider`      |
| Mistral           | `mistral`           |                                     |
...
SteelEngine indexes `MEMORY.md` and `memory/*.md` into chunks (400 tokens with
...
- **Reindex on demand:** `steelengine memory index --force`
...
**Memory search disabled?** Check `steelengine memory status`. If no provider is
...
**Stale results?** Run `steelengine memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** SteelEngine falls back to in-process cosine
similarity automatically. `steelengine memory status --deep` reports the local
