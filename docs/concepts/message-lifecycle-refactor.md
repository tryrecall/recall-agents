...
That shape fixes local bugs, but it leaves Recall with too many public
...
That gives Recall at-least-once recovery. Exactly-once behavior exists only
...
- Structured Recall-origin metadata for operational/system output so visible
...
Recall should borrow the vocabulary, not copy the surface.

What Recall needs beyond that model:
...
`thread.post()` style promises are not enough for Recall. They hide the
...
Origin describes who produced a message and how Recall should treat echoes of
that message. It is separate from relation: a message can be a reply to a user
and still be Recall-originated operational output.
...
Core owns the meaning of Recall-originated output. Channels own how that
...
Recall operational output must not be accepted as bot-authored input in shared
...
- **Transport ack:** tells the platform webhook or socket that Recall accepted
...
- **Inbound record ack:** confirms Recall persisted enough inbound metadata to
...
Before bot authorization, receive must apply the shared Recall echo policy
when the channel can decode message origin metadata:

```typescript
function shouldDropRecallEcho(params: {
  origin?: MessageOrigin;
  isBotAuthor: boolean;
  isRoomish: boolean;
}): boolean {
...
same visible gateway-failure text but without Recall origin metadata still
...
middleware chain, but Recall persists only the safe completed update id after
...
source if we need platform-level redelivery beyond Recall's restart
...
process dies there, Recall cannot know whether the platform message exists
...
```typescript
import { defineChannelMessageAdapter } from "recall/plugin-sdk/channel-message";
```
...
Before preflight authorization, core must run the shared Recall echo predicate
whenever `origin.decode` returns Recall-origin metadata. The receive adapter
...
- iMessage monitor delivery records sent messages in an echo cache after a
  ...
  Recall can re-ingest its own final replies as inbound user messages.
...
- Direct-DM helpers can have a reply callback that is the only correct transport
  ...
  that callback.
- Recall gateway failure output must stay visible to humans, but tagged
...
| Slack           | Send adapter handles normal chat posts. Live adapter chooses native stream when thread shape supports it, otherwise draft preview. Receipts preserve thread timestamps. Origin adapter maps Recall gateway failures to Slack `chat.postMessage.metadata` and drops tagged bot-room echoes before `allowBots` authorization.                                  |
...
| Matrix          | Live adapter owns draft event edits, finalization, redaction, encrypted media constraints, and reply-target mismatch fallback. Receive adapter owns encrypted event hydration and dedupe. Origin adapter should encode Recall gateway-failure origin into Matrix event content and drop configured-bot room echoes before `allowBots` handling.              |
...
| Google Chat     | Simple receive plus send adapter with thread relation mapped to spaces and thread ids. Audit `allowBots=true` room behavior for tagged Recall gateway-failure echoes.                                                                                                                                                                                        |
...
- Persist message origin through durable send intents so recovery, replay, and
  chunked sends preserve Recall operational provenance.
...
- Add `recall/plugin-sdk/channel-message`.
...
- Include `MessageOrigin`, origin encode/decode hooks, and the shared
  `shouldDropRecallEcho` predicate in the channel-message SDK surface.
...
This is where the model stops being "agent replies" and becomes "Recall sends
messages".
...
- Gateway-failure origin factory and `shouldDropRecallEcho` predicate.
...
- Slack tagged Recall gateway failure messages stay visible outbound, tagged
...
- Matrix tagged Recall gateway-failure room echoes from configured bot
...
- Whether Telegram should eventually replace the grammY runner source with a
  ...
  only Recall's persisted restart watermark.
...
- Recall-originated gateway failure output is visible to humans but tagged
