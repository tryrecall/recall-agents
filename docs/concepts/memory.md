---
summary: "How SteelEngine remembers things across sessions"
...
SteelEngine remembers things by writing plain Markdown files in your agent's
workspace (default `~/.steelengine/workspace`). The model only remembers what gets
...
If `MEMORY.md` grows past the bootstrap file budget, SteelEngine keeps the file on
...
prompt budget. Use `/context list`, `/context detail`, or `steelengine doctor` to
...
detected files, and confirm the import. SteelEngine copies only Markdown memory:
...
SteelEngine approval settings, sandboxing, and scheduled tasks for hard
...
memories for that case. SteelEngine infers them in a hidden background pass,
...
<Info>
SteelEngine uses OpenAI embeddings by default. Set
...
SteelEngine runs a silent turn that reminds the agent to save important context
...
```bash
steelengine memory rem-backfill --rollback
steelengine memory rem-backfill --rollback-short-term
```

## CLI

```bash
steelengine memory status          # Check index status and provider
steelengine memory search "query"  # Search from the command line
steelengine memory index --force   # Rebuild the index
```
