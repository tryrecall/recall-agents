---
summary: "How Recall remembers things across sessions"
...
Recall remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.recall/workspace`).
...
If `MEMORY.md` grows past the bootstrap file budget, Recall keeps the file on
...
`recall doctor` to see raw vs injected sizes and truncation status.
...
Memory can preserve approval context, but it does not enforce policy. Use Recall approval settings, sandboxing, and scheduled tasks for hard operational controls.
...
for that case. Recall infers them in a hidden background pass, scopes them to
...
<Info>
Recall auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, Recall
...
```bash
recall memory rem-backfill --rollback
recall memory rem-backfill --rollback-short-term
```

## CLI

```bash
recall memory status          # Check index status and provider
recall memory search "query"  # Search from the command line
recall memory index --force   # Rebuild the index
```
