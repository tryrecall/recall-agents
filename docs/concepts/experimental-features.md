---
summary: "What experimental flags mean in Recall and which ones are currently documented"
...
Experimental features in Recall are **opt-in preview surfaces**. They are
...
- If you are rolling Recall out broadly, test experimental flags in a smaller
...
| ------------------------ | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Local model runtime      | `agents.defaults.experimental.localModelLean`, `agents.list[].experimental.localModelLean` | A smaller or stricter local backend chokes on Recall's full default tool surface                                                | [Local Models](/gateway/local-models)                                                         |
...
| Codex harness            | `plugins.entries.codex.config.appServer.experimental.sandboxExecServer`                    | You want native Codex app-server 0.132.0 or newer to target an Recall sandbox-backed exec-server instead of disabling Code Mode | [Codex harness reference](/plugins/codex-harness-reference#sandboxed-native-execution)        |
...
`agents.defaults.experimental.localModelLean: true` is a pressure-release valve for weaker local-model setups. When it is on, Recall drops three default tools — `browser`, `cron`, and `message` — from the agent's tool surface for every turn. Nothing else changes. Use `agents.list[].experimental.localModelLean` to enable or disable the same behavior for one configured agent.

### Why these three tools

These three tools have the largest descriptions and the most parameter shapes in the default Recall runtime. On a small-context or stricter OpenAI-compatible backend that is the difference between:
...
Removing them does not silently rewire Recall — it just makes the tool list shorter. The model still has `read`, `write`, `edit`, `exec`, `apply_patch`, web search/fetch (when configured), memory, and session/agent tools available.
...
1. `recall infer model run --gateway --model <ref> --prompt "Reply with exactly: pong"` succeeds.
...
If a feature is experimental, Recall should say so plainly in docs and in the
