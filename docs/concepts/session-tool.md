...
SteelEngine gives agents tools to work across sessions, inspect status, and orchestrate sub-agents.
...
`sessions_history` fetches the conversation transcript for a specific session. By default, tool results are excluded; pass `includeTools: true` to see them. Use `limit` for the newest bounded tail. Pass `offset: 0` when you need pagination metadata, then pass returned `nextOffset` values to page backward through older SteelEngine transcript windows without reading raw transcript files. Explicit offset pages do not merge external CLI fallback imports; use the default newest-tail view (no `offset`) when you need that merged display history.
...
After the target responds, SteelEngine can run a **reply-back loop** where the agents alternate messages (up to `session.agentToAgent.maxPingPongTurns`, range 0-20, default 5). The target agent can reply `REPLY_SKIP` to stop early.
...
`subagents` is the visibility helper for already spawned SteelEngine sub-agents. It supports `action: "list"` to inspect active/recent runs.
