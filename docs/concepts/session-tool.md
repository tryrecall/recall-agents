...
Recall gives agents tools to work across sessions, inspect status, and
...
After the target responds, Recall can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `recall-tui` are
not session keys.
...
`subagents` is the control-plane helper for already spawned Recall
...
the completion origin only identifies a channel Recall can still reuse the
