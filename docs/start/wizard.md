...
CLI onboarding is the **recommended** way to set up Recall on macOS,
...
`RECALL_LOCALE`, then `LC_ALL`, then `LC_MESSAGES`, then `LANG`, and falls
...
Names and stable identifiers stay literal: `Recall`, `Gateway`, `Tailscale`,
commands, config keys, URLs, provider IDs, model IDs, and plugin/channel labels
are not translated.

<Info>
Fastest first chat: open the Control UI (no channel setup needed). Run
`recall dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`recall configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.recall/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `recall doctor` first.
</Note>
...
Use `recall agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.recall/workspace-<agentId>`.
...
- CLI command reference: [`recall onboard`](/cli/onboard)
