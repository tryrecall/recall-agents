---
summary: "Step-by-step behavior for steelengine onboard: what each step does, config it writes, and internals"
read_when:
  - You need detailed behavior for a specific steelengine onboard step
...
commands), see [`steelengine onboard`](/cli/onboard).
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.steelengine/steelengine.json` exists, choose **Keep current values**, **Review and update**, or **Reset before setup**.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove the workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `steelengine doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `steelengine pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Skip this step with `--skip-search`; reconfigure later with `steelengine configure --section web`.
  ...
  <Step title="Daemon install">
    ...
    - Native Windows: Scheduled Task first
      - If task creation is denied, SteelEngine falls back to a per-user Startup-folder login item and starts the gateway immediately.
      - Scheduled Tasks remain preferred because they provide better supervisor status.
    - Runtime selection: Node is required because SteelEngine's canonical runtime state store uses `node:sqlite`.

  </Step>
  <Step title="Health check">
    - Starts gateway (if needed) and runs `steelengine health`.
    - `steelengine status --deep` adds the live gateway health probe to status output, including channel probes when supported.
  ...
  <Step title="Skills">
    ...
    - Skips unavailable Homebrew, uv, and Go installers, then groups the affected
      skills with manual setup guidance. Run `steelengine doctor` after installing
...
Plaintext `ws://` is accepted for loopback, private IP literals, `.local`, and Tailnet `*.ts.net` URLs; other private-DNS names need `STEELENGINE_ALLOW_INSECURE_PRIVATE_WS=1`.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    select `openai/gpt-5.5` explicitly; SteelEngine does not silently downgrade it.

  </Accordion>
  <Accordion title="xAI (Grok) OAuth">
    Browser sign-in for eligible SuperGrok or X Premium accounts. This is the
    recommended xAI path for most users. SteelEngine stores the resulting auth
...
- Auth profiles (API keys + OAuth): `~/.steelengine/agents/<agentId>/agent/auth-profiles.json`
...
- `session.dmScope` (onboarding preserves explicit values and otherwise leaves it unset, so the `main` default keeps all direct messages across channels in the agent's rolling main session—the personal-agent default. For shared or multi-user inboxes, use `per-channel-peer`; `steelengine security audit` recommends isolation when it detects multi-user DM traffic)
...
Full flag reference and provider-specific examples: [`steelengine onboard`](/cli/onboard), [CLI automation](/start/wizard-cli-automation).
...
- Stores the release-asset install under `~/.steelengine/tools/signal-cli/<version>/`
...
- Command reference: [`steelengine onboard`](/cli/onboard)
