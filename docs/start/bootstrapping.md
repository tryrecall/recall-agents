...
On the first agent run, Recall bootstraps the workspace (default
`~/.recall/workspace`):
...
For embedded/local model runs, Recall keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `recall onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.recall/workspace`).
</Note>
