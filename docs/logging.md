...
read_when:
  - You need a beginner-friendly overview of SteelEngine logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

SteelEngine has two main log surfaces:
...
The date uses the gateway host's local timezone. When `/tmp/steelengine` is unsafe
or unavailable (and always on Windows), SteelEngine uses a user-scoped
`steelengine-<uid>` directory under the OS temp dir instead. Dated log files are
pruned after 24 hours.

Each file rotates when the next write would exceed `logging.maxFileBytes`
(default: 100 MB). SteelEngine keeps up to five numbered archives beside the
active file, such as `steelengine-YYYY-MM-DD.1.log`, and keeps writing to a fresh
active log instead of suppressing diagnostics.

You can override the path in `~/.steelengine/steelengine.json`:

```json
{
  "logging": {
    "file": "/path/to/steelengine.log"
  }
}
```
...
or times out before `logs.tail` answers, `steelengine logs` falls back to the
configured Gateway file log automatically. Explicit `--url` targets do not use
this fallback. `steelengine logs --follow` is stricter: on Linux it uses the active
...
```bash
steelengine channels logs --channel whatsapp
```
...
SteelEngine preserves the original structured log arguments alongside these fields
...
`steelengine gateway` also has WebSocket protocol logging for RPC traffic:
...
All logging configuration lives under `logging` in `~/.steelengine/steelengine.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/steelengine/steelengine-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`STEELENGINE_LOG_LEVEL`** environment variable (e.g. `STEELENGINE_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `steelengine.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `steelengine --log-level debug gateway run`), which overrides the environment variable for that command.
...
```bash
STEELENGINE_DEBUG_MODEL_TRANSPORT=1 steelengine gateway
STEELENGINE_DEBUG_MODEL_PAYLOAD=tools STEELENGINE_DEBUG_SSE=events steelengine gateway
```

Available flags:

- `STEELENGINE_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `STEELENGINE_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `STEELENGINE_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `STEELENGINE_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `STEELENGINE_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `STEELENGINE_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `STEELENGINE_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal SteelEngine logging, so `steelengine logs --follow`
...
`STEELENGINE_DEBUG_MODEL_TRANSPORT`, so basic model transport hygiene is visible
without debug flags.

### Trace correlation

File logs are JSONL. When a log call carries a valid diagnostic trace context,
SteelEngine writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
SteelEngine can redact sensitive tokens before they hit console output, file logs,
...
policy. SteelEngine still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `STEELENGINE_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `steelengine doctor` first.
