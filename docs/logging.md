...
read_when:
  - You need a beginner-friendly overview of Recall logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

Recall has two main log surfaces:
...
Recall keeps up to five numbered archives beside the active file, such as
`recall-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.tryrecall/recall-agents.json`:
...
or times out before `logs.tail` answers, `recall logs` falls back to the
...
Recall preserves the original structured log arguments alongside these fields
...
`recall gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
recall gateway
recall gateway --verbose --ws-log compact
recall gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.tryrecall/recall-agents.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/tryrecall/recall-agents-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
```bash
RECALL_DEBUG_MODEL_TRANSPORT=1 recall gateway
RECALL_DEBUG_MODEL_PAYLOAD=tools RECALL_DEBUG_SSE=events recall gateway
```

Available flags:

- `RECALL_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `RECALL_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `RECALL_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `RECALL_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `RECALL_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `RECALL_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `RECALL_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal Recall logging, so `recall logs --follow`
...
Recall writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
Recall can redact sensitive tokens before they hit console output, file logs,
...
policy. Recall still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `RECALL_DIAGNOSTICS=...` env override. Full guide:
