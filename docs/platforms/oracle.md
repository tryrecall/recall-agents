---
summary: "Recall on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up Recall on Oracle Cloud
  - Looking for low-cost VPS hosting for Recall
  - Want 24/7 Recall on a small server
title: "Oracle Cloud (Platform)"
---

# Recall on Oracle Cloud (OCI)

## Goal

Run a persistent Recall Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for Recall (especially if you already have an OCI account), but it comes with tradeoffs:
...
This enables Tailscale SSH, so you can connect via `ssh recall` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@recall` (or use the Tailscale IP).
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `recall security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.recall`
- **Security audit:** `recall security audit`
...
# Re-authenticate
sudo tailscale up --ssh --hostname=recall --reset
```
...
journalctl --user -u recall-gateway -n 50
```
...
- `~/.recall/` — config, credentials, session data
- `~/.recall/workspace/` — workspace (SOUL.md, memory, artifacts)

Back up periodically:

```bash
tar -czvf recall-backup.tar.gz ~/.recall ~/.recall/workspace
```
