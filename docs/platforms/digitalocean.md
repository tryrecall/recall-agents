---
summary: "Recall on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Recall on DigitalOcean
  - Looking for cheap VPS hosting for Recall
title: "DigitalOcean (Platform)"
---

# Recall on DigitalOcean

## Goal

Run a persistent Recall Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
recall channels login whatsapp
# Scan QR code
```
...
- `~/.recall/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u recall --no-pager -n 50
```
