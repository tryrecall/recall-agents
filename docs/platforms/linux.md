...
2. `npm i -g recall@latest`
3. `recall onboard --install-daemon`
...
Recall installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `recall gateway install` and
`recall onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/recall-gateway[-<profile>].service`:
...
sessions and channel connections. Recall therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, Recall starts the child through a short
...
- Recall-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `RECALL_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
