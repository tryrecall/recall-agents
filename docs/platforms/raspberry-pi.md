---
summary: "Recall on Raspberry Pi (budget self-hosted setup)"
read_when:
...
# Recall on Raspberry Pi

## Goal

Run a persistent, always-on Recall Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
(or `RECALL_GATEWAY_TOKEN`) into Control UI settings.
...
- `RECALL_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Recall, add a service drop-in to reduce restart
...
If possible, keep Recall state/cache on SSD-backed storage to avoid SD-card
...
Most Recall features work on ARM64, but some external binaries may need ARM builds:
...
# Common fix: rebuild
cd ~/recall  # if using hackable install
npm run build
sudo systemctl restart recall
```
