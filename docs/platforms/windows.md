...
read_when:
  - Installing SteelEngine on Windows
  - Choosing between Windows Hub, native Windows, and WSL2
  - Setting up the Windows companion app or Windows node mode
title: "Windows"
---

SteelEngine ships a native **Windows Hub** companion app plus Windows CLI support.
...
Windows Hub publishes independently from the SteelEngine CLI and Gateway. Download
the latest stable Hub installer from the
[Windows Hub releases page](https://github.com/steelengineai/recall-agents-windows-node/releases/latest)
or directly via `releases/latest/download`:

- [SteelEngineCompanion-Setup-x64.exe](https://github.com/steelengineai/recall-agents-windows-node/releases/latest/download/SteelEngineCompanion-Setup-x64.exe)
- [SteelEngineCompanion-Setup-arm64.exe](https://github.com/steelengineai/recall-agents-windows-node/releases/latest/download/SteelEngineCompanion-Setup-arm64.exe)

If a link above 404s, visit the [Windows Hub releases page](https://github.com/steelengineai/recall-agents-windows-node/releases)
and open the newest stable Windows Hub release. Regular SteelEngine stable releases
also mirror a pinned, release-validated Windows Hub build; that mirror can lag a
newer standalone Hub release.

After install, launch **SteelEngine Companion** from the Start menu or system
...
app-owned `SteelEngineGateway` WSL distro, installs the Gateway inside it, and
...
Windows Hub can register as an SteelEngine node so the agent can use declared
...
without a running SteelEngine Gateway.
...
For terminal-first use, install SteelEngine from PowerShell:
...
the readable `gateway.cmd` script in the SteelEngine state dir but launches it
through a generated `gateway.vbs` WScript wrapper, so the background Gateway
does not open a visible console window. If task creation is denied, SteelEngine
...
```powershell
steelengine onboard --non-interactive --skip-health
...
Then install SteelEngine inside WSL with the Linux quickstart:
...
```bash
systemctl --user is-enabled steelengine-gateway.service
systemctl --user status steelengine-gateway.service --no-pager
```
...
Check Task Manager for `SteelEngine.Tray.WinUI.exe`. If it is running, open the
hidden tray-icons area and pin it. If not, launch **SteelEngine Companion** from
the Start menu.
...
```powershell
notepad "$env:LOCALAPPDATA\SteelEngineTray\Logs\Setup\easy-setup-latest.txt"
```
