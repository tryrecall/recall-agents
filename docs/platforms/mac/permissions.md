...
- Same path: run the app from a fixed location (for Recall, `dist/Recall.app`).
...
Prefer granting Accessibility to Recall.app, Peekaboo.app, or another signed
...
```bash
sudo tccutil reset Accessibility ai.recall.mac
sudo tccutil reset ScreenCapture ai.recall.mac
...
Workaround: move files into the Recall workspace (`~/.recall/workspace`) if you want to avoid per-folder grants.
