...
- You can still use the CLI flow in [Gateway health](/gateway/health) (`recall status`, `recall status --deep`, `recall health --json`) and tail `/tmp/recall/recall-*.log` for `web-heartbeat` / `web-reconnect`.
