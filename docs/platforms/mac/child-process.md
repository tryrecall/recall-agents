...
service via the external `recall` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.recall.gateway`
  (or `ai.recall.<profile>` when using `--profile`/`RECALL_PROFILE`; legacy `com.recall.*` is supported).
