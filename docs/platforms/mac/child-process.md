...
enables the launchd service via the external `steelengine` CLI (no embedded
...
- The app installs a per-user LaunchAgent labeled `ai.steelengine.gateway` (or
  `ai.steelengine.<profile>` when using `--profile`/`STEELENGINE_PROFILE`).
...
Replace the label with `ai.steelengine.<profile>` when running a named profile.
...
`~/.steelengine/disable-launchagent`, so the app only attaches to an already
