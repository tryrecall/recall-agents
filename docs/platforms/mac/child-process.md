...
- The app installs a per‑user LaunchAgent labeled `ai.tryrecall.gateway`
  (or `ai.tryrecall.<profile>` when using `--profile`/`RECALL_PROFILE`; legacy `com.recall.*` is supported).
