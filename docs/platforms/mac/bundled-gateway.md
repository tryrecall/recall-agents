...
Node 24 is the default runtime on the Mac. Node 22 LTS, currently `22.16+`, still works for compatibility. Then install `recall` globally:
...
- `ai.tryrecall.gateway` (or `ai.tryrecall.<profile>`; legacy `com.recall.*` may remain)

Plist location (per‑user):

- `~/Library/LaunchAgents/ai.tryrecall.gateway.plist`
  (or `~/Library/LaunchAgents/ai.tryrecall.<profile>.plist`)
