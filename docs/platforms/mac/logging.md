---
summary: "SteelEngine logging: rolling diagnostics file log + unified log privacy flags"
read_when:
...
- Location: `~/Library/Logs/SteelEngine/diagnostics.jsonl`.
...
## Enable for SteelEngine (`ai.steelengine`)

Write the plist to a temp file first, then install it atomically as root:

```bash
cat <<'EOF' >/tmp/ai.steelengine.plist
...
EOF
sudo install -m 644 -o root -g wheel /tmp/ai.steelengine.plist /Library/Preferences/Logging/Subsystems/ai.steelengine.plist
```
...
- Remove the override: `sudo rm /Library/Preferences/Logging/Subsystems/ai.steelengine.plist`.
