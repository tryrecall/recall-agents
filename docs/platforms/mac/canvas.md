...
- `~/Library/Application Support/SteelEngine/canvas/<session>/...`

The Canvas panel serves those files via a custom URL scheme,
`steelengine-canvas://<session>/<path>`:

- `steelengine-canvas://main/` -> `<canvasRoot>/main/index.html`
- `steelengine-canvas://main/assets/app.css` -> `<canvasRoot>/main/assets/app.css`
- `steelengine-canvas://main/widgets/todo/` -> `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
steelengine nodes canvas present --node <id>
steelengine nodes canvas navigate --node <id> "/"
steelengine nodes canvas eval --node <id> --js "document.title"
steelengine nodes canvas snapshot --node <id>
```
...
Gateway-hosted targets under `/__steelengine__/canvas/` and
`/__steelengine__/a2ui/` are resolved through the node session's current scoped
...
`http://<gateway-host>:18789/__steelengine__/cap/<token>/__steelengine__/a2ui/?platform=macos`.
...
steelengine nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

Quick smoke test:

```bash
steelengine nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```

## Triggering agent runs from Canvas

Canvas can trigger new agent runs via `steelengine://agent?...` deep links:

```js
window.location.href = "steelengine://agent?message=Review%20this%20design";
```
