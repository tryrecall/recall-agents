...
`recall node install ...` and `recall node start` (or run
`recall node run ...` in the foreground), then target that browser-capable
node.
...
2. Under **Recall runs**, pick **Remote** and set:
