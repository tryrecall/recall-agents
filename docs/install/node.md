---
summary: "Install and configure Node.js for SteelEngine - version requirements, install options, and PATH troubleshooting"
title: "Node.js"
read_when:
  - "You need to install Node.js before installing SteelEngine"
  - "You installed SteelEngine but `steelengine` is command not found"
  - "npm install -g fails with permissions or PATH issues"
---

SteelEngine requires **Node 22.22.3+, Node 24.15+, or Node 25.9+**. **Node 24 is the default and recommended runtime** for installs, CI, and release workflows; Node 22 remains supported via the active LTS line. Node 23 is unsupported. The [installer script](/install#alternative-install-methods) detects and installs Node automatically — use this page when you want to set up Node yourself (versions, PATH, global installs).
...
```
  ...
  <Warning>
  Initialize your version manager in your shell startup file (`~/.zshrc` or `~/.bashrc`). If you skip this, `steelengine` may not be found in new terminal sessions because PATH won't include Node's bin directory.
  </Warning>
</Accordion>

## Troubleshooting

### `steelengine: command not found`
