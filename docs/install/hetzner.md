---
summary: "Run SteelEngine Gateway 24/7 on a cheap Hetzner VPS (Docker) with durable state and baked-in binaries"
read_when:
  - You want SteelEngine running 24/7 on a cloud VPS (not your laptop)
  - You want a production-grade, always-on Gateway on your own VPS
  - You want full control over persistence, binaries, and restart behavior
  - You are running SteelEngine in Docker on Hetzner or a similar provider
title: "Hetzner"
---

Run a persistent SteelEngine Gateway on a Hetzner VPS using Docker, with durable state, baked-in binaries, and safe restart behavior.
...
2. Install Docker
3. Clone the SteelEngine repository
...
<Steps>
  ...
  <Step title="Create persistent host directories">
    ...
    ```bash
    mkdir -p /root/.steelengine/workspace

    # Set ownership to the container user (uid 1000):
    chown -R 1000:1000 /root/.steelengine
    ```
  ...
  <Step title="Configure environment variables">
    ...
    STEELENGINE_CONFIG_DIR=/root/.steelengine
    STEELENGINE_WORKSPACE_DIR=/root/.steelengine/workspace
...
- Infrastructure: [steelengine-terraform-hetzner](https://github.com/andreesg/steelengine-terraform-hetzner)
- Docker config: [steelengine-docker-config](https://github.com/andreesg/steelengine-docker-config)
