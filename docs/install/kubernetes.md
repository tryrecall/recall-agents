---
summary: "Deploy Recall Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Recall on a Kubernetes cluster
  - You want to test Recall in a Kubernetes environment
title: "Kubernetes"
---

# Recall on Kubernetes

A minimal starting point for running Recall on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Recall is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/recall 18789:18789 -n recall
...
```bash
kubectl get secret recall-secrets -n recall -o jsonpath='{.data.RECALL_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: recall (configurable via RECALL_NAMESPACE)
├── Deployment/recall        # Single pod, init container + gateway
├── Service/recall           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/recall-config  # recall.json + AGENTS.md
└── Secret/recall-secrets    # Gateway token + API keys
```
...
Edit `recall.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret recall-secrets -n recall \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/recall -n recall
```

### Custom namespace

```bash
RECALL_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/recall/recall:latest # or pin to a specific version from https://github.com/recall/recall/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # recall.json + AGENTS.md
