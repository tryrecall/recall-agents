...
- `chmod 700 ~/.recall` to restrict credential file permissions.
- `recall security audit` for an Recall-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most Recall features work fine; a small number of native binaries need ARM builds:
