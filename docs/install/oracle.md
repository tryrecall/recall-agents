---
summary: "Host SteelEngine on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up SteelEngine on Oracle Cloud
  - Looking for free VPS hosting for SteelEngine
  - Want 24/7 SteelEngine on a small server
title: "Oracle Cloud"
---

Run a persistent SteelEngine Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname steelengine
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=steelengine
    ```

    From now on, connect via Tailscale: `ssh ubuntu@steelengine`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    steelengine config set gateway.bind loopback
    steelengine config set gateway.auth.mode token
    steelengine doctor --generate-gateway-token
    steelengine config set gateway.tailscale.mode serve
    steelengine config set gateway.trustedProxies '["127.0.0.1"]'
  ...
  <Step title="Verify">
    ...
    ```
    https://steelengine.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.steelengine` to restrict credential file permissions.
- `steelengine security audit` for an SteelEngine-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most SteelEngine features work fine; a small number of native binaries need ARM builds:
...
- `~/.steelengine/` -- `steelengine.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
- `~/.steelengine/workspace/` -- the agent workspace (SOUL.md, memory, artifacts).
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@steelengine
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=steelengine --reset` to re-authenticate.

**Gateway will not start** -- Run `steelengine doctor --non-interactive` and check logs with `journalctl --user -u steelengine-gateway.service -n 50`.
