...
Default label is `ai.recall.gateway` (or `ai.recall.<profile>`; legacy `com.recall.*` may still exist):

```bash
launchctl bootout gui/$UID/ai.recall.gateway
rm -f ~/Library/LaunchAgents/ai.recall.gateway.plist
```

If you used a profile, replace the label and plist name with `ai.recall.<profile>`. Remove any legacy `com.recall.*` plists if present.
