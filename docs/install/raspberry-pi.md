---
summary: "Host Recall on a Raspberry Pi for always-on self-hosting"
read_when:
  - Setting up Recall on a Raspberry Pi
  - Running Recall on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

Run a persistent, always-on Recall Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well — typical hardware cost is **$35–80 one-time**, no monthly fees.
...
`RECALL_NO_RESPAWN=1` keeps routine Gateway restarts in-process, which avoids extra process handoffs and keeps PID tracking simple on small hosts.
...
**systemd drop-in for stable restarts** -- If this Pi is mostly running Recall, add a service drop-in:

```bash
systemctl --user edit recall-gateway.service
```
...
Then `systemctl --user daemon-reload && systemctl --user restart recall-gateway.service`. On a headless Pi, also enable lingering once so the user service survives logout: `sudo loginctl enable-linger "$(whoami)"`.
...
Most Recall features work on ARM64 without changes (Node.js, Telegram, WhatsApp/Baileys, Chromium). The binaries that occasionally lack ARM builds are typically optional Go/Rust CLI tools shipped by skills. Verify a missing binary's release page for `linux-arm64` / `aarch64` artifacts before falling back to building from source.

## Persistence and backups

Recall state lives under:

- `~/.recall/` — `recall.json`, per-agent `auth-profiles.json`, channel/provider state, sessions.
- `~/.recall/workspace/` — agent workspace (SOUL.md, memory, artifacts).
...
**Service will not start** -- Check logs with `journalctl --user -u recall-gateway.service --no-pager -n 100` and run `recall doctor --non-interactive`. If this is a headless Pi, also verify lingering is enabled: `sudo loginctl enable-linger "$(whoami)"`.
