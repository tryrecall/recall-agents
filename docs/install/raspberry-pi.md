...
Run a persistent, always-on Recall Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well.
...
**Service will not start** -- Check logs with `journalctl -u recall --no-pager -n 100` and run `recall doctor --non-interactive`.
