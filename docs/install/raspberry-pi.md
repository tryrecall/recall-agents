---
summary: "Host SteelEngine on a Raspberry Pi for always-on self-hosting"
read_when:
  - Setting up SteelEngine on a Raspberry Pi
  - Running SteelEngine on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

Run a persistent, always-on SteelEngine Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well -- typical hardware cost is **$35-80 one-time**, no monthly fees.
...
<Steps>
  ...
  <Step title="Verify">
    ```bash
    steelengine status
    systemctl --user status steelengine-gateway.service
    journalctl --user -u steelengine-gateway.service -f
    ```
  </Step>
  ...
  <Step title="Access the Control UI">
    ...
    ```bash
    ssh user@gateway-host 'steelengine dashboard --no-open'
    ```
...
**Use a USB SSD** -- SD cards are slow and wear out. A USB SSD dramatically improves performance and survives more write cycles; use it for `STEELENGINE_STATE_DIR` if you keep the OS on SD. See the [Pi USB boot guide](https://www.raspberrypi.com/documentation/computers/raspberry-pi.html#usb-mass-storage-boot).

**Enable module compile cache** -- Speeds up repeated CLI invocations on lower-power Pi hosts. `STEELENGINE_NO_RESPAWN=1` keeps routine Gateway restarts in-process, avoiding extra process handoffs and keeping PID tracking simple on small hosts:

```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/steelengine-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
...
**systemd drop-in for stable restarts** -- If this Pi is mostly running SteelEngine, add a service drop-in:
...
Then `systemctl --user daemon-reload && systemctl --user restart steelengine-gateway.service`. On a headless Pi, also enable lingering once so the user service survives logout: `sudo loginctl enable-linger "$(whoami)"`.
...
Most SteelEngine features work on ARM64 without changes (Node.js, Telegram, WhatsApp/Baileys, Chromium). The binaries that occasionally lack ARM builds are typically optional Go/Rust CLI tools shipped by skills. Verify architecture with `uname -m` (should show `aarch64`), then check a missing binary's release page for `linux-arm64` / `aarch64` artifacts before falling back to building from source.

## Persistence and backups

SteelEngine state lives under:

- `~/.steelengine/` -- `steelengine.json`, per-agent `auth-profiles.json`, channel/provider state, sessions.
- `~/.steelengine/workspace/` -- agent workspace (SOUL.md, memory, artifacts).
...
**Service will not start** -- Check logs with `journalctl --user -u steelengine-gateway.service --no-pager -n 100` and run `steelengine doctor --non-interactive`. If this is a headless Pi, also verify lingering is enabled: `sudo loginctl enable-linger "$(whoami)"`.
...
- [Updating](/install/updating) -- keep SteelEngine up to date
