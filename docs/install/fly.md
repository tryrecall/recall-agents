...
<Steps>
  ...
  <Step title="Configure fly.toml">
    ...
    **Security note:** The default config exposes a public URL. For a hardened deployment with no public IP, see [Private Deployment](#private-deployment-hardened) or use `deploy/fly.private.toml`.
    ...
    The Recall Docker image uses `tini` as its entrypoint. Fly process commands replace Docker `CMD` without replacing `ENTRYPOINT`, so the process still runs under `tini`.
  ...
  <Step title="Set secrets">
    ...
    **Notes:**
    ...
    - Non-loopback binds (`--bind lan`) require a valid gateway auth path. This Fly.io example uses `RECALL_GATEWAY_TOKEN`, but `gateway.auth.password` or a correctly configured non-loopback `trusted-proxy` deployment also satisfy the requirement.
  ...
  <Step title="Create config file">
    ...
    **Note:** Replace `https://my-recall.fly.dev` with your real Fly app
  ...
  <Step title="Access the Gateway">
    ...
    token from `RECALL_GATEWAY_TOKEN`; if you switched to password auth, use
...
`--allow-unconfigured` only bypasses the startup guard. It does not create or repair `/data/recall.json`, so make sure your real config exists and includes `gateway.mode="local"` when you want a normal local gateway start.
