---
summary: "Install Recall - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Recall, and launches onboarding.
...
Use this when you want Recall and Node kept under a local prefix such as
`~/.recall`, without depending on a system-wide Node install:

```bash
curl -fsSL https://recall.ai/install-cli.sh | bash
```
...
`recall update --channel dev` and `recall update --channel stable`. See
...
<Tabs>
  <Tab title="npm">
    ...
    for the Recall package install. If you install manually with npm, your own
...
Or skip the link and use `pnpm recall ...` from inside the repo. See [Setup](/start/setup) for full development workflows.
...
```bash
recall --version      # confirm the CLI is available
recall doctor         # check for config issues
recall gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `recall onboard --install-daemon` or `recall gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Recall on a cloud server or VPS:
...
## Troubleshooting: `recall` not found

If the install succeeded but `recall` is not found in your terminal:
