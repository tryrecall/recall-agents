---
summary: "Host SteelEngine on a DigitalOcean Droplet"
read_when:
  - Setting up SteelEngine on DigitalOcean
  - Looking for a simple paid VPS for SteelEngine
title: "DigitalOcean"
---

Run a persistent SteelEngine Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    # Create the non-root user that will own SteelEngine state and services.
    adduser steelengine
    usermod -aG sudo steelengine
    loginctl enable-linger steelengine

    su - steelengine
    steelengine --version
    ```

    Use the root shell only for system bootstrap. Run SteelEngine commands as the non-root `steelengine` user so state lives under `/home/steelengine/.steelengine/` and the Gateway installs as that user's systemd `--user` service.
  ...
  <Step title="Access the Control UI">
    ...
    **Option C: Tailnet bind (no Serve)**
    ...
    ```bash
    steelengine config set gateway.bind tailnet
    steelengine gateway restart
    ```
...
- `~/.steelengine/` -- `steelengine.json`, channel/provider credentials, per-agent `auth-profiles.json`, and session data.
...
DigitalOcean snapshots back up the whole Droplet; `steelengine backup create` is portable across hosts.
