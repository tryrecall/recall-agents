---
summary: "ClawDock shell helpers for Docker-based SteelEngine installs"
read_when:
  - You run SteelEngine with Docker often and want shorter day-to-day commands
  - You want a helper layer for dashboard, logs, token setup, and pairing flows
title: "ClawDock"
---

ClawDock is a small shell-helper layer for Docker-based SteelEngine installs.
...
The helpers auto-detect your SteelEngine checkout on first use (checking common paths like `~/steelengine`, `~/projects/steelengine`) and cache the result in `~/.clawdock/config`. Set `CLAWDOCK_DIR` yourself if your checkout lives elsewhere.
...
| `clawdock-cli <command>`  | Run SteelEngine CLI commands in Docker           |
...
| `clawdock-config`      | Open `~/.steelengine`                      |
...
- The project `.env` next to `docker-compose.yml`: Docker-specific values like image name, ports, and `STEELENGINE_GATEWAY_TOKEN`. `clawdock-token` reads the token from here.
- `~/.steelengine/.env` (mounted into the container): env-backed secrets SteelEngine itself manages, alongside `steelengine.json` and `agents/<agentId>/agent/auth-profiles.json`.
...
Use `clawdock-show-config` to inspect `steelengine.json` and both `.env` files quickly; it redacts `.env` values in its printed output.

## Related

<CardGroup cols={2}>
  <Card title="Docker" href="/install/docker" icon="docker">
    Canonical Docker install for SteelEngine.
  </Card>
  ...
  <Card title="Updating" href="/install/updating" icon="arrow-up-right-from-square">
    Updating the SteelEngine package and managed services.
  </Card>
</CardGroup>
