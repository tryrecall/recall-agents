---
summary: "Run Recall Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Recall running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent Recall Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Recall 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
That mounted `~/.recall` state includes `recall.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-recall-project --name="Recall Gateway"
    gcloud config set project my-recall-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create recall-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh recall-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Configure environment variables">
    ...
    RECALL_CONFIG_DIR=/home/$USER/.recall
    RECALL_WORKSPACE_DIR=/home/$USER/.recall/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.recall
    ```

    Set `RECALL_GATEWAY_TOKEN` when you want to manage the stable gateway
    token through `.env`; otherwise configure `gateway.auth.token` before
    relying on clients across restarts. If neither source exists, Recall uses
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `RECALL_GATEWAY_TOKEN`.
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`RECALL_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm recall-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh recall-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop recall-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type recall-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start recall-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create recall-deploy \
     --display-name="Recall Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-recall-project \
     --member="serviceAccount:recall-deploy@my-recall-project.iam.gserviceaccount.com" \
