---
summary: "Run SteelEngine Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want SteelEngine running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent SteelEngine Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.
...
4. Clone the SteelEngine repository
...
<Steps>
  ...
  <Step title="Create a GCP project">
    ```bash
    gcloud projects create my-steelengine-project --name="SteelEngine Gateway"
    gcloud config set project my-steelengine-project
  ...
  <Step title="Create the VM">
    ...
    ```bash
    gcloud compute instances create steelengine-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    ```bash
    gcloud compute ssh steelengine-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the SteelEngine repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    STEELENGINE_IMAGE=steelengine:latest
    STEELENGINE_GATEWAY_TOKEN=
    STEELENGINE_GATEWAY_BIND=lan
    STEELENGINE_GATEWAY_PORT=18789

    STEELENGINE_CONFIG_DIR=/home/$USER/.steelengine
    STEELENGINE_WORKSPACE_DIR=/home/$USER/.steelengine/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.steelengine
    ```

    Set `STEELENGINE_GATEWAY_TOKEN` to manage the stable gateway token through
    `.env`; otherwise configure `gateway.auth.token` before relying on clients
    across restarts. If neither is set, SteelEngine uses a runtime-only token for
    ...
    **Do not commit this file.** It holds container/runtime env such as
    `STEELENGINE_GATEWAY_TOKEN`. Stored provider OAuth/API-key auth lives in the
    mounted `~/.steelengine/agents/<agentId>/agent/auth-profiles.json`.
    ...
    services:
      steelengine-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - STEELENGINE_GATEWAY_BIND=${STEELENGINE_GATEWAY_BIND}
          - STEELENGINE_GATEWAY_PORT=${STEELENGINE_GATEWAY_PORT}
        ...
        ports:
          ...
          - "127.0.0.1:${STEELENGINE_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`STEELENGINE_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm steelengine-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh steelengine-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop steelengine-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type steelengine-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start steelengine-gateway --zone=us-central1-a
```
...
```bash
gcloud iam service-accounts create steelengine-deploy \
  --display-name="SteelEngine Deployment"

gcloud projects add-iam-policy-binding my-steelengine-project \
  --member="serviceAccount:steelengine-deploy@my-steelengine-project.iam.gserviceaccount.com" \
