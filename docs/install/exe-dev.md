...
Tip: keep this VM **stateful**. Recall stores state under `~/.recall/` and `~/.recall/workspace/`.
...
token from `gateway.auth.token` on the VM (retrieve with `recall config get gateway.auth.token`, or generate one
with `recall doctor --generate-gateway-token`). Approve devices with `recall devices list` and
`recall devices approve <requestId>`. When in doubt, use Shelley from your browser!
