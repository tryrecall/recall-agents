...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/tryrecall/recall-agents)
...
your fork's `render.yaml` (but note: no persistent disk means Recall state
...
`RECALL_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
This creates a portable backup archive with Recall state plus any configured
...
regularly export a full backup via `recall backup create` in the Render shell.
