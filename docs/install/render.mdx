---
summary: "Deploy Recall on Render with Infrastructure-as-Code"
read_when:
  - Deploying Recall to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Recall on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/recall/recall)
...
repository configures everything needed to run Recall:
...
services:
  - type: web
    ...
    envVars:
      - key: RECALL_GATEWAY_PORT
        value: "8080"
      - key: RECALL_STATE_DIR
        value: /data/.recall
      - key: RECALL_WORKSPACE_DIR
        value: /data/workspace
      - key: RECALL_GATEWAY_TOKEN
...
Connect using the `RECALL_GATEWAY_TOKEN` value that was auto-generated during deploy (find it in **Dashboard → your service → Environment**).
...
If you use the original Recall repository, Render will not auto-deploy your Recall. To update it, run a manual Blueprint sync from the dashboard.
...
For Recall, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
- Missing `RECALL_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `RECALL_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export your config via `recall backup create` in the Render shell.
