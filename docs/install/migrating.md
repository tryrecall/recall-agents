...
read_when:
  - You are moving SteelEngine to a new laptop or server
  - You are coming from another agent system and want to keep state
  - You are upgrading an in-place plugin
title: "Migration guide"
---

SteelEngine supports three migration paths: importing from another agent system, moving an existing install to a new machine, and upgrading a plugin in place.

## Import from another agent system

Bundled migration providers bring instructions, MCP servers, skills, model config, and (opt-in) API keys into SteelEngine. Plans are previewed before any change, secrets are redacted in reports, and apply is backed by a verified backup.
...
The CLI entry point is [`steelengine migrate`](/cli/migrate). Onboarding can also offer migration when it detects a known source (`steelengine onboard --flow import`).

## Move SteelEngine to a new machine

Copy the **state directory** (`~/.steelengine/` by default) and your **workspace** to preserve:

- **Config** — `steelengine.json` and all gateway settings.
...
<Tip>
Run `steelengine status` on the old machine to confirm your state directory path. Custom profiles use `~/.steelengine-<profile>/` or a path set via `STEELENGINE_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf steelengine-state.tgz .steelengine
    ```

    If you use multiple profiles (for example `~/.steelengine-work`), archive each separately.

  </Step>

  <Step title="Install SteelEngine on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine. It is fine if onboarding creates a fresh `~/.steelengine/` — you overwrite it next.
  </Step>
...
```bash
awk -F= '/^(TELEGRAM_BOT_TOKEN|DISCORD_BOT_TOKEN)=/ { print $1 "=present" }' ~/.steelengine/.env
```

`steelengine doctor` also warns when an enabled default Telegram or Discord account has no configured token and the matching env variable is unavailable to the doctor process.

### Common pitfalls

<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `STEELENGINE_STATE_DIR` and the new one does not, channels will appear logged out and sessions will be empty. Launch the gateway with the **same** profile or state-dir you migrated, then rerun `steelengine doctor`.
  </Accordion>

  <Accordion title="Copying only steelengine.json">
...
- [ ] `steelengine status` shows the gateway running.
...
- [`steelengine migrate`](/cli/migrate): CLI reference for cross-system imports.
- [Install overview](/install): all installation methods.
- [Doctor](/gateway/doctor): post-migration health check.
- [Uninstall](/install/uninstall): removing SteelEngine cleanly.
