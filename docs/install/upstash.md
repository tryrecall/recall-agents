---
summary: "Host Recall on Upstash Box with keep-alive and SSH tunnel access"
read_when:
  - Deploying Recall to Upstash Box
  - You want a managed Linux environment for Recall with SSH-tunneled dashboard access
title: "Upstash Box"
---

Run a persistent Recall Gateway on Upstash Box, a managed Linux environment
...
Upstash maintains its current Recall Box walkthrough at
[Recall Setup](https://upstash.com/docs/box/guides/recall-setup).

## Connect with an SSH tunnel

Forward the Recall dashboard port to your local machine. Use your Box API key
...
```bash
recall config set gateway.bind lan
nohup recall gateway > gateway.log 2>&1 &
```
...
- [Updating Recall](/install/updating)
