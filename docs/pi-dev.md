...
This guide summarizes a sane workflow for working on the pi integration in Recall.
...
```bash
RECALL_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm recall agent --message "Hello" --thinking low`
...
State lives under the Recall state directory. Default is `~/.recall`. If `RECALL_STATE_DIR` is set, use that directory instead.

To reset everything:

- `recall.json` for config
