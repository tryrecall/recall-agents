---
summary: "Use StepFun models with Recall"
read_when:
  - You want StepFun models in Recall
  - You need StepFun setup guidance
title: "StepFun"
---

Recall includes a bundled StepFun provider plugin with two provider ids:
    ...
    **Best for:** general-purpose use via the standard StepFun endpoint.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        recall onboard --auth-choice stepfun-standard-api-key-intl
        ```

        Or for the China endpoint:

        ```bash
        recall onboard --auth-choice stepfun-standard-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        recall models list --provider stepfun
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Step Plan">
    **Best for:** Step Plan reasoning endpoint.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        recall onboard --auth-choice stepfun-plan-api-key-intl
        ```

        Or for the China endpoint:

        ```bash
        recall onboard --auth-choice stepfun-plan-api-key-cn
        ```
      </Step>
      <Step title="Non-interactive alternative">
        ```bash
      ...
      <Step title="Verify models are available">
        ```bash
        recall models list --provider stepfun-plan
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Notes">
    - The provider is bundled with Recall, so there is no separate plugin install step.
    ...
    - Use `recall models list` and `recall models set <provider/model>` to inspect or switch models.
