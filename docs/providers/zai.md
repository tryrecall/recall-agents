---
summary: "Use Z.AI (GLM models) with Recall"
read_when:
  - You want Z.AI / GLM models in Recall
...
Recall uses the `zai` provider with a Z.AI API key.
...
GLM is a model family, not a separate provider. In Recall, GLM models use
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. Recall probes supported Z.AI endpoints with your API key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        recall onboard --auth-choice zai-api-key
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
        recall models list --all --provider zai
        ```
      </Step>
    </Steps>
...
<Tip>
`zai-api-key` lets Recall detect the matching Z.AI endpoint from the key and
...
Recall ships the bundled `zai` provider catalog in the plugin manifest, so read-only
...
can change; run `recall models list --all --provider zai` to see the catalog
...
<AccordionGroup>
  ...
  <Accordion title="Thinking and preserved thinking">
    Z.AI thinking follows Recall's `/think` controls. With thinking off,
    Recall sends `thinking: { type: "disabled" }` to avoid responses that
    ...
    When enabled and thinking is on, Recall sends
  ...
  <Accordion title="Auth details">
    ...
    - The legacy env var `Z_AI_API_KEY` is still accepted; Recall copies it to `ZAI_API_KEY` at startup if `ZAI_API_KEY` is unset.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Recall config schema, including provider and model settings.
  </Card>
</CardGroup>
