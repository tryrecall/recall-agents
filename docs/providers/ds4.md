---
summary: "Run Recall through ds4, a local DeepSeek V4 Flash OpenAI-compatible server"
read_when:
  - You want to run Recall against antirez/ds4
  - You want a local DeepSeek V4 Flash backend with tool calls
  - You need the Recall config for ds4-server
title: "ds4"
---

[ds4](https://github.com/antirez/ds4) serves DeepSeek V4 Flash from a local
Metal backend with an OpenAI-compatible `/v1` API. Recall connects to ds4
through the generic `openai-completions` provider family.

ds4 is not a bundled Recall provider plugin. Configure it under
...
<Warning>
Recall agent turns include tool schemas and workspace context. A tiny context
...
<Steps>
  ...
  <Step title="Add the Recall provider config">
    ...
    recall infer model run \
      ...
      --thinking off \
      --prompt "Reply with exactly: recall-ds4-ok" \
      --json
    ```
...
aligned with `--tokens` unless you intentionally want Recall to request less
output than the server default.

## On-demand startup

Recall can start ds4 only when a `ds4/...` model is selected. Add
...
If you run that large context, update both the server flags and Recall model
metadata:
...
Then test Recall model routing:
...
<AccordionGroup>
  ...
  <Accordion title="500 prompt exceeds context">
    The configured `--ctx` is too small for the Recall turn. Raise
  ...
  <Accordion title="The first request is slow">
    ...
    `localService.readyTimeoutMs: 300000` when Recall starts the server on
    demand.
  </Accordion>
</AccordionGroup>
