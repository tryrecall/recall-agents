---
summary: "Use Mistral models and Voxtral transcription with Recall"
read_when:
  - You want to use Mistral models in Recall
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

Recall includes a bundled Mistral plugin that registers four contracts: chat completions, media understanding (Voxtral batch transcription), realtime STT for Voice Call (Voxtral Realtime), and memory embeddings (`mistral-embed`).
...
<Steps>
  ...
  <Step title="Verify the model is available">
    ```bash
    recall models list --provider mistral
    ```
  </Step>
</Steps>
...
Recall currently ships this bundled Mistral catalog:
...
```bash
recall models list --all --provider mistral --plain
```
...
<Note>
Recall defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning">
    ...
    Recall maps the session **thinking** level to Mistral's API:

    | Recall thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
    ...
    answers, turn thinking off/minimal so Recall sends
