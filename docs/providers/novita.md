---
summary: "Use NovitaAI's OpenAI-compatible API with SteelEngine"
read_when:
  - You want to run SteelEngine with NovitaAI models
...
It ships as a bundled SteelEngine provider (no separate plugin install), so
...
```bash
steelengine onboard --auth-choice novita-api-key
```
...
```bash
steelengine models list --provider novita
```
...
- `401`/`403`: verify the key in Novita's key management page and re-run
  `steelengine onboard --auth-choice novita-api-key` if the stored profile is
  stale.
