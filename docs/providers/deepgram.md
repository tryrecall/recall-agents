...
Deepgram is a speech-to-text API. In Recall it is used for inbound
...
For batch transcription, Recall uploads the complete audio file to Deepgram
and injects the transcript into the reply pipeline (`{{Transcript}}` +
`[Audio]` block). For Voice Call streaming, Recall forwards live G.711
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. Recall transcribes it
