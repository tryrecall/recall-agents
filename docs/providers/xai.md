---
summary: "Use xAI Grok models in Recall"
read_when:
  - You want to use Grok models in Recall
  - You are configuring xAI auth or model ids
title: "xAI"
---

Recall ships a bundled `xai` provider plugin for Grok models. For most
users, the recommended path is Grok OAuth with an eligible SuperGrok or X Premium
subscription. Recall stays local-first: the Gateway, config, routing, and
...
app. xAI may still show Grok Build on the consent screen because Recall uses
xAI's shared OAuth client.

## Choose your setup path

Use the path that matches your Recall install state:

<Steps>
  <Step title="New Recall install">
    ...
    ```bash
    recall onboard --install-daemon --auth-choice xai-device-code
    ```

    OAuth does not require an xAI API key. Recall does not require the Grok
    Build app. xAI may still label the consent app as Grok Build because
    Recall uses xAI's shared OAuth client.

  </Step>
  <Step title="Existing Recall install">
    If Recall is already configured, sign in to xAI only. Do not rerun full
    onboarding or reinstall the daemon just to connect Grok:

    ```bash
    recall models auth login --provider xai --method oauth
    ```
    ...
    ```bash
    recall models auth login --provider xai --device-code
    ```

    To make Grok the default model after signing in, apply it separately:

    ```bash
    recall models set xai/grok-4.3
    ```
  ...
  <Step title="API-key path">
    ...
    ```bash
    recall models auth login --provider xai --method api-key
...
<Note>
Recall uses the xAI Responses API as the bundled xAI transport. The same
credential from `recall models auth login --provider xai --method oauth`,
`recall models auth login --provider xai --device-code`, or
`recall models auth login --provider xai --method api-key` can also power first-class
...
- To inspect saved xAI auth profiles, run:
  ...
  ```bash
  recall models auth list --provider xai
...
<Tip>
Use `xai-device-code` when signing in from SSH, Docker, or a VPS. Recall
...
Recall includes the current xAI chat models out of the box, ordered newest
...
normalize to `grok-build-0.1`; Recall no longer shows the other retired
...
The bundled plugin maps xAI's current public API surface onto Recall's shared
...
| xAI capability             | Recall surface                          | Status                                                              |
| -------------------------- | ----------------------------------------- | ------------------------------------------------------------------- |
...
| Streaming TTS              | -                                         | Not exposed; Recall's TTS contract returns complete audio buffers |
| Batch speech-to-text       | `tools.media.audio` / media understanding | Yes                                                                 |
| Streaming speech-to-text   | Voice Call `streaming.provider: "xai"`    | Yes                                                                 |
| Realtime voice             | -                                         | Not exposed yet; different session/WebSocket contract               |
| Files / batches            | Generic model API compatibility only      | Not a first-class Recall tool                                     |

<Note>
Recall uses xAI's REST image/video/TTS/STT APIs for media generation,
...
code-execution tools. Features that need different Recall contracts, such as
...
<AccordionGroup>
  <Accordion title="Web search">
    ...
    ```bash
    recall models auth login --provider xai --method oauth
    recall config set tools.web.search.provider grok
    ```
  ...
  <Accordion title="Video generation">
    ...
    Recall can encode those as data URLs for xAI.
    </Warning>
  ...
  <Accordion title="Image generation">
    ...
    Recall asks xAI for `b64_json` image responses so generated media can be
    ...
    such as `1:2`, `2:1`, `9:20`, and `20:9`. Recall forwards only the
  ...
  <Accordion title="Text-to-speech">
    ...
    <Note>
    Recall uses xAI's batch `/v1/tts` endpoint. xAI also offers streaming TTS
    over WebSocket, but the Recall speech provider contract currently expects
  ...
  <Accordion title="Speech-to-text">
    The bundled `xai` plugin registers batch speech-to-text through Recall's
    ...
    transcription request. Prompt hints are accepted by the shared Recall
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as an Recall tool for searching
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as an Recall tool for
  ...
  <Accordion title="Known limits">
    - xAI auth can use an API key, environment variable, plugin config fallback,
      ...
      the consent page may show Grok Build even though Recall does not require
      the Grok Build app.
    - `grok-4.20-multi-agent-experimental-beta-0304` is not supported on the
      normal xAI provider path because it requires a different upstream API
      surface than the standard Recall xAI transport.
    - xAI Realtime voice is not registered as an Recall provider yet. It
  ...
  <Accordion title="Advanced notes">
    - Recall applies xAI-specific tool-schema and tool-call compatibility fixes
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as Recall
      tools. Recall enables the specific xAI built-in it needs inside each tool
...
RECALL_LIVE_TEST=1 RECALL_LIVE_TEST_QUIET=1 pnpm test:live -- extensions/xai/xai.live.test.ts
RECALL_LIVE_TEST=1 RECALL_LIVE_TEST_QUIET=1 RECALL_LIVE_IMAGE_GENERATION_PROVIDERS=xai pnpm test:live -- test/image-generation.runtime.live.test.ts
```
...
shared image live file verifies the same xAI provider through Recall's
