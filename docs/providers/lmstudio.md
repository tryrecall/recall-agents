---
summary: "Run Recall with LM Studio"
read_when:
  - You want to run Recall with open source models via LM Studio
...
If LM Studio authentication is disabled, you can leave the API key blank during interactive Recall setup.
...
```bash
recall models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys follow a `author/model-name` format (e.g. `qwen/qwen3.5-9b`). Recall
...
For unauthenticated LM Studio servers, omit the key; Recall stores a local non-secret marker.
...
`usage` object, Recall recovers token counts from llama.cpp-style
...
options, Recall exposes the matching OpenAI-compatible `reasoning_effort`
...
on `/v1/chat/completions`; Recall normalizes that binary discovery shape to
...
LM Studio supports just-in-time (JIT) model loading, where models are loaded on first request. Recall preloads models through LM Studio's native load endpoint by default, which helps when JIT is disabled. To let LM Studio's JIT, idle TTL, and auto-evict behavior own model lifecycle, disable Recall's preload step:
