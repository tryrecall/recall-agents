---
summary: "Use NVIDIA's OpenAI-compatible API in SteelEngine"
read_when:
  - You want to use open models in SteelEngine for free
...
[build.nvidia.com](https://build.nvidia.com/settings/api-keys). SteelEngine
...
<Steps>
  ...
  <Step title="Export the key and run onboarding">
    ...
    steelengine onboard --auth-choice nvidia-api-key
    ```
  </Step>
  <Step title="Set an NVIDIA model">
    ```bash
    steelengine models set nvidia/nvidia/nemotron-3-ultra-550b-a55b
    ```
  </Step>
</Steps>
...
```bash
steelengine onboard --auth-choice nvidia-api-key --nvidia-api-key "nvapi-..."
```
...
model-selection surfaces without waiting for an SteelEngine release. When the
...
SteelEngine falls back to the bundled catalog and bundled default below.

## Nemotron 3 Ultra

Nemotron 3 Ultra is the default NVIDIA model in SteelEngine. NVIDIA's build page for
...
<AccordionGroup>
  ...
  <Accordion title="Catalog and pricing">
    SteelEngine prefers NVIDIA's public featured-model catalog when NVIDIA auth is
  ...
  <Accordion title="OpenAI-compatible endpoint">
    SteelEngine talks to NVIDIA with the `openai-completions` adapter against the
  ...
  <Accordion title="Nemotron 3 Ultra reasoning params">
    ...
    and `reasoning_budget` for reasoning output. SteelEngine's bundled Ultra row
