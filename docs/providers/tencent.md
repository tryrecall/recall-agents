...
read_when:
  - You want to use Tencent Hy3 preview with Recall
  - You need the TokenHub API key setup
---

Tencent Cloud ships as a bundled provider plugin in Recall. It gives access to Tencent Hy3 preview through the TokenHub endpoint (`tencent-tokenhub`) using an OpenAI-compatible API.
...
```bash Onboarding
recall onboard --auth-choice tokenhub-api-key
```
...
```
  ...
  <Step title="Verify the model">
    ```bash
    recall models list --provider tencent-tokenhub
    ```
  </Step>
</Steps>
...
<Tip>
  The model id is `hy3-preview`. Do not confuse it with Tencent's `HY-3D-*` models, which are 3D generation APIs and are not the Recall chat model configured by this provider.
</Tip>
...
<AccordionGroup>
  <Accordion title="Endpoint override">
    Recall defaults to Tencent Cloud's `https://tokenhub.tencentmaas.com/v1` endpoint. Tencent also documents an international TokenHub endpoint:

    ```bash
    recall config set models.providers.tencent-tokenhub.baseUrl "https://tokenhub-intl.tencentmaas.com/v1"
    ```
  ...
  <Accordion title="Environment availability for the daemon">
    If the Gateway runs as a managed service (launchd, systemd, Docker), `TOKENHUB_API_KEY` must be visible to that process. Set it in `~/.recall/.env` or via `env.shellEnv` so launchd, systemd, or Docker exec environments can read it.
