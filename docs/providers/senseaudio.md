...
SenseAudio can transcribe inbound audio and voice-note attachments through Recall's shared `tools.media.audio` pipeline. Recall posts multipart audio to the OpenAI-compatible transcription endpoint and injects the returned text as `{{Transcript}}` plus an `[Audio]` block.
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. Recall uploads the
...
<Note>
SenseAudio is batch STT only in Recall. Voice Call realtime transcription
