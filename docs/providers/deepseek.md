...
title: "DeepSeek"
read_when:
  - You want to use DeepSeek with SteelEngine
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    steelengine onboard --auth-choice deepseek-api-key
    ```
  ...
  <Step title="Verify models are available">
    ```bash
    steelengine models list --provider deepseek
    ```

    To inspect the plugin's static catalog without a running Gateway:

    ```bash
    steelengine models list --all --provider deepseek
    ```
...
available to that process (for example, in `~/.steelengine/.env` or via
`env.shellEnv`).
</Warning>
...
SteelEngine's local cost estimates follow DeepSeek's published cache-hit,
...
<Tip>
V4 models support DeepSeek's `thinking` control. SteelEngine also replays
...
SteelEngine's DeepSeek plugin backfills that field automatically, so normal
...
When thinking is disabled (including the UI **None** selection), SteelEngine
...
```bash
STEELENGINE_LIVE_PROVIDERS=deepseek \
STEELENGINE_LIVE_MODELS="deepseek/deepseek-v4-flash,deepseek/deepseek-v4-pro" \
