...
<Steps>
  ...
  <Step title="Verify models are available">
    ```bash
    recall models list --provider deepseek
    ```
    ...
    ```bash
    recall models list --all --provider deepseek
    ```
...
<Tip>
V4 models support DeepSeek's `thinking` control. Recall also replays
...
`reasoning_content` on follow-up requests. Recall handles this inside the
...
DeepSeek `reasoning_content`. Recall fills that missing field on replayed
assistant messages for DeepSeek V4 thinking requests so the provider can accept
the history without requiring `/new`.

When thinking is disabled in Recall (including the UI **None** selection),
Recall sends DeepSeek `thinking: { type: "disabled" }` and strips replayed
...
```bash
RECALL_LIVE_PROVIDERS=deepseek \
RECALL_LIVE_MODELS="deepseek/deepseek-v4-flash,deepseek/deepseek-v4-pro" \
