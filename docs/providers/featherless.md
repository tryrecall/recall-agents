...
read_when:
  - You want to use Featherless AI with SteelEngine
...
OpenAI-compatible API. SteelEngine installs Featherless as an official external
...
| Package         | `@steelengine/featherless-provider`         |
...
```bash
steelengine onboard --auth-choice featherless-api-key
```
...
```bash
steelengine models list --provider featherless
```

## Default model

The plugin uses `Qwen/Qwen3-32B` as the setup default because Featherless
documents native tool calling for the Qwen 3 family. SteelEngine configures its
...
modes and SteelEngine does not embed account-specific plan or request-pricing
rates.
...
SteelEngine deliberately does not copy Featherless's full public model index into
...
- Managed Gateway cannot see the key: put it in `~/.steelengine/.env` or another
