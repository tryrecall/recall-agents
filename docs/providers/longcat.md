...
title: "LongCat"
read_when:
  - You want to use LongCat-2.0 with SteelEngine
  - You need the LongCat API key or model limits
---

[LongCat](https://longcat.ai) provides a hosted API for LongCat-2.0, a
reasoning model built for coding and agentic workloads. SteelEngine provides the
...
```bash
steelengine plugins install @steelengine/longcat-provider
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    steelengine onboard --auth-choice longcat-api-key
    ```
  </Step>
  <Step title="Verify the model">
    ```bash
    steelengine models list --provider longcat
    ```
  </Step>
</Steps>
...
LongCat exposes binary thinking control. SteelEngine maps enabled thinking levels
to `thinking: { type: "enabled" }` and `/think off` to
`thinking: { type: "disabled" }`. LongCat does not currently document
`reasoning_effort`, so SteelEngine does not send it.

LongCat returns reasoning in `reasoning_content`. SteelEngine preserves that field
...
model through an OpenAI-compatible runtime and use SteelEngine's existing
...
<AccordionGroup>
  <Accordion title="The key works in a shell but not in the Gateway">
    ...
    variable. Put `LONGCAT_API_KEY` in `~/.steelengine/.env`, configure it through
  ...
  <Accordion title="The model does not appear">
    Run `steelengine plugins list` and confirm the `longcat` plugin is
    enabled, then run `steelengine models list --provider longcat`.
  </Accordion>
</AccordionGroup>
