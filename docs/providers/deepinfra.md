---
summary: "Use DeepInfra's unified API to access the most popular open source and frontier models in SteelEngine"
read_when:
  - You want a single API key for the top open source LLMs
  - You want to run models via DeepInfra's API in SteelEngine
title: "DeepInfra"
---
...
```bash
steelengine onboard --deepinfra-api-key <key>
```
...
live from `https://api.deepinfra.com/v1/openai/models?sort_by=steelengine&filter=with_meta`
...
| Surface                  | Default model                                                                                         | SteelEngine config/tool                                     |
| ------------------------ | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
...
native model types. SteelEngine has no provider contract for those categories
yet, so this plugin does not register them.

## Available models

SteelEngine discovers DeepInfra models dynamically once a key is configured. Use
`/models deepinfra` or `steelengine models list --provider deepinfra` to see the
current list.
