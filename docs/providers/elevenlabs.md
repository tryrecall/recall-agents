---
summary: "Use ElevenLabs speech, Scribe STT, and realtime transcription with SteelEngine"
read_when:
  - You want ElevenLabs text-to-speech in SteelEngine
...
SteelEngine uses ElevenLabs for text-to-speech, batch speech-to-text with Scribe
...
| Capability               | SteelEngine surface                                                     | Default                  |
| ------------------------ | -------------------------------------------------------------------- | ------------------------ |
...
Set `modelId` to `eleven_v3` to use ElevenLabs v3 TTS. SteelEngine keeps
...
returned audio stream instead of waiting for SteelEngine to download the whole
audio file first. `latencyTier` maps to ElevenLabs' `optimize_streaming_latency`
query parameter for models that accept it; SteelEngine omits that parameter for
...
SteelEngine sends multipart audio to ElevenLabs `/v1/speech-to-text` with
