...
Kimi K3 always reasons at `reasoning_effort: "max"`. SteelEngine exposes only
    ...
    **Best for:** Kimi K3 and K2 models via the Moonshot Open Platform.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        steelengine onboard --auth-choice moonshot-api-key
        ```

        Or for the China endpoint:

        ```bash
        steelengine onboard --auth-choice moonshot-api-key-cn
        ```
      </Step>
      <Step title="Set Kimi K3 as the default model">
        ...
        ```bash
        steelengine models set moonshot/kimi-k3
        ```
      </Step>
      <Step title="Verify models are available">
        ```bash
        steelengine models list --provider moonshot
        ```
      </Step>
      <Step title="Run a live smoke test">
        ...
        ```bash
        STEELENGINE_CONFIG_PATH=/tmp/steelengine-kimi/steelengine.json \
        STEELENGINE_STATE_DIR=/tmp/steelengine-kimi \
  ...
  <Tab title="Kimi Coding">
    **Best for:** code-focused tasks via the Kimi Coding endpoint.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        steelengine onboard --auth-choice kimi-code-api-key
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        steelengine models list --provider kimi
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  <Accordion title="Native thinking mode">
    Moonshot API Kimi K3 always reasons at maximum effort. SteelEngine exposes only
    ...
    so SteelEngine preserves the requested `tool_choice`. For multi-turn tool use,
    SteelEngine preserves the assistant reasoning content required by Moonshot's
    replay contract.

    Kimi K2.7 Code always uses native thinking. Moonshot requires clients to
    omit the `thinking` field for this model, so SteelEngine exposes only `on` and
    ignores stale `off` settings. K2.7 also fixes `temperature`, `top_p`, `n`,
    `presence_penalty`, and `frequency_penalty`; SteelEngine omits configured
    ...
    SteelEngine maps runtime `/think` levels for those models:
    ...
    default strategy. SteelEngine only forwards `thinking.keep` for
    `moonshot/kimi-k2.6` and strips it from other models. Kimi K2.7 Code
    preserves full reasoning history by default while SteelEngine omits the entire
    `thinking` field.
  ...
  <Accordion title="Tool call id sanitization">
    Moonshot Kimi serves native tool_call ids shaped like `functions.<name>:<index>`. SteelEngine preserves the first occurrence of each native Kimi id and rewrites later duplicates to deterministic OpenAI-style `call_*` ids. Matching tool results are remapped with the same id so replay remains unique without stripping Kimi's first native id. This behavior is wired into the bundled Moonshot provider and is not a user-configurable setting.
  </Accordion>
  ...
  <Accordion title="Streaming usage compatibility">
    ...
    SteelEngine keys this off the endpoint host, not the provider id, so a custom
