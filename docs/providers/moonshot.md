    ...
    **Best for:** Kimi K2 models via the Moonshot Open Platform.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        recall onboard --auth-choice moonshot-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        recall models list --provider moonshot
        ```
      </Step>
      <Step title="Run a live smoke test">
        ...
        ```bash
        RECALL_CONFIG_PATH=/tmp/recall-kimi/recall.json \
        RECALL_STATE_DIR=/tmp/recall-kimi \
        recall agent --local \
    ...
    **Best for:** code-focused tasks via the Kimi Coding endpoint.
    ...
    <Steps>
      ...
      <Step title="Verify the model is available">
        ```bash
        recall models list --provider kimi
        ```
      </Step>
    </Steps>
...
Recall also ships **Kimi** as a `web_search` provider, backed by Moonshot web
search.
...
<AccordionGroup>
  <Accordion title="Native thinking mode">
    ...
    default strategy. Recall only forwards `thinking.keep` for
  ...
  <Accordion title="Tool call id sanitization">
    Moonshot Kimi serves tool_call ids shaped like `functions.<name>:<index>`. Recall preserves them unchanged so multi-turn tool calls keep working.
