---
summary: "Use OpenCode Zen and Go catalogs with Recall"
read_when:
...
OpenCode exposes two hosted catalogs in Recall:
...
Both catalogs use the same OpenCode API key. Recall keeps the runtime provider ids
    ...
    **Best for:** the curated OpenCode multi-model proxy (Claude, GPT, Gemini).
    ...
    <Steps>
      ...
      <Step title="Set a Zen model as the default">
        ```bash
        recall config set agents.defaults.model.primary "opencode/claude-opus-4-6"
        ```
      </Step>
      <Step title="Verify models are available">
        ```bash
        recall models list --provider opencode
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Gemini replay behavior">
    Gemini-backed OpenCode refs stay on the proxy-Gemini path, so Recall keeps
