---
summary: "Use OpenRouter's unified API to access many models in SteelEngine"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in SteelEngine
...
OpenAI-compatible, so SteelEngine talks to it over the same
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run OAuth onboarding">
        ```bash
        steelengine onboard --auth-choice openrouter-oauth
        ```

        SteelEngine opens OpenRouter's browser sign-in flow (PKCE), exchanges the
        code for an OpenRouter API key, and stores it in the default
        OpenRouter auth profile. On remote/headless hosts, SteelEngine prints the
      ...
      <Step title="(Optional) Switch to a specific model">
        ...
        ```bash
        steelengine models set openrouter/<provider>/<model>
        ```
  ...
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run API-key onboarding">
        ```bash
        steelengine onboard --auth-choice openrouter-api-key
        ```
      </Step>
      <Step title="(Optional) Switch to a specific model">
        Onboarding defaults to `openrouter/auto`. Pick a concrete model later:
...
SteelEngine sends image requests to OpenRouter's chat-completions image API with
...
SteelEngine submits text-to-video and image-to-video jobs, polls the returned
...
and also exposes `google/lyria-3-clip-preview`. SteelEngine sends `modalities:
...
SteelEngine sends OpenRouter STT requests as JSON with base64 audio under
`input_audio` (OpenRouter's STT contract), not as multipart OpenAI form
uploads.

## Fusion router

OpenRouter Fusion sends one SteelEngine model ref to several OpenRouter models in
...
`openrouter/fusion`, so the SteelEngine model ref carries both the SteelEngine
provider prefix and the upstream OpenRouter namespace:

```bash
steelengine models set openrouter/openrouter/fusion
```
...
in normal agent/chat turns to try to force Fusion: SteelEngine turns can include
...
SteelEngine adds a sanitized system-prompt note listing the configured analysis
...
login flow that issues an OpenRouter API key, so SteelEngine stores the result in
...
```bash
steelengine models auth login --provider openrouter --method oauth
steelengine models auth login --provider openrouter --method api-key
```

On verified OpenRouter requests (`https://openrouter.ai/api/v1`), SteelEngine adds
...
<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, SteelEngine
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    SteelEngine sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, SteelEngine maps the selected thinking level
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    V4's required follow-up shape. SteelEngine sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: SteelEngine keeps
  ...
  <Accordion title="Provider routing metadata">
    ...
    SteelEngine forwards that object to OpenRouter as the request `provider`
