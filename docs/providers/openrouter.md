---
summary: "Use OpenRouter's unified API to access many models in Recall"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in Recall
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    recall onboard --auth-choice openrouter-api-key
    ```
  </Step>
  <Step title="(Optional) Switch to a specific model">
    ...
    ```bash
    recall models set openrouter/<provider>/<model>
    ```
...
Recall submits text-to-video and image-to-video jobs to OpenRouter, polls
...
`google/lyria-3-clip-preview`. Recall sends `modalities: ["text",
...
Recall sends OpenRouter STT requests as JSON with base64 audio under
...
On real OpenRouter requests (`https://openrouter.ai/api/v1`), Recall also adds
...
<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, Recall
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    Recall sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Anthropic cache markers">
    ...
    OpenRouter-specific Anthropic `cache_control` markers that Recall uses for
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, Recall maps the selected thinking level to
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    required follow-up shape. Recall sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: Recall keeps
  ...
  <Accordion title="Provider routing metadata">
    ...
    Recall forwards that object to OpenRouter as the request `provider`
