...
read_when:
  - You want to use Vercel AI Gateway with Recall
...
<Tip>
Recall auto-discovers the Gateway `/v1/models` catalog, so
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    recall onboard --auth-choice ai-gateway-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    recall models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
Recall accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
...
configuration. Recall resolves the canonical form automatically.
</Tip>
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the Recall Gateway runs as a daemon (launchd/systemd), make sure
    ...
    the key in `~/.recall/.env` or via `env.shellEnv` to ensure the gateway
    process can read it.
    </Warning>
  ...
  <Accordion title="Thinking levels">
    `/think` options follow trusted upstream model prefixes when Recall knows
