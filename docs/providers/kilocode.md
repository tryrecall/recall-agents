---
summary: "Use Kilo Gateway's unified API to access many models in Recall"
title: "Kilo Gateway"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Recall
---
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    recall onboard --auth-choice kilocode-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    recall models list --provider kilocode
    ```
  </Step>
</Steps>
...
<Note>
Recall treats `kilocode/kilo/auto` as the stable default ref, but does not
...
hard-coded in Recall.
</Note>

## Built-in catalog

Recall dynamically discovers available models from the Kilo Gateway at startup. Use
...
<Tip>
At startup, Recall queries `GET https://api.kilo.ai/api/gateway/models` and merges
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path, so Recall keeps
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, Recall falls back to the bundled static catalog containing `kilocode/kilo/auto`.
    - Confirm your API key is valid and that your Kilo account has the desired models enabled.
    - When the Gateway runs as a daemon, ensure `KILOCODE_API_KEY` is available to that process (for example in `~/.recall/.env` or via `env.shellEnv`).
