---
summary: "Use Ollama Cloud directly with SteelEngine"
read_when:
...
SteelEngine registers `ollama-cloud` as its own provider id so cloud-only
...
```bash
steelengine onboard --auth-choice ollama-cloud --ollama-cloud-api-key "<key>"
```
...
- You want the same native Ollama chat API shape SteelEngine uses for local
...
SteelEngine discovers Ollama Cloud models live from the hosted catalog:

```bash
steelengine models list --provider ollama-cloud
steelengine models set ollama-cloud/kimi-k2.6
```
...
nothing, SteelEngine falls back to the bundled rows `kimi-k2.5:cloud`,
...
STEELENGINE_LIVE_OLLAMA_MODEL=kimi-k2.6 \
pnpm test:live -- extensions/ollama/ollama.live.test.ts
```

The cloud smoke runs text, native stream, and web search; set
`STEELENGINE_LIVE_OLLAMA_WEB_SEARCH=0` to skip web search. It skips embeddings by
default for `https://ollama.com` because Ollama Cloud API keys may not
authorize `/api/embed`; force them with `STEELENGINE_LIVE_OLLAMA_EMBEDDINGS=1`.
...
- Unknown model errors: run `steelengine models list --provider ollama-cloud` and
