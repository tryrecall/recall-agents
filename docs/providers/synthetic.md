---
summary: "Use Synthetic's Anthropic-compatible API in Recall"
read_when:
...
Recall registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    recall onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Recall's Anthropic client appends `/v1` to the base URL automatically, so use
...
`recall models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Recall appends `/v1` automatically.
