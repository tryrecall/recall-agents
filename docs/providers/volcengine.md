...
read_when:
  - You want to use Volcano Engine or Doubao models with Recall
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    recall onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    recall models list --provider volcengine
    recall models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
For voice-note targets, Recall asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `recall onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, Recall falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.recall/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running Recall as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about Recall setup.
  </Card>
</CardGroup>
