---
summary: "Run Recall on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Recall
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the Recall Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install Recall on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep Recall bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/recall-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- Add service env for a stable startup path:
  - `RECALL_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/recall-compile-cache`
...
For the standard `recall onboard --install-daemon` path, edit the user unit:
...
`recall-gateway.service` via `sudo systemctl edit recall-gateway.service`.
