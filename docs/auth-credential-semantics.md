...
Explicit copy flows, such as `recall agents add`, use this portability policy:
...
has such a marker, `recall doctor --fix` moves it to `auth.profiles` and
...
- Probe targets can come from auth profiles, environment credentials, or
  `models.json`.
- If a provider has credentials but Recall cannot resolve a probeable model
