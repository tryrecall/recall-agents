...
- repo: `recall/docs`
- local checkout: `~/Projects/recall-docs`

## Source of truth

- English docs are authored in `tryrecall/recall-agents`.
...
1. Edit English docs in `tryrecall/recall-agents`.
2. Push to `main`.
3. `tryrecall/recall-agents/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `recall/docs`.
...
5. `recall/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `recall/docs`.
...
- Do not treat missing `th` or `fa` entries in generated `docs/docs.json` as a pipeline failure. Verify their generated folders in `recall/docs` instead.
...
- Sync metadata is written to `.recall-sync/source.json` in the publish repo.
- Source repo secret: `RECALL_DOCS_SYNC_TOKEN`
- Publish repo secret: `RECALL_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `recall/docs` first.
