...
beta publish, pass `release_package_spec=steelengine@YYYY.M.PATCH-beta.N` to reuse
...
`release_package_spec` values derive `codex_plugin_spec=npm:@steelengine/codex@<version>`;
...
image artifact before dispatching Plugin Prerelease and SteelEngine Release Checks.
...
`STEELENGINE_EXTENSIONS=diagnostics-otel,codex`. It runs in parallel with the
...
| Product performance     | **Job:** `Run product performance evidence`<br />**Child workflow:** `SteelEngine Performance`<br />**Proves:** release-profile performance run (`profile=release`, `repeat=3`, `fail_on_regression=true`, `publish_reports=false`) against the target SHA. Kova output stays in workflow artifacts and the child must prove its report publisher was skipped. Required (blocking) only for `rerun_group=all` or `rerun_group=performance`; not required for narrower rerun groups.<br />**Rerun:** `rerun_group=performance`. |
...
`SteelEngine Performance` permits report publication only for scheduled runs or a
...
`SteelEngine Release Checks` is the largest child workflow. It resolves the target
...
| QA runtime parity        | **Job:** `Run QA Lab runtime parity lane`<br />**Backing workflow:** direct job<br />**Tests:** an `steelengine`/`codex` runtime-pair agentic parity lane (`pnpm steelengine qa suite --runtime-pair steelengine,codex`), including a standard tier and, with `run_release_soak=true`, a soak tier. Advisory: individual failures do not block the release-check verifier.<br />**Rerun:** `rerun_group=qa-parity` or `rerun_group=qa`.                                                                                                                                                    |
| QA runtime tool coverage | **Job:** `Enforce QA Lab runtime tool coverage`<br />**Backing workflow:** direct job<br />**Tests:** dynamic tool drift between `steelengine` and `codex` in the standard runtime-parity tier (`pnpm steelengine qa coverage --tools`), using the QA runtime parity lane's output. Blocking: this job is not advisory-overridable.<br />**Rerun:** `rerun_group=qa-parity` or `rerun_group=qa`.                                                                                                                                                                                        |
...
| QA live Telegram         | **Job:** `Run QA Lab live Telegram lane`<br />**Backing workflow:** trusted `SteelEngine Release Telegram QA` dispatch<br />**Tests:** live Telegram QA with Convex CI credential leases.<br />**Rerun:** `rerun_group=qa-live` or `rerun_group=qa`.                                                                                                                                                                                                                                                                                                                                 |
| QA live Discord          | **Job:** `Run QA Lab live Discord lane`<br />**Backing workflow:** direct advisory job<br />**Tests:** live Discord QA with Convex CI credential leases when `STEELENGINE_RELEASE_QA_DISCORD_LIVE_CI_ENABLED` is enabled.<br />**Rerun:** `rerun_group=qa-live` with `live_suite_filter=qa-live-discord`.                                                                                                                                                                                                                                                                            |
| QA live WhatsApp         | **Job:** `Run QA Lab live WhatsApp lane`<br />**Backing workflow:** direct advisory job<br />**Tests:** live WhatsApp QA with Convex CI credential leases when `STEELENGINE_RELEASE_QA_WHATSAPP_LIVE_CI_ENABLED` is enabled.<br />**Rerun:** `rerun_group=qa-live` with `live_suite_filter=qa-live-whatsapp`.                                                                                                                                                                                                                                                                        |
| QA live Slack            | **Job:** `Run QA Lab live Slack lane`<br />**Backing workflow:** direct advisory job<br />**Tests:** live Slack QA with Convex CI credential leases when `STEELENGINE_RELEASE_QA_SLACK_LIVE_CI_ENABLED` is enabled.<br />**Rerun:** `rerun_group=qa-live` with `live_suite_filter=qa-live-slack`.                                                                                                                                                                                                                                                                                    |
...
| `release-checks`    | All SteelEngine Release Checks stages.                                                             |
...
WhatsApp, or Slack, the matching `STEELENGINE_RELEASE_QA_*_LIVE_CI_ENABLED` repo
...
- `release-package-under-test` from `SteelEngine Release Checks`
