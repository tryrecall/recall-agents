...
- In scope: strictly user-supplied credentials that Recall does not mint or rotate.
- Out of scope: runtime-minted or rotating credentials, OAuth refresh material, and session-like artifacts.

## Supported credentials

### `recall.json` targets (`secrets configure` + `secrets apply` + `secrets audit`)
...
- In `recall.json`, SecretRefs must use structured objects such as `{"source":"env","provider":"default","id":"DISCORD_BOT_TOKEN"}`. Legacy `secretref-env:<ENV_VAR>` marker strings are rejected on SecretRef credential paths; run `recall doctor --fix` to migrate valid markers.
