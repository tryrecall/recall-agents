---
summary: "How Recall builds prompt context and reports token usage + costs"
read_when:
...
Recall tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Recall assembles its own system prompt on every run. It includes:
...
For images, Recall downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Recall session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `recall status --usage` and `recall channels list` show
...
available; otherwise Recall falls back to matching OAuth/API-key credentials
...
Recall keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, Recall shows tokens only. OAuth tokens
never show dollar cost.

After sidecars and channels reach the Gateway ready path, Recall starts an
...
Provider prompt caching only applies within the cache TTL window. Recall can
...
Anthropic's 1M context window is currently beta-gated. Recall can inject the
...
Recall skips the `context-1m-*` beta header because Anthropic currently
