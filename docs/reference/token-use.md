---
summary: "How SteelEngine builds prompt context and reports token usage + costs"
read_when:
...
SteelEngine tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

SteelEngine assembles its own system prompt on every run. It includes:
...
- Workspace + bootstrap files (`AGENTS.md`, `SOUL.md`, `TOOLS.md`,
  ...
  - Lowercase root `memory.md` is never injected. It is legacy repair input
    for `steelengine doctor --fix`, which migrates it into `MEMORY.md`.
...
`toolResultMaxChars` is unset by default, so SteelEngine derives the live
...
For images, SteelEngine downscales transcript/tool image payloads before
...
- `/usage off|tokens|full` -> appends a per-response usage footer to every
  ...
  - `/usage full` shows compact model/context/cost details; estimated cost
    appears only when SteelEngine has usage metadata and local pricing for the
    active model. Custom `messages.usageTemplate` layouts can include
    token/cache fields.
- `/usage cost` -> local cost summary from SteelEngine session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` and `/usage` are supported.
- **CLI:** `steelengine status --usage` and `steelengine channels list` show
...
SteelEngine falls back to matching OAuth/API-key credentials from auth
...
SteelEngine keeps provider usage accounting separate from the current context
...
After sidecars and channels reach the Gateway ready path, SteelEngine starts an
...
Provider prompt caching only applies within the cache TTL window. SteelEngine
...
SteelEngine sizes GA-capable Claude 4.x models such as Opus 4.8, Opus 4.7, Opus
...
Older configs can keep `context1m: true`, but SteelEngine no longer sends
...
(`sk-ant-oat-*`), SteelEngine preserves the OAuth-required Anthropic beta
