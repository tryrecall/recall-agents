...
<Steps>
  <Step title="Existing config detection">
    - If `~/.recall/recall.json` exists, choose **Keep / Modify / Reset**.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.recall/credentials/oauth.json`; auth profiles live in `~/.recall/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.recall/credentials/oauth.json` (or `$RECALL_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  ...
  <Step title="Health check">
    ...
    - Tip: `recall status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
Typical fields in `~/.recall/recall.json`:
