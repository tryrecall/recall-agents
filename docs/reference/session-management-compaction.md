...
Recall manages sessions end-to-end across these areas:
...
Recall is designed around a single **Gateway process** that owns session state.
...
Recall persists sessions in two layers:
...
- Transcripts: `~/.recall/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Recall resolves these via `src/config/sessions.ts`.
...
Recall no longer creates automatic `sessions.json.bak.*` rotation backups during Gateway writes. The legacy `session.maintenance.rotateBytes` key is ignored and `recall doctor --fix` removes it from older configs.
...
`RECALL_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS`, `RECALL_SESSION_WRITE_LOCK_STALE_MS`, and
`RECALL_SESSION_WRITE_LOCK_MAX_HOLD_MS`.
...
In `mode: "warn"`, Recall reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
recall sessions cleanup --dry-run
recall sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.recall/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
- **Parent fork policy** uses PI's active branch when creating a thread or subagent fork. If that branch is too large, Recall starts the child with isolated context instead of failing or inheriting unusable history. The sizing policy is automatic; legacy `session.parentForkMaxTokens` config is removed by `recall doctor --fix`.
...
Recall intentionally does **not** "fix up" transcripts; the Gateway uses `SessionManager` to read/write them.
...
When Recall splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Recall
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Recall preserves that pending tool block and keeps the unsummarized tail
  intact.
...
exceeded`, and similar provider-shaped variants) → compact → retry.
   If overflow recovery still fails, Recall surfaces explicit guidance to the
...
These are Pi runtime semantics (Recall consumes the events, but Pi decides when to compact).

Recall can also trigger a preflight local compaction before opening the next
...
reopen cost, not raw archival: Recall still runs normal semantic compaction,
...
next model call, Recall estimates the prompt pressure using the same preflight
...
Recall also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Recall bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it's already higher, Recall leaves it alone.
...
- When `agents.defaults.compaction.truncateAfterCompaction` is enabled,
  Recall rotates the active transcript to a compacted successor JSONL after
...
- If the provider fails or returns an empty result, Recall falls back to built-in LLM summarization automatically.
...
- `recall status` (CLI)
- `recall sessions` / `sessions --json`
- Gateway logs (`pnpm gateway:watch` or `recall logs --follow`): `embedded run auto-compaction start` + `complete`
...
Recall supports "silent" turns for background tasks where the user should not see intermediate output.
...
- Recall strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, Recall also suppresses **draft/typing streaming** when a
...
Recall uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Recall's
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `recall status`.
