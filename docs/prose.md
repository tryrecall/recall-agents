...
summary: "OpenProse is a markdown-first workflow format for multi-agent AI sessions. In SteelEngine it ships as a plugin with a /prose slash command and a skill pack."
read_when:
  - You want to run or write .prose workflow files
  - You want to enable the OpenProse plugin
  - You need to understand how OpenProse maps to SteelEngine primitives
---

OpenProse is a portable, markdown-first workflow format for orchestrating AI
sessions. In SteelEngine it ships as a plugin that installs an OpenProse skill
...
<Steps>
  <Step title="Enable the plugin">
    ...
    ```bash
    steelengine plugins enable open-prose
    ```
  ...
  <Step title="Verify">
    ```bash
    steelengine plugins list | grep prose
    ```
...
`steelengine plugins install ./extensions/open-prose`
...
## SteelEngine runtime mapping

OpenProse programs map to SteelEngine primitives:

| OpenProse concept         | SteelEngine tool                                   |
| ------------------------- | ----------------------------------------------- |
...
executed. Use SteelEngine tool allowlists and approval gates to control side
...
<CardGroup cols={2}>
  ...
  <Card title="Subagents" href="/tools/subagents" icon="users">
    SteelEngine's native multi-agent coordination layer.
  </Card>
