...
read_when:
  - You want SteelEngine to receive DMs via Nostr
  - You're setting up decentralized messaging
title: "Nostr"
---

Nostr is a downloadable channel plugin (`@steelengine/nostr`) that lets SteelEngine receive and answer NIP-04 encrypted direct messages over Nostr relays. One account per gateway; DMs only.
...
```bash
steelengine plugins install --link <path-to-local-nostr-plugin>
```

Restart the gateway after installing or enabling plugins. Onboarding (`steelengine onboard`) and `steelengine channels add` surface Nostr from the shared channel catalog once the plugin is installed.

### Non-interactive setup

```bash
steelengine channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
steelengine channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "steelengine@example.com",
        lud16: "steelengine@example.com",
      },
    },
  },
}
...
1. Note the bot pubkey from gateway logs or `steelengine channels status` (hex; convert to npub in your client if needed).
