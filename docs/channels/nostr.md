...
read_when:
  - You want Recall to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Recall to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Recall releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`recall onboard`) and `recall channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install the npm package directly.

```bash
recall plugins install @recall/nostr
```
...
```bash
recall plugins install --link <path-to-local-nostr-plugin>
```
...
recall channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "recall@example.com",
        lud16: "recall@example.com",
      },
    },
  },
}
