---
summary: "Zalo ClawBot channel setup through the external steelengine-zaloclawbot plugin"
read_when:
  - You want a personal Zalo assistant bot with QR-code login
  - You are installing or troubleshooting the steelengine-zaloclawbot channel plugin
title: "Zalo ClawBot"
---

SteelEngine connects to Zalo ClawBot through the catalog-listed external `@zalo-platforms/steelengine-zaloclawbot` plugin. Login uses a Zalo Mini App QR code; the plugin id in config is `steelengine-zaloclawbot`.

## Compatibility

| Plugin Version | SteelEngine Version | npm dist-tag | Status        |
| -------------- | ---------------- | ------------ | ------------- |
...
- Node.js >= 22
- [SteelEngine](https://docs.steelengine.ai/install) installed (`steelengine` CLI available)
...
```bash
steelengine plugins install "@zalo-platforms/steelengine-zaloclawbot@0.1.4"
```

Use the exact pinned version so SteelEngine verifies the package against the catalog integrity hash during install.

### 2. Enable the plugin in config

```bash
steelengine config set plugins.entries.steelengine-zaloclawbot.enabled true
```

### 3. Generate a QR code and log in

```bash
steelengine channels login --channel steelengine-zaloclawbot
```
...
The plugin manages bot credentials under the SteelEngine state directory. Treat that directory as sensitive and cover it under the same access-control and backup policy as the rest of SteelEngine state.

This plugin's runtime lives entirely in the external `@zalo-platforms/steelengine-zaloclawbot` package; behavior details below beyond install/config are as reported by the plugin's maintainers and are not verified against SteelEngine core source.
...
- **Gateway fails to load:** confirm your SteelEngine host version is `2026.4.10` or higher. Older versions do not support the external npm-plugin installation ledger this ID requires.
