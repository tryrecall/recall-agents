...
```bash
recall plugins install ./extensions/mattermost
```
...
4. Configure Recall and start the gateway.
...
Native slash commands are opt-in. When enabled, Recall registers `oc_*` slash commands via
...
When Recall sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Recall derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Recall run on the same host/network namespace.
...
`recall message send` and cron/webhook deliveries.
