---
summary: "Mattermost bot setup and Recall config"
read_when:
...
<Tabs>
  <Tab title="npm registry">
    ```bash
    recall plugins install @recall/mattermost
    ```
  </Tab>
  <Tab title="Local checkout">
    ```bash
    recall plugins install ./path/to/local/mattermost-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Ensure plugin is available">
    Current packaged Recall releases already bundle it. Older/custom installs can add it manually with the commands above.
  </Step>
  ...
  <Step title="Configure Recall and start the gateway">
    Minimal config:
...
Native slash commands are opt-in. When enabled, Recall registers `oc_*` slash commands via the Mattermost API and receives callback POSTs on the gateway HTTP server.
...
<AccordionGroup>
  <Accordion title="Behavior notes">
    ...
    - If `callbackUrl` is omitted, Recall derives one from gateway host/port + `callbackPath`.
    ...
    - Command callbacks are validated with the per-command tokens returned by Mattermost when Recall registers `oc_*` commands.
    - Recall refreshes current Mattermost command registration before accepting each callback so stale tokens from deleted or regenerated slash commands stop being accepted without a gateway restart.
  ...
  <Accordion title="Reachability requirement">
    ...
    - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Recall.
    - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Recall.
    - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Recall, not `404`.
...
- Approve via:
  - `recall pairing list mattermost`
  - `recall pairing approve mattermost <CODE>`
...
Use these target formats with `recall message send` or cron/webhooks:
...
Recall resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Recall sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Recall sends to a Mattermost DM target and needs to resolve the direct channel first, it retries transient direct-channel creation failures by default.
...
<AccordionGroup>
  ...
  <Accordion title="Streaming behavior notes">
    - If the stream cannot be finalized in place (for example the post was deleted mid-stream), Recall falls back to sending a fresh final post so the reply is never lost.
...
Normal agent replies can also include semantic `presentation` payloads. Recall renders value buttons as Mattermost interactive buttons, keeps URL buttons visible in the message text, and downgrades select menus to readable text.
...
<AccordionGroup>
  ...
  <Accordion title="Config and reachability">
    ...
    - If `interactions.callbackBaseUrl` is omitted, Recall derives the callback URL from `gateway.customBindHost` + `gateway.port`, then falls back to `http://localhost:<port>`.
    - Reachability rule: the button callback URL must be reachable from the Mattermost server. `localhost` only works when Mattermost and Recall run on the same host/network namespace.
...
<Steps>
  <Step title="Derive the secret from the bot token">
    `HMAC-SHA256(key="recall-mattermost-interactions", data=botToken)`
  </Step>
...
secret = hmac.new(
    b"recall-mattermost-interactions",
...
The Mattermost plugin includes a directory adapter that resolves channel and user names via the Mattermost API. This enables `#channel-name` and `@username` targets in `recall message send` and cron/webhook deliveries.
...
<AccordionGroup>
  ...
  <Accordion title="Native slash commands fail">
    - `Unauthorized: invalid command token.`: Recall did not accept the callback token. Typical causes:
    ...
    - If `callbackUrl` is omitted and logs warn that the callback resolved to `http://127.0.0.1:18789/...`, that URL is probably only reachable when Mattermost runs on the same host/network namespace as Recall. Set an explicit externally reachable `commands.callbackUrl` instead.
