---
summary: "Mattermost bot setup and SteelEngine config"
read_when:
...
<Tabs>
  ...
  <Tab title="Local checkout">
    ```bash
    steelengine plugins install ./path/to/local/mattermost-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Ensure plugin is available">
    Install `@steelengine/mattermost` with the command above, then restart the Gateway if it is already running.
  </Step>
  ...
  <Step title="Configure SteelEngine and start the gateway">
    Minimal config:
    ...
    ```bash
    steelengine channels add --channel mattermost --bot-token <token> --http-url https://chat.example.com
    ```
...
Native slash commands are opt-in. When enabled, SteelEngine registers `oc_*` slash commands on every team the bot is a member of and receives callback POSTs on the gateway HTTP server.
...
<AccordionGroup>
  <Accordion title="Behavior notes">
    ...
    - If `callbackUrl` is omitted, SteelEngine derives `http://<gateway.customBindHost or localhost>:<gateway.port, default 18789><callbackPath>`. Wildcard bind hosts (`0.0.0.0`, `::`) fall back to `localhost`.
    ...
    - Command callbacks are validated with the per-command tokens returned by Mattermost when SteelEngine registers `oc_*` commands.
    - SteelEngine refreshes current Mattermost command registration before accepting each callback, so stale tokens from deleted or regenerated slash commands stop being accepted without a gateway restart.
  ...
  <Accordion title="Reachability requirement">
    ...
    - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as SteelEngine.
    - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to SteelEngine.
    - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from SteelEngine, not `404`.
...
- Approve via:
  - `steelengine pairing list mattermost`
  - `steelengine pairing approve mattermost <CODE>`
...
Use these target formats with `steelengine message send` or cron/webhooks:
...
SteelEngine resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), SteelEngine sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When SteelEngine sends to a Mattermost DM target and needs to resolve the direct channel first, it retries transient direct-channel creation failures by default.
...
Preview streaming is **on by default** in `partial` mode. Configure via `channels.mattermost.streaming.mode` (legacy scalar/boolean `streaming` values are migrated by `steelengine doctor --fix`):
...
<AccordionGroup>
  ...
  <Accordion title="Streaming behavior notes">
    - If the stream cannot be finalized in place (for example the post was deleted mid-stream), SteelEngine falls back to sending a fresh final post so the reply is never lost.
...
Buttons come from the semantic `presentation` payload (in normal agent replies and in `message action=send`). SteelEngine renders value buttons as Mattermost interactive buttons, keeps URL buttons visible in the message text, and downgrades select menus to readable text.
...
<AccordionGroup>
  ...
  <Accordion title="Config and reachability">
    ...
    - If `interactions.callbackBaseUrl` is omitted, SteelEngine derives the callback URL from `gateway.customBindHost` + `gateway.port` (default 18789), then falls back to `http://localhost:<port>`. The callback path is `/mattermost/interactions/<accountId>`.
    - Reachability rule: the button callback URL must be reachable from the Mattermost server. `localhost` only works when Mattermost and SteelEngine run on the same host/network namespace.
...
External scripts and webhooks can post buttons directly via the Mattermost REST API instead of going through the agent's `message` tool. Prefer SteelEngine's `message` tool. For direct integrations, import `buildButtonAttachments` from `@steelengine/mattermost/api.js`; if posting raw JSON, follow these rules:
...
<Steps>
  <Step title="Derive the secret from the bot token">
    `HMAC-SHA256(key="steelengine-mattermost-interactions", data=botToken)`, hex-encoded.
  </Step>
...
secret = hmac.new(
    b"steelengine-mattermost-interactions",
...
The Mattermost plugin includes a directory adapter that resolves channel and user names via the Mattermost API. This enables `#channel-name` and `@username` targets in `steelengine message send` and cron/webhook deliveries.
...
<AccordionGroup>
  ...
  <Accordion title="Native slash commands fail">
    - `Unauthorized: invalid command token.`: SteelEngine did not accept the callback token. Typical causes:
    ...
    - If `callbackUrl` is omitted and logs warn that the callback resolved to a loopback URL like `http://localhost:18789/...`, that URL is probably only reachable when Mattermost runs on the same host/network namespace as SteelEngine. Set an explicit externally reachable `commands.callbackUrl` instead.
