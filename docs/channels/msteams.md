...
```bash
recall plugins install ./extensions/msteams
```

If you choose Teams during setup and a git checkout is detected,
Recall will offer the local install path automatically.
...
3. Configure Recall with those credentials.
...
5. Configure `msteams` in `~/.recall/recall.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Recall, you need to create an Azure Bot resource.
...
1. **Install the Microsoft Teams plugin**
   - From npm: `recall plugins install @recall/msteams`
   - From a local checkout: `recall plugins install ./extensions/msteams`
