...
Microsoft Teams ships as a bundled plugin in current Recall releases, so no
...
```bash
recall plugins install @recall/msteams
```
...
```bash
recall plugins install ./path/to/local/msteams-plugin
```
...
devtunnel create my-recall-bot --allow-anonymous
devtunnel port create my-recall-bot -p 3978 --protocol auto

# Each dev session:
devtunnel host my-recall-bot
...
**4. Configure Recall** using the credentials from the output:
...
- Talk to Recall via Teams DMs, group chats, or channels.
...
- Do not rely on UPN/display-name matching for allowlists - they can change. Recall disables direct name matching by default; opt in explicitly with `channels.msteams.dangerouslyAllowNameMatching: true`.
...
- On startup, Recall resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.tryrecall/recall-agents.json` (or env vars) and start the gateway.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `recall-msteams` (must be unique) |
...
For production deployments, Recall supports **federated authentication** as a more secure alternative to client secrets. Two methods are available:
...
3. At runtime, Recall uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint (`169.254.169.254`).
...
**Default behavior:** When `authType` is not set, Recall defaults to client secret authentication. Existing configurations continue to work without changes.
...
Recall exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "Recall in Teams", full: "Recall in Teams" },
...
Recall handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Recall only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/RecallShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Recall sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `recall message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.recall/msteams-polls.json`.
- The gateway must stay online to record votes.
- Polls do not auto-post result summaries yet (inspect the store file if needed).

## Presentation cards

Send semantic presentation payloads to Teams users or conversations using the `message` tool, CLI, or normal reply delivery. Recall renders them as Teams Adaptive Cards from the generic presentation contract.

The `presentation` parameter accepts semantic blocks. When `presentation` is provided, the message text is optional. Buttons render as Adaptive Card submit or URL actions. Select menus are not native in the Teams renderer yet, so Recall downgrades them to readable text before delivery.
...
```bash
recall message send --channel msteams \
...
recall message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
recall message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
recall message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send a presentation card to a conversation
recall message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --presentation '{"title":"Hello","blocks":[{"type":"text","text":"Hello"}]}'
```
...
- **Ignore** the `groupId` query parameter for Recall routing. It is the Microsoft Entra group ID, not the Bot Framework conversation ID used in incoming Teams activities.
