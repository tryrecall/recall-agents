...
Microsoft Teams ships as a bundled plugin in current SteelEngine releases; no separate install is required in the normal packaged build.

On an older build or a custom install that excludes bundled Teams, install the npm package directly:

```bash
steelengine plugins install @steelengine/msteams
```
...
```bash
steelengine plugins install ./path/to/local/msteams-plugin
```
...
devtunnel create my-steelengine-bot --allow-anonymous
devtunnel port create my-steelengine-bot -p 3978 --protocol auto

# Each dev session:
devtunnel host my-steelengine-bot
...
**4. Configure SteelEngine** using the credentials from the output:
...
- Talk to SteelEngine via Teams DMs, group chats, or channels.
...
- Do not rely on UPN/display-name matching for allowlists; they can change. SteelEngine disables direct name matching by default; opt in with `channels.msteams.dangerouslyAllowNameMatching: true`.
...
- On startup, SteelEngine resolves team/channel and user allowlist names to IDs (when Graph permissions allow) and logs the mapping. Unresolved names are kept as typed but ignored for routing unless `channels.msteams.dangerouslyAllowNameMatching: true` is set.
...
5. Configure `msteams` in `~/.steelengine/steelengine.json` (or env vars) and start the gateway.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `steelengine-msteams` (must be unique) |
...
For production, SteelEngine supports **federated authentication** as an alternative to client secrets, via `channels.msteams.authType: "federated"`. Two methods:
...
3. At runtime, SteelEngine uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint.
...
**Default:** when `authType` is unset, SteelEngine uses client-secret authentication (`appPassword`). Existing configs keep working unchanged.
...
These auth-related config keys can be set via environment variables instead of `steelengine.json` (other config keys, such as `groupPolicy` or `historyLimit`, are config-only):
...
SteelEngine exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve verified roster details for a configured conversation.
...
```json5
{
  ...
  description: { short: "SteelEngine in Teams", full: "SteelEngine in Teams" },
...
| Teams environment | SteelEngine config                                             | Proactive `serviceUrl`                             |
| ----------------- | ----------------------------------------------------------- | -------------------------------------------------- |
...
When `replyStyle: "thread"` is in effect and the bot was @mentioned from inside a channel thread, SteelEngine re-attaches the original thread root to the outbound conversation reference (`19:...@thread.tacv2;messageid=<root>`) so the reply lands inside the same thread. This holds for both live (in-turn) sends and proactive sends made after the Bot Framework turn context has expired (e.g., long-running agents, queued tool-call replies via `mcp__steelengine__message`).
...
By default, SteelEngine only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Per-user sharing is more secure since only chat participants can access the file. SteelEngine requires a successful member lookup for group chats; timeouts, transport failures, empty results, and Graph API denials fail the send instead of widening access to the organization.
...
Uploaded files are stored in a `/SteelEngineShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

SteelEngine sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `steelengine message poll --channel msteams --target conversation:<id> --poll-question "..." --poll-option "..." --poll-option "..."`.
- Votes are recorded by the gateway in SteelEngine plugin-state SQLite under `state/steelengine.sqlite`.
- Existing `msteams-polls.json` files are imported by `steelengine doctor --fix`, not by the running plugin.
- The gateway must stay online to record votes.
- Polls do not auto-post result summaries, and there is no poll-results CLI yet.

## Presentation cards

Send semantic presentation payloads to Teams users or conversations using the `message` tool, CLI, or normal reply delivery. SteelEngine renders them as Teams Adaptive Cards from the generic presentation contract.

The `presentation` parameter accepts semantic blocks. When `presentation` is provided, the message text is optional. Buttons render as Adaptive Card submit or URL actions. Select menus are not native in the Teams renderer, so SteelEngine downgrades them to readable text before delivery.
...
steelengine message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
steelengine message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
steelengine message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send a presentation card to a conversation
steelengine message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --presentation '{"title":"Hello","blocks":[{"type":"text","text":"Hello"}]}'
```
...
- Proactive messages are only possible **after** a user has interacted, because SteelEngine stores conversation references at that point.
...
- **Ignore** the `groupId` query parameter for SteelEngine routing. It is the Microsoft Entra group ID, not the Bot Framework conversation ID used in incoming Teams activities.
