...
read_when:
  ...
  - Reviewing Recall security posture
title: "Pairing"
---

"Pairing" is Recall's explicit access approval step.
...
Supported channels: `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `recall-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.
...
Stored under `~/.recall/credentials/`:
...
3. On your phone, open the Recall iOS app → Settings → Gateway.
...
<Note>
An already paired device does not get broader access silently. If it reconnects asking for more scopes or a broader role, Recall keeps the existing approval as-is and creates a fresh pending upgrade request. Use `recall devices list` to compare the currently approved access with the newly requested access before you approve.
</Note>
...
- The legacy `node.pair.*` API (CLI: `recall nodes pending|approve|reject|remove|rename`) is a
