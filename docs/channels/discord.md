...
recall config set channels.discord.enabled true --strict-json
recall gateway
```

    If Recall is already running as a background service, use `recall gateway restart` instead.
...
<AccordionGroup>
  ...
  <Accordion title="Copy IDs">
    ...
    Prefer numeric IDs in Recall config for reliable audits and probes.
...
<AccordionGroup>
  ...
  <Accordion title="Live stream preview">
    Recall can stream draft replies by sending a temporary message and editing it as text arrives.
...
```
    ...
    enabled for Discord, Recall skips the preview stream to avoid double streaming.
...
```
  ...
  <Accordion title="Exec approvals in Discord">
    ...
    - env-first local auth (`RECALL_GATEWAY_TOKEN` / `RECALL_GATEWAY_PASSWORD` then `gateway.auth.*`)
...
Recall uses Discord components v2 for exec approvals and cross-context markers. Discord message actions can also accept `components` for custom UI (advanced; requires Carbon component instances), while legacy `embeds` remain available but are not recommended.
...
Recall can join Discord voice channels for realtime, continuous conversations. This is separate from voice message attachments.
...
Discord voice messages show a waveform preview and require OGG/Opus audio plus metadata. Recall generates the waveform automatically, but it needs `ffmpeg` and `ffprobe` available on the gateway host to inspect and convert audio files.

Requirements and constraints:

- Provide a **local file path** (URLs are rejected).
- Omit text content (Discord does not allow text + voice message in the same payload).
- Any audio format is accepted; Recall converts to OGG/Opus when needed.
