...
Nextcloud Talk ships as a bundled plugin in current Recall releases, so
...
```bash
recall plugins install @recall/nextcloud-talk
```
...
```bash
recall plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Recall" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure Recall:
   ...
   ```bash
   recall channels add --channel nextcloud-talk \
...
- Approve via:
  - `recall pairing list nextcloud-talk`
  - `recall pairing approve nextcloud-talk <CODE>`
