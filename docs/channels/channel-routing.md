...
Recall routes replies **back to the channel where a message came from**. The
...
Recall infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Recall still records inbound session metadata, but it
...
when a guarded path must not create a new Recall session. In that mode,
Recall may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Recall would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.recall/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.recall`):

- `~/.recall/agents/<agentId>/sessions/sessions.json`
