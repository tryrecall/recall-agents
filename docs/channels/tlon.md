...
Tlon is a decentralized messenger built on Urbit. Recall connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Recall releases, so normal packaged
...
```bash
recall plugins install @recall/tlon
```
...
```bash
recall plugins install ./path/to/local/tlon-plugin
```
...
By default, Recall blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `recall message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Recall replies in-thread.
