...
Tlon is a decentralized messenger built on Urbit. SteelEngine connects to your Urbit ship and
...
Tlon ships bundled in current SteelEngine releases; packaged builds do not need a separate install.

On an older build or custom install that excludes it, install from npm:

```bash
steelengine plugins install @steelengine/tlon
```

Use the bare package name to track the current release tag. Pin a version (`@steelengine/tlon@x.y.z`)
only for reproducible installs.

From a local checkout:

```bash
steelengine plugins install ./path/to/local/tlon-plugin
```
...
```bash
steelengine channels add --channel tlon --ship ~sampel-palnet --url https://your-ship-host --code lidlut-tabwed-pillex-ridrup
```
...
SteelEngine blocks private/internal hostnames and IP ranges for SSRF protection by default. If your
...
<Note>
`channels.tlon.allowPrivateNetwork` (flat key) is retired. `steelengine doctor --fix` moves it to
...
prompt to yes and writes `true` explicitly. With it on, SteelEngine scries joined groups on startup,
...
Use with `steelengine message send` or cron delivery:
