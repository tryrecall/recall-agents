...
<Note>
Requires Recall 2026.4.25 or above. Run `recall --version` to check. Upgrade with `recall update`.
</Note>

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  recall channels login --channel feishu
  ```
...
4. Check logs: `recall logs --follow`
...
5. Ensure the gateway is running: `recall gateway status`
6. Check logs: `recall logs --follow`

### QR setup does not react in the Feishu mobile app

1. Rerun setup: `recall channels login --channel feishu`
...
3. Restart the gateway: `recall gateway restart`
  ...
  channels: {
    feishu: {
      ...
      dynamicAgentCreation: {
        enabled: true,
        workspaceTemplate: "~/.recall/workspace-{agentId}",
        agentDirTemplate: "~/.recall/agents/{agentId}/agent",
      },
    },
  },
  session: {
...
| `channels.feishu.dynamicAgentCreation.workspaceTemplate` | Path template for dynamic agent workspaces | `~/.recall/workspace-{agentId}`    |
| `channels.feishu.dynamicAgentCreation.agentDirTemplate`  | Agent directory name template              | `~/.recall/agents/{agentId}/agent` |
...
workspace: /Users/you/.recall/workspace-feishu-ou_xxxxxx
...
of raw `file_key` JSON. When `tools.media.audio` is configured, Recall
...
conversion fails, Recall falls back to a file attachment and logs the reason.
...
topic session key. If a native topic starter event omits `thread_id`, Recall
hydrates it from Feishu before routing the turn. Normal group replies that
Recall turns into threads keep using the reply root message ID (`om_*`) so the
