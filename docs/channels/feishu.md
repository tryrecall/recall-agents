...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Recall to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Recall releases, so no separate plugin install
is required.
...
If you just installed Recall, run onboarding:
...
2. Configuring app credentials in Recall
...
1. You already ran `recall channels add` for Feishu
2. The gateway is running (`recall gateway status`)
...
2. Run `recall logs --follow` and look for `chat_id`
...
2. Run `recall logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `recall gateway status`  | Show gateway status           |
| `recall gateway install` | Install/start gateway service |
| `recall gateway stop`    | Stop gateway service          |
| `recall gateway restart` | Restart gateway service       |
| `recall logs --follow`   | Tail gateway logs             |
