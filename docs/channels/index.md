---
summary: "Messaging platforms SteelEngine can connect to"
read_when:
  - You want to choose a chat channel for SteelEngine
  - You need a quick overview of supported messaging platforms
title: "Chat channels"
---

SteelEngine can talk to you on any chat app you already use. Each channel connects via the Gateway.
Text is supported everywhere; media and reactions vary by channel.

iMessage, Telegram, and the WebChat UI ship with the core install. Channels marked
"official plugin" install with one command (`steelengine plugins install @steelengine/<id>`)
or on demand during `steelengine onboard` / `steelengine channels add`, then need a Gateway
restart. "External plugin" channels are maintained outside the SteelEngine repo.
...
- [Reef](/channels/reef) - Guarded, end-to-end-encrypted claw-to-claw messaging between SteelEngine agents of different people (bundled plugin).
...
- Channels can run simultaneously; configure multiple and SteelEngine will route per chat.
