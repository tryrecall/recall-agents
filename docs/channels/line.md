...
read_when:
  - You want to connect Recall to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to Recall via the LINE Messaging API. The plugin runs as a webhook
...
```bash
recall plugins install @recall/line
```

Local checkout (when running from a git repo):

```bash
recall plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Recall applies strict pre-auth body limits and timeout before verification.
- Recall processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
recall pairing list line
recall pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.recall/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. Recall validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
