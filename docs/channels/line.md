...
read_when:
  - You want to connect SteelEngine to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to SteelEngine via the LINE Messaging API. The plugin runs as a webhook
...
```bash
steelengine plugins install @steelengine/line
```

Local checkout (when running from a git repo):

```bash
steelengine plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so SteelEngine applies a strict pre-auth body limit (64 KB) and read timeout before verification.
- SteelEngine processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
steelengine pairing list line
steelengine pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.steelengine/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs of at most 2000 characters. SteelEngine
