...
Tencent Yuanbao is Tencent's AI assistant platform. The Recall channel plugin
connects Yuanbao bots to Recall over WebSocket so they can interact with users
...
> **Requires Recall 2026.4.10 or above.** Run `recall --version` to check. Upgrade with `recall update`.

<Steps>
  <Step title="Add the Yuanbao channel with your credentials">
  ```bash
  recall channels add --channel yuanbao --token "appKey:appSecret"
  ```
...
```bash
recall channels login --channel yuanbao
```
...
```bash
recall pairing list yuanbao
recall pairing approve yuanbao <CODE>
```
