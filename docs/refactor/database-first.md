...
read_when:
  - Moving SteelEngine runtime data, cache, transcripts, task state, or scratch files into SQLite
...
- Global database: `~/.steelengine/state/steelengine.sqlite`
...
- Configuration stays file-backed: `steelengine.json` remains outside the
  database. Runtime auth profiles move to SQLite; external provider or CLI
  credential files remain owner-managed outside SteelEngine's database.
...
- Raw stream logging uses `STEELENGINE_RAW_STREAM=1` plus SQLite diagnostics rows.
  The old pi-mono `PI_RAW_STREAM`, `PI_RAW_STREAM_PATH`, and
  `raw-openai-completions.jsonl` file logger contract is not part of SteelEngine
  runtime or tests.
...
- One global SQLite database owns control-plane state:
  `state/steelengine.sqlite`.
- One per-agent SQLite database owns data-plane state:
  `agents/<agentId>/agent/steelengine-agent.sqlite`.
- Config remains file-backed. `steelengine.json` is not part of this database
  refactor.
...
- Task registry: `clean`. Task and Task Flow runtime rows live in
  `state/steelengine.sqlite`; unshipped sidecar SQLite importers are deleted.
...
- [x] Keep Kysely generated types aligned after any schema change.
      Files: `src/state/steelengine-state-schema.sql`,
...
- `steelengine doctor --fix` owns legacy file-to-database migration. Runtime
...
- The old runtime-owned JSONL transcript streaming helper was deleted. Doctor
  ...
  SQLite rows.
- Codex app-server bindings use the SteelEngine `sessionId` as the canonical
...
former stores now write to `state/steelengine.sqlite` or the owning
`steelengine-agent.sqlite` database.
...
- `src/state/steelengine-state-db.ts` opens `steelengine.sqlite`, sets WAL,
  `synchronous=NORMAL`, `busy_timeout=30000`, `foreign_keys=ON`, and applies
  the generated schema module derived from
  `src/state/steelengine-state-schema.sql`.
...
- Plugin migrations are data migrations over plugin-owned namespaces, not host
  ...
  `steelengine-state-schema.sql` unless the host itself is taking ownership of a
  new cross-plugin contract.
- `src/state/steelengine-agent-db.ts` opens
  `agents/<agentId>/agent/steelengine-agent.sqlite`, registers the database in the
...
- Per-agent external conversation identity is relational too:
  ...
  `session_conversations` links one SteelEngine session to one or more external
  ...
  SteelEngine session can represent multiple external DM peers without demoting
...
- TUI last-session restore pointers now live in typed shared
  ...
  excludes heartbeat sessions. `steelengine doctor --fix` strictly validates the
...
- Secret target metadata now talks about stores instead of pretending every
  credential target is a config file. `steelengine.json` remains the config store;
...
- Workspace bootstrap completion, attestation recency, and generated bootstrap
  ...
  `steelengine-workspace-state.json`, `.steelengine/workspace-state.json`, state-dir
  `workspace-attestations/*.attested`, or sibling `<workspace>.attested`
  sidecars. `steelengine doctor --fix` validates and claims legacy sources,
...
- GitHub Copilot compaction no longer writes `steelengine-compaction-*.json`
  workspace sidecars. The harness calls the SDK history compaction RPC for the
  tracked SDK session, and SteelEngine keeps durable session/transcript state in
  SQLite instead of compatibility marker files.
- The shared Swift runtime (`SteelEngineKit`) uses the same
  `state/steelengine.sqlite` rows for device identity and device auth. macOS app
...
- Android device identity uses the same TypeScript-compatible key material
  stored in typed `state/steelengine.sqlite#table/device_identities` rows. It never
  reads or writes `steelengine/identity/device.json`; a leftover legacy file blocks
  startup until doctor imports it into SQLite.
- Android cached device auth tokens also use typed
  `state/steelengine.sqlite#table/device_auth_tokens` rows and share the same
...
- Non-doctor commands do not auto-run legacy config repair. For example,
  `steelengine update --channel` now fails on invalid legacy config and asks the
...
- APNs runtime reads and writes only `apns_registrations`. Explicit
  `steelengine doctor --fix` strictly imports the retired
...
- Node-host config now uses a typed singleton row in the shared SQLite database.
  ...
  remains; explicit `steelengine doctor --fix` strictly imports and removes it
...
- Plugin state now uses the shared `state/steelengine.sqlite` database. The old
...
- Task and Task Flow runtime tables now live in the shared
  `state/steelengine.sqlite` database instead of `tasks/runs.sqlite` and
...
- Session target resolution now exposes per-agent database targets, not legacy
  ...
  `steelengine sessions` enumerate `agent_databases` plus configured agents.
...
- Cache trace, Anthropic payload, raw stream, and diagnostics timeline records
  ...
  `diagnostics.cacheTrace.filePath`, `STEELENGINE_CACHE_TRACE_FILE`,
  `STEELENGINE_ANTHROPIC_PAYLOAD_LOG_FILE`, and
  `STEELENGINE_DIAGNOSTICS_TIMELINE_PATH` JSONL override paths are removed, and
...
- Memory Wiki activity logs now use SQLite plugin state instead of
  `.steelengine-wiki/log.jsonl`. The Memory Wiki migration provider imports old
  JSONL logs; wiki markdown and user vault content stay file-backed as
  workspace content.
- Memory Wiki no longer creates `.steelengine-wiki/state.json` or the unused
  `.steelengine-wiki/locks` directory. The migration provider removes those retired
...
- The macOS companion no longer writes app-local `logs/config-audit.jsonl` or
  `logs/config-health.json` sidecars while editing `steelengine.json`. The config
...
- System-agent rescue pending approvals now use core SQLite plugin state instead
  of `crestodian/rescue-pending/*.json` or `steelengine/rescue-pending/*.json`.
...
- Codex app-server conversation bindings now key SQLite plugin state by
  SteelEngine session key or explicit `{agentId, sessionId}` scope. They must not
...
- Zalo hosted outbound media now uses shared SQLite `plugin_blob_entries`
  instead of `steelengine-zalo-outbound-media` JSON/bin temp sidecars.
...
- ACPX generated wrapper scripts and the isolated Codex home are temporary
  materialization under the SteelEngine temp root, not durable SteelEngine state. The
...
- Gateway media attachments now use the shared `media_blobs` SQLite table as
  ...
  `$STEELENGINE_STATE_DIR/media` or config-dir `media` roots; those directories are
  doctor import sources only.
- Shell completion no longer writes `$STEELENGINE_STATE_DIR/completions/*` cache
...
- Subagent inline attachments no longer materialize under workspace
  `.steelengine/attachments/*`. The spawn path prepares SQLite VFS seed entries,
...
- CLI image hydration no longer maintains stable `steelengine-cli-images` cache
...
- `steelengine secrets apply` scrubs the config file, env file, and SQLite
...
- User-facing auth docs now describe
  `state/steelengine.sqlite#table/auth_profile_stores/<agentDir>` instead of
...
- MCP OAuth sessions now use versioned `mcp_oauth_stores` rows in shared
  `state/steelengine.sqlite`. SDK-owned token, client-registration, and discovery
...
- Memory Wiki imported-source sync ledgers now store one SQLite plugin-state row
  per vault/source key instead of rewriting `.steelengine-wiki/source-sync.json`;
...
- Memory Wiki ChatGPT import-run records now store one SQLite plugin-state row
  per vault/run id instead of writing `.steelengine-wiki/import-runs/*.json`.
...
- Memory Wiki compiled digests now store compressed SQLite plugin-blob rows
  instead of writing `.steelengine-wiki/cache/agent-digest.json` and
  `.steelengine-wiki/cache/claims.jsonl`. The cache is rebuildable, so doctor
...
- ACPX wrapper scripts and the isolated Codex home are generated in the
  SteelEngine temp root. They are recreated as needed and are not backup or
  migration inputs.
...
- Backup stages the state directory before archiving, copies non-database files,
  ...
  completed backup runs in SQLite with the archive manifest. `steelengine backup
create` validates the written archive by default; `--no-verify` is the
  explicit fast path.
- `steelengine backup restore` validates the archive before extraction, reuses the
...
- `steelengine reset --scope config+creds+sessions` removes per-agent
  `steelengine-agent.sqlite` databases plus WAL/SHM sidecars, not only legacy
...
- Generated model catalog config is stored in typed global SQLite
  ...
  `ensureSteelEngineModelCatalog`; there is no `models.json` compatibility API in
...
- QMD's own `index.sqlite`, YAML collection config, and model downloads remain
  external-tool artifacts under `~/.steelengine/agents/<agentId>/qmd`; they are not
  mirrored into `plugin_blob_entries`. SteelEngine-owned QMD coordination is
...
- The optional `memory-lancedb` plugin no longer creates
  `~/.steelengine/memory/lancedb` as an implicit SteelEngine-managed store. It is an
...
`steelengine doctor --fix` invokes the state migration implementation after
ordinary config preflight and creates a verified backup before import. Runtime
startup and `steelengine migrate` must not import legacy SteelEngine state files.
...
- Add a single doctor-owned state migration service for file-to-database import.
  Done: `steelengine doctor --fix` uses the legacy-state migration implementation.
...
Keep shared coordination state in `state/steelengine.sqlite`:
...
- Verify by opening every DB snapshot and running `PRAGMA integrity_check`.
  `steelengine backup create` does this archive verification by default;
...
5. Verify every SQLite snapshot's file shape, then open canonical SteelEngine
   ...
   `steelengine backup create` does this by default; `--no-verify` is only for
...
- Use `{ agentId, sessionKey }` row operations.
  ...
  and the `steelengine sessions` listing command now read per-agent rows directly
...
8. Backup integration.
   ...
   - Add restore from verified archive snapshots. Done: `steelengine backup
...
- Remove tests that assert runtime creation of `sessions.json` or transcript
  ...
  SteelEngine test-state helper no longer exposes a legacy `auth-profiles.json`
...
- ad-hoc `steelengine-state.sqlite` runtime sidecars
- `tasks/runs.sqlite`
...
- `steelengine/rescue-pending/*.json`
- `plugins/phone-control/armed.json`
- Memory Wiki `.steelengine-wiki/log.jsonl`
- Memory Wiki `.steelengine-wiki/state.json`
- Memory Wiki `.steelengine-wiki/locks/`
- Memory Wiki `.steelengine-wiki/source-sync.json`
- Memory Wiki `.steelengine-wiki/import-runs/*.json`
- Memory Wiki `.steelengine-wiki/cache/agent-digest.json`
- Memory Wiki `.steelengine-wiki/cache/claims.jsonl`
- ClawHub `.clawhub/lock.json`
- ClawHub `.clawhub/origin.json`
- Browser profile decoration `.steelengine-profile-decorated`
