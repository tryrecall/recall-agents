...
- Added `extensions/canvas/recall.plugin.json`.
- Moved the agent `canvas` tool from `src/agents/tools/canvas-tool.ts` to `extensions/canvas/src/tool.ts`.
- Removed core registration of `createCanvasTool` from `src/agents/recall-tools.ts`.
...
- Added `api.registerNodeCliFeature(...)` so Canvas can declare `recall nodes canvas` as a plugin-owned node feature without manually spelling the parent command path.
...
- Kept the Canvas doctor migration so `recall doctor --fix` rewrites old `canvasHost` configs into `plugins.entries.canvas.config.host`.
...
- `rg "registerNodesCanvasCommands|nodes-canvas" src` is empty; the Canvas plugin registers `recall nodes canvas` through nested plugin CLI metadata.
