...
Recall's Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`recall agent`), so routing/permissions/config match your Gateway.
...
- use the matching Gateway HTTP auth path:
  ...
  - trusted-proxy local direct fallback: same-host callers with no `Forwarded`, `X-Forwarded-*`, or `X-Real-IP` headers can use `gateway.auth.password` / `RECALL_GATEWAY_PASSWORD`
  - private-ingress open auth (`gateway.auth.mode="none"`): no auth header
- treat the endpoint as full operator access for the gateway instance
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-recall-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-recall-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "recall"`, `model: "recall/default"`, `model: "recall/<agentId>"`, or `x-recall-agent-id`
- use `x-recall-model` when you want to override the selected agent's backend model
- use `x-recall-session-key` for explicit session routing
- use `x-recall-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: Recall reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
Recall normalizes common OpenAI-style aliases before those counters reach
