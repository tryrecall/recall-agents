...
- Service types:
  - `_recall-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  ...
  - `cliPath=<path>` (optional; absolute path to a runnable `recall` entrypoint or binary)
...
- `RECALL_SSH_PORT` overrides the SSH port advertised in TXT (defaults to 22).
