...
1. **State ownership lock** acquires a lock keyed by the canonical state directory. Every Gateway participates, including Gateways started with `STEELENGINE_ALLOW_MULTI_GATEWAY=1`, so destructive SQLite maintenance cannot race a live owner.
...
- If the port is occupied by a different, non-gateway process, the error is the same; free the port or choose another with `steelengine gateway --port <port>`.
- `STEELENGINE_ALLOW_MULTI_GATEWAY=1` permits multiple config/runtime instances, not shared mutable state. Each instance still needs a unique `STEELENGINE_STATE_DIR`.
