...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “Recall runs”).
...
- `recall gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`RECALL_GATEWAY_PORT`).
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `RECALL_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
