...
- Use the macOS app's remote mode (Settings → General → Recall runs).
...
- `recall health` and `recall status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `recall gateway status`, `recall gateway health`, `recall gateway probe`, and `recall gateway call` can also target the forwarded URL via `--url` when needed.

<Note>
Replace `18789` with your configured `gateway.port` (or `--port` or `RECALL_GATEWAY_PORT`).
</Note>
...
- URL override safety:
  ...
  - Env URL overrides (`RECALL_GATEWAY_URL`) may use env credentials only (`RECALL_GATEWAY_TOKEN` / `RECALL_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `RECALL_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `RECALL_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `RECALL_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `RECALL_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `RECALL_GATEWAY_*` only.
...
```bash
recall config set gateway.remote.token "<your-token>"
```

#### Step 4: create the LaunchAgent

Save this as `~/Library/LaunchAgents/ai.recall.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.recall.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.recall.ssh-tunnel.plist
```

The tunnel will start automatically at login, restart on crash, and keep the forwarded port live.

<Note>
If you have a leftover `com.recall.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
</Note>
...
```bash
launchctl kickstart -k gui/$UID/ai.recall.ssh-tunnel
```

Stop the tunnel:

```bash
launchctl bootout gui/$UID/ai.recall.ssh-tunnel
```
