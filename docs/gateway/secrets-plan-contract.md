...
read_when:
  - Generating or reviewing `recall secrets apply` plans
  - Debugging `Invalid plan target path` errors
  - Understanding target type and path validation behavior
title: "Secrets apply plan contract"
---

This page defines the strict contract enforced by `recall secrets apply`.

If a target does not match these rules, apply fails before mutating configuration.

## Plan file shape

`recall secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
- `providerUpserts` — an object keyed by provider alias. Each value is a
  ...
  `secrets.providers.<alias>` in `recall.json`, e.g. an `exec` or `file`
  provider).
...
configured in `recall.json` fail with `provider "<alias>" is not
configured`.
...
- `secrets apply` writes supported `recall.json` targets, supported `auth-profiles.json` targets, and optional scrub targets.
...
If apply fails with an invalid target path message, regenerate the plan with `recall secrets configure` or fix the target path to a supported shape above.
