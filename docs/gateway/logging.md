...
- Default rolling log file is under `/tmp/recall/` (one file per day): `recall-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.tryrecall/recall-agents.json`:
  - `logging.file`
  - `logging.level`
...
Recall can mask sensitive tokens before log or transcript output leaves the
...
`recall gateway` supports a per-gateway style switch:
