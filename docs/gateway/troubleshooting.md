...
- `recall gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `recall doctor` reports no blocking config/service issues.
- `recall channels status --probe` shows live per-account transport status and, where supported, probe/audit results such as `works` or `audit ok`.
...
- `Update restart` in `recall status` / `recall status --all`. Pending or
  failed handoffs include the next command to run.
- `plugin load failed: dependency tree corrupted; run recall doctor --fix`
  under Channels. That means the channel config still exists, but plugin
  registration failed before the channel could load.
- provider 401s after re-auth. `recall doctor --fix` checks for stale
...
Use this when a gateway service unexpectedly stops after an update, or logs show that one `recall` binary is older than the version that last wrote `recall.json`.

Recall stamps config writes with `meta.lastTouchedVersion`. Read-only commands can still inspect a config written by a newer Recall, but process and service mutations refuse to continue from an older binary. Blocked actions include gateway service start, stop, restart, uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which recall
recall --version
recall gateway status --deep
recall config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `recall` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `recall` binary.
  </Step>
</Steps>

<Warning>
For intentional downgrade or emergency recovery only, set `RECALL_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1` for the single command. Leave it unset for normal operation.
</Warning>

## Protocol mismatch after rollback

Use this when logs keep printing `protocol mismatch` after you downgrade or roll back Recall. This means an older Gateway is running, but a newer local client process is still trying to reconnect with a protocol range that the older Gateway cannot speak.
...
- `Established clients:` in `recall gateway status --deep` or `Gateway clients` in `recall doctor --deep`. This lists active TCP clients connected to the Gateway port, including PIDs and command lines when the OS allows it.
- A client process whose command line points at the newer Recall install or wrapper you rolled back from.

Fix:

1. Stop or restart the stale Recall client process shown by `gateway status --deep`.
2. Restart apps or wrappers that embed Recall, such as local dashboards, editors, app-server helpers, or long-running `recall logs --follow` shells.
3. Re-run `recall gateway status --deep` or `recall doctor --deep` and confirm the stale client PID is gone.
...
Recall treats every skill root as a containment boundary. A symlink under
`~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills`, or
`~/.recall/skills` is skipped when its real target resolves outside that root
...
recall config get agents.defaults.models
```
...
Do not assume this is always an Recall configuration issue. The response can
...
block. Prefer a narrowly scoped allow or skip rule for the API path Recall
...
- Recall model runs fail only on normal agent turns
...
recall infer model run --model <provider/model> --prompt "hi" --json
recall logs --follow
```

Look for:

- direct tiny calls succeed, but Recall runs fail only on larger prompts
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0` → the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. Recall retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - direct tiny requests succeed, but Recall agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds) → Recall transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    3. Set `compat.supportsTools: false` for models/backends that cannot handle Recall's tool schema surface reliably.
    ...
    5. If tiny direct requests keep passing while Recall agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
recall status
recall channels status --probe
recall pairing list --channel <channel> [--account <id>]
recall config get channels
recall logs --follow
```
...
If `curl` returns Recall HTML, the Gateway is working and the remaining issue
...
does not leave the service running, run `recall gateway start` and recheck
`recall gateway status`.
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `recall config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `recall devices list` then `recall devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                               |
...
If `recall devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`
- `recall devices rotate --scope ...` can only request operator scopes that the caller session already holds
...
recall logs --follow
recall doctor
recall gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `recall onboard --mode local` / `recall setup` to restamp the expected local-mode config. If you are running Recall via Podman, the default config path is `~/.tryrecall/recall-agents.json`.
    ...
    - `System-level Recall gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `RECALL_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `recall doctor --fix` or `recall gateway install --force`, then restart the gateway service.
...
- `critical memory pressure bundle written` appears shortly before restart → Recall captured a pre-OOM stability bundle. Inspect it with `recall gateway stability --bundle latest`.
- `memory pressure: level=critical ... memoryPressureSnapshot=disabled` appears in gateway logs → Recall detected critical memory pressure, but the pre-OOM stability snapshot is off.
...
- A timestamped `recall.json.rejected.*` file beside the active config
- A timestamped `recall.json.clobbered.*` file if `doctor --fix` repaired a broken direct edit
- Recall keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an Recall-owned write.
    - Gateway startup fails closed instead of rewriting `recall.json`.
    - Hot reload skips invalid external edits and keeps the current runtime config active.
    - Recall-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    - `recall doctor --fix` owns repair. It can remove non-JSON prefixes or restore the last-known-good copy while preserving the rejected payload as `.clobbered.*`.
    - When many repairs happen for one config path, Recall rotates older `.clobbered.*` files so the newest repaired payload is still available.

  </Accordion>
  <Accordion title="Inspect and repair">
    ```bash
    CONFIG="$(recall config file)"
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an Recall-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an Recall-owned write was rejected because it lost fields or size compared with the last-known-good backup.
    - `Config last-known-good promotion skipped` → the candidate contained redacted secret placeholders such as `***`.

  </Accordion>
  <Accordion title="Fix options">
    1. Run `recall doctor --fix` to let doctor repair prefixed/clobbered config or restore last-known-good.
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `recall config set` or `config.patch`.
    3. Run `recall config validate` before restarting.
...
Use this when `recall gateway probe` reaches something, but still prints a warning block.

```bash
recall gateway probe
recall gateway probe --json
recall gateway probe --ssh user@gateway-host
```
...
recall cron status
recall cron list
recall cron runs --id <jobId> --limit 20
recall system heartbeat last
recall logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so Recall skips the model call.
...
```bash
recall nodes status
recall nodes describe --node <idOrNameOrIp>
recall approvals get --node <idOrNameOrIp>
recall logs --follow
recall status
```
...
recall browser start --browser-profile recall
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `recall browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (Recall-managed)](/tools/browser)
...
<AccordionGroup>
  <Accordion title="1. Auth and URL override behavior changed">
    ```bash
    recall gateway status
    recall config get gateway.mode
    recall config get gateway.remote.url
    recall config get gateway.auth.mode
    ```
  ...
  <Accordion title="2. Bind and auth guardrails are stricter">
    ```bash
    recall config get gateway.bind
    recall config get gateway.auth.mode
