...
- `recall gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `recall doctor` reports no blocking config/service issues.
- `recall channels status --probe` shows connected/ready channels.
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `recall devices list` then `recall devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `recall configure`). If you are running Recall via Podman using the dedicated `recall` user, the config lives at `~recall/.recall/recall.json`.
