...
Recall can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **Recall tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
recall agent --message "hi" --model claude-cli/claude-sonnet-4-6
```
...
gateway host, Recall now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), Recall will
    ...
    defaults: {
      cliBackends: {
        "my-cli": {
          ...
          // from bounded raw Recall transcript history before compaction.
...
2. **Builds a system prompt** using the same Recall prompt + workspace context.
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
   The bundled `claude-cli` backend keeps a Claude stdio process alive per
   Recall session and sends follow-up turns over stream-json stdin.
4. **Parses output** (JSON or plain text) and returns the final text.
5. **Persists session ids** per backend, so follow-ups reuse the same CLI session.

<Note>
The bundled Anthropic `claude-cli` backend is supported again. Anthropic staff
told us Recall-style Claude CLI usage is allowed again, so Recall treats
`claude -p` usage as sanctioned for this integration unless Anthropic publishes
a new policy.
</Note>

The bundled Anthropic `claude-cli` backend receives the Recall skills snapshot
two ways: the compact Recall skills catalog in the appended system prompt, and
...
resolver sees the same filtered set that Recall would otherwise advertise in
the prompt. Skill env/API key overrides are still applied by Recall to the
child process environment for the run.

Claude CLI also has its own noninteractive permission mode. Recall maps that
...
`tools.exec.ask: "off"`), Recall adds `--permission-mode bypassPermissions`.
...
The bundled Anthropic `claude-cli` backend also maps Recall `/think` levels
...
Before Recall can use the bundled `claude-cli` backend, Claude Code itself
must already be logged in on the same host:

```bash
claude auth login
claude auth status --text
recall models auth login --provider anthropic --method cli --set-default
```
...
- `claude-cli` defaults to `liveSession: "claude-stdio"`, `output: "jsonl"`,
  ...
  exits, Recall resumes from the stored Claude session id. Stored session
...
- Claude live sessions keep bounded JSONL output guards. Defaults allow up to
  ...
  and `maxTurnLines`; Recall clamps those settings to 64 MiB and 100,000
  lines.
- Stored CLI sessions are provider-owned continuity. The implicit daily session
  ...
  do.
- Fresh CLI sessions normally reseed only from Recall's compaction summary
  plus post-compaction tail. To recover short sessions that are invalidated
  before compaction, a backend can opt in with
  `reseedFromRawTranscriptWhenUncompacted: true`. Recall still keeps raw
...
- Recall drops stored CLI session reuse when the selected auth identity changes,
  ...
  stable OAuth account id, Recall lets that CLI enforce resume permissions.

## Fallback prelude from claude-cli sessions

When a `claude-cli` attempt fails over to a non-CLI candidate in
[`agents.defaults.model.fallbacks`](/concepts/model-failover), Recall seeds
...
provider would start cold because Recall's own session transcript is empty
for `claude-cli` runs.
...
Recall will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, Recall appends the
...
- For Gemini CLI JSON output, Recall reads reply text from `response` and
...
agent runs use the Codex app-server harness through `openai/*`; Recall no
...
- `stats.cached` is normalized into Recall `cacheRead`.
- If `stats.input` is missing, Recall derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before Recall handles
...
CLI backends do **not** receive Recall tool calls directly, but a backend can
...
When bundle MCP is enabled, Recall:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`RECALL_MCP_TOKEN`)
...
If no MCP servers are enabled, Recall still injects a strict config when a
...
- **No direct Recall tool calls.** Recall does not inject tool calls into
