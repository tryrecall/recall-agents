...
Recall supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
agent can inspect, including `recall.json`, `auth-profiles.json`, `.env`, or
...
`recall secrets audit --check` reports no plaintext secret residue.
</Warning>
...
- legacy plaintext residue has been scrubbed from `recall.json`,
  `auth-profiles.json`, `.env`, and generated `models.json` files
- `recall secrets audit --check` is clean after the migration
...
<AccordionGroup>
  <Accordion title="Examples of inactive surfaces">
    ...
    - `gateway.auth.token` SecretRef is inactive for startup auth resolution when `RECALL_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Recall runs preflight validation before saving:
...
<AccordionGroup>
  ...
  <Accordion title="Exec provider">
    ...
    - Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Recall validates the resolved target path.
...
<AccordionGroup>
      ...
      secrets: {
        providers: {
          onepassword_openai: {
            ...
            args: ["read", "op://Personal/Recall QA API Key/password"],
            passEnv: ["HOME"],
            jsonOnly: false,
          },
        },
  ...
  <Accordion title="Bitwarden Secrets Manager (`bws`)">
    ...
    `scripts/secrets/recall-bws-resolver.mjs`; install or copy it to an absolute
    ...
    ```json5
    {
      secrets: {
        providers: {
          bws: {
            source: "exec",
            command: "/usr/local/bin/recall-bws-resolver.mjs",
        ...
        providers: {
          openai: {
            ...
            apiKey: {
              source: "exec",
              provider: "bws",
              id: "recall/providers/openai/apiKey",
            },
          },
        },
      },
    ...
    SecretRef id contract, such as `recall/providers/openai/apiKey`; env-var
    ...
    ```bash
    recall secrets audit --allow-exec
    ```
      ...
      secrets: {
        providers: {
          vault_openai: {
            ...
            allowSymlinkCommand: true, // required for Homebrew symlinked binaries
            trustedDirs: ["/opt/homebrew"],
            args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/recall"],
  ...
  <Accordion title="password-store (`pass`)">
    ...
    `/usr/local/bin/recall-pass-resolver`. The `#!/usr/bin/env node` shebang
    ...
    ```json5
    {
      secrets: {
        providers: {
          pass_store: {
            source: "exec",
            command: "/usr/local/bin/recall-pass-resolver",
...
- Recall resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__RECALL_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `recall.json` refs)
...
When reload-time activation fails after a healthy state, Recall enters degraded secrets state.
...
<Tabs>
  <Tab title="Strict command paths">
    For example `recall memory` remote-memory paths and `recall qr --remote` when it needs remote shared-secret refs. They read from the active snapshot and fail fast when a required SecretRef is unavailable.
  </Tab>
  <Tab title="Read-only command paths">
    For example `recall status`, `recall status --all`, `recall channels status`, `recall channels resolve`, `recall security audit`, and read-only doctor/config repair flows. They also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `recall secrets reload`.
...
<Steps>
  ...
  <Step title="Configure and apply SecretRefs">
    ```bash
    recall secrets configure --apply
    ```
  </Step>
  <Step title="Re-audit">
    ```bash
...
with `recall secrets apply --from <plan-path>` before the re-audit.

<AccordionGroup>
  <Accordion title="secrets audit">
    Findings include:

    - plaintext values at rest (`recall.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
    - plaintext sensitive provider header residues in generated `models.json` entries
    - unresolved refs
    - precedence shadowing (`auth-profiles.json` taking priority over `recall.json` refs)
    - legacy residues (`auth.json`, OAuth reminders)

    Exec note:

    - By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
    - Use `recall secrets audit --allow-exec` to execute exec providers during audit.
  ...
  <Accordion title="secrets configure">
    ...
    - lets you select supported secret-bearing fields in `recall.json` plus `auth-profiles.json` for one agent scope
    ...
    - `recall secrets configure --providers-only`
    - `recall secrets configure --skip-provider-setup`
    - `recall secrets configure --agent <id>`
  ...
  <Accordion title="secrets apply">
    ...
    ```bash
    recall secrets apply --from /tmp/recall-secrets-plan.json
    recall secrets apply --from /tmp/recall-secrets-plan.json --allow-exec
    recall secrets apply --from /tmp/recall-secrets-plan.json --dry-run
    recall secrets apply --from /tmp/recall-secrets-plan.json --dry-run --allow-exec
    ```
...
<Warning>
Recall intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
