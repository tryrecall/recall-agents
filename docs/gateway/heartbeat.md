...
- During heartbeat runs, SteelEngine treats `HEARTBEAT_OK` as an ack when it appears at the **start or end** of the reply. The token is stripped and the reply is dropped if the remaining content is **≤ `ackMaxChars`** (default: 300).
...
- Explicit session key (copy from `steelengine sessions --json` or the [sessions CLI](/cli/sessions)).
...
<AccordionGroup>
  ...
  <Accordion title="Visibility and skip behavior">
    ...
    - If only alert delivery is disabled, SteelEngine can still run the heartbeat, update due-task timestamps, restore the session idle timestamp, and suppress the outward alert payload.
    - If the resolved heartbeat target supports typing, SteelEngine shows typing while the heartbeat run is active. This uses the same target the heartbeat would send chat output to, and it is disabled by `typingMode: "never"`.
...
If **all three** are false, SteelEngine skips the heartbeat run entirely (no model call).
...
<AccordionGroup>
  <Accordion title="Behavior">
    - SteelEngine parses the `tasks:` block and checks each task against its own `interval`.
...
Use `steelengine system event` to enqueue a system event and optionally trigger an immediate heartbeat:

```bash
steelengine system event --text "Check for urgent follow-ups" --mode now
```
...
```bash
steelengine system heartbeat last     # show the last heartbeat event
steelengine system heartbeat enable   # enable heartbeats
steelengine system heartbeat disable  # disable heartbeats
```
