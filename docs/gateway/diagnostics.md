...
SteelEngine can build a local diagnostics `.zip` for bug reports: sanitized Gateway
...
2. SteelEngine sends a preamble and asks for one explicit exec approval, which runs
   `steelengine gateway diagnostics export --json`. Do not approve diagnostics via
   an allow-all rule.
3. After approval, SteelEngine replies with the local bundle path, manifest
   summary, privacy notes, and relevant session ids.

In group chats, an owner can still run `/diagnostics`, but SteelEngine sends the
...
approval also covers an OpenAI feedback upload for the Codex threads SteelEngine
...
approval, the reply lists channels, SteelEngine session ids, Codex thread ids, and
...
Persisted bundles live under `~/.steelengine/logs/stability/` when events exist.

## Useful options

```bash
steelengine gateway diagnostics export \
  --output steelengine-diagnostics.zip \
  --log-lines 5000 \
  --log-bytes 1000000
```
...
| ----------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------- |
| `--output <path>`       | `$STEELENGINE_STATE_DIR/logs/support/steelengine-diagnostics-<timestamp>-<pid>.zip` | Write to a specific zip path (or directory).       |
