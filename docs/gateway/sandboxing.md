...
Recall can run **tools inside sandbox backends** to reduce blast radius. This is **optional** and controlled by configuration (`agents.defaults.sandbox` or `agents.list[].sandbox`). If sandboxing is off, tools run on the host. The Gateway stays on the host; tool execution runs in an isolated sandbox when enabled.
...
<AccordionGroup>
  <Accordion title="Sandboxed browser details">
    ...
    - By default, sandbox browser containers use a dedicated Docker network (`recall-sandbox-browser`) instead of the global `bridge` network. Configure with `agents.defaults.sandbox.browser.network`.
...
Sandboxing is off by default. If you enable sandboxing and do not choose a backend, Recall uses the Docker backend. It executes tools and sandbox browsers locally via the Docker daemon socket (`/var/run/docker.sock`). Sandbox container isolation is determined by Docker namespaces.
...
If you deploy the Recall Gateway itself as a Docker container, it orchestrates sibling sandbox containers using the host's Docker socket (DooD). This introduces a specific path mapping constraint:

- **Config requires host paths**: The `recall.json` `workspace` configuration MUST contain the **Host's absolute path** (e.g. `/home/user/.recall/workspaces`), not the internal Gateway container path. When Recall asks the Docker daemon to spawn a sandbox, the daemon evaluates paths relative to the Host OS namespace, not the Gateway namespace.
- **FS bridge parity (identical volume map)**: The Recall Gateway native process also writes heartbeat and bridge files to the `workspace` directory. Because the Gateway evaluates the exact same string (the host path) from within its own containerized environment, the Gateway deployment MUST include an identical volume map linking the host namespace natively (`-v /home/user/.recall:/home/user/.recall`).
- **Codex code mode**: When an Recall sandbox is active, Recall disables Codex app-server native Code Mode, user MCP servers, and app-backed plugin execution for that turn because those native surfaces run from the Gateway-host app-server process instead of the Recall sandbox backend. Shell access is exposed through Recall sandbox-backed tools such as `sandbox_exec` and `sandbox_process` when the normal exec/process tools are available. Do not mount the host Docker socket into agent sandbox containers or custom Codex sandboxes.
...
Recall sandboxing and the service user is not allowed to create unprivileged
...
`recall doctor`; if it reports a Codex bwrap namespace probe failure, prefer
an AppArmor profile that grants the required namespaces to the Recall service
...
If you map paths internally without absolute host parity, Recall natively throws an `EACCES` permission error attempting to write its heartbeat inside the container environment because the fully qualified path string doesn't exist natively.
</Warning>

### SSH backend

Use `backend: "ssh"` when you want Recall to sandbox `exec`, file tools, and media reads on an arbitrary SSH-accessible machine.
...
Use `backend: "openshell"` when you want Recall to sandbox tools in an OpenShell-managed remote environment. For the full setup guide, configuration reference, and workspace mode comparison, see the dedicated [OpenShell page](/gateway/openshell).
...
<Tabs>
  <Tab title="none (default)">
    Tools see a sandbox workspace under `~/.recall/sandboxes`.
  </Tab>
  <Tab title="ro">
...
<Note>
**Skills note:** the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`, Recall mirrors eligible skills into the sandbox workspace (`.../skills`) so they can be read. With `"rw"`, workspace skills are readable from `/workspace/skills`.
</Note>
...
- Recall also blocks common home-directory credential roots such as `~/.aws`, `~/.cargo`, `~/.config`, `~/.docker`, `~/.gnupg`, `~/.netrc`, `~/.npm`, and `~/.ssh`.
- Bind validation is not just string matching. Recall normalizes the source path, then resolves it again through the deepest existing ancestor before re-checking blocked paths and allowed roots.
...
The `scripts/sandbox-setup.sh`, `scripts/sandbox-common-setup.sh`, and `scripts/sandbox-browser-setup.sh` helper scripts are only available when running from a [source checkout](https://github.com/tryrecall/recall-agents). They are not included in the npm package.

If you installed Recall via `npm install -g recall`, use the inline `docker build` commands shown below instead.
</Note>
...
<Steps>
  <Step title="Build the default image">
    ...
    ```bash
    docker build -t recall-sandbox:bookworm-slim - <<'DOCKERFILE'
    ...
    Recall does not silently substitute plain `debian:bookworm-slim` when `recall-sandbox:bookworm-slim` is missing. Sandbox runs that target the default image fail fast with a build instruction until you build it, because the bundled image carries `python3` for sandbox write/edit helpers.
  ...
  <Step title="Optional: build the common image">
    ...
    From an npm install, build the default image first (see above), then build the common image on top using the [`scripts/docker/sandbox/Dockerfile.common`](https://github.com/tryrecall/recall-agents/blob/main/scripts/docker/sandbox/Dockerfile.common) from the repository.

    Then set `agents.defaults.sandbox.docker.image` to `recall-sandbox-common:bookworm-slim`.
  ...
  <Step title="Optional: build the sandbox browser image">
    ...
    From an npm install, build using the [`scripts/docker/sandbox/Dockerfile.browser`](https://github.com/tryrecall/recall-agents/blob/main/scripts/docker/sandbox/Dockerfile.browser) from the repository.
...
<AccordionGroup>
  <Accordion title="Sandbox browser Chromium defaults">
    ...
    - The three graphics hardening flags (`--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu`) are optional and are useful when containers lack GPU support. Set `RECALL_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if your workload requires WebGL or other 3D/browser features.
    - `--disable-extensions` is enabled by default and can be disabled with `RECALL_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
    - `--renderer-process-limit=2` is controlled by `RECALL_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
For Docker gateway deployments, `scripts/docker/setup.sh` can bootstrap sandbox config. Set `RECALL_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can override socket location with `RECALL_DOCKER_SOCKET`. Full setup and env reference: [Docker](/install/docker#agent-sandbox).
