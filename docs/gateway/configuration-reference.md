---
summary: "Gateway config reference for core Recall keys, defaults, and links to dedicated subsystem references"
...
Core config reference for `~/.tryrecall/recall-agents.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Covers the main Recall config surfaces and links out when a subsystem has its own deeper reference. Channel- and plugin-owned command catalogs and deep memory/QMD knobs live on their own pages rather than on this one.

Code truth:

- `recall config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in when available
...
- `talk.*` (Talk mode)
  - `talk.consultThinkingLevel`: thinking level override for the full Recall agent run behind Control UI Talk realtime consults
...
- `models.providers.*.localService`: optional on-demand process manager for
  local model servers. Recall probes the configured health endpoint, starts
...
Recall-managed MCP server definitions live under `mcp.servers` and are
consumed by embedded Pi and other runtime adapters. The `recall mcp list`,
...
- `mcp.servers`: named stdio or remote MCP server definitions for runtimes that
  ...
  `type: "http"` is a CLI-native alias that `recall mcp set` and
  `recall doctor --fix` normalize into the canonical `transport` field.
- `mcp.servers.<name>.codex`: optional Codex app-server projection controls.
  This block is Recall metadata for Codex app-server threads only; it does not
  affect ACP sessions, generic Codex harness config, or other runtime adapters.
  Non-empty `codex.agents` limits the server to the listed Recall agent ids.
  ...
  `default_tools_approval_mode` for that server. Recall strips the `codex`
...
See [MCP](/cli/mcp#recall-as-an-mcp-client-registry) and
...
- `install.nodeManager`: node installer preference for `metadata.recall.install`
...
- Channel plugin account/runtime settings live under `channels.<id>` and should be described by the owning plugin's manifest `channelConfigs` metadata, not by a central Recall option registry.
...
`commitments` controls inferred follow-up memory: Recall can detect check-ins from conversation turns and deliver them through heartbeat runs.
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want Recall to discover `/json/version`; use WS(S)
...
- If an externally managed CDP service is reachable through loopback, set that
  profile's `attachOnly: true`; otherwise Recall treats the loopback port as a
...
- Local managed `recall` profiles auto-assign `cdpPort` and `cdpUrl`; only
...
- `tailscale.preserveFunnel`: when `true` and `tailscale.mode = "serve"`, Recall
...
- `gateway.handshakeTimeoutMs`: pre-auth Gateway WebSocket handshake timeout in milliseconds. Default: `15000`. `RECALL_HANDSHAKE_TIMEOUT_MS` takes precedence when set. Increase this on loaded or low-powered hosts where local clients can connect while startup warmup is still settling.
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/recall/tls/server.crt",
      keyPath: "/etc/recall/tls/server.key",
      caPath: "/etc/recall/tls/ca-bundle.crt",
    },
  },
}
```
...
- `transform` can point to a JS/TS module returning a hook action.
  ...
  - Keep `hooks.transformsDir` under `~/.recall/hooks/transforms`; workspace skill directories are rejected. If `recall doctor` reports this path as invalid, move the transform module into the hooks transforms directory or remove `hooks.transformsDir`.
...
- Hostname defaults to the system hostname when it is a valid DNS label, falling back to `recall`. Override with `RECALL_MDNS_HOSTNAME`.
...
- Legacy flat `auth-profiles.json` maps such as `{ "provider": { "apiKey": "..." } }` are not a runtime format; `recall doctor --fix` rewrites them to canonical `provider:default` API-key profiles with a `.legacy-flat.*.bak` backup.
...
- Default log file: `/tmp/tryrecall/recall-agents-YYYY-MM-DD.log`.
- Set `logging.file` for a stable path.
- `consoleLevel` bumps to `debug` when `--verbose`.
- `maxFileBytes`: maximum active log file size in bytes before rotation (positive integer; default: `104857600` = 100 MB). Recall keeps up to five numbered archives beside the active file.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.recall/logs/cache-trace.jsonl",
...
- `stuckSessionAbortMs`: no-progress age threshold in ms before eligible stalled active work may be abort-drained for recovery. When unset, Recall uses the safer extended embedded-run window of at least 5 minutes and 3x `stuckSessionWarnMs`.
...
- `RECALL_OTEL_PRELOADED=1`: environment toggle for hosts that already registered a global OpenTelemetry SDK. Recall then skips plugin-owned SDK startup/shutdown while keeping diagnostic listeners active.
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$RECALL_STATE_DIR/logs/cache-trace.jsonl`).
...
- Recall-owned writes that change only one top-level section backed by a single-file include write through to that included file. For example, `plugins install` updates `plugins: { $include: "./plugins.json5" }` in `plugins.json5` and leaves `recall.json` intact.
- Root includes, include arrays, and includes with sibling overrides are read-only for Recall-owned writes; those writes fail closed instead of flattening the config.
