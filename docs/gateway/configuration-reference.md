---
summary: "Gateway config reference for core SteelEngine keys, defaults, and links to dedicated subsystem references"
...
Field-level reference for `~/.steelengine/steelengine.json`: keys, defaults, and links to deeper subsystem pages. For task-oriented setup guidance, see [Configuration](/gateway/configuration). Channel- and plugin-owned command catalogs and deep memory/QMD knobs live on their own pages, not here.

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional; SteelEngine uses safe defaults when omitted.

Code truth beats this page:

- `steelengine config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in.
...
- `talk.*` (Talk mode)
  - `talk.consultThinkingLevel`: thinking level override for the full SteelEngine agent run behind Control UI Talk realtime consults
  ...
  - `talk.realtime.consultRouting`: Gateway relay fallback for finalized realtime Talk transcripts that skip `steelengine_agent_consult`
...
- `models.providers.*.localService`: optional on-demand process manager for
  local model servers. SteelEngine probes the configured health endpoint, starts
...
SteelEngine-managed MCP server definitions live under `mcp.servers` and are
consumed by embedded SteelEngine and other runtime adapters. The `steelengine mcp list`,
...
- `mcp.servers`: named stdio or remote MCP server definitions for runtimes that
  ...
  `type: "http"` is a CLI-native alias that `steelengine mcp set` and
  `steelengine doctor --fix` normalize into the canonical `transport` field.
- `mcp.servers.<name>.enabled`: set `false` to keep a saved server definition
  while excluding it from embedded SteelEngine MCP discovery and tool projection.
...
- `mcp.servers.<name>.auth`: set `"oauth"` for HTTP MCP servers that require
  OAuth. Run `steelengine mcp login <name>` to store tokens under SteelEngine state.
...
- `mcp.servers.<name>.codex`: optional Codex app-server projection controls.
  This block is SteelEngine metadata for Codex app-server threads only; it does not
  affect ACP sessions, generic Codex harness config, or other runtime adapters.
  Non-empty `codex.agents` limits the server to the listed SteelEngine agent ids.
  ...
  `default_tools_approval_mode` for that server. SteelEngine strips the `codex`
...
See [MCP](/cli/mcp#steelengine-as-an-mcp-client-registry) and
...
- `install.nodeManager`: node installer preference for `metadata.steelengine.install`
...
- Loaded from package or bundle directories under `~/.steelengine/extensions` and `<workspace>/.steelengine/extensions`, plus files or directories listed in `plugins.load.paths`.
...
- Discovery accepts native SteelEngine plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native SteelEngine plugin schema when available).
- Channel plugin account/runtime settings live under `channels.<id>` and should be described by the owning plugin's manifest `channelConfigs` metadata, not by a central SteelEngine option registry.
...
It does not enable Codex plugins for SteelEngine provider runs, ACP
...
- `plugins.entries.codex.config.codexPlugins.allow_destructive_actions`:
  ...
  to decline them, `"auto"` to route Codex-required approvals through SteelEngine
...
be installed and enabled, and their owned apps must be accessible; SteelEngine
...
- Enabled Claude bundle plugins can also contribute embedded SteelEngine defaults from `settings.json`; SteelEngine applies those as sanitized agent settings, not as raw SteelEngine config patches.
...
`commitments` controls inferred follow-up memory: SteelEngine can detect check-ins from conversation turns and deliver them through heartbeat runs.
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want SteelEngine to discover `/json/version`; use WS(S)
...
- If an externally managed CDP service is reachable through loopback, set that
  profile's `attachOnly: true`; otherwise SteelEngine treats the loopback port as a
...
- `existing-session` profiles can set `cdpUrl` when Chrome is already running
  ...
  mode SteelEngine passes the endpoint to Chrome MCP instead of using auto-connect;
...
- Local managed `steelengine` profiles auto-assign `cdpPort` and `cdpUrl`; set
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or STEELENGINE_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `STEELENGINE_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `tailscale.serviceName`: optional Tailscale Service name for Serve mode, such
  as `svc:steelengine`. When set, SteelEngine passes it to `tailscale serve
...
- `tailscale.preserveFunnel`: when `true` and `tailscale.mode = "serve"`, SteelEngine
...
- `terminal.shell`: optional shell executable. When unset, SteelEngine uses `$SHELL` on Unix and `%ComSpec%` on Windows.
...
- `gateway.push.apns.relay.baseUrl`: base HTTPS URL for the external APNs relay used after relay-backed iOS builds publish registrations to the gateway. Public App Store builds use the hosted SteelEngine relay. Custom relay URLs must match a deliberately separate iOS build/deployment path whose relay URL points at that relay.
- `gateway.push.apns.relay.timeoutMs`: gateway-to-relay send timeout in milliseconds. Defaults to `10000`.
- Relay-backed registrations are delegated to a specific gateway identity. The paired iOS app fetches `gateway.identity.get`, includes that identity in the relay registration, and forwards a registration-scoped send grant to the gateway. Another gateway cannot reuse that stored registration.
- `STEELENGINE_APNS_RELAY_BASE_URL` / `STEELENGINE_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `STEELENGINE_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
- `gateway.handshakeTimeoutMs`: pre-auth Gateway WebSocket handshake timeout in milliseconds. Default: `15000`. `STEELENGINE_HANDSHAKE_TIMEOUT_MS` takes precedence when set. Increase this on loaded or low-powered hosts where local clients can connect while startup warmup is still settling.
...
- `gateway.nodes.pairing.sshVerify`: SSH-verified auto-approval for first-time node device pairing (default: enabled). The gateway SSHes back to the pairing host (BatchMode, strict host keys) and approves only on an exact `steelengine node identity` device-key match. Same eligibility floor as `autoApproveCidrs`; probes are limited to private/CGNAT source addresses unless `cidrs` overrides them. Set `false` to disable, or `{ user, identity, timeoutMs, cidrs }` to tune. See [Node pairing](/gateway/pairing#ssh-verified-device-auto-approval-default).
...
```bash
STEELENGINE_CONFIG_PATH=~/.steelengine/a.json \
STEELENGINE_STATE_DIR=~/.steelengine-a \
steelengine gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.steelengine-dev` + port `19001`), `--profile <name>` (uses `~/.steelengine-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/steelengine/tls/server.crt",
      keyPath: "/etc/steelengine/tls/server.key",
      caPath: "/etc/steelengine/tls/ca-bundle.crt",
    },
  },
}
```
...
Cloud workers are opt-in. If `cloudWorkers` is absent, or `profiles` is empty, SteelEngine accepts no new worker creation. Durable records created earlier still reconcile and remain visible; the existing gateway/node projection is unchanged.
...
The bundled `crabbox` provider provisions an SSH-capable lease through the local Crabbox CLI. The inner `settings.provider` selects the Crabbox backend; it is separate from the outer SteelEngine provider id.
...
- `settings.ttl` and `settings.idleTimeout` (required): positive Go duration strings passed to `--ttl` and `--idle-timeout`. These provider-side failsafes are distinct from SteelEngine's stored `lifetime` policy below.
- `settings.binary`: optional absolute Crabbox executable path. Without it, SteelEngine checks the sibling Crabbox checkout, then executable entries on `PATH`, and finally invokes `crabbox` so a missing CLI remains a visible provider error.

Unknown settings are rejected. Crabbox credentials and backend-specific account configuration remain owned by Crabbox; do not place them in `settings`. SteelEngine invokes only the local CLI and makes no provider network calls from this plugin. Provisioning always passes `--keep=true`; SteelEngine owns the external lifecycle and destroys the lease with `crabbox stop`.

<Note>
  SteelEngine resolves Crabbox's lease-local `sshKey` path through the provider-owned secret resolver and pins the authoritative `sshHostKey` returned by `crabbox inspect --json`. AWS admission also requires `providerMetadata.instanceProfileAttached`. Install Crabbox 0.38.1 or newer for this closed inspection contract.
</Note>
...
This foundation installs and verifies the gateway build and provides tunnel start/stop lifecycle, but it does not launch the general SteelEngine CLI. The self-contained worker entry and loop land in the next cloud-worker milestone.
...
<Warning>
  ...
  Its operator must supply the expected `hostKey`; SteelEngine will not learn or accept a key from the first connection.
  Destroying its lease only releases SteelEngine's logical record; it does not stop or clean the host.
</Warning>
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.steelengine/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-steelengine-token: <token>`.
Query-string hook tokens are rejected.

Validation and safety notes:

- `hooks.enabled=true` requires a non-empty `hooks.token`.
- `hooks.token` should be distinct from active Gateway shared-secret auth (`gateway.auth.token` / `STEELENGINE_GATEWAY_TOKEN` or `gateway.auth.password` / `STEELENGINE_GATEWAY_PASSWORD`); startup logs a non-fatal security warning when it detects reuse.
- `steelengine security audit` flags hook/Gateway auth reuse as a critical finding, including Gateway password auth supplied only at audit time (`--auth password --password <password>`). Run `steelengine doctor --fix` to rotate a persisted reused `hooks.token`, then update external hook senders to use the new hook token.
...
- `transform` can point to a JS/TS module returning a hook action.
  ...
  - Keep `hooks.transformsDir` under `~/.steelengine/hooks/transforms`; workspace skill directories are rejected. If `steelengine doctor` reports this path as invalid, move the transform module into the hooks transforms directory or remove `hooks.transformsDir`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `STEELENGINE_SKIP_GMAIL_WATCHER=1` to disable.
  ...
  plugins: {
    entries: {
      canvas: {
        config: {
          host: {
            root: "~/.steelengine/workspace/canvas",
            liveReload: true,
            // enabled: false, // or STEELENGINE_SKIP_CANVAS_HOST=1
          },
        },
      },
    },
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__steelengine__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__steelengine__/a2ui/`
...
- Also serves A2UI at `/__steelengine__/a2ui/`.
...
- Hostname defaults to the system hostname when it is a valid DNS label, falling back to `steelengine`. Override with `STEELENGINE_MDNS_HOSTNAME`.
- `STEELENGINE_DISABLE_BONJOUR=1` disables mDNS advertising outright, overriding `discovery.mdns.mode`.
...
Writes a unicast DNS-SD zone under `~/.steelengine/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `steelengine dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.steelengine/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${STEELENGINE_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `steelengine.json` credential paths.
...
- Legacy flat `auth-profiles.json` maps such as `{ "provider": { "apiKey": "..." } }` are not a runtime format; `steelengine doctor --fix` rewrites them to canonical `provider:default` API-key profiles with a `.legacy-flat.*.bak` backup.
...
- Legacy OAuth imports from `~/.steelengine/credentials/oauth.json`.
...
[`steelengine audit`](/cli/audit) or the
...
- Default log file: `/tmp/steelengine/steelengine-YYYY-MM-DD.log`.
- Set `logging.file` for a stable path.
- `consoleLevel` bumps to `debug` when `--verbose`.
- `maxFileBytes`: maximum active log file size in bytes before rotation (positive integer; default: `104857600` = 100 MB). SteelEngine keeps up to five numbered archives beside the active file.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.steelengine/logs/cache-trace.jsonl",
...
- `stuckSessionAbortMs`: no-progress age threshold in ms before eligible stalled active work may be abort-drained for recovery. When unset, SteelEngine uses the safer extended embedded-run window of at least 5 minutes and 3x `stuckSessionWarnMs`.
...
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`: environment toggle for latest experimental GenAI inference span shape, including `{gen_ai.operation.name} {gen_ai.request.model}` span names, `CLIENT` span kind, and `gen_ai.provider.name` instead of legacy `gen_ai.system`. By default spans keep `steelengine.model.call` and `gen_ai.system` for compatibility; GenAI metrics use bounded semantic attributes.
- `STEELENGINE_OTEL_PRELOADED=1`: environment toggle for hosts that already registered a global OpenTelemetry SDK. SteelEngine then skips plugin-owned SDK startup/shutdown while keeping diagnostic listeners active.
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$STEELENGINE_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one SteelEngine.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `STEELENGINE_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `steelengine doctor --fix` can strip unknown keys).
...
- `webhook`: deprecated legacy fallback webhook URL (http/https) used by `steelengine doctor --fix` to migrate stored jobs that still have `notify: true`; runtime delivery uses per-job `delivery.mode="webhook"` plus `delivery.to`, or `delivery.completionDestination` when preserving announce delivery.
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `steelengine.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary. Set `STEELENGINE_INCLUDE_ROOTS` (absolute paths) to allow additional roots outside the config directory.
...
- SteelEngine-owned writes that change only one top-level section backed by a single-file include write through to that included file. For example, `plugins install` updates `plugins: { $include: "./plugins.json5" }` in `plugins.json5` and leaves `steelengine.json` intact.
- Root includes, include arrays, and includes with sibling overrides are read-only for SteelEngine-owned writes; those writes fail closed instead of flattening the config.
