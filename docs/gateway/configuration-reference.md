...
summary: "Complete reference for every Recall config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.recall/recall.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Recall uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.recall/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `recall doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `recall doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["recall-dm"] },
      guilds: {
        "123456789012345678": {
...
- Recall additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `recall plugins install @recall/mattermost`.
...
- `commands.callbackUrl` must resolve to the Recall gateway endpoint and be reachable from the Mattermost server.
...
Recall spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `recall channels add` (or channel onboarding) while still on a single-account top-level channel config, Recall moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `recall doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@recall", "recall"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@recall"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `recall.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Recall auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/recall" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Recall assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Recall aborts it. Default: `900`.
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.recall/sandboxes",
        docker: {
          ...
          apparmorProfile: "recall-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "recall-sandbox-browser:bookworm-slim",
          network: "recall-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Recall materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.recall/sandboxes`
...
In `remote` mode, host-local edits made outside Recall are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `recall.json`.
noVNC observer access uses VNC auth by default and Recall emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `recall-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `RECALL_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `RECALL_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `RECALL_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        agentDir: "~/.recall/agents/main/agent",
...
For `type: "acp"` entries, Recall resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  session: {
    ...
    store: "~/.recall/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Recall starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.recall/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Recall treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:recall`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.recall/attachments/<uuid>/` with a `.manifest.json`.
...
Recall uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.recall/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `RECALL_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Recall forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `recall onboard --auth-choice opencode-zen` or `recall onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `recall onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `recall onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `recall onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `recall onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `recall onboard --auth-choice minimax-api`.
...
- Loaded from `~/.recall/extensions`, `<workspace>/.recall/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Recall plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Recall plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Recall applies those as sanitized agent settings, not as raw Recall config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `recall plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      recall: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or RECALL_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `RECALL_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `RECALL_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `RECALL_APNS_RELAY_BASE_URL` / `RECALL_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `RECALL_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
RECALL_CONFIG_PATH=~/.recall/a.json \
RECALL_STATE_DIR=~/.recall-a \
recall gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.recall-dev` + port `19001`), `--profile <name>` (uses `~/.recall-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.recall/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-recall-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `RECALL_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.recall/workspace/canvas",
    liveReload: true,
    // enabled: false, // or RECALL_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__recall__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__recall__/a2ui/`
...
- Also serves A2UI at `/__recall__/a2ui/`.
...
- Hostname defaults to `recall`. Override with `RECALL_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.recall/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `recall dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.recall/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${RECALL_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `recall.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.recall/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/recall-vault-resolver",
...
- Legacy OAuth imports from `~/.recall/credentials/oauth.json`.
...
- Default log file: `/tmp/recall/recall-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Recall.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `RECALL_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `recall doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `recall.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
