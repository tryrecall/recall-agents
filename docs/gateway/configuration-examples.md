---
summary: "Schema-accurate configuration examples for common SteelEngine setups"
read_when:
  - Learning how to configure SteelEngine
...
Save to `~/.steelengine/steelengine.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.steelengine/agents/main/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/steelengine" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.steelengine/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
