---
summary: "Schema-accurate configuration examples for common Recall setups"
read_when:
  - Learning how to configure Recall
...
Save to `~/.tryrecall/recall-agents.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.recall/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/recall" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.recall/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
