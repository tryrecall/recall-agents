...
Recall reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.recall/recall.json`.
...
<Tabs>
  ...
  <Tab title="Direct edit">
    Edit `~/.recall/recall.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
The Gateway watches `~/.recall/recall.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `recall config set` for single keys.
    </Warning>
