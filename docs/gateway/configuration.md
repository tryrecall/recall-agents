...
read_when:
  - Setting up Recall for the first time
  - Looking for common configuration patterns
  - Navigating to specific config sections
title: "Configuration"
---

Recall reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.tryrecall/recall-agents.json`.
The active config path must be a regular file. Symlinked `recall.json`
layouts are unsupported for Recall-owned writes; an atomic write may replace
the path instead of preserving the symlink. If you keep config outside the
default state directory, point `RECALL_CONFIG_PATH` directly at the real file.

If the file is missing, Recall uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `recall onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    recall onboard       # full onboarding flow
    recall configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.tryrecall/recall-agents.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Recall only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`recall config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`recall doctor`, `recall logs`, `recall health`, `recall status`)
- Run `recall doctor` to see exact issues
- Run `recall doctor --fix` (or `--yes`) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically. If `recall.json`
...
Run `recall doctor --fix` (or `--yes`) to repair prefixed/clobbered config or
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Use `recall config set agents.defaults.models '<json>' --strict-json --merge` to add allowlist entries without removing existing models. Plain replacements that would remove entries are rejected unless you pass `--replace`.
  ...
  <Accordion title="Tune gateway WebSocket handshake timeout">
    ...
    - `RECALL_HANDSHAKE_TIMEOUT_MS` still takes precedence for one-off service or shell overrides.
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `recall.json`.
    ...
    - `RECALL_APNS_RELAY_BASE_URL` and `RECALL_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `RECALL_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-recall-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **Recall-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      Recall updates that included file and leaves `recall.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for Recall-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `recall.json`. To share a tree across machines or users, set
      `RECALL_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
The Gateway watches `~/.tryrecall/recall-agents.json` and applies changes automatically - no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits without rewriting `recall.json`. Recall-owned config
...
config`, inspect the config, run `recall config validate`, then run `recall
...
When you edit a source file that is referenced through `$include`, Recall plans
...
```bash
recall gateway call config.get --params '{}'  # capture payload.hash
...
Recall reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.recall/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Recall runs your login shell and imports only the missing keys:
...
Env var equivalent: `RECALL_LOAD_SHELL_ENV=1`
</Accordion>
