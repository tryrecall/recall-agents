...
`recall doctor` is the repair + migration tool for Recall. It fixes stale config/state, checks health, and provides actionable repair steps.
...
```bash
cat ~/.tryrecall/recall-agents.json
```

## Read-only lint mode

`recall doctor --lint` is the automation-friendly sibling of
`recall doctor --fix`. Both use doctor health checks, but their posture is
different:

| Mode                     | Prompts   | Writes config/state     | Output                 | Use it for                      |
| ------------------------ | --------- | ----------------------- | ---------------------- | ------------------------------- |
| `recall doctor`        | yes       | no                      | friendly health report | a human checking status         |
| `recall doctor --fix`  | sometimes | yes, with repair policy | friendly repair log    | applying approved repairs       |
| `recall doctor --lint` | no        | no                      | structured findings    | CI, preflight, and review gates |
...
```bash
recall doctor --lint
recall doctor --lint --severity-min warning
recall doctor --lint --json
recall doctor --lint --only core/doctor/gateway-config --json
```
...
<AccordionGroup>
  ...
  <Accordion title="State and integrity">
    ...
    - Extra workspace dir detection (`~/recall`).
  ...
  <Accordion title="Gateway, services, and supervisors">
    ...
    - Channel-specific permission checks live under `recall channels capabilities`; for example, Discord voice channel permissions are audited with `recall channels capabilities --channel discord --target channel:<channel-id>`.
...
The Control UI Dreams scene includes **Backfill**, **Reset**, and **Clear Grounded** actions for the grounded dreaming workflow. These actions use gateway doctor-style RPC methods, but they are **not** part of `recall doctor` CLI repair/migration.
...
```bash
recall memory rem-backfill --path ./memory --stage-short-term
```
...
<AccordionGroup>
  ...
  <Accordion title="2. Legacy config key migrations">
    When the config contains deprecated keys, other commands refuse to run and ask you to run `recall doctor`.

    Doctor will:

    - Explain which legacy keys were found.
    - Show the migration it applied.
    - Rewrite `~/.tryrecall/recall-agents.json` with the updated schema.

    Gateway startup refuses legacy config formats and asks you to run `recall doctor --fix`; it does not rewrite `recall.json` on startup. Cron job store migrations are also handled by `recall doctor --fix`.
  ...
  <Accordion title="2f. Codex route repair">
    Doctor checks for legacy `openai-codex/*` model refs. Native Codex harness routing uses canonical `openai/*` model refs; OpenAI agent turns go through the Codex app-server harness instead of the Recall PI OpenAI path.
  ...
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    - Sessions store + transcripts:
      - from `~/.recall/sessions/` to `~/.recall/agents/<agentId>/sessions/`
    - Agent dir:
      - from `~/.recall/agent/` to `~/.recall/agents/<agentId>/agent/`
    - WhatsApp auth state (Baileys):
      - from legacy `~/.recall/credentials/*.json` (except `oauth.json`)
      - to `~/.recall/credentials/whatsapp/<accountId>/...` (default account id: `default`)

    These migrations are best-effort and idempotent; doctor will emit warnings when it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates the legacy sessions + agent dir on startup so history/auth/models land in the per-agent path without a manual doctor run. WhatsApp auth is intentionally only migrated via `recall doctor`. Talk provider/provider-map normalization now compares by structural equality, so key-order-only diffs no longer trigger repeat no-op `doctor --fix` changes.
  ...
  <Accordion title="3b. Legacy cron store migrations">
    Doctor also checks the cron job store (`~/.recall/cron/jobs.json` by default, or `cron.store` when overridden) for old job shapes that the scheduler still accepts for compatibility.
    ...
    On Linux, doctor also warns when the user's crontab still invokes legacy `~/.recall/bin/ensure-whatsapp.sh`. That host-local script is not maintained by current Recall and can write false `Gateway inactive` messages to `~/.recall/logs/whatsapp-health.log` when cron cannot reach the systemd user bus. Remove the stale crontab entry with `crontab -e`; use `recall channels status --probe`, `recall doctor`, and `recall gateway status` for current health checks.

  </Accordion>
  <Accordion title="3c. Session lock cleanup">
    Doctor scans every agent session directory for stale write-lock files — files left behind when a session exited abnormally. For each lock file found it reports: the path, PID, whether the PID is still alive, lock age, and whether it is considered stale (dead PID, older than 30 minutes, or a live PID that can be proven to belong to a non-Recall process). In `--fix` / `--repair` mode it removes stale lock files automatically; otherwise it prints a note and instructs you to rerun with `--fix`.
  </Accordion>
  <Accordion title="3d. Session transcript branch repair">
    Doctor scans agent session JSONL files for the duplicated branch shape created by the 2026.4.24 prompt transcript rewrite bug: an abandoned user turn with Recall internal runtime context plus an active sibling containing the same visible user prompt. In `--fix` / `--repair` mode, doctor backs up each affected file next to the original and rewrites the transcript to the active branch so gateway history and memory readers no longer see duplicate turns.
  </Accordion>
  <Accordion title="4. State integrity checks (session persistence, routing, and safety)">
    ...
    - **Multiple state dirs**: warns when multiple `~/.recall` folders exist across home directories or when `RECALL_STATE_DIR` points elsewhere (history can split between installs).
    - **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run it on the remote host (the state lives there).
    - **Config file permissions**: warns if `~/.tryrecall/recall-agents.json` is group/world readable and offers to tighten to `600`.
  ...
  <Accordion title="5. Model auth health (OAuth expiry)">
    ...
    When an OAuth refresh fails permanently (for example `refresh_token_reused`, `invalid_grant`, or a provider telling you to sign in again), doctor reports that re-auth is required and prints the exact `recall models auth login --provider ...` command to run.
  ...
  <Accordion title="7b. Plugin install cleanup">
    Doctor removes legacy Recall-generated plugin dependency staging state in `recall doctor --fix` / `recall doctor --repair` mode. This covers stale generated dependency roots, old install-stage directories, package-local debris from earlier bundled-plugin dependency repair code, and orphaned or recovered managed npm copies of bundled `@recall/*` plugins that can shadow the current bundled manifest. Doctor also relinks the host `recall` package into managed npm plugins that declare `peerDependencies.recall`, so package-local runtime imports such as `recall/plugin-sdk/*` keep resolving after updates or npm repairs.

    Doctor can also reinstall missing downloadable plugins when config references them but the local plugin registry cannot find them. Examples include material `plugins.entries`, configured channel/provider/search settings, and configured agent runtimes. During package updates, doctor avoids running package-manager plugin repair while the core package is being swapped; run `recall doctor --fix` again after the update if a configured plugin still needs recovery. Gateway startup and config reload do not run package managers; plugin installs remain explicit doctor/install/update work.

  </Accordion>
  <Accordion title="8. Gateway service migrations and cleanup hints">
    Doctor detects legacy gateway services (launchd/systemd/schtasks) and offers to remove them and install the Recall service using the current gateway port. It can also scan for extra gateway-like services and print cleanup hints. Profile-named Recall gateway services are considered first-class and are not flagged as "extra."

    On Linux, if the user-level gateway service is missing but a system-level Recall gateway service exists, doctor does not install a second user-level service automatically. Inspect with `recall gateway status --deep` or `recall doctor --deep`, then remove the duplicate or set `RECALL_SERVICE_REPAIR_POLICY=external` when a system supervisor owns the gateway lifecycle.

  </Accordion>
  <Accordion title="8b. Startup Matrix migration">
    When a Matrix channel account has a pending or actionable legacy state migration, doctor (in `--fix` / `--repair` mode) creates a pre-migration snapshot and then runs the best-effort migration steps: legacy Matrix state migration and legacy encrypted-state preparation. Both steps are non-fatal; errors are logged and startup continues. In read-only mode (`recall doctor` without `--fix`) this check is skipped entirely.
  </Accordion>
  <Accordion title="8c. Device pairing and auth drift">
    ...
    - inspect pending requests with `recall devices list`
    - approve the exact request with `recall devices approve <requestId>`
    - rotate a fresh token with `recall devices rotate --device <deviceId> --role <role>`
    - remove and re-approve a stale record with `recall devices remove <deviceId>`
  ...
  <Accordion title="11. Workspace status (skills, plugins, and legacy dirs)">
    ...
    - **Legacy workspace dirs**: warns when `~/recall` or other legacy workspace directories exist alongside the current workspace.
  ...
  <Accordion title="11c. Shell completion">
    ...
    Run `recall completion --write-state` to regenerate the cache manually.
  ...
  <Accordion title="12. Gateway auth checks (local token)">
    ...
    - `recall doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
  ...
  <Accordion title="12b. Read-only SecretRef-aware repairs">
    ...
    - `recall doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
  ...
  <Accordion title="13b. Memory search readiness">
    ...
    Use `recall memory status --deep` to verify embedding readiness at runtime.
  ...
  <Accordion title="15. Supervisor config audit + repair">
    ...
    - `recall doctor` prompts before rewriting supervisor config.
    - `recall doctor --yes` accepts the default repair prompts.
    - `recall doctor --fix` applies recommended fixes without prompts (`--repair` is an alias).
    - `recall doctor --fix --force` overwrites custom supervisor configs.
    ...
    - Doctor service repairs refuse to rewrite, stop, or restart a gateway service from an older Recall binary when the config was last written by a newer version. See [Gateway troubleshooting](/gateway/troubleshooting#split-brain-installs-and-newer-config-guard).
    - You can always force a full rewrite via `recall gateway install --force`.
