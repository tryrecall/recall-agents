...
`steelengine doctor` is the repair and migration tool for SteelEngine. It fixes stale config/state, checks health, and provides actionable repair steps.
...
```bash
cat ~/.steelengine/steelengine.json
```

## Read-only lint mode

`steelengine doctor --lint` is the automation-friendly sibling of
`steelengine doctor --fix`. They share the same Doctor rule registry, but they do
not select or act on rules in the same way:

| Mode                     | Prompts   | Writes config/state     | Output                 | Use it for                      |
| ------------------------ | --------- | ----------------------- | ---------------------- | ------------------------------- |
| `steelengine doctor`        | yes       | no                      | friendly health report | a human checking status         |
| `steelengine doctor --fix`  | sometimes | yes, with repair policy | friendly repair log    | applying approved repairs       |
| `steelengine doctor --lint` | no        | no                      | structured findings    | CI, preflight, and review gates |
...
- `--json`, `--severity-min`, `--all`, `--only`, and `--skip` require `--lint`; plain `steelengine doctor` and `--fix` runs reject them.
...
<AccordionGroup>
  <Accordion title="Health, UI, and updates">
    ...
    - Problem-only skill and plugin notes; healthy inventory stays in `steelengine skills check` and `steelengine plugins list`.
  ...
  <Accordion title="Gateway, services, and supervisors">
    ...
    - Channel-specific permission checks live under `steelengine channels capabilities`; for example, Discord voice channel permissions are audited with `steelengine channels capabilities --channel discord --target channel:<channel-id>`.
...
The Control UI Dreams scene includes **Backfill**, **Reset**, and **Clear Grounded** actions for the grounded dreaming workflow. These use gateway doctor-style RPC methods but are **not** part of `steelengine doctor` CLI repair/migration.
...
```bash
steelengine memory rem-backfill --path ./memory --stage-short-term
```
...
<AccordionGroup>
  ...
  <Accordion title="2. Legacy config key migrations">
    When the config contains a deprecated key with an active migration, other commands refuse to run and ask you to run `steelengine doctor`. Doctor explains which legacy keys were found, shows the migration it applied, and rewrites `~/.steelengine/steelengine.json` with the updated schema. Gateway startup refuses legacy config formats and asks you to run `steelengine doctor --fix`; it does not rewrite `steelengine.json` on startup. Cron job store migrations are also handled by `steelengine doctor --fix`.
    ...
    | `agents.defaults/list[].systemPromptOverride`                                                    | removed (SteelEngine owns the generated system prompt)                        |
    ...
    <Note>
      ...
      the Voice Call plugin itself on every config load, not by `steelengine
      doctor`. The plugin also logs a startup warning pointing at `steelengine
      doctor --fix`, but doctor does not currently rewrite
      `steelengine.json` for these keys; the plugin's own normalization is what
  ...
  <Accordion title="2b. OpenCode provider overrides">
    If you have added `models.providers.opencode`, `opencode-zen`, or `opencode-go` manually, it overrides the built-in OpenCode catalog from `steelengine/plugin-sdk/llm`. That can force models onto the wrong API or zero out costs. Doctor warns so you can remove the override and restore per-model API routing + costs.
  </Accordion>
  ...
  <Accordion title="2g. Session route cleanup">
    ...
    `steelengine doctor --fix` can clear auto-created stale state such as `modelOverrideSource: "auto"` model pins, runtime model metadata, pinned harness ids, CLI session bindings, and auto auth-profile overrides when their owning route is no longer configured. Explicit user or legacy session model choices are reported for manual review and left untouched; switch them with `/model ...`, `/new`, or reset the session when that route is no longer intended.

  </Accordion>
  <Accordion title="3. Legacy state migrations (disk layout)">
    Doctor can migrate older on-disk layouts into the current structure:

    - Sessions store + transcripts: from `~/.steelengine/sessions/` to `~/.steelengine/agents/<agentId>/sessions/`
    - Agent dir: from `~/.steelengine/agent/` to `~/.steelengine/agents/<agentId>/agent/`
    - WhatsApp auth state (Baileys): from legacy `~/.steelengine/credentials/*.json` (except `oauth.json`) to `~/.steelengine/credentials/whatsapp/<accountId>/...` (default account id: `default`)

    These migrations are best-effort and idempotent; doctor emits warnings when it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates the legacy sessions + agent dir on startup so history/auth/models land in the per-agent path without a manual doctor run. WhatsApp auth is intentionally only migrated via `steelengine doctor`. Talk provider/provider-map normalization compares by structural equality, so key-order-only diffs no longer trigger repeat no-op `doctor --fix` changes.
  ...
  <Accordion title="3b. Legacy cron store migrations">
    Doctor also checks the cron job store (`~/.steelengine/cron/jobs.json` by default, or `cron.store` when overridden) for old job shapes that the scheduler still accepts for compatibility.
    ...
    On Linux, doctor also warns when the user's crontab still invokes legacy `~/.steelengine/bin/ensure-whatsapp.sh`. That host-local script is not maintained by current SteelEngine and can write false `Gateway inactive` messages to `~/.steelengine/logs/whatsapp-health.log` when cron cannot reach the systemd user bus. Remove the stale crontab entry with `crontab -e`; use `steelengine channels status --probe`, `steelengine doctor`, and `steelengine gateway status` for current health checks.

  </Accordion>
  <Accordion title="3c. Session lock cleanup">
    Doctor scans every agent session directory for stale write-lock files left behind when a session exited abnormally. For each lock file found it reports: the path, PID, whether the PID is still alive, lock age, and whether it is considered stale (dead PID, malformed owner metadata, older than 30 minutes, or a live PID proven to belong to a non-SteelEngine process). In `--fix` / `--repair` mode it removes locks with dead, orphaned, recycled, malformed-old, or non-SteelEngine owners automatically. Old locks still owned by a live SteelEngine process are reported but left in place so doctor does not cut off an active transcript writer.
  </Accordion>
  <Accordion title="3d. Session transcript branch repair">
    Doctor scans agent session JSONL files for the duplicated branch shape created by the 2026.4.24 prompt transcript rewrite bug: an abandoned user turn with SteelEngine internal runtime context plus an active sibling containing the same visible user prompt. In `--fix` / `--repair` mode, doctor backs up each affected file next to the original and rewrites the transcript to the active branch so gateway history and memory readers no longer see duplicate turns.
  </Accordion>
  <Accordion title="4. State integrity checks (session persistence, routing, and safety)">
    ...
    - **Multiple state dirs**: warns when multiple `~/.steelengine` folders exist across home directories, or when `STEELENGINE_STATE_DIR` points elsewhere (history can split between installs).
    - **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run it on the remote host (the state lives there).
    - **Config file permissions**: warns if `~/.steelengine/steelengine.json` is group/world readable and offers to tighten to `600`.
  ...
  <Accordion title="5. Model auth health (OAuth expiry)">
    ...
    When an OAuth refresh fails permanently (for example `refresh_token_reused`, `invalid_grant`, or a provider telling you to sign in again), doctor reports that re-auth is required and prints the exact `steelengine models auth login --provider ...` command to run.
    ...
    Legacy Codex OAuth profiles whose tokens live in macOS Keychain (older onboarding before the file-based sidecar layout) are repaired only by doctor. Run `steelengine doctor --fix` once from an interactive terminal to migrate Keychain-backed legacy tokens inline into `auth-profiles.json`; after that, embedded turns (Telegram, cron, sub-agent dispatch) resolve them as canonical OpenAI OAuth profiles.
  ...
  <Accordion title="7b. Plugin install cleanup">
    Doctor removes legacy SteelEngine-generated plugin dependency staging state in `steelengine doctor --fix` / `steelengine doctor --repair` mode: stale generated dependency roots, old install-stage directories, package-local debris from earlier bundled-plugin dependency repair code, and orphaned or recovered managed npm copies of bundled `@steelengine/*` plugins that can shadow the current bundled manifest. Doctor also relinks the host `steelengine` package into managed npm plugins that declare `peerDependencies.steelengine`, so package-local runtime imports such as `steelengine/plugin-sdk/*` keep resolving after updates or npm repairs.

    Doctor can also reinstall missing downloadable plugins when config references them but the local plugin registry cannot find them (material `plugins.entries`, configured channel/provider/search settings, configured agent runtimes). During package updates, doctor avoids reinstalling plugin packages while the core package is being swapped; run `steelengine doctor --fix` again after the update if a configured plugin still needs recovery. Outside the container image startup exception below, gateway startup and config reload do not run package repair; plugin installs remain explicit doctor/install/update work.

    Containerized gateway startup has a narrow upgrade exception: when `steelengine gateway run` starts on a new SteelEngine version, it runs safe state migrations and the existing post-core plugin convergence before readiness, then records a per-version checkpoint. This startup pass can clean stale bundled-plugin records, repair local plugin links, reinstall configured plugin packages when the convergence path requires it, and check active plugin payloads. If startup cannot repair safely, run the same image once with `steelengine doctor --fix` against the same mounted state/config before restarting the container normally.

  </Accordion>
  <Accordion title="8. Gateway service migrations and cleanup hints">
    Doctor detects legacy gateway services (launchd/systemd/schtasks) and offers to remove them and install the SteelEngine service using the current gateway port. It can also scan for extra gateway-like services and print cleanup hints. Profile-named SteelEngine gateway services are considered first-class and are not flagged as "extra."

    On Linux, if the user-level gateway service is missing but a system-level SteelEngine gateway service exists, doctor does not install a second user-level service automatically. Inspect with `steelengine gateway status --deep` or `steelengine doctor --deep`, then remove the duplicate or set `STEELENGINE_SERVICE_REPAIR_POLICY=external` when a system supervisor owns the gateway lifecycle.

  </Accordion>
  <Accordion title="8b. Startup Matrix migration">
    When a Matrix channel account has a pending or actionable legacy state migration, doctor (in `--fix` / `--repair` mode) creates a pre-migration snapshot and then runs the best-effort migration steps: legacy Matrix state migration and legacy encrypted-state preparation. Both steps are non-fatal; errors are logged and startup continues. In read-only mode (`steelengine doctor` without `--fix`) this check is skipped entirely.
  </Accordion>
  <Accordion title="8c. Device pairing and auth drift">
    ...
    - inspect pending requests with `steelengine devices list`
    - approve the exact request with `steelengine devices approve <requestId>`
    - rotate a fresh token with `steelengine devices rotate --device <deviceId> --role <role>`
    - remove and re-approve a stale record with `steelengine devices remove <deviceId>`
  ...
  <Accordion title="9. Security warnings">
    Doctor emits a Security note only when it finds a warning, such as a provider open to DMs without an allowlist or a dangerously configured policy. Use `steelengine security audit` for the full security inventory.
  </Accordion>
  ...
  <Accordion title="11. Workspace status (skills, plugins, and TaskFlows)">
    ...
    - **Skills**: lists allowed but unusable skill names; use `steelengine skills check` for requirement details and full counts.
    - **Plugins**: reports only errored plugin IDs; use `steelengine plugins list` for loaded, imported, disabled, and bundle-plugin inventory.
  ...
  <Accordion title="11c. Shell completion">
    ...
    - If the shell profile uses a slow dynamic completion pattern (`source <(steelengine completion ...)`), doctor upgrades it to the faster cached file variant.
  ...
  <Accordion title="12. Gateway auth checks (local token)">
    ...
    - `steelengine doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
  ...
  <Accordion title="12b. Read-only SecretRef-aware repairs">
    ...
    - `steelengine doctor --fix` uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
  ...
  <Accordion title="13b. Memory search readiness">
    ...
    Use `steelengine memory status --deep` to verify embedding readiness at runtime.
  ...
  <Accordion title="15. Supervisor config audit + repair">
    ...
    - `steelengine doctor` prompts before rewriting supervisor config.
    - `steelengine doctor --yes` accepts the default repair prompts.
    - `steelengine doctor --fix` applies recommended fixes without prompts (`--repair` is an alias).
    - `steelengine doctor --fix --force` overwrites custom supervisor configs.
    - `STEELENGINE_SERVICE_REPAIR_POLICY=external` keeps doctor read-only for gateway service lifecycle. It still reports service health and runs non-service repairs, but skips service install/start/restart/bootstrap, supervisor config rewrites, and legacy service cleanup because an external supervisor owns that lifecycle.
    ...
    - Doctor service repairs refuse to rewrite, stop, or restart a gateway service from an older SteelEngine binary when the config was last written by a newer version. See [Gateway troubleshooting](/gateway/troubleshooting#split-brain-installs-and-newer-config-guard).
    - You can always force a full rewrite via `steelengine gateway install --force`.
