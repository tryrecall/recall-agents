...
steelengine nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
runs `steelengine node identity --json` there, and approves only when the remote
...
- The gateway process user (or `sshVerify.user`) can SSH to the node host
  ...
  already trusted.
- `steelengine` resolves on the remote `PATH` for non-interactive `sh -lc`.
...
- Only records whose latest approval was same-host local (`silent`) are
  ...
  `steelengine nodes remove` for those.
...
changes (for example display name or client platform hints), SteelEngine treats
...
database under the Gateway state directory (default `~/.steelengine`):

- `~/.steelengine/state/steelengine.sqlite` (paired devices with device auth,
  approved node surfaces, pending surface requests, pending device pairing
  requests, and bootstrap tokens)

If you override `STEELENGINE_STATE_DIR`, the database moves with it. Gateways
...
- Rotating a device token uses `steelengine devices rotate` /
  `device.token.rotate`.
