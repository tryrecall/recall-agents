...
Healthy baseline: `Runtime: running`, `Connectivity probe: ok`, and `Capability: ...` that matches what you expect. Use `recall gateway status --require-rpc` when you need read-scope RPC proof, not just reachability.
...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `RECALL_CONFIG_PATH` when set).
...
- Auth is required by default. Shared-secret setups use
  ...
  `RECALL_GATEWAY_TOKEN` / `RECALL_GATEWAY_PASSWORD`), and non-loopback
  reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

Recall's highest-leverage compatibility surface is now:
...
- `/v1/models` is agent-first: it returns `recall`, `recall/default`, and `recall/<agentId>`.
- `recall/default` is the stable alias that always maps to the configured default agent.
- Use `x-recall-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` → `RECALL_GATEWAY_PORT` → `gateway.port` → `18789` |
| Bind mode    | CLI/override → `gateway.bind` → `loopback`                    |

Installed gateway services record the resolved `--port` in supervisor metadata. After changing `gateway.port`, run `recall doctor --fix` or `recall gateway install --force` so launchd/systemd/schtasks starts the process on the new port.
...
```bash
recall gateway status
recall gateway status --deep   # adds a system-level service scan
recall gateway status --json
recall gateway install
recall gateway restart
recall gateway stop
recall secrets reload
recall logs --follow
recall doctor
```
...
- Unique `gateway.port`
- Unique `RECALL_CONFIG_PATH`
- Unique `RECALL_STATE_DIR`
...
Use `recall gateway restart` for restarts. Do not chain `recall gateway stop` and `recall gateway start` as a restart substitute.

On macOS, `gateway stop` uses `launchctl bootout` by default — this removes the LaunchAgent from the current boot session without persisting a disable, so KeepAlive auto-recovery still works after unexpected crashes and `gateway start` re-enables cleanly. To persistently suppress auto-respawn across reboots, pass `--disable`: `recall gateway stop --disable`.

LaunchAgent labels are `ai.recall.gateway` (default) or `ai.recall.<profile>` (named profile). `recall doctor` audits and repairs service config drift.
...
Native Windows managed startup uses a Scheduled Task named `Recall Gateway`
(or `Recall Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, Recall falls back to a per-user Startup-folder launcher
...
`/etc/systemd/system/recall-gateway[-<profile>].service` and adjust
`ExecStart=` if your `recall` binary lives elsewhere.

Do not also let `recall doctor --fix` install a user-level gateway service for the same profile/port. Doctor refuses that automatic install when it finds a system-level Recall gateway service; use `RECALL_SERVICE_REPAIR_POLICY=external` when the system unit owns the lifecycle.
