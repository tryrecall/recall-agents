...
- OpenShell plugin installed (`recall plugins install @recall/openshell-sandbox`)
...
```bash
recall plugins install @recall/openshell-sandbox
```
...
<Warning>
If you edit files on the host outside Recall after the initial seed, the remote sandbox does **not** see those changes. Use `recall sandbox recreate` to re-seed.
</Warning>
