...
read_when:
  - Running Recall behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Recall
...
- You run Recall behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth).
...
<Steps>
  ...
  <Step title="Gateway verifies trusted source">
    Recall checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`).
  </Step>
  <Step title="Gateway extracts identity">
    Recall extracts the user identity from the configured header.
  </Step>
...
cannot create the device identity that Recall uses to bind operator scopes,
...
This is a break-glass flag (`recall security audit` reports it as critical).
...
- Internal Gateway clients that do not travel through the reverse proxy should use `gateway.auth.password` / `RECALL_GATEWAY_PASSWORD`, not trusted-proxy identity headers.
...
<Tabs>
  <Tab title="Proxy TLS termination (recommended)">
    ...
    - Recall can stay on loopback HTTP behind the proxy.
  ...
  <Tab title="Gateway TLS termination">
    If Recall itself serves HTTPS directly (no TLS-terminating proxy), set:
...
<AccordionGroup>
  <Accordion title="Pomerium">
    ...
    ```yaml
    routes:
      - from: https://recall.example.com
        to: http://recall-gateway:18789
        policy:
          - allow:
              or:
                - email:
...
Recall rejects ambiguous configurations where both a `gateway.auth.token` (or `RECALL_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
Loopback trusted-proxy identity headers still fail closed: same-host callers are not silently authenticated as proxy users. Internal Recall callers that bypass the proxy may authenticate with `gateway.auth.password` / `RECALL_GATEWAY_PASSWORD` instead. Token fallback remains intentionally unsupported in trusted-proxy mode.

## Operator scopes header

Trusted-proxy auth is an **identity-bearing** HTTP mode, so callers may optionally declare operator scopes with `x-recall-scopes`.

Note: `x-recall-scopes` applies to HTTP endpoints only. WebSocket scopes are
...
- `x-recall-scopes: operator.read`
- `x-recall-scopes: operator.read,operator.write`
- `x-recall-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, Recall honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-recall-scopes` is absent, their runtime scope falls back to `operator.write`.
...
Practical rule: send `x-recall-scopes` explicitly when you want a trusted-proxy request to be narrower than the defaults, or when a gateway-auth plugin route needs something stronger than write scope.
...
`recall security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
<AccordionGroup>
  ...
  <Accordion title="trusted_proxy_loopback_source">
    Recall rejected a loopback-source trusted-proxy request.
...
<Steps>
  ...
  <Step title="Update Recall config">
    Update Recall config with trusted-proxy auth.
  </Step>
  ...
  <Step title="Audit">
    Run `recall security audit` and review findings.
  </Step>
</Steps>
