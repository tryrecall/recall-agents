...
3. Recall checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Recall extracts the user identity from the configured header
...
3. Update Recall config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `recall security audit` and review findings
