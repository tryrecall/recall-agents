---
summary: "How SteelEngine handles local file access safely, and why the optional fs-safe Python helper is off by default"
read_when:
  - Changing file access, archive extraction, workspace storage, or plugin filesystem helpers
title: "Secure file operations"
---

SteelEngine uses [`@openclaw/fs-safe`](https://github.com/steelengine/fs-safe) for security-sensitive local file operations: root-bounded reads/writes, atomic replacement, archive extraction, temp workspaces, JSON state, and secret-file handling.

It is a **library guardrail** for trusted SteelEngine code that receives untrusted path names, not a sandbox. Host filesystem permissions, OS users, containers, and the agent/tool policy still define the real blast radius.

## Default: no Python helper

SteelEngine sets the fs-safe POSIX Python helper to **off** by default:
...
SteelEngine only changes the _default_. An explicit setting always wins:

```bash
# Default SteelEngine behavior: Node-only fs-safe fallbacks.
STEELENGINE_FS_SAFE_PYTHON_MODE=off

# Opt into the helper when available, falling back if unavailable.
STEELENGINE_FS_SAFE_PYTHON_MODE=auto

# Fail closed if the helper cannot start.
STEELENGINE_FS_SAFE_PYTHON_MODE=require

# Optional explicit interpreter path.
STEELENGINE_FS_SAFE_PYTHON=/usr/bin/python3
```
...
With the helper off, SteelEngine still gets fs-safe's Node-only guardrails:
...
This covers SteelEngine's normal threat model: trusted gateway code handling untrusted model/plugin/channel path input inside a single trusted operator boundary.
...
That narrows same-UID race windows where another process swaps a parent directory between validation and mutation — defense in depth on hosts where untrusted local processes can modify the same directories SteelEngine operates in.
...
- Plugin-facing file access should go through `steelengine/plugin-sdk/*` helpers, not raw `fs`, when a path comes from a message, model output, config, or plugin input.
- Core code should use the fs-safe wrappers under `src/infra/*` so SteelEngine's process policy applies consistently.
- Archive extraction should use the fs-safe archive helpers with explicit size, entry-count, link, and destination limits.
- Secrets should use SteelEngine secret helpers or fs-safe secret/private-state helpers; do not hand-roll mode checks around `fs.writeFile`.
