---
summary: "Pre-flight and rollback checklist before exposing an Recall Gateway beyond loopback"
...
- Backup location for `~/.tryrecall/recall-agents.json` and credentials.
...
```bash
recall gateway probe --url ws://127.0.0.1:18789 --token "$RECALL_GATEWAY_TOKEN"
```
...
Run `recall security audit --deep` after proxy changes. Trusted-proxy findings
...
1. Re-run `recall security audit --deep`.
...
5. Re-run `recall security audit --deep`.
