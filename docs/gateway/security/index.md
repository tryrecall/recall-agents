...
> Recall is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
- Recall can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want Recall, use one VPS/host per user.
...
Recall is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/recall/recall/blob/main/SECURITY.md#out-of-scope).
...
| ------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                           | critical      | Other users/processes can modify full Recall state                                 | filesystem perms on `~/.recall`                                                                    | yes      |
| `fs.config.perms_writable`                                    | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.recall/recall.json`                                                      | yes      |
...
| `tools.exec.auto_allow_skills_enabled`                        | warn          | Exec approvals trust skill bins implicitly                                           | `~/.recall/exec-approvals.json`                                                                    | no       |
...
`recall security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in Recall config
schema:
...
- If you install plugins (`recall plugins install <package>`), treat it like running untrusted code:
  - The install path is `~/.recall/extensions/<pluginId>/` (or `$RECALL_STATE_DIR/extensions/<pluginId>/`).
  - Recall uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
- `~/.recall/recall.json`: `600` (user read/write only)
...
- Canvas host: `/__recall__/canvas/` and `/__recall__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_recall-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `RECALL_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
paths, set `RECALL_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
...
Recall’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
...
1. Check Gateway logs: `/tmp/recall/recall-YYYY-MM-DD.log` (or `logging.file`).
