...
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to SteelEngine
...
External apps talk to SteelEngine through the Gateway protocol: WebSocket
...
<Warning>
  There is no public npm client package yet. Do not add SteelEngine client package
...
<Note>
  This page is for code outside the SteelEngine process. Plugin code that runs
  inside SteelEngine should use documented `steelengine/plugin-sdk/*` subpaths instead.
</Note>
...
| [`steelengine agent`](/cli/agent)          | Ready  | One-shot script integration when shelling out to the CLI is enough.                           |
| [`steelengine message`](/cli/message)      | Ready  | Sending messages or channel actions from scripts.                                             |
...
4. Pin the SteelEngine version you test against.
5. Recheck the RPC reference when upgrading SteelEngine.
...
fail-closed, and the host must not freeze or snapshot it. SteelEngine retries the
...
Preparation is refuse-only: SteelEngine closes new root/session/command admission,
...
<Tip>
  For host wake scheduling, keep the SteelEngine-facing part in an in-process
...
Use Gateway RPC when code lives outside SteelEngine:
...
Use the Plugin SDK when code runs inside SteelEngine:
...
External apps should not import `steelengine/plugin-sdk/*`; those subpaths are for
plugins loaded by SteelEngine.
