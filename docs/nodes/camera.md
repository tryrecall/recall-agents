...
Recall supports **camera capture** for agent workflows:
...
```bash
recall nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
recall nodes camera snap --node <id> --facing front
recall nodes camera clip --node <id> --duration 3000
recall nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`recall.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `recall` CLI to invoke camera commands on the macOS node.

Examples:

```bash
recall nodes camera list --node <id>            # list camera ids
recall nodes camera snap --node <id>            # prints MEDIA:<path>
recall nodes camera snap --node <id> --max-width 1280
recall nodes camera snap --node <id> --delay-ms 2000
recall nodes camera snap --node <id> --device-id <id>
recall nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
recall nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
recall nodes camera clip --node <id> --device-id <id>
recall nodes camera clip --node <id> --no-audio
```

Notes:

- `recall nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
recall nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
