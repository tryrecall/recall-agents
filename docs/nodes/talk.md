...
## Config (`~/.steelengine/steelengine.json`)
...
| `consultThinkingLevel`                   | unset                                      | Thinking level override for the agent run behind realtime `steelengine_agent_consult` calls.                                                                                                                                                                                  |
| `consultFastMode`                        | unset                                      | Fast-mode override for realtime `steelengine_agent_consult` calls.                                                                                                                                                                                                            |
...
| `realtime.consultRouting`                | -                                          | `provider-direct` preserves the provider's direct reply when it skips `steelengine_agent_consult`; `force-agent-consult` routes finalized user transcripts through SteelEngine instead.                                                                                          |
| `realtime.instructions`                  | -                                          | Appends provider-facing system instructions to SteelEngine's built-in realtime prompt (voice style/tone); the default `steelengine_agent_consult` guidance stays.                                                                                                                |
...
- macOS local MLX playback uses the bundled `steelengine-mlx-tts` helper when present, or an executable on `PATH`. Set `STEELENGINE_MLX_TTS_BIN` to point at a custom helper binary during development.
