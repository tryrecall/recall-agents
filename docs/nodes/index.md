...
WS server as one node (so `steelengine nodes …` works against this Mac). The app
adds native Canvas, camera, screen, notification, and computer-control commands
to the same node-host command surface used by `steelengine node run`. Do not start a
...
Pending pairing requests expire 5 minutes after the device's last retry — a device that keeps reconnecting keeps its one pending request (and `requestId`) alive instead of minting a new prompt every few minutes; see [Node pairing](/gateway/pairing) for the full request/approve lifecycle. If a node retries with changed auth details (role/scopes/public key), the prior pending request is superseded and a new `requestId` is created — clients get a `device.pair.resolved` event for the superseded request, and you should re-run `steelengine devices list` before approving.
...
| Approvals    | Enforced on the node host via `~/.steelengine/exec-approvals.json`. |
...
- For direct shell/runtime file executions, SteelEngine also best-effort binds one concrete local file operand and denies the run if that file changes before execution.
- If SteelEngine cannot identify exactly one concrete local file for an interpreter/runtime command, approval-backed execution is denied instead of pretending full runtime coverage. Use sandboxing, separate hosts, or an explicit trusted allowlist/full workflow for broader interpreter semantics.
...
```bash
steelengine node run --host <gateway-host> --port 18789 --display-name "Build Node"
```
...
export STEELENGINE_GATEWAY_TOKEN="<gateway-token>"
steelengine node run --host 127.0.0.1 --port 18790 --display-name "Build Node"
```

Notes:

- `steelengine node run` supports token or password auth.
- Env vars are preferred: `STEELENGINE_GATEWAY_TOKEN` / `STEELENGINE_GATEWAY_PASSWORD`.
...
- Node-host auth resolution only honors `STEELENGINE_GATEWAY_*` env vars.

### Start a node host (service)

```bash
steelengine node install --host <gateway-host> --port 18789 --display-name "Build Node"
steelengine node start
...
If the node retries with changed auth details, re-run `steelengine devices list` and approve the current `requestId`.

Naming options:

- `--display-name` on `steelengine node run` / `steelengine node install` (persists in the shared `node_host_config` SQLite row alongside the client instance ID and Gateway connection metadata).
- `steelengine nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).

### Node-hosted MCP servers

Configure MCP servers in `steelengine.json` on the node machine, not on the
Gateway:
...
older SteelEngine version may request a one-time command-surface upgrade after the
...
`steelengine node run` or `steelengine node restart` to apply node MCP config changes;
...
Install skills under the node machine's active SteelEngine skills directory,
`~/.steelengine/skills` by default. `STEELENGINE_HOME`, `STEELENGINE_STATE_DIR`, and
`STEELENGINE_CONFIG_PATH` move that active profile. `STEELENGINE_STATE_DIR` takes
precedence for skills; otherwise, `skills/` is beside the path printed by
`steelengine config file`. The headless node host publishes valid `SKILL.md` files
...
change. Restart `steelengine node run` or `steelengine node restart` after changing
...
its active SteelEngine state directory, so relative paths resolve on the node rather
...
- `~/.steelengine/state/steelengine.sqlite` (`node_host_config`): the client instance ID, display name, and Gateway connection metadata.
- `~/.steelengine/identity/device.json`: the signed device keypair and derived cryptographic device ID.
- `~/.steelengine/identity/device-auth.json`: paired device auth tokens keyed by cryptographic device ID and role.
...
```bash
steelengine approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
steelengine approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

Approvals live on the node host at `~/.steelengine/exec-approvals.json`.
...
```bash
steelengine config set tools.exec.host node
steelengine config set tools.exec.security allowlist
steelengine config set tools.exec.node "<id-or-name>"
```
...
The relay does not provide the full SteelEngine harness continuation and archive ownership contracts. **Continue** and **Archive** are therefore unavailable for remote rows. On the Gateway computer, stored and idle
...
This is a takeover of the native session; unlike SteelEngine adoption, it does not
...
Chat composer: SteelEngine imports bounded visible history, resumes with
...
row on that node becomes continuable: SteelEngine imports bounded history, binds
...
```bash
steelengine nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
steelengine nodes canvas snapshot --node <idOrNameOrIp> --format png
steelengine nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas controls

```bash
steelengine nodes canvas present --node <idOrNameOrIp> --target https://example.com
steelengine nodes canvas hide --node <idOrNameOrIp>
steelengine nodes canvas navigate https://example.com --node <idOrNameOrIp>
steelengine nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
steelengine nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
steelengine nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
steelengine nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
steelengine nodes camera list --node <idOrNameOrIp>
steelengine nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
steelengine nodes camera snap --node <idOrNameOrIp> --facing front
steelengine nodes camera snap --node <idOrNameOrIp> --device-id <id> --max-width 1200 --quality 0.9 --delay-ms 2000
```

Video clips (`mp4`):

```bash
steelengine nodes camera clip --node <idOrNameOrIp> --duration 10s
steelengine nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
steelengine nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
steelengine nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
steelengine nodes location get --node <idOrNameOrIp>
steelengine nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
For read-only SMS search, opt in explicitly in `steelengine.json`:
...
```bash
steelengine nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from SteelEngine"}'
```
...
```bash
steelengine nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
steelengine nodes invoke --node <idOrNameOrIp> --command device.apps --params '{"limit":10}'
steelengine nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
steelengine nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
...
```bash
steelengine nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
steelengine nodes invoke --node <idOrNameOrIp> --command system.which --params '{"bins":["git"]}'
```
...
- On headless node host, `system.run` is gated by exec approvals (`~/.steelengine/exec-approvals.json`); on macOS specifically, see the exec-host routing env vars under [Headless node host](#headless-node-host-cross-platform) below.
...
```bash
steelengine config set tools.exec.node "node-id-or-name"
```

Per-agent override:

```bash
steelengine config get agents.list
steelengine config set 'agents.list[0].tools.exec.node' "node-id-or-name"
```

Unset to allow any node:

```bash
steelengine config unset tools.exec.node
steelengine config unset 'agents.list[0].tools.exec.node'
```
...
SteelEngine can run a **headless node host** (no UI) that connects to the Gateway WebSocket and exposes `system.run` / `system.which`. This is useful on Linux/Windows or for running a minimal node alongside a server.

Start it:

```bash
steelengine node run --host <gateway-host> --port 18789
```
...
- Exec approvals are enforced locally via `~/.steelengine/exec-approvals.json` (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host executes `system.run` locally by default. Set `STEELENGINE_NODE_EXEC_HOST=app` to route `system.run` through the companion app exec host; add `STEELENGINE_NODE_EXEC_FALLBACK=0` to require the app host and fail closed if it is unavailable.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `steelengine nodes …` works against this Mac).
