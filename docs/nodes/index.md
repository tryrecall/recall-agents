...
`recall nodes …` works against this Mac). In remote gateway mode, browser
automation is handled by the CLI node host (`recall node run` or the
...
`recall devices list` before approving.
...
- `node.pair.*` (CLI: `recall nodes pending/approve/reject/remove/rename`) is a separate gateway-owned
  node pairing store; it does **not** gate the WS `connect` handshake.
- `recall nodes remove --node <id|name|ip>` deletes stale entries from that
...
- **Approvals**: enforced on the node host via `~/.recall/exec-approvals.json`.

Approval note:

- Approval-backed node runs bind exact request context.
- For direct shell/runtime file executions, Recall also best-effort binds one concrete local
...
```bash
recall node run --host <gateway-host> --port 18789 --display-name "Build Node"
```
...
export RECALL_GATEWAY_TOKEN="<gateway-token>"
recall node run --host 127.0.0.1 --port 18790 --display-name "Build Node"
```

Notes:

- `recall node run` supports token or password auth.
- Env vars are preferred: `RECALL_GATEWAY_TOKEN` / `RECALL_GATEWAY_PASSWORD`.
...
```bash
recall node install --host <gateway-host> --port 18789 --display-name "Build Node"
recall node start
recall node restart
```
...
If the node retries with changed auth details, re-run `recall devices list`
and approve the current `requestId`.

Naming options:

- `--display-name` on `recall node run` / `recall node install` (persists in `~/.recall/node.json` on the node).
- `recall nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
```bash
recall approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
recall approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

Approvals live on the node host at `~/.recall/exec-approvals.json`.
...
```bash
recall config set tools.exec.host node
recall config set tools.exec.security allowlist
recall config set tools.exec.node "<id-or-name>"
```
...
```bash
recall nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
recall nodes canvas snapshot --node <idOrNameOrIp> --format png
recall nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas controls

```bash
recall nodes canvas present --node <idOrNameOrIp> --target https://example.com
recall nodes canvas hide --node <idOrNameOrIp>
recall nodes canvas navigate https://example.com --node <idOrNameOrIp>
recall nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
recall nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
recall nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
recall nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
recall nodes camera list --node <idOrNameOrIp>
recall nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
recall nodes camera snap --node <idOrNameOrIp> --facing front
```

Video clips (`mp4`):

```bash
recall nodes camera clip --node <idOrNameOrIp> --duration 10s
recall nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
recall nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
recall nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
recall nodes location get --node <idOrNameOrIp>
recall nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
```bash
recall nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from Recall"}'
```
...
```bash
recall nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
recall nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
recall nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
...
```bash
recall nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
recall nodes invoke --node <idOrNameOrIp> --command system.which --params '{"name":"git"}'
```
...
- On headless node host, `system.run` is gated by exec approvals (`~/.recall/exec-approvals.json`).
...
```bash
recall config set tools.exec.node "node-id-or-name"
```
...
```bash
recall config unset tools.exec.node
recall config unset 'agents.list[0].tools.exec.node'
```
...
Recall can run a **headless node host** (no UI) that connects to the Gateway
...
- The node host stores its node id, token, display name, and gateway connection info in `~/.recall/node.json`.
- Exec approvals are enforced locally via `~/.recall/exec-approvals.json`
  (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host executes `system.run` locally by default. Set
  `RECALL_NODE_EXEC_HOST=app` to route `system.run` through the companion app exec host; add
  `RECALL_NODE_EXEC_FALLBACK=0` to require the app host and fail closed if it is unavailable.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `recall nodes …` works against this Mac).
