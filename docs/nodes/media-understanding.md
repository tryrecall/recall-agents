...
Vendor-specific media behavior is registered by vendor plugins, while Recall
...
- If `<capability>.enabled: true` but no models are configured, Recall tries the
  **active reply model** when its provider supports the capability.

### Auto-detect media understanding (default)

If `tools.media.<capability>.enabled` is **not** set to `false` and you haven’t
configured models, Recall auto-detects in this order and **stops at the first
working option**:
...
When provider-based **audio** and **video** media understanding is enabled, Recall
...
If the proxy value is malformed, Recall logs a warning and falls back to direct
fetch.

## Capabilities (optional)

If you set `capabilities`, the entry only runs for those media types. For shared
lists, Recall can infer defaults:
