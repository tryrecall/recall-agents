...
```bash
steelengine nodes describe --node <idOrNameOrIp>
steelengine nodes canvas snapshot --node <idOrNameOrIp>
steelengine logs --follow
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`steelengine approvals get --node ...`), not in the gateway pairing record.
...
steelengine nodes status
steelengine approvals get --node <idOrNameOrIp>
steelengine approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
| `ACCESSIBILITY_REQUIRED`               | Grant Accessibility to the current SteelEngine app bundle in macOS System Settings.                                                                                                        |
