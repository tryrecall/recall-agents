...
```bash
recall nodes describe --node <idOrNameOrIp>
recall nodes canvas snapshot --node <idOrNameOrIp>
recall logs --follow
```
...
recall nodes status
recall approvals get --node <idOrNameOrIp>
recall approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`recall approvals get --node ...`), not in the gateway pairing record.
