...
An owner is a ClawHub publisher handle, such as `@alice` or `@recall`.
...
`@recall/dronzer`, it can only be published as `@recall`. If you publish as
...
```text
Package scope "@recall" must match selected owner "@vintageayu".
Publish as "@recall" or rename this package to "@vintageayu/dronzer".
```
...
```sh
clawhub package transfer @opik/opik-recall --to opik
```
...
This protects org namespaces. A package named `@recall/dronzer` claims the
`@recall` namespace, so only publishers with access to the `@recall` owner
can publish it.
