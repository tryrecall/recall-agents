...
An owner is a ClawHub publisher handle, such as `@alice` or `@steelengine`.
...
`@steelengine/dronzer` can only be published as `@steelengine`. To publish as
...
```text
Package scope "@steelengine" must match selected owner "@vintageayu".
Publish as "@steelengine" or rename this package to "@vintageayu/dronzer".
```
...
```sh
clawhub package transfer @opik/opik-steelengine --to opik
```
...
`@steelengine/dronzer` claims the `@steelengine` namespace, so only publishers with
access to `@steelengine` can publish or transfer into it.
